...
bool ClassFileOracle::containsKnownAnnotation(UDATA knownAnnotationSet, UDATA knownAnnotation)
{
...
U_16 ClassFileOracle::LocalVariablesIterator::getGenericSignatureIndex()
{
    ...
    J9CfrLocalVariableTypeTableEntry* localVariableTypeTable
        = _localVariablesInfo[_index].localVariableTypeTable->localVariableTypeTable;
    if ((_localVariableTableIndex < _localVariablesInfo[_index].localVariableTypeTable->localVariableTypeTableLength)
        && (_localVariableTable[_localVariableTableIndex].index
               == localVariableTypeTable[_localVariableTableIndex].index)
        && (_localVariableTable[_localVariableTableIndex].startPC
               == localVariableTypeTable[_localVariableTableIndex].startPC)
        && (_localVariableTable[_localVariableTableIndex].length
               == localVariableTypeTable[_localVariableTableIndex].length)) {
    ...
    for (U_16 localVariableTypeTableIndex = 0;
        ...
        if ((_localVariableTable[_localVariableTableIndex].index
                == localVariableTypeTable[localVariableTypeTableIndex].index)
            && (_localVariableTable[_localVariableTableIndex].startPC
                   == localVariableTypeTable[localVariableTypeTableIndex].startPC)
            && (_localVariableTable[_localVariableTableIndex].length
                   == localVariableTypeTable[localVariableTypeTableIndex].length)) {
...
bool ClassFileOracle::LocalVariablesIterator::hasGenericSignature()
{
    Trc_BCU_Assert_NotEquals(NULL, _localVariableTable);
...
ClassFileOracle::ClassFileOracle(BufferManager* bufferManager, J9CfrClassFile* classFile,
    ConstantPoolMap* constantPoolMap, U_8* verifyExcludeAttribute, ROMClassCreationContext* context)
    : _buildResult(OK)
    ...
    _maxBranchCount(1)
    , /* This is required to support buffer size calculations for stackmap support code */
...
void ClassFileOracle::walkHeader()
{
...
void ClassFileOracle::walkFields()
{
     ...
     * will not be parsed, resulting in all subsequent valid fields to be noted as not having proper @Length annotation,
     * hence overwritting the good error message. Checking (OK == _buildResult) in for-loop condition achieves the
     * desired error handling behavior.
     */
    for (U_16 fieldIndex = 0; (OK == _buildResult) && (fieldIndex < fieldsCount); fieldIndex++) {
        ...
        for (U_16 attributeIndex = 0; (OK == _buildResult) && (attributeIndex < field->attributesCount);
             ++attributeIndex) {
            ...
            case CFR_ATTRIBUTE_RuntimeVisibleAnnotations: {
                J9CfrAttributeRuntimeVisibleAnnotations* attribAnnotations
                    = (J9CfrAttributeRuntimeVisibleAnnotations*)attrib;
                UDATA knownAnnotations = 0;
                if ((NULL != _context->javaVM())
                    && (J9_ARE_ALL_BITS_SET(
                           _context->javaVM()->extendedRuntimeFlags, J9_EXTENDED_RUNTIME_ALLOW_CONTENDED_FIELDS))
                    && (_context->isBootstrapLoader()
                           || (J9_ARE_ALL_BITS_SET(_context->javaVM()->extendedRuntimeFlags,
                                  J9_EXTENDED_RUNTIME_ALLOW_APPLICATION_CONTENDED_FIELDS)))) {
                ...
                if (0 == attribAnnotations->rawDataLength) {
                    UDATA foundAnnotations = walkAnnotations(
                        attribAnnotations->numberOfAnnotations, attribAnnotations->annotations, knownAnnotations);

                    if (containsKnownAnnotation(foundAnnotations, CONTENDED_ANNOTATION)
                        || containsKnownAnnotation(foundAnnotations, JAVA8_CONTENDED_ANNOTATION)) {
            ...
            case CFR_ATTRIBUTE_RuntimeVisibleTypeAnnotations: {
                J9CfrAttributeRuntimeVisibleTypeAnnotations* typeAnnotations
                    = (J9CfrAttributeRuntimeVisibleTypeAnnotations*)attrib;
                if (0
                    == typeAnnotations->rawDataLength) { /* rawDataLength non-zero in case of error in the attribute */
            ...
            case CFR_ATTRIBUTE_Deprecated:
                ...
                break;
            default:
                Trc_BCU_ClassFileOracle_walkFields_UnknownAttribute((U_32)attrib->tag,
                    (U_32)getUTF8Length(attrib->nameIndex), getUTF8Data(attrib->nameIndex), attrib->length);
                break;
            }
        }
    }
...
void ClassFileOracle::walkAttributes()
{
    ...
    for (U_16 attributeIndex = 0; attributeIndex < _classFile->attributesCount; attributeIndex++) {
        ...
        case CFR_ATTRIBUTE_RuntimeVisibleAnnotations: {
            ...
            if (0 == _annotationsAttribute->rawDataLength) {
                UDATA foundAnnotations = walkAnnotations(
                    _annotationsAttribute->numberOfAnnotations, _annotationsAttribute->annotations, knownAnnotations);
                if (containsKnownAnnotation(foundAnnotations, CONTENDED_ANNOTATION)
                    || containsKnownAnnotation(foundAnnotations, JAVA8_CONTENDED_ANNOTATION)) {
        ...
        case CFR_ATTRIBUTE_BootstrapMethods: {
            ...
            for (U_16 methodIndex = 0; methodIndex < _bootstrapMethodsAttribute->numberOfBootstrapMethods;
                 methodIndex++) {
                ...
                for (U_16 argumentIndex = 0; argumentIndex < bootstrapMethod->numberOfBootstrapArguments;
                     argumentIndex++) {
...
#endif /* J9VM_OPT_VALHALLA_NESTMATES */
        default:
            Trc_BCU_ClassFileOracle_walkAttributes_UnknownAttribute((U_32)attrib->tag,
                (U_32)getUTF8Length(attrib->nameIndex), getUTF8Data(attrib->nameIndex), attrib->length);
            break;
        }
        if (!_hasVerifyExcludeAttribute && (NULL != _verifyExcludeAttribute)) {
            U_8* found
                = (U_8*)strstr((const char*)_verifyExcludeAttribute, (const char*)getUTF8Data(attrib->nameIndex));
            if ((NULL != found) && ((found == _verifyExcludeAttribute) || (';' == (*(found - 1))))
                && (('\0' == found[getUTF8Length(attrib->nameIndex)])
                       || (';' == found[getUTF8Length(attrib->nameIndex)]))) {
...
void ClassFileOracle::walkInterfaces()
{
...
void ClassFileOracle::walkMethods()
{
    ...
    for (U_16 methodIndex = 0; (methodIndex < methodsCount) && (OK == _buildResult); ++methodIndex) {
        ...
        if (methodIsFinalize(methodIndex, 0 != (_methodsInfo[methodIndex].modifiers & J9AccForwarderMethod))) {
            ...
            /* If finalize() is empty, mark this class so it does not inherit CFR_ACC_FINALIZE_NEEDED from its
             * superclass */
...
void ClassFileOracle::walkMethodAttributes(U_16 methodIndex)
{
    ROMCLASS_VERBOSE_PHASE_HOT(_context, ClassFileMethodAttributesAnalysis);

    for (U_16 methodAttrIndex = 0;
         (methodAttrIndex < _classFile->methods[methodIndex].attributesCount) && (OK == _buildResult);
         ++methodAttrIndex) {
        ...
        case CFR_ATTRIBUTE_RuntimeVisibleParameterAnnotations: {
            J9CfrAttributeRuntimeVisibleParameterAnnotations* attribParameterAnnotations
                = (J9CfrAttributeRuntimeVisibleParameterAnnotations*)attrib;
            for (U_8 parameterAnnotationIndex = 0;
                ...
                walkAnnotations(
                    ...
                    attribParameterAnnotations->parameterAnnotations[parameterAnnotationIndex].annotations, 0);
            }
        ...
        case CFR_ATTRIBUTE_Deprecated:
            ...
            break;
        default:
            Trc_BCU_ClassFileOracle_walkMethods_UnknownAttribute((U_32)attrib->tag,
                (U_32)getUTF8Length(attrib->nameIndex), getUTF8Data(attrib->nameIndex), attrib->length);
            break;
        }
    }
...
ClassFileOracle::walkAnnotations(U_16 annotationsCount, J9CfrAnnotation* annotations, UDATA knownAnnotations)
{
    ...
    if (0 == knownAnnotations) {
        for (U_16 annotationsIndex = 0; (annotationsIndex < annotationsCount) && (OK == _buildResult);
             ++annotationsIndex) {
            ...
            for (U_16 elementValuePairsIndex = 0;
                ...
                markConstantAsUsedByAnnotation(
                    annotations[annotationsIndex].elementValuePairs[elementValuePairsIndex].elementNameIndex);
        ...
        for (U_16 annotationsIndex = 0; (annotationsIndex < annotationsCount) && (OK == _buildResult);
             ++annotationsIndex) {
            markConstantAsUsedByAnnotation(annotations[annotationsIndex].typeIndex);
            for (UDATA knownAnnotationIndex = 0; knownAnnotationIndex < KNOWN_ANNOTATION_COUNT;
                 ++knownAnnotationIndex) {
                if (containsKnownAnnotation(knownAnnotations, knownAnnotationIndex)
                    && isUTF8AtIndexEqualToString(annotations[annotationsIndex].typeIndex,
                           _knownAnnotations[knownAnnotationIndex].name,
                           _knownAnnotations[knownAnnotationIndex].size)) {
...
void ClassFileOracle::walkTypeAnnotations(U_16 annotationsCount, J9CfrTypeAnnotation* typeAnnotations)
{
...
void ClassFileOracle::walkAnnotationElement(J9CfrAnnotationElement* annotationElement)
{
    ...
    case 'S': /* fall thru */
    case 'Z': /* fall thru */
        /* Fall through - in Java 7, the annotation data is preserved in class file format, including referenced
...
void ClassFileOracle::computeSendSlotCount(U_16 methodIndex)
{
...
void ClassFileOracle::walkMethodThrownExceptions(U_16 methodIndex)
{
...
void ClassFileOracle::walkMethodCodeAttribute(U_16 methodIndex)
{
...
void ClassFileOracle::walkMethodMethodParametersAttribute(U_16 methodIndex)
{
    ...
    J9CfrAttributeMethodParameters* methodParametersAttribute
        = _classFile->methods[methodIndex].methodParametersAttribute;
    ...
    if (NULL != methodParametersAttribute) {
        for (U_8 methodParamsIndex = 0;
            ...
            if (J9_ARE_ANY_BITS_SET(
                    methodParametersAttribute->flags[methodParamsIndex], ~CFR_ATTRIBUTE_METHOD_PARAMETERS_MASK)) {
...
void ClassFileOracle::walkMethodCodeAttributeAttributes(U_16 methodIndex)
{
    ...
    for (U_16 attributeIndex = 0; (attributeIndex < attributesCount) && (OK == _buildResult); ++attributeIndex) {
        ...
        case CFR_ATTRIBUTE_StackMapTable: {
             ...
             *
             * Some frame types contain 1 or more TypeInfo entries. TypeInfo is a variable-length data type TODO add
             *doc?
             *
            ...
            _methodsInfo[methodIndex].stackMapFramesInfo
                = (StackMapFrameInfo*)_bufferManager->alloc(stackMap->numberOfEntries * sizeof(StackMapFrameInfo));
            if (NULL == _methodsInfo[methodIndex].stackMapFramesInfo) {
                Trc_BCU_ClassFileOracle_walkMethodCodeAttributeAttributes_OutOfMemory(
                    stackMap->numberOfEntries * sizeof(StackMapFrameInfo));
                _buildResult = OutOfMemory;
                break;
            }
            memset(
                _methodsInfo[methodIndex].stackMapFramesInfo, 0, stackMap->numberOfEntries * sizeof(StackMapFrameInfo));
            ...
            for (U_16 entryIndex = 0; entryIndex < entryCount; ++entryIndex) {
                ...
                if (CFR_STACKMAP_SAME_LOCALS_1_STACK > stackMapFramesInfo[entryIndex].frameType) { /* 0..63 */
                ...
                } else if (CFR_STACKMAP_SAME_LOCALS_1_STACK_END
                    > stackMapFramesInfo[entryIndex].frameType) { /* 64..127 */
                    /*
                     * SAME_LOCALS_1_STACK {
                ...
                } else if (CFR_STACKMAP_SAME_LOCALS_1_STACK_EXTENDED
                    > stackMapFramesInfo[entryIndex].frameType) { /* 128..246 */
                    /* Reserved frame types - no extra data */
                    Trc_BCU_Assert_ShouldNeverHappen();
                } else if (CFR_STACKMAP_SAME_LOCALS_1_STACK_EXTENDED
                    == stackMapFramesInfo[entryIndex].frameType) { /* 247 */
                    /*
                     * SAME_LOCALS_1_STACK_EXTENDED {
                     *		U_16 offsetDelta
                     *		TypeInfo stackItems[1]
                     ...
                     */
                    NEXT_U16(stackMapFramesInfo[entryIndex].offsetDelta, framePointer); /* Extract offsetDelta */
                    stackMapFramesInfo[entryIndex].localsCount
                        = stackMapFramesInfo[entryIndex].frameType - CFR_STACKMAP_APPEND_BASE;
                    ...
                    framePointer = walkStackMapSlots(framePointer, stackMapFramesInfo[entryIndex].localsCount);
                    NEXT_U16(
                        stackMapFramesInfo[entryIndex].stackItemsCount, framePointer); /* Extract stackItemsCount */
                    stackMapFramesInfo[entryIndex].stackItemsTypeInfo = framePointer;
                    framePointer = walkStackMapSlots(framePointer, stackMapFramesInfo[entryIndex].stackItemsCount);
                }
            }
        ...
        case CFR_ATTRIBUTE_LocalVariableTable:
            if (_context->shouldPreserveLocalVariablesInfo()) {
                /* There can be at most one entry per local variable, so we allocate the full table up-front and index
                 * by local variable index */
                if (NULL == _methodsInfo[methodIndex].localVariablesInfo) {
                    _methodsInfo[methodIndex].localVariablesInfo = (LocalVariableInfo*)_bufferManager->alloc(
                        codeAttribute->maxLocals * sizeof(LocalVariableInfo));
                    if (NULL == _methodsInfo[methodIndex].localVariablesInfo) {
                        Trc_BCU_ClassFileOracle_walkMethodCodeAttributeAttributes_OutOfMemory(
                            codeAttribute->maxLocals * sizeof(LocalVariableInfo));
                        _buildResult = OutOfMemory;
                    ...
                    memset(_methodsInfo[methodIndex].localVariablesInfo, 0,
                        codeAttribute->maxLocals * sizeof(LocalVariableInfo));
                }
                ...
                if (0 != localVariableTable->localVariableTableLength) {
                    for (U_16 localVariableTableIndex = 0;
                         localVariableTableIndex < localVariableTable->localVariableTableLength;
                        ...
                        if (codeAttribute->maxLocals <= index) {
                            Trc_BCU_ClassFileOracle_walkMethodCodeAttributeAttributes_LocalVariableTableIndexOutOfBounds(
                                index, codeAttribute->maxLocals,
                                (U_32)getUTF8Length(_classFile->methods[methodIndex].nameIndex),
                            ...
                            != _methodsInfo[methodIndex].localVariablesInfo[index].localVariableTable) {
                            Trc_BCU_ClassFileOracle_walkMethodCodeAttributeAttributes_DuplicateLocalVariableTable(
                                (U_32)getUTF8Length(_classFile->methods[methodIndex].nameIndex),
                                getUTF8Data(_classFile->methods[methodIndex].nameIndex), localVariableTable,
                                _methodsInfo[methodIndex].localVariablesInfo[index].localVariableTable);
                            _buildResult = GenericError;
                            break;
                        }
                        markConstantUTF8AsReferenced(
                            localVariableTable->localVariableTable[localVariableTableIndex].nameIndex);
                        markConstantUTF8AsReferenced(
                            localVariableTable->localVariableTable[localVariableTableIndex].descriptorIndex);
                    }
                }
            }
            break;
        case CFR_ATTRIBUTE_LocalVariableTypeTable:
            if (_context->shouldPreserveLocalVariablesInfo()) {
                ...
                J9CfrAttributeLocalVariableTypeTable* localVariableTypeTable
                    = (J9CfrAttributeLocalVariableTypeTable*)attribute;
                if (0 != localVariableTypeTable->localVariableTypeTableLength) {
                    for (U_16 localVariableTypeTableIndex = 0;
                         localVariableTypeTableIndex < localVariableTypeTable->localVariableTypeTableLength;
                         ++localVariableTypeTableIndex) {
                        ...
                        if (codeAttribute->maxLocals <= index) {
                            ...
                            _methodsInfo[methodIndex].localVariablesInfo[index].localVariableTypeTable
                                = localVariableTypeTable;
                        } else if (localVariableTypeTable
                            != _methodsInfo[methodIndex].localVariablesInfo[index].localVariableTypeTable) {
                            Trc_BCU_ClassFileOracle_walkMethodCodeAttributeAttributes_DuplicateLocalVariableTypeTable(
                                (U_32)getUTF8Length(_classFile->methods[methodIndex].nameIndex),
                                getUTF8Data(_classFile->methods[methodIndex].nameIndex), localVariableTypeTable,
                                _methodsInfo[methodIndex].localVariablesInfo[index].localVariableTypeTable);
                            _buildResult = GenericError;
                            break;
                        }
                        markConstantUTF8AsReferenced(
                            localVariableTypeTable->localVariableTypeTable[localVariableTypeTableIndex].signatureIndex);
                    }
                }
            }
            break;
        case CFR_ATTRIBUTE_RuntimeVisibleTypeAnnotations: {
            J9CfrAttributeRuntimeVisibleTypeAnnotations* typeAnnotations
                = (J9CfrAttributeRuntimeVisibleTypeAnnotations*)attribute;
            if (0 == typeAnnotations->rawDataLength) { /* rawDataLength non-zero in case of error in the attribute */
                walkTypeAnnotations(typeAnnotations->numberOfAnnotations, typeAnnotations->typeAnnotations);
            }
        ...
        default:
            Trc_BCU_ClassFileOracle_walkMethodCodeAttributeAttributes_UnknownAttribute((U_32)attribute->tag,
                (U_32)getUTF8Length(attribute->nameIndex), getUTF8Data(attribute->nameIndex), attribute->length);
            break;
        }
    }
...
int ClassFileOracle::compareLineNumbers(const void* left, const void* right)
{
...
void ClassFileOracle::compressLineNumberTable(U_16 methodIndex, U_32 lineNumbersCount)
{
    J9CfrAttributeCode* codeAttribute = _classFile->methods[methodIndex].codeAttribute;
    U_16 attributesCount = codeAttribute->attributesCount;
    ...
    if (NULL == lineNumbersInfoCompressed) {
        ...
        for (U_16 attributeIndex = 0; attributeIndex < attributesCount; ++attributeIndex) {
            ...
            if (CFR_ATTRIBUTE_LineNumberTable == attribute->tag) {
                J9CfrAttributeLineNumberTable* lineNumberTable = (J9CfrAttributeLineNumberTable*)attribute;
                if (!compressLineNumbers(lineNumberTable->lineNumberTable, lineNumberTable->lineNumberTableLength,
                        lastLineNumberTableEntry, &lineNumbersInfoCompressed)) {
                ...
                lastLineNumberTableEntry
                    = &lineNumberTable->lineNumberTable[lineNumberTable->lineNumberTableLength - 1];
            }
        }
...
void ClassFileOracle::sortAndCompressLineNumberTable(
    U_16 methodIndex, U_32 lineNumbersCount, U_8* lineNumbersInfoCompressedInitial)
{
    J9CfrAttributeCode* codeAttribute = _classFile->methods[methodIndex].codeAttribute;
    U_16 attributesCount = codeAttribute->attributesCount;
    MethodInfo* method = &(_methodsInfo[methodIndex]);
    ...
    J9CfrLineNumberTableEntry* lineNumbersInfo
        = (J9CfrLineNumberTableEntry*)(_bufferManager->alloc(lineNumbersInfoSize));
...
void ClassFileOracle::sortLineNumberTable(U_16 methodIndex, J9CfrLineNumberTableEntry* lineNumbersInfo)
{
    J9CfrAttributeCode* codeAttribute = _classFile->methods[methodIndex].codeAttribute;
    U_16 attributesCount = codeAttribute->attributesCount;
    ...
    for (U_16 attributeIndex = 0; attributeIndex < attributesCount; ++attributeIndex) {
        ...
        if (CFR_ATTRIBUTE_LineNumberTable == attribute->tag) {
            J9CfrAttributeLineNumberTable* lineNumberTable = (J9CfrAttributeLineNumberTable*)attribute;
            for (U_16 lineNumberTableIndex = 0; lineNumberTableIndex < lineNumberTable->lineNumberTableLength;
                 ++lineNumberTableIndex) {
                if (lineNumberTable->lineNumberTable[lineNumberTableIndex].startPC < lastPC) {
                    sorted = FALSE;
                }
                lastPC = lineNumbersInfo[lineNumbersIndex].startPC
                    = lineNumberTable->lineNumberTable[lineNumberTableIndex].startPC;
                lineNumbersInfo[lineNumbersIndex].lineNumber
                    = lineNumberTable->lineNumberTable[lineNumberTableIndex].lineNumber;
                ++lineNumbersIndex;
            }
        }
    }
    ...
    if (!sorted) {
        ...
        J9_SORT(
            lineNumbersInfo, lineNumbersIndex, sizeof(J9CfrLineNumberTableEntry), &ClassFileOracle::compareLineNumbers);
    }
}

void ClassFileOracle::walkMethodCodeAttributeCaughtExceptions(U_16 methodIndex)
{
    ...
    for (U_16 exceptionTableIndex = 0; exceptionTableIndex < codeAttribute->exceptionTableLength;
         ++exceptionTableIndex) {
...
void ClassFileOracle::addBytecodeFixupEntry(BytecodeFixupEntry* entry, U_32 codeIndex, U_16 cpIndex, U_8 type)
{
 ...
 */
static VMINLINE I_32 swapI32(U_8* code, U_32 codeIndex)
{
    I_32* dest = (I_32*)&code[codeIndex];
    I_32 value = I_32(U_32(code[codeIndex] << 24) | U_32(code[codeIndex + 1] << 16) | U_32(code[codeIndex + 2] << 8)
        | U_32(code[codeIndex + 3]));
    *dest = value;
    return value;
}
...
void ClassFileOracle::walkMethodCodeAttributeCode(U_16 methodIndex)
{
    ROMCLASS_VERBOSE_PHASE_HOT(_context, ClassFileMethodCodeAttributeCodeAnalysis);
    J9CfrAttributeCode* codeAttribute = _classFile->methods[methodIndex].codeAttribute;

    static const U_8 returnArgCharConversion[] = { 0, PARAM_INT, PARAM_INT, PARAM_DOUBLE, 0, PARAM_FLOAT, 0, 0,
        PARAM_INT, PARAM_LONG, 0, PARAM_OBJECT, 0, 0, 0, 0, 0, 0, PARAM_INT, 0, 0, PARAM_VOID, 0, 0, 0, PARAM_INT };
    ...
    for (U_32 codeIndex = 0; codeIndex < codeAttribute->codeLength;
         codeIndex += step) { /* NOTE codeIndex is modified below for CFR_BC_tableswitch and CFR_BC_lookupswitch */
...
U_8* ClassFileOracle::walkStackMapSlots(U_8* framePointer, U_16 typeInfoCount)
{
...
bool ClassFileOracle::methodIsFinalize(U_16 methodIndex, bool isForwarder)
{
...
#define FINALIZE_SIG "()V"
    if ((0 == (_classFile->methods[methodIndex].accessFlags & CFR_ACC_STATIC))
        && isUTF8AtIndexEqualToString(
               _classFile->methods[methodIndex].descriptorIndex, FINALIZE_SIG, sizeof(FINALIZE_SIG))
        && isUTF8AtIndexEqualToString(
               _classFile->methods[methodIndex].nameIndex, FINALIZE_NAME, sizeof(FINALIZE_NAME))) {
        return true;
    }
#undef FINALIZE_NAME
#undef FINALIZE_SIG
...
bool ClassFileOracle::methodIsEmpty(U_16 methodIndex)
{
    ROMCLASS_VERBOSE_PHASE_HOT(_context, MethodIsEmpty);
    if (0
        == (_classFile->methods[methodIndex].accessFlags
               & (CFR_ACC_ABSTRACT | CFR_ACC_NATIVE | CFR_ACC_SYNCHRONIZED))) {
...
bool ClassFileOracle::methodIsObjectConstructor(U_16 methodIndex)
{
    ...
    if ((0 == _classFile->superClass)
        && (0
               == (_classFile->methods[methodIndex].accessFlags
                      & (CFR_ACC_ABSTRACT | CFR_ACC_STATIC | CFR_ACC_PRIVATE | CFR_ACC_SYNCHRONIZED)))
 ...
 */
bool ClassFileOracle::methodIsClinit(U_16 methodIndex)
{
...
bool ClassFileOracle::methodIsForwarder(U_16 methodIndex)
{
    ROMCLASS_VERBOSE_PHASE_HOT(_context, MethodIsForwarder);
    if (_classFile->methods[methodIndex].accessFlags
        & (CFR_ACC_ABSTRACT | CFR_ACC_NATIVE | CFR_ACC_SYNCHRONIZED | CFR_ACC_STATIC)) {
        return false;
    }
...
#undef PARAM_U16
            ...
            if (!isUTF8AtIndexEqualToString(nas->slot1,
                    (const char*)getUTF8Data(_classFile->methods[methodIndex].nameIndex),
                    getUTF8Length(_classFile->methods[methodIndex].nameIndex) + 1)) {
                return false;
            }
            if (!isUTF8AtIndexEqualToString(nas->slot2,
                    (const char*)getUTF8Data(_classFile->methods[methodIndex].descriptorIndex),
                    getUTF8Length(_classFile->methods[methodIndex].descriptorIndex) + 1)) {
                return false;
            }
...
bool ClassFileOracle::methodIsGetter(U_16 methodIndex)
{
    ...
    if ((CFR_BC_aload_0 == sunBytecodes[0]) && (CFR_BC_getfield == sunBytecodes[1])
        && (CFR_BC_ireturn <= sunBytecodes[4]) && (sunBytecodes[4] <= CFR_BC_return)) {
        return true;
    }
...
bool ClassFileOracle::methodIsVirtual(U_16 methodIndex)
{
    ...
    if (0 == _classFile->superClass) { // TODO is this the same as J9ROMCLASS_SUPERCLASSNAME(romClass) == NULL???
        ...
        if (methodIsFinalInObject(getUTF8Length(_classFile->methods[methodIndex].nameIndex),
                getUTF8Data(_classFile->methods[methodIndex].nameIndex),
...
bool ClassFileOracle::methodIsNonStaticNonAbstract(U_16 methodIndex)
{
...
ClassFileOracle::shouldConvertInvokeVirtualToMethodHandleBytecodeForMethodRef(U_16 methodRefCPIndex)
{
    ...
    J9CfrConstantPoolInfo* targetClassName
        = &_classFile->constantPool[_classFile->constantPool[methodInfo->slot1].slot1];
...
bool ClassFileOracle::shouldConvertInvokeVirtualToInvokeSpecialForMethodRef(U_16 methodRefCPIndex)
{
    ...
    // TODO this hurts performance significantly - can we get away with not doing this? E.g. does it inflate vtables?
    // Does the JIT do "the right thing"?
    if (className->slot1 == targetClassName->slot1
        && strncmp((char*)className->bytes, (char*)targetClassName->bytes, className->slot1) == 0) {
        for (UDATA methodIndex = 0; methodIndex < _classFile->methodsCount; methodIndex++) {
            ...
            if (aName->slot1 == name->slot1 && aSig->slot1 == sig->slot1
...
void ClassFileOracle::markClassAsReferenced(U_16 classCPIndex)
{
...
void ClassFileOracle::markClassNameAsReferenced(U_16 classCPIndex)
{
    Trc_BCU_Assert_Equals(CFR_CONSTANT_Class, _classFile->constantPool[classCPIndex].tag);
...
void ClassFileOracle::markStringAsReferenced(U_16 cpIndex)
{
...
void ClassFileOracle::markMethodTypeAsReferenced(U_16 cpIndex)
{
...
void ClassFileOracle::markMethodHandleAsReferenced(U_16 cpIndex)
{
...
void ClassFileOracle::markNameAndDescriptorAsReferenced(U_16 nasCPIndex)
{
...
void ClassFileOracle::markFieldRefAsReferenced(U_16 cpIndex)
{
...
void ClassFileOracle::markMethodRefAsReferenced(U_16 cpIndex)
{
    Trc_BCU_Assert_True((CFR_CONSTANT_Methodref == _classFile->constantPool[cpIndex].tag)
        || (CFR_CONSTANT_InterfaceMethodref == _classFile->constantPool[cpIndex].tag));
 ...
 */
void ClassFileOracle::markMethodRefForMHInvocationAsReferenced(U_16 cpIndex) { markMethodRefAsReferenced(cpIndex); }

void ClassFileOracle::markConstantAsReferenced(U_16 cpIndex)
{
...
void ClassFileOracle::markConstantUTF8AsReferenced(U_16 cpIndex)
{
    if (0 != cpIndex) { /* Never, never, never mark constantPool[0] referenced */
        _constantPoolMap->markConstantUTF8AsReferenced(cpIndex);
    }
...
void ClassFileOracle::markConstantNameAndTypeAsReferenced(U_16 cpIndex)
{
    if (0 != cpIndex) { /* Never, never, never mark constantPool[0] referenced */
...
void ClassFileOracle::markConstantDynamicAsReferenced(U_16 cpIndex)
{
    markNameAndDescriptorAsReferenced(_classFile->constantPool[cpIndex].slot2);
    if (0 != cpIndex) { /* Never, never, never mark constantPool[0] referenced */
        _constantPoolMap->markConstantAsReferenced(cpIndex);
...
void ClassFileOracle::markConstantAsUsedByAnnotation(U_16 cpIndex)
{
...
void ClassFileOracle::markConstantAsUsedByLDC(U_8 cpIndex) { _constantPoolMap->markConstantAsUsedByLDC(cpIndex); }

void ClassFileOracle::markConstantAsUsedByLDC2W(U_16 cpIndex)
{
    _constantPoolMap->markConstantAsReferencedDoubleSlot(cpIndex);
}

void ClassFileOracle::markClassAsUsedByInstanceOf(U_16 classCPIndex)
{
    markClassAsReferenced(classCPIndex);
    _constantPoolMap->markClassAsUsedByInstanceOf(classCPIndex);
}

void ClassFileOracle::markClassAsUsedByCheckCast(U_16 classCPIndex)
{
    markClassAsReferenced(classCPIndex);
    _constantPoolMap->markClassAsUsedByCheckCast(classCPIndex);
}

void ClassFileOracle::markClassAsUsedByMultiANewArray(U_16 classCPIndex)
{
    markClassAsReferenced(classCPIndex);
    _constantPoolMap->markClassAsUsedByMultiANewArray(classCPIndex);
}

void ClassFileOracle::markClassAsUsedByANewArray(U_16 classCPIndex)
{
    markClassAsReferenced(classCPIndex);
    _constantPoolMap->markClassAsUsedByANewArray(classCPIndex);
}

void ClassFileOracle::markClassAsUsedByNew(U_16 classCPIndex)
{
    markClassAsReferenced(classCPIndex);
...
#if defined(J9VM_OPT_VALHALLA_VALUE_TYPES)
void ClassFileOracle::markClassAsUsedByDefaultValue(U_16 classCPIndex)
{
    markClassAsReferenced(classCPIndex);
    _constantPoolMap->markClassAsUsedByDefaultValue(classCPIndex);
}

void ClassFileOracle::markFieldRefAsUsedByWithField(U_16 fieldRefCPIndex)
{
...
void ClassFileOracle::markFieldRefAsUsedByGetStatic(U_16 fieldRefCPIndex)
{
    markFieldRefAsReferenced(fieldRefCPIndex);
    _constantPoolMap->markFieldRefAsUsedByGetStatic(fieldRefCPIndex);
}

void ClassFileOracle::markFieldRefAsUsedByPutStatic(U_16 fieldRefCPIndex)
{
    markFieldRefAsReferenced(fieldRefCPIndex);
    _constantPoolMap->markFieldRefAsUsedByPutStatic(fieldRefCPIndex);
}

void ClassFileOracle::markFieldRefAsUsedByGetField(U_16 fieldRefCPIndex)
{
    markFieldRefAsReferenced(fieldRefCPIndex);
    _constantPoolMap->markFieldRefAsUsedByGetField(fieldRefCPIndex);
}

void ClassFileOracle::markFieldRefAsUsedByPutField(U_16 fieldRefCPIndex)
{
    markFieldRefAsReferenced(fieldRefCPIndex);
    _constantPoolMap->markFieldRefAsUsedByPutField(fieldRefCPIndex);
}

void ClassFileOracle::markMethodRefAsUsedByInvokeVirtual(U_16 methodRefCPIndex)
{
    markMethodRefAsReferenced(methodRefCPIndex);
    _constantPoolMap->markMethodRefAsUsedByInvokeVirtual(methodRefCPIndex);
}

void ClassFileOracle::markMethodRefAsUsedByInvokeSpecial(U_16 methodRefCPIndex)
{
    markMethodRefAsReferenced(methodRefCPIndex);
    _constantPoolMap->markMethodRefAsUsedByInvokeSpecial(methodRefCPIndex);
}

void ClassFileOracle::markMethodRefAsUsedByInvokeStatic(U_16 methodRefCPIndex)
{
    markMethodRefAsReferenced(methodRefCPIndex);
    _constantPoolMap->markMethodRefAsUsedByInvokeStatic(methodRefCPIndex);
}

void ClassFileOracle::markMethodRefAsUsedByInvokeInterface(U_16 methodRefCPIndex)
{
    markMethodRefAsReferenced(methodRefCPIndex);
    _constantPoolMap->markMethodRefAsUsedByInvokeInterface(methodRefCPIndex);
}

void ClassFileOracle::markMethodRefAsUsedByInvokeHandle(U_16 methodRefCPIndex)
{
...
void ClassFileOracle::markMethodRefAsUsedByInvokeHandleGeneric(U_16 methodRefCPIndex)
{
    markMethodRefForMHInvocationAsReferenced(methodRefCPIndex);
    _constantPoolMap->markMethodRefAsUsedByInvokeHandleGeneric(methodRefCPIndex);
}

void ClassFileOracle::markInvokeDynamicInfoAsUsedByInvokeDynamic(U_16 cpIndex)
{
...
void ClassFileOracle::markConstantBasedOnCpType(U_16 cpIndex, bool assertNotDoubleOrLong)
{
