...
public:
    ...
    struct StackMapFrameVisitor {
        virtual void visitStackMapFrame(
            U_16 localsCount, U_16 stackItemsCount, U_16 offsetDelta, U_8 frameType, VerificationTypeInfo* typeInfo)
            = 0;
    };
    ...
    struct AnnotationElementVisitor {
        ...
        virtual void visitArray(
            U_16 elementNameIndex, U_16 elementCount, ArrayAnnotationElements* arrayAnnotationElements)
        {}
    ...
    struct AnnotationVisitor {
        ...
        virtual void visitTypeAnnotation(
            U_8 targetType, J9CfrTypeAnnotationTargetInfo* targetInfo, J9CfrTypePath* typePath)
            = 0;
    };
    ...
    struct AnnotationsAttributeVisitor {
        ...
        virtual void visitParameterAnnotationsAttribute(U_16 fieldOrMethodIndex, U_32 length, U_8 numberOfParameters)
            = 0;
    ...
    public:
        ...
        void localsDo(VerificationTypeInfoVisitor* visitor)
        {
            slotsDo(_stackMapFrameInfo->localsCount, _stackMapFrameInfo->localsTypeInfo, visitor);
        }
        void stackItemsDo(VerificationTypeInfoVisitor* visitor)
        {
            slotsDo(_stackMapFrameInfo->stackItemsCount, _stackMapFrameInfo->stackItemsTypeInfo, visitor);
        }
    ...
    public:
        ...
        {
            J9CfrAnnotationElement** endAnnotationElements
                = _annotationElementArray->values + _annotationElementArray->numberOfValues;
            for (J9CfrAnnotationElement** annotationElement = _annotationElementArray->values;
                 annotationElement != endAnnotationElements; ++annotationElement) {
    ...
    public:
        ...
        void annotationDo(AnnotationVisitor* annotationVisitor, AnnotationElementVisitor* annotationElementVisitor,
            AnnotationElementVisitor* annotationAnnotationVisitor)
        {
            ...
            if (NULL != annotationElementVisitor) {
                _classFileOracle->annotationElementsDo(
                    annotationElementVisitor, _annotation->elementValuePairs, _annotation->numberOfElementValuePairs);
            }
            if (NULL != annotationAnnotationVisitor) {
                _classFileOracle->annotationElementsDo(annotationAnnotationVisitor, _annotation->elementValuePairs,
                    _annotation->numberOfElementValuePairs);
            }
        }
    ...
    public:
        AnnotationAnnotationVisitor(
            AnnotationVisitor* annotationVisitor, AnnotationElementVisitor* annotationElementVisitor)
    ...
    public:
        ...
        {
            return (getAccessFlags() & CFR_ACC_STATIC) && (NULL != _fields[_index].constantValueAttribute);
        }
        ...
        U_32 getConstantValueSlot1() const
        {
            return _classFile->constantPool[getConstantValueConstantPoolIndex()].slot1;
        }
        U_32 getConstantValueSlot2() const
        {
            return _classFile->constantPool[getConstantValueConstantPoolIndex()].slot2;
        }
        U_16 getConstantValueConstantPoolIndex() const
        {
            return _fields[_index].constantValueAttribute->constantValueIndex;
        }
    ...
    private:
        ...
        {
            /*
             * _localVariableTable is a cached pointer to
             * _localVariablesInfo[_index].localVariableTable->localVariableTable. The LocalVariableInfo array is
             * indexed by the "local variable index" (a.k.a. slot number). An entry is valid if:
             * 	- there is a localVariableTable for the current slot (_index) and
             * 	- the entry's index matches the current slot number (_index)
             * The algorithm below checks if the current entry is valid; if not, the cached pointer is cleared and the
             * search begins. When a valid entry is found, the pointer to the corresponding localVariableTable is cached
             * to allow easy access from the getter methods above.
             */
            if ((NULL == _localVariableTable) /* Is the cached pointer invalid? */
                || (_localVariableTableIndex
                       >= _localVariablesInfo[_index]
                              .localVariableTable->localVariableTableLength) /* Have we walked off the end of the
                                                                                current localVariableTable? */
                || (_index != _localVariableTable[_localVariableTableIndex].index)) { /* Is the _index different from
                                                                                         the current entry's index? */
                _localVariableTable = NULL;
                while ((NULL == _localVariableTable)
                    && isNotDone()) { /* Keep looking until a valid entry is found or we run out of entries */
                    if ((NULL == _localVariablesInfo[_index].localVariableTable)
                        || (_localVariableTableIndex
                               >= _localVariablesInfo[_index].localVariableTable->localVariableTableLength)) {
                        /* If there is no localVariableTable or we've exhausted the entries for the current slot,
                         * advance to the next slot */
                        ++_index;
                        _localVariableTableIndex = 0;
                    } else if (_index
                        != _localVariablesInfo[_index]
                               .localVariableTable->localVariableTable[_localVariableTableIndex]
                               .index) {
    ...
    public:
        ...
        bool hasBackwardBranches() const
        {
            return 0 != (_methodsInfo[_methodIndex].modifiers & J9AccMethodHasBackwardBranches);
        }
        bool hasGenericSignature() const
        {
            return 0 != (_methodsInfo[_methodIndex].modifiers & J9AccMethodHasGenericSignature);
        }
        bool hasFrameIteratorSkipAnnotation() const
        {
            return _methodsInfo[_methodIndex].hasFrameIteratorSkipAnnotation;
        }
        bool hasAnnotationsData() const { return NULL != _methodsInfo[_methodIndex].annotationsAttribute; }
        bool hasParameterAnnotations() const
        {
            return NULL != _methodsInfo[_methodIndex].parameterAnnotationsAttribute;
        }
        bool hasMethodTypeAnnotations() const
        {
            return NULL != _methodsInfo[_methodIndex].methodTypeAnnotationsAttribute;
        }
        ...
        bool hasMethodParameters() const
        {
            return 0 != (_methodsInfo[_methodIndex].modifiers & J9AccMethodHasMethodParameters);
        }

        U_8 getMethodParametersCount() const
        {
            return (NULL == _classFile->methods[_methodIndex].methodParametersAttribute)
                ? 0
                : _classFile->methods[_methodIndex].methodParametersAttribute->numberOfMethodParameters;
        }
        ...
        U_16 getMaxLocals() const
        {
            return (NULL == _classFile->methods[_methodIndex].codeAttribute)
                ? 0
                : _classFile->methods[_methodIndex].codeAttribute->maxLocals;
        }
        ...
        U_16 getMaxStack() const
        {
            return (NULL == _classFile->methods[_methodIndex].codeAttribute)
                ? 0
                : _classFile->methods[_methodIndex].codeAttribute->maxStack;
        }
        U_16 getStackMapFramesCount() const { return _methodsInfo[_methodIndex].stackMapFramesCount; }
        U_16 getExceptionHandlersCount() const
        {
            return (NULL == _classFile->methods[_methodIndex].codeAttribute)
                ? 0
                : _classFile->methods[_methodIndex].codeAttribute->exceptionTableLength;
        }
        ...
        U_8* getCode() const
        {
            return (NULL == _classFile->methods[_methodIndex].codeAttribute)
                ? 0
                : _classFile->methods[_methodIndex].codeAttribute->code;
        }
        U_32 getCodeLength() const
        {
            return (NULL == _classFile->methods[_methodIndex].codeAttribute)
                ? 0
                : _classFile->methods[_methodIndex].codeAttribute->codeLength;
        }
        U_32 getLineNumbersCount() const { return _methodsInfo[_methodIndex].lineNumbersCount; }
        U_32 getLineNumbersInfoCompressedSize() const
        {
            return _methodsInfo[_methodIndex].lineNumbersInfoCompressedSize;
        }
        ...
        ClassFileOracle::BytecodeFixupEntry* getByteCodeFixupTable() const
        {
            return _methodsInfo[_methodIndex].byteCodeFixupTable;
        }

        void exceptionsThrownDo(ConstantPoolIndexVisitor* visitor)
        { // TODO refactor MethodInfo and MethodIterator to make this usable by
          // ClassFileOracle::walkMethodThrownExceptions
        ...
        void exceptionHandlersDo(ExceptionHandlerVisitor* visitor)
        {
            if (NULL != _classFile->methods[_methodIndex].codeAttribute) {
                J9CfrExceptionTableEntry* end
                    = _classFile->methods[_methodIndex].codeAttribute->exceptionTable + getExceptionHandlersCount();
                for (J9CfrExceptionTableEntry* entry = _classFile->methods[_methodIndex].codeAttribute->exceptionTable;
                     entry != end; ++entry) {
        ...
        {
            StackMapFrameInfo* end
                = _methodsInfo[_methodIndex].stackMapFramesInfo + _methodsInfo[_methodIndex].stackMapFramesCount;
            for (StackMapFrameInfo* info = _methodsInfo[_methodIndex].stackMapFramesInfo; info != end; ++info) {
                ...
                visitor->visitStackMapFrame(
                    info->localsCount, info->stackItemsCount, info->offsetDelta, info->frameType, &typeInfo);
            }
        }

        LocalVariablesIterator getLocalVariablesIterator()
        {
            return LocalVariablesIterator((NULL == _methodsInfo[_methodIndex].localVariablesInfo) ? 0 : getMaxLocals(),
                _methodsInfo[_methodIndex].localVariablesInfo);
        }
    ...
    void annotationElementDo(AnnotationElementVisitor* annotationElementVisitor, U_16 elementNameIndex,
        J9CfrAnnotationElement* annotationElement)
    {
        ...
        case 'e':
            ...
            break;
        case 'c':
            annotationElementVisitor->visitClass(
                elementNameIndex, ((J9CfrAnnotationElementClass*)annotationElement)->classInfoIndex);
            break;
        case '@': {
            NestedAnnotation nestedAnnotation(
                this, &(((J9CfrAnnotationElementAnnotation*)annotationElement)->annotationValue));
            annotationElementVisitor->visitNestedAnnotation(elementNameIndex, &nestedAnnotation);
            break;
        }
        case '[': {
            ...
            annotationElementVisitor->visitArray(elementNameIndex,
                ((J9CfrAnnotationElementArray*)annotationElement)->numberOfValues, &arrayAnnotationElements);
            break;
        }
        default:
            annotationElementVisitor->visitConstant(elementNameIndex,
                ((J9CfrAnnotationElementPrimitive*)annotationElement)->constValueIndex, annotationElement->tag);
            break;
        }
    }

    void annotationElementsDo(AnnotationElementVisitor* annotationElementVisitor,
        J9CfrAnnotationElementPair* elementValuePairs, U_16 elementValuePairCount)
    {
        J9CfrAnnotationElementPair* endElementValuePairs = elementValuePairs + elementValuePairCount;
        for (J9CfrAnnotationElementPair* elementValuePair = elementValuePairs; elementValuePair != endElementValuePairs;
             ++elementValuePair) {
            annotationElementDo(annotationElementVisitor, elementValuePair->elementNameIndex, elementValuePair->value);
        }
    }

    void defaultAnnotationDo(U_16 methodIndex, AnnotationsAttributeVisitor* annotationsAttributeVisitor,
        AnnotationElementVisitor* annotationElementVisitor)
    {
        if (NULL != _methodsInfo[methodIndex].defaultAnnotationAttribute) {
            if (NULL != annotationsAttributeVisitor) {
                annotationsAttributeVisitor->visitDefaultAnnotationAttribute(
                    methodIndex, _methodsInfo[methodIndex].defaultAnnotationAttribute->length);
            }
            if (NULL != annotationElementVisitor) {
                annotationElementDo(annotationElementVisitor, _classFile->methods[methodIndex].nameIndex,
                    _methodsInfo[methodIndex].defaultAnnotationAttribute->defaultValue);
            }
        }
    }

    VMINLINE void annotationsDo(U_16 index, J9CfrAttributeRuntimeVisibleAnnotations* annotationsAttribute,
        AnnotationsAttributeVisitor* annotationsAttributeVisitor, AnnotationVisitor* annotationVisitor,
        AnnotationElementVisitor* annotationElementVisitor)
    {
        if (NULL != annotationsAttribute) {
            if ((annotationsAttribute->rawDataLength > 0)
                && (NULL != annotationsAttributeVisitor)) { /* Detected bad attribute: this will fail when reparsed by
                                                               Java code. */
                annotationsAttributeVisitor->visitMalformedAnnotationsAttribute(
                    annotationsAttribute->rawDataLength, annotationsAttribute->rawAttributeData);
            } else {
                if (NULL != annotationsAttributeVisitor) {
                    annotationsAttributeVisitor->visitAnnotationsAttribute(
                        index, annotationsAttribute->length, annotationsAttribute->numberOfAnnotations);
                }
                if ((NULL != annotationVisitor) || (NULL != annotationElementVisitor)) {
                    J9CfrAnnotation* endAnnotations
                        = annotationsAttribute->annotations + annotationsAttribute->numberOfAnnotations;
                    for (J9CfrAnnotation* annotation = annotationsAttribute->annotations; annotation != endAnnotations;
                         ++annotation) {
                        if (NULL != annotationVisitor) {
                            annotationVisitor->visitAnnotation(
                                annotation->typeIndex, annotation->numberOfElementValuePairs);
                        }
                        if (NULL != annotationElementVisitor) {
                            annotationElementsDo(annotationElementVisitor, annotation->elementValuePairs,
    ...
    void classAnnotationsDo(AnnotationsAttributeVisitor* annotationsAttributeVisitor,
        AnnotationVisitor* annotationVisitor, AnnotationElementVisitor* annotationElementVisitor)
    {
        annotationsDo(
            0, _annotationsAttribute, annotationsAttributeVisitor, annotationVisitor, annotationElementVisitor);
    }

    void classTypeAnnotationsDo(AnnotationsAttributeVisitor* annotationsAttributeVisitor,
        AnnotationVisitor* annotationVisitor, AnnotationElementVisitor* annotationElementVisitor)
    {
        typeAnnotationsDo(
            0, _typeAnnotationsAttribute, annotationsAttributeVisitor, annotationVisitor, annotationElementVisitor);
    }

    void fieldAnnotationDo(U_16 fieldIndex, AnnotationsAttributeVisitor* annotationsAttributeVisitor,
        AnnotationVisitor* annotationVisitor, AnnotationElementVisitor* annotationElementVisitor)
    {
        annotationsDo(fieldIndex, _fieldsInfo[fieldIndex].annotationsAttribute, annotationsAttributeVisitor,
            annotationVisitor, annotationElementVisitor);
    }

    void fieldTypeAnnotationDo(U_16 fieldIndex, AnnotationsAttributeVisitor* annotationsAttributeVisitor,
        AnnotationVisitor* annotationVisitor, AnnotationElementVisitor* annotationElementVisitor)
    {
        typeAnnotationsDo(fieldIndex, _fieldsInfo[fieldIndex].typeAnnotationsAttribute, annotationsAttributeVisitor,
            annotationVisitor, annotationElementVisitor);
    }

    void methodAnnotationDo(U_16 methodIndex, AnnotationsAttributeVisitor* annotationsAttributeVisitor,
        AnnotationVisitor* annotationVisitor, AnnotationElementVisitor* annotationElementVisitor)
    {
        annotationsDo(methodIndex, _methodsInfo[methodIndex].annotationsAttribute, annotationsAttributeVisitor,
            annotationVisitor, annotationElementVisitor);
    }

    void methodTypeAnnotationDo(U_16 methodIndex, AnnotationsAttributeVisitor* annotationsAttributeVisitor,
        AnnotationVisitor* annotationVisitor, AnnotationElementVisitor* annotationElementVisitor)
    {
        typeAnnotationsDo(methodIndex, _methodsInfo[methodIndex].methodTypeAnnotationsAttribute,
            annotationsAttributeVisitor, annotationVisitor, annotationElementVisitor);
    }

    void methodCodeTypeAnnotationDo(U_16 methodIndex, AnnotationsAttributeVisitor* annotationsAttributeVisitor,
        AnnotationVisitor* annotationVisitor, AnnotationElementVisitor* annotationElementVisitor)
    {
        typeAnnotationsDo(methodIndex, _methodsInfo[methodIndex].codeTypeAnnotationsAttribute,
            annotationsAttributeVisitor, annotationVisitor, annotationElementVisitor);
    }

    void parameterAnnotationsDo(U_16 methodIndex, AnnotationsAttributeVisitor* annotationsAttributeVisitor,
        AnnotationVisitor* annotationVisitor, AnnotationElementVisitor* annotationElementVisitor)
    {
        J9CfrAttributeRuntimeVisibleParameterAnnotations* parameterAnnotationsAttribute
            = _methodsInfo[methodIndex].parameterAnnotationsAttribute;
        if (NULL != parameterAnnotationsAttribute) {
            if (parameterAnnotationsAttribute->rawDataLength
                > 0) { /* Detected bad attribute: this will fail when reparsed by Java code. */
                annotationsAttributeVisitor->visitMalformedAnnotationsAttribute(
                    parameterAnnotationsAttribute->rawDataLength, parameterAnnotationsAttribute->rawAttributeData);
            } else {
                if (NULL != annotationsAttributeVisitor) {
                    annotationsAttributeVisitor->visitParameterAnnotationsAttribute(methodIndex,
                        parameterAnnotationsAttribute->length, parameterAnnotationsAttribute->numberOfParameters);
                }
                U_8 parameterIndex = 0;
                J9CfrParameterAnnotations* endParameterAnnotations = parameterAnnotationsAttribute->parameterAnnotations
                    + parameterAnnotationsAttribute->numberOfParameters;
                for (J9CfrParameterAnnotations* parameterAnnotations
                     = parameterAnnotationsAttribute->parameterAnnotations;
                     parameterAnnotations != endParameterAnnotations; ++parameterAnnotations, ++parameterIndex) {
                    if (NULL != annotationVisitor) {
                        annotationVisitor->visitParameter(parameterAnnotations->numberOfAnnotations);
                    }
                    if ((NULL != annotationVisitor) || (NULL != annotationElementVisitor)) {
                        J9CfrAnnotation* endAnnotations
                            = parameterAnnotations->annotations + parameterAnnotations->numberOfAnnotations;
                        for (J9CfrAnnotation* annotation = parameterAnnotations->annotations;
                             annotation != endAnnotations; ++annotation) {
                            if (NULL != annotationVisitor) {
                                annotationVisitor->visitAnnotation(
                                    annotation->typeIndex, annotation->numberOfElementValuePairs);
                            }
        ...
        AnnotationElementVisitor* annotationElementVisitor)
    {
        ...
        if (typeAnnotationsAttribute->rawDataLength
            > 0) { /* Detected bad attribute: this will fail when reparsed by Java code. */
            annotationsAttributeVisitor->visitMalformedAnnotationsAttribute(
                typeAnnotationsAttribute->rawDataLength, typeAnnotationsAttribute->rawAttributeData);
        } else {
            if (NULL != annotationsAttributeVisitor) {
                annotationsAttributeVisitor->visitTypeAnnotationsAttribute(
                    index, typeAnnotationsAttribute->length, typeAnnotationsAttribute->numberOfAnnotations);
            }
            if ((NULL != annotationVisitor) || (NULL != annotationElementVisitor)) {
                for (U_16 typeAnnotationIndex = 0; typeAnnotationIndex < typeAnnotationsAttribute->numberOfAnnotations;
                     ++typeAnnotationIndex) {
                    J9CfrTypeAnnotation* typeAnnotation
                        = &(typeAnnotationsAttribute->typeAnnotations[typeAnnotationIndex]);
                    if (NULL != annotationVisitor) {
                        annotationVisitor->visitTypeAnnotation(
                            typeAnnotation->targetType, &(typeAnnotation->targetInfo), &(typeAnnotation->typePath));
                        annotationVisitor->visitAnnotation(
                            typeAnnotation->annotation.typeIndex, typeAnnotation->annotation.numberOfElementValuePairs);
                    }
                    if (NULL != annotationElementVisitor) {
                        annotationElementsDo(annotationElementVisitor, typeAnnotation->annotation.elementValuePairs,
                            typeAnnotation->annotation.numberOfElementValuePairs);
                    }
                }
            }
        }
    ...
    void bootstrapMethodsDo(BootstrapMethodVisitor* visitor)
    {
        if (NULL != _bootstrapMethodsAttribute) {
            J9CfrBootstrapMethod* end
                = _bootstrapMethodsAttribute->bootstrapMethods + _bootstrapMethodsAttribute->numberOfBootstrapMethods;
    ...
    ClassFileOracle(BufferManager* bufferManager, J9CfrClassFile* classFile, ConstantPoolMap* constantPoolMap,
    ...
    U_16 getEnclosingMethodNameAndSignatureIndex() const
    {
        return hasEnclosingMethod() ? _enclosingMethod->methodIndex : 0;
    }
    ...
    U_16 getNameAndSignatureNameUTF8Length(U_16 cpIndex) const
    {
        return getUTF8Length(U_16(_classFile->constantPool[cpIndex].slot1));
    }
    U_8* getNameAndSignatureNameUTF8Data(U_16 cpIndex) const
    {
        return getUTF8Data(U_16(_classFile->constantPool[cpIndex].slot1));
    }
    U_16 getNameAndSignatureSignatureUTF8Length(U_16 cpIndex) const
    {
        return getUTF8Length(U_16(_classFile->constantPool[cpIndex].slot2));
    }
    U_8* getNameAndSignatureSignatureUTF8Data(U_16 cpIndex) const
    {
        return getUTF8Data(U_16(_classFile->constantPool[cpIndex].slot2));
    }
    ...
    U_16 getBootstrapMethodCount() const
    {
        return hasBootstrapMethods() ? _bootstrapMethodsAttribute->numberOfBootstrapMethods : 0;
    }
    ...
    /*
     *  Long and Double CP entries are never tagged as "referenced" because they are only reached through LDC2W
     * instructions, so don't check the ".referenced" flag in isConstantLong() or isConstantDouble().
     */
    ...
    bool isConstantInteger0(U_16 cpIndex) const
    {
        return (CFR_CONSTANT_Integer == _classFile->constantPool[cpIndex].tag)
            && (0 == _classFile->constantPool[cpIndex].slot1);
    }
    bool isConstantFloat0(U_16 cpIndex) const
    {
        return (CFR_CONSTANT_Float == _classFile->constantPool[cpIndex].tag)
            && (0 == _classFile->constantPool[cpIndex].slot1);
    }
    bool isUTF8AtIndexEqualToString(U_16 cpIndex, const char* string, UDATA stringSize)
    {
        return (getUTF8Length(cpIndex) == (stringSize - 1))
            && (0 == memcmp(getUTF8Data(cpIndex), string, stringSize - 1));
    }
