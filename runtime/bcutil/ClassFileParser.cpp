...
BuildResult ClassFileParser::parseClassFile(
    ROMClassCreationContext* context, UDATA* initialBufferSize, U_8** classFileBuffer)
{
    ...
    while (BCT_ERR_OUT_OF_ROM == result) {
        result = j9bcutil_readClassFileBytes(_portLibrary, _verifyClassFunction, context->classFileBytes(),
            context->classFileSize(), buffer, bufferSize, context->bctFlags(), NULL,
            context->isVerbose() ? context : NULL, context->findClassFlags(), romMethodSortThreshold);
...
void ClassFileParser::restoreOriginalMethodBytecodes()
{
