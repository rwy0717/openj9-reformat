...
void ClassFileWriter::analyzeROMClass()
{
    _cpHashTable
        = hashTableNew(OMRPORT_FROM_J9PORT(_portLibrary), "ClassFileWriter::_cpHashTable", _romClass->classFileCPCount,
            sizeof(HashTableEntry), 0, 0, J9MEM_CATEGORY_JVMTI, hashFunction, equalFunction, NULL, NULL);
    ...
    if (0 != _romClass->bsmCount) {
        ...
        _bsmAttributeLength
            = (U_32)((bsmCursor - bsmData + 1) * sizeof(U_16)); /* +1 to account for num_bootstrap_methods */
    }

    /* Walk callSiteData and add InvokeDynamic CP entries.
     *
     * InvokeDynamic entries in J9ROMClass are stored as CallSite data.
     * For every call site that refers an InvokeDynamic entry, we store its J9ROMNameAndSignature and
     * bootstrapMethodIndex. First the SRPs to J9ROMNameAndSignature for each callsite are stored. This is followed by
     * the bootstrapMethodIndex for each callsite. So the CallSite data consists of a block of SRPs to
     * J9ROMNameAndSignature, followed by a block of bootstrapMethodIndices.
     *
...
void ClassFileWriter::analyzeConstantPool()
{
    ...
    for (U_16 i = 1; i < cpCount; i++) {
        ...
        case J9CPTYPE_DOUBLE:
            addEntry(
                &((J9ROMConstantRef*)cpItem)->slot1, i + (i - _romClass->ramConstantPoolCount), CFR_CONSTANT_Double);
            break;
...
void ClassFileWriter::analyzeInterfaces()
{
...
void ClassFileWriter::analyzeFields()
{
...
void ClassFileWriter::analyzeMethods()
{
...
void ClassFileWriter::writeConstantPool()
{
    ...
    for (U_16 i = 1; i < cpCount; i++) {
        ...
        case J9CPTYPE_ANNOTATION_UTF8:
            ...
            writeData(J9UTF8_LENGTH(J9ROMSTRINGREF_UTF8DATA((J9ROMStringRef*)cpItem)),
                J9UTF8_DATA(J9ROMSTRINGREF_UTF8DATA((J9ROMStringRef*)cpItem)));
            break;
        case J9CPTYPE_CONSTANT_DYNAMIC:
            writeU8(CFR_CONSTANT_Dynamic);
            writeU16(U_16((((J9ROMConstantDynamicRef*)cpItem)->bsmIndexAndCpType >> J9DescriptionCpTypeShift)
                & J9DescriptionCpBsmIndexMask));
...
void ClassFileWriter::writeField(J9ROMFieldShape* fieldShape)
{
...
void ClassFileWriter::writeMethod(J9ROMMethod* method)
{
    ...
    if (NULL != methodParametersData) {
        ...
        writeAttributeHeader(
            (J9UTF8*)&METHODPARAMETERS, sizeof(U_8) + ((sizeof(U_16) + sizeof(U_16)) * parameterCount));
...
void ClassFileWriter::writeAttributes()
{
    ...
    if ((0 != _romClass->innerClassCount) || J9_ARE_ANY_BITS_SET(_romClass->extraModifiers, J9AccClassInnerClass)) {
        ...
        for (UDATA i = 0; i < _romClass->innerClassCount; i++, innerClasses++) {
            ...
            /* NOTE: innerClassAccessFlags and innerNameIndex are not preserved in the ROM class - technically
             * incorrect, but this should only matter to compilers */
...
U_8 ClassFileWriter::computeArgsCount(U_16 methodRefIndex)
{
...
void ClassFileWriter::writeCodeAttribute(J9ROMMethod* method)
{
    ...
    if (NULL != debugInfo) {
        ...
        if (0 != varInfoCount) {
            ...
            if (0 != varInfoWithSigCount) {
                writeAttributeHeader(
                    (J9UTF8*)&LOCAL_VARIABLE_TYPE_TABLE, sizeof(U_16) + (sizeof(U_16) * 5 * varInfoWithSigCount));
...
void ClassFileWriter::rewriteBytecode(J9ROMMethod* method, U_32 length, U_8* code)
{
    /*
...
void ClassFileWriter::writeVerificationTypeInfo(U_16 count, U_8** typeInfo)
{
    U_8* cursor = *typeInfo;
    ...
    for (U_16 i = 0; i < count; i++) {
        ...
        case CFR_STACKMAP_TYPE_SHORT_ARRAY: {
            ...
            U_8 classUTF8[2 + 255
                + 1]; /* represents J9UTF8 for primitive class (size = length + arity + primitiveChar) */
            U_16 arity;
...
void ClassFileWriter::writeStackMapTableAttribute(J9ROMMethod* romMethod)
{
    U_16 numEntries = 0;
...
void ClassFileWriter::writeSignatureAttribute(J9UTF8* genericSignature)
{
    writeAttributeHeader((J9UTF8*)&SIGNATURE, 2);
    writeU16(indexForUTF8(genericSignature));
}

void ClassFileWriter::writeAnnotationElement(U_8** annotationElement)
{
...
void ClassFileWriter::writeAnnotation(U_8** annotation)
{
...
void ClassFileWriter::writeAnnotationsAttribute(U_32* annotationsData)
{
...
void ClassFileWriter::writeParameterAnnotationsAttribute(U_32* parameterAnnotationsData)
{
...
void ClassFileWriter::writeTypeAnnotationsAttribute(U_32* typeAnnotationsData)
{
...
void ClassFileWriter::writeAnnotationDefaultAttribute(U_32* annotationsDefaultData)
{
...
void ClassFileWriter::writeAttributeHeader(J9UTF8* name, U_32 length)
{
    writeU16(indexForUTF8(name));
    writeU32(length);
}
