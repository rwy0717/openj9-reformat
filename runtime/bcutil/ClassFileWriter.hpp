...
private:
    ...
    void addEntry(void* address, U_16 cpIndex, U_8 cpType)
    {
        ...
        if (NULL == result) {
            ...
            /* Update the existing entry to record the cpIndex - only expect this to happen for Annotation UTF8 entries
             * in the ROM constant pool */
...
#ifdef J9VM_ENV_LITTLE_ENDIAN
        *u32Addr = ((val & 0xff000000) >> 24) | ((val & 0x00ff0000) >> 8) | ((val & 0x0000ff00) << 8)
            | ((val & 0x000000ff) << 24);
#else /* J9VM_ENV_LITTLE_ENDIAN */
        *u32Addr = val;
#endif /* J9VM_ENV_LITTLE_ENDIAN */
    ...
    void writeU8At(U_8 val, U_8* address) { *address = val; }
    ...
    /*
     * Walk the ROM class, adding entries to the _cpHashTable for all referenced UTF8s & NASs, and all J9CPTYPE_INT,
     * J9CPTYPE_LONG, J9CPTYPE_DOUBLE CP entries. If the hashtable or a hashtable entry can't be allocated, returns with
     * _buildResult = OutOfMemory. If a class file CP entry can't be found, returns with _buildResult = GenericError.
     */
...
public:
        ...
        , _originalClassName(NULL)
    {
        ...
        if (J9_ARE_ALL_BITS_SET(_romClass->extraModifiers, J9AccClassAnonClass)) {
            ...
            _originalClassName
                = (J9UTF8*)j9mem_allocate_memory(sizeof(U_16) + originalNameLength + 1, J9MEM_CATEGORY_CLASSES);
