...
ComparingCursor::ComparingCursor(J9JavaVM* javaVM, SRPOffsetTable* srpOffsetTable, SRPKeyProducer* srpKeyProducer,
...
void ComparingCursor::writeU8(U_8 u8Value, DataType dataType)
{
    ComparingCursorHelper* countingcursor = getCountingCursor(dataType);
    if (shouldCheckForEquality(dataType)) {
        if (!isRangeValid(sizeof(U_8), dataType)
            || (u8Value != *(U_8*)(countingcursor->getBaseAddress() + countingcursor->getCount()))) {
            markUnEqual();
        }
    }
...
void ComparingCursor::writeU16(U_16 u16Value, DataType dataType)
{
    ComparingCursorHelper* countingcursor = getCountingCursor(dataType);
    if (shouldCheckForEquality(dataType)) {
        if (!isRangeValid(sizeof(U_16), dataType)
            || (u16Value != *(U_16*)(countingcursor->getBaseAddress() + countingcursor->getCount()))) {
            markUnEqual();
        }
    }
...
void ComparingCursor::writeU32(U_32 u32Value, DataType dataType)
{
    ComparingCursorHelper* countingcursor = getCountingCursor(dataType);
...
U_32 ComparingCursor::peekU32(DataType dataType)
{
    ComparingCursorHelper* countingcursor = getCountingCursor(dataType);
 ...
 * incremented
 */
void ComparingCursor::skip(UDATA byteCount, DataType dataType)
{
    Cursor* countingcursor = getCountingCursor(dataType);
    countingcursor->skip(byteCount);
}

void ComparingCursor::writeU64(U_32 u32ValueHigh, U_32 u32ValueLow, DataType dataType)
{
    ComparingCursorHelper* countingcursor = getCountingCursor(dataType);
    if (shouldCheckForEquality(dataType)) {
...
void ComparingCursor::writeSRP(UDATA srpKey, DataType dataType)
{
    ComparingCursorHelper* countingcursor = getCountingCursor(dataType);
    ...
    if (shouldCheckForEquality(dataType)) {
        if (isRangeValid(sizeof(J9SRP), dataType)) {
            ...
            case Cursor::SRP_TO_LOCAL_VARIABLE_DATA: {
                ...
                } else if ((_context->shouldPreserveLocalVariablesInfo() && isSRPNull(srpKey))
                    || !isRangeValidForPtr((U_8*)tmpvardata, sizeof(void*))) {
                    /* If variable information is not required then this SRP will not be marked during the 'mark &
                     * count' phase. Checking shouldPreserveLocalVariablesInfo() ensure we isSRPNull() only if it could
                     * have been marked.
                     */
                    markUnEqual();
                }
            ...
            case Cursor::SRP_TO_UTF8: {
                ...
                if (NULL == utf8) {
                    ...
                    if (J9UTF8_LENGTH(utf8) != _classFileOracle->getUTF8Length(cpIndex)) {
                        ...
                        != memcmp(J9UTF8_DATA(utf8), _classFileOracle->getUTF8Data(cpIndex), J9UTF8_LENGTH(utf8))) {
                        markUnEqual();
                    }
                }
                break;
                    ...
                    if (isSRPNull(srpKey) || !isRangeValidForUTF8Ptr(name) || !isRangeValidForUTF8Ptr(signature)) {
                        ...
                        if ((J9UTF8_LENGTH(name) != _classFileOracle->getNameAndSignatureNameUTF8Length(cpIndex))
                            || (J9UTF8_LENGTH(signature)
                                   != _classFileOracle->getNameAndSignatureSignatureUTF8Length(cpIndex))) {
                            markUnEqual();
                        } else if ((0
                                       != memcmp(J9UTF8_DATA(name),
                                              _classFileOracle->getNameAndSignatureNameUTF8Data(cpIndex),
                                              J9UTF8_LENGTH(name)))
                            || (0
                                   != memcmp(J9UTF8_DATA(signature),
                                          _classFileOracle->getNameAndSignatureSignatureUTF8Data(cpIndex),
                                          J9UTF8_LENGTH(signature)))) {
                            markUnEqual();
                        }
                    }
...
void ComparingCursor::writeWSRP(UDATA srpKey, DataType dataType)
{
    Cursor* countingcursor = getCountingCursor(dataType);
    if (shouldCheckForEquality(dataType)) {
...
void ComparingCursor::writeData(U_8* bytes, UDATA length, DataType dataType)
{
    ComparingCursorHelper* countingcursor = getCountingCursor(dataType);
...
void ComparingCursor::padToAlignment(UDATA byteAlignment, DataType dataType)
{
    Cursor* countingcursor = getCountingCursor(dataType);
    if (shouldCheckForEquality(dataType)) {
        // TODO
...
bool ComparingCursor::shouldCheckForEquality(DataType dataType, U_32 u32Value)
{
    if (!isEqual()) {
        return false;
    }
    ...
    case SOURCE_DEBUG_EXT_DATA:
        if (_context->romClassHasSourceDebugExtension()
            && !(_classFileOracle->hasSourceDebugExtension() && _context->shouldPreserveSourceDebugExtension())) {
    ...
    case OPTINFO_SOURCE_FILE_NAME:
        if (_context->romClassHasSourceFileName()
            && !(_classFileOracle->hasSourceFile() && _context->shouldPreserveSourceFileName())) {
...
bool ComparingCursor::isRangeValid(UDATA length, DataType dataType)
{
    ComparingCursorHelper* countingcursor = getCountingCursor(dataType);

    if (countingcursor != &(_mainHelper)) {
        if (_checkRangeInSharedCache) {
            return j9shr_Query_IsAddressInCache(
                       _javaVM, countingcursor->getBaseAddress() + countingcursor->getCount(), length)
                ? true
                : false;
        }
        return true;
...
bool ComparingCursor::isRangeValidForPtr(U_8* ptr, UDATA length)
{
    if (_checkRangeInSharedCache) {
...
bool ComparingCursor::isRangeValidForUTF8Ptr(J9UTF8* utf8)
{
    U_8* ptr = (U_8*)utf8;
...
ComparingCursor::getMaximumValidLengthForPtrInSegment(U_8* ptr)
{
    ...
    if (NULL != _javaVM) {
        ...
        J9MemorySegment* segment
            = (J9MemorySegment*)avl_search(&(_javaVM->classMemorySegments->avlTreeData), UDATA(ptr));
...
void ComparingCursor::notifyDebugDataWriteStart()
{
...
void ComparingCursor::notifyVariableTableWriteEnd()
{
...
ComparingCursorHelper* ComparingCursor::getCountingCursor(DataType dataType)
{
    switch (dataType) {
