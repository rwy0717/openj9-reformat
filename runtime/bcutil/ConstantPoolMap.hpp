...
public:
    ...
    {
        return _varHandleMethodTypeCount + (_varHandleMethodTypeCount & 0x1); /* Rounding up to an even number */
    }
    ...
    bool isStaticSplit(U_16 cfrCPIndex) const
    {
         ...
         * Sharing of CP index between 'invokestatic' and 'invokespecial' can also be handled using the side table
         * mechanism instead of marking it as J9CPTYPE_SHARED_METHOD, and hence the second condition. See Jazz103 Design
         * 40047.
         */
        return (isMarked(cfrCPIndex, INVOKE_STATIC)
            && (_context->alwaysSplitBytecodes() || isMarked(cfrCPIndex, INVOKE_INTERFACE)
    ...
    {
        /*
         * Note: Unlike the other markFooAsUsedByBar(U_16 cfrCPIndex) methods above, uses of InvokeDynamicInfo CP
         * entries are not marked with mark(cfrCPIndex, INVOKE_DYNAMIC) because the romCPBaseIndex for these entries is
         * an index into the callSites table instead of the ROM Constant Pool. Also see computeConstantPoolMapAndSizes.
         */
        mark(cfrCPIndex);
...
public:
    ...
    enum EntryUseFlags {
        ...
        ANNOTATION = SPLIT1,
        /* INVOKE_DYNAMIC does not use a constant pool entry. It is used only for
           ClassFileOracle::BytecodeFixupEntry.type. */
