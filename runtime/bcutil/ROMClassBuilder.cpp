...
ROMClassBuilder::ROMClassBuilder(J9JavaVM* javaVM, J9PortLibrary* portLibrary, UDATA maxStringInternTableSize,
    U_8* verifyExcludeAttribute, VerifyClassFunction verifyClassFunction)
    : _javaVM(javaVM)
...
ROMClassBuilder* ROMClassBuilder::getROMClassBuilder(J9PortLibrary* portLibrary, J9JavaVM* vm)
{
    ...
    if (NULL == romClassBuilder) {
        ...
        if (NULL != romClassBuilder) {
            ...
            new (romClassBuilder) ROMClassBuilder(vm, portLibrary, vm->maxInvariantLocalTableNodeCount,
...
extern "C" void shutdownROMClassBuilder(J9JavaVM* vm)
{
...
extern "C" IDATA j9bcutil_compareRomClass(U_8* classFileBytes, U_32 classFileSize, J9PortLibrary* portLib,
    struct J9BytecodeVerificationData* verifyBuffers, UDATA bctFlags, UDATA bcuFlags, J9ROMClass* romClass)
{
    ROMClassBuilder romClassBuilder(NULL, portLib, 0, NULL == verifyBuffers ? NULL : verifyBuffers->excludeAttribute,
        NULL == verifyBuffers ? NULL : j9bcv_verifyClassStructure);
...
extern "C" IDATA j9bcutil_buildRomClassIntoBuffer(U_8* classFileBytes, UDATA classFileSize, J9PortLibrary* portLib,
    J9BytecodeVerificationData* verifyBuffers, UDATA bctFlags, UDATA bcuFlags, UDATA findClassFlags, U_8* romSegment,
    UDATA romSegmentSize, U_8* lineNumberBuffer, UDATA lineNumberBufferSize, U_8* varInfoBuffer,
    UDATA varInfoBufferSize, U_8** classFileBufferPtr)
{
    SuppliedBufferAllocationStrategy suppliedBufferAllocationStrategy(
        romSegment, romSegmentSize, lineNumberBuffer, lineNumberBufferSize, varInfoBuffer, varInfoBufferSize);
    ROMClassBuilder romClassBuilder(NULL, portLib, 0, NULL == verifyBuffers ? NULL : verifyBuffers->excludeAttribute,
        NULL == verifyBuffers ? NULL : j9bcv_verifyClassStructure);
    ROMClassCreationContext context(
        portLib, classFileBytes, classFileSize, bctFlags, bcuFlags, findClassFlags, &suppliedBufferAllocationStrategy);
...
extern "C" IDATA j9bcutil_buildRomClass(J9LoadROMClassData* loadData, U_8* intermediateData,
    UDATA intermediateDataLength, J9JavaVM* javaVM, UDATA bctFlags, UDATA classFileBytesReplaced,
    UDATA isIntermediateROMClass, J9TranslationLocalBuffer* localBuffer)
{
    ...
    ROMClassCreationContext context(PORTLIB, javaVM, loadData->classData, loadData->classDataLength, bctFlags, bcuFlags,
        findClassFlags, &romClassSegmentAllocationStrategy, loadData->className, loadData->classNameLength,
        intermediateData, (U_32)intermediateDataLength, loadData->romClass, loadData->classBeingRedefined,
...
extern "C" IDATA j9bcutil_transformROMClass(
    J9JavaVM* javaVM, J9PortLibrary* portLibrary, J9ROMClass* romClass, U_8** classData, U_32* size)
{
...
BuildResult ROMClassBuilder::buildROMClass(ROMClassCreationContext* context)
{
...
BuildResult ROMClassBuilder::handleAnonClassName(J9CfrClassFile* classfile)
{
    ...
    for (i = 0; i < classfile->constantPoolCount; i++) {
        if ((CFR_CONSTANT_String == constantPool[i].tag) || (CFR_CONSTANT_NameAndType == constantPool[i].tag)) {
        ...
        if (CFR_CONSTANT_Class == constantPool[i].tag) {
            ...
            if (classNameSlot != newUtfCPEntry) {
                ...
                if ((classNameLength == originalStringLength)
                    && (0
                           == strncmp(originalStringBytes, (const char*)constantPool[classNameSlot].bytes,
...
U_8* ROMClassBuilder::releaseClassFileBuffer()
{
    U_8* result = _classFileBuffer;
    _classFileBuffer = NULL;
    return result;
}
void ROMClassBuilder::getSizeInfo(ROMClassCreationContext* context, ROMClassWriter* romClassWriter,
    SRPOffsetTable* srpOffsetTable, bool* countDebugDataOutOfLine, SizeInformation* sizeInformation)
{
    ...
    if (context->canPossiblyStoreDebugInfoOutOfLine()) {
        ...
        *countDebugDataOutOfLine = true;
        romClassWriter->writeROMClass(&mainAreaCursor, &lineNumberCursor, &variableInfoCursor, &utf8Cursor,
            (context->isIntermediateDataAClassfile()) ? &classDataCursor : NULL, 0, 0, 0, 0,
            ROMClassWriter::MARK_AND_COUNT_ONLY);
    } else {
        context->forceDebugDataInLine();
        romClassWriter->writeROMClass(&mainAreaCursor, &mainAreaCursor, &mainAreaCursor, &utf8Cursor,
            (context->isIntermediateDataAClassfile()) ? &classDataCursor : NULL, 0, 0, 0, 0,
            ROMClassWriter::MARK_AND_COUNT_ONLY);
    }
...
BuildResult ROMClassBuilder::prepareAndLaydown(
    BufferManager* bufferManager, ClassFileParser* classFileParser, ROMClassCreationContext* context)
{
    ...
    ClassFileOracle classFileOracle(
        bufferManager, classFileParser->getParsedClassFile(), &constantPoolMap, _verifyExcludeAttribute, context);
    ...
    if (context->shouldCompareROMClassForEquality()) {
        ...
        if (compareROMClassForEquality((U_8*)romClass, romClassIsShared, &romClassWriter, &srpOffsetTable,
                &srpKeyProducer, &classFileOracle, modifiers, extraModifiers, optionalFlags, context)) {
            return OK;
        } else {
    ...
    UDATA maxRequiredSize = sizeInformation.rcWithOutUTF8sSize + sizeInformation.lineNumberSize
        + sizeInformation.variableInfoSize + sizeInformation.utf8sSize + sizeInformation.rawClassDataSize
    ...
    if (context->isROMClassShareable()) {
        ...
        SCStoreTransaction sharedStoreClassTransaction
            = SCStoreTransaction(context->currentVMThread(), context->classLoader(), context->cpIndex(), loadType,
                classFileOracle.getUTF8Length(classFileOracle.getClassNameIndex()),
                classFileOracle.getUTF8Data(classFileOracle.getClassNameIndex()), context->classFileBytesReplaced(),
        ...
        if (sharedStoreClassTransaction.isOK()) {
            ...
            for (J9ROMClass* existingROMClass = sharedStoreClassTransaction.nextSharedClassForCompare();
                 NULL != existingROMClass; existingROMClass = sharedStoreClassTransaction.nextSharedClassForCompare()) {
                ...
                if (compareROMClassForEquality((U_8*)existingROMClass, /* romClassIsShared = */ true, &romClassWriter,
                        &srpOffsetTable, &srpKeyProducer, &classFileOracle, modifiers, extraModifiers, optionalFlags,
                        context)) {
                    /*
            ...
            sizeRequirements.romClassMinimalSize = U_32(sizeInformation.rcWithOutUTF8sSize + sizeInformation.utf8sSize
                + sizeInformation.rawClassDataSize + sizeInformation.varHandleMethodTypeLookupTableSize);

            sizeRequirements.romClassSizeFullSize = U_32(sizeRequirements.romClassMinimalSize
            ...
            if (sharedStoreClassTransaction.allocateSharedClass(&sizeRequirements)) {
                 ...
                 */
                U_32 romSize = finishPrepareAndLaydown((U_8*)sharedStoreClassTransaction.getRomClass(),
                    (U_8*)sharedStoreClassTransaction.getLineNumberTable(),
                    (U_8*)sharedStoreClassTransaction.getLocalVariableTable(), &sizeInformation, modifiers,
                    extraModifiers, optionalFlags, true, sharedStoreClassTransaction.hasSharedStringTableLock(),
                    &classFileOracle, &srpOffsetTable, &srpKeyProducer, &romClassWriter, context, &constantPoolMap);
                ...
                if ((NULL != _javaVM)
                    ...
                    checkDebugInfoCompression((J9ROMClass*)romClassBuffer, classFileOracle, &srpKeyProducer,
                        &constantPoolMap, &srpOffsetTable);
                }
                return OK;
            }
        }
    ...
    if (context->isIntermediateDataAClassfile()) {
         ...
         * when running with -Xshareclasses:enableBCI and class file bytes are not modified and re-transformation is not
         * enabled.
         */
    ...
    /*
     *  Use an if statement here and call finishPrepareAndLaydown() in both cases to allow the scope of
     * SCStringTransaction() to survive the life of the call to finishPrepareAndLaydown(). Otherwise, the scope of
     * SCStringTransaction() would end early and it would not be safe to us interned strings.
     */
    U_32 romSize = 0;
    ...
    if (NULL != context->javaVM()) {
        ...
        romSize = finishPrepareAndLaydown(romClassBuffer, lineNumberBuffer, variableInfoBuffer, &sizeInformation,
            modifiers, extraModifiers, optionalFlags, false, scStringTransaction.isOK(), &classFileOracle,
            &srpOffsetTable, &srpKeyProducer, &romClassWriter, context, &constantPoolMap);
    } else
#endif
    {
        romSize = finishPrepareAndLaydown(romClassBuffer, lineNumberBuffer, variableInfoBuffer, &sizeInformation,
            modifiers, extraModifiers, optionalFlags, false, false, &classFileOracle, &srpOffsetTable, &srpKeyProducer,
            &romClassWriter, context, &constantPoolMap);
    }
    ...
    if ((NULL != _javaVM) && (_javaVM->extendedRuntimeFlags & J9_EXTENDED_RUNTIME_CHECK_DEBUG_INFO_COMPRESSION)) {
        checkDebugInfoCompression(
            (J9ROMClass*)romClassBuffer, classFileOracle, &srpKeyProducer, &constantPoolMap, &srpOffsetTable);
    }
 ...
 * -Xcheck:vm:debuginfo
 */
void ROMClassBuilder::checkDebugInfoCompression(J9ROMClass* romClass, ClassFileOracle classFileOracle,
    SRPKeyProducer* srpKeyProducer, ConstantPoolMap* constantPoolMap, SRPOffsetTable* srpOffsetTable)
{
    PORT_ACCESS_FROM_PORT(_portLibrary);
    ...
    for (ClassFileOracle::MethodIterator methodIterator = classFileOracle.getMethodIterator();
         methodIterator.isNotDone(); methodIterator.next()) {
        ...
        if (0 != lineNumbersInfoSize) {
            J9CfrLineNumberTableEntry* lineNumbersInfo
                = (J9CfrLineNumberTableEntry*)j9mem_allocate_memory(lineNumbersInfoSize, J9MEM_CATEGORY_CLASSES);
                        ...
                        for (index = 0; index < lineNumbersCount; index++) {
                            ...
                            if ((lineNumber.lineNumber != lineNumberOriginal) || (lineNumber.location != pcOriginal)) {
                                ...
                                j9tty_printf(PORTLIB,
                                    "Error while uncompressing the debug information for the class %.*s\n",
        ...
        if (0 != localVariablesCount) {
            ...
            if (NULL != methodDebugInfo) {
                ...
                if (NULL != values) {
                    for (ClassFileOracle::LocalVariablesIterator localVariablesIterator
                         = methodIterator.getLocalVariablesIterator();
                         localVariablesIterator.isNotDone(); localVariablesIterator.next()) {
                        if (NULL == values) {
                        ...
                        Trc_BCU_Assert_Equals_Level1((J9UTF8*)(values->name),
                            (J9UTF8*)(srpOffsetTable->computeWSRP(
                                srpKeyProducer->mapCfrConstantPoolIndexToKey(localVariablesIterator.getNameIndex()),
                                0)));
                        Trc_BCU_Assert_Equals_Level1((J9UTF8*)(values->signature),
                            (J9UTF8*)(srpOffsetTable->computeWSRP(srpKeyProducer->mapCfrConstantPoolIndexToKey(
                                                                      localVariablesIterator.getDescriptorIndex()),
                                0)));
                        if (localVariablesIterator.hasGenericSignature()) {
                            Trc_BCU_Assert_Equals_Level1((J9UTF8*)(values->genericSignature),
                                (J9UTF8*)(srpOffsetTable->computeWSRP(
                                    srpKeyProducer->mapCfrConstantPoolIndexToKey(
                                        localVariablesIterator.getGenericSignatureIndex()),
                                    0)));
                        } else {
...
U_32 ROMClassBuilder::finishPrepareAndLaydown(U_8* romClassBuffer, U_8* lineNumberBuffer, U_8* variableInfoBuffer,
    SizeInformation* sizeInformation, U_32 modifiers, U_32 extraModifiers, U_32 optionalFlags, bool sharingROMClass,
    bool hasStringTableLock, ClassFileOracle* classFileOracle, SRPOffsetTable* srpOffsetTable,
    SRPKeyProducer* srpKeyProducer, ROMClassWriter* romClassWriter, ROMClassCreationContext* context,
    ConstantPoolMap* constantPoolMap)
{
    U_8* romClassBufferEndAddress = romClassBuffer + sizeInformation->rcWithOutUTF8sSize + sizeInformation->utf8sSize
        + sizeInformation->rawClassDataSize;
    ROMClassStringInternManager internManager(context, &_stringInternTable, srpOffsetTable, srpKeyProducer,
        romClassBuffer, romClassBufferEndAddress, sharingROMClass, hasStringTableLock);
    ...
    if (internManager.isInterningEnabled()) {
         ...
         * Therefore;
         * 	 if shared cache is totally out of the SRP range of local ROM classes, then local ROM classes can not
         * use UTF8s in shared cache. if shared cache is totally in the SRP range of local ROM classes, then local ROM
         * classes can use UTF8s in shared cache safely. if part of shared cache is in the SRP range, and part of it is
         * not, then for each UTF8 in the local ROM class, SRP range check is required in order to use shared UTFs in
         * the shared cache.
         *
...
#endif
        {
            ...
            for (ClassFileOracle::UTF8Iterator iterator = classFileOracle->getUTF8Iterator(); iterator.isNotDone();
                 iterator.next()) {
                U_16 cpIndex = iterator.getCPIndex();

                if (constantPoolMap->isUTF8ConstantReferenced(cpIndex)) {
                    internManager.visitUTF8(
                        cpIndex, iterator.getUTF8Length(), iterator.getUTF8Data(), sharedCacheSRPRangeInfo);
                }
            }
        }
    }
     ...
     */
    U_32 romSize = U_32(sizeInformation->rcWithOutUTF8sSize + sizeInformation->utf8sSize
        + sizeInformation->rawClassDataSize + sizeInformation->varHandleMethodTypeLookupTableSize);
    ...
    if (context->isIntermediateDataAClassfile()) {
         ...
         */
        srpOffsetTable->setBaseAddressForTag(
            INTERMEDIATE_TAG, romClassBuffer + sizeInformation->rcWithOutUTF8sSize + sizeInformation->utf8sSize);
    }
     ...
     */
    layDownROMClass(romClassWriter, srpOffsetTable, romSize, modifiers, extraModifiers, optionalFlags, &internManager,
        context, sizeInformation);
    return romSize;
}
...
U_32 ROMClassBuilder::computeExtraModifiers(ClassFileOracle* classFileOracle, ROMClassCreationContext* context)
{
...
#undef PHANTOM_NAME
    ...
    if (classFileOracle->isSynthetic()) {
        /* handle the synthetic attribute. In java 1.5 synthetic may be specified in the access flags as well so do not
         * unset bit here */
...
U_32 ROMClassBuilder::computeOptionalFlags(ClassFileOracle* classFileOracle, ROMClassCreationContext* context)
{
...
void ROMClassBuilder::layDownROMClass(ROMClassWriter* romClassWriter, SRPOffsetTable* srpOffsetTable, U_32 romSize,
    U_32 modifiers, U_32 extraModifiers, U_32 optionalFlags, ROMClassStringInternManager* internManager,
    ROMClassCreationContext* context, SizeInformation* sizeInformation)
{
    ...
    romClassWriter->writeROMClass(&writingCursor, lineNumberCursorPtr, variableInfoCursorPtr, &utf8Cursor,
        (context->isIntermediateDataAClassfile()) ? &classDataCursor : NULL, romSize, modifiers, extraModifiers,
        optionalFlags, ROMClassWriter::WRITE);
}

bool ROMClassBuilder::compareROMClassForEquality(U_8* romClass, bool romClassIsShared, ROMClassWriter* romClassWriter,
    SRPOffsetTable* srpOffsetTable, SRPKeyProducer* srpKeyProducer, ClassFileOracle* classFileOracle, U_32 modifiers,
    U_32 extraModifiers, U_32 optionalFlags, ROMClassCreationContext* context)
{
    ComparingCursor compareCursor(
        _javaVM, srpOffsetTable, srpKeyProducer, classFileOracle, romClass, romClassIsShared, context);
    romClassWriter->writeROMClass(&compareCursor, &compareCursor, &compareCursor, NULL, NULL, 0, modifiers,
        extraModifiers, optionalFlags, ROMClassWriter::WRITE);
 ...
 *	@return 3 (SC_PARTIALLY_IN_THE_SRP_RANGE) if part of the shared cache is in the SRP range, and part of it is
 *not.
 *
 *
 */
SharedCacheRangeInfo ROMClassBuilder::getSharedCacheSRPRangeInfo(void* address)
{
    if ((NULL != _javaVM) && (NULL != _javaVM->sharedClassConfig)) {
        ...
        /**
         * It checks whether shared cache(s) in the SRP range completely, not at all or partially against the specific
         * address passed to this function. Shared cache(s) are SC_COMPLETELY_IN_THE_SRP_RANGE if and only if all the
         * start and end addresses of shared cache(s) are in the srp range. Shared cache(s) are
         * SC_COMPLETELY_OUT_OF_THE_SRP_RANGE if and only if all the start and end addresses of shared cache(s) are out
         * of the srp range. In all other cases, shared cache(s) are SC_PARTIALLY_IN_THE_SRP_RANGE.
         */
        while (NULL != cache) {
            ...
            if (StringInternTable::areAddressesInSRPRange(cacheStart, address)) {
                ...
                if (StringInternTable::areAddressesInSRPRange(cacheEnd, address)) {
                    /**
                     * Current cache is in SRP range. Check whether previous cache(s) were in range or not if there is
                     * any. If current cache is the first cache in the list, then sharedCacheRangeInfo is set to
                     * SC_NO_RANGE_INFO. If all the previous caches are SC_COMPLETELY_OUT_OF_THE_SRP_RANGE, then return
                     * SC_PARTIALLY_IN_THE_SRP_RANGE since current cache is in range. If they are all
                     * SC_COMPLETELY_IN_THE_SRP_RANGE, then set sharedCacheRangeInfo to SC_COMPLETELY_IN_THE_SRP_RANGE
                     *
                     */
                ...
                if (StringInternTable::areAddressesInSRPRange(cacheEnd, address)) {
                    ...
                    /**
                     * Current cache is out of SRP range. Check whether previous cache(s) were in range or not if there
                     * is any. If current cache is the first cache in the list, then sharedCacheRangeInfo is set to
                     * SC_NO_RANGE_INFO. If all the previous caches are SC_PARTIALLY_IN_THE_SRP_RANGE, then return
                     * SC_PARTIALLY_IN_THE_SRP_RANGE since current cache is out of range. If they are all
                     * SC_COMPLETELY_OUT_OF_THE_SRP_RANGE, then set sharedCacheRangeInfo to
                     * SC_COMPLETELY_OUT_OF_THE_SRP_RANGE
                     *
                     */
