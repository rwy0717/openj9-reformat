...
public:
    ROMClassCreationContext(J9PortLibrary* portLibrary, U_8* classFileBytes, UDATA classFileSize, UDATA bctFlags,
        UDATA bcuFlags, J9ROMClass* romClass)
    ...
    ROMClassCreationContext(J9PortLibrary* portLibrary, U_8* classFileBytes, UDATA classFileSize, UDATA bctFlags,
        UDATA bcuFlags, UDATA findClassFlags, AllocationStrategy* allocationStrategy)
        : _portLibrary(portLibrary)
        , _javaVM(NULL)
        , _classFileBytes(classFileBytes)
        , _classFileSize(classFileSize)
    ...
    ROMClassCreationContext(J9PortLibrary* portLibrary, J9JavaVM* javaVM, U_8* classFileBytes, UDATA classFileSize,
        UDATA bctFlags, UDATA bcuFlags, UDATA findClassFlags, AllocationStrategy* allocationStrategy, U_8* className,
        UDATA classNameLength, U_8* intermediateClassData, U_32 intermediateClassDataLength, J9ROMClass* romClass,
        J9Class* clazz, J9ClassLoader* classLoader, bool classFileBytesReplaced, bool creatingIntermediateROMClass,
        ...
        _retransformAllowed(
            (NULL != javaVM) && (0 != (javaVM->requiredDebugAttributes & J9VM_DEBUG_ATTRIBUTE_ALLOW_RETRANSFORM)))
        ,
#else
        _retransformAllowed(false)
        ,
        ...
        , _creatingIntermediateROMClass(creatingIntermediateROMClass)
    {
        if ((NULL != _javaVM) && (NULL != _javaVM->dynamicLoadBuffers)) {
            ...
            _interningEnabled = J9_ARE_ALL_BITS_SET(_bcuFlags, BCU_ENABLE_INVARIANT_INTERNING)
                && J9_ARE_NO_BITS_SET(_findClassFlags, J9_FINDCLASS_FLAG_ANON);
    ...
    U_8* getIntermediateClassDataFromPreviousROMClass() const
    {
        return (NULL != _clazz) ? WSRP_GET(_clazz->romClass->intermediateClassData, U_8*) : NULL;
    }
    U_32 getIntermediateClassDataLengthFromPreviousROMClass() const
    {
        return (NULL != _clazz) ? _clazz->romClass->intermediateClassDataLength : 0;
    }
    ...
    bool isRedefining() const
    {
        return J9_FINDCLASS_FLAG_REDEFINING == (_findClassFlags & J9_FINDCLASS_FLAG_REDEFINING);
    }
    bool isRetransforming() const
    {
        return J9_FINDCLASS_FLAG_RETRANSFORMING == (_findClassFlags & J9_FINDCLASS_FLAG_RETRANSFORMING);
    }
    bool isClassLoaderSharedClassesEnabled() const
    {
        return NULL != _classLoader && (_classLoader->flags & J9CLASSLOADER_SHARED_CLASSES_ENABLED);
    }
    ...
    bool shouldPreserveLineNumbers() const
    {
        return 0 == (_bctFlags & (BCT_StripDebugLines | BCT_StripDebugAttributes));
    }
    bool shouldPreserveLocalVariablesInfo() const
    {
        return 0 == (_bctFlags & (BCT_StripDebugVars | BCT_StripDebugAttributes));
    }
    bool shouldPreserveSourceFileName() const
    {
        return 0 == (_bctFlags & (BCT_StripDebugSource | BCT_StripDebugAttributes));
    }
    bool shouldPreserveSourceDebugExtension() const
    {
        return 0 == (_bctFlags & (BCT_StripSourceDebugExtension | BCT_StripDebugAttributes));
    }
    ...
    {
        return (isSharedClassesEnabled() && isSharedClassesBCIEnabled() && !classFileBytesReplaced()
    ...
    bool isROMClassShareable() const
    {
         ...
         */
        if (isSharedClassesEnabled() && isClassLoaderSharedClassesEnabled() && !isClassUnsafe()
...
#undef J9WRONGNAME
                ...
                && (0 == strncmp((char*)className, "java/", 5)) && (!isClassAnon())) {
                /*
     ...
     */
    BuildResult reportInvalidAnnotation(U_8* className, U_16 classNameLength, U_8* memberName, U_16 memberNameLength,
        U_32 module_name, U_32 message_num)
    {
         ...
         */
        nlsMessage = OMRPORT_FROM_J9PORT(PORTLIB)->nls_lookup_message(OMRPORT_FROM_J9PORT(PORTLIB),
            J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE, module_name, message_num, NULL);
        if (NULL != nlsMessage) {
            UDATA messageLength
                = j9str_printf(PORTLIB, NULL, 0, nlsMessage, memberNameLength, memberName, classNameLength, className);
            U_8* message = (U_8*)j9mem_allocate_memory(messageLength, OMRMEM_CATEGORY_VM);
            if (NULL != message) {
                j9str_printf(PORTLIB, (char*)message, messageLength, nlsMessage, memberNameLength, memberName,
    ...
    void recordTranslationEnd()
    {
        ...
        Trc_BCU_buildRomClass_TranslateClassFileEnd(J9UTF8_LENGTH(J9ROMCLASS_CLASSNAME(romClass)),
            J9UTF8_DATA(J9ROMCLASS_CLASSNAME(romClass)), romClass->romSize, _classLoader);
        if (NULL != _dynamicLoadStats) {
            PORT_ACCESS_FROM_PORT(_portLibrary);
    ...
    void recordROMClass(J9ROMClass* romClass) { _romClass = romClass; }
    ...
    void freeClassFileBuffer(U_8* buffer)
    {
        ...
        if ((NULL != _javaVM) && (NULL != _javaVM->dynamicLoadBuffers)
            && (buffer == _javaVM->dynamicLoadBuffers->classFileError)) {
    ...
    void reportStatistics(J9TranslationLocalBuffer* localBuffer)
    {
        if (NULL != _dynamicLoadStats) {
            if ((NULL != _javaVM) && (NULL != _javaVM->dynamicLoadBuffers)
                && (NULL != _javaVM->dynamicLoadBuffers->reportStatisticsFunction)) {
    ...
    void forceDebugDataInLine() { _forceDebugDataInLine = true; }
    ...
    void startDebugCompare() { _doDebugCompare = true; }

    void endDebugCompare() { _doDebugCompare = false; }
    ...
    bool romMethodHasDebugData() { return (0 != (romMethodModifiers() & J9AccMethodHasDebugInfo)); }
    ...
    void romMethodCacheCurrentRomMethod(IDATA offset) { _existingRomMethod = romMethodFromOffset(offset); }

    J9ROMMethod* romMethodGetCachedMethod() { return _existingRomMethod; }
    ...
    bool romClassHasSourceFileName() { return (0 != (romClassOptionalFlags() & J9_ROMCLASS_OPTINFO_SOURCE_FILE_NAME)); }
