...
U_8* ROMClassSegmentAllocationStrategy::allocate(UDATA bytesRequired)
{
    U_8* result = NULL;
    ...
    if (!isLoadedByAnonClassLoader) {
        ...
        while (NULL != segment) {
            if ((segment->type & (MEMORY_TYPE_ROM_CLASS | MEMORY_TYPE_ALLOCATED))
                == (MEMORY_TYPE_ROM_CLASS | MEMORY_TYPE_ALLOCATED)) {
    ...
    if (NULL == result) {
        ...
        segment = _javaVM->internalVMFunctions->allocateClassMemorySegment(
            _javaVM, bytesRequired, MEMORY_TYPE_DYNAMIC_LOADED_CLASSES, _classLoader, classAllocationIncrement);
        if (segment != NULL) {
            result = segment->heapAlloc;
        }
    }
...
void ROMClassSegmentAllocationStrategy::updateFinalROMSize(UDATA finalSize)
{
