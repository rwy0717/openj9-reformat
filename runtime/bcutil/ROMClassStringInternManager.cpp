...
ROMClassStringInternManager::ROMClassStringInternManager(ROMClassCreationContext* context,
    StringInternTable* stringInternTable, SRPOffsetTable* srpOffsetTable, SRPKeyProducer* srpKeyProducer,
    U_8* baseAddress, U_8* endAddress, bool isSharedROMClass, bool hasStringTableLock)
    : _context(context)
 ...
 *
 */
void ROMClassStringInternManager::visitUTF8(
    U_16 cpIndex, U_16 utf8Length, U_8* utf8Data, SharedCacheRangeInfo sharedCacheSRPRangeInfo)
{
 ...
 *
 */
void ROMClassStringInternManager::internString(J9UTF8* string)
{
    if (!isInterningEnabled()) {
        return;
    }
