...
public:
    ...
    ~CheckSize() { Trc_BCU_Assert_Equals(_cursor->getCount() - _start, _expectedSize); }
...
public:
    ...
    virtual void visitArray(
        U_16 elementNameIndex, U_16 elementCount, ClassFileOracle::ArrayAnnotationElements* arrayAnnotationElements)
    {
...
class ROMClassWriter::AnnotationWriter : public ROMClassWriter::AnnotationElementWriter,
...
public:
    ...
    void visitDefaultAnnotationAttribute(U_16 fieldOrMethodIndex, U_32 length) { writeAnnotationAttribute(length); }
    ...
    void visitParameter(U_16 numberOfAnnotations) { _cursor->writeBigEndianU16(numberOfAnnotations, Cursor::GENERIC); }
...
private:
    ...
    void writeAnnotationAttribute(U_32 length) { _cursor->writeU32(length, Cursor::GENERIC); /* Native Endian */ }
};

ROMClassWriter::ROMClassWriter(BufferManager* bufferManager, ClassFileOracle* classFileOracle,
    SRPKeyProducer* srpKeyProducer, ConstantPoolMap* constantPoolMap, ROMClassCreationContext* context)
...
ROMClassWriter::~ROMClassWriter() { _bufferManager->free(_methodNotes); }

void ROMClassWriter::writeROMClass(Cursor* cursor, Cursor* lineNumberCursor, Cursor* variableInfoCursor,
    Cursor* utf8Cursor, Cursor* classDataCursor, U_32 romSize, U_32 modifiers, U_32 extraModifiers, U_32 optionalFlags,
    MarkOrWrite markOrWrite)
{
    ...
    if (markAndCountOnly) {
        ...
        cursor->writeSRP(
            _srpKeyProducer->mapCfrConstantPoolIndexToKey(_classFileOracle->getClassNameIndex()), Cursor::SRP_TO_UTF8);
        cursor->writeSRP(_srpKeyProducer->mapCfrConstantPoolIndexToKey(_classFileOracle->getSuperClassNameIndex()),
            Cursor::SRP_TO_UTF8);
        ...
        cursor->writeSRP(_srpKeyProducer->mapCfrConstantPoolIndexToKey(_classFileOracle->getOuterClassNameIndex()),
            Cursor::SRP_TO_UTF8);
...
#if defined(J9VM_OPT_VALHALLA_NESTMATES)
        cursor->writeSRP(_srpKeyProducer->mapCfrConstantPoolIndexToKey(_classFileOracle->getNestHostNameIndex()),
            Cursor::SRP_TO_UTF8);
     ...
     */
    if ((NULL != classDataCursor) && !_srpOffsetTable->isInterned(_intermediateClassDataSRPKey)) {
        classDataCursor->mark(_intermediateClassDataSRPKey);
        classDataCursor->writeData(_context->intermediateClassData(), _context->intermediateClassDataLength(),
            Cursor::INTERMEDIATE_CLASS_DATA);
...
public:
    ...
    void visitMethodType(U_16 cfrCPIndex, U_16 originFlags)
    {
        ...
        _cursor->writeSRP(_srpKeyProducer->mapCfrConstantPoolIndexToKey(cfrCPIndex), Cursor::SRP_TO_UTF8);
        _cursor->writeU32(
            (originFlags << BCT_J9DescriptionCpTypeShift) | BCT_J9DescriptionCpTypeMethodType, Cursor::GENERIC);
    }
    ...
    void visitMethodHandle(U_16 cfrKind, U_16 cfrCPIndex)
    {
        ...
        _cursor->writeU32(
            (cfrKind << BCT_J9DescriptionCpTypeShift) | BCT_J9DescriptionCpTypeMethodHandle, Cursor::GENERIC);
    }
    ...
    void visitConstantDynamic(U_16 bsmIndex, U_16 cfrCPIndex, U_32 primitiveFlag)
    {
        ...
        _cursor->writeU32(
            (bsmIndex << BCT_J9DescriptionCpTypeShift) | BCT_J9DescriptionCpTypeConstantDynamic | primitiveFlag,
            Cursor::GENERIC);
    }
    ...
    void visitFieldOrMethod(U_16 classRefCPIndex, U_16 nameAndSignatureCfrCPIndex)
    {
        ...
        _cursor->writeSRP(_srpKeyProducer->mapCfrConstantPoolIndexToKey(nameAndSignatureCfrCPIndex),
            Cursor::SRP_TO_NAME_AND_SIGNATURE);
    }
...
void ROMClassWriter::writeConstantPool(Cursor* cursor, bool markAndCountOnly)
{
 ...
 * 	4 bytes length of annotation data in bytes, followed by annotation data. Omitted if there are no field
 * annotations. 4 bytes length of type annotation data in bytes followed by type annotation data.  Omitted if there are
 * no type annotations.
 */

void ROMClassWriter::writeFields(Cursor* cursor, bool markAndCountOnly)
{
    ...
    while (iterator.isNotDone()) {
        if (markAndCountOnly) {
            ...
            cursor->writeSRP(
                _srpKeyProducer->mapCfrConstantPoolIndexToKey(iterator.getNameIndex()), Cursor::SRP_TO_UTF8);
            cursor->writeSRP(
                _srpKeyProducer->mapCfrConstantPoolIndexToKey(iterator.getDescriptorIndex()), Cursor::SRP_TO_UTF8);
            ...
            if (iterator.isSynthetic()) {
                /* handle the synthetic attribute. In java 1.5 synthetic may be specified in the access flags as well so
                 * do not unset bit here */
        ...
        if (iterator.isConstant()) { // Check not absolutely necessary, but might provide small performance win
            if (iterator.isConstantFloat() || iterator.isConstantInteger()) {
                ...
                cursor->writeU32(U_32(_constantPoolMap->getROMClassCPIndexForReference(
                                     iterator.getConstantValueConstantPoolIndex())),
                    Cursor::GENERIC);
            }
        }
        ...
        if (iterator.hasGenericSignature()) {
            if (markAndCountOnly) { // TODO check if this is a performance win
                ...
                cursor->writeSRP(_srpKeyProducer->mapCfrConstantPoolIndexToKey(iterator.getGenericSignatureIndex()),
                    Cursor::SRP_TO_UTF8);
            }
        }
        ...
        if (iterator.hasAnnotation()) {
            ...
            _classFileOracle->fieldAnnotationDo(
                iterator.getFieldIndex(), &annotationWriter, &annotationWriter, &annotationWriter);
            cursor->padToAlignment(sizeof(U_32), Cursor::GENERIC);
        }

        if (iterator.hasTypeAnnotation()) {
            AnnotationWriter annotationWriter(cursor, _constantPoolMap, _classFileOracle);
            _classFileOracle->fieldTypeAnnotationDo(
                iterator.getFieldIndex(), &annotationWriter, &annotationWriter, &annotationWriter);
            cursor->padToAlignment(sizeof(U_32), Cursor::GENERIC);
        }
...
const U_8 primitiveArrayTypeCharConversion[]
    = { 0, CFR_STACKMAP_TYPE_BYTE_ARRAY, CFR_STACKMAP_TYPE_CHAR_ARRAY, CFR_STACKMAP_TYPE_DOUBLE_ARRAY, 0,
          CFR_STACKMAP_TYPE_FLOAT_ARRAY, 0, 0, CFR_STACKMAP_TYPE_INT_ARRAY, CFR_STACKMAP_TYPE_LONG_ARRAY, 0, 0, 0, 0, 0,
          0, 0, 0, CFR_STACKMAP_TYPE_SHORT_ARRAY, 0, 0, 0, 0, 0, 0, CFR_STACKMAP_TYPE_BOOL_ARRAY, 0 };
...
class ROMClassWriter::Helper : private ClassFileOracle::ConstantPoolIndexVisitor,
...
public:
    Helper(Cursor* cursor, bool markAndCountOnly, ClassFileOracle* classFileOracle, SRPKeyProducer* srpKeyProducer,
        SRPOffsetTable* srpOffsetTable, ConstantPoolMap* constantPoolMap, UDATA expectedSize)
        : _cursor(cursor)
        , _classFileOracle(classFileOracle)
        , _srpKeyProducer(srpKeyProducer)
    ...
    void writeNameAndSignatureBlock()
    {
        if (!_markAndCountOnly) {
            for (ClassFileOracle::NameAndTypeIterator iterator = _classFileOracle->getNameAndTypeIterator();
                 iterator.isNotDone(); iterator.next()) {
                ...
                if (_constantPoolMap->isNATConstantReferenced(cpIndex)) {
                    ...
                    _cursor->writeSRP(
                        _srpKeyProducer->mapCfrConstantPoolIndexToKey(descriptorIndex), Cursor::SRP_TO_UTF8);
                }
            }
        }
    }
    ...
    {
        if (!_markAndCountOnly) {
            for (ClassFileOracle::UTF8Iterator iterator = _classFileOracle->getUTF8Iterator(); iterator.isNotDone();
                 iterator.next()) {
                U_16 cpIndex = iterator.getCPIndex();
        ...
        if (!_markAndCountOnly) {
            ...
            if (lookupTableCount > 0) {
                ...
                if (lookupTablePaddingLength > 0) {
                    _cursor->writeData(
                        (U_8*)lookupTablePadding, lookupTablePaddingLength * sizeof(U_16), Cursor::GENERIC);
                }
            }
        }
    }
...
private:
    ...
    void visitStackMapObject(U_8 slotType, U_16 classCPIndex, U_16 classNameCPIndex)
    {
        ...
        if ((nameData[0] == '[') && (nameData[nameLength - 1] != ';')) {
             ...
             *
             * The reason for encoding arity - 1 in verification type info in Stack maps for primitive array special
             * cases are: The newarray and anewarray bytecodes assume that the array has only a single dimension. To
             * create a multidimension array, multianewarray must be used. The primitive array access bytecodes (ie:
             * iaload) can only be used on single dimension arrays. aaload must be used to access every dimension prior
             * to the base dimension in a multi-arity primitive array. The constants in vrfytbl.c are based off the
             * constants for the primitive types, and can't have the arity of 1 encoded if the constant is to be used
             * for both purposes. (See rtverify.c verifyBytecodes() - the RTV_ARRAY_FETCH_PUSH & RTV_ARRAY_STORE cases
             * of the switch) In addition, the code all through the verifier assumes this condition. Notes: See
             * util/vrfytbl.c for bytecode tables. See constant definitions in cfreader.h and oti/bytecodewalk.h.
    ...
    void visitStackMapItem(U_8 slotType) { _cursor->writeU8(slotType, Cursor::GENERIC); }

    void visitStackMapFrame(U_16 localsCount, U_16 stackItemsCount, U_16 offsetDelta, U_8 frameType,
    ...
    {
        _cursor->writeSRP(
            _srpKeyProducer->mapCfrConstantPoolIndexToKey(nameAndSignatureIndex), Cursor::SRP_TO_NAME_AND_SIGNATURE);
    }

    void visitSplitEntry(U_16 cpIndex) { _cursor->writeU16(cpIndex, Cursor::GENERIC); }
...
void ROMClassWriter::writeInterfaces(Cursor* cursor, bool markAndCountOnly)
{
    cursor->mark(_interfacesSRPKey);
    UDATA size = UDATA(_classFileOracle->getInterfacesCount()) * sizeof(J9SRP);
    CheckSize _(cursor, size);
    Helper(cursor, markAndCountOnly, _classFileOracle, _srpKeyProducer, _srpOffsetTable, _constantPoolMap, size)
        .writeInterfaces();
}

void ROMClassWriter::writeInnerClasses(Cursor* cursor, bool markAndCountOnly)
{
...
#if defined(J9VM_OPT_VALHALLA_NESTMATES)
void ROMClassWriter::writeNestMembers(Cursor* cursor, bool markAndCountOnly)
{
...
void ROMClassWriter::writeNameAndSignatureBlock(Cursor* cursor)
{
    Helper(cursor, false, _classFileOracle, _srpKeyProducer, _srpOffsetTable, _constantPoolMap, 0)
        .writeNameAndSignatureBlock();
}

void ROMClassWriter::writeUTF8s(Cursor* cursor)
{
    Helper(cursor, false, _classFileOracle, _srpKeyProducer, _srpOffsetTable, _constantPoolMap, 0).writeUTF8Block();
    cursor->padToAlignment(sizeof(U_64), Cursor::GENERIC); // TODO why U_64 alignment and not U_32
}

void ROMClassWriter::writeMethods(
    Cursor* cursor, Cursor* lineNumberCursor, Cursor* variableInfoCursor, bool markAndCountOnly)
{
    /*
     * ****************************** PLEASE READ ***********************************
     *
    ...
    for (ClassFileOracle::MethodIterator iterator = _classFileOracle->getMethodIterator(); iterator.isNotDone();
         iterator.next()) {
        ...
        bool writeDebugInfo = ((iterator.getLineNumbersCount() > 0) || (iterator.getLocalVariablesCount() > 0))
        ...
        if (iterator.isAbstract()) {
            ...
            writeNativeSignature(
                cursor, _classFileOracle->getUTF8Data(iterator.getDescriptorIndex()), byteCodeSize - 2);
        } else {
        ...
        if ((0 != iterator.getExceptionHandlersCount()) || (0 != iterator.getExceptionsThrownCount())) {
            ...
            UDATA size = UDATA(iterator.getExceptionHandlersCount()) * sizeof(J9ExceptionHandler)
                + UDATA(iterator.getExceptionsThrownCount()) * sizeof(J9SRP);
            CheckSize _(cursor, size);
            Helper(cursor, markAndCountOnly, _classFileOracle, _srpKeyProducer, _srpOffsetTable, _constantPoolMap, size)
                .writeExceptionBlock(&iterator);
        }
        ...
        if (iterator.hasAnnotationsData()) {
             ...
             */
            AnnotationWriter annotationWriter(cursor, _constantPoolMap, _classFileOracle);
            _classFileOracle->methodAnnotationDo(
                iterator.getIndex(), &annotationWriter, &annotationWriter, &annotationWriter);
            cursor->padToAlignment(sizeof(U_32), Cursor::GENERIC);
        }
        ...
        if (iterator.hasParameterAnnotations()) {
             ...
             */
            AnnotationWriter annotationWriter(cursor, _constantPoolMap, _classFileOracle);
            _classFileOracle->parameterAnnotationsDo(
                iterator.getIndex(), &annotationWriter, &annotationWriter, &annotationWriter);
            cursor->padToAlignment(sizeof(U_32), Cursor::GENERIC);
        }
        ...
        if (iterator.hasMethodTypeAnnotations()) {
            AnnotationWriter annotationWriter(cursor, _constantPoolMap, _classFileOracle);
            _classFileOracle->methodTypeAnnotationDo(
                iterator.getIndex(), &annotationWriter, &annotationWriter, &annotationWriter);
            cursor->padToAlignment(sizeof(U_32), Cursor::GENERIC);
        }
        ...
        if (iterator.hasCodeTypeAnnotations()) {
            AnnotationWriter annotationWriter(cursor, _constantPoolMap, _classFileOracle);
            _classFileOracle->methodCodeTypeAnnotationDo(
                iterator.getIndex(), &annotationWriter, &annotationWriter, &annotationWriter);
            cursor->padToAlignment(sizeof(U_32), Cursor::GENERIC);
        }
        ...
        if (writeDebugInfo || existingDebugData) {
            ...
            if (!_context->shouldWriteDebugDataInline()) {
                cursor->writeSRP(_srpKeyProducer->mapMethodIndexToMethodDebugInfoKey(iterator.getIndex()),
                    Cursor::SRP_TO_DEBUG_DATA);
            }
        ...
        if (iterator.hasStackMap()) {
            ...
            cursor->writeBigEndianU16(
                iterator.getStackMapFramesCount(), Cursor::GENERIC); /* TODO: don't write this stuff in BigEndian??? */

            Helper(cursor, false, _classFileOracle, _srpKeyProducer, _srpOffsetTable, _constantPoolMap, 0)
                .writeStackMap(&iterator);
            cursor->padToAlignment(sizeof(U_32), Cursor::GENERIC);
            if (markAndCountOnly) {
                /* Following is adding PAD to stackmap size. First round is always markAndCountOnly.
                 * This logic is very difficult to catch. Cause of the padded stackmapsize, we dont use padding in
                 * nextROMMethod() in mthutil. Also I find this markAndCountOnly unneccesary and confusing for the
                 * following reasons
        ...
        if (0 != iterator.hasMethodParameters()) {
            ...
            if (true == markAndCountOnly) {
                ...
                CheckSize _(cursor, size);
                Helper(cursor, markAndCountOnly, _classFileOracle, _srpKeyProducer, _srpOffsetTable, _constantPoolMap,
                    size)
                    .writeMethodParameters(&iterator);
            }
            cursor->padToAlignment(sizeof(U_32), Cursor::GENERIC);
        }
    }
...
void ROMClassWriter::writeConstantPoolShapeDescriptions(Cursor* cursor, bool markAndCountOnly)
{
    cursor->mark(_cpDescriptionShapeSRPKey);
    UDATA size = ((UDATA(_constantPoolMap->getROMConstantPoolCount()) + J9_CP_DESCRIPTIONS_PER_U32 - 1)
                     / J9_CP_DESCRIPTIONS_PER_U32)
        * sizeof(U_32);
...
void ROMClassWriter::writeAnnotationInfo(Cursor* cursor)
{
...
void ROMClassWriter::AnnotationElementWriter::visitNestedAnnotation(
    U_16 elementNameIndex, ClassFileOracle::NestedAnnotation* nestedAnnotation)
{
    _cursor->writeU8('@', Cursor::GENERIC);
...
void ROMClassWriter::writeMethodDebugInfo(ClassFileOracle::MethodIterator* methodIterator, Cursor* lineNumberCursor,
    Cursor* variableInfoCursor, bool markAndCountOnly, bool existHasDebugInformation)
{
    /*
    ...
    if (preserveLineNumbers || preserveLocalVariablesInfo || existHasDebugInformation) {
        ...
        if (!(_context->shouldWriteDebugDataInline())) {
            lineNumberCursor->writeSRP(_srpKeyProducer->mapMethodIndexToVariableInfoKey(methodIterator->getIndex()),
                Cursor::SRP_TO_LOCAL_VARIABLE_DATA);
        } else {
            ...
            lineNumberCursor->writeU32(
                _methodNotes[methodIterator->getIndex()].debugInfoSize | 1, Cursor::METHOD_DEBUG_SIZE);
        }

        /* Encode the lineNumbersCount with the lineNumbersInfoCompressedSize
         * a = number of bytes in the compressed line number table
         * b = lineNumberCount
         * c = escape bit (0: use a and b, 1: lineNumberCount uses 31 bit and number of bytes in the compressed line
         * number table will be stored as the next U_32) aaaaaaaa aaaaaaaa bbbbbbbb bbbbbbbc
         *
        ...
        lineNumberCursor->writeData(methodIterator->getLineNumbersInfoCompressed(),
            methodIterator->getLineNumbersInfoCompressedSize(), Cursor::LINE_NUMBER_DATA);
        ...
        if (0 != localVariablesCount) {
            ...
            for (ClassFileOracle::LocalVariablesIterator localVariablesIterator
                ...
                variableInfoCursor->writeSRP(
                    _srpKeyProducer->mapCfrConstantPoolIndexToKey(localVariablesIterator.getNameIndex()),
                    Cursor::LOCAL_VARIABLE_DATA_SRP_TO_UTF8);
                variableInfoCursor->writeSRP(
                    _srpKeyProducer->mapCfrConstantPoolIndexToKey(localVariablesIterator.getDescriptorIndex()),
                    Cursor::LOCAL_VARIABLE_DATA_SRP_TO_UTF8);
                if (localVariablesIterator.hasGenericSignature()) {
                    variableInfoCursor->writeSRP(_srpKeyProducer->mapCfrConstantPoolIndexToKey(
...
void ROMClassWriter::writeSourceDebugExtension(Cursor* cursor)
{
    if ((_classFileOracle->hasSourceDebugExtension() && _context->shouldPreserveSourceDebugExtension())
        ...
        cursor->writeData(_classFileOracle->getSourceDebugExtensionData(),
            _classFileOracle->getSourceDebugExtensionLength(), Cursor::SOURCE_DEBUG_EXT_DATA);
        cursor->padToAlignment(sizeof(U_32), Cursor::GENERIC);
    }
}

void ROMClassWriter::writeOptionalInfo(Cursor* cursor)
{
    ...
    if (_classFileOracle->hasEnclosingMethod()) {
        ...
        cursor->writeU32(
            _constantPoolMap->getROMClassCPIndexForReference(_classFileOracle->getEnclosingMethodClassRefIndex()),
            Cursor::GENERIC);
        cursor->writeSRP(
            _srpKeyProducer->mapCfrConstantPoolIndexToKey(_classFileOracle->getEnclosingMethodNameAndSignatureIndex()),
            Cursor::SRP_TO_NAME_AND_SIGNATURE);
    }
    ...
    if ((_classFileOracle->hasSourceFile() && _context->shouldPreserveSourceFileName())
        ...
        cursor->writeSRP(_srpKeyProducer->mapCfrConstantPoolIndexToKey(_classFileOracle->getSourceFileIndex()),
            Cursor::OPTINFO_SOURCE_FILE_NAME);
    }

    if (_classFileOracle->hasGenericSignature()) {
        cursor->writeSRP(_srpKeyProducer->mapCfrConstantPoolIndexToKey(_classFileOracle->getGenericSignatureIndex()),
            Cursor::SRP_TO_UTF8);
    }
    ...
    if (_classFileOracle->hasSimpleName()) {
        cursor->writeSRP(
            _srpKeyProducer->mapCfrConstantPoolIndexToKey(_classFileOracle->getSimpleNameIndex()), Cursor::SRP_TO_UTF8);
    }

    if (_classFileOracle->hasVerifyExcludeAttribute()) {
        cursor->writeU32(0, Cursor::GENERIC); /* This is required for getSRPPtr in util/optinfo.c to work (counts bits
                                                 in romClass->optionalFlags) */
    }
...
void ROMClassWriter::writeCallSiteData(Cursor* cursor, bool markAndCountOnly)
{
...
void ROMClassWriter::writeVarHandleMethodTypeLookupTable(Cursor* cursor, bool markAndCountOnly)
{
    if (_constantPoolMap->hasVarHandleMethodRefs()) {
        ...
        CheckSize _(cursor, size);
        Helper(cursor, markAndCountOnly, _classFileOracle, _srpKeyProducer, _srpOffsetTable, _constantPoolMap, size)
            .writeVarHandleMethodTypeLookupTable();
    }
}

void ROMClassWriter::writeStaticSplitTable(Cursor* cursor, bool markAndCountOnly)
{
    if (_constantPoolMap->hasStaticSplitTable()) {
        ...
        CheckSize _(cursor, size);
        Helper(cursor, markAndCountOnly, _classFileOracle, _srpKeyProducer, _srpOffsetTable, _constantPoolMap, size)
            .writeStaticSplitTable();
    }
}

void ROMClassWriter::writeSpecialSplitTable(Cursor* cursor, bool markAndCountOnly)
{
    if (_constantPoolMap->hasSpecialSplitTable()) {
        ...
        CheckSize _(cursor, size);
        Helper(cursor, markAndCountOnly, _classFileOracle, _srpKeyProducer, _srpOffsetTable, _constantPoolMap, size)
            .writeSpecialSplitTable();
    }
}

void ROMClassWriter::writeByteCodes(Cursor* cursor, ClassFileOracle::MethodIterator* methodIterator)
{
...
U_32 ROMClassWriter::computeNativeSignatureSize(U_8* methodDescriptor)
{
...
void ROMClassWriter::writeNativeSignature(Cursor* cursor, U_8* methodDescriptor, U_8 nativeArgCount)
{
    /* mapping characters A..Z */
    static const U_8 nativeArgCharConversion[]
        = { 0, PARAM_BYTE, PARAM_CHAR, PARAM_DOUBLE, 0, PARAM_FLOAT, 0, 0, PARAM_INT, PARAM_LONG, 0, PARAM_OBJECT, 0, 0,
              0, 0, 0, 0, PARAM_SHORT, 0, 0, PARAM_VOID, 0, 0, 0, PARAM_BOOLEAN };
