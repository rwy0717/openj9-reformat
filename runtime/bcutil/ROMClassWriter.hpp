...
public:
    ROMClassWriter(BufferManager* bufferManager, ClassFileOracle* classFileOracle, SRPKeyProducer* srpKeyProducer,
        ConstantPoolMap* constantPoolMap, ROMClassCreationContext* context);
    ~ROMClassWriter();
    ...
    void setSRPOffsetTable(SRPOffsetTable* srpOffsetTable)
    {
        ...
        if (_context->isIntermediateDataAClassfile()) {
             ...
             * otherwise we would end up with intermediateClassData of shared ROMClass pointing to private memory of the
             * JVM.
             *
             * Do not re-use intermediateClassData if class is an anonClass. AnonClasses can be unloaded separately
             * which would cause a problem if romClasses reference each other
             */
    ...
    {
        if ((false == _context->isReusingIntermediateClassData()) && (true == _context->isRetransformAllowed())
            && (true == _context->isRetransforming())) {
            _srpOffsetTable->setInternedAt(
                _intermediateClassDataSRPKey, _context->getIntermediateClassDataFromPreviousROMClass());
            _context->setReusingIntermediateClassData();
        }
    }

    enum MarkOrWrite { MARK_AND_COUNT_ONLY, WRITE };

    /*
     * write out the entire ROMClass, including UTF8s
     */
    void writeROMClass(Cursor* cursor, Cursor* lineNumberCursor, Cursor* variableInfoCursor, Cursor* utf8Cursor,
        Cursor* classDataCursor, U_32 romSize, U_32 modifiers, U_32 extraModifiers, U_32 optionalFlags,
    ...
    U_32 getVarHandleMethodTypePaddedSize()
    {
        return _constantPoolMap->getVarHandleMethodTypePaddedCount() * sizeof(U_16);
    }
...
private:
    ...
    void writeMethodDebugInfo(ClassFileOracle::MethodIterator* methodIterator, Cursor* lineNumberCursor,
