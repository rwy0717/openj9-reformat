...
void SRPOffsetTable::insert(UDATA key, UDATA offset, UDATA tag)
{
...
bool SRPOffsetTable::isNotNull(UDATA key) { return _table[key].marked || _table[key].interned; }

bool SRPOffsetTable::isInterned(UDATA key) { return _table[key].interned; }

void SRPOffsetTable::setInternedAt(UDATA key, U_8* address)
{
...
void SRPOffsetTable::setBaseAddressForTag(UDATA tag, U_8* address)
{
    Trc_BCU_Assert_NotGreaterThan(tag, _maxTag);
...
U_8* SRPOffsetTable::getBaseAddressForTag(UDATA tag)
{
    Trc_BCU_Assert_NotGreaterThan(tag, _maxTag);
...
void SRPOffsetTable::clear() { memset(_table, 0, sizeof(Entry) * (_maxKey + 1)); }
