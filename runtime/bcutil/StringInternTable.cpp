...
static UDATA internHashEqualFn(void* leftKey, void* rightKey, void* userData)
{
...
static UDATA internHashFn(void* key, void* userData)
{
    UDATA length;
    U_8* data;
...
static void internHashClassLoadersUnloadHook(J9HookInterface** vmHooks, UDATA eventNum, void* eventData, void* userData)
{
    ...
    , _maximumNodeCount(maximumNodeCount)
{
    if (0 != maximumNodeCount) {
        _internHashTable = hashTableNew(OMRPORT_FROM_J9PORT(_portLibrary), J9_GET_CALLSITE(),
            U_32(maximumNodeCount + 1), sizeof(J9InternHashTableEntry), sizeof(char*), 0, J9MEM_CATEGORY_CLASSES,
            internHashFn, internHashEqualFn, NULL, vm);

#if defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING)
        if ((NULL != _vm) && (NULL != _internHashTable)) {
            J9HookInterface** vmHooks = _vm->internalVMFunctions->getVMHookInterface(vm);
            if (0
                != (*vmHooks)->J9HookRegisterWithCallSite(vmHooks, J9HOOK_VM_CLASS_LOADERS_UNLOAD,
                       internHashClassLoadersUnloadHook, OMR_GET_CALLSITE(), this)) {
    ...
    if (NULL != _internHashTable) {
        ...
        if (NULL != _vm) {
            ...
            (*vmHooks)->J9HookUnregister(
                vmHooks, J9HOOK_VM_CLASS_LOADERS_UNLOAD, internHashClassLoadersUnloadHook, this);
        }
#endif /* J9VM_GC_DYNAMIC_CLASS_UNLOADING */
    }
}

bool StringInternTable::findUtf8(J9InternSearchInfo* searchInfo, J9SharedInvariantInternTable* sharedInternTable,
    bool isSharedROMClass, J9InternSearchResult* result)
{
     ...
     * If ROM class is shared, then UTF8 can be used without SRP range check. (isSharedCacheInRange =
     ...
     * 	 	1 (SC_COMPLETELY_OUT_OF_THE_SRP_RANGE): Shared cache is not in the SRP range. UTF8 can not be used by local
     * ROM class. 2 (SC_COMPLETELY_IN_THE_SRP_RANGE): Shared cache is in the SRP range. UTF8 can be used by local ROM
     * class safely without need for SRP range check. 3 (SC_PARTIALLY_IN_THE_SRP_RANGE): Part of the shared cache is in
     * the SRP range. UTF8 can be used if it is in the SRP range of local ROM class. SRP range check is required.
     *
     */
    if ((SC_COMPLETELY_IN_THE_SRP_RANGE == searchInfo->sharedCacheSRPRangeInfo)
        || (SC_PARTIALLY_IN_THE_SRP_RANGE == searchInfo->sharedCacheSRPRangeInfo)) {
        if (NULL != sharedInternTable) {
            ...
            J9SharedInternSRPHashTableEntry* sharedNode = (J9SharedInternSRPHashTableEntry*)srpHashTableFind(
                sharedInternTable->sharedInvariantSRPHashtable, &sharedQuery);
            if (NULL != sharedNode) {
    ...
    if (NULL != node) {
        bool sharedUtf8 = (STRINGINTERNTABLES_NODE_FLAG_UTF8_IS_SHARED
            == (node->flags & STRINGINTERNTABLES_NODE_FLAG_UTF8_IS_SHARED));
        /**
         * node->utf8 can be used in 3 conditions out of 4.
         * 1. sharedUTF8 && isSharedROMClass	-	node->utf8 can be used and no SRP range check is required for 64
         * bit environment. (isSharedCacheInRange = SC_COMPLETELY_IN_THE_SRP_RANGE)
         * 2. sharedUTF8 && !isSharedROMClass	-	node->utf8 maybe used if it is in SRP range.
         * 											(isSharedCacheInRange =
         * SC_COMPLETELY_OUT_OF_THE_SRP_RANGE, SC_COMPLETELY_IN_THE_SRP_RANGE or SC_PARTIALLY_IN_THE_SRP_RANGE)
         * 3. !sharedUTF8 && isSharedROMClass	-	node->utf8 can not be used.
         * 											ROM classes in shared cache can only use sharedUTF8s
         * in the shared cache.
        ...
        if (sharedUtf8 || !isSharedROMClass) {
            ...
            if (!isSharedROMClass) {
                /* Condition 2 or 4 */
                if ((sharedUtf8 && (SC_PARTIALLY_IN_THE_SRP_RANGE == searchInfo->sharedCacheSRPRangeInfo))
                    || !sharedUtf8) {
...
void StringInternTable::markNodeAsUsed(J9InternSearchResult* result, J9SharedInvariantInternTable* sharedTable)
{
    VERIFY_ENTER();
...
void StringInternTable::internUtf8(
    J9UTF8* utf8, J9ClassLoader* classLoader, bool fromSharedROMClass, J9SharedInvariantInternTable* sharedInternTable)
{
    Trc_BCU_Assert_True(NULL != utf8);
...
#if defined(J9VM_OPT_SHARED_CLASSES)
    if ((NULL != sharedInternTable) && (0 == (sharedInternTable->flags & J9AVLTREE_DISABLE_SHARED_TREE_UPDATES))
        && fromSharedROMClass) {
        J9SharedInternSRPHashTableEntry* insertedEntry = insertSharedNode(sharedInternTable, utf8, 0,
            STRINGINTERNTABLES_NODE_FLAG_UTF8_IS_SHARED, /* promoteIfExistingFound = */ true);

        if (NULL != insertedEntry) {
            VERIFY_EXIT();
            return;
        } else {
            Trc_BCU_getNewStringTableNode_SharedTreeFull(
                sharedInternTable->sharedInvariantSRPHashtable->srpHashtableInternal->tableSize);
        }
    }
#endif
...
J9InternHashTableEntry* StringInternTable::insertLocalNode(J9InternHashTableEntry* node, bool promoteIfExistingFound)
{
...
void StringInternTable::deleteLocalNode(J9InternHashTableEntry* node)
{
    removeNodeFromList(node);
    hashTableRemove(_internHashTable, node);
}

void StringInternTable::removeNodeFromList(J9InternHashTableEntry* node)
{
...
void StringInternTable::promoteNodeToHead(J9InternHashTableEntry* node)
{
    Trc_BCU_Assert_True(NULL != node);
...
#define VERIFY_ASSERT(cond)                                                                                      \
    do {                                                                                                         \
        if (!(cond)) {                                                                                           \
            ...
            j9tty_printf(                                                                                        \
                PORTLIB, "StringInternTable verification condition [" #cond "] failed at %s:%d!\n", file, line); \
...
J9SharedInternSRPHashTableEntry* StringInternTable::insertSharedNode(
    J9SharedInvariantInternTable* table, J9UTF8* utf8, U_16 internWeight, U_16 flags, bool promoteIfExistingFound)
{
    J9SharedInternHashTableQuery sharedQuery;
    ...
    J9SharedInternSRPHashTableEntry* insertedNode
        = (J9SharedInternSRPHashTableEntry*)srpHashTableAdd(table->sharedInvariantSRPHashtable, &sharedQuery);
    if (insertedNode) {
...
void StringInternTable::deleteSharedNode(J9SharedInvariantInternTable* table, J9SharedInternSRPHashTableEntry* node)
{
...
void StringInternTable::removeSharedNodeFromList(
    J9SharedInvariantInternTable* table, J9SharedInternSRPHashTableEntry* sharedNode)
{
 ...
 */
void StringInternTable::updateLocalNodeWeight(J9InternHashTableEntry* node)
{
 ...
 */
void StringInternTable::updateSharedNodeWeight(
    J9SharedInvariantInternTable* table, J9SharedInternSRPHashTableEntry* sharedNode)
{
    J9UTF8* utf8 = J9SHAREDINTERNSRPHASHTABLEENTRY_UTF8SRP(sharedNode);
 ...
 */
bool StringInternTable::testNodePromotionWeight(J9SharedInvariantInternTable* table, J9InternHashTableEntry* node,
    J9SharedInternSRPHashTableEntry* sharedNodeToDisplace)
{
    /* Only allow a node into the shared tree IF:
     * - If J9AVLTREE_DISABLE_SHARED_TREE_UPDATES is not set
     * - It is STRINGINTERNTABLENODE_FLAG_UTF8_IS_SHARED. This means that the string it points to is shared and has been
     * succesfully relocated.
...
void StringInternTable::swapLocalNodeWithTailSharedNode(
    J9InternHashTableEntry* node, J9SharedInvariantInternTable* table)
{
    ...
    J9SharedInternSRPHashTableEntry* insertedSharedNode
        = insertSharedNode(table, node->utf8, node->internWeight, node->flags, FALSE);
    deleteLocalNode(node);
...
void StringInternTable::promoteSharedNodeToHead(
    J9SharedInvariantInternTable* table, J9SharedInternSRPHashTableEntry* node)
{
