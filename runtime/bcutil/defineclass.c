...
static J9ROMClass* createROMClassFromClassFile(
    J9VMThread* currentThread, J9LoadROMClassData* loadData, J9TranslationLocalBuffer* localBuffer);
...
static UDATA callDynamicLoader(J9JavaVM* vm, J9LoadROMClassData* loadData, U_8* intermediateClassData,
    UDATA intermediateClassDataLength, UDATA translationFlags, UDATA classFileBytesReplacedByRIA,
    UDATA classFileBytesReplacedByRCA, J9TranslationLocalBuffer* localBuffer);

static BOOLEAN hasSamePackageName(J9ROMClass* anonROMClass, J9ROMClass* hostROMClass);
static char* createErrorMessage(
    J9VMThread* vmStruct, J9ROMClass* anonROMClass, J9ROMClass* hostROMClass, const char* errorMsg);
static void setIllegalArgumentExceptionHostClassAnonClassHaveDifferentPackages(
    J9VMThread* vmStruct, J9ROMClass* anonROMClass, J9ROMClass* hostROMClass);
 ...
 */
J9Class* internalDefineClass(J9VMThread* vmThread, void* className, UDATA classNameLength, U_8* classData,
    UDATA classDataLength, j9object_t classDataObject, J9ClassLoader* classLoader, j9object_t protectionDomain,
    UDATA options, J9ROMClass* existingROMClass, J9Class* hostClass, J9TranslationLocalBuffer* localBuffer)
{
    ...
    if (!isAnonFlagSet) {
        ...
        if (NULL != classLoader->romClassOrphansHashTable) {
            ...
            if (NULL != orphanROMClass) {
                ...
                /* If enableBCI is specified, class was loaded from shared cache and previous attempt to create RAMClass
                 * failed, we can use the orphanROMClass created in previous attempt.
                 */
    ...
    if (NULL == romClass) {
         ...
         * If ClassFileLoadHook event modifies the class file, it creates a new ROMClass but does not store it in shared
         * class cache.
         */
    ...
    if (romClass) {
        ...
        if ((NULL != hostClass) && (J2SE_VERSION(vm) >= J2SE_19)) {
            ...
            /* From Java 9 and onwards, set IllegalArgumentException when host class and anonymous class have different
             * packages. */
        ...
        result = vm->internalVMFunctions->internalCreateRAMClassFromROMClass(vmThread, classLoader, romClass, options,
            NULL, loadData.protectionDomain, NULL, (IDATA)localBuffer->entryIndex, (IDATA)localBuffer->loadLocationType,
            NULL, hostClass);
        if (NULL == result) {
...
static void throwNoClassDefFoundError(J9VMThread* vmThread, J9LoadROMClassData* loadData)
{
    ...
    if (errorBuf) {
        ...
        vm->internalVMFunctions->setCurrentExceptionUTF(
            vmThread, J9VMCONSTANTPOOL_JAVALANGNOCLASSDEFFOUNDERROR, (char*)errorBuf);
 ...
 * If so, exit the classTableMutex, throw a NoClassDefFoundError (but only if throw-on-fail is set) and return the
 * existing class. Otherwise return NULL.
 */
static J9Class* checkForExistingClass(J9VMThread* vmThread, J9LoadROMClassData* loadData)
{
    J9JavaVM* vm = vmThread->javaVM;
    ...
    existingClass = J9_VM_FUNCTION(vmThread, hashClassTableAt)(
        loadData->classLoader, loadData->className, loadData->classNameLength);
...
internalLoadROMClass(J9VMThread* vmThread, J9LoadROMClassData* loadData, J9TranslationLocalBuffer* localBuffer)
{
    ...
    if ((J9_ARE_NO_BITS_SET(loadData->options, J9_FINDCLASS_FLAG_ANON))
        && (J9_EVENT_IS_HOOKED(vm->hookInterface, J9HOOK_VM_CLASS_LOAD_HOOK)
        ...
        if (J9_ARE_ALL_BITS_SET(loadData->options, J9_FINDCLASS_FLAG_SHRC_ROMCLASS_EXISTS)) {
            ...
            result
                = j9bcutil_transformROMClass(vm, PORTLIB, intermediateROMClass, &classFileBytes, &classFileBytesCount);
            ...
            /* If arraylets are in use, the classData will never be an object, it will have been copied before calling
             * internalDefineClass */
        ...
        if ((loadData->options & J9_FINDCLASS_FLAG_RETRANSFORMING) == 0) {
            ALWAYS_TRIGGER_J9HOOK_VM_CLASS_LOAD_HOOK(vm->hookInterface, vmThread, loadData->classLoader,
                loadData->protectionDomain, loadData->classBeingRedefined, className, loadData->classData,
                loadData->classDataLength, loadData->freeUserData, loadData->freeFunction, classFileBytesReplacedByRIA);
        }
        ...
        ALWAYS_TRIGGER_J9HOOK_VM_CLASS_LOAD_HOOK2(vm->hookInterface, vmThread, loadData->classLoader,
            loadData->protectionDomain, loadData->classBeingRedefined, className, loadData->classData,
            loadData->classDataLength, loadData->freeUserData, loadData->freeFunction, classFileBytesReplacedByRCA);
#endif
    ...
    if (0 != (loadData->options & J9_FINDCLASS_FLAG_SHRC_ROMCLASS_EXISTS)) {
        ...
        if ((FALSE == classFileBytesReplacedByRIA) && (FALSE == classFileBytesReplacedByRCA)) {
     ...
     * classCouldPossiblyBeShared() returns true when the classloader is a shared classes enabled loader AND the cache
     * is NOT full
     *
    ...
    if ((J9VM_DEBUG_ATTRIBUTE_RECORD_ALL == (vm->requiredDebugAttributes & J9VM_DEBUG_ATTRIBUTE_RECORD_ALL))
        ...
        /* either the class is not going to be shared  -or- shared classes does not require the debug information to be
         * maintained */
        UDATA stripFlags = 0;
        ...
        if (stripFlags
            == (BCT_StripDebugVars | BCT_StripDebugLines | BCT_StripDebugSource | BCT_StripSourceDebugExtension)) {
            stripFlags = BCT_StripDebugAttributes;
        }
        translationFlags |= stripFlags;
    }

    if ((0 == (loadData->options & J9_FINDCLASS_FLAG_UNSAFE)) && (0 != (vm->runtimeFlags & J9_RUNTIME_VERIFY))) {
    ...
    if (0 != (vm->runtimeFlags & J9_RUNTIME_XFUTURE)) {
        ...
        if ((vm->systemClassLoader == loadData->classLoader)
            && ((NULL == vm->bytecodeVerificationData)
                   || (0 == (vm->bytecodeVerificationData->verificationFlags & J9_VERIFY_BOOTCLASSPATH_STATIC)))
    ...
    /* TODO toss tracepoint?? Trc_BCU_internalLoadROMClass_AttemptExisting(vmThread, segment, romAvailable,
     * bytesRequired); */
    /* Attempt dynamic load */
    result = callDynamicLoader(vm, loadData, intermediateClassData, intermediateClassDataLength, translationFlags,
        classFileBytesReplacedByRIA, classFileBytesReplacedByRCA, localBuffer);
...
doneFreeMem:
    ...
    if (result == BCT_ERR_NO_ERROR) {
        ...
        BOOLEAN isSharedClassesBCIEnabled
            = (TRUE == isSharedClassesEnabled) && (TRUE == vm->sharedClassConfig->isBCIEnabled(vm));
        ...
        if ((TRUE == isSharedClassesBCIEnabled)
            && ((TRUE == classFileBytesReplacedByRCA) || (TRUE == classFileBytesReplacedByRIA))) {
            Trc_BCU_Assert_True(
                FALSE == j9shr_Query_IsAddressInCache(vm, loadData->romClass, loadData->romClass->romSize));
             ...
             */
            if (J9_ARE_NO_BITS_SET(loadData->options, J9_FINDCLASS_FLAG_RETRANSFORMING) && (NULL != intermediateData)) {
    ...
    if (vm->sharedInvariantInternTable != NULL) {
        /* This null check is added because a seg fault occurs otherwise. With the /g (code optimization) option
         * enabled, the following if-statement never gets executed at run-time as it has an empty body. However, since
         * the /g option is deprecated at Visual Studio 2010 compiler, the following if-statement is now executed at
         * run-time and causes a seg fault. This is only a temporary workaround, and if necessary, it will be fixed as
         * part of Fatih's work.
         */
        if ((vm->sharedInvariantInternTable->flags & J9AVLTREE_DO_VERIFY_TREE_STRUCT_AND_ACCESS)
    ...
    UDATA classFileBytesReplacedByRCA, J9TranslationLocalBuffer* localBuffer)
{
    ...
    /* Pass the verification of stackmaps control flags to be used by static verification (verifyClassFunction -
     * j9bcv_verifyClassStructure) */
    if (vm->bytecodeVerificationData) {
        translationFlags |= (vm->bytecodeVerificationData->verificationFlags
            & (J9_VERIFY_IGNORE_STACK_MAPS | J9_VERIFY_NO_FALLBACK));
    }
    ...
    if (J9_ARE_ANY_BITS_SET(loadData->options, J9_FINDCLASS_FLAG_SHRC_ROMCLASS_EXISTS)) {
        ...
        if (TRUE == classFileBytesReplacedByRCA) {
            if (J9_ARE_ANY_BITS_SET(loadData->options, J9_FINDCLASS_FLAG_RETRANSFORMING)) {
                /* class file bytes are modified during retransformation; use intermediate data from class being
                 * redefined */
    ...
    if (TRUE == createIntermediateROMClass) {
        ...
        if (J9_ARE_ALL_BITS_SET(vm->extendedRuntimeFlags, J9_EXTENDED_RUNTIME_FORCE_CLASSFILE_AS_INTERMEDIATE_DATA)) {
            ...
            rc = j9bcutil_buildRomClass(&intermediateLoadData, NULL, 0, vm, translationFlags, FALSE,
                TRUE /* isIntermediateROMClass */, localBuffer);
    ...
    result = j9bcutil_buildRomClass(loadData, (U_8*)intermediateData, intermediateDataLength, vm, translationFlags,
        classFileBytesReplacedByRIA | classFileBytesReplacedByRCA, FALSE, /* isIntermediateROMClass */
        localBuffer);

    /* The module of a class transformed by a JVMTI agent needs access to unnamed modules */
    if ((J2SE_VERSION(vm) >= J2SE_19) && (classFileBytesReplacedByRIA || classFileBytesReplacedByRCA)
        && (NULL != loadData->romClass)) {
        J9VMThread* currentThread = vm->internalVMFunctions->currentVMThread(vm);
        J9Module* module = vm->internalVMFunctions->findModuleForPackage(
            currentThread, loadData->classLoader, loadData->className, (U_32)packageNameLength(loadData->romClass));
        if (NULL != module) {
            module->isLoose = TRUE;
        }
    }
    ...
    if (J9_ARE_ANY_BITS_SET(vm->extendedRuntimeFlags, J9_EXTENDED_RUNTIME_RECREATE_CLASSFILE_ONLOAD)) {
        if (BCT_ERR_NO_ERROR == result) {
            ...
            if (BCT_ERR_NO_ERROR == result) {
                ...
                result
                    = j9bcutil_buildRomClass(loadData, intermediateData, intermediateDataLength, vm, translationFlags,
                        classFileBytesReplacedByRIA | classFileBytesReplacedByRCA, FALSE, /* isIntermediateROMClass */
                        localBuffer);
    ...
    J9VMThread* currentThread, J9LoadROMClassData* loadData, J9TranslationLocalBuffer* localBuffer)
{
    ...
    if (result == BCT_ERR_NO_ERROR) {
        ...
        Trc_BCU_createROMClassFromClassFile_postLoadNoErr(currentThread, J9UTF8_LENGTH(romName), J9UTF8_DATA(romName),
            classLoader, romClass, NULL); /* TODO update trace point*/
    ...
    case BCT_ERR_CLASS_READ: {
        ...
        /* We don't free vm->dynamicLoadBuffers->classFileError because it is also used as a classFileBuffer in
         * ROMClassBuilder */
        break;
    }
...
static UDATA classCouldPossiblyBeShared(J9VMThread* vmThread, J9LoadROMClassData* loadData)
{
    J9JavaVM* vm = vmThread->javaVM;
    return (
        (0 != (loadData->classLoader->flags & J9CLASSLOADER_SHARED_CLASSES_ENABLED)) && !j9shr_Query_IsCacheFull(vm));
}
    ...
    J9VMThread* vmStruct, J9ROMClass* anonROMClass, J9ROMClass* hostROMClass, const char* errorMsg)
{
    ...
    if (NULL != errorMsg) {
        ...
        bufLen = j9str_printf(
            PORTLIB, NULL, 0, errorMsg, hostClassNameLength, hostClassNameData, anonClassNameLength, anonClassNameData);
        if (bufLen > 0) {
            buf = j9mem_allocate_memory(bufLen, OMRMEM_CATEGORY_VM);
            if (NULL != buf) {
                j9str_printf(PORTLIB, buf, bufLen, errorMsg, hostClassNameLength, hostClassNameData,
    ...
    J9VMThread* vmStruct, J9ROMClass* anonROMClass, J9ROMClass* hostROMClass)
{
    ...
    const char* errorMsg = j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE,
        J9NLS_JCL_HOSTCLASS_ANONCLASS_DIFFERENT_PACKAGES, NULL);
