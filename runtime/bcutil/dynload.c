...
#endif /* J9VM_OPT_ZIP_SUPPORT */
static IDATA convertToOSFilename(
    J9JavaVM* javaVM, U_8* dir, UDATA dirLength, U_8* moduleName, U_8* className, UDATA classNameLength);
static IDATA checkSunClassFileBuffers(J9JavaVM* javaVM, U_32 sunClassFileSize);
static IDATA searchClassInModule(J9VMThread* vmThread, J9Module* j9module, U_8* className, UDATA classNameLength,
    BOOLEAN verbose, J9TranslationLocalBuffer* localBuffer);
static IDATA searchClassInClassPath(J9VMThread* vmThread, J9ClassPathEntry* classPath, UDATA classPathCount,
    U_8* className, UDATA classNameLength, BOOLEAN verbose, J9TranslationLocalBuffer* localBuffer);
static IDATA searchClassInPatchPaths(J9VMThread* vmThread, J9ClassPathEntry* patchPaths, UDATA patchPathCount,
    U_8* className, UDATA classNameLength, BOOLEAN verbose, J9TranslationLocalBuffer* localBuffer);
static IDATA searchClassInCPEntry(J9VMThread* vmThread, J9ClassPathEntry* cpEntry, J9Module* j9module, U_8* moduleName,
    U_8* className, UDATA classNameLength, BOOLEAN verbose);
        ...
        if (NULL == module) {
            if (J9_ARE_NO_BITS_SET(javaVM->runtimeFlags, J9_RUNTIME_JAVA_BASE_MODULE_CREATED)) {
                ...
                if (NULL != packageEnd) {
                    ...
                    module = J9_VM_FUNCTION(vmThread, findModuleForPackage)(
                        vmThread, classLoader, mbString, (U_32)((U_8*)packageEnd - mbString));
        ...
        if (NULL != module) {
            if (NULL != classLoader->moduleExtraInfoHashTable) {
                ...
                if ((NULL != moduleInfo) && (NULL != moduleInfo->patchPathEntries)) {
                    result = searchClassInPatchPaths(vmThread, moduleInfo->patchPathEntries, moduleInfo->patchPathCount,
                        mbString, mbLength, verbose, localBuffer);
                    if (0 == result) {
                        goto _end;
                    }
                }
            ...
            if (1 == result) {
                /* If we failed to find the class in the module passed as parameter and
                 * J9_FINDCLASS_FLAG_FIND_MODULE_ON_FAIL is set, then check the class in other modules defined by the
                 * loader.
                 */
                if ((NULL != j9module) && J9_ARE_ALL_BITS_SET(options, J9_FINDCLASS_FLAG_FIND_MODULE_ON_FAIL)) {
                    ...
                    if (NULL != packageEnd) {
                        ...
                        j9moduleFromPackage = J9_VM_FUNCTION(vmThread, findModuleForPackage)(
                            vmThread, classLoader, className, (U_32)((U_8*)packageEnd - className));

                        if ((NULL != j9moduleFromPackage) && (j9moduleFromPackage != j9module)) {
                            J9ModuleExtraInfo* moduleInfo = J9_VM_FUNCTION(vmThread, findModuleInfoForModule)(
                                vmThread, classLoader, j9moduleFromPackage);

                            if ((NULL != moduleInfo) && (NULL != moduleInfo->patchPathEntries)) {
                                result = searchClassInPatchPaths(vmThread, moduleInfo->patchPathEntries,
                                    moduleInfo->patchPathCount, className, classNameLength, verbose, localBuffer);
                            }
                            if (0 != result) {
                                result = searchClassInModule(
                                    vmThread, j9moduleFromPackage, mbString, mbLength, verbose, localBuffer);
                            }
                        }
                        omrthread_monitor_exit(javaVM->classLoaderModuleAndLocationMutex);
                    }
    ...
    BOOLEAN verbose, J9TranslationLocalBuffer* localBuffer)
{
    ...
    if (j9module == javaVM->javaBaseModule) {
        ...
        moduleName = J9_VM_FUNCTION(vmThread, copyStringToUTF8WithMemAlloc)(vmThread, j9module->moduleName,
    ...
    rc = searchClassInCPEntry(
        vmThread, javaVM->modulesPathEntry, j9module, (U_8*)moduleName, className, classNameLength, verbose);
    if (0 == rc) {
...
static IDATA convertToOSFilename(
    J9JavaVM* javaVM, U_8* dir, UDATA dirLength, U_8* moduleDir, U_8* className, UDATA classNameLength)
{
    ...
    CHECK_BUFFER(javaVM->dynamicLoadBuffers->searchFilenameBuffer, javaVM->dynamicLoadBuffers->searchFilenameSize,
...
static IDATA checkSunClassFileBuffers(J9JavaVM* javaVM, U_32 sunClassFileSize)
{
    ...
    CHECK_BUFFER(javaVM->dynamicLoadBuffers->sunClassFileBuffer, javaVM->dynamicLoadBuffers->sunClassFileSize,
        ROUND_TO(ROUNDING_GRANULARITY, sunClassFileSize));
...
static IDATA convertToClassFilename(J9JavaVM* javaVM, U_8* className, UDATA classNameLength)
{
    ...
    CHECK_BUFFER(javaVM->dynamicLoadBuffers->searchFilenameBuffer, javaVM->dynamicLoadBuffers->searchFilenameSize,
        ROUND_TO(ROUNDING_GRANULARITY, (classNameLength + SUFFIX_LENGTH + 1)));
...
static IDATA readZip(J9JavaVM* javaVM, J9ClassPathEntry* cpEntry)
{
    ...
    result = zipFunctions->zip_getZipEntryWithSize(VMI, zipFile, &entry,
        (char*)javaVM->dynamicLoadBuffers->searchFilenameBuffer, filenameLength, ZIP_FLAG_READ_DATA_POINTER);
    if (result) {
        /* Class not found. */
        result = 1;
        goto finished;
    ...
    result
        = zipFunctions->zip_getZipEntryData(VMI, zipFile, &entry, javaVM->dynamicLoadBuffers->sunClassFileBuffer, size);
    if (result) {
...
/**
 * Attempts to locate file whose name is in the global name search buffer in the JImage file specified by cpEntry in the
 * give module.
 *
...
static IDATA readFileFromJImage(J9VMThread* vmThread, J9Module* j9module, U_8* moduleName, J9ClassPathEntry* cpEntry)
{
    ...
    rc = jimageIntf->jimageFindResource(
        jimageIntf, jimageHandle, (const char*)moduleName, resourceName, &resourceLocation, &size);
    if (J9JIMAGE_NO_ERROR == rc) {
        ...
        if (checkSunClassFileBuffers(javaVM, (U_32)size)) {
            ...
            rc = jimageIntf->jimageGetResource(jimageIntf, jimageHandle, resourceLocation,
                (char*)dynamicLoadBuffers->sunClassFileBuffer, dynamicLoadBuffers->sunClassFileSize, NULL);
            if (J9JIMAGE_NO_ERROR == rc) {
