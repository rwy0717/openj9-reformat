...
static I_32 initJImageIntfCommon(
    J9JImageIntf** jimageIntf, J9JavaVM* vm, J9PortLibrary* portLibrary, UDATA libJImageHandle);
...
static I_32 lookupSymbolsInJImageLib(J9PortLibrary* portLibrary, UDATA libJImageHandle)
{
    ...
    if (0 != rc) {
        j9nls_printf(
            PORTLIB, J9NLS_WARNING, J9NLS_JIMAGE_INTF_LIBJIMAGE_SYMBOL_LOOKUP_FAILED, "JIMAGE_PackageToModule");
        goto _end;
    }
    ...
    UDATA* resourceLocation, I_64* size)
{
    ...
    if (jimageIntf->libJImageHandle) {
        JImageFile* jimage = (JImageFile*)handle;
        JImageLocationRef* locationRef
            = (JImageLocationRef*)j9mem_allocate_memory(sizeof(JImageLocationRef), J9MEM_CATEGORY_CLASSES);
        ;
        ...
        IDATA resourceNameLen = 1 + strlen(moduleName) + 1 + strlen(name)
            + 1; /* +1 for preceding '/' and, +1 for '/' between module and resource name, and +1 for \0 character */
