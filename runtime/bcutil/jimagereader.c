...
I_32 j9bcutil_loadJImage(J9PortLibrary* portlib, const char* fileName, J9JImage** pjimage)
{
    ...
    if (sizeof(JImageHeader) != bytesRead) {
        ...
        Trc_BCU_loadJImage_JImageReadHeaderFailed(
            fileName, portlibErrCode, portlibErrMsg, bytesRead, sizeof(JImageHeader));
...
void j9bcutil_dumpJImageInfo(J9PortLibrary* portlib, J9JImage* jimage)
{
    ...
    for (i = 0; i < jimageHeader->tableLength; i++) {
        ...
        j9tty_printf(PORTLIB, "%-8u| %-16x| %-16x| %-16llu| %-16llu| %s|%s|%s|%s\n", i, locationOffset,
            j9jimageLocation.resourceOffset, j9jimageLocation.compressedSize, j9jimageLocation.uncompressedSize,
    ...
    J9PortLibrary* portlib, J9JImage* jimage, J9JImageLocation* j9jimageLocation, const char* resourceName)
{
    ...
    if (0 == redirectValue) {
        ...
        if (lotIndex >= jimageHeader->tableLength) {
            Trc_BCU_lookupJImageResource_InvalidLocationOffsetsTableIndex(
                jimage->fileName, lotIndex, jimageHeader->tableLength, jimage);
        ...
        if (locationOffset >= JIMAGE_LOCATION_DATA_SIZE(*jimageHeader)) {
            Trc_BCU_lookupJImageResource_InvalidLocationOffset(
                jimage->fileName, locationOffset, JIMAGE_LOCATION_DATA_SIZE(*jimageHeader), jimage);
    ...
    void* imageLocation, J9JImageLocation* j9jimageLocation)
{
    ...
    do {
        ...
        case J9JIMAGE_ATTRIBUTE_TYPE_MODULE:
            ...
            if ((0 == offset) || (offset >= JIMAGE_STRING_DATA_SIZE(*jimageHeader))) {
                Trc_BCU_createAndVerifyJImageLocation_InvalidModuleStringOffset(
                    jimage->fileName, offset, imageLocation, JIMAGE_STRING_DATA_SIZE(*jimageHeader));
        ...
        case J9JIMAGE_ATTRIBUTE_TYPE_PARENT:
            /* Offset 0 is special for empty string. */
            if ((0 == offset) || (offset >= JIMAGE_STRING_DATA_SIZE(*jimageHeader))) {
                Trc_BCU_createAndVerifyJImageLocation_InvalidParentStringOffset(
                    jimage->fileName, offset, imageLocation, JIMAGE_STRING_DATA_SIZE(*jimageHeader));
        ...
        case J9JIMAGE_ATTRIBUTE_TYPE_BASE:
            /* Offset 0 is special for empty string. */
            if ((0 == offset) || (offset >= JIMAGE_STRING_DATA_SIZE(*jimageHeader))) {
                Trc_BCU_createAndVerifyJImageLocation_InvalidBaseStringOffset(
                    jimage->fileName, offset, imageLocation, JIMAGE_STRING_DATA_SIZE(*jimageHeader));
        ...
        case J9JIMAGE_ATTRIBUTE_TYPE_EXTENSION:
            /* Offset 0 is special for empty string. */
            if ((0 == offset) || (offset >= JIMAGE_STRING_DATA_SIZE(*jimageHeader))) {
                Trc_BCU_createAndVerifyJImageLocation_InvalidExtensionStringOffset(
                    jimage->fileName, offset, imageLocation, JIMAGE_STRING_DATA_SIZE(*jimageHeader));
        ...
        case J9JIMAGE_ATTRIBUTE_TYPE_OFFSET:
            if (offset >= (jimage->fileLength - JIMAGE_RESOURCE_AREA_OFFSET(*jimageHeader))) {
                Trc_BCU_createAndVerifyJImageLocation_InvalidResourceOffset(jimage->fileName, offset, imageLocation,
                    jimage->fileLength - JIMAGE_RESOURCE_AREA_OFFSET(*jimageHeader));
        ...
        case J9JIMAGE_ATTRIBUTE_TYPE_COMPRESSED:
            ...
            if (compressedSize >= (jimage->fileLength - JIMAGE_RESOURCE_AREA_OFFSET(*jimageHeader))) {
                Trc_BCU_createAndVerifyJImageLocation_InvalidCompressedSize(jimage->fileName, compressedSize,
                    imageLocation, jimage->fileLength - JIMAGE_RESOURCE_AREA_OFFSET(*jimageHeader));
        ...
        case J9JIMAGE_ATTRIBUTE_TYPE_UNCOMPRESSED:
            ...
            if (uncompressedSize >= (jimage->fileLength - JIMAGE_RESOURCE_AREA_OFFSET(*jimageHeader))) {
                Trc_BCU_createAndVerifyJImageLocation_InvalidUncompressedSize(jimage->fileName, uncompressedSize,
                    imageLocation, jimage->fileLength - JIMAGE_RESOURCE_AREA_OFFSET(*jimageHeader));
    ...
    if (NULL != resourceName) {
        ...
        rc = j9bcutil_getJImageResourceName(
            PORTLIB, jimage, moduleString, parentString, baseString, extensionString, &name);
        if (J9JIMAGE_NO_ERROR != rc) {
            goto _end;
        }

        if (0 != strncmp(name, resourceName, resourceNameLen)) {
            Trc_BCU_createAndVerifyJImageLocation_ResourceNameMismatch(
                jimage->fileName, resourceName, name, imageLocation);
...
I_32 j9bcutil_getJImageResource(
    J9PortLibrary* portlib, J9JImage* jimage, J9JImageLocation* j9jimageLocation, void* dataBuffer, U_64 dataBufferSize)
{
    J9JImageHeader* j9jimageHeader = NULL;
    JImageHeader* jimageHeader = NULL;
    I_64 seekResult = -1;
    ...
    if (-1 == seekResult) {
        I_32 portlibErrCode = j9error_last_error_number();
        const char* portlibErrMsg = j9error_last_error_message();
        Trc_BCU_getJImageResource_JImageFileSeekFailed(
            jimage->fileName, j9jimageLocation->resourceOffset, portlibErrCode, portlibErrMsg);
    ...
    if (0 != j9jimageLocation->compressedSize) {
        ...
        if (j9jimageLocation->compressedSize != bytesRead) {
            ...
            Trc_BCU_getJImageResource_JImageReadResourceDataFailed(
                jimage->fileName, bytesRead, j9jimageLocation->compressedSize, portlibErrCode, portlibErrMsg);
            rc = J9JIMAGE_FILE_READ_ERROR;
            goto _end;
        }
        ...
        do {
            ...
            decompressorName = (char*)((U_8*)jimageHeader + JIMAGE_STRING_DATA_OFFSET(*jimageHeader)
                + decompressorInfo->decompressorNameOffset);
            ...
            if (0 == decompressorInfo->decompressorFlag) {
                ...
                IDATA bytesToCopy = (dataBufferSize < decompressorInfo->uncompressedSize)
                    ? (IDATA)dataBufferSize
                    : (IDATA)decompressorInfo->uncompressedSize;
        ...
        IDATA bytesToRead = (dataBufferSize < j9jimageLocation->uncompressedSize)
            ? (IDATA)dataBufferSize
            : (IDATA)j9jimageLocation->uncompressedSize;
        bytesRead = j9file_read(jimage->fd, dataBuffer, bytesToRead);
        if (bytesToRead != bytesRead) {
            I_32 portlibErrCode = j9error_last_error_number();
            const char* portlibErrMsg = j9error_last_error_message();
            Trc_BCU_getJImageResource_JImageReadResourceDataFailed(
                jimage->fileName, bytesRead, bytesToRead, portlibErrCode, portlibErrMsg);
            rc = J9JIMAGE_FILE_READ_ERROR;
            goto _end;
        } else {
...
const char* j9bcutil_findModuleForPackage(J9PortLibrary* portlib, J9JImage* jimage, const char* package)
{
     ...
     * Eg resource named "/packages/java.lang" contains offset of the name of the module that contains java.lang
     * package.
     */
    ...
    /* "/packages/<package name>" contains 2 tuples of 4 bytes each: isEmpty|Offset. Use the first module that is not
     * empty. */
