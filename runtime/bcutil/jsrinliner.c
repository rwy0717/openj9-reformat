...
static J9JSRIExceptionListEntry* addNewExceptionEntryToList(
    J9JSRIExceptionListEntry** list, J9JSRIExceptionListEntry* entry, J9JSRIData* inlineBuffers);
...
static J9JSRICodeBlock* getCodeBlockParentInChainWithVar(
    J9JSRIJSRData* root, U_16 variable, U_32 currentPC, J9JSRIData* inlineBuffers);
static J9JSRICodeBlock* getNextGreatestBlock(J9JSRIJSRData* jsrData, UDATA* startAddress, J9JSRIData* inlineBuffers);
static void evaluateCodeBlock(I_32 hasRET, J9JSRICodeBlock** blockParentPointer, U_32 startingAddress,
    J9JSRIJSRData* jsrData, J9JSRIData* inlineBuffers);
...
static J9JSRIJSRData* createJSRData(
    J9JSRICodeBlock* parent, J9JSRIJSRData* previous, U_32 spawnPC, U_32 destinationPC, J9JSRIData* inlineBuffers);
...
static void allocateInlineBuffers(J9JSRIData* inlineBuffers)
{
    ...
    if (!inlineBuffers->codeBlockPool) {
        ...
        inlineBuffers->codeBlockPool = pool_new(sizeof(J9JSRICodeBlock), 0, 0, 0, J9_GET_CALLSITE(),
            J9MEM_CATEGORY_CLASSES, POOL_FOR_PORT(inlineBuffers->portLib));
        inlineBuffers->exceptionListEntryPool = pool_new(sizeof(J9JSRIExceptionListEntry), 0, 0, 0, J9_GET_CALLSITE(),
            J9MEM_CATEGORY_CLASSES, POOL_FOR_PORT(inlineBuffers->portLib));
    ...
    inlineBuffers->jsrDataPool
        = pool_new((sizeof(J9JSRIJSRData) + inlineBuffers->maxStack + inlineBuffers->maxLocals + sizeof(UDATA)), 0, 0,
            0, J9_GET_CALLSITE(), J9MEM_CATEGORY_CLASSES, POOL_FOR_PORT(inlineBuffers->portLib));
    ...
    if (!(inlineBuffers->codeBlockPool && inlineBuffers->exceptionListEntryPool && inlineBuffers->jsrDataPool
            && inlineBuffers->map)) {
...
static void correctJumpOffsets(J9JSRIData* inlineBuffers)
{
    ...
    inlineBuffers->bytesAddedByJSRInliner
        += (U_32)inlineBuffers->destBufferIndex - inlineBuffers->codeAttribute->codeLength;
    ...
    J9JSRICodeBlock* parent, J9JSRIJSRData* previous, U_32 spawnPC, U_32 destinationPC, J9JSRIData* inlineBuffers)
{
    ...
    if (newData) {
        ...
        j9tty_printf(inlineBuffers->portLib, "	new jsrdata %p previous %p - jsr = %d, ret = %d\n", newData,
            newData->previousJSR, newData->spawnPC, newData->retPC);
#endif
    } else {
        inlineBuffers->errorCode = BCT_ERR_OUT_OF_MEMORY;
    }
...
static void createNewMap(J9JSRIData* inlineBuffers)
{
    ...
    if (inlineBuffers->mapSize < size) { /* Grow the buffer, too small to reuse */
        ...
        if (inlineBuffers->map) {
            ...
            inlineBuffers->map->lineNumbers
                = j9mem_allocate_memory((UDATA)(size * sizeof(U_16)), J9MEM_CATEGORY_CLASSES);
        }

        if (!inlineBuffers->map || !inlineBuffers->map->entries || !inlineBuffers->map->reachable
            || !inlineBuffers->map->lineNumbers) {
    ...
    for (i = 0; i < inlineBuffers->codeAttribute->exceptionTableLength; i++) {
        ...
        inlineBuffers->map->reachable[entry->handlerPC]
            |= J9JSRI_MAP_MARKED_END_BLOCK + J9JSRI_MAP_MARKED_EXCEPTION_HANDLER;
    }
    ...
    if ((inlineBuffers->flags & BCT_StripDebugAttributes) == 0) {
        ...
        for (i = 0; i < inlineBuffers->codeAttribute->attributesCount; i++) {
            ...
            /* NB: local variable and local variable type tables are inclusive at the end point - either a valid
             * bytecode or 1 past the end */
            if (((inlineBuffers->codeAttribute->attributes[i]->tag == CFR_ATTRIBUTE_LocalVariableTable)
                ...
                localVariableTableAttribute
                    = (J9CfrAttributeLocalVariableTable*)inlineBuffers->codeAttribute->attributes[i];
                for (j = 0; j < localVariableTableAttribute->localVariableTableLength; j++) {
                    inlineBuffers->map->reachable[localVariableTableAttribute->localVariableTable[j].startPC]
                        |= J9JSRI_MAP_MARKED_END_BLOCK;
                    endPC = localVariableTableAttribute->localVariableTable[j].startPC
                        + localVariableTableAttribute->localVariableTable[j].length;
    ...
    *blockParentPointer = thisBlock;
    ...
    while (pc < inlineBuffers->sourceBufferSize) {
        ...
        case CFR_BC_aload_3:
            ...
            if (jsrData && (variable8 < inlineBuffers->maxLocals)) {
                if (jsrData->locals[variable8]) {
                    /* Jazz 82615: Set the verose error type and the local variable index in the case of illegal load
                     * operation */
        ...
        case CFR_BC_ret:
            ...
            thisBlock->secondaryChild
                = getCodeBlockParentInChainWithVar(jsrData, inlineBuffers->sourceBuffer[pc + 1], pc, inlineBuffers);
            if (inlineBuffers->errorCode) {
                return;
            }
        ...
        case CFR_BC_wide:
            ...
            case CFR_BC_aload:
                ...
                if (jsrData && ((UDATA)variable16 < inlineBuffers->maxLocals)) {
                    if (jsrData->locals[variable16]) {
                        /* Jazz 82615: Set the verose error type and the local variable index in the case of illegal
                         * load operation */
                        inlineBuffers->verboseErrorType = BCV_ERR_JSR_ILLEGAL_LOAD_OPERATION;
            ...
            case CFR_BC_ret:
                ...
                NEXT_U16(variable16, bcIndex);
                thisBlock->secondaryChild
                    = getCodeBlockParentInChainWithVar(jsrData, variable16, pc + 1, inlineBuffers);
                if (inlineBuffers->errorCode) {
                    return;
                }
                if (isRetMultiplyUsed(startingAddress, inlineBuffers)) {
        ...
        case CFR_BC_jsr_w:
            ...
            if (hasRET) {
                /* Abandon original and make a new JSR Data for following flows - for reuse of a local to save a return
                 * address */
...
static void flattenCodeBlockHeirarchyToList(J9JSRICodeBlock* root, J9JSRIData* inlineBuffers)
{
#if J9JSRI_DEBUG
    PORT_ACCESS_FROM_PORT(inlineBuffers->portLib);
#endif
    ...
    if (root) {
        ...
        while (root) {
            ...
            if (root->coloured & J9JSRI_COLOUR_FOR_JSR) {
                ...
                memcpy(&(inlineBuffers->destBuffer)[inlineBuffers->destBufferIndex],
                    &(inlineBuffers->sourceBuffer)[root->originalPC], root->length);
            }
...
#if J9JSRI_DEBUG
        j9tty_printf(
            inlineBuffers->portLib, "%p %d at %d - wide\n", root, root->originalPC, inlineBuffers->destBufferIndex);
#endif
...
Looks up the J9JSRICodeBlock that we need to return from by finding the first JSRData in rooted at root with a return
address in variable number variable
...
Notes:
This method could probably be optimized since we will only be needing to check the most recently added element but that
would require knowledge of the implementation which shouldn't be required.  Since this method is called infrequently, it
will be implemented in an implementation-agnostic way.
*/
...
static J9JSRICodeBlock* isMapMarkedAtLocation(UDATA location, J9JSRIJSRData* parentJSR, J9JSRIData* inlineBuffers)
{
    ...
    if (entry && (entry->jsrData->spawnPC == (U_32)-1)) {
        ...
        j9tty_printf(inlineBuffers->portLib,
            "	[][][] is unnested marked block at %d - entry %p - jsr = %d, ret = %d\n", location, entry,
            parentJSR->spawnPC, parentJSR->retPC);
#endif
        return entry;
    }
    ...
    while (entry) {
        if (areDataChainsEqual(entry->jsrData, parentJSR)) {
            ...
            j9tty_printf(inlineBuffers->portLib, "	is marked block at %d - entry %p - jsr = %d, ret = %d\n", location,
                entry, parentJSR->spawnPC, parentJSR->retPC);
#endif
            return entry;
        }
...
static void markMapAtLocation(UDATA location, J9JSRICodeBlock* block, J9JSRIData* inlineBuffers)
{
    ...
    j9tty_printf(inlineBuffers->portLib, "	mark block at %d - jsrdata %p - jsr = %d, ret = %d\n", location,
        block->jsrData, block->jsrData->spawnPC, block->jsrData->retPC);
#endif
...
static void rewriteExceptionHandlers(J9JSRIData* inlineBuffers)
{
    ...
    if (originalExceptions) {
        ...
        while (originalExceptions) {
            ...
            while (entry) {
                if ((entry->originalPC >= originalExceptions->startPC)
                    && (entry->originalPC < originalExceptions->endPC)) {
                        ...
                        _tryOuterJsrData:
                            ...
                            if (exceptionEntry == NULL) {
                                if (jsrData->previousJSR) {
                                    ...
                                    if (exceptionEntry == NULL) {
                                        j9tty_printf(
                                            inlineBuffers->portLib, "[][][] exception handler block not found\n");
                                    }
#endif
                                }
                            } else {
    ...
    inlineBuffers->bytesAddedByJSRInliner
        += ((U_16)exceptionCount - inlineBuffers->codeAttribute->exceptionTableLength) * sizeof(U_16) * 4;
    ...
    for (i = 0; i < inlineBuffers->codeAttribute->attributesCount; i++) {
        ...
        if (inlineBuffers->codeAttribute->attributes[i]->tag == CFR_ATTRIBUTE_LineNumberTable) {
            ...
            if (lineNumberTableLength >= (64 * 1024)) {
                ...
                inlineBuffers->bytesAddedByJSRInliner
                    += ((U_16)lineNumberTableLength - lineNumberTableAttribute->lineNumberTableLength) * sizeof(U_16)
                    * 2;
        ...
        while (uninitializedCount) {
            ...
            while (originalExceptions) {
                ...
                j9tty_printf(inlineBuffers->portLib, "Walk exception %p - %d to %d at %d\n", originalExceptions,
                    originalExceptions->startPC, originalExceptions->endPC, originalExceptions->handlerPC);
#endif
                ...
                if (originalExceptions->jsrData) {
                    ...
                    evaluateCodeBlock(hasRET, &originalExceptions->handlerBlock, originalExceptions->handlerPC, jsrData,
                        inlineBuffers);
...
static void rewriteLocalVariables(J9JSRIData* inlineBuffers, U_8 tableType)
{
        ...
        if (inlineBuffers->codeAttribute->attributes[i]->tag == tableType) {
            ...
            for (j = 0; j < localVariableTableAttribute->localVariableTableLength; j++, local++) {
                ...
                while (block) {
                    ...
                    if ((block->originalPC >= local->startPC)
                        && (block->originalPC < (local->startPC + local->length))) {
    ...
    if (foundTable) {
        inlineBuffers->bytesAddedByJSRInliner
            += ((U_16)x - firstLocalVariableTableAttribute->localVariableTableLength) * sizeof(U_16) * 5;
        ...
        if (entry) {
            ...
            while (entry) {
                ...
                j9tty_printf(inlineBuffers->portLib, "\tpri = %p, sec = %p, jsrDat = %p\n", entry->primaryChild,
                    entry->secondaryChild, entry->jsrData);
    ...
    while (bcIndex < bcEnd) {
        ...
        _collapseTest:
            ...
            if ((inlineBuffers->flags & BCT_Xfuture) == 0) {
                /* Don't perform this optimization with -Xfuture as it removes writes to locals that are supposed to
                 * cause JCK/TCK failures */
                if ((inlineBuffers->sourceBuffer[target] >= CFR_BC_astore_0)
                    && (inlineBuffers->sourceBuffer[target] <= CFR_BC_astore_3)) {
                ...
                } else if ((inlineBuffers->sourceBuffer[target] == CFR_BC_wide)
                    && (inlineBuffers->sourceBuffer[target + 1] == CFR_BC_astore)) {
                    reachable[target] |= J9JSRI_MAP_MARKED_JSR_COLLAPSIBLE;
        ...
        if ((thisBlock->originalPC >= entry->startPC) && (thisBlock->originalPC < entry->endPC)) {
            ...
            if (entry->jsrData == NULL) {
                ...
                j9tty_printf(PORTLIB, "	fill exception %p at %d to %d : %d, spawned %d\n", entry, entry->startPC,
                    entry->endPC, entry->handlerPC, jsrData->spawnPC);
#endif
                ...
                if (remainingEntries == NULL) {
                    ...
                    newEntry
                        = addNewExceptionEntryToList(&(inlineBuffers->originalExceptionTable), entry, inlineBuffers);
                    if (inlineBuffers->errorCode) {
                        return;
                    }
                    newEntry->startPC = entry->startPC;
