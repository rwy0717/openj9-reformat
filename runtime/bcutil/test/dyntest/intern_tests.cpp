...
static bool nodeFieldsEqual(J9InternHashTableEntry* node, J9UTF8* utf8, J9ClassLoader* classLoader,
    J9InternHashTableEntry* nextNode, J9InternHashTableEntry* prevNode);
static bool searchResultFieldsEqual(
    J9InternSearchResult* searchResult, J9UTF8* utf8, J9InternHashTableEntry* node, bool isSharedNode);
static J9SharedInvariantInternTable* createJ9SharedInvariantInternTable(J9PortLibrary* portLib,
    J9ClassLoader* systemClassLoader, J9SharedCacheHeader* dummyHeader, void* allocatedMemory, U_32 memorySize);
static void freeJ9SharedInvariantInternTable(
    J9PortLibrary* portLib, J9SharedInvariantInternTable* invariantInternTable);
...
static bool nodeFieldsEqual(J9InternHashTableEntry* node, J9UTF8* utf8, J9ClassLoader* classLoader,
    J9InternHashTableEntry* nextNode, J9InternHashTableEntry* prevNode)
{
    return (NULL != node) && (utf8 == node->utf8) && (classLoader == node->classLoader) && (nextNode == node->nextNode)
...
static bool searchResultFieldsEqual(
    J9InternSearchResult* searchResult, J9UTF8* utf8, J9InternHashTableEntry* node, bool isSharedNode)
{
    return (NULL != searchResult) && (utf8 == searchResult->utf8) && (node == searchResult->node)
        && (isSharedNode == searchResult->isSharedNode);
}
...
static IDATA testStringInternTableSRPRangeCheck(J9PortLibrary* portLib)
{
    ...
    invariantInternTable
        = createJ9SharedInvariantInternTable(portLib, &dummySystemClassLoader, &dummyHeader, allocatedMemory, 1000);
    ...
    /**
     * Build three UTF8. Make sure that shared one which will be interned into shared string intern table is in the
     * middle regarding to memory addresses. localCommonUTF8 : interned into local string intern table; classLoader =
     * dummyClassLoader; sharedUTF8 = false; sharedCommonUTF8 : interned into shared string intern table; sharedUTF8 =
     * true localCommonUTF8_2 : interned into local string intern table; classLoader = dummyClassLoader2; sharedUTF8 =
     * true
     *
     */
    ...
    stringInternTable.internUtf8(
        sharedCommonUTF8, &dummyClassLoader, true /* fromSharedROMClass */, invariantInternTable);

    /* Utf8s will be interned into local string intern table */
    invariantInternTable->flags = J9AVLTREE_DISABLE_SHARED_TREE_UPDATES;
    stringInternTable.internUtf8(
        localCommonUTF8, &dummyClassLoader, false /* fromSharedROMClass */, invariantInternTable);
    stringInternTable.internUtf8(
        localCommonUTF8_2, &dummyClassLoader2, true /* fromSharedROMClass */, invariantInternTable);
    ...
    /**
     *	Test: When shared cache is out of SRP range, local string table needs to be search and non sharedUTF8 should be
     *returned if one exist. sharedCacheSRPRangeInfo = 1 Success : localCommonUTF8 is found
     */
    /* 1 (SC_COMPLETELY_OUT_OF_THE_SRP_RANGE): Shared cache is out of range */
    searchInfo.sharedCacheSRPRangeInfo = SC_COMPLETELY_OUT_OF_THE_SRP_RANGE;
    searchInfo.classloader = &dummyClassLoader;
    found = stringInternTable.findUtf8(
        &searchInfo, invariantInternTable, /* requiresSharedUtf8 = */ false, &searchResult);
    ...
    /**
     *	Test: When shared cache is out of SRP range, local string table needs to be search and non sharedUTF8 should be
     *returned if one exist. sharedCacheSRPRangeInfo = 1 (SC_COMPLETELY_OUT_OF_THE_SRP_RANGE) Success :
     *localCommonUTF8_2 is not found since it is sharedUTF8
     */
    /* 1 (SC_COMPLETELY_OUT_OF_THE_SRP_RANGE): Shared cache is out of range */
    searchInfo.sharedCacheSRPRangeInfo = SC_COMPLETELY_OUT_OF_THE_SRP_RANGE;
    ...
    /**
     *	Test: If shared cache is partially in range, then range check is required for the found UTF8. In this test case,
     *it should be in range. sharedCacheSRPRangeInfo = 3 (SC_PARTIALLY_IN_THE_SRP_RANGE) Success : sharedCommonUTF8 is
     *found
     */
...
#if defined(J9VM_ENV_DATA64)
    /**
     * Test: If shared cache is partially in range, then range check is required for the found UTF8 in shared cache. In
     * this test case, it should not be in range. sharedUTF8 is out of SRP range sharedCacheSRPRangeInfo = 3
     * (SC_PARTIALLY_IN_THE_SRP_RANGE) Success : one of the local utf8s is found.
     *
     */
     ...
     * 1. sharedUTF8 && isSharedROMClass	-	node->utf8 can be used and no SRP range check is required for 64 bit
     * environment. (sharedCacheSRPRangeInfo = SC_COMPLETELY_IN_THE_SRP_RANGE)
     * 2. sharedUTF8 && !isSharedROMClass	-	node->utf8 maybe used if it if in SRP range.
     * 											(sharedCacheSRPRangeInfo = SC_COMPLETELY_OUT_OF_THE_SRP_RANGE, SC_COMPLETELY_IN_THE_SRP_RANGE or
     * SC_PARTIALLY_IN_THE_SRP_RANGE)
     * 3. !sharedUTF8 && isSharedROMClass	-	node->utf8 can not be used.
     * 											ROM classes in shared cache can only use sharedUTF8s in the shared
     * cache.
    ...
    searchInfo.romClassEndAddr = (U_8*)allocatedMemory + 10000;
    found
        = stringInternTable.findUtf8(&searchInfo, invariantInternTable, /* requiresSharedUtf8 = */ true, &searchResult);
    if (found) {
    ...
    J9ClassLoader* systemClassLoader, J9SharedCacheHeader* dummyHeader, void* allocatedMemory, U_32 memorySize)
{
    ...
    J9SharedInvariantInternTable* invariantInternTable = (J9SharedInvariantInternTable*)j9mem_allocate_memory(
        sizeof(J9SharedInvariantInternTable), J9MEM_CATEGORY_CLASSES);
    if (NULL == invariantInternTable) {
        return NULL;
    }
    memset(invariantInternTable, 0, sizeof(J9SharedInvariantInternTable));
    invariantInternTable->performNodeAction = (UDATA(*)(J9SharedInvariantInternTable*, J9SharedInternSRPHashTableEntry*,
        UDATA, void*))sharedInternTable_performNodeAction;
    ...
    invariantInternTable->sharedInvariantSRPHashtable
        = srpHashTableNewInRegion(portLib, "test_srphashtable", allocatedMemory, memorySize,
            sizeof(J9SharedInternSRPHashTableEntry), 0, sharedInternHashFn, sharedInternHashEqualFn, NULL, NULL);
...
static IDATA testStringInternTableStressShared(J9PortLibrary* portLib, UDATA numIterations)
{
    ...
    memoryUsedBySRPHashtable
        = srpHashTable_requiredMemorySize(utfs8Count, sizeof(J9SharedInternSRPHashTableEntry), TRUE);
    ...
    invariantInternTable = createJ9SharedInvariantInternTable(
        portLib, &classLoaders[0], &dummyHeader, allocatedMemory, memoryUsedBySRPHashtable);
    if (NULL == invariantInternTable) {
        outputErrorMessage(TEST_ERROR_ARGS, "createJ9SharedInvariantInternTable() failed!\n");
        goto _exit_test;
    }
