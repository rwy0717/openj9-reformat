 ...
 *
 */
static IDATA testLineNumberCompressionEdge(J9PortLibrary* portLib, const char* testName, U_16 rangePCLow,
    U_16 rangePCHigh, I_32 rangeLineLow, I_32 rangeLineHigh, I_32 maximumSize)
{
    ...
    for (startPCIndex = rangePCLow; startPCIndex < rangePCHigh; startPCIndex++) {
        ...
        for (lineNumberIndex = rangeLineLow; lineNumberIndex < rangeLineHigh; lineNumberIndex++) {
            ...
            if (lineNumberIndex < 0) {
                /* For the test to work, set the previous line number to 0xFFFF so that the result is positive with a
                 * negative offset */
            ...
            if (!getNextLineNumberFromTable(&bufferPtr, &lineNumber)) {
                outputErrorMessage(
                    TEST_ERROR_ARGS, "error in getNextLineNumberFromTable, byte format not recognized\n");
            }
            if ((lineNumber.lineNumber != lineNumberOffsetIntern) || (lineNumber.location != startPCIndex)) {
                outputErrorMessage(
                    TEST_ERROR_ARGS, "Original   lineNumber:%d, location:%d \n", lineNumberOffsetIntern, startPCIndex);
    ...
    if ((bufferPtr - buffer) > maximumSize) {
        outputErrorMessage(TEST_ERROR_ARGS,
            "Compression is less efficient, it used to take %d bytes and it takes %d bytes \n", maximumSize,
            bufferPtr - buffer);
    }
 ...
 *
 */
static IDATA testLineNumberCompressionDeltaEncoding(
    J9PortLibrary* portLib, const char* testName, I_32 from, I_32 to, I_32 increment)
{
    ...
    for (lineNumberOffset = from; lineNumberOffset != to;
         lineNumberOffset += increment, pcOffset += incrementPositive) {
        J9CfrLineNumberTableEntry lineNumberTableEntry;
...
static IDATA testLineNumberCompressionNegativeOffsetError(J9PortLibrary* portLib, const char* testName)
{
    PORT_ACCESS_FROM_PORT(portLib);
    ...
    if (compressLineNumbers(&line2, 1, &line1, &bufferPtr)) {
        outputErrorMessage(
            TEST_ERROR_ARGS, "Error, compressLineNumbers should give an error when the startPC are not increasing. \n");
    }
    j9mem_free_memory(buffer);
    return reportTestExit(PORTLIB, testName);
}

static IDATA testLineNumberRandomCompressionDecompression(J9PortLibrary* portLib, const char* testName)
{
    PORT_ACCESS_FROM_PORT(portLib);
    J9CfrLineNumberTableEntry* bufferOriginal = (J9CfrLineNumberTableEntry*)j9mem_allocate_memory(
        0xFFFF * sizeof(J9CfrLineNumberTableEntry), OMRMEM_CATEGORY_VM);
    ...
    for (pass = 0; pass < 1000000; pass++) {
        ...
        for (i = 0; i < countLineNumber; i++) {
            ...
            if ((bufferOriginalPtr->startPC != lineNumber.location)
                || (bufferOriginalPtr->lineNumber != lineNumber.lineNumber)) {
	...
	bufferOriginalPtr->startPC(%d) != lineNumber.location(%d) || bufferOriginalPtr->lineNumber(%d) != lineNumber.lineNumber(%d)",
                    bufferOriginalPtr->startPC, lineNumber.location, bufferOriginalPtr->lineNumber,
                    lineNumber.lineNumber);
            }
...
j9dyn_lineNumber_tests(J9PortLibrary* portLib, int randomSeed)
{
    ...
    rc |= testLineNumberCompressionEdge(
        portLib, "testLineNumberCompressionEdge: 0     , 0x1F  , 0     , 3     , 3    ", 0, 0x1F, 0, 3, 3);
    rc |= testLineNumberCompressionEdge(
        portLib, "testLineNumberCompressionEdge: 0     , 0x1F  , -256  , 0xFF  , 1786 ", 0, 0x1F, -256, 0xFF, 1786);
    rc |= testLineNumberCompressionEdge(
        portLib, "testLineNumberCompressionEdge: 0     , 0x7F  , -8192 , 0x1FFF, 65533", 0, 0x7F, -8192, 0x1FFF, 65533);
    rc |= testLineNumberCompressionEdge(portLib, "testLineNumberCompressionEdge: 0x80  , 0x100 , 0x2000, 0x4000, 40960",
        0x80, 0x100, 0x2000, 0x4000, 40960);
    rc |= testLineNumberCompressionEdge(portLib, "testLineNumberCompressionEdge: 0x80  , 0x100 , -10192, -8193 , 9995 ",
        0x80, 0x100, -10192, -8193, 9995);
    rc |= testLineNumberCompressionEdge(portLib, "testLineNumberCompressionEdge: 0xBFFF, 0xFFFF, 0xBFFF, 0xFFFF, 81920",
        0xBFFF, 0xFFFF, 0xBFFF, 0xFFFF, 81920);
    rc |= testLineNumberCompressionEdge(portLib, "testLineNumberCompressionEdge: 0xBFFF, 0xFFFF, -65535, -49150, 81925",
        0xBFFF, 0xFFFF, -65535, -49150, 81925);

    rc |= testLineNumberCompressionDeltaEncoding(
        portLib, "testLineNumberCompressionDeltaEncoding: 0,      0xFFFF, 1  ", 0, 0xFFFF, 1);
    rc |= testLineNumberCompressionDeltaEncoding(
        portLib, "testLineNumberCompressionDeltaEncoding: 0xFFFF, 0,      -1 ", 0xFFFF, 0, -1);
    rc |= testLineNumberCompressionDeltaEncoding(
        portLib, "testLineNumberCompressionDeltaEncoding: 0,      0xFFFF, 15 ", 0, 0xFFFF, 15);
    rc |= testLineNumberCompressionDeltaEncoding(
        portLib, "testLineNumberCompressionDeltaEncoding: 0xFFFF, 0,      -15", 0xFFFF, 0, -15);
