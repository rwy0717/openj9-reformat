 ...
 *
 */
static IDATA testLocalVariableTableCompressionEdge(J9PortLibrary* portLib, const char* testName, I_32 indexLow,
    I_32 indexHigh, I_32 startPCLow, I_32 startPCHigh, I_32 lengthLow, I_32 lengthHigh)
{
        ...
        for (startPCIndex = startPCLow; startPCIndex <= startPCHigh; startPCIndex++) {
            for (lengthIndex = lengthLow; lengthIndex <= lengthHigh; lengthIndex++) {
                ...
                if (values->slotNumber != indexIndex) {
                    outputErrorMessage(
                        TEST_ERROR_ARGS, "Original   slotNumber:%d, indexIndex:%d \n", values->slotNumber, indexIndex);
                }
 ...
 *
 */
static IDATA testLocalVariableTableCompressionDeltaEncoding(
    J9PortLibrary* portLib, const char* testName, I_32 from, I_32 to, I_32 increment)
{
    PORT_ACCESS_FROM_PORT(portLib);
    /* maximum memory possibly needed, the number 13 comes from the fact that compressLocalVariableTableEntry can write
     * from 1 to 13 bytes */
    ...
    startPCIndex = from;
    lengthIndex = from;
    for (indexIndex = from; indexIndex != to;
         indexIndex += increment, startPCIndex += increment, lengthIndex += increment) {
        variableTable += compressLocalVariableTableEntry(
            indexIndex - lastindexIndex, startPCIndex - laststartPCIndex, lengthIndex - lastlengthIndex, variableTable);
...
void printVariableTableValues(
    J9PortLibrary* portLib, const char* testName, I_32* indexIndexes, I_32* startPCIndexes, I_32* lengthIndexes)
{
    PORT_ACCESS_FROM_PORT(portLib);
    UDATA i;
    ...
    for (i = 1; i < variableInfoArraySize; i++) {
        outputErrorMessage(TEST_ERROR_ARGS,
            "VariableInfo %d \t slotNumber = %d \t startVisibility = %d \t visibilityLength = %d\n", i, indexIndexes[i],
            startPCIndexes[i], lengthIndexes[i]);
    }
}
 ...
 *
 */
static IDATA testLocalVariableTableCompressionDecompression(
    J9PortLibrary* portLib, const char* testName, int randomSeed)
{
    PORT_ACCESS_FROM_PORT(portLib);
    /* maximum memory possibly needed, the number 13 comes from the fact that compressLocalVariableTableEntry can write
     * from 1 to 13 bytes */
    ...
    for (pass = 0; pass < 1000000; pass++) {
        ...
        for (i = 1; i < variableInfoArraySize; i++) {
            ...
            variableTable += compressLocalVariableTableEntry(indexIndexes[i] - indexIndexes[i - 1],
                startPCIndexes[i] - startPCIndexes[i - 1], lengthIndexes[i] - lengthIndexes[i - 1], variableTable);
            variableTable += 2 * sizeof(J9SRP);
            if (variableTable < buffer || variableTable > bufferEnd) {
                outputErrorMessage(TEST_ERROR_ARGS, "Buffer overrun \n");
            }
        ...
        for (i = 1; i < variableInfoArraySize; i++) {
            ...
            if (values->slotNumber != indexIndexes[i]) {
                outputErrorMessage(TEST_ERROR_ARGS,
                    "Original   slotNumber:%d, indexIndex:%d , Failed variableInfo = %d, randomSeed = %d \n",
            ...
            if (values->startVisibility != startPCIndexes[i]) {
                outputErrorMessage(TEST_ERROR_ARGS,
                    "Original   startVisibility:%d, startPCIndex:%d , Failed variableInfo = %d, randomSeed = %d  \n",
            ...
            if ((U_16)values->visibilityLength != lengthIndexes[i]) {
                outputErrorMessage(TEST_ERROR_ARGS,
                    "Original   visibilityLength:%d, lengthIndex:%d , Failed variableInfo = %d, randomSeed = %d  \n",
...
j9dyn_localvariabletable_tests(J9PortLibrary* portLib, int randomSeed)
{
    ...
    rc |= testLocalVariableTableCompressionEdge(
        portLib, "testLocalVariableTableCompressionEdge 1bit", 0, 1, 0, 0, -32, 31);
    rc |= testLocalVariableTableCompressionEdge(
        portLib, "testLocalVariableTableCompressionEdge 2bits", 0, 1, -16, 15, -128, 127);
    rc |= testLocalVariableTableCompressionEdge(
        portLib, "testLocalVariableTableCompressionEdge 3bit", 0, 1, -256, 255, -1024, 1023);

    rc |= testLocalVariableTableCompressionEdge(
        portLib, "testLocalVariableTableCompressionEdge 5bit-1", 0, 3, -32768, -30000, -131072, -130000);
    rc |= testLocalVariableTableCompressionEdge(
        portLib, "testLocalVariableTableCompressionEdge 5bit-2", 0, 3, 30000, 32767, -131072, -130000);
    rc |= testLocalVariableTableCompressionEdge(
        portLib, "testLocalVariableTableCompressionEdge 5bit-3", 0, 3, -32768, -30000, 130000, 131071);
    rc |= testLocalVariableTableCompressionEdge(
        portLib, "testLocalVariableTableCompressionEdge 5bit-4", 0, 3, 30000, 32767, 130000, 131071);

    rc |= testLocalVariableTableCompressionEdge(portLib, "testLocalVariableTableCompressionEdge 13bit", 0xFFFFFEFF,
        0xFFFFFFFF, 0xFFFFFEFF, 0xFFFFFFFF, 0xFEFF, 0xFFFF);

    rc |= testLocalVariableTableCompressionDeltaEncoding(
        portLib, "testLocalVariableTableCompressionDeltaEncoding: 0,      0xFFFF, 1  ", 0, 0xFFFF, 1);
    rc |= testLocalVariableTableCompressionDeltaEncoding(
        portLib, "testLocalVariableTableCompressionDeltaEncoding: 0xFFFF, 0,      -1 ", 0xFFFF, 0, -1);
    rc |= testLocalVariableTableCompressionDeltaEncoding(
        portLib, "testLocalVariableTableCompressionDeltaEncoding: 0,      0xFFFF, 15 ", 0, 0xFFFF, 15);
    rc |= testLocalVariableTableCompressionDeltaEncoding(
        portLib, "testLocalVariableTableCompressionDeltaEncoding: 0xFFFF, 0,      -15", 0xFFFF, 0, -15);

    rc |= testLocalVariableTableCompressionDecompression(
        portLib, "testLineNumberRandomCompressionDecompression", randomSeed);
