...
static IDATA testClass(J9PortLibrary* portLib, const char* classFileName, UDATA testMode)
{
    ...
    rc = j9bcutil_buildRomClassIntoBuffer(classFileBytes, classFileSize, PORTLIB, verifyBuffers, flags, 0, 0,
        romClassBuffer, romClassBufferSize, lineNumberBuffer, lineNumberBufferSize, varInfoBuffer, varInfoBufferSize,
        NULL);
    ...
    rc = j9bcutil_compareRomClass(
        classFileBytes, classFileSize, PORTLIB, verifyBuffers, flags, 0, (J9ROMClass*)romClassBuffer);
    if (BCT_ERR_NO_ERROR != rc) {
