...
static void verifyClassFlags(
    J9PortLibrary* portLib, const char* testName, J9ROMClass* romClass, RCCorrectnessConfig* testData)
{
    ...
    if (TEST_J9AccSynthetic & testData->testFlagON) {
        if (0 == J9ROMCLASS_IS_SYNTHETIC(romClass)) {
            outputErrorMessage(
                TEST_ERROR_ARGS, "ROMClass for class: %s does not have J9AccSynthetic set\n", testData->classFileName);
        }
    }
    ...
    if (TEST_J9AccClassHasFinalFields & testData->testFlagOFF) {
        if (0 != J9ROMCLASS_HAS_FINAL_FIELDS(romClass)) {
            outputErrorMessage(
                TEST_ERROR_ARGS, "ROMClass for class: %s has J9AccClassHasFinalFields set\n", testData->classFileName);
        }
    }
    if (TEST_J9AccClassCloneable & testData->testFlagOFF) {
        if (0 != J9ROMCLASS_IS_CLONEABLE(romClass)) {
            outputErrorMessage(
                TEST_ERROR_ARGS, "ROMClass for class: %s has J9AccClassCloneable set\n", testData->classFileName);
        }
    }
    if (TEST_J9AccClassReferenceWeak & testData->testFlagOFF) {
        if (0 != J9ROMCLASS_REFERENCE_WEAK(romClass)) {
            outputErrorMessage(
                TEST_ERROR_ARGS, "ROMClass for class: %s has J9AccClassReferenceWeak set\n", testData->classFileName);
        }
    }
    if (TEST_J9AccClassReferenceSoft & testData->testFlagOFF) {
        if (0 != J9ROMCLASS_REFERENCE_SOFT(romClass)) {
            outputErrorMessage(
                TEST_ERROR_ARGS, "ROMClass for class: %s has J9AccClassReferenceSoft set\n", testData->classFileName);
        }
    }
    ...
    if (TEST_J9AccClassFinalizeNeeded & testData->testFlagOFF) {
        if (0 != J9ROMCLASS_FINALIZE_NEEDED(romClass)) {
            outputErrorMessage(
                TEST_ERROR_ARGS, "ROMClass for class: %s has J9AccClassFinalizeNeeded set\n", testData->classFileName);
        }
    }
    if (TEST_J9AccClassHasVerifyData & testData->testFlagOFF) {
        if (0 != J9ROMCLASS_HAS_VERIFY_DATA(romClass)) {
            outputErrorMessage(
                TEST_ERROR_ARGS, "ROMClass for class: %s has J9AccClassHasVerifyData set\n", testData->classFileName);
        }
    }
    if (TEST_J9AccClassUnsafe & testData->testFlagOFF) {
        if (0 != J9ROMCLASS_IS_UNSAFE(romClass)) {
            outputErrorMessage(
                TEST_ERROR_ARGS, "ROMClass for class: %s has J9AccClassUnsafe set\n", testData->classFileName);
        }
    }
    if (TEST_J9AccSynthetic & testData->testFlagOFF) {
        if (0 != J9ROMCLASS_IS_SYNTHETIC(romClass)) {
            outputErrorMessage(
                TEST_ERROR_ARGS, "ROMClass for class: %s has J9AccSynthetic set\n", testData->classFileName);
        }
    }
}

static IDATA testClass(J9PortLibrary* portLib, RCCorrectnessConfig* testData)
{
    PORT_ACCESS_FROM_PORT(portLib);
    ...
    rc = j9bcutil_buildRomClassIntoBuffer(classFileBytes, classFileSize, PORTLIB, verifyBuffers, flags, 0,
        testData->findClassFlags, romClassBuffer, romClassBufferSize, NULL, 0, NULL, 0, NULL);
...
RCCorrectnessConfig test1 = { "VM.class",

    TEST_J9AccClassHasVerifyData,

    TEST_J9AccClassHasFinalFields | TEST_J9AccClassCloneable | TEST_J9AccClassReferenceWeak
        | TEST_J9AccClassReferenceSoft | TEST_J9AccClassReferencePhantom | TEST_J9AccClassHasEmptyFinalize
        | TEST_J9AccClassFinalizeNeeded | TEST_J9AccSynthetic,

    0,

    0, NULL };

RCCorrectnessConfig test2 = { "PhantomReference.class",

    TEST_J9AccClassReferenceSoft | TEST_J9AccClassReferencePhantom | TEST_J9AccClassReferenceWeak
        | TEST_J9AccClassHasVerifyData,

    TEST_J9AccClassHasFinalFields | TEST_J9AccClassCloneable | TEST_J9AccClassHasEmptyFinalize
        | TEST_J9AccClassFinalizeNeeded | TEST_J9AccSynthetic,
...
RCCorrectnessConfig test3 = { "AbstractClassLoader$3.class",

    TEST_J9AccClassHasFinalFields | TEST_J9AccClassHasVerifyData,

    TEST_J9AccClassReferencePhantom | TEST_J9AccClassReferenceWeak | TEST_J9AccClassCloneable
        | TEST_J9AccClassReferenceSoft | TEST_J9AccClassFinalizeNeeded | TEST_J9AccSynthetic,

    0,

    0, NULL };

RCCorrectnessConfig test4 = { "AttachHandler$1.class",

    TEST_J9AccClassHasVerifyData | TEST_J9AccSynthetic,
...
RCCorrectnessConfig test5 = { "ZipEntry.class",

    TEST_J9AccClassCloneable | TEST_J9AccClassHasVerifyData,

    TEST_J9AccClassHasFinalFields | TEST_J9AccClassHasEmptyFinalize | TEST_J9AccClassReferenceWeak
        | TEST_J9AccClassReferenceSoft | TEST_J9AccClassReferencePhantom | TEST_J9AccClassFinalizeNeeded
        | TEST_J9AccSynthetic,
...
RCCorrectnessConfig test6 = { "ZipStream.class",

    TEST_J9AccClassFinalizeNeeded | TEST_J9AccClassHasVerifyData,
...
RCCorrectnessConfig test7 = { "PhantomReference.class",
