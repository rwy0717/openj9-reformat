...
static VMINLINE IDATA parseLocals(J9BytecodeVerificationData* verifyData, U_8** stackMapData,
    J9BranchTargetStack* liveStack, IDATA localDelta, IDATA localsCount, IDATA maxLocals);
static VMINLINE IDATA parseStack(J9BytecodeVerificationData* verifyData, U_8** stackMapData,
    J9BranchTargetStack* liveStack, UDATA stackCount, UDATA maxStack);
...
static J9UTF8* mergeClasses(J9BytecodeVerificationData* verifyData, U_8* firstClass, UDATA firstLength,
    U_8* secondClass, UDATA secondLength, IDATA* reasonCode);
 ...
 */
static void setInitializedThisStatus(J9BytecodeVerificationData* verifyData)
{
...
static IDATA buildBranchMap(J9BytecodeVerificationData* verifyData)
{
    ...
    while (bcIndex < bcEnd) {
        ...
        if (size == 0) {
            ...
            Trc_BCV_buildBranchMap_UnknownInstruction(verifyData->vmStruct,
                ...
                J9UTF8_DATA(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, romMethod)), bc, verifyData->errorPC,
                verifyData->errorPC);
    ...
    Trc_BCV_buildBranchMap_branchCount(verifyData->vmStruct,
        ...
        J9UTF8_DATA(J9ROMMETHOD_GET_NAME(verifyData->romClass, romMethod)),
        (UDATA)J9UTF8_LENGTH(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, romMethod)),
        J9UTF8_DATA(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, romMethod)), count);
...
static IDATA decompressStackMaps(J9BytecodeVerificationData* verifyData, IDATA localsCount, U_8* stackMapData)
{
    ...
    /* localsCount records the current locals depth as all stack maps (except full frame) are relative to the previous
     * frame */
    for (i = 0; i < (UDATA)verifyData->stackMapsCount; i++) {
        ...
        if (localsCount < 0) {
            ...
            Trc_BCV_decompressStackMaps_LocalsArrayOverFlowUnderFlow(verifyData->vmStruct,
                ...
                J9UTF8_DATA(J9ROMMETHOD_GET_NAME(verifyData->romClass, romMethod)),
                (UDATA)J9UTF8_LENGTH(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, romMethod)),
                J9UTF8_DATA(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, romMethod)), i, mapPC);
            rc = BCV_FAIL;
            break;
        }
        ...
        if (mapPC >= length) {
            ...
            Trc_BCV_decompressStackMaps_MapOutOfRange(verifyData->vmStruct,
                ...
                J9UTF8_DATA(J9ROMMETHOD_GET_NAME(verifyData->romClass, romMethod)),
                (UDATA)J9UTF8_LENGTH(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, romMethod)),
                J9UTF8_DATA(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, romMethod)), i, mapPC, length);
            rc = BCV_FAIL;
            break;
        }
...
static IDATA parseLocals(J9BytecodeVerificationData* verifyData, U_8** stackMapData, J9BranchTargetStack* liveStack,
    IDATA localDelta, IDATA localsCount, IDATA maxLocals)
{
    UDATA i;
    UDATA stackEntry;
    UDATA unusedLocals;
...
static IDATA parseStack(J9BytecodeVerificationData* verifyData, U_8** stackMapData, J9BranchTargetStack* liveStack,
    UDATA stackCount, UDATA maxStack)
{
    UDATA stackEntry;
    ...
    for (; stackCount; stackCount--) {
        ...
        if ((stackEntry == BCV_BASE_TYPE_DOUBLE) || (stackEntry == BCV_BASE_TYPE_LONG)) {
            if ((UDATA)(stackTop - stackBase) >= maxStack) {
                /* Jazz 82615: Set the error code and the location of wrong data type on 'stack' (only keep the maximum
                 * size for stack) */
                verifyData->errorDetailCode = BCV_ERR_STACKMAP_FRAME_STACK_OVERFLOW;
                verifyData->errorCurrentFramePosition = (U_32)(stackBase - liveStack->stackElements);
                if (maxStack > 0) {
...
static UDATA parseElement(J9BytecodeVerificationData* verifyData, U_8** stackMapData)
{
    ...
    if (entryType < CFR_STACKMAP_TYPE_INIT_OBJECT) {
        ...
        stackEntry = convertClassNameToStackMapType(
            verifyData, J9UTF8_DATA(className), J9UTF8_LENGTH(className), BCV_SPECIAL_INIT, 0);
...
static void copyStack(J9BranchTargetStack* source, J9BranchTargetStack* destination)
{
    UDATA pc = destination->pc;

    memcpy((UDATA*)destination, (UDATA*)source,
        (source->stackTopIndex + BCV_TARGET_STACK_HEADER_UDATA_SIZE) * sizeof(UDATA));
...
static IDATA mergeObjectTypes(J9BytecodeVerificationData* verifyData, UDATA sourceType, UDATA* targetTypePointer)
{
        ...
        if (BCV_ERR_INTERNAL_ERROR == reasonCode) {
            *targetTypePointer = (U_32)BCV_JAVA_LANG_OBJECT_INDEX << BCV_CLASS_INDEX_SHIFT;
            Trc_BCV_mergeObjectTypes_UnableToLoadClass(verifyData->vmStruct,
                ...
                J9UTF8_DATA(J9ROMMETHOD_GET_NAME(verifyData->romClass, verifyData->romMethod)),
                (UDATA)J9UTF8_LENGTH(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, verifyData->romMethod)),
                J9UTF8_DATA(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, verifyData->romMethod)), sourceType,
                targetType);
    ...
    if (targetType & BCV_TAG_BASE_ARRAY_OR_NULL) {
        Trc_BCV_mergeObjectTypes_DecayTargetArray(verifyData->vmStruct,
            ...
            J9UTF8_DATA(J9ROMMETHOD_GET_NAME(verifyData->romClass, verifyData->romMethod)),
            (UDATA)J9UTF8_LENGTH(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, verifyData->romMethod)),
            J9UTF8_DATA(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, verifyData->romMethod)), targetType);
    ...
    if (classArity == targetArity) {
        ...
        if (NULL == name) {
            if (BCV_ERR_INSUFFICIENT_MEMORY == reasonCode) {
                ...
                Trc_BCV_mergeObjectTypes_MergeClassesFail(verifyData->vmStruct,
                    ...
                    J9UTF8_DATA(J9ROMMETHOD_GET_NAME(verifyData->romClass, verifyData->romMethod)),
                    (UDATA)J9UTF8_LENGTH(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, verifyData->romMethod)),
                    J9UTF8_DATA(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, verifyData->romMethod)), sourceLength,
                    sourceName, targetLength, targetName);
                *targetTypePointer = sourceType;
                /* cause a re-walk */
                return (IDATA)BCV_FAIL;
            }
        ...
        Trc_BCV_mergeObjectTypes_MergeClassesSucceed(verifyData->vmStruct,
            ...
            (UDATA)J9UTF8_LENGTH(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, verifyData->romMethod)),
            J9UTF8_DATA(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, verifyData->romMethod)), sourceLength,
            sourceName, targetLength, targetName, J9UTF8_LENGTH(name), J9UTF8_DATA(name), classIndex);
        ...
        Trc_BCV_mergeObjectTypes_MergeClassesMinimumArity(verifyData->vmStruct,
            ...
            J9UTF8_DATA(J9ROMMETHOD_GET_NAME(verifyData->romClass, verifyData->romMethod)),
            (UDATA)J9UTF8_LENGTH(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, verifyData->romMethod)),
            J9UTF8_DATA(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, verifyData->romMethod)), classArity,
            targetArity);
    ...
    *targetTypePointer = classArity | (classIndex << BCV_CLASS_INDEX_SHIFT);
    Trc_BCV_mergeObjectTypes_MergedClass(verifyData->vmStruct,
        ...
        J9UTF8_DATA(J9ROMMETHOD_GET_NAME(verifyData->romClass, verifyData->romMethod)),
        (UDATA)J9UTF8_LENGTH(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, verifyData->romMethod)),
        J9UTF8_DATA(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, verifyData->romMethod)), *targetTypePointer);
    /* cause a re-walk */
    return (IDATA)BCV_FAIL;
}
...
static IDATA mergeStacks(J9BytecodeVerificationData* verifyData, UDATA target)
{
    ...
    Trc_BCV_mergeStacks_Entry(verifyData->vmStruct, (UDATA)J9UTF8_LENGTH(J9ROMCLASS_CLASSNAME(verifyData->romClass)),
        J9UTF8_DATA(J9ROMCLASS_CLASSNAME(verifyData->romClass)),
        (UDATA)J9UTF8_LENGTH(J9ROMMETHOD_GET_NAME(verifyData->romClass, verifyData->romMethod)),
        J9UTF8_DATA(J9ROMMETHOD_GET_NAME(verifyData->romClass, verifyData->romMethod)),
        (UDATA)J9UTF8_LENGTH(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, verifyData->romMethod)),
        J9UTF8_DATA(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, verifyData->romMethod)), target, target);
    ...
    if (targetStack->stackBaseIndex == -1) {
        ...
        Trc_BCV_mergeStacks_CopyStack(verifyData->vmStruct,
            ...
            J9UTF8_DATA(J9ROMMETHOD_GET_NAME(verifyData->romClass, verifyData->romMethod)),
            (UDATA)J9UTF8_LENGTH(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, verifyData->romMethod)),
            J9UTF8_DATA(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, verifyData->romMethod)), stackIndex, target,
            target);
        goto _finished;
        ...
        if (targetStack->stackTopIndex != liveStack->stackTopIndex) {
            ...
            Trc_BCV_mergeStacks_DepthMismatch(verifyData->vmStruct,
                ...
                (UDATA)J9UTF8_LENGTH(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, verifyData->romMethod)),
                J9UTF8_DATA(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, verifyData->romMethod)), stackIndex, target,
                target, liveStack->stackTopIndex, targetStack->stackTopIndex);
            goto _finished;
        }
            ...
            if (*sourceStackPtr != *targetStackPtr) {
                ...
                if (sourceStackPtr < sourceStackTemps) {
                    ...
                    if ((sourceItem | targetItem) & (BCV_BASE_OR_SPECIAL)) {
                        ...
                        if ((*sourceStackPtr != BCV_BASE_TYPE_NULL)
                            && (*targetStackPtr != (BCV_JAVA_LANG_OBJECT_INDEX << BCV_CLASS_INDEX_SHIFT))) {
                            ...
                            if (*targetStackPtr == BCV_BASE_TYPE_NULL) {
                                *targetStackPtr = *sourceStackPtr;
                                ...
                                if ((verifyData->verificationFlags & J9_VERIFY_OPTIMIZE) && (maxIndex <= 32)) {
                                    ...
                                    if (mergePC == ((UDATA)-1)) {
                                        mergePC = target;
                                        if (j9localmap_LocalBitsForPC(verifyData->portLib, romClass, romMethod, mergePC,
                                                &resultArrayBase, NULL, NULL, NULL)
                                            != 0) {
                                    ...
                                    if (resultArrayBase & (1 << index)) {
                                        ...
                                        Trc_BCV_mergeStacks_OptMergeRequired(verifyData->vmStruct,
                                            ...
                                            J9UTF8_DATA(J9ROMCLASS_CLASSNAME(verifyData->romClass)),
                                            (UDATA)J9UTF8_LENGTH(
                                                J9ROMMETHOD_GET_NAME(verifyData->romClass, verifyData->romMethod)),
                                            J9UTF8_DATA(
                                                J9ROMMETHOD_GET_NAME(verifyData->romClass, verifyData->romMethod)),
                                            (UDATA)J9UTF8_LENGTH(
...
_finished:
    ...
    Trc_BCV_mergeStacks_Exit(verifyData->vmStruct, (UDATA)J9UTF8_LENGTH(J9ROMCLASS_CLASSNAME(verifyData->romClass)),
        J9UTF8_DATA(J9ROMCLASS_CLASSNAME(verifyData->romClass)),
        (UDATA)J9UTF8_LENGTH(J9ROMMETHOD_GET_NAME(verifyData->romClass, verifyData->romMethod)),
        J9UTF8_DATA(J9ROMMETHOD_GET_NAME(verifyData->romClass, verifyData->romMethod)),
        (UDATA)J9UTF8_LENGTH(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, verifyData->romMethod)),
        J9UTF8_DATA(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, verifyData->romMethod)), rc);
...
static void printMethod(J9BytecodeVerificationData* verifyData)
{
    ...
    printf(" %.*s(", J9UTF8_LENGTH(J9ROMMETHOD_GET_NAME(romClass, method)),
        J9UTF8_DATA(J9ROMMETHOD_GET_NAME(romClass, method)));
...
#define CHECK_END                                     \
    ...
    while (pc < length) {
            ...
            if (checkIfInsideException && tempStoreChange) {
                ...
                for (exception = 0; exception < (UDATA)exceptionData->catchCount; exception++) {
                    ...
                    if (((UDATA)start >= handler->startPC) && ((UDATA)start < handler->endPC)) {
                        ...
                        if (handler->exceptionClassIndex) {
                            /* look up the class in the constant pool */
                            utf8string = J9ROMSTRINGREF_UTF8DATA(
        ...
        case RTV_SEND:
            ...
            if ((JBinvokestatic != bc) && (JBinvokedynamic != bc) && (JBinvokestaticsplit != bc)) {
                if ((JBinvokespecial == bc) || (JBinvokespecialsplit == bc)) {
    ...
    _checkFinished:
        ...
        if (verifyData->unwalkedQueueHead != verifyData->unwalkedQueueTail) {
            ...
            Trc_BCV_simulateStack_NewWalkFrom(verifyData->vmStruct,
                ...
                J9UTF8_DATA(J9ROMMETHOD_GET_NAME(verifyData->romClass, romMethod)),
                (UDATA)J9UTF8_LENGTH(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, romMethod)),
                J9UTF8_DATA(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, romMethod)), start, start, pc, pc);
...
_verifyError:
    ...
    storeVerifyErrorData(
        verifyData, (I_16)verboseErrorCode, (U_32)errorStackIndex, errorTargetType, errorTempData, start);

    BUILD_VERIFY_ERROR(errorModule, errorType);
    Trc_BCV_simulateStack_verifyError(verifyData->vmStruct, verifyData->errorPC, verifyData->errorCode);
    Trc_BCV_simulateStack_verifyErrorBytecode(verifyData->vmStruct,
        ...
        J9UTF8_DATA(J9ROMMETHOD_GET_NAME(verifyData->romClass, romMethod)),
        (UDATA)J9UTF8_LENGTH(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, romMethod)),
        J9UTF8_DATA(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, romMethod)), verifyData->errorCode,
        verifyData->errorPC, verifyData->errorPC, bc);
    Trc_BCV_simulateStack_Exit(verifyData->vmStruct);
...
allocateVerifyBuffers(J9PortLibrary* portLib, J9BytecodeVerificationData* verifyData)
{
    ...
    if (!(verifyData->classNameList && verifyData->classNameSegment && verifyData->bytecodeMap && verifyData->stackMaps
            && verifyData->unwalkedQueue && verifyData->rewalkQueue && verifyData->liveStack)) {
...
void* bcvalloc(J9BytecodeVerificationData* verifyData, UDATA byteCount)
{
    UDATA* returnVal = 0;
    ...
    if ((UDATA*)temp2 >= verifyData->internalBufferEnd) {
        ...
        Trc_BCV_bcvalloc_ExternalAlloc(verifyData->vmStruct,
            (UDATA)J9UTF8_LENGTH(J9ROMCLASS_CLASSNAME(verifyData->romClass)),
            J9UTF8_DATA(J9ROMCLASS_CLASSNAME(verifyData->romClass)), byteCount, returnVal);
        if (returnVal == 0) {
            return 0;
        }
    } else {
...
void bcvfree(J9BytecodeVerificationData* verifyData, void* address)
{
    J9BCVAlloc *temp1, *temp2;
    ...
    while (temp1 == temp2->prev) {
        ...
        if (temp1 == temp2) {
            ...
            verifyData->internalBufferStart
                = 0; /* Set the internal buffer start to zero so it will be re-allocated next time */
...
void freeVerifyBuffers(J9PortLibrary* portLib, J9BytecodeVerificationData* verifyData)
{
...
void j9bcv_freeVerificationData(J9PortLibrary* portLib, J9BytecodeVerificationData* verifyData)
{
 ...
 * returns NULL on OOM
 */
J9BytecodeVerificationData* j9bcv_initializeVerificationData(J9JavaVM* javaVM)
{
...
j9bcv_verifyBytecodes(
    J9PortLibrary* portLib, J9Class* clazz, J9ROMClass* romClass, J9BytecodeVerificationData* verifyData)
{
    UDATA i, j;
    ...
    BOOLEAN verboseVerification
        = (J9_VERIFY_VERBOSE_VERIFICATION == (verifyData->verificationFlags & J9_VERIFY_VERBOSE_VERIFICATION));

    PORT_ACCESS_FROM_PORT(portLib);

    Trc_BCV_j9bcv_verifyBytecodes_Entry(verifyData->vmStruct, (UDATA)J9UTF8_LENGTH((J9ROMCLASS_CLASSNAME(romClass))),
    ...
    for (i = 0; i < (UDATA)romClass->romMethodCount; i++) {
        ...
        Trc_BCV_j9bcv_verifyBytecodes_VerifyMethod(verifyData->vmStruct,
            ...
            J9UTF8_DATA(J9ROMMETHOD_GET_SIGNATURE(romClass, romMethod)), romMethod->modifiers);
        ...
        if (!((romMethod->modifiers & J9AccNative) || (romMethod->modifiers & J9AccAbstract))) {
            ...
            verifyData->stackSize = (J9_MAX_STACK_FROM_ROM_METHOD(romMethod) + J9_ARG_COUNT_FROM_ROM_METHOD(romMethod)
                                        + J9_TEMP_COUNT_FROM_ROM_METHOD(romMethod) + BCV_TARGET_STACK_HEADER_UDATA_SIZE
                                        + BCV_STACK_OVERFLOW_BUFFER_UDATA_SIZE)
                * sizeof(UDATA);
        ...
        _fallBack:
            ...
            if ((result == BCV_SUCCESS) && clazz) {
                ...
                if (newFormat && verboseVerification) {
                    ...
                    /*
                     * If verification failed and will fail over to older verifier, we only output stack map frame
                     * details if the frame count is bigger than 0.
                     */
                    if ((BCV_SUCCESS != result) && !classVersionRequiresStackmaps && !createStackMaps
                        && (J9_VERIFY_NO_FALLBACK != (verifyData->verificationFlags & J9_VERIFY_NO_FALLBACK))) {
                        willFailOver = TRUE;
                    }

                    if (!willFailOver || (verifyData->stackMapsCount > 0)) {
                        ALWAYS_TRIGGER_J9HOOK_VM_STACKMAPFRAME_VERIFICATION(
                            verifyData->javaVM->hookInterface, verifyData);
                    }
                }
            }
            /* If verify error */
            if (result) {
                ...
                if (classVersionRequiresStackmaps || createStackMaps
                    || (verifyData->verificationFlags & J9_VERIFY_NO_FALLBACK)) {
                    /* no retry */
                    result = BCV_ERR_INTERNAL_ERROR;
                    break;
                } else {
                    ...
                    if (verboseVerification) {
                        newFormat = FALSE;
                        ALWAYS_TRIGGER_J9HOOK_VM_CLASS_VERIFICATION_FALLBACK(
                            verifyData->javaVM->hookInterface, verifyData, newFormat);
                    }
...
j9bcv_J9VMDllMain(J9JavaVM* vm, IDATA stage, void* reserved)
{
    ...
    case BYTECODE_TABLE_SET:
        ...
        if ((*vmHooks)->J9HookRegisterWithCallSite(
                vmHooks, J9HOOK_VM_CLASSES_UNLOAD, bcvHookClassesUnload, OMR_GET_CALLSITE(), vm)) {
            returnVal = J9VMDLLMAIN_FAILED;
            break;
        }
#endif /* J9VM_GC_DYNAMIC_CLASS_UNLOADING */
        ...
        while (xVerifyColonIndex >= 0) {
            ...
            xVerifyColonIndex
                = FIND_NEXT_ARG_IN_VMARGS_FORWARD(STARTSWITH_MATCH, OPT_XVERIFY_COLON, NULL, xVerifyColonIndex);
        }
...
static IDATA setVerifyState(J9JavaVM* vm, char* option, char** errorString)
{
    ...
    if (0 == strcmp(option, OPT_ALL)) {
        ...
        if (0 != option[sizeof(OPT_EXCLUDEATTRIBUTE_EQUAL)]) {
            ...
            memcpy(vm->bytecodeVerificationData->excludeAttribute, &(option[sizeof(OPT_EXCLUDEATTRIBUTE_EQUAL) - 1]),
                length + 1);
        }
