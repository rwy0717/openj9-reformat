...
static IDATA matchStack(J9BytecodeVerificationData* verifyData, J9BranchTargetStack* liveStack,
    J9BranchTargetStack* targetStack, UDATA inlineMatch);
...
static void storeArgumentErrorData(
    J9BytecodeVerificationData* verifyData, U_32 errorCurrentFramePosition, U_16 errorArgumentIndex);
static void storeMethodInfo(J9BytecodeVerificationData* verifyData, J9UTF8* errorClassString, J9UTF8* errorMethodString,
    J9UTF8* errorSignatureString, IDATA currentPC);
 ...
 */
J9Class* j9rtv_verifierGetRAMClass(J9BytecodeVerificationData* verifyData, J9ClassLoader* classLoader, U_8* className,
    UDATA nameLength, IDATA* reasonCode)
{
    J9Class* found;
    ...
    if (!found) {
        ...
        if (BCV_ERR_INSUFFICIENT_MEMORY == allocateVerifyBuffers(tmpVMC->javaVM->portLibrary, verifyData)) {
            ...
            Trc_RTV_j9rtv_verifierGetRAMClass_OutOfMemoryException(
                verifyData->vmStruct, classLoader, nameLength, className);
        ...
        found = tmpVMC->javaVM->internalVMFunctions->internalFindClassUTF8(
            tmpVMC, className, nameLength, classLoader, J9_FINDCLASS_FLAG_THROW_ON_FAIL);
...
j9rtv_verifyBytecodes(J9BytecodeVerificationData* verifyData)
{
    ...
    Trc_RTV_j9rtv_verifyBytecodes_Entry(verifyData->vmStruct, (UDATA)J9UTF8_LENGTH(J9ROMCLASS_CLASSNAME(romClass)),
...
static IDATA findAndMatchStack(J9BytecodeVerificationData* verifyData, IDATA targetPC, IDATA currentPC)
{
    ...
    if (bytecodeMap[targetPC] & BRANCH_TARGET) {
        ...
        Trc_RTV_findAndMatchStack_StackNotFound(verifyData->vmStruct,
            ...
            J9UTF8_DATA(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, verifyData->romMethod)), targetPC);
        rc = BCV_FAIL;
    ...
    J9BranchTargetStack* targetStack, UDATA inlineMatch)
{
    ...
    if (size != (UDATA)targetStack->stackTopIndex) {
        Trc_RTV_matchStack_DepthMismatchException(verifyData->vmStruct,
            ...
            J9UTF8_DATA(J9ROMMETHOD_GET_NAME(verifyData->romClass, verifyData->romMethod)),
            (UDATA)J9UTF8_LENGTH(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, verifyData->romMethod)),
            J9UTF8_DATA(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, verifyData->romMethod)), size,
                ...
                if (FALSE == rc) {
                    if (BCV_ERR_INSUFFICIENT_MEMORY == reasonCode) {
                        Trc_RTV_matchStack_OutOfMemoryException(verifyData->vmStruct,
                            ...
                            J9UTF8_DATA(J9ROMMETHOD_GET_NAME(verifyData->romClass, verifyData->romMethod)),
                            (UDATA)J9UTF8_LENGTH(
                                J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, verifyData->romMethod)),
...
_incompatibleType:
     ...
     * Verification error data for incompatible data type should be saved here when any verification error occurs is
     * detected above.
     */
...
#define CHECK_END                                     \
    ...
    Trc_RTV_verifyBytecodes_Entry(verifyData->vmStruct, (UDATA)J9UTF8_LENGTH(J9ROMMETHOD_GET_NAME(romClass, romMethod)),
    ...
    while (pc < length) {
        ...
        _inconsistentStack2:
             ...
             */
            storeVerifyErrorData(
                verifyData, BCV_ERR_INCOMPATIBLE_TYPE, (U_32)errorStackIndex, errorTargetType, errorTempData, start);
            goto _verifyError;
        }

        if ((UDATA)(stackTop - stackBase) > maxStack) {
            errorType = J9NLS_BCV_ERR_STACK_OVERFLOW__ID;
            /* Jazz 82615: Set the error code and the location of wrong data type on stack (only keep the maximum size
             * for stack) */
            ...
            for (i = exceptionInfo->catchCount; i; i--, handler++) {
                if (handler->startPC == pc) {
                    ...
                    if (handler->exceptionClassIndex) {
                        catchName
                            = J9ROMSTRINGREF_UTF8DATA((J9ROMStringRef*)(&constantPool[handler->exceptionClassIndex]));
                        catchClass = convertClassNameToStackMapType(
                            verifyData, J9UTF8_DATA(catchName), J9UTF8_LENGTH(catchName), 0, 0);
                    } else {
            ...
            if (checkIfInsideException && tempStoreChange) {
                ...
                for (i = exceptionInfo->catchCount; i; i--, handler++) {
                    if (((UDATA)start >= handler->startPC) && ((UDATA)start < handler->endPC)) {
                        ...
                        /* Modify the liveStack temporarily to contain the handler exception */
                        if (handler->exceptionClassIndex) {
                            catchName = J9ROMSTRINGREF_UTF8DATA(
                                (J9ROMStringRef*)(&constantPool[handler->exceptionClassIndex]));
                            catchClass = convertClassNameToStackMapType(
                                verifyData, J9UTF8_DATA(catchName), J9UTF8_LENGTH(catchName), 0, 0);
                        } else {
                            catchClass = BCV_JAVA_LANG_THROWABLE_INDEX;
        ...
        case RTV_ARRAY_STORE:
            ...
            if (arrayType != BCV_BASE_TYPE_NULL) {
                if (bc == JBaastore) {
                    ...
                    if (inconsistentStack) {
                        /* Jazz 82615: Set the expected data type and the location of wrong data type on stack when the
                         * verification error occurs. Note: Given that the current location on stack is for arrayType,
                         * it needs to move forward by two slots to skip over the arrayType plus the index of array so
                         * as to reach the location of wrong type (value stored in the array).
                         */
                        errorTargetType = type1;
                    ...
                    if (inconsistentStack) {
                        /* Jazz 82615: Set the expected data type and the location of wrong data type on stack when the
                         * verification error occurs. Note: Given that the current location on stack is for arrayType,
                         * it needs to move forward by two slots to skip over the index of array and get to the location
                         * of wrong type (value stored in the array).
                         */
                        errorTargetType = type1;
                        errorStackIndex = (stackTop - liveStack->stackElements) + 2;
                        goto _inconsistentStack;
        ...
        case RTV_RETURN:
            ...
            case JBgenericReturn:
                ...
                if (returnChar) {
                    ...
                    if (returnChar != 'V') {
                        ...
                        if ((returnChar == 'J') || (returnChar == 'D')) {
                            ...
                            if (inconsistentStack) {
                                /* Jazz 82615: Set the expected long/double type and the location of wrong data type on
                                 * stack when the verification error occurs. */
                ...
                /* Don't rewrite the return bytecode for j.l.Object.<init>()V or if the romClass is in the shared
                 * classes cache*/
                if ((stackTop == stackBase)
                    && ((argCount != 1) || (returnBytecode != JBreturnFromConstructor)
                           || (J9ROMCLASS_SUPERCLASSNAME(romClass) != NULL))
            ...
            if (isInitMethod) {
                if (liveStack->uninitializedThis == (UDATA)TRUE) {
                    ...
                    /* Jazz 82615: Set the error code and the location of data type on stack in the case of the
                     * verification error unrelated to incompatible type.*/
                    verboseErrorCode = BCV_ERR_INIT_NOT_CALL_INIT;
                    /* Given that there is no real wrong data type on stack in such case, the stackTop pointer needs to
                     * step back by one slot to point to the current data type on stack so as to print all data types
                     * correctly in the error message framework.
                     */
                    errorStackIndex = (stackTop - liveStack->stackElements) - 1;
                    /* Always set to the location of the 1st data type on 'stack' to show up if stackTop <= stackBase */
                    if (stackTop <= stackBase) {
        ...
        case RTV_STATIC_FIELD_ACCESS: {
            ...
            utf8string
                = ((J9UTF8*)(J9ROMNAMEANDSIGNATURE_SIGNATURE(J9ROMFIELDREF_NAMEANDSIGNATURE((J9ROMFieldRef*)info))));
            ...
            if (bc & 1) {
                ...
                if (inconsistentStack) {
                    ...
                    /* Jazz 82615: This is an exception when storing the verification error data, which was originally
                     * caused by the hack (pushFieldType) above. The stackTop pointer doesn't get updated after calling
                     * pushFieldType() to store the return type, so *stackTop stores the expected data type rather than
                     * the wrong data type which has already been popped up from stack.
                     *
                     * According to the output format of error message framework, only the the wrong data type is
                     * expected to occur at *stackTop on stack rather than the expected data type. Thus, the wrong data
                     * type must be pushed back to *stackTop after saving the expected data type somewhere else.
                     */
                    PUSH(type);
                    goto _inconsistentStack;
                }
            ...
            if (inconsistentStack) {
                ...
                errorTargetType = convertClassNameToStackMapType(
                    verifyData, J9UTF8_DATA(utf8string), J9UTF8_LENGTH(utf8string), 0, 0);
        ...
        case RTV_SEND:
            if ((bc == JBinvokehandle) || (bc == JBinvokehandlegeneric)) {
                ...
                info = &constantPool[index];
                utf8string = ((
                    J9UTF8*)(J9ROMNAMEANDSIGNATURE_SIGNATURE(J9ROMMETHODREF_NAMEANDSIGNATURE((J9ROMMethodRef*)info))));
                ...
                if (inconsistentStack) {
                     ...
                     * For the return code from j9rtv_verifyArguments(), there is no need to print out error message
                     * framework in the case of BCV_FAIL for bad signature (should never happen - caught by cfreader.c
                     * in checkMethodSignature) and BCV_ERR_INSUFFICIENT_MEMORY for OOM as there is nothing meaningful
                     * to show up.
                     */
                    if (verifyData->errorDetailCode < 0) {
                        J9UTF8* utf8ClassString = J9ROMCLASSREF_NAME(
                            (J9ROMClassRef*)&constantPool[((J9ROMMethodRef*)info)->classRefCPIndex]);
                        J9UTF8* utf8MethodString = ((J9UTF8*)(J9ROMNAMEANDSIGNATURE_NAME(
                            J9ROMMETHODREF_NAMEANDSIGNATURE((J9ROMMethodRef*)info))));
                ...
                rc = isClassCompatibleByName(verifyData, type, (U_8*)"java/lang/invoke/MethodHandle",
                    sizeof("java/lang/invoke/MethodHandle") - 1, &reasonCode);
                if (FALSE == rc) {
                    if (BCV_ERR_INSUFFICIENT_MEMORY == reasonCode) {
                        goto _outOfMemoryError;
                    }
                    errorType = J9NLS_BCV_ERR_RECEIVER_NOT_COMPATIBLE__ID;
                    /* Jazz 82615: Store the index of the expected data type 'java/lang/invoke/MethodHandle' for later
                     * retrieval in classNameList */
            ...
            if (bc == JBinvokedynamic) {
                ...
                utf8string = ((J9UTF8*)(J9ROMNAMEANDSIGNATURE_SIGNATURE(
            ...
            if (inconsistentStack) {
                /* errorDetailCode has been initialized with 0 (SUCCESS) before any verification error is detected
                 * For the return code from j9rtv_verifyArguments(), there is no need to print out error message
                 * framework in the case of BCV_FAIL for bad signature (should never happen - caught by cfreader.c in
                 * checkMethodSignature) and BCV_ERR_INSUFFICIENT_MEMORY for OOM as there is nothing meaningful to show
                 * up.
                 */
                if (verifyData->errorDetailCode < 0) {
                    J9UTF8* utf8ClassString = J9ROMSTRINGREF_UTF8DATA(classRef);
                    J9UTF8* utf8MethodString = ((
                        J9UTF8*)(J9ROMNAMEANDSIGNATURE_NAME(J9ROMMETHODREF_NAMEANDSIGNATURE((J9ROMMethodRef*)info))));
                    storeMethodInfo(verifyData, utf8ClassString, utf8MethodString, utf8string, start);
                }
                errorType = J9NLS_BCV_ERR_INCONSISTENT_STACK__ID;
                goto _verifyError;
            ...
            if ((JBinvokestatic != bc) && (JBinvokestaticsplit != bc)) {
                IDATA reasonCode = 0;
                ...
                case JBinvokespecialsplit:
                    CHECK_STACK_UNDERFLOW;
                    /* cannot use isInitMethod here b/c invokespecial may be invoking a different <init> method then the
                     * one we are in */
                    if (J9UTF8_DATA(
                            J9ROMNAMEANDSIGNATURE_NAME(J9ROMMETHODREF_NAMEANDSIGNATURE((J9ROMMethodRef*)info)))[0]
                        == '<') {
                        ...
                        if ((type & BCV_SPECIAL) == 0) {
                            ...
                            if (temp1 & BCV_SPECIAL_INIT) {
                                 ...
                                 * update the stackTopIndex to include the wrong type on stack (already popped out of
                                 * stack).
                                 */
                            ...
                            if (type & BCV_SPECIAL_NEW) {
                                ...
                                classIndex = convertClassNameToStackMapType(
                                    verifyData, J9UTF8_DATA(utf8string), J9UTF8_LENGTH(utf8string), 0, 0);
                                if (classIndex != temp1) {
                            ...
                            if (type & BCV_SPECIAL_INIT) {
                                ...
                                superClassIndex = convertClassNameToStackMapType(
                                    verifyData, J9UTF8_DATA(utf8string), J9UTF8_LENGTH(utf8string), 0, 0);
                        ...
                        if (!methodIsFinalInObject(
                             ...
                             * In all three functions, returned boolean value would indicate an error occurred and the
                             * reasonCode is set to BCV_ERR_INSUFFICIENT_MEMORY in OOM cases. Hence, if any of the 3
                             * conditions should fail, if it is not on OOM as readonCode would indicate, it must be a
                             * verification error.
                             */
                            if ((FALSE
                                    == isClassCompatibleByName(verifyData, classIndex, J9UTF8_DATA(utf8string),
                                           J9UTF8_LENGTH(utf8string), &reasonCode))
                ...
                if (bc != JBinvokeinterface) {
                    ...
                    rc = isClassCompatibleByName(
                        verifyData, type, J9UTF8_DATA(utf8string), J9UTF8_LENGTH(utf8string), &reasonCode);
                    if (FALSE == rc) {
                        if (BCV_ERR_INSUFFICIENT_MEMORY == reasonCode) {
                            goto _outOfMemoryError;
                        }
                     ...
                     */
                    if ((BCV_TAG_BASE_TYPE_OR_TOP == (type & BCV_TAG_MASK)) || J9_ARE_ANY_BITS_SET(type, BCV_SPECIAL)) {
                        errorType = J9NLS_BCV_ERR_RECEIVER_NOT_COMPATIBLE__ID;
                        /* Jazz 82615: Set the expected data type and the location of wrong data type
                         * on stack when the verification error occurs.
                         */
        ...
        case RTV_PUSH_NEW:
            ...
            case JBmultianewarray:
                ...
                if ((stackTop - i1) < stackBase) {
                    ...
                    verboseErrorCode = BCV_ERR_STACK_UNDERFLOW;
                    /* Given that the actual data type involved has not yet been located through pop operation when
                     * stack underflow occurs, step back by one slot to the actual data type to be manipulated by the
                     * opcode.
                     */
                    errorStackIndex = (stackTop - liveStack->stackElements) - 1;
                    /* Always set to the location of the 1st data type on 'stack' to show up if stackTop <= stackBase */
        ...
        case RTV_MISC: {
            ...
            case JBathrow:
                POP_TOS_OBJECT(type);
                rc = isClassCompatible(
                    verifyData, type, (UDATA)(BCV_JAVA_LANG_THROWABLE_INDEX << BCV_CLASS_INDEX_SHIFT), &reasonCode);
                if (FALSE == rc) {
                    if (BCV_ERR_INSUFFICIENT_MEMORY == reasonCode) {
                        goto _outOfMemoryError;
                    }
            ...
            if ((UDATA)nextStackPC < length) {
                ...
                if (pc != (UDATA)nextStackPC) {
                    if (J9ROMCLASS_HAS_VERIFY_DATA(romClass) && (!verifyData->ignoreStackMaps)) {
                        ...
                        /* Jazz 82615: Set the error code, the next pc value, and the pc value of unexpected stackmap
                         * frame. */
                        errorType = J9NLS_BCV_ERR_INCONSISTENT_STACK__ID;
    ...
    if (J9ROMCLASS_HAS_VERIFY_DATA(romClass) && (action != RTV_RETURN) && (bc != JBathrow) && (bc != JBtableswitch)
        && (bc != JBlookupswitch) && (bc != JBgoto) && (bc != JBgotow)) {
...
_verifyError:
    ...
    Trc_RTV_verifyBytecodes_VerifyError(verifyData->vmStruct, errorType,
        (UDATA)J9UTF8_LENGTH(J9ROMMETHOD_GET_NAME(romClass, romMethod)),
        J9UTF8_DATA(J9ROMMETHOD_GET_NAME(romClass, romMethod)),
        (UDATA)J9UTF8_LENGTH(J9ROMMETHOD_GET_SIGNATURE(romClass, romMethod)),
        J9UTF8_DATA(J9ROMMETHOD_GET_SIGNATURE(romClass, romMethod)), start);
    ...
    Trc_RTV_verifyBytecodes_VerifyErrorBytecode(verifyData->vmStruct,
        ...
        J9UTF8_DATA(J9ROMMETHOD_GET_NAME(romClass, romMethod)),
        (UDATA)J9UTF8_LENGTH(J9ROMMETHOD_GET_SIGNATURE(romClass, romMethod)),
        J9UTF8_DATA(J9ROMMETHOD_GET_SIGNATURE(romClass, romMethod)), errorType, start, start, *(code + start));
    BUILD_VERIFY_ERROR(errorModule, errorType);

    return BCV_ERR_INTERNAL_ERROR;

_miscError:
    /* Jazz 82615: Store the error code and the location of wrong data type on stack in the case of the verification
     * error unrelated to incompatible type */
...
_outOfMemoryError:
    ...
    Trc_RTV_verifyBytecodes_OutOfMemoryException(verifyData->vmStruct, errorType,
        (UDATA)J9UTF8_LENGTH(J9ROMMETHOD_GET_NAME(romClass, romMethod)),
        J9UTF8_DATA(J9ROMMETHOD_GET_NAME(romClass, romMethod)),
        (UDATA)J9UTF8_LENGTH(J9ROMMETHOD_GET_SIGNATURE(romClass, romMethod)),
        J9UTF8_DATA(J9ROMMETHOD_GET_SIGNATURE(romClass, romMethod)), start);
...
static IDATA verifyExceptions(J9BytecodeVerificationData* verifyData)
{
    ...
    for (i = 0; i < exceptionInfo->catchCount; i++, handler++) {
        if (handler->exceptionClassIndex) {
            ...
            catchClass
                = convertClassNameToStackMapType(verifyData, J9UTF8_DATA(catchName), J9UTF8_LENGTH(catchName), 0, 0);
            tempRC = isClassCompatible(
                verifyData, catchClass, (UDATA)(BCV_JAVA_LANG_THROWABLE_INDEX << BCV_CLASS_INDEX_SHIFT), &reasonCode);
            if (FALSE == tempRC) {
                if (BCV_ERR_INSUFFICIENT_MEMORY == reasonCode) {
                    ...
                    Trc_RTV_verifyExceptions_VerifyError(verifyData->vmStruct,
                        ...
                        J9UTF8_DATA(J9ROMMETHOD_GET_NAME(romClass, romMethod)),
                        (UDATA)J9UTF8_LENGTH(J9ROMMETHOD_GET_SIGNATURE(romClass, romMethod)),
                        J9UTF8_DATA(J9ROMMETHOD_GET_SIGNATURE(romClass, romMethod)), i, J9UTF8_LENGTH(catchName),
                        J9UTF8_DATA(catchName));
                    rc = BCV_FAIL;
                    break;
                }
            }
...
j9rtv_verifyArguments(J9BytecodeVerificationData* verifyData, J9UTF8* utf8string, UDATA** pStackTop)
{
     ...
     * Set stackTop with the address at liveStack->stackElements to avoid compare against garbage that doesn't exist on
     * stack.
     */
            ...
            if (*signature == 'L') {
                ...
                if ((*signature < 'A') || (*signature > 'Z')) {
                    ...
                    Trc_RTV_j9rtv_verifyArguments_Unreachable(verifyData->vmStruct,
                        ...
                        J9UTF8_DATA(J9ROMMETHOD_GET_NAME(verifyData->romClass, verifyData->romMethod)),
                        (UDATA)J9UTF8_LENGTH(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, verifyData->romMethod)),
                        J9UTF8_DATA(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, verifyData->romMethod)), __LINE__);
                    mrc = BCV_FAIL;
                    break;
                }
            ...
            if (FALSE == rc) {
                if (BCV_ERR_INSUFFICIENT_MEMORY == reasonCode) {
                    ...
                    Trc_RTV_j9rtv_verifyArguments_ObjectMismatch(verifyData->vmStruct,
                        ...
                        J9UTF8_DATA(J9ROMMETHOD_GET_NAME(verifyData->romClass, verifyData->romMethod)),
                        (UDATA)J9UTF8_LENGTH(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, verifyData->romMethod)),
                        J9UTF8_DATA(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, verifyData->romMethod)), index,
                        J9UTF8_LENGTH(utf8string), J9UTF8_DATA(utf8string), stackTop[index]);
                    mrc = BCV_FAIL;
            ...
            if (baseType & BCV_WIDE_TYPE_MASK) {
                ...
                if (stackTop[index] != BCV_BASE_TYPE_TOP) {
                    Trc_RTV_j9rtv_verifyArguments_WidePrimitiveMismatch(verifyData->vmStruct,
                        ...
                        (UDATA)J9UTF8_LENGTH(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, verifyData->romMethod)),
                        J9UTF8_DATA(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, verifyData->romMethod)), index,
                        J9UTF8_LENGTH(utf8string), J9UTF8_DATA(utf8string));
                    mrc = BCV_FAIL;
    ...
    /* errorDetailCode has been initialized with 0 (SUCCESS).
     * Verification error data related to argument mismatch should be saved here when any verification error is detected
     * above.
     */
    if (verifyData->errorDetailCode < 0) {
...
static J9BranchTargetStack* nextStack(J9BytecodeVerificationData* verifyData, UDATA* nextMapIndex, IDATA* nextStackPC)
{
    ...
    *nextStackPC = J9_BYTECODE_SIZE_FROM_ROM_METHOD(verifyData->romMethod);
    ...
    Trc_RTV_nextStack_Result(verifyData->vmStruct, (UDATA)J9UTF8_LENGTH(J9ROMCLASS_CLASSNAME(verifyData->romClass)),
        J9UTF8_DATA(J9ROMCLASS_CLASSNAME(verifyData->romClass)),
        (UDATA)J9UTF8_LENGTH(J9ROMMETHOD_GET_NAME(verifyData->romClass, verifyData->romMethod)),
        J9UTF8_DATA(J9ROMMETHOD_GET_NAME(verifyData->romClass, verifyData->romMethod)),
        (UDATA)J9UTF8_LENGTH(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, verifyData->romMethod)),
        J9UTF8_DATA(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, verifyData->romMethod)), verifyData->stackMapsCount,
        *nextMapIndex, *nextStackPC, J9_BYTECODE_SIZE_FROM_ROM_METHOD(verifyData->romMethod));
    return returnStack;
}
...
static IDATA nextExceptionStart(J9BytecodeVerificationData* verifyData, J9ROMMethod* romMethod, IDATA lastPC)
{
    ...
    if (romMethod->modifiers & CFR_ACC_HAS_EXCEPTION_INFO) {
        ...
        Trc_RTV_nextExceptionStart_Result(verifyData->vmStruct,
            ...
            (UDATA)J9UTF8_LENGTH(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, verifyData->romMethod)),
            J9UTF8_DATA(J9ROMMETHOD_GET_SIGNATURE(verifyData->romClass, verifyData->romMethod)),
            exceptionInfo->catchCount, lastPC, nextPC, J9_BYTECODE_SIZE_FROM_ROM_METHOD(romMethod));
    }
    return nextPC;
}
...
/**
 * Postpone storing the method info to the checking of return code after the mismatched argument in method signature is
 * detected J9BytecodeVerificationData structure for outputting detailed error message
 * @param verifyData - pointer to J9BytecodeVerificationData
