...
static IDATA checkMethodStructure(J9PortLibrary* portLib, J9CfrClassFile* classfile, UDATA methodIndex,
    U_8* instructionMap, J9CfrError* error, U_32 flags, I_32* hasRET);
static IDATA buildInstructionMap(
    J9CfrClassFile* classfile, J9CfrAttributeCode* code, U_8* map, UDATA methodIndex, J9CfrError* error);
static IDATA checkBytecodeStructure(
    J9CfrClassFile* classfile, UDATA methodIndex, UDATA length, U_8* map, J9CfrError* error, U_32 flags, I_32* hasRET);
static IDATA checkStackMap(J9CfrClassFile* classfile, J9CfrMethod* method, J9CfrAttributeCode* code, U_8* map,
    UDATA flags, StackmapExceptionDetails* exceptionDetails);
static I_32 checkStackMapEntries(
    J9CfrClassFile* classfile, J9CfrAttributeCode* code, U_8* map, U_8** entries, UDATA slotCount, U_8* end);
...
_leaveProc:
    ...
    buildMethodError(error, errorType, CFR_ThrowVerifyError, (I_32)methodIndex, (U_32)pc,
        &classfile->methods[methodIndex], classfile->constantPool);
    return -1;
}
...
/*
        ...
        Invalid returns will cause us to modify the classfile in place such that the second pass of the verification can
   correctly throw the error (unless it turns out to be dead code).
...
#define CHECK_END                                    \
    ...
    sigChar = (UDATA)getReturnTypeFromSignature(classfile->constantPool[method->descriptorIndex].bytes,
        classfile->constantPool[method->descriptorIndex].slot1, NULL);
    ...
    while (bcIndex < bcEnd) {
        ...
        case CFR_BC_ldc_w:
            ...
            tag = (UDATA)info->tag;
            {
                ...
                case CFR_CONSTANT_Dynamic: {
                    J9CfrConstantPoolInfo* constantDynamicSignature
                        = &classfile->constantPool[classfile->constantPool[info->slot2].slot2];
        ...
        case CFR_BC_return:
            /* do not modify the bytecode at all, if the return matches we're good, if it does not match it will fail as
               expected if (sigChar != 'V') { fail, modify the bytecode to be incompatible in the second pass of
               verification
                                                     *(bcIndex - 1) = CFR_BC_return;	 leaving it alone will cause this
               to fail
                                    }
            */
            break;
        ...
        case CFR_BC_invokestatic:
            ...
            if (info->bytes[0] == '<') {
                if ((bc != CFR_BC_invokespecial) || (info->tag != CFR_CONSTANT_Utf8)
...
_verifyError:
    ...
    buildMethodErrorWithExceptionDetails(error, errorType, 0, CFR_ThrowVerifyError, (I_32)methodIndex, (U_32)start,
        method, classfile->constantPool, (U_32)errorDataIndex, -1, 0);
    result = -1;
        ...
        if (attribute->tag == CFR_ATTRIBUTE_StackMapTable) {
            ...
            for (j = 0; j < stackMap->numberOfEntries; j++) {
                ...
                if (frameType != CFR_STACKMAP_FULL) {
                    ...
                    slotCount = 0;
                    if ((frameType >= CFR_STACKMAP_SAME_LOCALS_1_STACK)
                        && (frameType <= CFR_STACKMAP_SAME_LOCALS_1_STACK_EXTENDED)) {
                        slotCount = 1;
                    }
...
_failedCheck:
    if (delayedErrorCode || (errorCode == FALLBACK_CLASS_FORMAT_ERROR) || (errorCode == FALLBACK_VERIFY_ERROR)) {
        if ((classfile->majorVersion < CFR_MAJOR_VERSION_REQUIRING_STACKMAPS)
            && ((flags & J9_VERIFY_NO_FALLBACK) == 0)) {
            /* Hide the bad StackMap/StackMapTable attribute and error.  Major version 51 and greater do not allow
    ...
    U_8* instructionMap, J9CfrError* error, U_32 flags, I_32* hasRET)
{
    ...
    /* Throw a class format error if we are given a static <init> method (otherwise later we will throw a verify error
     * due to back stack shape) */
    info = &(classfile->constantPool[method->nameIndex]);
    if (info->slot1 == 6) { /* size of '<init>' */
        if ((info->tag == CFR_CONSTANT_Utf8) && (!strncmp((char*)info->bytes, "<init>", 6))) {
    ...
    if ((flags & J9_VERIFY_IGNORE_STACK_MAPS) == 0) {
        ...
        if (result) {
            if (result == FALLBACK_VERIFY_ERROR) {
                ...
                buildMethodErrorWithExceptionDetails(error, errorType, 0, CFR_ThrowVerifyError, (I_32)methodIndex,
                    (U_32)pc, method, classfile->constantPool, 0, exceptionDetails.stackmapFrameIndex,
...
_formatError:
    ...
    buildMethodError(
        error, errorType, CFR_ThrowClassFormatError, (I_32)methodIndex, (U_32)pc, method, classfile->constantPool);
    result = -1;
    goto _leaveProc;
...
IDATA
j9bcv_verifyClassStructure(J9PortLibrary* portLib, J9CfrClassFile* classfile, U_8* segment, U_8* segmentLength,
    U_8* freePointer, U_32 vmVersionShifted, U_32 flags, I_32* hasRET)
{
    ...
    Trc_STV_j9bcv_verifyClassStructure_Entry(
    ...
    for (i = 0; i < classfile->methodsCount; i++) {
        ...
        if (isInit) {
            if (info->bytes[info->slot1 - 1] != 'V') {
                ...
                buildMethodError((J9CfrError*)segment, errorType, CFR_ThrowClassFormatError, (I_32)i, 0, method,
