...
#define J9CLASS_ARITY_FROM_CLASS_ENTRY(clazz) \
    (((clazz & BCV_ARITY_MASK) >> BCV_ARITY_SHIFT) + ((clazz & BCV_TAG_BASE_ARRAY_OR_NULL) >> 1))
...
static void getNameAndLengthFromClassNameList(
    J9BytecodeVerificationData* verifyData, UDATA listIndex, U_8** name, UDATA* length);
static IDATA findFieldFromRamClass(J9Class** ramClass, J9ROMFieldRef* field, UDATA firstSearch);
static IDATA findMethodFromRamClass(
    J9BytecodeVerificationData* verifyData, J9Class** ramClass, J9ROMNameAndSignature* method, UDATA firstSearch);
static VMINLINE UDATA* pushType(J9BytecodeVerificationData* verifyData, U_8* signature, UDATA* stackTop);
static IDATA isRAMClassCompatible(J9BytecodeVerificationData* verifyData, U_8* parentClass, UDATA parentLength,
    U_8* childClass, UDATA childLength, IDATA* reasonCode);
...
static UDATA addClassName(J9BytecodeVerificationData* verifyData, U_8* name, UDATA length, UDATA index)
{
    ...
    if ((verifyData->classNameSegmentFree + sizeof(UDATA) + sizeof(J9UTF8) + length + sizeof(UDATA))
        >= verifyData->classNameSegmentEnd) {
        /* Do some magic to grow the classNameSegment. */
        newSize = (UDATA)(
            ((length + sizeof(UDATA) + sizeof(J9UTF8) + sizeof(UDATA)) < (32 * (sizeof(UDATA) + sizeof(J9UTF8))))
        ...
        memcpy(newBuffer, verifyData->classNameSegment,
            (UDATA)(verifyData->classNameSegmentEnd - verifyData->classNameSegment));
        ...
        while (verifyData->classNameList[i]) {
            /* Second pass verification can have new strings and preverify data references - move only the new string
             * references */
            if (((U_8*)(verifyData->classNameList[i]) >= verifyData->classNameSegment)
                && ((U_8*)(verifyData->classNameList[i]) < verifyData->classNameSegmentEnd)) {
    ...
    if (&(verifyData->classNameList[index + 1]) >= verifyData->classNameListEnd) {
        ...
        memcpy(newBuffer, (U_8*)verifyData->classNameList,
            (UDATA)(((U_8*)verifyData->classNameListEnd) - ((U_8*)verifyData->classNameList)));
    ...
    if (classNameInClass) {
        ...
        verifyData->classNameSegmentFree
            += (length - 2 + sizeof(U_32) - 1) & ~(sizeof(U_32) - 1); /* next U_32 boundary */
    }
...
buildStackFromMethodSignature(J9BytecodeVerificationData* verifyData, UDATA** stackTopPtr, UDATA* argCount)
{
    ...
    *argCount = 0;
    ...
    if ((!(romMethod->modifiers & J9AccStatic)) && (argMax > 0)) {
        ...
        /* In the <init> method of Object the type of this is Object.  In other <init> methods, the type of this is
         * uninitializedThis */
        if ((J9UTF8_DATA(utf8string)[0] == '<') && (J9UTF8_DATA(utf8string)[1] == 'i')
    ...
    for (i = 1; args[i] != ')'; i++) {
        ...
        if (args[i] == 'L') {
            ...
            if (arity) {
                ...
                arity--;
                PUSH(BCV_TAG_BASE_ARRAY_OR_NULL | (UDATA)baseTypeCharConversion[args[i] - 'A']
                    | (arity << BCV_ARITY_SHIFT));
            } else {
...
UDATA* pushReturnType(J9BytecodeVerificationData* verifyData, J9UTF8* utf8string, UDATA* stackTop)
{
    U_8* signature;
...
UDATA* pushFieldType(J9BytecodeVerificationData* verifyData, J9UTF8* utf8string, UDATA* stackTop)
{
    return pushType(verifyData, J9UTF8_DATA(utf8string), stackTop);
}

UDATA* pushClassType(J9BytecodeVerificationData* verifyData, J9UTF8* utf8string, UDATA* stackTop)
{
    if (J9UTF8_DATA(utf8string)[0] == '[') {
        UDATA arrayType = parseObjectOrArrayName(verifyData, J9UTF8_DATA(utf8string));
        PUSH(arrayType);
    } else {
        PUSH(convertClassNameToStackMapType(
            verifyData, J9UTF8_DATA(utf8string), J9UTF8_LENGTH(utf8string), BCV_OBJECT_OR_ARRAY, 0));
    }

    return stackTop;
}

void initializeClassNameList(J9BytecodeVerificationData* verifyData)
{
    J9UTF8* name;
...
UDATA* pushLdcType(J9BytecodeVerificationData* verifyData, J9ROMClass* romClass, UDATA index, UDATA* stackTop)
{
    ...
    case J9CPTYPE_CONSTANT_DYNAMIC: {
        J9ROMConstantDynamicRef* romConstantDynamicRef
            = (J9ROMConstantDynamicRef*)(J9_ROM_CP_FROM_ROM_CLASS(romClass) + index);
        J9UTF8* signature
            = J9ROMNAMEANDSIGNATURE_SIGNATURE(J9ROMCONSTANTDYNAMICREF_NAMEANDSIGNATURE(romConstantDynamicRef));
    ...
    *reasonCode = 0;
    ...
    if (targetArity < sourceArity) {
        ...
        if ((IDATA)TRUE == isInterfaceClass(verifyData, targetName, targetLength, reasonCode)) {
             ...
             */
            if (((CLONEABLE_CLASS_NAME_LENGTH == targetLength)
                    && ((0 == strncmp((const char*)targetName, CLONEABLE_CLASS_NAME, CLONEABLE_CLASS_NAME_LENGTH))))
                || ((SERIALIZEABLE_CLASS_NAME_LENGTH == targetLength)
                       && (0
                              == strncmp((const char*)targetName, SERIALIZEABLE_CLASS_NAME,
                                     SERIALIZEABLE_CLASS_NAME_LENGTH)))) {
                return (IDATA)TRUE;
            }
        }
        return (IDATA)FALSE;
    ...
    U_8* childClass, UDATA childLength, IDATA* reasonCode)
{
    ...
    targetRAMClass
        = j9rtv_verifierGetRAMClass(verifyData, verifyData->classLoader, parentClass, parentLength, reasonCode);
    ...
    sourceRAMClass
        = j9rtv_verifierGetRAMClass(verifyData, verifyData->classLoader, childClass, childLength, reasonCode);
...
static UDATA* pushType(J9BytecodeVerificationData* verifyData, U_8* signature, UDATA* stackTop)
{
    if (*signature != 'V') {
 ...
 */
IDATA
isClassCompatibleByName(J9BytecodeVerificationData* verifyData, UDATA sourceClass, U_8* targetClassName,
    UDATA targetClassNameLength, IDATA* reasonCode)
{
    UDATA index;
...
U_8* j9bcv_createVerifyErrorString(J9PortLibrary* portLib, J9BytecodeVerificationData* error)
{
    ...
    if ((IDATA)error->errorPC == -1) {
        ...
        formatString = j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE,
            J9NLS_BCV_ERROR_TEMPLATE_NO_PC, "%s;%.*s,%.*s%.*s");
    } else {
         ...
         */
        if (J9_ARE_ALL_BITS_SET(error->verificationFlags, J9_VERIFY_ERROR_DETAILS) && (0 != error->errorDetailCode)) {
            /* Jazz 82615: The value of detailedErrMsg may change if the inital byteArray is insufficient to contain the
             * error message framework. Under such circumstances, it points to the address of newly allocated memory.
             */
             ...
             * detailedErrMsg is set to NULL and detailedErrMsgLength is set to 0 on return from
             * getRtvExceptionDetails(). Therefore, it ends up with a simple error message as the buffer for the error
             * message framework has been cleaned up.
             */
            detailedErrMsg
                = error->javaVM->verboseStruct->getRtvExceptionDetails(error, detailedErrMsg, &detailedErrMsgLength);
        }
        ...
        if (NULL == error->errorSignatureString) {
            ...
            formatString = j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE,
                J9NLS_BCV_ERROR_TEMPLATE_WITH_PC, "%s;%.*s,%.*s%.*s,%u");
        } else {
            /*Jazz 82615: J9NLS_BCV_ERROR_TEMPLATE_TYPE_MISMATCH=%1$s; class=%3$.*2$s, method=%5$.*4$s%7$.*6$s, pc=%8$u;
             * Mismatch, argument %9$d in signature %11$.*10$s.%13$.*12$s:%15$.*14$s does not match */
            formatString = j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE,
                J9NLS_BCV_ERROR_TEMPLATE_TYPE_MISMATCH, "%s;%.*s,%.*s%.*s,%u;,%d%.*s.%.*s:%.*s");
        }
    }

    /* Determine the size of the error string we need to allocate */
    errorString = OMRPORT_FROM_J9PORT(PORTLIB)->nls_lookup_message(OMRPORT_FROM_J9PORT(PORTLIB),
        J9NLS_DO_NOT_APPEND_NEWLINE, (U_32)(error->errorModule), (U_32)(error->errorCode), NULL);
    ...
    if (NULL != verifyError) {
        ...
        if (NULL == error->errorSignatureString) {
            errStrLength = j9str_printf(PORTLIB, (char*)verifyError, stringLength, formatString, errorString,
                (U_32)J9UTF8_LENGTH(romClassName), J9UTF8_DATA(romClassName), (U_32)J9UTF8_LENGTH(romMethodName),
                J9UTF8_DATA(romMethodName), (U_32)J9UTF8_LENGTH(romMethodSignatureString),
                J9UTF8_DATA(romMethodSignatureString), error->errorPC);
        } else {
            /* Jazz 82615: Print the corresponding NLS message to buffer for type mismatch error */
            errStrLength = j9str_printf(PORTLIB, (char*)verifyError, stringLength, formatString, errorString,
                (U_32)J9UTF8_LENGTH(romClassName), J9UTF8_DATA(romClassName), (U_32)J9UTF8_LENGTH(romMethodName),
                J9UTF8_DATA(romMethodName), (U_32)J9UTF8_LENGTH(romMethodSignatureString),
                J9UTF8_DATA(romMethodSignatureString), error->errorPC, error->errorArgumentIndex,
        ...
        if (detailedErrMsgLength > 0) {
            j9str_printf(PORTLIB, (char*)&verifyError[errStrLength], stringLength - errStrLength, "%.*s",
                detailedErrMsgLength, detailedErrMsg);
        }
    }
...
isFieldAccessCompatible(
    J9BytecodeVerificationData* verifyData, J9ROMFieldRef* fieldRef, UDATA bytecode, UDATA receiver, IDATA* reasonCode)
{
    J9ROMClass* romClass = verifyData->romClass;
    ...
    *reasonCode = 0;
    ...
    return isClassCompatibleByName(
        verifyData, receiver, J9UTF8_DATA(utf8string), J9UTF8_LENGTH(utf8string), reasonCode);
}
...
UDATA
isProtectedAccessPermitted(J9BytecodeVerificationData* verifyData, J9UTF8* declaringClassName, UDATA targetClass,
    void* member, UDATA isField, IDATA* reasonCode)
{
    J9ROMClass* romClass = verifyData->romClass;

    *reasonCode = 0;

    /* for performance reasons, don't do protection checks unless -Xfuture or -Xverify:doProtectedAccessCheck is
     * specified */
    ...
    if (J9CLASS_ARITY_FROM_CLASS_ENTRY(targetClass) == 0) {
        ...
        currentRamClass = j9rtv_verifierGetRAMClass(verifyData, verifyData->classLoader, J9UTF8_DATA(currentClassName),
            J9UTF8_LENGTH(currentClassName), reasonCode);
        ...
        definingRamClass = j9rtv_verifierGetRAMClass(verifyData, verifyData->classLoader,
            J9UTF8_DATA(declaringClassName), J9UTF8_LENGTH(declaringClassName), reasonCode);
        ...
        if (currentRamClass->packageID == definingRamClass->packageID)
            return TRUE;
        ...
        if (isSameOrSuperClassOf(definingRamClass, currentRamClass)) {
            ...
            if (targetClass != BCV_BASE_TYPE_NULL) {
                ...
                getNameAndLengthFromClassNameList(
                    verifyData, J9CLASS_INDEX_FROM_CLASS_ENTRY(targetClass), &targetClassName, &targetClassLength);
                targetRamClass = j9rtv_verifierGetRAMClass(
...
static UDATA compareTwoUTF8s(J9UTF8* first, J9UTF8* second)
{
    ...
    if (J9UTF8_LENGTH(first) != J9UTF8_LENGTH(second))
        return 0;
        ...
        while (currentField != NULL) {
            if ((currentField->modifiers & J9AccStatic) == 0) {
                if (compareTwoUTF8s(searchName, J9ROMFIELDSHAPE_NAME(currentField))) {
                    if (currentField->modifiers & CFR_ACC_PROTECTED)
                        return BCV_FAIL;
                    if (firstSearch)
                        return BCV_SUCCESS;
                }
    ...
    J9BytecodeVerificationData* verifyData, J9Class** ramClass, J9ROMNameAndSignature* method, UDATA firstSearch)
{
    ...
    for (;;) {
        ...
        for (redefinedClassIndex = 0; redefinedClassIndex < verifyData->redefinedClassesCount; redefinedClassIndex++) {
            J9ROMClass* currentRedefinedClass
                = verifyData->redefinedClasses[redefinedClassIndex].replacementClass.romClass;
        ...
        for (methodIndex = 0; methodIndex < romClass->romMethodCount; methodIndex++) {
            if ((0 != compareTwoUTF8s(searchName, J9ROMMETHOD_NAME(currentRomMethod)))
                ...
                if (currentRomMethod->modifiers & CFR_ACC_PROTECTED)
                    return BCV_FAIL;
                if (firstSearch)
                    return BCV_SUCCESS;
            }
