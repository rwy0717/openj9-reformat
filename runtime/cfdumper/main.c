...
typedef enum { SOURCE_files = 0, SOURCE_zip, SOURCE_jimage, SOURCE_rom } sourceType;
...
static void dumpAnnotations(
    J9CfrClassFile* classfile, J9CfrAnnotation* annotations, U_32 annotationCount, U_32 tabLevel);
static void dumpTypeAnnotations(
    J9CfrClassFile* classfile, J9CfrTypeAnnotation* annotations, U_32 annotationCount, U_32 tabLevel);
...
static void sun_formatBytecode(J9CfrClassFile* classfile, J9CfrMethod* method, BOOLEAN bigEndian, U_8* bcStart,
    U_8* bytes, U_8 bc, U_32 bytesLength, U_32 decode, char* formatString, IDATA length);
static void sun_formatBytecodes(J9CfrClassFile* classfile, J9CfrMethod* method, BOOLEAN bigEndian, U_8* bytecodes,
    U_32 bytecodesLength, char* formatString, IDATA stringLength);
...
static void j9_formatBytecode(J9ROMClass* romClass, J9ROMMethod* method, U_8* bcStart, U_8* bytes, U_8 bc,
    U_32 bytesLength, U_32 decode, char* formatString, IDATA length, U_32 flags);
static void j9_formatBytecodes(J9ROMClass* romClass, J9ROMMethod* method, U_8* bytecodes, U_32 bytecodesLength,
    char* formatString, IDATA stringLength, U_32 flags);
...
static void printDisassembledMethod(
    J9CfrClassFile* classfile, J9CfrMethod* method, BOOLEAN bigEndian, U_8* bytecodes, U_32 bytecodesLength);
 ...
 * @param [out] output    if not NULL then on exit points to list of converted class file names. Caller responsible for
 * cleanup
...
static I_32 convertToJImageLocations(J9JImage* jimage, char** files, JImageMatchInfo** output, I_32* fileCount)
{
    ...
    matchData
        = (JImageMatchInfo*)j9mem_allocate_memory((count * sizeof(JImageMatchInfo) + size), J9MEM_CATEGORY_CLASSES);
    if (NULL == matchData) {
        result = RET_ALLOCATE_FAILED;
    ...
    for (i = 0; i < count; ++i) {
        ...
        if ((length > sizeof(CFDUMP_CLASSFILE_EXTESION))
...
static void dumpField(J9CfrClassFile* classfile, J9CfrField* field)
{
    ...
    j9tty_printf(PORTLIB, "  Signature: %i -> %s\n", field->descriptorIndex,
        classfile->constantPool[field->descriptorIndex].bytes);
...
static void dumpMethod(J9CfrClassFile* classfile, J9CfrMethod* method)
{
    ...
    j9tty_printf(PORTLIB, "  Signature: %i -> %s\n", method->descriptorIndex,
        classfile->constantPool[method->descriptorIndex].bytes);
...
static void dumpAttribute(J9CfrClassFile* classfile, J9CfrAttribute* attrib, U_32 tabLevel)
{
    ...
    case CFR_ATTRIBUTE_ConstantValue:
        ...
        case CFR_CONSTANT_Long:
            ...
            j9tty_printf(PORTLIB, "(long) 0x%08X%08X\n", classfile->constantPool[index].slot1,
                classfile->constantPool[index].slot2);
            break;
    ...
    case CFR_ATTRIBUTE_InnerClasses:
        ...
        for (i = 0; i < classes->numberOfClasses; i++) {
            ...
            j9tty_printf(PORTLIB, "Inner Class: %i -> %s\n", classes->classes[i].innerClassInfoIndex,
                classfile->constantPool[index].bytes);

            for (j = 0; j < tabLevel + 2; j++)
                j9tty_printf(PORTLIB, "  ");
            if (classes->classes[i].outerClassInfoIndex == 0) {
                ...
                j9tty_printf(PORTLIB, "Outer Class: %i -> %s\n", classes->classes[i].outerClassInfoIndex,
                    classfile->constantPool[index].bytes);
            }
            ...
            printModifiers(
                PORTLIB, classes->classes[i].innerClassAccessFlags, ONLY_SPEC_MODIFIERS, MODIFIERSOURCE_CLASS);
            j9tty_printf(PORTLIB, " )\n");
        }
        break;
    ...
    case CFR_ATTRIBUTE_LineNumberTable:
        for (i = 0; i < ((J9CfrAttributeLineNumberTable*)attrib)->lineNumberTableLength; i++) {
            ...
            j9tty_printf(PORTLIB, "pc: %i line number: %i\n",
                ((J9CfrAttributeLineNumberTable*)attrib)->lineNumberTable[i].startPC,
                ((J9CfrAttributeLineNumberTable*)attrib)->lineNumberTable[i].lineNumber);
        }
        break;
    ...
    case CFR_ATTRIBUTE_LocalVariableTable:
        for (i = 0; i < ((J9CfrAttributeLocalVariableTable*)attrib)->localVariableTableLength; i++) {
            ...
            j9tty_printf(PORTLIB, "from %i to %i slot %i is %i -> %s type %i -> %s\n",
                ...
                ((J9CfrAttributeLocalVariableTable*)attrib)->localVariableTable[i].startPC
                    + ((J9CfrAttributeLocalVariableTable*)attrib)->localVariableTable[i].length,
                ((J9CfrAttributeLocalVariableTable*)attrib)->localVariableTable[i].index, index,
                classfile->constantPool[index].bytes, index2, classfile->constantPool[index2].bytes, 0);
        }
        break;
    ...
    case CFR_ATTRIBUTE_LocalVariableTypeTable:
        for (i = 0; i < ((J9CfrAttributeLocalVariableTypeTable*)attrib)->localVariableTypeTableLength; i++) {
            ...
            j9tty_printf(PORTLIB, "from %i to %i slot %i is %i -> %s type %i -> %s\n",
                ...
                ((J9CfrAttributeLocalVariableTypeTable*)attrib)->localVariableTypeTable[i].index, index,
                classfile->constantPool[index].bytes, index2, classfile->constantPool[index2].bytes, 0);
        }
        break;
    ...
    case CFR_ATTRIBUTE_RuntimeVisibleTypeAnnotations: {
        ...
        if (0 == annotations->rawDataLength) { /* the attribute was well-formed */
            dumpTypeAnnotations(
                classfile, annotations->typeAnnotations, annotations->numberOfAnnotations, tabLevel + 1);
        } else {
            dumpRawData("Malformed RuntimeVisibleTypeAnnotations attribute", annotations->rawDataLength,
                annotations->rawAttributeData);
        }
    }
    ...
    case CFR_ATTRIBUTE_RuntimeInvisibleTypeAnnotations: {
        J9CfrAttributeRuntimeInvisibleTypeAnnotations* annotations
            = (J9CfrAttributeRuntimeInvisibleTypeAnnotations*)attrib;
        if (0 == annotations->rawDataLength) { /* the attribute was well-formed */
            dumpTypeAnnotations(
                classfile, annotations->typeAnnotations, annotations->numberOfAnnotations, tabLevel + 1);
        } else {
            dumpRawData("Malformed  RuntimeInvisibleTypeAnnotations attribute", annotations->rawDataLength,
                annotations->rawAttributeData);
        }
    } break;
    ...
    case CFR_ATTRIBUTE_MethodParameters: {
        ...
        for (i = 0; i < ((J9CfrAttributeMethodParameters*)attrib)->numberOfMethodParameters; i++) {
            ...
            if (0 != index) {
                bytes
                    = classfile->constantPool[((J9CfrAttributeMethodParameters*)attrib)->methodParametersIndexTable[i]]
                          .bytes;
            }
        ...
        for (i = 0; i < ((J9CfrAttributeMethodParameters*)attrib)->numberOfMethodParameters; i++) {
            ...
            for (j = 0; j < tabLevel + 1; j++)
                j9tty_printf(PORTLIB, "  ");
            j9tty_printf(
                PORTLIB, "%i -> %s", ((J9CfrAttributeMethodParameters*)attrib)->methodParametersIndexTable[i], bytes);
            if (0 != ((J9CfrAttributeMethodParameters*)attrib)->flags[i]) {
                for (j = 0; j < maxLength - ((U_32)strlen((const char*)bytes)) + ((U_32)strlen("Name")); j++)
                    j9tty_printf(PORTLIB, " ");
                j9tty_printf(PORTLIB, "0x%x ( ", ((J9CfrAttributeMethodParameters*)attrib)->flags[i]);
                printModifiers(PORTLIB, ((J9CfrAttributeMethodParameters*)attrib)->flags[i], ONLY_SPEC_MODIFIERS,
    ...
    case CFR_ATTRIBUTE_RuntimeVisibleParameterAnnotations: {
        J9CfrAttributeRuntimeVisibleParameterAnnotations* annotations
        ...
        if (0 == annotations->rawDataLength) { /* the attribute was well-formed */
            ...
            dumpRawData("Malformed RuntimeVisibleParameterAnnotations attribute", annotations->rawDataLength,
                annotations->rawAttributeData);
        }
    }
    ...
    case CFR_ATTRIBUTE_RuntimeInvisibleParameterAnnotations: {
        J9CfrAttributeRuntimeInvisibleParameterAnnotations* annotations
            = (J9CfrAttributeRuntimeInvisibleParameterAnnotations*)attrib;
        if (0 == annotations->rawDataLength) { /* the attribute was well-formed */
            for (i = 0; i < tabLevel + 1; i++)
                j9tty_printf(PORTLIB, "  ");
            j9tty_printf(PORTLIB, "Parameter Count (%i):\n", annotations->numberOfParameters);
            ...
            dumpRawData("Malformed RuntimeInvisibleParameterAnnotations attribute", annotations->rawDataLength,
                annotations->rawAttributeData);
        }
    }
...
static void printClassFile(J9CfrClassFile* classfile)
{
    ...
    if (classfile->accessFlags & CFR_ACC_PUBLIC)
        j9tty_printf(PORTLIB, "public ");
    if (classfile->accessFlags & CFR_ACC_PRIVATE)
        j9tty_printf(PORTLIB, "protected ");
    if (classfile->accessFlags & CFR_ACC_PROTECTED)
        j9tty_printf(PORTLIB, "private ");
    if (classfile->accessFlags & CFR_ACC_INTERFACE)
        j9tty_printf(PORTLIB, "interface ");
    else {
        if (classfile->accessFlags & CFR_ACC_ABSTRACT)
...
static void printMethod(J9CfrClassFile* classfile, J9CfrMethod* method)
{
    ...
    /* Access flags. */
    if (method->accessFlags & CFR_ACC_PUBLIC)
        j9tty_printf(PORTLIB, "public ");
    if (method->accessFlags & CFR_ACC_PRIVATE)
        j9tty_printf(PORTLIB, "private ");
    if (method->accessFlags & CFR_ACC_PROTECTED)
        j9tty_printf(PORTLIB, "protected ");
    if (method->accessFlags & CFR_ACC_STATIC)
        j9tty_printf(PORTLIB, "static ");
    if (method->accessFlags & CFR_ACC_FINAL)
        j9tty_printf(PORTLIB, "final ");
    if (method->accessFlags & CFR_ACC_SYNCHRONIZED)
        j9tty_printf(PORTLIB, "synchronized ");
    if (method->accessFlags & CFR_ACC_NATIVE)
        j9tty_printf(PORTLIB, "native ");
    if (method->accessFlags & CFR_ACC_ABSTRACT)
        j9tty_printf(PORTLIB, "abstract ");
    if (method->accessFlags & CFR_ACC_STRICT)
...
static void printField(J9CfrClassFile* classfile, J9CfrField* field)
{
    ...
    /* Access flags. */
    if (field->accessFlags & CFR_ACC_PUBLIC)
        j9tty_printf(PORTLIB, "public ");
    if (field->accessFlags & CFR_ACC_PRIVATE)
        j9tty_printf(PORTLIB, "private ");
    if (field->accessFlags & CFR_ACC_PROTECTED)
        j9tty_printf(PORTLIB, "protected ");
    if (field->accessFlags & CFR_ACC_STATIC)
        j9tty_printf(PORTLIB, "static ");
    if (field->accessFlags & CFR_ACC_FINAL)
        j9tty_printf(PORTLIB, "final ");
    if (field->accessFlags & CFR_ACC_VOLATILE)
        j9tty_printf(PORTLIB, "volatile ");
    if (field->accessFlags & CFR_ACC_TRANSIENT)
    ...
    J9CfrClassFile* classfile, J9CfrMethod* method, BOOLEAN bigEndian, U_8* bytecodes, U_32 bytecodesLength)
{
    ...
    if (code->exceptionTableLength) {
        ...
        for (i = 0; i < code->exceptionTableLength; i++) {
            j9tty_printf(PORTLIB, "%5i%6i%10i   ", code->exceptionTable[i].startPC, code->exceptionTable[i].endPC,
                code->exceptionTable[i].handlerPC, 0);
...
static void dumpHelpText(J9PortLibrary* portLib, int argc, char** argv)
{
    ...
    j9file_printf(PORTLIB, J9PORT_TTY_OUT,
        "  -dr:<depth>      dump J9 ROM Class physical memory layout with specified nesting depth\n");
    ...
    j9file_printf(PORTLIB, J9PORT_TTY_OUT,
        "                       -drq:/methods/method[3]/name,/methods/method[3]/methodBytecodes\n");
    ...
    j9file_printf(
        PORTLIB, J9PORT_TTY_OUT, "  -jh:<jimagefile> load JImage file and display header and resources metadata\n");
    j9file_printf(PORTLIB, J9PORT_TTY_OUT,
        "  -jp:<jimagefile> display the module containing the given package in the JImage file\n");
    ...
    j9file_printf(
        PORTLIB, J9PORT_TTY_OUT, "  -oh              write out J9 ROM Class to file under the package hierarchy\n");
    ...
    j9file_printf(PORTLIB, J9PORT_TTY_OUT,
        "  -xm              translate to J9 ROM format with local, debug and stack maps (print only)\n");
...
static I_32 writeOutputFile(U_8* fileData, U_32 length, char* filename, char* extension)
{
    ...
    if (NULL != extension) {
        ...
        if ((i < (extnLen + 1)) || (0 != strcmp(filename + i - extnLen, extension))) {
            newFilename
                = j9mem_allocate_memory(i + extnLen + 1, J9MEM_CATEGORY_CLASSES); /* filename + extension + null */
...
static int parseCommandLine(int argc, char** argv, char** envp)
{
    ...
    for (i = 1; i < argc; i++) {
        ...
        case 'z':
            ...
            if (argv[i][2] != ':') {
                if ((strlen(argv[i]) < 5) || strncmp(argv[i], "-zip:", 5))
                    goto incomplete;
...
static U_32 buildFlags(void)
{
    ...
    if (options.options & OPTION_stripDebugAttributes)
        flags |= CFR_StripDebugAttributes;
    if (options.options & OPTION_stripDebugLines)
        flags |= BCT_StripDebugLines;
    if (options.options & OPTION_stripDebugSource)
        flags |= BCT_StripDebugSource;
    if (options.options & OPTION_stripDebugVars)
        flags |= BCT_StripDebugVars;
    if (options.options & OPTION_check)
        flags |= CFR_StaticVerification;
    if (options.options & OPTION_littleEndian)
        flags |= BCT_LittleEndianOutput;
    if (options.options & OPTION_bigEndian)
        flags |= BCT_BigEndianOutput;
    if (options.options & OPTION_leaveJSRs)
        flags |= CFR_LeaveJSRs;
    if (options.options & OPTION_pedantic)
        flags |= BCT_Xfuture;
    if (options.options & OPTION_dumpMaps)
        flags |= BCT_DumpMaps;
    return flags;
}
...
static I_32 processClassFile(J9CfrClassFile* classfile, U_32 dataLength, char* requestedFile, U_32 flags)
{
    ...
    case ACTION_formatBytecodes:
        for (i = 0; i < classfile->methodsCount; i++) {
            sun_formatBytecodes(
                classfile, &(classfile->methods[i]), TRUE, NULL, 0, options.actionString, strlen(options.actionString));
        }
        break;
...
static I_32 processFilesInZIP(char* zipFilename, char** files, U_32 flags)
{
    ...
    for (i = 0; i < fileCount; i++) {
        ...
        result = zip_getZipEntry(
            PORTLIB, &zipFile, &entry, requestedFile, filenameLength, J9ZIP_GETENTRY_READ_DATA_POINTER);
        if (result) {
 ...
 * @param [in/out] pointer to a buffer for storing resource data; if the buffer is not large enough, it is freed and
 * reallocated to desired amount
 * @param [in/out] pointer to integer specifying input buffer size; if the buffer is reallocated, then it is updated
 * with the new size
 * @param [in] flags build flags used during processing of class files
 *
 * @return RET_SUCCESS on success, negative error code on failure
 */
static I_32 processJImageResource(const char* jimageFileName, J9JImage* jimage, J9JImageLocation* j9jimageLocation,
    const char* resourceName, U_8** dataBuffer, UDATA* dataBufferSize, U_32 flags)
{
...
static I_32 processAllInJImage(J9JImage* jimage, char* jimageFileName, U_32 flags)
{
    ...
    for (lotIndex = 0; lotIndex < jimageHeader->tableLength; lotIndex++) {
        ...
        if (locationOffset >= JIMAGE_LOCATION_DATA_SIZE(*jimageHeader)) {
            j9tty_printf(PORTLIB,
                "Invalid location offset found at index %u in Locations Offset Table of jimage file %s\n", lotIndex,
                jimageFileName);
        ...
        result = j9bcutil_createAndVerifyJImageLocation(
            PORTLIB, jimage, NULL /* resourceName */, imageLocation, &j9jimageLocation);
        if (J9JIMAGE_NO_ERROR != result) {
        ...
        if ((NULL == j9jimageLocation.extensionString)
            || (strncmp(j9jimageLocation.extensionString, CFDUMP_CLASSFILE_EXTESION_WITHOUT_DOT,
                   sizeof(CFDUMP_CLASSFILE_EXTESION_WITHOUT_DOT)))) {
            continue;
        }

        result = j9bcutil_getJImageResourceName(PORTLIB, jimage, j9jimageLocation.moduleString,
            j9jimageLocation.parentString, j9jimageLocation.baseString, j9jimageLocation.extensionString,
            &resourceName);
        if (J9JIMAGE_NO_ERROR == result) {
            result = processJImageResource(
                jimageFileName, jimage, &j9jimageLocation, resourceName, &dataBuffer, &dataBufferSize, flags);
 ...
 * @return RET_SUCCESS on success, negative error code on failure
 */
static I_32 processFilesInJImage(J9JImage* jimage, char* jimageFileName, char** resources, U_32 flags)
{
    J9JImageHeader* j9jimageHeader = jimage->j9jimageHeader;
    JImageHeader* jimageHeader = j9jimageHeader->jimageHeader;
    ...
    for (i = 0; i < resourceCount; i++) {
        ...
        if (ACTION_writeJImageResource == options.action) {
            ...
            result = j9bcutil_getJImageResourceName(PORTLIB, jimage, matchInfo[i].module, matchInfo[i].parentString,
                matchInfo[i].baseString, CFDUMP_CLASSFILE_EXTESION_WITHOUT_DOT, &resourceName);
...
static J9CfrClassFile* loadClassFromBytes(U_8* data, U_32 dataLength, char* requestedFile, U_32 flags)
{
    ...
    if (verbose)
        j9tty_printf(PORTLIB, "<file size: %i bytes>\n", dataLength);
    if (verbose)
        j9tty_printf(PORTLIB, "<estimated memory size: %i bytes>\n", segmentLength);
    ...
    do {
        ...
        result = j9bcutil_readClassFileBytes(PORTLIB, verifyBuffers ? j9bcv_verifyClassStructure : NULL, data,
            dataLength, segment, segmentLength, flags, NULL, NULL, 0, UDATA_MAX);

        if (result == -2) {
            /* Not enough space. Retry. */
            if (verbose)
                j9tty_printf(PORTLIB, "<increasing space to: %i bytes>\n", segmentLength * 2);
...
static I_32 processROMClass(J9ROMClass* romClass, char* requestedFile, U_32 flags)
{
    ...
    case ACTION_formatBytecodes: {
        ...
        for (j = 0; j < romClass->romMethodCount; j++) {
            j9_formatBytecodes(
                romClass, currentMethod, NULL, 0, options.actionString, strlen(options.actionString), flags);
            currentMethod = nextROMMethod(currentMethod);
        }
    } break;
    ...
    case ACTION_queryRomClass:
        j9bcutil_queryROMClassCommaSeparated(
            portLib, romClass, NULL, options.actionString, validateROMClassAddressRange);
        break;
    ...
    U_8* bytes, U_8 bc, U_32 bytesLength, U_32 decode, char* formatString, IDATA length)
{
    ...
    while (i < length) {
        ...
        if (modifier || ch == '%') {
            ...
            if (ch == '\0' || ch == '%') {
                if (modifier)
                    j9tty_printf(PORTLIB, "%%%c", modifier);
                ...
                case 'B':
                    ...
                    case 'x':
                        for (j = 0; j < bytesLength; j++)
                            j9tty_printf(PORTLIB, "%02x", bytes[j]);
                        break;
                ...
                case 'X':
                    ...
                    if (formatString[i] == 'i' || formatString[i] == 'B' || formatString[i] == 'b'
                        || formatString[i] == 'p') {
                        modifier = ch;
                        break;
                    }
    ...
    U_32 bytecodesLength, char* formatString, IDATA stringLength)
{
    ...
    while ((U_32)pc < length) {
        ...
        case CFR_BC_bipush:
            sun_formatBytecode(
                classfile, method, bigEndian, bytecodes, bcIndex, bc, 2, CFR_DECODE_I8, formatString, stringLength);
            pc++;
            bcIndex += 2;
            break;
        ...
        case CFR_BC_sipush:
            sun_formatBytecode(
                classfile, method, bigEndian, bytecodes, bcIndex, bc, 3, CFR_DECODE_I16, formatString, stringLength);
            pc += 2;
            bcIndex += 3;
            break;
        ...
        case CFR_BC_ldc:
            sun_formatBytecode(
                classfile, method, bigEndian, bytecodes, bcIndex, bc, 2, CFR_DECODE_CP8, formatString, stringLength);
            pc++;
            bcIndex += 2;
            break;
        ...
        case CFR_BC_ldc2_w:
            sun_formatBytecode(
                classfile, method, bigEndian, bytecodes, bcIndex, bc, 3, CFR_DECODE_CP16, formatString, stringLength);
            pc += 2;
            bcIndex += 3;
            break;
        ...
        case CFR_BC_ret:
            if (wide) {
                sun_formatBytecode(classfile, method, bigEndian, bytecodes, bcIndex, bc, 4, CFR_DECODE_U16,
                    formatString, stringLength);
                pc += 2;
                bcIndex += 3;
                wide = FALSE;
            } else {
                sun_formatBytecode(
                    classfile, method, bigEndian, bytecodes, bcIndex, bc, 2, CFR_DECODE_U8, formatString, stringLength);
                pc++;
                bcIndex += 2;
            }
            break;
        ...
        case CFR_BC_iinc:
            if (wide) {
                sun_formatBytecode(classfile, method, bigEndian, bytecodes, bcIndex, bc, 6, CFR_DECODE_U16_I16,
                    formatString, stringLength);
                pc += 4;
                bcIndex += 5;
                wide = FALSE;
            } else {
                sun_formatBytecode(classfile, method, bigEndian, bytecodes, bcIndex, bc, 3, CFR_DECODE_U8_I8,
                    formatString, stringLength);
                pc += 2;
                bcIndex += 3;
            }
        ...
        case CFR_BC_ifnonnull:
            sun_formatBytecode(
                classfile, method, bigEndian, bytecodes, bcIndex, bc, 3, CFR_DECODE_L16, formatString, stringLength);
            pc += 2;
            bcIndex += 3;
            break;
        ...
        case CFR_BC_tableswitch:
            ...
            sun_formatBytecode(classfile, method, bigEndian, bytecodes, bcIndex, bc, bcLength, CFR_DECODE_TABLESWITCH,
                formatString, stringLength);
            bcIndex += bcLength;
            pc += bcLength - 1;
            break;
        ...
        case CFR_BC_lookupswitch:
            ...
            sun_formatBytecode(classfile, method, bigEndian, bytecodes, bcIndex, bc, bcLength, CFR_DECODE_LOOKUPSWITCH,
                formatString, stringLength);
            bcIndex += bcLength;
            pc += bcLength - 1;
            break;
        ...
        case CFR_BC_newarray:
            sun_formatBytecode(classfile, method, bigEndian, bytecodes, bcIndex, bc, 2, CFR_DECODE_NEWARRAY,
                formatString, stringLength);
            pc++;
            bcIndex += 2;
            break;
        ...
        case CFR_BC_multianewarray:
            sun_formatBytecode(classfile, method, bigEndian, bytecodes, bcIndex, bc, 4, CFR_DECODE_MULTIANEWARRAY,
                formatString, stringLength);
            pc += 3;
            bcIndex += 4;
            break;
        ...
        case CFR_BC_jsr_w:
            sun_formatBytecode(
                classfile, method, bigEndian, bytecodes, bcIndex, bc, 5, CFR_DECODE_L32, formatString, stringLength);
            pc += 4;
            bcIndex += 5;
            break;
        ...
        default:
            sun_formatBytecode(
                classfile, method, bigEndian, bytecodes, bcIndex, bc, 1, CFR_DECODE_SIMPLE, formatString, stringLength);
            bcIndex++;
            break;
        }
        j9tty_printf(PORTLIB, "\n");
            ...
            if (ch == '\0' || ch == '%') {
                ...
                case 'i':
                    ...
                    for (j = 0; j < classfile->interfacesCount;) {
                        ...
                        if (++j != classfile->interfacesCount)
                            j9tty_output_char(',');
                    }
                    break;
            ...
            if (ch == '\0' || ch == '%') {
                ...
                case 'p':
                    ...
                    case CFR_DECODE_J9_FIELDREF:
                        ...
                        default:
                            info = &constantPool[u16];
                            utfLength = J9UTF8_LENGTH(J9ROMCLASSREF_NAME(
                                (J9ROMClassRef*)&constantPool[((J9ROMFieldRef*)info)->classRefCPIndex]));
                            string = ((U_8*)J9ROMCLASSREF_NAME(
                                         (J9ROMClassRef*)&constantPool[((J9ROMFieldRef*)info)->classRefCPIndex]))
                                + 2;
                            for (j = 0; j < utfLength; j++) {
                                ch2 = string[j];
    ...
    char* formatString, IDATA stringLength, U_32 flags)
{
    ...
    if ((method->modifiers & CFR_ACC_NATIVE) || (method->modifiers & CFR_ACC_ABSTRACT))
        return;
    if (bytecodes) {
        length = bytecodesLength;
        if (length == 0)
    ...
    while ((U_32)pc < length) {
        ...
        case JBbipush:
            j9_formatBytecode(
                romClass, method, bytecodes, bcIndex, bc, 2, CFR_DECODE_I8, formatString, stringLength, flags);
            pc++;
            bcIndex += 2;
            break;
        ...
        case JBsipush:
            j9_formatBytecode(
                romClass, method, bytecodes, bcIndex, bc, 3, CFR_DECODE_I16, formatString, stringLength, flags);
            pc += 2;
            bcIndex += 3;
            break;
        ...
        case JBldc:
            j9_formatBytecode(
                romClass, method, bytecodes, bcIndex, bc, 2, CFR_DECODE_J9_LDC, formatString, stringLength, flags);
            pc++;
            bcIndex += 2;
            break;
        ...
        case JBldcw:
            j9_formatBytecode(
                romClass, method, bytecodes, bcIndex, bc, 3, CFR_DECODE_J9_LDCW, formatString, stringLength, flags);
            pc += 2;
            bcIndex += 3;
            break;
        ...
        case JBldc2dw:
            j9_formatBytecode(
                romClass, method, bytecodes, bcIndex, bc, 3, CFR_DECODE_J9_LDC2DW, formatString, stringLength, flags);
            pc += 2;
            bcIndex += 3;
            break;
        ...
        case JBldc2lw:
            j9_formatBytecode(
                romClass, method, bytecodes, bcIndex, bc, 3, CFR_DECODE_J9_LDC2LW, formatString, stringLength, flags);
            pc += 2;
            bcIndex += 3;
            break;
        ...
        case JBastore:
            j9_formatBytecode(
                romClass, method, bytecodes, bcIndex, bc, 2, CFR_DECODE_U8, formatString, stringLength, flags);
            pc++;
            bcIndex += 2;
            break;
        ...
        case JBastorew:
            j9_formatBytecode(
                romClass, method, bytecodes, bcIndex, bc, 4, CFR_DECODE_U16, formatString, stringLength, flags);
            pc += 2;
            bcIndex += 3;
            break;
        ...
        case JBiinc:
            j9_formatBytecode(
                romClass, method, bytecodes, bcIndex, bc, 3, CFR_DECODE_U8_I8, formatString, stringLength, flags);
            pc += 2;
            bcIndex += 3;
            break;
        ...
        case JBiincw:
            j9_formatBytecode(
                romClass, method, bytecodes, bcIndex, bc, 6, CFR_DECODE_U16_I16, formatString, stringLength, flags);
            pc += 4;
            bcIndex += 5;
            break;
        ...
        case JBifnonnull:
            j9_formatBytecode(
                romClass, method, bytecodes, bcIndex, bc, 3, CFR_DECODE_L16, formatString, stringLength, flags);
            pc += 2;
            bcIndex += 3;
            break;
        ...
        case JBtableswitch:
            ...
            high = (I_32)index;
            bcLength = (U_32)(tempIndex - bcIndex) + ((high - low + 1) * 4);
            j9_formatBytecode(romClass, method, bytecodes, bcIndex, bc, bcLength, CFR_DECODE_TABLESWITCH, formatString,
                stringLength, flags);
            bcIndex += bcLength;
            pc += bcLength - 1;
            break;
        ...
        case JBlookupswitch:
            ...
            NEXT_U32_ENDIAN(bigEndian, npairs, tempIndex);
            bcLength = (U_32)(tempIndex - bcIndex) + (npairs * 8);
            j9_formatBytecode(romClass, method, bytecodes, bcIndex, bc, bcLength, CFR_DECODE_LOOKUPSWITCH, formatString,
                stringLength, flags);
            bcIndex += bcLength;
            pc += bcLength - 1;
            break;
...
#endif /* J9VM_OPT_VALHALLA_VALUE_TYPES */
            j9_formatBytecode(
                romClass, method, bytecodes, bcIndex, bc, 3, CFR_DECODE_J9_FIELDREF, formatString, stringLength, flags);
            pc += 2;
            bcIndex += 3;
            break;
        ...
        case JBinvokespecialsplit:
            j9_formatBytecode(romClass, method, bytecodes, bcIndex, bc, 3, CFR_DECODE_J9_METHODREF, formatString,
                stringLength, flags);
            pc += 2;
            bcIndex += 3;
            break;
...
#endif /* J9VM_OPT_VALHALLA_VALUE_TYPES */
            j9_formatBytecode(
                romClass, method, bytecodes, bcIndex, bc, 3, CFR_DECODE_J9_CLASSREF, formatString, stringLength, flags);
            pc += 2;
            bcIndex += 3;
            break;
        ...
        case JBnewarray:
            j9_formatBytecode(
                romClass, method, bytecodes, bcIndex, bc, 2, CFR_DECODE_NEWARRAY, formatString, stringLength, flags);
            pc++;
            bcIndex += 2;
            break;

        case JBmultianewarray:
            j9_formatBytecode(romClass, method, bytecodes, bcIndex, bc, 4, CFR_DECODE_MULTIANEWARRAY, formatString,
                stringLength, flags);
            pc += 3;
            bcIndex += 4;
            break;
        ...
        case JBgotow:
            j9_formatBytecode(
                romClass, method, bytecodes, bcIndex, bc, 5, CFR_DECODE_L32, formatString, stringLength, flags);
            pc += 4;
            bcIndex += 5;
            break;
        ...
        default:
            j9_formatBytecode(
                romClass, method, bytecodes, bcIndex, bc, 1, CFR_DECODE_SIMPLE, formatString, stringLength, flags);
            bcIndex++;
            break;
        }
        j9tty_printf(PORTLIB, "\n");
...
static void reportClassLoadError(J9CfrError* error, char* requestedFile)
{
    ...
    if (error->errorMethod == -1) {
        ...
        j9tty_printf(PORTLIB, "Verification error in method %i (%.*s%.*s) at PC %i\n", error->errorMethod, name->slot1,
            name->bytes, sig->slot1, sig->bytes, error->errorPC);
...
static U_8* loadBytes(char* requestedFile, U_32* lengthReturn, U_32 flags)
{
    ...
    if (verbose)
        j9tty_printf(PORTLIB, "<file read time: %d us.>\n", endTime - startTime);
    ...
    J9CfrClassFile* classfile, J9CfrTypeAnnotation* annotations, U_32 annotationCount, U_32 tabLevel)
{
    ...
    for (annotationIndex = 0; annotationIndex < annotationCount; ++annotationIndex) {
        ...
        case CFR_TARGET_TYPE_TypeParameterGenericClass:
        case CFR_TARGET_TYPE_TypeParameterGenericMethod:
            j9tty_printf(PORTLIB, "type_parameter_index=%d",
                currentAnnotation->targetInfo.typeParameterTarget.typeParameterIndex);
            break;
        ...
        case CFR_TARGET_TYPE_TypeInBoundOfGenericMethod:
            j9tty_printf(PORTLIB, "type_parameter_index=%d bound_index=%d",
                currentAnnotation->targetInfo.typeParameterBoundTarget.typeParameterIndex,
                currentAnnotation->targetInfo.typeParameterBoundTarget.boundIndex);
            break;

        case CFR_TARGET_TYPE_TypeInFormalParam:
            j9tty_printf(PORTLIB, "formal_parameter_index=%d",
                currentAnnotation->targetInfo.methodFormalParameterTarget.formalParameterIndex);
            break;
        ...
        case CFR_TARGET_TYPE_TypeInExceptionParam:
            j9tty_printf(
                PORTLIB, "exception_table_index=%d", currentAnnotation->targetInfo.catchTarget.exceptiontableIndex);
            break;
        ...
        case CFR_TARGET_TYPE_TypeForGenericMethodInvocationInMethodRef:
            j9tty_printf(PORTLIB, "offset=%d type_argument_index=%d",
                currentAnnotation->targetInfo.typeArgumentTarget.offset,
                currentAnnotation->targetInfo.typeArgumentTarget.typeArgumentIndex);
            break;
        ...
        for (pi = 0; pi < currentAnnotation->typePath.pathLength; ++pi) {
            ...
            INDENT(tabLevel);
            j9tty_printf(
                PORTLIB, "type_path_kind=%d type_argument_index=%d\n", entry->typePathKind, entry->typeArgumentIndex);
        }
...
static U_8* dumpStackMapSlots(J9CfrClassFile* classfile, U_8* slotData, U_16 slotCount)
{
    ...
    const char* slotTypes[] = { "top", "int", "float", "double", "long", "null", "uninitialized_this" };
