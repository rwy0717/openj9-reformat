...
static void addUTF8RegionSRP(
    J9ROMClassGatherLayoutInfoState* state, J9ROMClass* romClass, J9SRP* srpPtr, const char* name);
...
static void getRegionValueString(
    J9PortLibrary* portLib, J9ROMClass* romClass, J9ROMClassRegion* region, char* str, size_t strSize);
static void getRegionDetailString(J9PortLibrary* portLib, J9ROMClassGatherLayoutInfoState* state, J9ROMClass* romClass,
    J9ROMClassRegion* region, char* str, size_t strSize, UDATA base);
static void printRegionLine(J9PortLibrary* portLib, J9ROMClassGatherLayoutInfoState* state, J9ROMClass* romClass,
    J9ROMClassRegion* section, UDATA base);
static BOOLEAN isSameRegion(J9ROMClassRegion* region1, J9ROMClassRegion* region2);
static void reportSuspectedPadding(J9PortLibrary* portLib, J9ROMClass* romClass, J9ROMClassGatherLayoutInfoState* state,
    UDATA lastOffset, UDATA offset, UDATA base);
...
static void addNASRegion(J9ROMClassGatherLayoutInfoState* state, J9ROMClass* romClass, J9SRP* srpPtr, const char* name)
{
    ...
    if (NULL != nas) {
        ...
        if ((offset >= base) && (offset < (base + romClass->romSize))) {
            addRegion(
                state->regionsPool, offset - base, sizeof(J9ROMNameAndSignature), J9ROM_SRP, "nameAndSignatureSRP");
...
static int compareRegions(const void* r1, const void* r2)
{
    ...
    J9ROMClassValidateRangeCallback validateRangeCallback, J9ROMClassGatherLayoutInfoState* state)
{
    ...
    state->regionsPool = pool_new(
        sizeof(J9ROMClassRegion), 64, 0, 0, J9_GET_CALLSITE(), J9MEM_CATEGORY_CLASSES, POOL_FOR_PORT(PORTLIB));
...
static BOOLEAN isSameRegion(J9ROMClassRegion* region1, J9ROMClassRegion* region2)
{
    BOOLEAN sameRegion = (region1->type == region2->type) && (region1->offset == region2->offset)
...
static void printRegionLine(J9PortLibrary* portLib, J9ROMClassGatherLayoutInfoState* state, J9ROMClass* romClass,
    J9ROMClassRegion* region, UDATA base)
{
    ...
    j9tty_printf(PORTLIB, "0x%p-0x%p [ %12s %-28s ]%s\n", base + region->offset, base + region->offset + region->length,
        valueString, region->name, detailString);
}
    ...
    J9ROMClassRegion* region, char* str, size_t strSize, UDATA base)
{
    ...
    *str = '\0';
    if (J9ROM_UTF8 == region->type) {
        ...
        j9str_printf(
            PORTLIB, str, strSize, " %5d bytes  !j9x 0x%p,0x%p", region->length, base + region->offset, region->length);
    }
...
void j9bcutil_linearDumpROMClass(J9PortLibrary* portLib, J9ROMClass* romClass, void* baseAddress,
    UDATA nestingThreshold, J9ROMClassValidateRangeCallback validateRangeCallback)
{
...
void j9bcutil_queryROMClass(J9PortLibrary* portLib, J9ROMClass* romClass, void* baseAddress, const char** queries,
    UDATA numQueries, J9ROMClassValidateRangeCallback validateRangeCallback)
{
    J9ROMClassGatherLayoutInfoState state;
    ...
    queryComponentPool = pool_new(
        sizeof(J9ROMClassQueryComponent), 64, 0, 0, J9_GET_CALLSITE(), J9MEM_CATEGORY_CLASSES, POOL_FOR_PORT(PORTLIB));
    ...
    for (queryIndex = 0; queryIndex < numQueries; queryIndex++) {
        ...
        for (i = 0; i < state.numRegions; i++) {
            ...
            if (NULL == componentToMatch) {
                ...
                if (J9ROM_SECTION_START == region->type) {
                    j9str_printf(
                        PORTLIB, buf, sizeof(buf), "Section Start: %s (%d bytes)", region->name, region->length);
                    j9tty_printf(PORTLIB, "=== %-59s ===\n", buf);
                    nesting++;
                } else if (J9ROM_SECTION_END == region->type) {
                    nesting--;
...
void j9bcutil_queryROMClassCommaSeparated(J9PortLibrary* portLib, J9ROMClass* romClass, void* baseAddress,
    const char* commaSeparatedQueries, J9ROMClassValidateRangeCallback validateRangeCallback)
{
    char** queries;
    UDATA i, numQueries;
    char* string;
...
static void printIndentation(J9PortLibrary* portLib, UDATA n)
{
    UDATA i;
    PORT_ACCESS_FROM_PORT(portLib);
...
static void getRegionValueStringXML(
    J9PortLibrary* portLib, J9ROMClass* romClass, J9ROMClassRegion* region, char* str, size_t strSize)
{
    void* fieldPtr;
...
void j9bcutil_linearDumpROMClassXML(
    J9PortLibrary* portLib, J9ROMClass* romClass, J9ROMClassValidateRangeCallback validateRangeCallback)
{
    J9ROMClassGatherLayoutInfoState state;
    UDATA i, nesting;
    BOOLEAN success;
