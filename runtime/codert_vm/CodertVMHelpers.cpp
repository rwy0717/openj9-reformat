...
static J9Method* jitGetExceptionCatcher(
    J9VMThread* currentThread, void* handlerPC, J9JITExceptionTable* metaData, IDATA* location);

void initializeCodertFunctionTable(J9JavaVM* javaVM)
{
...
static void jitEmptyObjectSlotIterator(
    J9VMThread* vmThread, J9StackWalkState* walkState, j9object_t* objectSlot, const void* stackLocation)
{}

void jitCheckScavengeOnResolve(J9VMThread* currentThread)
{
...
void jitMarkMethodReadyForDLT(J9VMThread* currentThread, J9Method* method)
{
    VM_AtomicSupport::bitOr((UDATA*)&method->constantPool, J9_STARTPC_DLT_READY);
}

void jitMethodFailedTranslation(J9VMThread* currentThread, J9Method* method)
{
...
void jitMethodTranslated(J9VMThread* currentThread, J9Method* method, void* jitStartAddress)
{
    J9JavaVM* vm = currentThread->javaVM;
    J9JITConfig* jitConfig = vm->jitConfig;
    if (J9_ARE_NO_BITS_SET(jitConfig->runtimeFlags, J9JIT_TOSS_CODE)) {
...
void* jitNewInstanceMethodStartAddress(J9VMThread* currentThread, J9Class* clazz)
{
    void* addr = NULL;
#if defined(J9VM_JIT_NEW_INSTANCE_PROTOTYPE)
...
void jitNewInstanceMethodTranslated(J9VMThread* currentThread, J9Class* clazz, void* jitStartAddress)
{
#if defined(J9VM_JIT_NEW_INSTANCE_PROTOTYPE)
...
void jitNewInstanceMethodTranslateFailed(J9VMThread* currentThread, J9Class* clazz)
{
#if defined(J9VM_JIT_NEW_INSTANCE_PROTOTYPE)
...
void jitUpdateInlineAttribute(J9VMThread* currentThread, J9Class* classPtr, void* jitCallBack)
{
