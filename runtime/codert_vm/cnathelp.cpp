...
static VMINLINE bool samePCs(void* pc1, void* pc2)
{
 ...
 */
static VMINLINE UDATA convertITableOffsetToVTableOffset(
    J9VMThread* currentThread, J9Class* receiverClass, J9Class* interfaceClass, UDATA iTableOffset)
{
...
static VMINLINE void* buildJITResolveFrameWithPC(
    J9VMThread* currentThread, UDATA flags, UDATA parmCount, bool checkScavengeOnResolve, UDATA spAdjust, void* oldPC)
{
...
static VMINLINE void* restoreJITResolveFrame(
    J9VMThread* currentThread, void* oldPC, bool checkAsync = true, bool checkException = true)
{
    void* addr = NULL;
    J9SFJITResolveFrame* resolveFrame = (J9SFJITResolveFrame*)currentThread->sp;
    if (checkAsync) {
        if (VM_VMHelpers::immediateAsyncPending(currentThread)) {
            if (J9_CHECK_ASYNC_POP_FRAMES
                == currentThread->javaVM->internalVMFunctions->javaCheckAsyncMessages(currentThread, FALSE)) {
...
#define JIT_PARM(number) \
    ((number <= J9SW_ARGUMENT_REGISTER_COUNT) ? JIT_PARM_IN_REGISTER(number) : JIT_PARM_IN_MEMORY(number))
...
#define JIT_RETURN_U64(value)                                                                                   \
    do {                                                                                                        \
        ((J9JITRegisters*)(currentThread->entryLocalStorage->jitGlobalStorageBase))->JIT_HI_U64_RETURN_REGISTER \
            = (UDATA)((value) >> 32);                                                                           \
        ((J9JITRegisters*)(currentThread->entryLocalStorage->jitGlobalStorageBase))->JIT_LO_U64_RETURN_REGISTER \
            = (UDATA)((value)&0xFFFFFFFF);                                                                      \
    } while (0)
#endif /* J9VM_ENV_DATA64 */
...
#else
#define PRESERVE_RETURN_ADDRESS() \
    do {                          \
    } while (0)
#define RESTORE_RETURN_ADDRESS() \
    do {                         \
    } while (0)
#endif
...
static VMINLINE void* setCurrentExceptionFromJIT(
    J9VMThread* currentThread, UDATA exceptionNumber, j9object_t detailMessage)
{
    TIDY_BEFORE_THROW();
    currentThread->javaVM->internalVMFunctions->setCurrentException(
        currentThread, exceptionNumber, (UDATA*)detailMessage);
    return J9_JITHELPER_ACTION_THROW;
}

static VMINLINE void* setCurrentExceptionNLSFromJIT(
    J9VMThread* currentThread, UDATA exceptionNumber, U_32 moduleName, U_32 messageNumber)
{
    TIDY_BEFORE_THROW();
    currentThread->javaVM->internalVMFunctions->setCurrentExceptionNLS(
        currentThread, exceptionNumber, moduleName, messageNumber);
    return J9_JITHELPER_ACTION_THROW;
}

static VMINLINE void* setCurrentExceptionUTFFromJIT(
    J9VMThread* currentThread, UDATA exceptionNumber, const char* errorUTF)
{
    TIDY_BEFORE_THROW();
...
static VMINLINE void* setNativeOutOfMemoryErrorFromJIT(J9VMThread* currentThread, U_32 moduleName, U_32 messageNumber)
{
    TIDY_BEFORE_THROW();
...
static VMINLINE void* setHeapOutOfMemoryErrorFromJIT(J9VMThread* currentThread)
{
    TIDY_BEFORE_THROW();
...
static VMINLINE void* setClassCastExceptionFromJIT(
    J9VMThread* currentThread, J9Class* instanceClass, J9Class* castClass)
{
    TIDY_BEFORE_THROW();
...
static VMINLINE void* buildJITResolveFrame(J9VMThread* currentThread, UDATA flags, UDATA helperParmCount,
    bool checkScavengeOnResolve = true, UDATA spAdjust = 0)
{
...
static VMINLINE void* buildJITResolveFrameForRuntimeHelper(J9VMThread* currentThread, UDATA helperParmCount)
{
    return buildJITResolveFrame(currentThread, J9_SSF_JIT_RESOLVE_RUNTIME_HELPER, helperParmCount);
}

static VMINLINE void* buildJITResolveFrameForTrapHandler(J9VMThread* currentThread)
{
...
static VMINLINE void buildJITResolveFrameForRuntimeCheck(J9VMThread* currentThread)
{
    void* oldPC = currentThread->jitReturnAddress;
#if defined(J9VM_ARCH_X86)
    U_8* pc = (U_8*)oldPC;
...
static VMINLINE bool fast_jitNewObjectImpl(
    J9VMThread* currentThread, J9Class* objectClass, bool checkClassInit, bool nonZeroTLH)
{
    ...
    }
    {
        ...
        j9object_t obj = currentThread->javaVM->memoryManagerFunctions->J9AllocateObjectNoGC(
            currentThread, objectClass, allocationFlags);
...
static VMINLINE void* slow_jitNewObjectImpl(J9VMThread* currentThread, bool checkClassInit, bool nonZeroTLH)
{
    ...
    if (J9_ARE_ANY_BITS_SET(objectClass->romClass->modifiers, J9AccAbstract | J9AccInterface)) {
        ...
        addr = setCurrentExceptionFromJIT(currentThread, J9VMCONSTANTPOOL_JAVALANGINSTANTIATIONERROR | J9_EX_CTOR_CLASS,
            J9VM_J9CLASS_TO_HEAPCLASS(objectClass));
        goto done;
    }
    if (checkClassInit) {
    ...
    buildJITResolveFrameWithPC(
        currentThread, J9_STACK_FLAGS_JIT_ALLOCATION_RESOLVE | J9_SSF_JIT_RESOLVE, parmCount, true, 0, oldPC);
...
void* J9FASTCALL old_slow_jitNewObject(J9VMThread* currentThread)
{
    return slow_jitNewObjectImpl(currentThread, true, false);
}

void* J9FASTCALL old_fast_jitNewObject(J9VMThread* currentThread)
{
...
void* J9FASTCALL old_slow_jitNewObjectNoZeroInit(J9VMThread* currentThread)
{
    return slow_jitNewObjectImpl(currentThread, true, true);
}

void* J9FASTCALL old_fast_jitNewObjectNoZeroInit(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(1);
    DECLARE_JIT_CLASS_PARM(objectClass, 1);
    void* slowPath = NULL;
...
static VMINLINE bool fast_jitANewArrayImpl(J9VMThread* currentThread, J9Class* elementClass, I_32 size, bool nonZeroTLH)
{
    bool slowPathRequired = false;
    ...
    }
    {
        ...
        j9object_t obj = currentThread->javaVM->memoryManagerFunctions->J9AllocateIndexableObjectNoGC(
            currentThread, arrayClass, (U_32)size, allocationFlags);
        if (J9_UNEXPECTED(NULL == obj)) {
            goto slow;
        }
        JIT_RETURN_UDATA(obj);
...
static VMINLINE void* slow_jitANewArrayImpl(J9VMThread* currentThread, bool nonZeroTLH)
{
    SLOW_JIT_HELPER_PROLOGUE();
    ...
    if (NULL == arrayClass) {
        ...
        arrayClass = vm->internalVMFunctions->internalCreateArrayClass(
            currentThread, (J9ROMArrayClass*)J9ROMIMAGEHEADER_FIRSTCLASS(vm->arrayROMClasses), elementClass);
        addr = restoreJITResolveFrame(currentThread, oldPC);
        if (NULL != addr) {
            goto done;
        }
    ...
    buildJITResolveFrameWithPC(
        currentThread, J9_STACK_FLAGS_JIT_ALLOCATION_RESOLVE | J9_SSF_JIT_RESOLVE, parmCount, true, 0, oldPC);
    obj = currentThread->javaVM->memoryManagerFunctions->J9AllocateIndexableObject(
        currentThread, arrayClass, (U_32)size, allocationFlags);
    if (NULL == obj) {
        addr = setHeapOutOfMemoryErrorFromJIT(currentThread);
        goto done;
...
void* J9FASTCALL old_slow_jitANewArray(J9VMThread* currentThread)
{
    return slow_jitANewArrayImpl(currentThread, false);
}

void* J9FASTCALL old_fast_jitANewArray(J9VMThread* currentThread)
{
...
void* J9FASTCALL old_slow_jitANewArrayNoZeroInit(J9VMThread* currentThread)
{
    return slow_jitANewArrayImpl(currentThread, true);
}

void* J9FASTCALL old_fast_jitANewArrayNoZeroInit(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(2);
    DECLARE_JIT_CLASS_PARM(elementClass, 1);
    DECLARE_JIT_INT_PARM(size, 2);
...
static VMINLINE bool fast_jitNewArrayImpl(J9VMThread* currentThread, I_32 arrayType, I_32 size, bool nonZeroTLH)
{
    ...
    if (size >= 0) {
        ...
        j9object_t obj = vm->memoryManagerFunctions->J9AllocateIndexableObjectNoGC(
            currentThread, arrayClass, (U_32)size, allocationFlags);
        if (NULL != obj) {
...
static VMINLINE void* slow_jitNewArrayImpl(J9VMThread* currentThread, bool nonZeroTLH)
{
    SLOW_JIT_HELPER_PROLOGUE();
...
void* J9FASTCALL old_slow_jitNewArray(J9VMThread* currentThread) { return slow_jitNewArrayImpl(currentThread, false); }

void* J9FASTCALL old_fast_jitNewArray(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(2);
...
void* J9FASTCALL old_slow_jitNewArrayNoZeroInit(J9VMThread* currentThread)
{
    return slow_jitNewArrayImpl(currentThread, true);
}

void* J9FASTCALL old_fast_jitNewArrayNoZeroInit(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(2);
    DECLARE_JIT_INT_PARM(arrayType, 1);
    DECLARE_JIT_INT_PARM(size, 2);
...
void* J9FASTCALL old_slow_jitAMultiNewArray(J9VMThread* currentThread)
{
    ...
    buildJITResolveFrameWithPC(
        currentThread, J9_STACK_FLAGS_JIT_ALLOCATION_RESOLVE | J9_SSF_JIT_RESOLVE, parmCount, true, 0, oldPC);
    obj = currentThread->javaVM->internalVMFunctions->helperMultiANewArray(currentThread, (J9ArrayClass*)arrayClass,
        (UDATA)dimensions, dimensionsArray, J9_GC_ALLOCATE_OBJECT_INSTRUMENTABLE);
    currentThread->floatTemp1 = (void*)obj; // in case of decompile
    addr = restoreJITResolveFrame(currentThread, oldPC);
    if (NULL != addr) {
        goto done;
...
void* J9FASTCALL old_slow_jitStackOverflow(J9VMThread* currentThread)
{
...
void* J9FASTCALL old_slow_jitResolveString(J9VMThread* currentThread)
{
    OLD_SLOW_ONLY_JIT_HELPER_PROLOGUE(3);
    ...
    if (NULL == *stringObjectPtr) {
        ...
        currentThread->javaVM->internalVMFunctions->resolveStringRef(
            currentThread, ramConstantPool, cpIndex, J9_RESOLVE_FLAG_RUNTIME_RESOLVE);
...
void J9FASTCALL fast_jitAcquireVMAccess(J9VMThread* currentThread)
{
    JIT_HELPER_PROLOGUE();
...
void J9FASTCALL fast_jitReleaseVMAccess(J9VMThread* currentThread)
{
    JIT_HELPER_PROLOGUE();
#if defined(J9VM_INTERP_ATOMIC_FREE_JNI)
...
void* J9FASTCALL old_slow_jitCheckAsyncMessages(J9VMThread* currentThread)
{
    OLD_SLOW_ONLY_JIT_HELPER_PROLOGUE(0);
    void* addr = NULL;
    if (VM_VMHelpers::asyncMessagePending(currentThread)) {
...
void* J9FASTCALL impl_jitClassCastException(J9VMThread* currentThread)
{
    SLOW_JIT_HELPER_PROLOGUE();
...
void* J9FASTCALL old_slow_jitCheckCast(J9VMThread* currentThread)
{
    SLOW_JIT_HELPER_PROLOGUE();
    J9Class* castClass = (J9Class*)currentThread->floatTemp1;
...
void* J9FASTCALL old_fast_jitCheckCast(J9VMThread* currentThread)
{
    void* slowPath = NULL;
    OLD_JIT_HELPER_PROLOGUE(2);
...
void* J9FASTCALL old_slow_jitCheckCastForArrayStore(J9VMThread* currentThread)
{
    SLOW_JIT_HELPER_PROLOGUE();
    buildJITResolveFrameForRuntimeHelper(currentThread, parmCount);
...
void* J9FASTCALL old_fast_jitCheckCastForArrayStore(J9VMThread* currentThread)
{
    void* slowPath = NULL;
    OLD_JIT_HELPER_PROLOGUE(2);
    DECLARE_JIT_CLASS_PARM(castClass, 1);
...
void J9FASTCALL old_fast_jitCheckIfFinalizeObject(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(1);
...
void J9FASTCALL old_fast_jitCollapseJNIReferenceFrame(J9VMThread* currentThread)
{
    JIT_HELPER_PROLOGUE();
...
void* J9FASTCALL old_slow_jitHandleArrayIndexOutOfBoundsTrap(J9VMThread* currentThread)
{
    JIT_HELPER_PROLOGUE();
...
void* J9FASTCALL old_slow_jitHandleIntegerDivideByZeroTrap(J9VMThread* currentThread)
{
    JIT_HELPER_PROLOGUE();
    buildJITResolveFrameForTrapHandler(currentThread);
    return setCurrentExceptionNLSFromJIT(
        currentThread, J9VMCONSTANTPOOL_JAVALANGARITHMETICEXCEPTION, J9NLS_VM_DIVIDE_BY_ZERO);
}

void* J9FASTCALL old_slow_jitHandleNullPointerExceptionTrap(J9VMThread* currentThread)
{
    JIT_HELPER_PROLOGUE();
    buildJITResolveFrameForTrapHandler(currentThread);
...
void* J9FASTCALL old_slow_jitHandleInternalErrorTrap(J9VMThread* currentThread)
{
    JIT_HELPER_PROLOGUE();
    buildJITResolveFrameForTrapHandler(currentThread);
...
void J9FASTCALL old_fast_jitInstanceOf(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(2);
    DECLARE_JIT_CLASS_PARM(castClass, 1);
    DECLARE_JIT_PARM(j9object_t, object, 2);
...
void* J9FASTCALL old_slow_jitLookupInterfaceMethod(J9VMThread* currentThread)
{
    SLOW_JIT_HELPER_PROLOGUE();
    ...
    if (0 == vTableOffset) {
        ...
        TIDY_BEFORE_THROW();
        currentThread->javaVM->internalVMFunctions->setIllegalAccessErrorNonPublicInvokeInterface(
            currentThread, method);
    }
    return J9_JITHELPER_ACTION_THROW;
}

void* J9FASTCALL old_fast_jitLookupInterfaceMethod(J9VMThread* currentThread)
{
...
void J9FASTCALL old_fast_jitMethodIsNative(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(1);
...
void J9FASTCALL old_fast_jitMethodIsSync(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(1);
    DECLARE_JIT_PARM(J9Method*, method, 1);
    UDATA isSync = 0;
...
static VMINLINE bool fast_jitMonitorEnterImpl(J9VMThread* currentThread, j9object_t syncObject, bool forMethod)
{
    bool slowPathRequired = false;
    IDATA monstatus
        = currentThread->javaVM->internalVMFunctions->objectMonitorEnterNonBlocking(currentThread, syncObject);
    switch (monstatus) {
    case 0:
    case 1:
        slowPathRequired = true;
...
static VMINLINE void* slow_jitMonitorEnterImpl(J9VMThread* currentThread, bool forMethod)
{
    SLOW_JIT_HELPER_PROLOGUE();
    void* addr = NULL;
    UDATA flags = J9_STACK_FLAGS_JIT_RESOLVE_FRAME
        | (forMethod ? J9_STACK_FLAGS_JIT_METHOD_MONITOR_ENTER_RESOLVE : J9_STACK_FLAGS_JIT_MONITOR_ENTER_RESOLVE);
    ...
    if (0 == monstatus) {
        if (forMethod) {
            ...
            if ((NULL == getJitInlinedCallInfo(metaData)) || (NULL == getFirstInlinedCallSite(metaData, inlineMap))) {
                J9SFJITResolveFrame* resolveFrame = (J9SFJITResolveFrame*)currentThread->sp;
                resolveFrame->specialFrameFlags
                    = (resolveFrame->specialFrameFlags & ~J9_STACK_FLAGS_JIT_FRAME_SUB_TYPE_MASK)
                    | J9_STACK_FLAGS_JIT_FAILED_METHOD_MONITOR_ENTER_RESOLVE;
            }
        }
...
void* J9FASTCALL old_slow_jitMethodMonitorEntry(J9VMThread* currentThread)
{
    return slow_jitMonitorEnterImpl(currentThread, true);
}

void* J9FASTCALL old_fast_jitMethodMonitorEntry(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(1);
...
void* J9FASTCALL old_slow_jitMonitorEntry(J9VMThread* currentThread)
{
    return slow_jitMonitorEnterImpl(currentThread, false);
}

void* J9FASTCALL old_fast_jitMonitorEntry(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(1);
    DECLARE_JIT_PARM(j9object_t, syncObject, 1);
    void* slowPath = NULL;
...
static VMINLINE bool fast_jitMonitorExitImpl(J9VMThread* currentThread, j9object_t syncObject, bool forMethod)
{
    bool slowPathRequired = true;
...
static VMINLINE void* slow_jitMonitorExitImpl(J9VMThread* currentThread, bool forMethod)
{
    SLOW_JIT_HELPER_PROLOGUE();
...
void* J9FASTCALL old_slow_jitMethodMonitorExit(J9VMThread* currentThread)
{
    return slow_jitMonitorExitImpl(currentThread, true);
}

void* J9FASTCALL old_slow_jitThrowIncompatibleReceiver(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(2);
    DECLARE_JIT_CLASS_PARM(receiverClass, 1);
    ...
    buildJITResolveFrameForRuntimeHelper(currentThread, parmCount);
    TIDY_BEFORE_THROW();
    currentThread->javaVM->internalVMFunctions->setIllegalAccessErrorReceiverNotSameOrSubtypeOfCurrentClass(
        currentThread, receiverClass, currentClass);
    return J9_JITHELPER_ACTION_THROW;
}

void* J9FASTCALL old_fast_jitMethodMonitorExit(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(1);
    DECLARE_JIT_PARM(j9object_t, syncObject, 1);
    void* slowPath = NULL;
...
void* J9FASTCALL old_slow_jitMonitorExit(J9VMThread* currentThread)
{
    return slow_jitMonitorExitImpl(currentThread, false);
}

void* J9FASTCALL old_fast_jitMonitorExit(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(1);
    DECLARE_JIT_PARM(j9object_t, syncObject, 1);
    void* slowPath = NULL;
...
static VMINLINE void* jitReportMethodEnterImpl(
    J9VMThread* currentThread, J9Method* method, j9object_t receiver, bool isStatic)
{
    SLOW_JIT_HELPER_PROLOGUE();
    J9JavaVM* vm = currentThread->javaVM;
    void* addr = NULL;
    ...
    if (hooked || traced) {
        ...
        if (!isStatic) {
            /* The current value of the jitException slot is preserved in the resolve frame, so it can be safely
             * overwritten */
...
void* J9FASTCALL old_slow_jitReportMethodEnter(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(2);
    DECLARE_JIT_PARM(J9Method*, method, 1);
...
void* J9FASTCALL old_slow_jitReportStaticMethodEnter(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(1);
    DECLARE_JIT_PARM(J9Method*, method, 1);
...
void* J9FASTCALL old_slow_jitReportMethodExit(J9VMThread* currentThread)
{
    ...
    if (hooked || traced) {
        ...
        if (hooked) {
            ALWAYS_TRIGGER_J9HOOK_VM_METHOD_RETURN(
                vm->hookInterface, currentThread, method, FALSE, returnValueAddress, 1);
        }
        addr = restoreJITResolveFrame(currentThread, oldPC, true, false);
    }
    SLOW_JIT_HELPER_EPILOGUE();
...
void* J9FASTCALL old_slow_jitResolveClass(J9VMThread* currentThread)
{
    OLD_SLOW_ONLY_JIT_HELPER_PROLOGUE(3);
    DECLARE_JIT_PARM(J9ConstantPool*, ramConstantPool, 1);
    DECLARE_JIT_INT_PARM(cpIndex, 2);
    ...
    if (NULL == clazz) {
        buildJITResolveFrameWithPC(currentThread, J9_SSF_JIT_RESOLVE_DATA, parmCount, true, 0, jitEIP);
        clazz = currentThread->javaVM->internalVMFunctions->resolveClassRef(
            currentThread, ramConstantPool, cpIndex, J9_RESOLVE_FLAG_RUNTIME_RESOLVE);
        addr = restoreJITResolveFrame(currentThread, jitEIP);
        if (NULL != addr) {
            goto done;
...
void* J9FASTCALL old_slow_jitResolveClassFromStaticField(J9VMThread* currentThread)
{
    OLD_SLOW_ONLY_JIT_HELPER_PROLOGUE(3);
    DECLARE_JIT_PARM(J9ConstantPool*, ramConstantPool, 1);
    DECLARE_JIT_INT_PARM(cpIndex, 2);
    ...
    if (J9_UNEXPECTED(!VM_VMHelpers::staticFieldRefIsResolved(flagsAndClass, valueOffset))) {
        ...
        currentThread->javaVM->internalVMFunctions->resolveStaticFieldRefInto(
            currentThread, NULL, ramConstantPool, cpIndex, J9_RESOLVE_FLAG_RUNTIME_RESOLVE, NULL, &localRef);
        addr = restoreJITResolveFrame(currentThread, jitEIP);
        if (NULL != addr) {
            goto done;
        }
...
void J9FASTCALL old_fast_jitResolvedFieldIsVolatile(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(3);
    DECLARE_JIT_PARM(J9ConstantPool*, ramConstantPool, 1);
    ...
    UDATA isVolatile = 0;
    if (isStatic) {
        if (J9_ARE_ANY_BITS_SET(((J9RAMStaticFieldRef*)ramConstantPool)[cpIndex].flagsAndClass,
                STATIC_FIELD_REF_BIT(J9StaticFieldRefVolatile))) {
            isVolatile = 1;
        }
    } else {
...
static VMINLINE void* old_slow_jitResolveFieldImpl(J9VMThread* currentThread, UDATA parmCount,
    J9ConstantPool* ramConstantPool, UDATA cpIndex, void* jitEIP, bool isSetter, bool direct)
{
    J9Method* method = NULL;
    UDATA resolveFlags = J9_RESOLVE_FLAG_RUNTIME_RESOLVE;
    J9JavaVM* vm = currentThread->javaVM;
    buildJITResolveFrameWithPC(
        currentThread, direct ? J9_SSF_JIT_RESOLVE : J9_SSF_JIT_RESOLVE_DATA, parmCount, true, 0, jitEIP);
    if (isSetter) {
    ...
    UDATA valueOffset = vm->internalVMFunctions->resolveInstanceFieldRef(
        currentThread, method, ramConstantPool, cpIndex, resolveFlags, NULL);
...
void* J9FASTCALL old_slow_jitResolveField(J9VMThread* currentThread)
{
    OLD_SLOW_ONLY_JIT_HELPER_PROLOGUE(3);
    DECLARE_JIT_PARM(J9ConstantPool*, ramConstantPool, 1);
    DECLARE_JIT_INT_PARM(cpIndex, 2);
...
void* J9FASTCALL old_slow_jitResolveFieldSetter(J9VMThread* currentThread)
{
    OLD_SLOW_ONLY_JIT_HELPER_PROLOGUE(3);
    DECLARE_JIT_PARM(J9ConstantPool*, ramConstantPool, 1);
    DECLARE_JIT_INT_PARM(cpIndex, 2);
...
void* J9FASTCALL old_slow_jitResolveFieldDirect(J9VMThread* currentThread)
{
    OLD_SLOW_ONLY_JIT_HELPER_PROLOGUE(2);
    DECLARE_JIT_PARM(J9ConstantPool*, ramConstantPool, 1);
    DECLARE_JIT_INT_PARM(cpIndex, 2);
...
void* J9FASTCALL old_slow_jitResolveFieldSetterDirect(J9VMThread* currentThread)
{
    OLD_SLOW_ONLY_JIT_HELPER_PROLOGUE(2);
    DECLARE_JIT_PARM(J9ConstantPool*, ramConstantPool, 1);
    DECLARE_JIT_INT_PARM(cpIndex, 2);
...
static VMINLINE void* old_slow_jitResolveStaticFieldImpl(J9VMThread* currentThread, UDATA parmCount,
    J9ConstantPool* ramConstantPool, UDATA cpIndex, void* jitEIP, bool isSetter, bool direct)
{
    J9JavaVM* vm = currentThread->javaVM;
    J9Method* method = NULL;
    ...
    UDATA field = (UDATA)vm->internalVMFunctions->resolveStaticFieldRef(
        currentThread, method, ramConstantPool, cpIndex, resolveFlags, NULL);
    if ((UDATA)-1 == field) {
...
void* J9FASTCALL old_slow_jitResolveStaticField(J9VMThread* currentThread)
{
    OLD_SLOW_ONLY_JIT_HELPER_PROLOGUE(3);
    DECLARE_JIT_PARM(J9ConstantPool*, ramConstantPool, 1);
    DECLARE_JIT_INT_PARM(cpIndex, 2);
    DECLARE_JIT_PARM(void*, jitEIP, 3);
    void* addr
        = old_slow_jitResolveStaticFieldImpl(currentThread, parmCount, ramConstantPool, cpIndex, jitEIP, false, false);
    SLOW_JIT_HELPER_EPILOGUE();
    return addr;
}

void* J9FASTCALL old_slow_jitResolveStaticFieldSetter(J9VMThread* currentThread)
{
    OLD_SLOW_ONLY_JIT_HELPER_PROLOGUE(3);
    DECLARE_JIT_PARM(J9ConstantPool*, ramConstantPool, 1);
    DECLARE_JIT_INT_PARM(cpIndex, 2);
    DECLARE_JIT_PARM(void*, jitEIP, 3);
    void* addr
        = old_slow_jitResolveStaticFieldImpl(currentThread, parmCount, ramConstantPool, cpIndex, jitEIP, true, false);
    SLOW_JIT_HELPER_EPILOGUE();
    return addr;
}

void* J9FASTCALL old_slow_jitResolveStaticFieldDirect(J9VMThread* currentThread)
{
    OLD_SLOW_ONLY_JIT_HELPER_PROLOGUE(2);
    DECLARE_JIT_PARM(J9ConstantPool*, ramConstantPool, 1);
    DECLARE_JIT_INT_PARM(cpIndex, 2);
    void* jitEIP = currentThread->jitReturnAddress;
    void* addr
        = old_slow_jitResolveStaticFieldImpl(currentThread, parmCount, ramConstantPool, cpIndex, jitEIP, false, true);
    SLOW_JIT_HELPER_EPILOGUE();
    return addr;
}

void* J9FASTCALL old_slow_jitResolveStaticFieldSetterDirect(J9VMThread* currentThread)
{
    OLD_SLOW_ONLY_JIT_HELPER_PROLOGUE(2);
    DECLARE_JIT_PARM(J9ConstantPool*, ramConstantPool, 1);
    DECLARE_JIT_INT_PARM(cpIndex, 2);
    void* jitEIP = currentThread->jitReturnAddress;
    void* addr
        = old_slow_jitResolveStaticFieldImpl(currentThread, parmCount, ramConstantPool, cpIndex, jitEIP, true, true);
    SLOW_JIT_HELPER_EPILOGUE();
    return addr;
}

void* J9FASTCALL old_slow_jitResolveInterfaceMethod(J9VMThread* currentThread)
{
    OLD_SLOW_ONLY_JIT_HELPER_PROLOGUE(2);
...
retry:
    ...
    if (!J9RAMINTERFACEMETHODREF_RESOLVED(interfaceClass, methodIndexAndArgCount)) {
        ...
        currentThread->javaVM->internalVMFunctions->resolveInterfaceMethodRef(
            currentThread, ramConstantPool, cpIndex, J9_RESOLVE_FLAG_RUNTIME_RESOLVE);
        addr = restoreJITResolveFrame(currentThread, jitEIP);
        if (NULL != addr) {
            goto done;
...
void* J9FASTCALL old_slow_jitResolveSpecialMethod(J9VMThread* currentThread)
{
    OLD_SLOW_ONLY_JIT_HELPER_PROLOGUE(3);
    ...
    /* If cpIndex has the J9_SPECIAL_SPLIT_TABLE_INDEX_FLAG bit set, it is an index into split table, otherwise it is an
     * index in constant pool */
    if (J9_ARE_ANY_BITS_SET(cpIndex, J9_SPECIAL_SPLIT_TABLE_INDEX_FLAG)) {
        method = currentThread->javaVM->internalVMFunctions->resolveSpecialSplitMethodRef(
            currentThread, ramConstantPool, cpIndex & J9_SPLIT_TABLE_INDEX_MASK, J9_RESOLVE_FLAG_RUNTIME_RESOLVE);
    } else {
        method = currentThread->javaVM->internalVMFunctions->resolveSpecialMethodRef(
            currentThread, ramConstantPool, cpIndex, J9_RESOLVE_FLAG_RUNTIME_RESOLVE);
    }
    void* addr = restoreJITResolveFrame(currentThread, jitEIP);
    if (NULL == addr) {
...
void* J9FASTCALL old_slow_jitResolveStaticMethod(J9VMThread* currentThread)
{
    OLD_SLOW_ONLY_JIT_HELPER_PROLOGUE(3);
    DECLARE_JIT_PARM(void*, jitEIP, 1);
    DECLARE_JIT_PARM(J9ConstantPool*, ramConstantPool, 2);
    ...
    /* If cpIndex has the J9_STATIC_SPLIT_TABLE_INDEX_FLAG bit set, it is an index into split table, otherwise it is an
     * index in constant pool */
    if (J9_ARE_ANY_BITS_SET(cpIndex, J9_STATIC_SPLIT_TABLE_INDEX_FLAG)) {
        method = (UDATA)currentThread->javaVM->internalVMFunctions->resolveStaticSplitMethodRef(currentThread,
            ramConstantPool, cpIndex & J9_SPLIT_TABLE_INDEX_MASK,
            J9_RESOLVE_FLAG_RUNTIME_RESOLVE | J9_RESOLVE_FLAG_CHECK_CLINIT);
    } else {
        method = (UDATA)currentThread->javaVM->internalVMFunctions->resolveStaticMethodRef(
            currentThread, ramConstantPool, cpIndex, J9_RESOLVE_FLAG_RUNTIME_RESOLVE | J9_RESOLVE_FLAG_CHECK_CLINIT);
    }
    if ((UDATA)-1 == method) {
        /* fetch result from floatTemp - stashed there in clinit case */
...
void* J9FASTCALL old_slow_jitResolveVirtualMethod(J9VMThread* currentThread)
{
    OLD_SLOW_ONLY_JIT_HELPER_PROLOGUE(2);
    DECLARE_JIT_PARM(UDATA*, indexAndLiteralsEA, 1);
    DECLARE_JIT_PARM(void*, jitEIP, 2);
...
retry:
    ...
    if (J9VTABLE_INITIAL_VIRTUAL_OFFSET == vTableOffset) {
        ...
        currentThread->javaVM->internalVMFunctions->resolveVirtualMethodRef(
            currentThread, ramConstantPool, cpIndex, J9_RESOLVE_FLAG_RUNTIME_RESOLVE, NULL);
        addr = restoreJITResolveFrame(currentThread, jitEIP);
        if (NULL != addr) {
            goto done;
        }
...
void* J9FASTCALL old_slow_jitResolveMethodType(J9VMThread* currentThread)
{
    OLD_SLOW_ONLY_JIT_HELPER_PROLOGUE(3);
    DECLARE_JIT_PARM(J9ConstantPool*, ramConstantPool, 1);
    DECLARE_JIT_INT_PARM(cpIndex, 2);
...
retry:
    ...
    if (NULL == *methodTypePtr) {
        buildJITResolveFrameWithPC(currentThread, J9_SSF_JIT_RESOLVE_DATA, parmCount, true, 0, jitEIP);
        currentThread->javaVM->internalVMFunctions->resolveMethodTypeRef(
            currentThread, ramConstantPool, cpIndex, J9_RESOLVE_FLAG_RUNTIME_RESOLVE);
        addr = restoreJITResolveFrame(currentThread, jitEIP);
        if (NULL != addr) {
            goto done;
...
void* J9FASTCALL old_slow_jitResolveMethodHandle(J9VMThread* currentThread)
{
    OLD_SLOW_ONLY_JIT_HELPER_PROLOGUE(3);
    DECLARE_JIT_PARM(J9ConstantPool*, ramConstantPool, 1);
    DECLARE_JIT_INT_PARM(cpIndex, 2);
...
retry:
    ...
    if (NULL == *methodHandlePtr) {
        buildJITResolveFrameWithPC(currentThread, J9_SSF_JIT_RESOLVE_DATA, parmCount, true, 0, jitEIP);
        currentThread->javaVM->internalVMFunctions->resolveMethodHandleRef(
            currentThread, ramConstantPool, cpIndex, J9_RESOLVE_FLAG_RUNTIME_RESOLVE);
        addr = restoreJITResolveFrame(currentThread, jitEIP);
        if (NULL != addr) {
            goto done;
...
void* J9FASTCALL old_slow_jitResolveInvokeDynamic(J9VMThread* currentThread)
{
    OLD_SLOW_ONLY_JIT_HELPER_PROLOGUE(3);
    DECLARE_JIT_PARM(J9ConstantPool*, ramConstantPool, 1);
...
retry:
    ...
    if (NULL == *callsitePtr) {
        buildJITResolveFrameWithPC(currentThread, J9_SSF_JIT_RESOLVE_DATA, parmCount, true, 0, jitEIP);
        currentThread->javaVM->internalVMFunctions->resolveInvokeDynamic(
            currentThread, ramConstantPool, index, J9_RESOLVE_FLAG_RUNTIME_RESOLVE);
        addr = restoreJITResolveFrame(currentThread, jitEIP);
        if (NULL != addr) {
            goto done;
        }
...
void* J9FASTCALL old_slow_jitResolveConstantDynamic(J9VMThread* currentThread)
{
    OLD_SLOW_ONLY_JIT_HELPER_PROLOGUE(3);
    DECLARE_JIT_PARM(J9ConstantPool*, ramConstantPool, 1);
    DECLARE_JIT_INT_PARM(cpIndex, 2);
...
retry:
    ...
    if ((NULL == *constantDynamicValue) && (ramCPEntry->exception != vm->voidReflectClass->classObject)) {
        ...
        buildJITResolveFrameWithPC(currentThread, J9_SSF_JIT_RESOLVE_DATA, parmCount, true, 0, jitEIP);
        vm->internalVMFunctions->resolveConstantDynamic(
            currentThread, ramConstantPool, cpIndex, J9_RESOLVE_FLAG_RUNTIME_RESOLVE);
        addr = restoreJITResolveFrame(currentThread, jitEIP);
        if (NULL != addr) {
            goto done;
...
void* J9FASTCALL old_slow_jitResolveHandleMethod(J9VMThread* currentThread)
{
    OLD_SLOW_ONLY_JIT_HELPER_PROLOGUE(3);
    DECLARE_JIT_PARM(J9ConstantPool*, ramConstantPool, 1);
    DECLARE_JIT_INT_PARM(cpIndex, 2);
    ...
    if (NULL == targetClass) {
        buildJITResolveFrameWithPC(currentThread, J9_SSF_JIT_RESOLVE_DATA, parmCount, true, 0, jitEIP);
        targetClass = vm->internalVMFunctions->resolveClassRef(
            currentThread, ramConstantPool, classRefCPIndex, J9_RESOLVE_FLAG_RUNTIME_RESOLVE);
        addr = restoreJITResolveFrame(currentThread, jitEIP);
        if (NULL != addr) {
            goto done;
        }
...
retry:
    ...
    if (NULL == *methodTypePtr) {
        ...
        buildJITResolveFrameWithPC(currentThread, J9_SSF_JIT_RESOLVE_DATA, parmCount, true, 0, jitEIP);
        vm->internalVMFunctions->resolveVirtualMethodRef(
            currentThread, ramConstantPool, cpIndex, J9_RESOLVE_FLAG_RUNTIME_RESOLVE, NULL);
        addr = restoreJITResolveFrame(currentThread, jitEIP);
        if (NULL != addr) {
            goto done;
...
void* J9FASTCALL old_slow_jitRetranslateCaller(J9VMThread* currentThread)
{
    OLD_SLOW_ONLY_JIT_HELPER_PROLOGUE(2);
    DECLARE_JIT_PARM(J9Method*, method, 1);
...
void* J9FASTCALL old_slow_jitRetranslateCallerWithPreparation(J9VMThread* currentThread)
{
    OLD_SLOW_ONLY_JIT_HELPER_PROLOGUE(3);
    DECLARE_JIT_PARM(J9Method*, method, 1);
    DECLARE_JIT_PARM(void*, oldJITStartAddr, 2);
...
void* J9FASTCALL old_slow_jitRetranslateMethod(J9VMThread* currentThread)
{
    OLD_SLOW_ONLY_JIT_HELPER_PROLOGUE(3);
    DECLARE_JIT_PARM(J9Method*, method, 1);
    DECLARE_JIT_PARM(void*, oldJITStartAddr, 2);
    ...
    if (callerAlreadyBeingDecompiled) {
         ...
         *	- decomp record is updated such that the savedPCAddres points to where the jitted method would save the
         *RA
         ...
         * record to point to the returnAddress field of the recently pushed resolve frame to keep the stack walker
         *happy.
         */
        J9SFJITResolveFrame* resolveFrame = (J9SFJITResolveFrame*)currentThread->sp;
...
void* J9FASTCALL old_slow_jitThrowCurrentException(J9VMThread* currentThread)
{
...
void* J9FASTCALL old_slow_jitThrowException(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(1);
...
void* J9FASTCALL old_slow_jitThrowAbstractMethodError(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(0);
    buildJITResolveFrameForRuntimeCheck(currentThread);
...
void* J9FASTCALL old_slow_jitThrowArithmeticException(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(0);
    buildJITResolveFrameForRuntimeCheck(currentThread);
    return setCurrentExceptionNLSFromJIT(
...
void* J9FASTCALL old_slow_jitThrowArrayIndexOutOfBounds(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(0);
    buildJITResolveFrameForRuntimeCheck(currentThread);
    return setCurrentExceptionFromJIT(currentThread, J9VMCONSTANTPOOL_JAVALANGARRAYINDEXOUTOFBOUNDSEXCEPTION, NULL);
...
void* J9FASTCALL impl_jitReferenceArrayCopy(J9VMThread* currentThread, UDATA lengthInBytes)
{
    JIT_HELPER_PROLOGUE();
    void* exception = NULL;
    if (-1
        != currentThread->javaVM->memoryManagerFunctions->referenceArrayCopy(currentThread,
               (J9IndexableObject*)currentThread->floatTemp1, (J9IndexableObject*)currentThread->floatTemp2,
               (fj9object_t*)currentThread->floatTemp3, (fj9object_t*)currentThread->floatTemp4,
...
void* J9FASTCALL old_slow_jitThrowArrayStoreException(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(0);
    buildJITResolveFrameForRuntimeCheck(currentThread);
    return setCurrentExceptionFromJIT(currentThread, J9VMCONSTANTPOOL_JAVALANGARRAYSTOREEXCEPTION, NULL);
...
void* J9FASTCALL old_slow_jitThrowArrayStoreExceptionWithIP(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(1);
    DECLARE_JIT_PARM(void*, jitEIP, 1);
...
void* J9FASTCALL old_slow_jitThrowExceptionInInitializerError(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(0);
    buildJITResolveFrameForRuntimeCheck(currentThread);
...
void* J9FASTCALL old_slow_jitThrowIllegalAccessError(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(0);
    buildJITResolveFrameForRuntimeCheck(currentThread);
...
void* J9FASTCALL old_slow_jitThrowIncompatibleClassChangeError(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(0);
    buildJITResolveFrameForRuntimeCheck(currentThread);
...
void* J9FASTCALL old_slow_jitThrowInstantiationException(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(0);
    buildJITResolveFrameForRuntimeCheck(currentThread);
...
void* J9FASTCALL old_slow_jitThrowNullPointerException(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(0);
    buildJITResolveFrameForRuntimeCheck(currentThread);
    return setCurrentExceptionFromJIT(currentThread, J9VMCONSTANTPOOL_JAVALANGNULLPOINTEREXCEPTION, NULL);
...
void* J9FASTCALL old_slow_jitThrowWrongMethodTypeException(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(0);
    buildJITResolveFrameForRuntimeCheck(currentThread);
...
void* J9FASTCALL old_slow_jitTypeCheckArrayStoreWithNullCheck(J9VMThread* currentThread)
{
    SLOW_JIT_HELPER_PROLOGUE();
    buildJITResolveFrameForRuntimeHelper(currentThread, parmCount);
    return setCurrentExceptionFromJIT(currentThread, J9VMCONSTANTPOOL_JAVALANGARRAYSTOREEXCEPTION, NULL);
...
static VMINLINE bool fast_jitTypeCheckArrayStoreImpl(
    J9VMThread* currentThread, j9object_t destinationObject, j9object_t objectBeingStored)
{
    bool slowPathRequired = false;
...
void* J9FASTCALL old_fast_jitTypeCheckArrayStoreWithNullCheck(J9VMThread* currentThread)
{
    void* slowPath = NULL;
    OLD_JIT_HELPER_PROLOGUE(2);
...
void* J9FASTCALL old_fast_jitTypeCheckArrayStore(J9VMThread* currentThread)
{
    /* Not omitting NULL check */
    return old_fast_jitTypeCheckArrayStoreWithNullCheck(currentThread);
}

void* J9FASTCALL old_slow_jitTypeCheckArrayStore(J9VMThread* currentThread)
{
    return old_slow_jitTypeCheckArrayStoreWithNullCheck(currentThread);
}

void J9FASTCALL fast_jitWriteBarrierBatchStore(J9VMThread* currentThread, j9object_t destinationObject)
{
    currentThread->javaVM->memoryManagerFunctions->J9WriteBarrierBatchStore(currentThread, destinationObject);
}

void J9FASTCALL old_fast_jitWriteBarrierBatchStore(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(1);
    DECLARE_JIT_PARM(j9object_t, destinationObject, 1);
...
void J9FASTCALL fast_jitWriteBarrierBatchStoreWithRange(
    J9VMThread* currentThread, j9object_t destinationObject, UDATA destinationPtr, UDATA range)
{
    currentThread->javaVM->memoryManagerFunctions->J9WriteBarrierBatchStore(currentThread, destinationObject);
}

void J9FASTCALL old_fast_jitWriteBarrierBatchStoreWithRange(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(3);
    DECLARE_JIT_PARM(j9object_t, destinationObject, 1);
...
void J9FASTCALL fast_jitWriteBarrierJ9ClassBatchStore(J9VMThread* currentThread, J9Class* destinationClass)
{
    currentThread->javaVM->memoryManagerFunctions->J9WriteBarrierJ9ClassBatchStore(currentThread, destinationClass);
}

void J9FASTCALL old_fast_jitWriteBarrierJ9ClassBatchStore(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(1);
...
void J9FASTCALL fast_jitWriteBarrierJ9ClassStore(
    J9VMThread* currentThread, J9Class* destinationClass, j9object_t objectBeingStored)
{
    currentThread->javaVM->memoryManagerFunctions->J9WriteBarrierJ9ClassStore(
        currentThread, destinationClass, objectBeingStored);
}

void J9FASTCALL old_fast_jitWriteBarrierJ9ClassStore(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(2);
    DECLARE_JIT_CLASS_PARM(destinationClass, 1);
    DECLARE_JIT_PARM(j9object_t, objectBeingStored, 2);
...
void J9FASTCALL fast_jitWriteBarrierStore(
    J9VMThread* currentThread, j9object_t destinationObject, j9object_t objectBeingStored)
{
    currentThread->javaVM->memoryManagerFunctions->J9WriteBarrierStore(
        currentThread, destinationObject, objectBeingStored);
}

void J9FASTCALL old_fast_jitWriteBarrierStore(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(2);
    DECLARE_JIT_PARM(j9object_t, destinationObject, 1);
    DECLARE_JIT_PARM(j9object_t, objectBeingStored, 2);
...
void J9FASTCALL fast_jitWriteBarrierStoreGenerational(
    J9VMThread* currentThread, j9object_t destinationObject, j9object_t objectBeingStored)
{
    MM_ObjectAccessBarrierAPI::internalPostObjectStoreGenerationalNoValueCheck(currentThread, destinationObject);
}

void J9FASTCALL old_fast_jitWriteBarrierStoreGenerational(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(2);
    DECLARE_JIT_PARM(j9object_t, destinationObject, 1);
    DECLARE_JIT_PARM(j9object_t, objectBeingStored, 2);
...
void J9FASTCALL fast_jitWriteBarrierStoreGenerationalAndConcurrentMark(
    J9VMThread* currentThread, j9object_t destinationObject, j9object_t objectBeingStored)
{
    MM_ObjectAccessBarrierAPI::internalPostObjectStoreCardTableAndGenerational(
        currentThread, destinationObject, objectBeingStored);
}

void J9FASTCALL old_fast_jitWriteBarrierStoreGenerationalAndConcurrentMark(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(2);
    DECLARE_JIT_PARM(j9object_t, destinationObject, 1);
    DECLARE_JIT_PARM(j9object_t, objectBeingStored, 2);
...
void J9FASTCALL fast_jitWriteBarrierClassStoreMetronome(J9VMThread* currentThread, j9object_t destinationClassObject,
    J9Object** destinationAddress, j9object_t objectBeingStored)
{
    currentThread->javaVM->memoryManagerFunctions->J9MetronomeWriteBarrierJ9ClassStore(
        currentThread, destinationClassObject, destinationAddress, objectBeingStored);
}

void J9FASTCALL old_fast_jitWriteBarrierClassStoreMetronome(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(3);
    ...
    fast_jitWriteBarrierClassStoreMetronome(
        currentThread, destinationClassObject, destinationAddress, objectBeingStored);
}

void J9FASTCALL fast_jitWriteBarrierStoreMetronome(J9VMThread* currentThread, j9object_t destinationObject,
    fj9object_t* destinationAddress, j9object_t objectBeingStored)
{
    currentThread->javaVM->memoryManagerFunctions->J9MetronomeWriteBarrierStore(
        currentThread, destinationObject, destinationAddress, objectBeingStored);
}

void J9FASTCALL old_fast_jitWriteBarrierStoreMetronome(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(3);
    DECLARE_JIT_PARM(j9object_t, destinationObject, 1);
...
void J9FASTCALL old_slow_jitCallJitAddPicToPatchOnClassUnload(J9VMThread* currentThread)
{
    OLD_SLOW_ONLY_JIT_HELPER_PROLOGUE(2);
...
void J9FASTCALL old_slow_jitCallCFunction(J9VMThread* currentThread)
{
    OLD_SLOW_ONLY_JIT_HELPER_PROLOGUE(3);
...
void fast_jitPreJNICallOffloadCheck(J9VMThread* currentThread)
{
...
void fast_jitPostJNICallOffloadCheck(J9VMThread* currentThread)
{
#if defined(J9VM_OPT_JAVA_OFFLOAD_SUPPORT)
    OLD_JIT_HELPER_PROLOGUE(0);
#if defined(J9VM_PORT_ZOS_CEEHDLRSUPPORT)
...
void J9FASTCALL old_fast_jitObjectHashCode(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(1);
    DECLARE_JIT_PARM(j9object_t, object, 1);
    J9JavaVM* vm = currentThread->javaVM;
...
static VMINLINE void* old_slow_jitInduceOSRAtCurrentPCImpl(J9VMThread* currentThread, void* oldPC)
{
...
void* J9FASTCALL old_slow_jitInduceOSRAtCurrentPCAndRecompile(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(0);
...
void* J9FASTCALL old_slow_jitInduceOSRAtCurrentPC(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(0);
    void* oldPC = buildJITResolveFrame(currentThread, J9_SSF_JIT_RESOLVE_INDUCE_OSR, parmCount);
    return old_slow_jitInduceOSRAtCurrentPCImpl(currentThread, oldPC);
...
void J9FASTCALL old_slow_jitInterpretNewInstanceMethod(J9VMThread* currentThread)
{
#if defined(J9VM_JIT_NEW_INSTANCE_PROTOTYPE)
...
void* J9FASTCALL old_slow_jitNewInstanceImplAccessCheck(J9VMThread* currentThread)
{
    void* addr = NULL;
#if defined(J9VM_JIT_NEW_INSTANCE_PROTOTYPE)
    OLD_SLOW_ONLY_JIT_HELPER_PROLOGUE(3);
    ...
    IDATA checkResult = vmFuncs->checkVisibility(
        currentThread, callerClass, thisClass, thisClass->romClass->modifiers, J9_LOOK_REFLECT_CALL);
    ...
    illegalAccess:
        ...
        j9object_t detailMessage = currentThread->javaVM->internalVMFunctions->catUtfToString4(currentThread,
            J9UTF8_DATA(classNameUTF), J9UTF8_LENGTH(classNameUTF), (U_8*)".", 1, J9UTF8_DATA(nameUTF),
            J9UTF8_LENGTH(nameUTF), J9UTF8_DATA(sigUTF), J9UTF8_LENGTH(sigUTF));
...
void J9FASTCALL old_slow_jitTranslateNewInstanceMethod(J9VMThread* currentThread)
{
#if defined(J9VM_JIT_NEW_INSTANCE_PROTOTYPE)
    OLD_SLOW_ONLY_JIT_HELPER_PROLOGUE(2);
...
redo:
    ...
    if ((IDATA)preCount >= 0) {
        ...
        UDATA result
            = VM_AtomicSupport::lockCompareExchange((UDATA*)&objectClass->newInstanceCount, preCount, postCount);
        if (result != preCount) {
            goto redo;
        }
...
void J9FASTCALL old_fast_jitVolatileReadLong(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(1);
...
void J9FASTCALL old_fast_jitVolatileWriteLong(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(3);
    DECLARE_JIT_PARM(U_64*, address, 1);
...
void J9FASTCALL old_fast_jitVolatileReadDouble(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(1);
    DECLARE_JIT_PARM(U_64*, address, 1);
    U_64 value = longVolatileRead(currentThread, address);
...
void J9FASTCALL old_fast_jitVolatileWriteDouble(J9VMThread* currentThread)
{
    OLD_JIT_HELPER_PROLOGUE(1);
    DECLARE_JIT_PARM(U_64*, address, 1);
...
void J9FASTCALL old_slow_icallVMprJavaSendPatchupVirtual(J9VMThread* currentThread)
{
...
void* J9FASTCALL fast_jitNewObject(J9VMThread* currentThread, J9Class* objectClass)
{
...
void* J9FASTCALL fast_jitNewObjectNoZeroInit(J9VMThread* currentThread, J9Class* objectClass)
{
...
#if defined(J9VM_ARCH_X86)
/* TODO Will be cleaned once all platforms adopt the correct parameter order */
fast_jitLookupInterfaceMethod(
    J9VMThread* currentThread, void* jitEIP, J9Class* receiverClass, UDATA* indexAndLiteralsEA)
#else /* J9VM_ARCH_X86 */
fast_jitLookupInterfaceMethod(
    J9VMThread* currentThread, J9Class* receiverClass, UDATA* indexAndLiteralsEA, void* jitEIP)
#endif /* J9VM_ARCH_X86 */
{
...
void* J9FASTCALL fast_jitMethodMonitorEntry(J9VMThread* currentThread, j9object_t syncObject)
{
...
void* J9FASTCALL fast_jitMonitorEntry(J9VMThread* currentThread, j9object_t syncObject)
{
...
void* J9FASTCALL fast_jitMethodMonitorExit(J9VMThread* currentThread, j9object_t syncObject)
{
...
void* J9FASTCALL fast_jitMonitorExit(J9VMThread* currentThread, j9object_t syncObject)
{
...
#if defined(J9VM_ARCH_X86) || defined(J9VM_ARCH_S390)
/* TODO Will be cleaned once all platforms adopt the correct parameter order */
fast_jitTypeCheckArrayStoreWithNullCheck(
    J9VMThread* currentThread, j9object_t objectBeingStored, j9object_t destinationObject)
#else /* J9VM_ARCH_X86 || J9VM_ARCH_S390*/
fast_jitTypeCheckArrayStoreWithNullCheck(
    J9VMThread* currentThread, j9object_t destinationObject, j9object_t objectBeingStored)
#endif /* J9VM_ARCH_X86 || J9VM_ARCH_S390*/
...
void* J9FASTCALL old_slow_jitReportFinalFieldModified(J9VMThread* currentThread)
{
...
void* J9FASTCALL old_slow_jitReportInstanceFieldRead(J9VMThread* currentThread)
{
    OLD_SLOW_ONLY_JIT_HELPER_PROLOGUE(2);
    ...
    if (J9_EVENT_IS_HOOKED(vm->hookInterface, J9HOOK_VM_GET_FIELD)) {
        if (J9_ARE_ANY_BITS_SET(J9OBJECT_CLAZZ(currentThread, object)->classFlags, J9ClassHasWatchedFields)) {
            void* oldPC = buildJITResolveFrameForRuntimeHelper(currentThread, parmCount);
            ALWAYS_TRIGGER_J9HOOK_VM_GET_FIELD(
                vm->hookInterface, currentThread, dataBlock->method, dataBlock->location, object, dataBlock->offset);
            addr = restoreJITResolveFrame(currentThread, oldPC, true, false);
        }
    }
...
void* J9FASTCALL old_slow_jitReportInstanceFieldWrite(J9VMThread* currentThread)
{
    OLD_SLOW_ONLY_JIT_HELPER_PROLOGUE(3);
    DECLARE_JIT_PARM(J9JITWatchedInstanceFieldData*, dataBlock, 1);
    DECLARE_JIT_PARM(j9object_t, object, 2);
    ...
    if (J9_EVENT_IS_HOOKED(vm->hookInterface, J9HOOK_VM_PUT_FIELD)) {
        if (J9_ARE_ANY_BITS_SET(J9OBJECT_CLAZZ(currentThread, object)->classFlags, J9ClassHasWatchedFields)) {
            void* oldPC = buildJITResolveFrameForRuntimeHelper(currentThread, parmCount);
            ALWAYS_TRIGGER_J9HOOK_VM_PUT_FIELD(vm->hookInterface, currentThread, dataBlock->method, dataBlock->location,
                object, dataBlock->offset, *(U_64*)valuePointer);
            addr = restoreJITResolveFrame(currentThread, oldPC, true, false);
        }
    }
...
void* J9FASTCALL old_slow_jitReportStaticFieldRead(J9VMThread* currentThread)
{
    ...
    if (J9_EVENT_IS_HOOKED(vm->hookInterface, J9HOOK_VM_GET_STATIC_FIELD)) {
        ...
        if (J9_ARE_ANY_BITS_SET(fieldClass->classFlags, J9ClassHasWatchedFields)) {
            ...
            ALWAYS_TRIGGER_J9HOOK_VM_GET_STATIC_FIELD(vm->hookInterface, currentThread, dataBlock->method,
                dataBlock->location, fieldClass, dataBlock->fieldAddress);
        restore:
...
void* J9FASTCALL old_slow_jitReportStaticFieldWrite(J9VMThread* currentThread)
{
    OLD_SLOW_ONLY_JIT_HELPER_PROLOGUE(2);
    DECLARE_JIT_PARM(J9JITWatchedStaticFieldData*, dataBlock, 1);
    ...
    if (J9_EVENT_IS_HOOKED(vm->hookInterface, J9HOOK_VM_PUT_STATIC_FIELD)) {
        ...
        if (J9_ARE_ANY_BITS_SET(fieldClass->classFlags, J9ClassHasWatchedFields)) {
            ...
            ALWAYS_TRIGGER_J9HOOK_VM_PUT_STATIC_FIELD(vm->hookInterface, currentThread, dataBlock->method,
                dataBlock->location, fieldClass, dataBlock->fieldAddress, *(U_64*)valuePointer);
        restore:
            addr = restoreJITResolveFrame(currentThread, oldPC, true, false);
        }
    }
...
void initPureCFunctionTable(J9JavaVM* vm)
{
    ...
    jitConfig->old_fast_jitWriteBarrierStoreGenerationalAndConcurrentMark
