...
static J9JITDecompilationInfo* deleteDecompilationForExistingFrame(
    J9VMThread* decompileThread, J9JITDecompilationInfo* info);
...
static void buildInlineStackFrames(J9VMThread* currentThread, J9JITDecompileState* decompileState,
    J9JITDecompilationInfo* decompRecord, UDATA inlineDepth, J9OSRFrame* osrFrame);
static void performDecompile(J9VMThread* currentThread, J9JITDecompileState* decompileState,
    J9JITDecompilationInfo* decompRecord, J9OSRFrame* osrFrame, UDATA numberOfFrames);
static void decompileOuterFrame(J9VMThread* currentThread, J9JITDecompileState* decompileState,
    J9JITDecompilationInfo* decompRecord, J9OSRFrame* osrFrame);
...
static UDATA osrAllFramesSize(
    J9VMThread* currentThread, J9JITExceptionTable* metaData, void* jitPC, UDATA resolveFrameFlags);
static UDATA performOSR(J9VMThread* currentThread, J9StackWalkState* walkState, J9OSRBuffer* osrBuffer,
    U_8* osrScratchBuffer, UDATA scratchBufferSize, UDATA jitStackFrameSize, UDATA* mustDecompile);
static UDATA* jitLocalSlotAddress(
    J9VMThread* currentThread, J9StackWalkState* walkState, UDATA slot, UDATA inlineDepth);
static void jitResetAllMethods(J9VMThread* currentThread);
static void fixStackForNewDecompilation(J9VMThread* currentThread, J9StackWalkState* walkState,
    J9JITDecompilationInfo* info, UDATA reason, J9JITDecompilationInfo** link);
...
static UDATA getPendingStackHeight(
    J9VMThread* currentThread, U_8* interpreterPC, J9Method* ramMethod, UDATA resolveFrameFlags);
...
static void decompPrintMethod(J9VMThread* currentThread, J9Method* method)
{
    ...
    Trc_Decomp_printMethod(currentThread, method, J9UTF8_LENGTH(className), J9UTF8_DATA(className), J9UTF8_LENGTH(name),
        J9UTF8_DATA(name), J9UTF8_LENGTH(sig), J9UTF8_DATA(sig));
}
        ...
        while (methodCount != 0) {
            if (0 == ((UDATA)method->extra & J9_STARTPC_NOT_TRANSLATED)) {
                ...
                if (0 == (J9_ROM_METHOD_FROM_RAM_METHOD(method)->modifiers & J9AccNative)) {
                    J9JITExceptionTable* metaData
                        = vm->jitConfig->jitGetExceptionTableFromPC(currentThread, (UDATA)method->extra);
                    if (NULL != metaData) {
...
J9JITDecompilationInfo* jitCleanUpDecompilationStack(
...
void jitCodeBreakpointAdded(J9VMThread* currentThread, J9Method* method)
{
    PORT_ACCESS_FROM_VMC(currentThread);
    ...
    breakpointedMethod
        = (J9JITBreakpointedMethod*)j9mem_allocate_memory(sizeof(J9JITBreakpointedMethod), OMRMEM_CATEGORY_JIT);
    ...
    do {
        ...
        walkState.flags = J9_STACKWALK_ITERATE_FRAMES | J9_STACKWALK_SKIP_INLINES | J9_STACKWALK_VISIBLE_ONLY
            | J9_STACKWALK_ITERATE_HIDDEN_JIT_FRAMES | J9_STACKWALK_MAINTAIN_REGISTER_MAP;
...
void jitCodeBreakpointRemoved(J9VMThread* currentThread, J9Method* method)
{
    PORT_ACCESS_FROM_VMC(currentThread);
    J9JITConfig* jitConfig = currentThread->javaVM->jitConfig;
...
void jitHotswapOccurred(J9VMThread* currentThread)
{
    /* We have exclusive */
...
void jitDecompileMethod(J9VMThread* currentThread, J9JITDecompilationInfo* decompRecord)
{
    ...
    walkState.flags = J9_STACKWALK_ITERATE_FRAMES | J9_STACKWALK_SKIP_INLINES | J9_STACKWALK_VISIBLE_ONLY
        | J9_STACKWALK_MAINTAIN_REGISTER_MAP | J9_STACKWALK_ITERATE_HIDDEN_JIT_FRAMES
    ...
    J9VMThread* currentThread, U_8* interpreterPC, J9Method* ramMethod, UDATA resolveFrameFlags)
{
    ...
    default:
        ...
        pendingStackHeight = (UDATA)vmFuncs->j9stackmap_StackBitsForPC(
            portLibrary, offsetPC, romClass, originalROMMethod, NULL, 0, NULL, NULL, NULL);
        ...
        case JBinvokestaticsplit:
            ...
            pendingStackHeight -= getSendSlotsFromSignature(
                J9UTF8_DATA(J9ROMNAMEANDSIGNATURE_SIGNATURE(getNASFromInvoke(interpreterPC, romClass))));
            break;
        }
        ...
        case J9_STACK_FLAGS_JIT_ALLOCATION_RESOLVE:
            /* Decompile after allocation completes.  The JIT has not mapped the bytecode parameters in the pending
             * stack */
            switch (bytecode) {
            case JBanewarray:
    ...
    J9JITDecompilationInfo* decompRecord, J9OSRFrame* osrFrame, UDATA numberOfFrames)
{
    ...
    if (FALSE == decompRecord->usesOSR) {
        ...
        UDATA* jitTempBase = ((UDATA*)(((U_8*)decompileState->bp)
                                 + ((J9JITStackAtlas*)decompileState->metaData->gcStackAtlas)->localBaseOffset))
            + decompileState->metaData->tempOffset;
        ...
        /* The pending slots immediately precede the temps in both the OSR frame and the JIT frame, so copy them all at
         * once */
        memcpy(osrTempBase - pendingStackHeight, jitTempBase - pendingStackHeight,
            (tempCount + pendingStackHeight) * sizeof(UDATA));
    }
    ...
    J9JITDecompilationInfo* decompRecord, J9OSRFrame* osrFrame)
{
    ...
    if (decompileState->previousFrameBytecodes) {
        ...
        memcpy(&(j2iFrame->J9SW_LOWEST_MEMORY_PRESERVED_REGISTER), decompileState->preservedRegisterValues,
            J9SW_JIT_CALLEE_PRESERVED_SIZE * sizeof(UDATA));
        ...
        /* If the next thing on the decompilation stack used to point to the return address save slot in the decompiled
         * JIT frame, point it to the slot in the J2I frame. */

        if ((info = currentThread->decompilationStack) != NULL) {
            if (info->pcAddress == jitReturnPCAddress) {
                Trc_Decomp_performDecompile_fixingDecompStack(
                    currentThread, info, info->pcAddress, &(j2iFrame->returnAddress), info->pc);
    ...
    *link = info;
    ...
    if (0 != walkState->resolveFrameFlags) {
        ...
        J9ROMNameAndSignature* nameAndSig
            = getNASFromInvoke(bytecodePC, J9_CLASS_FROM_METHOD(osrFrame->method)->romClass);
        sig = J9ROMNAMEANDSIGNATURE_SIGNATURE(nameAndSig);
        name = J9ROMNAMEANDSIGNATURE_NAME(nameAndSig);
        Trc_Decomp_addDecompilation_explainInvoke(
            currentThread, J9UTF8_LENGTH(name), J9UTF8_DATA(name), J9UTF8_LENGTH(sig), J9UTF8_DATA(sig));
...
static J9JITDecompilationInfo* addDecompilation(J9VMThread* currentThread, J9StackWalkState* walkState, UDATA reason)
{
    ...
    Trc_Decomp_addDecompilation_walkStateInfo(
        currentThread, walkState->bp, walkState->arg0EA, walkState->constantPool, walkState->pc);
    Trc_Decomp_addDecompilation_reason(currentThread, reason,
        (reason & JITDECOMP_CODE_BREAKPOINT) ? " CODE_BREAKPOINT" : "",
        (reason & JITDECOMP_DATA_BREAKPOINT) ? " DATA_BREAKPOINT" : "", (reason & JITDECOMP_HOTSWAP) ? " HOTSWAP" : "",
        (reason & JITDECOMP_POP_FRAMES) ? " POP_FRAMES" : "", (reason & JITDECOMP_SINGLE_STEP) ? " SINGLE_STEP" : "",
    ...
    if (osrFrame) {
        ...
        osrResultCode = performOSR(currentThread, walkState, &info->osrBuffer, (U_8*)osrScratchBuffer,
            scratchBufferSize, jitStackFrameSize, &mustDecompile);
...
void c_jitDecompileAtExceptionCatch(J9VMThread* currentThread)
{
...
void jitDecompileMethodForFramePop(J9VMThread* currentThread, UDATA skipCount)
{
    J9JITDecompileState decompileState;
    J9StackWalkState walkState;
    J9JITDecompilationInfo* decompRecord = currentThread->decompilationStack;
...
void jitDataBreakpointAdded(J9VMThread* currentThread)
{
    J9JavaVM* vm = currentThread->javaVM;
...
void jitDataBreakpointRemoved(J9VMThread* currentThread)
{
    J9JavaVM* vm = currentThread->javaVM;
...
void jitExceptionCaught(J9VMThread* currentThread)
{
    PORT_ACCESS_FROM_VMC(currentThread);
...
void jitSingleStepAdded(J9VMThread* currentThread)
{
    /* We have exclusive */
...
void jitSingleStepRemoved(J9VMThread* currentThread)
{
    /* We have exclusive */
...
static void jitFramePopNotificationAdded(J9VMThread* currentThread, J9StackWalkState* walkState, UDATA inlineDepth)
{
    ...
    Trc_Decomp_jitFramePopNotificationAdded_Entry(
        currentThread, walkState->walkThread, walkState->arg0EA, walkState->method);
    decompPrintMethod(currentThread, walkState->method);
...
void jitStackLocalsModified(J9VMThread* currentThread, J9StackWalkState* walkState)
{
...
void jitBreakpointedMethodCompiled(J9VMThread* currentThread, J9Method* method, void* startAddress)
{
    J9JITConfig* jitConfig = currentThread->javaVM->jitConfig;
    ...
    U_8* osrScratchBuffer, UDATA scratchBufferSize, UDATA jitStackFrameSize, UDATA* mustDecompile)
{
     ...
     */
    Assert_CodertVM_true(jitStackFrameSize
        == ((J9_ARG_COUNT_FROM_ROM_METHOD(J9_ROM_METHOD_FROM_RAM_METHOD(metaData->ramMethod)) + 1
                + metaData->totalFrameSize)
               * sizeof(UDATA)));
    ...
    Assert_CodertVM_true((((UDATA)osrBlock > metaData->startPC) && ((UDATA)osrBlock < metaData->endWarmPC))
        || ((0 != metaData->startColdPC)
               && (((UDATA)osrBlock >= metaData->startColdPC) && ((UDATA)osrBlock < metaData->endPC))));
 ...
 * @param[in] *jitPC compiled PC at which to OSR
 */
void induceOSROnCurrentThread(J9VMThread* currentThread)
{
    J9JavaVM* vm = currentThread->javaVM;
    PORT_ACCESS_FROM_JAVAVM(vm);
    J9JITExceptionTable* metaData = NULL;
    ...
    osrReturnCode = performOSR(currentThread, &walkState, &decompilationRecord->osrBuffer, osrScratchBuffer,
        scratchBufferSize, jitStackFrameSize, NULL);
    if (OSR_OK != osrReturnCode) {
    outOfMemory:
        decompilationRecord->reason = reason;
        freeDecompilationRecord(currentThread, decompilationRecord, FALSE);
    } else {
        fixStackForNewDecompilation(
            currentThread, &walkState, decompilationRecord, reason, &currentThread->decompilationStack);
    }
}
...
void c_jitDecompileAfterAllocation(J9VMThread* currentThread)
{
...
/* Resolve frame is already built */
void c_jitDecompileAtCurrentPC(J9VMThread* currentThread)
{
    /* Fetch and unstack the decompilation information for this frame */
    J9JITDecompilationInfo* decompRecord = fetchAndUnstackDecompilationInfo(currentThread);
    /* Fix the saved PC since the frame is still on the stack */
...
/* Resolve frame is already built */
void c_jitDecompileBeforeMethodMonitorEnter(J9VMThread* currentThread)
{
    /* Fetch and unstack the decompilation information for this frame */
    J9JITDecompilationInfo* decompRecord = fetchAndUnstackDecompilationInfo(currentThread);
...
/* Resolve frame is already built */
void c_jitDecompileBeforeReportMethodEnter(J9VMThread* currentThread)
{
    /* Fetch and unstack the decompilation information for this frame */
    J9JITDecompilationInfo* decompRecord = fetchAndUnstackDecompilationInfo(currentThread);
    J9Method* const method = decompRecord->method;
...
/* Resolve frame is already built */
void c_jitDecompileAfterMonitorEnter(J9VMThread* currentThread)
{
    /* Fetch and unstack the decompilation information for this frame */
    J9JITDecompilationInfo* decompRecord = fetchAndUnstackDecompilationInfo(currentThread);
    /* Fix the saved PC since the frame is still on the stack */
...
void c_jitDecompileOnReturn(J9VMThread* currentThread)
{
...
void c_jitReportExceptionCatch(J9VMThread* currentThread)
{
    ...
    if (J9_BUILDER_SYMBOL(jitDecompileAtExceptionCatch) == jitPC) {
        currentThread->decompilationStack->pcAddress
            = (U_8**)&(((J9SFJITResolveFrame*)currentThread->sp)->returnAddress);
    }
    /* Report the event */
