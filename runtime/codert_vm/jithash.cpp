...
#define DETERMINE_BUCKET(value, start, buckets) \
    (((((UDATA)(value) - (UDATA)(start)) >> (UDATA)9) * (UDATA)sizeof(UDATA)) + (UDATA)(buckets))
#define BUCKET_SIZE 512
#define METHOD_STORE_SIZE 256

#define COUNT_MARK_BIT 20

J9JITExceptionTable** hash_jit_artifact_array_insert(J9PortLibrary* portLibrary, J9JITHashTable* table,
    J9JITExceptionTable** array, J9JITExceptionTable* dataToInsert, UDATA startPC)
{
    ...
    if (LOW_BIT_SET(array)) {
        ...
        if ((table->currentAllocate + 2)
            > table->methodStoreEnd) { /* This comparison is safe since currentAllocate and methodStoreEnd will always
                                          be pointing into the same allocated block */
        ...
        if (*newElement == NULL) {
            ...
            *(newElement - 1) = dataToInsert;
            ...
            /* CMVC 117169 - increase the next allocation point only if the new element is not being stored into freed
             * space */
            ...
            if ((table->currentAllocate + chainLength + 1)
                > table->methodStoreEnd) { /* This comparison is safe since currentAllocate and methodStoreEnd will
            ...
            memcpy(returnVal + 1, array,
                chainLength * sizeof(UDATA)); /* safe to memcpy since the new array is not yet visible */
        }
    }
...
J9JITExceptionTable** hash_jit_artifact_array_remove(
    J9PortLibrary* portLibrary, J9JITExceptionTable** array, J9JITExceptionTable* dataToRemove)
{
    ...
    if (LOW_BIT_SET(*array)) {
        ...
        J9JITExceptionTable** rc
            = (J9JITExceptionTable**)*array; /* Only one pointer left.  Just return the one pointer */
        *array = NULL;
        return rc;
    }
...
UDATA
hash_jit_artifact_insert_range(
    J9PortLibrary* portLibrary, J9JITHashTable* table, J9JITExceptionTable* dataToInsert, UDATA startPC, UDATA endPC)
{
    J9JITExceptionTable** index;
    ...
    do {
        if (*index) {
            temp = hash_jit_artifact_array_insert(
                portLibrary, table, (J9JITExceptionTable**)*index, dataToInsert, startPC);
            if (!temp) {
                return 2;
            }
            VM_AtomicSupport::writeBarrier();
...
UDATA hash_jit_artifact_insert(J9PortLibrary* portLibrary, J9JITHashTable* table, J9JITExceptionTable* dataToInsert)
{
    ...
    result = hash_jit_artifact_insert_range(
        portLibrary, table, dataToInsert, dataToInsert->startPC, dataToInsert->endWarmPC);
    if (result)
        return result;
    if (dataToInsert->startColdPC)
        result = hash_jit_artifact_insert_range(
            portLibrary, table, dataToInsert, dataToInsert->startColdPC, dataToInsert->endPC);

    return result;
}

UDATA hash_jit_artifact_remove_range(
    J9PortLibrary* portLibrary, J9JITHashTable* table, J9JITExceptionTable* dataToRemove, UDATA startPC, UDATA endPC)
{
    J9JITExceptionTable** index;
    J9JITExceptionTable** endIndex;
    J9JITExceptionTable* temp;
    ...
    do {
        if (LOW_BIT_SET(*index)) {
            ...
            temp = (J9JITExceptionTable*)hash_jit_artifact_array_remove(
                portLibrary, (J9JITExceptionTable**)*index, dataToRemove);
            if (!temp)
                return (UDATA)1;
            else if (temp == (J9JITExceptionTable*)1)
                return (UDATA)2;
            else
                *index = temp;
        } else
...
UDATA hash_jit_artifact_remove(J9PortLibrary* portLibrary, J9JITHashTable* table, J9JITExceptionTable* dataToRemove)
{
    ...
    result = hash_jit_artifact_remove_range(
        portLibrary, table, dataToRemove, dataToRemove->startPC, dataToRemove->endWarmPC);
    if (result)
        return result;

    if (dataToRemove->startColdPC)
        result = hash_jit_artifact_remove_range(
            portLibrary, table, dataToRemove, dataToRemove->startColdPC, dataToRemove->endPC);
    return result;
}
...
J9JITHashTable* hash_jit_toJ9MemorySegment(
    J9JITHashTable* table, J9MemorySegment* codeCache, J9MemorySegment* dataCache)
{
    ...
    while ((*index == 0) && (index < endIndex))
        ++index;
    while ((*endIndex == 0) && (index <= endIndex))
        --endIndex;
