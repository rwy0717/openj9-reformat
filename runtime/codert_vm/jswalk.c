...
#define SET_A0_CP_METHOD(walkState)                                                                               \
    (walkState)->arg0EA                                                                                           \
        = (UDATA*)(((U_8*)((walkState)->bp + (walkState)->jitInfo->slots)) + sizeof(J9JITFrame) - sizeof(UDATA)), \
...
#define COMMON_UNWIND_TO_NEXT_FRAME(walkState) \
    (walkState)->resolveFrameFlags = 0, UPDATE_PC_FROM(walkState, ((J9JITFrame*)((walkState)->bp))->returnPC)
#else
#define COMMON_UNWIND_TO_NEXT_FRAME(walkState) UPDATE_PC_FROM(walkState, ((J9JITFrame*)((walkState)->bp))->returnPC)
#endif
...
#define UNWIND_TO_NEXT_FRAME(walkState)                                                    \
    (walkState)->unwindSP = ((UDATA*)(((J9JITFrame*)((walkState)->bp))                     \
        + 1)), /* arguments are passed in register, argument area lives in caller frame */ \
        COMMON_UNWIND_TO_NEXT_FRAME(walkState)
...
#define CLEAR_LOCAL_REGISTER_MAP_ENTRIES(walkState)                                                            \
    {                                                                                                          \
        UDATA clearRegNum;                                                                                     \
        for (clearRegNum = 0; clearRegNum < (J9SW_POTENTIAL_SAVED_REGISTERS - J9SW_JIT_CALLEE_PRESERVED_SIZE); \
             ++clearRegNum) {                                                                                  \
...
static void walkJITFrameSlots(J9StackWalkState* walkState, U_8* jitDescriptionBits, U_8* stackAllocMapBits,
    U_8** jitDescriptionCursor, U_8** stackAllocMapCursor, UDATA* jitBitsRemaining, UDATA* mapBytesRemaining,
    UDATA* scanCursor, UDATA slotsRemaining, void* stackMap, J9JITStackAtlas* gcStackAtlas, char* slotDescription);
...
static jvmtiIterationControl stackAllocatedObjectSlotWalkFunction(J9JavaVM* javaVM,
    J9MM_IterateObjectDescriptor* objectDesc, J9MM_IterateObjectRefDescriptor* refDesc, void* userData);

static UDATA countOwnedObjectMonitors(J9StackWalkState* walkState);
static UDATA walkLiveMonitorSlots(J9StackWalkState* walkState, J9JITStackAtlas* gcStackAtlas, U_8* liveMonitorMap,
    U_8* monitorMask, U_16 numberOfMapBits);
static void countLiveMonitorSlots(J9StackWalkState* walkState, J9JITStackAtlas* gcStackAtlas, U_8* liveMonitorMap,
    U_8* monitorMask, U_16 numberOfMapBits);
...
UDATA jitWalkStackFrames(J9StackWalkState* walkState)
{
    ...
    while ((walkState->jitInfo = jitGetExceptionTable(walkState)) != NULL) {
        ...
        if ((!(walkState->flags & J9_STACKWALK_SKIP_INLINES)) && getJitInlinedCallInfo(walkState->jitInfo)) {
            jitGetMapsFromPC(walkState->walkThread->javaVM, walkState->jitInfo, (UDATA)walkState->pc,
                &(walkState->stackMap), &(walkState->inlineMap));
            if (NULL != walkState->inlineMap) {
                ...
                if (NULL != walkState->inlinedCallSite) {
                    walkState->inlineDepth
                        = getJitInlineDepthFromCallSite(walkState->jitInfo, walkState->inlinedCallSite);
                    ...
                    do {
                        ...
                        walkState->bytecodePCOffset = (IDATA)getCurrentByteCodeIndexAndIsSameReceiver(
                            walkState->jitInfo, walkState->inlineMap, walkState->inlinedCallSite, NULL);
#ifdef J9VM_INTERP_STACKWALK_TRACING
                    ...
                    resumeWalkInline:
                        walkState->inlinedCallSite
                            = getNextInlinedCallSite(walkState->jitInfo, walkState->inlinedCallSite);
        ...
        if (walkState->flags & J9_STACKWALK_RECORD_BYTECODE_PC_OFFSET) {
            walkState->bytecodePCOffset = (NULL == walkState->inlineMap)
                ? (IDATA)-1
                : (IDATA)getCurrentByteCodeIndexAndIsSameReceiver(walkState->jitInfo, walkState->inlineMap, NULL, NULL);
        }
...
#ifdef J9VM_INTERP_STACKWALK_TRACING
    swPrintf(walkState, 2,
        "I2J values: PC = %p, A0 = %p, walkSP = %p, literals = %p, JIT PC = %p, pcAddress = %p, decomp = %p\n",
        walkState->pc, REMOTE_ADDR(walkState->arg0EA), REMOTE_ADDR(walkState->walkSP), walkState->literals, failedPC,
#ifdef J9VM_JIT_FULL_SPEED_DEBUG
...
static UDATA walkTransitionFrame(J9StackWalkState* walkState)
{
    ...
    if (walkState->frameFlags & J9_STACK_FLAGS_JIT_RESOLVE_FRAME) {
        J9SFJITResolveFrame* resolveFrame
            = (J9SFJITResolveFrame*)((U_8*)walkState->bp - sizeof(J9SFJITResolveFrame) + sizeof(UDATA));
        UDATA resolveFrameType = walkState->frameFlags & J9_STACK_FLAGS_JIT_FRAME_SUB_TYPE_MASK;

#ifdef J9VM_INTERP_STACKWALK_TRACING
        switch (resolveFrameType) {
        case J9_STACK_FLAGS_JIT_GENERIC_RESOLVE:
            swPrintf(walkState, 3, "\tGeneric resolve\n");
            break;
        case J9_STACK_FLAGS_JIT_STACK_OVERFLOW_RESOLVE_FRAME:
            swPrintf(walkState, 3, "\tStack overflow resolve\n");
            break;
        case J9_STACK_FLAGS_JIT_DATA_RESOLVE:
            swPrintf(walkState, 3, "\tData resolve\n");
            break;
        case J9_STACK_FLAGS_JIT_RUNTIME_HELPER_RESOLVE:
            swPrintf(walkState, 3, "\tRuntime helper resolve\n");
            break;
        case J9_STACK_FLAGS_JIT_LOOKUP_RESOLVE:
            swPrintf(walkState, 3, "\tInterface lookup resolve\n");
            break;
        case J9_STACK_FLAGS_JIT_INTERFACE_METHOD_RESOLVE:
            swPrintf(walkState, 3, "\tInterface method resolve\n");
            break;
        case J9_STACK_FLAGS_JIT_SPECIAL_METHOD_RESOLVE:
            swPrintf(walkState, 3, "\tSpecial method resolve\n");
            break;
        case J9_STACK_FLAGS_JIT_STATIC_METHOD_RESOLVE:
            swPrintf(walkState, 3, "\tStatic method resolve\n");
            break;
        case J9_STACK_FLAGS_JIT_VIRTUAL_METHOD_RESOLVE:
            swPrintf(walkState, 3, "\tVirtual method resolve\n");
            break;
        case J9_STACK_FLAGS_JIT_RECOMPILATION_RESOLVE:
            swPrintf(walkState, 3, "\tRecompilation resolve\n");
            break;
        case J9_STACK_FLAGS_JIT_MONITOR_ENTER_RESOLVE:
            swPrintf(walkState, 3, "\tMonitor enter resolve\n");
            break;
        case J9_STACK_FLAGS_JIT_METHOD_MONITOR_ENTER_RESOLVE:
            swPrintf(walkState, 3, "\tMethod monitor enter resolve\n");
            break;
        case J9_STACK_FLAGS_JIT_FAILED_METHOD_MONITOR_ENTER_RESOLVE:
            swPrintf(walkState, 3, "\tFailed method monitor enter resolve\n");
            break;
        case J9_STACK_FLAGS_JIT_ALLOCATION_RESOLVE:
            swPrintf(walkState, 3, "\tAllocation resolve\n");
            break;
        case J9_STACK_FLAGS_JIT_BEFORE_ANEWARRAY_RESOLVE:
            swPrintf(walkState, 3, "\tBefore anewarray resolve\n");
            break;
        case J9_STACK_FLAGS_JIT_BEFORE_MULTIANEWARRAY_RESOLVE:
            swPrintf(walkState, 3, "\tBefore multianewarray resolve\n");
            break;
        case J9_STACK_FLAGS_JIT_INDUCE_OSR_RESOLVE:
            swPrintf(walkState, 3, "\tInduce OSR resolve\n");
            break;
        case J9_STACK_FLAGS_JIT_EXCEPTION_CATCH_RESOLVE:
            swPrintf(walkState, 3, "\tException catch resolve\n");
            break;
        default:
            swPrintf(walkState, 3, "\tUnknown resolve type %p\n", resolveFrameType);
        }
#endif
        ...
        J9SFNativeMethodFrame* nativeMethodFrame
            = (J9SFNativeMethodFrame*)((U_8*)walkState->bp - sizeof(J9SFNativeMethodFrame) + sizeof(UDATA));
        ...
        J9SFSpecialFrame* specialFrame
            = (J9SFSpecialFrame*)((U_8*)walkState->bp - sizeof(J9SFSpecialFrame) + sizeof(UDATA));
...
static void jitWalkFrame(J9StackWalkState* walkState, UDATA walkLocals, void* stackMap)
{
    ...
    if (stackMap == NULL) {
        ...
        if (stackMap == NULL) {
            ...
            j9nls_printf(PORTLIB, J9NLS_ERROR | J9NLS_MULTI_LINE, J9NLS_CODERT_UNABLE_TO_LOCATE_JIT_STACKMAP_METHOD,
                (U_32)J9UTF8_LENGTH(className), J9UTF8_DATA(className), (U_32)J9UTF8_LENGTH(name), J9UTF8_DATA(name),
                (U_32)J9UTF8_LENGTH(sig), J9UTF8_DATA(sig), walkState->method);
            j9nls_printf(PORTLIB, J9NLS_ERROR | J9NLS_END_MULTI_LINE, J9NLS_CODERT_UNABLE_TO_LOCATE_JIT_STACKMAP_PC,
                walkState->pc, walkState->pc - walkState->jitInfo->startPC, walkState->jitInfo);
    ...
    swPrintf(walkState, 2, "\tstackMap=%p, slots=%d parmBaseOffset=%d, parmSlots=%d, localBaseOffset=%d\n",
        REMOTE_ADDR(stackMap), walkState->jitInfo->slots, gcStackAtlas->parmBaseOffset, gcStackAtlas->numberOfParmSlots,
        gcStackAtlas->localBaseOffset);
#endif
    ...
    stackAllocMapCursor
        = getStackAllocMapFromJitPC(walkState->walkThread->javaVM, walkState->jitInfo, (UDATA)walkState->pc, stackMap);
...
#ifdef J9VM_INTERP_STACKWALK_TRACING
        swPrintf(walkState, 4, "\tDescribed JIT args starting at %p for %d slots\n", REMOTE_ADDR(objectArgScanCursor),
            gcStackAtlas->numberOfParmSlots);
#endif
        walkJITFrameSlots(walkState, &jitDescriptionBits, &stackAllocMapBits, &jitDescriptionCursor,
            &stackAllocMapCursor, &jitBitsRemaining, &mapBytesRemaining, objectArgScanCursor,
            getJitNumberOfParmSlots(gcStackAtlas), stackMap, NULL, ": a");
    }
...
#endif
            walkJITFrameSlots(walkState, &jitDescriptionBits, &stackAllocMapBits, &jitDescriptionCursor,
                &stackAllocMapCursor, &jitBitsRemaining, &mapBytesRemaining, objectTempScanCursor,
                walkState->bp - objectTempScanCursor, stackMap, gcStackAtlas, ": t");
        }
    }
        ...
        if (*jitDescriptionBits & 1) {
            ...
            if (walkState->flags & J9_STACKWALK_CHECK_I_SLOTS_FOR_OBJECTS) {
                ...
                if (walkState->userData1 == (void*)8) {
                    swPrintf(walkState, 3,
                        "SCANNING I SLOT 0x%x contains: 0x%x, object is in heap 0x%x (0 is heap pointer) \n",
                        scanCursor, *scanCursor,
                        walkState->walkThread->javaVM->memoryManagerFunctions->j9gc_ext_check_is_valid_heap_object(
                            walkState->walkThread->javaVM, *(j9object_t*)scanCursor, 0));
                }
#endif
                if (walkState->walkThread->javaVM->memoryManagerFunctions->j9gc_ext_check_is_valid_heap_object(
                        walkState->walkThread->javaVM, *(j9object_t*)scanCursor, 0)
                    == 0) {
#ifdef J9VM_INTERP_STACKWALK_TRACING
                    ...
                    if ((walkState->userData1 == (void*)1) || (walkState->userData1 == (void*)8)) {
                        swPrintf(walkState, 3,
                            "Uncollected ref SLOT 0x%x pointing at object ref 0x%x for stackmap at seachPC 0x%x: \n",
                            scanCursor, *scanCursor, walkState->pc);
                    }
#endif
                    if (walkState->userData2 == (void*)4) {
                        walkState->walkThread->javaVM->memoryManagerFunctions->j9gc_modron_global_collect(
                            walkState->currentThread);
                    } else {
                        walkState->walkThread->javaVM->memoryManagerFunctions->j9gc_modron_local_collect(
                            walkState->currentThread);
                        walkState->walkThread->javaVM->memoryManagerFunctions->j9gc_modron_local_collect(
                            walkState->currentThread);
                    }
    ...
    if (registerMap || highWordRegisterMap) {
        ...
        while (count) {
            if (registerMap & 1) {
                ...
                swPrintf(walkState, 4, "\t\tJIT-RegisterMap-O-Slot[%p] = %p (%s)\n", REMOTE_ADDR(targetObject),
                    oldObject, jitRegisterNames[mapCursor - ((UDATA**)&(walkState->registerEAs))]);
#endif
                walkState->objectSlotWalkFunction(
                    walkState->walkThread, walkState, targetObject, REMOTE_ADDR(targetObject));
#ifdef J9VM_INTERP_STACKWALK_TRACING
#ifdef J9VM_OUT_OF_PROCESS
                ...
                if (highWordRegisterMap & 1) {
                    ...
                    swPrintf(walkState, 4, "\t\tJIT-RegisterMap-O-Slot (Low word) [%p] = 0x%x (%s)\n",
                        REMOTE_ADDR(targetCompressedObject), oldCompressedObject,
                        jitRegisterNames[mapCursor - ((UDATA**)&(walkState->registerEAs))]);
#endif

                    walkState->objectSlotWalkFunction(walkState->walkThread, walkState, targetDecompressedObject,
                        REMOTE_ADDR(targetDecompressedObject));
                ...
                if (targetSlot) {
                    swPrintf(walkState, 5, "\t\tJIT-RegisterMap-I-Slot[%p] = %p (%s)\n", REMOTE_ADDR(targetSlot),
                        *targetSlot, jitRegisterNames[mapCursor - ((UDATA**)&(walkState->registerEAs))]);
#ifdef J9VM_INTERP_LINEAR_STACKWALK_TRACING
...
#define JIT_FPR_PARM_ADDRESS(walkState, fpParmNumber)                                                           \
    (((U_64*)((walkState)->walkedEntryLocalStorage->jitFPRegisterStorageBase))                                  \
        + (J9_ARE_ANY_BITS_SET(                                                                                 \
               (walkState)->walkThread->javaVM->extendedRuntimeFlags, J9_EXTENDED_RUNTIME_USE_VECTOR_REGISTERS) \
...
static void jitWalkResolveMethodFrame_walkD(
    J9StackWalkState* walkState, UDATA** pendingSendScanCursor, UDATA* floatRegistersRemaining)
{
...
static void jitWalkResolveMethodFrame_walkF(
    J9StackWalkState* walkState, UDATA** pendingSendScanCursor, UDATA* floatRegistersRemaining)
{
    if (*floatRegistersRemaining) {
        UDATA fpParmNumber = J9SW_JIT_FLOAT_ARGUMENT_REGISTER_COUNT - *floatRegistersRemaining;
        ...
        if (walkState->flags & J9_STACKWALK_SAVE_STACKED_REGISTERS) {
            ...
            jitConvertStoredDoubleRegisterToSingle(
                JIT_FPR_PARM_ADDRESS(walkState, fpParmNumber), (U_32*)*pendingSendScanCursor);
#else
...
static void jitWalkResolveMethodFrame_walkJ(
    J9StackWalkState* walkState, UDATA** pendingSendScanCursor, UDATA** stackSpillCursor, UDATA* stackSpillCount)
{
...
static void jitWalkResolveMethodFrame_walkI(
    J9StackWalkState* walkState, UDATA** pendingSendScanCursor, UDATA** stackSpillCursor, UDATA* stackSpillCount)
{
...
static void jitWalkResolveMethodFrame(J9StackWalkState* walkState)
{
    ...
    if (resolveFrameType == J9_STACK_FLAGS_JIT_RECOMPILATION_RESOLVE) {
...
#endif
            WALK_O_SLOT(
                (j9object_t*)(walkState->unwindSP + J9SW_JIT_LOOKUP_INTERFACE_RESOLVE_OFFSET_TO_SAVED_RECEIVER));
        }
#endif
        ...
        if ((resolveFrameType == J9_STACK_FLAGS_JIT_STATIC_METHOD_RESOLVE)
            || (resolveFrameType == J9_STACK_FLAGS_JIT_SPECIAL_METHOD_RESOLVE)) {
#ifdef J9SW_JIT_FLOAT_ARGUMENT_REGISTER_COUNT
            floatRegistersRemaining = 0;
#endif
...
#endif
                WALK_O_SLOT(
                    (j9object_t*)(walkState->unwindSP + J9SW_JIT_VIRTUAL_METHOD_RESOLVE_OFFSET_TO_SAVED_RECEIVER));
            }
#endif
            walkState->unwindSP += getJitVirtualMethodResolvePushes();
        }
...
#ifdef J9VM_INTERP_LINEAR_STACKWALK_TRACING
            lswRecordSlot(walkState, REMOTE_ADDR(registerSaveAddress), LSW_TYPE_JIT_REG_SLOT, "%s: %s", description,
                jitRegisterNames[i]);
#endif
            swPrintf(walkState, 3, "\tJIT-%s-RegisterMap[%p] = %p (%s)\n", description,
                REMOTE_ADDR(registerSaveAddress), *registerSaveAddress, jitRegisterNames[i]);
        }
    }
}
 ...
 * walkState->decompilationStack, walkState->decompilationStack->pcAddress, walkState->decompilationStack->pc); */
#endif
        if (walkState->pcAddress == walkState->decompilationStack->pcAddress) {
            walkState->pc = walkState->decompilationStack->pc;
            if (J9_STACK_FLAGS_JIT_EXCEPTION_CATCH_RESOLVE
                == (walkState->resolveFrameFlags & J9_STACK_FLAGS_JIT_FRAME_SUB_TYPE_MASK)) {
                walkState->pc += 1;
            }
...
J9JITExceptionTable* jitGetExceptionTableFromPC(J9VMThread* vmThread, UDATA jitPC)
{
...
#ifdef J9VM_OUT_OF_PROCESS
    J9JITExceptionTable* remoteMetaData
        = dbgFindJITMetaData(vmThread->javaVM->jitConfig->translationArtifacts, maskedPC);
    ...
    if (NULL == artifactSearchCache) {
        ...
        artifactSearchCache = j9mem_allocate_memory(
            JIT_ARTIFACT_SEARCH_CACHE_SIZE * sizeof(TR_jit_artifact_search_cache), OMRMEM_CATEGORY_JIT);
        if (NULL == artifactSearchCache) {
...
static void jitDropToCurrentFrame(J9StackWalkState* walkState)
{
    ...
    if (J9_FSD_ENABLED(vmThread->javaVM)) {
        ...
        currentFrameDecompilation
            = vmThread->javaVM->jitConfig->jitCleanUpDecompilationStack(vmThread, walkState, FALSE);
...
static void jitWalkStackAllocatedObject(J9StackWalkState* walkState, j9object_t object)
{
    ...
    (REMOTE_ADDR(vm), vm->portLibrary, &descriptor, iterateObjectSlotsFlags, stackAllocatedObjectSlotWalkFunction,
        walkState);
}
...
jitGetOwnedObjectMonitors(J9StackWalkState* walkState)
{
    ...
    if (inlineMap) {
        ...
        for (inlinedCallSite = getFirstInlinedCallSite(walkState->jitInfo, inlineMap); inlinedCallSite != NULL;
    ...
    if (liveMonitorMap) {
        walkLiveMonitorSlots(
            walkState, gcStackAtlas, liveMonitorMap, getMonitorMask(gcStackAtlas, NULL), numberOfMapBits);
    }
...
static UDATA countOwnedObjectMonitors(J9StackWalkState* walkState)
{
    ...
    /* Get the live monitors for the outer frame */
    if (liveMonitorMap) {
        countLiveMonitorSlots(
            walkState, gcStackAtlas, liveMonitorMap, getMonitorMask(gcStackAtlas, 0), numberOfMapBits);
    }
    return J9_STACKWALK_KEEP_ITERATING;
}
 ...
 */
static J9OSRFrame* jitWalkOSRFrame(J9StackWalkState* walkState, J9OSRFrame* osrFrame)
{
...
#ifdef J9VM_INTERP_STACKWALK_TRACING
    {
        ...
        swPrintf(walkState, 3,
            "\tJIT-OSRFrame = %p, bytecodePC = %p, numberOfLocals = %d, maxStack = %d, pendingStackHeight = %d\n",
    ...
    walkBytecodeFrameSlots(
        walkState, method, offsetPC, localSlots - 1, pendingStackHeight, nextFrame - 1, numberOfLocals, TRUE);
