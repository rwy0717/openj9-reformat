...
/* Note these values must all be odd since the encoded bytes overlay a pointer field in the hash table entry where we
 * use low-tag to indicate an inline encoding */
...
void* j9ThunkLookupNameAndSig(void* jitConfig, void* parm)
{
    ...
    thunkAddress = j9ThunkLookupSignature(jitConfig, J9UTF8_LENGTH(J9ROMNAMEANDSIGNATURE_SIGNATURE(nameAndSignature)),
        (char*)&J9UTF8_DATA((J9ROMNAMEANDSIGNATURE_SIGNATURE(nameAndSignature))));
...
j9ThunkTableAllocate(J9JavaVM* vm)
{
    ...
    jitConfig->thunkHashTable = hashTableNew(OMRPORT_FROM_J9PORT(vm->portLibrary), /* portLibrary */
...
j9ThunkNewNameAndSig(void* jitConfig, void* parm, void* thunkAddress)
{
    ...
    return j9ThunkNewSignature(jitConfig, J9UTF8_LENGTH(J9ROMNAMEANDSIGNATURE_SIGNATURE(nameAndSignature)),
        (char*)&J9UTF8_DATA((J9ROMNAMEANDSIGNATURE_SIGNATURE(nameAndSignature))), thunkAddress);
}
...
void* j9ThunkVMHelperFromNameAndSig(void* jitConfig, void* parm)
{
    J9ROMNameAndSignature* nameAndSignature = (J9ROMNameAndSignature*)parm;

    return j9ThunkVMHelperFromSignature(jitConfig, J9UTF8_LENGTH(J9ROMNAMEANDSIGNATURE_SIGNATURE(nameAndSignature)),
        (char*)&J9UTF8_DATA((J9ROMNAMEANDSIGNATURE_SIGNATURE(nameAndSignature))));
}
