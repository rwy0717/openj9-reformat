...
int32_t TR::ARMJNILinkage::buildArgs(
...
void TR::ARMJNILinkage::buildVirtualDispatch(TR::Node* callNode, TR::RegisterDependencyConditions* dependencies,
...
TR::ARMLinkageProperties& TR::ARMJNILinkage::getProperties() { return _properties; }
...
TR::MemoryReference* TR::ARMJNILinkage::getOutgoingArgumentMemRef(
...
int32_t TR::ARMJNILinkage::buildJNIArgs(TR::Node* callNode, TR::RegisterDependencyConditions* dependencies,
    TR::Register*& vftReg, bool passReceiver, bool passEnvArg)
{
        ...
        case TR::Float:
            ...
            if (numIntegerArgRegIndex < numIntArgRegs) {
                if ((childType != TR::Address) && !cg()->canClobberNodesRegister(child, 0)) {
                    /* If the reg is shared by another node, we copy it to tempReg, so that the reg is not reused after
                     * returning the call. */
            ...
            if (numIntegerArgRegIndex < numIntArgRegs) {
                if (!cg()->canClobberNodesRegister(child, 0)) {
                    ...
                    if (bigEndian) {
                        generateTrg1Src1Instruction(
                            codeGen, ARMOp_mov, child, tempReg, reg->getRegisterPair()->getHighOrder());
                        reg = codeGen->allocateRegisterPair(reg->getRegisterPair()->getLowOrder(), tempReg);
                    } else {
                        generateTrg1Src1Instruction(
                            codeGen, ARMOp_mov, child, tempReg, reg->getRegisterPair()->getLowOrder());
                ...
                if ((numIntegerArgRegIndex == 0) && resType.isAddress())
                    dependencies->addPostCondition(
                        codeGen->allocateCollectedReferenceRegister(), TR::RealRegister::gr0);
                else
...
#ifdef DEBUG_ARM_LINKAGE
                    printf("pushing %s word of 64-bit arg %d %d\n", bigEndian ? "low" : "high", numIntegerArgRegIndex,
                        memArg);
                    fflush(stdout);
#endif
        ...
        case TR::Float:
            ...
            if ((numFloatArgRegIndex < numFloatArgRegs)
                ...
                if ((numFloatArgRegIndex == 0) && resType.isFloatingPoint()) {
                    ...
                    if (numSingleArgRegIndex == 0) {
                        dependencies->addPostCondition(
                            reg, jniLinkageProperties.getFloatArgumentRegister(numFloatArgRegIndex));
                    }
                }
        ...
        case TR::Double:
            ...
            if (numFloatArgRegIndex < numFloatArgRegs) {
                ...
                if ((numFloatArgRegIndex == 0) && resType.isFloatingPoint()) {
                    ...
                    addDependency(dependencies, reg, jniLinkageProperties.getFloatArgumentRegister(numFloatArgRegIndex),
                        TR_FPR, codeGen);
            } else {
                if (isEABI)
                    stackOffset = (stackOffset + 4) & (~7);
    ...
    for (i = 0; i < numMemArgs; i++) {
        ...
        generateMemSrc1Instruction(
            codeGen, pushToMemory[i].opCode, callNode, pushToMemory[i].argMemory, pushToMemory[i].argRegister);
...
TR::Register* TR::ARMJNILinkage::buildDirectDispatch(TR::Node* callNode)
{
    ...
    for (i = 0; i < jniLinkageProperties.getNumFloatArgRegs() * 2; i++) {
        spillDeps->addPostCondition(
            codeGen->allocateRegister(TR_FPR), (TR::RealRegister::RegNum)((uint32_t)TR::RealRegister::fs0 + i));
    }
    ...
    generateTrg1Src1Instruction(
        codeGen, ARMOp_mov, callNode, gr4Reg, new (trHeapMemory()) TR_ARMOperand2(flags.getByte3(), 24));
    generateTrg1Src2Instruction(
        codeGen, ARMOp_add, callNode, gr4Reg, gr4Reg, new (trHeapMemory()) TR_ARMOperand2(flags.getByte2(), 16));
    ...
    generateTrg1Src1Instruction(
        codeGen, ARMOp_mov, callNode, gr4Reg, new (trHeapMemory()) TR_ARMOperand2(ramMethod.getByte3(), 24));
    generateTrg1Src2Instruction(
        codeGen, ARMOp_add, callNode, gr4Reg, gr4Reg, new (trHeapMemory()) TR_ARMOperand2(ramMethod.getByte2(), 16));
    generateTrg1Src2Instruction(
        codeGen, ARMOp_add, callNode, gr4Reg, gr4Reg, new (trHeapMemory()) TR_ARMOperand2(ramMethod.getByte1(), 8));
    generateTrg1Src2Instruction(
        codeGen, ARMOp_add, callNode, gr4Reg, gr4Reg, new (trHeapMemory()) TR_ARMOperand2(ramMethod.getByte0(), 0));
    tempMR = new (trHeapMemory())
        TR::MemoryReference(stackPtr, -((int)TR::Compiler->om.sizeofReferenceAddress()), codeGen);
    tempMR->setImmediatePreIndexed();
    generateMemSrc1Instruction(codeGen, ARMOp_str, callNode, tempMR, gr4Reg);
    ...
    generateTrg1Src1Instruction(
        codeGen, ARMOp_mov, callNode, gr4Reg, new (trHeapMemory()) TR_ARMOperand2(frameType.getByte1(), 8));
    generateTrg1Src2Instruction(
        codeGen, ARMOp_add, callNode, gr4Reg, gr4Reg, new (trHeapMemory()) TR_ARMOperand2(frameType.getByte0(), 0));
    ...
    TR::Instruction* gcPoint = generateImmSymInstruction(
        codeGen, ARMOp_bl, callNode, (uint32_t)helperSymRef->getMethodAddress(), NULL, helperSymRef);
    ...
    if (codeGen->hasHardFloatReturn()) {
        ...
        case TR::Double: {
            ...
            bool bigEndian = TR::Compiler->target.cpu.isBigEndian();
            generateTrg1MemInstruction(codeGen, ARMOp_ldr, callNode,
    ...
    helperSymRef
        = codeGen->symRefTab()->findOrCreateRuntimeHelper(TR_ARMjitCollapseJNIReferenceFrame, false, false, false);
    // AOT relocation is handled in TR::ARMImmSymInstruction::generateBinaryEncoding()
    generateImmSymInstruction(codeGen, ARMOp_bl, callNode, (uint32_t)helperSymRef->getMethodAddress(), NULL,
    ...
    // AOT relocation is handled in TR::ARMImmSymInstruction::generateBinaryEncoding()
    gcPoint = generateImmSymInstruction(codeGen, ARMOp_bl, callNode, (uint32_t)helperSymRef->getMethodAddress(), NULL,
        helperSymRef, NULL, NULL, ARMConditionCodeNE);
    gcPoint->ARMNeedsGCMap(1 << (jniLinkageProperties.getIntegerReturnRegister() - TR::RealRegister::FirstGPR));
