...
TR::ARMLinkageProperties& TR::ARMPrivateLinkage::getProperties() { return properties; }
...
uint32_t TR::ARMPrivateLinkage::getRightToLeft() { return getProperties().getRightToLeft(); }
...
void TR::ARMPrivateLinkage::createPrologue(TR::Instruction* cursor)
{
    ...
    TR_ASSERT(
        offsetToSavedRegisters < 65536, "offset to saved registers from base of stack frame must be less than 2^16\n");
    ...
    if (constantIsImmed8r(totalFrameSize, &base, &rotate)) {
        cursor
            = generateTrg1Src1ImmInstruction(codeGen, ARMOp_sub, firstNode, stackPtr, stackPtr, base, rotate, cursor);
    } else {
    ...
    if (intRegistersSaved) {
        if (1 == intRegistersSaved) {
            ...
            cursor = new (trHeapMemory()) TR::ARMMultipleMoveInstruction(
                cursor, ARMOp_stm, firstNode, gr4, getAssignedRegisterList(machine, &linkage), codeGen);
        }
    }
        ...
        if (1 == fpRegistersSaved) {
            ...
            if (constantIsImmed8r(outgoingArgSize, &base, &rotate)) {
                cursor = generateTrg1Src1ImmInstruction(
                    codeGen, ARMOp_add, firstNode, gr4, stackPtr, base, rotate, cursor);
            } else {
                cursor = armLoadConstant(firstNode, outgoingArgSize, gr4, codeGen, cursor);
                cursor = generateTrg1Src2Instruction(codeGen, ARMOp_add, firstNode, gr4, stackPtr, gr4, cursor);
            }
            ...
            cursor = new (trHeapMemory())
                TR::ARMMultipleMoveInstruction(cursor, ARMOp_fstmd, firstNode, gr4, offset, codeGen);
            ...
            traceMsg("%d locals to be initialized in %s\n",
                ...
                if (numLocalsToBeInitialized > 8) {
                    ...
                    cursor = generateConditionalBranchInstruction(
                        codeGen, firstNode, ARMConditionCodeGE, loopLabel, cursor);
                } else {
...
void TR::ARMPrivateLinkage::createEpilogue(TR::Instruction* cursor)
{
    ...
    if (intRegistersSaved) {
        if (1 == intRegistersSaved) {
            ...
            cursor = new (trHeapMemory()) TR::ARMMultipleMoveInstruction(
                cursor, ARMOp_ldm, lastNode, gr4, getAssignedRegisterList(machine, &linkage), codeGen);
        }
    }
    ...
    if (fpRegistersSaved > 0) {
        ...
        if (1 == fpRegistersSaved) {
            ...
            if (constantIsImmed8r(outgoingArgSize, &base, &rotate)) {
                cursor
                    = generateTrg1Src1ImmInstruction(codeGen, ARMOp_add, lastNode, gr4, stackPtr, base, rotate, cursor);
            } else {
                cursor = armLoadConstant(lastNode, outgoingArgSize, gr4, codeGen, cursor);
                cursor = generateTrg1Src2Instruction(codeGen, ARMOp_add, lastNode, gr4, stackPtr, gr4, cursor);
            }
            ...
            uint16_t offset = ((TR::RealRegister::fp8 - TR::RealRegister::FirstFPR) << 12) | (fpRegistersSaved << 1);
            cursor = new (trHeapMemory())
                TR::ARMMultipleMoveInstruction(cursor, ARMOp_fldmd, lastNode, gr4, offset, codeGen);
            ((TR::ARMMultipleMoveInstruction*)cursor)->setIncrement();
        }
    }
...
TR::MemoryReference* TR::ARMPrivateLinkage::getOutgoingArgumentMemRef(
...
int32_t TR::ARMPrivateLinkage::buildArgs(
...
void TR::ARMPrivateLinkage::buildVirtualDispatch(TR::Node* callNode, TR::RegisterDependencyConditions* dependencies,
    TR::RegisterDependencyConditions* postDeps, TR::Register* vftReg, uint32_t sizeOfArguments)
{
        ...
        default:
            if (fej9->needsInvokeExactJ2IThunk(callNode, comp())) {
                comp()->getPersistentInfo()->getInvokeExactJ2IThunkTable()->addThunk(
                    TR::ARMCallSnippet::generateInvokeExactJ2IThunk(
                        callNode, sizeOfArguments, codeGen, methodSymbol->getMethod()->signatureChars()),
                    fej9);
            }
            break;
        }
    ...
    uint8_t* thunk = (uint8_t*)fej9->getJ2IThunk(
        methodSymbol->getMethod()->signatureChars(), methodSymbol->getMethod()->signatureLength(), comp());
    if (!thunk)
    ...
    if (methodSymbol->isVirtual()) {
        ...
        if (methodSymRef
            ...
            if (TR::Compiler->cls.classesOnHeap()) {
                classReg = gr11;
                generateTrg1MemInstruction(codeGen, ARMOp_ldr, callNode, gr11,
                    new (trHeapMemory())
                        TR::MemoryReference(gr0, fej9->getOffsetOfClassFromJavaLangClassField(), codeGen));
            } else
                classReg = gr0;
        } else {
            classReg = vftReg;
        ...
        if (methodSymRef->isUnresolved() || comp()->compileRelocatableCode()) {
            ...
            TR::LabelSymbol* snippetLabel = generateLabelSymbol(codeGen);
            codeGen->addSnippet(new (trHeapMemory()) TR::ARMVirtualUnresolvedSnippet(
                codeGen, callNode, snippetLabel, sizeOfArguments, doneLabel, thunk));
...
int32_t TR::ARMHelperLinkage::buildArgs(
    TR::Node* callNode, TR::RegisterDependencyConditions* dependencies, TR::Register*& vftReg, bool isVirtual)
{
