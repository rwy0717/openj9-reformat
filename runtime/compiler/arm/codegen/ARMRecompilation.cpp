...
TR::Instruction* TR_ARMRecompilation::generatePrePrologue()
{
    ...
    TR::SymbolReference* recompileMethodSymRef
        = cg()->symRefTab()->findOrCreateRuntimeHelper(TR_ARMsamplingRecompileMethod, false, false, false);
    ...
    if (couldBeCompiledAgain()) {
        ...
        if (useSampling()) {
            ...
            cursor = generateImmSymInstruction(cg(), ARMOp_bl, firstNode,
        ...
        cursor = new (cg()->trHeapMemory())
            TR::ARMImmInstruction(cursor, ARMOp_dd, firstNode, (int32_t)(intptrj_t)info, cg());
...
TR::Instruction* TR_ARMRecompilation::generatePrologue(TR::Instruction* cursor)
{
    if (couldBeCompiledAgain()) {
        ...
        cursor = generateTrg1MemInstruction(
            cg(), ARMOp_ldr, firstNode, gr4, new (cg()->trHeapMemory()) TR::MemoryReference(gr5, 0, cg()), cursor);
