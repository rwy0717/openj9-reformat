    ...
    for (int i = argStart; i < callNode->getNumChildren(); i++) {
        ...
        case TR::Float:
            ...
            if (intArgNum < linkage.getNumIntArgRegs()) {
                buffer = storeArgumentItem(ARMOp_str, buffer,
        ...
        case TR::Float:
            /* TODO
               ...
               {
               buffer = storeArgumentItem(ARMOp_stfs, buffer,
        ...
        case TR::Double:
            /* TODO
               ...
               if (floatArgNum < linkage.getNumFloatArgRegs())
               {
               buffer = storeArgumentItem(ARMOp_stfd, buffer,
               machine->getRealRegister(linkage.getFloatArgumentRegister(floatArgNum)), offset, cg);
               }
               floatArgNum++;
               if (linkage.getRightToLeft())
...
TR_RuntimeHelper TR::ARMCallSnippet::getHelper()
{
    ...
    default:
        TR_ASSERT(
            false, "Bad return data type for a call node.  DataType was %s\n", cg()->getDebug()->getName(dataType));
...
uint8_t* TR::ARMVirtualUnresolvedSnippet::emitSnippetBody()
{
    ...
    TR::SymbolReference* methodSymRef = getNode()->getSymbolReference();
    TR::SymbolReference* glueRef
        = cg()->symRefTab()->findOrCreateRuntimeHelper(TR_ARMvirtualUnresolvedHelper, false, false, false);
...
uint8_t* TR::ARMInterfaceCallSnippet::emitSnippetBody()
{
    ...
    TR::SymbolReference* methodSymRef = getNode()->getSymbolReference();
    TR::SymbolReference* glueRef
        = cg()->symRefTab()->findOrCreateRuntimeHelper(TR_ARMinterfaceCallHelper, false, false, false);
...
TR_J2IThunk* TR::ARMCallSnippet::generateInvokeExactJ2IThunk(
