...
static inline TR::Instruction* genNullTest(
    TR::CodeGenerator* cg, TR::Node* node, TR::Register* objectReg, TR::Register* scratchReg, TR::Instruction* cursor)
{
...
TR::Instruction* OMR::ARM::TreeEvaluator::generateVFTMaskInstruction(
    TR::CodeGenerator* cg, TR::Node* node, TR::Register* dstReg, TR::Register* srcReg, TR::Instruction* prev)
{
...
TR::Instruction* OMR::ARM::TreeEvaluator::generateVFTMaskInstruction(
    TR::CodeGenerator* cg, TR::Node* node, TR::Register* reg, TR::Instruction* prev)
{
    ...
    TR::LabelSymbol* failLabel, TR::Instruction* cursor, TR::LabelSymbol* doneLabel)
{
    ...
    if (failLabel) {
        ...
        cursor = generateImmSymInstruction(
            cg, ARMOp_bl, node, (uintptr_t)symRef->getMethodAddress(), NULL, symRef, NULL, NULL, ARMConditionCodeLE);
...
TR::Register* OMR::ARM::TreeEvaluator::VMcheckcastEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!testEqualClass && !testCastClassIsSuper) {
        ...
        TR::Instruction* gcPoint
            = generateImmSymInstruction(cg, ARMOp_bl, node, (uint32_t)symRef->getMethodAddress(), deps, symRef);
    ...
    if (testEqualClass) {
        ...
        if (testCastClassIsSuper)
            ...
            gcPoint = generateImmSymInstruction(cg, ARMOp_bl, node, (uintptr_t)symRef->getMethodAddress(), NULL, symRef,
                NULL, NULL, ARMConditionCodeNE);
            gcPoint->ARMNeedsGCMap(0xFFFFFFFF);
        }
    }
    ...
    if (testCastClassIsSuper) {
        ...
        genTestIsSuper(
            cg, node, objClassReg, castClassReg, scratch1Reg, scratch2Reg, castClassDepth, NULL, NULL, doneLabel);
        gcPoint = generateImmSymInstruction(
            cg, ARMOp_bl, node, (uintptr_t)symRef->getMethodAddress(), NULL, symRef, NULL, NULL, ARMConditionCodeNE);
        gcPoint->ARMNeedsGCMap(0xFFFFFFFF);
    }
...
TR::Register* OMR::ARM::TreeEvaluator::VMifInstanceOfEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (testCastClassIsSuper) {
        ...
        iCursor = genTestIsSuper(
            cg, node, objClassReg, castClassReg, scratch1Reg, scratch2Reg, castClassDepth, res0Label, iCursor, NULL);
        iCursor = generateConditionalBranchInstruction(cg, node, ARMConditionCodeNE, res0Label, iCursor);
        if (trueLabel == NULL) {
            if (needResult) {
                iCursor = generateTrg1ImmInstruction(cg, ARMOp_mov, node, resultReg, 1, 0, iCursor);
...
TR::Register* OMR::ARM::TreeEvaluator::VMinstanceOfEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (testCastClassIsSuper) {
        ...
        iCursor = genTestIsSuper(
            cg, node, objClassReg, castClassReg, scratch1Reg, scratch2Reg, castClassDepth, doneLabel, iCursor, NULL);
...
TR::Register* OMR::ARM::TreeEvaluator::VMmonexitEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.isSMP() && TR::Compiler->target.cpu.id() != TR_DefaultARMProcessor) {
        // instr = generateInstruction(cg, (TR::Compiler->target.cpu.id() == TR_ARMv6) ? ARMOp_dmb_v6 : ARMOp_dmb,
        // node);
    ...
    if (sizeInReg) {
        if (isVariableLen) {
            ...
            if (elementSize >= 4) {
                ...
                if (elementSize == 2) {
                    iCursor
                        = generateTrg1Src2Instruction(cg, ARMOp_add, node, dataSizeReg, dataSizeReg, enumReg, iCursor);
                }
            ...
            if ((round != 0 && round != 4) || !headerAligned) {
                iCursor = generateTrg1Src1ImmInstruction(
                    cg, ARMOp_add, node, sizeReg, dataSizeReg, allocSize + round - 1, 0, iCursor);
                iCursor = generateTrg1Src1ImmInstruction(cg, ARMOp_bic, node, sizeReg, sizeReg, round - 1, 0, iCursor);
            } else
                iCursor
                    = generateTrg1Src1ImmInstruction(cg, ARMOp_add, node, sizeReg, dataSizeReg, allocSize, 0, iCursor);
        } else
    ...
    TR::Register* classReg, TR::Register* resReg, TR::Register* zeroReg, TR::Register* temp1Reg, TR::Register* temp2Reg)
{
    ...
    if ((node->getOpCodeValue() != TR::New) && (lwOffset > 0))
        initMonitor = true;
#endif
    if (initMonitor) {
...
TR::Register* OMR::ARM::TreeEvaluator::VMnewEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (isArray) {
        ...
        if (isDoubleArray)
            genAlignDoubleArray(
                cg, node, iCursor, isVariableLen, resReg, objectSize, dataBegin, dataSizeReg, temp1Reg, temp2Reg);
        genInitArrayHeader(
            cg, node, iCursor, isVariableLen, clazz, resReg, zeroReg, enumReg, dataSizeReg, temp1Reg, temp2Reg);
    } else {
    ...
    if (!node->canSkipZeroInitialization() && (initInfo == NULL || initInfo->numZeroInitSlots > 0)) {
        ...
        } else // Init variable length array
        {
            ...
            iCursor = generateTrg1Src1Instruction(cg, ARMOp_mov_r, node, temp1Reg,
    ...
    iCursor = generateImmSymInstruction(
        cg, ARMOp_bl, node, (uint32_t)node->getSymbolReference()->getMethodAddress(), deps, node->getSymbolReference());
...
TR::Register* OMR::ARM::TreeEvaluator::VMmonentEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    generateTrg1MemInstruction(
        cg, ARMOp_ldrex, node, dataReg, new (cg->trHeapMemory()) TR::MemoryReference(addrReg, (int32_t)0, cg));
    ...
    generateTrg1MemSrc1Instruction(
        cg, ARMOp_strex, node, tempReg, new (cg->trHeapMemory()) TR::MemoryReference(addrReg, (int32_t)0, cg), metaReg);
    ...
    doneLabel->setEndInternalControlFlow();
#else
    generateTrg1MemInstruction(cg, ARMOp_ldr, node, addrReg,
        new (cg->trHeapMemory()) TR::MemoryReference(metaReg, offsetof(J9VMThread, objectFlagSpinLockAddress), cg));
    ...
    generateTrg1MemInstruction(
        cg, ARMOp_ldr, node, dataReg, new (cg->trHeapMemory()) TR::MemoryReference(objReg, lwOffset, cg));
    generateSrc1ImmInstruction(cg, ARMOp_cmp, node, dataReg, 0, 0);

    iCursor = generateMemSrc1Instruction(
        cg, ARMOp_str, node, new (cg->trHeapMemory()) TR::MemoryReference(objReg, lwOffset, cg), metaReg);
    iCursor->setConditionCode(ARMConditionCodeEQ);
    ...
    generateMemSrc1Instruction(
        cg, ARMOp_str, node, new (cg->trHeapMemory()) TR::MemoryReference(addrReg, (int32_t)0, cg), dataReg);
    generateLabelInstruction(cg, ARMOp_label, node, callLabel);
    ...
    TR::Register* flagReg, bool needDeps, TR::CodeGenerator* cg)
{
    ...
    if (gcMode != TR_WrtbarAlways) {
        ...
        // @@      if (flagReg == NULL)
        // @@         generateTrg1MemInstruction(cg, ARMOp_ldr, node, tempReg, new (cg->trHeapMemory())
        // TR::MemoryReference(dstReg, fej9->getWordOffsetToGCFlags(), cg));
        // @@      generateTrg1Src2Instruction(cg, ARMOp_and_r, node, tempReg, tempReg, new (cg->trHeapMemory())
        // TR_ARMOperand2(fej9->getWriteBarrierGCFlagMaskAsByte(), 8));
    }

    TR::Instruction* cursor = generateImmSymInstruction(cg, ARMOp_bl, node, (uintptr_t)wbref->getMethodAddress(),
