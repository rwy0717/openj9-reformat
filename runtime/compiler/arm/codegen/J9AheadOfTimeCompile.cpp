    ...
    *flagsCursor = modifier;
    ...
    case TR_MethodObject: {
        ...
        uintptr_t inlinedSiteIndex
            = self()->findCorrectInlinedSiteIndex(tempSR->getOwningMethod(comp)->constantPool(), recordInfo->data2);
        ...
        *(uintptrj_t*)cursor = (uintptr_t)tempSR->getOwningMethod(TR::comp())->constantPool();
        cursor += SIZEPOINTER;
    } break;
    ...
    case TR_FixedSequenceAddress2: {
        ...
        *(uint32_t*)cursor = relocation->getTargetAddress()
            ? (uint32_t)((uint8_t*)relocation->getTargetAddress() - aotMethodCodeStart)
            : 0x0;
        cursor += 4;
    } break;
