...
uint8_t* J9::ARM::UnresolvedDataSnippet::emitSnippetBody()
{
    ...
    *(int32_t*)cursor
        = encodeHelperBranchAndLink(cg()->symRefTab()->findOrCreateRuntimeHelper(getHelper(), false, false, false),
            cursor, getNode(), cg()); // BL resolve
    cursor += 4;
...
uint32_t J9::ARM::UnresolvedDataSnippet::getLength(int32_t estimatedSnippetStart)
{
    // *this   swipeable for debugger
    return 6 * 4;
}
