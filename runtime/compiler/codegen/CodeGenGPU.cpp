...
static_assert(sizeof(nvvmOpCodeNames) == (TR::NumIlOps * sizeof(char*)),
...
void J9::CodeGenerator::printArrayCopyNVVMIR(TR::Node* node, NVVMIRBuffer& ir, TR::Compilation* comp)
{
    ...
    */
    ...
    ir.print("  %%%d = load %s %%%d\n", node->getLocalIndex(),
        isWordCopy ? "i32*"
                   : isHalfwordCopy ? "i16*" : getTypeName(node->getChild(0 + childrenNodeOffset)->getDataType()),
        node->getLocalIndex() - 3);
    node->setLocalIndex(_gpuNodeCount++);
    ...
    ir.print("  store %s %%%d, %s %%%d\n", isWordCopy ? "i32" : isHalfwordCopy ? "i16" : "i8",
        node->getLocalIndex() - 1,
        isWordCopy ? "i32*"
                   : isHalfwordCopy ? "i16*" : getTypeName(node->getChild(1 + childrenNodeOffset)->getDataType()),
        node->getLocalIndex() - 3);
    ...
    ir.print("  %%%d = %s i64 %%%d, %d\n", node->getLocalIndex(), isBackwardsCopy ? "sub" : "add",
        node->getLocalIndex() - 2, isWordCopy ? 4 : isHalfwordCopy ? 2 : 1);
    node->setLocalIndex(_gpuNodeCount++);
    ...
    ir.print("  %%%d = sub %s %%%d, %d\n", node->getLocalIndex(),
        getTypeName(node->getChild(2 + childrenNodeOffset)->getDataType()),
        unknownCopy ? node->getLocalIndex() - 11 : node->getLocalIndex() - 13, isWordCopy ? 4 : isHalfwordCopy ? 2 : 1);
    ...
    TR::Node*& errorNode)
{
    ...
    if (opcode.isNullCheck()) {
        ...
        if (!isSMReference) {
            result = self()->printNVVMIR(
                ir, node->getChild(0), loop, targetBlocks, visitCount, sharedMemory, nextParmNum, errorNode);
            if (result != GPUSuccess)
                return result;
        }
        result = self()->printNVVMIR(
            ir, node->getChild(1), loop, targetBlocks, visitCount, sharedMemory, nextParmNum, errorNode);
        if (result != GPUSuccess)
            return result;
        ...
        result = self()->printNVVMIR(
            ir, idivNode->getChild(0), loop, targetBlocks, visitCount, sharedMemory, nextParmNum, errorNode);
        if (result != GPUSuccess)
            return result;

        result = self()->printNVVMIR(
            ir, idivNode->getChild(1), loop, targetBlocks, visitCount, sharedMemory, nextParmNum, errorNode);
        if (result != GPUSuccess)
            return result;
...
void J9::CodeGenerator::findExtraParms(
    TR::Node* node, int32_t& numExtraParms, TR_SharedMemoryAnnotations* sharedMemory, vcount_t visitCount)
{
    if (node->getVisitCount() == visitCount)
        return;
...
void J9::CodeGenerator::dumpInvariant(
    CS2::ArrayOf<gpuParameter, TR::Allocator>::Cursor pit, NVVMIRBuffer& ir, bool isbufferalign)
{
    return;
    ...
    bool* hasExceptionChecks)
{
    ...
    for (TR::TreeTop* tree = firstTreeTop; tree != lastTreeTop; tree = tree->getNextTreeTop()) {
        ...
        if (node->getOpCode().isBranch()) {
            ...
            if (tree->getNextTreeTop() && tree->getNextTreeTop()->getNextTreeTop()) {
                ...
                TR_ASSERT(
                    node->getOpCodeValue() == TR::BBStart, "Attempted to get Block number of a non-BBStart node.");
            ...
            for (int childIndex = 0; childIndex < node->getNumChildren(); ++childIndex) {
                if (node->getChild(childIndex)->getOpCode().isBranch()) {
                    TR_ASSERT(
                        node->getChild(childIndex)->getBranchDestination()->getNode()->getOpCodeValue() == TR::BBStart,
                        "Attempted to get Block number of a non-BBStart node.");
                    targetBlocks.set(
    ...
    for (TR::TreeTop* tree = firstTreeTop; tree != lastTreeTop; tree = tree->getNextTreeTop()) {
        ...
        result = self()->printNVVMIR(
            ir, tree->getNode(), loop, &targetBlocks, visitCount, &sharedMemory, nextParmNum, errorNode);
        if (result != GPUSuccess) {
            return result;
        }
    }
...
void J9::CodeGenerator::generateGPU()
{
    if (self()->comp()->isGPUCompilation()) {
        ...
        TR::SymbolReference* helper
            = self()->comp()->getSymRefTab()->findOrCreateRuntimeHelper(TR_callGPU, false, false, false);
...
uintptrj_t J9::CodeGenerator::objectLengthOffset() { return self()->fe()->getOffsetOfContiguousArraySizeField(); }

uintptrj_t J9::CodeGenerator::objectHeaderInvariant() { return self()->objectLengthOffset() + 4 /*length*/; }
