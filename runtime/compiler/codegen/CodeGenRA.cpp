...
void J9::CodeGenerator::pendingFreeVariableSizeSymRef(TR::SymbolReference* symRef)
{
    ...
    if (self()->traceBCDCodeGen())
        traceMsg(self()->comp(), "\tpendingFreeVariableSizeSymRef: #%d (%s) %s to pending free list\n",
            symRef->getReferenceNumber(), self()->getDebug()->getName(symRef->getSymbol()),
            found ? "do not add (already present)" : "add");
    if (!found)
    ...
    if (biggestSymRef) {
        ...
        while (i != _variableSizeSymRefFreeList.end()) {
            ...
            if ((*i)->getSymbol()->getSize() >= byteLength) {
                if (self()->traceBCDCodeGen()) {
                    traceMsg(self()->comp(),
                        "\t\tfound big enough free symRef #%d (%s) with length >= req length of %d\n",
