...
uint8_t* J9::AheadOfTimeCompile::emitClassChainOffset(uint8_t* cursor, TR_OpaqueClassBlock* classToRemember)
{
    ...
    uintptrj_t classChainForInlinedMethodOffsetInSharedCache
        = reinterpret_cast<uintptrj_t>(sharedCache->offsetInSharedCacheFromPointer(classChainForInlinedMethod));
...
uint8_t* J9::AheadOfTimeCompile::initializeCommonAOTRelocationHeader(
...
uint8_t* J9::AheadOfTimeCompile::dumpRelocationHeaderData(uint8_t* cursor, bool isVerbose)
{
    ...
    TR_RelocationRecord* reloRecord = TR_RelocationRecord::create(
        &storage, reloRuntime, reloTarget, reinterpret_cast<TR_RelocationRecordBinaryTemplate*>(cursor));
...
void J9::AheadOfTimeCompile::dumpRelocationData()
{
    ...
    if (!self()->comp()->getOption(TR_TraceRelocatableDataCG)
        && !self()->comp()->getOption(TR_TraceRelocatableDataDetailsCG) && !self()->comp()->getOption(TR_TraceReloCG)) {
        return;
    }
    ...
    if (self()->comp()->getOption(TR_TraceReloCG)) {
        ...
        if (!self()->comp()->getOption(TR_TraceRelocatableDataCG)
            && !self()->comp()->getOption(TR_TraceRelocatableDataDetailsCG)) {
    ...
    if (isVerbose) {
        traceMsg(
            self()->comp(), "Size of relocation data in AOT object is %d bytes\n", self()->getSizeOfAOTRelocations());
    }
    ...
    while (cursor < endOfData) {
        ...
        case TR_ConstantPoolOrderedPair:
            ...
            if (is64BitTarget) {
                ...
                if (isVerbose) {
                    traceMsg(self()->comp(), "\nInlined site index = %d, Constant pool = %x", *(uint32_t*)ep1,
                        *(uint64_t*)ep2);
                }
            } else {
                ep1 = cursor;
        ...
        case TR_AbsoluteHelperAddress:
            ...
            if (isVerbose) {
                ...
                traceMsg(self()->comp(), "\nHelper method address of %s(%d)", self()->getDebug()->getName(tempSR),
                    *(int32_t*)ep1);
            }
            break;
        ...
        case TR_MethodObject:
            ...
            if (is64BitTarget) {
                ...
                self()->traceRelocationOffsets(cursor, offsetSize, endOfCurrentRecord, orderedPair);
                if (isVerbose) {
                    traceMsg(self()->comp(), "\nMethod Object: Constant pool = %x, index = %d", *(uint64_t*)ep1,
                        *(uint64_t*)ep2);
                }
            } else {
                ep1 = cursor;
                ...
                self()->traceRelocationOffsets(cursor, offsetSize, endOfCurrentRecord, orderedPair);
                if (isVerbose) {
                    traceMsg(self()->comp(), "\nMethod Object: Constant pool = %x, index = %d", *(uint32_t*)ep1,
                        *(uint32_t*)ep2);
                }
            }
            break;
        ...
        case TR_J2IVirtualThunkPointer:
            ...
            if (isVerbose) {
                traceMsg(self()->comp(),
                    ...
                    (int64_t) * (uintptrj_t*)ep1, (uint64_t) * (uintptrj_t*)ep2, (uint64_t) * (uintptrj_t*)ep3);
            }
            break;
        case TR_Trampolines:
            // constant pool address is placed as the last word of the header
        ...
        case TR_VerifyClassObjectForAlloc: {
            ...
            if (isVerbose) {
                if (is64BitTarget) {
                    traceMsg(self()->comp(),
                        "\nVerify Class Object for Allocation: InlineCallSite index = %d, Constant pool = %x, index = "
                        "%d, binaryEncode = %x, size = %d",
            ...
            if (is64BitTarget) {
                ...
                if (isVerbose)
                    traceMsg(self()->comp(),
                        ...
                        "classChainForInlinedMethod %p, vTableSlot %d",
                        *(uint64_t*)ep1, *(uint64_t*)ep2, *(uint64_t*)ep3, *(uint64_t*)ep4, *(uint64_t*)ep5,
                        *(uint64_t*)ep6, *(uint64_t*)ep7);
            } else {
                ep1 = cursor; // inlinedSiteIndex
                ep2 = cursor + 4; // constantPool
                ep3 = cursor + 8; // cpIndex
                ...
                if (isVerbose) {
                    traceMsg(self()->comp(),
                        "\nInlined Interface Method: Inlined site index = %d, Constant pool = %x, cpIndex = %x, "
                        "romClassOffsetInSharedCache=%p, classChainIdentifyingLoaderOffsetInSharedCache=%p, "
                        "classChainForInlinedMethod %p, vTableSlot %d",
                        *(uint32_t*)ep1, *(uint32_t*)ep2, *(uint32_t*)ep3, *(uint32_t*)ep4, *(uint32_t*)ep5,
                        *(uint32_t*)ep6, *(uint32_t*)ep7);
                }
            }
            break;
        ...
        case TR_ValidateStaticField: {
            ...
            if (isVerbose) {
                if (is64BitTarget) {
                    traceMsg(self()->comp(),
                        "\nValidation Relocation: InlineCallSite index = %d, Constant pool = %x, cpIndex = %d, ROM "
                        "Class offset = %x",
                        *(uint64_t*)ep1, *(uint64_t*)ep2, *(uint64_t*)ep3, *(uint64_t*)ep4);
                } else {
                    traceMsg(self()->comp(),
        ...
        case TR_DebugCounter:
            ...
            if (is64BitTarget) {
                ...
                if (isVerbose) {
                    traceMsg(self()->comp(),
                        "\n Debug Counter: Inlined site index = %d, bcIndex = %d, offsetOfNameString = %p, delta = %d, "
                        "staticDelta = %d",
        ...
        case TR_ValidateClassByName: {
            ...
            if (isVerbose) {
                traceMsg(self()->comp(),
                    ...
                    (uint32_t)binaryTemplate->_classID, (uint32_t)binaryTemplate->_beholderID,
                    binaryTemplate->_primitiveType, binaryTemplate->_romClassOffsetInSCC);
            }
        ...
        case TR_ValidateProfiledClass: {
            ...
            if (isVerbose) {
                traceMsg(self()->comp(),
                    "\n Validate Profiled Class: classID=%d primitiveType=%c classChainOffsetInSCC=%d "
                    "classChainOffsetForCLInScc=%p ",
                    (uint32_t)binaryTemplate->_classID, binaryTemplate->_primitiveType,
                    binaryTemplate->_classChainOffsetInSCC, binaryTemplate->_classChainOffsetForCLInScc);
            }
        ...
        case TR_ValidateDefiningClassFromCP: {
            ...
            if (isVerbose) {
                traceMsg(self()->comp(),
                    ...
                    (uint32_t)binaryTemplate->_classID, (uint32_t)binaryTemplate->_beholderID, binaryTemplate->_cpIndex,
        ...
        case TR_ValidateClassFromMethod: {
            ...
            if (isVerbose) {
                traceMsg(self()->comp(), "\n Validate Class From Method: classID=%d, methodID=%d ",
                    (uint32_t)binaryTemplate->_classID, (uint32_t)binaryTemplate->_methodID);
            }
        ...
        case TR_ValidateComponentClassFromArrayClass: {
            ...
            if (isVerbose) {
                traceMsg(self()->comp(),
                    ...
                    (uint32_t)binaryTemplate->_componentClassID, (uint32_t)binaryTemplate->_arrayClassID);
            }
        ...
        case TR_ValidateArrayClassFromComponentClass: {
            ...
            if (isVerbose) {
                traceMsg(self()->comp(),
                    ...
                    (uint32_t)binaryTemplate->_arrayClassID, (uint32_t)binaryTemplate->_componentClassID);
            }
        ...
        case TR_ValidateSuperClassFromClass: {
            ...
            if (isVerbose) {
                traceMsg(self()->comp(), "\n Validate Super Class From Class: superClassID=%d, childClassID=%d ",
                    (uint32_t)binaryTemplate->_superClassID, (uint32_t)binaryTemplate->_childClassID);
            }
        ...
        case TR_ValidateClassInstanceOfClass: {
            ...
            if (isVerbose) {
                traceMsg(self()->comp(),
                    ...
                    (uint32_t)binaryTemplate->_classOneID, (uint32_t)binaryTemplate->_classTwoID,
        ...
        case TR_ValidateSystemClassByName: {
            ...
            if (isVerbose) {
                traceMsg(self()->comp(), "\n Validate System Class By Name: systemClassID=%d romClassOffsetInSCC=%p ",
                    (uint32_t)binaryTemplate->_systemClassID, binaryTemplate->_romClassOffsetInSCC);
            }
        ...
        case TR_ValidateClassClass: {
            ...
            if (isVerbose) {
                traceMsg(self()->comp(), "\n Validate Class Class: classClassID=%d, objectClassID=%d ",
                    (uint32_t)binaryTemplate->_classClassID, (uint32_t)binaryTemplate->_objectClassID);
            }
        ...
        case TR_ValidateConcreteSubClassFromClass: {
            ...
            if (isVerbose) {
                traceMsg(self()->comp(), "\n Validate Concrete SubClass From Class: childClassID=%d, superClassID=%d ",
                    (uint32_t)binaryTemplate->_childClassID, (uint32_t)binaryTemplate->_superClassID);
            }
        ...
        case TR_ValidateClassChain: {
            ...
            if (isVerbose) {
                traceMsg(self()->comp(), "\n Validate Class Chain: classID=%d classChainOffsetInSCC=%p ",
                    (uint32_t)binaryTemplate->_classID, binaryTemplate->_classChainOffsetInSCC);
            }
        ...
        case TR_ValidateMethodByName: {
            ...
            if (isVerbose) {
                traceMsg(self()->comp(),
                    "\n Validate Method By Name: methodID=%d, beholderID=%d, romClassOffsetInSCC=%p "
                    "romMethodOffsetInSCC=%p ",
                    (uint32_t)binaryTemplate->_methodID, (uint32_t)binaryTemplate->_beholderID,
                    binaryTemplate->_romClassOffsetInSCC, binaryTemplate->_romMethodOffsetInSCC);
            }
        ...
        case TR_ValidateMethodFromClass: {
            ...
            if (isVerbose) {
                traceMsg(self()->comp(), "\n Validate Method By Name: methodID=%d, beholderID=%d, index=%d ",
                    (uint32_t)binaryTemplate->_methodID, (uint32_t)binaryTemplate->_beholderID, binaryTemplate->_index);
            }
        ...
        case TR_ValidateVirtualMethodFromOffset: {
            ...
            if (isVerbose) {
                traceMsg(self()->comp(),
                    "\n Validate Virtual Method From Offset: methodID=%d, beholderID=%d, virtualCallOffset=%d, "
                    "ignoreRtResolve=%s ",
                    (uint32_t)binaryTemplate->_methodID, (uint32_t)binaryTemplate->_beholderID,
                    binaryTemplate->_virtualCallOffset, binaryTemplate->_ignoreRtResolve ? "true" : "false");
            }
        ...
        case TR_ValidateInterfaceMethodFromCP: {
            ...
            if (isVerbose) {
                traceMsg(self()->comp(),
                    ...
                    (uint32_t)binaryTemplate->_methodID, (uint32_t)binaryTemplate->_beholderID,
                    (uint32_t)binaryTemplate->_lookupID, binaryTemplate->_cpIndex);
            }
        ...
        case TR_ValidateMethodFromClassAndSig: {
            ...
            if (isVerbose) {
                traceMsg(self()->comp(),
                    "\n Validate Method From Class and Sig: methodID=%d, methodClassID=%d, beholderID=%d, "
                    "romMethodOffsetInSCC=%p ",
                    (uint32_t)binaryTemplate->_methodID, (uint32_t)binaryTemplate->_methodClassID,
                    (uint32_t)binaryTemplate->_beholderID, binaryTemplate->_romMethodOffsetInSCC);
            }
        ...
        case TR_ValidateStackWalkerMaySkipFramesRecord: {
            ...
            if (isVerbose) {
                traceMsg(self()->comp(),
                    "\n Validate Stack Walker May Skip Frames: methodID=%d, methodClassID=%d, beholderID=%d, "
                    "skipFrames=%s ",
                    (uint32_t)binaryTemplate->_methodID, (uint32_t)binaryTemplate->_methodClassID,
        ...
        case TR_ValidateClassInfoIsInitialized: {
            ...
            if (isVerbose) {
                traceMsg(self()->comp(), "\n Validate Class Info Is Initialized: classID=%d, isInitialized=%s ",
                    (uint32_t)binaryTemplate->_classID, binaryTemplate->_isInitialized ? "true" : "false");
            }
        ...
        case TR_ValidateMethodFromSingleImplementer: {
            ...
            if (isVerbose) {
                traceMsg(self()->comp(),
                    "\n Validate Method From Single Implementor: methodID=%d, thisClassID=%d, cpIndexOrVftSlot=%d, "
                    "callerMethodID=%d, useGetResolvedInterfaceMethod=%d ",
                    (uint32_t)binaryTemplate->_methodID, (uint32_t)binaryTemplate->_thisClassID,
                    binaryTemplate->_cpIndexOrVftSlot, (uint32_t)binaryTemplate->_callerMethodID,
        ...
        case TR_ValidateMethodFromSingleInterfaceImplementer: {
            ...
            if (isVerbose) {
                traceMsg(self()->comp(),
                    "\n Validate Method From Single Interface Implementor: methodID=%d, thisClassID=%d, cpIndex=%d, "
                    "callerMethodID=%d ",
                    (uint32_t)binaryTemplate->_methodID, (uint32_t)binaryTemplate->_thisClassID,
                    binaryTemplate->_cpIndex, (uint32_t)binaryTemplate->_callerMethodID);
            }
        ...
        case TR_ValidateMethodFromSingleAbstractImplementer: {
            ...
            if (isVerbose) {
                traceMsg(self()->comp(),
                    "\n Validate Method From Single Interface Implementor: methodID=%d, thisClassID=%d, vftSlot=%d, "
                    "callerMethodID=%d ",
                    (uint32_t)binaryTemplate->_methodID, (uint32_t)binaryTemplate->_thisClassID,
                    binaryTemplate->_vftSlot, (uint32_t)binaryTemplate->_callerMethodID);
            }
