...
void J9::CodeGenPhase::performInsertEpilogueYieldPointsPhase(TR::CodeGenerator* cg, TR::CodeGenPhase* phase)
{
    ...
    if ((comp->getCurrentMethod()->maxBytecodeIndex() >= BYTECODESIZE_THRESHOLD_FOR_ASYNCCHECKS)
        && !comp->mayHaveLoops() && comp->getCurrentMethod()->convertToMethod()->methodType() == TR_Method::J9
...
void J9::CodeGenPhase::performSplitWarmAndColdBlocksPhase(TR::CodeGenerator* cg, TR::CodeGenPhase*)
{
...
void J9::CodeGenPhase::performLateSequentialConstantStoreSimplificationPhase(
