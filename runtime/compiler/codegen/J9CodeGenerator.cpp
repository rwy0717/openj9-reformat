...
// J9
void J9::CodeGenerator::lowerCompressedRefs(
    TR::TreeTop* treeTop, TR::Node* node, vcount_t visitCount, TR_BitVector* childrenToBeLowered)
{
    bool isLowMem = false;
    ...
    if (loadOrStoreNode->getOpCode().isLoadIndirect() && shouldBeCompressed) {
        if (TR::Compiler->target.cpu.isZ() && TR::Compiler->om.shouldGenerateReadBarriersForFieldLoads()) {
            dumpOptDetails(self()->comp(),
                "compression sequence %p is not required for loads under concurrent scavenge on Z.\n", node);
            return;
        }

        // base object
        address = loadOrStoreNode->getFirstChild();
        loadOrStoreOp = TR::Compiler->om.shouldGenerateReadBarriersForFieldLoads()
...
void J9::CodeGenerator::lowerTreesPreChildrenVisit(TR::Node* parent, TR::TreeTop* treeTop, vcount_t visitCount)
{
    ...
    */
    ...
    if ((parent->getOpCodeValue() == TR::BNDCHKwithSpineCHK) || (parent->getOpCodeValue() == TR::SpineCHK)) {
        ...
        if (doIt) {
            ...
            while (i < numChildren) {
                ...
                TR::TreeTop* anchorTree = TR::TreeTop::create(
                    self()->comp(), TR::Node::create(TR::treetop, 1, childToBeAnchored), NULL, NULL);
    ...
    if (parent->getOpCode().isFunctionCall()) {
        // J9
        //
        // Hiding compressedref logic from CodeGen doesn't seem a good practise, the evaluator always need the
        // uncompressedref node for write barrier, therefore, this part is deprecated. It'll be removed once P and Z
        // update their corresponding evaluators.
        static bool UseOldCompareAndSwapObject = (bool)feGetEnv("TR_UseOldCompareAndSwapObject");
        if (self()->comp()->useCompressedPointers()
            && (UseOldCompareAndSwapObject || !TR::Compiler->target.cpu.isX86())) {
...
void J9::CodeGenerator::lowerTreeIfNeeded(TR::Node* node, int32_t childNumberOfNode, TR::Node* parent, TR::TreeTop* tt)
{
    ...
    if (!self()->shouldBuildStructure() && (self()->comp()->getMethodHotness() >= scorching)
        && !tt->getEnclosingBlock()->isCold()
        && strstr(fej9->sampleSignature(node->getOwningMethod(), 0, 0, self()->trMemory()),
               "java/util/TreeMap$UnboundedValueIterator.next()")) {
        self()->setShouldBuildStructure();
    }

    // J9
    if (node->getOpCode().isCall() && node->isUnsafePutOrderedCall() && node->isDontInlinePutOrderedCall()) {
        // Remove this treetop
    ...
    // J9
    if (!self()->comp()->getOption(TR_DisableUnsafe) && node->getOpCode().isCall() && node->getOpCodeValue() == TR::call
        ...
        && ((node->getSymbol()->castToMethodSymbol()->getRecognizedMethod() == TR::java_nio_Bits_copyToByteArray)
               || (node->getSymbol()->castToMethodSymbol()->getRecognizedMethod()
                      == TR::java_nio_Bits_copyFromByteArray))
    ...
    // J9
    if (self()->comp()->compileRelocatableCode() && (node->getOpCodeValue() == TR::loadaddr) && parent
    ...
    if (node->getOpCode().hasSymbolReference() && (node->getOpCode().isLoad() || node->getOpCode().isStore())) {
        ...
        if (symbol->isVolatile() && node->getDataType() == TR::Int64 && !symRef->isUnresolved()
            && TR::Compiler->target.is32Bit() && !self()->getSupportsInlinedAtomicLongVolatiles()) {
            bool isLoad = false;
            TR::SymbolReference* volatileLongSymRef = NULL;
            if (node->getOpCode().isLoadVar()) {
                volatileLongSymRef = self()->comp()->getSymRefTab()->findOrCreateRuntimeHelper(
                    TR_volatileReadLong, false, false, true);
                isLoad = true;
            } else
                volatileLongSymRef = self()->comp()->getSymRefTab()->findOrCreateRuntimeHelper(
                    TR_volatileWriteLong, false, false, true);
            ...
            if (address) {
                if (symRef->getOffset() == 0)
                    ...
                    addrNode = TR::Node::create(
                        TR::aiadd, 2, address, TR::Node::create(node, TR::iconst, 0, symRef->getOffset()));
    ...
    // J9
    if (self()->comp()->getOption(TR_ReservingLocks) && node->getOpCodeValue() == TR::monent) {
        ...
        TR_PersistentClassInfo* persistentClassInfo
            = self()->comp()->getPersistentInfo()->getPersistentCHTable()->findClassInfoAfterLocking(
                classPointer, self()->comp());
    ...
    // J9
    if (self()->comp()->getOptions()->enableDebugCounters() && node->getOpCode().isCall()
        ...
        if (insertMethod) {
            TR::DebugCounter::prependDebugCounter(self()->comp(),
                TR::DebugCounter::debugCounterName(
                    self()->comp(), "compilationReport.instructions:byMethod.numInvocations.(%s)", caller),
                tt);
        }
        TR::DebugCounter::prependDebugCounter(self()->comp(),
            TR::DebugCounter::debugCounterName(
                self()->comp(), "callers/(%s)/%d=%d", caller, bcInfo.getCallerIndex(), bcInfo.getByteCodeIndex()),
            tt);
        TR::DebugCounter::prependDebugCounter(self()->comp(),
            TR::DebugCounter::debugCounterName(self()->comp(), "callees/(%s)/(%s)/%d=%d", callee, caller,
    ...
    if (self()->comp()->getOption(TR_EnableRecompilationPushing) && !self()->getCurrentBlock()->isCold()
        && self()->comp()->allowRecompilation() && self()->comp()->getMethodHotness() >= veryHot
        ...
        if (pushCall) {
            if (!((methodSymbol && methodSymbol->getResolvedMethodSymbol()
                ...
                && methodSymbol->getResolvedMethodSymbol()->getResolvedMethod()) {
                void* methodAddress = methodSymbol->getResolvedMethodSymbol()
                                          ->getResolvedMethod()
                ...
                if (bodyInfo && bodyInfo->getHotness() <= warm && !bodyInfo->getIsProfilingBody()) {
                    ...
                    // printf ("Scorching method %s is calling warm (or called method) %s, in block_%d with frequency
                    // %d, is in loop %d\n", comp()->getMethodSymbol()->signature(),
                    // fej9->sampleSignature(method->getPersistentIdentifier(), 0, 0, trMemory()),
                    // getCurrentBlock()->getNumber(), getCurrentBlock()->getFrequency(), isInLoop);
    ...
    case TR:: instanceof: {
        ...
        if (self()->comp()->getOption(TR_TraceCG))
            traceMsg(self()->comp(), "Anchoring %s node %s [%p] under treetop [%p]\n", anchoringReason,
                node->getOpCode().getName(), node, ttNode);
        break;
    }
    default:
        break;
        ...
        for (TR::TreeTop* tt = self()->comp()->getStartTree(); tt;
            ...
            if (opCode == TR::BBStart) {
                ...
                for (auto e = b->getSuccessors().begin(); e != b->getSuccessors().end(); ++e)
                    if ((*e)->getTo() == self()->comp()->getFlowGraph()->getEnd()) {
                        ...
                        if (liveMonitorStack && liveMonitorStack->size() != 0 && !endsInThrow)
                            dumpOptDetails(self()->comp(),
                                "liveMonitorStack must be empty, unbalanced monitors found! %d\n",
                ...
                if (endOfEBB
                    && (b->branchesBackwards()
                           || numEBBsSinceFreeingVariableSizeSymRefs
                               > MAX_EBBS_BEFORE_FREEING_VARIABLE_SIZED_SYMREFS)) {
                    if (self()->traceBCDCodeGen())
                        traceMsg(self()->comp(),
                            "\tblock_%d branches backwards, so free all symbols in the "
            ...
            if (((opCode == TR::BBEnd) && !fixedUpBlock) || node->getOpCode().isBranch()
                ...
                if ((self()->comp()->getOption(TR_MimicInterpreterFrameShape)
                    ...
                    //
                    if ((!self()->comp()->getOption(TR_PoisonDeadSlots) && liveLocals)
                        || (self()->comp()->getOption(TR_PoisonDeadSlots)
                        ...
                        TR_BitVectorIterator bvi(
                            self()->comp()->getOption(TR_PoisonDeadSlots) ? *block->getLiveLocals() : *liveLocals);

                        if (self()->comp()->getOption(TR_TraceCG) && self()->comp()->getOption(TR_PoisonDeadSlots))
                            traceMsg(
                                self()->comp(), "POISON DEAD SLOTS --- Parent Block Number: %d\n", block->getNumber());
                        ...
                        while (bvi.hasMoreElements()) {
                            ...
                            if (self()->comp()->getOption(TR_TraceCG) && self()->comp()->getOption(TR_PoisonDeadSlots))
                                traceMsg(self()->comp(),
                                    "POISON DEAD SLOTS --- Parent Block: %d, Maintained Live Local: %d\n",
                                ...
                                for (auto succ = block->getSuccessors().begin();
                                    ...
                                    if (self()->comp()->getOption(TR_PoisonDeadSlots)
                                        ...
                                        if (self()->comp()->getOption(TR_TraceCG)
                                            && self()->comp()->getOption(TR_PoisonDeadSlots))
                                            traceMsg(self()->comp(),
                                                "POISON DEAD SLOTS --- Successor Block Number %d is extension of "
                                                "Parent Block %d ... skipping \n",
                                    ...
                                    if (succLiveLocals && !succLiveLocals->get(liveLocalIndex)) // added
                                    {
                                        ...
                                        if (zeroOut) {
                                            ...
                                            self()->zeroOutAutoOnEdge(
                                                liveAutoSymRef, block, succBlock, &newBlocks, &fsdStores);
                                        }
                                    }
                                    succ = next;
                                }
                    ...
                    if (liveMonitorStack) {
                        ...
                        if (!liveMonitorStack->isEmpty()) {
                            ...
                            if (self()->comp()->getOption(TR_PoisonDeadSlots)) {
                                ...
                                storeNode
                                    = generatePoisonNode(self()->comp(), self()->getCurrentEvaluationBlock(), symRef);
                                if (storeNode) {
                                    TR::TreeTop* storeTree = TR::TreeTop::create(self()->comp(), storeNode);
            ...
            if (liveLocals) {
                ...
                if (debug("checkBlockEntryLiveLocals")) {
                    ...
                    if ((opCode == TR::astore)
                        && ((!self()->comp()->getOption(TR_MimicInterpreterFrameShape))
                               || (!self()->comp()->areSlotsSharedByRefAndNonRef()) || (!fsdStores.find(node)))) {
                        liveSym = node->getSymbol()->getAutoSymbol();
                    }
                ...
                //
                if (self()->comp()->getOption(TR_MimicInterpreterFrameShape) && node->getOpCode().isStore()
                    && opCode != TR::astore) {
                ...
                while (it != _variableSizeSymRefPendingFreeList.end()) {
                    ...
                    if (self()->traceBCDCodeGen()) {
                        if (sym->getNodeToFreeAfter())
                            traceMsg(self()->comp(),
                                "pending free temps : looking at symRef #%d (%s) refCount %d sym->getNodeToFreeAfter() "
                                "%p ttNode %p (find sym in list %d)\n",
            ...
            if (self()->comp()->getOption(TR_TraceCG) || debug("traceGRA")) {
                ...
                if (tt == self()->getCurrentEvaluationTreeTop()) {
                    trfprintf(self()->comp()->getOutFile(), "------------------------------\n");
                    self()->comp()->getDebug()->dumpSingleTreeWithInstrs(
...
//
void J9::CodeGenerator::splitWarmAndColdBlocks()
{
    ...
    if (!(node->getOpCode().isGoto() || node->getOpCode().isJumpWithMultipleTargets()
        ...
        TR::TreeTop* gotoTreeTop
            = TR::TreeTop::create(self()->comp(), TR::Node::create(node, TR::Goto, 0, targetTreeTop));
...
void J9::CodeGenerator::populateOSRBuffer()
{
    ...
    const TR_Array<TR_OSRMethodData*>& methodDataArray
        = self()->comp()->getOSRCompilationData()->getOSRMethodDataArray();
    ...
    */
    ...
    for (TR::TreeTop* tt = self()->comp()->getStartTree(); tt; tt = tt->getNextTreeTop()) {
        ...
        if (traceOSR)
            traceMsg(self()->comp(),
                "callerIndex %d: max pending push slots=%d, # of auto slots=%d, # of arg slots=%d\n",
        ...
        for (int32_t child = firstSymChildIndex; child + 2 < callNode->getNumChildren(); child += 3) {
            ...
            if (sharedSlot) {
                insertionPoint = self()->genSymRefStoreToArray(
                    callNode, osrScratchBufferNode, NULL, loadNode, scratchBufferOffset, insertionPoint);
                ...
                insertionPoint = self()->genSymRefStoreToArray(
                    callNode, osrBufferNode, osrFrameIndex, loadNode, offset, insertionPoint);
            }
        }
        ...
        if ((numOfSymsThatShareSlot > 0) || self()->comp()->getOption(TR_EnablePrepareForOSREvenIfThatDoesNothing))
            emitCall = true;
...
void J9::CodeGenerator::processRelocations()
{
    ...
    if (self()->comp()->getOption(TR_AOT) != false) {
        ...
        if (inlinedCallSize > 0) {
            orderedInlinedSiteListTable = (TR_InlinedSiteHastTableEntry*)self()->comp()->trMemory()->allocateMemory(
                sizeof(TR_InlinedSiteHastTableEntry) * inlinedCallSize, heapAlloc);
        ...
        for (auto it = aotGuardSites->begin(); it != aotGuardSites->end(); ++it) {
            ...
            case TR_MethodEnterExitGuard:
                if ((*it)->getGuard()->getCallNode()->getOpCodeValue() == TR::MethodEnterHook)
                    ...
                    TR_ASSERT(0, "Unexpected TR_MethodEnterExitGuard at site %p guard %p node %p\n", *it,
                        (*it)->getGuard(), (*it)->getGuard()->getCallNode());
                break;
            ...
            case TR_InlinedInterfaceMethod:
                ...
                entry = (TR_InlinedSiteLinkedListEntry*)self()->comp()->trMemory()->allocateMemory(
            ...
            case TR_HCR:
                self()->addExternalRelocation(new (self()->trHeapMemory())
                                                  TR::ExternalRelocation((uint8_t*)(*it)->getLocation(),
                                                      (uint8_t*)(*it)->getDestination(), type, self()),
        ...
        if (!classInfo->empty()) {
            for (auto info = classInfo->begin(); info != classInfo->end(); ++info) {
                ...
                traceMsg(self()->comp(), "ramMethod: %p cp: %p cpIndex: %x relo %d\n", (*info)->_method,
                    (*info)->_constantPool, (*info)->_cpIndex, (*info)->_reloKind);
                ...
                if (ramMethod
                    != self()->comp()->getCurrentMethod()->getPersistentIdentifier()) // && info->_reloKind !=
                                                                                      // TR_ValidateArbitraryClass)
                {
                    int32_t i;
                    for (i = 0; i < self()->comp()->getNumInlinedCallSites(); i++) {
                        ...
                        // if (((uintptrj_t)ramMethod ==
                        // (uintptrj_t)aotMethodInfo->resolvedMethod->getPersistentIdentifier()) &&
                        // orderedInlinedSiteListTable[counter].first)
                        {
                    ...
                    if (i >= (int32_t)self()->comp()->getNumInlinedCallSites()) {
                        ...
                        // so we can't use a constant pool approach to validate: transform into
                        // TR_ValidateArbitraryClass kind of overkill for TR_ValidateStaticField, but still correct
                ...
                TR_ASSERT(siteIndex < (int32_t)self()->comp()->getNumInlinedCallSites(),
                    "did not find AOTClassInfo %p method in inlined site table", *info);

                self()->addExternalRelocation(new (self()->trHeapMemory())
                                                  TR::ExternalRelocation(NULL, (uint8_t*)(intptr_t)siteIndex,
                                                      (uint8_t*)(*info), (*info)->_reloKind, self()),
                    __FILE__, __LINE__, NULL);
            }
        }
            ...
            for (; counter >= 0; counter--) {
                ...
                while (currentSite) {
                    self()->addExternalRelocation(
                        new (self()->trHeapMemory()) TR::ExternalRelocation(currentSite->location,
                            currentSite->destination, currentSite->guard, currentSite->reloType, self()),
                        __FILE__, __LINE__, NULL);
        ...
        TR::SymbolValidationManager::SymbolValidationRecordList& validationRecords
            = self()->comp()->getSymbolValidationManager()->getValidationRecordList();
        if (!validationRecords.empty() && self()->comp()->getOption(TR_UseSymbolValidationManager)) {
            ...
            for (auto it = validationRecords.begin(); it != validationRecords.end(); it++) {
                self()->addExternalRelocation(new (self()->trHeapMemory())
                                                  TR::ExternalRelocation(NULL, (uint8_t*)(*it), (*it)->_kind, self()),
                    __FILE__, __LINE__, NULL);
            }
        }
    ...
    TR_ExternalRelocationTargetKind kind, char* generatingFileName, uintptr_t generatingLineNumber, TR::Node* node)
{
    (target2 == NULL)
        ? self()->addExternalRelocation(new (self()->trHeapMemory())
                                            TR::ExternalRelocation(location, target, kind, self()),
              generatingFileName, generatingLineNumber, node)
        : self()->addExternalRelocation(new (self()->trHeapMemory())
                                            TR::ExternalRelocation(location, target, target2, kind, self()),
              generatingFileName, generatingLineNumber, node);
}
    ...
    TR_ExternalRelocationTargetKind kind, char* generatingFileName, uintptr_t generatingLineNumber, TR::Node* node)
{
    (target2 == NULL)
        ...
        : self()->addExternalRelocation(new (self()->trHeapMemory()) TR::BeforeBinaryEncodingExternalRelocation(
                                            instr, target, target2, kind, self()),
              generatingFileName, generatingLineNumber, node);
}
...
void J9::CodeGenerator::jitAddUnresolvedAddressMaterializationToPatchOnClassRedefinition(void* firstInstruction)
{
    ...
    if (self()->comp()->compileRelocatableCode()) {
        self()->addExternalRelocation(new (self()->trHeapMemory())
                                          TR::ExternalRelocation((uint8_t*)firstInstruction, 0, TR_HCR, self()),
            __FILE__, __LINE__, NULL);
    } else {
...
void J9::CodeGenerator::compressedReferenceRematerialization()
{
    ...
    if (self()->comp()->useCompressedPointers() && (TR::Compiler->vm.heapBaseAddress() == 0) && !disableRematforCP) {
        for (tt = self()->comp()->getStartTree(); tt; tt = tt->getNextTreeTop()) {
            node = tt->getNode();
    ...
    int32_t childNum, TR::Node* node, vcount_t visitCount, List<TR::Node>* rematerializedNodes)
{
        ...
        && ((node->getFirstChild()->getOpCodeValue() == TR::ladd
               ...
               || ((node->getFirstChild()->getOpCodeValue() == TR::lshl)
                      ...
                      && self()->isAddressScaleIndexSupported(
                             (1 << TR::Compiler->om.compressedReferenceShiftOffset()))))) {
    ...
    if (node->getOpCode().isNullCheck() && reference
        && (!isLowMemHeap || self()->performsChecksExplicitly()
        ...
        if ((reference->getOpCodeValue() == TR::l2a)
            ...
            && performTransformation(self()->comp(),
                   "%sStrength reducing null check reference %p in null check node %p \n", OPT_DETAILS, reference,
                   node)) {
            bool addedToList = false;
            if (node->getFirstChild()->getOpCodeValue() == TR::PassThrough) {
    ...
    if (self()->materializesHeapBase() && !isLowMemHeap && parent && (!parent->getOpCode().isStore())
        && (node->getOpCodeValue() == TR::lconst) && (node->getLongInt() == TR::Compiler->vm.heapBaseAddress())
        && performTransformation(
               self()->comp(), "%sTransforming heap base constant node %p to auto load \n", OPT_DETAILS, node)) {
        if (!autoSymRef) {
            autoSymRef = self()->comp()->getSymRefTab()->createTemporary(
                self()->comp()->getMethodSymbol(), node->getDataType());
        ...
        if (address->getOpCodeValue() == TR::l2a
            ...
            if (node->getOpCode().isWrtBar() && node->skipWrtBar()) {
                ...
                if (!isPossiblyUnderArrayStoreCheck
                    && performTransformation(self()->comp(),
                           "%sStoring compressed pointer [%p] directly into %p in tree %p\n", OPT_DETAILS, address,
                           node, tt->getNode())) {
                    bool addedToList = false;
...
void J9::CodeGenerator::yankCompressedRefs(
    TR::TreeTop* tt, TR::Node* parent, int32_t childNum, TR::Node* node, vcount_t visitCount, vcount_t secondVisitCount)
{
    if (node->getVisitCount() >= visitCount)
        return;
    ...
    if (parent && (parent->getOpCodeValue() == TR::treetop) && (node->getOpCodeValue() == TR::l2a)
        ...
        if (firstTree != tt) {
            TR_BitVector symbolReferencesInNode(
                self()->comp()->getSymRefCount(), self()->comp()->trMemory(), stackAlloc);
            ...
            if (canYank) {
                ...
                while (cursorTree) {
                    numTrees++;
                    TR::Node* cursorNode = cursorTree->getNode();
                    // printf("canYank %d node %p cursor %p in %s\n", canYank, node, cursorNode, comp()->signature());
                    // fflush(stdout);
        ...
        if (rematNode->getOpCodeValue() == TR::l2a) {
            ...
            if (load->getOpCode().isLoadIndirect()
                ...
                if (self()->comp()->getOption(TR_TraceCG)) {
                    if (node->getOpCode().isStoreIndirect())
                        traceMsg(
                            self()->comp(), "Found previous load %p same as store %p, anchoring load\n", load, node);
                    else
    ...
    TR::Node* firstOffset, TR::Node* loadNode, int32_t secondOffset, TR::TreeTop* insertionPoint)
{
    ...
    TR::Node* addrNode
        = TR::Node::create(TR::Compiler->target.is64Bit() ? TR::aladd : TR::aiadd, 2, arrayAddressNode, offsetNode);
...
void J9::CodeGenerator::allocateLinkageRegisters()
{
    ...
    TR::Delimiter d(self()->comp(),
        self()->comp()->getOptions()->getAnyOption(TR_TraceOptDetails | TR_CountOptTransformations),
    ...
    TR_BitVector globalRegsWithRegLoad(self()->getNumberOfGlobalRegisters(), self()->comp()->trMemory(),
    ...
    if (numRegLoadsAdded > 0) {
        ...
        dumpOptDetails(self()->comp(), "  Created new GlRegDeps %s on BBStart %s\n",
            self()->comp()->getDebug()->getName(newRegDeps), self()->comp()->getDebug()->getName(bbStart));
    }
}
    ...
    TR_BitVector* globalRegsWithRegLoad, TR_BitVector& killedParms, vcount_t visitCount)
{
    ...
    if (node->getOpCode().isLoadVar() && node->getSymbol()->isParm()) {
        ...
        if (regLoads[parm->getOrdinal()] == NULL && lri != -1 && !killedParms.isSet(parm->getOrdinal())) {
            ...
            if ((node->getType().isInt64() && TR::Compiler->target.is32Bit() && !self()->use64BitRegsOn32Bit())
                ...
                if (self()->getDisableLongGRA()) {
                    ...
                    if (lowReg != -1 && highReg != -1 && !globalRegsWithRegLoad->isSet(lowReg)
                        ...
                        && performTransformation(self()->comp(),
                               "O^O LINKAGE REGISTER ALLOCATION: transforming %s into %s\n",
                               self()->comp()->getDebug()->getName(node),
                               self()->comp()->getDebug()->getName(regLoadOp))) {
                        // Both halves are in regs, and both regs are available.
                        // Transmute load into regload
                        //
                        if (parm->getDataType() == TR::Aggregate) {
                            dumpOptDetails(self()->comp(),
                                "\tNot doing transformation for parm %p because it is an aggregate.\n", node);
                        } else {
            ...
            } else if (TR::Compiler->target.cpu.isZ() && TR::Compiler->target.isLinux()
                && parm->getDataType() == TR::Aggregate
        ...
        for (int i = 0; i < node->getNumChildren(); i++)
            numNewRegLoads += self()->changeParmLoadsToRegLoads(
                node->getChild(i), regLoads, globalRegsWithRegLoad, killedParms, visitCount);
    }
...
bool J9::CodeGenerator::wantToPatchClassPointer(
    TR::Compilation* comp, const TR_OpaqueClassBlock* allegedClassPointer, const uint8_t* inCodeAt)
{
    return TR::CodeGenerator::wantToPatchClassPointer(comp, allegedClassPointer, "in code at", inCodeAt);
}

bool J9::CodeGenerator::wantToPatchClassPointer(const TR_OpaqueClassBlock* allegedClassPointer, const uint8_t* inCodeAt)
{
    return TR::CodeGenerator::wantToPatchClassPointer(self()->comp(), allegedClassPointer, inCodeAt);
}

bool J9::CodeGenerator::wantToPatchClassPointer(const TR_OpaqueClassBlock* allegedClassPointer, const TR::Node* forNode)
{
    return TR::CodeGenerator::wantToPatchClassPointer(self()->comp(), allegedClassPointer, "for node", forNode);
}

bool J9::CodeGenerator::supportsMethodEntryPadding() { return self()->fej9()->supportsMethodEntryPadding(); }
...
extern void VMgenerateCatchBlockBBStartPrologue(
    TR::Node* node, TR::Instruction* fenceInstruction, TR::CodeGenerator* cg);
...
void J9::CodeGenerator::registerAssumptions()
{
    for (auto it = self()->getJNICallSites().begin(); it != self()->getJNICallSites().end(); ++it) {
        ...
        TR_PatchJNICallSite::make(self()->fe(), self()->trPersistentMemory(), (uintptrj_t)method,
...
void J9::CodeGenerator::jitAddPicToPatchOnClassUnload(void* classPointer, void* addressToBePatched)
{
    createClassUnloadPicSite(
        classPointer, addressToBePatched, sizeof(uintptrj_t), self()->comp()->getMetadataAssumptionList());
...
void J9::CodeGenerator::jitAddPicToPatchOnClassRedefinition(
    void* classPointer, void* addressToBePatched, bool unresolved)
{
    if (!self()->comp()->compileRelocatableCode()) {
        createClassRedefinitionPicSite(unresolved ? (void*)-1 : classPointer, addressToBePatched, sizeof(uintptrj_t),
            unresolved, self()->comp()->getMetadataAssumptionList());
        self()->comp()->setHasClassRedefinitionAssumptions();
    }
}

void J9::CodeGenerator::jitAdd32BitPicToPatchOnClassRedefinition(
    void* classPointer, void* addressToBePatched, bool unresolved)
{
    if (!self()->comp()->compileRelocatableCode()) {
