...
static void traceInstanceOfOrCheckCastProfilingInfo(
    TR::CodeGenerator* cg, TR::Node* node, TR_OpaqueClassBlock* castClass)
{
    ...
    float* topClassProbability, bool* topClassWasCastClass)
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(cg->fe());
    TR_ASSERT(
        instanceOfOrCheckCastNode->getOpCode().isCheckCast() || instanceOfOrCheckCastNode->getOpCodeValue() == TR::
    ...
    //
        ...
        if (cg->supportsInliningOfIsInstance() && instanceOfOrCheckCastNode->getOpCodeValue() == TR:: instanceof
    ...
    //
        ...
        //
                ...
                *numberOfProfiledClass = getInstanceOfOrCheckCastTopProfiledClass(cg, instanceOfOrCheckCastNode,
                ...
                if (cg->comp()->getOption(TR_TraceCG)) {
                    for (int i = 0; i < numProfiledClasses; i++) {
                        J9UTF8* profiledClassName
                            = J9ROMCLASS_CLASSNAME(((J9Class*)profiledClassList[i].profiledClass)->romClass);
                        traceMsg(cg->comp(),
            ...
            //
            if (compileTimeGuessClass
                && (TR::Compiler->cls.isInterfaceClass(cg->comp(), castClass)
                       || TR::Compiler->cls.isAbstractClass(cg->comp(), castClass))) {
                ...
                if (!isInstanceOf) {
                    singleImplementerClass
                        = cg->comp()->getPersistentInfo()->getPersistentCHTable()->findSingleConcreteSubClass(
                            castClass, cg->comp());
                    if (cg->comp()->getOption(TR_TraceCG)) {
                        if (singleImplementerClass) {
                            J9UTF8* singleImplementerClassName
                                = J9ROMCLASS_CLASSNAME(((J9Class*)singleImplementerClass)->romClass);
                            traceMsg(cg->comp(),
 ...
 * if probability is not null, record class' coressponding probablity in this array. Used for cost/benefit analysis for
 * profiled check.
 */
uint8_t J9::TreeEvaluator::interpreterProfilingInstanceOfOrCheckCastInfo(
    TR::CodeGenerator* cg, TR::Node* node, TR_OpaqueClassBlock** classArray, float* probability, bool recordAll)
{
    TR_ByteCodeInfo bcInfo = node->getByteCodeInfo();
    TR::Compilation* comp = cg->comp();
    TR_ValueProfileInfoManager* valueProfileInfo = TR_ValueProfileInfoManager::get(comp);
    ...
    if (!valueInfo || valueInfo->getNumProfiledValues() == 0) {
        if (p1)
            traceMsg(comp, "==TPIC==No IProfiler info on node %p in %s\n", node, comp->signature());
        return 0;
    }
...
TR_OpaqueClassBlock* J9::TreeEvaluator::interpreterProfilingInstanceOfOrCheckCastInfo(
...
bool J9::TreeEvaluator::loadLookaheadAfterHeaderAccess(TR::Node* node, int32_t& fieldOffset, TR::CodeGenerator* cg)
{
    ...
    while (currTree) {
        ...
        if (currNode->getOpCodeValue() == TR::aloadi || currNode->getOpCodeValue() == TR::iloadi) {
            if (currNode->getFirstChild() == object) {
                ...
                if (sym) {
                    if (sym->isRegisterMappedSymbol() && sym->getRegisterMappedSymbol()->getOffset() != 0) {
            ...
            && (currNode->getFirstChild()->getOpCodeValue() == TR::aloadi
                   || currNode->getFirstChild()->getOpCodeValue() == TR::iloadi)) {
...
void J9::TreeEvaluator::evaluateLockForReservation(
    TR::Node* node, bool* reservingLock, bool* normalLockPreservingReservation, TR::CodeGenerator* cg)
{
