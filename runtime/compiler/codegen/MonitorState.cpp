    ...
    int32_t callerIndex, bool walkOnlyExceptionSuccs)
{
    ...
    for (TR::CFGEdge* edge = succs.getFirst(); edge; edge = succs.getNext()) {
        ...
        if (succBlock->getEntry()) {
                ...
                if (succBlock->getVisitCount() != _visitCount) {
                    ...
                    if (traceIt)
                        traceMsg(comp(), "adding monitorstack to successor %d (%p size %d)\n", succBlock->getNumber(),
                            newMonitorStack, newMonitorStack->size());
                } else {
                    ...
                    if (!succBlock->isOSRCatchBlock() && !succBlock->isOSRCodeBlock()) {
                        if (!isMonitorStateConsistentForBlock(succBlock, monitorStack, popMonitor))
                            ...
                            if (traceIt)
                                traceMsg(
                                    comp(), "verified block_%d monitorState is consistent\n", succBlock->getNumber());
                        }
                    }
                    continue;
                }
            ...
            if (traceIt)
                traceMsg(
                    comp(), "\tIn SMSOnBE::addSuccessors adding block %d to blocksToVisit\n", succBlock->getNumber());
...
bool J9::SetMonitorStateOnBlockEntry::isMonitorStateConsistentForBlock(
        ...
        if (callerIndex == -1) {
            if (comp->getJittedMethodSymbol()->isSynchronised()) {
                ...
                if (monitorStack->size() == 1 && !comp->getJittedMethodSymbol()->isStatic() && !blockExitsMethod) {
                    if (traceIt)
                        traceMsg(comp,
                            "monitorStack is empty (except for special DLT sync object slot) for DLT compile at "
                            "monexit %p\n",
                            node);
                    return false;
        ...
        for (TR::TreeTop* tt = block->getEntry();; tt = tt->getNextTreeTop()) {
            ...
            if ((node->getOpCode().isStore() && // only monents are represented by this store now
                ...
                if (monitorStack) {
                    ...
                    if (traceIt)
                        traceMsg(comp(), "adding monitor to stack symbol=%p symRef=%d (size=%d) (node %p)\n",
                            node->getSymbol(), node->getSymbolReference()->getReferenceNumber(),
                ...
                if (monitorStack->topIndex() == 0) {
                    monitorStack = new (trHeapMemory()) TR_Stack<TR::SymbolReference*>(*monitorStack);
                    if (traceIt)
                        traceMsg(comp(), "popping monitor off stack symRef=%d, BEFORE pop size=%d, ",
            ...
            if (tt == block->getExit()) {
                ...
                if ((monitorExitFence + monitorEnterStore) >= 2) {
                    if (traceIt)
                        traceMsg(comp(), "block_%d has monitorEnterStore=%d monitorExitFence=%d\n", block->getNumber(),
                            monitorEnterStore, monitorExitFence);
                    lmmdFailed = true;
                }
                ...
                if (traceIt)
                    traceMsg(comp(),
                        ...
                        blockHasMonent, blockHasMonexit, dontPropagateMonitor, callerIndex,
                        monitorPoppedForExceptionSucc);
            ...
            if (autos) {
                ...
                for (TR::RegisterMappedSymbol* a = iterator.getFirst(); a; a = iterator.getNext()) {
                    TR::DebugCounter::incStaticDebugCounter(
                        comp(), TR::DebugCounter::debugCounterName(comp(), "lmmdFailed/(%s)", comp()->signature()));
