 ...
 */
void operator delete(void*) { TR_ASSERT(0, "Invalid use of global operator delete"); }
...
J9::Compilation::Compilation(int32_t id, J9VMThread* j9vmThread, TR_FrontEnd* fe, TR_ResolvedMethod* compilee,
    ...
    : OMR::CompilationConnector(id, j9vmThread->omrVMThread, (firstCompileStarted = true, fe), compilee, ilGenRequest,
          options, heapMemoryRegion, m, optimizationPlan)
    ...
    , _reloRuntime(reloRuntime)
{
    ...
    _aotGuardPatchSites
        = new (m->trHeapMemory()) TR::list<TR_AOTGuardSite*>(getTypedAllocator<TR_AOTGuardSite*>(self()->allocator()));
...
int32_t J9::Compilation::canAllocateInlineOnStack(TR::Node* node, TR_OpaqueClassBlock*& classInfo)
{
    ...
    if (node->getOpCodeValue() == TR::New) {
        ...
        if (clazz->classDepthAndFlags
            & (J9_JAVA_CLASS_REFERENCE_WEAK | J9_JAVA_CLASS_REFERENCE_SOFT | J9_JAVA_CLASS_FINALIZE
...
int32_t J9::Compilation::canAllocateInline(TR::Node* node, TR_OpaqueClassBlock*& classInfo)
{
    ...
    || (defined(TR_TARGET_POWER) && defined(TR_HOST_POWER))
    ...
    if (node->getOpCodeValue() == TR::newarray || self()->useCompressedPointers()) {
        size = (int32_t)((size + (TR::Compiler->om.sizeofReferenceAddress() - 1))
            & ~(TR::Compiler->om.sizeofReferenceAddress() - 1));
    }
...
void J9::Compilation::addHWPInstruction(
    TR::Instruction* instruction, TR_HWPInstructionInfo::type instructionType, void* data)
{
...
void J9::Compilation::verifyCompressedRefsAnchors(
    TR::Node* parent, TR::Node* node, TR::TreeTop* tt, vcount_t visitCount)
{
...
static TR_Pair<TR::Node, TR::TreeTop>* findCPtrsInfo(
    TR::list<TR_Pair<TR::Node, TR::TreeTop>*>& haystack, TR::Node* needle)
{
...
{
    if (self()->getOption(TR_UseSymbolValidationManager) && self()->compileRelocatableCode()) {
        return self()->getSymbolValidationManager()->addMethodFromClassRecord(
            implementer->getPersistentIdentifier(), implementer->classOfMethod(), -1);
    }
    return true;
}
...
TR_OpaqueClassBlock* J9::Compilation::getClassClassPointer(bool isVettedForAOT)
{
    ...
    TR_OpaqueClassBlock* jlObject = self()->fej9()->getClassFromSignature(
        "Ljava/lang/Object;", 18, self()->getCurrentMethod(), jlObjectVettedForAOT);
...
bool J9::Compilation::notYetRunMeansCold()
{
    ...
    intptrj_t initialCount = currentMethod->hasBackwardBranches() ? self()->getOptions()->getInitialBCount()
