...
TR::SymbolReference* J9::SymbolReferenceTable::findOrCreateFloatSymbol(
...
TR::SymbolReference* J9::SymbolReferenceTable::findOrCreateDoubleSymbol(
    TR::ResolvedMethodSymbol* owningMethodSymbol, int32_t cpIndex)
{
...
TR::SymbolReference* J9::SymbolReferenceTable::createSystemRuntimeHelper(
    ...
    TR::ResolvedMethodSymbol* owningMethodSymbol, int32_t cpIndex)
{
    ...
    return findOrCreateMethodSymbol(
        owningMethodSymbol->getResolvedMethodIndex(), cpIndex, method, TR::MethodSymbol::Static, isUnresolvedInCP);
}
    ...
    TR::ResolvedMethodSymbol* owningMethodSymbol, int32_t cpIndex)
{
    ...
    return findOrCreateMethodSymbol(
        owningMethodSymbol->getResolvedMethodIndex(), cpIndex, method, TR::MethodSymbol::Special, isUnresolvedInCP);
}
    ...
    TR::ResolvedMethodSymbol* owningMethodSymbol, int32_t cpIndex)
{
    ...
    TR::SymbolReference* symRef = findOrCreateMethodSymbol(
        owningMethodSymbol->getResolvedMethodIndex(), cpIndex, 0, TR::MethodSymbol::Interface);
    ...
    TR::ResolvedMethodSymbol* owningMethodSymbol, int32_t callSiteIndex)
{
    ...
    TR_ResolvedMethod* method
        = owningMethodSymbol->getResolvedMethod()->getResolvedDynamicMethod(comp(), callSiteIndex);
    if (method)
        owningMethodSymbol->setMayHaveInlineableCall(true);

    TR::SymbolReference* symRef = findOrCreateMethodSymbol(
        owningMethodSymbol->getResolvedMethodIndex(), -1, method, TR::MethodSymbol::ComputedVirtual);
    methods->add(symRef);
    return symRef;
}
    ...
    TR::ResolvedMethodSymbol* owningMethodSymbol, int32_t cpIndex, char* signature)
{
    ...
    TR::SymbolReference* symRef = findOrCreateMethodSymbol(
        owningMethodSymbol->getResolvedMethodIndex(), cpIndex, method, TR::MethodSymbol::ComputedVirtual);
    return symRef;
}
    ...
    TR::ResolvedMethodSymbol* owningMethodSymbol, int32_t cpIndex)
{
    bool isUnresolvedInCP;
    TR_ResolvedMethod* method
        = owningMethodSymbol->getResolvedMethod()->getResolvedHandleMethod(comp(), cpIndex, &isUnresolvedInCP);
    if (method)
        owningMethodSymbol->setMayHaveInlineableCall(true);
    ...
    TR::ResolvedMethodSymbol* owningMethodSymbol, int32_t cpIndex)
{
    ...
    symRef = new (trHeapMemory()) TR::SymbolReference(
        self(), sym, owningMethodSymbol->getResolvedMethodIndex(), -1, isUnresolved ? _numUnresolvedSymbols++ : 0);
...
TR::SymbolReference* J9::SymbolReferenceTable::methodSymRefWithSignature(
    TR::SymbolReference* originalSymRef, char* effectiveSignature, int32_t effectiveSignatureLength)
{
    ...
    sprintf(fullSignature, "%.*s.%.*s%.*s", originalMethod->classNameLength(), originalMethod->classNameChars(),
        originalMethod->nameLength(), originalMethod->nameChars(), effectiveSignatureLength, effectiveSignature);
...
TR::SymbolReference* J9::SymbolReferenceTable::findOrCreateJavaLangReferenceReferentShadowSymbol(
...
TR::SymbolReference* J9::SymbolReferenceTable::findOrCreateShadowSymbol(
    TR::ResolvedMethodSymbol* owningMethodSymbol, int32_t cpIndex, bool isStore)
{
    ...
    bool resolved = owningMethod->fieldAttributes(
        comp(), cpIndex, &offset, &type, &isVolatile, &isFinal, &isPrivate, isStore, &isUnresolvedInCP, true);
...
TR::SymbolReference* J9::SymbolReferenceTable::findOrCreateClassAndDepthFlagsSymbolRef()
{
    if (!element(isClassAndDepthFlagsSymbol)) {
        TR_J9VMBase* fej9 = (TR_J9VMBase*)(fe());
        TR::Symbol* sym = TR::Symbol::createShadow(trHeapMemory(), TR::Int32);
        element(isClassAndDepthFlagsSymbol)
            = new (trHeapMemory()) TR::SymbolReference(self(), isClassAndDepthFlagsSymbol, sym);
...
TR::SymbolReference* J9::SymbolReferenceTable::findOrCreateJ9MethodConstantPoolFieldSymbolRef(intptrj_t offset)
{
...
TR::SymbolReference* J9::SymbolReferenceTable::findOrCreateJ9MethodExtraFieldSymbolRef(intptrj_t offset)
{
...
TR::SymbolReference* J9::SymbolReferenceTable::findOrCreateStartPCLinkageInfoSymbolRef(intptrj_t offset)
{
...
TR::SymbolReference* J9::SymbolReferenceTable::findOrCreatePerCodeCacheHelperSymbolRef(
    TR_CCPreLoadedCode helper, uintptrj_t helperAddr)
{
...
TR::SymbolReference* J9::SymbolReferenceTable::findOrCreateStringSymbol(
    TR::ResolvedMethodSymbol* owningMethodSymbol, int32_t cpIndex)
{
    TR_ResolvedMethod* owningMethod = owningMethodSymbol->getResolvedMethod();
...
TR::SymbolReference* J9::SymbolReferenceTable::findOrCreateClassStaticsSymbol(
    TR::ResolvedMethodSymbol* owningMethodSymbol, int32_t cpIndex)
{
    TR_ResolvedMethod* owningMethod = owningMethodSymbol->getResolvedMethod();
...
TR::SymbolReference* J9::SymbolReferenceTable::findOrCreateStaticSymbol(
    TR::ResolvedMethodSymbol* owningMethodSymbol, int32_t cpIndex, bool isStore)
{
    TR_ResolvedMethod* owningMethod = owningMethodSymbol->getResolvedMethod();
    void* dataAddress;
    ...
    bool resolved = owningMethod->staticAttributes(
        comp(), cpIndex, &dataAddress, &type, &isVolatile, &isFinal, &isPrivate, isStore, &isUnresolvedInCP);
    ...
    TR::KnownObjectTable::Index knownObjectIndex = TR::KnownObjectTable::UNKNOWN;
    if (resolved && isFinal && type == TR::Address && !comp()->compileRelocatableCode()) {
            ...
            if (declaringClass && fej9->isClassInitialized(declaringClass)) {
                ...
                if (J9::TransformUtil::foldFinalFieldsIn(declaringClass, clazzName, clazzNameLength, true, comp())) {
                    ...
                    TR_OpaqueClassBlock* varHandleClass
                        = fej9->getSystemClassFromClassName("java/lang/invoke/VarHandle", 26);
                    TR_OpaqueClassBlock* objectClass
                        = TR::Compiler->cls.objectClass(comp(), *((uintptrj_t*)dataAddress));

                    if (varHandleClass != NULL && objectClass != NULL
    ...
    if (resolved) {
        ...
        if (type != TR::Address && _compilation->cg()->getAccessStaticsIndirectly()
            && !_compilation->compileRelocatableCode()) {
...
TR::SymbolReference* J9::SymbolReferenceTable::findUnsafeSymbolRef(
    TR::DataType type, bool javaObjectReference, bool javaStaticReference, bool isVolatile)
{
    TR_Array<TR::SymbolReference*>* unsafeSymRefs = NULL;

    if (isVolatile) {
        unsafeSymRefs = javaStaticReference ? _unsafeJavaStaticVolatileSymRefs : _unsafeVolatileSymRefs;
    } else {
        unsafeSymRefs = javaStaticReference ? _unsafeJavaStaticSymRefs : _unsafeSymRefs;
    }
...
TR::SymbolReference* J9::SymbolReferenceTable::findOrCreateUnsafeSymbolRef(
    TR::DataType type, bool javaObjectReference, bool javaStaticReference, bool isVolatile)
{
    TR_Array<TR::SymbolReference*>* unsafeSymRefs = NULL;
        ...
        if (javaStaticReference) {
            if (_unsafeJavaStaticVolatileSymRefs == NULL)
                _unsafeJavaStaticVolatileSymRefs
                    = new (trHeapMemory()) TR_Array<TR::SymbolReference*>(comp()->trMemory(), TR::NumTypes);
...
TR_ImmutableInfo* J9::SymbolReferenceTable::findOrCreateImmutableInfo(TR_OpaqueClassBlock* clazz)
{
    ...
    TR_ImmutableInfo* newImmutableInfo = new (trHeapMemory()) TR_ImmutableInfo(
        clazz, new (trHeapMemory()) TR_BitVector(_size_hint, comp()->trMemory(), heapAlloc, growable), NULL);
    ...
    if (true) {
        if (!symRef->getSymbol()->isArrayShadowSymbol()) {
            ...
            if (sym->isPrivate() || sym->isFinal()) {
                ...
                TR_OpaqueClassBlock* classOfStatic = comp()->fe()->getClassFromSignature(
                    classNameOfFieldOrStatic, len, symRef->getOwningMethod(comp()));
...
TR::SymbolReference* J9::SymbolReferenceTable::findOrCreateImmutableGenericIntShadowSymbolReference(intptrj_t offset)
{
...
TR::SymbolReference* J9::SymbolReferenceTable::findOrCreateProfilingBufferSymbolRef(intptrj_t offset)
{
...
TR::SymbolReference* J9::SymbolReferenceTable::findShadowSymbol(
    TR_ResolvedMethod* owningMethod, int32_t cpIndex, TR::DataType type, TR::Symbol::RecognizedField* recognizedField)
{
    TR::SymbolReference* symRef;
    TR_SymRefIterator i(type == TR::Address
            ? aliasBuilder.addressShadowSymRefs()
    ...
    while (symRef = i.getNext()) {
        if ((recognizedField && *recognizedField != TR::Symbol::UnknownField
            ...
            if (cpIndex != -1 && owningMethod->classOfMethod() != symRef->getOwningMethod(comp())->classOfMethod()) {
                ...
                bool resolved = owningMethod->fieldAttributes(
                    comp(), cpIndex, &offset, &type, &isVolatile, &isFinal, &isPrivate, false, &isUnresolvedInCP, true);
                symRef->setHasBeenAccessedAtRuntime(isUnresolvedInCP ? TR_no : TR_maybe);
            }
            return symRef;
        }
...
TR::ParameterSymbol* J9::SymbolReferenceTable::createParameterSymbol(
