...
public:
    ...
    TR_ImmutableInfo(
        TR_OpaqueClassBlock* clazz, TR_BitVector* immutableSymRefs, TR_BitVector* immutableConstructorDefAliases)
        : _clazz(clazz)
...
public:
    ...
    TR::SymbolReference* createSystemRuntimeHelper(
        TR_RuntimeHelper, bool = false, bool = false, bool preservesAllRegisters = false);
    ...
    TR::SymbolReference* findOrCreateInterfaceMethodSymbol(
        TR::ResolvedMethodSymbol* owningMethodSymbol, int32_t cpIndex);
    TR::SymbolReference* findOrCreateDynamicMethodSymbol(
    ...
    TR::SymbolReference* findOrCreateHandleMethodSymbol(
        TR::ResolvedMethodSymbol* owningMethodSymbol, int32_t cpIndex, char* signature);
    TR::SymbolReference* findOrCreateCallSiteTableEntrySymbol(
        TR::ResolvedMethodSymbol* owningMethodSymbol, int32_t callSiteIndex);
    TR::SymbolReference* findOrCreateMethodTypeTableEntrySymbol(
        TR::ResolvedMethodSymbol* owningMethodSymbol, int32_t cpIndex);
    TR::SymbolReference* findOrCreateVarHandleMethodTypeTableEntrySymbol(
        TR::ResolvedMethodSymbol* owningMethodSymbol, int32_t cpIndex);
    TR::SymbolReference* methodSymRefWithSignature(
        TR::SymbolReference* original, char* effectiveSignature, int32_t effectiveSignatureLength);
    ...
    TR::SymbolReference* findOrCreateJavaLangReferenceReferentShadowSymbol(
        TR::ResolvedMethodSymbol*, bool, TR::DataType, uint32_t, bool);
    ...
    TR::SymbolReference* findOrFabricateShadowSymbol(TR::ResolvedMethodSymbol* owningMethodSymbol,
        ...
        bool isPrivate, bool isFinal, char* name = NULL);
    ...
    TR::SymbolReference* findOrCreateStaticSymbol(
        TR::ResolvedMethodSymbol* owningMethodSymbol, int32_t cpIndex, bool isStore);
     ...
     */
    TR::SymbolReference* findUnsafeSymbolRef(
        TR::DataType type, bool javaObjectReference = false, bool javaStaticReference = false, bool isVolatile = false);
     ...
     */
    TR::SymbolReference* findOrCreateUnsafeSymbolRef(
        TR::DataType type, bool javaObjectReference = false, bool javaStaticReference = false, bool isVolatile = false);

    TR::SymbolReference* findOrCreateImmutableGenericIntShadowSymbolReference(
        intptrj_t offset); // "Immutable" means no aliasing issues; ie. reads from these shadows can be freely reordered
                           // wrt anything else
...
private:
    TR::SymbolReference* createShadowSymbolWithoutCpIndex(
        TR::ResolvedMethodSymbol*, bool, TR::DataType, uint32_t, bool);
    TR::SymbolReference* findJavaLangReferenceReferentShadowSymbol(
        TR_ResolvedMethod* owningMethod, TR::DataType, uint32_t);
