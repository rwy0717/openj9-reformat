...
TR_OptimizationPlan* TR::DefaultCompilationStrategy::processEvent(TR_MethodEvent* event, bool* newPlanCreated)
{
    ...
    case TR_MethodEvent::InterpreterCounterTripped:
        ...
        if (hotnessLevel == veryHot && // we probably want to profile
            !TR::Options::getCmdLineOptions()->getOption(TR_DisableProfiling) && TR::Recompilation::countingSupported()
    ...
    case TR_MethodEvent::JitCompilationInducedByDLT:
        ...
        *newPlanCreated = true;
        break;
    case TR_MethodEvent::OtherRecompilationTrigger: // sync recompilation through fixMethodCode or recomp triggered from
                                                    // jitted code (like counting recompilation)
        ...
        if (methodInfo->getReasonForRecompilation() == TR_PersistentMethodInfo::RecompDueToInlinedMethodRedefinition
            || (methodInfo->getReasonForRecompilation() == TR_PersistentMethodInfo::RecompDueToJProfiling
                   && !bodyInfo->getIsProfilingBody())) // if the recompilation is triggered from a JProfiling block but
                                                        // not in a profiled compilation keep the current compilation
                                                        // level unchanged
        {
...
TR_OptimizationPlan* TR::DefaultCompilationStrategy::processInterpreterSample(TR_MethodEvent* event)
{
    ...
    if (!TR::CompilationInfo::isCompiled(j9method)) {
        ...
        if (!cmdLineOptions->getOption(TR_DisableInterpreterSampling)) {
            ...
            if (count > 0) {
                ...
                bool isJSR292Method = _J9ROMMETHOD_J9MODIFIER_IS_SET(
                    (J9_ROM_METHOD_FROM_RAM_METHOD(j9method)), J9AccMethodHasMethodHandleInvokes);
            ...
            } else // count==-1
            {
                if (TR::CompilationInfo::getJ9MethodVMExtra(j9method) == J9_JIT_QUEUED_FOR_COMPILATION) {
                    ...
                    if (logSampling)
                        curMsg += sprintf(curMsg, " cannot be compiled, extra field is %08X",
...
TR_OptimizationPlan* TR::DefaultCompilationStrategy::processJittedSample(TR_MethodEvent* event)
{
    TR_OptimizationPlan* plan = 0;
    TR::Options* cmdLineOptions = TR::Options::getCmdLineOptions();
    J9Method* j9method = event->_j9method;
...
#define SIG_SZ 150
        ...
        if (logSampling)
            curMsg += sprintf(curMsg, "(%d)\tCompiled %s\tPC=" POINTER_PRINTF_FORMAT "\t%+d\t", totalSampleCount, sig,
                startPC, pcOffset);
        if (TrcEnabled_Trc_JIT_Sampling && ((totalSampleCount % 4) == 0))
    ...
    if (bodyInfo) {
        ...
        void* currentStartPC
            = TR::CompilationInfo::isCompiled((J9Method*)j9m) ? (void*)TR::Compiler->mtd.startPC(j9m) : NULL;
                ...
                != -1) // prevent recompilation when opt level is specified
        {
            ...
            isAlreadyBeingCompiled
                = TR::Recompilation::isAlreadyBeingCompiled(methodInfo->getMethodInfo(), startPC, fe);
        ...
        if (getOut) {
            ...
            if (logSampling)
                curMsg += sprintf(curMsg,
                    " cnt=%d ncl=%d glblSmplCnt=%d startCnt=%d[-%u,+%u] samples=[%d %d] windows=[%d %u] "
                    "crtSmplIntrvlCnt=%u",
                    count, methodInfo->getNextCompileLevel(), totalSampleCount, startSampleCount,
                    bodyInfo->getOldStartCountDelta(), bodyInfo->getHotStartCountDelta(), globalSamples,
                    globalSamplesInHotWindow, scorchingSampleInterval, hotSampleInterval, crtSampleIntervalCount);
                ...
                if (!isAlreadyBeingCompiled) {
                    ...
                    } else // allow transition to HOT through exhaustion of count
                    {
                        ...
                        if (count < 0 && !methodInfo->disableMiscSamplingCounterDecrementation()) {
                            ...
                            methodInfo->setReasonForRecompilation(
                                TR_PersistentMethodInfo::RecompDueToMegamorphicCallProfile);
                        } else {
            ...
            if (!recompile && hotSamplingWindowComplete && totalSampleCount > startSampleCount) {
                ...
                bool isBigAppStartup
                    = (jitConfig->javaVM->phase != J9VM_PHASE_NOT_STARTUP && TR::Options::sharedClassCache()
                        && compInfo->getPersistentInfo()->getNumLoadedClasses() >= TR::Options::_bigAppThreshold
                ...
                if (isBigAppStartup) {
                    ...
                    useAggressiveRecompilations = false; // also to avoid potential hot recomps, this could have been
                                                         // set
                }
                ...
                if (useAggressiveRecompilations) {
                    int32_t samplesInSelf
                        = scorchingSamplingWindowComplete ? scorchingSampleInterval : crtSampleIntervalCount;
                ...
                if (globalSamplesInHotWindow <= scaledHotThreshold || secondCriteriaHot) {
                    ...
                    if (isBigAppStartup) {
                        scaledScorchingThreshold
                            >>= TR::Options::_bigAppSampleThresholdAdjust; // adjust to avoid scorching recomps
                ...
                if (recompile) {
                    ...
                    } else // the method is already being compiled; maybe we need to update the opt level
                    {
                        ...
                        if ((int32_t)nextOptLevel > (int32_t)methodInfo->getNextCompileLevel()) {
                            ...
                            TR_MethodToBeCompiled* entry = compInfo->adjustCompilationEntryAndRequeue(
                                details, methodInfo, nextOptLevel, useProfiling, CP_ASYNC_NORMAL, fe);
                            if (entry) {
                                if (logSampling)
                                    curMsg += sprintf(curMsg, " adj opt lvl to %d",
                                        (int32_t)(entry->_optimizationPlan->getOptLevel()));
                                int32_t measuredCpuUtil = crtSampleIntervalCount == 0
            ...
            if (!recompile) {
                if (bodyInfo->getFastRecompilation() && !isAlreadyBeingCompiled) {
                    ...
                    if (bodyInfo->getFastScorchingRecompilation()) {
                        if (!TR::Options::getCmdLineOptions()->getOption(TR_DisableProfiling)
                            && TR::Recompilation::countingSupported() && !(methodInfo->profilingDisabled())) {
                            nextOptLevel = veryHot;
                            useProfiling = true;
                        } else {
                            nextOptLevel = scorching;
                ...
                } else if (!postponeDecision && !TR::Options::getCmdLineOptions()->getOption(TR_DisableUpgrades) &&
                    // case (1) methods downgraded to cold
                    ((bodyInfo->getHotness() < warm
                         && (methodInfo->isOptLevelDowngraded()
                                || cmdLineOptions->getOption(TR_EnableUpgradingAllColdCompilations)))
                        ||
                ...
                // case (3) cold compilations for bootstrap methods, even if not downgraded
                {
                    ...
                    if ((uint32_t)crtSampleIntervalCount >= threshold
                        ...
                        && !compInfo->getPersistentInfo()->isClassLoadingPhase() && !isAlreadyBeingCompiled
                        ...
                        } else // AOT bodies
                        {
                            ...
                            } else // -Xquickstart (and AOT)
                            {
                                ...
                                if (cmdLineOptions->getOption(TR_UpgradeBootstrapAtWarm)
                                    && fe->isClassLibraryMethod((TR_OpaqueMethodBlock*)j9method)) {
#ifndef PUBLIC_BUILD
#if defined(HINTS_IN_SHAREDCACHE_OBJECT)
                                    bool expensiveComp = (TR_J9SharedCache*)(((TR_J9VMBase*)fe)->sharedCache())
                                                             ->isHint(j9method, TR_HintLargeMemoryMethodW);
#else
                                    bool expensiveComp
                                        = ((TR_J9VMBase*)fe)->isSharedCacheHint(j9method, TR_HintLargeMemoryMethodW);
#endif
                                    if (!expensiveComp)
#endif //! PUBLIC_BUILD
                                        nextOptLevel = warm;
            ...
            if (recompile) {
                ...
                if (plan) {
                    ...
                    if (logSampling) {
                        ...
                        if (useProfiling)
                            curMsg += sprintf(
                                curMsg, " --> recompile at level %d, profiled CPU=%.1f%%", nextOptLevel, cpu);
                        else
                        ...
                        if (methodInfo->getReasonForRecompilation() == TR_PersistentMethodInfo::RecompDueToThreshold) {
                            curMsg += sprintf(
                                curMsg, " scaledThresholds=[%d %d]", scaledScorchingThreshold, scaledHotThreshold);
                        }
                    }
                } else // OOM
                {
                ...
                if (isAlreadyBeingCompiled)
                    ...
                    curMsg
                        += sprintf(curMsg, " not recompiled, CPU=%.1f%% %s scaledThresholds=[%d %d]", measuredCpuUtil,
                            postponeDecision ? " postpone decision" : "", scaledScorchingThreshold, scaledHotThreshold);
                }
            }
        }
    } // endif (bodyInfo)
...
TR_OptimizationPlan* TR::DefaultCompilationStrategy::processHWPSample(TR_MethodEvent* event)
{
...
TR::ThresholdCompilationStrategy::ThresholdCompilationStrategy()
{
    ...
    //
    // The current strategy uses only noOpt -> warm -> scorching.   (and veryHot if instrumentation-based profiling is
    // used)
    ...
    if (!TR::Options::getCmdLineOptions()->getOption(TR_DisableProfiling)) {
        ...
        _samplesNeededToMoveTo[scorching] = SCORCHING_THRESH + 1; // Sampling is disable during instrumentation-based
                                                                  // profiling, so this is really just a place holder
    }
    ...
    for (int32_t curLevel = numHotnessLevels; curLevel >= noOpt; // should be "> minHotness" if it existed
         curLevel--) {
...
TR_Hotness TR::ThresholdCompilationStrategy::getInitialOptLevel() { return noOpt; }
...
TR_OptimizationPlan* TR::ThresholdCompilationStrategy::processJittedSample(TR_MethodEvent* event)
{
    TR_OptimizationPlan* plan = NULL;
    TR::Options* cmdLineOptions = TR::Options::getCmdLineOptions();
    J9Method* j9method = event->_j9method;
    ...
    if (bodyInfo) {
                ...
                != -1) // prevent recompilation when opt level is specified
        {
            ...
            if ((nextOptLevel != unknownHotness) && (sampleCount == getSamplesNeeded(nextOptLevel))) {
                ...
                plan = TR_OptimizationPlan::alloc(nextOptLevel, getPerformInstrumentation(nextOptLevel), useSampling);
            }
        }
    }
    return plan;
