...
public:
    ...
    int32_t getSamplesNeeded(TR_Hotness optLevel)
    {
        TR_ASSERT(_samplesNeededToMoveTo[optLevel] >= 0, "must be valid entry");
        return _samplesNeededToMoveTo[optLevel];
    }
    bool getPerformInstrumentation(TR_Hotness optLevel) { return _performInstrumentation[optLevel]; }
    TR_Hotness getNextOptLevel(TR_Hotness curOptLevel)
    {
        TR_ASSERT(_nextLevel[curOptLevel] != unknownHotness, "must be valid opt level");
        return _nextLevel[curOptLevel];
    }
