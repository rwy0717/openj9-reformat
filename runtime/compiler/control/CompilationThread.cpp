...
IDATA J9THREAD_PROC protectedCompilationThreadProc(
    J9PortLibrary* portLib, TR::CompilationInfoPerThread* compInfoPT /*void *vmthread*/);
...
static UDATA blankDumpSignalHandler(struct J9PortLibrary* portLibrary, U_32 gpType, void* gpInfo, void* arg)
{
    J9VMThread* vmThread = (J9VMThread*)arg;
    TR_VerboseLog::writeLineLocked(
        TR_Vlog_JITDUMP, "vmThread=%p Recursive crash occurred. Aborting JIT dump.", vmThread);

    // Returning J9PORT_SIG_EXCEPTION_RETURN will make us come back to the same crashing instruction over and over
    //
    return J9PORT_SIG_EXCEPTION_RETURN; // FIXME: is this the right return type? - This appears to be the right return
                                        // type
}
#endif

static UDATA jitSignalHandler(struct J9PortLibrary* portLibrary, U_32 gpType, void* gpInfo, void* arg)
{
...
int32_t compThreadActivationThresholdsonStarvation[MAX_TOTAL_COMP_THREADS + 1]
    = { -1, 800, 1600, 3200, 6400, 12800, 19200, 25600, 32000 };
...
bool TR::CompilationInfo::shouldDowngradeCompReq(TR_MethodToBeCompiled* entry)
{
    ...
    if (!isCompiled(method) && /*entry->_priority <= CP_ASYNC_MAX &&*/
        ...
        !entry->isDLTCompile() && !TR::Options::getCmdLineOptions()->getOption(TR_DontDowngradeToCold)) {
        ...
        // perform JNI at cold
            ...
            (entry->_methodIsInSharedCache == TR_yes && entry->_doNotUseAotCodeFromSharedCache
        ...
        // downgrade during IDLE
            ...
            !(TR::Options::getCmdLineOptions()->getOption(TR_UseIdleTime) && getCpuUtil()
                && getCpuUtil()->isFunctional() && getCpuUtil()->getCpuUsage() < 10
...
TR::CompilationInfoPerThreadBase::CompilationInfoPerThreadBase(
    TR::CompilationInfo& compInfo, J9JITConfig* jitConfig, int32_t id, bool onSeparateThread)
    : _compInfo(compInfo)
...
TR::CompilationInfoPerThread::CompilationInfoPerThread(
    TR::CompilationInfo& compInfo, J9JITConfig* jitConfig, int32_t id, bool isDiagnosticThread)
...
bool TR::CompilationInfo::initializeCompilationOnApplicationThread()
{
    ...
    if (!_compInfoForCompOnAppThread) {
        ...
        _compInfoForCompOnAppThread
            = new (PERSISTENT_NEW) TR::CompilationInfoPerThreadBase(*this, _jitConfig, 0, false);
...
bool TR::CompilationInfo::asynchronousCompilation()
{
    static bool answer = (!TR::Options::getJITCmdLineOptions()->getOption(TR_DisableAsyncCompilation)
        && useSeparateCompilationThread() && TR::Options::getJITCmdLineOptions()->getInitialBCount()
...
const char* TR::CompilationInfo::OperationNames[]
    = { "EMPTY", "HasAcquiredCompilationMonitor", "WillReleaseCompilationMonitor", "WillNotifyCompilationMonitor",
          "WillWaitOnCompilationMonitor", "HasFinishedWaitingOnCompilationMonitor", "Changed state",
          "WillWaitOnSlotMonitorAfterCompMonRelease", "CompileOnSeparateThreadEnter", "INVALID" };

void TR::CompilationInfo::addCompilationTraceEntry(
    J9VMThread* vmThread, TR_CompilationOperations op, uint32_t otherData)
{
...
uint32_t TR::CompilationInfo::getMethodBytecodeSize(J9ROMMethod* romMethod)
{
...
bool TR::CompilationInfo::isJSR292(
    J9Method* j9method) // Check to see if the J9AccMethodHasMethodHandleInvokes flag is set
{
    return (
        _J9ROMMETHOD_J9MODIFIER_IS_SET((J9_ROM_METHOD_FROM_RAM_METHOD(j9method)), J9AccMethodHasMethodHandleInvokes));
}
...
bool TR_LowPriorityCompQueue::createLowPriorityCompReqAndQueueIt(
    TR::IlGeneratorMethodDetails& details, void* startPC, uint8_t reason)
{
...
bool TR_LowPriorityCompQueue::addUpgradeReqToLPQ(TR_MethodToBeCompiled* compReq)
{
    ...
    if (compReq->isDLTCompile() || !TR::Options::getCmdLineOptions()->allowRecompilation())
        return false;
    return createLowPriorityCompReqAndQueueIt(
        compReq->getMethodDetails(), compReq->_newStartPC, TR_MethodToBeCompiled::REASON_UPGRADE);
}
...
bool TR::CompilationInfo::canProcessLowPriorityRequest()
{
    ...
    {
        if ((getCpuUtil() && getCpuUtil()->isFunctional() && getCpuUtil()->getCpuIdle() > idleThreshold()
                && // enough idle time
                getJvmCpuEntitlement() - getCpuUtil()->getVmCpuUsage()
                    >= 200)) // at least 2 cpus should be not utilized
            return true;
    ...
    return (getCpuUtil() && getCpuUtil()->isFunctional() && getCpuUtil()->getCpuIdle() > idleThreshold()
        && // enough idle time
...
void TR::CompilationInfo::decNumInvReqestsQueued(TR_MethodToBeCompiled* entry)
{
    if (entry->_entryIsCountedAsInvRequest) {
        ...
        TR_ASSERT(
            _numInvRequestsInCompQueue >= 0, "_numInvRequestsInCompQueue is negative : %d", _numInvRequestsInCompQueue);
    }
}
...
void TR::CompilationInfo::invalidateRequestsForNativeMethods(J9Class* clazz, J9VMThread* vmThread)
{
    ...
    if (verbose)
        TR_VerboseLog::writeLineLocked(
            TR_Vlog_HK, "invalidateRequestsForNativeMethods class=%p vmThread=%p", clazz, vmThread);
        ...
        if (methodBeingCompiled && !methodBeingCompiled->_unloadedMethod) {
            ...
            if (method) {
                ...
                if (classOfMethod == clazz && TR::CompilationInfo::isJNINative(method)) {
                    ...
                    if (verbose)
                        TR_VerboseLog::writeLineLocked(
                            TR_Vlog_HK, "Have marked as unloaded the JNI thunk compilation for method %p", method);
                }
            }
        }
    } // end for
...
void TR::CompilationInfo::invalidateRequestsForUnloadedMethods(
    TR_OpaqueClassBlock* clazz, J9VMThread* vmThread, bool hotCodeReplacement)
{
        ...
        if (methodBeingCompiled && !methodBeingCompiled->_unloadedMethod) {
            ...
            if ((!unloadedClass && hotCodeReplacement) || // replacement in FSD mode
                ...
                if (!hotCodeReplacement) {
                    TR_ASSERT(
                        methodBeingCompiled->_priority < CP_SYNC_MIN, "Unloading cannot happen for SYNC requests");
                } else {
                    TR_ASSERT(
                        methodBeingCompiled->_numThreadsWaiting == 0 || methodBeingCompiled->_priority >= CP_SYNC_MIN,
    ...
    while (cur) {
        ...
        if (method) {
            ...
            if (verboseDetails)
                TR_VerboseLog::writeLineLocked(
                    TR_Vlog_HD, "Looking at compile request for method %p class %p", method, methodClass);
            if ((!unloadedClass && hotCodeReplacement) || // replacement in FSD mode
                ...
                if (verbose)
                    TR_VerboseLog::writeLineLocked(
                        TR_Vlog_HK, "Invalidating compile request for method %p class %p", method, methodClass);
                if (!hotCodeReplacement) {
        ...
        && useSeparateCompilationThread()) // Retrials for compilation on application thread are not implemented
    {
        if (entry->_compilationAttemptsLeft > 0) {
            ...
            case compilationLambdaEnforceScorching:
                // This must be a warm compilation. Retry the same compilation at scorching even if it's a first time
                // compilation Watch out for the following scenario: (1) warm comp req. (2) Ilgen fails comp because it
                // wants a hot comp (3) hot comp fails due to some compiler limit and we lower opt level at warm; (4)
                // Ilgen fails again because it wants a hot compilation Also we should not fail the compilation for
                // fixed opt level.
            ...
            if (tryCompilingAgain && comp) {
                ...
                if (methodInfo)
                    methodInfo->setNextCompileLevel(
                        entry->_optimizationPlan->getOptLevel(), entry->_optimizationPlan->insertInstrumentation());
            }
...
void TR::CompilationInfo::suspendCompilationThread()
{
    ...
    if (!useSeparateCompilationThread()) {
        if (_compInfoForCompOnAppThread->getCompilationThreadState() == COMPTHREAD_ACTIVE)
            _compInfoForCompOnAppThread->setCompilationThreadState(
                COMPTHREAD_SUSPENDED); // for compilation on application thread there is only one state
            ...
            if (curCompThreadInfoPT->compilationThreadIsActive()) {
                ...
                if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseCompilationThreads)) {
                    TR_VerboseLog::writeLineLocked(TR_Vlog_PERF,
                        "t=%6u Suspension request for compThread %d sleeping=%s",
...
void TR::CompilationInfo::resumeCompilationThread()
{
    if (useSeparateCompilationThread()) {
        ...
        if (numActiveCompThreads != getNumCompThreadsActive()) {
            TR_ASSERT(false,
                "Discrepancy regarding active compilation threads: numActiveCompThreads=%d "
                "getNumCompThreadsActive()=%d\n",
        ...
        if (!J9ROMCLASS_IS_PRIMITIVE_OR_ARRAY(clazz->romClass)) {
            ...
            for (int32_t m = 0; m < clazz->romClass->romMethodCount; m++) {
                ...
                if (!(romMethod->modifiers & (J9_JAVA_NATIVE | J9_JAVA_ABSTRACT)) && method != newInstanceThunk
                 ...
                 utf8 = J9ROMMETHOD_GET_NAME(J9_CLASS_FROM_METHOD(method)->romClass,
                 J9_ROM_METHOD_FROM_RAM_METHOD(method)); fprintf(stderr, ".%.*s", J9UTF8_LENGTH(utf8), (char *)
                 J9UTF8_DATA(utf8)); utf8 = J9ROMMETHOD_GET_SIGNATURE(J9_CLASS_FROM_METHOD(method)->romClass,
                 J9_ROM_METHOD_FROM_RAM_METHOD(method)); fprintf(stderr, "%.*s", J9UTF8_LENGTH(utf8), (char *)
                 J9UTF8_DATA(utf8));
                    */
                    ...
                    if (count < 0) {
                        ...
                        intptrj_t initialCount = J9ROMMETHOD_HAS_BACKWARDS_BRANCHES(romMethod)
...
void TR::CompilationInfo::stopCompilationThreads()
{
    ...
    if (aotStatsEnabled) {
        ...
        fprintf(
            stderr, "numRuntimeUnresolvedInstanceFieldFromCP: %d\n", aotStats->numRuntimeUnresolvedInstanceFieldFromCP);
        ...
        fprintf(
            stderr, "numRuntimeClassAddressButFailValidation: %d\n", aotStats->numRuntimeClassAddressButFailValidation);
        ...
        fprintf(
            stderr, "numRuntimeClassAddressReloUnresolvedCP: %d\n", aotStats->numRuntimeClassAddressReloUnresolvedCP);
...
IDATA J9THREAD_PROC protectedCompilationThreadProc(
    J9PortLibrary*, TR::CompilationInfoPerThread* compInfoPT /*void *vmthread*/)
{
...
void TR::CompilationInfoPerThread::processEntries()
{
    ...
    J9VMThread* compThread = getCompilationThread();
    try {
        J9::SegmentAllocator scratchSegmentAllocator(
            MEMORY_TYPE_JIT_SCRATCH_SPACE | MEMORY_TYPE_VIRTUAL, *_jitConfig->javaVM);
        ...
        while (getCompilationThreadState() == COMPTHREAD_ACTIVE) {
            ...
            TR_MethodToBeCompiled* entry = compInfo->getNextMethodToBeCompiled(
                this, _previousCompilationThreadState == COMPTHREAD_WAITING, &compThreadAction);
            ...
            case TR::CompilationInfo::GO_TO_SLEEP_EMPTY_QUEUE: {
                ...
                if (getCompilationThreadState() == COMPTHREAD_WAITING) {
                     ...
                     */
                    {
                        ...
                        if (TR::Options::getVerboseOption(TR_VerboseCompilationDispatch)) {
                            TR_VerboseLog::writeLineLocked(TR_Vlog_DISPATCH,
                                "compThread=%d woke up after timeout on compMonitor\n", getCompThreadId());
                        }
                    }
                }
                break;
...
void TR::CompilationInfoPerThread::processEntry(
    TR_MethodToBeCompiled& entry, J9::J9SegmentProvider& scratchSegmentProvider)
{
    TR::CompilationInfo* compInfo = getCompilationInfo();
    J9VMThread* compThread = getCompilationThread();
    TR::IlGeneratorMethodDetails& details = entry.getMethodDetails();
    ...
    // enabled.
    jobject classObject = compThread->javaVM->internalVMFunctions->j9jni_createLocalRef(
        (JNIEnv*)compThread, J9VM_J9CLASS_TO_HEAPCLASS(details.getClass()));
    ...
    } else // compilation will not be retried
    {
        ...
        // entry->_optimizationPlan->creatorCanFreePlan());
        ...
        && (compInfo->getRampDownMCT() // force to have only one thread active
               ...
               || (!tryCompilingAgain
                      ...
                      && compInfo->getQueueWeight()
                          < compThreadSuspensionThresholds[compInfo->getNumCompThreadsActive()]))) {
        // Suspend this thread
        setCompilationThreadState(COMPTHREAD_SIGNAL_SUSPEND);
        compInfo->decNumCompThreadsActive();
        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseCompilationThreads)) {
...
bool TR::CompilationInfoPerThread::shouldPerformCompilation(TR_MethodToBeCompiled& entry)
{
    ...
    if (entry._reqFromSecondaryQueue) {
        ...
        {
            TR_ASSERT(
                entry._reqFromSecondaryQueue == TR_MethodToBeCompiled::REASON_UPGRADE, "wrong reason for upgrade");
    ...
    TR_YesNoMaybe methodIsInSharedCache)
{
        ...
        if (cur->_optimizationPlan->getOptLevel() != optimizationPlan->getOptLevel()) {
            ...
            if (pc) {
                ...
                if (methodInfo && methodInfo->getNextCompileLevel() != cur->_optimizationPlan->getOptLevel())
                    methodInfo->setNextCompileLevel(
                        cur->_optimizationPlan->getOptLevel(), cur->_optimizationPlan->insertInstrumentation());
            }
        }
    ...
    //
        ...
        if (queueWeight != _queueWeight) // QW
        {
            if (TR::Options::isAnyVerboseOptionSet())
                TR_VerboseLog::writeLineLocked(TR_Vlog_INFO,
                    "Discrepancy for queue weight while adding to queue: computed=%u recorded=%u\n", queueWeight,
                    _queueWeight);
            // correction
        ...
        } else if (details.isOrdinaryMethod()) // ordinary interpreted methods
        {
            ...
            if (method && async && (getInvocationCount(method) == 0) // this will filter out JNI natives
    ...
    && (defined(TR_HOST_X86) || defined(TR_HOST_POWER) || defined(TR_HOST_S390) || defined(TR_HOST_ARM))
        ...
        *queued = true; // set the flag that we added a new request to the queue
        ...
        Trc_JIT_CompRequest(vmThread, details.getMethod(), pc, !async, optimizationPlan->getOptLevel(), (int)priority,
            _numQueuedMethods);
            ...
            && getNumCompThreadsActive() == 1 && priority >= CP_ASYNC_BELOW_MAX) // AOT loads and JNI
        {
            ...
            if (lowPriCompThread) {
                ...
                uint8_t targetWeight
                    = compThreadSuspensionThresholds[2] <= 0xff ? compThreadSuspensionThresholds[2] : 0xff;
        ...
        if (activate == TR_yes) {
            ...
            // fprintf(stderr, "Activate compthread %d: _numQueuedMethods=%d queueWeight=%d getNumCompThreadsJobless=%d
            // numactive=%d\n",
            //   compInfoPT->getCompThreadId(), _numQueuedMethods, _queueWeight, getNumCompThreadsJobless(),
            //   getNumCompThreadsActive());
        }
    }
...
TR_MethodToBeCompiled* TR::CompilationInfo::requestExistsInCompilationQueue(
    TR::IlGeneratorMethodDetails& details, TR_FrontEnd* fe)
{
...
TR_MethodToBeCompiled* TR::CompilationInfo::getNextMethodToBeCompiled(
    TR::CompilationInfoPerThread* compInfoPT, bool compThreadCameOutOfSleep, TR_CompThreadActions* compThreadAction)
{
            ...
            if (!m) {
                *compThreadAction = GO_TO_SLEEP_CONCURRENT_EXPENSIVE_REQUESTS;
                ...
                TR_ASSERT(numHot >= 1 || numLowPriority >= 1,
                    "We must have a comp thread working on a hot or low priority method %d %d", numHot, numLowPriority);
...
void* TR::CompilationInfo::startPCIfAlreadyCompiled(
    J9VMThread* vmThread, TR::IlGeneratorMethodDetails& details, void* oldStartPC)
{
    if (details.isNewInstanceThunk()) {
        return jitNewInstanceMethodStartAddress(
            vmThread, static_cast<J9::NewInstanceThunkDetails&>(details).getClass());
...
static void deleteMethodHandleRef(J9::MethodHandleThunkDetails& details, J9VMThread* vmThread, TR_FrontEnd* fe)
{
    ...
    if (verboseDetails) {
        ...
        TR_VerboseLog::writeLineLocked(
            TR_Vlog_MHD, "%p   Deleting MethodHandle %p global reference", vmThread, (j9object_t)methodHandle);
    }

    vmThread->javaVM->internalVMFunctions->j9jni_deleteGlobalRef(
        (JNIEnv*)vmThread, (jobject)details.getHandleRef(), false);
    if (details.getArgRef())
        vmThread->javaVM->internalVMFunctions->j9jni_deleteGlobalRef(
            (JNIEnv*)vmThread, (jobject)details.getArgRef(), false);
}
    ...
    TR_OptimizationPlan* optimizationPlan)
{
    ...
    //
    if ((_jitConfig->runtimeFlags & J9JIT_SCAVENGE_ON_RESOLVE) && !(_jitConfig->runtimeFlags & J9JIT_TESTMODE)
    ...
    if (needCompile) {
        ...
        if (useSeparateCompilationThread() && !fe->isAOT_DEPRECATED_DO_NOT_USE())
            startPC = compileOnSeparateThread(
                vmThread, details, oldStartPC, requireAsyncCompile, compErrCode, queued, optimizationPlan);
        else
    ...
    if (!useSeparateCompilationThread()) {
        ...
        TR_ASSERT(!heldMonitor, "After taking a longjmp comp thread is holding TR::Monitor %p called %s\n", heldMonitor,
            ((TR_J9VMBase*)fe)->getJ9MonitorName((J9ThreadMonitor*)heldMonitor->getVMMonitor()));
...
#if defined(J9VM_OPT_SHARED_CLASSES)
extern bool validateSharedClassAOTHeader(
    J9JavaVM* javaVM, J9VMThread* curThread, TR::CompilationInfo* compInfo, TR_FrontEnd* fe);
#endif
    ...
    TR_OptimizationPlan* optimizationPlan)
{
    ...
    if (startPC && !details.isDumpMethod() && !optimizationPlan->isGPUCompilation()) {
    ...
    if ((getNumCompThreadsActive() == 0)
        || (getPersistentInfo()->getDisableFurtherCompilation() && oldStartPC == 0 && !details.isDumpMethod())) {
    ...
    if (TR::Options::sharedClassCache() && !TR::Options::getAOTCmdLineOptions()->getOption(TR_NoLoadAOT)
        ...
        if (!isJNINative(method) && !oldStartPC) {
            ...
            //
            if (javaVM->sharedClassConfig->existsCachedCodeForROMMethod(
                    vmThread, J9_ROM_METHOD_FROM_RAM_METHOD(method))) {
                TR_J9SharedCacheVM* fe
                    = (TR_J9SharedCacheVM*)TR_J9VMBase::get(jitConfig, vmThread, TR_J9VMBase::AOT_VM);
                if (static_cast<TR_JitPrivateConfig*>(jitConfig->privateConfig)->aotValidHeader == TR_yes
                    ...
                    //             TR::Options::getCmdLineOptions()->getOption(TR_ForceLoadAOT))
    ...
    TR_MethodToBeCompiled* entry = addMethodToBeCompiled(
        details, oldStartPC, compPriority, async, optimizationPlan, queued, methodIsInSharedCache);
    ...
    } else // synchronous request
    {
        ...
        TR_ASSERT_FATAL(
            !(entry->_freeTag & (ENTRY_DEALLOCATED | ENTRY_IN_POOL_FREE)), "Java thread waking up with a freed entry");
...
void* TR::CompilationInfo::compileOnApplicationThread(J9VMThread* vmThread, TR::IlGeneratorMethodDetails& details,
    void* oldStartPC, TR_CompilationErrorCode* compErrCode, TR_OptimizationPlan* optimizationPlan)
{
    ...
    if (_compInfoForCompOnAppThread->getCompilationThreadState() == COMPTHREAD_ACTIVE) {
        ...
        if (details.isNewInstanceThunk())
            setJ9MethodVMExtra(method,
                reinterpret_cast<uintptr_t>(static_cast<J9::NewInstanceThunkDetails&>(details).getClass())
                    | J9_STARTPC_NOT_TRANSLATED);

        if (getPersistentInfo()->isClassLoadingPhase()
            && !TR::Options::getCmdLineOptions()->getOption(TR_DontDowngradeToCold) && !isCompiled(method))
            TR::CompilationController::getCompilationStrategy()->adjustOptimizationPlan(
                &methodEntry, -1); // decrease opt level
    ...
    && (defined(TR_HOST_X86) || defined(TR_HOST_POWER) || defined(TR_HOST_S390) || defined(TR_HOST_ARM))
       ...
       // defined(TR_HOST_POWER) || defined(TR_HOST_S390) || defined(TR_HOST_ARM))
        ...
        if (oldStartPC) {
            TR_PersistentJittedBodyInfo* bodyInfo = TR::Recompilation::getJittedBodyInfoFromPC(oldStartPC);
            if (bodyInfo)
                bodyInfo->getMethodInfo()->setNextCompileLevel(
                    optimizationPlan->getOptLevel(), optimizationPlan->insertInstrumentation());
        }

        J9::SegmentAllocator scratchSegmentProvider(
            MEMORY_TYPE_JIT_SCRATCH_SPACE | MEMORY_TYPE_VIRTUAL, *_jitConfig->javaVM);
        startPC = _compInfoForCompOnAppThread->compile(vmThread, &methodEntry, scratchSegmentProvider);

        if (compErrCode)
            *compErrCode
                = (TR_CompilationErrorCode)methodEntry._compErrCode; // communicate the error code to the caller
...
void TR::CompilationInfoPerThreadBase::outputVerboseMMapEntries(
    TR_ResolvedMethod* compilee, TR_MethodMetaData* metaData, const char* profiledString, const char* compileString)
{
    ...
    TR_MethodToBeCompiled* entry, TR::Compilation* compiler)
{
    ...
    metaData = reloRuntime()->prepareRelocateAOTCodeAndData(
        vmThread, fe, aotMCCRuntimeCodeCache, cacheEntry, method, false, options, compiler, compilee);
    ...
    if (metaData) {
        ...
        if (TR::Options::isAnyVerboseOptionSet(TR_VerbosePerformance, TR_VerboseCompileEnd)) {
            ...
            TR_VerboseLog::write(
                " @ " POINTER_PRINTF_FORMAT "-" POINTER_PRINTF_FORMAT, metaData->startPC, metaData->endWarmPC);
    ...
    } else // relocation failed
    {
        if (entry) {
            ...
            entry->_tryCompilingAgain = _compInfo.shouldRetryCompilation(
                entry, compiler); // this will set entry->_doNotUseAotCodeFromSharedCache = true;
        ...
        if (_jitConfig->javaVM->phase == J9VM_PHASE_NOT_STARTUP
            ...
            if (!TR::Options::isQuickstartDetected())
                hotness = warm;
                         ...
                         TR_UpgradeBootstrapAtWarm)) // This is a JIT option because it perteins to JIT compilations
            {
...
#if defined(LINUX)
    TR_ASSERT(
        getCompilation() && getMetadata(), "generatePerfToolEntry() must be executed only for successful compilations");
    ...
    if (getPerfFile()) {
        ...
        if (getMetadata()->startColdPC) {
            j9jit_fprintf(getPerfFile(), "%p %lX %s_%s\n", getMetadata()->startColdPC,
                ...
                getCompilation()->getHotnessName(
                    getCompilation()->getMethodHotness())); // should we change the name of the method?
        }
 ...
 */
void TR::CompilationInfoPerThreadBase::preCompilationTasks(J9VMThread* vmThread, J9JavaVM* javaVM,
    TR_MethodToBeCompiled* entry, J9Method* method, const void** aotCachedMethod, TR_Memory& trMemory,
    bool& canDoRelocatableCompile, bool& eligibleForRelocatableCompile, TR_RelocationRuntime* reloRuntime)
{
    ...
    if (entry->_methodIsInSharedCache == TR_yes && !TR::CompilationInfo::isCompiled(method)
        && !entry->_doNotUseAotCodeFromSharedCache && !TR::Options::getAOTCmdLineOptions()->getOption(TR_NoLoadAOT)
        && !(jitConfig->runtimeFlags & J9JIT_TOSS_CODE)) {
        /*      if ((getPersistentInfo()->isClassLoadingPhase() ||
         * TR::Options::getAOTCmdLineOptions()->getOption(TR_ForceLoadAOT))) // use relocated code only during class
         * load phase */
        {
            ...
            if (canRelocateMethod && !entry->_oldStartPC) {
                UDATA flags = 0;
                *aotCachedMethod = javaVM->sharedClassConfig->findCompiledMethodEx1(
                    vmThread, J9_ROM_METHOD_FROM_RAM_METHOD(method), &flags);
                ...
                if (oldMetaData) {
                    ...
                    TR_ASSERT(oldBodyInfo && oldBodyInfo->getIsInvalidated(),
                        "entry is interpreted and has a non-NULL _oldStartPC but the old body has not been "
                        "invalidated.");
                }
            }
        }
    ...
    if (!entry->_aotCodeToBeRelocated) {
        ...
        TR::IlGeneratorMethodDetails& details = entry->getMethodDetails();
        eligibleForRelocatableCompile = TR::Options::sharedClassCache() && !details.isNewInstanceThunk()
            && !entry->isJNINative() && !details.isMethodHandleThunk() && !TR::CompilationInfo::isCompiled(method)
            && !entry->isDLTCompile() && !entry->_doNotUseAotCodeFromSharedCache
            ...
            && (!TR::Options::getAOTCmdLineOptions()->getOption(TR_AOTCompileOnlyFromBootstrap)
                       || TR_J9VMBase::get(jitConfig, vmThread)->isClassLibraryMethod((TR_OpaqueMethodBlock*)method),
                   true);

        bool sharedClassTest
            = eligibleForRelocatableCompile && !TR::Options::getAOTCmdLineOptions()->getOption(TR_NoStoreAOT);
    ...
    bool eligibleForRelocatableCompile, TR_RelocationRuntime* reloRuntime)
{
    ...
    if (entry->_unloadedMethod) // method was unloaded while we were trying to compile it
    {
        ...
        TR_ASSERT(entry->_compErrCode == compilationInterrupted,
            "if method was unloaded we must return we an error code of compilationInterrupted");
    ...
    {
        TR_PersistentJittedBodyInfo* bodyInfo;
        if (entry->isDLTCompile() && !startPC && TR::CompilationInfo::isCompiled(method)
            && // DLT compilation that failed too many times
            (bodyInfo = TR::Recompilation::getJittedBodyInfoFromPC(
                 method->extra))) // do not use entry->_oldStartPC which is probably 0. Use the most up-to-date startPC
        ...
        startPC = TR::CompilationInfo::compilationEnd(vmThread, entry->getMethodDetails(), jitConfig,
            metaData ? reinterpret_cast<void*>(metaData->startPC) : 0, entry->_oldStartPC, _vm, entry, _compiler);
            ...
            if (!(romMethod->modifiers & J9AccNative)) // Never change the extra field of a native method
            {
                ...
                if (methodVMExtra == 1 || methodVMExtra == J9_JIT_QUEUED_FOR_COMPILATION) {
                    ...
                    if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance)) {
                        ...
                        TR_VerboseLog::writeLineLocked(
                            TR_Vlog_PERF, "Reencoding count=%d for %s j9m=%p ", count, _compiler->signature(), method);
                    }
                } else {
 ...
 */
void* TR::CompilationInfoPerThreadBase::compile(
    J9VMThread* vmThread, TR_MethodToBeCompiled* entry, J9::J9SegmentProvider& scratchSegmentProvider)
{
    ...
    try {
        ...
        preCompilationTasks(vmThread, javaVM, entry, method, &aotCachedMethod, trMemory, canDoRelocatableCompile,
        ...
        CompileParameters compParam(this, _vm, vmThread, reloRuntime, entry->_optimizationPlan, regionSegmentProvider,
            dispatchRegion, trMemory, TR::CompileIlGenRequest(entry->getMethodDetails()));

        if ((TR::Options::canJITCompile() || canDoRelocatableCompile || entry->_aotCodeToBeRelocated)
            ...
            if (!noSignalWrapper && j9sig_can_protect(flags)) {
                ...
                if (entry->getMethodDetails().isDumpMethod()) {
                    ...
                    protectedResult = j9sig_protect((j9sig_protected_fn)wrappedCompile, static_cast<void*>(&compParam),
                        (j9sig_handler_fn)blankDumpSignalHandler, vmThread, flags, &result);
                } else {
                    protectedResult = j9sig_protect((j9sig_protected_fn)wrappedCompile, static_cast<void*>(&compParam),
                        (j9sig_handler_fn)jitSignalHandler, vmThread, flags, &result);
                }
...
TR_MethodMetaData* TR::CompilationInfoPerThreadBase::wrappedCompile(J9PortLibrary* portLib, void* opaqueParameters)
{
    ...
    try {
        ...
        if (details.isMethodHandleThunk()) {
            ...
            compilee
                = vm->createResolvedMethod(p->trMemory(), method, NULL, (TR_OpaqueClassBlock*)niDetails.getClass());
        } else {
        ...
        if (!that->methodCanBeCompiled(p->trMemory(), vm, compilee, filterInfo)) {
            ...
            if (options->getVerboseOption(TR_VerboseCompileExclude)) {
                ...
                if (jitConfig->runtimeFlags & J9JIT_TESTMODE)
                    ...
                    TR_VerboseLog::writeLine(
                        TR_Vlog_COMPFAIL, "%s cannot be translated", compilee->signature(p->trMemory()));
                TR_VerboseLog::vlogRelease();
            }
            ...
            //
            options = new (p->trMemory(), heapAlloc) TR::Options(p->trMemory(), optionSetIndex, lineNumber, compilee,
                that->_methodBeingCompiled->_oldStartPC, p->_optimizationPlan,
            ...
            if (!that->_methodBeingCompiled->_aotCodeToBeRelocated && !vm->isAOT_DEPRECATED_DO_NOT_USE()
                && options->getOption(TR_EnableAnnotations)) // KEN
            {
                if (!TR_AnnotationBase::scanForKnownAnnotationsAndRecord(
                        &that->_compInfo, details.getMethod(), vmThread->javaVM, vm))
            ...
            if (threadCompInfo && threadCompInfo->isDiagnosticThread() && options->getDebug()) {
                // Trace All
                ...
                if ((isValidVmStateIndex(index)) && (index == ((J9VMSTATE_JIT_CODEGEN >> 16) & 0xF))) {
            ...
            if (!that->_methodBeingCompiled->_aotCodeToBeRelocated) // exclude AOT loads
            {
                ...
                if (!options->getOption(TR_DisableSamplingJProfiling)) {
                    ...
                    // TODO: should we prevent SamplingJProfiling for AOT bodies?
                    {
                        ...
                        if (!options->isDisabled(OMR::samplingJProfiling) &&
                            ...
                            // TODO: should we disable sampling to prevent upgrades before the method collects enough
                            // profiling info?
                        }
                    }
                }
                ...
                // JProfiling may be enabled if TR_EnableJProfilingInProfilingCompilations is set and its a profiling
                // compilation. See optimizer/JProfilingBlock.cpp
                if (!doJProfile) {
                ...
                if (jitConfig->javaVM->phase != J9VM_PHASE_NOT_STARTUP
                    ...
                    if (options->getOptLevel() < warm) {
                        if (!vm->isAOT_DEPRECATED_DO_NOT_USE()) {
                            ...
                            // situations For AOT we can be more aggressive because the cost is payed only during first
                            // run
                    ...
                    if (!options->getOption(TR_NoOptServer) && !options->getOption(TR_DisableSelectiveNoOptServer)
                        ...
                        if (that->_methodBeingCompiled->_oldStartPC == 0) // first time compilations
                        {
                            ...
                            else if (jitConfig->javaVM->phase != J9VM_PHASE_NOT_STARTUP && details.isOrdinaryMethod()
                        ...
                        } else // recompilation requests
                        {
                            ...
                            } else // recompilations triggered through GCR need to be cheaper
                            {
                                ...
                                if (bodyInfo->getMethodInfo()->getReasonForRecompilation()
                                    ...
                                    if (bodyInfo->getHotness() < options->getOptLevel()) // prevent warm+NoOptServer -->
                                                                                         // warm+NoOptServer transitions
                                    {
                                        options->setOption(TR_NoOptServer);
                                    }
                                }
            ...
            compiler = new (p->trMemory(), heapAlloc) TR::Compilation(that->getCompThreadId(), vmThread, vm, compilee,
                p->_ilGenRequest, *options, p->_dispatchRegion, p->trMemory(), p->_optimizationPlan, reloRuntime);
            ...
            if (compiler) {
                ...
                if (compiler->getOption(TR_EnableSelfTuningScratchMemoryUsageBeforeCompile)) {
                    ...
                    uint64_t physicalLimit = compInfo->computeFreePhysicalLimitAndAbortCompilationIfLow(
                        compiler, incompleteInfo, TR::Options::getScratchSpaceLowerBound());
    ...
    TR_J9SharedCache* sc = (TR_J9SharedCache*)(vm->sharedCache());
    if (metaData && !that->_methodBeingCompiled->isDLTCompile() && !that->_methodBeingCompiled->_aotCodeToBeRelocated
        && sc) {
    ...
    TR::SegmentAllocator const& scratchSegmentProvider)
{
    ...
    try {
                ...
                : _compiler(compiler)
            {
                ...
                if (_compiler.getOutFile() != NULL && _compiler.getOption(TR_TraceAll))
                    traceMsg(&_compiler,
                        ...
                        _compiler.signature(), _compiler.getHotnessName(_compiler.getMethodHotness()),
        ...
        if (TR::Options::isAnyVerboseOptionSet(TR_VerbosePerformance, TR_VerboseCompileStart)) {
            ...
            if (freePhysicalMemorySizeB != OMRPORT_MEMINFO_NOT_AVAILABLE) {
                ...
                TR_VerboseLog::writeLineLocked(TR_Vlog_COMPSTART,
                    "(%s%s) Compiling %s %s %s j9m=%p t=%llu compThread=%d memLimit=%zu KB", compilationTypeString,
        ...
        if (!_methodBeingCompiled->_aotCodeToBeRelocated) {
            if (TR::Options::getVerboseOption(TR_VerboseCompilationDispatch)) {
                TR_VerboseLog::writeLineLocked(
                    TR_Vlog_DISPATCH, "Compiling %s @ %s", compiler->signature(), compiler->getHotnessName());
            }
            ...
            if (TR::Options::getVerboseOption(TR_VerboseCompilationDispatch) && !rtn) {
                ...
                TR_VerboseLog::writeLine(TR_Vlog_DISPATCH,
                    "Successfully created compiled body [" POINTER_PRINTF_FORMAT "-", compiler->cg()->getCodeStart());

                TR_VerboseLog::write(POINTER_PRINTF_FORMAT "] for %s @ %s", compiler->cg()->getCodeEnd(),
                    compiler->signature(), compiler->getHotnessName());
            ...
            metaData = installAotCachedMethod(vmThread, _methodBeingCompiled->_aotCodeToBeRelocated, method, loadvm,
                compiler->getOptions(), compilee, _methodBeingCompiled, compiler);
            ...
            if (!metaData) // Failed to relocate!
            {
                if (TR::Options::getVerboseOption(TR_VerboseCompilationDispatch)) {
                    TR_VerboseLog::writeLineLocked(TR_Vlog_DISPATCH,
                        "Failed to load previously AOT compiled body for %s @ %s", compiler->signature(),
...

        {
            ...
            if (chTable && chTable->commit(compiler) == false) {
                ...
                if (TR::Options::isAnyVerboseOptionSet(
                        TR_VerboseCompileEnd, TR_VerbosePerformance, TR_VerboseCompFailure)) {
                    TR_VerboseLog::writeLineLocked(
                        TR_Vlog_FAILURE, "Failure while committing chtable for %s", compiler->signature());
                }
        ...
        logCompilationSuccess(
            vmThread, vm, method, scratchSegmentProvider, compilee, compiler, metaData, optimizationPlan);
    ...
    TR_ASSERT_FATAL(
        !TR::MonitorTable::get()->getClassTableMutex()->owned_by_self(), "Should not still own classTableMutex");
...
static J9UTF8* getSourceFileName(J9JavaVM*& vm, J9Method*& _method)
{
...
static inline U_8* writeU64(U_8* bufferCursor, U_64 data)
{
    *((U_64*)bufferCursor) = data;
    return bufferCursor + sizeof(U_64);
}

static inline U_8* writeU32(U_8* bufferCursor, U_32 data)
{
    *((U_32*)bufferCursor) = data;
    return bufferCursor + sizeof(U_32);
}

static inline U_8* writeI32(U_8* bufferCursor, I_32 data)
{
    *((I_32*)bufferCursor) = data;
    return bufferCursor + sizeof(I_32);
}

static inline U_8* writeU16(U_8* bufferCursor, U_16 data)
{
    *((U_16*)bufferCursor) = data;
    return bufferCursor + sizeof(U_16);
}

static inline U_8* writeI16(U_8* bufferCursor, I_16 data)
{
    *((I_16*)bufferCursor) = data;
    return bufferCursor + sizeof(I_16);
}

static inline U_8* writeU8(U_8* bufferCursor, U_8 data)
{
    *bufferCursor = data;
    return bufferCursor + sizeof(U_8);
}

void TR::CompilationInfo::emitJvmpiCallSites(TR::Compilation*& compiler, J9VMThread* vmThread, J9Method*& _method)
{
    PORT_ACCESS_FROM_JAVAVM(_jitConfig->javaVM);
    ...
    ALWAYS_TRIGGER_J9HOOK_JIT_COMPILED_METHOD_LOAD(
        _jitConfig->hookInterface, vmThread, _method, buffer, bufferCursor - buffer, NULL);
    j9mem_free_memory(buffer);
}

// Compilation descriptor:
// <u32 eye-catcher> <u32 type> <u32 attributes> <u32 descriptor> <u64 warm-cc start address> <u64 cold-cc start
// address> descriptor:
...
int TR::CompilationInfo::bufferSizeCompilationAttributes() { return (8 * sizeof(U_32)); }

U_8* TR::CompilationInfo::bufferPopulateCompilationAttributes(
    U_8* buffer, TR::Compilation*& compiler, TR_MethodMetaData* metaData)
{
    U_8* bufferCursor = buffer;
...
int TR::CompilationInfo::bufferSizeInlinedCallSites(TR::Compilation*& compiler, TR_MethodMetaData* metaData)
{
    int bufferSize = 0;
    ...
    for (int16_t i = 0; i < (int16_t)compiler->getNumInlinedCallSites(); i++) {
        ...
        const char* calleeName
            = compiler->fej9()->sampleSignature(method); // FIXME we only need the length, not the signature
...
uint8_t* TR::CompilationInfo::bufferPopulateInlinedCallSites(
    uint8_t* buffer, TR::Compilation*& compiler, TR_MethodMetaData* metaData)
{
    U_8* bufferCursor = buffer;
...
int TR::CompilationInfo::bufferSizeLineNumberTable(
    TR::Compilation*& compiler, TR_MethodMetaData* metaData, J9Method*& _method)
{
    int bufferSize = 0;
...
uint8_t* TR::CompilationInfo::bufferPopulateLineNumberTable(
    uint8_t* buffer, TR::Compilation*& compiler, TR_MethodMetaData* metaData, J9Method*& _method)
{
    U_8* bufferCursor = buffer;
    TR::Instruction* instruction;
    ...
    *bufferCursor = '\0';
    ...
    for (instruction = compiler->cg()->getFirstInstruction(); instruction; instruction = instruction->getNext()) {
        ...
        needNewRecord = (isFirstLoop || // this means we don't need to init lastLineNumber
            numOffsets == UCHAR_MAX || lineNumber != lastLineNumber || lastIcsIndex != icsIndex);
        ...
        if (isFirstLoop || offset != lastOffset) {
            switch (sizeOfOffset) {
            case sizeof(U_8):
                bufferCursor = writeU8(bufferCursor, (U_8)offset);
                break;
            case sizeof(U_16):
                bufferCursor = writeU16(bufferCursor, (U_16)offset);
                break;
            case sizeof(U_32):
                bufferCursor = writeU32(bufferCursor, (U_32)offset);
                break;
            case sizeof(U_64):
                bufferCursor = writeU64(bufferCursor, (U_64)offset);
                break;
            default:
...
void TR::CompilationInfo::emitJvmpiExtendedDataBuffer(
    TR::Compilation*& compiler, J9VMThread* vmThread, J9Method*& _method, TR_MethodMetaData* metaData)
{
    PORT_ACCESS_FROM_JAVAVM(_jitConfig->javaVM);
    ...
    // Call event hook
    if (J9_EVENT_IS_HOOKED(_jitConfig->javaVM->hookInterface, J9HOOK_VM_DYNAMIC_CODE_LOAD))
        ALWAYS_TRIGGER_J9HOOK_VM_DYNAMIC_CODE_LOAD(_jitConfig->javaVM->hookInterface, vmThread, _method, buffer,
            bufferCursor - buffer, "JIT inlined body", NULL);
    j9mem_free_memory(buffer);
}

#if defined(JIT_METHODHANDLE_TRANSLATED)
// TODO: Remove this once the VM supplies this declaration
extern J9_CFUNC void jitMethodHandleTranslated(
    J9VMThread* currentThread, j9object_t methodHandle, j9object_t arg, void* jitEntryPoint, void* intrpEntryPoint);
#endif

// static method
void* TR::CompilationInfo::compilationEnd(J9VMThread* vmThread, TR::IlGeneratorMethodDetails& details,
    J9JITConfig* jitConfig, void* startPC, void* oldStartPC, TR_FrontEnd* fe, TR_MethodToBeCompiled* entry,
    TR::Compilation* comp)
{
    ...
    if (details.isNewInstanceThunk()) {
        ...
        if ((jitConfig->runtimeFlags & J9JIT_TOSS_CODE) && comp
    ...
    && (defined(TR_HOST_X86) || defined(TR_HOST_POWER) || defined(TR_HOST_S390) || defined(TR_HOST_ARM))
                    if (TR::Options::sharedClassCache()) {
                        ...
                        if (static_cast<TR_JitPrivateConfig*>(jitConfig->privateConfig)->aotValidHeader == TR_yes) {
                            safeToStore = true;
                        } else if (static_cast<TR_JitPrivateConfig*>(jitConfig->privateConfig)->aotValidHeader
                            == TR_maybe) {
                            ...
                            TR_J9SharedCacheVM* fe
                                = (TR_J9SharedCacheVM*)TR_J9VMBase::get(jitConfig, vmThread, TR_J9VMBase::AOT_VM);
                            safeToStore
                                = entry->_compInfoPT->reloRuntime()->storeAOTHeader(jitConfig->javaVM, fe, vmThread);
                        } else {
                            safeToStore = false;
                        }
                        ...
                        aotMethodHeaderEntry->compileFirstClassLocation
                            = (UDATA)jitConfig->javaVM->sharedClassConfig->cacheDescriptorList->romclassStartAddress;

                        if (safeToStore) {
                            switch (
                                reinterpret_cast<uintptr_t>(jitConfig->javaVM->sharedClassConfig->storeCompiledMethod(
                                    vmThread, J9_ROM_METHOD_FROM_RAM_METHOD(method), dataStart, dataSize, codeStart,
                                    codeSize, 0))) {
                            case J9SHR_RESOURCE_STORE_FULL: {
                                if (jitConfig->javaVM->sharedClassConfig->verboseFlags
                                    & J9SHR_VERBOSEFLAG_ENABLE_VERBOSE)
                            ...
                            case J9SHR_RESOURCE_STORE_ERROR: {
                                ...
                                TR_J9SharedCache::setSharedCacheDisabledReason(
                                    TR_J9SharedCache::SHARED_CACHE_STORE_ERROR);
                        ...
                        if (debug) {
                            ...
                            if (methodSignature) {
                                sprintf(methodSignature, "%.*s.%.*s%.*s", J9UTF8_LENGTH(className), utf8Data(className),
                            ...
                            if (relocatedMetaData) {
                                ...
                                if (J9_EVENT_IS_HOOKED(jitConfig->javaVM->hookInterface, J9HOOK_VM_DYNAMIC_CODE_LOAD)) {
                                    ...
                                    ALWAYS_TRIGGER_J9HOOK_VM_DYNAMIC_CODE_LOAD(jitConfig->javaVM->hookInterface,
                                        vmThread, method, reinterpret_cast<void*>(relocatedMetaData->startPC),
                                        relocatedMetaData->endWarmPC - relocatedMetaData->startPC, "JIT warm body",
                                        relocatedMetaData);
                                    if (relocatedMetaData->startColdPC) {
                                        // Register the cold code section too
                                        //
                                        ALWAYS_TRIGGER_J9HOOK_VM_DYNAMIC_CODE_LOAD(jitConfig->javaVM->hookInterface,
                                            vmThread, method, reinterpret_cast<void*>(relocatedMetaData->startColdPC),
                                            relocatedMetaData->endPC - relocatedMetaData->startColdPC, "JIT cold body",
                                            relocatedMetaData);
                                    }
                                    ccMethodHeader = getCodeCacheMethodHeader(
                                        reinterpret_cast<char*>(relocatedMetaData->startPC), 32, relocatedMetaData);
                                    if (ccMethodHeader && (relocatedMetaData->bodyInfo != NULL)) {
                                        TR_LinkageInfo* linkageInfo
                                            = TR_LinkageInfo::get(reinterpret_cast<void*>(relocatedMetaData->startPC));
                                        if (linkageInfo->isRecompMethodBody()) {
                                            ALWAYS_TRIGGER_J9HOOK_VM_DYNAMIC_CODE_LOAD(jitConfig->javaVM->hookInterface,
                                                vmThread, method, ccMethodHeader->_eyeCatcher + 4,
                                                relocatedMetaData->startPC
                                                    - reinterpret_cast<uintptr_t>(ccMethodHeader->_eyeCatcher + 4),
                        ...
                        if (!canRelocateMethod || !relocatedMetaData) {
                            ...
                            //
                            J9JITDataCacheHeader* aotMethodHeader
                            ...
                            TR::CodeCacheManager::instance()->addFreeBlock(
                                static_cast<void*>(metaData), reinterpret_cast<uint8_t*>(metaData->startPC));
       ...
       // defined(TR_HOST_POWER) || defined(TR_HOST_S390) || defined(TR_HOST_ARM))
                    {
...
#endif
                        TR_ASSERT(
                            0, "shared classes flag not enabled yet we got an AOT compilation"); // This is a branch
                                                                                                 // that is never
                                                                                                 // expected to enter
                        entry->_doNotUseAotCodeFromSharedCache = true;
                        entry->_useAotCompilation = false;
                        ...
                        if (TR::Options::isAnyVerboseOptionSet(
                                TR_VerboseCompileEnd, TR_VerbosePerformance, TR_VerboseCompFailure)) {
                            TR_VerboseLog::writeLineLocked(
                                TR_Vlog_INFO, "Shared Class not available for AOT compile for %s", comp->signature());
                        }
                    }
...
void TR::CompilationInfo::setProcessorByDebugOption()
{
    if (debug("rios1"))
...
//
bool TR::CompilationInfoPerThreadBase::methodCanBeCompiled(
    TR_Memory* trMemory, TR_FrontEnd* vm, TR_ResolvedMethod* method, TR_FilterBST*& filter)
{
    filter = NULL;
    ...
    TR_MethodMetaData* metaData, TR_OptimizationPlan* optimizationPlan)
{
    if (!_methodBeingCompiled->_aotCodeToBeRelocated) {
        ...
        //
        if (compiler->getOutFile() != NULL
            && (compiler->getOption(TR_TraceAll)
                   || compiler->getOptions()->getTraceCGOption(TR_TraceCGMixedModeDisassembly)))
        ...
        if (_jitConfig->runtimeFlags & J9JIT_TESTMODE) {
            ...
            if (TR::Options::isAnyVerboseOptionSet(
                    TR_VerboseCompileEnd, TR_VerboseGc, TR_VerboseRecompile, TR_VerbosePerformance, TR_VerboseOptimizer)
                ...
                TR_VerboseLog::writeLine(TR_Vlog_COMP, "(%s%s) %s @ " POINTER_PRINTF_FORMAT "-" POINTER_PRINTF_FORMAT,
                    compilationTypeString, hotnessString, compiler->signature(), metaData->startPC,
                    metaData->startColdPC ? metaData->endWarmPC : metaData->endPC);
                if (metaData->startColdPC) {
                    TR_VerboseLog::write(
                        "/" POINTER_PRINTF_FORMAT "-" POINTER_PRINTF_FORMAT, metaData->startColdPC, metaData->endPC);
                }
                ...
                if (TR::Options::getVerboseOption(TR_VerboseOptimizer)) {
                    TR_VerboseLog::write(
                        " opts=%d.%d", compiler->getLastPerformedOptIndex(), compiler->getLastPerformedOptSubIndex());
                }
            ...
            snprintf(compilationAttributes, sizeof(compilationAttributes), "%s %s %s %s %s %s %s", prexString,
                (_compInfo.useSeparateCompilationThread() && !_methodBeingCompiled->_async) ? "sync" : "",
                TR::CompilationInfo::isJNINative(method) ? "JNI" : "", compiler->getOption(TR_EnableOSR) ? "OSR" : "",
                (compiler->getRecompilationInfo()
                    && compiler->getRecompilationInfo()->getJittedBodyInfo()->getUsesGCR())
                    ? "GCR"
                    : "",
            ...
            *(compilationAttributes + sizeof(compilationAttributes) - 1) = '\0';
            ...
            Trc_JIT_compileEnd15(vmThread, compilationTypeString, hotnessString, compiler->signature(),
                metaData->startPC, metaData->endWarmPC, metaData->startColdPC, metaData->endPC, translationTime, method,
                metaData, recompReason, _compInfo.getMethodQueueSize(),
...
void TR::CompilationInfoPerThreadBase::processException(J9VMThread* vmThread,
    ...
    volatile bool& haveLockedClassUnloadMonitor, const char* exceptionName) throw()
{
    if (TR::Options::getVerboseOption(TR_VerboseCompilationDispatch)) {
    ...
    /* Excessive Complexity Exceptions End */
        ...
        if (TR::Options::isAnyVerboseOptionSet(TR_VerbosePerformance, TR_VerboseCompileEnd, TR_VerboseCompFailure)) {
            ...
            TR_VerboseLog::writeLineLocked(TR_Vlog_COMPFAIL, "%s time=%dus %s", compiler->signature(), translationTime,
        ...
        if (TR::Options::isAnyVerboseOptionSet(TR_VerbosePerformance, TR_VerboseCompileEnd, TR_VerboseCompFailure)) {
            ...
            TR_VerboseLog::writeLineLocked(
                TR_Vlog_COMPFAIL, "Assertion Failure: %s time=%dus", compiler->signature(), translationTime);
        }
        Trc_JIT_compilationFailed(vmThread, compiler->signature(), -1);
    } catch (...) {
    ...
    TR::SegmentAllocator const& scratchSegmentProvider, TR::Compilation* compiler, const char* exceptionName)
{
    ...
    if (TR::Options::isAnyVerboseOptionSet(TR_VerbosePerformance, TR_VerboseCompileEnd, TR_VerboseCompFailure)) {
        ...
        if (_methodBeingCompiled->_compErrCode != compilationFailure) {
            if ((_jitConfig->runtimeFlags & J9JIT_TESTMODE)
                && _methodBeingCompiled->_compErrCode == compilationInterrupted)
                TR_VerboseLog::writeLine(TR_Vlog_FAILURE, "<JIT: translating %s -- Interrupted because of %s",
                    compiler->signature(), exceptionName);
            else {
                bool incomplete;
                uint64_t freePhysicalMemorySizeB = _compInfo.computeAndCacheFreePhysicalMemory(incomplete);
                if (freePhysicalMemorySizeB != OMRPORT_MEMINFO_NOT_AVAILABLE) {
                    TR_VerboseLog::writeLine(TR_Vlog_COMPFAIL,
                        "%s time=%dus %s memLimit=%zu KB freePhysicalMemory=%llu MB", compiler->signature(),
                        translationTime, compilationErrorNames[_methodBeingCompiled->_compErrCode],
                        scratchSegmentProvider.allocationLimit() >> 10, freePhysicalMemorySizeB >> 20);
                } else {
                    TR_VerboseLog::writeLine(TR_Vlog_COMPFAIL, "%s time=%dus %s memLimit=%zu KB", compiler->signature(),
                        translationTime, compilationErrorNames[_methodBeingCompiled->_compErrCode],
                        scratchSegmentProvider.allocationLimit() >> 10);
                }
            }
            ...
            TR_VerboseLog::writeLine(TR_Vlog_COMPFAIL, "%s time=%dus %s <TRANSLATION FAILURE: out of scratch memory>",
                compiler->signature(), translationTime, exceptionName);
        }
...
void TR::CompilationInfo::triggerOrderedCompiles(TR_FrontEnd* f, intptr_t tickCount)
{
    ...
    for (prev = &TR::Options::getDebug()->getCompilationFilters()->samplingPoints, filter = *prev; filter;
         filter = *prev) {
        if (filter->getTickCount() > tickCount)
            break;

        J9Method* ramMethod
            = (J9Method*)fe->getMethodFromName(filter->getClass(), filter->getName(), filter->getSignature());
        if (!ramMethod) {
        ...
        if (filter->getFilterType() == TR_FILTER_SAMPLE_INTERPRETED) {
            if (!isCompiled(ramMethod)) {
                ...
                if (count > filter->getSampleCount()) {
                    ...
                    if (logSampling)
                        TR_VerboseLog::writeLineLocked(TR_Vlog_CR, "(%d) Interpreted %s.%s%s (SIMULATED) --> %d",
                            tickCount, filter->getClass(), filter->getName(), filter->getSignature(),
        ...
        if (isCompiled(ramMethod)) {
            ...
            if (linkageInfo->isRecompMethodBody()) {
                ...
                TR_PersistentJittedBodyInfo* bodyInfo = TR::Recompilation::getJittedBodyInfoFromPC(startPC);
                TR_PersistentMethodInfo* methodInfo = bodyInfo->getMethodInfo();
                if (bodyInfo->getHotness() != unknownHotness && bodyInfo->getHotness() >= filter->getSampleLevel()) {
                    *prev = filter->getNext();
                    continue;
                }
                ...
                //
                if (linkageInfo->isSamplingMethodBody()
                    && !TR::Recompilation::isAlreadyBeingCompiled(
                           (TR_OpaqueMethodBlock*)methodInfo->getMethodInfo(), startPC, fe)) {
                    methodInfo->setNextCompileLevel((TR_Hotness)filter->getSampleLevel(), filter->getSampleProfiled());
                    ...
                    if (logSampling) {
                        TR_VerboseLog::vlogAcquire();
                        TR_VerboseLog::writeLine(TR_Vlog_SAMPLING,
                            "(%d) Compiled %s.%s%s (SIMULATED) recompile at level --> %d", tickCount,
...
#if DEBUG
void TR::CompilationInfo::debugPrint(char* debugString)
{
    if (!_traceCompiling)
        return;
...
void TR::CompilationInfo::debugPrint(J9VMThread* vmThread, char* debugString)
{
    if (!_traceCompiling)
        return;
...
void TR::CompilationInfo::debugPrint(char* debugString, intptr_t val)
{
    if (!_traceCompiling)
        return;
...
void TR::CompilationInfo::debugPrint(J9VMThread* vmThread, char* debugString, IDATA val)
{
    if (!_traceCompiling)
        return;
    fprintf(stderr, "%8x:", vmThread);
...
void TR::CompilationInfo::debugPrint(char* debugString, J9Method* method)
{
    if (!_traceCompiling)
        return;
...
void TR::CompilationInfo::debugPrint(char* debugString, TR::IlGeneratorMethodDetails& details, J9VMThread* vmThread)
{
    if (!_traceCompiling)
        return;
    fprintf(stderr, "%8x:", vmThread);
...
void TR::CompilationInfo::debugPrint(J9VMThread* vmThread, char* msg, TR_MethodToBeCompiled* entry)
{
    if (!_traceCompiling)
        return;
...
TR_LMGuardedStorage* TR::CompilationInfo::getLMGuardedStorage() const
{
...
uint64_t TR::CompilationInfo::computeFreePhysicalLimitAndAbortCompilationIfLow(
    TR::Compilation* comp, bool& incompleteInfo, size_t sizeToAllocate)
{
    uint64_t freePhysicalMemorySizeB = computeAndCacheFreePhysicalMemory(incompleteInfo);
    if (OMRPORT_MEMINFO_NOT_AVAILABLE != freePhysicalMemorySizeB) {
        ...
        if (fail) {
            if (TR::Options::isAnyVerboseOptionSet(
                    TR_VerbosePerformance, TR_VerboseCompileEnd, TR_VerboseCompFailure)) {
                TR_VerboseLog::writeLineLocked(
                    TR_Vlog_FAILURE, "Aborting Compilation: Low On Physical Memory %lld B", freePhysicalMemorySizeB);
            }
        ...
        if (++entry->_count > _threshold) // hot entry; possibly queue compilation
        {
            if (!entry->_queuedForCompilation) {
                ...
                if (TR::Options::sharedClassCache() && // must have AOT enabled
                    ...
                    && vmThread->javaVM->sharedClassConfig->existsCachedCodeForROMMethod(
                           vmThread, J9_ROM_METHOD_FROM_RAM_METHOD(j9method))) {
                                ...
                                if (numCompThreadsSuspended > 1) {
                                    ...
                                    if (activate == TR_yes
                                        || (activate == TR_maybe
                                               ...
                                               _compInfo->getNumCompThreadsActive() + 2
                                                   < _compInfo->getNumTargetCPUs())) {
                                        TR::CompilationInfoPerThread* compInfoPT
...
TR_MethodToBeCompiled* TR_LowPriorityCompQueue::findAndDequeueFromLPQ(
    TR::IlGeneratorMethodDetails& details, uint8_t reason, TR_J9VMBase* fe, bool& dequeued)
{
    dequeued = false;
...
TR_CompilationErrorCode TR::CompilationInfo::scheduleLPQAndBumpCount(
    TR::IlGeneratorMethodDetails& details, TR_J9VMBase* fe)
{
    J9Method* method = details.getMethod();
    // Search the LPQ and if not found, add method to LPQ and bump invocation count
    ...
    if (count == 0) {
        bool dequeued = false;
        TR_MethodToBeCompiled* entry = getLowPriorityCompQueue().findAndDequeueFromLPQ(
            details, TR_MethodToBeCompiled::REASON_LOW_COUNT_EXPIRED, fe, dequeued);
        if (!dequeued) // Early compilation request
        {
            ...
            {
                queued = getLowPriorityCompQueue().addFirstTimeCompReqToLPQ(
                    method, TR_MethodToBeCompiled::REASON_LOW_COUNT_EXPIRED);
            }
            if (queued) {
                ...
                if (TR::CompilationInfo::setInvocationCount(method, count, newCount)) {
                    ...
                    bool removed;
                    TR_MethodToBeCompiled* req = getLowPriorityCompQueue().findAndDequeueFromLPQ(
                        details, TR_MethodToBeCompiled::REASON_LOW_COUNT_EXPIRED, fe, removed);
                    if (req && removed) {
...
void J9Method_HT::onClassUnloading(J9ClassLoader* j9classLoader)
{
    ...
    for (size_t bucketID = 0; bucketID < HT_SIZE; bucketID++) {
        ...
        for (; entry; prev = entry, entry = entry->_next) {
            if (NULL == entry->_j9method || J9_CLASS_FROM_METHOD(entry->_j9method)->classLoader == j9classLoader) {
                if (prev)
