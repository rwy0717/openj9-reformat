...
public:
    ...
    void preCompilationTasks(J9VMThread* vmThread, J9JavaVM* javaVM, TR_MethodToBeCompiled* entry, J9Method* method,
     ...
     * (with C++ linkage). However a brief search reveals that such an approach approach is fraught with its own set of
    ...
    public:
        ...
        ~InterruptibleOperation() { _compThread.exitInterruptibleOperation(); }
...
private:
    ...
    void processException(J9VMThread* vmThread, const TR::SegmentAllocator& scratchSegmentProvider,
        TR::Compilation* compiler, volatile bool& haveLockedClassUnloadMonitor, const char* exceptionName) throw();

    void processExceptionCommonTasks(J9VMThread* vmThread, TR::SegmentAllocator const& scratchSegmentProvider,
        TR::Compilation* compiler, const char* exceptionName);
