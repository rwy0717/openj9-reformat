...
TR::OptionSet* findOptionSet(J9Method* method, bool isAOT)
{
    ...
    if (methodSignature) {
        sprintf(methodSignature, "%.*s.%.*s%.*s", J9UTF8_LENGTH(className), utf8Data(className), J9UTF8_LENGTH(name),
        ...
        optionSet = TR::Options::findOptionSet(
            index, lineNum, methodSignature, TR::Options::getInitialHotnessLevel(hasBackwardBranches), isAOT);
        if (len >= 1024)
...
extern void getOutOfIdleStates(
    TR::CompilationInfo::TR_SamplerStates expectedState, TR::CompilationInfo* compInfo, const char* reason);
extern void getOutOfIdleStatesUnlocked(
    TR::CompilationInfo::TR_SamplerStates expectedState, TR::CompilationInfo* compInfo, const char* reason);
...
static void jitHookInitializeSendTarget(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    && (defined(TR_HOST_X86) || defined(TR_HOST_POWER) || defined(TR_HOST_S390) || defined(TR_HOST_ARM))
            if (compInfo->reloRuntime()->isRomClassForMethodInSharedCache(method, jitConfig->javaVM)) {
                ...
                if (jitConfig->javaVM->sharedClassConfig->existsCachedCodeForROMMethod(vmThread, romMethod)) {
                    ...
                    if ((TR_J9SharedCache*)(((TR_J9VMBase*)fe)->sharedCache())
                            ->isHint(method, TR_HintFailedValidation, &newScount))
#else
                    if (((TR_J9VMBase*)fe)->isSharedCacheHint(method, TR_HintFailedValidation, &newScount))
#endif
                    {
                        if ((scount == TR_QUICKSTART_INITIAL_SCOUNT)
                            || (scount == TR_INITIAL_SCOUNT)) { // If scount is not user specified (coarse way due to
                                                                // info being lost from options parsing)
                            ...
                            if (optionsAOT->getVerboseOption(TR_VerboseSCHints)
                                ...
                                TR_VerboseLog::writeLineLocked(TR_Vlog_SCHINTS,
                                    "Found hint in sc, increase scount to: %d, wanted scount: %d", scount, newScount);
                        }
                    }
                    count = scount;
...
#if defined(HINTS_IN_SHAREDCACHE_OBJECT)
                        bool wasCompiledDuringStartup = (TR_J9SharedCache*)(fe->sharedCache())
                                                            ->isHint(method, TR_HintMethodCompiledDuringStartup);
#else
                        bool wasCompiledDuringStartup
                            = fe->isSharedCacheHint(method, TR_HintMethodCompiledDuringStartup);
#endif
    ...
    if (TR::Options::getJITCmdLineOptions()->getOption(TR_DumpInitialMethodNamesAndCounts)
        ...
        int32_t sigLen = sprintf(buf, "%.*s.%.*s%.*s", className->length, utf8Data(className), name->length,
            utf8Data(name), signature->length, utf8Data(signature));
        printf("Initial: Signature %s Count %d isLoopy %d isAOT %d is in SCC %d SCCContainsProfilingInfo %d \n", buf,
            TR::CompilationInfo::getInvocationCount(method), J9ROMMETHOD_HAS_BACKWARDS_BRANCHES(romMethod),
            TR::Options::sharedClassCache()
                ? jitConfig->javaVM->sharedClassConfig->existsCachedCodeForROMMethod(vmThread, romMethod)
                : 0,
            TR::Options::sharedClassCache() ? compInfo->isRomClassForMethodInSharedCache(method, jitConfig->javaVM) : 0,
            containsInfo);
        fflush(stdout);
    }
}
...
static void turnOnInterpreterProfiling(J9JavaVM* javaVM, TR::CompilationInfo* compInfo)
{
    if (interpreterProfilingState == IPROFILING_STATE_OFF) {
        ...
        if ((*hook)->J9HookRegisterWithCallSite(
                hook, J9HOOK_VM_PROFILING_BYTECODE_BUFFER_FULL, jitHookBytecodeProfiling, OMR_GET_CALLSITE(), NULL)) {
...
static TR_YesNoMaybe shouldInitiateDLT(J9DLTInformationBlock* dltInfo, int32_t idx, bool* bcRepeats, int32_t* hitCnt)
{
    ...
    *hitCnt = hitCount;
    ...
    if (!TR::Options::getCmdLineOptions()->getOption(TR_DisableFastDLTOnLongRunningInterpreter)
        && TR::CompilationInfo::isCompiled(currentMethod)) {
...
static void emptyJitGCMapCheck(
    J9VMThread* currentThread, J9StackWalkState* walkState, j9object_t* objectSlot, const void* stackLocation)
{
    return;
}
    ...
    if (doPerformDLT) {
        ...
        if (enableDebugDLT != NULL) {
            ...
            if (enableDLTidx != -1) {
                ...
                if (enableDLTidx
                    >= (J9_BYTECODE_END_FROM_ROM_METHOD(romMethod)) - (J9_BYTECODE_START_FROM_ROM_METHOD(romMethod)))
                    return;
            ...
            dltOptLevel = options ? options->getDLTOptLevel() : TR::Options::getCmdLineOptions()->getDLTOptLevel();
        }
        ...
        if (vm->isLogSamplingSet()) {
            ...
            TR_VerboseLog::writeLineLocked(
                TR_Vlog_DLT, "Will try to queue DLT compilation for %s bcIndex=%d", sig, bcIndex);
        }
                    ...
                    if (TR::CompilationInfo::setInvocationCount(j9method, count, 0)) {
                        ...
                        if (vm->isAsyncCompilation()) {
                            ...
                            TR_OptimizationPlan* jitplan
                                = TR::CompilationController::getCompilationStrategy()->processEvent(
...
static void jitMethodSampleInterrupt(J9VMThread* vmThread, IDATA handlerKey, void* userData)
{
    ...
    } else if ((jitConfig->runtimeFlags & J9JIT_DEFER_JIT) == 0) // Reject any samples if we decided to postpone jitting
    {
        ...
        if (!TR::Options::getCmdLineOptions()->getOption(TR_MimicInterpreterFrameShape)
             ...
             */
            if (!skipSampleMethod)
                TR::Recompilation::sampleMethod(
                    vmThread, vm, startPC, codeSize, walkState.pc, walkState.method, jitConfig->samplingTickCount);
        }
...
typedef struct DumpCurrentILParamenters {
    DumpCurrentILParamenters(TR::Compilation* comp, J9VMThread* vmThread, J9JITConfig* jitConfig, TR::FILE* logFile)
        : _comp(comp)
        , _vmThread(vmThread)
...
static UDATA blankDumpCurrentILSignalHandler(struct J9PortLibrary* portLibrary, U_32 gpType, void* gpInfo, void* arg)
{
...
static void dumpCurrentIL(TR::Compilation* comp, J9VMThread* vmThread, J9JITConfig* jitConfig, TR::FILE* logFile)
{
    ...
    DumpCurrentILParamenters p(comp, vmThread, jitConfig, logFile);
    ...
    if (!noSignalWrapper && j9sig_can_protect(flags)) {
        ...
        protectedResult = j9sig_protect((j9sig_protected_fn)dumpCurrentILProtected, static_cast<void*>(&p),
            (j9sig_handler_fn)blankDumpCurrentILSignalHandler, vmThread, flags, &result);
    } else
#endif
...
static UDATA logStackIterator(J9VMThread* currentThread, J9StackWalkState* walkState)
{
    ...
    if (walkState->jitInfo) {
        ...
        TR_PersistentJittedBodyInfo* bodyInfo
            = TR::Recompilation::getJittedBodyInfoFromPC((void*)walkState->jitInfo->startPC);
...
static TR_CompilationErrorCode recompileMethodForLog(J9VMThread* vmThread, J9Method* ramMethod,
    TR::CompilationInfo* compInfo, TR_J9VMBase* frontendOfThread, TR_Hotness optimizationLevel, bool profilingCompile,
    void* oldStartPC, TR::FILE* logFile)
{
    if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseDump))
    ...
    if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseDump))
        TR_VerboseLog::writeLineLocked(TR_Vlog_JITDUMP,
            "dumpJitInfo: crashing thread returned from compileMethod() with errorCode=%d", compErrCode);
...
IDATA dumpJitInfo(J9VMThread* crashedThread, char* logFileLabel, J9RASdumpContext* context)
{
    ...
    if (recompilationThread) {
        if (options->getVerboseOption(TR_VerboseDump))
            TR_VerboseLog::writeLineLocked(
                TR_Vlog_JITDUMP, "dumpJitInfo: diagnostic compilation thread available. Will purge compilation queue");
    ...
    if (threadCompInfo == 0) {
        ...
        if (recompilationThread) {
            ...
            walkState.flags = (
                ...
                J9_STACKWALK_VISIBLE_ONLY | J9_STACKWALK_SKIP_INLINES | J9_STACKWALK_COUNT_SPECIFIED
            ...
            if (options->getVerboseOption(TR_VerboseDump))
                TR_VerboseLog::writeLineLocked(
                    TR_Vlog_JITDUMP, "found %d JITed methods on Java stack", currentMethodIndex);
            ...
            for (int i = 0; i < currentMethodIndex; i++) {
                ...
                TR_CompilationErrorCode compErrCode;
                compErrCode
                    = recompileMethodForLog(crashedThread, jittedMethodsOnStack[i]._method, compInfo, frontendOfThread,
                        jittedMethodsOnStack[i]._optLevel, false, jittedMethodsOnStack[i]._oldStartPC, logFile);
            } // for
    ...
    // if our compinfo is not null, we are a compilation thread
        ...
        if (comp) {
            ...
            if (recompilationThread) {
                ...
                if (currentMethodBeingCompiled && currentMethodBeingCompiled->getMethodDetails().isOrdinaryMethod()) {
                    // resume the healthy compilation thread
                    recompilationThreadInfo->resumeCompilationThread(); // TODO: Postpone this so that the thread does
                    ...
                    TR_CompilationErrorCode compErrCode;
                    compErrCode = recompileMethodForLog(crashedThread,
                        (J9Method*)(comp->getCurrentMethod()->getPersistentIdentifier()), compInfo, frontendOfThread,
                        (TR_Hotness)comp->getOptLevel(), comp->isProfilingCompilation(), oldStartPC, logFile);
...
static void jitHookAnonClassesUnload(J9HookInterface** hookInterface, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (fullChainOfMetaData) {
        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseClassUnloading))
            TR_VerboseLog::writeLineLocked(
                TR_Vlog_GC, "jitHookAnonClassesUnload: will remove %u metadata entities\n", numMetaData);
...
static void jitHookClassUnload(J9HookInterface** hookInterface, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (!disableUnloadedClassRanges)
        compInfo->getPersistentInfo()->addUnloadedClass(
            clazz, methodsStartAddr, (uint32_t)(methodsEndAddr - methodsStartAddr));
...
void jitClassesRedefined(J9VMThread* currentThread, UDATA classCount, J9JITRedefinedClass* classList)
{
        ...
        for (i = 0; i < classCount; i++) {
            ...
            for (j = 0; j < methodCount; j++) {
                ...
                if (staleMethod && freshMethod && compInfo->isCompiled(staleMethod)) {
                    ...
                    else if (_J9ROMMETHOD_J9MODIFIER_IS_SET(
                                 (J9_ROM_METHOD_FROM_RAM_METHOD(staleMethod)), J9AccNative)) {
                        reportHookDetail(currentThread, "jitClassesRedefined",
    ...
    for (i = 0; i < classCount; i++) {
        ...
        if (rat) {
            reportHookDetail(
                currentThread, "jitClassesRedefined", "  Notify RAT on class old=%p fresh=%p", oldClass, freshClass);
...
void jitMethodBreakpointed(J9VMThread* vmThread, J9Method* j9method)
{
    ...
    TR::CompilationInfo* compInfo = TR::CompilationInfo::get(jitConfig);
    TR_RuntimeAssumptionTable* rat = compInfo->getPersistentInfo()->getRuntimeAssumptionTable();
    OMR::RuntimeAssumption** headPtr = rat->getBucketPtr(
        RuntimeAssumptionOnMethodBreakPoint, TR_RuntimeAssumptionTable::hashCode((uintptrj_t)j9method));
...
#if defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING)
static void jitHookInterruptCompilation(
    J9HookInterface** hookInterface, UDATA eventNum, void* eventData, void* userData)
{
...
static void updateOverriddenFlag(J9VMThread* vm, J9Class* cl)
{
    ...
    if (classDepth >= 0) // Do nothing if we don't have a superclass
    {
        ...
        while (methodCount--) {
            ...
            bool methodModifiersAreSafe = (subROM->modifiers & J9AccForwarderMethod)
                && !((subROM->modifiers & J9AccSynchronized) || (subROM->modifiers & J9AccStrict)
                       || (subROM->modifiers & J9AccNative));
            ...
            if (traceIt) {
                ...
                char* superSignature = (char*)J9UTF8_DATA(J9ROMMETHOD_GET_SIGNATURE(
                    J9_CLASS_FROM_METHOD(superMethod)->romClass, J9_ROM_METHOD_FROM_RAM_METHOD(superMethod)));
                int32_t superSigLen = (int32_t)J9UTF8_LENGTH(J9ROMMETHOD_GET_SIGNATURE(
                    J9_CLASS_FROM_METHOD(superMethod)->romClass, J9_ROM_METHOD_FROM_RAM_METHOD(superMethod)));
                char* superName = (char*)J9UTF8_DATA(J9ROMMETHOD_GET_NAME(
                    J9_CLASS_FROM_METHOD(superMethod)->romClass, J9_ROM_METHOD_FROM_RAM_METHOD(superMethod)));
                int32_t superNameLen = (int32_t)J9UTF8_LENGTH(J9ROMMETHOD_GET_NAME(
                    J9_CLASS_FROM_METHOD(superMethod)->romClass, J9_ROM_METHOD_FROM_RAM_METHOD(superMethod)));
                char* subSignature = (char*)J9UTF8_DATA(J9ROMMETHOD_GET_SIGNATURE(
                    J9_CLASS_FROM_METHOD(subMethod)->romClass, J9_ROM_METHOD_FROM_RAM_METHOD(subMethod)));
                int32_t subSigLen = (int32_t)J9UTF8_LENGTH(J9ROMMETHOD_GET_SIGNATURE(
                    J9_CLASS_FROM_METHOD(subMethod)->romClass, J9_ROM_METHOD_FROM_RAM_METHOD(subMethod)));
                char* subName = (char*)J9UTF8_DATA(J9ROMMETHOD_GET_NAME(
                    J9_CLASS_FROM_METHOD(subMethod)->romClass, J9_ROM_METHOD_FROM_RAM_METHOD(subMethod)));
                int32_t subNameLen = (int32_t)J9UTF8_LENGTH(J9ROMMETHOD_GET_NAME(
                    J9_CLASS_FROM_METHOD(subMethod)->romClass, J9_ROM_METHOD_FROM_RAM_METHOD(subMethod)));

                printf("class = %.*s superSignature = %.*s, superName = %.*s, supermodifers = %x , subSignature = "
                       "%.*s, subName = %.*s, submodifiers = %x subMethod = %p methodModifiersAreSafe = %d\n",
                    ...
                    subMethod, methodModifiersAreSafe);
                printf("For subROM %p:\n", subROM);
                ...
                if (subROM->modifiers & J9AccEnum)
                    printf("\tsubMethod is J9AccEnum\n");
                if (subROM->modifiers & J9AccFinal)
                    printf("\tsubMethod is J9AccFinal\n");
                if (subROM->modifiers & J9AccForwarderMethod)
                    printf("\tsubMethod is J9AccForwarderMethod\n");
                ...
                if (superROM->modifiers & J9AccEnum)
                    printf("\tsuperMethod is J9AccEnum\n");
                if (superROM->modifiers & J9AccFinal)
                    printf("\tsuperMethod is J9AccFinal\n");
                if (superROM->modifiers & J9AccForwarderMethod)
                    printf("\tsuperMethod is J9AccForwarderMethod\n");
            ...
            if ((subMethod != superMethod) && methodModifiersAreSafe
                && (J9_BYTECODE_END_FROM_ROM_METHOD(subROM) - J9_BYTECODE_START_FROM_ROM_METHOD(subROM))
                    > 0) // If the j9methods don't match, method has been overridden
            {
                if (traceIt) {
                ...
                int32_t i = 0;
                uint16_t cpIndex = 0;
                for (int32_t nextLocalIndex = 0; !matches && !finishedArgs && nextLocalIndex < 255;
                     nextLocalIndex += curLocalSize) {
                    curLocalSize = 1;
                    ...
                    case J9BCinvokespecialsplit:
                        cpIndex = *(U_16*)(J9ROMCLASS_SPECIALSPLITMETHODREFINDEXES(ROMCl)
                            + BC_OP_U16(&_code[i])); // will need to compare signature with superMethod.
                        matches = true;
                        break;
                    default:
                        finishedArgs = true;
                ...
                if (matches) // so far we are matching our pattern
                {
                    ...
                    J9ROMNameAndSignature* nameAndSignature
                        = J9ROMFIELDREF_NAMEANDSIGNATURE(ref1); // this isn't a string its a struct.
                    callSignature = J9ROMNAMEANDSIGNATURE_SIGNATURE(nameAndSignature);
                    callName = J9ROMNAMEANDSIGNATURE_NAME(nameAndSignature);

                    J9UTF8* superSignature = J9ROMMETHOD_GET_SIGNATURE(
                        J9_CLASS_FROM_METHOD(superMethod)->romClass, J9_ROM_METHOD_FROM_RAM_METHOD(superMethod));
                    J9UTF8* superName = J9ROMMETHOD_GET_NAME(
                        J9_CLASS_FROM_METHOD(superMethod)->romClass, J9_ROM_METHOD_FROM_RAM_METHOD(superMethod));

                    if (J9UTF8_LENGTH(superSignature) != J9UTF8_LENGTH(callSignature)
                        || J9UTF8_LENGTH(superName) != J9UTF8_LENGTH(callName)
                        || !(strncmp((char*)J9UTF8_DATA(superSignature), (char*)J9UTF8_DATA(callSignature),
                                 J9UTF8_LENGTH(superSignature))
                               == 0)
                        || !(strncmp((char*)J9UTF8_DATA(superName), (char*)J9UTF8_DATA(callName),
                                 J9UTF8_LENGTH(superSignature))
                               == 0))

                    {
                        if (traceIt) {
                            printf("For submethod %p,signature compare fails after bc walk superSiglen = %d subsiglen "
                                   "= %d supernamelen = %d subnamelen =%d\n",
                                subMethod, J9UTF8_LENGTH(superSignature), J9UTF8_LENGTH(callSignature),
                                J9UTF8_LENGTH(superName), J9UTF8_LENGTH(callName));
                            fflush(stdout);
                        }
                        matches = false;
                    } else {
                        uint8_t opcode = _code[i + 2]; // an invokespecial takes up 3 bytes.. we point at second byte
                                                       // after for loop, so increment by 2
                        TR_J9ByteCode bc = TR_J9ByteCodeIterator::convertOpCodeToByteCodeEnum(opcode);
                        if (bc != J9BCgenericReturn
                            || &(_code[i + 3])
                                != J9_BYTECODE_END_FROM_ROM_METHOD(subROM)) // second condition ensures that the return
                                                                            // is the last bytecode. See defect 148222.
                        {
                            if (traceIt) {
                                printf("For submethod %p,signature compare fails after bc walk bc !=genericReturn at "
                                       "i(%d)+2\n",
                                    i);
                                fflush(stdout);
                            }
                ...
                if (!matches) // matches is false if method is overridden.
                {
                    ...
                    vm->javaVM->internalVMFunctions->atomicOrIntoConstantPool(
                        vm->javaVM, superMethod, J9_STARTPC_METHOD_IS_OVERRIDDEN);
                    ...
                    for (int32_t k = classDepth - 1; k >= 0; k--) {
                        ...
                        vm->javaVM->internalVMFunctions->atomicOrIntoConstantPool(
                            vm->javaVM, tempsuperMethod, J9_STARTPC_METHOD_IS_OVERRIDDEN);
                    }
                } else {
                    if (traceIt) {
                        printf("For submethod %p, determined it is not overridden by bytecode walk and signature "
                               "compare\n",
                            subMethod);
                        fflush(stdout);
                    }
                }
            } else if (superMethod != subMethod) // we don't have bytecodes, but the j9methods don't match, so set the
                                                 // overridden bit anyways
            {
                jitUpdateMethodOverride(vm, cl, superMethod, subMethod);
                vm->javaVM->internalVMFunctions->atomicOrIntoConstantPool(
                    vm->javaVM, superMethod, J9_STARTPC_METHOD_IS_OVERRIDDEN);
                ...
                if (traceIt) {
                    printf("For submethod %p, j9methods don't match.  Setting overridden bit. endbc - startbc = %d "
                           "methodmodifiersaresafe = %d\n",
                        subMethod,
                        (J9_BYTECODE_END_FROM_ROM_METHOD(subROM) - J9_BYTECODE_START_FROM_ROM_METHOD(subROM)),
                        methodModifiersAreSafe);
                    fflush(stdout);
                }
 ...
 */
int32_t getAvailableVirtualMemoryMB(
    TR::CompilationInfo* compInfo, J9VMThread* vmThread, uint32_t* availableVirtualMemoryMB)
{
    ...
    *availableVirtualMemoryMB = (uint32_t)(memInfo.availVirtual >> 20); // conversion to uint32_t is safe on 32-bit OS
    ...
    if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseJitMemory)) {
        ...
        TR_VerboseLog::writeLineLocked(
            TR_Vlog_MEMORY, "Virtual Memory Used: %d MB out of %d MB", virtualMemoryUsedMB, totalVirtualMemoryMB);
    }
...
void lowerCompilationLimitsOnLowVirtualMemory(TR::CompilationInfo* compInfo, J9VMThread* vmThread)
{
    ...
    if ((availableVirtualMemoryMB < (uint32_t)TR::Options::getLowVirtualMemoryMBThreshold() / 2)
        ...
        if (TR::Options::getScratchSpaceLimit() == (DEFAULT_SCRATCH_SPACE_LIMIT_KB * 1024)) {
            TR_ASSERT(DEFAULT_SCRATCH_SPACE_LIMIT_KB > TR::Options::getScratchSpaceLimitKBWhenLowVirtualMemory(),
                "assertion failure");
        ...
        if (!compInfo->getRampDownMCT()) {
            ...
            if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseCompilationThreads)) {
                TR_VerboseLog::writeLineLocked(
                    TR_Vlog_INFO, "t=%u setRampDownMCT because VMemAv=%u MB", crtTime, availableVirtualMemoryMB);
            }
        }
        ...
        if (doDisableGCR && TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance)) {
            TR_VerboseLog::writeLineLocked(
                TR_Vlog_PERF, "t=%6u WARNING: GCR disabled because VMemAv=%u MB", crtTime, availableVirtualMemoryMB);
        }
    ...
    if (TR::Options::getCmdLineOptions()->allowRecompilation()
        ...
        if (info) {
            ...
            if (vm->isInterfaceClass(clazz)) {
                ...
                if (!compInfo->getPersistentInfo()->getPersistentCHTable()->classGotInitialized(
                        vm, compInfo->persistentMemory(), clazz)) {
                    TR_PersistentClassInfo* arrayClazzInfo
                        = compInfo->getPersistentInfo()->getPersistentCHTable()->findClassInfo(clazz);
                    if (arrayClazzInfo)
                        compInfo->getPersistentInfo()->getPersistentCHTable()->removeClass(
                            vm, clazz, arrayClazzInfo, false);
                }
                ...
                if (compClazz) {
                    ...
                    if (clazzInfo && !clazzInfo->isInitialized()) {
                        ...
                        if (!compInfo->getPersistentInfo()->getPersistentCHTable()->classGotInitialized(
                                vm, compInfo->persistentMemory(), compClazz))
                            initFailed = true;
        ...
        if (lwOffset > 0) {
            ...
            if (reserve) {
                TR_PersistentClassInfo* classInfo
                    = compInfo->getPersistentInfo()->getPersistentCHTable()->findClassInfoAfterLocking(clazz, vm);
...
static void jitHookClassPreinitialize(J9HookInterface** hookInterface, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (TR::Options::getCmdLineOptions()->allowRecompilation()
        && !TR::Options::getCmdLineOptions()->getOption(TR_DisableCHOpts)) {
        if (!initFailed
            && !compInfo->getPersistentInfo()->getPersistentCHTable()->classGotInitialized(
                   vm, compInfo->persistentMemory(), clazz))
            initFailed = true;
...
static void dumpStats(J9JITConfig* jitConfig)
{
    ...
    if (jitConfig->totalCodeBytesUsed) {
        gcOverhead
            = (UDATA)(((SYS_FLOAT)jitConfig->totalGCDataBytesUsed / (SYS_FLOAT)jitConfig->totalCodeBytesUsed) * 100.0);
        atlasOverhead = (UDATA)(
    ...
    TR_VerboseLog::writeLine(
        TR_Vlog_INFO, "  %9d gcMap bytes (~%3d%% of code size)", jitConfig->totalGCDataBytesUsed, (UDATA)gcOverhead);
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "  %9d atlas bytes (~%3d%% of code size)",
        jitConfig->totalAtlasDataBytesUsed, (UDATA)atlasOverhead);
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "  %9d debug bytes (~%3d%% of code size)",
        jitConfig->totalDebugDataBytesUsed, (UDATA)debugOverhead);
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "  %9d code + data bytes",
        (jitConfig->totalCodeBytesUsed + jitConfig->totalGCDataBytesUsed + jitConfig->totalAtlasDataBytesUsed
            + jitConfig->totalDebugDataBytesUsed));
    ...
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "  %6d of %6d classes            (~%3d%%)", jitConfig->unresolvedClassRefs,
        ...
        (jitConfig->totalClassRefs
                ? (UDATA)(((SYS_FLOAT)jitConfig->unresolvedClassRefs / (SYS_FLOAT)jitConfig->totalClassRefs) * 100.0)
                : 0));
    ...
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "  %6d of %6d static methods     (~%3d%%)",
        ...
        (jitConfig->totalStaticMethodRefs ? (UDATA)(((SYS_FLOAT)jitConfig->unresolvedStaticMethodRefs
                                                        / (SYS_FLOAT)jitConfig->totalStaticMethodRefs)
                                                * 100.0)
                                          : 0));

    TR_VerboseLog::writeLine(TR_Vlog_INFO, "  %6d of %6d special methods    (~%3d%%)",
        jitConfig->unresolvedSpecialMethodRefs, jitConfig->totalSpecialMethodRefs,
        (jitConfig->totalSpecialMethodRefs ? (UDATA)(((SYS_FLOAT)jitConfig->unresolvedSpecialMethodRefs
                                                         / (SYS_FLOAT)jitConfig->totalSpecialMethodRefs)
                                                 * 100.0)
                                           : 0));

    TR_VerboseLog::writeLine(TR_Vlog_INFO, "  %6d of %6d interface methods  (~%3d%%)",
        jitConfig->unresolvedInterfaceMethodRefs, jitConfig->totalInterfaceMethodRefs,
        (jitConfig->totalInterfaceMethodRefs ? (UDATA)(((SYS_FLOAT)jitConfig->unresolvedInterfaceMethodRefs
                                                           / (SYS_FLOAT)jitConfig->totalInterfaceMethodRefs)
                                                   * 100.0)
                                             : 0));

    TR_VerboseLog::writeLine(TR_Vlog_INFO, "  %6d of %6d virtual methods    (~%3d%%)",
        jitConfig->unresolvedVirtualMethodRefs, jitConfig->totalVirtualMethodRefs,
        (jitConfig->totalVirtualMethodRefs ? (UDATA)(((SYS_FLOAT)jitConfig->unresolvedVirtualMethodRefs
                                                         / (SYS_FLOAT)jitConfig->totalVirtualMethodRefs)
                                                 * 100.0)
                                           : 0));

    TR_VerboseLog::writeLine(TR_Vlog_INFO, "  %6d of %6d static variables   (~%3d%%)",
        jitConfig->unresolvedStaticVariableRefs, jitConfig->totalStaticVariableRefs,
        (jitConfig->totalStaticVariableRefs ? (UDATA)(((SYS_FLOAT)jitConfig->unresolvedStaticVariableRefs
                                                          / (SYS_FLOAT)jitConfig->totalStaticVariableRefs)
                                                  * 100.0)
                                            : 0));

    TR_VerboseLog::writeLine(TR_Vlog_INFO, "  %6d of %6d instance variables (~%3d%%)",
        jitConfig->unresolvedInstanceFieldRefs, jitConfig->totalInstanceFieldRefs,
        (jitConfig->totalInstanceFieldRefs ? (UDATA)(((SYS_FLOAT)jitConfig->unresolvedInstanceFieldRefs
                                                         / (SYS_FLOAT)jitConfig->totalInstanceFieldRefs)
                                                 * 100.0)
                                           : 0));
    ...
    if (debug("monenterstats")) {
        ...
        TR_VerboseLog::writeLine(
            TR_Vlog_INFO, "Total number of nested monenters executed inline = %d", totalNestedMonEnters);
        TR_VerboseLog::writeLine(
            TR_Vlog_INFO, "Total number of uncontended monenters executed inline = %d", totalUncontendedMonEnters);
        TR_VerboseLog::writeLine(TR_Vlog_INFO, "Total number of contended monenters executed out of line = %d",
            totalMonEnters - totalNestedMonEnters - totalUncontendedMonEnters);
    }
#endif
...
void JitShutdown(J9JITConfig* jitConfig)
{
    ...
    if (options && options->getOption(TR_DumpFinalMethodNamesAndCounts)) {
        try {
            ...
            J9::SegmentAllocator segmentAllocator(
                MEMORY_TYPE_JIT_SCRATCH_SPACE | MEMORY_TYPE_VIRTUAL, *jitConfig->javaVM);
            J9::SystemSegmentProvider regionSegmentProvider(
                1 << 20, 1 << 20, TR::Options::getScratchSpaceLimit(), segmentAllocator, rawAllocator);
    ...
    */
    ...
    if (options && options->getOption(TR_VerboseInlineProfiling)) {
        ...
        j9tty_printf(
            PORTLIB, "\tFailed to devirtualize virtual calls:    %10d\n", TR::Options::INLINE_failedToDevirtualize);
        ...
        j9tty_printf(
            PORTLIB, "\tCallee method has too many nodes:        %10d\n", TR::Options::INLINE_calleeHasTooManyNodes);
        j9tty_printf(
            PORTLIB, "\tRan out of inlining budget:              %10d\n\n", TR::Options::INLINE_ranOutOfBudget);
    ...
    // Make it easier to stay in STARTUP if we are already in startup, but harder otherwise
        ...
        || (oldState != STARTUP_STATE && // to come back to STARTUP we need at least one class to be loaded
               ...
               && ((compInfo->_intervalStats._compiledMethodSamples == 0) ||
                      // If many interpreter samples then go to STARTUP, but only if we have enough overall samples to
                      // decide
    ...
    // first-time-compilations/sec > threshold ==> must take into account machine speed; counts also affect this
    {
        ...
        lastTimeInJITStartupMode = crtElapsedTime;
                   ...
                   && ((compInfo->getMethodQueueSize() > TR::CompilationInfo::SMALL_QUEUE)
                          ...
                          //(compInfo->_intervalStats._interpretedMethodSamples*1000/diffTime > 10) // too many int
                          //samples ==> don't go to steady state yet
    ...
    } else // We will go to STEADY or DEEP_STEADY
    {
        if (oldState == DEEPSTEADY_STATE || // keep the DEEPSTEADY_STATE
            ...
            oldState == STEADY_STATE
                && (persistentInfo->getJitTotalSampleCount() - persistentInfo->getJitSampleCountWhenActiveStateEntered()
                       > 60000))
    ...
    if (!TR::Options::getAOTCmdLineOptions()->getOption(TR_DisableGuardedCountingRecompilations)
        ...
        } else // GCR counting is already enabled; see if we need to disable it
        {
            if ((TR::Options::getAOTCmdLineOptions()->getOption(TR_EnableMultipleGCRPeriods)
                ...
                (newState == STARTUP_STATE || newState == DEEPSTEADY_STATE
        ...
        if (TR::Options::_hwProfilerExpirationTime != 0 && crtElapsedTime > TR::Options::_hwProfilerExpirationTime) {
            if (!hwProfiler->isExpired()) {
                ...
                if (TR::Options::isAnyVerboseOptionSet(TR_VerboseHWProfiler, TR_VerbosePerformance))
                    TR_VerboseLog::writeLineLocked(
                        TR_Vlog_HWPROFILER, "Buffer processing is disabled because expiration time has been reached");
            }
        ...
        } else // javaVM->phase == J9VM_PHASE_EARLY_STARTUP
        {
            ...
            if (!TR::Options::getCmdLineOptions()->getOption(TR_AssumeStartupPhaseUntilToldNotTo)
                ...
                if (crtElapsedTime
                    ...
                    javaVM->internalVMFunctions->jvmPhaseChange(
                        javaVM, (newState == STARTUP_STATE) ? J9VM_PHASE_STARTUP : J9VM_PHASE_NOT_STARTUP);
            } else {
                ...
                javaVM->internalVMFunctions->jvmPhaseChange(javaVM,
                    !persistentInfo->getExternalStartupEndedSignal() ? J9VM_PHASE_STARTUP : J9VM_PHASE_NOT_STARTUP);
            }
        }
        if (javaVM->phase == J9VM_PHASE_NOT_STARTUP) {
            ...
            if (TR::Options::getCmdLineOptions()->isAnyVerboseOptionSet(
                    TR_VerboseJitState, TR_VerboseCompileEnd, TR_VerbosePerformance)) {
                TR_VerboseLog::writeLineLocked(
                    TR_Vlog_JITSTATE, "t=%6u VM changed state to NOT_STARTUP", (uint32_t)crtElapsedTime);
            }
            ...
            if ((TR::Options::getCmdLineOptions()->getOption(TR_RestrictInlinerDuringStartup)
                ...
                if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance)) {
                    TR_VerboseLog::writeLineLocked(
                        TR_Vlog_PERF, "t=%6u Stopped restricting the inliner", (uint32_t)crtElapsedTime);
                }
            }
            ...
            if (TR::Options::getCmdLineOptions()->getOption(TR_UseHigherMethodCountsAfterStartup)
                && TR::Options::sharedClassCache()) {
                if (TR::Options::getCmdLineOptions()->isAnyVerboseOptionSet(
                        TR_VerboseJitState, TR_VerbosePerformance)) {
...
void CalculateOverallCompCPUUtilization(TR::CompilationInfo* compInfo, uint64_t crtTime, J9VMThread* currentThread)
{
    if (compInfo->getOverallCompCpuUtilization() >= 0) // No error so far
    {
        ...
        if (TR::Options::isAnyVerboseOptionSet(TR_VerboseCompilationThreads, TR_VerboseCompilationThreadsDetails)) {
            TR_VerboseLog::vlogAcquire();
            TR_VerboseLog::writeLine(
                TR_Vlog_INFO, "t=%6u TotalCompCpuUtil=%3d%%.", (uint32_t)crtTime, totalCompCPUUtilization);
            TR::CompilationInfoPerThread* const* arrayOfCompInfoPT = compInfo->getArrayOfCompilationInfoPerThread();
            for (uint8_t i = 0; i < compInfo->getNumUsableCompilationThreads(); i++) {
                const CpuSelfThreadUtilization& cpuUtil = arrayOfCompInfoPT[i]->getCompThreadCPU();
                TR_VerboseLog::write(" compThr%d:%3d%% (%2d%%, %2d%%) ", i, cpuUtilizationValues[i],
                ...
                if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseCompilationThreadsDetails))
                    TR_VerboseLog::write("(%dms, %dms, lastCheckpoint=%u) ",
                        (int32_t)cpuUtil.getLastMeasurementInterval() / 1000000,
                        (int32_t)cpuUtil.getSecondLastMeasurementInterval() / 1000000,
...
void CPUThrottleLogic(TR::CompilationInfo* compInfo, uint64_t crtTime)
{
    ...
    if (totalCompCPUUtilization >= 0) {
        ...
        // The solution is to go into a trasient state; so from TR_yes we go into TR_maybe and from TR_maybe we go into
        // TR_no
            ...
            && oldThrottleValue != compInfo->exceedsCompCpuEntitlement()) // did the value change?
        {
            TR_VerboseLog::writeLineLocked(TR_Vlog_INFO,
                ...
                (uint32_t)crtTime,
                compInfo->exceedsCompCpuEntitlement() == TR_yes
                    ? "YES"
                    : compInfo->exceedsCompCpuEntitlement() == TR_maybe ? "MAYBE" : "NO",
        ...
        if (numTicksCapped < 2) {
            if (numTicksCapped == 0) {
                ...
                if (TR::Options::getCmdLineOptions()->getOption(TR_TurnOffSelectiveNoOptServerIfNoStartupHint)
                    ...
                    if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance))
                        TR_VerboseLog::writeLineLocked(
                            TR_Vlog_INFO, "t=%u selectiveNoOptServer feature turned off", crtElapsedTime);
                }
            }
            numTicksCapped++;
        }
...
// int32_t samplerThreadStateFreqencies[TR::CompilationInfo::SAMPLER_LAST_STATE+1] = {0, 2, 1000, 100000, INT_MAX,
// INT_MAX, -1};
...
void getOutOfIdleStatesUnlocked(
    TR::CompilationInfo::TR_SamplerStates expectedState, TR::CompilationInfo* compInfo, const char* reason)
{
...
void samplerThreadStateLogic(TR::CompilationInfo* compInfo, TR_FrontEnd* fe, int32_t numActiveThreads)
{
    ...
    if (newVMState != currentVMState) {
        if (vm->internalVMFunctions->updateVMRuntimeState(vm, newVMState)
            && (TR::Options::getVerboseOption(TR_VerbosePerformance)))
            TR_VerboseLog::writeLineLocked(
                TR_Vlog_PERF, "t=%u\tSampling thread changed VM state to %u", (uint32_t)crtTime, newVMState);
    }
...
void inlinerAggressivenessLogic(TR::CompilationInfo* compInfo)
{
    ...
    if (TR::Options::getCmdLineOptions()->getOption(TR_UseVmTotalCpuTimeAsAbstractTime)) {
        ...
        } else // if we cannot get JVM cpu utilization, then the options to tune the algorithm will contain wrong values
        {
            ...
            if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance))
                TR_VerboseLog::writeLineLocked(
                    TR_Vlog_INFO, "Changed from JVM time to JIT samples for abstract time measurement");
            // Set default values
    ...
    if (inliningAggressiveness != persistentInfo->getInliningAggressiveness()) {
        ...
        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePerformance))
            TR_VerboseLog::writeLineLocked(
                TR_Vlog_INFO, "inliningAggressiveness changed to %d", inliningAggressiveness);
    }
}
...
static int32_t J9THREAD_PROC samplerThreadProc(void* entryarg)
{
    ...
    if (TR::Options::isAnyVerboseOptionSet()) {
        ...
        if (phMemAvail != OMRPORT_MEMINFO_NOT_AVAILABLE)
            TR_VerboseLog::writeLine(
                TR_Vlog_INFO, "Free Physical Memory: %lld MB %s", phMemAvail >> 20, incomplete ? "estimated" : "");
        else
        ...
        if (compInfo->isWarmSCC() != TR_maybe && TR::Options::getVerboseOption(TR_VerbosePerformance)) {
            ...
            TR_VerboseLog::writeLine(
                TR_Vlog_INFO, "\tSCCname:%s   SCCpath:%s", javacoreData->cacheName, javacoreData->cacheDir);
            TR_VerboseLog::writeLine(TR_Vlog_INFO,
    ...
    while (!shutdownSamplerThread) {
                ...
                == 0) // Anything non-0 is an error condition so we shouldn't do the sampling //!= J9THREAD_INTERRUPTED)
        {
            ...
            if (crtTime - oldSyncTime >= 100) // every 100 ms
            {
                ...
                if (crtTime - lastMinuteCheck >= (TR::Options::_virtualMemoryCheckFrequencySec * 1000)) {
                    ...
                    if (TR::Options::sharedClassCache() && TrcEnabled_Trc_JIT_SCCInfo && vm->sharedClassConfig
                        ...
                        vm->sharedClassConfig->getJavacoreData(
                            vm, scc); // need to rebuild javacore data or else it will be stale
                        Trc_JIT_SCCInfo(samplerThread, scc->cacheName, scc->cacheDir, scc->cacheSize, scc->freeBytes,
                            scc->softMaxBytes, scc->romClassBytes, scc->aotBytes, scc->aotDataBytes,
            ...
            do {
...
#endif /* J9VM_INTERP_ATOMIC_FREE_JNI */
                {
                    if (currentThread->publicFlags & J9_PUBLIC_FLAGS_VM_ACCESS) {
                        vm->internalVMFunctions->J9SignalAsyncEvent(
                            vm, currentThread, jitConfig->sampleInterruptHandlerKey);
            ...
            if (crtTime > lastProcNumCheck + 300000) // every 5 mins
            {
                ...
                // compInfo->setNumTargetCPUs((tempProc = j9sysinfo_get_number_CPUs_by_type(J9PORT_CPU_TARGET)) > 0 ?
                // tempProc : 1);
                ...
                if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseJitMemory)) {
                    ...
                    for (int32_t i = 0; i < LastAssumptionKind; i++)
                        TR_VerboseLog::writeLine(TR_Vlog_MEMORY, "\tAssumptionType=%d allocated=%d reclaimed=%d", i,
                            rat->getAssumptionCount(i), rat->getReclaimedAssumptionCount(i));
                    TR_VerboseLog::vlogRelease();
                }
                ...
                if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseVMemAvailable)) {
                    ...
                    if (j9sysinfo_get_memory_info(&memInfo) == 0
                        ...
                        TR_VerboseLog::writeLineLocked(TR_Vlog_MEMORY, "t=%6u VMemAv=%u MB", (uint32_t)crtTime,
                            (uint32_t)(memInfo.availVirtual >> 20));
                    }
                }
#endif
            ...
            samplingPeriod = std::max(static_cast<UDATA>(TR::Options::_minSamplingPeriod),
                (samplingFreq == MAX_SAMPLING_FREQUENCY) ? samplingFreq : samplingFreq * loadFactor);
            ...
            if (diffTime >= (uint32_t)TR::Options::_classLoadingPhaseInterval) // time to re-analyze
            {
                ...
                else if (TR::Options::isAnyVerboseOptionSet(
                             TR_VerboseCompilationThreads, TR_VerboseCompilationThreadsDetails)
...
void jitHookJNINativeRegistered(J9HookInterface** hookInterface, UDATA eventNum, void* eventData, void* userData)
{
    ...
    // Notify the assumptions table that the native has been registered
    {
        ...
        TR_RuntimeAssumptionTable* rat = compInfo->getPersistentInfo()->getRuntimeAssumptionTable();
        OMR::RuntimeAssumption** headPtr = rat->getBucketPtr(
            RuntimeAssumptionOnRegisterNative, TR_RuntimeAssumptionTable::hashCode((uintptrj_t)method));
...

{
    ...
    do {
        ...
        do {
            if (TR::Options::getCmdLineOptions()->realTimeGC()
                && !TR::Options::getCmdLineOptions()->getOption(TR_DisableIncrementalCCR))
    ...
    if (TR::Options::getCmdLineOptions()->realTimeGC()
        && !TR::Options::getCmdLineOptions()->getOption(TR_DisableIncrementalCCR)) { // clear flags
...
int32_t setUpHooks(J9JavaVM* javaVM, J9JITConfig* jitConfig, TR_FrontEnd* vm)
{
    ...
    if (TR::Options::getCmdLineOptions()->getOption(TR_noJitDuringBootstrap)
        ...
        if ((*vmHooks)->J9HookRegisterWithCallSite(
                vmHooks, J9HOOK_VM_LOOKUP_JNI_ID, jitHookAboutToRunMain, OMR_GET_CALLSITE(), NULL)) {
    ...
    if (TR::Options::getCmdLineOptions()->getOption(TR_RTGCMapCheck))
        initJitGCMapCheckAsyncHook(
            javaVM, javaVM->internalVMFunctions->J9RegisterAsyncEvent(javaVM, jitGCMapCheck, NULL), jitConfig);
    ...
    if (jitConfig->runtimeFlags & J9JIT_TOSS_CODE) {
        ...
        if ((*vmHooks)->J9HookRegisterWithCallSite(
                vmHooks, J9HOOK_VM_INITIALIZE_SEND_TARGET, jitHookInitializeSendTarget, OMR_GET_CALLSITE(), NULL)) {
        ...
        if (!TR::Options::getCmdLineOptions()->getOption(TR_DisableInterpreterProfiling) && iProfiler
            ...
            } else // start Iprofiler right away
            {
                if ((*vmHooks)->J9HookRegisterWithCallSite(vmHooks, J9HOOK_VM_PROFILING_BYTECODE_BUFFER_FULL,
                    ...
                    j9tty_printf(
                        PORTLIB, "Error: Unable to install J9HOOK_VM_PROFILING_BYTECODE_BUFFER_FULL listener\n");
                    return -1;
                }
                interpreterProfilingState = IPROFILING_STATE_ON;
            }
    ...
    if ((*gcOmrHooks)
            ->J9HookRegisterWithCallSite(
                gcOmrHooks, J9HOOK_MM_OMR_LOCAL_GC_START, jitHookLocalGCStart, OMR_GET_CALLSITE(), NULL)
        || (*gcOmrHooks)
               ->J9HookRegisterWithCallSite(
                   gcOmrHooks, J9HOOK_MM_OMR_LOCAL_GC_END, jitHookLocalGCEnd, OMR_GET_CALLSITE(), NULL)
        || (*gcOmrHooks)
               ->J9HookRegisterWithCallSite(
                   gcOmrHooks, J9HOOK_MM_OMR_GLOBAL_GC_START, jitHookGlobalGCStart, OMR_GET_CALLSITE(), NULL)
        || (*gcOmrHooks)
               ->J9HookRegisterWithCallSite(
                   gcOmrHooks, J9HOOK_MM_OMR_GLOBAL_GC_END, jitHookGlobalGCEnd, OMR_GET_CALLSITE(), NULL)) {
    ...
    if (!vmj9->isAOT_DEPRECATED_DO_NOT_USE()) {
        ...
        if (TR::Options::getCmdLineOptions()->realTimeGC()) {
            ...
            unableToRegisterHooks = ((*gcOmrHooks)
                                         ->J9HookRegisterWithCallSite(gcOmrHooks, J9HOOK_MM_OMR_GLOBAL_GC_END,
                                             jitHookReleaseCodeGlobalGCEnd, OMR_GET_CALLSITE(), NULL)
                || (*gcOmrHooks)
                       ->J9HookRegisterWithCallSite(gcOmrHooks, J9HOOK_MM_OMR_LOCAL_GC_END,
                           jitHookReleaseCodeLocalGCEnd, OMR_GET_CALLSITE(), NULL));
        }
    ...
    if ((*vmHooks)->J9HookRegisterWithCallSite(
            vmHooks, J9HOOK_VM_INTERNAL_CLASS_LOAD, jitHookClassLoad, OMR_GET_CALLSITE(), NULL)
        || (*vmHooks)->J9HookRegisterWithCallSite(
               vmHooks, J9HOOK_VM_CLASS_PREINITIALIZE, jitHookClassPreinitialize, OMR_GET_CALLSITE(), NULL)
        || (*vmHooks)->J9HookRegisterWithCallSite(
               vmHooks, J9HOOK_VM_CLASS_INITIALIZE, jitHookClassInitialize, OMR_GET_CALLSITE(), NULL)) {
...
#endif
        if ((*vmHooks)->J9HookRegisterWithCallSite(
                vmHooks, J9HOOK_VM_CLASS_UNLOAD, jitHookClassUnload, OMR_GET_CALLSITE(), NULL)
            || (*vmHooks)->J9HookRegisterWithCallSite(
                   vmHooks, J9HOOK_VM_CLASSES_UNLOAD, jitHookClassesUnload, OMR_GET_CALLSITE(), NULL)
            || (*vmHooks)->J9HookRegisterWithCallSite(
                   vmHooks, J9HOOK_VM_CLASS_LOADER_UNLOAD, jitHookClassLoaderUnload, OMR_GET_CALLSITE(), NULL)
            || (*vmHooks)->J9HookRegisterWithCallSite(
                   vmHooks, J9HOOK_VM_ANON_CLASSES_UNLOAD, jitHookAnonClassesUnload, OMR_GET_CALLSITE(), NULL)
            || (*gcHooks)->J9HookRegisterWithCallSite(
                   gcHooks, J9HOOK_MM_INTERRUPT_COMPILATION, jitHookInterruptCompilation, OMR_GET_CALLSITE(), NULL)
            || (*gcHooks)->J9HookRegisterWithCallSite(
                   gcHooks, J9HOOK_MM_CLASS_UNLOADING_END, jitHookClassesUnloadEnd, OMR_GET_CALLSITE(), NULL)) {
            j9tty_printf(PORTLIB, "Error: Unable to register class event hook\n");
            return -1;
        }
    }
    ...
    j9thread_monitor_enter(javaVM->vmThreadListMutex);
    if ((*vmHooks)->J9HookRegisterWithCallSite(
            vmHooks, J9HOOK_VM_THREAD_CREATED, jitHookThreadCreate, OMR_GET_CALLSITE(), NULL)
        || (*vmHooks)->J9HookRegisterWithCallSite(
               vmHooks, J9HOOK_VM_THREAD_END, jitHookThreadEnd, OMR_GET_CALLSITE(), NULL)
        || (*vmHooks)->J9HookRegisterWithCallSite(
               vmHooks, J9HOOK_VM_THREAD_CRASH, jitHookThreadCrash, OMR_GET_CALLSITE(), NULL)
        || (*vmHooks)->J9HookRegisterWithCallSite(
               vmHooks, J9HOOK_VM_THREAD_DESTROY, jitHookThreadDestroy, OMR_GET_CALLSITE(), NULL)
        || (*vmHooks)->J9HookRegisterWithCallSite(
               vmHooks, J9HOOK_VM_THREAD_STARTED, jitHookThreadStart, OMR_GET_CALLSITE(), NULL)) {
        ...
        if (currThread) {
            ...
            if (lmGuardedStorage) {
                // TODO (GuardedStorage): jitHookThreadStart is never triggered on the main thread, since it's created
                // before the JIT enables the hook. Since the main thread can execute JIT code we still need to
                // initialize it for guarded storage.
    ...
    if (!vmj9->isAOT_DEPRECATED_DO_NOT_USE()) {
        if ((*vmHooks)->J9HookRegisterWithCallSite(
                vmHooks, J9HOOK_VM_JNI_NATIVE_REGISTERED, jitHookJNINativeRegistered, OMR_GET_CALLSITE(), NULL)) {
