...
void J9::Recompilation::setupMethodInfo()
{
    ...
    if (_firstCompile) {
        ...
        //
        _methodInfo->setNextCompileLevel(
            optimizationPlan->getOptLevel(), (optimizationPlan->insertInstrumentation() != 0));
    ...
    _bodyInfo = TR_PersistentJittedBodyInfo::allocate(
        _methodInfo, _compilation->getMethodHotness(), (optimizationPlan->insertInstrumentation() != 0), _compilation);
    ...
    if (_compilation->getProfilingMode() == JProfiling && _compilation->getSkippedJProfilingBlock()) {
        TR::DebugCounter::incStaticDebugCounter(_compilation,
            TR::DebugCounter::debugCounterName(
                _compilation, "jprofiling.restartCompile/(%s)", _compilation->signature()));
...
void J9::Recompilation::shutdown()
{
    ...
    if (TR_RecompilationStats) {
        ...
        TR_VerboseLog::writeLine(
            TR_Vlog_INFO, "Methods recompiled via hot threshold = %d", hotThresholdMethodsCompiled);
        TR_VerboseLog::writeLine(
            TR_Vlog_INFO, "Methods recompiled via scorching threshold = %d", scorchingThresholdMethodsCompiled);
    }
}
...
TR_PersistentJittedBodyInfo::TR_PersistentJittedBodyInfo(
    TR_PersistentMethodInfo* methodInfo, TR_Hotness hotness, bool profile, TR::Compilation* comp)
    : _methodInfo(methodInfo)
    , _counter(INT_MAX)
...
TR_PersistentJittedBodyInfo* TR_PersistentJittedBodyInfo::allocate(
    TR_PersistentMethodInfo* methodInfo, TR_Hotness hotness, bool profile, TR::Compilation* comp)
{
