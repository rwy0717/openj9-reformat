...
char* compilationErrorNames[] = { "compilationOK", // 0
...
extern "C" void getOutOfIdleStates(
...
extern "C" IDATA j9jit_testarossa(
    struct J9JITConfig* jitConfig, J9VMThread* vmThread, J9Method* method, void* oldStartPC)
{
...
extern "C" IDATA retranslateWithPreparationForMethodRedefinition(
    struct J9JITConfig* jitConfig, J9VMThread* vmThread, J9Method* method, void* oldStartPC)
{
    return retranslateWithPreparation(
        jitConfig, vmThread, method, oldStartPC, TR_PersistentMethodInfo::RecompDueToInlinedMethodRedefinition);
}

extern "C" IDATA retranslateWithPreparation(
    struct J9JITConfig* jitConfig, J9VMThread* vmThread, J9Method* method, void* oldStartPC, UDATA reason)
{
    if (!TR::CompilationInfo::get()->asynchronousCompilation()
        && !TR_LinkageInfo::get(oldStartPC)->recompilationAttempted()) {
        TR::Recompilation::fixUpMethodCode(oldStartPC);
    }
...
extern "C" J9Method* getNewInstancePrototype(J9VMThread* context)
{
    ...
    J9Class* jlClass = intFunc->internalFindKnownClass(
        context, J9VMCONSTANTPOOL_JAVALANGCLASS, J9_FINDKNOWNCLASS_FLAG_EXISTING_ONLY);
    if (jlClass) {
...
extern "C" UDATA j9jit_createNewInstanceThunk(
    struct J9JITConfig* jitConfig, J9VMThread* vmThread, J9Class* classNeedingThunk)
{
    ...
    UDATA flagsParm, void* reserved0, I_32 xnojit)
{
    ...
    if (!TR_TranslationArtifactManager::initializeGlobalArtifactManager(
            jitConfig->translationArtifacts, jitConfig->javaVM))
        return -1;
    ...
    if (TR::Options::_hwProfilerEnabled == TR_yes) {
        ...
        ((TR_JitPrivateConfig*)(jitConfig->privateConfig))->hwProfiler
            = processor >= TR_PPCp8 ? TR_PPCHWProfiler::allocate(jitConfig) : NULL;
#else
    ...
    ((TR_JitPrivateConfig*)(jitConfig->privateConfig))->lmGuardedStorage
        = enableLMGS ? TR_PPCLMGuardedStorage::allocate(jitConfig, ebbSetupDone) : NULL;
#else
    ...
    if (!TR::Compiler->target.cpu.isI386()) {
        TR_J2IThunkTable* ieThunkTable
            = new (PERSISTENT_NEW) TR_J2IThunkTable(persistentMemory, "InvokeExactJ2IThunkTable");
...
typedef struct DebugExtParameters {
    DebugExtParameters(const char* name, void* addr, UDATA argCount, const char* args)
        : _name(name)
        , _addr(addr)
        , _argCount(argCount)
        , _args(args)
...
static UDATA blankDebugExtSignalHandler(struct J9PortLibrary* portLibrary, U_32 gpType, void* gpInfo, void* arg)
{
    dbgjit_Printf("\n\n\n");
    dbgjit_Printf("**** Debug Ext Signal Handler: trprint most likely tried to dereference a remote pointer (VFT or "
                  "otherwise).\n");
...
extern "C" TR_DebugExt* j9jit_createDebugExt(J9JavaVM* localVM, J9PortLibrary* dbgPortLib, TR_Printf_t dbgPrintf,
    void (*dbgReadMemory)(UDATA remoteAddr, void* localPtr, UDATA size, UDATA* bytesRead),
    UDATA (*dbgGetExpression)(const char* args), TR_Malloc_t dbgMalloc, TR_Free_t dbgFree)
{
    ...
    TR_InternalFunctions* jitFunctions
        = new (dbgMalloc) TR_InternalFunctionsExt(NULL, NULL, dbgPrintf, dbgMalloc, dbgFree);
    TR_DebugExt* debugExt = new (dbgMalloc)
        TR_DebugExt(jitFunctions, dbgPortLib, localVM, dbgPrintf, dbgReadMemory, dbgMalloc, dbgFree, dbgGetExpression);
    return debugExt;
}

extern "C" void JNICALL dbgjit_TrInitialize(J9JavaVM* localVM, J9PortLibrary* dbgPortLib,
    void (*dbgPrintf)(const char* s, ...),
    void (*dbgReadMemory)(UDATA remoteAddr, void* localPtr, UDATA size, UDATA* bytesRead),
    UDATA (*dbgGetExpression)(const char* args), void* (*dbgMalloc)(UDATA size, void* originalAddress),
    void* (*dbgMallocAndRead)(UDATA size, void* remoteAddress), void (*dbgFree)(void* addr),
        ...
        UDATA bytesToSearch, UDATA* bytesSearched))
{
    ...
    trDebugExt = j9jit_createDebugExt(
        localVM, dbgPortLib, dbgPrintf, dbgReadMemory, dbgGetExpression, dbgMalloc, dbgFree); /* safe upcast */
    return;
}

static UDATA dbgjit_TrPrintProtected(J9PortLibrary* portLib, void* opaqueParameters)
{
...
extern "C" void JNICALL dbgjit_TrPrint(const char* name, void* addr, UDATA argCount, const char* args)
{
    PORT_ACCESS_FROM_PORT(portLibrary);

    DebugExtParameters p(name, addr, argCount, args);
    ...
    if (j9sig_can_protect(flags)) {
        ...
        protectedResult = j9sig_protect((j9sig_protected_fn)dbgjit_TrPrintProtected, static_cast<void*>(&p),
            (j9sig_handler_fn)blankDebugExtSignalHandler, mainThread, flags, &result);
    } else
#endif
