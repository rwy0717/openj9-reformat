...
void TR::PatchMultipleNOPedGuardSites::dumpInfo()
{
    ...
    for (int i = 0; i < _patchSites->getSize(); ++i)
        TR_VerboseLog::write(
            " %d location=%p destination=%p", i, _patchSites->getLocation(i), _patchSites->getDestination(i));
}
...
TR_PatchNOPedGuardSiteOnMutableCallSiteChange* TR_PatchNOPedGuardSiteOnMutableCallSiteChange::make(TR_FrontEnd* fe,
    TR_PersistentMemory* pm, uintptrj_t key, uint8_t* location, uint8_t* destination, OMR::RuntimeAssumption** sentinel)
{
...
bool TR_CHTable::commit(TR::Compilation* comp)
{
    ...
    if (comp->fej9()->isAOT_DEPRECATED_DO_NOT_USE())
        return true;
    if (vguards.empty() && sideEffectPatchSites->empty() && !_preXMethods && !_classes
    ...
    if (_preXMethods) {
        ...
        for (i = 0; i <= last; ++i) {
            ...
            TR_PreXRecompileOnMethodOverride::make(
                comp->fe(), comp->trPersistentMemory(), method, startPC, comp->getMetadataAssumptionList());
    ...
    if (_classes) {
        ...
        for (int32_t i = 0; i <= last; ++i) {
            ...
            if (!alreadyAdded) {
                ...
                TR_PreXRecompileOnClassExtend::make(
                    comp->fe(), comp->trPersistentMemory(), classId, startPC, comp->getMetadataAssumptionList());
    ...
    if (_classesThatShouldNotBeNewlyExtended) {
        ...
        for (auto cl = it.getFirst(); cl; cl = it.getNext()) {
            ...
            TR_PreXRecompileOnClassExtend::make(
                comp->fe(), comp->trPersistentMemory(), cl->getClassId(), startPC, comp->getMetadataAssumptionList());
            comp->setHasClassExtendAssumptions();
        }
    ...
    for (int i = 0; i < clazzesOnStaticFinalFieldModification->size(); ++i) {
        ...
        if (TR::Compiler->cls.classHasIllegalStaticFinalFieldModification(clazz)) {
            if (TR::Options::isAnyVerboseOptionSet(
                    TR_VerboseRuntimeAssumptions, TR_VerboseCompileEnd, TR_VerbosePerformance, TR_VerboseCompFailure)) {
...
void TR_CHTable::commitOSRVirtualGuards(TR::Compilation* comp, TR::list<TR_VirtualGuard*>& vguards)
{
...
void TR_CHTable::commitVirtualGuard(
    TR_VirtualGuard* info, List<TR_VirtualGuardSite>& sites, TR_PersistentCHTable* table, TR::Compilation* comp)
{
    ...
    if (info->getKind() == TR_DummyGuard) {
        ...
        if (!dontInvalidateMCSTargetGuards) {
...

            {
                ...
                uintptrj_t currentEpoch = fej9->getVolatileReferenceField(
                    *mcsReferenceLocation, "epoch", "Ljava/lang/invoke/MethodHandle;");
                if (currentEpoch)
        ...
        if (guardedMethod->virtualMethodIsOverridden())
            nopAssumptionIsValid = false;
                   ...
                   && !TR::Compiler->cls.isAbstractClass(
                          comp, resolvedMethodSymbol->getResolvedMethod()->containingClass()))
...
public:
    CollectCompiledImplementors(TR::Compilation* comp, TR_OpaqueClassBlock* topClassId, TR_ResolvedMethod** implArray,
        ...
        TR_YesNoMaybe useGetResolvedInterfaceMethod = TR_maybe)
        : CollectImplementors(
              comp, topClassId, implArray, maxCount + 1, callerMethod, slotOrIndex, useGetResolvedInterfaceMethod)
    {
        _hotness = hotness;
    }
    virtual bool visitSubclass(TR_PersistentClassInfo* cl);
...
void TR_ClassQueries::getSubClasses(
    TR_PersistentClassInfo* clazz, TR_ScratchList<TR_PersistentClassInfo>& list, TR_FrontEnd* fe, bool locked)
{
    ...
    TR_YesNoMaybe useGetResolvedInterfaceMethod)
{
    ...
    CollectImplementors collector(
        comp, clazz->getClassId(), implArray, maxCount, callerMethod, slotOrIndex, useGetResolvedInterfaceMethod);
...
void TR_ClassQueries::collectLeafs(
    TR_PersistentClassInfo* clazz, TR_ScratchList<TR_PersistentClassInfo>& leafs, TR::Compilation* comp, bool locked)
{
...
void TR_ClassQueries::collectLeafsLocked(
    TR_PersistentClassInfo* clazz, TR_ScratchList<TR_PersistentClassInfo>& leafs, TR_CHTable::VisitTracker& marked)
{
    marked.visit(clazz);
...
void TR_ClassQueries::collectAllSubClasses(
    TR_PersistentClassInfo* clazz, TR_ScratchList<TR_PersistentClassInfo>* leafs, TR::Compilation* comp, bool locked)
{
 ...
 */
void TR_ClassQueries::collectAllSubClassesLocked(
    TR_PersistentClassInfo* clazz, TR_ScratchList<TR_PersistentClassInfo>* leafs, TR_CHTable::VisitTracker& marked)
{
    TR_SubClass* subClass = clazz->_subClasses.getFirst();
...
void TR_ClassQueries::collectAllNonIFSubClasses(
    TR_PersistentClassInfo* clazz, TR_ScratchList<TR_PersistentClassInfo>& leafs, TR::Compilation* comp, bool locked)
{
...
void TR_ClassQueries::addAnAssumptionForEachSubClass(
    TR_PersistentCHTable* table, TR_PersistentClassInfo* clazz, List<TR_VirtualGuardSite>& list, TR::Compilation* comp)
{
...
int32_t TR_ClassQueries::countAllNonIFSubClassesWithDepth(
    TR_PersistentClassInfo* clazz, TR::Compilation* comp, int32_t maxcount, bool locked)
{
