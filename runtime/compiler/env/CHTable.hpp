...
public:
    ...
    {}
    TR_AOTGuardSite(
        TR_VirtualGuardKind type, TR_VirtualGuard* virtualGuard, TR::Node* node, TR::Node* aconstNode = NULL)
        : TR_VirtualGuardSite()
        , _guardType(type)
...
protected:
    TR_PatchNOPedGuardSiteOnClassExtend(
        TR_PersistentMemory* pm, TR_OpaqueClassBlock* clazz, uint8_t* loc, uint8_t* dest)
        : TR::PatchNOPedGuardSite(pm, (uintptrj_t)clazz, RuntimeAssumptionOnClassExtend, loc, dest)
    {}
...
protected:
    TR_PatchNOPedGuardSiteOnMethodOverride(
        ...
        : TR::PatchNOPedGuardSite(pm, (uintptrj_t)method, RuntimeAssumptionOnMethodOverride, loc, dest)
    {}
...
private:
    TR_PatchNOPedGuardSiteOnClassPreInitialize(
        TR_PersistentMemory* pm, char* sig, uint32_t sigLen, uint8_t* loc, uint8_t* dest)
        : TR::PatchNOPedGuardSite(pm, (uintptrj_t)sig, RuntimeAssumptionOnClassPreInitialize, loc, dest)
        , _sigLen(sigLen)
    {}
    uint32_t _sigLen;
};
...
protected:
    TR_PatchMultipleNOPedGuardSitesOnStaticFinalFieldModification(
        ...
        : TR::PatchMultipleNOPedGuardSites(
              pm, (uintptrj_t)clazz, RuntimeAssumptionOnStaticFinalFieldModification, sites)
    {}
...
protected:
    TR_PatchNOPedGuardSiteOnMutableCallSiteChange(TR_PersistentMemory* pm, uintptrj_t key, uint8_t* loc, uint8_t* dest)
...
public:
    static TR_PatchNOPedGuardSiteOnMutableCallSiteChange* make(TR_FrontEnd* fe, TR_PersistentMemory* pm,
        uintptrj_t KEY_WILL_GO_HERE, uint8_t* loc, uint8_t* dest, OMR::RuntimeAssumption** sentinel);
...
protected:
    TR_PatchNOPedGuardSiteOnMethodBreakPoint(
        TR_PersistentMemory* pm, TR_OpaqueMethodBlock* j9method, uint8_t* location, uint8_t* destination)
...
public:
    TR_PersistentFieldInfo(char* sig, int32_t sigLength = -1, TR_PersistentFieldInfo* next = NULL,
        uint8_t b = VALID_BUT_NOT_ALWAYS_INITIALIZED, char* c = 0, int32_t numChars = -1)
...
public:
    TR_PersistentArrayFieldInfo(char* sig, int32_t sigLength = -1, TR_PersistentArrayFieldInfo* next = NULL,
        ...
        uint8_t b2 = VALID_BUT_NOT_ALWAYS_INITIALIZED, char* c = "", int32_t numChars = -1)
...
public:
    ...
    static int32_t countAllNonIFSubClassesWithDepth(
        TR_PersistentClassInfo* clazz, TR::Compilation* comp, int32_t maxCount, bool locked = false);

    static void addAnAssumptionForEachSubClass(
        TR_PersistentCHTable* table, TR_PersistentClassInfo* clazz, List<TR_VirtualGuardSite>&, TR::Compilation* comp);

private:
    static void collectLeafsLocked(
        TR_PersistentClassInfo* clazz, TR_ScratchList<TR_PersistentClassInfo>& leafs, TR_CHTable::VisitTracker& marked);
    static void collectAllSubClassesLocked(
        TR_PersistentClassInfo* clazz, TR_ScratchList<TR_PersistentClassInfo>* leafs, TR_CHTable::VisitTracker& marked);
};
