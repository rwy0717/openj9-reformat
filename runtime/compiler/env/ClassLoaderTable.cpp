...
int32_t TR_PersistentClassLoaderTable::hashLoader(void* classLoaderPointer)
{
    return ((uintptrj_t)classLoaderPointer % CLASSLOADERTABLE_SIZE);
}

int32_t TR_PersistentClassLoaderTable::hashClassChain(void* classChainPointer)
{
...
void TR_PersistentClassLoaderTable::associateClassLoaderWithClass(void* classLoaderPointer, TR_OpaqueClassBlock* clazz)
{
    if (!sharedCache())
        return;
    ...
    if (!info) {
        ...
        if (classChainPointer) {
            ...
            TR_ASSERT(
                inCache, "rememberClass should only return class chain pointers that are in the shared class cache");
            ...
            if (info) {
                ...
                // we have added this loader to _loaderTable, which has nice side benefit that we won't keep trying it,
                // so
                //    leave it there
...
void* TR_PersistentClassLoaderTable::lookupClassChainAssociatedWithClassLoader(void* classLoaderPointer)
{
    if (!sharedCache())
        return NULL;
...
void* TR_PersistentClassLoaderTable::lookupClassLoaderAssociatedWithClassChain(void* classChainPointer)
{
    if (!sharedCache())
        return NULL;
...
void TR_PersistentClassLoaderTable::removeClassLoader(void* classLoaderPointer)
{
    // need to acquire some kind of lock here
