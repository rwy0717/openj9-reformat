...
FilePointer::FilePointer(::FILE* stream) { initialize(stream, false); }

void FilePointer::initialize(::FILE* stream, bool encrypt)
{
    _stream = stream;
    _useJ9IO = false;
    _buffer = 0;
...
void FilePointer::initialize(J9PortLibrary* portLib, int32_t fileId, bool encrypt)
{
...
void FilePointer::initialize(bool enc)
{
    _pos = 0;
    _encrypt = enc;
    _i = 0;
    ...
    if (enc) {
        ...
        //
        static const uint8_t key[] = { 0xa6, 0x93, 0xa6, 0x96, 0x47, 0xc6, 0x03, 0x76, 0x66, 0x13, 0xc6, 0x33 };
...
int32_t FilePointer::write(J9PortLibrary* portLib, char* buf, int32_t length)
{
    ...
    if (length > 0) {
        ...
        if (_useJ9IO) {
            if ((BUFFER_SIZE == 0) || (_pos == 0 && length > BUFFER_SIZE))
...
uint8_t FilePointer::cipher(uint8_t data)
{
    _i = (_i + 1) % 256;
    _j = (_j + _s[_i]) % 256;
    swap();
...
void FilePointer::encrypt(char* buf, int32_t len)
{
