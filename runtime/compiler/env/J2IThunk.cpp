...
TR_J2IThunk* TR_J2IThunk::allocate(
    int16_t codeSize, char* signature, TR::CodeGenerator* cg, TR_J2IThunkTable* thunkTable)
{
...
int16_t TR_J2IThunkTable::terseSignatureLength(char* signature)
{
    int16_t numArgs = 0;
...
TR_J2IThunkTable::Node* TR_J2IThunkTable::Node::get(
    char* terseSignature, TR_PersistentArray<Node>& nodeArray, bool createIfMissing)
{
    Node* result = NULL;
...
TR_J2IThunk* TR_J2IThunkTable::findThunk(char* signature, TR_FrontEnd* fe, bool isForCurrentRun)
{
...
TR_J2IThunk* TR_J2IThunkTable::getThunk(char* signature, TR_FrontEnd* fe, bool isForCurrentRun)
{
...
TR_J2IThunk* TR_J2IThunkTable::findThunkFromTerseSignature(char* terseSignature, TR_FrontEnd* fe, bool isForCurrentRun)
{
...
void TR_J2IThunkTable::addThunk(TR_J2IThunk* thunk, TR_FrontEnd* fe, bool isForCurrentRun)
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(fe);
    ...
    if (fej9->isAOT_DEPRECATED_DO_NOT_USE() && !isForCurrentRun) {
        ...
        TR_ASSERT(findThunkFromTerseSignature(thunk->terseSignature(), fe, isForCurrentRun) == thunk,
            "TR_J2IThunkTable: setThunk must cause findThunk(%s) to return %p", thunk->terseSignature(), thunk);
        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseJ2IThunks))
            TR_VerboseLog::writeLineLocked(TR_Vlog_J2I, "add %s @%p", thunk->terseSignature(), thunk);
    }
}

void TR_J2IThunkTable::Node::dumpTo(TR_FrontEnd* fe, TR::FILE* file, TR_PersistentArray<Node>& nodeArray, int indent)
{
...
void TR_J2IThunkTable::dumpTo(TR_FrontEnd* fe, TR::FILE* file)
{
    OMR::CriticalSection criticalSection(_monitor);
