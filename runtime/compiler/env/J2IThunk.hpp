...
public:
    static TR_J2IThunk* allocate(
        int16_t codeSize, char* signature, TR::CodeGenerator* cg, TR_J2IThunkTable* thunkTable);

    static TR_J2IThunk* get(uint8_t* entryPoint) { return ((TR_J2IThunk*)entryPoint) - 1; }
...
public: // API
    ...
    TR_J2IThunk* getThunk(
        char* signature, TR_FrontEnd* frontend, bool isForCurrentRun = false); // same as findThunk but asserts !NULL
