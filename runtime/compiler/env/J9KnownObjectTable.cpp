...
TR::KnownObjectTable::Index J9::KnownObjectTable::getEndIndex() { return _references.size(); }

bool J9::KnownObjectTable::isNull(Index index) { return index == 0; }

TR::KnownObjectTable::Index J9::KnownObjectTable::getIndex(uintptrj_t objectPointer)
{
    ...
    _references[nextIndex] = (uintptrj_t*)thread->javaVM->internalVMFunctions->j9jni_createLocalRef(
        (JNIEnv*)thread, (j9object_t)objectPointer);
    return nextIndex;
}

uintptrj_t* J9::KnownObjectTable::getPointerLocation(Index index)
{
    TR_ASSERT(index != UNKNOWN && 0 <= index && index < _references.size(),
        "getPointerLocation(%d): index must be in range 0..%d", (int)index, _references.size());
...
void J9::KnownObjectTable::dumpObjectTo(TR::FILE* file, Index i, const char* fieldName, const char* sep,
    TR::Compilation* comp, TR_BitVector& visited, TR_VMFieldsInfo** fieldsInfoByIndex, int32_t depth)
{
    ...
    if (comp->getKnownObjectTable()->isNull(i)) {
        ...
        trfprintf(file, "%*s%s%sobj%d @ %p hash %8x %.*s", indent, "", fieldName, sep, i, *ref, hashCode, len - offs,
            className + offs);
        ...
        if (len == 29 && !strncmp("java/lang/invoke/DirectHandle", className, 29)) {
            ...
            trfprintf(file, "  vmSlot: %.*s.%.*s", J9UTF8_LENGTH(className) - offs, utf8Data(className) + offs,
            ...
            for (TR_VMField* field = refIter.getFirst(); field; field = refIter.getNext()) {
                if (field->isReference()) {
                    ...
                    if (targetIndex != UNKNOWN)
                        self()->dumpObjectTo(file, targetIndex, field->name,
...
void J9::KnownObjectTable::dumpTo(TR::FILE* file, TR::Compilation* comp)
{
    TR_J9VMBase* j9fe = (TR_J9VMBase*)self()->fe();
    J9MemoryManagerFunctions* mmf = jitConfig->javaVM->memoryManagerFunctions;
    TR::VMAccessCriticalSection j9KnownObjectTableDumpToCriticalSection(
        j9fe, TR::VMAccessCriticalSection::tryToAcquireVMAccess, comp);
        ...
        if (comp->getOption(TR_TraceKnownObjectGraph)) {
...

            {
                ...
                TR_VMFieldsInfo** fieldsInfoByIndex
                    = (TR_VMFieldsInfo**)alloca(self()->getEndIndex() * sizeof(TR_VMFieldsInfo*));
