...
TR_J9SharedCache::TR_J9SharedCache(TR_J9VMBase* fe)
{
    ...
    _initialHintSCount = std::min(
        TR::Options::getCmdLineOptions()->getInitialSCount(), TR::Options::getAOTCmdLineOptions()->getInitialSCount());
...
void TR_J9SharedCache::log(char* format, ...)
{
...
uint32_t TR_J9SharedCache::getHint(J9VMThread* vmThread, J9Method* method)
{
    uint32_t result = 0;
...
void TR_J9SharedCache::addHint(J9Method* method, TR_SharedCacheHint theHint)
{
    ...
    if (newHint) {
        ...
        uint16_t* hintCount = ((uint16_t*)&scHintData)
            + 1; // Flags and new count field needs to be in contiguous location to be stored into sharecache
                ...
                if (store == 0) {
                    if (_verboseHints)
                        TR_VerboseLog::writeLineLocked(TR_Vlog_SCHINTS, "hint added 0x%x, key = %s, scount: %d",
                            *hintFlags, methodSignature, *hintCount);
...
void TR_J9SharedCache::persistIprofileInfo(
    TR::ResolvedMethodSymbol* methodSymbol, TR_ResolvedMethod* method, TR::Compilation* comp)
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(fe());
...
J9Class* TR_J9SharedCache::matchRAMclassFromROMclass(J9ROMClass* clazz, TR::Compilation* comp)
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(fe());
    J9VMThread* vmThread = fej9->getCurrentVMThread();
...

    {
        ...
        ramClass = jitGetClassInClassloaderFromUTF8(vmThread,
            ((TR_ResolvedJ9Method*)comp->getCurrentMethod())->getClassLoader(), (char*)J9UTF8_DATA(className),
...
void* TR_J9SharedCache::pointerFromOffsetInSharedCache(void* offset)
{
    return (void*)((UDATA)offset + _cacheStartAddress);
}

void* TR_J9SharedCache::offsetInSharedCacheFromPointer(void* ptr) { return (void*)((UDATA)ptr - _cacheStartAddress); }

bool TR_J9SharedCache::isPointerInSharedCache(void* ptr, void*& cacheOffset)
{
...
UDATA* TR_J9SharedCache::rememberClass(J9Class* clazz, bool create)
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(fe());
    ...
    J9VMThread* vmThread = fej9->getCurrentVMThread();
    chainData = (UDATA*)sharedCacheConfig()->storeSharedData(vmThread, (const char*)key, keyLength, &dataDescriptor);
    if (chainData) {
...
uint32_t TR_J9SharedCache::numInterfacesImplemented(J9Class* clazz)
{
    uint32_t count = 0;
...
bool TR_J9SharedCache::writeClassToChain(J9ROMClass* romClass, UDATA*& chainPtr)
{
    ...
    LOG(9, {
        log("\t\tChain %p storing romclass %p (%.*s) offset %d\n", chainPtr, romClass, J9UTF8_LENGTH(className),
...
bool TR_J9SharedCache::writeClassesToChain(J9Class** superclasses, int32_t numSuperclasses, UDATA*& chainPtr)
{
...
bool TR_J9SharedCache::fillInClassChain(
    J9Class* clazz, UDATA* chainData, uint32_t chainLength, uint32_t numSuperclasses, uint32_t numInterfaces)
{
...
bool TR_J9SharedCache::romclassMatchesCachedVersion(J9ROMClass* romClass, UDATA*& chainPtr, UDATA* chainEnd)
{
    J9UTF8* className = J9ROMCLASS_CLASSNAME(romClass);
    LOG(9, {
        log("\t\tExamining romclass %p (%.*s) offset %d, comparing to %d\n", romClass, J9UTF8_LENGTH(className),
            J9UTF8_DATA(className), (UDATA)romClass - _cacheStartAddress, *chainPtr);
    });
...
UDATA* TR_J9SharedCache::findChainForClass(J9Class* clazz, const char* key, uint32_t keyLength)
{
    ...
    J9VMThread* vmThread = fej9->getCurrentVMThread();
    sharedCacheConfig()->findSharedData(
        vmThread, key, keyLength, J9SHR_DATA_TYPE_AOTCLASSCHAIN, FALSE, &dataDescriptor, NULL);
...
bool TR_J9SharedCache::classMatchesCachedVersion(J9Class* clazz, UDATA* chainData)
{
    J9UTF8* className = J9ROMCLASS_CLASSNAME(clazz->romClass);
    LOG(5,
        { log("classMatchesCachedVersion class %p %.*s\n", clazz, J9UTF8_LENGTH(className), J9UTF8_DATA(className)); });
...
TR_OpaqueClassBlock* TR_J9SharedCache::lookupClassFromChainAndLoader(uintptrj_t* chainData, void* classLoader)
{
    ...
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(fe());
    J9VMThread* vmThread = fej9->getCurrentVMThread();
    J9Class* clazz = jitGetClassInClassloaderFromUTF8(
        vmThread, (J9ClassLoader*)classLoader, (char*)J9UTF8_DATA(className), J9UTF8_LENGTH(className));
