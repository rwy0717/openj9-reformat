...
public:
    FindSingleJittedImplementer(
        TR::Compilation* comp, TR_OpaqueClassBlock* topClassId, TR_ResolvedMethod* callerMethod, int32_t slotOrIndex)
...
TR_PersistentClassInfo* TR_PersistentCHTable::findClassInfo(TR_OpaqueClassBlock* classId)
{
    TR_PersistentClassInfo* cl
        = _classes[TR_RuntimeAssumptionTable::hashCode((uintptrj_t)classId) % CLASSHASHTABLE_SIZE].getFirst();
 ...
 */
TR_PersistentClassInfo* TR_PersistentCHTable::findClassInfoAfterLocking(
    TR_OpaqueClassBlock* classId, TR::Compilation* comp, bool returnClassInfoForAOT, bool validate)
{
    ...
    if (classInfo && comp->compileRelocatableCode() && comp->getOption(TR_UseSymbolValidationManager) && validate) {
 ...
 */
TR_PersistentClassInfo* TR_PersistentCHTable::findClassInfoAfterLocking(
    TR_OpaqueClassBlock* classId, TR_FrontEnd* fe, bool returnClassInfoForAOT)
{
    TR::ClassTableCriticalSection findClassInfoAfterLocking(fe);
    return findClassInfo(classId);
}

bool TR_PersistentCHTable::isOverriddenInThisHierarchy(
    TR_ResolvedMethod* method, TR_OpaqueClassBlock* thisClass, int32_t vftSlot, TR::Compilation* comp, bool locked)
{
    if (comp->getOption(TR_DisableCHOpts))
    ...
    bool validate)
{
    ...
    TR_PersistentClassInfo* classInfo
        = comp->getPersistentInfo()->getPersistentCHTable()->findClassInfoAfterLocking(thisClass, comp, true, validate);
    if (!classInfo) {
        return 0;
    }
    ...
    if (implementer && comp->getOption(TR_UseSymbolValidationManager) && validate) {
        ...
        if (validated)
            validated
                = svm->addClassFromMethodRecord(implementer->classOfMethod(), implementer->getPersistentIdentifier());
    ...
    TR_ResolvedMethod* callerMethod, TR::Compilation* comp, bool locked, bool validate)
{
    ...
    if (implementer && comp->getOption(TR_UseSymbolValidationManager) && validate) {
        TR::SymbolValidationManager* svm = comp->getSymbolValidationManager();
        bool validated = svm->addMethodFromSingleInterfaceImplementerRecord(
            implementer->getPersistentIdentifier(), thisClass, cpIndex, callerMethod->getPersistentIdentifier());
    ...
    TR_ResolvedMethod* callerMethod, TR::Compilation* comp, TR_Hotness hotness, bool locked)
{
    ...
    return TR_ClassQueries::collectCompiledImplementorsCapped(
               classInfo, implArray, 3, cpIndex, callerMethod, comp, hotness, locked)
        == 3;
}
    ...
    TR_ResolvedMethod* callerMethod, TR::Compilation* comp, bool locked, bool validate)
{
    ...
    if (implementer && comp->getOption(TR_UseSymbolValidationManager) && validate) {
        TR::SymbolValidationManager* svm = comp->getSymbolValidationManager();
        bool validated = svm->addMethodFromSingleAbstractImplementerRecord(
            implementer->getPersistentIdentifier(), thisClass, vftSlot, callerMethod->getPersistentIdentifier());
...
TR_OpaqueClassBlock* TR_PersistentCHTable::findSingleConcreteSubClass(
    TR_OpaqueClassBlock* opaqueClass, TR::Compilation* comp, bool validate)
{
    ...
    TR_PersistentClassInfo* classInfo = comp->getPersistentInfo()->getPersistentCHTable()->findClassInfoAfterLocking(
        opaqueClass, comp, allowForAOT, validate);
    if (classInfo) {
        TR_ScratchList<TR_PersistentClassInfo> subClasses(comp->trMemory());
        ...
        for (TR_PersistentClassInfo* subClassInfo = subClassesIt.getFirst(); subClassInfo;
            ...
            if (!TR::Compiler->cls.isAbstractClass(comp, subClass)
                && !TR::Compiler->cls.isInterfaceClass(comp, subClass)) {
                if (concreteSubClass)
                    return NULL;
...
void TR_PersistentCHTable::resetVisitedClasses() // highly time consumming
{
    for (int32_t i = 0; i <= CLASSHASHTABLE_SIZE; ++i) {
