...
TR_Processor TR_J9VMBase::getPPCProcessorType() { return portLibCall_getProcessorType(); }

bool TR_J9VMBase::getPPCSupportsVSXRegisters()
{
...
void TR_J9VMBase::initializeX86ProcessorInfo(J9JITConfig* jitConfig)
{
...
const char* TR_J9VMBase::getX86ProcessorVendorId()
{
    TR_ASSERT(x86VendorIDInitialized, "X86 Vendor ID has not yet been initialized");
    return x86VendorID;
}

uint32_t TR_J9VMBase::getX86ProcessorSignature()
{
...
uint32_t TR_J9VMBase::getX86ProcessorFeatureFlags()
{
    TR_ASSERT(_jitConfig, "jitConfig is not defined!");
    TR_ASSERT(_jitConfig->javaVM, "jitConfig->javaVM is not defined!");
...
uint32_t TR_J9VMBase::getX86ProcessorFeatureFlags2()
{
    TR_ASSERT(_jitConfig, "jitConfig is not defined!");
    TR_ASSERT(_jitConfig->javaVM, "jitConfig->javaVM is not defined!");
...
uint32_t TR_J9VMBase::getX86ProcessorFeatureFlags8()
{
    TR_ASSERT(_jitConfig, "jitConfig is not defined!");
    TR_ASSERT(_jitConfig->javaVM, "jitConfig->javaVM is not defined!");
...
bool TR_J9VMBase::testOSForSSESupport()
{
    return true; // VM guarantees SSE/SSE2 are available
}

bool TR_J9VMBase::getX86SupportsSSE4_1()
{
...
bool TR_J9VMBase::getX86SupportsHLE()
{
...
bool TR_J9VMBase::getX86SupportsPOPCNT()
{
...
int32_t TR_J9VM::getCompInfo(char* processorName, int32_t stringLength)
{
    ...
    if (TR::Compiler->target.cpu.isZ()) {
        if (TR::Compiler->target.isZOS()) {
            returnValue = getS390MachineName(
                TR::Compiler->target.cpu.TO_PORTLIB_get390zOSMachineType(), processorName, stringLength);
        } else {
            returnValue = getS390MachineName(
                TR::Compiler->target.cpu.TO_PORTLIB_get390zLinuxMachineType(), processorName, stringLength);
        }
        return returnValue;
    }
#endif
...
void TR_J9VM::initializeProcessorType()
{
    ...
    if (TR::Compiler->target.cpu.isZ()) {
        ...
        TR_ASSERT(TR::Compiler->target.cpu.id() >= TR_First390Processor
                && TR::Compiler->target.cpu.id() <= TR_Last390Processor,
            "Not a valid 390 Processor Type");
        ...
        TR_ASSERT(TR::Compiler->target.cpu.id() >= TR_FirstPPCProcessor
                && TR::Compiler->target.cpu.id() <= TR_LastPPCProcessor,
...
#if defined(TR_TARGET_S390)
void TR_J9VMBase::initializeS390zLinuxProcessorFeatures()
{
...
void TR_J9VMBase::initializeS390zOSProcessorFeatures()
{
...
void TR_J9SharedCacheVM::initializeS390zLinuxProcessorFeatures() { return; }

void TR_J9SharedCacheVM::initializeS390zOSProcessorFeatures() { return; }
#endif
