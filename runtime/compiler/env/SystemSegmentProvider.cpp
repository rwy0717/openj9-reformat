...
{
    TR_ASSERT(defaultSegmentSize <= systemSegmentSize,
...
TR::MemorySegment& J9::SystemSegmentProvider::request(size_t requiredSize)
{
    ...
    if (!_freeSegments.empty() && !(defaultSegmentSize() < roundedSize)) {
    ...
    TR_ASSERT(newSegment.heapAlloc == newSegment.heapBase,
...
void J9::SystemSegmentProvider::release(TR::MemorySegment& segment) throw()
{
    ...
    if (segmentSize == defaultSegmentSize()) {
        ...
        for (size_t i = 0; i < chunks; ++i) {
            try {
                createSegmentFromArea(
                    defaultSegmentSize(), static_cast<uint8_t*>(oldSegmentArea) + (defaultSegmentSize() * i));
            } catch (...) {
                /* not much we can do here except leak */
            }
        }
...
TR::MemorySegment& J9::SystemSegmentProvider::allocateNewSegment(
    size_t size, TR::reference_wrapper<J9MemorySegment> systemSegment)
{
