...
bool acquireVMaccessIfNeeded(J9VMThread* vmThread, TR_YesNoMaybe isCompThread)
{
    ...
    {
        if (0
            == vmThread->javaVM->internalVMFunctions->internalTryAcquireVMAccessWithMask(
                   vmThread, J9_PUBLIC_FLAGS_HALT_THREAD_ANY_NO_JAVA_SUSPEND)) {
        ...
        } else // the GC is having exclusive VM access
        {
            ...
            TR_ASSERT(
                hadClassUnloadMonitor, "Comp thread must hold classUnloadMonitor when compiling without VMaccess");
#endif
            ...
            if (compInfoPT->compilationShouldBeInterrupted()) {
                // bail out
                // fprintf(stderr, "Released classUnloadMonitor and will throw an exception because GC unloaded
                // classes\n"); fflush(stderr);
...
bool TR_J9VMBase::isAnyMethodTracingEnabled(TR_OpaqueMethodBlock* method)
{
...
bool TR_J9VMBase::releaseClassUnloadMonitorAndAcquireVMaccessIfNeeded(
    TR::Compilation* comp, bool* hadClassUnloadMonitor)
{
    ...
    *hadClassUnloadMonitor = false;
    ...
    if (!TR::Options::getCmdLineOptions()->getOption(TR_DisableNoVMAccess)) {
        ...
        *hadClassUnloadMonitor
        ...
        TR_ASSERT(
            *hadClassUnloadMonitor, "releaseClassUnloadMonitorAndAcquireVMaccessIfNeeded without classUnloadMonitor\n");
#else
...
void TR_J9VMBase::acquireClassUnloadMonitorAndReleaseVMAccessIfNeeded(
    TR::Compilation* comp, bool hadVMAccess, bool hadClassUnloadMonitor)
{
...
bool TR_J9VMBase::isSharedCacheHint(J9Method* method, TR_SharedCacheHint hint, uint16_t* dataField)
{
    ...
    if (TR::Options::sharedClassCache() // shared classes and AOT must be enabled
        && !TR::Options::getAOTCmdLineOptions()->getOption(TR_DisableSharedCacheHints)
        && ((hint & TR::Options::getAOTCmdLineOptions()->getEnableSCHintFlags()) != 0) && hint != TR_NoHint) {
        J9ROMMethod* romMethod = J9_ROM_METHOD_FROM_RAM_METHOD(method);
        if (!romMethod)
            return false;
...
void TR_J9VMBase::addSharedCacheHint(J9Method* method, TR_SharedCacheHint hint)
{
    ...
    if (TR::Options::sharedClassCache() // shared classes and AOT must be enabled
        ...
        uint16_t* newScount = (uint16_t*)&scHints
            + 1; // Flags and new scount field needs to be in contiguous location to be stored into sharecache
        bool increaseHintCount = false;
        bool existsHintAlready = (*flagsToBeWritten != 0);

        if (hint & *flagsToBeWritten) { // Exists this hint already
            if (!isFailedValidationHint) {
                return;
            } else { // If exists a failed validation hint already, then find the current scount and increase it
        ...
        } else if (isFailedValidationHint) { // Creating a new hint for failed validation.  Note that if a user
                                             // specifies counts explicitly, then the user specified count is obeyed
            increaseHintCount = true;
            *newScount = 10
                * std::min(TR::Options::getCmdLineOptions()->getInitialSCount(),
                      TR::Options::getAOTCmdLineOptions()->getInitialSCount());
            if (*newScount == 0)
                *newScount = 10;
        }

        if (isFailedValidationHint && (*newScount > TR_DEFAULT_INITIAL_COUNT)) { // Cap the new scount
        ...
        if (!existsHintAlready) {
            ...
            if (!SCfull) {
                *flagsToBeWritten |= hint;
                ...
                if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseSCHints)) {
                    ...
                    if (store == 0) {
                        TR_VerboseLog::writeLineLocked(TR_Vlog_SCHINTS, "hint added 0x%x (%p), key = %s, scount: %d",
                            *flagsToBeWritten, store, myBuffer, *newScount);
            ...
            if (!isFailedValidationHint || (isFailedValidationHint && increaseHintCount)) {
                *flagsToBeWritten |= hint;
                ...
                descriptor.type = J9SHR_ATTACHED_DATA_TYPE_JITHINT;
                descriptor.flags = J9SHR_ATTACHED_DATA_NO_FLAGS;
                UDATA store = scConfig->updateAttachedData(
                    vmThread, romMethod, 0, &descriptor); // update the existing attached data

                if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseSCHints)) {
                    TR::CompilationInfo* compInfo = TR::CompilationInfo::get(jitConfig);

                    if (store == 0) {
                        TR_VerboseLog::writeLineLocked(TR_Vlog_SCHINTS, "hint updated 0x%x (%p), key = %s, scount: %d",
                            *flagsToBeWritten, store, myBuffer, *newScount);
                    } else {
...
J9Class* TR_J9VMBase::matchRAMclassFromROMclass(J9ROMClass* clazz, TR::Compilation* comp)
{
    ...
    J9Class* ramClass = jitGetClassInClassloaderFromUTF8(vmThread(),
        ((TR_ResolvedJ9Method*)comp->getCurrentMethod())->getClassLoader(), (char*)J9UTF8_DATA(utf8),
        J9UTF8_LENGTH(utf8));
    if (!ramClass) {
...
#if defined(J9VM_INTERP_AOT_COMPILE_SUPPORT)
    TR_J9VMBase* aotVMWithoutThreadInfo
        = static_cast<TR_J9VMBase*>(pointer_cast<J9AOTConfig*>(jitConfig)->aotCompilationInfo);
#endif
...
uintptrj_t TR_J9VMBase::getProcessID()
{
...
// static method
char* TR_J9VMBase::getJ9FormattedName(J9JITConfig* jitConfig, J9PortLibrary* portLibrary, char* buf, int32_t bufLength,
...
bool TR_J9VMBase::ensureOSRBufferSize(
    uintptrj_t osrFrameSizeInBytes, uintptrj_t osrScratchBufferSizeInBytes, uintptrj_t osrStackFrameSizeInBytes)
{
...
TR_OpaqueClassBlock* TR_J9VMBase::getObjectClass(uintptrj_t objectPointer)
{
...
uintptrj_t TR_J9VMBase::getStaticReferenceFieldAtAddress(uintptrj_t fieldAddress)
{
...
uintptrj_t TR_J9VMBase::getReferenceFieldAtAddress(uintptrj_t fieldAddress)
{
...
uintptrj_t TR_J9VMBase::getReferenceFieldAt(uintptrj_t objectPointer, uintptrj_t fieldOffset)
{
...
uintptrj_t TR_J9VMBase::getVolatileReferenceFieldAt(uintptrj_t objectPointer, uintptrj_t fieldOffset)
{
    TR_ASSERT(haveAccess(), "Must haveAccess in getVolatileReferenceFieldAt");
    return (uintptrj_t)vmThread()->javaVM->javaVM->memoryManagerFunctions->j9gc_objaccess_mixedObjectReadObject(
        vmThread(), (J9Object*)objectPointer, J9_OBJECT_HEADER_SIZE + fieldOffset, IS_VOLATILE);
}

int32_t TR_J9VMBase::getInt32FieldAt(uintptrj_t objectPointer, uintptrj_t fieldOffset)
{
    TR_ASSERT(haveAccess(), "Must haveAccess in getInt32Field");
    return *(int32_t*)(objectPointer + sizeof(J9Object) + fieldOffset);
}

int64_t TR_J9VMBase::getInt64FieldAt(uintptrj_t objectPointer, uintptrj_t fieldOffset)
{
    TR_ASSERT(haveAccess(), "Must haveAccess in getInt64Field");
    return *(int64_t*)(objectPointer + sizeof(J9Object) + fieldOffset);
}

void TR_J9VMBase::setInt64FieldAt(uintptrj_t objectPointer, uintptrj_t fieldOffset, int64_t newValue)
{
    TR_ASSERT(haveAccess(), "Must haveAccess in setInt64Field");
    *(int64_t*)(objectPointer + sizeof(J9Object) + fieldOffset) = newValue;
}

bool TR_J9VMBase::compareAndSwapInt64FieldAt(
    uintptrj_t objectPointer, uintptrj_t fieldOffset, int64_t oldValue, int64_t newValue)
{
...
intptrj_t TR_J9VMBase::getArrayLengthInElements(uintptrj_t objectPointer)
{
...
int32_t TR_J9VMBase::getInt32Element(uintptrj_t objectPointer, int32_t elementIndex)
{
    TR_ASSERT(haveAccess(), "getInt32Element requires VM access");
    return J9JAVAARRAYOFINT_LOAD(vmThread(), objectPointer, elementIndex);
}

uintptrj_t TR_J9VMBase::getReferenceElement(uintptrj_t objectPointer, intptrj_t elementIndex)
{
...
TR_OpaqueClassBlock* TR_J9VMBase::getClassFromJavaLangClass(uintptrj_t objectPointer)
{
    return (TR_OpaqueClassBlock*)J9VM_J9CLASS_FROM_HEAPCLASS(_vmThread, objectPointer);
}

uintptrj_t TR_J9VMBase::getConstantPoolFromMethod(TR_OpaqueMethodBlock* method)
{
...
void TR_J9VMBase::printVerboseLogHeader(TR::Options* cmdLineOptions)
{
    ...
    TR_VerboseLog::writeLine(
        TR_Vlog_INFO, "     Supports HardwareSQRT:%d", TR::Compiler->target.cpu.getSupportsHardwareSQRT());
    TR_VerboseLog::writeLine(
        TR_Vlog_INFO, "     Supports HardwareRound:%d", TR::Compiler->target.cpu.getSupportsHardwareRound());
    TR_VerboseLog::writeLine(
        TR_Vlog_INFO, "     Supports HardwareCopySign:%d", TR::Compiler->target.cpu.getSupportsHardwareCopySign());
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "     Supports FPU:%d", TR::Compiler->target.cpu.hasFPU());
    TR_VerboseLog::writeLine(
        TR_Vlog_INFO, "     Supports DFP:%d", TR::Compiler->target.cpu.supportsDecimalFloatingPoint());
    ...
    TR_VerboseLog::writeLine(
        TR_Vlog_INFO, "          VF: %d", TR::Compiler->target.cpu.getS390SupportsVectorFacility());
    TR_VerboseLog::writeLine(
        TR_Vlog_INFO, "          GS: %d", TR::Compiler->target.cpu.getS390SupportsGuardedStorageFacility());
    TR_VerboseLog::writeLine(TR_Vlog_INFO, "");
#endif
#endif
}
...
void TR_J9VMBase::printPID()
{
    ...
    TR_VerboseLog::write(
        "PID=%d, %4d/%02d/%02d", j9sysinfo_get_pid(), (p->tm_year) + 1900, (p->tm_mon) + 1, p->tm_mday);
#endif
}
...
bool TR_J9VMBase::generateCompressedPointers() { return TR::Options::useCompressedPointers(); }
...
int32_t* TR_J9VMBase::getCurrentLocalsMapForDLT(TR::Compilation* comp)
{
    ...
    jitConfig->javaVM->localMapFunction(_portLibrary, J9_CLASS_FROM_METHOD(j9method)->romClass,
        J9_ROM_METHOD_FROM_RAM_METHOD(j9method), comp->getDltBcIndex(), (U_32*)currentBundles, NULL, NULL, NULL);
#endif
...
{
    return J9_JAVA_CLASS_FINALIZE | J9_JAVA_CLASS_OWNABLE_SYNCHRONIZER;
}
...
UDATA
TR_J9VMBase::getMonitorOwnerOffset() { return offsetof(J9ThreadAbstractMonitor, owner); }

UDATA
TR_J9VMBase::getMonitorEntryCountOffset() { return offsetof(J9ThreadAbstractMonitor, count); }
...
int32_t TR_J9VMBase::getFirstArrayletPointerOffset(TR::Compilation* comp)
{
    ...
    int32_t headerSize = TR::Compiler->om.useHybridArraylets() ? sizeof(J9IndexableObjectDiscontiguous)
                                                               : sizeof(J9IndexableObjectContiguous);

    return (headerSize + TR::Compiler->om.sizeofReferenceField() - 1)
        & (-1) * (intptrj_t)(TR::Compiler->om.sizeofReferenceField());
}
...
uintptrj_t TR_J9VMBase::getOffsetOfIndexableSizeField() { return offsetof(J9ROMArrayClass, arrayShape); }
        ...
        *shouldRetryAllocation = false;
        ...
        if (dataCache) {
            ...
            if (!ptr) // designated data cache is not big enough
            {
                ...
                dataCache = TR_DataCacheManager::getManager()->reserveAvailableDataCache(
                    _vmThread, comp->getTotalNeededDataCacheSpace());
...
uint8_t* TR_J9VMBase::allocateRelocationData(TR::Compilation* comp, uint32_t numBytes)
{
    ...
    relocationData = allocateDataCacheRecord(
        numBytes, comp, isAOT_DEPRECATED_DO_NOT_USE(), &shouldRetryAllocation, J9_JIT_DCE_RELOCATION_DATA, &size);
    if (!relocationData) {
...
uint8_t* TR_J9VMBase::allocateCodeMemory(
    TR::Compilation* comp, uint32_t warmCodeSize, uint32_t coldCodeSize, uint8_t** coldCode, bool isMethodHeaderNeeded)
{
    ...
    uint8_t* warmCode = TR::CodeCacheManager::instance()->allocateCodeMemory(
        warmCodeSize, coldCodeSize, &codeCache, coldCode, isAOT_DEPRECATED_DO_NOT_USE(), isMethodHeaderNeeded);
    ...
    if (codeCache != comp->getCurrentCodeCache()) {
        ...
        TR_ASSERT(!codeCache || codeCache->isReserved(),
            "Substitute code cache isn't marked as reserved"); // Either we didn't get a code cache, or the one we
                                                               // should get is
...
void TR_J9VMBase::resizeCodeMemory(TR::Compilation* comp, U_8* bufferStart, uint32_t numBytes)
{
...
U_8* TR_J9VMBase::getCodeCacheBase() { return _jitConfig->codeCache->heapBase; }

U_8* TR_J9VMBase::getCodeCacheBase(TR::CodeCache* codeCache)
{
    // Used by S390 canUseRelativeLongInstructions to get code cache base.
    return codeCache->getCodeBase();
}

U_8* TR_J9VMBase::getCodeCacheTop() { return _jitConfig->codeCache->heapTop; }

U_8* TR_J9VMBase::getCodeCacheTop(TR::CodeCache* codeCache)
{
...
void TR_J9VMBase::markHotField(
    TR::Compilation* comp, TR::SymbolReference* symRef, TR_OpaqueClassBlock* clazz, bool isFixedClass)
{
    TR_MarkHotField marker(comp, symRef);
    marker.mark(TR::Compiler->cls.convertClassOffsetToClassPtr(clazz), isFixedClass);
}

bool TR_J9VMBase::scanReferenceSlotsInClassForOffset(
    TR::Compilation* comp, TR_OpaqueClassBlock* classPointer, int32_t offset)
{
    if (isAOT_DEPRECATED_DO_NOT_USE())
        return false;
    ...
    for (TR_VMField* field = iter.getFirst(); field != NULL; field = iter.getNext()) {
        ...
        if (field->isReference()) {
            ...
            int32_t fieldOffset = getInstanceFieldOffset(
                classPointer, fieldName, (uint32_t)strlen(fieldName), fieldSignature, (uint32_t)strlen(fieldSignature));
...
void TR_J9VMBase::markClassForTenuredAlignment(
    TR::Compilation* comp, TR_OpaqueClassBlock* opclazz, uint32_t alignFromStart)
{
    if (!isAOT_DEPRECATED_DO_NOT_USE()) {
        J9Class* clazz = TR::Compiler->cls.convertClassOffsetToClassPtr(opclazz);
        UDATA hotFieldsWordValue = 0x1; // mark for alignment

        TR_ASSERT((alignFromStart % getObjectAlignmentInBytes()) == 0,
            "alignment undershot should be multiple of %d bytes", getObjectAlignmentInBytes());
...
int32_t TR_J9VMBase::printTruncatedSignature(char* sigBuf, int32_t bufLen, TR_OpaqueMethodBlock* method)
{
    ...
    if (sigLen < bufLen) {
        sigLen = sprintf(sigBuf, "%.*s.%.*s%.*s", J9UTF8_LENGTH(className), utf8Data(className), J9UTF8_LENGTH(name),
    ...
    } else // truncate some parts of the signature
    {
        ...
        {
            sigLen
                = sprintf(sigBuf, "%.*s.%.*s%.*s", J9UTF8_LENGTH(className), utf8Data(className), J9UTF8_LENGTH(name),
                    utf8Data(name), (J9UTF8_LENGTH(signature) - (sigLen - bufLen)), utf8Data(signature));
        } else {
            ...
            if (nameLen == bufLen - 3) // not even the method name can be printed entireley
                ...
                sigLen = sprintf(sigBuf, "%.*s.%.*s", std::min<int32_t>(bufLen - 2 - nameLen, J9UTF8_LENGTH(className)),
                    utf8Data(className), nameLen, utf8Data(name));
        }
    }
    return sigLen;
}
...
{
    TR_ASSERT(
        catchBlock->isCatchBlock(), "shouldPerformEDO expected block_%d to be a catch block", catchBlock->getNumber());
    ...
    if (recomp && comp->getOptions()->allowRecompilation() && recomp->useSampling() && recomp->shouldBeCompiledAgain()
        && comp->getMethodHotness() < hot && comp->getNodeCount() < TR::Options::_catchSamplingSizeThreshold) {
        return true;
    } else {
        return false;
    }
...
bool TR_J9VMBase::maybeHighlyPolymorphic(TR::Compilation* comp, TR_ResolvedMethod* caller, int32_t cpIndex,
    TR_Method* callee, TR_OpaqueClassBlock* receiverClass)
{
...
bool TR_J9VMBase::supressInliningRecognizedInitialCallee(TR_CallSite* callsite, TR::Compilation* comp)
{
    ...
    if (callNode->getSymbol()->getResolvedMethodSymbol()) {
        ...
        case TR::com_ibm_dataaccess_ByteArrayUtils_trailingZerosQuadWordAtATime_:
            if (!comp->getOption(TR_DisablePackedDecimalIntrinsics) && !comp->getOption(TR_DisableDAATrailingZero)
        ...
        case TR::java_lang_String_hashCodeImplDecompressed:
            /*
             * X86 and z want to avoid inlining both java_lang_String_hashCodeImplDecompressed and
             * java_lang_String_hashCodeImplCompressed so they can be recognized and replaced with a custom fast
             * implementation. Power currently only has the custom fast implementation for
             * java_lang_String_hashCodeImplDecompressed. As a result, Power only wants to prevent inlining of
             * java_lang_String_hashCodeImplDecompressed. When Power gets a fast implementation of
             * TR::java_lang_String_hashCodeImplCompressed, this case can be merged into the case for
             * java_lang_String_hashCodeImplCompressed instead of using a fallthrough.
             */
            if (!TR::Compiler->om.canGenerateArraylets() && TR::Compiler->target.cpu.isPower()
                && TR::Compiler->target.cpu.id() >= TR_PPCp8 && getPPCSupportsVSXRegisters()
...
int TR_J9VMBase::checkInlineableTarget(
    TR_CallTarget* target, TR_CallSite* callsite, TR::Compilation* comp, bool inJSR292InliningPasses)
{
    TR_ResolvedMethod* resolvedMethod
        = target->_calleeSymbol ? target->_calleeSymbol->getResolvedMethod() : target->_calleeMethod;

    if (!comp->hasIntStreamForEach()) {
        // This first if statement controls inlining of a large class of JSR292 methods, which we want to consider ONLY
        // in certain cases in early rounds of inlining...
        if (resolvedMethod->convertToMethod()->isArchetypeSpecimen()
            ...
            if (resolvedMethod->getRecognizedMethod() == TR::java_lang_invoke_MethodHandle_invokeExactTargetAddress
                ...
                || resolvedMethod->getRecognizedMethod() == TR::java_lang_invoke_InterfaceHandle_invokeExact
                || resolvedMethod->getRecognizedMethod() == TR::java_lang_invoke_VirtualHandle_invokeExact) {
                // Always choose to inline these methods; the first two are small getters, and the last three are
                // single-level "leaf" handles
            } else if (!inJSR292InliningPasses) {
                // Last round of inlining, no more JSR292 methods (other than the above two things, to which we always
                // say yes)
                ...
                // We're in JSR292 Inlining rounds, and we are ourselves an archetype specimen, so we can inline other
                // archetype specimina whenever we see fit
                ...
                // We are in first rounds of inlining, we are warm or below, and we are not an archetype specimen
                // ourselves... No inlining of JSR292 methods.
                return DontInline_Callee;
            }
        } else if (inJSR292InliningPasses) {
            // If we aren't in the last round of inlining, don't inline anything that didn't fall into the above
            // category
            return DontInline_Callee;
            ...
            // We are trying to inline a non-JSR292 method, and we are in the last round of inlining, but we are a
            // shareable thunk, so say NO Except for do and undo customization logic which we still want to inline...
            //
    ...
    if (!TR_J9InlinerPolicy::isInlineableJNI(resolvedMethod, callsite->_callNode)
        || (TR_J9InlinerPolicy::isInlineableJNI(resolvedMethod, callsite->_callNode) && callsite->isIndirectCall())) {
...
void TR_J9VMBase::refineColdness(TR::Node* node, bool& isCold)
{
    ...
    if (symRef->getSymbol()->isResolvedMethod()
        ...
        inlineableJNI = TR_J9InlinerPolicy::isInlineableJNI(
            symRef->getSymbol()->castToResolvedMethodSymbol()->getResolvedMethod(), node);
...
bool TR_J9VMBase::isMethodEnterTracingEnabled(TR_OpaqueMethodBlock* method)
{
    J9JavaVM* javaVM = _jitConfig->javaVM;
...
bool TR_J9VMBase::isMethodExitTracingEnabled(TR_OpaqueMethodBlock* method)
{
    J9JavaVM* javaVM = _jitConfig->javaVM;
    J9HookInterface** vmHooks = javaVM->internalVMFunctions->getVMHookInterface(javaVM);
...
bool TR_J9VMBase::methodsCanBeInlinedEvenIfEventHooksEnabled() { return false; }
...
TR::TreeTop* TR_J9VMBase::lowerMethodHook(TR::Compilation* comp, TR::Node* root, TR::TreeTop* treeTop)
{
    ...
    bool isTrace = (event == J9HOOK_VM_METHOD_ENTER) ? isMethodEnterTracingEnabled(j9method)
                                                     : isMethodExitTracingEnabled(j9method);
    ...
    if (isAOT_DEPRECATED_DO_NOT_USE()) {
        ...
        TR::Block* remainderBlock
            = enclosingBlock->createConditionalBlocksBeforeTree(treeTop, cmpTree, ifTree, 0, comp->getFlowGraph());
            ...
            TR::Node::createif(TR::ificmpne,
                TR::Node::create(TR::iand, 2,
                    TR::Node::create(TR::bu2i, 1,
                        TR::Node::createWithSymRef(root, TR::buload, 0,
...
TR::TreeTop* TR_J9VMBase::lowerMultiANewArray(TR::Compilation* comp, TR::Node* root, TR::TreeTop* treeTop)
{
    ...
    for (int32_t i = dims; i > 0; i--) {
        ...
        TR::TreeTop::create(
            comp, treeTop->getPrevTreeTop(), TR::Node::createWithSymRef(TR::istore, 1, 1, root->getChild(i), symRef));
    ...
    TR::Node* tempRef = TR::Node::createWithSymRef(
        root, TR::loadaddr, 0, new (comp->trHeapMemory()) TR::SymbolReference(comp->getSymRefTab(), temp));
...
bool TR_J9VMBase::compilationShouldBeInterrupted(TR::Compilation* comp, TR_CallingContext callingContext)
{
    ...
    TR::CompilationInfoPerThreadBase* const compInfoPTB
        = compilingOnApplicationThread ? _compInfo->getCompInfoForCompOnAppThread() : _compInfoPT;
    ...
    if (!comp->getOption(TR_DisableNoVMAccess)) {
        ...
        if (exitClassUnloadMonitor) {
            ...
            //--- GC CAN INTERVENE HERE ---
            TR_ASSERT(
                (vmThread()->publicFlags & J9_PUBLIC_FLAGS_VM_ACCESS) == 0, "comp thread must not have vm access");
            if (!compilingOnApplicationThread && TR::Options::getCmdLineOptions()->realTimeGC()) {
                // no compilation on application thread
                TR_ASSERT(_compInfoPT, "Missing compilation info per thread.");
                _compInfoPT->waitForGCCycleMonitor(false);
...
bool TR_J9VMBase::tryToAcquireAccess(TR::Compilation* comp, bool* haveAcquiredVMAccess)
{
    ...
    *haveAcquiredVMAccess = false;
    if (!comp->getOption(TR_DisableNoVMAccess)) {
        if (!(vmThread()->publicFlags & J9_PUBLIC_FLAGS_VM_ACCESS)) {
            if (!vmThread()->javaVM->internalVMFunctions->internalTryAcquireVMAccessWithMask(
                    vmThread(), J9_PUBLIC_FLAGS_HALT_THREAD_ANY_NO_JAVA_SUSPEND)) {
                hasVMAccess = true;
                *haveAcquiredVMAccess = true;
            } else {
    ...
    while (clazz) {
        if (!J9ROMCLASS_IS_PRIMITIVE_OR_ARRAY(clazz->romClass)) {
            ...
            for (uint32_t m = 0; m < clazz->romClass->romMethodCount; m++) {
                ...
                if (!(romMethod->modifiers & (J9_JAVA_NATIVE | J9_JAVA_ABSTRACT)) && method != newInstanceThunk
                    ...
                    sprintf(fullMethodName, "%.*s.%.*s%.*s", J9UTF8_LENGTH(className), J9UTF8_DATA(className),
...
bool TR_J9VMBase::tossingCode()
{
    if ((_jitConfig->runtimeFlags & J9JIT_TOSS_CODE) || (_jitConfig->runtimeFlags & J9JIT_TESTMODE))
        return true;
    return false;
}
...
bool TR_J9VMBase::methodMayHaveBeenInterpreted(TR::Compilation* comp)
{
    if ((!TR::Options::getCmdLineOptions()->getOption(TR_DisableDFP)
        ...
        if (comp->getJittedMethodSymbol()->getResolvedMethod()->convertToMethod()->isBigDecimalMethod(comp)
            || comp->getJittedMethodSymbol()->getResolvedMethod()->convertToMethod()->isBigDecimalConvertersMethod(
                   comp))
            return false;
    } else if (comp->ilGenRequest().details().isMethodHandleThunk())
        return false;
...
{
    return (comp->ilGenRequest().details().isDumpMethod()
...
bool TR_J9VMBase::canDereferenceAtCompileTime(TR::SymbolReference* fieldRef, TR::Compilation* comp)
{
    ...
    if (fieldRef->getSymbol()->isShadow()) {
        if (fieldRef->getReferenceNumber() < comp->getSymRefTab()->getNumPredefinedSymbols()) {
            ...
            switch (fieldRef->getSymbol()->getRecognizedField()) {
            case TR::Symbol::Java_lang_invoke_MethodHandle_rawModifiers: // JTC 83328: Delete once VM changes promote.
                                                                         // Moved to PrimitiveHandle
            case TR::Symbol::Java_lang_invoke_MethodHandle_defc: // JTC 83328: Delete once VM changes promote.  Moved to
                                                                 // PrimitiveHandle
            ...
            default: {
                ...
                J9Class* fieldClass = (J9Class*)fieldRef->getOwningMethod(comp)->getClassFromFieldOrStatic(
                    comp, fieldRef->getCPIndex());
                if (!fieldClass)
                    return false;
...
//
TR::Node* TR_J9VMBase::initializeLocalObjectFlags(TR::Compilation* comp, TR::Node* allocationNode, J9Class* ramClass)
{
...
// on the stack.
//
void TR_J9VMBase::initializeLocalObjectHeader(
    TR::Compilation* comp, TR::Node* allocationNode, TR::TreeTop* allocationTreeTop)
{
...
//
void TR_J9VMBase::initializeLocalArrayHeader(
    TR::Compilation* comp, TR::Node* allocationNode, TR::TreeTop* allocationTreeTop)
{
    /*
    ...
    */
    ...
    TR::Node* classNode = TR::Node::createWithSymRef(allocationNode, TR::loadaddr, 0,
        comp->getSymRefTab()->findOrCreateClassSymbol(
            comp->getMethodSymbol(), -1, convertClassPtrToClassOffset(ramClass)));
...
TR::TreeTop* TR_J9VMBase::initializeClazzFlagsMonitorFields(
    TR::Compilation* comp, TR::TreeTop* prevTree, TR::Node* allocationNode, TR::Node* classNode, J9Class* ramClass)
{
    // -----------------------------------------------------------------------------------
    ...
    node = TR::Node::createWithSymRef(
        TR::astorei, 2, 2, allocationNode, classNode, comp->getSymRefTab()->findOrCreateVftSymbolRef());
    ...
    node = TR::Node::createWithSymRef(
        TR::astorei, 2, 2, allocationNode, node, comp->getSymRefTab()->findOrCreateVftSymbolRef());
#else
    node = TR::Node::createWithSymRef(
        TR::istorei, 2, 2, allocationNode, node, comp->getSymRefTab()->findOrCreateHeaderFlagsSymbolRef());
#endif
...
uintptrj_t TR_J9VMBase::getOverflowSafeAllocSize()
{
...
void* TR_J9VMBase::setJ2IThunk(TR_Method* method, void* thunkptr, TR::Compilation* comp)
{
    return setJ2IThunk(method->signatureChars(), method->signatureLength(), thunkptr, comp);
}

void* TR_J9VMBase::getJ2IThunk(TR_Method* method, TR::Compilation* comp)
{
...
void* TR_J9VMBase::findPersistentJ2IThunk(char* signatureChars)
{
    return findPersistentThunk(signatureChars, strlen(signatureChars));
}

void* TR_J9VMBase::findPersistentThunk(char* signatureChars, uint32_t signatureLength)
{
#if defined(J9VM_INTERP_AOT_COMPILE_SUPPORT) && defined(J9VM_OPT_SHARED_CLASSES) \
    && (defined(TR_HOST_X86) || defined(TR_HOST_POWER) || defined(TR_HOST_S390) || defined(TR_HOST_ARM))
    ...
    _jitConfig->javaVM->sharedClassConfig->findSharedData(
        curThread, signatureChars, signatureLength, J9SHR_DATA_TYPE_AOTTHUNK, false, &firstDescriptor, NULL);
...
void* TR_J9VMBase::persistJ2IThunk(void* thunk)
{
    return NULL; // only needed for AOT compilations
}

static char* getJ2IThunkSignature(
    char* invokeHandleSignature, uint32_t signatureLength, int argsToSkip, char* description, TR::Compilation* comp)
{
    char* argsToCopy;
    ...
    if (comp->getOption(TR_TraceCG))
        traceMsg(comp, "JSR292: j2i-thunk signature for %s of '%.*s' is '%s'\n", description, signatureLength,
            invokeHandleSignature, resultBuf);
    return resultBuf;
}

static TR::Node* getEquivalentVirtualCallNode(
    TR::Node* callNode, int argsToSkip, char* description, TR::Compilation* comp)
{
...
char* TR_J9VMBase::getJ2IThunkSignatureForDispatchVirtual(
    char* invokeHandleSignature, uint32_t signatureLength, TR::Compilation* comp)
{
    ...
    if (methodSymbol->isComputed()
        ...
        if (isAOT_DEPRECATED_DO_NOT_USE()) // While we're here... we need an AOT relocation for this call
            comp->cg()->addExternalRelocation(
                new (comp->trHeapMemory()) TR::ExternalRelocation(NULL, (uint8_t*)callNode,
                    (uint8_t*)methodSymbol->getMethod()->signatureChars(), TR_J2IThunks, comp->cg()),
...
uintptrj_t TR_J9VMBase::methodHandle_thunkableSignature(uintptrj_t methodHandle)
{
...
uintptrj_t TR_J9VMBase::methodHandle_type(uintptrj_t methodHandle)
{
...
uintptrj_t TR_J9VMBase::methodType_descriptor(uintptrj_t methodType)
{
...
static uint8_t* bypassBaseAddress(uintptrj_t mutableCallSite, TR_J9VMBase* fej9)
{
    ...
    */
    uintptrj_t* fieldAddress = (uintptrj_t*)fej9->getStaticFieldAddress(fej9->getObjectClass(mutableCallSite),
...
uintptrj_t* TR_J9VMBase::mutableCallSite_bypassLocation(uintptrj_t mutableCallSite)
{
...
uintptrj_t* TR_J9VMBase::mutableCallSite_findOrCreateBypassLocation(uintptrj_t mutableCallSite)
{
    TR_ASSERT(haveAccess(), "mutableCallSite_bypassLocation requires VM access");
    ...
    if (bypassOffset == 0) {
        ...
        jobject handleRef = vmThread()->javaVM->internalVMFunctions->j9jni_createGlobalRef(
            (JNIEnv*)vmThread(), (j9object_t)target, false);
        uint8_t* baseAddress = bypassBaseAddress(mutableCallSite, this);
...
static TR_OpaqueMethodBlock* findClosestArchetype(TR_OpaqueClassBlock* clazz, char* name, char* signature,
...
TR_OpaqueMethodBlock* TR_J9VMBase::lookupArchetype(TR_OpaqueClassBlock* clazz, char* name, char* signature)
{
...
TR_OpaqueMethodBlock* TR_J9VMBase::lookupMethodHandleThunkArchetype(uintptrj_t methodHandle)
{
    ...
    TR_OpaqueMethodBlock* result
        = lookupArchetype(getObjectClass((uintptrj_t)methodHandle), methodName, archetypeSpecimenSignature);
    if (!result) {
        strcpy(returnType, "I");
        result = lookupArchetype(
            getObjectClass((uintptrj_t)methodHandle), "invokeExact_thunkArchetype_X", archetypeSpecimenSignature);
    }
    return result;
}

TR_ResolvedMethod* TR_J9VMBase::createMethodHandleArchetypeSpecimen(TR_Memory* trMemory,
    TR_OpaqueMethodBlock* archetype, uintptrj_t* methodHandleLocation, TR_ResolvedMethod* owningMethod)
{
    intptrj_t length;
...
TR_ResolvedMethod* TR_J9VMBase::createMethodHandleArchetypeSpecimen(
    TR_Memory* trMemory, uintptrj_t* methodHandleLocation, TR_ResolvedMethod* owningMethod)
{
...
bool TR_J9VMBase::hasMethodTypesSideTable()
{
...
void* TR_J9VMBase::methodHandle_jitInvokeExactThunk(uintptrj_t methodHandle)
{
    TR_ASSERT(haveAccess(), "methodHandle_jitInvokeExactThunk requires VM access");
    return (void*)(intptrj_t)getInt64Field(
        getReferenceField(methodHandle, "thunks", "Ljava/lang/invoke/ThunkTuple;"), "invokeExactThunk");
}
 ...
 */
bool TR_J9VMBase::stringEquals(
    TR::Compilation* comp, uintptrj_t* stringLocation1, uintptrj_t* stringLocation2, int32_t& result)
{
 ...
 */
bool TR_J9VMBase::getStringHashCode(TR::Compilation* comp, uintptrj_t* stringLocation, int32_t& result)
{
    TR::VMAccessCriticalSection getStringHashCodeCriticalSection(
        this, TR::VMAccessCriticalSection::tryToAcquireVMAccess, comp);
...
bool TR_J9VMBase::getStringFieldByName(
    TR::Compilation* comp, TR::SymbolReference* stringRef, TR::SymbolReference* fieldRef, void*& pResult)
{
    TR::VMAccessCriticalSection getStringFieldCriticalSection(
        this, TR::VMAccessCriticalSection::tryToAcquireVMAccess, comp);
...
uintptrj_t TR_J9VMBase::getFieldOffset(
    TR::Compilation* comp, TR::SymbolReference* classRef, TR::SymbolReference* fieldRef)
{
    ...
    jitConfig->javaVM->internalVMFunctions->copyStringToUTF8Helper(vmThread(), classString,
    ...
    **/
    ...
    jitConfig->javaVM->internalVMFunctions->copyStringToUTF8Helper(vmThread(), fieldString,
        J9_STR_NULL_TERMINATE_RESULT, 0, J9VMJAVALANGSTRING_LENGTH(vmThread(), fieldString), u8FieldString, len + 1);
    ...
    for (field = itr.getFirst(); field != NULL; field = itr.getNext()) {
        ...
        if (!strncmp(field->name, (const char*)u8FieldString, len + 1)) {
            ...
            // Do we Need this?
            // offset = getInstanceFieldOffset(j9ClassPtr, field->name, strlen(field->name), field->signature,
            // strlen(field->signature),
            //                               J9_LOOK_NO_JAVA);
    ...
    for (field = itr.getFirst(); field != NULL; field = itr.getNext()) {
        if (!strncmp(field->name, (const char*)u8FieldString, len + 1)) {
            ...
            staticAddr = jitConfig->javaVM->internalVMFunctions->staticFieldAddress(_vmThread, (J9Class*)j9ClassPtr,
                u8FieldString, len, (U_8*)field->signature, (UDATA)strlen(field->signature), NULL, NULL,
...
bool TR_J9VMBase::isString(uintptrj_t objectPointer)
{
    TR_ASSERT(haveAccess(), "isString requires VM access");
    return isString(getObjectClass(objectPointer));
}

int32_t TR_J9VMBase::getStringLength(uintptrj_t objectPointer)
{
...
uint16_t TR_J9VMBase::getStringCharacter(uintptrj_t objectPointer, int32_t index)
{
...
intptrj_t TR_J9VMBase::getStringUTF8Length(uintptrj_t objectPointer)
{
...
char* TR_J9VMBase::getStringUTF8(uintptrj_t objectPointer, char* buffer, intptrj_t bufferSize)
{
...
void TR_J9VMBase::traceAssumeFailure(int32_t line, const char* file)
{
    ...
    if (!_compInfo->getRampDownMCT()) {
        ...
        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseCompilationThreads)) {
            TR_VerboseLog::writeLineLocked(
                TR_Vlog_INFO, "t=%u setRampDownMCT", (uint32_t)_compInfo->getPersistentInfo()->getElapsedTime());
        }
    }
}
...
void TR_J9VMBase::reserveTrampolineIfNecessary(
        ...
        if (!isAOT_DEPRECATED_DO_NOT_USE()) {
            ...
            if (retValue != OMR::CodeCacheErrorCode::ERRORCODE_SUCCESS) {
                    ...
                    && !inBinaryEncoding) // code cache full, allocate a new one
                {
                    ...
                    newCache = TR::CodeCacheManager::instance()->getNewCodeCache(
                        comp->getCompThreadID()); // class unloading may happen here
                    if (newCache) {
                        // check for class unloading that can happen in getNewCodeCache
                        TR::CompilationInfoPerThreadBase* const compInfoPTB = _compInfo->getCompInfoForCompOnAppThread()
...
intptrj_t TR_J9VMBase::indexedTrampolineLookup(int32_t helperIndex, void* callSite)
{
...
bool TR_J9VMBase::getSupportsRecognizedMethods()
{
    TR_ASSERT(TR::Compiler->target.cpu.isZ() || TR::Compiler->target.cpu.isX86() || TR::Compiler->target.cpu.isPower()
            || TR::Compiler->target.cpu.isARM() || !isAOT_DEPRECATED_DO_NOT_USE(),
        "getSupportsRecognizedMethods must be called only on X,P,Z or only for non-AOT");
    return true;
}

int32_t TR_J9VMBase::getMaxCallGraphCallCount()
{
...
int32_t TR_J9VMBase::getIProfilerCallCount(TR_OpaqueMethodBlock* caller, int32_t bcIndex, TR::Compilation* comp)
{
    TR_IProfiler* profiler = getIProfiler();
    if (profiler)
...
int32_t TR_J9VMBase::getIProfilerCallCount(
    TR_OpaqueMethodBlock* callee, TR_OpaqueMethodBlock* caller, int32_t bcIndex, TR::Compilation* comp)
{
    TR_IProfiler* profiler = getIProfiler();
    if (profiler)
...
int32_t TR_J9VMBase::getIProfilerCallCount(TR_ByteCodeInfo& bcInfo, TR::Compilation* comp)
{
    TR_IProfiler* profiler = getIProfiler();
    if (profiler)
...
void TR_J9VMBase::setIProfilerCallCount(
    TR_OpaqueMethodBlock* caller, int32_t bcIndex, int32_t count, TR::Compilation* comp)
{
    TR_IProfiler* profiler = getIProfiler();
    if (profiler)
...
void TR_J9VMBase::setIProfilerCallCount(TR_ByteCodeInfo& bcInfo, int32_t count, TR::Compilation* comp)
{
    TR_IProfiler* profiler = getIProfiler();
    if (profiler)
...
int32_t TR_J9VMBase::getCGEdgeWeight(TR::Node* callerNode, TR_OpaqueMethodBlock* callee, TR::Compilation* comp)
{
    TR_IProfiler* profiler = getIProfiler();
    if (profiler)
...
TR_ExternalValueProfileInfo* TR_J9VMBase::getValueProfileInfoFromIProfiler(
...
uint32_t* TR_J9VMBase::getAllocationProfilingDataPointer(
    TR_ByteCodeInfo& bcInfo, TR_OpaqueClassBlock* clazz, TR_OpaqueMethodBlock* method, TR::Compilation* comp)
{
    TR_IProfiler* iProfiler = getIProfiler();
    if (iProfiler == NULL)
        return NULL;
...
bool TR_J9VMBase::startAsyncCompile(
    TR_OpaqueMethodBlock* method, void* oldStartPC, bool* queued, TR_OptimizationPlan* optimizationPlan)
{
...
U_32 TR_J9VMBase::virtualCallOffsetToVTableSlot(U_32 offset) { return J9JIT_INTERP_VTABLE_OFFSET - offset; }

void* TR_J9VMBase::addressOfFirstClassStatic(TR_OpaqueClassBlock* j9Class)
{
...
void* TR_J9VMBase::getStaticFieldAddress(
    TR_OpaqueClassBlock* clazz, unsigned char* fieldName, uint32_t fieldLen, unsigned char* sig, uint32_t sigLen)
{
...
int32_t TR_J9VMBase::getInterpreterVTableSlot(TR_OpaqueMethodBlock* mBlock, TR_OpaqueClassBlock* clazz)
{
    ...
    int32_t result = vmThread()->javaVM->internalVMFunctions->getVTableOffsetForMethod(
        (J9Method*)mBlock, (J9Class*)clazz, vmThread());
    return result;
}
        ...
        if (stringClass) {
            TR_PersistentClassInfo* classInfo = (comp->getPersistentInfo()->getPersistentCHTable() == NULL)
                ...
                : comp->getPersistentInfo()->getPersistentCHTable()->findClassInfoAfterLocking(
                      stringClass, comp, isVettedForAOT);
            if (classInfo && classInfo->isInitialized()) {
                enableCompressionFieldAddr = (int32_t*)getStaticFieldAddress(
                    stringClass, (unsigned char*)"enableCompression", 17, (unsigned char*)"Z", 1);
...
bool TR_J9VMBase::isBigDecimalClass(J9UTF8* className)
{
    return (J9UTF8_LENGTH(className) == BDCLASSLEN && !strcmp(utf8Data(className), recognizedBigDecimalClasses[0]));
}

bool TR_J9VMBase::isBigDecimalConvertersClass(J9UTF8* className)
{
    return (J9UTF8_LENGTH(className) == 32 && !strcmp(utf8Data(className), "com/ibm/BigDecimalConverters"));
}
...
// d169771 [2177]
uintptrj_t TR_J9VMBase::getPersistentClassPointerFromClassPointer(TR_OpaqueClassBlock* clazz)
{
...
TR_ResolvedMethod* TR_J9VM::getObjectNewInstanceImplMethod(TR_Memory* trMemory)
{
    ...
    J9Class* jlObject = intFunc->internalFindKnownClass(
        vmThread(), J9VMCONSTANTPOOL_JAVALANGOBJECT, J9_FINDKNOWNCLASS_FLAG_EXISTING_ONLY);
...
uintptrj_t TR_J9VMBase::getBytecodePC(TR_OpaqueMethodBlock* method, TR_ByteCodeInfo& bcInfo)
{
...
int TR_J9VMBase::findOrCreateMethodSymRef(TR::Compilation* comp, TR::ResolvedMethodSymbol* owningMethodSym,
    char* classSig, char** methodSig, TR::SymbolReference** symRefs, int methodCount)
{
...
// This function handles static and virtual functions only.
TR::SymbolReference* TR_J9VMBase::findOrCreateMethodSymRef(
    TR::Compilation* comp, TR::ResolvedMethodSymbol* owningMethodSym, char* classSig, char* methodSig)
{
...
// This function handles static and virtual functions only.
TR::SymbolReference* TR_J9VMBase::findOrCreateMethodSymRef(
    TR::Compilation* comp, TR::ResolvedMethodSymbol* owningMethodSym, char* methodSig)
{
...
// this function handles static and virtual functions only
int TR_J9VMBase::findOrCreateMethodSymRef(TR::Compilation* comp, TR::ResolvedMethodSymbol* owningMethodSym,
    char** methodSig, TR::SymbolReference** symRefs, int methodCount)
{
    int numMethodsFound = 0;
    for (int i = 0; i < methodCount; i++) {
        if (!methodSig[i])
...
void TR_J9VMBase::getResolvedMethods(
    TR_Memory* trMemory, TR_OpaqueClassBlock* classPointer, List<TR_ResolvedMethod>* resolvedMethodsInClass)
{
 ...
 */
TR_OpaqueMethodBlock* TR_J9VMBase::getMatchingMethodFromNameAndSignature(
    TR_OpaqueClassBlock* classPointer, const char* methodName, const char* signature, bool validate)
{
...
TR_ResolvedMethod* TR_J9VMBase::getResolvedMethodForNameAndSignature(
    TR_Memory* trMemory, TR_OpaqueClassBlock* classPointer, const char* methodName, const char* signature)
{
...
uint32_t TR_J9VMBase::getInstanceFieldOffsetIncludingHeader(
    char* classSignature, char* fieldName, char* fieldSig, TR_ResolvedMethod* method)
{
...
uint32_t TR_J9VMBase::getInstanceFieldOffset(
    TR_OpaqueClassBlock* clazz, char* fieldName, uint32_t fieldLen, char* sig, uint32_t sigLen, UDATA options)
{
    TR::VMAccessCriticalSection getInstanceFieldOffset(this);
    TR_ASSERT(clazz, "clazz should be set!");
    uint32_t result
        = (uint32_t)vmThread()->javaVM->internalVMFunctions->instanceFieldOffset(vmThread(), (J9Class*)clazz,
            (unsigned char*)fieldName, fieldLen, (unsigned char*)sig, sigLen, (J9Class**)&clazz, (UDATA*)NULL, options);
...
uint32_t TR_J9VMBase::getInstanceFieldOffset(
    TR_OpaqueClassBlock* clazz, char* fieldName, uint32_t fieldLen, char* sig, uint32_t sigLen)
{
...
{
    return (J9CLASS_FLAGS(TR::Compiler->cls.convertClassOffsetToClassPtr(clazzPointer))
               & J9_JAVA_CLASS_HAS_BEEN_OVERRIDDEN)
        != 0;
}

bool TR_J9VM::classHasBeenReplaced(TR_OpaqueClassBlock* clazzPointer)
{
    return (J9CLASS_FLAGS(TR::Compiler->cls.convertClassOffsetToClassPtr(clazzPointer)) & J9_JAVA_CLASS_HOT_SWAPPED_OUT)
        != 0;
}

bool TR_J9VMBase::isGetImplInliningSupported()
{
    J9JavaVM* jvm = _jitConfig->javaVM;
 ...
 */
int32_t TR_J9VMBase::getJavaLangClassHashCode(
    TR::Compilation* comp, TR_OpaqueClassBlock* clazzPointer, bool& hashCodeComputed)
{
...
bool TR_J9VMBase::hasFinalizer(TR_OpaqueClassBlock* classPointer)
{
    return (J9CLASS_FLAGS(TR::Compiler->cls.convertClassOffsetToClassPtr(classPointer))
               & (J9_JAVA_CLASS_FINALIZE | J9_JAVA_CLASS_OWNABLE_SYNCHRONIZER))
        != 0;
}

uintptrj_t TR_J9VM::getClassDepthAndFlagsValue(TR_OpaqueClassBlock* classPointer)
{
...
TR_OpaqueMethodBlock* TR_J9VM::getMethodFromName(
    char* className, char* methodName, char* signature, J9ConstantPool* constantPool)
{
    ...
    {
        methodClass = jitGetClassInClassloaderFromUTF8(
            vmThread(), (J9ClassLoader*)vmThread()->javaVM->systemClassLoader, className, strlen(className));
    }
     ...
     */
    if (methodClass)
        result = (TR_OpaqueMethodBlock*)TR_J9VM::getMethodFromClass(
            (TR_OpaqueClassBlock*)methodClass, methodName, signature);
    return result;
}
 ...
 */
TR_OpaqueMethodBlock* TR_J9VMBase::getMethodFromClass(
    TR_OpaqueClassBlock* methodClass, char* methodName, char* signature, TR_OpaqueClassBlock* callingClass)
{
...
const char* TR_J9VM::sampleSignature(TR_OpaqueMethodBlock* aMethod, char* buf, int32_t bufLen, TR_Memory* memory)
{
    ...
    if (s)
        sprintf(s, "%.*s.%.*s%.*s", J9UTF8_LENGTH(className), utf8Data(className), J9UTF8_LENGTH(name), utf8Data(name),
...
TR_OpaqueClassBlock* TR_J9VM::getClassFromNewArrayType(int32_t arrayType)
{
    struct J9Class** arrayClasses = &_jitConfig->javaVM->booleanArrayClass;
...
TR_OpaqueClassBlock* TR_J9VM::getSystemClassFromClassName(const char* name, int32_t length, bool isVettedForAOT)
{
    ...
    TR_OpaqueClassBlock* result = convertClassPtrToClassOffset(jitGetClassInClassloaderFromUTF8(
        vmThread(), (J9ClassLoader*)vmThread()->javaVM->systemClassLoader, (void*)name, length));
    return result;
}
...
bool TR_J9VMBase::jitFieldsAreSame(
    TR_ResolvedMethod* method1, I_32 cpIndex1, TR_ResolvedMethod* method2, I_32 cpIndex2, int32_t isStatic)
{
...
bool TR_J9VMBase::jitStaticsAreSame(
    TR_ResolvedMethod* method1, I_32 cpIndex1, TR_ResolvedMethod* method2, I_32 cpIndex2)
{
...
TR::CodeCache* TR_J9VM::getResolvedTrampoline(
    TR::Compilation* comp, TR::CodeCache* curCache, J9Method* method, bool inBinaryEncoding)
{
    bool hadClassUnloadMonitor;
    bool hadVMAccess = releaseClassUnloadMonitorAndAcquireVMaccessIfNeeded(comp, &hadClassUnloadMonitor);
...
intptrj_t TR_J9VM::methodTrampolineLookup(TR::Compilation* comp, TR::SymbolReference* symRef, void* callSite)
{
    ...
    case TR::java_lang_invoke_ComputedCalls_dispatchJ9Method: {
        ...
        //
        TR_RuntimeHelper vmCallHelper = TR::MethodSymbol::getVMCallHelperFor(
            methodSym->getMethod()->returnType(), methodSym->isSynchronised(), false, comp);
...
TR_OpaqueClassBlock* TR_J9VM::getBaseComponentClass(TR_OpaqueClassBlock* clazz, int32_t& numDims)
{
...
TR_YesNoMaybe TR_J9VM::isInstanceOf(
    TR_OpaqueClassBlock* a, TR_OpaqueClassBlock* b, bool objectTypeIsFixed, bool castTypeIsFixed, bool optimizeForAOT)
{
...
TR_OpaqueClassBlock* TR_J9VM::getClassFromSignature(
    const char* sig, int32_t sigLength, TR_ResolvedMethod* method, bool isVettedForAOT)
{
    return getClassFromSignature(
        sig, sigLength, (TR_OpaqueMethodBlock*)method->getPersistentIdentifier(), isVettedForAOT);
}

TR_OpaqueClassBlock* TR_J9VM::getClassFromSignature(
    const char* sig, int32_t sigLength, TR_OpaqueMethodBlock* method, bool isVettedForAOT)
{
...
bool TR_J9VM::isDecimalFormatPattern(TR::Compilation* comp, TR_ResolvedMethod* method)
{
    ...
    if (bc == J9BCgetfield) {
        ...
        resolved = method->fieldAttributes(
            comp, bci.next2Bytes(), &fieldOffset, &type, &isVolatile, 0, &isPrivate, false, &isUnresolvedInCP);
...
void TR_J9VM::transformJavaLangClassIsArray(TR::Compilation* comp, TR::Node* callNode, TR::TreeTop* treeTop)
{
    ...
    TR::Node* vftLoad = TR::Node::createWithSymRef(
        callNode, TR::aloadi, 1, jlClass, comp->getSymRefTab()->findOrCreateClassFromJavaLangClassSymbolRef());

    if (TR::Compiler->target.is32Bit()) {
        classFlag = TR::Node::createWithSymRef(
            callNode, TR::iloadi, 1, vftLoad, symRefTab->findOrCreateClassAndDepthFlagsSymbolRef());
    } else {
        classFlag = TR::Node::createWithSymRef(
            callNode, TR::lloadi, 1, vftLoad, symRefTab->findOrCreateClassAndDepthFlagsSymbolRef());
...
void TR_J9VM::transformJavaLangClassIsArrayOrIsPrimitive(
    TR::Compilation* comp, TR::Node* callNode, TR::TreeTop* treeTop, int32_t andMask)
{
    ...
    if (TR::Compiler->cls.classesOnHeap()) {
        vftFieldInd = TR::Node::createWithSymRef(
            TR::aloadi, 1, 1, vftField, comp->getSymRefTab()->findOrCreateClassFromJavaLangClassSymbolRef());
        isArrayField
            = TR::Node::createWithSymRef(TR::aloadi, 1, 1, vftFieldInd, symRefTab->findOrCreateClassRomPtrSymbolRef());
    } else {
        isArrayField
            = TR::Node::createWithSymRef(TR::aloadi, 1, 1, vftField, symRefTab->findOrCreateClassRomPtrSymbolRef());
    ...
    if (treeTop->getNode()->getOpCode().isNullCheck()) {
        ...
        TR::TreeTop::create(comp, treeTop->getPrevTreeTop(),
            TR::Node::createWithSymRef(
                TR::NULLCHK, 1, 1, vftFieldInd, symRefTab->findOrCreateNullCheckSymbolRef(owningMethodSymbol)));
    }

    TR::Node::recreate(callNode, TR::idiv);
    callNode->setNumChildren(2);

    isArrayField = TR::Node::createWithSymRef(
        TR::iloadi, 1, 1, isArrayField, comp->getSymRefTab()->findOrCreateClassIsArraySymbolRef());
...
TR::Node* TR_J9VM::inlineNativeCall(TR::Compilation* comp, TR::TreeTop* callNodeTreeTop, TR::Node* callNode)
{
    ...
    case TR::java_lang_invoke_ComputedCalls_dispatchJ9Method: {
        ...
        // This is a hack, and it appears more than once.  Search for PROPER_DISPATCH_J9METHOD.
        //
        TR_RuntimeHelper vmCallHelper
            = TR::MethodSymbol::getVMCallHelperFor(method->returnType(), sym->isSynchronised(), false, comp);
        TR::SymbolReference* helperSymRef
            = comp->getSymRefTab()->findOrCreateRuntimeHelper(vmCallHelper, true, true, false);
    ...
    case TR::java_lang_invoke_MethodHandle_invokeWithArgumentsHelper: {
        ...
        //
        TR::SymbolReference* helperSymRef = comp->getSymRefTab()->findOrCreateRuntimeHelper(
            TR_icallVMprJavaSendInvokeWithArguments, true, true, false);
    ...
    case TR::java_lang_Class_isInstance:
        if (comp->cg()->supportsInliningOfIsInstance()
            ...
            TR::Node* j9class = TR::Node::createWithSymRef(
                TR::aloadi, 1, 1, jlClass, comp->getSymRefTab()->findOrCreateClassFromJavaLangClassSymbolRef());
    ...
    case TR::java_lang_Object_getClass:
        ...
        if (TR::Compiler->cls.classesOnHeap()) {
            callNode = TR::Node::createWithSymRef(
                TR::aloadi, 1, 1, callNode, comp->getSymRefTab()->findOrCreateJavaLangClassFromClassSymbolRef());
        }
        return callNode;
    ...
    case TR::sun_reflect_Reflection_getCallerClass:
        ...
        // 2      1      0
        {
            ...
            // char *caseName = (methodID == TR::sun_reflect_Reflection_getCallerClass) ? "s/r/R.getCallerClass" :
            // "j/l/CL.getStackClassLoader";
            ...
            if (iconstNode->getOpCodeValue() == TR::iconst) {
                ...
                while (true) {
                    if (callerIndex != -1) {
                        ...
                        if (isAOT_DEPRECATED_DO_NOT_USE()) {
                            callerClass
                                = (J9Class*)((TR_AOTMethodInfo*)callerMethod)->resolvedMethod->containingClass();
                        } else {
                    ...
                    skipFrame = stackWalkerMaySkipFrames(
                        (TR_OpaqueMethodBlock*)callerMethod, (TR_OpaqueClassBlock*)callerClass);
                ...
                if (!skipFrame && (inlineDepth == stackDepth)) {
                    ...
                    if (returnClassLoader) {
                        if (performTransformation(comp,
                                "O^O inlineNativeCall: inline classloader load [%p] for '%s' at bytecode %d\n",
                                callNode, callNode->getSymbolReference()->getName(comp->getDebug()),
                                callNode->getByteCodeInfo().getByteCodeIndex())) {
                            TR::Node::recreate(callNode, TR::aload);
                            callNode->removeAllChildren();
                            TR::SymbolReference* callerClassLoaderSymRef
                                = comp->getSymRefTab()->findOrCreateClassLoaderSymbolRef(
                                    comp->getMethodSymbol()->getResolvedMethod());
                        ...
                        if (performTransformation(comp,
                                ...
                                callNode, classNameLength, className,
    ...
    case TR::java_lang_ref_Reference_getImpl:
        if (comp->getGetImplInlineable()) {
            ...
            TR::SymbolReference* symRefField = comp->getSymRefTab()->findOrCreateJavaLangReferenceReferentShadowSymbol(
                callerSymRef->getOwningMethodSymbol(comp), true, TR::Address, offset, false);
    ...
    case TR::java_lang_J9VMInternals_rawNewArrayInstance: {
        ...
        callNode->setSymbolReference(
            comp->getSymRefTab()->findOrCreateANewArraySymbolRef(callNode->getSymbol()->castToResolvedMethodSymbol()));
        TR::Node* newNode = TR::Node::createWithSymRef(
            callNode, TR::aloadi, 1, comp->getSymRefTab()->findOrCreateArrayComponentTypeSymbolRef());
        TR::Node* newNodeChild = TR::Node::createWithSymRef(
            callNode, TR::aloadi, 1, comp->getSymRefTab()->findOrCreateClassFromJavaLangClassAsPrimitiveSymbolRef());
    ...
    case TR::java_lang_J9VMInternals_rawNewInstance: {
        ...
        callNode->setSymbolReference(
            comp->getSymRefTab()->findOrCreateNewObjectSymbolRef(callNode->getSymbol()->castToResolvedMethodSymbol()));
        TR::Node* newNode = TR::Node::createWithSymRef(
            callNode, TR::aloadi, 1, comp->getSymRefTab()->findOrCreateClassFromJavaLangClassAsPrimitiveSymbolRef());
    ...
    case TR::com_ibm_jit_JITHelpers_is32Bit: {
        // instance method so receiver object is first child, and other nodes may hold onto it so need to decrement its
        // ref count following code hammers this call node into an iconst which disconnects the child
...
int32_t TR_J9VMBase::adjustedInliningWeightBasedOnArgument(
    int32_t origWeight, TR::Node* argNode, TR::ParameterSymbol* parmSymbol, TR::Compilation* comp)
{
...
TR_OpaqueClassBlock* TR_J9VM::getClassClassPointer(TR_OpaqueClassBlock* objectClassPointer)
{
    ...
    if (TR::Compiler->cls.classesOnHeap()) {
        void* javaLangClass
            = J9VM_J9CLASS_TO_HEAPCLASS(TR::Compiler->cls.convertClassOffsetToClassPtr(objectClassPointer));

        // j9class points to the J9Class corresponding to java/lang/Object
        if (TR::Compiler->om.generateCompressedObjectHeaders())
            j9class = (J9Class*)*(
                (uint32_t*)((uintptrj_t)javaLangClass + (uintptrj_t)TR::Compiler->om.offsetOfObjectVftField()));
        else
            j9class = (J9Class*)(*(
                (J9Class**)((uintptrj_t)javaLangClass + (uintptrj_t)TR::Compiler->om.offsetOfObjectVftField())));
    } else
        j9class
            = (J9Class*)(*((J9Class**)((uintptrj_t)j9class + (uintptrj_t)TR::Compiler->om.offsetOfObjectVftField())));
...
{
    TR::Compilation* comp = _compInfo->getCompInfoForCompOnAppThread()
        ? _compInfo->getCompInfoForCompOnAppThread()->getCompilation()
        : _compInfoPT->getCompilation();
...
bool TR_J9SharedCacheVM::isMethodEnterTracingEnabled(TR_OpaqueMethodBlock* method)
{
    // We want to return the same answer as TR_J9VMBase unless we want to force it to allow tracing
    return TR_J9VMBase::isMethodEnterTracingEnabled(method)
        || TR::Options::getAOTCmdLineOptions()->getOption(TR_EnableAOTMethodEnter);
}

bool TR_J9SharedCacheVM::isMethodExitTracingEnabled(TR_OpaqueMethodBlock* method)
{
    return TR_J9VMBase::isMethodExitTracingEnabled(method)
...
bool TR_J9SharedCacheVM::methodsCanBeInlinedEvenIfEventHooksEnabled() { return true; }

int32_t TR_J9SharedCacheVM::getJavaLangClassHashCode(
    TR::Compilation* comp, TR_OpaqueClassBlock* clazzPointer, bool& hashCodeComputed)
{
    hashCodeComputed = false;
    return 0;
...
uint32_t TR_J9SharedCacheVM::getInstanceFieldOffset(
    TR_OpaqueClassBlock* classPointer, char* fieldName, uint32_t fieldLen, char* sig, uint32_t sigLen, UDATA options)
{
...
void TR_J9SharedCacheVM::getResolvedMethods(
    TR_Memory* trMemory, TR_OpaqueClassBlock* classPointer, List<TR_ResolvedMethod>* resolvedMethodsInClass)
{
    TR::Compilation* comp = _compInfo->getCompInfoForCompOnAppThread()
        ? _compInfo->getCompInfoForCompOnAppThread()->getCompilation()
...
TR_ResolvedMethod* TR_J9SharedCacheVM::getResolvedMethodForNameAndSignature(
    TR_Memory* trMemory, TR_OpaqueClassBlock* classPointer, const char* methodName, const char* signature)
{
    TR::Compilation* comp = _compInfo->getCompInfoForCompOnAppThread()
        ? _compInfo->getCompInfoForCompOnAppThread()->getCompilation()
    ...
    if (comp->getOption(TR_UseSymbolValidationManager)) {
         ...
         */
        validated
            = comp->getSymbolValidationManager()->addClassFromMethodRecord(getClassFromMethodBlock(method), method);
    } else {
        validated = ((TR_ResolvedRelocatableJ9Method*)comp->getCurrentMethod())
                        ->validateArbitraryClass(comp, (J9Class*)classPointer);
    }
...
TR_OpaqueMethodBlock* TR_J9SharedCacheVM::getMethodFromName(
    char* className, char* methodName, char* signature, TR_OpaqueMethodBlock* callingMethod)
{
...
TR_OpaqueMethodBlock* TR_J9SharedCacheVM::getMethodFromClass(
    TR_OpaqueClassBlock* methodClass, char* methodName, char* signature, TR_OpaqueClassBlock* callingClass)
{
    ...
    if (omb) {
        ...
        if (comp->getOption(TR_UseSymbolValidationManager)) {
            bool validated = callingClass
                && comp->getSymbolValidationManager()->addMethodFromClassAndSignatureRecord(
                       omb, methodClass, callingClass);
...
bool TR_J9SharedCacheVM::supportAllocationInlining(TR::Compilation* comp, TR::Node* node)
{
    ...
    if ((TR::Compiler->target.cpu.isX86() || TR::Compiler->target.cpu.isPower() || TR::Compiler->target.cpu.isZ())
...
TR_YesNoMaybe TR_J9SharedCacheVM::isInstanceOf(
    TR_OpaqueClassBlock* a, TR_OpaqueClassBlock* b, bool objectTypeIsFixed, bool castTypeIsFixed, bool optimizeForAOT)
{
    TR::Compilation* comp = TR::comp();
    ...
    if (comp && comp->getOption(TR_UseSymbolValidationManager)) {
        if (isAnInstanceOf != TR_maybe)
            validated = comp->getSymbolValidationManager()->addClassInstanceOfClassRecord(
                a, b, objectTypeIsFixed, castTypeIsFixed, (isAnInstanceOf == TR_yes));
    } else {
...
TR_OpaqueClassBlock* TR_J9SharedCacheVM::getClassFromSignature(
    const char* sig, int32_t sigLength, TR_ResolvedMethod* method, bool isVettedForAOT)
{
    return getClassFromSignature(
        sig, sigLength, (TR_OpaqueMethodBlock*)method->getPersistentIdentifier(), isVettedForAOT);
...
TR_OpaqueClassBlock* TR_J9SharedCacheVM::getSystemClassFromClassName(
    const char* name, int32_t length, bool isVettedForAOT)
{
    TR::Compilation* comp = TR::comp();
    ...
    if (comp && comp->getOption(TR_UseSymbolValidationManager)) {
        ...
        if (isVettedForAOT) {
            if (((TR_ResolvedRelocatableJ9Method*)TR::comp()->getCurrentMethod())
                    ->validateArbitraryClass(TR::comp(), (J9Class*)classPointer))
                validated = true;
        }
    }
...
uintptrj_t TR_J9SharedCacheVM::getClassDepthAndFlagsValue(TR_OpaqueClassBlock* classPointer)
{
    TR::Compilation* comp = _compInfo->getCompInfoForCompOnAppThread()
        ? _compInfo->getCompInfoForCompOnAppThread()->getCompilation()
        : _compInfoPT->getCompilation();
...
TR_OpaqueClassBlock* TR_J9SharedCacheVM::getClassClassPointer(TR_OpaqueClassBlock* objectClassPointer)
{
...
TR_OpaqueMethodBlock* TR_J9SharedCacheVM::getInlinedCallSiteMethod(TR_InlinedCallSite* ics)
{
...
bool TR_J9SharedCacheVM::isGetImplInliningSupported() { return false; }

TR_ResolvedMethod* TR_J9SharedCacheVM::getObjectNewInstanceImplMethod(TR_Memory*) { return NULL; }

////////////////// Under evaluation

TR::CodeCache* TR_J9SharedCacheVM::getResolvedTrampoline(
    TR::Compilation* comp, TR::CodeCache*, J9Method* method, bool inBinaryEncoding)
{
    return 0;
}

intptrj_t TR_J9SharedCacheVM::methodTrampolineLookup(TR::Compilation* comp, TR::SymbolReference* symRef, void* callSite)
{
...
void* TR_J9SharedCacheVM::setJ2IThunk(
    char* signatureChars, uint32_t signatureLength, void* thunkptr, TR::Compilation* comp)
{
    return persistThunk(signatureChars, signatureLength, (U_8*)thunkptr - 8 /* start of thunk */,
        *((U_32*)((U_8*)thunkptr - 8)) + 8 /* size of thunk */);
}

void* TR_J9SharedCacheVM::persistJ2IThunk(void* thunk)
{
    return persistThunk(((TR_J2IThunk*)thunk)->terseSignature(), strlen(((TR_J2IThunk*)thunk)->terseSignature()),
        (uint8_t*)thunk, ((TR_J2IThunk*)thunk)->totalSize());
}

void* TR_J9SharedCacheVM::persistThunk(
    char* signatureChars, uint32_t signatureLength, uint8_t* thunkStart, uint32_t totalSize)
{
#if defined(J9VM_INTERP_AOT_COMPILE_SUPPORT) && defined(J9VM_OPT_SHARED_CLASSES) \
    && (defined(TR_HOST_X86) || defined(TR_HOST_POWER) || defined(TR_HOST_S390) || defined(TR_HOST_ARM))
    ...
    const void* store = _jitConfig->javaVM->sharedClassConfig->storeSharedData(
        curThread, signatureChars, signatureLength, &dataDescriptor);
...
J9Class* TR_J9SharedCacheVM::getClassForAllocationInlining(TR::Compilation* comp, TR::SymbolReference* classSymRef)
{
    ...
    if (!classSymRef->isUnresolved())
        ...
        return (J9Class*)classSymRef->getOwningMethod(comp)->getClassFromConstantPool(
            comp, classSymRef->getCPIndex(), returnClassForAOT);
}
...
JNIEXPORT jlong JNICALL Java_java_lang_invoke_ThunkTuple_initialInvokeExactThunk(JNIEnv* env, jclass clazz)
{
#if defined(J9ZOS390)
    ...
    JNIEnv* env, jclass MutableCallSite, jlongArray cookieArrayObject)
{
    ...
    if (verbose)
        TR_VerboseLog::writeLineLocked(
            TR_Vlog_HK, "%x hook %s vmThread=%p ", threadID, "MutableCallSite.invalidate", vmThread);
    ...
    if (!env->ExceptionCheck()) {
        ...
        for (int32_t i = 0; i < numSites; i++) {
            ...
            if (cookie) {
                if (details)
                    TR_VerboseLog::writeLineLocked(
                        TR_Vlog_HD, "%x     notifying cookies[%3d] " UINT64_PRINTF_FORMAT_HEX, threadID, i, cookie);
