...
inline void getClassNameSignatureFromMethod(
    J9Method* method, J9UTF8*& methodClazz, J9UTF8*& methodName, J9UTF8*& methodSignature)
{
    ...
    methodSignature
        = J9ROMMETHOD_GET_SIGNATURE(J9_CLASS_FROM_METHOD(method)->romClass, J9_ROM_METHOD_FROM_RAM_METHOD(method));
}
...
public:
    ...
    virtual int32_t adjustedInliningWeightBasedOnArgument(
    ...
    {
        return getInstanceFieldOffset(
            classPointer, fieldName, (uint32_t)strlen(fieldName), sig, (uint32_t)strlen(sig), options);
    }

    virtual uint32_t getInstanceFieldOffset(TR_OpaqueClassBlock* classPointer, char* fieldName, uint32_t fieldLen,
        char* sig, uint32_t sigLen, UDATA options);
    virtual uint32_t getInstanceFieldOffset(
        TR_OpaqueClassBlock* classPointer, char* fieldName, uint32_t fieldLen, char* sig, uint32_t sigLen);
    ...
    TR_OpaqueMethodBlock* getMatchingMethodFromNameAndSignature(
        TR_OpaqueClassBlock* classPointer, const char* methodName, const char* signature, bool validate = true);
     ...
     */
    virtual TR_ResolvedMethod* getResolvedMethodForNameAndSignature(
        TR_Memory* trMemory, TR_OpaqueClassBlock* classPointer, const char* methodName, const char* signature);
    virtual TR_OpaqueMethodBlock* getResolvedVirtualMethod(
        TR_OpaqueClassBlock* classObject, int32_t cpIndex, bool ignoreReResolve = true);
    virtual TR_OpaqueMethodBlock* getResolvedInterfaceMethod(
        TR_OpaqueMethodBlock* ownerMethod, TR_OpaqueClassBlock* classObject, int32_t cpIndex);

    TR_OpaqueMethodBlock* getResolvedInterfaceMethod(
    ...
    {
        return getReferenceFieldAt(
            objectPointer, getInstanceFieldOffset(getObjectClass(objectPointer), fieldName, fieldSignature));
    }
    uintptrj_t getVolatileReferenceField(uintptrj_t objectPointer, char* fieldName, char* fieldSignature)
    {
        return getVolatileReferenceFieldAt(
            objectPointer, getInstanceFieldOffset(getObjectClass(objectPointer), fieldName, fieldSignature));
    }
    ...
    virtual TR_ResolvedMethod* createResolvedMethodWithSignature(TR_Memory*, TR_OpaqueMethodBlock*,
        TR_OpaqueClassBlock*, char* signature, int32_t signatureLength, TR_ResolvedMethod*);
    ...
    virtual bool ensureOSRBufferSize(
    ...
    uintptrj_t getReferenceFieldAtAddress(void* fieldAddress)
    {
        return getReferenceFieldAtAddress((uintptrj_t)fieldAddress);
    }
    ...
    bool compareAndSwapInt64FieldAt(
    ...
    {
        return compareAndSwapInt64FieldAt(
            objectPointer, getInstanceFieldOffset(getObjectClass(objectPointer), fieldName, "J"), oldValue, newValue);
    }
    ...
    virtual TR_OpaqueClassBlock* getSystemClassFromClassName(
        const char* name, int32_t length, bool callSiteVettedForAOT = false)
    {
        return 0;
    }
    ...
    virtual int32_t getJavaLangClassHashCode(
    ...
    virtual bool scanReferenceSlotsInClassForOffset(
    ...
    virtual bool maybeHighlyPolymorphic(TR::Compilation*, TR_ResolvedMethod* caller, int32_t cpIndex, TR_Method* callee,
    ...
    virtual int checkInlineableTarget(
    ...
    virtual bool startAsyncCompile(
        TR_OpaqueMethodBlock* methodInfo, void* oldStartPC, bool* queued, TR_OptimizationPlan* optimizationPlan = NULL);
    ...
    virtual char* getJ2IThunkSignatureForDispatchVirtual(
    ...
    virtual TR_ResolvedMethod* createMethodHandleArchetypeSpecimen(
        TR_Memory*, uintptrj_t* methodHandleLocation, TR_ResolvedMethod* owningMethod = 0);
    virtual TR_ResolvedMethod* createMethodHandleArchetypeSpecimen(TR_Memory*, TR_OpaqueMethodBlock* archetype,
        uintptrj_t* methodHandleLocation,
    ...
    virtual uintptrj_t getFieldOffset(
    ...
    virtual bool getStringFieldByName(
    ...
    virtual void initializeLocalObjectHeader(
        TR::Compilation*, TR::Node* allocationNode, TR::TreeTop* allocationTreeTop);
    virtual void initializeLocalArrayHeader(TR::Compilation*, TR::Node* allocationNode, TR::TreeTop* allocaitonTreeTop);
    virtual TR::TreeTop* initializeClazzFlagsMonitorFields(
        TR::Compilation*, TR::TreeTop* prevTree, TR::Node* allocationNode, TR::Node* classNode, J9Class* ramClass);
    ...
    virtual TR::CodeCache* getResolvedTrampoline(
        TR::Compilation*, TR::CodeCache* curCache, J9Method* method, bool inBinaryEncoding)
        = 0;
    ...
    virtual TR_ExternalValueProfileInfo* getValueProfileInfoFromIProfiler(
        TR_ByteCodeInfo& bcInfo, TR::Compilation* comp);
    uint32_t* getAllocationProfilingDataPointer(
    ...
    virtual int32_t getIProfilerCallCount(
        TR_OpaqueMethodBlock* callee, TR_OpaqueMethodBlock* caller, int32_t bcIndex, TR::Compilation*);
    ...
    virtual bool releaseClassUnloadMonitorAndAcquireVMaccessIfNeeded(
        TR::Compilation* comp, bool* hadClassUnloadMonitor);
    virtual void acquireClassUnloadMonitorAndReleaseVMAccessIfNeeded(
    ...
    virtual TR::SymbolReference* findOrCreateMethodSymRef(
    ...
    uint32_t getInstanceFieldOffsetIncludingHeader(
    ...
    virtual void markClassForTenuredAlignment(
...
public:
    ...
    TR_OpaqueMethodBlock* getMethodFromName(
    ...
    virtual TR_OpaqueClassBlock* getSystemClassFromClassName(
        const char* name, int32_t length, bool isVettedForAOT = false);
...
private:
    void transformJavaLangClassIsArrayOrIsPrimitive(
        TR::Compilation*, TR::Node* callNode, TR::TreeTop* treeTop, int32_t andMask);
