 ...
 */
bool TR_AnnotationBase::scanForKnownAnnotationsAndRecord(
    TR::CompilationInfo* compInfo, J9Method* method, J9JavaVM* javaVM, TR_FrontEnd* fe)
{
...
J9AnnotationInfoEntry* TR_AnnotationBase::getDefaultAnnotationInfo(const char* annotationName)
{
    ...
    J9AnnotationInfoEntry* defaultEntry = intFunc->getAnnotationDefaultsForNamedAnnotation(
        vmThread, clazz, (char*)className, classNameLength, J9_FINDCLASS_FLAG_EXISTING_ONLY);
    return defaultEntry;
}
...
J9AnnotationInfoEntry* TR_AnnotationBase::getAnnotationInfoEntry(
    TR::SymbolReference* symRef, AnnotationSignature annotationEnum)
{
...
J9AnnotationInfoEntry* TR_AnnotationBase::getTaggedAnnotationInfoEntry(
    TR::SymbolReference* symRef, AnnotationSignature annotationEnum)
{
    return getAnnotationInfoEntry(symRef, recognizedAnnotations[annotationEnum].name, true);
}
J9AnnotationInfoEntry* TR_AnnotationBase::getAnnotationInfoEntry(
    TR::SymbolReference* symRef, const char* annotationName, bool isTag)
...
/* for now must do a search though all the annotation entries--later when Charlie has fixed it use
 * more efficient query */
bool TR_AnnotationBase::getValue(
    TR::SymbolReference* symRef, const char* annotationName, AnnotationType type, void* ptr)
{
...
bool TR_AnnotationBase::extractValue(
    J9AnnotationInfoEntry* annotationInfoEntryPtr, const char* annotationName, AnnotationType type, void* ptr)
{
    if (NULL == annotationInfoEntryPtr)
        return false;
    J9JavaVM* javaVM = ((TR_J9VMBase*)_comp->fej9())->_jitConfig->javaVM;
