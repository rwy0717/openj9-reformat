...
void TR_Debug::printAnnotationInfoEntry(
    J9AnnotationInfo* annotationInfo, J9AnnotationInfoEntry* annotationInfoEntryPtr, int32_t indentationLevel)
{
...
#define ANNO_NAMEBUF_LEN 30
    ...
    defaultEntry = intFunc->getAnnotationDefaultsForAnnotation(
        vmContext, clazz, annotationInfoEntryPtr, J9_FINDCLASS_FLAG_EXISTING_ONLY);
    if (defaultEntry) {
        trfprintf(_file, "\n");
        int32_t j;
        for (j = 0; j < indentationLevel; ++j)
