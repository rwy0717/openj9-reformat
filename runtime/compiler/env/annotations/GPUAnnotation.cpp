...
bool currentMethodHasFpreductionAnnotation(TR::Compilation* comp, bool trace)
{
    J9ROMMethod* romMethod
        = (J9ROMMethod*)J9_ROM_METHOD_FROM_RAM_METHOD((J9Method*)comp->getCurrentMethod()->getPersistentIdentifier());
    ...
    if (trace)
        traceMsg(comp, "current method has %d annotations %p\n", numAnnotations, annotationsData);
    ...
    for (int i = 0; i < numAnnotations; i++) {
        ...
        if (trace)
            traceMsg(comp, "found annotation %.*s\n", len, str);

        if (len == 44 && strncmp(str, "Lorg/apache/spark/sql/execution/fpreduction;", len) == 0) {
            if (trace)
                traceMsg(comp, "current method has @fpreduction annotation\n");
            return true;
        }
    }
