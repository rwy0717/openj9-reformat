...
TR_VMField::TR_VMField(TR::Compilation* comp, J9Class* aClazz, J9ROMFieldShape* fieldShape, TR_AllocationKind allocKind)
{
    ...
    if (modifiers & J9AccStatic)
        ...
        offset = comp->fej9()->getInstanceFieldOffset(
            (TR_OpaqueClassBlock*)aClazz, name, (uint32_t)(nameLength - 1), signature, (uint32_t)(sigLength - 1));
...
TR_VMFieldsInfo::TR_VMFieldsInfo(TR::Compilation* comp, J9Class* aClazz, int buildFields, TR_AllocationKind allocKind)
{
    ...
    int numBitsInWord
        = 8 * sizeof(UDATA); // instanceDescription is always a UDATA so number of bits should reflect that
    ...
    // null terminated
    _gcDescriptor
        = (int32_t*)_comp->trMemory()->allocateMemory((_numRefSlotsInObject + 1) * sizeof(int32_t), allocKind);
