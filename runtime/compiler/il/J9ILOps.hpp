...
public:
    ...
    {
        return getOpCodeValue() == TR::zd2dfAbs || getOpCodeValue() == TR::zd2ddAbs || getOpCodeValue() == TR::zd2deAbs;
    }

    bool isPackedToDFPAbs() const
    {
        return getOpCodeValue() == TR::pd2dfAbs || getOpCodeValue() == TR::pd2ddAbs || getOpCodeValue() == TR::pd2deAbs;
    }

    bool isZonedOrPackedToDFPAbs() const { return isZonedToDFPAbs() || isPackedToDFPAbs(); }
    ...
    static TR::ILOpCodes dfpToPackedCleanOp(TR::DataType dfpType, TR::DataType zonedType)
    {
        if (zonedType == TR::PackedDecimal) {
            ...
            case TR::DecimalFloat:
                return TR::df2pdClean;
            case TR::DecimalDouble:
                return TR::dd2pdClean;
            case TR::DecimalLongDouble:
                return TR::de2pdClean;
            default:
                return TR::BadILOp;
            }
        }
    ...
    static TR::ILOpCodes dfpToZonedCleanOp(TR::DataType dfpType, TR::DataType zonedType)
    {
        if (zonedType == TR::ZonedDecimal) {
            switch (dfpType) {
            case TR::DecimalFloat:
                return TR::df2zdClean;
            case TR::DecimalDouble:
                return TR::dd2zdClean;
            case TR::DecimalLongDouble:
                return TR::de2zdClean;
            default:
                return TR::BadILOp;
            }
        }
    ...
    static TR::ILOpCodes dfpToZonedSetSignOp(TR::DataType dfpType, TR::DataType zonedType)
    {
        if (zonedType == TR::ZonedDecimal) {
            switch (dfpType) {
            case TR::DecimalFloat:
                return TR::df2zdSetSign;
            case TR::DecimalDouble:
                return TR::dd2zdSetSign;
            case TR::DecimalLongDouble:
                return TR::de2zdSetSign;
            default:
                return TR::BadILOp;
            }
        }
    ...
    static TR::ILOpCodes dfpToPackedSetSignOp(TR::DataType dfpType, TR::DataType packedType)
    {
        if (packedType == TR::PackedDecimal) {
            switch (dfpType) {
            case TR::DecimalFloat:
                return TR::df2pdSetSign;
            case TR::DecimalDouble:
                return TR::dd2pdSetSign;
            case TR::DecimalLongDouble:
                return TR::de2pdSetSign;
            default:
                return TR::BadILOp;
            }
        }
    ...
    {
        TR_ASSERT(type == TR::DecimalFloat || type == TR::DecimalDouble || type == TR::DecimalLongDouble,
            "Invalid DFP type specified\n");

        switch (bcdOp) {
        case TR::pdadd:
            return addOpCode(type, false);
        case TR::pdsub:
            return subtractOpCode(type);
        case TR::pdmul:
            return multiplyOpCode(type);
        case TR::pddiv:
            return divideOpCode(type);
        case TR::pdneg:
            return negateOpCode(type);
        case TR::pdshr: {
            switch (type) {
            case TR::DecimalFloat:
                return TR::dfshr;
        ...
        default:
            TR_ASSERT(0, "unexpected binary coded decimal op\n");
            return TR::BadILOp;
        }
    }
    ...
    static TR::ILOpCodes insertExponentOp(TR::DataType type)
    {
        ...
        case TR::DecimalLongDouble:
            return TR::deInsExp;
        default:
            TR_ASSERT(0, "no insertExponent opcode for this dt %s", type.toString());
        }
        return TR::BadILOp;
    }
    ...
    static TR::ILOpCodes modifyPrecisionOpCode(TR::DataType type)
    {
        switch (type) {
        case TR::DecimalFloat:
            return TR::dfModifyPrecision;
        case TR::DecimalDouble:
            return TR::ddModifyPrecision;
        case TR::DecimalLongDouble:
            return TR::deModifyPrecision;
        case TR::PackedDecimal:
    ...
    static TR::ILOpCodes setNegativeOpCode(TR::DataType type)
    {
        switch (type) {
        case TR::DecimalFloat:
            return TR::dfSetNegative;
        case TR::DecimalDouble:
            return TR::ddSetNegative;
        case TR::DecimalLongDouble:
            return TR::deSetNegative;
        default:
            TR_ASSERT(false, "no setNegative opcode for datatype %s", type.toString());
        }
        return TR::BadILOp;
    }
    ...
    static TR::ILOpCodes shiftRightRoundedOpCode(TR::DataType type)
    {
        switch (type) {
        case TR::DecimalFloat:
            return TR::dfshrRounded;
        case TR::DecimalDouble:
            return TR::ddshrRounded;
        case TR::DecimalLongDouble:
            return TR::deshrRounded;
        default:
            TR_ASSERT(false, "no shrRounded opcode for datatype %d", type.getDataType());
        }
        return TR::BadILOp;
    }
