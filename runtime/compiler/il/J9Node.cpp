    ...
    if ((fromUnionPropertyB_Type == toUnionPropertyB_Type)) {
        ...
        case HasDecimalInfo:
            ...
            if (toNode->getType().isBCD() && fromNode->getType().isBCD()) {
                ...
                toNode->_unionPropertyB._decimalInfo._hasCleanSign
                    = fromNode->hasKnownOrAssumedCleanSign(); //  '10 0d' might be truncated to negative zero '0 00d' --
                                                              //  so no longer clean
        ...
        case HasBcdFlags:
            ...
            TR_ASSERT(fromNode->getOpCodeValue() == toNode->getOpCodeValue(),
                "Unexpected comparison. Nodes that have _bcdFlags property should always be BCDCHK nodes. These are %s "
                "%p and %s %p nodes",
                fromNode->getOpCode().getName(), fromNode, toNode->getOpCode().getName(), toNode);
            TR_ASSERT(false,
                "This makes no sense as recreate does not change the opcodevalue when copying forward properties from "
                "node %s %p to node %s %p",
                fromNode->getOpCode().getName(), fromNode, toNode->getOpCode().getName(), toNode);
            break;
        default:
            /* HasNoUnionPropertyB */
...
void J9::Node::devirtualizeCall(TR::TreeTop* treeTop)
{
...
void J9::Node::truncateOrWidenBCDLiteral(
    TR::DataType dt, char* newLit, int32_t newPrecision, char* oldLit, int32_t oldPrecision)
{
...
#ifdef TR_TARGET_S390
int32_t J9::Node::getStorageReferenceSize()
{
    TR::Compilation* comp = TR::comp();
    if (self()->getType().isAggregate()) {
        return self()->getSize();
    } else if (self()->getType().isAnyZoned() && self()->getOpCode().isConversion()
        && self()->getFirstChild()->getType().isDFP()) {
        if (self()->hasSourcePrecision()
            && self()->getSourcePrecision() <= TR::DataType::getMaxExtendedDFPPrecision()) {
...
int32_t J9::Node::getStorageReferenceSourceSize()
{
    TR::Compilation* comp = TR::comp();
    int32_t size = 0;
    switch (self()->getOpCodeValue()) {
    ...
    case TR::pd2iOverflow:
        ...
        break;
    default:
        if (self()->getOpCode().isConversion() && self()->getType().isDFP()
            && (self()->getFirstChild()->getType().isAnyZoned() || self()->getFirstChild()->getType().isAnyPacked())) {
...
bool J9::Node::pdshrRoundIsConstantZero()
{
    if (self()->getOpCode().isPackedRightShift()) {
        ...
        TR_ASSERT(
            false, "only packed and DFP shift right operations have a round node (op %d)\n", self()->getOpCodeValue());
    }
    return false;
}
...
bool J9::Node::referencesMayKillAliasInSubTree(TR::Node* rootNode, vcount_t visitCount)
{
    TR::Compilation* comp = TR::comp();
...
TR_ParentOfChildNode* J9::Node::referencesSymbolExactlyOnceInSubTree(
    TR::Node* parent, int32_t childNum, TR::SymbolReference* symRef, vcount_t visitCount)
{
    ...
    if (self()->getOpCode().hasSymbolReference()
        && (self()->getSymbolReference()->getReferenceNumber() == symRef->getReferenceNumber())) {
...
long double J9::Node::setLongDouble(long double d)
{
    self()->freeExtensionIfExists();
    return (*(long double*)_unionBase._unionedWithChildren._constData = d);
}

long double J9::Node::getLongDouble() { return *(long double*)_unionBase._unionedWithChildren._constData; }
...
bool J9::Node::isDFPModifyPrecision()
{
...
void J9::Node::setDFPPrecision(int32_t p)
{
...
uint8_t J9::Node::getDFPPrecision()
{
...
void J9::Node::setHasKnownCleanSign(bool v)
{
    ...
    if (self()->getType().isBCD()
        && performNodeTransformation2(
               c, "O^O NODE FLAGS: Setting hasKnownCleanSign flag on node %p to %d\n", self(), v)) {
...
void J9::Node::setHasAssumedCleanSign(bool v)
{
    ...
    if (self()->getType().isBCD()
        && performNodeTransformation2(
               c, "O^O NODE FLAGS: Setting hasAssumedCleanSign flag on node %p to %d\n", self(), v)) {
...
void J9::Node::setHasKnownPreferredSign(bool v)
{
    ...
    if (self()->getType().isBCD()
        && performNodeTransformation2(
               c, "O^O NODE FLAGS: Setting hasKnownPreferredSign flag on node %p to %d\n", self(), v)) {
        self()->setSignStateIsKnown();
        TR_ASSERT(self()->hasDecimalInfo(),
            "attempting to access _decimalInfo._hasPreferredSign field for node %s %p that does not have it",
            self()->getOpCode().getName(), self());
...
void J9::Node::setHasAssumedPreferredSign(bool v)
{
    ...
    if (self()->getType().isBCD()
        && performNodeTransformation2(
               c, "O^O NODE FLAGS: Setting hasAssumedPreferredSign flag on node %p to %d\n", self(), v)) {
        self()->setSignStateIsAssumed();
        TR_ASSERT(self()->hasDecimalInfo(),
            "attempting to access _decimalInfo._hasPreferredSign field for node %s %p that does not have it",
            self()->getOpCode().getName(), self());
...
void J9::Node::setKnownOrAssumedSignCode(TR_RawBCDSignCode sign, bool isKnown)
{
    ...
    if (self()->getType().isBCD() && TR::Node::typeSupportedForSignCodeTracking(self()->getDataType())) {
        if (performNodeTransformation2(
                c, "O^O NODE FLAGS: Setting knownSignCode on node %p to %s\n", self(), TR::DataType::getName(sign))) {
            if (isKnown)
                self()->setSignStateIsKnown();
            else
                self()->setSignStateIsAssumed();
...
void J9::Node::setHasSignStateOnLoad(bool v)
{
    ...
    if (self()->getOpCode().isBCDLoad()
        && performNodeTransformation2(
               c, "O^O NODE FLAGS: Setting _hasNoSignStateOnLoad flag on node %p to %d\n", self(), v ? 0 : 1)) {
        TR_ASSERT(self()->hasDecimalInfo(),
            "attempting to access _decimalInfo._hasNoSignStateOnLoad field for node %s %p that does not have it",
            self()->getOpCode().getName(), self());
        _unionPropertyB._decimalInfo._hasNoSignStateOnLoad
...
void J9::Node::setSpineCheckWithArrayElementChild(bool v)
{
    TR::Compilation* c = TR::comp();
    TR_ASSERT(self()->getOpCode().isSpineCheck(), "assertion failure");
    if (performNodeTransformation2(
            c, "O^O NODE FLAGS: Setting spineCHKWithArrayElementChild flag on node %p to %d\n", self(), v))
...
const char* J9::Node::printSpineCheckWithArrayElementChild()
{
    return self()->chkSpineCheckWithArrayElementChild() ? "spineCHKWithArrayElementChild " : "";
}

bool J9::Node::isUnsafePutOrderedCall()
{
...
bool J9::Node::isDontInlinePutOrderedCall()
{
...
void J9::Node::setDontInlinePutOrderedCall()
{
    TR::Compilation* c = TR::comp();
    TR_ASSERT(self()->getOpCode().isCall(), " Can only call this routine for a call node \n");
    bool isPutOrdered = self()->isUnsafePutOrderedCall();
    ...
    if (isPutOrdered) {
        if (performNodeTransformation1(
                c, "O^O NODE FLAGS: Setting dontInlineUnsafePutOrderedCall flag on node %p\n", self()))
            _flags.set(dontInlineUnsafePutOrderedCall);
    }
}

bool J9::Node::chkDontInlineUnsafePutOrderedCall()
{
    bool isPutOrdered = self()->isUnsafePutOrderedCall();
    return isPutOrdered && _flags.testAny(dontInlineUnsafePutOrderedCall);
}

const char* J9::Node::printIsDontInlineUnsafePutOrderedCall()
{
...
bool J9::Node::isUnsafeGetPutCASCallOnNonArray()
{
    ...
    // TR_ASSERT(symbol->castToResolvedMethodSymbol()->getResolvedMethod()->isUnsafeWithObjectArg(), "Attempt to check
    // flag on a method that is not JNI Unsafe that needs special care for arraylets\n");
...
void J9::Node::setUnsafeGetPutCASCallOnNonArray()
{
    TR::Compilation* c = TR::comp();
    ...
    // TR_ASSERT(getSymbol()->getMethodSymbol()->castToResolvedMethodSymbol()->getResolvedMethod()->isUnsafeWithObjectArg(),"Attempt
    // to change flag on a method that is not JNI Unsafe that needs special care for arraylets\n");
...
void J9::Node::setBCDStoreIsTemporarilyALoad(bool v)
{
    ...
    if (self()->getOpCode().isBCDLoadVar()
        && performNodeTransformation2(
               c, "O^O NODE FLAGS: Setting IsBCDStoreTemporarilyALoad flag on node %p to %d\n", self(), v))
...
void J9::Node::setCleanSignDuringPackedLeftShift(bool v)
{
    ...
    if (self()->getOpCode().isPackedLeftShift()
        && performNodeTransformation2(
               c, "O^O NODE FLAGS: Setting CleanSignDuringPackedLeftShift flag on node %p to %d\n", self(), v))
...
const char* J9::Node::printCleanSignDuringPackedLeftShift()
{
...
const char* J9::Node::printSkipCopyOnLoad() { return self()->chkSkipCopyOnLoad() ? "skipCopyOnLoad " : ""; }
...
void J9::Node::setSkipCopyOnStore(bool v)
{
    ...
    if (self()->chkOpsSkipCopyOnStore()
        && performNodeTransformation2(
               c, "O^O NODE FLAGS: Setting skipCopyOnStore flag on node %p to %d\n", self(), v)) {
...
const char* J9::Node::printSkipCopyOnStore() { return self()->chkSkipCopyOnStore() ? "skipCopyOnStore " : ""; }
...
void J9::Node::setCleanSignInPDStoreEvaluator(bool v)
{
    ...
    if (self()->chkOpsCleanSignInPDStoreEvaluator()
        && performNodeTransformation2(
               c, "O^O NODE FLAGS: Setting cleanSignInPDStoreEvaluator flag on node %p to %d\n", self(), v))
...
const char* J9::Node::printCleanSignInPDStoreEvaluator()
{
...
void J9::Node::setUseStoreAsAnAccumulator(bool v)
{
    ...
    if (self()->chkOpsUseStoreAsAnAccumulator()
        && performNodeTransformation2(
               c, "O^O NODE FLAGS: Setting UseStoreAsAnAccumulator flag on node %p to %d\n", self(), v)) {
...
const char* J9::Node::printUseStoreAsAnAccumulator()
{
...
void J9::Node::setSkipPadByteClearing(bool v)
{
    ...
    if (self()->getType().isAnyPacked() && !self()->getOpCode().isStore()
        && (c == NULL
               || performNodeTransformation2(
                      c, "O^O NODE FLAGS: Setting skipPadByteClearing flag on node %p to %d\n", self(), v)))
...
const char* J9::Node::printSkipPadByteClearing()
{
...
const char* J9::Node::printIsInMemoryCopyProp() { return self()->chkIsInMemoryCopyProp() ? "IsInMemoryCopyProp " : ""; }
...
const char* J9::Node::printSharedMemory() { return self()->getType().isAddress() ? "sharedMemory " : ""; }
...
bool J9::Node::requiresRegisterPair(TR::Compilation* comp)
{
    return self()->getType().isLongDouble() || OMR::NodeConnector::requiresRegisterPair(comp);
}

bool J9::Node::canGCandReturn()
{
    TR::Compilation* comp = TR::comp();
