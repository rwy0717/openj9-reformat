...
public:
    ...
    static void truncateOrWidenBCDLiteral(
...
protected:
    ...
    struct DecimalInfo {
        ...
        int32_t _decimalAdjustOrFractionOrDivisor : 7; ///< range as Adjust -63->63 (on all except conversion nodes)
                                                       ...
                                                       ///< range as extFloatSrcIntDigits 0->63 (but should only need
                                                       ///< 16, on extFloat to other conversions, e.g. zf2uf,zf2d)
        ...
        uint32_t _castedToBCD : 1; ///< true/false // if true node was not originally a BCD type but casted to BCD by
                                   ///< the optimizer from a non-BCD type (e.g. aggr or int)
