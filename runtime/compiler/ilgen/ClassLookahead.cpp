...
TR_ClassLookahead::TR_ClassLookahead(
    TR_PersistentClassInfo* classInfo, TR_FrontEnd* fe, TR::Compilation* comp, TR::SymbolReferenceTable* symRefTab)
...
int32_t TR_ClassLookahead::perform()
{
    ...
    if (classInitializer) {
        ...
        _inFirstBlock = true;
        /// fprintf(stderr, "looking at method %s\n", classInitializer->getResolvedMethod()->nameChars());
        /// fflush(stderr);
...
static bool isCalledByNonConstructorMethodsInClass(
    TR_ResolvedMethod* calleeMethod, List<TR::ResolvedMethodSymbol>* resolvedMethodSyms)
{
    ...
    TR::ResolvedMethodSymbol** classInitializer, bool* peekFailedForAnyMethod)
{
    ...
    for (resolvedMethod = resolvedIt.getFirst(); resolvedMethod; resolvedMethod = resolvedIt.getNext()) {
        ...
        if (resolvedMethod->isStatic())
            methodSymRef = _symRefTab->findOrCreateMethodSymbol(
                JITTED_METHOD_INDEX, -1, resolvedMethod, TR::MethodSymbol::Static);
        else
            methodSymRef = _symRefTab->findOrCreateMethodSymbol(
                JITTED_METHOD_INDEX, -1, resolvedMethod, TR::MethodSymbol::Interface);
        ...
        if (resolvedMethod->isCompilable(comp()->trMemory()) && !resolvedMethod->isNewInstanceImplThunk()
            ...
            // printf("IL gen succeeded for %s resolvedMethod %x ramMethod %x\n", resolvedMethod->signature(),
            // resolvedMethod, resolvedMethod->getPersistentIdentifier());
            ...
            *peekFailedForAnyMethod = (NULL
                == resolvedMethodSymbol->getResolvedMethod()->genMethodILForPeeking(
                       resolvedMethodSymbol, comp(), true));
        }
    }
                ...
                if (fieldInfo->isTypeInfoValid()) {
                    ...
                    if (classPtrSame || isClassPtrInitialized) {
                        ...
                        if (_inClassInitializerMethod) {
                            if (_traceIt)
                                traceMsg(
                                    comp(), "CStatically initializing type info for symbol %x at node %x\n", sym, node);
                            ...
                            if (fieldInfo->isTypeInfoValid() == VALID_AND_INITIALIZED_STATICALLY) {
                                ...
                                if (!sym->isStatic()) {
                                    ...
                                    if (_traceIt)
                                        traceMsg(
                                            comp(), "C0Invalidating type info for symbol %x at node %x\n", sym, node);
                    ...
                    if (oldNumDimensions > -1) {
                        if (arrayFieldInfo->isDimensionInfoValid()) {
                            if (oldNumDimensions != numberOfDimensions) {
                                ...
                                if (_traceIt)
                                    traceMsg(
                                        comp(), "0Invalidating dimension info for symbol %x at node %x\n", sym, node);
                    ...
                    if (arrayFieldInfo->isDimensionInfoValid()) {
                        ...
                        for (; k < (rhsNumChildren - 1 - firstDimensionChild); k++) {
                            ...
                            if (_inFirstBlock && _inInitializerMethod && (rhsChild->getOpCodeValue() == TR::iconst)
                                ...
                                if (_inClassInitializerMethod) {
                                    ...
                                    if (arrayFieldInfo->isDimensionInfoValid() == VALID_AND_INITIALIZED_STATICALLY) {
                                        if (!_inFirstInitializerMethod && !sym->isStatic()) {
                                            if (_traceIt)
                                                traceMsg(comp(),
                                                    "0Not always initializing dimension info for symbol %x at node "
                                                    "%x\n",
                                                    sym, node);
                                            ...
                                            if (!sym->isStatic()) {
                                                if (_traceIt)
                                                    traceMsg(comp(),
                                                        "0Always initializing dimension info for symbol %x at node "
                                                        "%x\n",
                                                        sym, node);
                                ...
                                if (_traceIt)
                                    traceMsg(
                                        comp(), "C1Invalidating dimension info for symbol %x at node %x\n", sym, node);
                                arrayFieldInfo->setIsDimensionInfoValid(INVALID);
                                arrayFieldInfo->setDimensionInfo(k, -1);
                            }
                        }
                ...
                if (fieldInfo) {
                    ...
                    if (rhsOfStoreNode->getOpCode().isCall()
                        ...
                        if (rhsOfStoreNode->getSymbol()->getResolvedMethodSymbol()->getRecognizedMethod()
                            ...
                            // _currentMethodSymbol->getResolvedMethod()->nameChars()); fflush(stderr);
                        ...
                        if (rhsOfStoreNode->getOpCode().isLoadVar()
                            ...
                            if (field != TR::Symbol::Java_math_BigInteger_ZERO) {
                                ...
                                // fprintf(stderr, "Found load of big integer ZERO in for %s (big integer type %d) in %s
                                // compiling %s\n", sig, fieldInfo->isBigIntegerType(),
                                // _currentMethodSymbol->getResolvedMethod()->nameChars(), comp()->signature());
                                // fflush(stderr);
                            }
                        } else {
...
static bool isPureBigDecimalMethod(
    TR::Node* node, TR::Compilation* comp, TR_PersistentFieldInfo* fieldInfo, bool& isBigDecimal, bool& isBigInteger)
{
    if (!node)
        return false;
    ...
    //   node->getSymbolReference()->getOffset(), comp->getSymRefTab()->findVftSymbolRef());
    //   }
...
void TR_ClassLookahead::invalidateIfEscapingLoad(
    TR::TreeTop* nextTree, TR::Node* grandParent, TR::Node* parent, int32_t childNum, TR::Node* node)
{
    TR::SymbolReference* storedSymRef = node->getSymbolReference();
    TR::Symbol* sym = storedSymRef->getSymbol();
...
char* TR_ClassLookahead::getFieldSignature(
    TR::Compilation* comp, TR::Symbol* sym, TR::SymbolReference* symRef, int32_t& length)
{
...
TR_PersistentArrayFieldInfo* TR_ClassLookahead::getExistingArrayFieldInfo(
    TR::Symbol* fieldSym, TR::SymbolReference* fieldSymRef)
{
        ...
        if (length > -1) {
            ...
            if (newFieldInfo) {
                if (newFieldInfo->canChangeToArray()) {
                    ...
                    traceMsg(
                        comp(), "fieldInfo %p exists already for array field %s, so cannot morph\n", newFieldInfo, sig);
...
TR_PersistentFieldInfo* TR_ClassLookahead::getExistingFieldInfo(
    TR::Symbol* fieldSym, TR::SymbolReference* fieldSymRef, bool canMorph)
{
...
void TR_ClassLookahead::makeInfoPersistent()
{
    ...
    while (fieldInfo) {
        ...
        if (isTypeInfoValid || isDimensionInfoValid || isImmutable
            ...
            //   fieldInfo->getFieldSignature()); fflush(stdout);
            //   }
...
TR_PersistentFieldInfo* TR_PersistentClassInfoForFields::find(
    TR::Compilation* comp, TR::Symbol* fieldSym, TR::SymbolReference* fieldSymRef)
{
    int32_t length = 0;
    char* sig = TR_ClassLookahead::getFieldSignature(comp, fieldSym, fieldSymRef, length);
...
TR_PersistentFieldInfo* TR_PersistentClassInfoForFields::findFieldInfo(
    TR::Compilation* comp, TR::Node*& node, bool canBeArrayShadow)
{
