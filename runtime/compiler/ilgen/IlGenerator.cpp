...
bool TR_J9ByteCodeIlGenerator::genIL()
{
    ...
    if (success) {
        ...
        if (!strcmp(methodName,
                ...
                "lang/String;")
            || !strcmp(methodName,
                   "com/ibm/jit/DecimalFormatHelper.formatAsFloat(Ljava/text/DecimalFormat;Ljava/math/"
    ...
    if (success && !comp()->isPeekingMethod()) {
        ...
        for (TR::Block* block = _methodSymbol->getFirstTreeTop()->getEnclosingBlock(); block;
            ...
            _methodSymbol->addProfilingOffsetInfo(
                block->getEntry()->getNode()->getByteCodeIndex(), block->getExit()->getNode()->getByteCodeIndex());
    }
    ...
    if (recognizedMethod != TR::unknownMethod) {
        ...
        if (recognizedMethod == TR::com_ibm_jit_JITHelpers_supportsIntrinsicCaseConversion
            ...
            if (performTransformation(
                    comp(), "O^O IlGenerator: Generate com/ibm/jit/JITHelpers.supportsIntrinsicCaseConversion\n")) {
        ...
        if (recognizedMethod == TR::com_ibm_dataaccess_DecimalData_JITIntrinsicsEnabled) {
            if (performTransformation(
                    comp(), "O^O IlGenerator: Generate com/ibm/dataaccess/DecimalData.JITIntrinsicsEnabled\n")) {
        ...
        if (recognizedMethod == TR::com_ibm_rmi_io_FastPathForCollocated_isVMDeepCopySupported) {
            if (performTransformation(
                    comp(), "O^O IlGenerator: Generate com/ibm/rmi/io/FastPathForCollocated/isVMDeepCopySupported\n")) {
...
bool TR_J9ByteCodeIlGenerator::genILFromByteCodes()
{
        ...
        if (comp()->getOption(TR_FullSpeedDebug) || !comp()->getOption(TR_DisableLiveMonitorMetadata)) {
            ...
            if (comp()->getOption(TR_FullSpeedDebug))
                symRef = symRefTab()->findOrCreateAutoSymbol(
                    _methodSymbol, _methodSymbol->getSyncObjectTempIndex(), TR::Address);
            else
    ...
    if (_methodSymbol->getResolvedMethod()->isNonEmptyObjectConstructor()) {
        if (comp()->getOption(TR_FullSpeedDebug)) {
            TR::SymbolReference* symRef = symRefTab()->findOrCreateAutoSymbol(
                _methodSymbol, _methodSymbol->getThisTempForObjectCtorIndex(), TR::Address);
    ...
    TR_BoolArrayStoreTransformer::NodeSet bstoreiUnknownArrayTypeNodes(
        std::less<TR::Node*>(), comp()->trMemory()->currentStackRegion());
    TR_BoolArrayStoreTransformer::NodeSet bstoreiBoolArrayTypeNodes(
        std::less<TR::Node*>(), comp()->trMemory()->currentStackRegion());
    ...
    for (auto handlerInfoIter = _tryCatchInfo.begin(); handlerInfoIter != _tryCatchInfo.end(); ++handlerInfoIter) {
        ...
        if (isGenerated(firstIndex)) {
            ...
            if (!generateNewBlock) {
                if (handlerInfo._catchType != dupHandler->_catchType) {
                    lastBlock = cloneHandler(
                        &handlerInfo, dupHandler->_firstBlock, dupHandler->_lastBlock, lastBlock, &clonedCatchBlocks);
                    /*
                    TR_BlockCloner cloner(cfg());
        ...
        if (generateNewBlock) {
            ...
            if (bc != J9BCastore) {
                ...
                if (trace)
                    traceMsg(comp(),
                        "catch block first BC is not an astore, inserting explicit store of exception object\n");
            }
            ...
            if (handlerBlockFromNonExceptionControlFlow) {
                ...
                // traceMsg(comp(), "last node %p bc index %d and opcode %s\n", lastNode, lastNode->getByteCodeIndex(),
                // lastNode->getOpCode().getName());
...
TR::Node* TR_J9ByteCodeIlGenerator::genMethodEnterHook()
{
    if (method()->isStatic())
        return TR::Node::createWithSymRef(
            TR::MethodEnterHook, 0, symRefTab()->findOrCreateReportStaticMethodEnterSymbolRef(_methodSymbol));

    loadAuto(TR::Address, 0);
    return TR::Node::createWithSymRef(
        TR::MethodEnterHook, 1, 1, pop(), symRefTab()->findOrCreateReportMethodEnterSymbolRef(_methodSymbol));
}
    ...
    if (_methodSymbol->isSynchronised()) {
        ...
        if (TR::Compiler->cls.classesOnHeap()) {
            if (firstChild->getOpCodeValue() == TR::loadaddr && firstChild->getSymbol()->isClassObject()) {
                monitorEnter = TR::Node::createWithSymRef(
                    TR::aloadi, 1, 1, firstChild, symRefTab()->findOrCreateJavaLangClassFromClassSymbolRef());
        ...
        if (_methodSymbol->getSyncObjectTemp()) {
            if (_methodSymbol->isStatic())
                loadSymbol(
                    TR::loadaddr, symRefTab()->findOrCreateClassSymbol(_methodSymbol, 0, method()->containingClass()));
            else
                loadAuto(TR::Address, 0);
...
void TR_J9ByteCodeIlGenerator::prependGuardedCountForRecompilation(TR::Block* originalFirstBlock)
{
    ...
    if (comp()->getOption(TR_ImmediateCountingRecompilation)) {
        cmpFlagNode = TR::Node::createif(
            TR::ificmpeq, TR::Node::iconst(1234), TR::Node::iconst(5678), originalFirstBlock->getEntry());
    } else {
        TR::Node* loadFlagNode = TR::Node::createWithSymRef(
            node, TR::iload, 0, comp()->getSymRefTab()->findOrCreateCountForRecompileSymbolRef());
#ifndef PUBLIC_BUILD
        if (comp()->getOption(TR_EnableGCRPatching))
            cmpFlagNode = TR::Node::createif(
                TR::ificmpne, loadFlagNode, TR::Node::create(node, TR::iconst, 0, 1), originalFirstBlock->getEntry());
        else
#endif
            cmpFlagNode = TR::Node::createif(
                TR::ificmpeq, loadFlagNode, TR::Node::create(node, TR::iconst, 0, 0), originalFirstBlock->getEntry());
    }
    ...
    TR::Node* cmpCountNode = TR::Node::createif(
        TR::ificmpgt, bumpNode, TR::Node::create(TR::iconst, 0, 0), originalFirstBlock->getEntry());
    ...
    if (comp()->getOption(TR_EnableGCRPatching)) {
        ...
        callRecompileBlock->append(TR::TreeTop::create(comp(),
            TR::Node::createWithSymRef(
                TR::bstore, 1, 1, constNode, comp()->getSymRefTab()->findOrCreateGCRPatchPointSymbolRef())));
    }

#endif
    TR::TreeTop* callTree = TR::TransformUtil::generateRetranslateCallerWithPrepTrees(
        node, TR_PersistentMethodInfo::RecompDueToGCR, comp());
...
void TR_J9ByteCodeIlGenerator::genDFPGetHWAvailable()
{
...
void TR_J9ByteCodeIlGenerator::genJITIntrinsicsEnabled()
{
    ...
    static int32_t constToLoad
        = (TR::Compiler->target.isZOS() || isZLinux) && !comp()->getOption(TR_DisablePackedDecimalIntrinsics) ? 1 : 0;
...
TR::Node* TR_J9ByteCodeIlGenerator::genNewInstanceImplCall(TR::Node* classNode)
{
    ...
    TR::Node* callNode;
    if (TR::Compiler->cls.classesOnHeap()) {
        TR::Node* classNodeAsClass = TR::Node::createWithSymRef(
            TR::aloadi, 1, 1, classNode, symRefTab()->findOrCreateClassFromJavaLangClassSymbolRef());
        ...
        TR::Node* node = TR::Node::createWithSymRef(
            TR::loadaddr, 0, symRefTab()->findOrCreateClassSymbol(_methodSymbol, -1, caller->classOfMethod()));
        TR::Node* nodeAsObject = TR::Node::createWithSymRef(
            TR::aloadi, 1, 1, node, symRefTab()->findOrCreateJavaLangClassFromClassSymbolRef());

        callNode = TR::Node::createWithSymRef(TR::acalli, 3, 3, classNodeAsClass, classNode, nodeAsObject,
            symRefTab()->findOrCreateObjectNewInstanceImplSymbol(_methodSymbol));
    } else {
        // the call to findOrCreateClassSymbol is safe even though we pass CPI of -1 since we check for
        // !compileRelocatableCode() in the caller
        callNode = TR::Node::createWithSymRef(TR::acalli, 3, 3, classNode, classNode,
            TR::Node::createWithSymRef(
                TR::loadaddr, 0, symRefTab()->findOrCreateClassSymbol(_methodSymbol, -1, caller->classOfMethod())),
            symRefTab()->findOrCreateObjectNewInstanceImplSymbol(_methodSymbol));
    }
...
void TR_J9ByteCodeIlGenerator::genDLTransfer(TR::Block* firstBlock)
{
    ...
    if (_blocks[dltBCIndex] != NULL) {
        ...
        for (steerTarget = myBlock->getEntry();
            ...
            ;
        TR_ASSERT(
            steerTarget->getNode()->getByteCodeIndex() == dltBCIndex, "Cannot find the treeTop DLT point belongs to");
                    ...
                    if (addrNode == NULL || nodeRealOffset != realOffset) {
                        ...
                        if (TR::Compiler->target.is64Bit()) {
                            ...
                            addrNode = TR::Node::create(
                                TR::aiadd, 2, dltBufChild, TR::Node::create(TR::iconst, 0, realOffset));
                    }
                    loadNode = TR::Node::createWithSymRef(
                        comp()->il.opCodeForIndirectLoad(dataType), 1, 1, addrNode, shadowSymRef);
                } else {
                    switch (dataType) {
                    case TR::Int32:
    ...
    for (TR::Block* block = firstBlock; block; block = block->getNextBlock()) {
        ...
        for (; tt != block->getExit(); tt = tt->getNextTreeTop()) {
            ...
            if (node->getOpCode().isCall() && (node->getSymbolReference() == finalizeSymRef)) {
                ...
                TR::Node* classDepthAndFlagsNode = TR::Node::createWithSymRef(
                    loadOp, 1, 1, vftLoad, comp()->getSymRefTab()->findOrCreateClassAndDepthFlagsSymbolRef());
...
struct TR_J9ByteCodeIlGenerator::methodRenamePair TR_J9ByteCodeIlGenerator::_decFormatRenames[_numDecFormatRenames] = {
    ...
    { "com/ibm/jit/DecimalFormatHelper.subformat(Ljava/text/DecimalFormat;Ljava/lang/StringBuffer;Lcom/ibm/jit/"
      "DecimalFormatHelper$FieldDelegate;ZZIIII)Ljava/lang/StringBuffer;",
        "java/text/DecimalFormat.subformat(Ljava/lang/StringBuffer;Ljava/text/Format$FieldDelegate;ZZIIII)Ljava/lang/"
        "StringBuffer;" },
};
...
bool TR_J9ByteCodeIlGenerator::replaceField(
    TR::Node* node, char* destClass, char* destFieldName, char* destFieldSignature, int parmIndex)
{
    ...
    if (performTransformation(comp(), "%ssymref replaced by %s.%s %s in [%p]\n", OPT_DETAILS, destClass, destFieldName,
        ...
        TR::SymbolReference* symRef = new (trHeapMemory()) TR::SymbolReference(
            comp()->getSymRefTab(), sym, comp()->getMethodSymbol()->getResolvedMethodIndex(), -1, 0);
        ...
        if (!node->getOpCode().isIndirect()) {
            ...
            if (parmIndex == 1)
                p = parms.getNext();
            TR::Node* decFormObjLoad = TR::Node::createLoad(
                node, symRefTab()->findOrCreateAutoSymbol(_methodSymbol, p->getSlot(), p->getDataType()));
...
bool TR_J9ByteCodeIlGenerator::replaceStatic(TR::Node* node, char* dstClassName, char* staticName, char* type)
{
    ...
    void* dataAddress = fej9()->getStaticFieldAddress(
        c, (unsigned char*)staticName, strlen(staticName), (unsigned char*)type, strlen(type));
...
{
    bool result = true;
    if (matchFieldOrStaticName(
            comp(), node, "com/ibm/jit/DecimalFormatHelper.INSTANCE Lcom/ibm/jit/DecimalFormatHelper$FieldPosition;")) {
    ...
    } else if (matchFieldOrStaticName(
                   comp(), node, "com/ibm/jit/DecimalFormatHelper.symbols Ljava/text/DecimalFormatSymbols;")) {
        ...
        result = replaceField(node, "java/text/DecimalFormat", "useExponentialNotation", "Z", 0);
                   ...
                   comp(), node, "com/ibm/jit/DecimalFormatHelper.negativePrefix Ljava/lang/String;")) {
        result = replaceField(node, "java/text/DecimalFormat", "negativePrefix", "Ljava/lang/String;", 0);
    } else if (matchFieldOrStaticName(
                   comp(), node, "com/ibm/jit/DecimalFormatHelper.negativeSuffix Ljava/lang/String;")) {
        result = replaceField(node, "java/text/DecimalFormat", "negativeSuffix", "Ljava/lang/String;", 0);
    } else if (matchFieldOrStaticName(
                   comp(), node, "com/ibm/jit/DecimalFormatHelper.positivePrefix Ljava/lang/String;")) {
        result = replaceField(node, "java/text/DecimalFormat", "positivePrefix", "Ljava/lang/String;", 0);
    } else if (matchFieldOrStaticName(
                   comp(), node, "com/ibm/jit/DecimalFormatHelper.positiveSuffix Ljava/lang/String;")) {
...
void TR_J9ByteCodeIlGenerator::expandInvokeSpecialInterface(TR::TreeTop* tree)
{
    ...
    if (trace) {
        traceMsg(comp(), "expanding invokespecial in interface method at n%un\n", tree->getNode()->getGlobalIndex());
        if (verbose)
    ...
    if (trace) {
        traceMsg(comp(), "generated helper call n%un for type test failure\n", helperCall->getGlobalIndex());
    }
...
void TR_J9ByteCodeIlGenerator::separateNullCheck(TR::TreeTop* tree)
{
    ...
    if (comp()->getOption(TR_TraceILGen)) {
        traceMsg(comp(), "separating null check on n%un from n%un\n", checkedRef->getGlobalIndex(),
            nullCheck->getGlobalIndex());
    }

    TR::Node* const passthrough = TR::Node::create(TR::PassThrough, 1, checkedRef);

    TR::SymbolReference* const nullCheckSR = symRefTab()->findOrCreateNullCheckSymbolRef(_methodSymbol);
    TR::Node* const separateNullCheck = TR::Node::createWithSymRef(TR::NULLCHK, 1, 1, passthrough, nullCheckSR);
    ...
    case TR::ResolveAndNULLCHK:
        nullCheck->setSymbolReference(symRefTab()->findOrCreateResolveCheckSymbolRef(_methodSymbol));
...
TR::Node* TR_J9ByteCodeIlGenerator::loadCallSiteMethodTypeFromCP(TR::Node* methodHandleInvokeCall)
{
    int32_t cpIndex = next2Bytes();
...
TR::Node* TR_J9ByteCodeIlGenerator::loadCallSiteMethodType(TR::Node* methodHandleInvokeCall)
{
...
void TR_J9ByteCodeIlGenerator::insertCustomizationLogicTreeIfEnabled(TR::TreeTop* tree, TR::Node* methodHandle)
{
    if (comp()->getOption(TR_EnableMHCustomizationLogicCalls)) {
        TR::SymbolReference* doCustomizationLogic = comp()->getSymRefTab()->methodSymRefFromName(
            _methodSymbol, JSR292_MethodHandle, "doCustomizationLogic", "()V", TR::MethodSymbol::Special);
...
void TR_J9ByteCodeIlGenerator::expandInvokeHandleGeneric(TR::TreeTop* tree)
{
    ...
    TR::SymbolReference* typeConversionSymRef = comp()->getSymRefTab()->methodSymRefFromName(
        _methodSymbol, JSR292_MethodHandle, JSR292_asType, JSR292_asTypeSig, TR::MethodSymbol::Static);
...
void TR_J9ByteCodeIlGenerator::expandInvokeExact(TR::TreeTop* tree)
{
    ...
    TR::SymbolReference* invokeExactTargetAddrSymRef
        = comp()->getSymRefTab()->methodSymRefFromName(_methodSymbol, JSR292_MethodHandle,
            JSR292_invokeExactTargetAddress, JSR292_invokeExactTargetAddressSig, TR::MethodSymbol::Special);
    TR::Node* invokeExactTargetAddr
        = TR::Node::createWithSymRef(callNode, TR::lcall, 1, receiverHandle, invokeExactTargetAddrSymRef);
