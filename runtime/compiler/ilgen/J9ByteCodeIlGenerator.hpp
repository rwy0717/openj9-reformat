...
private:
    ...
    void genHandleTypeCheck(TR::Node* handle, TR::Node* expectedType)
    {
        push(handle);
        push(expectedType);
        genHandleTypeCheck();
    ...
    TR::Node* loadConstantValueIfPossible(
        TR::Node*, uintptrj_t, TR::DataType type = TR::Int32, bool isArrayLength = true);
    ...
    TR::Node* genNodeAndPopChildren(
        TR::ILOpCodes, int32_t, TR::SymbolReference*, int32_t firstIndex, int32_t lastIndex);
