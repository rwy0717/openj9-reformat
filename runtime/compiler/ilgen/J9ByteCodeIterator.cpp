...
int32_t TR_J9ByteCodeIterator::findFloatingPointInstruction()
{
    ...
    for (TR_J9ByteCode bc = first(); bc != J9BCunknown; bc = next())
        ...
        case J9BCgetstatic:
        case J9BCputstatic:
            method()->staticAttributes(_compilation, next2Bytes(), &staticAddress, &type, &isVolatile, NULL, &isPrivate,
                (bc == J9BCputstatic), NULL, false);
            if (type == TR::Float || type == TR::Double)
                return bcIndex();
            break;
...
void TR_J9ByteCodeIterator::printByteCodePrologue()
{
...
void TR_J9ByteCodeIterator::printByteCodeEpilogue()
{
    trfprintf(comp()->getOutFile(), "\n\n");
    comp()->getDebug()->printByteCodeAnnotations();
}

void TR_J9ByteCodeIterator::printFirst(int32_t i) { trfprintf(comp()->getOutFile(), "%5i", i); }

void TR_J9ByteCodeIterator::printCPIndex(int32_t i) { trfprintf(comp()->getOutFile(), "%13s%5i", "", i); }

void TR_J9ByteCodeIterator::printConstant(int32_t i) { trfprintf(comp()->getOutFile(), "%11s%12i  ", "", i); }

void TR_J9ByteCodeIterator::printConstant(double d) { trfprintf(comp()->getOutFile(), "%11s%12e  ", "", d); }

void TR_J9ByteCodeIterator::printFirstAndConstant(int32_t i, int32_t j)
{
    trfprintf(comp()->getOutFile(), "%5i%6s%12i  ", i, "", j);
}

void TR_J9ByteCodeIterator::printJumpIndex(int32_t offset)
{
    trfprintf(comp()->getOutFile(), "%5i,%5d,%11s ", offset, offset + bcIndex(), "");
}

void TR_J9ByteCodeIterator::printByteCode()
{
