...
protected:
        ...
        TR::TreeTop* tt2) // TR::Block *restartBlock)
    {
        ...
        for (int32_t i = 0; i < callNode->getNumChildren(); i++) {
            ...
            if (child->getSymbolReference() == symRef) {
                ...
                // printf("patchPartialInliningCallBack:  setting Child %d %p symRef %p to tempRef
    ...
    void addTempForPartialInliningCallBack(int32_t slot, TR::SymbolReference* tempRef, int32_t numParms)
    {
        ...
        // printf("addTempForPartialInliningCallBack : _replacedSYmRefs = %p _replacedSymRefs->isEmpty() =
    ...
    TR::TreeTop* genPartialInliningCallBack(int32_t index, TR::TreeTop* callTreeTop)
    {
        ...
        for (si = li.getFirst(), i = 0; si != NULL; si = li.getNext(), i++) {
            ...
            //         si->getDataType(),_compilation->il.opCodeForDirectLoad(si->getDataType()),i,_compilation->getSymRefTab()
            //         );
            ...
            if (callNode->getOpCode().isIndirect() && i == 0) // need to generate vft offset & call
            {
                ...
                callNode->setAndIncChild(0,
                    TR::Node::createWithSymRef(
                        TR::aloadi, 1, 1, thisPointer, _compilation->getSymRefTab()->findOrCreateVftSymbolRef()));
                i++;
            }
            TR::Node* child
                = TR::Node::createWithSymRef(_compilation->il.opCodeForDirectLoad(si->getDataType()), 0, ref);
            callNode->setAndIncChild(
                i, child); // setChild doesn't modify the ref count.  setAndIncChild() will properly inc the ref count.
        }
