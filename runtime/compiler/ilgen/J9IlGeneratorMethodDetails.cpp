...
TR::IlGeneratorMethodDetails* IlGeneratorMethodDetails::clone(
    TR::IlGeneratorMethodDetails& storage, const TR::IlGeneratorMethodDetails& other)
{
    ...
    TR::IlGeneratorMethodDetails& target, TR_ResolvedMethod* method)
{
    ...
    if (j9method->isNewInstanceImplThunk())
        return *new (&target) NewInstanceThunkDetails(
            (J9Method*)j9method->getNonPersistentIdentifier(), (J9Class*)j9method->classOfMethod());

    else if (j9method->convertToMethod()->isArchetypeSpecimen()) {
        if (j9method->getMethodHandleLocation())
            return *new (&target) CustomInvokeExactThunkDetails(
                (J9Method*)j9method->getNonPersistentIdentifier(), j9method->getMethodHandleLocation(), NULL);
        else
...
void IlGeneratorMethodDetails::printDetails(TR_FrontEnd* fe, TR::FILE* file)
{
...
void NewInstanceThunkDetails::printDetails(TR_FrontEnd* fe, TR::FILE* file)
{
    int32_t len;
...
void MethodInProgressDetails::printDetails(TR_FrontEnd* fe, TR::FILE* file)
{
    ...
    TR_InlineBlocks* blocksToInline)
{
    ...
    return new (comp->trHeapMemory()) TR_J9ByteCodeIlGenerator(*this, methodSymbol, static_cast<TR_J9VMBase*>(fe), comp,
        symRefTab, forceClassLookahead, blocksToInline, argPlaceholderSlot);
}

void MethodHandleThunkDetails::printDetails(TR_FrontEnd* fe, TR::FILE* file)
{
#if 0
