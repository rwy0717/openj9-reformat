...
TR::Node* TR_J9ByteCodeIlGenerator::handleVectorIntrinsicCall(TR::Node* node, TR::MethodSymbol* symbol)
{
    TR_ASSERT(TR::firstVectorIntrinsic <= symbol->getRecognizedMethod()
            && symbol->getRecognizedMethod() <= TR::lastVectorIntrinsic,
        "assertion failure");
...
TR::Node* TR_J9ByteCodeIlGenerator::genVectorUnaryOp(
    TR::Node* node, TR::ILOpCodes op1, TR::DataType dt1, TR::ILOpCodes op2, TR::DataType dt2)
{
...
TR::Node* TR_J9ByteCodeIlGenerator::genVectorAddress(TR::Node* node, TR::ILOpCodes op)
{
    if (!comp()->getOption(TR_EnableSIMDLibrary))
        return NULL;
...
TR::Node* TR_J9ByteCodeIlGenerator::genVectorBinaryOpNoTrg(TR::Node* node, TR::ILOpCodes op)
{
    if (comp()->getOption(TR_TraceILGen))
        traceMsg(comp(), "Vector intrinsic method call recognized in %p\n", node);
...
TR::Node* TR_J9ByteCodeIlGenerator::genVectorBinaryOp(
    TR::Node* node, TR::ILOpCodes op1, TR::DataType dt1, TR::ILOpCodes op2, TR::DataType dt2)
{
    if (comp()->getOption(TR_TraceILGen))
        traceMsg(comp(), "Vector intrinsic method call recognized in %p\n", node);
    ...
    if (comp()->getOption(TR_EnableSIMDLibrary)) {
        ...
        TR_ASSERT((valueNode->getDataType() == vectorType) || valueNode->getOpCode().isBooleanCompare(),
            "Data Type mismatching %s\n", valueNode->getDataType().toString());
...
TR::Node* TR_J9ByteCodeIlGenerator::genVectorTernaryOp(TR::Node* node, TR::ILOpCodes op)
{
    if (comp()->getOption(TR_TraceILGen))
        traceMsg(comp(), "Vector intrinsic method call recognized in %p\n", node);
...
TR::Node* TR_J9ByteCodeIlGenerator::genVectorSplatsOp(
    TR::Node* node, TR::ILOpCodes op, TR::DataType vectorType, TR::DataType scalarType)
{
    if (comp()->getOption(TR_TraceILGen))
        traceMsg(comp(), "Vector intrinsic method call recognized in %p\n", node);
    ...
    if (comp()->getOption(TR_EnableSIMDLibrary)) {
        ...
        if (value->getDataType() != scalarType)
            value = TR::Node::create(
                TR::ILOpCode::getProperConversion(value->getDataType(), scalarType, false), 1, value);
...
TR::Node* TR_J9ByteCodeIlGenerator::genVectorStore(TR::Node* node, TR::DataType dt, TR::DataType dt2)
{
    if (comp()->getOption(TR_TraceILGen))
        traceMsg(comp(), "Vector intrinsic method call recognized in %p\n", node);
...
TR::Node* TR_J9ByteCodeIlGenerator::genVectorGetElementOp(TR::Node* node, TR::ILOpCodes op)
{
    if (comp()->getOption(TR_TraceILGen))
        traceMsg(comp(), "Vector intrinsic method call recognized in %p\n", node);
...
TR::Node* TR_J9ByteCodeIlGenerator::genVectorSetElementOp(TR::Node* node, TR::ILOpCodes op)
{
    if (comp()->getOption(TR_TraceILGen))
        traceMsg(comp(), "Vector intrinsic method call recognized in %p\n", node);
...
TR::Node* TR_J9ByteCodeIlGenerator::genVectorAddReduceDouble(TR::Node* node)
{
    if (comp()->getOption(TR_TraceILGen))
        traceMsg(comp(), "Vector intrinsic method call recognized in %p\n", node);
...
TR::Node* TR_J9ByteCodeIlGenerator::genVectorLoadWithStrideDouble(TR::Node* node)
{
    if (comp()->getOption(TR_TraceILGen))
        traceMsg(comp(), "Vector intrinsic method call recognized in %p\n", node);
...
static int32_t getCPIndexForVectorElement(TR::Compilation* comp, TR::ResolvedMethodSymbol* method)
{
    ...
    while (true) {
        ...
        if (len == 8 && 0 == strncmp(fieldName, "element0", 8))
            break;
        cp++;
    }
    return cp;
            ...
            if (!nodesAlreadyPrinted.isSet(node->getGlobalIndex())) {
                for (int j = 0; j < node->getNumChildren(); ++j) {
                    ...
                    comp->getDebug()->printWithFixedPrefix(
                        comp->getOutFile(), node->getChild(j), 3, true, true, "      ");
                }
            }
        }
        traceMsg(comp, "\n");
...
TR::Block* TR_J9ByteCodeIlGenerator::walker(TR::Block* prevBlock)
{
    ...
    while (_bcIndex < _maxByteCodeIndex) {
        ...
        case J9BCbaload:
            ...
            break;
        case J9BCcaload:
            loadArrayElement(TR::Int16, TR::cloadi);
            genUnary(TR::su2i);
            _bcIndex += 1;
            break;
        case J9BCsaload:
        ...
        case J9BCcastore:
            genUnary(TR::i2s);
            storeArrayElement(TR::Int16, TR::cstorei);
            _bcIndex += 1;
            break;
        case J9BCsastore:
            genUnary(TR::i2s);
        ...
        case J9BCificmple:
            ...
            break;
        case J9BCifacmpeq:
            _bcIndex = genIfTwoOperand(TR::ifacmpeq);
            break;
        case J9BCifacmpne:
            _bcIndex = genIfTwoOperand(TR::ifacmpne);
            break;
        ...
        if (!nextBlock && lastRealNode->getOpCode().isIf()) {
            ...
            if (_blocksToInline) {
                if (!blocks(i + 3)) {
                    ...
                    nextBlock->append(TR::TreeTop::create(
                        comp(), TR::Node::create(lastRealNode, TR::Goto, 0, blocks(i + 3)->getEntry())));
                }
            } else {
...
TR::SymbolReference* TR_J9ByteCodeIlGenerator::placeholderWithSignature(
    char* prefix, int prefixLength, char* middle, int middleLength, char* suffix, int suffixLength)
{
    return symRefWithArtificialSignature(
        placeholderWithDummySignature(), ".#.#.#", prefix, prefixLength, middle, middleLength, suffix, suffixLength);
}

TR::SymbolReference* TR_J9ByteCodeIlGenerator::symRefWithArtificialSignature(
    TR::SymbolReference* original, char* effectiveSigFormat, ...)
{
    TR::StackMemoryRegion stackMemoryRegion(*comp()->trMemory());
...
TR::SymbolReference* TR_J9ByteCodeIlGenerator::expandPlaceholderSignature(
    TR::SymbolReference* symRef, int32_t numArgs, int32_t firstArgStackDepth)
{
...
TR::Node* TR_J9ByteCodeIlGenerator::genNodeAndPopChildren(
    TR::ILOpCodes opcode, int32_t numChildren, TR::SymbolReference* symRef, int32_t firstIndex, int32_t lastIndex)
{
...
TR::Node* TR_J9ByteCodeIlGenerator::genNodeAndPopChildren(
    TR::ILOpCodes opcode, int32_t numChildren, TR::SymbolReference* symRef, int32_t firstIndex)
{
...
TR::TreeTop* TR_J9ByteCodeIlGenerator::genTreeTop(TR::Node* n)
{
    ...
    bool isExceptionOnlyPoint
        = comp()->getOSRMode() == TR::involuntaryOSR && !n->canGCandReturn() && n->canGCandExcept();
...
void TR_J9ByteCodeIlGenerator::saveStack(int32_t targetIndex, bool anchorLoads)
{
    ...
    for (i = 0; i < _stack->size(); ++i) {
        ...
        if (!isPlaceholderCall(n)) {
            ...
            // all conditions
            TR_ASSERT_FATAL(
                !createTargetStack, "Cannot store and later load placeholder calls in current implementation");
        }
    }
}
...
void TR_J9ByteCodeIlGenerator::handlePendingPushSaveSideEffects(
    TR::Node* n, TR::NodeChecklist& checklist, int32_t stackSize)
{
...
void TR_J9ByteCodeIlGenerator::stashArgumentsForOSR(TR_J9ByteCode byteCode)
{
    ...
    case J9BCinvokehandlegeneric:
        ...
        break;
    case J9BCinvokestaticsplit:
        symRef = symRefTab()->findOrCreateStaticMethodSymbol(
            _methodSymbol, next2Bytes() | J9_STATIC_SPLIT_TABLE_INDEX_FLAG);
        break;
    case J9BCinvokespecialsplit:
        symRef = symRefTab()->findOrCreateSpecialMethodSymbol(
            _methodSymbol, next2Bytes() | J9_SPECIAL_SPLIT_TABLE_INDEX_FLAG);
        break;
    default:
        return;
    }
...
TR::Node* TR_J9ByteCodeIlGenerator::loadConstantValueIfPossible(
    TR::Node* topNode, uintptrj_t topFieldOffset, TR::DataType type, bool isArrayLength)
{
    ...
    if (topNode->getOpCode().hasSymbolReference()) {
        ...
        if (symRef->getSymbol()->isShadow() && symRef->getSymbol()->isFinal() && !symRef->isUnresolved()) {
    ...
    if (symRef && symRef->getSymbol()->isStatic() && !symRef->isUnresolved() && symRef->getSymbol()->isFinal()
        && !symRef->getSymbol()->isConstObjectRef() && _method->isSameMethod(symRef->getOwningMethod(comp()))) {
        ...
        TR_OpaqueClassBlock* classOfStatic
            = isResolved ? _method->classOfStatic(topNode->getSymbolReference()->getCPIndex()) : 0;
        if (classOfStatic == NULL) {
            ...
            if (classNameOfFieldOrStatic) {
                ...
                TR_OpaqueClassBlock* curClass
                    = fej9->getClassFromSignature(classNameOfFieldOrStatic, len, symRef->getOwningMethod(comp()));
        ...
        TR_PersistentClassInfo* classInfo = _noLookahead
            ? 0
        ...
        if (isResolved && symbol->isFinal() && !symRef->isUnresolved()
            && classOfStatic != comp()->getSystemClassPointer() && isClassInitialized) {
        ...
        if (canOptimizeFinalStatic) {
            TR::VMAccessCriticalSection loadConstantValueCriticalSection(
                fej9, TR::VMAccessCriticalSection::tryToAcquireVMAccess, comp());

            if (loadConstantValueCriticalSection.hasVMAccess()) {
                uintptrj_t objectPointer
                    = comp()->fej9()->getStaticReferenceFieldAtAddress((uintptrj_t)symbol->getStaticAddress());
                if (objectPointer) {
                    switch (symbol->getDataType()) {
                    case TR::Address: {
                        if (parent != topNode)
                            objectPointer = fej9->getReferenceFieldAt(objectPointer, fieldOffset);
                        if ((type == TR::Int32) || (type == TR::Int16) || (type == TR::Int8)) {
                            int32_t val;
                            if (isArrayLength)
...
// gen array
//----------------------------------------------
void TR_J9ByteCodeIlGenerator::genArrayLength()
{
    TR::Node* node = NULL;
    TR::Node* topNode = pop();
...
void TR_J9ByteCodeIlGenerator::genContiguousArrayLength(int32_t width)
{
    TR::Node* node = NULL;
    TR::Node* topNode = pop();
...
void TR_J9ByteCodeIlGenerator::genDivCheck()
{
    if (!_methodSymbol->skipDivChecks()) {
        ...
        genTreeTop(TR::Node::createWithSymRef(
            TR::DIVCHK, 1, 1, _stack->top(), symRefTab()->findOrCreateDivCheckSymbolRef(_methodSymbol)));
    }
}
        ...
        if (!c && grandChild->getOpCode().hasSymbolReference() && grandChild->getSymbolReference()
            ...
            if (trace())
                traceMsg(comp(), "Skipping NULLCHK (node %p) on String.value field : %s -> %s\n", grandChild,
                    comp()->signature(), _methodSymbol->signature(trMemory()));
        } else {
            return TR::Node::createWithSymRef(
                TR::NULLCHK, 1, 1, first, symRefTab()->findOrCreateNullCheckSymbolRef(_methodSymbol));
        }
    }
...
{
    return TR::Node::createWithSymRef(
        TR::ResolveCHK, 1, 1, first, symRefTab()->findOrCreateResolveCheckSymbolRef(_methodSymbol));
}
...
TR::Node* TR_J9ByteCodeIlGenerator::genResolveAndNullCheck(TR::Node* first)
{
    ...
    TR::Node* grandChild = first->getChild(0);
    return TR::Node::createWithSymRef(
        TR::ResolveAndNULLCHK, 1, 1, first, symRefTab()->findOrCreateNullCheckSymbolRef(_methodSymbol));
}
...
int32_t TR_J9ByteCodeIlGenerator::genGoto(int32_t target)
{
    ...
    if (_blocksToInline) // partial inlining - only generate goto if its in the list of blocks.
    {
        ...
        TR_ASSERT(
            success, " Walker: genGoto for a partial inline has no target in the list of blocks to be inlined.\n");
    } else {
...
int32_t TR_J9ByteCodeIlGenerator::genIfImpl(TR::ILOpCodes nodeop)
{
    ...
    if (_blocksToInline) {
        ...
        TR_InlineBlock* aBlock = NULL;
        for (aBlock = blocksIt.getCurrent(); aBlock; aBlock = blocksIt.getNext()) {
            //      printf("\tBlock bcIndex = %d owningMethod = %p depth =
        ...
        if (genFallThru && genBranchBC) {
            ...
            if (swapChildren(nodeop, first)) {
                TR::TreeTop* tt = genTreeTop(TR::Node::createif(
                    TR::ILOpCode(nodeop).getOpCodeForSwapChildren(), second, first, branchDestination));
            ...
            if (genFallThru) {
                ...
                //            printf("Walker: genIf : fallThru : getCallNodeTreeTop = %p, symreftab =
                ...
                branchDestination = _blocksToInline->hasGeneratedRestartTree()
                    ...
                    : _blocksToInline->setGeneratedRestartTree(genPartialInliningCallBack(
                          branchBC, _blocksToInline->getCallNodeTreeTop())); // not adding it to the queue
            ...
            if (genBranchBC) {
                // printf("Walker: genIf : BranchBC : hasGeneratedRestartTree = %d getGeneratedRestartTree = %p
                // getEnclosingBlock =
                // %p\n",_blocksToInline->hasGeneratedRestartTree(),_blocksToInline->getGeneratedRestartTree(),
                // _blocksToInline->getGeneratedRestartTree() ?
                // _blocksToInline->getGeneratedRestartTree()->getEnclosingBlock() : 0);
                //            fflush(stdout);
...
void TR_J9ByteCodeIlGenerator::genInvokeSpecial(int32_t cpIndex)
{
    ...
    if (!_invokeSpecialSeen) {
        ...
        J9Class* const textualDefClass = TR::Compiler->cls.convertClassOffsetToClassPtr(_method->containingClass());
        ...
        TR_OpaqueClassBlock* const defClass = fej9()->convertClassPtrToClassOffset(textualDefClass->hostClass);
        ...
        if (trace) {
            ...
            traceMsg(comp(), "within interface %p %.*s for the purpose of invokespecial\n", _invokeSpecialInterface,
                len, name);
        }
    }
        ...
        if (isOutermostMethod()) {
            TR::DebugCounter::incStaticDebugCounter(comp(),
                TR::DebugCounter::debugCounterName(
                    comp(), "ilgen.abort/aot-invokespecial-interface/root/(%s)/bc=%d", comp()->signature(), bcIndex));
        } else {
            TR::DebugCounter::incStaticDebugCounter(comp(),
...
void TR_J9ByteCodeIlGenerator::genInvokeVirtual(int32_t cpIndex)
{
    ...
    if (method != NULL) {
        isDirectCall = sym->isFinal() || method->isPrivate()
    ...
    if (improperMethod == NULL) {
        ...
        if (improperMethod->isPrivate() || improperMethod->convertToMethod()->isFinalInObject()) {
            ...
            TR::SymbolReference* symRef = symRefTab()->findOrCreateMethodSymbol(_methodSymbol->getResolvedMethodIndex(),
                ...
                improperMethod, TR::MethodSymbol::Virtual,
                /* isUnresolvedInCP = */ false);

            symRef->setCPIndex(cpIndex);
            TR_OpaqueMethodBlock* m = improperMethod->getPersistentIdentifier();
            symRef->setOffset(TR::Compiler->cls.vTableSlot(comp(), m, fej9()->getClassOfMethod(m)));
...
void TR_J9ByteCodeIlGenerator::genInvokeDynamic(int32_t callSiteIndex)
{
    ...
    if (!owningMethod->isUnresolvedCallSiteTableEntry(callSiteIndex)) {
        TR_ResolvedMethod* specimen = fej9()->createMethodHandleArchetypeSpecimen(
            trMemory(), (uintptrj_t*)owningMethod->callSiteTableEntryAddress(callSiteIndex), owningMethod);
        if (specimen)
            symRef = symRefTab()->findOrCreateMethodSymbol(
                _methodSymbol->getResolvedMethodIndex(), -1, specimen, TR::MethodSymbol::ComputedVirtual);
    }
...
TR::Node* TR_J9ByteCodeIlGenerator::genInvokeHandle(
    TR::SymbolReference* invokeExactSymRef, TR::Node* invokedynamicReceiver)
{
    if (comp()->getOption(TR_TraceILGen))
        ...
        if (!comp()->getHasMethodHandleInvoke()) {
            ...
            if (comp()->getOptions()->getVerboseOption(TR_VerboseMethodHandles))
                TR_VerboseLog::writeLineLocked(
                    TR_Vlog_MH, "Jitted method contains MethodHandle invoke: %s", comp()->signature());
        }
...
void TR_J9ByteCodeIlGenerator::genHandleTypeCheck()
{
    TR::Node* expectedType = pop();
    TR::SymbolReference* getTypeSymRef
        = comp()->getSymRefTab()->methodSymRefFromName(_methodSymbol, JSR292_MethodHandle, JSR292_getType,
            JSR292_getTypeSig, TR::MethodSymbol::Special); // TODO:JSR292: Too bad I can't do a more general lookup and
                                                           // let it optimize itself.  Virtual call doesn't seem to work
    genInvokeDirect(getTypeSymRef);
    TR::Node* handleType = pop();

    genTreeTop(TR::Node::createWithSymRef(TR::ZEROCHK, 1, 1, TR::Node::create(TR::acmpeq, 2, expectedType, handleType),
...
TR::Node* TR_J9ByteCodeIlGenerator::genInvoke(
    TR::SymbolReference* symRef, TR::Node* indirectCallFirstChild, TR::Node* invokedynamicReceiver)
{
    TR::MethodSymbol* symbol = symRef->getSymbol()->castToMethodSymbol();
    ...
    if (opcode != TR::BadILOp) {
        performTransformation(
            comp(), "O^O BIT OPCODE: convert call to method %s to bit opcode\n", calledMethod->signature(trMemory()));
    ...
    if (symbol->getRecognizedMethod() == TR::com_ibm_Compiler_Internal__TR_Prefetch) {
        ...
        if (2 == numArgs && n2->getOpCode().isInt()) {
            ...
            if (n3->getSymbolReference() && n3->getSymbolReference()->getSymbol()->isStatic()
                && n3->getSymbolReference()->getSymbol()->castToStaticSymbol()->isConstString()
                && n2->getSymbolReference() && n2->getSymbolReference()->getSymbol()->isStatic()
    ...
    /* Strategy on DFP HW is as follows:
       ...
       1        0             EnableDFP+DisableHys   1              0                 1               (enableDFP,
       disableHys) 1        1             EnableDFP+EnableHys    1              1                 n/a (enableDFP,
       enableHys)
...
#define DAA_PRINT(a)                                    \
    ...
    if (symbol->getRecognizedMethod() == TR::com_ibm_dataaccess_DecimalData_JITIntrinsicsEnabled) {
        ...
        int32_t constVal
            = (TR::Compiler->target.isZOS() || isZLinux) && !comp()->getOption(TR_DisablePackedDecimalIntrinsics) ? 1
                                                                                                                  : 0;
    ...
    if (!comp()->compileRelocatableCode()) {
        ...
        if (isPOWERDFP || is390DFP) {
            if (((symbol->getRecognizedMethod() == TR::java_math_BigDecimal_DFPHWAvailable)
                    ...
                    || // disable for DAA DFP
                    (!strncmp(
                        calledMethod->signature(comp()->trMemory(), stackAlloc), BDDFPHWAVAIL, BDDFPHWAVAILLEN)))) {
                int32_t* dfpHWAvailableFlag = NULL;
                if (!fej9()->isCachedBigDecimalClassFieldAddr()) {
                    TR_OpaqueClassBlock* classBlock
                        = fej9()->getClassFromSignature("java/math/BigDecimal", 20, comp()->getCurrentMethod());
                    ...
                    TR_ASSERT(NULL != classBlock,
                        "java/lang/BigDecimal is not loaded before com/ibm/dataaccess/DecimalData.");
                ...
                || (!strncmp(
                       calledMethod->signature(comp()->trMemory(), stackAlloc), BDDFPPERFORMHYS, BDDFPPERFORMHYSLEN))) {
                ...
                if (dfpbd && !nodfpbd) {
                    ...
                    TR_ASSERT(
                        NULL != clazz, "java/lang/BigDecimal is not loaded before com/ibm/dataaccess/DecimalData.");
                    ...
                    TR::Node* targetCallNode = genNodeAndPopChildren(callOpCode, 0,
    ...
    if (comp()->cg()->getSupportsInlineConcurrentLinkedQueue()
        && (comp()->getOptions()->getGcMode() != TR_WrtbarRealTime)
    ...
    if (comp()->getOption(TR_EnableSIMDLibrary)) {
        ...
        if (TR::firstVectorIntrinsic <= symbol->getRecognizedMethod()
            && symbol->getRecognizedMethod() <= TR::lastVectorIntrinsic) {
            ...
            if (isClassInitialized) {
                ...
                if (comp()->getOption(TR_TraceILGen))
                    traceMsg(comp(), "orbClass = %p, orbClassLoader %s systemClassLoader\n", orbClass,
                        (fej9()->getClassLoader(cl) != fej9()->getSystemClassLoader()) ? "!=" : "==");
                ...
                if (orbClass && (fej9()->getClassLoader(cl) != fej9()->getSystemClassLoader())) {
                    ...
                    if (replacementMethod) {
                        if (performTransformation(comp(),
                            ...
                            TR::Node* clazzLoad = TR::Node::createWithSymRef(
                                TR::loadaddr, 0, symRefTab()->findOrCreateClassSymbol(_methodSymbol, 0, cl));
        ...
        if (receiver && receiver->getOpCode().hasSymbolReference()) {
            ...
            if (receiverSig != NULL && (receiverLen == JAVA_SERIAL_CLASS_NAME_LEN)
                ...
                if ((calledMethod->nameLength() == JAVA_SERIAL_CALLEE_METHOD_NAME_LEN)
                    ...
                    if (comp()->getOption(TR_TraceILGen))
                        traceMsg(
                            comp(), "called method %s, containing class %p\n", calledMethod->signature(trMemory()), cl);
                    bool isClassInitialized = false;
                    TR_PersistentClassInfo* classInfo
                        = comp()->getPersistentInfo()->getPersistentCHTable()->findClassInfoAfterLocking(cl, comp());
                    if (classInfo && classInfo->isInitialized())
    ...
    if (resolvedMethodSymbol && resolvedMethodSymbol->getRecognizedMethod() == TR::java_lang_String_indexOf_String
        && !isPeekingMethod() && !comp()->getOption(TR_DisableInliningOfNatives)
        ...
        if (TR::Compiler->target.is32Bit()) {
            resultNode = TR::Node::createWithSymRef(
        ...
        TR::Node* j9Class = TR::Node::createWithSymRef(
            callNode, TR::aloadi, 1, jlClass, symRefTab()->findOrCreateClassFromJavaLangClassSymbolRef());

        if (TR::Compiler->target.is32Bit()) {
            resultNode = TR::Node::createWithSymRef(
                callNode, TR::iloadi, 1, j9Class, symRefTab()->findOrCreateInitializeStatusFromClassSymbolRef());
        } else {
            resultNode = TR::Node::createWithSymRef(
                callNode, TR::lloadi, 1, j9Class, symRefTab()->findOrCreateInitializeStatusFromClassSymbolRef());
            resultNode = TR::Node::create(callNode, TR::l2i, 1, resultNode);
        }
...
bool TR_J9ByteCodeIlGenerator::runMacro(TR::SymbolReference* symRef)
{
    ...
    case TR::java_lang_invoke_ILGenMacros_rawNew: {
        if (!comp()->compileRelocatableCode()) {
            push(TR::Node::createWithSymRef(
                TR::aloadi, 1, 1, pop(), symRefTab()->findOrCreateClassFromJavaLangClassSymbolRef()));
    ...
    case TR::java_lang_invoke_ILGenMacros_arrayElements: {
        if (!comp()->compileRelocatableCode()) {
            ...
            char* expandedArgsSignature
                = (char*)comp()->trMemory()->allocateStackMemory(numElements * arrayElementTypeLength + 1);
...
void TR_J9ByteCodeIlGenerator::loadInstance(int32_t cpIndex)
{
    ...
    if (!symRef->isUnresolved() && symRef->getSymbol()->isFinal()) {
    ...
    if (!comp()->compileRelocatableCode() && !comp()->getOption(TR_DisableDFP)
        ...
        if (className != NULL && BDCLASSLEN == strlen(className) && !strncmp(className, BDCLASS, BDCLASSLEN)) {
            int32_t fieldLen = 0;
...
void TR_J9ByteCodeIlGenerator::loadStatic(int32_t cpIndex)
{
    ...
    if (comp()->getOption(TR_TraceILGen))
        traceMsg(comp(), "load static symref %d created with knownObjectIndex %d", symRef->getReferenceNumber(),
    ...
    if (classOfStatic)
       ...
       printf("class name %s class init %d class %p classInfo %p no %d hotness %d\n", name, isClassInitialized,
    classOfStatic, classInfo, _noLookahead, comp()->getMethodHotness()); fflush(stdout);
       }
    */

    bool canOptimizeFinalStatic = false;
    if (isResolved && symbol->isFinal() && !symRef->isUnresolved() && classOfStatic != comp()->getSystemClassPointer()
        && isClassInitialized) {
        // if (type == TR::Address)
        {
    ...
    TR::VMAccessCriticalSection loadStaticCriticalSection(
        fej9, TR::VMAccessCriticalSection::tryToAcquireVMAccess, comp());

    if (canOptimizeFinalStatic && loadStaticCriticalSection.hasVMAccess()) {
        ...
        case TR::Double:
            loadConstant(TR::dconst, *(double*)p);
            break;
        case TR::Int64:
            loadConstant(TR::lconst, *(int64_t*)p);
            break;
        case TR::Float:
            loadConstant(TR::fconst, *(float*)p);
            break;
        case TR::Int32:
        default:
            loadConstant(TR::iconst, *(int32_t*)p);
            break;
        }
        ...
        } else if (cg()->getAccessStaticsIndirectly() && isResolved && type != TR::Address
            && (!comp()->compileRelocatableCode() || comp()->getOption(TR_UseSymbolValidationManager))) {
            TR::Node* statics = TR::Node::createWithSymRef(
                TR::loadaddr, 0, symRefTab()->findOrCreateClassStaticsSymbol(_methodSymbol, cpIndex));
 ...
 */
TR::SymbolReference* TR_J9ByteCodeIlGenerator::loadClassObjectForTypeTest(
    int32_t cpIndex, TR_CompilationOptions aotInhibit)
{
...
void TR_J9ByteCodeIlGenerator::loadClassObjectAndIndirect(int32_t cpIndex)
{
    ...
    node = TR::Node::createWithSymRef(
        TR::aloadi, 1, 1, node, symRefTab()->findOrCreateJavaLangClassFromClassSymbolRef());
    push(node);
}
        ...
        if (method()->isConstantDynamic(cpIndex)) {
            ...
            if (isCondyPrimitive) {
                ...
                case 'I':
                    ...
                    typeClassBlock
                        = comp()->fej9()->getClassFromSignature(autoboxClassSig, autoboxClassSigLength, method());
                ...
                if (!isCondyUnresolved) {
                    TR::VMAccessCriticalSection primitiveCondyCriticalSection(
                        comp()->fej9(), TR::VMAccessCriticalSection::tryToAcquireVMAccess, comp());
                    if (primitiveCondyCriticalSection.hasVMAccess()) {
                        uintptrj_t* objLocation
                            = (uintptrj_t*)_methodSymbol->getResolvedMethod()->dynamicConstant(cpIndex);
                        uintptrj_t obj = *objLocation;
                        TR_ASSERT(obj, "Resolved primitive Constant Dynamic-type CP entry %d must have autobox object",
                            cpIndex);
                        switch (returnTypeUtf8Data[0]) {
                        case 'I':
                        case 'Z':
            ...
            TR::Node* loadObjNode = loadSymbol(TR::aload,
                symRefTab()->findOrCreateConstantDynamicSymbol(
                    _methodSymbol, cpIndex, symbolTypeSig, symbolTypeSigLength, isCondyPrimitive));
...
void TR_J9ByteCodeIlGenerator::genNewArray(int32_t typeIndex)
{
    ...
    TR::Node* node = TR::Node::createWithSymRef(
        TR::newarray, 2, 2, firstChild, secondChild, symRefTab()->findOrCreateNewArraySymbolRef(_methodSymbol));
...
void TR_J9ByteCodeIlGenerator::genANewArray()
{
    ...
    TR::Node* firstChild = pop();
    TR::Node* node = TR::Node::createWithSymRef(
        TR::anewarray, 2, 2, firstChild, secondChild, symRefTab()->findOrCreateANewArraySymbolRef(_methodSymbol));
    _methodSymbol->setHasNews(true);
    genTreeTop(node);
    push(node);
    genFlush(0);
...
void TR_J9ByteCodeIlGenerator::genMultiANewArray(int32_t dims)
{
    // total number of params is 2+#dims
    //
    TR::Node* node = genNodeAndPopChildren(
        TR::multianewarray, dims + 2, symRefTab()->findOrCreateMultiANewArraySymbolRef(_methodSymbol), 1);
...
int32_t TR_J9ByteCodeIlGenerator::genReturn(TR::ILOpCodes nodeop, bool monitorExit)
{
    ...
    if ((fej9()->isMethodExitTracingEnabled(_methodSymbol->getResolvedMethod()->getPersistentIdentifier())
        ...
        if (comp()->getOption(TR_OldJVMPI))
            ...
            methodExitNode = TR::Node::createWithSymRef(
                TR::MethodExitHook, 1, 1, TR::Node::createWithSymRef(TR::loadaddr, 0, tempSymRef), methodExitSymRef);
        }
...
void TR_J9ByteCodeIlGenerator::storeInstance(int32_t cpIndex)
{
    ...
    if ((type == TR::Address && _generateWriteBarriersForGC) || _generateWriteBarriersForFieldWatch) {
        node = TR::Node::createWithSymRef(
            comp()->il.opCodeForIndirectWriteBarrier(type), 3, 3, addressNode, value, parentObject, symRef);
    } else {
    ...
    if (symbol->isPrivate() && _classInfo && comp()->getNeedsClassLookahead()) {
        ...
        if (storeCanBeRemovedForUnreadField(fieldInfo, value)
            && performTransformation(comp(),
                   "O^O CLASS LOOKAHEAD: Can skip store to instance field (that is never read) storing value %p based "
...
void TR_J9ByteCodeIlGenerator::storeStatic(int32_t cpIndex)
{
    ...
    } else if (!symRef->isUnresolved() && cg()->getAccessStaticsIndirectly() && type != TR::Address
        && (!comp()->compileRelocatableCode() || comp()->getOption(TR_UseSymbolValidationManager))) {
        TR::Node* statics = TR::Node::createWithSymRef(
            TR::loadaddr, 0, symRefTab()->findOrCreateClassStaticsSymbol(_methodSymbol, cpIndex));
...
{
    TR_ASSERT(storeValue->isDualHigh() || storeValue->isTernaryHigh(),
        "Coerced types only happen when a dual or ternary operator is generated.");
...
void TR_J9ByteCodeIlGenerator::storeAuto(TR::DataType type, int32_t slot, bool isAdjunct)
{
    ...
    if (_blocksToInline) {
        if (slot < numParmSlots) {
            // printf("Walker: (partial)storeAuto: storing into a Parameter. numParmSlots = %d isStatic = %d slot =
            // %d\n",numParmSlots,isStatic,slot); Need to create a temporary and use it in the callback
...
{
    int32_t i = 1;
    while ((intptrj_t)&_code[_bcIndex + i] & 3)
        ++i; // 4 byte align
