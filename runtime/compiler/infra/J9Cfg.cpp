            ...
            for (edge = pit.getFirst(); edge; edge = pit.getNext()) {
                ...
                if (lastTree && lastTree->getNode()->getOpCode().isIf()) {
                    ...
                    if (fallThrough && branchTarget) {
                        if (comp()->getOption(TR_TraceBFGeneration))
                            traceMsg(comp(),
                                "      checking effective singleton on block_%d with fallthrough block_%d and "
                                "branchTarget block_%d\n",
                ...
                if (((pred->getSuccessors().size() == 1) && pred->hasSuccessor(node)) || effectiveSingleton) {
                    ...
                    if (predRawFrequency > 0) {
                        ...
                        if (comp()->getOption(TR_TraceBFGeneration))
                            traceMsg(comp(),
                                "       combinedPredRawFrequency is now %d based on succCount %d and predRawFrequency "
                                "%d\n",
                ...
                for (auto e = node->getSuccessors().begin(); e != node->getSuccessors().end(); ++e) {
                    // if (e->getFrequency() <= 0)
                    {
                        ...
                        if ((*e)->getVisitCount() > 1) {
                            if (comp()->getOption(TR_TraceBFGeneration))
                                traceMsg(
                                    comp(), "\t\tedge frequency = %d is final, don't change\n", (*e)->getFrequency());
            ...
            if (_externalProfiler && (node->getSuccessors().size() == 2)) {
                ...
                if (frequencyIsIdentical) {
                    ...
                    if (!block->isCold() && block->getEntry() != NULL
                        ...
                        _externalProfiler->getBranchCounters(
                            ifNode, block->getNextBlock()->getEntry(), &branchToCount, &fallThroughCount, comp());
...
void J9::CFG::setBlockFrequenciesBasedOnInterpreterProfiler()
{
    ...
    while ((temp->getSuccessors().size() == 1) || !temp->asBlock()->getEntry()
        ...
        if (temp->asBlock()->getEntry() && initialCallScanFreq < 0) {
            int32_t methodScanFreq
                = scanForFrequencyOnSimpleMethod(temp->asBlock()->getEntry(), temp->asBlock()->getExit());
            if (methodScanFreq > 0)
    ...
    for (temp = upit.getFirst(); temp; temp = upit.getNext()) {
        ...
        if (comp()->getOption(TR_TraceBFGeneration))
            dumpOptDetails(
                comp(), "Set frequency of %d on block_%d\n", temp->asBlock()->getFrequency(), temp->getNumber());
    }
...
static int32_t getParentCallCount(TR::CFG* cfg, TR::Node* node)
{
    ...
    } else { // It'ss a dummy block in estimate code size
...
void J9::CFG::getInterpreterProfilerBranchCountersOnDoubleton(TR::CFGNode* cfgNode, int32_t* taken, int32_t* nottaken)
{
    ...
    if (*taken || *nottaken) {
        if (comp()->getOption(TR_TraceBFGeneration))
            dumpOptDetails(
                comp(), "If on node %p has branch counts: taken=%d, not taken=%d\n", node, *taken, *nottaken);
...
void J9::CFG::setSwitchEdgeFrequenciesOnNode(TR::CFGNode* node, TR::Compilation* comp)
{
    ...
    if (_externalProfiler->isSwitchProfileFlat(treeNode, comp)) {
        ...
        self()->setUniformEdgeFrequenciesOnNode(
            node, _externalProfiler->getFlatSwitchProfileCounts(treeNode, comp), false, comp);
        return;
    }
...
void J9::CFG::setBlockFrequency(TR::CFGNode* node, int32_t frequency, bool addFrequency)
{
    ...
    if (block->isCold()) {
        if (comp()->getOption(TR_TraceBFGeneration)) {
            traceMsg(comp(), "Leaving cold reason %d on block_%d\n", block->getFrequency(), block->getNumber());
        }
        return;
    }
...
int32_t J9::CFG::scanForFrequencyOnSimpleMethod(TR::TreeTop* tt, TR::TreeTop* endTT)
{
    if (comp()->getOption(TR_TraceBFGeneration))
    ...
    for (; tt && tt != endTT; tt = tt->getNextTreeTop()) {
        ...
        if (comp()->getOption(TR_TraceBFGeneration))
            traceMsg(comp(), "Scanning node %p, isBranch = %d, isCall = %d, isVirtualCall =%d\n", node,
                node->getOpCode().isBranch(), node->getOpCode().isCall(), node->getOpCode().isCallIndirect());
