...
TR::MonitorTable* J9::MonitorTable::init(J9PortLibrary* portLib, J9JavaVM* javaVM)
{
    ...
    if (!table->_tableMonitor.init("JIT-MonitorTableMonitor"))
        return 0;
    if (!table->_j9MemoryAllocMonitor.init("JIT-MemoryAllocMonitor"))
        return 0;
    ...
    table->_classUnloadMonitorHolders = (int32_t*)j9mem_allocate_memory(
        sizeof(*(table->_classUnloadMonitorHolders)) * MAX_TOTAL_COMP_THREADS, J9MEM_CATEGORY_JIT);
...
TR::Monitor* J9::MonitorTable::create(char* name)
{
...
bool J9::MonitorTable::isThreadInSafeMonitorState(J9VMThread* vmThread)
{
    ...
    if (_tableMonitor.owned_by_self() || _j9MemoryAllocMonitor.owned_by_self()
        || _j9ScratchMemoryPoolMonitor.owned_by_self() || _classTableMutex.owned_by_self()
