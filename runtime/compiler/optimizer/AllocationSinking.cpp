            ...
            for (TR::TreeTop* useTree = allocTree->getNextTreeTop();
                ...
                if (useNode->getOpCodeValue() == TR::allocationFence && useNode->getAllocation() == allocation) {
                    ...
                    || (useNode->getOpCodeValue() == TR::allocationFence
                           && useNode->getAllocation() == NULL) // found a flush that *might* be protecting this
                                                                // allocation and others; time to give up
                    || (trace()
                           && !performTransformation(comp(),
                                  "O^O ALLOCATION SINKING: Moving allocation %s down past %s\n",
                                  comp()->getDebug()->getName(allocation),
                    ...
                    }
                    {
                        ...
                        TR_ASSERT(
                            nodeNew->getFirstChild(), "Local Opts, expected the first child of a new not to be null");
