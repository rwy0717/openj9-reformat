...
TR_BoolArrayStoreTransformer::TR_BoolArrayStoreTransformer(
    NodeSet* bstoreiUnknownArrayTypeNodes, NodeSet* bstoreiBoolArrayTypeNodes)
...
void TR_BoolArrayStoreTransformer::perform()
{
    ...
    if (!_bstoreiUnknownArrayTypeNodes->empty()) {
        if (_hasByteArrayAutoOrCheckCast
            && _hasBoolArrayAutoOrCheckCast) // only need to iterate CFG if both byte and boolean array exist
                                              ...
                                              // boolean array
            {
                ...
                _bstoreiBoolArrayTypeNodes->insert(
                    _bstoreiUnknownArrayTypeNodes->begin(), _bstoreiUnknownArrayTypeNodes->end());
            } else
 ...
 */
TR_BoolArrayStoreTransformer::TypeInfo* TR_BoolArrayStoreTransformer::processBlock(
    TR::Block* block, TR_BoolArrayStoreTransformer::TypeInfo* blockStartTypeInfo)
{
...
void TR_BoolArrayStoreTransformer::transformUnknownTypeArrayStore()
{
    ...
    for (auto it = _bstoreiUnknownArrayTypeNodes->begin(); it != _bstoreiUnknownArrayTypeNodes->end(); it++) {
        ...
        TR::Node* vft = TR::Node::createWithSymRef(
            TR::aloadi, 1, 1, arrayBaseNode, comp()->getSymRefTab()->findOrCreateVftSymbolRef());
...
void TR_BoolArrayStoreTransformer::transformBoolArrayStoreNodes()
{
    for (auto it = _bstoreiBoolArrayTypeNodes->begin(); it != _bstoreiBoolArrayTypeNodes->end(); it++) {
        TR::Node* node = *it;
        dumpOptDetails(
            comp(), "%s truncate value child of bstorei node n%dn to 1 bit\n", OPT_DETAILS, node->getGlobalIndex());
