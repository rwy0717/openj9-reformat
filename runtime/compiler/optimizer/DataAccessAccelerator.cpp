        ...
        if (callSymbol != NULL) {
            if (!comp()->getOption(TR_DisablePackedDecimalIntrinsics)) {
                ...
                case TR::com_ibm_dataaccess_DecimalData_convertPackedDecimalToInteger_: {
                    result |= generatePD2IVariableParameter(treeTop, callNode, true, false);
                    continue;
                }
                case TR::com_ibm_dataaccess_DecimalData_convertPackedDecimalToInteger_ByteBuffer_: {
                    result |= generatePD2IVariableParameter(treeTop, callNode, true, true);
                    continue;
                }

                    // DAA Packed Decimal <-> Long
                case TR::com_ibm_dataaccess_DecimalData_convertPackedDecimalToLong_: {
                    result |= generatePD2IVariableParameter(treeTop, callNode, false, false);
                    continue;
                }
                case TR::com_ibm_dataaccess_DecimalData_convertPackedDecimalToLong_ByteBuffer_: {
                    result |= generatePD2IVariableParameter(treeTop, callNode, false, true);
                    continue;
                }
                default:
                    break;
            ...
            if (callSymbol != NULL) {
                ...
                if (!matched && (isZOS || isZLinux) && !block->isCold()
                    ...
                    case TR::com_ibm_dataaccess_DecimalData_convertIntegerToPackedDecimal_:
                        result |= generateI2PD(treeTop, callNode, true, false);
                        break;
                    case TR::com_ibm_dataaccess_DecimalData_convertIntegerToPackedDecimal_ByteBuffer_:
                        result |= generateI2PD(treeTop, callNode, true, true);
                        break;
                    ...
                    case TR::com_ibm_dataaccess_DecimalData_convertLongToPackedDecimal_:
                        result |= generateI2PD(treeTop, callNode, false, false);
                        break;
                    case TR::com_ibm_dataaccess_DecimalData_convertLongToPackedDecimal_ByteBuffer_:
                        result |= generateI2PD(treeTop, callNode, false, true);
                        break;
...
TR::Node* TR_DataAccessAccelerator::insertDecimalGetIntrinsic(
    TR::TreeTop* callTreeTop, TR::Node* callNode, int32_t sourceNumBytes, int32_t targetNumBytes)
{
    ...
    if (requiresByteSwap && !TR::Compiler->target.cpu.isZ()) {
        printInliningStatus(
            false, callNode, "Unmarshalling is not supported because ByteSwap IL evaluators are not implemented.");
        return NULL;
    }

    if (performTransformation(
            comp(), "O^O TR_DataAccessAccelerator: insertDecimalGetIntrinsic on callNode %p\n", callNode)) {
        ...
        switch (sourceNumBytes) {
        case 4:
            op = requiresByteSwap ? TR::iriload : TR::floadi;
            break;
        case 8:
            op = requiresByteSwap ? TR::irlload : TR::dloadi;
            break;
        }
        ...
        TR::Node* valueNode = TR::Node::createWithSymRef(op, 1, 1,
            createByteArrayElementAddress(callTreeTop, callNode, byteArrayNode, offsetNode),
        ...
        if (requiresByteSwap) {
            ...
            switch (sourceNumBytes) {
            case 4:
                valueNode = TR::Node::create(TR::ibits2f, 1, valueNode);
                break;
            case 8:
                valueNode = TR::Node::create(TR::lbits2d, 1, valueNode);
                break;
            }
        }
        ...
        if (sourceNumBytes != targetNumBytes) {
            valueNode = TR::Node::create(
                TR::ILOpCode::getProperConversion(sourceDataType, targetDataType, false), 1, valueNode);
        }
...
TR::Node* TR_DataAccessAccelerator::insertDecimalSetIntrinsic(
    TR::TreeTop* callTreeTop, TR::Node* callNode, int32_t sourceNumBytes, int32_t targetNumBytes)
{
    if (sourceNumBytes != 4 && sourceNumBytes != 8) {
        printInliningStatus(false, callNode, "sourceNumBytes is invalid. Valid sourceNumBytes values are 4 or 8.");
    ...
    if (performTransformation(
            comp(), "O^O TR_DataAccessAccelerator: insertDecimalSetIntrinsic on callNode %p\n", callNode)) {
        insertByteArrayNULLCHK(callTreeTop, callNode, byteArrayNode);
        ...
        switch (targetNumBytes) {
        case 4:
            op = requiresByteSwap ? TR::iristore : TR::fstorei;
            break;
        case 8:
            op = requiresByteSwap ? TR::irlstore : TR::dstorei;
            break;
        }
        ...
        if (requiresByteSwap) {
            ...
            switch (targetNumBytes) {
            case 4:
                valueNode = TR::Node::create(TR::fbits2i, 1, valueNode);
                break;
            case 8:
                valueNode = TR::Node::create(TR::dbits2l, 1, valueNode);
                break;
            }
        }

        return TR::Node::createWithSymRef(op, 2, 2,
            createByteArrayElementAddress(callTreeTop, callNode, byteArrayNode, offsetNode), valueNode,
            comp()->getSymRefTab()->findOrCreateGenericIntShadowSymbolReference(0));
    }
...
bool TR_DataAccessAccelerator::inlineCheckPackedDecimal(TR::TreeTop* callTreeTop, TR::Node* callNode)
{
    ...
    if (failMsg) {
        TR::DebugCounter::incStaticDebugCounter(
            comp(), TR::DebugCounter::debugCounterName(comp(), "DAA/rejected/chkPacked"));

        return printInliningStatus(false, callNode, failMsg);
    }

    if (performTransformation(
            comp(), "O^O TR_DataAccessAccelerator: inlineCheckPackedDecimal on callNode %p\n", callNode)) {
        TR::DebugCounter::incStaticDebugCounter(
            comp(), TR::DebugCounter::debugCounterName(comp(), "DAA/inlined/chkPacked"));
        ...
        TR::Node* pdchkChild0Node = TR::Node::createWithSymRef(
            TR::pdloadi, 1, 1, constructAddressNode(callNode, byteArrayNode, offsetNode), packedDecimalSymbolReference);
...
TR::Node* TR_DataAccessAccelerator::insertIntegerGetIntrinsic(
    TR::TreeTop* callTreeTop, TR::Node* callNode, int32_t sourceNumBytes, int32_t targetNumBytes)
{
    if (targetNumBytes != 1 && targetNumBytes != 2 && targetNumBytes != 4 && targetNumBytes != 8) {
        printInliningStatus(
            false, callNode, "targetNumBytes is invalid. Valid targetNumBytes values are 1, 2, 4, or 8.");
        return NULL;
    }
    ...
    if (sourceNumBytes == 0) {
        ...
        if (sourceNumBytes != 1 && sourceNumBytes != 2 && sourceNumBytes != 4 && sourceNumBytes != 8) {
            printInliningStatus(
                false, callNode, "sourceNumBytes is invalid. Valid targetNumBytes values are 1, 2, 4, or 8.");
            return NULL;
        }
    ...
    if (performTransformation(
            comp(), "O^O TR_DataAccessAccelerator: genSimpleGetBinary call: %p inlined.\n", callNode)) {
        insertByteArrayNULLCHK(callTreeTop, callNode, byteArrayNode);
        ...
        case 1:
            ...
            break;
        case 2:
            op = requiresByteSwap ? TR::irsload : TR::sloadi;
            break;
        case 4:
            op = requiresByteSwap ? TR::iriload : TR::iloadi;
            break;
        case 8:
            op = requiresByteSwap ? TR::irlload : TR::lloadi;
            break;
        }
...
TR::Node* TR_DataAccessAccelerator::insertIntegerSetIntrinsic(
    TR::TreeTop* callTreeTop, TR::Node* callNode, int32_t sourceNumBytes, int32_t targetNumBytes)
{
    if (sourceNumBytes != 1 && sourceNumBytes != 2 && sourceNumBytes != 4 && sourceNumBytes != 8) {
        printInliningStatus(
            false, callNode, "sourceNumBytes is invalid. Valid sourceNumBytes values are 1, 2, 4, or 8.");
        return NULL;
    }
    ...
    if (requiresByteSwap && !TR::Compiler->target.cpu.isZ()) {
        printInliningStatus(
            false, callNode, "Marshalling is not supported because ByteSwap IL evaluators are not implemented.");
        return NULL;
    }
    ...
    if (performTransformation(
            comp(), "O^O TR_DataAccessAccelerator: genSimplePutBinary call: %p inlined.\n", callNode)) {
        insertByteArrayNULLCHK(callTreeTop, callNode, byteArrayNode);
        ...
        case 1:
            ...
            break;
        case 2:
            op = requiresByteSwap ? TR::irsstore : TR::sstorei;
            break;
        case 4:
            op = requiresByteSwap ? TR::iristore : TR::istorei;
            break;
        case 8:
            op = requiresByteSwap ? TR::irlstore : TR::lstorei;
            break;
        }
...
TR::Node* TR_DataAccessAccelerator::constructAddressNode(TR::Node* callNode, TR::Node* arrayNode, TR::Node* offsetNode)
{
    ...
    if (TR::Compiler->target.is64Bit()) {
        ...
        headerConstNode
            = TR::Node::create(callNode, TR::iconst, 0, TR::Compiler->om.contiguousArrayHeaderSizeInBytes());
...
bool TR_DataAccessAccelerator::genComparisionIntrinsic(TR::TreeTop* treeTop, TR::Node* callNode, TR::ILOpCodes ops)
{
    ...
    TR::Node* bcdchkNode
        = TR::Node::createWithSymRef(TR::BCDCHK, 7, 7, pdOpNode, callNode->getChild(0), callNode->getChild(1),
            callNode->getChild(2), callNode->getChild(3), callNode->getChild(4), callNode->getChild(5), bcdChkSymRef);
...
bool TR_DataAccessAccelerator::generateI2PD(TR::TreeTop* treeTop, TR::Node* callNode, bool isI2PD, bool isByteBuffer)
{
    ...
    if (failMsg) {
        TR::DebugCounter::incStaticDebugCounter(
            comp(), TR::DebugCounter::debugCounterName(comp(), "DAA/rejected/%s", isI2PD ? "i2pd" : "l2pd"));
        return printInliningStatus(false, callNode, failMsg);
    }
    ...
    if (performTransformation(comp(), "O^O TR_DataAccessAccelerator: %s call: %p inlined.\n",
        ...
        TR::DebugCounter::incStaticDebugCounter(
            comp(), TR::DebugCounter::debugCounterName(comp(), "DAA/inlined/%s", isI2PD ? "i2pd" : "l2pd"));
         ...
         *
         * With AddrNode2 and AddrNode3 commoned up, the LocalCSE is able to copy propagate pdshlOverflow to the the
         * pd2zd tree and replace its pdloadi.
         */
        ...
        if (needsBCDCHK) {
             ...
             * break as all optimizations such as Value Propogation don't expect it to be there. Attaching the
        ...
        if (isByteBuffer) {
            ...
            callBlock->createConditionalBlocksBeforeTree(
                treeTop, isValidAddrTreeTop, slowPathTreeTop, fastPathTreeTop, cfg, false, true);
        }

        TR::SymbolReference* symRef = comp()->getSymRefTab()->findOrCreateArrayShadowSymbolRef(
            TR::PackedDecimal, outOfLineCopyBackAddr, 8, fe());
...
bool TR_DataAccessAccelerator::generatePD2IConstantParameter(
    TR::TreeTop* treeTop, TR::Node* callNode, bool isPD2i, bool isByteBuffer)
{
    ...
    if (failMsg) {
        TR::DebugCounter::incStaticDebugCounter(
            comp(), TR::DebugCounter::debugCounterName(comp(), "DAA/rejected/%s", isPD2i ? "pd2i" : "pd2l"));
        return printInliningStatus(false, callNode, failMsg);
    }
    ...
    if (performTransformation(comp(), "O^O TR_DataAccessAccelerator: %s call: %p inlined.\n",
        ...
        TR::DebugCounter::incStaticDebugCounter(
            comp(), TR::DebugCounter::debugCounterName(comp(), "DAA/inlined/%s", isPD2i ? "pd2i" : "pd2l"));
        if (isByteBuffer) {
            /* We will be creating a precision diamond for the fast / slow path and eliminating the original call.
             * Because we are about to split the CFG we would have to store the original parameters of the call into
             * temp slots as we will be duplicating the call node in the precision diamond but we don't need to since
...
bool TR_DataAccessAccelerator::generatePD2IVariableParameter(
    TR::TreeTop* treeTop, TR::Node* callNode, bool isPD2i, bool isByteBuffer)
{
    TR::Node* precisionNode = callNode->getChild(2);
    ...
    if (comp()->getOption(TR_DisableVariablePrecisionDAA)
        ...
        TR::DebugCounter::incStaticDebugCounter(
            comp(), TR::DebugCounter::debugCounterName(comp(), "DAA/rejected/%s", isPD2i ? "var-pd2i" : "var-pd2l"));
        return false;
    }

    TR::DebugCounter::incStaticDebugCounter(
        comp(), TR::DebugCounter::debugCounterName(comp(), "DAA/inlined/%s", isPD2i ? "var-pd2i" : "var-pd2l"));
    // We will be creating a precision diamond for the fast / slow path and eliminating the original call.
...
bool TR_DataAccessAccelerator::genArithmeticIntrinsic(TR::TreeTop* treeTop, TR::Node* callNode, TR::ILOpCodes opCode)
{
    ...
    if (performTransformation(comp(),
        ...
        TR::ILOpCodes op = comp()->il.opCodeForIndirectStore(TR::PackedDecimal);
        TR::SymbolReference* symRefPdstore = comp()->getSymRefTab()->findOrCreateArrayShadowSymbolRef(
            TR::PackedDecimal, outOfLineCopyBackAddr, 8, fe());
...
bool TR_DataAccessAccelerator::genShiftRightIntrinsic(TR::TreeTop* treeTop, TR::Node* callNode)
{
    ...
    TR::Symbol* symStore = TR::Symbol::createShadow(
        comp()->trHeapMemory(), TR::PackedDecimal, TR::DataType::getSizeFromBCDPrecision(TR::PackedDecimal, dstPrec));
    symStore->setArrayShadowSymbol();
    symRefPdstore->setSymbol(symStore);
...
bool TR_DataAccessAccelerator::generateUD2PD(TR::TreeTop* treeTop, TR::Node* callNode, bool isUD2PD)
{
    ...
    if (failMsg) {
        TR::DebugCounter::incStaticDebugCounter(
            comp(), TR::DebugCounter::debugCounterName(comp(), "DAA/rejected/ud2pd"));

        return printInliningStatus(false, callNode, failMsg);
    }

    if (performTransformation(
            comp(), "O^O TR_DataAccessAccelerator: generate UD2PD/ED2PD call: %p inlined.\n", callNode)) {
        TR::DebugCounter::incStaticDebugCounter(
            comp(), TR::DebugCounter::debugCounterName(comp(), "DAA/inlined/ud2pd"));
        ...
        if (TR::Compiler->target.is64Bit()) {
            ...
            multipliedOffsetNode
                = TR::Node::create(TR::lmul, 2, TR::Node::create(TR::i2l, 1, decimalOffsetNode), twoConstNode);
        ...
        TR::Symbol* symStore = TR::Symbol::createShadow(
            comp()->trHeapMemory(), TR::PackedDecimal, TR::DataType::getSizeFromBCDPrecision(TR::PackedDecimal, prec));
...
bool TR_DataAccessAccelerator::generatePD2UD(TR::TreeTop* treeTop, TR::Node* callNode, bool isPD2UD)
{
    ...
    if (failMsg) {
        TR::DebugCounter::incStaticDebugCounter(
            comp(), TR::DebugCounter::debugCounterName(comp(), "DAA/rejected/%s", isPD2UD ? "pd2ud" : "pd2ed"));

        return printInliningStatus(false, callNode, failMsg);
    }

    if (performTransformation(
            comp(), "O^O TR_DataAccessAccelerator: generate PD2UD/PD2ED call: %p inlined.\n", callNode)) {
        TR::DebugCounter::incStaticDebugCounter(
            comp(), TR::DebugCounter::debugCounterName(comp(), "DAA/inlined/%s", isPD2UD ? "pd2ud" : "pd2ed"));
...
void TR_DataAccessAccelerator::insertByteArrayNULLCHK(
    TR::TreeTop* callTreeTop, TR::Node* callNode, TR::Node* byteArrayNode)
{
...
void TR_DataAccessAccelerator::insertByteArrayBNDCHK(
...
TR::Node* TR_DataAccessAccelerator::createByteArrayElementAddress(
    TR::TreeTop* callTreeTop, TR::Node* callNode, TR::Node* byteArrayNode, TR::Node* offsetNode)
{
    ...
    if (TR::Compiler->target.is64Bit()) {
        byteArrayElementAddressNode = TR::Node::create(TR::aladd, 2, byteArrayNode,
            TR::Node::create(TR::ladd, 2,
                TR::Node::create(callNode, TR::lconst, 0, TR::Compiler->om.contiguousArrayHeaderSizeInBytes()),
                TR::Node::create(TR::i2l, 1, offsetNode)));
    } else {
        byteArrayElementAddressNode = TR::Node::create(TR::aiadd, 2, byteArrayNode,
