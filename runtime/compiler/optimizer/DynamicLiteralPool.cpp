...
bool TR_DynamicLiteralPool::visitTreeTop(
    TR::TreeTop* tt, TR::Node* grandParent, TR::Node* parent, TR::Node* node, vcount_t visitCount)
{
    ...
    if (cg()->supportsOnDemandLiteralPool()) {
        ...
        // add extra aload child for float conversions
            ...
            || (opCodeValue == TR::l2d && TR::Compiler->target.is32Bit() && !cg()->use64BitRegsOn32Bit())) {
            addNewAloadChild(node);
        }
    }
...
bool TR_DynamicLiteralPool::transformNeeded(TR::Node* grandParent, TR::Node* parent, TR::Node* child)
{
    ...
    if (parentOpCode.isAdd() || parentOpCode.isSub()
        ...
        if (child->getOpCode().isLong() && (TR::Compiler->target.is32Bit()))
            ...
            if (oldOpCode == TR::iuconst && (parentOpCode.isAdd() || parentOpCode.isSub())) {
                TR::Node::recreate(child, TR::iconst);
            } else if (oldOpCode == TR::luconst && (parentOpCode.isAdd() || parentOpCode.isSub())) {
                TR::Node::recreate(child, TR::lconst);
            } else if (oldOpCode == TR::cconst && (parentOpCode.isAdd() || parentOpCode.isSub())) {
...
bool TR_DynamicLiteralPool::transformConstToIndirectLoad(TR::Node* parent, TR::Node* child)
{
    ...
    {
        indirectLoad = TR::Node::createWithSymRef(
            comp()->il.opCodeForIndirectLoad(child->getDataType()), 1, 1, addrNode, shadow);
        dumpOptDetails(
            comp(), "New node created %p, refcount of const child was %d\n", indirectLoad, child->getReferenceCount());
...
bool TR_DynamicLiteralPool::transformStaticSymRefToIndirectLoad(TR::TreeTop* tt, TR::Node* parent, TR::Node*& child)
{
    ...
    if (childOpcodeValue == TR::loadaddr) {
        ...
        if (childOpcodeValue == TR::awrtbar) {
            ...
            child = TR::Node::create(
                TR::awrtbari, 3, loadLiteralFromThePool, child->getFirstChild(), child->getSecondChild());
            if (parent)
...
bool TR_DynamicLiteralPool::handleNodeUsingVMThread(
    TR::TreeTop* tt, TR::Node* parent, TR::Node* node, vcount_t visitCount)
{
    /*
...
bool TR_DynamicLiteralPool::handleNodeUsingSystemStack(
    TR::TreeTop* tt, TR::Node* parent, TR::Node* node, vcount_t visitCount)
{
    return true;
}
