...
int32_t TR_EscapeAnalysis::perform()
{
    ...
    if (getLastRun())
        _maxPassNumber = 0; // Notwithstanding our heursitics, if this is the last run, our max "pass number" is zero
...
static TR_YesNoMaybe candidateHasField(
    Candidate* candidate, TR::Node* fieldNode, int32_t fieldOffset, TR_EscapeAnalysis* ea)
{
        ...
        if (fieldClassInCP
            ...
            if (!debugEAFieldValidityCheck
                || performTransformation(comp,
                       "%sQuick Using candidateHasField=yes (withinObjectBound=%s) for candidate [%p] field access "
                       "[%p]\n",
    ...
    //
    if (result == TR_maybe) {
        TR::VMAccessCriticalSection candidateHasFieldCriticalSection(
        ...
        if (candidateHasFieldCriticalSection.hasVMAccess()) {
            TR_OpaqueClassBlock* fieldDeclaringClass
                = fieldSymRef->getOwningMethod(comp)->getDeclaringClassFromFieldOrStatic(
                    comp, fieldSymRef->getCPIndex());
...
int32_t TR_EscapeAnalysis::performAnalysisOnce()
{
    ...
    if (!_candidates.isEmpty()) {
        ...
        _aliasesOfAllocNode = _doLoopAllocationAliasChecking ? new (trStackMemory())
                                                                   TR_BitVector(0, trMemory(), stackAlloc, growable)
                                                             : NULL;
        _aliasesOfOtherAllocNode = _doLoopAllocationAliasChecking
    ...
    if ((manager()->numPassesCompleted() > 0) && !_candidates.isEmpty())
    ...
    if (!_candidates.isEmpty()) {
        ...
        if (cg()->getEnforceStoreOrder()) {
            TR_FlowSensitiveEscapeAnalysis flowSensitiveAnalysis(
    ...
    for (candidate = _candidates.getFirst(); candidate; candidate = next) {
        ...
        if (candidate->_dememoizedConstructorCall) {
            if (candidate->isLocalAllocation() && !candidate->mustBeContiguousAllocation()
                ...
                if (trace()) {
                    traceMsg(comp(),
                        "   Fail [%p] because dememoization failed; will%s attempt again on next EA pass\n",
                        candidate->_node, mayDememoizeNextTime ? "" : " NOT");
                    traceMsg(comp(),
                        "   Fail [%p] because dememoization failed; will%s attempt dememoization again on next EA "
                        "pass\n",
                        candidate->_node, mayDememoizeNextTime ? "" : " NOT");
        ...
        for (candidate = _candidates.getFirst(); candidate; candidate = next) {
            ...
            if (candidate->mustBeContiguousAllocation() || !candidate->hasCallSites()
                || (candidate->_stringCopyNode && (candidate->_stringCopyNode != candidate->_node)
                       && !candidate->escapesInColdBlocks() && !candidate->isLockedObject()
            ...
            if (trace())
                traceMsg(
                    comp(), "   Make [%p] non-local because we'll try it again in another pass\n", candidate->_node);
        }
    }
        ...
        if (candidate->_kind == TR::New) {
            ...
            if (candidate->isLocalAllocation() && candidate->isContiguousAllocation()) {
                if (!_createLocalObjects) {
                    ...
                    if (trace())
                        traceMsg(
                            comp(), "   Make [%p] non-local because we can't create local objects\n", candidate->_node);
                }
            }
    ...
    if (comp()->getStartBlock() && !_candidates.isEmpty()) {
        ...
        if (blockIsInLoop(block)) {
            ...
            comp()->getFlowGraph()->addEdge(TR::CFGEdge::createEdge(
                comp()->getFlowGraph()->getStart()->asBlock(), firstBlockReplacement, trMemory()));
                ...
                node->setAndIncChild(0,
                    TR::Node::createWithSymRef(node, TR::loadaddr, 0,
                        comp()->getSymRefTab()->findOrCreateClassSymbol(comp()->getMethodSymbol(), -1,
                            comp()->fej9()->getClassFromSignature(
                                "java/lang/Integer", 17, comp()->getCurrentMethod(), true))));
...
Candidate* TR_EscapeAnalysis::createCandidateIfValid(
    TR::Node* node, TR_OpaqueClassBlock*& classInfo, bool foundUserAnnotation)
{
    ...
    if (!foundUserAnnotation) {
        ...
        if (node->getOpCodeValue() == TR::New) {
            ...
            if (comp()->getRunnableClassPointer()
                && comp()->fej9()->isInstanceOf(
                       (TR_OpaqueClassBlock*)classSym->getStaticAddress(), comp()->getRunnableClassPointer(), true)
                    == TR_yes) {
                if (trace()) {
    ...
    if (classInfo
        && !TR::Compiler->cls.sameClassLoaders(
               comp(), classInfo, comp()->getJittedMethodSymbol()->getResolvedMethod()->containingClass())
    ...
    if (size <= 0) {
        if (trace())
            traceMsg(
                comp(), "   Node [%p] failed: VM can't skip allocation (code %d, class %p)\n", node, size, classInfo);
                ...
                if (usesValueNumber(candidate, valueNumber)) {
                    ...
                    for (; cursorTree; cursorTree = cursorTree->getNextTreeTop()) {
                        ...
                        if (!storeIntoOtherLocalObject) {
                            ...
                            if (collectValueNumbersOfIndirectAccessesToObject(
...
bool TR_EscapeAnalysis::collectValueNumbersOfIndirectAccessesToObject(
    TR::Node* node, Candidate* candidate, TR::Node* indirectStore, TR::NodeChecklist& visited, int32_t baseChildVN)
{
    if (visited.contains(node))
        return false;
    visited.add(node);
    ...
    if (node->getOpCode().isLoadIndirect()) {
        ...
        if (sameSymbol) {
            ...
            // node, candidate->_node, baseChildVN, baseVN);
    ...
    for (i = 0; i < node->getNumChildren(); i++) {
        if (collectValueNumbersOfIndirectAccessesToObject(
                node->getChild(i), candidate, indirectStore, visited, baseChildVN))
            foundAccess = true;
    }
    return foundAccess;
}
                        ...
                        if (i < 0) {
                            ...
                            if (candidate->isInsideALoop()) {
                                ...
                                if (!p) {
                                    ...
                                    if (!checkOtherDefsOfLoopAllocation(
                                            useNode, candidate, (next->getFirstChild() == candidate->_node))) {
                                        if (trace())
                                            traceMsg(comp(),
            ...
            && !(defNode->getOpCode().isStoreDirect()
                   && ((defNode->getFirstChild()->getOpCodeValue() == TR::aconst)
                          || (defNode->getFirstChild()->getOpCode().isLoadVar()
                                 && // load from a static or array shadow or shadow (field) based off a non local object
                                    // are fine since such memory locations would imply that the object being pointed at
                                    // escaped already (meaning there would be another escape point anyway)
                                 (defNode->getFirstChild()->getSymbol()->isStatic()
                                     || (defNode->getFirstChild()->getSymbol()->isShadow()
                                            && (defNode->getFirstChild()->getSymbol()->isArrayShadowSymbol()
                                                   || !_localObjectsValueNumbers->get(_valueNumberInfo->getValueNumber(
...
bool TR_EscapeAnalysis::checkAllNewsOnRHSInLoopWithAliasing(int32_t defIndex, TR::Node* useNode, Candidate* candidate)
{
    ...
    if ((defNode->getFirstChild() == candidate->_node)
        ...
        if (trace()) {
            traceMsg(comp(),
                "      Value numbers match for def node [%p] with candidate node [%p], and def node's symref is alias "
                "of candidate allocation\n",
                defNode, candidate->_node);
        }
        allnewsonrhs = true;
    } else {
        ...
        for (cursor2.SetToFirstOne(); cursor2.Valid(); cursor2.SetToNextOne()) {
            ...
            for (Candidate* otherAllocNode = _candidates.getFirst(); otherAllocNode;
                ...
                if (trace()) {
                    traceMsg(
                        comp(), "         Look at defNode2 [%p] with otherAllocNode [%p]\n", defNode2, otherAllocNode);
                }
                    ...
                    if (firstChild->getOpCode().hasSymbolReference()
                        ...
                        if (addr->getOpCode().isArrayRef()) {
                            ...
                            if (underlyingArray && underlyingArray->getOpCode().hasSymbolReference()
                                && underlyingArray->getSymbolReference()->getSymbol()->isStaticField()) {
                                fieldName = underlyingArray->getSymbolReference()->getOwningMethod(comp())->staticName(
                                    underlyingArray->getSymbolReference()->getCPIndex(), fieldNameLen,
                                    comp()->trMemory());
                            }
...
bool TR_EscapeAnalysis::checkOverlappingLoopAllocation(
    TR::Node* node, TR::Node* useNode, TR::Node* allocNode, rcount_t& numReferences)
{
    ...
    if (_doLoopAllocationAliasChecking && node->getOpCode().isStore() && node->getSymbol()->isAutoOrParm()) {
...
bool TR_EscapeAnalysis::checkIfUseIsInLoopAndOverlapping(Candidate* candidate, TR::TreeTop* defTree, TR::Node* useNode)
{
    ...
    bool b = checkIfUseIsInLoopAndOverlapping(
        allocTree->getNextTreeTop(), candidate->_block->getExit(), defTree, useNode, visited, vBlocks, decisionMade);
    if (trace())
    ...
    TR::Node* useNode, TR::NodeChecklist& visited, TR::BlockChecklist& vBlocks, bool& decisionMade)
{
    ...
    for (auto nextEdge = block->getSuccessors().begin(); nextEdge != block->getSuccessors().end(); ++nextEdge) {
        ...
        if (!vBlocks.contains(next) && (next != cfg->getEnd())) {
            ...
            bool b = checkIfUseIsInLoopAndOverlapping(
                next->getEntry(), next->getExit(), defTree, useNode, visited, vBlocks, decisionMade);
            if (decisionMade) {
                if (b)
                    return true;
            }
        ...
        if (usesValueNumber(candidate, valueNumber)) {
            if (!forceFail && checkIfEscapePointIsCold(candidate, reason)) {
                ...
                if (candidate->forceLocalAllocation()) {
                    if (trace())
                        traceMsg(comp(),
                            "   Normally would fail [%p] because it escapes via node [%p] (cold %d), but user forces "
                            "it to be local\n",
        ...
        if (candidate->isLocalAllocation() && usesValueNumber(candidate, valueNumber)) {
            ...
            if (locked) {
                ...
                if (_parms && fej9->hasTwoWordObjectHeader()) {
                    ...
                    fej9->getResolvedMethods(
                        trMemory(), (TR_OpaqueClassBlock*)candidate->_class, &resolvedMethodsInClass);
            ...
            if (type == MakeNonLocal) {
                ...
                if (type == MakeContiguous) {
                    ...
                    if (checkIfEscapePointIsCold(candidate, reason)) {
                        ...
                        if (trace())
                            traceMsg(
                                comp(), "   Make [%p] contiguous because of node [%p]\n", candidate->_node, reason);
                        candidate->setMustBeContiguousAllocation();
                    }
                    ...
                    if (trace())
                        traceMsg(
                            comp(), "   Make [%p] object-referenced because of node [%p]\n", candidate->_node, reason);
                    candidate->setObjectIsReferenced();
                    wasRestricted = true;
                }
            }
...
static void checkForDifferentSymRefs(
    Candidate* candidate, int32_t i, TR::SymbolReference* symRef, TR_EscapeAnalysis* ea, bool peeking)
{
...
bool FieldInfo::symRefIsForFieldInAllocatedClass(TR::SymbolReference* symRef)
{
    ...
    TR_ASSERT(
        0, "symRefIsForFieldInAllocatedClass expects symref #%d to have been remembered", symRef->getReferenceNumber());
    return true;
}

bool FieldInfo::hasBadFieldSymRef() { return !_badFieldSymrefs->isEmpty(); }

void FieldInfo::rememberFieldSymRef(
    TR::Node* fieldNode, int32_t fieldOffset, Candidate* candidate, TR_EscapeAnalysis* ea)
{
...
//
void TR_EscapeAnalysis::referencedField(
    TR::Node* base, TR::Node* field, bool isStore, bool seenSelfStore, bool seenStoreToLocalObject)
{
        ...
        if (candidate->isLocalAllocation() && usesValueNumber(candidate, valueNumber)) {
            ...
            for (int j = 0; j < N; j++) {
                ...
                if (i < 0) {
                    ...
                    (*candidate->_fields)[i]._goodFieldSymrefs
                        = new (trStackMemory()) TR_ScratchList<TR::SymbolReference>(trMemory());
                    (*candidate->_fields)[i]._badFieldSymrefs
                        = new (trStackMemory()) TR_ScratchList<TR::SymbolReference>(trMemory());
                }
                if (!isPeeking)
                ...
                for (candidate = _candidates.getFirst(); candidate; candidate = next) {
                    ...
                    if ((candidate->_stringCopyNode == NULL)
                        ...
                        if ((baseNode == candidate->_node) && (baseNode->getOpCodeValue() == TR::New)
                            ...
                            // candidate->_stringCopyNode, node);
                            TR::Node* baseNode = node->getFirstChild();
                            int32_t baseNodeVN = _valueNumberInfo->getValueNumber(baseNode);
...
static bool isFinalizableInlineTest(TR::Compilation* comp, TR::Node* candidate, TR::Node* root, TR::Node* vftLoad)
{
    ...
    */
    ...
    return root->getOpCodeValue() == ifOp && root->getFirstChild()->getOpCodeValue() == andOp
        ...
        && (is64Bit ? root->getFirstChild()->getSecondChild()->getLongInt() == fej9->getFlagValueForFinalizerCheck()
    ...
    if (node->getOpCodeValue() == TR::areturn) {
        ...
        if (_parms) {
            ...
            if (argument && doit
                ...
                if (trace())
                    traceMsg(
                        comp(), "call node is %p (depth %d) for which return opt was done\n", callNode, _sniffDepth);
#if 0
            //
        ...
        if (node->getOpCode().isIndirect()) {
            ...
            if (node->getOpCode().isStore()) {
                ...
                if ((node->getSecondChild() != baseObject)
                    ...
                    if ((!_localObjectsValueNumbers || !_notOptimizableLocalObjectsValueNumbers || !resolvedBaseObject
                            || (comp()->useCompressedPointers() && (TR::Compiler->om.compressedReferenceShift() > 3)
                                   && !TR::Compiler->target.cpu.isX86() && !TR::Compiler->target.cpu.isPower()
...
bool SniffCallCache::isInCache(
    TR_LinkHead<SniffCallCache>* sniffCacheList, TR_ResolvedMethod* vmMethod, bool isCold, int32_t& bytecodeSize)
{
            ...
            if (usesValueNumber(candidate, nodeVN)) {
                ...
                if (arg == firstArgIndex && methodSymbol
                    ...
                    if (methodId == TR::java_lang_Throwable_printStackTrace) {
                        ...
                        if (trace())
                            traceMsg(
                                comp(), "   Make [%p] contiguous because of setUsesStackTrace\n", candidate->_node);
                    }
                }
                ...
                if (1) {
                    ...
                    if (node->getSymbolReference() == finalizeSymRef) {
                        if (trace())
                            traceMsg(
                                comp(), "Candidate [%p] cannot escape because of node [%p]\n", candidate->_node, node);
                        continue;
                    }
                }
        ...
        for (candidate = _candidates.getFirst(); candidate; candidate = next) {
            ...
            if (candidate->isArgToCall(_sniffDepth)) {
                if (!candidate->isNonThisArgToCall(_sniffDepth) && node->getOpCode().isIndirect()
                    && ((candidate->_node->getOpCodeValue() == TR::New)) && (thisVN > -1)
                    ...
                    if (resolvedMethod) {
                        ...
                        if (!sniffCallInCache) {
                            ...
                            if (!newSymRef) {
                                newSymRef = getSymRefTab()->findOrCreateMethodSymbol(
                            ...
                            ignoreRecursion = oldIgnoreRecursion;
                            bytecodeSize = sniffCall(
                                node, newMethodSymbol->getResolvedMethodSymbol(), true, _inColdBlock, ignoreRecursion);
                        ...
                        // originalBytecodeSize, resolvedMethod->signature());
                    }
                }
                ...
                if ((bytecodeSize > 0)
                    ...
                    if (candidate->forceLocalAllocation()) {
                        if (trace())
                            traceMsg(comp(),
                                "   Normally [%p] would fail because child of call [%p] to %s, but user wants it "
                                "locally allocated\n",
                                candidate->_node, node,
...
int32_t TR_EscapeAnalysis::sniffCall(
    TR::Node* callNode, TR::ResolvedMethodSymbol* methodSymbol, bool ignoreOpCode, bool isCold, bool& ignoreRecursion)
{
    ...
    for (candidate = _candidates.getFirst(); candidate; candidate = next) {
        ...
        if (candidate->isContiguousAllocation()) {
            if (candidate->_size > MAX_SIZE_FOR_ONE_CONTIGUOUS_OBJECT) {
                ...
                if (trace())
                    traceMsg(
                        comp(), "   Fail [%p] because object size %d is too big\n", candidate->_node, candidate->_size);
                    ...
                    if (candidate->escapesInColdBlock(_curBlock)) {
                        ...
                        if (TR_yes == candidateHasField(candidate, node, fieldOffset, this)) {
                                                                            ...
                                                                            // leave in place to minimize disruption
                            {
                                for (i = candidate->_fields->size() - 1; i >= 0; i--) {
                                    if (candidate->_fields->element(i)._offset == fieldOffset) {
                                        candidate->_fields->element(i).rememberFieldSymRef(
                                            node, fieldOffset, candidate, this);
                                        if (candidate->isContiguousAllocation())
            ...
            if (calledMethod->getRecognizedMethod() == TR::java_lang_Throwable_fillInStackTrace) {
                ...
                if (candidate && candidate->fillsInStackTrace() && !candidate->usesStackTrace()
                    ...
                    && performTransformation(
                           comp(), "%sRemoving call node [%p] to fillInStackTrace\n", OPT_DETAILS, node)) {
            ...
            if (comp()->fej9()->isInstanceOf((TR_OpaqueClassBlock*)candidate->_class,
                ...
                if (node->getOpCodeValue() == TR:: instanceof) {
                    if (performTransformation(
                            comp(), "%sReplacing instanceof [%p] by constant 1\n", OPT_DETAILS, node)) {
                        node->removeAllChildren();
                        TR::Node::recreate(node, TR::iconst);
                        node->setInt(1);
                    }
                ...
                if (node->getOpCodeValue() == TR:: instanceof) {
                    if (performTransformation(
                            comp(), "%sReplacing instanceof [%p] by constant 0\n", OPT_DETAILS, node)) {
                        node->removeAllChildren();
                        TR::Node::recreate(node, TR::iconst);
                        node->setInt(0);
                    }
            ...
            if (calledMethod) {
                if (_desynchronizeCalls) {
                    if (trace()) {
                        ...
                        ///calledMethod->getResolvedMethod()->signature(trMemory()),
                        ///maxBytecodeIndex(calledMethod->getResolvedMethod()), comp()->signature());
                ...
                if (node->getOpCodeValue() == TR::allocationFence) {
                    if (candidate->isLocalAllocation()) {
                        if (trace())
                            traceMsg(comp(),
                                "Redundant flush node [%p] found! Set omitSync flag on redundant flush node.\n", node);
...
bool TR_EscapeAnalysis::fixupFieldAccessForNonContiguousAllocation(
    TR::Node* node, Candidate* candidate, TR::Node* parent)
{
    int32_t i;
    int32_t fieldOffset = (candidate->_origKind == TR::New) ? comp()->fej9()->getObjectHeaderSizeInBytes()
    ...
    if (i >= 0) {
        ...
        if (node->getOpCode().isLoadVar()) {
            ...
            if (autoSymRef->getSymbol()->getDataType().isVector() && !node->getDataType().isVector()) {
            ...
            if (autoSymRef->getSymbol()->getDataType().isVector() && !node->getDataType().isVector()) {
                ...
                TR::Node* newValue = TR::Node::create(
                    node, node->getDataType() == TR::VectorDouble ? TR::vdsetelem : TR::visetelem, 3);
        ...
        if (trace())
            traceMsg(comp(), "Change node [%p] into a direct load or store of #%d (%d bytes) field %d cand %p\n", node,
                autoSymRef->getReferenceNumber(), autoSymRef->getSymbol()->getSize(), i, candidate);
...
void TR_EscapeAnalysis::makeLocalObject(Candidate* candidate)
{
    ...
    if (candidate->_kind == TR::New) {
        symRef = getSymRefTab()->createLocalObject(
            candidate->_size, comp()->getMethodSymbol(), allocationNode->getFirstChild()->getSymbolReference());
        ...
        symRef = getSymRefTab()->createLocalAddrArray(
            candidate->_size, comp()->getMethodSymbol(), allocationNode->getSecondChild()->getSymbolReference());
        ...
        symRef = getSymRefTab()->createLocalPrimArray(
            candidate->_size, comp()->getMethodSymbol(), allocationNode->getSecondChild()->getInt());
        symRef->setStackAllocatedArrayAccess();
    }
    ...
    if (referenceSlots || (candidate->_kind != TR::New) // 174954: array size field must be initialized in the first
                                                        // block until we put a sumref on TR::arraylength
...
void TR_EscapeAnalysis::avoidStringCopyAllocation(Candidate* candidate)
{
    ...
    if (trace() || debug("traceContiguousESC")) {
        traceMsg(comp(), "secs (%d) String (copy) allocation of size %d found in %s\n", manager()->numPassesCompleted(),
            candidate->_size, comp()->signature());
    }
            ...
            if (performTransformation(comp(), "%sUse arrayset to initialize [%p]\n", OPT_DETAILS, allocationNode)) {
                ...
                TR::DebugCounter::prependDebugCounter(comp(),
                    TR::DebugCounter::debugCounterName(
                        comp(), "escapeAnalysis/zeroInitializeArrayset/%s", comp()->signature()),
                    arraysetTreeTop);
...
void TR_EscapeAnalysis::makeContiguousLocalAllocation(Candidate* candidate)
{
    ...
    if ((candidate->_node->getOpCodeValue() != TR::New) && candidate->_node->canSkipZeroInitialization())
        skipZeroInit = true;
    ...
    if (candidate->isExplicitlyInitialized()) {
        ...
        if (candidate->_kind == TR::newarray) {
            ...
            // involves verifying the instruction sequences each induvidual codegen will generate and only if the
            ...
            if (enableNewArrayArraySetPendingInvestigation && tryToZeroInitializeUsingArrayset(candidate, initTree)) {
                TR::DebugCounter::prependDebugCounter(comp(),
                    TR::DebugCounter::debugCounterName(
                        comp(), "escapeAnalysis/zeroInitializeArrayset/%s/primitive", comp()->signature()),
        ...
        int32_t headerSize = (candidate->_kind == TR::New) ? comp()->fej9()->getObjectHeaderSizeInBytes()
                                                           : TR::Compiler->om.contiguousArrayHeaderSizeInBytes();
                ...
                for (j = candidate->_fields->size() - 1; j >= 0; j--) {
                    ...
                    if (field._offset == i && field._symRef) {
                        ...
                        node = TR::Node::createWithSymRef(
                            comp()->il.opCodeForIndirectStore(type), 2, 2, allocationNode, node, field._symRef);
    ...
    if (referenceSlots) {
        ...
        if (tryToZeroInitializeUsingArrayset(candidate, initTree)) {
            TR::DebugCounter::prependDebugCounter(comp(),
                TR::DebugCounter::debugCounterName(
                    comp(), "escapeAnalysis/zeroInitializeArrayset/%s/reference", comp()->signature()),
                initTree->getNextTreeTop());
            ...
            if (candidate->_fields && candidate->_origKind == TR::New) {
                for (j = candidate->_fields->size() - 1; j >= 0; j--) {
                    ...
                    if (field._offset == offset && field._symRef) {
                        ...
                        node = createConst(comp(), allocationNode, type, 0);
                        node = TR::Node::createWithSymRef(
                            comp()->il.opCodeForIndirectStore(type), 2, 2, baseNode, node, field._symRef);
...
void TR_EscapeAnalysis::makeNonContiguousLocalAllocation(Candidate* candidate)
{
    ...
    if (candidate->objectIsReferenced()) {
        if (candidate->_kind != TR::New) {
            ...
            ///(see the first occurence of findOrCreateClassSymbol in EA; dememoizedConstructorCall
...
void TR_EscapeAnalysis::heapifyBeforeColdBlocks(Candidate* candidate)
{
    ...
    for (info = coldBlockInfoIt.getFirst(); info != NULL; info = coldBlockInfoIt.getNext()) {
        ...
        if (trace())
            traceMsg(comp(), "heapifying %p b1 %d b2 %d\n", candidate->_node, candidate->isContiguousAllocation(),
                candidate->_dememoizedMethodSymRef);
        ...
        //
        int32_t headerSize = (candidate->_origKind == TR::New) ? comp()->fej9()->getObjectHeaderSizeInBytes()
                                                               : TR::Compiler->om.contiguousArrayHeaderSizeInBytes();
        int32_t i;
        ...
        while (i < size) {
            ...
            if (candidate->_fields) {
                ...
                for (j = candidate->_fields->size() - 1; j >= 0; j--) {
                    ...
                    if (field._offset == i && field._symRef
                        ...
                        TR::Node* stackFieldLoad = NULL;
                        if (!candidate->isContiguousAllocation())
                            stackFieldLoad = TR::Node::createWithSymRef(
                                heapAllocation, comp()->il.opCodeForDirectLoad(type), 0, field._symRef);
                        else
                            stackFieldLoad = TR::Node::createWithSymRef(
                                comp()->il.opCodeForIndirectLoad(type), 1, 1, stackAllocation, field._symRef);
                        ...
                        if (stackFieldLoad->getDataType() == TR::Address) {
                            ...
                            if (comp()->useCompressedPointers()) {
                                translateTT = TR::TreeTop::create(
                                    comp(), TR::Node::createCompressedRefsAnchor(heapFieldStore), NULL, NULL);
                            }
                        } else
                            heapFieldStore = TR::Node::createWithSymRef(comp()->il.opCodeForIndirectStore(type), 2, 2,
                                heapAllocation->getFirstChild(), stackFieldLoad, field.fieldSymRef());
            ...
            //             TR::Node *heapFieldStore = TR::Node::create(TR::astorei, 2, heapAllocation->getFirstChild(),
            //             stackFieldLoad, intShadow); TR::TreeTop *heapFieldStoreTree = TR::TreeTop::create(comp(),
            //             heapFieldStore, NULL, NULL); treeBeforeInsertionPoint->join(heapFieldStoreTree);
            //             heapFieldStoreTree->join(heapAllocationExitTree);
            //             treeBeforeInsertionPoint = heapFieldStoreTree;
            //             i += refIncrVal;
            //             }
            ...
            TR::Node* heapFieldStore = TR::Node::createWithSymRef(
                TR::istorei, 2, 2, heapAllocation->getFirstChild(), stackFieldLoad, intShadow);
                    ...
                    if (candidate->_fields) {
                         ...
                         field._size == TR::Symbol::convertTypeToSize(TR_SInt32)*/)
                        {
                                ...
                                if (comp()->useCompressedPointers() && (type == TR::Address)) {
                                    translateTT = TR::TreeTop::create(
                                        comp(), TR::Node::createCompressedRefsAnchor(heapFieldLoad), NULL, NULL);
                                }
                                TR::Node* stackStore = TR::Node::createWithSymRef(
                                    comp()->il.opCodeForDirectStore(type), 1, 1, heapFieldLoad, field._symRef);
        ...
        for (symRef = symRefsIt.getFirst(); symRef; symRef = symRefsIt.getNext()) {
            ...
            // stores if required
            //
            TR::Node* comparisonNode
                = TR::Node::createif(TR::ifacmpne, TR::Node::createWithSymRef(candidate->_node, TR::aload, 0, symRef),
            ...
            TR::Block* comparisonBlock
                = toBlock(cfg->addNode(TR::Block::createEmptyBlock(comparisonNode, comp(), coldBlock->getFrequency())));
            ...
            TR::Node* storeNode = TR::Node::createWithSymRef(
                TR::astore, 1, 1, TR::Node::createWithSymRef(comparisonNode, TR::aload, 0, heapSymRef), symRef);
            ...
            if (!symRef->getSymbol()->isParm()) {
                TR::Node* initStoreNode
                    = TR::Node::createWithSymRef(TR::astore, 1, 1, TR::Node::aconst(comparisonNode, 0), symRef);
                if (symRef->getSymbol()->holdsMonitoredObject())
            ...
            TR::Block* storeBlock
                = toBlock(cfg->addNode(TR::Block::createEmptyBlock(storeNode, comp(), coldBlock->getFrequency())));
        ...
        for (auto pred = coldBlock->getPredecessors().begin(); pred != coldBlock->getPredecessors().end();) {
            ...
            pred++;
            if (((predNode != lastComparisonBlock) && (predNode != lastStoreBlock)) || coldBlock->isCatchBlock()) {
                ...
                if (!coldBlock->isCatchBlock()
                    ...
                    if (lastNode->getOpCode().isBranch() && (lastNode->getBranchDestination() == coldBlock->getEntry()))
                        ...
                        if (lastNode->getOpCode().isSwitch())
                            lastTree->adjustBranchOrSwitchTreeTop(
                                comp(), coldBlock->getEntry(), heapComparisonBlock->getEntry());
...
bool TR_EscapeAnalysis::devirtualizeCallSites()
{
    ...
    while (!_devirtualizedCallSites.isEmpty()) {
        ...
        if (calledMethod
            ...
            TR::Block* remainder = block->createConditionalBlocksBeforeTree(
            ...
            if (callSite->getNode()->getOpCode().hasSymbolReference())
                directCallTreeNode = TR::Node::createWithSymRef(
                    callSite->getNode()->getOpCodeValue(), 1, 1, directCall, callSite->getNode()->getSymbolReference());
            else
            ...
            if (callSite->getNode()->getOpCode().hasSymbolReference())
                coldTreeNode = TR::Node::createWithSymRef(
                    callSite->getNode()->getOpCodeValue(), 1, 1, coldCall, callSite->getNode()->getSymbolReference());
            else
...
void Candidate::print()
{
    ...
    if (_fields) {
        ...
        for (int32_t i = 0; i < _fields->size(); i++) {
            ...
            traceMsg(comp(), "     %2d: offset=%-3d size=%-2d vectorElem=%-2d ", i, field._offset, field._size,
                field._vectorElem);
            if (field._symRef)
...
static TR_DependentAllocations* getDependentAllocationsFor(
    Candidate* c, List<TR_DependentAllocations>* dependentAllocations)
{
...
TR_FlowSensitiveEscapeAnalysis::TR_FlowSensitiveEscapeAnalysis(
    TR::Compilation* comp, TR::Optimizer* optimizer, TR_Structure* rootStructure, TR_EscapeAnalysis* escapeAnalysis)
    ...
    , _splitBlocks(comp->trMemory())
{
    ...
    TR::MonitorElimination::collectPredsAndSuccs(comp->getFlowGraph()->getStart(), visitedNodes, _predecessorInfo,
        _successorInfo, &_cfgBackEdges, _loopEntryBlocks, comp);
                ...
                // if (postDominated)
                {
                    ...
                    if (!_blocksWithSyncs->get(nextSucc)) {
                        ...
                        // succCandidate->getNext())
                        {
                            ...
                            // if (trace())
                            //   traceMsg(comp, "succCandidate %p succCandidate num %d succCandidate Flush reqd %d
                            //   succBlockNum %d\n", succCandidate, succCandidate->_index,
                            //   succCandidate->_flushRequired, succBlockNum);
                    ...
                    if (_blocksWithSyncs->get(nextSucc) || succCandidate) {
                        ...
                        if (trace()) {
                            if (succCandidate) {
                                traceMsg(comp,
                                    "Flush for current allocation %d is post dominated by (and moved to) Flush for "
                                    "allocation %d\n",
                                    candidate->_index, succCandidate->_index);
                                // printf("Moved flush for allocation %p in block_%d to allocation %p in succ block_%d
                                // in method %s\n", candidate->_node, blockNum, succCandidate->_node, nextSucc,
                                // comp->signature());
                            } else {
                                traceMsg(comp,
                                    "Flush for current allocation %d is post dominated by (and moved to) real sync for "
                                    "in succ %d\n",
                                    candidate->_index, nextSucc);
                                // printf("Moved flush for allocation %p in block_%d to real sync in succ %d in method
...
bool TR_FlowSensitiveEscapeAnalysis::postInitializationProcessing()
{
    ...
    for (treeTop = comp()->getStartTree(); treeTop; treeTop = treeTop->getNextTreeTop()) {
        ...
        if (node->getOpCodeValue() == TR::monent) {
            ...
            || (node->getOpCode().isCall() && !node->hasUnresolvedSymbolReference()
...
bool TR_LocalFlushElimination::examineNode(TR::Node* node, TR::NodeChecklist& visited)
{
    ...
    if (!opCode.isCall()) {
        ...
        if (node->getOpCodeValue() == TR::monexit) {
            ...
            if (opCode.isCall() && !node->hasUnresolvedSymbolReference()
                && node->getSymbolReference()->getSymbol()->castToMethodSymbol()->isSynchronised())
                nodeHasSync = true;
        }
                    ...
                    if (nextAllocCanReach) {
                        ...
                        if (trace()) {
                            if (!nodeHasSync) {
                                ...
                                traceMsg(comp(),
                                    "Flush for current allocation %d is post dominated by (and moved to) sync in the "
                                    "same block\n",
