...
bool TR_CISCNode::isEqualOpc(TR_CISCNode* t)
{
    ...
    static_assert((int)TR::NumIlOps == TR_variable, "assumption for reducing compilation time");
...
// Debug print to file
//*****************************************************************************************
void TR_CISCNode::dump(TR::FILE* pOutFile, TR::Compilation* comp)
{
    int32_t i;
    char buf[256];
...
void TR_CISCGraph::dump(TR::FILE* pOutFile, TR::Compilation* comp)
{
...
bool TR_CISCTransformer::createLoopCandidates(List<TR_RegionStructure>* loopCandidates)
{
    ...
    detectWhileLoops(
        whileLoopsInnerFirst, whileLoops, doWhileLoopsInnerFirst, doWhileLoops, _cfg->getStructure(), true);
...
TR::Block* TR_CISCTransformer::addPreHeaderIfNeeded(TR_RegionStructure* region)
{
    ...
    if (!preHeader) {
        preHeader = TR::Block::createEmptyBlock(
            loopEntry->getEntry()->getNode(), comp(), loopEntry->getFrequency(), loopEntry);
    ...
    if (loopEntryFrequency <= 0) {
        ...
        if (outer) {
            ...
            if (outerFrequency < 1)
                outerFrequency = 1;
            if (trace())
                traceMsg(comp(), "\t\tOuter block %d: Frequency=%d Inner/Outer Ratio:(%f)\n", outer->getNumber(),
...
void TR_CISCTransformer::easyTreeSimplification(TR::Node* const node)
{
    ...
    if (node->getOpCode().isIf()) {
        ...
        if (node->getOpCodeValue() == TR::ificmplt && iconstC->getInt() == 1) {
            traceMsg(
                comp(), "\t\teasyTreeSimplification: Node: %p converted from ificmplt with 1 to ifcmple with 0", node);
...
void TR_CISCTransformer::showEmbeddedData(char* title, uint8_t* data)
{
    int32_t i, j;
    traceMsg(comp(), "%s\n    ", title);
...
//***************************************************************************************
bool TR_CISCTransformer::checkParents(
    TR_CISCNode* p, TR_CISCNode* t, uint8_t* result, bool* inLoop, bool* optionalParents)
{
...
bool TR_CISCTransformer::computeEmbeddedForData()
{
    ...
    while (true) // This loop is for narrowing down candidates of a leaf.
    {
                                ...
                                if (tOpc == TR_variable) {
                                    ...
                                    for (parent = parenti.getFirst(); parent; parent = parenti.getNext()) {
                                        if (!parent->isOutsideOfLoop() && parent->getIlOpCode().isStoreDirect()) {
                                            if (trace())
                                                traceMsg(comp(), "pID%d: tID%d isn't loop invariant because of %d\n",
                                    ...
                                    if ((chiData10 == _Embed || chiData11 == _Embed)
                                        ...
                                        || ((chiData11 == _Desc)
                                               && (!isChildDirectlyConnected
                                                      || (tch1->isLoadVarDirect()
                                                             && _Embed
                                                                 == result[pIdx1 + tch1->getChild(0)->getID()])))) {
                                        // OK!
                                    } else {
                                        isEmbed = false;
                                    }
                                        ...
                                        || ((chiData == _Desc)
                                               && (!isChildDirectlyConnected
                                                      || (tch->isLoadVarDirect()
                                                             && _Embed
                                                                 == result[idx(
                                                                        pch->getID(), tch->getChild(0)->getID())])))) {
                                        // OK!
                                    } else {
                                        isEmbed = false;
                                        break;
        ...
        for (ple = plistHead; ple; ple = ple->getNextElement()) {
            ...
            if (trace() && count == 0)
                traceMsg(comp(), "screening1: count=%d for p:%d\n", count, p->getID());
            count = 0;
            for (tle = tlistHead; tle; tle = tle->getNextElement()) {
                t = tle->getData();
                if (result[tmpIdx + t->getID()] == _Embed) {
                    ...
                    if (!checkParents(p, t, result, &inLoop, &allOptionalParents)
                        ...
                        if (trace())
                            traceMsg(comp(), "screening1: set _NotEmbed to (%d, %d)\n", p->getID(), t->getID());
                    } else {
                        count++;
                    }
                }
                ...
                for (s = si.getFirst(); s; s = si.getNext()) {
                    if (mOpcode == s->getOpcode()) {
                        ...
                        if (result[tmpMIdx + tID] == _Embed) {
                            ...
                            if (trace())
                                traceMsg(
                                    comp(), "screening2, sameOpcode: set _NotEmbed to (%d, %d)\n", m->getID(), tID);
                        }
                    }
                }
                    ...
                    for (s = si.getFirst(); s; s = si.getNext()) {
                        if (mOpcode != s->getOpcode()) {
                            ...
                            if (result[tmpMIdx + tID] == _Embed) {
                                ...
                                if (trace())
                                    traceMsg(
                                        comp(), "screening2, others: set _NotEmbed to (%d, %d)\n", m->getID(), tID);
                            }
                        }
                    }
                }
...
bool TR_CISCTransformer::makeLists()
{
    ...
    for (p = pi.getFirst(); p; p = pi.getNext()) {
        ...
        if (pList->isMultipleEntry() && p->getOpcode() == TR_variable) {
            if (!p->isOptionalNode()) {
                if (trace())
                    traceMsg(comp(), "makeLists: pid:%d a variable corresponds to multiple nodes\n", p->getID());
...
//*****************************************************************************
int32_t TR_CISCTransformer::analyzeConnectionOnePairChild(
    TR_CISCNode* const p, TR_CISCNode* const t, TR_CISCNode* const pn, TR_CISCNode* tn)
{
...
bool TR_CISCTransformer::alignTopOfRegion(TR_CISCNodeRegion* r)
{
    ...
    while (true) {
        ...
        if (t == 0) {
            if (!pTop->isOptionalNode()) {
                if (trace())
                    traceMsg(comp(),
                        "alignTopOfRegion failed. There is no target node corresponding to %d.  Check for nodes in "
                        "broken region listings above and x in SPBC listing.\n",
                        pTop->getID());
                return false;
            }
        } else {
    ...
    if (trace())
        traceMsg(comp(), "alignTopOfRegion: (pTop, t) is (%d, %d)\n", pTop->getID(), t->getID());
    ...
    if (trace())
        traceMsg(comp(), "alignTopOfRegion failed. Cannot find pTop:%d in the region.\n", pTop->getID());
    return false;
}
...
//*****************************************************************************
void TR_CISCTransformer::moveCISCNodesInList(
    List<TR_CISCNode>* l, TR_CISCNode* from, TR_CISCNode* to, TR_CISCNode* moveTo)
{
#if 0
   if (showMesssagesStdout())
      {
...
// * _T->_orderByData
//*****************************************************************************
void TR_CISCTransformer::moveCISCNodes(TR_CISCNode* from, TR_CISCNode* to, TR_CISCNode* moveTo, char* debugStr)
{
    if (showMesssagesStdout()) {
...
bool TR_CISCTransformer::verifyCandidate()
{
    ...
    for (b = bi.getFirst(); b; b = bi.getNext()) {
        while (true) {
            if (!le) {
                if (trace())
                    traceMsg(comp(), "Cannot find TR::BBStart of block_%d in the region\n", b->getNumber());
...
bool TR_CISCTransformer::findFirstNode(TR::TreeTop** retTree, TR::Node** retNode, TR::Block** retBlock)
{
    ...
    *retBlock = block;
    if (trace())
        traceMsg(comp(), "First node in candidate region - node: %p block_%d: %p\n", trNode, block->getNumber(), block);
    return true;
}
...
//*****************************************************************************************
void TR_CISCTransformer::removeEdgesExceptFor(
    TR::CFGEdgeList* succList, TR::Block* srcBlock, TR::Block* exceptDestBlock)
{
...
//*****************************************************************************************
void TR_CISCTransformer::setEdges(
    TR::CFGEdgeList* succList, TR::Block* srcBlock, TR::Block* destBlock0, TR::Block* destBlock1)
{
...
TR::Block* TR_CISCTransformer::setSuccessorEdges(TR::Block* block, TR::Block* target0, TR::Block* target1)
{
    ...
    if (trace()) {
        traceMsg(comp(), "setSuccessorEdges for block_%d [%p]: tgt0=%d tgt1=%d\n", block->getNumber(), block,
            target0->getNumber(), target1->getNumber());
    }
...
//*****************************************************************************************
TR::Block* TR_CISCTransformer::modifyBlockByVersioningCheck(
    TR::Block* block, TR::TreeTop* startTop, TR::Node* lengthNode, List<TR::Node>* guardList)
{
    ...
    if (versionLength >= 1) // Skip if versionLength is less than 1.
    {
        ...
        if (lengthNode->getOpCode().isLong()) {
            ...
            appender.add(TR::Node::createif(
                TR::ificmple, lengthNode, TR::Node::create(lengthNode, TR::iconst, 0, versionLength)));
        }
    }
...
//*****************************************************************************************
TR::Block* TR_CISCTransformer::modifyBlockByVersioningCheck(
    TR::Block* block, TR::TreeTop* startTop, List<TR::Node>* guardList)
{
...
//*****************************************************************************************
TR::Block* TR_CISCTransformer::cloneLoopBodyForPeel(
    TR::Block** firstBlock, TR::Block** lastBlock, TR_CISCNode* cmpifCISCNode)
{
    TR::CFG* cfg = comp()->getFlowGraph();
    cfg->setStructure(0);
...
bool TR_CISCTransformer::compareTrNodeTree(TR::Node* a, TR::Node* b)
{
    ...
    if (a->getOpCode().isLoadConst()) {
        ...
        case TR::sconst:
            ...
            break;
        case TR::cconst:
            if (a->getConst<uint16_t>() != b->getConst<uint16_t>())
                return false;
            break;
        default:
            return false;
...
TR::Block* TR_CISCTransformer::insertBeforeNodes(TR::Block* block)
{
    ...
    if (trace())
        traceMsg(comp(), "insertBeforeNodes added %d node(s) to block_%d [%p]\n", count, block->getNumber(), block);
    if (last && last->getOpCode().isBranch()) {
        ...
        for (auto edge = block->getSuccessors().begin(); edge != block->getSuccessors().end(); ++edge) {
            ...
            if (to != branchDestinationBlock && to != orgNextBlock) {
                if (trace())
                    traceMsg(
                        comp(), "insertBeforeNodes added the edge (%d, %d).\n", newBlock->getNumber(), to->getNumber());
        ...
        if (trace())
            traceMsg(comp(), "insertBeforeNodes created block_%d [%p]\n", newBlock->getNumber(), newBlock);
        block = newBlock;
    }
    return block;
}
...
TR::Block* TR_CISCTransformer::insertAfterNodes(TR::Block* block, List<TR::Node>* l, bool prepend)
{
    ...
    if (trace())
        traceMsg(comp(), "insertAfterNodes adds %d node(s)\n", count);
    return block;
}
            ...
            if (analyzeT2P(n, defNode) & _T2P_MatchMask) {
                ...
                if (n->getOpcode() == TR::Case) {
                    ...
                    while (!child1->isInterestingConstant()) {
                        ...
                        if (!child1) {
                            if (trace())
                                traceMsg(comp(), "analyzeBoolTable failed for %p. (no single chain)\n", n->getChild(1));
                            return false;
                        }
                        if (!child1->isStoreDirect()) {
                            if (trace())
                                traceMsg(comp(), "analyzeBoolTable failed for %p. (%p is not store)\n", n->getChild(1),
                                    child1);
                            return false;
                        }
                    ...
                    if (!n->isOutsideOfLoop()) {
                        ...
                        if (exitTreeTop != thisDestination) {
                            if (trace() && exitTreeTop)
                                traceMsg(comp(),
                                    "analyzeBoolTable - found conflicting successors.  Delimiter checking node %d "
                                    "targets treetop: %p (!= %p) block_%d: %p\n",
...
int32_t TR_CISCTransformer::analyzeByteBoolTable(
    TR_CISCNode* boolTable, uint8_t* table256, TR_CISCNode* ignoreNode, TR::TreeTop** retSameExit)
{
    TR::StackMemoryRegion stackMemoryRegion(*trMemory());
...
int32_t TR_CISCTransformer::analyzeCharBoolTable(
    TR_CISCNode* boolTable, uint8_t* table65536, TR_CISCNode* ignoreNode, TR::TreeTop** retSameExit)
{
    TR::StackMemoryRegion stackMemoryRegion(*trMemory());
...
//*****************************************************************************
bool TR_CISCTransformer::getBCIndexMinMax(
    List<TR_CISCNode>* l, int32_t* _minIndex, int32_t* _maxIndex, int32_t* _minLN, int32_t* _maxLN, bool allowInlined)
{
                        ...
                        case TR::iload:
                            if (def = ch->getNodeIfSingleChain()) {
                                if ((def->getNumChildren() > 0)
                                    && def->getChild(
                                           0)) // There is a bug where def is a TR_entrynode, so getChild(0) is null.
                                {
...
//*****************************************************************************
bool TR_CISCTransformer::canConvertArrayCmpSign(
    TR::Node* storeNode, List<TR::TreeTop>* compareIfs, bool* canConvertToArrayCmp)
{
    ...
    if (!info.IsZero()) {
        ...
        for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
            ...
            if (useNode->getReferenceCount() > 1) {
                if (trace())
                    traceMsg(comp(), "canConvertArrayCmpSign failed because ReferenceCount > 1. %p\n", useNode);
                return false;
            }
            ...
            if (!parentNode) {
                if (trace())
                    traceMsg(comp(), "canConvertArrayCmpSign failed because parentNode is NULL. %p\n", useNode);
                return false;
            }
            ...
            if (parentNode->getOpCode().isStoreDirect()) {
                if (!canConvertArrayCmpSign(parentNode, compareIfs, &convertToArrayCmp)) {
                    if (trace())
                        traceMsg(comp(), "canConvertArrayCmpSign failed because canConvertArrayCmpSign(p) failed. %p\n",
                            useNode);
                    return false;
                }
                ...
                if (theOtherChild->getInt() != 0 || theOtherChild->getOpCodeValue() != TR::iconst) {
                    if (trace())
                        traceMsg(comp(), "canConvertArrayCmpSign failed because theOtherChild is not iconst 0. %p\n",
                            useNode);
                    return false;
                }
                if (compareIfs)
...
bool TR_CISCTransformer::computeTopologicalEmbedding(TR_CISCGraph* P, TR_CISCGraph* T)
{
    ...
    if (!_candidateRegion || !verifyCandidate()
        ...
        if (trace())
            traceMsg(comp(), "computeTopologicalEmbedding: Graph transformations failed. (step 3)\n\n");
        registerCandidates();
    ...
    if (performTransformation(comp(), "%sReducing loop %d to %s\n", OPT_DETAILS,
        ...
        && !transformer(this)) {
        if (trace())
            traceMsg(comp(), "computeTopologicalEmbedding: IL Transformer failed. (step 4)\n\n");
        registerCandidates();
        _T->restoreListsDuplicator();
    ...
    if (trace() || showMesssagesStdout()) {
        ...
        if (minIndex <= maxIndex) {
            sprintf(tmpbuf, ", bcindex %d - %d linenumber %d - %d%s.", minIndex, maxIndex, minLN, maxLN,
                inlined ? " (inlined)" : "");
            bcinfo = tmpbuf;
        }
#endif
        ...
        if (trace())
            traceMsg(comp(), "***** Transformed *****, %s, %s, %s, loop:%d%s\n",
...
static void traceConflictingBranches(TR_CISCTransformer* opt, TR_CISCNode* pn, List<TR_CISCNode>* matches)
{
    ...
    traceMsg(comp, "Pattern node %d (%s) has conflicting branches:", pn->getID(),
    ...
    for (TR_CISCNode* tn = ti.getFirst(); tn != NULL; tn = ti.getNext()) {
        traceMsg(
            comp, "%s %d (%s)", first ? "" : ",", tn->getID(), TR_CISCNode::getName((TR_CISCOps)tn->getOpcode(), comp));
        first = false;
    }
...
bool TR_CISCTransformer::embeddingHasConflictingBranches()
{
    ...
    for (int32_t dag = 0; dag < dagCount; dag++) {
        ...
        for (TR_CISCNode* pn = pi.getFirst(); pn != NULL; pn = pi.getNext()) {
            ...
            bool isIf = op == (uint32_t)TR_ifcmpall || (op < (uint32_t)TR::NumIlOps && pn->getIlOpCode().isIf());
...
void TR_CISCTransformer::insertBitsKeepAliveCalls(TR::Block* block)
{
    ...
    for (TR_BitsKeepAliveInfo* info = bi.getFirst(); info != NULL; info = bi.getNext()) {
        ...
        if (trace()) {
            ...
            traceMsg(comp(),
                "\t\tInserting KeepAlive call clone node: %p from block %d [%p] node: %p into block :%d %p\n", callNode,
                keepAliveBlock->getNumber(), keepAliveBlock, tt->getNode(), block->getNumber(), block);
        }
    }
}
