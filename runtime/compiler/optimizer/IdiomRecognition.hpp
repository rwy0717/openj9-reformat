...
public:
    ...
    void setDest(TR_CISCNode* v)
    {
        ...
        if (v->_latestDest) {
            TR_ASSERT(
                v->_latestDest->_dest.find(v), "TR_CISCNode::setDest(), input variable is not found in _latestDest");
...
public:
    TR_PCISCNode(
        TR_Memory* m, uint32_t opc, TR::DataType dt, uint16_t id, int16_t dagId, uint16_t ncfgs, uint16_t nchildren)
...
public:
    ...
    void setImportantNodes(
        TR_CISCNode* n1, TR_CISCNode* n2, TR_CISCNode* n3, TR_CISCNode* n4, TR_CISCNode* n5, TR_CISCNode* n6)
    {
        setImportantNode(0, n1);
        setImportantNode(1, n2);
        setImportantNode(2, n3);
...
public:
    ...
    static bool searchNodeInTrees(
        TR::Node* top, TR::Node* target, TR::Node** retParent = NULL, int* retChildNum = NULL);
    ...
    bool makeLists();
    int32_t analyzeConnectionOnePairChild(
    ...
    int32_t analyzeByteBoolTable(
        TR_CISCNode* booltable, uint8_t* table256, TR_CISCNode* ignoreNode = 0, TR::TreeTop** retSameExit = 0);
    int32_t analyzeCharBoolTable(
        TR_CISCNode* booltable, uint8_t* table65536, TR_CISCNode* ignoreNode = 0, TR::TreeTop** retSameExit = 0);
    ...
    enum {
        ...
        _Embed
        = (_Desc | 0x4) // (1) the labels of p and t are equivalent and (2) descendents of t include all children of p.
    };
    ...
    TR::Block* modifyBlockByVersioningCheck(
        TR::Block* block, TR::TreeTop* startTop, TR::Node* lengthNode, List<TR::Node>* guardList = NULL);
