    ...
    if (!cacheInitialized) {
        ...
        if (stringClass != NULL) {
            ...
            if (comp->cg()->fej9()->getInstanceFieldOffset(stringClass, "value", "[C") != ~0) {
                cacheValue
                    = IS_STRING_COMPRESSION_ENABLED_VM(static_cast<TR_J9VMBase*>(comp->fe())->getJ9JITConfig()->javaVM);
            }
        }
    }
...
bool ChangeAlignmentOfRegion(TR_CISCTransformer* trans)
{
    ...
    for (t = T->getEntryNode(); t->isOutsideOfLoop();) {
        if (trans->analyzeT2P(t, pTop) & _T2P_MatchMask) {
            ...
            if (!chk->isOutsideOfLoop()) {
                if (disptrace)
                    traceMsg(comp, "ChangeAlignmentOfRegion : (t:%d p:%d) no need to change alignment\n", t->getID(),
                        pTop->getID());
        ...
        if (t->getNumSuccs() < 1) {
            if (disptrace)
                traceMsg(comp, "ChangeAlignmentOfRegion : #succs of tID:%d is 0\n", t->getID());
    ...
    if (disptrace)
        traceMsg(comp, "ChangeAlignmentOfRegion : tTop %d, pTop %d\n", t->getID(), pTop->getID());
    ...
    if (disptrace)
        traceMsg(comp, "ChangeAlignmentOfRegion : foundNode %d, lastNode %d\n", foundNode->getID(), lastNode->getID());
    if (branchCount > 0 && !lastNode->getIlOpCode().isBranch()) {
        if (disptrace)
            traceMsg(comp, "Fail: there is a branch in the region. lastNode must be a branch node.\n");
        return changed;
    }
    for (t = firstNode;; t = t->getSucc(0)) {
        r.append(t);
...
//*****************************************************************************************
TR_CISCNode* analyzeMoveNodeForward(
    TR_CISCTransformer* trans, List<TR_CISCNode>* l, TR_CISCNode* n, List<TR_CISCNode>* tgt)
{
    const bool disptrace = DISPTRACE(trans);
                    ...
                    if (trNode->getOpCode().isTreeTop()) {
                        if (trNode->getOpCode().isStoreDirect()) {
                            if (!generateCompensation0) {
                                ...
                                if (disptrace)
                                    traceMsg(comp,
                                        "analyzeMoveNodeForward: append the tree of 0x%p into "
                                        "AfterInsertionIdiomList\n",
                                        trNode);
        ...
        for (tParent = tParentIter.getFirst(); tParent; tParent = tParentIter.getNext()) {
            ...
            if (tParent->isStoreDirect() && !tParent->isNegligible()) {
                ...
                if (!storedVariable)
                    ...
                    if (DISPTRACE(trans))
                        traceMsg(
                            comp, "moveStoreOutOfLoopForward failed because all variables of stores are not same.\n");
                    success0 = false;
...
endSpecial0:
    ...
    if (targetList.isEmpty()) {
        if (DISPTRACE(trans))
            traceMsg(comp, "moveStoreOutOfLoopForward failed because targetList is empty.\n");
        success0 = false;
    }
    ...
    if (success0) {
        ...
        if (DISPTRACE(trans))
            traceMsg(comp, "moveStoreOutOfLoopForward: Target nodes ixload=%d, aload=%d, iload=%d\n",
        ...
        if (ixload && aload && iload && (iload->isLoadVarDirect() || iload->getOpcode() == TR_variable)) {
            ...
            if (DISPTRACE(trans))
                traceMsg(comp, "moveStoreOutOfLoopForward adds %d into compensation code [0] and [1]\n", t->getID());
            for (; t; t = ti.getNext())
...
//
static bool indicesAndStoresAreConsistent(
    TR::Compilation* comp, TR::Node* lhsSrcNode, TR::Node* rhsSrcNode, TR_CISCNode* lhsNode, TR_CISCNode* rhsNode)
{
...
static int32_t checkForPostIncrement(
    TR::Compilation* comp, TR::Block* loopHeader, TR::Node* loopCmpNode, TR::Symbol* ivSym)
{
...
static bool ivIncrementedBeforeBoolTableExit(
    TR::Compilation* comp, TR::Node* boolTableExit, TR::Block* entryBlock, TR::SymbolReference* ivSymRef)
{
...
bool CISCTransform2FindBytes(TR_CISCTransformer* trans)
{
    ...
    if (isLoopPreheaderLastBlockInMethod(comp, block)) {
        traceMsg(
            comp, "Bailing CISCTransform2FindBytes due to null TT - might be a preheader in last block of method\n");
        return false;
    }
    ...
    if (exitIfRep) {
        if (exitIfRep != trans->getP2TInLoopIfSingle(P->getImportantNode(1))) {
            if (disptrace)
                traceMsg(comp, "Give up because of multiple candidates of ificmpge.\n");
            return false;
        }
        bool isDecrement;
    ...
    if (!target) // multiple successors
    {
        ...
        if (listT->isEmpty() || trans->getNumOfBBlistSucc() != 2) {
            if (disptrace)
                traceMsg(comp,
                    "Currently, CISCTransform2FindBytes allows only the case where there is an ificmpge node and "
                    "successor is 2.\n");
            return false;
        }
    }
    ...
    if (!isTRT2Char) {
        if ((count = trans->analyzeByteBoolTable(pBoolTable, tmpTable, P->getImportantNode(1), &retSameExit)) <= 0) {
            ...
            if (!go) {
                if (disptrace)
                    traceMsg(comp, "analyzeByteBoolTable failed.\n");
        ...
        if ((count = trans->analyzeCharBoolTable(pBoolTable, tmpTable, P->getImportantNode(1), &retSameExit)) <= 0) {
            ...
            if (supportsSRSTU && // Confirm that the processor has the SRSTU instruction.
                ...
                if (disptrace)
                    traceMsg(comp, "analyzeCharBoolTable failed.\n");
                return false; // fail to analyze
            }
        } else {
                            ...
                            // delimiter.
            {
                if (disptrace && count > 1)
                    traceMsg(comp,
                        "Multiple exit conditions for a char array. We can implement this case using the TRTE "
                        "instruction on z6.\n");
    ...
    if (count == -1) // single delimiter which is not constant value
    {
        ...
        if (disptrace)
            traceMsg(comp, "Single non-constant delimiter found.  Setting %p as tableNode.\n",
    ...
    } else if (count == 1) // single delimiter
    {
        ...
        if (disptrace)
            traceMsg(comp, "Single delimiter found.  Setting 'iconst %d' [%p] as tableNode.\n", i,
    ...
    if (listT->isEmpty()) {
        ...
        if (disptrace)
            traceMsg(comp, "Loop has TR::ificmpge for comparing the index.\n");
        TR_CISCNode* lenNode;
    ...
    if (isNeedGenIcmpge) {
        ...
        if (disptrace)
            traceMsg(comp, "Block: okDest=%d failDest=%d\n", okDest->getEnclosingBlock()->getNumber(),
        ...
        icmpgeNode = TR::Node::createif(
            TR::ificmplt, TR::Node::createWithSymRef(indexRepNode, TR::iload, 0, indexVarSymRef), lenRepNode, okDest);
    }
    ...
    if (listT->isEmpty()) // no NULLCHK
    {
        ...
        if (disptrace)
            traceMsg(comp, "NULLCHK is found!\n");
...
TR_PCISCGraph* makeTRT2ByteGraph(TR::Compilation* c, int32_t ctrl)
{
    ...
    TR_PCISCNode* arrayLoad
        = createIdiomArrayLoadInLoop(tgt, ctrl, 1, bndChk, TR::cloadi, TR::Int16, charArray, iv, aHeader, mulFactor);
...
bool CISCTransform2NestedArrayFindBytes(TR_CISCTransformer* trans)
{
    ...
    TR_ASSERT(
        trans->isEmptyAfterInsertionIdiomList(0) && trans->isEmptyAfterInsertionIdiomList(1), "Not implemented yet!");
    ...
    if (listT->isEmpty()) {
        ...
        if (disptrace)
            traceMsg(comp, "TR::ificmpge for comaring the index is found!\n");
        TR_CISCNode* lenNode;
        TR::Node* lenRepNode;
        if (listT->isSingleton()) {
...
TR_PCISCGraph* makeTRT4NestedArrayGraph(TR::Compilation* c, int32_t ctrl)
{
    ...
    TR_PCISCNode* mapAload = createIdiomArrayLoadInLoop(
        tgt, ctrl, 1, bu2iNode, TR_ibcload, TR::NoType, mapArray, bu2iNode, aHeader, mulFactor);
    TR_PCISCNode* b2iNode = new (PERSISTENT_NEW)
...
bool CISCTransform2CopyingTROx(TR_CISCTransformer* trans)
{
    ...
    if (isOutputChar) {
        ...
        if (sx == TR_maybe) {
            traceMsg(comp, "Bailing CISCTransform2CopyingTROx : unknown integer conversion\n");
            return false;
        }
    ...
    if (additionHigh) {
        ...
        if (additionHigh->getChild(0) != loadResult && additionHigh->getChild(1) != loadResult) {
            traceMsg(comp, "Bailing CISCTransform2CopyingTROx : inscrutable iadd\n");
            return false;
        }
    }
    ...
    */
    ...
    if (isLoopPreheaderLastBlockInMethod(comp, block)) {
        traceMsg(
            comp, "Bailing CISCTransform2CopyingTROx due to null TT - might be a preheader in last block of method\n");
        return false;
    }
    ...
    if (trans->countGoodArrayIndex(indexVarSymRef) == 0) {
        if (disptrace)
            traceMsg(comp, "countGoodArrayIndex failed for %p\n", indexRepNode);
        return false;
    }
    ...
    if (!target) // multiple successors
    {
        ...
        if (trans->getNumOfBBlistSucc() != 2) {
            if (disptrace)
                traceMsg(comp, "current restrictions. The number of successors is %d\n", trans->getNumOfBBlistSucc());
            return false;
        }
    }
    ...
    if (comp->cg()->getSupportsArrayTranslateTROTNoBreak() || comp->cg()->getSupportsArrayTranslateTROT()) {
        ...
        if (termchar != 0) {
            ...
            if (isSignExtending) {
                traceMsg(comp, "Bailing CISCTransform2CopyingTROx due to sign-extension\n");
                return false;
            }
        }
    } else {
    ...
    if (trans->getOffsetOperand1()) {
        ...
        if (disptrace)
            traceMsg(comp, "The array header const of inputNode %p is modified. (offset=%d)\n", inputNode,
    ...
    if (trans->getOffsetOperand2()) {
        ...
        TR_ASSERT(constLoad, "Not implemented yet");
        if (disptrace)
            traceMsg(comp, "The array header const of outputNode %p is modified. (offset=%d)\n", outputNode, offset);
    }
    ...
    if (dstIndexRepNode) {
        ...
        if (dstIVStore && dstIVStore->getOpCode().hasSymbolReference()
            ...
            if (pattern == 1) {
                TR::Node* dstAddCountNode = createOP2(
                    comp, TR::iadd, TR::Node::createLoad(dstIndexRepNode, dstIndexVarSymRef), translateNode);
                if (dstIvNeedsUpdate)
                    dstAddCountNode
...
static TR_YesNoMaybe isSignExtendingCopyingTROx(TR_CISCTransformer* trans)
{
    ...
    if (!firstOp.isInteger() && !firstOp.isUnsigned()) {
        traceMsg(comp, "isSignExtendingCopyingTROx: conversion through non-integer type\n");
        return TR_maybe;
    }
...
TR_PCISCGraph* makeCopyingTROOSpecialGraph(TR::Compilation* c, int32_t ctrl)
{
    ...
    TR_PCISCNode* nn0 = createIdiomArrayStoreInLoop(
        tgt, ctrl | CISCUtilCtl_NoConversion, 1, n5, TR::bstorei, TR::Int8, v2, idx1, cmah, mulFactor, n5);
    ...
    static int versionLength
        = versionLengthStr ? atoi(versionLengthStr) : (TR::Compiler->target.cpu.isPower() ? 0 : 19);
...
TR_PCISCGraph* makeCopyingTROxGraph(TR::Compilation* c, int32_t ctrl, int pattern)
{
    ...
    TR_PCISCNode* charAddr = createIdiomArrayStoreInLoop(
        tgt, ctrl, 1, add, TR_ibcstore, TR::NoType, charArray, idx1, aHeader1, elemSize, add);
...
bool CISCTransform2TROTArray(TR_CISCTransformer* trans)
{
    ...
    if (isLoopPreheaderLastBlockInMethod(comp, block)) {
        traceMsg(
            comp, "Bailing CISCTransform2TROTArray due to null TT - might be a preheader in last block of method\n");
        return false;
    }
    ...
    TR::Node* indexNode = TR::Node::createWithSymRef(indexRepNode, TR::iload, 0, indexVarSymRef);
    TR::Node* lengthNode = createI2LIfNecessary(
        comp, trans->isGenerateI2L(), createOP2(comp, TR::isub, cmpRepNode->getChild(1)->duplicateTree(), indexNode));
    ...
    TR::TreeTop* dstIndVarUpdateTreeTop = NULL;
    if (dstIndexRepNode) {
        dstIndVarUpdateTreeTop = TR::TreeTop::create(
            comp, createStoreOP2(comp, dstIndexVarSymRef, TR::iadd, dstIndexVarSymRef, lengthTRxx, dstIndexRepNode));
    }
...
TR_PCISCGraph* makeTROTArrayGraph(TR::Compilation* c, int32_t ctrl)
{
    ...
    if (ctrl & CISCUtilCtl_64Bit) {
        ...
        mapAddr = createIdiomCharArrayLoadInLoop(
            tgt, ctrl | CISCUtilCtl_NoI2L, 1, convNode, map, convNode, aHeader, elemSize);
    } else {
        convNode = new (PERSISTENT_NEW)
    ...
    TR_PCISCNode* exitTest = new (PERSISTENT_NEW) TR_PCISCNode(
        c->trMemory(), TR::ificmpeq, TR::NoType, tgt->incNumNodes(), 1, 2, 2, c2iNode, c2iNode, exitValue); // optional
...
bool CISCTransform2CopyingTRTx(TR_CISCTransformer* trans)
{
    ...
    if (isLoopPreheaderLastBlockInMethod(comp, block)) {
        traceMsg(
            comp, "Bailing CISCTransform2CopyingTRTx due to null TT - might be a preheader in last block of method\n");
        return false;
    }
    ...
    if (trans->countGoodArrayIndex(indexVarSymRef) == 0
        ...
        if (disptrace)
            traceMsg(comp, "countGoodArrayIndex failed for %p, %p\n", indexRepNode, dstIndexRepNode);
        return false;
    }
    TR_ScratchList<TR::Node> variableList(comp->trMemory());
    variableList.add(indexRepNode);
    ...
    if (!target) // multiple successors
    {
        ...
        if (trans->getNumOfBBlistSucc() > 3) {
            if (disptrace)
                traceMsg(comp, "trans->getNumOfBBlistSucc() is %d.", trans->getNumOfBBlistSucc());
            return false;
        }
    }
    ...
    if ((count = trans->analyzeCharBoolTable(P->getImportantNode(0), tmpTable, P->getImportantNode(1), &retSameExit))
        <= 0) {
        if (disptrace)
            traceMsg(comp, "trans->analyzeCharBoolTable failed\n");
        return false;
    }
    ...
    if (necessaryCmp.getSize() >= 3) {
        if (disptrace)
            traceMsg(comp,
                "Too many (%d) loop exit tests to transform correctly.  Transformation only supports up to 2.  "
                "Abandoning reduction.\n",
    ...
    if (!testExitIF(icmpgeCISCnode1->getOpcode(), &isDecrement, &modLength)) {
        if (disptrace)
            traceMsg(comp, "testExitIF for icmpgeCISCnode1 failed\n");
        return false;
    }
    if (isDecrement) {
        if (disptrace)
            traceMsg(comp, "Not support a decrement loop. (icmpgeCISCnode1)\n");
        return false;
    }
    TR_ASSERT(modLength == 0 || modLength == 1, "error");
    ...
    if (cmpVarSymRef == NULL) {
        if (disptrace)
            traceMsg(comp, "Unable to determine the sym ref of induction variable in loop termination node.\n");
        return false;
    }
    ...
    if (P->getImportantNode(4)) {
        if (icmpgeCISCnode2) {
            if (disptrace)
                traceMsg(comp, "Not support yet more than three if-statements. (1)\n");
            return false;
        }
        icmpgeCISCnode2 = trans->getP2TInLoopIfSingle(P->getImportantNode(4));
        if (!icmpgeCISCnode2) {
            if (disptrace)
                traceMsg(comp, "Not support yet more than three if-statements. (2)\n");
            return false;
        }
    }
    ...
    if (icmpgeCISCnode2) {
        if (!testExitIF(icmpgeCISCnode2->getOpcode(), &isDecrement, &modLength)) {
            if (disptrace)
                traceMsg(comp, "testExitIF for icmpgeCISCnode2 failed\n");
            return false;
        }
        if (isDecrement) {
            if (disptrace)
                traceMsg(comp, "Not support a decrement loop. (icmpgeCISCnode2)\n");
            return false;
        }
        TR_ASSERT(modLength == 0 || modLength == 1, "error");
        ...
        TR::Node* lenTmpNode2 = createOP2(
            comp, TR::isub, lenRepNode2, TR::Node::createWithSymRef(indexRepNode, TR::iload, 0, icmpgeSymRef2));
    ...
    // prepare nodes that add the number of elements (which was translated) into the induction variables
    TR::Node* addCountNode = createOP2(
        comp, TR::iadd, TR::Node::createWithSymRef(indexRepNode, TR::iload, 0, indexVarSymRef), translateNode);
    if (ivNeedsUpdate)
    ...
    if (icmpgeCISCnode2) {
        ...
        if (disptrace)
            traceMsg(comp, "Now assuming that all exits of booltable are identical.\n");
        ...
        if (disptrace) {
            if (okDest == NULL)
                traceMsg(comp, "error, okDest == NULL!\n");
            if (failDest == NULL)
                traceMsg(comp, "error, failDest == NULL!\n");
            if (failDest2 == NULL)
                traceMsg(comp, "error, failDest2 == NULL!\n");
        }
        ...
        if (isNeedGenIcmpge) {
            if (disptrace)
                traceMsg(comp,
                    "Now assuming that all exits of booltable are identical and the exit of icmpge points "
                    "different.\n");
...
TR_PCISCGraph* makeCopyingTRTxGraph(TR::Compilation* c, int32_t ctrl, int pattern)
{
    ...
    TR_PCISCNode* exitTest = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR_booltable, TR::NoType, tgt->incNumNodes(), 1, 2, 1, c2iNode, c2iNode);
    tgt->addNode(exitTest);
    TR_PCISCNode* byteAddr = createIdiomArrayStoreInLoop(
        tgt, ctrl, 1, exitTest, TR_ibcstore, TR::NoType, byteArray, idx1, aHeader1, mulFactor, c2iNode);
    ...
    static int versionLength
        = versionLengthStr ? atoi(versionLengthStr) : (TR::Compiler->target.cpu.isPower() ? 0 : 15);
    tgt->setVersionLength(versionLength); // depending on each architecture
...
TR_PCISCGraph* makeCopyingTRTxThreeIfsGraph(TR::Compilation* c, int32_t ctrl)
{
    ...
    tgt->addNode(loopTest);
    TR_PCISCNode* byteAddr = createIdiomArrayStoreInLoop(
        tgt, ctrl, 1, loopTest, TR_ibcstore, TR::NoType, byteArray, j, aHeader1, mulFactor, c2iNode);
...
TR_PCISCGraph* makeCopyingTRTTSpecialGraph(TR::Compilation* c, int32_t ctrl)
{
    ...
    static int versionLength
        = versionLengthStr ? atoi(versionLengthStr) : (TR::Compiler->target.cpu.isPower() ? 0 : 20);
    tgt->setVersionLength(versionLength); // depending on each architecture
    return tgt;
}
...
bool CISCTransform2TRTOArray(TR_CISCTransformer* trans)
{
    ...
    if (isLoopPreheaderLastBlockInMethod(comp, block)) {
        traceMsg(
            comp, "Bailing CISCTransform2TRTOArray due to null TT - might be a preheader in last block of method\n");
        return false;
    }
...
TR_PCISCGraph* makeTRTOArrayGraph(TR::Compilation* c, int32_t ctrl)
{
    ...
    tgt->addNode(convNode);
    TR_PCISCNode* mapAddr = createIdiomArrayLoadInLoop(
        tgt, ctrl | CISCUtilCtl_NoI2L, 1, convNode, TR::bloadi, TR::Int8, map, convNode, aHeader, mulFactor);
    ...
    tgt->addNode(exitTest);
    TR_PCISCNode* byteAddr = createIdiomArrayStoreInLoop(
        tgt, ctrl, 1, exitTest, TR::bstorei, TR::Int8, byteArray, idx1, aHeader, mulFactor, b);
    TR_PCISCNode* iStore = createIdiomDecVarInLoop(tgt, ctrl, 1, byteAddr, i, increment);
    TR_PCISCNode* jStore = createIdiomIncVarInLoop(tgt, ctrl, 1, iStore, j, i, offset); // optional
    TR_PCISCNode* loopTest = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::ificmpge, TR::NoType, tgt->incNumNodes(), 1, 2, 2, jStore, i, end);
...
TR_PCISCGraph* makeTRTOArrayGraphSpecial(TR::Compilation* c, int32_t ctrl)
{
    ...
    tgt->addNode(exitTest);
    TR_PCISCNode* byteAddr = createIdiomArrayStoreInLoop(
        tgt, ctrl, 1, exitTest, TR::bstorei, TR::Int8, byteArray, idx1, aHeader, mulFactor, b2iNode);
    TR_PCISCNode* iStore = createIdiomDecVarInLoop(tgt, ctrl, 1, byteAddr, i, increment);
...
static StatusArrayStore checkArrayStore(
    TR::Compilation* comp, TR::Node* inputNode, TR::Node* outputNode, int elementSize, bool isForward)
{
    ...
    if (inputNode->getFirstChild()->getSymbol()->getRegisterMappedSymbol()
        ...
        if (!disableArraycopyOverlapTest && comp->getOptLevel() >= hot
            && performTransformation(comp,
                   ...
                   OPT_DETAILS)) {
            dumpOptDetails(
                comp, "src and dest are the same, generating guard code 'if (src/dest subranges nonoverlapping)'\n");
...
public:
    SubrangeOverlapTestGenerator(
        TR::Compilation* comp, TR::Node* arraycopy, TR::Node* byteLength, bool is64Bit, int elementSize);
    TR::Node* generate();
...
void SubrangeOverlapTestGenerator::simplifyConstSub()
{
    ...
    if (!performTransformation(
            _comp, "%sSimplifying arraycopy element range overlap test by looking through sub const\n", OPT_DETAILS))
        return;
...
void SubrangeOverlapTestGenerator::simplifyConstMul()
{
    ...
    // Ask permission to transform.
    if (!performTransformation(
            _comp, "%sSimplifying arraycopy element range overlap test by looking through mul const\n", OPT_DETAILS))
        return;
...
void SubrangeOverlapTestGenerator::simplifyI2L()
{
    ...
    // Ask permission to transform.
    if (!performTransformation(
            _comp, "%sSimplifying arraycopy element range overlap test by looking through i2l\n", OPT_DETAILS))
        return;
    ...
    TR::Node* exitVarRepNode, TR::Node* variableORconstRepNode)
{
    ...
    if (isLoopPreheaderLastBlockInMethod(comp, block)) {
        traceMsg(
            comp, "Bailing CISCTransform2ARrayCopySub due to null TT - might be a preheader in last block of method\n");
        return false;
    }
    ...
    if (!inLoadCISCNode || !inStoreCISCNode) {
        if (DISPTRACE(trans))
            traceMsg(comp, "CISCTransform2ArrayCopy failed. inLoadCISCNode %x inStoreCISCNode %x\n", inLoadCISCNode,
                inStoreCISCNode);
        return false;
    }
    ...
    if (!getMultiplier(trans, P->getImportantNode(2), &mulFactorNode, &elementSize, inLoadNode->getType())) {
        if (DISPTRACE(trans))
            traceMsg(comp, "CISCTransform2ArrayCopy getMultiplier failed.\n");
        return false;
    }
    ...
    if (!testExitIF(cmpIfAllCISCNode->getOpcode(), &isDecrementRet, &modLength, &modStartIdx)) {
        if (DISPTRACE(trans))
            traceMsg(comp, "CISCTransform2ArrayCopy testExitIF failed.\n");
        return false;
    }
    ...
    if (!trans->analyzeArrayIndex(indexVarSymRef)) {
        if (DISPTRACE(trans))
            traceMsg(comp, "analyzeArrayIndex failed. %x\n", indexRepNode);
        return false;
    }
    TR::SymbolReference* dstIndexVarSymRef = dstIndexRepNode ? dstIndexRepNode->getSymbolReference() : NULL;
    if (indexVarSymRef == dstIndexVarSymRef)
    ...
    if (!trans->searchNodeInTrees(inputNode, indexRepNode)) {
        if (DISPTRACE(trans))
            traceMsg(comp, "searchNodeInTrees for inputNode failed.\n");
        return false;
    }
    if (!trans->searchNodeInTrees(
            outputNode, dstIndexVarSymRef ? convertStoreToLoad(comp, dstIndexRepNode) : indexRepNode)) {
        if (DISPTRACE(trans))
            traceMsg(comp, "searchNodeInTrees for outputNode failed.\n");
        return false;
    }
    ...
    if (indexVarSymRef != exitVarSymRef && dstIndexVarSymRef != exitVarSymRef) {
        if (DISPTRACE(trans))
            traceMsg(comp, "CISCTransform2ArrayCopy cannot find exitVarSymRef correctly %x.\n", exitVarRepNode);
        return false;
    }
    ...
    int32_t postIncrement = checkForPostIncrement(
        comp, block, cmpIfAllCISCNode->getHeadOfTrNodeInfo()->_node, exitVarSymRef->getSymbol());
    ...
    bool needWriteBarrier = comp->getOptions()->needWriteBarriers() && (inStoreNode->getOpCodeValue() == TR::awrtbari);

    if (!comp->cg()->getSupportsReferenceArrayCopy() && needWriteBarrier) {
        if (DISPTRACE(trans))
            traceMsg(comp, "CISCTransform2ArrayCopy: needWriteBarrier but not getSupportsReferenceArrayCopy().\n");
        return false;
    }
    ...
    if (!needWriteBarrier) {
        ...
        arraycopy = TR::Node::createArraycopy(
            inputNode->getFirstChild(), outputNode->getFirstChild(), inputNode, outputNode, lengthNode);
    }
    ...
    if (statusArrayStore == GENERATE_ARRAY_ALIAS_TEST) {
        ...
        List<TR::Node> guardList(comp->trMemory());
        guardList.add(TR::Node::createif(
            TR::ifacmpeq, inputNode->getFirstChild()->duplicateTree(), outputNode->getFirstChild()->duplicateTree()));
        block = trans->modifyBlockByVersioningCheck(block, trTreeTop, lengthNode->duplicateTree(), &guardList);
    ...
    if (modLength || (postIncrement != 0)) {
        ...
        finalValue = createOP2(
            comp, TR::isub, finalValue, TR::Node::create(trNode, TR::iconst, 0, isDecrement ? incr : -incr));
    }
...
bool CISCTransform2ArrayCopyB2CorC2B(TR_CISCTransformer* trans)
{
    ...
    } else // type is a charToByte copy
    {
        ...
        if (dstIndexVarSymRef == exitVarSymRef) {
            ...
            updateTree1 = createStoreOP2(
                comp, indexVarSymRef, TR::iadd, indexVarSymRef, TR::Node::create(TR::idiv, 2, lengthNode, c2), trNode);
...
TR_PCISCGraph* makeMemCpyByteToCharGraph(TR::Compilation* c, int32_t ctrl)
{
    ...
    TR_PCISCNode* ns4 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::cstorei, TR::Int16, tgt->incNumNodes(), 1, 1, 2, ns3, ns1, ns3);
    tgt->addNode(ns4);
...
TR_PCISCGraph* makeMemCpyCharToByteGraph(TR::Compilation* c, int32_t ctrl)
{
    ...
    TR_PCISCNode* nl2 = new (PERSISTENT_NEW)
        TR_PCISCNode(c->trMemory(), TR::cloadi, TR::Int16, tgt->incNumNodes(), 1, 1, 1, nl1, nl1);
    tgt->addNode(nl2);
    ...
    tgt->addNode(ns14);
    TR_PCISCNode* ns20 = createIdiomArrayAddressIndexTreeInLoop(
        tgt, ctrl | CISCUtilCtl_NoI2L, 1, ns14, ns10->getChild(0)->getChild(0), cmah1, c1);
...
bool CISCTransform2ArrayCopyB2CBndchk(TR_CISCTransformer* trans)
{
    ...
    if (isExitVarChar) // The variable that checks the exit condition is for a 2-byte array.
    {
        ...
        updateTree1 = TR::Node::createWithSymRef(
            TR::istorei, 2, 2, indexLoadNode->getChild(0), endIndex, indexLoadNode->getSymbolReference());
...
bool CISCTransform2ArrayCopyC2BMixed(TR_CISCTransformer* trans)
{
    ...
    TR_ASSERT(
        searchNodeInBlock(flagIf->getSucc(1), LEloadMem) || searchNodeInBlock(flagIf->getSucc(1), BEloadMem), "error");
    ...
    if (DISPTRACE(trans))
        traceMsg(comp, "LEalongJumpPath = %d\n", LEalongJumpPath);
    ...
    TR::Node* BEMemCpy = TR::Node::createArraycopy(
        BELoadAddrTree, BEStoreAddrTree, createI2LIfNecessary(comp, trans->isGenerateI2L(), lengthNode));
    ...
    if (comp->cg()->getSupportsReverseLoadAndStore()) {
        ...
        ret = trans->searchNodeInTrees(
            isBig ? LEStoreAddrTree2 : LEStoreAddrTree, arrayHeaderConst, &replaceParent, &childNum);
        TR_ASSERT(ret, "error");
    ...
    blockLE->append(TR::TreeTop::create(
        comp, createStoreOP2(comp, dstIndexVarSymRef, TR::iadd, dstIndexVarSymRef, c2->duplicateTree(), trNode)));
    TR::Node* backIfLE = TR::Node::createif(
        TR::ificmplt, indexUpdateLE->getChild(0), variableORconstRepNode->duplicateTree(), blockLE->getEntry());
...
TR_PCISCGraph* makeMEMCPYChar2ByteMixedGraph(TR::Compilation* c, int32_t ctrl)
{
    ...
    TR_PCISCNode* bns0 = createIdiomArrayStoreInLoop(
        tgt, ctrl | CISCUtilCtl_ChildDirectConnected, 1, bns22, TR::bstorei, TR::Int8, v2, v3, cmah, c1, bns22);
    ...
    TR_PCISCNode* lns0 = createIdiomArrayStoreInLoop(
        tgt, ctrl | CISCUtilCtl_ChildDirectConnected, 1, lns22, TR::bstorei, TR::Int8, v2, v3, cmah1, c1, lns22);
...
bool CISCTransform2ArrayCopyC2BIf2(TR_CISCTransformer* trans)
{
    ...
    TR::Node* lenNode = createI2LIfNecessary(
        comp, trans->isGenerateI2L(), TR::Node::createWithSymRef(indexRepNode, TR::iload, 0, lengthByteTemp));
...
TR_PCISCGraph* makeMEMCPYChar2ByteGraph2(TR::Compilation* c, int32_t ctrl)
{
    ...
    TR_PCISCNode* ns20 = createIdiomArrayAddressIndexTreeInLoop(
        tgt, ctrl | CISCUtilCtl_NoI2L, 1, n6, ns10->getChild(0)->getChild(0), cmah1, c1);
    TR_PCISCNode* ns21 = createIdiomArrayAddressInLoop(tgt, ctrl, 1, ns20, v2, ns20);
    if ((ctrl & CISCUtilCtl_BigEndian)) {
        cvt1 = new (PERSISTENT_NEW)
            TR_PCISCNode(c->trMemory(), TR::s2b, TR::Int8, tgt->incNumNodes(), 1, 1, 1, ns21, nl2);
...
bool CISCTransform2ArrayCopyB2I(TR_CISCTransformer* trans)
{
    ...
    if (isLoopPreheaderLastBlockInMethod(comp, block)) {
        traceMsg(
            comp, "Bailing CISCTransform2ArrayCopyB2I due to null TT - might be a preheader in last block of method\n");
        return false;
    }
...
bool CISCTransform2PtrArraySet(TR_CISCTransformer* trans)
{
    ...
    if (trans->getNumOfBBlistBody() > 1) {
        if (trace)
            traceMsg(comp, "Need exactly 1 basic block\n");
        return false;
    }
    ...
    if (block->getNumberOfRealTreeTops() != 3) {
        if (trace)
            traceMsg(comp, "Need exactly 3 real treetops\n");
        return false;
    }
    ...
    if (!astore) {
        if (trace)
            traceMsg(comp, "astore missing\n");
        return false;
    }
    if (!Store) {
        if (trace)
            traceMsg(comp, "array element store missing\n");
        return false;
    }
    if (!ifcmp) {
        if (trace)
            traceMsg(comp, "if compare missing\n");
        return false;
    }
    ...
    if (!(astoreNode->getChild(0)->getChild(0) == StoreNode->getChild(0)
        ...
        if (trace)
            traceMsg(comp, "node trees not in required form\n");
        return false;
    }

    if (!ifcmpNode->getChild(0)->getOpCode().isLoadVar() && !ifcmpNode->getChild(1)->getOpCode().isLoadVar()) {
        if (trace)
            traceMsg(comp, "neither comparands are loadvar\n");
        return false;
    }
    ...
    if (ifcmpNode->getChild(0)->getOpCode().isLoadVar() ^ ifcmpNode->getChild(1)->getOpCode().isLoadVar()) {
        ...
        if (astoreNode->getChild(0) != nonLoadChild) {
            if (trace)
                traceMsg(comp, "iv is not a commoned child in if comparand\n");
            return false;
        }
    }
    ...
    if (!ifcmpNode->getOpCode().isCompareForOrder()
        ...
        if (trace)
            traceMsg(comp, "invalid compare condition\n");
        return false;
    }
    ...
    if (!StoreNode->getOpCode().isStoreIndirect()
        ...
        if (trace)
            traceMsg(comp,
                "array element store node is neither indirect store "
    ...
    if (StoreNode->getSize() != getAbs(increment)) {
        if (trace)
            traceMsg(comp, "increment size does not match element size\n");
        return false;
    }
    ...
    if (!endPtr) {
        if (trace)
            traceMsg(comp, "Could not get end pointer\n");
        return false;
    }
    ...
    if (increment < 0) {
        ...
        if (equal) {
            length = TR::Node::create(
                use64bit ? TR::ladd : TR::iadd, 2, length, use64bit ? TR::Node::lconst(1) : TR::Node::iconst(1));
        }
...
bool CISCTransform2ArraySet(TR_CISCTransformer* trans)
{
    ...
    if (isLoopPreheaderLastBlockInMethod(comp, block)) {
        traceMsg(
            comp, "Bailing CISCTransform2ArraySet due to null TT - might be a preheader in last block of method\n");
        return false;
    }
    ...
    if (!cmpIfAllCISCNode) {
        if (disptrace)
            traceMsg(comp, "Not implemented yet for multiple-if\n");
        return false;
    }
    ...
    for (inStoreCISCNode = ni.getFirst(); inStoreCISCNode; inStoreCISCNode = ni.getNext()) {
        if (!inStoreCISCNode->isOutsideOfLoop()) {
            ...
            if (!trans->comp()->cg()->getSupportsArraySet()
                ...
                dumpOptDetails(
                    comp, "the cg only supports arrayset to zero, but found a non-zero or non-constant value\n");
                return false;
            }
    ...
    if (indexVarSymRef != indexVar1SymRef) {
        ...
        if (!listStores.isSingleton()) {
            dumpOptDetails(
                comp, "Multiple induction variables with multiple stores not supported for arrayset transformation.\n");
            return false;
        }
        if (!isIncrement1) {
    ...
    for (inStoreNode = iteratorStores.getFirst(); inStoreNode; inStoreNode = iteratorStores.getNext()) {
        ...
        if (findAndOrReplaceNodesWithMatchingSymRefNumber(
                outputNode->getSecondChild(), NULL, indexVarSymRef->getReferenceNumber())) {
            loopIsIncrement = isIncrement0;
        } else {
            TR_ASSERT(findAndOrReplaceNodesWithMatchingSymRefNumber(
                          outputNode->getSecondChild(), NULL, indexVar1SymRef->getReferenceNumber()),
        ...
        if (!loopIsIncrement) // count-down loop
        {
            ...
            bool isFound = findAndOrReplaceNodesWithMatchingSymRefNumber(
                outputNode->getSecondChild(), lastLegalValue, indexVarSymRef->getReferenceNumber());
            if (!isFound && (indexVarSymRef != indexVar1SymRef))
                isFound = findAndOrReplaceNodesWithMatchingSymRefNumber(
                    outputNode->getSecondChild(), lastLegalValue, indexVar1SymRef->getReferenceNumber());
...
bool CISCTransform2Strlen16(TR_CISCTransformer* trans)
{
    ...
    if (block->getNumberOfRealTreeTops() != 2) {
        if (trace)
            traceMsg(comp, "Need exactly 2 real treetops\n");
        return false;
    }
...
TR_PCISCGraph* makeStrlen16Graph(TR::Compilation* c, int32_t ctrl)
{
    ...
    auto loopTest = new (PERSISTENT_NEW) TR_PCISCNode(
        c->trMemory(), TR_ifcmpall, TR::Int32, tgt->incNumNodes(), 1, 2, 2, conversion, conversion, nullChar);
    tgt->addNode(loopTest);
...
bool CISCTransform2ArrayCmp2Ifs(TR_CISCTransformer* trans)
{
    ...
    if (isLoopPreheaderLastBlockInMethod(comp, block)) {
        traceMsg(
            comp, "Bailing CISCTransform2ArrayCmp2Ifs due to null TT - might be a preheader in last block of method\n");
        return false;
    }
    ...
    getP2TTrRepNodes(
        trans, &src1IdxRepNode, &src2IdxRepNode, &src1BaseRepNode, &src2BaseRepNode, &variableORconstRepNode);
    if (!src2IdxRepNode)
    ...
    if (shrCount != 0) {
        equalLen = TR::Node::create(TR::ishr, 2, equalLen, TR::Node::create(equalLen, TR::iconst, 0, shrCount));
    }
...
bool CISCTransform2ArrayCmp(TR_CISCTransformer* trans)
{
    ...
    if (isLoopPreheaderLastBlockInMethod(comp, block, &preHeader)) {
        traceMsg(
            comp, "Bailing CISCTransform2ArrayCmp due to null TT - might be a preheader in last block of method\n");
        return false;
    }
    ...
    if (trans->isBlockInLoopBody(failDest->getNode()->getBlock())) {
        if (disptrace)
            traceMsg(
                comp, "CISCTransform2ArrayCmp failing transformer, ifcmpall test branch does not exit the loop.\n");
        return false;
    }
    ...
    if (true == (compareTo = trans->isCompareTo())) {
        if (!generateArraycmplen) {
            ...
            if (trans->canConvertArrayCmpSign(
                    trans->getP2TRep(P->getImportantNode(7))->getHeadOfTrNode(), &compareIfs, &canConvertToArrayCmp)) {
    ...
    if (inSrc1Node->getType() != inSrc2Node->getType()) {
        traceMsg(comp, "CISCTransform2ArrayCmp failed - Array access types differ. inSrc1: %s, inSrc2: %s\n",
    ...
    if (elementSize != expectedSize) {
        traceMsg(comp, "CISCTransform2ArrayCmp failed - Size Mismatch.  Element Size: %d, Expected Size: %d\n",
    ...
    if (generateArraycmplen) {
        ...
        TR::Node* arraycmplen = TR::Node::create(
            TR::arraycmp, 3, input1Node, input2Node, createI2LIfNecessary(comp, trans->isGenerateI2L(), lengthNode));
        arraycmplen->setArrayCmpLen(true);
        arraycmplen->setSymbolReference(comp->getSymRefTab()->findOrCreateArrayCmpSymbol());
        ...
        TR::Node* arraycmp = TR::Node::create(
            TR::arraycmp, 3, input1Node, input2Node, createI2LIfNecessary(comp, trans->isGenerateI2L(), lengthNode));
        ...
        if (compareTo) {
            ...
            if (generateArraycmpsign) {
                ...
                cmpIfNode = TR::Node::createif(
                    TR::ificmpeq, arraycmp, TR::Node::create(src1BaseRepNode, TR::iconst, 0, 0), okDest);
            } else {
                if (disptrace)
                    traceMsg(comp, "ArrayCmp: Convert compareTo into equals!\n");
    ...
    if (0 && isCompensateCode) {
        ...
        TR::Node* compareNode = TR::Node::createif(
            (TR::ILOpCodes)cmpIfAllCISCNode->getOpcode(), startNode, endNode, compensateBlock0->getEntry());
...
static TR::AutomaticSymbol* setPinningArray(
    TR::Compilation* comp, TR::Node* internalPtrStore, TR::Node* base, TR::Block* appendBlock)
{
...
bool CISCTransform2BitOpMem(TR_CISCTransformer* trans)
{
    ...
    if (isLoopPreheaderLastBlockInMethod(comp, block)) {
        traceMsg(
            comp, "Bailing CISCTransform2BitOpMem due to null TT - might be a preheader in last block of method\n");
        return false;
    }
    ...
    if (bitOpMem) { // src1 or src2 is equal to dest
        ...
        checkSrc3->append(TR::TreeTop::create(comp,
            TR::Node::createif(
                TR::ifacmpeq, createLoad(destBaseRepNode), createLoad(src1BaseRepNode), slowpad->getEntry())));

        // checkSrc4: if (dest.obj == src2.obj) goto slowpad
        checkSrc4->append(TR::TreeTop::create(comp,
            TR::Node::createif(
                TR::ifacmpeq, createLoad(destBaseRepNode), createLoad(src2BaseRepNode), slowpad->getEntry())));
...
bool CISCTransform2CountDecimalDigit(TR_CISCTransformer* trans)
{
    ...
    if (!constNode->getIlOpCode().isLoadConst()) {
        if (disptrace)
            traceMsg(comp, "%p is not isLoadConst().\n", constNode);
        return false;
    }
    ...
    if (inputVar->getDataType() == TR::Int64
        && (!TR::Compiler->target.cpu.isPower() && TR::Compiler->target.is32Bit())) {
        return false;
    }
    ...
    case TR::iflcmpeq:
        if (constNode->getOtherInfo() != 0) {
            if (disptrace)
                traceMsg(
                    comp, "The exit-if is TR::if*cmpeq but the constant value is %d.\n", constNode->getOtherInfo());
            return false;
        }
        break;
    case TR::ificmplt:
    case TR::iflcmplt:
        if (constNode->getOtherInfo() != 10) {
            if (disptrace)
                traceMsg(
                    comp, "The exit-if is TR::if*cmplt but the constant value is %d.\n", constNode->getOtherInfo());
            return false;
        }
    ...
    default:
        if (disptrace)
            traceMsg(comp, "The exit-if %p is not as expected. We may be able to implement this case.\n", ifcmp);
        return false;
    }
...
bool CISCTransform2LongToStringDigit(TR_CISCTransformer* trans)
{
    ...
    replaceParent->setAndIncChild(
        childNum, createOP2(comp, TR::isub, resultV2, TR::Node::create(trNode, TR::iconst, 0, -1)));
