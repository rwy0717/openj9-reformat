...
static int32_t getJ9InitialBytecodeSize(
    TR_ResolvedMethod* feMethod, TR::ResolvedMethodSymbol* methodSymbol, TR::Compilation* comp)
{
    ...
    TR_J9EstimateCodeSize::adjustEstimateForStringCompression(
        feMethod, size, TR_J9EstimateCodeSize::STRING_COMPRESSION_ADJUSTMENT_FACTOR);
...
static bool insideIntPipelineForEach(TR_ResolvedMethod* method, TR::Compilation* comp)
{
    ...
    // returned since IntPipeline$Head.forEach and IntPipeline.forEach needs to be inlined for JIT GPU. If not the method
    // name is checked to see if it is called accept. If the method in the chain just before IntPipeline.forEach is
    // accept, true is also returned This tries to inline accept and the methods inside accept IntPipelineHead.forEach
    // must also be inlined.
...
int32_t TR_J9InlinerPolicy::getInitialBytecodeSize(
    TR_ResolvedMethod* feMethod, TR::ResolvedMethodSymbol* methodSymbol, TR::Compilation* comp)
{
...
bool TR_J9InlinerPolicy::isMethodInSharedCache(TR_ResolvedMethod* interfaceMethod)
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(comp()->fe());
    TR_OpaqueClassBlock* clazz = interfaceMethod->classOfMethod();
    void* dummy;
    bool clazzInCache = fej9->sharedCache()->isPointerInSharedCache(
        (void*)fej9->getPersistentClassPointerFromClassPointer(clazz), dummy);
...
bool TR_J9InlinerPolicy::alwaysWorthInlining(TR_ResolvedMethod* calleeMethod, TR::Node* callNode)
{
    ...
    if (!strncmp(
            calleeMethod->classNameChars(), "java/util/concurrent/atomic/", strlen("java/util/concurrent/atomic/"))) {
        return true;
    }
...
void TR_J9InlinerUtil::adjustByteCodeSize(
...
TR::Node* TR_J9InlinerPolicy::createUnsafeAddressWithOffset(TR::Node* unsafeCall)
{
    if (TR::Compiler->target.is64Bit()) {
        ...
        return TR::Node::create(
            TR::aladd, 2, unsafeCall->getChild(1), TR::Node::create(TR::land, 2, unsafeCall->getChild(2), constNode));
    }
...
void TR_J9InlinerPolicy::createTempsForUnsafeCall(TR::TreeTop* callNodeTreeTop, TR::Node* unsafeCallNode)
{
    ...
    for (int32_t i = 0; i < unsafeCallNode->getNumChildren(); ++i) {
        ...
        debugTrace(
            tracer(), "Replacing callnode %p child %p with %p", unsafeCallNode, unsafeCallNode->getChild(i), value);
    ...
    bool isUnsafeGet)
{
    ...
    TR::Node* storeNode = TR::Node::createWithSymRef(
        comp()->il.opCodeForDirectStore(unsafeAddress->getDataType()), 1, 1, unsafeAddress, newSymbolReference);
    TR::TreeTop* storeTree = TR::TreeTop::create(comp(), storeNode);
    ...
    // Replace the old child with a load of the new sym ref
    unsafeAddress = TR::Node::createWithSymRef(
        unsafeAddress, comp()->il.opCodeForDirectLoad(unsafeAddress->getDataType()), 0, newSymbolReference);
    ...
    storeNode = TR::Node::createWithSymRef(
        comp()->il.opCodeForDirectStore(offset->getDataType()), 1, 1, offset, newSymbolReference);
    ...
    // Replace the old child with a load of the new sym ref
    offset = TR::Node::createWithSymRef(
        offset, comp()->il.opCodeForDirectLoad(offset->getDataType()), 0, newSymbolReference);
    ...
    if (!isUnsafeGet) {
        ...
        newSymbolReference = comp()->getSymRefTab()->createTemporary(comp()->getMethodSymbol(), dataType);
        storeNode = TR::Node::createWithSymRef(
            comp()->il.opCodeForDirectStore(value->getDataType()), 1, 1, value, newSymbolReference);
        storeTree = TR::TreeTop::create(comp(), storeNode);
        callNodeTreeTop->insertTreeTopsBeforeMe(storeTree);

        // Replace the old child with a load of the new sym ref
        value = TR::Node::createWithSymRef(
            value, comp()->il.opCodeForDirectLoad(value->getDataType()), 0, newSymbolReference);
...
// Extra boolean is for AOT case when we can not simplify the Class Test so we need Array Case in that scenario, But we
// can put indirect in cold instead of fallthrough for the tests
TR::TreeTop* TR_J9InlinerPolicy::genClassCheckForUnsafeGetPut(TR::Node* offset, bool isNotLowTagged)
{
    ...
    TR::ILOpCodes op
        = isNotLowTagged ? (isILoad ? TR::ificmpne : TR::iflcmpne) : (isILoad ? TR::ificmpeq : TR::iflcmpeq);
...
TR::TreeTop* TR_J9InlinerPolicy::genClassCheckForUnsafeGetPut(TR::Node* offset)
{
    // The low bit is tagged if the object being dereferenced is a
    // java/lang/Class object. This is because this is a special case when
    // an extra level of indirection is necessary
    bool isILoad = (offset->getOpCodeValue() == TR::iload);
    TR::Node* lowTag = TR::Node::create(
        isILoad ? TR::iand : TR::land, 2, offset, TR::Node::create(offset, isILoad ? TR::iconst : TR::lconst, 0, 0));
    if (isILoad)
    ...
    // Create the if to check if an extra level of indirection is needed
    TR::Node* cmp = TR::Node::createif(isILoad ? TR::ificmpne : TR::iflcmpne, lowTag, lowTag->getSecondChild(), NULL);
    TR::TreeTop* lowTagCmpTree = TR::TreeTop::create(comp(), cmp);
    return lowTagCmpTree;
}

TR::TreeTop* TR_J9InlinerPolicy::genDirectAccessCodeForUnsafeGetPut(
    TR::Node* callNode, bool conversionNeeded, bool isUnsafeGet)
{
...
TR::TreeTop* TR_J9InlinerPolicy::genIndirectAccessCodeForUnsafeGetPut(
    TR::Node* directAccessOrTempStoreNode, TR::Node* unsafeAddress)
{
    ...
    TR::Node* addressFirstChild = TR::Node::createWithSymRef(
        TR::aloadi, 1, 1, oldAddressFirstChild, comp()->getSymRefTab()->findOrCreateClassFromJavaLangClassSymbolRef());
    addressFirstChild = TR::Node::createWithSymRef(
        TR::aloadi, 1, 1, addressFirstChild, comp()->getSymRefTab()->findOrCreateRamStaticsFromClassSymbolRef());
    ...
    TR_ASSERT_FATAL(
        directSymbol->isUnsafeShadowSymbol(), "Expected to find an unsafe symbol for the get/put operation.");
    ...
    TR::SymbolReference* indirectSymRef = comp()->getSymRefTab()->findOrCreateUnsafeSymbolRef(
        directSymbol->getDataType(), true, true, directSymbol->isVolatile());
...
TR::Block* TR_J9InlinerPolicy::addNullCheckForUnsafeGetPut(TR::Node* unsafeAddress,
    TR::SymbolReference* newSymbolReferenceForAddress, TR::TreeTop* callNodeTreeTop, TR::TreeTop* directAccessTreeTop,
    TR::TreeTop* arrayDirectAccessTreeTop, TR::TreeTop* indirectAccessTreeTop)
{
    // Generate the treetop for the null comparison
    TR::Node* addrLoad = TR::Node::createWithSymRef(
        unsafeAddress, comp()->il.opCodeForDirectLoad(unsafeAddress->getDataType()), 0, newSymbolReferenceForAddress);
    TR::Node* nullCmp = TR::Node::createif(TR::ifacmpeq, addrLoad, TR::Node::create(addrLoad, TR::aconst, 0, 0), NULL);
    ...
    TR::Block* joinBlock = callNodeTreeTop->getEnclosingBlock()->createConditionalBlocksBeforeTree(
        callNodeTreeTop, nullComparisonTree, ifTree, elseTree, comp()->getFlowGraph(), false, false);
    return joinBlock;
}
...
void TR_J9InlinerPolicy::genCodeForUnsafeGetPut(TR::Node* unsafeAddress, TR::TreeTop* callNodeTreeTop,
    TR::TreeTop* prevTreeTop, TR::SymbolReference* newSymbolReferenceForAddress, TR::TreeTop* directAccessTreeTop,
    TR::TreeTop* lowTagCmpTree, bool needNullCheck, bool isUnsafeGet, bool conversionNeeded, TR::Block* joinBlock,
    TR_OpaqueClassBlock* javaLangClass, TR::Node* orderedCallNode = NULL)
{
    ...
    TR::Block* arrayDirectAccessBlock
        = conversionNeeded ? nullComparisonTree->getNode()->getBranchDestination()->getNode()->getBlock() : NULL;
    ...
    if (conversionNeeded) {
        ...
        directAccessBlock
            = TR::Block::createEmptyBlock(lowTagCmpTree->getNode(), comp(), indirectAccessBlock->getFrequency());
        directAccessBlock->append(directAccessTreeTop);
        directAccessBlock->append(TR::TreeTop::create(
            comp(), TR::Node::create(directAccessTreeTop->getNode(), TR::Goto, 0, joinBlock->getEntry())));
        arrayDirectAccessBlock->getExit()->insertTreeTopsAfterMe(
    ...
    TR::Block* lowTagCmpBlock = TR::Block::createEmptyBlock(unsafeAddress, comp(),
        conversionNeeded ? indirectAccessBlock->getFrequency() : directAccessBlock->getFrequency());
    ...
    if (conversionNeeded || javaLangClass == NULL) {
        ...
        TR::Node* andConstNode
            = TR::Node::create(isArrayField, TR::iconst, 0, TR::Compiler->cls.flagValueForArrayCheck(comp()));
        ...
        if (conversionNeeded) {
            indirectAccessBlock->getEntry()->insertTreeTopsBeforeMe(
                lowTagCmpBlock->getEntry(), lowTagCmpBlock->getExit());
            ...
            directAccessBlock->getEntry()->insertTreeTopsBeforeMe(
                lowTagCmpBlock->getEntry(), lowTagCmpBlock->getExit());
        ...
        cfg->addEdge(TR::CFGEdge::createEdge(
            isArrayBlock, conversionNeeded ? arrayDirectAccessBlock : directAccessBlock, trMemory()));
        cfg->addEdge(TR::CFGEdge::createEdge(nullComparisonBlock, isArrayBlock, trMemory()));

        if (comp()->getOption(TR_TraceUnsafeInlining)) {
            traceMsg(
                comp(), "\t In genCodeForUnsafeGetPut, Block %d created for array check\n", isArrayBlock->getNumber());
        }
    } else {
        ...
        if (comp()->getOption(TR_TraceUnsafeInlining)) {
            traceMsg(
                comp(), "\t In genCodeForUnsafeGetPut, Block %d created for isClass Test\n", isClassBlock->getNumber());
        }
    }
...
bool TR_J9InlinerPolicy::createUnsafePutWithOffset(TR::ResolvedMethodSymbol* calleeSymbol,
    ...
    bool isVolatile, bool needNullCheck, bool isOrdered)
{
    if (isVolatile && type == TR::Int64 && TR::Compiler->target.is32Bit()
    ...
    if (comp()->getOption(TR_TraceUnsafeInlining))
        traceMsg(comp(),
            "\tcreateUnsafePutWithOffset.  call tree %p offset(datatype) %d isvolatile %d needNullCheck %d isOrdered "
            "%d\n",
    ...
    if (callNodeTreeTop->getNode()->getOpCode().isNullCheck()) {
        ...
        TR::Node* checkNode = TR::Node::createWithSymRef(
            callNodeTreeTop->getNode(), TR::NULLCHK, 1, passthrough, callNodeTreeTop->getNode()->getSymbolReference());
    ...
    if (conversionNeeded) {
        ...
        unsafeNodeWithConversion = type == TR::Address && (comp()->getOptions()->getGcMode() != TR_WrtbarNone)
            ? TR::Node::createWithSymRef(
                  TR::awrtbari, 3, 3, unsafeAddress, valueWithConversion, unsafeCall->getChild(1), symRef)
            : TR::Node::createWithSymRef(
                  comp()->il.opCodeForIndirectArrayStore(type), 2, 2, unsafeAddress, valueWithConversion, symRef);
    ...
    TR::Node* unsafeNode = type == TR::Address && (comp()->getOptions()->getGcMode() != TR_WrtbarNone)
        ? TR::Node::createWithSymRef(
              TR::awrtbari, 3, 3, unsafeAddress, valueWithoutConversion, unsafeCall->getChild(1), symRef)
        : TR::Node::createWithSymRef(
              comp()->il.opCodeForIndirectStore(type), 2, 2, unsafeAddress, valueWithoutConversion, symRef);
    ...
    TR::Block* joinBlock = addNullCheckForUnsafeGetPut(unsafeAddress, newSymbolReferenceForAddress, callNodeTreeTop,
        directAccessTreeTop, arrayDirectAccessTreeTop, indirectAccessTreeTop);
    ...
    genCodeForUnsafeGetPut(unsafeAddress, callNodeTreeTop, prevTreeTop, newSymbolReferenceForAddress,
        directAccessTreeTop, lowTagCmpTree, needNullCheck, false, conversionNeeded, joinBlock, javaLangClass,
        orderedCallNode);
    ...
    if (callTreeForIllegalWriteReport) {
        ...
        auto isFinalStaticNode = TR::Node::createif(TR::iflcmpeq,
            TR::Node::create(TR::land, 2, duplicatedCallNode->getChild(2)->duplicateTree(),
        ...
        TR_ASSERT(storeToStaticFieldBlock->getSuccessors().size() == 1,
            "storeToStaticFieldBlock should have only one successor");
        TR::Block* mergeBlock = toBlock(storeToStaticFieldBlock->getSuccessors().front()->getTo());
        TR::TransformUtil::createConditionalAlternatePath(comp(), isFinalStaticTreeTop, callTreeForIllegalWriteReport,
            storeToStaticFieldBlock, mergeBlock, comp()->getMethodSymbol()->getFlowGraph(), true /*markCold*/);

        if (comp()->getOption(TR_TraceUnsafeInlining)) {
            traceMsg(comp(),
                "Created isFinal test node n%dn whose branch target is Block_%d to report illegal write to static "
                "final field\n",
...
bool TR_J9InlinerPolicy::createUnsafeCASCallDiamond(TR::TreeTop* callNodeTreeTop, TR::Node* callNode)
{
    ...
    // genClassCheck generates a ifcmpne offset&mask 1, meaning if it IS lowtagged (ie offset&mask == 1), the branch
    // will be taken
    ...
    callBlock->createConditionalBlocksBeforeTree(
        callNodeTreeTop, compareTree, ifTree, elseTree, comp()->getFlowGraph(), false, false);
    ...
    if (newSymbolReference) {
        TR::Node* ifStoreNode = TR::Node::createWithSymRef(
            comp()->il.opCodeForDirectStore(dataType), 1, 1, ifTree->getNode()->getFirstChild(), newSymbolReference);
        ...
        TR::Node* elseStoreNode = TR::Node::createWithSymRef(
            comp()->il.opCodeForDirectStore(dataType), 1, 1, elseTree->getNode()->getFirstChild(), newSymbolReference);
...
bool TR_J9InlinerPolicy::createUnsafeGetWithOffset(TR::ResolvedMethodSymbol* calleeSymbol,
    TR::ResolvedMethodSymbol* callerSymbol, TR::TreeTop* callNodeTreeTop, TR::Node* unsafeCall, TR::DataType type,
    bool isVolatile, bool needNullCheck)
{
    if (isVolatile && type == TR::Int64 && TR::Compiler->target.is32Bit()
        && !comp()->cg()->getSupportsInlinedAtomicLongVolatiles())
        return false;
    ...
    TR::TreeTop* arrayDirectAccessTreeTop
        = conversionNeeded ? genDirectAccessCodeForUnsafeGetPut(callNodeWithConversion, conversionNeeded, true) : NULL;
    TR::TreeTop* indirectAccessTreeTop
        = genIndirectAccessCodeForUnsafeGetPut(callNodeTreeTop->getNode(), unsafeAddress);
    // If we are not able to get javaLangClass it is still inefficient to put direct Access far
    // So in that case we will generate lowTagCmpTest to branch to indirect access if true
    TR_OpaqueClassBlock* javaLangClass
        = comp()->fe()->getClassFromSignature("Ljava/lang/Class;", 17, comp()->getCurrentMethod(), true);
    bool needNotLowTagged = javaLangClass != NULL || conversionNeeded;
    // If we can get a J9Class or we need conversion we generate test to branch to direct access if low bit is not
    // tagged Else in case we get NULL instead of j9Class we generate test to branch to indirect access if low bit is
    // tagged
    TR::TreeTop* lowTagCmpTree = genClassCheckForUnsafeGetPut(offset, needNotLowTagged);
    ...
    genCodeForUnsafeGetPut(unsafeAddress, callNodeTreeTop, prevTreeTop, newSymbolReferenceForAddress,
        directAccessTreeTop, lowTagCmpTree, needNullCheck, true, conversionNeeded, joinBlock, javaLangClass);
    ...
    TR::TreeTop* callNodeTreeTop, TR::Node* unsafeCall, TR::DataType type, bool compress)
{
    ...
    if (type == TR::Address) {
        if (TR::Compiler->target.is64Bit()) {
            unsafeNode = TR::Node::createWithSymRef(
                TR::lstorei, 2, 2, address, value, comp()->getSymRefTab()->findOrCreateUnsafeSymbolRef(TR::Int64));
        } else {
            unsafeNode = TR::Node::create(TR::l2i, 1, value);
            unsafeNode = TR::Node::createWithSymRef(
                TR::istorei, 2, 2, address, unsafeNode, comp()->getSymRefTab()->findOrCreateUnsafeSymbolRef(TR::Int32));
        }
    } else {
        switch (type) {
        case TR::Int8:
    ...
    TR::TreeTop* callNodeTreeTop, TR::Node* unsafeCall, TR::DataType type, bool compress)
{
    ...
    if (type == TR::Address) {
        if (TR::Compiler->target.is64Bit()) {
            ...
            unsafeNode = TR::Node::createWithSymRef(
                TR::iloadi, 1, 1, unsafeAddress, comp()->getSymRefTab()->findOrCreateUnsafeSymbolRef(TR::Int32));
    ...
    TR::ResolvedMethodSymbol* callerSymbol, TR::TreeTop* callNodeTreeTop, TR::Node* callNode)
{
    ...
    if (TR::Compiler->cls.classesOnHeap()) {
        j9cNode = TR::Node::createWithSymRef(
            TR::aloadi, 1, 1, j9cNode, comp()->getSymRefTab()->findOrCreateClassFromJavaLangClassSymbolRef());
    }

    TR::Node* nullCheckNode = TR::Node::createWithSymRef(
        TR::NULLCHK, 1, 1, j9cNode, comp()->getSymRefTab()->findOrCreateNullCheckSymbolRef(callerSymbol));
    TR::TreeTop* nullCheckTree = TR::TreeTop::create(comp(), nullCheckNode);
    TR::Node* romclassNode = TR::Node::createWithSymRef(
        TR::aloadi, 1, 1, j9cNode, comp()->getSymRefTab()->findOrCreateClassRomPtrSymbolRef());
    TR::Node* classAccessFlagsNode = TR::Node::createWithSymRef(
        TR::iloadi, 1, 1, romclassNode, comp()->getSymRefTab()->findOrCreateClassIsArraySymbolRef());
    ...
    TR::Node* modifiersIAndNode = TR::Node::create(
        TR::iand, 2, TR::Node::createLoad(callNode, modifiersSymRef), TR::Node::iconst(callNode, 0xFFF));
    ...
    block->createConditionalBlocksBeforeTree(
        callNodeTreeTop, compareTree, ifTree, elseTree, callerSymbol->getFlowGraph(), false);
    ...
    TR::ResolvedMethodSymbol* callerSymbol, TR::TreeTop* callNodeTreeTop, TR::Node* callNode)
{
    ...
    if ((TR::Compiler->vm.canAnyMethodEventsBeHooked(comp())
            && !comp()->fej9()->methodsCanBeInlinedEvenIfEventHooksEnabled())
    ...
    case TR::sun_misc_Unsafe_putObject_jlObjectJjlObject_V:
        return createUnsafePutWithOffset(
            calleeSymbol, callerSymbol, callNodeTreeTop, callNode, TR::Address, false, true);
    ...
    case TR::sun_misc_Unsafe_getObject_jlObjectJ_jlObject:
        return createUnsafeGetWithOffset(
            calleeSymbol, callerSymbol, callNodeTreeTop, callNode, TR::Address, false, true);
    ...
    case TR::sun_misc_Unsafe_putObjectVolatile_jlObjectJjlObject_V:
        return createUnsafePutWithOffset(
            calleeSymbol, callerSymbol, callNodeTreeTop, callNode, TR::Address, true, true);
    ...
    case TR::sun_misc_Unsafe_putByteOrdered_jlObjectJB_V:
        return createUnsafePutWithOffset(
            calleeSymbol, callerSymbol, callNodeTreeTop, callNode, TR::Int8, false, false, true);
    case TR::sun_misc_Unsafe_putBooleanOrdered_jlObjectJZ_V:
        return createUnsafePutWithOffset(
            calleeSymbol, callerSymbol, callNodeTreeTop, callNode, TR::Int8, false, false, true);
    case TR::sun_misc_Unsafe_putCharOrdered_jlObjectJC_V:
        return createUnsafePutWithOffset(
            calleeSymbol, callerSymbol, callNodeTreeTop, callNode, TR::Int16, false, false, true);
    case TR::sun_misc_Unsafe_putShortOrdered_jlObjectJS_V:
        return createUnsafePutWithOffset(
            calleeSymbol, callerSymbol, callNodeTreeTop, callNode, TR::Int16, false, false, true);
    case TR::sun_misc_Unsafe_putIntOrdered_jlObjectJI_V:
        return createUnsafePutWithOffset(
            calleeSymbol, callerSymbol, callNodeTreeTop, callNode, TR::Int32, false, false, true);
    case TR::sun_misc_Unsafe_putLongOrdered_jlObjectJJ_V:
        return createUnsafePutWithOffset(
            calleeSymbol, callerSymbol, callNodeTreeTop, callNode, TR::Int64, false, false, true);
    case TR::sun_misc_Unsafe_putFloatOrdered_jlObjectJF_V:
        return createUnsafePutWithOffset(
            calleeSymbol, callerSymbol, callNodeTreeTop, callNode, TR::Float, false, false, true);
    case TR::sun_misc_Unsafe_putDoubleOrdered_jlObjectJD_V:
        return createUnsafePutWithOffset(
            calleeSymbol, callerSymbol, callNodeTreeTop, callNode, TR::Double, false, false, true);
    case TR::sun_misc_Unsafe_putObjectOrdered_jlObjectJjlObject_V:
        return createUnsafePutWithOffset(
            calleeSymbol, callerSymbol, callNodeTreeTop, callNode, TR::Address, false, true, true);
...
void TR_J9InlinerPolicy::adjustFanInSizeInWeighCallSite(
    int32_t& weight, int32_t size, TR_ResolvedMethod* callee, TR_ResolvedMethod* caller, int32_t bcIndex)
{
    /*
...
bool TR_J9InlinerPolicy::_tryToGenerateILForMethod(
...
bool TR_J9InlinerPolicy::adjustFanInSizeInExceedsSizeThreshold(
    int bytecodeSize, uint32_t& calculatedSize, TR_ResolvedMethod* callee, TR_ResolvedMethod* caller, int32_t bcIndex)
{
    if (comp()->getMethodHotness() > warm)
        return false;
...
void TR_J9InlinerUtil::adjustMethodByteCodeSizeThreshold(
    TR::ResolvedMethodSymbol* callerSymbol, int& methodByteCodeSizeThreshold)
{
    if (inliner()->getPolicy()->aggressiveSmallAppOpts()
...
bool TR_J9InlinerPolicy::skipHCRGuardForCallee(TR_ResolvedMethod* callee)
{
    ...
    // VarHandle operation methods are also ignored here as they are implementation detail and are not expected to be
    // redefined.
...
void TR_MultipleCallTargetInliner::recursivelyWalkCallTargetAndPerformAction(TR_CallTarget* ct, FunctObj& action)
{
    ...
    debugTrace(tracer(),
        "recursivelyWalkingCallTargetAndPerformAction: Considering Target %p. node estimate before = %d maxbcindex = "
        "%d",
        ct, action.getNodeEstimate(), getPolicy()->getInitialBytecodeSize(ct->_calleeMethod, 0, comp()));
...
int32_t TR_MultipleCallTargetInliner::applyArgumentHeuristics(
    TR_LinkHead<TR_ParameterMapping>& map, int32_t originalWeight, TR_CallTarget* target)
{
    ...
    for (TR_ParameterMapping* parm = map.getFirst(); parm; parm = parm->getNext()) {
        if (parm->_parameterNode->getOpCode().isLoadConst()) {
            ...
            weight = comp()->fej9()->adjustedInliningWeightBasedOnArgument(
                weight, parm->_parameterNode, parm->_parmSymbol, comp());
    ...
    TR_VirtualGuardSelection* guard)
{
    ...
    if (nextTT->getNode()->getOpCodeValue() != TR::Return && nextTT->getNode()->getFirstChild() != callNode) {
                ...
                TR::Node::create(TR::ireturn, 1,
                    TR::Node::create(TR::idiv, 2,
                        TR::Node::create(TR::imul, 2, arithmeticNode,
                            TR::Node::create(
                                TR::iadd, 2, arithmeticNode, TR::Node::create(returnNode, TR::iconst, 0, 1))),
        ...
        TR::TreeTop::create(comp(), callNodeTreeTop->getPrevTreeTop(),
            TR::Node::createStore(
                temp, TR::Node::create(TR::iadd, 2, TR::Node::createLoad(returnNode, temp), arithmeticNode)));
    }
    ...
    if (!comp()->incInlineDepth(calleeSymbol, callNode->getByteCodeInfo(), callNode->getSymbolReference()->getCPIndex(),
            callNode->getSymbolReference(), !callNode->getOpCode().isCallIndirect(), 0)) {
        return false;
    }
...
void TR_MultipleCallTargetInliner::assignArgumentsToParameters(
    TR::ResolvedMethodSymbol* calleeSymbol, TR::TreeTop* prevTreeTop, TR::Node* callNode)
{
...
void TR_MultipleCallTargetInliner::walkCallSite(TR::ResolvedMethodSymbol* calleeSymbol, TR_CallStack* callStack,
    ...
    TR_OpaqueClassBlock* thisClass, bool inlineNonRecursively, int32_t walkDepth)
{
    ...
    if (!calleeSymbol->getFirstTreeTop()) {
        ...
        dumpOptDetails(comp(),
            "O^O INLINER: Peeking into the IL from walkCallSites as part of the inlining heuristic for [%p]\n",
            calleeSymbol);

        // comp()->setVisitCount(1);
        genILSucceeded = (NULL
            != calleeSymbol->getResolvedMethod()->genMethodILForPeekingEvenUnderMethodRedefinition(
                   calleeSymbol, comp()));
    ...
    // calleeSymbol->setFirstTreeTop(NULL); // We can reuse the peeked trees.  If we're doing real ILGen, the trees will
    // be re-created anyway.
}

void TR_MultipleCallTargetInliner::walkCallSites(TR::ResolvedMethodSymbol* callerSymbol, TR_CallStack* prevCallStack,
    TR_InnerPreexistenceInfo* innerPrexInfo, int32_t walkDepth)
{
        ...
        if (parent->getOpCodeValue() == TR::BBStart) {
            ...
            if (node->getOpCode().isCall() && node->getVisitCount() != _visitCount) {
                ...
                if (!isCold) {
                    ...
                    // TR_CallSite *callsite = new (trStackMemory()) TR_CallSite (symRef->getOwningMethod(comp()), tt,
                    // parent, node, calleeSymbol->getMethod(), 0, (int32_t)symRef->getOffset(), symRef->getCPIndex(), 0,
                    // calleeSymbol->getResolvedMethodSymbol(), node->getOpCode().isCallIndirect(),
                    // calleeSymbol->isInterface(), node->getByteCodeInfo(), comp());

                    TR_CallSite* callsite = TR_CallSite::create(
                        tt, parent, node, 0, symRef, (TR_ResolvedMethod*)0, comp(), trMemory(), stackAlloc);

                    debugTrace(tracer(),
                        "**WalkCallSites: Analysing Call at call node %p . Creating callsite %p to encapsulate call.",
                        node, callsite);
                    getSymbolAndFindInlineTargets(&callStack, callsite);

                    heuristicTrace(tracer(),
                        "**WalkCallSites:Searching for Targets returned %d targets for call at node %p. ",
                    ...
                    if (callsite->numTargets()) {
                        ...
                        for (int32_t i = 0; i < callsite->numTargets() && flag == false; i++) {
                            ...
                            if (!(callsite->getTarget(i)->_calleeSymbol->isVMInternalNative()
                                ...
                                if (TR::Compiler->mtd.isCompiledMethod(callsite->getTarget(i)
                                                                           ->_calleeSymbol->getResolvedMethod()
                                                                           ->getPersistentIdentifier())) {
                                    void* methodAddress = callsite->getTarget(i)
                                                              ->_calleeSymbol->getResolvedMethodSymbol()
                                                              ->getResolvedMethod()
                                                              ->startAddressForInterpreterOfJittedMethod();
                                    TR_PersistentJittedBodyInfo* bodyInfo
                                        = TR::Recompilation::getJittedBodyInfoFromPC(methodAddress);
                            ...
                            if (walkCall) {
                                ...
                                //                      calleeResolvedSymbol->getResolvedMethod();
                                TR_CallStack* cs
                                    = callStack.isCurrentlyOnTheStack(callsite->getTarget(i)->_calleeMethod, 1);
                                TR_PersistentMethodInfo* methodInfo = TR_PersistentMethodInfo::get(
                                    callsite->getTarget(i)->_calleeMethod); // calleeResolvedMethod);
                                bool alreadyVisited = false;

                                if (methodInfo && methodInfo->wasScannedForInlining()) {
                                    // printf("Already visited\n");
                                    debugTrace(tracer(),
                                        "Walk call sites for scanning: methodInfo %p already visited\n", methodInfo);

                                    alreadyVisited = true;
                                }

                                if (!(!alreadyVisited && cs
                                        && callsite->getTarget(i)->_calleeSymbol == callsite->_callNode->getSymbol()
                                        && eliminateTailRecursion(cs->_methodSymbol, &callStack,
                                               callsite->_callNodeTreeTop, callsite->_parent, callsite->_callNode,
                                               callsite->getTarget(i)->_guard))) {
                                    //                         walkCallSite(calleeResolvedSymbol, &callStack, tt,
                                    //                         parent, node, guard, thisClass, false, walkDepth);
                                    walkCallSite(callsite->getTarget(i)->_calleeSymbol, &callStack,
                                        callsite->_callNodeTreeTop, callsite->_parent, callsite->_callNode,
                                        callsite->getTarget(i)->_guard, callsite->getTarget(i)->_receiverClass, false,
                                        walkDepth);
                                    ...
                                    // TR_CallSite *callsite = new (trStackMemory()) TR_CallSite
                                    // (symRef->getOwningMethod(comp()), tt, parent, node,
                                    ...
                                    if (methodInfo) {
                                        ...
                                        debugTrace(tracer(),
                                            "Walk call sites for scanning: set scaneed for methodInfo %p\n",
                                            methodInfo);
                                    }
                                    // printf("Walk %s, method info %p\n", calleeResolvedSymbol->signature(trMemory()),
                                    // methodInfo);
                                }
                            }
                        }
...
bool TR_MultipleCallTargetInliner::inlineCallTargets(
    TR::ResolvedMethodSymbol* callerSymbol, TR_CallStack* prevCallStack, TR_InnerPreexistenceInfo* innerPrexInfo)
{
                ...
                if (node->getOpCode().isFunctionCall() && node->getVisitCount() != _visitCount) {
                    ...
                    if (!isCold && !node->isTheVirtualCallNodeForAGuardedInlinedCall()) {
                        ...
                        if (!prevCallStack && callsite->numTargets() > 0) {
                            ...
                            TR_PrexArgInfo::propagateArgsFromCaller(
                                thisCallSiteCallerSymbol, callsite, compArgInfo, tracer());
                            if (tracer()->heuristicLevel()) {
    ...
    if (prevCallStack == 0) {
        ...
        for (calltarget = _callTargets.getFirst(); calltarget; prev = calltarget, calltarget = calltarget->getNext()) {
            ...
            if ((tempTotalWeight > limit) && (calltarget->_weight > trivialWeightForLimit)) {
                ...
                if ((tempTotalWeight > limit) && (calltarget->_weight > trivialWeightForLimit)
                    ...
                    if (prev) {
                        heuristicTrace(tracer(),
                            "Chopping the Tail off the list of targets we're inlining. New last target = %p Choping "
                            "off at target %p",
                            prev, calltarget);
                        prev->setNext(0);
                    } else
        ...
        for (calltarget = _callTargets.getFirst(); calltarget; prev = calltarget, calltarget = calltarget->getNext()) {
            ...
            debugTrace(
                tracer(), "Estimated Number of Nodes is %d after calltarget %p", estimatedNumberOfNodes, calltarget);
            ...
            if (!forceInline(calltarget) && (uint32_t)(estimatedNumberOfNodes * factor) > _nodeCountThreshold) {
                heuristicTrace(tracer(),
                    "Estimated number of nodes of %d exceeds nodeCountThreshold of %d. Trimming list:",
                    (uint32_t)(estimatedNumberOfNodes * factor), _nodeCountThreshold);

                if (prev) {
                    heuristicTrace(tracer(),
                        "Chopping the Tail off the list of targets we're inlining. New last target = %p Choping off at "
                        "target %p",
                        prev, calltarget);
                    prev->setNext(0);
                } else
    ...
    if (prevCallStack == 0) {
        ...
        // call lower down was inlined first and created a temp t1, for a paremeter (the block doesn't get split).  It
...
void TR_MultipleCallTargetInliner::weighCallSite(
    TR_CallStack* callStack, TR_CallSite* callsite, bool currentBlockHasExceptionSuccessors, bool dontAddCalls)
{
    ...
    for (int32_t k = 0; k < callsite->numTargets(); k++) {
        ...
        if (calltarget->_calleeSymbol->getRecognizedMethod() == TR::java_lang_Class_newInstance
            ...
            debugTrace(tracer(),
                ...
                "= %d inlineit = %d error = %d ecs.sizeThreshold = %d",
            ...
            if (!inlineit && !callMustBeInlinedRegardlessOfSize(callsite)) {
                if (isWarm(comp())) {
                    if (comp()->isServerInlining()) {
                        ...
                        // sizeThreshold, calleeSymbol->signature(trMemory()));
                    }
                }
            ...
            if (isHot(comp())) {
                ...
                if (strncmp(m->nameChars(), sig, strlen(sig)) == 0) {
                    ...
                    if (strncmp(m->nameChars(), sig, strlen(sig)) == 0) {
                        size >>= 1;
                        heuristicTrace(
                            tracer(), "Setting size to %d because call is multiLeafArrayCopy and compile is hot", size);
                    }
                }
                ...
                if (calltarget->_calleeSymbol->getRecognizedMethod() == TR::java_math_BigDecimal_valueOf) {
                    size >>= 2;
                    heuristicTrace(
                        tracer(), "Setting size to %d because call is BigDecimal_valueOf and compile is hot", size);
                }
            }
            ...
            if (callNodeTreeTop) {
                ...
                frequency1 = comp()->convertNonDeterministicInput(
                    comp()->fej9()->getIProfilerCallCount(callsite->_callNode->getByteCodeInfo(), comp()),
                    MAX_BLOCK_COUNT + MAX_COLD_BLOCK_COUNT, randomGenerator(), 0);
                TR::Block* block = callNodeTreeTop->getEnclosingBlock();
                frequency2 = comp()->convertNonDeterministicInput(
                ...
                if (TR::isJ9() && !comp()->getMethodSymbol()->doJSR292PerfTweaks() && calltarget->_calleeMethod
                    ...
                    {
                        // to avoid divizion by zero crash. Semantically  freqs of 0 and 1 should be pretty close given
                        // maxFrequency of 10K
        ...
        if (/*calltarget->_calleeSymbol->getRecognizedMethod() == TR::com_ibm_simt_SPMDKernel_execute ||*/
            ...
            weight = 1;
            traceMsg(
                comp(), "Setting SIMD kernel methods weights to minimum(%d) node %p.", weight, callsite->_callNode);
        }
#endif
        ...
        if (possiblyVeryHotLargeCallee && !wouldBenefitFromInlining) {
            ...
            if (getPolicy()->aggressiveSmallAppOpts()
                ...
                weight <<= 1;
            heuristicTrace(
                tracer(), "Setting weight to %d because method is possibly a very hot and large callee", weight);
        }
    ...
    heuristicTrace(
        tracer(), "^^^ Done Weighing of all targets in CallSite %p callnode %p\n", callsite, callsite->_callNode);
}
    ...
    int32_t borderFrequency, TR_ResolvedMethod* calleeResolvedMethod, TR::Node* callNode, int32_t coldBorderFrequency)
{
    ...
    if (largeCompiledCallee) {
        ...
        heuristicTrace(
            tracer(), "exceedsSizeThreshold (mct): Scaled down size for call from %d to %d", oldSize, bytecodeSize);
    } else if (frequency < coldBorderFrequency) {
        int32_t oldSize = 0;
        if (comp()->trace(OMR::inlining))
            oldSize = bytecodeSize;

        // to avoid divizion by zero crash. Semantically  freqs of 0 and 1 should be pretty close given maxFrequency of
        // 10K
...
bool TR_MultipleCallTargetInliner::isLargeCompiledMethod(
    TR_ResolvedMethod* calleeResolvedMethod, int32_t bytecodeSize, int32_t callerBlockFrequency)
{
        ...
        if ((comp()->getMethodHotness() <= warm)) {
            if (bodyInfo && (bodyInfo->getHotness() >= warm)) {
                ...
                if (callerBlockFrequency > 0) {
                    if ((2 * callerBlockFrequency)
                        < comp()->getOptions()->getLargeCompiledMethodExemptionFreqCutoff()) {
    ...
    TR_ResolvedMethod* calleeResolvedMethod, TR::Node* callNode, bool allConsts)
{
    ...
    if (comp()->getMethodHotness() > warm && callSite->isInterface() && bytecodeSize > polymorphicCalleeSizeThreshold
        ...
        && (!trustedInterfaceRegex
               || !TR::SimpleRegex::match(
                      trustedInterfaceRegex, callSite->_interfaceMethod->signature(trMemory()), false))) {
    ...
    if (block) {
        ...
        frequency1 = comp()->convertNonDeterministicInput(comp()->fej9()->getIProfilerCallCount(bcInfo, comp()),
            MAX_BLOCK_COUNT + MAX_COLD_BLOCK_COUNT, randomGenerator(), 0);
        frequency2 = comp()->convertNonDeterministicInput(
            block->getFrequency(), MAX_BLOCK_COUNT + MAX_COLD_BLOCK_COUNT, randomGenerator(), 0);
        ...
        debugTrace(
            tracer(), "exceedsSizeThreshold: Call with block_%d has frequency %d", block->getNumber(), frequency2);
        ...
        if (allowBiggerMethods() && !comp()->getMethodSymbol()->doJSR292PerfTweaks() && calleeResolvedMethod
            ...
            bytecodeSize = scaleSizeBasedOnBlockFrequency(
                bytecodeSize, frequency2, borderFrequency, calleeResolvedMethod, callNode, veryColdBorderFrequency);
    ...
    if (callNode) {
        ...
        /*      if ( bytecodeSize != originalbcSize )
                 {
                 char tmpian[1024];
                 comp()->fej9()->sampleSignature(calleeResolvedMethod->getPersistentIdentifier(), tmpian, 1024,
           trMemory()); printf("R size of bc %d to %d meth %s comp
           %s\n",originalbcSize,bytecodeSize,tmpian,comp()->signature()); fflush(stdout);
                 }
        */
    } else if (allConsts) {
        ...
        /*
             ...
             {
                char tmpian[1024];
                comp()->fe()->sampleSignature(calleeResolvedMethod->getPersistentIdentifier(), tmpian, 1024,
           trMemory()); printf("R size of bc %d to %d meth %s comp
           %s\n",originalbcSize,bytecodeSize,tmpian,comp()->signature()); fflush(stdout);
             }
       */
    ...
    {
        // In JIT, having low caller information is equivallent to lack of information.  We want to exclude only cases
        // where we know we have alot of fan-in
        if (j9InlinerPolicy->adjustFanInSizeInExceedsSizeThreshold(
                bytecodeSize, calculatedSize, calleeResolvedMethod, callerResolvedMethod, bcInfo.getByteCodeIndex())) {
            return true;
        }
    }
    //   else
    ...
    if (isCold && (bytecodeSize > _methodInColdBlockByteCodeSizeThreshold)) {
        if (block) {
            TR::DebugCounter::prependDebugCounter(
                comp(), "inliner.callSites/failed/coldCallee/tooManyBytecodes", block->getFirstRealTreeTop());
    ...
    if (bytecodeSize > _methodInWarmBlockByteCodeSizeThreshold
        ...
        if (block) {
            TR::DebugCounter::prependDebugCounter(
                comp(), "inliner.callSites/failed/warmCallee/tooManyBytecodes", block->getFirstRealTreeTop());
            TR::DebugCounter::prependDebugCounter(comp(),
        ...
        if (bytecodeSize <= _methodInWarmBlockByteCodeSizeThreshold) {
            ...
            if (r) {
                ...
                printf(
                    ...
                    comp()->fej9()->sampleSignature(
                        calleeResolvedMethod->getPersistentIdentifier(), calleeName2, 1024, comp()->trMemory()),
            ...
            for (int32_t c = callNode->getNumChildren() - 1; c >= firstArgIndex; c--) {
                ...
                if (p && p->usedProfiledInfo()) {
                    ...
                    if (pc) {
                        ...
                        // callerSymbol->getResolvedMethod()->signature(trMemory()),
                        // calleeSymbol->getResolvedMethod()->signature(trMemory())); fflush(stdout);
                        ...
                        TR::Node* nullcmp = TR::Node::createWithSymRef(
                            argument, comp()->il.opCodeForDirectLoad(argument->getDataType()), 0, newSymbolReference);
...
bool TR_J9InlinerPolicy::doCorrectnessAndSizeChecksForInlineCallTarget(
    TR_CallStack* callStack, TR_CallTarget* calltarget, bool inlinefromgraph, TR_PrexArgInfo* argInfo)
{
...
bool TR_J9InlinerPolicy::supressInliningRecognizedInitialCallee(TR_CallSite* callsite, TR::Compilation* comp)
{
    return (comp->fej9()->supressInliningRecognizedInitialCallee(callsite, comp));
}

TR_InlinerFailureReason TR_J9InlinerPolicy::checkIfTargetInlineable(
...
TR_InlinerFailureReason TR_J9JSR292InlinerPolicy::checkIfTargetInlineable(
    TR_CallTarget* target, TR_CallSite* callsite, TR::Compilation* comp)
{
    return static_cast<TR_InlinerFailureReason>(comp->fej9()->checkInlineableTarget(target, callsite, comp, true));
}

void TR_J9InlinerUtil::estimateAndRefineBytecodeSize(
    TR_CallSite* callsite, TR_CallTarget* calltarget, TR_CallStack* callStack, int32_t& bytecodeSize)
{
    if (comp()->getOptLevel() >= warm) {
        TR_EstimateCodeSize::raiiWrapper ecsWrapper(
            inliner(), tracer(), inliner()->getMaxRecursiveCallByteCodeSizeEstimate());
        TR_EstimateCodeSize* ecs = ecsWrapper.getCodeEstimator();
...
TR_PrexArgInfo* TR_J9InlinerUtil::computePrexInfo(TR_CallTarget* target)
{
    ...
    for (int32_t c = site->_callNode->getNumChildren() - 1; c >= firstArgIndex; c--) {
        ...
        if (c == site->_callNode->getFirstArgumentIndex()) // if the argument is the this-child
        {
            if (target->_guard->_type == TR_VftTest && !disablePreexForChangedGuard) {
                if (priorKnowledge < FIXED_CLASS) {
                    argInfo->set(0,
                        new (inliner()->trStackMemory())
                            TR_PrexArgument(TR_PrexArgument::ClassIsFixed, target->_guard->_thisClass));
                    if (tracePrex) {
                ...
                if (priorKnowledge < KNOWN_OBJECT) {
                    argInfo->set(0,
                        new (inliner()->trStackMemory())
                            TR_PrexArgument(target->_guard->_mutableCallSiteEpoch, comp()));
                    if (tracePrex) {
                        TR::Node* call = site->_callNode;
                        TR::Node* targetMH = call->getArgument(0);
                        traceMsg(comp(),
                            "PREX.inl:      %p: MutableCallSite.target [%p] is known object obj%d in inlined call "
                            "[%p]\n",
        ...
        if (argument->getOpCode().hasSymbolReference() && argument->getSymbolReference()->hasKnownObjectIndex()) {
            if (priorKnowledge < KNOWN_OBJECT) {
                argInfo->set(argOrdinal,
                    new (inliner()->trStackMemory())
                        TR_PrexArgument(argument->getSymbolReference()->getKnownObjectIndex(), comp()));
                if (tracePrex)
            ...
            if (parmSymbol && !argInfo->get(argOrdinal)) {
                if (parmSymbol->getFixedType()) {
                    if (priorKnowledge < FIXED_CLASS) {
                        argInfo->set(argOrdinal,
                            new (inliner()->trStackMemory()) TR_PrexArgument(
                                TR_PrexArgument::ClassIsFixed, (TR_OpaqueClassBlock*)parmSymbol->getFixedType()));
                        if (tracePrex) {
                            char* sig = TR::Compiler->cls.classSignature(
                                comp(), (TR_OpaqueClassBlock*)parmSymbol->getFixedType(), trMemory());
                ...
                if (parmSymbol->getIsPreexistent()) {
                    if (priorKnowledge < PREEXISTENT) {
                        argInfo->set(argOrdinal,
                            new (inliner()->trStackMemory()) TR_PrexArgument(TR_PrexArgument::ClassIsPreexistent));
                        if (tracePrex)
...
void TR_J9InlinerUtil::checkForConstClass(TR_CallTarget* target, TR_InlinerTracer* tracer)
{
    ...
    if (tracePrex)
        traceMsg(comp, "checkForConstClass parm for [%p] %s %s\n", callNode, callNode->getOpCode().getName(),
            callNode->getSymbol()->castToMethodSymbol()->getMethod()->signature(tracer->trMemory(), stackAlloc));
    ...
    for (int32_t c = callNode->getNumChildren() - 1; c >= firstArgIndex; c--) {
        ...
        if (argument->getOpCode().hasSymbolReference()
            ...
            if (argFirstChild->getOpCode().hasSymbolReference() && argFirstChild->getSymbol()->isStatic()
                ...
                uintptrj_t objectReferenceLocation = (uintptrj_t)argFirstChild->getSymbolReference()
                                                         ->getSymbol()
                                                         ->castToStaticSymbol()
                                                         ->getStaticAddress();
                if (objectReferenceLocation) {
                    ...
                    if (knot) {
                        TR_J9VMBase* fej9 = (TR_J9VMBase*)(comp->fe());
                        knownObjectIndex = knot->getIndexAt(
                            (uintptrj_t*)(objectReferenceLocation + fej9->getOffsetOfJavaLangClassFromClassField()));
        ...
        if (argument->getOpCode().hasSymbolReference()
            ...
            if (priorKnowledge < KNOWN_OBJECT) {
                if (knownObjectClass) {
                    ...
                    ecsArgInfo->set(argOrdinal,
                        new (tracer->trStackMemory())
                            TR_PrexArgument(argument->getSymbolReference()->getKnownObjectIndex(), comp));
                    if (tracePrex)
                        traceMsg(comp, "checkForConstClass: %p: is known object obj%d\n", ecsArgInfo->get(argOrdinal),
                            argument->getSymbolReference()->getKnownObjectIndex());
                }
            }
        }
...
TR_PrexArgInfo* TR_PrexArgInfo::buildPrexArgInfoForMethodSymbol(
    TR::ResolvedMethodSymbol* methodSymbol, TR_InlinerTracer* tracer)
{
    ...
    heuristicTrace(
        tracer, "PREX-CSI:  Populating parmInfo of current method %s\n", feMethod->signature(tracer->trMemory()));
    int index = 0;
    for (TR::ParameterSymbol* p = parms.getFirst(); p != NULL; index++, p = parms.getNext()) {
        ...
        if (*sig == 'L') {
            ...
            if (clazz) {
                argInfo->set(
                    index, new (tracer->trHeapMemory()) TR_PrexArgument(TR_PrexArgument::ClassIsPreexistent, clazz));
...
static TR_PrexArgument* stronger(TR_PrexArgument* left, TR_PrexArgument* right, TR::Compilation* comp)
{
    if (TR_PrexArgument::knowledgeLevel(left) > TR_PrexArgument::knowledgeLevel(right))
        return left;
    else if (left && right && TR_PrexArgument::knowledgeLevel(left) == TR_PrexArgument::knowledgeLevel(right)
...
static void populateClassNameSignature(
    TR_Method* m, TR_ResolvedMethod* caller, TR_OpaqueClassBlock*& c, char*& nc, int32_t& nl, char*& sc, int32_t& sl)
{
...
{
    int32_t len = m->classNameLength();
    return classNameToSignature(
        m->classNameChars(), len /*don't care, cos this gives us a null terminated string*/, comp);
}

TR::Node* TR_PrexArgInfo::getCallNode(
    TR::ResolvedMethodSymbol* methodSymbol, TR_CallSite* callsite, TR_InlinerTracer* tracer)
{
    for (TR::TreeTop* tt = methodSymbol->getFirstTreeTop(); tt; tt = tt->getNextTreeTop()) {
        if (tt->getNode()->getNumChildren() > 0 && tt->getNode()->getFirstChild()->getOpCode().isCall()
            ...
            populateClassNameSignature(callsite->_initialCalleeMethod
                    ...
                    : // TR_ResolvedMethod doesn't extend TR_Method
                    callsite->_interfaceMethod,
                methodSymbol->getResolvedMethod(), callSiteClass, callSiteNameChars, callSiteNameLength,
                callSiteSignatureChars, callSiteSignatureLength);

            populateClassNameSignature(callNodeMS->getMethod(), methodSymbol->getResolvedMethod(), callNodeClass,
                callNodeNameChars, callNodeNameLength, callNodeSignatureChars, callNodeSignatureLength);

            // make sure classes are compatible

            if (!callNodeClass || !callSiteClass
                || methodSymbol->getResolvedMethod()->fe()->isInstanceOf(callNodeClass, callSiteClass, true, true, true)
                    != TR_yes) {
                if (tracer->heuristicLevel()) {
...
void TR_PrexArgInfo::propagateReceiverInfoIfAvailable(
    TR::ResolvedMethodSymbol* methodSymbol, TR_CallSite* callsite, TR_PrexArgInfo* argInfo, TR_InlinerTracer* tracer)
{
    ...
    heuristicTrace(
        tracer, "ARGS PROPAGATION: trying to propagate receiver's info for callsite %p at %p", callsite, callNode);
    ...
    if (TR_PrexArgInfo::hasArgInfoForChild(child, argInfo)) {
        heuristicTrace(
            tracer, "ARGS PROPAGATION: the receiver for callsite %p is also one of the caller's args", callsite);
...
bool TR_PrexArgInfo::validateAndPropagateArgsFromCalleeSymbol(
    ...
    for (int32_t i = 0; i < numArgsToEnhance; i++) {
        ...
        */
        if (fe->isInstanceOf(argsFromSymbol->get(i)->getClass(), argsFromTarget->get(i)->getClass(), true, true, true)
                != TR_yes
            && fe->isInstanceOf(
                   argsFromTarget->get(i)->getClass(), argsFromSymbol->get(i)->getClass(), true, true, true)
                != TR_yes) {
            return false;
        }
    }
...
void TR_PrexArgInfo::clearArgInfoForNonInvariantArguments(
    TR::ResolvedMethodSymbol* methodSymbol, TR_InlinerTracer* tracer)
{
...
void TR_PrexArgInfo::propagateArgsFromCaller(
    TR::ResolvedMethodSymbol* methodSymbol, TR_CallSite* callsite, TR_PrexArgInfo* argInfo, TR_InlinerTracer* tracer)
{
    if (tracer->comp()->getOption(TR_DisableInlinerArgsPropagation))
        return;
        ...
        if (TR_PrexArgInfo::hasArgInfoForChild(child, argInfo)) {
            ...
            for (int j = 0; j < callsite->numTargets(); j++) {
                ...
                if (!targetArgInfo->get(i - callNode->getFirstArgumentIndex()))
                    targetArgInfo->set(
                        i - callNode->getFirstArgumentIndex(), TR_PrexArgInfo::getArgForChild(child, argInfo));
            }
        }
    }
...
void TR_J9InlinerUtil::computeMethodBranchProfileInfo(
    ...
    if (cfgBlock) // isn't this equal to genILSucceeded??
    {
        ...
        if (!mbpInfo) {
            ...
            if (comp()->getOption(TR_TraceBFGeneration)) {
                traceMsg(comp(),
                    "Setting initial block count for a call with index %d to be %d, call factor %f where block %d (%p) "
                    "and blockFreq = %d\n",
...
void TR_J9TransformInlinedFunction::wrapCalleeInTryRegion(
    bool isSynchronized, bool putCatchInCaller, TR_ResolvedMethod* calleeMethod)
{
    ...
    TR::TreeTop* prevTreeTop, bool putCatchInCaller, int32_t catchType, int32_t handlerIndex, bool addBlocks)
{
    ...
    if (comp()->getOption(TR_EnableThisLiveRangeExtension)) {
        if (!_calleeSymbol->isStatic()
            ...
            TR::Node* anchoredThis = TR::Node::createWithSymRef(
                modelNode, TR::aload, 0, symRefTab->findOrCreateAutoSymbol(_calleeSymbol, 0, TR::Address));
            TR::SymbolReference* tempSymRef
    ...
    catchBlock->append(TR::TreeTop::create(comp(),
        TR::Node::createWithSymRef(
            TR::athrow, 1, 1, loadExcpSymbol, symRefTab->findOrCreateAThrowSymbolRef(_calleeSymbol))));
    ...
    TR::TreeTop* prevTreeTop, int32_t catchType, int32_t handlerIndex, bool addBlocks)
{
    ...
    catchBlock->setHandlerInfo(
        catchType, (uint8_t)comp()->getInlineDepth(), handlerIndex, calleeResolvedMethod, comp());
    catchBlock->setIsSynchronizedHandler();
    ...
    if (_calleeSymbol->isStatic())
        if (TR::Compiler->cls.classesOnHeap()) {
            monitorArgHandle = TR::Node::createWithSymRef(lastNode, TR::loadaddr, 0,
                symRefTab->findOrCreateClassSymbol(
                    _calleeSymbol, 0, _calleeSymbol->getResolvedMethod()->containingClass()));
            monitorArgHandle = TR::Node::createWithSymRef(
                TR::aloadi, 1, 1, monitorArgHandle, symRefTab->findOrCreateJavaLangClassFromClassSymbolRef());
        } else {
            monitorArgHandle = TR::Node::createWithSymRef(lastNode, TR::loadaddr, 0,
                symRefTab->findOrCreateClassSymbol(
                    _calleeSymbol, 0, _calleeSymbol->getResolvedMethod()->containingClass()));
        ...
        monitorArgHandle = TR::Node::createWithSymRef(
            lastNode, TR::aload, 0, symRefTab->findOrCreateAutoSymbol(_calleeSymbol, 0, TR::Address));
    ...
    if (!_calleeSymbol->isStatic()) {
        ...
        TR::Node* ifNode = TR::Node::createif(
            TR::ifacmpeq, monitorArg->duplicateTree(), TR::Node::aconst(monitorArg, 0), rethrowBlock->getEntry());
    ...
    TR::Node* monexitNode = TR::Node::createWithSymRef(
        TR::monexit, 1, 1, monitorArg, symRefTab->findOrCreateMonitorExitSymbolRef(_calleeSymbol));
    ...
    rethrowBlock->append(TR::TreeTop::create(comp(),
        TR::Node::createWithSymRef(TR::athrow, 1, 1, temp, symRefTab->findOrCreateAThrowSymbolRef(_calleeSymbol))));
...
bool TR_J9InlinerUtil::addTargetIfMethodIsNotOverridenInReceiversHierarchy(TR_IndirectCallSite* callsite)
{
    ...
    if (!chTable->isOverriddenInThisHierarchy(
            callsite->_initialCalleeMethod, callsite->_receiverClass, callsite->_vftSlot, comp())
        ...
        callsite->addTarget(
            comp()->trMemory(), inliner(), guard, callsite->_initialCalleeMethod, callsite->_receiverClass, heapAlloc);
        return true;
    }
    return false;
}

int32_t TR_J9InlinerUtil::getCallCount(TR::Node* callNode)
{
...
TR_PrexArgInfo* TR_J9InlinerUtil::createPrexArgInfoForCallTarget(
    TR_VirtualGuardSelection* guard, TR_ResolvedMethod* implementer)
{
    ...
    if (!comp()->getOption(TR_DisableInlinerArgsPropagation) && comp()->fej9()->supportsContextSensitiveInlining()) {
        ...
        if (guard->_type == TR_VftTest) {
            ...
            myPrexArgInfo->set(
                0, new (comp()->trHeapMemory()) TR_PrexArgument(TR_PrexArgument::ClassIsFixed, guard->_thisClass));
    ...
    if (hasInnerAssumptions()) {
        ...
        if (point
            && performTransformation(comp,
                   "%sIPREX: remove virtual guard for inlined call %p to %s because it inner preexists parm ordinal %d "
                   "of %s\n",
...
TR_J9InnerPreexistenceInfo::PreexistencePoint* TR_J9InnerPreexistenceInfo::getPreexistencePointImpl(
    int32_t ordinal, TR_CallStack* prevCallStack)
{
    ParmInfo* parmInfo = getParmInfo(ordinal);
...
bool TR_J9InlinerPolicy::replaceSoftwareCheckWithHardwareCheck(TR_ResolvedMethod* calleeMethod)
{
    if (calleeMethod && comp()->cg()->getSupportsBDLLHardwareOverflowCheck()
        && ((strncmp(calleeMethod->signature(comp()->trMemory()), "java/math/BigDecimal.noLLOverflowAdd(JJJ)Z", 42)
                == 0)
               || (strncmp(
                       calleeMethod->signature(comp()->trMemory()), "java/math/BigDecimal.noLLOverflowMul(JJJ)Z", 42)
                      == 0)))
        return true;
    else
        return false;
    ...
    if (guard->isHighProbablityProfiledGuard()) {
        if (comp->getMethodHotness() <= warm && comp->getPersistentInfo()->getJitState() == STARTUP_STATE) {
            ...
            TR::DebugCounter::incStaticDebugCounter(
                comp, TR::DebugCounter::debugCounterName(comp, "profiledPrivArgRemat/unsuitableForRemat/warmHighProb"));
        } else {
            TR::DebugCounter::incStaticDebugCounter(
                comp, TR::DebugCounter::debugCounterName(comp, "profiledPrivArgRemat/suitableForRemat/highProb"));
        }
    } else if (valueInfo) {
        ...
        TR::DebugCounter::incStaticDebugCounter(
            comp, TR::DebugCounter::debugCounterName(comp, "profiledPrivArgRemat/unsuitableForRemat/noinfo"));
        suitableForRemat = false;
    }
    ...
    if (heuristicLevel()) {
        ...
        for (profiledInfo = sortedValuesIt.getFirst(); profiledInfo != NULL; profiledInfo = sortedValuesIt.getNext()) {
            ...
            if (!isClassObsolete) {
                ...
                heuristicTrace(
                    this, "receiverClass %p is obsolete and has profiled frequency of %f", tempreceiverClass, val);
            }
        }
    }
}
