        ...
        if (!clazz) {
            ...
            if (!s) {
                *success = false;
                if (trace()) {
                    traceMsg(comp(),
                        "Found unresolved method call node %p while peeking whose class is unresolved and unable to "
                        "add assumption -- peek unsuccessful\n",
                        callNode);
                }
            } else {
                ...
                if (trace()) {
                    traceMsg(
                        comp(), "Found unresolved method call node %p while peeking -- add assumption\n", callNode);
                }
            }
            return 0;
        }
...
List<OMR::RuntimeAssumption>* TR::InterProceduralAnalyzer::analyzeMethod(
    TR::Node* callNode, TR_ResolvedMethod* method, bool* success)
{
    if (trace()) {
    ...
    if (0 && alreadyPeekedMethod(method, &successfulPriorPeek, &priorPeek)) {
        if (!successfulPriorPeek) {
            *success = false;
            ...
            if (trace()) {
                traceMsg(comp(),
                    "Prior peek success for method %s caused current peek to be successful -- prior assumptions "
                    "added\n",
                    method->signature(trMemory()));
            }
    ...
    if (method->isStatic())
        newSymRef = comp()->getSymRefTab()->findOrCreateMethodSymbol(
            symRef->getOwningMethodIndex(), -1, method, TR::MethodSymbol::Static);
    else
        newSymRef = comp()->getSymRefTab()->findOrCreateMethodSymbol(
            symRef->getOwningMethodIndex(), -1, method, TR::MethodSymbol::Interface);
...
bool TR::InterProceduralAnalyzer::alreadyPeekedMethod(
    TR_ResolvedMethod* method, bool* success, TR::PriorPeekInfo** priorPeek)
{
