...
public:
    ...
    void checkIfThereIsAParmLoadWithinDistance()
    {
        for (int i = 0; i < _size; i++) {
            if (_bci.bcIndex() - _loadIndices[i] <= _distance) {
                _needsPeeking = true;
                heuristicTraceIfTracerIsNotNull(_tracer,
                    "there is a parm load at %d which is within %d of a call at %d", _loadIndices[i], _distance,
                    _bci.bcIndex());
            }
        }
    };
    ...
    void processParameterLoad(int slotIndex)
    {
        ...
        if (slotIndex < _numOfArgs && _argInfo->get(slotIndex)) {
            heuristicTraceIfTracerIsNotNull(
                _tracer, "came across of a load of slot %d at %d", slotIndex, _bci.bcIndex());
...
static Block* getBlock(TR::Compilation* comp, Block** blocks, TR_ResolvedMethod* feMethod, int32_t i, TR::CFG& cfg)
{
    if (!blocks[i]) {

        TR::TreeTop* startTree = TR::TreeTop::create(comp, TR::Node::createOnStack(NULL, TR::BBStart, 0));
        TR::TreeTop* endTree = TR::TreeTop::create(comp, TR::Node::createOnStack(NULL, TR::BBEnd, 0));

        startTree->join(endTree);
        blocks[i] = new (comp->trStackMemory()) Block(startTree, endTree, comp->trMemory());
...
static void setupLastTreeTop(Block* currentBlock, TR_J9ByteCode bc, uint32_t bcIndex, Block* destinationBlock,
...
static bool hasArgInfoForChild(TR::Node* child)
{
    ...
    return child->getOpCode().hasSymbolReference() && child->getSymbolReference()->getSymbol()->isParm()
    ...
    TR::ResolvedMethodSymbol* methodSymbol, TR_CallSite* callsite, TR_PrexArgInfo* argInfo, TR_InlinerTracer* tracer)
{
    ...
    if (hasArgInfoForChild(child)) {
        ...
        TR_OpaqueClassBlock* argClass
            = (TR_OpaqueClassBlock*)child->getSymbolReference()->getSymbol()->getParmSymbol()->getFixedType();
    ...
    TR::ResolvedMethodSymbol* methodSymbol, TR_CallSite* callsite, TR_PrexArgInfo* argInfo, TR_InlinerTracer* tracer)
{
    ...
    for (int i = callNode->getFirstArgumentIndex(); i < callNode->getNumChildren(); i++) {
        ...
        {
            heuristicTrace(tracer, "ECS CSI -- arg %d at callsite %p matches caller's arg %d", i, callsite,
                child->getSymbolReference()->getSymbol()->getParmSymbol()->getOrdinal());

            for (int j = 0; j < callsite->numTargets(); j++) {
                TR_ASSERT(
                    callsite->getTarget(j)->_ecsPrexArgInfo, "shouldn't be null! always initialize it in calltarget");

                // ILGen did something funky with this call, skip arg propagation
                if (callsite->getTarget(j)->_ecsPrexArgInfo->getNumArgs()
                ...
                TR_OpaqueClassBlock* argClass
                    = (TR_OpaqueClassBlock*)child->getSymbolReference()->getSymbol()->getParmSymbol()->getFixedType();
                callsite->getTarget(j)->_ecsPrexArgInfo->set(i - callNode->getFirstArgumentIndex(),
                    new (tracer->trHeapMemory()) TR_PrexArgument(TR_PrexArgument::ClassIsFixed, argClass));
            }
        }
    }
...
bool TR_J9EstimateCodeSize::realEstimateCodeSize(
    TR_CallTarget* calltarget, TR_CallStack* prevCallStack, bool recurseDown)
{
    ...
    if (tracer()->heuristicLevel())
        callerName = comp()->fej9()->sampleSignature(
            calltarget->_calleeMethod->getPersistentIdentifier(), nameBuffer, 1024, comp()->trMemory());

    heuristicTrace(tracer(),
        "*** Depth %d: ECS to begin for target %p signature %s size assuming we can partially inline (optimistic size) "
        " = %d total real size so far = %d sizeThreshold %d",
        _recursionDepth, calltarget, callerName, _optimisticSize, _realSize, _sizeThreshold);
    ...
    if (_mayHaveVirtualCallProfileInfo) {
        if (!comp()->incInlineDepth(
                methodSymbol, calltarget->_myCallSite->_bcInfo, 0, NULL, !calltarget->_myCallSite->_isIndirectCall)) {
    ...
    TR_J9ByteCodeIterator bci(0, static_cast<TR_ResolvedJ9Method*>(calltarget->_calleeMethod),
    ...
    if (!TR_PrexArgInfo::validateAndPropagateArgsFromCalleeSymbol(
            argsFromSymbol, calltarget->_ecsPrexArgInfo, tracer())) {
            ...
            if ((isUnresolvedInCP && !resolvedMethod) || (resolvedMethod && resolvedMethod->isCold(comp(), true))) {
                ...
                if (tracer()->heuristicLevel()) {
                    if (resolvedMethod) {
                        ...
                        TR_Method* meth = comp()->fej9()->createMethod(
                            comp()->trMemory(), calltarget->_calleeMethod->containingClass(), cpIndex);
                        heuristicTrace(tracer(),
                            "Depth %d: Call at bc index %d is Cold.  Not searching for targets. Signature %s",
    ...
    if (nph.doPeeking() && recurseDown) {
        ...
        heuristicTrace(
            tracer(), "*** Depth %d: ECS CSI -- needsPeeking is true for calltarget %p", _recursionDepth, calltarget);

        methodSymbol = TR::ResolvedMethodSymbol::create(comp()->trHeapMemory(), calltarget->_calleeMethod, comp());
        bool ilgenSuccess = (NULL
            != methodSymbol->getResolvedMethod()->genMethodILForPeekingEvenUnderMethodRedefinition(
                   methodSymbol, comp(), false, NULL));
        if (ilgenSuccess) {
    ...
    heuristicTrace(tracer(),
        "--- Done Iterating over Bytecodes in call to %s.  size = %d _recursionDepth = %d _optimisticSize = %d "
        "_realSize = %d _sizeThreshold = %d",
        callerName, size, _recursionDepth, _optimisticSize, _realSize, _sizeThreshold);
    ...
    if (true) // Need to run the below code to inline partially anyway (even if there are no calls in this callee)
    {
        TR_Array<TR_J9ByteCodeIterator::TryCatchInfo> tryCatchInfo(
            comp()->trMemory(), calltarget->_calleeMethod->numberOfExceptionHandlers(), true, stackAlloc);
        ...
        for (i = calltarget->_calleeMethod->numberOfExceptionHandlers() - 1; i >= 0; --i) {
            ...
            tryCatchInfo[i].initialize((uint16_t)start, (uint16_t)end, (uint16_t)handler, (uint32_t)type);
        }

        TR::CFG cfg(comp(), calltarget->_calleeSymbol);
        cfg.setStartAndEnd(
            new (comp()->trStackMemory())
                TR::Block(TR::TreeTop::create(comp(), TR::Node::createOnStack(NULL, TR::BBStart, 0)),
                    TR::TreeTop::create(comp(), TR::Node::createOnStack(NULL, TR::BBEnd, 0)), comp()->trMemory()),
            new (comp()->trStackMemory())
                TR::Block(TR::TreeTop::create(comp(), TR::Node::createOnStack(NULL, TR::BBStart, 0)),
                    TR::TreeTop::create(comp(), TR::Node::createOnStack(NULL, TR::BBEnd, 0)), comp()->trMemory()));
        ...
        for (i = 0; i < maxIndex; ++i) {
            ...
            if (flags[i].testAny(isCold)) {
                partialTrace(tracer(),
                    "Setting block %p[%d] blocks[%d]=%p as cold because bytecode %d was identified as cold",
                    currentBlock, currentBlock->getNumber(), i, blocks[i], i);
            ...
            if (flags[i].testAny(isBranch)) {
                if (startIndex != i) {
                    ...
                    currentBlock->setBlockSize(1); // if there startIndex is the same as the current index then the
                                                   // block consists only of a branch
                    if (cfg.getMethodSymbol())
                        cfg.getMethodSymbol()->addProfilingOffsetInfo(
                            currentBlock->getEntry()->getNode()->getByteCodeIndex(),
                            currentBlock->getEntry()->getNode()->getByteCodeIndex() + currentBlock->getBlockSize());
                ...
                case J9BCifnonnull: {
                    ...
                    cfg.addEdge(currentBlock,
                        getBlock(comp(), blocks, calltarget->_calleeMethod, i + bci.relativeBranch(), cfg), stackAlloc);
                    addFallThruEdge = true;
                    break;
                }
                case J9BCgoto:
        ...
        for (i = 0; i < (int32_t)tryCatchInfo.size(); ++i) {
            ...
            blocks[handlerInfo->_handlerIndex]->setHandlerInfoWithOutBCInfo(
                handlerInfo->_catchType, 0, handlerInfo->_handlerIndex, calltarget->_calleeMethod, comp());
        ...
        if (callGraphEnabled && recurseDown) {
            ...
            int32_t callCount = profileManager->getCallGraphProfilingCount(method, bcIndex, comp());
            cfg._calledFrequency = callCount;

            if (callCount <= 0 && _lastCallBlockFrequency > 0)
                cfg._calledFrequency = _lastCallBlockFrequency;

            heuristicTrace(tracer(), "Depth %d: Setting called count for caller index %d, bytecode index %d of %d",
                _recursionDepth, calltarget->_myCallSite->_bcInfo.getCallerIndex(),
        ...
        for (; bc != J9BCunknown; bc = bci.next()) {
            ...
            if (flags[i].testAny(bbStart)) {
                currentInlinedBlock = getBlock(comp(), blocks, calltarget->_calleeMethod, i, cfg);
                debugTrace(tracer(), "Found current block %p, number %d\n", currentInlinedBlock,
                    (currentInlinedBlock) ? currentInlinedBlock->getNumber() : -1);
            }
            ...
            case J9BCinvokevirtual: {
                ...
                if ((isUnresolvedInCP && !resolvedMethod) || (resolvedMethod && resolvedMethod->isCold(comp(), true))) {
                    ...
                    heuristicTrace(tracer(), "numberOfExplicitParameters = %d  pca.getNumPrevConstArgs = %d\n",
                        resolvedMethod->numberOfExplicitParameters(),
                        pca.getNumPrevConstArgs(resolvedMethod->numberOfExplicitParameters()));
                    if (resolvedMethod->numberOfExplicitParameters() > 0
                        && resolvedMethod->numberOfExplicitParameters()
                            <= pca.getNumPrevConstArgs(resolvedMethod->numberOfExplicitParameters()))
                        allconsts = true;
                    ...
                    if (resolvedMethod->convertToMethod()->isArchetypeSpecimen()
                        ...
                        callsite = new (comp()->trHeapMemory()) TR_J9MethodHandleCallSite(callStack._method,
                            callNodeTreeTop, parent, callNode, interfaceMethod, resolvedMethod->classOfMethod(),
                            (int32_t)resolvedMethod->virtualCallSelector(cpIndex), cpIndex, resolvedMethod,
                            resolvedSymbol, isIndirectCall, isInterface, newBCInfo, comp(), _recursionDepth, allconsts);
                    } else if (resolvedMethod->getRecognizedMethod() == TR::java_lang_invoke_MethodHandle_invokeExact) {
                        callsite = new (comp()->trHeapMemory()) TR_J9MutableCallSite(callStack._method, callNodeTreeTop,
                            parent, callNode, interfaceMethod, resolvedMethod->classOfMethod(),
                            (int32_t)resolvedMethod->virtualCallSelector(cpIndex), cpIndex, resolvedMethod,
                            resolvedSymbol, isIndirectCall, isInterface, newBCInfo, comp(), _recursionDepth, allconsts);
                    } else if (isIndirectCall) {
                        callsite = new (comp()->trHeapMemory()) TR_J9VirtualCallSite(callStack._method, callNodeTreeTop,
                            parent, callNode, interfaceMethod, resolvedMethod->classOfMethod(),
                            (int32_t)resolvedMethod->virtualCallSelector(cpIndex), cpIndex, resolvedMethod,
                            resolvedSymbol, isIndirectCall, isInterface, newBCInfo, comp(), _recursionDepth, allconsts);

                    } else {
                        callsite = new (comp()->trHeapMemory()) TR_DirectCallSite(callStack._method, callNodeTreeTop,
                            parent, callNode, interfaceMethod, resolvedMethod->classOfMethod(),
                            (int32_t)resolvedMethod->virtualCallSelector(cpIndex), cpIndex, resolvedMethod,
                            resolvedSymbol, isIndirectCall, isInterface, newBCInfo, comp(), _recursionDepth, allconsts);
                    }
                    ...
                    if (pca.isArgAtIndexReceiverObject(resolvedMethod->numberOfExplicitParameters())) {
                        // heuristicTrace(tracer(),"Arg at index %d is receiver object.  Propagating
                        // prexarginfo",resolvedMethod->numberOfExplicitParameters()); callsite->_ecsPrexArgInfo =
                        // calltarget->_ecsPrexArgInfo;
                    }
            ...
            case J9BCinvokespecialsplit: {
                ...
                if (!resolvedMethod || isUnresolvedInCP || resolvedMethod->isCold(comp(), false)) {
                    ...
                    TR_CallSite* callsite
                        = new (comp()->trHeapMemory()) TR_DirectCallSite(callStack._method, callNodeTreeTop, parent,
                            callNode, interfaceMethod, resolvedMethod->classOfMethod(), -1, cpIndex, resolvedMethod,
                            resolvedSymbol, isIndirectCall, isInterface, newBCInfo, comp(), _recursionDepth, allconsts);
            ...
            case J9BCinvokestaticsplit: {
                ...
                if (!resolvedMethod || isUnresolvedInCP || resolvedMethod->isCold(comp(), false)) {
                    ...
                    TR_CallSite* callsite = new (comp()->trHeapMemory())
                        TR_DirectCallSite(callStack._method, callNodeTreeTop, parent, callNode, interfaceMethod,
                            resolvedMethod->classOfMethod(), -1, cpIndex, resolvedMethod, resolvedSymbol,
                            isIndirectCall, isInterface, newBCInfo, _inliner->comp(), _recursionDepth, allconsts);
            ...
            case J9BCinvokeinterface: {
                ...
                if (resolvedMethod != NULL) {
                    isInterface = false;
                    isIndirectCall
                        = !resolvedMethod->isPrivate() && !resolvedMethod->convertToMethod()->isFinalInObject();
                }

                TR_Method* interfaceMethod = NULL;
                if (isInterface)
                    interfaceMethod = comp()->fej9()->createMethod(
                        comp()->trMemory(), calltarget->_calleeMethod->containingClass(), cpIndex);
                ...
                if (isInterface) {
                    ...
                    callsite = new (comp()->trHeapMemory()) TR_J9InterfaceCallSite(callStack._method, callNodeTreeTop,
                        parent, callNode, interfaceMethod, thisClass, -1, cpIndex, resolvedMethod, resolvedSymbol,
                        isIndirectCall, isInterface, newBCInfo, comp(), _recursionDepth, allconsts);
                } else if (isIndirectCall) {
                    callsite = new (comp()->trHeapMemory()) TR_J9VirtualCallSite(callStack._method, callNodeTreeTop,
                        parent, callNode, interfaceMethod, resolvedMethod->classOfMethod(), -1, cpIndex, resolvedMethod,
                        resolvedSymbol, isIndirectCall, isInterface, newBCInfo, comp(), _recursionDepth, allconsts);
                } else {
                    callsite = new (comp()->trHeapMemory()) TR_DirectCallSite(callStack._method, callNodeTreeTop,
                        parent, callNode, interfaceMethod, resolvedMethod->classOfMethod(), -1, cpIndex, resolvedMethod,
                ...
                if (isInterface && pca.isArgAtIndexReceiverObject(interfaceMethod->numberOfExplicitParameters())) {
                    // heuristicTrace(tracer(),"Arg at index %d is receiver object.  Propagating
                    // prexarginfo",interfaceMethod->numberOfExplicitParameters()); callsite->_ecsPrexArgInfo =
                    // calltarget->_ecsPrexArgInfo;
                }
        ...
        heuristicTrace(tracer(),
            "--- Depth %d: Checking Optimistic size vs Size Threshold: _optimisticSize %d _sizeThreshold %d "
            "sizeThreshold %d ",
            _recursionDepth, _optimisticSize, _sizeThreshold, sizeThreshold);

        if (_optimisticSize > sizeThreshold) // even optimistically we've blown our budget
        {
            calltarget->_isPartialInliningCandidate = false;
            heuristicTrace(tracer(),
                "*** Depth %d: ECS end for target %p signature %s. optimisticSize exceeds Size Threshold",
                _recursionDepth, calltarget, callerName);
        ...
        for (i = 0; i < maxIndex && inlineableCallExists; ++i) {
            // heuristicTrace(tracer(),"--- Depth %d: Checking _real size vs Size Threshold: _realSize %d _sizeThreshold
            // %d sizeThreshold %d ",_recursionDepth, _realSize, _sizeThreshold, sizeThreshold);
            ...
            if (callSites[i]) {
                ...
                for (int32_t j = 0; j < callSites[i]->numTargets(); j++) {
                    ...
                    if (tracer()->heuristicLevel())
                        calleeName
                            = comp()->fej9()->sampleSignature(targetCallee->_calleeMethod->getPersistentIdentifier(),
                    ...
                    if (callGraphEnabled && !currentBlock->isCold()) {
                        ...
                        if (!callSites[i]->isIndirectCall()) {
                            profileManager->updateCallGraphProfilingCount(
                                currentBlock, calltarget->_calleeMethod->getPersistentIdentifier(), i, comp());
                            heuristicTrace(tracer(),
                                "Depth %d: Updating Call Graph Profiling Count for calltarget %p count = %d",
                                _recursionDepth, calltarget,
                                profileManager->getCallGraphProfilingCount(
                        ...
                        // for inliner.
                        //
                        bool coldCallInfoIsReliable
                            = !cameFromArchetypeSpecimen(currentBlock->getJ9EstimateCodeSizeMethod());
                        ...
                        if (_inliner->getPolicy()->tryToInline(targetCallee, &callStack, true)) {
                            ...
                            bool isColdCall
                                = (((comp()->getMethodHotness() <= warm)
                                       && profileManager->isColdCall(
                                              targetCallee->_calleeMethod->getPersistentIdentifier(),
                                              calltarget->_calleeMethod->getPersistentIdentifier(), i, comp()))
                            ...
                            if (coldCallInfoIsReliable && isColdCall) {
                                heuristicTrace(tracer(),
                                    "Depth %d: Skipping estimate on call %s, with count=%d and block frequency %d, "
                                    "because it's cold.",
                            ...
                            if (comp()->getMethodHotness() <= warm && comp()->isServerInlining()
                                ...
                                && !_inliner->alwaysWorthInlining(targetCallee->_calleeMethod, NULL)) {
                                heuristicTrace(tracer(),
                                    "Depth %d: Skipping estimate on call %s, with count=%d, because its warm call "
                                    "graph is too big.",
                                    _recursionDepth, calleeName,
                                    profileManager->getCallGraphProfilingCount(
                                        calltarget->_calleeMethod->getPersistentIdentifier(), i, comp()));
                    ...
                    if (currentBlock->isCold()
                        ...
                        heuristicTrace(tracer(),
                            "Depth %d: Skipping estimate on call %s, because it's in a cold block.", _recursionDepth,
                            calleeName);
                    ...
                    if (_optimisticSize <= sizeThreshold) // for multiple calltargets, is this the desired behaviour?
                    {
                        ...
                        debugTrace(tracer(),
                            "About to call ecs on call target %p at depth %d _optimisticSize = %d _realSize = %d "
                            "_sizeThreshold = %d",
                            targetCallee, _recursionDepth, _optimisticSize, _realSize, _sizeThreshold);
                        heuristicTrace(tracer(),
                            "--- Depth %d: EstimateCodeSize to recursively estimate call from %s to %s",
                            _recursionDepth, callerName, calleeName);
                        ...
                        if ((comp()->getMethodHotness() >= warm) && comp()->isServerInlining()) {
                            ...
                            if (_optimisticSize - origOptimisticSize > bigCalleeThreshold) {
                                ...
                                /// newBCInfo._byteCodeIndex);fflush(stdout);
                        ...
                        heuristicTrace(tracer(),
                            "Depth %d: estimateCodeSize aborting due to _optimisticSize: %d > sizeThreshold: %d",
                            _optimisticSize, sizeThreshold);
                        break;
                    }
                }
                        ...
                        ->numTargets()) // only add a callSite once, even though it may have more than one call target.
                {
                    ...
                    heuristicTrace(tracer(),
                        "Depth %d: Subtracting %d from optimistic and real size to account for eliminating call",
                        _recursionDepth, bci.estimatedCodeSize());
                    if (_optimisticSize > bci.estimatedCodeSize())
                        _optimisticSize -= bci.estimatedCodeSize(); // subtract what we added before for the size of the
                                                                    // call instruction
        ...
        if (adjustEstimateForStringCompression(
                calltarget->_calleeMethod, calltarget->_partialSize, STRING_COMPRESSION_ADJUSTMENT_FACTOR)) {
            heuristicTrace(tracer(),
                "*** Depth %d: Adjusting partial size for %s because of string compression from %d to %d",
        ...
        if (adjustEstimateForStringCompression(
                calltarget->_calleeMethod, calltarget->_fullSize, STRING_COMPRESSION_ADJUSTMENT_FACTOR)) {
            heuristicTrace(tracer(),
                "*** Depth %d: Adjusting full size for %s because of string compression from %d to %d", _recursionDepth,
                callerName, fullSizeBeforeAdjustment, calltarget->_fullSize);
        }

        auto realSizeBeforeAdjustment = _realSize;

        if (adjustEstimateForStringCompression(
                calltarget->_calleeMethod, _realSize, STRING_COMPRESSION_ADJUSTMENT_FACTOR)) {
            heuristicTrace(tracer(),
                "*** Depth %d: Adjusting real size for %s because of string compression from %d to %d", _recursionDepth,
                callerName, realSizeBeforeAdjustment, _realSize);
        }

        reduceDAAWrapperCodeSize(calltarget);

        /****************** PHASE 5: Figure out if We're really going to do a partial Inline and add whatever we do to
         * the realSize. *******************/

        if (isPartialInliningCandidate(calltarget, &callBlocks)) {
            if (comp()->getOption(TR_TraceBFGeneration))
                traceMsg(comp(), "Call Target %s is a partial inline Candidate with a partial size of %d", callerName,
                    calltarget->_partialSize);

            heuristicTrace(tracer(),
                "*** Depth %d: ECS end for target %p signature %s. It is a partial inline Candidate with a partial "
                "size of %d",
                _recursionDepth, calltarget, callerName, calltarget->_partialSize);
            _realSize += calltarget->_partialSize;
        } else {
            heuristicTrace(tracer(),
                "*** Depth %d: ECS end for target %p signature %s. It is a full inline Candidate with a full size of "
                "%d",
        ...
        if (_realSize > sizeThreshold) {
            heuristicTrace(tracer(),
                "*** Depth %d: ECS end for target %p signature %s. real size exceeds Size Threshold", _recursionDepth,
                calltarget, callerName);
            return returnCleanup(4);
        }
    }
...
bool TR_J9EstimateCodeSize::graphSearch(
    TR::CFG* cfg, TR::Block* startBlock, TR::Block::partialFlags searchItem, TR::Block::partialFlags searchPath)
{
...
int32_t TR_J9EstimateCodeSize::labelGraph(
    TR::CFG* cfg, TR_Queue<TR::Block>* unsanitizeableBlocks, TR_Queue<TR::Block>* callBlocks)
{
    TR_BitVector* blocksVisited
        = new (comp()->trStackMemory()) TR_BitVector(cfg->getNextNodeNumber(), comp()->trMemory(), stackAlloc);
    blocksVisited->empty();
...
bool TR_J9EstimateCodeSize::trimBlocksForPartialInlining(TR_CallTarget* calltarget, TR_Queue<TR::Block>* callBlocks)
{
        ...
        > -1) // a size of -1 means we didn't have any restart blocks - so no sense in doing a 'partial' inline
    {
        ...
        partialTrace(
            tracer(), "TrimBlocksForPartialInlining: No restart blocks found in candidate. Doing a full inline");
    }
