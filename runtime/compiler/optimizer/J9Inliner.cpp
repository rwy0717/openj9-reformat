...
static uintptrj_t* isMutableCallSiteTargetInvokeExact(TR_CallSite* callSite, TR_InlinerBase* inliner)
{
    ...
    if (mcsNode->getSymbolReference()->hasKnownObjectIndex()) {
        ...
        heuristicTrace(inliner->tracer(), "  Success: isMutableCallSiteTargetInvokeExact(%p)=%p (obj%d)", callSite,
            result, mcsNode->getSymbolReference()->getKnownObjectIndex());
        return result;
        ...
        heuristicTrace(inliner->tracer(),
            "  Success: isMutableCallSiteTargetInvokeExact(%p)=%p (fixed object reference)", callSite, result);
        return result;
    } else {
...

{
    ...
    if (callNode->getOpCode().isCallIndirect()) {
        if (calleeSymbol->isInterface()) {
            return new (trMemory, kind) TR_J9InterfaceCallSite(lCaller, callNodeTreeTop, parent, callNode,
                ...
                calleeSymbol->isInterface(), callNode->getByteCodeInfo(), comp);
        } else {
    ...
    return new (trMemory, kind)
        TR_DirectCallSite(lCaller, callNodeTreeTop, parent, callNode, calleeSymbol->getMethod(), receiverClass,
            (int32_t)symRef->getOffset(), symRef->getCPIndex(), resolvedMethod, calleeSymbol->getResolvedMethodSymbol(),
            callNode->getOpCode().isCallIndirect(), calleeSymbol->isInterface(), callNode->getByteCodeInfo(), comp);
}
    ...
    TR_CallStack* callStack, int32_t& numLivePendingPushSlots, int32_t& nestingDepth)
{
    if (optimizer->comp()->getOption(TR_EnableOSR)) {
        ...
        TR_OSRMethodData* osrMethodData = optimizer->comp()->getOSRCompilationData()->findOrCreateOSRMethodData(
            callNode->getByteCodeInfo().getCallerIndex(),
            callNode->getSymbolReference()->getOwningMethodSymbol(optimizer->comp()));
        TR_Array<List<TR::SymbolReference> >* pendingPushSymRefs
            = callNode->getSymbolReference()->getOwningMethodSymbol(optimizer->comp())->getPendingPushSymRefs();
...
static void populateOSRCallSiteRematTable(TR::Optimizer* optimizer, TR_CallTarget* calltarget, TR_CallStack* callStack)
{
            ...
            && (child->getSymbol()->isParm()
                   || (child->getSymbol()->isAuto()
                          && child->getSymbolReference()->getCPIndex()
                              < child->getSymbolReference()->getOwningMethodSymbol(comp)->getFirstJitTempIndex()))) {
    ...
    if (failedPP.size() > 0)
        RematTools::walkTreeTopsCalculatingRematFailureAlternatives(
            comp, blockStart, call, failedPP, scanTargets, safetyInfo, verboseCallSiteRemat != NULL);
    ...
    if (!scanTargets.IsZero())
        RematTools::walkTreesCalculatingRematSafety(
            comp, blockStart, call, scanTargets, unsafeSymRefs, verboseCallSiteRemat != NULL);
    ...
    for (uint32_t i = 0; i < safetyInfo.size(); ++i) {
        ...
        if (!unsafeSymRefs.Intersects(safetyInfo.symRefDependencies(i))) {
            ...
            } else if (node->getSymbol()->isParm() || node->getSymbol()->isPendingPush()
                || (node->getSymbol()->isAuto()
                       && node->getSymbolReference()->getCPIndex()
                           < node->getSymbolReference()->getOwningMethodSymbol(comp)->getFirstJitTempIndex())) {
                if (comp->trace(OMR::inlining))
                    traceMsg(comp, "callSiteRemat: adding pending push #%d with store #%d to remat table\n",
                        storeTree->getNode()->getSymbolReference()->getReferenceNumber(),
    ...
    TR_PrexArgInfo* argInfo, TR::TreeTop** cursorTreeTop)
{
    ...
    if (!comp()->incInlineDepth(calltarget->_calleeSymbol, calltarget->_myCallSite->_callNode->getByteCodeInfo(),
            calltarget->_myCallSite->_callNode->getSymbolReference()->getCPIndex(),
            calltarget->_myCallSite->_callNode->getSymbolReference(), !calltarget->_myCallSite->_isIndirectCall,
            argInfo)) {
        return false;
    }
    ...
    if (comp()->getOption(TR_EnableOSR)) {
        computeNumLivePendingSlotsAndNestingDepth(
            _optimizer, calltarget, callStack, numLivePendingPushSlots, nestingDepth);
    }
...
bool TR_J9InterfaceCallSite::findCallSiteTarget(TR_CallStack* callStack, TR_InlinerBase* inliner)
{
    ...
    TR_ResolvedMethod* calleeResolvedMethod
        = comp()->getPersistentInfo()->getPersistentCHTable()->findSingleImplementer(
            _receiverClass, _cpIndex, _callerResolvedMethod, inliner->comp(), false, TR_yes);
    ...
    if (calleeResolvedMethod && !calleeResolvedMethod->virtualMethodIsOverridden()) {
        TR_VirtualGuardSelection* guard
            = new (comp()->trHeapMemory()) TR_VirtualGuardSelection(TR_InterfaceGuard, TR_MethodTest);
...
void TR_J9InterfaceCallSite::findSingleProfiledMethod(
    ListIterator<TR_ExtraAddressInfo>& sortedValuesIt, TR_AddressInfo* valueInfo, TR_InlinerBase* inliner)
{
    return;
}
...
{
    uintptrj_t* mcsReferenceLocation
        = isMutableCallSiteTargetInvokeExact(this, inliner); // JSR292: Looking for calls through MutableCallSite
    if (mcsReferenceLocation) {
...

        {
            ...
            if (mcsObject) {
                ...
                uintptrj_t currentEpoch
                    = fej9->getVolatileReferenceField(mcsObject, "epoch", "Ljava/lang/invoke/MethodHandle;");
                if (currentEpoch)
        ...
        if (vgs->_mutableCallSiteEpoch != TR::KnownObjectTable::UNKNOWN) {
            ...
            TR_CallTarget* target
                = addTarget(comp()->trMemory(), inliner, vgs, specimenMethod, _receiverClass, heapAlloc);
            ...
            heuristicTrace(inliner->tracer(), "  addTarget: MutableCallSite.epoch is %p.obj%d (%p.%p)",
                vgs->_mutableCallSiteObject, vgs->_mutableCallSiteEpoch, *vgs->_mutableCallSiteObject,
...
void TR_ProfileableCallSite::findSingleProfiledReceiver(
    ListIterator<TR_ExtraAddressInfo>& sortedValuesIt, TR_AddressInfo* valueInfo, TR_InlinerBase* inliner)
{
    ...
    for (TR_ExtraAddressInfo* profiledInfo = sortedValuesIt.getFirst(); profiledInfo != NULL;
        ...
        static const float minProfiledCallFrequency = userMinProfiledCallFreq
            ? atof(userMinProfiledCallFreq)
        ...
        if ((val >= minProfiledCallFrequency || (firstInstanceOfCheckFailed && val >= SECOND_BEST_MIN_CALL_FREQUENCY))
            ...
            if (comp()->compileRelocatableCode()) {
                if (tempreceiverClass && comp()->getOption(TR_UseSymbolValidationManager)) {
                    ...
                    if (!comp()->getSymbolValidationManager()->addClassFromMethodRecord(
                            target_method->classOfMethod(), target_method->getPersistentIdentifier()))
                        continue;
                }
...
void TR_ProfileableCallSite::findSingleProfiledMethod(
    ListIterator<TR_ExtraAddressInfo>& sortedValuesIt, TR_AddressInfo* valueInfo, TR_InlinerBase* inliner)
{
    ...
    for (TR_ExtraAddressInfo* profiledInfo = sortedValuesIt.getFirst(); profiledInfo != NULL;
        ...
        if (comp()->compileRelocatableCode()) {
            ...
            TR_SharedCache* sharedCache = fej9->sharedCache();
            if (!sharedCache->canRememberClass(clazz) || !sharedCache->canRememberClass(callSiteClass)) {
                classValuesAreSane = false;
                break;
            }
        }
    ...
    if (comp()->trace(OMR::inlining))
        traceMsg(
            comp(), "OK, all classes check out, we'll try to get their method implementations (%d).\n", numMethods);
