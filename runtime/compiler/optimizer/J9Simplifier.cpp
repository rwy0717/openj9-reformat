...
TR::Node* J9::Simplifier::unaryCancelOutWithChild(
    ...
    if (node->getType().isBCD() && grandChild->getType().isBCD()
        ...
        if (childP < nodeP && childP < grandChildP) {
            ...
            dumpOptDetails(comp(),
                ...
                "] grandChild %s [" POINTER_PRINTF_FORMAT "] (%d != %d)\n",
                optDetailString(), grandChild->getOpCode().getName(), grandChild, node->getOpCode().getName(), node,
                origOrigGrandChild->getOpCode().getName(), origOrigGrandChild, nodeP, childP);
        }
    }
...
TR::Node* J9::Simplifier::convertCurrentTimeMillis(TR::Node* node, TR::Block* block)
{
    if (performTransformation(comp(),
        ...
        TR::Node* lcallNode = TR::Node::createWithSymRef(
            node, TR::lcall, 0, comp()->getSymRefTab()->findOrCreateCurrentTimeMaxPrecisionSymbol());
    ...
    if ((node->getOpCode().isCallDirect()) && !node->getSymbolReference()->isUnresolved()
        ...
        if (isSimplifiableMethod && (node->getReferenceCount() == 1)
            && performTransformation(comp(),
                   requiresArgNULLCHK
                       ? ("%sReplaced dead BigDecimal/BigInteger call node [" POINTER_PRINTF_FORMAT
            ...
            if (requiresArgNULLCHK) {
                ...
                TR::TreeTop::create(comp(), _curTree,
                    TR::Node::createWithSymRef(
                        node, TR::NULLCHK, 1, TR::Node::create(node, TR::PassThrough, 1, secondChild), nullCheckSR));
                _alteredBlock = true;
            }
...
TR::Node* J9::Simplifier::simplifyiOrPatterns(TR::Node* node)
{
    ...
    if (firstChild->getReferenceCount() == 1 && firstChildOp == TR::ior
        ...
        if ((addr = getUnsafeBaseAddr(byte2, -1)) && addr == byte1 && (addr = getUnsafeBaseAddr(byte3, -2))
            ...
            && performTransformation(
                   comp(), "%sconvert ior to iiload node [" POINTER_PRINTF_FORMAT "]\n", optDetailString(), node)) {
    ...
    if (!disableIORByteSwap && !TR::Compiler->target.cpu.isBigEndian() && // the bigEndian case needs more thought
        ...
        if (secondChild->getSecondChild()->getOpCodeValue() == TR::ior && secondChild->getReferenceCount() == 1
            ...
            && (byte3 = getArrayByteChildWithMultiplier(
                    secondChild->getSecondChild()->getFirstChild(), TR::bu2i, SHIFT8MUL))
    ...
    if (firstChild->getOpCodeValue() == TR::l2i && secondChild->getOpCodeValue() == TR::l2i
        ...
        if (secondGrandChild->getFirstChild()->getOpCodeValue() == TR::lneg
            ...
            if (secondGrandChild->getFirstChild()->getFirstChild() == loadVal
                ...
                && performTransformation(
                       comp(), "%sTransform ior to lcmp [" POINTER_PRINTF_FORMAT "]\n", optDetailString(), node)) {
...
TR::Node* J9::Simplifier::simplifyi2sPatterns(TR::Node* node)
{
    ...
    if (firstChild->getOpCodeValue() == TR::ior && firstChild->getReferenceCount() == 1
        ...
        && performTransformation(
               comp(), "%sconvert ior to isload node [" POINTER_PRINTF_FORMAT "]\n", optDetailString(), node)) {
...
TR::Node* J9::Simplifier::simplifyIndirectLoadPatterns(TR::Node* node)
{
    ...
    if (nodeOp == TR::aloadi || nodeOp == TR::iloadi || nodeOp == TR::lloadi) {
        if (childOp == TR::aloadi || childOp == TR::iloadi || childOp == TR::lloadi) {
            ...
            if (symRefPairMatches(nodeSymref, childSymref, getSymRefTab()->findClassFromJavaLangClassSymbolRef(),
            ...
            if (fieldsAreComplementary
                ...
                if (replacement->getDataType() != node->getDataType()) {
                    ...
                    TR_ASSERT(
                        convOpCode != TR::BadILOp, "Conversion between two different data types requires an opcode");
