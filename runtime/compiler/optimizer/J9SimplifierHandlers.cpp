...
static TR::Node* foldAndReplaceDominatedSetSign(
    TR::Node* setSign, bool setSignIsTheChild, TR::Node* other, TR::Block* block, TR::Simplifier* s);
...
static TR::Node* removeUnnecessaryDFPClean(TR::Node* child, TR::Node* parent, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (child->getOpCodeValue() == dfpCleanOp
        && performTransformation(s->comp(),
               "%s%s [" POINTER_PRINTF_FORMAT "] does not need DFP cleaning child %s [" POINTER_PRINTF_FORMAT
...
static TR::Node* getRedundantDFPSetSignDescendant(TR::Node* node, TR::Node** parent)
{
    ...
    if (!node->getType().isDFP() || node->getReferenceCount() > 1)
        return NULL;
    if (op == TR::dfabs || op == TR::ddabs || op == TR::deabs || op == TR::dfSetNegative || op == TR::ddSetNegative
        || op == TR::deSetNegative)
        return node;
    if (node->getOpCode().isConversion())
        return NULL;
    if (node->getOpCode().isAdd() || node->getOpCode().isSub() || node->getOpCode().isMul()
...
static TR::Node* foldSetSignFromGrandChild(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (node->getReferenceCount() == 1 && newSetSignOp != TR::BadILOp && child->getReferenceCount() == 1
        ...
        if (convertedSetSign != TR::DataType::getInvalidSignCode()
            ...
            if (newNode) {
                dumpOptDetails(
                    s->comp(), "%s node [" POINTER_PRINTF_FORMAT "]\n", newNode->getOpCode().getName(), newNode);
                                                      ...
                                                      // there is a pdclean)
        {
            ...
            if (shiftedPrecision <= TR::DataType::getMaxPackedDecimalPrecision()
                ...
                && performTransformation(s->comp(),
                       ...
                       s->optDetailString(), conversion->getOpCode().getName(), conversion,
                       shift->getOpCode().getName(), shift, shift->getDecimalPrecision(), shiftedPrecision)) {
                if (pdclean) {
...
static TR::Node* flipBinaryChildAndUnaryParent(
    TR::Node* parent, TR::Node* child, TR::ILOpCodes newParentOp, TR::Block* block, TR::Simplifier* s)
{
...

        {
            ...
            if (newSetSignIndex == 1) // child 2
            {
                if (child->getOpCode().isConversion() && child->getFirstChild()->getType().isUnicodeSeparateSign()
                    ...
                    TR_ASSERT(
                        false, "unexpected parent/child combination in pd2udSimplifier for newSetSignIndex == 1\n");
                }
...
TR::Node* udsx2pdSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (decodeConversionOpcode(node->getOpCode(), node->getDataType(), sourceDataType, targetDataType)) {
        TR::ILOpCodes inverseOp = TR::ILOpCode::getProperConversion(
            targetDataType, sourceDataType, false /* !wantZeroExtension */); // inverse conversion to what we are given
...
TR::Node* pd2udslSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (node->getFirstChild()->getOpCodeValue() == TR::pdSetSign) {
        TR::Node* newNode = foldSetSignIntoNode(
            node->getFirstChild(), true /* setSignIsTheChild */, node, true /* removeSetSign */, block, s);
        if (newNode != node)
            return newNode;
    }
...
TR::Node* pd2zdSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (node->getDecimalPrecision() == firstChild->getDecimalPrecision() && firstChild->isSimpleTruncation()
        && performTransformation(s->comp(),
               ...
               "]\n",
               s->optDetailString(), firstChild->getOpCode().getName(), firstChild, node->getOpCode().getName(),
               node)) {
    ...
    if ((child->getReferenceCount() == 1 && node->getReferenceCount() == 1)
        ...
        if ((node->getDecimalPrecision() == child->getDecimalPrecision())
            ...
            if (childOpValueForLookup != TR::BadILOp
                ...
                TR::ILOpCodes newConvOp = TR::ILOpCode::getProperConversion(
                    sourceDataType, TR::ZonedDecimal, false /* !wantZeroExtension */);
                TR::Node* newNode = NULL;
    ...
    if (!s->comp()->getOption(TR_DisableZonedToDFPReduction) && s->cg()->supportsZonedDFPConversions()
        && child->getOpCodeValue() == TR::pdclean && child->getFirstChild()->getOpCode().isConversion()
        ...
        //    deX
        //
        TR::ILOpCodes convOp = TR::ILOpCode::dfpToZonedCleanOp(
            child->getFirstChild()->getFirstChild()->getDataType(), node->getDataType());
        if (convOp != TR::BadILOp && !node->hasIntermediateTruncation()
            && performTransformation(s->comp(), "%sFold %s (0x%p) and dfpToZoned cleaning child %s (0x%p) to ",
                   s->optDetailString(), node->getOpCode().getName(), node, child->getOpCode().getName(), child)) {
            ...
            TR::Node::recreate(node,
                convOp); // pd2zd -> de2zdClean (momentarily type incorrect but not exposed outside this simplifier));
            node->setFlags(0);
            ...
            //    de2pd
            //       deX
            // correctBCDPrecision=false trunc checks already done and top level prec is being maintained (false is
            // needed so pdclean doesn't just get changed to a pdModPrec)
            node->setChild(0,
                s->replaceNodeWithChild(
                    node->getFirstChild(), node->getFirstChild()->getFirstChild(), s->_curTree, block, false));
            // pd2zd  (de2zdClean)
            //    de2pd
            //      deX
            // to
            ...
            //    deX
            // correctBCDPrecision=false trunc checks already done and top level prec is being maintained (false is
            // needed so de2pd doesn't just get changed to a pdModPrec)
            node->setChild(0,
                s->replaceNodeWithChild(
                    node->getFirstChild(), node->getFirstChild()->getFirstChild(), s->_curTree, block, false));
    ...
    if (!s->comp()->getOption(TR_DisableZonedToDFPReduction) && s->cg()->supportsZonedDFPConversions()
        && child->getOpCode().isConversion() && child->getFirstChild()->getType().isDFP()
        && child->getDecimalFraction() == 0 && !child->useCallForFloatToFixedConversion()) {
        // pd2zd
        ...
        bool isClean = (child->getOpCodeValue()
            == TR::ILOpCode::dfpToPackedCleanOp(child->getFirstChild()->getDataType(), child->getDataType()));
        ...
        if (!isSetSign || (sign != raw_bcd_sign_unknown)) {
            // could generalize the <= prec check below but would have to start looking at DFP max precision in
            // replaceNodeWithChild e.g. the de2pd could be truncating if the result pd precision is less than the max
            // possible from a de but for now just handle the common case of <= (this is safe as a truncation on top of
            // another truncation is ok)
            if (convOp != TR::BadILOp && node->getDecimalPrecision() <= child->getDecimalPrecision()
                && // don't lose a truncation
                performTransformation(s->comp(), "%sFold %s (0x%p) and dfpToPacked child %s (0x%p) to ",
                    s->optDetailString(), node->getOpCode().getName(), node, child->getOpCode().getName(), child)) {
                ...
                removePaddingNode(node, s);
                TR::Node::recreate(node,
                    convOp); // pd2zd -> de2zd (momentarily type incorrect but not exposed outside this simplifier)
                node->setFlags(0);
                if (isSetSign) {
                    node->resetSignState();
...
TR::Node* zd2dfSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    bool isAbs = false;
    if (node->getOpCodeValue() == TR::zd2dfAbs || node->getOpCodeValue() == TR::zd2ddAbs
    ...
    if (!isAbs) {
        TR::Node* result = s->unaryCancelOutWithChild(node, child, s->_curTree,
            TR::ILOpCode::getProperConversion(node->getDataType(), child->getDataType(), false));
        if (result != NULL)
            return result;
    }
...
TR::Node* df2zdSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    TR::Node* child = node->getFirstChild();
    TR::Node* result = s->unaryCancelOutWithChild(
        node, child, s->_curTree, TR::ILOpCode::getProperConversion(node->getDataType(), child->getDataType(), false));
    if (result != NULL)
        return result;
...
static TR::Node* lowerPackedShiftOrSetSignBelowDFPConv(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    TR::ILOpCodes op = node->getOpCodeValue();
    TR_ASSERT(
        op == TR::pdshl || op == TR::pdshr || op == TR::pdshlSetSign || op == TR::pdshrSetSign || op == TR::pdSetSign,
        "Node %s [0x%x] isn't a packed shift/setSign in lowerPackedShiftOrSetSignBelowDFPConv\n",
        node->getOpCode().getName(), node);

    TR::Node* firstChild = node->getFirstChild();

    if (firstChild->getOpCode().isConversion() && firstChild->getFirstChild()->getType().isDFP()
        ...
        // If the shift node widens, moving it below the conversion might require it to be a different DFP type than
        // dfpNode. Find the DFP types needed to represent the shift and conv nodes, and if they're not the same, insert
        // a dX2dY node. A shift could also widen and then truncate in one op, so we want to use the DFP type that fits
        // the max precision out of the node's precision, the child's precision, and the shifted amount, to ensure that
        // truncation only happens on the final DFP-to-packed conversion.
        ...
        if (isShift)
            maxDFPPrecision = std::max(
                firstChild->getDecimalPrecision() + node->getDecimalAdjust() + roundAmountBump, maxDFPPrecision);

        if (maxDFPPrecision > TR::DataType::getMaxExtendedDFPPrecision()) {
            if (s->trace() || s->comp()->cg()->traceBCDCodeGen())
                traceMsg(s->comp(),
                    "z^z : maxDFPPrecision %d > TR::DataType::getMaxExtendedDFPPrecision() %d in attempting to create "
                    "a dfp shift from %s (%p) at line_no=%d (offset=%06X)\n",
                    maxDFPPrecision, TR::DataType::getMaxExtendedDFPPrecision(), node->getOpCode().getName(), node,
                    s->comp()->getLineNumber(node), s->comp()->getLineNumber(node));
            return node;
        }
        ...
        int32_t sign = isSetSign ? signNode->get32bitIntegralValue()
                                 : 0xc; // Default of + is safe here; sign will only be set if isSetSign is true

        if ((round == 0 || round == 5) && resultType != TR::NoType && convType != TR::NoType
            && (!isSetSign || TR::DataType::isSupportedRawSign(sign))
            && performTransformation(s->comp(),
                   "%sMoving %s [" POINTER_PRINTF_FORMAT "] below child %s [" POINTER_PRINTF_FORMAT "]\n",
                   s->optDetailString(), node->getOpCode().getName(), node, firstChild->getOpCode().getName(),
                   firstChild)) {
            TR::Node *shift = node, *newShift = NULL, *conv = firstChild, *newConv, *dfpNode = conv->getFirstChild(),
                     *shiftAmountNode = node->getSecondChild();

            // Without rounding, pdshX[SetSign] becomes ddshX
            if (isShift) {
                if (round == 0) {
                    newShift = TR::Node::create(node,
                        TR::ILOpCode::dfpOpForBCDOp(
                            node->getOpCode().isRightShift() ? TR::pdshr : TR::pdshl, resultType),
                        2);
                } else if (round == 5) {
                    ...
                    if (roundType != resultType) {
                        TR::Node* t = TR::Node::create(
                            node, TR::ILOpCode::getProperConversion(roundType, resultType, false), 1);
                        t->setAndIncChild(0, roundNode);
                        dumpOptDetails(s->comp(),
                            "Created conv %s [" POINTER_PRINTF_FORMAT "] on child round %s [" POINTER_PRINTF_FORMAT
                            "]\n",
                    ...
                    dumpOptDetails(s->comp(),
                        "Created dfp shift %s [" POINTER_PRINTF_FORMAT "] and child round %s [" POINTER_PRINTF_FORMAT
                        "]\n",
                ...
                if (resultType != convType) {
                    TR::Node* dfpConv
                        = TR::Node::create(node, TR::ILOpCode::getProperConversion(convType, resultType, false), 1);
                    dfpConv->setAndIncChild(0, dfpNode);
                    newShift->setAndIncChild(0, dfpConv);
                    dumpOptDetails(s->comp(),
                        "%sCreated node %s [" POINTER_PRINTF_FORMAT "] on top of %s [" POINTER_PRINTF_FORMAT
                        "] to preserve shift precision\n",
                        s->optDetailString(), dfpConv->getOpCode().getName(), dfpConv, dfpNode->getOpCode().getName(),
                        dfpNode);
                } else
                ...
                dumpOptDetails(s->comp(),
                    "%screate newShift %s [" POINTER_PRINTF_FORMAT "] of %s [" POINTER_PRINTF_FORMAT
                    "] by %s [" POINTER_PRINTF_FORMAT "]\n",
                    s->optDetailString(), newShift->getOpCode().getName(), newShift,
                ...
                if (resultType != convType) {
                    ...
                    newShift = dfpConv;
                    dumpOptDetails(s->comp(),
                        "%sCreated node %s [" POINTER_PRINTF_FORMAT "] to preserve shift precision\n",
            ...
            if (isSetSign) {
                ...
                newConv = TR::Node::create(
                    node, TR::ILOpCode::getProperConversion(resultType, TR::PackedDecimal, false), 1);
            }
            ...
            //   iconst 4                    iconst 4
            if (conv->hasSourcePrecision()
                && conv->getSourcePrecision() + shift->getDecimalAdjust() + roundAmountBump > 0)
                newConv->setSourcePrecision(conv->getSourcePrecision() + shift->getDecimalAdjust() + roundAmountBump);
            dumpOptDetails(s->comp(),
                "%sReplacing %s [" POINTER_PRINTF_FORMAT "] and child %s [" POINTER_PRINTF_FORMAT
                "] with %s [" POINTER_PRINTF_FORMAT "]\n",
                s->optDetailString(), shift->getOpCode().getName(), shift, conv->getOpCode().getName(), conv,
...
static TR::Node* reduceShiftRightOverShiftLeft(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    if (node->getOpCode().isPackedRightShift() && node->getFirstChild()->getOpCode().isPackedLeftShift()
        ...
        if (performTransformation(s->comp(),
            ...
            if (hasAdjust) {
                node->setChild(
                    1, s->replaceNode(node->getChild(1), TR::Node::iconst(node, abs(combinedAdjust)), s->_curTree));
            }
            if (resultIsSetSign) {
    ...
    if (performTransformation(s->comp(),
        ...
        node->setChild(0,
            s->replaceNodeWithChild(node->getChild(0), node->getChild(0)->getFirstChild(), s->_curTree, block,
                false)); // correctBCDPrecision=false because truncChecks already done above
        node->setChild(
            1, s->replaceNode(node->getChild(1), TR::Node::iconst(node, nodeShift + childShift), s->_curTree));
...
// TypeReduction can convert packed math to DFP math. If the packed math is used in cases where integer math might
// overflow, we could end up with something like pd2l -> dd2pd -> ddadd. Converting DFP to packed is expensive, so we
// should go directly from DFP to binary. Sometimes, due to commoning, we may also end up with a tree like pd2i -> zd2pd
// -> dd2zdSetSign (with this node commoned) -> ddX. In this case, it's still faster to break commoning and go from DFP
// to integer directly, setting the sign as needed.
static TR::Node* cancelDFPtoBCDtoBinaryConversion(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    TR_ASSERT(op == TR::pd2l || op == TR::pd2lu || op == TR::pd2i || op == TR::pd2iu,
        "Node %s [0x%x] isn't a packed to binary conversion in cancelDFPtoBCDtoBinaryConversion\n",
        node->getOpCode().getName(), node);
    ...
    if (firstChild->getOpCode().isConversion() && firstChild->getType().isBCD()) {
        ...
        int32_t targetPrec = std::min<int32_t>(
            node->getDataType().TR::DataType::getMaxPrecisionFromType() - 1, firstChild->getDecimalPrecision());
        ...
        if (firstChild->getFirstChild()->getOpCode().isConversion() && firstChild->getFirstChild()->getType().isBCD()) {
            ...
            if (setSignNode == NULL
                && (firstChild->getOpCode().isSetSign() || firstChild->getOpCode().isSetSignOnNode()))
                setSignNode = firstChild;
        }
        ...
        if (!child->getType().isDFP()
            || !performTransformation(s->comp(), "%sFold %s [0x%p] with child %s [0x%p]", s->optDetailString(),
        ...
        // Truncation is always fastest in binary. Ideally, we can convert to the final integral type and use a rem to
        // truncate. If necessary, we'll convert to a larger type, use rem, and then convert to a smaller type.
        // intermediateType contains the type used for the rem; if no rem is needed, it contains the final integral
        // type. Using a DFP modify precision is too slow, so bail out in that case.
        ...
        if (setSignNode != NULL) {
            ...
            if (useddabs && (sign == raw_bcd_sign_0xf || sign == raw_bcd_sign_0xc)) {
                ...
                if (intermediateType != child->getDataType()) {
                    newConv = TR::Node::create(node,
                        TR::ILOpCode::getProperConversion(
                            child->getDataType(), intermediateType, node->getOpCode().isUnsigned()),
                        1);
        ...
        if (child->getDataType() != node->getDataType()) {
            newConv = TR::Node::create(node,
                TR::ILOpCode::getProperConversion(
                    child->getDataType(), node->getDataType(), node->getOpCode().isUnsigned()),
                1);
            newConv->setAndIncChild(0, child);
            child = newConv;
        }
...
static TR::Node* removeGrandChildClean(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getReferenceCount() == 1
        ...
        grandChild = firstChild->setChild(
            0, s->replaceNodeWithChild(grandChild, grandChild->getFirstChild(), s->_curTree, block));
    }
    return grandChild;
}

static TR::Node* cancelPackedToIntegralConversion(
    TR::Node* node, TR::ILOpCodes reverseOp, TR::Block* block, TR::Simplifier* s)
{
...
static TR_PackedCompareCondition packedCompareCommon(
    TR::Node* parent, TR::Node* op1, TR::Node* op2, bool ignoreConstSign, TR::Simplifier* s)
{
    if (s->trace())
        traceMsg(s->comp(),
            "packedCompare: (parent  %s (%p) castedToBCD=%s, op1 %s (%p), op2 %s (%p), absVal=%s) %s vs %s",
            parent->getOpCode().getName(), parent, parent->castedToBCD() ? "yes" : "no", op1->getOpCode().getName(),
            op1, op2->getOpCode().getName(), op2, ignoreConstSign ? "true" : "false", op1->getOpCode().getName(),
    ...
    if (child->getReferenceCount() == 1 && child->getOpCode().isConversion()
        ...
        if (survivingDigits > 0
            && performTransformation(s->comp(),
                   ...
                   s->optDetailString(), child->getOpCode().getName(), child, survivingDigits,
                   node->getOpCode().getName(), node)) {
...
static TR::Node* foldSetSignIntoGrandChild(TR::Node* setSign, TR::Block* block, TR::Simplifier* s)
{
    ...
    // operation dominates it) choose one that is the zone value (0xf) as this is most likely to end up being being an
...
TR::Node* pdsubSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->isZero()
        && performTransformation(s->comp(),
               ...
               s->optDetailString(), node->getOpCode().getName(), node, secondChild->getOpCode().getName(),
               secondChild)) {
...
TR::Node* pdcleanSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    // Fold pdclean over de2pd into de2pdClean at odd precisions only on ARCH(11). For even precisions, if the leftmost
    // nibble is non-zero, we won't clear it out until after the CPDT or CPXT, so they won't have the chance to clean
    // the sign.
    if (s->comp()->cg()->supportsFastPackedDFPConversions() && !node->hasIntermediateTruncation()
        && child->getOpCode().isConversion() && child->getFirstChild()->getType().isDFP()
        && child->getDecimalFraction() == 0 && !child->useCallForFloatToFixedConversion()
        ...
        //
        TR::ILOpCodes convOp
            = TR::ILOpCode::dfpToPackedCleanOp(child->getFirstChild()->getDataType(), node->getDataType());
    ...
    // since only oddPrecision cases can be handled for supportsFastPackedDFPConversions with de2pdClean in the
    // transformation above attempt to swing the pdclean below to a ddclean/declean regardless of
    // supportsFastPackedDFPConversions
    if ((child->getOpCodeValue() == TR::de2pd || child->getOpCodeValue() == TR::dd2pd)
        && // avoid doing for de2pdClean/dd2pdClean
        ...
        && !child->isTruncating()) // otherwise will need a post clean anyway so no point inserting a declean/ddclean
    {
        ...
        if (child->getFirstChild()->getOpCode().isSub() && child->getFirstChild()->getFirstChild()->isNonNegative()
            && child->getFirstChild()->getSecondChild()->isNonNegative()) {
            // Although the ddsub cannot be marked with isNonNegative itself there still will not be a negative zero for
            // this case when dd2pd is not truncated (already checked above) dd2pd  (not truncating)
            //   ddsub
            //     ddX X>=0
            //     ddY X>=0
        ...
        if (cleanOp != TR::BadILOp && !isSubtractSpecialCase
    ...
    if (child->hasKnownOrAssumedSignCode()) {
        ...
        case raw_bcd_sign_0xf: {
            ...
            if (isLoadVarChild && parent && parent->getOpCode().isPackedStore() && parent->getValueChild() == node) {
                ...
                // so prevent reversing this optimization here (otherwise both opts keep happening and flipping the IL
                // back and forth)
                if (s->trace())
                    traceMsg(s->comp(),
            ...
            if (removeClean
                ...
                TR::Node* setSignNode = TR::Node::create(
                    TR::pdSetSign, 2, child, TR::Node::iconst(child, TR::DataType::getPreferredPlusCode()));
                ...
                return s->replaceNode(
                    node, setSignNode, s->_curTree, false); // needAnchor=false as child is anchored under setSignNode
            }
            break;
        }
        default:
...
TR::Node* pdclearSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCodeValue() == TR::pdSetSign && firstChild->hasKnownOrAssumedSignCode()
        && performTransformation(s->comp(),
               "%sFold child %s [" POINTER_PRINTF_FORMAT "] into parent %s [" POINTER_PRINTF_FORMAT
               "] with sign 0x%x\n",
...
TR::Node* pdstoreSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if ((!s->comp()->getOption(TR_EnableLateCleanFolding) || s->comp()->getOptLevel() == noOpt)
        && // setting CleanSignInPDStoreEvaluator at opt interferes with copyPropagation and valueNumbering as it means
           // the store now has a side-effect
        !s->comp()->getOption(TR_KeepBCDWidening) && valueChild->getOpCodeValue() == TR::pdclean
        ...
        if (!s->comp()->getOption(TR_EnableLateCleanFolding)
            || s->comp()->getOptLevel()
                == noOpt) // setting CleanSignInPDStoreEvaluator at opt interferes with copyPropagation and
                          // valueNumbering as it means the store now has a side-effect
        {
            dumpOptDetails(s->comp(),
                " by setting CleanSignInPDStoreEvaluator flag on pdstore [" POINTER_PRINTF_FORMAT "]\n", node);
            node->setCleanSignInPDStoreEvaluator(true);
            valueChild = node->setValueChild(
                s->replaceNodeWithChild(valueChild, valueChild->getFirstChild(), s->_curTree, block));
        } else {
            dumpOptDetails(s->comp(),
                " by changing pdSetSign [" POINTER_PRINTF_FORMAT "] to pdclean and removing %s [" POINTER_PRINTF_FORMAT
                "]\n",
    ...
    if (node->getType().isAnyPacked() && node->mustCleanSignInPDStoreEvaluator() && valueChild->getReferenceCount() == 1
        ...
        && performTransformation(s->comp(),
               ...
               s->optDetailString(), valueChild->getFirstChild(), valueChild->getOpCode().getName(), valueChild,
               node)) {
    ...
    if (removeSimpleTruncation
        && performTransformation(s->comp(),
               ...
               "]\n",
               s->optDetailString(), valueChild->getOpCode().getName(), valueChild, node->getOpCode().getName(),
               node)) {
        valueChild = node->setValueChild(s->replaceNodeWithChild(valueChild, valueChild->getFirstChild(), s->_curTree,
    ...
    if (isShiftByZero) {
...

        {
            ...
            return s->simplify(node, block);
                       ...
                       || firstChild->getOpCodeValue() == TR::pdneg || firstChild->getOpCodeValue() == TR::df2pd
                       || firstChild->getOpCodeValue() == TR::dd2pd || firstChild->getOpCodeValue() == TR::de2pd)
            ...
            && performTransformation(s->comp(),
                   ...
                   s->optDetailString(), node->getOpCode().getName(), node, firstChild->getOpCode().getName(),
                   firstChild, node->getDecimalPrecision())) {
            firstChild->setDecimalPrecision(node->getDecimalPrecision());
    ...
    if (firstChild->getOpCodeValue() == TR::pdSetSign && firstChild->hasKnownOrAssumedSignCode()
        ...
        if (TR::DataType::getValue(childSign) == sign
            && performTransformation(s->comp(),
                   ...
                   s->optDetailString(), node->getOpCode().getName(), node, firstChild->getOpCode().getName(),
                   firstChild, firstChild->hasKnownSignCode() ? "known" : "assumed", sign)) {
            firstChild = node->setChild(
                0, s->replaceNodeWithChild(firstChild, firstChild->getFirstChild(), s->_curTree, block));
        }
    }
...
TR::Node* pd2dfSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    bool isAbs = false;
    if (node->getOpCodeValue() == TR::pd2dfAbs || node->getOpCodeValue() == TR::pd2ddAbs
    ...
    // pd2de over dd2pd: can go from dd2de directly if dd2de doesn't truncate due to type change or dd2pd doesn't
    // explicitly truncate
    ...
    else if (node->getOpCodeValue() == TR::pd2dd && child->getOpCodeValue() == TR::df2pd)
        isNonTruncatingConversionPair = true;
    else if (node->getOpCodeValue() == TR::pd2dd && child->getOpCodeValue() == TR::de2pd
    ...
    if (isNonTruncatingConversionPair && !child->isTruncating() && child->getFirstChild()->getType().isDFP()
        && !child->getOpCode().isSetSignOnNode() && // Exclude dd2pdSetSign
        node->getDataType() != child->getFirstChild()->getDataType() && node->getDecimalFraction() == 0
        && performTransformation(s->comp(), "%sFold %s (0x%p) and child %s (0x%p) to ", s->optDetailString(),
               node->getOpCode().getName(), node, child->getOpCode().getName(), child)) {
        TR::Node::recreate(
            node, TR::ILOpCode::getProperConversion(child->getFirstChild()->getDataType(), node->getDataType(), false));
        dumpOptDetails(s->comp(), "%s\n", node->getOpCode().getName());
        TR::Node* newChild = child->getFirstChild();
        if (isAbs) {
    ...
    if (!s->comp()->getOption(TR_DisableZonedToDFPReduction) && s->cg()->supportsZonedDFPConversions()
        && child->getOpCodeValue() == TR::zd2pd && node->getDecimalFraction() == 0) {
        // pd2df/pd2dfAbs
        //    zd2pd
        //       zdX
        // to:
        ...
        TR::ILOpCodes convOp = TR::ILOpCode::getProperConversion(
            child->getFirstChild()->getDataType(), node->getDataType(), false); // isUnsigned=false
        if (isAbs)
            convOp = TR::ILOpCode::zonedToDFPAbsOp(child->getFirstChild()->getDataType(), node->getDataType());

        // do not need an intermediateTruncation check above because replaceNodeWithChild will take care of inserting a
        // modPrec if needed
        if (convOp != TR::BadILOp
            && performTransformation(s->comp(), "%sFold %s (0x%p) and child %s (0x%p) to ", s->optDetailString(),
                   node->getOpCode().getName(), node, child->getOpCode().getName(), child)) {
    ...
    if (!s->comp()->getOption(TR_DisableZonedToDFPReduction) && s->cg()->supportsZonedDFPConversions()
        && child->getOpCodeValue() == TR::pdSetSign && child->isNonNegative()
        && child->getFirstChild()->getOpCodeValue() == TR::zd2pd && node->getDecimalFraction() == 0) {
        // pd2df
        //    pdSetSign >= 0
        //       zd2pd
        //          zdX
        ...
        //    zdX
        //
        // Doesn't matter here if we have a pd2dfAbs or pd2df; the pdSetSign is redundant given the pd2dfAbs, and we'll
        // end up with the same result anyway
        TR::ILOpCodes convOp = TR::ILOpCode::zonedToDFPAbsOp(
            child->getFirstChild()->getFirstChild()->getDataType(), node->getDataType());
        if (convOp != TR::BadILOp && !child->hasIntermediateTruncation()
            && performTransformation(s->comp(), "%sFold %s (0x%p) and child %s (0x%p) to ", s->optDetailString(),
                   node->getOpCode().getName(), node, child->getOpCode().getName(), child)) {
            int32_t childPrecision = child->getDecimalPrecision();
            TR::Node::recreate(
                node, convOp); // pd2df -> zd2dfAbs (momentarily type incorrect but not exposed outside this simplifier)
            node->setFlags(0);
            dumpOptDetails(s->comp(), "%s\n", node->getOpCode().getName());
            // pd2df (zd2dfAbs)
            //    pdSetSign
            ...
            //       zdX
            // correctBCDPrecision=false trunc checks already done and top level prec is being maintained (false is
            // needed so double replaceNodes work)
            node->setChild(0,
                s->replaceNodeWithChild(
                    node->getFirstChild(), node->getFirstChild()->getFirstChild(), s->_curTree, block, false));
            // pd2df (zd2dfAbs)
            ...
            if (childPrecision != node->getFirstChild()->getDecimalPrecision()) {
                ...
                TR::ILOpCodes modifyPrecisionOp
                    = TR::ILOpCode::modifyPrecisionOpCode(node->getFirstChild()->getDataType());
                TR_ASSERT(modifyPrecisionOp != TR::BadILOp, "no bcd modify precision opcode found\n");
    ...
    if (child->getOpCodeValue() == TR::pdSetSign && child->isNonNegative() && !child->hasIntermediateTruncation()
        ...
        //
        TR::ILOpCodes convOp
            = TR::ILOpCode::packedToDFPAbsOp(child->getFirstChild()->getDataType(), node->getDataType());
        if (convOp != TR::BadILOp
            && performTransformation(s->comp(), "%sFold %s (0x%p) and child %s (0x%p) to ", s->optDetailString(),
                   node->getOpCode().getName(), node, child->getOpCode().getName(), child)) {
            int32_t childPrecision = child->getDecimalPrecision();
...
TR::Node* pdnegSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    TR::Node* result = NULL;
    if ((result = s->unaryCancelOutWithChild(
             node, child, s->_curTree, TR::ILOpCode::negateOpCode(node->getDataType()), false))) // anchorChildren=false
        return result;
...
static TR::Node* foldSetSignIntoNode(
    TR::Node* setSign, bool setSignIsTheChild, TR::Node* other, bool removeSetSign, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (removeSetSign && parent->hasIntermediateTruncation()) {
        if (s->trace())
            traceMsg(s->comp(),
                "disallow foldSetSignIntoNode of setSign -- %s (%p) with other node -- %s (%p) because child %p (prec "
                "%d) truncates and parent %p (prec %d) widens\n",
    ...
    if (convertedSetSign != TR::DataType::getInvalidSignCode() && setSign->getReferenceCount() == 1
        ...
        if (newSetSignIndex == 1) // child 2
        {
            if (other->getOpCode().isConversion() && other->getFirstChild()->getType().isUnicodeSeparateSign()
                ...
                TR_ASSERT(
                    false, "unexpected parent/child combination in foldSetSignIntoNode for newSetSignIndex == 1\n");
            }
        } else if (newSetSignIndex == 2) // child 3
        {
            ...
            if (isShiftToShiftFold) {
                ...
                TR_ASSERT(
                    false, "unexpected parent/child combination in foldSetSignIntoNode for newSetSignIndex == 2\n");
            }
            if (newSecondChild) {
                if (paddingAddress)
        ...
        } else if (newSetSignIndex == 3) // child 4
        {
            ...
            if (isShiftToShiftFold) {
                ...
                TR_ASSERT(
                    false, "unexpected parent/child combination in foldSetSignIntoNode for newSetSignIndex == 3\n");
            }
        ...
        if (newNode) {
            ...
            if (removeSetSign) {
                ...
                stopUsingSingleNode(
                    setSign, false, s); // removePadding=false, paddingAddress is still attached to the newNode
    ...
    if (node->getReferenceCount() == 1 && child->getReferenceCount() == 1
        ...
        if (setSignOp != TR::BadILOp
            && performTransformation(s->comp(),
                   ...
                   s->optDetailString(), child->getOpCode().getName(), child, node->getOpCode().getName(), node)) {
            int32_t newSetSignIndex = TR::ILOpCode::getSetSignValueIndex(setSignOp);
            int32_t convertedSetSign = TR::DataType::convertSignEncoding(
                child->getDataType(), node->getDataType(), TR::DataType::getPreferredPlusCode());
            TR::Node* newSetSign = TR::Node::iconst(node, convertedSetSign);
            if (newSetSignIndex == 2) {
                ...
                if (node->getOpCode().isShift()) {
                    ...
                    TR_ASSERT(
                        false, "unexpected parent/child combination in pd2udslSimplifier for newSetSignIndex == 2\n");
                }
                if (newSecondChild) {
...
TR::Node* dfpModifyPrecisionSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCodeValue() == node->getOpCodeValue()
        && performTransformation(s->comp(), "%sFold %s (0x%p) <p=%d> and child %s (0x%p) <p=%d> to %s (0x%p) <p=%d>\n",
               s->optDetailString(), node->getOpCode().getName(), node, node->getDFPPrecision(),
               firstChild->getOpCode().getName(), firstChild, firstChild->getDFPPrecision(),
...
TR::Node* dfpSetSignSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (redundantDescendant != NULL
        && performTransformation(s->comp(), "%sRemove redundant setsign op %s (0x%p) below ancestor %s (0x%p)",
               s->optDetailString(), redundantDescendant->getOpCode().getName(), redundantDescendant,
               node->getOpCode().getName(), node)) {
    ...
    if (!node->isNonNegative() && node->getOpCode().isAbs()
