 ...
 */
TR::TreeTop* J9::TransformUtil::generateRetranslateCallerWithPrepTrees(
    TR::Node* node, TR_PersistentMethodInfo::InfoBits reason, TR::Compilation* comp)
{
    ...
    callNode->setAndIncChild(
        1, TR::Node::createWithSymRef(node, TR::loadaddr, 0, comp->getSymRefTab()->findOrCreateStartPCSymbolRef()));
...
static bool isFinalFieldPointingAtRepresentableNativeStruct(TR::SymbolReference* symRef, TR::Compilation* comp)
{
    ...
    case TR::SymbolReferenceTable::classFromJavaLangClassAsPrimitiveSymbol:
        ...
        // isn't expecting yet
        TR_ASSERT(
            symRef->getSymbol()->isShadow(), "isFinalFieldPointingAtRepresentableNativeStruct expected shadow symbol");
        return true;
    default:
        return false;
    }
...
static bool isJavaField(TR::SymbolReference* symRef, TR::Compilation* comp)
{
    ...
    if (symbol->isShadow()
        && (symRef->getCPIndex() >= 0 ||
               // java/lang/invoke/VarHandle.handleTable can be fabricated by VarHandleTransformer, thus does not have a
               // valid cp index
...
static bool verifyFieldAccess(void* curStruct, TR::SymbolReference* field, TR::Compilation* comp)
{
    ...
    if (isJavaField(field, comp)) {
        ...
        if (isFabricatedVarHandleField(field, comp))
            fieldClass
                = fej9->getSystemClassFromClassName("java/lang/invoke/VarHandle", strlen("java/lang/invoke/VarHandle"));
        else {
            //
 ...
 */
static void* dereferenceStructPointerChain(
    void* baseStruct, TR::Node* baseNode, TR::Node* curNode, TR::Compilation* comp)
{
    if (baseNode == curNode) {
        ...
        if (curStruct) {
            if (verifyFieldAccess((void*)curStruct, symRef, comp)) {
                ...
                traceMsg(
                    comp, "Unable to verify field access to %s on %p!\n", symRef->getName(comp->getDebug()), curNode);
                return NULL;
            }
        } else {
            return NULL;
...
bool J9::TransformUtil::foldFinalFieldsIn(
    TR_OpaqueClassBlock* clazz, char* className, int32_t classNameLength, bool isStatic, TR::Compilation* comp)
{
    ...
    if ((enableJCLFolding || comp->getOption(TR_AggressiveOpts)) && isStatic && comp->fej9()->isClassLibraryClass(clazz)
...
static bool changeIndirectLoadIntoConst(
    TR::Node* node, TR::ILOpCodes opCode, TR::Node** removedChild, TR::Compilation* comp)
{
...
TR::Node* J9::TransformUtil::transformStringIndexOfCall(TR::Compilation* comp, TR::Node* callNode)
{
    TR_J9VMBase* fej9 = comp->fej9();
    ...
    TR::VMAccessCriticalSection transformStringIndexCriticalSection(
    ...
    for (method = it.getCurrent(); method; method = it.getNext()) {
        ...
        if (!strncmp(method->nameChars(), name, strlen(name)) && !strncmp(method->signatureChars(), sig, strlen(sig)))
            break;
    }
    ...
    TR::SymbolReferenceTable* symRefTab = comp->getSymRefTab();
    TR::SymbolReference* newSymRef = symRefTab->findOrCreateMethodSymbol(
        callNode->getSymbolReference()->getOwningMethodIndex(), -1, method, TR::MethodSymbol::Static);
...
TR::Node* J9::TransformUtil::transformIndirectLoad(TR::Compilation* comp, TR::Node* node)
{
    ...
    TR_ASSERT(
        node->getOpCode().isLoadIndirect(), "Expecting indirect load; found %s %p", node->getOpCode().getName(), node);
    TR::SymbolReference* symRef = node->getSymbolReference();
    TR::Symbol* sym = node->getSymbol();

    if (!symRef->isUnresolved() && sym->isShadow()) {
        ...
        case TR::Symbol::Java_lang_invoke_MutableCallSiteDynamicInvokerHandle_mutableSite:
            if (!node->isNonNull()
                && performTransformation(
                       comp, "O^O transformIndirectLoad: [%p] recognized field is never null\n", node)) {
        ...
        default:
            ...
            //
            if (symRef == comp->getSymRefTab()->findJavaLangClassFromClassSymbolRef() && !symRef->hasKnownObjectIndex()
                && baseObject->getOpCodeValue() == TR::loadaddr && !baseObject->getSymbolReference()->isUnresolved()) {
                TR::SymbolReference* improvedSymRef;

                {
                    TR::VMAccessCriticalSection createSymRefWithKnownObject(comp->fej9());
                    uintptrj_t jlClass = (uintptrj_t)J9VM_J9CLASS_TO_HEAPCLASS(
                        (J9Class*)baseObject->getSymbol()->castToStaticSymbol()->getStaticAddress());
                    TR_ASSERT(jlClass, "java/lang/Class reference from heap class must be non null");
                    improvedSymRef
                        = comp->getSymRefTab()->findOrCreateSymRefWithKnownObject(node->getSymbolReference(), &jlClass);
                }

                if (improvedSymRef->hasKnownObjectIndex()
                    && performTransformation(comp,
                           "O^O transformIndirectLoad: [%p] use object-specific symref #%d (=obj%d) for load of "
                           "java/lang/Class\n",
        ...
        {
            J9Class* fieldClass
                = (J9Class*)symRef->getOwningMethod(comp)->getClassFromFieldOrStatic(comp, symRef->getCPIndex());
            if (!fieldClass)
                return NULL;
            ...
            if (sym->getRecognizedField() != TR::Symbol::assertionsDisabled
                && !J9::TransformUtil::foldFinalFieldsIn(
                       (TR_OpaqueClassBlock*)fieldClass, name, len, sym->isStaticField(), comp))
                return NULL;
            ...
            if (isFinalFieldOfNativeStruct(symRef, comp)) {
                ...
                if (typeIsConstible) // can't yet tell whether the expression chain results in a Java object, so just do
                                     // primitives
                {
                    while (baseObject->getOpCode().isLoadIndirect()
                        && isFinalFieldPointingAtUnrepresentableNativeStruct(baseObject->getSymbolReference(), comp)) {
                    ...
                    if (baseObject->getOpCode().isLoadIndirect()) {
                        if (isFinalFieldPointingAtNativeStruct(baseObject->getSymbolReference(), comp)) {
                            TR_ASSERT(
                                isFinalFieldPointingAtRepresentableNativeStruct(baseObject->getSymbolReference(), comp),
                                "Base expression pointing at native struct must be representable"); // Good luck to the
                                                                                                    // poor soul doing
                                                                                                    // triage trying to
                                                                                                    // figure out what
                                                                                                    // this means
                            ...
                            // a const or loadaddr.
                            dumpOptDetails(comp,
                                "Could have transformed %p if representable baseObject %p had already been "
                                "simplified\n",
                                node, baseObject);
                            return NULL;
                        } else {
                        ...
                        dumpOptDetails(comp,
                            "Can't yet transform %p based on representable baseObject %p that isn't a J9Class\n", node,
                            baseObject);
                        return NULL;
                    }
                    ...
                    if (clazz
                        && performTransformation(comp,
                               "O^O transformIndirectLoad: [%p] evaluate native structure walk into const from %s, "
                               "based on class pointer node %p\n",
                        ...
                        //
                        uint8_t* structure
                            = (uint8_t*)dereferenceStructPointerChain(clazz, baseObject, node->getFirstChild(), comp);
                        if (verifyFieldAccess(structure, node->getSymbolReference(), comp))
                            switch (loadType) {
                            case TR::Int32:
                                node = TR::Node::iconst(
                                    node, *(int32_t*)(structure + node->getSymbolReference()->getOffset()));
                                break;
                            case TR::Int64:
                                node = TR::Node::lconst(
                                    node, *(int64_t*)(structure + node->getSymbolReference()->getOffset()));
                                break;
                            default:
            ...
            if (baseObject->getOpCode().hasSymbolReference()
                && baseObject->getSymbolReference()->hasKnownObjectIndex()) {
            ...
            if (isFinalFieldOfNativeStruct(symRef, comp)) {
                if (baseObjectRefLocation && typeIsConstible) {
                    if (performTransformation(comp,
                            "O^O transformIndirectLoad: [%p] evaluate native structure walk into const from %s, based "
                            "on known object node %p\n",
                            node, node->getSymbolReference()->getName(comp->getDebug()), baseObject)) {
                        // We have object.a.b.c.d.primitiveField, where a.b.c.d walks through some native data
                        // structure. We can simplify this into a const.
                        //
...

                        {
                            ...
                            uintptrj_t fieldAddress
                                = baseObjectRef + firstDereference->getSymbolReference()->getOffset();
                        ...
                        TR_ASSERT(secondDereference,
                            "Expected expression %p like a.b...c where a is a Java object, b points at a native "
                            "struct, and c is a field in a native struct",
                            node);
                        if (secondDereference)
                            structure = (uint8_t*)dereferenceStructPointerChain(
                                structure, firstDereference, node->getFirstChild(), comp);
                ...
                && isFinalFieldPointingAtRepresentableNativeStruct(symRef, comp) && !comp->compileRelocatableCode()) {
                ...
                if (performTransformation(comp, "O^O transformIndirectLoad: [%p] turn load of J9Class into %s %p\n",
                    ...
                    return clazzNode;
            } else if (sym->isFinal() && !comp->compileRelocatableCode()) // Constructor can set different values for
                                                                          // the same field in different runs on AOT
            {
                int32_t fieldOffset = symRef->getOffset() - sizeof(J9Object); // blah
                if (typeIsConstible) {
                    if (performTransformation(comp,
                            "O^O transformIndirectLoad: [%p] turn final %s %s into load const\n", node,
...

                    {
                        ...
                        improvedSymRef = comp->getSymRefTab()->findOrCreateSymRefWithKnownObject(
                            node->getSymbolReference(), &targetObjectReference);
                    }

                    if (targetObjectReference) {
                        if (improvedSymRef->hasKnownObjectIndex()
                            && performTransformation(comp,
                                   "O^O transformIndirectLoad: [%p] use object-specific symref #%d (=obj%d) for %s of "
                                   "final field %s\n",
                                   node, improvedSymRef->getReferenceNumber(), improvedSymRef->getKnownObjectIndex(),
                                   node->getOpCode().getName(), symRef->getName(comp->getDebug()))) {
                            node->setSymbolReference(improvedSymRef);
                        ...
                        default:
                            if (performTransformation(comp,
                                    "O^O transformIndirectLoad: [%p] field is null - change to aconst NULL\n", node)) {
                                node->getAndDecChild(0);
                                node->setNumChildren(0);
...
TR_YesNoMaybe J9::TransformUtil::canFoldStaticFinalField(TR::Compilation* comp, TR::Node* node)
{
    ...
    TR_OpaqueClassBlock* declaringClass
        = symRef->getOwningMethod(comp)->getClassFromFieldOrStatic(comp, symRef->getCPIndex(), true);
    ...
    if (!comp->getOption(TR_RestrictStaticFieldFolding) || sym->getRecognizedField() == TR::Symbol::assertionsDisabled
...
bool J9::TransformUtil::foldStaticFinalFieldImpl(TR::Compilation* comp, TR::Node* node)
{
    ...
    if (typeIsConstible) {
        ...
        return true;
    } else if (*(uintptrj_t*)staticSym->getStaticAddress()
        == 0) // Seems ok just to check for a static to be NULL without vm access
    {
        ...
        default:
            if (performTransformation(
                    comp, "O^O transformDirectLoad: [%p] field is null - change to aconst NULL\n", node)) {
                TR::Node::recreate(node, TR::aconst);
                node->setAddress(0);
                node->setIsNull(true);
                node->setIsNonNull(false);
        ...
        TR::SymbolReference* improvedSymRef
            = comp->getSymRefTab()->findOrCreateSymRefWithKnownObject(node->getSymbolReference(), refLocation);
        if (improvedSymRef->hasKnownObjectIndex()
            && performTransformation(comp,
                   "O^O transformDirectLoad: [%p] use object-specific symref #%d (=obj%d) for %s %s\n", node,
                   improvedSymRef->getReferenceNumber(), improvedSymRef->getKnownObjectIndex(),
                   node->getOpCode().getName(), symRef->getName(comp->getDebug()))) {
            node->setSymbolReference(improvedSymRef);
...
{
    TR_ASSERT(
        node->getOpCode().isLoadVarDirect(), "Expecting direct load; found %s %p", node->getOpCode().getName(), node);
    ...
    uintptrj_t* baseReferenceLocation, TR::Node** removedNode)
{
    ...
    bool result = TR::TransformUtil::transformIndirectLoadChainImpl(
        comp, node, baseExpression, (void*)baseAddress, removedNode);
    return result;
}
    ...
    TR::KnownObjectTable::Index baseKnownObject, TR::Node** removedNode)
{
    ...
    bool result = TR::TransformUtil::transformIndirectLoadChainImpl(
        comp, node, baseExpression, (void*)comp->getKnownObjectTable()->getPointer(baseKnownObject), removedNode);
    return result;
}
 ...
 *
 */
bool J9::TransformUtil::transformIndirectLoadChainImpl(
    TR::Compilation* comp, TR::Node* node, TR::Node* baseExpression, void* baseAddress, TR::Node** removedNode)
{
    TR_J9VMBase* fej9 = comp->fej9();
    ...
    case TR::Address: {
        if (isFinalFieldPointingAtRepresentableNativeStruct(symRef, comp)) {
            if (fej9->isFinalFieldPointingAtJ9Class(symRef, comp)) {
                if (changeIndirectLoadIntoConst(node, TR::loadaddr, removedNode, comp)) {
                    ...
                    node->setSymbolReference(
                        comp->getSymRefTab()->findOrCreateClassSymbol(comp->getMethodSymbol(), -1, value));
                } else {
                    return false;
                }
            } else {
            ...
            if (value) {
                TR::SymbolReference* improvedSymRef = comp->getSymRefTab()->findOrCreateSymRefWithKnownObject(
                    symRef, &value, isArrayWithConstantElements(symRef, comp));
                if (improvedSymRef->hasKnownObjectIndex()
                    && performTransformation(comp,
...
TR::Node* J9::TransformUtil::calculateElementAddress(
    TR::Compilation* comp, TR::Node* array, TR::Node* index, TR::DataType type)
{
 ...
 */
TR::Node* J9::TransformUtil::calculateOffsetFromIndexInContiguousArray(
    TR::Compilation* comp, TR::Node* index, TR::DataType type)
{
 ...
 */
TR::Node* J9::TransformUtil::calculateIndexFromOffsetInContiguousArray(
    TR::Compilation* comp, TR::Node* offset, TR::DataType type)
{
    int32_t width = TR::Symbol::convertTypeToSize(type);
    // In compressedrefs mode, each element of an reference array is a compressed pointer,
    // modify the width accordingly, so the stride is 4bytes instead of 8
    ...
    TR::TreeTop* ifTree, TR::TreeTop* elseTree, bool changeBlockExtensions, bool markCold)
{
    ...
    callBlock->createConditionalBlocksBeforeTree(
        callTree, compareTree, ifTree, elseTree, comp->getFlowGraph(), changeBlockExtensions, markCold);
