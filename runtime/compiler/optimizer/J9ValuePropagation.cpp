...
TR::VP_BCDSign** J9::ValuePropagation::getBCDSignConstraints(TR::DataType dt)
{
    ...
    if (_bcdSignConstraints == NULL) {
        ...
        //    traceMsg(comp(),"create _bcdSignConstraints %p of size %d (numBCDTypes %d * TR_Sign_Num_Types %d *
        //    sizeof(TR::VP_BCDSign*) %d)\n",
    ...
    //      dt,_bcdSignConstraints,(dt-TR::FirstBCDType)*TR_Sign_Num_Types,_bcdSignConstraints +
    //      (dt-TR::FirstBCDType)*TR_Sign_Num_Types);
 ...
 */
void J9::ValuePropagation::transformCallToIconstWithHCRGuard(TR::TreeTop* callTree, int32_t result)
{
    ...
    TR_ASSERT(
        callNode->getSymbol()->isResolvedMethod(), "Expecting resolved call in transformCallToIconstWithHCRGuard");
    ...
    TR::Node* compareNode = TR_VirtualGuard::createHCRGuard(
        comp(), calleeIndex, callNode, NULL, calleeSymbol, calleeSymbol->getResolvedMethod()->classOfMethod());
    ...
    J9::TransformUtil::createDiamondForCall(
        this, callTree, compareTree, ifTree, elseTree, false /*changeBlockExtensions*/, true /*markCold*/);
    comp()->decInlineDepth();
}
 ...
 */
uintptrj_t* J9::ValuePropagation::getObjectLocationFromConstraint(TR::VPConstraint* constraint)
{
    ...
    if (constraint->isConstString()) {
        ...
        objectLocation = (uintptrj_t*)constraint->getClassType()
                             ->asConstString()
                             ->getSymRef()
                             ->getSymbol()
                             ->castToStaticSymbol()
 ...
 */
void J9::ValuePropagation::transformCallToIconstInPlaceOrInDelayedTransformations(
    TR::TreeTop* callTree, int32_t result, bool isGlobal, bool inPlace)
{
    TR::Node* callNode = callTree->getNode()->getFirstChild();
...
void J9::ValuePropagation::constrainRecognizedMethod(TR::Node* node)
{
    ...
    case TR::java_lang_Class_isInterface: {
        ...
        if (receiverChildConstraint && receiverChildConstraint->isJavaLangClassObject() == TR_yes
            ...
            transformCallToIconstInPlaceOrInDelayedTransformations(
                _curTree, isInterface, receiverChildGlobal, transformNonnativeMethodInPlace);
            TR::DebugCounter::incStaticDebugCounter(
                comp(), TR::DebugCounter::debugCounterName(comp(), "constrainCall/(%s)", signature));
            return;
        }
        break;
    }
    case TR::java_lang_String_hashCode: {
        ...
        if (isKnownStringObject(receiverChildConstraint)) {
            ...
            TR_ASSERT(
                stringLocation, "Expecting non null pointer to String object for constString or known String object");
            int32_t hashCode;
            ...
            transformCallToIconstInPlaceOrInDelayedTransformations(
                _curTree, hashCode, receiverChildGlobal, transformNonnativeMethodInPlace);
            TR::DebugCounter::incStaticDebugCounter(
                comp(), TR::DebugCounter::debugCounterName(comp(), "constrainCall/(%s)", signature));
            return;
        }
    ...
    case TR::java_lang_String_equals: {
        ...
        if (isStringObject(receiverChildConstraint) == TR_yes && receiverChildConstraint->isNonNullObject()
            ...
            if (objectChildConstraint->isNullObject()) {
                transformCallToIconstInPlaceOrInDelayedTransformations(
                    _curTree, 0, receiverChildGlobal && objectChildGlobal, transformNonnativeMethodInPlace);
                TR::DebugCounter::incStaticDebugCounter(
                    comp(), TR::DebugCounter::debugCounterName(comp(), "constrainCall/(%s)", signature));
                return;
            }
            ...
            } else // isObjectString == TR_yes
            {
                ...
                if (isKnownStringObject(receiverChildConstraint) && isKnownStringObject(objectChildConstraint)) {
                    ...
                    transformCallToIconstInPlaceOrInDelayedTransformations(
                        _curTree, result, receiverChildGlobal && objectChildGlobal, transformNonnativeMethodInPlace);
                    TR::DebugCounter::incStaticDebugCounter(
                        comp(), TR::DebugCounter::debugCounterName(comp(), "constrainCall/(%s)", signature));
                    return;
                }
    ...
    case TR::java_lang_String_lengthInternal: {
        ...
        if (isKnownStringObject(receiverChildConstraint)) {
            ...
            int32_t len;
            {
                TR::VMAccessCriticalSection getStringlength(comp(), TR::VMAccessCriticalSection::tryToAcquireVMAccess);
    ...
    case TR::java_lang_Class_getModifiersImpl: {
        ...
        if (classChildConstraint && classChildConstraint->isJavaLangClassObject() == TR_yes
            ...
            bool success
                = comp()->fej9()->javaLangClassGetModifiersImpl(classChildConstraint->getClass(), modifiersForClass);
            if (!success)
                break;
    ...
    case TR::java_lang_Class_isAssignableFrom: {
        ...
        if (firstClassChildConstraint && firstClassChildConstraint->isJavaLangClassObject() == TR_yes
            ...
            if (secondClassChildConstraint && secondClassChildConstraint->isJavaLangClassObject() == TR_yes
                ...
                transformCallToIconstInPlaceOrInDelayedTransformations(
                    _curTree, assignable, firstClassChildGlobal && secondClassChildGlobal, true);
                TR::DebugCounter::incStaticDebugCounter(
                    comp(), TR::DebugCounter::debugCounterName(comp(), "constrainCall/(%s)", signature));
                return;
            }
    ...
    case TR::java_lang_J9VMInternals_fastIdentityHashCode: {
        // Get the last child because sometimes the first child can be a pointer to java/lang/Class field in J9Class for
        ...
        if (classChildConstraint && classChildConstraint->isJavaLangClassObject() == TR_yes
            ...
            int32_t hashCodeForClass = comp()->fej9()->getJavaLangClassHashCode(
                comp(), classChildConstraint->getClass(), hashCodeWasComputed);
            if (hashCodeWasComputed) {
                transformCallToIconstInPlaceOrInDelayedTransformations(
                    _curTree, hashCodeForClass, classChildGlobal, true);
                TR::DebugCounter::incStaticDebugCounter(
                    comp(), TR::DebugCounter::debugCounterName(comp(), "constrainCall/(%s)", signature));
                return;
            }
        ...
        if (classChildConstraint && classChildConstraint->isJavaLangClassObject() == TR_yes
            ...
            if (comp()->fej9()->isClassArray(thisClass)) {
                ...
                if (!performTransformation(
                        comp(), "%sTransforming %s on node %p to an aloadi\n", OPT_DETAILS, signature, node))
                    break;
                ...
                if (knot) {
                    TR::KnownObjectTable::Index knownObjectIndex = knot->getIndexAt(
                        (uintptrj_t*)(arrayComponentClass + comp()->fej9()->getOffsetOfJavaLangClassFromClassField()));
                ...
                if (trace())
                    traceMsg(
                        comp(), "Class is not an array class, transforming %s on node %p to null\n", signature, node);
            ...
            if (!superClass || TR::Compiler->cls.isInterfaceClass(comp(), thisClass)
                ...
                if (trace()) {
                    if (TR::Compiler->cls.isInterfaceClass(comp(), thisClass))
                        ...
                        traceMsg(
                            comp(), "Node %p is java/lang/Object class and does not have a super class\n", classChild);
                ...
                if (!performTransformation(
                        comp(), "%sTransforming %s on node %p into an aloadi\n", OPT_DETAILS, signature, node))
                    break;
                ...
                if (knot) {
                    TR::KnownObjectTable::Index knownObjectIndex = knot->getIndexAt(
                        (uintptrj_t*)(superClass + comp()->fej9()->getOffsetOfJavaLangClassFromClassField()));
                    addBlockOrGlobalConstraint(node,
                        TR::VPClass::create(this, TR::VPKnownObject::createForJavaLangClass(this, knownObjectIndex),
                            TR::VPNonNullObject::create(this), NULL, NULL,
                            TR::VPObjectLocation::create(this, TR::VPObjectLocation::JavaLangClassObject)),
    ...
    if (!comp()->compileRelocatableCode()) {
        ...
        case TR::java_lang_invoke_PrimitiveHandle_initializeClassIfRequired: {
            ...
            if (mhConstraint && mhConstraint->getKnownObject() && mhConstraint->isNonNullObject()
                ...
                TR::VMAccessCriticalSection initializeClassIfRequired(
        ...
        case TR::java_lang_invoke_DirectHandle_nullCheckIfRequired: {
            ...
            if (receiverConstraint && receiverConstraint->isNonNullObject()) {
                ...
                TR::VMAccessCriticalSection nullCheckIfRequired(
                    comp(), TR::VMAccessCriticalSection::tryToAcquireVMAccess);
        ...
        case TR::java_lang_Object_newInstancePrototype: {
            ...
            if (!callerClassChildConstraint || !callerClassChildConstraint->isNonNullObject()
                ...
                if (trace())
                    traceMsg(comp(),
                        "Caller class %p is not a non-null java/lang/Class object with fixed class constraint, quit "
                        "transforming Object.newInstancePrototype on node %p\n",
            ...
            if (comp()->fej9()->isAbstractClass(newClass) || comp()->fej9()->isPrimitiveClass(newClass)
                || comp()->fej9()->isInterfaceClass(newClass) || comp()->fej9()->isClassArray(newClass)) {
                if (trace())
                    traceMsg(comp(),
            ...
            if (!comp()->fej9()->isClassVisible(callerClass, newClass)) {
                if (trace())
                    traceMsg(comp(),
                        "Class is not visialbe to caller class, quit transforming Object.newInstancePrototype on node "
                        "%p\n",
                        node);
                break;
            }
            ...
            TR_OpaqueMethodBlock* constructor
                = comp()->fej9()->getMethodFromClass(newClass, "<init>", "()V", callerClass);

            if (!constructor) {
                if (trace())
                    traceMsg(comp(),
                        "Cannot find the default constructor, quit transforming Object.newInstancePrototype on node "
                        "%p\n",
                        node);
                break;
            }
            ...
            if (performTransformation(comp(), "%sTransforming %s to new and a call to constructor on node %p\n",
                ...
                TR::TreeTop* constructorCall = TR::TreeTop::create(comp(), _curTree,
                    TR::Node::create(
                        node, TR::treetop, 1, TR::Node::createWithSymRef(node, TR::call, 1, node, constructorSymRef)));
...
void J9::ValuePropagation::doDelayedTransformations()
{
    ...
    for (TreeIntResultPair* it = callsToBeFoldedToIconst.getFirst(); it; it = callsToBeFoldedToIconst.getNext()) {
        ...
        if (!performTransformation(comp(), "%sTransforming call %s on node %p on tree %p to iconst %d\n", OPT_DETAILS,
                signature, callNode, callTree, result))
            break;
