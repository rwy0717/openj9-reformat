...
public:
    EdgeFrequencyInfo(
        TR::Compilation* comp, TR::CFGEdge* loopBack, int32_t numEdges, TR::Region& region, bool trace = false)
        : comp(comp)
        , loopBack(loopBack)
        , region(region)
        , trace(trace)
    ...
    bool computeBlockFrequencyFromPredecessors(
        TR_BitVector** blockCounters, TR_BlockFrequencyInfo* bfi, TR::Block* block)
    {
        ...
        /*else
           ...
           predItr.getCurrent()->getFrom()->asBlock()->getNumber());
           }*/
        return false;
    }
    ...
    bool computeBlockFrequencyFromSuccessors(TR_BitVector** blockCounters, TR_BlockFrequencyInfo* bfi, TR::Block* block)
    {
        ...
        /*else
           ...
           succItr.getCurrent()->getTo()->asBlock()->getNumber());
           }*/
...
private:
        ...
        TR_BitVector& additive, TR_BitVector& subtractive)
    {
        ...
        TR::DebugCounter::incStaticDebugCounter(
            comp, TR::DebugCounter::debugCounterName(comp, "jprofiling.counterSize/additive/%d", additiveElementCount));
        ...
        if (subtractiveElementCount > 0) {
            TR::DebugCounter::incStaticDebugCounter(comp,
                TR::DebugCounter::debugCounterName(
                    comp, "jprofiling.counterSize/subtractive/%d", subtractiveElementCount));
 ...
 */
void TR_JProfilingBlock::computeMinimumSpanningTree(
    BlockParents& parent, BlockPriorityQueue& Q, TR::StackMemoryRegion& stackMemoryRegion)
{
 ...
 *               and enter blocks
 */
int32_t TR_JProfilingBlock::processCFGForCounting(
    BlockParents& parent, TR::BlockChecklist& countedBlocks, TR::CFGEdge& loopBack)
{
    TR::CFG* cfg = comp()->getFlowGraph();
    ...
    for (CFGNodeIterator iter(cfg, this); iter.currentBlock() != NULL; ++iter) {
        ...
        for (TR::CFGEdge* edge = sit.getFirst(); edge; edge = sit.getNext()) {
            ...
            else if (block != to && (to->getPredecessors().size() == 1 && to->getExceptionPredecessors().size() == 0)
    ...
    for (CFGNodeIterator iter(cfg, this); iter.currentBlock() != NULL; ++iter) {
        ...
        if (cfg->getStart() == block || cfg->getEnd() == block)
            ...
            traceMsg(comp(), "block_%d [%d,%d]:\n", block->getNumber(),
                block->getEntry()->getNode()->getInlinedSiteIndex(), block->getEntry()->getNode()->getByteCodeIndex());
...
void TR_JProfilingBlock::addRecompilationTests(TR_BlockFrequencyInfo* blockFrequencyInfo)
{
    ...
    if (root != NULL) {
        ...
        TR::Block* guardBlock1 = TR::Block::createEmptyBlock(node, comp(), originalFirstBlock->getFrequency());
        {
            ...
            TR::Node* enableTest = TR::Node::createif(
                TR::ificmpeq, enableLoad, TR::Node::iconst(node, -1), originalFirstBlock->getEntry());
        ...
        TR::TreeTop* callTree = TR::TransformUtil::generateRetranslateCallerWithPrepTrees(
            node, TR_PersistentMethodInfo::RecompDueToJProfiling, comp());
...
int32_t TR_JProfilingBlock::perform()
{
    ...
    TR::DebugCounter::incStaticDebugCounter(
        comp(), TR::DebugCounter::debugCounterName(comp(), "jprofiling.instrument/success/(%s)", comp()->signature()));
    ...
    for (CFGNodeIterator iter(cfg, this); iter.currentBlock() != NULL; ++iter) {
        ...
        if (!countedBlocks.contains(block)) {
            TR::DebugCounter::incStaticDebugCounter(comp(),
                TR::DebugCounter::debugCounterName(
                    comp(), "jprofiling/uncounted/%s", comp()->getHotnessName(comp()->getMethodHotness())));
            continue;
        }
        ...
        TR::DebugCounter::incStaticDebugCounter(comp(),
            TR::DebugCounter::debugCounterName(
                comp(), "jprofiling/globalCounted/%s", comp()->getHotnessName(comp()->getMethodHotness())));
