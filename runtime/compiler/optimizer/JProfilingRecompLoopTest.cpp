 ...
 */
bool TR_JProfilingRecompLoopTest::isByteCodeInfoInCurrentTestLocationList(
    TR_ByteCodeInfo& bci, TR::list<TR_ByteCodeInfo, TR::Region&>& addedLocationBCIList)
{
...
int32_t TR_JProfilingRecompLoopTest::perform()
{
    if (comp()->getProfilingMode() != JProfiling) {
        if (trace())
            traceMsg(
                comp(), "JProfiling for profiling compilations has not been enabled, skip JProfilingRecompLoopTest\n");
        return 0;
    }
    ...
    while (cursor) {
        if (cursor->getNode()->getOpCodeValue() == TR::BBStart) {
             ...
             * which is not extenstion of previous block, we clear the list of byte code info.
             */
            ...
            TR_ASSERT_FATAL(
                currentBlock != NULL, "We should have encountered BBStart before and should have the enclosing block");
...
void TR_JProfilingRecompLoopTest::addRecompilationTests(
    TR::Compilation* comp, RecompilationTestLocationsInfo& testLocations)
{
    ...
    for (auto testLocationIter = testLocations.rbegin(), testLocationEnd = testLocations.rend();
        ...
        dumpOptDetails(
            comp, "%s Add recompilation test after asyncCheck node n%dn\n", optDetailString(), node->getGlobalIndex());
        ...
        // To safeguard this scenario, we also check if threshold is negative or zero we set this thrshold to
        ...
        remainingCodeBlock->getEntry()->insertTreeTopsBeforeMe(
            callRecompileBlock->getEntry(), callRecompileBlock->getExit());
        ...
        if (generateJProfilingRecompQueueTest) {
            ...
            TR::Node* checkIfQueueForRecompilation = TR::Node::createif(
                TR::ificmpeq, loadIsQueuedForRecompilation, TR::Node::iconst(node, -1), remainingCodeBlock->getEntry());
            ...
            callRecompileBlock->getEntry()->insertTreeTopsBeforeMe(
                calculateLoopRawFreq->getEntry(), calculateLoopRawFreq->getExit());
