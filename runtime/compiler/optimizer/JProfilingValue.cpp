...
void TR_JProfilingValue::removeRedundantProfilingValueCalls()
{
    ...
    while (cursor) {
        ...
        // low.
        else if (cursor->getNode()->getOpCodeValue() == TR::BBStart
...
void TR_JProfilingValue::performOnNode(TR::Node* node, TR::TreeTop* tt, TR::NodeChecklist* checklist)
{
...
void TR_JProfilingValue::addProfiling(TR::Node* value, TR::TreeTop* tt)
{
    ...
    TR_AbstractHashTableProfilerInfo* info = static_cast<TR_AbstractHashTableProfilerInfo*>(
        valueProfileInfo->getOrCreateProfilerInfo(value->getByteCodeInfo(), comp(), AddressInfo, HashTableProfiler));
    addProfilingTrees(comp(), tt, value, info, NULL, true, trace());
}

void TR_JProfilingValue::addVFTProfiling(TR::Node* address, TR::TreeTop* tt, bool addNullCheck)
{
    ...
    TR::Node* vftNode
        = TR::Node::createWithSymRef(address, TR::aloadi, 1, address, getSymRefTab()->findOrCreateVftSymbolRef());
    ...
    TR_ValueProfileInfo* valueProfileInfo
        = TR_PersistentProfileInfo::getCurrent(comp())->findOrCreateValueProfileInfo(comp());
    TR_AbstractHashTableProfilerInfo* info = static_cast<TR_AbstractHashTableProfilerInfo*>(
        valueProfileInfo->getOrCreateProfilerInfo(address->getByteCodeInfo(), comp(), AddressInfo, HashTableProfiler));
...
bool TR_JProfilingValue::addProfilingTrees(TR::Compilation* comp, TR::TreeTop* insertionPoint, TR::Node* value,
    TR_AbstractHashTableProfilerInfo* table, TR::Node* optionalTest, bool extendBlocks, bool trace)
{
    ...
    if (trace) {
        traceMsg(comp, "Inserting value profiling trees:\n  Value n%dn At n%dn\n  Table: %p\n", value->getGlobalIndex(),
            insertionPoint->getNode()->getGlobalIndex(), table);
    ...
    if (optionalTest) {
        ...
        // Usually splitting of block takes care of uncommoning but in cases when we have a first reference of these
        // objects in the TreeTop before we are adding the profiling trees (checkCast, instanceOf) as we dereference the
        // object before checkCast or instanceOf nodes, we might skip generating NULL test in later nodes. To avoid
        // that, we should store the object as well and replace subsequent references to it.
        TR::TreeTop* storeValue
            = TR::TreeTop::create(comp, insertionPoint, storeNode(comp, value->getFirstChild(), storedObjectSymRef));
        replaceNode(comp, extendedBlock->getEntry(), storeValue->getNextTreeTop(), value->getFirstChild(),
            TR::Node::createLoad(value->getFirstChild(), storedObjectSymRef));
        ...
        TR::TreeTop* storeProfilingValue
            = TR::TreeTop::create(comp, quickTest->getEntry(), storeNode(comp, value, storedValueSymRef));
        ...
        replaceNode(comp, extendedBlock->getEntry(), storeValue->getNextTreeTop(), value,
    ...
    TR::Node* ternarySelectNode
        = TR::Node::create(comp->il.opCodeForTernarySelect(systemType), 3, conditionNode, hashIndex, otherIndex);
    TR::TreeTop* incIndexTreeTop
        = TR::TreeTop::create(comp, quickTestLastProfilingTT, TR::Node::create(TR::treetop, 1, ternarySelectNode));
    ...
    TR::Node* checkIfTableIsLockedNode = TR::Node::create(
        value, comp->il.opCodeForCompareGreaterOrEquals(lockType), 2, lock, TR::Node::sconst(value, 0));
    TR::Node* checkNode = TR::Node::createif(comp->il.opCodeForIfCompareEquals(systemType),
        TR::Node::create(value, TR::ILOpCode::orOpCode(systemType), 2, conditionNode,
            convertType(checkIfTableIsLockedNode, systemType)),
    ...
    if (comp->isProfilingCompilation() && !disableJProfilingRecompQueueTest) {
        ...
        TR::Node* checkIfQueueForRecompilation = TR::Node::createif(
            TR::ificmpeq, loadIsQueuedForRecompilation, TR::Node::iconst(value, -1), mainlineReturn->getEntry());
    ...
    TR::TreeTop* helperCallTreeTop = TR::TreeTop::create(comp, helper->getEntry(),
        createHelperCall(comp, convertType(TR::Node::createLoad(value, storedValueSymRef), roundedType),
 ...
 */
void TR_JProfilingValue::replaceNode(
    TR::Node* check, TR::Node* origNode, TR::Node* newNode, TR::NodeChecklist& checklist)
{
 ...
 */
void TR_JProfilingValue::replaceNode(
    TR::Compilation* comp, TR::TreeTop* blockStart, TR::TreeTop* replaceStart, TR::Node* origNode, TR::Node* newNode)
{
    TR::NodeChecklist checklist(comp);
...
TR::Node* TR_JProfilingValue::effectiveAddress(TR::DataType dataType, TR::Node* base, TR::Node* index, TR::Node* offset)
{
    ...
    if (index) {
        ...
        if (index->getDataType() == TR::Int64)
            base = TR::Node::create(
                base, TR::aladd, 2, base, TR::Node::create(base, TR::lmul, 2, index, TR::Node::lconst(base, size)));
        else if (index->getDataType() == TR::Int32)
            base = TR::Node::create(
                base, TR::aiadd, 2, base, TR::Node::create(base, TR::imul, 2, index, TR::Node::iconst(base, size)));
        else
            TR_ASSERT_FATAL(0, "Invalid type for address calculation integer");
    }
 ...
 * \param offset Optional offset from the base.
 */
TR::Node* TR_JProfilingValue::loadValue(
    TR::Compilation* comp, TR::DataType dataType, TR::Node* base, TR::Node* index, TR::Node* offset)
{
 ...
 */
TR::Node* TR_JProfilingValue::systemConst(TR::Node* example, uint64_t value)
{
...
TR::Node* TR_JProfilingValue::convertType(TR::Node* index, TR::DataType dataType, bool zeroExtend)
{
    ...
    return TR::Node::create(
        index, TR::ILOpCode::getProperConversion(index->getDataType(), dataType, zeroExtend), 1, index);
}
 ...
 */
TR::Node* TR_JProfilingValue::computeHash(
    TR::Compilation* comp, TR_AbstractHashTableProfilerInfo* table, TR::Node* value, TR::Node* baseAddr)
{
    ...
    if (table->getHashType() == BitIndexHash) {
        ...
        TR::Node* hashAddr = TR::Node::create(
            value, addSys, 2, baseAddr, TR::Node::create(value, constSys, 0, table->getHashOffset()));
