...
int32_t TR_JitProfiler::performOnNode(TR::Node* node, TR::TreeTop* tt)
{
    ...
    if (node->getOpCode().isCall()
        ...
        changesPerformed++;
                   ...
                   instanceof || comp()->getOptions()->getSamplingJProfilingOption(TR_SamplingJProfilingCheckCast)
...
TR::Block* TR_JitProfiler::appendBranchTree(TR::Node* profilingNode, TR::Block* currentBlock)
{
    ...
    TR::Node* flagNode = TR::Node::createWithSymRef(
        profilingNode, TR::iload, 0, comp()->getSymRefTab()->findOrCreateThreadDebugEventData(4));
...
TR::Block* TR_JitProfiler::createProfilingBlocks(
    TR::Node* profilingNode, TR::Block* ifBlock, int32_t bufferSizeRequired)
{
    ...
    TR::Node* cursorNode = TR::Node::createWithSymRef(
        profilingNode, TR::aload, 0, getSymRefTab()->findOrCreateProfilingBufferCursorSymbolRef());
    ...
    TR::Node* endNode = TR::Node::createWithSymRef(
        profilingNode, TR::aload, 0, getSymRefTab()->findOrCreateProfilingBufferEndSymbolRef());
    ...
    TR::Node* vmThread = TR::Node::createWithSymRef(profilingNode, TR::loadaddr, 0,
        new (trHeapMemory()) TR::SymbolReference(
            getSymRefTab(), TR::RegisterMappedSymbol::createMethodMetaDataSymbol(trHeapMemory(), "vmThread")));
...
void TR_JitProfiler::addBranchProfiling(
    TR::Node* branchNode, TR::TreeTop* tt, TR::Block* currentBlock, ProfilingBlocksBypassBranchFlag addBypassBranch)
{
    TR_ASSERT(
        branchNode->getOpCode().isIf(), "Attempting to add branch profiling trees to something other than a branch.\n");
    if (!performTransformation(
            comp(), "%sAdding profiling trees for conditional branch node [%p]\n", optDetailString(), branchNode))
        return;
    ...
    ProfileBlockCreator falseCreator(
        this, blockPair.falseBlock, fallThrough, branchNode, TR::Compiler->om.sizeofReferenceAddress());
...
void TR_JitProfiler::addInstanceProfiling(
    TR::Node* instanceNode, TR::TreeTop* tt, TR::Block* currentBlock, ProfilingBlocksBypassBranchFlag addBypassBranch)
{
    ...
    if (!performTransformation(
            comp(), "%sAdding profiling trees for instanceof/checkcast node [%p]\n", optDetailString(), instanceNode))
        return;
    ...
    ProfileBlockCreator trueCreator(
        this, blockPair.trueBlock, nextBlock, instanceNode, TR::Compiler->om.sizeofReferenceAddress());
    ...
    ProfileBlockCreator falseCreator(
        this, blockPair.falseBlock, nextBlock, instanceNode, TR::Compiler->om.sizeofReferenceAddress());
...
void TR_JitProfiler::addCallProfiling(
    TR::Node* callNode, TR::TreeTop* tt, TR::Block* currentBlock, ProfilingBlocksBypassBranchFlag addBypassBranch)
{
    ...
    TR::Block* successorBlock, TR::Node* profilingNode, int32_t currentOffset)
{
    ...
    _cursor = TR::Node::createWithSymRef(
        profilingNode, TR::aload, 0, _jitProfiler->getSymRefTab()->findOrCreateProfilingBufferCursorSymbolRef());
    ...
    if (!_createdConditional) {
        if (_currentOffset != 0) {
            ...
            TR::Node* storeNode = TR::Node::createWithSymRef(
                TR::astore, 1, 1, addNode, _jitProfiler->getSymRefTab()->findOrCreateProfilingBufferCursorSymbolRef());
...
void TR_JitProfiler::ProfileBlockCreator::addProfilingTree(
    TR::ILOpCodes storeType, TR::Node* storeValue, int32_t storeWidth)
{
    ...
    TR::Node* storeNode = TR::Node::createWithSymRef(
        storeType, 2, 2, storeOffset, storeValue, _jitProfiler->getSymRefTab()->findOrCreateProfilingBufferSymbolRef());
    _profilingBlock->append(TR::TreeTop::create(_jitProfiler->comp(), storeNode));

    return;
}

TR_JitProfiler::ProfileBlockPair TR_JitProfiler::ProfileBlockCreator::addConditionTree(
    TR::ILOpCodes conditionType, TR::Node* firstChild, TR::Node* secondChild)
{
