...
private:
    ...
    // 3) No contention (ie single thread using a lock, or there's only one thread ever trying to aquire the lock) is
...
int32_t TR::MonitorElimination::perform()
{
...

    {
        ...
        if (comp()->getOSRMode() == TR::voluntaryOSR && comp()->getOption(TR_DisableLiveMonitorMetadata)) {
            if (trace())
                traceMsg(comp(),
                    "Cannot remove redundant monitors: voluntary OSR requires the monitor object stores from live "
                    "monitor metadata\n");
            attemptRedundantMonitors = false;
        }
            ...
            if (node->getOpCode().isStoreDirect() && node->getOpCode().hasSymbolReference()
                ...
                if (node->getSymbolReference()->holdsMonitoredObjectForSyncMethod()) {
                    ...
                    if (monitor->getMonitorNode()->getNumChildren() > 0)
                        TR_ASSERT(optimizer()->areNodesEquivalent(
                                      node->getFirstChild(), monitor->getMonitorNode()->getFirstChild()),
            ...
            if (node->getOpCodeValue() == TR::monent) {
                ...
                if (node->isSyncMethodMonitor() && syncMethodTemp) {
                    if (node->getNumChildren() > 0)
                        TR_ASSERT(optimizer()->areNodesEquivalent(
                                      syncMethodTemp->getNode()->getFirstChild(), node->getFirstChild()),
...
bool TR::MonitorElimination::addExceptionPaths(
    TR_ActiveMonitor* monitor, TR::CFGEdgeList& paths, uint32_t exceptionsRaised)
{
    for (auto edge = paths.begin(); edge != paths.end(); ++edge) {
        TR::Block* block = toBlock((*edge)->getTo());
        if (block->getEntry()) {
...
bool TR::MonitorElimination::evaluateMonitorsForTMCandidates()
{
    ...
    debugTrace(
        tracer(), "TM:In evaluateMonitorsForTMCandidates.  Number of monitors to consider = %d\n", _monitors.getSize());
...
TR_Array<TR::Block*>* TR::MonitorElimination::createFailHandlerBlocks(
    TR_ActiveMonitor* monitor, TR::SymbolReference* tempSymRef, TR::Block* monitorblock, TR::Block* tstartblock)
{
    ...
    TR::Node* resetTempNode = TR::Node::createWithSymRef(
        TR::istore, 1, 1, TR::Node::create(monitor->getMonitorNode(), TR::iconst, 0, 0), tempSymRef);
    ...
    // shoudl allow induction variable analysis to exclude the setting of the temp=0 from the loop, and make it a
    // counted loop.

    persistfhBlock->append(TR::TreeTop::create(
        comp(), TR::Node::create(monitor->getMonitorNode(), TR::Goto, 0, monitorblock->getEntry())));
...
bool TR::MonitorElimination::searchDownForOtherMonitorsInCurrentBlock(
    TR::TreeTop* tt, int32_t& size, TR::TreeTop*& monitorTT)
{
...
bool TR::MonitorElimination::searchDownForOtherMonitorsInSuccessors(
    TR::TreeTop* tt, int32_t& size, TR::list<TR::TreeTop*>& closeMonitors, int32_t maxDepth, int32_t minNumberOfNodes)
{
...
void TR::MonitorElimination::searchAndLabelNearbyMonitors(TR_ActiveMonitor* currentMonitor)
{
    ...
    for (et = treeIT.getCurrent(); et; et = treeIT.getNext()) {
        ...
        if (searchDownForOtherMonitorsInCurrentBlock(
                et, numberOfTTs, monitorTT)) // found the closest monent in the current block
        {
            if (numberOfTTs < minNumberOfNodes) {
...
void TR::MonitorElimination::transformMonitorsIntoTMRegions()
{
    ...
    for (monitor = it.getFirst(); monitor; monitor = it.getNext()) {
        ...
        if (!monitor->isTMCandidate()) {
            debugTrace(
                tracer(), "TM:monitor %p at node %p is NOT a TM Candidate\n", monitor, monitor->getTreeTopNode());
            continue;
        }
        ...
        tophalfmonitorblock->append(TR::TreeTop::create(
            comp(), TR::Node::create(monitor->getMonitorNode(), TR::Goto, 0, tstartblock->getEntry())));
        ...
        TR::TreeTop* monitorgoto = TR::TreeTop::create(
            comp(), TR::Node::create(monitor->getMonitorNode(), TR::Goto, 0, critSectStart->getEntry()));
        ...
        TR::Node* tstartNode = TR::Node::createWithRoomForFive(
            TR::tstart, persistentFailNode, transientFailNode, fallThroughNode, monitorObject);
        ...
        //      traceMsg(comp(), "\nTM: \n\tadded edge between fhBlock %d(%p) and monenter
        //      block_%d(%p)",fhBlock->getNumber(),fhBlock,monitorblock->getNumber(),monitorblock);
        ...
        for (TR::TreeTop* exitTree = exitTrees.getFirst(); exitTree; exitTree = exitTrees.getNext()) {
            ...
            if (!clonedExit) {
                ...
                monexitblock->append(TR::TreeTop::create(
                    comp(), TR::Node::create(exitTree->getNode(), TR::Goto, 0, remainderBlock->getEntry())));
            ...
            if (preservesReadRegion(lastMonitorNode, mergeBlock1, &monexitNode)
                ...
                if (canBeAdjusted && lastMonitorNode && monexitNode) {
                    ...
                    for (TR::Block* nextBlock = interIt.getFirst(); nextBlock; nextBlock = interIt.getNext()) {
                        ...
                        TR::Block* clonedBlockWithIf = adjustBlockToCreateReadMonitor(
                            lastMonitor, lastMonitorNode, lastMonitorTreeNum, lastMonitorBlock, nextBlock, block);
                        ...
                        if (targetBlock != mergeBlock1) {
                            ...
                            if (nextBlock && (nextBlock != mergeBlock1) && targetBlock->hasSuccessor(nextBlock)) {
                                ...
                                TR::Block* gotoBlock = TR::Block::createEmptyBlock(
                                    targetBlock->getEntry()->getNode(), comp(), e->getFrequency(), targetBlock);
                                TR::TreeTop* gotoTreeTop = TR::TreeTop::create(comp(),
                                    TR::Node::create(
                                        targetBlock->getEntry()->getNode(), TR::Goto, 0, nextBlock->getEntry()));
                                gotoBlock->append(gotoTreeTop);
                                lastTree->join(gotoBlock->getEntry());
                                gotoBlock->getExit()->setNextTreeTop(NULL);
                                clonedTargetBlock->getLastRealTreeTop()->adjustBranchOrSwitchTreeTop(
                                    comp(), nextBlock->getEntry(), gotoBlock->getEntry());
                        ...
                        if (nextBlock && mergeBlock1->hasSuccessor(nextBlock)) {
                            ...
                            gotoBlock = TR::Block::createEmptyBlock(
                                mergeBlock1->getEntry()->getNode(), comp(), e->getFrequency(), mergeBlock1);
                            TR::TreeTop* gotoTreeTop = TR::TreeTop::create(comp(),
                                TR::Node::create(
                                    mergeBlock1->getEntry()->getNode(), TR::Goto, 0, nextBlock->getEntry()));
                            gotoBlock->append(gotoTreeTop);
                            lastTree->join(gotoBlock->getEntry());
                            gotoBlock->getExit()->setNextTreeTop(NULL);
                            clonedMergeBlock1->getLastRealTreeTop()->adjustBranchOrSwitchTreeTop(
                                comp(), nextBlock->getEntry(), gotoBlock->getEntry());
                            lastTree = gotoBlock->getExit();
                            TR::CFGEdge* newEdge = NULL;
                            cfg->addNode(gotoBlock);
    ...
    int32_t lastMonitorTreeNum, TR::Block* lastMonitorBlock, TR::Block* blockToBeAdjusted, TR::Block* blockWithIf)
{
    ...
    cfg->addEdge(
        clonedBlockWithIf, blockWithIf->getLastRealTreeTop()->getNode()->getBranchDestination()->getNode()->getBlock());
...
void TR::MonitorElimination::removeMonitorNode(TR::Node* node)
{
    ...
    if (node->getOpCodeValue() == TR::NULLCHK) {
        ...
        TR_ASSERT(
            node->getOpCodeValue() == TR::monent || node->getOpCodeValue() == TR::monexit, "Expected monitor node");
...
bool TR::MonitorElimination::markBlocksAtSameNestingLevel(
    TR_Structure* structure, TR_BitVector* blocksAtCurrentNestingLevel)
{
...
void TR::MonitorElimination::collectPredsAndSuccs(TR::CFGNode* startNode, TR_BitVector* visitedNodes,
    ...
    TR_BitVector* loopEntryBlocks, TR::Compilation* comp)
{
                    ...
                    if (!(loopEntryBlocks->get(node->getNumber()) && cfgBackEdges->find(edge))) {
                        ...
                        *subtraction -= *predecessorInfo[node->getNumber()];
                        ...
                        if (!loopEntryBlocks->get(node->getNumber())
                            ...
                            TR_ASSERT(
                                0, "CFG node cannot have itself as a predecessor in a CFG walk ignoring back edges\n");
                        }
                    } else {
...
void TR::MonitorElimination::coarsenMonitorRanges()
{
    ...
    for (; treeTop; treeTop = treeTop->getNextTreeTop()) {
        ...
        if (node->getOpCode().isStore() || node->mightHaveVolatileSymbolReference()) {
    ...
    collectPredsAndSuccs(
        cfg->getStart(), visitedNodes, _predecessorInfo, _successorInfo, &_cfgBackEdges, _loopEntryBlocks, comp());
            ...
            *_inclusiveIntersection |= *_matchingMonentBlocks;
            ...
            while (listIt.hasMoreElements() && nodesInList) {
                ...
                if (!nodesInList) {
                    ...
                    if (nodesInList) {
                        nodesInList = checkIfSuccsInList(
                            block->getExceptionSuccessors(), _inclusiveIntersection, controlFlowInCatch, block);
                    }
                }
                ...
                *_scratch &= *_containsExceptions;
                ...
                while (succs.hasMoreElements()) {
                    ...
                    if (performTransformation(
                            comp(), "%s Success: Coarsening monexit in block_%d\n", OPT_DETAILS, nextSucc)) {
                        _invalidateUseDefInfo = true;
                        _invalidateValueNumberInfo = true;
                        block = _blockInfo[nextSucc];
                ...
                while (succs.hasMoreElements()) {
                    int32_t nextSucc = succs.getNextElement();
                    if (performTransformation(
                            comp(), "%s Success: Coarsening monent in block_%d\n", OPT_DETAILS, nextSucc)) {
                        _invalidateUseDefInfo = true;
                        _invalidateValueNumberInfo = true;
                        block = _blockInfo[nextSucc];
...
static bool sideEntranceOrExitExists(
    TR_BitVector* intersection, TR::Block** blockInfo, int32_t predNum, int32_t succNum)
{
    TR::Block* first = NULL;
    TR::CFGEdge* edge;
...
void TR::MonitorElimination::buildClosure(
    int32_t blockNum, TR_BitVector* monentBlocks, TR_BitVector* successors, int32_t lockedObject)
{
    collectSuccessors(blockNum, monentBlocks, successors, lockedObject);
}

void TR::MonitorElimination::collectSuccessors(
    int32_t blockNum, TR_BitVector* monentBlocks, TR_BitVector* successors, int32_t lockedObject)
{
    ...
    if (_containsCalls->get(blockNum)) {
        ...
        recoveryPossible = treesAllowCoarsening(
            _monexitTrees[blockNum]->getNextTreeTop(), block->getExit(), &peekedCall, &blockContainsCheck);
        if (recoveryPossible) {
            // if (peekedCall)
    ...
    while (succs.hasMoreElements()) {
        ...
        if (_monentBlockInfo[nextSucc] != -1 && _monentBlockInfo[nextSucc] == lockedObject) {
            ...
            *_subtraction -= *_loopEntryBlocks;
                    ...
                    *_subtraction -= *_scratch;
                    if (!(*_subtraction == *_intersection)) {
                        ...
                        if (succPostDominatesPred && isSimpleLockedRegion(_monentTrees[nextSucc])) {
                            if (symbolsAreNotWritten(_intersection)) {
                                if (symbolsAreNotWrittenInTrees(
                                        _monexitTrees[blockNum]->getNextTreeTop(), _blockInfo[blockNum]->getExit())
                    ...
                    if (succPostDominatesPred && isSimpleLockedRegion(_monentTrees[nextSucc])) {
                        ...
                        if (recoveryPossibleInInterveningBlocks) {
                            if (symbolsAreNotWritten(_intersection)
                                && symbolsAreNotWrittenInTrees(
                                       _monexitTrees[blockNum]->getNextTreeTop(), _blockInfo[blockNum]->getExit())
                                && symbolsAreNotWrittenInTrees(
                                       _blockInfo[nextSucc]->getEntry(), _monentTrees[nextSucc]->getPrevTreeTop())) {
                                canRecover = true;
                                if (trace()) {
                                    traceMsg(comp(),
                                        "Found a coarsening opportunity across call (peek done successfully)\n");
                                    // printf("Found a coarsening opportunity across call (peek done successfully) in
                                    // %s\n", comp()->signature());
                                }
                            }
                        }
            ...
            // canRecover, blockNum, nextSucc); if (canRecover)
            //   {
                ...
                if (!simpleCoarsening) {
                    ...
                    for (TR_ClassLoadCheck* clc = _classesThatShouldNotBeLoaded.getFirst(); clc; clc = clc->getNext())
                        addClassThatShouldNotBeLoaded(
                            clc->_name, clc->_length, comp()->getClassesThatShouldNotBeLoaded(), false);

                    for (TR_ClassExtendCheck* cec = _classesThatShouldNotBeNewlyExtended.getFirst(); cec;
                         cec = cec->getNext())
                        addClassThatShouldNotBeNewlyExtended(
                            cec->_clazz, comp()->getClassesThatShouldNotBeNewlyExtended(), false);
                } else {
...
void TR::MonitorElimination::collectPredecessors(
    int32_t blockNum, TR_BitVector* monexitBlocks, TR_BitVector* predecessors, int32_t lockedObject)
{
    bool peekedCall = false;
    bool recoveryPossible = true;
    bool blockContainsCheck = false;
    if (_containsCalls->get(blockNum)) {
        TR::Block* block = _blockInfo[blockNum];
        recoveryPossible = treesAllowCoarsening(
            block->getEntry(), _monentTrees[blockNum]->getPrevTreeTop(), &peekedCall, &blockContainsCheck);
        if (recoveryPossible) {
            // printf("Found a coarsening opportunity across call (peek done successfully) in %s\n",
            // comp()->signature());
        } else
    ...
    while (preds.hasMoreElements()) {
        ...
        if (_monexitBlockInfo[nextPred] != -1 && _monexitBlockInfo[nextPred] == lockedObject) {
            ...
            *_subtraction -= *_loopEntryBlocks;
                    ...
                    *_subtraction -= *_scratch;
                    if (!(*_subtraction == *_intersection)) {
                        ...
                        if (succPostDominatesPred && isSimpleLockedRegion(_monexitTrees[nextPred])) {
                            if (symbolsAreNotWritten(_intersection)
                                && symbolsAreNotWrittenInTrees(
                                       _monexitTrees[nextPred]->getNextTreeTop(), _blockInfo[nextPred]->getExit())
                                && symbolsAreNotWrittenInTrees(
                                       _blockInfo[blockNum]->getEntry(), _monentTrees[blockNum]->getPrevTreeTop())) {
                                canRecover = true;
                                if (trace()) {
                                    traceMsg(comp(), "Found a coarsening opportunity across loop\n");
                                    // printf("Found a coarsening opportunity across loop in %s\n",
                    ...
                    if (succPostDominatesPred && isSimpleLockedRegion(_monexitTrees[nextPred])) {
                        ...
                        if (recoveryPossibleInInterveningBlocks && peekedCall) {
                            ...
                            TR::Block* block = _blockInfo[blockNum];
                            recoveryPossibleInInterveningBlocks = treesAllowCoarsening(
                                block->getEntry(), _monentTrees[blockNum]->getPrevTreeTop(), &peekingDone);
                        }
            ...
            // canRecover, blockNum, nextPred); if (canRecover)
            //   {
            //   _matchingMonentBlocks->print(comp());
            //   _matchingMonexitBlocks->print(comp());
            //   }
...
void TR::MonitorElimination::adjustMonentAndMonexitBlocks(
    TR::Node* prevMonitorNode, TR_BitVector* blocks, int32_t lockedObject)
{
...
void TR::MonitorElimination::prependMonexitAndAppendMonentInBlock(
    TR::Node* prevMonitorNode, TR::Block* block, int32_t lockedObject)
{
...
void TR::MonitorElimination::prependMonexitInBlock(
    TR::Node* prevMonitorNode, TR::Block* nextBlock, int32_t lockedObject, bool insertNullTest)
{
    TR::CoarsenedMonitorInfo* monitorInfo = findOrCreateCoarsenedMonitorInfo(lockedObject, prevMonitorNode);
...
void TR::MonitorElimination::prependMonexitInBlock(TR::Node* prevMonitorNode, TR::Block* nextBlock, bool insertNullTest)
{
    ...
    TR::Node* treetopNode = TR::Node::createWithSymRef(
        TR::NULLCHK, 1, 1, monexitNode, symRefTab->findOrCreateNullCheckSymbolRef(owningMethodSymbol));
...
void TR::MonitorElimination::appendMonentInBlock(
    TR::Node* prevMonitorNode, TR::Block* nextBlock, int32_t lockedObject, bool insertNullTest)
{
    TR::CoarsenedMonitorInfo* monitorInfo = findOrCreateCoarsenedMonitorInfo(lockedObject, prevMonitorNode);
...
void TR::MonitorElimination::appendMonentInBlock(TR::Node* prevMonitorNode, TR::Block* nextBlock, bool insertNullTest)
{
    ...
    TR::Node* treetopNode = TR::Node::createWithSymRef(
        TR::NULLCHK, 1, 1, monentNode, symRefTab->findOrCreateNullCheckSymbolRef(owningMethodSymbol));
    if (treetopNode->getNullCheckReference()->getOpCodeValue() != TR::loadaddr) {
        treetopNode->getNullCheckReference()->setIsNonNull(false);
        treetopNode->getNullCheckReference()->setIsNull(false);
    }
...
void TR::MonitorElimination::insertNullTestBeforeBlock(TR::Node* prevMonitorNode, TR::Block* nextBlock)
{
    ...
    for (auto edge = nextBlock->getPredecessors().begin(); edge != nextBlock->getPredecessors().end(); ++edge)
        (*edge)->getFrom()->asBlock()->getLastRealTreeTop()->adjustBranchOrSwitchTreeTop(
            comp(), nextBlock->getEntry(), ifBlock->getEntry());
    ...
    while (listElem) {
        ...
        catchBlock->append(TR::TreeTop::create(comp(),
            TR::Node::createWithSymRef(
                TR::athrow, 1, 1, temp, symRefTab->findOrCreateAThrowSymbolRef(comp()->getMethodSymbol()))));
...
bool TR::MonitorElimination::addClassThatShouldNotBeLoaded(
    char* name, int32_t len, TR_LinkHead<TR_ClassLoadCheck>* classesThatShouldNotBeLoaded, bool stackAllocation)
{
    ...
    if (!comp()->getClassesThatShouldNotBeLoaded()->isEmpty()
        ...
        if (gotoBlock) {
            TR::TreeTop* gotoTreeTop = TR::TreeTop::create(
                comp(), TR::Node::create(block->getEntry()->getNode(), TR::Goto, 0, block->getNextBlock()->getEntry()));
            gotoBlock->append(gotoTreeTop);
            newBlock->getExit()->join(gotoBlock->getEntry());
            gotoBlock->getExit()->setNextTreeTop(NULL);
            gotoBlock->setIsCold();
            gotoBlock->setFrequency(VERSIONED_COLD_BLOCK_COUNT);
            newBlock->getLastRealTreeTop()->adjustBranchOrSwitchTreeTop(
                comp(), block->getNextBlock()->getEntry(), gotoBlock->getEntry());
        ...
        for (auto nextEdge = block->getPredecessors().begin(); nextEdge != block->getPredecessors().end(); ++nextEdge) {
            ...
            if (!pred->hasSuccessor(guardBlock)) {
                ...
                pred->getLastRealTreeTop()->adjustBranchOrSwitchTreeTop(
                    comp(), block->getEntry(), guardBlock->getEntry());
        ...
        // if (!_alreadyClonedNodes.find(currNode))
        {
            ...
            if (currNode->getOpCode().isStore()) {
                ...
                if (newNode && !_alreadyClonedNodes.find(currNode)) {
                    ...
                    TR::SymbolReference* newSymRef = comp()->getSymRefTab()->createTemporary(
...
bool TR::MonitorElimination::checkIfSuccsInList(
    TR::CFGEdgeList& edgesList, TR_BitVector* list, bool checkIfControlFlowInCatch, TR::Block* block)
{
...
TR::CoarsenedMonitorInfo* TR::MonitorElimination::findOrCreateCoarsenedMonitorInfo(
    int32_t lockedObject, TR::Node* lockedNode)
{
...
bool TR::MonitorElimination::treesAllowCoarsening(
    TR::TreeTop* startTree, TR::TreeTop* endTree, bool* peekedCall, bool* seenCheck)
{
                        ...
                        if (symRef->getSymbol()->isStatic()
                            ...
                            if (symRef->getSymbol()->isStatic()) {
                                if (symRef->getSymbol()->isConstObjectRef())
                                    ...
                                    sig = symRef->getOwningMethod(comp())->staticName(
                                        symRef->getCPIndex(), length, trMemory());
                            } else if (symRef->getSymbol()->isShadow()) {
                                sig = symRef->getOwningMethod(comp())->fieldName(
                                    symRef->getCPIndex(), length, trMemory());
                            }
...
bool TR::MonitorElimination::symbolsAreNotWrittenInTrees(TR::TreeTop* startTree, TR::TreeTop* endTree)
{
    ...
    while (cursorTree != endTree) {
        ...
        if (cursorNode->getOpCode().isStore() || cursorNode->mightHaveVolatileSymbolReference()) {
    ...
    if (symRef && symRef->isUnresolved()) {
        if (!(symRef->getSymbol()->isStatic() && symRef->getSymbol()->castToStaticSymbol()->isConstObjectRef())) {
            if ((node->getOpCodeValue() == TR::loadaddr)
                ...
                char* className = symRef->getOwningMethod(comp())->getClassNameFromConstantPool(
                    symRef->getCPIndex(), classNameLength);
                    ...
                    *success = false;
                    if (trace()) {
                        traceMsg(comp(),
                            ...
                            "unsuccessful\n",
                            node);
                        // printf("Found unresolved class object load %p while peeking and unable to add assumption --
                        // peek unsuccessful\n", node);
                    }
                } else {
                    ...
                    if (trace()) {
                        traceMsg(comp(),
                            "Found unresolved class object node %p while peeking -- add assumption -- skip peeking in "
                            "rest of block\n",
                            node);
                        // printf("Found unresolved class object node %p while peeking -- add assumption for class
                        // %s\n", node, className);
                    }
                ...
                if (symRef->getSymbol()->isShadow() || symRef->getSymbol()->isStatic()) {
                    className = symRef->getOwningMethod(comp())->classNameOfFieldOrStatic(
                        symRef->getCPIndex(), classnameLength);
                ...
                if (className) {
                    TR_OpaqueClassBlock* declaringClazz = comp()->fej9()->getClassFromSignature(
                        className, classnameLength, symRef->getOwningMethod(comp()));
                    if (!declaringClazz) {
                        addClassThatShouldNotBeLoaded(className, classnameLength);

                        if (trace()) {
                            traceMsg(comp(),
                                "Found unresolved class object node %p while peeking -- skip peeking in rest of "
                                "block\n",
                                node);
                            // printf("Found unresolved class object node %p while peeking -- add assumption for class
                            // %s\n", node, className);
                    ...
                    /*
                       ...
                       *success = false;
                       if (trace())
                          {
                          traceMsg(comp(), "Found unresolved node %p while peeking whose class in resolved -- peek
                    unsuccessful\n", node); printf("Found unresolved node %p while peeking whose class %s in resolved --
                    peek unsuccessful\n", node, className);
                          }
                       }
                    */
                    ...
                    *success = false;
                    if (trace()) {
                        traceMsg(comp(),
                            "Found unresolved node %p while peeking whose class is unresolved and unable to add "
                            "assumption -- peek unsuccessful\n",
                            node);
                        // printf("Found unresolved node %p while peeking whose class is unresolved and unable to add
                        // assumption -- peek unsuccessful\n", node);
                    }
                }
            }
