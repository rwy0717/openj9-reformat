...
int32_t TR_NewInitialization::performAnalysis(bool doGlobalAnalysis)
{
    ...
    //
    _allowMerge = cg()->getSupportsMergedAllocations() && !comp()->getOption(TR_DisableMergeNew)
        && !comp()->suppressAllocationInlining() && !comp()->compileRelocatableCode()
...
bool TR_NewInitialization::findNewCandidatesInBlock(TR::TreeTop* startTree, TR::TreeTop* endTree)
{
    ...
    for (treeTop = startTree; treeTop != endTree; treeTop = treeTop->getNextTreeTop()) {
        ...
        if (findAllocationNode(treeTop, child)) {
            ...
            if (_allowMerge && (treeTop->getNode()->getOpCodeValue() != TR::MergeNew)
...
bool TR_NewInitialization::findAllocationNode(TR::TreeTop* treeTop, TR::Node* node)
{
    ...
    if (node->getOpCodeValue() == TR::New) {
        ...
        if (node->getOpCodeValue() == TR::anewarray) {
            ...
            TR_ASSERT(
                node->getSecondChild()->getOpCodeValue() == TR::iconst, "Expected type as second child of newarray");
...
TR::ResolvedMethodSymbol* TR_NewInitialization::findInlinableMethod(TR::TreeTop* callTree)
{
    ...
    // TR_CallSite *callsite = TR_CallSite::create(callTree, parent, callNode, thisClass, symRef,  0, comp(),
    // trStackMemory());
    ...
    // (symRef->getOwningMethod(comp()),callTree,parent,callNode,calleeMethodSymbol->getMethod(),thisClass,(int32_t)symRef->getOffset(),symRef->getCPIndex(),0,calleeMethodSymbol->getResolvedMethodSymbol(),callNode->getOpCode().isCallIndirect(),calleeMethodSymbol->isInterface(),callNode->getByteCodeInfo(),comp());
    ...
    if (!canSniff) {
        if (trace())
            traceMsg(comp(), "\nCall at [%p] to %s is NOT inlineable\n", callTree->getNode()->getFirstChild(),
                calleeSymbol->getResolvedMethod()->signature(trMemory()));
        return NULL;
    }
...
void TR_NewInitialization::setGCPoint(TR::TreeTop* treeTop, TR::Node* node)
{
    ...
    //
    if (_firstMergeCandidate) {
        if (!_firstMergeCandidate->firstGCTree && (node == NULL || node->canGCandReturn())) {
...
TR_NewInitialization::Candidate* TR_NewInitialization::findCandidateReferenceInSubTree(
    TR::Node* node, TR_ScratchList<TR::Node>* seenNodes)
{
...
void TR_NewInitialization::escapeToGC(Candidate* c, TR::Node* cause)
{
    ...
    if (c->node->getOpCodeValue() == TR::New) {
        int32_t* referenceSlots = comp()->fej9()->getReferenceSlotsInClass(
            comp(), (TR_OpaqueClassBlock*)c->node->getFirstChild()->getSymbol()->getStaticSymbol()->getStaticAddress());
        if (referenceSlots) {
...
void TR_NewInitialization::findUninitializedWords()
{
    ...
    for (c = _candidates.getFirst(); c; c = c->getNext()) {
        ...
        if (trace()) {
            traceMsg(comp(), "Uninitialized words for candidate [%p] = %d/%d : ", c->node, c->numUninitializedWords,
                c->size / 4);
    ...
    while (!_inlinedCallSites.isEmpty()) {
        ...
        if (trace()) {
            traceMsg(comp(), "\nInlining method %s into treetop at [%p], total inlined size = %d\n",
                method->signature(trMemory()), treeTop->getNode(),
    ...
    if (numExtraCandidates) {
        if (trace())
            traceMsg(comp(), "Found %d news to be merged, %d words to be initialized in %s\n", numExtraCandidates + 1,
                numWordsToBeInitialized, comp()->signature());
        ...
        symRef = new (trHeapMemory())
            TR::SymbolReference(comp()->getSymRefTab(), *candidate->node->getSymbolReference(), 0);
...
int32_t TR_NewInitialization::buildInitializationInfo(
    Candidate* c, TR_BitVector* wordsToBeInitialized, int32_t startWord)
{
...
void TR_NewInitialization::modifyReferences(
    Candidate* candidate, Candidate* startOfNextMergeSequence, Candidate* startCandidate, TR::TreeTop* mergeTree)
{
...
void TR_NewInitialization::modifyReferences(
    Candidate* candidate, Candidate* startOfNextMergeSequence, Candidate* startCandidate, TR::Node* node)
{
    int32_t i; //, j;
    ...
    for (i = 0; i < node->getNumChildren(); i++) {
        ...
        if (child->getOpCodeValue() == TR::New || child->getOpCodeValue() == TR::newarray
                    ...
                    if (((TR::Compiler->target.is64Bit())
                        ...
                        if (!c->offsetReference) {
                            ...
                            if (TR::Compiler->target.is64Bit()) {
                                ...
                                c->offsetReference = TR::Node::create(TR::aiadd, 2, startCandidate->node,
                                    TR::Node::create(child, TR::iconst, 0, c->startOffset));
    ...
    //
    if (node->getOpCodeValue() == TR::awrtbari && firstChildIsCandidate && secondChildIsCandidate
        && !comp()->getOptions()->realTimeGC()) {
