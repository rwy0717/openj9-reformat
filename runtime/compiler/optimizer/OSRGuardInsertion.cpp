        ...
        if (!fearGeneratingNodes.isEmpty()) {
            ...
            if (trace()) {
                comp()->dumpMethodTrees(
                    "Trees after redundant potentialOSRPointHelper call removal", comp()->getMethodSymbol());
            }
...
TR_Structure* fakeRegion(TR::Compilation* comp)
{
    ...
    for (cfgNode = cfg->getFirstNode(); cfgNode; cfgNode = cfgNode->getNext()) {
        ...
        (*blocks)[block->getNumber()] = new (structureRegion)
            TR_StructureSubGraphNode(new (structureRegion) TR_BlockStructure(comp, block->getNumber(), block));
        ...
        if (!guardAnalysis || guardAnalysis->_blockAnalysisInfo[cursor->getNextBlock()->getNumber()]->isEmpty()) {
            if (guardInfo->getKind() == TR_HCRGuard
                && performTransformation(
                       comp(), "O^O HCR GUARD REMOVAL: removing HCRGuard node n%dn\n", node->getGlobalIndex())) {
                ...
                if (TR_FearPointAnalysis::virtualGuardsKillFear() && additionalVirtualGuard
                ...
                TR::DebugCounter::prependDebugCounter(
                    comp(), TR::DebugCounter::debugCounterName(comp(), "hcrGuardRemoval/success"), cursor->getExit());
...
int32_t TR_OSRGuardInsertion::insertOSRGuards(TR_BitVector& fearGeneratingNodes)
{
    ...
    TR_FearPointAnalysis fearAnalysis(
        comp(), optimizer(), comp()->getFlowGraph()->getStructure(), fearGeneratingNodes, true, trace());
    ...
    for (TR::TreeTop* cursor = comp()->findLastTree(); cursor; cursor = cursor->getPrevTreeTop()) {
        ...
        if (TR_FearPointAnalysis::virtualGuardsKillFear()
            && cursor->getNode()->isTheVirtualGuardForAGuardedInlinedCall() && comp()->cg()->supportsMergingGuards()) {
            TR::DebugCounter::prependDebugCounter(comp(),
            ...
            TR::DebugCounter::prependDebugCounter(comp(),
                TR::DebugCounter::debugCounterName(comp(), "osrGuardSummary/whitelist/asynccheck/%s/=%d",
                    comp()->getHotnessName(comp()->getMethodHotness()), block->getFrequency()),
                cursor);
        } else {
            if (cursor->getNode()->getGlobalIndex() < initialNodeCount)
...
void TR_OSRGuardInsertion::generateTriggeringRecompilationTrees(
    TR::TreeTop* osrGuard, TR_PersistentMethodInfo::InfoBits reason)
{
...
void TR_OSRGuardInsertion::performRemat(TR::TreeTop* osrPoint, TR::TreeTop* osrGuard, TR::TreeTop* rematDest)
{
    ...
    for (TR::TreeTop* cursor = realStart; cursor; cursor = cursor->getPrevTreeTop()) {
        ...
        if (node->getOpCode().isStoreDirect() && !seen[node->getSymbolReference()->getReferenceNumber()]) {
            ...
            TR_YesNoMaybe result = RematTools::gatherNodesToCheck(
                comp(), node, node->getFirstChild(), scanTargets, argSymRefsToCheck, trace());
    ...
    if (failedArgs.size() > 0) {
        RematTools::walkTreeTopsCalculatingRematFailureAlternatives(
            comp(), start, osrGuard, failedArgs, scanTargets, safetyInfo, blocksToVisit, trace());
        ...
        for (auto iter = failedArgs.begin(); iter != failedArgs.end(); ++iter) {
            ...
            TR::DebugCounter::prependDebugCounter(comp(),
                TR::DebugCounter::debugCounterName(
                    comp(), "osrGuardRemat/Failed/%s", store->getFirstChild()->getOpCode().getName()),
            ...
            TR::DebugCounter::incStaticDebugCounter(comp(),
                TR::DebugCounter::debugCounterName(
                    comp(), "osrGuardRemat.byReason/%s", store->getFirstChild()->getOpCode().getName()));
        }
    }
    ...
    if (!scanTargets.IsZero())
        RematTools::walkTreesCalculatingRematSafety(
            comp(), start, osrGuard, scanTargets, unsafeSymRefs, blocksToVisit, trace());
    ...
    for (int32_t i = safetyInfo.size() - 1; i >= 0; --i) {
        ...
        if (!unsafeSymRefs.Intersects(safetyInfo.symRefDependencies(i))) {
            ...
            TR::DebugCounter::incStaticDebugCounter(comp(),
                TR::DebugCounter::debugCounterName(
                    comp(), "osrGuardRemat.byMethod/(%s)/Succeeded", comp()->signature()));
            TR::DebugCounter::incStaticDebugCounter(
                comp(), TR::DebugCounter::debugCounterName(comp(), "osrGuardRemat.byReason/Success"));
            TR::DebugCounter::prependDebugCounter(comp(),
                TR::DebugCounter::debugCounterName(
                    comp(), "osrGuradRemat/Succeeded/(%s)", store->getFirstChild()->getOpCode().getName()),
                storeTree, 1, TR::DebugCounter::Expensive);
            ...
            TR::DebugCounter::incStaticDebugCounter(comp(),
                TR::DebugCounter::debugCounterName(
                    comp(), "osrGuardRemat.byReason/unsafeSymRef/%s", store->getFirstChild()->getOpCode().getName()));
            TR::DebugCounter::prependDebugCounter(comp(),
                TR::DebugCounter::debugCounterName(
                    comp(), "osrGuardRemat/Failed/(%s)", store->getFirstChild()->getOpCode().getName()),
                storeTree, 1, TR::DebugCounter::Expensive);
