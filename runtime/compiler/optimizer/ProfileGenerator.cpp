    ...
    if (nodeCount > MAX_NUMBER_OF_ALLOWED_NODES) {
        ...
        if (!comp()->getOption(TR_ProcessHugeMethods)
            && (((actualNodeCount + actualNodeCount) > USHRT_MAX)
                   ...
                   // we are avoiding cloning in more cases conservatively for profiling hot, only when double the
                   // actual node count is larger than a threshold this is even more conservative for profiling very
                   // hot, where we also consider the actual CPU time spent in compilation. the conditions below may be
                   // simplified further by only consider CPU time is expensive or not.
                   || switchAwayFromProfilingForHotAndVeryhot))
    ...
    for (treeTop = _firstOriginalTree; treeTop != _lastOriginalTree; treeTop = nextTreeTop) {
        ...
        if (node->getOpCodeValue() == TR::BBStart) {
            ...
            if (firstSplitPoint) {
                ...
                TR::Node* sumNode = TR::Node::create(
                    opCode, 2, TR::Node::createWithSymRef(node, TR::aload, 0, freqPtrSymRef), offsetNode);
                newNode = TR::Node::createWithSymRef(TR::astore, 1, 1, sumNode, freqPtrSymRef);
                treeTop = TR::TreeTop::create(comp(), treeTop, newNode);

                sumNode = TR::Node::create(
                    opCode, 2, TR::Node::createWithSymRef(node, TR::aload, 0, cntrPtrSymRef), offsetNode);
            ...
            newNode = TR::Node::createif(
                TR::ificmpeq, freqNode, TR::Node::create(node, TR::iconst, 0, INT_MAX), blockO1a->getEntry());
            TR::TreeTop::create(comp(), treeTop, newNode);
            if (regDeps) {
                newNode->setNumChildren(3);
                newNode->setChild(2, copyRegDeps(regDeps, true));
            ...
            newNode = TR::Node::createif(
                TR::ificmple, decrNode, TR::Node::create(node, TR::iconst, 0, 0), blockC1b->getEntry());
            TR::TreeTop::create(comp(), treeTop, newNode);
            if (regDeps) {
                newNode->setNumChildren(3);
                newNode->setChild(2, copyRegDeps(regDeps, true));
            ...
            newNode = TR::Node::createWithSymRef(
                TR::istore, 1, 1, TR::Node::create(node, TR::iconst, 0, INT_MAX), tempFreqSymRef);
            ...
            if (comp()->cg()->getAccessStaticsIndirectly() && !recompilationCounterSymRef->isUnresolved()
                && type != TR::Address) {
                TR::SymbolReference* symRefShadow
                    = comp()->getSymRefTab()->createKnownStaticDataSymbolRef(symbol->getStaticAddress(), type);
            ...
            newNode = TR::Node::createif(
                TR::ificmpgt, decrNode, TR::Node::create(node, TR::iconst, 0, 0), blockC1a->getEntry());
            treeTop = TR::TreeTop::create(comp(), treeTop, newNode);
            if (regDeps) {
                newNode->setNumChildren(3);
                newNode->setChild(2, copyRegDeps(regDeps, true));
