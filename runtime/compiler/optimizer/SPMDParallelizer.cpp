...
// SIMD_TODO: use visitCount or some caching?
bool TR_SPMDKernelParallelizer::isAffineAccess(
    TR::Compilation* comp, TR::Node* node, TR_RegionStructure* loop, TR::SymbolReference* piv, int32_t& pivStride)
{
    ...
    if (scalarOp.isStore()) {
        ...
        if (scalarOp.isIndirect()) {
            ...
            if (((TR::Node*)_loopDataType->getData(id))->getSize() != node->getSize() && isCheckMode) {
                if (trace)
                    traceMsg(comp, "   Node size does not match loop data type for node: %p\n", node);
                return false;
            }
            ...
            if (!isCheckMode) {
                ...
                vecSymRef = comp->getSymRefTab()->findOrCreateArrayShadowSymbolRef(
                    node->getDataType().scalarToVector(), NULL);
            ...
            if (isCheckMode) {
                ...
                if (reductionHashTab->locate(node->getSymbolReference(), id)) {
                    ...
                    if (trace)
                        traceMsg(comp, "   visitTreeTopToSIMDize: Found reductionInfo for node: %p, symRef: %p\n", node,
                    ...
                    if (trace)
                        traceMsg(comp, "   visitTreeTopToSIMDize: Created reductionInfo for node: %p, symRef: %p\n",
                ...
                if (reductionInfo->reductionOp != Reduction_Invalid) {
                    ...
                    if (isReduction(comp, loop, node->getFirstChild(), reductionInfo, reductionInfo->reductionOp)) {
                        if (trace)
                            traceMsg(comp, "   visitTreeTopToSIMDize: Reduction pattern match on node: %p\n", node);
                        ...
                        if (trace)
                            traceMsg(comp, "   visitTreeTopToSIMDize: Reduction pattern mismatch for node: %p\n", node);
                        ...
                        for (TR::Node* nextNode = nodeIt.getCurrent(); nextNode; nextNode = nodeIt.getNext()) {
                            if (trace)
                                traceMsg(comp,
                                    "   visitTreeTopToSIMDize: Adding recorded invalid reduction node to defsInLoop. "
                                    "node: %p\n",
                                    nextNode);
            ...
            if (!isCheckMode) {
                ...
                if (vecSymRef == NULL) {
                    vecSymRef = comp->cg()->allocateLocalTemp(
                        node->getDataType().scalarToVector()); // need to handle alignment?
                ...
                if (createdNewVecSym && reductionHashTab->locate(symRef, id)) {
                    ...
                    if (reductionInfo->reductionOp != Reduction_Invalid) {
                        if (trace)
                            traceMsg(comp, "   node: %p is a store to a reduction var\n", node);
    ...
    TR::SymbolReference* storeSymRef)
{
    ...
    // transformed trees are harder to debug in codegen and can happen due to multiple reasons. So for easy of debugging,
    // we go ahead and set BadIlOp.
    if (isCheckMode && vectorOpCode == TR::BadILOp) {
        if (trace) {
    ...
    if (scalarOp.isLoadVar()) {
            ...
            if (!scalarOp.isLoadIndirect()) {
                ...
                if (loadInductionVar) {
                    ...
                    platformSupport = platformSupport
                        && comp->cg()->getSupportsOpCodeForAutoSIMD(TR::vsetelem, node->getDataType());
                    platformSupport
                        = platformSupport && comp->cg()->getSupportsOpCodeForAutoSIMD(TR::vadd, node->getDataType());
                    platformSupport
                        = platformSupport && comp->cg()->getSupportsOpCodeForAutoSIMD(TR::vsplats, node->getDataType());
                    ...
                    if (trace && !platformSupport)
                        traceMsg(comp,
                            "   Found use of induction variable at node [%p] - platform does not support this "
                            "vectorization\n",
                            node);
        ...
        if (!isCheckMode) {
            ...
            if (!vecSymRef) {
                vecSymRef = scalarOp.isLoadIndirect()
                    ...
                    : vecSymRef
                    = comp->cg()->allocateLocalTemp(node->getDataType().scalarToVector()); // need to handle alignment?
                pSPMDInfo->addVectorSymRef(symRef, vecSymRef);
                createdNewVecSym = true;
            ...
            if (!scalarOp.isLoadIndirect()) {
                if (createdNewVecSym && reductionHashTab->locate(node->getSymbolReference(), id)) {
                    ...
                    if (reductionInfo->reductionOp != Reduction_Invalid) {
                        if (trace)
                            traceMsg(comp, "   node: %p is a load from a reduction var\n", node);
        ...
        if (scalarOp.isLoadIndirect()) {
            ...
            if (reductionHashTab->locate(node->getSymbolReference(), id)) {
                ...
                // = sum + a[i]) this code checks that this condition is being met. If not, the symRef is not a supported
                // reduction symRef
                if (reductionInfo->reductionOp != Reduction_Invalid && node->getSymbolReference() != storeSymRef) {
                    if (trace)
                        traceMsg(
                            comp, "   visitNodeToSIMDize: Load symRef does not match store symref at node: %p\n", node);
                    reductionInfo->reductionOp = Reduction_Invalid;
                    ListIterator<TR::Node> nodeIt(&reductionInfo->storeNodes);
                    for (TR::Node* nextNode = nodeIt.getCurrent(); nextNode; nextNode = nodeIt.getNext()) {
                        if (trace)
                            traceMsg(comp,
                                "   visitNodeToSIMDize: Adding recorded invalid reduction node to defsInLoop. node: "
                                "%p\n",
                                nextNode);
                        defsInLoop[nextNode->getGlobalIndex()] = true;
                        collectUses(nextNode, comp, useDefInfo, useNodesOfDefsInLoop);
                ...
                if (trace)
                    traceMsg(comp,
                        "   visitNodeToSIMDize: Never before seen symRef being recorded as not a reduction at node: "
                        "%p\n",
                        node);
        ...
        if (scalarOp.isNeg() || scalarOp.getOpCodeValue() == TR::l2d) {
            ...
            TR_ASSERT(
                firstChild->getDataType().isVector(), " Child of the node to be transformed must be a vector type");
            TR::Node::recreate(node, vectorOpCode);
            return true;
        }
    }
...
bool TR_SPMDKernelParallelizer::autoSIMDReductionSupported(TR::Compilation* comp, TR::Node* node)
{
    ...
    if (!comp->cg()->getSupportsOpCodeForAutoSIMD(TR::vsplats, node->getDataType())) {
        if (trace)
            traceMsg(comp, "   autoSIMDReductionSupported: vsplats is not supported for dataType: %s\n",
    ...
    if (!comp->cg()->getSupportsOpCodeForAutoSIMD(TR::vstore, node->getDataType())) {
        if (trace)
            traceMsg(comp, "   autoSIMDReductionSupported: vstore is not supported for dataType: %s\n",
                node->getDataType().toString());
        return false;
    }
    ...
    if (!comp->cg()->getSupportsOpCodeForAutoSIMD(TR::vload, node->getDataType())) {
        if (trace)
            traceMsg(comp, "   autoSIMDReductionSupported: vload is not supported for dataType: %s\n",
                node->getDataType().toString());
        return false;
    }
    ...
    if (!comp->cg()->getSupportsOpCodeForAutoSIMD(TR::getvelem, node->getDataType())) {
        if (trace)
            traceMsg(comp, "   autoSIMDReductionSupported: getvelem is not supported for dataType: %s\n",
                node->getDataType().toString());
        return false;
    }
    ...
    TR_SPMDReductionInfo* reductionInfo, TR_SPMDReductionOp pathOp)
{
    ...
    if (opCode.isLoadVar()) {
        if (opCode.isLoadDirect() && node->getSymbolReference() == reductionInfo->reductionSymRef) {
            if (trace)
                traceMsg(comp, "   isReduction: found potential reduction symRef. Node %p\n", node);
...
bool TR_SPMDKernelParallelizer::noReductionVar(
    TR::Compilation* comp, TR_RegionStructure* loop, TR::Node* node, TR_SPMDReductionInfo* reductionInfo)
{
    bool trace = comp->trace(OMR::SPMDKernelParallelization);
    ...
    if (opCode.isLoadVar()) {
        if (opCode.isLoadDirect() && node->getSymbolReference() == reductionInfo->reductionSymRef) {
            if (trace)
                traceMsg(comp, "   noReductionVar: found multiple uses of reduction symRef. Node %p\n", node);
            return false;
        } else
            return true;
    ...
    TR::SymbolReference* symRef, TR::SymbolReference* vecSymRef, TR_SPMDReductionOp reductionOp)
{
    ...
    if (trace)
        traceMsg(comp, "   reductionLoopEntranceProcessing: loop: %d, symRef: %p, vecSymRef: %p\n", loop->getNumber(),
            symRef, vecSymRef);
    ...
    if (!loopInvariantBlock) {
        if (trace)
            traceMsg(comp, "   reductionLoopEntranceProcessing: Loop: %d. No loop invariant block. Creating one.\n",
                loop->getNumber());
        loopInvariantBlock = createLoopInvariantBlockSIMD(comp, loop);
    }
    ...
    default:
        if (trace)
            traceMsg(
                comp, "   reductionLoopEntranceProcessing: Unknown vector data type during transformation phase.\n");
    ...
    if (trace)
        traceMsg(comp, "   reductionLoopEntranceProcessing: Loop: %d. Created reduction identity store node: %p\n",
    ...
    TR::SymbolReference* symRef, TR::SymbolReference* vecSymRef, TR_SPMDReductionOp reductionOp)
{
    ...
    if (trace)
        traceMsg(comp, "   reductionLoopExitProcessing: loop: %d, symRef: %p, vecSymRef: %p\n", loop->getNumber(),
            symRef, vecSymRef);
    ...
    if (!(reductionOp == Reduction_Add || reductionOp == Reduction_Mul)) {
        if (trace)
            traceMsg(
                comp, "   reductionLoopExitProcessing: Invalid or unknown reductionOp during transformation phase.\n");
        TR_ASSERT(0, "Invalid or unknown reductionOp during transformation phase");
        return false;
    }
    ...
    default:
        if (trace)
            traceMsg(comp, "   reductionLoopExitProcessing: Unknown vector data type during transformation phase.\n");
        TR_ASSERT(0, "Unknown vector data type during transformation phase.");
        return false;
        break;
    }
    ...
    for (edge = ei.getCurrent(); edge; edge = ei.getNext()) {
        ...
        if (trace)
            traceMsg(comp, "   reductionLoopExitProcessing: Created block: %d\n", reductionBlock->getNumber());
        ...
        if (trace)
            traceMsg(comp, "   reductionLoopExitProcessing: Created tree: %p\n", treetopNode);
    ...
    TR::Block* invariantBlock)
{
   ...
   unroller._optimizer->setEnableOptimization(inductionVariableAnalysis, true, NULL);
    ...
    for (TR::Block* nextBlock = blocksIt1.getCurrent(); nextBlock; nextBlock = blocksIt1.getNext()) {
        for (TR::TreeTop* tt = nextBlock->getEntry(); tt != nextBlock->getExit(); tt = tt->getNextTreeTop()) {
            ...
            if (storeNode->getOpCodeValue() == TR::istore
                && storeNode->getSymbolReference() == unroller._piv->getSymRef()) {
                // increment of PIV
                traceMsg(comp,
    ...
    for (TR::Block* nextBlock = blocksIt.getCurrent(); nextBlock; nextBlock = blocksIt.getNext()) {
        for (TR::TreeTop* tt = nextBlock->getEntry(); tt != nextBlock->getExit(); tt = tt->getNextTreeTop()) {
            visitTreeTopToSIMDize(
                tt, pSPMDInfo, false, loop, useNodesOfDefsInLoop, comp, NULL, defsInLoop, NULL, reductionHashTab);
        }
    }
...
void TR_SPMDKernelParallelizer::convertIntoParm(
    TR::Node* node, int32_t elementSize, ListAppender<TR::ParameterSymbol>& parms)
{
    ...
    bool postDominates)
{
                    ...
                    if (opcode.isLoadVar() || opcode.getOpCodeValue() == TR::arraylength
                        ...
                        if (!disableDataTransferElimination) {
                                ...
                                && gpuSymbolMap[nc]._rhsAddrExpr != INVALID_ADDR) // single read from that array
                            {
                                ...
                                traceMsg(
                                    comp(), "gpuSymbolMap[%d]._elementSize=%d\n", nc, gpuSymbolMap[nc]._elementSize);
                                ...
                                if ((pivStride != INVALID_STRIDE) && (pivStride > 0)) {
                                    ...
                                    traceMsg(
                                        comp(), "Detected affine load %p in RHS for gpuSymbolMap[%d]\n", addrExpr, nc);
                                } else {
    ...
    for (int32_t i = 0; i < node->getNumChildren(); i++) {
        if (!visitNodeToDetectArrayAccesses(
                node->getChild(i), loop, piv, visitCount, lineNumber, arrayNotFound, trace, postDominates))
            return false;
    }
...
void TR_SPMDKernelParallelizer::generateGPUParmsBlock(
    TR::SymbolReference* allocSymRef, TR::Block* populateParmsBlock, TR::Node* firstNode)
{
    CS2::ArrayOf<gpuMapElement, TR::Allocator>& gpuSymbolMap = comp()->cg()->_gpuSymbolMap;
    ...
    for (nc.SetToFirst(); nc.Valid(); nc.SetToNext()) {
        ...
        if (!hostSymRef->getSymbol()->getType().isAddress()) {
            ...
            parmLocationNode->setAndIncChild(0,
                TR::Node::createWithSymRef(
                    firstNode, addressLoadOpCode, 0, allocSymRef)); // base address of array to store GPU parameters in
            parmLocationNode->setAndIncChild(
                1, TR::Node::create(firstNode, TR::lconst, 0, parmSlot * sizeof(void*))); // offset into the array
            ...
            populateParmsNode->setAndIncChild(
                1, TR::Node::createWithSymRef(firstNode, TR::loadaddr, 0, hostSymRef)); // GPU parameter to store
        ...
        populateParmsNode->setAndIncChild(0, parmLocationNode); // address to store GPU parameter in
        populateParmsNode->setAndIncChild(
            1, TR::Node::createWithSymRef(firstNode, TR::loadaddr, 0, devSymRef)); // GPU parameter to store
        populateParmsBlock->append(TR::TreeTop::create(comp(), populateParmsNode));
    }
}
    ...
    TR::SymbolReference* scopeSymRef)
{
    ...
    estimateGPUNode->setAndIncChild(2,
        TR::Node::createWithSymRef(
            estimateGPUNode, TR::loadaddr, 0, comp()->getSymRefTab()->findOrCreateStartPCSymbolRef()));
    ...
    TR::Node* cmpNode = TR::Node::createif(
        TR::ificmpne, estimateGPUNode, TR::Node::create(firstNode, TR::iconst, 0, 0), origLoopBlock->getEntry());
...
void TR_SPMDKernelParallelizer::insertGPUInvalidateSequence(
    TR::Node* firstNode, TR::Block* invalidateGPUBlock, TR::SymbolReference* scopeSymRef)
{
    CS2::ArrayOf<gpuMapElement, TR::Allocator>& gpuSymbolMap = comp()->cg()->_gpuSymbolMap;
    CS2::ArrayOf<gpuMapElement, TR::Allocator>::Cursor nc(gpuSymbolMap);
...
void TR_SPMDKernelParallelizer::insertGPUErrorHandler(
    TR::Node* firstNode, TR::Block* errorHandleBlock, TR::SymbolReference* scopeSymRef, TR::Block* recoveryBlock)
{
    TR::SymbolReference* helper;
    TR::ILOpCodes addressLoadOpCode = TR::lload;
    TR::ILOpCodes addressCallOpCode = TR::lcall;
    ...
    TR::SymbolReference* scopeSymRef, TR::SymbolReference* launchSymRef, TR_PrimaryInductionVariable* piv)
{
    ...
    for (nc.SetToFirst(); nc.Valid(); nc.SetToNext()) {
        ...
        if (gpuSymbolMap[nc]._lhsAddrExpr && gpuSymbolMap[nc]._lhsAddrExpr != INVALID_ADDR) {
            copyFromGPUBlock->append(TR::TreeTop::create(
                comp(), TR::Node::createStore(piv->getSymRef(), piv->getEntryValue()->duplicateTree())));

            copyFromGPUBlock->append(TR::TreeTop::create(
                comp(), TR::Node::createStore(startAddress, gpuSymbolMap[nc]._lhsAddrExpr->duplicateTree())));

            copyFromGPUBlock->append(TR::TreeTop::create(
                comp(), TR::Node::createStore(piv->getSymRef(), piv->getExitBound()->duplicateTree())));

            copyFromGPUBlock->append(TR::TreeTop::create(
                comp(), TR::Node::createStore(endAddress, gpuSymbolMap[nc]._lhsAddrExpr->duplicateTree())));
        } else {
            copyFromGPUBlock->append(TR::TreeTop::create(
                comp(), TR::Node::createStore(startAddress, TR::Node::createAddressNode(firstNode, TR::aconst, 0))));
            copyFromGPUBlock->append(TR::TreeTop::create(
                comp(), TR::Node::createStore(endAddress, TR::Node::createAddressNode(firstNode, TR::aconst, 0))));
        }
...
void TR_SPMDKernelParallelizer::insertGPUCopyToSequence(
    TR::Node* firstNode, TR::Block* copyToGPUBlock, TR::SymbolReference* scopeSymRef, TR_PrimaryInductionVariable* piv)
{
    CS2::ArrayOf<gpuMapElement, TR::Allocator>& gpuSymbolMap = comp()->cg()->_gpuSymbolMap;
    CS2::ArrayOf<gpuMapElement, TR::Allocator>::Cursor nc(gpuSymbolMap);
    TR::ILOpCodes addressCallOpCode = TR::lcall;
    ...
    for (nc.SetToFirst(); nc.Valid(); nc.SetToNext()) {
        ...
        if (useLoopBoundsHtoD || useLoopBoundsDtoH) {
            savePiv = comp()->getSymRefTab()->createTemporary(
                comp()->getMethodSymbol(), piv->getSymRef()->getSymbol()->getDataType());
        ...
        if (useLoopBoundsHtoD) {
            copyToGPUBlock->append(TR::TreeTop::create(
                comp(), TR::Node::createStore(piv->getSymRef(), piv->getEntryValue()->duplicateTree())));

            copyToGPUBlock->append(TR::TreeTop::create(
                comp(), TR::Node::createStore(startAddressHtoD, gpuSymbolMap[nc]._rhsAddrExpr->duplicateTree())));

            copyToGPUBlock->append(TR::TreeTop::create(
                comp(), TR::Node::createStore(piv->getSymRef(), piv->getExitBound()->duplicateTree())));

            copyToGPUBlock->append(TR::TreeTop::create(
                comp(), TR::Node::createStore(endAddressHtoD, gpuSymbolMap[nc]._rhsAddrExpr->duplicateTree())));
        }
        ...
        if (useLoopBoundsDtoH) {
            ...
            copyToGPUBlock->append(TR::TreeTop::create(
                comp(), TR::Node::createStore(startAddressDtoH, gpuSymbolMap[nc]._lhsAddrExpr->duplicateTree())));

            copyToGPUBlock->append(TR::TreeTop::create(
                comp(), TR::Node::createStore(piv->getSymRef(), piv->getExitBound()->duplicateTree())));

            copyToGPUBlock->append(TR::TreeTop::create(
                comp(), TR::Node::createStore(endAddressDtoH, gpuSymbolMap[nc]._lhsAddrExpr->duplicateTree())));
        }
        ...
        copyToGPUNode->setAndIncChild(3,
            TR::Node::create(
                firstNode, TR::iconst, 0, gpuSymbolMap[nc]._accessKind & TR::CodeGenerator::ReadWriteAccesses));
...
void TR_SPMDKernelParallelizer::insertGPURegionEntry(
    TR::Block* loopInvariantBlock, TR::SymbolReference* scopeSymRef, int gpuPtxCount, TR_SPMDScopeInfoType scopeType)
{
    TR::CFG* cfg = comp()->getFlowGraph();
    ...
    regionEntryGPUNode->setAndIncChild(2,
        TR::Node::createWithSymRef(
            insertionPoint->getNode(), TR::loadaddr, 0, comp()->getSymRefTab()->findOrCreateStartPCSymbolRef()));

    switch (scopeType) {
    case scopeSingleKernel:
        regionEntryGPUNode->setAndIncChild(
            3, TR::Node::create(insertionPoint->getNode(), TR::iconst, 0, TR::CodeGenerator::singleKernelScope));
        break;
    case scopeNaturalLoop:
        regionEntryGPUNode->setAndIncChild(
            3, TR::Node::create(insertionPoint->getNode(), TR::iconst, 0, TR::CodeGenerator::naturalLoopScope));
        break;
    }

    regionEntryGPUNode->setAndIncChild(4,
        TR::Node::create(
            insertionPoint->getNode(), TR::iconst, 0, comp()->getOptions()->getEnableGPU(TR_EnableGPUForce)));
...
void TR_SPMDKernelParallelizer::insertFlushGPU(
    TR_BitVector* flushGPUBlocks, TR::Block** cfgBlocks, TR::SymbolReference* scopeSymRef)
{
...
TR::Node* TR_SPMDKernelParallelizer::insertFlushGPU(TR::Block* flushGPUBlock, TR::SymbolReference* scopeSymRef)
{
    ...
    // *cudaInfo
    flushGPUNode->setAndIncChild(
        0, TR::Node::createWithSymRef(insertionPoint->getNode(), addressLoadOpCode, 0, scopeSymRef));
    // blockId
    flushGPUNode->setAndIncChild(
        1, TR::Node::create(insertionPoint->getNode(), TR::iconst, 0, flushGPUBlock->getNumber()));
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void TR_SPMDKernelParallelizer::insertGPUTemporariesLivenessCode(
    List<TR::TreeTop>* exitPointsList, TR::SymbolReference* liveSymRef, bool firstKernel)
{
    CS2::ArrayOf<gpuMapElement, TR::Allocator>& gpuSymbolMap = comp()->cg()->_gpuSymbolMap;
    CS2::ArrayOf<gpuMapElement, TR::Allocator>::Cursor nc(gpuSymbolMap);
    ...
    for (TR::TreeTop* insertionPoint = lit.getFirst(); insertionPoint; insertionPoint = lit.getNext()) {
        ...
        if (firstKernel) {
            ...
            currentNode = TR::Node::create(
                TR::a2l, 1, TR::Node::createWithSymRef(insertionPoint->getNode(), TR::loadaddr, 0, liveSymRef));
        }
        ...
        for (nc.SetToFirst(); nc.Valid(); nc.SetToNext()) {
            ...
            currentNode = TR::Node::create(TR::ladd, 2,
                TR::Node::create(
                    TR::a2l, 1, TR::Node::createWithSymRef(insertionPoint->getNode(), TR::loadaddr, 0, hostSymRef)),
                currentNode);
            ...
            currentNode = TR::Node::create(TR::ladd, 2,
                TR::Node::create(
                    TR::a2l, 1, TR::Node::createWithSymRef(insertionPoint->getNode(), TR::loadaddr, 0, devSymRef)),
                currentNode);
        }
    ...
    int gpuPtxCount, TR::SymbolReference* liveSymRef, List<TR::TreeTop>* exitPointsList)
{
    ...
    for (exitBlock = si.getCurrent(); exitBlock; exitBlock = si.getNext()) {
        ...
        // *cudaInfo
        regionExitGPUNode->setAndIncChild(
            0, TR::Node::createWithSymRef(insertionPoint->getNode(), addressLoadOpCode, 0, scopeSymRef));
        ...
        // **liveSymRef
        regionExitGPUNode->setAndIncChild(
            4, TR::Node::createWithSymRef(insertionPoint->getNode(), TR::loadaddr, 0, liveSymRef));
...
bool TR_SPMDKernelParallelizer::addRegionCost(
    TR_RegionStructure* region, TR_RegionStructure* loop, TR::Block* estimateGPUBlock, TR::SymbolReference* lambdaCost)
{
    ...
    if (region != loop && region->isNaturalLoop() && piv && piv->getIncrement() == 1 && piv->getEntryValue()
        ...
        traceMsg(comp(), "adding cost of loop %d with piv %p entry %p exit %p %d %d\n", region->getNumber(), piv,
            piv->getEntryValue(), piv->getExitBound(), loop->isExprInvariant(piv->getEntryValue(), false),
...
bool TR_SPMDKernelParallelizer::estimateGPUCost(
    TR_RegionStructure* loop, TR::Block* estimateBlock, TR::SymbolReference* lambdaCost)
{
...
bool TR_SPMDKernelParallelizer::processGPULoop(TR_RegionStructure* loop, TR_SPMDScopeInfo* gpuScope)
{
   ...
   while (predBlock->getPredecessors().size() == 1)
    ...
    for (TR::TreeTop* tree = comp()->getStartTree(); tree != comp()->findLastTree(); tree = tree->getNextTreeTop()) {
        ...
        if (!visitNodeToDetectArrayAccesses(
                tree->getNode(), loop, piv, visitCount, lineNumber, arrayNotFound, _verboseTrace, postDominates)
    ...
    if (!comp()->getOptions()->getEnableGPU(TR_EnableGPUForce)) {
        ...
        TR::TreeTop::create(comp(), lastTree,
            TR::Node::createStore(
                lambdaCost, TR::Node::create(loopInvariantBlock->getLastRealTreeTop()->getNode(), TR::iconst, 0, 0)));

        TR::TreeTop::create(comp(), lastTree,
            TR::Node::createStore(
                dataCost, TR::Node::create(loopInvariantBlock->getLastRealTreeTop()->getNode(), TR::iconst, 0, 0)));
        ...
        if (!estimateGPUCost(loop, estimateGPUBlock, lambdaCost)) {
            ...
            if (_verboseTrace > 1)
                TR_VerboseLog::writeLine(
                    TR_Vlog_GPU, "\tforEach in %s at line %d does not have any loops", comp()->signature(), lineNumber);
    ...
    int deviceId = 0;
    ptxSource = generatePTX(
        _verboseTrace, programSource, deviceId, comp()->getPersistentInfo(), comp()->trMemory(), enableMath);
    ...
    *kernelLineNumber = lineNumber;
    ...
    for (nc.SetToFirst(); nc.Valid(); nc.SetToNext()) {
        ...
        if (hostSymRef->getSymbol()->isShadow()) {
            ...
            TR::SymbolReference* temp = comp()->getSymRefTab()->createTemporary(
                comp()->getMethodSymbol(), hostSymRef->getSymbol()->getDataType());
    ...
    case scopeSingleKernel:
        ...
        insertGPURegionExits(
            &exitBlocks, gpuScope->_scopeSymRef, gpuPtxCount, liveSymRef, gpuScope->getExitLocationsList());
    ...
    case scopeNaturalLoop:
        if (!gpuScope->_envelopeLoopTransformed) {
            ...
            insertGPURegionExitInRegionExits(
                &envelopeExitBlocks, &blocksInRegion, scopeSymRef, liveSymRef, gpuScope->getExitLocationsList());
            insertGPUTemporariesLivenessCode(gpuScope->getExitLocationsList(), liveSymRef, true);
    ...
    insertGPUKernelLaunch(
        allocSymRef, scopeSymRef, launchBlock, firstNode, piv, loopTestTree, gpuPtxCount, hasExceptionChecks);
    ...
    {
        traceMsg(
            comp(), "Reversing branch in node %p, Global Index %u\n", ibmTryGPUNode, ibmTryGPUNode->getGlobalIndex());
...
bool TR_SPMDKernelParallelizer::visitCPUNode(
    TR::Node* node, int32_t visitCount, TR::Block* block, TR_BitVector* flushGPUBlocks)
{
    if (node->getVisitCount() == visitCount)
        return true;
...
void TR_SPMDKernelParallelizer::collectGPUScopes(
    TR_RegionStructure* region, List<TR_RegionStructure>& gpuKernels, List<TR_SPMDScopeInfo>& gpuScopes)
{
    if (!comp()->getOptions()->getEnableGPU(TR_EnableGPU))
        return;
    ...
    if (getGpuDeviceCount(comp()->getPersistentInfo(), trace) == 0)
        return;
#endif
...
bool TR_SPMDKernelParallelizer::isSPMDKernelLoop(TR_RegionStructure* region, TR::Compilation* comp)
{
    if (region->isNaturalLoop() && region->getPrimaryInductionVariable()) {
        if (!SPMDPreCheck::isSPMDCandidate(comp, region)) {
            if (trace())
                traceMsg(
                    comp, "Natural loop %d has failed SPMD pre-check - skipping consideration\n", region->getNumber());
            return false;
        }
...

        {
            traceMsg(comp, "identified SPMD kernel execution loop in %s block = %d stride = %d symbol = %d\n",
                comp->signature(), branchBlock->getNumber(), piv->getDeltaOnBackEdge(),
                piv->getSymRef()->getReferenceNumber()); // method->nameChars());
            return true;
        }
#endif
    }
...
TR::Node* TR_SPMDKernelParallelizer::findSingleLoopVariant(
    TR::Node* node, TR_RegionStructure* loop, int* sign, int* constantsOnly)
{
    TR::Node* node1 = NULL;
    TR::Node* node2 = NULL;
...
bool TR_SPMDKernelParallelizer::checkConstantDistanceDependence(
    TR_RegionStructure* loop, TR::Node* node1, TR::Node* node2, TR::Compilation* comp, int type)
{
    ...
    if (!(loop->isExprInvariant(node1->getFirstChild()->getFirstChild())
            ...
            && areNodesEquivalent(
                   comp, node1->getFirstChild()->getFirstChild(), node2->getFirstChild()->getFirstChild()))) {
        return false;
    }
    ...
    TR::Compilation* comp)
{
    ...
    for (dc.SetToFirst(); dc.Valid(); dc.SetToNext()) {
        ...
        for (dc2.SetTo(dc + 1); dc2.Valid(); dc2.SetToNext()) {
            if (defs[dc2]->getOpCode().hasSymbolReference()
                ...
                traceMsg(
                    comp, "SPMD DEPENDENCE ANALYSIS: Testing (def %p, def %p) for dependence\n", defs[dc], defs[dc2]);
        ...
        for (uc.SetToFirst(); uc.Valid(); uc.SetToNext()) {
            if (uses[uc]->getOpCode().hasSymbolReference()
                ...
                traceMsg(
                    comp, "SPMD DEPENDENCE ANALYSIS: Testing (def %p, use %p) for dependence\n", defs[dc], uses[uc]);
                if (!loop->isExprInvariant(defs[dc]->getFirstChild())
    ...
    if (!defsOfDefsInLoop.IsZero()) {
        traceMsg(
            comp, "Both a definition from inside the loop and outside the loop reaches a temp used inside the loop.\n");
        return false;
    }
    ...
    TR_HashTab* reductionOperationsHashTab, TR_UseDefInfo* useDefInfo)
{
    ...
    if (isSPMDKernelLoop(region, comp())
        ...
        traceMsg(comp(), "Loop %d and piv = %d collected for Auto-Vectorization\n", region->getNumber(),
            region->getPrimaryInductionVariable()->getSymRef()->getReferenceNumber());
