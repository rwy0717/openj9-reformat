...
TR::Node* getBasePointerReferenceForSeqLoad(
    TR::Compilation* comp, TR::Node* rootNode, int32_t totalBytes, int32_t byteNumber)
{
    TR::Node* dummyNode = rootNode;
    if (byteNumber == 1) {
        for (int32_t i = 0; i < totalBytes; i++) {
...
bool TR_ShiftedValueTree::process(TR::Node* loadNode)
{
    ...
    if (shift) {
        if (loadNode->getFirstChild()->getOpCodeValue() != shiftCode
            ...
            switch (constCode) {
            case TR::cconst:
                _shiftValue = shiftVal->getConst<uint16_t>();
                break;
            case TR::sconst:
    ...
    if ((_shiftValue % 8) != 0 || (_shiftValue < 0) || (_shiftValue >= _valueSize * 8)) {
        dumpOptDetails(
            comp(), "Shifted Value: did not encounter valid shift value %d for var size %d\n", _shiftValue, _valueSize);
        return false;
    }
...
bool TR_arraycopySequentialStores::insertConsistentTree()
{
    ...
    TR::SymbolReference* activeBaseRef = _activeAddrTree->getBaseVarNode()->isNull()
        ? NULL
        : _activeAddrTree->getBaseVarNode()->getChild()->skipConversions()->getSymbolReference();
    ...
    if (_activeValueTree->getRootNode()->referencesSymbolInSubTree(baseRef, _comp->incOrResetVisitCount())) {
        dumpOptDetails(
            comp(), " insertTree: value tree %p references address tree base var\n", _activeValueTree->getRootNode());
        return false;
    }
...
int64_t TR_arraycopySequentialStores::constVal()
{
    ...
    for (int entry = 0; entry < _numBytes; ++entry) {
        ...
        case TR::bconst:
            ...
            break;
        case TR::cconst:
            curVal = (uint64_t)(_val[entry]->getValNode()->getConst<uint16_t>() & 0xFF);
            break;
        case TR::sconst:
...
static TR::TreeTop* generateArraycopyFromSequentialStores(
    TR::Compilation* comp, TR::TreeTop* prevTreeTop, TR::TreeTop* istoreTreeTop, TR::Node* istoreNode)
{
    ...
    if (!arraycopy.hasConstValue() && (loadSize != numBytes)) {
        dumpOptDetails(
            comp, " Sequential Store of size different than trees (%d,%d) not supported yet\n", loadSize, numBytes);
        return istoreTreeTop;
    }
...
static TR::TreeTop* generateArraycopyFromSequentialLoads(
    TR::Compilation* comp, TR::TreeTop* currentTreeTop, TR::Node* ibloadNode)
{
    ...
    TR::TreeTop* istoreTreeTop, TR::Node* istoreNode, bool* newTempsCreated)
{
    ...
    if (areSequentialArrayElements && (totalNumOfStores >= 1)) {
        ...
        if (!codeGen->getSupportsArraySet()) {
            ...
            && numBytes < 32) // tm - The threshold needs to be adjusted for PPC
        {
            // traceMsg(comp, "arrayset istore did not encounter large enough storage range. Range is: %d\n", numBytes);
            return istoreTreeTop;
        } else if ((numBytes < 8)
            || ((numBytes < 12)
                   && TR::Compiler->target.is64Bit())) // msf - change to codeGen->arrayInitMinimumNumberOfBytes())
        {
            // traceMsg(comp, "arrayset istore did not encounter large enough storage range. Range is: %d\n", numBytes);
            return istoreTreeTop;
        }
    ...
    // traceMsg(comp, " First store in sequence %p Load Ref:%p Number of bytes: %d. Offset range:%d to %d. Byte
    // Value:%d\n", istoreNode, arrayset.getALoadRef(), numBytes, arrayset.getBaseOffset(), arrayset.getBaseOffset() +
    // numBytes - 1, arrayset.getConstant());
    ...
    if (usingArrayset) {
        ...
        while (numBytesLeft > 0) {
            ...
            // numBytesToBeStored, totalNumOfStores);
            ...
            case 2: {
                ...
                if (istoreNode->getOpCodeValue() == TR::bstorei || istoreNode->getOpCodeValue() == TR::bustorei) {
            ...
            case 4: {
                ...
                } else if ((istoreNode->getOpCodeValue() == TR::cstorei)
                    || (istoreNode->getOpCodeValue() == TR::sstorei)) {
            ...
            case 8: {
                ...
                if (istoreNode->getOpCodeValue() == TR::bstorei || istoreNode->getOpCodeValue() == TR::bustorei) {
                    ...
                    longConstValue
                        = ((longConstValue << 48) | (longConstValue << 32) | (longConstValue << 16) | longConstValue);
            ...
            if (!disableHoist && comp->getJittedMethodSymbol() && // avoid NULL pointer on non-Wcode builds
                !comp->getJittedMethodSymbol()->isNoTemps() && comp->cg()->isMaterialized(constValueNode)) {
...
bool TR_ArrayShiftTreeCollection::insertTree(TR::TreeTop* currTree)
{
    ...
    if (_storeTrees[_numTrees]->process()) {
        if (_storeTrees[_numTrees]->getTargetIndexVarNode() == NULL
            ...
            dumpOptDetails(
                comp(), "Store tree [%p] has null index/array node\n", _storeTrees[_numTrees]->getRootNode());
            return false;
        }
        ...
        if (_storeTrees[_numTrees]->getTargetIndexVarNode() != _storeTrees[_numTrees]->getSourceIndexVarNode()
            ...
            dumpOptDetails(
                comp(), "Store tree[%p] with different index/array nodes\n", _storeTrees[_numTrees]->getRootNode());
            return false;
        }
        for (int32_t i = 0; i < _numTrees; ++i) {
        ...
        if (_storeTrees[_numTrees]->getStoreOpCode() != _storeTrees[0]->getStoreOpCode()
            ...
            dumpOptDetails(
                comp(), "Store tree[%p] with different store/load opcode\n", _storeTrees[_numTrees]->getRootNode());
            return false;
        }
...
static TR::TreeTop* reduceArrayLoad(
    TR_ArrayShiftTreeCollection* storeTrees, TR::Compilation* comp, TR::TreeTop* oldTreeTop)
{
    ...
    if (storeTrees->getNumTrees() > 1) {
        ...
        TR::Node* newStoreNode = TR::Node::createWithSymRef(
            storeOpCode, 2, 2, storeTrees->getTree(0)->getTargetAddress()->getRootNode(), sourceNode, symRef);
...
static TR::TreeTop* generateArrayshiftFromSequentialStores(
    TR::Compilation* comp, TR::TreeTop* prevTreeTop, TR::TreeTop* istoreTreeTop)
{
        ...
        while (currentTree) {
...

            {
                if (firstNodeInTree->getOpCode().isStore() && firstNodeInTree->getOpCode().isIndirect()) {
                    currentTree = generateArraysetFromSequentialStores(
                        comp(), prevTree, currentTree, firstNodeInTree, &newTempsCreated);
                ...
                while ((currentNode->getNumChildren() >= 1) && (currentNode->getFirstChild()->getNumChildren() >= 1)) {
                    ...
                    if (currentNode->getOpCodeValue() == TR::bu2i && currentNode->getFirstChild()->getOpCode().isLoad()
                        ...
                        currentTree
                            = generateArraycopyFromSequentialLoads(comp(), currentTree, currentNode->getFirstChild());
                    }
                }
            }
