...
bool shouldEnableSEL(TR::Compilation* comp)
{
    ...
    return ((enableSEL != NULL) && TR::Compiler->target.is64Bit());
}
...
{
    if (false)
        traceMsg(comp(), "emptying the hash table\n");
    ...
    TR_ScratchList<TR::Node>& useri2lList, bool isIndexSubtree)
{
    ...
    if (node->getVisitCount() != curVisit) {
        ...
        if (isi2lNode) {
            ...
            if (trace())
                traceMsg(comp(), "Found i2l %p, parent %p, is%s an aladd child\n", node, parent,
                    isIndexSubtree ? "" : " not");
            if (isIndexSubtree)
        ...
        for (int i = 0; i < node->getNumChildren(); i++) {
            ...
            case TR::isub:
                ...
                if (trace())
                    traceMsg(comp(), "node %p has %d references\n", child, child->getReferenceCount());
            default:
                break;
            }
            ...
            if (parentNode->getChild(j) == targetNode) {
                ...
                if (!parentNode->getOpCode().isLong() || parentNode->getOpCode().isCall()) {
                    ...
                    if (!performTransformation(
                            comp(), "%sInserting l2i node %p for %p\n", OPT_DETAILS, newL2INode, targetNode))
                        return;
                    ...
                    for (i2lParent = listElement.getFirst(); i2lParent != NULL; i2lParent = listElement.getNext()) {
                        ...
                        for (int32_t j = 0; j < numKids; ++j) {
                            if (i2lParent->getChild(j) == parentNode) {
                                if (trace())
                                    traceMsg(comp(), "Remove i2l node %p from %p, ->%p\n", parentNode, i2lParent,
                                        targetNode);
                                if (++refCnt > 1)
    ...
    for (parentNode = listElement.getFirst(); parentNode != NULL; parentNode = listElement.getNext()) {
        ...
        for (int32_t j = 0; j < numKids; ++j) {
            ...
            if (parentNode->getChild(j) == targetNode && !skipChild) {
                ...
                if (trace())
                    traceMsg(comp(), "Updated %p to point to %p\n", parentNode, newI2LNode);
                break; // if this parent has multiple references to this node, it will be seen again
            }
        }
    }
...
bool TR_SignExtendLoads::ConvertSubTreeToLong(TR::Node* parent, TR::Node* node, bool changeNode)
{
        ...
        if (changeNode && parent->getOpCode().isLong() /* && node->getReferenceCount()== 1 */) {
            ...
            for (int32_t j = 0; j < numKids; ++j) {
                if (parent->getChild(j) == node) {
                    ...
                    if (trace())
                        traceMsg(comp(), "Get rid of l2i %p of %p\n", node, parent);
                    break;
                }
            }
        }
    ...
    case TR::iconst: {
        ...
        if (changeNode) {
            ...
            if (!performTransformation(
                    comp(), "%sReplace %p iconst->%p lconst(%d)\n", OPT_DETAILS, node, newNode, constVal))
                return false;
    ...
    case TR::iload:
        ...
        if (trace())
            traceMsg(comp(), "inspecting load/i2l etc %p\n", node);
        if (changeNode) {
        ...
        if (trace())
            traceMsg(comp(), "...ok iload etc\n");
        return true;

    case TR::isub:
        if (trace())
            traceMsg(comp(), "inspecting isub %p\n", node);
        ...
        if (changeNode) {
            if (trace())
                traceMsg(comp(), "Converting isub %p\n", node);
    ...
    case TR::iadd:
        if (trace())
            traceMsg(comp(), "inspecting iadd %p\n", node);
        if (!node->cannotOverflow())
            return false;
...
void TR_SignExtendLoads::ReplaceI2LNode(TR::Node* i2lNode, TR::Node* newNode)
{
    ...
    for (TR::Node* parentNode = listElement.getFirst(); parentNode != NULL; parentNode = listElement.getNext()) {
        int32_t numKids = parentNode->getNumChildren();
        if (trace())
            traceMsg(comp(), "looking at parent %p of %p\n", parentNode, i2lNode);
        for (int32_t j = 0; j < numKids; ++j) {
            if (parentNode->getChild(j) == i2lNode) {
                parentNode->setChild(j, newNode);
                if (trace())
                    traceMsg(comp(), "updated i2l parent %p point to %p\n", parentNode, newNode);
                ++newRefs;
                if (newRefs > 1)
                    newNode->incReferenceCount();

                if (!performTransformation(
                        comp(), "%sUpdating reference to node %p with %p\n", OPT_DETAILS, i2lNode, newNode))
                    return;
            }
        }
    }
        ...
        for (int i = 0; i < parentNode->getNumChildren(); i++) {
            ...
            if (child->getOpCodeValue() == TR::i2l) {
                if (child->getReferenceCount() > 1 && (NULL == getListFromHash(child))) {
                    if (trace())
                        traceMsg(comp(), "Already processed parent %p--skipping %p\n", parentNode, child);
                    continue;
                }
                if (trace())
                    traceMsg(comp(), "Processing i2l node %p (parent:%p)\n", child, parentNode);
                ...
                case TR::iload:
                    if (i2lchild->getReferenceCount() < 2) {
                        if (trace())
                            traceMsg(comp(), "i2l(%p):iload not shared--skip\n", child);
                ...
                case TR::iadd:
                    if (!isAddressCalc)
                        continue;
                    if (trace())
                        traceMsg(comp(), "child of %p is add/sub\n", i2lchild);
                ...
                default:
                    ...
                    {
                        if (!performTransformation(
                                comp(), "%sRemoving i2l node %p from parent %p\n", OPT_DETAILS, child, parentNode))
                            break;
