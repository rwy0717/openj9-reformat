                    ...
                    if (findStringBuilderChainedAppendArguments(iter, currentNode, appendArguments) != NULL) {
                        ...
                        if (performTransformation(comp(),
                            ...
                            if (collectAppendStatistics || collectAllocationStatistics || collectAllocationBacktraces
                                ...
                                initNode->setAndIncChild(
                                    2, TR::Node::iconst(static_cast<int32_t>(collectAppendStatistics)));
                                initNode->setAndIncChild(
                                    3, TR::Node::iconst(static_cast<int32_t>(collectAllocationStatistics)));
                                initNode->setAndIncChild(
                                    4, TR::Node::iconst(static_cast<int32_t>(collectAllocationBacktraces)));
                                initNode->setAndIncChild(
                                    5, TR::Node::iconst(static_cast<int32_t>(collectAppendObjectTypes)));
                            } else {
                                TR::SymbolReference* newInitSymRef
                                    = getSymRefTab()->methodSymRefFromName(comp()->getMethodSymbol(),
                            ...
                            TR::DebugCounter::incStaticDebugCounter(comp(),
                                TR::DebugCounter::debugCounterName(
                                    comp(), "StringBuilderTransformer/Succeeded/%d/%s", capacity, comp()->signature()));
                        }
                    }
                }
            }
...
TR::Node* TR_StringBuilderTransformer::findStringBuilderInit(TR::TreeTopIterator iter, TR::Node* newNode)
{
    ...
    TR::DebugCounter::incStaticDebugCounter(comp(),
        TR::DebugCounter::debugCounterName(
            comp(), "StringBuilderTransformer/Failed/CouldNotLocateInit/%s", comp()->signature()));
 ...
 */
TR::Node* TR_StringBuilderTransformer::findStringBuilderChainedAppendArguments(
    TR::TreeTopIterator iter, TR::Node* newNode, List<TR_Pair<TR::Node*, TR::RecognizedMethod> >& appendArguments)
{
            ...
            if (acallNode->getOpCodeValue() == TR::acall && acallNode->getFirstChild() == stringBuilderReceiver) {
                ...
                if (acallSymbol != NULL) {
                    ...
                    case TR::java_lang_StringBuilder_append_Object: {
                        // The reference count for the new call and the flag foundOSRReference for the prior call are
                        // checked here
                        if (acallNode->getReferenceCount() == expectedReferenceCount && foundOSRReference) {
                            ...
                            appendArguments.add(new (trHeapMemory()) TR_Pair<TR::Node*, TR::RecognizedMethod>(
                                acallNode->getSecondChild(), recogniedMethod));
                            ...
                            TR::DebugCounter::incStaticDebugCounter(comp(),
                                TR::DebugCounter::debugCounterName(comp(),
                                    "StringBuilderTransformer/Failed/InvalidReferenceCount/%s", comp()->signature()));
                    ...
                    case TR::java_lang_StringBuilder_toString: {
                        ...
                        if (foundOSRReference) {
                            if (trace()) {
                                traceMsg(
                                    comp(), "[0x%p] Found java/lang/StringBuilder.toString acall node.\n", acallNode);
                            }
                    ...
                    default: {
                        if (trace()) {
                            traceMsg(
                                comp(), "[0x%p] java/lang/StringBuilder.append chain broken at node.\n", acallNode);
                        }

                        TR::DebugCounter::incStaticDebugCounter(comp(),
                            TR::DebugCounter::debugCounterName(
                                comp(), "StringBuilderTransformer/Failed/AppendChainBroken/%s", comp()->signature()));
                    ...
                    TR::DebugCounter::incStaticDebugCounter(comp(),
                        TR::DebugCounter::debugCounterName(
                            comp(), "StringBuilderTransformer/Failed/UnresolvedACall/%s", comp()->signature()));
    ...
    TR::DebugCounter::incStaticDebugCounter(comp(),
        TR::DebugCounter::debugCounterName(
            comp(), "StringBuilderTransformer/Failed/ToStringNotFound/%s", comp()->signature()));
    ...
    for (TR_Pair<TR::Node*, TR::RecognizedMethod>* pair = iter.getFirst(); pair != NULL; pair = iter.getNext()) {
        ...
        case TR::java_lang_StringBuilder_append_double: {
            if (argument->getOpCodeValue() == TR::dconst) {
                ...
                // The C Standard Library guarantees the printed array to be null-terminated except on certain versions
                // of MSVC++
