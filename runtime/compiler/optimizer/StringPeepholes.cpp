...
TR::SymbolReference* TR_StringPeepholes::MethodEnumToArgsForMethodSymRefFromName(StringpeepholesMethods m)
{
    ...
    TR_ASSERT(m != START_BIG_DECIMAL_HELPER_METHODS && m != START_STRING_METHODS && m != END_STRINGPEEPHOLES_METHODS,
        "wrong constant!!");

    static char* classNames[] = { "java/math/BigDecimal", "java/math/BigDecimal", "java/math/BigDecimal",
        "java/math/BigDecimal", NULL, "java/lang/String", "java/lang/String", "java/lang/String", "java/lang/String",
        "java/lang/String", "java/lang/String",
        NULL, // separator
        "com/ibm/jit/DecimalFormatHelper", "com/ibm/jit/DecimalFormatHelper" };

    static char* methodNames[] = { "SMAAMSS", "SMSS", "AAMSS", "MSS", NULL, "<init>", "<init>", "<init>", "<init>",
        "<init>", "<init>", NULL, "formatAsDouble", "formatAsFloat" };

    static char* signatures[] = { "(Ljava/math/BigDecimal;Ljava/math/BigDecimal;Ljava/math/BigDecimal;Ljava/math/"
        ...
        "(Ljava/lang/String;I)V", "([BIIZ)V", "(ILjava/lang/String;ILjava/lang/String;Ljava/lang/String;)V", NULL,
    ...
    if (m == SPH_String_init_AIIZ) {
        if (!fe()->getMethodFromName(classNames[m], methodNames[m], signatures[m],
            ...
            return comp()->getSymRefTab()->methodSymRefFromName(
                comp()->getMethodSymbol(), classNames[m], methodNames[m], "([CIIZ)V", TR::MethodSymbol::Special);
        }
    }

    if (strlen(methodNames[m]) == 6 && !strncmp(methodNames[m], "<init>", 6))
        return comp()->getSymRefTab()->methodSymRefFromName(
            comp()->getMethodSymbol(), classNames[m], methodNames[m], signatures[m], TR::MethodSymbol::Special);

    return comp()->getSymRefTab()->methodSymRefFromName(
        comp()->getMethodSymbol(), classNames[m], methodNames[m], signatures[m], TR::MethodSymbol::Static);
}
...
void TR_StringPeepholes::processBlock(TR::Block* block)
{
    ...
    for (TR::TreeTop* tt = block->getEntry(); tt != exit; tt = tt->getNextRealTreeTop()) {
        ...
        if (!_stringClassesRedefined && (node->getOpCodeValue() == TR::New)) {
                    ...
                    if (strncmp(m->signatureChars(), "([BII)", 6) == 0
                        ...
                        if (performTransformation(comp(),
                            ...
                            TR::Node::recreateWithoutProperties(
                                callNode, TR::call, 5, findSymRefForOptMethod(SPH_String_init_AIIZ));
            ...
            if (comp()->isOutermostMethod() && !optimizer()->isIlGenOpt()
                && !comp()->getOption(TR_DisableDecimalFormatPeephole)) {
...
TR_BDChain* TR_StringPeepholes::detectChain(
    TR::RecognizedMethod recognizedMethod, TR::TreeTop* cursorTree, TR::Node* cursorNode, TR_BDChain* prevInChain)
{
        ...
        if (cursorChain->_recognizedMethod == pattern[curLen]) {
            ...
            if ((cursorChain->_recognizedMethod == TR::java_math_BigDecimal_add)
                ...
                if (comp->getMethodHotness() != hot) {
                    ...
                    // traceMsg(comp, "Matched a BigDecimal (binary op) method in a chain in method %s with valueInfo
                    ...
                    if (valueInfo1 && (valueInfo1->getTopProbability() == 1.0f) && valueInfo2
                        ...
                        if ((curScale1 >= 0) && (flag1 == 1) && (curScale2 >= 0) && (flag2 == 1) && (resultScale >= 0)
                            ...
                            if (trace) {
                                traceMsg(comp,
                                    "Matched a BigDecimal (binary op) method with profile info in a chain with scale1 "
                                    "= %d scale2 = %d result scale = %d\n",
                                    curScale1, curScale2, resultScale);
                                printf("Matched a BigDecimal (binary op) method with profile info in a chain in method "
                                       "%s with scales = %d and %d result scale = %d\n",
    ...
    int32_t& matchedPattern, bool trace, bool& obtainProfilingInfo)
{
    ...
    while (i < TR_StringPeepholes::numBDPatterns) {
        ...
        TR_BDChain* matchedChain = matchBDPattern(
            chain, bdPatterns[i], bdPatternLengths[i], profileManager, comp, trace, obtainProfilingInfo);
        if (matchedChain) {
            matchedPattern = i;
            return matchedChain;
        }
...
TR::TreeTop* TR_StringPeepholes::detectFormatPattern(TR::TreeTop* tt, TR::TreeTop* exit, TR::Node* node)
{
    ...
    if ((isRecognizedMethod(node, TR::java_math_BigDecimal_doubleValue)
        ...
        if (!nextNode->getOpCode().isCall() && (nextNode->getNumChildren() > 0))
            nextNode = nextNode->getFirstChild();
        ...
        if (isRecognizedMethod(nextNode, TR::java_text_NumberFormat_format)) {
            ...
            if (nextNode->getSecondChild() == node && callSymRef
                && performTransformation(comp(), "%ssimplified number format pattern from node [%p] to [%p] \n",
                ...
                nextNode->setAndIncChild(
                    1, node->getOpCode().isIndirect() ? node->getSecondChild() : node->getFirstChild());
                ...
                if (needsNullCheck && node->getOpCode().isCall()) {
                    ...
                    TR::Node* nullChkNode = TR::Node::createWithSymRef(node, TR::NULLCHK, 1, arg,
                ...
                if (performTransformation(
                        comp(), "%sAttempting to inline call [%p]\n", optDetailString(), tt->getNode())) {
                    ...
                    if (inlineOK
                        && performTransformation(comp(),
                               "%sdf.format(bd.doubleValue()) (or df.format(bd.floatValue()) ) has been optimized\n",
                               optDetailString())) {
                        traceMsg(
                            comp(), "%sInlining success at call tree %p\n", optDetailString(), firstTree->getNode());
...
TR::TreeTop* TR_StringPeepholes::detectSubMulSetScalePattern(TR::TreeTop* tt, TR::TreeTop* exit, TR::Node* node)
{
            ...
            if (cursorNode->getOpCodeValue() == TR::acalli) {
                ...
                if (cursorNode->getOpCodeValue() == TR::acalli) {
                    ...
                    if (mulNode != cursorNode->getChild(cursorNode->getFirstArgumentIndex())) {
                        if (trace())
                            traceMsg(
                                comp(), "setScaleNode's %p receiver is not multiplyNode %p\n", cursorNode, mulNode);
                        continue;
                    }
    ...
    if (foundPattern) {
        ...
        if (comp()->getMethodHotness() > warm && comp()->getMethodHotness() != hot) {
            ...
            if (valueInfo1 && (valueInfo1->getTopProbability() == 1.0f) && valueInfo2
                ...
                if ((mulScale != 2) || (flag2 != 1) || (subScale < 0) || (subScale > 6) || (flag2 != 1)) {
                    if (trace())
                        traceMsg(comp(),
                            "Failed on profile info from %p for subtract multiply and setscale pattern with sub scale "
                            "%d, and mul scale %d\n",
            ...
            if (comp()->getMethodHotness() == hot && comp()->getRecompilationInfo()
                && performTransformation(comp(),
                       "%smight have simplified big decimal subtract multiply setscale pattern from node [%p] to [%p] "
                       "if profiling info was available. Switching to profiling.\n",
        ...
        if (trace()) {
            traceMsg(comp(),
                "Successfully obtained profile info from %p for subtract multiply and setscale pattern with sub scale "
                "%d, and mul scale %d\n",
                TR_PersistentProfileInfo::get(comp()), subScale, mulScale);
        }
...
TR::TreeTop* TR_StringPeepholes::detectBDPattern(TR::TreeTop* tt, TR::TreeTop* exit, TR::Node* node)
{
    ...
    TR_BDChain* firstInChain = NULL;
    if ((node->getOpCodeValue() == TR::acall) && !node->getSymbolReference()->isUnresolved()
    ...
    if (matchedChain) {
                    ...
                    if (matchedPattern == 0 || matchedPattern == 2) {
                        if ((add2Node->getChild(add2Node->getNumChildren() - 2) != add1Node)
                            || ((add2Node->getOpCode().isCallIndirect() && (add1Node->getReferenceCount() != 3))
                                   || (!add2Node->getOpCode().isCallIndirect()
                                          && (add1Node->getReferenceCount() != 2)))) {
                            if (trace())
                ...
                if (matchedPattern <= 1 && chainPos == 2) {
                    if ((mulNode->getChild(mulNode->getNumChildren() - 1) != subtractNode)
                        || ((mulNode->getOpCode().isCallIndirect() && (subtractNode->getReferenceCount() > 3))
                               || (!mulNode->getOpCode().isCallIndirect()
                                      && (subtractNode->getReferenceCount() > 2)))) {
                        if (trace())
                ...
                if (matchedPattern <= 3) {
                    if ((setScaleNode->getChild(setScaleNode->getNumChildren() - 3) != mulNode)
                        || ((setScaleNode->getOpCode().isCallIndirect() && (mulNode->getReferenceCount() != 3))
                               || (!setScaleNode->getOpCode().isCallIndirect()
                                      && (mulNode->getReferenceCount() != 2)))) {
                        if (trace())
        ...
        if (treesAsExpected && obtainProfilingInfo) {
            if (comp()->getMethodHotness() == hot && comp()->getRecompilationInfo()
                && performTransformation(comp(),
                       "%smight have simplified big decimal pattern from node [%p] to [%p] pattern=%d if profiling "
                       "info was available. Switching to profiling.\n",
            ...
            callSymRef = callSymRef
                ...
                : 0;
            if (matchedPattern == 0) {
                methodCall = TR::Node::createWithSymRef(
                    TR::acall, 9, 4, valueOf1Node, subChild, mul1Child, add1Child, callSymRef);
                ...
                methodCall = TR::Node::createWithSymRef(
                    TR::acall, 7, 4, valueOf1Node, add1Child, add2Child, mul1Child, callSymRef);
                    ...
                    if (!disableBDPrefetch && sourceNode && comp()->getMethodHotness() >= scorching
                        ...
                        for (TR::TreeTop* tt = comp()->getStartTree(); tt; tt = tt->getNextTreeTop()) {
                            ...
                            if (targetNode->getOpCode().hasSymbolReference()
                                ...
                                const char* fieldName = symRef->getOwningMethod(comp())->fieldName(
                                    symRef->getCPIndex(), len, comp()->trMemory());
                                ...
                                if (strstr(fieldName, "Ljava/math/BigDecimal") && scaleOffset > 0) {
                                    if (comp()->findPrefetchInfo(targetNode) < 0) {
                                        ...
                                        // fieldName, scaleOffset, targetNode);
                                    }
                                }
                                break;
                            }
    ...
    if (stringCount == 2) {
        if (specialCase) {
            if (pattern[1] == 'C') {
                TR::SymbolReference* initSymRef = findSymRefForOptMethod(SPH_String_init_SC)
                    ...
                    : 0;
                initCall = TR::Node::createWithSymRef(
                    TR::call, 3, 3, stringNode, appendedString[0], appendedString[1], initSymRef);
            } else {
                TR::SymbolReference* initSymRef3 = findSymRefForOptMethod(SPH_String_init_SI)
                    ...
                    : 0;
                initCall = TR::Node::createWithSymRef(
                    TR::call, 3, 3, stringNode, appendedString[0], appendedString[1], initSymRef3);
            }
        } else // use the string + string constructor
        {
            TR::SymbolReference* initSymRef1 = findSymRefForOptMethod(SPH_String_init_SS)
                ...
                : 0;
            initCall = TR::Node::createWithSymRef(
                TR::call, 3, 3, stringNode, appendedString[0], appendedString[1], initSymRef1);
        }
        ...
        TR::SymbolReference* initSymRef2 = findSymRefForOptMethod(SPH_String_init_SSS)
            ...
            : 0;
        initCall = TR::Node::createWithSymRef(
            TR::call, 4, 4, stringNode, appendedString[0], appendedString[1], appendedString[2], initSymRef2);
