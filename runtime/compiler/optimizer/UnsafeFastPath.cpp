...
static TR::SymbolReferenceTable::CommonNonhelperSymbol equivalentAtomicIntrinsic(TR::RecognizedMethod rm)
{
    ...
    case TR::sun_misc_Unsafe_getAndSetLong:
        return TR::Compiler->target.is64Bit() ? TR::SymbolReferenceTable::atomicSwapSymbol
    ...
    case TR::sun_misc_Unsafe_getAndAddLong:
        return TR::Compiler->target.is64Bit() ? TR::SymbolReferenceTable::atomicFetchAndAddSymbol
                                              : TR::SymbolReferenceTable::lastCommonNonhelperSymbol;
    default:
        break;
    }
 ...
 *
 */
bool TR_UnsafeFastPath::tryTransformUnsafeAtomicCallInVarHandleAccessMethod(
    TR::TreeTop* callTree, TR::RecognizedMethod callerMethod, TR::RecognizedMethod calleeMethod)
{
    ...
    if (!performTransformation(
            comp(), "%s turning the call [" POINTER_PRINTF_FORMAT "] into atomic intrinsic\n", optDetailString(), node))
        return false;
    ...
    if (callerMethod == TR::java_lang_invoke_StaticFieldVarHandle_StaticFieldVarHandleOperations_OpMethod) {
        ...
        TR::Node* j9Class = TR::Node::createWithSymRef(
            node, TR::aloadi, 1, jlClass, comp()->getSymRefTab()->findOrCreateClassFromJavaLangClassSymbolRef());
        TR::Node* ramStatics = TR::Node::createWithSymRef(
            node, TR::aloadi, 1, j9Class, comp()->getSymRefTab()->findOrCreateRamStaticsFromClassSymbolRef());
        ...
        unsafeAddress = TR::Compiler->target.is32Bit()
        ...
        TR::Node* offset = node->getChild(2);
        unsafeAddress = TR::Compiler->target.is32Bit()
            ? TR::Node::create(node, TR::aiadd, 2, object, TR::Node::create(node, TR::l2i, 1, offset))
    ...
    if (trace()) {
        traceMsg(
            comp(), "Transformed the call %p n%dn to codegen inlineable intrinsic\n", node, node->getGlobalIndex());
    }
        ...
        if (node && node->getOpCode().isCall() && !node->getSymbol()->castToMethodSymbol()->isHelper()) {
            ...
            if (!transformed.contains(node)) {
                ...
                if (TR_J9MethodBase::isVarHandleOperationMethod(caller)
                    && (isTransformableUnsafeAtomic(callee) || symbol->getMethod()->isUnsafeCAS(comp()))) {
                ...
                if (TR::Compiler->target.is64Bit()) {
                    addrCalc = TR::Node::create(TR::aladd, 2, charArray,
                        TR::Node::create(TR::ladd, 2,
                            TR::Node::create(
                                TR::lmul, 2, TR::Node::create(TR::i2l, 1, index), TR::Node::lconst(index, 4)),
            ...
            if (TR_J9MethodBase::isVarHandleOperationMethod(callerMethod)
                ...
                if (trace())
                    traceMsg(comp(),
                        "VarHandle operation: isArrayOperation %d type %s value %p isVolatile %d on node %p\n",
            ...
            if (type != TR::NoType
                && performTransformation(comp(),
                       ...
                       optDetailString(), node)) {
                TR::SymbolReference* unsafeSymRef
                    = comp()->getSymRefTab()->findOrCreateUnsafeSymbolRef(type, true, isStatic, isVolatile);
                ...
                if (isStatic) {
                    ...
                    TR::Node* ramStatics = TR::Node::createWithSymRef(
                        TR::aloadi, 1, 1, j9Class, comp()->getSymRefTab()->findOrCreateRamStaticsFromClassSymbolRef());
                    ...
                    TR::Node* newOffset = TR::Node::create(offset, TR::land, 2, offset, TR::Node::lconst(offset, ~1));
                ...
                if (mightBeArraylets) {
                    ...
                    if (value) {
                        ...
                        if (type == TR::Address && (comp()->getOptions()->getGcMode() != TR_WrtbarNone)) {
                            node = TR::Node::recreateWithoutProperties(
                                node, TR::awrtbari, 3, addrCalc, value, object, unsafeSymRef);
                            spineCHK->setAndIncChild(0, addrCalc);
                        } else {
                            if (value->getDataType() != type) {
                                TR::ILOpCodes conversionOpCode = TR::ILOpCode::getProperConversion(
                                    value->getDataType(), type, needUnsignedConversion(symbol->getRecognizedMethod()));
                            ...
                            node = TR::Node::recreateWithoutProperties(
                                node, comp()->il.opCodeForIndirectArrayStore(type), 2, addrCalc, value, unsafeSymRef);
                        ...
                        if (node->getDataType() != type) {
                            TR::ILOpCodes conversionOpCode = TR::ILOpCode::getProperConversion(
                                type, node->getDataType(), needUnsignedConversion(symbol->getRecognizedMethod()));
                            ...
                            TR::Node* load = TR::Node::createWithSymRef(
                                node, comp()->il.opCodeForIndirectArrayLoad(type), 1, unsafeSymRef);
                            ...
                            node = TR::Node::recreateWithoutProperties(
                                node, comp()->il.opCodeForIndirectArrayLoad(type), 1, addrCalc, unsafeSymRef);
                    ...
                    if (value) {
                        ...
                        if (type == TR::Address && (comp()->getOptions()->getGcMode() != TR_WrtbarNone))
                            ...
                            node = TR::Node::recreateWithoutProperties(
                                node, opCodeForIndirectStore, 2, addrCalc, value, unsafeSymRef);
                        }
                        ...
                        if (node->getDataType() != type) {
                            ...
                            node = TR::Node::recreateWithoutProperties(
                                node, opCodeForIndirectLoad, 1, addrCalc, unsafeSymRef);
                        }
