...
TR::VP_BCDValue::VP_BCDValue(
    char* lit, int32_t litSize, char* str, TR_BCDSignConstraint sign, TR::DataType dt, OMR::ValuePropagation* vp)
...
TR::VPConstraint* TR::VP_BCDSign::merge1(TR::VPConstraint* other, OMR::ValuePropagation* vp)
{
    ...
    if (asBCDSign() && other->asBCDSign()) {
        ...
        if (mergedSign != TR_Sign_Unknown) {
            ...
            if (vp->trace()) {
                traceMsg(vp->comp(), "\treturn mergedSignConstraint %p of sign %s: ", mergedSignConstraint,
