...
static X VarHandleMethods[] = {
    // We only get the original method name when the call is unresolved, but recognized method only works for resovled
    // method
    ...
    for (TR::TreeTop* tt = methodSymbol->getFirstTreeTop(); tt != NULL; tt = tt->getNextTreeTop()) {
        ...
        if (node && node->getOpCode().isCall() && !node->getSymbol()->castToMethodSymbol()->isHelper()) {
            ...
            if (varHandleAccessMethod != TR::unknownMethod) {
                ...
                if (callTree->getNode()->getOpCode().isCheck()) {
                    ...
                    if (hasNULLCHK) {
                        ...
                        TR::Node* checkNode = TR::Node::createWithSymRef(
                            TR::NULLCHK, 1, 1, passthrough, callTree->getNode()->getSymbolReference());
                ...
                dumpOptDetails(
                    comp(), "%sVarHandle access methods found, working on node %p\n", optDetailString(), node);
                ...
                TR::Node* handleTable = TR::Node::createWithSymRef(
                    comp()->il.opCodeForIndirectLoad(TR::Address), 1, 1, varHandle, handleTableSymRef);
                ...
                TR::SymbolReference* typeConversionSymRef = comp()->getSymRefTab()->methodSymRefFromName(
                    methodSymbol, JSR292_MethodHandle, JSR292_asType, JSR292_asTypeSig, OMR::MethodSymbol::Static);
                TR::Node* convertedMethodHandle = TR::Node::createWithSymRef(
                    TR::acall, 2, 2, methodHandle, callSiteMethodType, typeConversionSymRef);
                ...
                TR::Node* invokeCall = TR::Node::recreateWithoutProperties(
                    node, method->indirectCallOpCode(), numChildren, invokeExactSymRef);
