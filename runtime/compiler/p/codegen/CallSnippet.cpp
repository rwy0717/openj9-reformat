...
uint8_t* TR::PPCCallSnippet::setUpArgumentsInRegister(
...
uint8_t* TR::PPCCallSnippet::emitSnippetBody()
{
    ...
    TR_RuntimeHelper runtimeHelper = getInterpretedDispatchHelper(
        methodSymRef, callNode->getDataType(), methodSymbol->isSynchronised(), isNativeStatic, cg());
            ...
            *(intptrj_t*)cursor = (intptrj_t)methodSymbol->getMethodAddress();
            ...
            cg()->addExternalRelocation(
                new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor, (uint8_t*)methodSymRef,
                    getNode() ? (uint8_t*)getNode()->getInlinedSiteIndex() : (uint8_t*)-1, TR_MethodObject, cg()),
                __FILE__, __LINE__, callNode);
        }
    }
    cursor += TR::Compiler->om.sizeofReferenceAddress();
    ...
    *(intptrj_t*)cursor = (intptrj_t)methodSymRef->getOwningMethod(comp)->constantPool();
    ...
    cg()->addExternalRelocation(
        new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor, *(uint8_t**)cursor,
            getNode() ? (uint8_t*)getNode()->getInlinedSiteIndex() : (uint8_t*)-1, TR_Trampolines, cg()),
        __FILE__, __LINE__, getNode());
...
uint8_t* TR::PPCVirtualUnresolvedSnippet::emitSnippetBody()
{
    ...
    TR::Node* callNode = getNode();
    TR::SymbolReference* glueRef
        = cg()->symRefTab()->findOrCreateRuntimeHelper(TR_PPCvirtualUnresolvedHelper, false, false, false);
    intptrj_t distance;
    void* thunk
    ...
    *(intptrj_t*)cursor = (intptrj_t)thunk;
    ...
    cg()->addExternalRelocation(new (cg()->trHeapMemory()) TR::ExternalRelocation(
                                    j2iThunkRelocationPoint, (uint8_t*)info, NULL, TR_J2IVirtualThunkPointer, cg()),
        __FILE__, __LINE__, callNode);
...
uint8_t* TR::PPCInterfaceCallSnippet::emitSnippetBody()
{
    ...
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(comp->fe());
    TR::SymbolReference* glueRef
        = cg()->symRefTab()->findOrCreateRuntimeHelper(TR_PPCinterfaceCallHelper, false, false, false);
    intptrj_t distance;
    void* thunk
        = fej9->getJ2IThunk(callNode->getSymbolReference()->getSymbol()->castToMethodSymbol()->getMethod(), comp);
    uint8_t* j2iThunkRelocationPoint;
    ...
    if (TR::Compiler->target.is64Bit()) {
        if (getTOCOffset() != PTOC_FULL_INDEX) {
            ...
            if (!comp->compileRelocatableCode()) {
                // If the high nibble is 0 and the next nibble's high bit is clear, change the first instruction to a
                // nop and the third to a li Next nibble's high bit needs to be clear in order to use li (because li
                // will sign extend the immediate)
    ...
    *(intptrj_t*)(cursor + 3 * TR::Compiler->om.sizeofReferenceAddress()) = (intptrj_t)blAddress;
    ...
    cg()->addExternalRelocation(new (cg()->trHeapMemory())
                                    TR::ExternalRelocation(cursor + TR::Compiler->om.sizeofReferenceAddress(), NULL,
                                        TR_AbsoluteMethodAddress, cg()),
        __FILE__, __LINE__, callNode);

    // Register for relaction of the 2nd target address
    cg()->addExternalRelocation(new (cg()->trHeapMemory())
                                    TR::ExternalRelocation(cursor + 3 * TR::Compiler->om.sizeofReferenceAddress(), NULL,
                                        TR_AbsoluteMethodAddress, cg()),
        __FILE__, __LINE__, callNode);
...
uint8_t* TR::PPCCallSnippet::generateVIThunk(TR::Node* callNode, int32_t argSize, TR::CodeGenerator* cg)
{
    TR::Compilation* comp = cg->comp();
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(comp->fe());
    int32_t codeSize = 4 * (instructionCountForArguments(callNode, cg) + (TR::Compiler->target.is64Bit() ? 7 : 4))
    ...
    case TR::NoType:
        dispatcher = (intptrj_t)cg->symRefTab()
                         ->findOrCreateRuntimeHelper(TR_PPCicallVMprJavaSendVirtual0, false, false, false)
                         ->getMethodAddress();
        break;
    case TR::Int32:
        dispatcher = (intptrj_t)cg->symRefTab()
                         ->findOrCreateRuntimeHelper(TR_PPCicallVMprJavaSendVirtual1, false, false, false)
                         ->getMethodAddress();
        break;
    case TR::Address:
        if (TR::Compiler->target.is64Bit())
            dispatcher = (intptrj_t)cg->symRefTab()
                             ->findOrCreateRuntimeHelper(TR_PPCicallVMprJavaSendVirtualJ, false, false, false)
                             ->getMethodAddress();
        else
            dispatcher = (intptrj_t)cg->symRefTab()
                             ->findOrCreateRuntimeHelper(TR_PPCicallVMprJavaSendVirtual1, false, false, false)
    ...
    case TR::Float:
        dispatcher = (intptrj_t)cg->symRefTab()
                         ->findOrCreateRuntimeHelper(TR_PPCicallVMprJavaSendVirtualF, false, false, false)
                         ->getMethodAddress();
        break;
    case TR::Double:
        dispatcher = (intptrj_t)cg->symRefTab()
                         ->findOrCreateRuntimeHelper(TR_PPCicallVMprJavaSendVirtualD, false, false, false)
                         ->getMethodAddress();
        break;
    default:
        TR_ASSERT(0, "Bad return data type for a call node.  DataType was %s\n",
    ...
    if (TR::Compiler->target.is32Bit()
        && (((dispatcher & 0x80008000) == 0x80008000) || cg->comp()->compileRelocatableCode()))
        codeSize += 4;
    ...
    *(int32_t*)buffer = 0x4e800420;
    ...
    if (TR::Compiler->target.is32Bit() && comp->compileRelocatableCode()
        && !((dispatcher & 0x80008000)
               == 0x80008000)) // Make size of thunk larger for AOT even if extra instruction is not generated at
                               // compile time.  The extra instruction could be needed for the runtime address.
        sizeThunk += 4;
...
TR_J2IThunk* TR::PPCCallSnippet::generateInvokeExactJ2IThunk(
    TR::Node* callNode, int32_t argSize, TR::CodeGenerator* cg, char* signature)
{
    int32_t codeSize = 4 * (instructionCountForArguments(callNode, cg) + (TR::Compiler->target.is64Bit() ? 7 : 4))
        + 8; // Additional 4 bytes to hold size of thunk
    intptrj_t dispatcher;
    ...
    case TR::NoType:
        dispatcher = (intptrj_t)cg->symRefTab()
                         ->findOrCreateRuntimeHelper(TR_icallVMprJavaSendInvokeExact0, false, false, false)
                         ->getMethodAddress();
        break;
    case TR::Int32:
        dispatcher = (intptrj_t)cg->symRefTab()
                         ->findOrCreateRuntimeHelper(TR_icallVMprJavaSendInvokeExact1, false, false, false)
                         ->getMethodAddress();
        break;
    case TR::Address:
        if (TR::Compiler->target.is64Bit())
            dispatcher = (intptrj_t)cg->symRefTab()
                             ->findOrCreateRuntimeHelper(TR_icallVMprJavaSendInvokeExactJ, false, false, false)
                             ->getMethodAddress();
        else
            dispatcher = (intptrj_t)cg->symRefTab()
                             ->findOrCreateRuntimeHelper(TR_icallVMprJavaSendInvokeExact1, false, false, false)
    ...
    case TR::Float:
        dispatcher = (intptrj_t)cg->symRefTab()
                         ->findOrCreateRuntimeHelper(TR_icallVMprJavaSendInvokeExactF, false, false, false)
                         ->getMethodAddress();
        break;
    case TR::Double:
        dispatcher = (intptrj_t)cg->symRefTab()
                         ->findOrCreateRuntimeHelper(TR_icallVMprJavaSendInvokeExactD, false, false, false)
                         ->getMethodAddress();
        break;
    default:
...
uint8_t* TR_Debug::printPPCArgumentsFlush(TR::FILE* pOutFile, TR::Node* node, uint8_t* cursor, int32_t argSize)
{
    char* storeGPROpName;
        ...
        case TR::Int64:
            ...
            if (intArgNum < linkage.getNumIntArgRegs()) {
                ...
                if (TR::Compiler->target.is32Bit() && (intArgNum < linkage.getNumIntArgRegs() - 1)) {
                    ...
                    print(pOutFile, machine->getRealRegister(linkage.getIntegerArgumentRegister(intArgNum + 1)),
                        TR_WordReg);
                    cursor += 4;
                }
            }
...
void TR_Debug::print(TR::FILE* pOutFile, TR::PPCCallSnippet* snippet)
{
    ...
    if (isNativeStatic) {
        ...
        distance = (distance << 6) >> 6; // sign extend
        trfprintf(
            pOutFile, "b \t" POINTER_PRINTF_FORMAT "\t\t;%s", (intptrj_t)cursor + distance, " back to program code");
        cursor += 4;
...
void TR_Debug::print(TR::FILE* pOutFile, TR::PPCUnresolvedCallSnippet* snippet)
{
    // *this   swipeable for debugger
    ...
    printPrefix(pOutFile, NULL, cursor, 4);
    trfprintf(pOutFile, ".long \t0x%08x\t\t; Offset | Flag | CP Index",
...
void TR_Debug::print(TR::FILE* pOutFile, TR::PPCVirtualSnippet* snippet)
{
    // *this   swipeable for debugger
}

void TR_Debug::print(TR::FILE* pOutFile, TR::PPCVirtualUnresolvedSnippet* snippet)
{
    // *this   swipeable for debugger
    uint8_t* cursor = snippet->getSnippetLabel()->getCodeLocation();
    TR::Node* callNode = snippet->getNode();
    ...
    printPrefix(pOutFile, NULL, cursor, sizeof(intptrj_t));
    trfprintf(pOutFile, ".long \t" POINTER_PRINTF_FORMAT "\t\t; Call Site RA",
        (intptrj_t)snippet->getReturnLabel()->getCodeLocation());
    cursor += sizeof(intptrj_t);

    printPrefix(pOutFile, NULL, cursor, sizeof(intptrj_t));
    trfprintf(pOutFile, ".long \t" POINTER_PRINTF_FORMAT "\t\t; Constant Pool Pointer",
        (intptrj_t)getOwningMethod(callNode->getSymbolReference())->constantPool());
    cursor += sizeof(intptrj_t);

    printPrefix(pOutFile, NULL, cursor, sizeof(intptrj_t));
    trfprintf(pOutFile, ".long \t" POINTER_PRINTF_FORMAT "\t\t; Constant Pool Index",
...
void TR_Debug::print(TR::FILE* pOutFile, TR::PPCInterfaceCallSnippet* snippet)
{
    // *this   swipeable for debugger
    uint8_t* cursor = snippet->getSnippetLabel()->getCodeLocation();
    TR::Node* callNode = snippet->getNode();
