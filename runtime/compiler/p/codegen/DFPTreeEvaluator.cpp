...
extern TR::Instruction* loadConstant(TR::CodeGenerator* cg, TR::Node* node, int32_t value, TR::Register* trgReg,
    TR::Instruction* cursor, bool isPicSite);
extern TR::Instruction* loadConstant(TR::CodeGenerator* cg, TR::Node* node, int64_t value, TR::Register* trgReg,
    TR::Instruction* cursor, bool isPicSite);
...
extern bool loadAndEvaluateAsDouble(TR::Node* node, TR::Register*& reg, TR::CodeGenerator* cg)
{
    if (node->getOpCodeValue() == TR::lloadi && !node->getRegister() && node->getReferenceCount() == 1) {
...
static void overlapDFPOperandAndPrecisionLoad(TR::Node* node, TR::Node* lhsNode, TR::Node* rhsNode, TR::Node* precChild,
    TR::Register*& lhsFPRegister, TR::Register*& rhsFPRegister, TR::Register*& precFPRegister, int32_t toRound,
    bool isConst16Precision, TR::CodeGenerator* cg)
{
    ...
    /* We want to overlap the loading of the DFP operands and precision...
       Cases will be..
         ...
         32-bt:  Store-LoadLHS, Store-LoadRHS, Store-LoadPrec, (storeLHS, storeRHS, storePrec, loadPrec, loadRHS,
       loadLHS)
     */
    ...
    } else // in 32-bit mode or if there is no direct move, overlap the Store-Load sequence
    {
        ...
        if (!lhsLoaded) {
            ...
            if (TR::Compiler->target.is32Bit()) {
                ...
                generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                    new (cg->trHeapMemory()) TR::MemoryReference(node, *tempLHSMR, 4, 4, cg),
                    lhsRegister->getLowOrder());
        ...
        if (!rhsLoaded) {
            ...
            if (TR::Compiler->target.is32Bit()) {
                ...
                generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                    new (cg->trHeapMemory()) TR::MemoryReference(node, *tempRHSMR, 4, 4, cg),
                    rhsRegister->getLowOrder());
            } else
...
static void genSetDFPRoundingMode(TR::Node* node, TR::CodeGenerator* cg, int16_t rm)
{
    generateImm2Instruction(cg, TR::InstOpCode::mtfsfi, node, rm, dfpRMBitField);
}

static void genSetDFPRoundingMode(TR::Node* node, TR::CodeGenerator* cg, TR::Register* rmRegister)
{
    ...
    */
    ...
    if (TR::Compiler->target.is64Bit()) {
        ...
        generateTrg1Src1Imm2Instruction(
            cg, TR::InstOpCode::rldicr, node, newRMRegister, rmRegister, 32, CONSTANT64(0xFFFFFFFF00000000));
...
static void genTestDataGroup(
    TR::Node* node, TR::CodeGenerator* cg, TR::Register* fprRegister, TR::Register* crRegister, int32_t grp)
{
...
static void genClearFPSCRBits(TR::Node* node, TR::CodeGenerator* cg, int32_t bf)
{
    // Clear bit field bf in the 64-bit FPSCR
    generateImm2Instruction(cg, TR::InstOpCode::mtfsfi, node, 0x0, bf);
}

static void genStoreDFP(TR::Node* node, TR::CodeGenerator* cg, TR::Register* objRegister,
...
static TR::Register* inlineBigDecimalConstructor64(TR::Node* node, TR::CodeGenerator* cg, bool isLong, bool exp)
{
    bool p8DirectMoveTest = TR::Compiler->target.cpu.id() == TR_PPCp8;
    ...
    /*  Now a sequence of back2back direct moves if it is supported (generateMvFprGprInstructions() does this),
     * otherwise Load-Store-Hit */
    ...
    if (p8DirectMoveTest) {
        ...
        if (exp) {
            ...
            generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                new (cg->trHeapMemory()) TR::MemoryReference(node, *tempExpMR, 0, 4, cg), expRegister);
        }
        ...
        if (exp) {
            generateTrg1MemInstruction(cg, TR::InstOpCode::lfiwax, node, expFPRegister,
                new (cg->trHeapMemory()) TR::MemoryReference(node, *tempExpMR, 0, 4, cg));
    ...
    if (!inBCDForm) {
        ...
        TR::LabelSymbol* depLabel
            = TR::LabelSymbol::create(cg->trHeapMemory(), cg); // somewhere to hang the dependencies
...
static TR::Register* inlineBigDecimalConstructor32(TR::Node* node, TR::CodeGenerator* cg, bool isLong, bool exp)
{
    ...
    TR::Node* node, TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, bool scaled)
{
    ...
    overlapDFPOperandAndPrecisionLoad(node, lhsNode, rhsNode, precChild, lhsFPRegister, rhsFPRegister, precFPRegister,
    ...
    if (!scaled) {
        loadConstant(cg, node, 1, retRegister);
        genStoreDFP(
            node, cg, cg->evaluate(node->getFirstChild()), node->getSecondChild()->getSymbolReference(), resRegister);
        cg->decReferenceCount(node->getFirstChild());
        ...
        if (TR::Compiler->target.is64Bit() && p8DirectMoveTest) // sign extend it...
            generateTrg1Src1Instruction(
                cg, TR::InstOpCode::extsw, node, desiredBiasedExpRegister, desiredBiasedExpRegister);
        cg->decReferenceCount(node->getChild(3));
        ...
        generateTrg1Src2Instruction(
            cg, TR::InstOpCode::cmp4, node, crRegister, actualBiasedExpRegister, desiredBiasedExpRegister);
        ...
        generateTrg1Src2Instruction(
            cg, TR::InstOpCode::diex, node, dfpDummyFPRegister, fprDesiredBiasedExpRegister, dfpDummyFPRegister);
        ...
        generateTrg1Src2ImmInstruction(
            cg, TR::InstOpCode::dqua, node, resRegister, dfpDummyFPRegister, resRegister, 0x3);
        ...
        // generateTrg1ImmInstruction(cg, TR::InstOpCode::mcrfs, node, crRegister, 0x3); //move FPSCR field 4 (bits 44
        // to 47)
        ...
        loadConstant(cg, node, 1, retRegister);
        genStoreDFP(
            node, cg, node->getFirstChild()->getRegister(), node->getSecondChild()->getSymbolReference(), resRegister);
...
extern TR::Register* inlineBigDecimalScaledDivide(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    */
    ...
    if (TR::Compiler->target.is64Bit() && p8DirectMoveTest) {
        ...
        generateTrg1Src1Instruction(
            cg, TR::InstOpCode::mfvsrd, node, actualBiasedExpRegister, fprActualBiasedExpRegister);
    } else {
    ...
    if (TR::Compiler->target.is64Bit() && p8DirectMoveTest)
        generateTrg1Src1Instruction(
            cg, TR::InstOpCode::mtvsrd, node, fprDesiredBiasedExpRegister, desiredBiasedExpRegister);
    else {
    ...
    */
    ...
    if (checkInexact) {
        ...
        generateTrg1ImmInstruction(
            cg, TR::InstOpCode::mcrfs, node, crRegister, 0x3); // move FPSCR field 4 (bits 44 to 47)
        generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, node, labelEND, crRegister);
    }
...
extern TR::Register* inlineBigDecimalRound(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    loadConstant(cg, node, 1, retRegister);
    genStoreDFP(
        node, cg, cg->evaluate(node->getFirstChild()), node->getSecondChild()->getSymbolReference(), resFPRegister);
    cg->decReferenceCount(node->getFirstChild());
    ...
    } else // in 32-bit mode, generate Store-Load sequence
    {
        ...
        if (!dfpLoaded) {
            ...
            if (TR::Compiler->target.is32Bit()) {
                ...
                generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                    new (cg->trHeapMemory()) TR::MemoryReference(node, *tempDFPMR, 4, 4, cg),
                    dfpRegister->getLowOrder());
            } else
...
extern TR::Register* inlineBigDecimalUnscaledValue(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    } else // in 32-bit mode, generate Store-Load sequence
    {
        ...
        // prep the exponent for the Store-Load sequence
        TR::MemoryReference* tempExpMR
            = new (cg->trHeapMemory()) TR::MemoryReference(cg->getStackPointerRegister(), -4, 4, cg);
        tempExpMR->forceIndexedForm(node, cg);
