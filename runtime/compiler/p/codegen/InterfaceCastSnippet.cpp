...
void TR_Debug::print(TR::FILE* pOutFile, TR::PPCInterfaceCastSnippet* snippet)
{
    ...
    if (checkcast) {
        ...
        if (TR::Compiler->target.is64Bit() && !TR::Compiler->om.generateCompressedObjectHeaders())
            ...
            trfprintf(
                pOutFile, "lwz \t%s, [%s, %d]\t; Load object class", getName(scratch1Reg), getName(objReg), value);
        cursor += 4;

        printPrefix(pOutFile, NULL, cursor, 4);
        value = *((int32_t*)cursor) & 0x0ffff;
        if (TR::Compiler->target.is64Bit())
            trfprintf(pOutFile, "ld \t%s, [%s, %d]\t; Load castClassCache", getName(scratch1Reg), getName(scratch1Reg),
                value);
        else
            trfprintf(pOutFile, "lwz \t%s, [%s, %d]\t; Load castClassCache", getName(scratch1Reg), getName(scratch1Reg),
                value);
        cursor += 4;

        printPrefix(pOutFile, NULL, cursor, 4);
        trfprintf(pOutFile, "cmpl \t%s, %s, %s\t; Compare with type to cast", getName(cndReg), getName(castClassReg),
            getName(scratch1Reg));
        cursor += 4;
        ...
        value = *((int32_t*)cursor) & 0x0ffff;
        if (TR::Compiler->target.is64Bit() && !TR::Compiler->om.generateCompressedObjectHeaders())
            trfprintf(pOutFile, "ld \t%s, [%s, %d]\t; Load castClassCache", getName(scratch2Reg), getName(objClassReg),
                value);
        else
            trfprintf(pOutFile, "lwz \t%s, [%s, %d]\t; Load castClassCache", getName(scratch2Reg), getName(objClassReg),
                value);
        cursor += 4;

        printPrefix(pOutFile, NULL, cursor, 4);
        if (TR::Compiler->target.is64Bit())
            trfprintf(pOutFile, "rldicr \t%s, %s, 0, 0x3D; Clean last bit (cached result)", getName(scratch1Reg),
                getName(scratch2Reg));
        else
            trfprintf(pOutFile, "rlwinm \t%s, %s, 0, 0xFFFFFFFE; Clean last bit (cached result)", getName(scratch1Reg),
                getName(scratch2Reg));
        cursor += 4;

        printPrefix(pOutFile, NULL, cursor, 4);
        trfprintf(pOutFile, "cmpl \t%s, %s, %s\t; Compare with type to cast", getName(cndReg), getName(scratch1Reg),
        ...
        if (snippet->getNeedsResult()) {
            ...
            printPrefix(pOutFile, NULL, cursor, 4);
            trfprintf(pOutFile, "or \t%s, %s, %s; Set the last bit", getName(scratch1Reg), getName(scratch1Reg),
                getName(scratch2Reg));
            cursor += 4;

            printPrefix(pOutFile, NULL, cursor, 4);
            trfprintf(pOutFile, "xor. \t%s, %s, %s; Check if last bit is set in the cache", getName(resultReg),
            ...
            if (snippet->getFalseLabel() != snippet->getTrueLabel()) {
                ...
                printPrefix(pOutFile, NULL, cursor, 4);
                trfprintf(pOutFile, "and. \t%s, %s, %s; Check if last bit is set in the cache", getName(scratch1Reg),
                    getName(scratch1Reg), getName(scratch2Reg));
                cursor += 4;
