    ...
    *flagsCursor = modifier;
    ...
    case TR_MethodObject: {
        ...
        if (TR::Compiler->target.is64Bit()) {
            ...
            *(uint64_t*)cursor = (uint64_t)recordInfo->data2;
            ...
            TR_ASSERT((flags & RELOCATION_CROSS_PLATFORM_FLAGS_MASK) == 0,
                "reloFlags bits overlap cross-platform flags bits\n");
    ...
    case TR_ClassAddress: {
        ...
        *flagsCursor |= (flags & RELOCATION_RELOC_FLAGS_MASK);
        ...
        if (TR::Compiler->target.is64Bit()) {
            *(uint64_t*)cursor = (uint64_t)self()->findCorrectInlinedSiteIndex(
                tempSR->getOwningMethod(comp)->constantPool(), recordInfo->data2); // inlineSiteIndex
            cursor += 8;
            ...
            *(uint32_t*)cursor = (uint32_t)self()->findCorrectInlinedSiteIndex(
                tempSR->getOwningMethod(comp)->constantPool(), recordInfo->data2); // inlineSiteIndex
        ...
        *flagsCursor |= (flags & RELOCATION_RELOC_FLAGS_MASK);
        ...
        if (TR::Compiler->target.is64Bit()) {
            ...
            *(uint64_t*)cursor = relocation->getTargetAddress()
                ? (uint64_t)((uint8_t*)relocation->getTargetAddress() - aotMethodCodeStart)
                : 0x0;
            cursor += 8;
        } else {
