            ...
            if (methodSymbol->isJNI()) {
                ...
                if (TR::Compiler->target.is64Bit()) {
                    ...
                    TR::Instruction* cursor = new (self()->trHeapMemory()) TR::PPCImmInstruction(TR::InstOpCode::dd,
                        firstNode, TR::Compiler->target.cpu.isBigEndian() ? highBits : lowBits, NULL, self());
...
void J9::Power::CodeGenerator::lowerTreeIfNeeded(
...
bool J9::Power::CodeGenerator::suppressInliningOfRecognizedMethod(TR::RecognizedMethod method)
{
    ...
    if (method == TR::java_lang_Math_abs_F || method == TR::java_lang_Math_abs_D || method == TR::java_lang_Math_abs_I
        || method == TR::java_lang_Math_abs_L || method == TR::java_lang_Integer_highestOneBit
        || method == TR::java_lang_Integer_numberOfLeadingZeros || method == TR::java_lang_Integer_numberOfTrailingZeros
        || method == TR::java_lang_Integer_rotateLeft || method == TR::java_lang_Integer_rotateRight
        || method == TR::java_lang_Long_highestOneBit || method == TR::java_lang_Long_numberOfLeadingZeros
        || method == TR::java_lang_Long_numberOfTrailingZeros || method == TR::java_lang_Short_reverseBytes
        || method == TR::java_lang_Integer_reverseBytes || method == TR::java_lang_Long_reverseBytes
        || (TR::Compiler->target.is64Bit()
               && (method == TR::java_lang_Long_rotateLeft || method == TR::java_lang_Long_rotateRight))) {
        return true;
    }
...
bool J9::Power::CodeGenerator::enableAESInHardwareTransformations()
{
    if ((TR::Compiler->target.cpu.getPPCSupportsAES()
            || (TR::Compiler->target.cpu.getPPCSupportsVMX() && TR::Compiler->target.cpu.getPPCSupportsVSX()))
...
void J9::Power::CodeGenerator::insertPrefetchIfNecessary(TR::Node* node, TR::Register* targetRegister)
{
    ...
    if (node->getOpCodeValue() == TR::aloadi
        ...
        if (!disableHotFieldPrefetch && prefetchOffset >= 0) // Prefetch based on hot field information
        {
            ...
            if (!canSkipNullChk) {
                ...
                generateTrg1Src1ImmInstruction(
                    self(), TR::InstOpCode::cmpli4, node, condReg, targetRegister, NULLVALUE);
                ...
                if (TR::Compiler->target.is64Bit() && !comp()->useCompressedPointers()) {
                    TR::MemoryReference* tempMR
                        = new (self()->trHeapMemory()) TR::MemoryReference(targetRegister, prefetchOffset, 8, self());
                    generateTrg1MemInstruction(self(), TR::InstOpCode::ld, node, tempReg, tempMR);
                } else {
                    TR::MemoryReference* tempMR
                        = new (self()->trHeapMemory()) TR::MemoryReference(targetRegister, prefetchOffset, 4, self());
                    generateTrg1MemInstruction(self(), TR::InstOpCode::lwz, node, tempReg, tempMR);
                }

                TR::MemoryReference* targetMR
                    = new (self()->trHeapMemory()) TR::MemoryReference(tempReg, (int32_t)0, 4, self());
                ...
                TR_ASSERT(
                    useTree->getNode()->getOpCodeValue() == TR::BBStart, "Expecting a BBStart on the fall through\n");
                        ...
                        if (!fromRegLoad) {
                            ...
                            if (indexObject->getOpCode().isLoadConst()) {
                                ...
                                if (len >= LOWER_IMMED && len <= UPPER_IMMED)
                                    generateTrg1Src1ImmInstruction(
                                        self(), TR::InstOpCode::addi, node, pointerReg, baseReg, len);
                                else {
                                    indexReg = self()->allocateRegister();
                                    loadConstant(self(), node, len, indexReg);
                                    generateTrg1Src2Instruction(
                                        self(), TR::InstOpCode::add, node, pointerReg, baseReg, indexReg);
            ...
            if (!optDisabled) {
                ...
                uintptr_t heapTop
                    = comp()->getOptions()->getHeapTop() ? comp()->getOptions()->getHeapTop() : 0xFFFFFFFF;
                ...
                if (pointerReg && (heapTop > comp()->getOptions()->getHeapBase())) // Check for gencon
                {
                        ...
                        if (!prefetch2Ahead)
                            generateTrg1MemInstruction(self(), TR::InstOpCode::ld, node, temp3Reg,
                                new (self()->trHeapMemory()) TR::MemoryReference(
                                    pointerReg, (int32_t)TR::Compiler->om.sizeofReferenceField(), 8, self()));
                        else
                            generateTrg1MemInstruction(self(), TR::InstOpCode::ld, node, temp3Reg,
                                new (self()->trHeapMemory()) TR::MemoryReference(
                                    pointerReg, (int32_t)(TR::Compiler->om.sizeofReferenceField() * 2), 8, self()));
                    } else {
                        if (!prefetch2Ahead)
                            generateTrg1MemInstruction(self(), TR::InstOpCode::lwz, node, temp3Reg,
                                new (self()->trHeapMemory()) TR::MemoryReference(
                                    pointerReg, (int32_t)TR::Compiler->om.sizeofReferenceField(), 4, self()));
                        else
                            generateTrg1MemInstruction(self(), TR::InstOpCode::lwz, node, temp3Reg,
                                new (self()->trHeapMemory()) TR::MemoryReference(
                                    pointerReg, (int32_t)(TR::Compiler->om.sizeofReferenceField() * 2), 4, self()));
                    }
                    ...
                    if (comp()->getOptions()->getHeapBase() != NULL) {
                        ...
                        generateConditionalBranchInstruction(
                            self(), TR::InstOpCode::blt, node, endCtrlFlowLabel, condReg);
                    }
                    if (heapTop != 0xFFFFFFFF) {
                        loadAddressConstant(self(), node, (intptrj_t)(heapTop - prefetchOffset), tempReg);
                        generateTrg1Src2Instruction(self(), TR::InstOpCode::cmpl4, node, condReg, temp3Reg, tempReg);
                        generateConditionalBranchInstruction(
                            self(), TR::InstOpCode::bgt, node, endCtrlFlowLabel, condReg);
                    }
                    TR::MemoryReference* targetMR
                        = new (self()->trHeapMemory()) TR::MemoryReference(temp3Reg, (int32_t)0, 4, self());
                    targetMR->forceIndexedForm(node, self());
                    generateMemInstruction(
                        self(), TR::InstOpCode::dcbt, node, targetMR); // use dcbt for prefetch next element
        ...
        if (!disableStringObjPrefetch && 0 && node->getSymbolReference() && !node->getSymbolReference()->isUnresolved()
            ...
            if (fieldName && strstr(fieldName, "Ljava/lang/String;")) {
                TR::MemoryReference* targetMR
                    = new (self()->trHeapMemory()) TR::MemoryReference(targetRegister, (int32_t)0, 4, self());
                targetMR->forceIndexedForm(node, self());
                generateMemInstruction(self(), TR::InstOpCode::dcbt, node, targetMR);
            }
        }
    ...
    if (node->getOpCodeValue() == TR::aloadi
            ...
            if (TR::Compiler->target.is32Bit()) {
                if (!(firstChild && firstChild->getOpCodeValue() == TR::aiadd && firstChild->isInternalPointer()
                        && (strstr(comp()->fe()->sampleSignature(node->getOwningMethod(), 0, 0, self()->trMemory()),
                               ...
                               || strstr(
                                      comp()->fe()->sampleSignature(node->getOwningMethod(), 0, 0, self()->trMemory()),
        ...
        if (!optDisabled && comp()->getOption(TR_AggressiveOpts)) {
            ...
            if (baseReg && indexReg && loopSize > 0) {
                ...
                generateTrg1Src1ImmInstruction(self(), TR::InstOpCode::addi, node, tempReg, indexReg,
                    (int32_t)(prefetchElementStride * TR::Compiler->om.sizeofReferenceField()));
                if (TR::Compiler->target.is64Bit() && !comp()->useCompressedPointers()) {
                    TR::MemoryReference* targetMR
                        = new (self()->trHeapMemory()) TR::MemoryReference(baseReg, tempReg, 8, self());
                    generateTrg1MemInstruction(self(), TR::InstOpCode::ld, node, tempReg, targetMR);
                } else {
                    TR::MemoryReference* targetMR
                        = new (self()->trHeapMemory()) TR::MemoryReference(baseReg, tempReg, 4, self());
                    generateTrg1MemInstruction(self(), TR::InstOpCode::lwz, node, tempReg, targetMR);
                }

                if (comp()->useCompressedPointers()) {
                    generateShiftLeftImmediateLong(
                        self(), node, tempReg, tempReg, TR::Compiler->om.compressedReferenceShiftOffset());
                }
                TR::MemoryReference* target2MR
                    = new (self()->trHeapMemory()) TR::MemoryReference(tempReg, 0, 4, self());
        ...
        int32_t prefetchOffset = comp()->findPrefetchInfo(node);
        if (prefetchOffset >= 0) {
            TR::MemoryReference* targetMR = new (self()->trHeapMemory())
                TR::MemoryReference(targetRegister, prefetchOffset, TR::Compiler->om.sizeofReferenceAddress(), self());
            targetMR->forceIndexedForm(node, self());
            generateMemInstruction(self(), TR::InstOpCode::dcbt, node, targetMR);
        }
    }
