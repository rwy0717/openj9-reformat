    ...
    if (getOpCodeValue() == TR::InstOpCode::bl || getOpCodeValue() == TR::InstOpCode::b) {
                ...
                *(int32_t*)cursor |= distance & 0x03fffffc;
                ...
                if (refNum < TR_PPCnumRuntimeHelpers) {
                    ...
                    && refNum <= cg()->symRefTab()->getNonhelperIndex(
                                     TR::SymbolReferenceTable::lastPerCodeCacheHelperSymbol)) {
                    TR_ASSERT(comp->getCodeCacheSwitched(),
                    ...
                    TR_CCPreLoadedCode helper = (TR_CCPreLoadedCode)(refNum
                        - cg()->symRefTab()->getNonhelperIndex(
                              TR::SymbolReferenceTable::firstPerCodeCacheHelperSymbol));
                    _addrImmediate
                        = (uintptrj_t)fej9->getCCPreLoadedCodeAddress(comp->getCurrentCodeCache(), helper, cg());
        ...
        if (cg()->comp()->compileRelocatableCode() && label == NULL) {
            cg()->addExternalRelocation(new (cg()->trHeapMemory()) TR::ExternalRelocation(
                                            cursor, (uint8_t*)getSymbolReference(), TR_HelperAddress, cg()),
