...
TR::PPCMonitorEnterSnippet::PPCMonitorEnterSnippet(TR::CodeGenerator* codeGen, TR::Node* monitorNode, int32_t lwOffset,
    bool isPreserving, TR::LabelSymbol* incLabel, TR::LabelSymbol* callLabel, TR::LabelSymbol* restartLabel,
...
void TR::PPCMonitorEnterSnippet::print(TR::FILE* pOutFile, TR_Debug* debug)
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(cg()->fe());
    ...
    if (isReservationPreserving()) {
        ...
        debug->printPrefix(pOutFile, NULL, cursor, 4);
        trfprintf(pOutFile, "andc\t%s, %s, %s\t;", debug->getName(tempReg), debug->getName(monitorReg),
            debug->getName(tempReg));
        cursor += 4;

        debug->printPrefix(pOutFile, NULL, cursor, 4);
        trfprintf(
            pOutFile, "addi\t%s, %s, %d\t;", debug->getName(threadReg), debug->getName(metaReg), LOCK_RESERVATION_BIT);
        cursor += 4;

        debug->printPrefix(pOutFile, NULL, cursor, 4);
        trfprintf(pOutFile, "%s\t%s, %s, %s\t;", ppcOpCodeToNameMap[TR::InstOpCode::Op_cmpl][1],
            debug->getName(condReg), debug->getName(tempReg), debug->getName(threadReg));
        cursor += 4;
    } else {
        debug->printPrefix(pOutFile, NULL, cursor, 4);
        if (TR::Compiler->target.is64Bit())
            trfprintf(pOutFile,
                "rldicr \t%s, %s, 0, " INT64_PRINTF_FORMAT_HEX "; get thread value and high bit of count",
                debug->getName(threadReg), debug->getName(monitorReg),
                (int64_t)LOCK_THREAD_PTR_AND_UPPER_COUNT_BIT_MASK);
        else
            trfprintf(pOutFile, "rlwinm \t%s, %s, 0, 0x%x; get thread value and high bit of count",
                debug->getName(threadReg), debug->getName(monitorReg), LOCK_THREAD_PTR_AND_UPPER_COUNT_BIT_MASK);
        cursor += 4;

        debug->printPrefix(pOutFile, NULL, cursor, 4);
        if (TR::Compiler->target.is64Bit())
            trfprintf(pOutFile, "cmp8 \t%s, %s, %s; Compare these two", debug->getName(condReg),
                debug->getName(metaReg), debug->getName(threadReg));
        else
            trfprintf(pOutFile, "cmp4 \t%s, %s, %s; Compare these two", debug->getName(condReg),
                debug->getName(metaReg), debug->getName(threadReg));
        cursor += 4;
    }
    ...
    debug->printPrefix(pOutFile, NULL, cursor, 4);
    trfprintf(pOutFile, "addi \t%s, %s, %d; Increment the count", debug->getName(monitorReg),
        debug->getName(monitorReg), LOCK_INC_DEC_VALUE);
    cursor += 4;

    debug->printPrefix(pOutFile, NULL, cursor, 4);
    if (TR::Compiler->target.is64Bit())
        trfprintf(pOutFile, "std \t[%s, %d], %s; Store the new count", debug->getName(objReg), getLockWordOffset(),
            debug->getName(monitorReg));
    else
        trfprintf(pOutFile, "stw \t[%s, %d], %s; Store the new count", debug->getName(objReg), getLockWordOffset(),
            debug->getName(monitorReg));
    cursor += 4;
...
TR::PPCMonitorExitSnippet::PPCMonitorExitSnippet(TR::CodeGenerator* codeGen, TR::Node* monitorNode, int32_t lwOffset,
    bool flag, bool isPreserving, TR::LabelSymbol* decLabel, TR::LabelSymbol* restoreAndCallLabel,
    TR::LabelSymbol* callLabel, TR::LabelSymbol* restartLabel, TR::Register* objectClassReg)
    : _decLabel(decLabel)
...
void TR::PPCMonitorExitSnippet::print(TR::FILE* pOutFile, TR_Debug* debug)
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(cg()->fe());
    ...
    TR::RealRegister* monitorReg = cg()->machine()->getRealRegister(TR::RealRegister::gr11);
    TR::RealRegister* threadReg = cg()->machine()->getRealRegister(
        conditions->getPostConditions()->getRegisterDependency(2)->getRealRegister());
    ...
    if (isReservationPreserving()) {
        ...
        debug->printPrefix(pOutFile, NULL, cursor, 4);
        trfprintf(pOutFile, "andc\t%s, %s, %s\t;", debug->getName(threadReg), debug->getName(monitorReg),
            debug->getName(threadReg));
        cursor += 4;

        debug->printPrefix(pOutFile, NULL, cursor, 4);
        trfprintf(pOutFile, "%s\t%s, %s, %s\t;", ppcOpCodeToNameMap[TR::InstOpCode::Op_cmpl][1],
            debug->getName(condReg), debug->getName(threadReg), debug->getName(metaReg));
        cursor += 4;
        ...
        debug->printPrefix(pOutFile, NULL, cursor, 4);
        trfprintf(pOutFile, "andi.\t%s, %s, 0x%x\t;", debug->getName(threadReg), debug->getName(monitorReg),
        ...
        debug->printPrefix(pOutFile, NULL, cursor, 4);
        trfprintf(pOutFile, "andi.\t%s, %s, 0x%x\t;", debug->getName(threadReg), debug->getName(monitorReg),
            LOCK_OWNING_NON_INFLATED_COMPLEMENT);
        cursor += 4;

        debug->printPrefix(pOutFile, NULL, cursor, 4);
        trfprintf(pOutFile, "cmplwi %s, %s, 0x%x\t;", debug->getName(condReg), debug->getName(threadReg),
            LOCK_RES_CONTENDED_VALUE);
        cursor += 4;
        ...
        if (TR::Compiler->target.is64Bit()) {
            ...
            cursor += 4;
            debug->printPrefix(pOutFile, NULL, cursor, 4);
            trfprintf(pOutFile, "andc \t%s, %s, %s; Mask out count field", debug->getName(threadReg),
                debug->getName(monitorReg), debug->getName(threadReg));
        } else
            trfprintf(pOutFile, "rlwinm \t%s, %s, 0, 0x%x; Mask out count field", debug->getName(threadReg),
                debug->getName(monitorReg), ~OBJECT_HEADER_LOCK_RECURSION_MASK);
        cursor += 4;

        debug->printPrefix(pOutFile, NULL, cursor, 4);
        if (TR::Compiler->target.is64Bit())
            trfprintf(pOutFile, "cmp8 \t%s, %s, %s; Test for inflated or flc bit set", debug->getName(condReg),
                debug->getName(metaReg), debug->getName(threadReg));
        else
            trfprintf(pOutFile, "cmp4 \t%s, %s, %s; Test for inflated or flc bit set", debug->getName(condReg),
                debug->getName(metaReg), debug->getName(threadReg));
        cursor += 4;
    ...
    debug->printPrefix(pOutFile, NULL, cursor, 4);
    trfprintf(pOutFile, "addi \t%s, %s, %d; Decrement the count", debug->getName(monitorReg),
        debug->getName(monitorReg), -LOCK_INC_DEC_VALUE);
    cursor += 4;
...
TR::PPCLockReservationEnterSnippet::PPCLockReservationEnterSnippet(TR::CodeGenerator* cg, TR::Node* monitorNode,
    int32_t lwOffset, TR::LabelSymbol* startLabel, TR::LabelSymbol* enterCallLabel, TR::LabelSymbol* restartLabel,
    TR::Register* objectClassReg)
    : _lwOffset(lwOffset)
...
uint8_t* TR::PPCLockReservationEnterSnippet::emitSnippetBody()
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(cg()->fe());
    ...
    TR::RealRegister* valReg
        = cg()->machine()->getRealRegister(deps->getPostConditions()->getRegisterDependency(regNum)->getRealRegister());
    TR::RealRegister* tempReg = cg()->machine()->getRealRegister(
        deps->getPostConditions()->getRegisterDependency(regNum + 1)->getRealRegister());
...
void TR::PPCLockReservationEnterSnippet::print(TR::FILE* pOutFile, TR_Debug* debug)
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(cg()->fe());
    ...
    debug->printSnippetLabel(
        pOutFile, getStartLabel(), cursor, isPrimitive ? "Primitive Reservation Enter" : "Reservation Enter");
    ...
    TR::RealRegister* condReg = cg()->machine()->getRealRegister(TR::RealRegister::cr0);
    TR::RealRegister* monitorReg = cg()->machine()->getRealRegister(TR::RealRegister::gr11);
    TR::RealRegister* valReg = cg()->machine()->getRealRegister(
        conditions->getPostConditions()->getRegisterDependency(3)->getRealRegister());
    TR::RealRegister* tempReg = cg()->machine()->getRealRegister(
        conditions->getPostConditions()->getRegisterDependency(4)->getRealRegister());
    ...
    debug->printPrefix(pOutFile, NULL, cursor, 4);
    trfprintf(pOutFile, "bne \t%s, " POINTER_PRINTF_FORMAT "\t;", debug->getName(condReg),
        (intptrj_t)cursor + (isPrimitive ? 36 : 40));
    cursor += 4;
    ...
    if (!isPrimitive) {
        debug->printPrefix(pOutFile, NULL, cursor, 4);
        trfprintf(pOutFile, "addi\t%s, %s, %d\t;", debug->getName(valReg), debug->getName(metaReg),
            LOCK_RESERVATION_BIT | LOCK_INC_DEC_VALUE);
        cursor += 4;
    }

    debug->printPrefix(pOutFile, NULL, cursor, 4);
    trfprintf(pOutFile, "%s\t%s, [%s, %s]\t;", ppcOpCodeToNameMap[TR::InstOpCode::Op_larx][1],
        debug->getName(monitorReg), debug->getName(objReg), debug->getName(tempReg));
    cursor += 4;

    debug->printPrefix(pOutFile, NULL, cursor, 4);
    trfprintf(pOutFile, "%s %s, %s, 0\t;", ppcOpCodeToNameMap[TR::InstOpCode::Op_cmpli][1], debug->getName(condReg),
        debug->getName(monitorReg));
    cursor += 4;

    debug->printPrefix(pOutFile, NULL, cursor, 4);
    trfprintf(pOutFile, "bne \t%s, " POINTER_PRINTF_FORMAT "\t;", debug->getName(condReg),
        (intptrj_t)cursor + (isPrimitive ? 36 : 52));
    cursor += 4;

    debug->printPrefix(pOutFile, NULL, cursor, 4);
    trfprintf(pOutFile, "%s\t[%s,%s], %s\t;", ppcOpCodeToNameMap[TR::InstOpCode::Op_stcx_r][1], debug->getName(objReg),
        debug->getName(tempReg), debug->getName(valReg));
    cursor += 4;

    debug->printPrefix(pOutFile, NULL, cursor, 4);
    trfprintf(
        pOutFile, "bne \t%s, " POINTER_PRINTF_FORMAT "\t; Loop back", debug->getName(condReg), (intptrj_t)cursor - 16);
    cursor += 4;
    ...
    debug->printPrefix(pOutFile, NULL, cursor, 4);
    trfprintf(pOutFile, "li \t%s, 0x%x\t; reserved_check begins", debug->getName(tempReg),
        isPrimitive ? LOCK_RES_PRIMITIVE_ENTER_MASK : LOCK_RES_NON_PRIMITIVE_ENTER_MASK);
    cursor += 4;

    debug->printPrefix(pOutFile, NULL, cursor, 4);
    trfprintf(
        pOutFile, "andc\t%s, %s, %s\t;", debug->getName(tempReg), debug->getName(monitorReg), debug->getName(tempReg));
    cursor += 4;

    if (!isPrimitive) {
        debug->printPrefix(pOutFile, NULL, cursor, 4);
        trfprintf(
            pOutFile, "addi\t%s, %s, %d\t;", debug->getName(valReg), debug->getName(metaReg), LOCK_RESERVATION_BIT);
        cursor += 4;
    }

    debug->printPrefix(pOutFile, NULL, cursor, 4);
    trfprintf(pOutFile, "%s\t%s, %s, %s\t;", ppcOpCodeToNameMap[TR::InstOpCode::Op_cmpl][1], debug->getName(condReg),
        debug->getName(tempReg), debug->getName(valReg));
    cursor += 4;
    ...
    if (isPrimitive) {
        ...
        debug->printPrefix(pOutFile, NULL, cursor, 4);
        trfprintf(
            pOutFile, "beq \t%s, " POINTER_PRINTF_FORMAT "\t;", debug->getName(condReg), (intptrj_t)cursor + distance);
        cursor += 4;
    } else {
        debug->printPrefix(pOutFile, NULL, cursor, 4);
        ...
        debug->printPrefix(pOutFile, NULL, cursor, 4);
        trfprintf(pOutFile, "addi\t%s, %s, %d\t;", debug->getName(monitorReg), debug->getName(monitorReg),
            LOCK_INC_DEC_VALUE);
        cursor += 4;

        debug->printPrefix(pOutFile, NULL, cursor, 4);
        trfprintf(pOutFile, "%s\t[%s,%d], %s\t;", ppcOpCodeToNameMap[TR::InstOpCode::Op_st][1], debug->getName(objReg),
            getLockWordOffset(), debug->getName(monitorReg));
        cursor += 4;
...
uint32_t TR::PPCLockReservationEnterSnippet::getLength(int32_t estimatedSnippetStart)
{
...
int32_t TR::PPCLockReservationEnterSnippet::setEstimatedCodeLocation(int32_t p)
{
...
TR::PPCLockReservationExitSnippet::PPCLockReservationExitSnippet(TR::CodeGenerator* cg, TR::Node* monitorNode,
    int32_t lwOffset, TR::LabelSymbol* startLabel, TR::LabelSymbol* exitCallLabel, TR::LabelSymbol* restartLabel,
    TR::Register* objectClassReg)
    : _startLabel(startLabel)
    , _lwOffset(lwOffset)
    , _objectClassReg(objectClassReg)
...
uint8_t* TR::PPCLockReservationExitSnippet::emitSnippetBody()
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(cg()->fe());
...
void TR::PPCLockReservationExitSnippet::print(TR::FILE* pOutFile, TR_Debug* debug)
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(cg()->fe());
    uint8_t* cursor = getStartLabel()->getCodeLocation();
    int32_t distance;
    ...
    debug->printSnippetLabel(
        pOutFile, getStartLabel(), cursor, isPrimitive ? "Primitive Reservation Exit" : "Reservation Exit");
    ...
    debug->printPrefix(pOutFile, NULL, cursor, 4);
    trfprintf(pOutFile, "bne \t%s, " POINTER_PRINTF_FORMAT "\t;", debug->getName(condReg),
        (intptrj_t)cursor + (isPrimitive ? 20 : 24));
    cursor += 4;

    debug->printPrefix(pOutFile, NULL, cursor, 4);
    trfprintf(pOutFile, "andi.\t%s, %s, 0x%x\t;", debug->getName(tempReg), debug->getName(monitorReg),
    ...
    if (isPrimitive) {
        ...
        distance = (distance << 16) >> 16; // sign extend
        trfprintf(
            pOutFile, "bne \t%s, " POINTER_PRINTF_FORMAT "\t;", debug->getName(condReg), (intptrj_t)cursor + distance);
    } else {
        trfprintf(pOutFile, "beq \t%s, " POINTER_PRINTF_FORMAT "\t;", debug->getName(condReg), (intptrj_t)cursor + 16);
    }
    cursor += 4;
...
uint32_t TR::PPCLockReservationExitSnippet::getLength(int32_t estimatedSnippetStart)
{
...
int32_t TR::PPCLockReservationExitSnippet::setEstimatedCodeLocation(int32_t p)
{
    getStartLabel()->setEstimatedCodeLocation(p);
...
TR::PPCReadMonitorSnippet::PPCReadMonitorSnippet(TR::CodeGenerator* codeGen, TR::Node* monitorEnterNode,
    ...
    , TR::PPCHelperCallSnippet(
          codeGen, monitorExitNode, monExitCallLabel, monitorExitNode->getSymbolReference(), restartLabel)
{
...
uint8_t* TR::PPCReadMonitorSnippet::emitSnippetBody()
{
    ...
    *(int32_t*)buffer |= (getRestartLabel()->getCodeLocation() - buffer) & 0x03FFFFFC;
    ...
    intptrj_t distance
        = (intptrj_t)getMonitorEnterHelper()->getSymbol()->castToMethodSymbol()->getMethodAddress() - (intptrj_t)buffer;

    if (!(distance >= BRANCH_BACKWARD_LIMIT && distance <= BRANCH_FORWARD_LIMIT)) {
        distance = fej9->indexedTrampolineLookup(getMonitorEnterHelper()->getReferenceNumber(), (void*)buffer)
            - (intptrj_t)buffer;
        TR_ASSERT(
    ...
    if (comp->compileRelocatableCode()) {
        cg()->addExternalRelocation(new (cg()->trHeapMemory()) TR::ExternalRelocation(
                                        buffer, (uint8_t*)getMonitorEnterHelper(), TR_HelperAddress, cg()),
...
void TR::PPCReadMonitorSnippet::print(TR::FILE* pOutFile, TR_Debug* debug)
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(cg()->fe());
    ...
    debug->printPrefix(pOutFile, NULL, cursor, 4);
    if (TR::Compiler->target.is64Bit())
        trfprintf(pOutFile, "rldicr \t%s, %s, 0, " INT64_PRINTF_FORMAT_HEX "\t; Get owner thread value",
            debug->getName(monitorReg), debug->getName(monitorReg), (int64_t)LOCK_THREAD_PTR_MASK);
    else
        trfprintf(pOutFile, "rlwinm \t%s, %s, 0, 0x%x\t; Get owner thread value", debug->getName(monitorReg),
            debug->getName(monitorReg), LOCK_THREAD_PTR_MASK);
    cursor += 4;

    debug->printPrefix(pOutFile, NULL, cursor, 4);
    if (TR::Compiler->target.is64Bit())
        trfprintf(pOutFile, "cmp8 \t%s, %s, %s\t; Compare VMThread to owner thread", debug->getName(condReg),
            debug->getName(metaReg), debug->getName(monitorReg));
    else
        trfprintf(pOutFile, "cmp4 \t%s, %s, %s\t; Compare VMThread to owner thread", debug->getName(condReg),
            debug->getName(metaReg), debug->getName(monitorReg));
    cursor += 4;
    ...
    distance = (distance << 16) >> 16; // sign extend
    trfprintf(pOutFile, "bne %s, " POINTER_PRINTF_FORMAT "\t; Use Helpers", debug->getName(condReg),
        (intptrj_t)cursor + distance);
    cursor += 4;

    debug->printPrefix(pOutFile, NULL, cursor, 4);
    trfprintf(pOutFile, "%s \t%s, [%s, %d]\t; Load", ppcOpCodeToNameMap[getLoadOpCode()][1],
    ...
    distance = *((int32_t*)cursor) & 0x03fffffc;
    distance = (distance << 6) >> 6; // sign extend
    trfprintf(pOutFile, "bl \t" POINTER_PRINTF_FORMAT "\t\t; %s", (intptrj_t)cursor + distance,
...
TR::PPCHeapAllocSnippet::PPCHeapAllocSnippet(TR::CodeGenerator* codeGen, TR::Node* node, TR::LabelSymbol* callLabel,
    TR::SymbolReference* destination, TR::LabelSymbol* restartLabel, bool insertType)
...
uint8_t* TR::PPCHeapAllocSnippet::emitSnippetBody()
{
    ...
    TR::RealRegister* resultReg = cg()->machine()->getRealRegister(
        deps->getPostConditions()->getRegisterDependency(6)->getRealRegister()); // Refer to VMnewEvaluator

    if (getInsertType()) {
        TR::RealRegister* objectReg = cg()->machine()->getRealRegister(
            deps->getPostConditions()->getRegisterDependency(0)->getRealRegister()); // Refer to VMnewEvaluator
    ...
    if (!(distance >= BRANCH_BACKWARD_LIMIT && distance <= BRANCH_FORWARD_LIMIT)) {
        distance
            = fej9->indexedTrampolineLookup(getDestination()->getReferenceNumber(), (void*)buffer) - (intptrj_t)buffer;
        TR_ASSERT(
            distance >= BRANCH_BACKWARD_LIMIT && distance <= BRANCH_FORWARD_LIMIT, "CodeCache is more than 32MB.\n");
    }
    ...
    *(int32_t*)buffer |= distance & 0x03FFFFFC;
    if (cg()->comp()->compileRelocatableCode()) {
        cg()->addExternalRelocation(new (cg()->trHeapMemory()) TR::ExternalRelocation(
                                        buffer, (uint8_t*)getDestination(), TR_HelperAddress, cg()),
            __FILE__, __LINE__, getNode());
    }
    buffer += PPC_INSTRUCTION_LENGTH;
...
void TR::PPCHeapAllocSnippet::print(TR::FILE* pOutFile, TR_Debug* debug)
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(cg()->fe());
    uint8_t* cursor;
    int32_t distance;
    ...
    TR::RealRegister* resultReg
        = machine->getRealRegister(conditions->getPostConditions()->getRegisterDependency(6)->getRealRegister());
    ...
    if (getInsertType()) {
        TR::RealRegister* arrayTypeReg
            = machine->getRealRegister(conditions->getPostConditions()->getRegisterDependency(0)->getRealRegister());
        debug->printPrefix(pOutFile, NULL, cursor, 4);
    ...
    distance = *((int32_t*)cursor) & 0x03fffffc;
    distance = (distance << 6) >> 6; // sign extend
    TR_ASSERT((intptrj_t)cursor + distance == (intptrj_t)getRestartLabel()->getCodeLocation(),
        "heap snippet: bad branch to restart label");
    trfprintf(pOutFile, "b \t" POINTER_PRINTF_FORMAT "\t; Back to restart %s", (intptrj_t)cursor + distance,
        debug->getName(getRestartLabel()));
}
...
TR::PPCAllocPrefetchSnippet::PPCAllocPrefetchSnippet(
    TR::CodeGenerator* codeGen, TR::Node* node, TR::LabelSymbol* callLabel)
...
static uint8_t* initializeCCPreLoadedPrefetch(uint8_t* buffer, void** CCPreLoadedCodeTable, TR::CodeGenerator* cg)
{
    ...
    if (!TR::CodeGenerator::supportsTransientPrefetch() && !doL1Pref) {
        ...
        cursor = generateTrg1Src1ImmInstruction(
            cg, TR::InstOpCode::addi, n, r10, r8, skipLines * ppcCacheLineSize, cursor);
        cursor = generateTrg1Src1ImmInstruction(
            cg, TR::InstOpCode::addi, n, r11, r8, (skipLines + 1) * ppcCacheLineSize, cursor);
        cursor = generateMemInstruction(
            cg, TR::InstOpCode::dcbtst, n, new (cg->trHeapMemory()) TR::MemoryReference(NULL, r10, 4, cg), cursor);
        cursor = generateMemInstruction(
            cg, TR::InstOpCode::dcbtst, n, new (cg->trHeapMemory()) TR::MemoryReference(NULL, r11, 4, cg), cursor);
        ...
        cursor = generateMemSrc1Instruction(cg, TR::InstOpCode::Op_st, n,
            new (cg->trHeapMemory()) TR::MemoryReference(
                metaReg, offsetof(J9VMThread, tlhPrefetchFTA), TR::Compiler->om.sizeofReferenceAddress(), cg),
            r11, cursor);
    } else {
        // Transient version
        ...
        cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, n, r10,
            new (cg->trHeapMemory()) TR::MemoryReference(
                metaReg, offsetof(J9VMThread, debugEventData3), TR::Compiler->om.sizeofReferenceAddress(), cg),
            cursor);
        ...
        cursor = generateTrg1Src1ImmInstruction(
            cg, TR::InstOpCode::addi, n, r11, r8, (skipLines + 1) * ppcCacheLineSize, cursor);
        cursor = generateMemInstruction(
            cg, TR::InstOpCode::dcbtt, n, new (cg->trHeapMemory()) TR::MemoryReference(NULL, r10, 4, cg), cursor);
        cursor = generateMemInstruction(
            cg, TR::InstOpCode::dcbtt, n, new (cg->trHeapMemory()) TR::MemoryReference(NULL, r11, 4, cg), cursor);
        ...
        if (l3SkipLines > 0) {
            cursor = generateTrg1Src1ImmInstruction(
                cg, TR::InstOpCode::addi, n, r10, r10, ppcCacheLineSize * l3SkipLines, cursor);
            cursor = generateTrg1Src1ImmInstruction(
                cg, TR::InstOpCode::addi, n, r11, r11, ppcCacheLineSize * l3SkipLines, cursor);
        }
        ...
        for (uint32_t i = 0; i < linesToStPrefetch; i += 2) {
            ...
            cursor
                = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, n, r11, r11, ppcCacheLineSize * 2, cursor);
            cursor = generateMemInstruction(
                cg, TR::InstOpCode::dcbtstt, n, new (cg->trHeapMemory()) TR::MemoryReference(NULL, r10, 4, cg), cursor);
            cursor = generateMemInstruction(
                cg, TR::InstOpCode::dcbtstt, n, new (cg->trHeapMemory()) TR::MemoryReference(NULL, r11, 4, cg), cursor);
        }
    }
...
static uint8_t* initializeCCPreLoadedNonZeroPrefetch(
    uint8_t* buffer, void** CCPreLoadedCodeTable, TR::CodeGenerator* cg)
{
    TR::Compilation* comp = cg->comp();
    TR::Node* n = cg->getFirstInstruction()->getNode();
    ...
    if (!TR::CodeGenerator::supportsTransientPrefetch() && !doL1Pref) {
        ...
        cursor = generateMemSrc1Instruction(cg, TR::InstOpCode::Op_st, n,
            new (cg->trHeapMemory()) TR::MemoryReference(
                metaReg, offsetof(J9VMThread, nonZeroTlhPrefetchFTA), TR::Compiler->om.sizeofReferenceAddress(), cg),
            r11, cursor);
    } else {
        // Transient version
        static const char* s = feGetEnv("TR_l3SkipLines");
    ...
    TR::Register* condReg, uint32_t initOffset, TR::Instruction* cursor)
{
    ...
    // Fill the object with junk to make sure zero-init is working
    {
        ...
        cursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, n,
            new (cg->trHeapMemory()) TR::MemoryReference(iterReg, 0, 4, cg), zeroReg, cursor);
    ...
    for (int i = 0; i < 32; i += 4)
        cursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, n,
            new (cg->trHeapMemory()) TR::MemoryReference(iterReg, i, 4, cg), zeroReg, cursor);
    ...
    cursor = generateConditionalBranchInstruction(
        cg, TR::InstOpCode::bdnz, n, unrolledLoopStartLabel, /* Not used */ condReg, cursor);
    ...
    cursor = generateMemSrc1Instruction(
        cg, TR::InstOpCode::stw, n, new (cg->trHeapMemory()) TR::MemoryReference(iterReg, 0, 4, cg), zeroReg, cursor);
    cursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, n, iterReg, iterReg, 4, cursor);
    cursor = generateTrg1Src2Instruction(cg, TR::InstOpCode::Op_cmpl, n, condReg, iterReg, objEndReg, cursor);
...
static uint8_t* initializeCCPreLoadedObjectAlloc(uint8_t* buffer, void** CCPreLoadedCodeTable, TR::CodeGenerator* cg)
{
    ...
    zeroed_helperTrampolineLabel->setCodeLocation(
        (uint8_t*)TR::CodeCacheManager::instance()->findHelperTrampoline(buffer, TR_newObject));
    ...
    nonZeroed_helperTrampolineLabel->setCodeLocation(
        (uint8_t*)TR::CodeCacheManager::instance()->findHelperTrampoline(buffer, TR_newObjectNoZeroInit));
    ...
    cursor = generateTrg1Src1Imm2Instruction(
        cg, TR::InstOpCode::rlwinm, n, r10, r4, 0, fej9->getObjectAlignmentInBytes() - 1, cursor);
    cursor = generateSrc1Instruction(cg, TR::InstOpCode::twneqi, n, r10, 0, cursor);
    ...
    cursor = generateConditionalBranchInstruction(
        cg, TR::InstOpCode::beq, false, n, doNonZeroHeapTLHAlloc, cr2, cursor); // Set likeliness of branch to false.
    // 2 instructions so far.
    {
        ...
        cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, n, r10,
            new (cg->trHeapMemory()) TR::MemoryReference(
                metaReg, offsetof(struct J9VMThread, heapAlloc), TR::Compiler->om.sizeofReferenceAddress(), cg),
            cursor);
        cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, n, r11,
            new (cg->trHeapMemory()) TR::MemoryReference(
                metaReg, offsetof(struct J9VMThread, heapTop), TR::Compiler->om.sizeofReferenceAddress(), cg),
            cursor);
        // New heapAlloc
        ...
        cursor
            = generateConditionalBranchInstruction(cg, TR::InstOpCode::bgt, n, zeroed_branchToHelperLabel, cr2, cursor);
        // Check if we overflowed
        cursor = generateTrg1Src2Instruction(cg, TR::InstOpCode::Op_cmpl, n, cr2, r4, r10, cursor);
        cursor
            = generateConditionalBranchInstruction(cg, TR::InstOpCode::ble, n, zeroed_branchToHelperLabel, cr2, cursor);
        // Store new heapAlloc
        cursor = generateMemSrc1Instruction(cg, TR::InstOpCode::Op_st, n,
            new (cg->trHeapMemory()) TR::MemoryReference(
                metaReg, offsetof(struct J9VMThread, heapAlloc), TR::Compiler->om.sizeofReferenceAddress(), cg),
        ...
        cursor = generateMemSrc1Instruction(cg, Op_stclass, n,
            new (cg->trHeapMemory()) TR::MemoryReference(
                r10, TR::Compiler->om.offsetOfObjectVftField(), TR::Compiler->om.sizeofReferenceField(), cg),
            r3, cursor);
    ...
    cursor = generateLabelInstruction(cg, TR::InstOpCode::label, n, doNonZeroHeapTLHAlloc, cursor);
    {
        ...
        if (cg->enableTLHPrefetching()) {
            cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, n, r10,
                new (cg->trHeapMemory()) TR::MemoryReference(metaReg, offsetof(J9VMThread, nonZeroTlhPrefetchFTA),
        ...
        cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, n, r10,
            new (cg->trHeapMemory()) TR::MemoryReference(
                metaReg, offsetof(struct J9VMThread, nonZeroHeapAlloc), TR::Compiler->om.sizeofReferenceAddress(), cg),
            cursor);
        cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, n, r11,
            new (cg->trHeapMemory()) TR::MemoryReference(
                metaReg, offsetof(struct J9VMThread, nonZeroHeapTop), TR::Compiler->om.sizeofReferenceAddress(), cg),
            cursor);
        // New heapAlloc
        cursor = generateTrg1Src2Instruction(cg, TR::InstOpCode::add, n, r4, r10, r4, cursor);
        // Check if we exceeded the TLH
        cursor = generateTrg1Src2Instruction(cg, TR::InstOpCode::Op_cmpl, n, cr2, r4, r11, cursor);
        cursor = generateConditionalBranchInstruction(
            cg, TR::InstOpCode::bgt, n, nonZeroed_branchToHelperLabel, cr2, cursor);
        // Check if we overflowed
        cursor = generateTrg1Src2Instruction(cg, TR::InstOpCode::Op_cmpl, n, cr2, r4, r10, cursor);
        cursor = generateConditionalBranchInstruction(
            cg, TR::InstOpCode::ble, n, nonZeroed_branchToHelperLabel, cr2, cursor);
        // Store new heapAlloc
        cursor = generateMemSrc1Instruction(cg, TR::InstOpCode::Op_st, n,
            new (cg->trHeapMemory()) TR::MemoryReference(
                metaReg, offsetof(struct J9VMThread, nonZeroHeapAlloc), TR::Compiler->om.sizeofReferenceAddress(), cg),
        ...
        if (cg->enableTLHPrefetching()) {
            TR::LabelSymbol* prefetchHelperLabel = generateLabelSymbol(cg);
            TR_ASSERT(CCPreLoadedCodeTable[TR_NonZeroAllocPrefetch],
                "Expecting non-zero tlh prefetch helper to be initialized");
...
static uint8_t* initializeCCPreLoadedArrayAlloc(uint8_t* buffer, void** CCPreLoadedCodeTable, TR::CodeGenerator* cg)
{
    ...
    zeroed_aNewArrayTrampolineLabel->setCodeLocation(
        (uint8_t*)TR::CodeCacheManager::instance()->findHelperTrampoline(buffer, TR_aNewArray));
    zeroed_newArrayTrampolineLabel->setCodeLocation(
        (uint8_t*)TR::CodeCacheManager::instance()->findHelperTrampoline(buffer, TR_newArray));
    ...
    nonZeroed_aNewArrayTrampolineLabel->setCodeLocation(
        (uint8_t*)TR::CodeCacheManager::instance()->findHelperTrampoline(buffer, TR_aNewArrayNoZeroInit));
    nonZeroed_newArrayTrampolineLabel->setCodeLocation(
        (uint8_t*)TR::CodeCacheManager::instance()->findHelperTrampoline(buffer, TR_newArrayNoZeroInit));
    ...
    if (maxSafeSize < 0x00100000) {
        cursor
            = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::Op_cmpli, n, cr0, r4, (maxSafeSize >> 6) << 2, cursor);
        cursor
            = generateConditionalBranchInstruction(cg, TR::InstOpCode::bge, n, zeroed_branchToHelperLabel, cr0, cursor);
    } else {
        if (TR::Compiler->target.is64Bit())
            cursor = generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rldicr_r, n, r10, r4, cr0, 0,
                CONSTANT64(0xffffffffffffffff) << (27 - leadingZeroes(maxSafeSize)), cursor);
        else
            cursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::andis_r, n, r10, r4, cr0,
                (0xffffffff << (11 - leadingZeroes(maxSafeSize))) & 0x0000ffff, cursor);
        cursor
            = generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, n, zeroed_branchToHelperLabel, cr0, cursor);
    }
    // 2 instructions.
    ...
    if (TR::Compiler->om.useHybridArraylets()) {
        ...
        if (maxContiguousArraySize > UPPER_IMMED) {
            ...
            cursor = generateTrg1Src1ImmInstruction(
                cg, TR::InstOpCode::cmpli4, n, cr0, r5, maxContiguousArraySize, cursor);
        cursor
            = generateConditionalBranchInstruction(cg, TR::InstOpCode::bgt, n, zeroed_branchToHelperLabel, cr0, cursor);
    }
    ...
    cursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, n, r5, r5,
        sizeof(J9IndexableObjectContiguous) + fej9->getObjectAlignmentInBytes() - 1, cursor);
    cursor = generateTrg1Src1Imm2Instruction(cg,
        TR::Compiler->target.is64Bit() ? TR::InstOpCode::rldicr : TR::InstOpCode::rlwinm, n, r5, r5, 0,
        -fej9->getObjectAlignmentInBytes(), cursor);
    // Check if size < J9_GC_MINIMUM_OBJECT_SIZE and set size = J9_GC_MINIMUM_OBJECT_SIZE if so
    cursor
        = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::Op_cmpli, n, cr0, r5, J9_GC_MINIMUM_OBJECT_SIZE, cursor);
    ...
    // Trap if size does not preserve object alignment requirements
    cursor = generateTrg1Src1Imm2Instruction(
        cg, TR::InstOpCode::rlwinm, n, r10, r5, 0, fej9->getObjectAlignmentInBytes() - 1, cursor);
    cursor = generateSrc1Instruction(cg, TR::InstOpCode::twneqi, n, r10, 0, cursor);
    // Trap if size less than minimum object size
    ...
    cursor = generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::addi, n, r10, r5, -sizeof(J9IndexableObjectContiguous), cursor);
    ...
    // 2 instructions.
    {
        ...
        // a small i-cache footprint here it's better to have the variable length path fall through to the const length
        // path.
        ...
        // Store array length
        cursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, n,
            new (cg->trHeapMemory()) TR::MemoryReference(r10, fej9->getOffsetOfContiguousArraySizeField(), 4, cg), r4,
            cursor);
    ...
    cursor = generateConditionalBranchInstruction(
        cg, TR::InstOpCode::ble, n, zeroed_branchToPrimArrayHelperLabel, cr0, cursor);
    ...
    cursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::cmpli4, n, cr0, r3, 11, cursor);
    cursor = generateConditionalBranchInstruction(
        cg, TR::InstOpCode::ble, n, nonZeroed_branchToPrimArrayHelperLabel, cr0, cursor);
    ...
    uint32_t helperSize = 25 + 14 + 4
        + (TR::Compiler->om.useHybridArraylets()
                  ? (TR::Compiler->om.maxContiguousArraySizeInBytes() > UPPER_IMMED ? 4 : 2)
                  : 0)
...
static uint8_t* initializeCCPreLoadedWriteBarrier(uint8_t* buffer, void** CCPreLoadedCodeTable, TR::CodeGenerator* cg)
{
    ...
    helperTrampolineLabel->setCodeLocation(
        (uint8_t*)TR::CodeCacheManager::instance()->findHelperTrampoline(buffer, TR_writeBarrierStoreGenerational));
    TR::Instruction* entry = generateLabelInstruction(cg, TR::InstOpCode::label, n, entryLabel);
    TR::Instruction* cursor = entry;
#if defined(J9VM_INTERP_COMPRESSED_OBJECT_HEADER)
    ...
    TR_ASSERT((J9_OBJECT_HEADER_REMEMBERED_MASK_FOR_TEST <= UPPER_IMMED
                  && J9_OBJECT_HEADER_REMEMBERED_MASK_FOR_TEST >= LOWER_IMMED)
    ...
    if (TR::Compiler->target.is32Bit() && !comp->compileRelocatableCode() && constHeapBase) {
        ...
        cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, n, r5,
            new (cg->trHeapMemory()) TR::MemoryReference(metaReg, offsetof(struct J9VMThread, heapBaseForBarrierRange0),
                TR::Compiler->om.sizeofReferenceAddress(), cg),
            cursor);
    if (TR::Compiler->target.is32Bit() && !comp->compileRelocatableCode() && constHeapSize) {
        ...
        cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, n, r6,
            new (cg->trHeapMemory()) TR::MemoryReference(metaReg, offsetof(struct J9VMThread, heapSizeForBarrierRange0),
                TR::Compiler->om.sizeofReferenceAddress(), cg),
            cursor);
    ...
    cursor = generateTrg1MemInstruction(cg, Op_lclass, n, r11,
        new (cg->trHeapMemory()) TR::MemoryReference(
            r3, TR::Compiler->om.offsetOfObjectVftField(), TR::Compiler->om.sizeofReferenceField(), cg),
        cursor);
    ...
    const uint32_t helperSize = 12
        + (TR::Compiler->target.is32Bit() && !comp->compileRelocatableCode() && constHeapBase && heapBase > UPPER_IMMED
                      && heapBase < LOWER_IMMED
                  ? 1
                  : 0)
        + (TR::Compiler->target.is32Bit() && !comp->compileRelocatableCode() && constHeapSize && heapSize > UPPER_IMMED
...
static uint8_t* initializeCCPreLoadedWriteBarrierAndCardMark(
    uint8_t* buffer, void** CCPreLoadedCodeTable, TR::CodeGenerator* cg)
{
    TR::Compilation* comp = cg->comp();
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(cg->fe());
    ...
    helperTrampolineLabel->setCodeLocation((uint8_t*)TR::CodeCacheManager::instance()->findHelperTrampoline(
        buffer, TR_writeBarrierStoreGenerationalAndConcurrentMark));
    TR::Instruction* entry = generateLabelInstruction(cg, TR::InstOpCode::label, n, entryLabel);
    TR::Instruction* cursor = entry;
#if defined(J9VM_INTERP_COMPRESSED_OBJECT_HEADER)
    const TR::InstOpCode::Mnemonic Op_lclass = TR::InstOpCode::lwz;
    ...
    TR_ASSERT((J9_PRIVATE_FLAGS_CONCURRENT_MARK_ACTIVE <= UPPER_IMMED
    ...
    cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, n, r6,
        new (cg->trHeapMemory()) TR::MemoryReference(
            metaReg, offsetof(struct J9VMThread, activeCardTableBase), TR::Compiler->om.sizeofReferenceAddress(), cg),
        cursor);
    if (TR::Compiler->target.is64Bit())
    ...
    cursor = generateMemSrc1Instruction(
        cg, TR::InstOpCode::stbx, n, new (cg->trHeapMemory()) TR::MemoryReference(r6, r11, 1, cg), r5, cursor);
...
static uint8_t* initializeCCPreLoadedCardMark(uint8_t* buffer, void** CCPreLoadedCodeTable, TR::CodeGenerator* cg)
{
    ...
    cursor = generateMemSrc1Instruction(
        cg, TR::InstOpCode::stbx, n, new (cg->trHeapMemory()) TR::MemoryReference(r4, r5, 1, cg), r11, cursor);
    cursor = generateInstruction(cg, TR::InstOpCode::blr, n, cursor);
...
static uint8_t* initializeCCPreLoadedArrayStoreCHK(uint8_t* buffer, void** CCPreLoadedCodeTable, TR::CodeGenerator* cg)
{
    ...
    TR::LabelSymbol* helperTrampolineLabel = generateLabelSymbol(cg);
    helperTrampolineLabel->setCodeLocation(
        (uint8_t*)TR::CodeCacheManager::instance()->findHelperTrampoline(buffer, TR_typeCheckArrayStore));
    TR::Instruction* entry = generateLabelInstruction(cg, TR::InstOpCode::label, n, entryLabel);
    TR::Instruction* cursor = entry;
#if defined(J9VM_INTERP_COMPRESSED_OBJECT_HEADER)
    const TR::InstOpCode::Mnemonic Op_lclass = TR::InstOpCode::lwz;
    ...
    cursor = generateTrg1MemInstruction(cg, Op_lclass, n, r6,
        new (cg->trHeapMemory()) TR::MemoryReference(
            r4, TR::Compiler->om.offsetOfObjectVftField(), TR::Compiler->om.sizeofReferenceField(), cg),
        cursor);
    ...
    cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, n, r5,
        new (cg->trHeapMemory()) TR::MemoryReference(
            r5, offsetof(J9ArrayClass, componentType), TR::Compiler->om.sizeofReferenceAddress(), cg),
        cursor);
    ...
    cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, n, r7,
        new (cg->trHeapMemory())
            TR::MemoryReference(r6, offsetof(J9Class, castClassCache), TR::Compiler->om.sizeofReferenceAddress(), cg),
        cursor);
    ...
    cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, n, r7,
        new (cg->trHeapMemory())
            TR::MemoryReference(r5, offsetof(J9Class, romClass), TR::Compiler->om.sizeofReferenceAddress(), cg),
        cursor);
    cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, n, r7,
        new (cg->trHeapMemory()) TR::MemoryReference(r7, offsetof(J9ROMClass, modifiers), 4, cg), cursor);
    ...
    cursor = generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::andi_r, n, r7, r7, cr0, (J9_JAVA_CLASS_ARRAY | J9_JAVA_INTERFACE) >> 1, cursor);
    cursor = generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, n, skipSuperclassTestLabel, cr0, cursor);

    cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, n, r7,
        new (cg->trHeapMemory()) TR::MemoryReference(
            r6, offsetof(J9Class, classDepthAndFlags), TR::Compiler->om.sizeofReferenceAddress(), cg),
        cursor);
    TR_ASSERT(!(J9_JAVA_CLASS_DEPTH_MASK & ~0xffff), "Expecting class depth mask to fit in immediate");
    cursor
        = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::andi_r, n, r7, r7, cr0, J9_JAVA_CLASS_DEPTH_MASK, cursor);
    ...
    cursor = generateTrg1MemInstruction(cg, Op_lclass, n, r7,
        new (cg->trHeapMemory())
            TR::MemoryReference(r6, offsetof(J9Class, superclasses), TR::Compiler->om.sizeofReferenceField(), cg),
        cursor);
    cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, n, r7,
        new (cg->trHeapMemory()) TR::MemoryReference(r7, 0, TR::Compiler->om.sizeofReferenceAddress(), cg), cursor);
    cursor = generateTrg1Src2Instruction(cg, TR::InstOpCode::Op_cmpl, n, cr0, r5, r7, cursor);
    cursor = generateConditionalBranchInstruction(cg, TR::InstOpCode::beqlr, n, NULL, cr0, cursor);
...
uint8_t* TR::PPCAllocPrefetchSnippet::emitSnippetBody()
{
    ...
    TR_ASSERT(
        (uintptrj_t)((comp->getCurrentCodeCache())->getCCPreLoadedCodeAddress(TR_AllocPrefetch, cg())) != 0xDEADBEEF,
...
void TR::PPCAllocPrefetchSnippet::print(TR::FILE* pOutFile, TR_Debug* debug)
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(cg()->fe());
...
uint8_t* TR::PPCNonZeroAllocPrefetchSnippet::emitSnippetBody()
{
    ...
    TR_ASSERT((uintptrj_t)((comp->getCurrentCodeCache())->getCCPreLoadedCodeAddress(TR_NonZeroAllocPrefetch, cg()))
            != 0xDEADBEEF,
        "Invalid addr for code cache helper");
    intptrj_t distance
        = (intptrj_t)(comp->getCurrentCodeCache())->getCCPreLoadedCodeAddress(TR_NonZeroAllocPrefetch, cg())
        - (intptrj_t)buffer;
    opcode.setOpCodeValue(TR::InstOpCode::b);
    buffer = opcode.copyBinaryToBuffer(buffer);
    *(int32_t*)buffer |= distance & 0x03FFFFFC;
...
void TR::PPCNonZeroAllocPrefetchSnippet::print(TR::FILE* pOutFile, TR_Debug* debug)
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(cg()->fe());
    uint8_t* cursor = getSnippetLabel()->getCodeLocation();
