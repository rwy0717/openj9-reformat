...
static TR::RegisterDependencyConditions* createConditionsAndPopulateVSXDeps(
    TR::CodeGenerator* cg, int32_t nonVSXDepsCount)
{
...
static TR::Instruction* genNullTest(
    TR::Node* node, TR::Register* objectReg, TR::Register* crReg, TR::CodeGenerator* cg, TR::Instruction* cursor = NULL)
{
    ...
    TR::Register* scratchReg, TR::Instruction* cursor, TR::CodeGenerator* cg)
{
    ...
    iRet = generateTrg1Src2Instruction(
        cg, TR::InstOpCode::Op_cmpl, node, crReg, objectReg, scratchReg, (cursor == NULL) ? NULL : iRet);
#endif
    return (iRet);
}
    ...
    TR::Instruction* iCursor, TR::CodeGenerator* cg)
{
    ...
    for (i = 0; i < num_PICS - 1; i++) {
        ...
        if (cg->wantToPatchClassPointer(guessClassArray[i], node))
            iCursor = loadAddressConstantInSnippet(cg, node, (intptrj_t)(guessClassArray[i]), scratch1Reg, scratch2Reg,
                TR::InstOpCode::Op_load, true, iCursor);
        else
    ...
    if (cg->wantToPatchClassPointer(guessClassArray[num_PICS - 1], node))
        iCursor = loadAddressConstantInSnippet(cg, node, (intptrj_t)(guessClassArray[num_PICS - 1]), scratch1Reg,
            scratch2Reg, TR::InstOpCode::Op_load, true, iCursor);
    else
...
static void VMoutlinedHelperWrtbarEvaluator(
    TR::Node* node, TR::Register* srcObjectReg, TR::Register* dstObjectReg, bool srcIsNonNull, TR::CodeGenerator* cg)
{
    TR::Compilation* comp = cg->comp();
    ...
    const bool doWrtbar = gcMode == TR_WrtbarRealTime || gcMode == TR_WrtbarOldCheck
        || gcMode == TR_WrtbarCardMarkAndOldCheck || gcMode == TR_WrtbarAlways
        || TR::Options::getCmdLineOptions()->realTimeGC();

    const bool doCardMark = !node->isNonHeapObjectWrtBar()
        && (gcMode == TR_WrtbarCardMark || gcMode == TR_WrtbarCardMarkAndOldCheck
    ...
    if (!srcIsNonNull) {
        TR_ASSERT(
            NULLVALUE <= UPPER_IMMED && NULLVALUE >= LOWER_IMMED, "Expecting NULLVALUE to fit in immediate field");
...
static void VMnonNullSrcWrtBarCardCheckEvaluator(TR::Node* node, TR::Register* srcReg, TR::Register* dstReg,
    ...
    TR::Register* temp4Reg, TR::LabelSymbol* doneLabel, TR::RegisterDependencyConditions* deps,
    TR::MemoryReference* tempMR, bool dstAddrComputed, bool isCompressedRef, TR::CodeGenerator* cg,
    bool flagsInTemp1 = false)
{
    ...
    bool doWrtBar
        = (gcMode == TR_WrtbarRealTime || gcMode == TR_WrtbarOldCheck || gcMode == TR_WrtbarCardMarkAndOldCheck
            || gcMode == TR_WrtbarAlways || TR::Options::getCmdLineOptions()->realTimeGC());
    ...
    if (gcMode != TR_WrtbarAlways && !TR::Options::getCmdLineOptions()->realTimeGC()) {
        ...
        generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, tempRegister,
            new (cg->trHeapMemory()) TR::MemoryReference(metaReg, offsetof(J9VMThread, heapBaseForBarrierRange0),
        ...
        generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, temp2Reg,
            new (cg->trHeapMemory()) TR::MemoryReference(metaReg, offsetof(J9VMThread, heapSizeForBarrierRange0),
                TR::Compiler->om.sizeofReferenceAddress(), cg));
        ...
        if (!flagsInTemp1)
            generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, temp1Reg,
                new (cg->trHeapMemory()) TR::MemoryReference(dstReg, getOffsetOfJ9ObjectFlags(), 4, cg));
        ...
        if (inlineCrdmrk) {
            ...
            if (gcMode != TR_WrtbarCardMarkIncremental) {
                ...
                generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, temp2Reg,
                    new (cg->trHeapMemory()) TR::MemoryReference(
                        metaReg, offsetof(J9VMThread, privateFlags), TR::Compiler->om.sizeofReferenceAddress(), cg));
            ...
            generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, temp2Reg,
                new (cg->trHeapMemory()) TR::MemoryReference(
                    metaReg, offsetof(J9VMThread, activeCardTableBase), TR::Compiler->om.sizeofReferenceAddress(), cg));
#if defined(TR_TARGET_64BIT)
            ...
            generateMemSrc1Instruction(cg, TR::InstOpCode::stb, node,
                new (cg->trHeapMemory()) TR::MemoryReference(temp2Reg, temp3Reg, 1, cg), temp4Reg);
            ...
            if (gcMode == TR_WrtbarCardMarkAndOldCheck) {
                ...
                generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, temp2Reg,
                    new (cg->trHeapMemory()) TR::MemoryReference(metaReg,
                        offsetof(J9VMThread, heapBaseForBarrierRange0), TR::Compiler->om.sizeofReferenceAddress(), cg));
                ...
                generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, temp3Reg,
                    new (cg->trHeapMemory()) TR::MemoryReference(metaReg,
                        offsetof(J9VMThread, heapSizeForBarrierRange0), TR::Compiler->om.sizeofReferenceAddress(), cg));
            ...
            if (gcMode == TR_WrtbarCardMarkAndOldCheck || gcMode == TR_WrtbarOldCheck) {
                ...
                if (temp4Reg != NULL) {
                    ...
                    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, temp3Reg,
                        new (cg->trHeapMemory())
                            TR::MemoryReference(metaReg, offsetof(J9VMThread, heapBaseForBarrierRange0),
                                TR::Compiler->om.sizeofReferenceAddress(), cg));
        ...
        TR::Instruction* i = generateTrg1Src1ImmInstruction(
            cg, TR::InstOpCode::andi_r, node, temp2Reg, temp1Reg, condReg, J9_OBJECT_HEADER_REMEMBERED_MASK_FOR_TEST);
        ...
        if (!dstAddrComputed) {
            if (tempMR->getIndexRegister() != NULL)
                generateTrg1Src2Instruction(
                    cg, TR::InstOpCode::add, node, temp3Reg, tempMR->getBaseRegister(), tempMR->getIndexRegister());
            else
        ...
        if (!comp->getOption(TR_DisableInlineWriteBarriersRT)) {
            ...
            TR::MemoryReference* fragmentParentMR
                = new (cg->trHeapMemory()) TR::MemoryReference(cg->getMethodMetaDataRegister(),
            ...
            TR::MemoryReference* globalFragmentIDMR = new (cg->trHeapMemory()) TR::MemoryReference(
                temp2Reg, fej9->getRememberedSetGlobalFragmentOffset(), TR::Compiler->om.sizeofReferenceAddress(), cg);
            ...
            generateTrg1MemInstruction(
                cg, opCode, node, temp1Reg, new (cg->trHeapMemory()) TR::MemoryReference(temp3Reg, 0, size, cg));
            generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::Op_cmpi, node, condReg, temp1Reg, NULLVALUE);
            generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, node, doneLabel, condReg);
    ...
    generateDepImmSymInstruction(cg, TR::InstOpCode::bl, node,
        (uintptrj_t)wbRef->getSymbol()->castToMethodSymbol()->getMethodAddress(),
...
static void VMwrtbarEvaluator(TR::Node* node, TR::Register* srcReg, TR::Register* dstReg, TR::Register* dstAddrReg,
    TR::MemoryReference* tempMR, TR::RegisterDependencyConditions* conditions, bool srcNonNull, bool needDeps,
    bool isCompressedRef, TR::CodeGenerator* cg, TR::Register* flagsReg = NULL)
{
    TR::Compilation* comp = cg->comp();
    TR_WriteBarrierKind gcMode = comp->getOptions()->getGcMode();
    bool doWrtBar
    ...
    if (doWrtBar) {
        ...
        VMnonNullSrcWrtBarCardCheckEvaluator(node, srcReg, dstReg, cr0, temp1Reg, temp2Reg, temp3Reg, temp4Reg, label,
            conditions, tempMR, false, isCompressedRef, cg, flagsReg != NULL);
...
TR::Register* J9::Power::TreeEvaluator::awrtbarEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!node->skipWrtBar() && !node->hasUnresolvedSymbolReference()
        ...
        generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, flagsReg,
            new (cg->trHeapMemory()) TR::MemoryReference(destinationRegister, getOffsetOfJ9ObjectFlags(), 4, cg));
    }
    ...
    if (TR::Options::getCmdLineOptions()->realTimeGC()) {
        ...
        VMwrtbarEvaluator(node, sourceRegister, destinationRegister, destinationAddressRegister, tempMR, NULL,
            firstChild->isNonNull(), true, false, cg);

        TR::MemoryReference* dstMR = new (cg->trHeapMemory())
            TR::MemoryReference(destinationAddressRegister, 0, TR::Compiler->om.sizeofReferenceAddress(), cg);
        ...
        VMwrtbarEvaluator(node, sourceRegister, destinationRegister, NULL, NULL, NULL, firstChild->isNonNull(), true,
            false, cg, flagsReg);
    }
...
TR::Register* J9::Power::TreeEvaluator::awrtbariEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Options::getCmdLineOptions()->realTimeGC()) {
        ...
        VMwrtbarEvaluator(node, sourceRegister, destinationRegister, NULL, NULL, NULL, secondChild->isNonNull(), true,
...
TR::Register* J9::Power::TreeEvaluator::monexitfenceEvaluator(TR::Node* node, TR::CodeGenerator* cg) { return NULL; }
...
TR::Register* J9::Power::TreeEvaluator::asynccheckEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR_ASSERT(testNode->getOpCodeValue() == (TR::Compiler->target.is64Bit() ? TR::lcmpeq : TR::icmpeq),
...
#endif
    {
        ...
        if (TR::Compiler->target.is64Bit()) {
            ...
            generateTrg1Src1ImmInstruction(
                cg, TR::InstOpCode::cmpi8, node, condReg, src1Reg, secondChild->getLongInt());
        } else {
            TR_ASSERT(secondChild->getInt() == -1, "asynccheck bad format");
        ...
        if (TR::Compiler->target.cpu.id() >= TR_PPCgp)
            ...
            gcPoint = generateDepConditionalBranchInstruction(
                cg, TR::InstOpCode::beql, PPCOpProp_BranchUnlikely, node, snippetLabel, condReg, dependencies);
        else
            gcPoint = generateDepConditionalBranchInstruction(
                cg, TR::InstOpCode::beql, node, snippetLabel, condReg, dependencies);
...
{
    TR_ASSERT(
        cg->comp()->requiresSpineChecks(), "TR::arraylength should be lowered when hybrid arraylets are not in use");
    ...
    TR::MemoryReference* contiguousArraySizeMR
        = new (cg->trHeapMemory()) TR::MemoryReference(objectReg, fej9->getOffsetOfContiguousArraySizeField(), 4, cg);
    TR::MemoryReference* discontiguousArraySizeMR = new (cg->trHeapMemory())
        TR::MemoryReference(objectReg, fej9->getOffsetOfDiscontiguousArraySizeField(), 4, cg);
    ...
    if (TR::Compiler->target.cpu.id() >= TR_PPCgp) // Use PPC AS branch hint.
        generateConditionalBranchInstruction(
            cg, TR::InstOpCode::beq, PPCOpProp_BranchUnlikely, node, discontiguousArrayLabel, condReg);
    else
    ...
    if (!constDivisor || (type.isInt32() && secondChild->getInt() == 0)
        ...
        if (!constDivisor || cg->getHasResumableTrapHandler()) {
            ...
            if (type.isInt64() && TR::Compiler->target.is32Bit()) {
                ...
                generateTrg1Src2Instruction(
                    cg, TR::InstOpCode::OR, node, trgReg, srcReg->getHighOrder(), srcReg->getLowOrder());
                srcReg = trgReg;
                killSrc = true;
            }
        }
    ...
    TR::Register* arrayLengthReg, int32_t arrayLengthVal, TR::Register* condReg, bool noTrap)
{
    ...
    if (noTrap) {
        ...
        gcPoint = TR::Compiler->target.cpu.id() >= TR_PPCgp
            ? // Use PPC AS branch hint.
            generateConditionalBranchInstruction(
                cg, TR::InstOpCode::blel, PPCOpProp_BranchUnlikely, node, boundCheckFailSnippetLabel, condReg)
...
TR::Register* J9::Power::TreeEvaluator::BNDCHKEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (firstChild->getOpCode().isLoadConst() && firstChild->getInt() <= UPPER_IMMED
    ...
    if (!noTrap) {
        ...
        if (TR::Compiler->target.cpu.id() >= TR_PPCgp)
            // use PPC AS branch hint
            gcPoint
                = generateDepConditionalBranchInstruction(cg, reversed ? TR::InstOpCode::bgel : TR::InstOpCode::blel,
                    PPCOpProp_BranchUnlikely, node, snippetLabel, cndReg, conditions);
        else
            gcPoint = generateDepConditionalBranchInstruction(
                cg, reversed ? TR::InstOpCode::bgel : TR::InstOpCode::blel, node, snippetLabel, cndReg, conditions);
...
TR::Register* J9::Power::TreeEvaluator::ArrayCopyBNDCHKEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (directCase) {
    ...
    } else if (firstChild->getOpCode().isLoadConst() && firstChild->getInt() <= UPPER_IMMED
        ...
        if (noTrap) {
            ...
            if (TR::Compiler->target.cpu.id() >= TR_PPCgp)
                // use PPC AS branch hint
                gcPoint = generateConditionalBranchInstruction(
                    cg, TR::InstOpCode::bgtl, PPCOpProp_BranchUnlikely, node, snippetLabel, cndReg);
            else
        ...
        if (secondChild->getOpCode().isLoadConst() && secondChild->getInt() <= UPPER_IMMED) {
            if (noTrap) {
                TR::Register* cndReg = cg->allocateRegister(TR_CCR);
                generateTrg1Src1ImmInstruction(
                    cg, TR::InstOpCode::cmpi4, node, cndReg, boundReg, secondChild->getInt());
                if (TR::Compiler->target.cpu.id() >= TR_PPCgp)
                    // use PPC AS branch hint
                    gcPoint = generateConditionalBranchInstruction(
                        cg, TR::InstOpCode::bltl, PPCOpProp_BranchUnlikely, node, snippetLabel, cndReg);
                else
                    gcPoint
                        = generateConditionalBranchInstruction(cg, TR::InstOpCode::bltl, node, snippetLabel, cndReg);
                cg->stopUsingRegister(cndReg);
            } else {
...
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(cg->fe());
    TR::MemoryReference* contiguousArraySizeMR = new (cg->trHeapMemory())
        TR::MemoryReference(baseArrayReg, fej9->getOffsetOfContiguousArraySizeField(), 4, cg);
    ...
    TR::Register* arrayletReg, TR::Register* offsetReg, int32_t& offsetVal)
{
    ...
    if (indexReg) {
        ...
        generateTrg1Src1Imm2Instruction(
            cg, TR::InstOpCode::rlwinm, node, arrayletReg, arrayletReg, 0, ~(spinePointerSize - 1));
    ...
    //
    generateTrg1MemInstruction(
        cg, spinePointerSize == 8 ? TR::InstOpCode::ld : TR::InstOpCode::lwz, node, arrayletReg, spineMR);
    ...
    if (indexReg) {
        ...
        generateTrg1Src1Imm2Instruction(
            cg, TR::InstOpCode::rlwinm, node, offsetReg, indexReg, elementSizeShift, arrayletMask << elementSizeShift);
    } else
        offsetVal = (fej9->getLeafElementIndex(indexVal, elementSize) * elementSize);
}

static TR::InstOpCode::Mnemonic getLoadOrStoreFromDataType(
    TR::CodeGenerator* cg, TR::DataType dt, int32_t elementSize, bool isUnsigned, bool returnLoad)
{
    switch (dt) {
    case TR::Int8:
...
static void genDecompressPointer(
    TR::CodeGenerator* cg, TR::Node* node, TR::Register* ptrReg, TR::Register* condReg = NULL, bool nullCheck = true)
{
...
static TR::Register* genDecompressPointerNonNull2Regs(
    TR::CodeGenerator* cg, TR::Node* node, TR::Register* ptrReg, TR::Register* targetReg)
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(cg->comp()->fe());
    uintptrj_t heapBase = TR::Compiler->vm.heapBaseAddress();
    int32_t shiftAmount = TR::Compiler->om.compressedReferenceShift();
...
static TR::Register* genDecompressPointerNonNull2RegsWithTempReg(
    TR::CodeGenerator* cg, TR::Node* node, TR::Register* ptrReg, TR::Register* targetReg, TR::Register* tempReg)
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(cg->comp()->fe());
    uintptrj_t heapBase = TR::Compiler->vm.heapBaseAddress();
    int32_t shiftAmount = TR::Compiler->om.compressedReferenceShift();
...
static void genCompressPointerWithTempReg(TR::CodeGenerator* cg, TR::Node* node, TR::Register* ptrReg,
    TR::Register* tempReg, TR::Register* condReg = NULL, bool nullCheck = true)
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(cg->comp()->fe());
    uintptrj_t heapBase = TR::Compiler->vm.heapBaseAddress();
...
static void genCompressPointer(
    TR::CodeGenerator* cg, TR::Node* node, TR::Register* ptrReg, TR::Register* condReg = NULL, bool nullCheck = true)
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(cg->comp()->fe());
    uintptrj_t heapBase = TR::Compiler->vm.heapBaseAddress();
...
static TR::Register* genCompressPointerNonNull2RegsWithTempReg(
    TR::CodeGenerator* cg, TR::Node* node, TR::Register* ptrReg, TR::Register* targetReg, TR::Register* tempReg)
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(cg->comp()->fe());
    uintptrj_t heapBase = TR::Compiler->vm.heapBaseAddress();
...
static TR::Register* genCompressPointerNonNull2Regs(
    TR::CodeGenerator* cg, TR::Node* node, TR::Register* ptrReg, TR::Register* targetReg)
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(cg->comp()->fe());
    uintptrj_t heapBase = TR::Compiler->vm.heapBaseAddress();
...
TR::Register* J9::Power::TreeEvaluator::BNDCHKwithSpineCHKEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!indexChild->getOpCode().isLoadConst()
        || (indexChild->getInt() > UPPER_IMMED || indexChild->getInt() < LOWER_IMMED))
    ...
    // Label to the OOL code that will perform the load/store/agen for discontigous arrays (and the bound check if
    // needed).
    TR::LabelSymbol* discontiguousArrayLabel = generateLabelSymbol(cg);
    ...
    if (needsBoundCheck) {
        ...
        TR_ASSERT(arrayLengthChild->getOpCode().isConversion() || arrayLengthChild->getOpCodeValue() == TR::iloadi
            ...
            "Expecting array length child under BNDCHKwithSpineCHK to be a conversion, iiload, iload, iRegLoad or "
            "iconst");
        ...
        if (arrayLengthReg) {
            ...
            genBoundCheck(
                cg, node, indexReg, indexChild->getInt(), arrayLengthReg, arrayLengthChild->getInt(), condReg, noTrap);
    ...
    if (loadOrStoreChild->getOpCode().isStore() && loadOrStoreChild->getReferenceCount() > 1) {
        ...
        if ((loadOrStoreChild->getOpCodeValue() == TR::aload || loadOrStoreChild->getOpCodeValue() == TR::aRegLoad)
            && node->isSpineCheckWithArrayElementChild() && TR::Compiler->target.is64Bit()
    ...
    discontiguousArrayOOL->swapInstructionListsWithCompilation();
    {
        ...
        if (needsBoundCheckOOL) {
            ...
            TR::MemoryReference* discontiguousArraySizeMR = new (cg->trHeapMemory())
                TR::MemoryReference(baseArrayReg, fej9->getOffsetOfDiscontiguousArraySizeField(), 4, cg);
            ...
            if (doLoadOrStore) {
                ...
                if (loadOrStoreChild->getOpCode().isStore()) {
                    ...
                    if (storeOp == TR::InstOpCode::std && TR::Compiler->target.is32Bit()) {
                        ...
                        TR_ASSERT(
                            elementSize == 8 && loadOrStoreChild->getSecondChild()->getRegister()->getRegisterPair(),
                            "Expecting 64-bit store value to be in a register pair");
                        if (indexReg) {
                            TR::MemoryReference* arrayletMR
                                = new (cg->trHeapMemory()) TR::MemoryReference(arrayletReg, arrayletOffsetReg, 4, cg);
                            generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node, arrayletMR,
                                loadOrStoreChild->getSecondChild()->getRegister()->getHighOrder());

                            generateTrg1Src1ImmInstruction(
                                cg, TR::InstOpCode::addi, node, arrayletOffsetReg, arrayletOffsetReg, 4);
                            ...
                            TR::MemoryReference* arrayletMR
                                = new (cg->trHeapMemory()) TR::MemoryReference(arrayletReg, arrayletOffsetVal, 4, cg);
                            TR::MemoryReference* arrayletMR2 = new (cg->trHeapMemory())
                                TR::MemoryReference(arrayletReg, arrayletOffsetVal + 4, 4, cg);
                            generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node, arrayletMR,
                                loadOrStoreChild->getSecondChild()->getRegister()->getHighOrder());
                            generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node, arrayletMR2,
                                loadOrStoreChild->getSecondChild()->getRegister()->getLowOrder());
                        ...
                        TR::MemoryReference* arrayletMR = indexReg
                            ? new (cg->trHeapMemory())
                                  TR::MemoryReference(arrayletReg, arrayletOffsetReg, elementSize, cg)
                            : new (cg->trHeapMemory())
                                  TR::MemoryReference(arrayletReg, arrayletOffsetVal, elementSize, cg);
                        generateMemSrc1Instruction(
                            cg, storeOp, node, arrayletMR, loadOrStoreChild->getSecondChild()->getRegister());
                    }
                } else {
                    ...
                    if (loadOp == TR::InstOpCode::ld && TR::Compiler->target.is32Bit()) {
                        ...
                        if (indexReg) {
                            TR::MemoryReference* arrayletMR
                                = new (cg->trHeapMemory()) TR::MemoryReference(arrayletReg, arrayletOffsetReg, 4, cg);
                            generateTrg1MemInstruction(
                                cg, TR::InstOpCode::lwz, node, loadOrStoreReg->getHighOrder(), arrayletMR);
                            ...
                            TR::MemoryReference* arrayletMR2
                                = new (cg->trHeapMemory()) TR::MemoryReference(arrayletReg, arrayletOffsetReg, 4, cg);
                            generateTrg1MemInstruction(
                                cg, TR::InstOpCode::lwz, node, loadOrStoreReg->getLowOrder(), arrayletMR2);
                        } else {
                            TR::MemoryReference* arrayletMR
                                = new (cg->trHeapMemory()) TR::MemoryReference(arrayletReg, arrayletOffsetVal, 4, cg);
                            TR::MemoryReference* arrayletMR2 = new (cg->trHeapMemory())
                ...
                if (indexReg)
                    generateTrg1Src2Instruction(
                        cg, TR::InstOpCode::add, node, loadOrStoreReg, loadOrStoreReg, arrayletOffsetReg);
                else
    ...
    TR::Node* node, TR::Register* srcReg, TR::Register* dstReg, bool srcIsNonNull, TR::CodeGenerator* cg)
{
    ...
    if (cg->wantToPatchClassPointer(rootClass, node))
        loadAddressConstantInSnippet(
            cg, node, (intptrj_t)rootClass, rootClassReg, scratchReg, TR::InstOpCode::Op_load, false, NULL);
    else
...
TR::Instruction* J9::Power::TreeEvaluator::generateVFTMaskInstruction(
    TR::CodeGenerator* cg, TR::Node* node, TR::Register* dstReg, TR::Register* srcReg, TR::Instruction* preced)
{
    ...
    if (~mask == 0) {
        ...
        TR::InstOpCode::Mnemonic op
            = (TR::Compiler->target.is64Bit() && !isCompressed) ? TR::InstOpCode::rldicr : TR::InstOpCode::rlwinm;
...
static TR::Instruction* genTestIsSuper(TR::Node* node, TR::Register* objClassReg, TR::Register* castClassReg,
    ...
    TR::LabelSymbol* failLabel, TR::Instruction* cursor, TR::CodeGenerator* cg, int32_t depthInReg2 = 0)
{
    TR::Compilation* comp = cg->comp();
    ...
    cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, scratch1Reg,
        new (cg->trHeapMemory()) TR::MemoryReference(
            objClassReg, offsetof(J9Class, classDepthAndFlags), TR::Compiler->om.sizeofReferenceAddress(), cg),
        cursor);
    if (outOfBound) {
        cursor = loadConstant(cg, node, castClassDepth, scratch2Reg, cursor);
    }
    cursor = generateTrg1Src1Imm2Instruction(
        cg, TR::InstOpCode::rlwinm, node, scratch1Reg, scratch1Reg, 0, J9_JAVA_CLASS_DEPTH_MASK, cursor);
    ...
    if (outOfBound || depthInReg2) {
        ...
        cursor = generateTrg1Src1ImmInstruction(
            cg, TR::InstOpCode::cmpi4, node, crReg, scratch1Reg, castClassDepth, cursor);
    }
    ...
    cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, scratch1Reg,
        new (cg->trHeapMemory()) TR::MemoryReference(
            objClassReg, offsetof(J9Class, superclasses), TR::Compiler->om.sizeofReferenceAddress(), cg),
        cursor);

    if (outOfBound || depthInReg2) {
        cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_loadx, node, scratch1Reg,
            new (cg->trHeapMemory())
                TR::MemoryReference(scratch1Reg, scratch2Reg, TR::Compiler->om.sizeofReferenceAddress(), cg),
            cursor);
    } else {
        cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, scratch1Reg,
            new (cg->trHeapMemory())
                TR::MemoryReference(scratch1Reg, superClassOffset, TR::Compiler->om.sizeofReferenceAddress(), cg),
            cursor);
    }
#ifdef J9VM_INTERP_COMPRESSED_OBJECT_HEADER
    ...
    TR::LabelSymbol* fLabel, TR::CodeGenerator* cg)
{
    ...
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, t1Reg,
        new (cg->trHeapMemory()) TR::MemoryReference(
            t1Reg, (int32_t)offsetof(J9ArrayClass, componentType), TR::Compiler->om.sizeofReferenceAddress(), cg));
    // here we may need to convert from J9Class pointer contained in t1Reg
    // into class offset (so that we can compare two classes)
#else
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, t1Reg,
        new (cg->trHeapMemory()) TR::MemoryReference(
            dst, (int32_t)TR::Compiler->om.offsetOfObjectVftField(), TR::Compiler->om.sizeofReferenceAddress(), cg));
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, t2Reg,
        new (cg->trHeapMemory()) TR::MemoryReference(
            src, (int32_t)TR::Compiler->om.offsetOfObjectVftField(), TR::Compiler->om.sizeofReferenceAddress(), cg));
    TR::TreeEvaluator::generateVFTMaskInstruction(cg, node, t1Reg);
    TR::TreeEvaluator::generateVFTMaskInstruction(cg, node, t2Reg);
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, t1Reg,
        new (cg->trHeapMemory()) TR::MemoryReference(
    ...
    if (cg->wantToPatchClassPointer(rootClass, node))
        loadAddressConstantInSnippet(
            cg, node, (intptrj_t)rootClass, t3Reg, t4Reg, TR::InstOpCode::Op_load, false, NULL);
    else
        loadAddressConstant(cg, node, (intptrj_t)rootClass, t3Reg);

    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, t4Reg,
        new (cg->trHeapMemory()) TR::MemoryReference(
            t2Reg, (int32_t)offsetof(J9Class, castClassCache), TR::Compiler->om.sizeofReferenceAddress(), cg));
    ...
    if ((!comp->getOption(TR_DisableArrayStoreCheckOpts)) && node->getArrayComponentClassInNode()) {
        ...
        if (cg->wantToPatchClassPointer(castClass, node))
            loadAddressConstantInSnippet(
                cg, node, (intptrj_t)castClass, t3Reg, t4Reg, TR::InstOpCode::Op_load, false, NULL);
        else
    ...
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, t3Reg,
        new (cg->trHeapMemory()) TR::MemoryReference(
            t1Reg, (int32_t)offsetof(J9Class, romClass), TR::Compiler->om.sizeofReferenceAddress(), cg));
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, t4Reg,
        new (cg->trHeapMemory()) TR::MemoryReference(
            t1Reg, (int32_t)offsetof(J9Class, classDepthAndFlags), TR::Compiler->om.sizeofReferenceAddress(), cg));
    generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, t3Reg,
        new (cg->trHeapMemory()) TR::MemoryReference(t3Reg, (int32_t)offsetof(J9ROMClass, modifiers), 4, cg));
    ...
    TR_ASSERT(
        J9_JAVA_CLASS_ARRAY == (1 << 16) && J9_JAVA_INTERFACE == (1 << 9), "Verify code sequence is still right ...");
    generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::andi_r, node, t3Reg, t3Reg, cndReg, (J9_JAVA_CLASS_ARRAY | J9_JAVA_INTERFACE) >> 1);
...
TR::Register* J9::Power::TreeEvaluator::ArrayStoreCHKEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!comp->compileRelocatableCode() && !TR::Options::getCmdLineOptions()->realTimeGC()
        && !comp->useCompressedPointers() && !oldArrayStoreCHK)
    ...
    bool doWrtBar = (gcMode == TR_WrtbarRealTime || gcMode == TR_WrtbarOldCheck
                        || gcMode == TR_WrtbarCardMarkAndOldCheck || gcMode == TR_WrtbarAlways)
        || (TR::Options::getCmdLineOptions()->realTimeGC());
    ...
    generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, node,
        ((!TR::Options::getCmdLineOptions()->realTimeGC()) && doWrtBar) ? storeLabel : wbLabel, condReg);

    TR::LabelSymbol* helperCall = generateLabelSymbol(cg);
    VMarrayStoreCHKEvaluator(
        node, srcReg, dstReg, temp1Reg, temp2Reg, temp3Reg, temp4Reg, condReg, wbLabel, helperCall, cg);
    ...
    if (!firstChild->skipWrtBar()) {
        if (doCrdMrk && !doWrtBar)
            ...
            VMnonNullSrcWrtBarCardCheckEvaluator(node, srcReg, dstReg, condReg, temp1Reg, temp2Reg, temp3Reg, temp4Reg,
                doneLabel, conditions, NULL, false, usingCompressedPointers, cg);
        else if (doWrtBar && !doCrdMrk)
            VMnonNullSrcWrtBarCardCheckEvaluator(node, srcReg, dstReg, condReg, temp1Reg, temp2Reg, dstAddrReg,
                temp4Reg, wrtBarEndLabel, conditions, tempMR1, false, usingCompressedPointers, cg);
    }
...
TR::Register* J9::Power::TreeEvaluator::conditionalHelperEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    for (i = numArgs - 1; i >= 0; i--) {
        ...
        addDependency(conditions, argReg,
    ...
    TR::LabelSymbol* oppositeResultLabel, bool profiledClassIsInstanceOf, TR::CodeGenerator* cg)
{
    ...
    if (current == J9::TreeEvaluator::NullTest) {
        generateConditionalBranchInstruction(
            cg, TR::InstOpCode::beq, node, resultRegInitialValue == false ? doneLabel : oppositeResultLabel, condReg);
    ...
    TR_PPCScratchRegisterManager* srm, TR::CodeGenerator* cg)
{
    ...
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, instanceClassDepthReg,
        new (cg->trHeapMemory()) TR::MemoryReference(
            instanceClassReg, offsetof(J9Class, classDepthAndFlags), TR::Compiler->om.sizeofReferenceAddress(), cg));
    static_assert(J9_JAVA_CLASS_DEPTH_MASK < UINT_MAX, "Class depth is not containable in 32 bits");
    generateTrg1Src1Imm2Instruction(
        cg, TR::InstOpCode::rlwinm, node, instanceClassDepthReg, instanceClassDepthReg, 0, J9_JAVA_CLASS_DEPTH_MASK);
    ...
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, instanceClassSuperClassesArrayReg,
        new (cg->trHeapMemory()) TR::MemoryReference(
            instanceClassReg, offsetof(J9Class, superclasses), TR::Compiler->om.sizeofReferenceAddress(), cg));
    ...
    if (castClassDepthOffset <= UPPER_IMMED && castClassDepthOffset >= LOWER_IMMED)
        generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, instanceClassSuperClassReg,
            new (cg->trHeapMemory()) TR::MemoryReference(instanceClassSuperClassesArrayReg, castClassDepthOffset,
                TR::Compiler->om.sizeofReferenceAddress(), cg));
    else {
        if (!castClassDepthReg) {
            castClassDepthReg = srm->findOrCreateScratchRegister();
            generateTrg1ImmInstruction(
                cg, TR::InstOpCode::lis, node, castClassDepthReg, HI_VALUE(castClassDepthOffset));
            generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, instanceClassSuperClassReg,
                new (cg->trHeapMemory()) TR::MemoryReference(instanceClassSuperClassesArrayReg,
                    LO_VALUE(castClassDepthOffset), TR::Compiler->om.sizeofReferenceAddress(), cg));
        } else {
            if (TR::Compiler->target.is64Bit())
            ...
            generateTrg1MemInstruction(cg, TR::InstOpCode::Op_loadx, node, instanceClassSuperClassReg,
                new (cg->trHeapMemory()) TR::MemoryReference(instanceClassSuperClassesArrayReg, castClassDepthReg,
                    TR::Compiler->om.sizeofReferenceAddress(), cg));
        }
    }
    ...
    TR::Register* castClassReg, TR_PPCScratchRegisterManager* srm, TR::CodeGenerator* cg)
{
    ...
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, castClassCacheReg,
        new (cg->trHeapMemory()) TR::MemoryReference(
            instanceClassReg, offsetof(J9Class, castClassCache), TR::Compiler->om.sizeofReferenceAddress(), cg));
    ...
    TR::CodeGenerator* cg)
{
    ...
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, scratchReg,
        new (cg->trHeapMemory()) TR::MemoryReference(
            instanceClassReg, offsetof(J9Class, romClass), TR::Compiler->om.sizeofReferenceAddress(), cg));
    generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, scratchReg,
        new (cg->trHeapMemory()) TR::MemoryReference(scratchReg, offsetof(J9ROMClass, modifiers), 4, cg));
    if (J9AccClassArray <= UPPER_IMMED && J9AccClassArray >= LOWER_IMMED)
        generateTrg1Src1ImmInstruction(
            cg, TR::InstOpCode::andi_r, node, scratchReg, scratchReg, cr0Reg, J9AccClassArray);
    else
        generateTrg1Src1ImmInstruction(
            cg, TR::InstOpCode::andis_r, node, scratchReg, scratchReg, cr0Reg, J9AccClassArray >> 16);
    ...
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, scratchReg,
        new (cg->trHeapMemory()) TR::MemoryReference(
            instanceClassReg, offsetof(J9ArrayClass, componentType), TR::Compiler->om.sizeofReferenceAddress(), cg));
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, scratchReg,
        new (cg->trHeapMemory()) TR::MemoryReference(
            scratchReg, offsetof(J9Class, romClass), TR::Compiler->om.sizeofReferenceAddress(), cg));
    generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, scratchReg,
        new (cg->trHeapMemory()) TR::MemoryReference(scratchReg, offsetof(J9ROMClass, modifiers), 4, cg));
    if (J9AccClassInternalPrimitiveType <= UPPER_IMMED && J9AccClassInternalPrimitiveType >= LOWER_IMMED)
        generateTrg1Src1ImmInstruction(
            cg, TR::InstOpCode::andi_r, node, scratchReg, scratchReg, cr0Reg, J9AccClassInternalPrimitiveType);
    else
        generateTrg1Src1ImmInstruction(
            cg, TR::InstOpCode::andis_r, node, scratchReg, scratchReg, cr0Reg, J9AccClassInternalPrimitiveType >> 16);
...
static void genInstanceOfOrCheckCastHelperCall(
    TR::Node* node, TR::Register* objectReg, TR::Register* castClassReg, TR::Register* resultReg, TR::CodeGenerator* cg)
{
    ...
    TR::Instruction* gcPoint = generateDepImmSymInstruction(cg, TR::InstOpCode::bl, node,
        (uintptrj_t)node->getSymbolReference()->getMethodAddress(), deps, node->getSymbolReference());
    gcPoint->PPCNeedsGCMap(0xFFFFFFFF);
...
TR::Register* J9::Power::TreeEvaluator::VMcheckcastEvaluator2(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    while (numSequencesRemaining > 1) {
        ...
        case LoadObjectClass:
            ...
            generateTrg1MemInstruction(cg,
                ...
                node, objectClassReg,
                new (cg->trHeapMemory()) TR::MemoryReference(
                    objectReg, TR::Compiler->om.offsetOfObjectVftField(), TR::Compiler->om.sizeofReferenceField(), cg));
        ...
        case ClassEqualityTest:
            if (comp->getOption(TR_TraceCG))
                traceMsg(comp, "%s: Emitting ClassEqualityTest\n", node->getOpCode().getName());
            genInstanceOfOrCheckCastClassEqualityTest(node, cr0Reg, objectClassReg, castClassReg, cg);
            break;
        case SuperClassTest: {
            if (comp->getOption(TR_TraceCG))
                traceMsg(comp, "%s: Emitting SuperClassTest\n", node->getOpCode().getName());
            int32_t castClassDepth = castClassNode->getSymbolReference()->classDepth(comp);
            genInstanceOfOrCheckCastSuperClassTest(
                node, cr0Reg, objectClassReg, castClassReg, castClassDepth, nextSequenceLabel, srm, cg);
            break;
        }
        case ProfiledClassTest:
            if (comp->getOption(TR_TraceCG))
                traceMsg(comp, "%s: Emitting ProfiledClassTest\n", node->getOpCode().getName());
            genInstanceOfOrCheckCastArbitraryClassTest(
                node, cr0Reg, objectClassReg, profiledClassesList[0].profiledClass, srm, cg);
            break;
        case CompileTimeGuessClassTest:
            if (comp->getOption(TR_TraceCG))
                traceMsg(comp, "%s: Emitting CompileTimeGuessClassTest\n", node->getOpCode().getName());
    ...
    cg->generateDebugCounter(
        ...
        *srm);
    cg->generateDebugCounter(
        TR::DebugCounter::debugCounterName(comp, "instanceOfOrCheckCast.perMethod/%s/(%s)/%d/%d/fastPath",
    ...
    if (numSequencesRemaining > 0 && *iter != GoToTrue) {
        ...
        if (comp->getOption(TR_TraceCG))
            traceMsg(comp, "%s: Emitting HelperCall%s\n", node->getOpCode().getName(),
...
TR::Register* J9::Power::TreeEvaluator::VMinstanceOfEvaluator2(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    while (numSequencesRemaining > 1 || (numSequencesRemaining == 1 && *iter != HelperCall)) {
        ...
        case DynamicCacheObjectClassTest:
            TR_ASSERT_FATAL(
                false, "%s: DynamicCacheObjectClassTest is not implemented on P\n", node->getOpCode().getName());
            break;
        case DynamicCacheDynamicCastClassTest:
            TR_ASSERT_FATAL(
                false, "%s: DynamicCacheDynamicCastClassTest is not implemented on P\n", node->getOpCode().getName());
            break;
        case HelperCall:
            TR_ASSERT(false, "Doesn't make sense, HelperCall should be the terminal sequence");
            break;
    ...
    if (numSequencesRemaining > 0) {
        if (*iter == HelperCall) {
            if (comp->getOption(TR_TraceCG))
                traceMsg(comp, "%s: Emitting HelperCall\n", node->getOpCode().getName());
            TR_PPCOutOfLineCodeSection* outlinedHelperCall = new (cg->trHeapMemory())
            ...
            cg->generateDebugCounter(nextSequenceLabel->getInstruction(),
                TR::DebugCounter::debugCounterName(
                    comp, "instanceOfOrCheckCast/%s/helperCall", node->getOpCode().getName()));
            cg->generateDebugCounter(nextSequenceLabel->getInstruction(),
...
TR::Register* J9::Power::TreeEvaluator::VMcheckcastEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    bool testCache = needTestCache(!comp->getOption(TR_DisableInlineCheckCast), needsHelperCall, testCastClassIsSuper,
        castClassAddr, numberOfValidGuessClasses);
    bool needsNullTest = !objNode->isNonNull();
    ...
    if (!testEqualClass && !testCache && !testCastClassIsSuper) {
        gcPoint = generateDepImmSymInstruction(cg, TR::InstOpCode::bl, node,
            (uintptrj_t)node->getSymbolReference()->getMethodAddress(), conditions, node->getSymbolReference());
        gcPoint->PPCNeedsGCMap(0xFFFFFFFF);
    ...
    bool skipNullTest = ((TR::Compiler->target.is32Bit() && TR::Compiler->target.isAIX())
                            && (!testCastClassIsSuper
                                   || castClassSymRef->classDepth(comp)
                                       < (AIX_NULL_ZERO_AREA_SIZE / TR::Compiler->om.sizeofReferenceAddress()))
                            && offsetof(J9Class, classDepthAndFlags) < AIX_NULL_ZERO_AREA_SIZE
                            && offsetof(J9Class, superclasses) < AIX_NULL_ZERO_AREA_SIZE)
        && !dontSkipNullTest;
    ...
    generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, objClassReg,
        new (cg->trHeapMemory())
            TR::MemoryReference(objReg, (int32_t)TR::Compiler->om.offsetOfObjectVftField(), 4, cg));
#else
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, objClassReg,
        new (cg->trHeapMemory()) TR::MemoryReference(
            objReg, (int32_t)TR::Compiler->om.offsetOfObjectVftField(), TR::Compiler->om.sizeofReferenceAddress(), cg));
#endif
    ...
    if (testEqualClass) {
        ...
        if (testCache || testCastClassIsSuper) {
            ...
            if (!comp->getOption(TR_DisableOOL) && !disableOOLcheckcast) {
                ...
                TR::Instruction* OOLBranchInstr
                    = generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, node, startOOLLabel, cndReg);
                if (debugObj)
                ...
                TR::Instruction* OOLLabelInstr
                    = generateLabelInstruction(cg, TR::InstOpCode::label, node, startOOLLabel);
                if (debugObj)
    ...
    if (outlinedSlowPath) {
        snippet = new (cg->trHeapMemory())
            TR::PPCHelperCallSnippet(cg, node, callLabel, node->getSymbolReference(), helperReturnOOLLabel);
    } else {
        snippet = new (cg->trHeapMemory())
            TR::PPCHelperCallSnippet(cg, node, callLabel, node->getSymbolReference(), doneLabel);
    }
    ...
    if (testCache) {
        ...
        if (outlinedSlowPath) {
            genInlineTest(node, castClassAddr, validGuessClassArray, numberOfValidGuessClasses, objClassReg, NULL,
                cndReg, scratch1Reg, scratch2Reg, true, false, helperReturnOOLLabel, helperReturnOOLLabel,
                helperReturnOOLLabel, callForInlineLabel, testCastClassIsSuper, NULL, cg);
        } else {
            genInlineTest(node, castClassAddr, validGuessClassArray, numberOfValidGuessClasses, objClassReg, NULL,
                cndReg, scratch1Reg, scratch2Reg, true, false, doneLabel, doneLabel, doneLabel, callForInlineLabel,
                testCastClassIsSuper, NULL, cg);
        }
    }
    ...
    if (testCastClassIsSuper) {
        ...
        genTestIsSuper(
            node, objClassReg, castClassReg, cndReg, scratch1Reg, scratch2Reg, castClassDepth, callLabel, NULL, cg);
        if (outlinedSlowPath) {
            gcPoint
                = generateDepConditionalBranchInstruction(cg, TR::InstOpCode::bne, node, callLabel, cndReg, conditions);
        } else {
            gcPoint = generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, node, callLabel, cndReg);
        }
    }
    ...
    if (testCache) {
        ...
        //                           2- There is no guarantee in which order the snippets are created, so it is more
        //                           difficult for InterfaceCastSnippet
        //                              to know the location of the helper call snippet when it is created. Though a
        //                              relocation can be used for such case.
    ...
    if (outlinedSlowPath) {
        ...
        TR::Instruction* temp
            = generateDepLabelInstruction(cg, TR::InstOpCode::label, node, helperReturnOOLLabel, conditions);
        if (debugObj) {
...
TR::Register* J9::Power::TreeEvaluator::VMifInstanceOfEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    bool isFinalClass = (castClassSymRef == NULL) ? false : castClassSymRef->isNonArrayFinal(comp);
    bool needsHelperCall = needHelperCall(testCastClassIsSuper, isFinalClass);
    bool testCache = needTestCache(
        !comp->getOption(TR_DisableInlineCheckCast), needsHelperCall, testCastClassIsSuper, castClassAddr, num_PICS);
...
TR::Register* J9::Power::TreeEvaluator::VMinstanceOfEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    resultReg = TR::TreeEvaluator::VMgenCoreInstanceofEvaluator(node, cg, false, depIndex, numDep, depNode, needResult,
        needsHelperCall, testEqualClass, testCache, testCastClassIsSuper, doneLabel, res0Label, res1Label, branchOn1);
    ...
    TR::LabelSymbol* res0Label, TR::LabelSymbol* res1Label, bool branchOn1)
{
    ...
    if (needHelperCall) {
        ...
        if (!crReg) {
            // This is another hack to compensate for skipping the preservating of CCRs for helper calls in linkage
            // code. This hack wouldn't be necessary if this code wasn't being lazy and re-using the dependencies set up
            // by the direct call evaluator, but alas... Since we are not preserving CCRs and this code previously
            // expected to find a CCR in the call instruction's dependency conditions we allocate a new virtual here and
            // stick it in the dependencies, making sure to repeat everything that is normally done for virtual regs
            // when they are referenced by a generated instruction, which just means we record two uses to correspond to
            // the fact that we added them to the pre/post deps of the call.
    ...
    if (testEqualClass || testCache || testCastClassIsSuper) {
        ...
        iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, objClassReg,
            new (cg->trHeapMemory())
                TR::MemoryReference(objectReg, (int32_t)TR::Compiler->om.offsetOfObjectVftField(), 4, cg),
            iCursor);
#else
        iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, objClassReg,
            new (cg->trHeapMemory()) TR::MemoryReference(objectReg, (int32_t)TR::Compiler->om.offsetOfObjectVftField(),
                TR::Compiler->om.sizeofReferenceAddress(), cg),
            iCursor);
#endif
    ...
    if (testEqualClass) {
        ...
        if (testCache || testCastClassIsSuper) {
            ...
            if (castClassIsReferenceArray) {
                ...
                J9Class* jlobjectarrayclassBlock
                    = jlobjectclassBlock ? ((J9Class*)jlobjectclassBlock)->arrayClass : NULL;
                if (jlobjectarrayclassBlock != NULL) {
                    ...
                    iCursor = generateTrg1Src2Instruction(
                        cg, TR::InstOpCode::Op_cmpl, node, crReg, objClassReg, scratch1Reg, iCursor);
                    if (needResult) {
                        iCursor = generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, resultReg, 0, iCursor);
                    }
                    iCursor = generateConditionalBranchInstruction(
                        cg, TR::InstOpCode::beq, node, res0Label, crReg, iCursor);
                }
            }
        } else {
    ...
    if (testCastClassIsSuper) {
        ...
        iCursor = genTestIsSuper(
            node, objClassReg, castClassReg, crReg, scratch1Reg, scratch2Reg, castClassDepth, res0Label, iCursor, cg);
        iCursor = generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, node, res0Label, crReg, iCursor);
...
static TR::Register* reservationLockEnter(
    TR::Node* node, int32_t lwOffset, TR::Register* objectClassReg, TR::CodeGenerator* cg)
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(cg->fe());
    ...
    generateTrg1MemInstruction(cg, lockSize == 8 ? TR::InstOpCode::ld : TR::InstOpCode::lwz, node, monitorReg,
        new (cg->trHeapMemory()) TR::MemoryReference(objReg, lwOffset, lockSize, cg));
    ...
    if (!isPrimitive) {
        ...
        generateMemSrc1Instruction(cg, lockSize == 8 ? TR::InstOpCode::std : TR::InstOpCode::stw, node,
            new (cg->trHeapMemory()) TR::MemoryReference(objReg, lwOffset, lockSize, cg), tempReg);
        generateLabelInstruction(cg, TR::InstOpCode::b, node, doneLabel);
    } else
        generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, node, doneLabel, cndReg);
    ...
    if (!isPrimitive)
        generateTrg1Src1ImmInstruction(
            cg, TR::InstOpCode::addi, node, valReg, metaReg, LOCK_RESERVATION_BIT | LOCK_INC_DEC_VALUE);
    ...
    generateTrg1MemInstruction(cg, lockSize == 8 ? TR::InstOpCode::ldarx : TR::InstOpCode::lwarx,
        ...
        new (cg->trHeapMemory()) TR::MemoryReference(objReg, tempReg, lockSize, cg));
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::Op_cmpli, node, cndReg, monitorReg, 0);
    ...
    generateMemSrc1Instruction(cg, lockSize == 8 ? TR::InstOpCode::stdcx_r : TR::InstOpCode::stwcx_r, node,
        new (cg->trHeapMemory()) TR::MemoryReference(objReg, tempReg, lockSize, cg), valReg);
    ...
    if (!isPrimitive) {
        ...
        generateMemSrc1Instruction(cg, lockSize == 8 ? TR::InstOpCode::std : TR::InstOpCode::stw, node,
            new (cg->trHeapMemory()) TR::MemoryReference(objReg, lwOffset & 0x0000FFFF, lockSize, cg), monitorReg);
    }

    TR_PPCOutOfLineCodeSection* outlinedHelperCall
        = new (cg->trHeapMemory()) TR_PPCOutOfLineCodeSection(node, TR::call, NULL, callLabel, doneOOLLabel, cg);
    cg->getPPCOutOfLineCodeSectionList().push_front(outlinedHelperCall);
...
static TR::Register* reservationLockExit(
    TR::Node* node, int32_t lwOffset, TR::Register* objectClassReg, TR::CodeGenerator* cg)
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(cg->fe());
    TR::RegisterDependencyConditions* conditions = createConditionsAndPopulateVSXDeps(cg, 5);
    ...
    if (!node->isPrimitiveLockedRegion()) {
        generateTrg1Src1ImmInstruction(
            cg, TR::InstOpCode::ori, node, tempReg, metaReg, LOCK_RESERVATION_BIT + LOCK_INC_DEC_VALUE);
        generateTrg1Src2Instruction(cg, TR::InstOpCode::Op_cmpl, node, cndReg, monitorReg, tempReg);
    } else {
        generateTrg1Src1ImmInstruction(
            cg, TR::InstOpCode::andi_r, node, tempReg, monitorReg, LOCK_RES_PRIMITIVE_EXIT_MASK);
    ...
    if (!isPrimitive) {
        ...
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, valReg, metaReg, LOCK_RESERVATION_BIT);
        generateMemSrc1Instruction(cg, lockSize == 8 ? TR::InstOpCode::std : TR::InstOpCode::stw, node,
            new (cg->trHeapMemory()) TR::MemoryReference(objReg, lwOffset, lockSize, cg), valReg);
        generateLabelInstruction(cg, TR::InstOpCode::b, node, doneLabel);
    } else
        generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, node, resLabel, cndReg);
    ...
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, monitorReg, monitorReg,
        (isPrimitive ? LOCK_INC_DEC_VALUE : -LOCK_INC_DEC_VALUE) & 0x0000FFFF);
    generateMemSrc1Instruction(cg, lockSize == 8 ? TR::InstOpCode::std : TR::InstOpCode::stw, node,
        new (cg->trHeapMemory()) TR::MemoryReference(objReg, lwOffset, lockSize, cg), monitorReg);
    if (!isPrimitive)
        generateLabelInstruction(cg, TR::InstOpCode::b, node, doneLabel);
...
TR::Register* J9::Power::TreeEvaluator::VMmonexitEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (lwOffset <= 0) {
        ...
        generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, objectClassReg,
            new (cg->trHeapMemory()) TR::MemoryReference(objReg, (int32_t)TR::Compiler->om.offsetOfObjectVftField(),
                TR::Compiler->om.sizeofReferenceAddress(), cg));
#endif
        TR::TreeEvaluator::generateVFTMaskInstruction(cg, node, objectClassReg);
        ...
        TR::MemoryReference* tempMR = new (cg->trHeapMemory())
            TR::MemoryReference(objectClassReg, offsetOfLockOffset, TR::Compiler->om.sizeofReferenceAddress(), cg);
        generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, objectClassReg, tempMR);

        generateTrg1Src1ImmInstruction(cg,
            TR::Compiler->target.is64Bit() ? TR::InstOpCode::cmpi8 : TR::InstOpCode::cmpi4, node, condReg,
            objectClassReg, 0);
        ...
        if (comp->getOption(TR_EnableMonitorCacheLookup)) {
            ...
            if (TR::Compiler->target.is64Bit())
                ...
                simplifyANDRegImm(
                    node, lookupOffsetReg, lookupOffsetReg, J9VMTHREAD_OBJECT_MONITOR_CACHE_SIZE - 1, cg, objNode);
            ...
            generateTrg1Src1ImmInstruction(
                cg, TR::InstOpCode::addi, node, lookupOffsetReg, lookupOffsetReg, offsetOfMonitorLookupCache);

            generateTrg1MemInstruction(cg,
                (TR::Compiler->target.is64Bit() && fej9->generateCompressedLockWord()) ? TR::InstOpCode::lwz
                                                                                       : TR::InstOpCode::Op_load,
                node, objectClassReg,
                new (cg->trHeapMemory()) TR::MemoryReference(
                    cg->getMethodMetaDataRegister(), lookupOffsetReg, TR::Compiler->om.sizeofReferenceField(), cg));
            ...
            generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, lookupOffsetReg,
                new (cg->trHeapMemory()) TR::MemoryReference(
                    objectClassReg, offsetOfMonitor, TR::Compiler->om.sizeofReferenceAddress(), cg));

            int32_t offsetOfUserData = offsetof(J9ThreadAbstractMonitor, userData);
            generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, lookupOffsetReg,
                new (cg->trHeapMemory()) TR::MemoryReference(
                    lookupOffsetReg, offsetOfUserData, TR::Compiler->om.sizeofReferenceAddress(), cg));

            generateTrg1Src2Instruction(cg,
                TR::Compiler->target.is64Bit() ? TR::InstOpCode::cmp8 : TR::InstOpCode::cmp4, node, condReg,
                lookupOffsetReg, objReg);
            ...
            generateTrg1Src1ImmInstruction(
                cg, TR::InstOpCode::addi, node, objectClassReg, objectClassReg, offsetOfAlternateLockWord);
    ...
    if (!ppcSupportsReadMonitors || !node->isReadMonitor()) {
        ...
        generateTrg1MemInstruction(
            cg, opCode, node, monitorReg, new (cg->trHeapMemory()) TR::MemoryReference(baseReg, moffset, lockSize, cg));
        ...
        if (TR::Compiler->target.cpu.id() >= TR_PPCgp)
            // use PPC AS branch hint
            generateConditionalBranchInstruction(
                cg, TR::InstOpCode::bne, PPCOpProp_BranchUnlikely, node, decLabel, condReg);
        else
        ...
        generateMemSrc1Instruction(
            cg, opCode, node, new (cg->trHeapMemory()) TR::MemoryReference(baseReg, moffset, lockSize, cg), ctempReg);
        generateLabelInstruction(cg, TR::InstOpCode::b, node, doneLabel);
        ...
        if (TR::Compiler->target.is64Bit()) {
            ...
            generateTrg1Src1Imm2Instruction(
                cg, TR::InstOpCode::rlwinm, node, threadReg, monitorReg, 0, ~OBJECT_HEADER_LOCK_RECURSION_MASK);
        generateTrg1Src2Instruction(cg, TR::InstOpCode::Op_cmpl, node, condReg, threadReg, metaReg);
        generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, node, callLabel, condReg);
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, monitorReg, monitorReg, -LOCK_INC_DEC_VALUE);
        generateMemSrc1Instruction(
            cg, opCode, node, new (cg->trHeapMemory()) TR::MemoryReference(baseReg, moffset, lockSize, cg), monitorReg);
        ...
        generateMemSrc1Instruction(
            cg, opCode, node, new (cg->trHeapMemory()) TR::MemoryReference(baseReg, offsetReg, lockSize, cg), ctempReg);

        if (TR::Compiler->target.cpu.id() >= TR_PPCgp)
            // use PPC AS branch hint
            generateConditionalBranchInstruction(
                cg, TR::InstOpCode::bne, PPCOpProp_BranchUnlikely, node, loopLabel, condReg);
        else
        ...
        generateMemSrc1Instruction(cg, opCode, node,
            new (cg->trHeapMemory()) TR::MemoryReference(baseReg, offsetReg, lockSize, cg), threadReg);
        generateConditionalBranchInstruction(cg, TR::InstOpCode::beq, node, callLabel, condReg);
    ...
    TR::RegisterDependencyConditions* dependencies, TR::CodeGenerator* cg)
{
    ...
    if (TR::Options::getCmdLineOptions()->realTimeGC()) {
        ...
        iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, temp3Reg,
            new (cg->trHeapMemory()) TR::MemoryReference(
                metaReg, fej9->thisThreadJavaVMOffset(), TR::Compiler->om.sizeofReferenceAddress(), cg),
            iCursor);
        ...
        iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, temp3Reg,
            new (cg->trHeapMemory()) TR::MemoryReference(
                temp3Reg, fej9->getRealtimeSizeClassesOffset(), TR::Compiler->om.sizeofReferenceAddress(), cg),
            iCursor);
        ...
        if (isVariableLen) {
            ...
            if (elementSize < sizeof(UDATA))
                iCursor = generateTrg1Src1ImmInstruction(
                    cg, TR::InstOpCode::addi, node, dataSizeReg, scaledSizeReg, allocSize + sizeof(UDATA) - 1, iCursor);
            else
                iCursor = generateTrg1Src1ImmInstruction(
                    cg, TR::InstOpCode::addi, node, dataSizeReg, scaledSizeReg, allocSize, iCursor);
            ...
            if (elementSize < sizeof(UDATA)) {
                if (TR::Compiler->target.is64Bit())
                    ...
                    iCursor = generateTrg1Src1Imm2Instruction(
                        cg, TR::InstOpCode::rlwinm, node, dataSizeReg, dataSizeReg, 0, -sizeof(UDATA), iCursor);
            }

#ifdef J9VM_INTERP_FLAGS_IN_CLASS_SLOT
            iCursor = generateTrg1Src1ImmInstruction(
                cg, TR::InstOpCode::Op_cmpli, node, condReg, dataSizeReg, J9_GC_MINIMUM_OBJECT_SIZE, iCursor);
            TR::LabelSymbol* doneLabel = generateLabelSymbol(cg);
            if (TR::Compiler->target.cpu.id() >= TR_PPCgp)
                iCursor = generateConditionalBranchInstruction(
                    cg, TR::InstOpCode::bge, PPCOpProp_BranchLikely, node, doneLabel, condReg, iCursor);
            else
                iCursor
                    = generateConditionalBranchInstruction(cg, TR::InstOpCode::bge, node, doneLabel, condReg, iCursor);
            iCursor = generateTrg1ImmInstruction(
                cg, TR::InstOpCode::li, node, dataSizeReg, J9_GC_MINIMUM_OBJECT_SIZE, iCursor);
            ...
            // genInitArrayHeader();
            iCursor = generateTrg1Src1ImmInstruction(
                cg, TR::InstOpCode::addi, node, dataSizeReg, dataSizeReg, -allocSize, iCursor);
            iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, heapTopReg,
                new (cg->trHeapMemory()) TR::MemoryReference(
                    temp1Reg, fej9->getSizeClassesIndexOffset(), TR::Compiler->om.sizeofReferenceAddress(), cg),
                iCursor);
            ...
            iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, resReg,
                new (cg->trHeapMemory()) TR::MemoryReference(sizeReg, fej9->thisThreadAllocationCacheCurrentOffset(0),
                    TR::Compiler->om.sizeofReferenceAddress(), cg),
                iCursor);
            iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, temp1Reg,
                new (cg->trHeapMemory()) TR::MemoryReference(sizeReg, fej9->thisThreadAllocationCacheTopOffset(0),
                    TR::Compiler->om.sizeofReferenceAddress(), cg),
                iCursor);
            ...
            if (TR::Compiler->target.is64Bit())
                iCursor = generateShiftLeftImmediateLong(
                    cg, node, temp1Reg, heapTopReg, trailingZeroes((int32_t)sizeof(UDATA)), iCursor);
            else
                iCursor = generateShiftLeftImmediate(
                    cg, node, temp1Reg, heapTopReg, trailingZeroes((int32_t)sizeof(UDATA)), iCursor);
            ...
            iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, temp1Reg,
                new (cg->trHeapMemory()) TR::MemoryReference(
                    temp1Reg, fej9->getSmallCellSizesOffset(), TR::Compiler->om.sizeofReferenceAddress(), cg),
                iCursor);
            ...
            iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, resReg,
                new (cg->trHeapMemory())
                    TR::MemoryReference(metaReg, fej9->thisThreadAllocationCacheCurrentOffset(sizeClass),
                        TR::Compiler->om.sizeofReferenceAddress(), cg),
                iCursor);
            iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, temp2Reg,
                new (cg->trHeapMemory()) TR::MemoryReference(metaReg,
                    fej9->thisThreadAllocationCacheTopOffset(sizeClass), TR::Compiler->om.sizeofReferenceAddress(), cg),
                iCursor);
            iCursor
        ...
        if (isArrayAlloc) {
            if (TR::Compiler->om.useHybridArraylets()) {
                ...
                if (maxContiguousArraySize > UPPER_IMMED) {
                    ...
                    iCursor = generateTrg1Src2Instruction(
                        cg, TR::InstOpCode::cmpli4, node, condReg, enumReg, temp2Reg, iCursor);
                } else
                    iCursor = generateTrg1Src1ImmInstruction(
                        cg, TR::InstOpCode::cmpli4, node, condReg, enumReg, maxContiguousArraySize, iCursor);
            ...
            if (isVariableLen) { // Inline runtime zero length non-packed array allocation
                ...
                iCursor = generateConditionalBranchInstruction(
                    cg, TR::InstOpCode::bne, node, nonZeroLengthLabel, condReg, iCursor);

                int32_t zeroLenArraySize
                    = (sizeof(J9IndexableObjectDiscontiguous) + fej9->getObjectAlignmentInBytes() - 1)
                ...
                iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, resReg,
                    new (cg->trHeapMemory()) TR::MemoryReference(
                        metaReg, offsetof(J9VMThread, heapAlloc), TR::Compiler->om.sizeofReferenceAddress(), cg),
                    iCursor);
                iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, heapTopReg,
                    new (cg->trHeapMemory()) TR::MemoryReference(
                        metaReg, offsetof(J9VMThread, heapTop), TR::Compiler->om.sizeofReferenceAddress(), cg),
                    iCursor);

                iCursor = generateTrg1Src1ImmInstruction(
                    cg, TR::InstOpCode::addi, node, temp2Reg, resReg, zeroLenArraySize, iCursor);
                iCursor = generateTrg1Src2Instruction(
                    cg, TR::InstOpCode::Op_cmpl, node, condReg, temp2Reg, heapTopReg, iCursor);
                iCursor
                    = generateConditionalBranchInstruction(cg, TR::InstOpCode::bgt, node, callLabel, condReg, iCursor);
                ...
                iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                    new (cg->trHeapMemory()) TR::MemoryReference(resReg, offsetSizeFields - 4, 4, cg), enumReg,
                    iCursor);
                iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
        ...
        if (usingTLH) {
            ...
            if (isVariableLen) {
                ...
                if (maxSafeSize < 0x00100000) {
                    ...
                    iCursor = generateTrg1Src1ImmInstruction(
                        cg, TR::InstOpCode::Op_cmpli, node, condReg, enumReg, (maxSafeSize >> 6) << 2, iCursor);
                    iCursor = generateConditionalBranchInstruction(
                        cg, TR::InstOpCode::bge, node, callLabel, condReg, iCursor);
                } else {
                    uintptrj_t mask;
                    if (TR::Compiler->target.is64Bit()) {
                        mask = 0xFFFFFFFF << (27 - leadingZeroes(maxSafeSize));
                        iCursor = generateTrg1Src1Imm2Instruction(
                            cg, TR::InstOpCode::rlwinm_r, node, temp2Reg, enumReg, condReg, 0, mask, iCursor);
                    } else {
                        mask = (0xFFFFFFFF << (11 - leadingZeroes(maxSafeSize))) & 0x0000FFFF;
                        iCursor = generateTrg1Src1ImmInstruction(
                            cg, TR::InstOpCode::andis_r, node, temp2Reg, enumReg, condReg, mask, iCursor);
                    }
                    iCursor = generateConditionalBranchInstruction(
                        cg, TR::InstOpCode::bne, node, callLabel, condReg, iCursor);
                }
            }
            ...
            if (!cg->isDualTLH()) {
                ...
                if (!isVariableLen) {
                    ...
                    if (0 && !disableAlign && (node->getOpCodeValue() == TR::New)
                        ...
                        if (classChild && classChild->getSymbolReference()
                        ...
                        if (clazz && !cg->getCurrentEvaluationBlock()->isCold()
                            && TR::Compiler->cls.classInstanceSize(clazz) >= instanceSizeForAlignment) {
                            shouldAlignToCacheBoundary = true;

                            iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, temp1Reg,
                                new (cg->trHeapMemory()) TR::MemoryReference(metaReg, offsetof(J9VMThread, heapAlloc),
                                    TR::Compiler->om.sizeofReferenceAddress(), cg),
                                iCursor);

                            iCursor = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, resReg, temp1Reg,
                                instanceBoundaryForAlignment - 1, iCursor);
                            if (TR::Compiler->target.is64Bit())
                                iCursor = generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rldicr, node, resReg,
                                    resReg, 0, int64_t(-instanceBoundaryForAlignment), iCursor);
                            else
                                iCursor = generateTrg1Src1Imm2Instruction(cg, TR::InstOpCode::rlwinm, node, resReg,
                                    resReg, 0, -instanceBoundaryForAlignment, iCursor);
                        }
                    }
                }
                ...
                if (node->canSkipZeroInitialization()) {
                    ...
                    iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, resReg,
                        new (cg->trHeapMemory()) TR::MemoryReference(metaReg, offsetof(J9VMThread, nonZeroHeapAlloc),
                            TR::Compiler->om.sizeofReferenceAddress(), cg),
                        iCursor);

                    iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, heapTopReg,
                        new (cg->trHeapMemory()) TR::MemoryReference(metaReg, offsetof(J9VMThread, nonZeroHeapTop),
                            TR::Compiler->om.sizeofReferenceAddress(), cg),
                        iCursor);
            ...
            if (sizeInReg) {
                ...
                if (isVariableLen) {
                    ...
                    if (generateArraylets
                        ...
                        iCursor = generateTrg1Src1ImmInstruction(
                            cg, TR::InstOpCode::cmpi4, node, condReg, enumReg, maxArrayletSizeInElements, iCursor);
                    ...
                    if (elementSize >= 2) {
                        if (TR::Compiler->target.is64Bit())
                            ...
                            iCursor = generateShiftLeftImmediate(
                                cg, node, dataSizeReg, enumReg, trailingZeroes(elementSize), iCursor);
                    }
                    if (needZeroInit && elementSize <= 2) {
                        ...
                        if (elementSize == 2)
                            iCursor = generateTrg1Src1ImmInstruction(
                                cg, TR::InstOpCode::addi, node, dataSizeReg, dataSizeReg, 3, iCursor);
                        else // elementSize == 1
                            iCursor = generateTrg1Src1ImmInstruction(
                                cg, TR::InstOpCode::addi, node, dataSizeReg, enumReg, 3, iCursor);

                        if (TR::Compiler->target.is64Bit() && elementSize != 1)
                            iCursor = generateTrg1Src1Imm2Instruction(
                                cg, TR::InstOpCode::rldicr, node, dataSizeReg, dataSizeReg, 0, int64_t(-4), iCursor);
                        else
                            iCursor = generateTrg1Src1Imm2Instruction(
                                cg, TR::InstOpCode::rlwinm, node, dataSizeReg, dataSizeReg, 0, -4, iCursor);
                    }
                    ...
                    if ((round != 0 && (!needZeroInit || round != 4)) || !headerAligned) {
                        ...
                        if (elementSize == 1 && !needZeroInit)
                            iCursor = generateTrg1Src1ImmInstruction(
                                cg, TR::InstOpCode::addi, node, sizeReg, enumReg, allocSize + round - 1, iCursor);
                        else
                            iCursor = generateTrg1Src1ImmInstruction(
                                cg, TR::InstOpCode::addi, node, sizeReg, dataSizeReg, allocSize + round - 1, iCursor);
                        if (TR::Compiler->target.is64Bit() && elementSize != 1)
                            iCursor = generateTrg1Src1Imm2Instruction(
                                cg, TR::InstOpCode::rldicr, node, sizeReg, sizeReg, 0, int64_t(-round), iCursor);
                        else
                            iCursor = generateTrg1Src1Imm2Instruction(
                                cg, TR::InstOpCode::rlwinm, node, sizeReg, sizeReg, 0, -round, iCursor);
                    } else {
                        if (elementSize == 1 && !needZeroInit)
                            iCursor = generateTrg1Src1ImmInstruction(
                                cg, TR::InstOpCode::addi, node, sizeReg, enumReg, allocSize, iCursor);
                        else
                            iCursor = generateTrg1Src1ImmInstruction(
                                cg, TR::InstOpCode::addi, node, sizeReg, dataSizeReg, allocSize, iCursor);
                    }
                    ...
                    if (!((node->getOpCodeValue() == TR::New) || (node->getOpCodeValue() == TR::anewarray)
                        ...
                        iCursor = generateTrg1Src1ImmInstruction(
                            cg, TR::InstOpCode::Op_cmpli, node, condReg, sizeReg, J9_GC_MINIMUM_OBJECT_SIZE, iCursor);
                        TR::LabelSymbol* doneLabel = generateLabelSymbol(cg);
                        if (TR::Compiler->target.cpu.id() >= TR_PPCgp)
                            iCursor = generateConditionalBranchInstruction(
                                cg, TR::InstOpCode::bge, PPCOpProp_BranchLikely, node, doneLabel, condReg, iCursor);
                            ...
                            iCursor = generateConditionalBranchInstruction(
                                cg, TR::InstOpCode::bge, node, doneLabel, condReg, iCursor);

                        iCursor = generateTrg1ImmInstruction(
                            cg, TR::InstOpCode::li, node, sizeReg, J9_GC_MINIMUM_OBJECT_SIZE, iCursor);
                        iCursor = generateLabelInstruction(cg, TR::InstOpCode::label, node, doneLabel, iCursor);
                    }
#endif
                } // if (isVariableLen)
            ...
            if (sizeInReg)
                ...
                iCursor = generateTrg1Src1ImmInstruction(
                    cg, TR::InstOpCode::addi, node, temp2Reg, resReg, allocSize, iCursor);
            ...
            if (!isVariableLen && ((uint32_t)allocSize + padding) > maxSafeSize) {
                if (!shouldAlignToCacheBoundary)
                    iCursor = generateTrg1Src2Instruction(
                        cg, TR::InstOpCode::Op_cmpl, node, condReg, temp2Reg, resReg, iCursor);
                else
                    iCursor = generateTrg1Src2Instruction(
                        cg, TR::InstOpCode::Op_cmpl, node, condReg, temp2Reg, temp1Reg, iCursor);
                iCursor
            ...
            if (generateArraylets
                && (node->getOpCodeValue() == TR::anewarray || node->getOpCodeValue() == TR::newarray)) {
                iCursor = generateTrg1Src1ImmInstruction(
                    cg, TR::InstOpCode::addi, node, temp2Reg, temp2Reg, fej9->getObjectAlignmentInBytes() - 1, iCursor);
                iCursor = generateTrg1Src1Imm2Instruction(cg,
                    TR::Compiler->target.is64Bit() ? TR::InstOpCode::rldicr : TR::InstOpCode::rlwinm, node, temp2Reg,
                    temp2Reg, 0, int64_t(-fej9->getObjectAlignmentInBytes()), iCursor);
            ...
            if (!cg->isDualTLH()) {
                ...
                if (shouldAlignToCacheBoundary) {
                    ...
                    ;
                    iCursor = generateTrg1Src2Instruction(
                        cg, TR::InstOpCode::subf, node, dataSizeReg, temp1Reg, resReg, iCursor);

                    if (sizeInReg)
                        iCursor = generateTrg1Src2Instruction(
                            cg, TR::InstOpCode::add, node, sizeReg, dataSizeReg, sizeReg, iCursor);
                    else
                        iCursor = generateTrg1Src1ImmInstruction(
                            cg, TR::InstOpCode::addi, node, sizeReg, dataSizeReg, allocSize, iCursor);
                    ...
                    iCursor = generateTrg1Src1ImmInstruction(
                        cg, TR::InstOpCode::Op_cmpli, node, condReg, dataSizeReg, sizeof(uintptrj_t), iCursor);
                    iCursor = generateConditionalBranchInstruction(
                        cg, TR::InstOpCode::blt, node, doneAlignLabel, condReg, iCursor);
                    iCursor = generateConditionalBranchInstruction(
                        cg, TR::InstOpCode::bgt, node, multiSlotGapLabel, condReg, iCursor);
                    iCursor = generateTrg1ImmInstruction(
                        cg, TR::InstOpCode::li, node, dataSizeReg, J9_GC_SINGLE_SLOT_HOLE, iCursor);

                    if (TR::Compiler->target.is64Bit() && fej9->generateCompressedLockWord()) {
                        iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                            new (cg->trHeapMemory()) TR::MemoryReference(temp1Reg, 0, 4, cg), dataSizeReg, iCursor);
                        iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                            new (cg->trHeapMemory()) TR::MemoryReference(temp1Reg, 4, 4, cg), dataSizeReg, iCursor);
                    } else {
                        iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::Op_st, node,
                            new (cg->trHeapMemory())
                                TR::MemoryReference(temp1Reg, 0, TR::Compiler->om.sizeofReferenceAddress(), cg),
                            dataSizeReg, iCursor);
                    }
                    ...
                    iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::Op_st, node,
                        new (cg->trHeapMemory()) TR::MemoryReference(temp1Reg,
                            TR::Compiler->om.sizeofReferenceAddress(), TR::Compiler->om.sizeofReferenceAddress(), cg),
                        dataSizeReg, iCursor);
                    iCursor = generateTrg1ImmInstruction(
                        cg, TR::InstOpCode::li, node, dataSizeReg, J9_GC_MULTI_SLOT_HOLE, iCursor);
                    iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::Op_st, node,
                        new (cg->trHeapMemory())
                            TR::MemoryReference(temp1Reg, 0, TR::Compiler->om.sizeofReferenceAddress(), cg),
                        dataSizeReg, iCursor);
            ...
            if (cg->enableTLHPrefetching()) {
                ...
                if (cg->isDualTLH() && node->canSkipZeroInitialization()) {
                    ...
                    iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, temp1Reg,
                        new (cg->trHeapMemory()) TR::MemoryReference(metaReg,
                            offsetof(J9VMThread, nonZeroTlhPrefetchFTA), TR::Compiler->om.sizeofReferenceAddress(), cg),
                        iCursor);
                } else {
                    if (verboseDualTLH)
                        fprintf(stderr,
                    ...
                    iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, temp1Reg,
                        new (cg->trHeapMemory()) TR::MemoryReference(metaReg, offsetof(J9VMThread, tlhPrefetchFTA),
                            TR::Compiler->om.sizeofReferenceAddress(), cg),
                        iCursor);
                }
                ...
                if (sizeInReg)
                    iCursor = generateTrg1Src2Instruction(
                        cg, TR::InstOpCode::subf_r, node, temp1Reg, sizeReg, temp1Reg, condReg, iCursor);
                else
                    iCursor = generateTrg1Src1ImmInstruction(
                        cg, TR::InstOpCode::addic_r, node, temp1Reg, temp1Reg, condReg, -allocSize, iCursor);
                ...
                if (TR::Compiler->target.cpu.id() >= TR_PPCgp)
                    // use PPC AS branch hint
                    iCursor = generateDepConditionalBranchInstruction(cg, TR::InstOpCode::blel,
                        PPCOpProp_BranchUnlikely, node, callLabel, condReg, dependencies, iCursor);
                else
                    iCursor = generateDepConditionalBranchInstruction(
                        cg, TR::InstOpCode::blel, node, callLabel, condReg, dependencies, iCursor);
            ...
            if (cg->isDualTLH() && node->canSkipZeroInitialization()) {
                iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::Op_st, node,
                    new (cg->trHeapMemory()) TR::MemoryReference(
                        metaReg, offsetof(J9VMThread, nonZeroHeapAlloc), TR::Compiler->om.sizeofReferenceAddress(), cg),
                    temp2Reg, iCursor);
            } else {
                iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::Op_st, node,
                    new (cg->trHeapMemory()) TR::MemoryReference(
            ...
            iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, heapTopReg,
                new (cg->trHeapMemory()) TR::MemoryReference(
                    temp1Reg, offsetof(J9MemorySegment, heapTop), TR::Compiler->om.sizeofReferenceAddress(), cg),
                iCursor);
            iCursor = generateTrg1Src1ImmInstruction(
                cg, TR::InstOpCode::addi, node, dataSizeReg, temp1Reg, offsetof(J9MemorySegment, heapAlloc), iCursor);
            iCursor = generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, zeroReg, 0, iCursor);
            if (sizeInReg) {
                iCursor = generateTrg1ImmInstruction(cg, TR::InstOpCode::lis, node, sizeReg,
                    (allocSize >> 16) + ((allocSize & (1 << 15)) ? 1 : 0), iCursor);
                iCursor = generateTrg1Src1ImmInstruction(
                    cg, TR::InstOpCode::addi, node, sizeReg, sizeReg, allocSize & 0x0000FFFF, iCursor);
            }
            ...
            iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, resReg,
                new (cg->trHeapMemory())
                    TR::MemoryReference(dataSizeReg, 0, TR::Compiler->om.sizeofReferenceAddress(), cg),
                iCursor);
            ...
            if ((uint32_t)allocSize > maxSafeSize) {
                if (sizeInReg) {
                    iCursor = generateTrg1Src2Instruction(
                        cg, TR::InstOpCode::add, node, temp1Reg, resReg, sizeReg, iCursor);
                } else {
                    iCursor = generateTrg1Src1ImmInstruction(
                        cg, TR::InstOpCode::addi, node, temp1Reg, resReg, allocSize, iCursor);
                }

                iCursor = generateTrg1Src2Instruction(
                    cg, TR::InstOpCode::Op_cmpl, node, condReg, temp1Reg, resReg, iCursor);
            ...
            iCursor = generateTrg1Src2Instruction(
                cg, TR::InstOpCode::Op_cmpl, node, condReg, temp1Reg, heapTopReg, iCursor);
            iCursor = generateConditionalBranchInstruction(cg, TR::InstOpCode::bgt, node, callLabel, condReg, iCursor);
            ...
            iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::lwarx, node, temp2Reg,
                new (cg->trHeapMemory()) TR::MemoryReference(dataSizeReg, zeroReg, 4, cg), iCursor);
            ...
            iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stwcx_r, node,
                new (cg->trHeapMemory()) TR::MemoryReference(dataSizeReg, zeroReg, 4, cg), temp1Reg, iCursor);

            if (TR::Compiler->target.cpu.id() >= TR_PPCgp)
                // use PPC AS branch hint
                iCursor = generateConditionalBranchInstruction(
                    cg, TR::InstOpCode::bne, PPCOpProp_BranchUnlikely, node, tryLabel, condReg, iCursor);
            else
                iCursor
                    = generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, node, tryLabel, condReg, iCursor);
        }
    }
}
    ...
    bool needZeroInit, TR::CodeGenerator* cg)
{
    ...
    if (comp->compileRelocatableCode()) {
        if (node->getOpCodeValue() == TR::newarray) {
            iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, temp1Reg,
                new (cg->trHeapMemory()) TR::MemoryReference(
                    metaReg, offsetof(J9VMThread, javaVM), TR::Compiler->om.sizeofReferenceAddress(), cg),
                iCursor);
            iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, temp1Reg,
                new (cg->trHeapMemory()) TR::MemoryReference(temp1Reg,
            ...
            iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, temp1Reg,
                new (cg->trHeapMemory()) TR::MemoryReference(
                    classReg, offsetof(J9Class, arrayClass), TR::Compiler->om.sizeofReferenceAddress(), cg),
                iCursor);
            clzReg = temp1Reg;
        } else {
    ...
    if (clzReg == NULL) {
        ...
        if (cg->wantToPatchClassPointer(clazz, node)) {
            iCursor = loadAddressConstantInSnippet(
                cg, node, (int64_t)clazz, temp1Reg, temp2Reg, TR::InstOpCode::Op_load, false, iCursor);
            iCursor
                = generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::ori, node, temp1Reg, temp1Reg, orFlag, iCursor);
        } else {
            ...
            if (offset != PTOC_FULL_INDEX) {
                ...
                if (offset < LOWER_IMMED || offset > UPPER_IMMED) {
                    iCursor = generateTrg1Src1ImmInstruction(
                        cg, TR::InstOpCode::addis, node, temp1Reg, cg->getTOCBaseRegister(), HI_VALUE(offset), iCursor);
                    iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, temp1Reg,
                        new (cg->trHeapMemory()) TR::MemoryReference(
                            temp1Reg, LO_VALUE(offset), TR::Compiler->om.sizeofReferenceAddress(), cg),
                        iCursor);
                } else {
                    iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, temp1Reg,
                        new (cg->trHeapMemory()) TR::MemoryReference(
                            cg->getTOCBaseRegister(), offset, TR::Compiler->om.sizeofReferenceAddress(), cg),
                        iCursor);
                }
                if (orFlag != 0) {
                    iCursor = generateTrg1Src1ImmInstruction(
                        cg, TR::InstOpCode::ori, node, temp1Reg, temp1Reg, orFlag, iCursor);
                }
            } else {
        ...
        iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
            new (cg->trHeapMemory())
                TR::MemoryReference(resReg, (int32_t)TR::Compiler->om.offsetOfObjectVftField(), 4, cg),
            temp1Reg, iCursor);
#else
        iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::Op_st, node,
            new (cg->trHeapMemory()) TR::MemoryReference(resReg, (int32_t)TR::Compiler->om.offsetOfObjectVftField(),
                TR::Compiler->om.sizeofReferenceAddress(), cg),
            temp1Reg, iCursor);
#endif
    } else {
    ...
    if (isStaticFlag) {
        ...
        iCursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, temp2Reg,
            new (cg->trHeapMemory())
                TR::MemoryReference(metaReg, offsetof(J9VMThread, allocateThreadLocalHeap.objectFlags),
                    TR::Compiler->om.sizeofReferenceAddress(), cg),
            iCursor);
        ...
        if (staticFlag != 0) {
            if ((staticFlag & 0xFFFF0000) != 0) {
                iCursor = generateTrg1Src1ImmInstruction(
                    cg, TR::InstOpCode::oris, node, temp2Reg, temp2Reg, (staticFlag >> 16) & 0x0000FFFF, iCursor);
            }
            if ((staticFlag & 0x0000FFFF) != 0) {
                iCursor = generateTrg1Src1ImmInstruction(
                    cg, TR::InstOpCode::ori, node, temp2Reg, temp2Reg, staticFlag & 0x0000FFFF, iCursor);
            }
        }
        iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
            new (cg->trHeapMemory()) TR::MemoryReference(resReg, (int32_t)TMP_OFFSETOF_J9OBJECT_FLAGS, 4, cg), temp2Reg,
            iCursor);
    }
    ...
    if (needZeroInit)
        iCursor = generateMemSrc1Instruction(cg, opCode, node,
            new (cg->trHeapMemory()) TR::MemoryReference(resReg, TMP_OFFSETOF_J9OBJECT_MONITOR, lockSize, cg), zeroReg,
            iCursor);
#endif
    ...
    if (needZeroInit && (node->getOpCodeValue() != TR::New) && (lwOffset > 0) && !fej9->isPackedClass(classAddress))
        iCursor = generateMemSrc1Instruction(cg, opCode, node,
    ...
    TR::Register* temp2Reg, TR::CodeGenerator* cg)
{
    ...
    if (isVariableLen) {
        ...
        iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
            new (cg->trHeapMemory()) TR::MemoryReference(temp1Reg, dataBegin, 4, cg), temp2Reg, iCursor);
    } else if (objectSize > UPPER_IMMED) {
        iCursor = loadConstant(cg, node, objectSize, temp1Reg, iCursor);
        iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
            new (cg->trHeapMemory()) TR::MemoryReference(resReg, temp1Reg, 4, cg), temp2Reg, iCursor);
    } else {
        iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
            new (cg->trHeapMemory()) TR::MemoryReference(resReg, objectSize, 4, cg), temp2Reg, iCursor);
    }
    ...
    iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
        new (cg->trHeapMemory()) TR::MemoryReference(resReg, (int32_t)0, 4, cg), temp2Reg, iCursor);
    ...
    if ((node->getOpCodeValue() == TR::newarray || node->getOpCodeValue() == TR::anewarray)
        ...
        && (node->getFirstChild()->getInt()
               == 0)) { // constant zero length non-packed array
                        // Zero length arrays are discontiguous (i.e. they also need the discontiguous length field to
                        // be 0) because they are indistinguishable from non-zero length discontiguous arrays
        iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
            new (cg->trHeapMemory())
                TR::MemoryReference(resReg, fej9->getOffsetOfDiscontiguousArraySizeField() - 4, 4, cg),
    ...
    if (useInitInfo) {
            ...
            while (bvi.hasMoreElements()) {
                ...
                if (lastNotInit == -1) {
                    ...
                    generateMemSrc1Instruction(cg, TR::InstOpCode::std, node,
                        new (cg->trHeapMemory()) TR::MemoryReference(objectReg, headerSize + lastNotInit * 4, 8, cg),
                        zeroReg);
                    lastNotInit = -1;
                } else {
                    ...
                    generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                        new (cg->trHeapMemory()) TR::MemoryReference(objectReg, headerSize + lastNotInit * 4, 4, cg),
                        zeroReg);
                    lastNotInit = currElem;
                }
            }
        ...
        for (ofs = headerSize; ofs < totalSize - TR::Compiler->om.sizeofReferenceAddress();
            ...
            generateMemSrc1Instruction(cg, TR::InstOpCode::Op_st, node,
                new (cg->trHeapMemory())
                    TR::MemoryReference(objectReg, ofs, TR::Compiler->om.sizeofReferenceAddress(), cg),
                zeroReg);
        }
        if (ofs + 4 == totalSize)
            generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                new (cg->trHeapMemory()) TR::MemoryReference(objectReg, ofs, 4, cg), zeroReg);
        else if (ofs + 8 == totalSize)
            generateMemSrc1Instruction(cg, TR::InstOpCode::std, node,
                new (cg->trHeapMemory()) TR::MemoryReference(objectReg, ofs, 8, cg), zeroReg);
    }
...
TR::Register* outlinedHelperNewEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (objectSizeBytes < 0)
        doInline = false;
    else if (isArray && TR::Compiler->om.useHybridArraylets()
    ...
    const int32_t allocSizeBytes = isVariableLen
        ? sizeof(J9IndexableObjectContiguous)
        : (objectSizeBytes + fej9->getObjectAlignmentInBytes() - 1) & (-fej9->getObjectAlignmentInBytes());
    ...
    const bool useInitInfo = node->getSymbolReference()->getExtraInfo()
        && (node->getSymbolReference()->getExtraInfo())->zeroInitSlots
    ...
    const bool doZeroInitInline = needsZeroInit && !isVariableLen
        && (useInitInfo || (objectSizeBytes <= TR::Compiler->om.sizeofReferenceAddress() * maxInitSlots));

    TR::Instruction* appendInstruction = cg->getAppendInstruction();

    TR_ASSERT(objectSizeBytes >= J9_GC_MINIMUM_OBJECT_SIZE || isVariableLen,
        "Expecting object to be at least minimum required by GC");
    TR_ASSERT(
        (objectSizeBytes & TR::Compiler->om.sizeofReferenceAddress() - 1) == 0, "Expecting object size to be aligned");
    TR_ASSERT(((allocSizeBytes & fej9->getObjectAlignmentInBytes() - 1) == 0 || isVariableLen),
        "Expecting allocation to be aligned");
    ...
    TR::Instruction* tlhHeapSelector
        = generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, gr11, useNonZeroTLH ? 1 : 0);

    if (debugObj)
        debugObj->addInstructionComment(
            tlhHeapSelector, "Toggle between zeroInitialized (gr11 <-- 0) and non-zeroIntialized (gr11 <-- 1) TLHs.");

    if (!isArray) {
        if (verboseDualTLH)
            fprintf(stderr, "DELETEME outlinedHelperNewEvaluator useNonZeroTLH [%d] isArray [%d] node: [%p] %s [@%s]\n",
                useNonZeroTLH, isArray, node, comp->signature(), comp->getHotnessName(comp->getMethodHotness()));
        ...
        gcPoint = generateDepImmSymInstruction(cg, TR::InstOpCode::bl, node, helperAddr,
            TR_PPCScratchRegisterDependencyConditions::createDependencyConditions(cg, &preDeps, &postDeps),
        ...
        if (verboseDualTLH)
            fprintf(stderr,
                "DELETEME outlinedHelperNewEvaluator useNonZeroTLH [%d] isArray [%d] isVariableLen [%d] node: [%p]  %s "
                "[@%s]\n",
                useNonZeroTLH, isArray, isVariableLen, node, comp->signature(),
        ...
        if (isVariableLen) {
            ...
            case 2:
                ...
                generateTrg1Src2Instruction(
                    cg, TR::InstOpCode::add, node, allocSizeBytesReg, numElementsReg, numElementsReg);
                break;
            case 1:
            ...
            default:
                ...
                if (TR::Compiler->target.is64Bit())
                    generateShiftLeftImmediateLong(
                        cg, node, allocSizeBytesReg, numElementsReg, trailingZeroes(elementSize));
                else
        ...
        if (cg->wantToPatchClassPointer(clazz, node))
            loadAddressConstantInSnippet(
                cg, node, (intptrj_t)clazz, arrayClassReg, gr10, TR::InstOpCode::Op_load, false);
        else
    ...
    if (OMR_UNLIKELY(cg->comp()->compileRelocatableCode()) && opCode != TR::newarray) {
        ...
        TR_ExternalRelocationTargetKind reloKind
            = opCode == TR::New ? TR_VerifyClassObjectForAlloc : TR_VerifyRefArrayForAlloc;

        TR::Relocation* r = new (cg->trHeapMemory()) TR::BeforeBinaryEncodingExternalRelocation(
            firstInstruction, (uint8_t*)classSymRef, (uint8_t*)recordInfo, reloKind, cg);
...
TR::Register* J9::Power::TreeEvaluator::VMnewEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    doInline = doInline
        && performTransformation(
               comp, "O^O <%3d> Inlining Allocation of %s [0x%p].\n", count++, node->getOpCode().getName(), node);
    ...
    if (doInline) {
        ...
        // TR_ByteCodeIlGenerator::genNew, TR_ByteCodeIlGenerator::genNewArray, TR_ByteCodeIlGenerator::genANewArray
        ...
        if (isArray) {
            ...
            if (generateArraylets) {
                ...
                iCursor = generateTrg1Src1ImmInstruction(
                    cg, TR::InstOpCode::addi, node, tmp4Reg, resReg, dataBegin, iCursor);
                if (TR::Compiler->vm.heapBaseAddress() != 0)
                    iCursor = generateTrg1Src1ImmInstruction(
                        cg, TR::InstOpCode::addi, node, tmp4Reg, tmp4Reg, -TR::Compiler->vm.heapBaseAddress(), iCursor);
                if (TR::Compiler->om.compressedReferenceShiftOffset() > 0)
                    iCursor = generateShiftRightLogicalImmediateLong(
                        cg, node, tmp4Reg, tmp4Reg, TR::Compiler->om.compressedReferenceShiftOffset(), iCursor);
                iCursor = generateMemSrc1Instruction(cg,
                    (TR::Compiler->target.is64Bit() && !comp->useCompressedPointers()) ? TR::InstOpCode::std
                                                                                       : TR::InstOpCode::stw,
                    node,
                    new (cg->trHeapMemory()) TR::MemoryReference(resReg, fej9->getFirstArrayletPointerOffset(comp),
                        comp->useCompressedPointers() ? 4 : TR::Compiler->om.sizeofReferenceAddress(), cg),
                    tmp4Reg, iCursor);
            ...
            if (!node->canSkipZeroInitialization() && (initInfo == NULL || initInfo->numZeroInitSlots > 0)) {
                    ...
                    if (initInfo != NULL && initInfo->zeroInitSlots != NULL && initInfo->numZeroInitSlots <= 9
                        ...
                        if (TR::Compiler->target.is64Bit()) {
                            ...
                            while (bvi.hasMoreElements()) {
                                ...
                                if (lastNotInit == -1) {
                                    ...
                                    iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::std, node,
                                        new (cg->trHeapMemory())
                                            TR::MemoryReference(resReg, lastNotInit * 4 + dataBegin, 8, cg),
                                        zeroReg, iCursor);
                                    ...
                                    iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                                        new (cg->trHeapMemory())
                                            TR::MemoryReference(resReg, lastNotInit * 4 + dataBegin, 4, cg),
                                        zeroReg, iCursor);
                            ...
                            while (bvi.hasMoreElements()) {
                                iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                                    new (cg->trHeapMemory())
                                        TR::MemoryReference(resReg, bvi.getNextElement() * 4 + dataBegin, 4, cg),
                                    zeroReg, iCursor);
                            }
                        }
                        ...
                        for (idx = dataBegin; idx < (objectSize - TR::Compiler->om.sizeofReferenceAddress());
                            ...
                            iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::Op_st, node,
                                new (cg->trHeapMemory())
                                    TR::MemoryReference(resReg, idx, TR::Compiler->om.sizeofReferenceAddress(), cg),
                                zeroReg, iCursor);
                        }
                        if (idx + 4 == objectSize)
                            iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                                new (cg->trHeapMemory()) TR::MemoryReference(resReg, idx, 4, cg), zeroReg, iCursor);
                        else if (idx + 8 == objectSize)
                            iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::std, node,
                                new (cg->trHeapMemory()) TR::MemoryReference(resReg, idx, 8, cg), zeroReg, iCursor);
                    } else {
                        ...
                        if (!disableUnrollNewInitLoop1) {
                            ...
                            if (loopCount > 0) {
                                ...
                                for (int32_t i = 0; i < unrollFactor; i++)
                                    iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::Op_st, node,
                                        new (cg->trHeapMemory()) TR::MemoryReference(tmp4Reg, i * width, width, cg),
                                        zeroReg, iCursor);

                                iCursor = generateTrg1Src1ImmInstruction(
                                    cg, TR::InstOpCode::addi2, node, tmp4Reg, tmp4Reg, (unrollFactor * width), iCursor);
                                if (loopCount > 1)
                                    iCursor = generateConditionalBranchInstruction(
                                        cg, TR::InstOpCode::bdnz, node, loopStart, condReg, iCursor);
                            }
                            ...
                            if (res2 && residueCount != 0)
                                iCursor = generateTrg1Src1ImmInstruction(
                                    cg, TR::InstOpCode::addi2, node, tmp4Reg, tmp4Reg, residueCount * width, iCursor);

                            if (TR::Compiler->target.is64Bit() && res2 >= 4) // Should only be 0 or 4 here
                                iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                                    new (cg->trHeapMemory()) TR::MemoryReference(tmp4Reg, 0, 4, cg), zeroReg, iCursor);
                        } else {
                            ...
                            iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::Op_st, node,
                                new (cg->trHeapMemory())
                                    TR::MemoryReference(tmp4Reg, 0, TR::Compiler->om.sizeofReferenceAddress(), cg),
                                zeroReg, iCursor);
                            iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::Op_st, node,
                                new (cg->trHeapMemory())
                                    TR::MemoryReference(tmp4Reg, TR::Compiler->om.sizeofReferenceAddress(),
                                        TR::Compiler->om.sizeofReferenceAddress(), cg),
                                zeroReg, iCursor);
                            if (TR::Compiler->target.is32Bit()) {
                                iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                                    new (cg->trHeapMemory()) TR::MemoryReference(tmp4Reg, 8, 4, cg), zeroReg, iCursor);
                                iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                                    new (cg->trHeapMemory()) TR::MemoryReference(tmp4Reg, 12, 4, cg), zeroReg, iCursor);
                            }
                            iCursor = generateTrg1Src1ImmInstruction(
                                cg, TR::InstOpCode::addi, node, tmp4Reg, tmp4Reg, 16, iCursor);
                            iCursor = generateConditionalBranchInstruction(
                                cg, TR::InstOpCode::bdnz, node, initLoop, condReg, iCursor);
                            ...
                            if (residue & 0x2) {
                                ...
                                if (TR::Compiler->target.is32Bit())
                                    iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                                        new (cg->trHeapMemory()) TR::MemoryReference(tmp4Reg, 4, 4, cg), zeroReg,
                                        iCursor);
                            ...
                            if (residue & 0x1) {
                                iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                                    new (cg->trHeapMemory()) TR::MemoryReference(tmp4Reg, idx, 4, cg), zeroReg,
                                    iCursor);
                            }
                        }
                    }
                } else // Init variable length array
                {
                    ...
                    if (!disableUnrollNewInitLoop2) {
                        ...
                        iCursor = generateTrg1Src1ImmInstruction(
                            cg, TR::InstOpCode::addi, node, tmp5Reg, resReg, (dataBegin - width), iCursor);
                        iCursor = generateTrg1Src2Instruction(
                            cg, TR::InstOpCode::add, node, tmp4Reg, tmp5Reg, dataSizeReg, iCursor);
                        iCursor = generateTrg1Src2Instruction(
                            cg, TR::InstOpCode::Op_cmpl, node, condReg, tmp4Reg, tmp5Reg, iCursor);
                        iCursor = generateConditionalBranchInstruction(
                            cg, TR::InstOpCode::beq, node, doneLabel, condReg, iCursor);
                        ...
                        iCursor = generateTrg1Src2Instruction(
                            cg, TR::InstOpCode::Op_cmpl, node, condReg, tmp6Reg, tmp5Reg, iCursor);
                        iCursor = generateConditionalBranchInstruction(
                            cg, TR::InstOpCode::blt, node, resLabel, condReg, iCursor);
                        ...
                        for (; i <= unrollFactor; i++)
                            iCursor = generateMemSrc1Instruction(cg,
                                ...
                                new (cg->trHeapMemory()) TR::MemoryReference(tmp5Reg, i * width, width, cg), zeroReg,
                                iCursor);
                        iCursor = generateTrg1Src1ImmInstruction(
                            cg, TR::InstOpCode::addi, node, tmp5Reg, tmp5Reg, (int32_t)(unrollFactor * width), iCursor);

                        iCursor = generateTrg1Src2Instruction(
                            cg, TR::InstOpCode::Op_cmpl, node, condReg, tmp6Reg, tmp5Reg, iCursor);
                        iCursor = generateConditionalBranchInstruction(
                            cg, TR::InstOpCode::bge, node, loopLabel, condReg, iCursor);
                        ...
                        if (!disableNewZeroInit) {
                            // End pointer-4
                            iCursor = generateTrg1Src1ImmInstruction(
                                cg, TR::InstOpCode::addi, node, tmp6Reg, tmp4Reg, -4, iCursor);
                            ...
                            iCursor = generateTrg1Src2Instruction(
                                cg, TR::InstOpCode::Op_cmpl, node, condReg, tmp6Reg, tmp5Reg, iCursor);
                            iCursor = generateConditionalBranchInstruction(
                                cg, TR::InstOpCode::beq, node, left4Label, condReg, iCursor);
                            iCursor = generateConditionalBranchInstruction(
                                cg, TR::InstOpCode::blt, node, doneLabel, condReg, iCursor);

                            // Do 8-byte zero init and restart loop
                            iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::Op_st, node,
                                new (cg->trHeapMemory()) TR::MemoryReference(tmp5Reg, width, width, cg), zeroReg,
                                iCursor);

                            if (TR::Compiler->target.is32Bit()) {
                                iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                                    new (cg->trHeapMemory()) TR::MemoryReference(tmp5Reg, 8, 4, cg), zeroReg, iCursor);
                            }

                            iCursor = generateTrg1Src1ImmInstruction(
                                cg, TR::InstOpCode::addi, node, tmp5Reg, tmp5Reg, 8, iCursor);
                            ...
                            iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                                new (cg->trHeapMemory()) TR::MemoryReference(tmp5Reg, width, 4, cg), zeroReg, iCursor);
                        } else {
                            /* Need this in 64-bit as well */
                            iCursor = generateTrg1Src2Instruction(
                                cg, TR::InstOpCode::subf, node, tmp6Reg, tmp5Reg, tmp4Reg, iCursor);
                            iCursor = generateTrg1Src1ImmInstruction(
                                cg, TR::InstOpCode::Op_cmpli, node, condReg, tmp6Reg, 4, iCursor);
                            iCursor = generateConditionalBranchInstruction(
                                cg, TR::InstOpCode::beq, node, left4Label, condReg, iCursor);

                            iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::Op_st, node,
                                new (cg->trHeapMemory()) TR::MemoryReference(tmp5Reg, 8, width, cg), zeroReg, iCursor);

                            if (TR::Compiler->target.is32Bit()) {
                                iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                                    new (cg->trHeapMemory()) TR::MemoryReference(tmp5Reg, 12, 4, cg), zeroReg, iCursor);
                            }
                        ...
                        iCursor = generateTrg1Src1Imm2Instruction(
                            cg, TR::InstOpCode::rlwinm, node, tmp5Reg, dataSizeReg, 30, 0x3FFFFFFF, iCursor);
                        iCursor = generateTrg1Src1ImmInstruction(
                            cg, TR::InstOpCode::cmpi4, node, condReg, tmp5Reg, 0, iCursor);
                        iCursor = generateConditionalBranchInstruction(
                            cg, TR::InstOpCode::beq, node, doneLabel, condReg, iCursor);
                        iCursor = generateSrc1Instruction(cg, TR::InstOpCode::mtctr, node, tmp5Reg, 0, iCursor);
                        iCursor = generateTrg1Src1ImmInstruction(
                            cg, TR::InstOpCode::addi, node, tmp4Reg, resReg, dataBegin - 4, iCursor);

                        TR::LabelSymbol* initLoop = generateLabelSymbol(cg);
                        iCursor = generateLabelInstruction(cg, TR::InstOpCode::label, node, initLoop, iCursor);
                        iCursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                            new (cg->trHeapMemory()) TR::MemoryReference(tmp4Reg, 4, 4, cg), zeroReg, iCursor);
                        iCursor = generateTrg1Src1ImmInstruction(
                            cg, TR::InstOpCode::addi, node, tmp4Reg, tmp4Reg, 4, iCursor);
                        iCursor = generateConditionalBranchInstruction(
                            cg, TR::InstOpCode::bdnz, node, initLoop, condReg, iCursor);
                    }
                }
    ...
    TR::Register* objectClassReg, TR::Register* condReg, TR::Register* lookupOffsetReg)
{
    ...
    if (comp->getOption(TR_TraceCG) || debug("traceGRA")) {
        ...
        comp->getDebug()->dumpSingleTreeWithInstrs(
            nextTreeTop, NULL, true, false, true, comp->getOption(TR_TraceRegisterPressureDetails));
        trfprintf(comp->getOutFile(), "\n");
        comp->getDebug()->dumpSingleTreeWithInstrs(
            secondNextTreeTop, NULL, true, false, true, comp->getOption(TR_TraceRegisterPressureDetails));
    ...
    bool altsequence = (TR::Compiler->target.cpu.id() == TR_PPCgp || TR::Compiler->target.cpu.id() == TR_PPCgr)
        && tempMR->getBaseRegister() == objReg;
    ...
    generateLabelInstruction(cg, TR::InstOpCode::label, node, loopLabel);
    generateTrg1MemInstruction(
        cg, opCode, node, monitorReg, new (cg->trHeapMemory()) TR::MemoryReference(baseReg, offsetReg, lockSize, cg));
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::Op_cmpli, node, cndReg, monitorReg, 0);
    TR::Instruction* gcPoint
    ...
    if (altsequence) {
        if (TR::Compiler->target.is64Bit() && !fej9->generateCompressedLockWord())
            ...
            opCode = TR::InstOpCode::stwcx_r;
        generateMemSrc1Instruction(
            cg, opCode, node, new (cg->trHeapMemory()) TR::MemoryReference(baseReg, offsetReg, lockSize, cg), metaReg);
        if (TR::Compiler->target.cpu.id() >= TR_PPCgp)
            // use PPC AS branch hint
            generateConditionalBranchInstruction(
            ...
            generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, node, loopLabel, cndReg);
        generateTrg1Src2Instruction(
            cg, TR::InstOpCode::OR, node, tempMR->getBaseRegister(), tempMR->getBaseRegister(), monitorReg);
        ...
        if (TR::Compiler->target.is64Bit() && !fej9->generateCompressedLockWord())
            ...
            opCode = TR::InstOpCode::stw;
        generateMemSrc1Instruction(cg, opCode, node,
            new (cg->trHeapMemory()) TR::MemoryReference(baseReg,
        ...
        if (TR::Compiler->target.is64Bit() && !fej9->generateCompressedLockWord())
            ...
            opCode = TR::InstOpCode::stwcx_r;
        generateMemSrc1Instruction(cg, opCode, node,
            new (cg->trHeapMemory()) TR::MemoryReference(baseReg, offsetReg, lockSize, cg), monitorReg);
        if (TR::Compiler->target.cpu.id() >= TR_PPCgp)
            // use PPC AS branch hint
            generateConditionalBranchInstruction(
                cg, TR::InstOpCode::bne, PPCOpProp_BranchUnlikely, node, loopLabel, cndReg);
    ...
    TR::Snippet* snippet = new (cg->trHeapMemory()) TR::PPCReadMonitorSnippet(cg, node, secondNextTopNode,
        recurCheckLabel, monExitCallLabel, restartLabel, loadOpCode, tempMR->getOffset(), objectClassReg);
    cg->addSnippet(snippet);
...
TR::Register* J9::Power::TreeEvaluator::VMmonentEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (comp->getOption(TR_OptimizeForSpace) || comp->getOption(TR_MimicInterpreterFrameShape)
    ...
    if (!ppcSupportsReadMonitors || !node->isReadMonitor()) {
        ...
        generateTrg1Src1Imm2Instruction(cg,
            TR::Compiler->target.is64Bit() ? TR::InstOpCode::rldicr : TR::InstOpCode::rlwinm, node, threadReg,
            monitorReg, 0, LOCK_THREAD_PTR_AND_UPPER_COUNT_BIT_MASK);
        generateTrg1Src2Instruction(cg, TR::InstOpCode::Op_cmpl, node, condReg, threadReg, metaReg);
        generateConditionalBranchInstruction(cg, TR::InstOpCode::bne, node, callLabel, condReg);
        generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, monitorReg, monitorReg, LOCK_INC_DEC_VALUE);
        generateMemSrc1Instruction(cg, lockSize == 8 ? TR::InstOpCode::std : TR::InstOpCode::stw, node,
            new (cg->trHeapMemory()) TR::MemoryReference(baseReg, lwOffset, lockSize, cg), monitorReg);
        ...
        if (TR::Compiler->target.is64Bit() && !fej9->generateCompressedLockWord())
            ...
            opCode = TR::InstOpCode::stwcx_r;
        generateMemSrc1Instruction(cg, opCode, node,
            new (cg->trHeapMemory()) TR::MemoryReference(baseReg, offsetReg, lockSize, cg), readerReg);
...
TR::Register* J9::Power::TreeEvaluator::VMarrayCheckEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!node->isArrayChkPrimitiveArray1() && !node->isArrayChkReferenceArray1() && !node->isArrayChkPrimitiveArray2()
        ...
        generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, tmp1Reg,
            new (cg->trHeapMemory())
                TR::MemoryReference(obj1Reg, (int32_t)TR::Compiler->om.offsetOfObjectVftField(), 4, cg));
#else
        ...
        generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, tmp1Reg,
            new (cg->trHeapMemory())
                TR::MemoryReference(tmp1Reg, (int32_t)offsetof(J9Class, classDepthAndFlags), 4, cg));
    ...
    generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, tmp2Reg,
        new (cg->trHeapMemory())
            TR::MemoryReference(obj2Reg, (int32_t)TR::Compiler->om.offsetOfObjectVftField(), 4, cg));
    generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, tmp1Reg,
        new (cg->trHeapMemory())
            TR::MemoryReference(obj1Reg, (int32_t)TR::Compiler->om.offsetOfObjectVftField(), 4, cg));
#else
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, tmp2Reg,
        new (cg->trHeapMemory()) TR::MemoryReference(obj2Reg, (int32_t)TR::Compiler->om.offsetOfObjectVftField(),
            TR::Compiler->om.sizeofReferenceAddress(), cg));
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, tmp1Reg,
        new (cg->trHeapMemory()) TR::MemoryReference(obj1Reg, (int32_t)TR::Compiler->om.offsetOfObjectVftField(),
            TR::Compiler->om.sizeofReferenceAddress(), cg));
#endif
    ...
    // We have to take care of the un-equal class situation: both of them must be of reference array
        ...
        if (!node->isArrayChkReferenceArray1()) {
            ...
            // We already have classDepth&Flags in tmp1Reg.  X =
            // (ramclass->ClassDepthAndFlags)>>J9_JAVA_CLASS_RAM_SHAPE_SHIFT
            ...
            generateTrg1Src1Imm2Instruction(
                cg, TR::InstOpCode::rlwinm, node, tmp2Reg, tmp1Reg, 0, OBJECT_HEADER_SHAPE_MASK);
            generateTrg1Src1ImmInstruction(
                cg, TR::InstOpCode::cmpi4, node, cndReg, tmp2Reg, OBJECT_HEADER_SHAPE_POINTERS);
...
void J9::Power::TreeEvaluator::genArrayCopyWithArrayStoreCHK(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    gcPoint = generateDepConditionalBranchInstruction(
        cg, TR::InstOpCode::bnel, node, exceptionSnippetLabel, cr0Reg, conditions->cloneAndFix(cg));
...
void J9::Power::TreeEvaluator::genWrtbarForArrayCopy(
...
static TR::Register* genCAS(TR::Node* node, TR::CodeGenerator* cg, TR::Register* objReg, TR::Register* offsetReg,
    ...
    int32_t oldValue, bool oldValueInReg, bool isLong, bool casWithoutSync = false)
{
    TR::Register* resultReg = cg->allocateRegister();
    TR::Instruction* gcPoint;
    ...
    generateTrg1MemInstruction(cg, isLong ? TR::InstOpCode::ldarx : TR::InstOpCode::lwarx, node, resultReg,
        new (cg->trHeapMemory()) TR::MemoryReference(objReg, offsetReg, isLong ? 8 : 4, cg));
    if (oldValueInReg)
        generateTrg1Src2Instruction(
            cg, isLong ? TR::InstOpCode::cmp8 : TR::InstOpCode::cmp4, node, cndReg, resultReg, oldVReg);
    else
        generateTrg1Src1ImmInstruction(
            cg, isLong ? TR::InstOpCode::cmpi8 : TR::InstOpCode::cmpi4, node, cndReg, resultReg, oldValue);
    generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, resultReg, 0);
    ...
    generateMemSrc1Instruction(cg, isLong ? TR::InstOpCode::stdcx_r : TR::InstOpCode::stwcx_r, node,
        new (cg->trHeapMemory()) TR::MemoryReference(objReg, offsetReg, isLong ? 8 : 4, cg), newVReg);
...
static TR::Register* VMinlineCompareAndSwap(TR::Node* node, TR::CodeGenerator* cg, bool isLong)
{
    ...
    resultReg = genCAS(node, cg, objReg, offsetReg, oldVReg, newVReg, cndReg, doneLabel, secondChild, oldValue,
        oldValueInReg, isLong, casWithoutSync);
...
static TR::Register* VMinlineCompareAndSwapObject(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!TR::Options::getCmdLineOptions()->realTimeGC())
        resultReg = genCAS(node, cg, objReg, offsetReg, oldVReg, newVReg, cndReg, doneLabel, secondChild, 0, true,
    ...
    if (doWrtBar && doCrdMrk) {
        ...
        VMnonNullSrcWrtBarCardCheckEvaluator(node, comp->useCompressedPointers() ? wrtbarSrcReg : newVReg, objReg,
            cndReg, temp1Reg, temp2Reg, temp3Reg, temp4Reg, doneLabel, conditions, NULL, false, false, cg);
        ...
        VMnonNullSrcWrtBarCardCheckEvaluator(node,
            comp->useCompressedPointers() ? translatedNode->getRegister() : newVReg, objReg, cndReg, temp1Reg, temp2Reg,
            dstAddrReg, NULL, wrtBarEndLabel, conditions, NULL, dstAddrComputed, false, cg);
...
void J9::Power::TreeEvaluator::restoreTOCRegister(
    TR::Node* node, TR::CodeGenerator* cg, TR::RegisterDependencyConditions* dependencies)
{
    ...
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, tocReg,
        new (cg->trHeapMemory()) TR::MemoryReference(cg->getMethodMetaDataRegister(), offsetof(J9VMThread, jitTOC),
            TR::Compiler->om.sizeofReferenceAddress(), cg));
#else
...
void J9::Power::TreeEvaluator::buildArgsProcessFEDependencies(
    TR::Node* node, TR::CodeGenerator* cg, TR::RegisterDependencyConditions* dependencies)
{
...
TR::Register* J9::Power::TreeEvaluator::retrieveTOCRegister(
    TR::Node* node, TR::CodeGenerator* cg, TR::RegisterDependencyConditions* dependencies)
{
#if defined(DEBUG)
...
static TR::Register* inlineAtomicOps(
    TR::Node* node, TR::CodeGenerator* cg, int8_t size, TR::MethodSymbol* method, bool isArray)
{
    ...
    if (!isArray) {
        ...
        if (isArray) {
            ...
            TR::MemoryReference* tempMR
                = new (cg->trHeapMemory()) TR::MemoryReference(valueReg, arrayFieldOffset, isLong ? 8 : 4, cg);
            ...
            generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, memRefRegister,
                new (cg->trHeapMemory())
                    TR::MemoryReference(valueReg, arrayFieldOffset, TR::Compiler->om.sizeofReferenceAddress(), cg));
#endif
            ...
            generateTrg1Src1ImmInstruction(
                cg, TR::InstOpCode::addi2, node, fieldOffsetReg, fieldOffsetReg, fieldOffset);
        }
    }
    ...
    if (isAddOp) {
        if (isArgImmediateShifted)
            generateTrg1Src1ImmInstruction(
                cg, TR::InstOpCode::addis, node, deltaReg, resultReg, ((delta & 0xFFFF0000) >> 16));
        else if (isArgImmediate)
    ...
    generateMemSrc1Instruction(cg, isLong ? TR::InstOpCode::stdcx_r : TR::InstOpCode::stwcx_r, node,
        new (cg->trHeapMemory()) TR::MemoryReference(valueReg, fieldOffsetReg, isLong ? 8 : 4, cg), deltaReg);
...
static TR::Register* inlineAtomicOperation(TR::Node* node, TR::CodeGenerator* cg, TR::MethodSymbol* method)
{
    ...
    if (isUnsafe) {
        if (indexChild->getOpCode().isLoadConst() && indexChild->getRegister() == NULL
    ...
    } else // isArray
    {
        ...
        TR::MemoryReference* tempMR
            = new (cg->trHeapMemory()) TR::MemoryReference(valueReg, arrayFieldOffset, size, cg);
    ...
    if (isCAS) {
        ...
        generateTrg1MemInstruction(cg, size == 8 ? TR::InstOpCode::ldarx : TR::InstOpCode::lwarx, node, currentReg,
            new (cg->trHeapMemory()) TR::MemoryReference(valueReg,
                (fieldOffsetReg->getRegisterPair() ? fieldOffsetReg->getLowOrder() : fieldOffsetReg), size, cg));
        ...
        if (isArgImm) {
            generateTrg1Src1ImmInstruction(
                cg, size == 8 ? TR::InstOpCode::cmpi8 : TR::InstOpCode::cmpi4, node, cndReg, currentReg, expValImm);
        } else {
            generateTrg1Src2Instruction(
                cg, size == 8 ? TR::InstOpCode::cmp8 : TR::InstOpCode::cmp4, node, cndReg, currentReg, expComposedReg);
        }
        ...
        if (TR::Compiler->target.cpu.id() >= TR_PPCgp)
            generateConditionalBranchInstruction(
                cg, TR::InstOpCode::bne, PPCOpProp_BranchUnlikely, node, rsvFailLabel, cndReg);
        else
        ...
        if (!isWeak && size == 8 && TR::Compiler->target.is32Bit()) {
            ...
            if (TR::Compiler->target.cpu.id() >= TR_PPCgp)
                generateConditionalBranchInstruction(
                    cg, TR::InstOpCode::bne, PPCOpProp_BranchUnlikely, node, startLabel, cndReg);
            else
...
static TR::Register* inlineIntegerRotateRight(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::iconst || secondChild->getOpCodeValue() == TR::iuconst) {
        ...
        generateTrg1Src2ImmInstruction(
            cg, TR::InstOpCode::rlwnm, node, targetRegister, srcRegister, targetRegister, 0xffffffff);
    }
...
static TR::Register* inlineLongRotateRight(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::iconst || secondChild->getOpCodeValue() == TR::iuconst) {
        ...
        generateTrg1Src2ImmInstruction(cg, TR::InstOpCode::rldcl, node, targetRegister, srcRegister, targetRegister,
...
static TR::Register* inlineShortReverseBytes(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!firstNonConversionOpCodeNode->getRegister() && firstNonConversionOpCodeNode->getOpCode().isMemoryReference()
        && firstNonConversionOpCodeNode->getReferenceCount() == 1
        && (nodeType.isInt16() || nodeType.isInt32() || nodeType.isInt64())) {
...
static TR::Register* inlineIntegerReverseBytes(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    // ibyteswap provides the same functionality as reverseBytes, so the node is recreated as an ibyteswap and evaluated
    // as such
...
static TR::Register* inlineLongReverseBytes(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.is64Bit()) {
        ...
        if (TR::Compiler->target.cpu.id() >= TR_PPCp7 && !firstChild->getRegister()
            ...
            generateTrg1Src1Imm2Instruction(
                cg, TR::InstOpCode::rldicl, node, srcHRegister, srcLRegister, 32, 0x00ffffffff);

            generateTrg1Src1Imm2Instruction(
                cg, TR::InstOpCode::rlwinm, node, tgtRegister, srcHRegister, 8, 0x00000000ff);
            generateTrg1Src1Imm2Instruction(
                cg, TR::InstOpCode::rlwinm, node, tgtHRegister, srcLRegister, 8, 0x00000000ff);

            generateTrg1Src1Imm2Instruction(
                cg, TR::InstOpCode::rlwinm, node, tmp1Register, srcHRegister, 8, 0x0000ff0000);
            generateTrg1Src1Imm2Instruction(
                cg, TR::InstOpCode::rlwinm, node, tmp2Register, srcLRegister, 8, 0x0000ff0000);
            ...
            generateTrg1Src1Imm2Instruction(
                cg, TR::InstOpCode::rlwinm, node, tmp1Register, srcHRegister, 24, 0x000000ff00);
            generateTrg1Src1Imm2Instruction(
                cg, TR::InstOpCode::rlwinm, node, tmp2Register, srcLRegister, 24, 0x000000ff00);
            generateTrg1Src2Instruction(cg, TR::InstOpCode::OR, node, tgtRegister, tgtRegister, tmp1Register);
            generateTrg1Src2Instruction(cg, TR::InstOpCode::OR, node, tgtHRegister, tgtHRegister, tmp2Register);
    ...
    } else // 32-Bit Target
    {
        ...
        generateTrg1Src1Imm2Instruction(
            cg, TR::InstOpCode::rlwinm, node, tgtRegister->getLowOrder(), srcRegister->getHighOrder(), 8, 0x00000000ff);
        generateTrg1Src1Imm2Instruction(
            cg, TR::InstOpCode::rlwinm, node, tgtRegister->getHighOrder(), srcRegister->getLowOrder(), 8, 0x00000000ff);

        generateTrg1Src1Imm2Instruction(
            cg, TR::InstOpCode::rlwinm, node, tmp1Register, srcRegister->getHighOrder(), 8, 0x0000ff0000);
        generateTrg1Src1Imm2Instruction(
            cg, TR::InstOpCode::rlwinm, node, tmp2Register, srcRegister->getLowOrder(), 8, 0x0000ff0000);
        generateTrg1Src2Instruction(
        ...
        generateTrg1Src1Imm2Instruction(
            cg, TR::InstOpCode::rlwinm, node, tmp1Register, srcRegister->getHighOrder(), 24, 0x000000ff00);
        generateTrg1Src1Imm2Instruction(
            cg, TR::InstOpCode::rlwinm, node, tmp2Register, srcRegister->getLowOrder(), 24, 0x000000ff00);
        generateTrg1Src2Instruction(
            cg, TR::InstOpCode::OR, node, tgtRegister->getLowOrder(), tgtRegister->getLowOrder(), tmp1Register);
        generateTrg1Src2Instruction(
            cg, TR::InstOpCode::OR, node, tgtRegister->getHighOrder(), tgtRegister->getHighOrder(), tmp2Register);
        generateTrg1Src1Imm2Instruction(
            cg, TR::InstOpCode::rlwinm, node, tmp1Register, srcRegister->getHighOrder(), 24, 0x00ff000000);
        generateTrg1Src1Imm2Instruction(
            cg, TR::InstOpCode::rlwinm, node, tmp2Register, srcRegister->getLowOrder(), 24, 0x00ff000000);
        generateTrg1Src2Instruction(
            cg, TR::InstOpCode::OR, node, tgtRegister->getLowOrder(), tgtRegister->getLowOrder(), tmp1Register);
        generateTrg1Src2Instruction(
            cg, TR::InstOpCode::OR, node, tgtRegister->getHighOrder(), tgtRegister->getHighOrder(), tmp2Register);
...
static TR::Register* compressStringEvaluator(TR::Node* node, TR::CodeGenerator* cg, bool japaneseMethod)
{
    ...
    TR::RegisterDependencyConditions* conditions
        = new (cg->trHeapMemory()) TR::RegisterDependencyConditions(12, 12, cg->trMemory());
    ...
    TR::Register* regs[5] = { lengthReg, startReg, srcObjReg, dstObjReg, resultReg };
...
static TR::Register* compressStringNoCheckEvaluator(TR::Node* node, TR::CodeGenerator* cg, bool japaneseMethod)
{
    ...
    TR::RegisterDependencyConditions* conditions
        = new (cg->trHeapMemory()) TR::RegisterDependencyConditions(numOfRegs, numOfRegs, cg->trMemory());
    TR::Register* cndRegister = cg->allocateRegister(TR_CCR);
    addDependency(conditions, cndRegister, TR::RealRegister::cr0, TR_CCR, cg);
    addDependency(conditions, lengthReg, TR::RealRegister::gr8, TR_GPR, cg);
    addDependency(conditions, startReg, TR::RealRegister::gr7, TR_GPR, cg);
    ...
    TR::Register* regs[4] = { lengthReg, startReg, srcObjReg, dstObjReg };
...
static TR::Register* andORStringEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::Register* regs[4] = { lengthReg, startReg, srcObjReg, resultReg };
    conditions->stopUsingDepRegs(cg, 4, regs);
...
static TR::Register* inlineConcurrentLinkedQueueTMOffer(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    classBlock = fej9->getClassFromSignature(
        "Ljava/util/concurrent/ConcurrentLinkedQueue$Node;", 49, comp->getCurrentMethod(), true);
    ...
    classBlock = fej9->getClassFromSignature(
        "Ljava/util/concurrent/ConcurrentLinkedQueue;", 44, comp->getCurrentMethod(), true);
    ...
    generateTrg1MemInstruction(cg, loadOpCode, node, pReg,
        new (cg->trHeapMemory()) TR::MemoryReference(objReg, offsetTail, addressFieldSize, cg));
    ...
    generateTrg1MemInstruction(cg, loadOpCode, node, qReg,
        new (cg->trHeapMemory()) TR::MemoryReference(pReg, offsetNext, addressFieldSize, cg));
    ...
    if (usesCompressedrefs) {
        compressedReg = genCompressPointerNonNull2RegsWithTempReg(
            cg, node, nReg, qReg, resultReg); // In Java code checkNotNull() already ensures n != null
    }
    // ALG: p.next := n
    generateMemSrc1Instruction(cg, storeOpCode, node,
        new (cg->trHeapMemory()) TR::MemoryReference(pReg, offsetNext, addressFieldSize, cg), compressedReg);

    // ALG: this.tail = n
    generateMemSrc1Instruction(cg, storeOpCode, node,
        new (cg->trHeapMemory()) TR::MemoryReference(objReg, offsetTail, addressFieldSize, cg), compressedReg);
    ...
    if (doWrtBar) {
        ...
        VMnonNullSrcWrtBarCardCheckEvaluator(node, nReg, pReg, cndReg, qReg, retryCountReg, temp3Reg, temp4Reg,
            wrtbar1Donelabel, conditions, NULL, false, false, cg, false);
        ...
        VMnonNullSrcWrtBarCardCheckEvaluator(node, nReg, pReg, cndReg, qReg, retryCountReg, temp3Reg, temp4Reg,
            returnLabel, conditions, NULL, false, false, cg, false);
...
static TR::Register* inlineConcurrentLinkedQueueTMPoll(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR_OpaqueClassBlock* classBlock = fej9->getClassFromSignature(
        "Ljava/util/concurrent/ConcurrentLinkedQueue;", 44, comp->getCurrentMethod(), true);
    ...
    generateTrg1MemInstruction(cg, loadOpCode, node, temp3Reg,
        new (cg->trHeapMemory()) TR::MemoryReference(objReg, offsetHead, addressFieldSize, cg));
    ...
    generateTrg1MemInstruction(cg, loadOpCode, node, resultReg,
        new (cg->trHeapMemory()) TR::MemoryReference(decompressedP, offsetItem, addressFieldSize, cg));
    if (usesCompressedrefs) {
    ...
    // ALG: q := p.next
    generateTrg1MemInstruction(cg, loadOpCode, node, qReg,
        new (cg->trHeapMemory()) TR::MemoryReference(decompressedP, offsetNext, addressFieldSize, cg));

    // ALG: p.item := null
    generateTrg1ImmInstruction(cg, TR::InstOpCode::li, node, nullReg, 0);

    generateMemSrc1Instruction(cg, storeOpCode, node,
        new (cg->trHeapMemory()) TR::MemoryReference(decompressedP, offsetItem, addressFieldSize, cg), nullReg);
    ...
    generateMemSrc1Instruction(cg, storeOpCode, node,
        new (cg->trHeapMemory()) TR::MemoryReference(objReg, offsetHead, addressFieldSize, cg), qReg);
    ...
    // ALG: p.next := p
    generateMemSrc1Instruction(cg, storeOpCode, node,
        new (cg->trHeapMemory()) TR::MemoryReference(decompressedP, offsetNext, addressFieldSize, cg), temp3Reg);
    ...
    if (doWrtBar) {
        ...
        VMnonNullSrcWrtBarCardCheckEvaluator(node, qReg, objReg, cndReg, nullReg, pReg, temp3Reg, temp4Reg, returnLabel,
            conditions, NULL, false, false, cg, false);
    } else if (doCrdMrk) {
...
static TR::Register* dangerousGetCPU(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    generateMemSrc1Instruction(cg, TR::InstOpCode::Op_st, node,
        new (cg->trHeapMemory()) TR::MemoryReference(gr1, 0x14, TR::Compiler->om.sizeofReferenceAddress(), cg), gr2);
    intptr_t xx[3];
    ...
    generateDepLabelInstruction(
        cg, TR::InstOpCode::label, node, TR::LabelSymbol::create(cg->trHeapMemory(), cg), dependencies);
...
static TR::Register* inlineAbsLong(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.is64Bit()) {
        ...
        generateTrg1Src1Instruction(
            cg, TR::InstOpCode::mr, node, resultRegister->getLowOrder(), firstRegister->getLowOrder());
        generateTrg1Src1Instruction(
            cg, TR::InstOpCode::mr, node, resultRegister->getHighOrder(), firstRegister->getHighOrder());
        ...
        if (!firstChild->isNonNegative()) {
            ...
            generateTrg1Src1ImmInstruction(
                cg, TR::InstOpCode::srawi, node, tempRegister, resultRegister->getHighOrder(), 31);

            generateTrg1Src2Instruction(cg, TR::InstOpCode::XOR, node, resultRegister->getHighOrder(),
                resultRegister->getHighOrder(), tempRegister);
            generateTrg1Src2Instruction(cg, TR::InstOpCode::XOR, node, resultRegister->getLowOrder(),
                resultRegister->getLowOrder(), tempRegister);

            generateTrg1Src2Instruction(cg, TR::InstOpCode::subfc, node, resultRegister->getLowOrder(), tempRegister,
                resultRegister->getLowOrder());
            generateTrg1Src2Instruction(cg, TR::InstOpCode::subfe, node, resultRegister->getHighOrder(), tempRegister,
                resultRegister->getHighOrder());
...
static TR::Register* inlineIsAssignableFrom(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, receiverJ9ClassReg,
        new (cg->trHeapMemory()) TR::MemoryReference(receiverReg, fej9->getOffsetOfClassFromJavaLangClassField(),
            TR::Compiler->om.sizeofReferenceAddress(), cg));
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, parmJ9ClassReg,
        new (cg->trHeapMemory()) TR::MemoryReference(
            parmReg, fej9->getOffsetOfClassFromJavaLangClassField(), TR::Compiler->om.sizeofReferenceAddress(), cg));
...
static TR::Register* inlineStringHashcode(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    static uint32_t multiplierVectors_be[12] = { 0x94446F01, 0x94446F01, 0x94446F01, 0x94446F01, 0x67E12CDF, 887503681,
        28629151, 923521, 29791, 961, 31, 1 };
    static uint32_t multiplierVectors_le[12] = { 0x94446F01, 0x94446F01, 0x94446F01, 0x94446F01, 1, 31, 961, 29791,
        923521, 28629151, 887503681, 0x67E12CDF };
    ...
    generateTrg1MemInstruction(cg, TR::InstOpCode::lxvw4x, node, multiplierReg,
        new (cg->trHeapMemory()) TR::MemoryReference(multiplierAddrReg, constant0Reg, 16, cg));
    ...
    generateTrg1MemInstruction(cg, TR::InstOpCode::lvx, node, vtmp1Reg,
        new (cg->trHeapMemory()) TR::MemoryReference(valueReg, constant0Reg, 16, cg));
    loadConstant(cg, node, 0xF, tempReg);
    generateTrg1Src2Instruction(cg, TR::InstOpCode::AND, node, tempReg, valueReg, tempReg);
    ...
    generateTrg1MemInstruction(cg, TR::InstOpCode::lhzx, node, tempReg,
        new (cg->trHeapMemory()) TR::MemoryReference(valueReg, constant0Reg, 2, cg));
    ...
    // End of this method
    TR::RegisterDependencyConditions* dependencies
        = new (cg->trHeapMemory()) TR::RegisterDependencyConditions(0, 16, cg->trMemory());
...
static TR::Register* inlineEncodeUTF16(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    generateDepImmSymInstruction(
        cg, TR::InstOpCode::bl, node, (uintptrj_t)helperSym->getMethodAddress(), deps, helperSym);
...
static TR::Register* inlineIntrinsicIndexOf(TR::Node* node, bool isLatin1, TR::CodeGenerator* cg)
{
    ...
    generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, arrAddress, arrObjectAddress,
    ...
    // to each String.indexOf call is ' ', which is the most common whitespace character.
    {
        ...
        generateTrg1MemInstruction(cg, scalarLoadOp, node, value,
            new (cg->trHeapMemory()) TR::MemoryReference(result, arrAddress, isLatin1 ? 1 : 2, cg));
    ...
    generateTrg1Src1Imm2Instruction(
        cg, TR::InstOpCode::rldicr, node, endVectorAddress, endAddress, 0, CONSTANT64(0xfffffffffffffff0));
    ...
    generateTrg1Src1Imm2Instruction(
        cg, TR::InstOpCode::rldicr, node, startVectorAddress, currentAddress, 0, CONSTANT64(0xfffffffffffffff0));
    ...
    generateTrg1MemInstruction(cg, TR::InstOpCode::lvx, node, searchVector,
        new (cg->trHeapMemory()) TR::MemoryReference(zeroRegister, currentAddress, 16, cg));

    {
        ...
        generateTrg1Src2Instruction(cg,
            TR::Compiler->target.cpu.isLittleEndian() ? TR::InstOpCode::lvsl : TR::InstOpCode::lvsr, node,
            permuteVector, zeroRegister, scratchRegister);
        generateTrg1Src3Instruction(
            cg, TR::InstOpCode::vperm, node, searchVector, searchVector, searchVector, permuteVector);
        ...
        if (TR::Compiler->target.cpu.isLittleEndian()) {
            ...
            generateTrg1Src3Instruction(
                cg, TR::InstOpCode::vperm, node, searchVector, targetVectorNot, searchVector, permuteVector);
        } else {
            generateTrg1Src2Instruction(cg, TR::InstOpCode::lvsl, node, permuteVector, zeroRegister, scratchRegister);
            generateTrg1Src3Instruction(
                cg, TR::InstOpCode::vperm, node, searchVector, searchVector, targetVectorNot, permuteVector);
        }
...
extern TR::Register* inlineBigDecimalBinaryOp(
    TR::Node* node, TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, bool scaled);
...
extern TR::Register* inlineBigDecimalUnaryOp(
    TR::Node* node, TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, bool precision);
...
bool J9::Power::CodeGenerator::inlineDirectCall(TR::Node* node, TR::Register*& resultReg)
{
    ...
    if (comp->getSymRefTab()->isNonHelper(
            node->getSymbolReference(), TR::SymbolReferenceTable::singlePrecisionSQRTSymbol)) {
        ...
        case TR::sun_misc_Unsafe_copyMemory:
            if (comp->canTransformUnsafeCopyToArrayCopy()
                && performTransformation(
                       comp, "O^O Call arraycopy instead of Unsafe.copyMemory: %s\n", cg->getDebug()->getName(node))) {
        ...
        case TR::sun_misc_Unsafe_setMemory:
            if (comp->canTransformUnsafeSetMemory()) {
                ...
                TR::Node* copyMemNode
                    = TR::Node::createWithSymRef(TR::arrayset, 3, 3, dest, len, byteValue, node->getSymbolReference());
        ...
        case TR::java_lang_String_hashCodeImplDecompressed:
            if (!TR::Compiler->om.canGenerateArraylets() && TR::Compiler->target.cpu.id() >= TR_PPCp8
                && TR::Compiler->target.cpu.getPPCSupportsVSX() && !cg->comp()->compileRelocatableCode()) {
        ...
        case TR::java_util_concurrent_atomic_Fences_orderReads:
            if (performTransformation(
                    comp, "O^O PPC Evaluator: Replacing read/read Fence with an lwsync [%p].\n", node)) {
        ...
        case TR::java_util_concurrent_atomic_Fences_orderWrites:
            if (performTransformation(
                    comp, "O^O PPC Evaluator: Replacing store/store Fence with an lwsync [%p].\n", node)) {
                // mark as seen and then just don't bother generating instructions
                TR::Node* callNode = node;
                int32_t numArgs = callNode->getNumChildren();
                for (int32_t i = numArgs - 1; i >= 0; i--)
        ...
        case TR::java_util_concurrent_atomic_Fences_orderAccesses:
            if (performTransformation(
                    comp, "O^O PPC Evaluator: Replacing store/load Fence with a sync [%p].\n", node)) {
                TR::Node* callNode = node;
                int32_t numArgs = callNode->getNumChildren();
                for (int32_t i = numArgs - 1; i >= 0; i--)
                    cg->decReferenceCount(callNode->getChild(i));
        ...
        case TR::java_lang_Class_isAssignableFrom: {
            ...
            if (!disable
                && performTransformation(
                       comp, "O^O PPC Evaluator: Specialize call to java/lang/Class.isAssignableFrom [%p].\n", node)) {
...
void VMgenerateCatchBlockBBStartPrologue(TR::Node* node, TR::Instruction* fenceInstruction, TR::CodeGenerator* cg)
{
    ...
    if (fej9->shouldPerformEDO(block, comp)) {
        ...
        TR::MemoryReference* loadbiMR
            = new (cg->trHeapMemory()) TR::MemoryReference(biAddrReg, 0, TR::Compiler->om.sizeofReferenceAddress(), cg);
        TR::MemoryReference* storebiMR
            = new (cg->trHeapMemory()) TR::MemoryReference(biAddrReg, 0, TR::Compiler->om.sizeofReferenceAddress(), cg);
        cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, recompCounterReg, loadbiMR);
        TR::Register* cndRegister = cg->allocateRegister(TR_CCR);
        cursor = generateTrg1Src1ImmInstruction(
            cg, TR::InstOpCode::addi2_r, node, recompCounterReg, recompCounterReg, cndRegister, -1, cursor);
...
TR::Instruction* loadAddressRAM32(TR::CodeGenerator* cg, TR::Node* node, int32_t value, TR::Register* trgReg)
{
    ...
    if (value != 0x0) {
        ...
        if (isAOT)
            cg->addExternalRelocation(new (cg->trHeapMemory())
                                          TR::ExternalRelocation((uint8_t*)cursor, (uint8_t*)node->getSymbolReference(),
    ...
    cursor = generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::ori, node, trgReg, trgReg, isAOT ? 0 : value & 0x0000ffff, cursor);
...
TR::Instruction* loadAddressRAM(TR::CodeGenerator* cg, TR::Node* node, intptrj_t value, TR::Register* trgReg)
{
    ...
    cursor = generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::ori, node, trgReg, trgReg, isAOT ? 0 : ((value >> 32) & 0x0000ffff), cursor);
    ...
    cursor = generateTrg1Src1ImmInstruction(
        cg, TR::InstOpCode::oris, node, trgReg, trgReg, isAOT ? 0 : ((value >> 16) & 0x0000ffff), cursor);
...
TR::Register* J9::Power::TreeEvaluator::tstartEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (transientConditions) {
        generateDepConditionalBranchInstruction(
            cg, TR::InstOpCode::beq, node, labelTransientFailure, cndReg, transientConditions);
    } else {
    ...
    if (TR::Compiler->target.is64Bit() && cg->fej9()->generateCompressedLockWord()) {
        ...
        generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, node, monReg,
            new (cg->trHeapMemory())
                TR::MemoryReference(objReg, lwOffset, TR::Compiler->om.sizeofReferenceAddress(), cg));
    }
