...
uint8_t* J9::Power::UnresolvedDataSnippet::emitSnippetBody()
{
    ...
    if (getDataSymbol()->getShadowSymbol() != NULL) // instance data
    {
        ...
        glueRef
            = cg()->symRefTab()->findOrCreateRuntimeHelper(TR_PPCinterpreterUnresolvedStringGlue, false, false, false);
        ...
        glueRef = cg()->symRefTab()->findOrCreateRuntimeHelper(
            TR_PPCinterpreterUnresolvedConstantDynamicGlue, false, false, false);
    ...
    *(int32_t*)cursor = 0x48000001 | (distance & 0x03fffffc);
    cg()->addProjectSpecializedRelocation(
        cursor, (uint8_t*)glueRef, NULL, TR_HelperAddress, __FILE__, __LINE__, getNode());
        ...
        *(int32_t*)cursor = getMemoryReference()->getOffset(*(comp)); // offset
        if (getDataSymbol()->isConstObjectRef() || getDataSymbol()->isConstantDynamic()) {
            cg()->addProjectSpecializedRelocation(cursor, *(uint8_t**)(cursor - 4),
                getNode() ? (uint8_t*)getNode()->getInlinedSiteIndex() : (uint8_t*)-1, TR_ConstantPool, __FILE__,
                __LINE__, getNode());
        }
    }
    cursor += 4;
    ...
    *(int32_t*)cursor = 0xdeadbeef; // Pached with lis via runtime code
    ...
    TR_ASSERT(ra_distance <= BRANCH_FORWARD_LIMIT && ra_distance >= BRANCH_BACKWARD_LIMIT,
        "Return address is more than 32MB.\n");
...
void TR_Debug::print(TR::FILE* pOutFile, TR::UnresolvedDataSnippet* snippet)
{
    ...
    distance = *((int32_t*)cursor) & 0x03fffffc;
    distance = (distance << 6) >> 6; // sign extend
    trfprintf(pOutFile, "b \t" POINTER_PRINTF_FORMAT "\t\t; <clinit> case - Branch back to main line code",
