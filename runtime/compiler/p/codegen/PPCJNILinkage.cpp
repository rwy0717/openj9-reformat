...
int32_t TR::PPCJNILinkage::buildArgs(
...
void TR::PPCJNILinkage::buildVirtualDispatch(
...
const TR::PPCLinkageProperties& TR::PPCJNILinkage::getProperties() { return _properties; }
...
TR::Register* TR::PPCJNILinkage::buildDirectDispatch(TR::Node* callNode)
{
    ...
    if (!isGPUHelper) {
        ...
        gpuHelperSymRef = comp()->getSymRefTab()->methodSymRefFromName(
            comp()->getMethodSymbol(), "com/ibm/jit/JITHelpers", "GPUHelper", "()V", TR::MethodSymbol::Static);
    ...
    if (specialCaseJNI) {
        ...
        if (crc32m2 || crc32m3) {
            targetAddress = (uintptrj_t)(
                (TR::Compiler->target.cpu.id() >= TR_PPCp8 && TR::Compiler->target.cpu.getPPCSupportsVSX())
                    ? crc32_vpmsum
                    : crc32_no_vpmsum);

            // Assuming pre/postCondition have the same index, we use preCondition to map
            OMR::RegisterDependencyMap map(
                deps->getPreConditions()->getRegisterDependency(0), deps->getAddCursorForPre());
            ...
            if (crc32m3) {
                addrArg = map.getSourceWithTarget(
                    TR::Compiler->target.is64Bit() ? (TR::RealRegister::gr4) : (TR::RealRegister::gr5));
                posArg = map.getSourceWithTarget(
                    TR::Compiler->target.is64Bit() ? (TR::RealRegister::gr5) : (TR::RealRegister::gr6));
                lenArg = map.getSourceWithTarget(
                    TR::Compiler->target.is64Bit() ? (TR::RealRegister::gr6) : (TR::RealRegister::gr7));
            ...
            deps->getPreConditions()->setDependencyInfo(
                map.getTargetIndex(TR::RealRegister::gr4), addrArg, TR::RealRegister::gr4, UsesDependentRegister);
            deps->getPostConditions()->setDependencyInfo(
                map.getTargetIndex(TR::RealRegister::gr4), addrArg, TR::RealRegister::gr4, UsesDependentRegister);

            deps->getPreConditions()->setDependencyInfo(
                map.getTargetIndex(TR::RealRegister::gr5), lenArg, TR::RealRegister::gr5, UsesDependentRegister);
            deps->getPostConditions()->setDependencyInfo(
                map.getTargetIndex(TR::RealRegister::gr5), lenArg, TR::RealRegister::gr5, UsesDependentRegister);

            deps->getPreConditions()->setDependencyInfo(
                map.getTargetIndex(TR::RealRegister::gr6), posArg, TR::RealRegister::gr6, UsesDependentRegister);
            deps->getPostConditions()->setDependencyInfo(
                map.getTargetIndex(TR::RealRegister::gr6), posArg, TR::RealRegister::gr6, UsesDependentRegister);
    ...
    if (createJNIFrame) {
        ...
        generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_stu, callNode,
            new (trHeapMemory()) TR::MemoryReference(
                stackPtr, -TR::Compiler->om.sizeofReferenceAddress(), TR::Compiler->om.sizeofReferenceAddress(), cg()),
            gr11Reg);

        // skip savedPC slot (unused) and push return address (savedCP)
        cg()->fixedLoadLabelAddressIntoReg(callNode, gr11Reg, returnLabel);
        generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_stu, callNode,
            new (trHeapMemory()) TR::MemoryReference(stackPtr, -2 * TR::Compiler->om.sizeofReferenceAddress(),
        ...
        generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_st, callNode,
            new (trHeapMemory()) TR::MemoryReference(
                metaReg, fej9->thisThreadGetJavaFrameFlagsOffset(), TR::Compiler->om.sizeofReferenceAddress(), cg()),
            gr12Reg);
        ...
        generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_st, callNode,
            new (trHeapMemory()) TR::MemoryReference(
                metaReg, fej9->thisThreadGetJavaSPOffset(), TR::Compiler->om.sizeofReferenceAddress(), cg()),
            stackPtr);
        ...
        generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_st, callNode,
            new (trHeapMemory()) TR::MemoryReference(
                metaReg, fej9->thisThreadGetJavaPCOffset(), TR::Compiler->om.sizeofReferenceAddress(), cg()),
            gr11Reg);

        generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_st, callNode,
            new (trHeapMemory()) TR::MemoryReference(
                metaReg, fej9->thisThreadGetJavaLiteralsOffset(), TR::Compiler->om.sizeofReferenceAddress(), cg()),
            gr12Reg);
    }
    ...
    if (aix_style_linkage
        ...
        generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_load, callNode, gr0Reg,
            new (trHeapMemory()) TR::MemoryReference(gr12Reg, 0, TR::Compiler->om.sizeofReferenceAddress(), cg()));
        ...
        generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_load, callNode, gr2Reg,
            new (trHeapMemory()) TR::MemoryReference(
                gr12Reg, TR::Compiler->om.sizeofReferenceAddress(), TR::Compiler->om.sizeofReferenceAddress(), cg()));
        // load the environment register
        generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_load, callNode, gr11Reg,
            new (trHeapMemory()) TR::MemoryReference(gr12Reg, 2 * TR::Compiler->om.sizeofReferenceAddress(),
                TR::Compiler->om.sizeofReferenceAddress(), cg()));
    } else {
        // put the target address into the count register
    ...
    if (!isGPUHelper) {
        ...
        case TR::Address:
            if (wrapRefs) {
                ...
                generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_load, callNode, returnRegister,
                    new (trHeapMemory())
                        TR::MemoryReference(returnRegister, 0, TR::Compiler->om.sizeofReferenceAddress(), cg()));
        ...
        case TR::Int8:
            if (resolvedMethod->returnTypeIsUnsigned())
                generateTrg1Src1ImmInstruction(
                    cg(), TR::InstOpCode::andi_r, callNode, returnRegister, returnRegister, 0xff);
            else
        ...
        case TR::Int16:
            if (resolvedMethod->returnTypeIsUnsigned())
                generateTrg1Src1ImmInstruction(
                    cg(), TR::InstOpCode::andi_r, callNode, returnRegister, returnRegister, 0xffff);
            else
    ...
    if (createJNIFrame) {
        ...
        if (tearDownJNIFrame) {
            ...
            TR::SymbolReference* collapseSymRef
                = cg()->getSymRefTab()->findOrCreateRuntimeHelper(TR_PPCcollapseJNIReferenceFrame, false, false, false);

            generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_load, callNode, gr30Reg,
                new (trHeapMemory()) TR::MemoryReference(stackPtr, fej9->constJNICallOutFrameFlagsOffset(),
                    TR::Compiler->om.sizeofReferenceAddress(), cg()));
            ...
            generateDepImmSymInstruction(cg(), TR::InstOpCode::bl, callNode,
                (uintptrj_t)collapseSymRef->getMethodAddress(),
        ...
        generateTrg1Src1ImmInstruction(
            cg(), TR::InstOpCode::addi2, callNode, stackPtr, stackPtr, 5 * TR::Compiler->om.sizeofReferenceAddress());
    }

    if (checkExceptions) {
        generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_load, callNode, gr31Reg,
            new (trHeapMemory()) TR::MemoryReference(
                metaReg, fej9->thisThreadGetCurrentExceptionOffset(), TR::Compiler->om.sizeofReferenceAddress(), cg()));
        generateTrg1Src1ImmInstruction(cg(), TR::InstOpCode::Op_cmpi, callNode, cr0Reg, gr31Reg, 0);
...
void TR::PPCJNILinkage::releaseVMAccess(TR::Node* callNode, TR::RegisterDependencyConditions* deps,
    TR::RealRegister* metaReg, TR::Register* tempReg0, TR::Register* tempReg1, TR::Register* tempReg2)
{
    ...
    generateTrg1Src1ImmInstruction(
        cg(), TR::InstOpCode::addi2, callNode, gr28Reg, metaReg, fej9->thisThreadGetPublicFlagsOffset());
    generateInstruction(
        cg(), TR::InstOpCode::lwsync, callNode); // This is necessary for the fast path but redundant for the slow path
    ...
    generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_larx, callNode, tempReg2,
        new (trHeapMemory()) TR::MemoryReference(NULL, gr28Reg, TR::Compiler->om.sizeofReferenceAddress(), cg()));
    ...
    if (TR::Compiler->target.cpu.id() >= TR_PPCgp)
        ...
        generateConditionalBranchInstruction(
            cg(), TR::InstOpCode::bne, PPCOpProp_BranchUnlikely, callNode, loopHead, cr0Reg);
    else
...
// tempReg0 to tempReg2 are temporary registers
void TR::PPCJNILinkage::acquireVMAccess(TR::Node* callNode, TR::RegisterDependencyConditions* deps,
    TR::RealRegister* metaReg, TR::Register* tempReg0, TR::Register* tempReg1, TR::Register* tempReg2)
{
    // Acquire VM Access
    TR::Instruction* gcPoint;
    ...
    generateTrg1Src1ImmInstruction(
        cg(), TR::InstOpCode::addi2, callNode, tempReg0, metaReg, fej9->thisThreadGetPublicFlagsOffset());
    ...
    generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_larx, PPCOpProp_LoadReserveExclusiveAccess, callNode, tempReg2,
        new (trHeapMemory()) TR::MemoryReference(NULL, tempReg0, TR::Compiler->om.sizeofReferenceAddress(), cg()));
    ...
    if (TR::Compiler->target.cpu.id() >= TR_PPCgp)
        // use PPC AS branch hint
        generateConditionalBranchInstruction(
            cg(), TR::InstOpCode::bne, PPCOpProp_BranchUnlikely, callNode, loopHead2, cr0Reg);
    else
...
#ifdef J9VM_INTERP_ATOMIC_FREE_JNI
void TR::PPCJNILinkage::releaseVMAccessAtomicFree(TR::Node* callNode, TR::RegisterDependencyConditions* deps,
    TR::RealRegister* metaReg, TR::Register* cr0Reg, TR::Register* tempReg1, TR::Register* tempReg2)
{
    ...
    generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_st, callNode,
        new (trHeapMemory()) TR::MemoryReference(
            metaReg, (int32_t)offsetof(struct J9VMThread, inNative), TR::Compiler->om.sizeofReferenceAddress(), cg()),
        tempReg1);
#if !defined(J9VM_INTERP_ATOMIC_FREE_JNI_USES_FLUSH)
    ...
    generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_load, callNode, tempReg1,
        new (trHeapMemory()) TR::MemoryReference(
            metaReg, fej9->thisThreadGetPublicFlagsOffset(), TR::Compiler->om.sizeofReferenceAddress(), cg()));
    ...
    generateTrg1Src1ImmInstruction(
        cg(), TR::InstOpCode::Op_cmpli, callNode, cr0Reg, tempReg1, J9_PUBLIC_FLAGS_VM_ACCESS);

    TR::SymbolReference* jitReleaseVMAccessSymRef
        = comp()->getSymRefTab()->findOrCreateReleaseVMAccessSymbolRef(comp()->getJittedMethodSymbol());
    TR::LabelSymbol* releaseVMAcessSnippetLabel
    ...
    if (!releaseVMAcessSnippetLabel) {
        ...
        cg()->addSnippet(new (trHeapMemory())
                TR::PPCHelperCallSnippet(cg(), callNode, releaseVMAcessSnippetLabel, jitReleaseVMAccessSymRef));
    }

    if (TR::Compiler->target.cpu.id() >= TR_PPCgp)
        generateConditionalBranchInstruction(
            cg(), TR::InstOpCode::bnel, PPCOpProp_BranchUnlikely, callNode, releaseVMAcessSnippetLabel, cr0Reg);
    else
        generateConditionalBranchInstruction(cg(), TR::InstOpCode::bnel, callNode, releaseVMAcessSnippetLabel, cr0Reg);
}

void TR::PPCJNILinkage::acquireVMAccessAtomicFree(TR::Node* callNode, TR::RegisterDependencyConditions* deps,
    TR::RealRegister* metaReg, TR::Register* cr0Reg, TR::Register* tempReg1, TR::Register* tempReg2)
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)fe();
    ...
    TR::SymbolReference* jitAcquireVMAccessSymRef
        = comp()->getSymRefTab()->findOrCreateAcquireVMAccessSymbolRef(comp()->getJittedMethodSymbol());
    TR::LabelSymbol* acquireVMAcessSnippetLabel
    ...
    if (!acquireVMAcessSnippetLabel) {
        ...
        cg()->addSnippet(new (trHeapMemory())
                TR::PPCHelperCallSnippet(cg(), callNode, acquireVMAcessSnippetLabel, jitAcquireVMAccessSymRef));
    }

    if (TR::Compiler->target.cpu.id() >= TR_PPCgp)
        generateConditionalBranchInstruction(
            cg(), TR::InstOpCode::bnel, PPCOpProp_BranchUnlikely, callNode, acquireVMAcessSnippetLabel, cr0Reg);
    else
        generateConditionalBranchInstruction(cg(), TR::InstOpCode::bnel, callNode, acquireVMAcessSnippetLabel, cr0Reg);
}
#endif /* J9VM_INTERP_ATOMIC_FREE_JNI */

int32_t TR::PPCJNILinkage::buildJNIArgs(TR::Node* callNode, TR::RegisterDependencyConditions* dependencies,
...

{
    ...
    for (i = firstArgumentChild; i < callNode->getNumChildren(); i++) {
        ...
        case TR::Address: // have to do something for GC maps here
            ...
            if (numIntegerArgs < properties.getNumIntArgRegs()) {
                ...
                if (isFastJNI && (TR::Compiler->target.isLinux() && TR::Compiler->target.is64Bit())
            ...
            } else // numIntegerArgs >= properties.getNumIntArgRegs()
            {
                mref = getOutgoingArgumentMemRef(argSize, argRegister, TR::InstOpCode::Op_st, pushToMemory[argIndex++],
                    TR::Compiler->om.sizeofReferenceAddress(), properties);
            ...
            for (int r = 0; r < ((childType == TR::Float) ? 1 : 2); r++) {
                ...
                // numFloatArgs >= properties.getNumFloatArgRegs()
                {
                    mref = getOutgoingArgumentMemRef(
                        argSize, argReg, TR::InstOpCode::stfs, pushToMemory[argIndex++], 4, properties);
                    argSize += 4;
                }
                ...
                if (aix_style_linkage) {
                    ...
                    } else // numIntegerArgs >= properties.getNumIntArgRegs()
                    {
                        if (TR::Compiler->target.is64Bit() && TR::Compiler->target.isLinux()) {
                            mref = getOutgoingArgumentMemRef(
                                argSize + 4, argReg, TR::InstOpCode::stfs, pushToMemory[argIndex++], 4, properties);
                        } else {
                            mref = getOutgoingArgumentMemRef(
                                argSize, argReg, TR::InstOpCode::stfs, pushToMemory[argIndex++], 4, properties);
                        }
        ...
        case TR::Double:
            ...
            for (int r = 0; r < ((childType == TR::Double) ? 1 : 2); r++) {
                ...
                // numFloatArgs >= properties.getNumFloatArgRegs()
                {
                    argSize = (argSize + 4) & (~7);
                    mref = getOutgoingArgumentMemRef(
                        argSize, argReg, TR::InstOpCode::stfd, pushToMemory[argIndex++], 8, properties);
                    argSize += 8;
                }
    ...
    if (liveVMX || liveVSXScalar || liveVSXVector) {
        for (i = (TR::RealRegister::RegNum)((uint32_t)TR::RealRegister::LastFPR + 1); i <= TR::RealRegister::LastVSR;
            ...
            if (!properties.getPreserved((TR::RealRegister::RegNum)i) || (!isFastJNI && !isHelper)) {
    ...
    if (child->getOpCodeValue() == TR::loadaddr) {
        ...
        if (sym) {
            if (sym->isAddressOfClassObject()) {
                ...
                TR::MemoryReference* tmpMemRef = new (trHeapMemory())
                    TR::MemoryReference(addrReg, (int32_t)0, TR::Compiler->om.sizeofReferenceAddress(), cg());
