...
const TR::PPCLinkageProperties& TR::PPCPrivateLinkage::getProperties() { return _properties; }
...
uint32_t TR::PPCPrivateLinkage::getRightToLeft() { return getProperties().getRightToLeft(); }
...
void TR::PPCPrivateLinkage::mapSingleAutomatic(TR::AutomaticSymbol* p, uint32_t& stackIndex)
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(fe());
    int32_t roundup
        = (comp()->useCompressedPointers() && p->isLocalObject() ? fej9->getObjectAlignmentInBytes()
    ...
    TR::RealRegister* gr12, TR::RealRegister* cr0, TR::LabelSymbol* loopLabel, TR::Instruction* cursor)
{
    ...
    bool useVectorStores = (TR::Compiler->target.cpu.id() >= TR_PPCp8 && TR::Compiler->target.cpu.getPPCSupportsVSX()
        && !disableVSXMemInit);

    if (useVectorStores) {
        int32_t wordsUnrolledPerIteration = (128 * 4)
            / (TR::Compiler->om.sizeofReferenceAddress()
                  * 8); // (number of bits cleared by one iteration using 4 stxvw4x) / (number of bits per word i.e.
                        // bits in a java pointer)
        if (wordsToUnroll >= wordsUnrolledPerIteration) {
            ...
            cursor = generateTrg1Src2Instruction(
                cg, TR::InstOpCode::xxlxor, node, vectorNullReg, vectorNullReg, vectorNullReg, cursor);
            ...
            cursor = generateTrg1Src1ImmInstruction(
                cg, TR::InstOpCode::addi, node, gr12, baseInitReg, initSlotOffset, cursor);
            baseInitReg = gr12;
            initSlotOffset = 0;
            ...
            cursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stxvw4x, node,
                new (trHeapMemory) TR::MemoryReference(nullReg, baseInitReg, 16, cg), vectorNullReg, cursor);
            cursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stxvw4x, node,
                new (trHeapMemory) TR::MemoryReference(gr11, baseInitReg, 16, cg), vectorNullReg, cursor);
            cursor
            ...
            if (loopIterations > 1) {
                cursor = generateTrg1Src1ImmInstruction(
                    cg, TR::InstOpCode::addi, node, baseInitReg, baseInitReg, 32, cursor);
        ...
        while (wordsToUnroll > 0) {
            cursor = generateMemSrc1Instruction(cg, instruction, node,
                new (trHeapMemory) TR::MemoryReference(baseInitReg, initSlotOffset, storeSize, cg), nullReg, cursor);
        ...
        if ((use8Bytes && TR::Compiler->om.sizeofReferenceAddress() == 4) && (num % 2 == 1)) {
            cursor = generateMemSrc1Instruction(cg, TR::InstOpCode::stw, node,
                new (trHeapMemory) TR::MemoryReference(baseInitReg, initSlotOffset, 4, cg), nullReg, cursor);
        }
    } else {
        if (use8Bytes && TR::Compiler->om.sizeofReferenceAddress() == 4)
            wordsToUnroll /= 2;
        ...
        if (wordsToUnroll >= 4) {
            ...
            cursor = generateMemSrc1Instruction(cg, use8Bytes ? TR::InstOpCode::std : TR::InstOpCode::stw, node,
                new (trHeapMemory)
                    TR::MemoryReference(baseInitReg, initSlotOffset, TR::Compiler->om.sizeofReferenceAddress(), cg),
                nullReg, cursor);
            cursor = generateMemSrc1Instruction(cg, use8Bytes ? TR::InstOpCode::std : TR::InstOpCode::stw, node,
                new (trHeapMemory) TR::MemoryReference(
                    baseInitReg, initSlotOffset + (use8Bytes ? 8 : 4), TR::Compiler->om.sizeofReferenceAddress(), cg),
                nullReg, cursor);
            cursor = generateMemSrc1Instruction(cg, use8Bytes ? TR::InstOpCode::std : TR::InstOpCode::stw, node,
                new (trHeapMemory) TR::MemoryReference(
                    baseInitReg, initSlotOffset + (use8Bytes ? 16 : 8), TR::Compiler->om.sizeofReferenceAddress(), cg),
                nullReg, cursor);
            cursor = generateMemSrc1Instruction(cg, use8Bytes ? TR::InstOpCode::std : TR::InstOpCode::stw, node,
                new (trHeapMemory) TR::MemoryReference(
                    baseInitReg, initSlotOffset + (use8Bytes ? 24 : 12), TR::Compiler->om.sizeofReferenceAddress(), cg),
                nullReg, cursor);
            if (wordsToUnroll >= 8) {
                cursor = generateTrg1Src1ImmInstruction(
                    cg, TR::InstOpCode::addi, node, gr12, gr12, (use8Bytes ? 32 : 16), cursor);
                cursor = generateConditionalBranchInstruction(cg, TR::InstOpCode::bdnz, node, loopLabel, cr0, cursor);
            } else {
...
void TR::PPCPrivateLinkage::createPrologue(TR::Instruction* cursor)
{
    ...
    if (!comp()->isDLT())
        cursor = generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_load, firstNode, gr11,
            new (trHeapMemory()) TR::MemoryReference(
                metaBase, cg()->getStackLimitOffset(), TR::Compiler->om.sizeofReferenceAddress(), cg()),
            cursor);
    ...
    if (cg()->getFrameSizeInBytes() || saveLR) {
        if ((cg()->getFrameSizeInBytes() + TR::Compiler->om.sizeofReferenceAddress()) > (-LOWER_IMMED)) {
            ...
            if (!comp()->isDLT())
                cursor = generateTrg1Src2Instruction(
                    cg(), TR::InstOpCode::Op_cmpl, firstNode, cr0, stackPtr, gr11, cursor);

            if (saveLR) {
                // javaSP[totalFrameSize] <- linkRegister
                cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_stx, firstNode,
                    new (trHeapMemory())
                        TR::MemoryReference(stackPtr, gr12, TR::Compiler->om.sizeofReferenceAddress(), cg()),
                    gr0, cursor);
            }
        } else {
            // Small Frame Support
            ...
            if (saveLR) {
                ...
                cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_st, firstNode,
                    new (trHeapMemory()) TR::MemoryReference(
                        stackPtr, cg()->getFrameSizeInBytes(), TR::Compiler->om.sizeofReferenceAddress(), cg()),
                    gr0, cursor);
            }
        }
    } else {
    ...
    if (!comp()->isDLT()) {
        ...
        if (TR::Compiler->target.cpu.id() >= TR_PPCgp)
            ...
            cursor = generateConditionalBranchInstruction(
                cg(), TR::InstOpCode::ble, PPCOpProp_BranchUnlikely, firstNode, snippetLabel, cr0, cursor);
        else
            cursor
    ...
    if (intSavedFirst <= TR::RealRegister::LastGPR) {
        if (TR::Compiler->target.is64Bit()
            || (!comp()->getOption(TR_OptimizeForSpace) && TR::RealRegister::LastGPR - intSavedFirst <= 3)) {
            for (regIndex = intSavedFirst; regIndex <= TR::RealRegister::LastGPR;
                 regIndex = (TR::RealRegister::RegNum)((uint32_t)regIndex + 1)) {
                cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_st, firstNode,
                    new (trHeapMemory())
                        TR::MemoryReference(stackPtr, argSize, TR::Compiler->om.sizeofReferenceAddress(), cg()),
            ...
            cursor = generateMemSrc1Instruction(cg(),
                ...
                new (trHeapMemory())
                    TR::MemoryReference(stackPtr, argSize, 4 * (TR::RealRegister::LastGPR - intSavedFirst + 1), cg()),
        ...
        if ((numLocalsToBeInitialized > 0) || atlas->getInternalPointerMap()) {
            ...
            if (numLocalsToBeInitialized > 5) {
                ...
                if (residualSize >= -LOWER_IMMED) {
                    cursor = generateTrg1Src2Instruction(
                        cg(), TR::InstOpCode::add, firstNode, gr12, gr12, stackPtr, cursor);
                    ...
                    if (initBottomOffset < LOWER_IMMED)
                        cursor = generateTrg1Src1ImmInstruction(
                            cg(), TR::InstOpCode::addis, firstNode, gr12, gr12, HI_VALUE(initBottomOffset), cursor);
                    if (initBottomOffset != 0)
                        cursor = generateTrg1Src1ImmInstruction(
                            cg(), TR::InstOpCode::addi2, firstNode, gr12, gr12, LO_VALUE(initBottomOffset), cursor);
                } else {
                    adjustedFrameOnce = true;
                    if (size == residualSize) // only happens when LR is not saved
                        residualSize -= TR::Compiler->om.sizeofReferenceAddress();
                ...
                if (!disableUnrollPrologueInitLoop) {
                    ...
                    if (residualSize >= -LOWER_IMMED)
                        cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_stx, firstNode,
                            new (trHeapMemory())
                                TR::MemoryReference(gr12, gr11, TR::Compiler->om.sizeofReferenceAddress(), cg()),
                            nullValueRegister, cursor);
                    else
                        cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_stx, firstNode,
                            new (trHeapMemory()) TR::MemoryReference(
                                stackPtr, gr11, initSlotOffset + TR::Compiler->om.sizeofReferenceAddress(), cg()),
                            nullValueRegister, cursor);
                    cursor = generateTrg1Src1ImmInstruction(cg(), TR::InstOpCode::addic_r, firstNode, gr11, gr11, cr0,
                        TR::Compiler->om.sizeofReferenceAddress(), cursor);
                    cursor = generateConditionalBranchInstruction(
                        cg(), TR::InstOpCode::bne, firstNode, loopLabel, cr0, cursor);
                }
            } else {
                adjustedFrameOnce = true;
                if (size == residualSize) // only happens when LR is not saved
                ...
                if (!disableUnrollPrologueInitLoop && use8BytesOn32Bit) {
                    ...
                    if (numLocalsToBeInitialized >= 2) {
                        cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::std, firstNode,
                            new (trHeapMemory()) TR::MemoryReference(
                                stackPtr, offset + residualSize, TR::Compiler->om.sizeofReferenceAddress(), cg()),
                            nullValueRegister, cursor);
                        count++;
                        if (numLocalsToBeInitialized >= 4) {
                            cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::std, firstNode,
                                new (trHeapMemory()) TR::MemoryReference(stackPtr, offset + residualSize + 8,
                    ...
                    if (numLocalsToBeInitialized % 2 == 1)
                        cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::Op_st, firstNode,
                            new (trHeapMemory()) TR::MemoryReference(stackPtr, offset + residualSize + (count * 8),
                                TR::Compiler->om.sizeofReferenceAddress(), cg()),
                            nullValueRegister, cursor);
                } else {
...
TR::MemoryReference* TR::PPCPrivateLinkage::getOutgoingArgumentMemRef(int32_t argSize, TR::Register* argReg,
    TR::InstOpCode::Mnemonic opCode, TR::PPCMemoryArgument& memArg, uint32_t length)
{
    TR::MemoryReference* result
        = new (trHeapMemory()) TR::MemoryReference(cg()->getStackPointerRegister(), argSize, length, cg());
...
void TR::PPCPrivateLinkage::createEpilogue(TR::Instruction* cursor)
{
    ...
    if (restoreLR && frameSize <= UPPER_IMMED) {
        cursor = generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_load, currentNode, gr0,
            new (trHeapMemory())
                TR::MemoryReference(stackPtr, frameSize, TR::Compiler->om.sizeofReferenceAddress(), cg()),
            cursor);
        ...
        if (TR::Compiler->target.is64Bit()
            ...
            for (regIndex = savedFirst; regIndex <= TR::RealRegister::LastGPR;
                ...
                cursor = generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_load, currentNode,
                    ...
                    new (trHeapMemory())
                        TR::MemoryReference(stackPtr, saveSize, TR::Compiler->om.sizeofReferenceAddress(), cg()),
                    cursor);
            ...
            cursor = generateTrg1MemInstruction(cg(),
                ...
                new (trHeapMemory())
                    TR::MemoryReference(stackPtr, saveSize, 4 * (TR::RealRegister::LastGPR - savedFirst + 1), cg()),
                cursor);
    ...
    if (frameSize || saveLR) {
        ...
        if (saveSize > UPPER_IMMED) {
            ...
            cursor = generateTrg1Src1ImmInstruction(
                cg(), TR::InstOpCode::addi2, currentNode, stackPtr, stackPtr, saveSize, cursor);
        }

        if (restoreLR && frameSize > UPPER_IMMED) {
            cursor = generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_load, currentNode, gr0,
                new (trHeapMemory()) TR::MemoryReference(stackPtr, -TR::Compiler->om.sizeofReferenceAddress(),
                    TR::Compiler->om.sizeofReferenceAddress(), cg()),
                cursor);
            cursor = generateSrc1Instruction(cg(), TR::InstOpCode::mtlr, currentNode, gr0, 0, cursor);
        }
...
int32_t TR::PPCPrivateLinkage::buildArgs(TR::Node* callNode, TR::RegisterDependencyConditions* dependencies)
{
    return buildPrivateLinkageArgs(callNode, dependencies, TR_Private);
}

int32_t TR::PPCPrivateLinkage::buildPrivateLinkageArgs(
    TR::Node* callNode, TR::RegisterDependencyConditions* dependencies, TR_LinkageConventions linkage)
{
    ...
    if (linkage == TR_CHelper) {
        ...
        generateTrg1Src1Instruction(
            cg(), TR::InstOpCode::mr, callNode, vmThreadArgRegister, cg()->getMethodMetaDataRegister());
    ...
    for (int32_t i = from; (rightToLeft && i >= to) || (!rightToLeft && i <= to); i += step) {
                ...
                if (numIntegerArgs < numIntArgRegs) {
                    ...
                    if (numIntegerArgs == firstExplicitArg
                        ...
                        dependencies->addPreCondition(
                            argRegister, properties.getIntegerArgumentRegister(numIntegerArgs));
                        dependencies->addPostCondition(resultReg, TR::RealRegister::gr3);
                        if (firstExplicitArg == 1)
                            dependencies->addPostCondition(
                                argRegister, properties.getIntegerArgumentRegister(numIntegerArgs));
                    } else if (TR::Compiler->target.is32Bit() && numIntegerArgs == (firstExplicitArg + 1)
                        && resType.isInt64()) {
                ...
                } else // numIntegerArgs >= numIntArgRegs
                {
                    if (child->getDataType() == TR::Address)
                        ...
                        mref = getOutgoingArgumentMemRef(
                            totalSize - argSize, argRegister, TR::InstOpCode::stw, pushToMemory[argIndex++], 4);
                }
                numIntegerArgs++;
            }
            break;
        case TR::Int64:
            ...
            if (isSpecialArg) {
                    ...
                    if (TR::Compiler->target.is32Bit()) {
                        if (numIntegerArgs + 1 < numIntArgRegs) {
                            if (!cg()->canClobberNodesRegister(child, 0)) {
                                ...
                                tempRegister = cg()->allocateRegister();
                                generateTrg1Src1Instruction(
                                    cg(), TR::InstOpCode::mr, callNode, tempRegister, over_lowReg);
                            ...
                            if (numIntegerArgs == firstExplicitArg && resType.isInt64()) {
                                ...
                                if (firstExplicitArg == 1)
                                    dependencies->addPostCondition(
                                        argRegister, properties.getIntegerArgumentRegister(numIntegerArgs + 1));
                            } else
                ...
                } else // numIntegerArgs >= numIntArgRegs
                {
                    if (TR::Compiler->target.is64Bit())
                        mref = getOutgoingArgumentMemRef(
                            totalSize - argSize, argRegister, TR::InstOpCode::std, pushToMemory[argIndex++], 8);
                    else {
        ...
        case TR::Float:
            ...
            {
                mref = getOutgoingArgumentMemRef(
                    totalSize - argSize, argRegister, TR::InstOpCode::stfs, pushToMemory[argIndex++], 4);
            }
            numFloatArgs++;
            break;
        case TR::Double:
            ...
            {
                mref = getOutgoingArgumentMemRef(
                    totalSize - argSize, argRegister, TR::InstOpCode::stfd, pushToMemory[argIndex++], 8);
            }
            numFloatArgs++;
            break;
        }
    ...
    if (liveVSX || liveFPR) {
        ...
        addDependency(
            dependencies, NULL, (TR::RealRegister::RegNum)getProperties().getFloatReturnRegister(), TR_FPR, cg());
    }
    ...
    if (linkage == TR_CHelper) {
        if (TR::Compiler->target.isLinux() && TR::Compiler->target.is64Bit()
            ...
            generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_load, callNode,
                ...
                new (trHeapMemory()) TR::MemoryReference(
                    cg()->getTOCBaseRegister(), helperOffset, TR::Compiler->om.sizeofReferenceAddress(), cg()));
            ...
            generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_load, callNode,
                ...
                new (trHeapMemory()) TR::MemoryReference(cg()->getMethodMetaDataRegister(),
                    offsetof(J9VMThread, jitTOC), TR::Compiler->om.sizeofReferenceAddress(), cg()));
        }
    }
...
static bool getProfiledCallSiteInfo(
    TR::CodeGenerator* cg, TR::Node* callNode, uint32_t maxStaticPICs, TR_ScratchList<TR::PPCPICItem>& values)
{
    TR::Compilation* comp = cg->comp();
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(cg->fe());
...
static TR::Instruction* buildStaticPICCall(TR::CodeGenerator* cg, TR::Node* callNode, uintptrj_t profiledClass,
    ...
    TR::LabelSymbol* missLabel, uint32_t regMapForGC)
{
    ...
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(cg->fe());
    loadAddressConstant(cg, callNode, profiledClass, tempReg, NULL,
...
static void buildVirtualCall(
    TR::CodeGenerator* cg, TR::Node* callNode, TR::Register* vftReg, TR::Register* gr12, uint32_t regMapForGC)
{
    ...
    if (offset < LOWER_IMMED || offset > UPPER_IMMED) {
        ...
        generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, callNode, gr12,
            new (cg->trHeapMemory()) TR::MemoryReference(
                gr12, ((offset & 0x0000ffff) << 16) >> 16, TR::Compiler->om.sizeofReferenceAddress(), cg));
    } else {
        generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, callNode, gr12,
            new (cg->trHeapMemory())
                TR::MemoryReference(vftReg, offset, TR::Compiler->om.sizeofReferenceAddress(), cg));
    }
    ...
    uint32_t regMapForGC)
{
    ...
    if (TR::Compiler->target.is64Bit()) {
        ...
        if (beginIndex != PTOC_FULL_INDEX) {
            ...
            if (beginIndex < LOWER_IMMED || beginIndex > UPPER_IMMED) {
                generateTrg1Src1ImmInstruction(
                    cg, TR::InstOpCode::addis, callNode, gr12, cg->getTOCBaseRegister(), HI_VALUE(beginIndex));
                generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, callNode, gr12,
                    new (cg->trHeapMemory())
                        TR::MemoryReference(gr12, LO_VALUE(beginIndex), TR::Compiler->om.sizeofReferenceAddress(), cg));
            } else {
                generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, callNode, gr12,
                    new (cg->trHeapMemory()) TR::MemoryReference(
                        cg->getTOCBaseRegister(), beginIndex, TR::Compiler->om.sizeofReferenceAddress(), cg));
            }
            generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, callNode, gr11,
                new (cg->trHeapMemory()) TR::MemoryReference(gr12, 0, TR::Compiler->om.sizeofReferenceAddress(), cg));
        } else {
            TR::Instruction* q[4];
        ...
        ifcSnippet->setLowerInstruction(generateTrg1MemInstruction(
            cg, TR::InstOpCode::lwzu, callNode, gr11, new (cg->trHeapMemory()) TR::MemoryReference(gr12, 0, 4, cg)));
    }
    ...
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_loadu, callNode, gr11,
        new (cg->trHeapMemory()) TR::MemoryReference(
            gr12, 2 * TR::Compiler->om.sizeofReferenceAddress(), TR::Compiler->om.sizeofReferenceAddress(), cg));
    generateTrg1Src2Instruction(cg, TR::InstOpCode::Op_cmpl, callNode, cr0, vftReg, gr11);
    ...
    if (!comp->getOption(TR_DisableLastITableCache) && interfaceClassOfMethod &&
        ...
        generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, callNode, gr11,
            new (cg->trHeapMemory()) TR::MemoryReference(
                vftReg, fej9->getOffsetOfLastITableFromClassField(), TR::Compiler->om.sizeofReferenceAddress(), cg));
        // Load the interface class from the snippet rather than materializing it, again because we want to do this in a
        // single instruction
        generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, callNode, gr12,
            new (cg->trHeapMemory()) TR::MemoryReference(
                gr12, -4 * TR::Compiler->om.sizeofReferenceAddress(), TR::Compiler->om.sizeofReferenceAddress(), cg));
        generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, callNode, gr0,
            new (cg->trHeapMemory()) TR::MemoryReference(
                gr11, fej9->getOffsetOfInterfaceClassFromITableField(), TR::Compiler->om.sizeofReferenceAddress(), cg));
        ...
        generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, callNode, gr12,
            new (cg->trHeapMemory()) TR::MemoryReference(
                gr11, fej9->convertITableIndexToOffset(itableIndex), TR::Compiler->om.sizeofReferenceAddress(), cg));
    ...
    generateTrg1MemInstruction(cg, TR::InstOpCode::Op_load, callNode, gr11,
        new (cg->trHeapMemory()) TR::MemoryReference(
            gr12, TR::Compiler->om.sizeofReferenceAddress(), TR::Compiler->om.sizeofReferenceAddress(), cg));
#ifdef INLINE_LASTITABLE_CHECK
...
void TR::PPCPrivateLinkage::buildVirtualDispatch(
    TR::Node* callNode, TR::RegisterDependencyConditions* dependencies, uint32_t sizeOfArguments)
{
            ...
            if (fej9->needsInvokeExactJ2IThunk(callNode, comp())) {
                comp()->getPersistentInfo()->getInvokeExactJ2IThunkTable()->addThunk(
                    TR::PPCCallSnippet::generateInvokeExactJ2IThunk(
                        callNode, sizeOfArguments, cg(), methodSymbol->getMethod()->signatureChars()),
                    fej9);
            }
            break;
        }
    ...
    if (!thunk)
        thunk = fej9->setJ2IThunk(
            methodSymbol->getMethod(), TR::PPCCallSnippet::generateVIThunk(callNode, sizeOfArguments, cg()), comp());
    ...
    if (methodSymbol->isVirtual()) {
        ...
        if (methodSymRef->isUnresolved() || cg()->comp()->compileRelocatableCode()) {
            ...
            generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_load, callNode, gr12,
                new (trHeapMemory()) TR::MemoryReference(gr12, 0, TR::Compiler->om.sizeofReferenceAddress(), cg()));
            ...
            if (comp()->performVirtualGuardNOPing() && comp()->isVirtualGuardNOPingRequired()
                ...
                if (!resolvedMethod->virtualMethodIsOverridden() && !resolvedMethod->isAbstract()) {
                    ...
                    if (thisClass && TR::Compiler->cls.isAbstractClass(comp(), thisClass)
                        ...
                        TR_ResolvedMethod* calleeMethod = chTable->findSingleAbstractImplementer(
                ...
                if (virtualGuard) {
                    ...
                    uintptrj_t methodAddress
                        = resolvedMethod->isSameMethod(comp()->getCurrentMethod()) && !comp()->isDLT()
                        ? 0
                        : (uintptrj_t)resolvedMethod->startAddressForJittedMethod();
        ...
        if (getProfiledCallSiteInfo(cg(), callNode, maxStaticPICs, values)) {
            ...
            if (pic->_frequency > MAX_PROFILED_CALL_FREQUENCY) {
                ...
                if (methodSymbol->isInterface()) {
                    ...
                    TR::PPCInterfaceCallSnippet* ifcSnippet = new (trHeapMemory()) TR::PPCInterfaceCallSnippet(
                        cg(), callNode, ifcSnippetLabel, sizeOfArguments, doneOOLLabel, (uint8_t*)thunk);
                ...
                while (pic) {
                    ...
                    buildStaticPICCall(cg(), callNode, (uintptrj_t)pic->_clazz, pic->_method, vftReg, gr11, cr0,
...
void inlineCharacterIsMethod(
    TR::Node* node, TR::MethodSymbol* methodSymbol, TR::CodeGenerator* cg, TR::LabelSymbol*& doneLabel)
{
    TR::Compilation* comp = cg->comp();
...
void TR::PPCPrivateLinkage::buildDirectCall(TR::Node* callNode, TR::SymbolReference* callSymRef,
    TR::RegisterDependencyConditions* dependencies, const TR::PPCLinkageProperties& pp, int32_t argSize)
{
    ...
    bool myself = (vmm != NULL && vmm->isSameMethod(comp()->getCurrentMethod()) && !comp()->isDLT()) ? true : false;
    ...
    if ((callSymbol->isJITInternalNative()
            || (!callSymRef->isUnresolved() && !callSymbol->isInterpreted()
                   && ((comp()->compileRelocatableCode() && callSymbol->isHelper())
                          || !comp()->compileRelocatableCode())))) {
        gcPoint = generateDepImmSymInstruction(cg(), TR::InstOpCode::bl, callNode,
            myself ? 0 : (uintptrj_t)callSymbol->getMethodAddress(), dependencies,
...
int32_t TR::PPCHelperLinkage::buildArgs(TR::Node* callNode, TR::RegisterDependencyConditions* dependencies)
{
    ...
    const TR::PPCLinkageProperties& properties)
{
    ...
    TR::MemoryReference* result
        = new (trHeapMemory()) TR::MemoryReference(machine->getRealRegister(properties.getNormalStackPointerRegister()),
