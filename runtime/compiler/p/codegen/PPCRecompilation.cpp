...
TR::Instruction* TR_PPCRecompilation::generatePrePrologue()
{
    ...
    TR::SymbolReference* recompileMethodSymRef
        = cg()->symRefTab()->findOrCreateRuntimeHelper(TR_PPCsamplingRecompileMethod, false, false, false);
    ...
    if (useSampling() && couldBeCompiledAgain()) {
        ...
        cursor = generateDepImmSymInstruction(cg(), TR::InstOpCode::bl, firstNode,
            (uintptrj_t)recompileMethodSymRef->getMethodAddress(),
        ...
        if (TR::Compiler->target.is64Bit()) {
            ...
            cursor = generateImmInstruction(
                cg(), TR::InstOpCode::dd, firstNode, (int32_t)(intptrj_t)info, TR_BodyInfoAddress, cursor);
        }
    ...
    if (couldBeCompiledAgain()) {
        ...
        if (TR::Compiler->target.is64Bit()) {
            ...
            cursor = generateTrg1ImmInstruction(
                cg(), TR::InstOpCode::lis, firstNode, gr11, ((adjustedAddr >> 48) & 0x0000FFFF), cursor);
            // ori gr11, gr11, next 16-bit
            cursor = generateTrg1Src1ImmInstruction(
                cg(), TR::InstOpCode::ori, firstNode, gr11, gr11, ((adjustedAddr >> 32) & 0x0000FFFF), cursor);
            // rldicr gr11, gr11, 32, 31
            cursor = generateTrg1Src1Imm2Instruction(
                cg(), TR::InstOpCode::rldicr, firstNode, gr11, gr11, 32, CONSTANT64(0xFFFFFFFF00000000), cursor);
            // oris  gr11, gr11, next 16-bits
            cursor = generateTrg1Src1ImmInstruction(
                cg(), TR::InstOpCode::oris, firstNode, gr11, gr11, ((adjustedAddr >> 16) & 0x0000FFFF), cursor);

            // lwzu  gr0, last 16-bits(gr11)
            cursor = generateTrg1MemInstruction(cg(),
                (!isProfilingCompilation()) ? TR::InstOpCode::lwzu : TR::InstOpCode::lwz, firstNode, gr0,
                new (cg()->trHeapMemory()) TR::MemoryReference(gr11, ((addr & 0x0000FFFF) << 48) >> 48, 4, cg()),
                cursor);
        } else {
            ...
            cursor = generateTrg1MemInstruction(cg(),
                (!isProfilingCompilation()) ? TR::InstOpCode::lwzu : TR::InstOpCode::lwz, firstNode, gr0,
                new (cg()->trHeapMemory()) TR::MemoryReference(gr11, ((addr & 0x0000FFFF) << 16) >> 16, 4, cg()),
                cursor);
        }
