...
uint8_t* TR::PPCStackCheckFailureSnippet::emitSnippetBody()
{
    ...
    *(int32_t*)buffer = 0x48000001 | (distance & 0x03ffffff);
    cg()->addExternalRelocation(new (cg()->trHeapMemory())
                                    TR::ExternalRelocation(buffer, (uint8_t*)sofRef, TR_HelperAddress, cg()),
    ...
    *(int32_t*)buffer
        = 0x48000000 | (((intptrj_t)getReStartLabel()->getCodeLocation() - (intptrj_t)buffer) & 0x03fffffc);
...
void TR_Debug::print(TR::FILE* pOutFile, TR::PPCStackCheckFailureSnippet* snippet)
{
