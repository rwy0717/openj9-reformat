...
TR_PPCLMGuardedStorage* TR_PPCLMGuardedStorage::allocate(J9JITConfig* jitConfig, bool ebbSetupDone)
{
    if (!ebbSetupDone) {
...
bool TR_PPCLMGuardedStorage::initializeThread(J9VMThread* vmThread)
{
#if 0
   ...
   if (IS_THREAD_RI_INITIALIZED(vmThread))
    ...
    if (!IS_THREAD_RI_INITIALIZED(vmThread)) {
        TR_PPCHWProfilerEBBContext* context
            = (TR_PPCHWProfilerEBBContext*)jitPersistentAlloc(sizeof(TR_PPCHWProfilerEBBContext));
        if (!context) {
...
bool TR_PPCLMGuardedStorage::deinitializeThread(J9VMThread* vmThread)
{
#if 0
   PORT_ACCESS_FROM_VMC(vmThread);
