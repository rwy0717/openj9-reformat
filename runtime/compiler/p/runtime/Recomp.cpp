...
void J9::Recompilation::fixUpMethodCode(void* startPC)
{
    TR_LinkageInfo* linkageInfo
        = TR_LinkageInfo::get(startPC); //(TR_LinkageInfo *) (((uint8_t*)startPC) + PC_TO_LINKAGE_INFO);
...
void J9::Recompilation::methodHasBeenRecompiled(void* oldStartPC, void* newStartPC, TR_FrontEnd* fe)
{
    ...
    if (linkageInfo->isCountingMethodBody()) {
        ...
        if (!(distance <= BRANCH_FORWARD_LIMIT && distance >= BRANCH_BACKWARD_LIMIT)) {
            distance
                = fe->indexedTrampolineLookup(TR_PPCcountingPatchCallSite, (void*)patchAddr) - (intptrj_t)patchAddr;
        ...
        *patchAddr = newInstr;
        ...
        if (!(distance <= BRANCH_FORWARD_LIMIT && distance >= BRANCH_BACKWARD_LIMIT)) {
            distance
                = fe->indexedTrampolineLookup(TR_PPCsamplingPatchCallSite, (void*)patchAddr) - (intptrj_t)patchAddr;
            TR_ASSERT(distance <= BRANCH_FORWARD_LIMIT && distance >= BRANCH_BACKWARD_LIMIT,
                "CodeCache is more than 32MB.\n");
        }
...
void J9::Recompilation::methodCannotBeRecompiled(void* oldStartPC, TR_FrontEnd* fe)
{
    ...
    if (bodyInfo->getUsesPreexistence() // TODO: reconsider whether this is a race cond for info
        ...
        || (linkageInfo->isSamplingMethodBody()
               && !fej9->isAsyncCompilation())) // go interpreted for failed recomps in sync mode
    {
        ...
        *patchAddr = newInstr;
        ...
        int32_t* branchLocation
            = (int32_t*)((uint8_t*)oldStartPC + getJitEntryOffset(linkageInfo) + OFFSET_COUNTING_BRANCH_FROM_JITENTRY);
        patchAddr = (int32_t*)((uint8_t*)oldStartPC + getJitEntryOffset(linkageInfo));
