...
static bool isGoodPtr(void* p) { return (p != 0 && p > (void*)1024 && IS_4_BYTE_ALIGNED(p)); }

void* TR_DebugExt::operator new(size_t s, TR_Malloc_t dbgjit_Malloc) { return dbgjit_Malloc(s, NULL); }

bool TR_DebugExt::dxReadMemory(void* remotePtr, void* localPtr, uintptrj_t size)
{
    ...
    if (bytesRead != size) {
        _dbgPrintf("\n*** JIT Error: could not read memory at 0x%x for %zu bytes\n", remotePtr,
            size); // FIXME:: should use dbgError()
        if (_memchk)
            assert(false);
        return false;
    }
...
bool TR_DebugExt::dxReadField(void* classPtr, uintptrj_t fieldOffset, void* localPtr, uintptrj_t size)
{
...
void* TR_DebugExt::dxMalloc(uintptrj_t size, void* remotePtr, bool dontAddToMap)
{
    if (size == 0)
        return 0;
...
void* TR_DebugExt::dxMallocAndRead(uintptrj_t size, void* remotePtr, bool dontAddToMap)
{
    if (size == 0 || remotePtr == 0)
        return 0;
...
void* TR_DebugExt::dxMallocAndReadString(void* remotePtr, bool dontAddToMap)
{
    char c;
    int sz = 0;
...
void TR_DebugExt::dxFree(void* localAddr)
{
    if (!localAddr)
        return;
    ...
    if (_toRemotePtrMap->locate(localAddr, hi)) {
        if (_memchk)
            _dbgPrintf("   JIT: free pair (local=0x%p, remote=0x%p)\n", localAddr, _toRemotePtrMap->getData(hi));
...
void TR_DebugExt::updateLocalPersistentMemoryFunctionPointers(
    J9JITConfig* localJitConfig, TR_PersistentMemory* localPersistentMemory)
{
    if (localPersistentMemory) {
        TR::PersistentAllocator* temp = new (dxMalloc(sizeof(TR::PersistentAllocator), 0, true))
            TR::PersistentAllocator((TR::PersistentAllocatorKit(64 * 1024, *localJitConfig->javaVM)));
 ...
 */
TR_DebugExt::TR_DebugExt(TR_InternalFunctions* f, struct J9PortLibrary* dbgextPortLib, J9JavaVM* localVM,
    ...
    void* (*dbgjit_Malloc)(uintptrj_t size, void* originalAddress), void (*dbgjit_Free)(void* addr),
    ...
    , _structureValid(false)
{
    ...
    if (_remotePersistentMemory) {
        _localPersistentMemory
            = (TR_PersistentMemory*)dxMallocAndRead(sizeof(TR_PersistentMemory), _remotePersistentMemory, true);
    ...
    if (_remotePersistentInfo)
        _localPersistentInfo
            = (TR::PersistentInfo*)dxMallocAndRead(sizeof(TR::PersistentInfo), _remotePersistentInfo, true);
    else
...
bool TR_DebugExt::compInfosPerThreadAvailable()
{
    if (_localCompInfosPT == NULL) {
        _localCompInfosPT = (TR::CompilationInfoPerThread**)dxMalloc(
            sizeof(TR::CompilationInfoPerThread*) * MAX_TOTAL_COMP_THREADS, NULL);
        if (_localCompInfosPT) {
            for (size_t i = 0; i < MAX_TOTAL_COMP_THREADS; ++i) {
                _localCompInfosPT[i] = _localCompInfo->_arrayOfCompilationInfoPerThread[i] != NULL
                    ? (TR::CompilationInfoPerThread*)dxMallocAndRead(sizeof(TR::CompilationInfoPerThread),
                          _localCompInfo->_arrayOfCompilationInfoPerThread[i], true)
                    : NULL;
            }
        }
    }
...
bool TR_DebugExt::activeMethodsToBeCompiledAvailable()
{
    if (!compInfosPerThreadAvailable())
        return false;
    if (_localActiveMethodsToBeCompiled == NULL) {
        _localActiveMethodsToBeCompiled
            = (TR_MethodToBeCompiled**)dxMalloc(sizeof(TR_MethodToBeCompiled*) * MAX_TOTAL_COMP_THREADS, NULL);
        if (_localActiveMethodsToBeCompiled) {
            for (size_t i = 0; i < MAX_TOTAL_COMP_THREADS; ++i) {
                _localActiveMethodsToBeCompiled[i] = _localCompInfosPT[i] && _localCompInfosPT[i]->_methodBeingCompiled
                    ? (TR_MethodToBeCompiled*)dxMallocAndRead(
                          sizeof(TR_MethodToBeCompiled), _localCompInfosPT[i]->_methodBeingCompiled, true)
                    : NULL;
            }
        }
    }
...
bool TR_DebugExt::isAOTCompileRequested(TR::Compilation* remoteCompilation)
{
    if (!compInfosPerThreadAvailable())
        return false;
    if (!activeMethodsToBeCompiledAvailable())
        return false;

    for (size_t i = 0; i < MAX_TOTAL_COMP_THREADS; ++i) {
        if (_localCompInfosPT[i] && _localCompInfosPT[i]->_compiler == remoteCompilation
            && _localActiveMethodsToBeCompiled[i] && _localActiveMethodsToBeCompiled[i]->_useAotCompilation) {
            return true;
        }
    }
...
void TR_DebugExt::allocateLocalCompiler(TR::Compilation* remoteCompiler)
{
    ...
    if (_localCompiler) {
        ...
        _localCompiler->_codeGenerator
            = (TR::CodeGenerator*)dxMallocAndRead(sizeof(TR::CodeGenerator), _localCompiler->_codeGenerator);
        ...
        _localCompiler->_symRefTab
            = (TR::SymbolReferenceTable*)dxMallocAndRead(sizeof(TR::SymbolReferenceTable), _localCompiler->_symRefTab);
        if (_localCompiler->_currentSymRefTab)
            _localCompiler->_currentSymRefTab = (TR::SymbolReferenceTable*)dxMallocAndRead(
                sizeof(TR::SymbolReferenceTable), _localCompiler->_currentSymRefTab);

        _localCompiler->_methodSymbol = (TR::ResolvedMethodSymbol*)dxMallocAndRead(
            sizeof(TR::ResolvedMethodSymbol), _localCompiler->_methodSymbol);
        if (_localCompiler->_optimizer)
            _localCompiler->_optimizer->_methodSymbol = _localCompiler->_methodSymbol;

        _localCompiler->_knownObjectTable
            = (TR::KnownObjectTable*)dxMallocAndRead(sizeof(TR::KnownObjectTable), _localCompiler->_knownObjectTable);
        if (_localCompiler->_knownObjectTable) {
            ...
            TR_Array<uintptrj_t*>& localReferences
                = ((J9::KnownObjectTable*)_localCompiler->_knownObjectTable)->_references;
...
void TR_DebugExt::allocateLocalFrontEnd()
{
...
void TR_DebugExt::freeLocalCompiler()
{
    if (_localCompiler) {
...
void TR_DebugExt::freeLocalFrontEnd()
{
    if (_localCompiler->_fe)
        dxFree(_localCompiler->_fe);
}

bool TR_DebugExt::initializeDebug(TR::Compilation* remoteCompiler)
{
    bool ok = true;
    if (remoteCompiler && remoteCompiler != _remoteCompiler) {
        ...
        if (_localCompiler && _localCompiler->_fe && _localCompiler->_trMemory) {
            ...
            ((TR_InternalFunctionsExt*)_jit)
                ->reInitialize(this, _localCompiler, (TR_FrontEnd*)_localCompiler->_fe, _localCompiler->_trMemory);
        } else {
            ok = false;
        }
    }
...
void TR_DebugExt::dxAllocateLocalBlockInternals(TR::Node* localNode)
{
    if (localNode->hasBlock()) {
        ...
        if (localNode->getOpCodeValue() == TR::BBStart && localBlk->getEntry()) {
            ...
            if (_structureValid && localBlk->getStructureOf()) {
                ...
                TR_BlockStructure* localBlockStructure
                    = (TR_BlockStructure*)dxMallocAndRead(sizeof(TR_BlockStructure), blockStructure);
            ...
            if (localBlk->isCatchBlock()) {
                ...
                TR::Block::TR_CatchBlockExtension* localCatchBlockExtension
                    = (TR::Block::TR_CatchBlockExtension*)dxMallocAndRead(
                        sizeof(TR::Block::TR_CatchBlockExtension), catchBlockExtension);
...
void TR_DebugExt::dxFreeLocalBlockInternals(TR::Node* localNode)
{
    if (localNode->hasBlock()) {
        TR::Block* localBlk = localNode->getBlock();
...
void TR_DebugExt::dxAllocateSymRefInternals(TR::SymbolReference* localSymRef, bool complete)
{
    ...
    case TR::Symbol::IsResolvedMethod: {
        ...
        if (complete) {
            TR_J9MethodBase* localMethod = (TR_J9MethodBase*)dxMallocAndRead(
                sizeof(TR_J9MethodBase), (void*)localSym->getMethodSymbol()->getMethod());
...
void TR_DebugExt::dxFreeSymRefInternals(TR::Symbol* localSymbol, bool complete)
{
    if (localSymbol) {
        if (complete) {
...
TR::Node* TR_DebugExt::dxAllocateLocalNode(TR::Node* remoteNode, bool recursive, bool complete)
{
    ...
    if (localNode) {
        if (localNode->getOpCode().hasSymbolReference()) {
            localNode->_unionPropertyA._symbolReference = (TR::SymbolReference*)dxMallocAndRead(
                sizeof(TR::SymbolReference), localNode->_unionPropertyA._symbolReference);
            ...
            localNode->_unionPropertyA._regLoadStoreSymbolReference = (TR::SymbolReference*)dxMallocAndRead(
                sizeof(TR::SymbolReference), localNode->_unionPropertyA._regLoadStoreSymbolReference);
            ...
            localNode->_unionPropertyA._branchDestinationNode
                = (TR::TreeTop*)dxMallocAndRead(sizeof(TR::TreeTop), localNode->_unionPropertyA._branchDestinationNode);
        else if (localNode->hasBlock())
            localNode->_unionPropertyA._block
                = (TR::Block*)dxMallocAndRead(sizeof(TR::Block), localNode->_unionPropertyA._block);
        else if (localNode->hasPinningArrayPointer())
            localNode->_unionPropertyA._pinningArrayPointer = (TR::AutomaticSymbol*)dxMallocAndRead(
                sizeof(TR::AutomaticSymbol), localNode->_unionPropertyA._pinningArrayPointer);

#if defined(TR_TARGET_S390)
        if (localNode->getOpCode().canHaveStorageReferenceHint())
            localNode->_storageReferenceHint
                = (TR_StorageReference*)dxMallocAndRead(sizeof(TR_StorageReference), localNode->_storageReferenceHint);
#endif
        ...
        if (localNode->hasNodeExtension()) {
            ...
            size_t sizeOfNodeExtension
                = sizeof(TR::NodeExtension) + ((numElements - NUM_DEFAULT_ELEMS) * sizeof(uintptr_t));

            TR::NodeExtension* remoteNodeExtension = nodeExtensionStore->getExtensionPtr();
            TR::NodeExtension* localNodeExtension
                = (TR::NodeExtension*)dxMallocAndRead(sizeOfNodeExtension, remoteNodeExtension);
...
void TR_DebugExt::dxFreeLocalNode(TR::Node* localNode, bool recursive, bool complete)
{
    if (localNode) {
        if (recursive) {
            uint16_t numChildren = 0;
...
TR::TreeTop* TR_DebugExt::dxAllocateLocalTreeTop(TR::TreeTop* remoteTreeTop, bool recursive)
{
...
void TR_DebugExt::dxFreeLocalTreeTop(TR::TreeTop* localTreeTop, bool recursive)
{
    dxFreeLocalNode(localTreeTop->getNode(), recursive);
    dxFree(localTreeTop);
}

TR_PersistentMethodInfo* TR_DebugExt::Compilation2MethodInfo()
{
    ...
    TR::Recompilation* localRecompilationInfo
        = (TR::Recompilation*)dxMallocAndRead(sizeof(TR::Recompilation), (void*)recompilationInfo);
...
TR_PersistentProfileInfo* TR_DebugExt::Compilation2ProfileInfo()
{
    if (_localCompiler == 0 || _remoteCompiler == 0)
        return 0;
    ...
    if (minfo) {
        TR_PersistentMethodInfo* localMInfo
            = (TR_PersistentMethodInfo*)dxMallocAndRead(sizeof(TR_PersistentMethodInfo), (void*)minfo);
        if (localMInfo) {
 ...
 */
TR_RuntimeAssumptionTable* TR_DebugExt::PersistentInfo2RuntimeAssumptionTable()
{
    TR_RuntimeAssumptionTable* runtimeAssumptionTable = NULL;
    if (_localPersistentInfo && _remotePersistentInfo) {
        runtimeAssumptionTable = (TR_RuntimeAssumptionTable*)((char*)_remotePersistentInfo
            + ((char*)_localPersistentInfo->getRuntimeAssumptionTable() - (char*)_localPersistentInfo));
 ...
 */
TR_PersistentCHTable* TR_DebugExt::PersistentInfo2PersistentCHTable()
{
 ...
 * return (TR::PersistentInfo*)->getPersistentCHTable()->getCHTable();
 */
TR_CHTable* TR_DebugExt::PersistentInfo2CHTable() { return NULL; }

/*
 * return TR_PersistentMemory::getPersistentInfo()
 */
TR::PersistentInfo* TR_DebugExt::PersistentMemory2PersistentInfo()
{
 ...
 */
TR_PersistentMemory* TR_DebugExt::J9JITConfig2PersistentMemory()
{
 ...
 */
TR_FrontEnd* TR_DebugExt::J9JITConfig2FrontEnd()
{
    if (_localJITConfig == 0)
        return 0;
 ...
 */
TR::CompilationInfo* TR_DebugExt::FrontEnd2CompInfo(TR_FrontEnd* remoteFE)
{
    if (remoteFE == 0)
        return 0;
 ...
 */
TR::CompilationInfoPerThread** TR_DebugExt::CompInfo2ArrayOfCompilationInfoPerThread(uint8_t* numThreads)
{
 ...
 */
TR::Optimizer* TR_DebugExt::Compilation2Optimizer()
{
 ...
 */
TR::ResolvedMethodSymbol* TR_DebugExt::Optimizer2ResolvedMethodSymbol(TR::Optimizer* remoteOptimizer)
{
    ...
    dxReadField(
        remoteOptimizer, offsetof(TR::Optimizer, _methodSymbol), &methodSymbol, sizeof(TR::ResolvedMethodSymbol*));
    _dbgPrintf(
        "((TR::Optimizer*)0x%p)->_methodSymbol = (TR::ResolvedMethodSymbol*)0x%p\n", remoteOptimizer, methodSymbol);
    return methodSymbol;
}
 ...
 */
TR::ResolvedMethodSymbol* TR_DebugExt::Compilation2ResolvedMethodSymbol()
{
    if (_remoteCompiler == 0)
        return 0;
    ...
    TR::ResolvedMethodSymbol* methodSymbol;
    dxReadField(
        _remoteCompiler, offsetof(TR::Compilation, _methodSymbol), &methodSymbol, sizeof(TR::ResolvedMethodSymbol*));
    _dbgPrintf(
        "((TR::Compilation*)0x%p)->_methodSymbol = (TR::ResolvedMethodSymbol*)0x%p\n", _remoteCompiler, methodSymbol);
    return methodSymbol;
}
 ...
 */
TR::CFG* TR_DebugExt::Compilation2CFG()
{
    TR::CFG* cfg = NULL;
    TR::ResolvedMethodSymbol* methodSymbol = Compilation2ResolvedMethodSymbol();
    if (methodSymbol) {
        TR::ResolvedMethodSymbol* localMethodSymbol
            = (TR::ResolvedMethodSymbol*)dxMallocAndRead(sizeof(TR::ResolvedMethodSymbol), methodSymbol);
...
TR::CompilationInfoPerThread** TR_DebugExt::dxMallocAndGetArrayOfCompilationInfoPerThread(
    uint8_t numThreads, void* addr) // Helper function, remember to use dxFree after
{
    return static_cast<TR::CompilationInfoPerThread**>(
        dxMallocAndRead(sizeof(TR::CompilationInfoPerThread*) * numThreads, addr));
}

/*
 * print debugger extension usage
 */
void TR_DebugExt::dxPrintUsage()
{
    _dbgPrintf("Usage:\n");
    ...
    _dbgPrintf(
        "\t!trprint jittedbodyinfo <TR_PersistentJittedBodyInfo address>     // dump PersistentJittedBodyInfo\n");
    ...
    _dbgPrintf(
        "\t!trprint runtimeassumptiontable [TR_RuntimeAssumptionTable address] // dump RuntimeAssumptionTable\n");
    _dbgPrintf("\t!trprint runtimeassumptionarray <OMR::RuntimeAssumption*[] address> start end  // dump "
               "RuntimeAssumption array\n");
    _dbgPrintf("\t!trprint runtimeassumption <OMR::RuntimeAssumption* address>                       // dump "
               "RuntimeAssumption\n");

    _dbgPrintf("\n");

    _dbgPrintf("\t*** The following should ONLY be invoked after invoking !trprint compilation in order to cache the "
               "compilation object.\n");
    _dbgPrintf(
        "\t*** Additionally, make sure that the addresses passed in are associated with said compilation object.\n");
    _dbgPrintf(
        "\t*** This is necessary because the TR_Debug object assumes a comp object (among many others) exists\n");
    _dbgPrintf("\t\t!trprint compilationil [TR::Compilation address]          // dump all ILs of the method in "
               "compilation object; will update cached TR::Compilation object\n");
    _dbgPrintf(
        "\t\t!trprint inlinedcalls                                     // dump all inlined callsites in the method\n");
    ...
    _dbgPrintf(
        "\t\t!trprint persistentmethodinfo [TR_PersistentMethodInfo address]     // dump PersistentMethodInfo\n");
    _dbgPrintf(
        "\t\t!trprint persistentprofileinfo [TR_PersistentProfileInfo address]   // dump PersistentProfileInfo\n");
    ...
    _dbgPrintf("\t!trprint blockil <TR::Block address> [all=1|0]          // print all ILs in a basic block / bbs that "
               "follows\n");
    ...
    _dbgPrintf("\t!trprint freecodecacheblocklist <TR::CodeCache address>    // dump list of free blocks in this code "
               "cache\n");
    _dbgPrintf("\t!trprint freecodecacheblock <OMR::CodeCacheFreeCacheBlock address>   // dump "
               "OMR::CodeCacheFreeCacheBlock struct\n");
    ...
    _dbgPrintf("\t!trprint datacachesizebucket <TR_DataCacheManager::SizeBucket address>   // dump "
               "TR_DataCacheManager::SizeBucket contents\n");
    _dbgPrintf("\t!trprint datacacheallocation <TR_DataCacheManager::Allocation address>   // dump "
               "TR_DataCacheManager::Allocation contents\n");
    _dbgPrintf("\t!trprint datacachesizebucketlistelement "
               "<TR_DataCacheManager::InPlaceList<TR_DataCacheManager::SizeBucket>::ListElement address>   // dump "
               "TR_DataCacheManager::InPlaceList<TR_DataCacheManager::SizeBucket>::ListElement contents\n");
    _dbgPrintf("\t!trprint datacacheallocationlistelement "
               "<TR_DataCacheManager::InPlaceList<TR_DataCacheManager::Allocation>::ListElement address>   // dump "
               "TR_DataCacheManager::InPlaceList<TR_DataCacheManager::Allocation>::ListElement contents\n");
    ...
    _dbgPrintf("\t!trprint aotinfo [AOT data address]                    // print AOT information at [address] (see "
               "!shrc to get AOT data address)\n");
    _dbgPrintf("\t!trprint memchk [1|0]                                  // turn on/off verbose output for memory "
               "allocation\n");
    _dbgPrintf("\t!trprint debug                                         // let another debugger to attach to current "
 ...
 */
void TR_DebugExt::dxTrPrint(const char* name1, void* addr2, uintptrj_t argCount, const char* args)
{
    /*
...
#define MAX_ARGS 5
    ...
    for (argp2 = myArgs; *argp2;) {
        argv[argc++] = argp2;
        while (*argp2 && *argp2 != ' ' && *argp2 != ',')
            argp2++;
        while (*argp2 && (*argp2 == ' ' || *argp2 == ','))
            *argp2++ = '\0'; // advanced to next arg while replacing delimiter to null char
    }
    ...
    void* addr = 0;
    if (argc >= 2)
        addr = (void*)_dbgGetExpression(argv[1]);
    argCount = argc;
...
#undef MAX_ARGS
    ...
    } else if (stricmp_ignore_locale(className, "freecodecacheblock") == 0) // PersistentCHTable
    {
        ...
        dxPrintDataCacheAllocationListElement(static_cast<void*>(
            addr)); // TR_DataCacheManager::InPlaceList<TR_DataCacheManager::Allocation>::ListElement *
    } else if (stricmp_ignore_locale(className, "datacachesizebucketlistelement") == 0) {
        dxPrintDataCacheSizeBucketListElement(static_cast<void*>(
            addr)); // TR_DataCacheManager::InPlaceList<TR_DataCacheManager::SizeBucket>::ListElement *
    ...
    } else if (stricmp_ignore_locale(className, "runtimeassumptionarray") == 0
        && argCount >= 2) // RuntimeAssumption array
    {
        int32_t start = 0;
        int32_t end = 0;
        if (argCount > 2)
    ...
    } else if (stricmp_ignore_locale(className, "runtimeassumptionlistfrommetadata") == 0
        && argCount == 2) // RuntimeAssumptionList
    {
 ...
 */
const char* TR_DebugExt::dxGetName(const char* s, void* p)
{
#define BUFSIZE 5
...
const char* TR_DebugExt::getName(const char* s, int32_t len)
{
#define BUFSIZE 5
...
const char* TR_DebugExt::getName(const char* s) { return getName(s, -1); }

/*
 * Printing Compilation / IR Trees
 */
void TR_DebugExt::printDestination(TR::FILE* pOutFile, TR::TreeTop* treeTop)
{
...
void TR_DebugExt::printDestination(TR::TreeTop* treeTop, TR_PrettyPrinterString& output)
{
...
void TR_DebugExt::print(TR::FILE* pOutFile, TR::SymbolReference* symref)
{
    TR_PrettyPrinterString output(this);
...
void TR_DebugExt::printNodeInfo(TR::FILE* pOutFile, TR::Node* node)
{
    TR_PrettyPrinterString output(this);
...
void TR_DebugExt::printInlinedCallSites(TR::FILE* pOutFile, TR::ResolvedMethodSymbol* methodSymbol)
{
    ...
    TR::Compilation::TR_InlinedCallSiteInfo* localArray
        = (TR::Compilation::TR_InlinedCallSiteInfo*)dxMallocAndRead(sizeof(array[0]) * arraySize, array);
    ...
    for (int32_t i = 0; i < localInlinedCallSites.size(); ++i) {
        ...
        TR_OpaqueMethodBlock* mb = _isAOT
            ? (TR_OpaqueMethodBlock*)((TR_AOTMethodInfo*)ics._methodInfo)->resolvedMethod->getNonPersistentIdentifier()
            : ics._vmMethodInfo;

        trfprintf(pOutFile, "    %4d       %4d       %5d       %s !trprint j9method %p\n", i,
            ics._byteCodeInfo.getCallerIndex(), ics._byteCodeInfo.getByteCodeIndex(), getMethodName(mb), mb);
    }
 ...
 */
void TR_DebugExt::dxPrintBlockIL(TR::Block* block, seenNode** seenNodes, int32_t numBlocks)
{
    assert(numBlocks > 0);
 ...
 **/
void TR_DebugExt::freeSeenNodes(seenNode** seenNodes)
{
 ...
 */
void TR_DebugExt::dxPrintNodeIL(TR::Node* node, seenNode** seenNodes, int32_t indentation)
{
...
void TR_DebugExt::dxPrintMethodIL()
{
    seenNode* seenNodes = NULL;
    /*
...
void TR_DebugExt::dxPrintInlinedCallSites(TR::ResolvedMethodSymbol* methodSymbol)
{
    printInlinedCallSites(TR::IO::Stdout, methodSymbol);
}

void TR_DebugExt::dxPrintCompilationIL()
{
 ...
 * Verifying IR Trees
 */
void TR_DebugExt::dxVerifyTrees()
{
    TR::ResolvedMethodSymbol* remoteMethodSymbol = Compilation2ResolvedMethodSymbol();
    verifyTrees(remoteMethodSymbol);
}

void TR_DebugExt::verifyTrees(TR::ResolvedMethodSymbol* s) { _dbgPrintf("Not implemented\n"); }

/*
 * Verifying Blocks
 */
void TR_DebugExt::dxVerifyBlocks()
{
    TR::ResolvedMethodSymbol* remoteMethodSymbol = Compilation2ResolvedMethodSymbol();
    verifyBlocks(remoteMethodSymbol);
}

void TR_DebugExt::verifyBlocks(TR::ResolvedMethodSymbol* s) { _dbgPrintf("Not implemented\n"); }

/*
 * Verifying CFG
 */
void TR_DebugExt::dxVerifyCFG(TR::CFG* cfg)
{
 ...
 */
void TR_DebugExt::print(TR::FILE* pOutFile, TR_Structure* structure, uint32_t indentation)
{
    /*
...
#ifdef TRPRINT_STRUCTURE_SUBGRAPH
void TR_DebugExt::print(TR::FILE* pOutFile, TR_RegionStructure* regionStructure, uint32_t indentation)
{
    TR_RegionStructure* localRegionStructure
        = (TR_RegionStructure*)dxMallocAndRead(sizeof(TR_RegionStructure), regionStructure);
    TR_RegionStructure* versionedLoop = localRegionStructure->getVersionedLoop();
    if (versionedLoop)
        localRegionStructure->setVersionedLoop(
            (TR_RegionStructure*)dxMallocAndRead(sizeof(TR_RegionStructure), versionedLoop));
    TR_StructureSubGraphNode* entryNode = localRegionStructure->getEntry();
    TR_StructureSubGraphNode* localEntryNode
        = (TR_StructureSubGraphNode*)dxMallocAndRead(sizeof(TR_StructureSubGraphNode), entryNode);
 ...
 */
void TR_DebugExt::printSubGraph(TR::FILE* pOutFile, TR_RegionStructure* regionStructure, uint32_t indentation)
{
 ...
 */
void TR_DebugExt::print(TR::FILE* pOutFile, TR_InductionVariable* inductionVariable, uint32_t indentation)
{
...
void TR_DebugExt::print(TR::FILE* pOutFile, TR::Block* block, uint32_t indentation)
{
...
void TR_DebugExt::print(TR::FILE* pOutFile, TR::CFGNode* cfgNode, uint32_t indentation)
{
    print(pOutFile, (TR::Block*)cfgNode, 0); // force downcast to avoid virtual call asBlock()
}

TR::CFG* TR_DebugExt::newCFG(TR::CFG* cfg)
{
    TR::CFG* localCfg = (TR::CFG*)dxMallocAndRead(sizeof(TR::CFG), cfg);
    ...
    int32_t cfgNodeSize
        = std::max(sizeof(TR::Block), sizeof(TR_StructureSubGraphNode)); // explore all sub-classes possibilities
...
void TR_DebugExt::freeCFG(TR::CFG* localCfg)
{
 ...
 */
void TR_DebugExt::dxPrintCFG(TR::CFG* cfg)
{
 ...
 */
void TR_DebugExt::dxPrintBlockCFG(TR::Block* block)
{
    if (block == 0) {
...
void TR_DebugExt::dxPrintPersistentJittedBodyInfo(TR_PersistentJittedBodyInfo* bodyInfo)
{
    if (bodyInfo == 0) {
    ...
    TR_PersistentJittedBodyInfo* localBodyInfo
        = (TR_PersistentJittedBodyInfo*)dxMallocAndRead(sizeof(TR_PersistentJittedBodyInfo), bodyInfo);
    if (localBodyInfo) {
        ...
        _dbgPrintf("\tTR_Hotness                _hotness = %d (%s)\n", localBodyInfo->_hotness,
            localBodyInfo->_hotness == -1 ? "unknown" : comp()->getHotnessName(localBodyInfo->_hotness));
...
void* TR_DebugExt::dxGetJ9MethodFromMethodToBeCompiled(TR_MethodToBeCompiled* methodToBeCompiled)
{
    TR_MethodToBeCompiled* localMethodBeingCompiled
        = (TR_MethodToBeCompiled*)dxMallocAndRead(sizeof(TR_MethodToBeCompiled), methodToBeCompiled);
...
void TR_DebugExt::dxPrintMethodsBeingCompiled(TR::CompilationInfo* remoteCompInfo)
{
    if (remoteCompInfo == 0) {
    ...
    arrayOfCompInfoPT
        = dxMallocAndGetArrayOfCompilationInfoPerThread(numThreads, remoteCompInfo->_arrayOfCompilationInfoPerThread);

    for (uint8_t i = 0; i < numThreads; i++) {
        if (arrayOfCompInfoPT[i]) {
            TR::CompilationInfoPerThread* localCompInfoPT = (TR::CompilationInfoPerThread*)dxMallocAndRead(
                sizeof(TR::CompilationInfoPerThread), arrayOfCompInfoPT[i]);

            struct J9Method* ramMethod
                = (struct J9Method*)dxGetJ9MethodFromMethodToBeCompiled(localCompInfoPT->_methodBeingCompiled);
...
void TR_DebugExt::dxPrintCompilationInfo(TR::CompilationInfo* remoteCompInfo)
{
    ...
    if (localCompInfo) {
        _dbgPrintf("TR::CompilationInfoPerThread * const * _arrayOfCompilationInfoPerThread = !trprint "
                   "arrayofcompilationinfoperthread 0x%p\n",
            remoteCompInfo->_arrayOfCompilationInfoPerThread);
        _dbgPrintf("TR::CompilationInfoPerThreadBase *     _compInfoForCompOnAppThread      = !trprint "
                   "compilationinfoperthreadbase 0x%p\n",
            localCompInfo->_compInfoForCompOnAppThread);
        _dbgPrintf("TR_MethodToBeCompiled *               _methodQueue                     = !trprint "
                   "methodtobecompiled 0x%p\n",
            localCompInfo->_methodQueue);
        _dbgPrintf("TR_MethodToBeCompiled *               _methodPool                      = !trprint "
                   "methodtobecompiled 0x%p\n",
            localCompInfo->_methodPool);
        _dbgPrintf("J9JITConfig *                         _jitConfig                       = 0x%p\n",
            localCompInfo->_jitConfig);
        _dbgPrintf(
            "TR_PersistentMemory *                 _persistentMemory                = !trprint persistentmemory 0x%p\n",
        ...
        _dbgPrintf(
            "struct DLT_record *                   _dltHash[DLT_HASHSIZE]           = 0x%p\n", localCompInfo->_dltHash);
#endif
        ...
        _dbgPrintf("TR_MethodToBeCompiled *               _firstLPQentry                   = !trprint "
                   "methodtobecompiled 0x%p\n",
            localCompInfo->_lowPriorityCompilationScheduler._firstLPQentry);
        _dbgPrintf("TR_MethodToBeCompiled *               _lastLPQentry                    = !trprint "
                   "methodtobecompiled 0x%p\n",
...
void TR_DebugExt::dxPrintCompilationInfoPerThreadBase(TR::CompilationInfoPerThreadBase* remoteCompInfoPTB)
{
    ...
    TR::CompilationInfoPerThreadBase* localCompInfoPTB = (TR::CompilationInfoPerThreadBase*)dxMallocAndRead(
        sizeof(TR::CompilationInfoPerThreadBase), remoteCompInfoPTB);
    ...
    _dbgPrintf(
        "\tTR::Compilation *                  _compiler = !trprint compilation 0x%p\n", localCompInfoPTB->_compiler);
...
void TR_DebugExt::dxPrintCompilationInfoPerThread(TR::CompilationInfoPerThread* remoteCompInfoPT)
{
    ...
    TR::CompilationInfoPerThread* localCompInfoPT
        = (TR::CompilationInfoPerThread*)dxMallocAndRead(sizeof(TR::CompilationInfoPerThread), remoteCompInfoPT);

    _dbgPrintf("\n\tcompilationInfoPerThread at 0x%p\n", remoteCompInfoPT);
    _dbgPrintf(
        "\tTR::CompilationInfo *       _compInfo = !trprint compilationinfo 0x%p\n", &localCompInfoPT->_compInfo);
    _dbgPrintf("\tTR_MethodToBeCompiled *    _methodBeingCompiled = !trprint methodtobecompiled 0x%p\n",
        localCompInfoPT->_methodBeingCompiled);
    ...
    _dbgPrintf(
        "\tCompilationThreadState     _compilationThreadState = 0x%d\n", localCompInfoPT->_compilationThreadState);
    _dbgPrintf("\tTR_DataCache *             _reservedDataCache = !trprint datacache 0x%p\n",
...
void TR_DebugExt::dxPrintMethodToBeCompiled(TR_MethodToBeCompiled* remoteCompEntry)
{
    ...
    TR_MethodToBeCompiled* localCompEntry
        = (TR_MethodToBeCompiled*)dxMallocAndRead(sizeof(TR_MethodToBeCompiled), remoteCompEntry);
 ...
 */
void TR_DebugExt::dxPrintCompilationTracingBuffer()
{
    ...
    if (_remoteCompInfo && _localCompInfo) {
        ...
        _dbgPrintf(
            "*** JIT Info: compilationTracingFacility struct at 0x%p\n", &_remoteCompInfo->_compilationTracingFacility);
        ...
        if (localCompTracingFacility->isInitialized()) {
            ...
            TR_CompilationTracingEntry* localBuffer = (TR_CompilationTracingEntry*)dxMallocAndRead(
                sizeof(TR_CompilationTracingEntry) * size, tracingBuffer);
            for (int32_t i = 0; i < size; i++) // visit all elements
            {
                TR_CompilationTracingEntry* entry = localBuffer + crtIndex;
                const char* name = (entry->_operation < OP_LastValidOperation)
                    ? TR::CompilationInfo::OperationNames[entry->_operation]
                    : "INVALID";
                _dbgPrintf("Index=%d J9VMThread=0x%p operation=%s otherData=%u\n", crtIndex,
                    localCompTracingFacility->expandJ9VMThreadId(entry->_J9VMThreadId), name, entry->_otherData);
 ...
 */
void TR_DebugExt::dxPrintPersistentInfo(TR::PersistentInfo* remotePersistentInfo)
{
    if (remotePersistentInfo == NULL)
        remotePersistentInfo = _remotePersistentInfo;

    TR::PersistentInfo* localPersistentInfo
        = (TR::PersistentInfo*)dxMallocAndRead(sizeof(TR::PersistentInfo), remotePersistentInfo);
 ...
 */
void TR_DebugExt::dxPrintJ9MonitorTable(TR::MonitorTable* remoteMonTable)
{
    if (remoteMonTable == 0) {
    ...
    _dbgPrintf(
        "\tTR::Monitor * _iprofilerPersistenceMonitor = 0x%p\n", &(remoteMonTable->_iprofilerPersistenceMonitor));
 ...
 */
void TR_DebugExt::dxPrintCompilation()
{
    if (_remoteCompiler == 0 || _localCompiler == 0) {
    ...
    _dbgPrintf("\tTR_LinkHead<TR_ClassExtendCheck> _classesThatShouldNotBeNewlyExtended = 0x%p\n",
        (char*)_remoteCompiler
            + ((char*)&(localCompiler->_classesThatShouldNotBeNewlyExtended) - (char*)localCompiler));
    ...
    _dbgPrintf("\tList<TR_Snippet> _methodSnippetsToBePatchedOnClassUnload = 0x%p\n",
        (char*)_remoteCompiler
            + ((char*)&(localCompiler->_methodSnippetsToBePatchedOnClassUnload) - (char*)localCompiler));
    _dbgPrintf("\tList<TR_Pair<TR_Snippet,TR_ResolvedMethod> > _snippetsToBePatchedOnRegisterNative = 0x%p\n",
        (char*)_remoteCompiler
            + ((char*)&(localCompiler->_snippetsToBePatchedOnRegisterNative) - (char*)localCompiler));
    ...
    _dbgPrintf(
        "\tsize_t _scratchSpaceLimit = %llu\n", static_cast<unsigned long long>(localCompiler->_scratchSpaceLimit));

    dxFree(localCompiler);
}

void TR_DebugExt::dxPrintOptimizationPlan(TR_OptimizationPlan* remoteOptimizationPlan)
{
    ...
    TR_OptimizationPlan* localOptimizationPlan
        = (TR_OptimizationPlan*)dxMallocAndRead(sizeof(TR_OptimizationPlan), remoteOptimizationPlan);
    ...
    _dbgPrintf(
        "static unsigned long      _totalNumAllocatedPlans = 0x%lx\n", localOptimizationPlan->_totalNumAllocatedPlans);
...
void TR_DebugExt::dxPrintDataCache(TR_DataCache* remoteDataCache)
{
...
void TR_DebugExt::dxPrintPersistentMemory(TR_PersistentMemory* remotePersistentMemory)
{
    ...
    TR_PersistentMemory* localPersistentMemory
        = (TR_PersistentMemory*)dxMallocAndRead(sizeof(TR_PersistentMemory), remotePersistentMemory);
 ...
 */
void TR_DebugExt::dxPrintTRMemory(TR_Memory* remoteTrMemory)
{
    if (remoteTrMemory == 0) {
 ...
 * Printing CHTable
 */
void TR_DebugExt::dump(TR::FILE* pOutFile, TR_CHTable* chtable)
{
    TR_CHTable* localCHTable = (TR_CHTable*)dxMallocAndRead(sizeof(TR_CHTable), chtable);
    TR_Array<TR_OpaqueClassBlock*>* localClasses = (TR_Array<TR_OpaqueClassBlock*>*)dxMallocAndRead(
        sizeof(TR_Array<TR_OpaqueClassBlock*>), localCHTable->_classes);
    TR_Array<TR_ResolvedMethod*>* localPreXMethods = (TR_Array<TR_ResolvedMethod*>*)dxMallocAndRead(
        sizeof(TR_Array<TR_ResolvedMethod*>), localCHTable->_preXMethods);
    ...
    if (localClasses)
        localClasses->_array = (TR_OpaqueClassBlock**)newArray(
            sizeof(TR_OpaqueClassBlock*), localClasses->_internalSize, localClasses->_array);
    if (localPreXMethods)
        localPreXMethods->_array = (TR_ResolvedMethod**)newArray(
            sizeof(TR_ResolvedMethod*), localPreXMethods->_internalSize, localPreXMethods->_array);
...
void TR_DebugExt::dxPrintCHTable(TR_CHTable* chtable)
{
    if (chtable == 0) {
...
void TR_DebugExt::dxPrintPersistentProfileInfo(TR_PersistentProfileInfo* pinfo)
{
    if (pinfo == 0) {
        _dbgPrintf("PersistentProfileInfo is NULL\n");
        return;
    }
    TR_PersistentProfileInfo* localPInfo
        = (TR_PersistentProfileInfo*)dxMallocAndRead(sizeof(TR_PersistentProfileInfo), pinfo);

    _dbgPrintf("PersistentProfileInfo = 0x%p\n", pinfo);
    _dbgPrintf("  ->_callSiteInfo = (TR_CallSiteInfo*)0x%p\n", localPInfo->getCallSiteInfo());
    _dbgPrintf(
        "  ->_catchBlockProfileInfo = (TR_CatchBlockProfileInfo*)0x%p\n", localPInfo->getCatchBlockProfileInfo());
    ...
    _dbgPrintf("  ->_profilingFrequency = (int32_t)0x%p[%d]\n",
        (char*)pinfo + ((char*)localPInfo->getProfilingFrequencyArray() - (char*)localPInfo),
        PROFILING_INVOCATION_COUNT);
...
void TR_DebugExt::dxPrintCodeCacheInfo(TR::CodeCache* cacheInfo)
{
    if (cacheInfo == 0) {
...
OMR::CodeCacheFreeCacheBlock* TR_DebugExt::dxPrintFreeCodeCacheBlock(OMR::CodeCacheFreeCacheBlock* blockPtr)
{
    ...
    OMR::CodeCacheFreeCacheBlock* localBlockInfo
        = (OMR::CodeCacheFreeCacheBlock*)dxMallocAndRead(sizeof(OMR::CodeCacheFreeCacheBlock), blockPtr);
...
void TR_DebugExt::dxPrintFreeCodeCacheBlockList(TR::CodeCache* cacheInfo)
{
    if (cacheInfo == 0) {
        _dbgPrintf("TR::CodeCache is NULL\n");
        return;
    ...
    TR::CodeCache* localCacheInfo = (TR::CodeCache*)dxMallocAndRead(sizeof(TR::CodeCache), cacheInfo);
    _dbgPrintf(
        "  List of free block starting at:(OMR::CodeCacheFreeCacheBlock*)0x%p\n", localCacheInfo->freeBlockList());
...
void TR_DebugExt::dxPrintListOfCodeCaches()
{
    if (!_remotePersistentMemory)
        return;
...
void TR_DebugExt::dxPrintDataCacheManager(TR_DataCacheManager* manager)
{
    if (manager == 0) {
        _dbgPrintf("DataCacheManager is NULL\n");
        return;
    }
    TR_DataCacheManager* localManager
        = static_cast<TR_DataCacheManager*>(dxMallocAndRead(sizeof(TR_DataCacheManager), manager));
    ...
    _dbgPrintf(
        "  ->_sizeList._sentinel = TR_DataCacheManager::InPlaceList<TR_DataCacheManager::SizeBucket>::ListElement\n");
    _dbgPrintf("  ->_sizeList._sentinel._prev = "
               "(TR_DataCacheManager::InPlaceList<TR_DataCacheManager::SizeBucket>::ListElement *) 0x%p\n",
        localManager->_sizeList._sentinel._prev);
    _dbgPrintf("  ->_sizeList._sentinel._next = "
               "(TR_DataCacheManager::InPlaceList<TR_DataCacheManager::SizeBucket>::ListElement *) 0x%p\n",
...
void TR_DebugExt::dxPrintDataCacheSizeBucket(
    void* sizeBucket) // declared void * as forward declaration not possible in header
{
    if (sizeBucket == 0) {
        _dbgPrintf("SizeBucket is NULL\n");
        return;
    }
    TR_DataCacheManager::SizeBucket* localSizeBucket = static_cast<TR_DataCacheManager::SizeBucket*>(
        dxMallocAndRead(sizeof(TR_DataCacheManager::SizeBucket), sizeBucket));
    _dbgPrintf("TR_DataCacheManager::SizeBucket @ 0x%p\n", sizeBucket);
    _dbgPrintf("  ->_listElement = InPlaceList<SizeBucket>::ListElement\n");
    _dbgPrintf("  ->_listElement._prev = "
               "(TR_DataCacheManager::InPlaceList<TR_DataCacheManager::SizeBucket>::ListElement *)0x%p\n",
        localSizeBucket->_listElement._prev);
    _dbgPrintf("  ->_listElement._next = "
               "(TR_DataCacheManager::InPlaceList<TR_DataCacheManager::SizeBucket>::ListElement *)0x%p\n",
    ...
    _dbgPrintf("  ->_allocations._sentinel._prev = "
               "(TR_DataCacheManager::InPlaceList<TR_DataCacheManager::Allocation>::ListElement *) 0x%p\n",
        localSizeBucket->_allocations._sentinel._prev);
    _dbgPrintf("  ->_allocations._sentinel._next = "
               "(TR_DataCacheManager::InPlaceList<TR_DataCacheManager::Allocation>::ListElement *) 0x%p\n",
...
void TR_DebugExt::dxPrintDataCacheAllocation(
    void* allocation) // declared void * as forward declaration not possible in header
{
    if (allocation == 0) {
        _dbgPrintf("Allocation is NULL\n");
        return;
    }
    TR_DataCacheManager::Allocation* localAllocation = static_cast<TR_DataCacheManager::Allocation*>(
        dxMallocAndRead(sizeof(TR_DataCacheManager::Allocation), allocation));
...
void TR_DebugExt::dxPrintDataCacheAllocationListElement(
    void* listElement) // declared void * as forward declaration not possible in header
{
    if (listElement == 0) {
        _dbgPrintf("List Element is NULL\n");
        return;
    }
    TR_DataCacheManager::InPlaceList<TR_DataCacheManager::Allocation>::ListElement* localListElement
        = static_cast<TR_DataCacheManager::InPlaceList<TR_DataCacheManager::Allocation>::ListElement*>(dxMallocAndRead(
            sizeof(TR_DataCacheManager::InPlaceList<TR_DataCacheManager::Allocation>::ListElement), listElement));
...
void TR_DebugExt::dxPrintDataCacheSizeBucketListElement(
    void* listElement) // declared void * as forward declaration not possible in header
{
    if (listElement == 0) {
        _dbgPrintf("List Element is NULL\n");
    ...
    TR_DataCacheManager::InPlaceList<TR_DataCacheManager::SizeBucket>::ListElement* localListElement
        = static_cast<TR_DataCacheManager::InPlaceList<TR_DataCacheManager::SizeBucket>::ListElement*>(dxMallocAndRead(
            sizeof(TR_DataCacheManager::InPlaceList<TR_DataCacheManager::SizeBucket>::ListElement), listElement));
...
void TR_DebugExt::dxPrintPersistentMethodInfo(TR_PersistentMethodInfo* minfo)
{
    if (minfo == 0) {
    ...
    TR_PersistentMethodInfo* localMInfo
        = (TR_PersistentMethodInfo*)dxMallocAndRead(sizeof(TR_PersistentMethodInfo), minfo);
...
void TR_DebugExt::dxPrintPersistentCHTable(TR_PersistentCHTable* ptable)
{
    if (ptable == 0) {
...
void TR_DebugExt::dxPrintRuntimeAssumption(OMR::RuntimeAssumption* ra)
{
    if (ra == 0) {
        _dbgPrintf("RuntimeAssumption is NULL\n");
        return;
    }
    OMR::RuntimeAssumption* localRuntimeAssumption
        = (OMR::RuntimeAssumption*)dxMallocAndRead(sizeof(OMR::RuntimeAssumption), ra);
...
void TR_DebugExt::dxPrintRuntimeAssumptionList(OMR::RuntimeAssumption* ra)
{
    if (ra == 0) {
    ...
    while (nextRuntimeAssumption != ra) {
        OMR::RuntimeAssumption* localRuntimeAssumption
            = (OMR::RuntimeAssumption*)dxMallocAndRead(sizeof(OMR::RuntimeAssumption), nextRuntimeAssumption);
...
void TR_DebugExt::dxPrintRuntimeAssumptionListFromMetadata(J9JITExceptionTable* methodMetaData)
{
    J9JITExceptionTable* localMetadata
        = (J9JITExceptionTable*)dxMallocAndRead(sizeof(J9JITExceptionTable), methodMetaData);
    // dxPrintRuntimeAssumptionList(localMetadata->bodyInfo) // TODO: change this when we settle for a location
    dxFree(localMetadata);
}

void TR_DebugExt::dxPrintRuntimeAssumptionArray(OMR::RuntimeAssumption** raa, int32_t start, int32_t end)
{
    if (raa == 0) {
    ...
    bool dontMap = true;
    OMR::RuntimeAssumption** localRuntimeAssumptionArray
        = (OMR::RuntimeAssumption**)dxMallocAndRead(sizeof(OMR::RuntimeAssumption*) * (end + 1), raa);
    OMR::RuntimeAssumption* localRuntimeAssumption
        = (OMR::RuntimeAssumption*)dxMalloc(sizeof(OMR::RuntimeAssumption), 0, dontMap);
    ...
    for (i = start; i <= end; ++i) {
        if (localRuntimeAssumptionArray[i]) {
            _dbgPrintf(
                "(OMR::RuntimeAssumption*)[%d]= !trprint runtimeassumption 0x%p, ", i, localRuntimeAssumptionArray[i]);
            dxReadMemory(
                (void*)localRuntimeAssumptionArray[i], (void*)localRuntimeAssumption, sizeof(OMR::RuntimeAssumption));
...
void TR_DebugExt::dxPrintRuntimeAssumptionTable(TR_RuntimeAssumptionTable* rtable)
{
    if (rtable == 0) {
        _dbgPrintf("RuntimeAssumptionTable is NULL\n");
        return;
    }
    TR_RuntimeAssumptionTable* localRTable
        = (TR_RuntimeAssumptionTable*)dxMallocAndRead(sizeof(TR_RuntimeAssumptionTable), rtable);
    for (int i = 0; i < LastAssumptionKind; i++) {
        _dbgPrintf("&(((TR_RuntimeAssumptionTable*)0x%p)->%s)[%u]= !trprint runtimeassumptionarray 0x%p 0 %u\n", rtable,
            runtimeAssumptionKindNames[i], (uint32_t)localRTable->_tables[i]._spineArraySize,
...
void TR_DebugExt::dxPrintJ9RamAndRomMethod(struct J9Method* ramMethod)
{
    if (ramMethod == 0) {
    ...
    struct J9ROMMethod* localROMMethod
        = (J9ROMMethod*)dxMallocAndRead(sizeof(J9ROMMethod), J9_ROM_METHOD_FROM_RAM_METHOD(localRamMethod));
...
void TR_DebugExt::dxPrintMethodName(char* p, int32_t searchLimit)
{
    ...
    J9JITExceptionTable* localMetadata
        = (J9JITExceptionTable*)dxMallocAndRead(sizeof(J9JITExceptionTable), remoteMetadata);

    struct J9UTF8* localClassNameStruct = (J9UTF8*)dxMallocAndRead(sizeof(J9UTF8), localMetadata->className);
    char* className
        = (char*)dxMallocAndRead(J9UTF8_LENGTH(localClassNameStruct) + 1, (void*)J9UTF8_DATA(localMetadata->className));
    ...
    char* methodName = (char*)dxMallocAndRead(
        J9UTF8_LENGTH(localMethodNameStruct) + 1, (void*)J9UTF8_DATA(localMetadata->methodName));
    methodName[J9UTF8_LENGTH(localMethodNameStruct)] = 0;

    struct J9UTF8* localMethodSignatureStruct
        = (J9UTF8*)dxMallocAndRead(sizeof(J9UTF8), localMetadata->methodSignature);
    char* methodSignature = (char*)dxMallocAndRead(
        J9UTF8_LENGTH(localMethodSignatureStruct) + 1, (void*)J9UTF8_DATA(localMetadata->methodSignature));
    methodSignature[J9UTF8_LENGTH(localMethodSignatureStruct)] = 0;

    int32_t hotness = -1;
    bool invalidated = false;
    TR_PersistentJittedBodyInfo* localBodyInfo
        = (TR_PersistentJittedBodyInfo*)dxMalloc(sizeof(TR_PersistentJittedBodyInfo), localMetadata->bodyInfo);
    ...
    TR_LinkageInfo* linkageInfo
        = (TR_LinkageInfo*)dxMallocAndRead(sizeof(TR_LinkageInfo), (void*)((char*)localMetadata->startPC - 4));

    _dbgPrintf("\n\nMethod:\t%s.%s%s\n\n", className, methodName, methodSignature);
    dxPrintJ9RamAndRomMethod(localMetadata->ramMethod);
    _dbgPrintf("Method Hotness:\t%i = %s\n\n", hotness,
        hotness == -1 ? "unknown" : comp()->getHotnessName((TR_Hotness)hotness));
    ...
    if (localMetadata != NULL)
        dxFree(localMetadata);
    if (localClassNameStruct != NULL)
        dxFree(localClassNameStruct);
    if (localMethodNameStruct != NULL)
        dxFree(localMethodNameStruct);
    if (localMethodSignatureStruct != NULL)
        dxFree(localMethodSignatureStruct);
    if (className != NULL)
        dxFree(className);
    if (methodName != NULL)
        dxFree(methodName);
    if (methodSignature != NULL)
        dxFree(methodSignature);
    if (localBodyInfo != NULL)
        dxFree(localBodyInfo);
    if (linkageInfo != NULL)
        dxFree(linkageInfo);
 ...
 */
void TR_DebugExt::dxPrintNode(TR::Node* remoteNode)
{
    ...
    _dbgPrintf("\t\t&(GlobalRegisterInfo _globalRegisterInfo) = 0x%p\n",
        (char*)remoteNode
            + ((char*)&(localNode->_unionBase._unionedWithChildren._globalRegisterInfo) - (char*)localNode));
...
void TR_DebugExt::dxPrintAOTinfo(void* addr)
{
    ...
    TR_AOTMethodHeader* aotMethodHeaderEntry = (TR_AOTMethodHeader*)dxMallocAndRead(
        size - sizeof(J9JITDataCacheHeader), (U_8*)addr + sizeof(J9JITDataCacheHeader));
    ...
    J9JITDataCacheHeader* exceptionTableCacheEntry
        = (J9JITDataCacheHeader*)dxMallocAndRead(sizeof(J9JITDataCacheHeader) + sizeof(J9JITExceptionTable),
            (U_8*)addr + aotMethodHeaderEntry->offsetToExceptionTable);
    ...
    TR_RelocationHeader* header = (TR_RelocationHeader*)dxMallocAndRead(
        sizeof(TR_RelocationHeader), (U_8*)addr + aotMethodHeaderEntry->offsetToRelocationDataItems);
    TR_RelocationRecordHeader* firstRecord
        = (TR_RelocationRecordHeader*)dxMallocAndRead(header->size - sizeof(TR_RelocationHeader),
            (U_8*)addr + aotMethodHeaderEntry->offsetToRelocationDataItems + sizeof(TR_RelocationHeader));
    TR_RelocationRecordHeader* endRecord
        = (TR_RelocationRecordHeader*)((U_8*)firstRecord + header->size - sizeof(TR_RelocationHeader));
    ...
    _dbgPrintf("Size: %x, Header: %p, firstRecord: %p, End Record: %p, sizeof reloheader: %x\t\n", header->size, header,
        firstRecord, endRecord, sizeof(TR_RelocationHeader));
    ...
    while (reloRecord < endRecord) {
        ...
        _dbgPrintf("%20s\t",
            TR::ExternalRelocation::getName(
                (TR_ExternalRelocationTargetKind)(recordType & TR_ExternalRelocationTargetKindMask)));
        ...
        case TR_DataAddress: {
            ...
            _dbgPrintf("0x%-16x  0x%-16x  0x%-16x  0x%-16x", record->inlinedSiteIndex, record->constantPool,
                record->cpIndex, record->offset);
            ptr = (U_8*)(record + 1);
        } break;
        ...
        case TR_VerifyClassObjectForAlloc: {
            ...
            _dbgPrintf("0x%-16x  0x%-16x  0x%-16x", record->inlinedSiteIndex, record->constantPool, record->cpIndex,
                record->branchOffset, record->allocationSize);
            ptr = (U_8*)(record + 1);
        } break;
        ...
        case TR_InlinedHCRMethod: {
            ...
            _dbgPrintf("0x%-16x  0x%-16x  0x%-16x", record->inlinedSiteIndex, record->constantPool, record->cpIndex,
                record->destinationAddress, record->romClassOffsetInSharedCache);
            ptr = (U_8*)(record + 1);
        } break;
        ...
        case TR_ValidateClass: {
            ...
            _dbgPrintf("0x%-16x  0x%-16x  0x%-16x  0x%-16x", record->inlinedSiteIndex, record->constantPool,
                record->cpIndex, record->classChainOffsetInSharedCache);
            ptr = (U_8*)(record + 1);
        } break;
...
#define CODECACHE_WARM_EYE_CATCHER_DEPRECATED_DO_NOT_USE \
    {                                                    \
        '\xD1', '\xC9', '\xE3', '\xE6'                   \
    }
#define CODECACHE_COLD_EYE_CATCHER_DEPRECATED_DO_NOT_USE \
    {                                                    \
        '\xD1', '\xC9', '\xE3', '\xC3'                   \
    }
#else // ASCII
...
OMR::CodeCacheMethodHeader* TR_DebugExt::dxGet_CodeCacheMethodHeader(char* p, int32_t searchLimit)
{
    ...
    while (potentialEyeCatcher == NULL
        || (strncmp(potentialEyeCatcher, warmEyeCatcher, size) != 0
               && strncmp(potentialEyeCatcher, coldEyeCatcher, size) != 0)) {
        if (localCodeCacheMethodHeader != NULL)
            dxFree(localCodeCacheMethodHeader);
        if (sizeSearched >= searchLimit) {
            _dbgPrintf(
                "dxPrintMethodMetadata - could not find eyecatcher within search limit size of %i\n", sizeSearched);
            return NULL;
        }

        localCodeCacheMethodHeader
            = (OMR::CodeCacheMethodHeader*)dxMalloc(codeCacheMethodHeaderSize, (OMR::CodeCacheMethodHeader*)p);
    ...
    _dbgPrintf("Eye Catcher found after %i bytes: \t[0x%x]:\t%s\n", sizeSearched,
        &((OMR::CodeCacheMethodHeader*)(p + size))->_eyeCatcher,
        strncmp(potentialEyeCatcher, warmEyeCatcher, size) == 0 ? "JITW" : "JITC");
    return localCodeCacheMethodHeader;
}

const char* TR_DebugExt::dxGetSignature(J9UTF8* className, J9UTF8* name, J9UTF8* signature)
{
    ...
    sprintf(s, "%.*s.%.*s%.*s", classNameLen, J9UTF8_DATA(localClassName2), nameLen, J9UTF8_DATA(localName2), sigLen,
        J9UTF8_DATA(localSignature2));
...
const char* TR_DebugExt::getMethodName(TR::SymbolReference* symRef)
{
...
const char* TR_DebugExt::getMethodName(TR_OpaqueMethodBlock* mb)
{
    struct J9Method* localRamMethod = (J9Method*)dxMallocAndRead(sizeof(J9Method), (J9Method*)mb);
    if (!localRamMethod)
        return NULL;

    struct J9ConstantPool* localCP
        = (J9ConstantPool*)dxMallocAndRead(sizeof(J9ConstantPool), localRamMethod->constantPool);
    if (!localCP)
        return NULL;
    ...
    struct J9ROMNameAndSignature* localMethodNameAndSig = (J9ROMNameAndSignature*)dxMallocAndRead(
        sizeof(J9ROMNameAndSignature), (J9ROMNameAndSignature*)localMethodNameSigAddr);
    if (!localMethodNameAndSig)
        return NULL;

    intptrj_t localMethodNameAddr
        = (intptrj_t)(localMethodNameSigAddr + localMethodNameAndSig->name + offsetof(J9ROMNameAndSignature, name));
    intptrj_t localMethodSigAddr = (intptrj_t)(
        localMethodNameSigAddr + localMethodNameAndSig->signature + offsetof(J9ROMNameAndSignature, signature));
...
void TR_InternalFunctionsExt::fprintf(TR::FILE* file, const char* format, ...)
{
    va_list args;
