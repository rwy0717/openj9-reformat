...
public:
    ...
    TR_DebugExt(TR_InternalFunctions* f, struct J9PortLibrary* dbgextPortLib, J9JavaVM* localVM,
    ...
    virtual struct J9PortLibrary* getPortLib()
    {
        TR_ASSERT(_dbgextPortLib, "_dbgextPortLib should be initialized by dbgjit_TrInitialize().");
        return _dbgextPortLib;
    }
...
private:
    struct seenNode;
    void updateLocalPersistentMemoryFunctionPointers(
    ...
    void dxPrintDataCacheSizeBucket(
        void* p); // Can't use TR_DataCacheManager::SizeBucket without full definition of TR_DataCacheManager.
    void dxPrintDataCacheAllocation(void* p); // Same thing but for TR_DataCacheManager::Allocation
    void dxPrintDataCacheAllocationListElement(
        void* p); // TR_DataCacheManager::InPlaceList<TR_DataCacheManager::Allocation>::ListElement
    void dxPrintDataCacheSizeBucketListElement(
        void* p); // TR_DataCacheManager::InPlaceList<TR_DataCacheManager::SizeBucket>::ListElement
    ...
    void* newArray(uint32_t sizeofType, uint32_t size, void* array)
    {
        return dxMallocAndRead(sizeofType * size, array);
    }
