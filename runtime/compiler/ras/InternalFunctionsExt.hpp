...
public:
    ...
    TR_InternalFunctionsExt(
        TR::Compilation* comp, TR_FrontEnd* fe, TR_Printf_t dxPrintf, TR_Malloc_t dxMalloc, TR_Free_t dxFree)
        : TR_InternalFunctions(fe, 0, 0, comp)
        , _dxPrintf(dxPrintf)
        , _dxMalloc(dxMalloc)
        , _dxFree(dxFree)
        , _memchk(false)
    ...
    {
        if (_memchk)
            _dxPrintf("jit->free: 0x%p\n", p);
        _dxFree(p);
    }

    /* output redirection */
    virtual void fprintf(TR::FILE* file, const char* format, ...);
    virtual void fflush(TR::FILE* file)
    { /* do nothing */
        return;
    }
    ...
    virtual const char* signature(TR_ResolvedMethod* method, TR_AllocationKind allocKind = heapAlloc)
    {
        return _debug->getName(method);
    }
