...
#endif
{
        ...
        if (details) {
            ...
            if (inlinedMethod)
                TR_VerboseLog::writeLine(
                    TR_Vlog_OSRD, "%X   Inlined method: %s", (int)vmThreadArg, inlinedMethodSignature);
            ...
            for (i = osrFrame->pendingStackHeight - 1; i >= 0; --i)
                TR_VerboseLog::writeLine(
                    TR_Vlog_OSRD, "%X       stack %2d: %p", (int)vmThreadArg, i, arg0EA[-i - osrFrame->numberOfLocals]);
    ...
    if (numSymsThatShareSlot > 0) // this is a condition that depends on the current inlined method
    {
        ...
        if (numberOfMappings > 0) // this is a condition that depends on the whole compilation
        {
                ...
                if (jitPCOffset < instructionPC) {
                    if (details)
                        TR_VerboseLog::writeLineLocked(
                            TR_Vlog_OSRD, "%X   Found mapping @%d > %d", (int)vmThreadArg, instructionPC, jitPCOffset);
                    break;
                } else {
            ...
            TR_ASSERT(
                i > 0, "jitPC %x is smaller than first instructionPC, startPC=%p\n", jitPCOffset, metaData->startPC);
