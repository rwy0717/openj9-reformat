...
void TR_PersistentClassInfo::removeSubClasses()
{
    _classId = 0;
...
bool OMR::RuntimeAssumption::isAssumingRange(uintptrj_t rangeStartPC, uintptrj_t rangeEndPC,
    uintptrj_t rangeColdStartPC, uintptrj_t rangeColdEndPC, uintptrj_t rangeStartMD, uintptrj_t rangeEndMD)
{
...
bool OMR::RuntimeAssumption::isAssumingMethod(void* md, bool reclaimPrePrologueAssumptions)
{
    ...
    if (assumptionInRange((uintptrj_t)metaData, ((uintptrj_t)metaData) + ((uintptrj_t)metaData->size))) {
        ...
        if (bodyInfo && bodyInfo->getMethodInfo() && bodyInfo->getMethodInfo()->isInDataCache()) {
            ...
            if (!assumptionInRange((uintptrj_t)metaData->bodyInfo,
                    ((uintptrj_t)metaData->bodyInfo) + (uintptrj_t)sizeof(TR_PersistentJittedBodyInfo)
                        + (uintptrj_t)sizeof(TR_PersistentMethodInfo)))
                return true;
        } else {
            return true;
        }
...
void OMR::RuntimeAssumption::dequeueFromListOfAssumptionsForJittedBody()
{
    ...
    if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseRuntimeAssumptions)) {
        ...
        TR_VerboseLog::writeLine(
            TR_Vlog_RA, "Deleting %s assumption: ", runtimeAssumptionKindNames[getAssumptionKind()]);
        dumpInfo();
        TR_VerboseLog::vlogRelease();
    }
}
...
void TR_RuntimeAssumptionTable::addAssumption(
    OMR::RuntimeAssumption* a, TR_RuntimeAssumptionKind kind, TR_FrontEnd* fe, OMR::RuntimeAssumption** sentinel)
{
...
void TR_RuntimeAssumptionTable::detachFromRAT(OMR::RuntimeAssumption* assumption)
{
        ...
        if (_detachPending[kind] == true) // Is there anything to remove from this table?
        {
            ...
            for (size_t i = 0; i < hashTable->_spineArraySize; ++i) // for each bucket in the table
            {
                ...
                for (cursor = hashTable->_htSpineArray[i], prev = NULL;
                     cursor && hashTable->_markedforDetachCount[i] > 0; cursor = next) {
                    next = cursor->getNext();
...
void TR_RuntimeAssumptionTable::reclaimAssumptions(
    OMR::RuntimeAssumption** sentinel, void* metaData, bool reclaimPrePrologueAssumptions)
{
                ...
                while (cursor != sentry) {
                    ...
                    if (kind != RuntimeAssumptionOnClassRedefinitionPIC
                        ...
                        fprintf(stderr, "RA=%p kind=%d key=%p assumingPC=%p\n", cursor, cursor->getAssumptionKind(),
                            cursor->getKey(), cursor->getFirstAssumingPC());
...
void TR_RuntimeAssumptionTable::reclaimAssumptions(void* md, bool reclaimPrePrologueAssumptions)
{
    J9JITExceptionTable* metaData = (J9JITExceptionTable*)md;
    reclaimAssumptions(
        (OMR::RuntimeAssumption**)(&metaData->runtimeAssumptionList), metaData, reclaimPrePrologueAssumptions);
...
void TR_RuntimeAssumptionTable::notifyIllegalStaticFinalFieldModificationEvent(TR_FrontEnd* vm, void* key)
{
    ...
    OMR::RuntimeAssumption** headPtr
        = getBucketPtr(RuntimeAssumptionOnStaticFinalFieldModification, hashCode((uintptrj_t)key));
    OMR::RuntimeAssumption* cursor = *headPtr;
    OMR::RuntimeAssumption* prev = NULL;
    bool found = false;
...
void TR_RuntimeAssumptionTable::notifyClassUnloadEvent(
    TR_FrontEnd* vm, bool isSMP, TR_OpaqueClassBlock* assumingClass, TR_OpaqueClassBlock* unloadedClass)
{
    OMR::CriticalSection notifyClassUnloadEvent(assumptionTableMutex);
    OMR::RuntimeAssumption** headPtr
        = getBucketPtr(RuntimeAssumptionOnClassUnload, hashCode((uintptrj_t)assumingClass));
    ...
    while (cursor) {
        ...
        if (cursor->matches((uintptrj_t)assumingClass)
            && ((unloadedClass == assumingClass) || (cursor->getPicEntry() == (uintptrj_t)unloadedClass))) {
            cursor->compensate(vm, 0, 0);
            if (assumingClass == unloadedClass) {
                ...
                // TODO: replace these statements with CHTable::removeAssumptionFromList(OMR::RuntimeAssumption *&list,
                // OMR::RuntimeAssumption *assumption, OMR::RuntimeAssumption *prev);
                cursor->dequeueFromListOfAssumptionsForJittedBody();
                incReclaimedAssumptionCount(cursor->getAssumptionKind());
                cursor->paint(); // RAS
...
TR_RedefinedClassRPicSite* TR_RedefinedClassRPicSite::make(TR_FrontEnd* fe, TR_PersistentMemory* pm, uintptrj_t key,
...
TR_RedefinedClassUPicSite* TR_RedefinedClassUPicSite::make(TR_FrontEnd* fe, TR_PersistentMemory* pm, uintptrj_t key,
    uint8_t* picLocation, uint32_t size, OMR::RuntimeAssumption** sentinel)
{
...
TR_UnloadedClassPicSite* TR_UnloadedClassPicSite::make(TR_FrontEnd* fe, TR_PersistentMemory* pm, uintptrj_t key,
...
void TR_RuntimeAssumptionTable::notifyClassRedefinitionEvent(TR_FrontEnd* vm, bool isSMP, void* oldKey, void* newKey)
{
    ...
    OMR::RuntimeAssumption** oldHeadPtr
        = getBucketPtr(RuntimeAssumptionOnClassRedefinitionPIC, hashCode((uintptrj_t)oldKey));
    OMR::RuntimeAssumption** newHeadPtr
        = getBucketPtr(RuntimeAssumptionOnClassRedefinitionPIC, hashCode((uintptrj_t)newKey));
    ...
    if (reportDetails) {
        TR_VerboseLog::vlogAcquire();
        TR_VerboseLog::writeLine(
            TR_Vlog_RA, "Scanning for PIC assumptions for %p in array %p bucket %p", oldKey, raArray, oldHeadPtr);
        if (!pic_cursor)
    ...
    while (pic_cursor) {
        ...
        if (reportDetails)
            TR_VerboseLog::writeLineLocked(
                TR_Vlog_RA, "old=%p @ %p", pic_cursor->getKey(), pic_cursor->getPicLocation());
    ...
    if (reportDetails) {
        TR_VerboseLog::vlogAcquire();
        TR_VerboseLog::writeLine(
            TR_Vlog_RA, "Scanning for NOP assumptions for %p in array %p bucket %p", oldKey, raArray, oldHeadPtr);
        if (!nop_cursor)
                ...
                if ((uintptrj_t)oldKey == resolvedKey) {
                    if (reportDetails)
                        TR_VerboseLog::writeLineLocked(TR_Vlog_RA, "old=%p resolved=%p @ %p patching new=%p",
                            ...
                            *(uintptrj_t*)(pic_cursor->getPicLocation()));
        ...
        while (pic_cursor) {
            ...
            if (pic_cursor->isForAddressMaterializationSequence()) {
                ...
                if (oldKey4 == (resolvedKey4 & 0xffff) && oldKey3 == (resolvedKey3 & 0xffff)
                    ...
                    if (isAddressMaterialization) {
                        if (reportDetails)
                            TR_VerboseLog::writeLineLocked(TR_Vlog_RA, "o=%p @ %p  r=%p %p %p %p", initialKey,
                                pic_cursor->getPicLocation(), resolvedKey1, resolvedKey2, resolvedKey3, resolvedKey4);
                        *(uint32_t*)(pic_cursor->getPicLocation())
                            = resolvedKey1 & 0xffff0000 | (uint32_t)((uintptrj_t)newKey >> 48);
                        ;
                        *(uint32_t*)(pic_cursor->getPicLocation() + 4)
                            = resolvedKey2 & 0xffff0000 | (uint32_t)((uintptrj_t)newKey >> 32 & 0xffff);
                        ;
                        *(uint32_t*)(pic_cursor->getPicLocation() + 12)
                            = resolvedKey3 & 0xffff0000 | (uint32_t)((uintptrj_t)newKey >> 16 & 0xffff);
                        ;
                        *(uint32_t*)(pic_cursor->getPicLocation() + 16)
                            = resolvedKey4 & 0xffff0000 | (uint32_t)((uintptrj_t)newKey & 0xffff);
                        ;
                        if (reportDetails) {
                            TR_VerboseLog::vlogAcquire();
                            TR_VerboseLog::write(" patched n=%p %p %p %p", *(uint32_t*)(pic_cursor->getPicLocation()),
                                *(uint32_t*)(pic_cursor->getPicLocation() + 4),
                                *(uint32_t*)(pic_cursor->getPicLocation() + 12),
                                *(uint32_t*)(pic_cursor->getPicLocation() + 16));
                            TR_VerboseLog::vlogRelease();
                ...
                if (oldKey2 == (resolvedKey2 & 0xffff) && oldKey1 == (resolvedKey1 & 0xffff)) {
                    ...
                    if (isAddressMaterialization) {
                        if (reportDetails)
                            TR_VerboseLog::writeLineLocked(TR_Vlog_RA, "o=%p r=%p %p @ %p", initialKey, resolvedKey1,
                        ...
                        *(uint32_t*)(pic_cursor->getPicLocation() + 4)
                        ...
                        if (reportDetails) {
                            TR_VerboseLog::vlogAcquire();
                            TR_VerboseLog::write(" patched n=%p %p", *(uint32_t*)(pic_cursor->getPicLocation()),
                                *(uint32_t*)(pic_cursor->getPicLocation() + 4));
                            TR_VerboseLog::vlogRelease();
                        }
                    }
...
void TR_RuntimeAssumptionTable::notifyMutableCallSiteChangeEvent(TR_FrontEnd* fe, uintptrj_t cookie)
{
...
bool J9::PersistentInfo::isInUnloadedMethod(uintptrj_t address)
{
...
void J9::PersistentInfo::addUnloadedClass(TR_OpaqueClassBlock* clazz, uintptrj_t startAddress, uint32_t size)
{
    ...
    if (enableTraceDetails()) {
        ...
        for (int i = 0; i < _numAddressRanges; i += 4) {
            ...
            for (int j = 0; (j < 4) && (i + j < _numAddressRanges); j++)
                fprintf(
                    stderr, " %4d [%p - %p]", i + j, _addressRanges[i + j].getStart(), _addressRanges[i + j].getEnd());
            fprintf(stderr, "\n");
        }
    }
