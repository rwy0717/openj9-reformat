...
TR_DataCacheManager* TR_DataCacheManager::constructManager(
    J9JITConfig* jitConfig, TR::Monitor* monitor, uint32_t quantumSize, uint32_t minQuanta, bool newImplementation)
{
        ...
        if (_jitConfig->dataCacheList->totalSegmentSize < _jitConfig->dataCacheTotalKB * 1024) {
            ...
            if (dataCache) {
                ...
                J9MemorySegment* dataCacheSeg = NULL;
                {
                    ...
                    dataCacheSeg
                        = _jitConfig->javaVM->internalVMFunctions->allocateMemorySegmentInList(_jitConfig->javaVM,
                            _jitConfig->dataCacheList, segSize, MEMORY_TYPE_RAM, J9MEM_CATEGORY_JIT_DATA_CACHE);
                    if (dataCacheSeg)
                ...
                if (dataCacheSeg) {
                    ...
                    TR_VerboseLog::write(
                        "<JIT: non-fatal error: failed to allocate %d Kb data cache>\n", _jitConfig->dataCacheKB);
            ...
            {
                TR_VerboseLog::write(
                    "<JIT: non-fatal error: failed to allocate %d bytes for data cache>\n", sizeof(TR_DataCache));
                _jitConfig->runtimeFlags |= J9JIT_DATA_CACHE_FULL; // prevent future allocations
#ifdef DATA_CACHE_DEBUG
...
//----------------------------------------------------------------------------
uint8_t* TR_DataCacheManager::allocateDataCacheRecord(
    uint32_t numBytes, uint32_t allocationType, uint32_t* allocatedSizePtr)
{
    uint8_t* retValue = NULL;
    ...
    if (_newImplementation) {
        ...
        if (alloc) {
            ...
            alloc->prepareForUse();
            fillDataCacheHeader(
                static_cast<J9JITDataCacheHeader*>(static_cast<void*>(alloc)), allocationType, alloc->size());
...
TR_InstrumentedDataCacheManager::TR_InstrumentedDataCacheManager(J9JITConfig* jitConfig, TR::Monitor* monitor,
    ...
    , _allocationStatistics(
          "Bytes requested per allocation", roundToMultiple<uint32_t>(quantumSize, sizeof(uintptr_t)), 1 << 13)
