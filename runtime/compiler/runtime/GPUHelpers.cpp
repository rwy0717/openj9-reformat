        ...
        if (result != cudaSuccess) {                                                                        \
            if (tracing > 0)                                                                                \
                TR_VerboseLog::writeLine(                                                                   \
                    TR_Vlog_GPU, "[time.ms=%lld]: CUDA error %d", (int64_t)(currentTime() / 1000), result); \
            return error;                                                                                   \
        }                                                                                                   \
    }
...
#define checkNVVMError(result, tracing)                                                                     \
    {                                                                                                       \
        if (result != NVVM_SUCCESS) {                                                                       \
            if (tracing > 0)                                                                                \
                TR_VerboseLog::writeLine(                                                                   \
                    TR_Vlog_GPU, "[time.ms=%lld]: NVVM error %d", (int64_t)(currentTime() / 1000), result); \
            return NULL;                                                                                    \
        }                                                                                                   \
    }
...
static void saveSignalHandlers(struct sigaction sigactionArray[], bool trace)
{
    for (int i = 1; i <= 31; i++) {
        ...
        {
            if (trace)
                TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tsigaction error: Can't read sigaction for signal %d\n", i);
        }
    }
}
...
static void restoreSignalHandlers(struct sigaction sigactionArray[], bool trace)
{
    for (int i = 1; i <= 31; i++) {
        ...
        {
            if (trace)
                TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tsigaction error: Can't set sigaction for signal %d\n", i);
        }
    }
}
#endif
...
static bool checkDlError(int tracing, bool isNull)
{
    ...
    if (errorMessage) {
        if (tracing > 0)
            TR_VerboseLog::writeLine(TR_Vlog_GPU, "\tDynamic linking error: %s\n", errorMessage);
        return true;
    }
#endif
    ...
    if (isNull) {
        if (tracing > 0)
            TR_VerboseLog::writeLine(
                TR_Vlog_GPU, "\tDynamic linking error: null pointer while loading shared library\n");
        return true;
    }
...
static bool loadNVVMlibrary(int tracing)
{
    ...
    if (tracing > 0 && !libNvvmPointer)
        TR_VerboseLog::writeLine(
            TR_Vlog_GPU, "\tDynamic linking error: Unable to locate NVVM library nvvm64_30_0.dll or nvvm64_20_0.dll\n");
#endif
...
static bool loadCudaRuntimeLibrary(int tracing)
{
    ...
    if (tracing > 0 && !libCudartPointer)
        TR_VerboseLog::writeLine(
            TR_Vlog_GPU, "\tDynamic linking error: Unable to locate Cuda Runtime library cudart64_75.dll\n");
#endif
...
static bool loadCudaLibrary(int tracing)
{
    ...
    if (tracing > 0 && !libCudaPointer)
        TR_VerboseLog::writeLine(
            TR_Vlog_GPU, "\tDynamic linking error: Unable to locate Cuda Driver library nvcuda.dll\n");
#endif
    ...
    jitCuLinkCreate = (CUresult(*)(unsigned int, CUjit_option*, void**, CUlinkState*))GET_FUNCTION(
        libCudaPointer, stringMacro(cuLinkCreate));
    ...
    jitCuModuleGetFunction = (CUresult(*)(CUfunction*, CUmodule, const char*))GET_FUNCTION(
        libCudaPointer, stringMacro(cuModuleGetFunction));
    ...
    jitCuMemcpyHtoDAsync = (CUresult(*)(CUdeviceptr, const void*, size_t, CUstream))GET_FUNCTION(
        libCudaPointer, stringMacro(cuMemcpyHtoDAsync));
    if (checkDlError(tracing, !jitCuMemcpyHtoDAsync))
        return false;

    jitCuLaunchKernel = (CUresult(*)(CUfunction, unsigned int, unsigned int, unsigned int, unsigned int, unsigned int,
        unsigned int, unsigned int, CUstream, void**, void**))GET_FUNCTION(libCudaPointer, stringMacro(cuLaunchKernel));
    ...
    jitCuMemcpyDtoHAsync = (CUresult(*)(void*, CUdeviceptr, size_t, CUstream))GET_FUNCTION(
        libCudaPointer, stringMacro(cuMemcpyDtoHAsync));
...
static bool loadNVMLLibrary(int tracing)
{
    ...
    jitNvmlDeviceGetHandleByIndex = (nvmlReturn_t(*)(unsigned int, nvmlDevice_t*))GET_FUNCTION(
        libNvmlPointer, stringMacro(nvmlDeviceGetHandleByIndex));
    if (checkDlError(tracing, !jitNvmlDeviceGetHandleByIndex))
        return false;

    jitNvmlDeviceGetUtilizationRates = (nvmlReturn_t(*)(nvmlDevice_t, nvmlUtilization_t*))GET_FUNCTION(
        libNvmlPointer, stringMacro(nvmlDeviceGetUtilizationRates));
    if (checkDlError(tracing, !jitNvmlDeviceGetUtilizationRates))
        return false;

    jitNvmlDeviceGetMemoryInfo = (nvmlReturn_t(*)(nvmlDevice_t, nvmlMemory_t*))GET_FUNCTION(
        libNvmlPointer, stringMacro(nvmlDeviceGetMemoryInfo));
...
int getGpuDeviceCount(TR::PersistentInfo* persistentInfo, int tracing)
{
    ...
    for (int deviceId = 0; deviceId < deviceLoopLimit; deviceId++) {
        cudaDeviceProp prop[3]; // workaround to avoid memory collapsion since recent CUDA version requires large size
        ...
        if (deviceCCs[deviceId] != deviceCCs[0]) {
            if (tracing > 1)
                TR_VerboseLog::writeLine(
                    TR_Vlog_GPU, "GPU compute capability mismatch detected. Disabling multi GPU use");
            enableMultiGPU
...
static HashEntry* cudaInfoHashPut(
    CudaInfo* cudaInfo, void** hostref, int32_t elementSize, CUdeviceptr deviceArray, uint32_t accessMode)
{
    ...
    if (i == cudaInfo->hashSize) {
        ...
        if (!hashTable) {
            ...
            returnOnTrue(
                captureJITError("cudaInfoHashPut - out of JIT persistent memory", cudaInfo, detailsTrace), NULL);
        }
...
static int cudaModuleLoadData(
    CUlinkState* lState, CudaInfo* cudaInfo, const char* ptxSource, bool writeCUBIN, bool writeLINKER)
{
    ...
    returnOnTrue(captureCuError(jitCuLinkCreate(OPTIONSNUM, optionCmds, optionVals, lState), cudaInfo,
        ...
        ERROR_CODE);
    returnOnTrue(
        captureCuError(jitCuLinkAddData(*lState, CU_JIT_INPUT_PTX, (void*)ptxSource, strlen(ptxSource) + 1, 0, 0, 0, 0),
...
static CudaInfo* initCUDA(
    J9VMThread* vmThread, int tracing, const char* ptxSource, int deviceId, int ptxSourceID, GpuMetaData* gpuMetaData)
{
    ...
    if (trace)
        TR_VerboseLog::writeLine(
            TR_Vlog_GPU, "\tcontext setup Time: %.3f msec.", (contextSetupTime - deviceSetTime) / 1000);
...
static bool addlibDeviceToProgram(nvvmProgram program, int computeArch, bool trace, TR_Memory* m)
{
    ...
    if (jitNvvmAddModuleToProgram(program, buffer, size, libDevicePath) != NVVM_SUCCESS) {
        if (trace)
            TR_VerboseLog::writeLine(
                TR_Vlog_GPU, "\tFAILED to add the module %s to the compilation unit.", libDevicePath);
        return false;
    }
...
bool calculateComputeCapability(int tracing, short* computeMajor, short* computeMinor, int deviceId)
{
    ...
    } else // Compute Capability unknown
    {
        if (detailsTrace)
            TR_VerboseLog::writeLine(
                TR_Vlog_GPU, "\tUnknown Compute Capability. Compute Capability must be 2.0 or higher", major, minor);
        return false;
    }
...
#define OPTIONLENGTH 6
    ...
    char optionStr[OPTIONLENGTH][24]
    ...
    if (enableMath) {
        ...
        if (detailsTrace)
            TR_VerboseLog::writeLine(
                TR_Vlog_GPU, "\tAdding libdevice : %6.3f msec", (double)(end - start) * 1000 / CLOCKS_PER_SEC);
    }
...
static int selectGPUDevice(J9VMThread* vmThread, int tracing, unsigned long long* availableMemory, bool enforceGPU)
{
    ...
    if (threadGPUInfo == NULL) {
        ...
        if (threadGPUInfo == NULL) {
            if (tracing > 1)
                TR_VerboseLog::writeLine(
                    TR_Vlog_GPU, "[%16p] GPUdevice is not selected due to null threadGPUInfo", vmThread);
            return 0;
        }
    }
    ...
    for (uint32_t i = 0; i < deviceCount; i++) {
        ...
        if (NVML_SUCCESS != result) {
            if (tracing > 1)
                TR_VerboseLog::writeLine(
                    TR_Vlog_GPU, "Failed to get handle for device %d: %s", devId, jitNvmlErrorString(result));
            return 0;
        }
        ...
        if (NVML_SUCCESS != result) {
            if (tracing > 1)
                TR_VerboseLog::writeLine(
                    TR_Vlog_GPU, "Failed to get GPU utilization for device %d: %s", devId, jitNvmlErrorString(result));
            return 0;
        }
...
extern "C" CudaInfo* regionEntryGPU(
    int trace, int ptxSourceID, uint8_t* startPC, TR::CodeGenerator::GPUScopeType regionType, bool enforceGPU)
{
    if (trace > 1)
...
bool freeGPUScope(CudaInfo* cudaInfo)
{
    ...
    if (cudaInfo->exceptionPointer != NULL_DEVICE_PTR) {
        if (tracing > 1) {
            TR_VerboseLog::writeLine(
                TR_Vlog_GPU, "\tFree GPU Exception memory %p (%p)", cudaInfo->exceptionPointer, cudaInfo);
        }
...
extern "C" int regionExitGPU(CudaInfo* cudaInfo, uint8_t* startPC, int ptxSourceID, int flushBlock, void** liveRef)
{
    ...
    void* endAddressDtoH)
{
    ...
    if (doAllocate) {
        ...
        if (tracing > 1)
            TR_VerboseLog::writeLine(
                TR_Vlog_GPU, "\tallocated %d bytes at %p (%p)", allocNumBytes, deviceArray, cudaInfo);
        ...
        if (isPinned) {
            ...
            if (result == CUDA_SUCCESS)
                ...
                if (tracing > 0)
                    TR_VerboseLog::writeLine(
                        TR_Vlog_GPU, "FAILED with CUDA error : CUresult = %d (%p)", result, cudaInfo);
    ...
    if (doCopy) {
        ...
        if (offset == 0) {
            ...
            result = !disableAsyncOps
                ? jitCuMemcpyHtoDAsync(
                      deviceArray + (isalign ? HEADER_PADDING : 0), (char*)*hostRef, transNumBytes, stream)
                : jitCuMemcpyHtoD(deviceArray + (isalign ? HEADER_PADDING : 0), (char*)*hostRef, transNumBytes);
        } else {
            // copy header
            result = !disableAsyncOps
                ? jitCuMemcpyHtoDAsync(
                      deviceArray + (isalign ? HEADER_PADDING : 0), (char*)*hostRef, OBJECT_HEADER_SIZE, stream)
    ...
    int32_t isNoCopyDtoH, void* startAddress, void* endAddress, bool forceCopy)
{
    ...
    if (hostRef == NULL) {
        ...
        returnOnTrue(captureJITError("copyGPUtoHost - host array pointer is NULL", cudaInfo, tracing > 1),
            NULL_DEVICE_PTR); // TODO: better handling helper error conditions
    }
    ...
    if (parm_entry == NULL) {
        returnOnTrue(
            captureJITError("copyGPUtoHost - hashtable entry doesn't exist", cudaInfo, tracing > 1), NULL_DEVICE_PTR);
    }
    ...
    if (doCopy) {
        ...
        CUresult result = !disableAsyncOps
            ? jitCuMemcpyDtoHAsync(
                  (char*)*hostRef + offset, deviceArray + offset + (isalign ? HEADER_PADDING : 0), numBytes, stream)
            : jitCuMemcpyDtoH(
                  (char*)*hostRef + offset, deviceArray + offset + (isalign ? HEADER_PADDING : 0), numBytes);
        returnOnTrue(
            captureCuError(result, cudaInfo, "copyGPUtoHost - jitCuMemcpyDtoH"), TR::CodeGenerator::GPUHelperError);
...
extern "C" void** allocateGPUKernelParms(int trace, int argCount)
{
    ...
    if (!args) {
        ...
        if (trace > 1)
            TR_VerboseLog::writeLine(
                TR_Vlog_GPU, "\tAllocated %d parms + %d parms for GPU args array", argCount, NUMEXTRAPARMS);
    }
