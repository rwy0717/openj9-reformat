...
int32_t TR_HWProfiler::IAHash(uintptrj_t pc) { return (int32_t)(((pc >> 1) & 0x7FFFFFFF) % HASH_TABLE_SIZE); }

uintptrj_t TR_HWProfiler::getPCFromMethodAndBCIndex(
...
static int32_t J9THREAD_PROC hwProfilerThreadProc(void* entryarg)
{
    ...
    if (vm->javaOffloadSwitchOffNoEnvWithReasonFunc != 0)
        (*vm->javaOffloadSwitchOffNoEnvWithReasonFunc)(
            vm, j9thread_self(), J9_JNI_OFFLOAD_SWITCH_JIT_HARDWARE_PROFILER_THREAD);
#endif
...
U_8* TR_HWProfiler::swapBufferToWorkingQueue(
    const U_8* dataStart, UDATA size, UDATA bufferFilledSize, uint32_t dataTag, bool allocateNewBuffer)
{
    if (!_hwProfilerMonitor)
        return NULL;
            ...
            < newRecompDecisionsTotal) // avoid float arithmetic
        {
            ...
            if (TR::Options::isAnyVerboseOptionSet(TR_VerboseHWProfiler, TR_VerbosePerformance)) {
                ...
                TR_VerboseLog::writeLineLocked(TR_Vlog_HWPROFILER,
                    "t=%6u RI buffer processing disabled because recomp frequency is %.4f "
                    "newRecompDecisionsTotal=%llu\n",
...
bool TR_HWProfiler::checkAndTurnBufferProcessingOn()
{
    ...
    } else // Must look at the number of downgraded compilations since we turned RI off
    {
        if (_numDowngradesSinceTurnedOff > TR::Options::_numDowngradesToTurnRION) {
            ...
            if (TR::Options::isAnyVerboseOptionSet(TR_VerboseHWProfiler, TR_VerbosePerformance)) {
                TR_VerboseLog::writeLineLocked(TR_Vlog_HWPROFILER,
                    "RI buffer processing re-enabled because we downgraded %d methods at cold since RI was turned "
                    "off\n",
...
uintptrj_t TR_HWProfiler::getPCFromBCInfo(TR::Node* node, TR::Compilation* comp)
{
...
TR_HWPBytecodePCToIAMap TR_HWProfiler::createBCMap(
    uint8_t* ia, uint32_t bcIndex, TR_OpaqueMethodBlock* method, TR::Compilation* comp)
{
...
uintptrj_t TR_HWProfiler::getBytecodePCFromIA(J9VMThread* vmThread, uint8_t* IA)
{
    if (vmThread) {
    ...
    for (int32_t i = 0; i < hwpInstructionInfos->size(); i++) {
        ...
        if (!TR::Options::getCmdLineOptions()->getOption(TR_HWProfilerDisableAOT)
            ...
            TR::ExternalRelocation* relocation
                = new (comp->trHeapMemory()) TR::ExternalRelocation(ia, target, target2, relocationTargetKind, cg);
