...
void dispatchUnloadHooks(J9JITConfig* jitConfig, J9VMThread* vmThread, J9JITExceptionTable* metaData)
{
    ...
    if (J9_EVENT_IS_HOOKED(vm->hookInterface, J9HOOK_VM_DYNAMIC_CODE_UNLOAD)) {
        ALWAYS_TRIGGER_J9HOOK_VM_DYNAMIC_CODE_UNLOAD(
            vm->hookInterface, vmThread, metaData->ramMethod, (U_8*)metaData->startPC);

        if (metaData->startColdPC)
            ALWAYS_TRIGGER_J9HOOK_VM_DYNAMIC_CODE_UNLOAD(
                vm->hookInterface, vmThread, metaData->ramMethod, (U_8*)metaData->startColdPC);
        ...
        if (ccMethodHeader && (metaData->bodyInfo != NULL)) {
            ...
            if (linkageInfo->isRecompMethodBody()) {
                ALWAYS_TRIGGER_J9HOOK_VM_DYNAMIC_CODE_UNLOAD(
                    vm->hookInterface, vmThread, metaData->ramMethod, (U_8*)((char*)ccMethodHeader->_eyeCatcher + 4));
            }
        }
    }
}
...
inline void reclaimAssumptions(
    J9JITConfig* jitConfig, J9JITExceptionTable* metaData, bool reclaimPrePrologueAssumptions)
{
...
inline void markAssumptionsAndDetach(
    J9JITConfig* jitConfig, J9JITExceptionTable* metaData, bool reclaimPrePrologueAssumptions)
{
...
inline void updateExceptionTableLinkedList(
    J9VMThread* vmThread, J9JITExceptionTable* oldMetadata, J9JITExceptionTable* newMetadata)
{
...
inline J9JITExceptionTable* createStubExceptionTable(J9VMThread* vmThread, J9JITExceptionTable* metaData)
{
    ...
    if (!TR::Options::getCmdLineOptions()->getOption(TR_DisableMetadataReclamation)) {
        ...
        uint32_t size = 0;
        stubMetadata = (J9JITExceptionTable*)TR_DataCacheManager::getManager()->allocateDataCacheRecord(
            numBytes, J9_JIT_DCE_EXCEPTION_INFO, &size);
...
void jitReleaseCodeCollectMetaData(
    J9JITConfig* jitConfig, J9VMThread* vmThread, J9JITExceptionTable* metaData, OMR::FaintCacheBlock* faintCacheBlock)
{
    ...
    if (!TR::Options::getCmdLineOptions()->getOption(TR_DisableCodeCacheReclamation)
        ...
        if (faintCacheBlock) {
            ...
            TR::CodeCacheManager::instance()->freeFaintCacheBlock(
                faintCacheBlock, (uint8_t*)faintCacheBlock->_metaData->startPC);
            ...
            TR::CodeCacheManager::instance()->addFreeBlock(
                reinterpret_cast<void*>(metaData), reinterpret_cast<uint8_t*>(metaData->startPC));
            freeExistingExceptionTable = true;
        }
...
void vlogReclamation(const char* prefix, J9JITExceptionTable* metaData, size_t bytesToSaveAtStart)
{
    if (TR::Options::getVerboseOption(TR_VerboseReclamation)) {
        ...
        TR_VerboseLog::writeLine(TR_Vlog_RECLAMATION, "%s %.*s.%.*s%.*s @ %s [" POINTER_PRINTF_FORMAT "-", prefix,
        ...
        if (metaData->startColdPC) {
            TR_VerboseLog::write(
                POINTER_PRINTF_FORMAT "] & [" POINTER_PRINTF_FORMAT "-", metaData->endWarmPC, metaData->startColdPC);
        }

        TR_VerboseLog::write(POINTER_PRINTF_FORMAT "]", metaData->endPC);
