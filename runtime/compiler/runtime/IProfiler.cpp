...
static void printHashedCallSite(TR_IPHashedCallSite* hcs, ::FILE* fout = stderr, void* tag = NULL)
{
...
uintptrj_t TR_IProfiler::createBalancedBST(uintptrj_t* pcEntries, int32_t low, int32_t high, uintptrj_t memChunk,
    TR::Compilation* comp, uintptrj_t cacheStartAddress, uintptrj_t cacheSize)
{
    ...
    uintptrj_t leftChild
        = createBalancedBST(pcEntries, low, middle - 1, memChunk + bytes, comp, cacheStartAddress, cacheSize);
    ...
    uintptrj_t rightChild = createBalancedBST(
        pcEntries, middle + 1, high, memChunk + bytes + leftChild, comp, cacheStartAddress, cacheSize);
    if (rightChild) {
...
uint32_t TR_IProfiler::walkILTreeForEntries(uintptrj_t* pcEntries, uint32_t& numEntries,
    TR_J9ByteCodeIterator* bcIterator, TR_OpaqueMethodBlock* method, TR::Compilation* comp, uintptrj_t cacheOffset,
    uintptrj_t cacheSize, vcount_t visitCount, int32_t callerIndex, TR_BitVector* BCvisit, bool& abort)
{
    abort = false;
...
void TR_IProfiler::persistIprofileInfo(
    TR::ResolvedMethodSymbol* resolvedMethodSymbol, TR_ResolvedMethod* resolvedMethod, TR::Compilation* comp)
{
    ...
    if (TR::Options::sharedClassCache() // shared classes must be enabled
        && !comp->getOption(TR_DisablePersistIProfile) && isIProfilingEnabled()
        ...
        if (doit
            && _compInfo->reloRuntime()->isROMClassInSharedCaches(
                   (uintptrj_t)romMethod, _compInfo->getJITConfig()->javaVM)) {
            TR_ASSERT(_compInfo->reloRuntime()->isROMClassInSharedCaches(
                          (uintptrj_t)methodStart, _compInfo->getJITConfig()->javaVM),
            ...
            if (!found) {
                ...
                uintptrj_t* pcEntries = (uintptrj_t*)comp->trMemory()->allocateMemory(
                    sizeof(uintptrj_t) * bci.maxByteCodeIndex(), stackAlloc);
                bytesFootprint += walkILTreeForEntries(
                    pcEntries, numEntries, &bci, method, comp, cacheOffset, cacheSize, visitCount, -1, BCvisit, abort);
                ...
                if (numEntries && !abort) {
                    ...
                    if (!SCfull) {
                        ...
                        intptrj_t bytes = createBalancedBST(
                            pcEntries, 0, numEntries - 1, (uintptrj_t)memChunk, comp, cacheOffset, cacheSize);
...
TR_IProfiler::TR_IProfiler(J9JITConfig* jitConfig)
    ...
    , _iprofilerThreadExitFlag(0)
    , _iprofilerMonitor(NULL)
    , _crtProfilingBuffer(NULL)
    , _iprofilerThreadAttachAttempted(false)
    , _iprofilerNumRecords(0)
{
    ...
    // bytecode hashtable
    _bcHashTable
        = (TR_IPBytecodeHashTableEntry**)jitPersistentAlloc(BC_HASH_TABLE_SIZE * sizeof(TR_IPBytecodeHashTableEntry*));
    ...
    _allocHashTable
        = (TR_IPBCDataAllocation**)jitPersistentAlloc(ALLOC_HASH_TABLE_SIZE * sizeof(TR_IPBCDataAllocation*));
    if (_allocHashTable != NULL)
        memset(_allocHashTable, 0, ALLOC_HASH_TABLE_SIZE * sizeof(TR_IPBCDataAllocation*));
#endif
    _methodHashTable = (TR_IPMethodHashTableEntry**)jitPersistentAlloc(
        IPMETHOD_HASH_TABLE_SIZE * sizeof(TR_IPMethodHashTableEntry*));
    ...
    _readSampleRequestsHistory
        = (TR_ReadSampleRequestsHistory*)jitPersistentAlloc(sizeof(TR_ReadSampleRequestsHistory));
...
inline int32_t TR_IProfiler::bcHash(uintptrj_t pc) { return (int32_t)((pc & 0x7FFFFFFF) % BC_HASH_TABLE_SIZE); }

inline int32_t TR_IProfiler::allocHash(uintptrj_t pc) { return (int32_t)((pc & 0x7FFFFFFF) % ALLOC_HASH_TABLE_SIZE); }

inline int32_t TR_IProfiler::methodHash(uintptrj_t data)
{
...
bool TR_IProfiler::addSampleData(TR_IPBytecodeHashTableEntry* entry, uintptrj_t data, bool isRIData, uint32_t freq)
{
...
TR_IPBytecodeHashTableEntry* TR_IProfiler::searchForSample(uintptrj_t pc, int32_t bucket)
{
...
TR_IPBCDataAllocation* TR_IProfiler::searchForAllocSample(uintptrj_t pc, int32_t bucket)
{
#if defined(EXPERIMENTAL_IPROFILER)
...
TR_IPBytecodeHashTableEntry* TR_IProfiler::findOrCreateEntry(int32_t bucket, uintptrj_t pc, bool addIt)
{
...
TR_IPBCDataAllocation* TR_IProfiler::findOrCreateAllocEntry(int32_t bucket, uintptrj_t pc, bool addIt)
{
#if defined(EXPERIMENTAL_IPROFILER)
...
TR_IPMethodHashTableEntry* TR_IProfiler::findOrCreateMethodEntry(
    J9Method* callerMethod, J9Method* calleeMethod, bool addIt, uint32_t pcIndex)
{
...
TR_IPBCDataStorageHeader* TR_IProfiler::searchForPersistentSample(TR_IPBCDataStorageHeader* root, uintptrj_t pc)
{
...
TR_IPBytecodeHashTableEntry* TR_IProfiler::persistentProfilingSample(
    TR_OpaqueMethodBlock* method, uint32_t byteCodeIndex, TR::Compilation* comp, bool* methodProfileExistsInSCC)
{
    if (TR::Options::sharedClassCache()) // shared classes must be enabled
    {
        J9SharedClassConfig* scConfig = _compInfo->getJITConfig()->javaVM->sharedClassConfig;
...
TR_IPBytecodeHashTableEntry* TR_IProfiler::profilingSample(
    TR_OpaqueMethodBlock* method, uint32_t byteCodeIndex, TR::Compilation* comp, uintptrj_t data, bool addIt)
{
    ...
    if (!addIt) // read request
    {
        ...
        if (entry) // bytecode pc is present in the hashtable
        {
            ...
            if (bytecode != JBinvokestatic && bytecode != JBinvokespecial && bytecode != JBinvokestaticsplit
                && bytecode != JBinvokespecialsplit)
        ...
        */
        class EntryPlaceHolderClass {
            char buffer[MAX_THREE(
                            sizeof(TR_IPBCDataFourBytes), sizeof(TR_IPBCDataEightWords), sizeof(TR_IPBCDataCallGraph))
                + sizeof(void*)];
        };
        ...
        if (!comp->getOption(TR_DoNotUsePersistentIprofiler)) {
            ...
            if (preferHashtableData) {
                ...
                if (!entry) // Desired PC is not in SCC
                {
                    ...
                    if (bytecode != JBinvokestatic && bytecode != JBinvokespecial && bytecode != JBinvokestaticsplit
                        && bytecode != JBinvokespecialsplit && !methodProfileExistsInSCC) {
                ...
                if (persistentEntryStore) {
                    persistentEntryStore = persistentProfilingSample(
                        method, byteCodeIndex, comp, &methodProfileExistsInSCC, &cacheOffset, persistentEntryStore);
                }
                if (!persistentEntryStore) {
...
int32_t TR_IProfiler::getSamplingCount(TR_IPBytecodeHashTableEntry* entry, TR::Compilation* comp)
{
...
int32_t TR_IPBCDataEightWords::getSumSwitchCount()
{
    int32_t sum = 1;
...
TR_IPBytecodeHashTableEntry* TR_IProfiler::profilingSample(
    uintptrj_t pc, uintptrj_t data, bool addIt, bool isRIData, uint32_t freq)
{
...
TR_IPBytecodeHashTableEntry* TR_IProfiler::profilingSampleRI(uintptrj_t pc, uintptrj_t data, bool addIt, uint32_t freq)
{
    return profilingSample(pc, data, addIt, true, freq);
}

TR_IPBCDataAllocation* TR_IProfiler::profilingAllocSample(uintptrj_t pc, uintptrj_t data, bool addIt)
{
#if defined(EXPERIMENTAL_IPROFILER)
    TR_IPBCDataAllocation* entry = NULL;
...
static TR_OpaqueMethodBlock* getMethodFromBCInfo(TR_ByteCodeInfo& bcInfo, TR::Compilation* comp)
{
    ...
    if (comp->fej9()->isAOT_DEPRECATED_DO_NOT_USE()) {
        if (bcInfo.getCallerIndex() >= 0)
            method = (TR_OpaqueMethodBlock*)((
                (TR_AOTMethodInfo*)comp->getInlinedCallSite(bcInfo.getCallerIndex())._vmMethodInfo)
                                                 ->resolvedMethod->getNonPersistentIdentifier());
        else
...
uintptrj_t TR_IProfiler::getProfilingData(TR::Node* node, TR::Compilation* comp)
{
    if (!isIProfilingEnabled())
        return 0;
...
uintptrj_t TR_IProfiler::getProfilingData(TR_ByteCodeInfo& bcInfo, TR::Compilation* comp)
{
    if (!isIProfilingEnabled())
        return 0;
...
uintptrj_t getSearchPCFromMethodAndBCIndex(
    TR_OpaqueMethodBlock* method, uint32_t byteCodeIndex, TR_FrontEnd* vm, TR::Compilation* comp)

{
    ...
    static bool traceIProfiling = ((debug("traceIProfiling") != NULL));
    if (isInterfaceBytecode(*(U_8*)searchedPC) && byteCodeIndex >= 2 && isInterface2Bytecode(*(U_8*)(searchedPC - 2))) {
        searchedPC -= 2;
        if (traceIProfiling)
...
uintptrj_t getSearchPCFromMethodAndBCIndex(TR_OpaqueMethodBlock* method, uint32_t byteCodeIndex, TR::Compilation* comp)
{
    return getSearchPCFromMethodAndBCIndex(method, byteCodeIndex, comp->fej9(), comp);
}

uintptrj_t TR_IProfiler::getSearchPC(TR_OpaqueMethodBlock* method, uint32_t byteCodeIndex, TR::Compilation* comp)
{
...
uint32_t* TR_IProfiler::getAllocationProfilingDataPointer(
...
TR_IPBytecodeHashTableEntry* TR_IProfiler::getProfilingEntry(
    TR_OpaqueMethodBlock* method, uint32_t byteCodeIndex, TR::Compilation* comp)
{
...
uintptrj_t TR_IProfiler::getProfilingData(TR_OpaqueMethodBlock* method, uint32_t byteCodeIndex, TR::Compilation* comp)
{
...
int32_t TR_IProfiler::getMaxCount(bool isAOT)
{
    if (!isIProfilingEnabled())
        return 0;
...
int32_t TR_IProfiler::getSwitchCountForValue(TR::Node* node, int32_t value, TR::Compilation* comp)
{
    ...
    if (entry && entry->asIPBCDataEightWords()) {
        ...
        return getOrSetSwitchData(
            (TR_IPBCDataEightWords*)entry, (uint32_t)data, false, (node->getOpCodeValue() == TR::lookup));
    }
    return 0;
}
...
void TR_IProfiler::getBranchCounters(
    TR::Node* node, TR::TreeTop* fallThroughTree, int32_t* taken, int32_t* notTaken, TR::Compilation* comp)
{
    static bool traceIProfiling = ((debug("traceIProfiling") != NULL));
    ...
    if (data) {
        ...
        if (branchHasSameDirection(nodeOpCode, node, comp)) {
            ...
            *taken = (int32_t)fallThroughCount;
            ...
            if (traceIProfiling)
                traceMsg(comp,
                    "I couldn't figure out the branch direction after change for node [%p], so I gave default "
                    "direction \n",
                    node);
            *taken = (int32_t)branchToCount;
            *notTaken = (int32_t)fallThroughCount;
...
void TR_IProfiler::resetProfiler() { _maxCount = DEFAULT_PROFILING_COUNT; }

J9Class* TR_IProfiler::getInterfaceClass(J9Method* aMethod, TR::Compilation* comp)
{
       ...
       !bcInfo.doNotProfile()*/)
      {
            ...
            && !_compInfo->isCompiled((J9Method*)method)) // method is interpreted
        {
            ...
            if ((currentPersistentClassInfo->getTimeStamp() == (uint16_t)0x0FFFF)
                ...
                if (traceIProfiling) {
                    traceMsg(comp,
                        "\nThe time stamp for callee or caller class has expired, I refuse to give profiling "
                        "information back\n");
                }
                //#ifdef VERBOSE_INLINING_PROFILING
            ...
            if ((currentPersistentClassInfo->getTimeStamp() > calleePersistentClassInfo->getTimeStamp())
                ...
                if (traceIProfiling) {
                    traceMsg(comp,
                        ...
                        "refuse to give profiling info for this callee method (ownerClass time stamp %d, callee class "
                        "time stamp %d).\n",
            ...
            if (traceIProfiling) {
                traceMsg(comp,
                    "\nCallee method %s (callerIndex=%d) is interpreted I'll give profiling information for it, "
                    "ownerClass time stamp %d, callee class time stamp %d.\n",
                    _vm->sampleSignature(method, 0, 0, comp->trMemory()), bcInfo.getCallerIndex(),
                    currentPersistentClassInfo->getTimeStamp(), calleePersistentClassInfo->getTimeStamp());
            }
        }
            ...
            if (csInfo) {
                ...
                for (int32_t i = 1; i < NUM_CS_SLOTS; i++) {
                    ...
                    if (data) {
                        ...
                        list->incrementOrCreate(
                            address, &addrOfTotalFrequency, i, weight, &comp->trMemory()->heapMemoryRegion());
                    }
                }
...
bool TR_IProfiler::hasSameBytecodeInfo(
...
void* TR_IPBytecodeHashTableEntry::alignedPersistentAlloc(size_t size)
{
...
void* TR_IPBCDataCallGraph::operator new(size_t size) throw()
{
    return TR_IPBytecodeHashTableEntry::alignedPersistentAlloc(size);
}

void* TR_IPBCDataFourBytes::operator new(size_t size) throw()
{
    return TR_IPBytecodeHashTableEntry::alignedPersistentAlloc(size);
}

void TR_IPBCDataFourBytes::createPersistentCopy(
    uintptrj_t cacheStartAddress, uintptrj_t cacheSize, TR_IPBCDataStorageHeader* storage, TR::PersistentInfo* info)
{
...
void TR_IPBCDataFourBytes::loadFromPersistentCopy(
    TR_IPBCDataStorageHeader* storage, TR::Compilation* comp, uintptrj_t cacheStartAddress)
{
    TR_IPBCDataFourBytesStorage* store = (TR_IPBCDataFourBytesStorage*)storage;
...
void TR_IPBCDataFourBytes::copyFromEntry(TR_IPBytecodeHashTableEntry* originalEntry, TR::Compilation* comp)
{
...
int16_t TR_IPBCDataFourBytes::getSumBranchCount()
{
    uint16_t fallThroughCount = (uint16_t)(data & 0x0000FFFF) | 0x1;
    uint16_t branchToCount = (uint16_t)((data & 0xFFFF0000) >> 16) | 0x1;
...
void* TR_IPBCDataAllocation::operator new(size_t size) throw()
{
    return TR_IPBytecodeHashTableEntry::alignedPersistentAlloc(size);
}

void* TR_IPBCDataEightWords::operator new(size_t size) throw()
{
    return TR_IPBytecodeHashTableEntry::alignedPersistentAlloc(size);
}

void TR_IPBCDataEightWords::createPersistentCopy(
    uintptrj_t cacheStartAddress, uintptrj_t cacheSize, TR_IPBCDataStorageHeader* storage, TR::PersistentInfo* info)
{
...
void TR_IPBCDataEightWords::loadFromPersistentCopy(
    TR_IPBCDataStorageHeader* storage, TR::Compilation* comp, uintptrj_t cacheStartAddress)
{
    TR_IPBCDataEightWordsStorage* store = (TR_IPBCDataEightWordsStorage*)storage;
...
void TR_IPBCDataEightWords::copyFromEntry(TR_IPBytecodeHashTableEntry* originalEntry, TR::Compilation* comp)
{
...
int32_t TR_IPBCDataCallGraph::setData(uintptrj_t v, uint32_t freq)
{
...
int32_t TR_IPBCDataCallGraph::getSumCount(TR::Compilation* comp, bool)
{
    ...
    for (int32_t i = 0; i < NUM_CS_SLOTS; i++) {
        if (debug) {
            ...
            const char* s = _csInfo.getClazz(i)
                ...
                : "0";
            fprintf(
                stderr, "[%p] slot %d, class %p %s, weight %d : ", this, i, _csInfo.getClazz(i), s, _csInfo._weight[i]);
            fflush(stderr);
        }
        sumWeight += _csInfo._weight[i];
    }
...
uintptrj_t TR_IPBCDataCallGraph::getData(TR::Compilation* comp)
{
...
uint32_t TR_IPBCDataCallGraph::canBePersisted(
    uintptrj_t cacheStartAddress, uintptrj_t cacheSize, TR::PersistentInfo* info)
{
...
void TR_IPBCDataCallGraph::createPersistentCopy(
    uintptrj_t cacheStartAddress, uintptrj_t cacheSize, TR_IPBCDataStorageHeader* storage, TR::PersistentInfo* info)
{
...
void TR_IPBCDataCallGraph::loadFromPersistentCopy(
    TR_IPBCDataStorageHeader* storage, TR::Compilation* comp, uintptrj_t cacheStartAddress)
{
    TR_IPBCDataCallGraphStorage* store = (TR_IPBCDataCallGraphStorage*)storage;
    ...
    for (int32_t i = 0; i < NUM_CS_SLOTS; i++) {
        if (store->_csInfo.getClazz(i)) {
            J9Class* ramClass
                = ((TR_J9VM*)comp->fej9())
                      ->matchRAMclassFromROMclass((J9ROMClass*)(store->_csInfo.getClazz(i) + cacheStartAddress), comp);
            if (ramClass) {
...
void TR_IPBCDataCallGraph::copyFromEntry(TR_IPBytecodeHashTableEntry* originalEntry, TR::Compilation* comp)
{
...
TR_IPBCDataCallGraph* TR_IProfiler::getCGProfilingData(
    TR_OpaqueMethodBlock* method, uint32_t byteCodeIndex, TR::Compilation* comp)
{
    TR_IPBytecodeHashTableEntry* entry = profilingSample(method, byteCodeIndex, comp);
...
void TR_IProfiler::setCallCount(TR_OpaqueMethodBlock* method, int32_t bcIndex, int32_t count, TR::Compilation* comp)
{
...
void TR_IProfiler::setCallCount(TR_ByteCodeInfo& bcInfo, int32_t count, TR::Compilation* comp)
{
    setCallCount(getMethodFromBCInfo(bcInfo, comp), (int32_t)bcInfo.getByteCodeIndex(), count, comp);
}

int32_t TR_IProfiler::getCallCount(
    TR_OpaqueMethodBlock* calleeMethod, TR_OpaqueMethodBlock* method, int32_t bcIndex, TR::Compilation* comp)
{
...
int32_t TR_IProfiler::getCallCount(TR_OpaqueMethodBlock* method, int32_t bcIndex, TR::Compilation* comp)
{
    TR_IPBytecodeHashTableEntry* entry = profilingSample(method, bcIndex, comp);
...
void TR_IProfiler::setWarmCallGraphTooBig(
    TR_OpaqueMethodBlock* method, int32_t bcIndex, TR::Compilation* comp, bool set)
{
    TR_IPBytecodeHashTableEntry* entry = profilingSample(method, bcIndex, comp);
...
int32_t TR_IProfiler::getCallCount(TR_ByteCodeInfo& bcInfo, TR::Compilation* comp)
{
    return getCallCount(getMethodFromBCInfo(bcInfo, comp), (int32_t)bcInfo.getByteCodeIndex(), comp);
}

int32_t TR_IProfiler::getCGEdgeWeight(TR::Node* callerNode, TR_OpaqueMethodBlock* callee, TR::Compilation* comp)
{
...
int32_t TR_IProfiler::getMaxCallCount() { return _maxCallFrequency; }
...
uint32_t TR_IProfiler::releaseAllEntries()
{
    uint32_t count = 0;
...
void TR_IProfiler::copyDataFromEntry(
    TR_IPBytecodeHashTableEntry* oldEntry, TR_IPBytecodeHashTableEntry* newEntry, TR_IProfiler* ip)
{
...
void TR_IProfiler::checkMethodHashTable()
{
    ...
    for (int32_t bucket = 0; bucket < IPMETHOD_HASH_TABLE_SIZE; bucket++) {
        ...
        while (entry) {
            ...
            fprintf(fout, "%.*s.%.*s%.*s\t %p\t", J9UTF8_LENGTH(methodClazzUTRF8), J9UTF8_DATA(methodClazzUTRF8),
                J9UTF8_LENGTH(nameUTF8), J9UTF8_DATA(nameUTF8), J9UTF8_LENGTH(signatureUTF8),
                J9UTF8_DATA(signatureUTF8), method);
            fflush(fout);
#endif
            int32_t count = 0;
            ...
            for (TR_IPMethodData* it = &entry->_caller; it; it = it->next) {
                ...
                if (meth) {
                    ...
                    getClassNameSignatureFromMethod(
                        (J9Method*)meth, caller_methodClazzUTF8, caller_nameUTF8, caller_signatureUTF8);

                    fprintf(fout, "%p %.*s%.*s%.*s weight %d pc %p\n", meth, J9UTF8_LENGTH(caller_methodClazzUTF8),
                        ...
                        J9UTF8_DATA(caller_signatureUTF8), it->getWeight(), it->getPCIndex());
                    fflush(fout);
                } else {
...
void TR_IProfiler::getFaninInfo(
    TR_OpaqueMethodBlock* calleeMethod, uint32_t* count, uint32_t* weight, uint32_t* otherBucketWeight)
{
    uint32_t i = 0;
    uint32_t w = 0;
    uint32_t other = 0;
    ...
    uint32_t* weight, uint32_t pcIndex, TR::Compilation* comp)
{
    ...
    for (TR_IPMethodData* it = &entry->_caller; it; it = it->next) {
        // if (comp)
        //   traceMsg(comp, "comparing %p with %p and pc index %d pc address %p with other pc index %d other pc address
        //   %p\n", callerMethod, it->getMethod(), pcIndex, pcAddress, it->getPCIndex(), ((uintptrj_t) it->getPCIndex())
        //   + TR::Compiler->mtd.bytecodeStart(callerMethod));

        if (it->getMethod() == callerMethod
            && (!useTuples
                   || ((((uintptrj_t)it->getPCIndex()) + TR::Compiler->mtd.bytecodeStart(callerMethod))
                          == pcAddress))) {
            *weight = it->getWeight();
            return true;
        }
...
static int32_t J9THREAD_PROC iprofilerThreadProc(void* entryarg)
{
    ...
    // fprintf(stderr, "iprofilerThread will exit now numRequests=%u numRequestsHandedToIProfilerThread=%u
    // numRequestsSkipped=%u\n", numRequests, numRequestsHandedToIProfilerThread, numRequestsSkipped);
    return 0;
}
...
UDATA TR_IProfiler::parseBuffer(J9VMThread* vmThread, const U_8* dataStart, UDATA size, bool verboseReparse)
{
    ...
    while (cursor < dataStart + size) {
        ...
        case JBinvokeinterface2:
            ...
            if (callee && javaVM->initialMethods.initialVirtualMethod != callee && !fanInDisabled) {
                uint32_t offset = (uint32_t)(pc - caller->bytecodes);
                findOrCreateMethodEntry(caller, callee, true, offset);
                if (_compInfo->getLowPriorityCompQueue().isTrackingEnabled() && // is feature enabled?
                    vmThread == _iprofilerThread) // only IProfiler thread is allowed to execute this
        ...
        case JBtableswitch:
            ...
            if (TEST_verbose || verboseReparse) {
                j9tty_printf(
                    PORTLIB, "pc=%p (switch bc=%d) operand=%d cursor=%p\n", pc, *pc, switchOperand, cursorCopy);
            }
            break;
        default:
            if (!verboseReparse) {
...
inline uintptrj_t CallSiteProfileInfo::getClazz(int index)
{
...
inline void CallSiteProfileInfo::setClazz(int index, uintptrj_t clazzPointer)
{
#if defined(J9VM_GC_COMPRESSED_POINTERS) // compressed references
    // support for convert code, when it is implemented, do compression
    TR_OpaqueClassBlock* compressedOffset
        = J9JitMemory::convertClassPtrToClassOffset((J9Class*)clazzPointer); // compressed 32bit pointer
    // if we end up with something in the top 32bits, our compression is no good...
    TR_ASSERT((!(0xFFFFFFFF00000000 & (uintptrj_t)compressedOffset)),
        "Class pointer contains bits in the top word. Pointer given: %p Compressed: %p", clazzPointer,
        compressedOffset);
...
public:
    ...
    public:
        TR_CGChainedEntry(TR_IPBCDataCallGraph* entry)
            : _next(NULL)
            , _CGentry(entry)
        {}
    ...
    public:
        TR_AggregationHTNode(J9ROMMethod* romMethod, J9ROMClass* romClass, TR_IPBCDataCallGraph* entry)
            : _next(NULL)
    ...
    void add(J9ROMMethod* romMethod, J9ROMClass* romClass, TR_IPBCDataCallGraph* cgEntry)
    {
        ...
        if (!crtMethodNode) {
            ...
            TR_AggregationHTNode* newMethodNode
                = new (PERSISTENT_NEW) TR_AggregationHTNode(romMethod, romClass, cgEntry);
...
void TR_AggregationHT::sortByNameAndPrint(TR_J9VMBase* fe)
{
    ...
    for (int32_t bucket = 0; bucket < getSize(); bucket++) {
        ...
        for (; node; node = node->getNext()) {
            ...
            sprintf(wholeName, "%.*s.%.*s%.*s", J9UTF8_LENGTH(className), utf8Data(className), J9UTF8_LENGTH(name),
    ...
    for (size_t i = 0; i < numTrackedMethods(); ++i) {
        ...
        for (; cgEntry; cgEntry = cgEntry->getNext()) {
            ...
            switch (*pc) {
            case JBinvokestatic:
                fprintf(stderr, "JBinvokestatic\n");
                break;
            case JBinvokespecial:
                fprintf(stderr, "JBinvokespecial\n");
                break;
            case JBinvokeinterface:
                fprintf(stderr, "JBinvokeinterface\n");
                break;
            case JBinvokeinterface2:
                fprintf(stderr, "JBinvokeinterface2\n");
                break;
            case JBinvokevirtual:
                fprintf(stderr, "JBinvokevirtual\n");
                break;
            case JBinstanceof:
                fprintf(stderr, "JBinstanceof\n");
                break;
            case JBcheckcast:
                fprintf(stderr, "JBcheckcast\n");
                break;
            default:
                fprintf(stderr, "JBunknown\n");
            }
            ...
            if (cgEntry) {
                ...
                if (romClass) {
                    ...
                    for (U_32 i = 0; i < romClass->romMethodCount; i++) {
                        if (((UDATA)pc >= (UDATA)currentMethod)
