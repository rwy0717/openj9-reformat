...
public:
    ...
    TR_IPBytecodeHashTableEntry(uintptrj_t pc)
        : _next(NULL)
        , _pc(pc)
...
public:
    ...
    TR_IPBCDataFourBytes(uintptrj_t pc)
    ...
    {
        static uint32_t data_copy = (uint32_t)data;
        return &data_copy;
    }

    virtual int32_t setData(uintptrj_t value, uint32_t freq = 1)
    {
        data = (uint32_t)value;
        return 0;
    }
    ...
    virtual void createPersistentCopy(uintptrj_t cacheStartAddress, uintptrj_t cacheSize,
        TR_IPBCDataStorageHeader* storage, TR::PersistentInfo* info);
    virtual void loadFromPersistentCopy(
        TR_IPBCDataStorageHeader* storage, TR::Compilation* comp, uintptrj_t cacheStartAddress);
...
public:
    ...
    TR_IPBCDataAllocation(uintptrj_t pc)
        : TR_IPBytecodeHashTableEntry(pc)
        , clazz(0)
        , method(0)
        , data(0)
...
public:
    ...
    TR_IPBCDataEightWords(uintptrj_t pc)
        : TR_IPBytecodeHashTableEntry(pc)
    {
    ...
    { /*TR_ASSERT(0, "Don't call me, I'm empty"); */
        return 0;
    }
    virtual int32_t setData(uintptrj_t value, uint32_t freq = 1)
    { /*TR_ASSERT(0, "Don't call me, I'm empty");*/
...
public:
    ...
    TR_IPBCDataCallGraph(uintptrj_t pc)
        : TR_IPBytecodeHashTableEntry(pc)
    {
        _csInfo.initialize();
    }
    ...
    virtual bool isInvalid()
    {
        if (_csInfo.getClazz(0) == IPROFILING_INVALID)
            return true;
        return false;
    }
...
public:
    ...
    */
    ...
    // j9method.cpp
    void getFaninInfo(
        TR_OpaqueMethodBlock* calleeMethod, uint32_t* count, uint32_t* weight, uint32_t* otherBucketWeight = NULL);
    ...
    int32_t getCallCount(
        TR_OpaqueMethodBlock* calleeMethod, TR_OpaqueMethodBlock* method, int32_t bcIndex, TR::Compilation*);
...
private:
    ...
    TR_IPBytecodeHashTableEntry* profilingSample(
        uintptrj_t pc, uintptrj_t data, bool addIt, bool isRIData = false, uint32_t freq = 1);
    ...
    TR_IPBytecodeHashTableEntry* persistentProfilingSample(
    ...
    TR_IPBCDataStorageHeader* persistentProfilingSample(TR_OpaqueMethodBlock* method, uint32_t byteCodeIndex,
        TR::Compilation* comp, bool* methodProfileExistsInSCC, uintptrj_t* cacheOffset,
    ...
    TR_IPBytecodeHashTableEntry* getProfilingEntry(
        TR_OpaqueMethodBlock* method, uint32_t byteCodeIndex, TR::Compilation*);

    TR_IPBCDataCallGraph* getCGProfilingData(TR_ByteCodeInfo& bcInfo, TR::Compilation* comp);
    TR_IPBCDataCallGraph* getCGProfilingData(
    ...
    uint32_t walkILTreeForEntries(uintptrj_t* pcEntries, uint32_t& numEntries, TR_J9ByteCodeIterator* bcIterator,
        TR_OpaqueMethodBlock* method, TR::Compilation* comp, uintptrj_t cacheOffset, uintptrj_t cacheSize,
