...
bool J9::CodeCache::resizeCodeMemory(void* memoryBlock, size_t newSize)
{
...
TR::CodeCache* J9::CodeCache::allocate(
    TR::CodeCacheManager* cacheManager, size_t segmentSize, int32_t reservingCompThreadID)
{
...
bool J9::CodeCache::initialize(TR::CodeCacheManager* manager, TR::CodeCacheMemorySegment* codeCacheSegment,
    size_t codeCacheSizeAllocated, OMR::CodeCacheHashEntrySlab* hashEntrySlab)
{
        ...
        if (metaData->bodyInfo && !TR::Options::getCmdLineOptions()->getOption(TR_EnableHCR)
            ...
            if (!bi->getIsAotedBody()) // AOTed methods allocate the bodyInfo in the dataCache with allocateException
            {
                ...
                if (pmi && !pmi->isInDataCache()) {
                    ...
                    if (TR::Compiler->mtd.startPC((TR_OpaqueMethodBlock*)metaData->ramMethod)
                        == (uintptr_t)metaData->startPC) {
                        // Clear profile info
                        ...
                        if (TR::Options::getVerboseOption(TR_VerboseReclamation)) {
                            TR_VerboseLog::writeLineLocked(
                                TR_Vlog_RECLAMATION, "Reclaiming PersistentMethodInfo 0x%p.", pmi);
                        }
...
{
    TR_ASSERT(
        _manager->codeCacheConfig().needsMethodTrampolines(), "Attempting to purge trampolines when they do not exist");
        ...
        if (!entry) {
            ...
            } else // no space in this code cache; must allocate a new one
            {
                ...
                if (config.verboseCodeCache()) {
                    TR_VerboseLog::writeLineLocked(
                        TR_Vlog_CODECACHE, "CodeCache %p marked as full in reserveUnresolvedTrampoline", this);
                }
            }
        }
    }
...
void mcc_callPointPatching_unwrapper(void** argsPtr, void** resPtr)
{
    ...
    if (codeCache)
        codeCache->patchCallPoint(
            reinterpret_cast<TR_OpaqueMethodBlock*>(argsPtr[0]), argsPtr[1], argsPtr[2], argsPtr[3]);
}
...
void mcc_reservationAdjustment_unwrapper(void** argsPtr, void** resPtr)
{
    ...
    if (codeCache)
        codeCache->adjustTrampolineReservation(
            reinterpret_cast<TR_OpaqueMethodBlock*>(argsPtr[1]), argsPtr[2], static_cast<int32_t>((UDATA)argsPtr[3]));
}

void mcc_lookupHelperTrampoline_unwrapper(void** argsPtr, void** resPtr)
{
    OMR::CodeCacheTrampolineCode* trampoline
        = TR::CodeCacheManager::instance()->findHelperTrampoline(argsPtr[0], static_cast<int32_t>((UDATA)argsPtr[1]));
    *resPtr = reinterpret_cast<void*>(trampoline);
}

void mcc_reservationInterfaceCache_unwrapper(void** args, void** res)
{
    TR::CodeCacheManager::instance()->reservationInterfaceCache(
        args[0], reinterpret_cast<TR_OpaqueMethodBlock*>(args[1]));
}
...
{
    return TR::CodeCacheManager::instance()->replaceTrampoline(
        method, callSite, oldTrampoline, oldTargetPC, newTargetPC, needSync);
}
}
