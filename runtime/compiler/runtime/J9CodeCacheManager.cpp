...
void J9::CodeCacheManager::addCodeCache(TR::CodeCache* codeCache)
{
    ...
    if (j9segment) {
        ...
        jit_artifact_protected_add_code_cache(
            TR::CodeCacheManager::javaVM(), TR::CodeCacheManager::jitConfig()->translationArtifacts, j9segment, NULL);
...
TR::CodeCache* J9::CodeCacheManager::reserveCodeCache(
    bool compilationCodeAllocationsMustBeContiguous, size_t sizeEstimate, int32_t compThreadID, int32_t* numReserved)
{
    TR::CodeCache* codeCache = self()->OMR::CodeCacheManager::reserveCodeCache(
        compilationCodeAllocationsMustBeContiguous, sizeEstimate, compThreadID, numReserved);
    if (codeCache == NULL) {
...
TR::CodeCacheMemorySegment* J9::CodeCacheManager::allocateCodeCacheSegment(
    size_t segmentSize, size_t& codeCacheSizeToAllocate, void* preferredStartAddress)
{
    ...
    J9MemorySegment* codeCacheSegment = javaVM->internalVMFunctions->allocateVirtualMemorySegmentInList(
        javaVM, jitConfig->codeCacheList, codeCacheSizeToAllocate, segmentType, &vmemParams);

    uintptr_t someJitLibraryAddress = self()->getSomeJitLibraryAddress();
    // isInRange() checks whether the allocated codeCacheSegment baseAddress is in range of JitLibrary to avoid
    // trampoline
    if (codeCacheSegment && (vmemParams.options & J9PORT_VMEM_ADDRESS_HINT)
        ...
        // if jit library address is greater than MAX_DISTANCE, then we search in the addresses before the jit library
        // address if jit library address is less than MAX_DISTANCE, which means we do not have the full
        // MAX_DISTANCE(2GB-24MB) range before the jit library in this case we search in the addresses after the jit
        // library address
        if (someJitLibraryAddress > MAX_DISTANCE_NEAR_JITLIBRARY_TO_AVOID_TRAMPOLINE) {
            // align the startAddress to page boundary
            vmemParams.startAddress = (void*)align(
                (uint8_t*)(someJitLibraryAddress - MAX_DISTANCE_NEAR_JITLIBRARY_TO_AVOID_TRAMPOLINE), alignment - 1);
            vmemParams.endAddress = preferredStartAddress;
        } else {
            vmemParams.startAddress
                = (void*)align((uint8_t*)(someJitLibraryAddress + SAFE_DISTANCE_REPOSITORY_JITLIBRARY), alignment - 1);
...
#endif
        codeCacheSegment = javaVM->internalVMFunctions->allocateVirtualMemorySegmentInList(
            javaVM, jitConfig->codeCacheList, codeCacheSizeToAllocate, segmentType, &vmemParams);
    }
    ...
    if (codeCacheSegment) {
        ...
        if (config.verboseCodeCache()) {
            ...
            if (preferredStartAddress
                && self()->isInRange((uintptr_t)(codeCacheSegment->baseAddress), someJitLibraryAddress,
                       MAX_DISTANCE_NEAR_JITLIBRARY_TO_AVOID_TRAMPOLINE)) {
                verboseLogString = "The code cache repository was allocated between addresses %p and %p to be near the "
                                   "VM/JIT modules to avoid trampolines";
            }
            TR_VerboseLog::writeLineLocked(
                TR_Vlog_CODECACHE, verboseLogString, vmemParams.startAddress, vmemParams.endAddress);
        }
    } else {
    ...
    if (config.codeCachePadKB()) {
        void* rc = j9vmem_commit_memory(
            codeCacheSegment->vmemIdentifier.address, segmentSize, &codeCacheSegment->vmemIdentifier);
    ...
    if (config.verboseCodeCache())
        TR_VerboseLog::writeLineLocked(
            TR_Vlog_CODECACHE, "allocated code cache segment of size %u", codeCacheSizeToAllocate);
...
TR::CodeCacheMemorySegment* J9::CodeCacheManager::setupMemorySegmentFromRepository(
    ...
    if (!TR::Options::getCmdLineOptions()->getOption(TR_DisableSmartPlacementOfCodeCaches)) {
        ...
        if (largeCodePageSize >= 0x40000000) {
            if (someFunctionPointer > largeCodePageSize * 2) {
                ...
                startAddress = (void*)(((uintptr_t)someFunctionPointer - SAFE_DISTANCE_REPOSITORY_JITLIBRARY)
                    & ~(largeCodePageSize - 1));
