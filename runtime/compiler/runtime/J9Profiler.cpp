    ...
    if (blockStructure) {
        ...
        if (headerBlocks.get(block->getNumber())) {
            if (performTransformation(comp(),
                ...
                TR::TreeTop::createIncTree(
                    comp(), block->getEntry()->getNode(), _recompilation->getCounterSymRef(), -1, block->getEntry());
                setHasModifiedTrees(true);
            }
        }
        return;
...
TR_CatchBlockProfiler::TR_CatchBlockProfiler(TR::Compilation* c, TR::Recompilation* r, bool initialCompilation)
    ...
    for (TR::Block* b = firstTree->getNode()->getBlock(); b; b = b->getNextBlock())
        if (!b->getExceptionPredecessors().empty() && !b->isOSRCatchBlock()) {
            if (performTransformation(comp(),
                ...
                TR::TreeTop* profilingTree = TR::TreeTop::createIncTree(
                    comp(), b->getEntry()->getNode(), _catchCounterSymRef, 1, b->getEntry());
                profilingTree->getNode()->setIsProfilingCode();
                setHasModifiedTrees(true);
            }
        }
    ...
    for (TR::TreeTop* tt = comp()->getStartTree(); tt; tt = tt->getNextTreeTop()) {
        ...
        if (firstChild && firstChild->getOpCodeValue() == TR::arraycopy && !getInitialCompilation()
            ...
            if (!arrayCopyLen->getOpCode().isLoadConst() && !arrayCopyLen->getByteCodeInfo().doNotProfile()
                ...
                addProfilingTrees(
                    arrayCopyLen, tt, 0, LastValueInfo, LastProfiler, true, &firstChild->getByteCodeInfo());
            }
                                        ...
                                        // thunk body
            {
                ...
                if (methodSymbol->isInterface()) {
                    ...
                    if (thisClass) {
                        ...
                        picMiss = chTable->isKnownToHaveMoreThanTwoInterfaceImplementers(
                            thisClass, cpIndex, methodSymRef->getOwningMethod(comp()), comp());
                    }
                }
                        ...
                        if ((callChild->getOpCode().isLoadVarDirect()
                            ...
                            if (correctToClone && !callChild->isUnsafeToDuplicateAndExecuteAgain(&visitBudget)
                                && performTransformation(comp(),
                                       "%s CALL PROFILE VERIFICATION: Duplicating node %p so it can be profiled near "
                                       "%p\n",
            ...
            if (doStringOpt && !methodSymRef->isUnresolved() && !methodSymbol->isHelper()) {
                TR::ResolvedMethodSymbol* method
                    = firstChild->getSymbolReference()->getSymbol()->castToResolvedMethodSymbol();
...
void TR_ValueProfiler::visitNode(TR::Node* node, TR::TreeTop* tt, vcount_t visitCount)
{
    ...
    if (profileLongParms && (node->getType().isInt64()) && node->getOpCode().isLoadVar()
        && !node->getByteCodeInfo().doNotProfile()) {
        TR::Node* intNode = TR::Node::create(
            TR::l2i, 1, TR::Node::create(TR::lushr, 2, node, TR::Node::create(node, TR::iconst, 0, 32)));
    ...
    bool decrementRecompilationCounter)
{
    ...
    if (source == LinkedListProfiler || source == ArrayProfiler)
        addListOrArrayProfilingTrees(
            node, cursorTree, bci, numExpandedValues, kind, source, commonNode, decrementRecompilationCounter);
    ...
    TR_ValueInfoKind kind, TR_ValueInfoSource source, bool commonNode)
{
    ...
    if (_postLowering) {
        ...
        TR::SymbolReference* profiler
            = comp()->getSymRefTab()->findOrCreateRuntimeHelper(TR_jProfile32BitValue, false, false, true);
...
TR_AbstractInfo* TR_ValueProfileInfoManager::getProfiledValueInfo(
    TR::Node* node, TR::Compilation* comp, TR_ValueInfoKind kind, uint32_t source)
{
...
TR_AbstractInfo* TR_ValueProfileInfoManager::getProfiledValueInfo(
    TR_ByteCodeInfo& bci, TR::Compilation* comp, TR_ValueInfoKind kind, uint32_t source)
{
    TR_ValueProfileInfoManager* manager = TR_ValueProfileInfoManager::get(comp);
    return manager ? manager->getValueInfo(bci, comp, kind, source) : NULL;
}

TR_AbstractInfo* TR_ValueProfileInfoManager::getValueInfo(
    TR::Node* node, TR::Compilation* comp, TR_ValueInfoKind kind, uint32_t source)
{
...
TR_AbstractInfo* TR_ValueProfileInfoManager::getValueInfo(
    TR_ByteCodeInfo& bcInfo, TR::Compilation* comp, TR_ValueInfoKind kind, uint32_t source)
{
...
int32_t TR_ValueProfileInfoManager::getCallGraphProfilingCount(
    TR_OpaqueMethodBlock* method, int32_t byteCodeIndex, TR::Compilation* comp)
{
...
int32_t TR_ValueProfileInfoManager::getCallGraphProfilingCount(
    TR::Node* node, TR_OpaqueMethodBlock* method, TR::Compilation* comp)
{
    return comp->fej9()->getCGEdgeWeight(node, method, comp);
}

int32_t TR_ValueProfileInfoManager::getCallGraphProfilingCount(TR::Node* node, TR::Compilation* comp)
{
    return comp->fej9()->getIProfilerCallCount(node->getByteCodeInfo(), comp);
}

bool TR_ValueProfileInfoManager::isColdCall(TR_OpaqueMethodBlock* method, int32_t byteCodeIndex, TR::Compilation* comp)
{
    return (getCallGraphProfilingCount(method, byteCodeIndex, comp) < (comp->getFlowGraph()->getLowFrequency()));
}

bool TR_ValueProfileInfoManager::isColdCall(
    TR_OpaqueMethodBlock* calleeMethod, TR_OpaqueMethodBlock* method, int32_t byteCodeIndex, TR::Compilation* comp)
{
    return (getCallGraphProfilingCount(calleeMethod, method, byteCodeIndex, comp)
        < (comp->getFlowGraph()->getLowFrequency()));
}

bool TR_ValueProfileInfoManager::isColdCall(TR::Node* node, TR::Compilation* comp)
{
    return (
        comp->fej9()->getIProfilerCallCount(node->getByteCodeInfo(), comp) < (comp->getFlowGraph()->getLowFrequency()));
}

bool TR_ValueProfileInfoManager::isWarmCall(TR::Node* node, TR::Compilation* comp)
{
    return (comp->fej9()->getIProfilerCallCount(node->getByteCodeInfo(), comp)
        < (comp->getFlowGraph()->getLowFrequency() << 1));
}

bool TR_ValueProfileInfoManager::isHotCall(TR::Node* node, TR::Compilation* comp)
{
...
bool TR_ValueProfileInfoManager::isWarmCallGraphCall(
    TR::Node* node, TR_OpaqueMethodBlock* method, TR::Compilation* comp)
{
    if (node->getSymbolReference() && node->getSymbolReference()->getSymbol()
        && ((node->getSymbolReference()->getSymbol()->castToMethodSymbol()->getMethodKind()
...
void TR_ValueProfileInfoManager::updateCallGraphProfilingCount(
...
void TR_BranchProfileInfoManager::getBranchCounters(
    TR::Node* node, TR::TreeTop* treeTop, int32_t* branchToCount, int32_t* fallThroughCount, TR::Compilation* comp)
{
    if (_iProfiler == NULL) {
        *branchToCount = 0;
        *fallThroughCount = 0;
...
TR_MethodBranchProfileInfo* TR_MethodBranchProfileInfo::addMethodBranchProfileInfo(
    uint32_t callSiteIndex, TR::Compilation* comp)
{
...
TR_MethodBranchProfileInfo* TR_MethodBranchProfileInfo::getMethodBranchProfileInfo(
    uint32_t callSiteIndex, TR::Compilation* comp)
{
    TR::list<TR_MethodBranchProfileInfo*>& infos = comp->getMethodBranchInfos();
...
void TR_MethodBranchProfileInfo::resetMethodBranchProfileInfos(
 ...
 */
TR_ExternalValueProfileInfo* TR_ExternalValueProfileInfo::addInfo(
 ...
 * \return The matched profiling information or NULL if nothing was present.
 */
TR_AbstractProfilerInfo* TR_ValueProfileInfo::getProfilerInfo(
    TR_ByteCodeInfo& bcInfo, TR::Compilation* comp, TR_ValueInfoKind kind, TR_ValueInfoSource source, bool fuzz)
{
    TR_ASSERT(
        kind < LastValueInfo && source < LastProfiler, "Invalid value info request, kind %d source %d\n", kind, source);
 ...
 *
 * \param profileLog Dump all the tables that have been inspected, as well as a log of any that were cleared. NULL will
 * disable tracing.
 */
void TR_ValueProfileInfo::resetLowFreqValues(TR::FILE* profileLog)
{
    ...
    , _isQueuedForRecompilation(0)
{
    ...
    for (TR::CFGNode* node = comp->getFlowGraph()->getFirstNode(); node; node = node->getNext()) {
        ...
        if (startTree) {
            ...
            TR_ASSERT(
                callSite < static_cast<int32_t>(TR_CallSiteInfo::getCurrent(comp)->getNumCallSites() & 0x7FFFFFFF),
...
int32_t TR_BlockFrequencyInfo::getFrequencyInfo(TR::Block* block, TR::Compilation* comp)
{
    ...
    if (comp->getOption(TR_TraceBFGeneration))
        traceMsg(comp, "@@ block_%d [%d,%d] has raw count %d\n", block->getNumber(), bci.getCallerIndex(),
            bci.getByteCodeIndex(), frequency);
    return frequency;
}

int32_t TR_BlockFrequencyInfo::getFrequencyInfo(
    TR_ByteCodeInfo& bci, TR::Compilation* comp, bool normalizeForCallers, bool trace)
{
    ...
    int32_t frequency
        = getRawCount(callerIndex < 0 ? comp->getMethodSymbol() : comp->getInlinedResolvedMethodSymbol(callerIndex),
            bci, _callSiteInfo, maxCount, comp);
                    ...
                    if (info && info->getBlockFrequencyInfo()
                        ...
                        if (callStack.empty()
                            ...
                            if (normalizeForCallers) {
                                ...
                                int32_t entry = bfi->getRawCount(
                                    resolvedMethodSymbol, callee, info->getCallSiteInfo(), bfi->getMaxRawCount(), comp);
                                if (entry == 0) {
                                    frequency = 0;
                                    break;
                                } else if (entry > -1) {
                                ...
                                if (computedFrequency > -1) {
                                    ...
                                    frequency = (int32_t)(
                                        (outterProfiledFrequency * computedFrequency) / innerFrequencyScale);
                                    break;
                                }
                            } else {
                            ...
                            int32_t entryCount = bfi->getRawCount(
                                resolvedMethodSymbol, entry, info->getCallSiteInfo(), bfi->getMaxRawCount(), comp);
                            TR_ByteCodeInfo call = callStack.back().second;
                            call.setCallerIndex(-1);
                            int32_t rawCount = bfi->getRawCount(
                                resolvedMethodSymbol, call, info->getCallSiteInfo(), bfi->getMaxRawCount(), comp);
                            if (rawCount == 0) {
                                frequency = 0;
                                break;
                            } else if (rawCount > -1) {
                ...
                // interested in
                {
                    ...
                    if (normalizeForCallers) {
                        if (callStack.size() > 0) {
                            ...
                            //   TR_VerboseLog::writeLineLocked(TR_Vlog_INFO, " BFINFO: %s [%s:%d] - from IProfiler",
                            //   comp->signature(), (bci.getCallerIndex() < 0 ? comp->getMethodSymbol() :
                            //   comp->getInlinedResolvedMethodSymbol(bci.getCallerIndex()))->signature(comp->trMemory()),
                            //   bci.getByteCodeIndex());
                            frequency = (int32_t)(
 ...
 */
int32_t TR_BlockFrequencyInfo::getOriginalBlockNumberToGetRawCount(
    TR_ByteCodeInfo& bci, TR::Compilation* comp, bool trace)
{
    int32_t callerIndex = bci.getCallerIndex();
 ...
 */
TR::Node* TR_BlockFrequencyInfo::generateBlockRawCountCalculationSubTree(
    TR::Compilation* comp, TR::Node* node, bool trace)
{
    return generateBlockRawCountCalculationSubTree(
        comp, getOriginalBlockNumberToGetRawCount(node->getByteCodeInfo(), comp, trace), node);
}
 ...
 */
TR::Node* TR_BlockFrequencyInfo::generateBlockRawCountCalculationSubTree(
    TR::Compilation* comp, int32_t blockNumber, TR::Node* node)
{
    TR::Node* root = NULL;
    if (blockNumber > -1
...
int32_t TR_BlockFrequencyInfo::getRawCount(
    TR_ByteCodeInfo& bci, TR_CallSiteInfo* callSiteInfo, int64_t maxCount, TR::Compilation* comp)
{
...
bool TR_CallSiteInfo::hasSameBytecodeInfo(
...
int32_t TR_CallSiteInfo::hasSamePartialBytecodeInfo(
    TR_ByteCodeInfo& persistentBytecodeInfo, TR_ByteCodeInfo& currentBytecodeInfo, TR::Compilation* comp)
{
    if (persistentBytecodeInfo.getByteCodeIndex() != currentBytecodeInfo.getByteCodeIndex())
        return 0;
    ...
    if (!TR::Options::getCmdLineOptions()->getOption(TR_DisableJProfilerThread)) {
        if (info->_refCount == 0 && TR::Options::isAnyVerboseOptionSet(TR_VerboseProfiling, TR_VerboseReclamation))
            TR_VerboseLog::writeLineLocked(
                TR_Vlog_RECLAMATION, "PersistentProfileInfo 0x%p queued for reclamation.", info);
        ...
        if (TR::Options::getVerboseOption(TR_VerboseReclamation))
            TR_VerboseLog::writeLineLocked(
                TR_Vlog_RECLAMATION, "Reclaiming PersistentProfileInfo immediately 0x%p.", info);
...
TR_PersistentProfileInfo* TR_AccessedProfileInfo::compare(TR_PersistentMethodInfo* methodInfo)
{
    ...
    if (replace) {
        if (TR::Options::getVerboseOption(TR_VerboseProfiling))
            TR_VerboseLog::writeLineLocked(
                TR_Vlog_PROFILING, "For MethodInfo 0x%p, updating best from 0x%p to 0x%p", methodInfo, best, recent);
 ...
 */
TR_PersistentProfileInfo* TR_JProfilerThread::deleteProfileInfo(
    TR_PersistentProfileInfo** prevNext, TR_PersistentProfileInfo* info)
{
