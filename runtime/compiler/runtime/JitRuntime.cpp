...
void setDllSlip(char* CodeStart, char* CodeEnd, char* dllName, TR::Compilation* comp)
{
    ...
    if (sliphandle != 0) {
        ...
        if (do_slip_func != 0) {
            ...
            (*fptrl)(CodeStart, CodeEnd, (void*)NULL, (void*)NULL, (char*)NULL, (char*)NULL, (char*)NULL);
        } else if (comp) {
...
//
void J9::Recompilation::sampleMethod(
    void* thread, TR_FrontEnd* fe, void* startPC, int32_t codeSize, void* samplePC, void* methodInfo, int32_t tickCount)
{
    TR_J9VMBase* feJ9 = (TR_J9VMBase*)fe;
...
bool J9::Recompilation::induceRecompilation(
    TR_FrontEnd* fe, void* startPC, bool* queued, TR_OptimizationPlan* optimizationPlan)
{
        ...
        if (methodInfo->_optimizationPlan) {
            ...
            *queued = true; // to prevent the plan being deallocated by creator
            methodInfo->setNextCompileLevel(
                methodInfo->_optimizationPlan->getOptLevel(), methodInfo->_optimizationPlan->insertInstrumentation());
        }
...
void J9FASTCALL _jProfile32BitValue(uint32_t value, TR_HashTableProfilerInfo<uint32_t>* table) { table->addKey(value); }

void J9FASTCALL _jProfile64BitValue(uint64_t value, TR_HashTableProfilerInfo<uint64_t>* table) { table->addKey(value); }
}
    ...
    int32_t* recompilationCounter)
{
    ...
    if (value) {
        if ((((J9Object*)value)->clazz & (UDATA)(-J9_REQUIRED_CLASS_ALIGNMENT))
            == ((j9objectclass_t)bigdecimalj9class)) {
            readValues = true;
...
extern "C" {
void J9FASTCALL _jitProfileStringValue(uintptrj_t value, int32_t charsOffset, int32_t lengthOffset,
...
void dumpMethodsForClass(::FILE* fp, J9Class* classPointer)
{
    ...
    for (i = 0; i < numMethods; i++) {
        ...
        fprintf(fp, "\t%u, %.*s.%.*s%.*s\n", &(resolvedMethods[i]), J9UTF8_LENGTH(clazzUTRF8), J9UTF8_DATA(clazzUTRF8),
    ...
    for (superclassIndex = 0; superclassIndex <= depth; superclassIndex++) {
        ...
        while (romFieldCursor) {
            if ((romFieldCursor->modifiers & J9AccStatic) == 0) {
                ...
                fprintf(fp, "%u, %.*s, %.*s, %08x, ", instanceClass, J9UTF8_LENGTH(sigUTF), J9UTF8_DATA(sigUTF),
                    J9UTF8_LENGTH(nameUTF), J9UTF8_DATA(nameUTF), romFieldCursor->modifiers);
...
void dumpClassStaticsForClass(::FILE* fp, J9Class* clazz, J9VMThread* vmThread)
{
    ...
    while (romFieldCursor != NULL) {
        if (romFieldCursor->modifiers & J9AccStatic) {
            ...
            fprintf(fp, "%u, %.*s, %.*s, %08x, ", clazz, J9UTF8_LENGTH(sigUTF), J9UTF8_DATA(sigUTF),
                J9UTF8_LENGTH(nameUTF), J9UTF8_DATA(nameUTF), romFieldCursor->modifiers);
...
extern "C" void _patchJNICallSite(
    J9Method* method, uint8_t* callPC, uint8_t* newAddress, TR_FrontEnd* fe, int32_t smpFlag)
{
...
extern "C" _tr_try_lock(
    uint32_t* a, uint32_t b, uint32_t c); // return 1 if compareAndSwap "c" successfully into "a"; otherwise 0;
...
#if defined(TR_HOST_X86) || defined(TR_HOST_POWER) || defined(TR_HOST_S390) || (defined(TR_HOST_ARM))
uint32_t* getLinkageInfo(void* startPC) { return (uint32_t*)TR_LinkageInfo::get(startPC); }
...
void fixPersistentMethodInfo(void* table)
{
    ...
    TR_PersistentMethodInfo* methodInfo
        = (TR_PersistentMethodInfo*)((char*)bodyInfo + sizeof(TR_PersistentJittedBodyInfo));
...
static void printMethodHandleArgs(
    j9object_t methodHandle, void** stack, J9VMThread* vmThread, TR_VlogTag vlogTag, TR_FrontEnd* fe)
{
    ...
    if (stack[0] != methodHandle && TR::Options::isAnyVerboseOptionSet()) {
        ...
        ; // we want all the following output to appear together in the log
    ...
    TR_ASSERT(
        stack[0] == methodHandle, "Pointer %p on stack @ %p must match MethodHandle %p", stack[0], stack, methodHandle);
    ...
    if (vlogTag) {
        ...
        if (numArgs == 0) {
            ...
            TR_VerboseLog::writeLine(
                vlogTag, "%p     arg " POINTER_PRINTF_FORMAT " receiver handle", vmThread, stack[0]);
            stack--;
            ...
            for (curArg = sig + 1; curArg[0] != ')'; curArg = nextArg) {
                ...
                case '[':
                    TR_VerboseLog::writeLine(vlogTag, "%p     arg " POINTER_PRINTF_FORMAT " %.*s", vmThread,
                        (void*)(*(intptrj_t*)stack), nextArg - curArg, curArg);
                    stack -= 1;
                    break;
                default:
                    TR_VerboseLog::writeLine(
                        vlogTag, "%p     arg 0x%x %.*s", vmThread, (int)*(int32_t*)stack, nextArg - curArg, curArg);
                    stack -= 1;
                    break;
                }
            }
...
extern "C" uint8_t* compileMethodHandleThunk(
    j9object_t methodHandle, j9object_t arg, J9VMThread* vmThread, U_32 flags); // called from rossa.cpp
uint8_t* compileMethodHandleThunk(j9object_t methodHandle, j9object_t arg, J9VMThread* vmThread, U_32 flags)
{
    ...
    if (verbose) {
        TR_VerboseLog::vlogAcquire();
        TR_VerboseLog::writeLine(
            TR_Vlog_MH, "%p Starting compileMethodHandleThunk on MethodHandle %p", vmThread, methodHandle);
        if (arg)
    ...
    if ((verbose || details) && !isCustom) // custom thunks arrive with a different stack shape that we don't control
        printMethodHandleArgs(
            methodHandle, stack, vmThread, verbose ? TR_Vlog_MH : details ? TR_Vlog_MHD : TR_Vlog_null, fej9);
    ...
    if (cmdLineOptions->getOption(TR_DisableMethodHandleThunks)) {
        if (details)
            TR_VerboseLog::writeLineLocked(
                TR_Vlog_MHD, "%p   Thunks disabled -- will proceed in interpreter", vmThread);
    } else {
        if (details) {
        ...
        jobject handleRef = vmThread->javaVM->internalVMFunctions->j9jni_createGlobalRef(
            (JNIEnv*)vmThread, methodHandle, false); // TODO:JSR292: Make this weak?
        jobject argRef
        ...
        if (plan) {
            ...
            if (details)
                TR_VerboseLog::writeLineLocked(
                    TR_Vlog_MHD, "%p   Thunk cannot be compile due to OOM -- will proceed in interpreter", vmThread);
        }
    }
...
void* initialInvokeExactThunk(j9object_t methodHandle, J9VMThread* vmThread)
{
    ...
    if (verbose)
        TR_VerboseLog::writeLineLocked(
            TR_Vlog_MH, "%p initialInvokeExactThunk on MethodHandle %p", vmThread, methodHandle);
    ...
    uintptrj_t thunkTuple
        = fej9->getReferenceField((uintptrj_t)methodHandle, "thunks", "Ljava/lang/invoke/ThunkTuple;");
    if (details) {
        TR_OpaqueClassBlock* handleClass = fej9->getObjectClass((uintptrj_t)methodHandle);
        int32_t classNameLength;
        char* className = fej9->getClassNameChars(handleClass, classNameLength);
        ...
        TR_VerboseLog::writeLineLocked(TR_Vlog_MHD, "%p   %.*s %p hash %x type %p %s", vmThread, classNameLength,
            className, methodHandle, hashCode, methodType, descriptorNTS);
        TR_VerboseLog::writeLineLocked(
            TR_Vlog_MHD, "%p   ThunkTuple %p thunkableSignature: %s", vmThread, thunkTuple, thunkSignature);
    }
    ...
    if (j2iThunks) {
        ...
        if (details)
            TR_VerboseLog::writeLineLocked(
                TR_Vlog_MHD, "%p   J2I thunk is %p %s", vmThread, addressToDispatch, thunk->terseSignature());
    } else {
    ...
    if (cmdLineOptions->getOption(TR_DisableThunkTupleJ2I)) {
        if (details)
            TR_VerboseLog::writeLineLocked(
                TR_Vlog_MHD, "%p   ThunkTuple J2I disabled -- leave ThunkTuple %p unchanged", vmThread, thunkTuple);
    } else {
        uintptrj_t fieldOffset
            = fej9->getInstanceFieldOffset(fej9->getObjectClass(thunkTuple), "invokeExactThunk", "J");
#if defined(OSX)
        bool success = fej9->compareAndSwapInt64Field(thunkTuple, "invokeExactThunk",
            (uint64_t)(uintptrj_t)initialInvokeExactThunkGlue, (uint64_t)(uintptrj_t)addressToDispatch);
...
#else
        bool success = fej9->compareAndSwapInt64Field(thunkTuple, "invokeExactThunk",
            (uint64_t)(uintptrj_t)_initialInvokeExactThunkGlue, (uint64_t)(uintptrj_t)addressToDispatch);
...
void methodHandleJ2I(j9object_t methodHandle, void** stack, J9VMThread* vmThread)
{
    ...
    if (verbose) {
        ...
        uintptrj_t methodType
            = fej9->getReferenceField((uintptrj_t)methodHandle, "type", "Ljava/lang/invoke/MethodType;");
