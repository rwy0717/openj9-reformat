...
static uint8_t* allocateGCData(TR_J9VMBase* vm, uint32_t numBytes, TR::Compilation* comp)
{
    ...
    gcData = vm->allocateDataCacheRecord(
        numBytes, comp, vm->isAOT_DEPRECATED_DO_NOT_USE(), &shouldRetryAllocation, J9_JIT_DCE_STACK_ATLAS, &size);
    if (!gcData) {
...
static void createByteCodeInfoRange(
    TR_GCStackMap* map, uint8_t* location, bool fourByteOffsets, TR::GCStackAtlas* trStackAtlas, TR::Compilation* comp)
{
        ...
        *((location - numInternalPtrsForThisPinningArray) - 1) = numInternalPtrsForThisPinningArray;
        if (debug("traceIP"))
            printf("In VMJ9 numInternalPtrs = %d and memory %x\n", numInternalPtrsForThisPinningArray,
                (location - numInternalPtrsForThisPinningArray));
    ...
    TR::GCStackAtlas* trStackAtlas, uint32_t numberOfMapBytes, TR::Compilation* comp)
{
    ...
    if (map->getInternalPointerMap()) {
        ...
        if (debug("traceIP")) {
            printf("JIT address %x (lowCode %x) method %s with 4 byte offsets (%d) for register map %d (GC map %x ip "
                   "map %x)\n",
                location, lowCode, comp->signature(), fourByteOffsets, map->getRegisterMap(), map,
    ...
    uint32_t atlasSizeInBytes, const GCStackMapSet& nonmergeableBCI)
{
    ...
    vmAtlas->internalPointerMap = createInternalPtrStackMapInJ9Format(
        vm, trStackAtlas->getInternalPointerMap(), trStackAtlas, cg, cursor, comp);
...
static void populateBodyInfo(TR::Compilation* comp, TR_J9VMBase* vm, TR_MethodMetaData* data)
{
    ...
    if (recompInfo) {
        if (vm->isAOT_DEPRECATED_DO_NOT_USE()) {
            ...
            // to be persistent, we need to allocate them seperately. Later, we should probably reconsider what the
            ...
            uint8_t* persistentInfo = vm->allocateDataCacheRecord(
                bytesRequested, comp, true, &retryCompilation, J9_JIT_DCE_AOT_PERSISTENT_INFO, &bytesAllocated);
...
static void populateInlineCalls(
    TR::Compilation* comp, TR_J9VMBase* vm, TR_MethodMetaData* data, uint8_t* callSiteCursor, uint32_t numberOfMapBytes)
{
    TR_Array<List<TR::RegisterMappedSymbol>*>& monitorAutos = comp->getMonitorAutos();
    for (int32_t i = 0; i < comp->getNumInlinedCallSites(); ++i) {
        ...
        if (!vm->isAOT_DEPRECATED_DO_NOT_USE()) // For AOT, we should only have returned resolved info about a method if
                                                // the method came from same class loaders.
        {
            J9Class* j9clazz = (J9Class*)J9_CLASS_FROM_CP(
                ((J9RAMConstantPoolItem*)J9_CP_FROM_METHOD(((J9Method*)inlinedCallSite->_methodInfo))));
            TR_OpaqueClassBlock* clazzOfInlinedMethod
                = ((TR_J9VMBase*)comp->fej9())->convertClassPtrToClassOffset(j9clazz);
            if (comp->fej9()->isUnloadAssumptionRequired(clazzOfInlinedMethod, comp->getCurrentMethod())) {
                if (comp->getOption(TR_AOT) && comp->getOption(TR_TraceRelocatableDataDetailsCG)) {
                    ...
                    traceMsg(comp,
                        "createClassUnloadPicSite: clazzOfInlinedMethod %p, loader = %p, clazzOfCallerMethod %p, "
                        "loader = %p, callsiteCursor %p\n",
                ...
                createClassUnloadPicSite((void*)j9clazz,
                    (void*)(callSiteCursor + (TR::Compiler->target.cpu.isBigEndian() ? 4 : 0)), 4,
                    comp->getMetadataAssumptionList());
#else
                createClassUnloadPicSite(
                    (void*)j9clazz, (void*)callSiteCursor, sizeof(uintptrj_t), comp->getMetadataAssumptionList());
#endif
            }
        }
...
TR_MethodMetaData* createMethodMetaData(TR_J9VMBase& vmArg, TR_ResolvedMethod* vmMethod, TR::Compilation* comp)
{
    ...
    int32_t exceptionsSize = calculateExceptionsSize(
        comp, exceptionIterator, fourByteExceptionTableEntries, numberOfExceptionRangesWithBits);
    ...
    int32_t sizeOfStackAtlasInBytes = calculateSizeOfStackAtlas(
        vm, cg, fourByteOffsets, numberOfSlotsMapped, numberOfMapBytes, comp, nonmergeableBCI);
    ...
    */
    ...
    if (vm->isAOT_DEPRECATED_DO_NOT_USE()) {
        ...
        //
        if (!vm->isMethodExitTracingEnabled(vmMethod->getPersistentIdentifier()) && !vm->canMethodExitEventBeHooked()) {
        ...
        TR_ASSERT(
            comp->getTotalNeededDataCacheSpace() == aotMethodHeaderEntry->compileMethodDataSize, "Size missmatach");
    }
#endif
