...
void TR_Debug::printJ9JITExceptionTableDetails(J9JITExceptionTable* data, J9JITExceptionTable* dbgextRemotePtr)
{
...
void TR_Debug::print(J9JITExceptionTable* data, TR_ResolvedMethod* feMethod, bool fourByteOffsets)
{
    uintptrj_t startPC = (uintptrj_t)data->startPC;
    ...
    if (sa)
        offsetInfo = printStackAtlas(startPC, sa->getAtlasBits(), sa->getNumberOfSlotsMapped(), fourByteOffsets,
    ...
    if (numInlinedCallSites && callSiteCursor) {
        ...
        for (i = 0; i < numInlinedCallSites; i++) {
            ...
            trfprintf(_file,
                "ByteCodeInfo: <_callerIndex=%d, byteCodeIndex=%d>, _isSameReceiver=%d, _doNotProfile=%d\n",
...
int32_t* TR_Debug::printStackAtlas(uintptrj_t startPC, uint8_t* mapBits, int32_t numberOfSlotsMapped,
    bool fourByteOffsets, int32_t* sizeOfStackAtlas, int32_t frameSize)
{
    ...
    uint16_t indexOfFirstInternalPtr = printStackAtlasDetails(
        startPC, mapBits, numberOfSlotsMapped, fourByteOffsets, sizeOfStackAtlas, frameSize, offsetInfo);
    ...
    for (U_32 j = 0; j < stackAtlas->numberOfMaps; j++) {
        ...
        mapBits = printMapInfo(startPC, mapBits, numberOfSlotsMapped, fourByteOffsets, sizeOfStackAtlas, byteCodeInfo,
            indexOfFirstInternalPtr, offsetInfo);
    }

    return offsetInfo;
}

uint16_t TR_Debug::printStackAtlasDetails(uintptrj_t startPC, uint8_t* mapBits, int32_t numberOfSlotsMapped,
...
uint8_t* TR_Debug::printMapInfo(uintptrj_t startPC, uint8_t* mapBits, int32_t numberOfSlotsMapped, bool fourByteOffsets,
    ...
    bool nummaps)
{
    ...
    if (byteCodeInfo->doNotProfile()) {
        ...
        *sizeOfStackAtlas = *sizeOfStackAtlas + sizeof(U_32);
        ...
        if (!nummaps) {
            trfprintf(
                _file, "      registerSaveDescription: starting at [%08X] { %08X }\n", mapBits, *((U_32*)mapBits));
        }
        mapBits += sizeof(U_32);
        *sizeOfStackAtlas = *sizeOfStackAtlas + sizeof(U_32);
        ...
        if ((*(uint32_t*)byteCodeInfo == 0x00000000) && (registerMap == 0xFADECAFE) && !nummaps)
            ...
            if (registerMap & INTERNAL_PTR_REG_MASK) {
                ...
                while (i < numPinningArrays) {
                    ...
                    if (!nummaps)
                        trfprintf(_file,
                            "          number of internal pointers in registers for this pinning array = %d\n",
                            numInternalPointers);
                    uint8_t j = 0;
                    while (j < numInternalPointers) {
                        if (!nummaps)
                            trfprintf(
                                _file, "            internal pointer register number : %d\n", *internalPtrMapCursor);
                        internalPtrMapCursor++;
                        j++;
                    }
                    i++;
...
void TR_Debug::printStackMapInfo(
...
void TR_Debug::printByteCodeStack(int32_t parentStackIndex, uint16_t byteCodeIndex, char* indent)
{
    if (!_comp->fej9()->isAOT_DEPRECATED_DO_NOT_USE()) {
        J9Method* ramMethod;
        ...
        j9bcutil_dumpBytecodes(((TR_J9VMBase*)_comp->fej9())->_portLibrary, J9_CLASS_FROM_METHOD(ramMethod)->romClass,
            J9_BYTECODE_START_FROM_RAM_METHOD(ramMethod), byteCodeIndex, byteCodeIndex, flags, bcPrintFunc, this,
            indent);
