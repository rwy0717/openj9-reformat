...
static JITINLINE void initializeIteratorWithSpecifiedMap(
    TR_MapIterator* i, J9TR_MethodMetaData* methodMetaData, U_8* map, U_32 mapCount)
{
...
static JITINLINE TR_StackMapTable* initializeMapTable(
    J9JavaVM* javaVM, J9TR_MethodMetaData* metaData, UDATA fourByteOffsets)
{
...
static JITINLINE TR_StackMapTable* findOrCreateMapTable(
    J9JavaVM* javaVM, J9TR_MethodMetaData* metaData, UDATA fourByteOffsets)
{
...
static JITINLINE void* currentInlineMap(TR_MapIterator* i) { return i->_currentInlineMap; }

static JITINLINE void* currentStackMap(TR_MapIterator* i) { return i->_currentStackMap; }
...
static JITINLINE void findMapsAtPC(
    TR_MapIterator* i, UDATA offsetPC, void** stackMap, void** inlineMap, UDATA fourByteOffsets)
{
...
static void printMetaData(J9TR_MethodMetaData* methodMetaData)
{
    ...
    for (index = 0; index < iter._stackAtlas->numberOfMaps; ++index) {
        ...
        printf("index: %d map address: %p rangeStartOffset: %x rangeEndOffset: %x is inlineMap? %s\n", index,
            iter._currentMap, iter._rangeStartOffset, iter._rangeEndOffset,
...
static void printMapTable(TR_StackMapTable* stackMapTable, U_8* addressOfFirstMap)
{
    ...
    for (index = 0; index <= stackMapTable->_tableSize; ++index) {
        printf("index: %d map address: %p lowCodeOffset: %x stackMapOffset: %x mapCount: %u\n", index,
            stackMapTable->_table[index]._stackMapOffset + addressOfFirstMap,
...
void jitGetMapsFromPC(
    J9JavaVM* javaVM, J9TR_MethodMetaData* methodMetaData, UDATA jitPC, void** stackMap, void** inlineMap)
{
    TR_MapIterator i;
...
JITINLINE static J9JIT32BitExceptionTableEntry* getNext32BitExceptionDataField(
...
JITINLINE static J9JIT16BitExceptionTableEntry* getNext16BitExceptionDataField(
...
void JITINLINE setJitExceptionHandlerCache(
    TR_jitExceptionHandlerCache* jitExceptionHandlerCache, UDATA pc, J9Class* thrownClass)
{
...
JITINLINE TR_jitExceptionHandlerCache getJitExceptionHandlerCache(
        ...
        if (hasWideExceptions(walkState->jitInfo)) {
            ...
            while (numberOfRanges) {
                if ((deltaPC >= getJit32BitTableEntryStartPC(handlerCursor))
                    ...
                    if (
                        /* { RTSJ Support begins */
                        isExceptionTypeCaughtByHandler(walkState->walkThread, (J9Class*)walkState->userData4,
                            UNTAGGED_METHOD_CP(handlerCursor->ramMethod), handlerCursor->catchType, walkState))
                    /* } RTSJ Support ends */
                    {
                        if (bytecodePCBytes) {
                            walkState->userData1
            ...
            while (numberOfRanges) {
                if ((deltaPC >= getJit16BitTableEntryStartPC(handlerCursor))
                    ...
                    if (isExceptionTypeCaughtByHandler(walkState->walkThread, (J9Class*)walkState->userData4,
                            walkState->constantPool, handlerCursor->catchType, walkState))
                    /* } RTSJ Support ends */
                    {
                        if (bytecodePCBytes) {
                            walkState->userData1
    ...
    */
    ...
    while (i < numDistinctPinningArrays) {
        ...
        swPrintf(walkState, 6,
            ...
            REMOTE_ADDR(currPinningArrayCursor), oldPinningArrayAddress, REMOTE_ADDR(walkState->bp),
            offsetOfFirstInternalPtr);
#endif
        walkState->objectSlotWalkFunction(
            walkState->walkThread, walkState, currPinningArrayCursor, REMOTE_ADDR(currPinningArrayCursor));
...
#endif
        {
            ...
            if (internalPointersInRegisters) {
                ...
                tempJitDescriptionCursorForRegs
                    += 12; /*skip register map, register save description word and HighWordRegisterMap word */
#else
...
JITINLINE J9ConstantPool* getJitConstantPool(J9TR_MethodMetaData* md) { return md->constantPool; }

JITINLINE J9Method* getJitRamMethod(J9TR_MethodMetaData* md) { return md->ramMethod; }

JITINLINE UDATA getJittedMethodStartPC(J9TR_MethodMetaData* md) { return md->startPC; }

JITINLINE UDATA getJittedMethodEndPC(J9TR_MethodMetaData* md) { return md->endPC; }
...
JITINLINE U_16 getJitProloguePushes(J9TR_MethodMetaData* md) { return md->prologuePushes; }

JITINLINE I_16 getJitTempOffset(J9TR_MethodMetaData* md) { return md->tempOffset; }

JITINLINE U_16 getJitNumberOfExceptionRanges(J9TR_MethodMetaData* md) { return md->numExcptionRanges; }

JITINLINE I_32 getJitExceptionTableSize(J9TR_MethodMetaData* md) { return md->size; }
...
JITINLINE I_16 getJitLocalBaseOffset(J9TR_StackAtlas* sa) { return sa->localBaseOffset; }

JITINLINE U_32 getJit32BitTableEntryStartPC(J9JIT32BitExceptionTableEntry* te) { return te->startPC; }

JITINLINE U_32 getJit32BitTableEntryEndPC(J9JIT32BitExceptionTableEntry* te) { return te->endPC; }

JITINLINE U_32 getJit32BitTableEntryHandlerPC(J9JIT32BitExceptionTableEntry* te) { return te->handlerPC; }

JITINLINE U_32 getJit32BitTableEntryCatchType(J9JIT32BitExceptionTableEntry* te) { return te->catchType; }

JITINLINE J9Method* getJit32BitTableEntryRamMethod(J9JIT32BitExceptionTableEntry* te) { return te->ramMethod; }

JITINLINE U_16 getJit16BitTableEntryStartPC(J9JIT16BitExceptionTableEntry* te) { return te->startPC; }

JITINLINE U_16 getJit16BitTableEntryEndPC(J9JIT16BitExceptionTableEntry* te) { return te->endPC; }

JITINLINE U_16 getJit16BitTableEntryHandlerPC(J9JIT16BitExceptionTableEntry* te) { return te->handlerPC; }

JITINLINE U_16 getJit16BitTableEntryCatchType(J9JIT16BitExceptionTableEntry* te) { return te->catchType; }
...
void* getNextInlinedCallSite(J9TR_MethodMetaData* methodMetaData, void* inlinedCallSite)
{
    if (hasMoreInlinedMethods(inlinedCallSite)) {
        return getNotUnloadedInlinedCallSiteArrayElement(
            methodMetaData, ((TR_ByteCodeInfo*)getByteCodeInfo(inlinedCallSite))->_callerIndex);
    }
    return NULL;
}
...
UDATA getCurrentByteCodeIndexAndIsSameReceiver(
    J9TR_MethodMetaData* methodMetaData, void* stackMap, void* currentInlinedCallSite, UDATA* isSameReceiver)
{
