...
TR_RelocationRuntime::TR_RelocationRuntime(J9JITConfig* jitCfg)
{
    ...
    if (!_globalValuesInitialized) {
        ...
        setGlobalValue(
            TR_MethodEnterHookEnabledAddress, (uintptr_t) & (javaVM()->hookInterface.flags[J9HOOK_VM_METHOD_ENTER]));
        setGlobalValue(
            TR_MethodExitHookEnabledAddress, (uintptr_t) & (javaVM()->hookInterface.flags[J9HOOK_VM_METHOD_RETURN]));
    ...
    bool shouldUseCompiledCopy, TR::Options* options, TR::Compilation* comp, TR_ResolvedMethod* resolvedMethod)
{
    ...
    if (_exceptionTableCacheEntry->type == J9_JIT_DCE_EXCEPTION_INFO) {
        ...
        TR_ASSERT(
            codeSize > sizeof(OMR::CodeCacheMethodHeader), "codeSize for AOT loads should include the CodeCacheHeader");
...
//
void TR_RelocationRuntime::relocateAOTCodeAndData(
    U_8* tempDataStart, U_8* oldDataStart, U_8* codeStart, U_8* oldCodeStart)
{
    J9JITDataCacheHeader* cacheEntry = (J9JITDataCacheHeader*)(tempDataStart);
    UDATA startPC = 0;

    RELO_LOG(_reloLogger, 7,
    ...
    if (_exceptionTableCacheEntry->type == J9_JIT_DCE_EXCEPTION_INFO) {
        ...
        getClassNameSignatureFromMethod(
            _method, _exceptionTable->className, _exceptionTable->methodName, _exceptionTable->methodSignature);
        RELO_LOG(reloLogger(), 1, "relocateAOTCodeAndData: method %.*s.%.*s%.*s\n", _exceptionTable->className->length,
            _exceptionTable->className->data, _exceptionTable->methodName->length, _exceptionTable->methodName->data,
            _exceptionTable->methodSignature->length, _exceptionTable->methodSignature->data);
...
bool TR_RelocationRuntime::storeAOTHeader(J9JavaVM* pjavaVM, TR_FrontEnd* fe, J9VMThread* curThread)
{
    TR_ASSERT(0, "Error: storeAOTHeader not supported in this relocation runtime");
    return false;
}

TR_AOTHeader* TR_RelocationRuntime::createAOTHeader(J9JavaVM* pjavaVM, TR_FrontEnd* fe)
{
    TR_ASSERT(0, "Error: createAOTHeader not supported in this relocation runtime");
    return NULL;
}

bool TR_RelocationRuntime::validateAOTHeader(J9JavaVM* pjavaVM, TR_FrontEnd* fe, J9VMThread* curThread)
{
    TR_ASSERT(0, "Error: validateAOTHeader not supported in this relocation runtime");
    return false;
}

void* TR_RelocationRuntime::isROMClassInSharedCaches(UDATA romClassValue, J9JavaVM* pjavaVM)
{
    TR_ASSERT(0, "Error: isROMClassInSharedCaches not supported in this relocation runtime");
    return NULL;
}

bool TR_RelocationRuntime::isRomClassForMethodInSharedCache(J9Method* method, J9JavaVM* pjavaVM)
{
    TR_ASSERT(0, "Error: isRomClassForMethodInSharedCache not supported in this relocation runtime");
    return false;
}

TR_YesNoMaybe TR_RelocationRuntime::isMethodInSharedCache(J9Method* method, J9JavaVM* pjavaVM)
{
    TR_ASSERT(0, "Error: isMethodInSharedCache not supported in this relocation runtime");
    return TR_no;
}

TR_OpaqueClassBlock* TR_RelocationRuntime::getClassFromCP(
    J9VMThread* vmThread, J9JavaVM* pjavaVM, J9ConstantPool* constantPool, I_32 cpIndex, bool isStatic)
{
...
U_8* TR_SharedCacheRelocationRuntime::allocateSpaceInCodeCache(UDATA codeSize)
{
    ...
    if (!codeCache()) {
        ...
        _codeCache
            = manager->reserveCodeCache(false, codeSize, compThreadID, &numReserved); // Acquire a cold/warm code cache.
        if (!codeCache()) {
...
bool TR_SharedCacheRelocationRuntime::useDFPHardware(TR_FrontEnd* fe)
{
...
void TR_SharedCacheRelocationRuntime::incompatibleCache(U_32 module_name, U_32 reason, char* assumeMessage)
{
...
bool TR_SharedCacheRelocationRuntime::generateError(char* assumeMessage)
{
    TR_ASSERT(false, assumeMessage);
...
void TR_SharedCacheRelocationRuntime::checkAOTHeaderFlags(
    TR_FrontEnd* fe, TR_AOTHeader* hdrInCache, intptr_t featureFlags)
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)fe;
    bool defaultMessage = true;

    if (!TR::Compiler->target.cpu.isCompatible(
            (TR_Processor)hdrInCache->processorSignature, hdrInCache->processorFeatureFlags))
...
uint32_t TR_SharedCacheRelocationRuntime::getCurrentLockwordOptionHashValue(J9JavaVM* vm) const
{
...
bool TR_SharedCacheRelocationRuntime::validateAOTHeader(J9JavaVM* pjavaVM, TR_FrontEnd* fe, J9VMThread* curThread)
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)fe;
    ...
    javaVM()->sharedClassConfig->findSharedData(
        curThread, aotHeaderKey, aotHeaderKeyLength, J9SHR_DATA_TYPE_AOTHEADER, FALSE, &firstDescriptor, NULL);
    ...
    if (result) {
        ...
        strncpy(currentVersion.vmBuildVersion, EsBuildVersionString,
            std::min(strlen(EsBuildVersionString), sizeof(currentVersion.vmBuildVersion) - 1));
        strncpy(currentVersion.jitBuildVersion, TR_BUILD_NAME,
            std::min(strlen(TR_BUILD_NAME), sizeof(currentVersion.jitBuildVersion) - 1));
        ...
        if (hdrInCache->eyeCatcher != TR_AOTHeaderEyeCatcher
            ...
            || !TR::Compiler->target.cpu.isCompatible(
                   (TR_Processor)hdrInCache->processorSignature, hdrInCache->processorFeatureFlags)) {
...
TR_AOTHeader* TR_SharedCacheRelocationRuntime::createAOTHeader(J9JavaVM* pjavaVM, TR_FrontEnd* fe)
{
    PORT_ACCESS_FROM_JAVAVM(javaVM());
    ...
    if (aotHeader) {
        ...
        strncpy(aotHeaderVersion->jitBuildVersion, TR_BUILD_NAME,
            std::min(strlen(TR_BUILD_NAME), sizeof(aotHeaderVersion->jitBuildVersion)));
...
#if defined(J9VM_GC_COMPRESSED_POINTERS)
        aotHeader->compressedPointerShift = javaVM()->memoryManagerFunctions->j9gc_objaccess_compressedPointersShift(
            javaVM()->internalVMFunctions->currentVMThread(javaVM()));
#else
...
bool TR_SharedCacheRelocationRuntime::storeAOTHeader(J9JavaVM* pjavaVM, TR_FrontEnd* fe, J9VMThread* curThread)
{
...
void* TR_SharedCacheRelocationRuntime::isROMClassInSharedCaches(UDATA romClassValue, J9JavaVM* pjavaVM)
{
    ...
    while (!matchFound && currentCacheDescriptor) {
        // printf("currentCacheDescriptor: %p, currentCacheDescriptor->cacheStartAddress: %p, cacheSizeBytes: %x,
        // romClassStartAddress: %p, romClassValue :%p, descriptor->next: %p\n", currentCacheDescriptor,
        // currentCacheDescriptor->cacheStartAddress, currentCacheDescriptor->cacheSizeBytes,
        // currentCacheDescriptor->romclassStartAddress, romClassValue, currentCacheDescriptor->next); if
        // (((romClassValue < (UDATA)currentCacheDescriptor->romclassStartAddress +
        // currentCacheDescriptor->cacheSizeBytes)&& (romClassValue >=
        // (UDATA)currentCacheDescriptor->romclassStartAddress))) // Temporarily use romclassStartAddress because
        // cachStartAddress is NULL for new cache
        if (((romClassValue < (UDATA)currentCacheDescriptor->metadataStartAddress)
                && (romClassValue >= (UDATA)currentCacheDescriptor->romclassStartAddress))) {
            matchFound = true;
            break;
        }
...
bool TR_SharedCacheRelocationRuntime::isRomClassForMethodInSharedCache(J9Method* method, J9JavaVM* pjavaVM)
{
#if 1
...
TR_YesNoMaybe TR_SharedCacheRelocationRuntime::isMethodInSharedCache(J9Method* method, J9JavaVM* pjavaVM)
{
...
TR_OpaqueClassBlock* TR_SharedCacheRelocationRuntime::getClassFromCP(
    J9VMThread* vmThread, J9JavaVM* pjavaVM, J9ConstantPool* constantPool, I_32 cpIndex, bool isStatic)
{
    ...
    J9Class* resolvedClass = javaVM()->internalVMFunctions->resolveClassRef(
        vmThread, constantPool, romFieldRef->classRefCPIndex, J9_RESOLVE_FLAG_AOT_LOAD_TIME);
    ...
    if (isStatic) {
        ...
        IDATA fieldOffset = javaVM()->internalVMFunctions->instanceFieldOffset(vmThread, resolvedClass,
            J9UTF8_DATA(name), J9UTF8_LENGTH(name), J9UTF8_DATA(signature), J9UTF8_LENGTH(signature), &definingClass,
            (UDATA*)&field, J9_LOOK_NO_JAVA);
    }
    return (TR_OpaqueClassBlock*)definingClass;
}

uintptr_t TR_SharedCacheRelocationRuntime::generateFeatureFlags(TR_FrontEnd* fe)
{
