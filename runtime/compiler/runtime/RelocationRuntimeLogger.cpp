...
void TR_RelocationRuntimeLogger::setupOptions(TR::Options* options)
{
    ...
    if (options) {
        ...
        _logEnabled = (_logLevel > 0) || options->getOption(TR_TraceRelocatableDataRT)
            || options->getOption(TR_TraceRelocatableDataDetailsRT);
    }
}

void TR_RelocationRuntimeLogger::debug_printf(char* format, ...)
{
    va_list args;
...
void TR_RelocationRuntimeLogger::printf(char* format, ...)
{
    va_list args;
    char outputBuffer[512];
...
void TR_RelocationRuntimeLogger::method(bool newLine)
{
    ...
    U_8* methodName = J9UTF8_DATA(J9ROMMETHOD_GET_NAME(J9_CLASS_FROM_METHOD(reloRuntime()->method())->romClass,
        J9_ROM_METHOD_FROM_RAM_METHOD(reloRuntime()->method())));
    int methodNameLen = J9UTF8_LENGTH(J9ROMMETHOD_GET_NAME(J9_CLASS_FROM_METHOD(reloRuntime()->method())->romClass,
        J9_ROM_METHOD_FROM_RAM_METHOD(reloRuntime()->method())));
    U_8* signature = J9UTF8_DATA(J9ROMMETHOD_GET_SIGNATURE(J9_CLASS_FROM_METHOD(reloRuntime()->method())->romClass,
        J9_ROM_METHOD_FROM_RAM_METHOD(reloRuntime()->method())));
    int signatureLen = J9UTF8_LENGTH(J9ROMMETHOD_GET_SIGNATURE(J9_CLASS_FROM_METHOD(reloRuntime()->method())->romClass,
        J9_ROM_METHOD_FROM_RAM_METHOD(reloRuntime()->method())));

    bool wasLocked = lockLog();
    JITRT_PRINTF(jitConfig())
    (jitConfig(), patternString, classNameLen, className, methodNameLen, methodName, signatureLen, signature);
    unlockLog(wasLocked);
}
...
void TR_RelocationRuntimeLogger::relocationDump()
{
    ...
    if (verbose()) {
        if (0) {
            ...
            header = (TR_RelocationRecord*)((U_8*)reloRuntime()->exceptionTableCacheEntry()
                + reloRuntime()->aotMethodHeaderEntry()->offsetToRelocationDataItems);
...
void TR_RelocationRuntimeLogger::relocationTime()
{
    if (verbose()) {
        ...
        method(false);
        JITRT_PRINTF(jitConfig())
        (jitConfig(), " <%p-%p> ", reloRuntime()->exceptionTable()->startPC, reloRuntime()->exceptionTable()->endPC);
...
{
    char* warningMessageFormat = "AOT major/minor versions don't match the ones of running JVM: aotMajorVersion %d "
                                 "jvmMajorVersion %d aotMinorVersion %d jvmMinorVersion %d   ";
    if (verbose()) {
        bool wasLocked = lockLog();
        JITRT_PRINTF(jitConfig())
        (jitConfig(), warningMessageFormat, reloRuntime()->aotMethodHeaderEntry()->majorVersion,
            TR_AOTMethodHeader_MajorVersion, reloRuntime()->aotMethodHeaderEntry()->minorVersion,
...
void TR_RelocationRuntimeLogger::startTag(const char* tag) { JITRT_PRINTF(jitConfig())(jitConfig(), "<%s>\n", tag); }

void TR_RelocationRuntimeLogger::endTag(const char* tag) { JITRT_PRINTF(jitConfig())(jitConfig(), "</%s>\n", tag); }
