...
{
    TR_ASSERT(
        (_word & HasFailedRecompilation) == 0, "Cannot setHasBeenRecompiled because method has failed recompilation");
...
extern "C" {
TR_UnloadedClassPicSite* createClassUnloadPicSite(
    void* classPointer, void* addressToBePatched, uint32_t size, OMR::RuntimeAssumption** sentinel)
{
    TR_FrontEnd* fe = (TR_FrontEnd*)jitConfig->compilationInfo; // ugly, but codert cannot include VMJ9.h
    return TR_UnloadedClassPicSite::make(fe, trPersistentMemory, (uintptrj_t)classPointer, (uint8_t*)addressToBePatched,
...
void jitAdd32BitPicToPatchOnClassUnload(void* classPointer, void* addressToBePatched)
{
    ...
    if (!createClassUnloadPicSite(
            classPointer, addressToBePatched, 4, (OMR::RuntimeAssumption**)(&metaData->runtimeAssumptionList))
        || debug("failToRegisterPicSitesForClassUnloading")) {
        *(uint32_t*)addressToBePatched = 0x0101dead;
    }
}

void createClassRedefinitionPicSite(
    void* classPointer, void* addressToBePatched, uint32_t size, bool unresolved, OMR::RuntimeAssumption** sentinel)
{
    TR_FrontEnd* fe = (TR_FrontEnd*)jitConfig->compilationInfo; // ugly, but codert cannot include VMJ9.h
    if (unresolved) {
        TR_RedefinedClassUPicSite::make(
            fe, trPersistentMemory, (uintptrj_t)classPointer, (uint8_t*)addressToBePatched, size, sentinel);
    } else {
        TR_RedefinedClassRPicSite::make(
            fe, trPersistentMemory, (uintptrj_t)classPointer, (uint8_t*)addressToBePatched, size, sentinel);
    }
}
...
void jitAdd32BitPicToPatchOnClassRedefinition(void* classPointer, void* addressToBePatched, bool unresolved)
{
    ...
    createClassRedefinitionPicSite(
        classPointer, addressToBePatched, 4, unresolved, (OMR::RuntimeAssumption**)(&metaData->runtimeAssumptionList));
}
}
...
extern "C" OMR::CodeCacheMethodHeader* getCodeCacheMethodHeader_C_callable(
    char* p, int searchLimit, J9JITExceptionTable* metaData);
