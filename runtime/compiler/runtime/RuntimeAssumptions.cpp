...
void TR_PatchNOPedGuardSiteOnClassPreInitialize::compensate(TR_FrontEnd* fe, bool isSMP, void* data)
{
    reclaim();
    TR::PatchNOPedGuardSite::compensate(fe, isSMP, data);
}

uintptrj_t TR_PatchNOPedGuardSiteOnClassPreInitialize::hashCode(char* sig, uint32_t sigLen)
{
    uintptrj_t sum = 0;
    uintptrj_t scale = 1;
    ...
    TR_OpaqueClassBlock* superClassId, TR_OpaqueClassBlock* subClassId)
{
...

    {
        ...
        OMR::RuntimeAssumption** headPtr = table->getBucketPtr(
            RuntimeAssumptionOnClassExtend, TR_RuntimeAssumptionTable::hashCode((uintptrj_t)superClassId));
...
void TR_PersistentCHTable::removeClass(
    TR_FrontEnd* fe, TR_OpaqueClassBlock* classId, TR_PersistentClassInfo* info, bool removeInfo)
{
    if (!info)
        return;
...
bool TR_PersistentCHTable::classGotInitialized(
    TR_FrontEnd* fe, TR_PersistentMemory* persistentMemory, TR_OpaqueClassBlock* classId, TR_PersistentClassInfo* clazz)
{
    if (!clazz)
...

    {
        ...
        TR_RuntimeAssumptionTable* table = persistentMemory->getPersistentInfo()->getRuntimeAssumptionTable();
        OMR::RuntimeAssumption** headPtr = table->getBucketPtr(
            RuntimeAssumptionOnClassPreInitialize, TR_PatchNOPedGuardSiteOnClassPreInitialize::hashCode(sig, sigLen));
    ...
    TR_OpaqueMethodBlock* overriddingMethod, TR_OpaqueMethodBlock* overriddenMethod, int32_t smpFlag)
{
    ...
    TR_RuntimeAssumptionTable* table = persistentMemory->getPersistentInfo()->getRuntimeAssumptionTable();
    OMR::RuntimeAssumption** headPtr = table->getBucketPtr(
        RuntimeAssumptionOnMethodOverride, TR_RuntimeAssumptionTable::hashCode((uintptrj_t)overriddenMethod));
    OMR::RuntimeAssumption* cursor = *headPtr;
    OMR::RuntimeAssumption* prev = 0;
    while (cursor) {
        OMR::RuntimeAssumption* next = cursor->getNext();
...
void TR_PersistentCHTable::classGotRedefined(
    TR_FrontEnd* fe, TR_OpaqueClassBlock* oldClassId, TR_OpaqueClassBlock* newClassId)
{
    ...
    OMR::RuntimeAssumption** headPtr = table->getBucketPtr(
        RuntimeAssumptionOnClassExtend, TR_RuntimeAssumptionTable::hashCode((uintptrj_t)oldClassId));
    OMR::RuntimeAssumption* cursor = *headPtr;
    OMR::RuntimeAssumption* prev = 0;
    while (cursor) {
        OMR::RuntimeAssumption* next = cursor->getNext();
...
void TR_PersistentCHTable::removeAssumptionFromList(
