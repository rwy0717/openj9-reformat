...
public:
    ...
    void initialize(uintptrj_t start, uintptrj_t end)
    {
        _start = start;
        _end = end;
    }
...
protected:
    TR_UnloadedClassPicSite(
        TR_PersistentMemory* pm, uintptrj_t key, uint8_t* picLocation, uint32_t size = sizeof(uintptrj_t))
...
public:
    static TR_UnloadedClassPicSite* make(TR_FrontEnd* fe, TR_PersistentMemory* pm, uintptrj_t key, uint8_t* picLocation,
