    ...
    if (jitConfig) {
        ...
        if (exceptionTable == NULL) {
            ...
            case J9PORT_SIG_FLAG_SIGFPE_INT_DIV_BY_ZERO:
                ...
                if (((J9JITExceptionTable*)1) == exceptionTable) {
                    ...
                    *espPtr += 8 * sizeof(UDATA); /* 7 pushes + ret addr */
                    ...
                    /* add 1 so as to move past the instruction so that the exception thrower will -1 like it would in
                     * all other cases. */
            ...
            case J9PORT_SIG_FLAG_SIGSEGV:
                ...
                infoType = j9sig_info(
                    sigInfo, J9PORT_SIG_SIGNAL, J9PORT_SIG_SIGNAL_INACCESSIBLE_ADDRESS, &infoName, &infoValue);
                if (sigType == J9PORT_SIG_FLAG_SIGSEGV && infoType == J9PORT_SIG_VALUE_ADDRESS) {
                    if (*(UDATA*)infoValue > 0xFFFF) {
                        /* we know where the fault occurred, and it wasn't within the first page. This is an unexpected
                         * error */
                        break;
                    }
                }
            ...
            if (J9PORT_SIG_FLAG_SIGBUS == sigType) {
                ...
                *iarPtr = (UDATA)((void*)&jitHandleInternalErrorTrap);
...
#if (defined(LINUXPPC64) && !defined(__LITTLE_ENDIAN__)) || defined(AIXPPC)
                    *iarPtr
                        = *(UDATA*)((void*)&jitHandleNullPointerExceptionTrap); /* get routine address from TOC entry */
#else
...
#if (defined(LINUXPPC64) && !defined(__LITTLE_ENDIAN__)) || defined(AIXPPC)
                    *iarPtr = *(
                        UDATA*)((void*)&jitHandleArrayIndexOutOfBoundsTrap); /* get routine address from TOC entry */
#else
...
#if (defined(LINUXPPC64) && !defined(__LITTLE_ENDIAN__)) || defined(AIXPPC)
                    *iarPtr
                        = *(UDATA*)((void*)&jitHandleIntegerDivideByZeroTrap); /* get routine address from TOC entry */
#else
                ...
                case TRAP_TYPE_ASYNC_CHECK: {
                    UDATA* lrPtr;
                    infoType
                        = j9sig_info(sigInfo, J9PORT_SIG_CONTROL, J9PORT_SIG_CONTROL_POWERPC_LR, &infoName, &infoValue);
                    lrPtr = (UDATA*)infoValue;
        ...
        if (TRAP_TYPE_UNKNOWN == trapType) {
...
#endif
            {
                ...
                if (FPE_DECOVF == si_code) {
                     ...
                     * is not installed.
                     *
                     * With vector support, the same tree structure evaluates to VPSOP+VTP, the former throws Deciaml
