...
void TR::SymbolValidationManager::defineGuaranteedID(void* symbol)
{
...
void TR::SymbolValidationManager::setSymbolOfID(uint16_t id, void* symbol)
{
...
void* TR::SymbolValidationManager::getSymbolFromID(uint16_t id)
{
    void* symbol = NULL;
    if (id < _idToSymbolTable.size())
...
uint16_t TR::SymbolValidationManager::tryGetIDFromSymbol(void* symbol)
{
...
uint16_t TR::SymbolValidationManager::getIDFromSymbol(void* symbol)
{
...
void* TR::SymbolValidationManager::storeClassChain(TR_J9VMBase* fej9, TR_OpaqueClassBlock* clazz)
{
...
void TR::SymbolValidationManager::storeRecord(void* symbol, TR::SymbolValidationRecord* record)
{
...
bool TR::SymbolValidationManager::storeClassRecord(
    TR_OpaqueClassBlock* clazz, TR::ClassValidationRecord* record, int32_t arrayDimsToValidate, bool storeCurrentRecord)
{
    bool validated = false;
    ...
    if (fej9->isClassArray(clazz)) {
        ...
        if (validated) {
            ...
            SVM_ASSERT(validated && arrayDims == arrayDimsToValidate, "Failed to validate class %p", clazz);
        }
    }
...
bool TR::SymbolValidationManager::storeValidationRecordIfNecessary(
    ...
    if (!existsInList || arrayDimsToValidate) {
        if (record->isClassValidationRecord()) {
            validated = storeClassRecord(static_cast<TR_OpaqueClassBlock*>(symbol),
                reinterpret_cast<TR::ClassValidationRecord*>(record), arrayDimsToValidate, !existsInList);
        } else {
...
bool TR::SymbolValidationManager::addClassFromCPRecord(
    TR_OpaqueClassBlock* clazz, J9ConstantPool* constantPoolOfBeholder, uint32_t cpIndex)
{
    if (!clazz)
        return false;
    if (inHeuristicRegion())
...
bool TR::SymbolValidationManager::addDefiningClassFromCPRecord(
    TR_OpaqueClassBlock* clazz, J9ConstantPool* constantPoolOfBeholder, uint32_t cpIndex, bool isStatic)
{
    if (!clazz)
        return false;
    if (inHeuristicRegion())
...
bool TR::SymbolValidationManager::addStaticClassFromCPRecord(
    TR_OpaqueClassBlock* clazz, J9ConstantPool* constantPoolOfBeholder, uint32_t cpIndex)
{
    if (!clazz)
        return false;
...
bool TR::SymbolValidationManager::addClassFromMethodRecord(TR_OpaqueClassBlock* clazz, TR_OpaqueMethodBlock* method)
{
    if (!clazz)
        return false;
    if (inHeuristicRegion())
...
bool TR::SymbolValidationManager::addComponentClassFromArrayClassRecord(
    TR_OpaqueClassBlock* componentClass, TR_OpaqueClassBlock* arrayClass)
{
    if (!componentClass)
        return false;
    if (inHeuristicRegion())
...
bool TR::SymbolValidationManager::addArrayClassFromComponentClassRecord(
    TR_OpaqueClassBlock* arrayClass, TR_OpaqueClassBlock* componentClass)
{
    if (!arrayClass)
        return false;
    if (inHeuristicRegion())
...
bool TR::SymbolValidationManager::addSuperClassFromClassRecord(
    TR_OpaqueClassBlock* superClass, TR_OpaqueClassBlock* childClass)
{
    if (!superClass)
        return false;
    if (inHeuristicRegion())
    ...
    TR_OpaqueClassBlock* classTwo, bool objectTypeIsFixed, bool castTypeIsFixed, bool isInstanceOf)
{
    ...
    SymbolValidationRecord* record = new (_region)
        ClassInstanceOfClassRecord(classOne, classTwo, objectTypeIsFixed, castTypeIsFixed, isInstanceOf);
...
bool TR::SymbolValidationManager::addClassFromITableIndexCPRecord(
    TR_OpaqueClassBlock* clazz, J9ConstantPool* constantPoolOfBeholder, int32_t cpIndex)
{
    if (!clazz)
        return false;
    if (inHeuristicRegion())
...
bool TR::SymbolValidationManager::addDeclaringClassFromFieldOrStaticRecord(
    TR_OpaqueClassBlock* clazz, J9ConstantPool* constantPoolOfBeholder, int32_t cpIndex)
{
    if (!clazz)
        return false;
...
bool TR::SymbolValidationManager::addClassClassRecord(TR_OpaqueClassBlock* classClass, TR_OpaqueClassBlock* objectClass)
{
    if (!classClass)
        return false;
    if (inHeuristicRegion())
...
bool TR::SymbolValidationManager::addConcreteSubClassFromClassRecord(
    TR_OpaqueClassBlock* childClass, TR_OpaqueClassBlock* superClass)
{
    if (!superClass)
        return false;
    if (inHeuristicRegion())
...
bool TR::SymbolValidationManager::addClassChainRecord(TR_OpaqueClassBlock* clazz, void* classChain)
{
    if (!clazz)
        return false;
    if (inHeuristicRegion())
...
bool TR::SymbolValidationManager::addMethodByNameRecord(TR_OpaqueMethodBlock* method, TR_OpaqueClassBlock* beholder)
{
    if (!method)
        return false;
    if (inHeuristicRegion())
...
bool TR::SymbolValidationManager::addMethodFromClassRecord(
    TR_OpaqueMethodBlock* method, TR_OpaqueClassBlock* beholder, uint32_t index)
{
    if (!method)
        return false;
    if (inHeuristicRegion())
...
bool TR::SymbolValidationManager::addStaticMethodFromCPRecord(
    TR_OpaqueMethodBlock* method, J9ConstantPool* cp, int32_t cpIndex)
{
    if (!method)
        return false;
    if (inHeuristicRegion())
...
bool TR::SymbolValidationManager::addSpecialMethodFromCPRecord(
    TR_OpaqueMethodBlock* method, J9ConstantPool* cp, int32_t cpIndex)
{
    if (!method)
        return false;
...
bool TR::SymbolValidationManager::addVirtualMethodFromCPRecord(
    TR_OpaqueMethodBlock* method, J9ConstantPool* cp, int32_t cpIndex)
{
    if (!method)
        return false;
...
bool TR::SymbolValidationManager::addVirtualMethodFromOffsetRecord(
    TR_OpaqueMethodBlock* method, TR_OpaqueClassBlock* beholder, int32_t virtualCallOffset, bool ignoreRtResolve)
{
    if (!method)
        return false;
    if (inHeuristicRegion())
...
bool TR::SymbolValidationManager::addInterfaceMethodFromCPRecord(
    TR_OpaqueMethodBlock* method, TR_OpaqueClassBlock* beholder, TR_OpaqueClassBlock* lookup, int32_t cpIndex)
{
    if (!method)
        return false;
    if (inHeuristicRegion())
...
bool TR::SymbolValidationManager::addImproperInterfaceMethodFromCPRecord(
    TR_OpaqueMethodBlock* method, J9ConstantPool* cp, int32_t cpIndex)
{
    if (!method)
        return false;
...
bool TR::SymbolValidationManager::addMethodFromClassAndSignatureRecord(
    TR_OpaqueMethodBlock* method, TR_OpaqueClassBlock* methodClass, TR_OpaqueClassBlock* beholder)
{
    if (!method)
        return false;
    if (inHeuristicRegion())
    ...
    TR_YesNoMaybe useGetResolvedInterfaceMethod)
{
    ...
    SymbolValidationRecord* record = new (_region)
        MethodFromSingleImplementer(method, thisClass, cpIndexOrVftSlot, callerMethod, useGetResolvedInterfaceMethod);
    bool valid = storeValidationRecordIfNecessary(static_cast<void*>(method), record);
...
bool TR::SymbolValidationManager::addMethodFromSingleInterfaceImplementerRecord(
    TR_OpaqueMethodBlock* method, TR_OpaqueClassBlock* thisClass, int32_t cpIndex, TR_OpaqueMethodBlock* callerMethod)
{
    if (!method)
        return false;
    if (inHeuristicRegion())
...
bool TR::SymbolValidationManager::addMethodFromSingleAbstractImplementerRecord(
    TR_OpaqueMethodBlock* method, TR_OpaqueClassBlock* thisClass, int32_t vftSlot, TR_OpaqueMethodBlock* callerMethod)
{
    if (!method)
        return false;
    if (inHeuristicRegion())
    ...
    SymbolValidationRecord* record
        = new (_region) MethodFromSingleAbstractImplementer(method, thisClass, vftSlot, callerMethod);
    bool valid = storeValidationRecordIfNecessary(static_cast<void*>(method), record);
...
bool TR::SymbolValidationManager::addStackWalkerMaySkipFramesRecord(
    TR_OpaqueMethodBlock* method, TR_OpaqueClassBlock* methodClass, bool skipFrames)
{
...
bool TR::SymbolValidationManager::validateSymbol(uint16_t idToBeValidated, void* validSymbol)
{
    bool valid = false;
...
bool TR::SymbolValidationManager::validateClassByNameRecord(
    uint16_t classID, uint16_t beholderID, J9ROMClass* romClass, char primitiveType)
{
    TR::Compilation* comp = TR::comp();
    J9VMThread* vmThread = comp->j9VMThread();
    TR_J9VM* fej9 = (TR_J9VM*)TR_J9VMBase::get(vmThread->javaVM->jitConfig, vmThread);
...
bool TR::SymbolValidationManager::validateProfiledClassRecord(
    uint16_t classID, char primitiveType, void* classChainIdentifyingLoader, void* classChainForClassBeingValidated)
{
    TR::Compilation* comp = TR::comp();
    ...
    J9ClassLoader* classLoader = (J9ClassLoader*)comp->fej9()
                                     ->sharedCache()
                                     ->persistentClassLoaderTable()
                                     ->lookupClassLoaderAssociatedWithClassChain(classChainIdentifyingLoader);

    if (classLoader) {
        TR_OpaqueClassBlock* clazz = comp->fej9()->sharedCache()->lookupClassFromChainAndLoader(
            static_cast<uintptrj_t*>(classChainForClassBeingValidated), classLoader);
...
bool TR::SymbolValidationManager::validateDefiningClassFromCPRecord(
    uint16_t classID, uint16_t beholderID, uint32_t cpIndex, bool isStatic)
{
    TR::Compilation* comp = TR::comp();
    J9VMThread* vmThread = comp->j9VMThread();
    TR_J9VM* fej9 = (TR_J9VM*)TR_J9VMBase::get(vmThread->javaVM->jitConfig, vmThread);
    ...
    J9Class* beholder = static_cast<J9Class*>(getSymbolFromID(beholderID));
    J9ConstantPool* beholderCP = J9_CP_FROM_CLASS(beholder);
    TR_OpaqueClassBlock* classFromCP
        = reloRuntime->getClassFromCP(fej9->vmThread(), fej9->_jitConfig->javaVM, beholderCP, cpIndex, isStatic);
...
bool TR::SymbolValidationManager::validateStaticClassFromCPRecord(
    uint16_t classID, uint16_t beholderID, uint32_t cpIndex)
{
    TR::Compilation* comp = TR::comp();
    J9VMThread* vmThread = comp->j9VMThread();
    TR_J9VM* fej9 = (TR_J9VM*)TR_J9VMBase::get(vmThread->javaVM->jitConfig, vmThread);
...
bool TR::SymbolValidationManager::validateClassFromMethodRecord(uint16_t classID, uint16_t methodID)
{
...
bool TR::SymbolValidationManager::validateComponentClassFromArrayClassRecord(
    uint16_t componentClassID, uint16_t arrayClassID)
{
    TR::Compilation* comp = TR::comp();
    J9VMThread* vmThread = comp->j9VMThread();
    TR_J9VM* fej9 = (TR_J9VM*)TR_J9VMBase::get(vmThread->javaVM->jitConfig, vmThread);
...
bool TR::SymbolValidationManager::validateArrayClassFromComponentClassRecord(
    uint16_t arrayClassID, uint16_t componentClassID)
{
    TR::Compilation* comp = TR::comp();
    J9VMThread* vmThread = comp->j9VMThread();
    TR_J9VM* fej9 = (TR_J9VM*)TR_J9VMBase::get(vmThread->javaVM->jitConfig, vmThread);
...
bool TR::SymbolValidationManager::validateClassInstanceOfClassRecord(
    uint16_t classOneID, uint16_t classTwoID, bool objectTypeIsFixed, bool castTypeIsFixed, bool wasInstanceOf)
{
    TR::Compilation* comp = TR::comp();
    J9VMThread* vmThread = comp->j9VMThread();
    TR_J9VM* fej9 = (TR_J9VM*)TR_J9VMBase::get(vmThread->javaVM->jitConfig, vmThread);
...
bool TR::SymbolValidationManager::validateSystemClassByNameRecord(uint16_t systemClassID, J9ROMClass* romClass)
{
    TR::Compilation* comp = TR::comp();
    J9VMThread* vmThread = comp->j9VMThread();
    TR_J9VM* fej9 = (TR_J9VM*)TR_J9VMBase::get(vmThread->javaVM->jitConfig, vmThread);
    ...
    J9UTF8* className = J9ROMCLASS_CLASSNAME(romClass);
    TR_OpaqueClassBlock* systemClassByName = fej9->getSystemClassFromClassName(
...
bool TR::SymbolValidationManager::validateClassFromITableIndexCPRecord(
    uint16_t classID, uint16_t beholderID, uint32_t cpIndex)
{
    TR::Compilation* comp = TR::comp();
    J9VMThread* vmThread = comp->j9VMThread();
    ...
    TR_OpaqueClassBlock* clazz
        = TR_ResolvedJ9Method::getInterfaceITableIndexFromCP(fej9, beholderCP, cpIndex, &pITableIndex);
...
bool TR::SymbolValidationManager::validateDeclaringClassFromFieldOrStaticRecord(
    uint16_t definingClassID, uint16_t beholderID, int32_t cpIndex)
{
    ...
    J9Class* cpClass
        = (J9Class*)TR_ResolvedJ9Method::getClassFromCP(fej9, beholderCP, comp, classCPIndexOfFieldOrStatic);
    ...
    if (cpClass) {
        ...
        vmThread->javaVM->internalVMFunctions->instanceFieldOffset(
            vmThread, cpClass, (U_8*)field, fieldLen, (U_8*)sig, sigLen, &definingClass, NULL, J9_LOOK_NO_JAVA);
    } else {
        return false;
    }
...
bool TR::SymbolValidationManager::validateClassClassRecord(uint16_t classClassID, uint16_t objectClassID)
{
    TR::Compilation* comp = TR::comp();
    J9VMThread* vmThread = comp->j9VMThread();
    TR_J9VM* fej9 = (TR_J9VM*)TR_J9VMBase::get(vmThread->javaVM->jitConfig, vmThread);
...
bool TR::SymbolValidationManager::validateMethodByNameRecord(
    uint16_t methodID, uint16_t beholderID, J9ROMClass* romClass, J9ROMMethod* romMethod)
{
    TR::Compilation* comp = TR::comp();
    J9VMThread* vmThread = comp->j9VMThread();
...
bool TR::SymbolValidationManager::validateStaticMethodFromCPRecord(
    uint16_t methodID, uint16_t beholderID, int32_t cpIndex)
{
    TR::Compilation* comp = TR::comp();
    J9VMThread* vmThread = comp->j9VMThread();
    TR_J9VMBase* fej9 = TR_J9VMBase::get(vmThread->javaVM->jitConfig, vmThread);
...
bool TR::SymbolValidationManager::validateSpecialMethodFromCPRecord(
    uint16_t methodID, uint16_t beholderID, int32_t cpIndex)
{
    TR::Compilation* comp = TR::comp();
    J9VMThread* vmThread = comp->j9VMThread();
...
bool TR::SymbolValidationManager::validateVirtualMethodFromCPRecord(
    uint16_t methodID, uint16_t beholderID, int32_t cpIndex)
{
    TR::Compilation* comp = TR::comp();
    J9VMThread* vmThread = comp->j9VMThread();
    ...
    TR_OpaqueMethodBlock* ramMethod
        = TR_ResolvedJ9Method::getVirtualMethod(fej9, beholderCP, cpIndex, &vTableOffset, &unresolvedInCP);

    return validateSymbol(methodID, static_cast<void*>(ramMethod));
}

bool TR::SymbolValidationManager::validateVirtualMethodFromOffsetRecord(
...
bool TR::SymbolValidationManager::validateInterfaceMethodFromCPRecord(
    uint16_t methodID, uint16_t beholderID, uint16_t lookupID, int32_t cpIndex)
{
    TR::Compilation* comp = TR::comp();
    J9VMThread* vmThread = comp->j9VMThread();
    TR_J9VMBase* fej9 = TR_J9VMBase::get(vmThread->javaVM->jitConfig, vmThread);
...
bool TR::SymbolValidationManager::validateImproperInterfaceMethodFromCPRecord(
    uint16_t methodID, uint16_t beholderID, int32_t cpIndex)
{
    TR::Compilation* comp = TR::comp();
    J9VMThread* vmThread = comp->j9VMThread();
...
bool TR::SymbolValidationManager::validateMethodFromClassAndSignatureRecord(
    uint16_t methodID, uint16_t methodClassID, uint16_t beholderID, J9ROMMethod* romMethod)
{
    TR::Compilation* comp = TR::comp();
    J9VMThread* vmThread = comp->j9VMThread();
    TR_J9VMBase* fej9 = TR_J9VMBase::get(vmThread->javaVM->jitConfig, vmThread);
...
bool TR::SymbolValidationManager::validateMethodFromSingleImplementerRecord(uint16_t methodID, uint16_t thisClassID,
    int32_t cpIndexOrVftSlot, uint16_t callerMethodID, TR_YesNoMaybe useGetResolvedInterfaceMethod)
{
    ...
    TR_ResolvedMethod* calleeResolvedMethod = chTable->findSingleImplementer(
        thisClass, cpIndexOrVftSlot, callerResolvedMethod, comp, false, useGetResolvedInterfaceMethod, false);
...
bool TR::SymbolValidationManager::validateMethodFromSingleInterfaceImplementerRecord(
    uint16_t methodID, uint16_t thisClassID, int32_t cpIndex, uint16_t callerMethodID)
{
    TR::Compilation* comp = TR::comp();
    J9VMThread* vmThread = comp->j9VMThread();
    TR_J9VMBase* fej9 = TR_J9VMBase::get(vmThread->javaVM->jitConfig, vmThread);
    ...
    TR_ResolvedMethod* calleeResolvedMethod
        = chTable->findSingleInterfaceImplementer(thisClass, cpIndex, callerResolvedMethod, comp, false, false);
...
bool TR::SymbolValidationManager::validateMethodFromSingleAbstractImplementerRecord(
    uint16_t methodID, uint16_t thisClassID, int32_t vftSlot, uint16_t callerMethodID)
{
    ...
    TR_ResolvedMethod* callerResolvedMethod = fej9->createResolvedMethod(trMemory, callerMethod, NULL);
    TR_ResolvedMethod* calleeResolvedMethod
        = chTable->findSingleAbstractImplementer(thisClass, vftSlot, callerResolvedMethod, comp, false, false);
...
bool TR::SymbolValidationManager::validateStackWalkerMaySkipFramesRecord(
    uint16_t methodID, uint16_t methodClassID, bool couldSkipFrames)
{
    TR::Compilation* comp = TR::comp();
    J9VMThread* vmThread = comp->j9VMThread();
    TR_J9VMBase* fej9 = TR_J9VMBase::get(vmThread->javaVM->jitConfig, vmThread);
...
void TR::ClassValidationRecord::printFields() { traceMsg(TR::comp(), "\t_classChain=0x%p\n", _classChain); }
