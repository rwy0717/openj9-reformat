...
#define SVM_ASSERT_ALREADY_VALIDATED(svm, symbol)                                                              \
    do {                                                                                                       \
        ...
        SVM_ASSERT_IMPL("SVM_ASSERT_ALREADY_VALIDATED", false, (svm)->isAlreadyValidated(_0symbol),            \
            "isAlreadyValidated(" #symbol ")", "%s %p should have already been validated", #symbol, _0symbol); \
    } while (false);
...
struct ClassByNameRecord : public ClassValidationRecord {
    ClassByNameRecord(TR_OpaqueClassBlock* clazz, TR_OpaqueClassBlock* beholder, char primitiveType)
    ...
    {
        if (_class == rhs._class && _beholder == rhs._beholder && _primitiveType == rhs._primitiveType)
            return true;
        else
            return false;
    }

    virtual bool isEqual(SymbolValidationRecord* other) { return (*this == *static_cast<ClassByNameRecord*>(other)); }
...
struct ProfiledClassRecord : public ClassValidationRecord {
    ProfiledClassRecord(TR_OpaqueClassBlock* clazz, char primitiveType)
    ...
    {
        if (_class == rhs._class && _primitiveType == rhs._primitiveType)
            return true;
        else
            return false;
    }

    virtual bool isEqual(SymbolValidationRecord* other) { return (*this == *static_cast<ProfiledClassRecord*>(other)); }
...
struct ClassFromCPRecord : public ClassValidationRecord {
    ...
    {
        if (_class == rhs._class && _beholder == rhs._beholder && _cpIndex == rhs._cpIndex)
            return true;
        else
            return false;
    }

    virtual bool isEqual(SymbolValidationRecord* other) { return (*this == *static_cast<ClassFromCPRecord*>(other)); }
...
struct DefiningClassFromCPRecord : public ClassValidationRecord {
    DefiningClassFromCPRecord(
        TR_OpaqueClassBlock* clazz, TR_OpaqueClassBlock* beholder, uint32_t cpIndex, bool isStatic)
...
struct ClassFromMethodRecord : public ClassValidationRecord {
    ...
    {
        if (_class == rhs._class && _method == rhs._method)
            return true;
        else
            return false;
    }
...
struct ComponentClassFromArrayClassRecord : public ClassValidationRecord {
    ...
    {
        if (_componentClass == rhs._componentClass && _arrayClass == rhs._arrayClass)
            return true;
        else
            return false;
    }
...
struct ArrayClassFromComponentClassRecord : public ClassValidationRecord {
    ...
    {
        if (_arrayClass == rhs._arrayClass && _componentClass == rhs._componentClass)
            return true;
        else
            return false;
    }
...
struct SuperClassFromClassRecord : public ClassValidationRecord {
    ...
    {
        if (_superClass == rhs._superClass && _childClass == rhs._childClass)
            return true;
        else
            return false;
    }
...
struct ClassInstanceOfClassRecord : public SymbolValidationRecord {
    ...
    {
        if (_classOne == rhs._classOne && _classTwo == rhs._classTwo && _objectTypeIsFixed == rhs._objectTypeIsFixed
            && _castTypeIsFixed == rhs._castTypeIsFixed && _isInstanceOf == rhs._isInstanceOf)
            return true;
        else
            return false;
    }
...
struct ClassClassRecord : public ClassValidationRecord {
    ...
    {
        if (_classClass == rhs._classClass && _objectClass == rhs._objectClass)
            return true;
        else
            return false;
    }

    virtual bool isEqual(SymbolValidationRecord* other) { return (*this == *static_cast<ClassClassRecord*>(other)); }
...
struct ConcreteSubClassFromClassRecord : public ClassValidationRecord {
    ...
    {
        if (_childClass == rhs._childClass && _superClass == rhs._superClass)
            return true;
        else
            return false;
    }
...
struct ClassChainRecord : public SymbolValidationRecord {
    ...
    {
        if (_class == rhs._class && _classChain == rhs._classChain)
            return true;
        else
            return false;
    }

    virtual bool isEqual(SymbolValidationRecord* other) { return (*this == *static_cast<ClassChainRecord*>(other)); }
...
struct MethodByNameRecord : public SymbolValidationRecord {
    ...
    {
        if (_method == rhs._method && _beholder == rhs._beholder)
            return true;
        else
            return false;
    }

    virtual bool isEqual(SymbolValidationRecord* other) { return (*this == *static_cast<MethodByNameRecord*>(other)); }
...
struct MethodFromClassRecord : public SymbolValidationRecord {
    ...
    {
        if (_method == rhs._method && _beholder == rhs._beholder && _index == rhs._index)
            return true;
        else
            return false;
    }
...
struct StaticMethodFromCPRecord : public SymbolValidationRecord {
    ...
    {
        if (_method == rhs._method && _beholder == rhs._beholder && _cpIndex == rhs._cpIndex)
            return true;
        else
            return false;
    }
...
struct VirtualMethodFromOffsetRecord : public SymbolValidationRecord {
    ...
    {
        if (_method == rhs._method && _beholder == rhs._beholder && _virtualCallOffset == rhs._virtualCallOffset
...
struct InterfaceMethodFromCPRecord : public SymbolValidationRecord {
    ...
    {
        if (_method == rhs._method && _beholder == rhs._beholder && _lookup == rhs._lookup && _cpIndex == rhs._cpIndex)
            return true;
        else
            return false;
    }
...
struct MethodFromClassAndSigRecord : public SymbolValidationRecord {
    ...
    {
        if (_method == rhs._method && _methodClass == rhs._methodClass && _beholder == rhs._beholder)
            return true;
        else
            return false;
    }
...
struct StackWalkerMaySkipFramesRecord : public SymbolValidationRecord {
    ...
    {
        if (_method == rhs._method && _methodClass == rhs._methodClass && _skipFrames == rhs._skipFrames)
            return true;
        else
            return false;
    }
...
struct ClassInfoIsInitialized : public SymbolValidationRecord {
    ...
    {
        if (_class == rhs._class && _isInitialized == rhs._isInitialized)
            return true;
        else
            return false;
    }
...
struct MethodFromSingleImplementer : public SymbolValidationRecord {
    ...
    {
        if (_method == rhs._method && _thisClass == rhs._thisClass && _cpIndexOrVftSlot == rhs._cpIndexOrVftSlot
...
struct MethodFromSingleInterfaceImplementer : public SymbolValidationRecord {
    ...
    {
        if (_method == rhs._method && _thisClass == rhs._thisClass && _cpIndex == rhs._cpIndex
            && _callerMethod == rhs._callerMethod)
            return true;
        else
            return false;
...
struct MethodFromSingleAbstractImplementer : public SymbolValidationRecord {
    ...
    {
        if (_method == rhs._method && _thisClass == rhs._thisClass && _vftSlot == rhs._vftSlot
            && _callerMethod == rhs._callerMethod)
            return true;
        else
            return false;
...
public:
    ...
    bool isAlreadyValidated(void* symbol) { return inHeuristicRegion() || tryGetIDFromSymbol(symbol) != NO_ID; }
    ...
    bool addDefiningClassFromCPRecord(
        TR_OpaqueClassBlock* clazz, J9ConstantPool* constantPoolOfBeholder, uint32_t cpIndex, bool isStatic = false);
    bool addStaticClassFromCPRecord(
    ...
    bool addClassFromITableIndexCPRecord(
        TR_OpaqueClassBlock* clazz, J9ConstantPool* constantPoolOfBeholder, int32_t cpIndex);
    bool addDeclaringClassFromFieldOrStaticRecord(
        TR_OpaqueClassBlock* clazz, J9ConstantPool* constantPoolOfBeholder, int32_t cpIndex);
    ...
    bool addVirtualMethodFromOffsetRecord(
        TR_OpaqueMethodBlock* method, TR_OpaqueClassBlock* beholder, int32_t virtualCallOffset, bool ignoreRtResolve);
    bool addInterfaceMethodFromCPRecord(
        TR_OpaqueMethodBlock* method, TR_OpaqueClassBlock* beholder, TR_OpaqueClassBlock* lookup, int32_t cpIndex);
    bool addImproperInterfaceMethodFromCPRecord(TR_OpaqueMethodBlock* method, J9ConstantPool* cp, int32_t cpIndex);
    bool addMethodFromClassAndSignatureRecord(
        TR_OpaqueMethodBlock* method, TR_OpaqueClassBlock* methodClass, TR_OpaqueClassBlock* beholder);
    ...
    bool addStackWalkerMaySkipFramesRecord(
    ...
    bool validateProfiledClassRecord(uint16_t classID, char primitiveType, void* classChainIdentifyingLoader,
    ...
    bool validateClassInstanceOfClassRecord(
    ...
    bool validateMethodByNameRecord(
    ...
    bool validateMethodFromClassAndSignatureRecord(
        uint16_t methodID, uint16_t methodClassID, uint16_t beholderID, J9ROMMethod* romMethod);
    bool validateMethodFromSingleImplementerRecord(uint16_t methodID, uint16_t thisClassID, int32_t cpIndexOrVftSlot,
        uint16_t callerMethodID, TR_YesNoMaybe useGetResolvedInterfaceMethod);
    bool validateMethodFromSingleInterfaceImplementerRecord(
        uint16_t methodID, uint16_t thisClassID, int32_t cpIndex, uint16_t callerMethodID);
    bool validateMethodFromSingleAbstractImplementerRecord(
...
private:
    ...
    bool storeClassRecord(TR_OpaqueClassBlock* clazz, ClassValidationRecord* record, int32_t arrayDimsToValidate,
        bool storeCurrentRecord);
    bool storeValidationRecordIfNecessary(
        void* symbol, SymbolValidationRecord* record, int32_t arrayDimsToValidate = 0);
