...
void ppcCreateHelperTrampolines(uint8_t* trampPtr, int32_t numHelpers)
{
    ...
    static TR_Processor proc = customP4 ? portLibCall_getProcessorType() : TR_DefaultPPCProcessor;
...
bool ppcCodePatching(void* method, void* callSite, void* currentPC, void* currentTramp, void* newPC, void* extra)
{
    ...
    if ((oldBits & 0xFC000001) == 0x48000001
        ...
        oldBits &= 0xfc000003;
        if (TR::Options::getCmdLineOptions()->getOption(TR_StressTrampolines) || distance > BRANCH_FORWARD_LIMIT
            || distance < BRANCH_BACKWARD_LIMIT) {
            if (currentPC == newPC) {
                distance = (uint8_t*)currentTramp - patchAddr;
            } else {
                void* newTramp = mcc_replaceTrampoline(
                    reinterpret_cast<TR_OpaqueMethodBlock*>(method), callSite, currentTramp, currentPC, newPC, true);
                if (newTramp == NULL) {
    ...
    } else if (oldBits == 0x4e800421 || isLinkStackPreservingIPIC) // It is a bctrl or an IPIC bl
    {
        ...
        if (oldBits == 0x7d6903a6) // mtctr r11 used in interface dispatch
        {
            ...
            oldBits = *(
                int32_t*)((uint8_t*)callSite - encodingStartOffset); // The load of the first IPIC cache slot or rldimi
#if defined(TR_TARGET_64BIT)
            if (((oldBits >> 26) & 0x0000003F) != 30) {
                ...
                // PTOC was full and the load address is formed via 5 instructions: lis, lis, ori, rldimi, ldu; oldBits
                // is the rldimi
                ...
                distance += ((intptrj_t)(*(int32_t*)((uint8_t*)callSite - encodingStartOffset + 4)) & 0x0000FFFC) << 48
                    >> 48;
...
int32_t amd64CodePatching(
    void* theMethod, void* callSite, void* currentPC, void* currentTramp, void* newPC, void* extra)
{
    ...
    if (TR::Options::getCmdLineOptions()->getOption(TR_StressTrampolines) || !IS_32BIT_RIP(distance, 0)) {
        if (currentPC == newPC) {
            ...
            void* newTramp = mcc_replaceTrampoline(
                reinterpret_cast<TR_OpaqueMethodBlock*>(method), callSite, currentTramp, currentPC, newPC, false);
...
bool armCodePatching(
    void* callee, void* callSite, void* currentPC, void* currentTramp, void* newAddrOfCallee, void* extra)
{
    ...
    if (TR::Options::getCmdLineOptions()->getOption(TR_StressTrampolines) || distance > (intptrj_t)BRANCH_FORWARD_LIMIT
        || distance < (intptrj_t)BRANCH_BACKWARD_LIMIT) {
...
bool s390zOS64CodePatching(void* method, void* callSite, void* currentPC, void* currentTramp, void* newPC, void* extra)
{
    ...
    //#define CHECK_32BIT_TRAMPOLINE_RANGE(x,rip)  (((intptrj_t)(x) == (intptrj_t)(rip) + (int32_t)((intptrj_t)(x) -
    //(intptrj_t)(rip))) && (x % 2 == 0))
...
bool s390zLinux64CodePatching(
    void* method, void* callSite, void* currentPC, void* currentTramp, void* newPC, void* extra)
{
    TR_LinkageInfo* linkInfo = TR_LinkageInfo::get(newPC);
    // The location of the method call branch.
    uint8_t* entryAddress = (uint8_t*)newPC + linkInfo->getReservedWord();
