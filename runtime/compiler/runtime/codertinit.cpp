...
J9JITConfig* codert_onload(J9JavaVM* javaVM)
{
    ...
    if (J9HookInitializeInterface(
            J9_HOOK_INTERFACE(jitConfig->hookInterface), OMRPORTLIB, sizeof(jitConfig->hookInterface)))
        goto _abort;
    ...
    (*vmHooks)->J9HookRegisterWithCallSite(
        vmHooks, J9HOOK_VM_ABOUT_TO_BOOTSTRAP, codertOnBootstrap, OMR_GET_CALLSITE(), NULL);

    if ((*vmHooks)->J9HookRegisterWithCallSite(
            vmHooks, J9HOOK_VM_SHUTTING_DOWN, codertShutdown, OMR_GET_CALLSITE(), NULL)) {
