...
int32_t TR::AMD64JNILinkage::computeMemoryArgSize(
    TR::Node* callNode, int32_t first, int32_t last, int8_t direction, bool isJNI, bool passThread)
{
    int32_t size = 0;
...
int32_t TR::AMD64JNILinkage::buildArgs(
    TR::Node* callNode, TR::RegisterDependencyConditions* deps, bool isJNI, bool passThread, bool passReceiver)
{
...
TR::Register* TR::AMD64JNILinkage::buildVolatileAndReturnDependencies(
...
void TR::AMD64JNILinkage::buildJNICallOutFrame(TR::Node* callNode, TR::LabelSymbol* returnAddrLabel)
{
    ...
    //
    generateMemImmInstruction(SMemImm4(), callNode,
    ...
    //    0:   RAM method
    //
    TR::X86RegImmInstruction* instr = generateRegImmInstruction(SUBRegImms(), callNode, espReal,
    ...
    TR::MemoryReference* tempMR
        = generateX86MemoryReference(espReal, 4 * TR::Compiler->om.sizeofReferenceAddress(), cg());
    ...
    TR::AMD64RegImm64SymInstruction* returnAddressInstr = generateRegImm64SymInstruction(MOV8RegImm64, callNode,
    ...
    generateMemRegInstruction(S8MemReg, callNode,
        generateX86MemoryReference(espReal, 2 * TR::Compiler->om.sizeofReferenceAddress(), cg()), scratchReg, cg());
    ...
    //
    static const int reloTypes[]
        = { TR_VirtualRamMethodConst, 0 /*Interfaces*/, TR_StaticRamMethodConst, TR_SpecialRamMethodConst };
...
void TR::AMD64JNILinkage::buildJNIMergeLabelDependencies(TR::Node* callNode, bool killNonVolatileGPRs)
{
...
void TR::AMD64JNILinkage::buildOutgoingJNIArgsAndDependencies(
    TR::Node* callNode, bool passThread, bool passReceiver, bool killNonVolatileGPRs)
{
...
TR::Instruction* TR::AMD64JNILinkage::generateMethodDispatch(
    TR::Node* callNode, bool isJNIGCPoint, uintptrj_t targetAddress)
{
    TR::ResolvedMethodSymbol* callSymbol = callNode->getSymbol()->castToResolvedMethodSymbol();
    TR::RealRegister* espReal = machine()->getRealRegister(TR::RealRegister::esp);
    TR::Register* vmThreadReg = cg()->getMethodMetaDataRegister();
    ...
    //
    generateRegMemInstruction(LRegMem(), callNode, vmThreadReg,
    ...
    TR::X86RegInstruction* patchedInstr = generateRegImm64Instruction(
        MOV8RegImm64, callNode, _JNIDispatchInfo.dispatchTrampolineRegister, targetAddress, cg(), reloTypes[reloType]);

    TR::X86RegInstruction* instr = generateRegInstruction(
        CALLReg, callNode, _JNIDispatchInfo.dispatchTrampolineRegister, _JNIDispatchInfo.callPostDeps, cg());
    ...
    if (!cg()->getJNILinkageCalleeCleanup()) {
        ...
        if (TR::Compiler->target.is64Bit())
            TR_ASSERT(
                cleanUpSize <= 0x7fffffff, "Caller cleanup argument size too large for one instruction on AMD64.");
...
void TR::AMD64JNILinkage::switchToJavaStack(TR::Node* callNode)
{
    ...
    //
    generateRegMemInstruction(LRegMem(), callNode, espReal,
        generateX86MemoryReference(vmThreadReg, fej9->thisThreadGetJavaSPOffset(), cg()), cg());
}
...
void TR::AMD64JNILinkage::releaseVMAccess(TR::Node* callNode)
{
    ...
    generateRegMemInstruction(LRegMem(), callNode, scratchReg1,
...
void TR::AMD64JNILinkage::releaseVMAccessAtomicFree(TR::Node* callNode)
{
    ...
    generateMemImmInstruction(S8MemImm4, callNode,
        generateX86MemoryReference(vmThreadReg, offsetof(struct J9VMThread, inNative), cg()), 1, cg());

#if !defined(J9VM_INTERP_ATOMIC_FREE_JNI_USES_FLUSH)
    TR::MemoryReference* mr
        = generateX86MemoryReference(cg()->machine()->getRealRegister(TR::RealRegister::esp), intptrj_t(0), cg());
...
void TR::AMD64JNILinkage::cleanupReturnValue(
    TR::Node* callNode, TR::Register* linkageReturnReg, TR::Register* targetReg)
{
...
void TR::AMD64JNILinkage::checkForJNIExceptions(TR::Node* callNode)
{
    ...
    generateRegMemInstruction(LRegMem(), callNode, scratchReg,
        generateX86MemoryReference(vmThreadReg, fej9->thisThreadGetCurrentExceptionOffset(), cg()), cg());
    ...
    TR::Snippet* snippet = new (trHeapMemory()) TR::X86CheckFailureSnippet(cg(),
        cg()->symRefTab()->findOrCreateRuntimeHelper(TR_throwCurrentException, false, false, false), snippetLabel,
        instr, _JNIDispatchInfo.requiresFPstackPop);
...
void TR::AMD64JNILinkage::cleanupJNIRefPool(TR::Node* callNode)
{
    ...
    if (TR::Compiler->target.is64Bit() && (flagValue > 0x7fffffff)) {
        ...
        generateMemRegInstruction(
            TEST8MemReg, callNode, generateX86MemoryReference(espReal, flagsOffset, cg()), scratchReg, cg());
        cg()->stopUsingRegister(scratchReg);
    } else {
        TR_X86OpCodes op = (flagValue <= 255) ? TEST1MemImm1 : (TESTMemImm4());
        generateMemImmInstruction(
            op, callNode, generateX86MemoryReference(espReal, flagsOffset, cg()), flagValue, cg());
    }
...
TR::Register* TR::AMD64JNILinkage::buildDirectDispatch(TR::Node* callNode, bool spillFPRegs)
{
...
void TR::AMD64JNILinkage::populateJNIDispatchInfo()
{
...
TR::Register* TR::AMD64JNILinkage::buildDirectJNIDispatch(TR::Node* callNode)
{
    ...
    if (debug("reportJNI")) {
        printf("AMD64 JNI Dispatch: %s calling %s\n", comp()->signature(),
            comp()->getDebug()->getName(callNode->getSymbolReference()));
    }
#endif
    TR::SymbolReference* callSymRef = callNode->getSymbolReference();
    TR::MethodSymbol* callSymbol = callSymRef->getSymbol()->castToMethodSymbol();
    ...
    if (resolvedMethod->returnType() == TR::Address && wrapRefs) {
        ...
        generateRegMemInstruction(LRegMem(), callNode, targetReg, generateX86MemoryReference(targetReg, 0, cg()), cg());
    ...
    if (createJNIFrame) {
        generateRegMemInstruction(ADDRegMem(), callNode, espReal,
    ...
    //
    if (createJNIFrame) {
        TR::X86RegImmInstruction* instr = generateRegImmInstruction(ADDRegImms(), callNode, espReal,
            _JNIDispatchInfo.numJNIFrameSlotsPushed * TR::Compiler->om.sizeofReferenceAddress(), cg());
    }
