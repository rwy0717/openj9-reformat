...
static uint8_t* flushArgument(
    uint8_t* cursor, TR_X86OpCodes op, TR::RealRegister::RegNum regIndex, int32_t offset, TR::CodeGenerator* cg)
{
...
static int32_t flushArgumentSize(TR_X86OpCodes op, int32_t offset)
{
    int32_t size = TR_X86OpCode(op).length() + 1; // length including ModRM + 1 SIB
    return size + (((offset >= -128 && offset <= 127)) ? 1 : 4);
}

uint8_t* TR::AMD64PrivateLinkage::flushArguments(TR::Node* callNode, uint8_t* cursor, bool calculateSizeOnly,
...
TR::Instruction* TR::AMD64PrivateLinkage::generateFlushInstruction(TR::Instruction* prev,
...
TR::Instruction* TR::AMD64PrivateLinkage::flushArguments(
    TR::Instruction* prev, TR::ResolvedMethodSymbol* methodSymbol, bool isReturnAddressOnStack, bool isLoad)
{
    ...
    for (parmCursor = parameterIterator.getFirst(); parmCursor; parmCursor = parameterIterator.getNext()) {
        ...
        case TR::Address:
            offset -= slotSize;
            if (numGPArgs < getProperties().getNumIntegerArgumentRegisters()) {
                prev = generateFlushInstruction(
                    prev, ot, dt, getProperties().getIntegerArgumentRegister(numGPArgs), espReg, offset, cg());
            }
        ...
        case TR::Float:
            ...
            if (numFPArgs < getProperties().getNumFloatArgumentRegisters()) {
                prev = generateFlushInstruction(
                    prev, ot, dt, getProperties().getFloatArgumentRegister(numFPArgs), espReg, offset, cg());
            }
...
uint8_t* TR::AMD64PrivateLinkage::generateVirtualIndirectThunk(TR::Node* callNode)
{
    ...
    case TR::NoType:
        glueSymRef
            = cg()->symRefTab()->findOrCreateRuntimeHelper(TR_AMD64icallVMprJavaSendVirtual0, false, false, false);
        break;
    case TR::Int64:
        glueSymRef
            = cg()->symRefTab()->findOrCreateRuntimeHelper(TR_AMD64icallVMprJavaSendVirtualJ, false, false, false);
        break;
    case TR::Address:
        glueSymRef
            = cg()->symRefTab()->findOrCreateRuntimeHelper(TR_AMD64icallVMprJavaSendVirtualL, false, false, false);
        break;
    case TR::Int32:
        glueSymRef
            = cg()->symRefTab()->findOrCreateRuntimeHelper(TR_AMD64icallVMprJavaSendVirtual1, false, false, false);
        break;
    case TR::Float:
        glueSymRef
            = cg()->symRefTab()->findOrCreateRuntimeHelper(TR_AMD64icallVMprJavaSendVirtualF, false, false, false);
        break;
    case TR::Double:
        glueSymRef
            = cg()->symRefTab()->findOrCreateRuntimeHelper(TR_AMD64icallVMprJavaSendVirtualD, false, false, false);
        break;
    default:
...
TR_J2IThunk* TR::AMD64PrivateLinkage::generateInvokeExactJ2IThunk(TR::Node* callNode, char* signature)
{
    ...
    case TR::Address:
        glueSymRef
            = cg()->symRefTab()->findOrCreateRuntimeHelper(TR_icallVMprJavaSendInvokeExactL, false, false, false);
        break;
    case TR::Int32:
        glueSymRef
...
void TR::AMD64PrivateLinkage::mapIncomingParms(TR::ResolvedMethodSymbol* method)
{
    ...
    for (parmCursor = parameterIterator.getFirst(); parmCursor; parmCursor = parameterIterator.getNext()) {
        offset
            -= parmCursor->getRoundedSize() * ((DOUBLE_SIZED_ARGS && parmCursor->getDataType() != TR::Address) ? 2 : 1);
...
TR::Instruction* TR::AMD64PrivateLinkage::savePreservedRegisters(TR::Instruction* cursor)
{
    ...
    cursor = cg()->generateDebugCounter(
        cursor, "cg.prologues:no-preservedRegStoreBytesSaved", 1, TR::DebugCounter::Expensive);
...
TR::Instruction* TR::AMD64PrivateLinkage::restorePreservedRegisters(TR::Instruction* cursor)
{
    ...
    cursor = cg()->generateDebugCounter(
        cursor, "cg.epilogues:no-preservedRegStoreBytesSaved", 1, TR::DebugCounter::Expensive);
    return cursor;
}
...
int32_t TR::AMD64PrivateLinkage::buildArgs(TR::Node* callNode, TR::RegisterDependencyConditions* dependencies)
{
...
int32_t TR::AMD64PrivateLinkage::buildPrivateLinkageArgs(
    TR::Node* callNode, TR::RegisterDependencyConditions* dependencies, bool rightToLeft, bool passArgsOnStack)
{
    ...
    if (!getProperties().getReservesOutgoingArgsInPrologue()
        ...
        if (alignedParmAreaSize > 0)
            generateRegImmInstruction((alignedParmAreaSize <= 127 ? SUBRegImms() : SUBRegImm4()), callNode,
    ...
    for (i = start; i != stop; i += step) {
        ...
        default: {
            ...
            if (rregIndex == noReg) {
                rregIndex = (numIntArgs < NUM_INTEGER_LINKAGE_REGS) && (!passArgsOnStack || createDummyLinkageRegisters)
        ...
        if (rregIndex != noReg) {
            ...
            if (/*!child->getOpCode().isLoadConst() &&*/ (child->getReferenceCount() > 1)) {
                ...
                generateRegRegInstruction(
                    needsZeroExtension ? MOVZXReg8Reg4 : TR::Linkage::movOpcodes(RegReg, movType(child->getDataType())),
        ...
        if ((rregIndex == noReg) || (rregIndex != noReg && createDummyLinkageRegisters)) {
            if (vreg)
                ...
                generateMemImmInstruction(S8MemImm4, child,
    ...
    //
    TR_ASSERT(
        numIntArgs + numFloatArgs + numSpecialArgs == callNode->getNumChildren() - callNode->getFirstArgumentIndex(),
        "assertion failure");
...
TR::Instruction* TR::AMD64PrivateLinkage::buildPICSlot(
    TR::X86PICSlot picSlot, TR::LabelSymbol* mismatchLabel, TR::LabelSymbol* doneLabel, TR::X86CallSite& site)
{
    ...
    if (picSlot.getMethodAddress()) {
        ...
        firstInstruction
            = generateRegImm64Instruction(MOV8RegImm64, node, cachedAddressRegister, addrToBeCompared, cg());
    } else
        firstInstruction = generateRegImm64Instruction(
            MOV8RegImm64, node, cachedAddressRegister, (uint64_t)picSlot.getClassAddress(), cg());
    ...
    if (picSlot.getMethodAddress())
        generateMemRegInstruction(
            CMP8MemReg, node, generateX86MemoryReference(vftReg, picSlot.getSlot(), cg()), cachedAddressRegister, cg());
    else
    ...
    if (picSlot.getMethod()) {
        ...
        instr = generateImmSymInstruction(
            CALLImm4, node, (intptrj_t)picSlot.getMethod()->startAddressForJittedMethod(), callSymRef, cg());
        ...
        instr = generateImmSymInstruction(CALLImm4, node, (uint32_t)(uintptrj_t)helperMethod->getMethodAddress(),
...
void TR::AMD64PrivateLinkage::buildIPIC(
    TR::X86CallSite& site, TR::LabelSymbol* entryLabel, TR::LabelSymbol* doneLabel, uint8_t* thunk)
{
    ...
    if (useLastITableCache
        ...
        && performTransformation(comp(), "O^O useLastITableCache for n%dn itableIndex=%d: %.*s.%.*s%.*s\n",
               site.getCallNode()->getGlobalIndex(), (int)itableIndex, method->classNameLength(),
...
void TR::AMD64PrivateLinkage::buildVirtualOrComputedCall(
    TR::X86CallSite& site, TR::LabelSymbol* entryLabel, TR::LabelSymbol* doneLabel, uint8_t* thunk)
{
    ...
    if (comp()->getOption(TR_TraceCG)) {
        traceMsg(comp(), "buildVirtualOrComputedCall(%p), isComputed=%d\n", site.getCallNode(),
            methodSymRef->getSymbol()->castToMethodSymbol()->isComputed());
    }
    ...
    if (methodSymRef->getSymbol()->castToMethodSymbol()->isComputed()) {
        ...
        //
        buildVFTCall(
            site, CALLMem, NULL, generateX86MemoryReference(site.evaluateVFT(), methodSymRef->getOffset(), cg()));
    } else {
        site.evaluateVFT(); // We must evaluate the VFT here to avoid a later evaluation that pollutes the VPic shape.
        buildVPIC(site, entryLabel, doneLabel);
    }
