    ...
    *buffer++ = 0xe8;
    ...
    if (helperAddress && IS_32BIT_RIP(helperAddress, (buffer + 4))) {
        ...
        TR_RuntimeHelper helper
            = (comp->getOption(TR_EnableNewX86PrefetchTLH)) ? TR_X86newPrefetchTLH : TR_X86prefetchTLH;
        ...
        if (fej9->helpersNeedRelocation()) {
            cg()->addExternalRelocation(new (cg()->trHeapMemory()) TR::ExternalRelocation(
                                            buffer, (uint8_t*)helperSymRef, TR_HelperAddress, cg()),
...
template <TR::HeapTypes::Type>
struct vmThreadHeapOffsets;

template <>
struct vmThreadHeapOffsets<TR::HeapTypes::ZeroedHeap> {
...
template <>
struct vmThreadHeapOffsets<TR::HeapTypes::NonZeroedHeap> {
    ...
    uint8_t* CCPreLoadedCodeBase, uint8_t* CCPreLoadedCodeTop, void** CCPreLoadedCodeTable, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.is64Bit())
        cursor = TR::X86AllocPrefetchSnippet::emitSharedBody<TR::HeapTypes::ZeroedHeap, true>(
            cursor, cg->getX86ProcessorInfo());
    else
        cursor = TR::X86AllocPrefetchSnippet::emitSharedBody<TR::HeapTypes::ZeroedHeap, false>(
            cursor, cg->getX86ProcessorInfo());
    ...
    if (TR::Compiler->target.is64Bit())
        cursor = TR::X86AllocPrefetchSnippet::emitSharedBody<TR::HeapTypes::NonZeroedHeap, true>(
            cursor, cg->getX86ProcessorInfo());
    else
        cursor = TR::X86AllocPrefetchSnippet::emitSharedBody<TR::HeapTypes::NonZeroedHeap, false>(
            cursor, cg->getX86ProcessorInfo());
