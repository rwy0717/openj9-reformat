    ...
    *(uintptrj_t*)cursor = cpAddr;
    ...
    if (_hasJ2IThunkInPicData) {
        ...
        cg()->addExternalRelocation(new (cg()->trHeapMemory()) TR::ExternalRelocation(
                                        cursor, (uint8_t*)info, NULL, TR_J2IVirtualThunkPointer, cg()),
            __FILE__, __LINE__, _startOfPicInstruction->getNode());
    } else if (_thunkAddress) {
        TR_ASSERT(TR::Compiler->target.is64Bit(), "expecting a 64-bit target for thunk relocations");
        cg()->addExternalRelocation(new (cg()->trHeapMemory()) TR::ExternalRelocation(
                                        cursor, *(uint8_t**)cursor, (uint8_t*)inlinedSiteIndex, TR_Thunks, cg()),
            __FILE__, __LINE__, _startOfPicInstruction->getNode());
    } else {
        cg()->addExternalRelocation(new (cg()->trHeapMemory()) TR::ExternalRelocation(
                                        cursor, (uint8_t*)cpAddr, (uint8_t*)inlinedSiteIndex, TR_ConstantPool, cg()),
            __FILE__, __LINE__, _startOfPicInstruction->getNode());
    }
...
uint8_t* TR::X86PicDataSnippet::emitSnippetBody()
{
    ...
    if (isInterface()) {
        ...
        *(int32_t*)cursor = disp32;
        ...
        cg()->addExternalRelocation(new (cg()->trHeapMemory()) TR::ExternalRelocation(
                                        cursor, (uint8_t*)_dispatchSymRef, TR_HelperAddress, cg()),
            __FILE__, __LINE__, _startOfPicInstruction->getNode());
        cursor += 4;
        ...
        *(uintptrj_t*)cursor = 0;
        ...
        if (unresolvedDispatch()) {
            ...
            intptrj_t entryPoint = ((intptrj_t)cursor
                + ((3 * sizeof(uintptrj_t)) + (hasJ2IThunkInPicData() ? sizeof(uintptrj_t) : 0)
                      + (TR::Compiler->target.is64Bit() ? 4 : 1)));

            intptrj_t requiredEntryPoint = (entryPoint + (cg()->getLowestCommonCodePatchingAlignmentBoundary() - 1)
        ...
        _dispatchSymRef
            = cg()->symRefTab()->findOrCreateRuntimeHelper(TR_X86populateVPicVTableDispatch, false, false, false);
    ...
    if (_numberOfSlots >= 1) {
        ...
        *(int32_t*)picSlotCursor = disp32;
        ...
        cg()->addExternalRelocation(new (cg()->trHeapMemory()) TR::ExternalRelocation(
                                        picSlotCursor, (uint8_t*)resolveSlotHelperSymRef, TR_HelperAddress, cg()),
            __FILE__, __LINE__, _startOfPicInstruction->getNode());
        ...
        while (--numPicSlots) {
            ...
            *(int32_t*)picSlotCursor = disp32;
            ...
            cg()->addExternalRelocation(new (cg()->trHeapMemory()) TR::ExternalRelocation(
                                            picSlotCursor, (uint8_t*)populateSlotHelperSymRef, TR_HelperAddress, cg()),
                __FILE__, __LINE__, _startOfPicInstruction->getNode());
            picSlotCursor = (uint8_t*)(picSlotCursor - 1 + sizeofPicSlot);
        }
    }
...
void TR_Debug::print(TR::FILE* pOutFile, TR::X86PicDataSnippet* snippet)
{
    if (pOutFile == NULL)
        return;
    ...
    if (snippet->isInterface()) {
        ...
        trfprintf(pOutFile, "call\t%s \t\t%s " POINTER_PRINTF_FORMAT, getName(dispatchSymRef), commentString(),
        ...
        if (methodSymRef->isUnresolved() || fej9->forceUnresolvedDispatch()) {
            ...
            trfprintf(pOutFile, "%s\t" POINTER_PRINTF_FORMAT "\t\t%s owning method cpAddr", op,
                (void*)*(uintptrj_t*)bufferPos, commentString());
            bufferPos += sizeof(uintptrj_t);

            printPrefix(pOutFile, NULL, bufferPos, sizeof(uintptrj_t));
            trfprintf(pOutFile, "%s\t" POINTER_PRINTF_FORMAT "\t\t%s cpIndex", op, (void*)*(uintptrj_t*)bufferPos,
                commentString());
            bufferPos += sizeof(uintptrj_t);

            printPrefix(pOutFile, NULL, bufferPos, sizeof(uintptrj_t));
            trfprintf(pOutFile, "%s\t" POINTER_PRINTF_FORMAT "\t\t%s interface class (initially null)", op,
                (void*)*(uintptrj_t*)bufferPos, commentString());
            bufferPos += sizeof(uintptrj_t);

            printPrefix(pOutFile, NULL, bufferPos, sizeof(uintptrj_t));
            trfprintf(pOutFile, "%s\t" POINTER_PRINTF_FORMAT "\t\t%s itable offset%s (initially zero)", op,
                (void*)*(uintptrj_t*)bufferPos, commentString(),
            ...
            if (TR::Compiler->target.is64Bit()) {
                ...
                trfprintf(pOutFile, "%s\t%s%02x%s\t\t\t\t\t\t\t\t%s REX of MOVRegImm64", dbString(), hexPrefixString(),
                    *bufferPos, hexSuffixString(), commentString());
                bufferPos += 1;

                printPrefix(pOutFile, NULL, bufferPos, 1);
                trfprintf(pOutFile, "%s\t%02x\t\t\t\t\t\t\t\t%s MOV opcode of MOVRegImm64", dbString(), *bufferPos,
                    commentString());
                bufferPos += 1;

                if (snippet->hasJ2IThunkInPicData()) {
                    printPrefix(pOutFile, NULL, bufferPos, sizeof(uintptrj_t));
                    trfprintf(pOutFile, "%s\t" POINTER_PRINTF_FORMAT "\t\t%s j2i virtual thunk", op,
                        (void*)*(uintptrj_t*)bufferPos, commentString());
                    bufferPos += sizeof(uintptrj_t);
                }
            } else {
                ...
                //
                printPrefix(pOutFile, NULL, bufferPos, 1);
                trfprintf(pOutFile, "%s\t%s%02x%s\t\t\t\t\t\t\t\t%s ModRM of CMP", dbString(), hexPrefixString(),
                    *bufferPos, hexSuffixString(), commentString());
                bufferPos += 1;
            }
        }
        ...
        if (snippet->unresolvedDispatch()) {
            ...
            printPrefix(pOutFile, NULL, bufferPos, sizeof(uintptrj_t));
            trfprintf(pOutFile, "%s\t" POINTER_PRINTF_FORMAT "\t\t%s direct J9Method (initially null)", op,
                (void*)*(uintptrj_t*)bufferPos, commentString());
            bufferPos += sizeof(uintptrj_t);
            ...
            if (TR::Compiler->target.is64Bit()) {
                ...
                printPrefix(pOutFile, NULL, bufferPos, 1);
                trfprintf(
                    pOutFile, "%s\t%02x\t\t\t\t\t\t\t\t%s REX of MOVRegImm64", dbString(), *bufferPos, commentString());
                bufferPos += 1;
                ...
                printPrefix(pOutFile, NULL, bufferPos, 1);
                trfprintf(
                    pOutFile, "%s\t%02x\t\t\t\t\t\t\t\t%s REX of CallMem", dbString(), *bufferPos, commentString());
                bufferPos += 1;

                callModRM = *bufferPos;
                printPrefix(pOutFile, NULL, bufferPos, 1);
                trfprintf(
                    pOutFile, "%s\t%02x\t\t\t\t\t\t\t\t%s ModRM for CALLMem", dbString(), *bufferPos, commentString());
                bufferPos += 1;
            } else {
                printPrefix(pOutFile, NULL, bufferPos, 1);
                trfprintf(pOutFile, "%s\t%02x\t\t\t\t\t\t\t\t%s ModRM for CMPRegImm4", dbString(), *bufferPos,
                    commentString());
                bufferPos += 1;
            }
        }
        ...
        trfprintf(pOutFile, "call\t%s \t\t%s " POINTER_PRINTF_FORMAT "\tpatched with vtable call",
            getName(dispatchSymRef), commentString(), dispatchSymRef->getMethodAddress());
        bufferPos += length;
...
uint32_t TR::X86PicDataSnippet::getLength(int32_t estimatedSnippetStart)
{
    if (isInterface()) {
        ...
        return 6 // CALL [Mem] (pessimistically assume a SIB is needed)
            ...
            //  1 (ModRM for CMP)
            //
            + (TR::Compiler->target.is64Bit() ? 4 : 1) + cg()->getLowestCommonCodePatchingAlignmentBoundary() - 1;
    }
}

uint8_t* TR::X86CallSnippet::alignCursorForCodePatching(uint8_t* cursor, bool alignWithNOPs)
{
    intptrj_t alignedCursor = ((intptrj_t)cursor + (cg()->getLowestCommonCodePatchingAlignmentBoundary() - 1)
        & (intptrj_t)(~(cg()->getLowestCommonCodePatchingAlignmentBoundary() - 1)));
...
TR_RuntimeHelper TR::X86CallSnippet::getInterpretedDispatchHelper(
    TR::SymbolReference* methodSymRef, TR::DataType type, bool isSynchronized, TR::CodeGenerator* cg)
{
    ...
    switch (type) {
    case TR::NoType:
        helper = TR_X86interpreterVoidStaticGlue;
        break;

    case TR::Int8:
    case TR::Int16:
    case TR::Int32:
        helper = TR_X86interpreterIntStaticGlue;
        break;

    case TR::Address:
        helper = TR_X86interpreterAddressStaticGlue;
        break;

    case TR::Int64:
        helper = TR_X86interpreterLongStaticGlue;
        break;

    case TR::Float:
        helper = TR_X86interpreterFloatStaticGlue;
        break;

    case TR::Double:
        helper = TR_X86interpreterDoubleStaticGlue;
        break;
...
TR_RuntimeHelper TR::X86CallSnippet::getDirectToInterpreterHelper(
    TR::MethodSymbol* methodSymbol, TR::DataType type, bool synchronised)
{
...
uint8_t* TR::X86CallSnippet::emitSnippetBody()
{
    ...
    if (TR::Compiler->target.is64Bit() && methodSymbol->isHelper() && methodSymRef->isOSRInductionHelper()) {
    ...
    if (TR::Compiler->target.is64Bit()) {
        ...
        if (comp->getCodeCacheSwitched() && (methodSymRef->getReferenceNumber() >= TR_AMD64numRuntimeHelpers)) {
    ...
    if (methodSymRef->isUnresolved() || forceUnresolvedDispatch) {
        ...
        TR_ASSERT(
            !isJitInduceOSRCall || !forceUnresolvedDispatch, "calling jitInduceOSR is not supported yet under AOT\n");
        ...
        TR::SymbolReference* helperSymRef
            = cg()->symRefTab()->findOrCreateRuntimeHelper(resolutionHelper, false, false, false);
        ...
        *(int32_t*)cursor = disp32;
        ...
        cg()->addExternalRelocation(new (cg()->trHeapMemory())
                                        TR::ExternalRelocation(cursor, (uint8_t*)helperSymRef, TR_HelperAddress, cg()),
        ...
        helperSymRef = cg()->symRefTab()->findOrCreateRuntimeHelper(
            TR_X86updateInterpreterDispatchGlueSite, false, false, false);
        ...
        *(int32_t*)cursor = disp32;
        ...
        TR_RuntimeHelper dispatchHelper
            = getInterpretedDispatchHelper(methodSymRef, getNode()->getDataType(), false, cg());
        ...
        *(intptrj_t*)cursor = cpAddr;
        ...
        relocation = new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor, *(uint8_t**)cursor,
        ...
        *(uint32_t*)cursor = methodSymRef->getCPIndexForVM();
        ...
        if (!isJitInduceOSRCall) {
            ...
            *(intptrj_t*)cursor = ramMethod;
            ...
            if (comp->getOption(TR_UseSymbolValidationManager)) {
                cg()->addExternalRelocation(new (cg()->trHeapMemory())
                                                TR::ExternalRelocation(cursor, (uint8_t*)ramMethod,
        ...
        *cursor++ = 0xe9;
        ...
        dispatchHelper = getInterpretedDispatchHelper(
            methodSymRef, getNode()->getDataType(), methodSymbol->isSynchronised(), cg());
        TR::SymbolReference* dispatchSymRef
            = cg()->symRefTab()->findOrCreateRuntimeHelper(dispatchHelper, false, false, false);

        disp32 = cg()->branchDisplacementToHelperOrTrampoline(cursor + 4, dispatchSymRef);
        *(int32_t*)cursor = disp32;

        cg()->addExternalRelocation(
            new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor, (uint8_t*)dispatchSymRef, TR_HelperAddress, cg()),
            __FILE__, __LINE__, getNode());
        cursor += 4;
    }
...
uint8_t* TR::X86UnresolvedVirtualCallSnippet::emitSnippetBody()
{
    ...
    *cursor++ = 0xe8; // CALL resolveAndPopulateVTableDispatch
    ...
    cg()->addExternalRelocation(new (comp->trHeapMemory())
                                    TR::ExternalRelocation(cursor, (uint8_t*)glueSymRef, TR_HelperAddress, cg()),
        __FILE__, __LINE__, getNode());
    ...
    *(intptrj_t*)cursor = cpAddr;
    cg()->addExternalRelocation(
        new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor, (uint8_t*)cpAddr,
            getNode() ? (uint8_t*)getNode()->getInlinedSiteIndex() : (uint8_t*)-1, TR_ConstantPool, cg()),
        __FILE__, __LINE__, getNode());
...
void TR_Debug::print(TR::FILE* pOutFile, TR::X86UnresolvedVirtualCallSnippet* snippet)
{
    if (pOutFile == NULL)
        return;
    ...
    trfprintf(pOutFile, "\t\t\t\t%s mysterious new unresolved virtual call snippet code", commentString());
    return;
}
