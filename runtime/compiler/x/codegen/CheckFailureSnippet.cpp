...
void TR_Debug::print(TR::FILE* pOutFile, TR::X86CheckFailureSnippet* snippet)
{
    if (pOutFile == NULL)
        return;
    ...
    if (snippet->getRequiredFPstackPop()) {
        ...
        trfprintf(pOutFile, "fstp\tst(0)\t\t%s Discard top of FP stack", commentString());
        bufferPos += 2;
    }

    printPrefix(pOutFile, NULL, bufferPos, 5);
    trfprintf(pOutFile, "call\t%s \t\t%s Helper Address = " POINTER_PRINTF_FORMAT, getName(symRef), commentString(),
        sym->getMethodAddress());
    bufferPos += 5;

    printPrefix(pOutFile, NULL, bufferPos, 4);
    trfprintf(pOutFile, "%s \t%s%08x%s", ddString(), hexPrefixString(),
...
uint8_t* TR::X86CheckFailureSnippet::emitCheckFailureSnippetBody(uint8_t* buffer)
{
    ...
    *buffer++ = 0xe8; // CallImm4
    ...
    if (NEEDS_TRAMPOLINE(destinationAddress, buffer + 4, cg())) {
        ...
        TR_ASSERT(
            IS_32BIT_RIP(destinationAddress, buffer + 4), "Local helper trampoline should be reachable directly.\n");
    }
...
void TR_Debug::print(TR::FILE* pOutFile, TR::X86BoundCheckWithSpineCheckSnippet* snippet)
{
    if (pOutFile == NULL)
        return;
...
void TR_Debug::print(TR::FILE* pOutFile, TR::X86SpineCheckSnippet* snippet)
{
    if (pOutFile == NULL)
        return;
...
enum { resolveHasLiveXMMRs = 0x10000000, longPushTag = 0x00800000, dontPatchTag = 0x00400000 };
...
uint8_t* TR::X86CheckFailureSnippetWithResolve::emitSnippetBody()
{
    ...
    *(int32_t*)buffer = (int32_t)(intptr_t)getDataSymbolReference()->getOwningMethod(cg()->comp())->constantPool();
    cg()->addExternalRelocation(
        new (cg()->trHeapMemory()) TR::ExternalRelocation(buffer, *(uint8_t**)buffer,
            getCheckInstruction()->getNode() ? (uint8_t*)getCheckInstruction()->getNode()->getInlinedSiteIndex()
                                             : (uint8_t*)-1,
            TR_ConstantPool, cg()),
        __FILE__, __LINE__, getCheckInstruction()->getNode());
    buffer += 4;
    ...
    *(int32_t*)buffer = (int32_t)(glueAddress - (intptrj_t)(buffer + 4));
    cg()->addExternalRelocation(new (cg()->trHeapMemory())
                                    TR::ExternalRelocation(buffer, (uint8_t*)glueSymRef, TR_HelperAddress, cg()),
        __FILE__, __LINE__, getCheckInstruction()->getNode());
...
void TR_Debug::print(TR::FILE* pOutFile, TR::X86CheckFailureSnippetWithResolve* snippet)
{

    enum { longPushTag = 0x00800000, dontPatchTag = 0x00400000 };
    ...
    trfprintf(pOutFile, "push\t" POINTER_PRINTF_FORMAT "\t\t%s push return address which is the throw bellow",
        bufferPos + 24, commentString());
    bufferPos += 5;

    cpIndexValue |= dontPatchTag;
    cpIndexValue |= longPushTag;
    printPrefix(pOutFile, NULL, bufferPos, size);
    trfprintf(pOutFile, "push\t" POINTER_PRINTF_FORMAT "\t\t%s push cpIndex", cpIndexValue, commentString());
    bufferPos += size;

    printPrefix(pOutFile, NULL, bufferPos, 5);
    trfprintf(pOutFile, "push\t" POINTER_PRINTF_FORMAT "\t\t%s push address of constant pool",
