...
uint8_t* TR::X86ForceRecompilationSnippet::emitSnippetBody()
{
    ...
    TR::SymbolReference* helper = cg()->symRefTab()->findOrCreateRuntimeHelper(
        TR::Compiler->target.is64Bit() ? TR_AMD64induceRecompilation : TR_IA32induceRecompilation, false, false, false);
    ...
    *(int32_t*)buffer = ((uint8_t*)helperAddress - buffer) - 4;
    ...
    cg()->addExternalRelocation(new (cg()->trHeapMemory())
                                    TR::ExternalRelocation(buffer, (uint8_t*)helper, TR_HelperAddress, cg()),
...
void TR_Debug::print(TR::FILE* pOutFile, TR::X86ForceRecompilationSnippet* snippet)
{
    if (pOutFile == NULL)
        return;
    ...
    TR::SymbolReference* helper
        = _cg->getSymRef(TR::Compiler->target.is64Bit() ? TR_AMD64induceRecompilation : TR_IA32induceRecompilation);
    intptrj_t helperAddress = (intptrj_t)helper->getMethodAddress();
    printPrefix(pOutFile, NULL, bufferPos, 5);
    trfprintf(pOutFile, "call\t%s \t%s Helper Address = " POINTER_PRINTF_FORMAT, getName(helper), commentString(),
        helperAddress);
    bufferPos += 5;
