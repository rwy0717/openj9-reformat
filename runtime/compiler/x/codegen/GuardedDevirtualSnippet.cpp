...
void TR_Debug::print(TR::FILE* pOutFile, TR::X86GuardedDevirtualSnippet* snippet)
{
    if (pOutFile == NULL)
        return;
    ...
    printSnippetLabel(
        pOutFile, snippet->getSnippetLabel(), bufferPos, getName(snippet), "out of line full virtual call sequence");
    ...
    if (!snippet->getClassObjectRegister()) {
        ...
        trfprintf(pOutFile, "mov \t%cdi, [%cax]\t\t%s Load Class Object", regLetter, regLetter, commentString());
        bufferPos += movSize;
        ...
        trfprintf(pOutFile, "call\t[%s %d]\t\t%s call through vtable slot %d", getName(classObjectRegister, regSize),
