    ...
    , _stackFramePaddingSizeInBytes(0)
{
    ...
    if (cg->getX86ProcessorInfo().supportsSSE4_1() && !comp->getOption(TR_DisableSIMDStringCaseConv)
        && !TR::Compiler->om.canGenerateArraylets())
        cg->setSupportsInlineStringCaseConversion();

    if (cg->getX86ProcessorInfo().supportsSSSE3() && !comp->getOption(TR_DisableFastStringIndexOf)
        && !TR::Compiler->om.canGenerateArraylets()) {
        cg->setSupportsInlineStringIndexOf();
    }

    if (cg->getX86ProcessorInfo().supportsSSE4_1() && !comp->getOption(TR_DisableSIMDStringHashCode)
        && !TR::Compiler->om.canGenerateArraylets()) {
...
void J9::X86::CodeGenerator::beginInstructionSelection()
{
    ...
    if (recompilation && recompilation->generatePrePrologue() != NULL) {
        ...
        int32_t alignmentMargin = TR::Compiler->target.is64Bit()
            ? 2
        ...
        if (TR::Compiler->target.is64Bit())
            new (self()->trHeapMemory())
                TR::AMD64Imm64Instruction((TR::Instruction*)NULL, DQImm64, methodAddress, self());
        else
...
void J9::X86::CodeGenerator::endInstructionSelection()
{
    ...
    if (self()->enableSinglePrecisionMethods() && comp->getJittedMethodSymbol()->usesSinglePrecisionMode()) {
        ...
        auto cds = self()->findOrCreate2ByteConstant(
            self()->getLastCatchAppendInstruction()->getNode(), DOUBLE_PRECISION_ROUND_TO_NEAREST);
        generateMemInstruction(
            self()->getLastCatchAppendInstruction(), LDCWMem, generateX86MemoryReference(cds, self()), self());
    }
}

TR::Instruction* J9::X86::CodeGenerator::generateSwitchToInterpreterPrePrologue(
    TR::Instruction* prev, uint8_t alignment, uint8_t alignmentMargin)
{
    ...
    TR::SymbolReference* helperSymRef = self()->symRefTab()->findOrCreateRuntimeHelper(
        TR::X86CallSnippet::getDirectToInterpreterHelper(
            methodSymbol, methodSymbol->getMethod()->returnType(), methodSymbol->isSynchronised()),
        false, false, false);
    ...
    prev = new (self()->trHeapMemory())
        TR::X86ImmSymInstruction(prev, JMP4, (uintptrj_t)helperSymRef->getMethodAddress(), helperSymRef, deps, self());
...
bool J9::X86::CodeGenerator::allowGuardMerging() { return self()->fej9()->supportsGuardMerging(); }
...
{
    if (TR::CodeGenerator::getX86ProcessorInfo().supportsAESNI() && !self()->comp()->getOption(TR_DisableAESInHardware)
...
void J9::X86::CodeGenerator::reserveNTrampolines(int32_t numTrampolines)
{
        ...
        if (status != OMR::CodeCacheErrorCode::ERRORCODE_SUCCESS) {
            ...
            if (self()->getCodeGeneratorPhase() != TR::CodeGenPhase::BinaryEncodingPhase) {
                ...
                if (newCache) {
                    ...
                    TR_ASSERT(status == OMR::CodeCacheErrorCode::ERRORCODE_SUCCESS,
                        "Failed to reserve trampolines in fresh code cache.");
                }
            }
        }
    }
    ...
    if (newCache != curCache) {
        ...
        // If the old CC had pre-loaded code, the current compilation may have initialized it and will therefore depend
        // on it so we should initialize it in the new CC as well
