...
inline void generateLoadJ9Class(TR::Node* node, TR::Register* j9class, TR::Register* object, TR::CodeGenerator* cg)
{
    ...
    if (node->getOpCode().isReadBar() || node->getOpCode().isWrtBar())
        ...
        default:
            TR_ASSERT(opValue == TR::checkcast || opValue == TR:: instanceof,
    ...
    auto instr = generateRegMemInstruction(LRegMem(use64BitClasses), node, j9class,
    ...
    if (~mask != 0) {
        generateRegImmInstruction(
            ~mask <= 127 ? ANDRegImms(use64BitClasses) : ANDRegImm4(use64BitClasses), node, j9class, mask, cg);
    }
}
    ...
    TR::Register* valueReg, bool needsBoundCheck, TR::CodeGenerator* cg)
{
    ...
    if (needsBoundCheck) {
        ...
        cg->addSnippet(new (cg->trHeapMemory()) TR::X86CheckFailureSnippet(
            cg, node->getSymbolReference(), boundCheckFailureLabel, checkInstruction, false));
    ...
    if (indexReg) {
        ...
        arrayletMR = generateX86MemoryReference(
            scratchReg, scratchReg2, TR::MemoryReference::convertMultiplierToStride(elementSize), cg);
    ...
    if (!actualLoadOrStoreOrArrayElementNode->getOpCode().isStore()) {
        ...
        if (dt != TR::Address) {
            ...
            switch (dt) {
            case TR::Int8:
                op = valueReg ? S1MemReg : S1MemImm1;
                break;
            case TR::Int16:
                op = valueReg ? S2MemReg : S2MemImm2;
                break;
            case TR::Int32:
                op = valueReg ? S4MemReg : S4MemImm4;
                break;
            case TR::Int64:
                if (TR::Compiler->target.is64Bit()) {
                    ...
                    if (valueReg) {
                        ...
                        generateMemRegInstruction(S4MemReg, node, generateX86MemoryReference(*arrayletMR, 4, cg),
                        ...
                        generateMemImmInstruction(S4MemImm4, node, generateX86MemoryReference(*arrayletMR, 4, cg),
...
static TR::Instruction* generatePrefetchAfterHeaderAccess(
    TR::Node* node, TR::Register* objectReg, TR::CodeGenerator* cg)
{
        ...
        if (TR::TreeEvaluator::loadLookaheadAfterHeaderAccess(node, fieldOffset, cg)) {
            if (fieldOffset > 32)
                instr = generateMemInstruction(
                    PREFETCHT0, node, generateX86MemoryReference(objectReg, fieldOffset, cg), cg);
    ...
    TR::Register*& lookupOffsetReg, TR::Register* vmThreadReg, TR::Register* objectReg)
{
    ...
    TR_ASSERT(0,
        "the x86 evaluator requires lock nursery to be defined, otherwise you'll need to make sure the code below "
        "works");
#endif
    TR::Compilation* comp = cg->comp();
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(cg->fe());
    if (comp->getOption(TR_EnableMonitorCacheLookup)) {
        ...
        // generateRegMemInstruction(LRegMem(), node, objectClassReg, generateX86MemoryReference(vmThreadReg,
        // offsetOfMonitorLookupCache, cg), cg);
        generateRegRegInstruction(MOVRegReg(), node, lookupOffsetReg, objectReg, cg);

        generateRegImmInstruction(SARRegImm1(TR::Compiler->target.is64Bit()), node, lookupOffsetReg,
            trailingZeroes(fej9->getObjectAlignmentInBytes()), cg);
        ...
        generateRegImmInstruction(
            SHLRegImm1(), node, lookupOffsetReg, trailingZeroes(TR::Compiler->om.sizeofReferenceField()), cg);
        generateRegMemInstruction(
            (TR::Compiler->target.is64Bit() && fej9->generateCompressedLockWord()) ? L4RegMem : LRegMem(), node,
            objectClassReg, generateX86MemoryReference(vmThreadReg, lookupOffsetReg, 0, offsetOfMonitorLookupCache, cg),
            cg);
        ...
        generateRegMemInstruction(
            LRegMem(), node, lookupOffsetReg, generateX86MemoryReference(objectClassReg, offsetOfMonitor, cg), cg);

        int32_t offsetOfUserData = offsetof(J9ThreadAbstractMonitor, userData);
        generateRegMemInstruction(
            LRegMem(), node, lookupOffsetReg, generateX86MemoryReference(lookupOffsetReg, offsetOfUserData, cg), cg);
        ...
        // generateRegMemInstruction(LRegMem(), node, lookupOffsetReg, generateX86MemoryReference(objectClassReg,
        // offsetOfAlternateLockWord, cg), cg);
...
#ifdef TR_TARGET_64BIT
// Hack markers
#define JVMPI_HOOKS_WITHOUT_SNIPPETS \
    (!debug("enableSnippetsForJVMPI")) // Snippets can't yet do the necessary linkage logic
        ...
        for (TR::Instruction* cursor = cg->getAppendInstruction(); cursor != startInstruction;
            ...
            if (cursorDeps && cursor->getOpCodeValue() != ASSOCREGS) {
                ...
                for (int32_t i = 0; i < cursorDeps->getNumPostConditions(); i++) {
                    ...
                    postConditions->unionPostCondition(
                        cursorPostCondition->getRegister(), cursorPostCondition->getRealRegister(), cg);
                    if (debug("traceConditionalHelperEvaluator")) {
                        TR_Debug* debug = cg->getDebug();
                        diagnostic("conditionalHelperEvaluator:    [%s : %s]\n",
                            debug->getName(cursorPostCondition->getRegister()),
                            debug->getName(machine->getRealRegister(cursorPostCondition->getRealRegister())));
                    }
                }
            }
        }
...
TR::Register* J9::X86::TreeEvaluator::performHeapLoadWithReadBarrier(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::Instruction* load = generateRegMemInstruction(
        LRegMem(use64BitClasses), node, object, generateX86MemoryReference(address, 0, cg), cg);
    ...
    generateRegMemInstruction(CMPRegMem(use64BitClasses), node, object,
        generateX86MemoryReference(
            cg->getVMThreadRegister(), cg->comp()->fej9()->thisThreadGetEvacuateBaseAddressOffset(), cg),
        cg);
    generateLabelInstruction(JAE4, node, rdbarLabel, cg);
    {
        ...
        generateRegMemInstruction(CMPRegMem(use64BitClasses), node, object,
            generateX86MemoryReference(
                cg->getVMThreadRegister(), cg->comp()->fej9()->thisThreadGetEvacuateTopAddressOffset(), cg),
            cg);
        generateLabelInstruction(JA4, node, endLabel, cg);
        generateMemRegInstruction(SMemReg(), node,
            generateX86MemoryReference(cg->getVMThreadRegister(), offsetof(J9VMThread, floatTemp1), cg), address, cg);
...
TR::Register* J9::X86::TreeEvaluator::writeBarrierEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (node->getOpCodeValue() == TR::awrtbari) {
        ...
        if (comp->useCompressedPointers() && (node->getSymbolReference()->getSymbol()->getDataType() == TR::Address)
            ...
            if (TR::Compiler->vm.heapBaseAddress() == 0 || sourceObject->isNull())
...
TR::Register* J9::X86::TreeEvaluator::monexitfenceEvaluator(TR::Node* node, TR::CodeGenerator* cg) { return NULL; }
...
TR::Register* J9::X86::TreeEvaluator::asynccheckEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...

    {
        ...
        generateImmSymInstruction(
            CALLImm4, node, (uintptrj_t)node->getSymbolReference()->getMethodAddress(), node->getSymbolReference(), cg)
...
TR::Register* J9::X86::TreeEvaluator::multianewArrayEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    generateRegMemInstruction(L4RegMem, node, secondDimLenReg, generateX86MemoryReference(dimsPtrReg, 0, cg), cg);
    generateRegMemInstruction(L4RegMem, node, firstDimLenReg, generateX86MemoryReference(dimsPtrReg, 4, cg), cg);
    ...
    generateRegMemInstruction(
        LRegMem(), node, targetReg, generateX86MemoryReference(vmThreadReg, offsetof(J9VMThread, heapAlloc), cg), cg);
    generateRegMemInstruction(
        LEARegMem(), node, temp1Reg, generateX86MemoryReference(targetReg, zeroArraySize, cg), cg);
    generateRegMemInstruction(
        CMPRegMem(), node, temp1Reg, generateX86MemoryReference(vmThreadReg, offsetof(J9VMThread, heapTop), cg), cg);
    generateLabelInstruction(JA4, node, failLabel, cg);
    generateMemRegInstruction(
        SMemReg(), node, generateX86MemoryReference(vmThreadReg, offsetof(J9VMThread, heapAlloc), cg), temp1Reg, cg);

    // Init class
    bool use64BitClasses = TR::Compiler->target.is64Bit() && !TR::Compiler->om.generateCompressedObjectHeaders();
    generateMemRegInstruction(SMemReg(use64BitClasses), node,
    ...
    generateMemImmInstruction(
        S4MemImm4, node, generateX86MemoryReference(targetReg, fej9->getOffsetOfContiguousArraySizeField(), cg), 0, cg);
    ...
    if (TR::Compiler->target.is64Bit()
        && !(maxObjectSizeInElements > 0 && maxObjectSizeInElements <= (uintptrj_t)INT_MAX)) {
    ...
    generateRegImmInstruction(
        ADDRegImm4(), node, temp1Reg, TR::Compiler->om.contiguousArrayHeaderSizeInBytes() + disp32, cg);
    ...
    generateRegMemInstruction(
        CMPRegMem(), node, temp2Reg, generateX86MemoryReference(vmThreadReg, offsetof(J9VMThread, heapTop), cg), cg);
    generateLabelInstruction(JA4, node, failLabel, cg);
    generateMemRegInstruction(
        SMemReg(), node, generateX86MemoryReference(vmThreadReg, offsetof(J9VMThread, heapAlloc), cg), temp2Reg, cg);
    ...
    generateRegMemInstruction(LEARegMem(), node, temp1Reg,
    ...
    bool useRegForHeapBase = TR::Compiler->target.is64Bit() && comp->useCompressedPointers() && (heapBase != 0)
        && (!IS_32BIT_SIGNED(heapBase) || TR::Compiler->om.nativeAddressesCanChangeSize());
    ...
    generateMemImmInstruction(
        S4MemImm4, node, generateX86MemoryReference(temp2Reg, fej9->getOffsetOfContiguousArraySizeField(), cg), 0, cg);
    generateMemImmInstruction(S4MemImm4, node,
...
TR::Register* J9::X86::TreeEvaluator::arraycopyEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!node->isNoArrayStoreCheckArrayCopy()) {
        ...
        auto snippet = new (cg->trHeapMemory()) TR::X86CheckFailureSnippet(cg,
            cg->symRefTab()->findOrCreateRuntimeHelper(TR_arrayStoreException, false, false, false), snippetLabel,
            instr, false);
        cg->addSnippet(snippet);
    } else {
        bool use64BitClasses = TR::Compiler->target.is64Bit() && !TR::Compiler->om.generateCompressedObjectHeaders();
        ...
        if (TR::Compiler->om.shouldGenerateReadBarriersForFieldLoads()) {
            static_assert(IS_32BIT_SIGNED(J9_PRIVATE_FLAGS_CONCURRENT_SCAVENGER_ACTIVE),
                "Unexcepted value of J9_PRIVATE_FLAGS_CONCURRENT_SCAVENGER_ACTIVE.");
            TR::LabelSymbol* rdbarLabel = generateLabelSymbol(cg);
            generateMemImmInstruction(TEST4MemImm4, node,
                ...
                J9_PRIVATE_FLAGS_CONCURRENT_SCAVENGER_ACTIVE, cg);
        ...
        if (!node->isForwardArrayCopy()) {
            ...
            generateRegMemInstruction(
                LEARegMem(), node, RDI, generateX86MemoryReference(RDI, RSI, 0, cg), cg); // dst = dst + src
            ...
            generateRegMemInstruction(LEARegMem(), node, RSI,
                generateX86MemoryReference(RSI, RCX, 0, -TR::Compiler->om.sizeofReferenceField(), cg), cg);
            generateRegMemInstruction(LEARegMem(), node, RDI,
        ...
        TR::TreeEvaluator::VMwrtbarWithoutStoreEvaluator(
            node, node->getChild(1), NULL, NULL, cg->generateScratchRegisterManager(), cg);
    }
    ...
    TR::Node* node, bool needResolution, TR::CodeGenerator* cg)
{
    ...
    if (needExplicitCheck) {
        ...
        if (opCode.isCall() || !needResolution
            || TR::Compiler->target.is64Bit()) // TODO:AMD64: Implement the "withresolve" version
        {
            snippet = new (cg->trHeapMemory())
            ...
            snippet = new (cg->trHeapMemory()) TR::X86CheckFailureSnippetWithResolve(
                cg, node->getSymbolReference(), firstChild->getSymbolReference(), resolverCall, snippetLabel, appendTo);
...
TR::Register* J9::X86::TreeEvaluator::DIVCHKEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (platformNeedsExplicitCheck && operationNeedsCheck) {
        ...
        if (useRegisterPairs) {
            ...
            lowDivisorTestInstr
                = generateRegRegInstruction(TESTRegReg(use64BitRegisters), node, divisorReg, divisorReg, cg);

        generateLabelInstruction(JE4, node, divideByZeroSnippetLabel, cg);

        cg->addSnippet(new (cg->trHeapMemory()) TR::X86CheckFailureSnippet(
            cg, node->getSymbolReference(), divideByZeroSnippetLabel, cg->getAppendInstruction()));
        ...
        if (prevInstr->getOpCodeValue() == CMP4RegReg) {
            ...
            if (prevInstrTargetRegister && prevInstrSourceRegister
                && (((prevInstrSourceRegister == firstChildReg) && (prevInstrTargetRegister == secondChildReg))
                       || ((prevInstrSourceRegister == secondChildReg)
                              && (prevInstrTargetRegister == firstChildReg)))) {
...
TR::Register* J9::X86::TreeEvaluator::ArrayStoreCHKEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (nopASC) {
        ...
        TR::TreeEvaluator::VMarrayStoreCHKEvaluator(
            node, sourceChild, destinationChild, scratchRegisterManager, startOfWrtbarLabel, prevInstr, cg);
    }
    ...
    if (generateWriteBarrier) {
        ...
        TR::TreeEvaluator::VMwrtbarWithStoreEvaluator(
            node, tempMR, scratchRegisterManager, destinationChild, sourceChild, true, cg, true);
    ...
    deps->unionPostCondition(
        cg->getVMThreadRegister(), (TR::RealRegister::RegNum)cg->getVMThreadRegister()->getAssociation(), cg);
...
static TR::Register* numberOfLeadingZeros(
    TR::Node* node, TR::CodeGenerator* cg, TR::Register* reg, bool is64Bit, bool isLong)
{
    // xor r1, r1
    // bsr r2, reg
    // sete r1
...
static TR::Register* numberOfTrailingZeros(
    TR::Node* node, TR::CodeGenerator* cg, TR::Register* reg, bool is64Bit, bool isLong)
{
...
static TR::Register* bitCount(TR::Node* node, TR::CodeGenerator* cg, TR::Register* reg, bool is64Bit)
{
    TR::Register* bsfReg = cg->allocateRegister();
...
inline void generateInlinedCheckCastOrInstanceOfForInterface(
    TR::Node* node, TR_OpaqueClassBlock* clazz, TR::CodeGenerator* cg, bool isCheckCast)
{
    ...
    // I-Table lookup
    {
        ...
        // Obtain I-Table
        generateRegMemInstruction(
            LRegMem(), node, itable, generateX86MemoryReference(j9class, offsetof(J9Class, iTable), cg), cg);
        if (tmp) {
        ...
        if (tmp) {
            ...
            generateMemImmSymInstruction(
                CMP4MemImm4, node, interfaceMR, (uintptrj_t)clazz, node->getChild(1)->getSymbolReference(), cg);
        }
        generateRegMemInstruction(
            LRegMem(), node, itable, generateX86MemoryReference(itable, offsetof(J9ITable, next), cg), cg);
...
inline void generateInlinedCheckCastOrInstanceOfForClass(
    TR::Node* node, TR_OpaqueClassBlock* clazz, TR::CodeGenerator* cg, bool isCheckCast)
{
    ...
    if (!fej9->isClassFinal(clazz)) {
        ...
        generateRegMemInstruction(
            LRegMem(), node, tmp, generateX86MemoryReference(j9class, offsetof(J9Class, superclasses), cg), cg);
...
TR::Register* J9::X86::TreeEvaluator::checkcastinstanceofEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (clazz && !TR::Compiler->cls.isClassArray(cg->comp(), clazz) && // not yet optimized
        (!cg->comp()->compileRelocatableCode() || cg->comp()->getOption(TR_UseSymbolValidationManager))
        && !cg->comp()->getOption(TR_DisableInlineCheckCast) && !cg->comp()->getOption(TR_DisableInlineInstanceOf)) {
        ...
        if (node->getOpCodeValue() == TR::checkcastAndNULLCHK) {
            ...
            auto instr = generateMemImmInstruction(TEST1MemImm1, node,
...
static bool comesFromClassLib(TR::Node* node, TR::Compilation* comp)
{
    ...
    if (methodSig && (strncmp(methodSig, "java", 4) == 0 || strncmp(methodSig, "sun", 3) == 0))
        return true;
    return false;
}

static TR::MemoryReference* getMemoryReference(
    TR::Register* objectClassReg, TR::Register* objectReg, int32_t lwOffset, TR::CodeGenerator* cg)
{
    if (objectClassReg)
...
void J9::X86::TreeEvaluator::asyncGCMapCheckPatching(
    TR::Node* node, TR::CodeGenerator* cg, TR::LabelSymbol* snippetLabel)
{
    ...
    if (TR::Compiler->target.is64Bit()) {
        ...
        generatePatchableCodeAlignmentInstruction(TR::X86PatchableCodeAlignmentInstruction::CALLImm4AtomicRegions,
            generateLabelInstruction(CALLImm4, node, gcMapPatchingLabel, cg), cg);
        ...
        generateMemImmInstruction(S8MemImm4, node,
            generateX86MemoryReference(cg->getVMThreadRegister(), offsetof(J9VMThread, stackOverflowMark), cg), -1, cg);
        generateRegImmInstruction(
            MOV8RegImm4, node, tempReg, 1 << comp->getPersistentInfo()->getGCMapCheckEventHandle(), cg);
        generateMemRegInstruction(LOR8MemReg, node,
        ...
        generateRegMemInstruction(
            L8RegMem, node, patchValReg, generateX86MemoryReference(patchableAddrReg, -5, cg), cg);
        ...
        generateMemRegInstruction(
            S8MemReg, node, generateX86MemoryReference(patchableAddrReg, -5, cg), patchValReg, deps, cg);
        ...
        // 8,generateLabelInstruction(CALLImm4, node, gcMapPatchingLabel, cg), cg);
        ...
        generateMemImmInstruction(S4MemImm4, node,
            generateX86MemoryReference(cg->getVMThreadRegister(), offsetof(J9VMThread, stackOverflowMark), cg), -1, cg);
        generateRegImmInstruction(
            MOV4RegImm4, node, lowPatchValReg, 1 << comp->getPersistentInfo()->getGCMapCheckEventHandle(), cg);
        generateMemRegInstruction(LOR4MemReg, node,
        ...
        //
        generateRegMemInstruction(
            L4RegMem, node, lowExistingValReg, generateX86MemoryReference(patchableAddrReg, -5, cg), cg);
        generateRegMemInstruction(
            L4RegMem, node, highExistingValReg, generateX86MemoryReference(patchableAddrReg, -1, cg), cg);
    ...
    TR::LabelSymbol* snippetRestartLabel, bool reservingLock)
{
    ...
    generateRegMemInstruction(
        LRegMem(use64bitOp), node, lockWordReg, generateX86MemoryReference(objectReg, lwOffset, cg), cg);
    generateRegImmInstruction(
        ADDRegImm4(use64bitOp), node, lockWordReg, isMonitorEnter ? INC_DEC_VALUE : -INC_DEC_VALUE, cg);
    ...
    generateMemRegInstruction(
        SMemReg(use64bitOp), node, generateX86MemoryReference(objectReg, lwOffset, cg), lockWordReg, cg);
...
void J9::X86::TreeEvaluator::transactionalMemoryJITMonitorEntry(TR::Node* node, TR::CodeGenerator* cg,
    TR::LabelSymbol* startLabel, TR::LabelSymbol* snippetLabel, TR::LabelSymbol* JITMonitorEnterSnippetLabel,
    TR::Register* objectReg, int lwOffset)
...
TR::Register* J9::X86::TreeEvaluator::VMmonentEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (comp->getOption(TR_MimicInterpreterFrameShape)
        ...
        || comp->getOption(TR_DisableInlineMonEnt) || (firstMonEnt && (*firstMonEnt - '0') > monEntCount++)) {
        // Don't inline
        //
        TR::ILOpCodes opCode = node->getOpCodeValue();
    ...
    if (comp->getOption(TR_ReservingLocks)) {
        ...
        if (reservingLock && node->isPrimitiveLockedRegion() && dummyMethodMonitor) {
            if (node->getSymbolReference() == cg->getSymRef(TR_methodMonitorEntry))
                node->setSymbolReference(comp->getSymRefTab()->findOrCreateRuntimeHelper(
                    TR_AMD64JitMethodMonitorExitReservedPrimitive, true, true, true));
            else
                node->setSymbolReference(comp->getSymRefTab()->findOrCreateRuntimeHelper(
                    TR_AMD64JitMonitorExitReservedPrimitive, true, true, true));

            exitLabel = generateLabelSymbol(cg);
            TR_OutlinedInstructions* outlinedExitHelperCall
                = new (cg->trHeapMemory()) TR_OutlinedInstructions(node, TR::call, NULL, exitLabel, fallThru, cg);
            cg->getOutlinedInstructionsList().push_front(outlinedExitHelperCall);
        }

        TR_RuntimeHelper helper;
        bool success = TR::TreeEvaluator::monEntryExitHelper(
            true, node, reservingLock, normalLockPreservingReservation, helper, cg);
        if (success)
    ...
    if (TR::Compiler->target.is64Bit() && fej9->getX86SupportsHLE() && comp->getOption(TR_X86HLE)) {
        ...
        TR::TreeEvaluator::transactionalMemoryJITMonitorEntry(
            node, cg, startLabel, snippetLabel, JITMonitorEntrySnippetLabel, objectReg, lwOffset);
        outlinedHelperCall = new (cg->trHeapMemory()) TR_OutlinedInstructions(
            helperCallNode, TR::call, NULL, JITMonitorEntrySnippetLabel, (exitLabel) ? exitLabel : fallThru, cg);
    } else
        outlinedHelperCall = new (cg->trHeapMemory()) TR_OutlinedInstructions(
            helperCallNode, TR::call, NULL, snippetLabel, (exitLabel) ? exitLabel : snippetFallThru, cg);
    ...
    if (inlineRecursive) {
        ...
        TR::TreeEvaluator::inlineRecursiveMonitor(node, cg, fallThru, jitMonitorEnterSnippetLabel,
            inlineRecursiveSnippetLabel, objectReg, lwOffset, snippetFallThru, reservingLock);
    }
    ...
    if (lwOffset <= 0) {
        ...
        generateRegMemInstruction(
            LRegMem(), node, objectClassReg, generateX86MemoryReference(objectClassReg, offsetOfLockOffset, cg), cg);
    ...
    if (comp->getOption(TR_ReservingLocks) && reservingLock) {
...
#if defined(TRACE_LOCK_RESERVATION)
        {
            ...
            if (TR::Compiler->target.is64Bit() && fej9->generateCompressedLockWord()) {
                ...
                instr = generateRegMemInstruction(
                    L4RegMem, node, eaxReal, getMemoryReference(objectClassReg, objectReg, lwOffset, cg), cg);
            } else
                instr = generateRegMemInstruction(
                    LRegMem(), node, eaxReal, getMemoryReference(objectClassReg, objectReg, lwOffset, cg), cg);
        ...
        if (TR::Compiler->target.is64Bit() && fej9->generateCompressedLockWord()) {
            ...
            instr = generateMemRegInstruction(
                CMP4MemReg, node, getMemoryReference(objectClassReg, objectReg, lwOffset, cg), eaxReal, cg);
        } else
            instr = generateMemRegInstruction(CMPMemReg(TR::Compiler->target.is64Bit()), node,
        ...
        if (!node->isPrimitiveLockedRegion()) {
            if (TR::Compiler->target.is64Bit() && fej9->generateCompressedLockWord()) {
                ...
                generateMemImmInstruction(
                    ADD4MemImms, node, getMemoryReference(objectClassReg, objectReg, lwOffset, cg), REC_BIT, cg);
            } else
                generateMemImmInstruction(
                    ADDMemImms(), node, getMemoryReference(objectClassReg, objectReg, lwOffset, cg), REC_BIT, cg);
        }
            ...
            if (comp->getOption(TR_ReservingLocks) && normalLockPreservingReservation) {
                ...
                if (TR::Compiler->target.is64Bit() && fej9->generateCompressedLockWord())
                    instr = generateMemImmInstruction(
                        CMP4MemImms, node, getMemoryReference(objectClassReg, objectReg, lwOffset, cg), 0, cg);
                else
                    instr = generateMemImmInstruction(
                        CMPMemImms(), node, getMemoryReference(objectClassReg, objectReg, lwOffset, cg), 0, cg);
                cg->setImplicitExceptionPoint(instr);
                instr->setNeedsGCMap(0xFF00FFFF);
        ...
        if (doCmpFirst && !comesFromClassLib(node, comp)) {
            TR::X86MemImmInstruction* instr;
            if (TR::Compiler->target.is64Bit() && fej9->generateCompressedLockWord())
                instr = generateMemImmInstruction(
                    CMP4MemImms, node, getMemoryReference(objectClassReg, objectReg, lwOffset, cg), 0, cg);
        ...
        if (node->isReadMonitor()) {
            ...
            if (TR::Compiler->target.is64Bit() && fej9->generateCompressedLockWord())
                generateRegRegInstruction(
                    XOR4RegReg, node, lockedReg, lockedReg, cg); // After lockedReg is allocated zero it out.
        ...
        TR::X86MemRegInstruction* instr = generateMemRegInstruction(
            op, node, getMemoryReference(objectClassReg, objectReg, lwOffset, cg), lockedReg, cg);
        cg->setImplicitExceptionPoint(instr);
        instr->setNeedsGCMap(0xFF00FFFF);
    ...
    generateRegMemInstruction(
        LRegMem(), node, lockedReg, generateX86MemoryReference(lockedReg, fej9->getMonitorNextOffset(), cg), cg);
    generateMemRegInstruction(SMemReg(), node,
        generateX86MemoryReference(vmThreadReg, fej9->thisThreadMonitorCacheOffset(), cg), lockedReg, cg);
#endif
...
#if defined(TRACE_LOCK_RESERVATION)
    {
        ...
        if (TR::Compiler->target.is64Bit() && fej9->generateCompressedLockWord())
            instr = generateRegMemInstruction(
                L4RegMem, node, eaxReal, getMemoryReference(objectClassReg, objectReg, lwOffset, cg), cg) else instr
                = generateRegMemInstruction(
                    LRegMem(), node, eaxReal, getMemoryReference(objectClassReg, objectReg, lwOffset, cg), cg);
...
void J9::X86::TreeEvaluator::generateValueTracingCode(
    TR::Node* node, TR::Register* vmThreadReg, TR::Register* scratchReg, TR::Register* valueReg, TR::CodeGenerator* cg)
{
    ...
    generateRegMemInstruction(
        LRegMem(), node, scratchReg, generateX86MemoryReference(vmThreadReg, vmThreadCursor, cg), cg);
    generateRegImmInstruction(ADDRegImms(), node, scratchReg, 8, cg);

    generateMemRegInstruction(
        CMPMemReg(), node, generateX86MemoryReference(vmThreadReg, vmThreadTop, cg), scratchReg, cg);
    generateLabelInstruction(JG4, node, endLabel, cg);
    generateRegMemInstruction(
        LRegMem(), node, scratchReg, generateX86MemoryReference(vmThreadReg, vmThreadBase, cg), cg);
    generateLabelInstruction(LABEL, node, endLabel, cg);
    generateMemImmInstruction(
        SMemImm4(), node, generateX86MemoryReference(scratchReg, 0, cg), node->getOpCodeValue(), cg);
    generateMemRegInstruction(SMemReg(), node, generateX86MemoryReference(scratchReg, 0, cg), valueReg, cg);
    generateMemRegInstruction(
        SMemReg(), node, generateX86MemoryReference(vmThreadReg, vmThreadCursor, cg), scratchReg, cg);
}

void J9::X86::TreeEvaluator::generateValueTracingCode(TR::Node* node, TR::Register* vmThreadReg,
    TR::Register* scratchReg, TR::Register* valueRegHigh, TR::Register* valueRegLow, TR::CodeGenerator* cg)
{
    if (!cg->comp()->getOption(TR_EnableValueTracing))
        return;
...
TR::Register* J9::X86::TreeEvaluator::VMmonexitEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if ((comp->getOption(TR_MimicInterpreterFrameShape) /*&& !comp->getOption(TR_EnableLiveMonitorMetadata)*/)
        || noInline || comp->getOption(TR_DisableInlineMonExit)
    ...
    // load lock word
    generateRegMemInstruction(
        LRegMem(), node, tempReg, getMemoryReference(objectClassReg, objectReg, lwOffset, cg), cg);
...
#define LOCK_PINNED_BIT (0x1)
    ...
    generateRegMemInstruction(
        LRegMem(), node, eaxReal, generateX86MemoryReference(unlockedReg, fej9->getMonitorOwnerOffset(), cg), cg);
    generateRegMemInstruction(
        CMPRegMem(), node, eaxReal, generateX86MemoryReference(vmThreadReg, fej9->thisThreadOSThreadOffset(), cg), cg);
    generateLabelInstruction(JNE4, node, snippetLabel, cg);

    // monitors match so we can unlock it
    // decrement count, maybe unlock object
    generateRegMemInstruction(
        LRegMem(), node, eaxReal, generateX86MemoryReference(unlockedReg, fej9->getMonitorEntryCountOffset(), cg), cg);
    ...
    // create the outlined path that decrements the count
    {
        ...
        generateMemInstruction(
            DECMem(cg), node, generateX86MemoryReference(unlockedReg, fej9->getMonitorEntryCountOffset(), cg), cg);
        generateLabelInstruction(JMP4, node, fallThru, cg);
    }
    ...
    cg->setImplicitExceptionPoint(
        generateMemRegInstruction(op, node, getMemoryReference(objectClassReg, objectReg, lwOffset, cg), eaxReal, cg));
    ...
    generateRegMemInstruction(LRegMem(), node, eaxReal,
        generateX86MemoryReference(vmThreadReg, fej9->thisThreadMonitorCacheOffset(), cg), cg);
    generateMemRegInstruction(
        SMemReg(), node, generateX86MemoryReference(unlockedReg, fej9->getMonitorNextOffset(), cg), eaxReal, cg);
    generateMemRegInstruction(SMemReg(), node,
        generateX86MemoryReference(vmThreadReg, fej9->thisThreadMonitorCacheOffset(), cg), unlockedReg, cg);

    TR_OutlinedInstructions* outlinedHelperCall
        = new (cg->trHeapMemory()) TR_OutlinedInstructions(node, TR::call, NULL, snippetLabel, fallThru, cg);
    cg->getOutlinedInstructionsList().push_front(outlinedHelperCall);
    cg->generateDebugCounter(outlinedHelperCall->getFirstInstruction(),
        TR::DebugCounter::debugCounterName(comp, "helperCalls/%s/(%s)/%d/%d", node->getOpCode().getName(),
            comp->signature(), node->getByteCodeInfo().getCallerIndex(), node->getByteCodeInfo().getByteCodeIndex()),
    ...
    if (comp->getOption(TR_ReservingLocks)) {
        if (reservingLock || normalLockPreservingReservation) {
            TR_RuntimeHelper helper;
            bool success = TR::TreeEvaluator::monEntryExitHelper(
                false, node, reservingLock, normalLockPreservingReservation, helper, cg);
    ...
    TR_OutlinedInstructions* outlinedHelperCall
        = new (cg->trHeapMemory()) TR_OutlinedInstructions(node, TR::call, NULL, snippetLabel, snippetFallThru, cg);
    cg->getOutlinedInstructionsList().push_front(outlinedHelperCall);
    cg->generateDebugCounter(outlinedHelperCall->getFirstInstruction(),
        TR::DebugCounter::debugCounterName(comp, "helperCalls/%s/(%s)/%d/%d", node->getOpCode().getName(),
            comp->signature(), node->getByteCodeInfo().getCallerIndex(), node->getByteCodeInfo().getByteCodeIndex()),
    ...
    if (inlineRecursive) {
        ...
        snippetLabel = inlineRecursiveSnippetLabel;
        TR::TreeEvaluator::inlineRecursiveMonitor(node, cg, fallThru, jitMonitorExitSnippetLabel,
            inlineRecursiveSnippetLabel, objectReg, lwOffset, snippetFallThru, reservingLock);
    }
    ...
    if (node->isReadMonitor()) {
        ...
        if (reservingLock) {
            ...
            if (TR::Compiler->target.is64Bit() && fej9->generateCompressedLockWord()) {
                generateRegRegInstruction(
                    XORRegReg(), node, tempReg, tempReg, cg); // Zero out tempReg before LRegMem op.
            }
            cg->setImplicitExceptionPoint(generateRegMemInstruction(LRegMem(gen64BitInstr), node, tempReg,
            ...
            // generateImmSymInstruction(PUSHImm4, node,
            // (uintptrj_t)doneTestLabel->getStaticSymbol()->getStaticAddress(), node->getSymbolReference(), cg);
            ...
            if (0) {
                ...
                generateRegRegInstruction(XOR4RegReg, node, scratchReg, scratchReg, cg);
                generateRegMemInstruction(
                    LRegMem(), node, scratchReg, generateX86MemoryReference(scratchReg, 0, cg), cg);
                generateLabelInstruction(LABEL, node, doneTestLabel, cg);
            }
#endif
            if (node->isPrimitiveLockedRegion()) {
                ...
                generateRegMemInstruction(
                    LEARegMem(), node, tempReg, generateX86MemoryReference(vmThreadReg, (REC_BIT | RES_BIT), cg), cg);
                cg->setImplicitExceptionPoint(generateMemRegInstruction(CMPMemReg(gen64BitInstr), node,
                    getMemoryReference(objectClassReg, objectReg, lwOffset, cg), tempReg, cg));
            }
        } else {
            cg->setImplicitExceptionPoint(generateRegMemInstruction(CMPRegMem(gen64BitInstr), node, vmThreadReg,
                getMemoryReference(objectClassReg, objectReg, lwOffset, cg), cg));
        }
    }
    ...
    if (reservingDecrementNeeded) {
        ...
        generateMemImmInstruction(SUBMemImms(gen64BitInstr), node,
            ...
            cg); // I'm not sure SUB4MemImms will work.
    }
    ...
    if (!node->isReadMonitor() && !reservingLock) {
        ...
        if (fej9->getX86SupportsHLE() && comp->getOption(TR_X86HLE))
            generateMemImmInstruction(
                XRSMemImm4(gen64BitInstr), node, getMemoryReference(objectClassReg, objectReg, lwOffset, cg), 0, cg);
        else
            generateMemImmInstruction(
                SMemImm4(gen64BitInstr), node, getMemoryReference(objectClassReg, objectReg, lwOffset, cg), 0, cg);
    }
    ...
    if (reservingLock) {
        ...
        cg->setImplicitExceptionPoint(generateRegMemInstruction(
            LRegMem(gen64BitInstr), node, tempReg, getMemoryReference(objectClassReg, objectReg, lwOffset, cg), cg));
...
// memory segment.
//
static void genHeapAlloc(TR::Node* node, TR_OpaqueClassBlock* clazz, int32_t allocationSizeOrDataOffset,
    int32_t elementSize, TR::Register* sizeReg, TR::Register* eaxReal, TR::Register* segmentReg, TR::Register* tempReg,
    TR::LabelSymbol* failLabel, TR::CodeGenerator* cg)
{
    ...
    if (TR::Options::getCmdLineOptions()->realTimeGC()) {
        ...
        if (sizeReg) {
            ...
            generateRegImmInstruction(CMPRegImm4(), node, sizeReg,
            ...
            generateRegMemInstruction(LRegMem(), node, segmentReg,
                generateX86MemoryReference(vmThreadReg, fej9->thisThreadJavaVMOffset(), cg), cg);
            generateRegMemInstruction(LRegMem(), node, segmentReg,
        ...
        //
        generateRegMemInstruction(
            LRegMem(), node, eaxReal, generateX86MemoryReference(vmThreadReg, heapAlloc_offset, cg), cg);
        ...
        if (comp->getOption(TR_EnableNewAllocationProfiling)) {
            ...
            if (globalAllocationDataPointer) {
                ...
                generateMemImmInstruction(CMP4MemImm4, node,
                    generateX86MemoryReference((uint32_t)(uintptrj_t)globalAllocationDataPointer, cg), 0x07ffffff, cg);
                generateLabelInstruction(JAE4, node, doneProfilingLabel, cg);

                generateMemInstruction(INC4Mem, node, gmr, cg);
                uint32_t* dataPointer = fej9->getAllocationProfilingDataPointer(
                    node->getByteCodeInfo(), clazz, node->getOwningMethod(), comp);
                if (dataPointer) {
        ...
        if (sizeReg) {
            ...
            if ((node->getOpCodeValue() == TR::anewarray || node->getOpCodeValue() == TR::newarray)) {
                ...
                TR_ASSERT(J9_GC_MINIMUM_OBJECT_SIZE >= 8,
                    "Expecting a minimum object size >= 8 (actual minimum is %d)\n", J9_GC_MINIMUM_OBJECT_SIZE);
            ...
            allocationSizeOrDataOffset = (allocationSizeOrDataOffset + fej9->getObjectAlignmentInBytes() - 1)
                & (-fej9->getObjectAlignmentInBytes());
            ...
            if ((node->getOpCodeValue() == TR::New)
                ...
                if ((comp->getMethodHotness() >= hot) && clazz && !cg->getCurrentEvaluationBlock()->isCold()
                    && TR::Compiler->cls.classInstanceSize(clazz) >= instanceSizeForAlignment) {
                    shouldAlignToCacheBoundary = true;

                    generateRegMemInstruction(
                        LEARegMem(), node, eaxReal, generateX86MemoryReference(eaxReal, 63, cg), cg);
        ...
        generateRegMemInstruction(
            CMPRegMem(), node, tempReg, generateX86MemoryReference(vmThreadReg, heapTop_offset, cg), cg);
        ...
        if (shouldAlignToCacheBoundary) {
            ...
            generateRegMemInstruction(
                LRegMem(), node, tempReg, generateX86MemoryReference(vmThreadReg, heapAlloc_offset, cg), cg);
            ...
            if (TR::Compiler->target.is64Bit() && fej9->generateCompressedLockWord()) {
                generateMemImmInstruction(
                    S4MemImm4, node, generateX86MemoryReference(tempReg, 0, cg), singleSlotHole, cg);
                generateMemImmInstruction(
                    S4MemImm4, node, generateX86MemoryReference(tempReg, 4, cg), singleSlotHole, cg);
            } else {
                generateMemImmInstruction(
                    SMemImm4(), node, generateX86MemoryReference(tempReg, 0, cg), singleSlotHole, cg);
            }
            ...
            generateMemImmInstruction(SMemImm4(), node, generateX86MemoryReference(tempReg, 0, cg), multiSlotHole, cg);

            generateMemRegInstruction(
                SMemReg(), node, generateX86MemoryReference(tempReg, sizeof(uintptrj_t), cg), eaxReal, cg);
        ...
        if (generateArraylets && (node->getOpCodeValue() == TR::anewarray || node->getOpCodeValue() == TR::newarray)) {
            generateRegMemInstruction(LEARegMem(), node, tempReg,
                generateX86MemoryReference(tempReg, fej9->getObjectAlignmentInBytes() - 1, cg), cg);
            if (TR::Compiler->target.is64Bit())
        ...
        generateMemRegInstruction(
            SMemReg(), node, generateX86MemoryReference(vmThreadReg, heapAlloc_offset, cg), tempReg, cg);
        ...
        if (!isSmallAllocation && cg->enableTLHPrefetching()) {
            ...
            if (!comp->getOption(TR_EnableNewX86PrefetchTLH)) {
                ...
                generateMemRegInstruction(
                    SUB4MemReg, node, generateX86MemoryReference(vmThreadReg, tlhPrefetchFTA_offset, cg), tempReg, cg);
                if (!useDirectPrefetchCall)
                    ...
                    TR::SymbolReference* helperSymRef = cg->getSymRefTab()->findOrCreateRuntimeHelper(
                        TR_X86CodeCachePrefetchHelper, false, false, false);
...
#endif
                    generateImmSymInstruction(
                        CALLImm4, node, (uintptr_t)helperSymbol->getMethodAddress(), helperSymRef, cg);
                }
            } else {
                ...
                //
                generateRegMemInstruction(
                    CMP4RegMem, node, tempReg, generateX86MemoryReference(vmThreadReg, tlhPrefetchFTA_offset, cg), cg);
        ...
        generateMemRegInstruction(
            CMPXCHGMemReg(), node, generateX86MemoryReference(vmThreadReg, heapAlloc_offset, cg), tempReg, cg);
...
static void genHeapAlloc2(TR::Node* node, TR_OpaqueClassBlock* clazz, int32_t allocationSizeOrDataOffset,
    int32_t elementSize, TR::Register* sizeReg, TR::Register* eaxReal, TR::Register* segmentReg, TR::Register* tempReg,
    TR::LabelSymbol* failLabel, TR::CodeGenerator* cg)
{
    // Load the current heap segment and see if there is room in it. Loop if
...

    {
        ...
        generateRegMemInstruction(CMPRegMem(), node, segmentReg,
        ...
        if (shouldAlignToCacheBoundary) {
            ...
            if (TR::Compiler->target.is64Bit() && fej9->generateCompressedLockWord()) {
                generateMemImmInstruction(
                    S4MemImm4, node, generateX86MemoryReference(segmentReg, 0, cg), singleSlotHole, cg);
                generateMemImmInstruction(
                    S4MemImm4, node, generateX86MemoryReference(segmentReg, 4, cg), singleSlotHole, cg);
            } else {
                generateMemImmInstruction(
                    SMemImm4(), node, generateX86MemoryReference(segmentReg, 0, cg), singleSlotHole, cg);
            }
            ...
            generateMemImmInstruction(
                SMemImm4(), node, generateX86MemoryReference(segmentReg, 0, cg), multiSlotHole, cg);

            generateMemRegInstruction(
                SMemReg(), node, generateX86MemoryReference(segmentReg, sizeof(uintptrj_t), cg), eaxReal, cg);
        ...
        generateMemRegInstruction(SMemReg(), node,
            generateX86MemoryReference(vmThreadReg, offsetof(J9VMThread, heapAlloc), cg), segmentReg, cg);
...
static void genInitObjectHeader(TR::Node* node, TR_OpaqueClassBlock* clazz, TR::Register* classReg,
    TR::Register* objectReg, TR::Register* tempReg, bool isZeroInitialized, TR::CodeGenerator* cg)
{
    TR::Compilation* comp = cg->comp();
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(cg->fe());
    ...
    if (cg->needClassAndMethodPointerRelocations() && !comp->getOption(TR_UseSymbolValidationManager)) {
        ...
        if (node->getOpCodeValue() == TR::newarray) {
            ...
            generateRegMemInstruction(
                LRegMem(), node, tempReg, generateX86MemoryReference(classReg, offsetof(J9Class, arrayClass), cg), cg);
            clzReg = tempReg;
        } else {
    ...
    if (!clzReg) {
        ...
        if (use64BitClasses) {
            if (cg->needClassAndMethodPointerRelocations() && comp->getOption(TR_UseSymbolValidationManager))
                instr = generateRegImm64Instruction(
                    MOV8RegImm64, node, tempReg, ((intptrj_t)clazz | orFlagsClass), cg, TR_ClassPointer);
            else
                instr = generateRegImm64Instruction(MOV8RegImm64, node, tempReg, ((intptrj_t)clazz | orFlagsClass), cg);
            generateMemRegInstruction(S8MemReg, node,
                generateX86MemoryReference(objectReg, TR::Compiler->om.offsetOfObjectVftField(), cg), tempReg, cg);
        } else {
    ...
    if (romClass) {
        ...
        generateMemImmInstruction(
            S4MemImm4, node, generateX86MemoryReference(objectReg, TMP_OFFSETOF_J9OBJECT_FLAGS, cg), orFlags, cg);
    ...
    if (!isZeroInitialized || initReservable) {
        ...
        if (initLw) {
            ...
            TR_X86OpCodes op
                = (TR::Compiler->target.is64Bit() && fej9->generateCompressedLockWord()) ? S4MemImm4 : SMemImm4();
            generateMemImmInstruction(
                op, node, generateX86MemoryReference(objectReg, lwOffset, cg), initialLwValue, cg);
        }
    }
}
...
static void genInitArrayHeader(TR::Node* node, TR_OpaqueClassBlock* clazz, TR::Register* classReg,
    ...
    TR::Register* tempReg, bool isZeroInitialized, TR::CodeGenerator* cg)
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(cg->fe());
    ...
    bool canUseFastInlineAllocation
        = (!TR::Options::getCmdLineOptions()->realTimeGC() && !comp->generateArraylets()) ? true : false;
    ...
    if (sizeReg) {
        ...
        if (canUseFastInlineAllocation) {
            ...
            TR_X86OpCodes storeOp
                = (TR::Compiler->target.is64Bit() && !comp->useCompressedPointers()) ? S8MemReg : S4MemReg;
        ...
        if (canUseFastInlineAllocation) {
            ...
            //
            TR_X86OpCodes storeOp
                = (TR::Compiler->target.is64Bit() && !comp->useCompressedPointers()) ? S8MemImm4 : S4MemImm4;
    ...
    if (generateArraylets) {
        ...
        generateRegMemInstruction(
            LEARegMem(), node, tempReg, generateX86MemoryReference(objectReg, arrayletDataOffset, cg), cg);
        ...
        if (comp->useCompressedPointers()) {
            ...
            if (TR::Compiler->vm.heapBaseAddress() != 0)
                ...
                generateRegImmInstruction(
                    SHR8RegImm1, node, tempReg, TR::Compiler->om.compressedReferenceShiftOffset(), cg);
        } else {
            storeOp = SMemReg();
        }
...
static bool genZeroInitObject2(TR::Node* node, int32_t objectSize, int32_t elementSize, TR::Register* sizeReg,
    TR::Register* targetReg, TR::Register* tempReg, TR::Register* segmentReg, TR::Register*& scratchReg,
    TR::CodeGenerator* cg)
{
    ...
    if (sizeReg || objectSize >= minRepstosdWords) {
        ...
        // -----------
        generateRegMemInstruction(
            LEARegMem(), node, segmentReg, generateX86MemoryReference(targetReg, headerSize, cg), cg);
        if (TR::Compiler->target.is64Bit()) {
            scratchReg = cg->allocateRegister();
        ...
        while (objectSize >= 16) {
            generateMemRegInstruction(
                MOVDQUMemReg, node, generateX86MemoryReference(targetReg, headerSize + offset, cg), scratchReg, cg);
            objectSize -= 16;
            offset += 16;
        }
        switch (objectSize) {
        case 8:
            generateMemRegInstruction(
                MOVQMemReg, node, generateX86MemoryReference(targetReg, headerSize + offset, cg), scratchReg, cg);
            break;
        case 4:
            generateMemRegInstruction(
                MOVDMemReg, node, generateX86MemoryReference(targetReg, headerSize + offset, cg), scratchReg, cg);
            break;
        case 0:
            break;
        default:
...
//
static bool genZeroInitObject(TR::Node* node, int32_t objectSize, int32_t elementSize, TR::Register* sizeReg,
    TR::Register* targetReg, TR::Register* tempReg, TR::Register* segmentReg, TR::Register*& scratchReg,
    TR::CodeGenerator* cg)
{
    ...
    if (sizeReg || (numSlots + alignmentDelta) >= minRepstosdWords) {
        ...
        generateRegMemInstruction(
            LEARegMem(), node, segmentReg, generateX86MemoryReference(targetReg, startOfZeroInits, cg), cg);
        ...
        if (sizeReg) {
            ...
            case 4:
                if (TR::Compiler->target.is64Bit()) {
                    ...
                    generateRegMemInstruction(
                        LEA4RegMem, node, tempReg, generateX86MemoryReference(sizeReg, additionalSlots, cg), cg);
                }
                break;
            case 8:
                if (TR::Compiler->target.is64Bit()) {
                    generateRegMemInstruction(
                        LEA8RegMem, node, tempReg, generateX86MemoryReference(sizeReg, additionalSlots, cg), cg);
                } else {
                    generateRegMemInstruction(LEA4RegMem, node, tempReg,
                        generateX86MemoryReference(
                            NULL, sizeReg, TR::MemoryReference::convertMultiplierToStride(2), additionalSlots, cg),
                        cg);
                }
                break;
            }
        ...
        if (initLw) {
            TR_X86OpCodes op
                = (TR::Compiler->target.is64Bit() && fej9->generateCompressedLockWord()) ? S4MemReg : SMemReg();
            generateMemRegInstruction(
                op, node, generateX86MemoryReference(segmentReg, lwOffset - startOfZeroInits, cg), targetReg, cg);
        }
    ...
    if (numIterations > 1) {
        ...
        generateRegImmInstruction(
            MOVRegImm4(), node, segmentReg, -((numIterations - 1) * maxZeroInitWordsPerIteration), cg);
        ...
        for (i = 0; i < numSlots; i++) {
            ...
            generateMemRegInstruction(SMemReg(), node,
                generateX86MemoryReference(
                    targetReg, i * TR::Compiler->om.sizeofReferenceAddress() + startOfZeroInits, cg),
                tempReg, cg);
        }
    }
...
TR::Register* J9::X86::TreeEvaluator::VMnewEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (skipOutlineZeroInit
        && !performTransformation(
               comp, "O^O OUTLINED NEW: skip outlined zero init on %s %p\n", cg->getDebug()->getName(node), node))
    ...
    if (comp->getOption(TR_DisableDualTLH) || comp->getOptions()->realTimeGC()) {
        ...
        if (initInfo && initInfo->zeroInitSlots) {
            ...
            if (UseOldBVI) {
                ...
                while (bvi.hasMoreElements()) {
                    ...
                    if (span < 3) {
                        ...
                        generateMemRegInstruction(MOVDQUMemReg, node,
                        ...
                        if (lastSpan == 0) {
                            generateMemRegInstruction(MOVDMemReg, node,
                                generateX86MemoryReference(targetReg, lastElementIndex * 4 + dataOffset, cg),
                                scratchReg, cg);
                        } else if (lastSpan == 1) {
                ...
                if (lastSpan == 0) {
                    generateMemRegInstruction(MOVDMemReg, node,
                        generateX86MemoryReference(targetReg, lastElementIndex * 4 + dataOffset, cg), scratchReg, cg);
                } else if (lastSpan == 1) {
                    generateMemRegInstruction(MOVQMemReg, node,
                        generateX86MemoryReference(targetReg, lastElementIndex * 4 + dataOffset, cg), scratchReg, cg);
                } else if (lastSpan == 2) {
                    ...
                    generateMemRegInstruction(MOVDQUMemReg, node,
                        generateX86MemoryReference(targetReg, lastElementIndex * 4 + dataOffset - 4, cg), scratchReg,
                        cg);
                }
            }
            ...
            //
            if (canUseFastInlineAllocation) {
                useRepInstruction = genZeroInitObject2(
                    node, objectSize, elementSize, sizeReg, targetReg, tempReg, segmentReg, scratchReg, cg);
            } else {
                useRepInstruction = genZeroInitObject(
                    node, objectSize, elementSize, sizeReg, targetReg, tempReg, segmentReg, scratchReg, cg);
            }
    ...
    if (fej9->inlinedAllocationsMustBeVerified() && !comp->getOption(TR_UseSymbolValidationManager)
        ...
        genInitArrayHeader(
            node, clazz, classReg, targetReg, sizeReg, elementSize, dataOffset, tempReg, monitorSlotIsInitialized, cg);
    } else if (isArrayNew) {
        genInitArrayHeader(
            node, clazz, NULL, targetReg, sizeReg, elementSize, dataOffset, tempReg, monitorSlotIsInitialized, cg);
    } else {
    ...
    if (fej9->inlinedAllocationsMustBeVerified()
        ...
        cg->addExternalRelocation(new (cg->trHeapMemory()) TR::BeforeBinaryEncodingExternalRelocation(
                                      startInstr, (uint8_t*)classSymRef, (uint8_t*)recordInfo, reloKind, cg),
    ...
    if (!disableArrayStoreCheckOpts || !debug("noInlinedArrayStoreCHKs")) {
        ...
        if (TR::Compiler->target.is64Bit()) {
            ...
            instr = generateRegImmInstruction(
                CMP4RegImm4, node, destComponentClassReg, (uint32_t)((uint64_t)objectClass), cg);
            ...
            instr = generateRegImmInstruction(
                CMP4RegImm4, node, destComponentClassReg, (int32_t)(uintptr_t)objectClass, cg);
        }
        ...
        generateMemRegInstruction(CMP4MemReg, node,
            generateX86MemoryReference(sourceClassReg, offsetof(J9Class, castClassCache), cg), destComponentClassReg,
            cg);
        ...
        generateMemRegInstruction(CMPMemReg(), node,
            generateX86MemoryReference(sourceClassReg, offsetof(J9Class, castClassCache), cg), destComponentClassReg,
            cg);
        ...
        if (TR::Compiler->target.is64Bit())
           {
  #ifdef J9VM_INTERP_COMPRESSED_OBJECT_HEADER
           TR_ASSERT((((uintptr_t)objectClass) >> 32) == 0, "TR_OpaqueClassBlock must fit on 32 bits when using class
  pointer compression"); instr = generateRegImmInstruction(CMP4RegImm4, node, destComponentClassReg, (uint32_t)
  ((uint64_t) objectClass), cg);
           ...
           {
           instr = generateRegImmInstruction(CMP4RegImm4, node, destComponentClassReg, (int32_t)(uintptr_t) objectClass,
  cg);
           }
        ...
        */
        ...
        if (!(comp->getOption(TR_DisableArrayStoreCheckOpts)) && node->getArrayComponentClassInNode()) {
            ...
            if (TR::Compiler->target.is64Bit()) {
                ...
                TR_ASSERT((((uintptr_t)arrayComponentClass) >> 32) == 0,
                    "TR_OpaqueClassBlock must fit on 32 bits when using class pointer compression");
                instr = generateRegImmInstruction(
                    CMP4RegImm4, node, destComponentClassReg, (uint32_t)((uint64_t)arrayComponentClass), cg);
                ...
                if ((uintptrj_t)arrayComponentClass <= (uintptrj_t)0x7fffffff) {
                    instr = generateRegImmInstruction(
                        CMP8RegImm4, node, destComponentClassReg, (uintptr_t)arrayComponentClass, cg);
                    if (fej9->isUnloadAssumptionRequired(arrayComponentClass, comp->getCurrentMethod()))
                        comp->getStaticPICSites()->push_front(instr);
                    ...
                    instr = generateRegImm64Instruction(
                        MOV8RegImm64, node, arrayComponentClassReg, (uintptr_t)arrayComponentClass, cg);
                ...
                instr = generateRegImmInstruction(
                    CMP4RegImm4, node, destComponentClassReg, (int32_t)(uintptr_t)arrayComponentClass, cg);
                if (fej9->isUnloadAssumptionRequired(arrayComponentClass, comp->getCurrentMethod()))
                    comp->getStaticPICSites()->push_front(instr);
            }
        ...
        if (eliminateDepthMask) {
            if (TR::Compiler->target.is64Bit())
                generateRegMemInstruction(
                    MOVZXReg8Mem2, node, destComponentClassDepthReg, destComponentClassDepthMR, cg);
            else
                generateRegMemInstruction(
                    MOVZXReg4Mem2, node, destComponentClassDepthReg, destComponentClassDepthMR, cg);
        } else {
            generateRegMemInstruction(LRegMem(), node, destComponentClassDepthReg, destComponentClassDepthMR, cg);
        }

        if (!eliminateDepthMask) {
            if (TR::Compiler->target.is64Bit()) {
                TR_ASSERT(
                    !(J9_JAVA_CLASS_DEPTH_MASK & 0x80000000), "AMD64: need to use a second register for AND mask");
                if (!(J9_JAVA_CLASS_DEPTH_MASK & 0x80000000))
                    generateRegImmInstruction(
                        AND8RegImm4, node, destComponentClassDepthReg, J9_JAVA_CLASS_DEPTH_MASK, cg);
            } else {
        ...
        if (eliminateDepthMask) {
            ...
            generateRegMemInstruction(LRegMem(), node, sourceClassDepthReg, mr, cg);
        ...
        generateRegMemInstruction(LRegMem(), node, sourceSuperClassReg, tempMR, cg);

        TR::MemoryReference* leaMR = generateX86MemoryReference(
            sourceSuperClassReg, destComponentClassDepthReg, logBase2(sizeof(uintptrj_t)), 0, cg);
...
TR::Register* J9::X86::TreeEvaluator::VMarrayCheckEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (node->isArrayChkPrimitiveArray1() || node->isArrayChkReferenceArray1() || node->isArrayChkPrimitiveArray2()
...
#ifdef J9VM_INTERP_COMPRESSED_OBJECT_HEADER
        generateRegMemInstruction(L4RegMem, node, tempReg,
        ...
        generateMemImmInstruction(testOpCode, node,
            generateX86MemoryReference(tempReg, offsetof(J9Class, classDepthAndFlags), cg), J9_JAVA_CLASS_RAM_ARRAY,
            cg);
        if (!snippetLabel) {
    ...
    //
    generateRegMemInstruction(LRegMem(use64BitClasses), node, tempReg,
        generateX86MemoryReference(object1Reg, TR::Compiler->om.offsetOfObjectVftField(), cg), cg);
    generateRegMemInstruction(XORRegMem(use64BitClasses), node, tempReg,
    ...
    //
        ...
        if (!node->isArrayChkReferenceArray1()) {
            ...
            generateRegImmInstruction(
                ANDRegImm4(), node, tempReg, (OBJECT_HEADER_SHAPE_MASK << J9_JAVA_CLASS_RAM_SHAPE_SHIFT), cg);
            generateRegImmInstruction(
                CMPRegImm4(), node, tempReg, (OBJECT_HEADER_SHAPE_POINTERS << J9_JAVA_CLASS_RAM_SHAPE_SHIFT), cg);
...
static bool inlineNanoTime(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (fej9->supportsFastNanoTime()) { // Fully Inlined Version
        ...
        TR::SymbolReference* gtod
            = comp->getSymRefTab()->findOrCreateRuntimeHelper(TR_AMD64clockGetTime, false, false, false);
        ...
        // reg = tv_usec
        generateRegMemInstruction(
            L8RegMem, node, reg, generateX86MemoryReference(*tv_sec, offsetof(struct timespec, tv_nsec), cg), cg);
    ...
    if (fej9->supportsFastNanoTime()) {
        ...
        if (node->getNumChildren() == 1) {
            if (node->getReferenceCount() > 1
                ...
                generateRegMemInstruction(
                    L4RegMem, node, highReg, generateX86MemoryReference(resultAddress, 4, cg), cg);
        ...
        generateRegMemInstruction(
            L4RegMem, node, temp2, generateX86MemoryReference(vmThreadReg, offsetof(J9VMThread, javaVM), cg), cg);
        generateRegMemInstruction(
            L4RegMem, node, temp2, generateX86MemoryReference(temp2, offsetof(J9JavaVM, portLibrary), cg), cg);
        ...
        generateCallMemInstruction(
            CALLMem, node, generateX86MemoryReference(temp2, offsetof(OMRPortLibrary, time_hires_clock), cg), deps, cg);
...
static TR::Register* inlineStringHashCode(TR::Node* node, bool isCompressed, TR::CodeGenerator* cg)
{
    if (!cg->getSupportsInlineStringHashCode()) {
        ...
        generateRegMemInstruction(
            CMOVE4RegMem, node, index, generateX86MemoryReference(cg->findOrCreate4ByteConstant(node, size), cg), cg);
        ...
        // Prepend zeros
        {
            ...
            generateRegMemInstruction(isCompressed ? MOVDRegMem : MOVQRegMem, node, hashXMM,
                generateX86MemoryReference(
                    address, index, shift, -(size << shift) + TR::Compiler->om.contiguousArrayHeaderSizeInBytes(), cg),
                cg);
            generateRegMemInstruction(LEARegMem(), node, tmp,
                generateX86MemoryReference(
        ...
        // Reduction Loop
        {
            ...
            generateRegMemInstruction(MOVDQURegMem, node, multiplierXMM,
            ...
            generateRegMemInstruction(isCompressed ? PMOVZXBDRegMem : PMOVZXWDRegMem, node, tmpXMM,
                generateX86MemoryReference(
                    address, index, shift, TR::Compiler->om.contiguousArrayHeaderSizeInBytes(), cg),
                cg);
        ...
        // Finalization
        {
            ...
            generateRegMemInstruction(PMULLDRegMem, node, hashXMM,
                generateX86MemoryReference(cg->findOrCreate16ByteConstant(node, multiplier), cg), cg);
...
static TR::Register* inlineIntrinsicIndexOf(TR::Node* node, bool isLatin1, TR::CodeGenerator* cg)
{
    ...
    generateRegMemInstruction(PSHUFBRegMem, node, valueXMM,
    ...
    generateLabelInstruction(LABEL, node, begLabel, cg);
    generateRegMemInstruction(LEARegMem(), node, scratch,
        generateX86MemoryReference(address, result, shift, TR::Compiler->om.contiguousArrayHeaderSizeInBytes(), cg),
        cg);
    ...
    generateLabelInstruction(LABEL, node, loopLabel, cg);
    generateRegMemInstruction(MOVDQURegMem, node, scratchXMM,
        generateX86MemoryReference(address, result, shift, TR::Compiler->om.contiguousArrayHeaderSizeInBytes(), cg),
        cg);
    generateRegRegInstruction(compareOp, node, scratchXMM, valueXMM, cg);
    generateRegRegInstruction(PMOVMSKB4RegReg, node, scratch, scratchXMM, cg);
    ...
    generateRegMemInstruction(CMOVGERegMem(), node, result,
        generateX86MemoryReference(TR::Compiler->target.is32Bit() ? cg->findOrCreate4ByteConstant(node, -1)
...
static TR::Register* inlineCompareAndSwapObjectNative(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->om.shouldGenerateReadBarriersForFieldLoads()) {
        generateRegMemInstruction(
            LRegMem(use64BitClasses), node, tmp, generateX86MemoryReference(object, offset, 0, cg), cg);
        ...
        generateRegMemInstruction(CMPRegMem(use64BitClasses), node, tmp,
            generateX86MemoryReference(
                cg->getVMThreadRegister(), cg->comp()->fej9()->thisThreadGetEvacuateBaseAddressOffset(), cg),
            cg);
        generateLabelInstruction(JAE4, node, rdbarLabel, cg);
    ...
    generateMemRegInstruction(use64BitClasses ? LCMPXCHG8MemReg : LCMPXCHG4MemReg, node,
    ...
    TR::TreeEvaluator::VMwrtbarWithoutStoreEvaluator(
        node, objectNode, newValueNode, NULL, cg->generateScratchRegisterManager(), cg);
...
*/
static bool inlineCompareAndSwapNative(TR::Node* node, int8_t size, bool isObject, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    ...
    if (comp->useCompressedPointers() && isObject && (newValueChild->getDataType() != TR::Address)) {
        ...
        if (TR::Compiler->vm.heapBaseAddress() == 0 || newValueChild->isNull())
            usingLowMemHeap = true;
    ...
    TR::Register* oldValueRegister
        = (size == 8) ? cg->longClobberEvaluate(oldValueChild) : cg->intClobberEvaluate(oldValueChild);
    ...
    if (!comp->getOptions()->realTimeGC() && isObject) {
        ...
        TR::TreeEvaluator::VMwrtbarWithoutStoreEvaluator(
            node, objectChild, translatedNode, NULL, scratchRegisterManager, cg);
    }
    ...
    if (methodSymbol) {
            ...
            if (node->isSafeForCGToFastPathUnsafeCall()) {
                if (UseOldCompareAndSwapObject)
                    return inlineCompareAndSwapNative(
                        node, (TR::Compiler->target.is64Bit() && !comp->useCompressedPointers()) ? 8 : 4, true, cg);
                else {
        ...
        case TR::java_lang_Short_reverseBytes: {
            if (comp->getOption(TR_EnableJCLInline)
                && performTransformation(
                       comp, "O^O Enable JCL Integer/Long methods inline for: %s\n", cg->getDebug()->getName(node)))
        ...
        case TR::java_util_concurrent_atomic_Fences_orderReads: {
            if (cg->getX86ProcessorInfo().requiresLFENCE() && cg->getX86ProcessorInfo().supportsLFence()) {
                TR_X86OpCode fenceOp;
 ...
 */
static void generateWriteBarrierCall(TR_X86OpCodes branchOp, TR::Node* node, TR_WriteBarrierKind gcMode,
    TR::Register* owningObjectReg, TR::Register* sourceReg, TR::LabelSymbol* doneLabel, TR::CodeGenerator* cg)
{
    TR_ASSERT(gcMode != TR_WrtbarRealTime && !TR::Options::getCmdLineOptions()->realTimeGC(),
    ...
    if (TR::Compiler->target.is64Bit()) {
        ...
        generateImmSymInstruction(CALLImm4, node, (uintptrj_t)wrtBarSymRef->getMethodAddress(), wrtBarSymRef, cg)
            ->setAdjustsFramePointerBy(-4 * helperArgCount);
    }
...
static int32_t byteOffsetForMask(int32_t mask, TR::CodeGenerator* cg)
{
    ...
    if (result != -1
        && performTransformation(
               cg->comp(), "O^O TREE EVALUATION: Use 1-byte TEST with offset %d for mask %08x\n", result, mask))
        return result;
    ...
    TR_X86ScratchRegisterManager* srm, TR::CodeGenerator* cg)
{
    ...
    TR::Node* callNode = TR::Node::createWithSymRef(
        TR::call, 3, 3, sourceObject, dummyDestAddressNode, destOwningObject, wrtBarSymRef);
    ...
    TR_X86ScratchRegisterManager* srm, TR::CodeGenerator* cg)
{
    ...
    if (wrtbarNode) {
        TR_ASSERT(
            wrtbarNode->getOpCode().isWrtBar(), "Expected node " POINTER_PRINTF_FORMAT " to be a WrtBar", wrtbarNode);
    ...
    bool doCheckConcurrentMarkActive = (gcMode == TR_WrtbarCardMark || gcMode == TR_WrtbarCardMarkAndOldCheck
                                           || gcMode == TR_WrtbarCardMarkIncremental)
    ...
    TR::MemoryReference* fragmentParentMR = generateX86MemoryReference(
        cg->getVMThreadRegister(), fej9->thisThreadRememberedSetFragmentOffset() + fej9->getFragmentParentOffset(), cg);
    TR::MemoryReference* localFragmentIndexMR = generateX86MemoryReference(
        cg->getVMThreadRegister(), fej9->thisThreadRememberedSetFragmentOffset() + fej9->getLocalFragmentOffset(), cg);
    ...
    if (doInlineCardMarkingWithoutOldSpaceCheck && doCheckConcurrentMarkActive) {
        TR::MemoryReference* vmThreadPrivateFlagsMR
            = generateX86MemoryReference(cg->getVMThreadRegister(), offsetof(J9VMThread, privateFlags), cg);
        generateMemImmInstruction(
            TEST4MemImm4, node, vmThreadPrivateFlagsMR, J9_PRIVATE_FLAGS_CONCURRENT_MARK_ACTIVE, cg);
        ...
        if (comp->getOptions()->isVariableHeapBaseForBarrierRange0()) {
            TR::MemoryReference* vhbMR = generateX86MemoryReference(
                cg->getVMThreadRegister(), offsetof(J9VMThread, heapBaseForBarrierRange0), cg);
            ...
            if (TR::Compiler->target.is64Bit()
                ...
                generateRegImmInstruction(
                    SUBRegImm4(), node, tempReg, (int32_t)chb, cg, TR_HEAP_BASE_FOR_BARRIER_RANGE);
            }
        }
        ...
        if (doIsDestAHeapObjectCheck) {
            ...
            if (comp->getOptions()->isVariableHeapSizeForBarrierRange0()) {
                TR::MemoryReference* vhsMR = generateX86MemoryReference(
                    cg->getVMThreadRegister(), offsetof(J9VMThread, heapSizeForBarrierRange0), cg);
                ...
                if (TR::Compiler->target.is64Bit()
                    ...
                    generateRegImmInstruction(
                        CMPRegImm4(), node, tempReg, (int32_t)chs, cg, TR_HEAP_SIZE_FOR_BARRIER_RANGE);
                }
            }
        ...
        generateRegImmInstruction(
            SHRRegImm1(), node, tempReg, comp->getOptions()->getHeapAddressToCardAddressShift(), cg);
    ...
    if (doIsDestInOldSpaceCheck) {
        ...
        if (gcMode == TR_WrtbarAlways) {
            ...
            if (!comp->getOption(TR_DisableWriteBarriersRangeCheck) && (node->getOpCodeValue() == TR::awrtbari)
                && doInternalControlFlow) {
                bool is64Bit = TR::Compiler->target.is64Bit(); // On compressed refs, owningObjectReg is already
                                                               // uncompressed, and the vmthread fields are 64 bits
                labelAfterBranchToSnippet = generateLabelSymbol(cg);
                // AOT support to be implemented in another PR
                if (!comp->getOptions()->isVariableHeapSizeForBarrierRange0() && !cg->comp()->compileRelocatableCode()
                    && !disableWrtbarOpt) {
                    ...
                    if (TR::Compiler->target.is64Bit() && !IS_32BIT_SIGNED(che)) {
                        generateRegMemInstruction(CMP8RegMem, node, owningObjectReg,
                    ...
                    if (TR::Compiler->target.is64Bit()
                        ...
                        generateRegImm64Instruction(
                            MOV8RegImm64, node, chbReg, chb, cg, TR_HEAP_BASE_FOR_BARRIER_RANGE);
                        generateRegRegInstruction(SUBRegReg(), node, tempOwningObjReg, chbReg, cg);
                        srm->reclaimScratchRegister(chbReg);
                    } else {
                        generateRegImmInstruction(
                            SUBRegImm4(), node, tempOwningObjReg, (int32_t)chb, cg, TR_HEAP_BASE_FOR_BARRIER_RANGE);
                    }
                    TR::MemoryReference* vhsMR1 = generateX86MemoryReference(
                        cg->getVMThreadRegister(), offsetof(J9VMThread, heapSizeForBarrierRange0), cg);
            ...
            if (byteOffset != -1) {
                TR::MemoryReference* vmThreadPrivateFlagsMR = generateX86MemoryReference(
                    cg->getVMThreadRegister(), byteOffset + offsetof(J9VMThread, privateFlags), cg);
                generateMemImmInstruction(TEST1MemImm1, node, vmThreadPrivateFlagsMR,
        ...
        if (skipSnippetIfSrcNotOld || skipSnippetIfDestOld) {
            ...
            if (!comp->getOptions()->isVariableHeapSizeForBarrierRange0() && !cg->comp()->compileRelocatableCode()
                ...
                uintptr_t che = comp->getOptions()->getHeapBaseForBarrierRange0()
                    + comp->getOptions()->getHeapSizeForBarrierRange0();
                if (TR::Compiler->target.is64Bit() && !IS_32BIT_SIGNED(che)) {
                    generateRegMemInstruction(CMP8RegMem, node, checkDest ? owningObjectReg : srcReg,
                        generateX86MemoryReference(cg->findOrCreate8ByteConstant(node, che), cg), cg);
                } else {
                    generateRegImmInstruction(
                        CMPRegImm4(), node, checkDest ? owningObjectReg : srcReg, (int32_t)che, cg);
                }
            } else {
                uintptr_t chb = comp->getOptions()->getHeapBaseForBarrierRange0();
                TR::Register* tempReg = srm->findOrCreateScratchRegister();
            ...
            if (skipSnippetIfDestRemembered) {
                ...
                if (byteOffset != -1) {
                    TR::MemoryReference* MR = generateX86MemoryReference(
                        owningObjectReg, byteOffset + TR::Compiler->om.offsetOfHeaderFlags(), cg);
                    generateMemImmInstruction(
                        TEST1MemImm1, node, MR, J9_OBJECT_HEADER_REMEMBERED_MASK_FOR_TEST >> (8 * byteOffset), cg);
                } else {
                    TR::MemoryReference* MR
    ...
    bool isImplicitExceptionPoint, TR::CodeGenerator* cg, bool nullAdjusted)
{
    ...
    if (comp->useCompressedPointers()
        ...
        if ((TR::Compiler->vm.heapBaseAddress() == 0) || translatedStore->getSecondChild()->isNull())
            usingLowMemHeap = true;
    ...
    if (comp->getOptions()->alwaysCallWriteBarrier() && !isRealTimeGC) {
        ...
        if (TR::Compiler->target.is64Bit()) {
            ...
            TR::X86ImmSymInstruction* instr = generateImmSymInstruction(
                CALLImm4, node, (int32_t)(uintptr_t)wrtBarSymRef->getMethodAddress(), wrtBarSymRef, cg);
        ...
        if (isRealTimeGC) {
            ...
            TR::TreeEvaluator::VMwrtbarWithoutStoreEvaluator(
                node, destOwningObject, sourceObject, NULL, scratchRegisterManager, cg);
        }
    }
...
void J9::X86::TreeEvaluator::generateVFTMaskInstruction(TR::Node* node, TR::Register* reg, TR::CodeGenerator* cg)
{
    ...
    bool is64Bit = TR::Compiler->target.is64Bit(); // even with compressed object headers, a 64-bit mask operation is
...
void VMgenerateCatchBlockBBStartPrologue(TR::Node* node, TR::Instruction* fenceInstruction, TR::CodeGenerator* cg)
{
    ...
    if (fej9->shouldPerformEDO(block, comp)) {
        ...
        generateMemInstruction(DEC4Mem, node,
            generateX86MemoryReference((intptrj_t)comp->getRecompilationInfo()->getCounterAddress(), cg), cg);
        generateLabelInstruction(JE4, node, snippetLabel, cg);
        generateLabelInstruction(LABEL, node, restartLabel, cg);
...
TR::Register* J9::X86::TreeEvaluator::compressStringEvaluator(
...
static TR::Register* allocateRegAndAddCondition(
    TR::CodeGenerator* cg, TR::RegisterDependencyConditions* deps, TR_RegisterKinds rk = TR_GPR)
{
 ...
 */
TR::Register* J9::X86::TreeEvaluator::stringCaseConversionHelper(
...
#define iComment(str) \
    ...
    // charater to # byte conversion will cause overflow.
    ...
    cursor = generateRegMemInstruction(MOVDQURegMem, node, xmmRegLowerBndMinus1, lowerBndMinus1, cg);
    ...
    cursor = generateRegMemInstruction(MOVDQURegMem, node, xmmRegUpperBnd, upperBnd, cg);
    ...
    cursor = generateRegMemInstruction(MOVDQURegMem, node, xmmRegConversionDiff, conversionDiff, cg);
    iComment("case conversion diff value");

    auto minus1 = generateX86MemoryReference(cg->findOrCreate16ByteConstant(node, MINUS1), cg);
    cursor = generateRegMemInstruction(MOVDQURegMem, node, xmmRegMinus1, minus1, cg);
    iComment("-1");

    auto ascciUpperBnd = generateX86MemoryReference(cg->findOrCreate16ByteConstant(node, manager.getAsciiMax()), cg);
    cursor = generateRegMemInstruction(MOVDQURegMem, node, xmmRegAsciiUpperBnd, ascciUpperBnd, cg);
    iComment("maximum ascii value ");

    generateRegRegInstruction(MOVRegReg(), node, result, dstArray, cg);

    // initialize the loop counter
    cursor = generateRegRegInstruction(XORRegReg(), node, counter, counter, cg);
    ...
    cursor = generateRegRegInstruction(
        manager.isCompressedString() ? PCMPGTBRegReg : PCMPGTWRegReg, node, xmmRegArrayContentCopy1, xmmRegMinus1, cg);
    iComment(" > -1");
    ...
    iComment(" > maximum ascii value");
    cursor = generateRegRegInstruction(PANDNRegReg, node, xmmRegArrayContentCopy2, xmmRegArrayContentCopy1, cg);
    iComment(" >-1 && !(> maximum ascii value) valid when all bits are set");
    cursor = generateRegRegInstruction(PXORRegReg, node, xmmRegArrayContentCopy2, xmmRegMinus1, cg);
    ...
    cursor = generateRegRegInstruction(PANDNRegReg, node, xmmRegArrayContentCopy1, xmmRegArrayContentCopy0, cg);
    ...
    generateRegMemInstruction(
        manager.isCompressedString() ? MOVZXReg4Mem1 : MOVZXReg4Mem2, node, singleChar, srcArrayMemRef, cg);
    ...
    if (manager.toLowerCase())
        generateRegMemInstruction(LEARegMem(), node, singleChar, generateX86MemoryReference(singleChar, 0x20, cg), cg);

    else
        generateRegImmInstruction(SUB4RegImms, node, singleChar, 0x20, cg);
...
TR::Register* J9::X86::TreeEvaluator::compressStringNoCheckEvaluator(
    TR::Node* node, TR::CodeGenerator* cg, bool japaneseMethod)
{
    TR::Node *srcObjNode, *dstObjNode, *startNode, *lengthNode;
    TR::Register *srcObjReg, *dstObjReg, *lengthReg, *startReg;
    ...
    // Now that we have all the registers, set up the dependencies
    TR::RegisterDependencyConditions* dependencies = generateRegisterDependencyConditions((uint8_t)0, 5, cg);
    dependencies->addPostCondition(srcObjReg, TR::RealRegister::esi, cg);
    dependencies->addPostCondition(dstObjReg, TR::RealRegister::edi, cg);
    dependencies->addPostCondition(lengthReg, TR::RealRegister::ecx, cg);
    dependencies->addPostCondition(startReg, TR::RealRegister::eax, cg);
...
TR::Register* J9::X86::TreeEvaluator::andORStringEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR_RuntimeHelper helper = TR::Compiler->target.is64Bit() ? TR_AMD64andORString : TR_IA32andORString;
    generateHelperCallInstruction(node, helper, dependencies, cg);
    cg->stopUsingRegister(dummy);
 ...
 * jmp restartLabel
 */
void generateFillInDataBlockSequenceForUnresolvedField(
    TR::Node* node, J9Method* owningMethod, TR::X86DataSnippet* dataSnippet, bool isWrite, TR::CodeGenerator* cg)
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(cg->fe());
    TR::SymbolReference* symRef = node->getSymbolReference();
    ...
    generateMemImmInstruction(
        CMPMemImms(), node, generateX86MemoryReference(dataBlockReg, offsetInDataBlock, cg), -1, cg);
...

    {
        ...
        if (isStatic) {
            ...
            TR::Register* fieldClassReg
                = cg->evaluate(isWrite ? node->getSecondChild()->getFirstChild() : node->getFirstChild());
            generateMemRegInstruction(SMemReg(is64Bit), node,
                generateX86MemoryReference(
                    dataBlockReg, (intptrj_t)(offsetof(J9JITWatchedStaticFieldData, fieldClass)), cg),
                fieldClassReg, cg);
        }
        ...
        TR::Node* helperCallNode = TR::Node::createWithSymRef(TR::acall, 2, 2, cpIndexNode, cpAddressNode,
            cg->comp()->getSymRefTab()->findOrCreateRuntimeHelper(
                helperIndex, true /* canGCandReturn */, true /*canGCandExcept*/, false /* preservesAllRegisters*/));
        helperCallNode->incReferenceCount();
        TR::Register* resultReg
            = TR::TreeEvaluator::performCall(helperCallNode, false /* isIndirect */, false /* spillFPRegs */, cg);
        ...
        */
        if (!isStatic)
            generateRegImmInstruction(SubRegImm4(is64Bit, false /*isWithBorrow*/), node, resultReg,
        ...
        generateMemRegInstruction(
            SMemReg(is64Bit), node, generateX86MemoryReference(dataBlockReg, offsetInDataBlock, cg), resultReg, cg);
        generateLabelInstruction(JMP4, node, endLabel, cg);
 ...
 *
 */
void generateReportFieldAccessOutlinedInstructions(
    TR::Node* node, TR::LabelSymbol* endLabel, TR::X86DataSnippet* dataSnippet, bool isWrite, TR::CodeGenerator* cg)
{
    bool is64Bit = TR::Compiler->target.is64Bit();
    ...
    if (isWrite) {
        ...
        TR::SymbolReference *tempSymRef = cg->comp()->getSymRefTab()->createTemporary(cg->comp()->getMethodSymbol(),
        valueNode->getType()); TR::MemoryReference *mr = generateX86MemoryReference(tempSymRef, cg);
    ...
    if (is64Bit) {
        dependencies->addPostCondition(
            dataBlockReg, linkageProperties.getArgumentRegister(numArgs, false /* isFloat */), cg);
        numArgs++;

        if (isInstanceField) {
            dependencies->addPostCondition(
                objectReg, linkageProperties.getArgumentRegister(numArgs, false /* isFloat */), cg);
            numArgs++;
        }

        if (isWrite)
            dependencies->addPostCondition(
                valueReferenceReg, linkageProperties.getArgumentRegister(numArgs, false /* isFloat */), cg);
 ...
 */
void directReadWriteBarrierHelperForFieldWatch(
    TR::Node* node, J9Method* owningMethod, int32_t bcIndex, bool isWrite, TR::CodeGenerator* cg)
{
    TR::Compilation* comp = cg->comp();
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(cg->fe());
    TR::SymbolReference* symRef = node->getSymbolReference();
    ...
    if (isResolved) {
        fieldClass = (J9Class*)symRef->getOwningMethod(comp)->getDeclaringClassFromFieldOrStatic(
            cg->comp(), symRef->getCPIndex());
        TR_ASSERT(fieldClass, "valid field class is expected for resolved field reference" POINTER_PRINTF_FORMAT, node);
        staticFieldDataBlock.fieldAddress
            = (void*)node->getSymbolReference()->getSymbol()->getStaticSymbol()->getStaticAddress();
    ...
    TR_ASSERT(J9ClassHasWatchedFields >= SHRT_MIN && J9ClassHasWatchedFields <= SHRT_MAX,
        "expect value of J9ClassHasWatchedFields to be with in 16 bits");
 ...
 */
void indirectReadWriteBarrierHelperForFieldWatch(
    TR::Node* node, J9Method* owningMethod, int32_t bcIndex, bool isWrite, TR::CodeGenerator* cg)
{
    TR::Compilation* comp = cg->comp();
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(cg->fe());
    ...
    if (isResolved)
        instanceFieldDataBlock.offset
            = (UDATA)(node->getSymbolReference()->getOffset() - TR::Compiler->om.objectHeaderSizeInBytes());
    else
    ...
    TR_ASSERT(J9ClassHasWatchedFields >= SHRT_MIN && J9ClassHasWatchedFields <= SHRT_MAX,
        "expect value of J9ClassHasWatchedFields to be with in 16 bits");
    generateMemImmInstruction(TEST2MemImm2, node,
        generateX86MemoryReference(j9classReg, (uintptrj_t)(fej9->getOffsetOfClassFlags()), cg),
        J9ClassHasWatchedFields, cg);
    generateLabelInstruction(JNE4, node, fieldReportLabel, cg);
...
void J9::X86::TreeEvaluator::VMwrtbarEvaluatorForFieldWatch(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (isStatic)
        ...
        indirectReadWriteBarrierHelperForFieldWatch(node, owningMethod, bcIndex, true /* isWrite */, cg);
}
