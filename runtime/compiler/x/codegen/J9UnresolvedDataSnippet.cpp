...
uint8_t* J9::X86::UnresolvedDataSnippet::emitResolveHelperCall(uint8_t* cursor)
{
    ...
    cg()->addProjectSpecializedRelocation(
        cursor + 1, (uint8_t*)_glueSymRef, NULL, TR_HelperAddress, __FILE__, __LINE__, getNode());
...
uint32_t J9::X86::UnresolvedDataSnippet::getUnresolvedStaticStoreDeltaWithMemBarrier()
{
    ...
    // Find the delta between the start of the memory barrier that should be gaurding the store to the static field, and
    ...
    *(intptrj_t*)cursor = (intptrj_t)getDataSymbolReference()->getOwningMethod(comp)->constantPool();
    ...
    if (getDataReferenceInstruction()) {
        ...
        cg()->addProjectSpecializedRelocation(cursor,
            ...
            node ? (uint8_t*)node->getInlinedSiteIndex() : (uint8_t*)-1, TR_ConstantPool, __FILE__, __LINE__, node);
    }
...
uint8_t* J9::X86::UnresolvedDataSnippet::emitConstantPoolIndex(uint8_t* cursor)
{
    ...
    if (hasLiveXMMRegisters() && TR::Compiler->target.is32Bit())
    ...
    if (!comp->getOption(TR_DisableNewX86VolatileSupport) && getDataReferenceInstruction() != NULL) {
        ...
        if (TR::Compiler->target.isSMP()) {
            if (!getDataSymbol()->isClassObject() && !getDataSymbol()->isConstObjectRef() && isUnresolvedStore()
                && opCode != CMPXCHG8BMem && getDataSymbol()->isVolatile()) {
    ...
    if (!getDataSymbol()->isShadow() && !getDataSymbol()->isClassObject() && !getDataSymbol()->isConstObjectRef()) {
        ...
        if (TR::Compiler->target.is32Bit() && getDataSymbol()->isConstObjectRef()) {
            ...
            cg()->addProjectSpecializedRelocation(stringConstantPtr,
                (uint8_t*)getDataSymbolReference()->getOwningMethod(TR::comp())->constantPool(),
                getDataReferenceInstruction()->getNode()
                    ? (uint8_t*)getDataReferenceInstruction()->getNode()->getInlinedSiteIndex()
                    : (uint8_t*)-1,
...
void TR_Debug::print(TR::FILE* pOutFile, TR::UnresolvedDataSnippet* snippet)
{
    ...
    if (TR::Compiler->target.is64Bit()) {
        ...
        trfprintf(pOutFile, "%s\t" POINTER_PRINTF_FORMAT "\t%s address of constant pool for this method", dqString(),
            getOwningMethod(snippet->getDataSymbolReference())->constantPool(), commentString());
        bufferPos += 8;

        printPrefix(pOutFile, NULL, bufferPos, 4);
        trfprintf(pOutFile, "%s\t0x%08x\t\t\t\t%s constant pool index", ddString(),
        ...
        if (snippet->getDataSymbol()->isShadow()) {
            ...
            trfprintf(pOutFile, "%s\t%02x\t\t\t\t\t\t\t%s instruction descriptor: length=%d, disp32 offset=%d",
                dbString(), descriptor, commentString(), length, offset);
        ...
        if (!snippet->getDataReferenceInstruction()) {
            ...
            printPrefix(pOutFile, NULL, bufferPos, 1);
            trfprintf(pOutFile, "int \t3\t\t\t%s (No data reference instruction; NEVER CALLED)", commentString());
            return;
        }
        ...
        trfprintf(pOutFile, "push\t" POINTER_PRINTF_FORMAT "\t\t%s constant pool index",
            snippet->getDataSymbolReference()->getCPIndex(), commentString());
        bufferPos += size;

        printPrefix(pOutFile, NULL, bufferPos, 5);
        trfprintf(pOutFile, "push\t" POINTER_PRINTF_FORMAT "\t\t%s address of constant pool for this method",
            getOwningMethod(snippet->getDataSymbolReference())->constantPool(), commentString());
        bufferPos += 5;
    ...
    if (!snippet->getDataSymbol()->isShadow() && !snippet->getDataSymbol()->isClassObject()
        ...
        if (length < 8) {
            ...
            trfprintf(pOutFile, "%s\t(%d)\t\t\t%s patch instruction bytes + RET + residue", dbString(), bytesToCopy,
                commentString());
            printPrefix(pOutFile, NULL, bufferPos, 1);
            trfprintf(pOutFile, "%s\t\t\t\t\t\t%s byte that RET overwrote", dbString(), commentString());
            bufferPos++;
        } else {
            printPrefix(pOutFile, NULL, bufferPos, bytesToCopy + 1);
            bufferPos += (bytesToCopy + 1);
            trfprintf(pOutFile, "%s\t(%d)\t\t\t\t%s patch instruction bytes + RET", dbString(), (bytesToCopy + 1),
                commentString());
        }
    } else {
        int32_t bytesToCopy;
        ...
        if (TR::Compiler->target.is64Bit()) {
            if (snippet->getDataSymbol()->isShadow()) {
                ...
                printPrefix(pOutFile, NULL, bufferPos, bytesToCopy);
                bufferPos += bytesToCopy;
                trfprintf(pOutFile, "%s\t(%d)\t\t\t\t\t\t%s patch instruction bytes", dbString(), bytesToCopy,
                    commentString());
            } else {
                bytesToCopy = 2;
                printPrefix(pOutFile, NULL, bufferPos, bytesToCopy);
                bufferPos += bytesToCopy;
                trfprintf(pOutFile, "%s\t\t\t\t\t\t\t\t%s REX + op of MOV8RegImm64", dwString(), commentString());
            }
        } else {
            if (snippet->getDataSymbol()->isConstString()) {
                ...
                printPrefix(pOutFile, NULL, bufferPos, bytesToCopy);
                bufferPos += bytesToCopy;
                trfprintf(pOutFile, "%s\t(%d)\t\t\t\t\t\t%s patched string instruction bytes", dbString(), bytesToCopy,
                    commentString());
            } else {
                bytesToCopy = 8;
                printPrefix(pOutFile, NULL, bufferPos, bytesToCopy);
