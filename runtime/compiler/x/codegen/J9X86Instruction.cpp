...
TR::X86MemImmSnippetInstruction::X86MemImmSnippetInstruction(TR_X86OpCodes op, TR::Node* node, TR::MemoryReference* mr,
...
TR::X86MemImmSnippetInstruction::X86MemImmSnippetInstruction(TR::Instruction* precedingInstruction, TR_X86OpCodes op,
...
uint8_t* TR::X86MemImmSnippetInstruction::generateBinaryEncoding()
{
    ...
    if (cursor) {
        if (getOpCode().hasIntImmediate()) {
            if (std::find(
                    cg()->comp()->getStaticHCRPICSites()->begin(), cg()->comp()->getStaticHCRPICSites()->end(), this)
...
TR::X86MemImmSnippetInstruction* generateMemImmSnippetInstruction(TR_X86OpCodes op, TR::Node* node,
