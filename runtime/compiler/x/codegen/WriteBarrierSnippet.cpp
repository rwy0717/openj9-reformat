...
uint8_t* TR::IA32WriteBarrierSnippet::buildArgs(uint8_t* buffer, bool restoreRegs)
{
...
void TR_Debug::print(TR::FILE* pOutFile, TR::IA32WriteBarrierSnippet* snippet)
{
    ...
    if (needNewSpaceReg) {
        ...
        if (isRealTimeGCBarrier)
            trfprintf(pOutFile, "\t\t%s Object to be Stored Register", commentString());
        else
            trfprintf(pOutFile, "\t\t%s Source (new space) Register", commentString());
        bufferPos += 1;
    }
    ...
    if (needDestinationAddressReg) {
        ...
        trfprintf(pOutFile, "\t\t%s Destination Address Register", commentString());
        bufferPos += 1;
    }
    ...
    if (needOldSpaceReg) {
        ...
        if (isRealTimeGCBarrier)
            trfprintf(pOutFile, "\t\t%s Destination Object Register", commentString());
        else
            trfprintf(pOutFile, "\t\t%s Destination Owning Object (Old Space) Register", commentString());
        bufferPos += 1;
    }
...
static uint8_t* xchgRAX(uint8_t* buffer, TR::RealRegister* reg)
{
    buffer = TR_X86OpCode(XCHG8AccReg)
                 .binary(buffer,
                     TR::RealRegister::REX | TR::RealRegister::REX_W | reg->rexBits(TR::RealRegister::REX_B, false));
    reg->setRegisterFieldInOpcode(buffer - 1);

    return buffer;
}

static uint8_t* xchg(uint8_t* buffer, TR::RealRegister* reg1, TR::RealRegister* reg2)
{
    buffer = TR_X86OpCode(XCHG8RegReg)
                 .binary(buffer,
                     TR::RealRegister::REX | TR::RealRegister::REX_W | reg1->rexBits(TR::RealRegister::REX_R, false)
...
uint8_t* TR::AMD64WriteBarrierSnippet::buildArgs(uint8_t* buffer, bool restoreRegs)
{
    ...
    action = (restoreRegs ? 32 : 0) | (r1 == TR::RealRegister::eax ? 16 : 0) | (r1 == TR::RealRegister::esi ? 8 : 0)
        | (r2 == TR::RealRegister::esi ? 4 : 0) | (r2 == TR::RealRegister::eax ? 2 : 0) | (r1 == r2 ? 1 : 0);
...
TR::X86WriteBarrierSnippet* TR::generateX86WriteBarrierSnippet(TR::CodeGenerator* cg, TR::Node* node,
    ...
    int32_t helperArgCount, TR_WriteBarrierKind gcMode, TR::RegisterDependencyConditions* conditions)
{
    if (TR::Compiler->target.is64Bit()) {
        return new (cg->trHeapMemory()) TR::AMD64WriteBarrierSnippet(
            cg, node, restartLabel, snippetLabel, helperSymRef, helperArgCount, conditions);
    } else {
        return new (cg->trHeapMemory()) TR::IA32WriteBarrierSnippet(
            cg, node, restartLabel, snippetLabel, helperSymRef, helperArgCount, gcMode, conditions);
    }
}
