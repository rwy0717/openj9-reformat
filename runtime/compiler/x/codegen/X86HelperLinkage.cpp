...
#endif
const size_t TR::X86HelperCallSite::NumberOfIntParamRegisters
    = sizeof(IntParamRegisters) / sizeof(IntParamRegisters[0]);
const size_t TR::X86HelperCallSite::StackIndexAdjustment
...
static uint32_t X86HelperCallSiteCalculatePreservedRegisterMapForGC()
{
    uint32_t ret = 0;
    for (size_t i = 0; i
         < sizeof(TR::X86HelperCallSite::CalleeSavedRegisters) / sizeof(TR::X86HelperCallSite::CalleeSavedRegisters[0]);
         i++) {
    ...
    for (size_t i = 0; i < _Params.size(); i++) {
        ...
        if (index < NumberOfIntParamRegisters) {
            generateRegRegInstruction(
                MOVRegReg(), _Node, RealRegisters.Use(IntParamRegisters[index]), _Params[i], cg());
        } else {
            ...
            if (CalleeCleanup) {
                ...
                generateMemRegInstruction(
                    SMemReg(), _Node, generateX86MemoryReference(ESP, offset, cg()), _Params[i], cg());
            }
        }
    }

    // Call helper
    TR::X86ImmInstruction* instr = generateImmSymInstruction(CALLImm4, _Node, (uintptrj_t)_SymRef->getMethodAddress(),
