...
TR::X86PrivateLinkage::X86PrivateLinkage(TR::CodeGenerator* cg)
    : TR::Linkage(cg)
{
    ...
    _properties.setOutgoingArgAlignment(
        lcm(TR::Compiler->target.is32Bit() ? 4 : 16, cg->fej9()->getLocalObjectAlignmentInBytes()));
}

const TR::X86LinkageProperties& TR::X86PrivateLinkage::getProperties() { return _properties; }
        ...
        if (lri != NOT_LINKAGE) // This param should be in a linkage reg
        {
            ...
            if (isStore) {
                ...
                cursor = generateMemRegInstruction(
                    cursor, TR::Linkage::movOpcodes(MemReg, movDataType), memRef, reg, cg());
            } else {
                // lri := stack
                cursor = generateRegMemInstruction(
                    cursor, TR::Linkage::movOpcodes(RegMem, movDataType), reg, memRef, cg());
            }
        }
    }
    ...
    if (count <= 4) {
        ...
        for (int32_t i = 0; i < count; i++, offset += pointerSize) {
            cursor = new (cg->trHeapMemory()) TR::X86MemRegInstruction(
                cursor, SMemReg(), generateX86MemoryReference(framePointer, offset, cg), sourceReg, cg);
        }
    } else {
        ...
        cursor = new (cg->trHeapMemory()) TR::X86RegMemInstruction(
            cursor, LEARegMem(), loopReg, generateX86MemoryReference(sourceReg, count - 1, cg), cg);

        TR::LabelSymbol* loopLabel = generateLabelSymbol(cg);
        cursor = new (cg->trHeapMemory()) TR::X86LabelInstruction(cursor, LABEL, loopLabel, cg);

        cursor = new (cg->trHeapMemory()) TR::X86MemRegInstruction(cursor, SMemReg(),
            generateX86MemoryReference(
                framePointer, loopReg, TR::MemoryReference::convertMultiplierToStride(pointerSize), offset, cg),
            sourceReg, cg);
...
void TR::X86PrivateLinkage::createPrologue(TR::Instruction* cursor)
{
    ...
    //
    {
        ...
        if (trace)
            traceMsg(comp(), "Stack size was %d, and is adjusted by +%d (alignment %d, return address width %d)\n",
    ...
    if (metaDataReg) {
        ...
        if (doAllocateFrameSpeculatively) {
            ...
            const TR_X86OpCodes subOp
                = (allocSize <= 127 && getMinimumFirstInstructionSize() <= 3) ? SUBRegImms() : SUBRegImm4();
        ...
        if (doOverflowCheck) {
            ...
            cursor = generateStackOverflowCheckInstruction(vfp, CMPRegMem(), espReal,
            ...
            generateRegImmInstruction(
                MOV4RegImm4, cursor->getNode(), machine()->getRealRegister(TR::RealRegister::edi), allocSize, cg());
            if (doAllocateFrameSpeculatively) {
            ...
            jitOverflowCheck = generateImmSymInstruction(
                CALLImm4, cursor->getNode(), (uintptrj_t)helper->getMethodAddress(), helper, cg());
    ...
    if (allocSize == 0) {
        ...
        TR_ASSERT(
            minInstructionSize <= 5, "Can't guarantee SUB instruction will be at least %d bytes", minInstructionSize);
        const TR_X86OpCodes subOp
            = (allocSize <= 127 && getMinimumFirstInstructionSize() <= 3) ? SUBRegImms() : SUBRegImm4();
        cursor = new (trHeapMemory()) TR::X86RegImmInstruction(cursor, subOp, espReal, allocSize, cg());
    }
    ...
    if ((comp()->getOption(TR_PaintAllocatedFrameSlotsDead) || comp()->getOption(TR_PaintAllocatedFrameSlotsFauxObject))
        ...
        //
        cursor
            = new (trHeapMemory()) TR::X86RegImmInstruction(cursor, MOVRegImm4(), frameSlotIndexReg, paintSize, cg());
        cursor = new (trHeapMemory()) TR::X86LabelInstruction(cursor, LABEL, startLabel, cg());
        if (TR::Compiler->target.is64Bit())
            cursor = new (trHeapMemory()) TR::X86MemRegInstruction(cursor, S8MemReg,
                generateX86MemoryReference(espReal, frameSlotIndexReg, 0, (uint8_t)paintSlotsOffset, cg()), paintReg,
                cg());
        else
            cursor = new (trHeapMemory()) TR::X86MemImmInstruction(cursor, SMemImm4(),
                generateX86MemoryReference(espReal, frameSlotIndexReg, 0, (uint8_t)paintSlotsOffset, cg()),
                paintValue32, cg());
        cursor = new (trHeapMemory())
            TR::X86RegImmInstruction(cursor, SUBRegImms(), frameSlotIndexReg, sizeof(intptr_t), cg());
        cursor
            = new (trHeapMemory()) TR::X86RegImmInstruction(cursor, CMPRegImm4(), frameSlotIndexReg, paintBound, cg());
    ...
    if (atlas) {
        TR_ASSERT(
            _properties.getNumScratchRegisters() >= 2, "Need two scratch registers to initialize reference locals");
...
#if defined(DEBUG)
    debugFrameSlotInfo = new (trHeapMemory()) TR_DebugFrameSegmentInfo(
        comp(), -localSize - preservedRegsSize - outgoingArgSize, outgoingArgSize, "Outgoing args", debugFrameSlotInfo);
#endif
    ...
    cursor = cg()->generateDebugCounter(
        cursor, "cg.prologues:#frameBytes", cg()->getFrameSizeInBytes(), TR::DebugCounter::Expensive);
...
void TR::X86PrivateLinkage::createEpilogue(TR::Instruction* cursor)
{
    ...
    if (_properties.getAlwaysDedicateFramePointerRegister()) {
        ...
        cursor = generateRegInstruction(
            cursor, POPReg, machine()->getRealRegister(_properties.getFramePointerRegister()), cg());
    } else {
        auto frameSize = cg()->getFrameSizeInBytes();
        if (frameSize != 0) {
            cursor = generateRegImmInstruction(
                cursor, (frameSize <= 127) ? ADDRegImms() : ADDRegImm4(), espReal, frameSize, cg());
        }
    }
...
TR::Register* TR::X86PrivateLinkage::buildDirectDispatch(TR::Node* callNode, bool spillFPRegs)
{
    ...
    if (callNode->getType().isFloatingPoint()) {
        ...
        if (callNode->getReferenceCount() == 1 && returnRegister->getKind() == TR_X87) {
            ...
            //
            coerceFPReturnValueToXMMR(
                callNode, site.getPostConditionsUnderConstruction(), site.getMethodSymbol(), returnRegister);
        }
    }
    ...
    if (getMethodSymbol()->isVirtual() && !callNode->getSymbolReference()->isUnresolved()
        ...
        if (!comp()->getOption(TR_DisableInterpreterProfiling) && TR_ValueProfileInfoManager::get(comp())) {
            ...
            if (topValue) {
                ...
                // We can't do this guarded devirtualization if the profile data is not correct for this context. See
                // defect 98813
                else {
                    // printf("Checking is instanceof for top %p for %s\n", topValue,
                    // methodSymRef->getSymbol()->getResolvedMethodSymbol()->getResolvedMethod()->signature(comp()->trMemory()));
                    // fflush(stdout);
            ...
            if (topValue) {
                ...
                TR_ResolvedMethod* profiledVirtualMethod
                    = callNode->getSymbolReference()->getOwningMethod(comp())->getResolvedVirtualMethod(
                ...
                if (profiledVirtualMethod
                    ...
                    // profiledVirtualMethod->signature(comp()->trMemory(), stackAlloc));
                    ...
                    _profiledTargets->add(new (comp()->trStackMemory()) TR::X86PICSlot(
                        (uintptrj_t)topValue, profiledVirtualMethod, true, methodToBeCompared, slot));
                }
            }
        }
        ...
        if (addressInfo && addressInfo->getTopValue(topValue) > 0 && topValue
            ...
            for (TR_ExtraAddressInfo* profiledInfo = sortedValuesIt.getFirst(); profiledInfo != NULL;
                ...
                if (!comp()->getPersistentInfo()->isObsoleteClass((void*)thisType, fej9)) {
                    profiledInterfaceMethod = methodSymRef->getOwningMethod(comp())->getResolvedInterfaceMethod(
                        comp(), thisType, methodSymRef->getCPIndex());
                }
                if (profiledInterfaceMethod
                    ...
                    if (frequency < getMinProfiledCallFrequency()) {
                        ...
                        _profiledTargets->add(new (comp()->trStackMemory())
                                TR::X86PICSlot((uintptrj_t)thisType, profiledInterfaceMethod));
                        if (comp()->getOption(TR_TraceCG))
        ...
        if (_useLastITableCache && TR::Compiler->target.is64Bit() && _interfaceClassOfMethod
            && comp()->getPersistentInfo()->getJitState() != STARTUP_STATE) {
            int32_t numPICSlots
                = numStaticPICSlots + toX86PrivateLinkage(getLinkage())->IPicParameters.defaultNumberOfSlots;
            ...
            && TR::Compiler->target.is32Bit()) // Use the original heuristic for ia32 due to defect 111651
        {
...
bool TR::X86CallSite::resolvedVirtualShouldUseVFTCall()
{
    ...
    return !fej9->forceUnresolvedDispatch()
...
void TR::X86CallSite::stopAddingConditions()
{
    ...
    if (COPY_PRECONDITIONS_TO_POSTCONDITIONS) {
        ...
        for (uint8_t i = 0; i < getPreConditionsUnderConstruction()->getAddCursorForPre(); i++) {
            ...
            getPostConditionsUnderConstruction()->unionPreCondition(
                pre->getRegister(), pre->getRealRegister(), cg(), pre->getFlags());
            TR::RegisterDependency* post = postconditions->findDependency(
                pre->getRealRegister(), getPostConditionsUnderConstruction()->getAddCursorForPost());
            if (!post)
                getPostConditionsUnderConstruction()->addPostCondition(
                    pre->getRegister(), pre->getRealRegister(), cg(), pre->getFlags());
        }
    }
        ...
        if (profiledTargets) {
            ...
            while (picSlot) {
                ...
                if (fej9->isUnloadAssumptionRequired(
                        (TR_OpaqueClassBlock*)picSlot->getClassAddress(), comp()->getCurrentMethod())
    ...
    if (cg()->supportVMInternalNatives() && methodSymbol->isVMInternalNative()) {
        ...
        if (TR::Compiler->target.is64Bit()) {
            generateRegImm64Instruction(
                MOV8RegImm64, callNode, ramMethodReg, (uint64_t)(uintptrj_t)methodSymbol->getMethodAddress(), cg());
        } else {
            generateRegImmInstruction(
                MOV4RegImm4, callNode, ramMethodReg, (uint32_t)(uintptrj_t)methodSymbol->getMethodAddress(), cg());
        }
        ...
        generateRegImm64Instruction(
            MOV8RegImm64, callNode, nativeMethodReg, (uint64_t)(uintptrj_t)methodSymbol->getMethodAddress(), cg());
        ...
        callInstr = generateImmSymInstruction(
            CALLImm4, callNode, 0, new (trHeapMemory()) TR::SymbolReference(comp()->getSymRefTab(), label), cg());
        ...
        callInstr = generateImmSymInstruction(
            CALLImm4, callNode, (uintptrj_t)methodSymbol->getMethodAddress(), methodSymRef, cg());
...
void TR::X86PrivateLinkage::buildInterfaceCall(
...
void TR::X86PrivateLinkage::buildRevirtualizedCall(
    TR::X86CallSite& site, TR::LabelSymbol* revirtualizeLabel, TR::LabelSymbol* doneLabel)
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(fe());
...
bool TR::X86PrivateLinkage::buildVirtualGuard(TR::X86CallSite& site, TR::LabelSymbol* revirtualizeLabel)
{
    ...
    if (comp()->performVirtualGuardNOPing()) {
        ...
        && !comp()->getOption(TR_MimicInterpreterFrameShape)) // Explicitly-guarded devirtualization is pretty pointless
                                                              // without inlining
    {
...
TR::Instruction* TR::X86PrivateLinkage::buildVFTCall(TR::X86CallSite& site, TR_X86OpCode dispatchOp,
    TR::Register* targetAddressReg, TR::MemoryReference* targetAddressMemref)
{
    ...
    if (dispatchOp.sourceIsMemRef()) {
        ...
        if (mayReachJ2IThunk && dispatchOp.isCallOp()) {
            ...
            if (targetAddressReg && targetAddressReg->getRegisterPair()) {
                dependencies->unionPreCondition(
                    targetAddressReg->getRegisterPair()->getHighOrder(), TR::RealRegister::NoReg, cg());
                dependencies->unionPreCondition(
                    targetAddressReg->getRegisterPair()->getLowOrder(), TR::RealRegister::NoReg, cg());
            } else
    ...
    if (site.getSymbolReference()->isUnresolved() && !site.getMethodSymbol()->isInterface()) {
        ...
        TR::UnresolvedDataSnippet* snippet = new (comp()->trHeapMemory())
            TR::X86UnresolvedVirtualCallSnippet(callNode, site.getSymbolReference(), callInstr, cg());
...
TR::Register* TR::X86PrivateLinkage::buildCallPostconditions(TR::X86CallSite& site)
{
    ...
    dependencies->addPostCondition(
        cg()->getMethodMetaDataRegister(), getProperties().getMethodMetaDataRegister(), cg());
...
void TR::X86PrivateLinkage::buildInterfaceDispatchUsingLastITable(TR::X86CallSite& site, int32_t numIPicSlots,
    TR::X86PICSlot& lastPicSlot, TR::Instruction*& slotPatchInstruction, TR::LabelSymbol* doneLabel,
    TR::LabelSymbol* lookupDispatchSnippetLabel, TR_OpaqueClassBlock* declaringClass, uintptrj_t itableIndex)
{
    ...
    //          call lastITableDispatch                            ; if so, call the dispatch sequence with return
    //          address pointing here
    //    done:
    //          ...
    ...
    generateRegMemInstruction(SUBRegMem(), callNode, vtableIndexReg,
    ...
    if (breakBeforeInterfaceDispatchUsingLastITable)
        generateInstruction(BADIA32Op, callNode, cg());
    generateRegMemInstruction(LRegMem(), callNode, scratchReg,
    ...
    if (use32BitInterfacePointers) {
        ...
        generateMemRegInstruction(CMPMemReg(), callNode,
            generateX86MemoryReference(scratchReg, fej9->getOffsetOfInterfaceClassFromITableField(), cg()),
            interfaceClassReg, cg());
    }

    generateLongLabelInstruction(
        JNE4, callNode, lookupDispatchSnippetLabel, cg()); // PICBuilder needs this to have a 4-byte offset
    if (TR::Compiler->target.is32Bit())
