...
enum { BranchJNE = 0, BranchJE = 1, BranchNopJMP = 2 };
...
public:
    ...
    X86PICSlot(uintptrj_t classAddress, TR_ResolvedMethod* method, bool jumpToDone = true,
...
public:
    ...
    float getMinProfiledCallFrequency()
    {
        return .075F;
    } // Tuned for megamorphic site in jess; so bear in mind before changing
...
public:
    ...
    //
    virtual void buildDirectCall(TR::SymbolReference* methodSymRef,
        TR::X86CallSite& site); // NOTE: the methodSymRef being called is not necessarily site.getSymbolReference()
    virtual void buildVirtualOrComputedCall(
        ...
        = 0;
    virtual void buildInterfaceCall(
        TR::X86CallSite& site, TR::LabelSymbol* entryLabel, TR::LabelSymbol* doneLabel, uint8_t* thunk);
    ...
    virtual bool buildVirtualGuard(
        TR::X86CallSite& site, TR::LabelSymbol* revirtualizeLabel); // returns false if it can't build the guard
    virtual void buildRevirtualizedCall(
    ...
    virtual void buildIPIC(
        TR::X86CallSite& site, TR::LabelSymbol* entryLabel, TR::LabelSymbol* doneLabel, uint8_t* thunk)
        = 0;
    virtual TR::Instruction* buildVFTCall(TR::X86CallSite& site, TR_X86OpCode dispatchOp,
