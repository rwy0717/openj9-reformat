...
TR::Instruction* TR_X86Recompilation::generatePrePrologue()
{
    ...
    if (TR::Compiler->target.is64Bit()) {
        ...
        prev
            = new (trHeapMemory()) TR::X86ImmInstruction(prev, DDImm4, (uint32_t)(uintptrj_t)getJittedBodyInfo(), cg());
        prev->setNeedsAOTRelocation();
    }
...
TR::Instruction* TR_X86Recompilation::generatePrologue(TR::Instruction* cursor)
{
    ...
    if (couldBeCompiledAgain()) {
        if (!useSampling()) {
            ...
            if (TR::Compiler->target.is64Bit()) {
                TR_ASSERT(
                    linkage->getMinimumFirstInstructionSize() <= 10, "Can't satisfy first instruction size constraint");
                TR::RealRegister* scratchReg = machine->getRealRegister(TR::RealRegister::edi);
                cursor = new (trHeapMemory()) TR::AMD64RegImm64Instruction(
                    cursor, MOV8RegImm64, scratchReg, (uintptrj_t)getCounterAddress(), cg());
                mRef = generateX86MemoryReference(scratchReg, 0, cg());
            } else {
                TR_ASSERT(
                    linkage->getMinimumFirstInstructionSize() <= 5, "Can't satisfy first instruction size constraint");
