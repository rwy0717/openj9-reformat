...
TR::Register* TR::IA32J9SystemLinkage::buildVolatileAndReturnDependencies(
...
TR::Register* TR::IA32J9SystemLinkage::buildDirectDispatch(TR::Node* callNode, bool spillFPRegs)
{
    ...
    if (callNode->getOpCode().isFloat()) {
        ...
        generateRegMemInstruction(
            cg()->getXMMDoubleLoadOpCode(), callNode, returnReg, generateX86MemoryReference(*tempMR, 0, cg()), cg());
    }
...
int32_t TR::IA32J9SystemLinkage::buildParametersOnCStack(
    TR::Node* callNode, int firstParamIndex, bool passVMThread, bool wrapAddress)
{
    ...
    generateRegMemInstruction(L4RegMem, callNode, cSP,
            ...
            if (wrapAddress && child->getOpCodeValue() == TR::loadaddr) {
                ...
                } else // must be loadaddr of parm or local
                {
                    ...
                    generateMemImmInstruction(
                        CMP4MemImms, child, generateX86MemoryReference(child->getRegister(), 0, cg()), 0, cg());
