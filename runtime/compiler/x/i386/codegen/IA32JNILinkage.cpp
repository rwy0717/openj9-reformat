...
TR::Register* TR::IA32JNILinkage::buildJNIDispatch(TR::Node* callNode)
{
    ...
    if (createJNIFrame) {
        ...
        //
        generateMemImmInstruction(S4MemImm4, callNode,
        ...
        generateImmInstruction(
            PUSHImm4, callNode, (uintptrj_t)resolvedMethod->resolvedMethodAddress(), cg(), reloTypes[rType]);
    ...
    // Store out jsp.
    //
    generateMemRegInstruction(S4MemReg, callNode,
        generateX86MemoryReference(ebpReal, fej9->thisThreadGetJavaSPOffset(), cg()), espReal, cg());

    // Switch stacks.
    // Load up machine SP.
    //
    generateRegMemInstruction(L4RegMem, callNode, espReal,
    ...
    generateRegImmInstruction(
        argSize >= -128 && argSize <= 127 ? SUB4RegImms : SUB4RegImm4, callNode, espReal, argSize, cg());
    ...
    //
    TR::Instruction* instr = generateImmSymInstruction(CALLImm4, callNode,
    ...
    case TR::Int8:
        generateRegRegInstruction(isUnsigned ? MOVZXReg4Reg1 : MOVSXReg4Reg1, callNode, ecxReal, eaxReal, cg());
        break;
    case TR::Int16:
        generateRegRegInstruction(isUnsigned ? MOVZXReg4Reg2 : MOVSXReg4Reg2, callNode, ecxReal, eaxReal, cg());
        break;
    case TR::Address:
    case TR::Int64:
    case TR::Int32:
    ...
    if (createJNIFrame) {
        generateRegMemInstruction(ADD4RegMem, callNode, espReal,
    ...
    case TR::Float:
        deps->addPostCondition(
            returnRegister = cg()->allocateSinglePrecisionRegister(TR_X87), TR::RealRegister::st0, cg());
        break;
    case TR::Double:
    ...
    if (checkExceptions) {
        // Check exceptions.
        //
        generateMemImmInstruction(CMP4MemImms, callNode,
        ...
        TR::Snippet* snippet = new (trHeapMemory()) TR::X86CheckFailureSnippet(cg(),
            ...
            instr, callNode->getDataType() == TR::Float || callNode->getDataType() == TR::Double);
    ...
    if (callNode->getOpCode().isFloat()) {
        ...
        generateRegMemInstruction(
            MOVSSRegMem, callNode, returnRegister, generateX86MemoryReference(*tempMR, 0, cg()), cg());
