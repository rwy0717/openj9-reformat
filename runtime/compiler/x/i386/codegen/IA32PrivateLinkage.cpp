...
TR::Instruction* TR::IA32PrivateLinkage::savePreservedRegisters(TR::Instruction* cursor)
{
    ...
    for (int32_t pindex = numPreserved - 1; pindex >= 0; pindex--) {
        ...
        if (reg->getHasBeenAssignedInMethod() && reg->getState() != TR::RealRegister::Locked) {
            cursor = generateMemRegInstruction(cursor, S4MemReg,
...
TR::Instruction* TR::IA32PrivateLinkage::restorePreservedRegisters(TR::Instruction* cursor)
{
    ...
    for (int32_t pindex = numPreserved - 1; pindex >= 0; pindex--) {
        ...
        if (reg->getHasBeenAssignedInMethod()) {
            cursor = generateRegMemInstruction(cursor, L4RegMem, reg,
...
int32_t TR::IA32PrivateLinkage::buildArgs(TR::Node* callNode, TR::RegisterDependencyConditions* dependencies)
{
    int32_t argSize = 0;
    ...
    TR::Node* child, TR::SymbolReference* symRef, int32_t cpIndex)
{
    ...
    generateBoundaryAvoidanceInstruction(
        TR::X86BoundaryAvoidanceInstruction::unresolvedAtomicRegions, 8, 8, dataReferenceInstruction, cg());
...
static TR_AtomicRegion X86PicCallAtomicRegion[] = { { 1, 4 }, // disp32 of a CALL instruction
    { 0, 0 } };
    ...
    TR::X86PICSlot picSlot, TR::LabelSymbol* mismatchLabel, TR::LabelSymbol* doneLabel, TR::X86CallSite& site)
{
    ...
    if (picSlot.getMethod()) {
        instr = generateImmInstruction(
            CALLImm4, node, (uint32_t)(uintptrj_t)picSlot.getMethod()->startAddressForJittedMethod(), cg());
