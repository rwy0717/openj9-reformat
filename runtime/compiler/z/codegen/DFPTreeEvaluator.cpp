 ...
 */
void genRestoreFPCMaskBits(TR::Node* node, TR::CodeGenerator* cg, TR::Register* restRegister)
{
    // Extract FPC
...
static TR::Register* genClearFPCBits(TR::Node* node, TR::CodeGenerator* cg, uint8_t mask)
{
    // Extract FPC
    TR::Register* fpcRegister = cg->allocateRegister();
    generateRRInstruction(cg, TR::InstOpCode::EFPC, node, fpcRegister, fpcRegister);
...
static TR::MemoryReference* genSetDFPRoundingMode(TR::Node* node, TR::CodeGenerator* cg, int32_t rm)
{
...
static TR::MemoryReference* genSetDFPRoundingMode(TR::Node* node, TR::CodeGenerator* cg, TR::Register* rmRegister)
{
...
static TR::MemoryReference* genTestDataGroup(
    TR::Node* node, TR::CodeGenerator* cg, TR::Register* fprRegister, int32_t grp)
{
...
static TR::Register* genLoadDFP(TR::Node* node, TR::CodeGenerator* cg, TR::Node* fieldNode)
{
    ...
    if (fieldNode->getOpCodeValue() == TR::lloadi && !fieldNode->getRegister() && fieldNode->getReferenceCount() == 1) {
        ...
        if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
            ...
            generateRSInstruction(
                cg, TR::InstOpCode::SLLG, node, newRegister->getHighOrder(), newRegister->getHighOrder(), 32);
            generateRRInstruction(
                cg, TR::InstOpCode::LR, node, newRegister->getHighOrder(), newRegister->getLowOrder());
...
static void genStoreDFP(TR::Node* node, TR::CodeGenerator* cg, TR::Node* objAddrNode,
    TR::SymbolReference* dfpFieldSymbolReference, TR::Register* fprDFPRegister)
{
    ...
    if (dfpFieldOffset == -1) {
        ...
        bigDClass = fej9->getClassFromSignature(
            (char*)className, len, node->getSymbolReference()->getOwningMethod(comp), true);

        int16_t computedDfpFieldOffset
            = fej9->getInstanceFieldOffset(bigDClass, (char*)fieldName, fieldLen, (char*)sig, sigLen);
        ...
        dfpFieldOffset
            = computedDfpFieldOffset + fej9->getObjectHeaderSizeInBytes(); // size of a J9 object header to move past it
    }
...
extern TR::Register* inlineBigDecimalConstructor(TR::Node* node, TR::CodeGenerator* cg, bool isLong, bool exp)
{
    ...
    if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
        ...
        if (inBCDForm) {
            ...
            TR::S390RestoreGPR7Snippet* restoreSnippet = new (cg->trHeapMemory())
                TR::S390RestoreGPR7Snippet(cg, node, generateLabelSymbol(cg), jump2callSymbol);
            cg->addSnippet(restoreSnippet);
            TR::Instruction* cursor = generateS390BranchInstruction(
                cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_NOP, node, restoreSnippet->getSnippetLabel());

            cursor = generateS390BranchInstruction(
                cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_NOP, node, jump2callSymbol);

            // setting up OOL path
            TR_S390OutOfLineCodeSection* outlinedHelperCall
                = new (cg->trHeapMemory()) TR_S390OutOfLineCodeSection(jump2callSymbol, OOLlabelEND, cg);
            ...
            if (debugObj)
                debugObj->addInstructionComment(
                    cursor, "Denotes start of DFP inlineBigDecimalConstructor OOL BCDCHK sequence");

            generateLoad32BitConstant(cg, node, 0, retRegister, true);

            if (debugObj)
                debugObj->addInstructionComment(
                    cursor, "Denotes end of DFP inlineBigDecimalBinaryOp OOL BCDCHK sequence");

            cursor
                = generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, node, OOLlabelEND);
            outlinedHelperCall->swapInstructionListsWithCompilation();
        } else
            generateRRInstruction(cg, TR::InstOpCode::CDGTR, node, dfpRegister,
                valRegister); // CDGTR does not throw decimal operand exception
        ...
        if (!isLong) {
            ...
            if (inBCDForm) {
                TR::Instruction* instr
                    = generateRRInstruction(cg, TR::InstOpCode::CDUTR, node, dfpRegister, valRegister);
                bdInstr = instr;
                ...
                cursor = generateS390BranchInstruction(
                    cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, node, OOLlabelEND);
                outlinedHelperCall->swapInstructionListsWithCompilation();
            } else
            ...
            generateRSInstruction(
                cg, TR::InstOpCode::SLLG, node, valRegister->getHighOrder(), valRegister->getHighOrder(), 32);
            generateRRInstruction(
                cg, TR::InstOpCode::LR, node, valRegister->getHighOrder(), valRegister->getLowOrder());

            // convert to DFP
            if (inBCDForm) {
                TR::Instruction* instr
                    = generateRRInstruction(cg, TR::InstOpCode::CDUTR, node, dfpRegister, valRegister->getHighOrder());
                bdInstr = instr;
    ...
    if (toRound == 1) {
        ...
        generateRRFInstruction(
            cg, TR::InstOpCode::RRDTR, node, resRegister, precRegister, dfpRegister, USE_CURRENT_DFP_ROUNDING_MODE);
    ...
    TR::Node* node, TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, bool scaled)
{
    ...
    TR::Instruction* instr = generateRRFInstruction(
        cg, op, node, resFPRegister, rhsFPRegister, lhsFPRegister, USE_CURRENT_DFP_ROUNDING_MODE);
    ...
    if (toRound == 1 && rmRegister == NULL) {
        ...
        // perform the round
        generateRRFInstruction(
            cg, TR::InstOpCode::RRDTR, node, resFPRegister, precRegister, resFPRegister, USE_CURRENT_DFP_ROUNDING_MODE);
    }
    ...
    if (!scaled) {
        ...
        generateRRFInstruction(
            cg, TR::InstOpCode::IEDTR, node, dfpDummyFPRegister, desiredBiasedExponent, dfpDummyFPRegister);

        // perform the quantize
        generateRRFInstruction(cg, TR::InstOpCode::QADTR, node, resFPRegister, dfpDummyFPRegister, resFPRegister,
            USE_CURRENT_DFP_ROUNDING_MODE);
...
extern TR::Register* inlineBigDecimalScaledDivide(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::Instruction* instr = generateRRFInstruction(
        cg, TR::InstOpCode::DDTR, node, resFPRegister, rhsFPRegister, lhsFPRegister, USE_CURRENT_DFP_ROUNDING_MODE);
...
extern TR::Register* inlineBigDecimalRound(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::Instruction* instr = generateRRFInstruction(
        cg, TR::InstOpCode::RRDTR, node, resRegister, reqPrecRegister, dfpFPRegister, USE_CURRENT_DFP_ROUNDING_MODE);
...
extern TR::Register* inlineBigDecimalUnaryOp(TR::Node* node, TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op)
{
    ...
    */
    if (TR::Compiler->target.is32Bit() && !cg->use64BitRegsOn32Bit() && op == TR::InstOpCode::CUDTR) {
        ...
        cursor
            = generateRSInstruction(cg, TR::InstOpCode::SRLG, node, resRegisterPair->getHighOrder(), resRegister, 32);
...
extern TR::Register* inlineBigDecimalSetScale(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    // insert the biased exponent
    TR::Instruction* instr
        = generateRRFInstruction(cg, TR::InstOpCode::IEDTR, node, dfpDummyFPRegister, expRegister, dfpDummyFPRegister);
    cg->stopUsingRegister(expRegister); // since clobber eval'd
    ...
    TR::Register* resRegister = cg->allocateRegister(TR_FPR);
    generateRRFInstruction(
        cg, TR::InstOpCode::QADTR, node, resRegister, dfpDummyFPRegister, dfpFPRegister, USE_CURRENT_DFP_ROUNDING_MODE);
    cg->stopUsingRegister(dfpDummyFPRegister);
    cg->stopUsingRegister(dfpFPRegister);
...
extern TR::Register* inlineBigDecimalFromPackedConverter(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::S390RestoreGPR7Snippet* restoreSnippet
        = new (cg->trHeapMemory()) TR::S390RestoreGPR7Snippet(cg, node, generateLabelSymbol(cg), jump2callSymbol);
    cg->addSnippet(restoreSnippet);
    ...
    TR::Instruction* cursor
        = generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_NOP, node, jump2callSymbol);

    TR_S390OutOfLineCodeSection* outlinedHelperCall
        = new (cg->trHeapMemory()) TR_S390OutOfLineCodeSection(jump2callSymbol, passThroughLabel, cg);
    cg->getS390OutOfLineCodeSectionList().push_front(outlinedHelperCall);
    ...
    if (debugObj)
        debugObj->addInstructionComment(
            cursor, "Denotes end of DFP DFPConvertPackedToDFP OOL BCDCHK sequence: return to mainline");
...
extern TR::Register* inlineBigDecimalToPackedConverter(TR::Node* node, TR::CodeGenerator* cg)
{
    // see if we need to check for inexact exceptions
    TR_ASSERT(
        node->getSecondChild()->getOpCode().isLoadConst(), "inlineBigDecimalToPackedConverter: Unexpected Type\n");
...
inline TR::Register* dfp2fpHelper(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register* J9::Z::TreeEvaluator::df2fEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return dfp2fpHelper(node, cg);
}

TR::Register* J9::Z::TreeEvaluator::f2dfEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return dfp2fpHelper(node, cg);
}
 ...
 */
inline TR::Register* dfp2l(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Register* srcReg = cg->evaluate(firstChild);
 ...
 */
inline TR::Register* dfp2l64(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Register* srcReg = cg->evaluate(firstChild);
 ...
 */
inline TR::Register* dfp2lu(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Register* srcReg = cg->evaluate(firstChild);
    TR::Register* tempDDReg;
 ...
 * Convert Decimal float/Double/LongDouble to signed long int in 64 bit mode;
 */
inline TR::Register* dfp2lu64(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Register* srcReg = cg->evaluate(firstChild);
    TR::Register* tempDDReg = NULL;
 ...
 */
inline TR::Register* l2dfp(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Register* srcReg = cg->evaluate(firstChild);
 ...
 */
inline TR::Register* l2dfp64(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Register* srcReg = cg->evaluate(firstChild);
 ...
 */
inline TR::Register* lu2dfp(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Register* srcReg = cg->evaluate(firstChild);
 ...
 */
inline TR::Register* lu2dfp64(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Register* srcReg = cg->evaluate(firstChild);
 ...
 */
inline TR::Register* fixedToDFP(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
...
TR::Register* J9::Z::TreeEvaluator::i2ddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return fixedToDFP(node, cg);
}

TR::Register* J9::Z::TreeEvaluator::dd2lEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) ? dfp2l64(node, cg) : dfp2l(node, cg);
}

TR::Register* J9::Z::TreeEvaluator::dd2luEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) ? dfp2lu64(node, cg) : dfp2lu(node, cg);
}

TR::Register* J9::Z::TreeEvaluator::l2ddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) ? l2dfp64(node, cg) : l2dfp(node, cg);
}

TR::Register* J9::Z::TreeEvaluator::lu2ddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) ? lu2dfp64(node, cg) : lu2dfp(node, cg);
}

TR::Register* J9::Z::TreeEvaluator::df2ddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
...
TR::Register* J9::Z::TreeEvaluator::df2deEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
...
TR::Register* J9::Z::TreeEvaluator::dd2dfEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Register* targetReg = cg->allocateRegister(TR_FPR);
    TR::Register* srcReg = cg->evaluate(firstChild);
...
TR::Register* J9::Z::TreeEvaluator::dd2deEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Register* targetReg = cg->allocateFPRegisterPair();
    TR::Register* srcReg = cg->evaluate(firstChild);
...
TR::Register* J9::Z::TreeEvaluator::de2dxHelperAndSetRegister(
    TR::Register* targetReg, TR::Register* srcReg, TR::Node* node, TR::CodeGenerator* cg)
{
    TR_ASSERT(node->getDataType() == TR::DecimalDouble || node->getDataType() == TR::DecimalFloat,
        "expecting node %s (%p) dataType to be DecimalDouble or DecimalFloat\n", node->getOpCode().getName(), node);
    uint8_t m3 = 0x0;
    uint8_t m4 = 0x0;
    generateRRFInstruction(cg, TR::InstOpCode::LDXTR, node, targetReg, srcReg, m3, m4);
    if (node->getDataType() == TR::DecimalFloat)
        generateRRFInstruction(
            cg, TR::InstOpCode::LEDTR, node, targetReg->getHighOrder(), targetReg->getHighOrder(), m3, m4);
 ...
 */
TR::Register* J9::Z::TreeEvaluator::de2ddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Register* srcReg = cg->evaluate(firstChild);
    TR::Register* targetReg = cg->allocateFPRegisterPair();
...
TR::Register* J9::Z::TreeEvaluator::dfaddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register* J9::Z::TreeEvaluator::ddaddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register* J9::Z::TreeEvaluator::deaddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Register* sreg1 = cg->evaluate(node->getFirstChild());
    TR::Register* sreg2 = cg->evaluate(node->getSecondChild());
...
TR::Register* J9::Z::TreeEvaluator::dfsubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Register* sreg1 = cg->fprClobberEvaluate(node->getFirstChild());
    TR::Register* sreg2 = cg->fprClobberEvaluate(node->getSecondChild());
    TR::Register* treg = cg->allocateRegister(TR_FPR);
...
TR::Register* J9::Z::TreeEvaluator::ddsubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Register* sreg1 = cg->evaluate(node->getFirstChild());
    TR::Register* sreg2 = cg->evaluate(node->getSecondChild());
    TR::Register* treg = cg->allocateRegister(TR_FPR);
...
TR::Register* J9::Z::TreeEvaluator::desubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Register* sreg1 = cg->evaluate(node->getFirstChild());
    TR::Register* sreg2 = cg->evaluate(node->getSecondChild());
    TR::Register* treg = cg->allocateFPRegisterPair();
...
TR::Register* J9::Z::TreeEvaluator::dfmulEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Register* sreg1 = cg->fprClobberEvaluate(node->getFirstChild());
    TR::Register* sreg2 = cg->fprClobberEvaluate(node->getSecondChild());
    TR::Register* treg = cg->allocateRegister(TR_FPR);
...
TR::Register* J9::Z::TreeEvaluator::ddmulEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Register* sreg1 = cg->evaluate(node->getFirstChild());
    TR::Register* sreg2 = cg->evaluate(node->getSecondChild());
    TR::Register* treg = cg->allocateRegister(TR_FPR);
...
TR::Register* J9::Z::TreeEvaluator::demulEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Register* sreg1 = cg->evaluate(node->getFirstChild());
    TR::Register* sreg2 = cg->evaluate(node->getSecondChild());
    TR::Register* treg = cg->allocateFPRegisterPair();
...
TR::Register* J9::Z::TreeEvaluator::dfdivEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (checkDivisorForZero) {
        ...
        // Start OOL sequence
        TR_S390OutOfLineCodeSection* oolHelper
            = new (cg->trHeapMemory()) TR_S390OutOfLineCodeSection(oolEntryPoint, oolReturnPoint, cg);
...
TR::Register* J9::Z::TreeEvaluator::ifddcmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return generateS390CompareBranch(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BE, TR::InstOpCode::COND_BE, false);
}

TR::Register* J9::Z::TreeEvaluator::ifddcmpneEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return generateS390CompareBranch(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE, TR::InstOpCode::COND_BNE, false);
}

TR::Register* J9::Z::TreeEvaluator::ifddcmpltEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return generateS390CompareBranch(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BL, TR::InstOpCode::COND_BH, false);
}

TR::Register* J9::Z::TreeEvaluator::ifddcmpgeEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return generateS390CompareBranch(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNL, TR::InstOpCode::COND_BNH, false);
}

TR::Register* J9::Z::TreeEvaluator::ifddcmpgtEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return generateS390CompareBranch(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BH, TR::InstOpCode::COND_BL, false);
}

TR::Register* J9::Z::TreeEvaluator::ifddcmpleEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return generateS390CompareBranch(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNH, TR::InstOpCode::COND_BNL, false);
}

TR::Register* J9::Z::TreeEvaluator::ifddcmpequEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return generateS390CompareBranch(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BE, TR::InstOpCode::COND_BE, true);
}

TR::Register* J9::Z::TreeEvaluator::ifddcmpneuEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return generateS390CompareBranch(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE, TR::InstOpCode::COND_BNE, true);
}

TR::Register* J9::Z::TreeEvaluator::ifddcmpltuEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return generateS390CompareBranch(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BL, TR::InstOpCode::COND_BH, true);
}

TR::Register* J9::Z::TreeEvaluator::ifddcmpgeuEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return generateS390CompareBranch(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNL, TR::InstOpCode::COND_BNH, true);
}

TR::Register* J9::Z::TreeEvaluator::ifddcmpgtuEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return generateS390CompareBranch(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BH, TR::InstOpCode::COND_BL, true);
}

TR::Register* J9::Z::TreeEvaluator::ifddcmpleuEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return generateS390CompareBranch(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNH, TR::InstOpCode::COND_BNL, true);
}

TR::Register* J9::Z::TreeEvaluator::ddcmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return generateS390CompareBool(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BE, TR::InstOpCode::COND_BE, false);
}

TR::Register* J9::Z::TreeEvaluator::ddcmpneEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return generateS390CompareBool(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE, TR::InstOpCode::COND_BNE, false);
}

TR::Register* J9::Z::TreeEvaluator::ddcmpltEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return generateS390CompareBool(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BL, TR::InstOpCode::COND_BH, false);
}

TR::Register* J9::Z::TreeEvaluator::ddcmpgeEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return generateS390CompareBool(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNL, TR::InstOpCode::COND_BNH, false);
}

TR::Register* J9::Z::TreeEvaluator::ddcmpgtEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return generateS390CompareBool(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BH, TR::InstOpCode::COND_BL, false);
}

TR::Register* J9::Z::TreeEvaluator::ddcmpleEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return generateS390CompareBool(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNH, TR::InstOpCode::COND_BNL, false);
}

TR::Register* J9::Z::TreeEvaluator::ddcmpequEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return generateS390CompareBool(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BE, TR::InstOpCode::COND_BE, true);
}

TR::Register* J9::Z::TreeEvaluator::ddcmpneuEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return generateS390CompareBool(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE, TR::InstOpCode::COND_BNE, true);
}

TR::Register* J9::Z::TreeEvaluator::ddcmpltuEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return generateS390CompareBool(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BL, TR::InstOpCode::COND_BH, true);
}

TR::Register* J9::Z::TreeEvaluator::ddcmpgeuEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return generateS390CompareBool(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNL, TR::InstOpCode::COND_BNH, true);
}

TR::Register* J9::Z::TreeEvaluator::ddcmpgtuEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return generateS390CompareBool(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BH, TR::InstOpCode::COND_BL, true);
}

TR::Register* J9::Z::TreeEvaluator::ddcmpleuEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return generateS390CompareBool(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNH, TR::InstOpCode::COND_BNL, true);
}
 ...
 */
TR::Register* J9::Z::TreeEvaluator::ddnegEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
 ...
 */
TR::Register* J9::Z::TreeEvaluator::ddInsExpEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR_ASSERT(node->getOpCodeValue() == TR::ddInsExp || node->getOpCodeValue() == TR::deInsExp,
        "expecting op to be ddInsExp or deInsExp and not %d\n", node->getOpCodeValue());
    ...
    if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
        ...
        TR_ASSERT(biasedExpNode->getOpCode().getSize() == 4, "expecting a biasedExpNode of size 4 and not size %d\n",
            biasedExpNode->getOpCode().getSize());
        // need to ensure biasedExpReg is allocated as a 64 bit register so localRA will spill/reload all 64 bits if
        // needed
        if (biasedExpNode->getOpCode().isLoadConst()) {
            ...
            if (biasedExpValue >= MIN_IMMEDIATE_VAL && biasedExpValue <= MAX_IMMEDIATE_VAL) {
                ...
                generateRILInstruction(
                    cg, TR::InstOpCode::LGFI, node, biasedExpReg, static_cast<int32_t>(biasedExpValue));
            }
        }
        ...
        // the 64 bit registers biasedExpReg is going to be clobbered and R0 is safe to clobber regardless of the hgpr
        // (use64BitRegsOn32Bit) setting
        deps = new (cg->trHeapMemory()) TR::RegisterDependencyConditions(0, 1, cg);
        deps->addPostCondition(biasedExpReg, TR::RealRegister::GPR0);
    }

    TR::Instruction* inst = generateRRFInstruction(cg,
        (node->getDataType() == TR::DecimalLongDouble ? TR::InstOpCode::IEXTR : TR::InstOpCode::IEDTR), node, targetReg,
        biasedExpReg, srcReg);
    if (deps)
...
TR::Register* J9::Z::TreeEvaluator::dffloorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    generateRRFInstruction(
        cg, TR::InstOpCode::FIDTR, node, trgReg, trgReg, (uint8_t)9, (uint8_t)0); //  mask3=9 round to 0, mask4=0
...
TR::Register* J9::Z::TreeEvaluator::ddfloorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Register* srcReg = cg->evaluate(node->getFirstChild());
    TR::Register* trgReg = cg->allocateRegister(TR_FPR);

    generateRRFInstruction(
        cg, TR::InstOpCode::FIDTR, node, trgReg, srcReg, (uint8_t)9, (uint8_t)0); //  mask3=9 round to 0, mask4=0
...
TR::Register* J9::Z::TreeEvaluator::defloorEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Register* srcReg = cg->evaluate(node->getFirstChild());
    TR::Register* trgReg = cg->allocateFPRegisterPair();

    generateRRFInstruction(
        cg, TR::InstOpCode::FIXTR, node, trgReg, srcReg, (uint8_t)9, (uint8_t)0); //  mask3=9 round to 0, mask4=0
...
TR::Register* J9::Z::TreeEvaluator::deconstEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
#ifdef SUPPORT_DFP
...
inline TR::Register* deloadHelper(TR::Node* node, TR::CodeGenerator* cg, TR::MemoryReference* srcMR)
{
...
TR::Register* J9::Z::TreeEvaluator::deloadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return deloadHelper(node, cg, NULL);
}

inline TR::MemoryReference* destoreHelper(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* valueChild;
...
TR::Register* J9::Z::TreeEvaluator::destoreEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    destoreHelper(node, cg);
    return NULL;
}

TR::Register* J9::Z::TreeEvaluator::deRegLoadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register* J9::Z::TreeEvaluator::deRegStoreEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 */
TR::Register* J9::Z::TreeEvaluator::ddSetNegativeEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
 ...
 * Handles DFP shl ops
 */
TR::Register* J9::Z::TreeEvaluator::ddshlEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
 ...
 * Handles DFP shr ops
 */
TR::Register* J9::Z::TreeEvaluator::ddshrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* numNode = node->getChild(0);
    TR::Node* shiftNode = node->getChild(1);
 ...
 */
TR::Register* J9::Z::TreeEvaluator::ddshrRoundedEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    // This opcode is a replacement for pdshr (with round = 5); a ddshrRounded would get created by converting a pdshr
    // to ddshrRound. The assumption is thus made here that the value is represented with exponent 0. Thus, to get the
    // correct results, we modify the exponent and then use quantize to convert back to a number with exponent 0,
    // rounding in the process. eg. 1234567 (exp=0) shifted right by 2: Set the exponent to -2: 1234567 (exp=-2), or
    // 12345.67 Quantize to a value with exponent 0 using the value 1 (exp=0): the 6 causes a round up, so the result is
    // 12346 (exp=0). Using the rounding method "round to nearest with ties away from 0" gives the "round half away from
    // zero" behaviour used in Cobol and works with negative numbers (since the sign bit is separate and not even
    // considered with that rounding mode): -1234567 shifted right two will be -12346.
    TR::Node* numNode = node->getChild(0);
    TR::Node* shiftNode = node->getChild(1);
    ...
    if (node->getDataType() == TR::DecimalLongDouble) {
        ...
        targetReg = cg->allocateFPRegisterPair();
        TR::Instruction* inst
            = generateRRFInstruction(cg, TR::InstOpCode::IEXTR, node, targetReg, biasedExpReg, numReg);
 ...
 */
TR::Register* J9::Z::TreeEvaluator::ddModifyPrecisionEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* numNode = node->getChild(0);
    TR::Register* numReg = cg->evaluate(numNode);
    TR::Register* targetReg;

    // To set precision to p, shift left by maximum_precision - p (shifting out all but p digits of the original), then
    // shift right by the same amount
...
TR::Register* J9::Z::TreeEvaluator::ddcleanEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register* J9::Z::TreeEvaluator::decleanEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Register* sreg = cg->evaluate(node->getFirstChild());
...
TR::Register* J9::Z::TreeEvaluator::dd2iEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Register* srcReg = cg->evaluate(firstChild);
    TR::Register* tempDDReg = NULL;
