...
uint8_t* TR::S390ForceRecompilationSnippet::emitSnippetBody()
{
    ...
    TR::SymbolReference* glueRef
        = cg()->symRefTab()->findOrCreateRuntimeHelper(TR_S390induceRecompilation, false, false, false);
...
void TR_Debug::print(TR::FILE* pOutFile, TR::S390ForceRecompilationSnippet* snippet)
{
    ...
    printPrefix(pOutFile, NULL, bufferPos, 6);
    trfprintf(pOutFile, "BRCL \t<%p>\t\t# Branch to %s %s", snippet->getSnippetDestAddr(),
        getName(_cg->getSymRef(TR_S390induceRecompilation)), snippet->usedTrampoline() ? "- Trampoline Used." : "");
    bufferPos += 6;
}

TR::S390ForceRecompilationDataSnippet::S390ForceRecompilationDataSnippet(
    TR::CodeGenerator* cg, TR::Node* node, TR::LabelSymbol* restartLabel)
...
void TR_Debug::print(TR::FILE* pOutFile, TR::S390ForceRecompilationDataSnippet* snippet)
{
    uint8_t* bufferPos = snippet->getSnippetLabel()->getCodeLocation();
