        ...
        if (node->getOpCodeValue() == TR::BBStart) {
            ...
            if (!block->isExtensionOfPreviousBlock()) {
                if (cg->traceBCDCodeGen())
                    traceMsg(
                        comp(), "block_%d is a new extension so clear all load and store lists\n", block->getNumber());
                clearAllLists();
            }
        }
...
void InMemoryLoadStoreMarking::examineFirstReference(
    TR::Node* node, TR::TreeTop* tt) // called the first time 'node' is encountered
{
...
void InMemoryLoadStoreMarking::handleLoadLastReference(
    TR::Node* node, List<TR::Node>& nodeList, TR_NodeListTypes listType)
{
...
void InMemoryLoadStoreMarking::processBCDAggrNodeList(
    List<TR::Node>& nodeList, TR::Node* defNode, TR_NodeListTypes listType)
{
    if (!nodeList.isEmpty()) {
        if (cg->traceBCDCodeGen())
        ...
        for (TR::Node* listNode = listIt.getFirst(); listNode; listNode = listIt.getNext()) {
            TR_ASSERT(
                (isAnyTypeOfLoadList && listNode->getOpCode().isBCDLoadVar()) || listNode->getOpCode().isBCDStore(),
            ...
            if (isAnyTypeOfLoadList || listNode != defNode) // skip the current store treetop node
            {
                ...
                if (cg->traceBCDCodeGen())
                    traceMsg(comp(), "\t\tintersect defNode %s (%p) #%d aliases with listNode %s (%p) #%d\n",
                        ...
                        listNode->getOpCode().getName(), listNode,
                        listNode->getSymbolReference()->getReferenceNumber());

                if (!defNodeHasSymRef
                    || defNode->getSymbolReference()->getUseDefAliases().contains(
                           listNode->getSymbolReference(), comp())) {
                    if (isAnyTypeOfLoadList) {
                        ...
                        if (cg->traceBCDCodeGen())
                            traceMsg(comp(),
                                "\t\t\tfound an intersection so remove listNode %s (%p) from %s and set "
                                "setSkipCopyOnStore flag to false (defNodeHasSymRef=%s)\n",
                                listNode->getOpCode().getName(), listNode, getName(listType),
                                defNodeHasSymRef ? "yes" : "no");
...
// BCDOpportunities tracing has nothing to do with in memory load/store marking but this is a convenient place (late
// node walk) to do this
bool isBCDReductionOpportunity(TR::Node* node)
{
    ...
    if (node->getNumChildren() > 0 && !node->getOpCode().isLoadVar() && !node->getOpCode().isBasicPackedArithmetic()
        && !node->getOpCode().isExponentiation() && isBCDReductionOpportunity(node->getFirstChild())) {
        return true;
    }
...
void traceBCDOpportunities(TR::Node* node, TR::Compilation* comp)
{
    ...
    if (node->getOpCode().isBasicPackedArithmetic()) {
        ...
        if (isBCDReductionOpportunity(first) && isBCDReductionOpportunity(second)) {
            traceMsg(comp,
                "x^x : binaryReduction possibility -- node %s p=%d (%p) with op1 %s p=%d, op2 %s p=%d - line_no=%d "
                "(%s)\n",
                node->getOpCode().getName(), node->getDecimalPrecision(), node, first->getOpCode().getName(),
                first->getDecimalPrecision(), second->getOpCode().getName(), second->getDecimalPrecision(),
                comp->getLineNumber(node),
                node->getDecimalPrecision() <= TR::getMaxSignedPrecision<TR::Int64>()
                    ? "intOpp"
                    : (node->getDecimalPrecision() <= TR::DataType::getMaxExtendedDFPPrecision() ? "dfpOpp" : "noOpp"));
        }
    }

    if (node->getOpCode().isBasicOrSpecialPackedArithmetic() || node->getOpCodeValue() == TR::pdneg
        || node->getOpCodeValue() == TR::pdabs || node->getOpCode().isPackedArithmeticOverflowMessage()
        || (node->getOpCode().isConversion() && node->getType().isIntegral()
        ...
        if (node->getFirstChild()->getOpCode().isSetSign()
            ...
            traceMsg(comp, "x^x : found ascii pack opp -- %s/%s/%s (%p/%p/%p)\n", node->getOpCode().getName(),
                ...
                node->getFirstChild()->getFirstChild()->getOpCode().getName(), node, node->getFirstChild(),
                node->getFirstChild()->getFirstChild());
        }
    }

    if (node->getOpCode().isConversion() && node->getFirstChild()->getOpCode().isLoadConst()
        && (node->getType().isLongDouble() || node->getFirstChild()->getType().isLongDouble())) {
        traceMsg(comp, "x^x : long double const conv -- %s (%p)\n", node->getOpCode().getName(), node);
    }

    if (node->getOpCode().isConversion() && node->getType().isIntegral() && node->getFirstChild()->getType().isBCD()
    ...
    if ((node->getOpCodeValue() == TR::pd2l || node->getOpCodeValue() == TR::pd2i) && node->getNumChildren() >= 1
        ...
        traceMsg(comp, "x^x : binaryReduction possibility -- node %s (%p) with op1 %s p=%d - line_no=%d\n",
            ...
            node->getFirstChild()->getDecimalPrecision(), comp->getLineNumber(node));
    }
    ...
    if (node->getOpCode().isBasicOrSpecialPackedArithmetic()) {
        // setsign should be removed in this case as it isn't needed if first/second->getFirstChild() is already fixing
        // up sign
        TR::Node* first = node->getFirstChild();
        TR::Node* second = node->getOpCode().isBasicPackedArithmetic() ? node->getSecondChild() : NULL;
        if (first->getOpCodeValue() == TR::pdSetSign
            && first->getFirstChild()->getOpCode().isBasicOrSpecialPackedArithmetic()) {
            traceMsg(comp,
                "x^x : found arith with setsign first child over other arith -- node %s (%p), otherArith %s (%p)\n",
                node->getOpCode().getName(), node, first->getFirstChild()->getOpCode().getName(),
                first->getFirstChild());
        } else if (second && second->getOpCodeValue() == TR::pdSetSign
            && second->getFirstChild()->getOpCode().isBasicOrSpecialPackedArithmetic()) {
            traceMsg(comp,
                "x^x : found arith with setsign second child over other arith -- node %s (%p), otherArith %s (%p)\n",
                node->getOpCode().getName(), node, second->getFirstChild()->getOpCode().getName(),
                second->getFirstChild());
        }
    }
    ...
    if (node->getOpCode().isPackedShift() && node->getFirstChild()->getOpCode().isPackedShift()) {
        traceMsg(comp, "x^x : found shift %d over shift %d -- parent %s (%p), child %s (%p)\n",
            node->getDecimalAdjust(), node->getFirstChild()->getDecimalAdjust(), node->getOpCode().getName(), node,
            node->getFirstChild()->getOpCode().getName(), node->getFirstChild());
    }

    // should fold pdshl/pdsetsign to pdshlSetSign to get better codegen -- common reason for missing this is lack of
    // folding with child refCount > 1
    if (node->getOpCode().isPackedShift() && !node->getOpCode().isSetSign()
        && node->getFirstChild()->getOpCode().isSetSign()) {
        traceMsg(comp, "x^x : found shift over setSign -- parent %s (%p), child %s (%p)\n", node->getOpCode().getName(),
            node, node->getFirstChild()->getOpCode().getName(), node->getFirstChild());
    }
    ...
    if (node->getOpCode().isPackedLeftShift() && node->getOpCode().isSetSign()) {
        traceMsg(comp, "x^x : found pdshlSetSign by %d (%s) -- node %s (%p)\n", node->getDecimalAdjust(),
            isOdd(node->getDecimalAdjust()) ? "odd" : "even", node->getOpCode().getName(), node);
    }
    ...
    if (node->getOpCode().isConversion() && node->getType().isIntegral()
        ...
        traceMsg(comp,
            "x^x : found zoned to packed to binary conversion (task 59856) - node %s (%p), child %s, grandchild %s\n",
            node->getOpCode().getName(), node, node->getFirstChild()->getOpCode().getName(),
    ...
    if (node->getOpCodeValue() == TR::arraycmpWithPad) {
        ...
        if (cmpChildOneIsLit || cmpChildTwoIsLit) {
            traceMsg(comp,
                "x^x : found arraycmpWithPad with lit cmpOps (oneIsLit=%s,twoIsLit=%s) -- node %s (%p) lenOne %d, "
                "lenTwo %d, padCharIsLit=%s (0x%x) line_no=%d\n",
                cmpChildOneIsLit ? "yes" : "no", cmpChildTwoIsLit ? "yes" : "no", node->getOpCode().getName(), node,
    ...
    if (node->getOpCode().isConversion() && node->getType().isAnyZoned()
        && node->getFirstChild()->getType().isAnyPacked() && node->getFirstChild()->getOpCode().isShift()
        && node->getFirstChild()->getOpCode().isSetSign()) {
    ...
    if ((node->getOpCodeValue() == TR::pdSetSign || node->getOpCode().isPackedModifyPrecision())
        ...
        && node->getFirstChild()->getFirstChild()->getType().isIntegral()
        && node->getDecimalPrecision() < node->getFirstChild()->getDecimalPrecision() && node->isEvenPrecision()
        && node->getFirstChild()->isEvenPrecision()) {
        // in the below case both the pdModPrec and the l2pd will generate an NI to correct the top nibble when only the
        // pdModPrec NI is needed pdModPrec p = 4
        //    l2pd p = 18
        //
        traceMsg(comp, "x^x : found double even correction -- %s (%p) prec = %d with child %s (%p) prec = %d\n",
            node->getOpCode().getName(), node, node->getDecimalPrecision(),
            node->getFirstChild()->getOpCode().getName(), node->getFirstChild(),
    ...
    //
    // on < ARCH(11) the pd2ddEvaluator will have to use the longDouble conversion instruction as the double conv
    // instruction only converts 15 digits and >= 16 digits are needed When > 16 digits are needed then an NILL to clear
    // the 17 digit has to be done before returning the pd2dd result
    //
    // In this case it would be good to insert a pdModPrec to some odd amount (byte boundary) > the ddModPrec value
    // under the pd2dd as this will avoid the NILL and also loading more data then needed in the pd2ddEvaluator
    //
    if (!comp->cg()->supportsFastPackedDFPConversions() && node->getOpCodeValue() == TR::ddModifyPrecision
        ...
        if (pd2ddChild->getDecimalPrecision() >= TR::DataType::getMaxLongDFPPrecision()
            ...
            traceMsg(comp,
                "x^x : found larger srcLoad and/or redundant 17th digit correction about to happen in pd2ddEvaluator "
                "-- %s (%p) prec = %d with grandChild %s (%p) prec = %d on line_no=%d (offset %06X)\n",
                ddModPrec->getOpCode().getName(), ddModPrec, ddModPrec->getDFPPrecision(),
                pd2ddChild->getFirstChild()->getOpCode().getName(), pd2ddChild, pd2ddChild->getDecimalPrecision(),
                comp->getLineNumber(ddModPrec), comp->getLineNumber(ddModPrec));
        }
    }
    /*
