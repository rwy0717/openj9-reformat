...
void J9::Z::AheadOfTimeCompile::processRelocations()
{
    ...
    if (self()->getSizeOfAOTRelocations() != 0) {
        ...
        if (TR::Compiler->target.is64Bit()) {
            relocationDataCursor = self()->setRelocationData(
                fej9->allocateRelocationData(self()->comp(), self()->getSizeOfAOTRelocations() + 8));
...
uint8_t* J9::Z::AheadOfTimeCompile::initializeAOTRelocationHeader(TR::IteratedExternalRelocation* relocation)
{
    ...
    *(uint16_t*)cursor = relocation->getSizeOfRelocationData();
    AOTcgDiag5(comp(), "initializeAOTRelocationHeader cursor=%x size=%x wide=%x pair=%x kind=%x\n", cursor,
        *(uint16_t*)cursor, relocation->needsWideOffsets(), relocation->isOrderedPair(), relocation->getTargetKind());
    ...
    *flagsCursor = modifier;
    ...
    case TR_SpecialRamMethodConst: {
        ...
        inlinedSiteIndex = self()->findCorrectInlinedSiteIndex(
            tempSR->getOwningMethod(self()->comp())->constantPool(), inlinedSiteIndex);
    ...
    case TR_MethodPointer: {
        ...
        *(uintptrj_t*)cursor = inlinedSiteIndex;
        ...
        if (aconstNode->getOpCodeValue() == TR::loadaddr)
            j9method = (TR_OpaqueMethodBlock*)aconstNode->getSymbolReference()
                           ->getSymbol()
                           ->castToStaticSymbol()
                           ->getStaticAddress();
    ...
    case TR_ClassPointer: {
        ...
        *(uintptrj_t*)cursor = inlinedSiteIndex;
        ...
        if (relocation->getTargetAddress2())
            ...
            if (aconstNode->getOpCodeValue() == TR::loadaddr)
                j9class = (TR_OpaqueClassBlock*)aconstNode->getSymbolReference()
                              ->getSymbol()
                              ->castToStaticSymbol()
                              ->getStaticAddress();
        }
    ...
    case TR_ArbitraryClassAddress: {
        ...
        uintptr_t inlinedSiteIndex = self()->findCorrectInlinedSiteIndex(
            symRef->getOwningMethod(self()->comp())->constantPool(), (uintptr_t)relocation->getTargetAddress2());
    ...
    case TR_InlinedHCRMethod: {
        ...
        *(uintptrj_t*)cursor = (uintptrj_t)guard->getSymbolReference()
                                   ->getOwningMethod(self()->comp())
                                   ->constantPool(); // record constant pool
        cursor += SIZEPOINTER;
