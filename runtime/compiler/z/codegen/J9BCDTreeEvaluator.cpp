...
// RTC 93015: Using TypeReduction to treat all pdnegs as arith ops at ARCH(11) means that a tree with ipdstore -> pdneg
// -> pdX would get converted to ipdstore -> dd2pd -> ddneg -> ddX. Since the dd2pd could generate a CPDT that stores in
// place (rather than using a ZAP), this could mean that a ddneg producing a negative zero won't get cleaned; we'll use
// an LCDFR to flip the sign bit in DFP, then use a CPDT to write the final value. If we've decided to clean on the
// store, the clean isn't being generated, so we end up with a negative zero. The simplest solution is to *ALWAYS* clean
// the sign on the CPDT at ARCH(11), as there's no performance penalty in doing so.
#define TR_DFP_TO_PACKED_DEFAULT_MASK_ARCH11 \
    (0x9) ///< Result has a sign field (S=1), encode plus sign as 0xc (P=0) and clean (F=1)
    ...
    TR::MemoryReference* sourceMR, TR_PseudoRegister* childReg, TR::CodeGenerator* cg)
{
    ...
    generateSS1Instruction(
        cg, isUnicode ? TR::InstOpCode::PKU : TR::InstOpCode::PKA, node, pkxSourceSize - 1, destMR, sourceMR);
    ...
    if (node->getOpCode().isSetSign()) {
        ...
        if (sign == TR::DataType::getPreferredPlusCode())
            ...
            cg->genSignCodeSetting(
                node, targetReg, targetReg->getSize(), destMR, sign, targetReg, 0, false); // numericNibbleIsZero=false
...
TR::Register* J9::Z::TreeEvaluator::udsl2pdEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!node->getOpCode().isSetSign()) {
        ...
        if (cg->traceBCDCodeGen())
            traceMsg(comp,
                "\tudsl2pdEvaluator %p : op %s, isTruncation=%s, targetReg->isInit=%s, targetRegSize=%d, "
                ...
                childReg->getSize(), childReg->getDecimalPrecision(), sourceSignEndByte);
        ...
        if (isImplicitValue) {
            ...
            if (cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_z10)) {
                ...
                generateSIInstruction(cg, TR::InstOpCode::CLI, node,
                    generateS390LeftAlignedMemoryReference(*sourceMR, node, 0, cg, sourceSignEndByte), 0x00);
                generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE, node, cFlowRegionEnd);

                generateSIInstruction(cg, TR::InstOpCode::CLI, node,
                    generateS390LeftAlignedMemoryReference(*sourceMR, node, 1, cg, sourceSignEndByte), 0x2D);
                generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE, node, cFlowRegionEnd);
            }
        } else {
...
TR::Register* J9::Z::TreeEvaluator::pd2udslEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    static char* isVectorBCDEnv = feGetEnv("TR_enableVectorBCD");
    if (TR::Compiler->target.cpu.getS390SupportsVectorPackedDecimalFacility() && !comp->getOption(TR_DisableVectorBCD)
        || isVectorBCDEnv) {
    ...
    if (childReg->hasKnownSignCode()) {
        int32_t convertedSign = TR::DataType::convertSignEncoding(
            childNode->getDataType(), node->getDataType(), childReg->getKnownSignCode());
        if (convertedSign == TR::DataType::getNationalSeparateMinus()) {
            ...
            if (cg->traceBCDCodeGen())
                traceMsg(comp,
                    "\tchildReg has positive knownSignCode 0x%x so no more codegen is needed (an MVC of 002B was "
                    "already done)\n",
    ...
    TR_StorageReference* srcStorageReference)
{
    ...
    if (hint) {
        if (childReg->isInitialized() && hint == childReg->getStorageReference()) {
            TR_ASSERT(
                false, "pd2ud operands will overlap because child storageReference of pd2ud is initialized hint\n");
        } else {
    ...
    if (cg->traceBCDCodeGen())
        traceMsg(comp,
            ...
            node, node->getOpCode().getName(), targetReg->getSize(), targetReg->getDecimalPrecision(),
            childReg->getSize(), childReg->getDecimalPrecision());
    ...
    TR_PseudoRegister* targetReg, TR::MemoryReference* sourceMR, TR::MemoryReference* destMR, TR::CodeGenerator* cg)
{
    ...
    if (cg->traceBCDCodeGen())
        traceMsg(comp,
            "\tzonedToZonedSeparateSignHelper %p : op %s, isTruncation=%s, targetReg->knownSign=0x%x, "
            "trgSignIsZone=%s, targetReg->size=%d, targetRegPrec=%d, , digitsToClear=%d, (isSetSign=%s, sign 0x%x)\n",
            node, node->getOpCode().getName(), isTruncation ? "yes" : "no",
            targetReg->hasKnownOrAssumedSignCode() ? targetReg->getKnownOrAssumedSignCode() : 0,
            targetReg->knownOrAssumedSignIsZone() ? "yes" : "no", targetReg->getSize(),
            targetReg->getDecimalPrecision(), digitsToClear, isSetSign ? "yes" : "no", sign);
    ...
    if (isSetSign || (srcReg->hasKnownSignCode() && srcReg->knownSignIsEmbeddedPreferredOrUnsigned())) {
        ...
        TR_ASSERT(
            signToSet == TR::DataType::getZonedSeparatePlus() || signToSet == TR::DataType::getZonedSeparateMinus(),
        ...
        if (destSignCodeMR->getIndexRegister())
            dependencies->addPostConditionIfNotAlreadyInserted(
                destSignCodeMR->getIndexRegister(), TR::RealRegister::AssignAny);

        if (destSignCodeMR->getBaseRegister())
            dependencies->addPostConditionIfNotAlreadyInserted(
                destSignCodeMR->getBaseRegister(), TR::RealRegister::AssignAny);
...
TR::Register* J9::Z::TreeEvaluator::zdsls2zdEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (isEffectiveNop) {
        targetReg = evaluateBCDSignModifyingOperand(
            node, isEffectiveNop, false, false, sourceMR, cg); // isNondestructiveNop=false,initTarget=false
    } else {
    ...
    if (isSrcTrailingSign) {
        if (isEffectiveNop) {
            ...
            if (cg->traceBCDCodeGen())
                traceMsg(comp,
                    "\tisSrcTrailingSign=true and isEffectiveNop=true (zdsls2zd) : increment targetReg %s ignoredBytes "
                    "%d -> %d (by the TR::DataType::getZonedSignSize())\n",
...
TR::Register* J9::Z::TreeEvaluator::zdsle2zdEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (cg->traceBCDCodeGen())
        traceMsg(comp,
            "\tzdsle2zdEvaluator %p : op %s, isEffectiveNop=%s, isTruncation=%s, srcSignIsZone=%s, "
            "srcReg->getSize()=%d, (isSetSign=%s, sign 0x%x)\n",
    ...
    if (!isEffectiveNop && isLeadingDst
        ...
        if (cg->traceBCDCodeGen())
            traceMsg(comp,
                "\tperform an explicit widening (digitsToClear=%d, doWidening=yes, isEffectiveNop=no) set "
                "targetReg->prec to node->prec %d\n",
        ...
        if (!isEffectiveNop)
            sourceMR = generateS390RightAlignedMemoryReference(srcNode, srcReg->getStorageReference(), cg);
        targetReg = evaluateBCDSignModifyingOperand(
            node, isEffectiveNop, isNondestructiveNop, true /*initTarget*/, sourceMR, cg);
    ...
    if (!isEffectiveNop) {
        ...
        if (isSetSign) {
            if (sign == TR::DataType::getIgnoredSignCode()) {
                if (cg->traceBCDCodeGen())
                    traceMsg(comp, "\tisSetSign=true with ignored sign=0x%x\n", sign);
                ...
                if (cg->traceBCDCodeGen())
                    traceMsg(comp, "\tisSetSign=true : call genSignCodeSetting with sign=0x%x\n", sign);
                ...
                int32_t digitsCleared = cg->genSignCodeSetting(
                    node, targetReg, targetReg->getSize(), destMR, sign, targetReg, 0, numericNibbleIsZero);
        ...
        if (!isSetSign) {
            if (cg->traceBCDCodeGen())
                traceMsg(comp,
                    "\tisSetSign=false : generate MVZ of size 1 to transfer left aligned zdsle sign to right aligned "
                    "zd sign position\n");

            sourceMR = isTrailingSrc
                ? reuseS390RightAlignedMemoryReference(sourceMR, srcNode, srcReg->getStorageReference(), cg)
                : reuseS390LeftAlignedMemoryReference(
                      sourceMR, srcNode, srcReg->getStorageReference(), cg, srcReg->getSize());
            destMR = isTrailingDst
                ? reuseS390RightAlignedMemoryReference(destMR, node, targetReg->getStorageReference(), cg)
                : reuseS390LeftAlignedMemoryReference(
                      destMR, node, targetReg->getStorageReference(), cg, targetReg->getSize());
            int32_t mvzSize = 1;
        ...
        if (!(srcSignWillBeIgnored || srcSignResetRedundant || srcSignResetIllegal)) {
            {
                if (cg->traceBCDCodeGen())
                    traceMsg(comp, "\tgenerate OI 0xF0 to force %s-aligned high nibble to 0xF\n",
                        isTrailingSrc ? "right" : "left");
                generateSIInstruction(cg, TR::InstOpCode::OI, node,
                    generateS390LeftAlignedMemoryReference(
                        *destMR, node, 0, cg, isTrailingSrc ? 1 : targetReg->getSize()),
                    TR::DataType::getZonedCode());
            }
        }
        targetReg->setIsInitialized();
    ...
    TR::MemoryReference* sourceMR, TR_PseudoRegister* childReg, TR::CodeGenerator* cg)
{
    ...
    if (hint) {
        ...
        destSize = hint->getSymbolSize(); // may be be larger than the node->getSize() so take this opportunity to widen
                                          // as part of the PACK
        destPrecision = TR::DataType::getBCDPrecisionFromSize(
            node->getDataType(), destSize); // may be be larger than the node->getSize() so take this opportunity to
                                            // widen as part of the PACK
        targetStorageReference = hint;
    } else {
    ...
    if (destSize > 16) {
        if (cg->traceBCDCodeGen())
            traceMsg(comp,
                "\tdestSize %d > 16 so reduce destSize to 16 and destPrecision to 31 for PACK encoding and clear top "
                "%d byte(s)\n",
    ...
    if (sourcePrecision <= 16) {
        if (cg->traceBCDCodeGen())
            traceMsg(comp,
                "\tsourcePrecision %d <= 16 so generate a single PACK destSize %d, sourcePrecision %d, sourceEndByte "
                "%d\n",
        ...
        if (cg->traceBCDCodeGen()) {
            if (sourcePrecision >= 17 && sourcePrecision <= 30)
                ...
                traceMsg(
                    comp, "\tsourcePrecision == 31  so generate three PACKs with sourceEndByte %d\n", sourceEndByte);
        }
        ...
        if (sourcePrecision == 31) {
            ...
            if (cg->traceBCDCodeGen())
                traceMsg(comp,
                    "\tsourcePrecision == 31 so reduce sourcePrecision and destPrecision to 29 and update "
                    "sourceEndByte to %d\n",
                    sourceEndByte);
        }
        ...
        generateSS2Instruction(
            cg, TR::InstOpCode::PACK, node, pack1DestSize - 1, destMR, pack1SourceSize - 1, sourceMR);
        ...
        if (pack1DestSizeAsPrecision > pack1SourceSize) {
            ...
            targetReg->addRangeOfZeroDigits(
                pack1SourceSize + rightMostDigits, pack1DestSizeAsPrecision + rightMostDigits);
        }
        ...
        if (needsThirdPack) {
            ...
            generateSS2Instruction(cg, TR::InstOpCode::PACK, node, pack3DestSize - 1,
                ...
                pack3SourceSize - 1,
                generateS390LeftAlignedMemoryReference(
                    *sourceMR, node, pack3SourceOffset, cg, sourceMR->getLeftMostByte()));
        }
    } else {
    ...
    generateRXInstruction(
        cg, TR::InstOpCode::LLC, node, signCode, generateS390LeftAlignedMemoryReference(*destMR, node, 0, cg, 1));
    ...
    generateRXInstruction(
        cg, TR::InstOpCode::STC, node, signCode, generateS390LeftAlignedMemoryReference(*destMR, node, 0, cg, 1));
...
TR::Register* J9::Z::TreeEvaluator::zd2pdVectorEvaluatorHelper(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::MemoryReference* sourceMR = generateS390LeftAlignedMemoryReference(
        child, sourceReg->getStorageReference(), cg, child->getDecimalPrecision());
    ...
    TR::MemoryReference* sourceMR, TR_PseudoRegister* childReg, TR::CodeGenerator* cg)
{
    ...
    if (cg->traceBCDCodeGen())
        traceMsg(comp,
            ...
            "sourceSize %d\n",
            node, node->getOpCode().getName(), destPrecision, destSize, destEndByte, sourcePrecision,
            childReg->getSize());
    ...
    if (destPrecision <= 16 || sourcePrecision <= 16) {
        ...
        generateSS2Instruction(
            cg, TR::InstOpCode::UNPK, node, unpk1DestSize - 1, destMR, unpk1SourceSize - 1, sourceMR);
        ...
        generateSS2Instruction(cg, TR::InstOpCode::UNPK, node, unpk2DestSize - 1,
            ...
            unpk2SourceSize - 1,
            generateS390LeftAlignedMemoryReference(
                *sourceMR, node, unpk2SourceOffset, cg, sourceMR->getLeftMostByte()));
        if (needsThirdUnpk) {
            ...
            generateSS2Instruction(cg, TR::InstOpCode::UNPK, node, unpk3DestSize - 1,
                ...
                unpk3SourceSize - 1,
                generateS390LeftAlignedMemoryReference(
                    *sourceMR, node, unpk3SourceOffset, cg, sourceMR->getLeftMostByte()));
        }
    }
...
#define TR_DFP_TO_ZONED_PLUS \
    (0x2) ///< P: encode plus sign as 0xc (P=0) or as 0xf (P=1). Ignored when S=0 and sign is then determined by Z.
#define TR_DFP_TO_ZONED_FORCE_CLEAN \
    (0x1) ///< F: force negative zero to positive zero when F=1. The plus sign 0xc vs 0xf is determined by P.  Ignored
          ///< when S=0.
...
/**
 * Handles
 * TR::df2zd,TR::dd2zd,TR::de2zd,TR::df2zdSetSign,TR::dd2zdSetSign,TR::de2zdSetSign,TR::df2zdClean,TR::dd2zdClean,TR::de2zdClean
 */
TR::Register* J9::Z::TreeEvaluator::df2zdEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    cg->traceBCDExit("df2zd", node);

    TR_ASSERT(cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_zEC12),
        "CDZT/CXZT only valid on >= arch(10)\n");
    ...
    bool isClean = node->getOpCodeValue() == TR::df2zdClean || node->getOpCodeValue() == TR::dd2zdClean
        || node->getOpCodeValue() == TR::de2zdClean;

    if (cg->traceBCDCodeGen())
        traceMsg(comp, "\t%s : isSetSign %s, isClean %s\n", node->getOpCode().getName(), isSetSign ? "yes" : "no",
            isClean ? "yes" : "no");

    uint8_t mask = 0; // 0 is unsigned with ebcdic zone fields

    if (cg->traceBCDCodeGen())
        traceMsg(comp, "\tinit mask to 0x%x\n", mask);
    ...
    if (isSetSign) {
        // for all setSign cases do not set TR_DFP_TO_ZONED_SIGN in the mask -- for 0xf there is then nothing to do and
        // for 0xc/0xd the sign is easier to set on top of 0xf
        resultSignIsUnsigned = true;
        TR_ASSERT(!isClean, "setsign and clean must not be set together on node %s (%p)\n", node->getOpCode().getName(),
            node);
        TR_ASSERT(TR::DataType::getUnsignedCode() == 0xf,
            "expecting unsigned code to be 0xf and not 0x%x for node %s (%p)\n", TR::DataType::getUnsignedCode(),
        ...
        if (sign == raw_bcd_sign_0xf) {
            ...
            if (cg->traceBCDCodeGen())
                traceMsg(comp, "\t\tsetSign 0xf: leave mask at 0x%x for setSign 0xf\n", mask);
        } else if (sign == raw_bcd_sign_0xc) {
            setSignValue = TR::DataType::getPreferredPlusCode();
            if (cg->traceBCDCodeGen())
                traceMsg(comp, "\t\tsetSign 0xc: leave mask at 0x%x and encode setSign 0xc afterwards on top of 0xf\n",
                    mask);
            TR_ASSERT(TR::DataType::getPreferredPlusCode() == 0xc,
                "expecting preferred plus code to be 0xc and not 0x%x for node %s (%p)\n",
            ...
            if (cg->traceBCDCodeGen())
                traceMsg(comp, "\t\tsetSign 0xd: leave mask at 0x%x and encode setSign 0xd afterwards on top of 0xf\n",
                    mask);
            TR_ASSERT(TR::DataType::getPreferredMinusCode() == 0xd,
                "expecting preferred minus code to be 0xd and not 0x%x for node %s (%p)\n",
        ...
        if (cg->traceBCDCodeGen())
            traceMsg(comp, "\t\tsigned: update mask to 0x%x (+Sign 0x%x)\n", mask, TR_DFP_TO_ZONED_SIGN);
        if (isClean) {
            // TODO : signIsClean can also be set in non-setSign 0xf (TR_DFP_TO_ZONED_PLUS) cases where there is no
            // truncation
            //      : or could always set TR_DFP_TO_ZONED_FORCE_CLEAN (no harm unless this somehow makes the instruction
            //      more expensive)
            mask |= TR_DFP_TO_ZONED_FORCE_CLEAN;
            signIsClean = true;
            if (cg->traceBCDCodeGen())
                traceMsg(comp, "\t\tclean: update mask to 0x%x (+ForcePlus 0x%x to clean the sign)\n", mask,
                    TR_DFP_TO_ZONED_FORCE_CLEAN);
        }
    }
    ...
    if (hint) {
        ...
        // In case 3, using the hint size is fine, but there shouldn't be a hint generated since the p=4 dd2zd result
        // can't fit in the p=3 zdstore field
    ...
    // regardless of how the memSize and regSize were set up earlier ensure they do not exceed the limits for the CZDT
    // or CZXT instructions and assert below if these max instruction settings are >= the max # of significant digits
    // that can be represented for each DFP type
    if (isFloat) {
        TR_ASSERT(TR_DECIMAL_FLOAT_TO_ZONED_MAX_PRECISION >= TR::DataType::getMaxShortDFPPrecision(),
            "CZDT max %d must be >= DFP short max %d\n", TR_DECIMAL_FLOAT_TO_ZONED_MAX_PRECISION,
            TR::DataType::getMaxShortDFPPrecision());
        memSize
            = std::min(memSize, TR_DECIMAL_FLOAT_TO_ZONED_MAX_PRECISION); // cannot get more than 16 digits from the DFP
                                                                          // source (and larger encodings are illegal)
        regSize = std::min(regSize, TR_DECIMAL_FLOAT_TO_ZONED_MAX_PRECISION);
    } else if (isDouble) {
        TR_ASSERT(TR_DECIMAL_DOUBLE_TO_ZONED_MAX_PRECISION >= TR::DataType::getMaxLongDFPPrecision(),
            "CZDT max %d must be >= DFP long max %d\n", TR_DECIMAL_DOUBLE_TO_ZONED_MAX_PRECISION,
            TR::DataType::getMaxLongDFPPrecision());
        memSize = std::min(
            memSize, TR_DECIMAL_DOUBLE_TO_ZONED_MAX_PRECISION); // cannot get more than 16 digits from the DFP source
                                                                // (and larger encodings are illegal)
        regSize = std::min(regSize, TR_DECIMAL_DOUBLE_TO_ZONED_MAX_PRECISION);
    } else {
        TR_ASSERT(TR_DECIMAL_LONG_DOUBLE_TO_ZONED_MAX_PRECISION >= TR::DataType::getMaxExtendedDFPPrecision(),
            "CZXT max %d must be >= DFP long double max %d\n", TR_DECIMAL_LONG_DOUBLE_TO_ZONED_MAX_PRECISION,
            TR::DataType::getMaxExtendedDFPPrecision());
        memSize = std::min(
            memSize, TR_DECIMAL_LONG_DOUBLE_TO_ZONED_MAX_PRECISION); // cannot get more than 34 digits from the DFP
                                                                     // source (and larger encodings are illegal)
        regSize = std::min(regSize, TR_DECIMAL_LONG_DOUBLE_TO_ZONED_MAX_PRECISION);
    }

    if (regSize > memSize) {
        // since the memSize is the max # of digits possibly converted then cannot set a higher regSize as these upper
        // digits/bytes would be uninitialized
        if (cg->traceBCDCodeGen())
    ...
    TR::MemoryReference* destMR
        = generateS390LeftAlignedMemoryReference(node, targetStorageReference, cg, memSize, enforceSSLimits);

    if (cg->traceBCDCodeGen())
        traceMsg(comp,
            "\tgenerate DFP to zoned %s (%p) with op=%s and target zoned size %d, mask 0x%x (signIsClean %s, "
            "signIsPreferred %s)\n",
            node->getOpCode().getName(), node, isLongDouble ? "TR::InstOpCode::CZXT" : "TR::InstOpCode::CZDT", memSize,
            mask, signIsClean ? "yes" : "no", signIsPreferred ? "yes" : "no");

    if (isFloat) {
        TR_ASSERT(memSize > 0 && memSize <= TR_DECIMAL_FLOAT_TO_ZONED_MAX_PRECISION,
            "memSize %d out of allowed range 1 -> %d for convert DFP to zoned (CZDT)\n", memSize,
            TR_DECIMAL_FLOAT_TO_ZONED_MAX_PRECISION);
    } else if (isDouble) {
        TR_ASSERT(memSize > 0 && memSize <= TR_DECIMAL_DOUBLE_TO_ZONED_MAX_PRECISION,
            "memSize %d out of allowed range 1 -> %d for convert DFP to zoned (CZDT)\n", memSize,
            TR_DECIMAL_DOUBLE_TO_ZONED_MAX_PRECISION);
    } else {
        TR_ASSERT(isLongDouble, "type should be isLongDouble for node %s (%p)\n", node->getOpCode().getName(), node);
        TR_ASSERT(memSize > 0 && memSize <= TR_DECIMAL_LONG_DOUBLE_TO_ZONED_MAX_PRECISION,
            "memSize %d out of allowed range 1 -> %d for convert DFP to zoned (CZXT)\n", memSize,
            TR_DECIMAL_LONG_DOUBLE_TO_ZONED_MAX_PRECISION);
    }

    generateRSLbInstruction(
        cg, isLongDouble ? TR::InstOpCode::CZXT : TR::InstOpCode::CZDT, node, srcFPReg, memSize - 1, destMR, mask);
    ...
    if (cg->traceBCDCodeGen()) {
        traceMsg(comp, "\ttargetReg %s signState : cleanSign=%s, preferredSign=%s, knownSign=%s (0x%x)\n",
            cg->getDebug()->getName(targetReg), targetReg->hasKnownCleanSign() ? "yes" : "no",
            targetReg->hasKnownPreferredSign() ? "yes" : "no", targetReg->hasKnownSignCode() ? "yes" : "no",
            targetReg->hasKnownSignCode() ? targetReg->getKnownSignCode() : TR::DataType::getInvalidSignCode());
    }
 ...
 */
TR::Register* J9::Z::TreeEvaluator::zd2ddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    cg->traceBCDEntry("zd2dd", node);

    TR_ASSERT(cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_zEC12),
        "CDZT/CXZT only valid on >= arch(10)\n");
    TR_ASSERT(node->getDataType() == TR::DecimalDouble || node->getDataType() == TR::DecimalLongDouble,
        "expecting op to be zd2dd or zd2de and not %d\n", node->getOpCodeValue());
    TR_ASSERT(node->getDecimalFraction() == 0, "frac should be 0 and not %d\n", node->getDecimalFraction());

    bool isAbs = node->getOpCodeValue() == TR::zd2dfAbs || node->getOpCodeValue() == TR::zd2ddAbs
        || node->getOpCodeValue() == TR::zd2deAbs;
    ...
    bool enforceSSLimits = true; // RSL has same limits as SS (no index and 12 bit displacment)
    TR::MemoryReference* sourceMR
        = generateS390LeftAlignedMemoryReference(srcNode, srcStorageReference, cg, srcRegSize, enforceSSLimits);
    ...
    if (isLongDouble) {
        TR_ASSERT(srcRegSize > 0 && srcRegSize <= TR_ZONED_TO_DECIMAL_LONG_DOUBLE_MAX_SIZE,
            "srcRegSize %d not in valid CXZT range %d -> %d\n", srcRegSize, 0,
            TR_ZONED_TO_DECIMAL_LONG_DOUBLE_MAX_SIZE);
    } else {
        TR_ASSERT(srcRegSize > 0 && srcRegSize <= TR_ZONED_TO_DECIMAL_DOUBLE_MAX_SIZE,
            "srcRegSize %d not in valid CDZT range %d -> %d\n", srcRegSize, 0, TR_ZONED_TO_DECIMAL_DOUBLE_MAX_SIZE);
    }

    generateRSLbInstruction(cg, isLongDouble ? TR::InstOpCode::CXZT : TR::InstOpCode::CDZT, node, fprTargetReg,
        srcRegSize - 1, sourceMR, mask);
    ...
    uint32_t callChildStartIndex, bool isResultPD, bool isUseVector, bool isVariableParam)
{
    ...
    cg->generateDebugCounter(
        TR::DebugCounter::debugCounterName(cg->comp(), "DAA/OOL/(%s)/%p", cg->comp()->signature(), node), 1,
    ...
    if (isResultPD) {
        ...
        if (isUseVector) {
            ...
            TR::MemoryReference* targetMR = generateS390RightAlignedMemoryReference(
                pdopNode, static_cast<TR_PseudoRegister*>(bcdOpResultReg)->getStorageReference(), cg);
        ...
        if (isResultLong) {
            if (TR::Compiler->target.is32Bit() && !cg->use64BitRegsOn32Bit()) {
                ...
                generateRRInstruction(
                    cg, TR::InstOpCode::LR, node, bcdOpPair->getLowOrder(), callResultReg->getLowOrder());
                generateRRInstruction(
                    cg, TR::InstOpCode::LR, node, bcdOpPair->getHighOrder(), callResultReg->getHighOrder());
            } else {
    ...
    cursor = generateS390LabelInstruction(
        cg, TR::InstOpCode::LABEL, node, passThroughLabel, cg->getCurrentCheckNodeRegDeps());
...
TR::Register* J9::Z::TreeEvaluator::BCDCHKEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (resultReg != NULL) {
        ...
        resultReg = BCDCHKEvaluatorImpl(
            node, cg, numCallChildren, firstCallParamIndex, isResultPD, isEnableVectorBCD, isVariableParam);
...
TR::Register* J9::Z::TreeEvaluator::pdcmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.cpu.getS390SupportsVectorPackedDecimalFacility()
        ...
        targetReg = generateS390CompareBool(
...
TR::RegisterDependencyConditions* setupPackedDFPConversionGPRs(
    TR::Register*& gpr64, TR::Register*& gpr64Hi, TR::Register*& gpr64Lo, bool isLongDouble, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
        ...
        // the 64 bit registers gpr64Hi/gpr64Lo are going to be clobbered and R0/R1 are safe to clobber regardless of
        // the hgpr (use64BitRegsOn32Bit) setting
 ...
 */
TR::Register* J9::Z::TreeEvaluator::pd2ddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    cg->traceBCDEntry("pd2dd", node);

    TR_ASSERT(node->getDataType() == TR::DecimalDouble || node->getDataType() == TR::DecimalLongDouble,
        "expecting op to be pd2dd or pd2de and not %d\n", node->getOpCodeValue());
    TR_ASSERT(node->getDecimalFraction() == 0, "frac should be 0 and not %d\n", node->getDecimalFraction());

    bool isLongDouble = node->getDataType() == TR::DecimalLongDouble;
    bool isAbs = node->getOpCodeValue() == TR::pd2dfAbs || node->getOpCodeValue() == TR::pd2ddAbs
        || node->getOpCodeValue() == TR::pd2deAbs;
    TR::Node* srcNode = node->getFirstChild();
    TR_PseudoRegister* srcReg = cg->evaluateBCDNode(srcNode);
    TR_StorageReference* srcStorageReference = srcReg->getStorageReference();
    int32_t srcRegSize = srcReg->getSize();
    int32_t srcFixedSize
        = cg->getPackedToDecimalLongDoubleFixedSize(); // max that can be handled by TR::InstOpCode::CXSTR
    ...
    if (cg->traceBCDCodeGen())
        traceMsg(comp, "%s %p : srcRegPrec %d, srcRegSize %d  (srcFixedSize = %d, isInit = %s)\n",
            node->getOpCode().getName(), node, srcReg->getDecimalPrecision(), srcRegSize, srcFixedSize,
            srcReg->isInitialized() ? "yes" : "no");

    // At arch(11), we have an efficient instruction that converts the packed value, in memory, to the DFP value, in an
    // FP register
    if (cg->supportsFastPackedDFPConversions()) {
        ...
        if (cg->traceBCDCodeGen())
            traceMsg(comp,
                "\tgenerate packed to DFP %s (%p) with op=%s and src packed size %d (prec=%d), mask 0x%x%s\n",
                node->getOpCode().getName(), node, isLongDouble ? "CXPT" : "CDPT", srcRegSize, srcPrec, mask,
                isAbs ? " (abs)" : "");

        if (isLongDouble) {
            // 1 -> 18 bytes but for 18 bytes only 34 and not 35 digits are allowed (data exception otherwise)
            TR_ASSERT(srcPrec > 0 && srcPrec <= TR_PACKED_TO_DECIMAL_LONG_DOUBLE_PRECISION_ARCH11,
                "srcPrec %d not in valid CXPT range %d -> %d\n", srcPrec, 0,
                TR_PACKED_TO_DECIMAL_LONG_DOUBLE_PRECISION_ARCH11);
        } else {
            // 1 -> 9 bytes but for 9 bytes only 16 and not 17 digits are allowed (data exception otherwise)
            TR_ASSERT(srcPrec > 0 && srcPrec <= TR_PACKED_TO_DECIMAL_DOUBLE_PRECISION_ARCH11,
                "srcPrec %d not in valid CDPT range %d -> %d\n", srcPrec, 0,
                TR_PACKED_TO_DECIMAL_DOUBLE_PRECISION_ARCH11);
        }

        generateRSLbInstruction(cg, isLongDouble ? TR::InstOpCode::CXPT : TR::InstOpCode::CDPT, node, fprTargetReg,
            srcRegSize - 1, sourceMR, mask);
    } else {
        ...
        if (!isLongDouble && srcRegSize > cg->getPackedToDecimalDoubleFixedSize()) {
            // There is a mismatch between the max prec supported by CDSTR (15 digits + sign) and the max prec supported
            // in long (8-byte) DFP format (16 digits) For this case use the larger convert instruction CXSTR and round
            // down afterwards CXSTR can convert up to 34 digits but the subsequent round down will lose precision due
            // to rounding for any value with > 16 digits so only 9 bytes (16 or 17 digits) of packed digits will be
            // loaded and if the srcReg precision is > 16 then it will corrected back to a max of 16 digits with the
            // NILL instruction below
            isForcedLongDouble = true;
            isLongDouble = true;
            srcRegSize = 9;
            if (cg->traceBCDCodeGen())
                traceMsg(comp,
                    "\tsrcRegSize %d  > max of %d for CDSTR : set isLongDouble=true and reduce srcRegSize %d -> %d\n",
                    srcRegSize, cg->getPackedToDecimalDoubleFixedSize(), srcRegSize + 9, srcRegSize);
        }

        // TODO: examine the leftAlignedZeroDigits on the srcReg to see if  srcRegSize can be widened to the next 2,4,8
        // byte boundary so the load code below can be simplified
        bool enforceSSLimits = false;
        TR::MemoryReference* sourceMR
            = generateS390LeftAlignedMemoryReference(srcNode, srcStorageReference, cg, srcRegSize, enforceSSLimits);

        TR::RegisterDependencyConditions* deps
            = setupPackedDFPConversionGPRs(gpr64, gpr64Hi, gpr64Lo, isLongDouble, cg);
        if (isLongDouble)
        ...
        case 7:
            generateRRInstruction(cg, TR::InstOpCode::XGR, srcNode, gpr64Partial, gpr64Partial);
            generateRSInstruction(cg, TR::InstOpCode::ICMH, node, gpr64Partial,
                (uint32_t)((1 << (partialSize - 4)) - 1), sourceMR); // 5->mask=1, 6->mask=3, 7->mask=7
            generateRXInstruction(cg, TR::InstOpCode::L, srcNode, gpr64Partial,
                generateS390LeftAlignedMemoryReference(
                    *sourceMR, srcNode, partialSize - 4, cg, sourceMR->getLeftMostByte(), enforceSSLimits));
            break;
        case 8:
        ...
        if (isLongDouble) {
            if (srcRegSize > 8)
                generateRXInstruction(cg, TR::InstOpCode::LG, srcNode, gpr64Lo,
                    generateS390LeftAlignedMemoryReference(
                        *sourceMR, srcNode, partialSize, cg, sourceMR->getLeftMostByte(), enforceSSLimits));
            else
                generateRRInstruction(cg, TR::InstOpCode::XGR, srcNode, gpr64Hi, gpr64Hi);
        }

        if (isForcedLongDouble && srcReg->getDecimalPrecision() > TR::DataType::getMaxLongDFPPrecision()) {
            ...
            // if the srcReg->getDecimalPrecision() == TR::DataType::getMaxLongDFPPrecision() (16) then the NILL is not
            // needed.
            TR_ASSERT(srcRegSize == 9,
                "expecting srcRegSize == 9 and not %d on %s (%p) for the isForcedLongDouble case\n", srcRegSize,
                node->getOpCode().getName(), node);
            generateRIInstruction(cg, TR::InstOpCode::NILL, node, gpr64Hi, 0x000F);
            if (cg->traceBCDCodeGen())
                traceMsg(comp,
                    "\tgen NILL %s, 0x000F to clear out possible 17th digit from CXSTR in forced longDouble case\n",
                    cg->getDebug()->getName(gpr64Hi));
        }

        generateRRInstruction(
            cg, isLongDouble ? TR::InstOpCode::CXSTR : TR::InstOpCode::CDSTR, node, fprTargetReg, gpr64, deps);

        // For a pre-arch(11) pd2ddAbs, need to force the positive sign separately from the conversion
        if (isAbs)
            generateRRInstruction(cg, TR::InstOpCode::LPDFR, node,
                isLongDouble ? fprTargetReg->getHighOrder() : fprTargetReg,
                isLongDouble ? fprTargetReg->getHighOrder() : fprTargetReg);
 ...
 */
TR::Register* J9::Z::TreeEvaluator::df2pdEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (isSetSign) {
        ...
        if (sign == raw_bcd_sign_0xf) {
            ...
            signIsPreferred = false;
            if (cg->traceBCDCodeGen())
                traceMsg(comp, "\t\tsetSign 0xf: set mask to 0x%x\n", mask);
            TR_ASSERT(TR::DataType::getUnsignedCode() == 0xf,
                "expecting unsigned code to be 0xf and not 0x%x for node %s (%p)\n", TR::DataType::getUnsignedCode(),
                node->getOpCode().getName(), node);
        } else if (sign == raw_bcd_sign_0xc) {
            setSignValue = TR::DataType::getPreferredPlusCode();
            if (cg->traceBCDCodeGen())
                traceMsg(comp, "\t\tsetSign 0xc: leave mask at 0x%x\n", mask);
            TR_ASSERT(TR::DataType::getPreferredPlusCode() == 0xc,
                "expecting preferred plus code to be 0xc and not 0x%x for node %s (%p)\n",
                TR::DataType::getPreferredPlusCode(), node->getOpCode().getName(), node);
        } else if (sign == raw_bcd_sign_0xd) {
            ...
            isSignNegative = true;
            if (cg->traceBCDCodeGen())
                traceMsg(comp, "\t\tsetSign 0xd: leave mask at 0x%x\n", mask);
            TR_ASSERT(TR::DataType::getPreferredMinusCode() == 0xd,
                "expecting preferred minus code to be 0xd and not 0x%x for node %s (%p)\n",
                TR::DataType::getPreferredMinusCode(), node->getOpCode().getName(), node);
        } else {
    ...
    if (isFloat) {
        maxBytesFromConv = cg->getDecimalFloatToPackedFixedSize(); // CSDTR produces 15 digits == 8 bytes; CPDT produces
                                                                   // 16 digits == 9 bytes
        maxDigitsInSrc = std::min(node->getSourcePrecision(),
            TR::DataType::getMaxShortDFPPrecision()); // TR::DataType::getMaxShortDFPPrecision() == 7 digits == 4 bytes
    } else if (isDouble) {
        maxBytesFromConv = cg->getDecimalDoubleToPackedFixedSize(); // CSDTR produces 15 digits == 8 bytes; CPDT
                                                                    // produces 16 digits == 9 bytes
        maxDigitsInSrc = std::min(node->getSourcePrecision(),
            TR::DataType::getMaxLongDFPPrecision()); // TR::DataType::getMaxLongDFPPrecision() == 16 digits == 9 bytes
    } else if (isLongDouble) {
        maxBytesFromConv = cg->getDecimalLongDoubleToPackedFixedSize(); // CSXTR produces 31 digits == 16 bytes; CPXT
                                                                        // produces 34 digits == 18 bytes
        maxDigitsInSrc = std::min(
            node->getSourcePrecision(), TR::DataType::getMaxExtendedDFPPrecision()); // TR::DataType::getMaxExtendedDFPPrecision()
                                                                                     // == 34 digits == 18 bytes
    } else {
    ...
    if (isSetSign) {
        ...
        if (isLongDouble) // Existing node was long double; lengthening from double to long double is performed later
        {
            ...
            if (!canClobber)
                generateRRInstruction(
                    cg, TR::InstOpCode::LDR, node, srcFPRegTemp->getLowOrder(), srcFPReg->getLowOrder());
    ...
    if (isDouble && node->getSize() > maxBytesFromConv) {
        if (isSetSign)
            generateRRFInstruction(
                cg, TR::InstOpCode::LXDTR, node, srcFPRegTemp, srcFPReg->getLowOrder(), 0, false); // m4=0
        else
    ...
    if (cg->supportsFastPackedDFPConversions()) {
        ...
        if (node->hasSourcePrecision() && node->getSourcePrecision() <= TR::DataType::getMaxExtendedDFPPrecision()) {
            ...
            memSize = std::max(
                targetReg->getSize(), TR::DataType::packedDecimalPrecisionToByteLength(node->getSourcePrecision()));
        } else {
            // If source precision is unknown, assume the maximum possible value to avoid overflow errors
            if (isFloat || isDouble)
        ...
        if (hint) {
            ...
            // For case 1, using the hint size is good. For case 2, it's incorrect, as we'd lose the truncation to 4
            // digits. In case 3, using the hint size is fine, but there shouldn't be a hint generated since the p=5
            // dd2pd result can't fit in the p=4 pdstore field
            if (node->hasSourcePrecision() && !node->isTruncating()) {
                // Don't widen if doing so would require a conversion from double to long double
                if (!isDouble
                    || (hint->getSymbolSize() <= TR::DataType::packedDecimalPrecisionToByteLength(
                                                     TR::DataType::getMaxLongDFPPrecision()))) {
                    // Widen up to 32 digits (33 and 34 are slower)
                    if (regSize <= TR::DataType::packedDecimalPrecisionToByteLength(TR_MAX_DFP_PACKED_FAST_SIZE)
                        && hint->getSymbolSize()
                            >= TR::DataType::packedDecimalPrecisionToByteLength(TR_MAX_DFP_PACKED_FAST_SIZE))
                        regSize = TR::DataType::packedDecimalPrecisionToByteLength(TR_MAX_DFP_PACKED_FAST_SIZE);
                    else
                        regSize = hint->getSymbolSize(); // Widen up to the symbol size

                    // Enlarge the memory required if it's now smaller than the precision we'll set on the target
                    // register
                    if (regSize > memSize)
                        memSize = regSize;
                    if (cg->traceBCDCodeGen())
        ...
        // regardless of how the memSize and targetPrec were set up earlier ensure they do not exceed the limits for the
        // CPDT or CPXT instructions and assert below if these max instruction settings are >= the max # of significant
        // digits that can be represented for each DFP type
        int32_t maxConvertedPrecision = 0;
        if (isFloat) {
            TR_ASSERT(TR_DECIMAL_FLOAT_TO_PACKED_PRECISION_ARCH11 >= TR::DataType::getMaxShortDFPPrecision(),
                "CPDT max %d must be >= DFP short max %d\n", TR_DECIMAL_FLOAT_TO_PACKED_PRECISION_ARCH11,
                TR::DataType::getMaxShortDFPPrecision());
            memSize = std::min(
                memSize, TR_DECIMAL_FLOAT_TO_PACKED_SIZE_ARCH11); // cannot get more than 9 bytes (16 digits) from the
                                                                  // DFP source (and larger encodings are illegal)
            maxConvertedPrecision = TR_DECIMAL_FLOAT_TO_PACKED_PRECISION_ARCH11;
        } else if (isDouble) {
            TR_ASSERT(TR_DECIMAL_DOUBLE_TO_PACKED_PRECISION_ARCH11 >= TR::DataType::getMaxLongDFPPrecision(),
                "CPDT max %d must be >= DFP long max %d\n", TR_DECIMAL_DOUBLE_TO_PACKED_PRECISION_ARCH11,
                TR::DataType::getMaxLongDFPPrecision());
            memSize = std::min(
                memSize, TR_DECIMAL_DOUBLE_TO_PACKED_SIZE_ARCH11); // cannot get more than 9 bytes (16 digits) from the
                                                                   // DFP source (and larger encodings are illegal)
            maxConvertedPrecision = TR_DECIMAL_DOUBLE_TO_PACKED_PRECISION_ARCH11;
        } else {
            TR_ASSERT(TR_DECIMAL_LONG_DOUBLE_TO_PACKED_PRECISION_ARCH11 >= TR::DataType::getMaxExtendedDFPPrecision(),
                "CPXT max %d must be >= DFP long double max %d\n", TR_DECIMAL_LONG_DOUBLE_TO_PACKED_PRECISION_ARCH11,
                TR::DataType::getMaxExtendedDFPPrecision());
            memSize = std::min(memSize,
                TR_DECIMAL_LONG_DOUBLE_TO_PACKED_SIZE_ARCH11); // cannot get more than 18 bytes (34 digits) from the DFP
                                                               // source (and larger encodings are illegal)
        ...
        if (targetReg->getDecimalPrecision() > maxConvertedPrecision) {
            // since the finalPrecision is the max # of digits possibly converted then cannot set a higher targetPrec as
            // these upper digits would be uninitialized
            if (cg->traceBCDCodeGen())
        ...
        if (cg->traceBCDCodeGen())
            traceMsg(comp,
                "\tgenerate DFP to packed %s (%p) with op=%s and target packed size %d, mask 0x%x (signIsPreferred "
                "%s)\n",
                node->getOpCode().getName(), node, isLongDouble ? "TR::InstOpCode::CPXT" : "TR::InstOpCode::CPDT",
                memSize, mask, signIsPreferred ? "yes" : "no");

        generateRSLbInstruction(
            cg, isLongDouble ? TR::InstOpCode::CPXT : TR::InstOpCode::CPDT, node, srcFPReg, memSize - 1, destMR, mask);

        bool evenDigitNeedsClearing
            = !node->canSkipPadByteClearing() && isEven(node->getDecimalPrecision()) && node->isTruncating();
        ...
        if (evenDigitNeedsClearing)
            generateSIInstruction(cg, TR::InstOpCode::NI, node,
                destMR = generateS390LeftAlignedMemoryReference(*destMR, node, 0, cg, memSize, enforceSSLimits), 0x0F);
            ...
            if (srcNode->isNonNegative()) {
                if (cg->traceBCDCodeGen())
                    traceMsg(comp, "\tsrcNode %s (%p) isNonNegative = true : set setKnownSignCode to 0x%x on %s\n",
                        srcNode->getOpCode().getName(), srcNode, TR::DataType::getPreferredPlusCode(),
                        cg->getDebug()->getName(targetReg));
                targetReg->setKnownSignCode(TR::DataType::getPreferredPlusCode());
            } else if (!evenDigitNeedsClearing) {
                if (cg->traceBCDCodeGen())
                    traceMsg(comp, "\tisClean op = true (always clean when using %s) : setHasKnownCleanSign on %s\n",
                        isLongDouble ? "TR::InstOpCode::CPXT" : "TR::InstOpCode::CPDT",
                        cg->getDebug()->getName(targetReg));
                targetReg->setHasKnownCleanSign();
            } else if (signIsPreferred) {
                if (cg->traceBCDCodeGen())
        ...
        if (hint)
            ...
            targetStorageReference
                = TR_StorageReference::createTemporaryBasedStorageReference(targetReg->getSize(), comp);
        targetReg->setStorageReference(targetStorageReference, node);

        bool enforceSSLimits = false; // RX/RS form inst are being used so Y-form is available for these
        TR::MemoryReference* destMR = generateS390LeftAlignedMemoryReference(
            node, targetStorageReference, cg, targetReg->getSize(), enforceSSLimits);
        ...
        if (targetPrec > maxDigitsFromConv) {
            ...
            if (cg->traceBCDCodeGen())
                traceMsg(comp, "\ttargetPrec %d > maxDigitsFromConv %d : first convert high %d digit(s)\n", targetPrec,
                    maxDigitsFromConv, targetPrec - maxDigitsFromConv);
            // float cases have max targetPrec of 7 and a min maxDigitsFromConv of 15 so can never reach here
            // double case have max targetPrec of 16 and a maxDigitsFromConv of 15 (but only when targetPrec is <=15) or
            // a maxDigitsFromConv of 31 when up-converted to longDouble
            ...
            generateRXFInstruction(
                cg, TR::InstOpCode::SRXT, node, srcFPRegTemp, srcFPReg, shiftRef); // shift away low digits
            generateRRInstruction(
                cg, TR::InstOpCode::CSXTR, node, gpr64, srcFPRegTemp, deps); // convert high digits to packed
            ...
            default:
                TR_ASSERT(
                    false, "unexpected highDigits %d on node %s (%p)\n", highDigits, node->getOpCode().getName(), node);
            }
            if (cg->traceBCDCodeGen())
        ...
        if (isLongDouble) {
            if (partialSize > 8) {
                ...
                if (cg->traceBCDCodeGen())
                    traceMsg(
                        comp, "\tisExt=true and partialSize %d <= 8 so store from the low gpr64 only\n", partialSize);
            }
        }
        ...
        if (!node->canSkipPadByteClearing() && isEven(remainingTargetDigits) && isTruncation) {
            ...
            if (clearOp != TR::InstOpCode::BAD) {
                ...
                if (cg->traceBCDCodeGen())
                    traceMsg(comp,
                        "\tisTruncation=true and remainingTargetDigits isEven (%d) so perform regBased clear of top "
                        "nibble with NIxx\n",
                        remainingTargetDigits);
            }
        }
        ...
        case 7:
            generateRSInstruction(
                cg, TR::InstOpCode::STCMH, node, gpr64Partial, (uint32_t)((1 << (partialSize - 4)) - 1), destMR);
            generateRXInstruction(cg, TR::InstOpCode::ST, node, gpr64Partial,
                generateS390LeftAlignedMemoryReference(
                    *destMR, node, partialSize - 4, cg, destMR->getLeftMostByte(), enforceSSLimits));
            break;
        case 8:
        ...
        if (isLongDouble && remainingTargetBytes > 8)
            generateRXInstruction(cg, TR::InstOpCode::STG, node, gpr64Lo,
                generateS390LeftAlignedMemoryReference(
                    *destMR, node, partialSize, cg, destMR->getLeftMostByte(), enforceSSLimits));
        ...
        if (needsClear) {
            if (cg->traceBCDCodeGen())
                traceMsg(comp,
                    "\tisTruncation=true and targetReg->isEvenPrec (%d) so perform memBased clear of top nibble with "
                    "NI\n",
                    targetReg->getDecimalPrecision());
        ...
        if (isSetSign) {
            ...
            } else if (node->hasKnownCleanSign()) // perhaps a ddclean/declean has been inserted below in IL
            {
                ...
                targetReg->setHasKnownCleanSign();
            } else if (!isTruncation && srcNode->getOpCode().isSub() && srcNode->getFirstChild()->isNonNegative()
                && srcNode->getSecondChild()->isNonNegative()) {
                if (cg->traceBCDCodeGen())
                    traceMsg(comp,
                        "\t%s (%p) srcNode %s (%p) has two nonNegative children : setHasKnownCleanSign on %s\n",
                        node->getOpCode().getName(), node, srcNode->getOpCode().getName(), srcNode,
                        cg->getDebug()->getName(targetReg));
                targetReg->setHasKnownCleanSign();
            } else if (signIsPreferred) {
                if (cg->traceBCDCodeGen())
...
TR::Register* J9::Z::TreeEvaluator::pd2lVariableEvaluator(TR::Node* node, TR::CodeGenerator* cg, bool isUseVectorBCD)
{
    ...
    if (isUseVectorBCD) {
        ...
        if (!disableTPBeforePD2I) {
            ...
            generateS390BranchInstruction(
                cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_MASK7, node, cg->getCurrentBCDCHKHandlerLabel());
        }
        ...
        if (!disableTPBeforePD2I) {
            ...
            auto* testPackedInstruction = generateRSLInstruction(
                cg, TR::InstOpCode::TP, node, 0, generateS390MemoryReference(*sourceMR, 0, cg));
        ...
        if (PD2I) {
            generateRXInstruction(
                cg, TR::InstOpCode::CVB, node, returnReg, generateS390MemoryReference(*ZAPtargetMR, 0, cg));
        } else {
            generateRXInstruction(
                cg, TR::InstOpCode::CVBG, node, returnReg, generateS390MemoryReference(*ZAPtargetMR, 0, cg));
        }
    ...
    cg->generateDebugCounter(
        TR::DebugCounter::debugCounterName(cg->comp(), "DAA/variable/inline/(%s)/%p", cg->comp()->signature(), node), 1,
        TR::DebugCounter::Undetermined);
...
TR::Register* J9::Z::TreeEvaluator::generateVectorPackedToBinaryConversion(
...
TR::Register* J9::Z::TreeEvaluator::generatePackedToBinaryConversion(
    TR::Node* node, TR::InstOpCode::Mnemonic op, TR::CodeGenerator* cg)
{
    ...
    sourceMR = reuseS390LeftAlignedMemoryReference(
        sourceMR, firstChild, firstStorageReference, cg, requiredSourceSize, false); // enforceSSLimits=false for CVB
 ...
 *
 */
TR::Register* J9::Z::TreeEvaluator::generateBinaryToPackedConversion(
    TR::Node* node, TR::InstOpCode::Mnemonic op, TR::CodeGenerator* cg)
{
...
TR::Register* J9::Z::TreeEvaluator::pdnegEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    bool useRegBasedSequence
        = cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_z10) && srcReg->hasKnownValidSign();
    ...
    if (!needsFullInitialization && !targetReg->isInitialized() && targetReg->getSize() > 1) {
        ...
        if (cg->traceBCDCodeGen())
            traceMsg(comp,
                "\ttargetReg is not init and size %d > 1 so gen MVC with size targetRegSize-1 = %d and leftMostByte "
                "%d\n",
        ...
        generateSS1Instruction(cg, TR::InstOpCode::MVC, node, mvcSize - 1,
            reuseS390LeftAlignedMemoryReference(
                destMR, node, targetReg->getStorageReference(), cg, targetReg->getSize()),
            reuseS390LeftAlignedMemoryReference(
                sourceMR, srcNode, srcReg->getStorageReference(), cg, targetReg->getSize()));
    }
    ...
    if (isSrcSign0xF) {
        ...
        generateRRInstruction(cg, TR::Compiler->target.is64Bit() ? TR::InstOpCode::NGR : TR::InstOpCode::NR, node,
            targetSign, targetData);
        ...
        generateRXInstruction(cg, TR::Compiler->target.is64Bit() ? TR::InstOpCode::LLGC : TR::InstOpCode::LLC, node,
            sourceSign, reuseS390LeftAlignedMemoryReference(sourceMR, srcNode, srcReg->getStorageReference(), cg, 1));
        ...
        if (cg->isLiteralPoolOnDemandOn()) {
            ...
            litPoolBaseReg = cg->allocateRegister();
            generateRRInstruction(
                cg, TR::InstOpCode::getLoadRegOpCode(), node, litPoolBaseReg, cg->getLitPoolRealRegister());
        }
                                 ...
                                 // inst is coming (e.g. AP)
{
    ...
    TR_OpaquePseudoRegister* reg = evaluateValueModifyingOperand(
        node, initTarget, sourceMR, cg, trackSignState, sourceSize, alwaysLegalToCleanSign);
                                 ...
                                 // inst is coming (e.g. AP)
{
    ...
    if (cg->traceBCDCodeGen())
        traceMsg(comp,
            "\tevaluateValueModifyingOperand for %s (%p) with targetReg %s and firstReg %s (#%d isInit %s), "
            "sourceSize=%d : initTarget=%s, alwaysLegalToCleanSign=%s\n",
    ...
    if (isInitialized && !useNewStoreHint) {
        ...
        // newly created temporary storage reference are correct). Cache the values here as this state *will* presist up
        ...
        if (cg->traceBCDCodeGen())
            traceMsg(comp,
                "\tisInitialized==true: liveSymSize %d (symSize %d - firstReg->deadAndIgnoredBytes %d), resultSize = "
                "%d (nodeSize %d)\n",
                savedLiveSymbolSize, firstStorageReference->getSymbolSize(),
                firstReg->getRightAlignedDeadAndIgnoredBytes(), resultSize, node->getSize());
        if (savedLiveSymbolSize < resultSize) {
            ...
            if (firstStorageReference->isTemporaryBased()) {
                if (cg->traceBCDCodeGen()) {
                    ...
                    traceMsg(comp,
                        ...
                        cg->getDebug()->getName(targetReg));
                    traceMsg(comp,
                        "\t\t * setting rightAlignedIgnoredBytes %d from firstReg %s to targetReg %s (valueMod "
                        "incSize)\n",
                ...
                if (cg->traceBCDCodeGen())
                    traceMsg(comp,
                        "\t\tfirstStorageReference is not temporary based and liveSymSize < resultSize (%d < %d) so "
                        "alloc and init a new temp slot and clear left most bytes\n",
                        savedLiveSymbolSize, resultSize);
            ...
            if (cg->traceBCDCodeGen()) {
                traceMsg(comp,
                    "\tliveSymSize >= resultSize (%d >= %d) so can reuse the firstStorageReference #%d for the "
                    "targetStorageReference\n",
        ...
        if (node->getOpCode().canHaveStorageReferenceHint() && node->getStorageReferenceHint()) {
            ...
            if (targetStorageReference->isTemporaryBased()) {
                ...
                // removed when the node is evaluated. At the current node's (y) initializion point only x,y will be in
                // this list and only up to size=10 will be initialized.
                destLength = targetStorageReference->getMaxSharedNodeSize();
            }
        } else {
            targetStorageReference
                = TR_StorageReference::createTemporaryBasedStorageReference(node->getStorageReferenceSize(), comp);
            if (cg->traceBCDCodeGen())
        ...
        if (destLength > 0) {
            ...
            if (cg->traceBCDCodeGen())
                traceMsg(comp,
                    "\tsetting destLength and activeSize for initialization based on the smallest remaining node left "
                    "on the temp based hint #%d : %d\n",
    ...
    if (isBCD) {
        ...
        if (node->hasKnownOrAssumedSignCode()) {
            ...
            case raw_bcd_sign_0xf:
                if (node->hasKnownOrAssumedCleanSign()) {
                    ...
                    // This is a bug and should be fixed but in a prod build conservatively reset the clean sign flag
                    // and do transfer the sign to the targetPseudoReg
            ...
            default:
                TR_ASSERT(
                    false, "unexpected node->getKnownOrAssumedSignCode() of %d\n", node->getKnownOrAssumedSignCode());
            }
        }
        ...
        if (cg->traceBCDCodeGen()) {
            traceMsg(comp, "\tsignState on targetReg %s for %s (%p) :\n", cg->getDebug()->getName(targetPseudoReg),
                node->getOpCode().getName(), node);
            traceMsg(comp,
                "\t\tknownCleanSign=%d, knownPrefSign=%d, knownSign=0x%x, assumedCleanSign=%d, assumedPrefSign=%d, "
                "assumedSign=0x%x (signStateKnown %d, signStateAssumed %d)\n",
...
TR::Register* J9::Z::TreeEvaluator::pdstoreEvaluatorHelper(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (valueStorageReference->isNonConstantNodeBased() && comp->getOption(TR_PrivatizeOverlaps) && !overlapZAPIsAllowed
        ...
        if (cg->traceBCDCodeGen())
            traceMsg(comp,
                "\tisInMemoryCopyProp=%s, isUsingStorageRefFromAnotherStore=%s, valueRegHasDeadOrIgnoredBytes=%s : "
                "node %s (%p), valueReg %s, storageRefNode %s (%p)\n",
        ...
        if (node->isInMemoryCopyProp() || isUsingStorageRefFromAnotherStore || valueRegHasDeadOrIgnoredBytes
            ...
            if (isAggr && !copyIsRedundant && !valueRegHasDeadOrIgnoredBytes && allSizesMatch) {
                ...
                if (cg->getStorageDestructiveOverlapInfo(
                        valueStorageReference->getNode(), valueReg->getSize(), node, node->getSize())
            ...
            if (!copyIsRedundant) {
                ...
                if (cg->traceBCDCodeGen()) {
                    if (node->isInMemoryCopyProp())
                        traceMsg(comp,
                            "\ta^a : privatize needed due to isInMemoryCopyProp node %s (%p) on line_no=%d "
                            "(storeCase)\n",
                            node->getOpCode().getName(), node, comp->getLineNumber(node));
                    if (isUsingStorageRefFromAnotherStore)
                        traceMsg(comp,
                            "\ta^a : privatize needed due to isUsingStorageRefFromAnotherStore storageRefNode %s (%p) "
                            "on line_no=%d (storeCase)\n",
                            storageRefNode->getOpCode().getName(), storageRefNode, comp->getLineNumber(node));
                    if (valueRegHasDeadOrIgnoredBytes)
                        traceMsg(comp,
                            "\ta^a : privatize needed due to valueRegHasDeadOrIgnoredBytes valueReg %s valueChild %s "
                            "(%p) on line_no=%d (storeCase)\n",
    ...
    TR_ASSERT(
        isBCD || (destSize == sourceSize), "destSize %d != sourceSize %d for node %p\n", destSize, sourceSize, node);
    ...
    // useZAP = useZAP || (isPacked && isByteTruncation); // truncating packed stores that need oveflow exception should
    // be using pdshlOverflow
    ...
    if (useZAP && valueChild->getReferenceCount() > 1 && preserveSrcSign) {
        ...
        if (cg->traceBCDCodeGen()) {
            traceMsg(comp,
                "\tsetting savePreZappedValue=true because valueReg (from valueChild %p with refCount %d > 1) ",
    ...
    if (!isByteTruncation && !isLeadingSignByteWidening && !savePreZappedValue && tempStorageReference == NULL
        ...
        if (cg->traceBCDCodeGen())
            traceMsg(comp,
                "\tsetting isLegalToChangeCommonedChildAddress=true for valueChild %s (%p) because "
                "isByteTruncation=false, isLeadingSignByteWidening=false, refCount %d > 1, skipCopyOnStore=true and "
                "savePreZappedValue=false\n",
    ...
    if (!valueStorageReference->isTemporaryBased() && valueStorageReference->getNode() != node) {
        TR_ASSERT(
            !valueReg->isInitialized(), "expecting valueReg to not be initialized for valueChild %p\n", valueChild);
    ...
    if (valueStorageReference->isTemporaryBased() || (valueStorageReference->getNode() != node)) {
        if (cg->traceBCDCodeGen() && valueStorageReference->isTemporaryBased())
            traceMsg(comp,
                "\tvalueStorageReference->isTemporaryBased() case so see if changeCommonedChildAddress should be set "
                "to true\n");
        else if (cg->traceBCDCodeGen())
            traceMsg(comp,
                "\tvalueStorageReference->getNode() != node (%p != %p) case so see if changeCommonedChildAddress "
        ...
        if (isLegalToChangeCommonedChildAddress) {
            if (useZAP) {
                ...
                if (cg->traceBCDCodeGen())
                    traceMsg(comp, "\t\tset changeCommonedChildAddress = true due to ZAP\n");
            } else if (isByteWidening) {
                changeCommonedChildAddress = true;
                if (cg->traceBCDCodeGen())
                    traceMsg(comp, "\t\tset changeCommonedChildAddress = true due to byteWidening\n");
            }
            /* // disable this case, not a good enough reason for potential operand store compare
                        ...
                        {
                        changeCommonedChildAddress = true;
                        if (cg->traceBCDCodeGen()) traceMsg(comp,"\t\tset changeCommonedChildAddress = true due to
               cheaper addressability\n");
                        }
            */
                ...
                if (cg->traceBCDCodeGen())
                    traceMsg(comp, "\t\tleave changeCommonedChildAddress = false\n");
            }
        } else {
            if (cg->traceBCDCodeGen())
                traceMsg(comp,
        ...
        TR_ASSERT(
            childContainsAccumulatedResult, "expecting the pdstore child node to contain the accumulated result\n");
        ...
        changeCommonedChildAddress = true;
        if (cg->traceBCDCodeGen())
            traceMsg(comp, "\taccumulated hint case so unconditionally set changeCommonedChildAddress = true\n");
    }
    ...
    TR_StorageReference* targetStorageReference = TR_StorageReference::createNodeBasedStorageReference(
        node, changeCommonedChildAddress ? valueChild->getReferenceCount() : 1, comp);
    ...
    if (useZAP) {
        ...
        if (sourceNode) {
            ...
            if (isByteWidening) {
                if (cg->traceBCDCodeGen())
                    traceMsg(comp,
                        "\t\tdestSize > sourceSize (%d > %d) so check valueReg->getLiveSymbolSize() %d against "
                        "destSize %d before checking if the upper bytes are clear\n",
            ...
            if (savePreZappedValue) {
                ...
                TR_ASSERT(
                    tempStorageReference == NULL, "tempStorageReference == NULL should be null for node %p\n", node);
                ...
                if (cg->traceBCDCodeGen())
                    traceMsg(comp,
                        "\tsavePreZappedValue=true so gen MVC with sourceSize %d to copy #%d on pdstore for valueChild "
                        "%p with refCnt %d\n",
                ...
                if (savedRightAlignedDeadAndIgnoredBytes > 0) {
                    if (cg->traceBCDCodeGen())
                        traceMsg(comp,
                            "\tadd -savedRightAlignedDeadAndIgnoredBytes = -%d to sourceMR for savePreZappedValue "
                            "copy\n",
            ...
            sourceMR = generateS390RightAlignedMemoryReference(
                *targetMR, node, 0, cg); // ensure sourceMR and targetMR are the same when used for the ZAP below
        ...
        if (sourceNode) {
            ...
            if (isByteWidening) {
                ...
                if (cg->traceBCDCodeGen())
                    traceMsg(comp,
                        "\t\tdestSize > sourceSize (%d > %d) so try to reduce mvcSize by checking if the upper bytes "
                        "are clear\n",
                        destSize, sourceSize, valueReg->getLiveSymbolSize(), destSize);
                if (valueReg->getBytesToClear(sourceSize, destSize) == 0) {
                    needsClear = false;
            ...
            if (cg->traceBCDCodeGen())
                traceMsg(
                    comp, "\tuseZAP=false and sourceNode is NULL so just check if upper bytes need to be cleared\n");
            targetMR = generateS390RightAlignedMemoryReference(node, targetStorageReference, cg);
    ...
    if (valueChild->getReferenceCount() > 1) {
        if (changeCommonedChildAddress) {
            ...
            if (useZAP) {
                ...
                if (cg->traceBCDCodeGen())
                    traceMsg(comp, "\t\tsetting HasKnownCleanSign (due to ZAP) on valueReg %s on valueChild %p\n",
                   ...
                   (!node->skipCopyOnStore() || isLeadingSignByteWidening))) // comments 2 and 3 below
        {
            ...
            if (cg->traceBCDCodeGen())
                traceMsg(comp,
                    "\tlate pdstore privatization of valueChild : so gen MVC/memcpy with sourceSize %d to copy #%d "
                    "(%s) on %s for child %s (%p) with refCnt %d (mustPrivatizeValueChild %s)\n",
            ...
            if (childContainsAccumulatedResult && isLeadingSignByteWidening) {
                ...
                TR::MemoryReference* copyMR = reuseS390LeftAlignedMemoryReference(
                    copyTargetMR, valueChild, valueStorageReferenceCopy, cg, sourceSize);
    ...
    if (changeCommonedChildAddress && finalValueChildRefCount != origValueChildRefCount) {
        ...
        if (isIndirect && finalValueChildRefCount == 1) {
            ...
            if (cg->traceBCDCodeGen())
                traceMsg(comp,
                    "\tfinalValueChildRefCount < origValueChildRefCount (%d < %d) and is 1 so recursively dec "
                    "addrChild %s (%p) %d->%d\n",
    ...
    if (targetMR == NULL) {
        if (isIndirect) {
            ...
            if (!changeCommonedChildAddress) {
                ...
                if (cg->traceBCDCodeGen())
                    traceMsg(comp,
                        "\tno explicit store inst and changeCommonedChildAddress=true so do NOT recursively dec "
                        "addrChild %p (refCount stays at %d)\n",
                        node->getFirstChild(), node->getFirstChild()->getReferenceCount());
            }
        }
        if (cg->traceBCDCodeGen())
    ...
    bool isNondestructiveNop, bool initTarget, TR::MemoryReference* sourceMR, TR::CodeGenerator* cg)
{
    ...
    if (cg->traceBCDCodeGen()) {
        if (isBCD)
            traceMsg(comp,
                "\tevaluateSignModOperand %s (%p) : firstReg %s firstReg->getPseudoRegister()->prec %d (isInit %s, "
                "isLegalToCleanSign %s, isEffectiveNop %s, initTarget %s)\n",
            ...
            traceMsg(comp,
                "\tevaluateSignModOperand for aggr type %s (%p) : firstReg %s (isInit %s, isEffectiveNop %s, "
                "initTarget %s)\n",
                node->getOpCode().getName(), node, cg->getDebug()->getName(firstReg),
    ...
    if (isEffectiveNop) {
        ...
        if (firstReg->isInitialized()) {
            ...
            if (!comp->getOption(TR_DisableRefinedBCDClobberEval) && firstStorageReference->isTemporaryBased()
                ...
                if (cg->traceBCDCodeGen())
                    traceMsg(comp,
                        "%sskipping ssrClobberEvaluate for %s (%p) with child %s (%p) refCount %d %s 1 "
                        "owningRegisterCount %d %s 1-- %s mark #%d (%s) as readOnlyTemp (nondestructive nop case)\n",
        ...
        if (cg->traceBCDCodeGen()) {
            traceMsg(comp, "\t * setting rightAlignedDeadBytes %d from firstReg %s to targetReg %s (signMod isInit)\n",
                ...
                cg->getDebug()->getName(targetReg));
            traceMsg(comp,
                "\t * setting rightAlignedIgnoredBytes %d from firstReg %s to targetReg %s (signMod isInit)\n",
                firstReg->getRightAlignedIgnoredBytes(), cg->getDebug()->getName(firstReg),
                cg->getDebug()->getName(targetReg));
            traceMsg(comp,
        ...
        if (!comp->getOption(TR_DisableRefinedBCDClobberEval) && firstReg->canBeConservativelyClobberedBy(node)) {
            ...
            if (cg->traceBCDCodeGen())
                traceMsg(comp,
                    ...
                    "%d %s 1-- %s mark #%d (%s) as readOnlyTemp (isInit case)\n",
                    child->getReferenceCount() > 1 ? "y^y : " : "", node->getOpCode().getName(), node,
                    child->getOpCode().getName(), child, child->getReferenceCount(),
                    child->getReferenceCount() > 1 ? ">" : "<=", firstStorageReference->getOwningRegisterCount(),
                    firstStorageReference->getOwningRegisterCount() > 1 ? ">" : "<=",
    ...
    if (cg->traceBCDCodeGen() && targetReg->getStorageReference()->isReadOnlyTemporary())
        traceMsg(comp, "%sreset readOnlyTemp flag on storageRef #%d (%s) (signMod case)\n",
            resetReadOnly ? "" : "do not ", targetReg->getStorageReference()->getReferenceNumber(),
            cg->getDebug()->getName(targetReg->getStorageReference()->getSymbol()));
...
TR::Register* J9::Z::TreeEvaluator::pdSetSignHelper(TR::Node* node, int32_t sign, TR::CodeGenerator* cg)
{
    ...
    if (node->getType().isAnyPacked()) {
        ...
        if (!srcReg->isInitialized() && !isEffectiveNop)
            sourceMR = generateS390RightAlignedMemoryReference(srcNode, srcReg->getStorageReference(), cg);
        targetReg = evaluateBCDSignModifyingOperand(
            node, isEffectiveNop, isEffectiveNop, true, sourceMR, cg); // initTarget=true
        bool isTruncation = srcReg->getDecimalPrecision() > node->getDecimalPrecision();
        if (isTruncation)
            targetReg->setDecimalPrecision(node->getDecimalPrecision());
        else
...
TR::Register* J9::Z::TreeEvaluator::pdSetSignEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.cpu.getS390SupportsVectorPackedDecimalFacility()
        ...
        targetReg = vectorPerformSignOperationHelper(node, cg,
            ...
            0, node->hasKnownOrAssumedCleanSign(), SignOperationType::setSign,
...
TR::Register* J9::Z::TreeEvaluator::pdclearEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (isEven(leftMostDigit)) {
        if (cg->traceBCDCodeGen())
            traceMsg(comp,
                "\tleftMostDigit %d isEven : isInit=%s, truncatedIntoClearedDigits=%s, "
                "coincidentEvenDigitCorrection=%s -- adjust the leftMostNibble to preserve or clear the leftMostByte\n",
                                               ...
                                               // coincidentEvenDigitCorrection are true
        {
            ...
            if (cg->traceBCDCodeGen())
                traceMsg(comp,
                    "\t\tisInit=no or truncatedIntoClearedDigits=yes or coincidentEvenDigitCorrection=yes so inc "
                    "%d->%d to clear initialized leftMostNibble\n",
                    digitsToClear, digitsToClear + 1);
            ...
            if (isInitialized) {
                {
                    ...
                    generateSIInstruction(cg, TR::InstOpCode::NI, node,
                        reuseS390LeftAlignedMemoryReference(
                            destMR, node, targetReg->getStorageReference(), cg, leftMostByte),
                        0xF0);
                }
            } else {
                if (cg->traceBCDCodeGen())
...
TR::Register* J9::Z::TreeEvaluator::pdchkEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    generateRRInstruction(cg, TR::Compiler->target.is64Bit() ? TR::InstOpCode::XGR : TR::InstOpCode::XR, node,
        chkResultReg, chkResultReg);
    ...
    if (TR::Compiler->target.cpu.getS390SupportsVectorPackedDecimalFacility()
        ...
        generateRSLInstruction(
            cg, TR::InstOpCode::TP, pdloadNode, static_cast<TR_PseudoRegister*>(pdReg)->getSize() - 1, tempMR);
    }
 ...
 */
TR::Register* J9::Z::TreeEvaluator::pdArithmeticVectorEvaluatorHelper(
    TR::Node* node, TR::InstOpCode::Mnemonic op, TR::CodeGenerator* cg)
{
 ...
 * Handles pdadd,pdsub
 */
TR::Register* J9::Z::TreeEvaluator::pdaddsubEvaluatorHelper(
    TR::Node* node, TR::InstOpCode::Mnemonic op, TR::CodeGenerator* cg)
{
    ...
    TR_PseudoRegister* targetReg = evaluateBCDValueModifyingOperand(
        node, true, NULL, cg, trackSignState, 0, alwaysLegalToCleanSign); // initTarget=true, sourceMR=NULL, srcSize=0
    ...
    if (mayOverflow) {
        ...
        if (cg->traceBCDCodeGen()) {
            if (firstChild->isZero() || secondChild->isZero())
                traceMsg(comp,
                    "\t%s firstChild %p isZero=%s or secondChild %p isZero=%s so nibble clearing is NOT required and "
                    "set HasKnownCleanSign = true on reg %s\n",
                    isAdd ? "add" : "sub", firstChild, firstChild->isZero() ? "yes" : "no", secondChild,
                    secondChild->isZero() ? "yes" : "no", cg->getDebug()->getName(targetReg));
            else
                traceMsg(comp,
                    "\t%s result prec %d is > both reg1 prec %d and reg2 prec %d so nibble clearing is NOT required "
                    "and set HasKnownCleanSign = true on reg %s\n",
    ...
    if (isAdd && firstReg->hasKnownOrAssumedPositiveSignCode() && secondReg->hasKnownOrAssumedPositiveSignCode()) {
        if (cg->traceBCDCodeGen())
            traceMsg(comp,
                "\tfirstReg and secondReg have positive sign codes so set targetReg sign code to the preferred "
                "positive sign 0x%x\n",
                TR::DataType::getPreferredPlusCode());
...
TR::Register* J9::Z::TreeEvaluator::pdmulEvaluatorHelper(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR_ASSERT(
        targetReg->getSize() >= firstReg->getSize() + secondReg->getSize(), "MP may result in a data exception\n");
...
TR::Register* J9::Z::TreeEvaluator::pddivremEvaluatorHelper(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (isRem) {
        ...
        if (cg->traceBCDCodeGen()) {
            traceMsg(comp, "\tisDiv=true (pddivrem) : increment targetReg %s deadBytes %d -> %d (by the divisorSize)\n",
                ...
                targetReg->getRightAlignedDeadBytes());
            traceMsg(comp,
                "\tsetting targetReg prec to min(computedQuotPrec, nodePrec) = min(%d, %d) = %d (size %d), "
                "isTruncation=%s\n",
    ...
    if (!node->canSkipPadByteClearing() && targetReg->isEvenPrecision() && isTruncation) {
        TR_ASSERT(
            node->getStorageReferenceSize() >= dividendSize, "operand size should only shrink from original size\n");
        ...
        cg->genZeroLeftMostPackedDigits(
            node, targetReg, leftMostByte, 1, generateS390RightAlignedMemoryReference(*destMR, node, -deadBytes, cg));
    }
    ...
    if (isRem) {
        ...
        if (firstReg->hasKnownOrAssumedSignCode()) {
            ...
            if (cg->traceBCDCodeGen())
                traceMsg(comp,
                    "\tpdrem: firstReg has the knownSignCode 0x%x so set targetReg sign code to the preferred sign "
                    "0x%x\n",
        ...
        if (firstReg->hasKnownOrAssumedSignCode() && secondReg->hasKnownOrAssumedSignCode()) {
            ...
            if ((dividendSignIsPositive && divisorSignIsPositive)
                ...
                if (cg->traceBCDCodeGen())
                    traceMsg(comp,
                        "\tpddiv: dividendSign matches the divisorSign so set targetReg sign code to the preferred "
                        "sign 0x%x\n",
                        TR::DataType::getPreferredPlusCode());
            } else {
                ...
                if (cg->traceBCDCodeGen())
                    traceMsg(comp,
                        "\tpddiv: dividendSign does not match the divisorSign so set targetReg sign code to the "
                        "preferred sign 0x%x\n",
                        TR::DataType::getPreferredMinusCode());
            }
        }
    ...
    bool isSetSign, int32_t sign, bool signCodeIsInitialized, TR::CodeGenerator* cg)
{
    ...
    if (isSetSign) {
        ...
        if (clearingNeeded) {
            ...
            if (digitsToClear > 0 && (digitsToClear & 0x1) && sign == TR::DataType::getIgnoredSignCode()) {
                ...
                if (cg->traceBCDCodeGen())
                    traceMsg(comp,
                        "\tignored setSign case so inc digitsToClear %d->%d and setHasKnownBadSignCode=true on "
                        "targetReg %s\n",
        ...
        if (cg->traceBCDCodeGen()) {
            if (clearingNeeded)
                traceMsg(comp,
                    "\t\tisSetSign case (clearingNeeded==true): sign setting cleared %d digits so adjust digitsToClear "
                    "%d->%d\n",
    ...
    if (digitsToClear > 0) {
        if (cg->traceBCDCodeGen())
            traceMsg(comp, "\t\tdigitsToClear %d > 0 so call genClearLeftMostDigitsIfNeeded\n", digitsToClear);
    ...
    if (!signCodeIsInitialized) {
        if (cg->traceBCDCodeGen())
            traceMsg(comp,
                "\t\tsignCodeIsInitialized=false after clearing of %d digits : init the sign now with an MVN of size "
                "1\n",
...
TR_PseudoRegister* J9::Z::TreeEvaluator::simpleWideningOrTruncation(
    TR::Node* node, TR_PseudoRegister* srcReg, bool isSetSign, int32_t sign, TR::CodeGenerator* cg)
{
    ...
    if (!isDigitTruncation && srcReg->isEvenPrecision() && !srcReg->isLeftMostNibbleClear()) {
                                             ...
                                             // !canSkipPadByteClearing alone is not valid
        {
            ...
            needsTopNibbleClearing = true;
            if (cg->traceBCDCodeGen())
                traceMsg(comp, "z^z : new clear : simpleWide %p\n", node);
        }
    }
    ...
    if (cg->traceBCDCodeGen())
        traceMsg(comp,
            "\tisDigitTruncation=%s, srcPrecision=%d, isPassThrough=%s, needsTopNibbleClearing=%s, "
            "initTargetAndSign=%s\n",
    ...
    if (initTargetAndSign || needsTopNibbleClearing)
        ...
        targetReg = evaluateBCDSignModifyingOperand(
            node, isPassThrough, isNondestructiveNop, false, sourceMR, cg); // initTarget=false
    ...
    if (!isInitialized && !isPassThrough) {
        ...
        if (cg->traceBCDCodeGen())
            traceMsg(comp,
                "\tisInit=false and isPassThru=false so gen initializing MVC with size %d. Do not clear after MVC just "
                "set targetReg->prec to srcPrecision %d\n",
    ...
    if (needsTopNibbleClearing) {
        if (cg->traceBCDCodeGen())
            traceMsg(comp, "\tisDigitTruncation=true and targetReg->isEvenPrecision() (%d) so clear top nibble\n",
                targetReg->isEvenPrecision());
        int32_t leftMostByteForClear = TR::DataType::packedDecimalPrecisionToByteLength(srcPrecision);
        cg->genZeroLeftMostPackedDigits(
            node, targetReg, leftMostByteForClear, 1, generateS390RightAlignedMemoryReference(*destMR, node, 0, cg));
    }
    ...
    if (isSimpleCopy) {
        ...
        if (cg->traceBCDCodeGen()) {
            traceMsg(comp, "\tGen MVC + SRP + ZAP for shift: %s %p : shift by %d, roundAmount=%d, \
                ...
                node->getOpCode().getName(), node, shiftAmount, roundAmount, isNeedExtraShift, resultSize,
                resultPrecision, sourceSize, srcNode->getDecimalPrecision());
        }

        TR_StorageReference* tmpStorageRef
            = TR_StorageReference::createTemporaryBasedStorageReference(sourceSize, comp);
...
TR::Register* J9::Z::TreeEvaluator::vectorPerformSignOperationHelper(TR::Node* node, TR::CodeGenerator* cg,
    ...
    int32_t sign, bool setConditionCode)
{
    TR::Register* targetReg = NULL;
    ...
    // todo: we can probably come up with more complex optimization that will collapse modify precision and
    // setsign/pdclean to one instruction.
    generateVRIgInstruction(
        cg, TR::InstOpCode::VPSOP, node, targetReg, childReg, numPrecisionDigits, constImm4, setConditionCode);
    node->setRegister(targetReg);
    cg->decReferenceCount(pdNode);
    return targetReg;
}

TR::Register* J9::Z::TreeEvaluator::generateVectorBinaryToPackedConversion(
    TR::Node* node, TR::InstOpCode::Mnemonic op, TR::CodeGenerator* cg)
{
    ...
    if (isUseRegPair) {
        TR::Register* tempReg = cg->allocate64bitRegister();
        generateRSInstruction(
            cg, TR::InstOpCode::SLLG, node, tempReg, sourceReg->getRegisterPair()->getHighOrder(), 32);
...
TR::Register* J9::Z::TreeEvaluator::pdshlVectorEvaluatorHelper(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (isSkipShift) {
        ...
        targetReg = cg->allocateRegister(TR_VRF);
        generateVRIgInstruction(
            cg, TR::InstOpCode::VSRP, node, targetReg, sourceReg, decimalPrecision, shiftAmount, mask5);
    }
...
TR::Register* J9::Z::TreeEvaluator::pdshrVectorEvaluatorHelper(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    generateVRIgInstruction(
        cg, TR::InstOpCode::VSRP, node, targetReg, pdValueReg, node->getDecimalPrecision(), shiftAmount, 0x1);
