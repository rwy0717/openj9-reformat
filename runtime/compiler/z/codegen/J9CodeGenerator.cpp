    ...
    : J9::CodeGenerator()
{
    ...
    cg->setSupportsImplicitNullChecks(TR::Compiler->target.isLinux() && cg->getHasResumableTrapHandler()
    ...
    const bool accessStaticsIndirectly = !cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_z10)
        || comp->getOption(TR_DisableDirectStaticAccessOnZ) || comp->compileRelocatableCode();
...
void J9::Z::CodeGenerator::lowerTreesPreChildrenVisit(TR::Node* parent, TR::TreeTop* treeTop, vcount_t visitCount)
{
    ...
    if (parent != NULL && parent->getOpCode().isConversion() && parent->getType().isAnyZoned()
        && parent->getFirstChild()->getType().isDFP() && parent->isTruncating()) {
        TR::Node* ddMod
            = TR::Node::create(parent, TR::ILOpCode::modifyPrecisionOpCode(parent->getFirstChild()->getDataType()), 1);
    ...
    if (self()->codegenSupportsLoadlessBNDCheck() && parent->getOpCode().isBndCheck()
        ...
        if (memChild->getVisitCount() != self()->comp()->getVisitCount() && memChild->getReferenceCount() > 1
            && performTransformation(
                   self()->comp(), "%sRematerializing memref child %p from BNDCheck node\n", OPT_DETAILS, memChild)) {
...
void J9::Z::CodeGenerator::lowerTreeIfNeeded(
    ...
    if (self()->comp()->cg()->yankIndexScalingOp()
        ...
        if (tt->getPrevTreeTop() && tt->getNode()->getOpCodeValue() == TR::ArrayStoreCHK
            ...
            if ((mulNode->getOpCodeValue() == TR::imul || mulNode->getOpCodeValue() == TR::ishl
                    || mulNode->getOpCodeValue() == TR::lmul || mulNode->getOpCodeValue() == TR::lshl)
                && (performTransformation(
                       self()->comp(), "%sYank mul above ArrayStoreChk [%p] \n", OPT_DETAILS, node))) {
 ...
 */
void J9::Z::CodeGenerator::widenUnicodeSignLeadingSeparate(
    TR::Node* node, TR_PseudoRegister* reg, int32_t endByte, int32_t bytesToClear, TR::MemoryReference* targetMR)
{
    ...
    if (bytesToClear > 0) {
        ...
        if (self()->traceBCDCodeGen())
            traceMsg(self()->comp(),
                "\twidenUnicodeSignLeadingSeparate: node %p, reg %s targetStorageRef #%d, endByte %d, bytesToClear "
                "%d\n",
        ...
        self()->genZeroLeftMostUnicodeBytes(
            node, reg, endByte - TR::DataType::getUnicodeSignSize(), bytesToClear, targetMR);
    }
}
 ...
 * Input reg can be NULL (when called for a store node or other type that does not return a register)
 */
void J9::Z::CodeGenerator::genZeroLeftMostUnicodeBytes(
    TR::Node* node, TR_PseudoRegister* reg, int32_t endByte, int32_t bytesToClear, TR::MemoryReference* targetMR)
{
    TR_ASSERT(node->getType().isAnyUnicode(),
    ...
    if (bytesToClear > 0) {
        ...
        TR::MemoryReference* tempMR = generateS390LeftAlignedMemoryReference(
            node, tempStorageReference, self(), tempSize, true, true); // enforceSSLimits=true, isNewTemp=true
 ...
 * Input reg can be NULL (when called for a store node or other type that does not return a register)
 */
void J9::Z::CodeGenerator::widenZonedSignLeadingSeparate(
    TR::Node* node, TR_PseudoRegister* reg, int32_t endByte, int32_t bytesToClear, TR::MemoryReference* targetMR)
{
    ...
    if (bytesToClear > 0) {
        ...
        if (self()->traceBCDCodeGen())
            traceMsg(self()->comp(),
                "\tgen MVC of size 1 to move zoned leading separate sign code left by %d bytes to the widened left "
                "aligned position\n",
                bytesToClear);
        TR::MemoryReference* originalSignCodeMR
            = generateS390LeftAlignedMemoryReference(*targetMR, node, bytesToClear, self(), endByte);
        int32_t mvcSize = 1;
        generateSS1Instruction(self(), TR::InstOpCode::MVC, node, mvcSize - 1, targetMR, originalSignCodeMR);
        self()->genZeroLeftMostZonedBytes(
            node, reg, endByte - TR::DataType::getZonedSignSize(), bytesToClear, targetMR);
    }
}
 ...
 * Input reg can be NULL (when called for a store node or other type that does not return a register)
 */
void J9::Z::CodeGenerator::widenZonedSignLeadingEmbedded(
    TR::Node* node, TR_PseudoRegister* reg, int32_t endByte, int32_t bytesToClear, TR::MemoryReference* targetMR)
{
    ...
    if (bytesToClear > 0) {
        ...
        generateSS1Instruction(self(), TR::InstOpCode::MVZ, node, mvzSize - 1, targetMR,
            generateS390LeftAlignedMemoryReference(
                *originalSignCodeMR, node, 0, self(), originalSignCodeMR->getLeftMostByte()));
        {
            if (self()->traceBCDCodeGen())
                traceMsg(self()->comp(),
                    "\tgenerate OI 0xF0 to force original leading sign code at offset=bytesToClear=%d\n", bytesToClear);
            generateSIInstruction(self(), TR::InstOpCode::OI, node, originalSignCodeMR, TR::DataType::getZonedCode());
        }
    }
}

void J9::Z::CodeGenerator::genZeroLeftMostZonedBytes(
    TR::Node* node, TR_PseudoRegister* reg, int32_t endByte, int32_t bytesToClear, TR::MemoryReference* targetMR)
{
...
TR_OpaquePseudoRegister* J9::Z::CodeGenerator::evaluateOPRNode(TR::Node* node)
{
    ...
    TR_ASSERT(
        isBCD || isAggr, "evaluateOPRNode node %s (%p) must be BCD/Aggr type\n", node->getOpCode().getName(), node);
    ...
    if (hint && hint->isTemporaryBased() && hint->getTemporaryReferenceCount() == 0) {
        ...
        if (self()->traceBCDCodeGen())
            traceMsg(self()->comp(), "\tfreeing (pending) unused hint symRef #%d (%s) on %s (%p)\n",
                hint->getReferenceNumber(), self()->getDebug()->getName(hint->getTemporarySymbol()),
                node->getOpCode().getName(), node);
    }
}
...
bool J9::Z::CodeGenerator::storageReferencesMatch(TR_StorageReference* ref1, TR_StorageReference* ref2)
{
    ...
    if (ref1->isNodeBased() && (ref1->getNode()->getOpCode().isLoadVar() || ref1->getNode()->getOpCode().isStore())
        ...
        if (ref1->getNode()->getSize() != ref2->getNode()->getSize()) {
            if (self()->traceBCDCodeGen())
                traceMsg(self()->comp(),
                    "\tnode based storageRefs match = false : ref1 (#%d) and ref2 (#%d) addresses match but node1 %s "
                    "(%p) size=%d != node2 %s (%p) size=%d\n",
                    ref1->getReferenceNumber(), ref2->getReferenceNumber(), ref1->getNode()->getOpCode().getName(),
                    ref1->getNode(), ref1->getNode()->getSize(), ref2->getNode()->getOpCode().getName(),
                    ref2->getNode(), ref2->getNode()->getSize());
            refMatch = false;
        } else {
            if (self()->traceBCDCodeGen())
                traceMsg(self()->comp(),
                    "\tnode based storageRefs match = true : ref1 (#%d) %s (%p) == ref2 (#%d) %s (%p)\n",
    ...
    bool justLookForConflictingAddressNodes, bool isChainOfFirstChildren, bool mustCheckAllNodes)
{
    ...
    if (self()->traceBCDCodeGen())
        traceMsg(self()->comp(), "\t\texamining node %s (%p) (usage/budget = %d/%d)\n", node->getOpCode().getName(),
            node, self()->getAccumulatorNodeUsage(), TR_ACCUMULATOR_NODE_BUDGET);
        ...
        if (node->getOpaquePseudoRegister()) {
            ...
            if (storageRef->isTemporaryBased()) {
                ...
                if (_dummyTempStorageRefNode == NULL) {
                    _dummyTempStorageRefNode = TR::Node::createWithSymRef(
                        node, self()->comp()->il.opCodeForDirectLoad(node->getDataType()), 0, tempSymRef);
                } else {
                    TR::Node::recreate(
                        _dummyTempStorageRefNode, self()->comp()->il.opCodeForDirectLoad(node->getDataType()));
                ...
                if (self()->traceBCDCodeGen())
                    traceMsg(self()->comp(), "(nodeBased storageRefNode %s (%p))\n",
                ...
                if (self()->traceBCDCodeGen())
                    traceMsg(self()->comp(), "(constNodeBased storageRefNode %s (%p))\n",
                        storageRef->getNode()->getOpCode().getName(), storageRef->getNode());
    ...
    if (self()->traceBCDCodeGen() && nodeForAliasing && symRefForAliasing)
        traceMsg(self()->comp(), "\t\tgot nodeForAliasing %s (%p), symRefForAliasing #%d\n",
            nodeForAliasing->getOpCode().getName(), nodeForAliasing,
            symRefForAliasing ? symRefForAliasing->getReferenceNumber() : -1);

    bool useAliasing = true;
    if (self()->traceBCDCodeGen() && useAliasing && !storeAliases.isZero(self()->comp()) && symRefForAliasing) {
        if (self()->comp()->getOption(TR_TraceAliases)
            && !symRefForAliasing->getUseDefAliases().isZero(self()->comp())) {
        ...
        traceMsg(self()->comp(), "\t\t\tsymRefForAliasing #%d isSet in storeAliases = %s\n",
            ...
            storeAliases.contains(symRefForAliasing->getReferenceNumber(), self()->comp()) ? "yes" : "no");
    }
               ...
               storeAliases)) // if aliases are present node can be of any node type (a call for example)
    {
        ...
        if (parent && nodeForAliasing->getOpCode().isLoadVar() && (parent->getOpCode().isBasicPackedArithmetic())
            ...
            if (self()->traceBCDCodeGen())
                traceMsg(self()->comp(),
                    "\t\t\tallow hint (loadVar case) %s (%p) -- store %s #%d (%p) location = nodeForAliasing %s #%d "
                    "(%p) location\n",
            ...
            if (self()->traceBCDCodeGen())
                traceMsg(self()->comp(),
                    "\t\t\tallow hint (storeVar case) %s (%p) -- store %s #%d (%p) location = nodeForAliasing %s #%d "
                    "(%p) location\n",
                    parent->getOpCode().getName(), parent, store->getOpCode().getName(),
                    store->getSymbolReference()->getReferenceNumber(), store, nodeForAliasing->getOpCode().getName(),
                    symRefForAliasing->getReferenceNumber(), nodeForAliasing);
        ...
        // where the store is to the leading bytes of the load.  See RTC 95073
            ...
            if (self()->traceBCDCodeGen())
                traceMsg(self()->comp(),
                    "\t\t\tallow hint (pdshr in place case) %s (%p) -- store %s #%d (%p) location = nodeForAliasing %s "
                    "#%d (%p) location\n",
                    parent->getOpCode().getName(), parent, store->getOpCode().getName(),
                    store->getSymbolReference()->getReferenceNumber(), store, nodeForAliasing->getOpCode().getName(),
                    symRefForAliasing->getReferenceNumber(), nodeForAliasing);
            ...
            if (self()->traceBCDCodeGen())
                traceMsg(self()->comp(),
                    "\t\t\tallow hint (pdMod in place case) %s (%p) -- store %s #%d (%p) location = nodeForAliasing %s "
                    "#%d (%p) location\n",
                    parent->getOpCode().getName(), parent, store->getOpCode().getName(),
                    store->getSymbolReference()->getReferenceNumber(), store, nodeForAliasing->getOpCode().getName(),
                    symRefForAliasing->getReferenceNumber(), nodeForAliasing);
    ...
    if (node->getOpCode().isLoad()) {
        ...
        if (self()->traceBCDCodeGen())
            traceMsg(self()->comp(),
                "\t\t\tallow -- found base case evaluated reg %s on node %s (%p) under store %s (%p)\n",
    ...
    for (int32_t i = node->getNumChildren() - 1; i >= 0; --i) // recurse from original node and not nodeForAliasing
    {
        ...
        if (self()->foundConflictingNode(child, conflictingAddressNodes)) {
            ...
            if (self()->traceBCDCodeGen())
                traceMsg(self()->comp(),
                    "\t\t\ta^a: disallow -- useAccum=false because node %s (%p) was found commoned from address tree "
                    "on %s (%p)\n",
...
{
    TR_ASSERT(
        addr1->getType().isAddress(), "addr1 %s (%p) must an address type\n", addr1->getOpCode().getName(), addr1);
    TR_ASSERT(
        addr2->getType().isAddress(), "addr2 %s (%p) must an address type\n", addr2->getOpCode().getName(), addr2);
    ...
    if (!canGetOffset) {
        if (trace)
            traceMsg(self()->comp(),
                "\tvalidateAddressOneToAddressOffset = false : could not compute offset between addr1 %s (%p) (+%lld) "
                "and addr2 %s (%p) (+%lld)\n",
    ...
    if (addrOffset != expectedOffset) {
        if (trace)
            traceMsg(self()->comp(),
                "\tvalidateAddressOneToAddressOffset = false : addrOffset %d not the expected value of %d between "
    ...
    bool trace) // _baseLoadsThatAreNotKilled can be NULL
{
    ...
    *offset = 0;
    ...
    if (!foundOffset && self()->comp()->cg()->addressesMatch(addr1, addr2)) {
        foundOffset = true;
    ...
    if (!foundOffset && self()->comp()->cg()->isSupportedAdd(addr1) && self()->comp()->cg()->isSupportedAdd(addr2)
    ...
    //    iconst 8
    //
    if (!foundOffset && self()->comp()->cg()->isSupportedAdd(addr2)
        && addr2->getSecondChild()->getOpCode().isIntegralConst()
    ...
    if (!foundOffset && _baseLoadsThatAreNotKilled && !_baseLoadsThatAreNotKilled->empty()
        && self()->comp()->cg()->isSupportedAdd(addr1) && self()->comp()->cg()->isSupportedAdd(addr2)
...
// Z
void J9::Z::CodeGenerator::examineNode(
    TR::Node* parent, TR::Node* node, TR::Node*& bestNode, int32_t& storeSize, TR::list<TR::Node*>& leftMostNodesList)
{
    ...
    if (!node->hasBeenVisitedForHints()) // check other nodes using hasBeenVisitedForHints
    {
        ...
        if (nodeCanHaveHint && bestNode && node->getStorageReferenceSize() > bestNode->getStorageReferenceSize()
            && // end hint before
        ...
        if (isInterestingStore) {
            ...
            if ((bestNode == NULL) || (node->getStorageReferenceSize() > bestNode->getStorageReferenceSize())
                || (self()->nodeRequiresATemporary(node) && bestNode->getOpCode().isStore()
                       && !self()->isAcceptableDestructivePDShiftRight(
                              bestNode, NULL /* let the function find the load node */))) {
                if (!isAccumStore || node->useStoreAsAnAccumulator()) {
                    bestNode = node;
                    if (self()->traceBCDCodeGen()) {
                        if (isAccumStore)
                            traceMsg(self()->comp(),
                                "\t\tfound new store (canUse = %s) bestNode - %s (%p) with actual size %d and "
                                "storageRefResultSize %d\n",
                                bestNode->useStoreAsAnAccumulator() ? "yes" : "no", bestNode->getOpCode().getName(),
                                bestNode, bestNode->getSize(), bestNode->getStorageReferenceSize());
                        else
                            traceMsg(self()->comp(),
                                "\t\tfound new non-store bestNode - %s (%p) (isConversionToNonAggrOrNonBCD=%s, "
                                ...
                                bestNode->getStorageReferenceSize());
                    }
                }
            }
        ...
        if (endHintOnNode) {
            ...
            case TR::pd2udst:
                storeSize = TR::DataType::packedDecimalPrecisionToByteLength(
                    node->getDecimalPrecision()); // i.e. the size of the result in packed bytes (node->getSize is in ud
                                                  // bytes)
                break;
            default:
                break;
        ...
        if (reg) {
            if (self()->traceBCDCodeGen())
                traceMsg(self()->comp(),
                    "\t\tnode - %s (%p) with reg %s is an already evaluated bcd node with refCount=%d\n",
            ...
            if (!reg->getStorageReference()->isTemporaryBased()) {
                if (self()->traceBCDCodeGen())
                    traceMsg(self()->comp(),
                        "\t\t\treg storageRef #%d is not a temp so do not update bestNode with node - %s (%p) but do "
                        "reset reg %s isInit to false\n",
                ...
                && node->getReferenceCount()
                    >= 1) // use >= 1 so useNewStoreHint can always be used for ZAP widening on intializations
            {
                if (self()->traceBCDCodeGen())
                    traceMsg(self()->comp(),
    ...
    if ((leftMostNodesList.empty())
        || (checkNode == leftMostNodesList.front())) // just finished with a left most path and there are nodes to tag
                                                     // with hint just finished with a left most but there are no nodes
                                                     // to tag with a hint
    {
        if (self()->traceBCDCodeGen()) {
        ...
        if (leftMostNodesList.empty()) {
            ...
            if (self()->traceBCDCodeGen())
                traceMsg(self()->comp(),
                    "\tcalling processNodeList with bestNode - %s (%p) because leftMostNodesList is not empty for "
                    "current node - %s (%p)\n",
                    bestNode ? bestNode->getOpCode().getName() : "NULL", bestNode, node->getOpCode().getName(), node);
            self()->processNodeList(bestNode, storeSize, leftMostNodesList);
        }
...
// Z
void J9::Z::CodeGenerator::processNodeList(
    TR::Node*& bestNode, int32_t& storeSize, TR::list<TR::Node*>& leftMostNodesList)
{
    if (bestNode) {
        ...
        if (bestNode->getOpaquePseudoRegister()) {
            ...
            if (reg->getStorageReference()->isTemporaryBased() && storeSize > reg->getLiveSymbolSize()) {
                if (self()->traceBCDCodeGen())
                    traceMsg(self()->comp(),
                        "\t\treg->getStorageReference #%d is tempBased and requested storeSize %d > regLiveSymSize %d "
                        "so increase tempSize\n",
            ...
            if (!leftMostNodesList.empty()) {
                ...
                if (self()->traceBCDCodeGen()) {
                    traceMsg(self()->comp(),
                        "\t\tbestNode - %s (%p) is a BCD arithmetic or conversion op (isBCDToNonBCDConversion %s) and "
                        "list is not empty so allocate a new temporary based storage reference\n",
                ...
                if (tempSize == bestNodeSize) {
                    ...
                    if (self()->traceBCDCodeGen())
                        traceMsg(self()->comp(),
                            "\t\tsetting keepTrackOfSharedNodes=true because hintSize is based on a non-store "
                            "operation (bestNode %s - %p)\n",
                            bestNode->getOpCode().getName(), bestNode);
                }
            } else if (self()->traceBCDCodeGen()) {
                traceMsg(self()->comp(),
        ...
        for (auto listIt = leftMostNodesList.begin(); listIt != leftMostNodesList.end(); ++listIt) {
            ...
            if (TR::ILOpCode::isPackedConversionToWiderType((*listIt)->getOpCodeValue())) {
                ...
                if (firstChild->chkSkipPadByteClearing()
                    ...
                    if (self()->traceBCDCodeGen())
                        traceMsg(self()->comp(),
                            "\tUnset skipPadByteClearing on node %s (%p): storage ref hint has size %d and converted "
                            "node has size %d\n",
                            firstChild->getOpCode().getName(), firstChild, storageRefHint->getSymbolSize(),
                            TR::DataType::getSizeFromBCDPrecision(
...
void J9::Z::CodeGenerator::markStoreAsAnAccumulator(TR::Node* node)
{
    ...
    if (node->getOpCode().canUseStoreAsAnAccumulator()) {
        ...
        if (self()->traceBCDCodeGen()) {
            traceMsg(self()->comp(), "\nUseAsAnAccumulator check for store %s (%p) #%d", node->getOpCode().getName(),
        ...
        if (self()->traceBCDCodeGen())
            traceMsg(self()->comp(), "\n\texamine nodes in value subtree starting at %s [%s]\n",
                node->getValueChild()->getOpCode().getName(),
                node->getValueChild()->getName(self()->comp()->getDebug()));
        ...
        bool canUse = self()->canUseSingleStoreAsAnAccumulator(
            NULL, node->getValueChild(), node, aliases, &conflictingAddressNodes, false, true, false);
        if (self()->traceBCDCodeGen())
        ...
        if (canUse
            && performTransformation(self()->comp(), "%sset new UseStoreAsAnAccumulator=true on %s [%s]\n", OPT_DETAILS,
                   node->getOpCode().getName(), node->getName(self()->comp()->getDebug()))) {
...
TR::Register* J9::Z::CodeGenerator::evaluateAggregateToGPR(
    size_t destSize, TR::Node* srcNode, TR_OpaquePseudoRegister* srcReg, TR::MemoryReference* srcMR)
{
    TR::CodeGenerator* cg = self();
    ...
    if (srcSize > destSize) {
        ...
        if (cg->traceBCDCodeGen())
            traceMsg(self()->comp(),
                "\tisTruncation=true : set loadSize = %d and add %d to sourceMR offset (srcAggrSize %d > destIntSize "
                "%d)\n",
    ...
    bool useICM = false;
    // currently cannot have loadSize <= 4 and is64Bit true but keep the cases in the switch below in case there is a
    // need to use 64 bit registers for smaller sizes
    switch (loadSize) {
    case 1:
    case 2:
        if (targetReg->getKind() == TR_GPR64) {
            generateRXInstruction(
                cg, loadSize == 1 ? TR::InstOpCode::LLGC : TR::InstOpCode::LLGH, srcNode, targetReg, srcMR);
        } else {
            generateRXInstruction(
                cg, loadSize == 1 ? TR::InstOpCode::LLC : TR::InstOpCode::LLH, srcNode, targetReg, srcMR);
        }
        break;
    case 3:
        useICM = true;
    ...
    case 7: {
        ...
        if (targetReg->getRegisterPair()) {
            generateRRInstruction(
                cg, TR::InstOpCode::XR, srcNode, targetReg->getHighOrder(), targetReg->getHighOrder());
            generateRSInstruction(cg, TR::InstOpCode::ICM, srcNode, targetReg->getHighOrder(), mask, srcMR);
            generateRXInstruction(cg, TR::InstOpCode::L, srcNode, targetReg->getLowOrder(),
                generateS390MemoryReference(*srcMR, mrOffset, cg));
        } else {
            ...
            generateRXInstruction(
                cg, TR::InstOpCode::L, srcNode, targetReg, generateS390MemoryReference(*srcMR, mrOffset, cg));
        }
    } break;
    case 8:
        if (targetReg->getRegisterPair()) {
            ...
            generateRXInstruction(
                cg, TR::InstOpCode::L, srcNode, targetReg->getLowOrder(), generateS390MemoryReference(*srcMR, 4, cg));
        } else {
    ...
    case 16: {
        ...
        generateRXInstruction(
            cg, TR::InstOpCode::LG, srcNode, targetReg->getLowOrder(), generateS390MemoryReference(*srcMR, 8, cg));
    ...
    if (useICM) {
        TR_ASSERT(targetReg->getRegisterPair() == NULL,
            "targetReg must not be a regPair in the useICM case for node %p\n", srcNode);
        generateRRInstruction(cg, targetReg->getKind() == TR_GPR64 ? TR::InstOpCode::XGR : TR::InstOpCode::XR, srcNode,
            targetReg, targetReg);
        generateRSInstruction(cg, TR::InstOpCode::ICM, srcNode, targetReg, (uint32_t)((1 << (loadSize)) - 1),
            srcMR); // 1->mask=1, 2->mask=3, 3->mask=7
    }

    return targetReg;
}

void J9::Z::CodeGenerator::correctBadSign(
    TR::Node* node, TR_PseudoRegister* reg, int32_t endByte, TR::MemoryReference* memRef)
{
    ...
    bool numericNibbleIsZero)
{
    ...
    if (sign == TR::DataType::getIgnoredSignCode()) {
        ...
        if (targetReg != srcReg) {
            if (targetReg) {
                if (srcReg) {
                    targetReg->transferSignState(
                        srcReg, true); // digitsLost=true -- conservatively set as this may be part of a truncation
                } else {
    ...
    if (self()->traceBCDCodeGen())
        traceMsg(self()->comp(),
            ...
            numericNibbleIsZero ? "yes" : "no", srcSign,
            srcReg && srcReg->hasKnownOrAssumedCleanSign() ? "true" : "false",
    ...
    case TR::ZonedDecimalSignLeadingEmbedded: {
        if (getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_z10) && isPacked && digitsToClear >= 3) {
            ...
            digitsCleared = 1;
            if (self()->traceBCDCodeGen())
                traceMsg(self()->comp(), "\t\tusing MVI for sign setting : set digitsCleared=1\n");
        } else {
            if (knownSignIs0xF) {
                generateSIInstruction(
                    cg, TR::InstOpCode::NI, node, signCodeMR, isPacked ? (0xF0 | sign) : (0x0F | (sign << 4)));
            ...
            } else {
                {
                    ...
                    generateSIInstruction(cg, TR::InstOpCode::NI, node,
                        generateS390LeftAlignedMemoryReference(*signCodeMR, node, 0, cg, signCodeMR->getLeftMostByte()),
 ...
 * Input reg can be NULL (when called for a store node or other type that does not return a register)
 */
void J9::Z::CodeGenerator::widenBCDValue(
    TR::Node* node, TR_PseudoRegister* reg, int32_t startByte, int32_t endByte, TR::MemoryReference* targetMR)
{
 ...
 * Input reg can be NULL (when called for a store node or other type that does not return a register)
 */
void J9::Z::CodeGenerator::widenBCDValueIfNeeded(
    TR::Node* node, TR_PseudoRegister* reg, int32_t startByte, int32_t endByte, TR::MemoryReference* targetMR)
{
    TR_ASSERT(node->getType().isBCD(), "widenBCDValue is only valid for BCD types (type=%s)\n",
        node->getDataType().toString());
    ...
    if (self()->traceBCDCodeGen())
        traceMsg(self()->comp(), "\twidenBCDValue for node %s (%p) : %d->%d (%d bytes)\n", node->getOpCode().getName(),
            node, startByte, endByte, bytesToClear);
    if (bytesToClear > 0) {
    ...
    int32_t digitsToClear, TR::MemoryReference* targetMR, bool widenOnLeft)
{
    ...
    if (self()->traceBCDCodeGen())
        traceMsg(self()->comp(),
            "\tgenZeroLeftMostDigitsIfNeeded %s #%d for node %p: digitsToClear = %d, endByte = %d (digit range is "
            "%d->%d), widenOnLeft=%s\n",
    ...
    if (widenOnLeft && actualDigitsToClear > widenOnLeftThreshold && reg->getLiveSymbolSize() > endByte) {
        ...
        if (self()->traceBCDCodeGen())
            traceMsg(self()->comp(),
                "\ttargetMR->getStorageReference() #%d liveSymSize %d > endByte %d so increase endByte %d->%d "
                "(endDigit %d->%d) and retrieve the actualDigitsToClear based on this new endDigit\n",
                targetMR->getStorageReference()->getReferenceNumber(), reg->getLiveSymbolSize(), origEndByte,
                origEndByte, endByte, origEndDigit, endDigit);
    }
    ...
    if (actualDigitsToClear) {
        int32_t offset = reg->getByteOffsetFromLeftForClear(
            startDigit, endDigit, actualDigitsToClear, endByte); // might modify actualDigitsToClear
        switch (node->getDataType()) {
        case TR::PackedDecimal:
        ...
        default:
            TR_ASSERT(
                false, "unsupported datatype %s in genZeroLeftMostDigitsIfNeededB\n", node->getDataType().toString());
            break;
        }
    } else {
    ...
    if (digitsToClear) {
        ...
        if (fullBytesToClear) {
            ...
            generateSS1Instruction(self(), TR::InstOpCode::XC, node, fullBytesToClear - 1,
                generateS390LeftAlignedMemoryReference(
                    *targetMR, node, destOffset + memRefOffset, self(), targetMR->getLeftMostByte()), // left justified
                generateS390LeftAlignedMemoryReference(
                    *targetMR, node, destOffset + memRefOffset, self(), targetMR->getLeftMostByte())); // left justified
        }
    ...
    bool trackSignState)
{
    ...
    TR_ASSERT(
        !isBCD || destSize <= TR_MAX_MVC_SIZE, "destSize %d > max %d for node %p\n", destSize, TR_MAX_MVC_SIZE, node);
    ...
    if (!self()->storageReferencesMatch(dstStorageRef, srcStorageRef)) {
        ...
        if (self()->traceBCDCodeGen())
            traceMsg(self()->comp(),
                "\t\tnode %p : srcReg %s (hasBadSign %s) on srcNode %p has bytes %d->%d %salready clear "
                "(bytesToClear=%d), srcCastedToBCD=%d\n",
                node, self()->getDebug()->getName(srcReg),
                srcPseudoReg ? (srcPseudoReg->hasKnownBadSignCode() ? "yes" : "no") : "no", srcNode, sourceSize,
                destSize, bytesToClear == 0 ? "" : "not ", bytesToClear, srcCastedToBCD);
    ...
    bool trackSignState)
{
    if (self()->traceBCDCodeGen())
        traceMsg(self()->comp(),
            ...
            srcReg ? self()->getDebug()->getName(srcReg) : "NULL", srcReg ? srcReg->getSize() : 0, sourceSize,
            destSize);
    ...
    TR_ASSERT(
        srcReg, "expecting a non-null srcReg in initializeNewTemporaryStorageReference for srcNode %p\n", srcNode);

    TR::MemoryReference* destMR = NULL;
    if (srcReg->getPseudoRegister())
        destMR = generateS390RightAlignedMemoryReference(
            node, tempStorageReference, self(), true, true); // enforceSSLimits=true, isNewTemp=true
    else
...
TR_OpaquePseudoRegister* J9::Z::CodeGenerator::privatizePseudoRegister(
    TR::Node* node, TR_OpaquePseudoRegister* reg, TR_StorageReference* storageRef, size_t sizeOverride)
{
    TR::CodeGenerator* cg = self();
    ...
    if (self()->traceBCDCodeGen()) {
        if (sizeOverride != 0 && sizeOverride != regSize)
            traceMsg(self()->comp(),
                "\tsizeOverride=%d : use this as the size for privatizing reg %s (instead of regSize %d)\n",
    ...
    bool isBCD = node->getType().isBCD();
    TR_StorageReference* tempStorageReference
        = TR_StorageReference::createTemporaryBasedStorageReference(size, self()->comp());
    ...
    if (isBCD) {
        ...
        copyMR = generateS390RightAlignedMemoryReference(
            node, tempStorageReference, cg, true, true); // enforceSSLimits=true, isNewTemp=true
    } else {
    ...
    if (self()->traceBCDCodeGen())
        traceMsg(self()->comp(),
            ...
            self()->getDebug()->getName(tempStorageReference->getSymbol()), self()->comp()->getLineNumber(node));
    ...
    tempRegister->setStorageReference(
        tempStorageReference, NULL); // node==NULL as the temp refCounts are explicitly being managed as the temp will
                                     // only live for this evaluator
...
TR_OpaquePseudoRegister* J9::Z::CodeGenerator::privatizePseudoRegisterIfNeeded(
    TR::Node* parent, TR::Node* child, TR_OpaquePseudoRegister* childReg)
{
    ...
    if (hint && hint->isNodeBased()) {
        ...
        if (self()->traceBCDCodeGen())
            traceMsg(self()->comp(),
                "\tprivatizePseudoRegisterIfNeeded for %s (%p) with hint %s (%p) (isInMemoryCopyProp=%s) and child %s "
                "(%p) (child storageRef isNonConstNodeBased=%s)\n",
        ...
        if (childStorageRef && childStorageRef->isNonConstantNodeBased()
            ...
            if (hintNode->isInMemoryCopyProp() || isUsingStorageRefFromAnotherStore || childRegHasDeadOrIgnoredBytes) {
                ...
                if (self()->traceBCDCodeGen())
                    traceMsg(self()->comp(),
                        "\tcheck overlap between store hint %s (%p) and childStorageRefNode %s (%p)\n",
                        hintNode->getOpCode().getName(), hintNode, childStorageRefNode->getOpCode().getName(),
                        childStorageRefNode);
                if (self()->loadAndStoreMayOverlap(
                        hintNode, hintNode->getSize(), childStorageRefNode, childStorageRefNode->getSize())) {
                    ...
                    if (needsPrivitization) {
                        if (self()->traceBCDCodeGen()) {
                            if (hintNode->isInMemoryCopyProp())
                                traceMsg(self()->comp(),
                                    "\ta^a : privatize needed due to isInMemoryCopyProp hintNode %s (%p) on line_no=%d "
                                    "(privatizeCase)\n",
                                    hintNode->getOpCode().getName(), hintNode, self()->comp()->getLineNumber(hintNode));
                            if (isUsingStorageRefFromAnotherStore)
                                traceMsg(self()->comp(),
                                    "\ta^a : privatize needed due to isUsingStorageRefFromAnotherStore "
                                    "childStorageRefNode %s (%p) on line_no=%d (privatizeCase)\n",
                                    childStorageRefNode->getOpCode().getName(), childStorageRefNode,
                                    self()->comp()->getLineNumber(hintNode));
                            if (childRegHasDeadOrIgnoredBytes)
                                traceMsg(self()->comp(),
                                    "\ta^a : privatize needed due to childRegHasDeadOrIgnoredBytes valueReg %s child "
                                    "%s (%p) on line_no=%d (privatizeCase)\n",
                        ...
                        TR_ASSERT(!self()->comp()->getOption(TR_EnablePerfAsserts),
                    ...
                    if (self()->traceBCDCodeGen())
                        traceMsg(self()->comp(),
                            "\toverlap=false (from %s test) -- do not privatize the source memref\n",
                            useAliasing ? "aliasing" : "pattern");
                }
            } else {
                if (self()->traceBCDCodeGen())
                    traceMsg(self()->comp(),
                        "y^y : temp copy saved isInMemoryCopyProp = false on %s (%p) (privatizeCase)\n",
...
TR_PseudoRegister* J9::Z::CodeGenerator::privatizeBCDRegisterIfNeeded(
    TR::Node* parent, TR::Node* child, TR_OpaquePseudoRegister* childReg)
{
...
TR_StorageReference* J9::Z::CodeGenerator::privatizeStorageReference(
    TR::Node* node, TR_OpaquePseudoRegister* reg, TR::MemoryReference* memRef)
{
    ...
    if (force || (storageRef->isNodeBased() && node->getReferenceCount() > 1 && !node->skipCopyOnLoad())) {
        if (self()->traceBCDCodeGen()) {
            traceMsg(self()->comp(),
                "\tnode %p (%s) with skipCopyOnLoad=false does need to be privatized for node based storageRef node %p "
                "(%s-based) (force=%s)\n",
                node, node->getOpCode().getName(), storageRef->getNode(),
                storageRef->getNode()->getOpCode().isStore() ? "store" : "load", force ? "yes" : "no");
            traceMsg(self()->comp(),
                "\tb^b : gen memcpy of size = %d to privatizeStorageReference node %s (%p) with storageRef #%d (%s) on "
                "line_no=%d\n",
                reg->getSize(), node->getOpCode().getName(), node, storageRef->getReferenceNumber(),
                self()->getDebug()->getName(storageRef->getSymbol()), self()->comp()->getLineNumber(node));
        }
    ...
    int32_t resultSize, int32_t clearSize, bool updateStorageReference, bool alwaysEnforceSSLimits)
{
    ...
    if (self()->traceBCDCodeGen())
        traceMsg(self()->comp(),
            "\tmaterializeFullBCDValue evaluated %s (%p) (nodeSize %d, requested resultSize %d) to reg %s (regSize "
            "%d), clearSize=%d, updateStorageReference=%s\n",
            node->getOpCode().getName(), node, node->getStorageReferenceSize(), resultSize,
    ...
    if (regSize < resultSize && reg->getLiveSymbolSize() >= resultSize
        ...
        if (self()->traceBCDCodeGen())
            traceMsg(self()->comp(),
                ...
                regSize, clearSize);
        memRef = reuseS390RightAlignedMemoryReference(
            memRef, node, reg->getStorageReference(), self(), alwaysEnforceSSLimits);
        ...
        if (reg->isInitialized() && reg->getStorageReference()->isReadOnlyTemporary() && liveSymSize > regSize
            ...
            memRef = reuseS390RightAlignedMemoryReference(
                memRef, node, reg->getStorageReference(), self(), enforceSSLimitsForClear);
        ...
        if (reg->isInitialized() && reg->trackZeroDigits() && liveSymSize >= resultSize) {
            ...
            if (self()->traceBCDCodeGen())
                traceMsg(self()->comp(),
                    "\treg->getLiveSymbolSize() < resultSize (%d < %d) so call increaseTemporarySymbolSize but first "
                    "check for already cleared bytes\n",
            ...
            } else // if not clearing all the new bytes than the zero digits will not be left aligned
            {
                ...
                if (self()->traceBCDCodeGen() && savedLeftAlignedZeroDigits > 0)
                    traceMsg(self()->comp(),
                        "x^x : missed transferring savedLeftAlignedZeroDigits %d on matFull, node %p\n",
                        savedLeftAlignedZeroDigits, node);
            }
        } else {
            if (self()->traceBCDCodeGen())
                traceMsg(self()->comp(),
                    "\tstorageReference #%d is not tempBased (or is not packed) and reg->getLiveSymbolSize() < "
                    "resultSize (%d < %d) so alloc a new temporary reference\n",
            ...
            memRef = generateS390RightAlignedMemoryReference(
                node, tempStorageRef, self(), true, true); // enforceSSLimits=true, isNewTemp=true
...
bool J9::Z::CodeGenerator::inlineSmallLiteral(size_t srcSize, char* srcLiteral, size_t destSize, bool trace)
{
    ...
    if (srcSize != destSize) {
        ...
        if (trace)
            traceMsg(self()->comp(),
                "\t\tinlineLiteral=true : canCopyWithOneOrTwoInstrs = true (srcSize %d, destSize %d)\n", srcSize,
                destSize);
    } else {
        inlineLiteral = false;
        if (trace)
...
bool J9::Z::CodeGenerator::checkFieldAlignmentForAtomicLong()
{
    ...
    int32_t intOrBoolOffset = self()->fe()->getObjectHeaderSizeInBytes()
 ...
 */
uint32_t J9::Z::CodeGenerator::getPDMulEncodedSize(
    TR::Node* pdmul, TR_PseudoRegister* multiplicand, TR_PseudoRegister* multiplier)
{
...
int32_t J9::Z::CodeGenerator::getPDMulEncodedPrecision(
    TR::Node* pdmul, TR_PseudoRegister* multiplicand, TR_PseudoRegister* multiplier)
{
...
uint32_t J9::Z::CodeGenerator::getPackedToDecimalFloatFixedSize()
{
...
uint32_t J9::Z::CodeGenerator::getPackedToDecimalDoubleFixedSize()
{
    if (self()->supportsFastPackedDFPConversions())
...
uint32_t J9::Z::CodeGenerator::getPackedToDecimalLongDoubleFixedSize()
{
    if (self()->supportsFastPackedDFPConversions())
...
uint32_t J9::Z::CodeGenerator::getDecimalFloatToPackedFixedSize()
{
    if (self()->supportsFastPackedDFPConversions())
...
uint32_t J9::Z::CodeGenerator::getDecimalDoubleToPackedFixedSize()
{
    if (self()->supportsFastPackedDFPConversions())
...
uint32_t J9::Z::CodeGenerator::getDecimalLongDoubleToPackedFixedSize()
{
    if (self()->supportsFastPackedDFPConversions())
...
bool J9::Z::CodeGenerator::supportsPackedShiftRight(int32_t resultPrecision, TR::Node* shiftSource, int32_t shiftAmount)
{
    ...
    if (self()->traceBCDCodeGen())
        traceMsg(self()->comp(),
            "%ssupportsPackedShiftRight = %s : shiftSource %s (%p) p=%d by shiftAmount=%d -> shiftedPrec=%d "
            "(resultPrec %d) on line_no=%d (offset=%06X)\n",
...
int32_t J9::Z::CodeGenerator::getPDDivEncodedPrecision(
    TR::Node* node, TR_PseudoRegister* dividendReg, TR_PseudoRegister* divisorReg)
{
    TR_ASSERT(node->getOpCodeValue() == TR::pddiv || node->getOpCodeValue() == TR::pdrem,
        "getPackedDividendPrecision only valid for pddiv/pdrem\n");
    return self()->getPDDivEncodedPrecisionCommon(
        node, dividendReg->getDecimalPrecision(), divisorReg->getDecimalPrecision(), divisorReg->isEvenPrecision());
}

int32_t J9::Z::CodeGenerator::getPDDivEncodedPrecisionCommon(
    TR::Node* node, int32_t dividendPrecision, int32_t divisorPrecision, bool isDivisorEvenPrecision)
{
...
uint32_t J9::Z::CodeGenerator::getPDDivEncodedSize(
    TR::Node* node, TR_PseudoRegister* dividendReg, TR_PseudoRegister* divisorReg)
{
    TR_ASSERT(node->getOpCodeValue() == TR::pddiv || node->getOpCodeValue() == TR::pdrem,
        "getPDDivEncodedSize only valid for pddiv/pdrem\n");
...
bool J9::Z::CodeGenerator::canGeneratePDBinaryIntrinsic(
    TR::ILOpCodes opCode, TR::Node* op1PrecNode, TR::Node* op2PrecNode, TR::Node* resultPrecNode)
{
...
void J9::Z::CodeGenerator::incRefCountForOpaquePseudoRegister(
    TR::Node* node, TR::CodeGenerator* cg, TR::Compilation* comp)
{
    if (node->getOpaquePseudoRegister()) {
        ...
        if (ref && ref->isNodeBased() && ref->getNodeReferenceCount() > 0) {
            if (cg->traceBCDCodeGen())
                comp->getDebug()->trace("\tnode %s (%p) with storageRef #%d (%s): increment nodeRefCount %d->%d when "
                                        "artificially incrementing ref count\n",
                    node->getOpCode().getName(), node, ref->getReferenceNumber(),
                    comp->getDebug()->getName(ref->getSymbol()), ref->getNodeReferenceCount(),
                    ref->getNodeReferenceCount() + 1);
...
TR::Instruction* J9::Z::CodeGenerator::generateVMCallHelperSnippet(
    TR::Instruction* cursor, TR::LabelSymbol* vmCallHelperSnippetLabel)
{
    ...
    cursor = new (self()->trHeapMemory())
        TR::S390LabelInstruction(TR::InstOpCode::LABEL, node, vmCallHelperSnippetLabel, cursor, self());
    ...
    cursor = generateS390BranchInstruction(
        self(), TR::InstOpCode::BCR, node, TR::InstOpCode::COND_BCR, self()->getEntryPointRealRegister(), cursor);
    ...
    AOTcgDiag4(comp, "Add encodingRelocation = %p reloType = %p symbolRef = %p helperId = %x\n", encodingRelocation,
        encodingRelocation->getReloType(), encodingRelocation->getSymbolReference(), helperId);
    ...
    if (TR::Compiler->target.is64Bit()) {
        cursor = generateDataConstantInstruction(
            self(), TR::InstOpCode::DC, node, UPPER_4_BYTES(vmCallHelperAddress), cursor);
        cursor->setEncodingRelocation(encodingRelocation);

        cursor = generateDataConstantInstruction(
            self(), TR::InstOpCode::DC, node, LOWER_4_BYTES(vmCallHelperAddress), cursor);
    } else {
    ...
    AOTcgDiag2(
        comp, "Add encodingRelocation = %p reloType = %p\n", encodingRelocation, encodingRelocation->getReloType());

    const intptrj_t j9MethodAddress
        = reinterpret_cast<intptrj_t>(methodSymbol->getResolvedMethod()->resolvedMethodAddress());

    if (TR::Compiler->target.is64Bit()) {
        cursor
            = generateDataConstantInstruction(self(), TR::InstOpCode::DC, node, UPPER_4_BYTES(j9MethodAddress), cursor);
        cursor->setEncodingRelocation(encodingRelocation);

        cursor
            = generateDataConstantInstruction(self(), TR::InstOpCode::DC, node, LOWER_4_BYTES(j9MethodAddress), cursor);
    } else {
...
TR::Instruction* J9::Z::CodeGenerator::generateVMCallHelperPrePrologue(TR::Instruction* cursor)
{
    ...
    cursor = generateS390BranchInstruction(
        self(), TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, node, vmCallHelperSnippetLabel, cursor);
...
bool J9::Z::CodeGenerator::suppressInliningOfRecognizedMethod(TR::RecognizedMethod method)
{
    ...
    if (!self()->comp()->compileRelocatableCode() && !self()->comp()->getOption(TR_DisableDFP)
        ...
        if (method == TR::java_math_BigDecimal_DFPIntConstructor
            ...
            || method == TR::java_math_BigDecimal_DFPLongExpConstructor || method == TR::java_math_BigDecimal_DFPAdd
            || method == TR::java_math_BigDecimal_DFPSubtract || method == TR::java_math_BigDecimal_DFPMultiply
            || method == TR::java_math_BigDecimal_DFPDivide || method == TR::java_math_BigDecimal_DFPScaledAdd
            || method == TR::java_math_BigDecimal_DFPScaledSubtract
            || method == TR::java_math_BigDecimal_DFPScaledMultiply
            || method == TR::java_math_BigDecimal_DFPScaledDivide || method == TR::java_math_BigDecimal_DFPRound
            || method == TR::java_math_BigDecimal_DFPSetScale || method == TR::java_math_BigDecimal_DFPCompareTo
            || method == TR::java_math_BigDecimal_DFPSignificance || method == TR::java_math_BigDecimal_DFPExponent
            || method == TR::java_math_BigDecimal_DFPBCDDigits || method == TR::java_math_BigDecimal_DFPUnscaledValue
    ...
    if (method == TR::java_lang_Long_reverseBytes || method == TR::java_lang_Integer_reverseBytes
...
bool J9::Z::CodeGenerator::inlineDirectCall(TR::Node* node, TR::Register*& resultReg)
{
    ...
    if (comp->getSymRefTab()->isNonHelper(
            node->getSymbolReference(), TR::SymbolReferenceTable::currentTimeMaxPrecisionSymbol)) {
        ...
        return true;
                   ...
                   node->getSymbolReference(), TR::SymbolReferenceTable::synchronizedFieldLoadSymbol)) {
    ...
    case TR::sun_misc_Unsafe_compareAndSwapObject_jlObjectJjlObjectjlObject_Z:
        ...
        if ((!TR::Compiler->om.canGenerateArraylets() || node->isUnsafeGetPutCASCallOnNonArray())
            && node->isSafeForCGToFastPathUnsafeCall()) {
            resultReg = VMinlineCompareAndSwap(node, cg,
                (comp->useCompressedPointers() ? TR::InstOpCode::CS : TR::InstOpCode::getCmpAndSwapOpCode()), IS_OBJ);
            return true;
        }
        break;
    ...
    case TR::com_ibm_dataaccess_ByteArrayUtils_trailingZerosQuadWordAtATime_:
        ...
        if (cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_z10)
            && !comp->getOption(TR_DisableIntrinsics) && !comp->getOption(TR_DisableDAATrailingZero)
            && !TR::Compiler->om.canGenerateArraylets())
