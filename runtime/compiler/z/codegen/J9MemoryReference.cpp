    ...
    List<TR::Node>& nodesAlreadyEvaluatedBeforeFoldingList, TR::CodeGenerator* cg)
{
    ...
    if (cg->traceBCDCodeGen())
        traceMsg(comp, "\t\t\trecAdjust node - %s (%p) and add to list, increment %d: refCount %d->%d\n",
            node->getOpCode().getName(), node, increment, node->getReferenceCount(),
            node->getReferenceCount() + increment);
...
void J9::Z::MemoryReference::addInstrSpecificRelocation(
    TR::CodeGenerator* cg, TR::Instruction* instr, int32_t disp, uint8_t* cursor)
{}
    ...
    if (storageReference) {
        ...
        if (isImpliedMemoryReference) {
            ...
            if (self()->forceFolding() || self()->forceFirstTimeFolding()
                ...
                recursivelyIncrementReferenceCount(
                    addressChild, 1, _incrementedNodesList, nodesAlreadyEvaluatedBeforeFoldingList, cg);
            } else {
...
TR::UnresolvedDataSnippet* J9::Z::MemoryReference::createUnresolvedDataSnippet(
...
TR::UnresolvedDataSnippet* J9::Z::MemoryReference::createUnresolvedDataSnippetForiaload(
    TR::Node* node, TR::CodeGenerator* cg, TR::SymbolReference* symRef, TR::Register* tempReg, bool& isStore)
{
...
void J9::Z::MemoryReference::createUnresolvedSnippetWithNodeRegister(
...
void J9::Z::MemoryReference::createUnresolvedDataSnippetForBaseNode(
...
void J9::Z::MemoryReference::createPatchableDataInLitpool(
    TR::Node* node, TR::CodeGenerator* cg, TR::Register* tempReg, TR::UnresolvedDataSnippet* uds)
{
    ...
    if (cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_z10)) {
        ...
        LRLinst = (TR::S390RILInstruction*)generateRILInstruction(
            cg, TR::InstOpCode::getLoadRelativeLongOpCode(), node, tempReg, reinterpret_cast<uintptrj_t*>(0xBABE), 0);
 ...
 */
TR::MemoryReference* generateS390MemRefFromStorageRef(
    TR::Node* node, TR_StorageReference* storageReference, TR::CodeGenerator* cg, bool enforceSSLimits, bool isNewTemp)
{
    TR::Compilation* comp = cg->comp();
    ...
    if (storageReference->isTemporaryBased() && storageReference->getSymbolReference()->hasTemporaryNegativeOffset()) {
        if (cg->traceBCDCodeGen())
            traceMsg(comp,
                "\tgenerateS390AlignedMemoryReference mr %p: call addToTemporaryNegativeOffset flag for "
                "storageRef->symRef #%d (offset = %d) and node %s (%p) to memRef\n",
