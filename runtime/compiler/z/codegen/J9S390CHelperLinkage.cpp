...
////////////////////////////////////////////////////////////////////////////////
TR::S390CHelperLinkage::S390CHelperLinkage(
    ...
    : TR::Linkage(codeGen, elc, lc)
{
    ...
    // In case of 32 Bit Linux on Z, System Linkage only preserves FPR4 and FPR6, for all other target, FPR8-FPR15 is
    // preserved.
 ...
 */
TR::Register* TR::S390CHelperLinkage::buildDirectDispatch(
    TR::Node* callNode, TR::RegisterDependencyConditions** deps, TR::Register* returnReg)
{
    ...
    for (int i = 0; i < callNode->getNumChildren(); i++) {
        if (i < self()->getNumIntegerArgumentRegisters() - 1)
            preDeps->addPreCondition(
                cg()->gprClobberEvaluate(callNode->getChild(i)), getIntegerArgumentRegister(i + 1));
        else
            TR_ASSERT(false, "Parameters on Stack not supported yet");
        childNodeRegDeps->addPostConditionIfNotAlreadyInserted(
            callNode->getChild(i)->getRegister(), TR::RealRegister::AssignAny);
    }
    ...
    if (isFastPathOnly) {
        ...
        // register in GC map. Only possiblity of non-preserved register containing a live object is in argument to
