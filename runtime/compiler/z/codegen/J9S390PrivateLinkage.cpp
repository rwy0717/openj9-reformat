...
////////////////////////////////////////////////////////////////////////////////
TR::S390PrivateLinkage::S390PrivateLinkage(
    TR::CodeGenerator* codeGen, TR_S390LinkageConventions elc, TR_LinkageConventions lc)
    : TR::Linkage(codeGen, elc, lc)
{
    ...
    if (codeGen->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_z13)
        && TR::Compiler->target.cpu.getS390SupportsVectorFacility() && !comp()->getOption(TR_DisableSIMD)) {
...
////////////////////////////////////////////////////////////////////////////////
void TR::S390PrivateLinkage::initS390RealRegisterLinkage()
{
    ...
    if (freeingSSPDisabled || disableFreeJITSSP != NULL || !(cg()->supportsJITFreeSystemStackPointer())
        || (TR::Compiler->target.is32Bit() && !cg()->supportsHighWordFacility())
        || // Cannot use FreeSSP on 31-bit without highword tracking
        comp()->getOption(
            TR_Randomize)) // We can generate code for different hardware targets, which are incompatiable.
    {
...
void TR::S390PrivateLinkage::mapCompactedStack(TR::ResolvedMethodSymbol* method)
{
            ...
            if (localCursor->getGCMapIndex() >= 0) {
                ...
                if (cg()->getTraceRAOption(TR_TraceRASpillTemps))
                    traceMsg(comp(), "\nmapCompactedStack: changing %s (GC index %d) offset from %d to %d",
                        comp()->getDebug()->getName(localCursor), localCursor->getGCMapIndex(),
        ...
        while (variableSizeSymCursor != NULL) {
            if (variableSizeSymCursor->isReferenced()) {
                if (cg()->traceBCDCodeGen())
                    traceMsg(comp(), "map variableSize sym %p (size %d) because isReferenced=true ",
...
void TR::S390PrivateLinkage::mapStack(TR::ResolvedMethodSymbol* method)
{
    ...
    while (variableSizeSymCursor != NULL) {
        ...
        if (variableSizeSymCursor->isReferenced()) {
            if (cg()->traceBCDCodeGen())
                traceMsg(comp(), "map variableSize sym %p (size %d) because isReferenced=true ", variableSizeSymCursor,
...
////////////////////////////////////////////////////////////////////////////////
void TR::S390PrivateLinkage::mapSingleAutomatic(TR::AutomaticSymbol* p, uint32_t& stackIndex)
{
...
// TR::S390PrivateLinkage::mapSingleAutomatic - maps an automatic onto the stack
////////////////////////////////////////////////////////////////////////////////
void TR::S390PrivateLinkage::mapSingleAutomatic(TR::AutomaticSymbol* p, uint32_t size, uint32_t& stackIndex)
{

    p->setOffset(stackIndex -= size);
}

bool TR::S390PrivateLinkage::hasToBeOnStack(TR::ParameterSymbol* parm)
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(fe());
        ...
        ((parm->getLinkageRegisterIndex() == 0 && // is first parameter (this pointer)
             ...
             ((bodySymbol->isSynchronised())
                 || (!strncmp(bodySymbol->getResolvedMethod()->nameChars(), "<init>", 6)
                        && ((throwableClass = fej9->getClassFromSignature(
                                 "Ljava/lang/Throwable;", 21, bodySymbol->getResolvedMethod()))
                                   == 0
                               || fej9->isInstanceOf(
                                      bodySymbol->getResolvedMethod()->containingClass(), throwableClass, true)
                                   != TR_no))))
    ...
    if ((numBytes < 4096) && (numIters * 256 + baseOffset < 4096)) {
        ...
        if (baseOffset >= MIN_IMMEDIATE_VAL && baseOffset <= MAX_IMMEDIATE_VAL) {
            ...
            cursor = generateRIInstruction(
                codeGen, TR::InstOpCode::getAddHalfWordImmOpCode(), node, baseReg, baseOffset, cursor);
...
int32_t TR::S390PrivateLinkage::calculateRegisterSaveSize(TR::RealRegister::RegNum firstUsedReg,
    TR::RealRegister::RegNum lastUsedReg, TR::RealRegister::RegNum firstUsedHighWordReg,
    TR::RealRegister::RegNum lastUsedHighWordReg, int32_t& registerSaveDescription, int32_t& numIntSaved,
    int32_t& numFloatSaved, int32_t& numHighWordRegSaved)
{
...
int32_t TR::S390PrivateLinkage::setupLiteralPoolRegister(TR::Snippet* firstSnippet)
{
...
////////////////////////////////////////////////////////////////////////////////
void TR::S390PrivateLinkage::createPrologue(TR::Instruction* cursor)
{
    ...
    regSaveSize = calculateRegisterSaveSize(firstUsedReg, lastUsedReg, firstUsedHighWordReg, lastUsedHighWordReg,
        registerSaveDescription, numIntSaved, numFloatSaved, numHighWordRegSaved);
    ...
    if (comp()->getOption(TR_TraceCG)) {
        traceMsg(comp(), "\n regSaveSize = %d localSize = %d argSize = %d firstLocalOffset = %d \n", regSaveSize,
            localSize, argSize, firstLocalOffset);
        // traceMsg(comp(), " firstUsedReg = %d lastUsedReg = %d firstUsedHighWordReg = %d lastUsedHighWordReg = %d\n",
        // firstUsedReg, lastUsedReg, firstUsedHighWordReg, lastUsedHighWordReg); traceMsg(comp(), " numIntSaved = %d
        // numHighWordRegSaved = %d\n", numIntSaved, numHighWordRegSaved);
    ...
    if (largeStack) {
        ...
        cursor = generateRXInstruction(
            cg(), TR::InstOpCode::LAY, firstNode, spReg, generateS390MemoryReference(spReg, (size) * -1, cg()), cursor);
    }
    ...
    if (!comp()->isDLT()) {
        ...
        cursor = generateRXInstruction(
            cg(), TR::InstOpCode::getCmpLogicalOpCode(), firstNode, spReg, stackLimitMR, cursor);
        ...
        cursor = generateS390BranchInstruction(
            cg(), TR::InstOpCode::BRC, TR::InstOpCode::COND_BL, firstNode, stackOverflowSnippetLabel, cursor);
    ...
    if (bppoutline && cg()->_outlineCall._frequency != -1) {
        ...
        cursor = generateS390BranchPredictionPreloadInstruction(
            cg(), TR::InstOpCode::BPP, firstNode, cg()->_outlineCall._callLabel, (int8_t)0xD, tempMR, cursor);
    }
    if (bppoutline && cg()->_outlineArrayCall._frequency != -1) {
        ...
        TR::MemoryReference* tempMR = generateS390MemoryReference(epReg, 0, cg());
        cursor = generateS390BranchPredictionPreloadInstruction(
            cg(), TR::InstOpCode::BPP, firstNode, cg()->_outlineArrayCall._callLabel, (int8_t)0xD, tempMR, cursor);
    }
#endif

    if (cg()->getSupportsRuntimeInstrumentation())
        cursor = TR::TreeEvaluator::generateRuntimeInstrumentationOnOffSequence(
            cg(), TR::InstOpCode::RION, firstNode, cursor, true);
    ...
    if (lastUsedReg != TR::RealRegister::NoReg) {
        ...
        if (firstUsedReg != lastUsedReg) {
            ...
            cursor = generateRXInstruction(
                cg(), TR::InstOpCode::getStoreOpCode(), firstNode, getRealRegister(firstUsedReg), rsa, cursor);
        }
    }
    ...
    // save HPRs
    if (0 && cg()->supportsHighWordFacility() && !comp()->getOption(TR_DisableHighWordRA)
        && TR::Compiler->target.is32Bit() && lastUsedHighWordReg != TR::RealRegister::NoReg) {
        cursor = generateRSInstruction(cg(), TR::InstOpCode::STMH, firstNode, getRealRegister(firstUsedHighWordReg),
            getRealRegister(lastUsedHighWordReg), generateS390MemoryReference(spReg, disp, cg()), cursor);
    ...
    if (cg()->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_z10)) {
        ...
        if (cg()->isPrefetchNextStackCacheLine() && prefetchStack) {
            cursor = generateRXInstruction(
                cg(), TR::InstOpCode::PFD, firstNode, 2, generateS390MemoryReference(spReg, -256, cg()), cursor);
        }
    }
...
////////////////////////////////////////////////////////////////////////////////
void TR::S390PrivateLinkage::createEpilogue(TR::Instruction* cursor)
{
    ...
    if (enableBranchPreload && (cursor->getNext() == cg()->_hottestReturn._returnInstr)) {
        if (cg()->_hottestReturn._frequency > 6 && cg()->_hottestReturn._insertBPPInEpilogue) {
            ...
            cursor = generateS390BranchPredictionPreloadInstruction(
                cg(), TR::InstOpCode::BPP, nextNode, cg()->_hottestReturn._returnLabel, (int8_t)0x6, tempMR, cursor);
    ...
    if (lastUsedReg != TR::RealRegister::NoReg) {
        if (firstUsedReg != lastUsedReg) {
            cursor = restorePreservedRegs(
                firstUsedReg, lastUsedReg, blockNumber, cursor, nextNode, spReg, rsa, getStackPointerRegister());
        } else {
            cursor = generateRXInstruction(
                cg(), TR::InstOpCode::getLoadOpCode(), nextNode, getRealRegister(firstUsedReg), rsa, cursor);
        }
    ...
    if (adjustSize < MAXDISP) {
        cursor = generateRXInstruction(
            cg(), TR::InstOpCode::LA, nextNode, spReg, generateS390MemoryReference(spReg, adjustSize, cg()), cursor);
    } else if (adjustSize < MAXLONGDISP) {
        cursor = generateRXInstruction(
            cg(), TR::InstOpCode::LAY, nextNode, spReg, generateS390MemoryReference(spReg, adjustSize, cg()), cursor);
    } else {
    ...
    if (cg()->getSupportsRuntimeInstrumentation())
        cursor = TR::TreeEvaluator::generateRuntimeInstrumentationOnOffSequence(
            cg(), TR::InstOpCode::RIOFF, currentNode, cursor, true);
        ...
        if (cursor->getNext() == cg()->_hottestReturn._returnInstr) {
            if (cg()->_hottestReturn._frequency > 6) {
                cursor = generateS390LabelInstruction(
                    cg(), TR::InstOpCode::LABEL, currentNode, cg()->_hottestReturn._returnLabel, cursor);
            }
        }
    }
#endif
    ...
    cursor = generateS390RegInstruction(
        cg(), TR::InstOpCode::BCR, currentNode, getRealRegister(getReturnAddressRegister()), cursor);
...
////////////////////////////////////////////////////////////////////////////////
void TR::S390PrivateLinkage::buildVirtualDispatch(
    TR::Node* callNode, TR::RegisterDependencyConditions* dependencies, TR::Register* vftReg, uint32_t sizeOfArguments)
{
            ...
            if (!virtualThunk) {
                virtualThunk = fej9->setJ2IThunk(j2iSignature, signatureLen,
                    TR::S390J9CallSnippet::generateVIThunk(
                        fej9->getEquivalentVirtualCallNodeForDispatchVirtual(callNode, comp()), sizeOfArguments, cg()),
        ...
        default:
            if (fej9->needsInvokeExactJ2IThunk(callNode, comp())) {
                comp()->getPersistentInfo()->getInvokeExactJ2IThunkTable()->addThunk(
                    TR::S390J9CallSnippet::generateInvokeExactJ2IThunk(
                        callNode, sizeOfArguments, methodSymbol->getMethod()->signatureChars(), cg()),
                    fej9);
            }
            break;
        }
    ...
    if (methodSymbol->isVirtual()) {
        ...
        //
        TR::RegisterDependencyConditions* postDepsTemp = new (trHeapMemory()) TR::RegisterDependencyConditions(
        ...
        if (methodSymRef->isUnresolved() || comp()->compileRelocatableCode()) {
                ...
                if (resolvedMethod && !resolvedMethod->isInterpreted()
                    ...
                    if (!resolvedMethod->virtualMethodIsOverridden() && !resolvedMethod->isAbstract()) {
                        ...
                        virtualGuard = TR_VirtualGuard::createGuardedDevirtualizationGuard(
                            TR_NonoverriddenGuard, comp(), callNode);
                        if (comp()->getOption(TR_TraceCG)) {
                        ...
                        if (thisClass && TR::Compiler->cls.isAbstractClass(comp(), thisClass)
                            ...
                            TR_ResolvedMethod* method = chTable->findSingleAbstractImplementer(
                                thisClass, methodSymRef->getOffset(), methodSymRef->getOwningMethod(comp()), comp());
                            if (method
                                && ((method->isSameMethod(comp()->getCurrentMethod()) && !comp()->isDLT())
                                       || !method->isInterpreted() || method->isJITInternalNative())) {
                                performGuardedDevirtualization = true;
                                resolvedMethod = method;
                                virtualGuard = TR_VirtualGuard::createGuardedDevirtualizationGuard(
                                    TR_AbstractGuard, comp(), callNode);
                                if (comp()->getOption(TR_TraceCG)) {
                            ...
                            && !chTable->isOverriddenInThisHierarchy(
                                   resolvedMethod, refinedThisClass, methodSymRef->getOffset(), comp())) {
                            if (resolvedMethod->virtualMethodIsOverridden()) {
                                TR_ResolvedMethod* calleeMethod
                                    = methodSymRef->getOwningMethod(comp())->getResolvedVirtualMethod(
                                        comp(), refinedThisClass, methodSymRef->getOffset());
                                if (calleeMethod
                                    && ((calleeMethod->isSameMethod(comp()->getCurrentMethod()) && !comp()->isDLT())
                                           || !calleeMethod->isInterpreted() || calleeMethod->isJITInternalNative())) {
                                    performGuardedDevirtualization = true;
                    ...
                    if (performGuardedDevirtualization && virtualGuard) {
                        ...
                        generateVirtualGuardNOPInstruction(
                            cg(), callNode, virtualGuard->addNOPSite(), NULL, virtualLabel);
                        if (comp()->getOption(TR_EnableHCR)) {
                            if (cg()->supportsMergingGuards()) {
                                ...
                                generateVirtualGuardNOPInstruction(
                                    cg(), callNode, HCRGuard->addNOPSite(), NULL, virtualLabel);
                            }
                        }
                    }
                }
            ...
            if (!performGuardedDevirtualization && !comp()->getOption(TR_DisableInterpreterProfiling)
                && comp()->getOption(TR_enableProfiledDevirtualization) && TR_ValueProfileInfoManager::get(comp())
                && resolvedMethod) {
                ...
                if (topValue) {
                    ...
                    // We can't do this guarded devirtualization if the profile data is not correct for this context.
                    // See defect 98813
                    else {
                        TR_OpaqueClassBlock* callSiteMethod = methodSymRef->getSymbol()
                ...
                if (topValue) {
                    TR_ResolvedMethod* profiledVirtualMethod
                        = methodSymRef->getOwningMethod(comp())->getResolvedVirtualMethod(
                            comp(), (TR_OpaqueClassBlock*)topValue, methodSymRef->getOffset());
                    if (profiledVirtualMethod) {
                        if (comp()->getOption(TR_TraceCG)) {
                            traceMsg(comp(), "Profiled method {%s}\n",
            ...
            if (performGuardedDevirtualization || useProfiledValues) {
                ...
                if (useProfiledValues) {
                    TR::Instruction* unloadableConstInstr = generateRILInstruction(
                        cg(), TR::InstOpCode::LARL, callNode, RegZero, reinterpret_cast<uintptrj_t*>(profiledClass));
                ...
                if (comp()->getOption(TR_DisableOOL) || !virtualLabel)
                    generateS390BranchInstruction(
                        cg(), TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, callNode, doneVirtualLabel);
            }
        }
        ...
        if (!TR::Compiler->cls.classesOnHeap()
            && methodSymRef == comp()->getSymRefTab()->findObjectNewInstanceImplSymbol()) {
            classReg = RegThis;
            TR_ASSERT(offset >= 0,
                "TR::S390PrivateLinkage::buildVirtualDispatch - Offset to instanceOf method is assumed positive\n");
        } else {
            classReg = vftReg;
        }

        if (!TR::Compiler->cls.classesOnHeap() && offset >= 0
            && methodSymRef == comp()->getSymRefTab()->findObjectNewInstanceImplSymbol()) {
            cursor = generateRXInstruction(cg(), TR::InstOpCode::getLoadOpCode(), callNode, RegRA,
            ...
            if (offset >= (-32768)) {
                cursor = generateRIInstruction(
                    cg(), TR::InstOpCode::getLoadHalfWordImmOpCode(), callNode, RegZero, offset);
        ...
        if (outlinedSlowPath) {
            TR::Instruction* temp = generateS390BranchInstruction(
                cg(), TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, callNode, doneVirtualLabel);
            if (debugObj) {
        ...
        if (comp()->getOption(TR_TraceCG)) {
            if (numStaticPICs != 0)
                traceMsg(comp(),
                    "Interface dispatch with %d cache slots, added extra %d slot(s) for profiled classes.\n",
        ...
        TR::LabelSymbol* snippetLabel = generateLabelSymbol(cg());
        TR::S390InterfaceCallSnippet* ifcSnippet = new (trHeapMemory()) TR::S390InterfaceCallSnippet(
            cg(), callNode, snippetLabel, sizeOfArguments, numInterfaceCallCacheSlots, virtualThunk, false);
        ...
        if (numInterfaceCallCacheSlots == 0) {
            ...
            TR::RegisterDependencyConditions* postDeps
                = new (trHeapMemory()) TR::RegisterDependencyConditions(dependencies, 0, 3, cg());
            ...
            TR::Instruction* cursor
                = new (trHeapMemory()) TR::S390NOPInstruction(TR::InstOpCode::NOP, 2, callNode, cg());
            ...
            if (comp()->getOption(TR_enableInterfaceCallCachingSingleDynamicSlot)) {
                cursor = new (trHeapMemory()) TR::S390RILInstruction(
                    TR::InstOpCode::LARL, callNode, snippetReg, ifcSnippet->getDataConstantSnippet(), cg());
                ...
                if (TR::Compiler->target.is64Bit())
                    cursor = generateRXInstruction(cg(), TR::InstOpCode::LPQ, callNode, classMethodEPPairRegister,
                        generateS390MemoryReference(
                            snippetReg, ifcSnippet->getDataConstantSnippet()->getSingleDynamicSlotOffset(), cg()),
                        cursor);
                else
                ...
                cursor = generateS390LabelInstruction(
                    cg(), TR::InstOpCode::LABEL, callNode, paramSetupDummyLabel, preDeps, cursor);
                ...
                useCLFIandBRCL = false
                    && (TR::Compiler->target.is64Bit() && // Support for 64-bit
                           TR::Compiler->om
                               .generateCompressedObjectHeaders() // Classes are <2GB on CompressedRefs only.
                       );
                ...
                if (useCLFIandBRCL) {
                    ...
                    cursor = generateRILInstruction(
                        cg(), TR::InstOpCode::CLFI, callNode, vftReg, 0x0, cursor); // compare against 0
                    ...
                    for (i = 0; i < numInterfaceCallCacheSlots; i++) {
                        ...
                        cursor = generateRXInstruction(cg(), TR::InstOpCode::getLoadOpCode(), callNode, methodRegister,
                            generateS390MemoryReference(
                                snippetReg, slotOffset + TR::Compiler->om.sizeofReferenceAddress(), cg()),
                            cursor);
                ...
                cursor = generateS390LabelInstruction(
                    cg(), TR::InstOpCode::DC, callNode, ifcSnippet->getDataConstantSnippet()->getSnippetLabel());
...
TR::Instruction* TR::S390PrivateLinkage::buildDirectCall(TR::Node* callNode, TR::SymbolReference* callSymRef,
    TR::RegisterDependencyConditions* dependencies, int32_t argSize)
{
    ...
    if (isJitInduceOSR) {
        ...
        gcPoint = generateS390BranchInstruction(
            cg(), TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, callNode, snippetLabel, dependencies);
...
void TR::S390PrivateLinkage::callPreJNICallOffloadCheck(TR::Node* callNode)
{
    ...
    TR::SymbolReference* offloadOffSymRef
        = codeGen->symRefTab()->findOrCreateRuntimeHelper(TR_S390jitPreJNICallOffloadCheck, false, false, false);
    ...
    codeGen->addSnippet(new (trHeapMemory()) TR::S390HelperCallSnippet(
        codeGen, callNode, offloadOffSnippetLabel, offloadOffSymRef, offloadOffRestartLabel));
    generateS390LabelInstruction(codeGen, TR::InstOpCode::LABEL, callNode, offloadOffRestartLabel);
}

void TR::S390PrivateLinkage::callPostJNICallOffloadCheck(TR::Node* callNode)
{
    TR::CodeGenerator* codeGen = cg();
    ...
    TR::SymbolReference* offloadOnSymRef
        = codeGen->symRefTab()->findOrCreateRuntimeHelper(TR_S390jitPostJNICallOffloadCheck, false, false, false);
    codeGen->addSnippet(new (trHeapMemory()) TR::S390HelperCallSnippet(
        codeGen, callNode, offloadOnSnippetLabel, offloadOnSymRef, offloadOnRestartLabel));
    ...
    TR::RealRegister* javaStackPointerRealRegister, TR::Register* javaLitPoolVirtualRegister, TR::Register* tempReg)
{
    ...
    generateRXInstruction(codeGen, TR::InstOpCode::getAndOpCode(), callNode, tempReg,
        new (trHeapMemory()) TR::MemoryReference(
            javaStackPointerRealRegister, (int32_t)fej9->constJNICallOutFrameFlagsOffset(), codeGen));
    TR::Instruction* gcPoint = generateS390BranchInstruction(
        codeGen, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE, callNode, refPoolSnippetLabel);
    gcPoint->setNeedsGCMap(0);

    TR::SymbolReference* collapseSymRef
        = cg()->symRefTab()->findOrCreateRuntimeHelper(TR_S390collapseJNIReferenceFrame, false, false, false);
...
void TR::S390PrivateLinkage::setupJNICallOutFrame(TR::Node* callNode, TR::RealRegister* javaStackPointerRealRegister,
    ...
    TR::S390JNICallDataSnippet* jniCallDataSnippet)
{
    ...
    generateSS1Instruction(cg(), TR::InstOpCode::MVC, callNode, -stackAdjust - 1,
        ...
        new (trHeapMemory()) TR::MemoryReference(
            jniCallDataSnippet->getBaseRegister(), jniCallDataSnippet->getJNICallOutFrameDataOffset(), codeGen));
}
 ...
 *                but the helper call saves and restores all regs
 */
void TR::J9S390JNILinkage::releaseVMAccessMask(TR::Node* callNode, TR::Register* methodMetaDataVirtualRegister,
    ...
    TR::RegisterDependencyConditions* deps)
{
    ...
    generateRXInstruction(self()->cg(), TR::InstOpCode::getLoadOpCode(), callNode, methodAddressReg,
        generateS390MemoryReference(
            methodMetaDataVirtualRegister, fej9->thisThreadGetPublicFlagsOffset(), self()->cg()));
    ...
    generateRRInstruction(
        self()->cg(), TR::InstOpCode::getLoadRegOpCode(), callNode, javaLitOffsetReg, methodAddressReg);
    ...
    self()->cg()->addSnippet(new (self()->trHeapMemory()) TR::S390HelperCallSnippet(self()->cg(), callNode,
        ...
        self()->comp()->getSymRefTab()->findOrCreateReleaseVMAccessSymbolRef(self()->comp()->getJittedMethodSymbol()),
        cFlowRegionEnd));
    ...
    TR::Register* methodMetaDataVirtualRegister, TR::Register* methodAddressReg, TR::Register* javaLitOffsetReg)
{
    ...
    // marked MemRefMustNotSpill so that in case of long disp, we will resue the target reg as a scratch reg

    TR_J9VMBase* fej9 = (TR_J9VMBase*)(self()->fe());
    intptrj_t aValue = fej9->constAcquireVMAccessOutOfLineMask();

    TR::Instruction* loadInstr = (TR::Instruction*)genLoadAddressConstant(
        self()->cg(), callNode, aValue, methodAddressReg, NULL, NULL, javaLitPoolVirtualRegister);
    ...
    generateRRInstruction(
        self()->cg(), TR::InstOpCode::getXORRegOpCode(), callNode, javaLitOffsetReg, javaLitOffsetReg);
    ...
    generateRSInstruction(cg(), TR::InstOpCode::getCmpAndSwapOpCode(), callNode, javaLitOffsetReg, methodAddressReg,
        generateS390MemoryReference(
            methodMetaDataVirtualRegister, (int32_t)fej9->thisThreadGetPublicFlagsOffset(), self()->cg()));
    TR::Instruction* gcPoint = (TR::Instruction*)generateS390BranchInstruction(
        self()->cg(), TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE, callNode, longAcquireSnippetLabel);
    gcPoint->setNeedsGCMap(0);

    self()->cg()->addSnippet(new (self()->trHeapMemory()) TR::S390HelperCallSnippet(self()->cg(), callNode,
        longAcquireSnippetLabel,
        self()->comp()->getSymRefTab()->findOrCreateAcquireVMAccessSymbolRef(self()->comp()->getJittedMethodSymbol()),
        acquireDoneLabel));
 ...
 *
 */
void TR::J9S390JNILinkage::releaseVMAccessMaskAtomicFree(
    TR::Node* callNode, TR::Register* methodMetaDataVirtualRegister, TR::Register* tempReg1)
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)fe();
    ...
    TR_ASSERT_FATAL(
        J9_PUBLIC_FLAGS_VM_ACCESS >= MIN_IMMEDIATE_BYTE_VAL && J9_PUBLIC_FLAGS_VM_ACCESS <= MAX_IMMEDIATE_BYTE_VAL,
        "VM access bit must be immediate");
    generateRIEInstruction(cg, TR::InstOpCode::getCmpImmBranchRelOpCode(), callNode, tempReg1,
        J9_PUBLIC_FLAGS_VM_ACCESS, longReleaseSnippetLabel, TR::InstOpCode::COND_BNE);
 ...
 *
 * */
void TR::J9S390JNILinkage::acquireVMAccessMaskAtomicFree(
    TR::Node* callNode, TR::Register* methodMetaDataVirtualRegister, TR::Register* tempReg1)
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)fe();
    TR::CodeGenerator* cg = self()->cg();
    ...
    TR_ASSERT_FATAL(
        ...
        "VM access bit must be immediate");
    generateRIEInstruction(cg, TR::InstOpCode::getCmpImmBranchRelOpCode(), callNode, tempReg1,
        J9_PUBLIC_FLAGS_VM_ACCESS, longAcquireSnippetLabel, TR::InstOpCode::COND_BNE);
...
void TR::J9S390JNILinkage::checkException(
    TR::Node* callNode, TR::Register* methodMetaDataVirtualRegister, TR::Register* tempReg)
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(fe());
    // check exception
    ...
    generateRXInstruction(self()->cg(), TR::InstOpCode::getLoadOpCode(), callNode, tempReg,
        new (self()->trHeapMemory()) TR::MemoryReference(
            methodMetaDataVirtualRegister, fej9->thisThreadGetCurrentExceptionOffset(), self()->cg()));

    TR::Instruction* gcPoint
        = (TR::Instruction*)generateS390CompareAndBranchInstruction(self()->cg(), TR::InstOpCode::getCmpOpCode(),
            callNode, tempReg, (signed char)0, TR::InstOpCode::COND_BNE, exceptionSnippetLabel, false, true);
    gcPoint->setNeedsGCMap(0);

    self()->cg()->addSnippet(
        new (self()->trHeapMemory()) TR::S390HelperCallSnippet(self()->cg(), callNode, exceptionSnippetLabel,
            self()->comp()->getSymRefTab()->findOrCreateThrowCurrentExceptionSymbolRef(
...
TR::Register* TR::J9S390JNILinkage::buildDirectDispatch(TR::Node* callNode)
{
    ...
    killMask = killAndAssignRegister(killMask, deps, &methodAddressReg,
        (TR::Compiler->target.isLinux()) ? TR::RealRegister::GPR1 : TR::RealRegister::GPR9, codeGen, true);
    ...
    if (isPassJNIThread) {
        ...
        generateRRInstruction(
            codeGen, TR::InstOpCode::getLoadRegOpCode(), callNode, jniEnvRegister, methodMetaDataVirtualRegister);
    }
    ...
    if (isJNICallOutFrame || isReleaseVMAccess) {
        ...
        new (trHeapMemory()) TR::S390RILInstruction(
            TR::InstOpCode::LARL, callNode, jniCallDataSnippet->getBaseRegister(), jniCallDataSnippet, codeGen);
    ...
    if (isJNICallOutFrame) {
        ...
        generateRXInstruction(codeGen, TR::InstOpCode::getStoreOpCode(), callNode, javaStackPointerRealRegister,
            new (trHeapMemory()) TR::MemoryReference(
                methodMetaDataVirtualRegister, (int32_t)fej9->thisThreadGetJavaSPOffset(), codeGen));

        auto* literalOffsetMemoryReference = new (trHeapMemory()) TR::MemoryReference(
            methodMetaDataVirtualRegister, (int32_t)fej9->thisThreadGetJavaLiteralsOffset(), codeGen);

        // Set up literal offset slot to zero
        if (codeGen->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_z10)) {
            generateSILInstruction(
                codeGen, TR::InstOpCode::getMoveHalfWordImmOpCode(), callNode, literalOffsetMemoryReference, 0);
        } else {
            generateRIInstruction(codeGen, TR::InstOpCode::getLoadHalfWordImmOpCode(), callNode, javaLitOffsetReg, 0);

            generateRXInstruction(
                codeGen, TR::InstOpCode::getStoreOpCode(), callNode, javaLitOffsetReg, literalOffsetMemoryReference);
        }
    }
...
#else
        releaseVMAccessMask(
            callNode, methodMetaDataVirtualRegister, methodAddressReg, javaLitOffsetReg, jniCallDataSnippet, deps);
#endif
    }
...
#else
        acquireVMAccessMask(
            callNode, javaLitPoolVirtualRegister, methodMetaDataVirtualRegister, methodAddressReg, javaLitOffsetReg);
#endif
    }
    ...
    isUnwrapAddressReturnValue = (isUnwrapAddressReturnValue && (returnType == TR::Address)
        && (javaReturnRegister != NULL) && (javaReturnRegister->getKind() == TR_GPR));
    ...
    if (isUnwrapAddressReturnValue) {
        ...
        generateS390CompareAndBranchInstruction(codeGen, TR::InstOpCode::getCmpOpCode(), callNode, javaReturnRegister,
    ...
    OMR::Z::Linkage::generateDispatchReturnLable(
        callNode, codeGen, deps, javaReturnRegister, hasGlRegDeps, GlobalRegDeps);
...
////////////////////////////////////////////////////////////////////////////////
void TR::S390PrivateLinkage::doNotKillSpecialRegsForBuildArgs(TR::Linkage* linkage, bool isFastJNI, int64_t& killMask)
{
    TR::SystemLinkage* systemLinkage = (TR::SystemLinkage*)cg()->getLinkage(TR_System);
...
////////////////////////////////////////////////////////////////////////////////
void TR::S390PrivateLinkage::addSpecialRegDepsForBuildArgs(
    TR::Node* callNode, TR::RegisterDependencyConditions* dependencies, int32_t& from, int32_t step)
{
    ...
    if (specialArgReg != TR::RealRegister::NoReg) {
        ...
        TR::Register* specialArg = copyArgRegister(
            callNode, child, cg()->evaluate(child)); // TODO:JSR292: We don't need a copy of the highOrder reg on 31-bit
...
////////////////////////////////////////////////////////////////////////////////
int32_t TR::S390PrivateLinkage::storeExtraEnvRegForBuildArgs(TR::Node* callNode, TR::Linkage* linkage,
    ...
    uint32_t& numIntegerArgs)
{
    ...
    if (isFastJNI) // Account for extra parameter env
    {
        ...
        if (linkage->isAllParmsOnStack()) {
            TR::Register* stackRegister = linkage->getStackRegisterForOutgoingArguments(
                callNode, dependencies); // delay (possibly) creating this till needed
            storeArgumentOnStack(
                callNode, TR::InstOpCode::getStoreOpCode(), jniEnvRegister, &stackOffset, stackRegister);
        }
...
////////////////////////////////////////////////////////////////////////////////
int64_t TR::S390PrivateLinkage::addFECustomizedReturnRegDependency(
...
////////////////////////////////////////////////////////////////////////////////
TR::Register* TR::S390PrivateLinkage::buildDirectDispatch(TR::Node* callNode)
{
...
////////////////////////////////////////////////////////////////////////////////
TR::Register* TR::S390PrivateLinkage::buildIndirectDispatch(TR::Node* callNode)
{
...
////////////////////////////////////////////////////////////////////////////////
void TR::S390PrivateLinkage::mapIncomingParms(TR::ResolvedMethodSymbol* method)
{
    ...
    if (getRightToLeft()) {
        ...
        while (parmCursor != NULL) {
            ...
            {
                parmCursor->setParameterOffset(sizeOfParameterArea - parmCursor->getParameterOffset()
                    - parmCursor->getSize() * 2 + offsetToFirstParm);
            } else {
...
void TR::S390PrivateLinkage::setupBuildArgForLinkage(TR::Node* callNode, TR_DispatchType dispatchType,
    ...
    TR::Node* GlobalRegDeps, bool hasGlRegDeps, TR::SystemLinkage* systemLinkage)
{
    ...
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(fe());
    // call base class
    OMR::Z::Linkage::setupBuildArgForLinkage(
...
void TR::S390PrivateLinkage::setupRegisterDepForLinkage(TR::Node* callNode, TR_DispatchType dispatchType,
    ...
    TR::Node*& GlobalRegDeps, bool& hasGlRegDeps, TR::Register** methodAddressReg, TR::Register*& javaLitOffsetReg)
{
    ...
    if (dispatchType == TR_SystemDispatch) {
        killMask = killAndAssignRegister(killMask, deps, methodAddressReg,
            (TR::Compiler->target.isLinux()) ? TR::RealRegister::GPR14 : TR::RealRegister::GPR8, codeGen, true);
        killMask = killAndAssignRegister(killMask, deps, &javaLitOffsetReg,
            (TR::Compiler->target.isLinux()) ? TR::RealRegister::GPR8 : TR::RealRegister::GPR14, codeGen, true);
    }
    ...
    if (TR::Compiler->target.isZOS()) {
        ...
        if (systemStackRealRegister->getState() != TR::RealRegister::Locked) {
            ...
            killMask = killAndAssignRegister(
                killMask, deps, &systemStackVirtualRegister, systemStackPointerRegister, codeGen, true);
    ...
    if (codeGen->isLiteralPoolOnDemandOn()) {
        ...
        killMask = killAndAssignRegister(
            killMask, deps, &javaLitPoolVirtualRegister, javaLitPoolRealRegister, codeGen, true);
    ...
    // replacing it with TR::Compiler->target.isZOS().
    if (TR::Compiler->target.isZOS()) {
        TR_J9VMBase* fej9 = (TR_J9VMBase*)(fe());
        generateRXInstruction(codeGen, TR::InstOpCode::getLoadOpCode(), callNode, systemStackVirtualRegister,
            new (trHeapMemory()) TR::MemoryReference(
                methodMetaDataVirtualRegister, (int32_t)fej9->thisThreadGetSystemSPOffset(), codeGen));
    }
}
