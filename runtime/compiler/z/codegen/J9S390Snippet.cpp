...
uint8_t* TR::S390HeapAllocSnippet::emitSnippetBody()
{
    ...
    TR_ASSERT(jumpToCallDistance == -1 || jumpToCallDistance == (((intptrj_t)buffer) - branchToCallLocation),
...
uint32_t TR::S390HeapAllocSnippet::getLength(int32_t estimatedCodeStart)
{
    ...
    distance = estimatedCodeStart + (TR::Compiler->target.is64Bit() ? 10 : 8)
...
void TR::S390HeapAllocSnippet::print(TR::FILE* pOutFile, TR_Debug* debug)
{
    if (pOutFile == NULL) {
        return;
    }
    ...
    debug->printSnippetLabel(
        pOutFile, getSnippetLabel(), buffer, "HeapAlloc Snippet", debug->getName(getDestination()));
