...
static TR::Register* allocateWriteBarrierInternalPointerRegister(TR::CodeGenerator* cg, TR::Node* sourceChild)
{
    ...
    if (sourceChild->getRegister() != NULL && !cg->canClobberNodesRegister(sourceChild)) {
        ...
        generateRRInstruction(
            cg, TR::InstOpCode::getLoadRegOpCode(), sourceChild, sourceRegister, sourceChild->getRegister());
    } else {
 ...
 */
TR::Register* inlineVectorizedStringIndexOf(TR::Node* node, TR::CodeGenerator* cg, bool isUTF16)
{
    ...
    generateRIEInstruction(cg, TR::InstOpCode::getCmpRegAndBranchRelOpCode(), node, maxIndexReg, fromIndexReg,
        labelStringNotFound, TR::InstOpCode::COND_BLR);
    ...
    if (enableIndexOfDebugCounter) {
        ...
        generateRIEInstruction(cg, TR::InstOpCode::getCmpImmBranchRelOpCode(), node, s2LenReg, boundary10Char,
            labelS2LenGT10, TR::InstOpCode::COND_BH);
        ...
        generateRIEInstruction(cg, TR::InstOpCode::getCmpImmBranchRelOpCode(), node, s2LenReg, boundary30Char,
            labelS2LenGT30, TR::InstOpCode::COND_BH);
        ...
        generateRIEInstruction(cg, TR::InstOpCode::getCmpImmBranchRelOpCode(), node, s2LenReg, boundary60Char,
            labelS2LenGT60, TR::InstOpCode::COND_BH);
        ...
        generateRIEInstruction(cg, TR::InstOpCode::getCmpImmBranchRelOpCode(), node, s2LenReg, boundary100Char,
            labelS2LenGT100, TR::InstOpCode::COND_BH);
    ...
    generateVRXInstruction(cg, TR::InstOpCode::VLREP, node, s2Char1RepVReg,
        generateS390MemoryReference(s2ValueReg, headerSize, cg), elementSizeMask);
    ...
    generateRIEInstruction(
        cg, TR::InstOpCode::getAddHalfWordImmDistinctOperandOpCode(), node, loadLenReg, s1VecStartIndexReg, vectorSize);
    generateRIEInstruction(cg, TR::InstOpCode::getCmpRegAndBranchRelOpCode(), node, loadLenReg, s1LenReg,
        labelLoadLen16, TR::InstOpCode::COND_BNHR);
    generateRRRInstruction(
        cg, TR::InstOpCode::getSubtractThreeRegOpCode(), node, loadLenReg, s1LenReg, s1VecStartIndexReg);
    ...
    generateVRSbInstruction(
        cg, TR::InstOpCode::VLL, node, s1PartialVReg, loadLenReg, generateS390MemoryReference(tmpReg, headerSize, cg));
    generateVRRbInstruction(
        cg, TR::InstOpCode::VFEE, node, tmpVReg, s1PartialVReg, s2Char1RepVReg, 0x1, elementSizeMask);
    ...
    generateRIEInstruction(cg, TR::InstOpCode::getCmpRegAndBranchRelOpCode(), node, s1VecStartIndexReg, maxIndexReg,
        labelStringNotFound, TR::InstOpCode::COND_BHR);
    ...
    generateVRScInstruction(
        cg, TR::InstOpCode::VLGV, node, char1IndexReg, tmpVReg, generateS390MemoryReference(7, cg), 0);
    generateRIEInstruction(cg, TR::InstOpCode::getCmpRegAndBranchRelOpCode(), node, char1IndexReg, loadLenReg,
        labelStringNotFound, TR::InstOpCode::COND_BNLR);

    generateRRInstruction(cg, TR::InstOpCode::getAddRegOpCode(), node, char1IndexReg, s1VecStartIndexReg);
    generateRIEInstruction(cg, TR::InstOpCode::getCmpRegAndBranchRelOpCode(), node, char1IndexReg, maxIndexReg,
        labelStringNotFound, TR::InstOpCode::COND_BHR);
    ...
    generateRIEInstruction(cg, TR::InstOpCode::getCmpImmBranchRelOpCode(), node, loadLenReg, (int8_t)0,
        labelMatchS2LoopSetup, TR::InstOpCode::COND_BE);
    ...
    generateVRSbInstruction(
        cg, TR::InstOpCode::VLL, node, s1PartialVReg, loadLenReg, generateS390MemoryReference(tmpReg, headerSize, cg));
    generateVRSbInstruction(cg, TR::InstOpCode::VLL, node, s2PartialVReg, loadLenReg,
        generateS390MemoryReference(s2ValueReg, headerSize, cg));
    generateRIInstruction(cg, TR::InstOpCode::getAddHalfWordImmOpCode(), node, loadLenReg, 1);

    generateVRRbInstruction(
        cg, TR::InstOpCode::VCEQ, node, tmpVReg, s1PartialVReg, s2PartialVReg, 1, elementSizeMask); // 1 for set CC
    generateS390BranchInstruction(
        cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_CC0, node, labelMatchS2LoopSetup); // cc == 0 means residue match

    // The residue does not match. Continue to find the 1st char in s1, starting from the next element.
    generateRIEInstruction(cg, TR::InstOpCode::getAddHalfWordImmDistinctOperandOpCode(), node, s1VecStartIndexReg,
        char1IndexReg, isUTF16 ? 2 : 1);
    generateRIEInstruction(cg, TR::InstOpCode::getCmpRegAndBranchRelOpCode(), node, s1VecStartIndexReg, maxIndexReg,
        labelStringNotFound, TR::InstOpCode::COND_BHR);
    generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, node, labelFindS2Head);
    ...
    generateVRXInstruction(cg, TR::InstOpCode::VL, node, s1PartialVReg,
        generateS390MemoryReference(s1ValueReg, s1VecStartIndexReg, headerSize, cg));
    generateVRXInstruction(cg, TR::InstOpCode::VL, node, s2PartialVReg,
        generateS390MemoryReference(s2ValueReg, s2VecStartIndexReg, headerSize, cg));
    ...
    generateRIEInstruction(cg, TR::InstOpCode::getCmpImmBranchRelOpCode(), node, loopCountReg, (int8_t)0,
        labelMatchS2Loop, TR::InstOpCode::COND_BNE);
...
TR::Register* caseConversionHelper(TR::Node* node, TR::CodeGenerator* cg, bool isToUpper, bool isCompressedString)
{
    ...
    // Characters àáâãäåæçèéêëìíîïðñòóôõö (0xE0-0xF6) when to upper and ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖ (0xC0-0xD6) when to
    // lower
    ...
    // Condition code equal for capital sharp and mu (bit 0=0x80) and greater than (bit 2=0x20) for codes larger than
    // 0xFF
    if (isToUpper) {
        if (isCompressedString) {
            ...
            generateVRRaInstruction(
                cg, TR::InstOpCode::VUPLH, node, invalidRangeVector, invalidRangeVector, 0, 0, 0, 0);
    ...
    generateVRIaInstruction(
        cg, TR::InstOpCode::VREPI, node, charOffsetVector, static_cast<uint16_t>(0x20), elementSizeMask);
    ...
    generateVRReInstruction(
        cg, TR::InstOpCode::VSEL, node, modifiedCaseVector, modifiedCaseVector, charBufferVector, selectionVector);
    ...
    generateRIEInstruction(
        cg, TR::InstOpCode::CIJ, node, lengthRegister, sizeOfVector, success, TR::InstOpCode::COND_BL);
...
extern TR::Register* intrinsicIndexOf(TR::Node* node, TR::CodeGenerator* cg, bool isCompressed)
{
    ...
    generateVRIcInstruction(cg, TR::InstOpCode::VREP, node, valueVector, valueVector,
        (sizeOfVector / (1 << elementSizeMask)) - 1, elementSizeMask);
    ...
    generateRXInstruction(cg, TR::InstOpCode::LA, node, offsetAddress,
        generateS390MemoryReference(address, indexRegister, headerSize, cg));
    generateVRSbInstruction(
        cg, TR::InstOpCode::VLL, node, charBufferVector, loadLength, generateS390MemoryReference(offsetAddress, 0, cg));

    generateVRRbInstruction(
        cg, TR::InstOpCode::VFEE, node, resultVector, charBufferVector, valueVector, 0x1, elementSizeMask);
    generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_MASK1, node, notFoundInResidue);

    generateVRScInstruction(
        cg, TR::InstOpCode::VLGV, node, scratch, resultVector, generateS390MemoryReference(7, cg), 0);
    generateRIEInstruction(
        cg, TR::InstOpCode::CRJ, node, scratch, loadLength, foundLabelExtractedScratch, TR::InstOpCode::COND_BNH);
...
TR::Instruction* J9::Z::TreeEvaluator::genLoadForObjectHeaders(
...
TR::Instruction* J9::Z::TreeEvaluator::genLoadForObjectHeadersMasked(
    TR::CodeGenerator* cg, TR::Node* node, TR::Register* reg, TR::MemoryReference* tempMR, TR::Instruction* iCursor)
{
...
static inline TR::Instruction* genNullTest(
    TR::CodeGenerator* cg, TR::Node* node, TR::Register* tgtReg, TR::Register* srcReg, TR::Instruction* cursor)
{
    TR::Instruction* iRet;
...
static TR::Instruction* genTestIsSuper(TR::CodeGenerator* cg, TR::Node* node, TR::Register* objClassReg,
    TR::Register* castClassReg, TR::Register* scratch1Reg, TR::Register* scratch2Reg, TR::Register* resultReg,
    TR::Register* litPoolBaseReg, int32_t castClassDepth, TR::LabelSymbol* failLabel, TR::LabelSymbol* trueLabel,
    TR::LabelSymbol* callHelperLabel, TR::RegisterDependencyConditions* conditions, TR::Instruction* cursor,
    bool addDataSnippetAsSecondaryCache, TR::Register* classObjectClazzSnippetReg,
    TR::Register* instanceOfClazzSnippetReg)
{
    ...
    bool eliminateSuperClassArraySizeCheck
        = (!dynamicCastClass && (castClassDepth < cg->comp()->getOptions()->_minimumSuperclassArraySize));
    ...
    if (dynamicCastClass) {
        ...
        TR_ASSERT(
            (node->getOpCodeValue() == TR:: instanceof &&node->getSecondChild()->getOpCodeValue() != TR::loadaddr),
            "genTestIsSuper: castClassDepth == -1 is only supported for transformed isInstance calls.");
        ...
        cursor = generateRILInstruction(cg, TR::InstOpCode::NILF, node, scratch1Reg,
            static_cast<int32_t>((J9AccInterface | J9AccClassArray)), cursor);

        if (debugObj)
            debugObj->addInstructionComment(
                cursor, "Check if castClass is an interface or class array and jump to helper sequence");

        // insert snippet check
        if (addDataSnippetAsSecondaryCache) {
            cursor = generateS390BranchInstruction(
                cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BE, node, notInterfaceLabel, cursor);
            ...
            cursor = generateRXInstruction(cg, TR::InstOpCode::getCmpOpCode(), node, objClassReg,
                generateS390MemoryReference(classObjectClazzSnippetReg, 0, cg), cursor);
            cursor = generateS390BranchInstruction(
                cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE, node, callHelperLabel, cursor);
            cursor = generateRXInstruction(cg, TR::InstOpCode::getCmpOpCode(), node, castClassReg,
                generateS390MemoryReference(instanceOfClazzSnippetReg, 0, cg), cursor);
            cursor = generateS390BranchInstruction(
                cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BE, node, trueLabel, cursor);
            cursor = generateS390BranchInstruction(
                cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, node, callHelperLabel, cursor);
        ...
        if (dynamicCastClass)
            ...
            if (TR::Compiler->target.is64Bit()) {
                ...
                cursor = generateLoad32BitConstant(
                    cg, node, castClassDepth, scratch2Reg, false, cursor, conditions, litPoolBaseReg);
            }
            generateCompareAndBranchIsPossible = true;
        } else {
            cursor = generateRIInstruction(
                cg, TR::InstOpCode::getCmpHalfWordImmOpCode(), node, scratch1Reg, castClassDepth, cursor);
        }

        if (generateCompareAndBranchIsPossible)
            cursor = generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::getCmpRegOpCode(), node, scratch1Reg,
                scratch2Reg, TR::InstOpCode::COND_BNH, failLabel, false, false);
        else
            cursor = generateS390BranchInstruction(
                cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNH, node, failLabel, cursor);
    ...
    bool needsResult, bool trueFallThrough, TR::CodeGenerator* cg)
{
    ...
    generateRILInstruction(
        cg, TR::InstOpCode::NILF, node, scratchReg1, static_cast<int32_t>(J9AccClassInternalPrimitiveType));
...
static TR::Register* establishLitPoolBaseReg(TR::Node* castOrInstanceOfNode, TR::CodeGenerator* cg)
{
    if (castOrInstanceOfNode->getNumChildren() != 3) {
        ...
        TR_ASSERT(
            (litPoolBaseChild->getOpCodeValue() == TR::aload) || (litPoolBaseChild->getOpCodeValue() == TR::aRegLoad),
...
static bool generateInlineTest(TR::CodeGenerator* cg, TR::Node* node, TR::Node* castClassNode,
    TR::Register* objClassReg, TR::Register* resultReg, TR::Register* scratchReg, TR::Register* litPoolReg,
    bool needsResult, TR::LabelSymbol* falseLabel, TR::LabelSymbol* trueLabel, TR::LabelSymbol* doneLabel,
    bool isCheckCast, int32_t maxNum_PICS = NUM_PICS)
{
    TR::Compilation* comp = cg->comp();
    ...
    if (isCheckCast) {
        ...
        uint8_t numberOfGuessClasses
            = TR::TreeEvaluator::interpreterProfilingInstanceOfOrCheckCastInfo(cg, node, tempGuessClassArray);
    ...
    for (i = 0; i < num_PICs; i++) {
        ...
        if (cg->needClassAndMethodPointerRelocations())
            unloadableConstInstr[i] = generateRegLitRefInstruction(cg, TR::InstOpCode::getLoadOpCode(), node,
                scratchReg, (uintptrj_t)guessClassArray[i], TR_ClassPointer, NULL, NULL, NULL);
        else
            unloadableConstInstr[i]
                = generateRILInstruction(cg, TR::InstOpCode::LARL, node, scratchReg, guessClassArray[i]);
        ...
        if (needsResult)
            generateRIInstruction(
                cg, TR::InstOpCode::getLoadHalfWordImmOpCode(), node, resultReg, (int32_t)result_bool);
        generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::getCmpLogicalRegOpCode(), node, objClassReg,
            scratchReg, TR::InstOpCode::COND_BE, result_label);
    }
    return true;
}
static void generateTestBitFlag(
    TR::CodeGenerator* cg, TR::Node* node, TR::Register* mdReg, int32_t offset, int32_t size, uint64_t bitFlag)
{
    ...
    TR::RegisterDependencyConditions* conditions, TR::CodeGenerator* cg, bool doCompileTimeCheckForHeapObj = true)
{
    ...
    if (gcMode != TR_WrtbarAlways) {
        ...
        if (constantHeapCase) {
            // these return uintptrj_t but because of the if(constantHeapCase) they are guaranteed to be <=
            ...
            if (!doCrdMrk && !disableSrcObjCheck) {
                ...
                generateS390CompareAndBranchInstruction(cg, is64Bit ? TR::InstOpCode::CLG : TR::InstOpCode::CL, node,
                    temp1Reg, heapSize, TR::InstOpCode::COND_BH, doneLabel, false, false, NULL, conditions);
            }
        } else {
            TR::MemoryReference* offset = generateS390MemoryReference(
                cg->getMethodMetaDataRealRegister(), offsetof(J9VMThread, heapBaseForBarrierRange0), cg);
            TR::MemoryReference* size = generateS390MemoryReference(
                cg->getMethodMetaDataRealRegister(), offsetof(J9VMThread, heapSizeForBarrierRange0), cg);
            generateRRInstruction(cg, opLoadReg, node, temp1Reg, owningObjectReg);
        ...
        if (!TR::Options::getCmdLineOptions()->realTimeGC()) {
            ...
            if (doCrdMrk && !isDefinitelyNonHeapObj) {
                ...
                if (!(gcMode == TR_WrtbarCardMarkIncremental || gcMode == TR_WrtbarRealTime)) {
                    ...
                    generateS390BranchInstruction(
                        cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BE, node, srcObjChkLabel);
                }
                ...
                generateSIInstruction(
                    cg, TR::InstOpCode::MVI, node, generateS390MemoryReference(cardOffReg, 0x0, cg), CARD_DIRTY);
        ...
        if (!disableSrcObjCheck && !(!doCrdMrk && constantHeapCase)) {
            ...
            if (constantHeapCase) {
                ...
                uint32_t heapSize = comp->getOptions()->getHeapSizeForBarrierRange0();
                generateRILInstruction(
                    cg, is64Bit ? TR::InstOpCode::SLGFI : TR::InstOpCode::SLFI, node, temp1Reg, heapBase);
                generateS390CompareAndBranchInstruction(cg, is64Bit ? TR::InstOpCode::CLG : TR::InstOpCode::CL, node,
                    temp1Reg, heapSize, TR::InstOpCode::COND_BL, doneLabel, false);
            } else {
                TR::MemoryReference* offset = generateS390MemoryReference(
                    cg->getMethodMetaDataRealRegister(), offsetof(J9VMThread, heapBaseForBarrierRange0), cg);
                TR::MemoryReference* size = generateS390MemoryReference(
    ...
    && !defined(J9VM_INTERP_COMPRESSED_OBJECT_HEADER)
        offsetToAgeBits += 4;
#endif
    ...
    TR::LabelSymbol* doneLabel = NULL, bool doCompileTimeCheckForHeapObj = true)
{
    if (!TR::Options::getCmdLineOptions()->realTimeGC()) {
        ...
        TR_ASSERT((comp->getOptions()->getGcMode() == TR_WrtbarCardMark
...
static void VMwrtbarEvaluator(
    TR::Node* node, TR::Register* srcReg, TR::Register* owningObjectReg, bool srcNonNull, TR::CodeGenerator* cg)
{
    TR::Instruction* cursor;
    TR::Compilation* comp = cg->comp();
    TR_WriteBarrierKind gcMode = comp->getOptions()->getGcMode();
    ...
    if (doWrtBar) // generational or gencon
    {
        ...
        VMnonNullSrcWrtBarCardCheckEvaluator(
            node, owningObjectReg, srcReg, epReg, raReg, doneLabel, wbRef, conditions, cg, false);
...
///////////////////////////////////////////////////////////////////////////////////////
TR::Register* J9::Z::TreeEvaluator::monexitfenceEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register* J9::Z::TreeEvaluator::asynccheckEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.is32Bit() && (firstChild->getOpCodeValue() == TR::iload)
...
static void generateNullChkSnippet(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Compilation* comp = cg->comp();
    TR::LabelSymbol* snippetLabel = generateLabelSymbol(cg);
    TR::S390BranchInstruction* brInstr = (TR::S390BranchInstruction*)generateS390BranchInstruction(
        cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BE, node, snippetLabel);
 ...
 */
TR::Register* J9::Z::TreeEvaluator::generateHelperCallForVMNewEvaluators(
    TR::Node* node, TR::CodeGenerator* cg, bool doInlineAllocation, TR::Register* resReg)
{
    TR::S390CHelperLinkage* helperLink = static_cast<TR::S390CHelperLinkage*>(cg->getLinkage(TR_CHelper));
    TR::ILOpCodes opCode = node->getOpCodeValue();
    TR::Node* helperCallNode = TR::Node::createWithSymRef(
        node, TR::acall, (opCode == TR::New || opCode == TR::variableNew) ? 1 : 2, node->getSymbolReference());
    ...
    if (!(opCode == TR::New || opCode == TR::variableNew)) {
        ...
        // After helper call is generated we decrese reference count of this node so that a register will be marked dead
        // for RA.
...
///////////////////////////////////////////////////////////////////////////////////////
TR::Register* J9::Z::TreeEvaluator::resolveCHKEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register* J9::Z::TreeEvaluator::DIVCHKEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getNumChildren() != 0 && secondChild->getOpCode().isMemoryReference()
        ...
        // [0x00000200075228cc] (  1)      iiload #251[0x000002000745c940]+12  Shadow[<array-size>]   <flags:"0x1100"
    ...
    if (cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_z10) && divisorIsFieldAccess
        ...
        if (dtype.isInt64() && TR::Compiler->target.is32Bit() && !cg->use64BitRegsOn32Bit()) {
            ...
            TR::S390RIEInstruction* cursor = new (cg->trHeapMemory())
                TR::S390RIEInstruction(TR::InstOpCode::CLFIT, node, tempReg, (int16_t)0, TR::InstOpCode::COND_BE, cg);
            cursor->setExceptBranchOp();
    ...
    // z9 legacy instructions
        ...
        if (!constDivisor || (dtype.isInt32() && secondChild->getInt() == 0)
            ...
            if (constDivisor) {
                cursor = generateS390BranchInstruction(
                    cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, node, snippetLabel);
                cursor->setExceptBranchOp();
            } else {
...
TR::Register* J9::Z::TreeEvaluator::BNDCHKEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (firstChild->getOpCodeValue() == TR::l2i && firstChild->isSingleRefUnevaluated() && firstChild->getFirstChild()
    ...
    if (secondChild->getOpCodeValue() == TR::l2i && secondChild->isSingleRefUnevaluated()
        && secondChild->getFirstChild() && secondChild->getFirstChild()->getRegister()) {
    ...
    if (useS390CompareAndTrap && (arrayIndexReg != NULL && arrayLengthReg != NULL)) {
        ...
        if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
            ...
            if (firstChild->getInt() > secondChildConstValue && secondChildConstValue >= 0) {
                ...
                cursor = generateS390BranchInstruction(
                    cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, node, boundCheckFailureLabel);
        ...
        if (oneConst && constValue <= MAX_UNSIGNED_IMMEDIATE_VAL && // CLFIT takes 16bit unsigned immediate
            ...
            TR::InstOpCode::S390BranchCondition bc = alwaysFailBNDCHK
                ? TR::InstOpCode::COND_BRC
                : isForward ? TR::InstOpCode::COND_BNH : TR::InstOpCode::COND_BNL;
            ...
            cursor = generateRSInstruction(cg, opCode, node, regChild->getRegister(),
                getMaskForBranchCondition(compareCondition), generateS390MemoryReference(memChild, cg));
            cursor->setExceptBranchOp();
            cg->setCanExceptByTrap(true);
            cursor->setNeedsGCMap(0x0000FFFF);
            ...
            if (alwaysFailBNDCHK) {
                ...
                cursor = generateS390CompareAndBranchInstruction(
                    cg, TR::InstOpCode::CL, node, testRegister, constValue, bc, boundCheckFailureLabel, false, true);
            }
    ...
    if (cg->getHasResumableTrapHandler() && !disableS390CompareAndTrap && arrayTargetLengthReg != NULL
        ...
        if (firstChild->getOpCode().isLoadConst()) {
            ...
            // check if we can use Compare-and-Branch at least
                ...
                && secondChild->getOpCode().isLoadConst() && secondChild->getInt() <= MAX_IMMEDIATE_BYTE_VAL
            ...
            // z9
                ...
                instr = generateS390CompareOps(
                    node, cg, TR::InstOpCode::COND_BL, TR::InstOpCode::COND_BH, boundCheckFailureLabel);
...
TR::Register* J9::Z::TreeEvaluator::BNDCHKwithSpineCHKEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (needsBoundCheck) {
        ...
        cursor = generateRSInstruction(
            cg, TR::InstOpCode::ICM, node, tmpReg, (uint32_t)0xF, contiguousArraySizeMR2, cursor);
        cg->stopUsingRegister(tmpReg);

        // Throw AIOB if continuousArraySizeMR is zero.
        cursor = generateS390BranchInstruction(
            cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE, node, boundCheckFailureLabel, cursor);
        cursor->setExceptBranchOp();
        ...
        cursor = generateS390BranchInstruction(
            cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNL, node, boundCheckFailureLabel, cursor);
        cursor->setExceptBranchOp();
        cg->addSnippet(new (cg->trHeapMemory())
                TR::S390HelperCallSnippet(cg, node, boundCheckFailureLabel, node->getSymbolReference()));
    }
    ...
    if (useCompressedPointers) {
        ...
        if (heapBaseValue != 0)
            cursor = generateRegLitRefInstruction(
                cg, TR::InstOpCode::AG, node, tmpReg, (int64_t)heapBaseValue, NULL, cursor, NULL, false);
    }
    ...
    if (TR::Compiler->target.is64Bit()) {
        if (cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_zEC12)) {
            ...
            cursor = generateRSInstruction(
                cg, TR::InstOpCode::SRLG, node, tmpReg2, tmpReg2, 64 - spineShift - elementShift, cursor);
        }
    } else {
        generateShiftAndKeepSelected31Bit(
            node, cg, tmpReg2, indexReg, 32 - spineShift - elementShift, 31 - elementShift, elementShift, true, false);
    }
    ...
    if (!actualLoadOrStoreChild->getOpCode().isStore()) {
        ...
        if ((!actualLoadOrStoreChild->getOpCode().hasSymbolReference()
            ...
            case TR::Int8:
                ...
                break;
            case TR::Int16:
                if (actualLoadOrStoreChild->getOpCode().isShort()) {
        ...
        if (doLoadDecompress) {
            ...
            if (cmpRefsShift == 1) {
                ...
                cursor = generateRSInstruction(
                    cg, TR::InstOpCode::SLLG, node, loadOrStoreReg, loadOrStoreReg, cmpRefsShift, cursor);
            }
            ...
            if (heapBaseValue != 0)
                cursor = generateRegLitRefInstruction(
                    cg, TR::InstOpCode::AG, node, loadOrStoreReg, (int64_t)heapBaseValue, NULL, cursor, NULL, false);
        }
            ...
            case TR::Int64:
                if (TR::Compiler->target.is64Bit()) {
                    ...
                    cursor = generateRXInstruction(
                        cg, TR::InstOpCode::ST, node, valueReg->getLowOrder(), arrayletMR, cursor);
    ...
    cursor = generateS390BranchInstruction(
        cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, node, oolReturnLabel, cursor);
    if (debugObj)
...
static void VMarrayStoreCHKEvaluator(TR::Node* node, TR::S390CHelperLinkage* helperLink, TR::Node* callNode,
    ...
    TR::RegisterDependencyConditions* conditions, TR::CodeGenerator* cg)
{
...
#ifdef J9VM_INTERP_COMPRESSED_OBJECT_HEADER
    cursor = generateS390CompareAndBranchInstruction(
        cg, TR::InstOpCode::CR, node, t1Reg, srcRegVal, TR::InstOpCode::COND_BER, wbLabel, false, false);
#else
    ...
    if (doObjectArrayCheck
        && (cg->wantToPatchClassPointer((TR_OpaqueClassBlock*)objectClass, node)
        ...
        if (cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_z10) && cg->isLiteralPoolOnDemandOn()) {
            ...
            if (TR::Compiler->target.is64Bit()) {
                ...
                cursor = (TR::S390RILInstruction*)generateRILInstruction(
                    cg, TR::InstOpCode::CLGRL, node, t1Reg, targetsnippet, 0);
            } else {
                targetsnippet = cg->findOrCreate4ByteConstant(node, (int32_t)objectClass);
                cursor = (TR::S390RILInstruction*)generateRILInstruction(
                    cg, TR::InstOpCode::CLRL, node, t1Reg, targetsnippet, 0);
            }
            ...
            if (cg->needClassAndMethodPointerRelocations()) {
                ...
                AOTcgDiag4(comp,
                    ...
                    targetsnippet->getSymbolReference()->getSymbol(), TR_ClassPointer);
            }
        } else {
            if (cg->needClassAndMethodPointerRelocations()) {
                generateRegLitRefInstruction(cg, TR::InstOpCode::getLoadOpCode(), node, t2Reg, (uintptrj_t)objectClass,
                    TR_ClassPointer, conditions, NULL, NULL);
            } else {
                genLoadAddressConstantInSnippet(
                    cg, node, (intptr_t)objectClass, t2Reg, cursor, conditions, litPoolBaseReg, true);
            }
    ...
    // TODO Attching helper call predependency to BRASL instruction and combine ICF conditions with post dependency
    ...
    static_assert(J9_JAVA_CLASS_DEPTH_MASK == 0xffff,
        "VMarrayStoreCHKEvaluator::J9_JAVA_CLASS_DEPTH_MASK should have be 16 bit of ones");
...
TR::Register* J9::Z::TreeEvaluator::ArrayStoreCHKEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    bool nopASC = false;
    if (comp->performVirtualGuardNOPing() && node->getArrayStoreClassInNode() && !comp->getOption(TR_DisableOOL)
    ...
    if (doWrtBar || doCrdMrk || (classChild == sourceChild)) {
        ...
        if (sourceChild->getOpCode().isLoadVar() && sourceChild->getRegister() == NULL && !sourceChild->isNonNull()) {
            ...
            generateRXInstruction(cg, TR::InstOpCode::getLoadTestOpCode(), sourceChild, srcReg,
                generateS390MemoryReference(sourceChild, cg));
    ...
    if (!sourceChild->isNonNull()) {
        ...
        TR::InstOpCode::Mnemonic compareAndBranchMnemonic
            = usingCompressedPointers ? TR::InstOpCode::C : TR::InstOpCode::getCmpOpCode();

        generateS390CompareAndBranchInstruction(cg, compareAndBranchMnemonic, node, srcReg, 0, TR::InstOpCode::COND_BE,
            (doWrtBar || doCrdMrk) ? simpleStoreLabel : wbLabel, false, true);
    }
    TR::S390CHelperLinkage* helperLink = static_cast<TR::S390CHelperLinkage*>(cg->getLinkage(TR_CHelper));
    if (nopASC) {
    ...
    if (doWrtBar) {
        ...
        VMnonNullSrcWrtBarCardCheckEvaluator(
            firstChild, classReg, srcReg, tyReg, txReg, cFlowRegionEnd, wbRef, conditions, cg, false);
    } else if (doCrdMrk) {
...
TR::Register* J9::Z::TreeEvaluator::VMgenCoreInstanceofEvaluator(TR::Node* node, TR::CodeGenerator* cg,
    ...
    TR::RegisterDependencyConditions* baseConditions, bool isIfInstanceof)
{
    TR::Compilation* comp = cg->comp();
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(comp->fe());
    TR::LabelSymbol* doneLabel = generateLabelSymbol(cg);
    ...
    bool addDataSnippetForSuperTest
        = isIfInstanceof && dynamicClassPointer && comp->getOption(TR_EnableOnsiteCacheForSuperClassTest);
    ...
    dumpOptDetails(comp,
        "\nInstanceOf: testEqual:%d testSuper: %d testCache: %d needsHelper:%d, dynamicClassPointer:%d falseLabel:%p "
        "trueLabel:%p",
        testEqualClass, testCastClassIsSuper, testCache, needsHelperCall, dynamicClassPointer, falseLabel, trueLabel);
    dumpOptDetails(comp,
        "\nInstanceOf: addDataSnippetForSuperTest: %d, performReferenceArrayTestInline: %d, true fall through:%d need "
        "result:%d\n",
        addDataSnippetForSuperTest, performReferenceArrayTestInline, trueFallThrough, needsResult);
    if (baseConditions) {
        conditions = new (cg->trHeapMemory())
            TR::RegisterDependencyConditions(baseConditions, 16, maxInstanceOfPostDependencies(), cg);
    } else {
    ...
    if (testCastClassIsSuper || testCache) {
        ...
        int32_t superClassOffset = castClassDepth * TR::Compiler->om.sizeofReferenceAddress();
        bool outOfBound
            = (superClassOffset > MAX_IMMEDIATE_VAL || superClassOffset < MIN_IMMEDIATE_VAL || dynamicClassPointer)
            ? true
            : false;
...
#endif
        {
            // On 64-bit systems trampolines may kill the EP register so we need to add it to post-dependencies. If
            // there is no OOL path we need to assign any real register to scratch1Reg.
            scratch1RegAssignment = (needsHelperCall)
                ? static_cast<TR::RealRegister::RegDep>(cg->getEntryPointRegister())
    ...
    cg->generateDebugCounter(
        TR::DebugCounter::debugCounterName(comp, "instanceOfStats/(%s)/MethodEntry", comp->signature()), 1,
    ...
    TR::TreeEvaluator::genLoadForObjectHeadersMasked(cg, node, objClassReg,
        generateS390MemoryReference(objectReg, (int32_t)TR::Compiler->om.offsetOfObjectVftField(), cg), NULL);
    ...
    if (performReferenceArrayTestInline) {
        ...
        cg->generateDebugCounter(
            ...
            TR::DebugCounter::Undetermined);
        genIsReferenceArrayTest(node, objClassReg, scratch1Reg, scratch2Reg, needsResult ? resultReg : NULL, falseLabel,
            trueLabel, needsResult, trueFallThrough, cg);
    }

    if (testCache) {
        cg->generateDebugCounter(
            TR::DebugCounter::debugCounterName(comp, "instanceOfStats/(%s)/Profiled", comp->signature()), 1,
            TR::DebugCounter::Undetermined);
        generateInlineTest(cg, node, castClassNode, objClassReg, resultReg, scratch1Reg, litPoolReg, needsResult,
            falseLabel, trueLabel, doneLabel, false);
        cg->generateDebugCounter(
            TR::DebugCounter::debugCounterName(comp, "instanceOfStats/(%s)/ProfiledFail", comp->signature()), 1,
            TR::DebugCounter::Undetermined);
        ...
        cg->generateDebugCounter(
            TR::DebugCounter::debugCounterName(comp, "instanceOfStats/(%s)/CacheTest", comp->signature()), 1,
            TR::DebugCounter::Undetermined);
        if (cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_zEC12)) {
        ...
        generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::getCmpRegOpCode(), node, castClassReg, scratch1Reg,
            TR::InstOpCode::COND_BNE, doneTestCacheLabel, false, false);
        cg->generateDebugCounter(
            TR::DebugCounter::debugCounterName(comp, "instanceOfStats/(%s)/CacheClassSuccess", comp->signature()), 1,
            TR::DebugCounter::Undetermined);
        if (needsResult) {
            // For cases when cached value has a result (ie: instanceof)
            // value at offsetof(J9Class, castClassCache) is actually j9class + last bit is set for the result of
            // instanceof: 1: false, 0: true, so we need to check and set resultsReg the opposite (0: false, 1: true)
    ...
    if (testEqualClass) {
        ...
        cg->generateDebugCounter(
            ...
            TR::DebugCounter::Undetermined);
        generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::getCmpRegOpCode(), node, objClassReg, castClassReg,
            TR::InstOpCode::COND_BE, trueLabel, false, false);
        cg->generateDebugCounter(
    ...
    if (testCastClassIsSuper) {
        cg->generateDebugCounter(
            ...
            TR::DebugCounter::Undetermined);
        generateInlineTest(cg, node, castClassNode, objClassReg, resultReg, scratch1Reg, litPoolReg, needsResult,
            continueLabel, trueLabel, doneLabel, false, 1);
        cg->generateDebugCounter(
        ...
        if (addDataSnippetForSuperTest) {
            ...
            if (classObjectClazzSnippet == NULL || instanceOfClazzSnippet == NULL) {
                ...
                generateRILInstruction(
                    cg, TR::InstOpCode::LARL, node, objectClazzSnippetReg, classObjectClazzSnippet, 0);
                generateRILInstruction(
                    cg, TR::InstOpCode::LARL, node, instanceOfClazzSnippetReg, instanceOfClazzSnippet, 0);
            }
        }
        cg->generateDebugCounter(
            TR::DebugCounter::debugCounterName(comp, "instanceOfStats/(%s)/SuperClassTest", comp->signature()), 1,
            TR::DebugCounter::Undetermined);
        cg->generateDebugCounter(
            TR::DebugCounter::debugCounterName(comp, "instanceOfStats/(%s)/MethodExit", comp->signature()), 1,
            TR::DebugCounter::Undetermined);
        genTestIsSuper(cg, node, objClassReg, castClassReg, scratch1Reg, scratch2Reg, needsResult ? resultReg : NULL,
            litPoolReg, castClassDepth, falseLabel, trueLabel, callHelper, conditions, NULL, addDataSnippetForSuperTest,
            objectClazzSnippetReg, instanceOfClazzSnippetReg);
        generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BE, node, trueLabel);
    }
    ...
    if (needsHelperCall) {
        ...
        if (dynamicClassPointer) {
            ...
            outlinedSlowPath = new (cg->trHeapMemory()) TR_S390OutOfLineCodeSection(callHelper, doneOOLLabel, cg);
            cg->getS390OutOfLineCodeSectionList().push_front(outlinedSlowPath);
            outlinedSlowPath->swapInstructionListsWithCompilation();
        ...
        cg->generateDebugCounter(TR::DebugCounter::debugCounterName(comp, "instanceOf/(%s)/Helper", comp->signature()),
            1, TR::DebugCounter::Undetermined);
        cg->generateDebugCounter(
            TR::DebugCounter::debugCounterName(comp, "instanceOfStats/(%s)/Helper", comp->signature()), 1,
            TR::DebugCounter::Undetermined);
        ...
        // when Snippet is Added, do Update the dataSnippet with the result of the call. only when the callResult is
        // true.
        if (addDataSnippetForSuperTest) {
            // when Snippet is Added, do Update the dataSnippet with the result of the call. only when the callResult is
            // true.
             ...
             * compare with "-1 loded register", dataSnippet1, and if not update datasnippet with objectClassReg(Compare
             * and Swap instr) if we didnot update, we don't update the next one->branch out to doneUpdateSnippetLabel
            ...
            generateS390BranchInstruction(
                cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BE, node, doneUpdateSnippetLabel);
            ...
            generateRSInstruction(cg, TR::InstOpCode::getCmpAndSwapOpCode(), node, tempNeg1LoadedRegister,
                tempObjectClassReg, generateS390MemoryReference(objectClazzSnippetReg, 0, cg));
            generateS390BranchInstruction(
                cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE, node, doneUpdateSnippetLabel);

            generateRXInstruction(cg, TR::InstOpCode::getStoreOpCode(), node, castClassReg,
                generateS390MemoryReference(instanceOfClazzSnippetReg, 0, cg));
            ...
            generateRRInstruction(cg, TR::InstOpCode::getLoadTestRegOpCode(), node, callResult,
                callResult); // condition code needs to be re-set. this is secondCache specific so included in this
                             // block.
        }
...
static TR::Register* reservationLockEnter(TR::Node* node, int32_t lwOffset, TR::Register* objectClassReg,
    TR::CodeGenerator* cg, TR::S390CHelperLinkage* helperLink)
{
    ...
    instr = generateS390CompareAndBranchInstruction(
        cg, compareOp, node, valReg, monitorReg, TR::InstOpCode::COND_BNE, resLabel, false, false);
    ...
    if (outlinedSlowPath) // Means we have OOL
    {
        ...
        generateS390CompareAndBranchInstruction(
            cg, compareImmOp, node, monitorReg, 0, TR::InstOpCode::COND_BNE, reserved_checkLabel, false);
        if (!isPrimitive) {
        ...
        generateS390CompareAndBranchInstruction(
            cg, compareOp, node, tempReg, valReg, TR::InstOpCode::COND_BNE, callLabel, false, false);
        ...
        if (debugObj) {
            if (isPrimitive)
                debugObj->addInstructionComment(
                    instr, "Denotes end of OOL primitive reservation enter sequence: return to mainline");
            else
                debugObj->addInstructionComment(
                    instr, "Denotes end of OOL non-primitive reservation enter sequence: return to mainline");
        }
    ...
    TR::CodeGenerator* cg, TR::S390CHelperLinkage* helperLink)
{
    ...
    if (!isPrimitive) {
        ...
        instr = generateS390CompareAndBranchInstruction(
            cg, compareOp, node, tempReg, monitorReg, TR::InstOpCode::COND_BNE, resLabel, false, false);
        ...
        instr = generateS390CompareAndBranchInstruction(
            cg, compareImmOp, node, tempReg, LOCK_RESERVATION_BIT, TR::InstOpCode::COND_BNE, resLabel, false);
    }
    ...
    if (!isPrimitive) {
        generateRIInstruction(
            cg, use64b ? TR::InstOpCode::AGHI : TR::InstOpCode::AHI, node, tempReg, -LOCK_INC_DEC_VALUE);
    ...
    if (outlinedSlowPath) // Means we have OOL
    {
        ...
        if (isPrimitive)
            generateS390BranchInstruction(
                cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE, node, helperReturnOOLLabel);
        else {
            generateS390BranchInstruction(
                cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BE, node, callLabel /*,conditions*/);
        }
        ...
        if (!isPrimitive)
            generateS390BranchInstruction(
                cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, node, helperReturnOOLLabel);
        // call to jithelper
        generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, callLabel);
        ...
        if (debugObj) {
            if (isPrimitive)
                debugObj->addInstructionComment(
                    instr, "Denotes end of OOL primitive reversation exit sequence: return to mainline");
            else
                debugObj->addInstructionComment(
                    instr, "Denotes end of OOL non-primitive reversation exit sequence: return to mainline");
        }
        outlinedSlowPath->swapInstructionListsWithCompilation(); // Toggle instruction list
        instr = generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, doneOOLLabel);
        if (debugObj)
 ...
 */
static bool genInstanceOfOrCheckcastSuperClassTest(TR::Node* node, TR::CodeGenerator* cg, TR::Register* objClassReg,
    TR::Register* castClassReg, int32_t castClassDepth, TR::LabelSymbol* falseLabel, TR::LabelSymbol* callHelperLabel,
    TR_S390ScratchRegisterManager* srm)
{
    ...
    bool eliminateSuperClassArraySizeCheck
        = (!dynamicCastClass && (castClassDepth < cg->comp()->getOptions()->_minimumSuperclassArraySize));
    // In case of dynamic Cast Class, We do not know the depth of the cast Class at compile time. So following routine
    // compares depth at run time.
    if (dynamicCastClass) {
        ...
        TR_ASSERT(((J9AccInterface | J9AccClassArray) < UINT_MAX && (J9AccInterface | J9AccClassArray) > 0),
            "genTestIsSuper::(J9AccInterface | J9AccClassArray) is not a 32-bit number\n");
        cursor = generateRILInstruction(cg, TR::InstOpCode::NILF, node, scratchRegister1,
            static_cast<int32_t>((J9AccInterface | J9AccClassArray)), cursor);
        cursor = generateS390BranchInstruction(
            cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE, node, callHelperLabel, cursor);
    ...
    if (!eliminateSuperClassArraySizeCheck) {
        ...
        if (dynamicCastClass)
            cursor = generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::getCmpRegOpCode(), node,
                objectClassDepthReg, castClassDepthReg, TR::InstOpCode::COND_BNH, falseLabel, false, false);
        else
            cursor = generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::getCmpOpCode(), node,
                objectClassDepthReg, castClassDepth, TR::InstOpCode::COND_BNH, falseLabel, true, false, cursor);
    ...
    if (dynamicCastClass) {
        if (TR::Compiler->target.is64Bit()) {
            cursor = generateRSInstruction(
                cg, TR::InstOpCode::SLLG, node, castClassDepthReg, castClassDepthReg, 3, cursor);
        } else {
...
{
    const bool isNullTestImplicit
        = node->getOpCodeValue() == TR::checkcastAndNULLCHK && cg->getHasResumableTrapHandler();

    if (isNullTestImplicit) {
        TR::Instruction* compareAndTrapInsturction = generateRIEInstruction(
            cg, TR::InstOpCode::getCmpImmTrapOpCode(), node, objectReg, 0, TR::InstOpCode::COND_BE);
 ...
 *
 *  \details
 *     This funcition generates a sequence to check per site cache for object class and cast class before calling out to
...
static void genInstanceOfDynamicCacheAndHelperCall(TR::Node* node, TR::CodeGenerator* cg, TR::Register* castClassReg,
    TR::Register* objClassReg, TR::Register* resultReg, TR_S390ScratchRegisterManager* srm, TR::LabelSymbol* doneLabel,
    ...
    bool cacheCastClass, bool ifInstanceOf, bool trueFallThrough)
{
     ...
     */
    int32_t snippetSizeInBytes
        = ((cacheCastClass ? 2 : 1) * maxOnsiteCacheSlots * addressSize) + (addressSize * (maxOnsiteCacheSlots != 1));
    if (generateDynamicCache) {
        TR::S390WritableDataSnippet* dynamicCacheSnippet = NULL;
        /* We can only request the snippet size of power 2, following table summarizes bytes needed for corresponding
         * number of cache slots Case 1 : Cast class is runtime variable Case 2 : Cast class is interface / unresolved
        ...
        // NOTE: For single slot cache, we initialize snippet with addressSize (4/8) assuming which can not be
        // objectClass In all cases, we use first addressSize bytes to store offset of the circular buffer and rest of
        // buffer will be initialized with 0xf.
        ...
        for (auto i = 0; i < maxOnsiteCacheSlots; i++) {
            if (cacheCastClass) {
                ...
                generateRXInstruction(cg, TR::InstOpCode::getCmpOpCode(), node, castClassReg,
                    generateS390MemoryReference(dynamicCacheReg, currentIndex + addressSize, cg));
                generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE, node, gotoNextTest);
            }
            generateRXInstruction(cg, TR::InstOpCode::getLoadOpCode(), node, cachedObjectClass,
            ...
            if (i + 1 == maxOnsiteCacheSlots) {
                if (trueFallThrough) {
                    ...
                    generateS390BranchInstruction(
                        cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE, node, helperCallLabel);
                } else {
                    generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BE, node, trueLabel);
                    generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::getCmpOpCode(), node, cachedObjectClass,
    ...
    if (generateDynamicCache) {
        ...
        generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, cFlowRegionStart);
        cFlowRegionStart->setStartInternalControlFlow();
        generateRIEInstruction(cg, TR::Compiler->target.is64Bit() ? TR::InstOpCode::CGIJ : TR::InstOpCode::CIJ, node,
            resultReg, (uint8_t)1, skipSettingBitForFalseResult, TR::InstOpCode::COND_BE);
        ...
        if (maxOnsiteCacheSlots == 1) {
            ...
            generateRXInstruction(cg, TR::InstOpCode::getStoreOpCode(), node, objClassReg,
                generateS390MemoryReference(dynamicCacheReg, 0, cg));
            if (cacheCastClass)
                generateRXInstruction(cg, TR::InstOpCode::getStoreOpCode(), node, castClassReg,
                    generateS390MemoryReference(dynamicCacheReg, addressSize, cg));
        } else {
            ...
            // NOTE: In OOL helper call is not within ICF hence we can avoid passing dependency to helper call dispatch
            // function and stretching it to merge label. Although internal control flow starts after returning from
            // helper we need to define starting point and ending point of internal control flow.
            generateRXInstruction(cg, TR::InstOpCode::getLoadOpCode(), node, offsetRegister,
                generateS390MemoryReference(dynamicCacheReg, 0, cg));
            generateRXInstruction(cg, TR::InstOpCode::getStoreOpCode(), node, objClassReg,
                generateS390MemoryReference(dynamicCacheReg, offsetRegister, 0, cg));
            if (cacheCastClass)
                generateRXInstruction(cg, TR::InstOpCode::getStoreOpCode(), node, castClassReg,
                    generateS390MemoryReference(dynamicCacheReg, offsetRegister, addressSize, cg));
            generateRIInstruction(cg, TR::InstOpCode::getAddHalfWordImmOpCode(), node, offsetRegister,
                static_cast<int32_t>(cacheCastClass ? addressSize * 2 : addressSize));
            generateRIEInstruction(cg, TR::InstOpCode::CIJ, node, offsetRegister, snippetSizeInBytes,
                skipResetOffsetLabel, TR::InstOpCode::COND_BNE);
            ...
            generateRXInstruction(cg, TR::InstOpCode::getStoreOpCode(), node, offsetRegister,
                generateS390MemoryReference(dynamicCacheReg, 0, cg));
    ...
    // In case if there is GLRegDeps attached to ifIntsanceOf node, it will be evaluated and attached as post dependency
 ...
 * ifInstanceOf case.
 */
TR::Register* J9::Z::TreeEvaluator::VMgenCoreInstanceofEvaluator2(TR::Node* node, TR::CodeGenerator* cg,
    ...
    TR::RegisterDependencyConditions* graDeps, bool ifInstanceOf)
{
    ...
    if (needResult) {
        resultReg = cg->allocateRegister();
        cursor = generateRIInstruction(
            cg, TR::InstOpCode::getLoadHalfWordImmOpCode(), node, resultReg, static_cast<int32_t>(initialResult));
    }
    ...
    while (numSequencesRemaining > 1 || (numSequencesRemaining == 1 && *iter != HelperCall)) {
        ...
        case LoadObjectClass:
            ...
            TR::TreeEvaluator::genLoadForObjectHeadersMasked(cg, node, objClassReg,
                generateS390MemoryReference(
                    objectReg, static_cast<int32_t>(TR::Compiler->om.offsetOfObjectVftField()), cg),
                NULL);
            break;
        case GoToTrue:
        ...
        case ClassEqualityTest:
             ...
             * COND_BNE ) brnachLabel = ifInstanceOf ? (!trueFallThrough ? trueLabel : falseLabel ) : doneLabel CGRJ
            ...
            break;
        case SuperClassTest: {
            /*** genInstanceOfOrCheckcastSuperClassTest generates sequences for Super Class Test handling all cases when
             * we have a normal static class or dynamic class Mostly this will be last test except in case of dynamic
             * cast class. case-1 instanceof , initial Result = false: BRC 0x8, doneLabel case-2 instanceof , initial
             * Result = true: BRC 0x6, doneLabel case-3 ifInstanceOf , trueLabel == branchLabel : BRC 0x8, branchLabel
            ...
            cg->generateDebugCounter(
                TR::DebugCounter::debugCounterName(comp, "instanceOfStats/(%s)/SuperClassTest", comp->signature()), 1,
                TR::DebugCounter::Undetermined);
            // For dynamic cast class genInstanceOfOrCheckcastSuperClassTest will generate branch to either helper call
            // or dynamicCacheTest depending on the next generated test.
            dynamicCastClass = genInstanceOfOrCheckcastSuperClassTest(node, cg, objClassReg, castClassReg,
                castClassDepth, falseLabel,
                *(iter + 1) == DynamicCacheDynamicCastClassTest ? dynamicCacheTestLabel : callLabel, srm);
        ...
        case ProfiledClassTest: {
            ...
            while (numPICs < numberOfProfiledClass) {
                if (cg->needClassAndMethodPointerRelocations())
                    temp = generateRegLitRefInstruction(cg, TR::InstOpCode::getLoadOpCode(), node, arbitraryClassReg1,
                        (uintptrj_t)profiledClassesList[numPICs].profiledClass, TR_ClassPointer, NULL, NULL, NULL);
                else
                    temp = generateRILInstruction(
                        cg, TR::InstOpCode::LARL, node, arbitraryClassReg1, profiledClassesList[numPICs].profiledClass);

                // Adding profiled class to the static PIC slots.
                if (fej9->isUnloadAssumptionRequired(
                        (TR_OpaqueClassBlock*)(profiledClassesList[numPICs].profiledClass), comp->getCurrentMethod()))
        ...
        case CompileTimeGuessClassTest: {
            ...
            cg->generateDebugCounter(
                TR::DebugCounter::debugCounterName(comp, "instanceOfStats/(%s)/compTimeGuess", comp->signature()), 1,
                TR::DebugCounter::Undetermined);
            ...
            cg->generateDebugCounter(
                TR::DebugCounter::debugCounterName(comp, "instanceOfStats/(%s)/compTimeGuessFail", comp->signature()),
                1, TR::DebugCounter::Undetermined);
        ...
        case DynamicCacheDynamicCastClassTest: {
            ...
            TR_ASSERT(dynamicCacheTestLabel != NULL,
                "DynamicCacheDynamicCastClassTest: dynamicCacheTestLabel should be generated by SuperClassTest before "
                "reaching this point");
            if (comp->getOption(TR_TraceCG))
    ...
    if (numSequencesRemaining > 0 && *iter == HelperCall)
        genInstanceOfDynamicCacheAndHelperCall(node, cg, castClassReg, objClassReg, resultReg, srm, doneLabel,
            callLabel, dynamicCacheTestLabel, branchLabel, trueLabel, falseLabel, dynamicCastClass,
    ...
    TR::RegisterDependencyConditions* conditions
        = new (cg->trHeapMemory()) TR::RegisterDependencyConditions(graDeps, 0, 4 + srm->numAvailableRegisters(), cg);
    if (objClassReg)
        conditions->addPostCondition(objClassReg, TR::RealRegister::AssignAny);
    if (needResult)
        conditions->addPostCondition(resultReg, TR::RealRegister::AssignAny);
 ...
 * instanceOf Otherwise calls VMgenCoreInstanceOfEvaluator2with parameters to generate instructions for ifInstanceOf.
 */
TR::Register* J9::Z::TreeEvaluator::VMifInstanceOfEvaluator2(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register* J9::Z::TreeEvaluator::VMifInstanceOfEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::TreeEvaluator::VMgenCoreInstanceofEvaluator(
        instanceOfNode, cg, falseLabel, trueLabel, needResult, trueFallThrough, graDeps, true);
...
TR::Register* J9::Z::TreeEvaluator::VMinstanceOfEvaluator2(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Compilation* comp = cg->comp();
...
TR::Register* J9::Z::TreeEvaluator::VMinstanceOfEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Compilation* comp = cg->comp();
 ...
 */
TR::Register* J9::Z::TreeEvaluator::VMcheckcastEvaluator2(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Compilation* comp = cg->comp();
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(comp->fe());
    ...
    // We need here at maximum two scratch registers so forcing scratchRegisterManager to create pool of two registers
    // only.
    TR_S390ScratchRegisterManager* srm = cg->generateScratchRegisterManager(2);
    ...
    while (numSequencesRemaining > 1) {
        ...
        case ClassEqualityTest:
            ...
            if (outLinedTest && !comp->getOption(TR_DisableOOL)) {
                ...
                cg->generateDebugCounter(
                    TR::DebugCounter::debugCounterName(comp, "checkCastStats/(%s)/EqualOOLPass", comp->signature()), 1,
                    TR::DebugCounter::Undetermined);
                outlinedSlowPath
        ...
        case SuperClassTest: {
            ...
            if (comp->getOption(TR_TraceCG))
                traceMsg(comp, "%s: Emitting Super Class Test, Cast Class Depth=%d\n", node->getOpCode().getName(),
                    castClassDepth);
            dynamicCastClass = genInstanceOfOrCheckcastSuperClassTest(
                node, cg, objClassReg, castClassReg, castClassDepth, callLabel, NULL, srm);
             ...
             * if the result of SuperClassTest is true and branch to resultLabel which will branch back to the doneLabel
             * from OOL code. In normal cases SuperClassTest will be inlined with doneLabel as fallThroughLabel so we
             * need to branch to callLabel to generate CastClassException through helper call if result of
             * SuperClassTest turned out to be false.
             */
...
TR::Register* J9::Z::TreeEvaluator::VMcheckcastEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Compilation* comp = cg->comp();
    ...
    TR::LabelSymbol *doneLabel, *callLabel, *startOOLLabel, *doneOOLLabel, *helperReturnOOLLabel, *resultLabel,
        *continueLabel;
    ...
    if (needsNullTest) {
        ...
        if (cg->getHasResumableTrapHandler() && isCheckcastAndNullChk) {
            ...
            TR::S390RIEInstruction* cursor = new (cg->trHeapMemory()) TR::S390RIEInstruction(
                TR::InstOpCode::getCmpImmTrapOpCode(), node, objReg, (int16_t)0, TR::InstOpCode::COND_BE, cg);
            cursor->setExceptBranchOp();
            cursor->setNeedsGCMap(0x0000FFFF);
        } else if (needsNullTest && !objNode->getRegister() && !objNode->getOpCode().isLoadConst()
            ...
            if ((objNode->getOpCode().isLoadIndirect() || objNode->getOpCodeValue() == TR::aload)
                && !sym->isInternalPointer()) {
    ...
    if (!testCache && !testEqualClass && !testCastClassIsSuper) {
        cg->generateDebugCounter(
            TR::DebugCounter::debugCounterName(comp, "checkCastStats/(%s)/Helper", comp->signature()), 1,
            TR::DebugCounter::Undetermined);
        cg->generateDebugCounter(TR::DebugCounter::debugCounterName(comp, "checkCast/(%s)/Helper", comp->signature()),
            1, TR::DebugCounter::Undetermined);
    ...
    if (testEqualClass) {
        TR::TreeEvaluator::genLoadForObjectHeadersMasked(cg, node, objClassReg,
            generateS390MemoryReference(compareReg, (int32_t)TR::Compiler->om.offsetOfObjectVftField(), cg), NULL);
        ...
        if (testCastClassIsSuper) {
            ...
            TR_OpaqueClassBlock* castClassAddr = TR::TreeEvaluator::getCastClassAddress(castClassNode);
            TR_OpaqueClassBlock* topGuessClassAddr
                = TR::TreeEvaluator::interpreterProfilingInstanceOfOrCheckCastInfo(cg, node);
            ...
            if (!comp->getOption(TR_DisableOOL) && castClassAddr == topGuessClassAddr && topProb >= 0.5) {
                ...
                cg->generateDebugCounter(
                    TR::DebugCounter::debugCounterName(comp, "checkCastStats/(%s)/EqualOOL", comp->signature()), 1,
                    TR::DebugCounter::Undetermined);
                TR::Instruction* temp = generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::getCmpRegOpCode(),
                    node, castClassReg, objClassReg, TR::InstOpCode::COND_BNE, startOOLLabel, false, false);
                cg->generateDebugCounter(
                    TR::DebugCounter::debugCounterName(comp, "checkCastStats/(%s)/EqualOOLPass", comp->signature()), 1,
                    TR::DebugCounter::Undetermined);
                if (debugObj)
                ...
                cg->generateDebugCounter(
                    TR::DebugCounter::debugCounterName(comp, "checkCastStats/(%s)/Equal", comp->signature()), 1,
                    TR::DebugCounter::Undetermined);
                gcPoint = generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::getCmpRegOpCode(), node,
                    castClassReg, objClassReg, TR::InstOpCode::COND_BE, doneLabel, false, false);
                cg->generateDebugCounter(
                    TR::DebugCounter::debugCounterName(comp, "checkCastStats/(%s)/EqualFail", comp->signature()), 1,
                    TR::DebugCounter::Undetermined);
            }
            ...
            cg->generateDebugCounter(
                TR::DebugCounter::debugCounterName(comp, "checkCastStats/(%s)/Equal", comp->signature()), 1,
                TR::DebugCounter::Undetermined);
            gcPoint = generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::getCmpRegOpCode(), node, castClassReg,
                objClassReg, TR::InstOpCode::COND_BNE, callLabel, false, false);
            cg->generateDebugCounter(
                TR::DebugCounter::debugCounterName(comp, "checkCastStats/(%s)/EqualPass", comp->signature()), 1,
                TR::DebugCounter::Undetermined);
        }
    }
    ...
    TR::Snippet* snippet = new (cg->trHeapMemory())
        TR::S390HelperCallSnippet(cg, node, callLabel, node->getSymbolReference(), resultLabel);
    cg->addSnippet(snippet);
    ...
    if (testCache) {
        cg->generateDebugCounter(
            TR::DebugCounter::debugCounterName(comp, "checkCastStats/(%s)/Profiled", comp->signature()), 1,
            TR::DebugCounter::Undetermined);
        generateInlineTest(cg, node, castClassNode, objClassReg, NULL, scratch1Reg, litPoolBaseReg, false, resultLabel,
            resultLabel, resultLabel, true);
        cg->generateDebugCounter(
            TR::DebugCounter::debugCounterName(comp, "checkCastStats/(%s)/ProfiledFail", comp->signature()), 1,
            TR::DebugCounter::Undetermined);
        ...
        if (testCastClassIsSuper) {
            gcPoint
                = generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BE, node, resultLabel);
            cg->generateDebugCounter(
                TR::DebugCounter::debugCounterName(comp, "checkCastStats/(%s)/CacheFail", comp->signature()), 1,
                TR::DebugCounter::Undetermined);
        } else {
            ...
            cg->generateDebugCounter(
                TR::DebugCounter::debugCounterName(comp, "checkCastStats/(%s)/CacheSuccess", comp->signature()), 1,
                TR::DebugCounter::Undetermined);
        }
    }
    ...
    if (testCastClassIsSuper) {
        ...
        cg->generateDebugCounter(
            TR::DebugCounter::debugCounterName(comp, "checkCastStats/(%s)/Profiled", comp->signature()), 1,
            TR::DebugCounter::Undetermined);
        generateInlineTest(cg, node, castClassNode, objClassReg, NULL, scratch1Reg, litPoolBaseReg, false,
            continueLabel, resultLabel, resultLabel, true, 1);
        cg->generateDebugCounter(
            TR::DebugCounter::debugCounterName(comp, "checkCastStats/(%s)/ProfiledFail", comp->signature()), 1,
            TR::DebugCounter::Undetermined);
        generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, continueLabel);
        ...
        cg->generateDebugCounter(
            TR::DebugCounter::debugCounterName(comp, "checkCastStats/(%s)/SuperClass", comp->signature()), 1,
            TR::DebugCounter::Undetermined);
        genTestIsSuper(cg, node, objClassReg, castClassReg, scratch1Reg, scratch2Reg, NULL, litPoolBaseReg,
            castClassDepth, callLabel, NULL, NULL, conditions, NULL, false, NULL, NULL);
        gcPoint = generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE, node, callLabel);
...
TR::Register* J9::Z::TreeEvaluator::VMmonentEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (comp->getOption(TR_OptimizeForSpace) || (comp->getOption(TR_FullSpeedDebug) && node->isSyncMethodMonitor())
    ...
    if (lwOffset <= 0) {
        ...
        TR::MemoryReference* tempMR
            = generateS390MemoryReference(objReg, TR::Compiler->om.offsetOfObjectVftField(), cg);
        // TODO We don't need objectClassReg except in this ifCase. We can use scratchRegisterManager to allocate one
        // here.
        ...
        if (TR::Compiler->target.is64Bit())
            generateRXInstruction(
                cg, TR::InstOpCode::LA, node, tempRegister, generateS390MemoryReference(objReg, tempRegister, 0, cg));
        else
        ...
        if (comp->getOption(TR_EnableMonitorCacheLookup)) {
            ...
            if (cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_zEC12)
                ...
                generateRIEInstruction(
                    cg, TR::InstOpCode::RISBGN, node, lookupOffsetReg, objReg, start, end + 0x80, shiftAmount);
            else if (cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_z10)
                && TR::Compiler->target.is64Bit())
                generateRIEInstruction(
                    cg, TR::InstOpCode::RISBG, node, lookupOffsetReg, objReg, start, end + 0x80, shiftAmount);
            else {
            ...
            TR::MemoryReference* temp2MR = generateS390MemoryReference(
                cg->getMethodMetaDataRealRegister(), lookupOffsetReg, offsetOfMonitorLookupCache, cg);

#if defined(J9VM_INTERP_COMPRESSED_OBJECT_HEADER)
            generateRXInstruction(cg, TR::InstOpCode::LLGF, node, tempRegister, temp2MR, NULL);
            startICF = generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::getCmpOpCode(), node, tempRegister,
                NULLVALUE, TR::InstOpCode::COND_BE, helperCallLabel, false, true);
#else
            generateRXInstruction(cg, TR::InstOpCode::getLoadTestOpCode(), node, tempRegister, temp2MR);

            startICF = generateS390BranchInstruction(
                cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BE, node, helperCallLabel);
#endif
            ...
            if (peekFirst) {
                generateRXInstruction(
                    cg, TR::InstOpCode::C, node, monitorReg, generateS390MemoryReference(baseReg, lwOffset, cg));
                generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE, node, helperCallLabel);
            }
            ...
            if (!disableOOL) {
                ...
                if (debugObj)
                    debugObj->addInstructionComment(
                        cursor, "Denotes end of OOL monent monitorCacheLookup: return to mainline");
...
TR::Register* J9::Z::TreeEvaluator::VMmonexitEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    int32_t lwOffset = fej9->getByteOffsetToLockword((TR_OpaqueClassBlock*)cg->getMonClass(node));
    TR::S390CHelperLinkage* helperLink = static_cast<TR::S390CHelperLinkage*>(cg->getLinkage(TR_CHelper));
    if (comp->getOption(TR_OptimizeForSpace) || comp->getOption(TR_DisableInlineMonExit)
        || comp->getOption(TR_FullSpeedDebug)) // Required for Live Monitor Meta Data in FSD.
    {
        TR::ILOpCodes opCode = node->getOpCodeValue();
        TR::Node::recreate(node, TR::call);
    ...
    if (lwOffset <= 0) {
        ...
        if (comp->getOption(TR_EnableMonitorCacheLookup)) {
            ...
            if (TR::Compiler->target.is64Bit() && fej9->generateCompressedLockWord())
                generateRXInstruction(
                    cg, TR::InstOpCode::C, node, metaReg, generateS390MemoryReference(baseReg, lwOffset, cg));
            else
                generateRXInstruction(cg, TR::InstOpCode::getCmpOpCode(), node, metaReg,
                    generateS390MemoryReference(baseReg, lwOffset, cg));

            // If VMThread does not match, call helper.
            TR::Instruction* helperBranch = generateS390BranchInstruction(
                cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE, node, helperCallLabel);
            ...
            if (!cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_z10)) {
                if (TR::Compiler->target.is64Bit() && fej9->generateCompressedLockWord()) {
                    generateRRInstruction(cg, TR::InstOpCode::XR, node, monitorReg, monitorReg);
                    gcPoint = generateRXInstruction(
                        cg, TR::InstOpCode::ST, node, monitorReg, generateS390MemoryReference(baseReg, lwOffset, cg));
                } else {
                    generateRRInstruction(cg, TR::InstOpCode::getXORRegOpCode(), node, monitorReg, monitorReg);
                    gcPoint = generateRXInstruction(cg, TR::InstOpCode::getStoreOpCode(), node, monitorReg,
                        generateS390MemoryReference(baseReg, lwOffset, cg));
                }
            } else {
                if (TR::Compiler->target.is64Bit() && fej9->generateCompressedLockWord())
                    gcPoint = generateSILInstruction(
                        cg, TR::InstOpCode::MVHI, node, generateS390MemoryReference(baseReg, lwOffset, cg), 0);
                else
            ...
            if (!disableOOL) {
                ...
                if (debugObj)
                    debugObj->addInstructionComment(
                        cursor, "Denotes end of OOL monexit monitorCacheLookup: return to mainline");
    ...
    if (inlineRecursive) {
        ...
        if (!normalLockWithReservationPreserving) {
            ...
            startICF = generateS390CompareAndBranchInstruction(
                cg, compareOp, node, scratchRegister, metaReg, TR::InstOpCode::COND_BNE, callHelper, false, false);
...
static void roundArrayLengthToObjectAlignment(TR::CodeGenerator* cg, TR::Node* node, TR::Instruction*& iCursor,
    TR::Register* dataSizeReg, TR::RegisterDependencyConditions* conditions, TR::Register* litPoolBaseReg,
    int32_t allocSize, int32_t elementSize, TR::Register* sizeReg, TR::LabelSymbol* exitOOLLabel = NULL)
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(cg->fe());
    ...
    bool canCombineAGRs = (((allocSize % alignmentConstant) == 0) && (elementSize < alignmentConstant));

    if (!canCombineAGRs)
        iCursor = generateRRInstruction(cg, TR::InstOpCode::getLoadRegOpCode(), node, sizeReg, dataSizeReg, iCursor);

    if (needsAlignment) {
        iCursor = generateRIInstruction(
            cg, TR::InstOpCode::getAddHalfWordImmOpCode(), node, sizeReg, alignmentConstant - 1 + allocSize, iCursor);
        if (TR::Compiler->target.is64Bit())
...
static void genHeapAlloc(TR::Node* node, TR::Instruction*& iCursor, bool isVariableLen, TR::Register* enumReg,
    TR::Register* resReg, TR::Register* zeroReg, TR::Register* dataSizeReg, TR::Register* sizeReg,
    TR::LabelSymbol* callLabel, int32_t allocSize, int32_t elementSize, TR::CodeGenerator* cg,
    TR::Register* litPoolBaseReg, TR::RegisterDependencyConditions* conditions, TR::Instruction*& firstBRCToOOL,
    TR::Instruction*& secondBRCToOOL, TR::LabelSymbol* exitOOLLabel = NULL)
{
    TR::Compilation* comp = cg->comp();
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(comp->fe());
    if (!TR::Options::getCmdLineOptions()->realTimeGC()) {
        ...
        if (isVariableLen) {
            ...
            iCursor = generateS390BranchInstruction(
                cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE, node, callLabel, iCursor);
            if (!firstBRCToOOL) {
        ...
        if (isVariableLen) {
            ...
            roundArrayLengthToObjectAlignment(
                cg, node, iCursor, dataSizeReg, conditions, litPoolBaseReg, allocSize, elementSize, sizeReg);
        ...
        if (allocSize > cg->getMaxObjectSizeGuaranteedNotToOverflow()) {
            iCursor = generateS390BranchInstruction(
                cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BO, node, callLabel, iCursor);
            if (!firstBRCToOOL) {
                firstBRCToOOL = iCursor;
            } else {
                secondBRCToOOL = iCursor;
        ...
        if (!comp->getOption(TR_DisableDualTLH) && useDualTLH && node->canSkipZeroInitialization()) {
            ...
            // opt is enabled
            iCursor = generateS390BranchInstruction(
                cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BH, node, callLabel, iCursor);
            if (!firstBRCToOOL) {
                firstBRCToOOL = iCursor;
            } else {
                secondBRCToOOL = iCursor;
        ...
        if (disableBatchClear
            && disableInitClear == NULL) //&& (node->getOpCodeValue() == TR::anewarray) &&
                                         //(node->getFirstChild()->getInt()>0) && (node->getFirstChild()->getInt()<6) )
        {
            ...
            iCursor = generateS390BranchInstruction(
                cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BZ, node, fillerRemLabel, iCursor);
            ...
            iCursor = generateRXInstruction(
                cg, TR::InstOpCode::LA, node, addressReg, generateS390MemoryReference(addressReg, 256, cg), iCursor);
            ...
            iCursor = generateS390BranchInstruction(
                cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BL, node, doneLabel, iCursor);
...
static void genInitObjectHeader(TR::Node* node, TR::Instruction*& iCursor, TR_OpaqueClassBlock* classAddress,
    TR::Register* classReg, TR::Register* resReg, TR::Register* zeroReg, TR::Register* temp1Reg,
    TR::Register* litPoolBaseReg, TR::RegisterDependencyConditions* conditions, TR::CodeGenerator* cg,
    TR::Register* enumReg = NULL, bool canUseIIHF = false)
{
    TR::Compilation* comp = cg->comp();
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(comp->fe());
    ...
    if (!TR::Options::getCmdLineOptions()->realTimeGC()) {
        ...
        if (comp->compileRelocatableCode()) {
            if (node->getOpCodeValue() == TR::newarray) {
                ...
                iCursor = generateRXInstruction(cg, TR::InstOpCode::getLoadOpCode(), node, temp1Reg,
                    generateS390MemoryReference(
                        temp1Reg, fej9vm->getPrimitiveArrayOffsetInJavaVM(node->getSecondChild()->getInt()), cg),
                    iCursor);
                clzReg = temp1Reg;
                ...
                TR_ASSERT(
                    node->getOpCodeValue() == TR::New && classReg, "must have a classReg for TR::New in AOT mode");
                clzReg = classReg;
            }
        }
        ...
        if (clzReg == NULL) {
            if (cg->wantToPatchClassPointer(classAddress, node)) {
                iCursor = genLoadAddressConstantInSnippet(cg, node, (intptr_t)classAddress | (intptrj_t)orFlag,
                    temp1Reg, iCursor, conditions, litPoolBaseReg, true);
                if (orFlag != 0) {
#ifdef J9VM_INTERP_COMPRESSED_OBJECT_HEADER
                    iCursor = generateS390ImmOp(
                        cg, TR::InstOpCode::O, node, temp1Reg, temp1Reg, (int32_t)orFlag, conditions, litPoolBaseReg);
#else
                    if (TR::Compiler->target.is64Bit())
                        iCursor = generateS390ImmOp(cg, TR::InstOpCode::OG, node, temp1Reg, temp1Reg, (int64_t)orFlag,
                            conditions, litPoolBaseReg);
                    else
                        iCursor = generateS390ImmOp(cg, TR::InstOpCode::O, node, temp1Reg, temp1Reg, (int32_t)orFlag,
                            conditions, litPoolBaseReg);
#endif
                }
            } else {
                ...
                if (!canUseIIHF)
                    iCursor = genLoadAddressConstant(cg, node, (intptr_t)classAddress | (intptrj_t)orFlag, temp1Reg,
                        iCursor, conditions, litPoolBaseReg);
            }
            if (canUseIIHF) {
        ...
        if (isStaticFlag) {
            ...
            if (staticFlag != 0) {
                if (cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_z10)
                    && staticFlag >= MIN_IMMEDIATE_VAL && staticFlag <= MAX_IMMEDIATE_VAL) {
                    ...
                    iCursor = generateLoad32BitConstant(
                        cg, node, staticFlag, temp1Reg, true, iCursor, conditions, litPoolBaseReg);
                    // Store the flags
                    iCursor = generateRXInstruction(cg, TR::InstOpCode::ST, node, temp1Reg,
            ...
            if (!comp->getOption(TR_DisableDualTLH) && useDualTLH && node->canSkipZeroInitialization())
                iCursor = generateRXInstruction(cg, TR::InstOpCode::getLoadOpCode(), node, temp1Reg,
                    generateS390MemoryReference(
                        metaReg, offsetof(J9VMThread, nonZeroAllocateThreadLocalHeap.objectFlags), cg),
                    iCursor);
            else
                iCursor = generateRXInstruction(cg, TR::InstOpCode::getLoadOpCode(), node, temp1Reg,
            ...
            if (staticFlag)
                iCursor = generateS390ImmOp(
                    cg, TR::InstOpCode::O, node, temp1Reg, temp1Reg, staticFlag, conditions, litPoolBaseReg);
            // Store the flags
            iCursor = generateRXInstruction(cg, TR::InstOpCode::ST, node, temp1Reg,
                generateS390MemoryReference(resReg, TMP_OFFSETOF_J9OBJECT_FLAGS, cg), iCursor);
            // iCursor = generateRXInstruction(cg, TR::InstOpCode::getStoreOpCode(), node, temp1Reg,
        ...
        if ((zeroReg != NULL) && (node->getOpCodeValue() != TR::New) && (lwOffset > 0)) {
            if (TR::Compiler->target.is64Bit() && fej9->generateCompressedLockWord())
                iCursor = generateRXInstruction(cg, TR::InstOpCode::ST, node, zeroReg,
...
static void genInitArrayHeader(TR::Node* node, TR::Instruction*& iCursor, bool isVariableLen,
    TR_OpaqueClassBlock* classAddress, TR::Register* classReg, TR::Register* resReg, TR::Register* zeroReg,
    TR::Register* eNumReg, TR::Register* dataSizeReg, TR::Register* temp1Reg, TR::Register* litPoolBaseReg,
    TR::RegisterDependencyConditions* conditions, TR::CodeGenerator* cg)
{
    TR::Compilation* comp = cg->comp();
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(comp->fe());
    ...
    genInitObjectHeader(node, iCursor, classAddress, classReg, resReg, zeroReg, temp1Reg, litPoolBaseReg, conditions,
        cg, eNumReg, canUseIIHF);
...
TR::Register* J9::Z::TreeEvaluator::VMnewEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (doInline) {
        ...
        if (maxNumOfOOLOptStr) {
            ...
            traceMsg(
                cg->comp(), "TR_MaxNumOfOOLOpt: count=%d, maxNumOfOOLOpt=%d %d\n", count, maxNumOfOOLOpt, __LINE__);
        ...
        if (opCode != TR::anewarray && opCode != TR::New && opCode != TR::newarray) {
            ...
            if (comp->getOption(TR_TraceCG))
                traceMsg(comp,
                    "OUTLINED NEW: Disable for %s %p because outlined allocation can't deal with arraylets\n",
                    node->getOpCode().getName(), node);
            disableOutlinedNew = true;
            cg->generateDebugCounter(TR::DebugCounter::debugCounterName(
                                         comp, "cg.new/refusedToOutline/arraylets/%s", node->getOpCode().getName()),
                1, TR::DebugCounter::Undetermined);
            ...
            disableOutlinedNew = true;
            cg->generateDebugCounter(
                TR::DebugCounter::debugCounterName(comp, "cg.new/refusedToOutline/optlevel/%s", comp->getHotnessName()),
                1, TR::DebugCounter::Undetermined);
        } else if (needZeroReg) {
            if (comp->getOption(TR_TraceCG))
        ...
        if (!disableOutlinedNew
            && performTransformation(comp, "O^O OUTLINED NEW: outlining %s %p, size %d\n", node->getOpCode().getName(),
                   node, allocateSize))
            outlineNew = true;
        ...
        if (isVariableLen) {
             ...
             * SLLG    GPR14,GPR14,1
             ...
             1     AGHI    GPR14,7  <---can combine 1 & 3 when allocateSize (8) is multiple of alignmentConstant (8),
             but need to re-arrange some of the registers, result is expected in GPR15 2     NILF    GPR14,-8 3     LGHI
             GPR15,8 4     AGR     GPR15,GPR14 5     AGR     GPR15,GPR3 CLG     GPR15,#513 104(GPR13)
            ...
            /*     if (elementSize >= 2)
             ...
             trailingZeroes(elementSize), iCursor); else iCursor = generateRSInstruction(cg, TR::InstOpCode::SLL, node,
             dataSizeReg, trailingZeroes(elementSize), iCursor);
             } */
            if (callLabel != NULL
                && (node->getOpCodeValue() == TR::anewarray || node->getOpCodeValue() == TR::newarray)) {
                TR_Debug* debugObj = cg->getDebug();
                ...
                if (cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_z10)
                    ...
                    iCursor = generateRIEInstruction(cg, TR::InstOpCode::RISBG, node, tmp, enumReg,
                        (int8_t)(32 - shift_amount), (int8_t)((63 - shift_amount) | 0x80), (int8_t)shift_amount);
                } else {
                    iCursor = generateRRInstruction(
                        cg, TR::InstOpCode::getLoadTestRegWidenOpCode(), node, tmp, enumReg, iCursor);
                    if (elementSize >= 2) {
                        if (TR::Compiler->target.is64Bit())
                            iCursor = generateRSInstruction(
                                cg, TR::InstOpCode::SLLG, node, tmp, tmp, trailingZeroes(elementSize), iCursor);
                        else
                            iCursor = generateRSInstruction(
                                cg, TR::InstOpCode::SLL, node, tmp, trailingZeroes(elementSize), iCursor);
                    }
                }
                ...
                if (!comp->getOption(TR_DisableOOL)) {
                    if (comp->getOption(TR_DisableDualTLH) && useDualTLH || allocZeroArrayWithVM == NULL) {
                        iCursor = generateS390BranchInstruction(
                            cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BE, node, startOOLLabel, iCursor);
                        TR_Debug* debugObj = cg->getDebug();
                        ...
                        if (debugObj)
                            debugObj->addInstructionComment(
                                cursor, "Denotes start of OOL for allocating zero size arrays");

                        /* using TR::Compiler->om.discontiguousArrayHeaderSizeInBytes() -
                         TR::Compiler->om.contiguousArrayHeaderSizeInBytes()
                         * for byte size for discontinous 0 size arrays becasue later instructions do ( + 15 & -8) to
                        ...
                        generateS390BranchInstruction(
                            cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, node, exitOOLLabel, cursor);
                        zeroSizeArrayChckOOL->swapInstructionListsWithCompilation();
                    } else {
                        iCursor = generateS390BranchInstruction(
                            cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BE, node, callLabel, iCursor);
                        if (!firstBRCToOOL) {
                            firstBRCToOOL = iCursor;
                        } else {
                            secondBRCToOOL = iCursor;
                    ...
                    iCursor = generateS390BranchInstruction(
                        cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNE, node, exitOOLLabel, iCursor);
                ...
                iCursor
                    = generateRRInstruction(cg, TR::InstOpCode::getLoadRegWidenOpCode(), node, tmp, enumReg, iCursor);
        ...
        if (comp->compileRelocatableCode() && (opCode == TR::New || opCode == TR::anewarray)) {
            iCursor = firstInstruction = generateS390BranchInstruction(
                cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_VGNOP, node, callLabel, current);
            if (!firstBRCToOOL) {
                firstBRCToOOL = iCursor;
            } else {
                secondBRCToOOL = iCursor;
        ...
        if (outlineNew) {
            if (isVariableLen)
                roundArrayLengthToObjectAlignment(cg, node, iCursor, dataSizeReg, conditions, litPoolBaseReg,
                    allocateSize, elementSize, resReg, exitOOLLabel);
            ...
            if (bppoutline) {
                ...
                if (helper == TR_S390OutlinedNew && cg->_outlineCall._frequency == -1) {
                    ...
                    cg->_outlineArrayCall._callSymRef
                        = cg->symRefTab()->findOrCreateRuntimeHelper(helper, false, false, false);
            ...
            // TODO When we decide to enable outlinedNew we need to make sure we have class and size in right register
            // as per expectation
            iCursor = generateDirectCall(
                cg, node, false, cg->symRefTab()->findOrCreateRuntimeHelper(helper, false, false, false), conditions);
            ...
            // If we overflow the nonZeroTLH, set the destination to the right VM runtime helper (eg
            // jitNewObjectNoZeroInit, etc...) The zeroed-TLH versions have their correct destinations already setup in
            // TR_ByteCodeIlGenerator::genNew, TR_ByteCodeIlGenerator::genNewArray, TR_ByteCodeIlGenerator::genANewArray
            // TR::PPCHeapAllocSnippet retrieves this destination via node->getSymbolReference() below after
            // genHeapAlloc.
            if (!comp->getOption(TR_DisableDualTLH) && useDualTLH && node->canSkipZeroInitialization()) {
            ...
            genHeapAlloc(node, iCursor, isVariableLen, enumReg, resReg, zeroReg, dataSizeReg, temp1Reg, callLabel,
                allocateSize, elementSize, cg, litPoolBaseReg, conditions, firstBRCToOOL, secondBRCToOOL, exitOOLLabel);
        }
        ...
        if (isArray) {
            if (comp->compileRelocatableCode() && opCode == TR::anewarray)
                genInitArrayHeader(node, iCursor, isVariableLen, classAddress, classReg, resReg, zeroReg, enumReg,
                    dataSizeReg, temp1Reg, litPoolBaseReg, conditions, cg);
            else
                genInitArrayHeader(node, iCursor, isVariableLen, classAddress, NULL, resReg, zeroReg, enumReg,
                    dataSizeReg, temp1Reg, litPoolBaseReg, conditions, cg);

            // Write Arraylet Pointer
            if (generateArraylets) {
                iCursor = generateS390ImmOp(
                    cg, TR::InstOpCode::getAddOpCode(), node, temp1Reg, resReg, dataBegin, conditions, litPoolBaseReg);
                if (TR::Compiler->vm.heapBaseAddress() == 0)
                    iCursor = generateS390ImmOp(cg, TR::InstOpCode::getAddOpCode(), node, temp1Reg, temp1Reg,
                        -((int64_t)(TR::Compiler->vm.heapBaseAddress())), conditions, litPoolBaseReg);
                if (TR::Compiler->om.compressedReferenceShiftOffset() > 0)
                    iCursor = generateRSInstruction(cg, TR::InstOpCode::SRL, node, temp1Reg,
                        TR::Compiler->om.compressedReferenceShiftOffset(), iCursor);

                iCursor = generateRXInstruction(cg,
                    (TR::Compiler->target.is64Bit() && !comp->useCompressedPointers()) ? TR::InstOpCode::STG
                                                                                       : TR::InstOpCode::ST,
                    node, temp1Reg,
                    generateS390MemoryReference(resReg, fej9->getOffsetOfContiguousArraySizeField(), cg), iCursor);
            }
            ...
            genInitObjectHeader(
                node, iCursor, classAddress, classReg, resReg, zeroReg, temp1Reg, litPoolBaseReg, conditions, cg);
        }
        ...
        if (cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_z10) && cg->enableTLHPrefetching()) {
            iCursor = generateS390MemInstruction(
                cg, TR::InstOpCode::PFD, node, 2, generateS390MemoryReference(resReg, 0x100, cg), iCursor);
        }
        ...
        if (!comp->getOption(TR_DisableHeapAllocOOL)) {
            if (secondBRCToOOL) {
                ...
                generateS390LabelInstruction(
                    cg, TR::InstOpCode::LABEL, node, cFlowRegionStart, firstBRCToOOL->getPrev());
                cFlowRegionStart->setStartInternalControlFlow();

                generateS390LabelInstruction(
                    cg, TR::InstOpCode::LABEL, node, cFlowRegionEnd, conditions, secondBRCToOOL);
                cFlowRegionEnd->setEndInternalControlFlow();
            }
...
TR::Register* J9::Z::TreeEvaluator::VMarrayCheckEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    //
        ...
        if (!node->isArrayChkReferenceArray1()) {
            ...
            generateRIInstruction(
                cg, TR::InstOpCode::getLoadHalfWordImmOpCode(), node, tempClassReg, OBJECT_HEADER_SHAPE_MASK);
            generateRRInstruction(cg, TR::InstOpCode::NR, node, tempClassReg, tempReg);

            generateRIInstruction(
                cg, TR::InstOpCode::getLoadHalfWordImmOpCode(), node, tempReg, OBJECT_HEADER_SHAPE_POINTERS);
    ...
    deps->addPostConditionIfNotAlreadyInserted(
        object2Reg, TR::RealRegister::AssignAny); // 1st and 2nd object may be the same.
...
/////////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////////
static bool inlineMathSQRT(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    ...
    if (firstChild->getOpCode().isLoadConst()) {
        ...
        generateRXInstruction(
            cg, TR::InstOpCode::LD, node, targetRegister, generateS390MemoryReference(cds, cg, 0, node));
    } else {
...
static bool inlineIsAssignableFrom(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (classDepth != -1) {
        ...
        genTestIsSuper(cg, node, objClassReg, castClassReg, scratch1Reg, scratch2Reg, tempReg, NULL, classDepth,
            failLabel, doneLabel, NULL, deps, NULL, false, NULL, NULL);
    ...
    bool isForward)
{
    ...
    TR_ASSERT_FATAL(TR::Compiler->target.is64Bit(), "GSCS: Guarded Load OOL Path only defined in 64 bit mode");
    ...
    TR::MemoryReference* privFlagMR = generateS390MemoryReference(
        vmReg, TR::Compiler->vm.thisThreadGetConcurrentScavengeActiveByteAddressOffset(cg->comp()), cg);
    ...
    TR::RegisterDependencyConditions* loopDeps = TR::TreeEvaluator::generateMemToMemElementCopy(
        node, cg, byteSrcReg, byteDstReg, byteLenReg, srm, isForward, true);
    ...
    TR::CodeGenerator* cg)
{
    ...
    if (comp->compileRelocatableCode())
        generateRegLitRefInstruction(cg, TR::InstOpCode::getLoadOpCode(), node, helperReg, (intptrj_t)funcdescrptr,
    ...
    snippet = new (cg->trHeapMemory()) TR::S390HelperCallSnippet(cg, node, callLabel,
        cg->symRefTab()->findOrCreateRuntimeHelper(TR_S390referenceArrayCopyHelper, false, false, false), doneLabel);
    cg->addSnippet(snippet);
    void* destAddr = cg->symRefTab()
                         ->findOrCreateRuntimeHelper(TR_S390referenceArrayCopyHelper, false, false, false)
                         ->getMethodAddress();
...
void J9::Z::TreeEvaluator::restoreGPR7(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::Register* tempReg = cg->allocateRegister();
    generateRXInstruction(
        cg, TR::InstOpCode::getLoadOpCode(), node, cg->machine()->getRealRegister(TR::RealRegister::GPR7), tempMR);
}

void J9::Z::TreeEvaluator::genWrtbarForArrayCopy(
    TR::Node* node, TR::Register* srcReg, TR::Register* dstReg, bool srcNonNull, TR::CodeGenerator* cg)
{
    TR::Instruction* cursor;
    ...
    bool doWrtBar
        = (gcMode == TR_WrtbarOldCheck || gcMode == TR_WrtbarCardMarkAndOldCheck || gcMode == TR_WrtbarAlways);
    // Do not do card marking when gcMode is TR_WrtbarCardMarkAndOldCheck - we go through helper, which performs CM, so
    // it is redundant.
...
extern TR::Register* VMinlineCompareAndSwap(
    TR::Node* node, TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic casOp, bool isObj)
{
    ...
    if (isObj && comp->useCompressedPointers() && decompressedValueNode->getOpCodeValue() == TR::l2i) {
        ...
        if (decompressedValueNode->getOpCode().isSub() || TR::Compiler->vm.heapBaseAddress() == 0
            || newVNode->isNull()) {
        ...
        if (isValueCompressedReference) {
            while (
                (decompressedValueNode->getNumChildren() > 0) && (decompressedValueNode->getOpCodeValue() != TR::a2l)) {
                decompressedValueNode = decompressedValueNode->getFirstChild();
            }
    ...
    if (TR::Compiler->om.shouldGenerateReadBarriersForFieldLoads() && isObj) {
        ...
        generateRXInstruction(
            cg, guardedLoadMnemonic, node, tempReadBarrier, generateS390MemoryReference(*casMemRef, 0, cg));
    ...
    if (isObj && (doWrtBar || doCrdMrk)) {
        ...
        if (doWrtBar) {
            ...
            if (gcMode == TR_WrtbarCardMarkAndOldCheck || gcMode == TR_WrtbarOldCheck)
                ...
                wbRef = comp->getSymRefTab()->findOrCreateWriteBarrierStoreSymbolRef(comp->getMethodSymbol());
            VMnonNullSrcWrtBarCardCheckEvaluator(
                node, objReg, compressedValueRegister, epReg, raReg, doneLabelWrtBar, wbRef, condWrtBar, cg, false);
        }
...
TR::Instruction* J9::Z::TreeEvaluator::generateVFTMaskInstruction(
    TR::Node* node, TR::Register* reg, TR::CodeGenerator* cg, TR::Instruction* preced)
{
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(cg->fe());
...
TR::Instruction* J9::Z::TreeEvaluator::generateRuntimeInstrumentationOnOffSequence(
    TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* node, TR::Instruction* preced, bool postRA)
{
    TR::Compilation* comp = cg->comp();
    ...
    TR_ASSERT(
        0 != (J9_JIT_TOGGLE_RI_IN_COMPILED_CODE & 0xFF), "Cannot use TM to test for J9_JIT_TOGGLE_RI_IN_COMPILED_CODE");
    ...
    preced = generateSIInstruction(
        cg, TR::InstOpCode::TM, node, vmThreadMemRef, J9_JIT_TOGGLE_RI_IN_COMPILED_CODE, preced);
    preced = generateS390BranchInstruction(
        cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_MASK7, node, OOLStartLabel, preced);
...
extern TR::Register* inlineCurrentTimeMaxPrecision(TR::CodeGenerator* cg, TR::Node* node)
{
    ...
    if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
        ...
        if (TR::Compiler->target.isZOS()) {
            // Load FFCVT(R0)
            generateRXInstruction(
                cg, TR::InstOpCode::LLGT, node, tempRegister, generateS390MemoryReference(offsets[0], cg));
        ...
        generateRXInstruction(
            cg, TR::InstOpCode::LG, node, targetRegister, generateS390MemoryReference(node, reusableTempSlot, cg));

        int64_t todJanuary1970 = 0x7D91048BCA000000LL;
        generateRegLitRefInstruction(
            cg, TR::InstOpCode::SLG, node, targetRegister, todJanuary1970, NULL, NULL, literalBaseRegister);
        ...
        if (TR::Compiler->target.isZOS()) {
            // Load FFCVT(r0)
            generateRXInstruction(
                cg, TR::InstOpCode::L, node, tempRegister1, generateS390MemoryReference(offsets[0], cg));

            // Load CVTEXT2 - CVT
            generateRXInstruction(
                cg, TR::InstOpCode::L, node, tempRegister1, generateS390MemoryReference(tempRegister1, offsets[1], cg));
        }
#endif

        generateRSInstruction(
            cg, TR::InstOpCode::LM, node, targetRegister, generateS390MemoryReference(node, reusableTempSlot, cg));

        int32_t todJanuary1970High = 0x7D91048B;
        int32_t todJanuary1970Low = 0xCA000000;

        // tempRegister2 is not actually used
        generateS390ImmOp(cg, TR::InstOpCode::SL, node, tempRegister2, targetRegister->getLowOrder(), todJanuary1970Low,
            NULL, literalBaseRegister);
        generateS390ImmOp(cg, TR::InstOpCode::SLB, node, tempRegister2, targetRegister->getHighOrder(),
            todJanuary1970High, NULL, literalBaseRegister);

#if defined(TR_HOST_S390) && defined(J9ZOS390)
        if (TR::Compiler->target.isZOS()) {
            // Subtract the LSO offset
            generateRXInstruction(cg, TR::InstOpCode::SL, node, targetRegister->getLowOrder(),
                generateS390MemoryReference(tempRegister1, offsets[2] + 4, cg));
            generateRXInstruction(cg, TR::InstOpCode::SLB, node, targetRegister->getHighOrder(),
                generateS390MemoryReference(tempRegister1, offsets[2], cg));
        }
#endif
...
extern TR::Register* inlineAtomicOps(
    TR::Node* node, TR::CodeGenerator* cg, int8_t size, TR::MethodSymbol* method, bool isArray = false)
{
    TR::Compilation* comp = cg->comp();
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(comp->fe());
    ...
    if (cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_z196)) {
        if (isAddOp) // getAndAdd or andAndGet
        {
            if (node->getNumChildren() > 1) {
                ...
                if (isLong && TR::Compiler->target.is32Bit()
                    && !cg->use64BitRegsOn32Bit()) // deal with reg pairs on 31bit platform
                {
                    ...
                    if (deltaChild->getOpCode().isLoadConst() && !deltaChild->getRegister()) {
                        ...
                        if (value < MIN_IMMEDIATE_VAL || value > MAX_IMMEDIATE_VAL)
                            generateRILInstruction(
                                cg, TR::InstOpCode::IIHF, node, deltaReg, static_cast<int32_t>(value >> 32));
                    } else {
                        // evaluate 2nd child will return a reg pair, shift them into a 64-bit reg
                        TR::Register* deltaRegPair = cg->evaluate(deltaChild);
                        generateRSInstruction(
                            cg, TR::InstOpCode::SLLG, node, deltaReg, deltaRegPair->getHighOrder(), 32);
            ...
            if (isLong && TR::Compiler->target.is32Bit()
                && !cg->use64BitRegsOn32Bit()) // on 31-bit platoform, restore the result back into a register pair
            {
    ...
    if (isAddOp) {
        ...
        if (isArgConstant) {
            generateS390ImmOp(
                cg, TR::InstOpCode::getAddOpCode(), node, resultReg, resultReg, (int32_t)delta, dependencies, NULL);
        } else {
...
static TR::Register* evaluateTwo32BitLoadsInA64BitRegister(
    TR::Node* node, TR::CodeGenerator* cg, TR::Node* highNode, TR::Node* lowNode)
{
...
extern TR::Register* inlineAtomicFieldUpdater(TR::Node* node, TR::CodeGenerator* cg, TR::MethodSymbol* method)
{
    ...
    generateRRFInstruction(
        cg, TR::InstOpCode::LOCR, node, tempReg, trueReg, getMaskForBranchCondition(TR::InstOpCode::COND_BNER), true);

    // obj == null?
    generateRRInstruction(
        cg, TR::Compiler->target.is64Bit() ? TR::InstOpCode::LTGR : TR::InstOpCode::LTR, node, objReg, objReg);
    generateRRFInstruction(
        cg, TR::InstOpCode::LOCR, node, tempReg, trueReg, getMaskForBranchCondition(TR::InstOpCode::COND_BER), true);
    ...
    if (comp->useCompressedPointers()) {
        ...
        // get tclass
        generateRXInstruction(
            cg, TR::InstOpCode::LLGF, node, tClassReg, generateS390MemoryReference(thisReg, tclass, cg));
        int32_t shiftAmount = TR::Compiler->om.compressedReferenceShift();
        if (shiftAmount != 0) {
        ...
        // inline the getClass() method = grab it from j9class
        generateRXInstruction(cg, TR::Compiler->target.is64Bit() ? TR::InstOpCode::LG : TR::InstOpCode::L, node,
            objClassReg, generateS390MemoryReference(objClassReg, fej9->getOffsetOfJavaLangClassFromClassField(), cg));

        // get tclass
        generateRXInstruction(cg, TR::Compiler->target.is64Bit() ? TR::InstOpCode::LG : TR::InstOpCode::L, node,
            tClassReg, generateS390MemoryReference(thisReg, tclass, cg));
    }
    generateRRInstruction(
        cg, TR::Compiler->target.is64Bit() ? TR::InstOpCode::CGR : TR::InstOpCode::CR, node, objClassReg, tClassReg);
    generateRRFInstruction(
        cg, TR::InstOpCode::LOCR, node, tempReg, trueReg, getMaskForBranchCondition(TR::InstOpCode::COND_BNER), true);
    ...
    TR::Register* resultReg, bool checkResultRegForTMSuccess)
{
    ...
    if (doWrtBar || doCrdMrk) {
        ...
        if (checkResultRegForTMSuccess) {
            ...
            generateRRInstruction(cg, TR::Compiler->target.is64Bit() ? TR::InstOpCode::LTGR : TR::InstOpCode::LTR, node,
                resultReg, resultReg);
        ...
        if (doWrtBar) {
            ...
            VMnonNullSrcWrtBarCardCheckEvaluator(
                node, objReg, srcReg, epReg, raReg, doneLabelWrtBar, wbRef, condWrtBar, cg, false);
        } else if (doCrdMrk) {
            VMCardCheckEvaluator(node, objReg, epReg, condWrtBar, cg, false, doneLabelWrtBar, false);
            // true #1 -> copy of objReg just happened, it's safe to clobber tempReg
            // false #2 -> Don't do compile time check for heap obj
...
extern TR::Register* inlineConcurrentLinkedQueueTMOffer(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (classBlock1 && classBlock2) {
        offsetNext = fej9->getObjectHeaderSizeInBytes()
            + fej9->getInstanceFieldOffset(
                  classBlock1, "next", 4, "Ljava/util/concurrent/ConcurrentLinkedQueue$Node;", 49);
        offsetTail = fej9->getObjectHeaderSizeInBytes()
            + fej9->getInstanceFieldOffset(
                  classBlock2, "tail", 4, "Ljava/util/concurrent/ConcurrentLinkedQueue$Node;", 49);
    } else
        disableTMOffer = true;
    ...
    if (!disableTMOffer) {
        ...
        if (usesCompressedrefs) {
            cursor = generateRXInstruction(
                cg, TR::InstOpCode::LLGF, node, rP, generateS390MemoryReference(rThis, offsetTail, cg));
            ...
            cursor = generateRXInstruction(
                cg, TR::InstOpCode::getLoadOpCode(), node, rP, generateS390MemoryReference(rThis, offsetTail, cg));
        }
        ...
        if (usesCompressedrefs) {
            cursor = generateRXInstruction(
                cg, TR::InstOpCode::LT, node, rQ, generateS390MemoryReference(rP, offsetNext, cg));
            ...
            cursor = generateRXInstruction(
                cg, TR::InstOpCode::getLoadTestOpCode(), node, rQ, generateS390MemoryReference(rP, offsetNext, cg));
        }
        ...
        if (usesCompressedrefs) {
            if (shiftAmount != 0) {
                ...
                cursor = generateRXInstruction(
                    cg, TR::InstOpCode::ST, node, rQ, generateS390MemoryReference(rP, offsetNext, cg));
                cursor = generateRXInstruction(
                    cg, TR::InstOpCode::ST, node, rQ, generateS390MemoryReference(rThis, offsetTail, cg));
            } else {
                cursor = generateRXInstruction(
                    cg, TR::InstOpCode::ST, node, rN, generateS390MemoryReference(rP, offsetNext, cg));
                cursor = generateRXInstruction(
                    cg, TR::InstOpCode::ST, node, rN, generateS390MemoryReference(rThis, offsetTail, cg));
            }
        } else {
            cursor = generateRXInstruction(
                cg, TR::InstOpCode::getStoreOpCode(), node, rN, generateS390MemoryReference(rP, offsetNext, cg));
            cursor = generateRXInstruction(
                cg, TR::InstOpCode::getStoreOpCode(), node, rN, generateS390MemoryReference(rThis, offsetTail, cg));
        }
...
extern TR::Register* inlineConcurrentLinkedQueueTMPoll(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (classBlock1 && classBlock2) {
        offsetHead = fej9->getObjectHeaderSizeInBytes()
            + fej9->getInstanceFieldOffset(
                  classBlock1, "head", 4, "Ljava/util/concurrent/ConcurrentLinkedQueue$Node;", 49);
        offsetNext = fej9->getObjectHeaderSizeInBytes()
            + fej9->getInstanceFieldOffset(
                  classBlock2, "next", 4, "Ljava/util/concurrent/ConcurrentLinkedQueue$Node;", 49);
    ...
    if (!disableTMPoll) {
        ...
        if (usesCompressedrefs) {
            cursor = generateRXInstruction(
                cg, TR::InstOpCode::LLGF, node, rP, generateS390MemoryReference(rThis, offsetHead, cg));
            ...
            cursor = generateRXInstruction(
                cg, TR::InstOpCode::getLoadOpCode(), node, rP, generateS390MemoryReference(rThis, offsetHead, cg));
        }
        ...
        if (usesCompressedrefs) {
            cursor = generateRXInstruction(
                cg, TR::InstOpCode::LLGF, node, rE, generateS390MemoryReference(rP, offsetItem, cg));
            ...
            cursor = generateRXInstruction(
                cg, TR::InstOpCode::getLoadOpCode(), node, rE, generateS390MemoryReference(rP, offsetItem, cg));
        }
        ...
        if (usesCompressedrefs) {
            ...
            cursor = generateSILInstruction(
                cg, TR::InstOpCode::MVHI, node, generateS390MemoryReference(rP, offsetItem, cg), 0);
        } else {
            cursor = generateRXInstruction(
                cg, TR::InstOpCode::getLoadTestOpCode(), node, rQ, generateS390MemoryReference(rP, offsetNext, cg));
        ...
        if (usesCompressedrefs) {
            cursor = generateRXInstruction(
                cg, TR::InstOpCode::ST, node, rQ, generateS390MemoryReference(rThis, offsetHead, cg));

            if (shiftAmount != 0) {
                cursor = generateRSInstruction(cg, TR::InstOpCode::SRLG, node, rTmp, rP, shiftAmount);
                cursor = generateRXInstruction(
                    cg, TR::InstOpCode::ST, node, rTmp, generateS390MemoryReference(rP, offsetNext, cg));
            } else {
                cursor = generateRXInstruction(
                    cg, TR::InstOpCode::ST, node, rP, generateS390MemoryReference(rP, offsetNext, cg));
            }
        } else {
            cursor = generateRXInstruction(
                cg, TR::InstOpCode::getStoreOpCode(), node, rQ, generateS390MemoryReference(rThis, offsetHead, cg));
            cursor = generateRXInstruction(
                cg, TR::InstOpCode::getStoreOpCode(), node, rP, generateS390MemoryReference(rP, offsetNext, cg));
        }
...
void VMgenerateCatchBlockBBStartPrologue(TR::Node* node, TR::Instruction* fenceInstruction, TR::CodeGenerator* cg)
{
    ...
    if (fej9->shouldPerformEDO(block, comp)) {
        ...
        dependencies->addPostCondition(tempReg1, cg->getEntryPointRegister());
        dependencies->addPostCondition(tempReg2, cg->getReturnAddressRegister());
        // Branch to induceRecompilation helper routine if counter is 0 - based on condition code of the precedeing
        // adds.
        generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, cFlowRegionStart);
        cFlowRegionStart->setStartInternalControlFlow();
...
void TR_J9VMBase::generateBinaryEncodingPrologue(TR_BinaryEncodingData* beData, TR::CodeGenerator* cg)
{
    ...
    if (comp->getJittedMethodSymbol()->isJNI() && !comp->getOption(TR_FullSpeedDebug)) {
        data->preProcInstruction = (TR::Compiler->target.is64Bit()) ? data->cursorInstruction->getNext()->getNext()
                                                                    : data->cursorInstruction->getNext();
    } else {
...
float J9::Z::TreeEvaluator::interpreterProfilingInstanceOfOrCheckCastTopProb(TR::CodeGenerator* cg, TR::Node* node)
{
    ...
    TR_AddressInfo* valueInfo = static_cast<TR_AddressInfo*>(valueProfileInfo->getValueInfo(
        bcInfo, comp, AddressInfo, TR_ValueProfileInfoManager::justInterpreterProfileInfo));
...
TR::Register* J9::Z::TreeEvaluator::tstartEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (labelTransientFailure == labelPersistentFailure) {
        ...
        cursor = generateS390BranchInstruction(
            cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_MASK7, node, labelTransientFailure, depsPersistent);
    } else {
        cursor = generateS390BranchInstruction(
            cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BH, node, labelTransientFailure, depsTransient);
        cursor = generateS390BranchInstruction(
            cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_MASK7, node, labelPersistentFailure, depsPersistent);
    }
    ...
    if (TR::Compiler->target.is64Bit() && cg->fej9()->generateCompressedLockWord())
        cursor = generateRXInstruction(
            cg, TR::InstOpCode::LT, node, monitorReg, generateS390MemoryReference(objReg, lwOffset, cg), cursor);
    else
    ...
    cursor = generateS390BranchInstruction(
        cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BE, node, startLabel, deps, cursor);
    ...
    cursor = generateS390BranchInstruction(
        cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, node, labelTransientFailure, depsTransient, cursor);
...
TR::Register* J9::Z::TreeEvaluator::tabortEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    generateRIInstruction(
        cg, TR::Compiler->target.is64Bit() ? TR::InstOpCode::LGHI : TR::InstOpCode::LHI, node, codeReg, 0);
    ...
    generateRIInstruction(
        cg, TR::Compiler->target.is64Bit() ? TR::InstOpCode::LGHI : TR::InstOpCode::LHI, node, codeReg, 0x100);
