...
J9::Z::UnresolvedDataSnippet::UnresolvedDataSnippet(
    TR::CodeGenerator* cg, TR::Node* node, TR::SymbolReference* symRef, bool isStore, bool canCauseGC)
...
uint8_t* J9::Z::UnresolvedDataSnippet::emitSnippetBody()
{
    ...
    if (getDataSymbol()->getShadowSymbol() != NULL) {
        if (resolveForStore()) {
            glueRef = cg()->symRefTab()->findOrCreateRuntimeHelper(
                TR_S390interpreterUnresolvedInstanceDataStoreGlue, false, false, false);
        } else {
            glueRef = cg()->symRefTab()->findOrCreateRuntimeHelper(
                TR_S390interpreterUnresolvedInstanceDataGlue, false, false, false);
        }
    } else if (getDataSymbol()->isClassObject()) {
        if (getDataSymbol()->addressIsCPIndexOfStatic()) {
            glueRef = cg()->symRefTab()->findOrCreateRuntimeHelper(
                TR_S390interpreterUnresolvedClassGlue2, false, false, false);
        } else {
            glueRef = cg()->symRefTab()->findOrCreateRuntimeHelper(
                TR_S390interpreterUnresolvedClassGlue, false, false, false);
        }
    } else if (getDataSymbol()->isConstString()) {
        glueRef
            = cg()->symRefTab()->findOrCreateRuntimeHelper(TR_S390interpreterUnresolvedStringGlue, false, false, false);
    } else if (getDataSymbol()->isConstMethodType()) {
        glueRef
            = cg()->symRefTab()->findOrCreateRuntimeHelper(TR_interpreterUnresolvedMethodTypeGlue, false, false, false);
    } else if (getDataSymbol()->isConstMethodHandle()) {
        glueRef = cg()->symRefTab()->findOrCreateRuntimeHelper(
            TR_interpreterUnresolvedMethodHandleGlue, false, false, false);
    } else if (getDataSymbol()->isCallSiteTableEntry()) {
        glueRef = cg()->symRefTab()->findOrCreateRuntimeHelper(
            TR_interpreterUnresolvedCallSiteTableEntryGlue, false, false, false);
    } else if (getDataSymbol()->isMethodTypeTableEntry()) {
        glueRef = cg()->symRefTab()->findOrCreateRuntimeHelper(
            TR_interpreterUnresolvedMethodTypeTableEntryGlue, false, false, false);
    } else if (getDataSymbol()->isConstantDynamic()) {
        glueRef
            = cg()->symRefTab()->findOrCreateRuntimeHelper(TR_S390jitResolveConstantDynamicGlue, false, false, false);
    } else // must be static data
    {
        if (resolveForStore()) {
            glueRef = cg()->symRefTab()->findOrCreateRuntimeHelper(
                TR_S390interpreterUnresolvedStaticDataStoreGlue, false, false, false);
        } else {
            glueRef = cg()->symRefTab()->findOrCreateRuntimeHelper(
                TR_S390interpreterUnresolvedStaticDataGlue, false, false, false);
        }
    }
    ...
    *(uintptrj_t*)cursor = (uintptrj_t)glueRef->getMethodAddress();
    ...
    cg()->addProjectSpecializedRelocation(
        cursor, (uint8_t*)glueRef, NULL, TR_AbsoluteHelperAddress, __FILE__, __LINE__, getNode());
    ...
    *(uintptrj_t*)cursor = 0x0;
    ...
    if (isInstanceData()) {
        ...
        *(uintptrj_t*)offsetMarker = (uintptrj_t)cursor;
        ...
        cg()->addProjectSpecializedRelocation(
            offsetMarker, NULL, NULL, TR_AbsoluteMethodAddress, __FILE__, __LINE__, getNode());
...
void TR_Debug::print(TR::FILE* pOutFile, TR::UnresolvedDataSnippet* snippet)
{
    ...
    printPrefix(pOutFile, NULL, bufferPos, sizeof(intptrj_t));
    trfprintf(pOutFile, "DC   \t0x%p \t# Address Of Literal Pool Slot", (intptrj_t)(snippet->getLiteralPoolSlot()));
    bufferPos += sizeof(intptrj_t);

    // Snippet has out-of-line sequence for large offsets for instance data
    if (snippet->isInstanceData()) {
        printPrefix(pOutFile, NULL, bufferPos, sizeof(intptrj_t));
        trfprintf(pOutFile, "DC   \t0x%p \t# Address to large offset slot", (intptrj_t)((intptrj_t*)bufferPos));
        bufferPos += sizeof(intptrj_t);
        ...
        trfprintf(pOutFile, "DC   \t0x%p \t# Displacement from helper branch to out-of-line sequence",
            (int32_t)((intptrj_t)bufferPos + sizeof(int32_t)
                - (intptrj_t)snippet->getBranchInstruction()->getBinaryEncoding() + 6));
        ...
        printPrefix(pOutFile, NULL, bufferPos, sizeof(int32_t));
        trfprintf(pOutFile, "DC   \t0x%p \t# Offset slot", (int32_t)(*(int32_t*)bufferPos));
        bufferPos += sizeof(int32_t);
    }
}
