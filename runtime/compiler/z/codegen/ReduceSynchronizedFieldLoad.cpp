...
void ReduceSynchronizedFieldLoad::inlineSynchronizedFieldLoad(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    cg->generateDebugCounter(TR::DebugCounter::debugCounterName(
        cg->comp(), "codegen/z/ReduceSynchronizedFieldLoad/success/OOL/%s", cg->comp()->signature()));
    ...
    TR_ASSERT_SAFE_FATAL(
        loadMemoryReference->getIndexRegister() == NULL, "Load memory reference must not have an index register\n");
    ...
    if (is32BitLock && is32BitLoad) {
        ...
        if (is32BitLoad) {
            if ((loadMemoryReference->getOffset() & 7) == 0) {
                ...
                // This is because we must use LPDG to load a 32-bit value using displacement -4
                TR_ASSERT_SAFE_FATAL(
                    (loadMemoryReference->getOffset() & 3) == 0, "Field must be aligned on a word boundary\n");
            ...
            TR_ASSERT_SAFE_FATAL(
                (loadMemoryReference->getOffset() & 7) == 0, "Field must be aligned on a double-word boundary\n");
        }

        generateSSFInstruction(
            cg, TR::InstOpCode::LPDG, node, registerPair, alignedLoadMemoryReference, alignedLockMemoryReference);
    ...
    if (is32BitLock && is32BitLoad) {
        ...
        generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::CR, node, lockRegister,
            cg->getMethodMetaDataRealRegister(), TR::InstOpCode::COND_BE, mergeLabel, false);

        // Lock could be free as well
        generateS390CompareAndBranchInstruction(
            cg, TR::InstOpCode::C, node, lockRegister, 0, TR::InstOpCode::COND_BE, mergeLabel, false);
    } else {
        // Now check if we loaded the lock of the current thread
        generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::CGR, node, lockRegister,
            cg->getMethodMetaDataRealRegister(), TR::InstOpCode::COND_BE, mergeLabel, false);

        // Lock could be free as well
        generateS390CompareAndBranchInstruction(
            cg, TR::InstOpCode::CG, node, lockRegister, 0, TR::InstOpCode::COND_BE, mergeLabel, false);
    }
...
bool ReduceSynchronizedFieldLoad::perform()
{
    ...
    if (cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_z196)) {
        if (!cg->comp()->getOption(TR_DisableSynchronizedFieldLoad)
            && cg->comp()->getMethodSymbol()->mayContainMonitors()) {
            ...
            for (++iter; iter != endTreeTop; ++iter) {
                if (iter.currentNode()->getOpCodeValue() == TR::monexit
                    ...
                    if (synchronizedObjectNode == monexitNode->getFirstChild()) {
                        if (cg->comp()->getOption(TR_TraceCG)) {
                            traceMsg(cg->comp(),
                                "Children of monent and monexit are synchronizing on the same object\n", monexitNode);
                        }

                        TR::Node* loadNode = findLoadInSynchornizedRegion(
                            startTreeTop, endTreeTop, monentTreeTop, monexitTreeTop, synchronizedObjectNode);
                        ...
                        if (loadNode != NULL) {
                            ...
                            int32_t lockWordOffset = static_cast<TR_J9VMBase*>(cg->comp()->fe())
                            ...
                            if (lockWordOffset > 0 && lockWordOffset < 4096) {
                                if (performTransformation(cg->comp(),
                                    ...
                                    TR::SymbolReference* methodSymRef
                                        = cg->comp()->getSymRefTab()->findOrCreateCodeGenInlinedHelper(
                                    ...
                                    TR::DebugCounter::incStaticDebugCounter(cg->comp(),
                                        TR::DebugCounter::debugCounterName(cg->comp(),
                                            "codegen/z/ReduceSynchronizedFieldLoad/success/%s",
                                            cg->comp()->signature()));
                                }
                            } else {
                                TR::DebugCounter::incStaticDebugCounter(cg->comp(),
