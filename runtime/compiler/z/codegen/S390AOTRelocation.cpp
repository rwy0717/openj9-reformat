...
void TR::S390EncodingRelocation::addRelocation(
    TR::CodeGenerator* cg, uint8_t* cursor, char* file, uintptr_t line, TR::Node* node)
{
    ...
    if (_reloType == TR_ClassAddress) {
        ...
        *((uintptrj_t*)cursor)
            = fej9->getPersistentClassPointerFromClassPointer((TR_OpaqueClassBlock*)(*((uintptrj_t*)cursor)));
        cg->addExternalRelocation(new (cg->trHeapMemory()) TR::ExternalRelocation(cursor, (uint8_t*)_symbolReference,
                                      (uint8_t*)_inlinedSiteIndex, TR_ClassAddress, cg),
            file, line, node);
    } else if (_reloType == TR_RamMethod) {
        AOTcgDiag1(comp, "TR_RamMethod cursor=%x\n", cursor);
        cg->addExternalRelocation(
            new (cg->trHeapMemory()) TR::ExternalRelocation(cursor, NULL, TR_RamMethod, cg), file, line, node);

    } else if (_reloType == TR_HelperAddress) {
        AOTcgDiag1(comp, "TR_HelperAddress cursor=%x\n", cursor);
        cg->addExternalRelocation(new (cg->trHeapMemory())
                                      TR::ExternalRelocation(cursor, (uint8_t*)_symbolReference, TR_HelperAddress, cg),
            file, line, node);
    } else if (_reloType == TR_AbsoluteHelperAddress) {
        AOTcgDiag1(comp, "TR_AbsoluteHelperAddress cursor=%x\n", cursor);
        cg->addExternalRelocation(new (cg->trHeapMemory()) TR::ExternalRelocation(
                                      cursor, (uint8_t*)_symbolReference, TR_AbsoluteHelperAddress, cg),
            file, line, node);
        ...
        if (TR::Compiler->target.is64Bit()) {
            cg->addExternalRelocation(new (cg->trHeapMemory()) TR::ExternalRelocation(
                                          cursor, (uint8_t*)*((uint64_t*)cursor), TR_BodyInfoAddress, cg),
                file, line, node);
        } else {
            cg->addExternalRelocation(new (cg->trHeapMemory()) TR::ExternalRelocation(
                                          cursor, (uint8_t*)(intptr_t) * ((uint32_t*)cursor), TR_BodyInfoAddress, cg),
                file, line, node);
        }
