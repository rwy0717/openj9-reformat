...
uint8_t* TR::S390J9CallSnippet::generateVIThunk(TR::Node* callNode, int32_t argSize, TR::CodeGenerator* cg)
{
    ...
    case TR::NoType:
        dispatcherSymbol
            = cg->symRefTab()->findOrCreateRuntimeHelper(TR_S390icallVMprJavaSendVirtual0, false, false, false);
        break;
    case TR::Int32:
        dispatcherSymbol
            = cg->symRefTab()->findOrCreateRuntimeHelper(TR_S390icallVMprJavaSendVirtual1, false, false, false);
        break;
    case TR::Address:
        if (TR::Compiler->target.is64Bit()) {
            dispatcherSymbol
                = cg->symRefTab()->findOrCreateRuntimeHelper(TR_S390icallVMprJavaSendVirtualJ, false, false, false);
        } else {
            dispatcherSymbol
                = cg->symRefTab()->findOrCreateRuntimeHelper(TR_S390icallVMprJavaSendVirtual1, false, false, false);
        }
    ...
    case TR::Float:
        dispatcherSymbol
            = cg->symRefTab()->findOrCreateRuntimeHelper(TR_S390icallVMprJavaSendVirtualF, false, false, false);
        break;
    case TR::Double:
        dispatcherSymbol
            = cg->symRefTab()->findOrCreateRuntimeHelper(TR_S390icallVMprJavaSendVirtualD, false, false, false);
        break;
    default:
...
TR_J2IThunk* TR::S390J9CallSnippet::generateInvokeExactJ2IThunk(
    TR::Node* callNode, int32_t argSize, char* signature, TR::CodeGenerator* cg)
{
    uint32_t rEP = (uint32_t)cg->getEntryPointRegister() - 1;
    TR::Compilation* comp = cg->comp();
    TR_J9VMBase* fej9 = (TR_J9VMBase*)(comp->fe());
    ...
    case TR::NoType:
        dispatcherSymbol
            = cg->symRefTab()->findOrCreateRuntimeHelper(TR_icallVMprJavaSendInvokeExact0, false, false, false);
        break;
    case TR::Int32:
        dispatcherSymbol
            = cg->symRefTab()->findOrCreateRuntimeHelper(TR_icallVMprJavaSendInvokeExact1, false, false, false);
        break;
    case TR::Address:
        if (TR::Compiler->target.is64Bit())
            dispatcherSymbol
                = cg->symRefTab()->findOrCreateRuntimeHelper(TR_icallVMprJavaSendInvokeExactJ, false, false, false);
        else
            dispatcherSymbol
                = cg->symRefTab()->findOrCreateRuntimeHelper(TR_icallVMprJavaSendInvokeExact1, false, false, false);
        break;
    ...
    case TR::Float:
        dispatcherSymbol
            = cg->symRefTab()->findOrCreateRuntimeHelper(TR_icallVMprJavaSendInvokeExactF, false, false, false);
        break;
    case TR::Double:
        dispatcherSymbol
            = cg->symRefTab()->findOrCreateRuntimeHelper(TR_icallVMprJavaSendInvokeExactD, false, false, false);
        break;
    default:
        TR_ASSERT(0, "Bad return data type for a call node.  DataType was %s\n",
            comp->getDebug()->getName(callNode->getDataType()));
    ...
    if (verbose) {
        *(int16_t*)cursor = 0xC0F4; // BRCL   <Helper Addr>
        ...
        TR::SymbolReference* helper
            = cg->symRefTab()->findOrCreateRuntimeHelper(TR_methodHandleJ2IGlue, false, false, false);
...
uint8_t* TR::S390J9CallSnippet::emitSnippetBody()
{
    ...
    int32_t pad_bytes
        = (dataStartAddr + (sizeof(uintptrj_t) - 1)) / sizeof(uintptrj_t) * sizeof(uintptrj_t) - dataStartAddr;
    ...
    pad_bytes = (((uintptrj_t)cursor + (sizeof(uintptrj_t) - 1)) / sizeof(uintptrj_t) * sizeof(uintptrj_t)
        - (uintptrj_t)cursor);
    ...
    *(uintptrj_t*)cursor = (uintptrj_t)glueRef->getMethodAddress();
    ...
    cg()->addExternalRelocation(new (cg()->trHeapMemory())
                                    TR::ExternalRelocation(cursor, (uint8_t*)glueRef, TR_AbsoluteHelperAddress, cg()),
        ...
        if (methodSymRef->isUnresolved() || comp->compileRelocatableCode()) {
            ...
            *(uintptrj_t*)cursor = 0;
            if (comp->getOption(TR_EnableHCR)) {
                ...
                cg()->addExternalRelocation(new (cg()->trHeapMemory())
                                                TR::ExternalRelocation((uint8_t*)cursor, NULL, TR_HCR, cg()),
...
uint8_t* TR::S390UnresolvedCallSnippet::emitSnippetBody()
{
    ...
    *(uintptrj_t*)cursor = (uintptrj_t)methodSymRef->getOwningMethod(comp)->constantPool();
    ...
    {
        cg()->addExternalRelocation(
            new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor, *(uint8_t**)cursor,
                getNode() ? (uint8_t*)(intptr_t)getNode()->getInlinedSiteIndex() : (uint8_t*)-1, TR_Trampolines, cg()),
            __FILE__, __LINE__, getNode());
    }
#if defined(J9ZOS390)
    else {
        cg()->addExternalRelocation(
            new (cg()->trHeapMemory()) TR::ExternalRelocation(cursor, *(uint8_t**)cursor,
                getNode() ? (uint8_t*)getNode()->getInlinedSiteIndex() : (uint8_t*)-1, TR_ConstantPool, cg()),
            __FILE__, __LINE__, getNode());
    }
#endif
#else
...
uint8_t* TR::S390VirtualSnippet::emitSnippetBody()
{
    // *this   swipeable for debugger
    return NULL;
}
...
uint8_t* TR::S390VirtualUnresolvedSnippet::emitSnippetBody()
{
    ...
    TR::SymbolReference* glueRef
        = cg()->symRefTab()->findOrCreateRuntimeHelper(TR_S390virtualUnresolvedHelper, false, false, false);
    ...
    *(uintptrj_t*)cursor = 0;
    ...
    // Field used by nestmate private calls
    // J2I thunk address
    // No explicit call to `addExternalReloation` because its relocation info is passed to CP_addr
    // `addExternalReloation` call.
    *(uintptrj_t*)cursor = (uintptrj_t)thunkAddress;
    ...
    cg()->addExternalRelocation(new (cg()->trHeapMemory()) TR::ExternalRelocation((uint8_t*)cpAddrPosition,
                                    (uint8_t*)j2iRelocInfo, NULL, TR_J2IVirtualThunkPointer, cg()),
        __FILE__, __LINE__, callNode);
    cursor += sizeof(uintptrj_t);
    ...
    TR_ASSERT_FATAL(
        getIndirectCallInstruction() && getIndirectCallInstruction()->getOpCodeValue() == TR::InstOpCode::BASR,
        "Unexpected branch instruction in VirtualUnresolvedSnippet.\n");

    *(uintptrj_t*)cursor = (uintptrj_t)(
        getIndirectCallInstruction()->getBinaryEncoding() + getIndirectCallInstruction()->getBinaryLength());
...
TR::S390InterfaceCallSnippet::S390InterfaceCallSnippet(
    TR::CodeGenerator* cg, TR::Node* c, TR::LabelSymbol* lab, int32_t s, int8_t n, void* thunkPtr, bool useCLFIandBRCL)
...
uint8_t* TR::S390InterfaceCallSnippet::emitSnippetBody()
{
    ...
    if (getNumInterfaceCallCacheSlots() == 0) {
        ...
        glueRef = cg()->symRefTab()->findOrCreateRuntimeHelper(
            TR_S390interfaceCallHelperSingleDynamicSlot, false, false, false);
    } else {
        glueRef
            = cg()->symRefTab()->findOrCreateRuntimeHelper(TR_S390interfaceCallHelperMultiSlots, false, false, false);
    }
    ...
    *(int32_t*)cursor = (int32_t)((destAddr - (intptrj_t)(cursor - 2)) / 2);
    ...
    cg()->addExternalRelocation(new (cg()->trHeapMemory())
                                    TR::ExternalRelocation(cursor, (uint8_t*)glueRef, TR_HelperAddress, cg()),
        __FILE__, __LINE__, getNode());
    cursor += sizeof(int32_t);
...
void TR_Debug::print(TR::FILE* pOutFile, TR::S390UnresolvedCallSnippet* snippet)
{
    uint8_t* bufferPos = snippet->getSnippetLabel()->getCodeLocation() + snippet->getLength(0) - sizeof(intptrj_t)
    ...
    trfprintf(
        pOutFile, "DC   \t0x%08x \t\t# Offset | Flag | CP Index", helperLookupOffset | methodSymRef->getCPIndexForVM());
}

void TR_Debug::print(TR::FILE* pOutFile, TR::S390VirtualSnippet* snippet)
{
    uint8_t* bufferPos = snippet->getSnippetLabel()->getCodeLocation();
    printSnippetLabel(pOutFile, snippet->getSnippetLabel(), bufferPos, "Virtual Call Snippet");
}

void TR_Debug::print(TR::FILE* pOutFile, TR::S390VirtualUnresolvedSnippet* snippet)
{
    uint8_t* bufferPos = snippet->getSnippetLabel()->getCodeLocation();
    TR::SymbolReference* methodSymRef = snippet->getNode()->getSymbolReference();
    ...
    printPrefix(pOutFile, NULL, bufferPos, sizeof(intptrj_t));
    trfprintf(
        pOutFile, "DC   \t%p \t\t# Address Of Constant Pool", (intptrj_t)getOwningMethod(methodSymRef)->constantPool());
    bufferPos += sizeof(intptrj_t);
    ...
    printPrefix(pOutFile, NULL, bufferPos, sizeof(intptrj_t));
    trfprintf(pOutFile, "DC   \t%p \t\t# Instruction to be patched with vft offset",
    ...
    printPrefix(pOutFile, NULL, bufferPos, sizeof(intptrj_t));
    trfprintf(pOutFile, "DC   \t%p \t\t# RA for private",
        snippet->getBranchInstruction()->getBinaryEncoding()
            + snippet->getIndirectCallInstruction()->getBinaryLength());
}

void TR_Debug::print(TR::FILE* pOutFile, TR::S390InterfaceCallSnippet* snippet)
{
    uint8_t* bufferPos = snippet->getSnippetLabel()->getCodeLocation();
    TR::SymbolReference* methodSymRef = snippet->getNode()->getSymbolReference();
    ...
    printPrefix(pOutFile, NULL, bufferPos, 6);
    trfprintf(pOutFile, "BRCL \t<%p>\t\t# Branch to Helper %s", snippet->getSnippetDestAddr(),
        snippet->usedTrampoline() ? "- Trampoline Used." : "");
    bufferPos += 6;
}

void TR_Debug::print(TR::FILE* pOutFile, TR::J9S390InterfaceCallDataSnippet* snippet)
{
    uint8_t* bufferPos = snippet->getSnippetLabel()->getCodeLocation();
...
///////////////////////////////////////////////////////////////////////////////
TR::J9S390InterfaceCallDataSnippet::J9S390InterfaceCallDataSnippet(
    TR::CodeGenerator* cg, TR::Node* node, uint8_t n, void* thunkPtr, bool useCLFIandBRCL)
...
uint8_t* TR::J9S390InterfaceCallDataSnippet::emitSnippetBody()
{
    ...
    *(intptrj_t*)cursor = 0;
    ...
    // J2I thunk address.
    // Note that J2I thunk relocation is associated with CP addr and the call to `addExternalReloation` uses CP-Addr
    // cursor.
    *(intptrj_t*)cursor = (intptrj_t)_thunkAddress;
    ...
    if (profiledClassesList) {
        for (auto valuesIt = profiledClassesList->begin(); valuesIt != profiledClassesList->end(); ++valuesIt) {
            ...
            TR_ResolvedMethod* profiledMethod = methodSymRef->getOwningMethod(comp)->getResolvedInterfaceMethod(
                comp, (TR_OpaqueClassBlock*)(*valuesIt), methodSymRef->getCPIndex());
            numInterfaceCallCacheSlots--;
            updateField = true;
    ...
    if (updateField) {
        *(intptrj_t*)(cursorlastCachedSlot) = snippetStart + getFirstSlotOffset()
