...
TR_PersistentMethodInfo* TR_S390Recompilation::getExistingMethodInfo(TR_ResolvedMethod* method)
{
    ...
    if (debug("traceRecompilation")) {
        // diagnostic("RC>>Recompiling %s at level %d\n", signature(_compilation->getCurrentMethod()),
        // info->getHotness());
    }
...
TR::Instruction* TR_S390Recompilation::generatePrePrologue()
{
    ...
    if (useSampling()) {
        ...
        cursor = new (cg->trHeapMemory())
            TR::S390LabelInstruction(TR::InstOpCode::LABEL, node, samplingRecompileMethodSnippetLabel, cursor, cg);
        ...
        cursor = generateRXInstruction(
            cg, TR::InstOpCode::getStoreOpCode(), node, cg->getEntryPointRealRegister(), epSaveAreaMemRef, cursor);
        ...
        cursor = generateRIInstruction(
            cg, TR::InstOpCode::getAddHalfWordImmOpCode(), node, cg->getReturnAddressRealRegister(), 0, cursor);
        ...
        cursor = generateS390BranchInstruction(
            cg, TR::InstOpCode::BCR, node, TR::InstOpCode::COND_BCR, cg->getEntryPointRealRegister(), cursor);
        ...
        TR::SymbolReference* helperSymRef
            = cg->symRefTab()->findOrCreateRuntimeHelper(TR_S390samplingRecompileMethod, false, false, false);
        ...
        AOTcgDiag4(comp, "Add encodingRelocation = %p reloType = %p symbolRef = %p helperId = %x\n", encodingRelocation,
            encodingRelocation->getReloType(), encodingRelocation->getSymbolReference(),
            TR_S390samplingRecompileMethod);
        ...
        if (TR::Compiler->target.is64Bit()) {
            cursor = generateDataConstantInstruction(
                cg, TR::InstOpCode::DC, node, UPPER_4_BYTES(samplingRecompileMethodAddress), cursor);
            cursor->setEncodingRelocation(encodingRelocation);

            cursor = generateDataConstantInstruction(
                cg, TR::InstOpCode::DC, node, LOWER_4_BYTES(samplingRecompileMethodAddress), cursor);
        } else {
            cursor
                = generateDataConstantInstruction(cg, TR::InstOpCode::DC, node, samplingRecompileMethodAddress, cursor);
            cursor->setEncodingRelocation(encodingRelocation);
        }
        ...
        cursor = generateS390BranchInstruction(
            cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, node, samplingRecompileMethodSnippetLabel, cursor);
    }

    if (cg->mustGenerateSwitchToInterpreterPrePrologue()) {
        cursor = generateS390BranchInstruction(
            cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, node, vmCallHelperSnippetLabel, cursor);
    } else {
...
TR::Instruction* TR_S390Recompilation::generatePrologue(TR::Instruction* cursor)
{
    ...
    cursor = generateRIInstruction(
        cg, TR::InstOpCode::getAddHalfWordImmOpCode(), node, cg->getEntryPointRealRegister(), offsetToIntEP, cursor);
    ...
    cursor = generateS390BranchInstruction(
        cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNL, node, resumeMethodExecutionLabel, cursor);
    ...
    TR::SymbolReference* helperSymRef
        = cg->symRefTab()->findOrCreateRuntimeHelper(TR_S390countingRecompileMethod, false, false, false);
    ...
    AOTcgDiag4(comp, "Add encodingRelocation = %p reloType = %p symbolRef = %p helperId = %x\n", encodingRelocation,
        encodingRelocation->getReloType(), encodingRelocation->getSymbolReference(), TR_S390countingRecompileMethod);

    const intptrj_t countingRecompileMethodAddress = reinterpret_cast<intptrj_t>(helperSymRef->getMethodAddress());

    if (TR::Compiler->target.is64Bit()) {
        cursor = generateDataConstantInstruction(
            cg, TR::InstOpCode::DC, node, UPPER_4_BYTES(countingRecompileMethodAddress), cursor);
        cursor->setEncodingRelocation(encodingRelocation);

        countingRecompileMethodAddressDataConstant = cursor;

        cursor = generateDataConstantInstruction(
            cg, TR::InstOpCode::DC, node, LOWER_4_BYTES(countingRecompileMethodAddress), cursor);
    } else {
    ...
    cursor = generateS390LabelInstruction(
        cg, TR::InstOpCode::LABEL, node, patchCallerBranchToCountingRecompiledMethodLabel, cursor);
    ...
    // Save EP register
    cursor = generateRXInstruction(
        cg, TR::InstOpCode::getStoreOpCode(), node, cg->getEntryPointRealRegister(), epRegisterSlotMemRef, cursor);
    ...
    cursor = generateRIInstruction(
        cg, TR::InstOpCode::getAddHalfWordImmOpCode(), node, cg->getReturnAddressRealRegister(), offsetToIntEP, cursor);
    ...
    cursor = generateRXInstruction(
        cg, TR::InstOpCode::getStoreOpCode(), node, cg->getReturnAddressRealRegister(), intEPAddressSlotMemRef, cursor);
    ...
    cursor = generateRXInstruction(
        cg, TR::InstOpCode::getLoadOpCode(), node, cg->getReturnAddressRealRegister(), bodyInfoAddressMemRef, cursor);
    ...
    AOTcgDiag4(comp, "Add encodingRelocation = %p reloType = %p symbolRef = %p helperId = %x\n", encodingRelocation,
        encodingRelocation->getReloType(), encodingRelocation->getSymbolReference(), TR_S390countingPatchCallSite);

    const intptrj_t countingPatchCallSiteAddress = reinterpret_cast<intptrj_t>(helperSymRef->getMethodAddress());

    if (TR::Compiler->target.is64Bit()) {
        cursor = generateDataConstantInstruction(
            cg, TR::InstOpCode::DC, node, UPPER_4_BYTES(countingPatchCallSiteAddress), cursor);
        cursor->setEncodingRelocation(encodingRelocation);

        countingPatchCallSiteAddressDataConstant = cursor;

        cursor = generateDataConstantInstruction(
            cg, TR::InstOpCode::DC, node, LOWER_4_BYTES(countingPatchCallSiteAddress), cursor);
    } else {
...
void TR_S390Recompilation::postCompilation()
{
    ...
    */
    ...
    if (debug("traceRecompilation")) {
        diagnostic(
            "RC>>Saved entry point %p(%x) to saveLocn %p(%x)\n", jitEntryPoint, *jitEntryPoint, saveLocn, *saveLocn);
    }
}
