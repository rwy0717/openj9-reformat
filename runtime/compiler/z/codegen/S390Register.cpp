...
TR_StorageReference* TR_StorageReference::createTemporaryBasedStorageReference(
    TR::SymbolReference* t, TR::Compilation* c)
{
...
TR_StorageReference* TR_StorageReference::createNodeBasedStorageReference(
    TR::Node* n, int32_t refCount, TR::Compilation* c)
{
        ...
        if (getNodesToUpdateOnClobber()) {
            ...
            for (TR::Node* update = toUpdate.getFirst(); update; update = toUpdate.getNext()) {
                ...
                if (updateReg && updateReg != reg && updateReg->getStorageReference() == reg->getStorageReference()) {
                    if (comp()->cg()->traceBCDCodeGen())
                        traceMsg(comp(),
                            "\ty^y : clear left-aligned state of reg %s with ref #%d (%s) on node %s (%p) refCount %d "
                            "from _nodesToUpdateOnClobber\n",
                            comp()->getDebug()->getName(updateReg),
                            updateReg->getStorageReference()->getReferenceNumber(),
...
bool TR_StorageReference::isNodeBased()
{
    if (getNode()) {
        TR_ASSERT(
            getTemporarySymbolReference() == NULL, "a node based storage reference should have a null temporary\n");
        return true;
    } else {
...
void TR_StorageReference::addNodeToUpdateOnClobber(TR::Node* node)
{
    if (node->getReferenceCount() > 1 || node->getRegister() != NULL) {
        ...
        if (_nodesToUpdateOnClobber->find(node)) {
            if (comp()->cg()->traceBCDCodeGen())
                traceMsg(comp(),
                    "\tNOT adding node %s (%p refCount %d) with reg %s to _nodesToUpdateOnClobber on ref #%d (%s) "
                    "(already present in the list)\n",
                    node->getOpCode().getName(), node, node->getReferenceCount(),
                    node->getRegister() ? comp()->getDebug()->getName(node->getRegister()) : "NULL",
                    getReferenceNumber(), comp()->getDebug()->getName(getSymbol()));
        } else {
    ...
    if (_storageReference && (ref != _storageReference)) {
        if (_storageReference->isTemporaryBased()) {
            ...
            if (storageRefNode->getOpCode().isLoadConst() || storageRefNode->getOpCode().isIndirect()) {
                ...
                if (comp()->cg()->traceBCDCodeGen())
                    traceMsg(comp(),
                        "\t\t\tdoRecursiveDecrement=%s on addrChild %p (refCount=%d), addrChild->firstChild %p "
                        "(refCount %d) if storageRefNode %p == node %p (%s) or nodeRefCount %d == 0 (%s)\n",
        ...
        if (comp()->cg()->traceBCDCodeGen())
            traceMsg(comp(),
                "\tsetting the new storageRef #%d (over existing storageRef #%d) on reg %s so reset "
                "leftAlignedZeroDigits and deadAndIgnoredBytes to 0\n",
...
// is +06666 indicies  43 21 0x   with x=-1 Note that this range is not inclusive. For example if
// startDigit=1,endDigit=3 then the first index included in the range is the startDigit value of 1 and the last number
// included in the range is endDigit-1=2 and the range size is 2 (endDigit-startDigit) This input scheme is used so the
    ...
    if (deadAndIgnoredBytes) {
        ...
        if (comp()->cg()->traceBCDCodeGen())
            traceMsg(comp(),
                ...
                deadAndIgnoredBytes, digitOffset, startDigit, startDigit + digitOffset, endDigit,
...
int32_t TR_PseudoRegister::getDigitsToClear(int32_t startDigit, int32_t endDigit)
{
    ...
    if (getLiveSymbolSize() < TR::DataType::getSizeFromBCDPrecision(getDataType(), endDigit)) {
        if (comp()->cg()->traceBCDCodeGen())
            traceMsg(comp(), "\tliveSymSize %d < endByte %d so return a conservative digitsToClear of %d\n",
                getLiveSymbolSize(), TR::DataType::getSizeFromBCDPrecision(getDataType(), endDigit),
                endDigit - startDigit);
        return endDigit - startDigit;
    }
                                          ...
                                          // so, then some digits will have to be cleared)
    {
            ...
            < leftAlignedZeroDigits) // is the startDigit of range is within the cleared range (i.e. an overlap)
        {
            if (comp()->cg()->traceBCDCodeGen())
                traceMsg(comp(),
                    "\t\tadjusting digitsToClear %d -> %d due to an overlap (rangeStart %d < leftAlignedZeroDigits "
                    "%d)\n",
                    digitsToClear, digitsToClear - (leftAlignedZeroDigits - rangeStart), rangeStart,
                    leftAlignedZeroDigits);
...
int32_t TR_PseudoRegister::getBytesToClear(
    int32_t startByte, int32_t endByte) // input range of bytes is right to left : startByte->endByte-1
{
...
int32_t TR_PseudoRegister::getByteOffsetFromLeftForClear(
    int32_t startDigit, int32_t endDigit, int32_t& digitsToClear, int32_t resultSize)
{
    TR_ASSERT(getKind() == TR_SSR, "only valid for TR_SSR registers\n");
    if (comp()->cg()->traceBCDCodeGen())
    ...
    if (leftAlignedZeroDigits > rangeStart) {
        if (comp()->cg()->traceBCDCodeGen())
            traceMsg(comp(),
                "\t\tsetting digitOffset to %d (== leftAlignedZeroDigits) as leftAlignedZeroDigits %d > rangeStart %d "
                "(an overlap)\n",
    ...
    if (digitOffset && (liveSymbolSize > resultSize)) {
        ...
        if (comp()->cg()->traceBCDCodeGen())
            traceMsg(comp(),
                ...
                liveSymbolSize, resultSize, getStorageReference()->getSymbolSize(),
                getRightAlignedDeadAndIgnoredBytes());
...
void TR_PseudoRegister::addRangeOfZeroDigits(int32_t startDigit, int32_t endDigit)
{
    ...
    if (rangeStart <= leftAlignedZeroDigits && rangeEnd > leftAlignedZeroDigits) {
        if (comp()->cg()->traceBCDCodeGen())
            traceMsg(comp(),
                "\t\tsetting leftAlignedZeroDigits to %d (leftAlignedZeroDigits %d + (rangeEnd %d - "
                "leftAlignedZeroDigits %d) because new range overlaps or is adjancent to current zero range\n",
...
void TR_PseudoRegister::addRangeOfZeroBytes(
    int32_t startByte, int32_t endByte) // input range of bytes is right to left : startByte->endByte-1
{
    if (startByte == endByte)
        return;
...
void TR_PseudoRegister::removeRangeOfZeroBytes(
    int32_t startByte, int32_t endByte) // input range of bytes is right to left : startByte->endByte-1
{
    if (!trackZeroDigits())
        return;
    ...
    if (comp()->cg()->traceBCDCodeGen())
        traceMsg(comp(), "\tremoveRangeOfZeroBytes %s (%s): (startByte=%d, endByte=%d)%s\n",
            comp()->cg()->getDebug()->getName(this), TR::DataType::getName(getDataType()), startByte, endByte,
            (leftAlignedZeroDigits == 0) ? " (zeroDigits==0 -- nothing to remove)"
                                         : ": defer to removeRangeOfZeroDigits");
    if (leftAlignedZeroDigits == 0)
        return;
