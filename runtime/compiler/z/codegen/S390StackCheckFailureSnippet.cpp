...
uint8_t* TR::S390StackCheckFailureSnippet::emitSnippetBody()
{
    ...
    *(int32_t*)cursor = (int32_t)((destAddr - (intptrj_t)(cursor - 2)) / 2);
    AOTcgDiag5(cg()->comp(), "cursor=%x destAddr=%x TR_HelperAddress=%x cg=%x %x\n", cursor,
        getDestination()->getSymbol(), TR_HelperAddress, cg(), *((int*)cursor));
    cg()->addProjectSpecializedRelocation(
        cursor, (uint8_t*)getDestination(), NULL, TR_HelperAddress, __FILE__, __LINE__, getNode());
...
void TR_Debug::print(TR::FILE* pOutFile, TR::S390StackCheckFailureSnippet* snippet)
{
    if (pOutFile == NULL) {
        return;
    }
    ...
    printPrefix(pOutFile, NULL, bufferPos, 6);
    trfprintf(pOutFile, "BRCL \t<%p>\t# Return to Main Code", snippet->getReStartLabel()->getCodeLocation());
}
