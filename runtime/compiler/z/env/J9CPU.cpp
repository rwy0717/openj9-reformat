...
TR_S390MachineType CPU::TO_PORTLIB_get390zLinuxMachineType()
{
    char line[80];
            ...
            if (len > PROC_HEADER_SIZE && !memcmp(line, procHeader, PROC_HEADER_SIZE)) {
                if (len == PROC_LINE_SIZE) {
                    ...
                    for (i = 0; i < sizeof(S390UnsupportedMachineTypes) / sizeof(int); ++i) {
                        if (machine == S390UnsupportedMachineTypes[i]) {
                            PORT_ACCESS_FROM_ENV(jitConfig->javaVM);
                            j9nls_printf(jitConfig->javaVM->portLibrary, J9NLS_ERROR,
                                J9NLS_J9JIT_390_UNSUPPORTED_HARDWARE, machine);
                            TR_ASSERT_FATAL(0, "Hardware is not supported.");
                        }
                    }
...
TR_S390MachineType CPU::TO_PORTLIB_get390zOSMachineType()
{
...
bool CPU::TO_PORTLIB_get390_supportsZNext()
{
...
bool CPU::TO_PORTLIB_get390_supportsZ14()
{
#if defined(TR_HOST_S390) && (defined(J9ZOS390) || defined(LINUX))
    J9ProcessorDesc* processorDesc = TR::Compiler->target.cpu.TO_PORTLIB_getJ9ProcessorDesc();
...
bool CPU::TO_PORTLIB_get390_supportsZ13()
{
#if defined(TR_HOST_S390) && (defined(J9ZOS390) || defined(LINUX))
    J9ProcessorDesc* processorDesc = TR::Compiler->target.cpu.TO_PORTLIB_getJ9ProcessorDesc();
...
bool CPU::TO_PORTLIB_get390_supportsZ6()
{
#if defined(TR_HOST_S390) && (defined(J9ZOS390) || defined(LINUX))
    J9ProcessorDesc* processorDesc = TR::Compiler->target.cpu.TO_PORTLIB_getJ9ProcessorDesc();
...
bool CPU::TO_PORTLIB_get390_supportsZGryphon()
{
...
bool CPU::TO_PORTLIB_get390_supportsZHelix()
{
#if defined(TR_HOST_S390) && (defined(J9ZOS390) || defined(LINUX))
    J9ProcessorDesc* processorDesc = TR::Compiler->target.cpu.TO_PORTLIB_getJ9ProcessorDesc();
...
bool CPU::TO_PORTLIB_get390zOS_N3Support()
{
...
bool CPU::TO_PORTLIB_get390zOS_ZArchSupport()
{
...
bool CPU::TO_PORTLIB_get390zOS_TrexSupport()
{
...
bool CPU::TO_PORTLIB_get390zOS_GoldenEagleSupport()
{
    ...
    return (((*(int*)200) & 0x00000400) != 0) && (((*(int*)200) & 0x00000200) != 0);
}

bool CPU::TO_PORTLIB_get390zOS_supportsStoreExtendedFacilityList()
{
...
bool CPU::getS390SupportsTM()
{
    return (_flags.testAny(S390SupportsTM) && !(TR::Options::getCmdLineOptions()->getOption(TR_DisableZEC12)));
}

bool CPU::getS390SupportsRI()
{
    return (_flags.testAny(S390SupportsRI) && !TR::Options::getCmdLineOptions()->getOption(TR_DisableZEC12)
        && !TR::Options::getCmdLineOptions()->getOption(TR_DisableZ13));
}

bool CPU::getS390SupportsVectorFacility()
{
    return (
        _flags.testAny(S390SupportsVectorFacility) && !(TR::Options::getCmdLineOptions()->getOption(TR_DisableZ13)));
}

bool CPU::getS390SupportsVectorPackedDecimalFacility()
{
    return (_flags.testAny(S390SupportsVectorPackedDecimalFacility)
        && !(TR::Options::getCmdLineOptions()->getOption(TR_DisableZ14)));
}

bool CPU::getS390SupportsGuardedStorageFacility()
{
    return (_flags.testAny(S390SupportsGuardedStorageFacility) && _flags.testAny(S390SupportsSideEffectAccessFacility));
}

////////////////////////////////////////////////////////////////////////////////

void CPU::initializeS390zLinuxProcessorFeatures()
{
    J9ProcessorDesc* processorDesc = TR::Compiler->target.cpu.TO_PORTLIB_getJ9ProcessorDesc();
    ...
    if (!TR::Compiler->target.cpu.getS390SupportsZ10()
        ...
        } else if (machineType == TR_TREX || machineType == TR_MIRAGE || machineType == TR_MIRAGE2
            || machineType == TR_Z890) {
    ...
    if (TR::Compiler->target.cpu.getS390SupportsZEC12()
        && j9sysinfo_processor_has_feature(processorDesc, J9PORT_S390_FEATURE_TE))
...
void CPU::initializeS390zOSProcessorFeatures()
{
    J9PortLibrary* privatePortLibrary = TR::Compiler->portLib;

    if (!TR::Compiler->target.cpu.TO_PORTLIB_get390zOS_N3Support()
        || !TR::Compiler->target.cpu.TO_PORTLIB_get390zOS_ZArchSupport()) {
    ...
    if (TR::Compiler->target.cpu.getS390SupportsZ900()
        ...
        if (TR::Compiler->target.cpu.getS390SupportsZEC12()) {
            ...
            U_8 cvttxj = *(
                cvtptr + 0x17b); // CVTTX and CVTTXC are at offset +0x17B of CVT, bits 8 and 4, indicating full support
            if ((cvttxj & 0xc) == 0xc) {
                TR::Compiler->target.cpu.setS390SupportsTM();
            }
...
bool CPU::isCompatible(TR_Processor processorSignature, TR_ProcessorFeatureFlags processorFeatureFlags)
{
    ...
    for (int i = 0; i < PROCESSOR_FEATURES_SIZE; i++) {
        ...
        if ((processorFeatureFlags.featureFlags[i] & self()->getProcessorFeatureFlags().featureFlags[i])
            != processorFeatureFlags.featureFlags[i])
            return false;
    }
    return true;
}
