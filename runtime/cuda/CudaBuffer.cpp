...
size_t arraySize(ArrayType array, jint length);
...
#define SPECIALIZE_ARRAY_SIZE(Type, Name)                                \
    template <>                                                          \
    inline size_t arraySize<Type##Array>(Type##Array array, jint length) \
    {                                                                    \
 ...
 */
template <typename ArrayType>
void getArrayRegion(JNIEnv* env, ArrayType array, jint start, jint length, void* buffer);
...
#define SPECIALIZE_GET_ARRAY_REGION(Type, Name)                                                                     \
    template <>                                                                                                     \
    inline void getArrayRegion<Type##Array>(JNIEnv * env, Type##Array array, jint start, jint length, void* buffer) \
    {                                                                                                               \
 ...
 */
template <typename ArrayType>
void setArrayRegion(JNIEnv* env, ArrayType array, jint start, jint length, void* buffer);
...
#define SPECIALIZE_SET_ARRAY_REGION(Type, Name)                                                                     \
    template <>                                                                                                     \
    inline void setArrayRegion<Type##Array>(JNIEnv * env, Type##Array array, jint start, jint length, void* buffer) \
    {                                                                                                               \
 ...
 */
template <typename ArrayType>
void copyFromHost(JNIEnv* env, jint deviceId, jlong devicePtr, ArrayType array, jint fromIndex, jint toIndex)
{
    ...
    if (NULL != buffer) {
        ...
        error = j9cuda_memcpyHostToDevice((uint32_t)deviceId, (void*)(uintptr_t)devicePtr, buffer, byteCount);
 ...
 */
template <typename ArrayType>
void copyToHost(JNIEnv* env, jint deviceId, jlong devicePtr, ArrayType array, jint fromIndex, jint toIndex)
{
    jint length = toIndex - fromIndex;
    size_t byteCount = arraySize(array, length);
    ...
    if (NULL != buffer) {
        error = j9cuda_memcpyDeviceToHost((uint32_t)deviceId, buffer, (void*)(uintptr_t)devicePtr, byteCount);
 ...
 */
jlong JNICALL Java_com_ibm_cuda_CudaBuffer_allocate(JNIEnv* env, jclass, jint deviceId, jlong byteCount)
{
 ...
 */
jobject JNICALL Java_com_ibm_cuda_CudaBuffer_allocateDirectBuffer(JNIEnv* env, jclass, jlong capacity)
{
    J9VMThread* thread = (J9VMThread*)env;
 ...
 */
void JNICALL Java_com_ibm_cuda_CudaBuffer_copyFromDevice(JNIEnv* env, jclass, jint targetDeviceId, jlong targetAddress,
    jint sourceDeviceId, jlong sourceAddress, jlong byteCount)
{
    J9VMThread* thread = (J9VMThread*)env;

    Trc_cuda_bufferCopyFromDevice_entry(
        thread, targetDeviceId, (uintptr_t)targetAddress, sourceDeviceId, (uintptr_t)sourceAddress, byteCount);
    ...
    if (targetDeviceId == sourceDeviceId) {
        error = j9cuda_memcpyDeviceToDevice((uint32_t)targetDeviceId, (void*)(uintptr_t)targetAddress,
            (const void*)(uintptr_t)sourceAddress, (uintptr_t)byteCount);
    } else {
        error = j9cuda_memcpyPeer((uint32_t)targetDeviceId, (void*)(uintptr_t)targetAddress, (uint32_t)sourceDeviceId,
            (const void*)(uintptr_t)sourceAddress, (uintptr_t)byteCount);
    }
    ...
    JNIEnv* env, jclass, jint deviceId, jlong devicePtr, jobject source, jlong fromOffset, jlong toOffset)
{
    ...
    if (NULL != buffer) {
        ...
        error = j9cuda_memcpyHostToDevice(
            (uint32_t)deviceId, (void*)(uintptr_t)devicePtr, &buffer[fromOffset], (size_t)(toOffset - fromOffset));
    }
    ...
    JNIEnv* env, jclass, jint deviceId, jlong devicePtr, jobject target, jlong fromOffset, jlong toOffset)
{
    ...
    if (NULL != buffer) {
        ...
        error = j9cuda_memcpyDeviceToHost(
            (uint32_t)deviceId, &buffer[fromOffset], (void*)(uintptr_t)devicePtr, (size_t)(toOffset - fromOffset));
    }
 ...
 */
void JNICALL Java_com_ibm_cuda_CudaBuffer_freeDirectBuffer(JNIEnv* env, jclass, jobject byteBuffer)
{
    J9VMThread* thread = (J9VMThread*)env;
 ...
 * @param[in] devicePtr  the device pointer
 */
void JNICALL Java_com_ibm_cuda_CudaBuffer_release(JNIEnv* env, jclass, jint deviceId, jlong devicePtr)
{
    J9VMThread* thread = (J9VMThread*)env;
