...
void* dbgMalloc(UDATA size, void* originalAddress)
{
    ...
    if (smallDbgAddressSpace.basePointer == NULL) {
        ...
        smallDbgAddressSpace.basePointer
            = j9vmem_reserve_memory(NULL, smallDbgAddressSpace.size, &smallDbgAddressSpace.vmemIdentifier,
                J9PORT_VMEM_MEMORY_MODE_READ | J9PORT_VMEM_MEMORY_MODE_WRITE, pageSize, OMRMEM_CATEGORY_VM);
        if (smallDbgAddressSpace.basePointer == NULL) {
    ...
    if ((size > smallDbgAddressSpace.size - (sizeof(dbgNode) + LOCAL_MEMORY_ALIGNMENT))
        || ((void*)((U_8*)smallDbgAddressSpace.basePointer + smallDbgAddressSpace.size - (size + sizeof(dbgNode)))
               < smallDbgAddressSpace.currentPointer)) {
        if (dbgVerboseMode) {
            dbgPrint("Unable to allocate requested %zu bytes in debug scratch space (%zu MB).\n",
                size + +LOCAL_MEMORY_ALIGNMENT + sizeof(dbgNode), smallDbgAddressSpace.size / (1024 * 1024));
        }
        return NULL;
    }
    ...
    smallDbgAddressSpace.currentPointer
        = (void*)(((UDATA)allocatedAddress + size + LOCAL_MEMORY_ALIGNMENT + sizeof(dbgNode) + sizeof(U_64) - 1)
            & ~(sizeof(U_64) - 1));
    ...
    while (smallDbgAddressSpace.commitPointer < smallDbgAddressSpace.currentPointer) {
        ...
        if (NULL
            == j9vmem_commit_memory(
                   smallDbgAddressSpace.commitPointer, pageSize, &smallDbgAddressSpace.vmemIdentifier)) {
            return NULL;
        }
...
void* dbgTargetToLocal(void* addr) { return dbgTargetToLocalWithSize(addr, 0); }
...
J9JavaVM* dbgSniffForJavaVM(void)
{
    ...
    if (totalBytesSearched == 0) {
        ...
        while (NULL
            != (eyecatcher = dbgFindPatternInRange(
                    (U_8*)"J9VMRAS", sizeof("J9VMRAS"), 8, startFrom, bytesToSearch, &bytesRead))) {
            J9RAS ras;
            totalBytesSearched += bytesRead;
...
U_8 dbgReadByte(U_8* remoteAddress)
{
    ...
    if (bytesRead != 1)
        dbgError("could not read byte at %p\n", remoteAddress);
    return byte;
}
        ...
        if (localVM) {
            ...
            if (localVM->jitConfig) {
                ...
                if (localVM->jitConfig->i2jReturnTable) {
                    DBG_TRY
                    {
                        localVM->jitConfig->i2jReturnTable = dbgMallocAndRead(
                            J9SW_JIT_RETURN_TABLE_SIZE * sizeof(UDATA), localVM->jitConfig->i2jReturnTable);
                    }
                    DBG_CATCH
                    {
...
dbgReadUDATA(UDATA* remoteAddress)
{
    ...
    if (bytesRead != sizeof(UDATA))
        dbgError("could not read UDATA at %p\n", remoteAddress);
    return data;
}
...
U_64 dbgReadU64(U_64* remoteAddress)
{
    ...
    if (bytesRead != sizeof(U_64))
        dbgError("could not read U_64 at %p\n", remoteAddress);
    return data;
}
...
/* since the order and size of the data are both orthogonally variable, we need an ifdef to split out the endian from
 * the problem */
#if !defined(J9VM_ENV_LITTLE_ENDIAN)
    value = value >> (8 * (sizeof(UDATA) - size));
...
U_16 dbgReadU16(U_16* remoteAddress)
{
    ...
    if (bytesRead != sizeof(U_16))
        dbgError("could not read U_16 at %p\n", remoteAddress);
    return data;
}
...
U_32 dbgReadU32(U_32* remoteAddress)
{
    ...
    if (bytesRead != sizeof(U_32))
        dbgError("could not read U_32 at %p\n", remoteAddress);
    return data;
}
...
void dbgext_setvm(const char* args) { dbgSetVM((void*)dbgGetExpression(args)); }
        ...
        if (((U_8*)addr >= lowAddr) && ((U_8*)addr < highAddr)) {
            if ((U_8*)addr + size > highAddr) {
                dbgError("Found partial memory match for %p at %p, but it only has %d bytes (needed %d)\n", addr,
                    current, highAddr - (U_8*)addr, size);
                return NULL;
            } else {
...
#if defined(J9ZOS390)
/* on z/OS, we redefine the portlib file_write_text as well as file_write so create this wrapper to satisfy the type
 * system */
static IDATA dbg_write_text(OMRPortLibrary* portLibrary, IDATA fd, const char* buf, IDATA nbytes)
{
...
void dbgSetVerboseMode(int verboseMode) { dbgVerboseMode = verboseMode; }
...
{
    UDATA len = 0;
    while (dbgReadByte((U_8*)remoteAddress + len))
        len++;
    return dbgMallocAndRead(len + 1, remoteAddress);
}

static J9PoolPuddle* dbgReadPoolPuddle(J9Pool* head, J9PoolPuddle* remotePuddle)
{
...
static J9PoolPuddleList* dbgReadPoolPuddleList(J9Pool* pool, J9PoolPuddleList* removePuddleList)
{
 ...
 * bytesSearched: number of bytes searched is returned through this pointer. 0 indicates that no attempt was made to
 * find the pattern.
 *
 * Returns:
