...
void(JNICALL* dbgjit_TrInitialize)(J9JavaVM* local_vm, J9PortLibrary* dbgPortLib,
    void (*dbgjit_Print)(const char* s, ...),
    void (*dbgjit_ReadMemory)(UDATA address, void* structure, UDATA size, UDATA* bytesRead),
    UDATA (*dbgjit_GetExpression)(const char* args), void* (*dbgjit_Malloc)(UDATA size, void* originalAddress),
    void* (*dbgjit_MallocAndRead)(UDATA size, void* remoteAddress), void (*dbgjit_Free)(void* addr),
    void* (*dbgjit_FindPatternInRange)(U_8* pattern, UDATA patternLength, UDATA patternAlignment, U_8* startSearchFrom,
...
dbgTrInitialize(void)
{
    ...
    dbgjit_TrInitialize(localVM, PORTLIB, &dbgPrint, &dbgReadMemory, &dbgGetExpression, &dbgMalloc, &dbgMallocAndRead,
        &dbgFree, &dbgFindPatternInRange);
