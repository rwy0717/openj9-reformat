...
#if defined(WIN32)
int translated_main(int argc, char** argv, char** envp)
#else
    ...
    if (0 == omrthread_init_library()) {
        ...
        if (0 == omrthread_attach_ex(&attachedThread, J9THREAD_ATTR_DEFAULT)) {
            ...
            if (0 == omrport_init_library(&j9portLibrary, sizeof(OMRPortLibrary))) {
                ...
                if (j9portLibrary.sig_protect(&j9portLibrary, ddrSignalProtectedMain, &options, genericSignalHandler,
                        NULL, J9PORT_SIG_FLAG_SIGALLSYNC, &result)
                    == 0) {
                    rc = (int)result;
                }
