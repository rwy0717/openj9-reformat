...
static jint processDDRConstant PROTOTYPE(
    (J9DDRTranslationData * data, const J9DDRConstantDeclaration* ddrConstant, U_32* size));
...
static void ddr_setNLSCatalog(OMRPortLibrary* portLib, char** argv)
{
    char* exename = NULL;
...
static jint createDDRCoreData(J9DDRTranslationData* data)
{
    ...
    stringTable = hashTableNew(OMRPORTLIB, J9_GET_CALLSITE(), 0, sizeof(J9DDRStringTableEntry), 0, 0,
        OMRMEM_CATEGORY_VM, stringTableHash, stringTableEquals, NULL, NULL);
    if (stringTable == NULL) {
