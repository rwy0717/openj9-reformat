...
static ddrExtEnv ddrEnv = { NULL, NULL, NULL };
...
jboolean ddrValidatePath(char* path)
{
#if defined(WIN32)
    DWORD result;
...
jboolean ddrValidateClassPath(char* path)
{
#if defined(WIN32)
...
static int aliasCommandNames(char** names)
{
    int i = 0;
    char* name;
...
static char** retrieveCommandNames(ddrExtEnv* ddrenv)
{
    char** names;
    jarray commandNames;
    ...
    if (mid == NULL) {
        dbgWriteString(
            "DDREXT: failed to find getCommandNames method in com.ibm.j9ddr.tools.ddrinteractive.DDRInteractive\n");
        return JNI_FALSE;
    }
...
JavaVM* ddrStart()
{
    ...
    mid = (*env)->GetStaticMethodID(
        env, cls, "instantiateDDR", "()Lcom/ibm/j9ddr/tools/ddrinteractive/DDRInteractive;");
    if (mid == NULL) {
...
failed_detection:
    ...
    dbgPrint(
        "DDR: Quite likely running on a machine that does not have /bluebird l:\\ (QV) or /j9vm (Torolab) mounted\n");
    ...
    dbgPrint("DDR: export "
             "J9DDR_CLASSPATH=/workspace/debugtools/DDR_Artifacts/libs/j9ddr.jar:/workspace/debugtools/DDR_VM/lib/"
             "asm-3.1.jar\n");
