 ...
 *    space.
 */
void* dbgFindPatternInRangeFullScan(U_8* pattern, UDATA patternLength, UDATA patternAlignment, U_8* startSearchFrom,
    ...
    *bytesSearched = 0;
    ...
    if (dbgSupportsRegions()) {
        ...
        while (dbgRegionIteratorHasNext(&regionIteratorState)) {
            ...
            bytesToSearchWithinTheRegion
                = (UDATA)(lastByteToSearchWithinTheRegion - whereToStartTheSearchWithinTheRegion + 1);
            bytesSearchedWithinTheRegion = 0;
            patternAddress = dbgFindPatternInRangeFullScan(pattern, patternLength, patternAlignment,
                whereToStartTheSearchWithinTheRegion, bytesToSearchWithinTheRegion, &bytesSearchedWithinTheRegion);
            if (NULL != patternAddress) {
                /* found the pattern, so consider that the total bytes searched is from the initial start point to where
                 * we found it (plus the pattern itself) */
                *bytesSearched = (UDATA)(patternAddress - startSearchFrom) + patternLength;
                return patternAddress;
            } else {
                /* did not find the pattern, so consider that the total bytes searched so far is from the initial start
                 * point to where we stopped within the region */
        ...
        *bytesSearched = (UDATA)(lastByteToSearch - startSearchFrom + 1);
        ...
        /* do it the slow way. */
        return dbgFindPatternInRangeFullScan(
            pattern, patternLength, patternAlignment, startSearchFrom, bytesToSearch, bytesSearched);
    }
}
