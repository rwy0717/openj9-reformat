...
static IDATA convertString(
    JNIEnv* env, J9PortLibrary* j9portLibrary, jclass utilClass, jmethodID utilMid, char* chars, jstring* str);
...
I_32 main_appendToClassPath(J9PortLibrary* portLib, U_16 sep, J9StringBuffer** classPath, char* toAppend)
{
    ...
    if ((NULL != *classPath) && ((*classPath)->data[strlen((const char*)(*classPath)->data)] != sep)) {
        char separator[2];
...
int main_runJavaMain(
    JNIEnv* env, char* mainClassName, int nameIsUTF, int java_argc, char** java_argv, J9PortLibrary* j9portLibrary)
{
    int i, rc = 0;
    jclass cls;
    ...
    *slashes = '\0';
    ...
    if (NULL == utilClass) {
        /* J9NLS_EXELIB_INTERNAL_VM_ERR_FAILED_CREATE_JLS_FOR_CLASSNAME=Internal VM error: Failed to create
         * java/lang/String for class name %s\n */
    ...
    if (nameIsUTF) {
        ...
        if (rcConvert == 1) {
            /* J9NLS_EXELIB_INTERNAL_VM_ERR_FAILED_CREATE_BA=Internal VM error: Failed to create byte array for class
             * name %s\n */
        ...
        if (rcConvert == 2) {
             ...
             * java/lang/String for class name %s\n */
            j9nls_printf(
                PORTLIB, J9NLS_ERROR, J9NLS_EXELIB_INTERNAL_VM_ERR_FAILED_CREATE_JLS_FOR_CLASSNAME, mainClassName);
            rc = 11;
            goto done;
        }
        utfClassName = (const char*)(*env)->GetStringUTFChars(env, str, &isCopy);
        if (utfClassName == NULL) {
            /* J9NLS_EXELIB_INTERNAL_VM_ERR_OUT_OF_MEMORY_CONVERTING=Internal VM error: Out of memory converting string
             * to UTF Chars for class name %s\n */
    ...
    for (i = 0; i < java_argc; ++i) {
        ...
        if (rcConvert == 1) {
            /* J9NLS_EXELIB_INTERNAL_VM_ERR_FAILED_CREATE_BYTE_ARRAY=Internal VM error: Failed to create byte array for
             * argument %s\n */
        ...
        if (rcConvert == 2) {
            /* J9NLS_EXELIB_INTERNAL_VM_ERR_FAILED_CREATE_JLS_FOR_ARG=Internal VM error: Failed to create
             * java/lang/String for argument %s\n*/
        ...
        if ((*env)->ExceptionCheck(env)) {
            /* J9NLS_EXELIB_INTERNAL_VM_ERR_FAILED_SET_ARRAY_ELEM=Internal VM error: Failed to set array element for
             * %s\n */
            j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_EXELIB_INTERNAL_VM_ERR_FAILED_SET_ARRAY_ELEM, java_argv[i]);
            rc = 9;
            goto done;
        }
        (*env)->DeleteLocalRef(env, str);
...
char* main_vmVersionString(void)
{
    ...
    if ((minorVersion % 10) == 0)
        minorVersion /= 10;
    sprintf(versionStringPtr, "%d.%d%s,%s %s", EsVersionMajor, minorVersion, EsExtraVersionString, native,
...
char* vmDetailString(J9PortLibrary* portLib, char* detailString, UDATA detailStringLength)
{
    ...
    j9str_printf(PORTLIB, detailString, detailStringLength, "%s (%s %s %s)", EsBuildVersionString,
        ostype ? ostype : "unknown", osversion ? osversion : "unknown", osarch ? osarch : "unknown");
    return detailString;
}
...
void main_setNLSCatalog(J9PortLibrary* portLib, char** argv)
{
    ...
    if (j9sysinfo_get_executable_name(argv[0], &exename) == 0) {
        ...
        if (NULL != j9lib) {
            ...
            /* leave the platform name, it gets removed by j9nls_set_catalog as it ignores everything after the last
             * slash. */
#endif /* WIN32 */
            nlsPaths[1] = j9lib;
        }
...
I_32 main_findDefaultOptionsFile(J9PortLibrary* portLib, char* argv0, char** optionFileName)
{
    ...
    defaultOptionFileName
        = j9mem_allocate_memory(strlen(exeName) + 6, OMRMEM_CATEGORY_VM); /* +6 == ".j9vm" + zero termination */
