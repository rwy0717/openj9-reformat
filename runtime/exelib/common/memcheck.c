...
static const UDATA unlockMode
    = J9PORT_VMEM_MEMORY_MODE_READ | J9PORT_VMEM_MEMORY_MODE_WRITE /*| J9PORT_VMEM_MEMORY_MODE_EXECUTE*/;
static const UDATA allocateMode
...
#define J9_MEMCHECK_LOCK(ptr)                                                                   \
    ...
    ((memoryCheck_lockGuardPages(memCheckPortLib, (ptr), J9_MEMCHECK_PAGE_SIZE, lockMode) == 0) \
            ? (void)0                                                                           \
            : memCheckPortLib->tty_printf(memCheckPortLib, "LOCK FAIL: (%s)(%d)\n", __FILE__, __LINE__))

#define J9_MEMCHECK_UNLOCK(ptr)                                                                             \
    ((memoryCheck_lockGuardPages(memCheckPortLib, (ptr), J9_MEMCHECK_PAGE_SIZE, unlockMode) == 0)           \
            ? (void)0                                                                                       \
            : memCheckPortLib->tty_printf(memCheckPortLib, "UNLOCK FAIL: (%s)(%d)\n", __FILE__, __LINE__)); \
    ((J9MemoryCheckHeader*)(ptr))->isLocked = 0 /* Set to unlocked */

#define J9_MEMCHECK_LOCK_BODY(blockHeader) memoryCheck_lockWrappedBlock(memCheckPortLib, (blockHeader), lockMode)

#define J9_MEMCHECK_UNLOCK_BODY(blockHeader) memoryCheck_lockWrappedBlock(memCheckPortLib, (blockHeader), unlockMode)
 ...
 */
#define BYTES_FOR_ALIGNMENT(numberOfBytes) \
    (U_32)((sizeof(double) - (((UDATA)numberOfBytes) & (sizeof(double) - 1))) & (sizeof(double) - 1))
...
static void memoryCheck_update_callSites_allocate(
    OMRPortLibrary* portLib, J9MemoryCheckHeader* header, const char* callSite, UDATA byteAmount);
static void* memoryCheck_wrapper_allocate_memory(OMRPortLibrary* portLib, UDATA byteAmount, char const* operationName,
    J9_MEM_ALLOCATE_FUNC allocator, U_32 paddingValue, U_32 fillValue, U_32 freedValue, const char* callSite,
    U_32 category);
...
static void memoryCheck_fill_bytes(
    OMRPortLibrary* portLib, U_8* fillAddress, UDATA fillSize, U_32 fillValue, U_8* blockAddress);
...
static BOOLEAN memoryCheck_describe_freed_block(
    OMRPortLibrary* portLib, char const* operationName, J9MemoryCheckHeader* blockHeader);
...
static void* memoryCheck_wrapper_reallocate_memory(OMRPortLibrary* portLib, void* memoryPointer, UDATA byteAmount,
    char const* operationName, J9_MEM_ALLOCATE_FUNC allocator, J9_MEM_FREE_FUNC deallocator, U_32 paddingValue,
    U_32 fillValue, U_32 freedValue, const char* callSite, U_32 category);
static UDATA memoryCheck_verify_backward(
    OMRPortLibrary* portLib, U_8* fillAddress, UDATA fillSize, U_32 fillValue, U_8* blockAddress);
static void memoryCheck_print_stats_callSite(OMRPortLibrary* portLib, J9MEMAVLTreeNode* node);
static void* memoryCheck_reallocate_memory(
    struct OMRPortLibrary* portLibrary, void* memoryPointer, UDATA byteAmount, const char* callsite, U_32 category);
...
static void* memoryCheck_allocate_memory(
    OMRPortLibrary* portLib, UDATA byteAmount, const char* callSite, U_32 category);
static void* memoryCheck_allocate_memory32(
    OMRPortLibrary* portLib, UDATA byteAmount, const char* callSite, U_32 category);
...
static UDATA memoryCheck_verify_forward(
    OMRPortLibrary* portLib, U_8* fillAddress, UDATA fillSize, U_32 fillValue, U_8* blockAddress);
...
static void memoryCheck_wrapper_free_memory(OMRPortLibrary* portLib, void* memoryPointer, char const* operationName,
    J9_MEM_FREE_FUNC deallocator, U_32 paddingValue, U_32 fillValue, U_32 freedValue);
static void memoryCheck_set_AVLTree_prevStats(J9MEMAVLTreeNode* node);
static BOOLEAN memoryCheck_describe_block(
    OMRPortLibrary* portLib, char const* operationName, J9MemoryCheckHeader* blockHeader);
...
static void memoryCheck_lockAllBlocks(
    OMRPortLibrary* portLib, J9MemoryCheckHeader* listHead, const UDATA lockFlags, const UDATA lockBody);
...
#define J9_MEMCHECK_ADJUSTED_PADDING                                                                   \
    ((((J9_MEMCHECK_PADDING_SIZE > J9_MEMCHECK_MINIMUM_HEADER_SIZE) ? J9_MEMCHECK_PADDING_SIZE         \
                                                                    : J9_MEMCHECK_MINIMUM_HEADER_SIZE) \
         + 7)                                                                                          \
        & ~7)
...
#define DLL_NAME J9_THREAD_DLL_NAME
    ...
    if (portLib->sl_lookup_name(
            portLib, j9thrDescriptor, "omrthread_attach_ex", (UDATA*)&f_omrthread_attach_ex, "LL")) {
        portLib->nls_printf(portLib, J9NLS_ERROR, J9NLS_MEMCHK_INIT_ERROR);
        return -1;
    }
    ...
    if (0 != (mode & J9_MCMODE_NO_SCAN)) {
        ...
        if (0
            != (mode & (~J9_MCMODE_PRINT_CALLSITES_SMALL) & (~J9_MCMODE_PRINT_CALLSITES) & (~J9_MCMODE_ZERO)
                   & (~J9_MCMODE_NO_SCAN) & (~J9_MCMODE_SUB_ALLOCATOR) & (~J9_MCMODE_FAIL_AT))) {
            /* TODO - this should be an NLS message */
            portLib->tty_err_printf(portLib,
                "-Xcheck:memory:noscan is only supported with 'callsitesmall', 'callsite', 'failat' and 'zero'. "
                "Calling exit(3)\n",
                mode);
            exit(3);
        }
    ...
    vmemIDTable = hashTableNew(memCheckPortLib, J9_GET_CALLSITE(), 9391, sizeof(J9PortVmemIdentifier*), 0, 0,
        OMRMEM_CATEGORY_VM, memoryCheck_hashFn, memoryCheck_hashEqualFn, NULL, NULL);
    if (NULL == vmemIDTable) {
    ...
    if (mode & J9_MCMODE_SUB_ALLOCATOR) {
        ...
        if (NULL == j9heap) {
            ...
            portLib->tty_printf(
                portLib, "Initial allocation of subAllocator heap failed. Tried for %i MB\n", heapSizeMegaBytes);

            while (heapSizeBytes >= 1 * 1024 * 1024) {
                heapSizeBytes = heapSizeBytes / 2;
                j9heap = memCheckPortLib->mem_allocate_memory(
                    portLib, heapSizeBytes, J9_GET_CALLSITE(), OMRMEM_CATEGORY_VM);
                if (NULL != j9heap) {
                    break;
                }
            }
        ...
        if (NULL == j9heap) {
            /* give up */
            portLib->tty_printf(
                portLib, "Unable to allocate subAllocator heap of size %i MB), calling exit(3)\n\n", heapSizeMegaBytes);
            exit(3);
        }

        portLib->tty_printf(portLib, "Successfully allocated subAllocator heap of size %i MB\n\n", heapSizeMegaBytes);
        subAllocator_init_heap(
            j9heap, heapSizeBytes / sizeof(UDATA)); /* NOTE: the j9heap is an array of UDATA sized blocks  */
...
static BOOLEAN memoryCheck_parseOption(OMRPortLibrary* portLib, char const* option, size_t optionLen)
{
    ...
    if ((optionLen > optLenIgnoreUnfreedCallsite)
        && !strncmp(option, optStrIgnoreUnfreedCallsite, optLenIgnoreUnfreedCallsite)) {
        if (strlen(&(option[optLenIgnoreUnfreedCallsite])) < IGNORECALLSITESTRLENGTH) {
            ...
            portLib->tty_printf(
                portLib, "WARNING : IgnoreUnfreedCallsite option too long for internal buffer. Ignoring\n");
        }
        return TRUE;
    }
    ...
    if ((optionLen == optLenIgnoreUnknownBlocks)
        && !strncmp(option, optStrIgnoreUnknownBlocks, optLenIgnoreUnknownBlocks)) {
...
/* This method prints whatever information as it can gather about a block from the block and its immediate neighbors (if
   any). If the block looks like a correct block, TRUE is returned.  If anything bogus is discovered about the block,
   FALSE is returned. */
    ...
    OMRPortLibrary* portLib, char const* operationName, J9MemoryCheckHeader* blockHeader)
{
    ...
    portLib->tty_printf(
        portLib, "%s describing block at %p (header at %p):\n", operationName, wrappedBlock, blockHeader);
        ...
        if ((UDATA)topPadding % 8 == 4) {
            if (!memoryCheck_verify_forward(portLib, topPadding + 4, 8, J9_MEMCHECK_DATA_PADDING_VALUE, wrappedBlock)) {
                paddingValue = J9_MEMCHECK_DATA_PADDING_VALUE;
                           ...
                           portLib, topPadding + 4, 8, J9_MEMCHECK_CODE_PADDING_VALUE, wrappedBlock)) {
                paddingValue = J9_MEMCHECK_CODE_PADDING_VALUE;
            }
    ...
    if (mode & J9_MCMODE_MPROTECT) {
        ...
        bottomPaddingSmashed
            = memoryCheck_verify_backward(portLib, bottomPadding, checkSize, paddingValue, wrappedBlock);
    } else {
        bottomPaddingSmashed
            = memoryCheck_verify_backward(portLib, bottomPadding, bottomPaddingSize, paddingValue, wrappedBlock);
    }
    ...
    if (mode & J9_MCMODE_MPROTECT) {
        memoryCheck_dump_bytes(
            portLib, bottomPadding, blockHeader->totalAllocation - (bottomPadding - blockHeader->self));
    } else {
...
/* This method fills fillSize bytes at fillAddress with copies of a 64-bit value formed by splitting fillValue into 2
   16-bit values and inserting the low 32 bits of the block address in between.  Each instance of fillValue is aligned
   on an 8-byte boundary, so if fillAddress and/or fillAddress+fillSize is not 8-byte aligned, a partial copy will
   appear at the start and/or end. */
...
static UDATA memoryCheck_hashDoFn(void* entry, void* portLib)
{
    if (entry && portLib) {
        ...
        /* Ensure the node was allocated by us, as every node will have an address which is a multiple of
 ...
 */
static IDATA memoryCheck_lockGuardPages(OMRPortLibrary* portLib, void* memcheckHeader, UDATA size, UDATA requestedMode)
{
    if (mode & J9_MCMODE_MPROTECT) {
        void* ret = NULL;
        ...
        if (!J9_ALIGN_BOTTOM) {
            ...
            topPageBoundary
                = topPageBoundary - (UDATA)topPageBoundary % J9_MEMCHECK_PAGE_SIZE; /* Align it to a page Boundary */
#endif
        }
 ...
 * @return 0 on Success, -1 on failure
 */
static IDATA memoryCheck_lockWrappedBlock(
    OMRPortLibrary* portLib, J9MemoryCheckHeader* blockHeader, const UDATA requestedMode)
{
    if (mode & J9_MCMODE_MPROTECT) {
        void* ret = NULL;
        U_8* topPageBoundary = NULL;
        ...
        userPages = blockHeader->wrappedBlockSize / J9_MEMCHECK_PAGE_SIZE
            + (blockHeader->wrappedBlockSize % J9_MEMCHECK_PAGE_SIZE ? 1 : 0);

        /* Update and commit struct to lock the userPages */
        oldMode = getFromTablePtr->mode;
        getFromTablePtr->mode = requestedMode;
        ret = omrvmem_commit_memory(
            topPageBoundary + getFromTablePtr->pageSize, userPages * getFromTablePtr->pageSize, getFromTablePtr);
        getFromTablePtr->mode = oldMode;
        if (NULL == ret) {
            return -1;
        }
...
static BOOLEAN memoryCheck_scan_all_blocks(OMRPortLibrary* portLib)
{
    ...
    for (prevBlockHeader = NULL, blockHeader = mostRecentBlock; blockHeader != NULL;
         prevBlockHeader = blockHeader, blockHeader = blockHeader->previousBlock) {
                ...
                if (memoryCheck_verify_forward(
                    ...
                    if (blockHeader->wrappedBlockSize == J9_MEMCHECK_FREED_SIZE) {
                        portLib->tty_printf(portLib,
                            "ERROR: Found an already freed block %p (header at %p) on the allocated list (should never "
                            "happen)!\n",
                            wrappedBlock, blockHeader);
                        goto describe_block_and_fail;
                    }

                    /* Block not freed yet, but top padding at least is damaged.  If the nextBlock pointer is damaged
                       too then we assume the header is useless and we stop scanning (to reduce the chances of a GPF).
                     */
        ...
        if (prevBlockHeader && (blockHeader->allocationNumber >= prevBlockHeader->allocationNumber)) {
            portLib->tty_printf(portLib,
                "ERROR: Allocation number %d in block %p (header at %p) should be lower than %d in block %p (header at "
                "%p)\n",
                blockHeader->allocationNumber, wrappedBlock, blockHeader, prevBlockHeader->allocationNumber,
                ((U_8*)prevBlockHeader) + J9_MEMCHECK_ADJUSTED_PADDING, prevBlockHeader);
            goto describe_block_and_fail; /* If header is screwed then we risk a GPF by continuing.. */
        }
        ...
        if (blockHeader->nextBlock != prevBlockHeader) {
            portLib->tty_printf(portLib,
                ...
                ((U_8*)prevBlockHeader) + J9_MEMCHECK_ADJUSTED_PADDING, prevBlockHeader, wrappedBlock, blockHeader);
        }
        ...
        if (mode & J9_MCMODE_MPROTECT) {
            ...
            /* Adjust bottomPadding to force it to end on a double boundary */
            bottomPaddingSize
                = blockHeader->totalAllocation - J9_MEMCHECK_PAGE_SIZE - (bottomPadding - blockHeader->topPage);
        } else {
    ...
    if (mode & J9_MCMODE_NEVER_FREE) {
        ...
        for (prevBlockHeader = NULL, blockHeader = mostRecentFreedBlock; blockHeader != NULL;
             prevBlockHeader = blockHeader, blockHeader = blockHeader->previousBlock) {
            ...
            if ((mode & J9_MCMODE_MPROTECT) && !J9_ALIGN_BOTTOM) {
                ...
                if (memoryCheck_verify_forward(
                    ...
                    if (memoryCheck_verify_forward(
                        ...
                        if (blockHeader->nextBlock != prevBlockHeader) {
                            portLib->tty_printf(portLib,
                                "ERROR: Header of previously freed block at %p (header at %p) is damaged, scan "
                                "stopped.\n",
                                wrappedBlock, blockHeader);
            ...
            if (blockHeader->allocationNumber > memStats.totalBlocksAllocated) {
                portLib->tty_printf(portLib,
                    "ERROR: Invalid allocation number %d in previously freed block %p (header at %p)\n",
                    blockHeader->allocationNumber, wrappedBlock, blockHeader);
            ...
            if (blockHeader->nextBlock != prevBlockHeader) {
                portLib->tty_printf(portLib,
                    "ERROR: Previously freed block list links between %p (header at %p) and %p (header at %p) are "
                    "corrupt\n",
                    ((U_8*)prevBlockHeader) + J9_MEMCHECK_ADJUSTED_PADDING, prevBlockHeader, wrappedBlock, blockHeader);
            }
    ...
    U_32 category)
{
    ...
    if (0 == (mode & J9_MCMODE_NO_SCAN)) {
        ...
        if (mode & J9_MCMODE_MPROTECT) {
            ...
            allocAmount = sizeof(J9MemoryCheckHeader) + topPaddingSize
                + (byteAmount / J9_MEMCHECK_PAGE_SIZE) * J9_MEMCHECK_PAGE_SIZE
        ...
        if ((mode & J9_MCMODE_FAIL_AT) && (newAllocationNumber >= failAtAlloc)) {
            ...
            if ((memStats.totalBlocksAllocated + memStats.failedAllocs) == failAtAlloc) {
                portLib->tty_printf(portLib,
                    "WARNING: failat: Returning NULL for allocation attempt %d in %s at %s (%d bytes were requested)\n",
                    (memStats.totalBlocksAllocated + memStats.failedAllocs), operationName, callSite, byteAmount);
                portLib->tty_printf(
                    portLib, "         All subsequent memory allocation attempt will be failed intentionally\n");
            }
        ...
        if ((mode & J9_MCMODE_LIMIT) && (newTotalBytesAllocated > limitAlloc)) {
            if (memStats.currentBytesAllocated <= limitAlloc) {
                portLib->tty_printf(portLib,
                    "WARNING: limit: Returning NULL for attempt to allocate %d bytes in %s at %s (%d bytes already "
                    "allocated)\n",
        ...
        if (0 == (mode & J9_MCMODE_MPROTECT)) {
             ...
             */
            vmemID = memCheckPortLib->mem_allocate_memory(
                memCheckPortLib, sizeof(J9PortVmemIdentifier), callSite, OMRMEM_CATEGORY_VM);
            if (NULL == vmemID) {
                ++memStats.failedAllocs;
                portLib->tty_printf(portLib,
                    "WARNING: J9PortVmemIdentifier struct could not be allocated!\n"
                    "WARNING: Out of memory in %s at %s, returning NULL (%d bytes were requested)\n",
             ...
             * macro for the pageSize parameter to this function call only!  The macro may return a multiple of the
             * system page size due to the default implementation of j9vmem using sizeof(UDATA) as a pagesize.
             */
            ...
            if (!hashTableAdd(vmemIDTable, &vmemID)) {
                ...
                ++memStats.failedAllocs;
                portLib->tty_printf(portLib,
                    "WARNING: Unable to add vmemID to hashTable! In %s, returning NULL (%d bytes were requested)\n",
            ...
            bottomPadding = wrappedBlock + byteAmount;
            bottomPage = wrappedBlock + (byteAmount / J9_MEMCHECK_PAGE_SIZE) * J9_MEMCHECK_PAGE_SIZE
                + (byteAmount % J9_MEMCHECK_PAGE_SIZE ? 1 : 0) * J9_MEMCHECK_PAGE_SIZE;
            ...
            if (J9_ALIGN_BOTTOM) {
                ...
                if (wrappedBlock - J9_MEMCHECK_PAGE_SIZE + sizeof(J9MemoryCheckHeader)
                    > topPage + J9_MEMCHECK_PAGE_SIZE) {
                    memmove(wrappedBlock - J9_MEMCHECK_PAGE_SIZE - sizeof(J9MemoryCheckHeader), blockHeader,
                        sizeof(J9MemoryCheckHeader));
                    blockHeader
                        = (J9MemoryCheckHeader*)(wrappedBlock - J9_MEMCHECK_PAGE_SIZE - sizeof(J9MemoryCheckHeader));
                } else {
        ...
        if (!(mode & J9_MCMODE_MPROTECT)) {
            ...
            if ((mode & J9_MCMODE_SKIP_TO) && (newAllocationNumber < skipToAlloc)) {
                ...
                if (blockHeader->self != blockHeader->topPage) {
                    memoryCheck_fill_bytes(portLib, blockHeader->self, blockHeader->topPage - blockHeader->self,
                        paddingValue, wrappedBlock);
                }
                /* Normal top Padding */
                memoryCheck_fill_bytes(portLib, topPadding, wrappedBlock - topPadding, paddingValue, wrappedBlock);
                /* Extra padding below */
                memoryCheck_fill_bytes(portLib, bottomPadding, allocAmount - (bottomPadding - blockHeader->self),
                    paddingValue, wrappedBlock);
                if (mode & J9_MCMODE_ZERO) {
                    memset(wrappedBlock, 0, byteAmount);
                } else {
        ...
        if (mode & J9_MCMODE_SIMULATE) {
            ...
            portLib->tty_printf(
                portLib, "BLOCK_%d = j9mem_allocate_memory(%d); %s\n", newAllocationNumber, byteAmount, SIM_TAG);
        }
#endif
    ...
    J9_MEM_FREE_FUNC deallocator, U_32 paddingValue, U_32 fillValue, U_32 freedValue)
{
    ...
    if (0 == (mode & J9_MCMODE_NO_SCAN)) {
        ...
        if ((mode & J9_MCMODE_FULL_SCANS) || (!currentBlockOkay)) {
            /* The current block could be an unknown block, but before we check to see if the block is on the allocate
             * list we have to scan all blocks to make sure the list is not trashed */
        ...
        if ((mode & J9_MCMODE_FULL_SCANS) || (!currentBlockOkay && listHeadersAndPaddingOkay)) {
            /* Walk the allocated list to make sure this block is on it.  If not, then we're either freeing random
               garbage or this block has already been freed, or this block was not allocated by the port library */
            ...
            if (mode & J9_MCMODE_IGNORE_UNKNOWN_BLOCKS) {
                ...
                portLib->tty_printf(portLib,
                    "WARNING: Tried %s on %p (header at %p), but block is not on the allocated list.\n", operationName,
                    wrappedBlock, topHeader);
                ...
                portLib->tty_printf(portLib,
                    "ERROR: Tried %s on %p (header at %p), but block is not on the allocated list\n", operationName,
                    wrappedBlock, topHeader);
                goto describe_block_and_fail;
            }
        } else {
        ...
        if (mode & J9_MCMODE_MPROTECT) {
            if (J9_ALIGN_BOTTOM) {
                if (topHeader->self != topHeader->topPage) {
                    memoryCheck_fill_bytes(
                        portLib, topHeader->self, topHeader->topPage - topHeader->self, freedValue, wrappedBlock);
                }
            }
            memoryCheck_fill_bytes(portLib, topPadding,
                topHeader->totalAllocation - ((UDATA)topPadding - (UDATA)topHeader->self), freedValue, wrappedBlock);
        } else if (0 == (mode & J9_MCMODE_NO_SCAN)) {
            memoryCheck_fill_bytes(portLib, topPadding,
                topPaddingSize + topHeader->wrappedBlockSize + bottomPaddingSize, freedValue, wrappedBlock);
        }
    }
    ...
    if (!blockWasSkipped && (mode & J9_MCMODE_NEVER_FREE)) {
        ...
        if (!(mode & J9_MCMODE_MPROTECT)) {
            ...
            /* Create a J9PortVmemIdentifier with the correct address, that can be used to get the right object from the
             * hashTable */
...
/**
 * @param portLib	The "memCheckPortLib" with its orignal functions is the one that should be used here (I think that
 * should read:
...
static void memoryCheck_print_stats(OMRPortLibrary* portLib)
{
    ...
    portLib->tty_printf(
        portLib, "Total blocks allocated = %u ( = most recent allocationNumber)\n", memStats.totalBlocksAllocated);
...
/**
 * This method prints whatever information as it can gather about a block from the block and its immediate neighbors (if
 *any). If the block looks like an uncorrupted freed block, TRUE is returned.  If detectable corruption of the block is
 *discovered, FALSE is returned.
 *
 * The function will Lock and Unlock the body of the wrappedBlocked, but the top and bottom page must be unlocked when
 *the block is passed in
 *
    ...
    OMRPortLibrary* portLib, char const* operationName, J9MemoryCheckHeader* blockHeader)
{
        ...
        if ((UDATA)topPadding % 8 == 4) {
            if (!memoryCheck_verify_forward(portLib, topPadding + 4, 8, J9_MEMCHECK_DATA_FREED_VALUE, wrappedBlock)) {
                paddingValue = J9_MEMCHECK_DATA_FREED_VALUE;
                           ...
                           portLib, topPadding + 4, 8, J9_MEMCHECK_CODE_FREED_VALUE, wrappedBlock)) {
                paddingValue = J9_MEMCHECK_CODE_FREED_VALUE;
            }
            portLib->tty_printf(portLib,
                "Previously freed block has unrecognized padding %08x %08x (header is probably trashed)!\n",
                ((U_32*)topPadding)[0], ((U_32*)topPadding)[1]);
            everythingOkay = FALSE;
            goto dump_header_only;
        } else {
    ...
    if (mode & J9_MCMODE_MPROTECT) {
        ...
        if (blockHeader->self != blockHeader->topPage) {
            topPaddingSmashed = memoryCheck_verify_forward(
                portLib, blockHeader->self, blockHeader->topPage - blockHeader->self, paddingValue, wrappedBlock);
            if (topPaddingSmashed) {
...
/**
 * This function scans the region of fillSize bytes at fillAddress, from lower to higher addresses.  It determines the
 *number of correct bytes of padding in the scan direction before the first incorrect byte.  If an incorrect byte is
 *found, all remaining bytes are considered incorrect, and the number of incorrect bytes (remaining+1) is returned.
...
memoryCheck_parseCmdLine(J9PortLibrary* j9portLibrary, UDATA lastLegalArg, char** argv)
{
    ...
    for (i = lastLegalArg; i >= 1; i--) {
        ...
        if (0 == strcmp("-Xcheck", argv[i])) {
            ...
            return i;
        } else if ((0 == strcmp("-Xcheck:none", argv[i])) || (0 == strcmp("-Xcheck:memory:none", argv[i]))
...
static UDATA memoryCheck_filter_nonVM_unFreed_Blcoks(OMRPortLibrary* portLib)
{
    ...
    while (NULL != strPtr) {
        ...
        if (strlen(strPtr) >= MAX_CALLSITE_LENGTH) {
            portLib->tty_printf(
                portLib, "ignoredCallSite %s length exceeds internal buffer size. Callsite discarded\n", strPtr);
            strPtr = strtok(NULL, CALLSITE_DELIMITER);
            continue;
        }
...
static void memoryCheck_print_summary(OMRPortLibrary* portLib, I_32 shutdownMode)
{
    ...
    if ((shutdownMode == J9_MEMCHECK_SHUTDOWN_NORMAL) && (mostRecentBlock)) {
        ...
        if (unfreedBlocks > J9_MEMCHECK_MAX_DUMP_LEAKED_BLOCKS) {
            ...
            portLib->tty_printf(
                portLib, "WARNING: only %d most recent leaked blocks will be described\n", unfreedBlocks);
        }
...
static void memoryCheck_dump_callSites(OMRPortLibrary* portLibrary, J9AVLTree* tree)
{
    ...
    portLibrary->tty_printf(
        portLibrary, " total alloc   | total freed   | delta alloc   | delta freed   | high water	| largest\n");
    portLibrary->tty_printf(portLibrary,
...
static void memoryCheck_print_stats_callSite(OMRPortLibrary* portLib, J9MEMAVLTreeNode* node)
{
    portLib->tty_printf(portLib, "%7u %7llu %7u %7llu %7u %7lld %7u %7lld %7u %7u %7u %7u %s\n",
        node->stats->totalBlocksAllocated, node->stats->totalBytesAllocated, node->stats->totalBlocksFreed,
        node->stats->totalBytesFreed, node->stats->totalBlocksAllocated - node->prevStats->totalBlocksAllocated,
        ...
        node->stats->totalBytesFreed - node->prevStats->totalBytesFreed, node->stats->hiWaterBlocksAllocated,
        node->stats->hiWaterBytesAllocated, node->stats->largestBlockAllocated, node->stats->largestBlockAllocNum,
        node->callSite);
}
    ...
    OMRPortLibrary* portLib, J9MemoryCheckHeader* header, const char* callSite, UDATA byteAmount)
{
    ...
    if (node) {
        ...
        node = old_mem_allocate_memory(portLib,
            sizeof(J9MEMAVLTreeNode) + strlen(callSite) + 1 + sizeof(J9MemoryCheckStats) + sizeof(J9MemoryCheckStats),
            J9_GET_CALLSITE(), OMRMEM_CATEGORY_VM);
        if (node) {
...
static void memoryCheck_print_stats_callSite_small(OMRPortLibrary* portLib, J9MEMAVLTreeNode* node)
{
    portLib->tty_printf(portLib, "%7u %7llu %7u %7llu %7u %7llu %7u %7llu %s\n", node->stats->totalBlocksAllocated,
        node->stats->totalBytesAllocated, node->stats->totalBlocksFreed, node->stats->totalBytesFreed,
        node->stats->totalBlocksAllocated - node->prevStats->totalBlocksAllocated,
        node->stats->totalBytesAllocated - node->prevStats->totalBytesAllocated,
        node->stats->totalBlocksFreed - node->prevStats->totalBlocksFreed,
        node->stats->totalBytesFreed - node->prevStats->totalBytesFreed, node->callSite);
}
...
#endif
    return memoryCheck_wrapper_reallocate_memory(memCheckPortLib, memoryPointer, byteAmount, "reallocate_memory",
        globalAllocator, globalDeallocator, J9_MEMCHECK_DATA_PADDING_VALUE, J9_MEMCHECK_DATA_FILL_VALUE,
        J9_MEMCHECK_DATA_FREED_VALUE, NULL == callsite ? "unknown" : callsite, category);
}
    ...
    U_32 fillValue, U_32 freedValue, const char* callSite, U_32 category)
{
    ...
    if (memoryPointer == NULL) {
        newBlock = memoryCheck_wrapper_allocate_memory(
            portLib, byteAmount, operationName, allocator, paddingValue, fillValue, freedValue, "unknown", category);
    } else if (byteAmount == 0) {
        memoryCheck_wrapper_free_memory(
            portLib, memoryPointer, operationName, deallocator, paddingValue, fillValue, freedValue);
        newBlock = NULL;
    } else {
        newBlock = memoryCheck_wrapper_allocate_memory(
            portLib, byteAmount, operationName, allocator, paddingValue, fillValue, freedValue, "unknown", category);
        ...
        if (newBlock != NULL) {
            ...
            if (0 == (mode & J9_MCMODE_NO_SCAN)) {
                if (mode & J9_MCMODE_MPROTECT) {
                    topHeader = (J9MemoryCheckHeader*)(wrappedBlock - J9_MEMCHECK_PAGE_SIZE);
                    if ((((UDATA)topHeader & 0XF000)
                            != ((UDATA)((U_8*)topHeader + sizeof(J9MemoryCheckHeader)) & 0XF000))) {
                        if ((UDATA)((U_8*)topHeader + sizeof(J9MemoryCheckHeader)) % J9_MEMCHECK_PAGE_SIZE != 0) {
                            topHeader = (J9MemoryCheckHeader*)((U_8*)topHeader - sizeof(J9MemoryCheckHeader));
                        }
                    }
 ...
 */
static void memoryCheck_null_mem_free_memory(OMRPortLibrary* portLib, void* memoryPointer) { return; }

#ifdef MEMDEBUG
static void subAllocator_audit(OMRPortLibrary* portLib, UDATA mask)
{
    UDATA top = STARTIDX;
    UDATA bottom;
    UDATA i;
    ...
    if (mask & BUCKET) {
        j9tty_printf(
            NULL, "BUCKET: [%d], %d, %d, %d, %d %d", bucket[0], bucket[1], bucket[2], bucket[3], bucket[4], bucket[5]);
    }
}
#endif
...
static UDATA subAllocator_findFirstFreeBlock(UDATA index, IDATA* j9heap)
{
...
static void* subAllocator_allocate_memory(
    OMRPortLibrary* portLib, UDATA byteAmount, const char* callSite, U_32 category)
{
