...
remoteConsole_parseCmdLine(J9PortLibrary* portLibrary, UDATA lastLegalArg, char** argv)
{
    ...
    for (i = 1; i <= lastLegalArg; i++) {
        if (argv[i][0] == '-') {
            if ((strncmp(&argv[i][1], "IO", 2) == 0) || (strncmp(&argv[i][1], "io", 2) == 0)) {
...
static IDATA remoteConsole_tosock_file_write(struct OMRPortLibrary* portLib, IDATA fd, const void* buf, IDATA nbytes)
{
...
static IDATA remoteConsole_tosock_get_chars(struct OMRPortLibrary* portLib, char* s, UDATA length)
{
    /*
...
static void remoteConsole_tosock_shutdown(struct OMRPortLibrary* portLib)
{
    PORT_ACCESS_FROM_PORT(_portLibrary);
    /* Close the socket. */
    j9sock_close(&remoteConsoleSocket);
...
static void remoteConsole_tofile_shutdown(struct OMRPortLibrary* portLib)
{
    PORT_ACCESS_FROM_PORT(_portLibrary);
...
static IDATA remoteConsole_tofile_file_write(struct OMRPortLibrary* portLib, IDATA fd, const void* buf, IDATA nbytes)
{
    if ((fd == J9PORT_TTY_OUT) || (fd == J9PORT_TTY_ERR)) {
...
static IDATA remoteConsole_tofile_startup(struct J9PortLibrary* portLib, const char* path)
{
    /*
    ...
    remoteConsoleFile = j9file_open(path, EsOpenCreateNew | EsOpenWrite, 0644);
    if (remoteConsoleFile == -1) {
        if (j9error_last_error_number() == J9PORT_ERROR_FILE_EXIST) {
            ...
            remoteConsoleFile = j9file_open(path, EsOpenCreate | EsOpenTruncate | EsOpenWrite, 0644);
        }
    }
