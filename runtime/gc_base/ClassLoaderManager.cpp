...
MM_ClassLoaderManager* MM_ClassLoaderManager::newInstance(MM_EnvironmentBase* env, MM_GlobalCollector* globalCollector)
{
    MM_ClassLoaderManager* classLoaderManager = (MM_ClassLoaderManager*)env->getForge()->allocate(
        sizeof(MM_ClassLoaderManager), MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
...
void MM_ClassLoaderManager::kill(MM_EnvironmentBase* env)
{
    tearDown(env);
    env->getForge()->free(this);
}

void MM_ClassLoaderManager::tearDown(MM_EnvironmentBase* env)
{
    ...
    if (MM_GCExtensions::getExtensions(env)->isMetronomeGC()) {
        ...
        if (NULL != vmHookInterface) {
            (*vmHookInterface)
                ->J9HookUnregister(vmHookInterface, J9HOOK_VM_CLASS_LOADER_INITIALIZED, classLoaderLoadHook, this);
        }
    }
#endif /* defined(J9VM_GC_REALTIME) */
}
 ...
 */
bool MM_ClassLoaderManager::initialize(MM_EnvironmentBase* env)
{
#if defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING)
    ...
    if (MM_GCExtensions::getExtensions(env)->isMetronomeGC()) {
        if ((*vmHookInterface)
                ->J9HookRegisterWithCallSite(vmHookInterface, J9HOOK_VM_CLASS_LOADER_INITIALIZED, classLoaderLoadHook,
                    OMR_GET_CALLSITE(), this)) {
            return false;
        }
    }
#endif /* defined(J9VM_GC_REALTIME) */
...
#if defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING)
void MM_ClassLoaderManager::enqueueUndeadClassSegments(J9MemorySegment* listRoot)
{
    if (NULL != listRoot) {
...
void MM_ClassLoaderManager::flushUndeadSegments(MM_EnvironmentBase* env)
{
    omrthread_monitor_enter(_undeadSegmentListMonitor);
...
void MM_ClassLoaderManager::setLastUnloadNumOfClassLoaders()
{
    _lastUnloadNumOfClassLoaders = (UDATA)pool_numElements(_javaVM->classLoaderBlocks);
}

void MM_ClassLoaderManager::setLastUnloadNumOfAnonymousClasses()
{
    _lastUnloadNumOfAnonymousClasses = _javaVM->anonClassCount;
}
#endif /* J9VM_GC_DYNAMIC_CLASS_UNLOADING */

void MM_ClassLoaderManager::unlinkClassLoader(J9ClassLoader* classLoader)
{
...
void MM_ClassLoaderManager::linkClassLoader(J9ClassLoader* classLoader)
{
    omrthread_monitor_enter(_classLoaderListMonitor);
...
bool MM_ClassLoaderManager::isTimeForClassUnloading(MM_EnvironmentBase* env)
{
    bool result = false;
    ...
    Trc_MM_GlobalCollector_isTimeForClassUnloading_Entry(_extensions->dynamicClassUnloading, numClassLoaderBlocks,
        _extensions->dynamicClassUnloadingThreshold, _lastUnloadNumOfClassLoaders);

    Trc_MM_GlobalCollector_isTimeForClassUnloading_anonClasses(
        numAnonymousClasses, _lastUnloadNumOfAnonymousClasses, _extensions->classUnloadingAnonymousClassWeight);
...
#if defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING)
J9ClassLoader* MM_ClassLoaderManager::identifyClassLoadersToUnload(
    MM_EnvironmentBase* env, MM_HeapMap* markMap, MM_ClassUnloadStats* classUnloadStats)
{
    ...
    while (NULL != (classLoader = classLoaderIterator.nextSlot())) {
        ...
        if (J9_GC_CLASS_LOADER_DEAD == (classLoader->gcFlags & J9_GC_CLASS_LOADER_DEAD)) {
            ...
            Assert_MM_true(
                0 != (classLoader->gcFlags & (J9_GC_CLASS_LOADER_UNLOADING | J9_GC_CLASS_LOADER_ENQ_UNLOAD)));
            ...
            if ((NULL != classLoaderObject) && (!markMap->isBitSet(classLoaderObject))) {
                ...
                if (MM_GCExtensions::getExtensions(env)->isVLHGC()) {
                    ...
                    if (0 == (classLoader->flags & J9CLASSLOADER_ANON_CLASS_LOADER)) {
                        Assert_MM_true(
                            J9_GC_CLASS_LOADER_SCANNED == (classLoader->gcFlags & J9_GC_CLASS_LOADER_SCANNED));
                    }
...
void MM_ClassLoaderManager::cleanUpClassLoadersStart(MM_EnvironmentBase* env, J9ClassLoader* classLoaderUnloadList,
    MM_HeapMap* markMap, MM_ClassUnloadStats* classUnloadStats)
{
    ...
    anonymousClassUnloadList = addDyingClassesToList(
        env, _javaVM->anonClassLoader, markMap, false, anonymousClassUnloadList, &anonymousClassUnloadCount);
    ...
    if (0 != anonymousClassUnloadCount) {
        ...
        Trc_MM_cleanUpClassLoadersStart_triggerAnonymousClassesUnload(
            env->getLanguageVMThread(), anonymousClassUnloadCount);
...
J9Class* MM_ClassLoaderManager::addDyingClassesToList(MM_EnvironmentBase* env, J9ClassLoader* classLoader,
    MM_HeapMap* markMap, bool setAll, J9Class* classUnloadListStart, UDATA* classUnloadCountResult)
{
    J9VMThread* vmThread = (J9VMThread*)env->getLanguageVMThread();
...
void MM_ClassLoaderManager::cleanUpClassLoadersEnd(MM_EnvironmentBase* env, J9ClassLoader* unloadLink)
{
    J9VMThread* vmThread = (J9VMThread*)env->getLanguageVMThread();
    ...
    /* Unload classes in the unload link and pass back any RAM Classes that we encounter so the caller can decide when
     * to free them */
...
void MM_ClassLoaderManager::cleanUpSegmentsAlongClassLoaderLink(
    J9JavaVM* javaVM, J9MemorySegment* segment, J9MemorySegment** reclaimedSegments)
{
    while (NULL != segment) {
        ...
        if (segment->type & MEMORY_TYPE_RAM_CLASS) {
            ...
            /* we also need to unset the fact that this is a RAM CLASS since some code which walks the segment list is
             * looking for still-valid ones */
...
void MM_ClassLoaderManager::cleanUpSegmentsInAnonymousClassLoader(
    MM_EnvironmentBase* env, J9MemorySegment** reclaimedSegments)
{
        ...
        while (NULL != segment) {
            ...
            if (MEMORY_TYPE_RAM_CLASS == (segment->type & MEMORY_TYPE_RAM_CLASS)) {
                ...
                if (J9_JAVA_CLASS_DYING == (J9CLASS_FLAGS(clazz) & J9_JAVA_CLASS_DYING)) {
                    ...
                    if (!_extensions->objectModel.isIndexable(clazz)) {
                        ...
                        if (NULL != romClass) {
                            ...
                            while (NULL != segmentROM) {
                                ...
                                if ((MEMORY_TYPE_ROM_CLASS == (segmentROM->type & MEMORY_TYPE_ROM_CLASS))
                    ...
                    segment->type |= MEMORY_TYPE_UNDEAD_CLASS;
                    /* we also need to unset the fact that this is a RAM CLASS since some code which walks the segment
                     * list is looking for still-valid ones */
                    segment->type &= ~MEMORY_TYPE_RAM_CLASS;
                    segment->nextSegmentInClassLoader = *reclaimedSegments;
                    *reclaimedSegments = segment;
                    segment->classLoader = NULL;
...
void MM_ClassLoaderManager::removeFromSubclassHierarchy(MM_EnvironmentBase* env, J9Class* clazzPtr)
{
...
void MM_ClassLoaderManager::cleanUpClassLoaders(MM_EnvironmentBase* env, J9ClassLoader* classLoadersUnloadedList,
    J9MemorySegment** reclaimedSegments, J9ClassLoader** unloadLink, volatile bool* finalizationRequired)
{
    ...
    *unloadLink = NULL;
    ...
    while (NULL != classLoadersUnloadedList) {
        ...
        Assert_MM_true(
            0 == (classLoader->gcFlags & J9_GC_CLASS_LOADER_SCANNED)); /* we don't use the SCANNED flag in VLHGC */
...
bool MM_ClassLoaderManager::tryEnterClassUnloadMutex(MM_EnvironmentBase* env)
{
    bool result = true;
...
U_64 MM_ClassLoaderManager::enterClassUnloadMutex(MM_EnvironmentBase* env)
{
...
void MM_ClassLoaderManager::exitClassUnloadMutex(MM_EnvironmentBase* env)
{
