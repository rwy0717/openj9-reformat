 ...
 */
void GC_FinalizeListManager::lock() const { omrthread_monitor_enter(_mutex); }

/**
 * unlock the finalize list manager
 */
void GC_FinalizeListManager::unlock() const { omrthread_monitor_exit(_mutex); }
 ...
 */
GC_FinalizeListManager* GC_FinalizeListManager::newInstance(MM_EnvironmentBase* env)
{
    GC_FinalizeListManager* manager;

    manager = (GC_FinalizeListManager*)env->getForge()->allocate(
        sizeof(GC_FinalizeListManager), MM_AllocationCategory::FINALIZE, J9_GET_CALLSITE());
    if (manager) {
 ...
 */
void GC_FinalizeListManager::kill(MM_EnvironmentBase* env)
{
    tearDown();
 ...
 */
bool GC_FinalizeListManager::initialize()
{
 ...
 */
void GC_FinalizeListManager::tearDown()
{
    if (NULL != _mutex) {
...
void GC_FinalizeListManager::addSystemFinalizableObjects(j9object_t head, j9object_t tail, UDATA objectCount)
{
    lock();
...
j9object_t GC_FinalizeListManager::popSystemFinalizableObject()
{
...
void GC_FinalizeListManager::addDefaultFinalizableObjects(j9object_t head, j9object_t tail, UDATA objectCount)
{
    lock();
...
j9object_t GC_FinalizeListManager::popDefaultFinalizableObject()
{
...
void GC_FinalizeListManager::addReferenceObjects(j9object_t head, j9object_t tail, UDATA objectCount)
{
    lock();
...
j9object_t GC_FinalizeListManager::popReferenceObject()
{
...
void GC_FinalizeListManager::addClassLoaders(J9ClassLoader* head, J9ClassLoader* tail, UDATA count)
{
    lock();
...
J9ClassLoader* GC_FinalizeListManager::popClassLoader()
{
...
J9ClassLoader* GC_FinalizeListManager::popRequiredClassLoaderForForcedClassLoaderUnload()
{
...
GC_FinalizeJob* GC_FinalizeListManager::consumeJob(J9VMThread* vmThread, GC_FinalizeJob* job)
{
