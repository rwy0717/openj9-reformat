...
private:
    ...
    j9object_t _systemFinalizableObjects; /**< head of the linked list of objects allocated by the system classloader
                                             that need to be finalized */
    UDATA _systemFinalizableObjectCount; /** count of the system finalizable object  */
    j9object_t _defaultFinalizableObjects; /**< head of the linked list of objects allocated by non system classloaders
                                              that need to be finalized */
    ...
    J9ClassLoader*
        _classLoaders; /**< head of the linked list of unloaded classloaders which have open native libraries  */
...
public:
    ...
    {
        lock();
        UDATA count = _classLoaderCount + _defaultFinalizableObjectCount + _systemFinalizableObjectCount
            + _referenceObjectCount;
        unlock();
        return count;
    }
     ...
     * @return the current head of the list, or NULL if the list is empty
     */
    virtual j9object_t peekSystemFinalizableObject() { return _systemFinalizableObjects; }
    /**
     ...
     * @return the current head of the list, or NULL if the list is empty
     */
    virtual j9object_t peekDefaultFinalizableObject() { return _defaultFinalizableObjects; }
    /**
     ...
     * @return the current head of the list, or NULL if the list is empty
     */
    MMINLINE j9object_t peekReferenceObject() { return _referenceObjects; }
    /**
     ...
     * @return the current head of the list, or NULL if the list is empty
     */
    MMINLINE J9ClassLoader* peekClassLoader() { return _classLoaders; }
    /**
     ...
     */
    MMINLINE J9ClassLoader* peekNextClassLoader(J9ClassLoader* current) { return current->unloadLink; }
     ...
     * Friends
     */
    friend class GC_CheckFinalizableList; /* Temporary until DDR gccheck is updated to iterate multi-tenant finalizable
                                             queues */
};
