...
void* finalizeForcedClassLoaderUnload(J9VMThread* vmThread)
{
 ...
 */
void finalizeForcedUnfinalizedToFinalizable(J9VMThread* vmThread)
{
#if defined(J9VM_GC_FINALIZATION)
...
IDATA FinalizeMasterRunFinalization(J9JavaVM* vm, omrthread_t* indirectSlaveThreadHandle,
    struct finalizeSlaveData** indirectSlaveData, IDATA finalizeCycleLimit, IDATA mode);
...
static int J9THREAD_PROC FinalizeMasterThread(void* javaVM)
{
    ...
    if (NULL != vm->javaOffloadSwitchOnNoEnvWithReasonFunc) {
        (*vm->javaOffloadSwitchOnNoEnvWithReasonFunc)(
            vm, vm->finalizeMasterThread, J9_JNI_OFFLOAD_SWITCH_GC_FINALIZE_MASTER_THREAD);
    }
#endif
    ...
    do {
                ...
                if (currentWaitTime == -2) {
                    ...
                    do {
                        cycleIntervalWaitResult
                            = omrthread_monitor_wait_timed(vm->finalizeMasterMonitor, currentWaitTime, 0);
                    } while (!(vm->finalizeMasterFlags & J9_FINALIZE_FLAGS_MASTER_WORK_REQUEST)
                        && cycleIntervalWaitResult != J9THREAD_TIMED_OUT);
                }
            }
        }
        ...
        /* Check for a wake up request, which means the garbage collector has placed objects on the finalizable queue
         * for processing */
        ...
        /* If RUN_FINALIZATION is set, make the interval time is set to -1 -> This will override any "wake up" request
         * made by the garbage collector */
        if (vm->finalizeMasterFlags
    ...
    if (vm->finalizeMasterFlags & J9_FINALIZE_FLAGS_RUN_FINALIZERS_ON_EXIT) {
        ...
        while (!doneRunFinalizersOnExit) {
            ...
            do {
                ...
                result = FinalizeMasterRunFinalization(
                    vm, &slaveThreadHandle, &slaveData, finalizeCycleLimit, FINALIZE_SLAVE_MODE_FORCED);
            } while (result == -2);
    ...
    if (NULL != vm->javaOffloadSwitchOffNoEnvWithReasonFunc) {
        (*vm->javaOffloadSwitchOffNoEnvWithReasonFunc)(
            vm, vm->finalizeMasterThread, J9_JNI_OFFLOAD_SWITCH_GC_FINALIZE_MASTER_THREAD);
    }
#endif
...
static void process_finalizable(
    J9VMThread* vmThread, j9object_t object, jclass j9VMInternalsClass, jmethodID runFinalizeMID)
{
...
static void process_reference(J9VMThread* vmThread, j9object_t reference, jmethodID refMID)
{
    J9InternalVMFunctions* fns;
    J9JavaVM* vm;
...
static void process_classloader(J9VMThread* vmThread, J9ClassLoader* classLoader)
{
    J9InternalVMFunctions* fns;
    J9JavaVM* vm;
...
static void process(J9VMThread* vmThread, const GC_FinalizeJob* finalizeJob, jclass j9VMInternalsClass,
    jmethodID runFinalizeMID, jmethodID referenceEnqueueImplMID)
{
...
static int J9THREAD_PROC FinalizeSlaveThread(void* arg)
{
    ...
    if (vm->jclFlags & J9_JCL_FLAG_FINALIZATION) {
        ...
        if (j9VMInternalsClass) {
            ...
            if (j9VMInternalsClass) {
                runFinalizeMID
                    = ((JNIEnv*)env)->GetStaticMethodID(j9VMInternalsClass, "runFinalize", "(Ljava/lang/Object;)V");
            }
        }
        if (!runFinalizeMID) {
    ...
    do {
        ...
        do {
            ...
            if (slaveData->mode == FINALIZE_SLAVE_MODE_CL_UNLOAD) {
                ...
                if (NULL
                    == (localJob.classLoader = (J9ClassLoader*)finalizeForcedClassLoaderUnload((J9VMThread*)env))) {
                    break;
                } else {
    ...
    if (vm->javaOffloadSwitchOffNoEnvWithReasonFunc != NULL) {
        (*vm->javaOffloadSwitchOffNoEnvWithReasonFunc)(
            vm, omrthread_self(), J9_JNI_OFFLOAD_SWITCH_FINALIZE_SLAVE_THREAD);
    }
#endif
    ...
    struct finalizeSlaveData** indirectSlaveData, IDATA finalizeCycleLimit, IDATA mode)
{
    ...
    if (NULL == slaveThreadHandle) {
        ...
        slaveData = (struct finalizeSlaveData*)forge->allocate(
            sizeof(struct finalizeSlaveData), MM_AllocationCategory::FINALIZE, J9_GET_CALLSITE());
        if (NULL == slaveData) {
        ...
        IDATA result = vm->internalVMFunctions->createThreadWithCategory(&slaveThreadHandle, vm->defaultOSStackSize,
            MM_GCExtensions::getExtensions(vm)->finalizeSlavePriority, 0, &gpProtectedFinalizeSlaveThread, slaveData,
    ...
    do {
        ...
        (slaveWaitResult == J9THREAD_TIMED_OUT && publicFlags & J9_PUBLIC_FLAGS_HALT_VM_DUTIES && !slaveData->finished)
        || (slaveWaitResult != J9THREAD_TIMED_OUT && !slaveData->finished));
    omrthread_monitor_exit(slaveData->monitor);
...
static UDATA FinalizeSlaveThreadGlue(J9PortLibrary* portLib, void* userData) { return FinalizeSlaveThread(userData); }
...
static int J9THREAD_PROC gpProtectedFinalizeSlaveThread(void* entryArg)
{
    ...
    j9sig_protect(FinalizeSlaveThreadGlue, slaveData, slaveData->vm->internalVMFunctions->structuredSignalHandlerVM,
        slaveData->vm, J9PORT_SIG_FLAG_SIGALLSYNC | J9PORT_SIG_FLAG_MAY_CONTINUE_EXECUTION, &rc);

    return 0;
}

void j9gc_finalizer_completeFinalizersOnExit(J9VMThread* vmThread)
{
...
int j9gc_finalizer_startup(J9JavaVM* vm)
{
    ...
    result = vm->internalVMFunctions->createThreadWithCategory(NULL, vm->defaultOSStackSize,
        MM_GCExtensions::getExtensions(vm)->finalizeMasterPriority, 0, &FinalizeMasterThread, vm,
 ...
 */
void runFinalization(J9VMThread* vmThread)
{
        ...
        if (NULL != vmThread->gcClassUnloadingMutex) {
            ...
            if (J9THREAD_TIMED_OUT == waitResult) {
                ...
                if ((NULL != vmThread->gcClassUnloadingThreadPrevious)
                    || (NULL != vmThread->gcClassUnloadingThreadNext)) {
                    if (NULL == vmThread->gcClassUnloadingThreadPrevious) {
                        classLoader->gcThreadNotification = vmThread->gcClassUnloadingThreadNext;
                    } else {
                        vmThread->gcClassUnloadingThreadPrevious->gcClassUnloadingThreadNext
                            = vmThread->gcClassUnloadingThreadNext;
                    }
                    if (NULL != vmThread->gcClassUnloadingThreadNext) {
                        vmThread->gcClassUnloadingThreadNext->gcClassUnloadingThreadPrevious
                            = vmThread->gcClassUnloadingThreadPrevious;
                    }
 ...
 */
void j9gc_runFinalizersOnExit(J9VMThread* vmThread, UDATA run)
{
    J9JavaVM* jvm = vmThread->javaVM;
