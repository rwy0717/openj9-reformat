...
protected:
    ...
    UDATA _compressedPointersShift; /**< the number of bits to shift by when converting between the compressed pointers
                                       heap and real heap */
    ...
    UDATA _ownableSynchronizerLinkOffset; /** Offset within java/util/concurrent/locks/AbstractOwnableSynchronizer of
                                             the ownable synchronizer link field */
public:
    /* member function */
private:
    /**
...
protected:
     ...
     */
    MMINLINE void* indexableEffectiveAddress(
        J9VMThread* vmThread, J9IndexableObject* array, I_32 index, UDATA elementSize)
    {
    ...
    virtual mm_j9object_t readObjectImpl(
        J9VMThread* vmThread, mm_j9object_t srcObject, fj9object_t* srcAddress, bool isVolatile = false);
    virtual mm_j9object_t staticReadObjectImpl(
    ...
    virtual void* readAddressImpl(
        J9VMThread* vmThread, mm_j9object_t srcObject, void** srcAddress, bool isVolatile = false);
    ...
    virtual void storeObjectImpl(J9VMThread* vmThread, mm_j9object_t destObject, fj9object_t* destAddress,
        mm_j9object_t value, bool isVolatile = false);
    virtual void staticStoreObjectImpl(
        J9VMThread* vmThread, J9Class* clazz, j9object_t* destAddress, mm_j9object_t value, bool isVolatile = false);
    virtual void storeObjectToInternalVMSlotImpl(
        J9VMThread* vmThread, j9object_t* destAddress, mm_j9object_t value, bool isVolatile = false);

    virtual void storeAddressImpl(
        J9VMThread* vmThread, mm_j9object_t destObject, void** destAddress, void* value, bool isVolatile = false);
    virtual void storeU8Impl(
        J9VMThread* vmThread, mm_j9object_t destObject, U_8* destAddress, U_8 value, bool isVolatile = false);
    virtual void storeI8Impl(
        J9VMThread* vmThread, mm_j9object_t destObject, I_8* destAddress, I_8 value, bool isVolatile = false);
    virtual void storeU16Impl(
        J9VMThread* vmThread, mm_j9object_t destObject, U_16* destAddress, U_16 value, bool isVolatile = false);
    virtual void storeI16Impl(
        J9VMThread* vmThread, mm_j9object_t destObject, I_16* destAddress, I_16 value, bool isVolatile = false);
    virtual void storeU32Impl(
        J9VMThread* vmThread, mm_j9object_t destObject, U_32* destAddress, U_32 value, bool isVolatile = false);
    virtual void storeI32Impl(
        J9VMThread* vmThread, mm_j9object_t destObject, I_32* destAddress, I_32 value, bool isVolatile = false);
    virtual void storeU64Impl(
        J9VMThread* vmThread, mm_j9object_t destObject, U_64* destAddress, U_64 value, bool isVolatile = false);
    virtual void storeI64Impl(
        J9VMThread* vmThread, mm_j9object_t destObject, I_64* destAddress, I_64 value, bool isVolatile = false);
...
public:
    ...
    virtual J9Object* mixedObjectReadObject(
        J9VMThread* vmThread, J9Object* srcObject, UDATA srcOffset, bool isVolatile = false);
    virtual void* mixedObjectReadAddress(
        J9VMThread* vmThread, J9Object* srcObject, UDATA srcOffset, bool isVolatile = false);
    virtual U_32 mixedObjectReadU32(
        J9VMThread* vmThread, J9Object* srcObject, UDATA srcOffset, bool isVolatile = false);
    virtual I_32 mixedObjectReadI32(
        J9VMThread* vmThread, J9Object* srcObject, UDATA srcOffset, bool isVolatile = false);
    virtual U_64 mixedObjectReadU64(
        J9VMThread* vmThread, J9Object* srcObject, UDATA srcOffset, bool isVolatile = false);
    virtual I_64 mixedObjectReadI64(
        J9VMThread* vmThread, J9Object* srcObject, UDATA srcOffset, bool isVolatile = false);

    virtual void mixedObjectStoreObject(
        J9VMThread* vmThread, J9Object* destObject, UDATA destOffset, J9Object* value, bool isVolatile = false);
    virtual void mixedObjectStoreAddress(
        J9VMThread* vmThread, J9Object* destObject, UDATA destOffset, void* value, bool isVolatile = false);
    virtual void mixedObjectStoreU32(
        J9VMThread* vmThread, J9Object* destObject, UDATA destOffset, U_32 value, bool isVolatile = false);
    virtual void mixedObjectStoreI32(
        J9VMThread* vmThread, J9Object* destObject, UDATA destOffset, I_32 value, bool isVolatile = false);
    virtual void mixedObjectStoreU64(
        J9VMThread* vmThread, J9Object* destObject, UDATA destOffset, U_64 value, bool isVolatile = false);
    virtual void mixedObjectStoreI64(
        J9VMThread* vmThread, J9Object* destObject, UDATA destOffset, I_64 value, bool isVolatile = false);

    virtual J9Object* indexableReadObject(
        J9VMThread* vmThread, J9IndexableObject* srcObject, I_32 srcIndex, bool isVolatile = false);
    virtual void* indexableReadAddress(
        J9VMThread* vmThread, J9IndexableObject* srcObject, I_32 srcIndex, bool isVolatile = false);
    virtual U_8 indexableReadU8(
        J9VMThread* vmThread, J9IndexableObject* srcObject, I_32 srcIndex, bool isVolatile = false);
    virtual I_8 indexableReadI8(
        J9VMThread* vmThread, J9IndexableObject* srcObject, I_32 srcIndex, bool isVolatile = false);
    virtual U_16 indexableReadU16(
        J9VMThread* vmThread, J9IndexableObject* srcObject, I_32 srcIndex, bool isVolatile = false);
    virtual I_16 indexableReadI16(
        J9VMThread* vmThread, J9IndexableObject* srcObject, I_32 srcIndex, bool isVolatile = false);
    virtual U_32 indexableReadU32(
        J9VMThread* vmThread, J9IndexableObject* srcObject, I_32 srcIndex, bool isVolatile = false);
    virtual I_32 indexableReadI32(
        J9VMThread* vmThread, J9IndexableObject* srcObject, I_32 srcIndex, bool isVolatile = false);
    virtual U_64 indexableReadU64(
        J9VMThread* vmThread, J9IndexableObject* srcObject, I_32 srcIndex, bool isVolatile = false);
    virtual I_64 indexableReadI64(
        J9VMThread* vmThread, J9IndexableObject* srcObject, I_32 srcIndex, bool isVolatile = false);

    virtual void indexableStoreObject(
        J9VMThread* vmThread, J9IndexableObject* destObject, I_32 destIndex, J9Object* value, bool isVolatile = false);
    virtual void indexableStoreAddress(
        J9VMThread* vmThread, J9IndexableObject* destObject, I_32 destIndex, void* value, bool isVolatile = false);
    virtual void indexableStoreU8(
        J9VMThread* vmThread, J9IndexableObject* destObject, I_32 destIndex, U_8 value, bool isVolatile = false);
    virtual void indexableStoreI8(
        J9VMThread* vmThread, J9IndexableObject* destObject, I_32 destIndex, I_8 value, bool isVolatile = false);
    virtual void indexableStoreU16(
        J9VMThread* vmThread, J9IndexableObject* destObject, I_32 destIndex, U_16 value, bool isVolatile = false);
    virtual void indexableStoreI16(
        J9VMThread* vmThread, J9IndexableObject* destObject, I_32 destIndex, I_16 value, bool isVolatile = false);
    virtual void indexableStoreU32(
        J9VMThread* vmThread, J9IndexableObject* destObject, I_32 destIndex, U_32 value, bool isVolatile = false);
    virtual void indexableStoreI32(
        J9VMThread* vmThread, J9IndexableObject* destObject, I_32 destIndex, I_32 value, bool isVolatile = false);
    virtual void indexableStoreU64(
        J9VMThread* vmThread, J9IndexableObject* destObject, I_32 destIndex, U_64 value, bool isVolatile = false);
    virtual void indexableStoreI64(
        J9VMThread* vmThread, J9IndexableObject* destObject, I_32 destIndex, I_64 value, bool isVolatile = false);

#if defined(J9VM_GC_ARRAYLETS)
    enum { ARRAY_COPY_SUCCESSFUL = -1, ARRAY_COPY_NOT_DONE = -2 };
    virtual I_32 doCopyContiguousForward(J9VMThread* vmThread, J9IndexableObject* srcObject,
        J9IndexableObject* destObject, I_32 srcIndex, I_32 destIndex, I_32 lengthInSlots);
    virtual I_32 doCopyContiguousBackward(J9VMThread* vmThread, J9IndexableObject* srcObject,
        J9IndexableObject* destObject, I_32 srcIndex, I_32 destIndex, I_32 lengthInSlots);
    virtual I_32 backwardReferenceArrayCopyIndex(J9VMThread* vmThread, J9IndexableObject* srcObject,
        J9IndexableObject* destObject, I_32 srcIndex, I_32 destIndex, I_32 lengthInSlots)
    {
        return -2;
    }
    virtual I_32 forwardReferenceArrayCopyIndex(J9VMThread* vmThread, J9IndexableObject* srcObject,
        J9IndexableObject* destObject, I_32 srcIndex, I_32 destIndex, I_32 lengthInSlots)
    {
        return -2;
    }
    ...
    virtual J9Object* staticReadObject(
        J9VMThread* vmThread, J9Class* clazz, J9Object** srcSlot, bool isVolatile = false);
    ...
    virtual void staticStoreObject(
        J9VMThread* vmThread, J9Class* clazz, J9Object** destSlot, J9Object* value, bool isVolatile = false);
    virtual void staticStoreAddress(
        J9VMThread* vmThread, J9Class* clazz, void** destSlot, void* value, bool isVolatile = false);
    virtual void staticStoreU32(
        J9VMThread* vmThread, J9Class* clazz, U_32* destSlot, U_32 value, bool isVolatile = false);
    virtual void staticStoreI32(
        J9VMThread* vmThread, J9Class* clazz, I_32* destSlot, I_32 value, bool isVolatile = false);
    virtual void staticStoreU64(
        J9VMThread* vmThread, J9Class* clazz, U_64* destSlot, U_64 value, bool isVolatile = false);
    virtual void staticStoreI64(
        J9VMThread* vmThread, J9Class* clazz, I_64* destSlot, I_64 value, bool isVolatile = false);
    ...
    virtual void copyObjectFields(J9VMThread* vmThread, J9Class* valueClass, J9Object* srcObject, UDATA srcOffset,
        J9Object* destObject, UDATA destOffset);
    virtual void cloneIndexableObject(
    ...
    virtual bool compareAndSwapObject(J9VMThread* vmThread, J9Object* destObject, fj9object_t* destAddress,
        J9Object* compareObject, J9Object* swapObject);
    virtual bool staticCompareAndSwapObject(J9VMThread* vmThread, J9Class* destClass, j9object_t* destAddress,
        J9Object* compareObject, J9Object* swapObject);
    virtual bool mixedObjectCompareAndSwapInt(
        J9VMThread* vmThread, J9Object* destObject, UDATA offset, U_32 compareValue, U_32 swapValue);
    virtual bool staticCompareAndSwapInt(
        J9VMThread* vmThread, J9Class* destClass, U_32* destAddress, U_32 compareValue, U_32 swapValue);
    virtual bool mixedObjectCompareAndSwapLong(
        J9VMThread* vmThread, J9Object* destObject, UDATA offset, U_64 compareValue, U_64 swapValue);
    virtual bool staticCompareAndSwapLong(
        J9VMThread* vmThread, J9Class* destClass, U_64* destAddress, U_64 compareValue, U_64 swapValue);
    virtual J9Object* compareAndExchangeObject(J9VMThread* vmThread, J9Object* destObject, fj9object_t* destAddress,
        J9Object* compareObject, J9Object* swapObject);
    virtual J9Object* staticCompareAndExchangeObject(J9VMThread* vmThread, J9Class* destClass, j9object_t* destAddress,
        J9Object* compareObject, J9Object* swapObject);
    virtual U_32 mixedObjectCompareAndExchangeInt(
        J9VMThread* vmThread, J9Object* destObject, UDATA offset, U_32 compareValue, U_32 swapValue);
    virtual U_32 staticCompareAndExchangeInt(
        J9VMThread* vmThread, J9Class* destClass, U_32* destAddress, U_32 compareValue, U_32 swapValue);
    virtual U_64 mixedObjectCompareAndExchangeLong(
        J9VMThread* vmThread, J9Object* destObject, UDATA offset, U_64 compareValue, U_64 swapValue);
    virtual U_64 staticCompareAndExchangeLong(
        J9VMThread* vmThread, J9Class* destClass, U_64* destAddress, U_64 compareValue, U_64 swapValue);

    virtual void deleteHeapReference(MM_EnvironmentBase* env, J9Object* object) {}

    virtual bool preObjectStore(
        J9VMThread* vmThread, J9Object* destObject, fj9object_t* destAddress, J9Object* value, bool isVolatile = false);
    virtual bool preObjectStore(
        J9VMThread* vmThread, J9Object* destClass, J9Object** destAddress, J9Object* value, bool isVolatile = false);
    virtual bool preObjectStore(J9VMThread* vmThread, J9Object** destAddress, J9Object* value, bool isVolatile = false);
    virtual void postObjectStore(
        J9VMThread* vmThread, J9Object* destObject, fj9object_t* destAddress, J9Object* value, bool isVolatile = false);
    virtual void postObjectStore(
        J9VMThread* vmThread, J9Class* destClass, J9Object** destAddress, J9Object* value, bool isVolatile = false);
    virtual void postObjectStore(
        J9VMThread* vmThread, J9Object** destAddress, J9Object* value, bool isVolatile = false);
     ...
     */
    MMINLINE mm_j9object_t convertPointerFromToken(fj9object_t token)
    {
#if defined(J9VM_GC_COMPRESSED_POINTERS)
     ...
     * @note this function is not virtual because it must be callable from out-of-process
     */
    MMINLINE fj9object_t convertTokenFromPointer(mm_j9object_t pointer)
    {
#if defined(J9VM_GC_COMPRESSED_POINTERS)
     ...
     *
     */
    MMINLINE UDATA compressedPointersShift()
    {
#if defined(J9VM_GC_COMPRESSED_POINTERS)
    ...
    virtual void fillArrayOfObjects(
        J9VMThread* vmThread, J9IndexableObject* destObject, I_32 destIndex, I_32 count, J9Object* value);
    ...
    /**
     * Fetch the ownableSynchronizerLink link field of the specified reference object.(for Compact or forwardedObject
     * case)
     ...
     * @return 				a pointer to the array data; this may be a malloc'd copy, or a pointer into the
     * heap
     */
     ...
     * @param isCopy		NULL, or a pointer to a jboolean that will set to JNI_TRUE if a copy is made
     * @return 				a pointer to the character data; this may be a malloc'd copy, or a pointer into the
     * heap
     */
     ...
     */
    virtual bool checkClassLive(J9JavaVM* javaVM, J9Class* classPtr) { return true; }
#endif /* defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING) */

    MM_ObjectAccessBarrier(MM_EnvironmentBase* env)
        : MM_BaseVirtual()
        , _extensions(NULL)
        , _heap(NULL)
#if defined(J9VM_GC_COMPRESSED_POINTERS)
