...
J9Object** MM_PacketSlotIterator::nextSlot()
{
    /* we can't assume that _nextSlot points inside the packet or contains a value which we want to return so cue it up
     * to the next valid slot */
    bool found = false;
    while ((!found) && (_nextSlot < (J9Object**)_packet->_currentPtr)) {
        if ((NULL != *_nextSlot) && (PACKET_ARRAY_SPLIT_TAG != (PACKET_ARRAY_SPLIT_TAG & (UDATA)*_nextSlot))) {
            ...
            /* this slot was not acceptable so advance to the next - we will fall out of the loop if this advances us
             * past the end */
            _nextSlot += 1;
        }
    }
...
void MM_PacketSlotIterator::resetSplitTagIndexForObject(J9Object* correspondingObject, UDATA newValue)
{
