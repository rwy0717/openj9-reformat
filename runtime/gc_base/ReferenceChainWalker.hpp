...
private:
    ...
    J9MODRON_REFERENCE_CHAIN_WALKER_CALLBACK*
        _userCallback; /**< User callback function to be called on every reachable slot */
    ...
    bool _shouldPreindexInterfaceFields; /**< if true, indexes interface fields of the class being visited before class
                                            and superclass fields, otherwise, returns them in the order they appear in
                                            an object instance (CMVC 142897) */
    ...
    MMINLINE virtual CompletePhaseCode scanWeakReferencesComplete(MM_EnvironmentBase* env) { return complete_phase_OK; }
    MMINLINE virtual CompletePhaseCode scanSoftReferencesComplete(MM_EnvironmentBase* env) { return complete_phase_OK; }
    ...
    virtual CompletePhaseCode scanMonitorReferencesComplete(MM_EnvironmentBase* env) { return complete_phase_OK; }
...
protected:
public:
    MM_ReferenceChainWalker(MM_EnvironmentBase* env, UDATA queueSlots,
        J9MODRON_REFERENCE_CHAIN_WALKER_CALLBACK* userCallback, void* userData)
        ...
        , /* default to behaviour required for Java6/heap11 */
        _markMap(NULL)
        , _heap(NULL)
        , _heapBase(NULL)
        , _heapTop(NULL)
     ...
     * Detail:  heap10 requires no pre-indexing in order to preserve Java5 behaviour but heap11 requires pre-indexing to
     * pass a Java6 JCK
     */
    void setPreindexInterfaceFields(bool shouldPreindexInterfaceFields)
    {
        _shouldPreindexInterfaceFields = shouldPreindexInterfaceFields;
    }
};
