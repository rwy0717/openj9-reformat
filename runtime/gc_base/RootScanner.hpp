...
protected:
    ...
    bool _nurseryReferencesOnly; /**< Should the iterator only scan structures that currently contain nursery references
                                  */
    bool
        _nurseryReferencesPossibly; /**< Should the iterator only scan structures that may contain nursery references */
    bool _includeStackFrameClassReferences; /**< Should the iterator include Classes which have a method running on the
                                               stack */
#if defined(J9VM_GC_MODRON_SCAVENGER)
    bool _includeRememberedSetReferences; /**< Should the iterator include references in the Remembered Set (if
                                             applicable) */
#endif /* J9VM_GC_MODRON_SCAVENGER */
    bool _classDataAsRoots; /**< Should all classes (and class loaders) be treated as roots. Default true, should set to
                               false when class unloading */
    bool _includeJVMTIObjectTagTables; /**< Should the iterator include the JVMTIObjectTagTables. Default true, should
                                          set to false when doing JVMTI object walks */
    bool _trackVisibleStackFrameDepth; /**< Should the stack walker be told to track the visible frame depth. Default
                                          false, should set to true when doing JVMTI walks that report stack slots */

    U_64 _entityStartScanTime; /**< The start time of the scan of the current scanning entity, or 0 if no entity is
                                  being scanned.  Defaults to 0. */
    U_64 _entityIncrementStartTime; /**< Start time of current increment with a scan entity (Metronome may have several
                                       increment for each entity) */
    U_64 _entityIncrementEndTime; /**< End time of the current increment */
    RootScannerEntity _scanningEntity; /**< The root scanner entity that is currently being scanned. Defaults to
                                          RootScannerEntity_None. */
    RootScannerEntity
        _lastScannedEntity; /**< The root scanner entity that was last scanned. Defaults to RootScannerEntity_None. */
...
private:
     ...
     */
    void scanArrayObject(MM_EnvironmentBase* env, J9Object* objectPtr, MM_MemoryPool* memoryPool,
        MM_HeapRegionManager* manager, UDATA memoryType);
...
protected:
     ...
     */
    MMINLINE void reportScanningStarted(RootScannerEntity scanningEntity)
    {
     ...
     */
    MMINLINE void reportScanningEnded(RootScannerEntity scannedEntity)
    {
...
public:
     ...
     */
    MMINLINE void reportScanningSuspended()
    {
        if (_extensions->rootScannerStatsEnabled) {
            OMRPORT_ACCESS_FROM_OMRVM(_omrVM);
     ...
     */
    MMINLINE void reportScanningResumed()
    {
        if (_extensions->rootScannerStatsEnabled) {
            OMRPORT_ACCESS_FROM_OMRVM(_omrVM);
    ...
    void setStringTableAsRoot(bool stringTableAsRoot) { _stringTableAsRoot = stringTableAsRoot; }

#if defined(J9VM_GC_MODRON_SCAVENGER)
    /** Set whether the iterator will only scan structures which contain nursery references */
    void setNurseryReferencesOnly(bool nurseryReferencesOnly) { _nurseryReferencesOnly = nurseryReferencesOnly; }
    ...
    void setClassDataAsRoots(bool classDataAsRoots) { _classDataAsRoots = classDataAsRoots; }
    ...
    /** General object slot handler to be reimplemented by specializing class. This handler is called for every
     * reference to a J9Object. */
    virtual void doSlot(J9Object** slotPtr) = 0;

    /** General class slot handler to be reimplemented by specializing class. This handler is called for every reference
     * to a J9Class. */
    ...
    virtual void doJNIGlobalReferenceSlot(
