...
/* the grow tag is used by VLHGC and can only be set if the FORWARDED_TAG is already set.  It signifies that the object
 * grew a hash field when moving */
...
public:
    /**
     * Called at the start of a copy-forward (minimally, must be called at least once after trace is enabled but before
     * we use this class) to validate sizing and object geometry assumptions.
     */
    ...
    /**
     * Similar to setForwardedObject but optionally sets the "GROW_TAG" on the forwarding pointer so that we know the
     * object grew a hashcode slot when it moved.
     ...
     * @return The location of the new copy of the object (will be destinationObjectPtr if the calling thread succeeded
     * in the copy)
     */
     ...
     */
    MMINLINE bool isForwardedPointer() { return FORWARDED_TAG == ((UDATA)_preserved.clazz & FORWARDED_TAG); }
     ...
     */
    MMINLINE bool hasBeenMoved()
    {
        return (
            OBJECT_HEADER_HAS_BEEN_MOVED_IN_CLASS == ((UDATA)_preserved.clazz & OBJECT_HEADER_HAS_BEEN_MOVED_IN_CLASS));
    }
     ...
     */
    MMINLINE bool hasBeenHashed()
    {
        return (0
            != ((UDATA)_preserved.clazz
     ...
     * @return true if object has been hashed
     */
    MMINLINE bool isHasBeenHashedBitSet()
    {
        return (OBJECT_HEADER_HAS_BEEN_HASHED_IN_CLASS
            == ((UDATA)_preserved.clazz & OBJECT_HEADER_HAS_BEEN_HASHED_IN_CLASS));
    }
     ...
     */
    MMINLINE omrobjectptr_t getForwardedObject()
    {
     ...
     */
    MMINLINE J9Class* getPreservedClass()
    {
     ...
     * @return the flags of the object
     */
    MMINLINE UDATA getPreservedFlags() { return (UDATA)_preserved.clazz & J9GC_J9OBJECT_CLAZZ_FLAGS_MASK; }

    /**
     * Return back object's Age
     */
    MMINLINE UDATA getPreservedAge() { return (UDATA)_preserved.clazz & OBJECT_HEADER_AGE_MASK; }
    /**
     ...
     */
    MMINLINE void fixupCopiedObject(omrobjectptr_t copiedObjectPtr)
    {
     ...
     */
    MMINLINE void fixupCopiedObject(omrobjectptr_t copiedObjectPtr, UDATA newAge, bool setOldFlag)
    {
        volatile MutableHeaderFields* newHeader = (volatile MutableHeaderFields*)copiedObjectPtr;
    ...
    MMINLINE U_32 getPreservedIndexableSize()
    {
#if defined(J9VM_INTERP_COMPRESSED_OBJECT_HEADER)
        /* in compressed headers, the size of the object is stored in the other half of the UDATA read when we read
         * clazz so read it from there instead of the heap (since the heap copy would have been over-written by the
    ...
    MMINLINE UDATA getHashcodeOffset(J9JavaVM* javaVM, J9Object* dest)
    {
...
protected:
     ...
     */
    MMINLINE omrobjectptr_t getForwardedObjectNoCheck()
    {
