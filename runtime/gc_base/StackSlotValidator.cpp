...
void MM_StackSlotValidator::threadCrash(MM_EnvironmentBase* env)
{
    reportStackSlot(env, "Unhandled exception while validating object in stack frame");
}

void MM_StackSlotValidator::reportStackSlot(MM_EnvironmentBase* env, const char* message)
{
    ...
    if (NULL != method) {
        ...
        j9tty_printf(PORTLIB, "%p:\tmethod=%p (%.*s.%.*s%.*s) (%s)\n", walkThread, _walkState->method,
            ...
            (U_32)J9UTF8_LENGTH(sig), J9UTF8_DATA(sig), type);
        Trc_MM_StackSlotValidator_method(env->getLanguageVMThread(), _walkState->method, (U_32)J9UTF8_LENGTH(className),
            J9UTF8_DATA(className), (U_32)J9UTF8_LENGTH(name), J9UTF8_DATA(name), (U_32)J9UTF8_LENGTH(sig),
            J9UTF8_DATA(sig), type);
    }

    j9tty_printf(PORTLIB, "%p:\tstack=%p-%p\n", walkThread, walkThread->stackObject + 1, walkThread->stackObject->end);
    Trc_MM_StackSlotValidator_stack(
        env->getLanguageVMThread(), walkThread->stackObject + 1, walkThread->stackObject->end);
