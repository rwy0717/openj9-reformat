...
public:
    enum {
        COULD_BE_FORWARDED = 1, /**< indicates that a scavenge or copy-forward operation is in progress and that the
                                   value in _slotPtr could point to a forwarded object */
...
public:
    ...
    bool validate(MM_EnvironmentBase* env)
    {
        ...
        if (J9_INVALID_OBJECT == _slotValue) {
            ...
            reportStackSlot(env, "Misaligned stack-allocated object"); /* stack allocated objects might only be pointer
                                                                          aligned, not necessarily 8-aligned */
            result = false;
