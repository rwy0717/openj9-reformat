...
static IDATA stringComparatorFn(
...
MM_StringTable* MM_StringTable::newInstance(MM_EnvironmentBase* env, UDATA tableCount)
{
    MM_StringTable* stringTable = (MM_StringTable*)env->getForge()->allocate(
        sizeof(MM_StringTable), MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
    if (stringTable) {
...
bool MM_StringTable::initialize(MM_EnvironmentBase* env)
{
    ...
    for (UDATA tableIndex = 0; tableIndex < _tableCount; tableIndex++) {
        _table[tableIndex] = collisionResilientHashTableNew(OMRPORT_FROM_J9PORT(javaVM->portLibrary), J9_GET_CALLSITE(),
            initialSize, sizeof(j9object_t), 0, OMRMEM_CATEGORY_MM, listToTreeThreshold, stringHashFn,
            stringComparatorFn, NULL, javaVM);
...
void MM_StringTable::tearDown(MM_EnvironmentBase* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
...
void MM_StringTable::kill(MM_EnvironmentBase* env)
{
    tearDown(env);
 ...
 */
j9object_t MM_StringTable::hashAt(UDATA tableIndex, j9object_t string)
{
    j9object_t* result;
...
j9object_t MM_StringTable::hashAtUTF8(UDATA tableIndex, U_8* utf8Data, UDATA utf8Length, U_32 hash)
{
    ...
    ptr = &query;
    ptr = (void*)((UDATA)ptr
        | TYPE_UTF8); /* Least significant bit indicates that this is a pointer to a stringTableUTF8Query */
    return hashAt(tableIndex, (j9object_t)ptr);
}

j9object_t MM_StringTable::hashAtPut(UDATA tableIndex, j9object_t string)
{
...
j9object_t MM_StringTable::addStringToInternTable(J9VMThread* vmThread, j9object_t string)
{
...
static IDATA stringComparatorFn(struct J9AVLTree* tree, struct J9AVLTreeNode* leftNode, struct J9AVLTreeNode* rightNode)
{
    ...
    if (stu8Ptr & TYPE_UTF8) {
        ...
        if (!isMetronome) {
            /* Check if string was copy-forwarded.  Only do this on non-metronome since metronome re-uses the FORWARDED
             * bit */
 ...
 * @param rightKey pointer to a string. If the least significant bit is zero, it is a pointer to a pointer to
 * stringTableUTF8Query struct, otherwise it is a String object.
...
U_32 computeJavaHashForExpandedString(J9JavaVM* javaVM, j9object_t s)
{
    U_32 hash = 0;
    I_32 i;
 ...
 * @param key pointer to a low-tagged pointer either to a struct containing a UTF string pointer and length (bit 0 set)
 * or a String object (bit 0 cleared)
...
UDATA
j9gc_stringHashFn(void* key, void* userData) { return stringHashFn(key, userData); }

j9object_t j9gc_createJavaLangString(J9VMThread* vmThread, U_8* data, UDATA length, UDATA stringFlags)
{
    ...
    if (NULL == result) {
        ...
        stringClass = vm->internalVMFunctions->internalFindKnownClass(
            vmThread, J9VMCONSTANTPOOL_JAVALANGSTRING, J9_FINDKNOWNCLASS_FLAG_EXISTING_ONLY);
        if (stringClass == NULL) {
            goto nomem;
        }
        ...
        if (J9_ARE_ANY_BITS_SET(vm->runtimeFlags, J9_RUNTIME_STRING_BYTE_ARRAY)) {
            if (isCompressable) {
                ...
                charArray
                    = J9AllocateIndexableObject(vmThread, vm->byteArrayClass, (U_32)unicodeLength * 2, allocateFlags);
            }
        } else {
            if (isCompressable) {
                charArray = J9AllocateIndexableObject(
                    vmThread, vm->charArrayClass, (U_32)(unicodeLength + 1) / 2, allocateFlags);
            } else {
        ...
        if (IS_STRING_COMPRESSION_ENABLED_VM(vm)) {
            if (isCompressable) {
                ...
                if (J9VMJAVALANGSTRING_COMPRESSIONFLAG(vmThread, stringClass) == 0) {
                    /*
                     * jitHookClassPreinitialize will process initialization events for String compression
                     * sideEffectGuards so we must initialize the class if this is the first time we are loading it
                     */
                    J9Class* flagClass = vm->internalVMFunctions->internalFindKnownClass(vmThread,
                        J9VMCONSTANTPOOL_JAVALANGSTRINGSTRINGCOMPRESSIONFLAG, J9_FINDKNOWNCLASS_FLAG_INITIALIZE);
 ...
 */
j9object_t setupCharArray(J9VMThread* vmThread, j9object_t sourceString, j9object_t newString)
{
    J9JavaVM* vm = vmThread->javaVM;
    ...
    if (J9_ARE_ANY_BITS_SET(vm->runtimeFlags, J9_RUNTIME_STRING_BYTE_ARRAY)) {
        if (isCompressed) {
            newChars = J9AllocateIndexableObject(vmThread, vm->byteArrayClass, (U_32)length,
                J9_GC_ALLOCATE_OBJECT_TENURED | J9_GC_ALLOCATE_OBJECT_NON_INSTRUMENTABLE);
        } else {
            newChars = J9AllocateIndexableObject(vmThread, vm->byteArrayClass, (U_32)length * 2,
                J9_GC_ALLOCATE_OBJECT_TENURED | J9_GC_ALLOCATE_OBJECT_NON_INSTRUMENTABLE);
        }
    } else {
        if (isCompressed) {
            newChars = J9AllocateIndexableObject(vmThread, vm->charArrayClass, (U_32)(length + 1) / 2,
                J9_GC_ALLOCATE_OBJECT_TENURED | J9_GC_ALLOCATE_OBJECT_NON_INSTRUMENTABLE);
        } else {
            newChars = J9AllocateIndexableObject(vmThread, vm->charArrayClass, (U_32)length,
                J9_GC_ALLOCATE_OBJECT_TENURED | J9_GC_ALLOCATE_OBJECT_NON_INSTRUMENTABLE);
        }
    }
...
j9object_t j9gc_internString(J9VMThread* vmThread, j9object_t sourceString)
{
    ...
    if (NULL == internedString) {
        ...
        PUSH_OBJECT_IN_SPECIAL_FRAME(vmThread, sourceString);
        newString = J9AllocateObject(vmThread, J9OBJECT_CLAZZ(vmThread, sourceString),
            J9_GC_ALLOCATE_OBJECT_TENURED | J9_GC_ALLOCATE_OBJECT_NON_INSTRUMENTABLE);
        sourceString = POP_OBJECT_IN_SPECIAL_FRAME(vmThread);
        if (NULL != newString) {
...
j9object_t j9gc_allocStringWithSharedCharData(J9VMThread* vmThread, U_8* data, UDATA length, UDATA resolveFlags)
{
    ...
    if (isCompressable) {
        if (J9_ARE_ANY_BITS_SET(vm->runtimeFlags, J9_RUNTIME_STRING_BYTE_ARRAY)) {
            charArray = (J9IndexableObject*)J9AllocateIndexableObject(
                vmThread, vm->byteArrayClass, (U_32)unicodeLength, allocateFlags);
        } else {
            charArray = (J9IndexableObject*)J9AllocateIndexableObject(
                vmThread, vm->charArrayClass, (U_32)(unicodeLength + 1) / 2, allocateFlags);
        }
        ...
        vm->internalVMFunctions->copyUTF8ToCompressedUnicode(
            vmThread, data, length, J9_STR_INTERN, (j9object_t)charArray, 0);
    } else {
        if (J9_ARE_ANY_BITS_SET(vm->runtimeFlags, J9_RUNTIME_STRING_BYTE_ARRAY)) {
            charArray = (J9IndexableObject*)J9AllocateIndexableObject(
                vmThread, vm->byteArrayClass, (U_32)unicodeLength * 2, allocateFlags);
        } else {
            charArray = (J9IndexableObject*)J9AllocateIndexableObject(
                vmThread, vm->charArrayClass, (U_32)unicodeLength, allocateFlags);
        }
    ...
    if (IS_STRING_COMPRESSION_ENABLED_VM(vm)) {
        if (isCompressable) {
            ...
            if (IS_STRING_COMPRESSION_ENABLED_VM(vm)
                && J9VMJAVALANGSTRING_COMPRESSIONFLAG(vmThread, stringClass) == 0) {
                /**
                 ...
                 */
                J9Class* flagClass = vm->internalVMFunctions->internalFindKnownClass(
                    vmThread, J9VMCONSTANTPOOL_JAVALANGSTRINGSTRINGCOMPRESSIONFLAG, J9_FINDKNOWNCLASS_FLAG_INITIALIZE);
