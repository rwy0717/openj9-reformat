...
MM_PacketList* MM_PacketListIterator::nextPacketList(MM_EnvironmentBase* env)
{
...
MM_PacketList::PacketSublist* MM_PacketSublistIterator::nextSublist(MM_EnvironmentBase* env)
{
    MM_PacketList::PacketSublist* sublist = NULL;
    if (NULL != _listToWalk) {
        /* this if is only to protect us against the degenerate case where we were initialized with a NULL list - it is
         * otherwise invariant in the following loop */
...
MM_Packet* MM_PacketIterator::nextPacket(MM_EnvironmentBase* env)
{
...
MM_Packet* MM_WorkPacketsIterator::nextPacket(MM_EnvironmentBase* env)
{
    ...
    if (NULL == packet) {
        ...
        if (NULL == nextSublist) {
            ...
            if (NULL != nextList) {
                ...
                /* the packet list iterator will never return an empty list so there must be at least one sublist in
                 * this list */
        ...
        if (NULL != nextSublist) {
            ...
            /* the packet sublist iterator will never return an empty sublist and this is a new sublist (either from our
             * existing iterator or the new one we just created) so it must be non-empty */
            Assert_MM_true(NULL != nextListBase);
            _packetIterator = MM_PacketIterator(env, nextListBase);
            packet = _packetIterator.nextPacket(env);
            /* because of our knowledge of how the packet iterator works, we know that its first returned packet must be
             * the packet used to initialize it */
