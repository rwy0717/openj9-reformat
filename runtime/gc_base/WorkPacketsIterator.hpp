...
private:
    ...
    MM_PacketList*
        _packetLists[_numPacketLists + 1]; /**< Cached pointers to the MM_PacketList instances in the MM_WorkPackets
                                              instance with which the receiver was initialized */
...
public:
     ...
     * @param env[in] A GC thread
     * @paran workPackets[in] The MM_WorkPackets instance which the receiver will iterate in order to find MM_PacketList
     * instances
     */
...
private:
    ...
    UDATA _nextSublistIndex; /**< The next sublist index from which to extract a sublist from _listToWalk when we are
                                next requested */
protected:
public:
    /* Member Functions */
...
public:
    ...
    /**
     * @param env[in] A GC thread
     * @param packetList[in] The packet to iterate for its sublists (passing NULL is permitted and creates an empty
     * iterator)
     */
...
class MM_WorkPacketsIterator : public MM_BaseNonVirtual {
    /* Data Members */
private:
    MM_PacketListIterator _packetListIterator; /**< The iterator instance used to walk the packet lists inside the
                                                  MM_WorkPackets instance with which the receiver was initialized */
    MM_PacketSublistIterator _packetSublistIterator; /**< The iterator instance used to walk the packet sublists inside
                                                        the packet list instance currently being iterated */
    MM_PacketIterator _packetIterator; /**< The iterator instance used to walk chains of packets once the receiver has
                                          found the root of the next packet list */
protected:
public:
    /* Member Functions */
private:
