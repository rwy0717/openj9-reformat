 ...
 * 	<li>J9MMCONSTANT_IMPLICIT_GC_PERCOLATE_UNLOADING_CLASSES - collect due to scavanger percolate (unloading
 * classes)</li> <li>J9MMCONSTANT_IMPLICIT_GC_PERCOLATE_AGGRESSIVE - collect due to aggressive scavanger percolate</li>
 ...
 *  <li>J9MMCONSTANT_EXPLICIT_GC_NATIVE_OUT_OF_MEMORY - a native out of memory has occurred -- attempt to recover as
 * much native memory as possible</li> <li>J9MMCONSTANT_EXPLICIT_GC_RASDUMP_COMPACT - a dump agent has requested
 * compaction (usually before producing a heapdump). Note that the dump agent must have already acquired exclusive
 * access before triggering the GC.</li>
 * </ul>
 ...
 * possible memorypool ID is defined in omrgcconsts.h, one bit for each possible memorypool, the bit is set if the
 * memory pool is supported by currrent jvm. gc also can decide which memory pools should expose to outside via updating
 * this method.
 */
UDATA
...
/**
 * return integer presents all garbage collector IDs(one bit for each possible garbage collector) supported by current
 * jvm.
 */
UDATA
...
j9gc_pool_maxmemory(J9JavaVM* javaVM, UDATA poolID)
{
    ...
    case J9_GC_MANAGEMENT_POOL_TENURED:
        ...
        break;
    case J9_GC_MANAGEMENT_POOL_NURSERY_ALLOCATE: {
        double tiltRatio = ((double)(extensions->heap->getActiveMemorySize(MEMORY_TYPE_NEW)
                               - extensions->heap->getActiveSurvivorMemorySize(MEMORY_TYPE_NEW)))
            / (double)extensions->heap->getActiveMemorySize(MEMORY_TYPE_NEW);
 ...
 */
UDATA
j9gc_get_softmx(J9JavaVM* javaVM) { return MM_GCExtensions::getExtensions(javaVM)->softMx; }
 ...
 */
UDATA
j9gc_get_initial_heap_size(J9JavaVM* javaVM) { return MM_GCExtensions::getExtensions(javaVM)->initialMemorySize; }
 ...
 */
const char* j9gc_get_gcmodestring(J9JavaVM* javaVM) { return MM_GCExtensions::getExtensions(javaVM)->gcModeString; }
 ...
 */
void j9gc_allocation_threshold_changed(J9VMThread* currentThread)
{
...
void j9gc_set_allocation_threshold(J9VMThread* vmThread, UDATA low, UDATA high)
{
    ...
    Trc_MM_AllocationThreshold_setAllocationThreshold_Entry(
        vmThread, low, high, ext->lowAllocationThreshold, ext->highAllocationThreshold);
...
{
    return MM_EnvironmentBase::getEnvironment(vmThread->omrVMThread)
        ->_objectAllocationInterface->getAllocationStats()
        ->bytesAllocated();
}
...
void j9gc_notifyGCOfClassReplacement(J9VMThread* vmThread, J9Class* oldClass, J9Class* newClass, UDATA isFastHCR)
{
    ...
    if (NULL != oldClass->gcLink) {
        ...
        /*
         * gcLink in Balanced would refer to Remembered Set information (low tagged value or pointer to malloc'ed bit
         * array) Class replacement can not introduce new roots so Remembered Set is good enough In case of Fast HCR old
         * class still be an actual class after replacement so RS pointer is in correct place already Otherwise RS
         * pointer should be moved from old class to new class. An old class RS pointer can be set to NULL. There are no
...
UDATA
j9gc_concurrent_getCardSize(J9JavaVM* javaVM) { return j9gc_incrementalUpdate_getCardSize(javaVM->omrVM); }

UDATA
j9gc_concurrent_getHeapBase(J9JavaVM* javaVM) { return j9gc_incrementalUpdate_getHeapBase(javaVM->omrVM); }

uintptr_t j9gc_incrementalUpdate_getCardTableVirtualStart(OMR_VM* omrVM)
{
    uintptr_t start = 0;
...
uintptr_t j9gc_incrementalUpdate_getCardSize(OMR_VM* omrVM)
{
    MM_GCExtensionsBase* extensions = MM_GCExtensionsBase::getExtensions(omrVM);
...
uintptr_t j9gc_incrementalUpdate_getCardTableShiftValue(OMR_VM* omrVM)
{
    MM_GCExtensionsBase* extensions = MM_GCExtensionsBase::getExtensions(omrVM);
 ...
 */
uintptr_t j9gc_incrementalUpdate_getHeapBase(OMR_VM* omrVM)
{
    MM_GCExtensionsBase* extensions = MM_GCExtensionsBase::getExtensions(omrVM);
