...
const GC_CheckCycle::funcStruct GC_CheckCycle::funcArray[] = {
    /* move ownablesynchronizer check before objectheap check in order to ownablesynchronizer check runs after
     * objectheap check(the checks in stack, last in first out) for ownablesynchronizer consistency check, which defends
     * on both ownablesynchronizer check before objectheap check. the verify consistency only at the end of
     * ownablesynchronizer check.
     */
    ...
    { "jniweakglobalrefs", J9MODRON_GCCHK_SCAN_JNI_WEAK_GLOBAL_REFERENCES,
        GC_CheckJNIWeakGlobalReferences::newInstance },
 ...
 */
void GC_CheckCycle::printHelp(J9PortLibrary* portLib)
{
 ...
 * going to run.
 */
void GC_CheckCycle::generateCheckList(UDATA scanFlags)
{
 ...
 */
bool GC_CheckCycle::initialize(const char* options)
{
...
void GC_CheckCycle::run(GCCheckInvokedBy invokedBy, UDATA filterFlags)
{
 ...
 *
 */
void GC_CheckCycle::fixDeadObjects(GCCheckInvokedBy invokedBy)
{
    _invokedBy = invokedBy;
...
GC_CheckCycle* GC_CheckCycle::newInstance(
    J9JavaVM* javaVM, GC_CheckEngine* engine, const char* args, UDATA manualCountInvocation)
{
    MM_Forge* forge = MM_GCExtensions::getExtensions(javaVM)->getForge();

    GC_CheckCycle* checkCycle
        = (GC_CheckCycle*)forge->allocate(sizeof(GC_CheckCycle), MM_AllocationCategory::DIAGNOSTIC, J9_GET_CALLSITE());
    if (checkCycle) {
