...
GC_Check* GC_CheckFinalizableList::newInstance(J9JavaVM* javaVM, GC_CheckEngine* engine)
{
    MM_Forge* forge = MM_GCExtensions::getExtensions(javaVM)->getForge();

    GC_CheckFinalizableList* check = (GC_CheckFinalizableList*)forge->allocate(
        sizeof(GC_CheckFinalizableList), MM_AllocationCategory::DIAGNOSTIC, J9_GET_CALLSITE());
    if (check) {
...
void GC_CheckFinalizableList::kill()
{
    MM_Forge* forge = MM_GCExtensions::getExtensions(_javaVM)->getForge();
    forge->free(this);
}

void GC_CheckFinalizableList::check()
{
    GC_FinalizeListManager* finalizeListManager = _extensions->finalizeListManager;
    {
        ...
        while (NULL != systemObject) {
            if (_engine->checkSlotFinalizableList(_javaVM, &systemObject, finalizeListManager)
...

    {
        ...
        while (NULL != defaultObject) {
            if (_engine->checkSlotFinalizableList(_javaVM, &defaultObject, finalizeListManager)
                != J9MODRON_SLOT_ITERATOR_OK) {
                return;
            }
...

    {
        ...
        while (NULL != referenceObject) {
            if (_engine->checkSlotFinalizableList(_javaVM, &referenceObject, finalizeListManager)
                != J9MODRON_SLOT_ITERATOR_OK) {
                return;
            }
...
void GC_CheckFinalizableList::print()
{
    GC_FinalizeListManager* finalizeListManager = _extensions->finalizeListManager;
...
j9object_t GC_CheckFinalizableList::peekSystemFinalizableObject(GC_FinalizeListManager* finalizeListManager)
{
    return finalizeListManager->_systemFinalizableObjects;
}

j9object_t GC_CheckFinalizableList::peekNextSystemFinalizableObject(
    GC_FinalizeListManager* finalizeListManager, j9object_t current)
{
...
j9object_t GC_CheckFinalizableList::peekDefaultFinalizableObject(GC_FinalizeListManager* finalizeListManager)
{
    return finalizeListManager->_defaultFinalizableObjects;
}

j9object_t GC_CheckFinalizableList::peekNextDefaultFinalizableObject(
    GC_FinalizeListManager* finalizeListManager, j9object_t current)
{
    MM_GCExtensions* extensions = finalizeListManager->_extensions;
    MM_ObjectAccessBarrier* barrier = extensions->accessBarrier;
