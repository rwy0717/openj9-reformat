...
GC_Check* GC_CheckOwnableSynchronizerList::newInstance(J9JavaVM* javaVM, GC_CheckEngine* engine)
{
    MM_Forge* forge = MM_GCExtensions::getExtensions(javaVM)->getForge();

    GC_CheckOwnableSynchronizerList* check = (GC_CheckOwnableSynchronizerList*)forge->allocate(
        sizeof(GC_CheckOwnableSynchronizerList), MM_AllocationCategory::DIAGNOSTIC, J9_GET_CALLSITE());
    if (NULL != check) {
...
void GC_CheckOwnableSynchronizerList::kill()
{
    MM_Forge* forge = MM_GCExtensions::getExtensions(_javaVM)->getForge();
    forge->free(this);
}

void GC_CheckOwnableSynchronizerList::check()
{
    ...
    UDATA maximumOwnableSynchronizerCountOnHeap
        = heapRegionManager->getTotalHeapSizeInBytes() / J9_GC_MINIMUM_OBJECT_SIZE;
    ...
    while (NULL != ownableSynchronizerObjectList) {
        ...
        while (NULL != objectPtr) {
            if (J9MODRON_SLOT_ITERATOR_OK
                != _engine->checkSlotOwnableSynchronizerList(_javaVM, &objectPtr, ownableSynchronizerObjectList)) {
                return;
            }
            ...
            if (ownableSynchronizerCount > maximumOwnableSynchronizerCountOnHeap) {
                ...
                j9tty_printf(PORTLIB,
                    "  <gc check: found that circular reference in the OwnableSynchronizerList=%p, maximum "
                    "OwnableSynchronizerCount =%zu >\n",
     ...
     * verifyOwnableSynchronizerObjectCounts need both the count calculated in CheckObjectHeap and in
     * CheckOwnableSynchronizerList assumption: CheckOwnableSynchronizerList always happens after CheckObjectHeap in
     * CheckCycle
     */
    _engine->verifyOwnableSynchronizerObjectCounts();
}

void GC_CheckOwnableSynchronizerList::print()
{
    MM_ObjectAccessBarrier* barrier = _extensions->accessBarrier;
    MM_OwnableSynchronizerObjectList* ownableSynchronizerObjectList = _extensions->ownableSynchronizerObjectLists;
