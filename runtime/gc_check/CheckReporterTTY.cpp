...
const static char* errorTypes[] = {
    ...
    "class ramStatics must be NULL for hot swapped class", /* J9MODRON_GCCHK_RC_CLASS_HOT_SWAPPED_POINTS_TO_STATICS (31)
                                                            */
    "class ramStatics field points to object but out of GC scan range", /* J9MODRON_GCCHK_RC_CLASS_STATICS_REFERENCE_IS_NOT_IN_SCANNING_RANGE
                                                                           (32) */
    "class ramStatics number of references not equal specified in ROM class", /* J9MODRON_GCCHK_RC_CLASS_STATICS_WRONG_NUMBER_OF_REFERENCES
                                                                                 (33) */
    "class object not a subclass of java.util.concurrent.locks.AbstractOwnableSynchronizer", /* J9MODRON_GCCHK_RC_OWNABLE_SYNCHRONIZER_INVALID_CLASS
                                                                                                (38) */
    ...
    "Ownable Synchronizer Object is not attached to the list", /* J9MODRON_GCCHK_OWNABLE_SYNCHRONIZER_OBJECT_IS_NOT_ATTACHED_TO_THE_LIST
                                                                  (42) */
    "Ownable Synchronizer List has a circular reference", /* J9MODRON_GCCHK_OWNABLE_SYNCHRONIZER_LIST_HAS_CIRCULAR_REFERENCE
                                                             (43) */
 ...
 */
GC_CheckReporterTTY* GC_CheckReporterTTY::newInstance(J9JavaVM* javaVM)
{
    MM_Forge* forge = MM_GCExtensions::getExtensions(javaVM)->getForge();

    GC_CheckReporterTTY* reporter = (GC_CheckReporterTTY*)forge->allocate(
        sizeof(GC_CheckReporterTTY), MM_AllocationCategory::DIAGNOSTIC, J9_GET_CALLSITE());
    if (reporter) {
 ...
 */
void GC_CheckReporterTTY::report(GC_CheckError* error)
{
    ...
    if (error->_slot) {
        ...
        if (invocation_manual == error->_cycle->getInvoker()) {
            j9tty_printf(PORTLIB, "  <gc check (%zu): %s (%zu): %s: %sslot %p(%p) -> %p: %s>\n", error->_errorNumber,
                invokedByStrings[error->_cycle->getInvoker()], error->_cycle->getManualCheckNumber(),
                error->_check->getCheckName(), error->_elementName, error->_object, slot, slotValue,
                errorTypes[error->_errorCode]);
        } else {
            j9tty_printf(PORTLIB, "  <gc check (%zu): %s: %s: %sslot %p(%p) -> %p: %s>\n", error->_errorNumber,
                invokedByStrings[error->_cycle->getInvoker()], error->_check->getCheckName(), error->_elementName,
                error->_object, slot, slotValue, errorTypes[error->_errorCode]);
        }
    } else {
        if (invocation_manual == error->_cycle->getInvoker()) {
            j9tty_printf(PORTLIB, "  <gc check (%zu): %s (%zu): %s: %s%p: %s>\n", error->_errorNumber,
                invokedByStrings[error->_cycle->getInvoker()], error->_cycle->getManualCheckNumber(),
                error->_check->getCheckName(), error->_elementName, error->_object, errorTypes[error->_errorCode]);
        } else {
            j9tty_printf(PORTLIB, "  <gc check (%zu): %s: %s: %s%p: %s>\n", error->_errorNumber,
                invokedByStrings[error->_cycle->getInvoker()], error->_check->getCheckName(), error->_elementName,
                error->_object, errorTypes[error->_errorCode]);
        }
 ...
 */
void GC_CheckReporterTTY::reportObjectHeader(GC_CheckError* error, J9Object* objectPtr, const char* prefix)
{
    ...
    if (extensions->objectModel.isIndexable(objectPtr)) {
        ...
        j9tty_printf(
            PORTLIB, "  <gc check (%zu): %s%s %p header:", error->_errorNumber, prefixString, elementName, objectPtr);
    }
 ...
 * if no prefix is desired
 */
void GC_CheckReporterTTY::reportClass(GC_CheckError* error, J9Class* clazz, const char* prefix)
{
    const char* prefixString = prefix ? prefix : "";
    PORT_ACCESS_FROM_PORT(_portLibrary);
    ...
    j9tty_printf(PORTLIB, "  <gc check (%zu): %sClass %p>\n", error->_errorNumber, prefixString, clazz);
}

/**
 * Report the fact that a fatal error has occurred.
 */
void GC_CheckReporterTTY::reportFatalError(GC_CheckError* error)
{
    PORT_ACCESS_FROM_PORT(_portLibrary);

    j9tty_printf(
        PORTLIB, "  <gc check (%zu): Cannot resolve problem detected on heap, aborting check>\n", error->_errorNumber);
}

/**
 * Print to the terminal that an error has occurred while walking the heap.
 */
void GC_CheckReporterTTY::reportHeapWalkError(GC_CheckError* error, GC_CheckElement previousObjectPtr1,
