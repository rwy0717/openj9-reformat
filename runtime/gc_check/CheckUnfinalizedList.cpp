...
GC_Check* GC_CheckUnfinalizedList::newInstance(J9JavaVM* javaVM, GC_CheckEngine* engine)
{
    MM_Forge* forge = MM_GCExtensions::getExtensions(javaVM)->getForge();

    GC_CheckUnfinalizedList* check = (GC_CheckUnfinalizedList*)forge->allocate(
        sizeof(GC_CheckUnfinalizedList), MM_AllocationCategory::DIAGNOSTIC, J9_GET_CALLSITE());
    if (check) {
...
void GC_CheckUnfinalizedList::kill()
{
    MM_Forge* forge = MM_GCExtensions::getExtensions(_javaVM)->getForge();
    forge->free(this);
}

void GC_CheckUnfinalizedList::check()
{
    ...
    while (NULL != unfinalizedObjectList) {
        ...
        while (NULL != objectPtr) {
            if (_engine->checkSlotUnfinalizedList(_javaVM, &objectPtr, unfinalizedObjectList)
...
void GC_CheckUnfinalizedList::print()
{
    MM_ObjectAccessBarrier* barrier = _extensions->accessBarrier;
    MM_UnfinalizedObjectList* unfinalizedObjectList = _extensions->unfinalizedObjectLists;
