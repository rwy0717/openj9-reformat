 ...
 */
void checkStackSlotIterator(J9JavaVM* javaVM, J9Object** objectIndirect, void* localData, J9StackWalkState* walkState,
    const void* stackLocation)
{
    GC_CheckVMThreadStacks::checkStackIteratorData* checkStackIteratorData
        = (GC_CheckVMThreadStacks::checkStackIteratorData*)localData;

    if (J9MODRON_SLOT_ITERATOR_RECOVERABLE_ERROR
        == checkStackIteratorData->gcCheck->checkSlotStack(checkStackIteratorData->gcCheck->getJavaVM(), objectIndirect,
               checkStackIteratorData->walkThread, stackLocation)) {
 ...
 * @see GC_VMThreadStackSlotIterator::scanSlots
 */
void printStackSlotIterator(J9JavaVM* javaVM, J9Object** objectIndirect, void* localData, J9StackWalkState* walkState,
    const void* stackLocation)
{
    GC_CheckVMThreadStacks::printStackIteratorData* printStackIteratorData
        = (GC_CheckVMThreadStacks::printStackIteratorData*)localData;

    printStackIteratorData->scanFormatter->entry((void*)*objectIndirect);
}

GC_Check* GC_CheckVMThreadStacks::newInstance(J9JavaVM* javaVM, GC_CheckEngine* engine)
{
    MM_Forge* forge = MM_GCExtensions::getExtensions(javaVM)->getForge();

    GC_CheckVMThreadStacks* check = (GC_CheckVMThreadStacks*)forge->allocate(
        sizeof(GC_CheckVMThreadStacks), MM_AllocationCategory::DIAGNOSTIC, J9_GET_CALLSITE());
    if (check) {
...
void GC_CheckVMThreadStacks::kill()
{
    MM_Forge* forge = MM_GCExtensions::getExtensions(_javaVM)->getForge();
    forge->free(this);
}

void GC_CheckVMThreadStacks::check()
{
    ...
    while ((walkThread = vmThreadListIterator.nextVMThread()) != NULL) {
        ...
        if (NULL != toScanWalkThread) {
            GC_VMThreadStackSlotIterator::scanSlots(
                toScanWalkThread, toScanWalkThread, (void*)&localData, checkStackSlotIterator, false, false);
...
void GC_CheckVMThreadStacks::print()
{
    ...
    while ((walkThread = newVMThreadListIterator.nextVMThread()) != NULL) {
        ...
        toScanWalkThread = walkThread;
        GC_VMThreadStackSlotIterator::scanSlots(
            toScanWalkThread, toScanWalkThread, (void*)&localData, printStackSlotIterator, false, false);
