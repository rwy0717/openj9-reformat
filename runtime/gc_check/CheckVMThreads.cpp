...
GC_Check* GC_CheckVMThreads::newInstance(J9JavaVM* javaVM, GC_CheckEngine* engine)
{
    MM_Forge* forge = MM_GCExtensions::getExtensions(javaVM)->getForge();

    GC_CheckVMThreads* check = (GC_CheckVMThreads*)forge->allocate(
        sizeof(GC_CheckVMThreads), MM_AllocationCategory::DIAGNOSTIC, J9_GET_CALLSITE());
    if (check) {
...
void GC_CheckVMThreads::check()
{
    ...
    while ((walkThread = vmThreadListIterator.nextVMThread()) != NULL) {
        ...
        while ((slotPtr = vmthreadIterator.nextSlot()) != NULL) {
            if (_engine->checkSlotVMThread(_javaVM, slotPtr, walkThread, check_type_other, &vmthreadIterator)
