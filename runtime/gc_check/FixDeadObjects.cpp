...
GC_Check* GC_FixDeadObjects::newInstance(J9JavaVM* javaVM, GC_CheckEngine* engine)
{
    MM_Forge* forge = MM_GCExtensions::getExtensions(javaVM)->getForge();

    GC_FixDeadObjects* check = (GC_FixDeadObjects*)forge->allocate(
        sizeof(GC_FixDeadObjects), MM_AllocationCategory::DIAGNOSTIC, J9_GET_CALLSITE());
    if (check) {
    ...
    J9JavaVM* vm, J9MM_IterateHeapDescriptor* heapDesc, void* userData)
{
    ...
    vm->memoryManagerFunctions->j9mm_iterate_spaces(
        vm, castUserData->portLibrary, heapDesc, 0, fix_spaceIteratorCallback, castUserData);
    ...
    J9JavaVM* vm, J9MM_IterateSpaceDescriptor* spaceDesc, void* userData)
{
    ...
    vm->memoryManagerFunctions->j9mm_iterate_regions(
        vm, castUserData->portLibrary, spaceDesc, 0, fix_regionIteratorCallback, castUserData);
    return JVMTI_ITERATION_CONTINUE;
}
    ...
    J9JavaVM* vm, J9MM_IterateRegionDescriptor* regionDesc, void* userData)
{
    ...
    vm->memoryManagerFunctions->j9mm_iterate_region_objects(
        vm, castUserData->portLibrary, regionDesc, 0, fix_objectIteratorCallback, castUserData);
    return JVMTI_ITERATION_CONTINUE;
}
    ...
    J9JavaVM* vm, J9MM_IterateObjectDescriptor* objectDesc, void* userData)
{
    ...
    } else if (castUserData->engine->checkObjectHeap(vm, objectDesc, castUserData->regionDesc)
        != J9MODRON_SLOT_ITERATOR_OK) {
