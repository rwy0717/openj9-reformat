...
private:
protected:
    OMR_VM* _omrVM; /**< used so that we can pull the arrayletLeafSize and arrayletLeafLogSize for arraylet sizing
                       calculations */
    ...
    UDATA _largestDesirableArraySpineSize; /**< A cached copy of the subspace's _largestDesirableArraySpineSize to be
                                              used when we don't have access to a subspace. */
public:
    typedef enum ArrayLayout { Illegal = 0, InlineContiguous, Discontiguous, Hybrid } ArrayLayout;
...
public:
     ...
     */
    MMINLINE UDATA getSizeInElements(J9IndexableObject* arrayPtr)
    {
     ...
     */
    MMINLINE void setSizeInElementsForContiguous(J9IndexableObject* arrayPtr, UDATA size)
    {
     ...
     * @param size Size in elements to set.
     */
    MMINLINE void setSizeInElementsForDiscontiguous(J9IndexableObject* arrayPtr, UDATA size)
    {
#if defined(J9VM_GC_HYBRID_ARRAYLETS)
     ...
     */
    MMINLINE bool shouldAlignSpineDataSection(J9Class* clazz)
    {
     ...
     */
    MMINLINE UDATA arrayletSize(J9IndexableObject* objPtr, UDATA index, UDATA dataSizeInBytes, UDATA numArraylets)
    {
    ...
    MMINLINE UDATA numArraylets(UDATA unadjustedDataSizeInBytes)
    {
        ...
        if (UDATA_MAX != leafSize) {
             ...
             */
            UDATA dataSizeInBytes
                = (UDATA_MAX == unadjustedDataSizeInBytes) ? UDATA_MAX : (unadjustedDataSizeInBytes + 1);
             ...
             */
            numberOfArraylets = ((dataSizeInBytes >> leafLogSize)
                + (((dataSizeInBytes & leafSizeMask) + leafSizeMask) >> leafLogSize));
        }
     ...
     * @param largestDesirableArraySpineSize The subspace's _largestDesirableArraySpineSize to be used when we don't
     * have access to a subspace.
     */
    void expandArrayletSubSpaceRange(
