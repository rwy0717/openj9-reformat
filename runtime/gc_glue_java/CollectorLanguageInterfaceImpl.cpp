...
MM_CollectorLanguageInterfaceImpl* MM_CollectorLanguageInterfaceImpl::newInstance(MM_EnvironmentBase* env)
{
    ...
    cliJava = (MM_CollectorLanguageInterfaceImpl*)env->getForge()->allocate(
        sizeof(MM_CollectorLanguageInterfaceImpl), MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
    if (NULL != cliJava) {
 ...
 */
bool MM_CollectorLanguageInterfaceImpl::initialize(MM_EnvironmentBase* env) { return true; }

/**
 * Free any internal structures associated to the receiver.
 */
void MM_CollectorLanguageInterfaceImpl::tearDown(MM_EnvironmentBase* env) {}

#if defined(OMR_GC_MODRON_SCAVENGER)
void MM_CollectorLanguageInterfaceImpl::scavenger_masterSetupForGC(MM_EnvironmentBase* envBase)
{
    ...
    _scavenger_shouldScavengeFinalizableObjects
        = _extensions->finalizeListManager->isFinalizableObjectProcessingRequired();
...
{
    Assert_GC_true_with_message2(envBase,
        _extensions->scavengerJavaStats._ownableSynchronizerCandidates
            >= _extensions->scavengerJavaStats._ownableSynchronizerTotalSurvived,
        "[MM_CollectorLanguageInterfaceImpl::scavenger_reportScavengeEnd]: _extensions->scavengerJavaStats: "
        "_ownableSynchronizerCandidates=%zu < _ownableSynchronizerTotalSurvived=%zu\n",
        _extensions->scavengerJavaStats._ownableSynchronizerCandidates,
        _extensions->scavengerJavaStats._ownableSynchronizerTotalSurvived);

    if (!scavengeSuccessful) {
        /* for backout case, the ownableSynchronizerObject lists is restored before scavenge, so all of candidates are
         * survived */
        _extensions->scavengerJavaStats._ownableSynchronizerTotalSurvived
            = _extensions->scavengerJavaStats._ownableSynchronizerCandidates;

        _extensions->scavengerJavaStats._ownableSynchronizerNurserySurvived
            = _extensions->scavengerJavaStats._ownableSynchronizerCandidates;
    }
}
    ...
    MM_EnvironmentBase* envBase)
{
     ...
     */
    /* Temporarily disabling the update of salt for Concurrent Scavenger. Since salt is changed at the end of a GC
     * cycle, object that are allocated and hashed during a cycle would have different hash value before and after the
     * cycle end.
     */
...
bool MM_CollectorLanguageInterfaceImpl::scavenger_internalGarbageCollect_shouldPercolateGarbageCollect(
    MM_EnvironmentBase* envBase, PercolateReason* percolateReason, U_32* percolateType)
{
    ...
    MM_EnvironmentStandard* env, omrobjectptr_t objectPtr, void* allocSpace, uintptr_t flags)
{
        ...
        if (GC_ObjectScanner::isHeapScan(flags)) {
            ...
            case J9_JAVA_CLASS_REFERENCE_WEAK:
                ...
                if (!referentMustBeCleared && shouldScavengeReferenceObject
                    && !_scavenger_shouldScavengeWeakReferenceObjects) {
            ...
            case J9_JAVA_CLASS_REFERENCE_SOFT:
                ...
                if (!referentMustBeCleared && shouldScavengeReferenceObject
                    && !_scavenger_shouldScavengeSoftReferenceObjects) {
            ...
            case J9_JAVA_CLASS_REFERENCE_PHANTOM:
                ...
                if (!referentMustBeCleared && shouldScavengeReferenceObject
                    && !_scavenger_shouldScavengePhantomReferenceObjects) {
            ...
            objectScanner
                = GC_ReferenceObjectScanner::newInstance(env, objectPtr, referentSlotAddress, allocSpace, flags);
        } else {
            objectScanner = GC_MixedObjectScanner::newInstance(env, objectPtr, allocSpace, flags);
        }
        break;
    ...
    case GC_ObjectModel::SCAN_POINTER_ARRAY_OBJECT: {
        ...
        if (!GC_ObjectScanner::isIndexableObjectNoSplit(flags)) {
            splitAmount = _extensions->scavenger->getArraySplitAmount(
                env, _extensions->indexableObjectModel.getSizeInElements((omrarrayptr_t)objectPtr));
        }
    ...
    MM_EnvironmentStandard* env, omrobjectptr_t objectPtr)
{
    ...
    do {
        ...
        GC_ConstantPoolObjectSlotIterator constantPoolObjectSlotIterator(
            (J9JavaVM*)_omrVM->_language_vm, classToScan, true);
        while ((slotPtr = constantPoolObjectSlotIterator.nextSlot()) != NULL) {
            shouldBeRemembered = _extensions->scavenger->copyObjectSlot(env, slotPtr) || shouldBeRemembered;
        }
        shouldBeRemembered = _extensions->scavenger->copyObjectSlot(env, (omrobjectptr_t*)&(classToScan->classObject))
            || shouldBeRemembered;
...
void MM_CollectorLanguageInterfaceImpl::scavenger_fixupIndirectObjectSlots(
    MM_EnvironmentStandard* env, omrobjectptr_t objectPtr)
{
...
{
    GC_SegmentIterator classSegmentIterator(
        ((J9JavaVM*)_omrVM->_language_vm)->classMemorySegments, MEMORY_TYPE_RAM_CLASS);
...
void MM_CollectorLanguageInterfaceImpl::scavenger_reverseForwardedObject(
    MM_EnvironmentBase* env, MM_ForwardedHeader* originalForwardedHeader)
{
    if (originalForwardedHeader->isForwardedPointer()) {
        ...
        if (OBJECT_HEADER_HAS_BEEN_MOVED_IN_CLASS
            == (forwardedFlags & (OBJECT_HEADER_HAS_BEEN_HASHED_IN_CLASS | OBJECT_HEADER_HAS_BEEN_MOVED_IN_CLASS))) {
...
void MM_CollectorLanguageInterfaceImpl::scavenger_fixupDestroyedSlot(
    MM_EnvironmentBase* env, MM_ForwardedHeader* originalForwardedHeader, MM_MemorySubSpaceSemiSpace* subSpaceNew)
{
    /* Nothing to do if overlap slot is not an object reference, and the destroyed slot for indexable objects is the
     * size */
    if ((0 != originalForwardedHeader->getPreservedOverlap())
        ...
        if (isObjectSlot) {
            ...
            omrobjectptr_t survivingCopyAddress
                = barrier->convertPointerFromToken(originalForwardedHeader->getPreservedOverlap());
            ...
            if (0 == ((uintptr_t)survivingCopyAddress & (_extensions->getObjectAlignmentInBytes() - 1))) {
                ...
                if (subSpaceNew->isObjectInNewSpace(survivingCopyAddress, topOfObject)
                    ...
                    /* if the slot points to a reverse-forwarded object, restore the original location (in evacuate
                     * space) */
                    ...
                    if (reverseForwardedHeader.isReverseForwardedPointer()) {
                        ...
                        originalForwardedHeader->restoreDestroyedOverlap((uint32_t)barrier->convertTokenFromPointer(
...
void MM_CollectorLanguageInterfaceImpl::private_scavenger_addOwnableSynchronizerObjectInList(
    MM_EnvironmentStandard* env, omrobjectptr_t object)
{
    omrobjectptr_t link
        = MM_GCExtensions::getExtensions(_extensions)->accessBarrier->isObjectInOwnableSynchronizerList(object);
    ...
    if (NULL != link) {
        /* this method expects the caller (scanObject) never pass the same object twice, which could cause circular loop
         * when walk through the list. the assertion partially could detect duplication case */
...
void MM_CollectorLanguageInterfaceImpl::private_scavenger_setupForOwnableSynchronizerProcessing(
...
bool MM_CollectorLanguageInterfaceImpl::private_scavenger_shouldPercolateGarbageCollect_classUnloading(
    MM_EnvironmentBase* envBase)
{
...
bool MM_CollectorLanguageInterfaceImpl::private_scavenger_shouldPercolateGarbageCollect_activeJNICriticalRegions(
    MM_EnvironmentBase* envBase)
{
    bool shouldGCPercolate = false;
...
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
bool MM_CollectorLanguageInterfaceImpl::scavenger_shouldYield()
{
    return (J9_XACCESS_PENDING == ((J9JavaVM*)_omrVM->_language_vm)->exclusiveAccessState);
}

void MM_CollectorLanguageInterfaceImpl::scavenger_switchConcurrentForThread(MM_EnvironmentBase* env)
{
    ...
    if (_extensions->isConcurrentScavengerInProgress()) {
        ...
        vmThread->readBarrierRangeCheckBaseCompressed
            = _extensions->accessBarrier->convertTokenFromPointer((mm_j9object_t)vmThread->readBarrierRangeCheckBase);
        vmThread->readBarrierRangeCheckTopCompressed
            = _extensions->accessBarrier->convertTokenFromPointer((mm_j9object_t)vmThread->readBarrierRangeCheckTop);
        ...
        if (_extensions->isConcurrentScavengerHWSupported()) {
            ...
            Assert_MM_true(top
                <= (void*)((uintptr_t)_extensions->getConcurrentScavengerPageStartAddress()
                       + _extensions->getConcurrentScavengerPageSectionSize() * CONCURRENT_SCAVENGER_PAGE_SECTIONS));

            uintptr_t sectionCount
                = ((uintptr_t)top - (uintptr_t)base) / _extensions->getConcurrentScavengerPageSectionSize();
            uintptr_t startOffsetInBits
                = ((uintptr_t)base - (uintptr_t)_extensions->getConcurrentScavengerPageStartAddress())
                / _extensions->getConcurrentScavengerPageSectionSize();
            uint64_t bitMask = (((uint64_t)1 << sectionCount) - 1)
                << (CONCURRENT_SCAVENGER_PAGE_SECTIONS - (sectionCount + startOffsetInBits));
            ...
            if (_extensions->isDebugConcurrentScavengerPageAlignment()) {
                ...
                void* pageTop = (void*)((uintptr_t)pageBase
                    + _extensions->getConcurrentScavengerPageSectionSize() * CONCURRENT_SCAVENGER_PAGE_SECTIONS);

                j9tty_printf(PORTLIB,
                    "%p: Nursery [%p,%p] Evacuate [%p,%p] GS [%p,%p] Section size 0x%zx, sections %lu bit offset %lu "
                    "bit mask 0x%zx\n",
                    vmThread, nurseryBase, nurseryTop, base, top, pageBase, pageTop,
                    _extensions->getConcurrentScavengerPageSectionSize(), sectionCount, startOffsetInBits, bitMask);
            }
            j9gs_enable(&vmThread->gsParameters, _extensions->getConcurrentScavengerPageStartAddress(),
                _extensions->getConcurrentScavengerPageSectionSize(), bitMask);
        }
    } else {
        if (_extensions->isConcurrentScavengerHWSupported()) {
