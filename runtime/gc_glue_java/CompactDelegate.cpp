...
bool MM_CompactDelegate::initialize(
    MM_EnvironmentBase* env, OMR_VM* omrVM, MM_MarkMap* markMap, MM_CompactScheme* compactScheme)
{
    _omrVM = omrVM;
 ...
 */
void MM_CompactDelegate::tearDown(MM_EnvironmentBase* env) {}

void MM_CompactDelegate::verifyHeap(MM_EnvironmentBase* env, MM_MarkMap* markMap)
{
        ...
        while (NULL != (objectPtr = markedObjectIterator.nextObject())) {
            ...
            case GC_ObjectModel::SCAN_REFERENCE_MIXED_OBJECT: {
                ...
                while (GC_SlotObject* slotObject = it.nextSlot()) {
                    if ((slotObject->readReferenceFromSlot() >= extensions->getHeap()->getHeapBase())
                        && (slotObject->readReferenceFromSlot() < extensions->getHeap()->getHeapTop())) {
...
void MM_CompactDelegate::fixupRoots(MM_EnvironmentBase* env, MM_CompactScheme* compactScheme)
{
    MM_CompactSchemeFixupRoots rootScanner(env, compactScheme);
    rootScanner.scanAllSlots(env);
}

void MM_CompactDelegate::workerCleanupAfterGC(MM_EnvironmentBase* env)
{
...
void MM_CompactDelegate::masterSetupForGC(MM_EnvironmentBase* env)
{
    MM_GCExtensionsBase* extensions = env->getExtensions();
    MM_HeapRegionDescriptorStandard* region = NULL;
