 ...
 */
void concurrentStackSlotIterator(J9JavaVM* javaVM, omrobjectptr_t* objectIndirect, void* localData,
    J9StackWalkState* walkState, const void* stackLocation)
{
    MM_ConcurrentMarkingDelegate::markSchemeStackIteratorData* data
        = (MM_ConcurrentMarkingDelegate::markSchemeStackIteratorData*)localData;

    omrobjectptr_t object = *objectIndirect;
    if (data->env->getExtensions()->heap->objectIsInGap(object)) {
        /* CMVC 136483:  Ensure that the object is not in the gap of a split heap (stack-allocated object) since we
         * can't mark that part of the address space */
...
void MM_ConcurrentMarkingDelegate::signalThreadsToTraceStacks(MM_EnvironmentBase* env)
{
...
void MM_ConcurrentMarkingDelegate::signalThreadsToActivateWriteBarrier(MM_EnvironmentBase* env)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
    GC_VMInterface::lockVMThreadList(extensions);
...
void MM_ConcurrentMarkingDelegate::signalThreadsToDeactivateWriteBarrier(MM_EnvironmentBase* env)
{
...
void MM_ConcurrentMarkingDelegate::signalThreadsToDirtyCards(MM_EnvironmentBase* env)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
    GC_VMInterface::lockVMThreadList(extensions);
...
void MM_ConcurrentMarkingDelegate::signalThreadsToStopDirtyingCards(MM_EnvironmentBase* env)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(_javaVM);
    if (extensions->optimizeConcurrentWB) {
        GC_VMInterface::lockVMThreadList(extensions);
...
bool MM_ConcurrentMarkingDelegate::scanThreadRoots(MM_EnvironmentBase* env)
{
    J9VMThread* vmThread = (J9VMThread*)env->getLanguageVMThread();
    Assert_GC_true_with_message(env, vmThread->privateFlags & J9_PRIVATE_FLAGS_CONCURRENT_MARK_ACTIVE,
    ...
    GC_VMThreadStackSlotIterator::scanSlots(
        vmThread, vmThread, (void*)&localData, concurrentStackSlotIterator, true, false);
 ...
 *
 */
void MM_ConcurrentMarkingDelegate::collectJNIRoots(MM_EnvironmentBase* env, bool* completedJNIRoots)
{
    *completedJNIRoots = false;

    Assert_GC_true_with_message(env,
        ((J9VMThread*)env->getLanguageVMThread())->privateFlags & J9_PRIVATE_FLAGS_CONCURRENT_MARK_ACTIVE,
        "MM_ConcurrentStats::_executionMode = %zu\n", _collector->getConcurrentGCStats()->getExecutionMode());
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
 ...
 *
 */
void MM_ConcurrentMarkingDelegate::collectClassRoots(
    MM_EnvironmentBase* env, bool* completedClassRoots, bool* classesMarkedAsRoots)
{
    *completedClassRoots = false;
    *classesMarkedAsRoots = false;
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
#if defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING)
    MM_GCExtensions::DynamicClassUnloading dynamicClassUnloadingFlag
        = (MM_GCExtensions::DynamicClassUnloading)extensions->dynamicClassUnloading;
 ...
 *
 */
void MM_ConcurrentMarkingDelegate::collectFinalizableRoots(MM_EnvironmentBase* env, bool* completedFinalizableRoots)
{
 ...
 *
 */
void MM_ConcurrentMarkingDelegate::collectStringRoots(
    MM_EnvironmentBase* env, bool* completedStringRoots, bool* collectedStringRoots)
{
...
void MM_ConcurrentMarkingDelegate::abortCollection(MM_EnvironmentBase* env)
{
...
#if defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING)
uintptr_t MM_ConcurrentMarkingDelegate::concurrentClassMark(MM_EnvironmentBase* env, bool* completedClassMark)
{
    ...
    *completedClassMark = false;
    ...
    while ((classLoader = classLoaderIterator.nextSlot()) != NULL) {
        ...
        if ((0 == (classLoader->gcFlags & J9_GC_CLASS_LOADER_DEAD))
            && (0 == (classLoader->flags & J9CLASSLOADER_ANON_CLASS_LOADER))) {
            /* Check if the class loader has not been scanned but the class loader is live */
            if (!(classLoader->gcFlags & J9_GC_CLASS_LOADER_SCANNED)
                && _markingScheme->isMarkedOutline(classLoader->classLoaderObject)) {
                ...
                while (NULL != (segment = segmentIterator.nextSegment())) {
                    ...
                    while (NULL != (clazz = classHeapIterator.nextClass())) {
                        ...
                        if (env->isExclusiveAccessRequestWaiting()) { /* interrupt if exclusive access request is
                                                                         waiting */
