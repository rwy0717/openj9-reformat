...
public:
    ...
    MMINLINE uintptr_t collectRoots(
    ...
    {
#if defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING)
        _scanClassesMode.setScanClassesMode(
            MM_ScanClassesMode::SCAN_CLASSES_DISABLED); /* disable concurrent classes scan - default state */
#endif /* J9VM_GC_DYNAMIC_CLASS_UNLOADING */
    }
    ...
    MMINLINE void cardCleaningStarted(MM_EnvironmentBase* env)
    {
        ...
        _scanClassesMode.switchScanClassesMode(
            MM_ScanClassesMode::SCAN_CLASSES_COMPLETE, MM_ScanClassesMode::SCAN_CLASSES_NEED_TO_BE_EXECUTED);
#endif /* J9VM_GC_DYNAMIC_CLASS_UNLOADING */
    }
    ...
    MMINLINE bool startConcurrentScanning(MM_EnvironmentBase* env, uintptr_t* bytesTraced, bool* collectedRoots)
    {
        ...
        *collectedRoots = false;
        bool started = false;
#if defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING)
        if (_scanClassesMode.switchScanClassesMode(MM_ScanClassesMode::SCAN_CLASSES_NEED_TO_BE_EXECUTED,
                MM_ScanClassesMode::SCAN_CLASSES_CURRENTLY_ACTIVE)) { /* currently not running */
    ...
    MMINLINE void concurrentScanningStarted(MM_EnvironmentBase* env, uintptr_t bytesTraced)
    {
        ...
        if (0 != bytesTraced) {
            _scanClassesMode.setScanClassesMode(
                MM_ScanClassesMode::SCAN_CLASSES_NEED_TO_BE_EXECUTED); /* need more iterations */
        } else {
