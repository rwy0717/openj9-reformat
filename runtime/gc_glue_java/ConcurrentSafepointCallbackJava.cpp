...
MM_ConcurrentSafepointCallbackJava* MM_ConcurrentSafepointCallbackJava::newInstance(MM_EnvironmentBase* env)
{
    MM_ConcurrentSafepointCallbackJava* callback;

    callback = (MM_ConcurrentSafepointCallbackJava*)env->getForge()->allocate(
        sizeof(MM_ConcurrentSafepointCallbackJava), MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
    if (NULL != callback) {
...
void MM_ConcurrentSafepointCallbackJava::kill(MM_EnvironmentBase* env)
{
    if (-1 != _asyncEventKey) {
...
bool MM_ConcurrentSafepointCallbackJava::initialize(MM_EnvironmentBase* env)
{
    ...
    if (NULL == env->getOmrVMThread()) {
        ...
        (*vmHooks)->J9HookRegisterWithCallSite(
            vmHooks, J9HOOK_VM_SHUTTING_DOWN, vmTerminating, OMR_GET_CALLSITE(), this);
    } else {
 ...
 *
 */
void MM_ConcurrentSafepointCallbackJava::vmInitialized(
...
void MM_ConcurrentSafepointCallbackJava::registerAsyncEventHandler(
    MM_EnvironmentBase* env, MM_ConcurrentSafepointCallbackJava* callback)
{
    J9JavaVM* javaVM = (J9JavaVM*)env->getLanguageVM();
...
void MM_ConcurrentSafepointCallbackJava::asyncEventHandler(J9VMThread* vmThread, intptr_t handlerKey, void* userData)
{
 ...
 *
 */
void MM_ConcurrentSafepointCallbackJava::vmTerminating(
    J9HookInterface** hook, uintptr_t eventNum, void* eventData, void* userData)
{
 ...
 * @param userData reference to MM_ConcurrentSafepointCallbackJava
 */
void MM_ConcurrentSafepointCallbackJava::reportGlobalGCComplete(
    J9HookInterface** hook, uintptr_t eventNum, void* eventData, void* userData)
{
...
#endif /* defined(AIXPPC) || defined(LINUXPPC) */
{
    ...
    if (_cancelAfterGC) {
        ...
        J9HookInterface** mmHooks = J9_HOOK_INTERFACE(env->getExtensions()->omrHookInterface);
        (*mmHooks)->J9HookRegisterWithCallSite(
            mmHooks, J9HOOK_MM_OMR_GLOBAL_GC_END, reportGlobalGCComplete, OMR_GET_CALLSITE(), this);
    }
#endif /* defined(AIXPPC) || defined(LINUXPPC) */
}

void MM_ConcurrentSafepointCallbackJava::requestCallback(MM_EnvironmentBase* env)
{
...
void MM_ConcurrentSafepointCallbackJava::cancelCallback(MM_EnvironmentBase* env)
{
    J9JavaVM* javaVM = (J9JavaVM*)env->getLanguageVM();
