...
typedef struct MM_HeapRegionDescriptorStandardExtension {
    ...
    MM_OwnableSynchronizerObjectList*
        _ownableSynchronizerObjectLists; /**< An array of lists of ownable synchronizer objects in this region */
    MM_ReferenceObjectList*
        _referenceObjectLists; /**< An array of lists of reference objects (i.e. weak/soft/phantom) in this region */
...
public:
    bool initialize(MM_EnvironmentBase* env, MM_GCWriteBarrierType writeBarrierType, MM_GCAllocationType allocationType)
    {
        ...
        /*
         *  note that these are the default thresholds but Realtime Configurations override these values, in their
         * initialize methods (hence it is key for them to call their super initialize, first)
         */
...
#define DYNAMIC_CLASS_UNLOADING_KICKOFF_THRESHOLD 80000
    ...
    static MM_HeapRegionDescriptorStandardExtension* getHeapRegionDescriptorStandardExtension(
    ...
    bool initializeHeapRegionDescriptorExtension(MM_EnvironmentBase* env, MM_HeapRegionDescriptor* region)
    {
        ...
        if (extensions->isStandardGC()) {
            ...
            uintptr_t allocSize = sizeof(MM_HeapRegionDescriptorStandardExtension)
                + (listCount
                      * (sizeof(MM_UnfinalizedObjectList) + sizeof(MM_OwnableSynchronizerObjectList)
                            + sizeof(MM_ReferenceObjectList)));
            MM_HeapRegionDescriptorStandardExtension* regionExtension
                = (MM_HeapRegionDescriptorStandardExtension*)env->getForge()->allocate(
                    allocSize, MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
            if (NULL == regionExtension) {
                return false;
            }

            regionExtension->_maxListIndex = listCount;
            regionExtension->_unfinalizedObjectLists = (MM_UnfinalizedObjectList*)((uintptr_t)regionExtension
                + sizeof(MM_HeapRegionDescriptorStandardExtension));
            regionExtension->_ownableSynchronizerObjectLists
                = (MM_OwnableSynchronizerObjectList*)(regionExtension->_unfinalizedObjectLists + listCount);
            regionExtension->_referenceObjectLists
                = (MM_ReferenceObjectList*)(regionExtension->_ownableSynchronizerObjectLists + listCount);
            ...
            for (uintptr_t list = 0; list < listCount; list++) {
                ...
                if (NULL != extensions->unfinalizedObjectLists) {
                    extensions->unfinalizedObjectLists->setPreviousList(
                        &regionExtension->_unfinalizedObjectLists[list]);
                }
                ...
                regionExtension->_ownableSynchronizerObjectLists[list].setNextList(
                    extensions->ownableSynchronizerObjectLists);
                ...
                if (NULL != extensions->ownableSynchronizerObjectLists) {
                    extensions->ownableSynchronizerObjectLists->setPreviousList(
                        &regionExtension->_ownableSynchronizerObjectLists[list]);
                }
    ...
    bool heapInitialized(MM_EnvironmentBase* env)
    {
        ...
        javaVM->identityHashData
            = (J9IdentityHashData*)env->getForge()->allocate(size, MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
        ...
        if (result) {
            ...
            case J9_IDENTITY_HASH_SALT_POLICY_STANDARD:
                javaVM->identityHashData->hashSaltTable[J9GC_HASH_SALT_NURSERY_INDEX]
                    = (U_32)convertValueToHash(javaVM, 1421595292 ^ (U_32)(uintptr_t)javaVM);
                break;
            case J9_IDENTITY_HASH_SALT_POLICY_REGION:
                for (uintptr_t index = 0; index < hashSaltCount; index++) {
                    javaVM->identityHashData->hashSaltTable[index] = (U_32)convertValueToHash(javaVM,
                        1421595292 ^ (U_32)(uintptr_t)heapRegionManager->physicalTableDescriptorForIndex(index));
                }
    ...
    bool environmentInitialized(MM_EnvironmentBase* env)
    {
        ...
        if (extensions->isConcurrentMarkEnabled()) {
            ...
            if (!extensions->optimizeConcurrentWB
                || (CONCURRENT_OFF < concurrentGC->getConcurrentGCStats()->getExecutionMode())) {
    ...
    {
        J9JavaVM* javaVM = (J9JavaVM*)env->getOmrVM()->_language_vm;
        /* processing estimate Fragmentation only is on non startup stage to avoid startup regression(fragmentation
           during startup is not meaningful for the estimation) it is only for jit mode(for int mode javaVM->phase is
           always J9VM_PHASE_NOT_STARTUP) */
        return (J9VM_PHASE_NOT_STARTUP == javaVM->phase);
    }

    uintptr_t getMaxGCThreadCount(MM_EnvironmentBase* env) { return _maximumDefaultNumberOfGCThreads; }
