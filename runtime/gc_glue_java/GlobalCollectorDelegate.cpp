 ...
 */
static void fixObjectIfClassDying(
    OMR_VMThread* omrVMThread, MM_HeapRegionDescriptor* region, omrobjectptr_t object, void* userData)
{
    ...
    if (0 != (classFlags & J9_JAVA_CLASS_DYING)) {
        ...
        uintptr_t deadObjectByteSize
            = MM_GCExtensions::getExtensions(omrVMThread)->objectModel.getConsumedSizeInBytesWithHeader(object);
...
bool MM_GlobalCollectorDelegate::initialize(
    MM_EnvironmentBase* env, MM_GlobalCollector* globalCollector, MM_MarkingScheme* markingScheme)
{
...
void MM_GlobalCollectorDelegate::tearDown(MM_EnvironmentBase* env)
{
...
void MM_GlobalCollectorDelegate::masterThreadGarbageCollectStarted(MM_EnvironmentBase* env)
{
    ...
    case MM_GCExtensions::DYNAMIC_CLASS_UNLOADING_ON_CLASS_LOADER_CHANGES:
        ...
        _extensions->runtimeCheckDynamicClassUnloading
            = forceUnloading || _extensions->classLoaderManager->isTimeForClassUnloading(env);
        break;
    default:
        break;
    }
...
void MM_GlobalCollectorDelegate::masterThreadGarbageCollectFinished(MM_EnvironmentBase* env, bool compactedThisCycle)
{
    ...
    /* make sure that we are going to get at least some number of bytes back since this will otherwise waste time in
     * monitor operations and potentially get exclusive in order to do nothing */
    ...
    if (reclaimableMemory > 0) {
        if (!compactedThisCycle) {
            ...
            if (isExplicitGC || (reclaimableMemory > _extensions->deadClassLoaderCacheSize)) {
                ...
                UDATA fixedObjectCount = parallelGlobalCollector->fixHeapForWalk(
                    env, MEMORY_TYPE_RAM, FIXUP_CLASS_UNLOADING, fixObjectIfClassDying);
...
void MM_GlobalCollectorDelegate::postMarkProcessing(MM_EnvironmentBase* env)
{
    ...
    if (_extensions->runtimeCheckDynamicClassUnloading != 0) {
        ...
        TRIGGER_J9HOOK_MM_PRIVATE_CLASS_UNLOADING_START(
            _extensions->privateHookInterface, vmThread, j9time_hires_clock(), J9HOOK_MM_PRIVATE_CLASS_UNLOADING_START);

        unloadDeadClassLoaders(env);

        MM_ClassUnloadStats* classUnloadStats = &_extensions->globalGCStats.classUnloadStats;
        Trc_MM_ClassUnloadingEnd((J9VMThread*)vmThread->_language_vmthread, classUnloadStats->_classLoaderUnloadedCount,
            classUnloadStats->_classesUnloadedCount);
        TRIGGER_J9HOOK_MM_CLASS_UNLOADING_END(_extensions->hookInterface, (J9VMThread*)vmThread->_language_vmthread,
            j9time_hires_clock(), J9HOOK_MM_CLASS_UNLOADING_END,
            classUnloadStats->_endTime - classUnloadStats->_startTime, classUnloadStats->_classLoaderUnloadedCount,
            classUnloadStats->_classesUnloadedCount, classUnloadStats->_classUnloadMutexQuiesceTime,
...
bool MM_GlobalCollectorDelegate::isAllowUserHeapWalk()
{
    /* Enable  only if required for debugging. */
    return (0 != (_javaVM->requiredDebugAttributes & J9VM_DEBUG_ATTRIBUTE_ALLOW_USER_HEAP_WALK));
}

void MM_GlobalCollectorDelegate::prepareHeapForWalk(MM_EnvironmentBase* env)
{
#if defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING)
...
bool MM_GlobalCollectorDelegate::heapAddRange(
...
bool MM_GlobalCollectorDelegate::heapRemoveRange(MM_EnvironmentBase* env, MM_MemorySubSpace* subspace, UDATA size,
    void* lowAddress, void* highAddress, void* lowValidAddress, void* highValidAddress)
{
    if (NULL != _extensions->referenceChainWalkerMarkMap) {
        return _extensions->referenceChainWalkerMarkMap->heapRemoveRange(
            env, size, lowAddress, highAddress, lowValidAddress, highValidAddress);
    }
    return true;
}

bool MM_GlobalCollectorDelegate::isTimeForGlobalGCKickoff()
{
    bool result = false;
    ...
    Trc_MM_GlobalCollector_isTimeForGlobalGCKickoff_Entry(_extensions->dynamicClassUnloading, numClassLoaderBlocks,
        _extensions->dynamicClassUnloadingKickoffThreshold,
        _extensions->classLoaderManager->getLastUnloadNumOfClassLoaders());

    Trc_MM_GlobalCollector_isTimeForGlobalGCKickoff_anonClasses(numAnonymousClasses,
    ...
    if ((0 != _extensions->dynamicClassUnloadingKickoffThreshold)
        && (_extensions->dynamicClassUnloading != MM_GCExtensions::DYNAMIC_CLASS_UNLOADING_NEVER)) {
        uintptr_t recentlyLoaded
            = (uintptr_t)((numAnonymousClasses - _extensions->classLoaderManager->getLastUnloadNumOfAnonymousClasses())
                * _extensions->classUnloadingAnonymousClassWeight);
        ...
        if (numClassLoaderBlocks > _extensions->classLoaderManager->getLastUnloadNumOfClassLoaders()) {
            recentlyLoaded
                += (numClassLoaderBlocks - _extensions->classLoaderManager->getLastUnloadNumOfClassLoaders());
        }
...
void MM_GlobalCollectorDelegate::postCollect(MM_EnvironmentBase* env, MM_MemorySubSpace* subSpace)
{
    /* update the dynamic soft reference age based on the free space in the oldest generation after this collection so
     * we know how many to clear next time */
...
#if defined(J9VM_GC_MODRON_COMPACTION)
CompactPreventedReason MM_GlobalCollectorDelegate::checkIfCompactionShouldBePrevented(MM_EnvironmentBase* env)
{
...
#if defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING)
bool MM_GlobalCollectorDelegate::enterClassUnloadMutex(MM_EnvironmentBase* env, bool force)
{
    bool result = true;
...
void MM_GlobalCollectorDelegate::exitClassUnloadMutex(MM_EnvironmentBase* env)
{
    _extensions->classLoaderManager->exitClassUnloadMutex(env);
}

void MM_GlobalCollectorDelegate::unloadDeadClassLoaders(MM_EnvironmentBase* env)
{
    ...
    J9ClassLoader* classLoadersUnloadedList = _extensions->classLoaderManager->identifyClassLoadersToUnload(
        env, _markingScheme->getMarkMap(), classUnloadStats);
    _extensions->classLoaderManager->cleanUpClassLoadersStart(
        env, classLoadersUnloadedList, _markingScheme->getMarkMap(), classUnloadStats);
    ...
    _extensions->classLoaderManager->cleanUpClassLoaders(
        env, classLoadersUnloadedList, &reclaimedSegments, &unloadLink, &_finalizationRequired);
