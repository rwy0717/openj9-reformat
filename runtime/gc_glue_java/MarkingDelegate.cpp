...
bool MM_MarkingDelegate::initialize(MM_EnvironmentBase* env, MM_MarkingScheme* markingScheme)
{
    _omrVM = env->getOmrVM();
    _extensions = MM_GCExtensions::getExtensions(env);
    _markingScheme = markingScheme;
#if defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING)
    _markMap = (_extensions->dynamicClassUnloading != MM_GCExtensions::DYNAMIC_CLASS_UNLOADING_NEVER)
        ? markingScheme->getMarkMap()
        : NULL;
...
#if defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING)
void MM_MarkingDelegate::clearClassLoadersScannedFlag(MM_EnvironmentBase* env)
{
...
void MM_MarkingDelegate::masterSetupForWalk(MM_EnvironmentBase* env)
{
#if defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING)
    _markMap = NULL;
#endif /* J9VM_GC_DYNAMIC_CLASS_UNLOADING */
...
void MM_MarkingDelegate::workerSetupForGC(MM_EnvironmentBase* env)
{
...
void MM_MarkingDelegate::workerCompleteGC(MM_EnvironmentBase* env)
{
...
void MM_MarkingDelegate::workerCleanupAfterGC(MM_EnvironmentBase* env)
{
    GC_Environment* gcEnv = env->getGCEnvironment();
...
void MM_MarkingDelegate::masterSetupForGC(MM_EnvironmentBase* env)
{
#if defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING)
...
void MM_MarkingDelegate::scanRoots(MM_EnvironmentBase* env)
{
        ...
        while (NULL != (region = regionIterator.nextRegion())) {
            ...
            for (UDATA i = 0; i < regionExtension->_maxListIndex; i++) {
                ...
                MM_OwnableSynchronizerObjectList* ownableSynchronizerObjectList
                    = &(regionExtension->_ownableSynchronizerObjectLists[i]);
    ...
    if (isDynamicClassUnloadingEnabled()) {
        ...
        if (env->isMasterThread()) {
            ...
            _markingScheme->markObject(
                env, (omrobjectptr_t)((J9ClassLoader*)javaVM->systemClassLoader)->classLoaderObject);
            if (javaVM->applicationClassLoader) {
                ((J9ClassLoader*)javaVM->applicationClassLoader)->gcFlags |= J9_GC_CLASS_LOADER_SCANNED;
                _markingScheme->markObject(
                    env, (omrobjectptr_t)((J9ClassLoader*)javaVM->applicationClassLoader)->classLoaderObject);
            }
        }
    }
#endif /* J9VM_GC_DYNAMIC_CLASS_UNLOADING */
...
void MM_MarkingDelegate::completeMarking(MM_EnvironmentBase* env)
{
#if defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING)
    if (isDynamicClassUnloadingEnabled()) {
        J9ClassLoader* classLoader;
                    ...
                    if (0 == (classLoader->gcFlags & J9_GC_CLASS_LOADER_DEAD)) {
                        if (J9CLASSLOADER_ANON_CLASS_LOADER == (classLoader->flags & J9CLASSLOADER_ANON_CLASS_LOADER)) {
                            ...
                            while (NULL != (segment = segmentIterator.nextSegment())) {
                                ...
                                while (NULL != (clazz = classHeapIterator.nextClass())) {
                                    Assert_MM_true(
                                        !J9_ARE_ANY_BITS_SET(clazz->classDepthAndFlags, J9_JAVA_CLASS_DYING));
                                    if ((0 == (J9CLASS_EXTENDED_FLAGS(clazz) & J9ClassGCScanned))
                                        && _markingScheme->isMarked(clazz->classObject)) {
                            ...
                            if ((0 == (classLoader->gcFlags & J9_GC_CLASS_LOADER_SCANNED))
                                && _markingScheme->isMarked(classLoader->classLoaderObject)) {
                                ...
                                /*
                                 * We believe that (NULL == classLoader->classHashTable) is set ONLY for DEAD class
                                 * loader so, if this pointer happend to be NULL at this point let it crash here
                                 */
                                ...
                                J9Module** modulePtr
                                    = (J9Module**)hashTableStartDo(classLoader->moduleHashTable, &moduleWalkState);
...
void MM_MarkingDelegate::scanClass(MM_EnvironmentBase* env, J9Class* clazz)
{
...
void MM_MarkingDelegate::processReferenceList(MM_EnvironmentBase* env, MM_HeapRegionDescriptorStandard* region,
    omrobjectptr_t headOfList, MM_ReferenceStats* referenceStats)
{
        ...
        if (NULL != referentSlotObject.readReferenceFromSlot()) {
            ...
            if (_markingScheme->isMarked(referent)) {
                ...
                if ((J9_JAVA_CLASS_REFERENCE_PHANTOM == referenceObjectType)
                    && ((J2SE_VERSION(javaVM) & J2SE_VERSION_MASK) <= J2SE_18)) {
...
bool MM_MarkingDelegate::processReference(MM_EnvironmentBase* env, omrobjectptr_t objectPtr)
{
...
bool MM_MarkingDelegate::getReferenceStatus(
    MM_EnvironmentBase* env, omrobjectptr_t objectPtr, bool* referentMustBeMarked, bool* isReferenceCleared)
{
    /*
    ...
    *isReferenceCleared = (GC_ObjectModel::REF_STATE_CLEARED == referenceState)
...
void MM_MarkingDelegate::clearReference(
    MM_EnvironmentBase* env, omrobjectptr_t objectPtr, bool isReferenceCleared, bool referentMustBeCleared)
{
    if (referentMustBeCleared) {
        ...
        if (!isReferenceCleared) {
            /* for overflow case we assume only 3 active reference states(REF_STATE_INITIAL, REF_STATE_CLEARED,
               REF_STATE_ENQUEUED), REF_STATE_REMEMBERED is only for balanced case, ReferenceCleared(REF_STATE_CLEARED,
               REF_STATE_ENQUEUED) notReferenceCleared(REF_STATE_INITIAL),
...
fomrobject_t* MM_MarkingDelegate::setupReferenceObjectScanner(
    MM_EnvironmentBase* env, omrobjectptr_t objectPtr, MM_MarkingSchemeScanReason reason)
{
    bool isReferenceCleared = false;
    bool referentMustBeMarked = false;
    bool referentMustBeCleared = getReferenceStatus(env, objectPtr, &referentMustBeMarked, &isReferenceCleared);
...
uintptr_t MM_MarkingDelegate::setupPointerArrayScanner(MM_EnvironmentBase* env, omrobjectptr_t objectPtr,
    MM_MarkingSchemeScanReason reason, uintptr_t* sizeToDo, uintptr_t* slotsToDo)
{
    ...
    if (PACKET_ARRAY_SPLIT_TAG == (workItem & PACKET_ARRAY_SPLIT_TAG)) {
        ...
        /* since we are putting extra tagged objects on the work stack we are responsible for ensuring that the object
         * scanned count is correct.  The MM_MarkingScheme::scanObject code will increment _objectScanned by 1 for EVERY
         * object popped off of the work stack if the scan reason is SCAN_REASON_PACKET. This code is only executed
         * during regular packet scanning.
         */
    ...
    if (sizeInElements > 0) {
         ...
         * additionally, the less busy we are, the smaller the split amount, while obeying specified minimum and
         * maximum.
         */

        uintptr_t arraySplitSize = slotsToScan
            / (_extensions->dispatcher->activeThreadCount()
                  + 2 * _markingScheme->getWorkPackets()->getThreadWaitCount());
