...
void MM_MarkingSchemeRootClearer::doSlot(omrobjectptr_t* slotPtr) { Assert_MM_unreachable(); }

void MM_MarkingSchemeRootClearer::doClass(J9Class* clazz) { Assert_MM_unreachable(); }

void MM_MarkingSchemeRootClearer::doFinalizableObject(omrobjectptr_t object) { Assert_MM_unreachable(); }

void MM_MarkingSchemeRootClearer::scanWeakReferenceObjects(MM_EnvironmentBase* env)
{
    ...
    while (NULL != (region = regionIterator.nextRegion())) {
        ...
        /* NOTE: we can't look at the list to determine if there's work to do since another thread may have already
         * processed it and deleted everything */
        for (UDATA i = 0; i < regionExtension->_maxListIndex; i++) {
            if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
                ...
                if (!list->wasWeakListEmpty()) {
                    _markingDelegate->processReferenceList(
                        env, region, list->getPriorWeakList(), &gcEnv->_markJavaStats._weakReferenceStats);
                }
            }
        }
    }
...
MM_RootScanner::CompletePhaseCode MM_MarkingSchemeRootClearer::scanWeakReferencesComplete(MM_EnvironmentBase* env)
{
...
void MM_MarkingSchemeRootClearer::scanSoftReferenceObjects(MM_EnvironmentBase* env)
{
        ...
        for (UDATA i = 0; i < regionExtension->_maxListIndex; i++) {
            if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
                ...
                if (!list->wasSoftListEmpty()) {
                    _markingDelegate->processReferenceList(
                        env, region, list->getPriorSoftList(), &gcEnv->_markJavaStats._softReferenceStats);
                }
            }
        }
    }
...
MM_RootScanner::CompletePhaseCode MM_MarkingSchemeRootClearer::scanSoftReferencesComplete(MM_EnvironmentBase* env)
{
    /* do nothing -- no new objects could have been discovered by soft reference processing */
    return complete_phase_OK;
}

void MM_MarkingSchemeRootClearer::scanPhantomReferenceObjects(MM_EnvironmentBase* env)
{
        ...
        for (UDATA i = 0; i < regionExtension->_maxListIndex; i++) {
            if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
                ...
                if (!list->wasPhantomListEmpty()) {
                    _markingDelegate->processReferenceList(
                        env, region, list->getPriorPhantomList(), &gcEnv->_markJavaStats._phantomReferenceStats);
                }
            }
        }
    }
...
MM_RootScanner::CompletePhaseCode MM_MarkingSchemeRootClearer::scanPhantomReferencesComplete(MM_EnvironmentBase* env)
{
...
void MM_MarkingSchemeRootClearer::scanUnfinalizedObjects(MM_EnvironmentBase* env)
{
                    ...
                    if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
                        ...
                        while (NULL != object) {
                            ...
                            if (_markingScheme->inlineMarkObject(env, object)) {
                                /* object was not previously marked -- it is now finalizable so push it to the local
                                 * buffer */
                                ...
                                if (!finalizationRequired) {
                                    MM_GlobalCollector* globalCollector
                                        = (MM_GlobalCollector*)_extensions->getGlobalCollector();
...
MM_RootScanner::CompletePhaseCode MM_MarkingSchemeRootClearer::scanUnfinalizedObjectsComplete(MM_EnvironmentBase* env)
{
    if (_markingDelegate->shouldScanUnfinalizedObjects()) {
        ...
        env->_currentTask->synchronizeGCThreads(env, UNIQUE_ID);
        /* TODO: consider relaxing completeMarking into completeScan (which will skip over 'complete class marking'
         * step). This is to avoid potentianlly unnecessery sync point in class marking step. The class marking step
         * that we do after phantom processing might be sufficient.
         */
        _markingScheme->completeMarking(env);
...
void MM_MarkingSchemeRootClearer::scanOwnableSynchronizerObjects(MM_EnvironmentBase* env)
{
    if (_markingDelegate->shouldScanOwnableSynchronizerObjects()) {
        ...
        while (NULL != (region = regionIterator.nextRegion())) {
            ...
            /* correct scavenger statistics for ownableSynchronizerObjects, adjust survived ownableSynchronizerObject
             * count in Nursery, only in generational gc */
            if (_extensions->scavengerEnabled && (MEMORY_TYPE_NEW == (region->getTypeFlags() & MEMORY_TYPE_NEW))) {
                gcEnv->_scavengerJavaStats.updateOwnableSynchronizerNurseryCounts(
                    gcEnv->_markJavaStats._ownableSynchronizerCandidates
                    - gcEnv->_markJavaStats._ownableSynchronizerCleared);
            }
...
void MM_MarkingSchemeRootClearer::doMonitorReference(
...
MM_RootScanner::CompletePhaseCode MM_MarkingSchemeRootClearer::scanMonitorReferencesComplete(MM_EnvironmentBase* env)
{
...
void MM_MarkingSchemeRootClearer::doJNIWeakGlobalReference(omrobjectptr_t* slotPtr)
{
...
void MM_MarkingSchemeRootClearer::doRememberedSetSlot(
    omrobjectptr_t* slotPtr, GC_RememberedSetSlotIterator* rememberedSetSlotIterator)
{
    omrobjectptr_t objectPtr = *slotPtr;
    if (objectPtr == NULL) {
...
void MM_MarkingSchemeRootClearer::doStringTableSlot(
    omrobjectptr_t* slotPtr, GC_StringTableIterator* stringTableIterator)
{
 ...
 */
void MM_MarkingSchemeRootClearer::doStringCacheTableSlot(omrobjectptr_t* slotPtr)
{
    omrobjectptr_t objectPtr = *slotPtr;
...
void MM_MarkingSchemeRootClearer::doJVMTIObjectTagSlot(
    omrobjectptr_t* slotPtr, GC_JVMTIObjectTagTableIterator* objectTagTableIterator)
{
    if (!_markingScheme->isMarked(*slotPtr)) {
