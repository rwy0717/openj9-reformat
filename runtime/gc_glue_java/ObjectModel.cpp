...
bool GC_ObjectModel::initialize(MM_GCExtensionsBase* extensions)
{
    bool result = true;
    ...
    if (NULL == vmHookInterface) {
        result = false;
                   ...
                   ->J9HookRegisterWithCallSite(vmHookInterface, J9HOOK_VM_INTERNAL_CLASS_LOAD, internalClassLoadHook,
                       OMR_GET_CALLSITE(), this)) {
        result = false;
    } else if ((*vmHookInterface)
                   ->J9HookRegisterWithCallSite(
                       vmHookInterface, J9HOOK_VM_CLASSES_REDEFINED, classesRedefinedHook, OMR_GET_CALLSITE(), this)) {
        result = false;
    }
...
void GC_ObjectModel::tearDown(MM_GCExtensionsBase* extensions)
{
    J9HookInterface** vmHookInterface = _javaVM->internalVMFunctions->getVMHookInterface(_javaVM);
    if (NULL != vmHookInterface) {
        (*vmHookInterface)
            ->J9HookUnregister(vmHookInterface, J9HOOK_VM_INTERNAL_CLASS_LOAD, internalClassLoadHook, this);
        (*vmHookInterface)->J9HookUnregister(vmHookInterface, J9HOOK_VM_CLASSES_REDEFINED, classesRedefinedHook, this);
    }
}

GC_ObjectModel::ScanType GC_ObjectModel::getSpecialClassScanType(J9Class* objectClazz)
{
    ...
    } else if ((NULL != _atomicMarkableReferenceClass)
        && isSameOrSuperClassOf(_atomicMarkableReferenceClass, objectClazz)) {
...
void GC_ObjectModel::internalClassLoadHook(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (clazz->classLoader == vmThread->javaVM->systemClassLoader) {
        ...
        if (0
            == compareUTF8Length(J9UTF8_DATA(className), J9UTF8_LENGTH(className), (U_8*)atomicMarkableReference,
                   strlen(atomicMarkableReference))) {
            clazz->classDepthAndFlags |= J9_JAVA_CLASS_GC_SPECIAL;
            objectModel->_atomicMarkableReferenceClass = clazz;
        } else if (0
            == compareUTF8Length(J9UTF8_DATA(className), J9UTF8_LENGTH(className), (U_8*)javaLangClassLoader,
                   strlen(javaLangClassLoader))) {
            clazz->classDepthAndFlags |= J9_JAVA_CLASS_GC_SPECIAL;
            objectModel->_classLoaderClass = clazz;
        } else if (0
            == compareUTF8Length(
                   J9UTF8_DATA(className), J9UTF8_LENGTH(className), (U_8*)javaLangClass, strlen(javaLangClass))) {
            clazz->classDepthAndFlags |= J9_JAVA_CLASS_GC_SPECIAL;
            objectModel->_classClass = clazz;
        } else if (0
            == compareUTF8Length(J9UTF8_DATA(className), J9UTF8_LENGTH(className), (U_8*)abstractOwnableSynchronizer,
                   strlen(abstractOwnableSynchronizer))) {
            clazz->classDepthAndFlags |= J9_JAVA_CLASS_OWNABLE_SYNCHRONIZER;
        }
    }
}

void GC_ObjectModel::classesRedefinedHook(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    GC_ObjectModel* objectModel = (GC_ObjectModel*)userData;
