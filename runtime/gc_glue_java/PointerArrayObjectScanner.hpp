...
protected:
     ...
     */
    MMINLINE GC_PointerArrayObjectScanner(MM_EnvironmentBase* env, omrobjectptr_t arrayPtr, fomrobject_t* basePtr,
    ...
    MMINLINE GC_PointerArrayObjectScanner(
        MM_EnvironmentBase* env, GC_PointerArrayObjectScanner* objectScanner, uintptr_t splitAmount)
     ...
     */
    MMINLINE void initialize(MM_EnvironmentBase* env) { GC_IndexableObjectScanner::initialize(env); }
...
public:
     ...
     * @param[in] flags Scanning context flags
     * @param[in] splitAmount If >0, the number of elements to include for this scanner instance; if 0, include all
     * elements
     * @param[in] startIndex The index of the first element to scan
     */
    MMINLINE static GC_PointerArrayObjectScanner* newInstance(MM_EnvironmentBase* env, omrobjectptr_t objectPtr,
        void* allocSpace, uintptr_t flags, uintptr_t splitAmount, uintptr_t startIndex = 0)
    {
    ...
    virtual fomrobject_t* getNextSlotMap(uintptr_t& scanMap, uintptr_t& leafMap, bool& hasNextSlotMap)
    {
        leafMap = 0;
