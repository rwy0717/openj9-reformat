...
void MM_ScavengerRootClearer::processReferenceList(MM_EnvironmentStandard* env, MM_HeapRegionDescriptorStandard* region,
    omrobjectptr_t headOfList, MM_ReferenceStats* referenceStats)
{
    ...
    while (NULL != referenceObj) {
        ...
        Assert_GC_true_with_message(env, _scavenger->isObjectInNewSpace(referenceObj),
            "Scavenged reference object not in new space: %p\n", referenceObj);
        ...
        if (NULL != referent) {
            ...
            if (_scavenger->isObjectInEvacuateMemory(referent)) {
                uintptr_t referenceObjectType
                    = J9CLASS_FLAGS(J9GC_J9OBJECT_CLAZZ(referenceObj)) & J9_JAVA_CLASS_REFERENCE_MASK;
...
void MM_ScavengerRootClearer::scavengeReferenceObjects(MM_EnvironmentStandard* env, uintptr_t referenceObjectType)
{
    ...
    while (NULL != (region = regionIterator.nextRegion())) {
        if (MEMORY_TYPE_NEW == (region->getTypeFlags() & MEMORY_TYPE_NEW)) {
            ...
            for (uintptr_t i = 0; i < regionExtension->_maxListIndex; i++) {
                /* NOTE: we can't look at the list to determine if there's work to do since another thread may have
                 * already processed it and deleted everything */
...
void MM_ScavengerRootClearer::scavengeUnfinalizedObjects(MM_EnvironmentStandard* env)
{
                        ...
                        while (NULL != object) {
                            ...
                            if (!forwardedHeader.isForwardedPointer()) {
                                ...
                                if (NULL == finalizableObject) {
                                    ...
                                    /* object was not previously forwarded -- it is now finalizable so push it to the
                                     * local buffer */
