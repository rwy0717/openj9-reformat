...
MM_StandardAccessBarrier* MM_StandardAccessBarrier::newInstance(MM_EnvironmentBase* env)
{
    MM_StandardAccessBarrier* barrier;

    barrier = (MM_StandardAccessBarrier*)env->getForge()->allocate(
        sizeof(MM_StandardAccessBarrier), MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
    if (barrier) {
 ...
 */
void MM_StandardAccessBarrier::setDoubleBarrierActiveOnThread(MM_EnvironmentBase* env)
{
    MM_GCExtensions::getExtensions(env)->sATBBarrierRememberedSet->preserveLocalFragmentIndex(
        env, &(((J9VMThread*)env->getLanguageVMThread())->sATBBarrierRememberedSetFragment));
}

void MM_StandardAccessBarrier::initializeForNewThread(MM_EnvironmentBase* env)
{
#if defined(OMR_GC_REALTIME)
    if (_extensions->configuration->isSnapshotAtTheBeginningBarrierEnabled()) {

        _extensions->sATBBarrierRememberedSet->initializeFragment(
            env, &(((J9VMThread*)env->getLanguageVMThread())->sATBBarrierRememberedSetFragment));
...
bool MM_StandardAccessBarrier::initialize(MM_EnvironmentBase* env)
{
...
void MM_StandardAccessBarrier::kill(MM_EnvironmentBase* env)
{
    tearDown(env);
    env->getForge()->free(this);
}

void MM_StandardAccessBarrier::tearDown(MM_EnvironmentBase* env)
{
#if defined(J9VM_GC_GENERATIONAL)
 ...
 */
void MM_StandardAccessBarrier::rememberObjectToRescan(MM_EnvironmentBase* env, J9Object* object)
{
 ...
 */
void MM_StandardAccessBarrier::rememberObjectImpl(MM_EnvironmentBase* env, J9Object* object)
{
    J9VMThread* vmThread = (J9VMThread*)env->getLanguageVMThread();
    _extensions->sATBBarrierRememberedSet->storeInFragment(
        env, &vmThread->sATBBarrierRememberedSetFragment, (UDATA*)object);
}

bool MM_StandardAccessBarrier::preObjectStoreImpl(
 ...
 * old value.
 **/
bool MM_StandardAccessBarrier::preObjectStore(
    J9VMThread* vmThread, J9Object* destObject, fj9object_t* destAddress, J9Object* value, bool isVolatile)
{
 ...
 */
void MM_StandardAccessBarrier::postObjectStore(
    J9VMThread* vmThread, J9Object* destObject, fj9object_t* destAddress, J9Object* value, bool isVolatile)
{
 ...
 */
void MM_StandardAccessBarrier::postObjectStore(
    J9VMThread* vmThread, J9Class* destClass, J9Object** destAddress, J9Object* value, bool isVolatile)
{
...
bool MM_StandardAccessBarrier::preBatchObjectStore(J9VMThread* vmThread, J9Object* destObject, bool isVolatile)
{
...
bool MM_StandardAccessBarrier::preBatchObjectStore(J9VMThread* vmThread, J9Class* destClass, bool isVolatile)
{
    j9object_t destObject = J9VM_J9CLASS_TO_HEAPCLASS(destClass);
 ...
 */
void MM_StandardAccessBarrier::postObjectStoreImpl(J9VMThread* vmThread, J9Object* dstObject, J9Object* srcObject)
{
 ...
 */
void MM_StandardAccessBarrier::preBatchObjectStoreImpl(J9VMThread* vmThread, J9Object* dstObject)
{
#if defined(OMR_GC_MODRON_CONCURRENT_MARK)
    /* Call the concurrent write barrier if required */
    if (_extensions->concurrentMark && (vmThread->privateFlags & J9_PRIVATE_FLAGS_CONCURRENT_MARK_ACTIVE)
 ...
 */
void MM_StandardAccessBarrier::recentlyAllocatedObject(J9VMThread* vmThread, J9Object* dstObject)
{
#if defined(J9VM_GC_GENERATIONAL)
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(vmThread);

    if (extensions->scavengerEnabled && !extensions->isConcurrentScavengerEnabled() && extensions->isOld(dstObject)
        && !extensions->objectModel.isPrimitiveArray(dstObject)) {

        Trc_MM_StandardAccessBarrier_treatObjectAsRecentlyAllocated(vmThread, dstObject);

        if (extensions->objectModel.atomicSwitchReferencedState(
                dstObject, OMR_TENURED_STACK_OBJECT_CURRENTLY_REFERENCED)) {
...
void* MM_StandardAccessBarrier::jniGetPrimitiveArrayCritical(J9VMThread* vmThread, jarray array, jboolean* isCopy)
{
    void* data = NULL;
    ...
    bool alwaysCopyInCritical
        = (javaVM->runtimeFlags & J9_RUNTIME_ALWAYS_COPY_JNI_CRITICAL) == J9_RUNTIME_ALWAYS_COPY_JNI_CRITICAL;
...
void MM_StandardAccessBarrier::jniReleasePrimitiveArrayCritical(
...
const jchar* MM_StandardAccessBarrier::jniGetStringCritical(J9VMThread* vmThread, jstring str, jboolean* isCopy)
{
    jchar* data = NULL;
    J9JavaVM* javaVM = vmThread->javaVM;
    J9InternalVMFunctions* functions = javaVM->internalVMFunctions;
        ...
        if (NULL == data) {
            ...
            if (isCompressed) {
                ...
                if (J9_ARE_ANY_BITS_SET(javaVM->runtimeFlags, J9_RUNTIME_STRING_BYTE_ARRAY)) {
                    // This API determines the stride based on the type of valueObject so in the [B case we must passin
                    // the length in bytes
...
void MM_StandardAccessBarrier::jniReleaseStringCritical(J9VMThread* vmThread, jstring str, const jchar* elems)
{
    J9JavaVM* javaVM = vmThread->javaVM;
    J9InternalVMFunctions* functions = javaVM->internalVMFunctions;
    bool hasVMAccess = false;
    ...
    if (shouldCopy) {
        ...
        /*
         * We have not put assertion here to check is elems valid for str similar way as in
         * jniReleasePrimitiveArrayCritical because of complexity of required code
         */
...
I_32 MM_StandardAccessBarrier::doCopyContiguousBackwardWithReadBarrier(J9VMThread* vmThread,
    J9IndexableObject* srcObject, J9IndexableObject* destObject, I_32 srcIndex, I_32 destIndex, I_32 lengthInSlots)
{
...
I_32 MM_StandardAccessBarrier::doCopyContiguousForwardWithReadBarrier(J9VMThread* vmThread,
    J9IndexableObject* srcObject, J9IndexableObject* destObject, I_32 srcIndex, I_32 destIndex, I_32 lengthInSlots)
{
    fj9object_t* srcSlot = (fj9object_t*)indexableEffectiveAddress(vmThread, srcObject, srcIndex, sizeof(fj9object_t));
    fj9object_t* destSlot
 ...
 */
I_32 MM_StandardAccessBarrier::backwardReferenceArrayCopyIndex(J9VMThread* vmThread, J9IndexableObject* srcObject,
    J9IndexableObject* destObject, I_32 srcIndex, I_32 destIndex, I_32 lengthInSlots)
{
    ...
    if (0 == lengthInSlots) {
        ...
        if (_extensions->isConcurrentScavengerInProgress()) {
             ...
             */
            retValue = doCopyContiguousBackwardWithReadBarrier(
 ...
 * @return ARRAY_COPY_SUCCESSFUL if copy was successful, ARRAY_COPY_NOT_DONE no copy is done
 */
I_32 MM_StandardAccessBarrier::forwardReferenceArrayCopyIndex(J9VMThread* vmThread, J9IndexableObject* srcObject,
    J9IndexableObject* destObject, I_32 srcIndex, I_32 destIndex, I_32 lengthInSlots)
{
    // TODO SATB re-enable opt
    I_32 retValue = ARRAY_COPY_NOT_DONE;
    ...
    if (0 == lengthInSlots) {
        ...
        if (_extensions->isConcurrentScavengerInProgress()) {
             ...
             * For WB same rules apply - just need the final batch barrier.
             */
            retValue = doCopyContiguousForwardWithReadBarrier(
                vmThread, srcObject, destObject, srcIndex, destIndex, lengthInSlots);
        } else
#endif /* OMR_GC_CONCURRENT_SCAVENGER */
        {
...
J9Object* MM_StandardAccessBarrier::asConstantPoolObject(
    J9VMThread* vmThread, J9Object* toConvert, UDATA allocationFlags)
{
...
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
bool MM_StandardAccessBarrier::preMonitorTableSlotRead(J9VMThread* vmThread, j9object_t* srcAddress)
{
    ...
    if (_extensions->scavenger->isObjectInEvacuateMemory(object)) {
        ...
        if (NULL != forwardPtr) {
            /* Object has been copied - ensure the object is fully copied before exposing it, update the slot and
             * return. Slot update needs to be atomic, only if there is a mutator thread racing with a write operation
             * to this same slot. ATM, this barrier is only used to update Monitor table entries, which should not be
             * ever reinitialized by any mutator. Updated can occur, but only by GC threads during STW clearing phase,
             * thus no race with this barrier. */
...
bool MM_StandardAccessBarrier::preMonitorTableSlotRead(J9JavaVM* vm, j9object_t* srcAddress)
{
    omrobjectptr_t object = (omrobjectptr_t)*srcAddress;
...
bool MM_StandardAccessBarrier::preObjectRead(J9VMThread* vmThread, J9Class* srcClass, j9object_t* srcAddress)
{
    ...
    if (_extensions->scavenger->isObjectInEvacuateMemory(object)) {
        ...
        if (NULL != forwardPtr) {
            /* Object has been strictly (remotely) forwarded. Ensure the object is fully copied before exposing it,
            ...
            if (NULL == destinationObjectPtr) {
                ...
                if (forwardPtr != object) {
                    ...
                    MM_AtomicOperations::lockCompareExchange(
                        (uintptr_t*)srcAddress, (uintptr_t)object, (uintptr_t)forwardPtr);
                }
            } else {
                /* Update the slot. copyObject() ensures that the object is fully copied. */
                MM_AtomicOperations::lockCompareExchange(
                    (uintptr_t*)srcAddress, (uintptr_t)object, (uintptr_t)destinationObjectPtr);
            }
        }
    }
...
bool MM_StandardAccessBarrier::preObjectRead(J9VMThread* vmThread, J9Object* srcObject, fj9object_t* srcAddress)
{
    MM_EnvironmentStandard* env = MM_EnvironmentStandard::getEnvironment(vmThread->omrVMThread);
    Assert_GC_true_with_message(env,
        !_extensions->scavenger->isObjectInEvacuateMemory((omrobjectptr_t)srcAddress)
            || _extensions->isScavengerBackOutFlagRaised(),
        "readObject %llx in Evacuate\n", srcAddress);

    /* with volatile cast, ensure that we are really getting a snapshot (instead of the slot being re-read at later
     * points with possibly different values) */
    ...
    if (_extensions->scavenger->isObjectInEvacuateMemory(object)) {
        ...
        if (GLOBAL_READ_BARRIR_STATS_UPDATE_THRESHOLD == env->_scavengerStats._readObjectBarrierUpdate) {
            MM_AtomicOperations::addU64(
                &_extensions->scavengerStats._readObjectBarrierUpdate, GLOBAL_READ_BARRIR_STATS_UPDATE_THRESHOLD);
        ...
        if (NULL != forwardPtr) {
            ...
            if (NULL == destinationObjectPtr) {
                ...
                /* Successfully copied (or copied by another thread). copyObject() ensures that the object is fully
                 * copied. */
                ...
                if (GLOBAL_READ_BARRIR_STATS_UPDATE_THRESHOLD == env->_scavengerStats._readObjectBarrierCopy) {
                    MM_AtomicOperations::addU64(
                        &_extensions->scavengerStats._readObjectBarrierCopy, GLOBAL_READ_BARRIR_STATS_UPDATE_THRESHOLD);
