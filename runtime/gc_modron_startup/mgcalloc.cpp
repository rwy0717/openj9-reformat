...
static void traceAllocateIndexableObject(
    J9VMThread* vmThread, J9Class* clazz, uintptr_t objSize, uintptr_t numberOfIndexedFields);
static void traceAllocateObject(
    J9VMThread* vmThread, J9Class* clazz, uintptr_t objSize, uintptr_t numberOfIndexedFields = 0);
static bool traceObjectCheck(J9VMThread* vmThread);

#define STACK_FRAMES_TO_DUMP 8

/**
 * High level fast path allocate routine (used by VM and JIT) to allocate a single object.  This method does not need to
 *be called with a resolve frame as it cannot cause a GC.  If the attempt at allocation fails, the method will return
 *null and it is the caller's responsibility to call through to the "slow path" J9AllocateObject function after setting
 *up a resolve frame. NOTE:  This function can only be called for instrumentable allocates!
 *
 ...
 */
J9Object* J9AllocateObjectNoGC(J9VMThread* vmThread, J9Class* clazz, uintptr_t allocateFlags)
{
    ...
    if (extensions->instrumentableAllocateHookEnabled || !env->isInlineTLHAllocateEnabled()) {
        /* This function is restricted to only being used for instrumentable allocates so we only need to check that one
         * allocation hook. Note that we can't handle hooked allocates since we might be called without a JIT resolve
         * frame and that is required for us to report the allocation event.
         */
        return NULL;
    }
        ...
        if (mixedOAM.initializeAllocateDescription(env)) {
            ...
            if (NULL != objectPtr) {
                uintptr_t allocatedBytes
                    = env->getExtensions()->objectModel.getConsumedSizeInBytesWithHeader(objectPtr);
    ...
    J9VMThread* vmThread, J9Class* clazz, uintptr_t objSize, uintptr_t numberOfIndexedFields)
{
    if (traceObjectCheck(vmThread)) {
        ...
        if (J9ROMCLASS_IS_ARRAY(romClass)) {
            ...
            Trc_MM_J9AllocateObject_outOfLineObjectAllocation(vmThread, clazz,
                J9UTF8_LENGTH(J9ROMCLASS_CLASSNAME(romClass)), J9UTF8_DATA(J9ROMCLASS_CLASSNAME(romClass)), objSize);
        }
...
/**
 * High level fast path allocate routine (used by VM and JIT) to allocate an indexable object.  This method does not
 *need to be called with a resolve frame as it cannot cause a GC.  If the attempt at allocation fails, the method will
 *return null and it is the caller's responsibility to call through to the "slow path" J9AllocateIndexableObject
 *function after setting up a resolve frame. NOTE:  This function can only be called for instrumentable allocates!
 *
 * @param vmThread The thread requesting the allocation
 * @param clazz The class of the object to be allocated
 ...
 * @return Pointer to the object header, or NULL
 */
J9Object* J9AllocateIndexableObjectNoGC(
    J9VMThread* vmThread, J9Class* clazz, uint32_t numberOfIndexedFields, uintptr_t allocateFlags)
{
    MM_EnvironmentBase* env = MM_EnvironmentBase::getEnvironment(vmThread->omrVMThread);
 ...
 * @return pointer to the object header, or NULL
 */
J9Object* J9AllocateObject(J9VMThread* vmThread, J9Class* clazz, uintptr_t allocateFlags)
{
    MM_EnvironmentBase* env = MM_EnvironmentBase::getEnvironment(vmThread->omrVMThread);
    ...
    if (env->_failAllocOnExcessiveGC && (NULL != objectPtr)) {
        /* If we have garbage collected too much, return NULL as if we had failed to allocate the object (effectively
         * triggering an OOM).
         * TODO: The ordering of this call wrt/ allocation really needs to change - this is just a temporary solution
         * until we reorganize the rest of the code base.
         */
        objectPtr = NULL;
    ...
    if (NULL != objectPtr) {
        ...
        if (OMR_GC_ALLOCATE_OBJECT_INSTRUMENTABLE == (OMR_GC_ALLOCATE_OBJECT_INSTRUMENTABLE & allocateFlags)) {
            TRIGGER_J9HOOK_VM_OBJECT_ALLOCATE_INSTRUMENTABLE(
                vmThread->javaVM->hookInterface, vmThread, objectPtr, sizeInBytesRequired);
        } else {
        ...
        if (!mixedOAM.getAllocateDescription()->isCompletedFromTlh()) {
            TRIGGER_J9HOOK_MM_PRIVATE_NON_TLH_ALLOCATION(
                extensions->privateHookInterface, vmThread->omrVMThread, objectPtr);
        }
        ...
        if ((sizeInBytesRequired >= lowThreshold) && (sizeInBytesRequired <= highThreshold)) {
            Trc_MM_AllocationThreshold_triggerAllocationThresholdEvent(
                vmThread, sizeInBytesRequired, lowThreshold, highThreshold);
            TRIGGER_J9HOOK_VM_OBJECT_ALLOCATE_WITHIN_THRESHOLD(
                vmThread->javaVM->hookInterface, vmThread, objectPtr, sizeInBytesRequired, lowThreshold, highThreshold);
        }
    }
    ...
    if (NULL == objectPtr) {
        ...
        TRIGGER_J9HOOK_MM_PRIVATE_OUT_OF_MEMORY(extensions->privateHookInterface, vmThread->omrVMThread,
            j9time_hires_clock(), J9HOOK_MM_PRIVATE_OUT_OF_MEMORY, memorySpace, memorySpace->getName());
    } else {
 ...
 * @return pointer to the object header, or NULL *
 */
J9Object* J9AllocateIndexableObject(
    J9VMThread* vmThread, J9Class* clazz, uint32_t numberOfIndexedFields, uintptr_t allocateFlags)
{
    MM_EnvironmentBase* env = MM_EnvironmentBase::getEnvironment(vmThread->omrVMThread);
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
    ...
    if (NULL != objectPtr) {
        ...
        uintptr_t highThreshold = extensions->highAllocationThreshold;
        if ((sizeInBytesRequired >= lowThreshold) && (sizeInBytesRequired <= highThreshold)) {
            Trc_MM_AllocationThreshold_triggerAllocationThresholdEventIndexable(
                vmThread, sizeInBytesRequired, lowThreshold, highThreshold);
            TRIGGER_J9HOOK_VM_OBJECT_ALLOCATE_WITHIN_THRESHOLD(
                vmThread->javaVM->hookInterface, vmThread, objectPtr, sizeInBytesRequired, lowThreshold, highThreshold);
        }
 ...
 */
void memoryManagerTLHAsyncCallbackHandler(J9VMThread* vmThread, IDATA handlerKey, void* userData)
{
    ...
    extensions->instrumentableAllocateHookEnabled
        = (0 != J9_EVENT_IS_HOOKED(vm->hookInterface, J9HOOK_VM_OBJECT_ALLOCATE_INSTRUMENTABLE));
...
#if defined(J9VM_GC_THREAD_LOCAL_HEAP)
            extensions->disableInlineCacheForAllocationThreshold
                = (extensions->lowAllocationThreshold < (extensions->tlhMaximumSize + extensions->tlhMinimumSize));
#endif /* defined(J9VM_GC_THREAD_LOCAL_HEAP) */
        } else if (extensions->isSegregatedHeap()) {
#if defined(J9VM_GC_SEGREGATED_HEAP)
            extensions->disableInlineCacheForAllocationThreshold
                = (extensions->lowAllocationThreshold <= J9VMGC_SIZECLASSES_MAX_SMALL_SIZE_BYTES);
    ...
    if (extensions->isStandardGC() || extensions->isVLHGC()) {
        ...
        if (extensions->fvtest_disableInlineAllocation || extensions->instrumentableAllocateHookEnabled
            || extensions->disableInlineCacheForAllocationThreshold) {
            Trc_MM_memoryManagerTLHAsyncCallbackHandler_disableInlineTLHAllocates(vmThread,
                extensions->lowAllocationThreshold, extensions->highAllocationThreshold, extensions->tlhMinimumSize,
                extensions->tlhMaximumSize);
            if (env->isInlineTLHAllocateEnabled()) {
            ...
            Trc_MM_memoryManagerTLHAsyncCallbackHandler_enableInlineTLHAllocates(vmThread,
                extensions->lowAllocationThreshold, extensions->highAllocationThreshold, extensions->tlhMinimumSize,
                extensions->tlhMaximumSize);
            if (!env->isInlineTLHAllocateEnabled()) {
                /* BEN TODO: Collapse the env->enable/disableInlineTLHAllocate with these
        ...
        if (extensions->fvtest_disableInlineAllocation || extensions->instrumentableAllocateHookEnabled
            || extensions->disableInlineCacheForAllocationThreshold) {
            Trc_MM_memoryManagerTLHAsyncCallbackHandler_disableAllocationCache(
                vmThread, extensions->lowAllocationThreshold, extensions->highAllocationThreshold);
            if (allocationInterface->cachedAllocationsEnabled(env)) {
                allocationInterface->disableCachedAllocations(env);
            }
        } else {
            Trc_MM_memoryManagerTLHAsyncCallbackHandler_enableAllocationCache(
                vmThread, extensions->lowAllocationThreshold, extensions->highAllocationThreshold);
            if (!allocationInterface->cachedAllocationsEnabled(env)) {
                allocationInterface->enableCachedAllocations(env);
            }
