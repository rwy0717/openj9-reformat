...
static void hookValidatorVMThreadCrash(
...
initializeMutatorModelJava(J9VMThread* vmThread)
{
    ...
    if (extensions->isStandardGC()) {
        if (extensions->isConcurrentScavengerEnabled()) {
            /* Ensure that newly created threads invoke VM access using slow path, so that the associated hook is
             * invoked.
             * GC will register to the hook to enable local thread resources if a thread happens to be created in a
             * middle of Concurrent Scavenge */
        ...
        // todo: dagar lowTenureAddress, highTenureAddress, heapBaseForBarrierRange0, heapSizeForBarrierRange0 are
        // duplicated
        ...
        /* lowTenureAddress and highTenureAddress are actually supposed to be the low and high addresses of the heap for
         * which card dirtying is required (the JIT uses this as a range check to determine if it needs to dirty a card
         * when writing into an object).  Setting these for Tarok is just a work-around until a more generic solution is
         * implemented
         */
 ...
 */
void cleanupMutatorModelJava(J9VMThread* vmThread)
{
    ...
    if (NULL != env) {
        ...
        if (!IS_STAGE_COMPLETED(loadInfo->completedBits, HEAP_STRUCTURES_FREED)) {
            /* this can only be called if the heap still exists since it will ask the TLH chunk to be abandoned with
             * crashes if the heap is deallocated */
 ...
 */
static void reportPrivateHeapDelete(J9JavaVM* javaVM, void* memorySpace)
{
    ...
    if (modronMemorySpace) {
        if (!(javaVM->runtimeFlags & J9_RUNTIME_SHUTDOWN)) {
            TRIGGER_J9HOOK_MM_PRIVATE_HEAP_DELETE(
                MM_GCExtensions::getExtensions(javaVM)->privateHookInterface, env.getOmrVMThread(), modronMemorySpace);
        }
    }
}
...
void gcCleanupHeapStructures(J9JavaVM* vm)
{
    ...
    /* Flush any allocation contexts so that their memory is returned to the memory spaces before we tear down the
     * memory spaces */
...
j9gc_initialize_heap(J9JavaVM* vm, IDATA* memoryParameterTable, UDATA heapBytesRequested)
{
    ...
    if (NULL == extensions->heap) {
        ...
        if (MM_GCExtensionsBase::HEAP_INITIALIZATION_FAILURE_REASON_NO_ERROR
        ...
        case MM_GCExtensionsBase::HEAP_INITIALIZATION_FAILURE_REASON_CAN_NOT_INSTANTIATE_SPLIT_HEAP_OLD_SPACE:
            splitFailure = j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE,
                J9NLS_GC_FAILED_TO_ALLOCATE_OLD_SPACE, "Failed to allocate old space");
            break;
        case MM_GCExtensionsBase::HEAP_INITIALIZATION_FAILURE_REASON_CAN_NOT_INSTANTIATE_SPLIT_HEAP_NEW_SPACE:
            splitFailure = j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE,
                J9NLS_GC_FAILED_TO_ALLOCATE_NEW_SPACE, "Failed to allocate new space");
            break;
        case MM_GCExtensionsBase::HEAP_INITIALIZATION_FAILURE_REASON_CAN_NOT_INSTANTIATE_SPLIT_HEAP_GEOMETRY:
            splitFailure = j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE,
                J9NLS_GC_SPLIT_HEAP_ENTEXTS_WRONG_ORDER, "Required split heap memory geometry could not be allocated");
            break;
        ...
        case MM_GCExtensionsBase::HEAP_INITIALIZATION_FAILURE_REASON_CAN_NOT_ALLOCATE_LOW_MEMORY_RESERVE: {
            ...
            char* format = (char*)j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE,
        ...
        case MM_GCExtensionsBase::HEAP_INITIALIZATION_FAILURE_REASON_CAN_NOT_INSTANTIATE_HEAP: {
            ...
            char* format = (char*)j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE,
                J9NLS_GC_FAILED_TO_INSTANTIATE_HEAP_SIZE_REQUESTED, "Failed to instantiate heap.  %zu%s requested");
            UDATA formatLength = strlen(format) + 32; /* 2^64 is 20 digits, so have a few extra */
        ...
        case MM_GCExtensionsBase::HEAP_INITIALIZATION_FAILURE_REASON_CAN_NOT_SATISFY_REQUESTED_PAGE_SIZE: {
            ...
            loadInfo->fatalErrorStr = (char*)j9mem_allocate_memory(formatLength, OMRMEM_CATEGORY_MM);
            if (loadInfo->fatalErrorStr) {
                j9str_printf(PORTLIB, loadInfo->fatalErrorStr, formatLength, format, heapSize, heapSizeQualifier,
                    pageSize, pageSizeQualifier);
                loadInfo->loadFlags |= FREE_ERROR_STRING; /* indicates that buffer should be freed later */
            }
        ...
        if (NULL != splitFailure) {
            const char* format = j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE,
            ...
            UDATA formatLength = j9str_printf(
                PORTLIB, NULL, 0, format, splitFailure, newSpaceSize, newQualifier, oldSpaceSize, oldQualifier);

            loadInfo->fatalErrorStr = (char*)j9mem_allocate_memory(formatLength, OMRMEM_CATEGORY_MM);
            if (loadInfo->fatalErrorStr) {
                j9str_printf(PORTLIB, loadInfo->fatalErrorStr, formatLength, format, splitFailure, newSpaceSize,
                    newQualifier, oldSpaceSize, oldQualifier);
                loadInfo->loadFlags |= FREE_ERROR_STRING; /* indicates that buffer should be freed later */
            }
        }
        ...
        if (NULL == loadInfo->fatalErrorStr) {
            loadInfo->fatalErrorStr
                = (char*)j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE,
                    J9NLS_GC_FAILED_TO_INSTANTIATE_HEAP, "Failed to instantiate heap.");
        }
    ...
    extensions->dispatcher = extensions->configuration->createDispatcher(
        &env, (omrsig_handler_fn)vm->internalVMFunctions->structuredSignalHandlerVM, vm, vm->defaultOSStackSize);
    if (NULL == extensions->dispatcher) {
        loadInfo->fatalErrorStr
            = (char*)j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE,
                J9NLS_GC_FAILED_TO_INSTANTIATE_TASK_DISPATCHER, "Failed to instantiate task dispatcher.");
        goto error_no_memory;
    }
    ...
    if (NULL == globalCollector) {
        if (MM_GCExtensionsBase::HEAP_INITIALIZATION_FAILURE_REASON_METRONOME_DOES_NOT_SUPPORT_4BIT_SHIFT
            == extensions->heapInitializationFailureReason) {
            j9nls_printf(
                PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTION_OVERFLOW, displayXmxOrMaxRAMPercentage(memoryParameterTable));
        }
        loadInfo->fatalErrorStr = (char*)j9nls_lookup_message(
            J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE,
            J9NLS_GC_FAILED_TO_INSTANTIATE_GLOBAL_GARBAGE_COLLECTOR, "Failed to instantiate global garbage collector.");
        goto error_no_memory;
    }
    ...
    if (omrthread_monitor_init_with_name(&extensions->gcStatsMutex, 0, "MM_GCExtensions::gcStats")) {
        loadInfo->fatalErrorStr
            = (char*)j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE,
                J9NLS_GC_FAILED_TO_INITIALIZE_MUTEX, "Failed to initialize mutex for GC statistics.");
        goto error_no_memory;
    }
...
jint gcInitializeHeapStructures(J9JavaVM* vm)
{
    ...
    if ((vm->memorySegments = vm->internalVMFunctions->allocateMemorySegmentList(vm, 10, OMRMEM_CATEGORY_VM)) == NULL) {
        loadInfo->fatalErrorStr
            = (char*)j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE,
                J9NLS_GC_FAILED_TO_ALLOCATE_VM_MEMORY_SEGMENTS, "Failed to allocate VM memory segments.");
        goto error;
    }

    /* For now, number of segments to default in pool */
    if ((vm->classMemorySegments = vm->internalVMFunctions->allocateMemorySegmentListWithFlags(
             vm, 10, MEMORY_SEGMENT_LIST_FLAG_SORT, J9MEM_CATEGORY_CLASSES))
        == NULL) {
        loadInfo->fatalErrorStr
            = (char*)j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE,
                J9NLS_GC_FAILED_TO_ALLOCATE_VM_CLASS_MEMORY_SEGMENTS, "Failed to allocate VM class memory segments.");
        goto error;
    }
    ...
    defaultMemorySpace = internalAllocateMemorySpaceWithMaximum(vm, extensions->initialMemorySize,
        extensions->minNewSpaceSize, extensions->newSpaceSize, extensions->maxNewSpaceSize, extensions->minOldSpaceSize,
        extensions->oldSpaceSize, extensions->maxOldSpaceSize, extensions->maxSizeDefaultMemorySpace, 0,
        MEMORY_TYPE_DISCARDABLE);
    if (defaultMemorySpace == NULL) {
        loadInfo->fatalErrorStr
            = (char*)j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE,
                J9NLS_GC_FAILED_TO_ALLOCATE_DEFAULT_MEMORY_SPACE, "Failed to allocate default memory space.");
        goto error;
    }
    ...
    if (!(extensions->finalizeListManager = GC_FinalizeListManager::newInstance(&env))) {
        loadInfo->fatalErrorStr
            = (char*)j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE,
                J9NLS_GC_FAILED_TO_INITIALIZE_FINALIZER_MANAGEMENT, "Failed to initialize finalizer management.");
        goto error;
    }
#endif /* J9VM_GC_FINALIZATION */
 ...
 */
int gcStartupHeapManagement(J9JavaVM* javaVM)
{
 ...
 */
void gcShutdownHeapManagement(J9JavaVM* javaVM)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(javaVM);
...
static UDATA normalizeParameter(
    UDATA parameter, UDATA numerator, UDATA denominator, UDATA max, UDATA min, UDATA roundTo)
{
 ...
 */
static void gcCalculateAndStoreMemoryParameter(MM_GCExtensions* destinationStruct, MM_GCExtensions* sourceStruct,
    const J9GcMemoryParameter* parameterInfo, IDATA* memoryParameters)
{
    if (-1 == memoryParameters[parameterInfo->optionName]) {
        ...
        destinationStruct->*(parameterInfo->fieldOffset)
            = normalizeParameter(sourceStruct->*(parameterInfo->valueBaseOffset), parameterInfo->scaleNumerator,
                parameterInfo->scaleDenominator, parameterInfo->valueMax, parameterInfo->valueMin,
 ...
 * @param memoryParameters array of parameter values
 */
static jint gcInitializeCalculatedValues(J9JavaVM* javaVM, IDATA* memoryParameters)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(javaVM);
    jint result = JNI_OK;
    ...
    const struct J9GcMemoryParameter GCExtensionsParameterTable[] = {
        { &MM_GCExtensions::initialMemorySize, opt_Xms, initialXmsValueMax, initialXmsValueMin,
            &MM_GCExtensions::maxSizeDefaultMemorySpace, 1, 1, extensions->regionSize },
        { &MM_GCExtensions::minNewSpaceSize, opt_Xmns, (UDATA)-1, 2 * MINIMUM_NEW_SPACE_SIZE,
            &MM_GCExtensions::initialMemorySize, 1, 4, 2 * extensions->regionSize },
        { &MM_GCExtensions::newSpaceSize, opt_Xmns, (UDATA)-1, 2 * MINIMUM_NEW_SPACE_SIZE,
            &MM_GCExtensions::initialMemorySize, 1, 4, 2 * extensions->regionSize },
        { &MM_GCExtensions::maxNewSpaceSize, opt_Xmnx, (UDATA)-1, 2 * MINIMUM_NEW_SPACE_SIZE,
            &MM_GCExtensions::maxSizeDefaultMemorySpace, 1, 4, 2 * extensions->regionSize },
        { &MM_GCExtensions::minOldSpaceSize, opt_Xmos, initialXmsValueMax, MINIMUM_OLD_SPACE_SIZE,
            &MM_GCExtensions::initialMemorySize, 3, 4, extensions->regionSize },
        { &MM_GCExtensions::oldSpaceSize, opt_Xmos, initialXmsValueMax, MINIMUM_OLD_SPACE_SIZE,
            &MM_GCExtensions::initialMemorySize, 3, 4, extensions->regionSize },
        { &MM_GCExtensions::maxOldSpaceSize, opt_Xmox, (UDATA)-1, MINIMUM_OLD_SPACE_SIZE,
            &MM_GCExtensions::maxSizeDefaultMemorySpace, 1, 1, extensions->regionSize },
        { &MM_GCExtensions::allocationIncrement, opt_Xmoi, J9_ALLOCATION_INCREMENT_MAX, J9_ALLOCATION_INCREMENT_MIN,
            &MM_GCExtensions::maxSizeDefaultMemorySpace, J9_ALLOCATION_INCREMENT_NUMERATOR,
            J9_ALLOCATION_INCREMENT_DENOMINATOR, extensions->regionSize },
        { &MM_GCExtensions::fixedAllocationIncrement, opt_none, J9_FIXED_SPACE_SIZE_MAX, J9_FIXED_SPACE_SIZE_MIN,
            &MM_GCExtensions::maxSizeDefaultMemorySpace, J9_FIXED_SPACE_SIZE_NUMERATOR, J9_FIXED_SPACE_SIZE_DENOMINATOR,
            extensions->regionSize },
    };

    const IDATA GCExtensionsParameterTableSize
        = (sizeof(GCExtensionsParameterTable) / sizeof(struct J9GcMemoryParameter));
    IDATA tableIndex;
    ...
    if (-1 == memoryParameters[opt_Xmca]) {
        javaVM->ramClassAllocationIncrement
            = normalizeParameter(extensions->maxSizeDefaultMemorySpace, J9_RAM_CLASS_ALLOCATION_INCREMENT_NUMERATOR,
                J9_RAM_CLASS_ALLOCATION_INCREMENT_DENOMINATOR, J9_RAM_CLASS_ALLOCATION_INCREMENT_MAX,
                J9_RAM_CLASS_ALLOCATION_INCREMENT_MIN, J9_RAM_CLASS_ALLOCATION_INCREMENT_ROUND_TO);
    }
    if (-1 == memoryParameters[opt_Xmco]) {
        javaVM->romClassAllocationIncrement
            = normalizeParameter(extensions->maxSizeDefaultMemorySpace, J9_ROM_CLASS_ALLOCATION_INCREMENT_NUMERATOR,
                J9_ROM_CLASS_ALLOCATION_INCREMENT_DENOMINATOR, J9_ROM_CLASS_ALLOCATION_INCREMENT_MAX,
                J9_ROM_CLASS_ALLOCATION_INCREMENT_MIN, J9_ROM_CLASS_ALLOCATION_INCREMENT_ROUND_TO);
    }

    /* Walk the dependency table fixing unspecified parameters to calculated defaults (mapping GCExtensions values to
     * other GCExtensions values) */
    for (tableIndex = 0; tableIndex < GCExtensionsParameterTableSize; tableIndex++) {
        gcCalculateAndStoreMemoryParameter(
            extensions, extensions, &(GCExtensionsParameterTable[tableIndex]), memoryParameters);
    }
 ...
 */
jint setConfigurationSpecificMemoryParameters(J9JavaVM* javaVM, IDATA* memoryParameters, bool flatConfiguration)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(javaVM);
        ...
        if (extensions->initialMemorySize > extensions->maxSizeDefaultMemorySpace) {
            ...
            if (!opt_XmosSet) {
                extensions->oldSpaceSize
                    = MM_Math::roundToFloor(extensions->heapAlignment, extensions->initialMemorySize / 2);
                extensions->oldSpaceSize = MM_Math::roundToFloor(extensions->regionSize, extensions->oldSpaceSize);
                extensions->oldSpaceSize = (extensions->oldSpaceSize >= extensions->absoluteMinimumOldSubSpaceSize)
                    ? extensions->oldSpaceSize
                    : extensions->absoluteMinimumOldSubSpaceSize;
            ...
            if (!flatConfiguration) {
                if (!opt_XmnsSet) {
                    ...
                    extensions->newSpaceSize
                        ...
                        ? extensions->newSpaceSize
                        : (2 * extensions->absoluteMinimumNewSubSpaceSize);
 ...
 */
const char* displayXmnOrXmns(IDATA* memoryParameters)
{
 ...
 */
const char* displayXmnOrXmnx(IDATA* memoryParameters)
{
 ...
 */
const char* displayXmoOrXmos(IDATA* memoryParameters)
{
 ...
 */
const char* displayXmoOrXmox(IDATA* memoryParameters)
{
 ...
 * @param memoryOption1 If non NULL then this and memoryOption2 are used in error messages as a sum (e.g. sum of
 * memoryOption1 and memoryOption2 is ...)
 * @param memoryOption2 See memoryOption1
 *
 * @return JNI_OK on success, JNI_ERR on failure.
 * @note error message is printed out indicating type of failure
 */
static jint gcInitializeXmxXmdxVerification(J9JavaVM* javaVM, IDATA* memoryParameters, bool flatConfiguration,
    UDATA minimumSizeValue, const char* memoryOption1, const char* memoryOption2)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(javaVM);
    ...
    extensions->maxSizeDefaultMemorySpace
        = MM_Math::roundToFloor(extensions->heapAlignment, extensions->maxSizeDefaultMemorySpace);
    extensions->memoryMax = MM_Math::roundToFloor(extensions->regionSize, extensions->memoryMax);
    extensions->maxSizeDefaultMemorySpace
        = MM_Math::roundToFloor(extensions->regionSize, extensions->maxSizeDefaultMemorySpace);
...
_subSpaceTooSmallForValue : {
    ...
    j9nls_printf(
        PORTLIB, J9NLS_ERROR, J9NLS_GC_SUBSPACE_TOO_SMALL_FOR_VALUE, memoryOption1, minimumSizeValue, qualifier);
    return JNI_ERR;
}
...
_subSpaceTooLarge:
    if (NULL == memoryOption2) {
        ...
        j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_SUBSPACE_SUM_OF_TOO_LARGE, memoryOption1, memoryOption2,
            subSpaceTooLargeOption);
    }
    return JNI_ERR;
 ...
 * @note error message is printed out indicating type of failure
 */
jint independentMemoryParameterVerification(J9JavaVM* javaVM, IDATA* memoryParameters, bool flatConfiguration)
{
    PORT_ACCESS_FROM_JAVAVM(javaVM);
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(javaVM);
    ...
    if (opt_XmsSet) {
        ...
        if (flatConfiguration) {
            ...
            /* Gencon required at least three regions to start with (one for Tenure and two for Nursery - one for each
             * half) */
     ...
     */
    if (gcInitializeXmxXmdxVerification(
            javaVM, memoryParameters, flatConfiguration, XmsMinimumValue, memoryOption, memoryOption2)
        != JNI_OK) {
        return JNI_ERR;
    }
    ...
    if (extensions->allocationIncrementSetByUser) {
        extensions->allocationIncrement
            = MM_Math::roundToCeiling(extensions->heapAlignment, extensions->allocationIncrement);
        extensions->allocationIncrement
            = MM_Math::roundToCeiling(extensions->regionSize, extensions->allocationIncrement);
    }

#if defined(J9VM_GC_COMPRESSED_POINTERS)
    /* Align the Xmcrs if necessary */
    extensions->suballocatorInitialSize
        = MM_Math::roundToCeiling(SUBALLOCATOR_ALIGNMENT, extensions->suballocatorInitialSize);
#endif /* defined(J9VM_GC_COMPRESSED_POINTERS) */
...
_subSpaceTooSmallForValue : {
    ...
    qualifiedSize(&minimumSizeValue, &qualifier);
    j9nls_printf(
        PORTLIB, J9NLS_ERROR, J9NLS_GC_SUBSPACE_TOO_SMALL_FOR_VALUE, memoryOption, minimumSizeValue, qualifier);
    return JNI_ERR;
}
...
_subSpaceTooLarge:
    if (NULL == memoryOption2) {
        ...
        j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_SUBSPACE_SUM_OF_TOO_LARGE, memoryOption, memoryOption2,
            subSpaceTooLargeOption);
    }
    return JNI_ERR;
 ...
 */
static bool isLessThanEqualOrUnspecifiedAgainstFixed(MM_UserSpecifiedParameterUDATA* toTest, UDATA maximum)
{
    bool isSafe = true;
 ...
 */
static bool isLessThanEqualOrUnspecifiedAgainstOption(
    MM_UserSpecifiedParameterUDATA* minimum, MM_UserSpecifiedParameterUDATA* maximum)
{
    bool isSafe = true;
 ...
 * @note error message is printed out indicating type of failure
 */
jint combinationMemoryParameterVerification(J9JavaVM* javaVM, IDATA* memoryParameters, bool flatConfiguration)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(javaVM);
    ...
    if (flatConfiguration) {
        ...
        UDATA newSpaceSizeMinimum
            = opt_XmnsSet ? extensions->minNewSpaceSize : 2 * extensions->absoluteMinimumNewSubSpaceSize;
        UDATA oldSpaceSizeMinimum
            = opt_XmosSet ? extensions->minOldSpaceSize : extensions->absoluteMinimumOldSubSpaceSize;
        ...
        case XMOS_XMNS:
            ...
            if (opt_XmsSet
                && ((extensions->oldSpaceSize + extensions->newSpaceSize) != extensions->initialMemorySize)) {
                memoryOption = displayXmoOrXmos(memoryParameters);
                memoryOption2 = displayXmnOrXmns(memoryParameters);
                subSpaceTooLargeOption = displayXmsOrInitialRAMPercentage(memoryParameters);
        ...
        case XMOS:
            ...
            if (opt_XmsSet) {
                ...
                if (extensions->oldSpaceSize < (6 * extensions->regionSize)) {
                    /*
                     * Minimum initial heap size to provide 75/25 split is 8 regions, six for Tenure and two for
                     * Nursery. If Tenure Initial size is smaller then six regions it is not large enough to provide
                     * 75/25 proportion So set initial Nursery size to minimum (two regions) and use reminder for Tenure
                     */
                    candidateXmnsValue = extensions->regionSize * 2;
                } else {
                    /* Make Xmns 1/4 of the Xms value, i.e. 1/3 of Xmos. Xms will be reset */
                    candidateXmnsValue
                        = MM_Math::roundToFloor(extensions->heapAlignment * 2, extensions->oldSpaceSize / 3);
        ...
        case NONE:
            ...
            if (opt_XmsSet) {
                if (extensions->initialMemorySize < (8 * extensions->regionSize)) {
                    ...
                    candidateXmnsValue
                        = MM_Math::roundToFloor(extensions->heapAlignment * 2, extensions->initialMemorySize * 1 / 4);
                    candidateXmnsValue = MM_Math::roundToFloor(extensions->regionSize * 2, candidateXmnsValue);
                }
        ...
        if (extensions->maxSizeDefaultMemorySpace < combinedXmosXmnsSize) {
            if (!opt_XmdxSet) {
                ...
                memoryOption = "-Xmdx";
                minimumSizeValue = extensions->absoluteMinimumOldSubSpaceSize
                    + (2 * extensions->absoluteMinimumNewSubSpaceSize); /* smallest configuration */
                goto _subSpaceTooSmallForValue;
            }
        }
    ...
    if (extensions->enableSplitHeap) {
        if (opt_XmoxSet) {
            ...
            if (opt_XmnxSet) {
                ...
                /* neither were set so just correct the old space size since it will be set too big for us to use
                 * correctly */
                extensions->maxOldSpaceSize = extensions->memoryMax - extensions->maxNewSpaceSize;
            }
        }
...
#if defined(J9VM_GC_VLHGC)
    {
        ...
        if (!isLessThanEqualOrUnspecifiedAgainstOption(
                &extensions->userSpecifiedParameters._Xmns, &extensions->userSpecifiedParameters._Xmnx)) {
            memoryOption = "-Xmnx";
        ...
        /* eden size can not be smaller than 2 * region size
           ...
           2, if eden size is smaller than 2 times of region size, it would cause wrong calculating taxationThreshold
           for GMP and PGC(the PGC after GMP) */
        ...
        /* since our current implementation of Eden sizing only uses these values for end-points in our sizing
         * interpolation, they don't need to be rounded */
...
_subSpaceCombinationTooLarge:
    j9nls_printf(
        PORTLIB, J9NLS_ERROR, J9NLS_GC_SUBSPACE_SUM_OF_TOO_LARGE, memoryOption, memoryOption2, subSpaceTooLargeOption);
    return JNI_ERR;
...
_subSpaceCombinationNotEqual:
    j9nls_printf(
        PORTLIB, J9NLS_ERROR, J9NLS_GC_SUBSPACE_SUM_OF_NOT_EQUAL, memoryOption, memoryOption2, subSpaceTooLargeOption);
    return JNI_ERR;

_subSpaceCombinationTooSmall:
    j9nls_printf(
        PORTLIB, J9NLS_ERROR, J9NLS_GC_SUBSPACE_SUM_OF_TOO_SMALL, memoryOption, memoryOption2, subSpaceTooSmallOption);
    return JNI_ERR;
...
_subSpaceCombinationTooClose:
    j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTION_MINIMUM_DIFFERENCE, memoryOption, minValue,
        memoryOptionMinimumDifference, memoryOption2, maxValue);
    return JNI_ERR;
 ...
 * @return JNI_OK if OK
 */
jint gcCalculateMemoryParameters(J9JavaVM* javaVM, IDATA* memoryParameters, bool flatConfiguration)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(javaVM);
    jint result;
    ...
    assume0(
        extensions->minOldSpaceSize >= OMR_MAX(extensions->absoluteMinimumOldSubSpaceSize, extensions->heapAlignment));
#if defined(DEBUG)
    if (!flatConfiguration) {
        assume0(extensions->minNewSpaceSize
            >= 2 * OMR_MAX(extensions->absoluteMinimumNewSubSpaceSize, extensions->heapAlignment));
    }
#endif /* DEBUG */
    ...
    /* initialize the dynamicMaxSoftReferenceAge to the maxSoftReferenceAge since the first GC starts with reference age
     * at its maximum */
 ...
 * @return JNI_OK if OK
 */
jint gcInitializeVerification(J9JavaVM* javaVM, IDATA* memoryParameters, bool flatConfiguration)
{
    ...
    if (extensions->scavengerScanCacheMaximumSize < extensions->scavengerScanCacheMinimumSize) {
        PORT_ACCESS_FROM_JAVAVM(javaVM);
        j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_MUST_BE_NO_GREATER_THAN, "-XXgc:scanCacheMinimumSize",
            "-XXgc:scanCacheMaximumSize");
        return JNI_ERR;
    }
    /* make sure scavengerScanCacheMinimumSize and scavengerScanCacheMaximumSize are properly aligned */
    extensions->scavengerScanCacheMaximumSize
        = MM_Math::roundToCeiling(extensions->tlhMinimumSize, extensions->scavengerScanCacheMaximumSize);
 ...
 */
bool reduceXmxValueForHeapInitialization(J9JavaVM* javaVM, IDATA* memoryParameterTable, UDATA memoryMinimum)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(javaVM);
    ...
    extensions->memoryMax
        = (extensions->memoryMax / DEFAULT_XMX_REDUCTION_DENOMINATOR) * DEFAULT_XMX_REDUCTION_NUMERATOR;
    extensions->memoryMax = MM_Math::roundToFloor(extensions->heapAlignment, extensions->memoryMax);
    extensions->memoryMax = MM_Math::roundToFloor(extensions->regionSize, extensions->memoryMax);
 ...
 */
void warnIfPageSizeNotSatisfied(J9JavaVM* vm, MM_GCExtensions* extensions)
{
    PORT_ACCESS_FROM_JAVAVM(vm);
    ...
    if ((extensions->heap->getPageSize() != extensions->requestedPageSize) && extensions->largePageWarnOnError) {
        ...
        j9nls_printf(PORTLIB, J9NLS_INFO, J9NLS_GC_OPTIONS_XLP_PAGE_NOT_AVAILABLE_WARN, requestedSize,
            requestedSizeQualifier, actualSize, actualSizeQualifier);
    }
}
 ...
 */
void setDefaultConfigOptions(
    MM_GCExtensions* extensions, bool scavenge, bool concurrentMark, bool concurrentSweep, bool largeObjectArea)
{
#if defined(J9VM_GC_MODRON_SCAVENGER)
...
void setConfigOptionsForNoGc(MM_GCExtensions* extensions)
{
 ...
 */
MM_Configuration* configurateGCWithPolicyAndOptionsStandard(MM_EnvironmentBase* env)
{
    ...
    if (extensions->scavengerEnabled) {
        ...
        if (!extensions->concurrentSweep) {
             ...
             * 	(this is fast check without full parameters analysis for Concurrent Scavenger prototype, it is
             * recommended to provide -Xmn explicitly)
             * - select Concurrent Scavenger Page size based at estimated Nursery size: if should be rounded up to next
             * power of 2 but not smaller then 32M
            ...
            if (extensions->isConcurrentScavengerHWSupported()) {
                ...
                /*
                 * correctness of -Xmn* values will be analyzed later on with with initialization error in case of bad
                 * combination so assume here all of them are correct, just check none of them is larger then entire
                 * heap size
                 */
                if (extensions->userSpecifiedParameters._Xmn._wasSpecified) {
                    ...
                    /* maximum Nursery size was set in -Xmns, it is related in case if it is larger then default maximum
                     * Nursery size */
                ...
                if (extensions->isDebugConcurrentScavengerPageAlignment()) {
                    omrtty_printf("Nursery size early projection 0x%zx, Concurrent Scavenger Page size 0x%zx, Section "
 ...
 * @return pointer to created configuration or NULL if failed
 */
MM_Configuration* configurateGCWithPolicyAndOptions(OMR_VM* omrVM)
{
    MM_Configuration* result = NULL;
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(omrVM);
    MM_EnvironmentBase env(omrVM);
...
jint gcInitializeDefaults(J9JavaVM* vm)
{
    ...
    if (!memoryParameterTable) {
        loadInfo->fatalErrorStr
            = (char*)j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE,
                J9NLS_GC_FAILED_TO_INITIALIZE_OUT_OF_MEMORY, "Failed to initialize, out of memory.");
        goto error;
    }
    ...
    /* enable estimateFragmentation for all GCs as default for java, but not the estimated result would not affect
     * concurrentgc kickoff by default */
    ...
    if (JNI_OK != gcParseCommandLineAndInitializeWithValues(vm, memoryParameterTable)) {
        loadInfo->fatalErrorStr
            = (char*)j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE,
                J9NLS_GC_FAILED_TO_INITIALIZE_PARSING_COMMAND_LINE, "Failed to initialize, parsing command line.");
        goto error;
    }

    if ((-1 == memoryParameterTable[opt_Xms]) && (-1 != memoryParameterTable[opt_initialRAMPercent])) {
        extensions->initialMemorySize
            = (uintptr_t)(((double)extensions->usablePhysicalMemory / 100.0) * extensions->initialRAMPercent);
    ...
    if ((-1 == memoryParameterTable[opt_Xmx]) && (-1 != memoryParameterTable[opt_maxRAMPercent])) {
        extensions->memoryMax
            = (uintptr_t)(((double)extensions->usablePhysicalMemory / 100.0) * extensions->maxRAMPercent);
    ...
    if (gc_policy_metronome == extensions->configurationOptions._gcPolicy) {
        ...
        vm->segregatedAllocationCacheSize
            = (J9VMGC_SIZECLASSES_NUM_SMALL + 1) * sizeof(J9VMGCSegregatedAllocationCacheEntry);

        vm->realtimeSizeClasses
            = (J9VMGCSizeClasses*)j9mem_allocate_memory(realtimeSizeClassesAllocationSize, OMRMEM_CATEGORY_VM);
    ...
    vm->vmThreadSize
        = J9_VMTHREAD_SEGREGATED_ALLOCATION_CACHE_OFFSET + vm->segregatedAllocationCacheSize + sizeof(OMR_VMThread);
    ...
    if (NULL == extensions->configuration) {
        loadInfo->fatalErrorStr
            = (char*)j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE,
                J9NLS_GC_FAILED_TO_INITIALIZE, "Failed to initialize.");
        goto error;
    }

    extensions->trackMutatorThreadCategory
        = J9_ARE_NO_BITS_SET(vm->extendedRuntimeFlags, J9_EXTENDED_RUNTIME_REDUCE_CPU_MONITOR_OVERHEAD);
    ...
    while (true) {
        ...
        if (JNI_OK
            != gcInitializeXmxXmdxVerification(
                   vm, memoryParameterTable, flatConfiguration, minimumVMSize, NULL, NULL)) {
            loadInfo->fatalErrorStr
                = (char*)j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE,
                    J9NLS_GC_FAILED_TO_INITIALIZE, "Failed to initialize.");
            goto error;
...
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
static void hookAcquireVMAccess(J9HookInterface** hook, UDATA eventNum, void* voidEventData, void* userData)
{
...
jint triggerGCInitialized(J9VMThread* vmThread)
{
    ...
    TRIGGER_J9HOOK_MM_OMR_INITIALIZED(extensions->omrHookInterface, vmThread->omrVMThread, j9time_hires_clock(),
        j9gc_get_gcmodestring(vm), 0, /* unused */
        j9gc_get_maximum_heap_size(vm), j9gc_get_initial_heap_size(vm), j9sysinfo_get_physical_memory(),
        j9sysinfo_get_number_CPUs_by_type(J9PORT_CPU_ONLINE), extensions->gcThreadCount,
        j9sysinfo_get_CPU_architecture(), j9sysinfo_get_OS_type(), j9sysinfo_get_OS_version(),
        extensions->accessBarrier->compressedPointersShift(), beatMicro, timeWindowMicro, targetUtilizationPercentage,
        gcInitialTrigger, headRoom, extensions->heap->getPageSize(),
        getPageTypeString(extensions->heap->getPageFlags()), extensions->requestedPageSize,
        getPageTypeString(extensions->requestedPageFlags), numaNodes, regionSize, regionCount, arrayletLeafSize);
...
static void hookVMRegistrationEvent(J9HookInterface** hook, UDATA eventNum, void* voidEventData, void* userData)
{
...
static bool gcInitializeVMHooks(MM_GCExtensionsBase* extensions)
{
    ...
    if (NULL == vmHookInterface) {
        result = false;
    } else if ((*vmHookInterface)
                   ->J9HookRegisterWithCallSite(
                       vmHookInterface, J9HOOK_VM_THREAD_CRASH, hookValidatorVMThreadCrash, OMR_GET_CALLSITE(), NULL)) {
        result = false;
    } else if ((*vmHookInterface)
                   ->J9HookRegisterWithCallSite(vmHookInterface, J9HOOK_REGISTRATION_EVENT, hookVMRegistrationEvent,
                       OMR_GET_CALLSITE(), javaVM)) {
        result = false;
    }
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
        ...
        && (*vmHookInterface)
               ->J9HookRegisterWithCallSite(
                   vmHookInterface, J9HOOK_VM_ACQUIREVMACCESS, hookAcquireVMAccess, OMR_GET_CALLSITE(), NULL)) {
        result = false;
    }
#endif /* OMR_GC_CONCURRENT_SCAVENGER */
...
static void gcCleanupVMHooks(MM_GCExtensionsBase* extensions)
{
    ...
    if (NULL != vmHookInterface) {
        ...
        (*vmHookInterface)
            ->J9HookUnregister(vmHookInterface, J9HOOK_REGISTRATION_EVENT, hookVMRegistrationEvent, javaVM);
    }
}
