...
static const char* optionGroupClassGC[] = { "-Xnoclassgc", "-Xclassgc", "-Xalwaysclassgc", NULL };
/**
 * @}
 */
 ...
 * @{
 */
enum { optionGroupConMeter_index_soa = 0, optionGroupConMeter_index_loa, optionGroupConMeter_index_dynamic };

static const char* optionGroupConMeter[] = { "-Xconmeter:soa", "-Xconmeter:loa", "-Xconmeter:dynamic", NULL };
/**
 * @}
 */
...
typedef enum { PARSING_FIRST_OPTION = 1, PARSING_OPTION, PARSING_COMMA, PARSING_ERROR } parsingStates;
/**
 * @}
 */
 ...
 */
static IDATA option_set(J9JavaVM* vm, const char* option, IDATA match)
{
 ...
 *
 * @return OPTION_OK if option is found and consumed or option not present, OPTION_MALFORMED  if the option was
 * malformed, OPTION_OVERFLOW if the option overflowed.
 ...
 */
static IDATA option_set_to_opt(J9JavaVM* vm, const char* option, IDATA* optionIndex, IDATA match, UDATA* address)
{
    IDATA element;
 ...
 * @note optionIndex contains position of argument on command line if success returned, else -1
 */
static IDATA option_set_to_opt_percent(
    J9JavaVM* vm, const char* option, IDATA* optionIndex, IDATA match, UDATA* address)
{
    IDATA element;
    IDATA returnCode = OPTION_OK;
    UDATA value;
 ...
 * @note optionIndex contains position of argument on command line if success returned, else -1
 */
static IDATA option_set_to_opt_integer(
    J9JavaVM* vm, const char* option, IDATA* optionIndex, IDATA match, UDATA* address)
{
    IDATA element;
    IDATA returnCode = OPTION_OK;
 ...
 * @note optionIndex contains position of argument on command line if success returned, else -1
 */
static IDATA option_set_to_opt_double(
    J9JavaVM* vm, const char* option, IDATA* optionIndex, IDATA match, double* address)
{
    IDATA element = -1;
    IDATA returnCode = OPTION_OK;
    double value = 0.0;
 ...
 * @note if successful consumed optionIndex contains 0 if the rightmost argument is option1, 1 if the rightmost argument
 * is option2, else -1
 */
static IDATA option_set_pair(J9JavaVM* vm, const char* option1, const char* option2, IDATA* optionPairIndex)
{
    IDATA index1, index2;
 ...
 */
static IDATA option_set_group(J9JavaVM* vm, const char** optionGroup, IDATA* optionGroupIndex)
{
    ...
    *optionGroupIndex = -1;
    ...
    while (NULL != *optionGroup) {
        /* ensure that we skip over entries in the table which aren't supported on this configuration (but preserved for
         * shape consistency) */
 ...
 */
static XlpErrorState xlpSubOptionsParser(J9JavaVM* vm, IDATA xlpIndex, XlpError* xlpError, UDATA* requestedPageSize,
    UDATA* requestedPageFlags, bool* strict, bool* warn)
{
 ...
 */
static bool gcParseXlpOption(J9JavaVM* vm)
{
    ...
    if ((-1 != xlpObjectHeapIndex) && (xlpObjectHeapIndex > xlpMemIndex)) {
         ...
         */
        xlpErrorState = xlpSubOptionsParser(vm, xlpObjectHeapIndex, &xlpError, &requestedPageSize, &requestedPageFlags,
            &extensions->largePageFailOnError, &extensions->largePageWarnOnError);
    ...
    /* If a valid -Xlp<size> or -Xlp:objectheap:pagesize=<size> is present, check if the requested page size is
     * supported */
    ...
    if ((-1 != xlpMemIndex) || (-1 != xlpObjectHeapIndex)) {
        ...
        if (!isRequestedSizeSupported) {
            ...
            if (NULL == oldPageType) {
                if (NULL == newPageType) {
                    j9nls_printf(PORTLIB, J9NLS_INFO, J9NLS_GC_OPTIONS_LARGE_PAGE_SIZE_NOT_SUPPORTED, oldSize,
                        oldQualifier, newSize, newQualifier);
                } else {
                    j9nls_printf(PORTLIB, J9NLS_INFO, J9NLS_GC_OPTIONS_LARGE_PAGE_SIZE_NOT_SUPPORTED_WITH_NEW_PAGETYPE,
                        oldSize, oldQualifier, newSize, newQualifier, newPageType);
                }
            } else {
                if (NULL == newPageType) {
                    j9nls_printf(PORTLIB, J9NLS_INFO,
                        J9NLS_GC_OPTIONS_LARGE_PAGE_SIZE_NOT_SUPPORTED_WITH_REQUESTED_PAGETYPE, oldSize, oldQualifier,
                        oldPageType, newSize, newQualifier);
                } else {
                    j9nls_printf(PORTLIB, J9NLS_INFO, J9NLS_GC_OPTIONS_LARGE_PAGE_SIZE_NOT_SUPPORTED_WITH_PAGETYPE,
    ...
    if (-1 != xlpGCIndex) {
         ...
         */
        xlpErrorState
            = xlpSubOptionsParser(vm, xlpGCIndex, &xlpError, &gcmetadataPageSize, &gcmetadataPageFlags, NULL, NULL);
        ...
        if (!found) {
            ...
            j9nls_printf(PORTLIB, J9NLS_INFO, J9NLS_GC_OPTIONS_XLP_PAGE_NOT_SUPPORTED, "gcmetadata", oldSize,
...
_reportXlpError:
    ...
    if (XLP_NO_ERROR != xlpErrorState) {
        ...
        case XLP_OPTION_NOT_SUPPORTED:
            j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_SYSTEM_CONFIG_OPTION_NOT_SUPPORTED,
                xlpError.xlpOptionErrorString);
            break;
        case XLP_MEM_NAN:
        ...
        case XLP_PAGE_SIZE_INCORRECT:
            j9nls_printf(
                PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INCORRECT_MEMORY_SIZE, "-Xlp:objectheap:pagesize=<size>");
            break;
        case XLP_INCOMPLETE_OPTION:
            j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_XLP_INCOMPLETE_OPTION, xlpError.xlpOptionErrorString,
                xlpError.xlpMissingOptionString);
            break;
        case XLP_LARGE_PAGE_NOT_SUPPORTED: {
            ...
            if (NULL == pageType) {
                ...
                j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_XLP_LARGE_PAGE_NOT_SUPPORTED_WITH_PAGETYPE,
                    pageSize, qualifier, pageType);
            }

            break;
        }
        case XLP_PARAMETER_NOT_RECOGNIZED:
            j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_XLP_UNRECOGNIZED_OPTION,
                xlpError.xlpOptionErrorStringSize, xlpError.xlpOptionErrorString);
            break;
        default:
 ...
 */
static UDATA gcParseSovereignArguments(J9JavaVM* vm)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(vm);
    IDATA result = 0;
    UDATA inputValue = 0;
    ...
    result = option_set_to_opt_percent(
        vm, "-Xminf", &index, EXACT_MEMORY_MATCH, &extensions->heapFreeMinimumRatioMultiplier);
    if (OPTION_OK != result) {
        if (OPTION_MALFORMED == result) {
    ...
    result = option_set_to_opt_percent(
        vm, "-Xmaxf", &index, EXACT_MEMORY_MATCH, &extensions->heapFreeMaximumRatioMultiplier);
    if (OPTION_OK != result) {
        if (OPTION_MALFORMED == result) {
    ...
    result = option_set_to_opt_percent(
        vm, "-Xmaxt", &index, EXACT_MEMORY_MATCH, &extensions->heapExpansionGCTimeThreshold);
    if (OPTION_OK != result) {
        if (OPTION_MALFORMED == result) {
    ...
    result = option_set_to_opt_percent(
        vm, "-Xmint", &index, EXACT_MEMORY_MATCH, &extensions->heapContractionGCTimeThreshold);
    if (OPTION_OK != result) {
        if (OPTION_MALFORMED == result) {
    ...
    if (-1 != FIND_ARG_IN_VMARGS(EXACT_MEMORY_MATCH, "-Xgcworkpackets", NULL)) {
        result = option_set_to_opt_integer(
            vm, "-Xgcworkpackets", &index, EXACT_MEMORY_MATCH, &extensions->workpacketCount);
        if (OPTION_OK != result) {
            if (OPTION_MALFORMED == result) {
    ...
    result = option_set_to_opt_integer(
        vm, "-Xconcurrentbackground", &index, EXACT_MEMORY_MATCH, &extensions->concurrentBackground);
    if (OPTION_OK != result) {
        if (OPTION_MALFORMED == result) {
    ...
    result
        = option_set_to_opt_integer(vm, "-Xconcurrentlevel", &index, EXACT_MEMORY_MATCH, &extensions->concurrentLevel);
    if (OPTION_OK != result) {
        if (OPTION_MALFORMED == result) {
    ...
    if (NULL != optionFound) {
        result
            = option_set_to_opt_integer(vm, optionFound, &index, EXACT_MEMORY_MATCH, &extensions->maxSoftReferenceAge);
        if (OPTION_OK != result) {
            if (OPTION_MALFORMED == result) {
    ...
    if (-1 != FIND_ARG_IN_VMARGS(EXACT_MEMORY_MATCH, "-XX:stringTableListToTreeThreshold=", NULL)) {
        UDATA threshold = 0;
        result = option_set_to_opt_integer(
            vm, "-XX:stringTableListToTreeThreshold=", &index, EXACT_MEMORY_MATCH, &threshold);
        if (OPTION_OK != result) {
            if (OPTION_MALFORMED == result) {
                j9nls_printf(
                ...
                j9nls_printf(
                    PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_VALUE_OVERFLOWED, "-XX:stringTableListToTreeThreshold=");
            }
            goto _error;
        }
...
#define FOUR_GB ((UDATA)4 * 1024 * 1024 * 1024)
    ...
    result = option_set_to_opt(
        vm, OPT_XMNS, &index, EXACT_MEMORY_MATCH, &extensions->userSpecifiedParameters._Xmns._valueSpecified);
    if (OPTION_OK != result) {
        goto _error;
    }
    ...
    result = option_set_to_opt(
        vm, OPT_XMNX, &index, EXACT_MEMORY_MATCH, &extensions->userSpecifiedParameters._Xmnx._valueSpecified);
    if (OPTION_OK != result) {
        goto _error;
    }
    ...
    result = option_set_to_opt(
        vm, OPT_XMN, &index, EXACT_MEMORY_MATCH, &extensions->userSpecifiedParameters._Xmn._valueSpecified);
    if (OPTION_OK != result) {
        goto _error;
    }
    ...
    if (memoryParameters[opt_maxRAMPercent] != -1) {
        if ((extensions->maxRAMPercent < 0.0) || (extensions->maxRAMPercent > 100.0)) {
            j9nls_printf(
                PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_PERCENT_OUT_OF_RANGE, "-XX:MaxRAMPercentage", 0.0, 100.0);
            return JNI_EINVAL;
        }
    }

    result = option_set_to_opt_double(
        vm, OPT_XXINITIALRAMPERCENT, &index, EXACT_MEMORY_MATCH, &extensions->initialRAMPercent);
    if (OPTION_OK != result) {
        goto _error;
    }
    ...
    if (memoryParameters[opt_initialRAMPercent] != -1) {
        if ((extensions->initialRAMPercent < 0.0) || (extensions->initialRAMPercent > 100.0)) {
            j9nls_printf(
                PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_PERCENT_OUT_OF_RANGE, "-XX:InitialRAMPercentage", 0.0, 100.0);
            return JNI_EINVAL;
        }
    }

    /* Parse the option to disable NUMA-awareness.  This is parsed on all platforms but only Balanced currently does
     * anything with the result. Note that this option does NOT disable heap interleaving since that is considered a
     * harmless and "passive" optimization.
     */
