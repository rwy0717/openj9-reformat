    ...
    while (scan_start < scan_limit) {
        ...
        if (try_scan(&scan_start, "minimumFreeEntrySize=")) {
            if (!scan_udata_memory_size_helper(
                    vm, &scan_start, &(extensions->minimumFreeEntrySize), "minimumFreeEntrySize=")) {
                returnValue = JNI_EINVAL;
                break;
            }
            continue;
        ...
        if (try_scan(&scan_start, "traceCostToCheckYield=")) {
            if (!scan_udata_memory_size_helper(
                    vm, &scan_start, &(extensions->traceCostToCheckYield), "traceCostToCheckYield=")) {
                returnValue = JNI_EINVAL;
                break;
            }
            continue;
        ...
        if (try_scan(&scan_start, "sweepCostToCheckYield=")) {
            if (!scan_udata_memory_size_helper(
                    vm, &scan_start, &(extensions->sweepCostToCheckYield), "sweepCostToCheckYield=")) {
                returnValue = JNI_EINVAL;
                break;
            }
            continue;
        ...
        if (try_scan(&scan_start, "allocationTrackerMaxTotalError=")) {
            if (!scan_udata_memory_size_helper(
                    vm, &scan_start, &extensions->allocationTrackerMaxTotalError, "allocationTrackerMaxTotalError=")) {
                extensions->allocationTrackerFlushThreshold
                    = OMR_MIN(extensions->allocationTrackerMaxThreshold, extensions->allocationTrackerMaxTotalError);
                returnValue = JNI_EINVAL;
                break;
            }
            continue;
        ...
        if (try_scan(&scan_start, "allocationTrackerMaxThreshold=")) {
            if (!scan_udata_memory_size_helper(
                    vm, &scan_start, &extensions->allocationTrackerMaxThreshold, "allocationTrackerMaxThreshold=")) {
                extensions->allocationTrackerFlushThreshold
                    = OMR_MIN(extensions->allocationTrackerMaxThreshold, extensions->allocationTrackerMaxTotalError);
                returnValue = JNI_EINVAL;
                break;
        ...
        if (try_scan(&scan_start, "allocationContextCount=")) {
            if (!scan_udata_helper(
                    vm, &scan_start, &(extensions->managedAllocationContextCount), "allocationContextCount=")) {
                returnValue = JNI_EINVAL;
                break;
            }
            ...
            if (extensions->managedAllocationContextCount <= 0) {
                j9nls_printf(
                    PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_VALUE_MUST_BE_ABOVE, "allocationContextCount=", (UDATA)0);
                returnValue = JNI_EINVAL;
                break;
            }
        ...
        if (try_scan(&scan_start, "forcedShiftingCompressionAmount=")) {
            ...
            if (!scan_udata_helper(vm, &scan_start, &(extensions->forcedShiftingCompressionAmount),
        ...
        if (try_scan(&scan_start, "tarokNurseryMaxAge=")) {
            if (!scan_udata_memory_size_helper(
                    vm, &scan_start, &(extensions->tarokNurseryMaxAge._valueSpecified), "tarokNurseryMaxAge=")) {
                returnValue = JNI_EINVAL;
                break;
            }
        ...
        if (try_scan(&scan_start, "tarokRememberedSetCardListMaxSize=")) {
            if (!scan_udata_memory_size_helper(vm, &scan_start, &(extensions->tarokRememberedSetCardListMaxSize),
                    "tarokRememberedSetCardListMaxSize=")) {
                returnValue = JNI_EINVAL;
                break;
            }
        ...
        if (try_scan(&scan_start, "tarokRememberedSetCardListSize=")) {
            if (!scan_udata_memory_size_helper(vm, &scan_start, &(extensions->tarokRememberedSetCardListSize),
                    "tarokRememberedSetCardListSize=")) {
                returnValue = JNI_EINVAL;
                break;
            }
        ...
        if (try_scan(&scan_start, "tarokRegionMaxAge=")) {
            if (!scan_udata_memory_size_helper(
                    vm, &scan_start, &(extensions->tarokRegionMaxAge), "tarokRegionMaxAge=")) {
                returnValue = JNI_EINVAL;
                break;
            }
            continue;
        ...
        if (try_scan(&scan_start, "tarokKickoffHeadroomRegionRate=")) {
            if (!scan_u32_helper(vm, &scan_start, &(extensions->tarokKickoffHeadroomRegionRate),
                    "tarokKickoffHeadroomRegionRate=")) {
                returnValue = JNI_EINVAL;
                break;
            }
            if (50 < extensions->tarokKickoffHeadroomRegionRate) {
                j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INTEGER_OUT_OF_RANGE,
                    "tarokKickoffHeadroomRegionRate=", (UDATA)0, (UDATA)50);
                returnValue = JNI_EINVAL;
                break;
            }
            continue;
        ...
        if (try_scan(&scan_start, "tarokKickoffHeadroomInBytes=")) {
            if (!scan_udata_memory_size_helper(
                    vm, &scan_start, &(extensions->tarokKickoffHeadroomInBytes), "tarokKickoffHeadroomInBytes=")) {
                returnValue = JNI_EINVAL;
                break;
            }
        ...
        if (try_scan(&scan_start, "tarokDynamicCollectionSetSelectionAbsoluteBudget=")) {
            if (!scan_udata_helper(vm, &scan_start, &extensions->tarokDynamicCollectionSetSelectionAbsoluteBudget,
                    "tarokDynamicCollectionSetSelectionAbsoluteBudget=")) {
                returnValue = JNI_EINVAL;
                break;
            }
        ...
        if (try_scan(&scan_start, "tarokDynamicCollectionSetSelectionPercentageBudget=")) {
            UDATA percentage = 0;
            if (!scan_udata_helper(
                    vm, &scan_start, &percentage, "tarokDynamicCollectionSetSelectionPercentageBudget=")) {
                returnValue = JNI_EINVAL;
                break;
            }
            if (percentage > 100) {
        ...
        if (try_scan(&scan_start, "tarokCoreSamplingAbsoluteBudget=")) {
            if (!scan_udata_helper(vm, &scan_start, &extensions->tarokCoreSamplingAbsoluteBudget,
                    "tarokCoreSamplingAbsoluteBudget=")) {
                returnValue = JNI_EINVAL;
                break;
            }
        ...
        if (try_scan(&scan_start, "tarokGlobalMarkIncrementTimeMillis=")) {
            if (!scan_udata_helper(vm, &scan_start, &extensions->tarokGlobalMarkIncrementTimeMillis,
                    "tarokGlobalMarkIncrementTimeMillis=")) {
                returnValue = JNI_EINVAL;
                break;
            }
            continue;
        ...
        if (try_scan(&scan_start, "tarokDefragmentEmptinessThreshold=")) {
            ...
            if (try_scan(&scan_start, "auto")) {
                ...
                if (percentage > 100) {
                    j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INTEGER_OUT_OF_RANGE,
                        "-XXgc:tarokDefragmentEmptinessThreshold=", (UDATA)0, (UDATA)100);
                    returnValue = JNI_EINVAL;
                    break;
                }
        ...
        if (try_scan(&scan_start, "tarokAllocationAgeUnit=")) {
            if (!scan_udata_memory_size_helper(
                    vm, &scan_start, &(extensions->tarokAllocationAgeUnit), "tarokAllocationAgeUnit=")) {
                returnValue = JNI_EINVAL;
                break;
            }
            continue;
        ...
        if (try_scan(&scan_start, "tarokMaximumAgeInBytes=")) {
            if (!scan_u64_memory_size_helper(
                    vm, &scan_start, &(extensions->tarokMaximumAgeInBytes), "tarokMaximumAgeInBytes=")) {
                returnValue = JNI_EINVAL;
                break;
            }
            continue;
        ...
        if (try_scan(&scan_start, "tarokMaximumNurseryAgeInBytes=")) {
            if (!scan_u64_memory_size_helper(
                    vm, &scan_start, &(extensions->tarokMaximumNurseryAgeInBytes), "tarokMaximumNurseryAgeInBytes=")) {
                returnValue = JNI_EINVAL;
                break;
            }
            continue;
        ...
        if (try_scan(&scan_start, "tarokWorkSplittingPeriod=")) {
            if (!scan_hex_helper(
                    vm, &scan_start, &(extensions->tarokWorkSplittingPeriod), "tarokWorkSplittingPeriod=")) {
                returnValue = JNI_EINVAL;
                break;
            }
            continue;
        ...
        if (try_scan(&scan_start, "tarokMinimumGMPWorkTargetBytes=")) {
            ...
            if (0 == workTarget) {
                j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_VALUE_MUST_BE_ABOVE,
                    "-XXgc:tarokMinimumGMPWorkTargetBytes", (UDATA)0);
                returnValue = JNI_EINVAL;
                break;
            }
        ...
        if (try_scan(&scan_start, "packetListLockSplit=")) {
            ...
            if (0 == extensions->packetListSplit) {
                j9nls_printf(
                    PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_VALUE_MUST_BE_ABOVE, "-XXgc:packetListLockSplit", (UDATA)0);
                returnValue = JNI_EINVAL;
                break;
            }
            continue;
        ...
        if (try_scan(&scan_start, "cacheListLockSplit=")) {
            ...
            if (0 == extensions->cacheListSplit) {
                j9nls_printf(
                    PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_VALUE_MUST_BE_ABOVE, "-XXgc:cacheListLockSplit", (UDATA)0);
                returnValue = JNI_EINVAL;
                break;
            }
            continue;
        ...
        if (try_scan(&scan_start, "markingArraySplitMaximumAmount=")) {
            ...
            if (extensions->markingArraySplitMaximumAmount < extensions->markingArraySplitMinimumAmount) {
                j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_VALUE_MUST_BE_ABOVE,
                    "-XXgc:markingArraySplitMaximumAmount", (UDATA)extensions->markingArraySplitMinimumAmount);
                returnValue = JNI_EINVAL;
                break;
            }
        ...
        if (try_scan(&scan_start, "stdSplitFreeListSplitAmount=h")) {
            ...
            if (!scan_udata_helper(
                    vm, &scan_start, &extensions->splitFreeListSplitAmount, "stdSplitFreeListSplitAmount=h")) {
                returnValue = JNI_EINVAL;
                break;
            }
            if (1 >= extensions->splitFreeListSplitAmount) {
                j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_VALUE_MUST_BE_ABOVE,
                    "-XXgc:stdSplitFreeListSplitAmount=h", (UDATA)1);
                returnValue = JNI_EINVAL;
                break;
            }
        ...
        if (try_scan(&scan_start, "stdSplitFreeListSplitAmount=")) {
            ...
            if (!scan_udata_helper(
                    vm, &scan_start, &extensions->splitFreeListSplitAmount, "stdSplitFreeListSplitAmount=")) {
                returnValue = JNI_EINVAL;
                break;
            }
            if (0 == extensions->splitFreeListSplitAmount) {
                j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_VALUE_MUST_BE_ABOVE,
                    "-XXgc:stdSplitFreeListSplitAmount", (UDATA)0);
                returnValue = JNI_EINVAL;
                break;
            }
            continue;
        ...
        if (try_scan(&scan_start, "scanCacheMinimumSize=")) {
            ...
            if (!scan_udata_helper(
                    vm, &scan_start, &extensions->scavengerScanCacheMinimumSize, "scanCacheMinimumSize=")) {
                returnValue = JNI_EINVAL;
                break;
            }
            if (0 == extensions->scavengerScanCacheMinimumSize) {
                j9nls_printf(
                    PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_VALUE_MUST_BE_ABOVE, "-XXgc:scanCacheMinimumSize", (UDATA)0);
                returnValue = JNI_EINVAL;
                break;
            }
            continue;
        ...
        if (try_scan(&scan_start, "scanCacheMaximumSize=")) {
            /* Read in restricted scan cache size */
            if (!scan_udata_helper(
                    vm, &scan_start, &extensions->scavengerScanCacheMaximumSize, "scanCacheMaximumSize=")) {
                returnValue = JNI_EINVAL;
                break;
            }
            if (extensions->scavengerScanCacheMinimumSize > extensions->scavengerScanCacheMaximumSize) {
                j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_VALUE_MUST_BE_ABOVE, "-XXgc:scanCacheMaximumSize",
                    (UDATA)extensions->scavengerScanCacheMinimumSize);
                returnValue = JNI_EINVAL;
                break;
            }
            continue;
        ...
        if (try_scan(&scan_start, "scvArraySplitMaximumAmount=")) {
            ...
            if (extensions->scvArraySplitMaximumAmount < extensions->scvArraySplitMinimumAmount) {
                j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_VALUE_MUST_BE_ABOVE,
                    "-XXgc:scvArraySplitMaximumAmount", (UDATA)extensions->scvArraySplitMinimumAmount);
                returnValue = JNI_EINVAL;
                break;
            }
        ...
        if (try_scan(&scan_start, "frequentObjectAllocationSamplingDepth=")) {
            if (!scan_u32_helper(vm, &scan_start, &extensions->frequentObjectAllocationSamplingDepth,
                    "frequentObjectAllocationSamplingDepth=")) {
                returnValue = JNI_EINVAL;
                break;
            }
            if (0 == extensions->frequentObjectAllocationSamplingDepth) {
                j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_VALUE_MUST_BE_ABOVE,
                    "-XXgc:frequentObjectAllocationSamplingDepth", (UDATA)0);
                returnValue = JNI_EINVAL;
                break;
            }
        ...
        if (try_scan(&scan_start, "largeObjectAllocationProfilingThreshold=")) {
            if (!scan_udata_helper(vm, &scan_start, &extensions->largeObjectAllocationProfilingThreshold,
                    "largeObjectAllocationProfilingThreshold=")) {
                returnValue = JNI_EINVAL;
                break;
            }
            continue;
        ...
        if (try_scan(&scan_start, "largeObjectAllocationProfilingVeryLargeObjectThreshold=")) {
            if (!scan_udata_helper(vm, &scan_start, &extensions->largeObjectAllocationProfilingVeryLargeObjectThreshold,
        ...
        if (try_scan(&scan_start, "largeObjectAllocationProfilingTopK=")) {
            if (!scan_u32_helper(vm, &scan_start, &extensions->largeObjectAllocationProfilingTopK,
                    "largeObjectAllocationProfilingTopK=")) {
                returnValue = JNI_EINVAL;
                break;
            }

            if ((extensions->largeObjectAllocationProfilingTopK < 1)
                || (128 < extensions->largeObjectAllocationProfilingTopK)) {
                j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INTEGER_OUT_OF_RANGE,
                    "largeObjectAllocationProfilingTopK=", (UDATA)1, (UDATA)128);
                returnValue = JNI_EINVAL;
                break;
            }
        ...
        if (try_scan(&scan_start, "largeObjectAllocationProfilingSizeClassRatio=")) {
            if (!scan_u32_helper(vm, &scan_start, &extensions->largeObjectAllocationProfilingSizeClassRatio,
                    "largeObjectAllocationProfilingSizeClassRatio=")) {
                returnValue = JNI_EINVAL;
                break;
            }

            if ((extensions->largeObjectAllocationProfilingSizeClassRatio < 101)
                || (200 < extensions->largeObjectAllocationProfilingSizeClassRatio)) {
                j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INTEGER_OUT_OF_RANGE,
                    "largeObjectAllocationProfilingSizeClassRatio=", (UDATA)101, (UDATA)200);
                returnValue = JNI_EINVAL;
                break;
            }
        ...
        if (try_scan(&scan_start, "fvtest_holdRandomThreadBeforeHandlingWorkUnitPeriod=")) {
            if (!scan_udata_helper(vm, &scan_start, &(extensions->_holdRandomThreadBeforeHandlingWorkUnitPeriod),
                    "fvtest_holdRandomThreadBeforeHandlingWorkUnitPeriod=")) {
                returnValue = JNI_EINVAL;
                break;
            }
        ...
        if (try_scan(&scan_start, "fvtest_forceRandomBackoutsAfterScanPeriod=")) {
            if (!scan_udata_helper(vm, &scan_start, &(extensions->_forceRandomBackoutsAfterScanPeriod),
                    "fvtest_forceRandomBackoutsAfterScanPeriod=")) {
                returnValue = JNI_EINVAL;
                break;
            }
        ...
        if (try_scan(&scan_start, "fvtest_concurrentCardTablePreparationDelay=")) {
            if (!scan_udata_helper(vm, &scan_start, &(extensions->fvtest_concurrentCardTablePreparationDelay),
                    "fvtest_concurrentCardTablePreparationDelay=")) {
                returnValue = JNI_EINVAL;
                break;
            }
            continue;
        ...
        if (try_scan(&scan_start, "fvtest_forceConcurrentTLHMarkMapCommitFailure=")) {
            if (!scan_udata_helper(vm, &scan_start, &(extensions->fvtest_forceConcurrentTLHMarkMapCommitFailure),
                    "fvtest_forceConcurrentTLHMarkMapCommitFailure=")) {
                returnValue = JNI_EINVAL;
                break;
            }
            if (extensions->fvtest_forceConcurrentTLHMarkMapCommitFailure > 0) {
                extensions->fvtest_forceConcurrentTLHMarkMapCommitFailureCounter
        ...
        if (try_scan(&scan_start, "fvtest_forceConcurrentTLHMarkMapDecommitFailure=")) {
            if (!scan_udata_helper(vm, &scan_start, &(extensions->fvtest_forceConcurrentTLHMarkMapDecommitFailure),
                    "fvtest_forceConcurrentTLHMarkMapDecommitFailure=")) {
                returnValue = JNI_EINVAL;
                break;
            }
            if (extensions->fvtest_forceConcurrentTLHMarkMapDecommitFailure > 0) {
                extensions->fvtest_forceConcurrentTLHMarkMapDecommitFailureCounter
        ...
        if (try_scan(&scan_start, "fvtest_forceCardTableCommitFailure=")) {
            if (!scan_udata_helper(vm, &scan_start, &(extensions->fvtest_forceCardTableCommitFailure),
                    "fvtest_forceCardTableCommitFailure=")) {
                returnValue = JNI_EINVAL;
                break;
            }
            if (extensions->fvtest_forceCardTableCommitFailure > 0) {
                extensions->fvtest_forceCardTableCommitFailureCounter
        ...
        if (try_scan(&scan_start, "fvtest_forceCardTableDecommitFailure=")) {
            if (!scan_udata_helper(vm, &scan_start, &(extensions->fvtest_forceCardTableDecommitFailure),
                    "fvtest_forceCardTableDecommitFailure=")) {
                returnValue = JNI_EINVAL;
                break;
            }
            if (extensions->fvtest_forceCardTableDecommitFailure > 0) {
                extensions->fvtest_forceCardTableDecommitFailureCounter
        ...
        if (try_scan(&scan_start, "fvtest_forceSweepChunkArrayCommitFailure=")) {
            if (!scan_udata_helper(vm, &scan_start, &(extensions->fvtest_forceSweepChunkArrayCommitFailure),
                    "fvtest_forceSweepChunkArrayCommitFailure=")) {
                returnValue = JNI_EINVAL;
                break;
            }
            if (extensions->fvtest_forceSweepChunkArrayCommitFailure > 0) {
                extensions->fvtest_forceSweepChunkArrayCommitFailureCounter
        ...
        if (try_scan(&scan_start, "fvtest_forceMarkMapCommitFailure=")) {
            if (!scan_udata_helper(vm, &scan_start, &(extensions->fvtest_forceMarkMapCommitFailure),
                    "fvtest_forceMarkMapCommitFailure=")) {
                returnValue = JNI_EINVAL;
                break;
            }
        ...
        if (try_scan(&scan_start, "fvtest_forceMarkMapDecommitFailure=")) {
            if (!scan_udata_helper(vm, &scan_start, &(extensions->fvtest_forceMarkMapDecommitFailure),
                    "fvtest_forceMarkMapDecommitFailure=")) {
                returnValue = JNI_EINVAL;
                break;
            }
            if (extensions->fvtest_forceMarkMapDecommitFailure > 0) {
                extensions->fvtest_forceMarkMapDecommitFailureCounter
        ...
        if (try_scan(&scan_start, "fvtest_forceReferenceChainWalkerMarkMapCommitFailure=")) {
            if (!scan_udata_helper(vm, &scan_start, &(extensions->fvtest_forceReferenceChainWalkerMarkMapCommitFailure),
                    "fvtest_forceReferenceChainWalkerMarkMapCommitFailure=")) {
                returnValue = JNI_EINVAL;
                break;
            }
            if (extensions->fvtest_forceReferenceChainWalkerMarkMapCommitFailure > 0) {
                extensions->fvtest_forceReferenceChainWalkerMarkMapCommitFailureCounter
        ...
        if (try_scan(&scan_start, "fvtest_forceCopyForwardHybridMarkCompactRatio=")) {
            ...
            if (!scan_udata_helper(vm, &scan_start, &(extensions->fvtest_forceCopyForwardHybridRatio),
                    "fvtest_forceCopyForwardHybridMarkCompactRatio=")) {
                returnValue = JNI_EINVAL;
                break;
            }
            if ((extensions->fvtest_forceCopyForwardHybridRatio < 1)
                || (100 < extensions->fvtest_forceCopyForwardHybridRatio)) {
                j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INTEGER_OUT_OF_RANGE,
                    "fvtest_forceCopyForwardHybridMarkCompactRatio=", (UDATA)1, (UDATA)100);
                returnValue = JNI_EINVAL;
                break;
            }
            continue;
        ...
        if (try_scan(&scan_start, "numaCommonThreadClass=")) {
            ...
            if (NULL == pattern) {
                ...
                if (0 == parseWildcard(pattern, strlen(pattern), &needle, &needleLength, &matchFlag)) {
                    MM_Wildcard* wildcard
                        = MM_Wildcard::newInstance(extensions, matchFlag, needle, needleLength, pattern);
                    if (NULL != wildcard) {
        ...
        if (try_scan(&scan_start, "stringTableListToTreeThreshold=")) {
            if (!scan_u32_helper(vm, &scan_start, &(extensions->_stringTableListToTreeThreshold),
                    "stringTableListToTreeThreshold=")) {
                returnValue = JNI_EINVAL;
                break;
            }
            continue;
        ...
        if (try_scan(&scan_start, "objectListFragmentCount=")) {
            if (!scan_udata_helper(
                    vm, &scan_start, &(extensions->objectListFragmentCount), "objectListFragmentCount=")) {
                returnValue = JNI_EINVAL;
                break;
            }
            continue;
