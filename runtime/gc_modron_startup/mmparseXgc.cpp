 ...
 */
static UDATA j9gc_initialize_parse_gc_colon(J9JavaVM* javaVM, char** scan_start)
{
    ...
    if (try_scan(scan_start, "tlhSurvivorDiscardThreshold=")) {
        if (!scan_udata_helper(
                javaVM, scan_start, &extensions->tlhSurvivorDiscardThreshold, "tlhSurvivorDiscardThreshold=")) {
            goto _error;
        }
        goto _exit;
    }
    if (try_scan(scan_start, "tlhTenureDiscardThreshold=")) {
        if (!scan_udata_helper(
                javaVM, scan_start, &extensions->tlhTenureDiscardThreshold, "tlhTenureDiscardThreshold=")) {
            goto _error;
        }
        goto _exit;
    }
    ...
    if (try_scan(scan_start, "allocationCacheMinimumSize=")) {
        if (!scan_udata_helper(
                javaVM, scan_start, &extensions->allocationCacheMinimumSize, "allocationCacheMinimumSize=")) {
            goto _error;
        }
        goto _exit;
    }
    if (try_scan(scan_start, "allocationCacheMaximumSize=")) {
        if (!scan_udata_helper(
                javaVM, scan_start, &extensions->allocationCacheMaximumSize, "allocationCacheMaximumSize=")) {
            goto _error;
        }
        goto _exit;
    }
    if (try_scan(scan_start, "allocationCacheInitialSize=")) {
        if (!scan_udata_helper(
                javaVM, scan_start, &extensions->allocationCacheInitialSize, "allocationCacheInitialSize=")) {
            goto _error;
        }
        goto _exit;
    }
    if (try_scan(scan_start, "allocationCacheIncrementSize=")) {
        if (!scan_udata_helper(
                javaVM, scan_start, &extensions->allocationCacheIncrementSize, "allocationCacheMaximumSize=")) {
            goto _error;
        }
        goto _exit;
    }
    ...
    if (try_scan(scan_start, "targetUtilization=")) {
        ...
        if ((extensions->targetUtilizationPercentage < 1) || (99 < extensions->targetUtilizationPercentage)) {
            j9nls_printf(
                PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INTEGER_OUT_OF_RANGE, "targetUtilization=", (UDATA)1, (UDATA)99);
            goto _error;
        }
        goto _exit;
    }
...
// Remove once LOA code stable
    ...
    /* Parsing of concurrentScavengeBackground/Slack must happen before concurrentScavenge, since the later option is a
     * substring of the former(s). However, there is no effective limitation on relative order of these options in a
     * command line. */
    ...
    if (try_scan(scan_start, "concurrentScavengeBackground=")) {
        if (!scan_udata_helper(javaVM, scan_start, &extensions->concurrentScavengerBackgroundThreads,
                "concurrentScavengeBackground=")) {
            goto _error;
        }
    ...
    if (try_scan(scan_start, "failedTenureThreshold=")) {
        if (!scan_udata_helper(
                javaVM, scan_start, &extensions->scavengerFailedTenureThreshold, "failedTenureThreshold=")) {
            goto _error;
        }
        goto _exit;
    }
    ...
    if (try_scan(scan_start, "scvCollectorExpandRatio=")) {
        ...
        if ((0 == value) || (100 < value)) {
            j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INTEGER_OUT_OF_RANGE,
                "scvCollectorExpandRatio=", (UDATA)1, (UDATA)100);
        }
    ...
    if (try_scan(scan_start, "scvTenureSurvivalThreshold=")) {
        ...
        if (value > 100) {
            j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INTEGER_OUT_OF_RANGE,
                "scvTenureSurvivalThreshold=", (UDATA)0, (UDATA)100);
            goto _error;
        }
    ...
    if (try_scan(scan_start, "tenureAge=")) {
        ...
        if ((tenureAge > OBJECT_HEADER_AGE_MAX) || (tenureAge < OBJECT_HEADER_AGE_MIN)) {
            j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INTEGER_OUT_OF_RANGE,
                "tenureAge=", (UDATA)OBJECT_HEADER_AGE_MIN, (UDATA)OBJECT_HEADER_AGE_MAX);
            goto _error;
        }
    ...
    if (try_scan(scan_start, "scvTenureAge=")) {
        ...
        if ((tenureAge > OBJECT_HEADER_AGE_MAX) || (tenureAge < OBJECT_HEADER_AGE_MIN)) {
            j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INTEGER_OUT_OF_RANGE,
                "scvTenureAge=", (UDATA)OBJECT_HEADER_AGE_MIN, (UDATA)OBJECT_HEADER_AGE_MAX);
            goto _error;
        }
        extensions->scvTenureAdaptiveTenureAge = tenureAge;
        extensions->scvTenureFixedTenureAge = tenureAge;
    ...
    if (try_scan(scan_start, "scvAdaptiveTenureAge=")) {
        ...
        if ((adaptiveTenureAge > OBJECT_HEADER_AGE_MAX) || (adaptiveTenureAge < OBJECT_HEADER_AGE_MIN)) {
            j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INTEGER_OUT_OF_RANGE,
                "scvAdaptiveTenureAge=", (UDATA)OBJECT_HEADER_AGE_MIN, (UDATA)OBJECT_HEADER_AGE_MAX);
            goto _error;
        }
    ...
    if (try_scan(scan_start, "scvFixedTenureAge=")) {
        ...
        if ((fixedTenureAge > OBJECT_HEADER_AGE_MAX) || (fixedTenureAge < OBJECT_HEADER_AGE_MIN)) {
            j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INTEGER_OUT_OF_RANGE,
                "scvFixedTenureAge=", (UDATA)OBJECT_HEADER_AGE_MIN, (UDATA)OBJECT_HEADER_AGE_MAX);
            goto _error;
        }
    ...
    if (try_scan(scan_start, "scvTiltRatioMax=")) {
        ...
        if ((50 > value) || (90 < value)) {
            j9nls_printf(
                PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INTEGER_OUT_OF_RANGE, "scvTiltRatioMax=", (UDATA)50, (UDATA)90);
        }
    ...
    if (try_scan(scan_start, "scvTiltRatioMin=")) {
        ...
        if ((50 > value) || (90 < value)) {
            j9nls_printf(
                PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INTEGER_OUT_OF_RANGE, "scvTiltRatioMin=", (UDATA)50, (UDATA)90);
        }
    ...
    if (try_scan(scan_start, "dnssMaximumContraction=")) {
        ...
        if (value > 100) {
            j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INTEGER_OUT_OF_RANGE,
                "dnssMaximumContraction=", (UDATA)0, (UDATA)100);
            goto _error;
        }
    ...
    if (try_scan(scan_start, "dnssMaximumExpansion=")) {
        ...
        if (value > 100) {
            j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INTEGER_OUT_OF_RANGE, "dnssMaximumExpansion=", (UDATA)0,
                (UDATA)100);
            goto _error;
        }
    ...
    if (try_scan(scan_start, "dnssMinimumContraction=")) {
        ...
        if (value > 100) {
            j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INTEGER_OUT_OF_RANGE,
                "dnssMinimumContraction=", (UDATA)0, (UDATA)100);
            goto _error;
        }
    ...
    if (try_scan(scan_start, "loaFreeHistorySize=")) {
        ...
        if (value > 100) {
            j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INTEGER_OUT_OF_RANGE, "loaFreeHistorySize=", (UDATA)0,
                (UDATA)100);
            goto _error;
        }
    ...
    if (try_scan(scan_start, "dnssMinimumExpansion=")) {
        ...
        if (value > 100) {
            j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INTEGER_OUT_OF_RANGE, "dnssMinimumExpansion=", (UDATA)0,
                (UDATA)100);
            goto _error;
        }
    ...
    if (try_scan(scan_start, "dnssExpectedTimeRatioMinimum=")) {
        ...
        if (value > 100) {
            j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INTEGER_OUT_OF_RANGE,
                "dnssExpectedTimeRatioMinimum=", (UDATA)0, (UDATA)100);
            goto _error;
        }
    ...
    if (try_scan(scan_start, "dnssExpectedTimeRatioMaximum=")) {
        ...
        if (value > 100) {
            j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INTEGER_OUT_OF_RANGE,
                "dnssExpectedTimeRatioMaximum=", (UDATA)0, (UDATA)100);
            goto _error;
        }
    ...
    if (try_scan(scan_start, "dnssWeightedTimeRatioFactorIncreaseSmall=")) {
        ...
        if (value > 100) {
            j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INTEGER_OUT_OF_RANGE,
                "dnssWeightedTimeRatioFactorIncreaseSmall=", (UDATA)0, (UDATA)100);
            goto _error;
        }
    ...
    if (try_scan(scan_start, "dnssWeightedTimeRatioFactorIncreaseMedium=")) {
        ...
        if (value > 100) {
            j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INTEGER_OUT_OF_RANGE,
                "dnssWeightedTimeRatioFactorIncreaseMedium=", (UDATA)0, (UDATA)100);
            goto _error;
        }
    ...
    if (try_scan(scan_start, "dnssWeightedTimeRatioFactorIncreaseLarge=")) {
        ...
        if (value > 100) {
            j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INTEGER_OUT_OF_RANGE,
                "dnssWeightedTimeRatioFactorIncreaseLarge=", (UDATA)0, (UDATA)100);
            goto _error;
        }
    ...
    if (try_scan(scan_start, "dnssWeightedTimeRatioFactorDecrease=")) {
        ...
        if (value > 100) {
            j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INTEGER_OUT_OF_RANGE,
                "dnssWeightedTimeRatioFactorDecrease=", (UDATA)0, (UDATA)100);
            goto _error;
        }
    ...
    if (try_scan(scan_start, "globalMaximumContraction=")) {
        ...
        if (value > 100) {
            j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INTEGER_OUT_OF_RANGE,
                "globalMaximumContraction=", (UDATA)0, (UDATA)100);
            goto _error;
        }
    ...
    if (try_scan(scan_start, "globalMinimumContraction=")) {
        ...
        if (value > 100) {
            j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INTEGER_OUT_OF_RANGE,
                "globalMinimumContraction=", (UDATA)0, (UDATA)100);
            goto _error;
        }
    ...
    if (try_scan(scan_start, "maxContractPercent=")) {
        ...
        if (value > 100) {
            j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INTEGER_OUT_OF_RANGE, "maxContractPercent=", (UDATA)0,
                (UDATA)100);
            goto _error;
        }
        extensions->globalMaximumContraction = ((double)value) / ((double)100);
    ...
    if (try_scan(scan_start, "minContractPercent=")) {
        ...
        if (value > 100) {
            j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INTEGER_OUT_OF_RANGE, "minContractPercent=", (UDATA)0,
                (UDATA)100);
            goto _error;
        }
        extensions->globalMinimumContraction = ((double)value) / ((double)100);
    ...
    if (try_scan(scan_start, "excessiveGCratio=")) {
        ...
        if ((value > 100) || (value < 10)) {
            j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INTEGER_OUT_OF_RANGE, "excessiveGCratio=", (UDATA)10,
                (UDATA)100);
            goto _error;
        }
    ...
    if (try_scan(scan_start, "excessiveGCFreeSizeRatio=")) {
        ...
        if (value > 100) {
            j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INTEGER_OUT_OF_RANGE,
                "excessiveGCFreeSizeRatio=", (UDATA)0, (UDATA)100);
            goto _error;
        }
    ...
    if (try_scan(scan_start, "excessiveGCnewRatioWeight=")) {
        ...
        if ((value < 1) || (value > 90)) {
            j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INTEGER_OUT_OF_RANGE,
                "excessiveGCnewRatioWeight=", (UDATA)1, (UDATA)90);
            goto _error;
        }
    ...
    if (try_scan(scan_start, "concurrentSlack=")) {
        ...
        /* if concurrentSlack=macrofrag is set, we will count estimateFragmentation when deciding concurrentgc kickoff ,
         * concurrentSlackFragmentationAdjustmentWeight is set as 1.0 as default */
    ...
    if (try_scan(scan_start, "concurrentSlackFragmentationAdjustmentWeight=")) {
        ...
        if (value > 500) {
            j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INTEGER_OUT_OF_RANGE,
                "concurrentSlackFragmentationAdjustmentWeight==", (UDATA)0, (UDATA)500);
            goto _error;
        }
    ...
    if (try_scan(scan_start, "cardCleaningPasses=")) {
        ...
        if (value > 2) {
            j9nls_printf(
                PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INTEGER_OUT_OF_RANGE, "cardCleaningPasses=", (UDATA)0, (UDATA)2);
            goto _error;
        }
    ...
    if (try_scan(scan_start, "fvtest=")) {
        ...
        if (try_scan(scan_start, "forceExcessiveAllocFailureAfter=")) {
            if (!scan_udata_helper(javaVM, scan_start, &(extensions->fvtest_forceExcessiveAllocFailureAfter),
                    "forceExcessiveAllocFailureAfter=")) {
                goto _error;
            }
            goto _exit;
        }
        ...
        if (try_scan(scan_start, "verifyHeapAbove=")) {
            if (!scan_udata_memory_size_helper(
                    javaVM, scan_start, (UDATA*)&(extensions->fvtest_verifyHeapAbove), "verifyHeapAbove=")) {
                goto _error;
            }
            goto _exit;
        }
        if (try_scan(scan_start, "verifyHeapBelow=")) {
            if (!scan_udata_memory_size_helper(
                    javaVM, scan_start, (UDATA*)&(extensions->fvtest_verifyHeapBelow), "verifyHeapBelow=")) {
                goto _error;
            }
            goto _exit;
        }
...
jint gcParseXgcArguments(J9JavaVM* vm, char* optArg)
{
    ...
    while (scan_start < scan_limit) {
        ...
        if (try_scan(&scan_start, "finalizeMasterPriority=")) {
            ...
            if ((extensions->finalizeMasterPriority < J9THREAD_PRIORITY_USER_MIN)
                || (extensions->finalizeMasterPriority > J9THREAD_PRIORITY_USER_MAX)) {
                j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INTEGER_OUT_OF_RANGE, "-Xgc:finalizeMasterPriority",
                    (UDATA)J9THREAD_PRIORITY_USER_MIN, (UDATA)J9THREAD_PRIORITY_USER_MAX);
                returnValue = JNI_EINVAL;
                break;
            }
            continue;
        ...
        if (try_scan(&scan_start, "finalizeSlavePriority=")) {
            ...
            if ((extensions->finalizeSlavePriority < J9THREAD_PRIORITY_USER_MIN)
                || (extensions->finalizeSlavePriority > J9THREAD_PRIORITY_USER_MAX)) {
                j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_INTEGER_OUT_OF_RANGE, "-Xgc:finalizeSlavePriority",
                    (UDATA)J9THREAD_PRIORITY_USER_MIN, (UDATA)J9THREAD_PRIORITY_USER_MAX);
                returnValue = JNI_EINVAL;
                break;
            }
        ...
        if (try_scan(&scan_start, "deadClassLoaderCache=")) {
            if (!scan_udata_memory_size_helper(
                    vm, &scan_start, &extensions->deadClassLoaderCacheSize, "deadClassLoaderCache=")) {
                returnValue = JNI_EINVAL;
                break;
            }
            continue;
        ...
        if (try_scan(&scan_start, "classUnloadingKickoffThreshold=")) {
            if (!scan_udata_helper(vm, &scan_start, &extensions->dynamicClassUnloadingKickoffThreshold,
        ...
        if (try_scan(&scan_start, "classUnloadingAnonymousClassWeight=")) {
            ...
            if (0 == divisor) {
                j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_VALUE_MUST_BE_ABOVE,
                    "classUnloadingAnonymousClassWeight=", (UDATA)0);
                returnValue = JNI_EINVAL;
                break;
            } else {
        ...
        if (try_scan(&scan_start, "allocationSamplingGranularity=")) {
            if (!scan_udata_memory_size_helper(vm, &scan_start, &extensions->oolObjectSamplingBytesGranularity,
                    "allocationSamplingGranularity=")) {
                returnValue = JNI_EINVAL;
                break;
            }
        ...
        if (try_scan(&scan_start, "suballocatorInitialSize=")) {
            if (!scan_udata_memory_size_helper(
                    vm, &scan_start, &extensions->suballocatorInitialSize, "suballocatorInitialSize=")) {
                returnValue = JNI_EINVAL;
                break;
            }
            if (0 == extensions->suballocatorInitialSize) {
                j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_VALUE_MUST_BE_ABOVE,
                    "-Xgc:suballocatorInitialSize=", (UDATA)0);
                returnValue = JNI_EINVAL;
                break;
            }
            continue;
        ...
        if (try_scan(&scan_start, "suballocatorCommitSize=")) {
            if (!scan_udata_memory_size_helper(
                    vm, &scan_start, &extensions->suballocatorCommitSize, "suballocatorCommitSize=")) {
                returnValue = JNI_EINVAL;
                break;
            }
            if (0 == extensions->suballocatorCommitSize) {
                j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_GC_OPTIONS_VALUE_MUST_BE_ABOVE,
                    "-Xgc:suballocatorCommitSize=", (UDATA)0);
                returnValue = JNI_EINVAL;
                break;
            }
            continue;
        ...
        if (try_scan(&scan_start, "fvtest_tarokForceNUMANode=")) {
            if (!scan_udata_helper(
                    vm, &scan_start, &extensions->fvtest_tarokForceNUMANode, "fvtest_tarokForceNUMANode=")) {
                returnValue = JNI_EINVAL;
                break;
            }
            continue;
        ...
        if (try_scan(&scan_start, "fvtest_tarokFirstContext=")) {
            if (!scan_udata_helper(
                    vm, &scan_start, &extensions->fvtest_tarokFirstContext, "fvtest_tarokFirstContext=")) {
                returnValue = JNI_EINVAL;
                break;
            }
            continue;
