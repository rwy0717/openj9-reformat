 ...
 */
MM_BarrierSynchronization* MM_BarrierSynchronization::newInstance(MM_EnvironmentBase* env)
{
    MM_BarrierSynchronization* barrierSync;

    barrierSync = (MM_BarrierSynchronization*)env->getForge()->allocate(
        sizeof(MM_BarrierSynchronization), MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
    if (barrierSync) {
 ...
 */
bool MM_BarrierSynchronization::initialize(MM_EnvironmentBase* env) { return true; }

/**
 * Release resources.
 */
void MM_BarrierSynchronization::kill(MM_EnvironmentBase* env)
{
    tearDown(env);
 ...
 * Teardown
 */
void MM_BarrierSynchronization::tearDown(MM_EnvironmentBase* env) {}
 ...
 */
void MM_BarrierSynchronization::preRequestExclusiveVMAccess(J9VMThread* threadRequestingExclusive)
{
 ...
 * exclusive vm access.
 */
void MM_BarrierSynchronization::postRequestExclusiveVMAccess(J9VMThread* threadRequestingExclusive)
{
 ...
 * @param block boolean input paramter specifing whether we should block and wait, if another party is requesting at the
 * same time, or we return
...
{
    return _vm->internalVMFunctions->requestExclusiveVMAccessMetronomeTemp(
        _vm, block, &_vmResponsesRequiredForExclusiveVMAccess, &_jniResponsesRequiredForExclusiveVMAccess, gcPriority);
}
 ...
 */
void MM_BarrierSynchronization::waitForExclusiveVMAccess(MM_EnvironmentBase* env, bool waitRequired)
{
    J9VMThread* masterGCThread = (J9VMThread*)env->getLanguageVMThread();

    if (waitRequired) {
        _vm->internalVMFunctions->waitForExclusiveVMAccessMetronomeTemp((J9VMThread*)env->getLanguageVMThread(),
            _vmResponsesRequiredForExclusiveVMAccess, _jniResponsesRequiredForExclusiveVMAccess);
    }
 ...
 * @param The requesting thread.
 */
void MM_BarrierSynchronization::acquireExclusiveVMAccess(MM_EnvironmentBase* env, bool waitRequired)
{
    J9VMThread* masterGCThread = (J9VMThread*)env->getLanguageVMThread();
 ...
 * @param The requesting thread.
 */
void MM_BarrierSynchronization::releaseExclusiveVMAccess(MM_EnvironmentBase* env, bool releaseRequired)
{
    J9VMThread* masterGCThread = (J9VMThread*)env->getLanguageVMThread();
