...
bool MM_ConfigurationRealtime::initialize(MM_EnvironmentBase* env)
{
...
void MM_ConfigurationRealtime::tearDown(MM_EnvironmentBase* env)
{
...
MM_Heap* MM_ConfigurationRealtime::createHeapWithManager(
...
MM_MemorySpace* MM_ConfigurationRealtime::createDefaultMemorySpace(
    MM_EnvironmentBase* envBase, MM_Heap* heap, MM_InitializationParameters* parameters)
{
    ...
    memorySubSpaceMetronome = MM_MemorySubSpaceMetronome::newInstance(env, physicalSubArena, memoryPool, true,
    ...
    return MM_MemorySpace::newInstance(env, heap, physicalArena, memorySubSpaceMetronome, parameters,
        MEMORY_SPACE_NAME_METRONOME, MEMORY_SPACE_DESCRIPTION_METRONOME);
}

MM_EnvironmentBase* MM_ConfigurationRealtime::allocateNewEnvironment(
    MM_GCExtensionsBase* extensions, OMR_VMThread* omrVMThread)
{
    return MM_EnvironmentRealtime::newInstance(extensions, omrVMThread);
}

J9Pool* MM_ConfigurationRealtime::createEnvironmentPool(MM_EnvironmentBase* env)
{
     ...
     * to OS page size) */
    return pool_new(sizeof(MM_EnvironmentRealtime), numberOfElements, sizeof(U_64), 0, J9_GET_CALLSITE(),
        OMRMEM_CATEGORY_MM, POOL_FOR_PORT(PORTLIB));
}

bool MM_ConfigurationRealtime::initializeEnvironment(MM_EnvironmentBase* envBase)
{
    MM_EnvironmentRealtime* env = MM_EnvironmentRealtime::getEnvironment(envBase);
    if (MM_Configuration::initializeEnvironment(env)) {
        MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
        if (extensions->globalAllocationManager->acquireAllocationContext(env)) {
            MM_MemoryPoolSegregated* memoryPool = (MM_MemoryPoolSegregated*)extensions->heap->getDefaultMemorySpace()
...
void MM_ConfigurationRealtime::defaultMemorySpaceAllocated(MM_GCExtensionsBase* extensions, void* defaultMemorySpace)
{
...
MM_HeapRegionManager* MM_ConfigurationRealtime::createHeapRegionManager(MM_EnvironmentBase* env)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
    ...
    MM_HeapRegionManagerTarok* heapRegionManager = MM_HeapRegionManagerTarok::newInstance(env, extensions->regionSize,
        descriptorSize, MM_HeapRegionDescriptorRealtime::initializer, MM_HeapRegionDescriptorRealtime::destructor);
    return heapRegionManager;
}

MM_Dispatcher* MM_ConfigurationRealtime::createDispatcher(
