...
MM_EnvironmentRealtime* MM_EnvironmentRealtime::newInstance(MM_GCExtensionsBase* extensions, OMR_VMThread* omrVMThread)
{
    void* envPtr;
...
void MM_EnvironmentRealtime::kill() { MM_EnvironmentBase::kill(); }

bool MM_EnvironmentRealtime::initialize(MM_GCExtensionsBase* extensionsBase)
{
    ...
    _overflowCache = (MM_HeapRegionDescriptorRealtime**)getForge()->allocate(
        sizeof(MM_HeapRegionDescriptorRealtime*) * extensions->overflowCacheCount, MM_AllocationCategory::FIXED,
        J9_GET_CALLSITE());
...
void MM_EnvironmentRealtime::tearDown(MM_GCExtensionsBase* extensions)
{
...
void MM_EnvironmentRealtime::disableYield() { _yieldDisableDepth++; }
...
void MM_EnvironmentRealtime::reportScanningSuspended()
{
...
void MM_EnvironmentRealtime::reportScanningResumed()
{
    if (NULL != _rootScanner) {
