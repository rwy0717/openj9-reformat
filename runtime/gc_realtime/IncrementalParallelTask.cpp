...
void MM_IncrementalParallelTask::synchronizeGCThreads(MM_EnvironmentBase* envBase, const char* id)
{
    ...
    if (1 < _totalThreadCount) {
        ...
        if (_synchronizeCount == _threadCount) {
            ...
            do {
                if (_yieldCollaborator.getYieldCount() + _synchronizeCount >= _threadCount
                ...
                /* We check synchronizeIndex, so we can exit this iteration ASAP before synchedThreads is overwritten by
                 * another event in the next iteration (We may be overly cautious here, since we are not that sure that
                 * overlap between iterations may even happen)
                 */
                do {
                    env->reportScanningSuspended();
                    omrthread_monitor_wait(_synchronizeMutex);
                    env->reportScanningResumed();
                } while ((index == _synchronizeIndex) && !env->isMasterThread()
                    && (_yieldCollaborator.getResumeEvent() != MM_YieldCollaborator::synchedThreads));
...
bool MM_IncrementalParallelTask::synchronizeGCThreadsAndReleaseMaster(MM_EnvironmentBase* envBase, const char* id)
{
    MM_EnvironmentRealtime* env = MM_EnvironmentRealtime::getEnvironment(envBase);
...
bool MM_IncrementalParallelTask::synchronizeGCThreadsAndReleaseSingleThread(MM_EnvironmentBase* env, const char* id)
{
...
void MM_IncrementalParallelTask::releaseSynchronizedGCThreads(MM_EnvironmentBase* env)
{
    if (1 == _totalThreadCount) {
        return;
    }
