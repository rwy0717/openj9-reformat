...
void MM_MemorySubSpaceMetronome::yieldWhenRequested(MM_EnvironmentBase* env)
{
 ...
 */
void* MM_MemorySubSpaceMetronome::allocateObject(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription,
 ...
 */
void* MM_MemorySubSpaceMetronome::allocateArrayletLeaf(MM_EnvironmentBase* env,
...
void MM_MemorySubSpaceMetronome::collectOnOOM(
    MM_EnvironmentBase* env, MM_GCCode gcCode, MM_AllocateDescription* allocDescription)
{
    ...
    if (sched->isInitialized()) {
        ...
        sched->continueGC(envRealtime, OUT_OF_MEMORY_TRIGGER, allocDescription->getBytesRequested(),
            (J9VMThread*)env->getLanguageVMThread(), true);
    }
 ...
 */
void* MM_MemorySubSpaceMetronome::allocateMixedObjectOrArraylet(
 ...
 */
void MM_MemorySubSpaceMetronome::systemGarbageCollect(MM_EnvironmentBase* env, U_32 gcCode)
{
    MM_EnvironmentRealtime* envRealtime = MM_EnvironmentRealtime::getEnvironment(env);
    ...
    if (sched->isInitialized()) {
        ...
        sched->continueGC(envRealtime, SYSTEM_GC_TRIGGER, 0, (J9VMThread*)envRealtime->getLanguageVMThread(),
            J9MMCONSTANT_EXPLICIT_GC_RASDUMP_COMPACT != gcCode);
 ...
 */
void MM_MemorySubSpaceMetronome::collect(MM_EnvironmentBase* env, MM_GCCode gcCode)
{
 ...
 * @todo Provide class documentation
 */
MM_MemorySubSpaceMetronome* MM_MemorySubSpaceMetronome::newInstance(MM_EnvironmentBase* env,
    MM_PhysicalSubArena* physicalSubArena, MM_MemoryPool* memoryPool, bool usesGlobalCollector, UDATA minimumSize,
    UDATA initialSize, UDATA maximumSize)
{
    MM_MemorySubSpaceMetronome* memorySubSpace;

    memorySubSpace = (MM_MemorySubSpaceMetronome*)env->getForge()->allocate(
        sizeof(MM_MemorySubSpaceMetronome), MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
    if (NULL != memorySubSpace) {
        new (memorySubSpace) MM_MemorySubSpaceMetronome(
 ...
 * @todo Provide class documentation
 */
bool MM_MemorySubSpaceMetronome::initialize(MM_EnvironmentBase* env)
{
