...
MM_MetronomeAlarmThread* MM_MetronomeAlarmThread::newInstance(MM_EnvironmentBase* env)
{
    MM_MetronomeAlarmThread* alarmThread;

    alarmThread = (MM_MetronomeAlarmThread*)env->getForge()->allocate(
        sizeof(MM_MetronomeAlarmThread), MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
    if (alarmThread) {
...
void MM_MetronomeAlarmThread::kill(MM_EnvironmentBase* env)
{
    tearDown(env);
    env->getForge()->free(this);
}

void MM_MetronomeAlarmThread::tearDown(MM_EnvironmentBase* env)
{
...
bool MM_MetronomeAlarmThread::initialize(MM_EnvironmentBase* env)
{
 ...
 */
int J9THREAD_PROC MM_MetronomeAlarmThread::metronomeAlarmThreadWrapper(void* userData)
{
    MM_MetronomeAlarmThread* alarmThread = (MM_MetronomeAlarmThread*)userData;
    J9JavaVM* javaVM = (J9JavaVM*)alarmThread->getScheduler()->_extensions->getOmrVM()->_language_vm;
    ...
    j9sig_protect(MM_MetronomeAlarmThread::signalProtectedFunction, (void*)userData,
        ...
        J9PORT_SIG_FLAG_SIGALLSYNC | J9PORT_SIG_FLAG_MAY_CONTINUE_EXECUTION, &rc);
 ...
 */
bool MM_MetronomeAlarmThread::startThread(MM_EnvironmentBase* env)
{
    ...
    if (0
        != vm->internalVMFunctions->createThreadWithCategory(&_thread, 64 * 1024, startPriority, 0,
               MM_MetronomeAlarmThread::metronomeAlarmThreadWrapper, this, J9THREAD_CATEGORY_SYSTEM_GC_THREAD)) {
        return false;
    }
 ...
 * C++ entrypoint for the newly-created alarm thread.
 */
void MM_MetronomeAlarmThread::run(MM_EnvironmentRealtime* env)
{
    omrthread_monitor_enter(_mutex);
