...
bool MM_RealtimeAccessBarrier::initialize(MM_EnvironmentBase* env)
{
...
void MM_RealtimeAccessBarrier::tearDown(MM_EnvironmentBase* env) { MM_ObjectAccessBarrier::tearDown(env); }
 ...
 */
J9Object* MM_RealtimeAccessBarrier::referenceGet(J9VMThread* vmThread, J9Object* refObject)
{
 ...
 */
void MM_RealtimeAccessBarrier::jniDeleteGlobalReference(J9VMThread* vmThread, J9Object* reference)
{
    MM_EnvironmentBase* env = MM_EnvironmentBase::getEnvironment(vmThread->omrVMThread);
 ...
 * the remembered set.
 */
void MM_RealtimeAccessBarrier::stringConstantEscaped(J9VMThread* vmThread, J9Object* stringConst)
{
    MM_EnvironmentBase* env = MM_EnvironmentBase::getEnvironment(vmThread->omrVMThread);
 ...
 * from this method).
 */
void MM_RealtimeAccessBarrier::deleteHeapReference(MM_EnvironmentBase* env, J9Object* object)
{
 ...
 */
void MM_RealtimeAccessBarrier::validateWriteBarrier(
    J9VMThread* vmThread, J9Object* dstObject, fj9object_t* dstAddress, J9Object* srcObject)
{
    ...
    case GC_ObjectModel::SCAN_REFERENCE_MIXED_OBJECT: {
        ...
        UDATA dataSizeInSlots
            = MM_Bits::convertBytesToSlots(_extensions->objectModel.getSizeInBytesWithHeader(dstObject));
        if (slotIndex >= dataSizeInSlots) {
            j9tty_printf(PORTLIB, "validateWriteBarrier: slotIndex (%d) >= object size in slots (%d)", slotIndex,
                dataSizeInSlots);
    ...
    case GC_ObjectModel::SCAN_POINTER_ARRAY_OBJECT: {
        ...
        GC_ArrayletObjectModel::ArrayLayout layout
            = _extensions->indexableObjectModel.getArrayLayout((J9IndexableObject*)dstObject);
        switch (layout) {
        case GC_ArrayletObjectModel::InlineContiguous: {
            ...
            UDATA* dataEnd
                = dataStart + _extensions->indexableObjectModel.getSizeInElements((J9IndexableObject*)dstObject);
            if ((UDATA*)dstAddress < dataStart || (UDATA*)dstAddress >= dataEnd) {
                j9tty_printf(PORTLIB, "validateWriteBarrier: IC: store to %p not in data section of array %p to %p",
                    dstAddress, dataStart, dataEnd);
                printClass(javaVM, J9GC_J9OBJECT_CLAZZ(dstObject));
                j9tty_printf(PORTLIB, "\n");
            }
            break;
        ...
        case GC_ArrayletObjectModel::Discontiguous: {
            MM_HeapRegionDescriptorRealtime* region
                = (MM_HeapRegionDescriptorRealtime*)regionManager->tableDescriptorForAddress(dstAddress);
            if (!region->isArraylet()) {
                j9tty_printf(
                    PORTLIB, "validateWriteBarrier: D: dstAddress (%p) is not on an arraylet region", dstAddress);
                printClass(javaVM, J9GC_J9OBJECT_CLAZZ(dstObject));
                j9tty_printf(PORTLIB, "\n");
            } else {
                UDATA* arrayletParent
                    = region->getArrayletParent(region->whichArraylet((UDATA*)dstAddress, javaVM->arrayletLeafLogSize));
                if (arrayletParent != (UDATA*)dstObject) {
                    j9tty_printf(PORTLIB, "validateWriteBarrier: D: parent of arraylet (%p) is not destObject (%p)",
                        arrayletParent, dstObject);
                    printClass(javaVM, J9GC_J9OBJECT_CLAZZ(dstObject));
                    j9tty_printf(PORTLIB, "\n");
                }
            }
        ...
        case GC_ArrayletObjectModel::Hybrid: {
            ...
            if ((UDATA*)dstAddress < dataStart || (UDATA*)dstAddress >= dataEnd) {
                ...
                if (!region->isArraylet()) {
                    j9tty_printf(
                        PORTLIB, "validateWriteBarrier: H: dstAddress (%p) is not on an arraylet region", dstAddress);
                    printClass(javaVM, J9GC_J9OBJECT_CLAZZ(dstObject));
                } else {
                    UDATA* arrayletParent = region->getArrayletParent(
                        region->whichArraylet((UDATA*)dstAddress, javaVM->arrayletLeafLogSize));
                    if (arrayletParent != (UDATA*)dstObject) {
                        j9tty_printf(PORTLIB, "validateWriteBarrier: H: parent of arraylet (%p) is not destObject (%p)",
                            arrayletParent, dstObject);
                        printClass(javaVM, J9GC_J9OBJECT_CLAZZ(dstObject));
                        j9tty_printf(PORTLIB, "\n");
                    }
        ...
        if ((UDATA*)dstAddress < dataStart || (UDATA*)dstAddress >= dataEnd) {
            j9tty_printf(PORTLIB, "validateWriteBarrier: store to %p not in data section of array %p to %p", dstAddress,
                dataStart, dataEnd);
            printClass(javaVM, J9GC_J9OBJECT_CLAZZ(dstObject));
            j9tty_printf(PORTLIB, "\n");
        }
...
void MM_RealtimeAccessBarrier::printClass(J9JavaVM* javaVM, J9Class* clazz)
{
    J9ROMClass* romClass;
    J9UTF8* utf;
    PORT_ACCESS_FROM_JAVAVM(javaVM);
 ...
 */
void MM_RealtimeAccessBarrier::rememberObject(MM_EnvironmentBase* env, J9Object* object)
{
 ...
 */
mm_j9object_t MM_RealtimeAccessBarrier::readObjectFromInternalVMSlotImpl(
 ...
 */
void MM_RealtimeAccessBarrier::storeObjectToInternalVMSlot(J9VMThread* vmThread, J9Object** destSlot, J9Object* value)
{
 ...
 */
void MM_RealtimeAccessBarrier::rememberObjectIfBarrierEnabled(J9VMThread* vmThread, J9Object* object)
{
...
void* MM_RealtimeAccessBarrier::jniGetPrimitiveArrayCritical(J9VMThread* vmThread, jarray array, jboolean* isCopy)
{
    void* data = NULL;
    J9JavaVM* javaVM = vmThread->javaVM;
...
void MM_RealtimeAccessBarrier::jniReleasePrimitiveArrayCritical(
...
const jchar* MM_RealtimeAccessBarrier::jniGetStringCritical(J9VMThread* vmThread, jstring str, jboolean* isCopy)
{
    jchar* data = NULL;
    J9JavaVM* javaVM = vmThread->javaVM;
    J9InternalVMFunctions* functions = javaVM->internalVMFunctions;
...
void MM_RealtimeAccessBarrier::jniReleaseStringCritical(J9VMThread* vmThread, jstring str, const jchar* elems)
{
    J9JavaVM* javaVM = vmThread->javaVM;
    J9InternalVMFunctions* functions = javaVM->internalVMFunctions;
    bool hasVMAccess = false;
...
bool MM_RealtimeAccessBarrier::checkClassLive(J9JavaVM* javaVM, J9Class* classPtr)
{
    J9ClassLoader* classLoader = classPtr->classLoader;
    bool result = false;

    if ((0 == (classLoader->gcFlags & J9_GC_CLASS_LOADER_DEAD))
        && (0 == (J9CLASS_FLAGS(classPtr) & J9_JAVA_CLASS_DYING))) {
        /*
 ...
 */
J9Object* j9gc_objaccess_referenceGet(J9VMThread* vmThread, j9object_t refObject)
{
    MM_RealtimeAccessBarrier* barrier
        = (MM_RealtimeAccessBarrier*)MM_GCExtensions::getExtensions(vmThread)->accessBarrier;
    return barrier->referenceGet(vmThread, refObject);
}

void j9gc_objaccess_jniDeleteGlobalReference(J9VMThread* vmThread, J9Object* reference)
{
    MM_RealtimeAccessBarrier* barrier
        = (MM_RealtimeAccessBarrier*)MM_GCExtensions::getExtensions(vmThread)->accessBarrier;
...
j9gc_objaccess_checkStringConstantsLive(J9JavaVM* javaVM, j9object_t stringOne, j9object_t stringTwo)
{
    ...
    if (realtimeGC->isBarrierEnabled()) {
        if (realtimeGC->_unmarkedImpliesStringsCleared) {
             ...
             */
            return realtimeGC->getMarkingScheme()->isMarked((J9Object*)stringOne)
                && realtimeGC->getMarkingScheme()->isMarked((J9Object*)stringTwo);
        } else {
            J9VMThread* vmThread = javaVM->internalVMFunctions->currentVMThread(javaVM);
            ((MM_RealtimeAccessBarrier*)extensions->accessBarrier)
