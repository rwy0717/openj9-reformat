...
void MM_RealtimeGC::setGCThreadPriority(OMR_VMThread* vmThread, UDATA newGCThreadPriority)
{
 ...
 * Initialization.
 */
bool MM_RealtimeGC::initialize(MM_EnvironmentBase* env)
{
    ...
    if (METRONOME_DEFAULT_BEAT_MICRO != _extensions->beatMicro) {
        ...
        if ((hrtPeriodMicro < METRONOME_DEFAULT_HRT_PERIOD_MICRO)
            && (METRONOME_DEFAULT_HRT_PERIOD_MICRO < _extensions->beatMicro)) {
            /* If the adjusted value is too small for the hires clock resolution, we will use the default HRT period
             * provided that the default period is smaller than the quanta time specified. Otherwise we fail to
             * initialize the alarm thread with an error message.
             */
...
bool MM_RealtimeGC::allocateAndInitializeReferenceObjectLists(MM_EnvironmentBase* env)
{
    const UDATA listCount = MM_HeapRegionDescriptorRealtime::getReferenceObjectListCount(env);
    Assert_MM_true(0 < listCount);
    _extensions->referenceObjectLists = (MM_ReferenceObjectList*)env->getForge()->allocate(
        (sizeof(MM_ReferenceObjectList) * listCount), MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
...
bool MM_RealtimeGC::allocateAndInitializeUnfinalizedObjectLists(MM_EnvironmentBase* env)
{
    const UDATA listCount = MM_HeapRegionDescriptorRealtime::getUnfinalizedObjectListCount(env);
    Assert_MM_true(0 < listCount);
    MM_UnfinalizedObjectList* unfinalizedObjectLists = (MM_UnfinalizedObjectList*)env->getForge()->allocate(
        (sizeof(MM_UnfinalizedObjectList) * listCount), MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
    ...
    for (UDATA index = 0; index < listCount; index++) {
         ...
         */
        MM_UnfinalizedObjectList* previousUnfinalizedObjectList
            = (0 == index) ? NULL : &unfinalizedObjectLists[index - 1];
        MM_UnfinalizedObjectList* nextUnfinalizedObjectList
            = ((listCount - 1) == index) ? NULL : &unfinalizedObjectLists[index + 1];
...
bool MM_RealtimeGC::allocateAndInitializeOwnableSynchronizerObjectLists(MM_EnvironmentBase* env)
{
    ...
    MM_OwnableSynchronizerObjectList* ownableSynchronizerObjectLists
        = (MM_OwnableSynchronizerObjectList*)env->getForge()->allocate(
            (sizeof(MM_OwnableSynchronizerObjectList) * listCount), MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
    ...
    for (UDATA index = 0; index < listCount; index++) {
         ...
         * to ensure uniformity with SE/Balanced.
         */
        MM_OwnableSynchronizerObjectList* previousOwnableSynchronizerObjectList
            = (0 == index) ? NULL : &ownableSynchronizerObjectLists[index - 1];
        MM_OwnableSynchronizerObjectList* nextOwnableSynchronizerObjectList
            = ((listCount - 1) == index) ? NULL : &ownableSynchronizerObjectLists[index + 1];
 ...
 * Initialization.
 */
void MM_RealtimeGC::tearDown(MM_EnvironmentBase* env)
{
 ...
 */
void MM_RealtimeGC::masterSetupForGC(MM_EnvironmentBase* env)
{
    ...
    case MM_GCExtensions::DYNAMIC_CLASS_UNLOADING_ON_CLASS_LOADER_CHANGES:
        _extensions->runtimeCheckDynamicClassUnloading
            = (_extensions->aggressive || _extensions->classLoaderManager->isTimeForClassUnloading(env));
        break;
    default:
        break;
    }
 ...
 */
void MM_RealtimeGC::masterCleanupAfterGC(MM_EnvironmentBase* env)
{
#if defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING)
 ...
 */
void MM_RealtimeGC::workerSetupForGC(MM_EnvironmentBase* env) {}

/**
 */
void MM_RealtimeGC::clearGCStats(MM_EnvironmentBase* env)
{
...
/**
 */
void MM_RealtimeGC::mergeGCStats(MM_EnvironmentBase* env) {}

#if defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING)
void MM_RealtimeGC::processDyingClasses(MM_EnvironmentRealtime* env, UDATA* classUnloadCountResult,
    UDATA* anonymousClassUnloadCountResult, UDATA* classLoaderUnloadCountResult,
    J9ClassLoader** classLoaderUnloadListResult)
{
     ...
     *
     * Do this walk before classloaders to be unloaded walk to create list of anonymous classes to be unloaded and use
     * it as sublist to continue to build general list of classes to be unloaded
     *
     ...
     */
    anonymousClassUnloadList = addDyingClassesToList(
        env, _javaVM->anonClassLoader, false, anonymousClassUnloadList, &anonymousClassUnloadCount);
    ...
    if (0 != anonymousClassUnloadCount) {
        ...
        TRIGGER_J9HOOK_VM_ANON_CLASSES_UNLOAD(
            _javaVM->hookInterface, vmThread, anonymousClassUnloadCount, anonymousClassUnloadList);
        yieldFromClassUnloading(env);
    }
...
J9Class* MM_RealtimeGC::addDyingClassesToList(MM_EnvironmentRealtime* env, J9ClassLoader* classLoader, bool setAll,
    J9Class* classUnloadListStart, UDATA* classUnloadCountResult)
{
    J9VMThread* vmThread = (J9VMThread*)env->getLanguageVMThread();
 ...
 */
void MM_RealtimeGC::processUnlinkedClassLoaders(MM_EnvironmentBase* envModron, J9ClassLoader* deadClassLoaders)
{
    ...
    while ((NULL != nextClass) && (jlObject != nextClass)) {
        if (J9CLASS_FLAGS(nextClass) & J9_JAVA_CLASS_DYING) {
            while ((NULL != nextClass->subclassTraversalLink) && (jlObject != nextClass)
                && (J9CLASS_FLAGS(nextClass) & 0x08000000)) {
...
void MM_RealtimeGC::updateClassUnloadStats(
    MM_EnvironmentBase* env, UDATA classUnloadCount, UDATA anonymousClassUnloadCount, UDATA classLoaderUnloadCount)
{
 ...
 *
 */
void MM_RealtimeGC::unloadDeadClassLoaders(MM_EnvironmentBase* envModron)
{
    MM_EnvironmentRealtime* env = MM_EnvironmentRealtime::getEnvironment(envModron);
    J9ClassLoader* unloadLink = NULL;
    UDATA classUnloadCount = 0;
    ...
    processDyingClasses(
        env, &classUnloadCount, &anonymousClassUnloadCount, &classLoaderUnloadCount, &classLoadersUnloadedList);
    ...
    while (NULL != classLoadersUnloadedList) {
        ...
        _extensions->classLoaderManager->cleanUpSegmentsAlongClassLoaderLink(
            _javaVM, classLoader->classSegments, &reclaimedSegments);
         ...
         */
        if (((NULL != classLoader->sharedLibraries) && (0 != pool_numElements(classLoader->sharedLibraries)))
 ...
 * again.
 */
void MM_RealtimeGC::yieldFromClassUnloading(MM_EnvironmentRealtime* env)
{
    if (shouldYield(env)) {
        unlockClassUnloadMonitor(env);
        yield(env);
 ...
 */
void MM_RealtimeGC::lockClassUnloadMonitor(MM_EnvironmentRealtime* env)
{
 ...
 */
void MM_RealtimeGC::unlockClassUnloadMonitor(MM_EnvironmentRealtime* env)
{
#if defined(J9VM_JIT_CLASS_UNLOAD_RWMONITOR)
...
void MM_RealtimeGC::enqueuePointerArraylet(MM_EnvironmentRealtime* env, fj9object_t* arraylet)
{
    env->getWorkStack()->push(env, (void*)ARRAYLET_TO_ITEM(arraylet));
}

UDATA
MM_RealtimeGC::verbose(MM_EnvironmentBase* env) { return _sched->verbose(); }

/**
 * @note only called by master thread.
 */
void MM_RealtimeGC::doAuxilaryGCWork(MM_EnvironmentBase* env)
{
#if defined(J9VM_GC_FINALIZATION)
 ...
 * @note only called by master thread.
 */
void MM_RealtimeGC::incrementalCollect(MM_EnvironmentRealtime* env)
{
     ...
     */
#if defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING)
    bool fixupForClassUnload
        = (_extensions->classLoaderManager->reclaimableMemory() > _extensions->deadClassLoaderCacheSize);
#else /* defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING) */
    bool fixupForClassUnload = false;
#endif /* defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING) */
    if (J9VM_DEBUG_ATTRIBUTE_ALLOW_USER_HEAP_WALK
            == (((J9JavaVM*)env->getLanguageVM())->requiredDebugAttributes & J9VM_DEBUG_ATTRIBUTE_ALLOW_USER_HEAP_WALK)
        || fixupForClassUnload) {
...
void MM_RealtimeGC::flushCachedFullRegions(MM_EnvironmentBase* env)
{
 ...
 * color to GC_UNMARK.
 **/
void MM_RealtimeGC::allThreadsAllocateUnmarked(MM_EnvironmentBase* env)
{
...
/**
 */
void MM_RealtimeGC::internalPreCollect(
...
/**
 */
void MM_RealtimeGC::setupForGC(MM_EnvironmentBase* env) {}

/**
 * @note only called by master thread.
 */
bool MM_RealtimeGC::internalGarbageCollect(
...
void MM_RealtimeGC::internalPostCollect(MM_EnvironmentBase* env, MM_MemorySubSpace* subSpace)
{
    ...
    if (_extensions->minimumFreeEntrySize != (UDATA)-1) {
        ...
        float percentFreeHeapAfterCollect = _extensions->heap->getApproximateActiveFreeMemorySize() * 100.0f
        ...
        if (minFreeEntrySize != _memoryPool->getMinimumFreeEntrySize()) {
            /* Yes, it did => make sure it changed enough (more than 1% up or below the range boundary) to accept it (in
             * the example, 78.9 is ok, but 79.1 is not */
     ...
     * processed before the very last METRONOME_INCREMENT_STOP event before the PRIVATE_GC_POST_CYCLE_END event.
     * Otherwise the METRONOME_INCREMENT_START/END events become out of order and verbose GC will fail.
     */
    ...
    /*
     * We could potentially yield during reportGCCycleEnd (e.g. due to JIT callbacks) and the scheduler will only wake
     * up the master if _gcOn is true. Turn off _gcOn flag at the very last, after cycle end has been reported.
     */
    _sched->stopGC(rtEnv);
    env->_cycleState->_activeSubSpace = NULL;
}

void MM_RealtimeGC::reportGCCycleFinalIncrementEnding(MM_EnvironmentBase* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);

    MM_CommonGCData commonData;
    TRIGGER_J9HOOK_MM_OMR_GC_CYCLE_END(_extensions->omrHookInterface, env->getOmrVMThread(), j9time_hires_clock(),
 ...
 */
void MM_RealtimeGC::reportSyncGCStart(MM_EnvironmentBase* env, GCReason reason, UDATA reasonParameter)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    ...
    Trc_MM_SynchGCStart(env->getLanguageVMThread(), reason, getGCReasonAsString(reason), reasonParameter,
        approximateFreeFreeMemorySize, 0);
    ...
    TRIGGER_J9HOOK_MM_PRIVATE_METRONOME_SYNCHRONOUS_GC_START(_extensions->privateHookInterface, env->getOmrVMThread(),
        j9time_hires_clock(), J9HOOK_MM_PRIVATE_METRONOME_SYNCHRONOUS_GC_START, reason, reasonParameter,
        approximateFreeFreeMemorySize, 0, classLoaderUnloadedCount, classesUnloadedCount,
 ...
 * @ingroup GC_Metronome methodGroup
 */
void MM_RealtimeGC::reportSyncGCEnd(MM_EnvironmentBase* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    ...
    Trc_MM_SynchGCEnd(env->getLanguageVMThread(), approximateFreeMemorySize, 0, classLoaderUnloadCount,
        classUnloadCount, weakReferenceCount, softReferenceCount, maxSoftReferenceAge, softReferenceAge,
        phantomReferenceCount, finalizerCount, packetOverflowCount, objectOverflowCount);

    TRIGGER_J9HOOK_MM_PRIVATE_METRONOME_SYNCHRONOUS_GC_END(_extensions->privateHookInterface, env->getOmrVMThread(),
        j9time_hires_clock(), J9HOOK_MM_PRIVATE_METRONOME_SYNCHRONOUS_GC_END, approximateFreeMemorySize, 0,
        classLoaderUnloadCount, classUnloadCount, anonymousClassUnloadCount, weakReferenceCount, softReferenceCount,
        maxSoftReferenceAge, softReferenceAge, phantomReferenceCount, finalizerCount, packetOverflowCount,
        objectOverflowCount);
}
 ...
 * @ingroup GC_Metronome methodGroup
 */
void MM_RealtimeGC::reportGCCycleStart(MM_EnvironmentBase* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    ...
    TRIGGER_J9HOOK_MM_OMR_GC_CYCLE_START(_extensions->omrHookInterface, env->getOmrVMThread(), j9time_hires_clock(),
 ...
 * @ingroup GC_Metronome methodGroup
 */
void MM_RealtimeGC::reportGCCycleEnd(MM_EnvironmentBase* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    omrthread_monitor_enter(((J9JavaVM*)env->getLanguageVM())->gcCycleOnMonitor);
    ...
    if (_memoryPool->getBytesInUse() < _extensions->gcInitialTrigger) {
        ...
        TRIGGER_J9HOOK_MM_PRIVATE_METRONOME_TRIGGER_END(_extensions->privateHookInterface, env->getOmrVMThread(),
            j9time_hires_clock(), J9HOOK_MM_PRIVATE_METRONOME_TRIGGER_END);
    }
 ...
 * @ingroup GC_Metronome methodGroup
 */
bool MM_RealtimeGC::heapAddRange(
...
/**
 */
bool MM_RealtimeGC::heapRemoveRange(MM_EnvironmentBase* env, MM_MemorySubSpace* subspace, UDATA size, void* lowAddress,
 ...
 * memory changed.
 */
void MM_RealtimeGC::heapReconfigured(MM_EnvironmentBase* env) {}

/**
 */
bool MM_RealtimeGC::collectorStartup(MM_GCExtensionsBase* extensions)
{
...
/**
 */
void MM_RealtimeGC::collectorShutdown(MM_GCExtensionsBase* extensions) {}
 ...
 */
MM_WorkPacketsRealtime* MM_RealtimeGC::allocateWorkPackets(MM_EnvironmentBase* env)
{
 ...
 */
bool MM_RealtimeGC::shouldYield(MM_EnvironmentBase* env)
{
 ...
 */
void MM_RealtimeGC::yield(MM_EnvironmentBase* env) { _sched->yieldFromGC(MM_EnvironmentRealtime::getEnvironment(env)); }

/**
 * Yield only if the Scheduler deems yielding should occur at the time of the
 * call to this method.
 */
bool MM_RealtimeGC::condYield(MM_EnvironmentBase* env, U_64 timeSlackNanoSec)
{
    return _sched->condYieldFromGC(MM_EnvironmentRealtime::getEnvironment(env), timeSlackNanoSec);
}

bool MM_RealtimeGC::isMarked(void* objectPtr) { return _markingScheme->isMarked(static_cast<J9Object*>(objectPtr)); }

void MM_RealtimeGC::reportMarkStart(MM_EnvironmentBase* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
...
void MM_RealtimeGC::reportMarkEnd(MM_EnvironmentBase* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
...
void MM_RealtimeGC::reportSweepStart(MM_EnvironmentBase* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    Trc_MM_SweepStart(env->getLanguageVMThread());

    TRIGGER_J9HOOK_MM_PRIVATE_SWEEP_START(
        _extensions->privateHookInterface, env->getOmrVMThread(), j9time_hires_clock(), J9HOOK_MM_PRIVATE_SWEEP_START);
}

void MM_RealtimeGC::reportSweepEnd(MM_EnvironmentBase* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    Trc_MM_SweepEnd(env->getLanguageVMThread());

    TRIGGER_J9HOOK_MM_PRIVATE_SWEEP_END(
        _extensions->privateHookInterface, env->getOmrVMThread(), j9time_hires_clock(), J9HOOK_MM_PRIVATE_SWEEP_END);
}

#if defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING)
void MM_RealtimeGC::reportClassUnloadingStart(MM_EnvironmentBase* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
...
void MM_RealtimeGC::reportClassUnloadingEnd(MM_EnvironmentBase* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    MM_ClassUnloadStats* classUnloadStats = &_extensions->globalGCStats.classUnloadStats;
...
void MM_RealtimeGC::reportGCStart(MM_EnvironmentBase* env)
{
...
void MM_RealtimeGC::reportGCEnd(MM_EnvironmentBase* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    ...
    UDATA approximateTotalActiveFreeMemorySize
