...
public:
    ...
    bool _unmarkedImpliesStringsCleared; /**< If true, we can assume that unmarked strings in the string table will be
                                            cleared */
...
protected:
     ...
     */
    void processDyingClasses(MM_EnvironmentRealtime* env, UDATA* classUnloadCountResult,
        UDATA* anonymousClassUnloadCount, UDATA* classLoaderUnloadCountResult,
     ...
     */
    J9Class* addDyingClassesToList(MM_EnvironmentRealtime* env, J9ClassLoader* classLoader, bool setAll,
...
public:
    ...
    void doAuxilaryGCWork(
        MM_EnvironmentBase* env); /* Wake up finalizer thread. Discard segments freed by class unloading. */
    ...
    MMINLINE bool isBarrierEnabled()
    {
        return (_gcPhase == GC_PHASE_ROOT || _gcPhase == GC_PHASE_TRACE || _gcPhase == GC_PHASE_CONCURRENT_TRACE);
    }
    ...
    MMINLINE void setCollectorIdle()
    {
        _gcPhase = GC_PHASE_IDLE;
        _sched->_gcPhaseSet |= GC_PHASE_IDLE;
    }
    MMINLINE void setCollectorRootMarking()
    {
        _gcPhase = GC_PHASE_ROOT;
        _sched->_gcPhaseSet |= GC_PHASE_ROOT;
    }
    MMINLINE void setCollectorTracing()
    {
        _gcPhase = GC_PHASE_TRACE;
        _sched->_gcPhaseSet |= GC_PHASE_TRACE;
    }
    MMINLINE void setCollectorUnloadingClassLoaders()
    {
        _gcPhase = GC_PHASE_UNLOADING_CLASS_LOADERS;
        _sched->_gcPhaseSet |= GC_PHASE_UNLOADING_CLASS_LOADERS;
    }
    MMINLINE void setCollectorSweeping()
    {
        _gcPhase = GC_PHASE_SWEEP;
        _sched->_gcPhaseSet |= GC_PHASE_SWEEP;
    }
    MMINLINE void setCollectorConcurrentTracing()
    {
        _gcPhase = GC_PHASE_CONCURRENT_TRACE;
        _sched->_gcPhaseSet |= GC_PHASE_CONCURRENT_TRACE;
    }
    MMINLINE void setCollectorConcurrentSweeping()
    {
        _gcPhase = GC_PHASE_CONCURRENT_SWEEP;
        _sched->_gcPhaseSet |= GC_PHASE_CONCURRENT_SWEEP;
    }
    ...
    MMINLINE void setPreviousCycleBelowTrigger(bool previousCycleBelowTrigger)
    {
        _previousCycleBelowTrigger = previousCycleBelowTrigger;
    }
