...
public:
     ...
     */
    virtual const char* scannerName() { return "Mark"; }
    ...
    virtual void scanOneThreadImpl(MM_EnvironmentRealtime* env, J9VMThread* walkThread, void* localData)
    {
         ...
         * *)walkThreadEnv->_objectAllocationInterface)->preMarkCache(walkThreadEnv);*/
    ...
    virtual void doFinalizableObject(j9object_t object) { _markingScheme->markObject(_env, object); }
     ...
     */
    virtual void doSlot(J9Object** slot) { _markingScheme->markObject(_env, *slot); }
    ...
    virtual void doStackSlot(J9Object** slotPtr, void* walkState, const void* stackLocation)
    {
        ...
        if (_markingScheme->isHeapObject(object)) {
            ...
            Assert_MM_validStackSlot(
                MM_StackSlotValidator(MM_StackSlotValidator::NOT_ON_HEAP, object, stackLocation, walkState)
                    .validate(_env));
        }
    }
     ...
     */
    virtual void scanIncrementalRoots(MM_EnvironmentRealtime* env)
    {
...
public:
     ...
     * This scanner can be instantiated so we must give it a name.
     */
    virtual const char* scannerName() { return "Clearable"; }
     ...
     */
    virtual void scanMonitorReferences(MM_EnvironmentBase* envBase)
    {
     ...
     */
    virtual void scanUnfinalizedObjects(MM_EnvironmentBase* envBase)
    {
        MM_EnvironmentRealtime* env = MM_EnvironmentRealtime::getEnvironment(envBase);
    ...
    virtual void scanOwnableSynchronizerObjects(MM_EnvironmentBase* envBase)
    {
        MM_EnvironmentRealtime* env = MM_EnvironmentRealtime::getEnvironment(envBase);
     ...
     */
    virtual void scanJNIWeakGlobalReferences(MM_EnvironmentBase* envBase)
    {
        MM_EnvironmentRealtime* env = MM_EnvironmentRealtime::getEnvironment(envBase);
 ...
 */
MM_RealtimeMarkingScheme* MM_RealtimeMarkingScheme::newInstance(MM_EnvironmentBase* env, MM_RealtimeGC* realtimeGC)
{
    MM_RealtimeMarkingScheme* instance;

    instance = (MM_RealtimeMarkingScheme*)env->getForge()->allocate(
        sizeof(MM_RealtimeMarkingScheme), MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
    if (instance) {
 ...
 */
void MM_RealtimeMarkingScheme::kill(MM_EnvironmentBase* env)
{
    tearDown(env);
 ...
 *
 */
bool MM_RealtimeMarkingScheme::initialize(MM_EnvironmentBase* env)
{
 ...
 *
 */
void MM_RealtimeMarkingScheme::tearDown(MM_EnvironmentBase* env)
{
 ...
 *
 */
void MM_RealtimeMarkingScheme::markRoots(MM_EnvironmentRealtime* env, MM_RealtimeMarkingSchemeRootMarker* rootScanner)
{
 ...
 *
 */
void MM_RealtimeMarkingScheme::markLiveObjects(MM_EnvironmentRealtime* env)
{
...
MMINLINE UDATA MM_RealtimeMarkingScheme::scanPointerArraylet(MM_EnvironmentRealtime* env, fj9object_t* arraylet)
{
    fj9object_t* startScanPtr = arraylet;
    fj9object_t* endScanPtr
        = startScanPtr + (((J9JavaVM*)env->getLanguageVM())->arrayletLeafSize / sizeof(fj9object_t));
    return scanPointerRange(env, startScanPtr, endScanPtr);
}

MMINLINE UDATA MM_RealtimeMarkingScheme::scanPointerRange(
    MM_EnvironmentRealtime* env, fj9object_t* startScanPtr, fj9object_t* endScanPtr)
{
...
MMINLINE UDATA MM_RealtimeMarkingScheme::scanObject(MM_EnvironmentRealtime* env, J9Object* objectPtr)
{
...
MMINLINE UDATA MM_RealtimeMarkingScheme::scanMixedObject(MM_EnvironmentRealtime* env, J9Object* objectPtr)
{
    /* Object slots */
...
MMINLINE UDATA MM_RealtimeMarkingScheme::scanReferenceMixedObject(MM_EnvironmentRealtime* env, J9Object* objectPtr)
{
    fj9object_t* scanPtr = (fj9object_t*)(objectPtr + 1);
    UDATA objectSize = _gcExtensions->mixedObjectModel.getSizeInBytesWithHeader(objectPtr);
    fj9object_t* endScanPtr = (fj9object_t*)(((U_8*)objectPtr) + objectSize);
        ...
        referentMustBeMarked = referentMustBeMarked
            || (((0 == (referenceObjectOptions & MM_CycleState::references_soft_as_weak))
                   && ((UDATA)J9GC_J9VMJAVALANGSOFTREFERENCE_AGE(env, objectPtr)
                          < _gcExtensions->getDynamicMaxSoftReferenceAge())));
        break;
    ...
    if (referentMustBeCleared) {
        /* Discovering this object at this stage in the GC indicates that it is being resurrected. Clear its referent
         * slot. */
...
MMINLINE UDATA MM_RealtimeMarkingScheme::scanPointerArrayObject(
    MM_EnvironmentRealtime* env, J9IndexableObject* objectPtr)
{
    UDATA pointerFields = 0;
    ...
    bool canSetAsScanned = (isContiguous
        && (_gcExtensions->minArraySizeToSetAsScanned
               <= _gcExtensions->indexableObjectModel.arrayletSize(objectPtr, 0)));
    ...
    if (isContiguous || (0 == sizeInElements)) {
        fj9object_t* startScanPtr
            = (fj9object_t*)_gcExtensions->indexableObjectModel.getDataPointerForContiguous(objectPtr);
        ...
        for (UDATA i = 0; i < numArraylets; i++) {
            ...
            /* need to check leaf pointer because this can be a partially allocated arraylet (not all leafs are
             * allocated) */
 ...
 * Mark this class
 */
bool MM_RealtimeMarkingScheme::markClass(MM_EnvironmentRealtime* env, J9Class* clazz)
{
    bool result = false;
    if (clazz != NULL) {
 ...
 * not finished yet.
 */
bool MM_RealtimeMarkingScheme::incrementalConsumeQueue(MM_EnvironmentRealtime* env, UDATA maxCount)
{
    UDATA item;
    ...
    while (0 != (item = (UDATA)env->getWorkStack()->pop(env))) {
        ...
        if (((countSinceLastYieldCheck * 2) + scannedPointersSumSinceLastYieldCheck)
            > _gcExtensions->traceCostToCheckYield) {
            _scheduler->condYieldFromGC(env);
...
#if defined(J9VM_GC_FINALIZATION)
void MM_RealtimeMarkingScheme::scanUnfinalizedObjects(MM_EnvironmentRealtime* env)
{
...
void MM_RealtimeMarkingScheme::scanOwnableSynchronizerObjects(MM_EnvironmentRealtime* env)
{
    ...
    if (env->_currentTask->synchronizeGCThreadsAndReleaseMaster(env, UNIQUE_ID)) {
        ...
        for (listIndex = 0; listIndex < maxIndex; ++listIndex) {
            MM_OwnableSynchronizerObjectList* ownableSynchronizerObjectList
                = &_gcExtensions->ownableSynchronizerObjectLists[listIndex];
...
void MM_RealtimeMarkingScheme::scanWeakReferenceObjects(MM_EnvironmentRealtime* env)
{
    GC_Environment* gcEnv = env->getGCEnvironment();
    ...
    for (listIndex = 0; listIndex < maxIndex; ++listIndex) {
        if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
            ...
            processReferenceList(
                env, NULL, referenceObjectList->getPriorWeakList(), &gcEnv->_markJavaStats._weakReferenceStats);
            _scheduler->condYieldFromGC(env);
        }
    }
    Assert_MM_true(gcEnv->_referenceObjectBuffer->isEmpty());
...
void MM_RealtimeMarkingScheme::scanSoftReferenceObjects(MM_EnvironmentRealtime* env)
{
    GC_Environment* gcEnv = env->getGCEnvironment();
    Assert_MM_true(gcEnv->_referenceObjectBuffer->isEmpty());
    const UDATA maxIndex = MM_HeapRegionDescriptorRealtime::getReferenceObjectListCount(env);
    ...
    for (listIndex = 0; listIndex < maxIndex; ++listIndex) {
        if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
            ...
            processReferenceList(
                env, NULL, referenceObjectList->getPriorSoftList(), &gcEnv->_markJavaStats._softReferenceStats);
            _scheduler->condYieldFromGC(env);
        }
    }
    Assert_MM_true(gcEnv->_referenceObjectBuffer->isEmpty());
...
void MM_RealtimeMarkingScheme::scanPhantomReferenceObjects(MM_EnvironmentRealtime* env)
{
    GC_Environment* gcEnv = env->getGCEnvironment();
    ...
    for (listIndex = 0; listIndex < maxIndex; ++listIndex) {
        if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
            ...
            processReferenceList(
                env, NULL, referenceObjectList->getPriorPhantomList(), &gcEnv->_markJavaStats._phantomReferenceStats);
            _scheduler->condYieldFromGC(env);
        }
    }
    Assert_MM_true(gcEnv->_referenceObjectBuffer->isEmpty());
...
void MM_RealtimeMarkingScheme::processReferenceList(MM_EnvironmentRealtime* env,
    MM_HeapRegionDescriptorRealtime* region, J9Object* headOfList, MM_ReferenceStats* referenceStats)
{
    UDATA objectsVisited = 0;
#if defined(J9VM_GC_FINALIZATION)
    ...
    while (NULL != referenceObj) {
        ...
        GC_SlotObject referentSlotObject(
            _gcExtensions->getOmrVM(), &J9GC_J9VMJAVALANGREFERENCE_REFERENT(env, referenceObj));
