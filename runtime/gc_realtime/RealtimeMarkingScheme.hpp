...
public:
    ...
    MMINLINE void unmark(omrobjectptr_t objPtr) { _markMap->clearBit(objPtr); }

    MMINLINE void mark(omrobjectptr_t objectPtr) { _markMap->setBit(objectPtr); }

    MMINLINE void markAtomic(omrobjectptr_t objectPtr) { _markMap->atomicSetBit(objectPtr); }
    ...
    MMINLINE bool markObject(MM_EnvironmentRealtime* env, J9Object* objectPtr, bool leafType = false)
    {
        if (objectPtr == NULL) {
            return false;
        }
...
private:
     ...
     */
    void processReferenceList(MM_EnvironmentRealtime* env, MM_HeapRegionDescriptorRealtime* region,
