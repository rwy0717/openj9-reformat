 ...
 */
MM_Scheduler* MM_Scheduler::newInstance(
    MM_EnvironmentBase* env, omrsig_handler_fn handler, void* handler_arg, UDATA defaultOSStackSize)
{
    MM_Scheduler* scheduler = (MM_Scheduler*)env->getForge()->allocate(
        sizeof(MM_Scheduler), MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
    if (scheduler) {
 ...
 * @ingroup GC_Metronome methodGroup
 */
void MM_Scheduler::kill(MM_EnvironmentBase* env) { tearDown(env); }
 ...
 * @ingroup GC_Metronome methodGroup
 */
void MM_Scheduler::tearDown(MM_EnvironmentBase* env)
{
...
MM_Scheduler::getParameter(UDATA which, char* keyBuffer, I_32 keyBufferSize, char* valueBuffer, I_32 valueBufferSize)
{
    ...
    switch (which) {
    case 0:
        j9str_printf(PORTLIB, keyBuffer, keyBufferSize, "Verbose Level");
...
void MM_Scheduler::showParameters(MM_EnvironmentBase* env)
{
...
void MM_Scheduler::initializeForVirtualSTW(MM_GCExtensions* ext)
{
 ...
 * @ingroup GC_Metronome methodGroup
 */
bool MM_Scheduler::initialize(MM_EnvironmentBase* env)
{
    ...
    _threadResumedTable = (bool*)env->getForge()->allocate(
        _threadCountMaximum * sizeof(bool), MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
    if (NULL == _threadResumedTable) {
        goto error_no_memory;
    }
...
void MM_Scheduler::collectorInitialized(MM_RealtimeGC* gc)
{
    _gc = gc;
    _osInterface = _gc->_osInterface;
}

void MM_Scheduler::checkStartGC(MM_EnvironmentRealtime* env)
{
 ...
 *
 */
void MM_Scheduler::startGC(MM_EnvironmentBase* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    if (verbose() >= 3) {
    ...
    if (METRONOME_GC_OFF == MM_AtomicOperations::lockCompareExchangeU32(&_gcOn, METRONOME_GC_OFF, METRONOME_GC_ON)) {
        if (_gc->isPreviousCycleBelowTrigger()) {
            ...
            TRIGGER_J9HOOK_MM_PRIVATE_METRONOME_TRIGGER_START(_extensions->privateHookInterface, env->getOmrVMThread(),
                j9time_hires_clock(), J9HOOK_MM_PRIVATE_METRONOME_TRIGGER_START);
        }
    }
}
 ...
 */
void MM_Scheduler::stopGC(MM_EnvironmentBase* env) { _gcOn = METRONOME_GC_OFF; }

bool MM_Scheduler::isGCOn() { return (METRONOME_GC_ON == _gcOn); }

bool MM_Scheduler::continueGC(MM_EnvironmentRealtime* env, GCReason reason, UDATA resonParameter, J9VMThread* thr,
    bool doRequestExclusiveVMAccess)
{
    UDATA gcPriority;
    bool didGC = true;
    ...
    if (_exclusiveVMAccessRequired) {
         ...
         */
        if (FALSE
            == _barrierSynchronization->requestExclusiveVMAccess(
                   _threadWaitingOnMasterThreadMonitor, FALSE /* do not block */, &gcPriority)) {
            didGC = false;
            goto exit;
        }
    ...
    /* set the waiting thread to NULL while we are in the _masterThreadMonitor so that nobody else will notify the
     * waiting thread */
...
void MM_Scheduler::waitForMutatorsToStop(MM_EnvironmentRealtime* env)
{
    ...
    _extensions->globalGCStats.metronomeStats._microsToStopMutators
        = j9time_hires_delta(exclusiveAccessTime, j9time_hires_clock(), J9PORT_TIME_DELTA_IN_MICROSECONDS);
}

void MM_Scheduler::startMutators(MM_EnvironmentRealtime* env)
{
    _mode = WAKING_MUTATOR;
    _barrierSynchronization->releaseExclusiveVMAccess(env, _exclusiveVMAccessRequired);
}

void MM_Scheduler::startGCTime(MM_EnvironmentRealtime* env, bool isDoubleBeat)
{
...
void MM_Scheduler::stopGCTime(MM_EnvironmentRealtime* env)
{
    if (env->isMasterThread()) {
...
bool MM_Scheduler::shouldGCDoubleBeat(MM_EnvironmentRealtime* env)
{
...
bool MM_Scheduler::shouldMutatorDoubleBeat(MM_EnvironmentRealtime* env, MM_Timer* timer)
{
...
void MM_Scheduler::reportStartGCIncrement(MM_EnvironmentRealtime* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    ...
    if (_completeCurrentGCSynchronously) {
        ...
        exclusiveAccessTimeMicros
            = j9time_hires_delta(0, env->getExclusiveAccessTime(), J9PORT_TIME_DELTA_IN_MICROSECONDS);
        meanExclusiveAccessIdleTimeMicros
            = j9time_hires_delta(0, env->getMeanExclusiveAccessIdleTime(), J9PORT_TIME_DELTA_IN_MICROSECONDS);
        Trc_MM_ExclusiveAccess(env->getLanguageVMThread(), (U_32)(exclusiveAccessTimeMicros / 1000),
            (U_32)(exclusiveAccessTimeMicros % 1000), (U_32)(meanExclusiveAccessIdleTimeMicros / 1000),
            (U_32)(meanExclusiveAccessIdleTimeMicros % 1000), env->getExclusiveAccessHaltedThreads(),
            env->getLastExclusiveAccessResponder(), env->exclusiveAccessBeatenByOtherThread());

        _gc->reportSyncGCStart(
            env, _completeCurrentGCSynchronouslyReason, _completeCurrentGCSynchronouslyReasonParameter);
    }
    ...
    TRIGGER_J9HOOK_MM_PRIVATE_METRONOME_INCREMENT_START(_extensions->privateHookInterface, env->getOmrVMThread(),
        j9time_hires_clock(), J9HOOK_MM_PRIVATE_METRONOME_INCREMENT_START,
        _extensions->globalGCStats.metronomeStats._microsToStopMutators);
...
void MM_Scheduler::reportStopGCIncrement(MM_EnvironmentRealtime* env, bool isCycleEnd)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    ...
    TRIGGER_J9HOOK_MM_PRIVATE_METRONOME_INCREMENT_END(_extensions->privateHookInterface, env->getOmrVMThread(),
        j9time_hires_clock(), J9HOOK_MM_PRIVATE_METRONOME_INCREMENT_END,
        _extensions->heap->getApproximateActiveFreeMemorySize(), 0,
        ...
        _extensions->markJavaStats._softReferenceStats._cleared, _extensions->getMaxSoftReferenceAge(),
        _extensions->getDynamicMaxSoftReferenceAge(), _extensions->markJavaStats._phantomReferenceStats._cleared,
...
void MM_Scheduler::restartMutatorsAndWait(MM_EnvironmentRealtime* env)
{
    startMutators(env);
...
bool MM_Scheduler::shouldGCYield(MM_EnvironmentRealtime* env, U_64 timeSlack)
{
 ...
 */
MMINLINE bool MM_Scheduler::internalShouldGCYield(MM_EnvironmentRealtime* env, U_64 timeSlack)
{
    if (_completeCurrentGCSynchronouslyMasterThreadCopy) {
...
bool MM_Scheduler::condYieldFromGCWrapper(MM_EnvironmentBase* env, U_64 timeSlack)
{
 ...
 */
bool MM_Scheduler::condYieldFromGC(MM_EnvironmentBase* envBase, U_64 timeSlack)
{
...
void MM_Scheduler::prepareThreadsForTask(MM_EnvironmentBase* env, MM_Task* task, uintptr_t threadCount)
{
...
void MM_Scheduler::completeTask(MM_EnvironmentBase* env)
{
    if (env->isMasterThread()) {
...
bool MM_Scheduler::startUpThreads()
{
    PORT_ACCESS_FROM_JAVAVM(_vm);
 ...
 */
void MM_Scheduler::recomputeActiveThreadCount(MM_EnvironmentBase* env) { _activeThreadCount = _threadCount; }
 ...
 */
void MM_Scheduler::slaveEntryPoint(MM_EnvironmentBase* envModron)
{
 ...
 */
void MM_Scheduler::masterEntryPoint(MM_EnvironmentBase* envModron)
{
    MM_EnvironmentRealtime* env = MM_EnvironmentRealtime::getEnvironment(envModron);
 ...
 */
void MM_Scheduler::completeCurrentGCSynchronously(MM_EnvironmentRealtime* env)
{
 ...
 */
void MM_Scheduler::wakeUpThreads(UDATA count)
{
    assert1(count > 0);
 ...
 */
void MM_Scheduler::wakeUpSlaveThreads(UDATA count) { omrthread_monitor_notify_all(_slaveThreadMutex); }

/**
 * @copydoc MM_ParallelDispatcher::shutDownThreads()
 */
void MM_Scheduler::shutDownThreads()
{
 ...
 */
void MM_Scheduler::shutDownSlaveThreads()
{
 ...
 */
void MM_Scheduler::shutDownMasterThread()
{
    omrthread_monitor_enter(_slaveThreadMutex);
 ...
 * increment now.
 */
void MM_Scheduler::startGCIfTimeExpired(MM_EnvironmentBase* envModron)
{
    MM_EnvironmentRealtime* env = MM_EnvironmentRealtime::getEnvironment(envModron);
    if (isInitialized() && isGCOn()
        && env->getTimer()->hasTimeElapsed(getStartTimeOfCurrentMutatorSlice(), beatNanos)) {
        continueGC(env, TIME_TRIGGER, 0, (J9VMThread*)env->getLanguageVMThread(), true);
    }
}

UDATA
MM_Scheduler::incrementMutatorCount() { return MM_AtomicOperations::add(&_mutatorCount, 1); }

extern "C" {

void j9gc_startGCIfTimeExpired(J9VMThread* vmThread)
{
