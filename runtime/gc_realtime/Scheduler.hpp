...
class MM_Scheduler : public MM_ParallelDispatcher {
     ...
     */
private:
    U_64 _mutatorStartTimeInNanos; /**< Time in nanoseconds when the mutator slice started.  This is updated at
                                      increment end and when a GC quantum is skipped due to shouldMutatorDoubleBeat */
    U_64 _incrementStartTimeInNanos; /**< Time in nanoseconds when the last gc increment started */
    MM_GCCode
        _gcCode; /**< The gc code that will be used for the next GC cycle.  If this is modified during a collect it will
                    be unused.  This variable is reset at the end of every cycle to the default collection type */
protected:
public:
    ...
    bool* _threadResumedTable; /**< Used to keep track of threads taken out of the suspended state when wakeUpThreads is
                                  called */

    bool _masterThreadMustShutDown; /**< Set when the master thread must shutdown */

    bool _exclusiveVMAccessRequired; /**< This flag is used by the master thread to see if it needs to get exclusive vm
                                        access */
...
public:
    ...
    void popYieldCollaborator() { _yieldCollaborator = _yieldCollaborator->pop(); }
    ...
    static MM_Scheduler* newInstance(
    ...
    bool continueGC(MM_EnvironmentRealtime*, GCReason reason, UDATA reasonParameter, J9VMThread* _vmThread,
        bool doRequestExclusiveVMAccess); /* Non-blocking and typicallly called by an alarm handler.  Returns 1 if we
                                             did resume GC (non-recursive). */
