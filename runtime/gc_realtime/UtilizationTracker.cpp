 ...
 */
MM_UtilizationTracker* MM_UtilizationTracker::newInstance(
    MM_EnvironmentBase* env, double timeWindow, U_64 maxSliceNano, double targetUtil)
{
    MM_UtilizationTracker* tracker;

    tracker = (MM_UtilizationTracker*)env->getForge()->allocate(
        sizeof(MM_UtilizationTracker), MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
    if (tracker) {
 ...
 */
void MM_UtilizationTracker::kill(MM_EnvironmentBase* env)
{
    tearDown(env);
 ...
 */
void MM_UtilizationTracker::tearDown(MM_EnvironmentBase* env) {}

/**
 * Initialize an instance of the UtilizationTracker class.
 */
bool MM_UtilizationTracker::initialize(MM_EnvironmentBase* env)
{
 ...
 */
double MM_UtilizationTracker::getTargetUtilization() { return _targetUtilization; }
 ...
 */
void MM_UtilizationTracker::compactTimeSliceWindowAndUpdateCurrentUtil(MM_EnvironmentRealtime* env)
{
    IDATA i = 0;
 ...
 * @note Synchronization must be provided externally when calling this method.
 */
U_64 MM_UtilizationTracker::addTimeSlice(MM_EnvironmentRealtime* env, MM_Timer* timer, bool isMutator)
{
    ...
    if (_timeSliceCursor >= _totalSlices) {
        ...
        TRIGGER_J9HOOK_MM_PRIVATE_UTILIZATION_TRACKER_OVERFLOW(
            MM_GCExtensions::getExtensions(env)->privateHookInterface, env->getOmrVMThread(), j9time_hires_clock(),
 ...
 * @note Synchronization must be provided externally when calling this method.
 */
void MM_UtilizationTracker::updateCurrentUtil(MM_EnvironmentRealtime* env)
{
 ...
 */
double MM_UtilizationTracker::getCurrentUtil() { return _currentUtilization; }

/**
 * Returns how many nano-seconds we can run for before we violate maxGCSlice or target utilization.
 */
I_64 MM_UtilizationTracker::getNanosLeft(MM_EnvironmentRealtime* env, U_64 sliceStartTimeInNanos)
{
