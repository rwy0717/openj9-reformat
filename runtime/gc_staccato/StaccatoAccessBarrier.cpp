 ...
 */
MM_StaccatoAccessBarrier* MM_StaccatoAccessBarrier::newInstance(MM_EnvironmentBase* env)
{
    MM_StaccatoAccessBarrier* barrier;

    barrier = (MM_StaccatoAccessBarrier*)env->getForge()->allocate(
        sizeof(MM_StaccatoAccessBarrier), MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
    if (barrier) {
 ...
 */
bool MM_StaccatoAccessBarrier::initialize(MM_EnvironmentBase* env) { return MM_RealtimeAccessBarrier::initialize(env); }

void MM_StaccatoAccessBarrier::kill(MM_EnvironmentBase* env)
{
    tearDown(env);
    env->getForge()->free(this);
}

void MM_StaccatoAccessBarrier::tearDown(MM_EnvironmentBase* env) { MM_RealtimeAccessBarrier::tearDown(env); }
 ...
 */
void MM_StaccatoAccessBarrier::rememberObjectImpl(MM_EnvironmentBase* env, J9Object* object)
{
    J9VMThread* vmThread = (J9VMThread*)env->getLanguageVMThread();
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(vmThread->javaVM);

    extensions->sATBBarrierRememberedSet->storeInFragment(
        env, &vmThread->sATBBarrierRememberedSetFragment, (UDATA*)object);
}

void MM_StaccatoAccessBarrier::forcedToFinalizableObject(J9VMThread* vmThread, J9Object* object)
{
 ...
 * old value.
 */
bool MM_StaccatoAccessBarrier::preObjectStoreInternal(
 ...
 * old value.
 **/
bool MM_StaccatoAccessBarrier::preObjectStore(
    J9VMThread* vmThread, J9Object* destObject, fj9object_t* destAddress, J9Object* value, bool isVolatile)
{
 ...
 */
void MM_StaccatoAccessBarrier::setDoubleBarrierActiveOnThread(MM_EnvironmentBase* env)
{
 ...
 */
void MM_StaccatoAccessBarrier::setDoubleBarrierInactiveOnThread(MM_EnvironmentBase* env)
{
    MM_GCExtensions::getExtensions(env)->sATBBarrierRememberedSet->restoreLocalFragmentIndex(
        env, &(((J9VMThread*)env->getLanguageVMThread())->sATBBarrierRememberedSetFragment));
}

void MM_StaccatoAccessBarrier::initializeForNewThread(MM_EnvironmentBase* env)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
    extensions->sATBBarrierRememberedSet->initializeFragment(
        env, &(((J9VMThread*)env->getLanguageVMThread())->sATBBarrierRememberedSetFragment));
...
void MM_StaccatoAccessBarrier::scanContiguousArray(MM_EnvironmentRealtime* env, J9IndexableObject* objectPtr)
{
...
bool MM_StaccatoAccessBarrier::markAndScanContiguousArray(MM_EnvironmentRealtime* env, J9IndexableObject* objectPtr)
{
    UDATA arrayletSize = _extensions->indexableObjectModel.arrayletSize(objectPtr, /* arraylet index */ 0);
 ...
 */
I_32 MM_StaccatoAccessBarrier::backwardReferenceArrayCopyIndex(J9VMThread* vmThread, J9IndexableObject* srcObject,
 ...
 * @return ARRAY_COPY_SUCCESSFUL if copy was succesful, ARRAY_COPY_NOT_DONE no copy is done
 */
I_32 MM_StaccatoAccessBarrier::forwardReferenceArrayCopyIndex(J9VMThread* vmThread, J9IndexableObject* srcObject,
    J9IndexableObject* destObject, I_32 srcIndex, I_32 destIndex, I_32 lengthInSlots)
{
    MM_EnvironmentRealtime* env = MM_EnvironmentRealtime::getEnvironment(vmThread);
