...
MM_StaccatoGC* MM_StaccatoGC::newInstance(MM_EnvironmentBase* env)
{
    MM_StaccatoGC* globalGC = (MM_StaccatoGC*)env->getForge()->allocate(
        sizeof(MM_StaccatoGC), MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
    if (globalGC) {
...
void MM_StaccatoGC::kill(MM_EnvironmentBase* env)
{
    tearDown(env);
    env->getForge()->free(this);
}

bool MM_StaccatoGC::initialize(MM_EnvironmentBase* env)
{
...
void MM_StaccatoGC::tearDown(MM_EnvironmentBase* env)
{
    if (NULL != _extensions->sATBBarrierRememberedSet) {
 ...
 */
MM_RealtimeAccessBarrier* MM_StaccatoGC::allocateAccessBarrier(MM_EnvironmentBase* env)
{
 ...
 */
void MM_StaccatoGC::enableWriteBarrier(MM_EnvironmentBase* env)
{
 ...
 */
void MM_StaccatoGC::disableWriteBarrier(MM_EnvironmentBase* env)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
 ...
 */
void MM_StaccatoGC::enableDoubleBarrier(MM_EnvironmentBase* env)
{
 ...
 */
void MM_StaccatoGC::disableDoubleBarrierOnThread(MM_EnvironmentBase* env, J9VMThread* vmThread)
{
 ...
 */
void MM_StaccatoGC::disableDoubleBarrier(MM_EnvironmentBase* env)
{
...
void MM_StaccatoGC::flushRememberedSet(MM_EnvironmentRealtime* env)
{
 ...
 */
void MM_StaccatoGC::doTracing(MM_EnvironmentRealtime* env)
{
 ...
 */
bool MM_StaccatoGC::doClassTracing(MM_EnvironmentRealtime* env)
{
        ...
        if (0 == (classLoader->gcFlags & J9_GC_CLASS_LOADER_DEAD)) {
            if (J9CLASSLOADER_ANON_CLASS_LOADER == (classLoader->flags & J9CLASSLOADER_ANON_CLASS_LOADER)) {
                ...
                if (!(classLoader->gcFlags & J9_GC_CLASS_LOADER_SCANNED)
                    && _markingScheme->isMarked((J9Object*)classLoader->classLoaderObject)) {
                    ...
                    /*
                     * CMVC 178060 : disable hash table growth to prevent hash table entries from being rehashed during
                     * GC yield while GC was in the middle of iterating the hash table.
                     */
                    ...
                    while (NULL != clazz) {
                        ...
                        /**
                         * Jazz103 55784: We cannot rehash the table in the middle of iteration and the Space-opt
                         * hashtable cannot grow if J9HASH_TABLE_DO_NOT_REHASH is enabled. Don't yield if the hashtable
                         * is space-optimized because we run the risk of the mutator not being able to grow to
                    ...
                    /*
                     * CMVC 178060 : re-enable hash table growth. disable hash table growth to prevent hash table
                     * entries from being rehashed during GC yield while GC was in the middle of iterating the hash
                     * table.
                     */
