...
MM_FrequentObjectsStats* MM_FrequentObjectsStats::newInstance(MM_EnvironmentBase* env)
{
    ...
    U_32 frequentObjectAllocationSamplingDepth
        = MM_GCExtensions::getExtensions(env)->frequentObjectAllocationSamplingDepth;

    frequentObjectsStats = (MM_FrequentObjectsStats*)env->getForge()->allocate(
        sizeof(MM_FrequentObjectsStats), MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
...
void MM_FrequentObjectsStats::traceStats(MM_EnvironmentBase* env)
{
    ...
    for (i = 0; i < spaceSavingGetCurSize(_spaceSaving) && i < _topKFrequent; i++) {
        ...
        if (J9ROMCLASS_IS_ARRAY(clazz->romClass)) {
            ...
            Trc_MM_FrequentObjectStats_AllocationCacheIndexableObjectAllocation(vmThread, clazz, J9UTF8_LENGTH(utf),
                J9UTF8_DATA(utf), arity * 2, brackets, count, (UDATA)(((float)count) * sampleFreq));
        } else {
            Trc_MM_FrequentObjectStats_AllocationCacheObjectAllocation(vmThread, clazz,
                J9UTF8_LENGTH(J9ROMCLASS_CLASSNAME(clazz->romClass)),
                J9UTF8_DATA(J9ROMCLASS_CLASSNAME(clazz->romClass)), clazz->totalInstanceSize, count,
                (UDATA)(((float)count) * sampleFreq));
        }
    }
    return;
}
...
void MM_FrequentObjectsStats::merge(MM_FrequentObjectsStats* frequentObjectsStats)
{
    ...
    for (i = 0; i < spaceSavingGetCurSize(spaceSaving); i++) {
        spaceSavingUpdate(_spaceSaving, spaceSavingGetKthMostFreq(spaceSaving, i + 1),
            spaceSavingGetKthMostFreqCount(spaceSaving, i + 1));
    }
}
