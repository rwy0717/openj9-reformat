...
private:
    ...
    MMINLINE void recalculateArrayletOffsets()
    {
        if (_index > 0) {
            ...
            _currentArrayletIndex
                = ((U_32)_index - 1) / _fobjectsPerLeaf; /* The index of the arraylet containing _index */
            _currentArrayletOffset
                = ((U_32)_index - 1) % _fobjectsPerLeaf; /* The offset of _index in the current arraylet */
...
public:
    MMINLINE void initialize(J9Object* objectPtr)
    {
        ...
        /* if we are using hybrid arraylets, we need to ensure that we don't initialize the discontiguous iterator since
         * it will try to read off the end of the header when reading the non-existent arrayoid.  In the case of small
         * arrays near the end of a region, this could cause us to read an uncommitted page.
         */
    ...
    MMINLINE GC_SlotObject* nextSlot()
    {
        if (_index > 0) {
            ...
            _index -= 1;
            /* If we reach the end of the arraylet leaf, we need to recalculate our arrayletBaseAddress for the next
             * _index */
     ...
     */
    MMINLINE UDATA getIndex() { return _index; }
