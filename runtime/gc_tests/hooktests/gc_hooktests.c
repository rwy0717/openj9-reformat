...
jint JNICALL JVM_OnLoad(JavaVM* jvm, char* options, void* reserved)
{
    ...
    if (0
        != (*vmHooks)->J9HookRegisterWithCallSite(vmHooks, J9HOOK_VM_OBJECT_ALLOCATE_WITHIN_THRESHOLD,
               allocationThresholdHandler, OMR_GET_CALLSITE(), NULL)) {
    ...
    if (0
        != (*vmHooks)->J9HookRegisterWithCallSite(
               vmHooks, J9HOOK_VM_OBJECT_ALLOCATE, commonAllocHandler, OMR_GET_CALLSITE(), NULL)) {
        j9tty_printf(PORTLIB, "Unable to register for hook\n");
        return JNI_ERR;
    }

    /*if (0 != (*vmHooks)->J9HookRegisterWithCallSite(vmHooks, J9HOOK_VM_OBJECT_ALLOCATE_INSTRUMENTABLE,
    commonAllocHandler, OMR_GET_CALLSITE(), NULL)) { j9tty_printf(PORTLIB,"Unable to register for hook\n"); return
    JNI_ERR;
    }*/

    if (0
        != (*vmHooks)->J9HookRegisterWithCallSite(
               vmHooks, J9HOOK_VM_SHUTTING_DOWN, vmShutdownHandler, OMR_GET_CALLSITE(), NULL)) {
