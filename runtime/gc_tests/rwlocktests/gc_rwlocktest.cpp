 ...
 *        that are used
 */
static IDATA J9THREAD_PROC runRequest(SupportThreadInfo* info)
{
    IDATA result = 0;
    UDATA i = 0;
 ...
 */
SupportThreadInfo* createSupportThreadInfo(omrthread_entrypoint_t* functionsToRun, UDATA numberFunctions)
{
    PORT_ACCESS_FROM_PORT(sharedPortLibrary);
    SupportThreadInfo* info
        = (SupportThreadInfo*)j9mem_allocate_memory(sizeof(SupportThreadInfo), OMRMEM_CATEGORY_THREADS);
    info->readCounter = 0;
    info->writeCounter = 0;
...
startConcurrentThread(SupportThreadInfo* info)
{
    ...
    omrthread_monitor_enter(info->synchronization);
    createThreadWithCategory(&newThread, 0, /* default stack size */
        J9THREAD_PRIORITY_NORMAL, 0, /* start immediately */
        (omrthread_entrypoint_t)runRequest, (void*)info, J9THREAD_CATEGORY_SYSTEM_GC_THREAD);
 ...
 */
static IDATA J9THREAD_PROC enter_rwlock_read(SupportThreadInfo* info)
{
    info->lock->enterRead();
    info->readCounter++;
    return 0;
 ...
 * @param info the SupporThreadInfo which can be used by the step
 */
static IDATA J9THREAD_PROC exit_rwlock_read(SupportThreadInfo* info)
{
    info->lock->exitRead();
    info->readCounter--;
    return 0;
 ...
 * @param info the SupporThreadInfo which can be used by the step
 */
static IDATA J9THREAD_PROC enter_rwlock_write(SupportThreadInfo* info)
{
 ...
 * @param info the SupporThreadInfo which can be used by the step
 */
static IDATA J9THREAD_PROC exit_rwlock_write(SupportThreadInfo* info)
{
    info->lock->exitWrite();
    info->writeCounter--;
    return 0;
 ...
 */
void Test_RWLock_InitializeTest(CuTest* tc)
{
    IDATA result;
    MM_LightweightNonReentrantReaderWriterLock lock;
    /* initialize */
 ...
 */
void Test_RWLock_RWReadEnterExitTest(CuTest* tc)
{
    IDATA result;
    MM_LightweightNonReentrantReaderWriterLock lock;
    /* initialize */
 ...
 */
void Test_RWLock_RWWriteEnterExitTest(CuTest* tc)
{
    IDATA result;
    MM_LightweightNonReentrantReaderWriterLock lock;
    /* initialize */
 ...
 */
void Test_RWLock_multipleReadersTest(CuTest* tc)
{
 ...
 */
void Test_RWLock_readersExcludedTest(CuTest* tc)
{
    SupportThreadInfo* info;
    omrthread_entrypoint_t functionsToRun[2];
    functionsToRun[0] = (omrthread_entrypoint_t)&enter_rwlock_read;
 ...
 */
void Test_RWLock_writersExcludedTest(CuTest* tc)
{
    SupportThreadInfo* info;
 ...
 */
void Test_RWLock_writerExcludesWriterTest(CuTest* tc)
{
    SupportThreadInfo* info;
    omrthread_entrypoint_t functionsToRun[2];
    functionsToRun[0] = (omrthread_entrypoint_t)&enter_rwlock_write;
 ...
 */
void Test_RWLock_waitingWriterExcludes2ndReader(CuTest* tc)
{
    SupportThreadInfo* info;
    SupportThreadInfo* infoReader;
 ...
 */
void Test_RWLock_AllReadersProceedTest(CuTest* tc)
{
