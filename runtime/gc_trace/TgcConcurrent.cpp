 ...
 */
static void tgcHookConcurrentBackgroundThreadActivated(
    J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    tgcExtensions->printf("<CONCURRENT GC BK thread 0x%08.8zx activated after GC(%zu)>\n",
        event->currentThread->_language_vmthread, concurrentExtensions->gcCountAtBackgroundThreadActivation);
}
 ...
 */
static void tgcHookConcurrentBackgroundThreadFinished(
    J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
...
bool tgcConcurrentInitialize(J9JavaVM* javaVM)
{
    ...
    (*privateHooks)
        ->J9HookRegisterWithCallSite(privateHooks, J9HOOK_MM_PRIVATE_CONCURRENT_BACKGROUND_THREAD_ACTIVATED,
            tgcHookConcurrentBackgroundThreadActivated, OMR_GET_CALLSITE(), NULL);
    (*privateHooks)
        ->J9HookRegisterWithCallSite(privateHooks, J9HOOK_MM_PRIVATE_CONCURRENT_BACKGROUND_THREAD_FINISHED,
            tgcHookConcurrentBackgroundThreadFinished, OMR_GET_CALLSITE(), NULL);
