...
static void tgcHookCopyForwardEnd(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    J9VMThread* vmThread
        = static_cast<J9VMThread*>(((MM_CopyForwardEndEvent*)eventData)->currentThread->_language_vmthread);
    ...
    while ((walkThread = threadIterator.nextVMThread()) != NULL) {
        ...
        if ((walkThread == vmThread) || (env->getThreadType() == GC_SLAVE_THREAD)) {
            tgcExtensions->printf("%4zu:   %7zu   %7zu   %7zu   %7zu   %7zu   %7zu\n", env->getSlaveID(),
                env->_copyForwardStats._objectsCardClean, env->_copyForwardStats._objectsScannedFromWorkPackets,
                env->_copyForwardStats._objectsScannedFromOverflowedRegion,
                env->_copyForwardStats._objectsScannedFromNextInChain,
                env->_copyForwardStats._objectsScannedFromDepthStack, env->_copyForwardStats._objectsScannedFromRoot);
        }
    }
}
...
bool tgcCopyForwardInitialize(J9JavaVM* javaVM)
{
    ...
    (*privateHooks)
        ->J9HookRegisterWithCallSite(
            privateHooks, J9HOOK_MM_PRIVATE_COPY_FORWARD_END, tgcHookCopyForwardEnd, OMR_GET_CALLSITE(), javaVM);
