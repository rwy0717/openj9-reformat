...
MM_TgcExtensions* MM_TgcExtensions::newInstance(MM_GCExtensions* extensions)
{
    MM_TgcExtensions* tgcExtensions = (MM_TgcExtensions*)extensions->getForge()->allocate(
        sizeof(MM_TgcExtensions), MM_AllocationCategory::DIAGNOSTIC, J9_GET_CALLSITE());
...
void MM_TgcExtensions::tearDown(MM_GCExtensions* extensions)
{
...
void MM_TgcExtensions::kill(MM_GCExtensions* extensions)
{
    tearDown(extensions);
...
void MM_TgcExtensions::vprintf(const char* format, va_list args)
{
    PORT_ACCESS_FROM_PORT(getPortLibrary());
    j9file_vprintf(_outputFile, format, args);
}

void MM_TgcExtensions::printf(const char* format, ...)
{
    va_list args;
...
bool MM_TgcExtensions::setOutputFile(const char* filename)
{
    bool result = true;
    PORT_ACCESS_FROM_PORT(getPortLibrary());
