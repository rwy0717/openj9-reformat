...
static void tgcHeapPrintStats(OMR_VMThread* omrVMThread)
{
...
static void tgcHeapMicroFragmentPrintStats(OMR_VMThread* omrVMThread)
{
    ...
    tgcExtensions->printf(
        "   address start - address end     ( size)     free KBytes(   Pct) fragment KBytes(   Pct)\n");
    ...
    if (0 != tgcRegionDarkmatter_count) {
        ...
        tgcExtensions->printf("%p - %p(%3zuMB)%14zuKB(%5.2f%%)%14zuKB(%5.2f%%)\n", tgcRegionDarkmatter_address_start,
            tgcRegionDarkmatter_address_end, regionSize >> 20, tgcRegionFree_bytes >> 10,
            100 * (float)tgcRegionFree_bytes / (float)regionSize, tgcRegionDarkmatter_bytes >> 10,
            100 * (float)tgcRegionDarkmatter_bytes / (float)regionSize);
    ...
    tgcExtensions->printf("Tenure(%zuMB) Free Size:		 %12zu(%zuMB, %5.2f%%), Micro Fragment Size:	"
                          "%12zu(%zuMB, %5.2f%%)\n",
        extensions->heap->getActiveMemorySize(MEMORY_TYPE_OLD) >> 20, tenureFreeBytes, tenureFreeBytes >> 20,
        100 * (float)tenureFreeBytes / (float)extensions->heap->getActiveMemorySize(MEMORY_TYPE_OLD),
        tenureDarkMatterBytes, tenureDarkMatterBytes >> 20,
        100 * (float)tenureDarkMatterBytes / (float)extensions->heap->getActiveMemorySize(MEMORY_TYPE_OLD));

    MM_SweepStats* sweepStats = &extensions->globalGCStats.sweepStats;

    U_64 duration
        = omrtime_hires_delta(sweepStats->_startTime, sweepStats->_endTime, J9PORT_TIME_DELTA_IN_MICROSECONDS);
    tgcExtensions->printf("Sweep Time(ms): total=\"%llu.%03.3llu\", DarkMatter Samples: %zu\n", duration / 1000,
        duration % 1000, tenureMemorySubspace->getMemoryPool()->getDarkMatterSamples());
}

static void tgcHookHeapGlobalPrintStats(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    MM_GlobalGCStartEvent* event = (MM_GlobalGCStartEvent*)eventData;
    tgcHeapPrintStats(event->currentThread);
}

static void tgcHookHeapLocalPrintStats(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    MM_LocalGCStartEvent* event = (MM_LocalGCStartEvent*)eventData;
    tgcHeapPrintStats(event->currentThread);
}

static void tgcHookGlobalGcSweepEndPrintStats(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
...
static void tgcShowRegions(OMR_VMThread* omrVMThread, const char* comment)
{
    MM_TgcExtensions* tgcExtensions = MM_TgcExtensions::getExtensions(omrVMThread);
    ...
    for (int32_t sizeClass = OMR_SIZECLASSES_MIN_SMALL; sizeClass <= OMR_SIZECLASSES_MAX_SMALL; sizeClass++) {
        ...
        while (OMR_VMThread* thread = vmThreadListIterator.nextOMRVMThread()) {
            ...
            allocCacheSize
                += MM_SegregatedAllocationInterface::getObjectAllocationInterface(envThread)->getAllocatableSize(
                    sizeClass);
        }
    ...
    tgcExtensions->printf(">>> small total (full, available) region count %d (%d, %d)\n", countTotal,
        countFullSmallTotal, countAvailableSmallTotal);
    ...
    tgcExtensions->printf(">>> dark matter total bytes %d (%2.2f%% of heap)\n", darkMatterBytesTotal,
        100.0 * darkMatterBytesTotal / heapSize);
    tgcExtensions->printf(">>> allocation cache total bytes %d (%2.2f%% of heap)\n", allocCacheBytesTotal,
        100.0 * allocCacheBytesTotal / heapSize);
    tgcExtensions->printf(">>> -------------------------------------------------------\n");
}

static void tgcShowHeapStatistics(OMR_VMThread* omrVMThread)
{
    MM_TgcExtensions* tgcExtensions = MM_TgcExtensions::getExtensions(omrVMThread);
    MM_GCExtensionsBase* extensions = MM_GCExtensions::getExtensions(omrVMThread);
    MM_MemoryPool* memoryPool
        = extensions->getHeap()->getDefaultMemorySpace()->getDefaultMemorySubSpace()->getMemoryPool();

    tgcExtensions->printf(">>> minimum free entry size: %d\n", memoryPool->getMinimumFreeEntrySize());
}

static void tgcHookSegregatedGlobalGcSweepStartPrintStats(
...
static void tgcHookSegregatedGlobalGcSweepEndPrintStats(
    J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    MM_SweepEndEvent* event = (MM_SweepEndEvent*)eventData;
...
static void tgcHookSegregatedGlobalGcSynchronousGCStart(
    J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
...
bool tgcHeapInitialize(J9JavaVM* javaVM)
{
    ...
    if (extensions->isStandardGC() || extensions->isVLHGC()) {
        (*omrHooks)->J9HookRegisterWithCallSite(
            omrHooks, J9HOOK_MM_OMR_GLOBAL_GC_START, tgcHookHeapGlobalPrintStats, OMR_GET_CALLSITE(), NULL);
        (*omrHooks)->J9HookRegisterWithCallSite(
            omrHooks, J9HOOK_MM_OMR_LOCAL_GC_START, tgcHookHeapLocalPrintStats, OMR_GET_CALLSITE(), NULL);
        (*privateHooks)
            ->J9HookRegisterWithCallSite(
                privateHooks, J9HOOK_MM_PRIVATE_SWEEP_END, tgcHookGlobalGcSweepEndPrintStats, OMR_GET_CALLSITE(), NULL);
    } else if (extensions->isSegregatedHeap()) {
#if defined(J9VM_GC_SEGREGATED_HEAP)
        (*privateHooks)
            ->J9HookRegisterWithCallSite(privateHooks, J9HOOK_MM_PRIVATE_SWEEP_START,
                tgcHookSegregatedGlobalGcSweepStartPrintStats, OMR_GET_CALLSITE(), NULL);
        (*privateHooks)
            ->J9HookRegisterWithCallSite(privateHooks, J9HOOK_MM_PRIVATE_SWEEP_END,
                tgcHookSegregatedGlobalGcSweepEndPrintStats, OMR_GET_CALLSITE(), NULL);
        (*privateHooks)
            ->J9HookRegisterWithCallSite(privateHooks, J9HOOK_MM_PRIVATE_METRONOME_SYNCHRONOUS_GC_START,
                tgcHookSegregatedGlobalGcSynchronousGCStart, OMR_GET_CALLSITE(), NULL);
#endif /* defined(J9VM_GC_SEGREGATED_HEAP) */
    }
    return result;
}
