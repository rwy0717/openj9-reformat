 ...
 */
static void tgcHookReportNumaStatistics(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (NULL == numaExtensions->nodeData) {
        ...
        numaExtensions->nodeData = (TgcNumaExtensions::NumaNodeData*)forge->allocate(
            dataSizeInBytes, MM_AllocationCategory::DIAGNOSTIC, J9_GET_CALLSITE());
    }
    ...
    if (NULL != numaExtensions->nodeData) {
        ...
        while (NULL != (walkThread = threadIterator.nextVMThread())) {
            ...
            UDATA threadNode = 0;
            UDATA nodeCount = 1;
            if ((0 != omrthread_numa_get_node_affinity(walkThread->osThread, &threadNode, &nodeCount))
                || (0 == nodeCount)) {
                threadNode = 0;
            }
        ...
        for (UDATA i = 0; i <= numaExtensions->numaNodes; i++) {
            tgcExtensions->printf(
                ...
                numaExtensions->nodeData[i].totalRegions, numaExtensions->nodeData[i].committedRegions,
                numaExtensions->nodeData[i].freeRegions, numaExtensions->nodeData[i].threads,
...
bool tgcNumaInitialize(J9JavaVM* javaVM)
{
    ...
    (*hooks)->J9HookRegisterWithCallSite(
        hooks, J9HOOK_MM_OMR_GLOBAL_GC_START, tgcHookReportNumaStatistics, OMR_GET_CALLSITE(), NULL);
    (*hooks)->J9HookRegisterWithCallSite(
        hooks, J9HOOK_MM_OMR_LOCAL_GC_START, tgcHookReportNumaStatistics, OMR_GET_CALLSITE(), NULL);
    (*hooks)->J9HookRegisterWithCallSite(
        hooks, J9HOOK_MM_OMR_LOCAL_GC_END, tgcHookReportNumaStatistics, OMR_GET_CALLSITE(), NULL);
