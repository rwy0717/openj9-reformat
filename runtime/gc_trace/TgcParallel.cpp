...
static void tgcHookGlobalGcEnd(J9HookInterface** hook, uintptr_t eventNum, void* eventData, void* userData)
{
        ...
        while ((walkThread = markThreadListIterator.nextVMThread()) != NULL) {
            ...
            if ((walkThread == vmThread) || (env->getThreadType() == GC_SLAVE_THREAD)) {
                ...
                if (extensions->isStandardGC()) {
                     ...
                     * so rely on Mark Task stats gcCount, which is atm identical (all phases of one GC are run with
                ...
                if (shouldIncludeThread) {
                    tgcExtensions->printf("%4zu:  %5llu  %5llu   %5zu     %5zu     %5zu\n", env->getSlaveID(),
                        j9time_hires_delta(parallelExtensions->RSScanStartTime,
                            env->_workPacketStatsRSScan.getStallTime(), J9PORT_TIME_DELTA_IN_MILLISECONDS),
                        j9time_hires_delta(env->_workPacketStatsRSScan.getStallTime(),
                            parallelExtensions->RSScanEndTime, J9PORT_TIME_DELTA_IN_MILLISECONDS),
            ...
            if ((walkThread == vmThread) || (env->getThreadType() == GC_SLAVE_THREAD)) {
                ...
                if (shouldIncludeThread) {
                    ...
                    tgcExtensions->printf("%4zu:  %5llu  %5llu    %5zu     %5zu     %5zu       %5zu     %7zu\n",
                        env->getSlaveID(),
                        j9time_hires_delta(0,
                            markTotalTime - (markStatsStallTime + env->_workPacketStats.getStallTime()),
                            J9PORT_TIME_DELTA_IN_MILLISECONDS),
                        j9time_hires_delta(0, markStatsStallTime + env->_workPacketStats.getStallTime(),
                            J9PORT_TIME_DELTA_IN_MILLISECONDS),
                        env->_workPacketStats.workPacketsAcquired, env->_workPacketStats.workPacketsReleased,
                        env->_workPacketStats.workPacketsExchanged, splitArraysProcessed, avgSplitSize);
                }
            ...
            if ((walkThread == vmThread) || (env->getThreadType() == GC_SLAVE_THREAD)) {
                ...
                if (shouldIncludeThread) {
                    tgcExtensions->printf("%4zu: %6llu %6llu %8zu\n", env->getSlaveID(),
                        ...
                        j9time_hires_delta(0, sweepIdleTime, J9PORT_TIME_DELTA_IN_MILLISECONDS), sweepChunksProcessed);
                }
            }
        }
    }
...
static void tgcHookGlobalGcMarkStart(J9HookInterface** hook, uintptr_t eventNum, void* eventData, void* userData)
{
...
static void tgcHookGlobalGcMarkEnd(J9HookInterface** hook, uintptr_t eventNum, void* eventData, void* userData)
{
...
static void tgcHookLocalGcEnd(J9HookInterface** hook, uintptr_t eventNum, void* eventData, void* userData)
{
    ...
    tgcExtensions->printf(
        "Scavenger parallel and progress stats, timestamp=\"%s.%ld\"\n", timestamp, timeInMillis % 1000);

    tgcExtensions->printf("          gc thrID     busy    stall   acquire   release   acquire   release   acquire     "
                          "split avg split  alias to\n");
    tgcExtensions->printf("                   (micros) (micros)  freelist  freelist  scanlist  scanlist      lock    "
                          "arrays arraysize copycache\n");
        ...
        if ((walkThread == vmThread) || (env->getThreadType() == GC_SLAVE_THREAD)) {
            ...
            if (env->_scavengerStats._gcCount == extensions->scavengerStats._gcCount) {
                ...
                if (0 != env->_scavengerStats._arraySplitCount) {
                    avgArraySplitAmount
                        = env->_scavengerStats._arraySplitAmount / env->_scavengerStats._arraySplitCount;
                }
                tgcExtensions->printf("SCV.T %6zu  %4zu %8llu %8llu     %5zu     %5zu     %5zu     %5zu     %5zu     "
                                      "%5zu     %5zu   %7zu\n",
                    gcCount, env->getSlaveID(),
                    j9time_hires_delta(
                        0, scavengeTotalTime - env->_scavengerStats.getStallTime(), J9PORT_TIME_DELTA_IN_MICROSECONDS),
                    j9time_hires_delta(0, env->_scavengerStats.getStallTime(), J9PORT_TIME_DELTA_IN_MICROSECONDS),
                    env->_scavengerStats._acquireFreeListCount, env->_scavengerStats._releaseFreeListCount,
                    env->_scavengerStats._acquireScanListCount, env->_scavengerStats._releaseScanListCount,
                    env->_scavengerStats._acquireListLockCount, env->_scavengerStats._arraySplitCount,
                    avgArraySplitAmount, env->_scavengerStats._aliasToCopyCacheCount);
            }
        }
    }
    ...
    while (historyRecord < endRecord) {
        ...
        tgcExtensions->printf("SCV.H %6zu %6zu %6.1f %6.1f %6.1f %6.1f   %6.1f %8zu %8zu %7zu  %0.4f", gcCount,
            elapsedMicros, waitingThreads, busyThreads, threads, lists, caches, historyRecord->copied,
            historyRecord->scanned, historyRecord->updates, scalingFactor);
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
        if (extensions->isConcurrentScavengerEnabled()) {
            tgcExtensions->printf("  %7zu %9zu\n", historyRecord->readObjectBarrierCopy - prevReadObjectBarrierCopy,
                historyRecord->readObjectBarrierUpdate - prevReadObjectBarrierUpdate);
    ...
    tgcExtensions->printf("\n");
    tgcExtensions->printf(
        "     \tgc\tleaf\talias\tsync-#\tsync-ms\twork-#\twork-ms\tend-#\tend-ms\tscaling\tupdates\toverflow\n");
    tgcExtensions->printf("SCV.M\t%lu\t%lu\t%lu\t%lu\t%lu\t%lu\t%lu\t%lu\t%lu\t%.3f\t%lu\t%lu\n", gcCount,
        ...
        extensions->scavengerStats._syncStallCount,
        j9time_hires_delta(0, extensions->scavengerStats._syncStallTime, J9PORT_TIME_DELTA_IN_MICROSECONDS),
        extensions->scavengerStats._workStallCount,
        j9time_hires_delta(0, extensions->scavengerStats._workStallTime, J9PORT_TIME_DELTA_IN_MICROSECONDS),
        extensions->scavengerStats._completeStallCount,
        j9time_hires_delta(0, extensions->scavengerStats._completeStallTime, J9PORT_TIME_DELTA_IN_MICROSECONDS),
        extensions->copyScanRatio.getScalingFactor(env), extensions->copyScanRatio.getScalingUpdateCount(),
...
#if defined(J9VM_GC_VLHGC)
static void tgcHookCopyForwardEnd(J9HookInterface** hook, uintptr_t eventNum, void* eventData, void* userData)
{
    MM_LocalGCStartEvent* event = (MM_LocalGCStartEvent*)eventData;
    J9VMThread* vmThread = (J9VMThread*)event->currentThread->_language_vmthread;
    MM_EnvironmentVLHGC* masterEnv = MM_EnvironmentVLHGC::getEnvironment(vmThread);
    ...
    tgcExtensions->printf("CP-FW:  total           | rem-set | copy                                                    "
                          "         | mark\n");
    tgcExtensions->printf("        busy    stall   | stall   | stall   acquire   release   acquire   release    split "
                          "terminate | stall   acquire   release   exchange   split\n");
    tgcExtensions->printf("         (ms)    (ms)   |  (ms)   |  (ms)   freelist  freelist  scanlist  scanlist   arrays "
                          "  (ms)   |  (ms)   packets   packets   packets    arrays\n");

    MM_CopyForwardStats* copyForwardStats
        = &static_cast<MM_CycleStateVLHGC*>(masterEnv->_cycleState)->_vlhgcIncrementStats._copyForwardStats;
    ...
    while ((walkThread = threadListIterator.nextVMThread()) != NULL) {
        ...
        if ((walkThread == vmThread) || (env->getThreadType() == GC_SLAVE_THREAD)) {
            if (env->_copyForwardStats._gcCount == MM_GCExtensions::getExtensions(env)->globalVLHGCStats.gcCount) {
                ...
                tgcExtensions->printf("%4zu:   %5llu   %5llu     %5llu     %5llu    %5zu     %5zu     %5zu     %5zu    "
                                      "%5zu    %5llu     %5llu    %5zu     %5zu     %5zu     %5zu\n",
                    env->getSlaveID(),
                    ...
                    j9time_hires_delta(
                        0, env->_copyForwardStats.getCopyForwardStallTime(), J9PORT_TIME_DELTA_IN_MILLISECONDS),
                    ...
                    j9time_hires_delta(0, env->_copyForwardStats._markStallTime + env->_workPacketStats.getStallTime(),
                        J9PORT_TIME_DELTA_IN_MILLISECONDS),
                    env->_workPacketStats.workPacketsAcquired, env->_workPacketStats.workPacketsReleased,
...
static void tgcHookGlobalGcSweepStart(J9HookInterface** hook, uintptr_t eventNum, void* eventData, void* userData)
{
...
static void tgcHookGlobalGcSweepEnd(J9HookInterface** hook, uintptr_t eventNum, void* eventData, void* userData)
{
...
static void tgcHookConcurrentRSStart(J9HookInterface** hook, uintptr_t eventNum, void* eventData, void* userData)
{
...
static void tgcHookConcurrentRSEnd(J9HookInterface** hook, uintptr_t eventNum, void* eventData, void* userData)
{
...
bool tgcParallelInitialize(J9JavaVM* javaVM)
{
    ...
    (*privateHooks)
        ->J9HookRegisterWithCallSite(
            privateHooks, J9HOOK_MM_PRIVATE_MARK_START, tgcHookGlobalGcMarkStart, OMR_GET_CALLSITE(), NULL);
    (*privateHooks)
        ->J9HookRegisterWithCallSite(
            privateHooks, J9HOOK_MM_PRIVATE_MARK_END, tgcHookGlobalGcMarkEnd, OMR_GET_CALLSITE(), NULL);
    (*privateHooks)
        ->J9HookRegisterWithCallSite(
            privateHooks, J9HOOK_MM_PRIVATE_SWEEP_START, tgcHookGlobalGcSweepStart, OMR_GET_CALLSITE(), NULL);
    (*privateHooks)
        ->J9HookRegisterWithCallSite(
            privateHooks, J9HOOK_MM_PRIVATE_SWEEP_END, tgcHookGlobalGcSweepEnd, OMR_GET_CALLSITE(), NULL);
    (*privateHooks)
        ->J9HookRegisterWithCallSite(privateHooks, J9HOOK_MM_PRIVATE_CONCURRENT_REMEMBERED_SET_SCAN_START,
            tgcHookConcurrentRSStart, OMR_GET_CALLSITE(), NULL);
    (*privateHooks)
        ->J9HookRegisterWithCallSite(privateHooks, J9HOOK_MM_PRIVATE_CONCURRENT_REMEMBERED_SET_SCAN_END,
            tgcHookConcurrentRSEnd, OMR_GET_CALLSITE(), NULL);

    if (extensions->isVLHGC()) {
#if defined(J9VM_GC_VLHGC)
        (*privateHooks)
            ->J9HookRegisterWithCallSite(
                privateHooks, J9HOOK_MM_PRIVATE_COPY_FORWARD_END, tgcHookCopyForwardEnd, OMR_GET_CALLSITE(), NULL);
#endif /* J9VM_GC_VLHGC*/
    }
    (*omrHooks)->J9HookRegisterWithCallSite(
        omrHooks, J9HOOK_MM_OMR_GLOBAL_GC_END, tgcHookGlobalGcEnd, OMR_GET_CALLSITE(), NULL);
    if (extensions->isStandardGC()) {
#if defined(J9VM_GC_MODRON_SCAVENGER)
        (*omrHooks)->J9HookRegisterWithCallSite(
            omrHooks, J9HOOK_MM_OMR_LOCAL_GC_END, tgcHookLocalGcEnd, OMR_GET_CALLSITE(), NULL);
#endif /* J9VM_GC_MODRON_SCAVENGER */
    }
