    ...
    if (!extensions->rootScannerStatsEnabled) {
        ...
        (*mmOmrHooks)
            ->J9HookRegisterWithCallSite(
                mmOmrHooks, J9HOOK_MM_OMR_LOCAL_GC_END, tgcHookGCEnd, OMR_GET_CALLSITE(), NULL);
        (*mmOmrHooks)
            ->J9HookRegisterWithCallSite(
                mmOmrHooks, J9HOOK_MM_OMR_GLOBAL_GC_END, tgcHookGCEnd, OMR_GET_CALLSITE(), NULL);
    }
    ...
    if (extensions->rootScannerStatsUsed) {
        ...
        while (NULL != (thread = threadIterator.nextVMThread())) {
            ...
            if (((GC_SLAVE_THREAD == env->getThreadType()) || (thread == currentThread))
                && env->_rootScannerStats._statsUsed) {
                ...
                for (UDATA entityIndex = 1; entityIndex < RootScannerEntity_Count; entityIndex++) {
                    if (0 != env->_rootScannerStats._entityScanTime[entityIndex]) {
                        U_64 scanTime = j9time_hires_delta(
                            0, env->_rootScannerStats._entityScanTime[entityIndex], J9PORT_TIME_DELTA_IN_MICROSECONDS);

                        tgcExtensions->printf(
                            " %s=\"%llu.%03.3llu\"", attributeNames[entityIndex], scanTime / 1000, scanTime % 1000);
        ...
        for (UDATA entityIndex = 1; entityIndex < RootScannerEntity_Count; entityIndex++) {
            if (0 != entityScanTimeTotal[entityIndex]) {
                U_64 scanTime
                    = j9time_hires_delta(0, entityScanTimeTotal[entityIndex], J9PORT_TIME_DELTA_IN_MICROSECONDS);
