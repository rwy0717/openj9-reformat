 ...
 * (if one exists)
 */
static ClassEntry* findClassInList(ClassEntry* classList, J9Class* clazz)
{
    ClassEntry* mover;
 ...
 */
static ClassEntry* addClassEntry(J9VMThread* vmThread, ClassEntry* classList, J9Class* clazz, UDATA age)
{
    MM_Forge* forge = MM_GCExtensions::getExtensions(vmThread->javaVM)->getForge();

    ClassEntry* newEntry
        = (ClassEntry*)forge->allocate((UDATA)sizeof(ClassEntry), MM_AllocationCategory::DIAGNOSTIC, J9_GET_CALLSITE());
    if (newEntry) {
 ...
 */
static UDATA countObjects(ClassEntry* classEntry)
{
    UDATA count = 0, i;
 ...
 */
static void printHistogram(J9VMThread* vmThread, ClassEntry* classList)
{
    ClassEntry* mover;
 ...
 */
static void deleteClassList(J9VMThread* vmThread, ClassEntry* classList)
{
    MM_Forge* forge = MM_GCExtensions::getExtensions(vmThread->javaVM)->getForge();
 ...
 */
static void tgcHookScavengerReportObjectHistogram(
...
static void tgcHookScavengerFlipSizeHistogram(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    for (UDATA j = 0; j < SCAVENGER_FLIP_HISTORY_SIZE; ++j) {
        ...
        /* Flipped bytes */
        for (UDATA i = 0; i <= OBJECT_HEADER_AGE_MAX + 1; ++i) {
            /* Blank out the top left table elements because data will not be populated for this cell until the next
             * scavenge. */
            if ((0 == j) && (0 == i)) {
                tgcExtensions->printf("           ");
            } else {
        ...
        for (UDATA i = 1; i <= OBJECT_HEADER_AGE_MAX + 1; ++i) {
            if (j < SCAVENGER_FLIP_HISTORY_SIZE - 1) {
                ...
                if (previousFlipBytes != 0) {
                    survivalPercentage
                        = ((double)(currentFlipBytes + currentTenureBytes) / (double)previousFlipBytes) * 100.0;
                }
...
static void tgcHookScavengerDiscardedSpace(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    MM_ScavengeEndEvent* event = (MM_ScavengeEndEvent*)eventData;
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(event->currentThread);
    MM_TgcExtensions* tgcExtensions = MM_TgcExtensions::getExtensions(extensions);
     ...
     * it may indicate that discard threasholds are too low (and possibly causing contention while poping/pushing scan
     * queue
     */
    tgcExtensions->printf("Scavenger flipped=%zu discard=%zu TLHRemainderReuse=%zu\n", stats->_flipBytes,
        stats->_flipDiscardBytes, stats->_survivorTLHRemainderCount);
    tgcExtensions->printf("Scavenger tenured=%zu discard=%zu TLHRemainderReuse=%zu\n", stats->_tenureAggregateBytes,
        stats->_tenureDiscardBytes, stats->_tenureTLHRemainderCount);
}

static void tgcHookScavengerAllocationPaths(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    MM_ScavengeEndEvent* event = (MM_ScavengeEndEvent*)eventData;
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(event->currentThread);
    MM_TgcExtensions* tgcExtensions = MM_TgcExtensions::getExtensions(extensions);
...
bool tgcScavengerInitialize(J9JavaVM* javaVM)
{
    ...
    /*	Object histogram is expensive, but for legacy reasons we still support it. Other, cheaper, scavenger stats have
     * been given special options, but also reported through this generic scavenge tgc options. */
    (*omrHooks)->J9HookRegisterWithCallSite(
        omrHooks, J9HOOK_MM_OMR_LOCAL_GC_END, tgcHookScavengerReportObjectHistogram, OMR_GET_CALLSITE(), NULL);
...
bool tgcScavengerSurvivalStatsInitialize(J9JavaVM* javaVM)
{
    ...
    (*privateHooks)
        ->J9HookRegisterWithCallSite(
            privateHooks, J9HOOK_MM_PRIVATE_SCAVENGE_END, tgcHookScavengerFlipSizeHistogram, OMR_GET_CALLSITE(), NULL);
...
bool tgcScavengerMemoryStatsInitialize(J9JavaVM* javaVM)
{
    ...
    (*privateHooks)
        ->J9HookRegisterWithCallSite(
            privateHooks, J9HOOK_MM_PRIVATE_SCAVENGE_END, tgcHookScavengerDiscardedSpace, OMR_GET_CALLSITE(), NULL);
    (*privateHooks)
        ->J9HookRegisterWithCallSite(
            privateHooks, J9HOOK_MM_PRIVATE_SCAVENGE_END, tgcHookScavengerAllocationPaths, OMR_GET_CALLSITE(), NULL);
