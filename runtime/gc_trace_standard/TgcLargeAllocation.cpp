...
static void tgcFreeMemoryPrintStatsForMemoryPool(OMR_VMThread* omrVMThread, MM_MemoryPool* memoryPool)
{
        ...
        for (IDATA sizeClassIndex = (IDATA)stats->getMaxSizeClasses() - 1; sizeClassIndex >= 0; sizeClassIndex--) {
            ...
            if (0 != count) {
                ...
                MM_FreeEntrySizeClassStats::FrequentAllocation* curr
                    = stats->getFreeEntrySizeClassStats()->getFrequentAllocationHead(sizeClassIndex);
                ...
                tgcExtensions->printf("    %4zu %11zu %8zu %9zu %9zuM (%5.2f%%) %9zuM (%5.2f%%)\n", sizeClassIndex,
                    stats->getSizeClassSizes(sizeClassIndex), count, frequentAllocCount, approxFreeMemory >> 20,
                    100 * (float)approxFreeMemory / (float)memoryPool->getActualFreeMemorySize(),
                    approxTotalFreeMemory >> 20,
                    100 * (float)approxTotalFreeMemory / (float)memoryPool->getActualFreeMemorySize());
            }
        }
        tgcExtensions->printf("    Total Count: %zu (%zu). Approx total free memory: %zuMB (%zuMB)\n", totalCount,
            memoryPool->getActualFreeEntryCount(), approxTotalFreeMemory >> 20,
            memoryPool->getActualFreeMemorySize() >> 20);
        Assert_MM_true(totalCount == memoryPool->getActualFreeEntryCount());
    }
}

static void tgcTlhAllocPrintStatsForMemoryPool(OMR_VMThread* omrVMThread, MM_MemoryPool* memoryPool)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(omrVMThread);
    MM_TgcExtensions* tgcExtensions = MM_TgcExtensions::getExtensions(extensions);
        ...
        for (IDATA sizeClassIndex = (IDATA)stats->getMaxTLHSizeClasses() - 1; sizeClassIndex >= 0; sizeClassIndex--) {
            ...
            if (0 != countTlhAlloc) {
                ...
                tgcExtensions->printf("    %4zu %11zu %8zu %9zuK\n", sizeClassIndex,
                    stats->getSizeClassSizes(sizeClassIndex), countTlhAlloc, approxTlhAllocMemory >> 10);
            }
        }
    }
}

static void tgcFreeMemoryPrintStatsForMemorySubSpace(
    OMR_VMThread* omrVMThread, MM_MemorySubSpace* memorySubSpace, bool globalGC)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(omrVMThread);
    MM_TgcExtensions* tgcExtensions = MM_TgcExtensions::getExtensions(extensions);
        ...
        if ((globalGC
            ...
            || (!globalGC
                   && (LOCALGC_ESTIMATE_FRAGMENTATION
                          == (extensions->estimateFragmentation & LOCALGC_ESTIMATE_FRAGMENTATION)))) {
            ...
            for (IDATA sizeClassIndex = (IDATA)stats->getMaxSizeClasses() - 1; sizeClassIndex >= 0; sizeClassIndex--) {
                ...
                UDATA frequentAllocCount
                    = extensions->freeEntrySizeClassStatsSimulated.getFrequentAllocCount(sizeClassIndex);
                count += frequentAllocCount;
                if (0 != count) {
                    totalCount += count;
                    UDATA approxFreeMemory = (count - frequentAllocCount) * stats->getSizeClassSizes(sizeClassIndex);
                    ...
                    MM_FreeEntrySizeClassStats::FrequentAllocation* curr
                        = extensions->freeEntrySizeClassStatsSimulated.getFrequentAllocationHead(sizeClassIndex);
                    UDATA veryLargeEntrySizeClass = stats->getFreeEntrySizeClassStats()->getVeryLargeEntrySizeClass();
                    while (NULL != curr) {
                        if (sizeClassIndex >= (IDATA)veryLargeEntrySizeClass) {
                            tgcExtensions->printf("    VeryLarge size %8zu count %8zu\n", curr->_size, curr->_count);
...
static void tgcLargeAllocationPrintCurrentStatsForMemoryPool(OMR_VMThread* omrVMThread, MM_MemoryPool* memoryPool)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(omrVMThread);
    MM_TgcExtensions* tgcExtensions = MM_TgcExtensions::getExtensions(extensions);
    ...
    if (NULL != stats) {
        ...
        for (U_16 i = 0; i < spaceSavingGetCurSize(stats->getSpaceSavingSizes()) && i < stats->getMaxAllocateSizes();
             i++) {
            UDATA size = 0;
            UDATA sizeCount = 0;
            ...
            if (i < spaceSavingGetCurSize(stats->getSpaceSavingSizeClasses())) {
                ...
                if (0 != sizeClass) {
                    sizeClassCount
                        = spaceSavingGetKthMostFreqCount(stats->getSpaceSavingSizeClasses(), i + 1) / sizeClass;
                }
            }

            tgcExtensions->printf("    %4zu | %9zu %10zuK %6zu | %9zu %10zuK %6zu\n", i, size, sizeCount * size >> 10,
                sizeCount, sizeClass, sizeClass * sizeClassCount >> 10, sizeClassCount);
        }
    }
}

static void tgcLargeAllocationPrintAverageStatsForMemoryPool(OMR_VMThread* omrVMThread, MM_MemoryPool* memoryPool)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(omrVMThread);
    MM_TgcExtensions* tgcExtensions = MM_TgcExtensions::getExtensions(extensions);
    ...
    if (NULL != stats) {
        ...
        for (U_16 i = 0;
             i < spaceSavingGetCurSize(stats->getSpaceSavingSizesAveragePercent()) && i < stats->getMaxAllocateSizes();
             i++) {

            UDATA sizeForAveragePercent = 0;
            float averagePercent = 0;
            if (i < spaceSavingGetCurSize(stats->getSpaceSavingSizesAveragePercent())) {
                sizeForAveragePercent
                    = (UDATA)spaceSavingGetKthMostFreq(stats->getSpaceSavingSizesAveragePercent(), i + 1);
                averagePercent = stats->convertPercentUDATAToFloat(
                    spaceSavingGetKthMostFreqCount(stats->getSpaceSavingSizesAveragePercent(), i + 1));
            }
            ...
            if (i < spaceSavingGetCurSize(stats->getSpaceSavingSizeClassesAveragePercent())) {
                sizeClassForAveragePercent
                    = (UDATA)spaceSavingGetKthMostFreq(stats->getSpaceSavingSizeClassesAveragePercent(), i + 1);
                sizeClassAveragePercent = stats->convertPercentUDATAToFloat(
                    spaceSavingGetKthMostFreqCount(stats->getSpaceSavingSizeClassesAveragePercent(), i + 1));
            }

            tgcExtensions->printf("    %4zu | %9zu %7.4f%% | %9zu %7.4f%%\n", i, sizeForAveragePercent, averagePercent,
                sizeClassForAveragePercent, sizeClassAveragePercent);
        }
    }
}

static void tgcLargeAllocationPrintCurrentStatsForMemorySubSpace(
    OMR_VMThread* omrVMThread, MM_MemorySubSpace* memorySubSpace)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(omrVMThread);
    MM_TgcExtensions* tgcExtensions = MM_TgcExtensions::getExtensions(extensions);

    /* we fetch directly the stats from the SubSpace. It will know if it needs to merge stats from child
     * SubSpaces/Pools, or fetch directly from child Pool */
    MM_LargeObjectAllocateStats* stats = memorySubSpace->getLargeObjectAllocateStats();
...
static void tgcLargeAllocationPrintAverageStatsForMemorySubSpace(
    OMR_VMThread* omrVMThread, MM_MemorySubSpace* memorySubSpace)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(omrVMThread);
    MM_TgcExtensions* tgcExtensions = MM_TgcExtensions::getExtensions(extensions);
    ...
    if (NULL != stats) {
        ...
        for (U_16 i = 0;
             i < spaceSavingGetCurSize(stats->getSpaceSavingSizesAveragePercent()) && i < stats->getMaxAllocateSizes();
             i++) {
            UDATA sizeForAveragePercent
                = (UDATA)spaceSavingGetKthMostFreq(stats->getSpaceSavingSizesAveragePercent(), i + 1);
            float averagePercent = stats->convertPercentUDATAToFloat(
                spaceSavingGetKthMostFreqCount(stats->getSpaceSavingSizesAveragePercent(), i + 1));

            UDATA sizeClassForAveragePercent
                = (UDATA)spaceSavingGetKthMostFreq(stats->getSpaceSavingSizeClassesAveragePercent(), i + 1);
            float sizeClassAveragePercent = stats->convertPercentUDATAToFloat(
                spaceSavingGetKthMostFreqCount(stats->getSpaceSavingSizeClassesAveragePercent(), i + 1));
...
static void tgcLargeAllocationPrintStatsForAllocateMemory(OMR_VMThread* omrVMThread)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(omrVMThread);
    MM_TgcExtensions* tgcExtensions = MM_TgcExtensions::getExtensions(extensions);
    ...
    if (defaultMemorySubspace->isPartOfSemiSpace()) {
        ...
        /* We are in generational configuration. defaultMemorySubspace points to one of two allocate Subspaces, but we
         * want both. Thus we access the parent (SemiSpace subspace) */
...
static void tgcLargeAllocationPrintAverageStatsForTenureMemory(OMR_VMThread* omrVMThread, UDATA eventNum)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(omrVMThread);
    MM_TgcExtensions* tgcExtensions = MM_TgcExtensions::getExtensions(extensions);
    MM_EnvironmentBase* env = MM_EnvironmentBase::getEnvironment(omrVMThread);
    ...
    tgcExtensions->printf(
        "Tenure subspace: %llx (%s) - AVERAGE\n", tenureMemorySubspace, tenureMemorySubspace->getName());
    tgcLargeAllocationPrintAverageStatsForMemorySubSpace(omrVMThread, tenureMemorySubspace);

    tgcExtensions->printf("=============================================\n");
}

static void tgcFreeMemoryPrintStats(OMR_VMThread* omrVMThread, bool globalGC)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(omrVMThread);
    MM_TgcExtensions* tgcExtensions = MM_TgcExtensions::getExtensions(extensions);
    MM_EnvironmentBase* env = MM_EnvironmentBase::getEnvironment(omrVMThread);
...
static void tgcEstimateFragmentationPrintStats(OMR_VMThread* omrVMThread)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(omrVMThread);
    MM_TgcExtensions* tgcExtensions = MM_TgcExtensions::getExtensions(extensions);
    ...
    tgcExtensions->printf(
        "<tgcLargeAllocation op=\"estimateFragmentation\" timems=\"%llu.%03.3llu\" cputimes=\"%llu.%03.3llu\" "
        "remainingFreeMemory=\"%zu\" initialFreeMemory=\"%zu\" timestamp=\"%s\" />\n",
        time / 1000, time % 1000, cpuTime / 1000, cpuTime % 1000, stats->getRemainingFreeMemoryAfterEstimate(),
        stats->getFreeMemoryBeforeEstimate(), timestamp);
}

/**
 * print time for mergeLargeAllocateStats, mergeTLH, mergeFreeEntry and averageLargeAllocateStats
 */
static void tgcMergeAveragePrintStats(OMR_VMThread* omrVMThread)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(omrVMThread);
    MM_TgcExtensions* tgcExtensions = MM_TgcExtensions::getExtensions(extensions);
    MM_MemorySpace* defaultMemorySpace = extensions->heap->getDefaultMemorySpace();
...
static void tgcVerifyBackoutInScavenger(J9VMThread* vmThread)
{
    ...
    if (scavengerStats->_backout) {
        tgcExtensions->printf("<tgcLargeAllocation op=\"gc\" type=\"scavenge\" details=\"aborted collection due to "
                              "insufficient free space\" />\n");
    }
}

static void tgcHookLargeAllocationGlobalPrintStats(
...
static void tgcHookLargeAllocationLocalPrintStats(
    J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
...
static void tgcHookFreeMemoryGlobalPrintStats(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    MM_GlobalGCEndEvent* event = (MM_GlobalGCEndEvent*)eventData;
...
static void tgcHookFreeMemoryLocalPrintStats(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    MM_LocalGCEndEvent* event = (MM_LocalGCEndEvent*)eventData;
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(event->currentThread);
    MM_TgcExtensions* tgcExtensions = MM_TgcExtensions::getExtensions(extensions);
...
static void tgcVerifyHaltedInConcurrentGC(J9VMThread* vmThread, const char* state, const char* status)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(vmThread);
    MM_TgcExtensions* tgcExtensions = MM_TgcExtensions::getExtensions(extensions);
    tgcExtensions->printf("<tgcLargeAllocation op=\"gc\" type=\"concurrent\" details=\"concurrent-halted\" "
                          "state=\"%s\" status=\"%s\" />\n",
        state, status);
}

static void tgcHookVerifyHaltedInConcurrentGC(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
...
#undef CONCURRENT_STATUS_BUFFER_LENGTH
    ...
    tgcVerifyHaltedInConcurrentGC(
        static_cast<J9VMThread*>(event->currentThread->_language_vmthread), stateString, statusString);
}
...
bool tgcLargeAllocationInitialize(J9JavaVM* javaVM)
{
    ...
    /*TODO: remove condition check for concurrentSweep, currently tgc:largeAllocation can not be enabled in
     * concurrentSweep case   */
    if (extensions->processLargeAllocateStats && extensions->isStandardGC()) {
        if (!extensions->isConcurrentSweepEnabled()) {
            ...
            if (tgcExtensions->_largeAllocationVerboseRequested) {
                (*omrHooks)->J9HookRegisterWithCallSite(omrHooks, J9HOOK_MM_OMR_GLOBAL_GC_START,
                    tgcHookLargeAllocationGlobalPrintStats, OMR_GET_CALLSITE(), NULL);
                (*omrHooks)->J9HookRegisterWithCallSite(omrHooks, J9HOOK_MM_OMR_LOCAL_GC_START,
                    tgcHookLargeAllocationLocalPrintStats, OMR_GET_CALLSITE(), NULL);
                (*omrHooks)->J9HookRegisterWithCallSite(omrHooks, J9HOOK_MM_OMR_LOCAL_GC_END,
                    tgcHookLargeAllocationLocalPrintStats, OMR_GET_CALLSITE(), NULL);
            }

            (*omrHooks)->J9HookRegisterWithCallSite(
                omrHooks, J9HOOK_MM_OMR_GLOBAL_GC_END, tgcHookFreeMemoryGlobalPrintStats, OMR_GET_CALLSITE(), NULL);
            (*omrHooks)->J9HookRegisterWithCallSite(
                omrHooks, J9HOOK_MM_OMR_LOCAL_GC_END, tgcHookFreeMemoryLocalPrintStats, OMR_GET_CALLSITE(), NULL);

            J9HookInterface** privateHooks = J9_HOOK_INTERFACE(extensions->privateHookInterface);
            (*privateHooks)
                ->J9HookRegisterWithCallSite(privateHooks, J9HOOK_MM_PRIVATE_CONCURRENT_HALTED,
                    tgcHookVerifyHaltedInConcurrentGC, OMR_GET_CALLSITE(), NULL);
        }
    }
    return result;
}
