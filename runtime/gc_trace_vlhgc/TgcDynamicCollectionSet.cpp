...
private:
    ...
    bool
        _historyPrimed; /**< Flag indicating if the decay analysis history has been primed with initial values or not */
...
MM_TgcDynamicCollectionSetData* MM_TgcDynamicCollectionSetData::newInstance(J9JavaVM* javaVM)
{
    MM_TgcDynamicCollectionSetData* dcsData = NULL;

    dcsData = (MM_TgcDynamicCollectionSetData*)MM_GCExtensions::getExtensions(javaVM)->getForge()->allocate(
        sizeof(MM_TgcDynamicCollectionSetData), MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
    if (NULL != dcsData) {
...
void MM_TgcDynamicCollectionSetData::kill(J9JavaVM* javaVM)
{
    tearDown(javaVM);
    MM_GCExtensions::getExtensions(javaVM)->getForge()->free(this);
}

bool MM_TgcDynamicCollectionSetData::initialize(J9JavaVM* javaVM)
{
    PORT_ACCESS_FROM_JAVAVM(javaVM);
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(javaVM);

    for (UDATA index = 0; index < MM_TGC_DYNAMIC_COLLECTION_SET_HISTORY_TABLE_DEPTH; index++) {
        _historyTable[index] = (MM_TgcDynamicCollectionSetHistoryElement*)j9mem_allocate_memory(
            sizeof(MM_TgcDynamicCollectionSetHistoryElement) * (extensions->tarokRegionMaxAge + 1), OMRMEM_CATEGORY_MM);
        if (NULL == _historyTable[index]) {
            goto error_no_memory;
        }
        memset(_historyTable[index], 0,
            sizeof(MM_TgcDynamicCollectionSetHistoryElement) * (extensions->tarokRegionMaxAge + 1));
    }
...
void MM_TgcDynamicCollectionSetData::tearDown(J9JavaVM* javaVM)
{
    PORT_ACCESS_FROM_JAVAVM(javaVM);
...
void MM_TgcDynamicCollectionSetData::decayPrintValue(MM_EnvironmentVLHGC* env, UDATA value)
{
...
void MM_TgcDynamicCollectionSetData::decayPrintDelta(MM_EnvironmentVLHGC* env, UDATA before, UDATA after)
{
    MM_TgcExtensions* tgcExtensions = MM_TgcExtensions::getExtensions(MM_GCExtensions::getExtensions(env));
...
void MM_TgcDynamicCollectionSetData::dumpDynamicCollectionSetStatistics(MM_EnvironmentVLHGC* env)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
    MM_TgcExtensions* tgcExtensions = MM_TgcExtensions::getExtensions(extensions);

    MM_CollectionSetDelegate::SetSelectionData* setSelectionDataTable
        = (MM_CollectionSetDelegate::SetSelectionData*)extensions->tarokTgcSetSelectionDataTable;

    /* Right now we'll just gather results and dump them out */

    /* Rotate the table depth such that "0" is the most recent */
    MM_TgcDynamicCollectionSetHistoryElement* currentHistoryTable
        = _historyTable[MM_TGC_DYNAMIC_COLLECTION_SET_HISTORY_TABLE_DEPTH - 1];
    ...
    memset(
        currentHistoryTable, 0, sizeof(MM_TgcDynamicCollectionSetHistoryElement) * (extensions->tarokRegionMaxAge + 1));
    ...
    for (UDATA ageIndex = 0; ageIndex <= extensions->tarokRegionMaxAge; ageIndex++) {
        ...
        currentHistoryTable[ageIndex].avgHis_regionCount
            += ((double)currentHistoryTable[ageIndex].regionCount) * weight;

        currentHistoryTable[ageIndex].avgLcl_regionCount = _historyTable[1][ageIndex].avgLcl_regionCount / (double)2.0;
        currentHistoryTable[ageIndex].avgLcl_regionCount
            += ((double)currentHistoryTable[ageIndex].regionCount) / (double)2.0;
    }
    ...
    for (UDATA compactGroupIndex = 0; compactGroupIndex < compactGroupCount; compactGroupIndex++) {
        tgcExtensions->printf(
            " %5zu", setSelectionDataTable[compactGroupIndex]._reclaimStats._regionCountArrayletLeafBefore);
    }
    tgcExtensions->printf("\n RgCtA  ");
    for (UDATA compactGroupIndex = 0; compactGroupIndex < compactGroupCount; compactGroupIndex++) {
    ...
    for (UDATA compactGroupIndex = 0; compactGroupIndex < compactGroupCount; compactGroupIndex++) {
        tgcExtensions->printf(
            " %5zu", setSelectionDataTable[compactGroupIndex]._reclaimStats._regionCountArrayletLeafAfter);
    }
    ...
    for (UDATA compactGroupIndex = 0; compactGroupIndex < compactGroupCount; compactGroupIndex++) {
        tgcExtensions->printf(
            " %5zu", setSelectionDataTable[compactGroupIndex]._reclaimStats._regionCountArrayletLeafOverflow);
    }

    tgcExtensions->printf("\n RcRgB  ");
    for (UDATA compactGroupIndex = 0; compactGroupIndex < compactGroupCount; compactGroupIndex++) {
        tgcExtensions->printf(
            " %5zu", setSelectionDataTable[compactGroupIndex]._reclaimStats._reclaimableRegionCountBefore);
    }
    tgcExtensions->printf("\n  RcLfB ");
    for (UDATA compactGroupIndex = 0; compactGroupIndex < compactGroupCount; compactGroupIndex++) {
        tgcExtensions->printf(
            " %5zu", setSelectionDataTable[compactGroupIndex]._reclaimStats._reclaimableRegionCountArrayletLeafBefore);
    }
    tgcExtensions->printf("\n RcRgA  ");
    for (UDATA compactGroupIndex = 0; compactGroupIndex < compactGroupCount; compactGroupIndex++) {
        tgcExtensions->printf(
            " %5zu", setSelectionDataTable[compactGroupIndex]._reclaimStats._reclaimableRegionCountAfter);
    }
    tgcExtensions->printf("\n  RcLfA ");
    for (UDATA compactGroupIndex = 0; compactGroupIndex < compactGroupCount; compactGroupIndex++) {
        tgcExtensions->printf(
            " %5zu", setSelectionDataTable[compactGroupIndex]._reclaimStats._reclaimableRegionCountArrayletLeafAfter);
    }
 ...
 */
static void tgcHookReportDynamicCollectionSetStatistics(
    J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (MM_GCExtensions::getExtensions(env)->tarokEnableDynamicCollectionSetSelection) {
        MM_TgcExtensions* tgcExtensions = MM_TgcExtensions::getExtensions(MM_GCExtensions::getExtensions(env));
        MM_TgcDynamicCollectionSetData* dcsData
            = (MM_TgcDynamicCollectionSetData*)tgcExtensions->_dynamicCollectionSetData;
 ...
 */
void dumpLegend(J9JavaVM* javaVM)
{
    ...
    tgcExtensions->printf(
        "\n AvgHxx  - Half-life average number of regions found in the age group for the last %zu iterations",
        (UDATA)MM_TGC_DYNAMIC_COLLECTION_SET_HISTORY_TABLE_DEPTH);

    tgcExtensions->printf("\nROR      - Rate of return when collecting (trace) the region set (amount of currently "
                          "consumed data that is rendered reclaimable)");
...
bool tgcDynamicCollectionSetInitialize(J9JavaVM* javaVM)
{
    ...
    if (NULL != tgcExtensions->_dynamicCollectionSetData) {
        ...
        (*privateHooks)
            ->J9HookRegisterWithCallSite(privateHooks, J9HOOK_MM_PRIVATE_VLHGC_GARBAGE_COLLECT_COMPLETED,
                tgcHookReportDynamicCollectionSetStatistics, OMR_GET_CALLSITE(), NULL);
        dumpLegend(javaVM);
    } else {
        result = false;
    }
