 ...
 */
static void tgcHookReportInterRegionReferenceCounting(
    J9HookInterface** hookInterface, UDATA eventNum, void* eventData, void* userData)
{
    ...
    while (NULL != (region = regionIterator.nextRegion())) {
        if (region->containsObjects()) {
            ...
            while (NULL != (object = iterator.nextObject())) {
                ...
                case GC_ObjectModel::SCAN_CLASSLOADER_OBJECT: {
                    ...
                    while (NULL != (slotObject = mixedObjectIterator.nextSlot())) {
                        ...
                        if (NULL != target) {
                            MM_HeapRegionDescriptorVLHGC* dest
                                = (MM_HeapRegionDescriptorVLHGC*)regionManager->tableDescriptorForAddress(target);
                            if (dest == region) {
                                inSlotCount += 1;
                                thisObjectIn += 1;
                    ...
                    /* note that this will double-count objects which point both out _and_ beyond but that data is
                     * considered more valuable than only counting it as one type of external reference */
                    if (doesPointOut) {
                        outObjectCount += 1;
                    }
                    if (doesPointBeyond) {
            ...
            j9tty_printf(PORTLIB,
                "Region based %p (group %zu) has %zu objects.  %zu \"in\" slots, %zu \"out\" slots (%zu objects), %zu "
                "\"beyond\" slots (%zu objects).  Creates %zu out/beyond cards\n",
                regionBase, regionCompactGroup, objectCount, inSlotCount, outSlotCount, outObjectCount, beyondSlotCount,
                beyondObjectCount, cardCount);
    ...
    j9tty_printf(PORTLIB,
        "Total heap has %zu objects.  %zu \"in\" slots, %zu \"out\" slots (%zu objects), %zu \"beyond\" slots (%zu "
        "objects).  Creates %zu out/beyond cards\n",
        heapObjectCount, heapInSlotCount, heapOutSlotCount, heapOutObjectCount, heapBeyondSlotCount,
        heapBeyondObjectCount, heapCardCount);
}
...
bool tgcInterRegionReferencesInitialize(J9JavaVM* javaVM)
{
    ...
    (*privateHooks)
        ->J9HookRegisterWithCallSite(privateHooks, J9HOOK_MM_PRIVATE_COPY_FORWARD_END,
            tgcHookReportInterRegionReferenceCounting, OMR_GET_CALLSITE(), javaVM);
    PORT_ACCESS_FROM_JAVAVM(javaVM);
    j9tty_printf(PORTLIB,
        "TGC inter-region references initialized.\nLegend:\n"
        "\t\"in\" slot refers to a slot which points into the same region which contains its object\n"
        "\t\"out\" slot refers to a slot which points into a different region from that which contains its object, yet "
        "is in the same compact group\n"
        "\t\"beyond\" slot refers to a slot which points into a different region from that which contains its object "
        "which is also in a different compact group\n"
        "\t\"out\" or \"beyond\" objects contain \"out\" or \"beyond\" slots, respectively (an object which contains "
        "both will be in both totals)\n");
