...
enum {
    DISTINCT_FLAG_ARRAY_SHIFT = 4 /* for duplicate/distinct card list count we used a hash table, this is a factor which
                                     tells how hash table is big relative to RSCL */
};

static UDATA getUpperBoundDuplicatesCount(MM_EnvironmentVLHGC* env, MM_RememberedSetCardList* rscl)
{
    ...
    if (MM_GCExtensions::getExtensions(env)->tarokTgcEnableRememberedSetDuplicateDetection) {
        ...
        if (NULL != tgcExtensions->_rsclDistinctFlagArray) {
            ...
            while (NULL != (cardAddress = rsclCardIterator.nextReferencingCardHeapAddress(env))) {
                /* card addresses are CARD_SIZE bytes aligned, thus stripping low bits that are 0s (use the shift value
 ...
 */
static void calculateAndPrintHistogram(J9VMThread* vmThread, MM_HeapRegionManager* regionManager,
    const char* eventString, UDATA totalReferences, UDATA maxReferences)
{
    ...
    double totalPercent
        = ((double)totalReferences / ((double)regionCount * (double)extensions->tarokRememberedSetCardListSize))
        * 100.0;
    ...
    if (maxReferences > 0) {
        ...
        tgcExtensions->printf("{RSCL: Histogram of reference cards TO regions after %s. Duplicates percentage in "
                              "brackets left to the region count. }\n{RSCL: Refs     ",
            eventString);
        ...
        for (UDATA age = 0; age <= extensions->tarokRegionMaxAge; age++) {
            for (i = 0; i < regionCount; i++) {
                MM_HeapRegionDescriptorVLHGC* region
                    = (MM_HeapRegionDescriptorVLHGC*)regionManager->physicalTableDescriptorForIndex(i);
 ...
 */
static void tgcHookReportInterRegionRememberedSetHistogram(
    J9HookInterface** hookInterface, UDATA eventNum, void* eventData, void* userData)
{
     ...
     */
    if (NULL == tgcExtensions->_rsclDistinctFlagArray) {
        tgcExtensions->_rsclDistinctFlagArray = (U_8*)extensions->getForge()->allocate(
            extensions->tarokRememberedSetCardListMaxSize << DISTINCT_FLAG_ARRAY_SHIFT, MM_AllocationCategory::FIXED,
            J9_GET_CALLSITE());
    }
...
bool tgcInterRegionRememberedSetInitialize(J9JavaVM* javaVM)
{
    ...
    (*privateHooks)
        ->J9HookRegisterWithCallSite(privateHooks, J9HOOK_MM_PRIVATE_MARK_END,
            tgcHookReportInterRegionRememberedSetHistogram, OMR_GET_CALLSITE(), javaVM);
    (*omrHooks)->J9HookRegisterWithCallSite(omrHooks, J9HOOK_MM_OMR_COMPACT_END,
        tgcHookReportInterRegionRememberedSetHistogram, OMR_GET_CALLSITE(), javaVM);
    (*privateHooks)
