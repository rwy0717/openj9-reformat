...
struct ClassTableEntry {
    ...
    static UDATA equal(void* leftKey, void* rightKey, void* userData)
    {
        return ((ClassTableEntry*)leftKey)->clazz == ((ClassTableEntry*)rightKey)->clazz;
    }
};
...
void TgcParallelHeapWalkTask::run(MM_EnvironmentBase* env)
{
    UDATA errorCount = 0;
    ...
    J9HashTable* hashTable = hashTableNew(env->getPortLibrary(), J9_GET_CALLSITE(), 8 * 1024, sizeof(ClassTableEntry),
        sizeof(char*), 0, OMRMEM_CATEGORY_MM, ClassTableEntry::hash, ClassTableEntry::equal, NULL, NULL);
        ...
        while (NULL != (region = regionIterator.nextRegion())) {
            if (region->hasValidMarkMap()) {
                if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
                    MM_HeapMapIterator mapIterator(
                        extensions, markMap, (UDATA*)region->getLowAddress(), (UDATA*)region->getHighAddress(), false);
...
static void reportInterRegionRememberedSetDemographics(MM_EnvironmentBase* env)
{
    ...
    /* at this point, the helper threads have completed populating the shared hash table with the all classes with
    ...
    for (UDATA index = 0; index < listCount; index++) {
        ...
        if (NULL != clazz) {
            ...
            tgcExtensions->printf(
                "%8zu %.2f %.2f %p ", entry->rememberedInstances, fractionRemembered, fractionOfAllRemembered, clazz);
...
bool tgcInterRegionRememberedSetDemographicsInitialize(J9JavaVM* javaVM)
{
    ...
    J9HashTable* hashTable
        = hashTableNew(OMRPORT_FROM_J9PORT(javaVM->portLibrary), J9_GET_CALLSITE(), 8 * 1024, sizeof(ClassTableEntry),
            sizeof(char*), 0, OMRMEM_CATEGORY_MM, ClassTableEntry::hash, ClassTableEntry::equal, NULL, NULL);
    tgcExtensions->_interRegionRememberedSetDemographics.classHashTable = hashTable;

    if (NULL != hashTable) {
        if (0
            == omrthread_monitor_init_with_name(&tgcExtensions->_interRegionRememberedSetDemographics.mutex, 0,
                   "InterRegionRememberedSetDemographics")) {
            J9HookInterface** privateHooks = J9_HOOK_INTERFACE(extensions->privateHookInterface);
            (*privateHooks)
                ->J9HookRegisterWithCallSite(privateHooks, J9HOOK_MM_PRIVATE_GC_INCREMENT_START, tgcHookIncrementStart,
