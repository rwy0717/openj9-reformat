 ...
 */
static void tgcHookReportProjectedStatsStatistics(
...
static void reportAverageAbsoluteDeviationForCompactGroups(MM_EnvironmentVLHGC* env)
{
    ...
    for (UDATA compactGroupIndex = 0; compactGroupIndex < compactGroupCount; compactGroupIndex++) {
        if (MM_CompactGroupManager::getRegionAgeFromGroup(env, compactGroupIndex) == 0) {
            tgcExtensions->printf("\nAvAbDv  ");
            UDATA allocationContextNumber
                = MM_CompactGroupManager::getAllocationContextNumberFromGroup(env, compactGroupIndex);
    ...
    if (0
        != (*privateHooks)
               ->J9HookRegisterWithCallSite(privateHooks, J9HOOK_MM_PRIVATE_VLHGC_GARBAGE_COLLECT_COMPLETED,
                   tgcHookReportProjectedStatsStatistics, OMR_GET_CALLSITE(), NULL)) {
        result = false;
    }
