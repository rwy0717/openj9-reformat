 ...
 */
static void tgcHookCompactEnd(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    while ((walkThread = compactionThreadListIterator.nextVMThread()) != NULL) {
        ...
        if ((walkThread == vmThread) || (env->getThreadType() == GC_SLAVE_THREAD)) {
            ...
            tgcExtensions->printf("Compact(%zu): Thread %zu, setup stage: %llu ms.\n", gcCount, slaveID,
                j9time_hires_delta(stats->_setupStartTime, stats->_setupEndTime, J9PORT_TIME_DELTA_IN_MILLISECONDS));
            tgcExtensions->printf(
                "Compact(%zu): Thread %zu, move stage: handled %zu objects in %llu ms, bytes moved %zu.\n", gcCount,
                slaveID, stats->_movedObjects,
                j9time_hires_delta(stats->_moveStartTime, stats->_moveEndTime, J9PORT_TIME_DELTA_IN_MILLISECONDS),
                stats->_movedBytes);
            tgcExtensions->printf(
                "Compact(%zu): Thread %zu, fixup stage: handled %zu objects in %zu ms, root fixup time %zu ms.\n",
                ...
                j9time_hires_delta(
                    stats->_rootFixupStartTime, stats->_rootFixupEndTime, J9PORT_TIME_DELTA_IN_MILLISECONDS));
    ...
    tgcExtensions->printf("Compact(%zu): Summary, move stage: handled %zu (min=%zu, max=%zu, stddev=%.2f) objects, "
                          "bytes moved %zu (min=%zu, max=%zu, stddev=%.2f).\n",
        gcCount, movedObjectsTotal, movedObjectsMin, movedObjectsMax, sqrt(movedObjectsSquareSum / threadCount),
    ...
    tgcExtensions->printf("Compact(%zu): Summary, fixup stage: handled %zu (min=%zu, max=%zu, stddev=%.2f) objects.\n",
        gcCount, fixupTotal, fixupMin, fixupMax, sqrt(fixupSquareSum / threadCount));
}
...
bool tgcWriteOnceCompactionInitialize(J9JavaVM* javaVM)
{
    ...
    (*omrHooks)->J9HookRegisterWithCallSite(
        omrHooks, J9HOOK_MM_OMR_COMPACT_END, tgcHookCompactEnd, OMR_GET_CALLSITE(), NULL);
