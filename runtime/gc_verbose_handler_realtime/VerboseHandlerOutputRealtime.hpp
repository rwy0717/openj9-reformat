...
private:
    ...
    UDATA _classLoadersUnloadedTotal; /**< Sum of _classLoadersUnloaded from all events between two heartbeat verbose gc
                                         reports */
    UDATA
        _classesUnloadedTotal; /**< Sum of _classesUnloaded from all events between two heartbeat verbose gc reports */
    UDATA _anonymousClassesUnloadedTotal; /**< Sum of _anonymousClassesUnloaded from all events between two heartbeat
                                             verbose gc reports */
    ...
    UDATA _phantomReferenceClearCountTotal; /**< Number of phantom references between two heartbeats (total of all
                                               quantas) */

    UDATA _finalizableCountTotal; /**< Number of finalizable objects  between two heartbeats (total of all quantas) */

    UDATA _workPacketOverflowCountTotal; /**< Sum of _workPacketCount from all events between two heartbeat verbose gc
                                            reports */
    UDATA _objectOverflowCountTotal; /**< Sum of _objectOverflowCount from all events between two heartbeat verbose gc
                                        reports */

    UDATA _nonDeterministicSweepTotal; /**< Number of non deterministic sweeps between two heartbeats (total of all
                                          quantas) */
    UDATA _nonDeterministicSweepConsecutiveMax; /**< The maximum of all longest streaks, between two heartbeats */
    U_64 _nonDeterministicSweepDelayMax; /**< The longest delay caused by non deterministic sweep among all allocs
                                            between two heartbeats */

    U_64 _maxExclusiveAccessTime; /**< The maximum time (of all events of the chain) GC master thread spent stopping
                                     mutators */
    U_64 _minExclusiveAccessTime; /**< The minimum time (of all events of the chain) GC master thread spent stopping
                                     mutators */
    U_64 _totalExclusiveAccessTime; /**< The average time (of all events of the chain) GC master thread spent stopping
                                       mutators */

    UDATA _maxStartPriority; /**< The maximum start priority of all increments in the event chain. Only used for the
                                last event in the chain. */
    UDATA _minStartPriority; /**< The minimum start priority of all increments in the event chain. Only used for the
                                last event in the chain. */

    typedef enum { INACTIVE = 0, PRE_COLLECT, MARK, CLASS_UNLOAD, SWEEP, POST_COLLECT } GCPhase;

    GCPhase _gcPhase; /**< Current GC phase. */
    GCPhase _previousGCPhase; /**< The GC phase at the start of the current increment. Different from the current
                                 gcPhase if current increment spans across more than one gc phase. */
    ...
    MM_LightweightNonReentrantLock
        _outputLock; /**< Since VLHGC can sometimes have multiple threads trying to access verbose (one reporting events
                        while another reports exclusive), this lock is required to ensure that the writer only sees each
                        report, atomically */
...
private:
    ...
    const char* getGCPhaseAsString(GCPhase gcPhase)
    {
        switch (gcPhase) {
        case PRE_COLLECT:
            return "precollect";
     ...
     */
    bool incrementStartsInPreviousGCPhase() { return (_gcPhase != _previousGCPhase); }
