...
MM_VerboseHandlerOutput* MM_VerboseHandlerOutputStandardJava::newInstance(
    MM_EnvironmentBase* env, MM_VerboseManager* manager)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env->getOmrVM());

    MM_VerboseHandlerOutputStandardJava* verboseHandlerOutput
        = (MM_VerboseHandlerOutputStandardJava*)extensions->getForge()->allocate(
            sizeof(MM_VerboseHandlerOutputStandardJava), MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
...
bool MM_VerboseHandlerOutputStandardJava::initialize(MM_EnvironmentBase* env, MM_VerboseManager* manager)
{
...
void MM_VerboseHandlerOutputStandardJava::tearDown(MM_EnvironmentBase* env)
{
    MM_VerboseHandlerOutputStandard::tearDown(env);
}

void MM_VerboseHandlerOutputStandardJava::outputMemoryInfoInnerStanzaInternal(
...
void MM_VerboseHandlerOutputStandardJava::enableVerbose()
{
...
#endif /* defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING) */
    (*_vmHooks)->J9HookRegisterWithCallSite(
        _vmHooks, J9HOOK_VM_SLOW_EXCLUSIVE, verboseHandlerSlowExclusive, OMR_GET_CALLSITE(), (void*)this);
}

void MM_VerboseHandlerOutputStandardJava::disableVerbose()
{
...
bool MM_VerboseHandlerOutputStandardJava::getThreadName(char* buf, UDATA bufLen, OMR_VMThread* vmThread)
{
    return MM_VerboseHandlerJava::getThreadName(buf, bufLen, vmThread);
}

void MM_VerboseHandlerOutputStandardJava::writeVmArgs(MM_EnvironmentBase* env)
{
    MM_VerboseHandlerJava::writeVmArgs(_manager, env, static_cast<J9JavaVM*>(_omrVM->_language_vm));
}

void MM_VerboseHandlerOutputStandardJava::outputUnfinalizedInfo(
    MM_EnvironmentBase* env, UDATA indent, UDATA unfinalizedCandidates, UDATA unfinalizedEnqueuedCount)
{
    if (0 != unfinalizedCandidates) {
        _manager->getWriterChain()->formatAndOutput(env, indent, "<finalization candidates=\"%zu\" enqueued=\"%zu\" />",
            unfinalizedCandidates, unfinalizedEnqueuedCount);
    }
}

void MM_VerboseHandlerOutputStandardJava::outputOwnableSynchronizerInfo(
...
void MM_VerboseHandlerOutputStandardJava::outputReferenceInfo(MM_EnvironmentBase* env, UDATA indent,
    const char* referenceType, MM_ReferenceStats* referenceStats, UDATA dynamicThreshold, UDATA maxThreshold)
{
...
void MM_VerboseHandlerOutputStandardJava::handleMarkEndInternal(MM_EnvironmentBase* env, void* eventData)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env->getOmrVM());
    ...
    outputOwnableSynchronizerInfo(
        env, 1, markJavaStats->_ownableSynchronizerCandidates, markJavaStats->_ownableSynchronizerCleared);

    outputReferenceInfo(env, 1, "soft", &markJavaStats->_softReferenceStats,
...
void MM_VerboseHandlerOutputStandardJava::handleSlowExclusive(J9HookInterface** hook, UDATA eventNum, void* eventData)
{
    ...
    writer->formatAndOutput(env, 0,
        "<warning details=\"slow exclusive request due to %s\" threadname=\"%s\" timems=\"%zu\" />",
        (event->reason == 1) ? "JNICritical" : "Exclusive Access", threadName, event->timeTaken);
    writer->flush(env);
    exitAtomicReportingBlock();
}

#if defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING)
void MM_VerboseHandlerOutputStandardJava::handleClassUnloadEnd(J9HookInterface** hook, UDATA eventNum, void* eventData)
{
    ...
    bool deltaTimeSuccess
        = getTimeDeltaInMicroSeconds(&duration, classUnloadStats->_startTime, classUnloadStats->_endTime);

    enterAtomicReportingBlock();
    handleGCOPOuterStanzaStart(env, "classunload", env->_cycleState->_verboseContextID, duration, deltaTimeSuccess);

    U_64 setupTime = j9time_hires_delta(
        classUnloadStats->_startSetupTime, classUnloadStats->_endSetupTime, J9PORT_TIME_DELTA_IN_MICROSECONDS);
    U_64 scanTime = j9time_hires_delta(
        classUnloadStats->_startScanTime, classUnloadStats->_endScanTime, J9PORT_TIME_DELTA_IN_MICROSECONDS);
    U_64 postTime = j9time_hires_delta(
        classUnloadStats->_startPostTime, classUnloadStats->_endPostTime, J9PORT_TIME_DELTA_IN_MICROSECONDS);
    ...
    writer->formatAndOutput(env, 1,
        ...
        classUnloadStats->_classLoaderCandidates, classUnloadStats->_classLoaderUnloadedCount,
        classUnloadStats->_classesUnloadedCount, classUnloadStats->_anonymousClassesUnloadedCount,
        classUnloadStats->_classUnloadMutexQuiesceTime / 1000, classUnloadStats->_classUnloadMutexQuiesceTime % 1000,
        setupTime / 1000, setupTime % 1000, scanTime / 1000, scanTime % 1000, postTime / 1000, postTime % 1000);
...
void MM_VerboseHandlerOutputStandardJava::handleScavengeEndInternal(MM_EnvironmentBase* env, void* eventData)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env->getOmrVM());
    ...
    if (event->cycleEnd) {
        ...
        outputUnfinalizedInfo(
            env, 1, scavengerJavaStats->_unfinalizedCandidates, scavengerJavaStats->_unfinalizedEnqueued);

        outputOwnableSynchronizerInfo(env, 1, scavengerJavaStats->_ownableSynchronizerCandidates,
            (scavengerJavaStats->_ownableSynchronizerCandidates
                - scavengerJavaStats->_ownableSynchronizerTotalSurvived));

        outputReferenceInfo(env, 1, "soft", &scavengerJavaStats->_softReferenceStats,
            extensions->getDynamicMaxSoftReferenceAge(), extensions->getMaxSoftReferenceAge());
...
const char* MM_VerboseHandlerOutputStandardJava::getConcurrentKickoffReason(void* eventData)
{
...
#if defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING)
void MM_VerboseHandlerOutputStandardJava::handleConcurrentHaltedInternal(MM_EnvironmentBase* env, void* eventData)
{
...
void verboseHandlerSlowExclusive(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
