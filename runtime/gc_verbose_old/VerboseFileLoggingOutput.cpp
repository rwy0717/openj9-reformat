 ...
 */
MM_VerboseFileLoggingOutput* MM_VerboseFileLoggingOutput::newInstance(
    MM_EnvironmentBase* env, char* filename, UDATA numFiles, UDATA numCycles)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env->getOmrVM());

    MM_VerboseFileLoggingOutput* agent = (MM_VerboseFileLoggingOutput*)extensions->getForge()->allocate(
        sizeof(MM_VerboseFileLoggingOutput), MM_AllocationCategory::DIAGNOSTIC, J9_GET_CALLSITE());
    if (agent) {
 ...
 */
bool MM_VerboseFileLoggingOutput::initialize(
    MM_EnvironmentBase* env, const char* filename, UDATA numFiles, UDATA numCycles)
{
    _numFiles = numFiles;
 ...
 */
void MM_VerboseFileLoggingOutput::tearDown(MM_EnvironmentBase* env)
{
 ...
 */
bool MM_VerboseFileLoggingOutput::initializeTokens(MM_EnvironmentBase* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    char pidBuffer[64];
 ...
 */
bool MM_VerboseFileLoggingOutput::initializeFilename(MM_EnvironmentBase* env, const char* filename)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env->getOmrVM());
    ...
    if (_mode == rotating_files) {
        ...
        _filename
            = (char*)extensions->getForge()->allocate(nameLen, MM_AllocationCategory::DIAGNOSTIC, J9_GET_CALLSITE());
        if (NULL == _filename) {
            return false;
        }
        ...
        *write = '\0';
        ...
        _filename = (char*)extensions->getForge()->allocate(
            strlen(filename) + 1, MM_AllocationCategory::DIAGNOSTIC, J9_GET_CALLSITE());
        if (NULL == _filename) {
            return false;
        }
 ...
 */
void MM_VerboseFileLoggingOutput::formatAndOutput(J9VMThread* vmThread, UDATA indent, const char* format, ...)
{
 ...
 */
char* MM_VerboseFileLoggingOutput::expandFilename(MM_EnvironmentBase* env, UDATA currentFile)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env->getOmrVM());

    if (_mode == rotating_files) {
        j9str_set_token(PORTLIB, _tokens, "seq", "%03zu",
    ...
    char* filenameToOpen
        = (char*)extensions->getForge()->allocate(len, MM_AllocationCategory::DIAGNOSTIC, J9_GET_CALLSITE());
 ...
 */
bool MM_VerboseFileLoggingOutput::openFile(MM_EnvironmentBase* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    ...
    if (-1 == _logFileDescriptor) {
        ...
        /* Try again */
        _logFileDescriptor
            = j9file_open(filenameToOpen, EsOpenRead | EsOpenWrite | EsOpenCreate | EsOpenTruncate, 0666);
        if (-1 == _logFileDescriptor) {
 ...
 */
void MM_VerboseFileLoggingOutput::closeFile(MM_EnvironmentBase* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
 ...
 */
void MM_VerboseFileLoggingOutput::closeStream(MM_EnvironmentBase* env) { closeFile(env); }

/**
 * Flushes the verbose buffer to the output stream.
 * Also cycles the output files if necessary.
 */
void MM_VerboseFileLoggingOutput::endOfCycle(J9VMThread* vmThread)
{
    MM_EnvironmentBase* env = MM_EnvironmentBase::getEnvironment((OMR_VMThread*)vmThread->omrVMThread);
    PORT_ACCESS_FROM_ENVIRONMENT(env);
 ...
 */
bool MM_VerboseFileLoggingOutput::reconfigure(
    MM_EnvironmentBase* env, const char* filename, UDATA numFiles, UDATA numCycles)
{
    closeFile(env);
