 ...
 */
void MM_VerboseTraceOutput::formatAndOutput(J9VMThread* vmThread, UDATA indent, const char* format, ...)
{
 ...
 */
MM_VerboseTraceOutput* MM_VerboseTraceOutput::newInstance(MM_EnvironmentBase* env)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env->getOmrVM());

    MM_VerboseTraceOutput* agent = (MM_VerboseTraceOutput*)extensions->getForge()->allocate(
        sizeof(MM_VerboseTraceOutput), MM_AllocationCategory::DIAGNOSTIC, J9_GET_CALLSITE());
    if (agent) {
 ...
 */
bool MM_VerboseTraceOutput::initialize(MM_EnvironmentBase* env) { return true; }

/**
 */
void MM_VerboseTraceOutput::endOfCycle(J9VMThread* vmThread) {}

/**
 * Closes the agents output stream.
 */
void MM_VerboseTraceOutput::closeStream(MM_EnvironmentBase* env) {}
