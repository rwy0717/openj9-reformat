 ...
 */
MM_VerboseEvent* MM_VerboseEventAFEnd::newInstance(MM_AllocationFailureEndEvent* event, J9HookInterface** hookInterface)
{
 ...
 */
void MM_VerboseEventAFEnd::consumeEvents(void)
{
    MM_VerboseEventStream* eventStream = _manager->getEventStream();
    MM_VerboseEventAFStart* event = NULL;

    if (NULL
        != (event = (MM_VerboseEventAFStart*)eventStream->returnEvent(J9HOOK_MM_PRIVATE_ALLOCATION_FAILURE_START,
    ...
    /* Set last AF time */
    (MEMORY_TYPE_NEW == event->getSubSpaceType()) ? (_manager->setLastNurseryAFTime(_time))
                                                  : (_manager->setLastTenureAFTime(_time));
}
 ...
 */
void MM_VerboseEventAFEnd::formattedOutput(MM_VerboseOutputAgent* agent)
{
