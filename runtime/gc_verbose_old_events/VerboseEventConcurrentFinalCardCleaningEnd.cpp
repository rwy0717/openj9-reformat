 ...
 */
MM_VerboseEvent* MM_VerboseEventConcurrentFinalCardCleaningEnd::newInstance(
    MM_ConcurrentCollectionCardCleaningEndEvent* event, J9HookInterface** hookInterface)
{
    ...
    eventObject = (MM_VerboseEventConcurrentFinalCardCleaningEnd*)MM_VerboseEvent::create(
        event->currentThread, sizeof(MM_VerboseEventConcurrentFinalCardCleaningEnd));
    if (NULL != eventObject) {
 ...
 */
void MM_VerboseEventConcurrentFinalCardCleaningEnd::consumeEvents(void)
{
    MM_VerboseEventStream* eventStream = _manager->getEventStream();
    MM_VerboseEventConcurrentFinalCardCleaningStart* event = NULL;

    if (NULL
        != (event = (MM_VerboseEventConcurrentFinalCardCleaningStart*)eventStream->returnEvent(
                J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_CARD_CLEANING_START, _manager->getPrivateHookInterface(),
 ...
 */
void MM_VerboseEventConcurrentFinalCardCleaningEnd::formattedOutput(MM_VerboseOutputAgent* agent)
{
    UDATA indentLevel = _manager->getIndentLevel();
    U_64 timeInMicroSeconds;

    agent->formatAndOutput(
        static_cast<J9VMThread*>(_omrThread->_language_vmthread), indentLevel, "<con event=\"final card cleaning\">");
    ...
    if (!getTimeDeltaInMicroSeconds(&timeInMicroSeconds, _conFinalCCStartTime, _time)) {
        agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), indentLevel,
            "<warning details=\"clock error detected in stats durationms\" />");
    }
    agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), indentLevel,
        "<stats cardscleaned=\"%zu\" traced=\"%zu\" durationms=\"%llu.%03.3llu\" />", _finalcleanedCards, _bytesTraced,
