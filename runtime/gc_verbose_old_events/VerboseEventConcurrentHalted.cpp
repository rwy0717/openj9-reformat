 ...
 */
MM_VerboseEvent* MM_VerboseEventConcurrentHalted::newInstance(
    MM_ConcurrentHaltedEvent* event, J9HookInterface** hookInterface)
{
    MM_VerboseEventConcurrentHalted* eventObject;

    eventObject = (MM_VerboseEventConcurrentHalted*)MM_VerboseEvent::create(
        event->currentThread, sizeof(MM_VerboseEventConcurrentHalted));
    if (NULL != eventObject) {
 ...
 */
void MM_VerboseEventConcurrentHalted::consumeEvents(void) {}

/**
 * Passes a format string and data to the output routine defined in the passed output agent.
 * @param agent Pointer to an output agent.
 */
void MM_VerboseEventConcurrentHalted::formattedOutput(MM_VerboseOutputAgent* agent)
{
...
#define CONCURRENT_STATUS_BUFFER_LENGTH 32
    ...
    const char* statusString = MM_ConcurrentGCStats::getConcurrentStatusString(
        env, _executionMode, statusBuffer, CONCURRENT_STATUS_BUFFER_LENGTH);
#undef CONCURRENT_STATUS_BUFFER_LENGTH
    agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), indentLevel,
        "<con event=\"halted\" mode=\"%s\" state=\"%s\">", statusString,
        getConcurrentStateAsString(_isCardCleaningComplete, _scanClassesMode, _isTracingExhausted));

    _manager->incrementIndent();
    indentLevel = _manager->getIndentLevel();

    agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), indentLevel,
        "<stats tracetarget=\"%zu\">", _traceTarget);

    _manager->incrementIndent();
    indentLevel = _manager->getIndentLevel();

    agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), indentLevel,
        "<traced total=\"%zu\" mutators=\"%zu\" helpers=\"%zu\" percent=\"%zu\" />", _tracedTotal, _tracedByMutators,
        _tracedByHelpers, _traceTarget == 0 ? 0 : (UDATA)(((U_64)_tracedTotal * 100) / (U_64)_traceTarget));

    agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), indentLevel,
        "<cards cleaned=\"%zu\" kickoff=\"%zu\" />", _cardsCleaned, _cardCleaningThreshold);
 ...
 */
const char* MM_VerboseEventConcurrentHalted::getConcurrentStateAsString(
