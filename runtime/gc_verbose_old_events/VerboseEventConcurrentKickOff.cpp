 ...
 */
MM_VerboseEvent* MM_VerboseEventConcurrentKickOff::newInstance(
    MM_ConcurrentKickoffEvent* event, J9HookInterface** hookInterface)
{
    MM_VerboseEventConcurrentKickOff* eventObject;

    eventObject = (MM_VerboseEventConcurrentKickOff*)MM_VerboseEvent::create(
        event->currentThread, sizeof(MM_VerboseEventConcurrentKickOff));
    if (NULL != eventObject) {
...
void MM_VerboseEventConcurrentKickOff::initialize(void)
{
 ...
 */
void MM_VerboseEventConcurrentKickOff::consumeEvents(void) {}

/**
 * Passes a format string and data to the output routine defined in the passed output agent.
 * @param agent Pointer to an output agent.
 */
void MM_VerboseEventConcurrentKickOff::formattedOutput(MM_VerboseOutputAgent* agent)
{
    OMRPORT_ACCESS_FROM_OMRVMTHREAD(_omrThread);
    char timestamp[32];
    ...
    agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), indentLevel,
        "<con event=\"kickoff\" timestamp=\"%s\">", timestamp);

    _manager->incrementIndent();
    indentLevel = _manager->getIndentLevel();

    agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), indentLevel,
        "<kickoff reason=\"%s\" />", getKickoffReasonAsString(_kickOffReason, _languageKickOffReason));

    if (static_cast<J9JavaVM*>(_omrThread->_vm->_language_vm)
            ->memoryManagerFunctions->j9gc_scavenger_enabled(static_cast<J9JavaVM*>(_omrThread->_vm->_language_vm))) {
        agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), indentLevel,
            "<stats tenurefreebytes=\"%zu\" nurseryfreebytes=\"%zu\" tracetarget=\"%zu\" kickoff=\"%zu\"  />",
            _tenureFreeBytes, _nurseryFreeBytes, _traceTarget, _kickOffThreshold);
    } else {
        agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), indentLevel,
            "<stats tenurefreebytes=\"%zu\" tracetarget=\"%zu\" kickoff=\"%zu\" />", _tenureFreeBytes, _traceTarget,
            _kickOffThreshold);
    }
 ...
 */
const char* MM_VerboseEventConcurrentKickOff::getKickoffReasonAsString(UDATA reason, UDATA languageReason)
{
