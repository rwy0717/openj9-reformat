 ...
 */
MM_VerboseEvent* MM_VerboseEventConcurrentStart::newInstance(
    MM_ConcurrentCollectionStartEvent* event, J9HookInterface** hookInterface)
{
    MM_VerboseEventConcurrentStart* eventObject;

    eventObject = (MM_VerboseEventConcurrentStart*)MM_VerboseEvent::create(
        event->currentThread, sizeof(MM_VerboseEventConcurrentStart));
    if (NULL != eventObject) {
 ...
 */
void MM_VerboseEventConcurrentStart::consumeEvents(void)
{
 ...
 */
void MM_VerboseEventConcurrentStart::formattedOutput(MM_VerboseOutputAgent* agent)
{
    ...
    agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), indentLevel,
        "<con event=\"collection\" id=\"%zu\" timestamp=\"%s\" intervalms=\"%llu.%03.3llu\">", _conCollectionCount,
        timestamp, timeInMicroSeconds / 1000, timeInMicroSeconds % 1000);
    ...
    agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), indentLevel,
        "<cards cleaned=\"%zu\" kickoff=\"%zu\" reason=\"%s\" />", _cardsCleaned, _cardCleaningPhase1Threshold,
    ...
    if (_extensions->verboseExtensions) {
         ...
         */
        agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), indentLevel,
            "<threads kickoff=\"%zu\" scanned=\"%zu\" />", _threadsToScanCount, _threadsScannedCount);
    }
