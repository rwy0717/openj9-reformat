...
void MM_VerboseEventGCEnd::initialize(void)
{
 ...
 */
void MM_VerboseEventGCEnd::gcEndFormattedOutput(MM_VerboseOutputAgent* agent)
{
    ...
    if (static_cast<J9VMThread*>(_omrThread->_language_vmthread)
        ...
        agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), indentLevel,
            "<nursery freebytes=\"%zu\" totalbytes=\"%zu\" percent=\"%zu\" />", _gcEndData.commonData.nurseryFreeBytes,
            _gcEndData.commonData.nurseryTotalBytes,
            (UDATA)(
                ((U_64)_gcEndData.commonData.nurseryFreeBytes * 100) / (U_64)_gcEndData.commonData.nurseryTotalBytes));
    }

    agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), indentLevel,
        "<tenured freebytes=\"%zu\" totalbytes=\"%zu\" percent=\"%zu\" %s>", _gcEndData.commonData.tenureFreeBytes,
    ...
    if (_extensions->verboseExtensions) {
        agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), indentLevel,
            "<rememberedset count=\"%zu\" />", _gcEndData.commonData.rememberedSetCount);
    }
}
 ...
 */
bool MM_VerboseEventGCEnd::hasDetailedTenuredOutput()
{
 ...
 */
void MM_VerboseEventGCEnd::loaFormattedOutput(MM_VerboseOutputAgent* agent)
{
#if defined(J9VM_GC_LARGE_OBJECT_AREA)
    if (_gcEndData.commonData.loaEnabled) {
        UDATA indentLevel = _manager->getIndentLevel();
        ...
        agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), indentLevel,
            "<loa freebytes=\"%zu\" totalbytes=\"%zu\" percent=\"%zu\" />", _gcEndData.commonData.tenureLOAFreeBytes,
            _gcEndData.commonData.tenureLOATotalBytes,
            (_gcEndData.commonData.tenureLOATotalBytes == 0
                    ? 0
                    : (UDATA)(((U_64)_gcEndData.commonData.tenureLOAFreeBytes * 100)
                          / (U_64)_gcEndData.commonData.tenureLOATotalBytes)));
    }
#endif /* J9VM_GC_LARGE_OBJECT_AREA*/
}
