...
public:
    ...
    MM_VerboseEventGlobalGCEnd(MM_GlobalGCIncrementEndEvent* event, J9HookInterface** hookInterface)
        ...
        , _workStackOverflowOccured(MM_GCExtensions::getExtensions(event->omrVMThread)
                                        ->globalGCStats.workPacketStats.getSTWWorkStackOverflowOccured())
        , _workStackOverflowCount(MM_GCExtensions::getExtensions(event->omrVMThread)
                                      ->globalGCStats.workPacketStats.getSTWWorkStackOverflowCount())
        , _workpacketCount(MM_GCExtensions::getExtensions(event->omrVMThread)
                               ->globalGCStats.workPacketStats.getSTWWorkpacketCountAtOverflow())
        , _weakReferenceClearCount(
              MM_GCExtensions::getExtensions(event->omrVMThread)->markJavaStats._weakReferenceStats._cleared)
        , _softReferenceClearCount(
              MM_GCExtensions::getExtensions(event->omrVMThread)->markJavaStats._softReferenceStats._cleared)
        , _dynamicSoftReferenceThreshold(
              MM_GCExtensions::getExtensions(event->omrVMThread)->getDynamicMaxSoftReferenceAge())
        ...
        , _phantomReferenceClearCount(
              MM_GCExtensions::getExtensions(event->omrVMThread)->markJavaStats._phantomReferenceStats._cleared)
