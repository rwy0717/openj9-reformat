 ...
 */
MM_VerboseEvent* MM_VerboseEventHeapResize::newInstance(MM_HeapResizeEvent* event, J9HookInterface** hookInterface)
{
    MM_VerboseEventHeapResize* eventObject;

    eventObject
        = (MM_VerboseEventHeapResize*)MM_VerboseEvent::create(event->currentThread, sizeof(MM_VerboseEventHeapResize));
    if (NULL != eventObject) {
 ...
 */
void MM_VerboseEventHeapResize::consumeEvents(void)
{
 ...
 */
void MM_VerboseEventHeapResize::formattedOutput(MM_VerboseOutputAgent* agent)
{
        ...
        if (_amount) {
            if (0 == _ratio) {
                agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), indentLevel,
                    "<expansion type=\"%s\" amount=\"%zu\" newsize=\"%zu\" timetaken=\"%llu.%03.3llu\" reason=\"%s\" "
                    "/>",
                    (_subSpaceType == MEMORY_TYPE_OLD) ? "tenured" : "nursery", _amount, _newHeapSize,
                ...
                agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), indentLevel,
                    ...
                    (_subSpaceType == MEMORY_TYPE_OLD) ? "tenured" : "nursery", _amount, _newHeapSize,
                    timeInMicroSeconds / 1000, timeInMicroSeconds % 1000,
                    getExpandReasonAsString((ExpandReason)_reason), _ratio);
            }
        } else {
            agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), indentLevel,
                "<expansion type=\"%s\" result=\"failed\" />",
    ...
    case HEAP_CONTRACT:
        if (_amount) {
            if (0 == _ratio) {
                agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), indentLevel,
                    "<contraction type=\"%s\" amount=\"%zu\" newsize=\"%zu\" timetaken=\"%llu.%03.3llu\" reason=\"%s\" "
                    "/>",
                    (_subSpaceType == MEMORY_TYPE_OLD) ? "tenured" : "nursery", _amount, _newHeapSize,
                    timeInMicroSeconds / 1000, timeInMicroSeconds % 1000,
                ...
                agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), indentLevel,
                    ...
                    (_subSpaceType == MEMORY_TYPE_OLD) ? "tenured" : "nursery", _amount, _newHeapSize,
                    timeInMicroSeconds / 1000, timeInMicroSeconds % 1000,
                    getContractReasonAsString((ContractReason)_reason), _ratio);
            }
        } else {
            agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), indentLevel,
                "<contraction type=\"%s\" result=\"failed\" />",
                (_subSpaceType == MEMORY_TYPE_OLD) ? "tenured" : "nursery");
        }
        break;
    }
