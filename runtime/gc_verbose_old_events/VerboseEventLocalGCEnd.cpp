 ...
 */
MM_VerboseEvent* MM_VerboseEventLocalGCEnd::newInstance(MM_LocalGCEndEvent* event, J9HookInterface** hookInterface)
{
    MM_VerboseEventLocalGCEnd* eventObject;

    eventObject
        = (MM_VerboseEventLocalGCEnd*)MM_VerboseEvent::create(event->currentThread, sizeof(MM_VerboseEventLocalGCEnd));
    if (NULL != eventObject) {
 ...
 */
void MM_VerboseEventLocalGCEnd::consumeEvents(void)
{
    MM_VerboseEventStream* eventStream = _manager->getEventStream();
    MM_VerboseEventLocalGCStart* event = NULL;

    if (NULL
        != (event = (MM_VerboseEventLocalGCStart*)eventStream->returnEvent(
                J9HOOK_MM_OMR_LOCAL_GC_START, _manager->getHookInterface(), (MM_VerboseEvent*)this))) {
 ...
 */
void MM_VerboseEventLocalGCEnd::formattedOutput(MM_VerboseOutputAgent* agent)
{
    ...
    if (_rememberedSetOverflowed) {
        agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), indentLevel,
            "<warning details=\"remembered set overflow detected\" />");
    }
    if ((_causedRememberedSetOverflow) && (_rememberedSetOverflowed)) {
        agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), indentLevel,
            "<warning details=\"remembered set overflow triggered\" />");
    }
    if (_scanCacheOverflow) {
        agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), indentLevel,
            "<warning details=\"scan cache overflow detected\" />");
    }
    if (0 != _failedFlipCount) {
        agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), indentLevel,
            "<failed type=\"flipped\" objectcount=\"%zu\" bytes=\"%zu\" />", _failedFlipCount, _failedFlipBytes);
    }
    if (0 != _failedTenureCount) {
        agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), indentLevel,
            "<failed type=\"tenured\" objectcount=\"%zu\" bytes=\"%zu\" />", _failedTenureCount, _failedTenureBytes);
    }
    if (_backout) {
        agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), indentLevel,
            "<warning details=\"aborted collection\" />", gcCount);
    }

    /* Memory stats */
    agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), indentLevel,
        "<flipped objectcount=\"%zu\" bytes=\"%zu\" />", _flipCount, _flipBytes);
    agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), indentLevel,
        "<tenured objectcount=\"%zu\" bytes=\"%zu\" />", _tenureCount, _tenureBytes);
    ...
    if (_tilted) {
        agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), indentLevel,
            "<scavenger tiltratio=\"%zu\" />",
            (UDATA)((U_64)_nurseryTotalBytes * 100 / ((U_64)_totalMemorySize - _tenureTotalBytes)));
    }
#endif /* J9VM_GC_TILTED_NEW_SPACE */

    agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), indentLevel,
        "<nursery freebytes=\"%zu\" totalbytes=\"%zu\" percent=\"%zu\" tenureage=\"%zu\" />", _nurseryFreeBytes,
        _nurseryTotalBytes, (UDATA)(((U_64)_nurseryFreeBytes * 100) / (U_64)_nurseryTotalBytes), _tenureAge);
