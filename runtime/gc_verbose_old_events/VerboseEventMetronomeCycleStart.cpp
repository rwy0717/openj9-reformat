 ...
 */
MM_VerboseEvent* MM_VerboseEventMetronomeCycleStart::newInstance(
    MM_GCCycleStartEvent* event, J9HookInterface** hookInterface)
{
    MM_VerboseEventMetronomeCycleStart* eventObject;

    eventObject = (MM_VerboseEventMetronomeCycleStart*)MM_VerboseEvent::create(
        event->omrVMThread, sizeof(MM_VerboseEventMetronomeCycleStart));
    if (NULL != eventObject) {
 ...
 */
void MM_VerboseEventMetronomeCycleStart::consumeEvents(void)
{
 ...
 */
void MM_VerboseEventMetronomeCycleStart::formattedOutput(MM_VerboseOutputAgent* agent)
{
     ...
     * There should be no heartbeat nither syncGC/OOM events in between the two, since no GC should be happening in
     * between. SyncGC due to Explict GC are possible between cycle end and cycle start */
    ...
    agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), manager->getIndentLevel(),
        "<gc type=\"cycle start\" id=\"%zu\" timestamp=\"%s\" intervalms=\"%llu.%03.3llu\" heapfreebytes=\"%zu\" />",
        manager->getMetronomeCycleCount(), timestamp, timeSinceLastCycleEnd / 1000, timeSinceLastCycleEnd % 1000,
        _heapFree);
