 ...
 */
MM_VerboseEvent* MM_VerboseEventMetronomeGCEnd::newInstance(
    MM_MetronomeIncrementEndEvent* event, J9HookInterface** hookInterface)
{
    MM_VerboseEventMetronomeGCEnd* eventObject;

    eventObject = (MM_VerboseEventMetronomeGCEnd*)MM_VerboseEvent::create(
        event->currentThread, sizeof(MM_VerboseEventMetronomeGCEnd));
    if (NULL != eventObject) {
...
void MM_VerboseEventMetronomeGCEnd::initialize(void)
{
 ...
 */
void MM_VerboseEventMetronomeGCEnd::consumeEvents(void)
{
    OMRPORT_ACCESS_FROM_OMRVMTHREAD(_omrThread);
    ...
    if ((NULL != eventMetronomeGCStart)
        && (J9HOOK_MM_PRIVATE_METRONOME_INCREMENT_START == eventMetronomeGCStart->getEventType())
        && (_manager->getPrivateHookInterface() == eventMetronomeGCStart->getHookInterface())) {
        _incrementTime
            = omrtime_hires_delta(eventMetronomeGCStart->getTimeStamp(), _time, J9PORT_TIME_DELTA_IN_MICROSECONDS);
    } else {
    ...
    while (NULL != event) {
        if ((J9HOOK_MM_PRIVATE_METRONOME_INCREMENT_START == event->getEventType())
            && (_manager->getPrivateHookInterface() == event->getHookInterface())) {
            ...
            _minExclusiveAccessTime
                = OMR_MIN(_minExclusiveAccessTime, ((MM_VerboseEventMetronomeGCStart*)event)->_exclusiveAccessTime);
            _meanExclusiveAccessTime += ((MM_VerboseEventMetronomeGCStart*)event)->_exclusiveAccessTime;
            _maxExclusiveAccessTime
                = OMR_MAX(_maxExclusiveAccessTime, ((MM_VerboseEventMetronomeGCStart*)event)->_exclusiveAccessTime);
        }
        if ((J9HOOK_MM_PRIVATE_METRONOME_INCREMENT_END == event->getEventType())
            && (_manager->getPrivateHookInterface() == event->getHookInterface())) {
            ...
            _nonDeterministicSweepConsecutiveMax = OMR_MAX(_nonDeterministicSweepConsecutiveMax,
                ((MM_VerboseEventMetronomeGCEnd*)event)->_nonDeterministicSweepConsecutive);
            _nonDeterministicSweepDelayMax = OMR_MAX(
                _nonDeterministicSweepDelayMax, ((MM_VerboseEventMetronomeGCEnd*)event)->_nonDeterministicSweepDelay);
        }
...
bool MM_VerboseEventMetronomeGCEnd::definesOutputRoutine()
{
    MM_VerboseEvent* event = (MM_VerboseEvent*)this;
    MM_VerboseEvent* previousEvent;

    previousEvent = getPreviousEvent();
    if ((NULL == previousEvent) || (J9HOOK_MM_PRIVATE_METRONOME_INCREMENT_START != previousEvent->getEventType())
        || (_manager->getPrivateHookInterface() != previousEvent->getHookInterface())) {
...
bool MM_VerboseEventMetronomeGCEnd::endsEventChain()
{
 ...
 */
void MM_VerboseEventMetronomeGCEnd::formattedOutput(MM_VerboseOutputAgent* agent)
{
    OMRPORT_ACCESS_FROM_OMRVMTHREAD(_omrThread);
    char timestamp[32];
    ...
    U_64 timeSinceLastEvent
        = omrtime_hires_delta(_manager->getLastMetronomeTime(), _time, J9PORT_TIME_DELTA_IN_MICROSECONDS);

    omrstr_ftime(timestamp, sizeof(timestamp), VERBOSEGC_DATE_FORMAT, _timeInMilliSeconds);

    agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), _manager->getIndentLevel(),
        "<gc type=\"heartbeat\" id=\"%zu\" timestamp=\"%s\" intervalms=\"%llu.%03.3llu\">",
        _manager->getMetronomeHeartbeatCount(), timestamp, /* Reported timestamp is for the last event in the chain */
        timeSinceLastEvent / 1000, timeSinceLastEvent % 1000);
    _manager->incrementIndent();
    agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), _manager->getIndentLevel(),
        "<summary quantumcount=\"%zu\">", _endIncrementCount);
    _manager->incrementIndent();
    agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), _manager->getIndentLevel(),
        "<quantum minms=\"%llu.%03.3llu\" meanms=\"%llu.%03.3llu\" maxms=\"%llu.%03.3llu\" />",
        _minIncrementTime / 1000, _minIncrementTime % 1000, _meanIncrementTime / 1000, _meanIncrementTime % 1000,
        _maxIncrementTime / 1000, _maxIncrementTime % 1000);
    agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), _manager->getIndentLevel(),
        "<exclusiveaccess minms=\"%llu.%03.3llu\" meanms=\"%llu.%03.3llu\" maxms=\"%llu.%03.3llu\" />",
        _minExclusiveAccessTime / 1000, _minExclusiveAccessTime % 1000, _meanExclusiveAccessTime / 1000,
        _meanExclusiveAccessTime % 1000, _maxExclusiveAccessTime / 1000, _maxExclusiveAccessTime % 1000);
    if (0 != _classLoadersUnloadedTotal) {
        agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), _manager->getIndentLevel(),
            "<classunloading classloaders=\"%zu\" classes=\"%zu\" />", _classLoadersUnloadedTotal,
            _classesUnloadedTotal);
    }
    if ((0 != _weakReferenceClearCountTotal) || (0 != _softReferenceClearCountTotal)
        || (0 != _phantomReferenceClearCountTotal)) {
        agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), _manager->getIndentLevel(),
            "<refs_cleared soft=\"%zu\" threshold=\"%zu\" maxThreshold=\"%zu\" weak=\"%zu\" phantom=\"%zu\" />",
            _softReferenceClearCountTotal, _dynamicSoftReferenceThreshold, _softReferenceThreshold,
            _weakReferenceClearCountTotal, _phantomReferenceClearCountTotal);
    }
    if (0 != _finalizableCountTotal) {
        agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), _manager->getIndentLevel(),
            "<finalization objectsqueued=\"%zu\" />", _finalizableCountTotal);
    }

    if ((0 != _workPacketOverflowCountTotal) || (0 != _objectOverflowCount)) {
        agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), _manager->getIndentLevel(),
            "<warning details=\"overflow occured\" packetCount=\"%zu\" directObjectCount=\"%zu\" />",
            _workPacketOverflowCountTotal, _objectOverflowCountTotal);
    }
    if (0 != _nonDeterministicSweepTotal) {
        agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), _manager->getIndentLevel(),
            "<nondeterministicsweep  maxms=\"%llu.%03.3llu\" totalregions=\"%zu\" maxregions=\"%zu\" />",
            _nonDeterministicSweepDelayMax / 1000, _nonDeterministicSweepDelayMax % 1000, _nonDeterministicSweepTotal,
            _nonDeterministicSweepConsecutiveMax);
    }
    agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), _manager->getIndentLevel(),
        "<heap minfree=\"%zu\" meanfree=\"%llu\" maxfree=\"%zu\" />", _minHeapFree, _meanHeapFree, _maxHeapFree);
    agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), _manager->getIndentLevel(),
        "<gcthreadpriority max=\"%zu\" min=\"%zu\" />", _maxStartPriority, _minStartPriority);

    _manager->decrementIndent();
    agent->formatAndOutput(
        static_cast<J9VMThread*>(_omrThread->_language_vmthread), _manager->getIndentLevel(), "</summary>");
    _manager->decrementIndent();
    agent->formatAndOutput(
        static_cast<J9VMThread*>(_omrThread->_language_vmthread), _manager->getIndentLevel(), "</gc>");
