...
private:
    ...
    UDATA _classLoadersUnloaded; /**< Snapshot of classloader unloaded count for each reported event (ie each Metronome
                                    GC increment) */
    UDATA _classLoadersUnloadedTotal; /**< Sum of _classLoadersUnloaded from all events between two heartbeat verbose gc
                                         reports */
    UDATA _classesUnloaded; /**< Snapshot of classes unloaded count for each reported event (ie each Metronome GC
                               increment) */
    UDATA
    ...
    UDATA _nonDeterministicSweepTotal; /**< Number of non deterministic sweeps between two heartbeats (total of all
                                          quantas) */
    UDATA _nonDeterministicSweepConsecutive; /**< The longest streak of consecutive sweeps for one allocation, among all
                                                allocations between two quantas */
    UDATA _nonDeterministicSweepConsecutiveMax; /**< The maximum of all longest streaks, between two heartbeats */
    U_64 _nonDeterministicSweepDelay; /**< The longest delay caused by non deterministic sweep among all allocs between
                                         two GC quantas */
    ...
    U_64 _minExclusiveAccessTime; /**< The minimum time (of all events of the chain) GC master thread spent stopping
                                     mutators */
    U_64 _meanExclusiveAccessTime; /**< The average time (of all events of the chain) GC master thread spent stopping
                                      mutators */
