 ...
 */
MM_VerboseEvent* MM_VerboseEventMetronomeSynchronousGCEnd::newInstance(
    MM_MetronomeSynchronousGCEndEvent* event, J9HookInterface** hookInterface)
{
    MM_VerboseEventMetronomeSynchronousGCEnd* eventObject;

    eventObject = (MM_VerboseEventMetronomeSynchronousGCEnd*)MM_VerboseEvent::create(
        event->currentThread, sizeof(MM_VerboseEventMetronomeSynchronousGCEnd));
    if (NULL != eventObject) {
...
void MM_VerboseEventMetronomeSynchronousGCEnd::initialize(MM_MetronomeSynchronousGCEndEvent* event)
{
 ...
 */
void MM_VerboseEventMetronomeSynchronousGCEnd::consumeEvents(void)
{
    ...
    if (NULL
        != (eventSyncGCStart = (MM_VerboseEventMetronomeSynchronousGCStart*)eventStream->returnEvent(
                J9HOOK_MM_PRIVATE_METRONOME_SYNCHRONOUS_GC_START, _manager->getPrivateHookInterface(),
 ...
 */
void MM_VerboseEventMetronomeSynchronousGCEnd::formattedOutput(MM_VerboseOutputAgent* agent)
{
    ...
    if (manager->getLastMetronomeTime()) {
        ...
        timeSinceLastEvent
            = omrtime_hires_delta(manager->getLastMetronomeTime(), _startTime, J9PORT_TIME_DELTA_IN_MICROSECONDS);
    } else {
    ...
    agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), manager->getIndentLevel(),
        "<gc type=\"synchgc\" id=\"%zu\" timestamp=\"%s\" intervalms=\"%llu.%03.3llu\">",
        manager->getMetronomeSynchGCCount(), _timestamp, /* This is the timestamp of matching SyncGC start event! */
    ...
    if (_reason == OUT_OF_MEMORY_TRIGGER) {
        agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), manager->getIndentLevel(),
            "<details reason=\"%s\" requested_bytes=\"%zu\" />", hookReason, _reasonParameter);
    } else {
        agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), manager->getIndentLevel(),
            "<details reason=\"%s\" />", hookReason);
    }

    if (!deltaTimeSuccess) {
        agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), _manager->getIndentLevel(),
            "<warning details=\"clock error detected, following timing may be inaccurate\" />");
    }

    agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), manager->getIndentLevel(),
        "<duration timems=\"%llu.%03.3llu\" />", syncGCTimeDuration / 1000, syncGCTimeDuration % 1000);
    if ((0 != _workPacketOverflowCount) || (0 != _objectOverflowCount)) {
        agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), _manager->getIndentLevel(),
            "<warning details=\"overflow occured\" packetCount=\"%zu\" directObjectCount=\"%zu\" />",
            _workPacketOverflowCount, _objectOverflowCount);
    }
    if (_classLoadersUnloadedEnd != _classLoadersUnloadedStart) {
        agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), manager->getIndentLevel(),
            "<classunloading classloaders=\"%zu\" classes=\"%zu\" />",
            _classLoadersUnloadedEnd - _classLoadersUnloadedStart, _classesUnloadedEnd - _classesUnloadedStart);
    }
    if ((0 != _weakReferenceClearCount) || (0 != _softReferenceClearCount) || (0 != _phantomReferenceClearCount)) {
        agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), manager->getIndentLevel(),
            ...
            _softReferenceClearCount, _dynamicSoftReferenceThreshold, _softReferenceThreshold, _weakReferenceClearCount,
            _phantomReferenceClearCount);
    }
    if (0 != _finalizableCount) {
        agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), manager->getIndentLevel(),
            "<finalization objectsqueued=\"%zu\" />", _finalizableCount);
    }
    agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), manager->getIndentLevel(),
        "<heap freebytesbefore=\"%zu\" />", _heapFreeBefore);
    agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), manager->getIndentLevel(),
        "<heap freebytesafter=\"%zu\" />", _heapFreeAfter);
    agent->formatAndOutput(static_cast<J9VMThread*>(_omrThread->_language_vmthread), manager->getIndentLevel(),
        "<synchronousgcpriority value=\"%zu\" />", _gcThreadPriority);
    manager->decrementIndent();
    agent->formatAndOutput(
        static_cast<J9VMThread*>(_omrThread->_language_vmthread), manager->getIndentLevel(), "</gc>");
