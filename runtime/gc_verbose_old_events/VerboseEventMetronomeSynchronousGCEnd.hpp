...
private:
    ...
    UDATA _classLoadersUnloadedStart; /**< Number of classLoaders unloaded just just before SyncGC started, but only for
                                         the current GC cycle */
    UDATA _classesUnloadedStart; /**< Number of classes unloaded just just before SyncGC started, but only for the
                                    current GC cycle */
    ...
    UDATA
        _gcThreadPriority; /**< Priority of the gc thread at the end of the synchronous GC. Note that the start of the
                              synch GC is reported by the java thread and that's why the start priority isn't taken. */
