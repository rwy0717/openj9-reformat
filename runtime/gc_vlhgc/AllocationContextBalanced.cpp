...
MM_AllocationContextBalanced* MM_AllocationContextBalanced::newInstance(
    MM_EnvironmentBase* env, MM_MemorySubSpaceTarok* subspace, UDATA numaNode, UDATA allocationContextNumber)
{
    MM_AllocationContextBalanced* context = (MM_AllocationContextBalanced*)env->getForge()->allocate(
        sizeof(MM_AllocationContextBalanced), MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
    if (context) {
 ...
 * Initialization.
 */
bool MM_AllocationContextBalanced::initialize(MM_EnvironmentBase* env)
{
    ...
    J9MemoryNodeDetail const* freeProcessorNodes
        = extensions->_numaManager.getFreeProcessorPool(&freeProcessorNodeCount);
    ...
    _freeProcessorNodes
        = (UDATA*)env->getForge()->allocate(arraySizeInBytes, MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
 ...
 * Shut down.
 */
void MM_AllocationContextBalanced::tearDown(MM_EnvironmentBase* env)
{
...
void MM_AllocationContextBalanced::flushInternal(MM_EnvironmentBase* env)
{
...
void MM_AllocationContextBalanced::flush(MM_EnvironmentBase* env) { flushInternal(env); }

void MM_AllocationContextBalanced::flushForShutdown(MM_EnvironmentBase* env) { flushInternal(env); }

void* MM_AllocationContextBalanced::allocateTLH(MM_EnvironmentBase* env, MM_AllocateDescription* allocateDescription,
    MM_ObjectAllocationInterface* objectAllocationInterface, bool shouldCollectOnFailure)
{
    ...
    if (NULL == result) {
        result = lockedReplenishAndAllocate(
            env, objectAllocationInterface, allocateDescription, MM_MemorySubSpace::ALLOCATION_TYPE_TLH);
    }
    unlockCommon();
    /* if that still fails, try to invoke the collector */
    if (shouldCollectOnFailure && (NULL == result)) {
        result = _subspace->replenishAllocationContextFailed(env, _subspace, this, objectAllocationInterface,
            allocateDescription, MM_MemorySubSpace::ALLOCATION_TYPE_TLH);
    }
    return result;
}
void* MM_AllocationContextBalanced::lockedAllocateTLH(MM_EnvironmentBase* env,
    MM_AllocateDescription* allocateDescription, MM_ObjectAllocationInterface* objectAllocationInterface)
{
    void* result = NULL;
    ...
    /* if we couldn't satisfy the allocate, go to the non-full region list (slow-path) before failing over into
     * replenishment */
    if (NULL == result) {
        /* scan through our regions which are still active for allocation and attempt the TLH allocation in each.  Any
         * which are too full or fragmented to satisfy a TLH allocation must be moved to the "discard" list so we won't
         * consider them for allocation until after the next collection */
        ...
        while ((NULL == result) && (NULL != region)) {
            ...
            if (NULL == result) {
                ...
                Trc_MM_AllocationContextBalanced_lockedAllocateTLH_setAllocationRegion(
                    env->getLanguageVMThread(), this, region);
            }
            region = next;
        }
    }
...
void* MM_AllocationContextBalanced::allocateObject(
    MM_EnvironmentBase* env, MM_AllocateDescription* allocateDescription, bool shouldCollectOnFailure)
{
    ...
    /* if that still fails, try to invoke the collector */
    if (shouldCollectOnFailure && (NULL == result)) {
        result = _subspace->replenishAllocationContextFailed(
            env, _subspace, this, NULL, allocateDescription, MM_MemorySubSpace::ALLOCATION_TYPE_OBJECT);
    }
    if (NULL != result) {
...
void* MM_AllocationContextBalanced::lockedAllocateObject(
    MM_EnvironmentBase* env, MM_AllocateDescription* allocateDescription)
{
    ...
    if (NULL != _allocationRegion) {
        ...
        if (NULL == result) {
            Assert_MM_true(spaceBefore == spaceAfter);
            /* if we failed the allocate, move the region into the non-full list since a TLH allocate can consume any
             * space remaining, prior to discarding */
            _nonFullRegions.insertRegion(_allocationRegion);
            _allocationRegion = NULL;
            Trc_MM_AllocationContextBalanced_lockedAllocateObject_clearAllocationRegion(
                env->getLanguageVMThread(), this);
        } else {
            Assert_MM_true(spaceBefore > spaceAfter);
            _freeMemorySize -= (spaceBefore - spaceAfter);
        }
    ...
    if (NULL == result) {
        Assert_MM_true(NULL == _allocationRegion);
        /* scan through our active region list and attempt the allocation in each.  Failing to satisfy a one-off object
         * allocation, such as this, will not force a region into the discard list, however */
        MM_HeapRegionDescriptorVLHGC* region = _nonFullRegions.peekFirstRegion();
        while ((NULL == result) && (NULL != region)) {
            MM_MemoryPoolBumpPointer* memoryPool = (MM_MemoryPoolBumpPointer*)region->getMemoryPool();
            Assert_MM_true(NULL != memoryPool);
...
void* MM_AllocationContextBalanced::allocateArrayletLeaf(
    MM_EnvironmentBase* env, MM_AllocateDescription* allocateDescription, bool shouldCollectOnFailure)
{
    /* this AC implementation doesn't try to cache leaf regions so just call into the subspace to hand us a region and
     * then we will use it in lockedAllocateArrayletLeaf */
    void* result = NULL;
    lockCommon();
    ...
    if (shouldCollectOnFailure && (NULL == result)) {
        result = _subspace->replenishAllocationContextFailed(
            env, _subspace, this, NULL, allocateDescription, MM_MemorySubSpace::ALLOCATION_TYPE_LEAF);
    }
    if (NULL != result) {
...
void* MM_AllocationContextBalanced::lockedAllocateArrayletLeaf(MM_EnvironmentBase* envBase,
    MM_AllocateDescription* allocateDescription, MM_HeapRegionDescriptorVLHGC* freeRegionForArrayletLeaf)
{
    ...
    MM_HeapRegionDescriptorVLHGC* spineRegion
        = (MM_HeapRegionDescriptorVLHGC*)_heapRegionManager->tableDescriptorForAddress(spine);
    /* the leaf requires a pointer back to the spine object so that it can verify its liveness elsewhere in the
     * collector */
     ...
     * An exceptional scenario: A thread allocates a spine (and possibly a few arraylets), but does not complete the
     * allocation. A global GC (or a series of regular PGCs) occurs that age out regions to max age. The spine moves
     * into a common context. Now, we successfully resume the leaf allocation, but the common lock that we already hold
     * is not sufficient any more. We need to additionally acquire common context' common lock, since multiple spines
     * from different ACs could have come into this state, and worse multiple spines originally allocated from different
     * ACs may end up in a single common context region.
     */
...
MM_HeapRegionDescriptorVLHGC* MM_AllocationContextBalanced::collectorAcquireRegion(MM_EnvironmentBase* env)
{
...
MM_HeapRegionDescriptorVLHGC* MM_AllocationContextBalanced::internalCollectorAcquireRegion(MM_EnvironmentBase* env)
{
    MM_HeapRegionDescriptorVLHGC* region = NULL;
    ...
    if (NULL != region) {
        ...
        _allocationRegion = NULL;
        Trc_MM_AllocationContextBalanced_internalCollectorAcquireRegion_clearAllocationRegion(
            env->getLanguageVMThread(), this);
...
void* MM_AllocationContextBalanced::allocate(MM_EnvironmentBase* env,
...
void* MM_AllocationContextBalanced::lockedAllocate(MM_EnvironmentBase* env,
    MM_ObjectAllocationInterface* objectAllocationInterface, MM_AllocateDescription* allocateDescription,
    MM_MemorySubSpace::AllocationType allocationType)
{
    void* result = NULL;
...
void MM_AllocationContextBalanced::setNextSibling(MM_AllocationContextBalanced* sibling)
{
...
void MM_AllocationContextBalanced::setStealingCousin(MM_AllocationContextBalanced* cousin)
{
...
void MM_AllocationContextBalanced::recycleRegion(MM_EnvironmentVLHGC* env, MM_HeapRegionDescriptorVLHGC* region)
{
...
void MM_AllocationContextBalanced::tearDownRegion(MM_EnvironmentBase* env, MM_HeapRegionDescriptorVLHGC* region)
{
    MM_MemoryPoolBumpPointer* memoryPool = (MM_MemoryPoolBumpPointer*)region->getMemoryPool();
    if (NULL != memoryPool) {
...
void MM_AllocationContextBalanced::addRegionToFreeList(MM_EnvironmentBase* env, MM_HeapRegionDescriptorVLHGC* region)
{
...
void MM_AllocationContextBalanced::resetLargestFreeEntry()
{
    lockCommon();
    if (NULL != _allocationRegion) {
...
MM_HeapRegionDescriptorVLHGC* MM_AllocationContextBalanced::acquireMPBPRegionFromHeap(
    MM_EnvironmentBase* env, MM_MemorySubSpace* subspace, MM_AllocationContextTarok* requestingContext)
{
    ...
    if ((NULL == region) && (_nextToSteal != this)) {
        ...
        do {
            ...
            /* advance to the next node whether we succeeded or not since we want to distribute our "theft" as evenly as
             * possible */
            _nextToSteal = _nextToSteal->getStealingCousin();
            if (this == _nextToSteal) {
                /* never try to steal from ourselves since that wouldn't be possible and the code interprets this case
                 * as a uniform system */
                _nextToSteal = _nextToSteal->getStealingCousin();
            }
...
MM_HeapRegionDescriptorVLHGC* MM_AllocationContextBalanced::acquireFreeRegionFromHeap(MM_EnvironmentBase* env)
{
...
MM_HeapRegionDescriptorVLHGC* MM_AllocationContextBalanced::acquireMPBPRegionFromNode(
    MM_EnvironmentBase* env, MM_MemorySubSpace* subSpace, MM_AllocationContextTarok* requestingContext)
{
    Trc_MM_AllocationContextBalanced_acquireMPBPRegionFromNode_Entry(
        env->getLanguageVMThread(), this, requestingContext);
    ...
    MM_HeapRegionDescriptorVLHGC* region
        = _cachedReplenishPoint->acquireMPBPRegionFromContext(env, subSpace, requestingContext);
    ...
    if (NULL != region) {
        /* Regions made available for allocation are identified by their region type (BUMP_ALLOCATED, as opposed to
...
MM_HeapRegionDescriptorVLHGC* MM_AllocationContextBalanced::acquireMPBPRegionFromContext(
    MM_EnvironmentBase* envBase, MM_MemorySubSpace* subSpace, MM_AllocationContextTarok* requestingContext)
{
    MM_EnvironmentVLHGC* env = MM_EnvironmentVLHGC::getEnvironment(envBase);
    ...
    if (NULL != region) {
        if (MM_HeapRegionDescriptor::FREE == region->getRegionType()) {
            if (region->_allocateData.taskAsMemoryPoolBumpPointer(env, requestingContext)) {
                ...
                mpaol->expandWithRange(
                    env, region->getSize(), region->getLowAddress(), region->getHighAddress(), false);
                mpaol->recalculateMemoryPoolStatistics(env);
            } else {
                /* something went wrong so put the region back in the free list and return NULL (even though the region
                 * might have been found in another context, where we put it back is largely arbitrary and this path
                 * should never actually be taken) */
                addRegionToFreeList(env, region);
                region = NULL;
            }
...
MM_HeapRegionDescriptorVLHGC* MM_AllocationContextBalanced::acquireFreeRegionFromNode(MM_EnvironmentBase* env)
{
...
MM_HeapRegionDescriptorVLHGC* MM_AllocationContextBalanced::acquireFreeRegionFromContext(MM_EnvironmentBase* env)
{
    _freeListLock.acquire();
...
void MM_AllocationContextBalanced::lockCommon() { _contextLock.acquire(); }
void MM_AllocationContextBalanced::unlockCommon() { _contextLock.release(); }
...
UDATA
MM_AllocationContextBalanced::getFreeRegionCount() { return _idleMPBPRegions.listSize() + _freeRegions.listSize(); }

void MM_AllocationContextBalanced::migrateRegionToAllocationContext(
    MM_HeapRegionDescriptorVLHGC* region, MM_AllocationContextTarok* newOwner)
{
    /*
     * This is the point where we reconcile the data held in the region descriptors and the contexts.  Prior to this,
     * compaction planning may have decided to migrate a region into a new context but couldn't update the contexts'
     * meta-structures due to performance concerns around the locks required to manipulate the lists.  After this call
     * returns, region's meta-data will be consistent with its owning context.
     */
    ...
    /* we can only do direct migration between contexts with the same NUMA properties, at this time (note that 0 is
     * special since it can accept memory from any node) */
    Assert_MM_true((region->getNumaNode() == newOwner->getNumaNode()) || (0 == newOwner->getNumaNode()));
}

void MM_AllocationContextBalanced::acceptMigratingRegion(MM_HeapRegionDescriptorVLHGC* region)
{
    _flushedRegions.insertRegion(region);
}

void MM_AllocationContextBalanced::resetHeapStatistics(bool globalCollect)
{
    lockCommon();
    if (NULL != _allocationRegion) {
...
void MM_AllocationContextBalanced::mergeHeapStats(MM_HeapStats* heapStats, UDATA includeMemoryType)
{
    lockCommon();
    if (NULL != _allocationRegion) {
...
void* MM_AllocationContextBalanced::lockedReplenishAndAllocate(MM_EnvironmentBase* env,
    MM_ObjectAllocationInterface* objectAllocationInterface, MM_AllocateDescription* allocateDescription,
    MM_MemorySubSpace::AllocationType allocationType)
{
    void* result = NULL;
    ...
    Trc_MM_AllocationContextBalanced_lockedReplenishAndAllocate_Entry(
        env->getLanguageVMThread(), regionSize, contiguousAllocationSize);
    ...
    if (MM_MemorySubSpace::ALLOCATION_TYPE_LEAF == allocationType) {
        if (_subspace->consumeFromTaxationThreshold(env, regionSize)) {
            ...
            if (NULL != leafRegion) {
                ...
                Trc_MM_AllocationContextBalanced_lockedReplenishAndAllocate_acquiredFreeRegion(
                    env->getLanguageVMThread(), regionSize);
            }
        }
    } else
#endif /* defined(J9VM_GC_ARRAYLETS) */
...
MM_HeapRegionDescriptorVLHGC* MM_AllocationContextBalanced::internalReplenishActiveRegion(
    MM_EnvironmentBase* env, bool payTax)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
    UDATA regionSize = extensions->regionSize;
    ...
    if (!payTax || _subspace->consumeFromTaxationThreshold(env, regionSize)) {
        ...
        if (NULL != newRegion) {
            Trc_MM_AllocationContextBalanced_internalReplenishActiveRegion_convertedFreeRegion(
                env->getLanguageVMThread(), newRegion, regionSize);
            _allocationRegion = newRegion;
            Trc_MM_AllocationContextBalanced_internalReplenishActiveRegion_setAllocationRegion(
                env->getLanguageVMThread(), this, newRegion);
...
void MM_AllocationContextBalanced::accountForRegionLocation(
    MM_HeapRegionDescriptorVLHGC* region, UDATA* localCount, UDATA* foreignCount)
{
...
void MM_AllocationContextBalanced::countRegionsInList(MM_RegionListTarok* list, UDATA* localCount, UDATA* foreignCount)
{
...
void MM_AllocationContextBalanced::getRegionCount(UDATA* localCount, UDATA* foreignCount)
{
    if (NULL != _allocationRegion) {
...
void MM_AllocationContextBalanced::removeRegionFromFlushedList(MM_HeapRegionDescriptorVLHGC* region)
{
    _flushedRegions.removeRegion(region);
}

MM_HeapRegionDescriptorVLHGC* MM_AllocationContextBalanced::selectRegionForContraction(MM_EnvironmentBase* env)
{
...
bool MM_AllocationContextBalanced::setNumaAffinityForThread(MM_EnvironmentBase* env)
{
    bool success = true;
