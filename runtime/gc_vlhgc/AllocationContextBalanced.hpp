...
private:
    ...
    MM_LightweightNonReentrantLock _freeListLock; /**< protects updates to the context's free list (managed distinctly
                                                     from the rest of the context to support region stealing */
    ...
    MM_HeapRegionDescriptorVLHGC*
        _allocationRegion; /**< The region currently satisfying allocations within this context - also the most recently
                              replenished/stolen region in this context */
    MM_RegionListTarok _nonFullRegions; /**< Regions which failed to satisfy a large object allocation but which are not
                                           yet full.  The _allocationRegion is added to this list when it fails to
                                           satisfy an object allocation but this list will quickly be consumed by TLH
                                           allocation requests which can soak up all remaining space.  This list is
                                           consulted after _allocationRegion but before replenishment or theft. */
    MM_RegionListTarok _discardRegionList; /**< The list of MPBP regions currently privately owned by this context but
                                              either too full or too fragmented to be used to satisfy allocations.
                                              These regions must be flushed back to the subspace before a collection */
    MM_RegionListTarok _flushedRegions; /**< The list of MPBP regions which have been flushed from active use, for a GC,
                                           and have unknown stats (at any point after the GC, however, these regions
                                           could all be migrated to _ownedRegions) */
    MM_RegionListTarok
        _freeRegions; /**< The list regions which are owned by this context but currently marked as FREE */
    MM_RegionListTarok
        _idleMPBPRegions; /**< The list regions which are owned by this context, currently marked as
                             BUMP_ALLOCATED_IDLE, but contain no objects (this also implies that they can migrate to
                             other contexts on this node, for free, since the receiver isn't using them) */
    UDATA _freeMemorySize; /**< The amount of free memory currently managed by the context in the _ownedRegions list
                              only (note that dark matter and small holes are not considered free memory).  This value
                              is always accurate (that is, there is no time when it becomes out of sync with the actual
                              amount of free memory managed by the context). */
    UDATA _regionCount[MM_HeapRegionDescriptor::LAST_REGION_TYPE]; /**< Count of regions that are owned by this AC
                                                                      (accurate only for TGC purposes) */
    ...
    MM_AllocationContextBalanced* _nextSibling; /**< Instances of the receiver are built into a circular linked-list.
                                                   This points to the next adjacent downstream neighbour in this list
                                                   (may be pointing to this if there is only one context in the node) */
    MM_AllocationContextBalanced*
        _cachedReplenishPoint; /**< The sibling context which most recently replenished the receiver */
    MM_AllocationContextBalanced*
        _stealingCousin; /**< A context in the "next" node which the receiver will use for spilling memory requests
                            across NUMA nodes.  Points back at the receiver if this is non-NUMA */
    MM_AllocationContextBalanced*
        _nextToSteal; /**< A pointer to the next context we will try to steal from (we steal in a round-robin to try to
                         distribute the heap's assymetry).  Points back at the receiver if this is non-NUMA */
    MM_HeapRegionManager* _heapRegionManager; /**< A cached pointer to the HeapRegionManager */
    UDATA* _freeProcessorNodes; /**< The array listing all the NUMA node numbers which account for the nodes with
                                   processors but no memory plus an empty slot for each context to use (element 0 is
                                   used by this context) - this is used when setting affinity */
    UDATA _freeProcessorNodeCount; /**< The length, in elements, of the _freeProcessorNodes array (always at least 1
                                      after startup) */

    /* Methods */
public:
    static MM_AllocationContextBalanced* newInstance(
    ...
    /**
     * Perform an allocate of the given allocationType and return it.  Note that the receiver can assume that either the
     * context is locked or the calling thread has exclusive.
     *
     ...
     */
    virtual void* lockedAllocate(MM_EnvironmentBase* env, MM_ObjectAllocationInterface* objectAllocationInterface,
        MM_AllocateDescription* allocateDescription, MM_MemorySubSpace::AllocationType allocationType);

    /**
     * Called when the given region has been deemed empty, but is not a FREE region.  The receiver is responsible for
     * changing the region type and ensuring that it is appropriately stored.
    ...
    /**
     * Called during both expansion and during region recycling (if the region was owned by this context, it will come
     * through this path when recycled).  This method can only accept FREE regions.
    ...
    /**
     * Return the amount of free memory currently managed by the context.  This value is always accurate (that is, there
     * is no time when it becomes out of sync with the actual amount of free memory managed by the context).
     *
    ...
    /**
     * Return the number of free (empty) regions currently managed by the context.   This value is always accurate (that
     * is, there is no time when it becomes out of sync with the actual number of regions managed by the context).
     *
     ...
     * @param sibling[in] The next downstream neighbour in the list (might be this if there is only one context on the
     * node)
     */
    void setNextSibling(MM_AllocationContextBalanced* sibling);

    /**
     * @return The next region in the circular linked list of contexts on this node (could be this if there is only one
     * context on the node)
     */
     ...
     * @param cousin[in] The context onto which the receiver will spill failed allocates within its node (must not be
     * NULL)
     */
    ...
    /**
     * Called to reset the largest free entry in all the MemoryPoolBumpPointer instances in the regions managed by the
     * receiver.
     */
    ...
    /**
     * Called to move the given region directly from the receiver and into the target context instance.  Note that this
     * method can only be called in one thread on any instance since it directly manipulates lists across contexts.  The
     * caller is responsible for ensuring that these threading semantics are preserved.
     ...
     */
    virtual void migrateRegionToAllocationContext(
    ...
    /**
     * Used by TGC to get the count of regions owned by this context.  Differentiates between regions node-local to the
     * context and node-foreign.
     * @param localCount[out] The number of regions owned by the receiver which are bound to the node in which the
     * receiver logically operates
     * @param foreignCount[out] The number of regions owned by the receiver which are NOT bound to the node in which the
     * receiver logically operates
     */
...
private:
    ...
    /**
     * Tries to acquire an MPBP region from the receiver (either from an existing, but idle, BUMP_ALLOCATED or by
     * converting a free region). If the receiver does not have a valid candidate it will check its siblings and cousins
     * for one. The region that this method returns will have been removed from the receiver's management and its owning
     * context is set to requestingContext.
     *
     ...
     */
    MM_HeapRegionDescriptorVLHGC* acquireMPBPRegionFromHeap(
        MM_EnvironmentBase* env, MM_MemorySubSpace* subspace, MM_AllocationContextTarok* requestingContext);

    /**
     * Tries to acquire a FREE region from the receiver (either from an existing FREE region or by converting an idle
     * region). If the receiver does not have a valid candidate it will check its siblings and cousins for one. The
     * region that this method returns will have been removed from the receiver's management and its owning context is
     * set to requestingContext.
     *
     * @note Caller must hold this context's @ref _contextLock
     *
     * @param env[in] The thread attempting the allocation
    ...
    /**
     * Returns a region descriptor of BUMP_ALLOCATED type on the node where the receiver is resident.  The region may
     * not have been found in the receiver (it may have been managed by a sibling) but it will be placed under the
     * management of the given requestingContext before this call returns.
     * @note The caller must own the receiver's _contextLock
     * @param env[in] The thread requesting the region
     * @param subSpace[in] The subSpace to which the allocated pool must be attached
     * @param requestingContext[in] The context which is requesting the region and asking this context to search its
     * node
     * @return A region of type BUMP_ALLOCATED (or NULL if no BUMP_ALLOCATED regions could be found or created on the
     * receiver's node)
     */
    MM_HeapRegionDescriptorVLHGC* acquireMPBPRegionFromNode(
        MM_EnvironmentBase* env, MM_MemorySubSpace* subSpace, MM_AllocationContextTarok* requestingContext);

    /**
     * Returns a region descriptor of FREE type on the node where the receiver is resident.  The region may not have
     * been found in the receiver (it may have been managed by a sibling) but it will not be under any context's
     * management, when it is returned and the caller accepts responsibility for managing it.
     * @note The caller must own the receiver's _contextLock
    ...
    /**
     * Tries to acquire an MPBP region from the receiver (either from an existing, but idle, BUMP_ALLOCATED or by
     * converting a free region). The region that this method returns will have been removed from the receiver's
     * management and its owning context is set to requestingContext.
     *
     * @param env[in] The thread attempting the allocation
     * @param subSpace[in] The subSpace to which the allocated pool must be attached
     * @paran requestingContext[in] The context requesting a region from the receiver
     ...
     */
    MM_HeapRegionDescriptorVLHGC* acquireMPBPRegionFromContext(
        MM_EnvironmentBase* env, MM_MemorySubSpace* subSpace, MM_AllocationContextTarok* requestingContext);

    /**
     * Tries to acquire a FREE region from the receiver (either from an existing FREE region or by converting an idle
     * region). The region that this method returns will have been removed from the receiver's management and its owning
     * context is set to requestingContext.
     *
     * @param env[in] The thread attempting the allocation
     * @return The region or NULL if there were none available in the receiver
    ...
    /**
     * Perform a TLH allocation.  Note that the receiver can assume that either the context is locked or the calling
     * thread has exclusive.
     *
     * @param[in] env The calling thread
     * @param[in] allocateDescription The allocation to perform
     * @param[in] objectAllocationInterface The interface through which the allocation request was initiated (required
     * to initialize the TLH)
     *
     * @return The base pointer of the allocated TLH (or NULL, if the allocation failed)
     */
    void* lockedAllocateTLH(MM_EnvironmentBase* env, MM_AllocateDescription* allocateDescription,
        MM_ObjectAllocationInterface* objectAllocationInterface);
    /**
     * Allocate an object.  Note that the receiver can assume that either the context is locked or the calling thread
     * has exclusive.
     *
     * @param[in] env The calling thread
     * @param[in] allocateDescription The allocation to perform
...
#if defined(J9VM_GC_ARRAYLETS)
    /**
     * Allocate an arraylet leaf.  Note that the receiver can assume that either the context is locked or the calling
     * thread has exclusive. NOTE:  The returned arraylet leaf is UNINITIALIZED MEMORY (since it can't be zeroed under
     * lock) so the caller must zero it before it can be seen by the collector or user code.
     *
     * @param[in] env The calling thread
     * @param[in] allocateDescription The allocation to perform
     ...
     */
    void* lockedAllocateArrayletLeaf(MM_EnvironmentBase* env, MM_AllocateDescription* allocateDescription,
        MM_HeapRegionDescriptorVLHGC* freeRegionForArrayletLeaf);
#endif /* defined(J9VM_GC_ARRAYLETS) */
    ...
    /**
     * Replenishes the receiver's active region list.  Called by lockedReplenishAndAllocate in the cases where we aren't
     * allocating a leaf.
     ...
     * @param localCount[in/out] The number of regions owned by the receiver which are bound to the node in which the
     * receiver logically operates
     * @param foreignCount[in/out] The number of regions owned by the receiver which are NOT bound to the node in which
     * the receiver logically operates
     */
    void accountForRegionLocation(MM_HeapRegionDescriptorVLHGC* region, UDATA* localCount, UDATA* foreignCount);

    /**
     * A helper counts the regions in a specific region list and differentiates between node-local and node-foreign
     * regions
     * @param localCount[out] The number of regions owned by the receiver which are bound to the node in which the
     * receiver logically operates
     * @param foreignCount[out] The number of regions owned by the receiver which are NOT bound to the node in which the
