...
protected:
private:
    const UDATA _allocationContextNumber; /**< A unique 0-indexed number assigned to this context instance (to be used
                                             for indexing into meta-data arrays, etc) */
    const AllocationContextType
        _allocationContextType; /**< The type of this AC, set during instantiation and used for instanceof checks **/
...
protected:
    ...
    /**
     * Replenish the receiver to satisfy the given allocation description and type and then re-attempt the allocation,
     ...
     * @param objectAllocationInterface[in] If this is a TLH allocation request, this will be the interface used to
     * initialize the TLH (is NULL in non-TLH allocations)
