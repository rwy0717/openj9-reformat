...
void MM_CardListFlushTask::masterSetup(MM_EnvironmentBase* env)
{
    Assert_MM_true(MM_CycleState::CT_PARTIAL_GARBAGE_COLLECTION == env->_cycleState->_collectionType);
}

void MM_CardListFlushTask::masterCleanup(MM_EnvironmentBase* env) {}

void MM_CardListFlushTask::writeFlushToCardState(Card* card, bool gmpIsActive)
{
    Card fromState = *card;
    switch (fromState) {
    case CARD_CLEAN:
    ...
    case CARD_PGC_MUST_SCAN:
        if (gmpIsActive) {
            /* PGC_MUST_SCAN is a slightly stronger statement than REMEMBERED so we only add the GMP requirement by
             * marking this DIRTY */
            *card = CARD_DIRTY;
        } else {
            /* do nothing */
...
void MM_CardListFlushTask::run(MM_EnvironmentBase* envBase)
{
    ...
    bool shouldFlushBuffersForUnregisteredRegions
        = interRegionRememberedSet->getShouldFlushBuffersForDecommitedRegions();
            ...
            if (region->_markData._shouldMark) {
                if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
                    ...
                    while (0 != (card = rsclCardIterator.nextReferencingCard(env))) {
                        ...
                        MM_HeapRegionDescriptorVLHGC* referencingRegion
                            = interRegionRememberedSet->tableDescriptorForRememberedSetCard(card);
                ...
                if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
                    /* flush the content of buffers owned by decommitted regions (but used/borrowed by other active
                     * regions) */
                    ...
                    while (NULL
                        != (cardBufferControlBlockCurrent
                               = rsclBufferIterator.nextBuffer(env, &lastCardInCurrentBuffer))) {
                        /* find a region owning current Buffer being iterated */
                        MM_HeapRegionDescriptorVLHGC* bufferOwningRegion
                            = interRegionRememberedSet->getBufferOwningRegion(cardBufferControlBlockCurrent);
                        ...
                        if (!bufferOwningRegion->isCommitted()) {
                            ...
                            for (MM_RememberedSetCard* cardSlot = cardBufferControlBlockCurrent->_card;
                                 cardSlot < lastCardInCurrentBuffer; cardSlot++) {
                                MM_RememberedSetCard card = *cardSlot;
                                MM_HeapRegionDescriptorVLHGC* referencingRegion
                                    = interRegionRememberedSet->tableDescriptorForRememberedSetCard(card);
                                if (referencingRegion->containsObjects() && !referencingRegion->_markData._shouldMark) {
                                    Card* cardAddress
                                        = interRegionRememberedSet->rememberedSetCardToCardAddr(env, card);
                                    writeFlushToCardState(cardAddress, gmpIsActive);
                                }
                                toRemoveCount += 1;
                            }
...
void MM_CardListFlushTask::setup(MM_EnvironmentBase* env)
{
...
void MM_CardListFlushTask::cleanup(MM_EnvironmentBase* env)
{
    if (env->isMasterThread()) {
        Assert_MM_true(_cycleState == env->_cycleState);
    } else {
...
void MM_CardListFlushTask::synchronizeGCThreads(MM_EnvironmentBase* env, const char* id)
{
    /* unused in this task */
    Assert_MM_unreachable();
}

bool MM_CardListFlushTask::synchronizeGCThreadsAndReleaseMaster(MM_EnvironmentBase* env, const char* id)
{
    /* unused in this task */
    Assert_MM_unreachable();
    return true;
...
bool MM_CardListFlushTask::synchronizeGCThreadsAndReleaseSingleThread(MM_EnvironmentBase* env, const char* id)
{
    /* unused in this task */
    Assert_MM_unreachable();
    return true;
