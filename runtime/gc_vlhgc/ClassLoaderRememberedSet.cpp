...
MM_ClassLoaderRememberedSet* MM_ClassLoaderRememberedSet::newInstance(MM_EnvironmentBase* env)
{
    MM_ClassLoaderRememberedSet* classLoaderRememberedSet = (MM_ClassLoaderRememberedSet*)env->getForge()->allocate(
        sizeof(MM_ClassLoaderRememberedSet), MM_AllocationCategory::REMEMBERED_SET, J9_GET_CALLSITE());
...
bool MM_ClassLoaderRememberedSet::initialize(MM_EnvironmentBase* env)
{
    ...
    if (_extensions->tarokEnableIncrementalClassGC) {
        _bitVectorPool = pool_new(_bitVectorSize * sizeof(UDATA), 0, sizeof(UDATA), 0, J9_GET_CALLSITE(),
            OMRMEM_CATEGORY_MM, poolAllocateHelper, poolFreeHelper, this);
...
void MM_ClassLoaderRememberedSet::tearDown(MM_EnvironmentBase* env)
{
    if (NULL != _bitVectorPool) {
...
void MM_ClassLoaderRememberedSet::kill(MM_EnvironmentBase* env)
{
    tearDown(env);
    env->getForge()->free(this);
}

void* MM_ClassLoaderRememberedSet::poolAllocateHelper(
    void* userData, U_32 size, const char* callSite, U_32 memoryCategory, U_32 type, U_32* doInit)
{
...
void MM_ClassLoaderRememberedSet::poolFreeHelper(void* userData, void* address, U_32 type)
{
...
void MM_ClassLoaderRememberedSet::rememberInstance(MM_EnvironmentBase* env, J9Object* object)
{
...
void MM_ClassLoaderRememberedSet::rememberRegionInternal(
    MM_EnvironmentBase* env, UDATA regionIndex, volatile UDATA* gcRememberedSetAddress)
{
...
void MM_ClassLoaderRememberedSet::installBitVector(MM_EnvironmentBase* env, volatile UDATA* gcRememberedSetAddress)
{
    _lock.acquire();
    UDATA gcRememberedSet = *gcRememberedSetAddress;
...
void MM_ClassLoaderRememberedSet::setBit(MM_EnvironmentBase* env, volatile UDATA* bitVector, UDATA bit)
{
...
bool MM_ClassLoaderRememberedSet::isBitSet(MM_EnvironmentBase* env, volatile UDATA* bitVector, UDATA bit)
{
    UDATA wordIndex = bit / BITS_PER_UDATA;
    UDATA bitIndex = bit % BITS_PER_UDATA;
    UDATA bitMask = ((UDATA)1) << bitIndex;
...
bool MM_ClassLoaderRememberedSet::isRemembered(MM_EnvironmentBase* env, J9ClassLoader* classLoader)
{
...
bool MM_ClassLoaderRememberedSet::isClassRemembered(MM_EnvironmentBase* env, J9Class* clazz)
{
...
bool MM_ClassLoaderRememberedSet::isRememberedInternal(MM_EnvironmentBase* env, UDATA gcRememberedSet)
{
...
bool MM_ClassLoaderRememberedSet::isInstanceRemembered(MM_EnvironmentBase* env, J9Object* object)
{
    bool isRemembered = false;
    Assert_MM_true(NULL != object);
...
bool MM_ClassLoaderRememberedSet::isRegionRemembered(MM_EnvironmentBase* env, UDATA regionIndex, UDATA gcRememberedSet)
{
    bool isRemembered = false;
    UDATA taggedRegionIndex = asTaggedRegionIndex(regionIndex);
...
void MM_ClassLoaderRememberedSet::killRememberedSet(MM_EnvironmentBase* env, J9ClassLoader* classLoader)
{
    Assert_MM_true(!J9_ARE_ANY_BITS_SET(classLoader->flags, J9CLASSLOADER_ANON_CLASS_LOADER));
...
void MM_ClassLoaderRememberedSet::killRememberedSetInternal(MM_EnvironmentBase* env, UDATA gcRememberedSet)
{
    if (0 == gcRememberedSet) {
        /* nothing to do */
    } else {
...
void MM_ClassLoaderRememberedSet::resetRegionsToClear(MM_EnvironmentBase* env)
{
    Assert_MM_true(NULL != _bitsToClear);
    memset(_bitsToClear, 0, _bitVectorSize * sizeof(UDATA));
}

void MM_ClassLoaderRememberedSet::prepareToClearRememberedSetForRegion(
...
void MM_ClassLoaderRememberedSet::clearRememberedSets(MM_EnvironmentBase* env)
{
...
void MM_ClassLoaderRememberedSet::clearRememberedSetsInternal(
    MM_EnvironmentBase* env, volatile UDATA* gcRememberedSetAddress)
{
    UDATA gcRememberedSet = *gcRememberedSetAddress;
    if (0 == gcRememberedSet) {
...
void MM_ClassLoaderRememberedSet::setupBeforeGC(MM_EnvironmentBase* env)
{
