...
bool MM_CollectionSetDelegate::initialize(MM_EnvironmentVLHGC* env)
{
...
void MM_CollectionSetDelegate::tearDown(MM_EnvironmentVLHGC* env)
{
    if (NULL != _setSelectionDataTable) {
...
static int compareRateOfReturnScoreFunc(const void* element1, const void* element2)
{
    ...
    /* We actually do the checks instead of simple math (w/ multiplier) to catch slight differences.  Perhaps if we
     * multiply by 1000 then the differences are so minor it doesn't matter?
     */
...
MM_CollectionSetDelegate::createNurseryCollectionSet(MM_EnvironmentVLHGC* env)
{
    ...
    while (NULL != (region = regionIterator.nextRegion())) {
        ...
        if (region->containsObjects()) {
            ...
            /* For CopyForwardHybrid mode we allow eden regions, which has jniCritical,to be part of nursery
             * collectionSet, those regions would be marked instead of copyforwarded. Would not add any non Eden regions
             * with jniCritical to collectionSet for copyforward For Non CopyForwardHybrid mode we do not check Eden
             * regions with jniCritical here, because we have already set MarkCompact PGC mode for the case in early. */
...
UDATA
MM_CollectionSetDelegate::selectRegionsForBudget(
    ...
    while ((0 != ageGroupBudgetRemaining) && (NULL != regionSelectionPtr)) {
        ...
        if (regionSelectionIndex >= regionSelectionThreshold) {
            ...
            Trc_MM_CollectionSetDelegate_selectRegionsForBudget(env->getLanguageVMThread(), tableIndex, compactGroup,
                (100 * freeMemory) / regionSize, (UDATA)0, (UDATA)0);
        }
...
void MM_CollectionSetDelegate::createRateOfReturnCollectionSet(MM_EnvironmentVLHGC* env, UDATA nurseryRegionCount)
{
    ...
    while ((0 != regionBudget) && (rorIndex < sortListSize)) {
        ...
        Trc_MM_CollectionSetDelegate_createRegionCollectionSetForPartialGC_dynamicRegionSelection(
            env->getLanguageVMThread(), rorEntry->_compactGroup, rorEntry->_regionCount,
            1000.0 * rorEntry->_rateOfReturn, compactGroupBudget, budgetConsumed);
...
void MM_CollectionSetDelegate::createCoreSamplingCollectionSet(MM_EnvironmentVLHGC* env, UDATA nurseryRegionCount)
{
...
void MM_CollectionSetDelegate::createRegionCollectionSetForPartialGC(MM_EnvironmentVLHGC* env)
{
    Assert_MM_true(MM_CycleState::CT_PARTIAL_GARBAGE_COLLECTION == env->_cycleState->_collectionType);
...
void MM_CollectionSetDelegate::deleteRegionCollectionSetForPartialGC(MM_EnvironmentVLHGC* env)
{
    Assert_MM_true(MM_CycleState::CT_PARTIAL_GARBAGE_COLLECTION == env->_cycleState->_collectionType);
...
void MM_CollectionSetDelegate::createRegionCollectionSetForGlobalGC(MM_EnvironmentVLHGC* env)
{
...
void MM_CollectionSetDelegate::deleteRegionCollectionSetForGlobalGC(MM_EnvironmentVLHGC* env)
{
    Assert_MM_true(MM_CycleState::CT_GLOBAL_GARBAGE_COLLECTION == env->_cycleState->_collectionType);
...
MM_HeapRegionDescriptorVLHGC* MM_CollectionSetDelegate::getNextRegion(MM_HeapRegionDescriptorVLHGC* cursor)
{
...
void MM_CollectionSetDelegate::rateOfReturnCalculationBeforeSweep(MM_EnvironmentVLHGC* env)
{
    if (_extensions->tarokEnableDynamicCollectionSetSelection) {
        UDATA compactGroupCount = MM_CompactGroupManager::getCompactGroupMaxCount(env);
        for (UDATA compactGroup = 0; compactGroup < compactGroupCount; compactGroup++) {
...
void MM_CollectionSetDelegate::rateOfReturnCalculationAfterSweep(MM_EnvironmentVLHGC* env)
{
    if (_extensions->tarokEnableDynamicCollectionSetSelection) {
