...
public:
     ...
     */
    MMINLINE static UDATA getCompactGroupNumberForAge(
        MM_EnvironmentVLHGC* env, UDATA age, MM_AllocationContextTarok* migrationDestination)
    {
     ...
     * @param migrationDestination The context to consider for compact group number instead of the owning context of the
     * given region
     * @return A 0-indexed value which can be used to look up the region's compact group if it were owned by
     * migrationDestination
     */
    MMINLINE static UDATA getCompactGroupNumberInContext(
        MM_EnvironmentVLHGC* env, MM_HeapRegionDescriptorVLHGC* region, MM_AllocationContextTarok* migrationDestination)
    {
     ...
     * @param env GC thread.
     * @return a 0-indexed value which represents the maximum that can be returned as a compact group (duration of the
     * system).
     */
    ...
    MMINLINE static bool isRegionDCSSCandidate(MM_EnvironmentVLHGC* env, MM_HeapRegionDescriptorVLHGC* region)
    {
        ...
        if (extensions->tarokAllocationAgeEnabled) {
            result
                = !isRegionInNursery(env, region) && (region->getAllocationAge() < extensions->tarokMaximumAgeInBytes);
        } else {
    ...
    MMINLINE static bool isCompactGroupDCSSCandidate(MM_EnvironmentVLHGC* env, UDATA compactGroupNumber)
    {
         ...
         */
        result = (ageOfGroup > extensions->tarokNurseryMaxAge._valueSpecified)
            && (ageOfGroup < extensions->tarokRegionMaxAge);
...
protected:
private:
    MM_CompactGroupManager() { _typeId = __FUNCTION__; }
};
