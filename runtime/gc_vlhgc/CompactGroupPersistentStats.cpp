...
MM_CompactGroupPersistentStats* MM_CompactGroupPersistentStats::allocateCompactGroupPersistentStats(
    MM_EnvironmentVLHGC* env)
{
    ...
    MM_CompactGroupPersistentStats* result = (MM_CompactGroupPersistentStats*)extensions->getForge()->allocate(
        persistentStatsAllocationSize, MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
    if (NULL != result) {
        ...
        for (UDATA i = 0; i < compactGroupCount; i++) {
            ...
            if (0 != extensions->tarokAllocationAgeUnit) {
                ...
                if (ageGroup != extensions->tarokRegionMaxAge) {
                    result[i]._maxAllocationAge
                        = MM_CompactGroupManager::calculateMaximumAllocationAge(env, ageGroup + 1);
                } else {
...
void MM_CompactGroupPersistentStats::killCompactGroupPersistentStats(
    MM_EnvironmentVLHGC* env, MM_CompactGroupPersistentStats* persistentStats)
{
    MM_GCExtensions::getExtensions(env)->getForge()->free(persistentStats);
}

void MM_CompactGroupPersistentStats::deriveWeightedSurvivalRates(
    MM_EnvironmentVLHGC* env, MM_CompactGroupPersistentStats* persistentStats)
{
    ...
    for (UDATA contextIndex = 0; contextIndex < managedAllocationContextCount; contextIndex++) {
        ...
        for (UDATA age = regionMaxAge; age <= regionMaxAge; age--) {
            ...
            Trc_MM_CompactGroupPersistentStats_deriveWeightedSurvivalRates_group(
                env->getLanguageVMThread(), contextIndex, age, thisSurvivalRate, weightedSurvivalRate);
...
void MM_CompactGroupPersistentStats::calculateAgeGroupFractionsAtEdenBoundary(MM_EnvironmentVLHGC* env,
    U_64 ageInThisAgeGroup, U_64* ageInThisCompactGroup, U_64 currentAge, U_64 allocatedSinceLastPGC,
    U_64* edenFractionOfCompactGroup, U_64* nonEdenFractionOfCompactGroup)
{
    ...
    /* we do not have historic info which fraction of this age group was allocated in this compact group; just take a
     * fair share */
...
void MM_CompactGroupPersistentStats::updateProjectedSurvivalRate(
    MM_EnvironmentVLHGC* env, MM_CompactGroupPersistentStats* persistentStatsArray, UDATA compactGroup)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
    ...
    if (liveBeforeCollect > 0) {
        ...
        /* Note that this calculation doesn't try to limit the maximum impact of our new survival rate on the historic
         * score which is consistent with the existing _rorStats.  Since we always collect all of the nursery, this
         * means that we will always replace the survival rate data of the nursery after each PGC
         */
        ...
        /* Due to dark matter estimates the survival rate could appear to exceed 1.0 by a small amount, so cap it
         * at 1.0. */
        ...
        double newSurvivalRate
            = (weightOfOldStats * persistentStats->_historicalSurvivalRate) + (weightOfNewStats * thisSurvivalRate);
    ...
    U_64 allocatedSinceLastPGC
        = ((MM_IncrementalGenerationalGC*)extensions->getGlobalCollector())->getAllocatedSinceLastPGC();
    if ((projectedLiveBytesSelected > 0) && (allocatedSinceLastPGC > 0)) {
        ...
        UDATA measuredLiveBytesNotParticipating = (persistentStats->_measuredLiveBytesBeforeCollectInGroup
            - persistentStats->_measuredLiveBytesBeforeCollectInCollectedSet);
        UDATA measuredLiveBytesAfter
            = (persistentStats->_measuredLiveBytesAfterCollectInCollectedSet + measuredLiveBytesNotParticipating);
        double oldSurvivalRate = persistentStats->_projectedInstantaneousSurvivalRatePerAgeUnit;
        /* By definition Instantaneous Survival Rate = (live bytes at the end of the current age group)/(live bytes at
         * the end of the previous age group) per tarokAllocationAgeUnit (tarokAllocationAgeUnit defaults to Eden size,
         * but could be far less than Eden size)
         */

        /* We'll initially find survivorRate since last PGC and take out survivor rate of previous age groups to get
         * survivor rate of current age group */
        U_64 bytesRemaining = allocatedSinceLastPGC;
        /* For Eden age groups, we'll take out "only" age groups since the moment they were created (they may not exist
         * at previous PGC) */
        ...
        /* ageInThisCompactGroup essentially represents the amount of bytes allocated in this compact group out of this
         * age group */
        U_64 ageInThisCompactGroup = 0;

        UDATA currentCompactGroup = compactGroup;

        U_64 edenFractionOfCompactGroup
            = persistentStats->_projectedLiveBytesAfterPreviousPGCInCollectedSetForEdenFraction;
        U_64 nonEdenFractionOfCompactGroup = 0;
        calculateAgeGroupFractionsAtEdenBoundary(env, ageInThisAgeGroup, &ageInThisCompactGroup, currentAge,
            allocatedSinceLastPGC, &edenFractionOfCompactGroup, &nonEdenFractionOfCompactGroup);

        /* Calculate separately survivorRate for Eden and non Eden fractions of the age group */
        double thisSurvivalRateForEdenFraction
            = (double)(measuredLiveBytesAfter - measuredLiveBytesNotParticipating) / ageInThisCompactGroup;
        ...
        double projectedLiveBytesAfterPreviousPGCInCollectedSetForNonEdenFraction
            = (double)(persistentStats->_projectedLiveBytesAfterPreviousPGCInCollectedSet - edenFractionOfCompactGroup);
        Trc_MM_CompactGroupPersistentStats_updateProjectedSurvivalRate_Entry(env->getLanguageVMThread(), compactGroup,
            (double)persistentStats->_projectedLiveBytesAfterPreviousPGCInCollectedSetForNonEdenFraction
                / (1024 * 1024),
            ...
            (double)measuredLiveBytesAfter / (1024 * 1024), (double)measuredLiveBytesNotParticipating / (1024 * 1024),
            projectedLiveBytesAfterPreviousPGCInCollectedSetForNonEdenFraction / (1024 * 1024));
        Trc_MM_CompactGroupPersistentStats_calculateAgeGroupFractions(env->getLanguageVMThread(),
            persistentStats->_maxAllocationAge, maxAgeInThisAgeGroup, ageInThisAgeGroup, edenFractionOfCompactGroup,
            nonEdenFractionOfCompactGroup);

        /* TODO: check cases when projectedLiveBytesAfterPreviousPGCInCollectedSetForNonEdenFraction < 0. Should be
         * completely or almost non-existent */
        if (projectedLiveBytesAfterPreviousPGCInCollectedSetForNonEdenFraction > 0.0) {
            thisSurvivalRateForNonEdenFraction = thisSurvivalRateForEdenFraction * nonEdenFractionOfCompactGroup
                / projectedLiveBytesAfterPreviousPGCInCollectedSetForNonEdenFraction;
        }
        if (thisSurvivalRateForNonEdenFraction > 1.0) {
            thisSurvivalRateForNonEdenFraction = 1.0;
        }

        /* Skip all ageUnits in current compact group, to the maximum of what we allocated since last GC - this is where
         * we want to calculate survivorRate */
        ...
        while (((bytesRemaining > 0) || (0 != edenFractionOfCompactGroup)) && (currentAge > 0)) {
            ...
            double baseSurvivalRate
                = persistentStatsArray[currentCompactGroup]._projectedInstantaneousSurvivalRateThisPGCPerAgeUnit;
            ...
            Trc_MM_CompactGroupPersistentStats_updateProjectedSurvivalRate_eden(env->getLanguageVMThread(),
                thisSurvivalRateForEdenFraction, (double)currentAge / (1024 * 1024),
                (double)bytesRemaining / (1024 * 1024), (double)ageInCurrentGroupForEden / (1024 * 1024), survivalRate,
                baseSurvivalRate, ageUnitsInCurrentGroup, currentCompactGroup);
            ...
            Trc_MM_CompactGroupPersistentStats_updateProjectedSurvivalRate_nonEden(env->getLanguageVMThread(),
                thisSurvivalRateForNonEdenFraction, (double)currentAge / (1024 * 1024),
                (double)bytesRemaining / (1024 * 1024), (double)ageInCurrentGroupForNonEden / (1024 * 1024),
                survivalRate, baseSurvivalRate, ageUnitsInCurrentGroup, currentCompactGroup);
        ...
        if (0 != ageInThisCompactGroup) {
            weightedMeanSurvivalRate = (thisSurvivalRateForEdenFraction * edenFractionOfCompactGroup
                                           + thisSurvivalRateForNonEdenFraction * nonEdenFractionOfCompactGroup)
                / ageInThisCompactGroup;
        }
        ...
        if (projectedLiveBytesBefore > 0) {
            observedWeight
                = newObservationWeight * (double)projectedLiveBytesSelected / (double)projectedLiveBytesBefore;
        }
        persistentStats->_projectedInstantaneousSurvivalRatePerAgeUnit
            = (observedWeight * thisSurvivalRate) + ((1.0 - observedWeight) * oldSurvivalRate);
        persistentStats->_projectedInstantaneousSurvivalRate
            = pow(persistentStats->_projectedInstantaneousSurvivalRatePerAgeUnit, maxAgeUnitsInThisGroup);

        Trc_MM_CompactGroupPersistentStats_updateProjectedSurvivalRate_Exit(env->getLanguageVMThread(), compactGroup,
            thisSurvivalRateForEdenFraction, thisSurvivalRateForNonEdenFraction, weightedMeanSurvivalRate,
            ageUnitsInThisAgeGroup, thisSurvivalRate, persistentStats->_projectedInstantaneousSurvivalRate,
            persistentStats->_projectedInstantaneousSurvivalRatePerAgeUnit);
    }
}

void MM_CompactGroupPersistentStats::deriveProjectedLiveBytesStats(
    MM_EnvironmentVLHGC* env, MM_CompactGroupPersistentStats* persistentStats)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
...
void MM_CompactGroupPersistentStats::resetLiveBytesStats(
    MM_EnvironmentVLHGC* env, MM_CompactGroupPersistentStats* persistentStats)
{
    UDATA compactGroupCount = MM_CompactGroupManager::getCompactGroupMaxCount(env);
    for (UDATA compactGroup = 0; compactGroup < compactGroupCount; compactGroup++) {
...
void MM_CompactGroupPersistentStats::calculateLiveBytesForRegion(MM_EnvironmentVLHGC* env,
    MM_CompactGroupPersistentStats* persistentStats, UDATA compactGroup, MM_HeapRegionDescriptorVLHGC* region,
    UDATA measuredLiveBytes, UDATA projectedLiveBytes)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
    U_64 allocatedSinceLastPGC
        = ((MM_IncrementalGenerationalGC*)extensions->getGlobalCollector())->getAllocatedSinceLastPGC();
    ...
    if (region->isEden()) {
        persistentStats[compactGroup]._projectedLiveBytesAfterPreviousPGCInCollectedSetForEdenFraction
            += region->_projectedLiveBytesPreviousPGC;
        persistentStats[compactGroup]._projectedLiveBytesAfterPreviousPGCInCollectedSet
            += region->_projectedLiveBytesPreviousPGC;
    } else {
        ...
        /* Only the upper portion of the non-Eden span is a good representative. About to calculate the size of the
         * sample */
        U_64 sampleSpan = 0;

        /* We use a premise: given a point of time m (expressed in MB allocated), liveness of objects are constant
         * between m/expBase and m. For non, linear aging this range (m/expBase,m) can span multiple age groups, which
         * may have different liveness.
         */
        ...
        if (maxAllocationAge > allocatedSinceLastPGC) {
            ...
            /* between oldestAgePointObjectsCameFrom - sampleSpan and oldestAgePointObjectsCameFrom, we consider objects
             * had same liveness. */
            /* however, object are already forward-aged at the end of previous PGC, so we are talking about
             * (maxAllocationAge - sampleSpan, maxAllocationAge) span */
        ...
        if (region->getAllocationAge() >= boundary) {
            ...
            if (region->getLowerAgeBound() < boundary) {
                ...
                UDATA liveBytesBelowBoundary
                    = (UDATA)(liveBytesBelowAverageAge * (boundary - region->getLowerAgeBound())
            ...
            if (region->getUpperAgeBound() > boundary) {
                ...
                liveBytesAboveBoundary = (UDATA)(liveBytesAboveAverageAge * (region->getUpperAgeBound() - boundary)
                    / (region->getUpperAgeBound() - region->getAllocationAge()));
            }
        }

        liveBytesAboveBoundary = (UDATA)(nonEdenSpanExpandRatio * liveBytesAboveBoundary);
        persistentStats[compactGroup]._projectedLiveBytesAfterPreviousPGCInCollectedSetForNonEdenFraction
...
void MM_CompactGroupPersistentStats::updateStatsBeforeCopyForward(
    MM_EnvironmentVLHGC* env, MM_CompactGroupPersistentStats* persistentStats)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
        ...
        if (region->containsObjects()) {
            ...
            if (!persistentStats[compactGroup]._statsHaveBeenUpdatedThisCycle) {
                ...
                if (region->_markData._shouldMark) {
                    calculateLiveBytesForRegion(
                        env, persistentStats, compactGroup, region, measuredLiveBytes, projectedLiveBytes);
                }
            }
        }
    }
...
void MM_CompactGroupPersistentStats::updateStatsAfterCopyForward(
    MM_EnvironmentVLHGC* env, MM_CompactGroupPersistentStats* persistentStats)
{
    UDATA compactGroupCount = MM_CompactGroupManager::getCompactGroupMaxCount(env);
    for (UDATA compactGroup = 0; compactGroup < compactGroupCount; compactGroup++) {
        if (!persistentStats[compactGroup]._statsHaveBeenUpdatedThisCycle) {
            ...
            if (liveBeforeCollect > 0) {
                ...
                UDATA totalBytesAfterCollect
                    = (persistentStats[compactGroup]._measuredBytesCopiedFromGroupDuringCopyForward
                        + measuredNonParticipatingLiveBytes);
                ...
                persistentStats[compactGroup]._measuredLiveBytesAfterCollectInCollectedSet
...
void MM_CompactGroupPersistentStats::updateStatsBeforeSweep(
    MM_EnvironmentVLHGC* env, MM_CompactGroupPersistentStats* persistentStats)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
    MM_HeapRegionManager* regionManager = extensions->heapRegionManager;
...
void MM_CompactGroupPersistentStats::updateStatsAfterSweep(
    MM_EnvironmentVLHGC* env, MM_CompactGroupPersistentStats* persistentStats)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
    MM_HeapRegionManager* regionManager = extensions->heapRegionManager;
...
void MM_CompactGroupPersistentStats::updateStatsBeforeCompact(
    MM_EnvironmentVLHGC* env, MM_CompactGroupPersistentStats* persistentStats)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
    MM_HeapRegionManager* regionManager = extensions->heapRegionManager;
...
void MM_CompactGroupPersistentStats::updateStatsAfterCompact(
    MM_EnvironmentVLHGC* env, MM_CompactGroupPersistentStats* persistentStats)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
    MM_HeapRegionManager* regionManager = extensions->heapRegionManager;
...
void MM_CompactGroupPersistentStats::initProjectedLiveBytes(MM_EnvironmentVLHGC* env)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
    GC_HeapRegionIteratorVLHGC regionIterator(extensions->heapRegionManager, MM_HeapRegionDescriptor::ALL);
    MM_HeapRegionDescriptorVLHGC* region = NULL;
...
void MM_CompactGroupPersistentStats::updateStatsAfterCollectionOperation(
    MM_EnvironmentVLHGC* env, MM_CompactGroupPersistentStats* persistentStats)
{
...
void MM_CompactGroupPersistentStats::decayProjectedLiveBytesForRegions(MM_EnvironmentVLHGC* env)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
    GC_HeapRegionIteratorVLHGC regionIterator(extensions->heapRegionManager, MM_HeapRegionDescriptor::ALL);
    MM_HeapRegionDescriptorVLHGC* region = NULL;
    ...
    while (NULL != (region = regionIterator.nextRegion())) {
        if (region->containsObjects()) {
            ...
            while ((bytesRemaining > 0) && (currentAge > 0)) {
                ...
                U_64 ageInThisGroup
                    = OMR_MIN((U_64)bytesRemaining, (U_64)currentAge - minAllocationAgeForThisCompactGroup);
                ...
                Trc_MM_CompactGroupPersistentStats_decayProjectedLiveBytesForRegions(env->getLanguageVMThread(),
                    extensions->heapRegionManager->mapDescriptorToRegionTableIndex(region),
                    (double)oldProjectedLiveBytes / (1024 * 1024), (double)region->_projectedLiveBytes / (1024 * 1024),
                    compactGroup, (double)bytesRemaining / (1024 * 1024), (double)currentAge / (1024 * 1024),
                    survivalRate, baseSurvivalRate, ageUnitsInThisGroup, currentCompactGroup);
...
void MM_CompactGroupPersistentStats::updateStatsBeforeCollect(
    MM_EnvironmentVLHGC* env, MM_CompactGroupPersistentStats* persistentStats)
{
