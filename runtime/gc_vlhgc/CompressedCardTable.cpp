 ...
 * 1 means bit per card, so compressed card table has full information and no need to look at original card table
 * element We can try to reduce size if compressed card table representing more then one card per bit however if
 * compressed card table bit is set an original card must be checked as well
 */
#define COMPRESSED_CARD_TABLE_DIV 1

MM_CompressedCardTable* MM_CompressedCardTable::newInstance(MM_EnvironmentBase* env, MM_Heap* heap)
{
    MM_CompressedCardTable* compressedCardTable = (MM_CompressedCardTable*)env->getForge()->allocate(
        sizeof(MM_CompressedCardTable), MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
...
bool MM_CompressedCardTable::initialize(MM_EnvironmentBase* env, MM_Heap* heap)
{
    /* heap maximum physical range must be aligned with compressed card table word */
    Assert_MM_true(
        0 == (heap->getMaximumPhysicalRange() % (CARD_SIZE * COMPRESSED_CARD_TABLE_DIV * COMPRESSED_CARDS_PER_WORD)));

    /* Calculate compressed card table size in bytes */
    UDATA compressedCardTableSize
        = heap->getMaximumPhysicalRange() / (CARD_SIZE * COMPRESSED_CARD_TABLE_DIV * BITS_PER_BYTE);

    /* Allocate compressed card table */
    _compressedCardTable
        = (UDATA*)env->getForge()->allocate(compressedCardTableSize, MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
...
void MM_CompressedCardTable::kill(MM_EnvironmentBase* env)
{
    tearDown(env);
    env->getForge()->free(this);
}

void MM_CompressedCardTable::tearDown(MM_EnvironmentBase* env)
{
...
MMINLINE bool MM_CompressedCardTable::isDirtyCardForPartialCollect(Card state)
{
    bool result = false;
...
void MM_CompressedCardTable::setCompressedCardsDirtyForPartialCollect(void* startHeapAddress, void* endHeapAddress)
{
...
void MM_CompressedCardTable::rebuildCompressedCardTableForPartialCollect(
    MM_EnvironmentBase* env, void* startHeapAddress, void* endHeapAddress)
{
...
bool MM_CompressedCardTable::isCompressedCardDirtyForPartialCollect(MM_EnvironmentBase* env, void* heapAddr)
{
...
void MM_CompressedCardTable::cleanCardsInRegion(
...
void MM_CompressedCardTable::cleanCardsInRange(
    MM_EnvironmentBase* env, MM_CardCleaner* cardCleaner, void* startHeapAddress, void* endHeapAddress)
{
    UDATA compressedCardStartOffset = ((UDATA)startHeapAddress - _heapBase) / (CARD_SIZE * COMPRESSED_CARD_TABLE_DIV);
    UDATA compressedCardEndOffset = ((UDATA)endHeapAddress - _heapBase) / (CARD_SIZE * COMPRESSED_CARD_TABLE_DIV);
    UDATA compressedCardStartIndex = compressedCardStartOffset / COMPRESSED_CARDS_PER_WORD;
...
bool MM_CompressedCardTable::isReady()
{
