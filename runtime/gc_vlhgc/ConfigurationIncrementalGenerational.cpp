...
MM_Configuration* MM_ConfigurationIncrementalGenerational::newInstance(MM_EnvironmentBase* env)
{
    MM_ConfigurationIncrementalGenerational* configuration
        = (MM_ConfigurationIncrementalGenerational*)env->getForge()->allocate(
            sizeof(MM_ConfigurationIncrementalGenerational), MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
 ...
 */
MM_GlobalCollector* MM_ConfigurationIncrementalGenerational::createGlobalCollector(MM_EnvironmentBase* envBase)
{
 ...
 */
MM_Heap* MM_ConfigurationIncrementalGenerational::createHeapWithManager(
    MM_EnvironmentBase* env, UDATA heapBytesRequested, MM_HeapRegionManager* regionManager)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);

    MM_Heap* heap
        = MM_HeapVirtualMemory::newInstance(env, extensions->heapAlignment, heapBytesRequested, regionManager);
    if (NULL != heap) {
        /* when we try to attach this heap to a region manager, we will need the card table since it needs to be
         * NUMA-affinitized using the same logic as the heap so initialize it here */
        ...
        if (NULL == extensions->cardTable) {
            ...
            if (extensions->tarokEnableCompressedCardTable) {
                extensions->compressedCardTable
                    = MM_CompressedCardTable::newInstance(MM_EnvironmentVLHGC::getEnvironment(env), heap);
...
MM_EnvironmentBase* MM_ConfigurationIncrementalGenerational::allocateNewEnvironment(
    MM_GCExtensionsBase* extensions, OMR_VMThread* omrVMThread)
{
    return MM_EnvironmentVLHGC::newInstance(extensions, omrVMThread);
}

J9Pool* MM_ConfigurationIncrementalGenerational::createEnvironmentPool(MM_EnvironmentBase* env)
{
     ...
     * to OS page size) */
    return pool_new(sizeof(MM_EnvironmentVLHGC), numberOfElements, sizeof(U_64), 0, J9_GET_CALLSITE(),
        OMRMEM_CATEGORY_MM, POOL_FOR_PORT(PORTLIB));
}

bool MM_ConfigurationIncrementalGenerational::initializeEnvironment(MM_EnvironmentBase* env)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
...
MM_MemorySpace* MM_ConfigurationIncrementalGenerational::createDefaultMemorySpace(
    MM_EnvironmentBase* env, MM_Heap* heap, MM_InitializationParameters* parameters)
{
    ...
    extensions->sweepPoolManagerBumpPointer
        = MM_SweepPoolManagerVLHGC::newInstance(MM_EnvironmentVLHGC::getEnvironment(env));
    ...
    extensions->rememberedSetCardBucketPool = (MM_RememberedSetCardBucket*)extensions->getForge()->allocate(
        allocateSize, MM_AllocationCategory::REMEMBERED_SET, J9_GET_CALLSITE());
    ...
    MM_MemorySubSpaceTarok* memorySubspaceTarok = MM_MemorySubSpaceTarok::newInstance(env, physicalSubArena, gamt,
        usesGlobalCollector, parameters->_minimumSpaceSize, parameters->_initialOldSpaceSize,
        parameters->_maximumSpaceSize, MEMORY_TYPE_OLD, 0);
    if (NULL == memorySubspaceTarok) {
        return NULL;
    }
    /* the subspace exists so now we can request that the allocation contexts are created (since they require the
     * subspace) */
    ...
    return MM_MemorySpace::newInstance(env, heap, physicalArena, memorySubspaceTarok, parameters,
        MEMORY_SPACE_NAME_FLAT, MEMORY_SPACE_DESCRIPTION_FLAT);
}

void MM_ConfigurationIncrementalGenerational::defaultMemorySpaceAllocated(
    MM_GCExtensionsBase* extensions, void* defaultMemorySpace)
{
    ...
    /* initialize TaxationThreshold and RememberedSetCardBucketPool before first gc occurs and all of dependance has
     * been set. */
...
bool MM_ConfigurationIncrementalGenerational::initialize(MM_EnvironmentBase* env)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
...
#define DEFAULT_MAX_AGE_FOR_ALLOCATION_BASED 5
    ...
    if (!extensions->tarokMinimumGMPWorkTargetBytes._wasSpecified) {
        /* default to a region size.  No real reason for choosing this number other than that it is sized relative to
         * the heap */
...
void MM_ConfigurationIncrementalGenerational::tearDown(MM_EnvironmentBase* env)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
...
MM_HeapRegionManager* MM_ConfigurationIncrementalGenerational::createHeapRegionManager(MM_EnvironmentBase* env)
{
    MM_GCExtensionsBase* extensions = env->getExtensions();
    return MM_HeapRegionManagerVLHGC::newInstance(env, extensions->regionSize, sizeof(MM_HeapRegionDescriptorVLHGC),
        MM_HeapRegionDescriptorVLHGC::initializer, MM_HeapRegionDescriptorVLHGC::destructor);
}

void MM_ConfigurationIncrementalGenerational::cleanUpClassLoader(MM_EnvironmentBase* env, J9ClassLoader* classLoader)
{
    MM_GCExtensionsBase* extensions = env->getExtensions();
...
void MM_ConfigurationIncrementalGenerational::prepareParameters(OMR_VM* omrVM, UDATA minimumSpaceSize,
    UDATA minimumNewSpaceSize, UDATA initialNewSpaceSize, UDATA maximumNewSpaceSize, UDATA minimumTenureSpaceSize,
    UDATA initialTenureSpaceSize, UDATA maximumTenureSpaceSize, UDATA memoryMax, UDATA tenureFlags,
    MM_InitializationParameters* parameters)
{
    ...
    MM_Configuration::prepareParameters(omrVM, minimumSpaceSize, minimumNewSpaceSize, initialNewSpaceSize,
        maximumNewSpaceSize, minimumTenureSpaceSize, initialTenureSpaceSize, maximumTenureSpaceSize, memoryMax,
        tenureFlags, parameters);
}

bool MM_ConfigurationIncrementalGenerational::verifyRegionSize(MM_EnvironmentBase* env, UDATA regionSize)
{
    return regionSize >= TAROK_MINIMUM_REGION_SIZE_BYTES;
}

bool MM_ConfigurationIncrementalGenerational::initializeNUMAManager(MM_EnvironmentBase* env)
{
    MM_GCExtensionsBase* extensions = env->getExtensions();
    ...
    if (result && !disabledByForce) {
        ...
        if ((1 + affinityLeaderCount)
            != MM_GlobalAllocationManagerTarok::calculateIdealManagedContextCount(extensions)) {
