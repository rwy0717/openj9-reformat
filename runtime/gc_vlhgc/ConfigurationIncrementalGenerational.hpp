...
public:
    ...
    virtual void prepareParameters(OMR_VM* omrVM, UDATA minimumSpaceSize, UDATA minimumNewSpaceSize,
        UDATA initialNewSpaceSize, UDATA maximumNewSpaceSize, UDATA minimumTenureSpaceSize,
    ...
    MM_ConfigurationIncrementalGenerational(MM_EnvironmentBase* env)
        ...
        : MM_Configuration(env, gc_policy_balanced, mm_regionAlignment, calculateDefaultRegionSize(env), 0,
              gc_modron_wrtbar_cardmark_incremental, gc_modron_allocation_type_tlh)
#else
        : MM_Configuration(env, gc_policy_balanced, mm_regionAlignment, calculateDefaultRegionSize(env), 0,
              gc_modron_wrtbar_cardmark, gc_modron_allocation_type_tlh)
...
protected:
     ...
     *
     * lpnguyen TODO: move this out of configuration as we should not have to "configure" NUMA.  The only reason this is
     * here is because ConfigurationIncrementalGenerational.hpp will disable physical NUMA if it would create too many
     * ACs and ideal AC calculation requires configuration to be done (regionSize set up).
     *
     * @param env[in] - the current environment
     */
...
private:
    static UDATA calculateDefaultRegionSize(MM_EnvironmentBase* env)
    {
        UDATA regionSize = 0;
