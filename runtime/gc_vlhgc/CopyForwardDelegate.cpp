...
bool MM_CopyForwardDelegate::initialize(MM_EnvironmentVLHGC* env)
{
...
void MM_CopyForwardDelegate::tearDown(MM_EnvironmentVLHGC* env)
{
...
bool MM_CopyForwardDelegate::performCopyForwardForPartialGC(MM_EnvironmentVLHGC* env)
{
...
void MM_CopyForwardDelegate::preCopyForwardSetup(MM_EnvironmentVLHGC* env) {}

void MM_CopyForwardDelegate::postCopyForwardCleanup(MM_EnvironmentVLHGC* env)
{
...
MM_CopyForwardDelegate::estimateRequiredSurvivorBytes(MM_EnvironmentVLHGC* env)
{
    ...
    while (NULL != (region = regionIterator.nextRegion())) {
        if (region->_markData._shouldMark) {
            ...
            if (region->isEden()) {
                /* if a GMP just completed, there may be marked regions in Eden. We still use getAllocatableBytes() for
                 * them because our survival rate is for the whole region, not just for objects which survived a GMP */
                Assert_MM_true((MM_HeapRegionDescriptor::BUMP_ALLOCATED == region->getRegionType())
                    || (MM_HeapRegionDescriptor::BUMP_ALLOCATED_MARKED == region->getRegionType()));
