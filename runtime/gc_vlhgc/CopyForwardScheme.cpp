...
public:
     ...
     */
    MM_CopyForwardSchemeTask(MM_EnvironmentVLHGC* env, MM_Dispatcher* dispatcher,
        MM_CopyForwardScheme* copyForwardScheme, MM_CycleState* cycleState)
...
#if defined(J9MODRON_TGC_PARALLEL_STATISTICS)
void MM_CopyForwardSchemeTask::synchronizeGCThreads(MM_EnvironmentBase* envBase, const char* id)
{
...
bool MM_CopyForwardSchemeTask::synchronizeGCThreadsAndReleaseMaster(MM_EnvironmentBase* envBase, const char* id)
{
    PORT_ACCESS_FROM_ENVIRONMENT(envBase);
    MM_EnvironmentVLHGC* env = MM_EnvironmentVLHGC::getEnvironment(envBase);
    U_64 startTime = j9time_hires_clock();
...
bool MM_CopyForwardSchemeTask::synchronizeGCThreadsAndReleaseSingleThread(MM_EnvironmentBase* envBase, const char* id)
{
    PORT_ACCESS_FROM_ENVIRONMENT(envBase);
    MM_EnvironmentVLHGC* env = MM_EnvironmentVLHGC::getEnvironment(envBase);
    U_64 startTime = j9time_hires_clock();
...
bool MM_CopyForwardSchemeTask::synchronizeGCThreadsAndReleaseMasterForAbort(MM_EnvironmentBase* envBase, const char* id)
{
    PORT_ACCESS_FROM_ENVIRONMENT(envBase);
    MM_EnvironmentVLHGC* env = MM_EnvironmentVLHGC::getEnvironment(envBase);
    U_64 startTime = j9time_hires_clock();
...
void MM_CopyForwardSchemeTask::synchronizeGCThreadsForMark(MM_EnvironmentBase* envBase, const char* id)
{
    PORT_ACCESS_FROM_ENVIRONMENT(envBase);
    MM_EnvironmentVLHGC* env = MM_EnvironmentVLHGC::getEnvironment(envBase);
    U_64 startTime = j9time_hires_clock();
...
bool MM_CopyForwardSchemeTask::synchronizeGCThreadsAndReleaseMasterForMark(MM_EnvironmentBase* envBase, const char* id)
{
    PORT_ACCESS_FROM_ENVIRONMENT(envBase);
    MM_EnvironmentVLHGC* env = MM_EnvironmentVLHGC::getEnvironment(envBase);
    U_64 startTime = j9time_hires_clock();
...
void MM_CopyForwardSchemeTask::synchronizeGCThreadsForInterRegionRememberedSet(
...
MM_CopyForwardScheme* MM_CopyForwardScheme::newInstance(MM_EnvironmentVLHGC* env, MM_HeapRegionManager* manager)
{
    MM_CopyForwardScheme* scheme = (MM_CopyForwardScheme*)env->getForge()->allocate(
        sizeof(MM_CopyForwardScheme), MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
    if (scheme) {
...
void MM_CopyForwardScheme::kill(MM_EnvironmentVLHGC* env)
{
    tearDown(env);
    env->getForge()->free(this);
}

bool MM_CopyForwardScheme::initialize(MM_EnvironmentVLHGC* env)
{
    ...
    _cacheScanLists = (MM_CopyScanCacheListVLHGC*)env->getForge()->allocate(
        scanListsSizeInBytes, MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
    ...
    for (UDATA i = 0; i < listsToCreate; i++) {
        ...
        if (!_cacheScanLists[i].initialize(env)) {
            /* if we failed part-way through the list, adjust the _scanCacheListSize since tearDown will otherwise fail
             * to invoke on the entries in the array which didn't have their constructors called
             */
    ...
    /* Get the estimated cache count required.  The cachesPerThread argument is used to ensure there are at least enough
     * active caches for all working threads (threadCount * cachesPerThread)
     */
    ...
    /* Each thread can have a scan cache and compactGroupCount copy caches. In hierarchical, there could also be a
     * deferred cache. */
    ...
    _reservedRegionList = (MM_ReservedRegionListHeader*)env->getForge()->allocate(
        sizeof(MM_ReservedRegionListHeader) * _compactGroupMaxCount, MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
    ...
    for (UDATA index = 0; index < _compactGroupMaxCount; index++) {
        ...
        for (UDATA sublistIndex = 0; sublistIndex < MM_ReservedRegionListHeader::MAX_SUBLISTS; sublistIndex++) {
            ...
            if (!_reservedRegionList[index]._sublists[sublistIndex]._lock.initialize(
                    env, &_extensions->lnrlOptions, "MM_CopyForwardScheme:_reservedRegionList[]._sublists[]._lock")) {
                return false;
            }
        }
        _reservedRegionList[index]._tailCandidates = NULL;
        _reservedRegionList[index]._tailCandidateCount = 0;
        if (!_reservedRegionList[index]._tailCandidatesLock.initialize(
                env, &_extensions->lnrlOptions, "MM_CopyForwardScheme:_reservedRegionList[]._tailCandidatesLock")) {
            return false;
        }
    }

    /* Set the min/max sizes for copy scan cache allocation when allocating a general purpose area (does not include
     * non-standard sized objects) */
    ...
    _compactGroupBlock = (MM_CopyForwardCompactGroup*)_extensions->getForge()->allocate(
        allocateSize, MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
...
void MM_CopyForwardScheme::tearDown(MM_EnvironmentVLHGC* env)
{
...
MM_AllocationContextTarok* MM_CopyForwardScheme::getPreferredAllocationContext(
    MM_AllocationContextTarok* suggestedContext, J9Object* objectPtr)
{
...
void MM_CopyForwardScheme::raiseAbortFlag(MM_EnvironmentVLHGC* env)
{
    if (!_abortFlag) {
        ...
        if (!_abortFlag) {
            _abortFlag = true;
            didSetFlag = true;
            /* if any threads are waiting, notify them so that they can get out of the monitor since nobody else is
             * going to push work for them */
        ...
        if (didSetFlag) {
            ...
            TRIGGER_J9HOOK_MM_PRIVATE_COPY_FORWARD_ABORT(MM_GCExtensions::getExtensions(env)->privateHookInterface,
                env->getOmrVMThread(), j9time_hires_clock(), J9HOOK_MM_PRIVATE_COPY_FORWARD_ABORT);
        }
    }
}
 ...
 */
void MM_CopyForwardScheme::clearGCStats(MM_EnvironmentVLHGC* env)
{
...
#if defined(J9VM_GC_ARRAYLETS)
void MM_CopyForwardScheme::updateLeafRegions(MM_EnvironmentVLHGC* env)
{
    ...
    while (NULL != (region = regionIterator.nextRegion())) {
        if (region->isArrayletLeaf()) {
            ...
            if (updatedSpineObject != spineObject) {
                MM_HeapRegionDescriptorVLHGC* spineRegion
                    = (MM_HeapRegionDescriptorVLHGC*)_regionManager->tableDescriptorForAddress(spineObject);
                MM_HeapRegionDescriptorVLHGC* updatedSpineRegion
                    = (MM_HeapRegionDescriptorVLHGC*)_regionManager->tableDescriptorForAddress(updatedSpineObject);
...
void MM_CopyForwardScheme::preProcessRegions(MM_EnvironmentVLHGC* env)
{
    GC_HeapRegionIteratorVLHGC regionIterator(_regionManager);
    MM_HeapRegionDescriptorVLHGC* region = NULL;
        ...
        if (region->containsObjects()) {
            ...
            if (region->_markData._shouldMark) {
                ...
                if ((region->_criticalRegionsInUse > 0)
                    || (randomDecideForceNonEvacuatedRegion(_extensions->fvtest_forceCopyForwardHybridRatio))) {
     ...
     * in case partial constructing ownableSynchronizerObject has been moved during previous PGC, notification for new
     * allocation would happen after gc, so it is counted for new allocation, but not in Eden region. loose assertion
     * for this special case
     */
    Assert_MM_true(_extensions->allocationStats._ownableSynchronizerObjectCount >= ownableSynchronizerCountInEden);
    static_cast<MM_CycleStateVLHGC*>(env->_cycleState)
        ->_vlhgcIncrementStats._copyForwardStats._ownableSynchronizerCandidates
        = ownableSynchronizerCandidates;
}

void MM_CopyForwardScheme::postProcessRegions(MM_EnvironmentVLHGC* env)
{
    GC_HeapRegionIteratorVLHGC regionIterator(_regionManager);
    MM_HeapRegionDescriptorVLHGC* region = NULL;
    ...
    while (NULL != (region = regionIterator.nextRegion())) {
        ...
        if (region->_copyForwardData._evacuateSet) {
            if (region->isEden()) {
                static_cast<MM_CycleStateVLHGC*>(env->_cycleState)
                    ->_vlhgcIncrementStats._copyForwardStats._edenEvacuateRegionCount
                    += 1;
            } else {
                static_cast<MM_CycleStateVLHGC*>(env->_cycleState)
                    ->_vlhgcIncrementStats._copyForwardStats._nonEdenEvacuateRegionCount
                    += 1;
            }
        } else if (region->isSurvivorRegion() && !region->isTailFilledSurvivorRegion()) {
            if (region->isEden()) {
                static_cast<MM_CycleStateVLHGC*>(env->_cycleState)
                    ->_vlhgcIncrementStats._copyForwardStats._edenSurvivorRegionCount
                    += 1;
            } else {
                static_cast<MM_CycleStateVLHGC*>(env->_cycleState)
                    ->_vlhgcIncrementStats._copyForwardStats._nonEdenSurvivorRegionCount
                    += 1;
            }
        }

        /* Any region which is part of the survivor set should be set to "shouldMark" to appear as part of the
         * collection set (and be swept, etc) */
    ...
    static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._copyForwardStats._nonEvacuateRegionCount
...
bool MM_CopyForwardScheme::isLiveObject(J9Object* objectPtr)
{
    bool result = true;
...
MMINLINE bool MM_CopyForwardScheme::isObjectInEvacuateMemory(J9Object* objectPtr)
{
    bool result = false;
...
MMINLINE bool MM_CopyForwardScheme::isObjectInEvacuateMemoryNoCheck(J9Object* objectPtr)
{
    bool result = false;
...
MMINLINE bool MM_CopyForwardScheme::isObjectInSurvivorMemory(J9Object* objectPtr)
{
    bool result = false;
    ...
    if (NULL != objectPtr) {
        ...
        Assert_MM_true(region->_copyForwardData._initialLiveSet
            || (!region->_markData._shouldMark && !region->_copyForwardData._initialLiveSet));
        result = region->isSurvivorRegion() && (objectPtr >= region->_copyForwardData._survivorBase);
    }
    return result;
}

MMINLINE bool MM_CopyForwardScheme::isObjectInNurseryMemory(J9Object* objectPtr)
{
    bool result = false;
    ...
    if (NULL != objectPtr) {
        ...
        region = (MM_HeapRegionDescriptorVLHGC*)_regionManager->tableDescriptorForAddress(objectPtr);
        result = region->_markData._shouldMark
            || (region->isSurvivorRegion() && (objectPtr >= region->_copyForwardData._survivorBase));
    }
    return result;
}

MMINLINE void MM_CopyForwardScheme::calculateObjectDetailsForCopy(MM_ScavengerForwardedHeader* forwardedHeader,
    UDATA* objectCopySizeInBytes, UDATA* objectReserveSizeInBytes, bool* doesObjectNeedHash,
    bool* isObjectGrowingHashSlot)
{
    /* NOTE: the size is fetched by hand from the class in the mixed case because a forwarding pointer could have been
     * substituted into the clazz slot. the class pointer passed into this routine is guaranteed to have been checked
     */
    ...
    if (forwardedHeader->isIndexable()) {
        ...
        GC_ArrayletObjectModel::ArrayLayout layout
            = _extensions->indexableObjectModel.getArrayletLayout(clazz, dataSizeInBytes);
        *objectCopySizeInBytes
            = _extensions->indexableObjectModel.getSizeInBytesWithHeader(clazz, layout, numberOfElements);
    ...
    if (hashcodeOffset == *objectCopySizeInBytes) {
        if (forwardedHeader->hasBeenMoved()) {
            *objectCopySizeInBytes += sizeof(UDATA);
            ...
            /* IF the object has been hashed and has not been moved and hashcodeOffset point to end of the object, need
             * to grow size for hash slot  */
...
MMINLINE void MM_CopyForwardScheme::reinitCache(
    MM_EnvironmentVLHGC* env, MM_CopyScanCacheVLHGC* cache, void* base, void* top, UDATA compactGroup)
{
    ...
    /* Count one slot before the base in order to get the true atomic head location.  Regions who do not start on a
     * partial boundary will never see the slot previous.
     */
    if (base == _heapBase) {
        ...
        compactGroupForMarkData->_markMapAtomicHeadSlotIndex
            = _markMap->getSlotIndex((J9Object*)(((UDATA)base) - _markMap->getObjectGrain()));
    }
    ...
    MM_HeapRegionDescriptorVLHGC* region
        = (MM_HeapRegionDescriptorVLHGC*)_regionManager->tableDescriptorForAddress(cache->cacheBase);
    Trc_MM_CopyForwardScheme_reinitCache(env->getLanguageVMThread(),
        _regionManager->mapDescriptorToRegionTableIndex(region), cache,
        region->getAllocationAgeSizeProduct() / (1024 * 1024) / (1024 * 1024),
        (double)((UDATA)cache->cacheAlloc - (UDATA)region->getLowAddress()) / (1024 * 1024));
...
MMINLINE void MM_CopyForwardScheme::reinitArraySplitCache(
    MM_EnvironmentVLHGC* env, MM_CopyScanCacheVLHGC* cache, J9IndexableObject* array, UDATA nextIndex)
{
    ...
    cache->flags = (J9VM_MODRON_SCAVENGER_CACHE_TYPE_SPLIT_ARRAY | J9VM_MODRON_SCAVENGER_CACHE_TYPE_CLEARED)
        | (cache->flags & J9VM_MODRON_SCAVENGER_CACHE_MASK_PERSISTENT);
}

void MM_CopyForwardScheme::clearReservedRegionLists(MM_EnvironmentVLHGC* env)
{
    Trc_MM_CopyForwardScheme_clearReservedRegionLists_Entry(env->getLanguageVMThread(), _compactGroupMaxCount);

    for (UDATA index = 0; index < _compactGroupMaxCount; index++) {
        Trc_MM_CopyForwardScheme_clearReservedRegionLists_compactGroup(env->getLanguageVMThread(), index,
            _reservedRegionList[index]._evacuateRegionCount, _reservedRegionList[index]._sublistCount,
            _reservedRegionList[index]._maxSublistCount, _reservedRegionList[index]._tailCandidateCount);
        ...
        for (UDATA sublistIndex = 0; sublistIndex < _reservedRegionList[index]._sublistCount; sublistIndex++) {
            ...
            if (0 != regionList->_cacheAcquireCount) {
                Trc_MM_CopyForwardScheme_clearReservedRegionLists_sublist(env->getLanguageVMThread(), index,
                    sublistIndex, regionList->_cacheAcquireCount, regionList->_cacheAcquireBytes,
                    regionList->_cacheAcquireBytes / regionList->_cacheAcquireCount);
            }
...
MM_HeapRegionDescriptorVLHGC* MM_CopyForwardScheme::acquireRegion(
    MM_EnvironmentVLHGC* env, MM_ReservedRegionListHeader::Sublist* regionList, UDATA compactGroup)
{
    ...
    if (!_failedToExpand) {
        ...
        MM_AllocationContextTarok* allocationContext
            = (MM_AllocationContextTarok*)_extensions->globalAllocationManager->getAllocationContextByIndex(
        ...
        if (NULL != newRegion) {
            ...
            /* a new region starts as BUMP_ALLOCATED but we will always have valid mark map data for this region so set
             * its type now */
...
void MM_CopyForwardScheme::insertRegionIntoLockedList(
    MM_EnvironmentVLHGC* env, MM_ReservedRegionListHeader::Sublist* regionList, MM_HeapRegionDescriptorVLHGC* newRegion)
{
...
void MM_CopyForwardScheme::releaseRegion(
    MM_EnvironmentVLHGC* env, MM_ReservedRegionListHeader::Sublist* regionList, MM_HeapRegionDescriptorVLHGC* region)
{
    MM_HeapRegionDescriptorVLHGC* next = region->_copyForwardData._nextRegion;
...
void* MM_CopyForwardScheme::reserveMemoryForObject(
    MM_EnvironmentVLHGC* env, UDATA compactGroup, UDATA objectSize, MM_LightweightNonReentrantLock** listLock)
{
    ...
    /* Measure the number of acquires before and after we acquire the lock. If it changed, then there is probably
    ...
    if ((NULL == result) && (NULL != _reservedRegionList[compactGroup]._tailCandidates)) {
        ...
        while ((NULL == result) && (NULL != region)) {
            ...
            /* make sure that we can't be copying objects into the area covered by a card which is meant to describe
             * objects which were already in the region */
    ...
    if ((NULL != result) && (sublistCount < _reservedRegionList[compactGroup]._maxSublistCount)) {
        ...
        if (acceptableAcquireCountForContention < acquireCountAfter) {
            ...
            MM_AtomicOperations::lockCompareExchange(
                &_reservedRegionList[compactGroup]._sublistCount, sublistCount, sublistCount + 1);
        }
    }
...
bool MM_CopyForwardScheme::reserveMemoryForCache(MM_EnvironmentVLHGC* env, UDATA compactGroup, UDATA maxCacheSize,
    void** addrBase, void** addrTop, MM_LightweightNonReentrantLock** listLock)
{
    ...
    while ((!result) && (NULL != region)) {
        ...
        void* tlhTop = NULL;
        result
            = (NULL != memoryPool->collectorAllocateTLH(env, &allocDescription, maxCacheSize, tlhBase, tlhTop, false));
    ...
    if ((!result) && (NULL != _reservedRegionList[compactGroup]._tailCandidates)) {
        ...
        if (NULL != region) {
            ...
            void* tlhTop = NULL;
            result = (NULL
                != memoryPool->collectorAllocateTLH(env, &allocDescription, maxCacheSize, tlhBase, tlhTop, false));
    ...
    if (!result) {
        ...
        if (NULL != region) {
            ...
            void* tlhBase = NULL;
            void* tlhTop = NULL;
            /* note that we called alignAllocationPointer on this pool when adding it to our copy-forward destination
             * list so this address won't share a card with non-moving objects */
            result = (NULL
                != memoryPool->collectorAllocateTLH(env, &allocDescription, maxCacheSize, tlhBase, tlhTop, false));
...
MM_CopyScanCacheVLHGC* MM_CopyForwardScheme::createScanCacheForOverflowInHeap(MM_EnvironmentVLHGC* env)
{
    ...
    /* this we are allocating this in a part of the heap which the copy-forward mechanism will have to walk before it
     * finishes, we need to hide this in a hole so add that header size */
    ...
    while ((NULL == result) && (suggestedCompactGroup < _compactGroupMaxCount)) {
        ...
        if (NULL != extentBase) {
            ...
            /* lock the region list for this group and write-back the memory we consumed as free space immediately (this
             * is a rare case so the lock is an acceptable cost to avoid trying to defer the write-back of the free
             * memory size since this case is unusual)
             */
            Assert_MM_true(NULL != listLock);
            MM_HeapRegionDescriptorVLHGC* region
                = (MM_HeapRegionDescriptorVLHGC*)_regionManager->tableDescriptorForAddress(extentBase);
            MM_MemoryPoolBumpPointer* pool = (MM_MemoryPoolBumpPointer*)region->getMemoryPool();
            listLock->acquire();
...
MM_CopyForwardScheme::getDesiredCopyCacheSize(MM_EnvironmentVLHGC* env, UDATA compactGroup)
{
     ...
     * The upper bound on fragmentation is approximately this fraction, with the expected fragmentation about half of
     * the fraction.
     */
    const double allowableFragmentation = 2.0 * _extensions->tarokCopyForwardFragmentationTarget;
    const double bytesCopiedInCompactGroup
        = (double)(env->_copyForwardCompactGroups[compactGroup]._edenStats._copiedBytes
            + env->_copyForwardCompactGroups[compactGroup]._nonEdenStats._copiedBytes);
    ...
    UDATA perThreadSurvivalEstimatedSize = (UDATA)(((double)stats->_measuredLiveBytesBeforeCollectInCollectedSet
                                                       * stats->_historicalSurvivalRate * allowableFragmentation)
        / (double)env->_currentTask->getThreadCount());
...
MM_CopyScanCacheVLHGC* MM_CopyForwardScheme::reserveMemoryForCopy(MM_EnvironmentVLHGC* env, J9Object* objectToEvacuate,
    MM_AllocationContextTarok* reservingContext, UDATA objectReserveSizeInBytes)
{
    void* addrBase = NULL;
    void* addrTop = NULL;
    ...
    MM_HeapRegionDescriptorVLHGC* region
        = (MM_HeapRegionDescriptorVLHGC*)_regionManager->tableDescriptorForAddress(objectToEvacuate);
    ...
    if (NULL != copyCache) {
        ...
        if ((((UDATA)copyCache->cacheTop) - ((UDATA)copyCache->cacheAlloc)) >= minimumRequiredCacheSize) {
            ...
            if (copyCache->isCurrentlyBeingScanned()) {
                /* this cache is already being scanned. The scanning thread will add it to the free list when it's
                 * finished */
                copyCache = NULL;
            } else {
                ...
                if (copyCache->isScanWorkAvailable()) {
                    ...
                    /* we have decided to stop copying into this cache so ensure that we won't try to keep using it as
                     * one (we will allocate a new cache structure if the allocate succeeds) */
    ...
    /* if objectReserveSizeInBytes is larger then the size we failed to allocate in past during this (aborted) collect,
     * do not bother attempting again */
    ...
    if (NULL == copyCache) {
        ...
        if (_extensions->tarokEnableExpensiveAssertions) {
            ...
            Assert_MM_true(NULL
                == MM_HeapMapIterator(
                       _extensions, _markMap, (UDATA*)copyCache->cacheAlloc, (UDATA*)copyCache->cacheTop, false)
                       .nextObject());
        }
    }
...
MMINLINE bool MM_CopyForwardScheme::copyAndForward(
    MM_EnvironmentVLHGC* env, MM_AllocationContextTarok* reservingContext, volatile j9object_t* objectPtrIndirect)
{
    ...
    if ((NULL != objectPtr) && isObjectInEvacuateMemory(objectPtr)) {
        ...
        if (NULL != objectPtr) {
            ...
            *objectPtrIndirect = objectPtr;
            ...
            Assert_GC_true_with_message(env, (UDATA)0x99669966 == forwardHeader.getPreservedClass()->eyecatcher,
                "Invalid class in objectPtr=%p\n", originalObjectPtr);
...
MMINLINE bool MM_CopyForwardScheme::copyAndForward(MM_EnvironmentVLHGC* env,
    MM_AllocationContextTarok* reservingContext, J9Object* objectPtr, GC_SlotObject* slotObject)
{
...
MMINLINE bool MM_CopyForwardScheme::copyAndForward(MM_EnvironmentVLHGC* env,
    MM_AllocationContextTarok* reservingContext, J9Object* objectPtr, volatile j9object_t* slot)
{
    ...
    if (success) {
        ...
        Assert_MM_false(_abortInProgress);
        /* Because there is a caller where the slot could be scanned by multiple threads at once, it is possible on
         * failure that the value of the slot HAS in fact changed (other thread had room to satisfy).  Because of this,
         * we do cannot check if the preserved slot value would be unchanged (unlike other copyAndForward()
         * implementations).
         */
        env->_workStack.push(env, objectPtr);
    }
...
MMINLINE bool MM_CopyForwardScheme::copyAndForwardPointerArray(MM_EnvironmentVLHGC* env,
    MM_AllocationContextTarok* reservingContext, J9IndexableObject* arrayPtr, UDATA startIndex,
    GC_SlotObject* slotObject)
{
    ...
    if (success) {
        ...
        void* element2 = (void*)((startIndex << PACKET_ARRAY_SPLIT_SHIFT) | PACKET_ARRAY_SPLIT_TAG
            | PACKET_ARRAY_SPLIT_CURRENT_UNIT_ONLY_TAG);
...
MMINLINE bool MM_CopyForwardScheme::copyAndForwardObjectClass(
    MM_EnvironmentVLHGC* env, MM_AllocationContextTarok* reservingContext, J9Object* objectPtr)
{
    bool success = true;
    ...
    if (isDynamicClassUnloadingEnabled()) {
        ...
        if (copyAndForward(env, reservingContext, &classObject)) {
            /* we don't need to update anything with the new address of the class object since objectPtr points at the
             * immobile J9Class */
        } else {
 ...
 */
void MM_CopyForwardScheme::masterCleanupForCopyForward(MM_EnvironmentVLHGC* env)
{
    /* make sure that we have dropped any remaining references to any on-heap scan caches which we would have allocated
     * if we hit overflow */
    ...
    Assert_MM_true(static_cast<MM_CycleStateVLHGC*>(env->_cycleState)
                       ->_vlhgcIncrementStats._copyForwardStats._ownableSynchronizerCandidates
        >= static_cast<MM_CycleStateVLHGC*>(env->_cycleState)
               ->_vlhgcIncrementStats._copyForwardStats._ownableSynchronizerSurvived);
}
 ...
 */
void MM_CopyForwardScheme::masterSetupForCopyForward(MM_EnvironmentVLHGC* env)
{
    clearAbortFlag();
    _abortInProgress = false;
    _clearableProcessingStarted = false;
    ...
    /* We don't want to split too aggressively so take the base2 log of our thread count as our current contention
     * trigger. Note that this number could probably be improved upon but log2 "seemed" to make sense for contention
     * measurement and provided a measurable performance benefit in the tests we were running.
     */
 ...
 */
void MM_CopyForwardScheme::workerSetupForCopyForward(MM_EnvironmentVLHGC* env)
{
 ...
 */
void MM_CopyForwardScheme::mergeGCStats(MM_EnvironmentVLHGC* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    ...
    for (UDATA compactGroupNumber = 0; compactGroupNumber < _compactGroupMaxCount; compactGroupNumber++) {
        ...
        localStats->_copyObjectsTotal
            += compactGroup->_edenStats._copiedObjects + compactGroup->_nonEdenStats._copiedObjects;
        localStats->_copyBytesTotal += totalCopiedBytes;
        localStats->_scanObjectsTotal
            += compactGroup->_edenStats._scannedObjects + compactGroup->_nonEdenStats._scannedObjects;
        localStats->_scanBytesTotal
            += compactGroup->_edenStats._scannedBytes + compactGroup->_nonEdenStats._scannedBytes;
        ...
        UDATA copyDiscardBytesEden = (totalCopiedBytes > 0)
            ? (UDATA)((double)compactGroup->_discardedBytes * (double)compactGroup->_edenStats._copiedBytes
                  / (double)totalCopiedBytes)
            : 0;
        ...
        if (0 != totalLiveBytes) {
            MM_AtomicOperations::add(
                &persistentStats[compactGroupNumber]._measuredBytesCopiedFromGroupDuringCopyForward, totalLiveBytes);
        }

        if (0 != totalCopiedBytes) {
            MM_AtomicOperations::add(
                &persistentStats[compactGroupNumber]._measuredBytesCopiedToGroupDuringCopyForward, totalCopiedBytes);
            MM_AtomicOperations::addU64(
                &persistentStats[compactGroupNumber]._measuredAllocationAgeToGroupDuringCopyForward,
                compactGroup->_allocationAge);
        }

        if (0 != (totalCopiedBytes + compactGroup->_discardedBytes)) {
            Trc_MM_CopyForwardScheme_mergeGCStats_efficiency(env->getLanguageVMThread(), compactGroupNumber,
                totalCopiedBytes, compactGroup->_discardedBytes,
                (double)(compactGroup->_discardedBytes) / (double)(totalCopiedBytes + compactGroup->_discardedBytes));
        }
    }
    ...
    static_cast<MM_CycleStateVLHGC*>(env->_cycleState)
        ->_vlhgcIncrementStats._workPacketStats.merge(&env->_workPacketStats);
     ...
     * -Xtgc:parallel */
    Trc_MM_CopyForwardScheme_parallelStats(env->getLanguageVMThread(), (U_32)env->getSlaveID(),
        ...
        (U_32)env->_copyForwardStats._workStallCount, (U_32)env->_copyForwardStats._completeStallCount,
        (U_32)env->_copyForwardStats._syncStallCount, (U_32)env->_copyForwardStats._irrsStallCount,
        env->_copyForwardStats._acquireFreeListCount, env->_copyForwardStats._releaseFreeListCount,
        env->_copyForwardStats._acquireScanListCount, env->_copyForwardStats._releaseScanListCount,
        env->_copyForwardStats._copiedArraysSplit);

    if (env->_copyForwardStats._aborted) {
        Trc_MM_CopyForwardScheme_parallelStatsForAbort(env->getLanguageVMThread(), (U_32)env->getSlaveID(),
            ...
            (U_32)env->_copyForwardStats._markStallCount, (U_32)env->_copyForwardStats._abortStallCount,
            env->_workPacketStats.workPacketsAcquired, env->_workPacketStats.workPacketsReleased,
            env->_workPacketStats.workPacketsExchanged, env->_copyForwardStats._markedArraysSplit);
    }
}

bool MM_CopyForwardScheme::copyForwardCollectionSet(MM_EnvironmentVLHGC* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    ...
    /* stats management */
    static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._copyForwardStats._startTime
        = j9time_hires_clock();
    ...
    static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._copyForwardStats._endTime
        = j9time_hires_clock();
 ...
 */
bool MM_CopyForwardScheme::copyForwardCompletedSuccessfully(MM_EnvironmentVLHGC* env) { return !abortFlagRaised(); }
...
/* getFreeCache makes the assumption that there will be at least 1 entry on the scan list if there are no entries on the
 * free list. This requires that there be at (N * _cachesPerThread) scan cache entries, where N is the number of threads
 * (Master + workers)
 */
MM_CopyScanCacheVLHGC* MM_CopyForwardScheme::getFreeCache(MM_EnvironmentVLHGC* env)
{
#if defined(J9MODRON_TGC_PARALLEL_STATISTICS)
    ...
    if (NULL == cache) {
        /* we couldn't get a free cache so we must be in an overflow scenario.  Try creating new cache structures on the
         * heap */
...
void MM_CopyForwardScheme::addCacheEntryToFreeCacheList(MM_EnvironmentVLHGC* env, MM_CopyScanCacheVLHGC* newCacheEntry)
{
    _cacheFreeList.pushCache(env, newCacheEntry);
}

void MM_CopyForwardScheme::addCacheEntryToScanCacheListAndNotify(
    MM_EnvironmentVLHGC* env, MM_CopyScanCacheVLHGC* newCacheEntry)
{
...
void MM_CopyForwardScheme::flushCache(MM_EnvironmentVLHGC* env, MM_CopyScanCacheVLHGC* cache)
{
...
void MM_CopyForwardScheme::clearCache(MM_EnvironmentVLHGC* env, MM_CopyScanCacheVLHGC* cache)
{
    ...
    TRIGGER_J9HOOK_MM_PRIVATE_CACHE_CLEARED(_extensions->privateHookInterface, env->getOmrVMThread(),
        env->_cycleState->_activeSubSpace, cache->cacheBase, cache->cacheAlloc, cache->cacheTop);

    cache->flags |= J9VM_MODRON_SCAVENGER_CACHE_TYPE_CLEARED;
}

MM_CopyScanCacheVLHGC* MM_CopyForwardScheme::stopCopyingIntoCache(MM_EnvironmentVLHGC* env, UDATA compactGroup)
{
    ...
    if (NULL != copyCache) {
        ...
        MM_HeapRegionDescriptorVLHGC* region
            = (MM_HeapRegionDescriptorVLHGC*)_regionManager->tableDescriptorForAddress(copyCache->cacheBase);

        /* atomically add (age * usedBytes) product from this cache to the regions product */
        double newAllocationAgeSizeProduct
            = region->atomicIncrementAllocationAgeSizeProduct(copyCache->_allocationAgeSizeProduct);
        ...
        Trc_MM_CopyForwardScheme_stopCopyingIntoCache(env->getLanguageVMThread(),
            _regionManager->mapDescriptorToRegionTableIndex(region), copyCache,
            (double)(newAllocationAgeSizeProduct - copyCache->_allocationAgeSizeProduct) / (1024 * 1024)
                / (1024 * 1024),
            (double)((UDATA)copyCache->cacheAlloc - (UDATA)region->getLowAddress()) / (1024 * 1024),
            (double)copyCache->_allocationAgeSizeProduct / (1024 * 1024) / (1024 * 1024),
            (double)copyCache->_objectSize / (1024 * 1024),
            (double)newAllocationAgeSizeProduct / (1024 * 1024) / (1024 * 1024));
...
void MM_CopyForwardScheme::updateProjectedLiveBytesFromCopyScanCache(
    MM_EnvironmentVLHGC* env, MM_CopyScanCacheVLHGC* cache)
{
    MM_HeapRegionDescriptorVLHGC* region
        = (MM_HeapRegionDescriptorVLHGC*)_regionManager->tableDescriptorForAddress(cache->cacheBase);
...
void MM_CopyForwardScheme::discardRemainingCache(MM_EnvironmentVLHGC* env, MM_CopyScanCacheVLHGC* cache,
    MM_LightweightNonReentrantLock* cacheLock, UDATA wastedMemory)
{
    Assert_MM_false(cache->isSplitArray());
    ...
    if ((0 != discardSize) || (0 != wastedMemory)) {
        ...
        if ((0 != discardSize) && (pool->getAllocationPointer() == cache->cacheTop)
            && (potential >= pool->getMinimumFreeEntrySize())) {
            ...
            env->_copyForwardCompactGroups[cache->_compactGroup]._markMapAtomicTailSlotIndex
                = _markMap->getSlotIndex((J9Object*)cache->cacheTop);
        }
...
void MM_CopyForwardScheme::addCopyCachesToFreeList(MM_EnvironmentVLHGC* env)
{
    for (UDATA index = 0; index < _compactGroupMaxCount; index++) {
...
J9Object* MM_CopyForwardScheme::updateForwardedPointer(J9Object* objectPtr)
{
    J9Object* forwardPtr;
...
MMINLINE MM_AllocationContextTarok* MM_CopyForwardScheme::getContextForHeapAddress(void* address)
{
    return ((MM_HeapRegionDescriptorVLHGC*)_regionManager->tableDescriptorForAddress(address))
        ->_allocateData._owningContext;
}

J9Object* MM_CopyForwardScheme::copy(
    MM_EnvironmentVLHGC* env, MM_AllocationContextTarok* reservingContext, MM_ScavengerForwardedHeader* forwardedHeader)
{
    ...
    if (_abortInProgress || noEvacuation) {
        /* Once threads agreed that abort is in progress or the object is in noEvacation region, only mark/push should
         * be happening, no attempts even to allocate/copy */
        ...
        calculateObjectDetailsForCopy(forwardedHeader, &objectCopySizeInBytes, &objectReserveSizeInBytes,
            &doesObjectNeedHash, &isObjectGrowingHashSlot);
        ...
        if (NULL == copyCache) {
            ...
            destinationObjectPtr
                = forwardedHeader->setForwardedObjectGrowing(destinationObjectPtr, isObjectGrowingHashSlot);
            Assert_MM_true(NULL != destinationObjectPtr);
            if (destinationObjectPtr == originalDestinationObjectPtr) {
                ...
                if (_extensions->objectModel.isIndexable(destinationObjectPtr)) {
                    updateInternalLeafPointersAfterCopy(
                        (J9IndexableObject*)destinationObjectPtr, (J9IndexableObject*)forwardedHeader->getObject());
                }
#endif /* J9VM_GC_ARRAYLETS */
                ...
                if (_tracingEnabled) {
                    PORT_ACCESS_FROM_ENVIRONMENT(env);
                    j9tty_printf(PORTLIB, "Cache alloc: %p newAlloc: %p origO: %p copyO: %p\n", copyCache->cacheAlloc,
                        newCacheAlloc, forwardedHeader->getObject(), destinationObjectPtr);
                }
                ...
                MM_HeapRegionDescriptorVLHGC* sourceRegion
                    = (MM_HeapRegionDescriptorVLHGC*)_regionManager->tableDescriptorForAddress(object);
                ...
                if (sourceRegion->isEden()) {
                    ...
                    env->_copyForwardCompactGroups[destinationCompactGroup]._edenStats._copiedBytes
                        += objectCopySizeInBytes;
                } else {
                    env->_copyForwardCompactGroups[sourceCompactGroup]._nonEdenStats._liveObjects += 1;
                    env->_copyForwardCompactGroups[sourceCompactGroup]._nonEdenStats._liveBytes
                        += objectCopySizeInBytes;
                    env->_copyForwardCompactGroups[destinationCompactGroup]._nonEdenStats._copiedObjects += 1;
                    env->_copyForwardCompactGroups[destinationCompactGroup]._nonEdenStats._copiedBytes
                        += objectCopySizeInBytes;
                }
                copyCache->_allocationAgeSizeProduct
                    += ((double)objectReserveSizeInBytes * (double)sourceRegion->getAllocationAge());
...
#if defined(J9VM_GC_LEAF_BITS)
void MM_CopyForwardScheme::copyLeafChildren(
    MM_EnvironmentVLHGC* env, MM_AllocationContextTarok* reservingContext, J9Object* objectPtr)
{
    ...
    if (GC_ObjectModel::SCAN_MIXED_OBJECT == _extensions->objectModel.getScanType(clazz)) {
        ...
        /* For now we only support leaf children in small objects. If the leaf description isn't immediate, ignore it to
 ...
 */
void MM_CopyForwardScheme::updateInternalLeafPointersAfterCopy(
    J9IndexableObject* destinationPtr, J9IndexableObject* sourcePtr)
{
    if (_extensions->indexableObjectModel.hasArrayletLeafPointers(destinationPtr)) {
        ...
        UDATA sourceEndAddress
            = sourceStartAddress + _extensions->indexableObjectModel.getSizeInBytesWithHeader(destinationPtr);
        ...
        while (NULL != (leafSlotObject = leafIterator.nextLeafPointer())) {
            ...
            if ((sourceStartAddress < leafAddress) && (leafAddress < sourceEndAddress)) {
                leafSlotObject->writeReferenceToSlot(
                    (J9Object*)((UDATA)destinationPtr + (leafAddress - sourceStartAddress)));
            }
        }
    }
}
...
void MM_CopyForwardScheme::flushCacheMarkMap(MM_EnvironmentVLHGC* env, MM_CopyScanCacheVLHGC* cache)
{
    ...
    Assert_MM_false(UDATA_MAX == compactGroup->_markMapPGCSlotIndex); /* Safety check from flushing to see if somehow
                                                                         the cache is being resurrected */
    Assert_MM_false(UDATA_MAX == compactGroup->_markMapGMPSlotIndex); /* Safety check from flushing to see if somehow
                                                                         the cache is being resurrected */
    Assert_MM_false(cache->isSplitArray());

    if (0 != compactGroup->_markMapPGCBitMask) {
        UDATA pgcFlushSlotIndex = compactGroup->_markMapPGCSlotIndex;
        if ((pgcFlushSlotIndex == compactGroup->_markMapAtomicHeadSlotIndex)
            || (pgcFlushSlotIndex == compactGroup->_markMapAtomicTailSlotIndex)) {
    ...
    if (NULL != env->_cycleState->_externalCycleState) {
        if (0 != compactGroup->_markMapGMPBitMask) {
            ...
            if ((gmpFlushSlotIndex == compactGroup->_markMapAtomicHeadSlotIndex)
                || (gmpFlushSlotIndex == compactGroup->_markMapAtomicTailSlotIndex)) {
                env->_cycleState->_externalCycleState->_markMap->atomicSetSlot(
                    gmpFlushSlotIndex, compactGroup->_markMapGMPBitMask);
            } else {
                env->_cycleState->_externalCycleState->_markMap->setSlot(
                    gmpFlushSlotIndex, compactGroup->_markMapGMPBitMask);
            }
...
void MM_CopyForwardScheme::updateMarkMapCache(MM_EnvironmentVLHGC* env, MM_MarkMap* markMap, J9Object* object,
...
void MM_CopyForwardScheme::updateMarkMapAndCardTableOnCopy(
    MM_EnvironmentVLHGC* env, J9Object* srcObject, J9Object* dstObject, MM_CopyScanCacheVLHGC* dstCache)
{
    ...
    Assert_MM_false(UDATA_MAX == destinationGroup->_markMapPGCSlotIndex); /* Safety check from flushing to see if
                                                                             somehow the cache is being resurrected */
    Assert_MM_false(UDATA_MAX == destinationGroup->_markMapGMPSlotIndex); /* Safety check from flushing to see if
                                                                             somehow the cache is being resurrected */
    Assert_MM_false(dstCache->isSplitArray());

    updateMarkMapCache(env, _markMap, dstObject, &destinationGroup->_markMapPGCSlotIndex,
        &destinationGroup->_markMapPGCBitMask, destinationGroup->_markMapAtomicHeadSlotIndex,
        destinationGroup->_markMapAtomicTailSlotIndex);
    ...
    if (NULL != env->_cycleState->_externalCycleState) {
        ...
        if (externalMap->isBitSet(srcObject)) {
            ...
            updateMarkMapCache(env, externalMap, dstObject, &destinationGroup->_markMapGMPSlotIndex,
                &destinationGroup->_markMapGMPBitMask, destinationGroup->_markMapAtomicHeadSlotIndex,
                destinationGroup->_markMapAtomicTailSlotIndex);
 ...
 ****************************************
 */
MMINLINE void MM_CopyForwardScheme::scanOwnableSynchronizerObjectSlots(
    MM_EnvironmentVLHGC* env, MM_AllocationContextTarok* reservingContext, J9Object* objectPtr, ScanReason reason)
{
...
void MM_CopyForwardScheme::scanMixedObjectSlots(
    MM_EnvironmentVLHGC* env, MM_AllocationContextTarok* reservingContext, J9Object* objectPtr, ScanReason reason)
{
    if (_tracingEnabled) {
        PORT_ACCESS_FROM_ENVIRONMENT(env);
...
void MM_CopyForwardScheme::scanReferenceObjectSlots(
    MM_EnvironmentVLHGC* env, MM_AllocationContextTarok* reservingContext, J9Object* objectPtr, ScanReason reason)
{
    bool success = copyAndForwardObjectClass(env, reservingContext, objectPtr);
    ...
    bool isReferenceCleared = (GC_ObjectModel::REF_STATE_CLEARED == referenceState)
        || (GC_ObjectModel::REF_STATE_ENQUEUED == referenceState);
        ...
        case J9_JAVA_CLASS_REFERENCE_SOFT:
            ...
            referentMustBeMarked = referentMustBeMarked
                || (((0 == (referenceObjectOptions & MM_CycleState::references_soft_as_weak))
                       && ((UDATA)J9GC_J9VMJAVALANGSOFTREFERENCE_AGE(env, objectPtr)
...
UDATA
MM_CopyForwardScheme::createNextSplitArrayWorkUnit(
        ...
        if (slotsToScan > _arraySplitSize) {
            ...
            if (_abortInProgress || noEvacuation) {
                if (!currentSplitUnitOnly) {
                    /* work stack driven */
                    env->_workStack.push(env, (void*)arrayPtr,
                        (void*)((nextIndex << PACKET_ARRAY_SPLIT_SHIFT) | PACKET_ARRAY_SPLIT_TAG));
                ...
                if (NULL != splitCache) {
                    ...
                    env->_copyForwardStats._markedArraysSplit += 1;
#endif /* J9MODRON_TGC_PARALLEL_STATISTICS */
                    Trc_MM_CopyForwardScheme_scanPointerArrayObjectSlotsSplit_failedToAllocateCache(
                        env->getLanguageVMThread(), sizeInElements);
                }
            }
        }
    }
...
UDATA
MM_CopyForwardScheme::scanPointerArrayObjectSlotsSplit(MM_EnvironmentVLHGC* env,
    MM_AllocationContextTarok* reservingContext, J9IndexableObject* arrayPtr, UDATA startIndex,
    bool currentSplitUnitOnly)
{
    if (_tracingEnabled) {
        PORT_ACCESS_FROM_ENVIRONMENT(env);
    ...
    if (slotsToScan > 0) {
        /* TODO: this iterator scans the array backwards - change it to forward, and optimize it since we can guarantee
         * the range will be contiguous */
...
void MM_CopyForwardScheme::scanClassObjectSlots(
    MM_EnvironmentVLHGC* env, MM_AllocationContextTarok* reservingContext, J9Object* classObject, ScanReason reason)
{
    ...
    if (NULL != classPtr) {
        ...
        do {
            ...
            /*
             * Usually we don't care about class to class references because its can be marked as a part of alive
             * classloader or find in Hash Table However we need to scan them for case of Anonymous classes. Its are
             * unloaded on individual basis so it is important to reach each one
             */
            ...
            if (success) {
                 ...
                 */
                /* By scanning the class object, we've committed to it either being in a card external to the collection
                 * set, or that it is already part of a copied set and being scanned through the copy/scan cache.  In
                 * either case, a simple pointer forward update is all that is required.
                 */
...
void MM_CopyForwardScheme::scanClassLoaderObjectSlots(MM_EnvironmentVLHGC* env,
    MM_AllocationContextTarok* reservingContext, J9Object* classLoaderObject, ScanReason reason)
{
    scanMixedObjectSlots(env, reservingContext, classLoaderObject, reason);

    J9ClassLoader* classLoader
        = J9VMJAVALANGCLASSLOADER_VMREF((J9VMThread*)env->getLanguageVMThread(), classLoaderObject);
    if (NULL != classLoader) {
        /* By scanning the class loader object, we've committed to it either being in a card external to the collection
         * set, or that it is already part of a copied set and being scanned through the copy/scan cache.  In either
         * case, a simple pointer forward update is all that is required.
         */
                ...
                if (success) {
                    if (NULL != module->moduleName) {
                        success = copyAndForward(
                            env, reservingContext, classLoaderObject, (J9Object**)&(module->moduleName));
                    }
                }
                if (success) {
                    if (NULL != module->version) {
                        success
                            = copyAndForward(env, reservingContext, classLoaderObject, (J9Object**)&(module->version));
                    }
                }
 ...
 ****************************************
 */
bool MM_CopyForwardScheme::isScanCacheWorkAvailable(MM_CopyScanCacheListVLHGC* scanCacheList)
{
    return !scanCacheList->isEmpty();
}

bool MM_CopyForwardScheme::isAnyScanCacheWorkAvailable()
{
    bool result = false;
...
bool MM_CopyForwardScheme::isAnyScanWorkAvailable(MM_EnvironmentVLHGC* env)
{
    return (isAnyScanCacheWorkAvailable()
        || ((0 != _regionCountCannotBeEvacuated) && !_abortInProgress && !abortFlagRaised()
               && env->_workStack.inputPacketAvailableFromWorkPackets(env)));
}

MM_CopyScanCacheVLHGC* MM_CopyForwardScheme::getSurvivorCacheForScan(MM_EnvironmentVLHGC* env)
{
...
MM_CopyForwardScheme::ScanReason MM_CopyForwardScheme::getNextWorkUnit(
    MM_EnvironmentVLHGC* env, UDATA preferredNumaNode)
{
    env->_scanCache = NULL;
...
MM_CopyForwardScheme::ScanReason MM_CopyForwardScheme::getNextWorkUnitOnNode(MM_EnvironmentVLHGC* env, UDATA numaNode)
{
    ScanReason ret = SCAN_REASON_NONE;
...
MM_CopyForwardScheme::ScanReason MM_CopyForwardScheme::getNextWorkUnitNoWait(
    MM_EnvironmentVLHGC* env, UDATA preferredNumaNode)
{
    UDATA nodeLists = _scanCacheListSize;
    ScanReason ret = SCAN_REASON_NONE;
 ...
 */
MMINLINE UDATA MM_CopyForwardScheme::scanToCopyDistance(MM_CopyScanCacheVLHGC* cache)
{
    if (cache == NULL) {
 ...
 */
MMINLINE bool MM_CopyForwardScheme::bestCacheForScanning(
    MM_CopyScanCacheVLHGC* copyCache, MM_CopyScanCacheVLHGC** scanCache)
{
    if (!copyCache->isScanWorkAvailable()) {
        return false;
    }
...
MMINLINE bool MM_CopyForwardScheme::aliasToCopyCache(MM_EnvironmentVLHGC* env, MM_CopyScanCacheVLHGC** nextScanCache)
{
...
MMINLINE void MM_CopyForwardScheme::scanObject(
    MM_EnvironmentVLHGC* env, MM_AllocationContextTarok* reservingContext, J9Object* objectPtr, ScanReason reason)
{
    J9Class* clazz = J9GC_J9OBJECT_CLAZZ(objectPtr);
...
MMINLINE void MM_CopyForwardScheme::updateScanStats(MM_EnvironmentVLHGC* env, J9Object* objectPtr, ScanReason reason)
{
    bool noEvacuation = false;
    if (0 != _regionCountCannotBeEvacuated) {
...
void MM_CopyForwardScheme::scanPointerArrayObjectSlots(MM_EnvironmentVLHGC* env,
    MM_AllocationContextTarok* reservingContext, J9IndexableObject* arrayPtr, ScanReason reason)
{
    UDATA index = 0;
    ...
    if (_abortInProgress || noEvacuation) {
        ...
        if ((PACKET_ARRAY_SPLIT_TAG == (peekValue & PACKET_ARRAY_SPLIT_TAG))) {
            ...
            currentSplitUnitOnly = ((
                PACKET_ARRAY_SPLIT_CURRENT_UNIT_ONLY_TAG == (peekValue & PACKET_ARRAY_SPLIT_CURRENT_UNIT_ONLY_TAG)));
        }
    } else {
 ...
 */
void MM_CopyForwardScheme::completeScanCache(MM_EnvironmentVLHGC* env)
{
    ...
    if (scanCache->isSplitArray()) {
        ...
        /* we want to perform a NUMA-aware analogue to "hierarchical scanning" so this scan cache should pull other
         * objects into its node */
        MM_AllocationContextTarok* reservingContext = getContextForHeapAddress(scanCache->scanCurrent);
        do {
            GC_ObjectHeapIteratorAddressOrderedList heapChunkIterator(
                _extensions, (J9Object*)scanCache->scanCurrent, (J9Object*)scanCache->cacheAlloc, false);
...
MMINLINE bool MM_CopyForwardScheme::incrementalScanMixedObjectSlots(MM_EnvironmentVLHGC* env,
    MM_AllocationContextTarok* reservingContext, MM_CopyScanCacheVLHGC* scanCache, J9Object* objectPtr,
...
MMINLINE bool MM_CopyForwardScheme::incrementalScanClassObjectSlots(MM_EnvironmentVLHGC* env,
    MM_AllocationContextTarok* reservingContext, MM_CopyScanCacheVLHGC* scanCache, J9Object* objectPtr,
    bool hasPartiallyScannedObject, MM_CopyScanCacheVLHGC** nextScanCache)
{
    /* NOTE: An incremental scan solution should be provided here.  For now, just use a full scan and ignore any
     * hierarchical needs. */
    scanClassObjectSlots(env, reservingContext, objectPtr);
    return false;
}

MMINLINE bool MM_CopyForwardScheme::incrementalScanClassLoaderObjectSlots(MM_EnvironmentVLHGC* env,
    MM_AllocationContextTarok* reservingContext, MM_CopyScanCacheVLHGC* scanCache, J9Object* objectPtr,
    bool hasPartiallyScannedObject, MM_CopyScanCacheVLHGC** nextScanCache)
{
    /* NOTE: An incremental scan solution should be provided here.  For now, just use a full scan and ignore any
...
MMINLINE bool MM_CopyForwardScheme::incrementalScanPointerArrayObjectSlots(MM_EnvironmentVLHGC* env,
    MM_AllocationContextTarok* reservingContext, MM_CopyScanCacheVLHGC* scanCache, J9Object* objectPtr,
    bool hasPartiallyScannedObject, MM_CopyScanCacheVLHGC** nextScanCache)
{
...
MMINLINE bool MM_CopyForwardScheme::incrementalScanReferenceObjectSlots(MM_EnvironmentVLHGC* env,
    MM_AllocationContextTarok* reservingContext, MM_CopyScanCacheVLHGC* scanCache, J9Object* objectPtr,
    bool hasPartiallyScannedObject, MM_CopyScanCacheVLHGC** nextScanCache)
{
    GC_MixedObjectIterator mixedObjectIterator(_javaVM->omrVM);
    ...
    if (J9_JAVA_CLASS_REFERENCE_SOFT
        == (J9CLASS_FLAGS(J9GC_J9OBJECT_CLAZZ(objectPtr)) & J9_JAVA_CLASS_REFERENCE_MASK)) {
...
void MM_CopyForwardScheme::incrementalScanCacheBySlot(MM_EnvironmentVLHGC* env)
{
    MM_CopyScanCacheVLHGC* scanCache = (MM_CopyScanCacheVLHGC*)env->_scanCache;
    J9Object* objectPtr;
...
nextCache:
    ...
    if (scanCache->isScanWorkAvailable()) {
        /* we want to perform a NUMA-aware analogue to "heirarchical scanning" so this scan cache should pull other
         * objects into its node */
        ...
        do {
            ...
            GC_ObjectHeapIteratorAddressOrderedList heapChunkIterator(
                _extensions, (J9Object*)scanCache->scanCurrent, (J9Object*)cacheAlloc, false);
            ...
            while ((objectPtr = heapChunkIterator.nextObject()) != NULL) {
                ...
                case GC_ObjectModel::SCAN_MIXED_OBJECT:
                case GC_ObjectModel::SCAN_OWNABLESYNCHRONIZER_OBJECT:
                    hasPartiallyScannedObject = incrementalScanMixedObjectSlots(
                        env, reservingContext, scanCache, objectPtr, hasPartiallyScannedObject, &nextScanCache);
                    break;
                case GC_ObjectModel::SCAN_CLASS_OBJECT:
                    hasPartiallyScannedObject = incrementalScanClassObjectSlots(
                        env, reservingContext, scanCache, objectPtr, hasPartiallyScannedObject, &nextScanCache);
                    break;
                case GC_ObjectModel::SCAN_CLASSLOADER_OBJECT:
                    hasPartiallyScannedObject = incrementalScanClassLoaderObjectSlots(
                        env, reservingContext, scanCache, objectPtr, hasPartiallyScannedObject, &nextScanCache);
                    break;
                case GC_ObjectModel::SCAN_POINTER_ARRAY_OBJECT:
                    hasPartiallyScannedObject = incrementalScanPointerArrayObjectSlots(
                        env, reservingContext, scanCache, objectPtr, hasPartiallyScannedObject, &nextScanCache);
                    break;
                case GC_ObjectModel::SCAN_REFERENCE_MIXED_OBJECT:
                    hasPartiallyScannedObject = incrementalScanReferenceObjectSlots(
                        env, reservingContext, scanCache, objectPtr, hasPartiallyScannedObject, &nextScanCache);
                    break;
                case GC_ObjectModel::SCAN_PRIMITIVE_ARRAY_OBJECT:
                    continue;
...
void MM_CopyForwardScheme::cleanRegion(MM_EnvironmentVLHGC* env, MM_HeapRegionDescriptorVLHGC* region, U_8 flagToClean)
{
    ...
    if (flagToClean == (flags & flagToClean)) {
        ...
        /* Force our write of the overflow flags from our cache and ensure that we have no stale mark map data before we
         * walk */
        ...
        MM_HeapMapIterator objectIterator
            = MM_HeapMapIterator(MM_GCExtensions::getExtensions(env), env->_cycleState->_markMap, heapBase, heapTop);
...
bool MM_CopyForwardScheme::isWorkPacketsOverflow(MM_EnvironmentVLHGC* env)
{
...
bool MM_CopyForwardScheme::handleOverflow(MM_EnvironmentVLHGC* env)
{
    MM_WorkPackets* packets = (MM_WorkPackets*)(env->_cycleState->_workPackets);
    bool result = false;
    ...
    if (packets->getOverflowFlag()) {
        result = true;
        if (((MM_CopyForwardSchemeTask*)env->_currentTask)
                ->synchronizeGCThreadsAndReleaseMasterForMark(env, UNIQUE_ID)) {
        ...
        U_8 flagToRemove
            = MM_RegionBasedOverflowVLHGC::overflowFlagForCollectionType(env, env->_cycleState->_collectionType);
...
void MM_CopyForwardScheme::completeScanForAbort(MM_EnvironmentVLHGC* env)
{
...
void MM_CopyForwardScheme::completeScanWorkPacket(MM_EnvironmentVLHGC* env)
{
    MM_AllocationContextTarok* reservingContext = _commonContext;
    J9Object* objectPtr = NULL;
...
void MM_CopyForwardScheme::completeScan(MM_EnvironmentVLHGC* env)
{
    UDATA nodeOfThread = 0;

    /* if we aren't using NUMA, we don't want to check the thread affinity since we will have only one list of scan
     * caches */
...
MMINLINE void MM_CopyForwardScheme::addOwnableSynchronizerObjectInList(MM_EnvironmentVLHGC* env, j9object_t object)
{
...
void MM_CopyForwardScheme::scanUnfinalizedObjects(MM_EnvironmentVLHGC* env)
{
                ...
                while (NULL != pointer) {
                    ...
                    if (NULL == forwardedPtr) {
                        if (_markMap->isBitSet(pointer)) {
                            ...
                            if (NULL == forwardedPtr) {
                                /* We failed to copy the object. This must have caused an abort. This will be dealt with
                                 * in scanUnfinalizedObjectsComplete */
                                Assert_MM_false(_abortInProgress);
                                Assert_MM_true(abortFlagRaised());
                                forwardedPtr = pointer;
                            }
...
void MM_CopyForwardScheme::cleanCardTable(MM_EnvironmentVLHGC* env)
{
...
void MM_CopyForwardScheme::cleanCardTableForPartialCollect(MM_EnvironmentVLHGC* env, MM_CardCleaner* cardCleaner)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    ...
    MM_HeapRegionDescriptorVLHGC* region = NULL;
    while (NULL != (region = regionIterator.nextRegion())) {
        /* Don't include survivor regions as we scan - they don't need to be processed and this will throw off the work
         * unit indices */
        if (region->containsObjects() && region->_copyForwardData._initialLiveSet) {
            if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
                if (!region->_markData._shouldMark) {
                    ...
                    /* this region is part of the collection set, so just change its dirty cards to clean (or
                     * GMP_MUST_SCAN) */
...
void MM_CopyForwardScheme::updateOrDeleteObjectsFromExternalCycle(MM_EnvironmentVLHGC* env)
{
    ...
    while (NULL != (region = regionIterator.nextRegion())) {
        if (region->_markData._shouldMark) {
            if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
                ...
                if (abortFlagRaised() || region->_markData._noEvacuation) {
                    /* Walk the mark map range for the region and fixing mark bits to be the subset of the current mark
                     * map. (Those bits that are cleared have been moved and their bits are already set).
                     */
    ...
    /* Clear or update references on external cycle work packets, depending on whether the reference has been forwarded
     * or not */
    UDATA totalCount = 0;
    ...
    Trc_MM_CopyForwardScheme_deleteDeadObjectsFromExternalCycle(
        env->getLanguageVMThread(), totalCount, deletedCount, preservedCount);
}

bool MM_CopyForwardScheme::scanObjectsInRange(
    MM_EnvironmentVLHGC* env, void* lowAddress, void* highAddress, bool rememberedObjectsOnly)
{
    ...
    /* card cleaning is done after stack processing so any objects we copy should be copied into the node which refers
     * to them, even from cards */
...
private:
    MM_CopyForwardScheme*
        _copyForwardScheme; /**< Local reference back to the copy forward scheme driving the collection */
...
private:
    ...
    virtual void doStackSlot(J9Object** slotPtr, void* walkState, const void* stackLocation)
    {
        if (_copyForwardScheme->isHeapObject(*slotPtr)) {
            ...
            Assert_MM_validStackSlot(
                MM_StackSlotValidator(MM_StackSlotValidator::COULD_BE_FORWARDED, *slotPtr, stackLocation, walkState)
                    .validate(_env));
            /* we know that threads are bound to nodes so relocalize this object into the node of the thread which
             * directly references it */
            J9VMThread* thread = ((J9StackWalkState*)walkState)->currentThread;
            MM_AllocationContextTarok* reservingContext
                = (MM_AllocationContextTarok*)MM_EnvironmentVLHGC::getEnvironment(thread)->getAllocationContext();
            _copyForwardScheme->copyAndForward(MM_EnvironmentVLHGC::getEnvironment(_env), reservingContext, slotPtr);
        } else if (NULL != *slotPtr) {
            /* stack object - just validate */
            Assert_MM_validStackSlot(
                MM_StackSlotValidator(MM_StackSlotValidator::NOT_ON_HEAP, *slotPtr, stackLocation, walkState)
                    .validate(_env));
        }
    }
    ...
    {
        /* Should never try to scan J9Class structures - these are handled by j.l.c and class loader references on the
         * heap */
        Assert_MM_unreachable();
    }
    ...
    virtual void doClassLoader(J9ClassLoader* classLoader)
    {
        if (0 == (classLoader->gcFlags & J9_GC_CLASS_LOADER_DEAD)) {
             ...
             */
            MM_AllocationContextTarok* reservingContext
                = _copyForwardScheme->getContextForHeapAddress(classLoader->classLoaderObject);
            _copyForwardScheme->copyAndForward(
                MM_EnvironmentVLHGC::getEnvironment(_env), reservingContext, &classLoader->classLoaderObject);
        }
    }

#if defined(J9VM_GC_FINALIZATION)
    virtual void doFinalizableObject(j9object_t object) { Assert_MM_unreachable(); }
    ...
    virtual void scanFinalizableObjects(MM_EnvironmentBase* env)
    {
        ...
        if (_copyForwardScheme->_shouldScanFinalizableObjects) {
            ...
            Assert_MM_true(
                !MM_GCExtensions::getExtensions(env)->finalizeListManager->isFinalizableObjectProcessingRequired());
        }
...
public:
    ...
    {
        /* threads and their stacks tell us more about NUMA affinity than anything else so ensure that we scan them
         * first and process all scan caches that they produce before proceeding */
        scanThreads(env);
...
private:
    ...
    virtual CompletePhaseCode scanUnfinalizedObjectsComplete(MM_EnvironmentBase* env)
    {
        ...
        if (!wasAbortAlreadyInProgress && _copyForwardScheme->_abortInProgress) {
            /* an abort occurred during unfinalized processing: there could be unscanned or unforwarded objects on the
             * finalizable list */
            if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
                /* since we know we're in abort handling mode and won't be copying any of these objects we don't need to
                 * synchronize here */
                _copyForwardScheme->scanFinalizableObjects(MM_EnvironmentVLHGC::getEnvironment(env));
            }
    ...
    virtual CompletePhaseCode scanPhantomReferencesComplete(MM_EnvironmentBase* envBase)
    {
        ...
        env->_currentTask->synchronizeGCThreads(env, UNIQUE_ID);
        Assert_MM_true(MM_CycleState::references_clear_phantom
    ...
    virtual void doMonitorReference(J9ObjectMonitor* objectMonitor, GC_HashTableIterator* monitorReferenceIterator)
    {
        ...
        if (!_copyForwardScheme->isLiveObject(objectPtr)) {
            ...
            if (NULL != forwardPtr) {
                 ...
                 */
                static_cast<J9JavaVM*>(_omrVM->_language_vm)
                    ->internalVMFunctions->objectMonitorDestroy(static_cast<J9JavaVM*>(_omrVM->_language_vm),
                        (J9VMThread*)_env->getLanguageVMThread(), (omrthread_monitor_t)monitor);
            }
        }
    }
    ...
    virtual CompletePhaseCode scanMonitorReferencesComplete(MM_EnvironmentBase* envBase)
    {
        ...
        ((J9JavaVM*)env->getLanguageVM())
            ->internalVMFunctions->objectMonitorDestroyComplete(
                (J9JavaVM*)env->getLanguageVM(), (J9VMThread*)env->getLanguageVMThread());
...
void MM_CopyForwardScheme::clearMarkMapForPartialCollect(MM_EnvironmentVLHGC* env)
{
    Assert_MM_true(MM_CycleState::CT_PARTIAL_GARBAGE_COLLECTION == env->_cycleState->_collectionType);
...
void MM_CopyForwardScheme::clearCardTableForPartialCollect(MM_EnvironmentVLHGC* env)
{
    Assert_MM_true(MM_CycleState::CT_PARTIAL_GARBAGE_COLLECTION == env->_cycleState->_collectionType);
    bool gmpIsRunning = (NULL != env->_cycleState->_externalCycleState);
...
{
    /* make sure that we can't be copying objects into the area covered by a card which is meant to describe objects
     * which were already in the region */
...
void MM_CopyForwardScheme::workThreadGarbageCollect(MM_EnvironmentVLHGC* env)
{
    ...
    /* pre-populate the _reservedRegionList with the flushed regions from every context (required to bootstrap
     * tail-filling) */
    ...
    if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
        ...
        /* initialize the maximum number of sublists for each compact group; ensure that we try to produce fewer
         * survivor regions than evacuate regions */
        for (UDATA index = 0; index < _compactGroupMaxCount; index++) {
            UDATA evacuateCount = _reservedRegionList[index]._evacuateRegionCount;
            /* Arbitrarily set the max to half the evacuate count. This means that, if it's possible, we'll use no more
             * than half as many survivor regions as there were evacuate regions */
    ...
    if (abortFlagRaised()) {
        ...
        /* rescan to fix up root slots, but also to complete scanning of roots that we miss to mark/push in original
         * root scanning */
        scanRoots(env);
    ...
    if (env->_currentTask->synchronizeGCThreadsAndReleaseSingleThread(env, UNIQUE_ID)) {
        ...
        /* Soft and weak references resurrected by finalization need to be cleared immediately since weak and soft
         * processing has already completed. This has to be set before unfinalizable (and phantom) processing, because
         * it can copy object to a tail filled region, in which case we do not want to put GMP refs to REMEMBERED state
         * (we want have a chance to put it back to INITIAL state).
         */
        ...
        /* since we need a sync point here anyway, use this opportunity to determine which regions contain weak and soft
         * references or unfinalized objects */
...
void MM_CopyForwardScheme::scanRoots(MM_EnvironmentVLHGC* env)
{
...
#if defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING)
    if (isDynamicClassUnloadingEnabled()) {
        /* A single thread processes all class loaders, marking any loader which has instances outside of the collection
         * set. */
        if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
                    ...
                    if (J9_ARE_ANY_BITS_SET(classLoader->flags, J9CLASSLOADER_ANON_CLASS_LOADER)) {
                        ...
                        while (NULL != (segment = segmentIterator.nextSegment())) {
                            ...
                            while (NULL != (clazz = classHeapIterator.nextClass())) {
                                if (classLoaderRememberedSet->isClassRemembered(env, clazz)) {
                                    MM_AllocationContextTarok* reservingContext
                                        = getContextForHeapAddress(clazz->classObject);
                        ...
                        if (classLoaderRememberedSet->isRemembered(env, classLoader)) {
                            foundSystemClassLoader
                                = foundSystemClassLoader || (classLoader == _javaVM->systemClassLoader);
                            foundApplicationClassLoader
                                = foundApplicationClassLoader || (classLoader == _javaVM->applicationClassLoader);
                            if (NULL != classLoader->classLoaderObject) {
                                /* until we decide if class loaders should be common, just relocate this object back
                                 * into its existing node */
                                MM_AllocationContextTarok* reservingContext
                                    = getContextForHeapAddress(classLoader->classLoaderObject);
                                copyAndForward(env, reservingContext, &classLoader->classLoaderObject);
                            } else {
                                /* Only system/app classloaders can have a null classloader object (only during early
                                 * bootstrap) */
                                Assert_MM_true((classLoader == _javaVM->systemClassLoader)
                                    || (classLoader == _javaVM->applicationClassLoader));

                                /* We will never find the object for this class loader during scanning, so scan its
                                 * class table immediately */
                                GC_ClassLoaderClassesIterator iterator(_extensions, classLoader);
                                J9Class* clazz = NULL;
                                bool success = true;
                                ...
                                while (success && (NULL != (clazz = iterator.nextClass()))) {
                                    Assert_MM_true(NULL != clazz->classObject);
                                    MM_AllocationContextTarok* clazzContext
                                        = getContextForHeapAddress(clazz->classObject);
                                ...
                                J9HashTableState walkState;
                                J9Module** modulePtr
                                    = (J9Module**)hashTableStartDo(classLoader->moduleHashTable, &walkState);
                                while (success && (NULL != modulePtr)) {
                                    J9Module* const module = *modulePtr;
                                    success = copyAndForward(env, getContextForHeapAddress(module->moduleObject),
                                    ...
                                    if (success) {
                                        if (NULL != module->version) {
                                            success = copyAndForward(env, getContextForHeapAddress(module->version),
                                                (J9Object**)&(module->version));
                                        }
                                    }
                                    modulePtr = (J9Module**)hashTableNextDo(&walkState);
                                }
...
void MM_CopyForwardScheme::verifyDumpObjectDetails(MM_EnvironmentVLHGC* env, const char* title, J9Object* object)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    ...
    if (NULL != object) {
        ...
        j9tty_printf(PORTLIB, "\tregion:%p base:%p top:%p regionProperties:%u\n", region, region->getLowAddress(),
            region->getHighAddress(), region->getRegionProperties());

        j9tty_printf(PORTLIB,
            "\t\tbitSet:%c externalBitSet:%c shouldMark:%c initialLiveSet:%c survivorSet:%c survivorBase:%p age:%zu\n",
            _markMap->isBitSet(object) ? 'Y' : 'N',
            (NULL == env->_cycleState->_externalCycleState)
                ? 'N'
                : (env->_cycleState->_externalCycleState->_markMap->isBitSet(object) ? 'Y' : 'N'),
            region->_markData._shouldMark ? 'Y' : 'N', region->_copyForwardData._initialLiveSet ? 'Y' : 'N',
            region->isSurvivorRegion() ? 'Y' : 'N', region->_copyForwardData._survivorBase, region->getLogicalAge());
    }
}
...
private:
    void verifyObject(J9Object** slotPtr)
    {
        ...
        J9Object* objectPtr = *slotPtr;
        if (!_copyForwardScheme->_abortInProgress && !_copyForwardScheme->isObjectInNoEvacuationRegions(env, objectPtr)
            && _copyForwardScheme->verifyIsPointerInEvacute(env, objectPtr)) {
            PORT_ACCESS_FROM_ENVIRONMENT(env);
    ...
    virtual void doSlot(J9Object** slotPtr) { verifyObject(slotPtr); }
    ...
    virtual void doClass(J9Class* clazz)
    {
        ...
        if (NULL != classObject) {
            if (_copyForwardScheme->isDynamicClassUnloadingEnabled()
                && !_copyForwardScheme->isLiveObject(classObject)) {
    ...
    virtual void doClassLoader(J9ClassLoader* classLoader)
    {
        ...
        if (NULL != classLoaderObject) {
            if (_copyForwardScheme->isDynamicClassUnloadingEnabled()
                && !_copyForwardScheme->isLiveObject(classLoaderObject)) {
    ...
    virtual void doUnfinalizedObject(J9Object* objectPtr, MM_UnfinalizedObjectList* list)
    {
        ...
        if (!_copyForwardScheme->_abortInProgress && !_copyForwardScheme->isObjectInNoEvacuationRegions(env, objectPtr)
            ...
            PORT_ACCESS_FROM_ENVIRONMENT(env);
            j9tty_printf(
                PORTLIB, "Unfinalized object list points into evacuate!  list %p object %p\n", list, objectPtr);
            Assert_MM_unreachable();
        }
    }
#endif /* J9VM_GC_FINALIZATION */
    ...
    virtual void doOwnableSynchronizerObject(J9Object* objectPtr, MM_OwnableSynchronizerObjectList* list)
    {
        ...
        if (!_copyForwardScheme->_abortInProgress && !_copyForwardScheme->isObjectInNoEvacuationRegions(env, objectPtr)
            ...
            PORT_ACCESS_FROM_ENVIRONMENT(env);
            j9tty_printf(
                PORTLIB, "OwnableSynchronizer object list points into evacuate!  list %p object %p\n", list, objectPtr);
            Assert_MM_unreachable();
        }
    }
...
void MM_CopyForwardScheme::verifyCopyForwardResult(MM_EnvironmentVLHGC* env)
{
    ...
    while (NULL != (region = regionIterator.nextRegion())) {
        if (region->isArrayletLeaf()) {
            ...
            /* otherwise, it must not be forwarded (since that would imply that the spine survived but the pointer
             * wasn't updated) */
            if (!_markMap->isBitSet(spineObject)) {
                ...
                if (forwardedSpine.isForwardedPointer()) {
                    PORT_ACCESS_FROM_ENVIRONMENT(env);
                    j9tty_printf(PORTLIB,
                        "Spine pointer is not marked and is forwarded (leaf region's pointer to spine not updated)!  "
                        "Region %p Spine %p (should be %p)\n",
            ...
            if (region->containsObjects()) {
                if (region->isSurvivorRegion()) {
                    ...
                    void* endOfAllocatedObjects
                        = ((MM_MemoryPoolBumpPointer*)region->getMemoryPool())->getAllocationPointer();
                    MM_HeapMapIterator mapIterator(_extensions, _markMap,
                        (UDATA*)region->_copyForwardData._survivorBase, (UDATA*)endOfAllocatedObjects, false);
                    GC_ObjectHeapIteratorAddressOrderedList heapChunkIterator(_extensions,
                        (J9Object*)region->_copyForwardData._survivorBase, (J9Object*)endOfAllocatedObjects, false);
                    J9Object* objectPtr = NULL;
                    ...
                    while (NULL != (objectPtr = heapChunkIterator.nextObject())) {
                        ...
                        if (objectPtr != mapObjectPtr) {
                            PORT_ACCESS_FROM_ENVIRONMENT(env);
                            j9tty_printf(PORTLIB,
                                "ChunkIterator and mapIterator did not match up during walk of survivor space! "
                                "ChunkSlot %p MapSlot %p\n",
                ...
                if (region->_copyForwardData._initialLiveSet) {
                    ...
                    MM_HeapMapIterator iterator(
                        _extensions, _markMap, (UDATA*)region->getLowAddress(), highAddress, false);
                    J9Object* objectPtr = NULL;
...
void MM_CopyForwardScheme::verifyObject(MM_EnvironmentVLHGC* env, J9Object* objectPtr)
{
    J9Class* clazz = J9GC_J9OBJECT_CLAZZ(objectPtr);
    Assert_MM_mustBeClass(clazz);
    switch (_extensions->objectModel.getScanType(clazz)) {
...
void MM_CopyForwardScheme::verifyMixedObjectSlots(MM_EnvironmentVLHGC* env, J9Object* objectPtr)
{
    GC_MixedObjectIterator mixedObjectIterator(_javaVM->omrVM, objectPtr);
    GC_SlotObject* slotObject = NULL;
    ...
    while (NULL != (slotObject = mixedObjectIterator.nextSlot())) {
        ...
        if (!_abortInProgress && !isObjectInNoEvacuationRegions(env, dstObject)
            && verifyIsPointerInEvacute(env, dstObject)) {
            PORT_ACCESS_FROM_ENVIRONMENT(env);
            j9tty_printf(PORTLIB, "Mixed object slot points to evacuate!  srcObj %p slot %p dstObj %p\n", objectPtr,
                slotObject->readAddressFromSlot(), dstObject);
...
void MM_CopyForwardScheme::verifyReferenceObjectSlots(MM_EnvironmentVLHGC* env, J9Object* objectPtr)
{
    ...
    if (!_abortInProgress && !isObjectInNoEvacuationRegions(env, referentPtr)
        && verifyIsPointerInEvacute(env, referentPtr)) {
        PORT_ACCESS_FROM_ENVIRONMENT(env);
        j9tty_printf(
            PORTLIB, "RefMixed referent slot points to evacuate!  srcObj %p dstObj %p\n", objectPtr, referentPtr);
        Assert_MM_unreachable();
    }
...
void MM_CopyForwardScheme::verifyPointerArrayObjectSlots(MM_EnvironmentVLHGC* env, J9Object* objectPtr)
{
...
void MM_CopyForwardScheme::verifyClassObjectSlots(MM_EnvironmentVLHGC* env, J9Object* classObject)
{
        ...
        do {
            ...
            while (NULL != (slotPtr = classStaticsIterator.nextSlot())) {
                ...
                if (!_abortInProgress && !isObjectInNoEvacuationRegions(env, dstObject)
                    ...
                    PORT_ACCESS_FROM_ENVIRONMENT(env);
                    j9tty_printf(PORTLIB,
                        "Class static slot points to evacuate!  srcObj %p J9Class %p slot %p dstObj %p\n", classObject,
                        classPtr, slotPtr, dstObject);
                    Assert_MM_unreachable();
                }
                if ((NULL != dstObject) && !_markMap->isBitSet(dstObject)) {
                    PORT_ACCESS_FROM_ENVIRONMENT(env);
                    j9tty_printf(PORTLIB,
                        "Class static slot points to unmarked object!  srcObj %p J9Class %p slot %p dstObj %p\n",
            ...
            while (NULL != (slotPtr = callSitesIterator.nextSlot())) {
                ...
                if (!_abortInProgress && !isObjectInNoEvacuationRegions(env, dstObject)
                    ...
                    PORT_ACCESS_FROM_ENVIRONMENT(env);
                    j9tty_printf(PORTLIB,
                        "Class call site slot points to evacuate!  srcObj %p J9Class %p slot %p dstObj %p\n",
                        classObject, classPtr, slotPtr, dstObject);
                    Assert_MM_unreachable();
                }
                if ((NULL != dstObject) && !_markMap->isBitSet(dstObject)) {
                    PORT_ACCESS_FROM_ENVIRONMENT(env);
                    j9tty_printf(PORTLIB,
                        "Class call site slot points to unmarked object!  srcObj %p J9Class %p slot %p dstObj %p\n",
                        classObject, classPtr, slotPtr, dstObject);
                    verifyDumpObjectDetails(env, "classObject", classObject);
                    verifyDumpObjectDetails(env, "dstObj", dstObject);
                    Assert_MM_unreachable();
            ...
            while (NULL != (slotPtr = methodTypesIterator.nextSlot())) {
                ...
                if (!_abortInProgress && !isObjectInNoEvacuationRegions(env, dstObject)
                    ...
                    PORT_ACCESS_FROM_ENVIRONMENT(env);
                    j9tty_printf(PORTLIB,
                        "Class MethodType slot points to evacuate!  srcObj %p J9Class %p slot %p dstObj %p\n",
                        classObject, classPtr, slotPtr, dstObject);
                    Assert_MM_unreachable();
                }
                if ((NULL != dstObject) && !_markMap->isBitSet(dstObject)) {
                    PORT_ACCESS_FROM_ENVIRONMENT(env);
                    j9tty_printf(PORTLIB,
                        "Class MethodType slot points to unmarked object!  srcObj %p J9Class %p slot %p dstObj %p\n",
                        classObject, classPtr, slotPtr, dstObject);
                    verifyDumpObjectDetails(env, "classObject", classObject);
                    verifyDumpObjectDetails(env, "dstObj", dstObject);
                    Assert_MM_unreachable();
             ...
             * scan VarHandle MethodTypes
             */
            GC_MethodTypesIterator varHandleMethodTypesIterator(
                classPtr->romClass->varHandleMethodTypeCount, classPtr->varHandleMethodTypes);
            ...
            while (NULL != (slotPtr = constantPoolIterator.nextSlot())) {
                ...
                if ((NULL != dstObject) && !_markMap->isBitSet(dstObject)) {
                    PORT_ACCESS_FROM_ENVIRONMENT(env);
                    j9tty_printf(PORTLIB,
                        "Class CP slot points to unmarked object!  srcObj %p J9Class %p slot %p dstObj %p\n",
                        classObject, classPtr, slotPtr, dstObject);
                    verifyDumpObjectDetails(env, "classObject", classObject);
                    verifyDumpObjectDetails(env, "dstObj", dstObject);
                    Assert_MM_unreachable();
...
void MM_CopyForwardScheme::verifyClassLoaderObjectSlots(MM_EnvironmentVLHGC* env, J9Object* classLoaderObject)
{
    ...
    if ((NULL != classLoader) && (0 == (classLoader->flags & J9CLASSLOADER_ANON_CLASS_LOADER))) {
        ...
        while (NULL != (clazz = iterator.nextClass())) {
            if (!_abortInProgress && !isObjectInNoEvacuationRegions(env, (J9Object*)clazz->classObject)
                && verifyIsPointerInEvacute(env, (J9Object*)clazz->classObject)) {
                PORT_ACCESS_FROM_ENVIRONMENT(env);
                j9tty_printf(PORTLIB,
                    "Class loader table class object points to evacuate!  srcObj %p clazz %p clazzObj %p\n",
            ...
            if ((NULL != clazz->classObject) && !_markMap->isBitSet((J9Object*)clazz->classObject)) {
                PORT_ACCESS_FROM_ENVIRONMENT(env);
                j9tty_printf(PORTLIB,
                    "Class loader table class object points to unmarked object!  srcObj %p clazz %p clazzObj %p\n",
                    classLoaderObject, clazz, clazz->classObject);
...
void MM_CopyForwardScheme::verifyExternalState(MM_EnvironmentVLHGC* env)
{
    /* this function has knowledge of the collection set, which is only valid during a PGC */
    Assert_MM_true(NULL != env->_cycleState->_externalCycleState);
        ...
        if (region->containsObjects()) {
            if (region->_markData._shouldMark) {
                ...
                if (_abortInProgress || region->_markData._noEvacuation) {
                    MM_HeapMapIterator mapIterator(_extensions, externalMarkMap, (UDATA*)region->getLowAddress(),
                        (UDATA*)region->getHighAddress(), false);
                    J9Object* objectPtr = NULL;
                ...
                MM_HeapMapIterator mapIterator(_extensions, externalMarkMap,
                    (UDATA*)region->_copyForwardData._survivorBase, (UDATA*)region->getHighAddress(), false);
                J9Object* objectPtr = NULL;
    ...
    while (NULL != (packet = packetIterator.nextPacket(env))) {
        if (!packet->isEmpty()) {
            ...
            while (NULL != (slot = slotIterator.nextSlot())) {
                ...
                Assert_MM_true(NULL != object);
                if (PACKET_INVALID_OBJECT != (UDATA)object) {
                    Assert_MM_false(!_abortInProgress && !isObjectInNoEvacuationRegions(env, object)
                        && verifyIsPointerInEvacute(env, object));
                    Assert_MM_true(!verifyIsPointerInSurvivor(env, object)
                        || (_markMap->isBitSet(object) && externalMarkMap->isBitSet(object)));
                }
            }
        }
    }
...
bool MM_CopyForwardScheme::verifyIsPointerInSurvivor(MM_EnvironmentVLHGC* env, J9Object* object)
{
    if (NULL == object) {
        return false;
    }
...
bool MM_CopyForwardScheme::verifyIsPointerInEvacute(MM_EnvironmentVLHGC* env, J9Object* object)
{
    if (NULL == object) {
        return false;
    }
...
void MM_CopyForwardScheme::scanWeakReferenceObjects(MM_EnvironmentVLHGC* env)
{
    ...
    while (NULL != (region = regionIterator.nextRegion())) {
        if ((region->isSurvivorRegion() || region->_copyForwardData._evacuateSet)
            && !region->getReferenceObjectList()->wasWeakListEmpty()) {
            if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
                processReferenceList(env, region, region->getReferenceObjectList()->getPriorWeakList(),
                    &env->_copyForwardStats._weakReferenceStats);
            }
        }
    }
...
void MM_CopyForwardScheme::scanSoftReferenceObjects(MM_EnvironmentVLHGC* env)
{
    Assert_MM_true(env->getGCEnvironment()->_referenceObjectBuffer->isEmpty());
    ...
    while (NULL != (region = regionIterator.nextRegion())) {
        if ((region->isSurvivorRegion() || region->_copyForwardData._evacuateSet)
            && !region->getReferenceObjectList()->wasSoftListEmpty()) {
            if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
                processReferenceList(env, region, region->getReferenceObjectList()->getPriorSoftList(),
                    &env->_copyForwardStats._softReferenceStats);
            }
        }
    }
...
void MM_CopyForwardScheme::scanPhantomReferenceObjects(MM_EnvironmentVLHGC* env)
{
    ...
    while (NULL != (region = regionIterator.nextRegion())) {
        if (region->_copyForwardData._requiresPhantomReferenceProcessing) {
            ...
            if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
                processReferenceList(env, region, region->getReferenceObjectList()->getPriorPhantomList(),
                    &env->_copyForwardStats._phantomReferenceStats);
            }
        }
    }
...
void MM_CopyForwardScheme::processReferenceList(MM_EnvironmentVLHGC* env, MM_HeapRegionDescriptorVLHGC* region,
    J9Object* headOfList, MM_ReferenceStats* referenceStats)
{
    ...
    while (NULL != referenceObj) {
        ...
        GC_SlotObject referentSlotObject(
            _extensions->getOmrVM(), &J9GC_J9VMJAVALANGREFERENCE_REFERENT(env, referenceObj));
        ...
        if (NULL != referent) {
            ...
            if (isLiveObject(referent)) {
                ...
                Assert_MM_true((GC_ObjectModel::REF_STATE_INITIAL == previousState)
                    || (GC_ObjectModel::REF_STATE_REMEMBERED == previousState));
                ...
                if ((J9_JAVA_CLASS_REFERENCE_PHANTOM == referenceObjectType)
                    && ((J2SE_VERSION(_javaVM) & J2SE_VERSION_MASK) <= J2SE_18)) {
                    ...
                    if (GC_ObjectModel::REF_STATE_REMEMBERED == previousState) {
                        Assert_MM_true(NULL != env->_cycleState->_externalCycleState);
                        /* We changed the state from REMEMBERED to CLEARED, so this will not be enqueued back to
                         * region's reference queue. However, GMP has to revisit this reference to mark the referent in
                         * its own mark map.
                         */
        ...
        case GC_ObjectModel::REF_STATE_REMEMBERED:
            Assert_MM_true(NULL != env->_cycleState->_externalCycleState);
            /* This reference object was on a list of GMP reference objects at the start of the cycle. Restore it to its
             * original condition. */
...
void MM_CopyForwardScheme::rememberReferenceList(MM_EnvironmentVLHGC* env, J9Object* headOfList)
{
    ...
    while (NULL != referenceObj) {
        ...
        case GC_ObjectModel::REF_STATE_ENQUEUED:
             ...
             * Explicit enqueue() will clear reference queue field. So, if we still see it in the list, the queue must
             * be null. This GC will rebuild the list, after which the reference must not be on the list anymore.
             * *
             */
...
void MM_CopyForwardScheme::rememberReferenceListsFromExternalCycle(MM_EnvironmentVLHGC* env)
{
    GC_HeapRegionIteratorVLHGC regionIterator(_regionManager);
    MM_HeapRegionDescriptorVLHGC* region = NULL;
    while (NULL != (region = regionIterator.nextRegion())) {
...
void MM_CopyForwardScheme::rememberAndResetReferenceLists(
    MM_EnvironmentVLHGC* env, MM_HeapRegionDescriptorVLHGC* region)
{
    ...
    if (0 == (referenceObjectOptions & MM_CycleState::references_clear_weak)) {
        ...
        if (NULL != headOfList) {
            Trc_MM_CopyForwardScheme_rememberAndResetReferenceLists_rememberWeak(
                env->getLanguageVMThread(), region, headOfList);
    ...
    if (0 == (referenceObjectOptions & MM_CycleState::references_clear_soft)) {
        ...
        if (NULL != headOfList) {
            Trc_MM_CopyForwardScheme_rememberAndResetReferenceLists_rememberSoft(
                env->getLanguageVMThread(), region, headOfList);
            rememberReferenceList(env, headOfList);
        }
    }
    ...
    if (0 == (referenceObjectOptions & MM_CycleState::references_clear_phantom)) {
        ...
        if (NULL != headOfList) {
            Trc_MM_CopyForwardScheme_rememberAndResetReferenceLists_rememberPhantom(
                env->getLanguageVMThread(), region, headOfList);
            rememberReferenceList(env, headOfList);
        }
    }
...
#if defined(J9VM_GC_FINALIZATION)
void MM_CopyForwardScheme::scanFinalizableObjects(MM_EnvironmentVLHGC* env)
{
    GC_FinalizeListManager* finalizeListManager = _extensions->finalizeListManager;

    /* If we're copying objects this code must be run single-threaded and we should only be here if work is actually
     * required */
    /* This function is also used during abort; these assertions aren't applicable to that case because objects can't be
     * copied during abort */
...
void MM_CopyForwardScheme::scanFinalizableList(MM_EnvironmentVLHGC* env, j9object_t headObject)
{
...
void MM_CopyForwardScheme::removeTailCandidate(
    MM_EnvironmentVLHGC* env, MM_ReservedRegionListHeader* regionList, MM_HeapRegionDescriptorVLHGC* tailRegion)
{
...
void MM_CopyForwardScheme::insertTailCandidate(
    MM_EnvironmentVLHGC* env, MM_ReservedRegionListHeader* regionList, MM_HeapRegionDescriptorVLHGC* tailRegion)
{
...
void MM_CopyForwardScheme::convertTailCandidateToSurvivorRegion(
    MM_EnvironmentVLHGC* env, MM_HeapRegionDescriptorVLHGC* region, void* survivorBase)
{
    Trc_MM_CopyForwardScheme_convertTailCandidateToSurvivorRegion_Entry(
        env->getLanguageVMThread(), region, survivorBase);
    ...
    /* TODO: Remembering does not really have to be done under a lock, but dual (prev, current) list implementation
     * indirectly forces us to do it this way. */
    rememberAndResetReferenceLists(env, region);

    Trc_MM_CopyForwardScheme_convertTailCandidateToSurvivorRegion_Exit(env->getLanguageVMThread());
}

void MM_CopyForwardScheme::setRegionAsSurvivor(
    MM_EnvironmentVLHGC* env, MM_HeapRegionDescriptorVLHGC* region, void* survivorBase)
{
    MM_MemoryPoolBumpPointer* memoryPool = (MM_MemoryPoolBumpPointer*)region->getMemoryPool();
    ...
    Trc_MM_CopyForwardScheme_setRegionAsSurvivor(env->getLanguageVMThread(),
        _regionManager->mapDescriptorToRegionTableIndex(region),
        MM_CompactGroupManager::getCompactGroupNumber(env, region), (double)region->getAllocationAge() / (1024 * 1024),
        (double)usedBytes / (1024 * 1024), allocationAgeSizeProduct / (1024 * 1024) / (1024 * 1024));
    ...
    /* update the pool so it only knows about the free memory occurring before survivor base.  We will add whatever we
     * don't use at the end of the copy-forward */
...
void MM_CopyForwardScheme::setAllocationAgeForMergedRegion(
    MM_EnvironmentVLHGC* env, MM_HeapRegionDescriptorVLHGC* region)
{
    UDATA compactGroup = MM_CompactGroupManager::getCompactGroupNumber(env, region);
    ...
    Trc_MM_CopyForwardScheme_setAllocationAgeForMergedRegion(env->getLanguageVMThread(),
        _regionManager->mapDescriptorToRegionTableIndex(region), compactGroup,
        region->getAllocationAgeSizeProduct() / (1024 * 1024) / (1024 * 1024), (double)usedBytes / (1024 * 1024),
        (double)newAllocationAge / (1024 * 1024), (double)region->getLowerAgeBound() / (1024 * 1024),
    ...
    if (_extensions->tarokAllocationAgeEnabled) {
        ...
        Assert_MM_true((MM_CompactGroupManager::getRegionAgeFromGroup(env, compactGroup) == 0)
...
bool MM_CopyForwardScheme::isObjectInNoEvacuationRegions(MM_EnvironmentVLHGC* env, J9Object* objectPtr)
{
...
bool MM_CopyForwardScheme::randomDecideForceNonEvacuatedRegion(UDATA ratio)
{
    bool ret = false;
