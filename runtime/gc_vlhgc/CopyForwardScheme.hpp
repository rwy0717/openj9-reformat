...
private:
    ...
    MM_InterRegionRememberedSet*
        _interRegionRememberedSet; /**< A cached pointer to the inter-region reference tracking mechanism */
    ...
    public:
        ...
        MM_HeapRegionDescriptorVLHGC*
            _tailCandidates; /**< A linked list of regions in this compact group which have empty tails */
    ...
    UDATA _phantomReferenceRegionsToProcess; /**< A count, for verification purposes, of the number of regions to be
                                                processed for phantom references */
    ...
    MM_CopyScanCacheListVLHGC
        _cacheFreeList; /**< Caches which are not bound to heap memory and available to be populated */
    MM_CopyScanCacheListVLHGC* _cacheScanLists; /**< An array of per-node caches which contains objects still to be
                                                   scanned (1+node_count elements in array)*/
    UDATA _scanCacheListSize; /**< The number of entries in _cacheScanLists */
    volatile UDATA _scanCacheWaitCount; /**< The number of threads currently sleeping on _scanCacheMonitor, awaiting
                                           scan cache work */
    omrthread_monitor_t _scanCacheMonitor; /**< Used when waiting on work on any of the _cacheScanLists */

    volatile UDATA* _workQueueWaitCountPtr; /**< The number of threads currently sleeping on *_workQueueMonitorPtr,
                                               awaiting scan cache work or work from packets*/
    omrthread_monitor_t*
        _workQueueMonitorPtr; /**< Used when waiting on work on any of the _cacheScanLists or workPackets*/

    volatile UDATA _doneIndex; /**< Incremented when _cacheScanLists are empty and we want all threads to fall out of
    ...
    volatile bool _abortFlag; /**< Flag indicating whether the current copy forward cycle should be aborted due to
                                 insufficient heap to complete */
    bool _abortInProgress; /**< Flag indicating that the copy forward mechanism is now operating in abort mode, which is
                              attempting to secure integrity of the heap to continue execution */

    UDATA _regionCountCannotBeEvacuated; /**<The number of eden regions, which can not be copyforward */

    UDATA _cacheLineAlignment; /**< The number of bytes per cache line which is used to determine which boundaries in
                                  memory represent the beginning of a cache line */

    bool _clearableProcessingStarted; /**< Flag indicating that clearable processing had been started during this cycle
                                         (used for abort purposes) */

#if defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING)
    bool _dynamicClassUnloadingEnabled; /**< Local cached value from cycle state for performance reasons (TODO:
                                           Reevaluate) */
    ...
    MM_AllocationContextTarok* _commonContext; /**< The common context is used as an opaque token to represent cases
                                                  where we don't want to relocate objects during NUMA-aware copy-forward
                                                  since relocating to the common context is currently disabled */
    MM_CopyForwardCompactGroup* _compactGroupBlock; /**< A block of MM_CopyForwardCompactGroup structs which is
                                                       subdivided among the GC threads */
    UDATA _arraySplitSize; /**< The number of elements to be scanned in each array chunk (this determines the degree of
                              parallelization) */

    UDATA
        _regionSublistContentionThreshold /**< The number of threads which must be contending on the same region sublist
                                             for us to decide that another sublist should be created to alleviate
                                             contention (reset at the beginning of every CopyForward task) */
        ;

    volatile bool _failedToExpand; /**< Record if we've failed to expand in this collection already, in order to avoid
                                      repeated expansion attempts */
    bool _shouldScanFinalizableObjects; /**< Set to true at the beginning of a collection if there are any pending
                                           finalizable objects */
...
private:
     ...
     * @param wastedMemory[in] The number of bytes of memory which couldn't be used in the allocated portion of the copy
     * cache and should be considered free memory when flushed back to the pool
     */
    void discardRemainingCache(MM_EnvironmentVLHGC* env, MM_CopyScanCacheVLHGC* cache,
    ...
    /**
     * Determine whether the object is live relying on survivor/evacuate region flags and mark map. null object is
     * considered marked.
    ...
    /**
     * Reinitializes the cache with the given base address, top address, as a copy cache.  Also updates the mark map
     * cache values in the env's CopyForwardCompactGroup for the given cache.
     ...
     */
    MMINLINE void reinitArraySplitCache(
    ...
    /**
     * Adds the given newCacheEntry to the scan cache list and will notify any waiting threads if the list was empty
     * prior to adding newCacheEntry.
     * @note The implementation will lock the list prior to modification.
     * @param env current GC thread.
     * @param newCacheEntry[in] The entry to add to cacheList
     */
     ...
     */
    MMINLINE void scanOwnableSynchronizerObjectSlots(
        MM_EnvironmentVLHGC* env, MM_AllocationContextTarok* reservingContext, J9Object* objectPtr, ScanReason reason);

    /**
     * Called whenever a ownable synchronizer object is scaned during CopyForwardScheme. Places the object on the
    ...
    /**
     * Set region as survivor (and its survivor base address). Also set the special state if region was created by
     * phantom reference processing.
     * @param env[in] the current thread
     * @param region[in] region to set as survivor
     * @param survivorBase[in] survivor base address (all object below this address, if different than low region
     * address are not part of survivor)
     */
     ...
     */
    void processReferenceList(MM_EnvironmentVLHGC* env, MM_HeapRegionDescriptorVLHGC* region, J9Object* headOfList,
        MM_ReferenceStats* referenceStats);
    ...
    /**
     * Call rememberReferenceList() for each of the weak, soft and phantom lists in referenceObjectList, then reset
     * them.
     * @param env[in] the current thread
     ...
     * @param reason to scan (dirty card, packet, scan cache, overflow)
     */
    void scanPointerArrayObjectSlots(MM_EnvironmentVLHGC* env, MM_AllocationContextTarok* reservingContext,
        J9IndexableObject* arrayPtr, ScanReason reason);
     ...
     * @param currentSplitUnitOnly[in] do only current array split work unit; do not push the rest of array on the work
     * stack
     * @return number of slots scanned
     */
    UDATA scanPointerArrayObjectSlotsSplit(MM_EnvironmentVLHGC* env, MM_AllocationContextTarok* reservingContext,
        J9IndexableObject* arrayPtr, UDATA startIndex, bool currentSplitUnitOnly = false);

    /**
     * For the given object and current starting index, determine the number of slots being scanned now, and create a
     * work unit for the rest of the array. Works for both copy-scan cache and workstack driven phases.
     * @param env[in] the current thread
     ...
     */
    UDATA createNextSplitArrayWorkUnit(
        MM_EnvironmentVLHGC* env, J9IndexableObject* arrayPtr, UDATA startIndex, bool currentSplitUnitOnly = false);
     ...
     * @param reason to scan (dirty card, packet, scan cache, overflow)
     */
    void scanClassObjectSlots(MM_EnvironmentVLHGC* env, MM_AllocationContextTarok* reservingContext,
        J9Object* classObject, ScanReason reason = SCAN_REASON_COPYSCANCACHE);
    /**
     ...
     * @param reason to scan (dirty card, packet, scan cache, overflow)
     */
    void scanClassLoaderObjectSlots(MM_EnvironmentVLHGC* env, MM_AllocationContextTarok* reservingContext,
        J9Object* classLoaderObject, ScanReason reason = SCAN_REASON_COPYSCANCACHE);
     ...
     */
    MMINLINE void calculateObjectDetailsForCopy(MM_ScavengerForwardedHeader* forwardedHeader,
     ...
     */
    MM_HeapRegionDescriptorVLHGC* acquireRegion(
        MM_EnvironmentVLHGC* env, MM_ReservedRegionListHeader::Sublist* regionList, UDATA compactGroup);

    /**
     * Inserts newRegion into the given regionList.  The implementation assumes that the calling thread can modify
     ...
     * @param regionList[in] The region list to which the implementation will add newRegion (call site must ensure that
     * no other thread is able to view this until the method returns)
     * @param newRegion[in] The region which the implementation must add to regionList (the implementation assumes that
     * newRegion is not already in a region list)
     */
    void insertRegionIntoLockedList(MM_EnvironmentVLHGC* env, MM_ReservedRegionListHeader::Sublist* regionList,
        MM_HeapRegionDescriptorVLHGC* newRegion);
     ...
     */
    void releaseRegion(MM_EnvironmentVLHGC* env, MM_ReservedRegionListHeader::Sublist* regionList,
        MM_HeapRegionDescriptorVLHGC* region);

    /**
     * Insert the specified tail candidate into the tail candidate list.  The implementation assumes that the calling
     * thread can modify regionList without locking it so the callsite either needs to have locked the list or be
     * single-threaded.
     * @param env[in] The GC thread
     ...
     */
    void insertTailCandidate(
        MM_EnvironmentVLHGC* env, MM_ReservedRegionListHeader* regionList, MM_HeapRegionDescriptorVLHGC* tailRegion);

    /**
     * Remove the specified tail candidate from the tail candidate list.  The implementation assumes that the calling
     * thread can modify regionList without locking it so the callsite either needs to have locked the list or be
     * single-threaded.
     * @param env[in] The GC thread
     ...
     */
    bool reserveMemoryForCache(MM_EnvironmentVLHGC* env, UDATA compactGroup, UDATA maxCacheSize, void** addrBase,
        void** addrTop, MM_LightweightNonReentrantLock** listLock);
     ...
     * @param reservingContext[in] The context to which we would prefer to copy any objects discovered in this method
     * @param objectReserveSizeInBytes Amount of bytes to be reserved (can be greater than the original object size) for
     * copying.
     * @return a CopyScanCache which contains the reserved memory or NULL if the reserve was not successful.
     */
    MMINLINE MM_CopyScanCacheVLHGC* reserveMemoryForCopy(MM_EnvironmentVLHGC* env, J9Object* objectToEvacuate,
        MM_AllocationContextTarok* reservingContext, UDATA objectReserveSizeInBytes);

    void flushCaches(MM_CopyScanCacheVLHGC* cache);

    /**
     * Called at the end of the copy forward operation to flush any remaining data from copy caches and return them to
     * the free list.
    ...
    /**
     * Tries to find a scan cache from the specified NUMA node or return SCAN_REASON_NONE if there was no work available
     * on that node
     ...
     */
    MMINLINE bool incrementalScanMixedObjectSlots(MM_EnvironmentVLHGC* env, MM_AllocationContextTarok* reservingContext,
        MM_CopyScanCacheVLHGC* scanCache, J9Object* objectPtr, bool hasPartiallyScannedObject,
        MM_CopyScanCacheVLHGC** nextScanCache);
    /**
     ...
     */
    MMINLINE bool incrementalScanClassObjectSlots(MM_EnvironmentVLHGC* env, MM_AllocationContextTarok* reservingContext,
        MM_CopyScanCacheVLHGC* scanCache, J9Object* objectPtr, bool hasPartiallyScannedObject,
        MM_CopyScanCacheVLHGC** nextScanCache);
    /**
     ...
     * @see MM_CopyForwardScheme:incrementalScanMixedObjectSlots
     */
    MMINLINE bool incrementalScanClassLoaderObjectSlots(MM_EnvironmentVLHGC* env,
     ...
     * @see MM_CopyForwardScheme:incrementalScanMixedObjectSlots
     */
    MMINLINE bool incrementalScanPointerArrayObjectSlots(MM_EnvironmentVLHGC* env,
        MM_AllocationContextTarok* reservingContext, MM_CopyScanCacheVLHGC* scanCache, J9Object* objectPtr,
        bool hasPartiallyScannedObject, MM_CopyScanCacheVLHGC** nextScanCache);
    /**
     ...
     * @see MM_CopyForwardScheme:incrementalScanMixedObjectSlots
     */
    MMINLINE bool incrementalScanReferenceObjectSlots(MM_EnvironmentVLHGC* env,
        MM_AllocationContextTarok* reservingContext, MM_CopyScanCacheVLHGC* scanCache, J9Object* objectPtr,
        bool hasPartiallyScannedObject, MM_CopyScanCacheVLHGC** nextScanCache);
    /**
     ...
     * Attempt to copy (either flip or tenure) the object and install a forwarding pointer at the new location. The
     * object may have already been copied. In either case, update the slot to point at the new location of the object.
     *
     * @param reservingContext[in] The context to which we would prefer to copy any objects discovered in this method
     ...
     */
    MMINLINE bool copyAndForward(MM_EnvironmentVLHGC* env, MM_AllocationContextTarok* reservingContext,
        J9Object* objectPtr, volatile j9object_t* slot);
     ...
     */
    MMINLINE bool copyAndForward(MM_EnvironmentVLHGC* env, MM_AllocationContextTarok* reservingContext,
        J9Object* objectPtr, GC_SlotObject* slotObject);
     ...
     * @return true if copy succeeded (or no copying was involved)
     */
    MMINLINE bool copyAndForwardPointerArray(MM_EnvironmentVLHGC* env, MM_AllocationContextTarok* reservingContext,
        J9IndexableObject* arrayPtr, UDATA startIndex, GC_SlotObject* slotObject);
     ...
     * @return true if copy succeeded (or no copying was involved)
     */
    MMINLINE bool copyAndForwardObjectClass(
     ...
     * Attempt to copy and forward the given object header between the evacuate and survivor areas.  If the object has
     * already been copied, or the copy is successful, return the updated information.  If the copy is not successful
     * due to insufficient heap memory, return the original object pointer and raise the "abort" flag.
     * @param reservingContext[in] The context to which we would prefer to copy any objects discovered in this method
     ...
     */
    J9Object* copy(MM_EnvironmentVLHGC* env, MM_AllocationContextTarok* reservingContext,
        MM_ScavengerForwardedHeader* forwardedHeader);
     ...
     */
    MMINLINE void updateMarkMapAndCardTableOnCopy(
    ...
    /**
     * Used by the logic which relocates objects to different NUMA nodes by looking up the context which owns a given
     * "source" address in the heap.
     ...
     */
    void convertTailCandidateToSurvivorRegion(
...
protected:
    ...
    /**
     * Helper routine to clean cards during a partial collection for the purpose of finding "roots" into the collection
     * set.
     * @param env A GC thread involved in the cleaning.
     ...
     * This routine will clear the external mark map for the collection set (if successful) or adjust the map on moved
     * objects (abort) and remove any dead objects from external cycle work packets.  It will also update (forward) any
     * work packet pointers which have survived the collection.  The responsibility of setting the external cycle mark
     * map survivor bits and updating cards lies with object copying.
     */
    void updateOrDeleteObjectsFromExternalCycle(MM_EnvironmentVLHGC* env);

    /**
     * Scans the marked objects in the [lowAddress..highAddress) range.  If rememberedObjectsOnly is set, we will
     * further constrain this scanned set by only scanning objects in the range which have the OBJECT_HEADER_REMEMBERED
     * bit set.  The receiver uses its _markMap to determine which objects in the range are marked.
     *
     * @param env[in] A GC thread (note that this method could be called by multiple threads, in parallel, but on
     ...
     * @param rememberedObjectsOnly If true, the receiver only scans remembered live objects in the range.  If false, it
     * scans all live objects in the range
     ...
     * @return The reservingContext or the object's owning context if the suggestedContext is not a preferred object
     * relocation context
     */
    MMINLINE MM_AllocationContextTarok* getPreferredAllocationContext(
...
public:
     ...
     */
    bool isHybrid(MM_EnvironmentVLHGC* env) { return (0 != _regionCountCannotBeEvacuated); }
