...
bool MM_CopyScanCacheListVLHGC::initialize(MM_EnvironmentVLHGC* env)
{
    ...
    _sublists = (CopyScanCacheSublist*)extensions->getForge()->allocate(
        sublistBytes, MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
    if (NULL == _sublists) {
        _sublistCount = 0;
        return false;
    }
    ...
    for (UDATA i = 0; i < _sublistCount; i++) {
        if (!_sublists[i]._cacheLock.initialize(
                env, &extensions->lnrlOptions, "MM_CopyScanCacheListVLHGC:_sublists[]._cacheLock")) {
            return false;
        }
    }
...
void MM_CopyScanCacheListVLHGC::tearDown(MM_EnvironmentVLHGC* env)
{
...
bool MM_CopyScanCacheListVLHGC::appendCacheEntries(MM_EnvironmentVLHGC* env, UDATA cacheEntryCount)
{
    CopyScanCacheSublist* cacheList = &_sublists[getSublistIndex(env)];
    MM_CopyScanCacheChunkVLHGC* chunk
        = MM_CopyScanCacheChunkVLHGC::newInstance(env, cacheEntryCount, &cacheList->_cacheHead, _chunkHead);
    if (NULL != chunk) {
        _chunkHead = chunk;
...
bool MM_CopyScanCacheListVLHGC::resizeCacheEntries(MM_EnvironmentVLHGC* env, UDATA totalCacheEntryCount)
{
...
void MM_CopyScanCacheListVLHGC::removeAllHeapAllocatedChunks(MM_EnvironmentVLHGC* env)
{
...
MM_CopyScanCacheVLHGC* MM_CopyScanCacheListVLHGC::allocateCacheEntriesInExistingMemory(
    MM_EnvironmentVLHGC* env, void* buffer, UDATA bufferLengthInBytes)
{
    CopyScanCacheSublist* cacheList = &_sublists[getSublistIndex(env)];
    MM_CopyScanCacheVLHGC* result = NULL;
    MM_CopyScanCacheChunkVLHGCInHeap* chunk = MM_CopyScanCacheChunkVLHGCInHeap::newInstance(
...
void MM_CopyScanCacheListVLHGC::lock()
{
    for (UDATA i = 0; i < _sublistCount; ++i) {
...
void MM_CopyScanCacheListVLHGC::unlock()
{
    for (UDATA i = 0; i < _sublistCount; ++i) {
...
void MM_CopyScanCacheListVLHGC::pushCacheInternal(
    MM_EnvironmentVLHGC* env, MM_CopyScanCacheVLHGC* cacheEntry, CopyScanCacheSublist* sublist)
{
...
MM_CopyScanCacheVLHGC* MM_CopyScanCacheListVLHGC::popCacheInternal(
    MM_EnvironmentVLHGC* env, CopyScanCacheSublist* sublist)
{
...
void MM_CopyScanCacheListVLHGC::pushCacheNoLock(MM_EnvironmentVLHGC* env, MM_CopyScanCacheVLHGC* cacheEntry)
{
    CopyScanCacheSublist* cacheList = &_sublists[getSublistIndex(env)];
    pushCacheInternal(env, cacheEntry, cacheList);
}

void MM_CopyScanCacheListVLHGC::pushCache(MM_EnvironmentVLHGC* env, MM_CopyScanCacheVLHGC* cacheEntry)
{
    CopyScanCacheSublist* cacheList = &_sublists[getSublistIndex(env)];
...
MM_CopyScanCacheVLHGC* MM_CopyScanCacheListVLHGC::popCacheNoLock(MM_EnvironmentVLHGC* env)
{
...
MM_CopyScanCacheVLHGC* MM_CopyScanCacheListVLHGC::popCache(MM_EnvironmentVLHGC* env)
{
    UDATA indexStart = getSublistIndex(env);
    MM_CopyScanCacheVLHGC* cache = NULL;
    for (UDATA i = 0; (i < _sublistCount) && (NULL == cache); ++i) {
...
bool MM_CopyScanCacheListVLHGC::isEmpty()
{
    MM_CopyScanCacheVLHGC* cache = NULL;
    for (UDATA i = 0; (i < _sublistCount) && (NULL == cache); ++i) {
