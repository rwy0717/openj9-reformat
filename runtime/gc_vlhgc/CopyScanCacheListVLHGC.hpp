...
public:
    ...
    /**
     * Removes any heap-allocated scan cache chunks from the receiver's chunk list (calls tearDown on them after
     ...
     * @return A new cache entry if the chunk was successfully allocated and inserted into the receiver's chunk list,
     * NULL if a failure occurred
     */
    MM_CopyScanCacheVLHGC* allocateCacheEntriesInExistingMemory(
    ...
    /**
     * Answer the total number of caches owned by the receiver, whether or not they are currently in the receiver's
     * lists.
