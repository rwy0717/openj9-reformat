...
public:
    ...
    MM_CopyForwardCompactGroup*
        _copyForwardCompactGroups; /**< List of copy-forward data for each compact group for the given GC thread (only
                                      for GC threads during copy forward operations) */

    UDATA _previousConcurrentYieldCheckBytesScanned; /**< The number of bytes scanned in the mark stats at the end of
                                                        the previous shouldYieldFromTask check in concurrent mark */
    ...
    MM_RememberedSetCardBucket*
        _rememberedSetCardBucketPool; /**< GC thread local pool of RS Card Buckets for each Region (its Card List) */
    MM_RememberedSetCardList* _lastOverflowedRsclWithReleasedBuffers; /**< in global list of overflowed RSCL, this is
                                                                         the last RSCL this thread visited */
...
public:
    ...
    MMINLINE static MM_EnvironmentVLHGC* getEnvironment(J9VMThread* vmThread)
    {
        return static_cast<MM_EnvironmentVLHGC*>(vmThread->gcExtensions);
    }
    MMINLINE static MM_EnvironmentVLHGC* getEnvironment(OMR_VMThread* omrVMThread)
    {
        return static_cast<MM_EnvironmentVLHGC*>(omrVMThread->_gcOmrVMThreadExtensions);
    }
    MMINLINE static MM_EnvironmentVLHGC* getEnvironment(MM_EnvironmentBase* env)
    {
        return static_cast<MM_EnvironmentVLHGC*>(env);
    }
