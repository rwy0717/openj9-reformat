...
MM_GlobalAllocationManagerTarok* MM_GlobalAllocationManagerTarok::newInstance(MM_EnvironmentBase* env)
{
    MM_GlobalAllocationManagerTarok* allocationManager = (MM_GlobalAllocationManagerTarok*)env->getForge()->allocate(
        sizeof(MM_GlobalAllocationManagerTarok), MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
...
void MM_GlobalAllocationManagerTarok::kill(MM_EnvironmentBase* env)
{
    tearDown(env);
    env->getForge()->free(this);
}

bool MM_GlobalAllocationManagerTarok::shouldIdentifyThreadAsCommon(MM_EnvironmentBase* env)
{
    bool result = false;

    if (_extensions->tarokAttachedThreadsAreCommon) {
        result = (J9_PRIVATE_FLAGS_ATTACHED_THREAD
            == ((J9_PRIVATE_FLAGS_ATTACHED_THREAD | J9_PRIVATE_FLAGS_SYSTEM_THREAD)
                   & ((J9VMThread*)env->getLanguageVMThread())->privateFlags));
    }
...
bool MM_GlobalAllocationManagerTarok::acquireAllocationContext(MM_EnvironmentBase* env)
{
    ...
    /* this check is kind of gratuitous but it ensures that the env accepted the context we gave it and that this GAM is
     * correctly initialized */
    return (context == env->getAllocationContext());
}

void MM_GlobalAllocationManagerTarok::releaseAllocationContext(MM_EnvironmentBase* env)
{
    /* just disassociate the env from this context */
    env->setAllocationContext(NULL);
}

bool MM_GlobalAllocationManagerTarok::initializeAllocationContexts(
    MM_EnvironmentBase* env, MM_MemorySubSpaceTarok* subspace)
{
    UDATA allocationSize = sizeof(MM_AllocationContextBalanced*) * _managedAllocationContextCount;
    MM_AllocationContextBalanced** contexts = (MM_AllocationContextBalanced**)env->getForge()->allocate(
        allocationSize, MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
    if (NULL == contexts) {
        return false;
    }
    ...
    _perNodeContextSets = (MM_AllocationContextBalanced**)env->getForge()->allocate(
        owningByNodeSize, MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
    ...
    MM_AllocationContextBalanced* commonContext
        = MM_AllocationContextBalanced::newInstance(env, subspace, 0, COMMON_CONTEXT_INDEX);
    ...
    for (UDATA i = 0; i < affinityLeaderCount; i++) {
        ...
        MM_AllocationContextBalanced* context
            = MM_AllocationContextBalanced::newInstance(env, subspace, numaNode, nextContextIndex);
        if (NULL == context) {
            return false;
        }
    ...
    _nextAllocationContext = (1 == _managedAllocationContextCount)
        ? 0
        : (_extensions->fvtest_tarokFirstContext % (_managedAllocationContextCount - 1));

    return true;
}

bool MM_GlobalAllocationManagerTarok::initialize(MM_EnvironmentBase* env)
{
 ...
 */
void MM_GlobalAllocationManagerTarok::tearDown(MM_EnvironmentBase* env)
{
 ...
 */
void MM_GlobalAllocationManagerTarok::printAllocationContextStats(
    MM_EnvironmentBase* env, UDATA eventNum, J9HookInterface** hookInterface)
{
    ...
    for (UDATA i = 0; i < _managedAllocationContextCount; i++) {
        ...
        j9tty_printf(PORTLIB,
            "AC %3d %s MPAOL regionCount %5d (AO %5d AO_IDLE %5d AO_MARKED %5d) mutatorCount %3d numaNode %d (%d "
            "local, %d foreign)\n",
            i, eventName, acRegionCount, ac->getRegionCount(MM_HeapRegionDescriptor::BUMP_ALLOCATED),
            ac->getRegionCount(MM_HeapRegionDescriptor::BUMP_ALLOCATED_IDLE),
            ac->getRegionCount(MM_HeapRegionDescriptor::BUMP_ALLOCATED_MARKED), ac->getThreadCount(), ac->getNumaNode(),
            localCount, foreignCount);
    }

    j9tty_printf(PORTLIB, "AC sum %s MPAOL regionCount %5d (total %d) \n", eventName, totalRegionCount,
        MM_GCExtensions::getExtensions(env)->heapRegionManager->getTableRegionCount());
}
...
UDATA
MM_GlobalAllocationManagerTarok::getApproximateFreeMemorySize() { return getActualFreeMemorySize(); }

void MM_GlobalAllocationManagerTarok::resetLargestFreeEntry()
{
    for (UDATA i = 0; i < _managedAllocationContextCount; i++) {
        ((MM_AllocationContextTarok*)_managedAllocationContexts[i])->resetLargestFreeEntry();
    }
}
void MM_GlobalAllocationManagerTarok::expand(MM_EnvironmentBase* env, MM_HeapRegionDescriptorVLHGC* region)
{
...
void MM_GlobalAllocationManagerTarok::resetHeapStatistics(bool globalCollect)
{
    for (UDATA i = 0; i < _managedAllocationContextCount; i++) {
        ((MM_AllocationContextTarok*)_managedAllocationContexts[i])->resetHeapStatistics(globalCollect);
    }
}
void MM_GlobalAllocationManagerTarok::mergeHeapStats(MM_HeapStats* heapStats, UDATA includeMemoryType)
{
    for (UDATA i = 0; i < _managedAllocationContextCount; i++) {
...
MM_GlobalAllocationManagerTarok::calculateIdealManagedContextCount(MM_GCExtensionsBase* extensions)
{
    ...
    /* heuristic -- ACs are permitted to waste up to 1/8th of the heap in slack regions. This number may need to be
     * adjusted */
...
MM_AllocationContextBalanced* MM_GlobalAllocationManagerTarok::getAllocationContextForNumaNode(UDATA numaNode)
{
