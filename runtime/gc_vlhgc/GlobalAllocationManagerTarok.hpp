...
private:
    MM_AllocationContextBalanced**
        _perNodeContextSets; /**< an array which is extensions->numaNodes elements long, containing the "first"
                                AllocationContextVLHGC in each corresponding per-node cicular list */

    MM_RuntimeExecManager _runtimeExecManager; /**< A helper object used to intercept Runtime.exec() and manage affinity
                                                  to workaround limitations on Linux */
...
public:
    ...
    /**
     * Called after the receiver and the subspace have been initialized to request that the receiver build the
     * allocation contexts it will manager over the course of the run.  The subspace is required because the allocation
     * contexts need to know which subspace logically "owns" its memory.
     * @param env[in] The current thread (this is the thread bootstrapping the VM)
     * @param subspace[in] The subspace which logically owns the memory managed by the contexts the receiver is called
     * to create
    ...
    /**
     * Expands into the given region by finding an AllocationContextTarok to manage it.  Once this method returns, the
     * given region will be managed by a context.
    ...
    /**
     * This helper merely forwards the resetHeapStatistics call on to all the AllocationContextTarok instances owned by
     * the receiver.
    ...
    /**
     * This helper merely forwards the mergeHeapStats call on to all the AllocationContextTarok instances owned by the
     * receiver.
     ...
     */
    MMINLINE UDATA getManagedAllocationContextCount() { return _managedAllocationContextCount; }

    /**
     * @param extensions[in] The global GC extensions
     * @return The ideal number of total allocation contexts (managed + non-managed) with which the receiver assumes it
     * is working
     */
