...
MM_GlobalMarkCardScrubber::MM_GlobalMarkCardScrubber(
    MM_EnvironmentVLHGC* env, MM_HeapMap* map, UDATA yieldCheckFrequency)
    : MM_CardCleaner()
    , _markMap(map)
...
void MM_GlobalMarkCardScrubber::clean(
...
bool MM_GlobalMarkCardScrubber::scrubObjectsInRange(MM_EnvironmentVLHGC* env, void* lowAddress, void* highAddress)
{
    bool doScrub = true;
    UDATA count = 0;
...
bool MM_GlobalMarkCardScrubber::scrubObject(MM_EnvironmentVLHGC* env, J9Object* objectPtr)
{
    bool doScrub = true;
...
bool MM_GlobalMarkCardScrubber::scrubMixedObject(MM_EnvironmentVLHGC* env, J9Object* objectPtr)
{
    bool doScrub = true;

    /* No need to look at the class since it is immutable. However we can't assert that it's marked since the compactor
     * may have deleted part of the mark map. */
...
bool MM_GlobalMarkCardScrubber::scrubPointerArrayObject(MM_EnvironmentVLHGC* env, J9Object* objectPtr)
{
    bool doScrub = true;
...
bool MM_GlobalMarkCardScrubber::scrubClassObject(MM_EnvironmentVLHGC* env, J9Object* classObject)
{
...
bool MM_GlobalMarkCardScrubber::scrubClassLoaderObject(MM_EnvironmentVLHGC* env, J9Object* classLoaderObject)
{
...
bool MM_GlobalMarkCardScrubber::mayScrubReference(MM_EnvironmentVLHGC* env, J9Object* fromObject, J9Object* toObject)
{
    bool doScrub = true;
    ...
    if (doScrub && (NULL != toObject)) {
        doScrub = (_markMap->isBitSet(toObject))
            && !(_interRegionRememberedSet->shouldRememberReferenceForGlobalMark(env, fromObject, toObject));
    }
    return doScrub;
}

void MM_ParallelScrubCardTableTask::run(MM_EnvironmentBase* envBase)
{
    ...
    Trc_MM_ParallelScrubCardTableTask_scrubCardTable_Exit(env->getLanguageVMThread(), env->getSlaveID(),
        scrubber.getScrubbedObjects(), scrubber.getScrubbedCards(), scrubber.getDirtyCards(),
        scrubber.getGMPMustScanCards(),
        j9time_hires_delta(cleanStartTime, cleanEndTime, J9PORT_TIME_DELTA_IN_MICROSECONDS),
        didTimeout() ? "true" : "false");
}

void MM_ParallelScrubCardTableTask::setup(MM_EnvironmentBase* env)
{
...
void MM_ParallelScrubCardTableTask::cleanup(MM_EnvironmentBase* env)
{
    if (!env->isMasterThread()) {
...
bool MM_ParallelScrubCardTableTask::shouldYieldFromTask(MM_EnvironmentBase* env)
{
    if (!_timeLimitWasHit) {
        PORT_ACCESS_FROM_ENVIRONMENT(env);
...
void MM_ParallelScrubCardTableTask::synchronizeGCThreads(MM_EnvironmentBase* env, const char* id)
{
...
bool MM_ParallelScrubCardTableTask::synchronizeGCThreadsAndReleaseMaster(MM_EnvironmentBase* env, const char* id)
{
    /* this task doesn't use synchronization */
    Assert_MM_unreachable();
...
bool MM_ParallelScrubCardTableTask::synchronizeGCThreadsAndReleaseSingleThread(MM_EnvironmentBase* env, const char* id)
{
    /* this task doesn't use synchronization */
    Assert_MM_unreachable();
