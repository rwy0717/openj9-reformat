...
private:
    ...
    struct {
        ...
        UDATA
            _scrubbedCards; /**< A count of the cards successfully scrubbed (changed state) by this scrubber instance */
...
private:
    ...
    /**
     * Scans the marked objects in the [lowAddress..highAddress) range to determine if these objects need to be scanned
     * during card cleaning. The receiver uses its _markMap to determine which objects in the range are marked.
     *
...
public:
     ...
     */
    MM_ParallelScrubCardTableTask(
        MM_EnvironmentBase* env, MM_Dispatcher* dispatcher, I_64 timeThreshold, MM_CycleState* cycleState)
