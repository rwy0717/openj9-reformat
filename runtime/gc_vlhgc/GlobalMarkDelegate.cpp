...
bool MM_GlobalMarkDelegate::initialize(MM_EnvironmentVLHGC* env)
{
...
void MM_GlobalMarkDelegate::tearDown(MM_EnvironmentVLHGC* env)
{
    _dispatcher = NULL;
...
bool MM_GlobalMarkDelegate::heapAddRange(
...
bool MM_GlobalMarkDelegate::heapRemoveRange(MM_EnvironmentVLHGC* env, MM_MemorySubSpace* subspace, UDATA size,
...
void MM_GlobalMarkDelegate::performMarkSetInitialState(MM_EnvironmentVLHGC* env)
{
...
void MM_GlobalMarkDelegate::performMarkInit(MM_EnvironmentVLHGC* env)
{
...
void MM_GlobalMarkDelegate::performMarkForGlobalGC(MM_EnvironmentVLHGC* env)
{
    static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._globalMarkIncrementType
        = MM_VLHGCIncrementStats::mark_global_collection;
 ...
 *
 * 			  Operations like "Mark Roots" or "Cleaning Cards" might be joint together with followed "Process
 *Work Packets" in one state
 *
 *
 * 				timeout
 * 		Note: ===========>	This is exit on timeout and continue an another incremental step (timeout
 *detected)
 *
 ...
 *:-----------------------: | | V V
 * 		:-----------------------:						:-----------------------:
 *timeout :	  Final Complete	:						:		Clean Cards
 *:====================================
 * 		:-----------------------:						:  Process Work Packets :
 *|		=================
 * 					|									:-----------------------:
 ...
 *:				:		 Final Mark Roots		: :
 *:				:								: :	Final Process
 *Work Packets	:				:		Final Cleaning Cards	: :
 *:				:								: :		Final
 *Complete			:				:	Final Process Work Packets	:
 * 													:-------------------------------:
 *:	                      		:
 * 																	|
 *:		  Final Complete		: |								:
 ...
 *
 */
bool MM_GlobalMarkDelegate::performMarkIncremental(MM_EnvironmentVLHGC* env, I_64 markIncrementEndTime)
{
    ...
    static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._globalMarkIncrementType
        = MM_VLHGCIncrementStats::mark_incremental;

    while (!timeout) {
        switch (cycleState->_markDelegateState) {
        case MM_CycleState::state_mark_map_init: {
            Trc_MM_GlobalMarkDelegate_performMarkIncremental_State(
                env->getLanguageVMThread(), "state_mark_map_init", MM_CycleState::state_mark_map_init);
        ...
        case MM_CycleState::state_initial_mark_roots: {
            Trc_MM_GlobalMarkDelegate_performMarkIncremental_State(
                env->getLanguageVMThread(), "state_initial_mark_roots", MM_CycleState::state_initial_mark_roots);
            ...
            if (timeout) {
                ...
                if (timeout) {
                    ...
                    /* even if no timeout occurred act as if one did so that final root processing runs in a separate
                     * increment */
                    timeout = true;
                }
            }
        ...
        case MM_CycleState::state_process_work_packets_after_initial_mark: {
            Trc_MM_GlobalMarkDelegate_performMarkIncremental_State(env->getLanguageVMThread(),
        ...
        case MM_CycleState::state_final_roots_complete: {
            Trc_MM_GlobalMarkDelegate_performMarkIncremental_State(
                env->getLanguageVMThread(), "state_final_roots_complete", MM_CycleState::state_final_roots_complete);
        ...
        default:
            Trc_MM_GlobalMarkDelegate_performMarkIncremental_State(
                env->getLanguageVMThread(), "unexpected", cycleState->_markDelegateState);
            Assert_MM_unreachable();
        }
    }
...
{
    Assert_MM_true(
        MM_CycleState::state_process_work_packets_after_initial_mark == env->_cycleState->_markDelegateState);

    static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._globalMarkIncrementType
        = MM_VLHGCIncrementStats::mark_concurrent;

    MM_ConcurrentGlobalMarkTask markTask(
        env, _dispatcher, _markingScheme, MARK_SCAN, totalBytesToScan, forceExit, env->_cycleState);
...
void MM_GlobalMarkDelegate::markAll(MM_EnvironmentVLHGC* env)
{
...
bool MM_GlobalMarkDelegate::markInit(MM_EnvironmentVLHGC* env, I_64 timeThreshold)
{
    _markingScheme->masterSetupForGC(env);
    /* run the mark */
...
void MM_GlobalMarkDelegate::markRoots(MM_EnvironmentVLHGC* env)
{
    MM_ParallelGlobalMarkTask markTask(env, _dispatcher, _markingScheme, MARK_ROOTS, I_64_MAX, env->_cycleState);
    _dispatcher->run(env, &markTask);
}

bool MM_GlobalMarkDelegate::markScan(MM_EnvironmentVLHGC* env, I_64 timeThreshold)
{
...
bool MM_GlobalMarkDelegate::markScrubCardTable(MM_EnvironmentVLHGC* env, I_64 timeThreshold)
{
...
void MM_GlobalMarkDelegate::markComplete(MM_EnvironmentVLHGC* env)
{
...
void MM_GlobalMarkDelegate::postMarkCleanup(MM_EnvironmentVLHGC* env) {}
