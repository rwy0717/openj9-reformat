...
public:
     ...
     * @param totalBytesToScan[in] The number of bytes which must be scanned by the concurrent task before it stops
     * attempting to do more work
     * @param forceExit[in] A "kill switch" which, when set to true, implies that the concurrent operation should
     * terminate (volatile as it is written by another thread)
    ...
    /**
     * Executes mark initialization only and advances the mark state machine to the root step.  The caller must ensure
     * that the mark state machine is set to the initial state, prior to calling this method (otherwise, it will
     * assert).
     *
     * @param env[in] The master GC thread
     */
...
private:
     ...
     *	@param timeThreshold[in] The scheduled end time by which the operation is expected to end, expressed in system
     *milliseconds
