...
void MM_ParallelGlobalMarkTask::masterSetup(MM_EnvironmentBase* env)
{
...
void MM_ParallelGlobalMarkTask::masterCleanup(MM_EnvironmentBase* envBase)
{
...
void MM_ParallelGlobalMarkTask::run(MM_EnvironmentBase* envBase)
{
    MM_EnvironmentVLHGC* env = MM_EnvironmentVLHGC::getEnvironment(envBase);
...
void MM_ParallelGlobalMarkTask::setup(MM_EnvironmentBase* envBase)
{
    MM_EnvironmentVLHGC* env = MM_EnvironmentVLHGC::getEnvironment(envBase);
...
void MM_ParallelGlobalMarkTask::cleanup(MM_EnvironmentBase* envBase)
{
    MM_EnvironmentVLHGC* env = MM_EnvironmentVLHGC::getEnvironment(envBase);
     ...
     * -Xtgc:parallel */
    Trc_MM_ParallelGlobalMarkTask_parallelStats(env->getLanguageVMThread(), (U_32)env->getSlaveID(),
...
bool MM_ParallelGlobalMarkTask::shouldYieldFromTask(MM_EnvironmentBase* env)
{
    if (!_timeLimitWasHit) {
        PORT_ACCESS_FROM_ENVIRONMENT(env);
...
void MM_ParallelGlobalMarkTask::synchronizeGCThreads(MM_EnvironmentBase* envBase, const char* id)
{
    MM_EnvironmentVLHGC* env = MM_EnvironmentVLHGC::getEnvironment(envBase);
    PORT_ACCESS_FROM_ENVIRONMENT(env);
...
bool MM_ParallelGlobalMarkTask::synchronizeGCThreadsAndReleaseMaster(MM_EnvironmentBase* envBase, const char* id)
{
    MM_EnvironmentVLHGC* env = MM_EnvironmentVLHGC::getEnvironment(envBase);
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    U_64 startTime = j9time_hires_clock();
...
bool MM_ParallelGlobalMarkTask::synchronizeGCThreadsAndReleaseSingleThread(MM_EnvironmentBase* envBase, const char* id)
{
    MM_EnvironmentVLHGC* env = MM_EnvironmentVLHGC::getEnvironment(envBase);
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    U_64 startTime = j9time_hires_clock();
 ...
 */
MM_GlobalMarkingScheme* MM_GlobalMarkingScheme::newInstance(MM_EnvironmentVLHGC* env)
{
    MM_GlobalMarkingScheme* markingScheme;

    markingScheme = (MM_GlobalMarkingScheme*)env->getForge()->allocate(
        sizeof(MM_GlobalMarkingScheme), MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
    if (markingScheme) {
 ...
 */
void MM_GlobalMarkingScheme::kill(MM_EnvironmentVLHGC* env)
{
    tearDown(env);
 ...
 */
bool MM_GlobalMarkingScheme::initialize(MM_EnvironmentVLHGC* env)
{
 ...
 */
void MM_GlobalMarkingScheme::tearDown(MM_EnvironmentVLHGC* env) {}
 ...
 */
bool MM_GlobalMarkingScheme::heapAddRange(
 ...
 * @return true if operation completes with success (always)
 */
bool MM_GlobalMarkingScheme::heapRemoveRange(MM_EnvironmentVLHGC* env, MM_MemorySubSpace* subspace, UDATA size,
 ...
 ****************************************
 */
void MM_GlobalMarkingScheme::masterSetupForGC(MM_EnvironmentVLHGC* env)
{
...
void MM_GlobalMarkingScheme::masterCleanupAfterGC(MM_EnvironmentVLHGC* env)
{
    _interRegionRememberedSet->setRegionsAsRebuildingComplete(env);
}

void MM_GlobalMarkingScheme::workerSetupForGC(MM_EnvironmentVLHGC* env)
{
...
void MM_GlobalMarkingScheme::setCachedState(MM_MarkMap* markMap, bool dynamicClassUnloadingEnabled)
{
...
void MM_GlobalMarkingScheme::clearCachedState() { _markMap = NULL; }

/****************************************
 * Marking Helpers
 ****************************************
 */
bool MM_GlobalMarkingScheme::isMarked(J9Object* objectPtr)
{
...
MMINLINE bool MM_GlobalMarkingScheme::markObjectNoCheck(MM_EnvironmentVLHGC* env, J9Object* objectPtr, bool leafType)
{
...
MMINLINE bool MM_GlobalMarkingScheme::markObject(MM_EnvironmentVLHGC* env, J9Object* objectPtr, bool leafType)
{
    bool didMark = false;
...
MMINLINE void MM_GlobalMarkingScheme::rememberReferenceIfRequired(
...
MMINLINE void MM_GlobalMarkingScheme::markObjectClass(MM_EnvironmentVLHGC* env, J9Object* objectPtr)
{
#if defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING)
...
MMINLINE void MM_GlobalMarkingScheme::updateScanStats(MM_EnvironmentVLHGC* env, UDATA bytesScanned, ScanReason reason)
{
 ...
 ****************************************
 */
void MM_GlobalMarkingScheme::scanMixedObject(MM_EnvironmentVLHGC* env, J9Object* objectPtr, ScanReason reason)
{
...
void MM_GlobalMarkingScheme::scanReferenceMixedObject(MM_EnvironmentVLHGC* env, J9Object* objectPtr, ScanReason reason)
{
    fj9object_t* endScanPtr;
    UDATA* descriptionPtr;
    UDATA descriptionBits;
...
UDATA
MM_GlobalMarkingScheme::scanPointerArrayObjectSplit(
...
void MM_GlobalMarkingScheme::scanPointerArrayObject(
...
void MM_GlobalMarkingScheme::scanClassObject(MM_EnvironmentVLHGC* env, J9Object* classObject, ScanReason reason)
{
...
void MM_GlobalMarkingScheme::scanClassLoaderObject(
...
void MM_GlobalMarkingScheme::scanClassLoaderSlots(
...
void MM_GlobalMarkingScheme::scanObject(MM_EnvironmentVLHGC* env, J9Object* objectPtr, ScanReason reason)
{
    if (PACKET_INVALID_OBJECT == (UDATA)objectPtr) {
        /* This means that the object was pushed during a GMP cycle but collected by an intervening PGC cycle and
         * invalidated */
        Assert_MM_true(SCAN_REASON_PACKET == reason);
    } else {
 ...
 */
MMINLINE void MM_GlobalMarkingScheme::completeScan(MM_EnvironmentVLHGC* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    do {
...
#if defined(J9VM_GC_FINALIZATION)
void MM_GlobalMarkingScheme::scanUnfinalizedObjects(MM_EnvironmentVLHGC* env)
{
...
void MM_GlobalMarkingScheme::scanOwnableSynchronizerObjects(MM_EnvironmentVLHGC* env)
{
    env->_currentTask->synchronizeGCThreads(env, UNIQUE_ID);
...
void MM_GlobalMarkingScheme::cleanCardTableForGlobalCollect(MM_EnvironmentVLHGC* env, MM_CardCleaner* cardCleaner)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
...
void MM_GlobalMarkingScheme::initializeMarkMap(MM_EnvironmentVLHGC* env)
{
    MM_HeapRegionDescriptorVLHGC* region = NULL;
 ...
 */
void MM_GlobalMarkingScheme::markLiveObjectsInit(MM_EnvironmentVLHGC* env)
{
    workerSetupForGC(env);

    if (MM_CycleState::CT_GLOBAL_GARBAGE_COLLECTION == env->_cycleState->_collectionType) {
        /* clear all card states (since we will run as a global collection but then be stored as the data used by the
         * next PGC) */
 ...
 */
void MM_GlobalMarkingScheme::markLiveObjectsRoots(MM_EnvironmentVLHGC* env)
{
...
void MM_GlobalMarkingScheme::markLiveObjectsScan(MM_EnvironmentVLHGC* env) { completeScan(env); }

/**
 *  Final Mark services including scanning of Clearables
 *  @param[in] env - passed Environment
 */
void MM_GlobalMarkingScheme::markLiveObjectsComplete(MM_EnvironmentVLHGC* env)
{
    ...
    /* If this weren't a GMP, we'd need to deleteDeadObjectsFromExternalCycle here, but since it is there can't be an
     * external cycle state */
    Assert_MM_true(NULL == env->_cycleState->_externalCycleState);
}

void MM_GlobalMarkingScheme::resolveOverflow(MM_EnvironmentVLHGC* env)
{
    do {
...
bool MM_GlobalMarkingScheme::handleOverflow(MM_EnvironmentVLHGC* env)
{
...
void MM_GlobalMarkingScheme::cleanRegion(
...
void MM_GlobalMarkingScheme::scanObjectsInRange(MM_EnvironmentVLHGC* env, void* lowAddress, void* highAddress)
{
...
void MM_GlobalMarkingScheme::scanWeakReferenceObjects(MM_EnvironmentVLHGC* env)
{
...
void MM_GlobalMarkingScheme::scanSoftReferenceObjects(MM_EnvironmentVLHGC* env)
{
    Assert_MM_true(env->getGCEnvironment()->_referenceObjectBuffer->isEmpty());
...
void MM_GlobalMarkingScheme::scanPhantomReferenceObjects(MM_EnvironmentVLHGC* env)
{
...
void MM_GlobalMarkingScheme::processReferenceList(
    MM_EnvironmentVLHGC* env, J9Object* headOfList, MM_ReferenceStats* referenceStats)
{
    ...
    while (NULL != referenceObj) {
        ...
        Assert_MM_true(GC_ObjectModel::REF_STATE_REMEMBERED
            != J9GC_J9VMJAVALANGREFERENCE_STATE(
                   env, referenceObj)); /* we should only see REMEMBERED references in partial collections */
        ...
        if (NULL != referent) {
            ...
            if (isMarked(referent)) {
                ...
                Assert_MM_true(
                    (GC_ObjectModel::REF_STATE_INITIAL == J9GC_J9VMJAVALANGREFERENCE_STATE(env, referenceObj))
                    || (GC_ObjectModel::REF_STATE_REMEMBERED == J9GC_J9VMJAVALANGREFERENCE_STATE(env, referenceObj)));
...
void MM_GlobalMarkingScheme::flushBuffers(MM_EnvironmentVLHGC* env)
{
    env->_workStack.flush(env);
    env->getGCEnvironment()->_referenceObjectBuffer->flush(env);
}

bool MM_ConcurrentGlobalMarkTask::shouldYieldFromTask(MM_EnvironmentBase* envBase)
{
...
void MM_ConcurrentGlobalMarkTask::setup(MM_EnvironmentBase* envBase)
{
    MM_ParallelGlobalMarkTask::setup(envBase);

    /* initialize _previousConcurrentYieldCheckBytesScanned since we need it to check the change in the scanned stats
     * between shouldYieldFromTask calls */
    MM_EnvironmentVLHGC* env = MM_EnvironmentVLHGC::getEnvironment(envBase);
    env->_previousConcurrentYieldCheckBytesScanned = env->_markVLHGCStats._bytesScanned;
}

void MM_ConcurrentGlobalMarkTask::cleanup(MM_EnvironmentBase* envBase)
{
    MM_EnvironmentVLHGC* env = MM_EnvironmentVLHGC::getEnvironment(envBase);
