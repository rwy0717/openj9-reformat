...
enum MarkAction { MARK_ALL = 1, MARK_INIT, MARK_ROOTS, MARK_SCAN, MARK_COMPLETE };
...
public:
     ...
     */
    MM_ParallelGlobalMarkTask(MM_EnvironmentBase* env, MM_Dispatcher* dispatcher, MM_GlobalMarkingScheme* markingScheme,
        MarkAction action, I_64 timeThreshold, MM_CycleState* cycleState)
...
private:
    ...
    bool _didReturnEarly; /**< True if any thread in this task returned before the mark phase completed (either due to a
                             scanning limit, a timing limit, or an external termination request) */
    volatile bool* const
        _forceExit; /**< Shared state concurrently updated by an external thread to force the receiver to cause all
                       threads to yield (by setting the destination of the pointer to true) */
protected:
public:
    /* Member Functions */
private:
protected:
public:
    virtual UDATA getVMStateID() { return OMRVMSTATE_GC_CONCURRENT_MARK_TRACE; }

    UDATA getBytesScanned() { return _bytesScanned; }
    bool didReturnEarly() { return _didReturnEarly; }
    virtual void setup(MM_EnvironmentBase* env);
    virtual void cleanup(MM_EnvironmentBase* env);
    virtual bool shouldYieldFromTask(MM_EnvironmentBase* env);
     ...
     */
    MM_ConcurrentGlobalMarkTask(MM_EnvironmentBase* env, MM_Dispatcher* dispatcher,
        MM_GlobalMarkingScheme* markingScheme, MarkAction action, UDATA bytesToScan, volatile bool* forceExit,
...
private:
    ...
    MM_HeapRegionManager* const _heapRegionManager; /**< This HRM is retained since it is needed to quickly resolve the
                                                       table region of a given object */
    ...
    const UDATA _regionSize; /**< Cached copy of the region size used for short-circuiting region matching checks with
                                the XOR-and-compare */
...
private:
    ...
    /**
     * Update scan stats: object and byte counts, separately for each of reasons. Object count is just incremented by 1,
     * and byte count is incremented by provided byte count.
     * @param env[in] the current thread
     * @param bytesScanned[in] count of bytes scanned (typically object size, but could be less in case of split array
     * scanning)
     ...
     *
     * Should only be used by J9ClassLoaders which may not have a java.lang.ClassLoader object (system and application).
     * Everyone else should go through scanClassLoaderObjectSlots().
     * @param env current GC thread.
     * @param objectPtr current object being scanned.
     * @param reason a code indicating why the object is being scanned
     */
    void scanClassLoaderSlots(
        MM_EnvironmentVLHGC* env, J9ClassLoader* classLoaderObject, ScanReason reason = SCAN_REASON_PACKET);
     ...
     * @param reason a code indicating why the object is being scanned
     */
    UDATA scanPointerArrayObjectSplit(
        MM_EnvironmentVLHGC* env, J9IndexableObject* objectPtr, UDATA indexArg, ScanReason reason);
