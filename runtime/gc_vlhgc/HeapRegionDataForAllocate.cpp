...
bool MM_HeapRegionDataForAllocate::initialize(
...
void MM_HeapRegionDataForAllocate::tearDown(MM_EnvironmentVLHGC* env)
{
...
bool MM_HeapRegionDataForAllocate::taskAsMemoryPoolBumpPointer(
    MM_EnvironmentBase* env, MM_AllocationContextTarok* context)
{
    MM_IncrementalGenerationalGC* globalCollector
...
void MM_HeapRegionDataForAllocate::taskAsFreePool(MM_EnvironmentBase* env)
{
#if defined(J9VM_GC_ARRAYLETS)
...
void MM_HeapRegionDataForAllocate::taskAsIdlePool(MM_EnvironmentVLHGC* env)
{
    Assert_MM_true((MM_HeapRegionDescriptor::BUMP_ALLOCATED == _region->getRegionType())
        || (MM_HeapRegionDescriptor::BUMP_ALLOCATED_MARKED == _region->getRegionType()));
...
#if defined(J9VM_GC_ARRAYLETS)
void MM_HeapRegionDataForAllocate::taskAsArrayletLeaf(MM_EnvironmentBase* env)
{
    Assert_MM_true(NULL == _nextArrayletLeafRegion);
    Assert_MM_true(NULL == _previousArrayletLeafRegion);
 ...
 */
void MM_HeapRegionDataForAllocate::removeFromArrayletLeafList()
{
 ...
 */
void MM_HeapRegionDataForAllocate::addToArrayletLeafList(MM_HeapRegionDescriptorVLHGC* newSpineRegion)
{
    Assert_MM_true(_region->isArrayletLeaf());
...
void MM_HeapRegionDataForAllocate::setSpine(J9IndexableObject* spineObject)
{
    Assert_MM_true(_region->isArrayletLeaf());
    _spine = spineObject;
}
...
bool MM_HeapRegionDataForAllocate::participatesInCompaction() { return _region->containsObjects(); }
