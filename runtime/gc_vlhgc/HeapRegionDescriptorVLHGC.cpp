...
MM_HeapRegionDescriptorVLHGC::MM_HeapRegionDescriptorVLHGC(
    MM_EnvironmentVLHGC* env, void* lowAddress, void* highAddress)
...
bool MM_HeapRegionDescriptorVLHGC::initialize(MM_EnvironmentBase* env, MM_HeapRegionManager* regionManager)
{
...
void MM_HeapRegionDescriptorVLHGC::tearDown(MM_EnvironmentBase* env)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
...
bool MM_HeapRegionDescriptorVLHGC::initializer(MM_EnvironmentBase* env, MM_HeapRegionManager* regionManager,
    MM_HeapRegionDescriptor* descriptor, void* lowAddress, void* highAddress)
{
    new ((MM_HeapRegionDescriptorVLHGC*)descriptor)
        MM_HeapRegionDescriptorVLHGC((MM_EnvironmentVLHGC*)env, lowAddress, highAddress);
    return ((MM_HeapRegionDescriptorVLHGC*)descriptor)->initialize(env, regionManager);
}

void MM_HeapRegionDescriptorVLHGC::destructor(
...
bool MM_HeapRegionDescriptorVLHGC::allocateSupportingResources(MM_EnvironmentBase* env)
{
    return MM_GCExtensions::getExtensions(env)->interRegionRememberedSet->allocateRegionBuffers(
        (MM_EnvironmentVLHGC*)env, this);
}
...
void MM_HeapRegionDescriptorVLHGC::resetAge(MM_EnvironmentVLHGC* env, U_64 allocationAge)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
    UDATA logicalAge = 0;
