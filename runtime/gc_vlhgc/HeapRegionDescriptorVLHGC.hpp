...
public:
    ...
    enum RefersToCollectionSet {
        ...
        overflowedRSCardListRegion /**< This region does refer to Collection Set, but specifically to a region with
                                      overflowed RS Card List */
    };
    struct {
        ...
        bool _noEvacuation; /**< true if the region is set that do not copyforward, it is valid if _shouldMark is true.
                             */
        UDATA _dynamicMarkCost; /**< The cost of marking this region (that is, the number of other regions (not
                                   including this one) which will need to be scanned - this value is dynamic in that
                                   converting the regions which refer to it to scan or mark reduces this number.  It is
                                   only valid during GC setup */
        U_8 _overflowFlags; /**< Used to denote that work packet overflow occurred for an object in this region - bits
                               0x1 is GMP or global while 0x2 is PGC */
    } _markData;
    struct {
        bool _shouldReclaim; /**< true if the collector is to sweep and/or compact this region during the collection
                                cycle */
    } _reclaimData;
    struct {
        bool _alreadySwept; /**< true if the collector has already swept this region during the last collection
                               increment */
        UDATA _lastGCNumber; /**< initially 0 but set to the GC's collection ID every time it is swept so that the GC
                                can ensure it doesn't over-collect the same set */
    } _sweepData;
    struct {
        bool _initialLiveSet; /**< true if the region was part of the live set at the start of collection */
        bool _survivorSetAborted; /**< true if the region was part of the survivor set at the time that an abort had
                                     occurred, false otherwise */
        bool _evacuateSet; /**< true if the region was part of the evacuate set at the beginning of the copy-forward
                              (flag not changed during abort) */
        bool _requiresPhantomReferenceProcessing; /**< Set to true by master thread if this region must be processed
                                                     during parallel phantom reference processing */
        volatile void* _survivorBase; /**< The base pointer for storage used as survivor, which will NOT match the
                                         region base if tail filling has occurred */
        MM_HeapRegionDescriptorVLHGC*
            _nextRegion; /**< Region list link for compact group resource management during a copyforward operation */
        MM_HeapRegionDescriptorVLHGC* _previousRegion; /**< Region list link for compact group resource management
                                                          during a copyforward operation */
    } _copyForwardData;
#if defined(J9VM_GC_MODRON_COMPACTION)
    MM_HeapRegionDataForCompactVLHGC _compactData;
#endif /* defined (J9VM_GC_MODRON_COMPACTION) */
    /* this is only of interest for collectors who can move objects so we ifdef it out, in other cases, to prove that no
     * other specs are absorbing the cost of looking up regions and tracking critical counts */
    UDATA volatile _criticalRegionsInUse; /**< The number of JNI critical regions which are currently in use within the
                                             receiver's address range (typically 0).  This value can be modified
                                             concurrently so atomics are required */

    bool _previousMarkMapCleared; /**< if true, previous mark map is guarantied to be cleared; if false it like is not
                                     cleared */
    bool
        _nextMarkMapCleared; /**< if true, next mark map is guarantied to be cleared; if false it like is not cleared */
    UDATA _projectedLiveBytes; /**< number of bytes in the region projected to be live at the beginning of the current
                                  GC. UDATA_MAX has a special meaning of 'uninitialized'*/
    UDATA _projectedLiveBytesPreviousPGC; /**< _projectedLiveBytes value from previous PGC; updated just before we apply
                                             decay for this PGC */
    IDATA _projectedLiveBytesDeviation; /**< difference between actual live bytes and projected live bytes. Note: not
                                           always update to date and can be negative. */
    MM_HeapRegionDescriptorVLHGC*
        _compactDestinationQueueNext; /**< pointer to next compact destination region in the queue */
    bool _defragmentationTarget; /**< indicates whether this region should be considered for defragmentation, currently
                                    this means the region has been GMPed but not collected yet */
...
private:
    ...
    double _allocationAgeSizeProduct; /**< sum of (age * size) products for each object in the region. used for age
                                         merging math in surivovor regions */
    ...
    MM_RememberedSetCard*
        _rsclBufferPool; /**< RSCL Buffer pool owned by this region (Buffers can still be shared among other regions) */

    MM_HeapRegionDescriptorVLHGC* _dynamicSelectionNext; /**< Linked list pointer used during dynamic set selection
                                                            (NOTE: Valid only during dynamic set selection) */
    ...
    MM_OwnableSynchronizerObjectList
        _ownableSynchronizerObjectList; /**< A list of ownable synchronizer objects in this region */
    MM_ReferenceObjectList
        _referenceObjectList; /**< A list of reference objects (i.e. weak/soft/phantom) in this region */
...
public:
     ...
     */
    MMINLINE double getAllocationAgeSizeProduct() { return _allocationAgeSizeProduct; }
     ...
     */
    bool isArrayletLeaf() { return (ARRAYLET_LEAF == getRegionType()); }
    ...
    /**
     * @return True if the receiver was a tail-filled survivor region (that is, was not freshly allocated from a free
     * region) in the copy-forward which just finished
     */
    ...
    /**
     * @return True if region is in survivor set (there is no explict flag, but info is inferred from _survivorBase
     * being non-null
     */
     ...
     * when region is being committed. For VLHGC region, the resource is RSCL Buffer pool
