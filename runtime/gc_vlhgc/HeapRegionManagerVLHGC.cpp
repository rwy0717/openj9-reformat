...
MM_HeapRegionManagerVLHGC::MM_HeapRegionManagerVLHGC(MM_EnvironmentBase* env, UDATA regionSize,
    UDATA tableDescriptorSize, MM_RegionDescriptorInitializer regionDescriptorInitializer,
...
MM_HeapRegionManagerVLHGC* MM_HeapRegionManagerVLHGC::newInstance(MM_EnvironmentBase* env, UDATA regionSize,
    UDATA tableDescriptorSize, MM_RegionDescriptorInitializer regionDescriptorInitializer,
    MM_RegionDescriptorDestructor regionDescriptorDestructor)
{
    MM_HeapRegionManagerVLHGC* regionManager = (MM_HeapRegionManagerVLHGC*)env->getForge()->allocate(
        sizeof(MM_HeapRegionManagerVLHGC), MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
    if (NULL != regionManager) {
        new (regionManager) MM_HeapRegionManagerVLHGC(
            env, regionSize, tableDescriptorSize, regionDescriptorInitializer, regionDescriptorDestructor);
...
bool MM_HeapRegionManagerVLHGC::initialize(MM_EnvironmentBase* env)
{
    return MM_HeapRegionManagerTarok::initialize(env);
}

void MM_HeapRegionManagerVLHGC::tearDown(MM_EnvironmentBase* env) { MM_HeapRegionManagerTarok::tearDown(env); }

bool MM_HeapRegionManagerVLHGC::enableRegionsInTable(MM_EnvironmentBase* env, MM_MemoryHandle* handle)
{
    bool result = true;
    ...
    /* ask for the number of affinity leaders since that will ensure that is how we will logically split up the regions
     * - we only use physical details for binding */
    UDATA nodeCount = 0;
            ...
            if (0 < thisNodeSize) {
                ...
                if (hasPhysicalNUMASupport) {
                    ...
                    if (result) {
                        ...
                        result = cardTable->setNumaAffinityCorrespondingToHeapRange(
                            env, nodeToBind, (void*)heapAddress, topOfExtent);
                    }
                } else {
                    result = true;
                }
...
MM_HeapMemorySnapshot* MM_HeapRegionManagerVLHGC::getHeapMemorySnapshot(
    MM_GCExtensionsBase* extensions, MM_HeapMemorySnapshot* snapshot, bool gcEnd)
{
    ...
    while (NULL != (region = regionIterator.nextRegion())) {
        if (region->isFreeOrIdle()) {
             ...
             * which is triggered by gcEnd event, so at gcEnd, counts any of regions with logic age = 0 as survivor
             * regions.
    ...
    /* there would be a case that mutators use more than assigned regions, correct totalRegionEdenSize to avoid
