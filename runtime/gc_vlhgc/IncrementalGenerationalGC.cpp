...
MM_IncrementalGenerationalGC* MM_IncrementalGenerationalGC::newInstance(
    MM_EnvironmentVLHGC* env, MM_HeapRegionManager* manager)
{
    MM_IncrementalGenerationalGC* globalGC;

    globalGC = (MM_IncrementalGenerationalGC*)env->getForge()->allocate(
        sizeof(MM_IncrementalGenerationalGC), MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
    if (globalGC) {
...
void MM_IncrementalGenerationalGC::kill(MM_EnvironmentBase* env)
{
 ...
 */
bool MM_IncrementalGenerationalGC::initialize(MM_EnvironmentVLHGC* env)
{
    ...
    if (NULL
        == (_interRegionRememberedSet = MM_InterRegionRememberedSet::newInstance(env, extensions->heapRegionManager))) {
        goto error_no_memory;
    }
    ...
    if (NULL
        == (_workPacketsForPartialGC
               = MM_WorkPacketsVLHGC::newInstance(env, MM_CycleState::CT_PARTIAL_GARBAGE_COLLECTION))) {
        goto error_no_memory;
    }

    if (NULL
        == (_workPacketsForGlobalGC
               = MM_WorkPacketsVLHGC::newInstance(env, MM_CycleState::CT_GLOBAL_GARBAGE_COLLECTION))) {
        goto error_no_memory;
    }
    ...
    if (0 == extensions->tarokMaximumAgeInBytes) {
        extensions->tarokMaximumAgeInBytes
            = MM_CompactGroupManager::calculateMaximumAllocationAge(env, extensions->tarokRegionMaxAge);
    } else {
        /*
        ...
        if (extensions->tarokAllocationAgeEnabled) {
            UDATA maxLogicalAge
                = MM_CompactGroupManager::calculateLogicalAgeForRegion(env, extensions->tarokMaximumAgeInBytes);
            /*
             * There are a few data structures have been allocated already with size based on tarokRegionMaxAge(in TGC
             * for example) so new value can not be larger then old one
             */
    ...
    /*
     * Set threshold for Nursery collection set for allocation-based aging system here as a doubled estimated "ideal"
     * taxation interval except it has not been hard coded in command line
     */
    ...
    /*
     * For Allocation-based aging system take tarokMaximumNurseryAgeInBytes as a primary value and set
     * tarokNurseryMaxAge
     */
    if (extensions->tarokAllocationAgeEnabled) {
        extensions->tarokNurseryMaxAge._valueSpecified
            = MM_CompactGroupManager::calculateLogicalAgeForRegion(env, extensions->tarokMaximumNurseryAgeInBytes);
    }
    ...
    (*mmPrivateHooks)
        ->J9HookRegisterWithCallSite(mmPrivateHooks, J9HOOK_MM_PRIVATE_TAROK_INCREMENT_START,
            globalGCHookIncrementStart, OMR_GET_CALLSITE(), NULL);
    (*mmPrivateHooks)
        ->J9HookRegisterWithCallSite(
            mmPrivateHooks, J9HOOK_MM_PRIVATE_TAROK_INCREMENT_END, globalGCHookIncrementEnd, OMR_GET_CALLSITE(), NULL);
 ...
 */
void MM_IncrementalGenerationalGC::tearDown(MM_EnvironmentVLHGC* env)
{
 ...
 ****************************************
 */
void MM_IncrementalGenerationalGC::setupBeforeGC(MM_EnvironmentBase* env)
{
#if defined(J9VM_GC_FINALIZATION)
    /* this should not be set by the GC since it is used by components in order to record that they performed some
     * operation which will require that we do some finalization */
...
void MM_IncrementalGenerationalGC::masterThreadGarbageCollect(
...
void MM_IncrementalGenerationalGC::globalMarkPhase(MM_EnvironmentVLHGC* env, bool incrementalMark)
{
    ...
    static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._markStats._startTime
        = j9time_hires_clock();
    ...
    if (incrementalMark) {
        ...
        Assert_MM_true(
            (incrementalMarkIsComplete) == (MM_CycleState::state_mark_idle == env->_cycleState->_markDelegateState));
    } else {
        reportGlobalGCMarkStart(env);
        /* a global collect will enter with a different cycle state instance than the _persistentGlobalMarkPhaseState so
         * we need to copy the _markDelegateState out and back.
         */
    ...
    _persistentGlobalMarkPhaseState._vlhgcCycleStats.merge(
        &static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats);
 ...
 ****************************************
 */
void MM_IncrementalGenerationalGC::internalPreCollect(
    MM_EnvironmentBase* env, MM_MemorySubSpace* subSpace, MM_AllocateDescription* allocDescription, U_32 gcCode)
{
    ...
    if (NULL == env->_cycleState) {
        ...
        Assert_MM_true((env->_cycleState->_collectionType == MM_CycleState::CT_PARTIAL_GARBAGE_COLLECTION)
...
void MM_IncrementalGenerationalGC::internalPostCollect(MM_EnvironmentBase* env, MM_MemorySubSpace* subSpace)
{
    MM_GlobalCollector::internalPostCollect(env, subSpace);
}

void MM_IncrementalGenerationalGC::setupForGC(MM_EnvironmentBase* env) {}

bool MM_IncrementalGenerationalGC::internalGarbageCollect(
    MM_EnvironmentBase* env, MM_MemorySubSpace* subSpace, MM_AllocateDescription* allocDescription)
{
    ...
    /* we make the decision to treat soft refs as weak once we know what kind of cycle this is to be.  If it is an OOM,
     * we need to clear all possible soft refs in the GC before we throw this exception */
    ...
    bool didAttemptCollect
        = _masterGCThread.garbageCollect(envVLHGC, static_cast<MM_AllocateDescription*>(allocDescription));
 ...
 */
bool MM_IncrementalGenerationalGC::heapAddRange(
            ...
            if (result) {
                ...
                if (result) {
                    if (NULL != _extensions->referenceChainWalkerMarkMap) {
                        result = _extensions->referenceChainWalkerMarkMap->heapAddRange(
                            envVLHGC, size, lowAddress, highAddress);
                        if (!result) {
                            /* Reference Chain Walker Mark Map expansion has failed
                             * Mark Map Manager, Global Mark Delegate, Partial Mark Delegate and Reclaim Delegate
                             * expansions must be reversed
                             */
                            _reclaimDelegate.heapRemoveRange(
                                envVLHGC, subspace, size, lowAddress, highAddress, NULL, NULL);
                            _partialMarkDelegate.heapRemoveRange(
                                envVLHGC, subspace, size, lowAddress, highAddress, NULL, NULL);
                            _globalMarkDelegate.heapRemoveRange(
                                envVLHGC, subspace, size, lowAddress, highAddress, NULL, NULL);
                            _markMapManager->heapRemoveRange(
                                envVLHGC, subspace, size, lowAddress, highAddress, NULL, NULL);
                        }
                    }
                } else {
 ...
 */
bool MM_IncrementalGenerationalGC::heapRemoveRange(MM_EnvironmentBase* env, MM_MemorySubSpace* subspace, UDATA size,
    void* lowAddress, void* highAddress, void* lowValidAddress, void* highValidAddress)
{
    MM_EnvironmentVLHGC* envVLHGC = MM_EnvironmentVLHGC::getEnvironment(env);

    bool result = _markMapManager->heapRemoveRange(
        envVLHGC, subspace, size, lowAddress, highAddress, lowValidAddress, highValidAddress);
    ...
    if (NULL != _extensions->referenceChainWalkerMarkMap) {
        result = result
            && _extensions->referenceChainWalkerMarkMap->heapRemoveRange(
                   envVLHGC, size, lowAddress, highAddress, lowValidAddress, highValidAddress);
    }
    return result;
}
 ...
 */
void MM_IncrementalGenerationalGC::heapReconfigured(MM_EnvironmentBase* env)
{
    MM_EnvironmentVLHGC* envVLHGC = MM_EnvironmentVLHGC::getEnvironment(env);
...
void MM_IncrementalGenerationalGC::preMasterGCThreadInitialize(MM_EnvironmentBase* envBase)
{
    MM_EnvironmentVLHGC* env = MM_EnvironmentVLHGC::getEnvironment(envBase);
...
void MM_IncrementalGenerationalGC::initializeTaxationThreshold(MM_EnvironmentVLHGC* env)
{
    /**
     * This may be called from either real Master GC thread or acting Master GC thread (any thread that caused GC in
     * absence of real Master GC thread). taxationThreshold will be initialized only once.
     */
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);

    /* we need to calculate the taxation threshold after the initial heap inflation, which happens before
     * collectorStartup */
    ...
    if (_taxationThreshold < minimumKickOff) {
        /* first taxation can't happen before any of the heap has been allocated so allocate at least 2 regions, first
         * (note that this number is purely a placeholder until a less arbitrary value is determined) */
...
bool MM_IncrementalGenerationalGC::collectorStartup(MM_GCExtensionsBase* extensions)
{
    /*
     * WARNING:  GCs can occur before this function is run!  Any initialization which a GC MUST rely on should be in
     * initialize.
     */
    /*
...
void MM_IncrementalGenerationalGC::collectorShutdown(MM_GCExtensionsBase* extensions) { _masterGCThread.shutdown(); }

/**
 * Determine if a  expand is required
 * @return expand size if rator expand required or 0 otheriwse
 */
U_32 MM_IncrementalGenerationalGC::getGCTimePercentage(MM_EnvironmentBase* env)
{
...
void MM_IncrementalGenerationalGC::globalGCHookSysStart(
...
void MM_IncrementalGenerationalGC::globalGCHookSysEnd(
    J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
...
void MM_IncrementalGenerationalGC::globalGCHookAFCycleStart(
    J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    Trc_MM_IncrementalGenerationalGC_globalGCHookAFStart(
        event->currentThread->_language_vmthread, extensions->globalVLHGCStats.gcCount);
...
void MM_IncrementalGenerationalGC::globalGCHookAFCycleEnd(
    J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    Trc_MM_IncrementalGenerationalGC_globalGCHookAFEnd(
        event->currentThread->_language_vmthread, extensions->globalVLHGCStats.gcCount);
...
void MM_IncrementalGenerationalGC::globalGCHookIncrementStart(
    J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    Trc_MM_IncrementalGenerationalGC_globalGCHookIncrementStart(
        event->currentThread->_language_vmthread, extensions->globalVLHGCStats.gcCount);
...
void MM_IncrementalGenerationalGC::globalGCHookIncrementEnd(
    J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    Trc_MM_IncrementalGenerationalGC_globalGCHookIncrementEnd(
        event->currentThread->_language_vmthread, extensions->globalVLHGCStats.gcCount);
 ...
 */
void* MM_IncrementalGenerationalGC::createSweepPoolState(MM_EnvironmentBase* env, MM_MemoryPool* memoryPool)
{
 ...
 */
void MM_IncrementalGenerationalGC::deleteSweepPoolState(MM_EnvironmentBase* env, void* sweepPoolState)
{
    _reclaimDelegate.deleteSweepPoolState(env, sweepPoolState);
}

void MM_IncrementalGenerationalGC::taxationEntryPoint(
    MM_EnvironmentBase* envModron, MM_MemorySubSpace* subspace, MM_AllocateDescription* allocDescription)
{
    ...
    if (J9_EVENT_IS_HOOKED(_extensions->privateHookInterface, J9HOOK_MM_PRIVATE_TAROK_INCREMENT_START)) {
        ...
        ALWAYS_TRIGGER_J9HOOK_MM_PRIVATE_TAROK_INCREMENT_START(_extensions->privateHookInterface, env->getOmrVMThread(),
            j9time_hires_clock(), J9HOOK_MM_PRIVATE_TAROK_INCREMENT_START, _extensions->globalVLHGCStats.incrementCount,
            &commonData, _taxationThreshold);
    }
    ...
    if (J9_EVENT_IS_HOOKED(_extensions->privateHookInterface, J9HOOK_MM_PRIVATE_TAROK_INCREMENT_END)) {
        ...
        ALWAYS_TRIGGER_J9HOOK_MM_PRIVATE_TAROK_INCREMENT_END(_extensions->privateHookInterface, env->getOmrVMThread(),
            j9time_hires_clock(), J9HOOK_MM_PRIVATE_TAROK_INCREMENT_END, env->getExclusiveAccessTime(), &commonData);
    }
}

void MM_IncrementalGenerationalGC::runPartialGarbageCollect(
    MM_EnvironmentVLHGC* env, MM_AllocateDescription* allocDescription)
{
...
void MM_IncrementalGenerationalGC::runGlobalMarkPhaseIncrement(MM_EnvironmentVLHGC* env)
{
    /*
     * Collection preparation work including cache flushing and stats reporting.
     */
    ...
    if ((_globalMarkPhaseIncrementBytesStillToScan > 0)
        || (MM_CycleState::state_process_work_packets_after_initial_mark
               != _persistentGlobalMarkPhaseState._markDelegateState)) {
    ...
    if (isGlobalMarkPhaseRunning()) {
        ...
        if (J9_EVENT_IS_HOOKED(_extensions->omrHookInterface, J9HOOK_MM_OMR_OBJECT_DELETE)) {
            _markMapManager->reportDeletedObjects(
                env, _markMapManager->getPartialGCMap(), _markMapManager->getGlobalMarkPhaseMap());
        }
...
void MM_IncrementalGenerationalGC::runGlobalGarbageCollection(
    MM_EnvironmentVLHGC* env, MM_AllocateDescription* allocDescription)
{
    /* If a GMP is already running, then the cycle will be commandeered.  Otherwise, start a new cycle representing the
     * global collection.  */
    if (isGlobalMarkPhaseRunning()) {
    ...
    UDATA compactSelectionGoalInBytes = 0;
    {
        ...
        _reclaimDelegate.runReclaimCompleteSweep(
            env, allocDescription, env->_cycleState->_activeSubSpace, env->_cycleState->_gcCode);
        _reclaimDelegate.runReclaimCompleteCompact(env, allocDescription, env->_cycleState->_activeSubSpace,
            env->_cycleState->_gcCode, _markMapManager->getGlobalMarkPhaseMap(), compactSelectionGoalInBytes);
    ...
    _reclaimDelegate.estimateReclaimableRegions(env, _schedulingDelegate.getAverageEmptinessOfCopyForwardedRegions(),
        &reclaimableRegions, &defragmentReclaimableRegions);
...
void MM_IncrementalGenerationalGC::reportGCCycleFinalIncrementEnding(MM_EnvironmentBase* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
    MM_CommonGCData commonData;
    TRIGGER_J9HOOK_MM_OMR_GC_CYCLE_END(extensions->omrHookInterface, env->getOmrVMThread(), j9time_hires_clock(),
        J9HOOK_MM_OMR_GC_CYCLE_END, extensions->getHeap()->initializeCommonGCData(env, &commonData),
        env->_cycleState->_type, omrgc_condYieldFromGC);
}

void MM_IncrementalGenerationalGC::reportGCStart(MM_EnvironmentBase* env)
{
    /* this function is deprecated in IGGC since we need to report more specific events for PGC, GMP, and global */
    Assert_MM_unreachable();
}

void MM_IncrementalGenerationalGC::reportGCEnd(MM_EnvironmentBase* env)
{
    /* this function is deprecated in IGGC since we need to report more specific events for PGC, GMP, and global */
    Assert_MM_unreachable();
}

void MM_IncrementalGenerationalGC::flushRememberedSetIntoCardTable(MM_EnvironmentVLHGC* env)
{
...
bool MM_IncrementalGenerationalGC::attemptHeapResize(MM_EnvironmentVLHGC* env, MM_AllocateDescription* allocDescription)
{
...
void MM_IncrementalGenerationalGC::partialGarbageCollect(
    MM_EnvironmentVLHGC* env, MM_AllocateDescription* allocDescription)
{
    ...
    if (_schedulingDelegate.isGlobalSweepRequired()) {
        ...
        _reclaimDelegate.runGlobalSweepBeforePGC(
            env, allocDescription, env->_cycleState->_activeSubSpace, env->_cycleState->_gcCode);

        /* TODO: lpnguyen make another statisticsDelegate or something that both schedulingDelegate and reclaimDelegate
         * can see so that we can avoid this kind of stats-passing mess.
         */
        ...
        double optimalEmptinessRegionThreshold = _reclaimDelegate.calculateOptimalEmptinessRegionThreshold(
            env, regionConsumptionRate, avgSurvivorRegions, avgCopyForwardRate, scanTimeCostPerGMP);
        _schedulingDelegate.setAutomaticDefragmentEmptinessThreshold(optimalEmptinessRegionThreshold);
    }

    /* For Non CopyForwardHybrid mode, we don't allow any eden rgions with jniCritical for copyforward, if there is any,
     * would switch MarkCompact PGC mode
     * For CopyForwardHybrid mode, we do not care about jniCritical eden regions, the eden rgions with jniCritical would
     * be marked instead copyforwarded during collection.*/
        ...
        if (env->_cycleState->_shouldRunCopyForward) {
            ...
            while (env->_cycleState->_shouldRunCopyForward && (NULL != (region = iterator.nextRegion()))) {
                if ((region->_criticalRegionsInUse > 0) && region->isEden()) {
                    /* if we find any Eden regions which have critical regions in use, we need to switch to Mark-Compact
                     * since we have to collect Eden but can't move objects in this region */
                    env->_cycleState->_shouldRunCopyForward = false;
                    /* record this observation in the cycle state so that verbose can see it and produce an appropriate
                     * message */
     ...
     */
    if (env->_cycleState->_shouldRunCopyForward) {
        MM_GlobalAllocationManagerTarok* allocationmanager
            = (MM_GlobalAllocationManagerTarok*)_extensions->globalAllocationManager;
        /* We need at least one region per context for surviving Eden objects. Older objects can move into tail-fill
         * regions. */
...
void MM_IncrementalGenerationalGC::partialGarbageCollectUsingCopyForward(
    MM_EnvironmentVLHGC* env, MM_AllocateDescription* allocDescription)
{
    ...
    static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._copyForwardStats._freeMemoryBefore
        = freeMemoryForSurvivor;
    static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._copyForwardStats._totalMemoryBefore
        = _extensions->getHeap()->getMemorySize();
    ...
    Trc_MM_IncrementalGenerationalGC_partialGarbageCollectUsingCopyForward_ChooseCompactor(env->getLanguageVMThread(),
        estimatedSurvivorRequired, desiredCompactWork, freeMemoryForSurvivor,
        useSlidingCompactor ? "sliding" : "copying");
    ...
    if (!useSlidingCompactor) {
        ...
        static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._copyForwardStats._externalCompactBytes
            = 0;
    }

    _schedulingDelegate.partialGarbageCollectStarted(env);

    /* flush the RSList and RSM from our currently selected regions into the card table since we will rebuild them as we
     * process the table */
    ...
    static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._copyForwardStats._freeMemoryAfter
        = _extensions->getHeap()->getActualFreeMemorySize();
    static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._copyForwardStats._totalMemoryAfter
        = _extensions->getHeap()->getMemorySize();
    ...
    if (_extensions->tarokEnableExpensiveAssertions) {
        /* all objects in collection set regions are now marked so ensure that there are no more bump-allocated-only
         * regions */
    ...
    /* It is possible that we could end up with evacuate regions which were not compacted (inaccurate RSCL) so we need
     * to detect that case and sweep such regions, before completing the PGC */
    ...
    if (useSlidingCompactor) {
        ...
        _reclaimDelegate.runCompact(env, allocDescription, env->_cycleState->_activeSubSpace, desiredCompactWork,
            env->_cycleState->_gcCode, _markMapManager->getGlobalMarkPhaseMap(),
            &regionsSkippedByCompactorRequiringSweep);
        /* we can't tell exactly how many bytes were added to meet the compact goal, so we'll assume it was an exact
         * match. The precise amount could be lower or higher */
        static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._copyForwardStats._externalCompactBytes
            = desiredCompactWork;
        ...
        _reclaimDelegate.runReclaimForAbortedCopyForward(env, allocDescription, env->_cycleState->_activeSubSpace,
            env->_cycleState->_gcCode, _markMapManager->getGlobalMarkPhaseMap(),
            &regionsSkippedByCompactorRequiringSweep);
    }

    if (regionsSkippedByCompactorRequiringSweep > 0) {
        /* there were regions which we needed to compact but we couldn't compact so at least sweep them to ensure that
         * their stats are correct */
        _reclaimDelegate.performAtomicSweep(
            env, allocDescription, env->_cycleState->_activeSubSpace, env->_cycleState->_gcCode);
    }

    /* calculatePGCCompactionRate() has to be after PGC due to half of Eden regions has not been marked after final GMP
     * (the sweep could not collect those regions) */
    /* calculatePGCCompactionRate() has to be before estimateReclaimableRegions(), which need to use the result of
     * calculatePGCCompactionRate() - region->_defragmentationTarget */
...
void MM_IncrementalGenerationalGC::partialGarbageCollectUsingMarkCompact(
    MM_EnvironmentVLHGC* env, MM_AllocateDescription* allocDescription)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    ...
    UDATA defragmentReclaimableRegions = 0;
    UDATA reclaimableRegions = 0;
    _reclaimDelegate.estimateReclaimableRegions(
        env, 0.0 /* copy-forward loss */, &reclaimableRegions, &defragmentReclaimableRegions);
    _schedulingDelegate.partialGarbageCollectCompleted(env, reclaimableRegions, defragmentReclaimableRegions);
...
void MM_IncrementalGenerationalGC::setupBeforePartialGC(MM_EnvironmentVLHGC* env, MM_GCCode gcCode)
{
...
void MM_IncrementalGenerationalGC::setupBeforeGlobalGC(MM_EnvironmentVLHGC* env, MM_GCCode gcCode)
{
...
void MM_IncrementalGenerationalGC::setConfiguredSubspace(
    MM_EnvironmentBase* env, MM_MemorySubSpaceTarok* configuredSubspace)
{
    Assert_MM_true(NULL == _configuredSubspace);
...
void MM_IncrementalGenerationalGC::initialRegionAgesSetup(MM_EnvironmentVLHGC* env, UDATA givenAge)
{
...
void MM_IncrementalGenerationalGC::incrementRegionAges(MM_EnvironmentVLHGC* env, UDATA increment, bool isPGC)
{
    GC_HeapRegionIteratorVLHGC regionIterator(_regionManager, MM_HeapRegionDescriptor::MANAGED);
    MM_HeapRegionDescriptorVLHGC* region = NULL;
    ...
    while (NULL != (region = regionIterator.nextRegion())) {
        ...
        if (region->containsObjects() || region->isArrayletLeaf()) {
            ...
            if (owner->shouldMigrateRegionToCommonContext(env, region)) {
                ...
                if (owner != commonContext) {
                    if ((NULL == region->_allocateData._originalOwningContext)
                        && (commonContext->getNumaNode() != owner->getNumaNode())) {
            ...
            if (region->containsObjects() && (region->getLogicalAge() == env->getExtensions()->tarokRegionMaxAge)) {
                ...
                /* regions that age out, but are not full (thus not stable => accurate), should merge with other old
                 * non-full regions (in same AC) */
...
void MM_IncrementalGenerationalGC::incrementRegionAge(
    MM_EnvironmentVLHGC* env, MM_HeapRegionDescriptorVLHGC* region, UDATA increment, bool isPGC)
{
    ...
    Trc_MM_IncrementalGenerationalGC_incrementRegionAge(env->getLanguageVMThread(),
        _regionManager->mapDescriptorToRegionTableIndex(region), isPGC, (double)increment / (1024 * 1024),
        (double)allocationAgeBefore / (1024 * 1024), (double)allocationAge / (1024 * 1024),
        (double)region->getLowerAgeBound() / (1024 * 1024), (double)region->getUpperAgeBound() / (1024 * 1024),
        logicalAgeBefore, logicalAge);

    region->setAge(allocationAge, logicalAge);
}

void MM_IncrementalGenerationalGC::setRegionAgesToMax(MM_EnvironmentVLHGC* env)
{
    GC_HeapRegionIteratorVLHGC regionIterator(_regionManager, MM_HeapRegionDescriptor::MANAGED);
    MM_HeapRegionDescriptorVLHGC* region = NULL;
    MM_AllocationContextTarok* commonContext = (MM_AllocationContextTarok*)env->getCommonAllocationContext();
...
void MM_IncrementalGenerationalGC::declareAllRegionsAsMarked(MM_EnvironmentVLHGC* env)
{
...
bool MM_IncrementalGenerationalGC::isMarked(void* objectPtr)
{
...
void MM_IncrementalGenerationalGC::assertWorkPacketsEmpty(MM_EnvironmentVLHGC* env, MM_WorkPacketsVLHGC* packets)
{
...
void MM_IncrementalGenerationalGC::verifyMarkMapClosure(MM_EnvironmentVLHGC* env, MM_MarkMap* markMap)
{
        ...
        if (region->containsObjects()) {
            ...
            while (NULL != (object = iterator.nextObject())) {
                ...
                case GC_ObjectModel::SCAN_REFERENCE_MIXED_OBJECT:
                    Assert_MM_true(
                        GC_ObjectModel::REF_STATE_REMEMBERED != J9GC_J9VMJAVALANGREFERENCE_STATE(env, object));
                    /* fall through */
...
void MM_IncrementalGenerationalGC::assertTableClean(MM_EnvironmentVLHGC* env, Card additionalCleanState)
{
    /* first, walk all regions which have subspaces */
    GC_HeapRegionIterator regionIterator(_regionManager);
    MM_HeapRegionDescriptor* region = NULL;
    while (NULL != (region = regionIterator.nextRegion())) {
        /* we will be tolerant of dirty cards in regions which don't contain objects but we may want a more strict
         * definition of what that means, in the future.
         */
        if (region->containsObjects()) {
            ...
            for (Card* thisCard = lowCard; thisCard < highCard; thisCard++) {
                ...
                Assert_GC_true_with_message2(env, ((additionalCleanState == cardValue) || (CARD_CLEAN == cardValue)),
                    "The card %p is not clean, value %u\n", thisCard, cardValue);
            }
        }
    }
}

void MM_IncrementalGenerationalGC::reportPGCStart(MM_EnvironmentVLHGC* env)
{
...
void MM_IncrementalGenerationalGC::reportPGCEnd(MM_EnvironmentVLHGC* env)
{
    Trc_MM_PGCEnd(env->getLanguageVMThread(),
        static_cast<MM_CycleStateVLHGC*>(env->_cycleState)
            ->_vlhgcIncrementStats._workPacketStats.getSTWWorkStackOverflowOccured(),
        static_cast<MM_CycleStateVLHGC*>(env->_cycleState)
            ->_vlhgcIncrementStats._workPacketStats.getSTWWorkStackOverflowCount(),
...
void MM_IncrementalGenerationalGC::reportGMPIncrementStart(MM_EnvironmentVLHGC* env)
{
...
void MM_IncrementalGenerationalGC::reportGMPIncrementEnd(MM_EnvironmentVLHGC* env)
{
    ...
    Trc_MM_GMPIncrementEnd(env->getLanguageVMThread(),
        ...
        static_cast<MM_CycleStateVLHGC*>(env->_cycleState)
            ->_vlhgcIncrementStats._workPacketStats.getSTWWorkStackOverflowCount(),
        _extensions->heap->getApproximateActiveFreeMemorySize(MEMORY_TYPE_OLD),
        _extensions->heap->getActiveMemorySize(MEMORY_TYPE_OLD), incrementNumber);

    triggerGlobalGCEndHook(env);
}

void MM_IncrementalGenerationalGC::reportGlobalGCStart(MM_EnvironmentVLHGC* env)
{
    /* currently we only differ for the tracepoint since we still need to fire the global hook */
...
void MM_IncrementalGenerationalGC::reportGlobalGCEnd(MM_EnvironmentVLHGC* env)
{
...
void MM_IncrementalGenerationalGC::triggerGlobalGCStartHook(MM_EnvironmentVLHGC* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    ...
    TRIGGER_J9HOOK_MM_OMR_GLOBAL_GC_START(_extensions->omrHookInterface, env->getOmrVMThread(), j9time_hires_clock(),
        J9HOOK_MM_OMR_GLOBAL_GC_START, _extensions->globalVLHGCStats.gcCount, 0, isExplicitGC, isAggressiveGC,
        _bytesRequested);
}

void MM_IncrementalGenerationalGC::triggerGlobalGCEndHook(MM_EnvironmentVLHGC* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    ...
    TRIGGER_J9HOOK_MM_PRIVATE_REPORT_MEMORY_USAGE(_extensions->privateHookInterface, env->getOmrVMThread(),
        j9time_hires_clock(), J9HOOK_MM_PRIVATE_REPORT_MEMORY_USAGE, _extensions->getForge()->getCurrentStatistics());

    TRIGGER_J9HOOK_MM_OMR_GLOBAL_GC_END(_extensions->omrHookInterface, env->getOmrVMThread(), j9time_hires_clock(),
        ...
        static_cast<MM_CycleStateVLHGC*>(env->_cycleState)
            ->_vlhgcIncrementStats._workPacketStats.getSTWWorkStackOverflowCount(),
        static_cast<MM_CycleStateVLHGC*>(env->_cycleState)
            ->_vlhgcIncrementStats._workPacketStats.getSTWWorkpacketCountAtOverflow(),
        ...
        approximateActiveFreeMemorySize, activeMemorySize, FIXUP_NONE, 0);
}

void MM_IncrementalGenerationalGC::reportCopyForwardStart(MM_EnvironmentVLHGC* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);

    Trc_MM_CopyForwardStart(env->getLanguageVMThread());
    TRIGGER_J9HOOK_MM_PRIVATE_COPY_FORWARD_START(_extensions->privateHookInterface, env->getOmrVMThread(),
        j9time_hires_clock(), J9HOOK_MM_PRIVATE_COPY_FORWARD_START,
        &static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._copyForwardStats);
}

void MM_IncrementalGenerationalGC::reportCopyForwardEnd(MM_EnvironmentVLHGC* env, U_64 timeTaken)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);

    Trc_MM_CopyForwardEnd(env->getLanguageVMThread());
    TRIGGER_J9HOOK_MM_PRIVATE_COPY_FORWARD_END(_extensions->privateHookInterface, env->getOmrVMThread(),
        j9time_hires_clock(), J9HOOK_MM_PRIVATE_COPY_FORWARD_END,
        &static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._copyForwardStats,
...
bool MM_IncrementalGenerationalGC::isConcurrentWorkAvailable(MM_EnvironmentBase* env)
{
    bool isConcurrentEnabled = _extensions->tarokEnableConcurrentGMP;
    bool isGMPRunning = isGlobalMarkPhaseRunning();
    bool isProcessingWorkPackets = MM_CycleState::state_process_work_packets_after_initial_mark
        == _persistentGlobalMarkPhaseState._markDelegateState;
    ...
    return isConcurrentEnabled && isGMPRunning && isProcessingWorkPackets && isStillPermittedToRun
        && isGMPWorkAvailable;
}

void MM_IncrementalGenerationalGC::preConcurrentInitializeStatsAndReport(
    MM_EnvironmentBase* env, MM_ConcurrentPhaseStatsBase* stats)
{
    ...
    TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_PHASE_START(_extensions->privateHookInterface, env->getOmrVMThread(),
        j9time_hires_clock(), J9HOOK_MM_PRIVATE_CONCURRENT_PHASE_START, stats);
}

uintptr_t MM_IncrementalGenerationalGC::masterThreadConcurrentCollect(MM_EnvironmentBase* envBase)
{
    MM_EnvironmentVLHGC* env = MM_EnvironmentVLHGC::getEnvironment(envBase);

    /* note that we can't check isConcurrentWorkAvailable at this point since another thread could have set
     * _forceConcurrentTermination since the master thread calls this outside of the control monitor
     */
    Assert_MM_true(NULL == env->_cycleState);
    Assert_MM_true(isGlobalMarkPhaseRunning());
    Assert_MM_true(MM_CycleState::state_process_work_packets_after_initial_mark
        == _persistentGlobalMarkPhaseState._markDelegateState);

    env->_cycleState = &_persistentGlobalMarkPhaseState;
    static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats.clear();

    /* We pass a pointer to _forceConcurrentTermination so that we can cause the concurrent to terminate early by
     * setting the flag to true if we want to interrupt it so that the master thread returns to the control mutex in
     * order to receive a new GC request.
     */
    UDATA bytesConcurrentlyScanned = _globalMarkDelegate.performMarkConcurrent(
        env, _globalMarkPhaseIncrementBytesStillToScan, &_forceConcurrentTermination);
    _globalMarkPhaseIncrementBytesStillToScan
        = MM_Math::saturatingSubtract(_globalMarkPhaseIncrementBytesStillToScan, bytesConcurrentlyScanned);
    ...
    /* return the number of bytes scanned since the caller needs to pass it into postConcurrentUpdateStatsAndReport for
     * stats reporting */
    return bytesConcurrentlyScanned;
}

void MM_IncrementalGenerationalGC::postConcurrentUpdateStatsAndReport(
    MM_EnvironmentBase* env, MM_ConcurrentPhaseStatsBase* stats, UDATA bytesConcurrentlyScanned)
{
    ...
    TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_PHASE_END(_extensions->privateHookInterface, env->getOmrVMThread(),
        j9time_hires_clock(), J9HOOK_MM_PRIVATE_CONCURRENT_PHASE_END, stats);
}

void MM_IncrementalGenerationalGC::forceConcurrentFinish()
{
    /* A pointer to _forceConcurrentTermination is passed into the concurrent increment manager so we can cause it to
     * return early by setting this flag.
     */
    _forceConcurrentTermination = true;
}

void MM_IncrementalGenerationalGC::reportGMPCycleStart(MM_EnvironmentBase* env)
{
    reportGCCycleStart(env);
    Trc_MM_GMPCycleStart(env->getLanguageVMThread());
}

void MM_IncrementalGenerationalGC::reportGMPCycleContinue(MM_EnvironmentBase* env)
{
    Trc_MM_GMPCycleEnd(env->getLanguageVMThread());
    reportGCCycleContinue(env, OMR_GC_CYCLE_TYPE_VLHGC_GLOBAL_MARK_PHASE);
}

void MM_IncrementalGenerationalGC::reportGMPCycleEnd(MM_EnvironmentBase* env)
{
    Trc_MM_GMPCycleEnd(env->getLanguageVMThread());
    reportGCCycleEnd(env);
}

void MM_IncrementalGenerationalGC::reportGCCycleStart(MM_EnvironmentBase* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
    MM_CommonGCData commonData;
    ...
    Trc_MM_CycleStart(
        env->getLanguageVMThread(), env->_cycleState->_type, _extensions->getHeap()->getActualFreeMemorySize());

    TRIGGER_J9HOOK_MM_OMR_GC_CYCLE_START(extensions->omrHookInterface, env->getOmrVMThread(), j9time_hires_clock(),
        J9HOOK_MM_OMR_GC_CYCLE_START, extensions->getHeap()->initializeCommonGCData(env, &commonData),
        env->_cycleState->_type);
}

void MM_IncrementalGenerationalGC::reportGCCycleContinue(MM_EnvironmentBase* env, UDATA oldCycleStateType)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
    MM_CommonGCData commonData;

    Trc_MM_CycleContinue(env->getLanguageVMThread(), oldCycleStateType, env->_cycleState->_type,
        _extensions->getHeap()->getActualFreeMemorySize());

    TRIGGER_J9HOOK_MM_OMR_GC_CYCLE_CONTINUE(extensions->omrHookInterface, env->getOmrVMThread(), j9time_hires_clock(),
        J9HOOK_MM_OMR_GC_CYCLE_CONTINUE, extensions->getHeap()->initializeCommonGCData(env, &commonData),
        oldCycleStateType, env->_cycleState->_type);
}

void MM_IncrementalGenerationalGC::reportGCCycleEnd(MM_EnvironmentBase* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
    MM_CommonGCData commonData;
    ...
    TRIGGER_J9HOOK_MM_PRIVATE_GC_POST_CYCLE_END(extensions->privateHookInterface, env->getOmrVMThread(),
        j9time_hires_clock(), J9HOOK_MM_PRIVATE_GC_POST_CYCLE_END,
...
void MM_IncrementalGenerationalGC::exportStats(
    MM_EnvironmentVLHGC* env, MM_CollectionStatisticsVLHGC* stats, bool classesPotentiallyUnloaded)
{
    ...
    if (MM_CycleState::CT_GLOBAL_MARK_PHASE != env->_cycleState->_collectionType) {
        /* numaNodes is just used as indication to verbose GC that stats we collected are valid and indeed should be
         * reported */
        ...
        while (NULL != (region = regionIterator.nextRegion())) {
            if (!region->isFreeOrIdle()) {
                ...
                if (region->containsObjects()) {
                    ...
                    /* for Eden region containing objects Allocation Age must be smaller then amount allocated since
                     * last PGC */
                    if (region->getAllocationAge() <= _allocatedSinceLastPGC) {
                        stats->_edenHeapSize += regionSize;
                        /* region is not collected yet, so getActualFreeMemorySize might not be accurate - using
                         * getAllocatableBytes instead */
            ...
            if (region->isArrayletLeaf()) {
                 ...
                 * there might be unswept arraylet leaf regions, for which we must not try to access class data (for
                 * scan type). Therefore, we count these arraylets as 'unknown' type.
                 */
                if (classesPotentiallyUnloaded && !isMarked((J9Object*)spine)) {
                    stats->_arrayletUnknownLeaves += 1;
                    /* is this first arraylet leaf? */
                    if (region->getLowAddress()
                        == mmPointerFromToken((J9VMThread*)env->getLanguageVMThread(),
                               _extensions->indexableObjectModel.getArrayoidPointer(spine)[0])) {
                        stats->_arrayletUnknownObjects += 1;
                    }
                } else if (GC_ObjectModel::SCAN_POINTER_ARRAY_OBJECT
                    == _extensions->objectModel.getScanType((J9Object*)spine)) {
                    ...
                    Assert_MM_true(GC_ObjectModel::SCAN_PRIMITIVE_ARRAY_OBJECT
                        == _extensions->objectModel.getScanType((J9Object*)spine));
...
void MM_IncrementalGenerationalGC::reportGCIncrementStart(
    MM_EnvironmentBase* env, const char* incrementDescription, UDATA incrementCount)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
...
void MM_IncrementalGenerationalGC::reportGCIncrementEnd(MM_EnvironmentBase* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    MM_CollectionStatisticsVLHGC* stats = (MM_CollectionStatisticsVLHGC*)env->_cycleState->_collectionStatistics;
    stats->collectCollectionStatistics(env, stats);
...
void MM_IncrementalGenerationalGC::reportMarkStart(MM_EnvironmentBase* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    Trc_MM_MarkStart(env->getLanguageVMThread());

    TRIGGER_J9HOOK_MM_PRIVATE_MARK_START(
        _extensions->privateHookInterface, env->getOmrVMThread(), j9time_hires_clock(), J9HOOK_MM_PRIVATE_MARK_START);
}

void MM_IncrementalGenerationalGC::reportMarkEnd(MM_EnvironmentBase* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    Trc_MM_MarkEnd(env->getLanguageVMThread());

    TRIGGER_J9HOOK_MM_PRIVATE_MARK_END(
        _extensions->privateHookInterface, env->getOmrVMThread(), j9time_hires_clock(), J9HOOK_MM_PRIVATE_MARK_END);
}

void MM_IncrementalGenerationalGC::reportGMPMarkStart(MM_EnvironmentBase* env)
{
    reportMarkStart(env);

    TRIGGER_J9HOOK_MM_PRIVATE_GMP_MARK_START(_extensions->privateHookInterface, env->getOmrVMThread(),
        &static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._markStats,
        &static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._workPacketStats);
}

void MM_IncrementalGenerationalGC::reportGMPMarkEnd(MM_EnvironmentBase* env)
{
    reportMarkEnd(env);

    TRIGGER_J9HOOK_MM_PRIVATE_GMP_MARK_END(_extensions->privateHookInterface, env->getOmrVMThread(),
        &static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._markStats,
        &static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._workPacketStats);
}

void MM_IncrementalGenerationalGC::reportGlobalGCMarkStart(MM_EnvironmentBase* env)
{
    reportMarkStart(env);

    TRIGGER_J9HOOK_MM_PRIVATE_VLHGC_GLOBAL_GC_MARK_START(_extensions->privateHookInterface, env->getOmrVMThread(),
        &static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._markStats,
        &static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._workPacketStats);
}

void MM_IncrementalGenerationalGC::reportGlobalGCMarkEnd(MM_EnvironmentBase* env)
{
    reportMarkEnd(env);

    TRIGGER_J9HOOK_MM_PRIVATE_VLHGC_GLOBAL_GC_MARK_END(_extensions->privateHookInterface, env->getOmrVMThread(),
        &static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._markStats,
        &static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._workPacketStats);
}

void MM_IncrementalGenerationalGC::reportPGCMarkStart(MM_EnvironmentBase* env)
{
    reportMarkStart(env);

    TRIGGER_J9HOOK_MM_PRIVATE_PGC_MARK_START(_extensions->privateHookInterface, env->getOmrVMThread(),
        &static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._markStats,
        &static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._workPacketStats);
}

void MM_IncrementalGenerationalGC::reportPGCMarkEnd(MM_EnvironmentBase* env)
{
    reportMarkEnd(env);

    TRIGGER_J9HOOK_MM_PRIVATE_PGC_MARK_END(_extensions->privateHookInterface, env->getOmrVMThread(),
        &static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._markStats,
        &static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._workPacketStats,
        &static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._irrsStats);
}

void MM_IncrementalGenerationalGC::collectorExpanded(
    MM_EnvironmentBase* envBase, MM_MemorySubSpace* subSpace, UDATA expandSize)
{
    MM_EnvironmentVLHGC* env = MM_EnvironmentVLHGC::getEnvironment(envBase);
...
void MM_IncrementalGenerationalGC::postMarkMapCompletion(MM_EnvironmentVLHGC* env)
{
#if defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING)
...
#if defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING)
void MM_IncrementalGenerationalGC::unloadDeadClassLoaders(MM_EnvironmentVLHGC* env)
{
    ...
    MM_ClassUnloadStats* classUnloadStats
        = &static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._classUnloadStats;
    ...
    J9ClassLoader* classLoadersUnloadedList = _extensions->classLoaderManager->identifyClassLoadersToUnload(
        env, env->_cycleState->_markMap, classUnloadStats);
    _extensions->classLoaderManager->cleanUpClassLoadersStart(
        env, classLoadersUnloadedList, env->_cycleState->_markMap, classUnloadStats);
    ...
    if (0 < (classUnloadStats->_classesUnloadedCount + classUnloadStats->_classLoaderUnloadedCount)) {
        ...
        _extensions->classLoaderManager->cleanUpClassLoaders(
            env, classLoadersUnloadedList, &reclaimedSegments, &unloadLink, &env->_cycleState->_finalizationRequired);

        /* Free the class memory segments associated with dead classLoaders, unload (free) the dead classLoaders that
         * don't require finalization, and perform any final clean up after the dead classLoaders are gone.
         */
        classUnloadStats->_endScanTime = j9time_hires_clock();
        classUnloadStats->_startPostTime = classUnloadStats->_endScanTime;
        /* enqueue all the segments we just salvaged from the dead class loaders for delayed free (this work was
         * historically attributed in the unload end operation so it goes after the timer start) */
    ...
    /* If there was dynamic class unloading checks during the run, record the new number of class loaders last seen
     * during a DCU pass */
...
void MM_IncrementalGenerationalGC::reportClassUnloadingStart(MM_EnvironmentBase* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    Trc_MM_ClassUnloadingStart(env->getLanguageVMThread());

    TRIGGER_J9HOOK_MM_PRIVATE_CLASS_UNLOADING_START(_extensions->privateHookInterface, env->getOmrVMThread(),
        j9time_hires_clock(), J9HOOK_MM_PRIVATE_CLASS_UNLOADING_START);
}

void MM_IncrementalGenerationalGC::reportClassUnloadingEnd(MM_EnvironmentBase* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    MM_ClassUnloadStats* classUnloadStats
        = &static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._classUnloadStats;

    Trc_MM_ClassUnloadingEnd(env->getLanguageVMThread(), classUnloadStats->_classLoaderUnloadedCount,
        classUnloadStats->_classesUnloadedCount);

    TRIGGER_J9HOOK_MM_CLASS_UNLOADING_END(_extensions->hookInterface, (J9VMThread*)env->getLanguageVMThread(),
        j9time_hires_clock(), J9HOOK_MM_CLASS_UNLOADING_END, classUnloadStats->_endTime - classUnloadStats->_startTime,
        classUnloadStats->_classLoaderUnloadedCount, classUnloadStats->_classesUnloadedCount,
