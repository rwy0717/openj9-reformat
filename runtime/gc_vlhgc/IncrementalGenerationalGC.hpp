...
private:
    ...
    MM_HeapRegionManager* _regionManager; /**< The manager which will be walked to configure the collection flags on
                                             regions in the heap */

    MM_MemorySubSpaceTarok*
        _configuredSubspace; /**< The subspace which is configured in the system and triggers early AF - must be reset
                                after a collection to allow the mutator to proceed */

    MM_MarkMapManager* _markMapManager; /**< Tracking and managing all mark map assets */

    MM_InterRegionRememberedSet*
        _interRegionRememberedSet; /**< An abstraction layer over the tracking of inter-region references which is built
                                      by marking and used/updated by compact */

    MM_ClassLoaderRememberedSet* _classLoaderRememberedSet; /**< An abstraction layer over the tracking where instances
                                                               defined by each class loader can be found*/

    MM_CopyForwardDelegate
        _copyForwardDelegate; /**< Delegate responsible for handling copy and forward GC operations */
    ...
    MM_ProjectedSurvivalCollectionSetDelegate
        _projectedSurvivalCollectionSetDelegate; /** Collection set delegate based on using a region's projected
                                                    survival rate in order to select collection set */

    MM_CollectionStatisticsVLHGC
        _globalCollectionStatistics; /** Common collect stats (memory, time etc.), specifically for Global collects */
    MM_CollectionStatisticsVLHGC
        _partialCollectionStatistics; /** Common collect stats (memory, time etc.), specifically for Partial collects */
    ...
    MM_CycleStateVLHGC
        _persistentGlobalMarkPhaseState; /**< Since the GMP can be fragmented into increments running across several
                                            pauses, we need to store the cycle state data */
    volatile bool
        _forceConcurrentTermination; /**< Setting this to true will cause any concurrent GMP work being done for this
                                        collector to stop and return.  It is volatile because it is shared state between
                                        this and the concurren task's increment manager */

    UDATA _globalMarkPhaseIncrementBytesStillToScan; /**< The number of bytes which must be scanned in the next GMP
                                                        increment.  This is used by the concurrent GMP task to determine
...
public:
    ...
    virtual bool heapRemoveRange(MM_EnvironmentBase* env, MM_MemorySubSpace* subspace, UDATA size, void* lowAddress,
     ...
     */
    MMINLINE MM_MarkMapManager* getMarkMapManager() { return _markMapManager; }
    ...
    UDATA getAllocatedSinceLastPGC() { return _allocatedSinceLastPGC; }

    /**
     * The _configuredSubspace ivar is mutable via this method only to simplify start-up in the Configuration framework
     * but it should only be set once (implementation asserts this).
     */
     ...
     */
    bool isGlobalMarkPhaseRunning()
    {
        return (MM_CycleState::state_mark_idle != _persistentGlobalMarkPhaseState._markDelegateState);
    }
     ...
     * TODO: Try fetching it through cycleState (currently not doable, due to relative order of reportGCStart and
     * cycleState initialization)
     */
    ...
    /**
     * Called after a successful allocation if the given environment has been flagged as responsible for running GC
     * related work. The implementation can assume that the thread is at a safe point but does not have exclusive
     * access.
     * @param env[in] The thread which has been tasked with leading this increment (it will act as "master" in the
     * collect).  This thread does not have exclusive but is at a safe point
    ...
    /**
     * Called by the MasterGCThread while it still owns the GC control monitor in order to allow for the initial
     * population of stats and reporting of triggers to occur in-order relative to threads outside the GC.
     * @param env[in] The master GC thread
     * @param stats[out] The stats data structure meant to be populated with the state of the collector before the
    ...
    /**
     * The entry-point used by the master GC thread to perform concurrent GMP work.  isConcurrentWorkAvailable must be
     * true.
     * @param env[in] The master GC thread
    ...
    /**
     * Called by the MasterGCThread after it re-acquires the GC control monitor in order to allow for the final
     * population of stats and reporting of triggers to occur in-order relative to threads outside the GC.
     * @param env[in] The master GC thread
     * @param stats[in/out] The stats data structure meant to be populated with the state of the collector after the
     * concurrent operation ends (data populated before the collect is still in the structure)
...
private:
    /**
     * Perform a partial garbage collection (PGC) operation.  The main (internal) entry point to running a PGC
     * operation.
    ...
    /**
     * Runs a GMP (global mark only).  This call only runs the mark, collects mark time statistics, and calls the RSM
     * updated hook.
     * @param env[in] The master GC thread
     * @param incrementalMark True if we should perform only one increment of the global mark (resuming where we left
     * off) or false if we want to complete the global mark in this call
     */
    void globalMarkPhase(MM_EnvironmentVLHGC* env, bool incrementalMark);
    /**
     * Runs a PGC collect.  This call does report events, before and after the collection, but does not collect
     * statistics.
     * @param env[in] The master GC thread
     * @param allocDescription[in] The allocation request which triggered the collect
     */
    ...
    /**
     * Runs a PGC collect using the copy forward mechanism. This call does report events, before and after the
     * collection, but does not collect statistics.
     * @param env[in] The master GC thread
     * @param allocDescription[in] The allocation request which triggered the collect
     */
    void partialGarbageCollectUsingCopyForward(MM_EnvironmentVLHGC* env, MM_AllocateDescription* allocDescription);
    /**
     * Runs a PGC collect using the mark compact mechanism. This call does report events, before and after the
     * collection, but does not collect statistics.
     * @param env[in] The master GC thread
     * @param allocDescription[in] The allocation request which triggered the collect
     */
     ...
     * @param classesPotentiallyUnloaded[in], let the method if classes have recently been unloaded, so it does not do
     * any unsafe operation that may be based on class info (like scan type)
     */
    void exportStats(
        MM_EnvironmentVLHGC* env, MM_CollectionStatisticsVLHGC* stats, bool classesPotentiallyUnloaded = false);

    /**
     * Called at the beginning of a PGC to call tracepoints and hooks required before we can begin doing
     * increment-specific work.
     * @param env[in] The master GC thread
     */
    ...
    /**
     * Called at the beginning of a GMP increment to call tracepoints and hooks required before we can begin doing
     * increment-specific work.
     * @param env[in] The master GC thread
     */
    ...
    /**
     * Called at the end of a GMP increment to call tracepoints and hooks required before we can return from the
     * collector.
     * @param env[in] The master GC thread
     */
    ...
    /**
     * Called at the beginning of a global collection (OOM or SYS) to call tracepoints and hooks required before we can
     * begin executing the collect.
     * @param env[in] The master GC thread
     */
    void reportGlobalGCStart(MM_EnvironmentVLHGC* env);

    /**
     * Called at the end of a global collection (OOM or SYS) to call tracepoints and hooks required before we can return
     * from the collector.
     * @param env[in] The master GC thread
     */
     ...
     * @param additionalCleanState An additional state which will be interpreted as "clean" from the point of view of
     * this verification
     */
