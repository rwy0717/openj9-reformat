...
MM_InterRegionRememberedSet* MM_InterRegionRememberedSet::newInstance(
    MM_EnvironmentVLHGC* env, MM_HeapRegionManager* heapRegionManager)
{
    MM_InterRegionRememberedSet* interRegionRememberedSet = (MM_InterRegionRememberedSet*)env->getForge()->allocate(
        sizeof(MM_InterRegionRememberedSet), MM_AllocationCategory::REMEMBERED_SET, J9_GET_CALLSITE());
...
void MM_InterRegionRememberedSet::exportStats(MM_EnvironmentVLHGC* env, MM_CollectionStatisticsVLHGC* stats)
{
    ...
    stats->_rememberedSetBytesFree
        = _freeBufferCount * MM_RememberedSetCardBucket::MAX_BUFFER_SIZE * sizeof(MM_RememberedSetCard);
    stats->_rememberedSetBytesTotal
        = _bufferCountTotal * MM_RememberedSetCardBucket::MAX_BUFFER_SIZE * sizeof(MM_RememberedSetCard);
...
void MM_InterRegionRememberedSet::threadLocalInitialize(MM_EnvironmentVLHGC* env)
{
    for (UDATA index = 0; index < _heapRegionManager->getTableRegionCount(); index++) {
        MM_HeapRegionDescriptorVLHGC* region
            = (MM_HeapRegionDescriptorVLHGC*)_heapRegionManager->physicalTableDescriptorForIndex(index);
        region->getRememberedSetCardList()->initialize(env, index);
    }
}

MM_HeapRegionDescriptorVLHGC* MM_InterRegionRememberedSet::getBufferOwningRegion(
    MM_CardBufferControlBlock* cardBufferControlBlock)
{
...
void MM_InterRegionRememberedSet::flushBuffersForDecommitedRegions(MM_EnvironmentVLHGC* env)
{
    if (_shouldFlushBuffersForDecommitedRegions) {
        _shouldFlushBuffersForDecommitedRegions = false;

        /* At this point there should be no non-empty buffers used by decommitted regions.
         * Now, walk the global pool of free buffers, and remove them from the list if they are owned by decommitted
         * region. Before that, move all free buffes from thread local pools to the global
         */
        ...
        for (UDATA i = 0; i < _heapRegionManager->getTableRegionCount(); i++) {
            MM_HeapRegionDescriptorVLHGC* region
                = (MM_HeapRegionDescriptorVLHGC*)_heapRegionManager->mapRegionTableIndexToDescriptor(i);
...
bool MM_InterRegionRememberedSet::allocateRegionBuffers(MM_EnvironmentVLHGC* env, MM_HeapRegionDescriptorVLHGC* region)
{
    bool success = false;
    ...
    if (NULL != region->_rsclBufferPool) {
        ...
        region->_rsclBufferPool = (MM_RememberedSetCard*)ext->getForge()->allocate(
            rsclBufferPoolSize, MM_AllocationCategory::REMEMBERED_SET, J9_GET_CALLSITE());

        if (NULL != region->_rsclBufferPool) {
            /* the pool of buffers have to be bufferSize aligned */
            MM_RememberedSetCard* rsclBufferPoolAligned
                = (MM_RememberedSetCard*)(((UDATA)region->_rsclBufferPool + bufferSize) & ~(bufferSize - 1));

            /* Initialize BufferControlBlocks */

            UDATA regionIndex
                = MM_GCExtensions::getExtensions(env)->heapRegionManager->mapDescriptorToRegionTableIndex(region);
            UDATA bufferControlBlockCountPerRegion
                = ext->tarokRememberedSetCardListSize / MM_RememberedSetCardBucket::MAX_BUFFER_SIZE;
            ...
            for (UDATA i = firstBufferControlBlockIndex; i < lastBufferControlBlockIndex; i++) {
                _rsclBufferControlBlockPool[i]._card = &rsclBufferPoolAligned[(i - firstBufferControlBlockIndex)
                    * MM_RememberedSetCardBucket::MAX_BUFFER_SIZE];
            ...
            Assert_MM_true(
                _bufferCountTotal <= (_bufferControlBlockCountPerRegion * _heapRegionManager->getTableRegionCount()));
...
bool MM_InterRegionRememberedSet::initialize(MM_EnvironmentVLHGC* env)
{
    MM_GCExtensions* ext = MM_GCExtensions::getExtensions(env);
    ...
    /* if tarokRememberedSetCardListSize is not a multiple of  MM_RememberedSetCardBucket::MAX_BUFFER_SIZE, we are going
     * to allocate a little less */
    _bufferControlBlockCountPerRegion
        = ext->tarokRememberedSetCardListSize / MM_RememberedSetCardBucket::MAX_BUFFER_SIZE;
     ...
     */
    _rsclBufferControlBlockPool = (MM_CardBufferControlBlock*)ext->getForge()->allocate(
        rsclBufferControlBlockPoolSize, MM_AllocationCategory::REMEMBERED_SET, J9_GET_CALLSITE());
...
void MM_InterRegionRememberedSet::rememberReferenceInternal(
    MM_EnvironmentVLHGC* env, J9Object* fromObject, MM_HeapRegionDescriptorVLHGC* toRegion)
{
...
MM_CardBufferControlBlock* MM_InterRegionRememberedSet::allocateCardBufferControlBlockFromLocalPool(
...
void MM_InterRegionRememberedSet::allocateCardBufferControlBlockList(MM_EnvironmentVLHGC* env, UDATA bufferCount)
{
...
UDATA
MM_InterRegionRememberedSet::releaseCardBufferControlBlockListToLocalPool(
    MM_EnvironmentVLHGC* env, MM_CardBufferControlBlock* controlBlockHead, UDATA maxBuffersToLocalPool)
{
    UDATA bufferCount = 0;
...
UDATA
MM_InterRegionRememberedSet::releaseCardBufferControlBlockListToGlobalPool(
    MM_EnvironmentVLHGC* env, MM_CardBufferControlBlock* controlBlockHead)
{
    return releaseCardBufferControlBlockListToLocalPool(env, controlBlockHead, MAX_LOCAL_RSCL_BUFFER_POOL_SIZE);
}

UDATA
MM_InterRegionRememberedSet::releaseCardBufferControlBlockList(
    MM_EnvironmentVLHGC* env, MM_CardBufferControlBlock* controlBlockHead, MM_CardBufferControlBlock* controlBlockTail)
{
    UDATA bufferCount = 0;
...
void MM_InterRegionRememberedSet::releaseCardBufferControlBlockListForThread(
    MM_EnvironmentVLHGC* env, MM_EnvironmentVLHGC* threadEnv)
{
    threadEnv->_rsclBufferControlBlockCount -= releaseCardBufferControlBlockList(
        env, threadEnv->_rsclBufferControlBlockHead, threadEnv->_rsclBufferControlBlockTail);
...
void MM_InterRegionRememberedSet::releaseCardBufferControlBlockLocalPools(MM_EnvironmentVLHGC* env)
{
...
void MM_InterRegionRememberedSet::tearDown(MM_EnvironmentBase* env)
{
    MM_GCExtensions* ext = MM_GCExtensions::getExtensions(env);
...
void MM_InterRegionRememberedSet::kill(MM_EnvironmentBase* env)
{
    tearDown(env);
    env->getForge()->free(this);
}

void MM_InterRegionRememberedSet::resetOverflowedList()
{
    _overflowedListHead = NULL;
    _overflowedListTail = NULL;
}

void MM_InterRegionRememberedSet::setUnusedRegionThreshold(
    MM_EnvironmentVLHGC* env, double averageEmptinessOfCopyForwardedRegions)
{
    _unusedRegionThreshold = averageEmptinessOfCopyForwardedRegions;
}

void MM_InterRegionRememberedSet::overflowIfStableRegion(MM_EnvironmentVLHGC* env, MM_HeapRegionDescriptorVLHGC* region)
{
...
void MM_InterRegionRememberedSet::enqueueOverflowedRscl(
    MM_EnvironmentVLHGC* env, MM_RememberedSetCardList* rsclToEnqueue)
{
    ...
    /* make sure rsclToEnqueue->_nonEmptyOverflowedNext does not point to a stale RSCL before we make it visible to the
     * other users of the list */
    ...
    while (
        (UDATA)oldTail != MM_AtomicOperations::lockCompareExchange(localAddr, (UDATA)oldTail, (UDATA)rsclToEnqueue)) {
...
MM_RememberedSetCardList* MM_InterRegionRememberedSet::findRsclToOverflow(MM_EnvironmentVLHGC* env)
{
    MM_RememberedSetCardList* listToOverflow = NULL;

    MM_RememberedSetCardList* candidateListToOverflow = NULL;
    /* if we started walking the list of overflowed regions in this cycle, continue walking at where we stopped last
     * time otherwise start from the beginning of the list
     */
    ...
    while (NULL != candidateListToOverflow) {
        ...
        /* make sure _nonEmptyOverflowedNext is not prematurely pre-fetched (probably only possible if compiler unrolls
         * the loop) */
        MM_AtomicOperations::loadSync();

        /* remember that this is the last RSCL we examined (and found no buffers in it) and only then advance to the
         * next in the list */
        candidateListToOverflow = candidateListToOverflow->_nonEmptyOverflowedNext;
    }

    /* if we found no previously overflowed region with unreleased buffers for the current bucket, we will try to find a
     * new RSCL to overflow. during the search some other threads could have overflowed some other RSCLs (not very
     * likely in practice) so we did not find them in the global list of overflowed RSCLs, but want still to consider
     * them. bottom line, we do not limit ourselves on non-overflowed RSCLs in this search. not even those that are
     * being rebuilt (if we are in GMP). what we do look for is the largest RSCL with at least one buffer in our bucket
     * (this is just one of many possible heuristics).
     */
    if (NULL == listToOverflow) {
        ...
        while (NULL != (region = regionIterator.nextRegion())) {
            ...
            if ((MM_HeapRegionDescriptor::BUMP_ALLOCATED_MARKED == region->getRegionType())
                && (0 < rscl->mapToBucket(env)->_bufferCount)) {
...
void MM_InterRegionRememberedSet::prepareRegionsForGlobalCollect(MM_EnvironmentVLHGC* env, bool gmpInProgress)
{
    if (!gmpInProgress) {
        Assert_MM_true(0 == _beingRebuiltRegionCount);
        /* since we aren't picking up a partially-completed mark state, we can safely clear all remembered object
         * meta-data */
        GC_HeapRegionIteratorVLHGC regionIterator(_heapRegionManager);
        MM_HeapRegionDescriptorVLHGC* region = NULL;
        while (NULL != (region = regionIterator.nextRegion())) {
...
void MM_InterRegionRememberedSet::prepareOverflowedRegionsForRebuilding(MM_EnvironmentVLHGC* env)
{
...
void MM_InterRegionRememberedSet::setRegionsAsRebuildingComplete(MM_EnvironmentVLHGC* env)
{
    ...
    Trc_MM_InterRegionRememberedSet_setRegionsAsRebuildingComplete_rebuildingSummary(
        env->getLanguageVMThread(), beingRebuiltCount, stillOverflowedCount);
    Assert_MM_true(0 == _beingRebuiltRegionCount);
}

void MM_InterRegionRememberedSet::rememberReferenceForMarkInternal(
    MM_EnvironmentVLHGC* env, J9Object* fromObject, J9Object* toObject)
{
    MM_HeapRegionDescriptorVLHGC* toRegion
        = (MM_HeapRegionDescriptorVLHGC*)_heapRegionManager->tableDescriptorForAddress(toObject);
...
void MM_InterRegionRememberedSet::rememberReferenceForCompactInternal(
    MM_EnvironmentVLHGC* env, J9Object* fromObject, J9Object* toObject)
{
    MM_HeapRegionDescriptorVLHGC* toRegion
        = (MM_HeapRegionDescriptorVLHGC*)_heapRegionManager->tableDescriptorForAddress(toObject);
...
void MM_InterRegionRememberedSet::rememberReferenceForCopyForwardInternal(
    MM_EnvironmentVLHGC* env, J9Object* fromObject, J9Object* toObject)
{
    MM_HeapRegionDescriptorVLHGC* toRegion
        = (MM_HeapRegionDescriptorVLHGC*)_heapRegionManager->tableDescriptorForAddress(toObject);
...
bool MM_InterRegionRememberedSet::isReferenceRememberedForMark(
    MM_EnvironmentVLHGC* env, J9Object* fromObject, J9Object* toObject)
{
    ...
    if (NULL != toObject) {
        MM_HeapRegionDescriptorVLHGC* toRegion
            = (MM_HeapRegionDescriptorVLHGC*)_heapRegionManager->tableDescriptorForAddress(toObject);
        MM_HeapRegionDescriptorVLHGC* fromRegion
            = (MM_HeapRegionDescriptorVLHGC*)_heapRegionManager->tableDescriptorForAddress(fromObject);
...
void MM_InterRegionRememberedSet::clearReferencesToRegion(
    MM_EnvironmentVLHGC* env, MM_HeapRegionDescriptorVLHGC* toRegion)
{
 ...
 */
void MM_InterRegionRememberedSet::clearFromRegionReferencesForCompact(MM_EnvironmentVLHGC* env)
{
...
void MM_InterRegionRememberedSet::clearFromRegionReferencesForCompactDirect(MM_EnvironmentVLHGC* env)
{
        ...
        if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
            if (!region->getRememberedSetCardList()->isOverflowed()) {
                ...
                while (0 != (card = rsclCardIterator.nextReferencingCard(env))) {
                    ...
                    /* Regions that are completely swept after a GMP, might still have outgoing references (thus we
                     * consider empty regions too) */
                    Card* cardAddress = rememberedSetCardToCardAddr(env, card);
                    if (fromRegion->_compactData._shouldCompact || !fromRegion->containsObjects()
                        || isDirtyCardForPartialCollect(env, cardTable, cardAddress)) {
                        toRemoveCount += 1;
                ...
                if (0 != toRemoveCount) {
                    ...
                    Trc_MM_InterRegionRememberedSet_clearFromRegionReferencesForCompact_cardCounts(
                        env->getLanguageVMThread(), MM_GCExtensions::getExtensions(env)->globalVLHGCStats.gcCount,
                        _heapRegionManager->mapDescriptorToRegionTableIndex(region), totalCountBefore, toRemoveCount,
                        totalCountAfter);
    ...
    env->_irrsStats._clearFromRegionReferencesTimesus
        = j9time_hires_delta(startTime, j9time_hires_clock(), J9PORT_TIME_DELTA_IN_MICROSECONDS);
    ...
    Trc_MM_InterRegionRememberedSet_clearFromRegionReferencesForCompact_timesus(
        env->getLanguageVMThread(), env->_irrsStats._clearFromRegionReferencesTimesus, 0);
}

void MM_InterRegionRememberedSet::clearFromRegionReferencesForCompactOptimized(MM_EnvironmentVLHGC* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    MM_CardTable* cardTable = MM_GCExtensions::getExtensions(env)->cardTable;
            ...
            if (!region->getRememberedSetCardList()->isOverflowed()) {
                ...
                while (0 != (card = rsclCardIterator.nextReferencingCard(env))) {
                    ...
                    if (tableIsReady) {
                        ...
                        remove = compressedCardTable->isCompressedCardDirtyForPartialCollect(
                            env, convertHeapAddressFromRememberedSetCard(card));
                    } else {
    ...
    env->_irrsStats._clearFromRegionReferencesTimesus
        = j9time_hires_delta(startTime, j9time_hires_clock(), J9PORT_TIME_DELTA_IN_MICROSECONDS);
    env->_irrsStats._rebuildCompressedCardTableTimesus
        = j9time_hires_delta(startTime, timeAfterRebuild, J9PORT_TIME_DELTA_IN_MICROSECONDS);
    env->_irrsStats._clearFromRegionReferencesCardsProcessed = cardsProcessed;
    env->_irrsStats._clearFromRegionReferencesCardsCleared = cardsRemoved;

    Trc_MM_InterRegionRememberedSet_clearFromRegionReferencesForCompact_timesus(env->getLanguageVMThread(),
        env->_irrsStats._clearFromRegionReferencesTimesus, env->_irrsStats._rebuildCompressedCardTableTimesus);
}
 ...
 * optimized/non-optimized version is chosen
 */
void MM_InterRegionRememberedSet::clearFromRegionReferencesForMark(MM_EnvironmentVLHGC* env)
{
    if (MM_GCExtensions::getExtensions(env)->tarokEnableCompressedCardTable) {
...
void MM_InterRegionRememberedSet::clearFromRegionReferencesForMarkDirect(MM_EnvironmentVLHGC* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    MM_CardTable* cardTable = MM_GCExtensions::getExtensions(env)->cardTable;
    U_64 startTime = j9time_hires_clock();
        ...
        if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
            if (!region->getRememberedSetCardList()->isOverflowed()) {
                ...
                while (0 != (card = rsclCardIterator.nextReferencingCard(env))) {
                    ...
                    /* Regions that are completely swept after a GMP, might still have outgoing references (thus we
                     * consider empty regions too) */
                    if (fromRegion->_markData._shouldMark || !fromRegion->containsObjects()
                        || isDirtyCardForPartialCollect(env, cardTable, cardAddress)) {
                        toRemoveCount += 1;
                        rsclCardIterator.removeCurrentCard();
                    }
                ...
                if (0 != toRemoveCount) {
                    ...
                    Trc_MM_InterRegionRememberedSet_clearFromRegionReferencesForMark_cardCounts(
                        env->getLanguageVMThread(), MM_GCExtensions::getExtensions(env)->globalVLHGCStats.gcCount,
                        _heapRegionManager->mapDescriptorToRegionTableIndex(region), totalCountBefore, toRemoveCount,
                        totalCountAfter);
                    Assert_MM_true(totalCountBefore == toRemoveCount + totalCountAfter);
    ...
    Trc_MM_InterRegionRememberedSet_clearFromRegionReferencesForMark_timesus(
        env->getLanguageVMThread(), env->_irrsStats._clearFromRegionReferencesTimesus, 0);
}

void MM_InterRegionRememberedSet::clearFromRegionReferencesForMarkOptimized(MM_EnvironmentVLHGC* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    MM_CardTable* cardTable = MM_GCExtensions::getExtensions(env)->cardTable;
    MM_CompressedCardTable* compressedCardTable = MM_GCExtensions::getExtensions(env)->compressedCardTable;
    ...
    Trc_MM_InterRegionRememberedSet_clearFromRegionReferencesForMark_timesus(env->getLanguageVMThread(),
        env->_irrsStats._clearFromRegionReferencesTimesus, env->_irrsStats._rebuildCompressedCardTableTimesus);
}

void MM_InterRegionRememberedSet::rebuildCompressedCardTableForMark(MM_EnvironmentVLHGC* env)
{
    MM_CompressedCardTable* compressedCardTable = MM_GCExtensions::getExtensions(env)->compressedCardTable;
    UDATA totalNumber = 0;
    ...
    while (NULL != (region = regionIterator.nextRegion())) {
        ...
        if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
            if (!region->containsObjects() || region->_markData._shouldMark) {
                ...
                compressedCardTable->setCompressedCardsDirtyForPartialCollect(
                    region->getLowAddress(), region->getHighAddress());
            } else {
                /* rebuild compressed card table for region */
                compressedCardTable->rebuildCompressedCardTableForPartialCollect(
                    env, region->getLowAddress(), region->getHighAddress());
            }
            doneByThisThread += 1;
        }
    }
...
void MM_InterRegionRememberedSet::rebuildCompressedCardTableForCompact(MM_EnvironmentVLHGC* env)
{
    MM_CompressedCardTable* compressedCardTable = MM_GCExtensions::getExtensions(env)->compressedCardTable;
    UDATA totalNumber = 0;
    UDATA doneByThisThread = 0;
...
void MM_InterRegionRememberedSet::clearFromRegionReferencesForCopyForward(MM_EnvironmentVLHGC* env)
{
    /* The same collection set tagging mechanism is used for marking and copy forwarding - so just forward on to the
     * equivalent routine for mark */
    clearFromRegionReferencesForMark(env);
}

bool MM_InterRegionRememberedSet::isDirtyCardForPartialCollect(
    MM_EnvironmentVLHGC* env, MM_CardTable* cardTable, Card* card)
{
    bool result = false;
...
void MM_InterRegionRememberedSet::setupForPartialCollect(MM_EnvironmentVLHGC* env)
{
    if (MM_GCExtensions::getExtensions(env)->tarokEnableCompressedCardTable) {
        MM_CompressedCardTable* compressedCardTable = MM_GCExtensions::getExtensions(env)->compressedCardTable;
    ...
    /* List of overflowed RSCLs is built from scratch for every Mark/Copy-Forward/Compact. It will contain only newly
     * overflowed RSCLs. Previously overflowed RSCLs will have buffers released already and are not interesting for this
     * list.
     */
