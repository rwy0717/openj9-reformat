...
/* value for MAX_LOCAL_RSCL_BUFFER_POOL_SIZE is empirically chosen to be the lowest one but still reduces most of
 * contention on global pool lock */
...
public:
    ...
    MM_CardBufferControlBlock* _rsclBufferControlBlockPool; /**< starting address of the global pool of control blocks
                                                               (kept around to be able to release memory at the end) */
    MM_CardBufferControlBlock* volatile _rsclBufferControlBlockHead; /**< current head of BufferControlBlock global pool
                                                                        list */
    ...
    MM_LightweightNonReentrantLock
        _lock; /**< lock used to protect concurrent alloc/free from/to BufferControlBlock global pool */

    MM_RememberedSetCardList* volatile _overflowedListHead; /**< global list of overflowed regions that have unreleased
                                                               buffers - list head */
    MM_RememberedSetCardList* volatile _overflowedListTail; /**< global list of overflowed regions that have unreleased
                                                               buffers - list tail */

    UDATA _regionSize; /**< Cached region size */

    bool _shouldFlushBuffersForDecommitedRegions; /**< set to true at the end of a GC, if contraction occured. this is a
                                                     signal for the next GC to perform flush buffers from regions
                                                     contracted */
    ...
    double _unusedRegionThreshold; /**< fraction of region unused (free&fragmented) to be considered full (used for
    ...
    UDATA _cardToRegionShift; /**< the shift value to use against RememberedSetCards to determine the corresponding
                                 region index */
    UDATA _cardToRegionDisplacement; /**< the displacement value to use against RememberedSetcards to determine the
                                        corresponding region index */
    MM_CardTable* _cardTable; /**< cached copy of card table */

    MM_RememberedSetCardBucket*
        _rememberedSetCardBucketPool; /**< RS bucket pool (for all regions) for Master thread or any other thread that
                                         caused GC in absence of Master thread */
...
private:
    ...
    /**
     * Remember reference  from an object to an object (or appropriate structure (like card or region) the object
     * belongs to)
     * @param fromObject object (its slot) pointing from
     * @param toRegion region being pointed at
     */
    MMINLINE void rememberReferenceInternal(
...
public:
     ...
     * @param env[in] of a GC thread
     * @param averageEmptinessOfCopyForwardedRegions[in] average fragmentation of copy-forward is used to determine the
     * final threshold
     */
    ...
    /**
     * Allocate a BufferControlBlock (that represents a buffer for a Buffer in a Card List Bucket) from the thread local
     * pool Replenish local pool from global pool, if needed
     ...
     */
    UDATA releaseCardBufferControlBlockList(MM_EnvironmentVLHGC* env, MM_CardBufferControlBlock* controlBlockHead,
        MM_CardBufferControlBlock* controlBlockTail = NULL);

    /**
     * release the complete list (from head to tail) of BufferControlBlocks for a given thread, and set the head and
     ...
     * Only a portion of the list (for example, up to MAX_LOCAL_RSCL_BUFFER_POOL_SIZE) will be released to the local
     * pool. The rest is released to the global pool.
     * @param controlBlockHead head of the list to be released
     * @param maxBuffersToLocalPool portion of the list provided to be returned to the local pool. typically UDATA_MAX
     * (all goes to local) or MAX_LOCAL_RSCL_BUFFER_POOL_SIZE (small part goes to local, rest to global)
     * @return the count of buffers in the pool being returned
     */
    UDATA releaseCardBufferControlBlockListToLocalPool(
        MM_EnvironmentVLHGC* env, MM_CardBufferControlBlock* controlBlockHead, UDATA maxBuffersToLocalPool);

    /**
     * release a BufferControlBlock list to the global pool of buffers. This is multithreaded safe, but a lock is used
     * (may cause contention on large SMPs) A small fraction of the list may be release to the local pool before.
     * @param controlBlockHead head of the list to be released
     * @return the count of buffers in the pool being returned
     */
    UDATA releaseCardBufferControlBlockListToGlobalPool(
    ...
    /**
     * During marking, remember reference  from an object to an object (or appropriate structure (like card or region)
     * the object belongs to)
     * @param fromObject object (its slot) pointing from
     * @param toObject object being pointed
     */
    MMINLINE void rememberReferenceForMark(MM_EnvironmentVLHGC* env, J9Object* fromObject, J9Object* toObject)
    {
        if (NULL != toObject) {
     ...
     * @param toObject object being pointed
     */
    MMINLINE bool shouldRememberReferenceForGlobalMark(
    ...
    /**
     * During compact, remember reference  from an object to an object (or appropriate structure (like card or region)
     * the object belongs to)
     * @param fromObject object (its slot) pointing from
     * @param toObject object being pointed
     */
    MMINLINE void rememberReferenceForCompact(MM_EnvironmentVLHGC* env, J9Object* fromObject, J9Object* toObject)
    {
    ...
    /**
     * During copy forward operations, remember reference  from an object to an object (or appropriate structure (like
     * card or region) the object belongs to).
     * @param fromObject object (its slot) pointing from
     * @param toObject object being pointed
     */
    MMINLINE void rememberReferenceForCopyForward(MM_EnvironmentVLHGC* env, J9Object* fromObject, J9Object* toObject)
    {
        if (NULL != toObject) {
            /* We use the XOR and region mask of object pointers to determine if the objects share the same region.
             * Getting the region descriptors for comparison is a much more expensive operation (and reserved for when
     ...
     */
    MMINLINE MM_RememberedSetCard getRememberedSetCardFromJ9Object(J9Object* object)
    {
     ...
     */
    MMINLINE MM_RememberedSetCard convertRememberedSetCardFromHeapAddress(void* address)
    {
     ...
     */
    MMINLINE void* convertHeapAddressFromRememberedSetCard(MM_RememberedSetCard card)
    {
        void* address = NULL;
#if defined(J9VM_GC_COMPRESSED_POINTERS)
