 ...
 */
MM_MarkMapManager* MM_MarkMapManager::newInstance(MM_EnvironmentVLHGC* env)
{
    MM_MarkMapManager* markMapManager;

    markMapManager = (MM_MarkMapManager*)env->getForge()->allocate(
        sizeof(MM_MarkMapManager), MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
 ...
 */
void MM_MarkMapManager::kill(MM_EnvironmentVLHGC* env)
{
    tearDown(env);
 ...
 */
bool MM_MarkMapManager::initialize(MM_EnvironmentVLHGC* env)
{
 ...
 */
void MM_MarkMapManager::tearDown(MM_EnvironmentVLHGC* env)
{
...
bool MM_MarkMapManager::collectorStartup(MM_GCExtensions* extensions)
{
    bool result = true;
 ...
 */
bool MM_MarkMapManager::heapAddRange(
 ...
 *
 */
bool MM_MarkMapManager::heapRemoveRange(MM_EnvironmentVLHGC* env, MM_MemorySubSpace* subspace, UDATA size,
    void* lowAddress, void* highAddress, void* lowValidAddress, void* highValidAddress)
{
    ...
    result = result
        && _previousMarkMap->heapRemoveRange(env, size, lowAddress, highAddress, lowValidAddress, highValidAddress);
    if (NULL != _deleteEventShadowMarkMap) {
        result = result
            && _deleteEventShadowMarkMap->heapRemoveRange(
                   env, size, lowAddress, highAddress, lowValidAddress, highValidAddress);
    }
    return result;
}

MM_MarkMap* MM_MarkMapManager::savePreviousMarkMapForDeleteEvents(MM_EnvironmentVLHGC* env)
{
...
void MM_MarkMapManager::reportDeletedObjects(MM_EnvironmentVLHGC* env, MM_MarkMap* oldMap, MM_MarkMap* newMap)
{
    Assert_MM_true(NULL != _deleteEventShadowMarkMap);
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
    MM_HeapRegionManager* regionManager = extensions->heap->getHeapRegionManager();
            ...
            while (NULL != (object = iterator.nextObject())) {
                if (!newMap->isBitSet(object)) {
                    ...
                    TRIGGER_J9HOOK_MM_OMR_OBJECT_DELETE(
                        env->getExtensions()->omrHookInterface, env->getOmrVMThread(), object, memorySubSpace);
                }
            }
        } else if (region->containsObjects()) {
            GC_ObjectHeapIteratorAddressOrderedList iterator(extensions, (J9Object*)region->getLowAddress(),
                (J9Object*)((MM_MemoryPoolBumpPointer*)region->getMemoryPool())->getAllocationPointer(), false, false);
            J9Object* object = NULL;
            while (NULL != (object = iterator.nextObject())) {
                if (!newMap->isBitSet(object)) {
...
void MM_MarkMapManager::verifyNextMarkMapSubsetOfPrevious(MM_EnvironmentVLHGC* env)
{
    ...
    while (NULL != region) {
        ...
        if (region->containsObjects()) {
            MM_HeapMapIterator nextMapWalker(
                _extensions, _nextMarkMap, (UDATA*)region->getLowAddress(), (UDATA*)region->getHighAddress());
...
void MM_MarkMapManager::swapMarkMaps()
{
    ...
    for (UDATA i = 0; i < regionCount; i++) {
        MM_HeapRegionDescriptorVLHGC* region
            = (MM_HeapRegionDescriptorVLHGC*)regionManager->mapRegionTableIndexToDescriptor(i);
