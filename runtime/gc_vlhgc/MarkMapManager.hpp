...
private:
    ...
    MM_MarkMap* _deleteEventShadowMarkMap; /**< Used only when J9HOOK_MM_OMR_OBJECT_DELETE is hooked or reserved.  Used
                                              to represent the mark map state prior to a PGC mark or a GMP completion so
                                              that the difference in map states can be used to report deleted objects */
...
public:
     ...
     */
    MM_MarkMap* getGlobalMarkPhaseMap() { return _nextMarkMap; }

    /**
     * Return the mark map used for the partial garbage collection phase.
     * @return an MM_MarkMap
     */
    MM_MarkMap* getPartialGCMap() { return _previousMarkMap; }

    /**
     * Swaps the _inProgressMarkMap and the _completeMarkMap.  Called at the end of a GMP when it has completed a new
     * map.
     */
    void swapMarkMaps();
    ...
    /**
     * Checks all regions in the heap for differences between oldMap and newMap.  If a region has a valid mark map in
     * both versions, objects which were marked in oldMap but not in newMap are reported.  If the region only has a
     * valid mark map in newMap, the region is walked address-ordered and objects which are not marked in newMap are
     * reported.
     * @note can only be called if J9HOOK_MM_OMR_OBJECT_DELETE was reserved or hooked at startup (asserts otherwise)
     * @param env[in] The master GC thread
    ...
    /**
     * Called when _nextMarkMap has been completed but before it is swapped with _previousMarkMap.  Ensures that
     * _nextMarkMap is a subset of _previousMarkMap.  Asserts on failure. Note that this must be called before
     * _nextMarkMap is used to move objects on the heap since there is no way to fixup _previousMarkMap such that this
     * method will pass, once that has been done (since it refers to objects which are dead in the map used for the
     * object movement)
     * @param env[in] The master GC thread
     */
