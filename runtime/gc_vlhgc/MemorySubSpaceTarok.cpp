 ...
 */
MM_MemoryPool* MM_MemorySubSpaceTarok::getMemoryPool()
{
 ...
 * @return MM_MemoryPool
 */
MM_MemoryPool* MM_MemorySubSpaceTarok::getMemoryPool(void* addr)
{
    MM_MemoryPool* pool = NULL;

    if (NULL != addr) {
        MM_HeapRegionDescriptorVLHGC* descriptor
            = (MM_HeapRegionDescriptorVLHGC*)_heapRegionManager->tableDescriptorForAddress(addr);
 ...
 * @return MM_MemoryPool
 */
MM_MemoryPool* MM_MemorySubSpaceTarok::getMemoryPool(UDATA size)
{
    ...
    MM_EnvironmentBase* env, void* addrBase, void* addrTop, void*& highAddr)
{
    ...
    if ((NULL != addrBase) && (NULL != addrTop)) {
        MM_HeapRegionDescriptorVLHGC* descriptor
            = (MM_HeapRegionDescriptorVLHGC*)_heapRegionManager->tableDescriptorForAddress(addrBase);
        MM_HeapRegionDescriptorVLHGC* highDescriptor
            = (MM_HeapRegionDescriptorVLHGC*)_heapRegionManager->tableDescriptorForAddress((void*)((UDATA)addrTop - 1));
        ...
        if (descriptor->containsObjects()) {
            ...
            if (descriptor != highDescriptor) {
                ...
                /* they requested a single region so set the highAddr to NULL to communicate that it is fully contained
                 * in the region */
                highAddr = NULL;
            }
        }
...
UDATA
MM_MemorySubSpaceTarok::getActiveMemorySize() { return getCurrentSize(); }
...
void MM_MemorySubSpaceTarok::mergeHeapStats(MM_HeapStats* heapStats) { Assert_MM_unreachable(); }

void MM_MemorySubSpaceTarok::mergeHeapStats(MM_HeapStats* heapStats, UDATA includeMemoryType)
{
    _globalAllocationManagerTarok->mergeHeapStats(heapStats, includeMemoryType);
}

void MM_MemorySubSpaceTarok::resetHeapStatistics(bool globalCollect)
{
 ...
 */
MM_AllocationFailureStats* MM_MemorySubSpaceTarok::getAllocationFailureStats()
{
...
void* MM_MemorySubSpaceTarok::allocateObject(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription,
...
void* MM_MemorySubSpaceTarok::allocateArrayletLeaf(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription,
    MM_MemorySubSpace* baseSubSpace, MM_MemorySubSpace* previousSubSpace, bool shouldCollectOnFailure)
{
    Assert_MM_unreachable();
    return NULL;
...
void* MM_MemorySubSpaceTarok::allocationRequestFailed(MM_EnvironmentBase* env,
...
void* MM_MemorySubSpaceTarok::allocateTLH(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription,
    MM_ObjectAllocationInterface* objectAllocationInterface, MM_MemorySubSpace* baseSubSpace,
 ...
 ****************************************
 */
void* MM_MemorySubSpaceTarok::collectorAllocate(
...
void* MM_MemorySubSpaceTarok::collectorAllocateTLH(MM_EnvironmentBase* env, MM_Collector* requestCollector,
    MM_AllocateDescription* allocDescription, UDATA maximumBytesRequired, void*& addrBase, void*& addrTop)
{
    Assert_MM_unreachable();
    return NULL;
}
...
void MM_MemorySubSpaceTarok::abandonHeapChunk(void* addrBase, void* addrTop)
{
    if (addrBase != addrTop) {
        MM_HeapRegionDescriptorVLHGC* base
            = (MM_HeapRegionDescriptorVLHGC*)_heapRegionManager->tableDescriptorForAddress(addrBase);
        MM_HeapRegionDescriptorVLHGC* verify
            = (MM_HeapRegionDescriptorVLHGC*)_heapRegionManager->tableDescriptorForAddress((void*)((UDATA)addrTop - 1));
 ...
 ****************************************
 */
MM_MemorySubSpace* MM_MemorySubSpaceTarok::getDefaultMemorySubSpace() { return this; }

MM_MemorySubSpace* MM_MemorySubSpaceTarok::getTenureMemorySubSpace() { return this; }

bool MM_MemorySubSpaceTarok::isActive()
{
 ...
 */
bool MM_MemorySubSpaceTarok::completeFreelistRebuildRequired(MM_EnvironmentBase* env)
{
    /*
...
void MM_MemorySubSpaceTarok::reset()
{
 ...
 */
void MM_MemorySubSpaceTarok::rebuildFreeList(MM_EnvironmentBase* env) { Assert_MM_unreachable(); }

void MM_MemorySubSpaceTarok::resetLargestFreeEntry()
{
    _globalAllocationManagerTarok->resetLargestFreeEntry();
    Assert_MM_true(NULL == getChildren());
}

void MM_MemorySubSpaceTarok::recycleRegion(MM_EnvironmentBase* env, MM_HeapRegionDescriptor* region)
{
 ...
 * Initialization
 */
MM_MemorySubSpaceTarok* MM_MemorySubSpaceTarok::newInstance(MM_EnvironmentBase* env,
    MM_PhysicalSubArena* physicalSubArena, MM_GlobalAllocationManagerTarok* gamt, bool usesGlobalCollector,
    UDATA minimumSize, UDATA initialSize, UDATA maximumSize, UDATA memoryType, U_32 objectFlags)
{
    MM_MemorySubSpaceTarok* memorySubSpace;

    memorySubSpace = (MM_MemorySubSpaceTarok*)env->getForge()->allocate(
        sizeof(MM_MemorySubSpaceTarok), MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
    if (NULL != memorySubSpace) {
        MM_HeapRegionManager* heapRegionManager = MM_GCExtensions::getExtensions(env)->heapRegionManager;
        new (memorySubSpace) MM_MemorySubSpaceTarok(env, physicalSubArena, gamt, heapRegionManager, usesGlobalCollector,
...
bool MM_MemorySubSpaceTarok::initialize(MM_EnvironmentBase* env)
{
    if (!MM_MemorySubSpace::initialize(env)) {
        return false;
    }

    if (!_expandLock.initialize(
            env, &MM_GCExtensions::getExtensions(env)->lnrlOptions, "MM_MemorySubSpaceTarok:_expandLock")) {
        return false;
    }
...
void MM_MemorySubSpaceTarok::tearDown(MM_EnvironmentBase* env)
{
 ...
 */
bool MM_MemorySubSpaceTarok::expanded(
    MM_EnvironmentBase* env, MM_PhysicalSubArena* subArena, MM_HeapRegionDescriptor* region, bool canCoalesce)
{
    ...
    if (result) {
        ...
        MM_GCExtensions::getExtensions(_extensions)
            ->indexableObjectModel.expandArrayletSubSpaceRange(
        ...
        if (_extensions->tarokEnableExpensiveAssertions) {
            /* dirty the mark map (not assert code per se, but helps enhance other asserts when checkBitsForRegion is
             * called) */
            MM_MarkMapManager* markMapManager
                = ((MM_IncrementalGenerationalGC*)_extensions->getGlobalCollector())->getMarkMapManager();
 ...
 */
void MM_MemorySubSpaceTarok::addExistingMemory(MM_EnvironmentBase* env, MM_PhysicalSubArena* subArena, UDATA size,
 ...
 * @note Size information (current) is not updated.
 */
void* MM_MemorySubSpaceTarok::removeExistingMemory(
    MM_EnvironmentBase* env, MM_PhysicalSubArena* subArena, UDATA contractSize, void* lowAddress, void* highAddress)
{
    /* Routine is normally used to contract within a memory pool (removing free memory) or adjusting barrier ranges,
     * neither of which in this configuration are handled here (a contract is a full pool / region removal, and there is
     * no barrier range "change" as a result of removing regions).
     */

    return lowAddress;
}

MM_HeapRegionDescriptor* MM_MemorySubSpaceTarok::selectRegionForContraction(MM_EnvironmentBase* env, UDATA numaNode)
{
    MM_AllocationContextTarok* allocationContext
        = _globalAllocationManagerTarok->getAllocationContextForNumaNode(numaNode);
...
void* MM_MemorySubSpaceTarok::lockedReplenishAndAllocate(MM_EnvironmentBase* env, MM_AllocationContext* context,
    ...
    AllocationType allocationType)
{
    ...
    void* result
        = tarokContext->lockedReplenishAndAllocate(env, objectAllocationInterface, allocateDescription, allocationType);
    if (NULL == result) {
        Trc_MM_MemorySubSpaceTarok_lockedReplenishAndAllocate_Failure(
            env->getLanguageVMThread(), _bytesRemainingBeforeTaxation);
    } else {
        Trc_MM_MemorySubSpaceTarok_lockedReplenishAndAllocate_Success(
            env->getLanguageVMThread(), result, _bytesRemainingBeforeTaxation);
    }

    return result;
}

void MM_MemorySubSpaceTarok::setBytesRemainingBeforeTaxation(UDATA remaining)
{
    Trc_MM_setBytesRemainingBeforeTaxation(remaining);
    _bytesRemainingBeforeTaxation = remaining;
}

bool MM_MemorySubSpaceTarok::consumeFromTaxationThreshold(MM_EnvironmentBase* env, UDATA bytesToConsume)
{
    bool success = false;
    ...
    do {
        ...
        if (oldBytesRemaining < bytesToConsume) {
            ...
            success = true;
            thresholdUpdated = (MM_AtomicOperations::lockCompareExchange(
                                    &_bytesRemainingBeforeTaxation, oldBytesRemaining, newBytesRemaining)
                == oldBytesRemaining);
        }
...
void* MM_MemorySubSpaceTarok::replenishAllocationContextFailed(MM_EnvironmentBase* env,
    MM_MemorySubSpace* replenishingSpace, MM_AllocationContext* context,
    MM_ObjectAllocationInterface* objectAllocationInterface, MM_AllocateDescription* allocateDescription,
    AllocationType allocationType)
{
    void* result = NULL;
    Trc_MM_MemorySubSpaceTarok_replenishAllocationContextFailed_Entry(
        env->getLanguageVMThread(), context, (UDATA)allocationType, allocateDescription->getContiguousBytes());
     ...
     */
    MM_IncrementalGenerationalGC* collector
        = (MM_IncrementalGenerationalGC*)MM_GCExtensions::getExtensions(env)->getGlobalCollector();
    ...
    if (!env->acquireExclusiveVMAccessForGC(collector, true, true)) {
        ...
        /* don't have exclusive access - another thread beat us to the GC.  retry the allocate using the standard
         * locking path */
        ...
        if (NULL == result) {
            ...
            if (!env->acquireExclusiveVMAccessForGC(collector)) {
                allocateDescription->restoreObjects(env);
                /* now that we have exclusive, see if there is memory to satisfy the allocate (since we might not be the
                 * first to get in here) */
    ...
    if (NULL == result) {
        ...
        if (0 == _bytesRemainingBeforeTaxation) {
            ...
            allocateDescription->restoreObjects(env);
            result = lockedAllocate(env, context, objectAllocationInterface, allocateDescription, allocationType);
            Trc_MM_MemorySubSpaceTarok_replenishAllocationContextFailed_didPerformTaxationAndReplenish(
                env->getLanguageVMThread(), context, (UDATA)allocationType, allocateDescription->getContiguousBytes(),
                result);
        }
    }
    if (NULL == result) {
        ...
        result = lockedAllocate(env, context, objectAllocationInterface, allocateDescription, allocationType);
        Trc_MM_MemorySubSpaceTarok_replenishAllocationContextFailed_didPerformResizeAndReplenish(
            env->getLanguageVMThread(), context, (UDATA)allocationType, allocateDescription->getContiguousBytes(),
            result);
        if (NULL == result) {
            ...
            result = collector->garbageCollect(env, this, allocateDescription, J9MMCONSTANT_IMPLICIT_GC_DEFAULT,
                objectAllocationInterface, replenishingSpace, context);
            Trc_MM_MemorySubSpaceTarok_replenishAllocationContextFailed_didPerformCollect(env->getLanguageVMThread(),
                context, (UDATA)allocationType, allocateDescription->getContiguousBytes(), result);
            allocateDescription->restoreObjects(env);
            if (NULL == result) {
                /* we _still_ failed so invoke an aggressive collect */
                allocateDescription->saveObjects(env);
                result = collector->garbageCollect(env, this, allocateDescription, J9MMCONSTANT_IMPLICIT_GC_AGGRESSIVE,
                    objectAllocationInterface, replenishingSpace, context);
                Trc_MM_MemorySubSpaceTarok_replenishAllocationContextFailed_didPerformAggressiveCollect(
                    env->getLanguageVMThread(), context, (UDATA)allocationType,
                    allocateDescription->getContiguousBytes(), result);
                allocateDescription->restoreObjects(env);
            }
        }
...
void* MM_MemorySubSpaceTarok::lockedAllocate(MM_EnvironmentBase* env, MM_AllocationContext* context,
    MM_ObjectAllocationInterface* objectAllocationInterface, MM_AllocateDescription* allocateDescription,
    AllocationType allocationType)
{
    void* result = NULL;

    /* arraylet leaves need to be allocated directly after a new region has been found so fall through to the replenish
     * path in that case */
#if defined(J9VM_GC_ARRAYLETS)
    if (MM_MemorySubSpace::ALLOCATION_TYPE_LEAF != allocationType)
#endif /* defined(J9VM_GC_ARRAYLETS) */
    {
        result = ((MM_AllocationContextTarok*)context)
                     ->lockedAllocate(env, objectAllocationInterface, allocateDescription, allocationType);
    }
    if (NULL == result) {
        /* we failed so do the locked replenish */
        result
            = lockedReplenishAndAllocate(env, context, objectAllocationInterface, allocateDescription, allocationType);
    }
...
UDATA
MM_MemorySubSpaceTarok::collectorExpand(
    MM_EnvironmentBase* env, MM_Collector* requestCollector, MM_AllocateDescription* allocDescription)
{
 ...
 * @return the actual amount of resize (having IDATA return result will contain valid value only if contract/expand size
 * is half of maxOfUDATA)
 */
IDATA
 ...
 */
void MM_MemorySubSpaceTarok::checkResize(
 ...
 */
bool MM_MemorySubSpaceTarok::timeForHeapExpand(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription)
{
    bool doExpand = false;
 ...
 */
bool MM_MemorySubSpaceTarok::timeForHeapContract(
    MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription, bool systemGC)
{
    ...
    if (NULL != allocDescription) {
        ...
        if (sizeInRegionsRequired >= freeRegionCount) {
            Trc_MM_MemorySubSpaceTarok_timeForHeapContract_Exit4(
                env->getLanguageVMThread(), sizeInRegionsRequired, freeRegionCount);
            _contractionSize = 0;
            return false;
        }
    }
    ...
    if (systemGC) {
        ...
        if (freeBytesAtSystemGCStart < minimumFree) {
            Trc_MM_MemorySubSpaceTarok_timeForHeapContract_Exit6(
...
{
    Trc_MM_MemorySubSpaceTarok_calculateTargetContractSize_Entry(
    ...
    if (allocSize > getApproximateActiveFreeMemorySize()) {
        ...
        UDATA maximumFreePercent = ratioContract
        ...
        if (currentFree > maximumFree) {
            ...
            if (currentHeapSize < targetHeapSize) {
                ...
                Trc_MM_MemorySubSpaceTarok_calculateTargetContractSize_Event1(
                ...
                Trc_MM_MemorySubSpaceTarok_calculateTargetContractSize_Event2(
...
MM_MemorySubSpaceTarok::calculateExpandSize(MM_EnvironmentBase* env, UDATA bytesRequired, bool expandToSatisfy)
{
    ...
    Trc_MM_MemorySubSpaceTarok_calculateExpandSize_Exit1(
...
MM_MemorySubSpaceTarok::checkForRatioExpand(MM_EnvironmentBase* env, UDATA bytesRequired)
{
    ...
    if (gcPercentage < _extensions->heapExpansionGCTimeThreshold) {
        ...
        ratioExpandAmount = (UDATA)(
            ((U_64)getActiveMemorySize() * HEAP_FREE_RATIO_EXPAND_MULTIPLIER) / ((U_64)HEAP_FREE_RATIO_EXPAND_DIVISOR));
        ...
        Trc_MM_MemorySubSpaceTarok_checkForRatioExpand_Exit3(
 ...
 */
bool MM_MemorySubSpaceTarok::checkForRatioContract(MM_EnvironmentBase* env)
{
 ...
 */
MMINLINE UDATA MM_MemorySubSpaceTarok::adjustExpansionWithinFreeLimits(MM_EnvironmentBase* env, UDATA expandSize)
{
 ...
 * @return Updated expand size
 */
MMINLINE UDATA MM_MemorySubSpaceTarok::adjustExpansionWithinSoftMax(
    MM_EnvironmentBase* env, UDATA expandSize, UDATA minimumBytesRequired)
{
    ...
    if (0 != actualSoftMx) {
        if ((minimumBytesRequired != 0) && ((activeMemorySize + minimumBytesRequired) > actualSoftMx)) {
            if (J9_EVENT_IS_HOOKED(
                    MM_GCExtensions::getExtensions(env)->omrHookInterface, J9HOOK_MM_OMR_OOM_DUE_TO_SOFTMX)) {
                ALWAYS_TRIGGER_J9HOOK_MM_OMR_OOM_DUE_TO_SOFTMX(MM_GCExtensions::getExtensions(env)->omrHookInterface,
                    env->getOmrVMThread(), j9time_hires_clock(), heap->getMaximumMemorySize(),
                    heap->getActiveMemorySize(), MM_GCExtensions::getExtensions(env)->softMx, minimumBytesRequired);
