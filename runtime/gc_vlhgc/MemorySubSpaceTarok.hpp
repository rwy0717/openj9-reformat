...
private:
    MM_GlobalAllocationManagerTarok*
        _globalAllocationManagerTarok; /**< Provides the API for accessing information about the underlying regions
                                          (owned by the AllocationContextTarok instances) */
    bool _allocateAtSafePointOnly;
    volatile UDATA _bytesRemainingBeforeTaxation; /**< The number of bytes which this subspace can use for allocation
                                                     before triggering an early allocation failure */

    MM_HeapRegionManager* _heapRegionManager; /**< Stored so that we can resolve the table descriptor for given
                                                 addresses when asked for a pool corresponding to a specific address */
    MM_LightweightNonReentrantLock
        _expandLock; /**< Most of the common expand code is not multi-threaded safe (since it used in standard
                        collectors on alloc path fail path which is single threaded)  */
...
private:
     ...
     */
    void* lockedAllocate(MM_EnvironmentBase* env, MM_AllocationContext* context,
...
public:
    static MM_MemorySubSpaceTarok* newInstance(MM_EnvironmentBase* env, MM_PhysicalSubArena* physicalSubArena,
        MM_GlobalAllocationManagerTarok* gamt, bool usesGlobalCollector, UDATA minimumSize, UDATA initialSize,
        UDATA maximumSize, UDATA memoryType, U_32 objectFlags);
    ...
    virtual UDATA largestDesirableArraySpine() { return _extensions->getOmrVM()->_arrayletLeafSize; }
    ...
    virtual void addExistingMemory(MM_EnvironmentBase* env, MM_PhysicalSubArena* subArena, UDATA size, void* lowAddress,
        void* highAddress, bool canCoalesce);
    virtual void* removeExistingMemory(
        MM_EnvironmentBase* env, MM_PhysicalSubArena* subArena, UDATA size, void* lowAddress, void* highAddress);
     ...
     * @param[in] replenishingSpace The subspace which was initially asked to fill the replenishment request and which
     * will be aksed to replenish it once the GC is complete
     ...
     */
    void* replenishAllocationContextFailed(MM_EnvironmentBase* env, MM_MemorySubSpace* replenishingSpace,
        MM_AllocationContext* context, MM_ObjectAllocationInterface* objectAllocationInterface,
     ...
     */
    MM_MemorySubSpaceTarok(MM_EnvironmentBase* env, MM_PhysicalSubArena* physicalSubArena,
        MM_GlobalAllocationManagerTarok* gamt, MM_HeapRegionManager* heapRegionManager, bool usesGlobalCollector,
