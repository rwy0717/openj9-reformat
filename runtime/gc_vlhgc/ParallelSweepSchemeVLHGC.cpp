 ...
 */
void MM_ParallelSweepVLHGCTask::run(MM_EnvironmentBase* envBase)
{
 ...
 */
void MM_ParallelSweepVLHGCTask::setup(MM_EnvironmentBase* envBase)
{
    MM_EnvironmentVLHGC* env = MM_EnvironmentVLHGC::getEnvironment(envBase);
 ...
 */
void MM_ParallelSweepVLHGCTask::cleanup(MM_EnvironmentBase* envBase)
{
    MM_EnvironmentVLHGC* env = MM_EnvironmentVLHGC::getEnvironment(envBase);
    ...
    Trc_MM_ParallelSweepVLHGCTask_parallelStats(env->getLanguageVMThread(), (U_32)env->getSlaveID(),
...
void MM_ParallelSweepVLHGCTask::masterSetup(MM_EnvironmentBase* env) { _sweepScheme->setCycleState(_cycleState); }

void MM_ParallelSweepVLHGCTask::masterCleanup(MM_EnvironmentBase* envBase)
{
 ...
 */
void MM_ParallelSweepVLHGCTask::synchronizeGCThreads(MM_EnvironmentBase* envBase, const char* id)
{
    MM_EnvironmentVLHGC* env = MM_EnvironmentVLHGC::getEnvironment(envBase);
    PORT_ACCESS_FROM_ENVIRONMENT(env);
 ...
 */
bool MM_ParallelSweepVLHGCTask::synchronizeGCThreadsAndReleaseMaster(MM_EnvironmentBase* envBase, const char* id)
{
    MM_EnvironmentVLHGC* env = MM_EnvironmentVLHGC::getEnvironment(envBase);
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    U_64 startTime = j9time_hires_clock();
...
bool MM_ParallelSweepVLHGCTask::synchronizeGCThreadsAndReleaseSingleThread(MM_EnvironmentBase* envBase, const char* id)
{
    MM_EnvironmentVLHGC* env = MM_EnvironmentVLHGC::getEnvironment(envBase);
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    U_64 startTime = j9time_hires_clock();
 ...
 */
MM_ParallelSweepSchemeVLHGC* MM_ParallelSweepSchemeVLHGC::newInstance(MM_EnvironmentVLHGC* env)
{
    MM_ParallelSweepSchemeVLHGC* sweepScheme;

    sweepScheme = (MM_ParallelSweepSchemeVLHGC*)env->getForge()->allocate(
        sizeof(MM_ParallelSweepSchemeVLHGC), MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
    if (sweepScheme) {
 ...
 */
void MM_ParallelSweepSchemeVLHGC::kill(MM_EnvironmentVLHGC* env)
{
    tearDown(env);
 ...
 */
bool MM_ParallelSweepSchemeVLHGC::initialize(MM_EnvironmentVLHGC* env)
{
 ...
 */
void MM_ParallelSweepSchemeVLHGC::tearDown(MM_EnvironmentVLHGC* env)
{
    if (NULL != _sweepHeapSectioning) {
 ...
 */
void* MM_ParallelSweepSchemeVLHGC::createSweepPoolState(MM_EnvironmentBase* env, MM_MemoryPool* memoryPool)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);

    omrthread_monitor_enter(_mutexSweepPoolState);
    if (NULL == _poolSweepPoolState) {
        _poolSweepPoolState = pool_new(sizeof(MM_SweepPoolState), 0, 2 * sizeof(UDATA), 0, J9_GET_CALLSITE(),
            OMRMEM_CATEGORY_MM, POOL_FOR_PORT(PORTLIB));
        if (NULL == _poolSweepPoolState) {
 ...
 */
void MM_ParallelSweepSchemeVLHGC::deleteSweepPoolState(MM_EnvironmentBase* env, void* sweepPoolState)
{
 ...
 */
MM_SweepPoolState* MM_ParallelSweepSchemeVLHGC::getPoolState(MM_MemoryPool* memoryPool)
{
 ...
 */
bool MM_ParallelSweepSchemeVLHGC::heapAddRange(
 ...
 * @return true if operation completes with success (always)
 */
bool MM_ParallelSweepSchemeVLHGC::heapRemoveRange(MM_EnvironmentVLHGC* env, MM_MemorySubSpace* subspace, UDATA size,
 ...
 *
 */
void MM_ParallelSweepSchemeVLHGC::heapReconfigured(MM_EnvironmentVLHGC* env) { _sweepHeapSectioning->update(env); }
 ...
 */
void MM_ParallelSweepSchemeVLHGC::setupForSweep(MM_EnvironmentVLHGC* env)
{
...
MMINLINE void MM_ParallelSweepSchemeVLHGC::sweepMarkMapBody(UDATA*& markMapCurrent, UDATA*& markMapChunkTop,
    UDATA*& markMapFreeHead, UDATA& heapSlotFreeCount, UDATA*& heapSlotFreeCurrent, UDATA*& heapSlotFreeHead)
{
...
MMINLINE void MM_ParallelSweepSchemeVLHGC::sweepMarkMapHead(
...
MMINLINE void MM_ParallelSweepSchemeVLHGC::sweepMarkMapTail(
    UDATA* markMapCurrent, UDATA* markMapChunkTop, UDATA& heapSlotFreeCount)
{
...
UDATA
MM_ParallelSweepSchemeVLHGC::performSamplingCalculations(
    MM_ParallelSweepChunk* sweepChunk, UDATA* markMapCurrent, UDATA* heapSlotFreeCurrent)
{
    ...
    UDATA prevObjectSize = _extensions->objectModel.getConsumedSizeInBytesWithHeader(prevObject);
    /* Any object we visit for dark matter estimate, we'll consider for other estimates too (for example scannable vs
     * non-scannable ratio) */
    ...
    UDATA* startSearchAt
        = (UDATA*)MM_Math::roundToFloor(J9MODRON_HEAP_SLOTS_PER_MARK_SLOT * sizeof(UDATA), (UDATA)endOfPrevObject);
    UDATA* endSearchAt = (UDATA*)MM_Math::roundToCeiling(
        J9MODRON_HEAP_SLOTS_PER_MARK_SLOT * sizeof(UDATA), (UDATA)endOfPrevObject + minimumFreeEntrySize);
 ...
 */
bool MM_ParallelSweepSchemeVLHGC::sweepChunk(MM_EnvironmentVLHGC* env, MM_ParallelSweepChunk* sweepChunk)
{
    ...
    markMapChunkBase = (UDATA*)(_currentSweepBits
        + (MM_Math::roundToFloor(
               J9MODRON_HEAP_SLOTS_PER_MARK_SLOT * sizeof(UDATA), (UDATA)sweepChunk->chunkBase - (UDATA)_heapBase)
              / J9MODRON_HEAP_SLOTS_PER_MARK_SLOT));
    markMapChunkTop = (UDATA*)(_currentSweepBits
        + (MM_Math::roundToFloor(
               J9MODRON_HEAP_SLOTS_PER_MARK_SLOT * sizeof(UDATA), ((UDATA)sweepChunk->chunkTop) - ((UDATA)_heapBase))
              / J9MODRON_HEAP_SLOTS_PER_MARK_SLOT));
     ...
     */
    Trc_MM_ParallelSweepScheme_sweepChunk_darkMatterComparison(env->getLanguageVMThread(), sweepChunk->chunkBase,
        sweepChunk->chunkTop, darkMatterSamples, darkMatterCandidates, sweepChunk->_darkMatterBytes,
 ...
 */
void MM_ParallelSweepSchemeVLHGC::sweepAllChunks(MM_EnvironmentVLHGC* env, UDATA totalChunkCount)
{
 ...
 */
void MM_ParallelSweepSchemeVLHGC::connectChunk(MM_EnvironmentVLHGC* env, MM_ParallelSweepChunk* chunk)
{
...
void MM_ParallelSweepSchemeVLHGC::connectAllChunks(MM_EnvironmentVLHGC* env, UDATA totalChunkCount)
{
 ...
 */
void MM_ParallelSweepSchemeVLHGC::initializeSweepStates(MM_EnvironmentBase* envModron)
{
 ...
 */
void MM_ParallelSweepSchemeVLHGC::flushFinalChunk(MM_EnvironmentBase* envModron, MM_MemoryPool* memoryPool)
{
    MM_SweepPoolManager* sweepPoolManager = memoryPool->getSweepPoolManager();
 ...
 */
void MM_ParallelSweepSchemeVLHGC::flushAllFinalChunks(MM_EnvironmentBase* env)
{
    GC_HeapRegionIteratorVLHGC regionIterator(_regionManager);
    MM_HeapRegionDescriptorVLHGC* region = NULL;
    while (NULL != (region = regionIterator.nextRegion())) {
 ...
 */
void MM_ParallelSweepSchemeVLHGC::internalSweep(MM_EnvironmentVLHGC* env)
{
 ...
 */
void MM_ParallelSweepSchemeVLHGC::sweep(MM_EnvironmentVLHGC* env)
{
    setupForSweep(env);
 ...
 */
void MM_ParallelSweepSchemeVLHGC::completeSweep(MM_EnvironmentBase* env, SweepCompletionReason reason)
{
 ...
 */
bool MM_ParallelSweepSchemeVLHGC::sweepForMinimumSize(
 ...
 */
bool MM_ParallelSweepSchemeVLHGC::replenishPoolForAllocate(
...
void MM_ParallelSweepSchemeVLHGC::recycleFreeRegions(MM_EnvironmentVLHGC* env)
{
...
void MM_ParallelSweepSchemeVLHGC::updateProjectedLiveBytesAfterSweep(MM_EnvironmentVLHGC* env)
{
    MM_HeapRegionDescriptorVLHGC* region = NULL;
