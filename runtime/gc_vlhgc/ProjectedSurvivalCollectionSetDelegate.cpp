...
MM_ProjectedSurvivalCollectionSetDelegate::MM_ProjectedSurvivalCollectionSetDelegate(
...
bool MM_ProjectedSurvivalCollectionSetDelegate::initialize(MM_EnvironmentVLHGC* env)
{
    if (_extensions->tarokEnableDynamicCollectionSetSelection) {
        ...
        _setSelectionDataTable = (SetSelectionData*)env->getForge()->allocate(
            tableAllocationSizeInBytes, MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
        ...
        _dynamicSelectionList = (SetSelectionData**)env->getForge()->allocate(
            sizeof(SetSelectionData*) * setSelectionEntryCount, MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
        if (NULL == _dynamicSelectionList) {
            goto error_no_memory;
        }
        _dynamicSelectionRegionList = (MM_HeapRegionDescriptorVLHGC**)env->getForge()->allocate(
            sizeof(MM_HeapRegionDescriptorVLHGC*) * regionCount, MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
...
void MM_ProjectedSurvivalCollectionSetDelegate::tearDown(MM_EnvironmentVLHGC* env)
{
    if (NULL != _setSelectionDataTable) {
 ...
 */
int MM_ProjectedSurvivalCollectionSetDelegate::compareRateOfReturnScoreFunc(const void* element1, const void* element2)
{
 ...
 */
int MM_ProjectedSurvivalCollectionSetDelegate::compareCoreSampleScoreFunc(const void* element1, const void* element2)
{
    MM_ProjectedSurvivalCollectionSetDelegate::SetSelectionData* coreSample1
        = *(MM_ProjectedSurvivalCollectionSetDelegate::SetSelectionData**)element1;
    MM_ProjectedSurvivalCollectionSetDelegate::SetSelectionData* coreSample2
        = *(MM_ProjectedSurvivalCollectionSetDelegate::SetSelectionData**)element2;
...
MM_ProjectedSurvivalCollectionSetDelegate::createNurseryCollectionSet(MM_EnvironmentVLHGC* env)
{
    ...
    Trc_MM_CollectionSetDelegate_createNurseryCollectionSet_Entry(
        env->getLanguageVMThread(), dynamicCollectionSet ? "true" : "false");
    ...
    while (NULL != (region = regionIterator.nextRegion())) {
        ...
        if (region->containsObjects()) {
            ...
            if (region->getRememberedSetCardList()->isAccurate()
                && (!isSelectionForCopyForward || !regionHasCriticalRegions
                       || (regionHasCriticalRegions && region->isEden()))) {
                ...
                /* Add the region to appropriate dynamic collection set data age group (building up information for
                 * later use) */
...
void MM_ProjectedSurvivalCollectionSetDelegate::selectRegion(
    MM_EnvironmentVLHGC* env, MM_HeapRegionDescriptorVLHGC* region)
{
    ...
    Trc_MM_CollectionSetDelegate_selectRegionsForBudget(env->getLanguageVMThread(), tableIndex, compactGroup,
        (100 * freeMemory) / regionSize, (100 * projectedFreeMemoryAfterGC) / regionSize,
        (100 * projectedReclaimableBytes) / regionSize);
}

UDATA
MM_ProjectedSurvivalCollectionSetDelegate::selectRegionsForBudget(
    MM_EnvironmentVLHGC* env, UDATA ageGroupBudget, SetSelectionData* setSelectionData)
{
    ...
    /* Walk the rate of return age group using a remainder skip-system to select the appropriate % of regions to collect
     * (based on budget) */
    ...
    Trc_MM_CollectionSetDelegate_selectRegionsForBudget_Exit(
        env->getLanguageVMThread(), ageGroupBudget - ageGroupBudgetRemaining);
    return ageGroupBudgetRemaining;
}

void MM_ProjectedSurvivalCollectionSetDelegate::createRateOfReturnCollectionSet(
    MM_EnvironmentVLHGC* env, UDATA nurseryRegionCount)
{
    ...
    J9_SORT(
        _dynamicSelectionRegionList, sortListSize, sizeof(MM_HeapRegionDescriptorVLHGC*), compareRateOfReturnScoreFunc);
    ...
    Trc_MM_CollectionSetDelegate_createRegionCollectionSetForPartialGC_dynamicRegionSelectionBudget(
        env->getLanguageVMThread(), nurseryRegionCount, regionBudget);
    ...
    while ((0 != regionBudget) && (regionIndex < sortListSize)) {
        ...
        if (projectedReclaimableBytesFraction > _extensions->tarokCopyForwardFragmentationTarget) {
            ...
            /* Since _dynamicSelectionRegionList is sorted by projectedReclaimableBytes, they'll be no more regions to
             * select so break */
            break;
        }
        regionIndex += 1;
    ...
    Trc_MM_CollectionSetDelegate_createRegionCollectionSetForPartialGC_dynamicRegionSelectionBudgetRemaining(
        env->getLanguageVMThread(), regionBudget);
}

void MM_ProjectedSurvivalCollectionSetDelegate::createCoreSamplingCollectionSet(
    MM_EnvironmentVLHGC* env, UDATA nurseryRegionCount)
{
    /* Collect and sort all regions into the core sample buckets so that we can find them quickly (this is an
     * optimization) */
    ...
    for (UDATA compactGroup = 0; compactGroup < compactGroupCount; compactGroup++) {
        if (MM_CompactGroupManager::isCompactGroupDCSSCandidate(env, compactGroup)) {
            ...
            if (!tableEntry->_dynamicSelectionThisCycle && (0 != tableEntry->_regionCount)) {
                /* The region is a collection candidate and eligible for dynamic selection - count it as part of the
                 * total possible core sampling group */
    ...
    /* Given a region budget, select regions for core sampling according to population and whether the age group has
     * participated in other dynamically added activities.
     */
    UDATA regionBudget = 0;
    ...
    Trc_MM_CollectionSetDelegate_createRegionCollectionSetForPartialGC_coreSamplingBudget(
        env->getLanguageVMThread(), totalCoreSampleRegions, regionBudget);
    ...
    while ((regionBudget != 0) && (coreSampleIndex < sortListSize)) {
        ...
        UDATA compactGroupBudget
            = (UDATA)(((double)regionBudget) * ((double)coreSample->_regionCount) / ((double)totalCoreSampleRegions));
        ...
        Trc_MM_CollectionSetDelegate_createRegionCollectionSetForPartialGC_coreSamplingSelection(
            env->getLanguageVMThread(), compactGroup, coreSample->_regionCount, compactGroupBudget, budgetConsumed);
...
void MM_ProjectedSurvivalCollectionSetDelegate::createRegionCollectionSetForPartialGC(MM_EnvironmentVLHGC* env)
{
    Assert_MM_true(MM_CycleState::CT_PARTIAL_GARBAGE_COLLECTION == env->_cycleState->_collectionType);
    ...
    if (dynamicCollectionSet) {
        ...
        for (UDATA compactGroup = 0; compactGroup < compactGroupCount; compactGroup++) {
            ...
            /* survival rate can be more than 100% if the compact group is being used as a migration target due to NUMA
             * proximity */
    ...
    if (dynamicCollectionSet) {
        ...
        /* Clean up any linkage data that was computed during set selection but will potentially become stale over the
         * course of the run */
...
void MM_ProjectedSurvivalCollectionSetDelegate::deleteRegionCollectionSetForPartialGC(MM_EnvironmentVLHGC* env)
{
    Assert_MM_true(MM_CycleState::CT_PARTIAL_GARBAGE_COLLECTION == env->_cycleState->_collectionType);
...
void MM_ProjectedSurvivalCollectionSetDelegate::createRegionCollectionSetForGlobalGC(MM_EnvironmentVLHGC* env)
{
...
void MM_ProjectedSurvivalCollectionSetDelegate::deleteRegionCollectionSetForGlobalGC(MM_EnvironmentVLHGC* env)
{
    Assert_MM_true(MM_CycleState::CT_GLOBAL_GARBAGE_COLLECTION == env->_cycleState->_collectionType);
...
MM_HeapRegionDescriptorVLHGC* MM_ProjectedSurvivalCollectionSetDelegate::getNextRegion(
    MM_HeapRegionDescriptorVLHGC* cursor)
{
...
void MM_ProjectedSurvivalCollectionSetDelegate::rateOfReturnCalculationBeforeSweep(MM_EnvironmentVLHGC* env)
{
    if (_extensions->tarokEnableDynamicCollectionSetSelection) {
        UDATA compactGroupCount = MM_CompactGroupManager::getCompactGroupMaxCount(env);
        for (UDATA compactGroup = 0; compactGroup < compactGroupCount; compactGroup++) {
        ...
        /* Walk and count all regions, and count any region that will be included in the sweep set (those in trace or
         * those that require re-sweeping due to a GMP) */
        GC_HeapRegionIteratorVLHGC regionIterator(_regionManager, MM_HeapRegionDescriptor::ALL);
        MM_HeapRegionDescriptorVLHGC* region = NULL;
        while (NULL != (region = regionIterator.nextRegion())) {
            if (region->containsObjects()) {
                SetSelectionData* stats
                    = &_setSelectionDataTable[MM_CompactGroupManager::getCompactGroupNumber(env, region)];
                MM_MemoryPoolBumpPointer* memoryPool = (MM_MemoryPoolBumpPointer*)region->getMemoryPool();
                ...
                MM_HeapRegionDescriptorVLHGC* parentRegion
                    = (MM_HeapRegionDescriptorVLHGC*)_regionManager->regionDescriptorForAddress(
                        (void*)region->_allocateData.getSpine());
                Assert_MM_true(parentRegion->containsObjects());
                SetSelectionData* stats
                    = &_setSelectionDataTable[MM_CompactGroupManager::getCompactGroupNumber(env, parentRegion)];
...
void MM_ProjectedSurvivalCollectionSetDelegate::rateOfReturnCalculationAfterSweep(MM_EnvironmentVLHGC* env)
{
    if (_extensions->tarokEnableDynamicCollectionSetSelection) {
        ...
        /* We now have an expected change as a result of tracing and sweeping (parts of) the heap.  Calculate the
         * rate-of-return (ROR) on tracing for age groups where work was done. Use a weighted running average to
         * calculate the ROR, where the weight is the % of regions in an age group that we are examining. NOTE: We leave
         * the ROR for the last age group as 0.
         */
        UDATA compactGroupCount = MM_CompactGroupManager::getCompactGroupMaxCount(env);

        for (UDATA compactGroup = 0; compactGroup < compactGroupCount; compactGroup++) {
            if (MM_CompactGroupManager::getRegionAgeFromGroup(env, compactGroup) < _extensions->tarokRegionMaxAge) {
                /* We set the compact group for each entry every time through a table.  This is overkill, but allows us
                 * to be sure when examining a ROR element whether in fact it represents the compact group with think it
                 * does.
                 */
                SetSelectionData* stats = &_setSelectionDataTable[compactGroup];
                ...
                if (0 == stats->_reclaimStats._reclaimableRegionCountBefore) {
                    ...
                    Assert_MM_true(
                        stats->_reclaimStats._regionCountBefore >= stats->_reclaimStats._reclaimableRegionCountBefore);
                    Assert_MM_true(stats->_reclaimStats._regionCountBefore >= stats->_reclaimStats._regionCountAfter);
                    Assert_MM_true(stats->_reclaimStats._reclaimableRegionCountBefore
                        >= stats->_reclaimStats._reclaimableRegionCountAfter);

                    UDATA bytesConsumedBefore
                        = (stats->_reclaimStats._reclaimableRegionCountBefore * _extensions->regionSize)
                        - stats->_reclaimStats._regionBytesFreeBefore - stats->_reclaimStats._regionDarkMatterBefore;
                    ...
                    UDATA bytesConsumedAfter
                        ...
                        - stats->_reclaimStats._regionBytesFreeAfter - stats->_reclaimStats._regionDarkMatterAfter;
