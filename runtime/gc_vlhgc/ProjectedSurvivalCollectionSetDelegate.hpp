...
public:
    ...
    public:
        ...
        UDATA _regionCountArrayletLeafBefore; /**< Number of regions that are arraylet leafs in the age group before a
                                                 sweep */
        UDATA _regionCountArrayletLeafAfter; /**< Number of regions that are arraylet leafs in the age group after a
                                                sweep */
        ...
        UDATA _regionDarkMatterBefore; /**< Number of free bytes (fragmented) in the age group after a trace and sweep
                                          have concluded */
        UDATA _regionBytesFreeAfter; /**< Number of dark matter bytes in the age group before a sweep event */
        UDATA _regionDarkMatterAfter; /**< Number of dark matter bytes in the age group after a trace and sweep have
                                         concluded */

        UDATA _reclaimableRegionCountBefore; /**< Number of regions to be included in the sweep set (includes tracing
                                                and recently traced but unswept due to GMP) */
        UDATA _reclaimableRegionCountAfter; /**< Number of regions in the reclaimable set after having been swept */
        UDATA _reclaimableRegionCountArrayletLeafBefore; /**< Number of regions that are arraylet leafs in the age group
                                                            before a sweep */
        UDATA _reclaimableRegionCountArrayletLeafAfter; /**< Number of regions that are arraylet leafs in the age group
                                                           after a sweep */

        UDATA _reclaimableBytesConsumedBefore; /**< Number of bytes consumed before a sweep in the set of regions marked
                                                  as part of the reclaimable set */
        UDATA _reclaimableBytesConsumedAfter; /**< Number of bytes consumed after a sweep in the set of regions marked
                                                 as part of the reclaimable set */
    ...
    public:
        ...
        MM_HeapRegionDescriptorVLHGC* _regionList; /**< List of regions associated to the compact group (NOTE: valid
                                                      only during collection set building) */
        UDATA _regionCount; /**< The number of regions that appear in the region list (NOTE: valid only during
                               collection set building) */

        RegionReclaimableStats _reclaimStats; /**< general reclaim statistics for the age group that are used for
                                                 calculations and diagnostics */
        double _rateOfReturn; /**< The fractional percentage of the expected ROR when tracing the entire set of regions
                                 at the corresponding age */
        bool _dynamicSelectionThisCycle; /**< Flag indicating if the age group has regions that were dynamically
...
private:
    ...
    SetSelectionData*
        _setSelectionDataTable; /**< Storage table for set selection statistics and variables (grouped by age) */
    SetSelectionData** _dynamicSelectionList; /**< Pointer table used for sorting or iterating over candidate dynamic
                                                 selection elements */

    MM_HeapRegionDescriptorVLHGC**
        _dynamicSelectionRegionList; /**< Pointer table used for sorting or iterating over regions */
...
private:
     ...
     * Find all regions whose age allow them to be counted as part of the nursery and, if the region is collectable
     * (contains objects and isn't in an RSCL overflow state) add it to the collection set.
     ...
     * Given a set selection age group and a budget, use an form of counting to select the budgeted number of regions
     * available in the age group. The counting attempts to evenly distribute the selection of regions across all
     * regions in the age group.
     * @param env[in] The master GC thread
    ...
    /**
     * Include a set of regions, based on rate of return calculations, outside of the nursery for collection set
     * purposes. Given the total regions in the nursery, select a number of regions outside the nursery for inclusion in
     * the PartialGC collection set. The selection will be past on historical rate of return (ROR) percentages, regions
     * with higher ROR values being selected first.
     * @param env[in] The master GC thread
    ...
    /**
     * Include a set of regions, base on not being selected for collection and having a high age group population count,
     * for collection set purposes. Find region age groups that have not participated in nursery or ROR collection set
     * selection, and select a set of regions for collection set sampling purposes.  The aim is to find age groups where
     * collection opportunities might exist (increasing the ROR and being dynamically selected).
     * @param env[in] The master GC thread
     * @param nurseryRegionCount[in] Number of regions selected as the core nursery collection set
     */
