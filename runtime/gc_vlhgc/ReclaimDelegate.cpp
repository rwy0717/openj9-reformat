...
MM_ReclaimDelegate::MM_ReclaimDelegate(
    MM_EnvironmentBase* env, MM_HeapRegionManager* manager, MM_CollectionSetDelegate* collectionSetDelegate)
    : MM_BaseNonVirtual()
...
bool MM_ReclaimDelegate::initialize(MM_EnvironmentVLHGC* env)
{
    ...
    if (extensions->tarokEnableScoreBasedAtomicCompact) {
        _compactGroups = (MM_ReclaimDelegate_ScoreBaseCompactTable*)j9mem_allocate_memory(
            sizeof(MM_ReclaimDelegate_ScoreBaseCompactTable) * _compactGroupMaxCount, OMRMEM_CATEGORY_MM);
        if (NULL == _compactGroups) {
            goto error_no_memory;
        }
    }

    _regionsSortedByEmptinessArray = (MM_HeapRegionDescriptorVLHGC**)j9mem_allocate_memory(
        sizeof(MM_HeapRegionDescriptorVLHGC*) * regionCount, OMRMEM_CATEGORY_MM);
...
void MM_ReclaimDelegate::tearDown(MM_EnvironmentVLHGC* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    _dispatcher = NULL;
...
void MM_ReclaimDelegate::tagRegionsBeforeSweep(MM_EnvironmentVLHGC* env)
{
    /* NOTE:  This method currently only provided to symmetry.
     *
     * This method currently does nothing since sweep will sweep all regions which have not already been swept but have
     * a valid mark map. In the future, this may be incrementalized:  see JAZZ 20233
     */
}
...
UDATA
MM_ReclaimDelegate::tagRegionsBeforeCompactWithWorkGoal(
    MM_EnvironmentVLHGC* env, bool isCopyForward, UDATA desiredWorkToDo, UDATA* skippedRegionCountRequiringSweep)
{
    ...
    Trc_MM_ReclaimDelegate_tagRegionsBeforeCompactWithWorkGoal_searching(
        env->getLanguageVMThread(), _currentSortedRegionCount);
    ...
    for (UDATA i = 0; (i < _currentSortedRegionCount) && !maxBytesToMoveReached; i++) {
        ...
        if (0.0 < region->_compactData._compactScore) {
            ...
            Trc_MM_ReclaimDelegate_tagRegionsBeforeCompactWithWorkGoal_addingToCompactSet(env->getLanguageVMThread(),
                regionIndex, compactGroup, region->getLogicalAge(), region->_compactData._compactScore,
                (100 * freeMemory) / regionSize,
                MM_GCExtensions::getExtensions(env)->compactGroupPersistentStats[compactGroup]._weightedSurvivalRate);
    ...
    Trc_MM_ReclaimDelegate_tagRegionsBeforeCompactWithWorkGoal_Exit(
        env->getLanguageVMThread(), bytesToBeMoved, expectedBytesToReclaimThroughCompaction / regionSize);
    return regionCount;
}

void MM_ReclaimDelegate::estimateReclaimableRegions(MM_EnvironmentVLHGC* env,
    double averageEmptinessOfCopyForwardedRegions, UDATA* reclaimableRegions, UDATA* defragmentReclaimableRegions)
{
    ...
    MM_GlobalAllocationManagerTarok* globalAllocationManager
        = (MM_GlobalAllocationManagerTarok*)extensions->globalAllocationManager;
    ...
    *defragmentReclaimableRegions = freeRegions;
    ...
    if (extensions->tarokEnableScoreBasedAtomicCompact) {
        ...
        while (NULL != (region = regionIterator.nextRegion())) {
            ...
            if (region->hasValidMarkMap() && region->getRememberedSetCardList()->isAccurate()) {
                ...
                if (env->_cycleState->_shouldRunCopyForward) {
                    UDATA estimatedNonrecoverableMemory
                        = (UDATA)((double)(regionSize - freeMemory) * averageEmptinessOfCopyForwardedRegions);
        ...
        for (UDATA compactGroup = 0; compactGroup < _compactGroupMaxCount; compactGroup++) {
            ...
            UDATA defragmentReclaimableRegionsInGeneration
                = _compactGroups[compactGroup].defragmentRecoverableBytes / regionSize;
            if (reclaimableRegionsInGeneration > 0) {
                Trc_MM_ReclaimDelegate_estimateReclaimableRegions_generationSummary(env->getLanguageVMThread(),
                    compactGroup, _compactGroups[compactGroup].regionCount, _compactGroups[compactGroup].freeBytes,
                    _compactGroups[compactGroup].recoverableBytes, reclaimableRegionsInGeneration,
                    defragmentReclaimableRegionsInGeneration);
    ...
    Trc_MM_ReclaimDelegate_estimateReclaimableRegions_Exit(
        env->getLanguageVMThread(), freeRegions, *reclaimableRegions, *defragmentReclaimableRegions);
}
 ...
 */
static int compareEmptinessFunc(const void* element1, const void* element2)
{
...
void MM_ReclaimDelegate::rebuildRegionsSortedByEmptinessArray(MM_EnvironmentVLHGC* env)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
    ...
    J9_SORT(_regionsSortedByEmptinessArray, _regionsSortedByEmptinessArraySize, sizeof(MM_HeapRegionDescriptorVLHGC*),
        compareEmptinessFunc);
}

double MM_ReclaimDelegate::calculateOptimalEmptinessRegionThreshold(MM_EnvironmentVLHGC* env,
    double regionConsumptionRate, double avgSurvivorRegions, double avgCopyForwardRate, U_64 scanTimeCostPerGMP)
{
    ...
    /* Initially set bytesRecovered to current amount of non-required free regions.  PGC hasn't completed yet, so don't
     * factor in Eden Regions */
    ...
    /* TODO: lpnguyen reconcile this with dynamic eden resizing.  GMP scheduling will have to be modified for dynamic
     * eden re-sizing as well
     *
    ...
    while (regionSortedByEmptinessIndex < _regionsSortedByEmptinessArraySize) {
        ...
        while (bytesRecovered < defragmentRecoveryTargetPerGMP) {
            ...
            freeAndDarkMatterBytes
                = ((MM_MemoryPoolBumpPointer*)region->getMemoryPool())->getFreeMemoryAndDarkMatterBytes();
        ...
        double avgDefragmentCopyForwardDurationPerPGC
            = ((double)defragmentBytesCopyForwardedPerGMP / avgCopyForwardRate) / (double)gmpPeriod;
    ...
    Assert_MM_true(
        (optimalDefragmentEmptinessRegionThreshold >= 0.0) && (optimalDefragmentEmptinessRegionThreshold <= 1.0));

    Trc_MM_ReclaimDelegate_calculateOptimalEmptinessRegionThreshold(env->getLanguageVMThread(), freeRegions,
        regionConsumptionRate, avgSurvivorRegions, avgCopyForwardRate, scanTimeCostPerGMP, minimumTotalCostPerPGC,
        optimalGMPPeriod, optimalDefragmentEmptinessRegionThreshold);

    return optimalDefragmentEmptinessRegionThreshold;
}

void MM_ReclaimDelegate::runGlobalSweepBeforePGC(MM_EnvironmentVLHGC* env, MM_AllocateDescription* allocDescription,
    MM_MemorySubSpace* activeSubSpace, MM_GCCode gcCode)
{
    performAtomicSweep(env, allocDescription, activeSubSpace, gcCode);

    /* Now that dark matter and free bytes data have been updated in all memoryPools, we want to rebuild the
     * _regionsSortedByEmptinessArray table */
    rebuildRegionsSortedByEmptinessArray(env);
}

void MM_ReclaimDelegate::untagRegionsAfterSweep()
{
    GC_HeapRegionIteratorVLHGC regionIterator(_regionManager);
    MM_HeapRegionDescriptorVLHGC* region = NULL;
    while (NULL != (region = regionIterator.nextRegion())) {
...
void MM_ReclaimDelegate::doSweep(MM_EnvironmentVLHGC* env, MM_AllocateDescription* allocDescription,
    MM_MemorySubSpace* activeSubSpace, MM_GCCode gcCode)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._sweepStats._startTime
        = j9time_hires_clock();
    ...
    static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._sweepStats._endTime
        = j9time_hires_clock();
    reportSweepEnd(env);
}

void MM_ReclaimDelegate::performAtomicSweep(MM_EnvironmentVLHGC* env, MM_AllocateDescription* allocDescription,
    MM_MemorySubSpace* activeSubSpace, MM_GCCode gcCode)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
...
void MM_ReclaimDelegate::createRegionCollectionSetForPartialGC(MM_EnvironmentVLHGC* env, UDATA desiredWorkToDo)
{
...
void MM_ReclaimDelegate::deriveCompactScore(MM_EnvironmentVLHGC* env)
{
    ...
    while (NULL != (region = regionIterator.nextRegion())) {
        if (region->hasValidMarkMap()) {
            if (!region->_defragmentationTarget) {
                ...
                /* Overflowed regions or those that RSCL is being rebuilt will not be be compacted (not even put into
                 * the sorted list) */
                region->_compactData._compactScore = 0.0;
                ...
                if (region->_markData._shouldMark) {
                    ...
                    if (0.0 < region->_compactData._compactScore) {
                        ...
                        for (UDATA scanForRegion = 0; scanForRegion < _currentSortedRegionCount; scanForRegion++) {
                            if (regionToSwap->_compactData._compactScore
                                > _regionSortedByCompactScore[scanForRegion]->_compactData._compactScore) {
    ...
    for (IDATA i = _currentSortedRegionCount - 1; i >= 0; i--) {
        ...
        if (MM_Math::roundToFloor(regionSize, recoverableBytesAfter)
            == MM_Math::roundToFloor(regionSize, recoverableBytesBefore)) {
            _compactGroups[compactGroup].recoverableBytes -= freeMemory;
    ...
    Trc_MM_ReclaimDelegate_deriveCompactScore_Exit(env->getLanguageVMThread(), overflowedRegionCount,
        unsweptRegionCount, alreadySelectedRegionCount, fullRegionCount, jniCriticalReservedRegions,
        noncontributingRegionCount, contributingRegionCount, notDefragmentationTarget);
}

void MM_ReclaimDelegate::postCompactCleanup(MM_EnvironmentVLHGC* env, MM_AllocateDescription* allocDescription,
    MM_MemorySubSpace* activeSubSpace, MM_GCCode gcCode)
{
    ...
    Trc_MM_ReclaimDelegate_rememberedObjectCount(env->getLanguageVMThread(), totalFixupRegions, nonOverflowRegions,
        (0 == nonOverflowRegions) ? 0 : nonOverFlowObjectsRemembered / nonOverflowRegions, overflowRegions,
        (0 == overflowRegions) ? 0 : overFlowObjectsRemembered / overflowRegions, zeroRememberedRegions);

    /* ----- end of cleanupAfterCollect ------*/
}

void MM_ReclaimDelegate::runReclaimCompleteSweep(MM_EnvironmentVLHGC* env, MM_AllocateDescription* allocDescription,
    MM_MemorySubSpace* activeSubSpace, MM_GCCode gcCode)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
    MM_GlobalAllocationManagerTarok* globalAllocationManager
...
void MM_ReclaimDelegate::runReclaimCompleteCompact(MM_EnvironmentVLHGC* env, MM_AllocateDescription* allocDescription,
    MM_MemorySubSpace* activeSubSpace, MM_GCCode gcCode, MM_MarkMap* nextMarkMap, UDATA compactSelectionGoalInBytes)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
    MM_GlobalAllocationManagerTarok* globalAllocationManager
        = (MM_GlobalAllocationManagerTarok*)extensions->globalAllocationManager;
    ...
    UDATA ignoreSkippedRegions = 0;
    runCompact(
        env, allocDescription, activeSubSpace, compactSelectionGoalInBytes, gcCode, nextMarkMap, &ignoreSkippedRegions);
    Trc_MM_ReclaimDelegate_runReclaimComplete_freeAfterCompact(
        env->getLanguageVMThread(), globalAllocationManager->getFreeRegionCount());
}

void MM_ReclaimDelegate::runReclaimForAbortedCopyForward(MM_EnvironmentVLHGC* env,
    MM_AllocateDescription* allocDescription, MM_MemorySubSpace* activeSubSpace, MM_GCCode gcCode,
    MM_MarkMap* nextMarkMap, UDATA* skippedRegionCountRequiringSweep)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
    MM_CompactGroupPersistentStats* persistentStats = extensions->compactGroupPersistentStats;

    Trc_MM_ReclaimDelegate_runReclaimForAbortedCopyForward_Entry(env->getLanguageVMThread(),
        ((MM_GlobalAllocationManagerTarok*)MM_GCExtensions::getExtensions(env)->globalAllocationManager)
            ->getFreeRegionCount());
    Assert_MM_true(env->_cycleState->_shouldRunCopyForward);
    ...
    Trc_MM_ReclaimDelegate_runReclaimForAbortedCopyForward_Exit(env->getLanguageVMThread(),
        ((MM_GlobalAllocationManagerTarok*)MM_GCExtensions::getExtensions(env)->globalAllocationManager)
            ->getFreeRegionCount(),
        regionsCompacted);
}

void MM_ReclaimDelegate::runCompact(MM_EnvironmentVLHGC* env, MM_AllocateDescription* allocDescription,
    MM_MemorySubSpace* activeSubSpace, UDATA compactSelectionGoalInBytes, MM_GCCode gcCode, MM_MarkMap* nextMarkMap,
    UDATA* skippedRegionCountRequiringSweep)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
    MM_CompactGroupPersistentStats* persistentStats = extensions->compactGroupPersistentStats;
    ...
    if (extensions->tarokEnableScoreBasedAtomicCompact
        && (MM_CycleState::CT_PARTIAL_GARBAGE_COLLECTION == env->_cycleState->_collectionType)) {
        regionsCompacted = tagRegionsBeforeCompactWithWorkGoal(
            env, false, compactSelectionGoalInBytes, skippedRegionCountRequiringSweep);
    } else {
...
#if defined(J9VM_GC_MODRON_COMPACTION)
void MM_ReclaimDelegate::compactAndCorrectStats(
    MM_EnvironmentVLHGC* env, MM_AllocateDescription* allocDescription, MM_MarkMap* nextMarkMap)
{
    /* clear the stats so that we don't get stale data from the previous compaction - this is normally done at the
     * beginning of a collect (as part of the clear fall for the stats structure) but we need to do it before each
     * compact increment, as well
     */
...
bool MM_ReclaimDelegate::heapAddRange(
...
bool MM_ReclaimDelegate::heapRemoveRange(MM_EnvironmentVLHGC* env, MM_MemorySubSpace* subspace, UDATA size,
    void* lowAddress, void* highAddress, void* lowValidAddress, void* highValidAddress)
{
    return _sweepScheme->heapRemoveRange(
        env, subspace, size, lowAddress, highAddress, lowValidAddress, highValidAddress);
}

void MM_ReclaimDelegate::heapReconfigured(MM_EnvironmentVLHGC* env) { _sweepScheme->heapReconfigured(env); }

void* MM_ReclaimDelegate::createSweepPoolState(MM_EnvironmentBase* env, MM_MemoryPool* memoryPool)
{
    return _sweepScheme->createSweepPoolState(env, memoryPool);
}

void MM_ReclaimDelegate::deleteSweepPoolState(MM_EnvironmentBase* env, void* sweepPoolState)
{
    _sweepScheme->deleteSweepPoolState(env, sweepPoolState);
}

#if defined(J9VM_GC_MODRON_COMPACTION)
void MM_ReclaimDelegate::masterThreadCompact(
    MM_EnvironmentVLHGC* env, MM_AllocateDescription* allocDescription, MM_MarkMap* nextMarkMap)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);

    static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._compactStats._startTime
        = j9time_hires_clock();
    ...
    static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._compactStats._endTime
        = j9time_hires_clock();
    reportCompactEnd(env);
}
#endif /* J9VM_GC_MODRON_COMPACTION */

void MM_ReclaimDelegate::masterThreadRestartAllocationCaches(MM_EnvironmentVLHGC* env)
{
...
void MM_ReclaimDelegate::reportSweepStart(MM_EnvironmentBase* env)
{
    J9VMThread* vmThread = (J9VMThread*)env->getLanguageVMThread();
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    Trc_MM_SweepStart(vmThread);

    TRIGGER_J9HOOK_MM_PRIVATE_SWEEP_START(MM_GCExtensions::getExtensions(env)->privateHookInterface,
        env->getOmrVMThread(), j9time_hires_clock(), J9HOOK_MM_PRIVATE_SWEEP_START);

    TRIGGER_J9HOOK_MM_PRIVATE_RECLAIM_SWEEP_START(MM_GCExtensions::getExtensions(env)->privateHookInterface,
        env->getOmrVMThread(), &static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._sweepStats);
}

void MM_ReclaimDelegate::reportSweepEnd(MM_EnvironmentBase* env)
{
    J9VMThread* vmThread = (J9VMThread*)env->getLanguageVMThread();
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    Trc_MM_SweepEnd(vmThread);

    TRIGGER_J9HOOK_MM_PRIVATE_SWEEP_END(MM_GCExtensions::getExtensions(env)->privateHookInterface,
        env->getOmrVMThread(), j9time_hires_clock(), J9HOOK_MM_PRIVATE_SWEEP_END);

    TRIGGER_J9HOOK_MM_PRIVATE_RECLAIM_SWEEP_END(MM_GCExtensions::getExtensions(env)->privateHookInterface,
        env->getOmrVMThread(), &static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._sweepStats);
}

void MM_ReclaimDelegate::reportGlobalGCCollectComplete(MM_EnvironmentBase* env)
{
    J9VMThread* vmThread = (J9VMThread*)env->getLanguageVMThread();
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    Trc_MM_GlobalGCCollectComplete(vmThread);

    TRIGGER_J9HOOK_MM_PRIVATE_GLOBAL_GC_COLLECT_COMPLETE(MM_GCExtensions::getExtensions(env)->privateHookInterface,
        env->getOmrVMThread(), j9time_hires_clock(), J9HOOK_MM_PRIVATE_GLOBAL_GC_COLLECT_COMPLETE);
}

#if defined(J9VM_GC_MODRON_COMPACTION)
void MM_ReclaimDelegate::reportCompactStart(MM_EnvironmentBase* env)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
    CompactReason compactReason = (CompactReason)(
        static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._compactStats._compactReason);
    J9VMThread* vmThread = (J9VMThread*)env->getLanguageVMThread();
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    Trc_MM_CompactStart(vmThread, getCompactionReasonAsString(compactReason));

    TRIGGER_J9HOOK_MM_PRIVATE_COMPACT_START(extensions->privateHookInterface, env->getOmrVMThread(),
        j9time_hires_clock(), J9HOOK_MM_PRIVATE_COMPACT_START, extensions->globalVLHGCStats.gcCount);

    TRIGGER_J9HOOK_MM_PRIVATE_RECLAIM_COMPACT_START(extensions->privateHookInterface, env->getOmrVMThread(),
        &static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._compactStats);
}

void MM_ReclaimDelegate::reportCompactEnd(MM_EnvironmentBase* env)
{
    MM_GCExtensions* extensions = MM_GCExtensions::getExtensions(env);
    J9VMThread* vmThread = (J9VMThread*)env->getLanguageVMThread();
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    MM_CompactVLHGCStats* stats
        = &static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._compactStats;

    Trc_MM_CompactEnd(vmThread, stats->_movedBytes);

    TRIGGER_J9HOOK_MM_OMR_COMPACT_END(
        extensions->omrHookInterface, env->getOmrVMThread(), j9time_hires_clock(), J9HOOK_MM_OMR_COMPACT_END);

    TRIGGER_J9HOOK_MM_PRIVATE_RECLAIM_COMPACT_END(extensions->privateHookInterface, env->getOmrVMThread(), stats,
