...
private:
    ...
    MM_ParallelSweepSchemeVLHGC*
        _sweepScheme; /**< The sweep scheme component which this delegate owns and uses to reclaim memory */
    ...
    MM_HeapRegionDescriptorVLHGC**
        _regionsSortedByEmptinessArray; /**< descending list of regions sorted by their emptiness (amount of free and
                                           dark matter bytes in their memory pool) */
    UDATA _regionsSortedByEmptinessArraySize; /**< The populated size of _regionsSortedByEmptinessArray */

    double _compactRateOfReturn; /**< The historical rate of return of compact (essentially, the accuracy of the compact
                                    region selection) */
    ...
    MM_ReclaimDelegate_ScoreBaseCompactTable*
        _compactGroups; /**< Score based compaction recovered details per compactGroup */
    const UDATA _compactGroupMaxCount; /**< The number of compact groups in _compactGroups - used for walking the
                                          _compactGroups table and validating calculated compaction group numbers */
...
public:
     ...
     */
    void runReclaimCompleteSweep(MM_EnvironmentVLHGC* env, MM_AllocateDescription* allocDescription,
     ...
     * @param compactSelectionGoalInBytes[in] The minimum number of free bytes or  The minimum amount of compact work to
     * be done
     */
    void runReclaimCompleteCompact(MM_EnvironmentVLHGC* env, MM_AllocateDescription* allocDescription,
        MM_MemorySubSpace* activeSubSpace, MM_GCCode gcCode, MM_MarkMap* nextMarkMap,
        UDATA compactSelectionGoalInBytes);
     ...
     * @param gcCode[in] The description of the nature of the collection
     * @param nextMarkMap[in] The next mark map (since the compactor uses it for fixup data storage)
     * @param skippedRegionCountRequiringSweep[out] The number of regions which should be compacted but couldn't be (due
     * to inaccurate RSCL or active critical regions) and also haven't yet been swept
     */
    void runReclaimForAbortedCopyForward(MM_EnvironmentVLHGC* env, MM_AllocateDescription* allocDescription,
        MM_MemorySubSpace* activeSubSpace, MM_GCCode gcCode, MM_MarkMap* nextMarkMap,
     ...
     */
    void estimateReclaimableRegions(MM_EnvironmentVLHGC* env, double averageEmptinessOfCopyForwardedRegions,
        UDATA* reclaimableRegions, UDATA* defragmentReclaimableRegions);

    /**
     * Called to invoke a global sweep to update free region data and dark matter estimates following the completion of
     * a GMP. This is intended to be called before the PGC runs either its Copy-Forward or Mark-Sweep-Compact so that it
     * can use the most up-to-date statistics when selecting its collection set.
     * @param env[in] The master GC thread
     * @param allocDescription[in] The description of the allocation which triggered the collection
     * @param activeSubSpace[in] The active subspace which ran out of memory
     * @param gcCode[in] The description of the nature of the collection
     */
    void runGlobalSweepBeforePGC(MM_EnvironmentVLHGC* env, MM_AllocateDescription* allocDescription,
        MM_MemorySubSpace* activeSubSpace, MM_GCCode gcCode);
     ...
     */
    UDATA tagRegionsBeforeCompactWithWorkGoal(
     ...
     * for later consideration when selecting regions to compact. Used only for compactWorkGoal selection for both
     * CopyForward and MarkSweepCompact
     * @param env[in] The master GC thread
     */
     ...
     * to inaccurate RSCL or active critical regions) and also haven't yet been swept
     */
    void runCompact(MM_EnvironmentVLHGC* env, MM_AllocateDescription* allocDescription,
     ...
     * @param gcCode[in] The description of the nature of the collection
     */
    void performAtomicSweep(MM_EnvironmentVLHGC* env, MM_AllocateDescription* allocDescription,
        MM_MemorySubSpace* activeSubSpace, MM_GCCode gcCode);

    /**
     * TODO currently _regionManager is used by tagRegionBeforeCollectPhase and untagRegionAfterCollectPhase.
     * Both functions will be commonized when markDelegate and reclaim delegate are fully implemented, then we can
     * remove the _regionManager field.
     */
    MM_ReclaimDelegate(
     ...
     */
    double calculateOptimalEmptinessRegionThreshold(MM_EnvironmentVLHGC* env, double regionConsumptionRate,
        double avgSurvivorRegions, double avgCopyForwardRate, U_64 scanTimeCostPerGMP);
...
private:
     ...
     */
    void doSweep(MM_EnvironmentVLHGC* env, MM_AllocateDescription* allocDescription, MM_MemorySubSpace* activeSubSpace,
        MM_GCCode gcCode);

    void postCompactCleanup(MM_EnvironmentVLHGC* env, MM_AllocateDescription* allocDescription,
        MM_MemorySubSpace* activeSubSpace, MM_GCCode gcCode);
     ...
     *
     * NOTE:  This method currently does nothing and only exists to provide symmetry when compared to compact (it may be
     * used to incrementalize sweep, in the future)
     *
     * @param env[in] The current thread
     */
