 ...
 */
MM_RegionBasedOverflowVLHGC* MM_RegionBasedOverflowVLHGC::newInstance(
    MM_EnvironmentBase* env, MM_WorkPackets* workPackets, U_8 overflowFlag)
{
    Assert_MM_true(0 != overflowFlag);
    MM_RegionBasedOverflowVLHGC* overflow = (MM_RegionBasedOverflowVLHGC*)env->getForge()->allocate(
        sizeof(MM_RegionBasedOverflowVLHGC), MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
    if (NULL != overflow) {
 ...
 */
bool MM_RegionBasedOverflowVLHGC::initialize(MM_EnvironmentBase* env) { return MM_WorkPacketOverflow::initialize(env); }

MM_RegionBasedOverflowVLHGC::MM_RegionBasedOverflowVLHGC(
    MM_EnvironmentBase* env, MM_WorkPackets* workPackets, U_8 overflowFlag)
 ...
 */
void MM_RegionBasedOverflowVLHGC::tearDown(MM_EnvironmentBase* env) { MM_WorkPacketOverflow::tearDown(env); }
 ...
 *
 */
void MM_RegionBasedOverflowVLHGC::emptyToOverflow(MM_EnvironmentBase* env, MM_Packet* packet, MM_OverflowType type)
{
 ...
 *
 */
void MM_RegionBasedOverflowVLHGC::overflowItem(MM_EnvironmentBase* env, void* item, MM_OverflowType type)
{
    MM_EnvironmentVLHGC* envVLHGC = MM_EnvironmentVLHGC::getEnvironment(env);
    _overflow = true;
 ...
 *
 */
void MM_RegionBasedOverflowVLHGC::overflowItemInternal(MM_EnvironmentBase* env, void* item, MM_OverflowType type)
{
    MM_EnvironmentVLHGC* envVLHGC = MM_EnvironmentVLHGC::getEnvironment(env);
    ...
    if ((PACKET_INVALID_OBJECT != (UDATA)item)
        && (PACKET_ARRAY_SPLIT_TAG != (((UDATA)objectPtr) & PACKET_ARRAY_SPLIT_TAG))) {
        ...
        /* Removed for JAZZ 46465 since CopyForwardDepthFirst needs to use this overflow mechanism but its mark map
         * caches out out of sync with the global mark map so it isn't safe to check that an overflow item is marked:
         * Assert_MM_true(envVLHGC->_cycleState->_markMap->isBitSet(objectPtr));
         */
        MM_HeapRegionDescriptorVLHGC* region
            = (MM_HeapRegionDescriptorVLHGC*)_heapRegionManager->tableDescriptorForAddress(item);
        /* ensure that we have flushed the updated mark map bit from our cache and that we see any other updates to the
         * overflow flags for this region */
        ...
        if (GC_ObjectModel::SCAN_REFERENCE_MIXED_OBJECT == scantype) {
            /* since we popped this object from the work packet, it is our responsibility to record it in the list of
             * reference objects */
            ...
            if ((GC_ObjectModel::REF_STATE_INITIAL == referenceState)
                || (GC_ObjectModel::REF_STATE_REMEMBERED == referenceState)) {
                ...
                if (referentMustBeCleared) {
                    GC_SlotObject referentPtr(
                        envVLHGC->getOmrVM(), &J9GC_J9VMJAVALANGREFERENCE_REFERENT(envVLHGC, objectPtr));
        ...
        } else if ((OMR_GC_CYCLE_TYPE_VLHGC_PARTIAL_GARBAGE_COLLECT == envVLHGC->_cycleState->_type)
            && (GC_ObjectModel::SCAN_OWNABLESYNCHRONIZER_OBJECT == scantype)) {
             ...
             */
            if (isEvacuateRegion(region)
                && (NULL != _extensions->accessBarrier->isObjectInOwnableSynchronizerList(objectPtr))) {
                /* To avoid adding duplication item (abort case the object need to be rescan and interregions remembered
                 * objects) To avoid adding constructing object
                 */
 ...
 * the card table.
 */
void MM_RegionBasedOverflowVLHGC::fillFromOverflow(MM_EnvironmentBase* env, MM_Packet* packet) {}

void MM_RegionBasedOverflowVLHGC::reset(MM_EnvironmentBase* env) {}

bool MM_RegionBasedOverflowVLHGC::isEmpty() { return true; }

U_8 MM_RegionBasedOverflowVLHGC::overflowFlagForCollectionType(
    MM_EnvironmentBase* env, MM_CycleState::CollectionType collectionType)
{
    U_8 flag = 0;
