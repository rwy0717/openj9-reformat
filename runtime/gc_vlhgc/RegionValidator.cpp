...
void MM_RegionValidator::threadCrash(MM_EnvironmentBase* env)
{
    reportRegion(env, "Unhandled exception while validating region");
}

void MM_RegionValidator::reportRegion(MM_EnvironmentBase* env, const char* message)
{
    ...
    if (_region->isArrayletLeaf()) {
        j9tty_printf(PORTLIB, "ERROR: %s in region %p; type=%zu; range=%p-%p; spine=%p\n", message, _region,
            (UDATA)regionType, _region->getLowAddress(), _region->getHighAddress(), _region->_allocateData.getSpine());
        Trc_MM_RegionValidator_leafRegion(env->getLanguageVMThread(), message, _region, regionType,
            _region->getLowAddress(), _region->getHighAddress(), _region->_allocateData.getSpine());
    } else {
        j9tty_printf(PORTLIB, "ERROR: %s in region %p; type=%zu; range=%p-%p\n", message, _region, (UDATA)regionType,
            _region->getLowAddress(), _region->getHighAddress());
        Trc_MM_RegionValidator_objectRegion(env->getLanguageVMThread(), message, _region, regionType,
            _region->getLowAddress(), _region->getHighAddress());
    }
    ...
    if (thisIndex > 0) {
        MM_HeapRegionDescriptorVLHGC* previousRegion
            = (MM_HeapRegionDescriptorVLHGC*)regionManager->tableDescriptorForIndex(thisIndex - 1);
        ...
        if (previousRegion->isArrayletLeaf()) {
            j9tty_printf(PORTLIB, "ERROR: (Previous region %p; type=%zu; range=%p-%p; spine=%p)\n", previousRegion,
                (UDATA)previousRegionType, previousRegion->getLowAddress(), previousRegion->getHighAddress(),
                previousRegion->_allocateData.getSpine());
            Trc_MM_RegionValidator_previousLeafRegion(env->getLanguageVMThread(), previousRegion, previousRegionType,
                previousRegion->getLowAddress(), previousRegion->getHighAddress(),
                previousRegion->_allocateData.getSpine());
        } else {
            j9tty_printf(PORTLIB, "ERROR: (Previous region %p; type=%zu; range=%p-%p)\n", previousRegion,
                (UDATA)previousRegionType, previousRegion->getLowAddress(), previousRegion->getHighAddress());
            Trc_MM_RegionValidator_previousObjectRegion(env->getLanguageVMThread(), previousRegion, previousRegionType,
                previousRegion->getLowAddress(), previousRegion->getHighAddress());
        }
    }
...
bool MM_RegionValidator::validate(MM_EnvironmentBase* env)
{
    ...
    if (MM_HeapRegionDescriptor::BUMP_ALLOCATED == regionType) {
        ...
        /* verify that the first object in this region is valid (checks against the previous region overflowing into
         * this one) */
        MM_HeapMapWordIterator firstWordIterator(
            MM_GCExtensions::getExtensions(env)->previousMarkMap, _region->getLowAddress());
        ...
        /* Do a quick check to ensure that arraylets look reasonable before the collection to help debug problems like
         * CMVC 174687 */
