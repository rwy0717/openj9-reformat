...
bool MM_RememberedSetCardBucket::initialize(
    MM_EnvironmentVLHGC* env, MM_RememberedSetCardList* rscl, MM_RememberedSetCardBucket* next)
{
    _rscl = rscl;
    _next = next;
...
void MM_RememberedSetCardBucket::tearDown(MM_GCExtensions* extensions)
{
    _cardBufferControlBlockHead = NULL;
    _current = NULL;
}

void MM_RememberedSetCardBucket::setListAsOverflow(MM_EnvironmentVLHGC* env, MM_RememberedSetCardList* listToOverflow)
{
...
void MM_RememberedSetCardBucket::addToNewBuffer(MM_EnvironmentVLHGC* env, MM_RememberedSetCard card)
{
    ...
    if (!_rscl->_overflowed) {
        ...
        _bufferCount += 1;
        if ((_rscl->_bufferCount * MAX_BUFFER_SIZE)
            > MM_GCExtensions::getExtensions(env)->tarokRememberedSetCardListMaxSize) {
            ...
            MM_CardBufferControlBlock* newBuffer
                = interRegionRememberedSet->allocateCardBufferControlBlockFromLocalPool(env);
            if (NULL == newBuffer) {
                MM_AtomicOperations::subtract(&_rscl->_bufferCount, 1);
                _bufferCount -= 1;

                MM_RememberedSetCardList* rsclToOverflow
                    = interRegionRememberedSet->findRsclToOverflow((MM_EnvironmentVLHGC*)env);
                if (NULL == rsclToOverflow) {
                    /* Failed to find an appropriate region to overflow. Abort this transaction by overflowing the
                     * current list */
                    setListAsOverflow(env, _rscl);
                } else {
                    ...
                    if (NULL == newBuffer) {
                        /* No luck, failed even after overflowing another list. Abort this transaction by overflowing
                         * the current list */
                        setListAsOverflow(env, _rscl);
                    } else {
...
bool MM_RememberedSetCardBucket::isRemembered(MM_EnvironmentVLHGC* env, MM_RememberedSetCard card)
{
    UDATA bufferCount = 0;
...
void MM_RememberedSetCardBucket::releaseBuffers(MM_EnvironmentVLHGC* env, UDATA buffersToLocalPoolCount)
{
    Assert_MM_true(_rscl->_bufferCount >= _bufferCount);

    UDATA releasedCount
        = MM_GCExtensions::getExtensions(env)->interRegionRememberedSet->releaseCardBufferControlBlockListToLocalPool(
            env, _cardBufferControlBlockHead, buffersToLocalPoolCount);
...
void MM_RememberedSetCardBucket::globalReleaseBuffers(MM_EnvironmentVLHGC* env)
{
    releaseBuffers(env, MAX_LOCAL_RSCL_BUFFER_POOL_SIZE);
}

void MM_RememberedSetCardBucket::localReleaseBuffers(MM_EnvironmentVLHGC* env) { releaseBuffers(env, UDATA_MAX); }
...
void MM_RememberedSetCardBucket::compact(MM_EnvironmentVLHGC* env)
{
    Assert_MM_true(_rscl->_bufferCount >= _bufferCount);
    ...
    if (NULL != _cardBufferControlBlockHead) {
        ...
        UDATA releasedCount = MM_GCExtensions::getExtensions(env)
                                  ->interRegionRememberedSet->releaseCardBufferControlBlockListToLocalPool(
                                      env, toDeleteCardBufferControlBlock, UDATA_MAX);
