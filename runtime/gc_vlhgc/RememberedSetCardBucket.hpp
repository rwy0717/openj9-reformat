...
private:
     ...
     * @param buffersToLocalPoolCount max buffers returned to the local pool. typically UDATA_MAX (all goes to local) or
     * MAX_LOCAL_RSCL_BUFFER_POOL_SIZE (small part goes to local, rest to global)
     */
...
public:
    ...
    /**
     * Clear the list. Release buffers back to the global buffer pool. A small fraction may be release to the thread
     * local pool.
     */
     ...
     */
    bool isEmpty(MM_EnvironmentVLHGC* env) { return (NULL == _current); }
     ...
     */
    UDATA getBufferCount(MM_EnvironmentVLHGC* env) { return _bufferCount; }
