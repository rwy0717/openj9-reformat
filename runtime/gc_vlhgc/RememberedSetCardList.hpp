...
private:
    ...
    MM_RememberedSetCardList* volatile _nonEmptyOverflowedNext; /**< overflowed RSCL found during a GC cycle are linked
                                                                   into a single liked list - this is next pointer */
private:
    /**
     * Remove an entry. This just NULLs the entry. Compaction/shifting is to be done later, explicitly.
     */
    void removeCard(MM_RememberedSetCard* bucketCardList, UDATA index) { bucketCardList[index] = 0; }
...
public:
     ...
     */
    bool isOverflowed() { return (TRUE == _overflowed); }

    /*
     * return  true if overflowed as stable region
     */
    bool isStable() { return _stable; }

    /**
     * Check if the list is being rebuilt
     */
    bool isBeingRebuilt() { return _beingRebuilt; }

    /**
     * Check if the content is accurate (not overflowed and not being rebuilt)
     */
    bool isAccurate() { return (FALSE == _overflowed) && !_beingRebuilt; }
     ...
     */
    void setAsBeingRebuilt() { _beingRebuilt = true; }

    /**
     * The rebuilding is complete. Reset rebuild flag.
     */
    void setAsRebuildingComplete() { _beingRebuilt = false; }
