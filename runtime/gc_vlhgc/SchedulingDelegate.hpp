...
private:
    ...
    UDATA _taxationIndex; /**< Counter of the number of times that taxation has occurred. This is used to determine if
                             the current taxation point should perform a GMP and/or PGC */
    UDATA _remainingGMPIntermissionIntervals; /**< Counter of the remaining intervals to skip before starting another
                                                 GMP cycle */
    ...
    bool _nextIncrementWillDoGlobalMarkPhase; /**< Record whether a GMP increment is planned for the next taxation point
                                               */
    bool _nextPGCShouldCopyForward; /**< True if the next PGC increment will be attempted using copy-forward (read and
                                       set when PGC starts).  False implies compaction will be used, instead */
    bool _globalSweepRequired; /**< Set when a GMP finishes so that the next PGC to run knows that it is responsible for
                                  the first global sweep on the new mark map */
    bool _disableCopyForwardDuringCurrentGlobalMarkPhase; /**< Set to true when a PGC Abort happens during GMP. Reset
                                                             only when GMP is complete. */
    ...
    double _edenSurvivalRateCopyForward; /**< The running average ratio of the number of regions consumed to
                                            copy-forward Eden to the number of regions allocated as Eden */
    UDATA _nonEdenSurvivalCountCopyForward; /**< The running average count of the number of non-Eden regions allocated
                                               by copy-forward for its survival set (Eden regions are excluded since
                                               they are tracked with _edenSurvivalRateCopyForward) */

    UDATA _previousReclaimableRegions; /**< The number of reclaimable regions at the end of the last reclaim. Used to
                                          estimate consumption rate. */
    UDATA
        _previousDefragmentReclaimableRegions; /**< The number of reclaimable regions in the defragment set at the end
                                                  of the last reclaim. Used to estimate GMP kickoff and compact rate. */
    ...
    double _bytesCompactedToFreeBytesRatio; /**< Ratio of bytes to be compacted / bytes to be freed, for regions created
                                               after GMP, and used to spread the work until next GMP */
    double _averageCopyForwardBytesCopied; /**< Weighted average of bytes copied by the copy-forward scheme */
    double
        _averageCopyForwardBytesDiscarded; /**< Weighted average of bytes discarded (lost) by the copy-forward scheme */
    double _averageSurvivorSetRegionCount; /**< Weighted average of survivor regions */
    double _averageCopyForwardRate; /**< Weighted average of (bytesCopied / timeSpentInCopyForward).  Disregards time
                                       spent related RSCL clearing. Measured in bytes/microseconds */
    double _averageMacroDefragmentationWork; /**< Average work to be done to mitigate influx of fragmented regions into
                                                the oldest age */
    ...
    UDATA
        _previousLiveSetBytesAfterGlobalSweep; /**< Live set estimate recorded value for PGC after previous GMP sweep */
    double _scannableBytesRatio; /**< Ratio of scannable byte vs total bytes (scannable + non-scannable) as measured
                                    from past */
    U_64 _historicTotalIncrementalScanTimePerGMP; /**< Historic average of the total wall-clock time we spend scanning
                                                     in all stop-the-world global mark increments over a GMP cycle in
                                                     microseconds */
    UDATA _historicBytesScannedConcurrentlyPerGMP; /**< Historic average amount of bytes we scan concurrently per GMP
                                                      cycle */

    U_64 _partialGcStartTime; /**< Start time of the in progress Partial GC in hi-resolution format (recorded to track
                                 total time spent in Partial GC) */
    U_64 _historicalPartialGCTime; /**< Weighted historical average of Partial GC times */

    UDATA _dynamicGlobalMarkIncrementTimeMillis; /**< The dynamically calculated current time to be spent per GMP
                                                    increment (subject to change over the course of the run) */
...
private:
    ...
    /**
     * @return The estimated number of bytes which a global collect would need to scan (based on live bytes, heap
     * occupancy trends, and the ratio of scannable bytes)
     */
    ...
    /**
     * @return The estimated amount of time (wall-clock time in millis) we need to spend scanning to finish the current
     * GMP. If there is no GMP currently running, returns the estimated amount of time we'd need to scan to finish a
     * GMP.
     */
     ...
     */
    void measureConsumptionForPartialGC(
        MM_EnvironmentVLHGC* env, UDATA currentReclaimableRegions, UDATA defragmentReclaimableRegions);
    ...
    /**
     * Updates the _edenSurvivalRateCopyForward and _nonEdenSurvivalCountCopyForward running averages by including
     * thisEdenSurvivalRate and thisNonEdenSurvivorCount, respectively, for a copy-forward which has just completed.
     * @param thisEdenSurvivalRate[in] A value >= 0.0 (and typically <= 1.0) representing the number of regions out of
     * those allocated for this Eden which survived
    ...
    /**
     * Get number of GMP increments we wish to have as headroom to ensure that the GMP cycle finishes before AF with the
     * desired pause time.
     * @param env[in] the master GC thread
     */
    ...
    /**
     * Called after a GMP completes to update GMP-related statistics necessary for scheduling.  This is done by
     * examining env->cycleState.
     * @param env[in] the master GC thread
     */
...
public:
    ...
    /**
     * Returns the average copy forward rate.  Disregards time spent related to RSCL clearing. Measured in
     * bytes/microseconds
     *
    ...
    /**
     * Return the minimum emptiness(free and dark matter bytes) a region must have in order to be considered for
     * defragmentation
     */
     ...
     * defragmentation
     */
    void setAutomaticDefragmentEmptinessThreshold(double defragmentEmptinessThreshold)
    {
        _automaticDefragmentEmptinessThreshold = defragmentEmptinessThreshold;
    }

    /**
     * Calculate compact-bytes/free-bytes ratio after global sweep, to be used for determining amount of compact work on
     * each PGC
     * @param env[in] The master GC thread
     ...
     * it should be called before estimating defragmentReclaimableRegions in order to calculate GMPIntermission more
     * accurate.
     * TODO: might need to recalculate desiredCompactWork for sliding Compact of PGC (MacroDefragment part, right now it
     * is calculated at the end of TaxationEntryPoint, but we need to decide sliding compaction before Copyforward PGC).
     */
    ...
    /**
     * Inform the receiver that an increment in the Global Mark Phase, or the mark portion of a global collect, has
     * completed
     * @param env[in] the master GC thread
     */
     ...
     */
    void globalGarbageCollectCompleted(
     ...
     * @param defragmentReclaimableRegions[in] an estimate of the reclaimable defragment memory
     */
    void partialGarbageCollectCompleted(
        MM_EnvironmentVLHGC* env, UDATA reclaimableRegions, UDATA defragmentReclaimableRegions);
