...
MM_SweepPoolManagerVLHGC* MM_SweepPoolManagerVLHGC::newInstance(MM_EnvironmentBase* env)
{
    MM_SweepPoolManagerVLHGC* sweepPoolManager = (MM_SweepPoolManagerVLHGC*)env->getForge()->allocate(
        sizeof(MM_SweepPoolManagerVLHGC), MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
...
bool MM_SweepPoolManagerVLHGC::initialize(MM_EnvironmentBase* env) { return MM_SweepPoolManager::initialize(env); }

MM_SweepPoolState* MM_SweepPoolManagerVLHGC::getPoolState(MM_MemoryPool* memoryPool)
{
...
MMINLINE void MM_SweepPoolManagerVLHGC::calculateTrailingDetails(
    MM_ParallelSweepChunk* sweepChunk, UDATA* trailingCandidate, UDATA trailingCandidateSlotCount)
{
    ...
    UDATA projection = _extensions->objectModel.getConsumedSizeInBytesWithHeader(
                           (J9Object*)(trailingCandidate - J9MODRON_HEAP_SLOTS_PER_MARK_BIT))
...
void MM_SweepPoolManagerVLHGC::connectChunk(MM_EnvironmentBase* env, MM_ParallelSweepChunk* chunk)
{
    ...
    if ((previousFreeEntry != NULL) && ((((U_8*)previousFreeEntry) + previousFreeEntrySize) == (U_8*)leadingFreeEntry)
    ...
    if (NULL != previousConnectChunk) {
        if ((NULL != leadingFreeEntry)
            && ((((U_8*)previousConnectChunk->trailingFreeCandidate) + previousConnectChunk->trailingFreeCandidateSize)
                   == (U_8*)leadingFreeEntry)
            && (previousConnectChunk->memoryPool == memoryPool) && chunk->_coalesceCandidate) {
            ...
            if (memoryPool->canMemoryBeConnectedToPool(env, previousConnectChunk->trailingFreeCandidate,
                ...
                if (0 != previousConnectChunk->trailingFreeCandidateSize) {
                    ...
                    sweepState->_largestFreeEntry
                        = OMR_MAX(previousConnectChunk->trailingFreeCandidateSize, sweepState->_largestFreeEntry);
                }
            }
        }
    }
...
void MM_SweepPoolManagerVLHGC::flushFinalChunk(MM_EnvironmentBase* envModron, MM_MemoryPool* memoryPool)
{
    MM_SweepPoolState* sweepState = getPoolState(memoryPool);
    ...
    if ((sweepState->_connectPreviousChunk != NULL)
        ...
        if (((MM_MemoryPoolBumpPointer*)memoryPool)
                ->canMemoryBeConnectedToPool(envModron, sweepState->_connectPreviousChunk->trailingFreeCandidate,
            ...
            sweepState->_sweepFreeHoles += 1;
            sweepState->_largestFreeEntry
                = OMR_MAX(sweepState->_connectPreviousChunk->trailingFreeCandidateSize, sweepState->_largestFreeEntry);
        }
    }
}

void MM_SweepPoolManagerVLHGC::connectFinalChunk(MM_EnvironmentBase* envModron, MM_MemoryPool* memoryPool)
{
...
void MM_SweepPoolManagerVLHGC::poolPostProcess(MM_EnvironmentBase* envModron, MM_MemoryPool* memoryPool)
{
    Assert_MM_unreachable();
}

void MM_SweepPoolManagerVLHGC::updateTrailingFreeMemory(
...
bool MM_SweepPoolManagerVLHGC::addFreeMemory(
    MM_EnvironmentBase* env, MM_ParallelSweepChunk* sweepChunk, UDATA* address, UDATA size)
{
    bool result = false;
    ...
    if (address == (UDATA*)sweepChunk->chunkBase) {
        ...
        UDATA objectSizeDelta = _extensions->objectModel.getConsumedSizeInBytesWithHeader(object)
            - (J9MODRON_HEAP_SLOTS_PER_MARK_BIT * sizeof(UDATA));
        Assert_MM_true(objectSizeDelta <= freeSizeInBytes);
        freeSizeInBytes -= objectSizeDelta;
        address = (UDATA*)(((UDATA)address) + objectSizeDelta);

        if (((MM_MemoryPoolBumpPointer*)sweepChunk->memoryPool)
                ->canMemoryBeConnectedToPool(env, address, freeSizeInBytes)) {
