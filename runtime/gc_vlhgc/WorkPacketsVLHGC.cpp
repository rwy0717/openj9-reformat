 ...
 */
MM_WorkPacketsVLHGC* MM_WorkPacketsVLHGC::newInstance(MM_EnvironmentBase* env, MM_CycleState::CollectionType markType)
{
    MM_WorkPacketsVLHGC* workPackets;

    workPackets = (MM_WorkPacketsVLHGC*)env->getForge()->allocate(
        sizeof(MM_WorkPacketsVLHGC), MM_AllocationCategory::WORK_PACKETS, J9_GET_CALLSITE());
    if (NULL != workPackets) {
 ...
 */
MM_WorkPacketOverflow* MM_WorkPacketsVLHGC::createOverflowHandler(MM_EnvironmentBase* env, MM_WorkPackets* wp)
{
...
void MM_WorkPacketsVLHGC::wakeUpThreads(MM_EnvironmentBase* env) { MM_WorkPackets::notifyWaitingThreads(env); }
