...
void MM_ParallelWriteOnceCompactTask::run(MM_EnvironmentBase* envBase)
{
    MM_EnvironmentVLHGC* env = MM_EnvironmentVLHGC::getEnvironment(envBase);
    _compactScheme->compact(env);
}

void MM_ParallelWriteOnceCompactTask::setup(MM_EnvironmentBase* envBase)
{
    MM_EnvironmentVLHGC* env = MM_EnvironmentVLHGC::getEnvironment(envBase);
...
void MM_ParallelWriteOnceCompactTask::cleanup(MM_EnvironmentBase* envBase)
{
    MM_EnvironmentVLHGC* env = MM_EnvironmentVLHGC::getEnvironment(envBase);

    static_cast<MM_CycleStateVLHGC*>(env->_cycleState)
        ->_vlhgcIncrementStats._compactStats.merge(&env->_compactVLHGCStats);
...
void MM_ParallelWriteOnceCompactTask::masterSetup(MM_EnvironmentBase* envBase)
{
    MM_EnvironmentVLHGC* env = MM_EnvironmentVLHGC::getEnvironment(envBase);
...
void MM_ParallelWriteOnceCompactTask::masterCleanup(MM_EnvironmentBase* envBase)
{
    MM_EnvironmentVLHGC* env = MM_EnvironmentVLHGC::getEnvironment(envBase);
 ...
 */
MM_WriteOnceCompactor* MM_WriteOnceCompactor::newInstance(MM_EnvironmentVLHGC* env)
{
    MM_WriteOnceCompactor* compactScheme = (MM_WriteOnceCompactor*)env->getForge()->allocate(
        sizeof(MM_WriteOnceCompactor), MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
...
bool MM_WriteOnceCompactor::initialize(MM_EnvironmentVLHGC* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    _interRegionRememberedSet = _extensions->interRegionRememberedSet;
    bool monitorInitialized
        = (0 == omrthread_monitor_init_with_name(&_workListMonitor, 0, "MM_WriteOnceCompactor::_workListMonitor"));
    ...
    if (monitorInitialized) {
        ...
        _compactGroupDestinations = (MM_CompactGroupDestinations*)j9mem_allocate_memory(
            sizeof(MM_CompactGroupDestinations) * compactGroups, OMRMEM_CATEGORY_MM);
        ...
        if (didAllocateTable) {
            ...
            for (UDATA i = 0; i < compactGroups; i++) {
                ...
                didAllocateTable = didAllocateTable
                    && _compactGroupDestinations[i].lock.initialize(
                           env, &_extensions->lnrlOptions, "MM_WriteOnceCompactor:_compactGroupDestinations[].lock");
            }
        }
    }
...
void MM_WriteOnceCompactor::tearDown(MM_EnvironmentVLHGC* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
 ...
 */
void MM_WriteOnceCompactor::kill(MM_EnvironmentVLHGC* env)
{
    tearDown(env);
...
private:
    J9Object* _destinationAddress; /**< The base address of the contiguous area to which the objects in this page are
                                      moving. */
    UDATA _growBits; /**< A bit map of objects in this page which need to grow by a slot when moving (support for new
                        hash) */
...
public:
    ...
    /**
     * Sets the estimated post-move size of this page.  Note that this will assert if we attempt to over-write any
     * existing size or destination address.
     * @param size The estimated post-move size of this page, in bytes
     */
    void setEstimatedSize(UDATA size)
    {
    ...
    /**
     * Retrieves the stored estimated post-move size of this page.  Note that this will assert if the size has been
     * replaced with a destination address.
     ...
     */
    bool isGrowBitSet(J9Object* objectPtr)
    {
     ...
     */
    void setGrowBit(J9Object* objectPtr)
    {
        UDATA offset = growBitOffset(objectPtr);
     ...
     */
    void clearGrowBit(J9Object* objectPtr)
    {
        UDATA offset = growBitOffset(objectPtr);
    ...
    /**
     * Reverts the table entry back to an unused state.  This is called in evacuation if we setup forwarding and grow
     * data for some objects in a page and then determine we are not able to evacuate the page.
     */
    void revert()
    {
...
typedef struct J9MM_FixupCache {
    /* Note that this cache could be made more generic than just remembering the fixup data for the previous and next
     * objects but these names make it easier to understand how we are presently using the cache.
     */
...
MMINLINE J9Object* MM_WriteOnceCompactor::getForwardWrapper(
    MM_EnvironmentVLHGC* env, J9Object* objectPtr, J9MM_FixupCache* cache)
{
...
void MM_WriteOnceCompactor::setCycleState(MM_CycleState* cycleState, MM_MarkMap* nextMarkMap)
{
...
void MM_WriteOnceCompactor::clearCycleState()
{
    _cycleState = MM_CycleStateVLHGC();
    _nextMarkMap = NULL;
}

void MM_WriteOnceCompactor::masterSetupForGC(MM_EnvironmentVLHGC* env)
{
...
void MM_WriteOnceCompactor::initRegionCompactDataForCompactSet(MM_EnvironmentVLHGC* env)
{
    ...
    while (NULL != (region = regionIterator.nextRegion())) {
        if (region->_compactData._shouldCompact) {
            ...
            /* clear all reference lists in compacted regions, since the GMP will need to rediscover all of these
             * objects */
...
void MM_WriteOnceCompactor::compact(MM_EnvironmentVLHGC* env)
{
    PORT_ACCESS_FROM_ENVIRONMENT(env);
    UDATA objectCount = 0;
    UDATA byteCount = 0;
    ...
    /* we need to clear the next mark map which underlies the regions we are going to compact since we need to store
     * relocation data there */
    ...
    /* tail-marking and planning will currently be handled as part of setup but we may want more specific timers in the
     * future */
    /* first, tail-mark the regions in the collection set */
    /* synchronize here so that initialization finishes before tail marking starts (once tail marking is part of
     * planning, this sync point can be re-evaluated) */
    env->_currentTask->synchronizeGCThreads(env, UNIQUE_ID);
    ...
    /* fixup in-progress mark map and work stack.  The mark map fixup must be done by one thread (since it must be
     * low-to-high, to avoid fixing up twice).  The work packets can be fixed up in parallel, however, since each packet
     * is logically independent of the others. In order to better balance the cost of mark map fixup, the first thread
     * in will handle the entire mark map while the other threads fixup the work packets.
     */
...
#if defined(J9VM_GC_ARRAYLETS)
    if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
        /* now, correct any leaf pages to point at their moved spines - this must be called AFTER (aka:  post-sync) we
         * are sure to have finished fixing up leaf contents since it relies on knowing exactly which version of the
         * spine pointer it sees */
    ...
    if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
         ...
         * (recycling is single-threaded since it modifies a linked list in the parent memory pool and rebuilding the
         * free list is not enough work to justify the overhead of parallel synchronization)
         */
        recycleFreeRegionsAndFixFreeLists(env);
    }
    env->_compactVLHGCStats._recycleEndTime = j9time_hires_clock();

    /* sync because we are about to rebuild the mark maps so the heap must be walkable and we can no longer use the
     * fixup data */
    env->_currentTask->synchronizeGCThreads(env, UNIQUE_ID);
    ...
    /* rebuilding the mark bits requires that we can use the forwarding data stored in the next mark map so synchronize
     * before clearMarkMapCompactSet destroys this data */
    env->_currentTask->synchronizeGCThreads(env, UNIQUE_ID);
    ...
    if (NULL != stateToFixup) {
        ...
        /* sync because we are about to rebuild the next mark map which requires that we have finished clearing it
         * (clearMarkMapCompactSet) */
        env->_currentTask->synchronizeGCThreads(env, UNIQUE_ID);
...
void MM_WriteOnceCompactor::fixupExternalWorkPackets(MM_EnvironmentVLHGC* env, MM_WorkPacketsVLHGC* packets)
{
                ...
                while (NULL != (slot = slotIterator.nextSlot())) {
                    ...
                    if (PACKET_INVALID_OBJECT != (UDATA)pointer) {
                        MM_HeapRegionDescriptorVLHGC* region
                            = (MM_HeapRegionDescriptorVLHGC*)_regionManager->tableDescriptorForAddress(pointer);
                        if (region->_compactData._shouldCompact) {
...
void MM_WriteOnceCompactor::rebuildNextMarkMapFromPackets(
    MM_EnvironmentVLHGC* env, MM_WorkPacketsVLHGC* packets, MM_MarkMap* markMap)
{
...
void MM_WriteOnceCompactor::evacuatePage(
    MM_EnvironmentVLHGC* env, void* page, J9MM_FixupTuple* previousTwoObjects, J9Object** newLocationPtr)
{
    ...
    for (UDATA bias = 0; bias < sizeof_page; bias += J9MODRON_HEAP_BYTES_PER_UDATA_OF_HEAP_MAP) {
        ...
        while (NULL != objectPtr) {
            ...
            /* objectPtr is a head so see if there is a tail in this mark map word and skip it (if not, remember that we
             * need to skip the first bit in the next word) */
            ...
            if (newLocation != objectPtr) {
                ...
                /* the object has just moved so we can't trust the remembered bit.  Clear it and it will be re-added,
                 * during fixup, if required */
...
void MM_WriteOnceCompactor::fixupNonMovingPage(MM_EnvironmentVLHGC* env, void* page)
{
    bool skipTail = false;
    for (UDATA bias = 0; bias < sizeof_page; bias += J9MODRON_HEAP_BYTES_PER_UDATA_OF_HEAP_MAP) {
        void* subPageBase = (void*)((UDATA)page + bias);
...
MMINLINE void MM_WriteOnceCompactor::trailingObjectFixup(MM_EnvironmentVLHGC* env, J9MM_FixupTuple* previousObjects)
{
...
void MM_WriteOnceCompactor::moveObjects(MM_EnvironmentVLHGC* env)
{
    ...
    while (NULL != (region = popWork(env))) {
        if (region->_compactData._shouldCompact) {
            ...
            /* since we typically work on one destination region (existingTargetRegion), cache the high address and
             * update it only when the destination region changes */
            ...
            for (page = startAddress; (NULL == earlyExit) && (page < high); page = (void*)((UDATA)page + sizeof_page)) {
                ...
                if (NULL != target) {
                    MM_HeapRegionDescriptorVLHGC* newTargetRegion
                        = (MM_HeapRegionDescriptorVLHGC*)_regionManager->tableDescriptorForAddress(target);
                    ...
                    if (region == existingTargetRegion) {
                        ...
                        /* for now, we will state that there should be two pages at the target (although we could go
                         * tighter than this, so long as it is enough to account for object growth) */
                        ...
                        if ((postMoveEnd <= endOfEvacuatedTarget)
                            || (targetRegionHighAddress == endOfEvacuatedTarget)) {
                            /* do the evacuation */
                            evacuatePage(env, page, previousObjects, &newLocation);
                        } else {
            ...
            /* since we just moved some objects in this region, clear the card table under them since they are now
             * invalid */
        ...
        } else if ((MM_CycleState::CT_GLOBAL_GARBAGE_COLLECTION == env->_cycleState->_collectionType)
            && (region->_criticalRegionsInUse > 0)) {
            /* in the case of a global collection, mark will have avoided updating the RSCL but this region has pinned
             * objects so the entire region must be walked for fixup */
            void* lowAddress = region->getLowAddress();
            void* highAddress = region->getHighAddress();
            for (void* cardToScan = lowAddress; cardToScan < highAddress;
                 cardToScan = (void*)((UDATA)cardToScan + CARD_SIZE)) {
            ...
            /* there is some fixup work to do while we wait for move work to become available.  Clean cards for this
             * subarea */
 ...
 */
MMINLINE void MM_WriteOnceCompactor::saveForwardingPtr(J9Object* objectPtr, J9Object* forwardingPtr)
{
...
MM_WriteOnceCompactor::tailMarkObjectsInRegion(MM_EnvironmentVLHGC* env, MM_HeapRegionDescriptorVLHGC* region)
{
    ...
    /* walk the objects in this page (following this object) in their previous mark map and tail-mark them to denote
     * their sizes */
    ...
    if (estimatedSize > region->getSize()) {
        /* make sure that the compact destination doesn't overflow - we can't compact this region so just make the
         * destination the top of the region */
...
J9Object* MM_WriteOnceCompactor::getForwardingPtr(J9Object* objectPtr) const
{
    ...
    if (!region->_compactData._shouldCompact) {
        ...
        if (NULL == targetBaseAddress) {
            ...
            /* by this point, we know that objects in this page have moved in some way (might have only been the case of
             * compaction within the page but movement, nonetheless) */
            ...
            if ((newPageIndex == oldPageIndex) && (targetBaseAddress >= objectPtr)) {
                /* this only happens when internally compacting a page - we are to ignore the forwarding pointer since
                 * it implies that this object was before the sliding objects and did not move */
                forwardPointer = objectPtr;
            } else {
                /* sliding compact or not, this object moved */
                bool isSliding = (oldPageIndex == newPageIndex);
                if (isSliding) {
                    /* this is a sliding compact within one page but the object is after the forwarding pointer so it is
                     * sliding down */
 ...
 */
static U_8 combinedByteValueLookupTable[] = { 0x8,  };

/* the number of bits representing live data in a mark map byte with value offset, assuming we ARE NOT already in an
 * object*/
#define BITS_OCCUPIED(offset) (combinedByteValueLookupTable[offset] >> 4)
/* the number of bits representing live data in a mark map byte with value offset, assuming we ARE already in an
 * object*/
...
MM_WriteOnceCompactor::bytesAfterSlidingTargetToLocateObject(J9Object* oldPointer, J9Object* slidingTarget) const
{
    ...
    void* base = pageStart(oldPageIndex);
    /* we need to know if we have seen an even or odd number of bits, to know if each mark map byte starts inside an
     * object, or not */
    UDATA bitsCounted = 0;
    U_8* heapMapBits = (U_8*)_cycleState._markMap->getHeapMapBits();
    /* initialize the map pointer to the first mark map word in the page.  We will skip ahead by one slot each time
     * through this loop */
    UDATA* mapPointer
        = (UDATA*)(heapMapBits + (((UDATA)base - (UDATA)_heapBase) / J9MODRON_HEAP_SLOTS_PER_HEAPMAP_SLOT));
    /* we will count the number of live bytes represented by each word of the mark map since it is our native unit of
     * computation so loop over the page, heap map word at a time */
    for (UDATA bias = 0; (((UDATA)base + bias) < (UDATA)oldPointer) && (bias < sizeof_page);
        ...
        if ((slidingTarget > baseAddress) && (pageIndex((J9Object*)baseAddress) == slidingIndex)) {
            /* mask off the early part of the word since we don't want to count the size consumed before the base
        ...
        if ((oldPointer < next) && (oldPointer >= baseAddress)) {
            /* mask off the high part of the word since we don't want to count the size consumed after the object we are
             * looking up */
        ...
        /* now, count the number of bytes consumed in each byte of the mark map, after we have masked off the parts in
         * which we aren't interested */
    ...
    if (totalSize > 0) {
        /* if there were objects before this, account for the number of objects which have grown (each object needs to
         * be aligned so add object alignment as the "per-object growth quantity" even though the hash is likely smaller
         * than this)
         */
...
void MM_WriteOnceCompactor::fixupObjectsInRange(
...
void MM_WriteOnceCompactor::fixupMixedObject(MM_EnvironmentVLHGC* env, J9Object* objectPtr, J9MM_FixupCache* cache)
{
...
#endif
void MM_WriteOnceCompactor::fixupClassObject(MM_EnvironmentVLHGC* env, J9Object* classObject, J9MM_FixupCache* cache)
{
...
void MM_WriteOnceCompactor::fixupClassLoaderObject(
    MM_EnvironmentVLHGC* env, J9Object* classLoaderObject, J9MM_FixupCache* cache)
{
...
void MM_WriteOnceCompactor::fixupPointerArrayObject(
    MM_EnvironmentVLHGC* env, J9Object* objectPtr, J9MM_FixupCache* cache)
{
    /* object may have moved so ensure that its class loader knows where it is */
    _extensions->classLoaderRememberedSet->rememberInstance(env, objectPtr);

    /* arraylet leaves are walked separately in fixupArrayletLeafRegionContentsAndObjectLists(), to increase
     * parallelism. Just walk the spine */
    GC_ArrayletObjectModel::ArrayLayout layout
        = _extensions->indexableObjectModel.getArrayLayout((J9IndexableObject*)objectPtr);
    ...
    if (GC_ArrayletObjectModel::InlineContiguous == layout) {
        ...
        if (NULL != GC_PointerArrayIterator(_javaVM, objectPtr).nextSlot()) {
            ...
            UDATA totalElementCount
                = _extensions->indexableObjectModel.getSizeInElements((J9IndexableObject*)objectPtr);
            UDATA externalArrayletCount
                = _extensions->indexableObjectModel.numExternalArraylets((J9IndexableObject*)objectPtr);
...
void MM_WriteOnceCompactor::fixupObject(MM_EnvironmentVLHGC* env, J9Object* objectPtr, J9MM_FixupCache* cache)
{
...
void MM_WriteOnceCompactor::writeFlushToCardState(Card* card, bool isGlobalMarkPhaseRunning)
{
    Card fromState = *card;
    ...
    case CARD_CLEAN:
        if (isGlobalMarkPhaseRunning) {
            /* This card may refer an object for which we do not rebuild next mark bits via Work Packet fixup so ensure
             * that the GMP has to see it */
...
void MM_WriteOnceCompactor::flushRememberedSetIntoCardTable(MM_EnvironmentVLHGC* env)
{
                ...
                if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
                    ...
                    while (0 != (card = rsclCardIterator.nextReferencingCard(env))) {
                        ...
                        MM_HeapRegionDescriptorVLHGC* targetRegion
                            = _interRegionRememberedSet->tableDescriptorForRememberedSetCard(card);
                    ...
                    /* Clear remembered references to each region in Collection Set (completely clear RS Card List for
                     * those regions and appropriately update RSM). We are about to rebuild those references in PGC.
                     */
...
#if defined(J9VM_GC_ARRAYLETS)
void MM_WriteOnceCompactor::tagArrayletLeafRegionsForFixup(MM_EnvironmentVLHGC* env)
{
    GC_HeapRegionIteratorVLHGC regionIterator(_regionManager);
    MM_HeapRegionDescriptorVLHGC* region = NULL;
    while (NULL != (region = regionIterator.nextRegion())) {
...
public:
    MM_WriteOnceCompactFixupRoots(
        MM_EnvironmentVLHGC* env, MM_WriteOnceCompactor* compactScheme, void* heapBase, void* heapTop)
        : MM_RootScanner(env)
...
void MM_WriteOnceCompactor::fixupRoots(MM_EnvironmentVLHGC* env)
{
...
void MM_WriteOnceCompactor::rebuildMarkbits(MM_EnvironmentVLHGC* env)
{
    ...
    /* all threads will implicitly synchronize in the above loop and only exit once all work is done so assert that they
     * all see work being completed */
    Assert_MM_true(NULL == _rebuildWorkList);
}

void* MM_WriteOnceCompactor::rebuildMarkbitsInRegion(MM_EnvironmentVLHGC* env, MM_HeapRegionDescriptorVLHGC* region)
{
    ...
    /* obviously, the starting address has to come first (protects against stale data and ensures that we didn't just
     * get a region which is already done) */
    Assert_MM_true(startAddress < high);
    /* the page we are presently trying to rebuild "from" */
    void* page = NULL;
    /* earlyExit is used in case we run into blocking point (that is, we find a page whose destination's mark bits have
     * not yet been moved to their new location) */
    void* earlyExit = NULL;
    /* evacuationTarget is used in the case where earlyExit is used - it represents the destination address upon which
     * we are blocked */
    void* evacuationTarget = NULL;
    ...
    for (page = startAddress; (NULL == earlyExit) && (page < high); page = (void*)((UDATA)page + sizeof_page)) {
        ...
        if (NULL != target) {
            ...
            if (region == targetRegion) {
                ...
                /* we are evacuating so check the state of the target region and ensure that it isn't still waiting to
                 * evacuate the mark bits in the location where we want to write ours */
                void* rebuildMarkerInTarget = targetRegion->_compactData._nextRebuildCandidate;
                /* for now, we will state that there should be at least two pages at the target (although we could go
                 * tighter than this, so long as it is enough to account for object growth) */
                if ((((UDATA)target + (2 * sizeof_page)) <= (UDATA)rebuildMarkerInTarget)
                    || (rebuildMarkerInTarget == targetRegion->getHighAddress())) {
                    rebuildMarkMapInMovingPage(env, markMap, page);
                } else {
    ...
    if (NULL == earlyExit) {
        /* we are finished so ensure that page points at the end of the region, we have no evacuationTarget, and then
        ...
        /* something was in the way so we need to record the address we will pick up from and then return the address we
         * were blocked on */
        Assert_MM_true(NULL != evacuationTarget);
...
void MM_WriteOnceCompactor::verifyHeapObjectSlot(J9Object* object)
{
...
void MM_WriteOnceCompactor::verifyHeapMixedObject(J9Object* objectPtr)
{
    GC_MixedObjectIterator it(_javaVM->omrVM, objectPtr);
    while (GC_SlotObject* slotObject = it.nextSlot()) {
...
void MM_WriteOnceCompactor::verifyHeapArrayObject(J9Object* objectPtr)
{
    GC_PointerArrayIterator it(_javaVM, objectPtr);
    while (GC_SlotObject* slotObject = it.nextSlot()) {
        verifyHeapObjectSlot(slotObject->readReferenceFromSlot());
...
void MM_WriteOnceCompactor::verifyHeap(MM_EnvironmentVLHGC* env, bool beforeCompaction)
{
    ...
    while (NULL != (region = regionIterator.nextRegion())) {
        ...
        MM_HeapMapIterator markedObjectIterator(
            _extensions, _cycleState._markMap, (UDATA*)lowAddress, (UDATA*)highAddress);
...
void MM_WriteOnceCompactor::recycleFreeRegionsAndFixFreeLists(MM_EnvironmentVLHGC* env)
{
    ...
    while (NULL != (region = regionIterator.nextRegion())) {
        if (region->_compactData._shouldCompact) {
            ...
            if (region->getLowAddress() == currentFreeBase) {
                /* if we are recycling a region, we better not have tried to make it a compact destination, migrating
                 * contexts */
                ...
                if (NULL != region->_compactData._previousContext) {
                    /* we migrated this region into a new context so ask its previous owner to migrate the contexts'
                     * views of the region's ownership to make the meta-structures consistent */
                    region->_compactData._previousContext->migrateRegionToAllocationContext(
                        region, region->_allocateData._owningContext);
                ...
                if (currentFreeSize > regionPool->getMinimumFreeEntrySize()) {
                    /* The MemoryPoolBumpPointer doesn't require that its memory be walkable so just update the top
                     * pointer (which is more for consistency of the meta-structure than anything else) */
 ...
 */
void MM_WriteOnceCompactor::updateInternalLeafPointersAfterCopy(
...
void MM_WriteOnceCompactor::fixupArrayletLeafRegionSpinePointers()
{
    GC_HeapRegionIteratorVLHGC regionIterator(_regionManager);
    MM_HeapRegionDescriptorVLHGC* region = NULL;
    ...
    while (NULL != (region = regionIterator.nextRegion())) {
        ...
        if (NULL != spine) {
            ...
            if (newSpine != spine) {
                MM_HeapRegionDescriptorVLHGC* spineRegion
                    = (MM_HeapRegionDescriptorVLHGC*)_regionManager->tableDescriptorForAddress(spine);
                MM_HeapRegionDescriptorVLHGC* newSpineRegion
                    = (MM_HeapRegionDescriptorVLHGC*)_regionManager->tableDescriptorForAddress(newSpine);

                /* Note that the previous spine region may be recycled while we are fixing up this region (recycling is
                 * done in parallel with this method) so we can't assert anything about the state of the previous
                 * region.
                 */
...
void MM_WriteOnceCompactor::fixupArrayletLeafRegionContentsAndObjectLists(MM_EnvironmentVLHGC* env)
{
    GC_HeapRegionIteratorVLHGC regionIterator(_regionManager);
    MM_HeapRegionDescriptorVLHGC* region = NULL;
            ...
            if (!region->getUnfinalizedObjectList()->wasEmpty()) {
                if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
                    ...
                    while (NULL != pointer) {
                        ...
                        /* store the object in this thread's buffer. It will be flushed to the appropriate list when
                         * necessary. */
...
void MM_WriteOnceCompactor::clearMarkMapCompactSet(MM_EnvironmentVLHGC* env, MM_MarkMap* markMap)
{
    GC_HeapRegionIteratorVLHGC regionIterator(_regionManager);
    MM_HeapRegionDescriptorVLHGC* region = NULL;
...
void MM_WriteOnceCompactor::planCompaction(
    MM_EnvironmentVLHGC* env, UDATA* objectCount, UDATA* byteCount, UDATA* skippedObjectCount)
{
    ...
    while (NULL != (region = regionIterator.nextRegion())) {
        if (region->_compactData._shouldCompact) {
            if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
                ...
                region->_compactData._projectedLiveBytesRatio
                    = (double)region->_projectedLiveBytes / (double)postMoveRegionSizeInBytes;
...
void MM_WriteOnceCompactor::planRegion(MM_EnvironmentVLHGC* env, MM_HeapRegionDescriptorVLHGC* subAreaRegion,
    UDATA postMoveRegionSizeInBytes, UDATA* objectCount, UDATA* byteCount, UDATA* skippedObjectCount)
{
    ...
    while (targetSpaceRequired > 0) {
        ...
        if (NULL == targetPointer) {
            ...
            doPlanSlide(env, sourceRegionBase, (J9Object*)copyStart, (J9Object*)sourceRegionTop, objectCount,
                &bytesConsumedInMove);
            *byteCount += bytesConsumedInMove;
            ...
            UDATA bytesConsumedInMove = 0;
            copyStart = doPlanEvacuate(env, &freeChunk, topEdge, (J9Object*)copyStart, (J9Object*)sourceRegionTop,
                objectCount, &bytesConsumedInMove);
            *byteCount += bytesConsumedInMove;
            Assert_MM_true(bytesConsumedInMove <= targetSpaceRequired);
...
bool MM_WriteOnceCompactor::getEvacuateExtent(MM_EnvironmentVLHGC* env, UDATA targetSpaceRequired,
    MM_HeapRegionDescriptorVLHGC* subAreaRegion, void** targetPointer, void** topEdge)
{
    ...
    if (NULL == _compactGroupDestinations[compactGroupIndex].head) {
        ...
        subAreaRegion->_compactData._vineDepth
            = OMR_MAX(subAreaRegion->_compactData._vineDepth, destinationRegion->_compactData._vineDepth + 1);
        /*
         ...
         */
        destinationRegion->_projectedLiveBytes
            += (UDATA)subAreaRegion->_compactData._projectedLiveBytesRatio * ((UDATA)top - (UDATA)base);
    }
...
J9Object* MM_WriteOnceCompactor::doPlanEvacuate(MM_EnvironmentVLHGC* env, void** copyDestinationBase,
    void* copyDestinationTop, J9Object* firstToCopy, J9Object* endOfCopyBlock, UDATA* objectCount, UDATA* byteCount)
{
    ...
    Assert_MM_true(0 == (sizeof_page % J9MODRON_HEAP_BYTES_PER_UDATA_OF_HEAP_MAP));
    /* the compact destination starts at the deadObject we were given but will be bumped along as we compact.  We need
     * to save this back to deadObject, at the end, since it is an in/out parameter */
    ...
    Assert_MM_true(_regionManager->tableDescriptorForAddress(firstToCopy)
        != _regionManager->tableDescriptorForAddress(compactDestination));
    ...
    for (IDATA page = startPage; (NULL == firstObjectNotEvacuated) && (page < finishPage); page++) {
        ...
        if (0 == estimatedSizeOfPage) {
            ...
            /* we are going to copy this page so find the first object for relocation data, count the number of objects,
             * and account for the size of the page */
            /* first, find the object */
            J9Object* firstObject = NULL;
            for (UDATA bias = 0; (NULL == firstObject) && (bias < sizeof_page);
                 bias += J9MODRON_HEAP_BYTES_PER_UDATA_OF_HEAP_MAP) {
            ...
            for (UDATA bias = 0; (NULL == firstObjectNotEvacuated) && (bias < sizeof_page);
                 bias += J9MODRON_HEAP_BYTES_PER_UDATA_OF_HEAP_MAP) {
                void* baseAddress = (void*)((UDATA)baseOfPage + bias);
                MM_HeapMapWordIterator pagePieceIterator(_cycleState._markMap, baseAddress);
...
void MM_WriteOnceCompactor::doPlanSlide(MM_EnvironmentVLHGC* env, void* copyDestinationBase, J9Object* firstToCopy,
    J9Object* endOfCopyBlock, UDATA* objectCount, UDATA* byteCount)
{
    /* we use the MM_HeapMapWordIterator in this function so ensure that it is safe (can't be tested during init since
     * assertions don't work at that point) */
    Assert_MM_true(0 == (sizeof_page % J9MODRON_HEAP_BYTES_PER_UDATA_OF_HEAP_MAP));
    ...
    MM_HeapMapIterator markedObjectIterator(
        _extensions, _cycleState._markMap, (UDATA*)firstToCopy, (UDATA*)pageStart(pageIndex(endOfCopyBlock)), false);
    ...
    for (objectPtr = markedObjectIterator.nextObject(); NULL != objectPtr; objectPtr = nextObject) {
        ...
        /* if there is a compact page boundary between objectPtr and nextObject, nextObject is a head pointer.
         * Otherwise, it is a tail */
        if (objectIndex == nextIndex) {
            J9Object* tail = nextObject;
            nextObject = markedObjectIterator.nextObject();
            postMoveObjectSize = (UDATA)tail - (UDATA)objectPtr + J9MODRON_HEAP_BYTES_PER_HEAPMAP_BIT
                + (_compactTable[objectIndex].isGrowBitSet(objectPtr) ? _objectAlignmentInBytes : 0);
        } else {
        ...
        if (compactDestination == objectPtr) {
            ...
            /* save out the forwarding pointer if the object moved (otherwise, there is no point and we would confuse an
             * assertion in getForwardingPtr which assures that an object never moves on top of itself) */
    ...
    /* if the destination, after planning every object, extends beyond the end of the extent we were sliding then we are
     * going to corrupt the next extent (which would be the next region) and this region should never have been
     * compacted */
...
void MM_WriteOnceCompactor::setupMoveWorkStack(MM_EnvironmentVLHGC* env)
{
...
MM_HeapRegionDescriptorVLHGC* MM_WriteOnceCompactor::popWork(MM_EnvironmentVLHGC* env)
{
    omrthread_monitor_enter(_workListMonitor);
    while ((NULL == _readyWorkListHighPriority) && (NULL == _readyWorkList) && (NULL == _fixupOnlyWorkList)
        && !_moveFinished) {
        _threadsWaiting += 1;
        if (env->_currentTask->getThreadCount() == _threadsWaiting) {
            _moveFinished = true;
            if (_extensions->tarokEnableExpensiveAssertions) {
                /* verify that move didn't discard or otherwise lose any regions.  Every region in the compact set
                 * should now be in the rebuild work list */
                MM_HeapRegionDescriptorVLHGC* region = NULL;
                GC_HeapRegionIteratorVLHGC regionIterator(_regionManager);
...
void MM_WriteOnceCompactor::pushMoveWork(MM_EnvironmentVLHGC* env, MM_HeapRegionDescriptorVLHGC* finishedRegion,
    void* evacuationTarget, UDATA evacuationSize)
{
    ...
    if (nextEvacuationCandidate < finishedRegion->getHighAddress()) {
         ...
         */
        /* if we setting this regions aside and it isn't completely finished, we must have specified the target upon
         * which we were blocked */
        Assert_MM_true(NULL != evacuationTarget);
        /* look up the region into which we were attempting to evacuate objects */
        MM_HeapRegionDescriptorVLHGC* targetRegion
            = (MM_HeapRegionDescriptorVLHGC*)_regionManager->tableDescriptorForAddress(evacuationTarget);
        /* we can't push a region which is blocked on itself */
        Assert_MM_true(targetRegion != finishedRegion);
        /* look up how far into the target region the evacuation has progressed (since it is possible that it proceeded
         * past our blocking point before we picked up the monitor) */
        ...
        if (targetProgress < (void*)((UDATA)evacuationTarget + evacuationSize)) {
            /* if the target region still hasn't progressed past the end of the extent we would require (our target plus
             * the number of bytes we need), then put finishedRegion on its blocked list */
            ...
            /* the target has proceeded far enough while we acquired the monitor that it is no longer blocked so add it
             * to the work stack */
            pushRegionOntoWorkStack(&_readyWorkList, &_readyWorkListHighPriority, finishedRegion);
        }
    } else {
        /* we are going to drop this entry, since it is fully evacuated, but first make sure that we enqueue all the
         * items from its blocked list */
...
MM_HeapRegionDescriptorVLHGC* MM_WriteOnceCompactor::popRebuildWork(MM_EnvironmentVLHGC* env)
{
    omrthread_monitor_enter(_workListMonitor);
    while ((NULL == _rebuildWorkListHighPriority) && (NULL == _rebuildWorkList) && !_rebuildFinished) {
        ...
        if (env->_currentTask->getThreadCount() == _threadsWaiting) {
            ...
            if (_extensions->tarokEnableExpensiveAssertions) {
                /* ensure that none of the regions in the compact set are still in a work list and that no regions are
                 * blocked */
                MM_HeapRegionDescriptorVLHGC* region = NULL;
                GC_HeapRegionIteratorVLHGC regionIterator(_regionManager);
                while (NULL != (region = regionIterator.nextRegion())) {
...
void MM_WriteOnceCompactor::pushRebuildWork(
    MM_EnvironmentVLHGC* env, MM_HeapRegionDescriptorVLHGC* finishedRegion, void* evacuationTarget)
{
    /* rebuildMarkbitsInRegion assumes that one source page could consume more than one but less than 2 destination
     * pages so allow 2 pages of clearance */
    ...
    if (nextRebuildCandidate < finishedRegion->getHighAddress()) {
         ...
         * entry has since become complete - in which case we put the entry back in the ready list)
         */
        /* if we aren't finished rebuilding this region, it must be blocked on something, so ensure that it has a
         * non-NULL target */
        Assert_MM_true(NULL != evacuationTarget);
        ...
        Assert_MM_true(targetRegion != finishedRegion);
        /* see where the progress of the target is in terms of what it will be rebuilding next (it might have progressed
         * past evacuationTarget before we got the monitor so we might be able to return this work immediately to the
         * ready stack)
         */
        void* targetProgress = targetRegion->_compactData._nextRebuildCandidate;
        /* targetHighEdge is a conservative estimate of the end of the heap area for which we might need to update the
         * mark map for at least one page of movement (obviously must be capped at the top of the region) */
        void* targetHighEdge
            = OMR_MIN(targetRegion->getHighAddress(), (void*)((UDATA)evacuationTarget + evacuationSize));
        ...
        if (targetProgress < targetHighEdge) {
            /* we are actually blocked on our target so just make sure it isn't finished (which would mean we stored an
             * invalid target) and then add us to its blocked queue */
            Assert_MM_true(targetProgress != targetRegion->getHighAddress());
            finishedRegion->_compactData._nextInWorkList = targetRegion->_compactData._blockedList;
            targetRegion->_compactData._blockedList = finishedRegion;
        } else {
        ...
        /* we are going to drop this entry, since it is fully rebuilt, but first make sure that we enqueue all the items
...
void MM_WriteOnceCompactor::rebuildMarkMapInMovingPage(MM_EnvironmentVLHGC* env, MM_MarkMap* markMap, void* pageBase)
{
    /* the page starts on a head but each J9MODRON_HEAP_BYTES_PER_UDATA_OF_HEAP_MAP may not so remember whether we are
     * expecting a tail or head bit */
    bool skipTail = false;
    ...
    for (UDATA bias = 0; bias < sizeof_page; bias += J9MODRON_HEAP_BYTES_PER_UDATA_OF_HEAP_MAP) {
        ...
        /* read the word and clear it in case the objects moved into this mark word.  We will manually feed this mark
         * word into the iterator so we won't be modifying the data under the iterator */
        ...
        if (skipTail) {
            ...
            if (NULL != tail) {
                UDATA size = ((UDATA)tail - (UDATA)previousHead)
                    + (_compactTable[index].isGrowBitSet(previousHead) ? _objectAlignmentInBytes : 0);
...
void MM_WriteOnceCompactor::removeTailMarksInPage(MM_EnvironmentVLHGC* env, MM_MarkMap* markMap, void* pageBase)
{
    UDATA slotIndex = markMap->getSlotIndex((J9Object*)pageBase);
    bool isHead = true;
    for (UDATA bias = 0; bias < sizeof_page; bias += J9MODRON_HEAP_BYTES_PER_UDATA_OF_HEAP_MAP) {
...
MM_HeapRegionDescriptorVLHGC* MM_WriteOnceCompactor::popNextRegionFromWorkStack(
    MM_HeapRegionDescriptorVLHGC** workStackBase)
{
...
void MM_WriteOnceCompactor::pushRegionOntoWorkStack(MM_HeapRegionDescriptorVLHGC** workStackBase,
    MM_HeapRegionDescriptorVLHGC** workStackBaseHighPriority, MM_HeapRegionDescriptorVLHGC* region)
{
...
#if defined(J9VM_GC_FINALIZATION)
void MM_WriteOnceCompactor::fixupFinalizableObjects(MM_EnvironmentVLHGC* env)
{
...
void MM_WriteOnceCompactor::fixupFinalizableList(MM_EnvironmentVLHGC* env, j9object_t headObject)
{
...
void MM_WriteOnceCompactor::rememberClassLoaders(MM_EnvironmentVLHGC* env)
{
    /* TODO: ideally we should have per region lists of class loaders and call this from clearMarkMapCompactSet(),
     * improving its parallelism */
            ...
            if (J9_ARE_ANY_BITS_SET(classLoader->flags, J9CLASSLOADER_ANON_CLASS_LOADER)) {
                ...
                while (NULL != (segment = segmentIterator.nextSegment())) {
                    ...
                    while (NULL != (clazz = classHeapIterator.nextClass())) {
                        ...
                        Assert_MM_true(NULL != classObject);
                        MM_HeapRegionDescriptorVLHGC* region
                            = (MM_HeapRegionDescriptorVLHGC*)_regionManager->tableDescriptorForAddress(classObject);
                ...
                if (NULL != classLoaderObject) {
                    MM_HeapRegionDescriptorVLHGC* region
                        = (MM_HeapRegionDescriptorVLHGC*)_regionManager->tableDescriptorForAddress(classLoaderObject);
...
void MM_WriteOnceCompactor::rebuildNextMarkMapFromClassLoaders(MM_EnvironmentVLHGC* env)
{
...
void MM_WriteOnceCompactor::clearClassLoaderRememberedSetsForCompactSet(MM_EnvironmentVLHGC* env)
{
