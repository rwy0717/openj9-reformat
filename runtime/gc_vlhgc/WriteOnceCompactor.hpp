...
private:
    ...
    MM_MarkMap* _nextMarkMap; /**< The next mark map which will be passed to the compactor (it uses it as temporary
                                 storage for fixup data) */
...
public:
     ...
     */
    MM_ParallelWriteOnceCompactTask(MM_EnvironmentBase* env, MM_Dispatcher* dispatcher,
        MM_WriteOnceCompactor* compactScheme, MM_CycleState* cycleState, MM_MarkMap* nextMarkMap)
...
private:
    ...
    MM_HeapRegionManager* const _regionManager; /**< The region manager which holds the MM_HeapRegionDescriptor
    ...
    MM_InterRegionRememberedSet*
        _interRegionRememberedSet; /**< A cached pointer to the  inter-region reference tracking  */
    omrthread_monitor_t
        _workListMonitor; /**< The monitor used to control work sharing of object movement/fixup tasks */
    MM_HeapRegionDescriptorVLHGC*
        _readyWorkList; /**< The root of the list of regions which can have some work done on them */
    MM_HeapRegionDescriptorVLHGC*
        _readyWorkListHighPriority; /**< Like _readyWorkList but is higher priority as it only contains regions which
                                       are compact destinations (that is, they block other move operations) */
    MM_HeapRegionDescriptorVLHGC* _fixupOnlyWorkList; /**< The root of the list of regions which must have their cards
                                                         cleaned in order to fixup references into the compact set */
    MM_HeapRegionDescriptorVLHGC*
        _rebuildWorkList; /**< The root of the list of regions which must have their previous mark map extents rebuilt
                             (this list is built as the object movement phase completes) */
    MM_HeapRegionDescriptorVLHGC* _rebuildWorkListHighPriority; /**< Like _rebuildWorkList but is higher priority as it
                                                                   only contains regions which are compact destinations
                                                                   (that is, they block other rebuild operations) */
    UDATA _threadsWaiting; /**< The number of threads waiting for work on _workListMonitor */
    bool _moveFinished; /**< A flag set when all move work is done to allow all threads waiting for work to exit the
                           monitor */
    bool _rebuildFinished; /**< A flag set when all mark map rebuilding work is done to allow all threads waiting for
                              work to exit the monitor */
    UDATA _lockCount; /**< Number of locks initialized for compact groups, based on NUMA. Stored for control to make
                         sure that we dealocate same number that we allocated in initialize. */
...
private:
    ...
    /**
     * A wrapper around the getForwardedPtr call which will try to apply its cache, first, and return immediately if the
     * objectPtr hits in the cache.  If it isn't in the cache, this method calls through to getForwardedPtr.
     ...
     */
    MMINLINE J9Object* pageStart(UDATA i) const { return (J9Object*)((UDATA)_heapBase + (i * sizeof_page)); }
     ...
     * @return If the region was fully rebuilt, this will be NULL.  Otherwise, it is the address, in another region,
     * which must be rebuilt before rebuilding this region can proceed
     */
    ...
    /**
     * Called to rebuild the given mark map from references in the given work packets.  Any packet slot which points
     * into the compact set is marked in the given mark map.
     *
     ...
     * This function is analogous to moveObjects.  It is meant to be the non-moving equivalent which only performs
     * planning actions but does not move objects or write to the heap.
     * @param env[in] The master GC thread
    ...
    /**
     * This function plans the compaction for the given subAreaRegion.  This may be an evacuation or an internal sliding
     * compaction, depending on what other regions are available as compaction targets.
     * @param env[in] A GC thread
     * @param subAreaRegion[in] The region for which the receiver must plan a compaction
     * @param postMoveRegionSizeInBytes[in] The size of all the objects in subAreaRegion, in bytes, if every object it
     * contains were to move
     * @param objectCount[in/out] The number of objects the receiver planned to move
     * @param byteCount[in/out] The number of bytes the receiver planned to move
     * @param skippedObjectCount[in/out] The number of objects the receiver planned to skip
     */
    void planRegion(MM_EnvironmentVLHGC* env, MM_HeapRegionDescriptorVLHGC* subAreaRegion,
     ...
     */
    bool getEvacuateExtent(MM_EnvironmentVLHGC* env, UDATA targetSpaceRequired,
        MM_HeapRegionDescriptorVLHGC* subAreaRegion, void** targetPointer, void** topEdge);

    /**
     * Part of planning refactoring (JAZZ 21595).
     * This function is analogous to doCompact.  It is meant to be the non-moving equivalent which only performs
     * planning actions but does not move objects or write to the heap.
     * @param env[in] The master GC thread
     * @param copyDestinationBase[in/out] The base address of object copy destinations.  This is updated to the byte
     * after the space consumed by the copy before this method returns
     ...
     * @param byteCount[in/out] The number of bytes the receiver planned to move
     */
    J9Object* doPlanEvacuate(MM_EnvironmentVLHGC* env, void** copyDestinationBase, void* copyDestinationTop,
        J9Object* firstToCopy, J9Object* endOfCopyBlock, UDATA* objectCount, UDATA* byteCount);
    /**
     * Part of planning refactoring (JAZZ 21595).
     * This function is analogous to doCompact.  It is meant to be the non-moving equivalent which only performs
     * planning actions but does not move objects or write to the heap.
     ...
     * @param byteCount[in/out] The number of bytes the receiver planned to move
     */
    void doPlanSlide(MM_EnvironmentVLHGC* env, void* copyDestinationBase, J9Object* firstToCopy,
        J9Object* endOfCopyBlock, UDATA* objectCount, UDATA* byteCount);

    /**
     * A helper used by moveObjects to do whatever cleanup is required at the end of a move extent.  This means fixing
     * up the previous object we moved and clearing out the contents of the previousObjects array.
     * @param env[in] A GC thread
     * @param previousObjects[in/out] The previous two objects moved.  The most recent one will be fixed up and then the
     * array will be cleared (MUST HAVE 2 ELEMENTS)
     */
    void trailingObjectFixup(MM_EnvironmentVLHGC* env, J9MM_FixupTuple* previousObjects);

    /**
     * Walks the previous mark map to find all objects in regions to be compacted and then, using forwarding pointer
     * data, finds their new locations and copies them.
     * @param env[in] The master GC thread
     */
     ...
     * @param previousTwoObjects[in/out] The fixup data for the previous two objects found in this move extent (may be
     * pre-populated and must be updated by this method)
     * @param newLocationPtr[in/out] The next destination of moved objects (or NULL if not yet computed).  This must be
    ...
    /**
     * Determine the size, in bytes, that the objects residing in the page based at page would consume after they are
     * moved.
     * @param env[in] A GC thread
    ...
    /**
     * Called in the master thread to do the initial population of the move work stack.  This call must be made prior to
     * moveObjects().
     * @param env[in] The master GC thread
     */
    ...
    /**
     * Pops a ready region from the move work stack or the fixup work stack.  Move work is favoured but fixup work will
     * be returned if any is available.  If no work is available, this method will block until move work becomes
     * available or all work is done (in which case, NULL will be returned). The caller must check the type of the
     * region underlying the subarea to see if this needs movement or fixup.
     * @param env[in] A GC thread
    ...
    /**
     * Pushes a region which has just been the subject of some amount of work.  If the entry is finished, it is
     * discarded (from the move stack but pushed onto the rebuild stack) and not pushed onto the stack.  If some work
     * remains, it is either pushed onto the ready work stack or pushed onto the blocked stack of the region which must
     * be evacuated (or partially evacuated) before this item is ready.
     * @param env[in] A GC thread
     * @param finishedRegion[in] The region which is being returned to the work stack because further progress is
     * blocked or because it is finished
     * @param evacuationTarget[in] If we are pushing this because we were blocked, this will be the target we are
     * waiting for (NULL otherwise)
     * @param evacuationSize If we are pushing this because we were blocked, this will be the size we need at
     * evacuationTarget
     */
    void pushMoveWork(MM_EnvironmentVLHGC* env, MM_HeapRegionDescriptorVLHGC* finishedRegion, void* evacuationTarget,
        UDATA evacuationSize);

    /**
     * Pops a ready region from the rebuild work stack.  If no work is available, this method will block until move work
     * becomes available or all work is done (in which case, NULL will be returned).
     * @param env[in] A GC thread
    ...
    /**
     * Pushes a region which has just been the subject of some amount of work.  If the entry is finished, it is
     * discarded and not pushed onto the stack.  If some work remains, it is either pushed onto the ready rebuild work
     * stack or pushed onto the blocked stack of the region which must be rebuilt (or partially rebuilt) before this
     * item is ready.
     * @param env[in] A GC thread
     * @param finishedRegion[in] The region which is being returned to the work stack because further progress is
     * blocked or because it is finished
    ...
    /**
     * Rebuilds the given mark map by clearing all the mark bits in the page at pageBase and setting the bits which
     * correspond to the new locations of the objects in that page.  The page is updated by one thread but, because
     * multiple pages can be copied into the same mark word, any mark map updates must either be atomic or must be able
     * to prove that only the one thread will be writing to the word.
     * @param env[in] A GC thread
    ...
    /**
     * Clears the tail markings in the given mark map fragment underlying the page at pageBase.  This update does not
     * require atomics as the page is a multiple of mark words.
     * @param env[in] A GC thread
     * @param markMap[in] The mark map to update
    ...
    MMINLINE void addOwnableSynchronizerObjectInList(MM_EnvironmentVLHGC* env, J9Object* objectPtr)
    {
         ...
         * it could be the constructing object which would be added in the list after the construction finish later.
         * ignore the object to avoid duplicated reference in the list.
         */
        if (NULL != _extensions->accessBarrier->isObjectInOwnableSynchronizerList(objectPtr)) {
            ((MM_OwnableSynchronizerObjectBufferVLHGC*)env->getGCEnvironment()->_ownableSynchronizerObjectBuffer)
                ->addForOnlyCompactedRegion(env, objectPtr);
        }
    }
    ...
    /**
     * At the beginning of a compaction, record any class loaders in the compact set which are marked in the next mark
     * map. The next mark map must still be valid.
     * @param env[in] A GC thread
     */
    ...
    /**
     * At the beginning of a compaction, clear the remembered sets of all class loaders for all regions in the
     * compaction set. These will be rebuilt as we relocate objects.
     * @param env[in] A GC thread
     */
...
public:
    ...
    /**
     * Fixes the marked objects in the [lowAddress..highAddress) range.  If rememberedObjectsOnly is set, we will
     ...
     * @param rememberedObjectsOnly If true, the receiver only fixes remembered live objects in the range.  If false, it
     * fixes all live objects in the range
     */
     ...
     * @param workStackBase[in/out] The work stack from which the region must be popped.  This is a reference parameter
     * which will be updated before the method returns
    ...
    /**
     * A helper method to push a region onto one of the given work stacks, based on whether or not it has its
     * _isCompactDestination flag set (true implies high priority).  Updates the stack base of the stack receiving the
     * region before returning.
     * @param workStackBase[in/out] The "low priority" work stack base.  This reference parameter will be updated before
     * the function returns is region is low priority
     * @param workStackBaseHighPriority[in/out] The "high priority" work stack base.  This reference parameter will be
     * updated before the function returns is region is high priority
     */
    void pushRegionOntoWorkStack(MM_HeapRegionDescriptorVLHGC** workStackBase,
