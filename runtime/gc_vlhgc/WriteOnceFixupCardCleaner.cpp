...
MM_WriteOnceFixupCardCleaner::MM_WriteOnceFixupCardCleaner(
    MM_WriteOnceCompactor* compactScheme, MM_CycleState* cycleState, MM_HeapRegionManager* regionManager)
    : MM_CardCleaner()
...
void MM_WriteOnceFixupCardCleaner::clean(
    MM_EnvironmentBase* envModron, void* lowAddress, void* highAddress, Card* cardToClean)
{
    ...
    case CARD_MARK_COMPACT_TRANSITION:
        /* presently, this state needs to be treated much the same was as PGC_MUST_SCAN but we know that any objects
         * under the card which point to other regions have been rememebered */
        rememberedOnly = true;
    case CARD_PGC_MUST_SCAN:
        if (_isGlobalMarkPhaseRunning) {
             ...
             * -a GMP runs, marking the referenced object but not updating the RSCL and then sets the card to
             * PGC_MUST_SCAN -a PGC runs, marks the referenced object but realizes that it points into the collection
             * set so it doesn't clear the state and doesn't update the RSCL -compact then moves the object, adds the
             * reference to the RSCL, and fixup sets its state to CLEAN -the GMP then runs, ignores the now-clean card
             * and finishes marking -observe that the new location of the referenced object was never marked in the next
             * mark map so the map is invalid
             */
            toState = CARD_GMP_MUST_SCAN;
        } else {
            toState = CARD_CLEAN;
    ...
    if (CARD_INVALID != toState) {
        *cardToClean = toState;
        ...
        Assert_MM_false(((MM_HeapRegionDescriptorVLHGC*)_regionManager->tableDescriptorForAddress(lowAddress))
                            ->_compactData._shouldCompact);
