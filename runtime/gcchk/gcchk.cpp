...
static IDATA OnLoad(J9JavaVM* javaVM, const char* options)
{
    ...
    J9HookInterface** mmPrivateHooks
        = J9_HOOK_INTERFACE((MM_GCExtensions::getExtensions(javaVM))->privateHookInterface);
    ...
    extensions = (GCCHK_Extensions*)forge->allocate(
        sizeof(GCCHK_Extensions), MM_AllocationCategory::DIAGNOSTIC, J9_GET_CALLSITE());
    if (!extensions) {
    ...
    extensions->checkCycle
        = (void*)GC_CheckCycle::newInstance(javaVM, (GC_CheckEngine*)extensions->checkEngine, options);
    ...
    if (!(((GC_CheckCycle*)extensions->checkCycle)->getMiscFlags() & J9MODRON_GCCHK_MANUAL)) {
        ...
        (*mmOmrHooks)
            ->J9HookRegisterWithCallSite(
                mmOmrHooks, J9HOOK_MM_OMR_GC_CYCLE_START, hookGcCycleStart, OMR_GET_CALLSITE(), NULL);
        (*mmOmrHooks)
            ->J9HookRegisterWithCallSite(
                mmOmrHooks, J9HOOK_MM_OMR_GC_CYCLE_END, hookGcCycleEnd, OMR_GET_CALLSITE(), NULL);

#if defined(J9VM_GC_MODRON_SCAVENGER)
        /* Install scavengerBackOut hooks */
        (*mmPrivateHooks)
            ->J9HookRegisterWithCallSite(
                mmPrivateHooks, J9HOOK_MM_PRIVATE_SCAVENGER_BACK_OUT, hookScavengerBackOut, OMR_GET_CALLSITE(), NULL);
#endif /* J9VM_GC_MODRON_SCAVENGER */

#if defined(J9VM_GC_GENERATIONAL)
        (*mmPrivateHooks)
            ->J9HookRegisterWithCallSite(mmPrivateHooks, J9HOOK_MM_PRIVATE_REMEMBEREDSET_OVERFLOW,
                hookRememberedSetOverflow, OMR_GET_CALLSITE(), NULL);
#endif /* J9VM_GC_GENERATIONAL */
    }
    ...
    (*mmPrivateHooks)
        ->J9HookRegisterWithCallSite(
            mmPrivateHooks, J9HOOK_MM_PRIVATE_INVOKE_GC_CHECK, hookInvokeGCCheck, OMR_GET_CALLSITE(), NULL);
 ...
 */
bool excludeLocalGc(J9JavaVM* javaVM)
{
    ...
    /* Don't check after Concurrent Scavenge aborted (even if J9MODRON_GCCHK_SCAVENGER_BACKOUT requested). Nursery is
     * not walkable between aborted CS and following percolated global GC. The check will be done after percolate GC is
     * complete (which will do most of heap fixup). Since gcCheck->_scavengerBackout is only set if
     * J9MODRON_GCCHK_SCAVENGER_BACKOUT is requested, relying on gcExtensions->isScavengerBackOutFlagRaised() instead.
     */
 ...
 * @return true if we should NOT perform a check
 */
bool excludeGlobalGc(J9VMThread* vmThread)
{
    ...
    /* If Concurrent Scavenger aborted, do not check before global GC (regardless if J9MODRON_GCCHK_SCAVENGER_BACKOUT is
     * requested or not). Since checkEngine->_scavengerBackout is set only if J9MODRON_GCCHK_SCAVENGER_BACKOUT
     * requested, gcExtensions->isScavengerBackOutFlagRaised() is checked instead.  */
    if (gcExtensions->isConcurrentScavengerEnabled() && gcExtensions->isScavengerBackOutFlagRaised()
        && (OMRVMSTATE_GC_CHECK_BEFORE_GC == vmThread->omrVMThread->vmState)) {
        return true;
    }
    ...
    if (cycle->getMiscFlags() & J9MODRON_GCCHK_SCAVENGER_BACKOUT) {
        /* if J9MODRON_GCCHK_SCAVENGER_BACKOUT is requested, the check will be done at the end of percolate global GC
         * (not at the end of the aborted scavenge). Recovery from the aborted CS is mostly done in the global GC and
         * Nursery is not walkable between aborted CS and percolate global. Since GC check is done in global GC cycle
         * end hook, gcExtensions->isScavengerBackOutFlagRaised just has been reset, so we have to rely on
         * gcCheck->_scavengerBackout, which is yet to be cleared in the initialization of this check cycle  */
    ...
    if (OMR_GC_CYCLE_TYPE_GLOBAL == event->cycleType) {
        ...
        if (!excludeGlobalGc(vmThread)) {
            if (cycle->getMiscFlags() & J9MODRON_GCCHK_VERBOSE) {
                j9tty_printf(
                    PORTLIB, "<gc check: start verifying slots before global gc (%zu)>\n", extensions->globalGcCount);
            }
        ...
        if (!excludeLocalGc(javaVM)) {
            if (cycle->getMiscFlags() & J9MODRON_GCCHK_VERBOSE) {
                j9tty_printf(
                    PORTLIB, "<gc check: start verifying slots before local gc (%zu)>\n", extensions->localGcCount);
            }
            ...
            if (cycle->getMiscFlags() & J9MODRON_GCCHK_VERBOSE) {
                j9tty_printf(
                    PORTLIB, "<gc check: finished verifying slots before local gc (%zu)>\n", extensions->localGcCount);
            }
        }
    }
        ...
        if (!excludeGlobalGc(vmThread)) {
            if (cycle->getMiscFlags() & J9MODRON_GCCHK_VERBOSE) {
                j9tty_printf(
                    PORTLIB, "<gc check: start verifying slots before default gc (%zu)>\n", extensions->globalGcCount);
            }
...
void hookGcCycleEnd(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (OMR_GC_CYCLE_TYPE_GLOBAL == event->cycleType) {
        if (!excludeGlobalGc(vmThread)) {
            if (cycle->getMiscFlags() & J9MODRON_GCCHK_VERBOSE) {
                j9tty_printf(
                    PORTLIB, "<gc check: start verifying slots after global gc (%zu)>\n", extensions->globalGcCount);
            }
            ...
            if (cycle->getMiscFlags() & J9MODRON_GCCHK_VERBOSE) {
                j9tty_printf(
                    PORTLIB, "<gc check: finished verifying slots after global gc (%zu)>\n", extensions->globalGcCount);
            }
        }
    }
#if defined(J9VM_GC_MODRON_SCAVENGER)
        ...
        if (!excludeLocalGc(javaVM)) {
            if (cycle->getMiscFlags() & J9MODRON_GCCHK_VERBOSE) {
                j9tty_printf(
                    PORTLIB, "<gc check: start verifying slots after local gc (%zu)>\n", extensions->localGcCount);
            }
            ...
            if (cycle->getMiscFlags() & J9MODRON_GCCHK_VERBOSE) {
                j9tty_printf(
                    PORTLIB, "<gc check: finished verifying slots after local gc (%zu)>\n", extensions->localGcCount);
            }
        }
    }
#endif /* defined(J9VM_GC_MODRON_SCAVENGER) */
        ...
        if (!excludeGlobalGc(vmThread)) {
            if (cycle->getMiscFlags() & J9MODRON_GCCHK_VERBOSE) {
                j9tty_printf(
                    PORTLIB, "<gc check: start verifying slots after default gc (%zu)>\n", extensions->globalGcCount);
            }
...
void hookRememberedSetOverflow(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    GCCHK_Extensions* extensions
        = (GCCHK_Extensions*)(MM_GCExtensions::getExtensions(event->currentThread))->gcchkExtensions;
    GC_CheckEngine* checkEngine = (GC_CheckEngine*)extensions->checkEngine;
    GC_CheckCycle* cycle = (GC_CheckCycle*)extensions->checkCycle;

    if (cycle->getMiscFlags() & J9MODRON_GCCHK_REMEMBEREDSET_OVERFLOW) {
        checkEngine->_rsOverflowState
            = MM_GCExtensions::getExtensions(event->currentThread)->isRememberedSetInOverflowState();
    }
}
#endif /* J9VM_GC_GENERATIONAL */
