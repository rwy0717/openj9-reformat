...
struct plugin_service_ops {
    ...
    struct cmd_list_element* (*add_command)(
        char* name, enum command_class cmd_class, void (*func)(char*, int), char* doc);
...
struct plugin_ops {
    ...
    int (*session_init)(int session_id, struct plugin_service_ops* plugin_service);

    /* pointer to the next loaded plugin */
    struct plugin_ops* next;
};

struct plugin_ops* init_plugin(char* path);

void finish_plugin(struct plugin_ops* plugin);
