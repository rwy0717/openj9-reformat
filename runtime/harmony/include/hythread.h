...
typedef struct HyThreadLibrary {
    ...
    IDATA (*thread_create)
    (struct HyThreadLibrary* threadLibrary, hythread_t* handle, UDATA stacksize, UDATA priority, UDATA suspend,
    ...
    IDATA (*thread_monitor_init_with_name)
    (struct HyThreadLibrary* threadLibrary, hythread_monitor_t* handle, UDATA flags, char* name);
    ...
    IDATA (*thread_tls_set)
    (struct HyThreadLibrary* threadLibrary, hythread_t thread, hythread_tls_key_t key, void* value);
    /** self_handle*/
    void* self_handle;
} HyThreadLibrary;
...
extern HY_CFUNC I_32 hythread_create_library(
    struct HyThreadLibrary* threadLibrary, struct HyThreadLibraryVersion* version, UDATA size);
extern HY_CFUNC I_32 hythread_init_library(
    struct HyThreadLibrary* threadLibrary, struct HyThreadLibraryVersion* version, UDATA size);
extern HY_CFUNC I_32 hythread_shutdown_library(struct HyThreadLibrary* threadLibrary);
extern HY_CFUNC I_32 hythread_startup_library(struct HyThreadLibrary* threadLibrary);
/** Thread library self allocation routines */
extern HY_CFUNC I_32 hythread_allocate_library(
    struct HyThreadLibraryVersion* expectedVersion, struct HyThreadLibrary** threadLibrary);
 ...
 * @{
 */
extern HY_CFUNC UDATA hythread_getSize(struct HyThreadLibraryVersion* version);
extern HY_CFUNC I_32 hythread_getVersion(struct HyThreadLibrary* threadLibrary, struct HyThreadLibraryVersion* version);
extern HY_CFUNC I_32 hythread_isCompatible(struct HyThreadLibraryVersion* expectedVersion);
...
#define hythread_global_monitor() \
    (*(hythread_monitor_t*)privateThreadLibrary->thread_global(privateThreadLibrary, "global_monitor"))
#define hythread_monitor_init(pMon, flags) \
    privateThreadLibrary->thread_monitor_init_with_name(privateThreadLibrary, pMon, flags, #pMon)
...
#define hythread_create(param1, param2, param3, param4, param5, param6) \
    privateThreadLibrary->thread_create(privateThreadLibrary, param1, param2, param3, param4, param5, param6)
...
#define hythread_monitor_init_with_name(param1, param2, param3) \
    privateThreadLibrary->thread_monitor_init_with_name(privateThreadLibrary, param1, param2, param3)
#define hythread_monitor_notify(param1) privateThreadLibrary->thread_monitor_notify(privateThreadLibrary, param1)
#define hythread_monitor_notify_all(param1) \
    privateThreadLibrary->thread_monitor_notify_all(privateThreadLibrary, param1)
...
#define hythread_tls_set(param1, param2, param3) \
    privateThreadLibrary->thread_tls_set(privateThreadLibrary, param1, param2, param3)
