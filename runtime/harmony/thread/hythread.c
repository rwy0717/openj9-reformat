...
IDATA
hythread_create(HyThreadLibrary* threadLib, hythread_t* handle, UDATA stacksize, UDATA priority, UDATA suspend,
    hythread_entrypoint_t entrypoint, void* entryarg);

void hythread_detach(HyThreadLibrary* threadLib, hythread_t thread);

void OMRNORETURN hythread_exit(HyThreadLibrary* threadLib, hythread_monitor_t monitor);

UDATA* hythread_global(HyThreadLibrary* threadLib, char* name);
...
hythread_t hythread_self(HyThreadLibrary* threadLib);
...
void* hythread_tls_get(HyThreadLibrary* threadLib, hythread_t thread, hythread_tls_key_t key);
 ...
 */
I_32 hythread_init_library(struct HyThreadLibrary* threadLibrary, struct HyThreadLibraryVersion* version, UDATA size)
{
 ...
 */
I_32 hythread_shutdown_library(struct HyThreadLibrary* threadLibrary)
{
 ...
 */
I_32 hythread_create_library(struct HyThreadLibrary* threadLibrary, struct HyThreadLibraryVersion* version, UDATA size)
{
 ...
 */
I_32 hythread_startup_library(struct HyThreadLibrary* threadLibrary) { return 0; }
 ...
 */
I_32 hythread_getVersion(struct HyThreadLibrary* threadLibrary, struct HyThreadLibraryVersion* version)
{
    if (NULL == version) {
        return -1;
    }
 ...
 */
I_32 hythread_isCompatible(struct HyThreadLibraryVersion* expectedVersion)
{
 ...
 */
I_32 hythread_isFunctionOverridden(struct HyThreadLibrary* threadLibrary, UDATA offset)
{
    UDATA requiredSize;
 ...
 */
I_32 hythread_allocate_library(struct HyThreadLibraryVersion* version, struct HyThreadLibrary** threadLibrary)
{
...
IDATA
hysem_destroy(HyThreadLibrary* threadLib, hysem_t s) { return j9sem_destroy(s); }

IDATA
hysem_init(HyThreadLibrary* threadLib, hysem_t* sp, I_32 initValue) { return j9sem_init(sp, initValue); }

IDATA
hysem_post(HyThreadLibrary* threadLib, hysem_t s) { return j9sem_post(s); }

IDATA
hysem_wait(HyThreadLibrary* threadLib, hysem_t s) { return j9sem_wait(s); }
...
void hythread_detach(HyThreadLibrary* threadLib, hythread_t thread) { omrthread_detach(thread); }
...
UDATA* hythread_global(HyThreadLibrary* threadLib, char* name) { return omrthread_global(name); }
...
hythread_t hythread_self(HyThreadLibrary* threadLib) { return omrthread_self(); }

IDATA
hythread_sleep(HyThreadLibrary* threadLib, I_64 millis) { return omrthread_sleep(millis); }

IDATA
hythread_tls_alloc(HyThreadLibrary* threadLib, hythread_tls_key_t* handle) { return omrthread_tls_alloc(handle); }

IDATA
hythread_tls_free(HyThreadLibrary* threadLib, hythread_tls_key_t key) { return omrthread_tls_free(key); }
