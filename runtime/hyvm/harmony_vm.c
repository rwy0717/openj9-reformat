...
typedef I_32 (*hyport_allocate_library_maptoj9)(
...
static struct HyPortLibrary* createHarmonyPortLibrary(J9PortLibrary* j9PortLib)
{
    HyPortLibraryVersion portLibraryVersion;
    I_32 rc;
    UDATA hyprtshimLibHandle = 0;

    if (harmonyPortLibrary != NULL)
    ...
    if (GlobalHyPortAllocateLibraryMapToJ9 == NULL) {
        ...
        if (j9sl_lookup_name(hyprtshimLibHandle, "hyport_allocate_library_maptoj9",
                (void*)&GlobalHyPortAllocateLibraryMapToJ9, "ILLL")) {
            return NULL;
        }
    }
