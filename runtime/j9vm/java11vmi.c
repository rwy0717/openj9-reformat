...
static UDATA addMulPackageDefinitions(
    J9VMThread* currentThread, J9Module* fromModule, const char* const* packages, U_32 numPackages);
static void removeMulPackageDefinitions(
    J9VMThread* currentThread, J9Module* fromModule, const char* const* packages, U_32 packagesIndex);
static UDATA addModuleDefinition(
    J9VMThread* currentThread, J9Module* fromModule, const char* const* packages, U_32 numPackages, jstring version);
static BOOLEAN isPackageDefined(J9VMThread* currentThread, J9ClassLoader* classLoader, const char* packageName);
static BOOLEAN areNoPackagesDefined(
    J9VMThread* currentThread, J9ClassLoader* classLoader, const char* const* packages, U_32 numPackages);
...
static UDATA exportPackageToModule(
    J9VMThread* currentThread, J9Module* fromModule, const char* package, J9Module* toModule);
...
static void trcModulesAddModuleExportsToAllUnnamed(
    J9VMThread* currentThread, J9Module* fromModule, const char* package);
static void trcModulesAddModuleExports(
    J9VMThread* currentThread, J9Module* fromModule, const char* package, J9Module* toModule);
...
static void removeMulPackageDefinitions(
    J9VMThread* currentThread, J9Module* fromModule, jobjectArray packages, U_32 packagesIndex);
static UDATA addModuleDefinition(
    J9VMThread* currentThread, J9Module* fromModule, jobjectArray packages, jstring version);
...
static UDATA exportPackageToModule(
    J9VMThread* currentThread, J9Module* fromModule, jstring package, J9Module* toModule);
...
static J9Module* createModule(
    J9VMThread* currentThread, j9object_t moduleObject, J9ClassLoader* classLoader, j9object_t moduleName);
...
static void trcModulesAddReadsModule(
    J9VMThread* currentThread, jobject toModule, J9Module* j9FromMod, J9Module* j9ToMod);
...
static void throwExceptionHelper(J9VMThread* currentThread, UDATA errCode)
{
    ...
    if (ERRCODE_SUCCESS != errCode) {
        vmFuncs->setCurrentExceptionUTF(
            currentThread, J9VMCONSTANTPOOL_JAVALANGILLEGALARGUMENTEXCEPTION, (const char*)errMessages[errCode]);
    }
}
    ...
    J9VMThread* currentThread, j9object_t moduleObject, J9ClassLoader* classLoader, j9object_t moduleName)
{
    ...
    if (J9_ARE_ALL_BITS_SET(vm->runtimeFlags, J9_RUNTIME_JAVA_BASE_MODULE_CREATED)) {
        ...
        if (NULL == moduleName) {
            /* moduleName is passed as NULL for the unnamed module for bootloader created by
...
#endif /* J9VM_JAVA9_BUILD >= 156 */
{
    ...
    char* packageNameUTF = vmFuncs->copyStringToUTF8WithMemAlloc(currentThread, package, J9_STR_NULL_TERMINATE_RESULT,
        "", 0, packageNameBuf, J9VM_PACKAGE_NAME_BUFFER_LENGTH, NULL);
#endif /* J9VM_JAVA9_BUILD < 156 */
...
#endif /* J9VM_JAVA9_BUILD >= 156 */
{
    ...
    char* fromModuleNameUTF = vmFuncs->copyStringToUTF8WithMemAlloc(currentThread, fromModule->moduleName,
        J9_STR_NULL_TERMINATE_RESULT, "", 0, fromModuleNameBuf, J9VM_PACKAGE_NAME_BUFFER_LENGTH, NULL);
    if (NULL != fromModuleNameUTF) {
#if J9VM_JAVA9_BUILD >= 156
        Trc_MODULE_add_module_exports_to_all(currentThread, package, fromModuleNameUTF);
#else
        char packageNameBuf[J9VM_PACKAGE_NAME_BUFFER_LENGTH];
        char* packageNameUTF = vmFuncs->copyStringToUTF8WithMemAlloc(currentThread, J9_JNI_UNWRAP_REFERENCE(package),
            J9_STR_NULL_TERMINATE_RESULT, "", 0, packageNameBuf, J9VM_PACKAGE_NAME_BUFFER_LENGTH, NULL);
        if (NULL != packageNameUTF) {
...
#else /* J9VM_JAVA9_BUILD >= 156 */
    J9Package* const j9package
        = getPackageDefinition(currentThread, fromModule, J9_JNI_UNWRAP_REFERENCE(package), &retval);
#endif /* J9VM_JAVA9_BUILD >= 156 */
    if (NULL != j9package) {
...
#endif /* J9VM_JAVA9_BUILD >= 156 */
{
    ...
    char* fromModuleNameUTF = vmFuncs->copyStringToUTF8WithMemAlloc(currentThread, fromModule->moduleName,
        J9_STR_NULL_TERMINATE_RESULT, "", 0, fromModuleNameBuf, J9VM_PACKAGE_NAME_BUFFER_LENGTH, NULL);
    char* toModuleNameUTF = vmFuncs->copyStringToUTF8WithMemAlloc(currentThread, toModule->moduleName,
        J9_STR_NULL_TERMINATE_RESULT, "", 0, toModuleNameBuf, J9VM_PACKAGE_NAME_BUFFER_LENGTH, NULL);
    ...
    if (NULL != j9package) {
        if (isModuleDefined(currentThread, toModule)) {
            if (0 == hashTableAtPut(j9package->exportsHashTable, (void*)&toModule, FALSE)) {
                 ...
                 * before fromModule. An unloaded module in packageHashtable will result in a crash when doing hashtable
                 * lookups. We use this hashtable to remove instances of toModule in packages when unloading toModule.
                ...
                if (NULL == toModule->removeExportsHashTable) {
                    ...
                    toModule->removeExportsHashTable
                        = vm->internalVMFunctions->hashPackageTableNew(vm, INITIAL_INTERNAL_PACKAGE_HASHTABLE_SIZE);
                }
                if (NULL != toModule->removeExportsHashTable) {
    ...
    if (isModuleDefined(currentThread, fromModule)) {
        ...
        if (J9_IS_J9MODULE_UNNAMED(vm, toModule)) {
            ...
            if (0 == hashTableAtPut(toModule->readAccessHashTable, (void*)&fromModule, FALSE)) {
                 ...
                 * We use removeAccessHashTable to remove instances of fromModule in toModules when unloading
                 * fromModule. We only need to worry about modules in different layers as modules in the same layer are
                 * unloaded at the same time.
                 */
                if (NULL == fromModule->removeAccessHashTable) {
                    fromModule->removeAccessHashTable
                        = vm->internalVMFunctions->hashModuleTableNew(vm, INITIAL_INTERNAL_MODULE_HASHTABLE_SIZE);
                }
                if (NULL != fromModule->removeAccessHashTable) {
...
/**
 * Define a module containing the specified packages. It will create the module record in the  ClassLoader's module hash
 * table and create package records in the class loader's package hash table if necessary.
 *
...
jobject JNICALL
#if J9VM_JAVA9_BUILD >= 156
JVM_DefineModule(JNIEnv* env, jobject module, jboolean isOpen, jstring version, jstring location,
    const char* const* packages, jsize numPackages)
...
#endif /* J9VM_JAVA9_BUILD >= 148 */
{
    ...
    if (NULL == module) {
        vmFuncs->setCurrentExceptionNLS(
            currentThread, J9VMCONSTANTPOOL_JAVALANGNULLPOINTEREXCEPTION, J9NLS_VM_MODULE_IS_NULL);
    } else {
        j9object_t modObj = J9_JNI_UNWRAP_REFERENCE(module);
...
#define JAVASLASH "java/"
                if ((0 == strcmp(packageName, "java")) || (0 == strncmp(packageName, JAVADOT, sizeof(JAVADOT) - 1))
...
#undef JAVADOT
        ...
        if (NULL == moduleName) {
            vmFuncs->setCurrentExceptionNLS(
                currentThread, J9VMCONSTANTPOOL_JAVALANGILLEGALARGUMENTEXCEPTION, J9NLS_VM_MODULE_IS_UNNAMED);
        } else if (!isModuleNameValid(moduleName)) {
            vmFuncs->setCurrentExceptionNLS(
                currentThread, J9VMCONSTANTPOOL_JAVALANGILLEGALARGUMENTEXCEPTION, J9NLS_VM_MODULE_NAME_IS_INVALID);
            ...
            PORT_ACCESS_FROM_VMC(currentThread);
            nameUTF = vmFuncs->copyStringToUTF8WithMemAlloc(currentThread, moduleName, J9_STR_NULL_TERMINATE_RESULT, "",
                0, buf, J9VM_PACKAGE_NAME_BUFFER_LENGTH, NULL);
            if (NULL == nameUTF) {
                vmFuncs->setNativeOutOfMemoryError(currentThread, 0, 0);
                ...
                if (NULL != j9mod) {
                    ...
                    if (success) {
                        /* For "java.base" module setting of jrt URL and patch paths is already done during startup.
                         * Avoid doing it here. */
                        if (J9_ARE_ALL_BITS_SET(vm->runtimeFlags, J9_RUNTIME_JAVA_BASE_MODULE_CREATED)) {
                                    ...
                                    if (0 == strcmp(nameUTF, moduleName)) {
                                        ...
                                        if ((NULL == entry)
                                            ...
                                            j9nls_printf(PORTLIB, J9NLS_WARNING,
                                                J9NLS_VM_FAILED_TO_LOAD_MODULE_REQUIRED_DLL, J9_SHARED_DLL_NAME,
                                                moduleName);
                                        }
                                    }
                                }
    ...
    J9VMThread* currentThread, jobject toModule, J9Module* j9FromMod, J9Module* j9ToMod)
{
    ...
    char fromModuleNameBuf[J9VM_PACKAGE_NAME_BUFFER_LENGTH];
    char toModuleNameBuf[J9VM_PACKAGE_NAME_BUFFER_LENGTH];
    char* fromModuleNameUTF = vmFuncs->copyStringToUTF8WithMemAlloc(currentThread, j9FromMod->moduleName,
        J9_STR_NULL_TERMINATE_RESULT, "", 0, fromModuleNameBuf, J9VM_PACKAGE_NAME_BUFFER_LENGTH, NULL);
    char* toModuleNameUTF = NULL;

    if (NULL != toModule) {
        toModuleNameUTF = vmFuncs->copyStringToUTF8WithMemAlloc(currentThread, j9ToMod->moduleName,
            J9_STR_NULL_TERMINATE_RESULT, "", 0, toModuleNameBuf, J9VM_PACKAGE_NAME_BUFFER_LENGTH, NULL);
    } else {
 ...
 * If toModule is null then fromModule is marked as a loose module (i.e., fromModule can read all current and future
 * unnamed modules).
 *
 * @throws IllegalArgumentExceptions if
 * 1) if fromModule is null or if modules do not exist.
 */
void JNICALL JVM_AddReadsModule(JNIEnv* env, jobject fromModule, jobject toModule)
{
 ...
 */
jboolean JNICALL JVM_CanReadModule(JNIEnv* env, jobject askModule, jobject srcModule)
{
    J9VMThread* const currentThread = (J9VMThread*)env;
    J9JavaVM const* const vm = currentThread->javaVM;
    J9InternalVMFunctions const* const vmFuncs = vm->internalVMFunctions;
...
jstring JNICALL JVM_GetSimpleBinaryName(JNIEnv* env, jclass arg1)
{
    assert(!"JVM_GetSimpleBinaryName unimplemented"); /* Jazz 108925: Revive J9JCL raw pConfig build */
    return NULL;
}

void JNICALL JVM_SetMethodInfo(JNIEnv* env, jobject arg1)
{
    assert(!"JVM_SetMethodInfo unimplemented"); /* Jazz 108925: Revive J9JCL raw pConfig build */
}

jint JNICALL JVM_ConstantPoolGetNameAndTypeRefIndexAt(JNIEnv* env, jobject arg1, jobject arg2, jint arg3)
{
    assert(!"JVM_ConstantPoolGetNameAndTypeRefIndexAt unimplemented"); /* Jazz 108925: Revive J9JCL raw pConfig build */
    return -1;
}

jint JNICALL JVM_MoreStackWalk(
    JNIEnv* env, jobject arg1, jlong arg2, jlong arg3, jint arg4, jint arg5, jobjectArray arg6, jobjectArray arg7)
{
    assert(!"JVM_MoreStackWalk unimplemented"); /* Jazz 108925: Revive J9JCL raw pConfig build */
    return -1;
}

jint JNICALL JVM_ConstantPoolGetClassRefIndexAt(JNIEnv* env, jobject arg1, jlong arg2, jint arg3)
{
    assert(!"JVM_ConstantPoolGetClassRefIndexAt unimplemented"); /* Jazz 108925: Revive J9JCL raw pConfig build */
    return -1;
}

jobjectArray JNICALL JVM_GetVmArguments(JNIEnv* env)
{
    ...
    if (NULL != vmClass) {
        J9Method* method
            = internalFunctions->findJNIMethod(currentThread, vmClass, "getVMArgs", "()[Ljava/lang/String;");
...
void JNICALL JVM_FillStackFrames(JNIEnv* env, jclass arg1, jint arg2, jobjectArray arg3, jint arg4, jint arg5)
{
    assert(!"JVM_FillStackFrames unimplemented"); /* Jazz 108925: Revive J9JCL raw pConfig build */
}

jclass JNICALL JVM_FindClassFromCaller(JNIEnv* env, const char* arg1, jboolean arg2, jobject arg3, jclass arg4)
{
    assert(!"JVM_FindClassFromCaller unimplemented"); /* Jazz 108925: Revive J9JCL raw pConfig build */
    return NULL;
}

jobjectArray JNICALL JVM_ConstantPoolGetNameAndTypeRefInfoAt(JNIEnv* env, jobject arg1, jobject arg2, jint arg3)
{
    assert(!"JVM_ConstantPoolGetNameAndTypeRefInfoAt unimplemented"); /* Jazz 108925: Revive J9JCL raw pConfig build */
    return NULL;
}

jbyte JNICALL JVM_ConstantPoolGetTagAt(JNIEnv* env, jobject arg1, jobject arg2, jint arg3)
{
    assert(!"JVM_ConstantPoolGetTagAt unimplemented"); /* Jazz 108925: Revive J9JCL raw pConfig build */
    return 0;
}

jobject JNICALL JVM_CallStackWalk(
    JNIEnv* env, jobject arg1, jlong arg2, jint arg3, jint arg4, jint arg5, jobjectArray arg6, jobjectArray arg7)
{
 ...
 */
void JNICALL JVM_SetBootLoaderUnnamedModule(JNIEnv* env, jobject module)
{
    J9VMThread* const currentThread = (J9VMThread*)env;
    J9JavaVM* vm = currentThread->javaVM;
    J9InternalVMFunctions const* const vmFuncs = vm->internalVMFunctions;
    ...
    if (module == NULL) {
        ...
        if (J2SE_SHAPE(vm) < J2SE_SHAPE_B165) {
            moduleClass = vmFuncs->internalFindKnownClass(
                currentThread, J9VMCONSTANTPOOL_JAVALANGREFLECTMODULE, J9_FINDKNOWNCLASS_FLAG_INITIALIZE);
        } else {
            moduleClass = vmFuncs->internalFindKnownClass(
        ...
        if (!currentThread->currentException) {
            if (!isModuleUnnamed(currentThread, modObj)) {
                vmFuncs->setCurrentExceptionUTF(
                    currentThread, J9VMCONSTANTPOOL_JAVALANGILLEGALARGUMENTEXCEPTION, "named module was supplied");
            } else if (!isSameOrSuperClassOf(moduleClass, instanceClazz)) {
                vmFuncs->setCurrentExceptionUTF(currentThread, J9VMCONSTANTPOOL_JAVALANGILLEGALARGUMENTEXCEPTION,
                ...
                if (NULL
                    == J9VMJAVALANGCLASSLOADER_UNNAMEDMODULE(currentThread, systemClassLoader->classLoaderObject)) {
                    J9Module* j9mod
                    ...
                    vmFuncs->setCurrentExceptionUTF(currentThread, J9VMCONSTANTPOOL_JAVALANGINTERNALERROR,
...
void JNICALL JVM_ToStackTraceElement(JNIEnv* env, jobject arg1, jobject arg2)
{
    assert(!"JVM_ToStackTraceElement unimplemented");
}

void JNICALL JVM_GetStackTraceElements(JNIEnv* env, jobject throwable, jobjectArray elements)
{
    assert(!"JVM_GetStackTraceElements unimplemented");
}

void JNICALL JVM_InitStackTraceElementArray(JNIEnv* env, jobjectArray elements, jobject throwable)
{
    assert(!"JVM_InitStackTraceElementArray unimplemented");
}

void JNICALL JVM_InitStackTraceElement(JNIEnv* env, jobject element, jobject stackFrameInfo)
{
 ...
 */
jobject JNICALL JVM_GetModuleByPackageName(JNIEnv* env, jobject classLoader, jstring packageName)
{
    J9VMThread* const currentThread = (J9VMThread*)env;
    J9JavaVM* vm = currentThread->javaVM;
    J9InternalVMFunctions const* const vmFuncs = vm->internalVMFunctions;
    ...
    if (NULL == packageName) {
        vmFuncs->setCurrentExceptionUTF(
            currentThread, J9VMCONSTANTPOOL_JAVALANGNULLPOINTEREXCEPTION, "package name is null");
    } else {
        ...
        if (NULL == classLoader) {
            ...
            J9Class* classLoaderClass = vmFuncs->internalFindKnownClass(
                currentThread, J9VMCONSTANTPOOL_JAVALANGCLASSLOADER, J9_FINDKNOWNCLASS_FLAG_INITIALIZE);
            ...
            if (!isSameOrSuperClassOf(classLoaderClass, thisClassLoaderClass)) {
                vmFuncs->setCurrentExceptionUTF(currentThread, J9VMCONSTANTPOOL_JAVALANGILLEGALARGUMENTEXCEPTION,
                    "classLoader is not same or subclass of java/lang/ClassLoader");
                goto exit;
            }
        ...
        packageUTF8 = vmFuncs->copyStringToJ9UTF8WithMemAlloc(
            currentThread, packageObj, J9_STR_NULL_TERMINATE_RESULT, "", 0, buf, J9VM_PACKAGE_NAME_BUFFER_LENGTH);
        ...
        if (NULL != strchr((const char*)J9UTF8_DATA(packageUTF8), '.')) {
            vmFuncs->setCurrentExceptionUTF(currentThread, J9VMCONSTANTPOOL_JAVALANGILLEGALARGUMENTEXCEPTION,
                "package name contains '.' instead of '/'");
        } else {
            if (vmFuncs->isAnyClassLoadedFromPackage(
                    thisVMClassLoader, J9UTF8_DATA(packageUTF8), J9UTF8_LENGTH(packageUTF8))) {
                vmModule = vmFuncs->findModuleForPackageUTF8(currentThread, thisVMClassLoader, packageUTF8);
                if (NULL == vmModule) {
                    moduleObj
                        = J9VMJAVALANGCLASSLOADER_UNNAMEDMODULE(currentThread, thisVMClassLoader->classLoaderObject);
                } else {
 ...
 */
jlong JNICALL JVM_GetNanoTimeAdjustment(JNIEnv* env, jclass clazz, jlong offsetSeconds)
{
...
void JNICALL JVM_BeforeHalt() { /* To be implemented via https://github.com/eclipse/openj9/issues/1459 */ }
JNIEXPORT jclass JNICALL JVM_GetNestHost(JNIEnv* env, jclass clz)
{
    assert(!"JVM_GetNestHost unimplemented");
    return NULL;
}
JNIEXPORT jobjectArray JNICALL JVM_GetNestMembers(JNIEnv* env, jclass clz)
{
 ...
 */
JNIEXPORT jboolean JNICALL JVM_AreNestMates(JNIEnv* env, jclass jClassOne, jclass jClassTwo)
{
