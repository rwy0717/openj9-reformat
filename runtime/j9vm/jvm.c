...
static int findDirContainingFile(
    J9StringBuffer** result, char* paths, char pathSeparator, char* fileToFind, int elementsToSkip);
static int findDirUplevelToDirContainingFile(
    J9StringBuffer** result, char* pathEnvar, char pathSeparator, char* fileInPath, int upLevels, int elementsToSkip);
...
#if defined(WIN32)
#define J9_SIGNAL_MAP_ENTRY(name, value) \
    {                                    \
        name, value                      \
    }
#else /* defined(WIN32) */
...
static char* jvmBufferData(J9StringBuffer* buffer) { return buffer ? buffer->data : NULL; }
...
static BOOLEAN preloadLibraries(void)
{
    ...
    wchar_t unicodeDLLName[J9_MAX_PATH + 1]
        = { 0 }; /*extra character is added to check if path is truncated by GetModuleFileNameW()*/
    ...
    WideCharToMultiByte(
        OS_ENCODING_CODE_PAGE, OS_ENCODING_WC_FLAGS, unicodeDLLName, -1, jvmDLLName, J9_MAX_PATH, NULL, NULL);
    ...
    MultiByteToWideChar(
        OS_ENCODING_CODE_PAGE, OS_ENCODING_MB_FLAGS, jvmBufferData(jvmDLLNameBuffer), -1, unicodeDLLName, J9_MAX_PATH);
    ...
    if (!f_threadGlobal || !f_threadAttachEx || !f_threadDetach || !f_monitorEnter || !f_monitorExit || !f_monitorInit
        || !f_monitorDestroy || !f_monitorWaitTimed || !f_monitorNotify || !f_monitorNotifyAll || !f_threadLibControl
...
static void bfuThreadStart(J9HookInterface** vmHooks, UDATA eventNum, void* eventData, void* userData)
{
...
static void bfuThreadEnd(J9HookInterface** vmHooks, UDATA eventNum, void* eventData, void* userData)
{
...
static jint initializeWin32ThreadEvents(J9JavaVM* javaVM)
{
    J9VMThread* aThread;
    ...
    if ((*hookInterface)
            ->J9HookRegisterWithCallSite(
                hookInterface, J9HOOK_VM_THREAD_CREATED, bfuThreadStart, OMR_GET_CALLSITE(), NULL)) {
        return JNI_ERR;
    }
    if ((*hookInterface)
            ->J9HookRegisterWithCallSite(
                hookInterface, J9HOOK_VM_THREAD_DESTROY, bfuThreadEnd, OMR_GET_CALLSITE(), NULL)) {
        return JNI_ERR;
    }
...
#define VMDLL_NAME J9_VM_DLL_NAME
    ...
    while (
        foundPosition = findDirUplevelToDirContainingFile(&result, LD_ENV_PATH, ':', "libjvm.so", 0, foundPosition)) {
        /* now screen to see if match is the right libjvm.so - it needs to have a j9vm DLL either in this dir, or in the
         * parent. */
 ...
 */
static void removeSuffix(char* string, const char* suffix)
{
...
static BOOLEAN preloadLibraries(void)
{
    ...
    if (!globalGetStringPlatform || !globalGetStringPlatformLength || !globalNewStringPlatform
        || !global_a2e_vsprintf) {
        dlclose(vmDLL);
        dlclose(javaDLL);
...
int jio_vfprintf(FILE* stream, const char* format, va_list args)
{
...
int jio_vsnprintf(char* str, int n, const char* format, va_list args)
{
    int result;
 ...
 * Table to map textual props file entries to numeric constants.
 */
VersionSetting VERSION_SETTINGS[] = { { "1.8", J2SE_18 }, { "1.9", J2SE_19 } };
#define NUM_VERSION_SETTINGS (sizeof(VERSION_SETTINGS) / sizeof(VersionSetting))

static UDATA decodeSetting(const char* key, const char* value, VersionSetting* settings, IDATA numSettings)
{
    IDATA index = 0;
 ...
 *         or 0 if otherwise.
 */
static UDATA getVersionFromClasslibPropertiesFile(void)
{
 ...
 *         or 0 if otherwise.
 */
static UDATA getVersionFromReleaseFile(void)
{
    PORT_ACCESS_FROM_PORT(&j9portLibrary);
    J9StringBuffer* propsPathBuffer = NULL;
    j9props_file_t propsFile = NULL;
 ...
 */
static UDATA getVersionFromPropertiesFile(void)
{
    if (-1 == jvmSEVersion) {
        UDATA finalVersion = 0;
 ...
 */
static UDATA initialArgumentScan(JavaVMInitArgs* args, J9SpecialArguments* specialArgs)
{
    ...
    if (TRUE == xCheckFound) {
        /* scan backwards for -Xcheck:memory.  There may be multiple -Xcheck options, so check them all, stop when we
         * hit -Xcheck:memeory */
...
static void printVmArgumentsList(J9VMInitArgs* argList)
{
    UDATA i;
    ...
    for (i = 0; i < argList->nOptions; ++i) {
        ...
        fprintf(stderr, "Option %" J9PRIz "d optionString=\"%s\" extraInfo=%p from environment variable =\"%s\"\n", i,
            actualArgs->options[i].optionString, actualArgs->options[i].extraInfo, envVar);
    }
}

#ifdef J9VM_OPT_HARMONY
static IDATA addHarmonyPortLibrary(J9PortLibrary* portLib, J9JavaVMArgInfoList* vmArgumentsList, UDATA verboseFlags)
{
    JavaVMInitArgs dummyArgs
        = { 0, 0, NULL, FALSE }; /* In this case, addHarmonyPortLibToVMArgs looks only at nOptions */
...
static void setNLSCatalog(struct J9PortLibrary* portLib, UDATA j2seVersion)
{
...
static jint initializeReflectionGlobals(JNIEnv* env, BOOLEAN includeAccessors)
{
    ...
    if (J2SE_SHAPE_RAW != J2SE_SHAPE(vm)) {
        ...
        currentClassLoaderMID
            = (*env)->GetStaticMethodID(env, clazz, "currentClassLoader", "()Ljava/lang/ClassLoader;");
...
jint JNICALL JNI_CreateJavaVM(JavaVM** pvm, void** penv, void* vm_args)
{
    ...
    /* no tracing for this function, since it's unlikely to be used once the VM is running and the trace engine is
     * initialized */
    preloadLibraries();
...
#endif /* CALL_BUNDLED_FUNCTIONS_DIRECTLY */
            fprintf(stderr,
                "Error: Port Library failed to initialize: expected library size %" J9PRIz "u, actual size is %" J9PRIz
                "u\n",
    ...
    {
#if CALL_BUNDLED_FUNCTIONS_DIRECTLY
        IDATA threadCategoryResult
            = omrthread_lib_control(J9THREAD_LIB_CONTROL_GET_MEM_CATEGORIES, (UDATA)&j9MasterMemCategorySet);
#else
        IDATA threadCategoryResult
            = f_threadLibControl(J9THREAD_LIB_CONTROL_GET_MEM_CATEGORIES, (UDATA)&j9MasterMemCategorySet);
#endif /* CALL_BUNDLED_FUNCTIONS_DIRECTLY */
    ...
    j9portLibrary.omrPortLibrary.port_control(
        &j9portLibrary.omrPortLibrary, J9PORT_CTLDATA_MEM_CATEGORIES_SET, (UDATA)&j9MasterMemCategorySet);
...

    {
        ...
        vmArgumentsList.pool = pool_new(sizeof(J9JavaVMArgInfo),
            ...
            0, 0, J9_GET_CALLSITE(), OMRMEM_CATEGORY_VM, POOL_FOR_PORT(&j9portLibrary));
        ...
        if (
            ...
            (0
                != addOptionsDefaultFile(
                       &j9portLibrary, &vmArgumentsList, optionsDefaultFileLocation, localVerboseLevel))
            || (0 != addXjcl(&j9portLibrary, &vmArgumentsList, j2seVersion))
            || (0
                   != addBootLibraryPath(&j9portLibrary, &vmArgumentsList, "-Dcom.ibm.oti.vm.bootstrap.library.path=",
                          jvmBufferData(j9binBuffer), jvmBufferData(jrebinBuffer)))
            || (0
                   != addBootLibraryPath(&j9portLibrary, &vmArgumentsList,
                          "-Dsun.boot.library.path=", jvmBufferData(j9binBuffer), jvmBufferData(jrebinBuffer)))
            || (0
                   != addJavaLibraryPath(&j9portLibrary, &vmArgumentsList, argEncoding, jvmInSubdir,
                          jvmBufferData(j9binBuffer), jvmBufferData(jrebinBuffer), libpathValue, ldLibraryPathValue))
            || (0 != addJavaHome(&j9portLibrary, &vmArgumentsList, altJavaHomeSpecified, jvmBufferData(j9libBuffer)))
            || (doAddExtDir
                   && (0 != addExtDir(&j9portLibrary, &vmArgumentsList, jvmBufferData(j9libBuffer), args, j2seVersion)))
            || (0 != addUserDir(&j9portLibrary, &vmArgumentsList, cwd))
            || (0 != addJavaPropertiesOptions(&j9portLibrary, &vmArgumentsList, localVerboseLevel))
            || (0
                   != addJarArguments(
                          &j9portLibrary, &vmArgumentsList, specialArgs.executableJarPath, zipFuncs, localVerboseLevel))
            || (0 != addEnvironmentVariables(&j9portLibrary, args, &vmArgumentsList, localVerboseLevel))
            || (0
                   != addLauncherArgs(&j9portLibrary, args, launcherArgumentsSize, &vmArgumentsList, &xServiceBuffer,
                          argEncoding, localVerboseLevel))
#ifdef J9VM_OPT_HARMONY
    ...
    if (VERBOSE_INIT == localVerboseLevel) {
        fprintf(stderr, "VM known paths\t- j9libvm directory: %s\n\t\t- j2seRoot directory: %s\n",
            createParams.j9libvmDirectory, createParams.j2seRootDirectory);
 ...
 * @returns 0 for not found, and a positive integer on success, which represents which path element the file was found
 *in.
 **/
int findDirContainingFile(
    J9StringBuffer** result, char* paths, char pathSeparator, char* fileToFind, int elementsToSkip)
{
...
int findDirUplevelToDirContainingFile(
    J9StringBuffer** result, char* pathEnvar, char pathSeparator, char* fileInPath, int upLevels, int elementsToSkip)
{
    char* paths;
    int rc;
...
static void* preloadLibrary(char* dllName, BOOLEAN inJVMDir)
{
    ...
    MultiByteToWideChar(
        OS_ENCODING_CODE_PAGE, OS_ENCODING_MB_FLAGS, bufferData, -1, unicodePath, (int)bufferLength + 1);
 ...
 */
int pre_block(pre_block_t buf) { return 0; }

int post_block() { return 0; }

#if defined(AIXPPC)
static void setLibpath(const char* libpath) { setenv("LIBPATH", libpath, 1); }
#endif /* AIXPPC */

static void addToLibpath(const char* dir, BOOLEAN isPrepend)
{
#if defined(AIXPPC) || defined(J9ZOS390)
 ...
 */
static void backupLibpath(J9LibpathBackup* libpathBackup, size_t origLibpathLen)
{
 ...
 * @param libpathBackup  The structure that stores a copy of the current LIBPATH.
 */
static void freeBackupLibpath(J9LibpathBackup* libpathBackup)
{
    if (NULL != libpathBackup->fullpath) {
 ...
 */
static void restoreLibpath(J9LibpathBackup* libpathBackup)
{
 ...
 */
static const char* findInLibpath(const char* libpath, const char* backupPath)
{
...
static jint formatErrorMessage(int errorCode, char* inBuffer, jint inBufferLength)
{
    ...
    rc = FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, NULL, errorCode,
        MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), buffer, JVM_DEFAULT_ERROR_BUFFER_SIZE, NULL);
    if (rc == 0) {
        inBuffer[0] = '\0';
        return 0;
    }
 ...
 */
void* JNICALL JVM_LoadSystemLibrary(const char* libName)
{
    JNIEnv* env;
    char errMsg[512];
    ...
    if ((libNameLen <= 4) || ('.' != libName[libNameLen - 4]) || ('d' != j9_cmdla_tolower(libName[libNameLen - 3]))
        || ('l' != j9_cmdla_tolower(libName[libNameLen - 2])) || ('l' != j9_cmdla_tolower(libName[libNameLen - 1]))) {
...
void* JNICALL JVM_LoadLibrary(const char* libName)
{
    JNIEnv* env;
    JavaVM* vm = (JavaVM*)BFUjavaVM;
    char errMsg[512];
...
_end:
    ...
    (*vm)->GetEnv(vm, (void**)&env, JNI_VERSION_1_2);
    if (NULL != env) {
        j9portLibrary.omrPortLibrary.str_printf(
            &j9portLibrary.omrPortLibrary, errMsg, sizeof(errMsg), "Failed to load library \"%s\"", libName);
        throwNewUnsatisfiedLinkError(env, errMsg);
    }
...
void* JNICALL JVM_FindLibraryEntry(void* handle, const char* functionName)
{
    void* result;
 ...
 * JVM_Write
 */
jint JNICALL JVM_Write(jint descriptor, const char* buffer, jint length)
{
 ...
 * JVM_Close
 */
jint JNICALL JVM_Close(jint descriptor)
{
    jint result = 0;
 ...
 * JVM_Available
 */
jint JNICALL JVM_Available(jint descriptor, jlong* bytes)
{
    jlong curr = 0;
    jlong end = 0;
...
#else
    /* CMVC 104382:  Linux lseek uses native word size off_t so we need to use the 64-bit _llseek on 32-bit Linux.  AIX
     * and z/OS use 64-bit API implicitly when we define _LARGE_FILES. */
 ...
 * JVM_Lseek
 */
jlong JNICALL JVM_Lseek(jint descriptor, jlong bytesToSeek, jint origin)
{
    jlong result = 0;
#if defined(LINUX)
    loff_t longResult = 0;
    ...
    /* CMVC 104382:  Linux lseek uses native word size off_t so we need to use the 64-bit _llseek on 32-bit Linux.  AIX
     * and z/OS use 64-bit API implicitly when we define _LARGE_FILES. */
    result = _llseek(descriptor, (unsigned long)((bytesToSeek >> 32) & 0xFFFFFFFF),
        (unsigned long)(bytesToSeek & 0xFFFFFFFF), &longResult, origin);
    if (0 == result) {
        /* the call was successful so set the result to what we read */
 ...
 * JVM_Read
 */
jint JNICALL JVM_Read(jint descriptor, char* buffer, jint bytesToRead)
{
    PORT_ACCESS_FROM_JAVAVM(BFUjavaVM);
    jint result = 0;
 ...
 * JVM_Open method
 */
jint JNICALL JVM_Open(const char* filename, jint flags, jint mode)
{
    int errorVal = 0;
    jint returnVal = 0;
...
#if !defined(J9ZTPF)
    flags &= (O_CREAT | O_APPEND | O_RDONLY | O_RDWR | O_TRUNC | O_WRONLY | O_EXCL | O_NOCTTY | O_NONBLOCK | O_NDELAY
        | O_SYNC | O_DSYNC);
#else /* !defined(J9ZTPF) */
    flags &= (O_CREAT | O_APPEND | O_RDONLY | O_RDWR | O_TRUNC | O_WRONLY | O_EXCL | O_NOCTTY | O_NONBLOCK | O_SYNC
        | O_DSYNC);
#endif /* !defined(J9ZTPF) */
#endif /* defined(J9UNIX) || defined(J9ZOS390) */
 ...
 * JVM_Sync
 */
jint JNICALL JVM_Sync(jint descriptor)
{
    jint result;
 ...
 */
jboolean JNICALL JVM_RaiseSignal(jint sigNum)
{
 ...
 */
void* JNICALL JVM_RegisterSignal(jint sigNum, void* handler)
{
 ...
 */
jint JNICALL JVM_FindSignal(const char* sigName)
{
 ...
 */
jint JNICALL JVM_GetLastErrorString(char* buffer, jint length)
{
    ...
    if (errorCode) {
        ...
        sigRC = j9sig_protect(protectedStrerror, (void*)(IDATA)savedErrno, strerrorSignalHandler, NULL,
            J9PORT_SIG_FLAG_SIGALLSYNC | J9PORT_SIG_FLAG_MAY_RETURN, &fnRC);
        if (sigRC == 0) {
 ...
 */
void JNICALL JVM_TraceInstructions(jboolean on) { Trc_SC_TraceInstructions(on); }

/**
 * Method stub for method not yet implemented.
 */
void JNICALL JVM_TraceMethodCalls(jboolean on) { Trc_SC_TraceMethodCalls(on); }
 ...
 * JVM_Accept
 */
jint JNICALL JVM_Accept(jint descriptor, struct sockaddr* address, int* length)
{
    jint retVal;
 ...
 * JVM_Connect
 */
jint JNICALL JVM_Connect(jint descriptor, const struct sockaddr* address, int length)
{
    jint retVal;
 ...
 */
jlong JNICALL JVM_CurrentTimeMillis(JNIEnv* env, jint unused1)
{
    jlong result;
 ...
 * JVM_Recv
 */
jint JNICALL JVM_Recv(jint descriptor, char* buffer, jint length, jint flags)
{
    jint retVal;
 ...
 * JVM_RecvFrom
 */
jint JNICALL JVM_RecvFrom(
    jint descriptor, char* buffer, jint length, jint flags, struct sockaddr* fromAddr, int* fromLength)
{
    jint retVal;
 ...
 * JVM_Send
 */
jint JNICALL JVM_Send(jint descriptor, const char* buffer, jint numBytes, jint flags)
{
    jint retVal;
 ...
 * JVM_SendTo
 */
jint JNICALL JVM_SendTo(
    jint descriptor, const char* buffer, jint length, jint flags, const struct sockaddr* toAddr, int toLength)
{
    jint retVal;
 ...
 */
jint JNICALL JVM_SocketAvailable(jint descriptor, jint* result)
{
    jint retVal = 0;
 ...
 * JVM_SocketClose
 */
jint JNICALL JVM_SocketClose(jint descriptor)
{
    jint retVal;
 ...
 * JVM_Timeout
 */
jint JNICALL JVM_Timeout(jint descriptor, jint timeout)
{
    jint result = 0;
    struct timeval tval;
#ifdef WIN32
 ...
 */
jboolean JNICALL JVM_CX8Field(JNIEnv* env, jobject obj, jfieldID field, jlong oldval, jlong newval)
{
 ...
 */
void JNICALL JVM_RegisterUnsafeMethods(JNIEnv* env, jclass unsafeClz) { Trc_SC_RegisterUnsafeMethods(env); }
...
jint JNICALL JVM_ActiveProcessorCount(void)
{
     ...
     * RTC 112959: [was 209402] Liberty JAX-RS Default Executor poor performance.  Match reference implementation
     * behaviour to return the bound CPUs rather than physical CPUs.
     *
 ...
 * JVM_GetClassName
 */
jstring JNICALL JVM_GetClassName(JNIEnv* env, jclass theClass)
{
...
jint JNICALL JVM_Sleep(JNIEnv* env, jclass thread, jlong timeout)
{
...
jint JNICALL JVM_UcsOpen(const jchar* filename, jint flags, jint mode)
{
#ifdef WIN32
    WCHAR* prefixStr;
    DWORD prefixLen;
    ...
    if (filename[0] == L'\0' || filename[1] == L'\0')
        fullPathLen += 3;
 ...
 */
jclass JNICALL JVM_ConstantPoolGetClassAt(JNIEnv* env, jobject anObject, jobject constantPool, jint index)
{
 ...
 */
jclass JNICALL JVM_ConstantPoolGetClassAtIfLoaded(JNIEnv* env, jobject anObject, jobject constantPool, jint index)
{
 ...
 */
jdouble JNICALL JVM_ConstantPoolGetDoubleAt(JNIEnv* env, jobject anObject, jobject constantPool, jint index)
{
 ...
 */
jobject JNICALL JVM_ConstantPoolGetFieldAt(JNIEnv* env, jobject anObject, jobject constantPool, jint index)
{
 ...
 */
jobject JNICALL JVM_ConstantPoolGetFieldAtIfLoaded(JNIEnv* env, jobject anObject, jobject constantPool, jint index)
{
 ...
 */
jfloat JNICALL JVM_ConstantPoolGetFloatAt(JNIEnv* env, jobject anObject, jobject constantPool, jint index)
{
 ...
 */
jint JNICALL JVM_ConstantPoolGetIntAt(JNIEnv* env, jobject anObject, jobject constantPool, jint index)
{
 ...
 */
jlong JNICALL JVM_ConstantPoolGetLongAt(JNIEnv* env, jobject anObject, jobject constantPool, jint index)
{
 ...
 */
jobjectArray JNICALL JVM_ConstantPoolGetMemberRefInfoAt(JNIEnv* env, jobject anObject, jobject constantPool, jint index)
{
 ...
 */
jobject JNICALL JVM_ConstantPoolGetMethodAt(JNIEnv* env, jobject anObject, jobject constantPool, jint index)
{
 ...
 */
jobject JNICALL JVM_ConstantPoolGetMethodAtIfLoaded(JNIEnv* env, jobject anObject, jobject constantPool, jint index)
{
 ...
 */
jint JNICALL JVM_ConstantPoolGetSize(JNIEnv* env, jobject anObject, jobject constantPool)
{
 ...
 */
jstring JNICALL JVM_ConstantPoolGetStringAt(JNIEnv* env, jobject anObject, jobject constantPool, jint index)
{
 ...
 */
jstring JNICALL JVM_ConstantPoolGetUTF8At(JNIEnv* env, jobject anObject, jobject constantPool, jint index)
{
 ...
 */
jclass JNICALL JVM_DefineClassWithSource(JNIEnv* env, const char* className, jobject classLoader,
    const jbyte* classArray, jsize length, jobject domain, const char* source)
{
    ...
    if (CLASSNAME_INVALID == vmFuncs->verifyQualifiedName(currentThread, J9_JNI_UNWRAP_REFERENCE(classNameString))) {
        vmFuncs->setCurrentException(
            currentThread, J9VMCONSTANTPOOL_JAVALANGNOCLASSDEFFOUNDERROR, (UDATA*)*(j9object_t*)classNameString);
 ...
 */
jobjectArray JNICALL JVM_DumpThreads(JNIEnv* env, jclass thread, jobjectArray threadArray)
{
 ...
 */
jobjectArray JNICALL JVM_GetAllThreads(JNIEnv* env, jclass aClass)
{
 ...
 */
jbyteArray JNICALL JVM_GetClassAnnotations(JNIEnv* env, jclass target)
{
 ...
 */
jobject JNICALL JVM_GetClassConstantPool(JNIEnv* env, jclass target)
{
 ...
 */
jstring JNICALL JVM_GetClassSignature(JNIEnv* env, jclass target)
{
 ...
 */
jobjectArray JNICALL JVM_GetEnclosingMethodInfo(JNIEnv* env, jclass theClass)
{
 ...
 */
jlong JNICALL JVM_NanoTime(JNIEnv* env, jclass aClass)
{
    jlong ticks, freq;
...
struct J9PortLibrary* JNICALL JVM_GetPortLibrary(void) { return &j9portLibrary; }

jint JNICALL JVM_ExpandFdTable(jint fd) { return 0; }

void JNICALL JVM_ZipHook(JNIEnv* env, const char* filename, jint newState)
{
#ifdef J9VM_OPT_ZIP_SUPPORT
...
void* JNICALL JVM_RawAllocate(size_t size, const char* callsite)
{
    return j9portLibrary.omrPortLibrary.mem_allocate_memory(&j9portLibrary.omrPortLibrary, (UDATA)size,
        (char*)((callsite == NULL) ? J9_GET_CALLSITE() : callsite), J9MEM_CATEGORY_SUN_JCL);
}

void* JNICALL JVM_RawRealloc(void* ptr, size_t size, const char* callsite)
{
    return j9portLibrary.omrPortLibrary.mem_reallocate_memory(
        &j9portLibrary.omrPortLibrary, ptr, (UDATA)size, callsite, J9MEM_CATEGORY_SUN_JCL);
}

void* JNICALL JVM_RawCalloc(size_t nmemb, size_t size, const char* callsite)
{
...
void* JNICALL JVM_RawAllocateInCategory(size_t size, const char* callsite, jint category)
{
    return j9portLibrary.omrPortLibrary.mem_allocate_memory(&j9portLibrary.omrPortLibrary, (UDATA)size,
        (char*)((callsite == NULL) ? J9_GET_CALLSITE() : callsite), category);
}

void* JNICALL JVM_RawReallocInCategory(void* ptr, size_t size, const char* callsite, jint category)
{
    return j9portLibrary.omrPortLibrary.mem_reallocate_memory(
        &j9portLibrary.omrPortLibrary, ptr, (UDATA)size, callsite, category);
}

void* JNICALL JVM_RawCallocInCategory(size_t nmemb, size_t size, const char* callsite, jint category)
{
    size_t byteSize = nmemb * size;
...
void JNICALL JVM_RawFree(void* ptr)
{
 ...
 * JVM_IsNaN
 */
jboolean JNICALL JVM_IsNaN(jdouble dbl)
{
 ...
 */
jint JNICALL JVM_Startup(JavaVM* vm, JNIEnv* env) { return JNI_OK; }
