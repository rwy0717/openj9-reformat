 ...
 */
static BOOLEAN isFileOwnedByMe(JNIEnv* env, const char* pathUTF)
{
...
jstring JNICALL Java_com_ibm_tools_attach_target_IPC_getTempDirImpl(JNIEnv* env, jclass clazz)
{
    ...
    if (NULL != charResult) {
        ...
        conversionResult = j9str_convert(J9STR_CODE_PLATFORM_OMR_INTERNAL, J9STR_CODE_MUTF8, charResult, pathLength,
            (char*)defaultConversionBuffer, sizeof(defaultConversionBuffer));
        if (OMRPORT_ERROR_STRING_BUFFER_TOO_SMALL == conversionResult) {
            /* determine how much space we really need */
            int32_t requiredSize
                = j9str_convert(J9STR_CODE_PLATFORM_OMR_INTERNAL, J9STR_CODE_MUTF8, charResult, pathLength, NULL, 0);
            if (requiredSize > 0) {
                ...
                if (NULL != conversionBuffer) {
                    j9str_convert(J9STR_CODE_PLATFORM_OMR_INTERNAL, J9STR_CODE_MUTF8, charResult, pathLength,
 ...
 * @param path file system path
 * @return Returns 0 on Windows and the effective UID of owner on other operation systems. Returns -1 in the case of an
 * error.
 */
...
jboolean JNICALL Java_com_ibm_tools_attach_target_IPC_isUsingDefaultUid(JNIEnv* env, jclass clazz)
{
    ...
    U_32* PSATOLD_ADDR
        = (U_32*)(UDATA)0x21c; /* z/OS Pointer to current TCB or zero if in SRB mode. Field fixed by architecture. */
    U_32 tcbBase; /* base of the z/OS Task Control Block */
    const TCBSENV_OFFSET
        = 0x154; /* offset of the TCBSENV field in the TCB.  This field contains a pointer to the ACEE. */
    U_32 aceeBaseAddr; /* Address of a control block field which contains a pointer to the base of the RACF Accessor
    ...
    if (0 == aceeBase) { /* not using task-level ACEE */
        ...
        const U_32 ASCBASXB_OFFSET
            = 0x6c; /* offset of the address space extension control block (ASXB) field in the ASCB */
        ...
        const U_32 ASXBSENV_OFFSET
            = 0xc8; /* offset of the ASXBSENV field in the ASXB. This field contains a pointer to the ACEE. */
    ...
    JNIEnv* env, jclass clazz, jstring absolutePath, jint cdPerms)
{
    ...
    if (NULL != absolutePathUTF) {
        ...
        /* ensure that the directory group is the same as this process's.  Some OS's set the group to that of the parent
         * directory */
...
static jint JNICALL openSemaphore(
    JNIEnv* env, jclass clazz, jstring ctrlDirName, jstring semaName, struct j9shsem_handle** semaphore)
{
    ...
    JNIEnv* env, jclass clazz, jstring ctrlDirName, jstring semaName)
{
    ...
    if ((J9PORT_INFO_SHSEM_OPENED == rc) || (J9PORT_INFO_SHSEM_OPENED_STALE == rc)
        || (J9PORT_INFO_SHSEM_CREATED == rc)) {
        rc = JNI_OK;
    }
    ...
    JNIEnv* env, jclass clazz, jstring ctrlDirName, jstring semaName, jint numberOfDecrements)
{
    ...
    if ((J9PORT_INFO_SHSEM_OPENED == status) || (J9PORT_INFO_SHSEM_OPENED_STALE == status)) {
        ...
        /* Jazz 27080. this was supposed to be consuming posts on an existing semaphore, but it appears to have
         * disappeared */
        status = (jint)j9shsem_destroy(&semaphore);
    }
 ...
 * J9PORT_ERROR_FILE_LOCK_BADLOCK (-316) indicates that the file could not be locked.
 */
    ...
    JNIEnv* env, jclass clazz, jstring path, jint mode, jboolean blocking)
{
    ...
    if (NULL != pathUTF) {
        ...
        if (0 >= fd) {
             ...
             */
            lockStatus = j9file_lock_bytes(fd,
                J9PORT_FILE_WRITE_LOCK | ((0 == blocking) ? J9PORT_FILE_NOWAIT_FOR_LOCK : J9PORT_FILE_WAIT_FOR_LOCK), 0,
                1);
            if (0 != lockStatus) {
                j9file_close(fd);
