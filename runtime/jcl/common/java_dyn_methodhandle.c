...
static VMINLINE UDATA lookupImpl(J9VMThread* currentThread, J9Class* lookupClass, J9UTF8* name, J9UTF8* signature,
    J9Class* senderClass, UDATA options, BOOLEAN* foundDefaultConflicts);
...
static jlong JNICALL vmConvertITableIndexToVTableIndex(
...
static BOOLEAN accessCheckMethodSignature(
    J9VMThread* currentThread, J9Method* method, j9object_t methodType, J9UTF8* lookupSig);
static BOOLEAN accessCheckFieldSignature(
    J9VMThread* currentThread, J9Class* lookupClass, UDATA romField, j9object_t methodType, J9UTF8* lookupSig);
static char* expandNLSTemplate(J9VMThread* vmThread, const char* nlsTemplate, ...);
#if defined(J9VM_OPT_JAVA_OFFLOAD_SUPPORT)
static void clearNonZAAPEligibleBit(
    JNIEnv* env, jclass nativeClass, const JNINativeMethod* nativeMethods, jint nativeMethodCount);
 ...
 */
static UDATA lookupStaticMethod(J9VMThread* currentThread, J9Class* lookupClass, J9UTF8* name, J9UTF8* signature)
{
    return lookupImpl(currentThread, lookupClass, name, signature, NULL, J9_LOOK_STATIC, NULL);
}

static UDATA lookupInterfaceMethod(
    J9VMThread* currentThread, J9Class* lookupClass, J9UTF8* name, J9UTF8* signature, UDATA* methodIndex)
{
    J9Method* method;
    ...
    *methodIndex = -1;
    ...
    if (method != NULL) {
        ...
        if ((J2SE_VERSION(currentThread->javaVM) < J2SE_V11)
            && J9_ARE_ANY_BITS_SET(J9_ROM_METHOD_FROM_RAM_METHOD(method)->modifiers, J9AccPrivate)) {
            /* [PR 67082] private interface methods require invokespecial, not invokeinterface.*/
            vmFuncs->setCurrentExceptionNLS(currentThread, J9VMCONSTANTPOOL_JAVALANGINCOMPATIBLECLASSCHANGEERROR,
                J9NLS_JCL_PRIVATE_INTERFACE_REQUIRES_INVOKESPECIAL);
            method = NULL;
        } else {
            if (J9_ARE_ANY_BITS_SET(J9_CLASS_FROM_METHOD(method)->romClass->modifiers, J9_JAVA_INTERFACE)) {
                *methodIndex = getITableIndexForMethod(method, lookupClass);
                if (-1 == *methodIndex) {
                    ...
                    size_t nameAndSigLength = J9UTF8_LENGTH(className) + J9UTF8_LENGTH(methodName) + J9UTF8_LENGTH(sig)
                        + 4 /* period, parentheses, and terminating null */;
                    char* msg = j9mem_allocate_memory(nameAndSigLength, OMRMEM_CATEGORY_VM);
                    if (NULL != msg) {
                        j9str_printf(PORTLIB, msg, nameAndSigLength, "%.*s.%.*s(%.*s)", J9UTF8_LENGTH(className),
                            J9UTF8_DATA(className), J9UTF8_LENGTH(methodName), J9UTF8_DATA(methodName),
                            J9UTF8_LENGTH(sig), J9UTF8_DATA(sig));
                        vmFuncs->setCurrentExceptionUTF(currentThread, J9VMCONSTANTPOOL_JAVALANGNOSUCHMETHODERROR, msg);
 ...
 */
static UDATA lookupSpecialMethod(J9VMThread* currentThread, J9Class* lookupClass, J9UTF8* name, J9UTF8* signature,
    J9Class* specialCaller, BOOLEAN isForVirtualLookup)
{
    ...
    method = (J9Method*)lookupImpl(
        currentThread, lookupClass, name, signature, specialCaller, lookupOptions, &foundDefaultConflicts);

    if (foundDefaultConflicts) {
        J9InternalVMFunctions* vmFuncs = currentThread->javaVM->internalVMFunctions;
        /* Don't need to clear the current exception here - it will be cleared when setting the exception. */
        vmFuncs->setCurrentExceptionWithCause(currentThread,
            J9VMCONSTANTPOOL_JAVALANGINVOKEDEFAULTMETHODCONFLICTEXCEPTION, NULL, currentThread->currentException);
    } else if (NULL != method) {
        ...
        if (isForVirtualLookup || isDirectSuperInterface(currentThread, resolvedClass, specialCaller)) {
            /* CMVC 197763 seq#4: Use the method's class, not the lookupClass, as the resolvedClass arg.  LookupClass
             * may be unrelated */
            method = getMethodForSpecialSend(currentThread, specialCaller, resolvedClass, method,
                J9_LOOK_VIRTUAL | J9_LOOK_NO_VISIBILITY_CHECK | J9_LOOK_IGNORE_INCOMPATIBLE_METHODS);
 ...
 */
static UDATA lookupImpl(J9VMThread* currentThread, J9Class* lookupClass, J9UTF8* name, J9UTF8* signature,
    J9Class* senderClass, UDATA options, BOOLEAN* foundDefaultConflicts)
{
    J9Method* method;
    ...
    method = (J9Method*)vm->internalVMFunctions->javaLookupMethodImpl(currentThread, lookupClass,
        (J9ROMNameAndSignature*)&nameAndSig, senderClass,
        options /* | J9_LOOK_NO_THROW | J9_LOOK_CLCONSTRAINTS */ | J9_LOOK_DIRECT_NAS, foundDefaultConflicts);
...
void JNICALL Java_java_lang_invoke_MethodHandle_requestCustomThunkFromJit(JNIEnv* env, jobject handle, jobject thunk)
{
    ...
    vm->jitConfig->translateMethodHandle(
        vmThread, J9_JNI_UNWRAP_REFERENCE(handle), J9_JNI_UNWRAP_REFERENCE(thunk), J9_METHOD_HANDLE_COMPILE_CUSTOM);
    ...
    JNIEnv* env, jobject handle, jclass lookupClass, jstring name, jstring signature, jbyte kind, jclass specialCaller)
{
    ...
    nameUTF8 = vmFuncs->copyStringToJ9UTF8WithMemAlloc(
        vmThread, J9_JNI_UNWRAP_REFERENCE(name), J9_STR_NONE, "", 0, nameUTF8Buffer, sizeof(nameUTF8Buffer));
    ...
    signatureUTF8 = vmFuncs->copyStringToJ9UTF8WithMemAlloc(vmThread, J9_JNI_UNWRAP_REFERENCE(signature), J9_STR_NONE,
        "", 0, signatureUTF8Buffer, sizeof(signatureUTF8Buffer));
    ...
    case J9_METHOD_HANDLE_KIND_VIRTUAL:
         ...
         * and then convert it into a VirtualHandle using setVMSlotAndRawModifiersFromSpecialHandle().  This is
         * necessary as not every method invokeable by invokevirtual is in the vtable (ie: final methods in Object). The
         * reason why we can't go directly to the KIND_SPECIAL case is because we need to allow lookup of VirtualHandle
        ...
        J9VMJAVALANGINVOKEMETHODHANDLE_SET_KIND(
            vmThread, J9_JNI_UNWRAP_REFERENCE(handle), J9_METHOD_HANDLE_KIND_SPECIAL);
    ...
    case J9_METHOD_HANDLE_KIND_SPECIAL:
        ...
        method = lookupSpecialMethod(
            vmThread, j9LookupClass, nameUTF8, signatureUTF8, j9SpecialCaller, isForVirtualLookup);
        vmSlotValue = method;
        break;
    default:
    ...
    if (method != (UDATA)NULL) {
        ...
        if (!accessCheckMethodSignature(vmThread, (J9Method*)method,
                J9VMJAVALANGINVOKEMETHODHANDLE_TYPE(vmThread, J9_JNI_UNWRAP_REFERENCE(handle)), signatureUTF8)) {
            J9Class* methodClass = J9_CLASS_FROM_METHOD((J9Method*)method);
        ...
        J9VMJAVALANGINVOKEPRIMITIVEHANDLE_SET_RAWMODIFIERS(
            vmThread, J9_JNI_UNWRAP_REFERENCE(handle), J9_ROM_METHOD_FROM_RAM_METHOD(method)->modifiers);
    ...
    if (NULL != verifyData) {
        ...
        if ('L' == lookupSigData[sigOffset]) {
            ...
            if (ramClass->classLoader != targetClassloader) {
                ...
                if (verifyData->checkClassLoadingConstraintForNameFunction(currentThread, targetClassloader,
                        ramClass->classLoader, &lookupSigData[sigOffset], &lookupSigData[sigOffset], sigLength, TRUE)
                    != 0) {
                    result = FALSE;
                }
    ...
    if (NULL != verifyData) {
        ...
        for (i = start; i < numParameters; i++) {
            ...
            if ('L' == lookupSigData[index]) {
                ...
                if (argumentRamClass->classLoader != targetClassloader) {
                    if (verifyData->checkClassLoadingConstraintForNameFunction(currentThread, targetClassloader,
                            argumentRamClass->classLoader, &J9UTF8_DATA(targetSig)[index], &lookupSigData[index],
                            endIndex - index, TRUE)
                        != 0) {
                        result = FALSE;
        ...
        if ('L' == lookupSigData[index]) {
            ...
            if (targetClassloader != returnRamClass->classLoader) {
                ...
                if (verifyData->checkClassLoadingConstraintForNameFunction(currentThread, targetClassloader,
                        returnRamClass->classLoader, &J9UTF8_DATA(targetSig)[index], &lookupSigData[index],
                        endIndex - index, TRUE)
                    != 0) {
                    result = FALSE;
                    goto releaseMutexAndReturn;
...
UDATA
lookupField(JNIEnv* env, jboolean isStatic, J9Class* j9LookupClass, jstring name, J9UTF8* sigUTF,
    J9Class** definingClass, UDATA* romField, jclass accessClass)
{
    ...
    if (JNI_TRUE == isStatic) {
        field = (UDATA)vmFuncs->staticFieldAddress(vmThread, j9LookupClass, J9UTF8_DATA(nameUTF8),
            J9UTF8_LENGTH(nameUTF8), J9UTF8_DATA(sigUTF), J9UTF8_LENGTH(sigUTF), definingClass, romField, 0,
            j9AccessClass);
        if (0 == field) {
        ...
        field = (UDATA)vmFuncs->instanceFieldOffset(vmThread, j9LookupClass, J9UTF8_DATA(nameUTF8),
            J9UTF8_LENGTH(nameUTF8), J9UTF8_DATA(sigUTF), J9UTF8_LENGTH(sigUTF), definingClass, romField, 0);
        if (-1 == field) {
            /* IllegalAccessError / IncompatibleClassChangeError / NoSuchFieldError will be pending */
            Assert_JCL_notNull(vmThread->currentException);
    ...
    jstring name, jstring signature, jboolean isStatic, jclass accessClass)
{
    ...
    /* Check signature for classloader visibility */
    if (!accessCheckFieldSignature(vmThread, j9LookupClass, romField,
            J9VMJAVALANGINVOKEMETHODHANDLE_TYPE(vmThread, J9_JNI_UNWRAP_REFERENCE(handle)), signatureUTF8)) {
    ...
    if (NULL != accessClass) {
        ...
        if ((j9AccessClass->classLoader != j9LookupClass->classLoader)
            && !accessCheckFieldSignature(vmThread, j9AccessClass, romField,
                   J9VMJAVALANGINVOKEMETHODHANDLE_TYPE(vmThread, J9_JNI_UNWRAP_REFERENCE(handle)), signatureUTF8)) {
    ...
    J9VMJAVALANGINVOKEPRIMITIVEHANDLE_SET_RAWMODIFIERS(
        vmThread, J9_JNI_UNWRAP_REFERENCE(handle), ((J9ROMFieldShape*)romField)->modifiers);
    ...
    JNIEnv* env, jclass clazz, jobject handle, jobject reflectField)
{
    ...
    J9VMJAVALANGINVOKEPRIMITIVEHANDLE_SET_RAWMODIFIERS(
        vmThread, J9_JNI_UNWRAP_REFERENCE(handle), fieldID->field->modifiers);
    result = JNI_TRUE;
...
jboolean JNICALL Java_java_lang_invoke_PrimitiveHandle_setVMSlotAndRawModifiersFromMethod(
    JNIEnv* env, jclass clazz, jobject handle, jclass unused, jobject method, jbyte kind, jclass specialToken)
{
    J9VMThread* vmThread = (J9VMThread*)env;
    J9JavaVM* vm = vmThread->javaVM;
    J9InternalVMFunctions* vmFuncs = vm->internalVMFunctions;
    ...
    case J9_METHOD_HANDLE_KIND_SPECIAL:
        ...
        if ((J9AccMethodVTable != (modifiers & J9AccMethodVTable))) {
            ...
            vmSlotValue = (UDATA)getMethodForSpecialSend(vmThread, j9SpecialToken,
                J9_CLASS_FROM_METHOD(methodID->method), methodID->method,
                J9_LOOK_VIRTUAL | J9_LOOK_NO_VISIBILITY_CHECK | J9_LOOK_IGNORE_INCOMPATIBLE_METHODS);
    ...
    JNIEnv* env, jclass clazz, jobject handle, jobject ctor)
{
    ...
    J9VMJAVALANGINVOKEPRIMITIVEHANDLE_SET_RAWMODIFIERS(
        vmThread, J9_JNI_UNWRAP_REFERENCE(handle), J9_ROM_METHOD_FROM_RAM_METHOD(methodID->method)->modifiers);
 ...
 */
void JNICALL Java_java_lang_invoke_MethodHandles_00024Lookup_setAccessClassForSecuritCheck(
    JNIEnv* env, jclass ignored, jclass accessClass)
{
    J9VMThread* vmThread = (J9VMThread*)env;
    J9JavaVM* vm = vmThread->javaVM;
    J9InternalVMFunctions* vmFuncs = vm->internalVMFunctions;
...
jlong JNICALL Java_java_lang_invoke_MethodHandles_findNativeAddress(JNIEnv* env, jclass jlClass, jstring methodName)
{
    ...
    JNIEnv* env, jclass mutableCallSite, jlong bypassOffset)
{
    if (0 != bypassOffset) {
        ...
        vmFuncs->internalEnterVMFromJNI(vmThread);
        globalRef = (jobject)(
            (UDATA)J9VM_J9CLASS_FROM_HEAPCLASS(vmThread, J9_JNI_UNWRAP_REFERENCE(mutableCallSite))->ramStatics
            + ((UDATA)bypassOffset & ~(UDATA)J9_SUN_FIELD_OFFSET_MASK));
...
{
    JNINativeMethod natives[] = { { "initialInvokeExactThunk", "()J", (void*)&vmInitialInvokeExactThunk } };
    JNINativeMethod finalizeNative[] = { { "finalizeImpl", "(J)V", (void*)&vmFinalizeImpl } };
...
{
    JNINativeMethod natives[] = { { "invalidate", "([J)V", (void*)&vmInvalidate } };
    registerVMNativesIfNoJIT(env, nativeClass, natives, 1);
    return;
}
...
void JNICALL Java_java_lang_invoke_InterfaceHandle_registerNatives(JNIEnv* env, jclass nativeClass)
{
    JNINativeMethod natives[]
        = { { "convertITableIndexToVTableIndex", "(JIJ)I", (void*)&vmConvertITableIndexToVTableIndex } };
    registerVMNativesIfNoJIT(env, nativeClass, natives, 1);
    return;
}
...
static jlong JNICALL vmInitialInvokeExactThunk(JNIEnv* env, jclass ignored) { return 0; }
...
static void JNICALL vmFinalizeImpl(JNIEnv* env, jclass methodHandleClass, jlong thunkAddress) { return; }
...
void setClassLoadingConstraintLinkageError(J9VMThread* vmThread, J9Class* methodOrFieldClass, J9UTF8* signatureUTF8)
{
    ...
    const char* nlsTemplate = j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE,
        J9NLS_JCL_METHODTYPE_CLASS_LOADING_CONSTRAINT,
        "loading constraint violation: %2$.*1$s not visible from %4$.*3$s");
    char* msg = expandNLSTemplate(vmThread, nlsTemplate, J9UTF8_LENGTH(signatureUTF8), J9UTF8_DATA(signatureUTF8),
