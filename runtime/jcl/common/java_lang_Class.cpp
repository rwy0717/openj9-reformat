...
typedef enum { STATE_INITIAL = -2, STATE_ERROR = -1, STATE_STOP = 0, STATE_IMPLIED = 1 } StackWalkingStates;
...
static UDATA frameIteratorGetAccSnapshotHelper(
    J9VMThread* currentThread, J9StackWalkState* walkState, j9object_t acc, j9object_t perm);
static j9object_t storePDobjectsHelper(J9VMThread* vmThread, J9Class* arrayClass, J9StackWalkState* walkState,
    j9object_t contextObject, U_32 arraySize, UDATA framesWalked, I_32 startPos, BOOLEAN dupCallerPD);
...
static UDATA isPrivilegedFrameIterator(J9VMThread* currentThread, J9StackWalkState* walkState)
{
    ...
    if (J9_ARE_ALL_BITS_SET(
            J9_ROM_METHOD_FROM_RAM_METHOD(currentMethod)->modifiers, J9_JAVA_METHOD_FRAME_ITERATOR_SKIP)) {
    ...
    if (NULL == walkState->userData2) {
        ...
        if ((walkState->method == vm->jlrMethodInvoke) || (walkState->method == vm->jliMethodHandleInvokeWithArgs)
            || (walkState->method == vm->jliMethodHandleInvokeWithArgsList)
            || (vm->srMethodAccessor
                   && VM_VMHelpers::isSameOrSuperclass(
                          J9VM_J9CLASS_FROM_JCLASS(currentThread, vm->srMethodAccessor), currentClass))) {
    ...
    if (((doPrivilegedWithContextMethodID1 != NULL) && (currentMethod == doPrivilegedWithContextMethodID1->method))
        || ((doPrivilegedWithContextMethodID2 != NULL)
               && (currentMethod == doPrivilegedWithContextMethodID2->method))) {
    ...
    JNIEnv* env, jclass jlHeapClass, jint maxDepth, jboolean stopAtPrivileged)
{
    ...
    UDATA walkFlags = J9_STACKWALK_CACHE_METHODS | J9_STACKWALK_COUNT_SPECIFIED | J9_STACKWALK_VISIBLE_ONLY
    ...
    walkState.userData1 = STACK_WALK_STATE_MAGIC; /* value which will not be written into userData1 by the frame walk
                                                     function (which writes only null or an object pointer) */
    walkState.userData2 = STACK_WALK_STATE_MAGIC; /* value which will not be written into userData2 by the frame walk
                                                     function (which writes only null) */
    ...
    for (i = framesWalked; i > 0; i--) {
        ...
        if (J9_ARE_ALL_BITS_SET(
                J9_ROM_METHOD_FROM_RAM_METHOD(currentMethod)->modifiers, J9_JAVA_METHOD_FRAME_ITERATOR_SKIP)
            || (vm->jliArgumentHelper
                   && instanceOfOrCheckCast(currentClass, J9VM_J9CLASS_FROM_JCLASS(vmThread, vm->jliArgumentHelper)))
            || (vm->srMethodAccessor
                   && vmFuncs->instanceOfOrCheckCast(
                          currentClass, J9VM_J9CLASS_FROM_HEAPCLASS(vmThread, *((j9object_t*)vm->srMethodAccessor))))
            || (vm->srConstructorAccessor
                   && vmFuncs->instanceOfOrCheckCast(currentClass,
                          J9VM_J9CLASS_FROM_HEAPCLASS(vmThread, *((j9object_t*)vm->srConstructorAccessor))))) {
            currentClass = NULL;
            framesWalked--;
        }
    ...
    arrayObject = vm->memoryManagerFunctions->J9AllocateIndexableObject(
        vmThread, arrayClass, (U_32)framesWalked, J9_GC_ALLOCATE_OBJECT_NON_INSTRUMENTABLE);
    if (NULL == arrayObject) {
...
jobject JNICALL Java_com_ibm_oti_vm_VM_getClassNameImpl(JNIEnv* env, jclass recv, jclass jlClass)
{
    ...
    if (NULL != utfData) {
        ...
        j9object_t classNameObject
            = vm->memoryManagerFunctions->j9gc_createJavaLangString(currentThread, utfData, utfLength, flags);
...
jobject JNICALL Java_java_lang_Class_getGenericSignature(JNIEnv* env, jobject recv)
{
    J9VMThread* currentThread = (J9VMThread*)env;
    J9JavaVM* vm = currentThread->javaVM;
    J9InternalVMFunctions* vmFuncs = vm->internalVMFunctions;
    ...
    if (NULL != signature) {
        j9object_t stringObject = vm->memoryManagerFunctions->j9gc_createJavaLangString(
            currentThread, J9UTF8_DATA(signature), J9UTF8_LENGTH(signature), 0);
 ...
 */
static VMINLINE bool isSpecialMethod(J9ROMMethod* romMethod)
{
 ...
 */
static VMINLINE bool isConstructor(J9ROMMethod* romMethod)
{
 ...
 */
static VMINLINE bool isNormalStaticMethod(J9ROMMethod* romMethod)
{
 ...
 * @returns true if this is <init>, false if not
 */
static jobject getConstructorsHelper(JNIEnv* env, jobject recv, bool mustBePublic)
{
    J9VMThread* currentThread = (J9VMThread*)env;
    J9JavaVM* vm = currentThread->javaVM;
    J9InternalVMFunctions* vmFuncs = vm->internalVMFunctions;
...
retry:
    ...
    if (NULL != arrayClass) {
        resultObject = mmFuncs->J9AllocateIndexableObject(
            currentThread, arrayClass, size, J9_GC_ALLOCATE_OBJECT_NON_INSTRUMENTABLE);
        if (vm->hotSwapCount != preCount) {
            ...
            while (currentMethod != endOfMethods) {
                J9ROMMethod* romMethod = J9_ROM_METHOD_FROM_RAM_METHOD(currentMethod);
                if (isConstructor(romMethod)
                    && (!mustBePublic || J9_ARE_ANY_BITS_SET(romMethod->modifiers, J9AccPublic))) {
                    PUSH_OBJECT_IN_SPECIAL_FRAME(currentThread, resultObject);
                    j9object_t element
                        = vm->reflectFunctions.createConstructorObject(currentMethod, clazz, NULL, currentThread);
 ...
 * @returns true if this is <init>, false if not
 */
static jobject getConstructorHelper(
    JNIEnv* env, jobject recv, jobject parameterTypes, jobject signature, bool mustBePublic)
{
    J9VMThread* currentThread = (J9VMThread*)env;
    J9JavaVM* vm = currentThread->javaVM;
    J9InternalVMFunctions* vmFuncs = vm->internalVMFunctions;
        ...
        if (!J9ROMCLASS_IS_PRIMITIVE_OR_ARRAY(romClass)) {
            ...
            while (currentMethod != endOfMethods) {
                ...
                if (isConstructor(romMethod)
                    ...
                    if (0
                        != vmFuncs->compareStringToUTF8(currentThread, signatureObject, TRUE, J9UTF8_DATA(signatureUTF),
                               J9UTF8_LENGTH(signatureUTF))) {
                        j9object_t parameterTypesObject = NULL;
                        if (NULL != parameterTypes) {
                            parameterTypesObject = J9_JNI_UNWRAP_REFERENCE(parameterTypes);
                        }
                        resultObject = vm->reflectFunctions.createConstructorObject(
                            currentMethod, clazz, (j9array_t)parameterTypesObject, currentThread);
                        if (vm->hotSwapCount != preCount) {
                            goto retry;
                        }
                        break;
...
jobject JNICALL Java_java_lang_Class_allocateAndFillArray(JNIEnv* env, jobject recv, jint size)
{
    J9VMThread* currentThread = (J9VMThread*)env;
    J9JavaVM* vm = currentThread->javaVM;
    J9InternalVMFunctions* vmFuncs = vm->internalVMFunctions;
    ...
    if (NULL != arrayClass) {
        ...
        oom:
            ...
            for (U_32 i = 0; i < (U_32)size; ++i) {
                PUSH_OBJECT_IN_SPECIAL_FRAME(currentThread, resultObject);
                j9object_t element
                    = mmFuncs->J9AllocateObject(currentThread, clazz, J9_GC_ALLOCATE_OBJECT_NON_INSTRUMENTABLE);
                resultObject = POP_OBJECT_IN_SPECIAL_FRAME(currentThread);
                if (NULL == element) {
                    goto oom;
                }
...
jobject JNICALL Java_java_lang_Class_getConstructorImpl(
    JNIEnv* env, jobject recv, jobject parameterTypes, jobject signature)
{
    return getConstructorHelper(env, recv, parameterTypes, signature, true);
}

jobject JNICALL Java_java_lang_Class_getConstructorsImpl(JNIEnv* env, jobject recv)
{
    return getConstructorsHelper(env, recv, true);
}

jobject JNICALL Java_java_lang_Class_getDeclaredClassesImpl(JNIEnv* env, jobject recv)
{
    J9VMThread* currentThread = (J9VMThread*)env;
    J9JavaVM* vm = currentThread->javaVM;
    J9InternalVMFunctions* vmFuncs = vm->internalVMFunctions;
    ...
    if (NULL != arrayClass) {
        ...
        if (vm->hotSwapCount != preCount) {
            ...
            for (U_32 i = 0; i < size; ++i) {
                ...
                PUSH_OBJECT_IN_SPECIAL_FRAME(currentThread, resultObject);
                J9Class* innerClazz = vmFuncs->internalFindClassUTF8(currentThread, J9UTF8_DATA(className),
                    J9UTF8_LENGTH(className), classLoader, J9_FINDCLASS_FLAG_THROW_ON_FAIL);
                resultObject = POP_OBJECT_IN_SPECIAL_FRAME(currentThread);
                if (vm->hotSwapCount != preCount) {
                    goto retry;
                }
...
jobject JNICALL Java_java_lang_Class_getDeclaredConstructorImpl(
    JNIEnv* env, jobject recv, jobject parameterTypes, jobject signature)
{
    return getConstructorHelper(env, recv, parameterTypes, signature, false);
}

jobject JNICALL Java_java_lang_Class_getDeclaredConstructorsImpl(JNIEnv* env, jobject recv)
{
    return getConstructorsHelper(env, recv, false);
}

jobject JNICALL Java_java_lang_Class_getDeclaredMethodImpl(
    JNIEnv* env, jobject recv, jobject name, jobject parameterTypes, jobject partialSignature, jobject startingPoint)
{
    J9VMThread* currentThread = (J9VMThread*)env;
    J9JavaVM* vm = currentThread->javaVM;
    J9InternalVMFunctions* vmFuncs = vm->internalVMFunctions;
        ...
        if (!J9ROMCLASS_IS_PRIMITIVE_OR_ARRAY(romClass)) {
            ...
            while (currentMethod != endOfMethods) {
                ...
                if (!isSpecialMethod(romMethod)) {
                    ...
                    J9UTF8* signatureUTF = J9ROMMETHOD_SIGNATURE(romMethod);
                    if (0
                        != vmFuncs->compareStringToUTF8(
                               currentThread, nameObject, FALSE, J9UTF8_DATA(nameUTF), J9UTF8_LENGTH(nameUTF))) {
                        if (0
                            != compareJavaStringToPartialUTF8(currentThread, signatureObject, J9UTF8_DATA(signatureUTF),
                                   J9UTF8_LENGTH(signatureUTF))) {
                            j9object_t parameterTypesObject = NULL;
                            if (NULL != parameterTypes) {
                                parameterTypesObject = J9_JNI_UNWRAP_REFERENCE(parameterTypes);
                            }
                            resultObject = vm->reflectFunctions.createDeclaredMethodObject(
                                currentMethod, clazz, (j9array_t)parameterTypesObject, currentThread);
                            if (vm->hotSwapCount != preCount) {
                                goto retry;
                            }
...
jobject JNICALL Java_java_lang_Class_getDeclaredMethodsImpl(JNIEnv* env, jobject recv)
{
    J9VMThread* currentThread = (J9VMThread*)env;
    J9JavaVM* vm = currentThread->javaVM;
    J9InternalVMFunctions* vmFuncs = vm->internalVMFunctions;
        ...
        if (vm->hotSwapCount != preCount) {
            ...
            while (currentMethod != endOfMethods) {
                ...
                if (!isSpecialMethod(romMethod)) {
                    PUSH_OBJECT_IN_SPECIAL_FRAME(currentThread, resultObject);
                    j9object_t element
                        = vm->reflectFunctions.createDeclaredMethodObject(currentMethod, clazz, NULL, currentThread);
                    resultObject = POP_OBJECT_IN_SPECIAL_FRAME(currentThread);
                    if (vm->hotSwapCount != preCount) {
                        goto retry;
                    }
...
jobject JNICALL Java_java_lang_Class_getDeclaringClassImpl(JNIEnv* env, jobject recv)
{
    J9VMThread* currentThread = (J9VMThread*)env;
    J9JavaVM* vm = currentThread->javaVM;
    J9InternalVMFunctions* vmFuncs = vm->internalVMFunctions;
    ...
    if (NULL != outerClassName) {
        J9Class* outerClass = vmFuncs->internalFindClassUTF8(currentThread, J9UTF8_DATA(outerClassName),
            J9UTF8_LENGTH(outerClassName), clazz->classLoader, J9_FINDCLASS_FLAG_THROW_ON_FAIL);
...
jobject JNICALL Java_java_lang_Class_getEnclosingObject(JNIEnv* env, jobject recv)
{
    J9VMThread* currentThread = (J9VMThread*)env;
    J9JavaVM* vm = currentThread->javaVM;
    J9InternalVMFunctions* vmFuncs = vm->internalVMFunctions;
    ...
    if (!J9ROMCLASS_IS_PRIMITIVE_OR_ARRAY(romClass)) {
        ...
        if (NULL != methodRef) {
            ...
            if (NULL != nas) {
                J9Class* resolvedClass = vmFuncs->resolveClassRef(currentThread, J9_CP_FROM_CLASS(clazz),
                    methodRef->classRefCPIndex, J9_RESOLVE_FLAG_RUNTIME_RESOLVE);
                if (NULL != resolvedClass) {
                    ...
                    J9Method* method = vmFuncs->searchClassForMethod(resolvedClass, J9UTF8_DATA(enclosingMethodNameUTF),
                        J9UTF8_LENGTH(enclosingMethodNameUTF), J9UTF8_DATA(enclosingMethodSigUTF),
                        J9UTF8_LENGTH(enclosingMethodSigUTF));
                    if (NULL != method) {
                        J9ROMMethod* romMethod = J9_ROM_METHOD_FROM_RAM_METHOD(method);
                        if (!isSpecialMethod(romMethod)) {
                            resultObject = vm->reflectFunctions.createDeclaredMethodObject(
                                method, resolvedClass, NULL, currentThread);
                        } else if (isConstructor(romMethod)) {
                            resultObject = vm->reflectFunctions.createDeclaredConstructorObject(
                                method, resolvedClass, NULL, currentThread);
                        }
                    }
                }
...
jobject JNICALL Java_java_lang_Class_getEnclosingObjectClass(JNIEnv* env, jobject recv)
{
    J9VMThread* currentThread = (J9VMThread*)env;
    J9JavaVM* vm = currentThread->javaVM;
    J9InternalVMFunctions* vmFuncs = vm->internalVMFunctions;
    ...
    if (!J9ROMCLASS_IS_PRIMITIVE_OR_ARRAY(romClass)) {
        J9EnclosingObject* methodRef = getEnclosingMethodForROMClass(vm, clazz->classLoader, romClass);
        if (NULL != methodRef) {
            J9Class* resolvedClass = vmFuncs->resolveClassRef(
                currentThread, J9_CP_FROM_CLASS(clazz), methodRef->classRefCPIndex, J9_RESOLVE_FLAG_RUNTIME_RESOLVE);
...
jobject JNICALL Java_java_lang_Class_getFieldImpl(JNIEnv* env, jobject recv, jstring name)
{
    return getFieldHelper(env, recv, name);
}

jobject JNICALL Java_java_lang_Class_getDeclaredFieldImpl(JNIEnv* env, jobject recv, jstring name)
{
    return getDeclaredFieldHelper(env, recv, name);
}

jarray JNICALL Java_java_lang_Class_getFieldsImpl(JNIEnv* env, jobject recv) { return getFieldsHelper(env, recv); }

jobject JNICALL Java_java_lang_Class_getMethodImpl(
    JNIEnv* env, jobject recv, jobject name, jobject parameterTypes, jobject partialSignature)
{
    J9VMThread* currentThread = (J9VMThread*)env;
    J9JavaVM* vm = currentThread->javaVM;
    J9InternalVMFunctions* vmFuncs = vm->internalVMFunctions;
    ...
    if ((NULL == name) || (NULL == partialSignature)) {
        ...
        if (!J9ROMCLASS_IS_PRIMITIVE_OR_ARRAY(romClass)) {
            ...
            j9object_t nameObject = J9_JNI_UNWRAP_REFERENCE(name);
            j9object_t signatureObject = J9_JNI_UNWRAP_REFERENCE(partialSignature);
            UDATA lookupFLags = J9_LOOK_JNI | J9_LOOK_NO_THROW | J9_LOOK_PARTIAL_SIGNATURE
                | (J9ROMCLASS_IS_INTERFACE(romClass) ? (J9_LOOK_INTERFACE | J9_LOOK_NO_JLOBJECT) : 0);
            ...
            nameAndSig.name = vmFuncs->copyStringToUTF8WithMemAlloc(currentThread, nameObject,
                J9_STR_NULL_TERMINATE_RESULT, "", 0, nameBuffer, J9VM_PACKAGE_NAME_BUFFER_LENGTH, &nameBufferLength);
            if (NULL == nameAndSig.name) {
                goto _done;
            }
            nameAndSig.nameLength = (U_32)nameBufferLength;

            nameAndSig.signature = vmFuncs->copyStringToUTF8WithMemAlloc(currentThread, signatureObject,
                J9_STR_NULL_TERMINATE_RESULT | J9_STR_XLAT, "", 0, signatureBuffer, J9VM_PACKAGE_NAME_BUFFER_LENGTH,
                &signatureLength);
            ...
            currentMethod = (J9Method*)vmFuncs->javaLookupMethodImpl(
                currentThread, clazz, ((J9ROMNameAndSignature*)&nameAndSig), NULL, lookupFLags, NULL);
            ...
            Trc_JCL_getMethodImpl_result(currentThread, nameAndSig.nameLength, nameAndSig.name,
                nameAndSig.signatureLength, nameAndSig.signature, currentMethod);
        _done:
            ...
            if (NULL != currentMethod) {
                ...
                if (J9_ARE_ALL_BITS_SET(romMethod->modifiers, J9AccPublic) && !isSpecialMethod(romMethod)) {
                    ...
                    resultObject = vm->reflectFunctions.createMethodObject(
                        currentMethod, clazz, (j9array_t)parameterTypesObject, currentThread);
                }
            }
        }
...
jint JNICALL Java_java_lang_Class_getStaticMethodCountImpl(JNIEnv* env, jobject recv)
{
    J9VMThread* currentThread = (J9VMThread*)env;
    J9JavaVM* vm = currentThread->javaVM;
    J9InternalVMFunctions* vmFuncs = vm->internalVMFunctions;
...
jboolean JNICALL Java_java_lang_Class_getStaticMethodsImpl(
    JNIEnv* env, jobject recv, jobject array, jint start, jint count)
{
    J9VMThread* currentThread = (J9VMThread*)env;
    J9JavaVM* vm = currentThread->javaVM;
    J9InternalVMFunctions* vmFuncs = vm->internalVMFunctions;
...
jint JNICALL Java_java_lang_Class_getVirtualMethodCountImpl(JNIEnv* env, jobject recv)
{
    J9VMThread* currentThread = (J9VMThread*)env;
    J9JavaVM* vm = currentThread->javaVM;
    J9InternalVMFunctions* vmFuncs = vm->internalVMFunctions;
    ...
    for (UDATA index = 0; index < count; ++index) {
        ...
        if (J9_ARE_ANY_BITS_SET(romMethod->modifiers, J9AccPublic)) {
            if (VM_VMHelpers::hasDefaultConflictSendTarget(currentMethod)) {
                 ...
                 * The method pointer has a link to the actual conflicting method.  Return the method that's actually in
                 * the VTable. Class.getMethods() will pick up the alternate implementation when it scans the class's
                 * interfaces.
                 */
...
jboolean JNICALL Java_java_lang_Class_getVirtualMethodsImpl(
    JNIEnv* env, jobject recv, jobject array, jint start, jint count)
{
    J9VMThread* currentThread = (J9VMThread*)env;
    J9JavaVM* vm = currentThread->javaVM;
    ...
    /* First walk the vTable */
    {
        ...
        for (UDATA progress = 0; ((progress < vTableSize) && (numMethodFound < count)); ++progress) {
            ...
            if (J9_ARE_ANY_BITS_SET(romMethod->modifiers, J9AccPublic)) {
                ...
                j9object_t resultObject = J9_JNI_UNWRAP_REFERENCE(array);
                j9object_t methodObject = J9JAVAARRAYOFOBJECT_LOAD(currentThread, resultObject, index);
                vm->reflectFunctions.fillInReflectMethod(
                    methodObject, declaringClass, (jmethodID)methodID, currentThread);
                if (VM_VMHelpers::exceptionPending(currentThread)) {
                    goto done;
                }
                index += 1;
    ...
    J9VMThread* currentThread, J9StackWalkState* walkState, j9object_t acc, j9object_t perm)
{
    ...
    DoPrivilegedMethodArgs* doPrivilegedMethodsArgsTmp
        = (DoPrivilegedMethodArgs*)j9mem_allocate_memory(sizeof(DoPrivilegedMethodArgs), J9MEM_CATEGORY_VM_JCL);
 ...
 * For doPrivileged methods, this finds the callers of each doPrivileged method and the AccessControlContext discovered
 * during stack walking, either from a privilege frame or the contextObject from current thread For
 * doPrivilegedWithCombiner, this finds the caller of doPrivilegedWithCombiner method, and the AccessControlContext
 * 		discovered during stack walking, either from a privilege frame or the contextObject from current thread
 *
 * 	Notes:
 * 		walkState.userData1
 * 			initial value is STACK_WALK_STATE_MAGIC((void*)1), set to NULL when a limited doPrivileged frame is
 * discovered walkState.userData2 initial value is contextObject from current thread, set to DoPrivilegedMethodArgs*
 * when a limited doPrivileged frame is discovered walkState.userData3 initial value is
 * STACK_WALK_STATE_MAGIC((void*)1), set to STACK_WALK_STATE_LIMITED_DOPRIVILEGED((void*)2) for searching the caller of
 * a limited doPrivileged method, and reset to STACK_WALK_STATE_MAGIC after identified the caller of that limited
 * doPrivileged method set to STACK_WALK_STATE_FULL_DOPRIVILEGED ((void*)3) for searching the caller of a full
 * privileged doPrivileged method, there is no need for resetting cause PrivilegedFrameIterator method exits with
 * J9_STACKWALK_STOP_ITERATING walkState.userData4 (only for doPrivilegedWithCombiner) initial value is NULL, set to
 * walkState->framesWalked when found the non reflection/MethodHandleInvoke frame caller of doPrivilegedWithCombiner
 *
...
static UDATA isPrivilegedFrameIteratorGetAccSnapshot(J9VMThread* currentThread, J9StackWalkState* walkState)
{
    ...
    J9JNIMethodID* doPrivilegedWithContextMethodID1 = (J9JNIMethodID*)vm->doPrivilegedWithContextMethodID1;
    J9JNIMethodID* doPrivilegedWithContextMethodID2 = (J9JNIMethodID*)vm->doPrivilegedWithContextMethodID2;
    J9JNIMethodID* doPrivilegedWithContextPermissionMethodID1
        = (J9JNIMethodID*)vm->doPrivilegedWithContextPermissionMethodID1;
    J9JNIMethodID* doPrivilegedWithContextPermissionMethodID2
        = (J9JNIMethodID*)vm->doPrivilegedWithContextPermissionMethodID2;
    J9Method* currentMethod = walkState->method;
    ...
    if ((NULL == walkState->userData4) || (STACK_WALK_STATE_LIMITED_DOPRIVILEGED == walkState->userData3)
    ...
    if (((doPrivilegedWithContextPermissionMethodID1 != NULL)
            && (currentMethod == doPrivilegedWithContextPermissionMethodID1->method))
        || ((doPrivilegedWithContextPermissionMethodID2 != NULL)
               && (currentMethod == doPrivilegedWithContextPermissionMethodID2->method))) {
        ...
        if (NULL != walkState->userData1) {
            ...
            return frameIteratorGetAccSnapshotHelper(
                currentThread, walkState, (j9object_t)walkState->arg0EA[-1], (j9object_t)walkState->arg0EA[-2]);
        }
    }
 ...
 *
 * Pre-JEP140 format: AccessControlContext/ProtectionDomain..., and the length of the object array is NOT divisible by
 *OBJS_ARRAY_SIZE First element is an AccessControlContext object which might be null, either from a full permission
 *privileged frame or from current thread if there is no such privileged frames ProtectionDomain elements after
 *AccessControlContext object could be in one of following two formats: For doPrivileged methods - flag
 *forDoPrivilegedWithCombiner is false: the ProtectionDomain element might be null, first ProtectionDomain element is a
 *duplicate of the ProtectionDomain of the caller of doPrivileged rest of ProtectionDomain elements are from the callers
 *discovered during stack walking the start index of the actual ProtectionDomain element is 2 of the object array
 *returned For doPrivilegedWithCombiner methods - flag forDoPrivilegedWithCombiner is true: there are only two
 *ProtectionDomain elements, first one is the ProtectionDomain of the caller of doPrivileged and the other is the
 *ProtectionDomain of the caller of doPrivilegedWithCombiner the fourth element of the object array is NULL for padding
 *to ensure that the length of the object array is NOT divisible by OBJS_ARRAY_SIZE either
 *
 ...
 * 				an array of ProtectionDomain objects in which first ProtectionDomain element is a duplicate
 *of the ProtectionDomain of the caller of doPrivileged the start index of the actual ProtectionDomain element is 1 of
 *this ProtectionDomain objects array For doPrivilegedWithCombiner methods - flag forDoPrivilegedWithCombiner is true:
 ...
 * 			ProtectionDomain objects array with same format above when flag forDoPrivilegedWithCombiner is
 *false or just the ProtectionDomain of the caller of doPrivileged in case of flag forDoPrivilegedWithCombiner is true
 ...
 *			This helped to address performance issue raised by JAZZ 66091: Perf work for LIR 28261 (Limited
 *doPrivileged / JEP 140)
 *		 2. The reason to duplicate the ProtectionDomain object of the caller of doPrivileged is to avoid creating
 *a new object array without NULL and duplicate ProtectionDomain objects discovered during stack walking while still
 *keeping same order of those objects.
 *
    ...
    JNIEnv* env, jclass jsAccessController, jint startingFrame, jboolean forDoPrivilegedWithCombiner)
{
    ...
    walkState.userData2 = contextObject; /* initially set to contextObject, set to DoPrivilegedMethodArgs* when a
                                            limited doPrivileged frame is discovered */
    walkState.userData3
        = STACK_WALK_STATE_MAGIC; /* set to NULL when to find the caller of a limited doPrivileged method */
    walkState.userData4 = NULL; /* when forDoPrivilegedWithCombiner is true, set to walkState->framesWalked of non
                                   reflection/MethodHandleInvoke frame caller of doPrivilegedWithCombiner */
    ...
    if (STACK_WALK_STATE_MAGIC == walkState.userData1) {
        ...
        if (forDoPrivilegedWithCombiner) {
            ...
            arrayObject = vmThread->javaVM->memoryManagerFunctions->J9AllocateIndexableObject(
                vmThread, arrayClass, 4, J9_GC_ALLOCATE_OBJECT_NON_INSTRUMENTABLE);
            ...
            J9JAVAARRAYOFOBJECT_STORE(vmThread, arrayObject, 1,
                J9VMJAVALANGCLASS_PROTECTIONDOMAIN(vmThread, J9VM_J9CLASS_TO_HEAPCLASS(J9_CLASS_FROM_CP(*cachePtr))));
            cachePtr = walkState.cache + (framesWalked - 1);
            J9JAVAARRAYOFOBJECT_STORE(vmThread, arrayObject, 2,
                J9VMJAVALANGCLASS_PROTECTIONDOMAIN(vmThread, J9VM_J9CLASS_TO_HEAPCLASS(J9_CLASS_FROM_CP(*cachePtr))));
        } else {
            /* contextObject in first slot, duplicate of the ProtectionDomain of the caller of doPrivileged in second
             * slot (add 2 to size before allocation). */
            U_32 arraySize = (U_32)(walkState.framesWalked + 2);
            if (0 == (arraySize % OBJS_ARRAY_SIZE)) {
                arraySize++; /* increase one more slot to make sure the size of arrayObject is NOT divisible by
                                OBJS_ARRAY_SIZE */
            }
            arrayObject = storePDobjectsHelper(vmThread, arrayClass, &walkState, (j9object_t)walkState.userData2,
                arraySize, walkState.framesWalked, 2, TRUE);
            if (NULL == arrayObject) {
                goto _throwException;
            }
        }
        ...
        UDATA nbrPDblockToBuild = 1; /* the number of ProtectionDomain block walked, minimum 1 when no target method
                                        walked, i.e., walked whole stack */
        UDATA counter = 0;
        ...
        /* Array structure: AccessControlContext (could be null), ProtectionDomain object array, Permission object array
         * (null means a normal method) */
        ...
        if (NULL != dpMethodsArgsTmp->permissions) {
            ...
            DoPrivilegedMethodArgs* doPrivilegedMethodsArgsTmp2
                = (DoPrivilegedMethodArgs*)j9mem_allocate_memory(sizeof(DoPrivilegedMethodArgs), J9MEM_CATEGORY_VM_JCL);
        ...
        arrayObject = vm->memoryManagerFunctions->J9AllocateIndexableObject(
            vmThread, arrayClass, (U_32)nbrPDblockToBuild * OBJS_ARRAY_SIZE, J9_GC_ALLOCATE_OBJECT_NON_INSTRUMENTABLE);
        if (NULL == arrayObject) {
        ...
        for (signedCounter = (counter - 1); signedCounter >= 0; signedCounter--) {
            ...
            J9JAVAARRAYOFOBJECT_STORE(vmThread, arrayObject,
                (signedCounter * OBJS_ARRAY_SIZE + OBJS_ARRAY_IDX_PERMS_OR_CACHECHECKED), permissions);
        }
        ...
        if (forDoPrivilegedWithCombiner) {
             ...
             * 	rest of arrayObject[signedCounter*OBJS_ARRAY_SIZE + OBJS_ARRAY_IDX_PDS] are a single ProtectionDomain
             * object of the callers of each limited/full doPrivileged methods
             * */
            signedCounter = 0;
            dpMethodsArgsTmp = dpMethodsArgs;
            do {
                ...
                if (0 == signedCounter) {
                    ...
                    pdArrayTmp = vm->memoryManagerFunctions->J9AllocateIndexableObject(
                        vmThread, arrayClass, 2, J9_GC_ALLOCATE_OBJECT_NON_INSTRUMENTABLE);
                    ...
                    J9JAVAARRAYOFOBJECT_STORE(vmThread, pdArrayTmp, 0,
                        J9VMJAVALANGCLASS_PROTECTIONDOMAIN(
                            vmThread, J9VM_J9CLASS_TO_HEAPCLASS(J9_CLASS_FROM_CP(*cachePtr))));
                    cachePtr = walkState.cache + (framesWalked - 1);
                    J9JAVAARRAYOFOBJECT_STORE(vmThread, pdArrayTmp, 1,
                        J9VMJAVALANGCLASS_PROTECTIONDOMAIN(
                            vmThread, J9VM_J9CLASS_TO_HEAPCLASS(J9_CLASS_FROM_CP(*cachePtr))));
                    J9JAVAARRAYOFOBJECT_STORE(
                        vmThread, arrayObject, (signedCounter * OBJS_ARRAY_SIZE + OBJS_ARRAY_IDX_PDS), pdArrayTmp);
                } else {
                    cachePtr = walkState.cache + (dpMethodsArgsTmp->frameCounter - 1);
                    J9JAVAARRAYOFOBJECT_STORE(vmThread, arrayObject,
                        (signedCounter * OBJS_ARRAY_SIZE + OBJS_ARRAY_IDX_PDS),
                        J9VMJAVALANGCLASS_PROTECTIONDOMAIN(
                            vmThread, J9VM_J9CLASS_TO_HEAPCLASS(J9_CLASS_FROM_CP(*cachePtr))));
                }
                signedCounter++;
            ...
            do {
                 ...
                 * rest elements: the ProtectionDomains of the callers discovered during stack walking including the
                 * caller of doPrivileged The reason to duplicate the ProtectionDomain of the caller of doPrivileged is
                 * to keep current structure and performance
                 * */
                PUSH_OBJECT_IN_SPECIAL_FRAME(vmThread, (j9object_t)arrayObject);
                pdArrayTmp = vm->memoryManagerFunctions->J9AllocateIndexableObject(vmThread, arrayClass,
                    (U_32)(dpMethodsArgsTmp->frameCounter - lastFrameCounter + 1),
                ...
                do {
                    pd = J9VMJAVALANGCLASS_PROTECTIONDOMAIN(
                        vmThread, J9VM_J9CLASS_TO_HEAPCLASS(J9_CLASS_FROM_CP(*cachePtr)));
                    cachePtr += 1;
                    if ((NULL != pd) && (pd != lastPD)) {
                        ...
                        while (sc < signedCounter) {
                            ...
                            UDATA j = 0;
                            j9object_t pdTmp = J9JAVAARRAYOFOBJECT_LOAD(
                                vmThread, arrayObject, (sc * OBJS_ARRAY_SIZE + OBJS_ARRAY_IDX_PDS));
 ...
 * @return J9_STACKWALK_STOP_ITERATING, J9_STACKWALK_KEEP_ITERATING
 */
static UDATA isPrivilegedFrameIteratorGetCallerPD(J9VMThread* currentThread, J9StackWalkState* walkState)
{
    if (J9_ARE_ALL_BITS_SET(
            J9_ROM_METHOD_FROM_RAM_METHOD(walkState->method)->modifiers, J9_JAVA_METHOD_FRAME_ITERATOR_SKIP)) {
        /* Skip methods with java.lang.invoke.FrameIteratorSkip annotation */
        return J9_STACKWALK_KEEP_ITERATING;
    }
    ...
    if ((walkState->method == vm->jlrMethodInvoke) || (walkState->method == vm->jliMethodHandleInvokeWithArgs)
        ...
        || (vm->srMethodAccessor
               && VM_VMHelpers::isSameOrSuperclass(
                      J9VM_J9CLASS_FROM_JCLASS(currentThread, vm->srMethodAccessor), currentClass))
        /* there is no need to check srConstructorAccessor because doPrivilegedXX are method calls and not affected by
...
/**
 * This native retrieves the ProtectionDomain object of the non-reflection/MethodHandleInvoke caller as per the
 startingFrame specified.
    ...
    j9object_t contextObject, U_32 arraySize, UDATA framesWalked, I_32 startPos, BOOLEAN dupCallerPD)
{
    ...
    PUSH_OBJECT_IN_SPECIAL_FRAME(vmThread, contextObject);
    arrayObject = vmThread->javaVM->memoryManagerFunctions->J9AllocateIndexableObject(
        vmThread, arrayClass, arraySize, J9_GC_ALLOCATE_OBJECT_NON_INSTRUMENTABLE);
    contextObject = POP_OBJECT_IN_SPECIAL_FRAME(vmThread);
    if (NULL == arrayObject) {
        vmThread->javaVM->internalVMFunctions->setHeapOutOfMemoryError(vmThread);
        return NULL;
...
jobject JNICALL Java_java_lang_Class_getNestMembersImpl(JNIEnv* env, jobject recv)
{
    ...
    resultObject = mmFuncs->J9AllocateIndexableObject(
        currentThread, arrayClass, 1 + nestMemberCount, J9_GC_ALLOCATE_OBJECT_NON_INSTRUMENTABLE);
    if (NULL == resultObject) {
        vmFuncs->setHeapOutOfMemoryError(currentThread);
        goto _done;
    }
    ...
    if (0 != nestMemberCount) {
        ...
        for (i = 0; i < nestMemberCount; i++) {
            ...
            PUSH_OBJECT_IN_SPECIAL_FRAME(currentThread, resultObject);
            nestMember = vmFuncs->internalFindClassUTF8(currentThread, J9UTF8_DATA(nestMemberName),
                J9UTF8_LENGTH(nestMemberName), classLoader, J9_FINDCLASS_FLAG_THROW_ON_FAIL);
            resultObject = POP_OBJECT_IN_SPECIAL_FRAME(currentThread);
            ...
            if (nestMember->nestHost != nestHost) {
                vmFuncs->setNestmatesError(
                    currentThread, nestMember, nestHost, J9_VISIBILITY_NEST_MEMBER_NOT_CLAIMED_ERROR);
                goto _done;
            }
