...
static UDATA stackFrameFilter(J9VMThread* currentThread, J9StackWalkState* walkState)
{
    ...
    if (NULL != walkState->userData2) { /* look for stackWalkerMethod */
        ...
        if (compareUTF8Length(J9UTF8_DATA(utf), J9UTF8_LENGTH(utf), (void*)stackWalkerMethod, strlen(stackWalkerMethod))
            == 0) {
            utf = J9ROMCLASS_CLASSNAME(romClass);
            if (J9UTF8_LITERAL_EQUALS(J9UTF8_DATA(utf), J9UTF8_LENGTH(utf), "java/lang/StackWalker")) {
                walkState->userData2
                    = NULL; /* Iteration will skip hidden frames and stop at the true caller of stackWalkerMethod. */
            }
        }
        ...
        result = J9_STACKWALK_KEEP_ITERATING;
    } else if (J9_ARE_NO_BITS_SET((UDATA)(walkState->userData1), SHOW_HIDDEN_FRAMES)
        && VM_VMHelpers::isHiddenMethod(walkState->method)) {
        result = J9_STACKWALK_KEEP_ITERATING;
    } else {
...
jobject JNICALL Java_java_lang_StackWalker_walkWrapperImpl(
    JNIEnv* env, jclass clazz, jint flags, jstring stackWalkerMethod, jobject function)
{
    ...
    walkState->flags = J9_STACKWALK_ITERATE_FRAMES | J9_STACKWALK_WALK_TRANSLATE_PC | J9_STACKWALK_INCLUDE_NATIVES
...
static j9object_t utfToStringObject(JNIEnv* env, J9UTF8* utf, UDATA flags)
{
    j9object_t result = NULL;
    if (NULL != utf) {
        J9VMThread* vmThread = (J9VMThread*)env;
        J9JavaVM* vm = vmThread->javaVM;
        result = vm->memoryManagerFunctions->j9gc_createJavaLangString(
            vmThread, J9UTF8_DATA(utf), (U_32)J9UTF8_LENGTH(utf), flags);
    }
    return result;
}

jobject JNICALL Java_java_lang_StackWalker_getImpl(JNIEnv* env, jobject clazz, jlong walkStateP)
{
    J9VMThread* vmThread = (J9VMThread*)env;
    J9JavaVM* vm = vmThread->javaVM;
    ...
    if (J9SF_FRAME_TYPE_END_OF_STACK != walkState->pc) {
        ...
        j9object_t frame = vm->memoryManagerFunctions->J9AllocateObject(
            vmThread, frameClass, J9_GC_ALLOCATE_OBJECT_NON_INSTRUMENTABLE);
        if (NULL == frame) {
            ...
            UDATA lineNumber
                = getLineNumberForROMClassFromROMMethod(vm, romMethod, romClass, 0, classLoader, bytecodeOffset);
            ...
            /* Fill in line number - Java wants -2 for natives, -1 for no line number (which will be 0 coming in from
             * the iterator) */
            ...
            J9VMJAVALANGSTACKWALKERSTACKFRAMEIMPL_SET_CLASSNAME(
                vmThread, PEEK_OBJECT_IN_SPECIAL_FRAME(vmThread, 0), stringObject);

            stringObject = utfToStringObject(env, J9ROMMETHOD_GET_NAME(romClass, romMethod), J9_STR_INTERN);
            if (VM_VMHelpers::exceptionPending(vmThread)) {
                goto _pop_frame;
            }
            J9VMJAVALANGSTACKWALKERSTACKFRAMEIMPL_SET_METHODNAME(
                vmThread, PEEK_OBJECT_IN_SPECIAL_FRAME(vmThread, 0), stringObject);
            ...
            J9VMJAVALANGSTACKWALKERSTACKFRAMEIMPL_SET_METHODSIGNATURE(
                vmThread, PEEK_OBJECT_IN_SPECIAL_FRAME(vmThread, 0), stringObject);

            stringObject
                = utfToStringObject(env, getSourceFileNameForROMClass(vm, classLoader, romClass), J9_STR_INTERN);
            if (VM_VMHelpers::exceptionPending(vmThread)) {
                goto _pop_frame;
            }
            J9VMJAVALANGSTACKWALKERSTACKFRAMEIMPL_SET_FILENAME(
                vmThread, PEEK_OBJECT_IN_SPECIAL_FRAME(vmThread, 0), stringObject);

            if (J9ROMMETHOD_IS_CALLER_SENSITIVE(romMethod)) {
                J9VMJAVALANGSTACKWALKERSTACKFRAMEIMPL_SET_CALLERSENSITIVE(
                    vmThread, PEEK_OBJECT_IN_SPECIAL_FRAME(vmThread, 0), TRUE);
            }
