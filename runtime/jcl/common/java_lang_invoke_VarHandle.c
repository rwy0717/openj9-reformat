...
static BOOLEAN accessCheckFieldType(J9VMThread* currentThread, J9Class* lookupClass, J9Class* type, J9UTF8* lookupSig)
{
    ...
    if (NULL != verifyData) {
        ...
        if ('L' == *lookupSigData) {
            ...
            if (typeClassloader != lookupClassloader) {
                ...
                if (verifyData->checkClassLoadingConstraintForNameFunction(currentThread, lookupClassloader,
                        typeClassloader, &lookupSigData[1], &lookupSigData[1], J9UTF8_LENGTH(lookupSig) - 2, TRUE)
                    != 0) {
                    result = FALSE;
                }
    ...
    jstring name, jstring signature, jclass type, jboolean isStatic, jclass accessClass)
{
    ...
    J9VMJAVALANGINVOKEVARHANDLE_SET_MODIFIERS(
