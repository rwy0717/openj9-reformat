...
#define VENDOR_INFO ""
    ...
    if (BUFFER_SIZE <= j9str_printf(PORTLIB, vminfo, BUFFER_SIZE + 1,
                           ...
                           j2se_version_info, (NULL != osname ? osname : " "), osarch, EsBuildVersionString, jitEnabled,
                           aotEnabled)) {
...
static jint initializeStaticMethod(J9JavaVM* vm, UDATA offset)
{
    ...
    if ((J9CPTYPE_STATIC_METHOD == cpType) || (J9CPTYPE_INTERFACE_STATIC_METHOD == cpType)) {
        if (NULL
            == vm->internalVMFunctions->resolveStaticMethodRef(vm->mainThread, jclConstantPool, offset,
                   J9_RESOLVE_FLAG_NO_THROW_ON_FAIL | J9_RESOLVE_FLAG_JCL_CONSTANT_POOL)) {
            if (NULL == J9VMCONSTANTPOOL_CLASSREF_AT(vm, romMethodConstantPool[offset].classRefCPIndex)->value) {
                Trc_JCL_initializeKnownClasses_ClassRefNotResolvedForMethodRef(
                    vm->mainThread, romMethodConstantPool[offset].classRefCPIndex, offset);
            } else {
                Trc_JCL_initializeKnownClasses_ExitError(vm->mainThread, offset);
                return JNI_ERR;
            }
        } else {
            Trc_JCL_initializeKnownClasses_ResolvedStaticMethodRef(
                vm->mainThread, offset, staticMethodConstantPool[offset].method);
        }
    }
...
static jint initializeStaticField(J9JavaVM* vm, UDATA offset, UDATA resolveFlags)
{
    J9ConstantPool* jclConstantPool = (J9ConstantPool*)vm->jclConstantPool;
    ...
    if (J9CPTYPE_FIELD == J9_CP_TYPE(cpShapeDescription, offset)) {
        if (NULL
            == vm->internalVMFunctions->resolveStaticFieldRef(
                   vm->mainThread, NULL, jclConstantPool, offset, resolveFlags, NULL)) {
            if (NULL == J9VMCONSTANTPOOL_CLASSREF_AT(vm, romFieldConstantPool[offset].classRefCPIndex)->value) {
                Trc_JCL_initializeKnownClasses_ClassRefNotResolvedForStaticFieldRef(
                    vm->mainThread, romFieldConstantPool[offset].classRefCPIndex, offset);
            } else {
                Trc_JCL_initializeKnownClasses_ExitError(vm->mainThread, offset);
                return JNI_ERR;
            }
        } else {
            Trc_JCL_initializeKnownClasses_ResolvedStaticFieldRef(
                vm->mainThread, offset, J9RAMSTATICFIELDREF_VALUEADDRESS(staticFieldConstantPool + offset));
        }
        return JNI_OK;
    }
    return JNI_EINVAL;
    ...
    for (i = 0; i < constPoolCount; i++) {
        if (J9CPTYPE_FIELD == J9_CP_TYPE(cpShapeDescription, i)) {
            ...
            if (0 == (romClassRef->runtimeFlags & runtimeFlags)) {
                Trc_JCL_initializeKnownClasses_SkippingResolve(
                    vm->mainThread, i, romClassRef, romClassRef->runtimeFlags, runtimeFlags);
            } else {
                /* Try resolving as a static fieldref, then as an instance fieldref. */
                if (NULL
                    != vmFuncs->resolveStaticFieldRef(
                           vm->mainThread, NULL, jclConstantPool, i, J9_RESOLVE_FLAG_NO_THROW_ON_FAIL, NULL)) {
                    Trc_JCL_initializeKnownClasses_ResolvedStaticFieldRef(
                        vm->mainThread, i, J9RAMSTATICFIELDREF_VALUEADDRESS(staticFieldConstantPool + i));
                } else if (-1
                    != vmFuncs->resolveInstanceFieldRef(
                           vm->mainThread, NULL, jclConstantPool, i, J9_RESOLVE_FLAG_NO_THROW_ON_FAIL, NULL)) {
                    Trc_JCL_initializeKnownClasses_ResolvedInstanceFieldRef(
                        vm->mainThread, i, instanceFieldConstantPool[i].valueOffset);
                     ...
                     */
                    Trc_JCL_initializeKnownClasses_ClassRefNotResolvedForInstanceFieldRef(
                        vm->mainThread, romFieldConstantPool[i].classRefCPIndex, i);
                } else {
            ...
            if (0 == (romClassRef->runtimeFlags & runtimeFlags)) {
                ...
                if (0
                    != vmFuncs->resolveVirtualMethodRef(vm->mainThread, jclConstantPool, i,
                           J9_RESOLVE_FLAG_NO_THROW_ON_FAIL | J9_RESOLVE_FLAG_JCL_CONSTANT_POOL, &resolvedMethod)) {
                    Trc_JCL_initializeKnownClasses_ResolvedVirtualMethodRef(
                        vm->mainThread, i, virtualMethodConstantPool[i].methodIndexAndArgCount);
                    resolved = TRUE;
                }
                if (NULL
                    != vmFuncs->resolveSpecialMethodRef(vm->mainThread, jclConstantPool, i,
                           J9_RESOLVE_FLAG_NO_THROW_ON_FAIL | J9_RESOLVE_FLAG_JCL_CONSTANT_POOL)) {
                    Trc_JCL_initializeKnownClasses_ResolvedSpecialMethodRef(
                        vm->mainThread, i, specialMethodConstantPool[i].method);
                    resolved = TRUE;
                }
                if (!resolved) {
                    if (NULL == J9VMCONSTANTPOOL_CLASSREF_AT(vm, romMethodConstantPool[i].classRefCPIndex)->value) {
                        Trc_JCL_initializeKnownClasses_ClassRefNotResolvedForMethodRef(
                            vm->mainThread, romMethodConstantPool[i].classRefCPIndex, i);
                    } else {
                        Trc_JCL_initializeKnownClasses_ExitError(vm->mainThread, i);
                        return JNI_ERR;
                    }
            ...
            if (0 == (romClassRef->runtimeFlags & runtimeFlags)) {
                ...
                if (NULL
                    == vmFuncs->resolveStaticMethodRef(vm->mainThread, jclConstantPool, i,
                           J9_RESOLVE_FLAG_NO_THROW_ON_FAIL | J9_RESOLVE_FLAG_JCL_CONSTANT_POOL)) {
                    if (NULL == J9VMCONSTANTPOOL_CLASSREF_AT(vm, romMethodConstantPool[i].classRefCPIndex)->value) {
                        Trc_JCL_initializeKnownClasses_ClassRefNotResolvedForMethodRef(
                            vm->mainThread, romMethodConstantPool[i].classRefCPIndex, i);
                    ...
                    Trc_JCL_initializeKnownClasses_ResolvedStaticMethodRef(
                        vm->mainThread, i, staticMethodConstantPool[i].method);
                }
            }
            ...
            if (0 == (romClassRef->runtimeFlags & runtimeFlags)) {
                ...
                if (NULL
                    == vmFuncs->resolveInterfaceMethodRef(vm->mainThread, jclConstantPool, i,
                           J9_RESOLVE_FLAG_NO_THROW_ON_FAIL | J9_RESOLVE_FLAG_JCL_CONSTANT_POOL)) {
                    if (NULL == J9VMCONSTANTPOOL_CLASSREF_AT(vm, romMethodConstantPool[i].classRefCPIndex)->value) {
                        Trc_JCL_initializeKnownClasses_ClassRefNotResolvedForMethodRef(
                            vm->mainThread, romMethodConstantPool[i].classRefCPIndex, i);
                    ...
                    Trc_JCL_initializeKnownClasses_ResolvedInterfaceMethodRef(
                        vm->mainThread, i, interfaceMethodConstantPool[i].methodIndexAndArgCount);
                }
            }
        }
    }
...
static UDATA doJCLCheck(J9JavaVM* vm, J9Class* j9VMInternalsClass)
{
    ...
    vmFuncs->staticFieldAddress(vmThread, j9VMInternalsClass, (U_8*)"j9Config", sizeof("j9Config") - 1, (U_8*)"J", 1,
        NULL, (UDATA*)&jclField, J9_RESOLVE_FLAG_NO_THROW_ON_FAIL, NULL);
    if (jclField != NULL) {
        jclConfigPtr = (U_8*)&jclField->initialValue;
        /* get the jcl version from the class library (i.e. java.lang.J9VMInternals) */
        vmFuncs->staticFieldAddress(vmThread, j9VMInternalsClass, (U_8*)"j9Version", sizeof("j9Version") - 1, (U_8*)"I",
            1, NULL, (UDATA*)&jclField, J9_RESOLVE_FLAG_NO_THROW_ON_FAIL, NULL);
        if (jclField != NULL) {
 ...
 */
static jint initializeStaticIntField(J9VMThread* vmThread, J9Class* clazz, UDATA vmCPIndex, I_32 value)
{
...
static const J9IntConstantMapping intVMConstants[] = {
    ...
    { J9VMCONSTANTPOOL_COMIBMOTIVMVM_J9_GC_WRITE_BARRIER_TYPE_CARDMARK_INCREMENTAL,
        J9_GC_WRITE_BARRIER_TYPE_CARDMARK_INCREMENTAL },
    { J9VMCONSTANTPOOL_COMIBMOTIVMVM_J9_GC_WRITE_BARRIER_TYPE_CARDMARK_AND_OLDCHECK,
        J9_GC_WRITE_BARRIER_TYPE_CARDMARK_AND_OLDCHECK },
 ...
 * @return a JNI result code
 */
static jint intializeVMConstants(J9VMThread* currentThread)
{
    ...
    J9Class* vmClass = vm->internalVMFunctions->internalFindKnownClass(
        currentThread, J9VMCONSTANTPOOL_COMIBMOTIVMVM, J9_FINDKNOWNCLASS_FLAG_NON_FATAL);
    ...
    rc = initializeStaticIntField(
        currentThread, vmClass, J9VMCONSTANTPOOL_COMIBMOTIVMVM_J9_GC_WRITE_BARRIER_TYPE, (I_32)vm->gcWriteBarrierType);
    if (JNI_OK != rc) {
        goto done;
    }
    rc = initializeStaticIntField(
        currentThread, vmClass, J9VMCONSTANTPOOL_COMIBMOTIVMVM_J9_GC_ALLOCATION_TYPE, (I_32)vm->gcAllocationType);
    if (JNI_OK != rc) {
        goto done;
    }
    rc = initializeStaticIntField(
        currentThread, vmClass, J9VMCONSTANTPOOL_COMIBMOTIVMVM_J9_GC_POLICY, (I_32)omrVM->gcPolicy);
    if (JNI_OK != rc) {
        goto done;
    }

    rc = initializeStaticIntField(currentThread, vmClass, J9VMCONSTANTPOOL_COMIBMOTIVMVM_J9_JIT_STRING_DEDUP_POLICY,
        vm->memoryManagerFunctions->j9gc_get_jit_string_dedup_policy(vm));
    if (JNI_OK != rc) {
        goto done;
    }

    rc = initializeStaticIntField(currentThread, vmClass, J9VMCONSTANTPOOL_COMIBMOTIVMVM_J9_STRING_COMPRESSION_ENABLED,
        (I_32)IS_STRING_COMPRESSION_ENABLED_VM(vm));
    if (JNI_OK != rc) {
        goto done;
    }
...
initializeRequiredClasses(J9VMThread* vmThread, char* dllName)
{
    ...
    if (J2SE_SHAPE(vm) < J2SE_SHAPE_B165) {
        if (0
            != vmFuncs->addHiddenInstanceField(
                   vm, "java/lang/reflect/Module", "modulePointer", "J", &vm->modulePointerOffset)) {
            return 1;
        }
    } else {
        if (0
            != vmFuncs->addHiddenInstanceField(
                   vm, "java/lang/Module", "modulePointer", "J", &vm->modulePointerOffset)) {
            return 1;
        }
    }
    ...
    objectClass
        = vmFuncs->internalFindKnownClass(vmThread, J9VMCONSTANTPOOL_JAVALANGOBJECT, J9_FINDKNOWNCLASS_FLAG_NON_FATAL);
    if (NULL == objectClass) {
        return 1;
    }
    vmInternalsClass = vmFuncs->internalFindKnownClass(
        vmThread, J9VMCONSTANTPOOL_JAVALANGJ9VMINTERNALS, J9_FINDKNOWNCLASS_FLAG_NON_FATAL);
    ...
    lockClass = vmFuncs->internalFindKnownClass(
        vmThread, J9VMCONSTANTPOOL_JAVALANGJ9VMINTERNALSCLASSINITIALIZATIONLOCK, J9_FINDKNOWNCLASS_FLAG_NON_FATAL);
    ...
    classClass
        = vmFuncs->internalFindKnownClass(vmThread, J9VMCONSTANTPOOL_JAVALANGCLASS, J9_FINDKNOWNCLASS_FLAG_NON_FATAL);
    ...
    do {
        j9object_t classObj = gcFuncs->J9AllocateObject(vmThread, classClass,
     ...
     */
    stringClass
        = vmFuncs->internalFindKnownClass(vmThread, J9VMCONSTANTPOOL_JAVALANGSTRING, J9_FINDKNOWNCLASS_FLAG_NON_FATAL);
     ...
     * in StringTable.cpp
     */
    if (initializeStaticField(vm, J9VMCONSTANTPOOL_JAVALANGSTRING_COMPRESSIONFLAG,
            J9_RESOLVE_FLAG_NO_THROW_ON_FAIL | J9_RESOLVE_FLAG_NO_CLASS_INIT)) {
        return 1;
    }
