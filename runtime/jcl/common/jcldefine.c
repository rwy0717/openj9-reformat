...
jclass defineClassCommon(JNIEnv* env, jobject classLoaderObject, jstring className, jbyteArray classRep, jint offset,
    jint length, jobject protectionDomain, UDATA options, J9Class* hostClass)
{
...
#define MAX_RETRY_COUNT 2
    ...
    if (isContiguousClassBytes) {
        ...
        if ((offset < 0) || (length < 0)
            || (((U_32)offset + (U_32)length)
                   > J9INDEXABLEOBJECT_SIZE(currentThread, *(J9IndexableObject**)classRep))) {
...
retry:
    ...
    if (isContiguousClassBytes) {
        ...
        /* TMP_J9JAVACONTIGUOUSARRAYOFBYTE_EA returns I_8 * (since Java bytes are I_8) so this cast will silence the
         * warning */
    ...
    /* Try to find classLocation. Ignore return code because there are valid cases where it might not find it (ie.
     * bytecode spinning). If the class is not found the default class location is fine.
     */
    dynFuncs->findLocallyDefinedClassFunction(currentThread, NULL, utf8Name, (U_32)utf8Length, classLoader,
        classLoader->classPathEntries, classLoader->classPathEntryCount, (UDATA)FALSE, &localBuffer);
    ...
    if (J9_ARE_NO_BITS_SET(options, J9_FINDCLASS_FLAG_ANON)) {
        ...
        if (NULL != loadedClass) {
            ...
            if ((NULL == vm->sharedClassConfig) || (0 == vm->sharedClassConfig->isBCIEnabled(vm))) {
                clazz = vmFuncs->internalCreateRAMClassFromROMClass(currentThread, classLoader, loadedClass, 0, NULL,
                    protectionDomain ? *(j9object_t*)protectionDomain : NULL, NULL, J9_CP_INDEX_NONE,
                    localBuffer.loadLocationType, NULL, hostClass);
