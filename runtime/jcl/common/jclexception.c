...
static UDATA getStackTraceIterator(J9VMThread* vmThread, void* voidUserData, J9ROMClass* romClass,
 ...
 */
static BOOLEAN setStackTraceElementSource(
    J9VMThread* vmThread, j9object_t stackTraceElement, J9ClassLoader* classLoader, J9ROMClass* romClass)
{
    ...
    J9Class* clazz = vmfns->internalFindClassUTF8(
        vmThread, J9UTF8_DATA(name), J9UTF8_LENGTH(name), classLoader, J9_FINDCLASS_FLAG_EXISTING_ONLY);
    if (NULL == clazz) {
        return FALSE;
    }
    ...
    if (element == NULL) {
        ...
        if (romMethod != NULL) {
            ...
            if (j2seVersion >= J2SE_19) {
                ...
                UDATA length = 0;
                j9object_t classLoaderName
                    = J9VMJAVALANGCLASSLOADER_CLASSLOADERNAME(vmThread, classLoader->classLoaderObject);
                ...
                if (J9_ARE_NO_BITS_SET(vm->runtimeFlags, J9_RUNTIME_JAVA_BASE_MODULE_CREATED)) {
                    string = vm->memoryManagerFunctions->j9gc_createJavaLangString(
                        vmThread, (U_8*)JAVA_BASE_MODULE, LITERAL_STRLEN(JAVA_BASE_MODULE), J9_STR_XLAT);
                    if (string == NULL) {
                        rc = FALSE;
                        /* exception is pending from the call */
                        goto done;
                    ...
                    string = vm->memoryManagerFunctions->j9gc_createJavaLangString(
                        vmThread, (U_8*)JAVA_MODULE_VERSION, LITERAL_STRLEN(JAVA_MODULE_VERSION), J9_STR_XLAT);
                    if (string == NULL) {
                        rc = FALSE;
                        /* exception is pending from the call */
                        goto done;
            ...
            string = vm->memoryManagerFunctions->j9gc_createJavaLangString(
                vmThread, J9UTF8_DATA(utf), (U_32)J9UTF8_LENGTH(utf), J9_STR_XLAT);
            if (string == NULL) {
                rc = FALSE;
                /* exception is pending from the call */
                goto done;
            ...
            if (fileName != NULL) {
                utf = J9ROMCLASS_CLASSNAME(romClass);
                string = vm->memoryManagerFunctions->j9gc_createJavaLangString(
                    vmThread, J9UTF8_DATA(fileName), (U_32)J9UTF8_LENGTH(fileName), 0);
                if (string == NULL) {
                    rc = FALSE;
                    /* exception is pending from the call */
                    goto done;
...
retry:
    ...
    if (arrayClass == NULL) {
        ...
        arrayClass = vmfns->internalCreateArrayClass(
            vmThread, (J9ROMArrayClass*)J9ROMIMAGEHEADER_FIRSTCLASS(vm->arrayROMClasses), elementClass);
        if (arrayClass == NULL) {
            /* exception is pending from the call */
            return NULL;
        }
