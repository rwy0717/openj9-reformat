...
#define JCL_CACHE_GET(env, x) (((JniIDCache*)J9VMLS_GET((env), JCL_ID_CACHE))->x)

#define JCL_CACHE_SET(env, x, v) (((JniIDCache*)J9VMLS_GET((env), JCL_ID_CACHE))->x = (v))

#define jclmem_allocate_memory(env, byteAmount) j9mem_allocate_memory(byteAmount, J9MEM_CATEGORY_VM_JCL)

#define jclmem_free_memory(env, buf) j9mem_free_memory(buf)
