...
void JNICALL Java_com_ibm_oti_vm_VM_localGC(JNIEnv* env, jclass clazz)
{
...
void JNICALL Java_com_ibm_oti_vm_VM_globalGC(JNIEnv* env, jclass clazz)
{
    J9VMThread* currentThread = (J9VMThread*)env;
    J9JavaVM* vm = currentThread->javaVM;
    J9InternalVMFunctions* vmFuncs = vm->internalVMFunctions;
 ...
 * String2 has its bytes set to be string1-> bytes if the offsets already match and the bytes are not already set to the
 * same value The bytes being set already could happen frequently as this primitive will be used repeatedly to remerge
 * strings in the runtime
 */
jint JNICALL Java_com_ibm_oti_vm_VM_setCommonData(JNIEnv* env, jclass unused, jobject string1, jobject string2)
{
    ...
    if (OMR_GC_ALLOCATION_TYPE_SEGREGATED != javaVM->gcAllocationType) {
        if (string1 == NULL || string2 == NULL) {
            ...
            /* skip merging of bytes if they are already the same. As this primitive is called repeatedly, then its
             * likely the character data will be the same on repeat calls.
             */
...
static UDATA allInstances(JNIEnv* env, jclass clazz, jobjectArray target)
{
    ...
    vmThread->javaVM->memoryManagerFunctions->j9mm_iterate_all_objects(
        vmThread->javaVM, vmThread->javaVM->portLibrary, 0, collectInstances, &data);
