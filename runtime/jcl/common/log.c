...
jint JNICALL Java_com_ibm_jvm_Log_SetOptionsImpl(JNIEnv* env, jclass clazz, jstring options)
{
    ...
    if (JVMTI_ERROR_NONE != rc) {
        raiseException(
            env, "java/lang/RuntimeException", J9NLS_JVMTI_COM_IBM_LOG_SET_OPT_ERROR, "Could not set JVM log options");
        return JNI_ERR;
    }
...
static void raiseException(JNIEnv* env, const char* type, U_32 moduleNumber, U_32 messageNumber, const char* message)
{
    ...
    nlsMessage = OMRPORT_FROM_J9PORT(PORTLIB)->nls_lookup_message(
        OMRPORT_FROM_J9PORT(PORTLIB), J9NLS_ERROR, moduleNumber, messageNumber, message);
