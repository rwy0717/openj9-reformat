 ...
 */
jint JNICALL Java_com_ibm_virtualization_management_internal_HypervisorMXBeanImpl_isEnvironmentVirtualImpl(
 ...
 */
jstring JNICALL Java_com_ibm_virtualization_management_internal_HypervisorMXBeanImpl_getVendorImpl(
    JNIEnv* env, jobject obj)
{
    PORT_ACCESS_FROM_ENV(env);
...
typedef enum { GUEST_PROCESSOR = 0, GUEST_MEMORY } objTypes;
...
static jint handle_error(JNIEnv* env, IDATA error, jint type)
{
    ...
    if (J9PORT_ERROR_HYPERVISOR_MEMORY_ALLOC_FAILED == error) {
        ((J9VMThread*)env)
            ->javaVM->internalVMFunctions->throwNativeOOMError(env, J9NLS_PORT_HYPERVISOR_OUT_OF_MEMORY_ERROR_MSG);
        return 0;
    }
    ...
    formatString = j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE,
        J9NLS_JCL_HYPERVISOR_USAGE_RETRIEVAL_ERROR_MSG, NULL);
    ...
    JNIEnv* env, jobject beanInstance, jobject procUsageObject)
{
    ...
    if (NULL == CLS_GuestOSProcessorUsage) {
        ...
        JCL_CACHE_SET(
            env, MID_java_com_ibm_virtualization_management_GuestOSProcessorUsage_updateValues, MID_updateValues);
    } else {
        MID_updateValues
            = JCL_CACHE_GET(env, MID_java_com_ibm_virtualization_management_GuestOSProcessorUsage_updateValues);
    }
    ...
    (*env)->CallVoidMethod(env, procUsageObject, MID_updateValues, (jlong)procUsage.cpuTime, (jlong)procUsage.timestamp,
        (jfloat)procUsage.cpuEntitlement, (jlong)procUsage.hostCpuClockSpeed);
    ...
    JNIEnv* env, jobject beanInstance, jobject memUsageObject)
{
    ...
    if (NULL == CLS_GuestOSMemoryUsage) {
        ...
        JCL_CACHE_SET(
            env, MID_java_com_ibm_virtualization_management_GuestOSMemoryUsage_updateValues, MID_updateValues);
    } else {
        MID_updateValues
            = JCL_CACHE_GET(env, MID_java_com_ibm_virtualization_management_GuestOSMemoryUsage_updateValues);
    }
    ...
    /* Call the update values method to update the object with the values obtained */
    (*env)->CallVoidMethod(env, memUsageObject, MID_updateValues, (jlong)memUsage.memUsed, (jlong)memUsage.timestamp,
