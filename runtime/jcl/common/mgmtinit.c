...
typedef enum { CLASS_MEMORY = 0, MISC_MEMORY, JIT_CODECACHE, JIT_DATACACHE } nonHeapMemoryPoolIndex;
...
static void gcEndEvent(J9JavaVM* vm, UDATA heapSize, UDATA heapUsed, UDATA* totals, UDATA* frees, UDATA* maxs,
    UDATA collectorID, OMR_VMThread* omrVMThread);
...
static void getSegmentSizes(J9JavaVM* javaVM, J9MemorySegmentList* segList, U_64* storedSize, U_64* storedUsed,
    U_64* storedPeakSize, U_64* storedPeakUsed);
...
jint managementInit(J9JavaVM* vm)
{
    ...
    if ((*vmHooks)->J9HookRegisterWithCallSite(
            vmHooks, J9HOOK_VM_INTERNAL_CLASS_LOAD, managementClassLoadCounter, OMR_GET_CALLSITE(), mgmt)) {
        return JNI_ERR;
    }

#ifdef J9VM_GC_DYNAMIC_CLASS_UNLOADING
    if ((*vmHooks)->J9HookRegisterWithCallSite(
            vmHooks, J9HOOK_VM_CLASSES_UNLOAD, managementClassUnloadCounter, OMR_GET_CALLSITE(), mgmt)) {
        return JNI_ERR;
    }
#endif
    ...
    if ((*omrGCHooks)
            ->J9HookRegisterWithCallSite(
                omrGCHooks, J9HOOK_MM_OMR_GLOBAL_GC_START, managementGlobalGCStart, OMR_GET_CALLSITE(), vm)) {
        return JNI_ERR;
    }
    if ((*omrGCHooks)
            ->J9HookRegisterWithCallSite(
                omrGCHooks, J9HOOK_MM_OMR_LOCAL_GC_START, managementLocalGCStart, OMR_GET_CALLSITE(), vm)) {
        return JNI_ERR;
    }
    if ((*omrGCHooks)
            ->J9HookRegisterWithCallSite(
                omrGCHooks, J9HOOK_MM_OMR_GLOBAL_GC_END, managementGlobalGCEnd, OMR_GET_CALLSITE(), vm)) {
        return JNI_ERR;
    }
    if ((*omrGCHooks)
            ->J9HookRegisterWithCallSite(
                omrGCHooks, J9HOOK_MM_OMR_LOCAL_GC_END, managementLocalGCEnd, OMR_GET_CALLSITE(), vm)) {
        return JNI_ERR;
    }
    if ((*omrGCHooks)
            ->J9HookRegisterWithCallSite(
                omrGCHooks, J9HOOK_MM_OMR_COMPACT_END, managementCompactEnd, OMR_GET_CALLSITE(), vm)) {
        return JNI_ERR;
    }
    ...
    if ((*vmHooks)->J9HookRegisterWithCallSite(
            vmHooks, J9HOOK_VM_THREAD_CREATED, managementThreadStartCounter, OMR_GET_CALLSITE(), mgmt)) {
        omrthread_monitor_exit(vm->vmThreadListMutex);
        return JNI_ERR;
    }
    if ((*vmHooks)->J9HookRegisterWithCallSite(
            vmHooks, J9HOOK_VM_THREAD_DESTROY, managementThreadEndCounter, OMR_GET_CALLSITE(), mgmt)) {
        omrthread_monitor_exit(vm->vmThreadListMutex);
        return JNI_ERR;
    }
    ...
    if (jitHooks) {
        if ((*jitHooks)->J9HookRegisterWithCallSite(
                jitHooks, J9HOOK_JIT_COMPILING_START, managementCompilingStartTime, OMR_GET_CALLSITE(), mgmt)) {
            return JNI_ERR;
        }
        if ((*jitHooks)->J9HookRegisterWithCallSite(
                jitHooks, J9HOOK_JIT_COMPILING_END, managementCompilingEndTime, OMR_GET_CALLSITE(), mgmt)) {
            return JNI_ERR;
        }
    }
#endif
...
static void managementCompilingStartTime(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (0 < mgmt->threadsCompiling) {
        mgmt->totalCompilationTime
            += checkedTimeInterval((U_64)now, (U_64)mgmt->lastCompilationStart) * mgmt->threadsCompiling;
    }
...
static void managementGC(OMR_VMThread* omrVMThread, void* userData, BOOLEAN isEnd)
{
                ...
                if (0 == (memoryPool->notificationState & THRESHOLD_EXCEEDED)) {
                    ...
                    if (NOTIFICATION_QUEUE_MAX > mgmt->notificationsPending) {
                        ...
                        if (NULL != notification) {
                            ...
                            notification->usageThreshold
                                = j9mem_allocate_memory(sizeof(*notification->usageThreshold), J9MEM_CATEGORY_VM_JCL);
                        ...
                        if (NULL != notification) {
                            ...
                            notification->usageThreshold->thresholdCrossingCount
                                = memoryPool->usageThresholdCrossedCount;
    ...
    UDATA collectorID, OMR_VMThread* omrVMThread)
{
    ...
    for (idx = 0; idx < mgmt->supportedMemoryPools; ++idx) {
        if (0
            != mmFuncs->j9gc_is_managedpool_by_collector(vm, (gcData->id & J9VM_MANAGEMENT_GC_HEAP_ID_MASK),
                   (memoryPools[idx].id & J9VM_MANAGEMENT_POOL_HEAP_ID_MASK))) {
    ...
    mmFuncs->j9gc_get_CPU_times(
        vm, &mgmt->gcMasterCpuTime, &mgmt->gcSlaveCpuTime, &mgmt->gcMaxThreads, &mgmt->gcCurrentThreads);
...
static jint initMemoryManagement(J9JavaVM* vm)
{
    ...
    mgmt->memoryPools
        = j9mem_allocate_memory((sizeof(*mgmt->memoryPools) * mgmt->supportedMemoryPools), J9MEM_CATEGORY_VM_JCL);
    ...
    mgmt->garbageCollectors
        = j9mem_allocate_memory((sizeof(*mgmt->garbageCollectors) * mgmt->supportedCollectors), J9MEM_CATEGORY_VM_JCL);
...
#endif
    mgmt->nonHeapMemoryPools = j9mem_allocate_memory(
        (sizeof(*mgmt->nonHeapMemoryPools) * mgmt->supportedNonHeapMemoryPools), J9MEM_CATEGORY_VM_JCL);
    ...
    for (idx = 0; idx < mgmt->supportedNonHeapMemoryPools; idx++) {
        ...
        getSegmentSizes(vm, segList, &mgmt->nonHeapMemoryPools[idx].initialSize, &used,
            &mgmt->nonHeapMemoryPools[idx].peakSize, &mgmt->nonHeapMemoryPools[idx].peakUsed);
    }
    return 0;
}
...
static void updateNonHeapMemoryPoolSizes(J9JavaVM* vm, J9JavaLangManagementData* mgmt, BOOLEAN isGCEND)
{
    ...
    for (; idx < mgmt->supportedNonHeapMemoryPools; ++idx) {
        ...
        getSegmentSizes(vm, segList, storedSize, storedUsed, &mgmt->nonHeapMemoryPools[idx].peakSize,
            &mgmt->nonHeapMemoryPools[idx].peakUsed);
    }
}
