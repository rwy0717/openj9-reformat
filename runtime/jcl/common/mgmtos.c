...
static void enqueueNotification(
    struct J9PortLibrary* portLibrary, struct J9JavaLangManagementData* mgmt, U_32 type, U_64 data);

jlong JNICALL Java_com_ibm_lang_management_internal_ExtendedOperatingSystemMXBeanImpl_getTotalPhysicalMemoryImpl(
...
jint JNICALL Java_com_ibm_lang_management_internal_ExtendedOperatingSystemMXBeanImpl_getProcessingCapacityImpl(
    JNIEnv* env, jobject beanInstance)
{
    PORT_ACCESS_FROM_ENV(env);
...
jboolean JNICALL Java_com_ibm_lang_management_internal_ExtendedOperatingSystemMXBeanImpl_isDLPAREnabled(
    JNIEnv* env, jobject beanInstance)
{
    PORT_ACCESS_FROM_ENV(env);
...
void JNICALL Java_com_ibm_lang_management_internal_OperatingSystemNotificationThread_processNotificationLoop(
    JNIEnv* env, jobject instance)
{
    ...
    while (1) {
        ...
        if (notification->type != DLPAR_NOTIFIER_SHUTDOWN_REQUEST) {
            (*env)->CallVoidMethod(env, instance, helperID, (jint)notification->type, (jlong)notification->data,
...
void JNICALL Java_com_ibm_lang_management_internal_OperatingSystemNotificationThreadShutdown_sendShutdownNotification(
    JNIEnv* env, jobject instance)
{
    J9JavaVM* javaVM = ((J9VMThread*)env)->javaVM;
    J9JavaLangManagementData* mgmt = javaVM->managementData;
 ...
 */
jobject JNICALL Java_com_ibm_lang_management_internal_SysinfoCpuTime_getCpuUtilizationImpl(JNIEnv* env, jclass clazz)
{
    ...
    result
        = (*env)->NewObject(env, clazz, tempMethod, cpuTime.timestamp, cpuTime.cpuTime, cpuTime.numberOfCpus, myStatus);
    return result;
}
...
static UDATA reconfigHandler(struct J9PortLibrary* portLibrary, U_32 gpType, void* gpInfo, void* userData)
{
    ...
    /* we just caught SIGRECONFIG - rather than try to figure out the DLPAR event directly, we just check whether the 3
     * numbers we're interested in have actually changed */
