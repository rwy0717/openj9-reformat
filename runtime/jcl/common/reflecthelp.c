...
typedef struct AllFieldData {
    ...
    U_32 restartRequired; /**< restart operation required due to inconsistency between romFieldShape and
                             J9Class->romClass caused by class redefinition */
} AllFieldData;
...
static UDATA isConstructor(J9Method* ramMethod)
{
    J9ROMMethod* romMethod = J9_ROM_METHOD_FROM_RAM_METHOD(ramMethod);
    return (0 == (romMethod->modifiers & J9_JAVA_STATIC))
        && ('<' == J9UTF8_DATA(J9ROMMETHOD_GET_NAME(methodClass->romClass, romMethod))[0]);
}
 ...
 */
static j9object_t getAnnotationDataAsByteArray(struct J9VMThread* vmThread, U_32* annotationData)
{
    U_32 i = 0;
...
static j9object_t getAnnotationDataFromROMMethodHelper(
    struct J9VMThread* vmThread, J9Method* ramMethod, U_32* (*getAnnotationDataFromROMMethod)(J9ROMMethod* romMethod))
{
 ...
 */
static jclass findClassAndCreateGlobalRef(JNIEnv* env, char* className)
{
 ...
 */
static U_8 computeArgCount(J9ROMMethod* method)
{
 ...
 */
jobjectArray getMethodParametersAsArray(JNIEnv* env, jobject jlrExecutable)
{
    ...
    if (NULL == initMethodID) {
        initMethodID = (*env)->GetMethodID(
            env, jlrParameterClass, "<init>", "(Ljava/lang/String;ILjava/lang/reflect/Executable;I)V");
        if (NULL == initMethodID) {
    ...
    if (NULL != executableID) {
        ...
        if (NULL == parametersData) {
            /* NULL return value from this method eventually JVM_GetMethodParameters triggers
             * j.l.r.Executable.synthesizeAllParams() */
            goto finished;
        }
        ...
        for (index = 0; index < methodParametersLimit; index++) {
                    ...
                    if (NULL != nameUTF) {
                        ...
                        if ((utf8Length + 1) > sizeof(buffer)) {
                            ...
                            if (NULL == nameCharArray) {
                                ...
                                vmFuncs->internalExitVMToJNI(vmThread);
                                Trc_JCL_getMethodParametersAsArray_Failed_To_Allocate_Memory_For_NameCharArray(
                                    env, utf8Length + 1);
                                goto finished;
                            }
                        }
            ...
            parameterObject
                = (*env)->NewObject(env, jlrParameterClass, initMethodID, nameStr, flags, jlrExecutable, index);

            if ((*env)->ExceptionCheck(env)) {
                Trc_JCL_getMethodParametersAsArray_Failed_To_Create_java_lang_reflect_Parameter_Object(
                    env, nameStr, flags, index);
                goto finished;
            }
...
static J9Class* classForSignature(struct J9VMThread* vmThread, U_8** sigDataPtr, struct J9ClassLoader* classLoader)
{
    ...
    case 'L': {
        ...
        clazz = vm->internalVMFunctions->internalFindClassUTF8(
            vmThread, tempData, length, classLoader, J9_FINDCLASS_FLAG_THROW_ON_FAIL);
        break;
    }
    case 'I':
...
static U_32 getArgCountFromSignature(J9UTF8* signature)
{
    U_32 argCount = 0;
    UDATA i = 0;
...
static j9object_t parameterTypesForMethod(
    struct J9VMThread* vmThread, struct J9Method* ramMethod, struct J9Class** returnType)
{
    ...
    params = vmThread->javaVM->memoryManagerFunctions->J9AllocateIndexableObject(vmThread,
        fetchArrayClass(vmThread, J9VMJAVALANGCLASS_OR_NULL(vmThread->javaVM)), argCount,
...
static j9object_t exceptionTypesForMethod(struct J9VMThread* vmThread, struct J9Method* ramMethod)
{
    ...
    /* create the array of classes */
    exceptions = vmThread->javaVM->memoryManagerFunctions->J9AllocateIndexableObject(vmThread,
        fetchArrayClass(vmThread, J9VMJAVALANGCLASS_OR_NULL(vmThread->javaVM)), count,
        J9_GC_ALLOCATE_OBJECT_NON_INSTRUMENTABLE);
    if (NULL == exceptions) {
        vmThread->javaVM->internalVMFunctions->setHeapOutOfMemoryError(vmThread);
        return NULL;
    ...
    if (0 != count) {
        ...
        for (i = 0; (i < count) && (NULL != exceptions); i++, throwNames++) {
            ...
            J9Class* clazz = vmThread->javaVM->internalVMFunctions->internalFindClassUTF8(
                vmThread, J9UTF8_DATA(throwUTF), J9UTF8_LENGTH(throwUTF), classLoader, J9_FINDCLASS_FLAG_THROW_ON_FAIL);
            if (NULL == clazz) {
...
static void fillInReflectMethod(
    j9object_t methodObject, struct J9Class* declaringClass, jmethodID methodID, struct J9VMThread* vmThread)
{
    ...
    nameString = gcFunctions->j9gc_createJavaLangString(
        vmThread, J9UTF8_DATA(nameUTF), (U_32)J9UTF8_LENGTH(nameUTF), J9_STR_INTERN);
    if (NULL == nameString) {
        DROP_OBJECT_IN_SPECIAL_FRAME(vmThread); /* methodObject */
        return;
    }
    ...
    if (J9ROMMETHOD_HAS_GENERIC_SIGNATURE(romMethod)) {
        ...
        j9object_t sigString
            = gcFunctions->j9gc_createJavaLangString(vmThread, J9UTF8_DATA(sigUTF), (U_32)J9UTF8_LENGTH(sigUTF), 0);
        if (NULL == sigString) {
            DROP_OBJECT_IN_SPECIAL_FRAME(vmThread); /* methodObject */
            return;
        }
...
static j9object_t createField(struct J9VMThread* vmThread, jfieldID fieldID)
{
    ...
    fieldObject = vmThread->javaVM->memoryManagerFunctions->J9AllocateObject(
        vmThread, jlrFieldClass, J9_GC_ALLOCATE_OBJECT_NON_INSTRUMENTABLE);
    if (NULL == fieldObject) {
        vmThread->javaVM->internalVMFunctions->setHeapOutOfMemoryError(vmThread);
        return NULL;
    }
    ...
    nameString = vmThread->javaVM->memoryManagerFunctions->j9gc_createJavaLangString(
        vmThread, J9UTF8_DATA(nameUTF), (U_32)J9UTF8_LENGTH(nameUTF), J9_STR_INTERN);
    if (NULL == nameString) {
        DROP_OBJECT_IN_SPECIAL_FRAME(vmThread); /* fieldObject */
        return NULL;
    ...
    if (0 != (j9FieldID->field->modifiers & J9FieldFlagHasGenericSignature)) {
        ...
        j9object_t sigString = vmThread->javaVM->memoryManagerFunctions->j9gc_createJavaLangString(
            vmThread, J9UTF8_DATA(sigUTF), (U_32)J9UTF8_LENGTH(sigUTF), 0);
        if (NULL == sigString) {
            DROP_OBJECT_IN_SPECIAL_FRAME(vmThread); /* fieldObject */
            return NULL;
        }
    ...
    J9VMJAVALANGREFLECTFIELD_SET_DECLARINGCLASS(
        vmThread, fieldObject, J9VM_J9CLASS_TO_HEAPCLASS(j9FieldID->declaringClass));
#if defined(USE_SUN_REFLECT)
...
static j9object_t createMethod(struct J9VMThread* vmThread, J9JNIMethodID* methodID, j9object_t parameterTypes)
{
    ...
    methodObject = vmThread->javaVM->memoryManagerFunctions->J9AllocateObject(
        vmThread, jlrMethodClass, J9_GC_ALLOCATE_OBJECT_NON_INSTRUMENTABLE);
...
static j9object_t createConstructor(struct J9VMThread* vmThread, J9JNIMethodID* methodID, j9object_t parameterTypes)
{
    j9object_t exceptionTypes = NULL;
    ...
    constructorObject = vmThread->javaVM->memoryManagerFunctions->J9AllocateObject(
        vmThread, jlrConstructorClass, J9_GC_ALLOCATE_OBJECT_NON_INSTRUMENTABLE);
    ...
    J9VMJAVALANGREFLECTCONSTRUCTOR_SET_DECLARINGCLASS(
        vmThread, constructorObject, J9VM_J9CLASS_TO_HEAPCLASS(declaringClass));

    J9VMJAVALANGREFLECTCONSTRUCTOR_SET_INTMETHODID(vmThread, constructorObject, (U_32)getMethodIndex(ramMethod));

#if defined(USE_SUN_REFLECT)
    J9VMJAVALANGREFLECTCONSTRUCTOR_SET_MODIFIERS(
        vmThread, constructorObject, romMethod->modifiers & CFR_METHOD_ACCESS_MASK);
#endif

    return constructorObject;
}

static struct J9JNIFieldID* idFromFieldObject(
    J9VMThread* vmThread, j9object_t declaringClassObject, j9object_t fieldObject)
{
...
static struct J9JNIMethodID* idFromMethodObject(J9VMThread* vmThread, j9object_t methodObject)
{
...
static struct J9JNIMethodID* idFromConstructorObject(J9VMThread* vmThread, j9object_t constructorObject)
{
    J9JNIMethodID* methodID = NULL;
...
static jobject idToReflectField(J9VMThread* vmThread, jfieldID fieldID)
{
    jobject result = NULL;
    enterVMFromJNI(vmThread);
    if (NULL == fieldID) {
...
static jobject idToReflectMethod(J9VMThread* vmThread, jmethodID methodID)
{
    jobject result = NULL;
    enterVMFromJNI(vmThread);
    if (NULL == methodID) {
...
static j9object_t createMethodObject(
    struct J9Method* ramMethod, struct J9Class* declaringClass, j9array_t parameterTypes, struct J9VMThread* vmThread)
{
...
static j9object_t createConstructorObject(
    struct J9Method* ramMethod, struct J9Class* declaringClass, j9array_t parameterTypes, struct J9VMThread* vmThread)
{
    j9object_t method = NULL;
    J9JNIMethodID* methodID = vmThread->javaVM->internalVMFunctions->getJNIMethodID(vmThread, ramMethod);
...
static j9object_t createInstanceFieldObject(struct J9ROMFieldShape* romField, struct J9Class* declaringClass,
    struct J9Class* lookupClass, struct J9VMThread* vmThread, UDATA* inconsistentData)
{
    ...
    IDATA offset = vmFuncs->instanceFieldOffset(vmThread, declaringClass, J9UTF8_DATA(nameUTF), J9UTF8_LENGTH(nameUTF),
        J9UTF8_DATA(sigUTF), J9UTF8_LENGTH(sigUTF), NULL, NULL, 0);
    jfieldID fieldID
        = (jfieldID)vmFuncs->getJNIFieldID(vmThread, declaringClass, romField, (UDATA)offset, inconsistentData);
...
static j9object_t createStaticFieldObject(struct J9ROMFieldShape* romField, struct J9Class* declaringClass,
    struct J9Class* lookupClass, struct J9VMThread* vmThread, UDATA* inconsistentData)
{
    const J9InternalVMFunctions* vmFuncs = vmThread->javaVM->internalVMFunctions;
    j9object_t field = NULL;
    ...
    J9UTF8* nameUTF = J9ROMFIELDSHAPE_NAME(romField);
    J9UTF8* sigUTF = J9ROMFIELDSHAPE_SIGNATURE(romField);
    void* offset = vmFuncs->staticFieldAddress(vmThread, declaringClass, J9UTF8_DATA(nameUTF), J9UTF8_LENGTH(nameUTF),
        J9UTF8_DATA(sigUTF), J9UTF8_LENGTH(sigUTF), NULL, NULL, 0, NULL);
    if (NULL != offset) {
        fieldID = (jfieldID)vmFuncs->getJNIFieldID(
            vmThread, declaringClass, romField, (UDATA)offset - (UDATA)declaringClass->ramStatics, inconsistentData);
    }
    if (NULL != fieldID) {
        field = createField(vmThread, fieldID);
    }
...
static jfieldID reflectFieldToID(J9VMThread* vmThread, jobject reflectField)
{
...
void preloadReflectWrapperClasses(J9JavaVM* javaVM)
{
    ...
    for (i = J9VMCONSTANTPOOL_JAVALANGBYTE; i < J9VMCONSTANTPOOL_JAVALANGVOID; i++) {
        (void)javaVM->internalVMFunctions->internalFindKnownClass(
            javaVM->mainThread, i, J9_FINDKNOWNCLASS_FLAG_INITIALIZE);
    }
    (void)javaVM->internalVMFunctions->internalFindKnownClass(javaVM->mainThread,
        J9VMCONSTANTPOOL_JAVALANGREFLECTINVOCATIONTARGETEXCEPTION, J9_FINDKNOWNCLASS_FLAG_INITIALIZE);
    ...
    JNIEnv* env, jclass unusedClass, jclass componentType, jint dimensions, jintArray dimensionsArray)
{
    ...
    if (NULL != componentTypeClassObject) {
        ...
        if (J9ROMCLASS_IS_ARRAY(componentTypeClass->romClass)
            && ((((J9ArrayClass*)componentTypeClass)->arity + dimensions) > J9_ARRAY_DIMENSION_LIMIT)) {
            /* The spec says to throw IllegalArgumentException if the number of dimensions is greater than
        ...
        if (!exceptionIsPending) {
            ...
            directObject = vmThread->javaVM->internalVMFunctions->helperMultiANewArray(vmThread,
                (J9ArrayClass*)componentTypeClass, (UDATA)dimensions, onStackDimensions,
                J9_GC_ALLOCATE_OBJECT_NON_INSTRUMENTABLE);
    ...
    while (NULL != romField) {
        ...
        if (0
            != vmFuncs->compareStringToUTF8(
                   vmThread, fieldNameObj, 0, J9UTF8_DATA(romFieldName), J9UTF8_LENGTH(romFieldName))) {
...
nosuchfield:
    vmFuncs->setCurrentException(
        vmThread, J9VMCONSTANTPOOL_JAVALANGNOSUCHFIELDEXCEPTION, (UDATA*)*(j9object_t*)fieldName);
    goto done;
...
retry:
    ...
    fieldArrayObject = (j9array_t)vm->memoryManagerFunctions->J9AllocateIndexableObject(
        vmThread, fieldArrayClass, fieldCount, J9_GC_ALLOCATE_OBJECT_NON_INSTRUMENTABLE);
    ...
    if (fieldCount > 0) {
        ...
        while (NULL != romField) {
            ...
            /* Storing newly allocated objects into newly allocated array. Both in same space, so no exception can
             * occur. */
...
static J9WalkFieldAction findFieldIterator(J9ROMFieldShape* romField, J9Class* declaringClass, void* userData)
{
    if (romField->modifiers & J9AccPublic) {
        ...
        /* compare name with this field */
        if (0
            != vm->internalVMFunctions->compareStringToUTF8(
                   data->currentThread, data->fieldNameObj, 0, J9UTF8_DATA(fieldName), J9UTF8_LENGTH(fieldName))) {
