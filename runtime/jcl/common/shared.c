...
static J9ClassPathEntry* getCachedURL(
    JNIEnv* env, jint helperID, const char* pathChars, jsize pathLen, UDATA cpeType, U_16 cpeStatus);
static const char* copyString(
    J9PortLibrary* portlib, const char* toCopy, UDATA length, J9SharedStringFarm** farmRoot, const J9UTF8** makeUTF8);
UDATA urlHashFn(void* item, void* userData);
static jint createURLEntry(
    JNIEnv* env, jint helperID, J9ClassPathEntry** cpEntry_, char* correctedPathCopy, UDATA cpeType, U_16 cpeStatus);
...
static jint createToken(
    JNIEnv* env, jint helperID, J9ClassPathEntry** cpEntry_, const char* tokenChars, jsize tokenSize);
static UDATA correctURLPath(
    JNIEnv* env, const char* pathChars, jsize pathLen, char** correctedPathPtr, J9SharedStringFarm** jclStringFarm);
static const char* getCachedString(
    JNIEnv* env, const char* input, jsize length, J9SharedStringFarm** farmRoot, const J9UTF8** getUTF8);
...
static jint createCPEntries(
    JNIEnv* env, jint helperID, jint urlCount, J9ClassPathEntry** cpEntries_, URLElements* urlArrayElements);
UDATA utfHashFn(void* item, void* userData);
static UDATA getStringChars(JNIEnv* env, const char** chars, jsize* len, jstring obj);
static UDATA getStringPair(
    JNIEnv* env, const char** chars1, jsize* len1, const char** chars2, jsize* len2, jstring obj1, jstring obj2);
...
static UDATA getPathProtocolFromURL(
    JNIEnv* env, jobject url, jmethodID URLgetPathID, jmethodID URLgetProtocolID, URLElements* urlElements);
...
/* Pass a jclStringFarm if this string is to be copied and kept, otherwise pass a large enough char buffer in
 * correctedPathPtr */
    ...
    JNIEnv* env, const char* pathChars, jsize pathLen, char** correctedPathPtr, J9SharedStringFarm** jclStringFarm)
{
    ...
    if (!jclStringFarm) {
        ...
        if (!(*correctedPathPtr
                = (char*)getCachedString(env, (const char*)bufPtr, (jsize)copiedChars, jclStringFarm, NULL))) {
            Trc_JCL_com_ibm_oti_shared_correctURLPath_Exit0(env);
            returnVal = 0;
        }
    }
...
static jint createROMClassCookie(JNIEnv* env, J9JavaVM* vm, J9ROMClass* romClass, jbyteArray romClassCookieBuffer)
{
    ...
    /* Create a magic number that we can check in romutil.c to prevent people hacking up ROMClass cookies from the java
     * space */
    romClassCookie.magic = J9_ROM_CLASS_COOKIE_MAGIC(vmthread->javaVM, romClass);

    (*env)->SetByteArrayRegion(
        env, romClassCookieBuffer, (jsize)0, sizeof(J9ROMClassCookieSharedClass), (jbyte*)&romClassCookie);
    ...
    JNIEnv* env, jint helperID, jint urlCount, J9ClassPathEntry** cpEntries_, URLElements* urlArrayElements)
{
    ...
    for (i = 0; i < urlCount; i++) {
        ...
        cpeType = getCpeTypeForProtocol((char*)urlArrayElements[i].protocolChars, urlArrayElements[i].protocolLen,
            urlArrayElements[i].pathChars, urlArrayElements[i].pathLen);
        if (CPE_TYPE_UNKNOWN == cpeType) {
            Trc_JCL_com_ibm_oti_shared_createCPEntries_ExitFalse4(env);
            goto _error;
        }
        if (!correctURLPath(env, urlArrayElements[i].pathChars, urlArrayElements[i].pathLen, &correctPath,
                &(vm->sharedClassConfig->jclStringFarm))) {
...
static J9ClassPathEntry* getCachedToken(JNIEnv* env, jint helperID, const char* tokenChars, jsize tokenSize)
{
    ...
    while (anElement
        && (!(helperID == anElement->header.id && hashCompare == anElement->tokenHash
...
static void notifyCacheFull(JNIEnv* env, jbyteArray nativeFlags)
{
    jbyte value = 1;
    ...
    JNIEnv* env, jint helperID, J9ClassPathEntry** cpEntry_, const char* tokenChars, jsize tokenSize)
{
    ...
    if (!(newEntry->path = (U_8*)getCachedString(
              env, (const char*)tokenChars, (jsize)tokenSize, &(vm->sharedClassConfig->jclStringFarm), NULL))) {
    ...
    JNIEnv* env, jint helperID, const char* pathChars, jsize pathLen, UDATA cpeType, U_16 cpeStatus)
{
    ...
    if (anElement) {
        ...
        if (!(origPathCopy
                = getCachedString(env, (const char*)pathChars, (jsize)pathLen, &(config->jclStringFarm), NULL))) {
        ...
        /* The original (not corrected) path is used as a key so that the correction does not have to be performed for
         * each lookup */
        ...
        if (!config->jclURLHashTable) {
            config->jclURLHashTable = hashTableNew(OMRPORT_FROM_J9PORT(PORTLIB), J9_GET_CALLSITE(), 500,
                sizeof(URLhtEntry), sizeof(char*), 0, J9MEM_CATEGORY_VM_JCL, urlHashFn, urlHashEqualFn, NULL, vm);
            if (!config->jclURLHashTable) {
                goto _error;
            }
        }
    ...
    JNIEnv* env, const char* input, jsize length, J9SharedStringFarm** farmRoot, const J9UTF8** getUTF8)
{
        ...
        if (searchResult) {
            ...
            if (getUTF8) {
                if (!searchResult->cachedUTFString) {
                    /* This will make another inline copy of the input string - this should only happen if the string
                     * has been copied once, but a UTF8 was not made */
                    if (!copyString(
                            vm->portLibrary, input, (UDATA)length, farmRoot, &(searchResult->cachedUTFString))) {
    ...
    if (!result) {
        ...
        if (!config->jclUTF8HashTable) {
            config->jclUTF8HashTable = hashTableNew(OMRPORT_FROM_J9PORT(PORTLIB), J9_GET_CALLSITE(), 1000,
                sizeof(UTF8htEntry), sizeof(char*), 0, J9MEM_CATEGORY_VM_JCL, utfHashFn, utfHashEqualFn, NULL, vm);
            if (!config->jclUTF8HashTable) {
...
static J9Pool* getClasspathCache(JNIEnv* env)
{
    ...
    if (!*cpPool) {
        *cpPool = pool_new(sizeof(struct J9ClasspathByID), 0, 0, 0, J9_GET_CALLSITE(), J9MEM_CATEGORY_VM_JCL,
            POOL_FOR_PORT(vm->portLibrary));
    }
    return *cpPool;
}
...
static J9Pool* getURLCache(JNIEnv* env)
{
    ...
    if (!*urlPool) {
        *urlPool = pool_new(sizeof(struct J9URLByID), 0, 0, 0, J9_GET_CALLSITE(), J9MEM_CATEGORY_VM_JCL,
            POOL_FOR_PORT(vm->portLibrary));
    }
    return *urlPool;
}
...
static J9Pool* getTokenCache(JNIEnv* env)
{
    ...
    if (!*tokenPool) {
        *tokenPool = pool_new(sizeof(struct J9TokenByID), 0, 0, 0, J9_GET_CALLSITE(), J9MEM_CATEGORY_VM_JCL,
            POOL_FOR_PORT(vm->portLibrary));
    }
    return *tokenPool;
}
    ...
    JNIEnv* env, jobject thisObj, jobject classloader)
{
    ...
    if (NULL == nativeClassloader) {
        nativeClassloader
            = vm->internalVMFunctions->internalAllocateClassLoader(vm, J9_JNI_UNWRAP_REFERENCE(classloader));
        if (NULL == nativeClassloader) {
 ...
 */
void JNICALL Java_com_ibm_oti_shared_SharedClassURLHelperImpl_init(JNIEnv* env, jclass clazz) { getURLMethodIDs(env); }
    ...
    jboolean doStore, jbyteArray romClassCookie, jboolean minimizeUpdateChecks)
{
    ...
    if (!(cpeType = getCpeTypeForProtocol(
              (char*)urlElements.protocolChars, urlElements.protocolLen, urlElements.pathChars, urlElements.pathLen))) {
        goto _errorPostClassNamePartition;
    }
    ...
    releaseStringPair(
        env, urlElements.pathObj, urlElements.pathChars, urlElements.protocolObj, urlElements.protocolChars);
    ...
    jboolean minimizeUpdateChecks, jbyteArray nativeFlags)
{
    ...
    if (vm->sharedClassConfig != NULL) {
        ...
        omrthread_monitor_enter(vm->classTableMutex);
        newROMClass
            = sharedapi->jclUpdateROMClassMetaData((J9VMThread*)env, classloader, urlEntry, 1, 0, partition, romClass);
        omrthread_monitor_exit(vm->classTableMutex);
    }
    ...
    jint cpLoadIndex, jclass clazzObj, jbyteArray nativeFlags)
{
    ...
    if (vm->sharedClassConfig != NULL) {
        SCAbstractAPI* sharedapi = (SCAbstractAPI*)(vm->sharedClassConfig->sharedAPIObject);
        omrthread_monitor_enter(vm->classTableMutex);
        newROMClass = sharedapi->jclUpdateROMClassMetaData(
            (J9VMThread*)env, classloader, cpEntries, entryCount, cpLoadIndex, partition, romClass);
        omrthread_monitor_exit(vm->classTableMutex);
    }
    ...
    if (NULL != urlArrayElements) {
        for (i = 0; i < urlCount; i++) {
            ...
            releaseStringPair(env, urlArrayElements[i].pathObj, urlArrayElements[i].pathChars,
                urlArrayElements[i].protocolObj, urlArrayElements[i].protocolChars);
        }
    ...
    jbyteArray romClassCookie)
{
    ...
    if ((helperID > 0xFFFF) || (vm->sharedClassConfig->runtimeFlags & J9SHR_RUNTIMEFLAG_DENY_CACHE_ACCESS)) {
        ...
        Trc_JCL_com_ibm_oti_shared_SharedClassURLClasspathHelperImpl_findSharedClassImpl_ExitDenyAccess_Event(
            env, helperID);
    ...
    if (!getStringPair(env, &nameChars, &nameLen, &partitionChars, &partitionLen, classNameObj, partitionObj)) {
        Trc_JCL_com_ibm_oti_shared_SharedClassURLClasspathHelperImpl_findSharedClassImpl_ExitError2_Event(
            env, helperID);
        goto _errorPostClassNamePartition;
    }
    ...
    if (!cpEntries) {
        if (!createCPEntries(env, helperID, urlCount, &cpEntries, urlArrayElements)) {
            ...
            Trc_JCL_com_ibm_oti_shared_SharedClassURLClasspathHelperImpl_findSharedClassImpl_ExitError3_Event(
                env, helperID);
            goto _errorWithMutex;
        } else {
            classloader->classPathEntries = cpEntries;
    ...
    if (partitionChars) {
        if (!getCachedString(env, partitionChars, partitionLen, &(vm->sharedClassConfig->jclStringFarm), &partition)) {
            Trc_JCL_com_ibm_oti_shared_SharedClassURLClasspathHelperImpl_findSharedClassImpl_ExitError4_Event(
                env, helperID);
            goto _errorWithMutex;
        }
    }
    ...
    omrthread_monitor_enter(vm->classMemorySegments->segmentMutex);
    ALWAYS_TRIGGER_J9HOOK_VM_FIND_LOCALLY_DEFINED_CLASS(vm->hookInterface, (J9VMThread*)env, classloader, NULL,
        (const char*)nameChars, (UDATA)nameLen, cpEntries, entryCount, confirmedCount, partition, !doFind, !doStore,
        &indexFoundAt, romClass);
    omrthread_monitor_exit(vm->classMemorySegments->segmentMutex);
    omrthread_monitor_exit(vm->classTableMutex);
    ...
    if (romClass) {
        createROMClassCookie(env, vm, romClass, romClassCookie);
        /* trace event is at level 1 and trace exit message is at level 2 as per CMVC 155318/157683 */
        Trc_JCL_com_ibm_oti_shared_SharedClassURLClasspathHelperImpl_findSharedClassImpl_ExitResult_Event(
            env, indexFoundAt, helperID);
    ...
    JNIEnv* env, jobject thisObj, jint helperID, jstring tokenObj)
{
    ...
    numElem = config->findSharedData(
        (J9VMThread*)env, (const char*)tokenChars, (UDATA)tokenLen, J9SHR_DATA_TYPE_JCL, FALSE, &existingData, NULL);
...
jboolean JNICALL Java_com_ibm_oti_shared_SharedAbstractHelper_getIsVerboseImpl(JNIEnv* env, jobject thisObj)
{
    ...
    vm = ((J9VMThread*)env)->javaVM;
    if (vm->sharedClassConfig != NULL) {
        result = (jboolean)J9_ARE_ALL_BITS_SET(
            vm->sharedClassConfig->verboseFlags, J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_HELPER);
    }
 ...
 */
static IDATA populateSharedCacheInfo(J9JavaVM* vm, J9SharedCacheInfo* event_data, void* user_data)
{
#if defined(J9VM_OPT_SHARED_CLASSES)
    jstring cacheName;
    jclass javaClass;
    ...
    sharedCacheInfoObject = (*env)->NewObject(env, javaClass, mid, cacheName, (jboolean)event_data->isCompatible,
        (jboolean)(J9PORT_SHR_CACHE_TYPE_PERSISTENT == (jboolean)event_data->cacheType), (jint)event_data->os_shmid,
        (jint)event_data->os_semid, (jlong)event_data->lastDetach, (jint)event_data->modLevel,
        (jint)event_data->addrMode, (jboolean)(0 < (IDATA)event_data->isCorrupt),
        (-1 == event_data->cacheSize) ? (jlong)-1 : (jlong)event_data->cacheSize,
        (-1 == event_data->freeBytes) ? (jlong)-1 : (jlong)event_data->freeBytes, (jint)event_data->cacheType,
...
jint JNICALL Java_com_ibm_oti_shared_SharedClassUtilities_getSharedCacheInfoImpl(
    JNIEnv* env, jclass clazz, jstring cacheDir, jint flags, jboolean useCommandLineValue, jobject arrayList)
{
#if defined(J9VM_OPT_SHARED_CLASSES)
    J9JavaVM* vm;
    ...
    result = (I_32)vm->sharedCacheAPI->iterateSharedCaches(
        vm, (const char*)dir, (UDATA)flags, useCommandLineValue, populateSharedCacheInfo, (void*)arrayList);
    if ((JNI_FALSE == useCommandLineValue) && (NULL != cacheDir)) {
    ...
    JNIEnv* env, jclass clazz, jstring cacheDir, jint cacheType, jstring cacheName, jboolean useCommandLineValue)
{
    ...
    result = (I_32)vm->sharedCacheAPI->destroySharedCache(
        vm, (const char*)dir, (const char*)name, cacheType, useCommandLineValue);
