 ...
 */
U_32 computeJCLRuntimeFlags(J9JavaVM* vm)
{
...
jint standardInit(J9JavaVM* vm, char* dllName)
{
    ...
    if (result == 0) {
        ...
        if ((vmThread->currentException != NULL) || (vmThread->threadObject == NULL)) {
            ...
            vmFuncs->internalFindKnownClass(vmThread, J9VMCONSTANTPOOL_JAVALANGTHREADDEATH,
    ...
    if (vm->reflectFunctions.idToReflectMethod) {
        ...
        if (!clazz)
            goto _fail;
        invokeMethod = (*(JNIEnv*)vmThread)
                           ->GetMethodID((JNIEnv*)vmThread, clazz, "invoke",
        ...
        if (J2SE_SHAPE(vm) != J2SE_SHAPE_RAW) {
            ...
            if (!clazz)
                goto _fail;
            invokeMethod = (*(JNIEnv*)vmThread)
                               ->GetMethodID((JNIEnv*)vmThread, clazz, "invokeWithArguments",
            ...
            if (NULL != clazz) {
                ...
                (void)(*(JNIEnv*)vmThread)
                    ->GetStaticFieldID((JNIEnv*)vmThread, clazz, "helpers", "Lcom/ibm/jit/JITHelpers;");
                (*(JNIEnv*)vmThread)->DeleteLocalRef((JNIEnv*)vmThread, clazz);
            }
        }
    }
    ...
    if (result != JNI_OK)
        ...
        /*
         * We need to initialize the methodID caches for String.<init> and String.getBytes(String) while we are still in
         * single threaded cade. The JCL natives that initialize this are not thread safe and we run the risk of
         * invoking these methods with a NULL methodID. This code is a work around that forces the methodID cache
...
static IDATA initializeSystemThreadGroup(J9JavaVM* vm, JNIEnv* env)
{
    ...
    if (!systemThreadGroupGlobalRef)
        goto done;
    vm->systemThreadGroupRef = (j9object_t*)systemThreadGroupGlobalRef;
    result = JNI_OK;
done:
    if (threadGroupClass)
        (*env)->DeleteLocalRef(env, threadGroupClass);
    if (threadClass)
        (*env)->DeleteLocalRef(env, threadClass);
    if (threadGroupRef)
        (*env)->DeleteLocalRef(env, threadGroupRef);
    if (groupNameRef)
        (*env)->DeleteLocalRef(env, groupNameRef);
    return result;
}
...
void internalInitializeJavaLangClassLoader(JNIEnv* env)
{
    ...
    appClassLoaderField
        = (*env)->GetStaticFieldID(env, jlClassLoader, "applicationClassLoader", "Ljava/lang/ClassLoader;");
    if ((*env)->ExceptionCheck(env)) {
        goto done;
    }
    ...
    if (NULL == vm->applicationClassLoader) {
         ...
         */
        vm->applicationClassLoader
            = (void*)(UDATA)(vmFuncs->internalAllocateClassLoader(vm, J9_JNI_UNWRAP_REFERENCE(appClassLoader)));
...
jint JNICALL JVM_OnUnload(JavaVM* jvm, void* reserved) { return 0; }
...
checkJCL(J9VMThread* vmThread, U_8* dllValue, U_8* jclConfig, UDATA j9Version, UDATA jclVersion)
{
    ...
    /* If jclConfig is NULL or jclVersion is -1, then we didn't find the fields in java.lang.Class. Make sure the
     * dllValue and jclConfig match. */
...
#endif
    loader->classPathEntryCount = vmFuncs->initializeClassPath(
        vm, path, classpathSeparator[0], CPE_FLAG_BOOTSTRAP, initClassPathEntry, &loader->classPathEntries);
...
static IDATA computeBootstrapClassPathAppend(J9JavaVM* vm)
{
    ...
    for (i = 0; i < args->nOptions; i++) {
        if (strncmp(
                args->options[i].optionString, VMOPT_XBOOTCLASSPATH_A_COLON, sizeof(VMOPT_XBOOTCLASSPATH_A_COLON) - 1)
            == 0) {
...
jint completeInitialization(J9JavaVM* vm)
{
    ...
    if (NULL == currentThread->currentException) {
        /* ensure ClassLoader.applicationClassLoader updated via system property java.system.class.loader is updated in
         * VM as well */
