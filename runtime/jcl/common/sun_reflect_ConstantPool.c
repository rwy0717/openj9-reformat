...
#define J9CLASS_FROMCPINTERNALRAMCLASS(vmThread, j9ClassRef) \
    J9VMJAVALANGINTERNALRAMCLASS_VMREF(vmThread, J9_JNI_UNWRAP_REFERENCE(j9ClassRef))
...
static void clearException(J9VMThread* vmThread)
{
    vmThread->currentException = NULL;
    vmThread->privateFlags &= ~J9_PRIVATE_FLAGS_REPORT_EXCEPTION_THROW;
}

static SunReflectCPResult getRAMConstantRefAndType(
    J9VMThread* vmThread, jobject constantPoolOop, jint cpIndex, UDATA* cpType, J9RAMConstantRef** ramConstantRef)
{
...
static SunReflectCPResult getRAMConstantRef(J9VMThread* vmThread, jobject constantPoolOop, jint cpIndex,
    UDATA expectedCPType, J9RAMConstantRef** ramConstantRef)
{
...
static SunReflectCPResult getROMCPItemAndType(
    J9VMThread* vmThread, jobject constantPoolOop, jint cpIndex, UDATA* cpType, J9ROMConstantPoolItem** romCPItem)
{
    J9Class* ramClass = J9CLASS_FROMCPINTERNALRAMCLASS(vmThread, constantPoolOop);
    J9ROMClass* romClass = ramClass->romClass;
    SunReflectCPResult result = CP_INDEX_OUT_OF_BOUNDS_EXCEPTION;
...
static SunReflectCPResult getROMCPItem(J9VMThread* vmThread, jobject constantPoolOop, jint cpIndex,
    UDATA expectedCPType, J9ROMConstantPoolItem** romCPItem)
{
    UDATA cpType = J9CPTYPE_UNUSED;
...
static SunReflectCPResult getJ9ClassAt(
    J9VMThread* vmThread, jobject constantPoolOop, jint cpIndex, UDATA resolveFlags, J9Class** clazz)
{
    J9RAMClassRef* ramClassRefWrapper = NULL;
    J9InternalVMFunctions* vmFunctions = vmThread->javaVM->internalVMFunctions;
    SunReflectCPResult result = getRAMConstantRef(
        vmThread, constantPoolOop, cpIndex, J9CPTYPE_CLASS, (J9RAMConstantRef**)&ramClassRefWrapper);
    if (OK == result) {
...
static jclass getStringAt(JNIEnv* env, jobject unusedObject, jobject constantPoolOop, jint cpIndex, U_8 cpType)
{
    ...
    if (NULL != constantPoolOop) {
        ...
        if (OK == result) {
            ...
            if (NULL == stringObject) {
                stringObject = vmFunctions->resolveStringRef(vmThread, J9_CP_FROM_CLASS(ramClass), cpIndex,
                    J9_RESOLVE_FLAG_RUNTIME_RESOLVE | J9_RESOLVE_FLAG_NO_THROW_ON_FAIL);
            }
            if (NULL != stringObject) {
...
static jclass getClassAt(JNIEnv* env, jobject constantPoolOop, jint cpIndex, UDATA resolveFlags)
{
...
static jobject getMethodAt(JNIEnv* env, jobject constantPoolOop, jint cpIndex, UDATA resolveFlags)
{
    jobject returnValue = NULL;
    J9VMThread* vmThread = (J9VMThread*)env;
    J9InternalVMFunctions* vmFunctions = vmThread->javaVM->internalVMFunctions;
    ...
    if (NULL != constantPoolOop) {
        ...
        if (OK == result) {
            ...
            case J9CPTYPE_INTERFACE_INSTANCE_METHOD:
                ...
                if ((NULL == method) || (NULL == method->constantPool)) {
                    if (0
                        == vmFunctions->resolveVirtualMethodRef(
                               vmThread, constantPool, cpIndex, resolveFlags, &method)) {
                    ...
                    if (NULL == method) {
                         ...
                         * non-static method.
                         */
                        method = vmFunctions->resolveStaticMethodRefInto(
                            vmThread, constantPool, cpIndex, resolveFlags, NULL);
                    }
                }
                break;
            ...
            case J9CPTYPE_INTERFACE_STATIC_METHOD:
                ...
                /* TODO is this the right check for an unresolved method? Can I check against
                 * vm->initialMethods.initialStaticMethod */
                if ((NULL == method) || (NULL == method->constantPool)) {
        ...
        if (NULL != methodID) {
            if (NULL != jlClass) {
                const jboolean isStatic
                    = (J9CPTYPE_STATIC_METHOD == cpType) || (J9CPTYPE_INTERFACE_STATIC_METHOD == cpType);
...
static jobject getFieldAt(JNIEnv* env, jobject constantPoolOop, jint cpIndex, UDATA resolveFlags)
{
    jobject returnValue = NULL;
    J9VMThread* vmThread = (J9VMThread*)env;
    J9InternalVMFunctions* vmFunctions = vmThread->javaVM->internalVMFunctions;
    ...
    retry:
        ...
        if (OK == result) {
            ...
            case J9CPTYPE_FIELD: {
                ...
                offset = (UDATA)vmFunctions->resolveInstanceFieldRef(
                    vmThread, NULL, constantPool, cpIndex, resolveFlags, &resolvedField);
                if (NULL == resolvedField) {
                    void* fieldAddress = vmFunctions->resolveStaticFieldRef(
                        vmThread, NULL, constantPool, cpIndex, resolveFlags, &resolvedField);
            ...
            if (NULL != resolvedField) {
                ...
                result
                    = getROMCPItem(vmThread, constantPoolOop, cpIndex, cpType, (J9ROMConstantPoolItem**)&romFieldRef);
                if (OK == result) {
                    result = getJ9ClassAt(
                        vmThread, constantPoolOop, romFieldRef->classRefCPIndex, resolveFlags, &declaringClass);
                }
                if (OK == result) {
                    UDATA inconsistentData = 0;
                    fieldID = (jfieldID)vmFunctions->getJNIFieldID(
                        vmThread, declaringClass, resolvedField, offset, &inconsistentData);
...
static U_32 getSingleSlotConstant(JNIEnv* env, jobject constantPoolOop, jint cpIndex, UDATA expectedCPType)
{
    U_32 returnValue = 0;
    J9VMThread* vmThread = (J9VMThread*)env;
    J9InternalVMFunctions* vmFunctions = vmThread->javaVM->internalVMFunctions;
...
static U_64 getDoubleSlotConstant(JNIEnv* env, jobject constantPoolOop, jint cpIndex, UDATA expectedCPType)
{
...
{
    return getClassAt(
        env, constantPoolOop, cpIndex, J9_RESOLVE_FLAG_RUNTIME_RESOLVE | J9_RESOLVE_FLAG_NO_THROW_ON_FAIL);
}
...
{
    return getClassAt(
        env, constantPoolOop, cpIndex, J9_RESOLVE_FLAG_JIT_COMPILE_TIME | J9_RESOLVE_FLAG_NO_THROW_ON_FAIL);
}
    ...
    if (NULL != constantPoolOop) {
        ...
        if (OK == result) {
            ...
            case J9CPTYPE_CLASS: {
                ...
                j9object_t internalClassNameObject = gcFunctions->j9gc_createJavaLangString(
                    vmThread, J9UTF8_DATA(className), (U_32)J9UTF8_LENGTH(className), 0);
    ...
    if (NULL != constantPoolOop) {
        ...
        if (OK == result) {
            ...
            if (NULL != nameAndSignature) {
                ...
                result = getROMCPItem(
                    vmThread, constantPoolOop, classRefCPIndex, J9CPTYPE_CLASS, (J9ROMConstantPoolItem**)&classRef);
                if (OK == result) {
                    J9UTF8* className = J9ROMCLASSREF_NAME(classRef);
                    j9object_t internalClassNameObject = gcFunctions->j9gc_createJavaLangString(
                        vmThread, J9UTF8_DATA(className), (U_32)J9UTF8_LENGTH(className), 0);
                    j9object_t internalNameObject = gcFunctions->j9gc_createJavaLangString(
                        vmThread, J9UTF8_DATA(name), (U_32)J9UTF8_LENGTH(name), 0);
                    j9object_t internalSignatureObject = gcFunctions->j9gc_createJavaLangString(
                        vmThread, J9UTF8_DATA(signature), (U_32)J9UTF8_LENGTH(signature), 0);
                    classNameObject = vmFunctions->j9jni_createLocalRef(env, internalClassNameObject);
    ...
    JNIEnv* env, jclass unusedClass, jobject constantPoolOop, jint cpIndex)
{
    ...
    if (NULL != constantPoolOop) {
        ...
        vmFunctions->internalEnterVMFromJNI(vmThread);
        result = getRAMConstantRef(
            vmThread, constantPoolOop, cpIndex, J9CPTYPE_METHOD_TYPE, (J9RAMConstantRef**)&ramMethodTypeRef);
        if (OK == result) {
            J9Class* ramClass = J9CLASS_FROMCPINTERNALRAMCLASS(vmThread, constantPoolOop);
            j9object_t methodTypeObject = J9STATIC_OBJECT_LOAD(vmThread, ramClass, &ramMethodTypeRef->type);

            if (NULL == methodTypeObject) {
                methodTypeObject = vmFunctions->resolveMethodTypeRef(
                    vmThread, J9_CP_FROM_CLASS(ramClass), cpIndex, J9_RESOLVE_FLAG_RUNTIME_RESOLVE);
            }
            if (NULL != methodTypeObject) {
    ...
    JNIEnv* env, jclass unusedClass, jobject constantPoolOop, jint cpIndex)
{
    ...
    if (NULL != constantPoolOop) {
        ...
        vmFunctions->internalEnterVMFromJNI(vmThread);
        result = getRAMConstantRef(
            vmThread, constantPoolOop, cpIndex, J9CPTYPE_METHODHANDLE, (J9RAMConstantRef**)&ramMethodHandleRef);
        if (OK == result) {
            J9Class* ramClass = J9CLASS_FROMCPINTERNALRAMCLASS(vmThread, constantPoolOop);
            ...
            if (NULL == methodHandleObject) {
                methodHandleObject = vmFunctions->resolveMethodHandleRef(
                    vmThread, J9_CP_FROM_CLASS(ramClass), cpIndex, J9_RESOLVE_FLAG_RUNTIME_RESOLVE);
            }
            if (NULL != methodHandleObject) {
    ...
    JNIEnv* env, jclass unusedClass, jobject constantPoolOop, jint cpIndex)
{
    ...
    if (NULL != constantPoolOop) {
        ...
        vmFunctions->internalEnterVMFromJNI(vmThread);
        result = getRAMConstantRef(
            vmThread, constantPoolOop, cpIndex, J9CPTYPE_CONSTANT_DYNAMIC, (J9RAMConstantRef**)&ramConstantDynamicRef);
        if (OK == result) {
            J9Class* ramClass = J9CLASS_FROMCPINTERNALRAMCLASS(vmThread, constantPoolOop);
            ...
            if ((NULL == value)
                && (ramConstantDynamicRef->exception != vmThread->javaVM->voidReflectClass->classObject)) {
                /* If entry resolved to an exception previously, same exception will be set by resolution code */
                value = vmFunctions->resolveConstantDynamic(
                    vmThread, J9_CP_FROM_CLASS(ramClass), (UDATA)cpIndex, J9_RESOLVE_FLAG_RUNTIME_RESOLVE);
            }
...
jint registerJdkInternalReflectConstantPoolNatives(JNIEnv* env)
{
    ...
    result = (*env)->RegisterNatives(
        env, jdk_internal_reflect_ConstantPool, natives, sizeof(natives) / sizeof(JNINativeMethod));
