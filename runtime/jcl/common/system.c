...
jobject JNICALL Java_java_lang_System_getPropertyList(JNIEnv* env, jclass clazz) { return getPropertyList(env); }
...
jstring JNICALL Java_java_lang_System_mapLibraryName(JNIEnv* env, jclass unusedClass, jstring inName)
{
    ...
    if (!(outNameUTF
            = jclmem_allocate_memory(env, strlen(inNameUTF) + 20))) /* allow for platform extras plus trailing zero */
        return NULL;
...
#define USERNAME_LENGTH 128
     ...
     * then follow the propIndex++ convention and consume 2 * slots for each property. 2 * number of property keys is
     * the correct allocation. Also note the call to addSystemProperties below, which may add some
     * configuration-specific properties.  Be sure to leave enough room in the property list for all possibilities.
     */
    ...
    /* The JCLs use ibm.signalhandling.rs to determine if they should prevent the registration of signal handlers for
     * what we consider to be asynchronous signals. The JCLs do not install handlers for any synchronous signals */
...
jstring getEncoding(JNIEnv* env, jint encodingType)
{
    ...
    case 1: /* platform encoding */
...
#if JAVA_SPEC_VERSION >= 11
        {
            ...
            UDATA written = 0;
            PORT_ACCESS_FROM_ENV(env);
            /* libjava.[so|dylib] is in the jdk/lib/ directory, one level up from the default/ & compressedrefs/
             * directories */
...
void JNICALL Java_java_lang_System_startSNMPAgent(JNIEnv* env, jclass jlClass)
{
...
void JNICALL Java_java_lang_System_rasInitializeVersion(JNIEnv* env, jclass unusedClass, jstring javaRuntimeVersion)
{
