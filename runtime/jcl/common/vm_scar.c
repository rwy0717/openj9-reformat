...
jint JNICALL JVM_OnLoad(JavaVM* jvm, char* options, void* reserved) { return 0; }
...
static UDATA addBFUSystemProperties(J9JavaVM* javaVM)
{
    ...
    if (J9SYSPROP_ERROR_NOT_FOUND == vmfunc->getSystemProperty(javaVM, "java.util.prefs.PreferencesFactory", NULL)) {
        rc = vmfunc->addSystemProperty(
            javaVM, "java.util.prefs.PreferencesFactory", "java.util.prefs.FileSystemPreferencesFactory", 0);
        if (J9SYSPROP_ERROR_NONE != rc) {
            return rc;
        }
    }
...
jint scarInit(J9JavaVM* vm)
{
    ...
    /* Preload this dependency from the JVM directories so the system does not load it from system path, as the wrong
     * version could be found. */
    result = (jint)vmFuncs->registerBootstrapLibrary(vm->mainThread, dbgwrapperStr, (J9NativeLibrary**)&handle, FALSE);
    if (0 != result) {
        PORT_ACCESS_FROM_JAVAVM(vm);
        /* If library can not be found, print a warning and proceed anyway as the dependency on this library may have
         * been removed. */
        j9nls_printf(PORTLIB, J9NLS_WARNING, J9NLS_JCL_WARNING_DLL_COULDNOT_BE_REGISTERED_AS_BOOTSTRAP_LIB,
            dbgwrapperStr, result);
    }
...
J9VMDllMain(J9JavaVM* vm, IDATA stage, void* reserved)
{
    ...
    case ALL_LIBRARIES_LOADED:
        ...
        vm->jclFlags |= J9_JCL_FLAG_REFERENCE_OBJECTS | J9_JCL_FLAG_FINALIZATION | J9_JCL_FLAG_THREADGROUPS;
 ...
 * @param cursor
 *
 * @return Number of entries added on success, negative error code on failure (-1 on malloc failure, -2 on too many
 * entries).
                ...
                if ('\0' != *currentEntry) {
                    if (MAX_PROPSFILE_BOOTPATH_ENTRIES <= count) {
                        j9tty_printf(PORTLIB,
                            "Error: MAX_PROPSFILE_BOOTPATH_ENTRIES (%d) exceeded in classlib.properties file.\n",
...
#define SE_SERVICE_JAR "se-service.jar"
    {
        int serviceJarPathLength
            = 1 + javaHomePathLength + 1 + LITERAL_STRLEN("lib") + 1 + LITERAL_STRLEN(SE_SERVICE_JAR) + 1;
