...
static jint callFindPattern(
    U_8* pattern, jint patternLength, jint patternAlignment, jlong startSearchFrom, jlong* resultP);
...
static jboolean validateDump(JNIEnv* env)
{
    ...
    for (;;) {
        ...
        if (eyecatcher == (jlong)-1) {
            j9str_printf(PORTLIB, errBuf, sizeof(errBuf),
                "JVM anchor block (J9VMRAS) not found in dump. Dump may be truncated, corrupted or contains a "
                "partially initialized JVM.");
        ...
        if ((U_64)eyecatcher > (U_64)0xFFFFFFFF) {
            j9str_printf(PORTLIB, errBuf, sizeof(errBuf),
                "J9RAS is out of range for a 32-bit pointer (0x%16.16llx). This version of jextract is incompatible "
                "with this dump.",
                eyecatcher);
            (*env)->ThrowNew(env, errorClazz, errBuf);
            return JNI_FALSE;
        ...
        if (ras != NULL) {
            if (ras->bitpattern1 == 0xaa55aa55 && ras->bitpattern2 == 0xaa55aa55) {
                if (ras->version != J9RASVersion) {
                    j9str_printf(PORTLIB, errBuf, sizeof(errBuf),
                        "J9RAS.version is incorrect (found %u, expecting %u). This version of jextract is incompatible "
                        "with this dump.",
                        ras->version, J9RASVersion);
                    (*env)->ThrowNew(env, errorClazz, errBuf);
                    return JNI_FALSE;
                }
                if (ras->length != sizeof(J9RAS)) {
                    j9str_printf(PORTLIB, errBuf, sizeof(errBuf),
                        "J9RAS.length is incorrect (found %u, expecting %u). This version of jextract is incompatible "
                        "with this dump.",
                        ras->length, sizeof(J9RAS));
                    (*env)->ThrowNew(env, errorClazz, errBuf);
                    return JNI_FALSE;
                }
                if (ras->buildID != J9UniqueBuildID) {
                    j9str_printf(PORTLIB, errBuf, sizeof(errBuf),
                        "J9RAS.buildID is incorrect (found %llx, expecting %llx). This version of jextract is "
                        "incompatible with this dump.",
                        ras->buildID, (U_64)J9UniqueBuildID);
                    (*env)->ThrowNew(env, errorClazz, errBuf);
                    return JNI_FALSE;
                }
    ...
    U_8* pattern, jint patternLength, jint patternAlignment, jlong startSearchFrom, jlong* resultP)
{
    ...
    result = (*globalEnv)
                 ->CallLongMethod(globalEnv, globalDumpObj, globalFindPatternMid, patternArray, (jint)patternAlignment,
...
static void callGetMemoryBytes(UDATA address, void* structure, UDATA size, UDATA* bytesRead)
{
    ...
    *bytesRead = 0;
    ...
    if ((*globalEnv)->ExceptionCheck(globalEnv)) {
        ...
        (*globalEnv)->ExceptionClear(globalEnv);
        /* note that the error cases where we are missing an exception, a class or printStackTrace, we have no good
         * solution */
        ...
        /* if we throw while trying to print a stack trace, all bets are off in terms of how to handle that exception so
         * just clear it */
        (*globalEnv)->ExceptionClear(globalEnv);
