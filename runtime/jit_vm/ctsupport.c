...
#define J9_IS_SPLIT_TABLE_INDEX(value) \
    J9_ARE_ANY_BITS_SET(value, J9_STATIC_SPLIT_TABLE_INDEX_FLAG | J9_SPECIAL_SPLIT_TABLE_INDEX_FLAG)
...
const J9NameAndSignature newInstancePrototypeNameAndSig
    = { (J9UTF8*)&newInstancePrototypeName, (J9UTF8*)&newInstancePrototypeSig };
 ...
 */
J9Class* jitGetClassInClassloaderFromUTF8(
    J9VMThread* vmStruct, J9ClassLoader* classLoader, void* signatureChars, UDATA signatureLength)
{
    J9Class* result = NULL;

    if (0 != signatureLength) {
        result = vmStruct->javaVM->internalVMFunctions->internalFindClassUTF8(
            vmStruct, signatureChars, signatureLength, classLoader, J9_FINDCLASS_FLAG_EXISTING_ONLY);
    }
...
J9Class* jitGetClassOfFieldFromCP(J9VMThread* vmStruct, J9ConstantPool* constantPool, UDATA fieldIndex)
{
...
void jitAcquireClassTableMutex(J9VMThread* vmThread) { omrthread_monitor_enter(vmThread->javaVM->classTableMutex); }

/**
 * @param vmTread
 */

void jitReleaseClassTableMutex(J9VMThread* vmThread) { omrthread_monitor_exit(vmThread->javaVM->classTableMutex); }
...
jitGetFieldType(UDATA fieldIndex, J9Method* ramMethod)
{
    ...
    switch (sigChar) {
    case 'Z':
        result = J9FieldTypeBoolean;
        break;
    case 'C':
        result = J9FieldTypeChar;
        break;
    case 'F':
        result = J9FieldTypeFloat;
        break;
    case 'D':
        result = (J9FieldSizeDouble + J9FieldTypeDouble);
        break;
    case 'B':
        result = J9FieldTypeByte;
        break;
    case 'S':
        result = J9FieldTypeShort;
        break;
    case 'I':
        result = J9FieldTypeInt;
        break;
    case 'J':
        result = (J9FieldSizeDouble + J9FieldTypeLong);
        break;
    default:
        result = J9FieldFlagObject;
        break;
    }
 ...
 */
void jitParseSignature(const J9UTF8* signature, U_8* paramBuffer, UDATA* paramElements, UDATA* paramSlots)
{
        ...
        if (')' == *sigChar) {
            ...
            *paramSlots = slotCount;
            ...
            case '[':
                ...
                break;
            case 'Z':
                next = J9_NATIVE_TYPE_BOOLEAN;
                break;
            case 'B':
                next = J9_NATIVE_TYPE_BYTE;
                break;
            case 'S':
                next = J9_NATIVE_TYPE_SHORT;
                break;
            case 'C':
                next = J9_NATIVE_TYPE_CHAR;
                break;
            case 'I':
                next = J9_NATIVE_TYPE_INT;
                break;
            case 'J':
 ...
 */
UDATA
jitCTInstanceOf(J9Class* instanceClass, J9Class* castClass) { return instanceOfOrCheckCast(instanceClass, castClass); }
    ...
    J9VMThread* vmStruct, J9Method* method, UDATA fieldIndex, UDATA resolveFlags, J9ROMFieldShape** resolvedField)
{
    ...
    if (J9_ARE_NO_BITS_SET(vm->extendedRuntimeFlags, J9_EXTENDED_RUNTIME_JIT_INLINE_WATCHES)) {
        ALWAYS_TRIGGER_J9HOOK_JIT_CHECK_FOR_DATA_BREAKPOINT(vm->jitConfig->hookInterface, vmStruct,
            result, /* can be modified */
            fieldIndex, constantPool, *resolvedField, 0, resolveFlags);
    }
...
void* jitCTResolveStaticFieldRefWithMethod(
    J9VMThread* vmStruct, J9Method* method, UDATA fieldIndex, UDATA resolveFlags, J9ROMFieldShape** resolvedField)
{
    J9JavaVM* vm = vmStruct->javaVM;
    J9ConstantPool* constantPool = J9_CP_FROM_METHOD(method);
    ...
    if (J9_ARE_NO_BITS_SET(vm->extendedRuntimeFlags, J9_EXTENDED_RUNTIME_JIT_INLINE_WATCHES)) {
        ALWAYS_TRIGGER_J9HOOK_JIT_CHECK_FOR_DATA_BREAKPOINT(vm->jitConfig->hookInterface, vmStruct,
            result, /* can be modified */
            fieldIndex, constantPool, *resolvedField, 1, resolveFlags);
    }
 ...
 */
void jitResetAllMethodsAtStartup(J9VMThread* vmStruct)
{
 ...
 */
static UDATA findField(
    J9VMThread* vmStruct, J9ConstantPool* constantPool, UDATA index, BOOLEAN isStatic, J9Class** declaringClass)
{
    J9JavaVM* javaVM = vmStruct->javaVM;
 ...
 * If 'cpOrSplitIndex' is an index into split table, J9Method is obtained by accessing the split table stored in
 * J9Class. Otherwise, J9Method is obtained by accessing the constant pool of J9Class.
 *
 * @param currentThread		The current J9VMThread
 ...
 */
J9Method* jitGetJ9MethodUsingIndex(J9VMThread* currentThread, J9ConstantPool* constantPool, UDATA cpOrSplitIndex)
{
 ...
 */
J9Method* jitResolveStaticMethodRef(
    J9VMThread* vmStruct, J9ConstantPool* constantPool, UDATA cpOrSplitIndex, UDATA resolveFlags)
{
    J9Method* method = NULL;

    if (J9_IS_STATIC_SPLIT_TABLE_INDEX(cpOrSplitIndex)) {
        method = vmStruct->javaVM->internalVMFunctions->resolveStaticSplitMethodRef(
            vmStruct, constantPool, (cpOrSplitIndex & J9_SPLIT_TABLE_INDEX_MASK), resolveFlags);
    } else {
        method = vmStruct->javaVM->internalVMFunctions->resolveStaticMethodRef(
            vmStruct, constantPool, cpOrSplitIndex, resolveFlags);
    }
    return method;
}
 ...
 * @return 					J9Method obtained by resolving 'cpOrSplitIndex'
 */
J9Method* jitResolveSpecialMethodRef(
    J9VMThread* vmStruct, J9ConstantPool* constantPool, UDATA cpOrSplitIndex, UDATA resolveFlags)
{
    J9Method* method = NULL;

    if (J9_IS_SPECIAL_SPLIT_TABLE_INDEX(cpOrSplitIndex)) {
        method = vmStruct->javaVM->internalVMFunctions->resolveSpecialSplitMethodRef(
            vmStruct, constantPool, (cpOrSplitIndex & J9_SPLIT_TABLE_INDEX_MASK), resolveFlags);
    } else {
        method = vmStruct->javaVM->internalVMFunctions->resolveSpecialMethodRef(
            vmStruct, constantPool, cpOrSplitIndex, resolveFlags);
    }
    return method;
}
