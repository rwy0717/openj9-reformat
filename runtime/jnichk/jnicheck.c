...
static IDATA jniDecodeValue(
    J9VMThread* vmThread, UDATA sigChar, void* valuePtr, char** outputBuffer, UDATA* outputBufferLength);
...
static void jniCheckCall(
    const char* function, JNIEnv* env, jobject receiver, UDATA methodType, UDATA returnType, jmethodID method);
...
static char* jniCheckObjectArg(
    const char* function, JNIEnv* env, jobject aJobject, char* sigArgs, UDATA argNum, UDATA trace);
static void jniTraceFieldID(JNIEnv* env, jfieldID fid);
static void jniIsLocalRefOSlotWalkFunction(
    J9VMThread* aThread, J9StackWalkState* walkState, j9object_t* slot, const void* stackLocation);
...
J9VMDllMain(J9JavaVM* vm, IDATA stage, void* reserved)
{
    ...
    case ALL_VM_ARGS_CONSUMED:
        ...
        if ((*hook)->J9HookRegisterWithCallSite(
                hook, J9HOOK_VM_NATIVE_METHOD_ENTER, methodEnterHook, OMR_GET_CALLSITE(), NULL)) {
            j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_JNICHK_UNABLE_TO_HOOK);
            return J9VMDLLMAIN_FAILED;
        }

        if ((*hook)->J9HookRegisterWithCallSite(
                hook, J9HOOK_VM_NATIVE_METHOD_RETURN, methodExitHook, OMR_GET_CALLSITE(), NULL)) {
            j9tty_err_printf(PORTLIB, "<JNI check utility: unable to hook event>\n");
            return J9VMDLLMAIN_FAILED;
        }

        vm->checkJNIData.jniGlobalRefHashTab
            = hashTableNew(OMRPORT_FROM_J9PORT(PORTLIB), J9_GET_CALLSITE(), 0, /* default table size */
                sizeof(JNICHK_GREF_HASHENTRY), /* entry size */
                0, /* default entry alignment */
                0, /* flags */
                OMRMEM_CATEGORY_VM, (J9HashTableHashFn)globrefHashTableHashFn,
                (J9HashTableEqualFn)globrefHashTableEqualFn, NULL, NULL);
...
static IDATA jniCheckProcessCommandLine(J9JavaVM* vm, J9VMDllLoadInfo* loadInfo)
{
    char* options = "";
    char* globalOptions = "";
    IDATA rc;
...
static IDATA jniCheckParseOptions(J9JavaVM* vm, char* options)
{
    PORT_ACCESS_FROM_JAVAVM(vm);
...
void jniCheckClass(
    JNIEnv* env, const char* function, IDATA argNum, jobject aJobject, J9Class* expectedClass, const char* expectedType)
{
    ...
    const U_32* descriptor, JNIEnv* env, ...)
{
    ...
    if (exceptionSafe == 0) {
        ...
        if (exceptionSetter != NULL) {
            jniCheckWarningNLS(env, J9NLS_JNICHK_MISSING_EXCEPTION_CHECK, function, exceptionSetter, function);
        }
    }
    ...
    while (*code) {
        ...
        case JNIC_REFLECTMETHOD:
            ...
            jniCheckNull(env, function, argNum, aJobject);
            jniCheckRef(env, function, argNum, aJobject);
            /* jniCheckSubclass(env, function, argNum, aJobject, "java/lang/reflect/Constructor" or
        ...
        case JNIC_WEAKREF:
            aJobject = va_arg(va, jobject);
            if (aJobject != NULL)
                jniCheckWeakGlobalRef(env, function, argNum, aJobject);
            if (trace)
                jniTraceObject(env, aJobject);
            break;
        ...
        case JNIC_GLOBALREF:
            aJobject = va_arg(va, jobject);
            if (aJobject != NULL)
                jniCheckGlobalRef(env, function, argNum, aJobject);
            if (trace)
                jniTraceObject(env, aJobject);
            break;
        ...
        case JNIC_LOCALREF:
            aJobject = va_arg(va, jobject);
            if (aJobject != NULL)
                jniCheckLocalRef(env, function, argNum, aJobject);
            if (trace)
                jniTraceObject(env, aJobject);
            break;
...
void jniVerboseGetID(const char* function, JNIEnv* env, jclass classRef, const char* name, const char* sig)
{
    J9VMThread* vmThread = (J9VMThread*)env;
    ...
    if (vmThread->javaVM->checkJNIData.options & JNICHK_VERBOSE) {
        ...
        j9tty_printf(
            PORTLIB, "<JNI %s: %.*s.%s %s>\n", function, J9UTF8_LENGTH(className), J9UTF8_DATA(className), name, sig);
    }
}
...
static void printJnichkHelp(J9PortLibrary* portLib)
{
    ...
    j9file_printf(PORTLIB, J9PORT_TTY_OUT,
        j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_JNICHK_HELP_1, NULL), J9JVM_VERSION_STRING);
    j9file_printf(PORTLIB, J9PORT_TTY_OUT, J9_COPYRIGHT_STRING "\n\n");
    j9file_printf(
        PORTLIB, J9PORT_TTY_OUT, j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_JNICHK_HELP_2, NULL));
    j9file_printf(PORTLIB, J9PORT_TTY_OUT, "\n");
    j9file_printf(
        PORTLIB, J9PORT_TTY_OUT, j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_JNICHK_HELP_3, NULL));
    j9file_printf(
        PORTLIB, J9PORT_TTY_OUT, j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_JNICHK_HELP_4, NULL));
    j9file_printf(
        PORTLIB, J9PORT_TTY_OUT, j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_JNICHK_HELP_5, NULL));
    j9file_printf(
        PORTLIB, J9PORT_TTY_OUT, j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_JNICHK_HELP_6, NULL));
    j9file_printf(
        PORTLIB, J9PORT_TTY_OUT, j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_JNICHK_HELP_7, NULL));
    j9file_printf(
        PORTLIB, J9PORT_TTY_OUT, j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_JNICHK_HELP_8, NULL));
    j9file_printf(
        PORTLIB, J9PORT_TTY_OUT, j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_JNICHK_HELP_14, NULL));
    j9file_printf(
        PORTLIB, J9PORT_TTY_OUT, j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_JNICHK_HELP_9, NULL));
    j9file_printf(
        PORTLIB, J9PORT_TTY_OUT, j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_JNICHK_HELP_15, NULL));
    j9file_printf(
        PORTLIB, J9PORT_TTY_OUT, j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_JNICHK_HELP_10, NULL));
    j9file_printf(
        PORTLIB, J9PORT_TTY_OUT, j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_JNICHK_HELP_13, NULL));
    j9file_printf(
        PORTLIB, J9PORT_TTY_OUT, j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_JNICHK_HELP_11, NULL));
    j9file_printf(
        PORTLIB, J9PORT_TTY_OUT, j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_JNICHK_HELP_12, NULL));
    j9file_printf(PORTLIB, J9PORT_TTY_OUT, "\n");
}
...
static void jniCheckPrintMethod(JNIEnv* env, U_32 level)
{
    ...
    if (method != NULL) {
        ...
        if (isLoadLibraryWithPath(className, name)) {
            ...
            case J9NLS_ERROR:
            default:
                j9nls_printf(PORTLIB, level, J9NLS_JNICHK_ERROR_IN_METHOD, J9UTF8_LENGTH(className),
                    J9UTF8_DATA(className), J9UTF8_LENGTH(name), J9UTF8_DATA(name), J9UTF8_LENGTH(sig),
                    J9UTF8_DATA(sig));
                break;
            case J9NLS_WARNING:
                j9nls_printf(PORTLIB, level, J9NLS_JNICHK_WARNING_IN_METHOD, J9UTF8_LENGTH(className),
                    J9UTF8_DATA(className), J9UTF8_LENGTH(name), J9UTF8_DATA(name), J9UTF8_LENGTH(sig),
                    J9UTF8_DATA(sig));
                break;
            case J9NLS_INFO:
                j9nls_printf(PORTLIB, level, J9NLS_JNICHK_ADVICE_IN_METHOD, J9UTF8_LENGTH(className),
                    J9UTF8_DATA(className), J9UTF8_LENGTH(name), J9UTF8_DATA(name), J9UTF8_LENGTH(sig),
                    J9UTF8_DATA(sig));
                break;
            }
...
static UDATA jniIsLocalRef(JNIEnv* currentEnv, JNIEnv* env, jobject reference)
{
    ...
    if (vmThread->javaVM->checkJNIData.options & JNICHK_PEDANTIC) {
        ...
        if (((U_8*)reference >= (U_8*)vmThread->sp && (U_8*)reference <= bp)
...
void jniCheckRef(JNIEnv* env, const char* function, IDATA argNum, jobject reference)
{
    ...
    if (jniIsLocalRef(env, env, reference) || jniIsGlobalRef(env, reference) || jniIsWeakGlobalRef(env, reference)) {
        return;
    } else {
        ...
        if (argNum == -1) {
            ...
            jniCheckFatalErrorNLS(
                env, J9NLS_JNICHK_ARGUMENT_IS_NOT_REF, function, argNum, reference, getRefType(env, reference));
        }
    }
}
...
void jniCheckLocalRef(JNIEnv* env, const char* function, IDATA argNum, jobject reference)
{
    ...
    if (jniIsLocalRef(env, env, reference)) {
        ...
        jniCheckFatalErrorNLS(
            env, J9NLS_JNICHK_ARGUMENT_IS_NOT_LOCAL_REF, function, argNum, reference, getRefType(env, reference));
    }
}
...
void jniCheckGlobalRef(JNIEnv* env, const char* function, IDATA argNum, jobject reference)
{
    ...
    if (jniIsGlobalRef(env, reference)) {
        ...
        jniCheckFatalErrorNLS(
            env, J9NLS_JNICHK_ARGUMENT_IS_NOT_GLOBAL_REF, function, argNum, reference, getRefType(env, reference));
    }
}
...
void jniCheckWeakGlobalRef(JNIEnv* env, const char* function, IDATA argNum, jobject reference)
{
    ...
    if (jniIsWeakGlobalRef(env, reference)) {
        ...
        jniCheckFatalErrorNLS(
            env, J9NLS_JNICHK_ARGUMENT_IS_NOT_WEAK_GLOBAL_REF, function, argNum, reference, getRefType(env, reference));
    }
}
...
static void jniCheckValidClass(JNIEnv* env, const char* function, UDATA argNum, jobject obj)
{
    ...
    if (classDepthAndFlags & J9_JAVA_CLASS_HOT_SWAPPED_OUT) {
        ...
        jniCheckFatalErrorNLS(
            env, J9NLS_JNICHK_OBSOLETE_CLASS, function, J9UTF8_LENGTH(className), J9UTF8_DATA(className));
    }
}
...
void jniCheckLocalRefTracking(JNIEnv* env, const char* function, J9JniCheckLocalRefState* savedState)
{
    ...
    if ((vm->checkJNIData.options & JNICHK_NOWARN))
        return;
    ...
    if (currentState.globalRefCapacity > savedState->globalRefCapacity) {
        jniCheckWarningNLS(env, J9NLS_JNICHK_GREW_GLOBAL_REF_POOL, function, savedState->globalRefCapacity,
            currentState.globalRefCapacity);
    }
    if (currentState.weakRefCapacity > savedState->weakRefCapacity) {
        jniCheckWarningNLS(env, J9NLS_JNICHK_GREW_WEAK_GLOBAL_REF_POOL, function, savedState->weakRefCapacity,
    ...
    jniCheckWarningNLS(env, J9NLS_JNICHK_GREW_LOCAL_REF_FRAME, function, savedState->numLocalRefs,
        currentState.topFrameCapacity + J9_SSF_CO_REF_SLOT_CNT, currentState.numLocalRefs);
}
...
static void methodEnterHook(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (trace) {
        ...
        j9tty_printf(PORTLIB, "%p %*sCall JNI: %.*s.%.*s%.*s {\n", vmThread, indent * 2, "",
            (U_32)J9UTF8_LENGTH(classNameUTF), J9UTF8_DATA(classNameUTF), (U_32)J9UTF8_LENGTH(nameUTF),
            J9UTF8_DATA(nameUTF), (U_32)J9UTF8_LENGTH(sigUTF), J9UTF8_DATA(sigUTF));
        ++indent;
    ...
    J9VMThread* vmThread, UDATA sigChar, void* valuePtr, char** outputBuffer, UDATA* outputBufferLength)
{
    ...
    case 'S':
        ...
        break;
    case 'Z':
        written = j9str_printf(
            PORTLIB, *outputBuffer, *outputBufferLength, "(jboolean)%s", *((I_32*)valuePtr) ? "true" : "false");
        break;
    case 'L':
...
void jniCallInReturn_void(JNIEnv* env, void* vaptr) { jniCallInReturn((J9VMThread*)env, vaptr, NULL, 'V'); }
...
static void jniCallIn(J9VMThread* vmThread)
{
    ...
    if (trace) {
        omrthread_tls_set(vmThread->osThread, jniEntryCountKey,
            (void*)(((UDATA)omrthread_tls_get(vmThread->osThread, jniEntryCountKey)) + 1));
    }
}
...
static void jniCallInReturn(J9VMThread* vmThread, void* vaptr, void* stackResult, UDATA returnType)
{
    ...
    if (trace) {
        ...
        argBuffer[sizeof(argBuffer) - 1] = '\0';
        j9tty_printf(PORTLIB, "%p %*sReturn: %s\n", vmThread, indent * 2, "",
            (vmThread->currentException ? "<exception>" : argBuffer));
        --indent;
        omrthread_tls_set(vmThread->osThread, jniEntryCountKey, (void*)indent);
        j9tty_printf(PORTLIB, "%p %*s}\n", vmThread, indent * 2, "");
    }
...
static int isLoadLibraryWithPath(J9UTF8* className, J9UTF8* methodName)
{
    ...
    return (J9UTF8_LENGTH(className) == classNameLen) && (J9UTF8_LENGTH(methodName) == methodNameLen)
...
{
    J9VMThread* vmThread = (J9VMThread*)env;
    UDATA refBytes = (((J9SFMethodFrame*)((U_8*)vmThread->sp + (UDATA)vmThread->literals))->specialFrameFlags
                         & J9_SSF_JNI_PUSHED_REF_COUNT_MASK)
        * sizeof(UDATA);
...
void jniCheckDirectBuffer(JNIEnv* env, const char* function, IDATA argNum, jobject aJobject)
{
    ...
    /* either we failed to find java/nio/Buffer or aJobject is not a java/nio/Buffer. Either way, it can't be a
     * java/nio/Buffer */
...
static UDATA globrefHashTableHashFn(void* entry, void* userData) { return ((JNICHK_GREF_HASHENTRY*)entry)->reference; }
