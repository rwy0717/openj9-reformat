...
static UDATA checkArrayCrc(JNIEnv* env, const char* acquireFunction, const char* releaseFunction, jobject object,
...
jint jniCheckMemoryInit(J9JavaVM* javaVM)
{
    ...
    if (MemPoolGlobal == NULL) {
        MemPoolGlobal = pool_new(sizeof(MemRecord), 0, 0, 0, J9_GET_CALLSITE(), OMRMEM_CATEGORY_VM,
        ...
        if (poolElement->env == env && poolElement->memory == memory) {
            ...
            if (!jniCheckIsSameObject(env, poolElement->object, object)) {
                jniCheckFatalErrorNLS(
                    env, J9NLS_JNICHK_WRONG_OBJECT, releaseFunction, memory, poolElement->originalObject, object);
            } else if (strcmp(poolElement->acquireFunction, acquireFunction) != 0) {
                jniCheckFatalErrorNLS(env, J9NLS_JNICHK_WRONG_FUNCTION, releaseFunction, releaseFunction, memory,
                    poolElement->acquireFunction);
            } else {
                if (checkCRC) {
                    isCopy = checkArrayCrc(env, acquireFunction, releaseFunction, object, memory, mode, poolElement);
                }
            }
    ...
    const void* memory, jint mode, MemRecord* poolElement)
{
    ...
    if (!isCopy) {
        if (mode == JNI_COMMIT) {
            jniCheckAdviceNLS(env, J9NLS_JNICHK_IGNORING_JNI_COMMIT, releaseFunction, poolElement->acquireFunction);
        } else if (mode == JNI_ABORT && bufCrc != poolElement->crc) {
            jniCheckAdviceNLS(env, J9NLS_JNICHK_IGNORING_JNI_ABORT, releaseFunction, poolElement->crc, bufCrc,
                poolElement->acquireFunction);
        }
    } else {
        switch (mode) {
        ...
        case JNI_COMMIT:
            if (isCopy && arrayCrc != poolElement->crc) {
                jniCheckWarningNLS(env, J9NLS_JNICHK_ARRAY_DATA_MODIFIED, releaseFunction, poolElement->acquireFunction,
                    releaseFunction, poolElement->crc, arrayCrc, poolElement->acquireFunction);
            }
        }
    }
    ...
    while (poolElement) {
        if (poolElement->env == env) {
            if (poolElement->frameOffset == frameOffset) {
                jniCheckWarningNLS(
                    env, J9NLS_JNICHK_ARRAY_MEMORY_NOT_RELEASED, poolElement->memory, poolElement->acquireFunction);
