...
IDATA setupArguments(
    struct j9cmdlineOptions* startupOptions, JavaVMInitArgs* vm_args, void** vmOptionsTable, jint version);
...
static int testInvocation(J9PortLibrary* portLib, struct j9cmdlineOptions* args, int argc, char** argv, UDATA handle,
    jint version, jboolean ignoreUnrecognized, int expectResult);
...
static int testAttachBehaviour(
    J9PortLibrary* portLib, struct j9cmdlineOptions* args, int argc, char** argv, UDATA handle);
 ...
 */
    ...
    if (testIFAOffload(PORTLIB, args, argc, argv, handle))
        return FAIL;
#endif
 ...
 * @param vmOptionsTable options table that can be used by the function to generate the options, this should be passed
 * to cleanupArguments once the test is complete to free allocated memory
 * @returns 0 on success, non-zero otherwise
 *
 */
IDATA setupArguments(
...
static void JNICALL exitHook(jint rc) { exitCode = rc; }
...
static int testIgnoreUnrecognized(J9PortLibrary* portLib, struct j9cmdlineOptions* args, UDATA handle)
{
    ...
    for (ignoreUnrec = 0; ignoreUnrec <= 1; ignoreUnrec++) {
        if (testInvocation(PORTLIB, args, 6, allRecognized, handle, JNI_VERSION_1_2, ignoreUnrec, 1))
            goto error;
    }

    testName = "unrecognized -X args";
    for (ignoreUnrec = 0; ignoreUnrec <= 1; ignoreUnrec++) {
        if (testInvocation(PORTLIB, args, 3, unRecognizedX, handle, JNI_VERSION_1_2, ignoreUnrec, ignoreUnrec))
            goto error;
    }

    testName = "unrecognized underscore args";
    for (ignoreUnrec = 0; ignoreUnrec <= 1; ignoreUnrec++) {
        if (testInvocation(PORTLIB, args, 3, unRecognizedUS, handle, JNI_VERSION_1_2, ignoreUnrec, ignoreUnrec))
            goto error;
    }

    testName = "completely invalid arguments";
    for (ignoreUnrec = 0; ignoreUnrec <= 1; ignoreUnrec++) {
        if (testInvocation(PORTLIB, args, 3, unRecognizedBogus, handle, JNI_VERSION_1_2, ignoreUnrec, 0))
            goto error;
    }
...
error:
    ...
    j9tty_printf(PORTLIB, "****** VM OUTPUT END ******\n\n");
    j9tty_printf(
        PORTLIB, "FAILED: IgnoreUnrecognized test \"%s\" failed with ignoreUnrecognized=%d", testName, ignoreUnrec);
...
static int isIFAOffloadEnabled()
{
    ...
    const U_32 TCBSTCB_OFFSET
        = 0x138; /* offset of the TCBSTCB field in the TCB.  This field contains a pointer to the STCB. */
    U_32 stcbBaseAddr; /* Address of a control block field which contains a pointer to the base of the Secondary Task
                          Control Block (STCB) */
    U_32 stcbBase; /* absolute address of the start of the STCB */
    const U_32 STCBWEB_OFFSET
        = 0xe8; /* offset of the STCBWEB field in the STCB.  This field contains a pointer to the WEB. */
    U_32 webBaseAddr; /* Address of a control block field which contains a pointer to the base of a Control Block owned
                         by Task Management (WEB) */
