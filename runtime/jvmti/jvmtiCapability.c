...
#endif
static IDATA mapCapabilitiesToEvents(
    J9JVMTIEnv* j9env, jvmtiCapabilities* capabilities, J9EventHookFunction eventHookFunction);
...
jvmtiError JNICALL jvmtiGetPotentialCapabilities(jvmtiEnv* env, jvmtiCapabilities* capabilities_ptr)
{
    ...
    /* Get the JVMTI mutex to ensure to prevent multple agents acquiring capabilities that can only be held by one agent
     * at a time */
    ...
    if (isEventHookable(j9env, JVMTI_EVENT_EXCEPTION) && isEventHookable(j9env, JVMTI_EVENT_EXCEPTION_CATCH)) {
    ...
    if (isEventHookable(j9env, JVMTI_EVENT_MONITOR_CONTENDED_ENTER)
        ...
        && isEventHookable(j9env, JVMTI_EVENT_MONITOR_WAIT) && isEventHookable(j9env, JVMTI_EVENT_MONITOR_WAITED)) {
        rv_capabilities.can_generate_monitor_events = 1;
    }

    rv_capabilities.can_get_owned_monitor_stack_depth_info = 1;

    if (((j9env->flags & (J9JVMTIENV_FLAG_CLASS_LOAD_HOOK_EVER_ENABLED | J9JVMTIENV_FLAG_RETRANSFORM_CAPABLE))
            != J9JVMTIENV_FLAG_CLASS_LOAD_HOOK_EVER_ENABLED)
        && ((*vmHook)->J9HookIsEnabled(vmHook, J9HOOK_VM_REQUIRED_DEBUG_ATTRIBUTES)
               || (vm->requiredDebugAttributes & J9VM_DEBUG_ATTRIBUTE_ALLOW_RETRANSFORM))) {
...
jvmtiError JNICALL jvmtiAddCapabilities(jvmtiEnv* env, const jvmtiCapabilities* capabilities_ptr)
{
    ...
    if (rc == JVMTI_ERROR_NONE) {
        ...
        /* Get the JVMTI mutex to ensure to prevent multple agents acquiring capabilities that can only be held by one
         * agent at a time */
...
jvmtiError JNICALL jvmtiRelinquishCapabilities(jvmtiEnv* env, const jvmtiCapabilities* capabilities_ptr)
{
    ...
    /* Get the JVMTI mutex to ensure to prevent multple agents releasing capabilities that can only be held by one agent
     * at a time */
