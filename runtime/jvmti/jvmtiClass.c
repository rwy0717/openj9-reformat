...
static jvmtiError redefineClassesCommon(jvmtiEnv* env, jint class_count, const jvmtiClassDefinition* class_definitions,
    J9VMThread* currentThread, UDATA options);
...
static jvmtiError jvmtiGetConstantPool_writeConstants(
    jvmtiGcp_translation* translation, unsigned char* constantPoolBuf);
static jvmtiError jvmtiGetConstantPool_addNAS(
    jvmtiGcp_translation* translation, J9ROMNameAndSignature* nas, U_32* sunCpIndex, U_32* refIndex);
static jvmtiError jvmtiGetConstantPool_addReference(
    jvmtiGcp_translation* translation, UDATA cpIndex, U_8 cpType, J9ROMFieldRef* ref, U_32* sunCpIndex);
static jvmtiError jvmtiGetConstantPool_addUTF8(
    jvmtiGcp_translation* translation, J9UTF8* utf8, U_32* sunCpIndex, U_32* refIndex);
static jvmtiError jvmtiGetConstantPool_addClassOrString(
    jvmtiGcp_translation* translation, UDATA cpIndex, U_8 cpType, J9UTF8* utf8, U_32* sunCpIndex, U_32* refIndex);
static jvmtiError jvmtiGetConstantPool_addIntFloat(
    jvmtiGcp_translation* translation, UDATA cpIndex, U_8 cpType, U_32 data, U_32* sunCpIndex);
static jvmtiError jvmtiGetConstantPool_addLongDouble(
    jvmtiGcp_translation* translation, UDATA cpIndex, U_8 cpType, U_32 slot1, U_32 slot2, U_32* sunCpIndex);
static jvmtiError jvmtiGetConstantPool_addMethodHandle(
    jvmtiGcp_translation* translation, UDATA cpIndex, U_8 cpType, J9ROMMethodHandleRef* ref, U_32* sunCpIndex);
static jvmtiError jvmtiGetConstantPool_addMethodType(
    jvmtiGcp_translation* translation, UDATA cpIndex, U_8 cpType, J9ROMMethodTypeRef* ref, U_32* sunCpIndex);
static jvmtiError jvmtiGetConstantPool_addNAS_name_sig(
    jvmtiGcp_translation* translation, void* key, J9UTF8* name, J9UTF8* sig, U_32* sunCpIndex, U_32* refIndex);
static jvmtiError jvmtiGetConstantPool_addInvokeDynamic(
    jvmtiGcp_translation* translation, UDATA key, J9ROMNameAndSignature* nameAndSig, U_32 bsmIndex, U_32* sunCpIndex);
 ...
 */
static jint jvmtiGetLoadedClassesCount(J9JavaVM* vm)
{
        ...
        if (classRefs == NULL) {
            ...
            while (clazz) {
                ...
                if (i == lastClassCount) {
                    ...
                    tempClassRefs
                        = j9mem_reallocate_memory(classRefs, lastClassCount * sizeof(jclass), J9MEM_CATEGORY_JVMTI);
                ...
                if (J9ROMCLASS_IS_PRIMITIVE_TYPE(clazz->romClass) == 0) {
                    if ((J9CLASS_FLAGS(clazz) & J9AccClassHotSwappedOut) == 0) {
                        classRefs[i++] = (jclass)vm->internalVMFunctions->j9jni_createLocalRef(
                            (JNIEnv*)currentThread, J9VM_J9CLASS_TO_HEAPCLASS(clazz));
                    }
                }
    ...
    if (rc == JVMTI_ERROR_NONE) {
        ...
        if (signature_ptr != NULL) {
            if (J9ROMCLASS_IS_PRIMITIVE_TYPE(clazz->romClass)) {
                ...
                signature[0] = (clazz->arrayClass == NULL)
                    ? 'V'
                    : J9UTF8_DATA(J9ROMCLASS_CLASSNAME(clazz->arrayClass->romClass))[1];
                signature[1] = '\0';
        ...
        if (fieldIDs == NULL) {
            ...
            while (NULL != result->field) {
                ...
                J9JNIFieldID* fieldID
                    = vmFuncs->getJNIFieldID(currentThread, clazz, result->field, result->offset, &inconsistentData);
        ...
        if (!J9ROMCLASS_IS_PRIMITIVE_OR_ARRAY(romClass)) {
            ...
            if (interfaces == NULL) {
                ...
                for (i = 0; i < interfaceCount; i++) {
                    ...
                    interfaceClass
                        = vm->internalVMFunctions->internalFindClassUTF8(currentThread, J9UTF8_DATA(interfaceName),
                            J9UTF8_LENGTH(interfaceName), clazz->classLoader, J9_FINDCLASS_FLAG_EXISTING_ONLY);
                    interfaces[i] = (jclass)vm->internalVMFunctions->j9jni_createLocalRef(
                        (JNIEnv*)currentThread, J9VM_J9CLASS_TO_HEAPCLASS(interfaceClass));
                }
            }
        }
...
jvmtiError JNICALL jvmtiGetClassLoader(jvmtiEnv* env, jclass klass, jobject* classloader_ptr)
{
    ...
    if (rc == JVMTI_ERROR_NONE) {
        ...
        if (classLoader == vm->systemClassLoader) {
            ...
            rv_classloader = vm->internalVMFunctions->j9jni_createLocalRef(
                (JNIEnv*)currentThread, J9CLASSLOADER_CLASSLOADEROBJECT(currentThread, classLoader));
        }
    done:
        vm->internalVMFunctions->internalExitVMToJNI(currentThread);
    }
...
static void fixWatchedFields(J9JavaVM* vm, J9HashTable* classHashTable)
{
    ...
    if (rc == JVMTI_ERROR_NONE) {
        ...
        if (rc == JVMTI_ERROR_NONE) {
            ...
            if (!extensionsEnabled) {
                ...
                rc = fixMethodEquivalences(
                    currentThread, classPairs, jitEventDataPtr, TRUE, &methodEquivalences, extensionsUsed);
                if (rc != JVMTI_ERROR_NONE) {
                    goto failed;
                }
                ...
                rc = fixMethodEquivalences(
                    currentThread, classPairs, jitEventDataPtr, FALSE, &methodEquivalences, extensionsUsed);
                if (rc != JVMTI_ERROR_NONE) {
                    goto failed;
                }
                /* Fix the vTables of all subclasses */
...
static void restoreBreakpointsInClasses(J9VMThread* currentThread, J9HashTable* classPairs)
{
    J9HashTableState hashTableState;
    J9JVMTIClassPair* classPair;
    J9JVMTIData* jvmtiData = J9JVMTI_DATA_FROM_VM(currentThread->javaVM);
...
jvmtiError JNICALL jvmtiRetransformClasses(jvmtiEnv* env, jint class_count, const jclass* classes)
{
    ...
    if (rc == JVMTI_ERROR_NONE) {
        ...
        /* Create a jvmtiClassDefinition for each retransformed class from the stored bytes and call the common redefine
         * helper */
        ...
        if (class_definitions == NULL) {
            ...
            for (i = 0; i < class_count; ++i) {
                ...
                if (!J9ROMCLASS_IS_INTERMEDIATE_DATA_A_CLASSFILE(clazz->romClass)) {
                    ...
                    /* -XX:-StoreIntermediateClassfile (enabled by default) : recreate class file bytes from
                     * intermediateROMClass pointed by current ROMClass */
                    result = vm->dynamicLoadBuffers->transformROMClassFunction(
                        vm, PORTLIB, intermediateROMClass, &classFileBytes, &classFileBytesCount);
            ...
            if (rc == JVMTI_ERROR_NONE) {
                rc = redefineClassesCommon(
                    env, class_count, class_definitions, currentThread, J9_FINDCLASS_FLAG_RETRANSFORMING);
            }
 ...
 * @param constant_pool_bytes_ptr 	On return, points to the raw constant pool, that is the bytes defined by the
 * constant_pool item of the Class File Format. Agent passes a pointer to a unsigned char*. On return, the unsigned
 * char* points to a newly allocated array of size *constant_pool_byte_count_ptr. The array should be freed with
 * Deallocate.
...
jvmtiError jvmtiGetConstantPool_translateCP(
    J9PortLibrary* privatePortLibrary, jvmtiGcp_translation* translation, J9Class* class, jboolean translateUTF8andNAS)
{
    U_32 sunCpIndex = 1;
    UDATA cpIndex, i;
    U_8 cpItemType;
    ...
    translation->ht = hashTableNew(OMRPORT_FROM_J9PORT(privatePortLibrary), J9_GET_CALLSITE(), 256,
        sizeof(jvmtiGcp_translationEntry), 0, 0, J9MEM_CATEGORY_JVMTI, jvmtiGetConstantPool_HashFn,
        jvmtiGetConstantPool_HashEqualFn, NULL, NULL);
    ...
    translation->cp
        = j9mem_allocate_memory((constantPoolCount * 6) * sizeof(jvmtiGcp_translationEntry*), J9MEM_CATEGORY_JVMTI);
    ...
    for (cpIndex = 1; cpIndex < constantPoolCount; cpIndex++) {
        ...
        case J9CPTYPE_ANNOTATION_UTF8:
            ...
            rc = jvmtiGetConstantPool_addClassOrString(translation, cpIndex,
                ...
                J9ROMSTRINGREF_UTF8DATA((J9ROMStringRef*)cpItem), &sunCpIndex, NULL);
            if (rc != JVMTI_ERROR_NONE) {
                JVMTI_ERROR(rc);
            }
            break;
        ...
        case J9CPTYPE_FIELD:
            ...
            rc = jvmtiGetConstantPool_addReference(
                translation, cpIndex, (U_8)CFR_CONSTANT_Fieldref, (J9ROMFieldRef*)cpItem, &sunCpIndex);
            if (rc != JVMTI_ERROR_NONE) {
                JVMTI_ERROR(rc);
            }
            break;
        ...
        case J9CPTYPE_INTERFACE_STATIC_METHOD: {
            ...
            if ((cpItemType == J9CPTYPE_INTERFACE_METHOD) || (cpItemType == J9CPTYPE_INTERFACE_STATIC_METHOD)
                || (cpItemType == J9CPTYPE_INTERFACE_INSTANCE_METHOD)) {
                cpType = (U_8)CFR_CONSTANT_InterfaceMethodref;
            }

            rc = jvmtiGetConstantPool_addReference(translation, cpIndex, cpType, (J9ROMFieldRef*)cpItem, &sunCpIndex);
            if (rc != JVMTI_ERROR_NONE) {
                JVMTI_ERROR(rc);
            }
            break;
        ...
        case J9CPTYPE_METHOD_TYPE:
            rc = jvmtiGetConstantPool_addMethodType(
                translation, cpIndex, (U_8)CFR_CONSTANT_MethodType, (J9ROMMethodTypeRef*)cpItem, &sunCpIndex);
        ...
        case J9CPTYPE_METHODHANDLE:
            ...
            rc = jvmtiGetConstantPool_addMethodHandle(
                translation, cpIndex, (U_8)CFR_CONSTANT_MethodHandle, (J9ROMMethodHandleRef*)cpItem, &sunCpIndex);
            if (rc != JVMTI_ERROR_NONE) {
                JVMTI_ERROR(rc);
            }
        ...
        case J9CPTYPE_FLOAT:
            ...
            rc = jvmtiGetConstantPool_addIntFloat(translation, cpIndex,
                ...
                ((J9ROMSingleSlotConstantRef*)cpItem)->data, &sunCpIndex);
            if (rc != JVMTI_ERROR_NONE) {
                JVMTI_ERROR(rc);
            }
            break;
        ...
        case J9CPTYPE_DOUBLE:
            rc = jvmtiGetConstantPool_addLongDouble(translation, cpIndex,
                ...
                ((J9ROMConstantRef*)cpItem)->slot1, ((J9ROMConstantRef*)cpItem)->slot2, &sunCpIndex);
            if (rc != JVMTI_ERROR_NONE) {
                JVMTI_ERROR(rc);
            }
            break;
    ...
    if (romClass->callSiteCount > 0) {
        ...
        for (callSiteIndex = 0; callSiteIndex < romClass->callSiteCount; callSiteIndex++) {
            ...
            rc = jvmtiGetConstantPool_addInvokeDynamic(translation,
                ...
                nameAndSig, bsmIndex, &sunCpIndex);
            if (rc != JVMTI_ERROR_NONE) {
                JVMTI_ERROR(rc);
            }
        }
    ...
    if (translateUTF8andNAS) {
        ...
        for (i = 1; i < translation->cpSize; i++) {
            ...
            case CFR_CONSTANT_String:
                ...
                rc = jvmtiGetConstantPool_addUTF8(
                    translation, htEntry->type.clazz.utf8, &sunCpIndex, &htEntry->type.clazz.nameIndex);
                if (rc != JVMTI_ERROR_NONE) {
                    JVMTI_ERROR(rc);
                }
            ...
            case CFR_CONSTANT_InterfaceMethodref:
                ...
                jvmtiGetConstantPoolTranslate_printf(("	Ref : Class [%*s] NAS [%*s][%*s]\n", J9UTF8_LENGTH(utf8),
                    J9UTF8_DATA(utf8), J9UTF8_LENGTH(J9ROMNAMEANDSIGNATURE_NAME(nas)),
                    J9UTF8_DATA(J9ROMNAMEANDSIGNATURE_NAME(nas)), J9UTF8_LENGTH(J9ROMNAMEANDSIGNATURE_SIGNATURE(nas)),
                ...
                rc = jvmtiGetConstantPool_addNAS(translation, nas, &sunCpIndex, &htEntry->type.ref.nameAndTypeIndex);
                if (rc != JVMTI_ERROR_NONE) {
                    return rc;
                }
            ...
            case CFR_CONSTANT_InvokeDynamic:
                ...
                jvmtiGetConstantPoolTranslate_printf(("	invokedynamic : bsmIndex [%i] NAS [%*s][%*s]\n",
                    htEntry->type.invokedynamic.bsmIndex, J9UTF8_LENGTH(J9ROMNAMEANDSIGNATURE_NAME(nas)),
                    J9UTF8_DATA(J9ROMNAMEANDSIGNATURE_NAME(nas)), J9UTF8_LENGTH(J9ROMNAMEANDSIGNATURE_SIGNATURE(nas)),
                    J9UTF8_DATA(J9ROMNAMEANDSIGNATURE_SIGNATURE(nas))));

                /* Add the referenced NameAndSignature item to the HT */
                rc = jvmtiGetConstantPool_addNAS(
                    translation, nas, &sunCpIndex, &htEntry->type.invokedynamic.nameAndTypeIndex);
                if (rc != JVMTI_ERROR_NONE) {
                    return rc;
                }
            ...
            case CFR_CONSTANT_MethodType: {
                ...
                case J9_METHOD_TYPE_ORIGIN_LDC:
                    ...
                    rc = jvmtiGetConstantPool_addUTF8(translation, htEntry->type.methodType.utf8, &sunCpIndex,
                        &htEntry->type.methodType.methodTypeIndex);
                    if (rc != JVMTI_ERROR_NONE) {
                        JVMTI_ERROR(rc);
                    }
                    break;
                ...
                case J9_METHOD_TYPE_ORIGIN_INVOKE_EXACT:
                     ...
                     * pass would be needed once the CFR_CONSTANT_Class case adds it) */
                    rc = jvmtiGetConstantPool_addClassOrString(translation, (UDATA)&MH_methodHandle_utf,
                        (U_8)CFR_CONSTANT_Class, (J9UTF8*)&MH_methodHandle_utf, &sunCpIndex,
                        &(htEntry->type.methodType.classIndex));
                    if (rc != JVMTI_ERROR_NONE) {
                        return rc;
                    }
                    ...
                    rc = jvmtiGetConstantPool_addNAS_name_sig(translation,
                        ...
                        (origin == J9_METHOD_TYPE_ORIGIN_INVOKE_EXACT ? (J9UTF8*)&MH_invokeExact_utf
                                                                      : (J9UTF8*)&MH_invoke_utf),
                        htEntry->type.methodType.utf8, &sunCpIndex, &htEntry->type.ref.nameAndTypeIndex);
                    if (rc != JVMTI_ERROR_NONE) {
                        return rc;
                    }
                    break;
...

{
    ...
    for (sunCpIndex = 1; sunCpIndex < translation->cpSize; sunCpIndex++) {
        ...
        jvmtiGetConstantPoolWrite_printf(("W %2d/%d type: %2d    [%3d/%3d]     ---  ", sunCpIndex, translation->cpSize,
            cpItemType, constantPoolBufIndex - constantPoolBuf, translation->totalSize));

        switch (cpItemType) {
        case CFR_CONSTANT_Class:
            jvmtiGetConstantPoolWrite_printf(
                ("        HT CPT %2d <Class> UTF8 %d->[%s]\n", htEntry->cpType, htEntry->type.clazz.nameIndex, ""));
        ...
        case CFR_CONSTANT_InterfaceMethodref:
            jvmtiGetConstantPoolWrite_printf(("        HT CPT %2d <Ref> Class %d  NAS %d\n", htEntry->cpType,
                htEntry->type.ref.classIndex, htEntry->type.ref.nameAndTypeIndex));
        ...
        case CFR_CONSTANT_MethodHandle:
            jvmtiGetConstantPoolWrite_printf(
                ("        HT CPT %2d <MethodHandle> method/fieldIndex %d  refType %d\n", htEntry->cpType,
                    htEntry->type.methodHandle.methodOrFieldRefIndex, htEntry->type.methodHandle.handleType));
            GCP_WRITE_U8(constantPoolBufIndex, cpItemType);
        ...
        case CFR_CONSTANT_MethodType:
            if (J9_METHOD_TYPE_ORIGIN_LDC
                == (htEntry->type.methodType.methodType->cpType >> J9DescriptionCpTypeShift)) {
        ...
        case CFR_CONSTANT_Long:
            jvmtiGetConstantPoolWrite_printf(("        HT CPT %2d <Double/Long> [0x%04x%04x]\n", htEntry->cpType,
                htEntry->type.longDouble.slot1, htEntry->type.longDouble.slot2));
        ...
        case CFR_CONSTANT_Utf8:
            jvmtiGetConstantPoolWrite_printf(("        HT CPT %2d <UTF8> [%*s]\n", htEntry->cpType,
                J9UTF8_LENGTH(htEntry->type.utf8.data), J9UTF8_DATA(htEntry->type.utf8.data)));
        ...
        case CFR_CONSTANT_NameAndType:
            ...
            jvmtiGetConstantPoolWrite_printf(("        HT CPT %2d <NAS> Name %d->[%s] Sig %d->[%s]\n", htEntry->cpType,
                htEntry->type.nas.nameIndex, "", htEntry->type.nas.signatureIndex, ""));
    ...
    jvmtiGcp_translation* translation, UDATA cpIndex, U_8 cpType, J9ROMMethodTypeRef* ref, U_32* sunCpIndex)
{
    ...
    if ((ref->cpType >> J9DescriptionCpTypeShift) == J9_METHOD_TYPE_ORIGIN_LDC) {
        ...
        rc = jvmtiGetConstantPool_addClassOrString(translation, (UDATA)&MH_methodHandle_utf, (U_8)CFR_CONSTANT_Class,
            (J9UTF8*)&MH_methodHandle_utf, sunCpIndex, NULL);
    }
    ...
    jvmtiGcp_translation* translation, UDATA cpIndex, U_8 cpType, J9ROMMethodHandleRef* ref, U_32* sunCpIndex)
{
    ...
    return jvmtiGetConstantPool_addReference(translation, ref->methodOrFieldRefIndex, fieldOrMethodCpType,
        (J9ROMFieldRef*)&translation->romConstantPool[ref->methodOrFieldRefIndex], sunCpIndex);
}

static jvmtiError jvmtiGetConstantPool_addNAS(
    jvmtiGcp_translation* translation, J9ROMNameAndSignature* nas, U_32* sunCpIndex, U_32* refIndex)
{
    return jvmtiGetConstantPool_addNAS_name_sig(
        translation, nas, J9ROMNAMEANDSIGNATURE_NAME(nas), J9ROMNAMEANDSIGNATURE_SIGNATURE(nas), sunCpIndex, refIndex);
}
    ...
    jvmtiGcp_translation* translation, void* key, J9UTF8* name, J9UTF8* sig, U_32* sunCpIndex, U_32* refIndex)
{
    ...
    jvmtiGetConstantPoolTranslate_printf(
