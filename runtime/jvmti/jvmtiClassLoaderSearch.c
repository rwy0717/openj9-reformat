...
jvmtiError JNICALL jvmtiAddToBootstrapClassLoaderSearch(jvmtiEnv* env, const char* segment)
{
    ...
    if (jvmtiData->phase == JVMTI_PHASE_ONLOAD) {
        rc = (jvmtiError)vm->internalVMFunctions->addToBootstrapClassLoaderSearch(
            vm, segment, CLS_TYPE_ADD_TO_SYSTEM_PROPERTY, FALSE);
    } else {
        rc = (jvmtiError)vm->internalVMFunctions->addToBootstrapClassLoaderSearch(
            vm, segment, CLS_TYPE_ADD_TO_SYSTEM_CLASSLOADER, TRUE);
    }
...
jvmtiError JNICALL jvmtiAddToSystemClassLoaderSearch(jvmtiEnv* env, const char* segment)
{
    ...
    if (jvmtiData->phase == JVMTI_PHASE_ONLOAD) {
        rc = (jvmtiError)vm->internalVMFunctions->addToSystemClassLoaderSearch(
            vm, segment, CLS_TYPE_ADD_TO_SYSTEM_PROPERTY, FALSE);
    } else {
        rc = (jvmtiError)vm->internalVMFunctions->addToSystemClassLoaderSearch(
            vm, segment, CLS_TYPE_ADD_TO_SYSTEM_CLASSLOADER, TRUE);
