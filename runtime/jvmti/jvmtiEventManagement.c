...
jvmtiError JNICALL jvmtiSetEventCallbacks(jvmtiEnv* env, const jvmtiEventCallbacks* callbacks, jint size_of_callbacks)
{
    ...
    /* Verify that the size is legal for the current JVMTI version, if callbacks is not NULL (NULL means remove all
     * callbacks) */
    ...
    /*	Copy/zero must take place a pointer at a time to avoid corrupting the function pointers (memcpy may copy
     * byte-wise) */
    ...
    jvmtiEnv* env, jvmtiEventMode mode, jvmtiEvent event_type, jthread event_thread, ...)
{
    ...
    if (rc == JVMTI_ERROR_NONE) {
        ...
        rc = setEventNotificationMode(
            j9env, currentThread, mode, event_type, event_thread, J9JVMTI_LOWEST_EVENT, J9JVMTI_HIGHEST_EVENT);
    ...
    if (jvmtiData->compileEventThread != NULL) {
        ...
        if (rc == JVMTI_ERROR_NONE) {
            ...
            if (event_type == JVMTI_EVENT_COMPILED_METHOD_LOAD) {
                ...
                classLoader
                    = vm->internalVMFunctions->allClassLoadersStartDo(&walkState, vm, J9CLASSLOADERWALK_INCLUDE_DEAD);
                while (NULL != classLoader) {
                    block = classLoader->jniRedirectionBlocks;
                    while (NULL != block) {
                        TRIGGER_J9HOOK_VM_DYNAMIC_CODE_LOAD(vm->hookInterface, currentThread, NULL, block,
                            J9JNIREDIRECT_BLOCK_SIZE, "JNI trampoline area", NULL);
                        block = block->next;
                    }
