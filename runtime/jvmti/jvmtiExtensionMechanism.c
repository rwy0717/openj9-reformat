...
static jvmtiError JNICALL jvmtiQueryVmDump(
    jvmtiEnv* jvmti_env, jint buffer_size, void* options_buffer, jint* data_size_ptr, ...);
static jvmtiError copyExtensionFunctionInfo(
    jvmtiEnv* env, jvmtiExtensionFunctionInfo* dest, const J9JVMTIExtensionFunctionInfo* source);
static jvmtiError JNICALL jvmtiJlmSet(jvmtiEnv* env, jint option, ...);
static jvmtiError copyExtensionEventInfo(
    jvmtiEnv* env, jvmtiExtensionEventInfo* dest, const J9JVMTIExtensionEventInfo* source);
...
static jvmtiError JNICALL jvmtiGetStackTraceExtended(jvmtiEnv* env, jint type, jthread thread, jint start_depth,
    jint max_frame_count, void* frame_buffer, jint* count_ptr, ...);
static jvmtiError JNICALL jvmtiGetAllStackTracesExtended(
    jvmtiEnv* env, jint type, jint max_frame_count, void** stack_info_ptr, jint* thread_count_ptr, ...);
static jvmtiError JNICALL jvmtiGetThreadListStackTracesExtended(jvmtiEnv* env, jint type, jint thread_count,
    const jthread* thread_list, jint max_frame_count, void** stack_info_ptr, ...);
static jvmtiError jvmtiInternalGetStackTraceExtended(jvmtiEnv* env, J9JVMTIStackTraceType type,
    J9VMThread* currentThread, J9VMThread* targetThread, jint start_depth, UDATA max_frame_count,
...
static jvmtiError JNICALL jvmtiIterateSharedCaches(jvmtiEnv* env, jint version, const char* cacheDir, jint flags,
    jboolean useCommandLineValues, jvmtiIterateSharedCachesCallback callback, void* user_data, ...);
static jvmtiError JNICALL jvmtiDestroySharedCache(jvmtiEnv* env, const char* cacheDir, const char* name, jint cacheType,
    jboolean useCommandLineValues, jint* internalErrorCode, ...);

static jvmtiError JNICALL jvmtiRemoveAllTags(jvmtiEnv* env, ...);

static jvmtiError JNICALL jvmtiRegisterTraceSubscriber(jvmtiEnv* env, char* description,
    jvmtiTraceSubscriber subscriber, jvmtiTraceAlarm alarm, void* userData, void** subscriptionID, ...);
...
static jvmtiError JNICALL jvmtiGetMethodAndClassNames(jvmtiEnv* env, void* ramMethods, jint ramMethodCount,
    jvmtiExtensionRamMethodData* ramMethodDataDescriptors, jchar* ramMethodStrings, jint* ramMethodStringsSize, ...);

static jvmtiError JNICALL jvmtiQueryVmLogOptions(
    jvmtiEnv* jvmti_env, jint buffer_size, void* options_buffer, jint* data_size, ...);
...
static UDATA jvmtiGetMemoryCategoriesCallback(U_32 categoryCode, const char* categoryName, UDATA liveBytes,
    UDATA liveAllocations, BOOLEAN isRoot, U_32 parentCategoryCode, OMRMemCategoryWalkState* state);
static UDATA jvmtiCountMemoryCategoriesCallback(U_32 categoryCode, const char* categoryName, UDATA liveBytes,
    UDATA liveAllocations, BOOLEAN isRoot, U_32 parentCategoryCode, OMRMemCategoryWalkState* state);
static UDATA jvmtiCalculateSlotsForCategoriesMappingCallback(U_32 categoryCode, const char* categoryName,
    UDATA liveBytes, UDATA liveAllocations, BOOLEAN isRoot, U_32 parentCategoryCode, OMRMemCategoryWalkState* state);
...
static jvmtiError JNICALL jvmtiGetMemoryCategories(jvmtiEnv* env, jint version, jint max_categories,
    jvmtiMemoryCategory* categories_buffer, jint* written_count_ptr, jint* total_categories_ptr, ...);

static jvmtiError JNICALL jvmtiRegisterVerboseGCSubscriber(jvmtiEnv* env, char* description,
    jvmtiVerboseGCSubscriber subscriber, jvmtiVerboseGCAlarm alarm, void* userData, void** subscriptionID, ...);
...
static jvmtiError JNICALL jvmtiRegisterTracePointSubscriber(jvmtiEnv* env, char* description,
    jvmtiTraceSubscriber subscriber, jvmtiTraceAlarm alarm, void* userData, void** subscriptionID, ...);
...
static const jvmtiParamInfo jvmtiTraceSet_params[] = { { "option", JVMTI_KIND_IN, JVMTI_TYPE_JINT, JNI_FALSE } };

/* (jvmtiEnv *jvmti_env, const char* option) */
static const jvmtiParamInfo jvmtiDumpSet_params[] = { { "option", JVMTI_KIND_IN_BUF, JVMTI_TYPE_CCHAR, JNI_FALSE } };

/* (jvmtiEnv *jvmti_env,  jint option) */
static const jvmtiParamInfo jvmtiJlmSet_params[] = { { "option", JVMTI_KIND_IN_PTR, JVMTI_TYPE_JINT, JNI_FALSE } };
...
/* (jvmtiEnv *jvmti_env, jint option) */
static const jvmtiParamInfo jvmtiVmDumpStart_params[] = { { "label", JVMTI_KIND_IN_BUF, JVMTI_TYPE_CCHAR, JNI_FALSE },
    { "event", JVMTI_KIND_IN_BUF, JVMTI_TYPE_CCHAR, JNI_FALSE },
    { "detail", JVMTI_KIND_IN_BUF, JVMTI_TYPE_CCHAR, JNI_TRUE } };

/* (jvmtiEnv *jvmti_env, jint option) */
static const jvmtiParamInfo jvmtiVmDumpEnd_params[] = { { "label", JVMTI_KIND_IN_BUF, JVMTI_TYPE_CCHAR, JNI_FALSE },
    { "event", JVMTI_KIND_IN_BUF, JVMTI_TYPE_CCHAR, JNI_FALSE },
    { "detail", JVMTI_KIND_IN_BUF, JVMTI_TYPE_CCHAR, JNI_TRUE } };
...
/* (jvmtiEnv *jvmti_env, jint version, const char *cacheDir, jboolean useCommandLineValues,
 * jvmtiIterateSharedCachesCallback *callback, void *user_data) */
...
/* (jvmtiEnv *jvmti_env, char *description, jvmtiTraceSubscriber *subscriber, jvmtiTraceAlarm *alarm, void *userData,
 * void **subscriptionID) */
...
/* (jvmtiEnv *jvmti_env, jvmtiTraceSubscriber *subscriber, void (*alarm)(void)) */
static const jvmtiParamInfo jvmtiGetTraceMetadata_params[] = { { "data", JVMTI_KIND_OUT, JVMTI_TYPE_CVOID, JNI_FALSE },
    { "length", JVMTI_KIND_OUT, JVMTI_TYPE_JINT, JNI_FALSE } };

/* (jvmtiEnv *jvmti_env,  void * ramMethods, jint ramMethodCount, jvmtiExtensionRamMethodData *
   ramMethodDataDescriptors, jchar * ramMethodStrings, jint * ramMethodDataDescriptorsCount) */
...
/* (jvmtiEnv* env, jint version, jint max_categories, jvmtiMemoryCategory * categories_buffer, jint * written_count_ptr,
...
/* (jvmtiEnv *env, char *description, jvmtiVerboseGCSubscriber subscriber, jvmtiVerboseGCAlarm alarm, void *userData,
 * void **subscriptionID) */
...
static const jvmtiParamInfo jvmtiGetJ9vmThread_params[] = { { "thread", JVMTI_KIND_IN, JVMTI_TYPE_JTHREAD, JNI_TRUE },
    { "vmThreadPtr", JVMTI_KIND_OUT, JVMTI_TYPE_CVOID, JNI_TRUE } };

static const jvmtiParamInfo jvmtiGetJ9method_params[] = { { "mid", JVMTI_KIND_IN, JVMTI_TYPE_JMETHODID, JNI_TRUE },
    { "j9MethodPtr", JVMTI_KIND_OUT, JVMTI_TYPE_CVOID, JNI_TRUE } };

/* (jvmtiEnv *env, char *description, jvmtiTraceSubscriber subscriber, jvmtiTraceAlarm alarm, void *userData, void
 * **subscriptionID) */
...
static const jvmtiError nullPointer_errors[] = { JVMTI_ERROR_NULL_POINTER };

static const jvmtiError notAvailable_errors[] = { JVMTI_ERROR_NOT_AVAILABLE };

static const jvmtiError ras_errors[]
    = { JVMTI_ERROR_NULL_POINTER, JVMTI_ERROR_OUT_OF_MEMORY, JVMTI_ERROR_INVALID_ENVIRONMENT, JVMTI_ERROR_WRONG_PHASE,
          JVMTI_ERROR_INTERNAL, JVMTI_ERROR_NOT_AVAILABLE, JVMTI_ERROR_ILLEGAL_ARGUMENT };
static const jvmtiError jlm_set_errors[]
    = { JVMTI_ERROR_NULL_POINTER, JVMTI_ERROR_WRONG_PHASE, JVMTI_ERROR_INTERNAL, JVMTI_ERROR_NOT_AVAILABLE };
...
static const jvmtiError get_os_thread_id_errors[]
    = { JVMTI_ERROR_WRONG_PHASE, JVMTI_ERROR_INVALID_THREAD, JVMTI_ERROR_THREAD_NOT_ALIVE, JVMTI_ERROR_NULL_POINTER };

static const jvmtiError jvmtiGetStack_errors[] = { JVMTI_ERROR_WRONG_PHASE, JVMTI_ERROR_INVALID_THREAD,
    JVMTI_ERROR_THREAD_NOT_ALIVE, JVMTI_ERROR_NULL_POINTER, JVMTI_ERROR_ILLEGAL_ARGUMENT, JVMTI_ERROR_OUT_OF_MEMORY };

static const jvmtiError jvmtiIterateSharedCaches_errors[] = { JVMTI_ERROR_NULL_POINTER, JVMTI_ERROR_OUT_OF_MEMORY,
    JVMTI_ERROR_WRONG_PHASE, JVMTI_ERROR_INTERNAL, JVMTI_ERROR_NOT_AVAILABLE, JVMTI_ERROR_UNSUPPORTED_VERSION,
    JVMTI_ERROR_ILLEGAL_ARGUMENT, JVMTI_ERROR_INVALID_ENVIRONMENT };

static const jvmtiError jvmtiDestroySharedCache_errors[] = { JVMTI_ERROR_OUT_OF_MEMORY, JVMTI_ERROR_WRONG_PHASE,
    JVMTI_ERROR_INTERNAL, JVMTI_ERROR_NOT_AVAILABLE, JVMTI_ERROR_ILLEGAL_ARGUMENT, JVMTI_ERROR_INVALID_ENVIRONMENT };

static const jvmtiError jvmtiRegisterTraceSubscriber_errors[]
    = { JVMTI_ERROR_NULL_POINTER, JVMTI_ERROR_OUT_OF_MEMORY, JVMTI_ERROR_INVALID_ENVIRONMENT, JVMTI_ERROR_WRONG_PHASE };

static const jvmtiError jvmtiDeregisterTraceSubscriber_errors[] = { JVMTI_ERROR_NULL_POINTER, JVMTI_ERROR_OUT_OF_MEMORY,
    JVMTI_ERROR_INVALID_ENVIRONMENT, JVMTI_ERROR_WRONG_PHASE, JVMTI_ERROR_ILLEGAL_ARGUMENT, JVMTI_ERROR_NOT_AVAILABLE };

static const jvmtiError jvmtiFlushTraceData_errors[]
    = { JVMTI_ERROR_OUT_OF_MEMORY, JVMTI_ERROR_WRONG_PHASE, JVMTI_ERROR_INVALID_ENVIRONMENT };

static const jvmtiError jvmtiGetTraceMetadata_errors[]
    = { JVMTI_ERROR_NULL_POINTER, JVMTI_ERROR_WRONG_PHASE, JVMTI_ERROR_INVALID_ENVIRONMENT };

static const jvmtiError jvmtiGetMemoryCategories_errors[] = { JVMTI_ERROR_UNSUPPORTED_VERSION,
    JVMTI_ERROR_ILLEGAL_ARGUMENT, JVMTI_ERROR_INVALID_ENVIRONMENT, JVMTI_ERROR_OUT_OF_MEMORY };

static const jvmtiError jvmtiRegisterVerboseGCSubscriber_errors[] = { JVMTI_ERROR_NULL_POINTER,
    JVMTI_ERROR_OUT_OF_MEMORY, JVMTI_ERROR_INVALID_ENVIRONMENT, JVMTI_ERROR_WRONG_PHASE, JVMTI_ERROR_INTERNAL };

static const jvmtiError jvmtiDeregisterVerboseGCSubscriber_errors[] = { JVMTI_ERROR_NULL_POINTER,
    JVMTI_ERROR_OUT_OF_MEMORY, JVMTI_ERROR_INVALID_ENVIRONMENT, JVMTI_ERROR_WRONG_PHASE, JVMTI_ERROR_NOT_AVAILABLE };

static const jvmtiError jvmtiGet_j9vmthread_errors[]
    = { JVMTI_ERROR_WRONG_PHASE, JVMTI_ERROR_INVALID_THREAD, JVMTI_ERROR_THREAD_NOT_ALIVE, JVMTI_ERROR_NULL_POINTER };

static const jvmtiError jvmtiGet_j9method_errors[]
    = { JVMTI_ERROR_WRONG_PHASE, JVMTI_ERROR_NOT_AVAILABLE, JVMTI_ERROR_NULL_POINTER };

static const jvmtiError jvmtiRegisterTracePointSubscriber_errors[] = { JVMTI_ERROR_NULL_POINTER,
    JVMTI_ERROR_OUT_OF_MEMORY, JVMTI_ERROR_INVALID_ENVIRONMENT, JVMTI_ERROR_WRONG_PHASE, JVMTI_ERROR_INTERNAL };

static const jvmtiError jvmtiDeregisterTracePointSubscriber_errors[]
    = { JVMTI_ERROR_NULL_POINTER, JVMTI_ERROR_OUT_OF_MEMORY, JVMTI_ERROR_INVALID_ENVIRONMENT, JVMTI_ERROR_WRONG_PHASE,
          JVMTI_ERROR_NOT_AVAILABLE, JVMTI_ERROR_INTERNAL };
...
static const J9JVMTIExtensionFunctionInfo J9JVMTIExtensionFunctionInfoTable[] = {
    { (jvmtiExtensionFunction)jvmtiTraceSet, COM_IBM_SET_VM_TRACE, J9NLS_JVMTI_COM_IBM_JVM_TRACE_SET_DESCRIPTION,
        SIZE_AND_TABLE(jvmtiTraceSet_params), SIZE_AND_TABLE(ras_errors) },
    { (jvmtiExtensionFunction)jvmtiDumpSet, COM_IBM_SET_VM_DUMP, J9NLS_JVMTI_COM_IBM_JVM_DUMP_SET_DESCRIPTION,
        SIZE_AND_TABLE(jvmtiDumpSet_params), SIZE_AND_TABLE(ras_errors) },
    { (jvmtiExtensionFunction)jvmtiJlmSet, COM_IBM_SET_VM_JLM, J9NLS_JVMTI_COM_IBM_JVM_JLM_SET_DESCRIPTION,
        SIZE_AND_TABLE(jvmtiJlmSet_params), SIZE_AND_TABLE(jlm_set_errors) },
    { (jvmtiExtensionFunction)jvmtiJlmDump, COM_IBM_SET_VM_JLM_DUMP, J9NLS_JVMTI_COM_IBM_JVM_JLM_DUMP_DESCRIPTION,
        SIZE_AND_TABLE(jvmtiJlmDump_params), SIZE_AND_TABLE(jlm_dump_errors) },
    { (jvmtiExtensionFunction)jvmtiTriggerVmDump, COM_IBM_TRIGGER_VM_DUMP,
        J9NLS_JVMTI_COM_IBM_JVM_TRIGGER_VM_DUMP_DESCRIPTION, SIZE_AND_TABLE(jvmtiTriggerVmDump_params),
        SIZE_AND_TABLE(ras_errors) },
    { (jvmtiExtensionFunction)jvmtiGetOSThreadID, COM_IBM_GET_OS_THREAD_ID, J9NLS_JVMTI_COM_IBM_GET_OS_THREAD_ID,
        SIZE_AND_TABLE(jvmtiGetOSThreadID_params), SIZE_AND_TABLE(get_os_thread_id_errors) },
    { (jvmtiExtensionFunction)jvmtiGetStackTraceExtended, COM_IBM_GET_STACK_TRACE_EXTENDED,
        J9NLS_JVMTI_COM_IBM_GET_STACK_TRACE_EXTENDED_DESCRIPTION, SIZE_AND_TABLE(jvmtiGetStackTraceExtended_params),
        SIZE_AND_TABLE(jvmtiGetStack_errors) },
    { (jvmtiExtensionFunction)jvmtiGetAllStackTracesExtended, COM_IBM_GET_ALL_STACK_TRACES_EXTENDED,
        J9NLS_JVMTI_COM_IBM_GET_ALL_STACK_TRACES_EXTENDED_DESCRIPTION,
        SIZE_AND_TABLE(jvmtiGetAllStackTracesExtended_params), SIZE_AND_TABLE(jvmtiGetStack_errors) },
    { (jvmtiExtensionFunction)jvmtiGetThreadListStackTracesExtended, COM_IBM_GET_THREAD_LIST_STACK_TRACES_EXTENDED,
        J9NLS_JVMTI_COM_IBM_GET_THREAD_LIST_STACK_TRACES_EXTENDED_DESCRIPTION,
        SIZE_AND_TABLE(jvmtiGetThreadListStackTracesExtended_params), SIZE_AND_TABLE(jvmtiGetStack_errors) },
    { (jvmtiExtensionFunction)jvmtiResetVmDump, COM_IBM_RESET_VM_DUMP,
        J9NLS_JVMTI_COM_IBM_JVM_RESET_VM_DUMP_DESCRIPTION, 0, NULL, SIZE_AND_TABLE(ras_errors) },
    { (jvmtiExtensionFunction)jvmtiGetHeapFreeMemory, COM_IBM_GET_HEAP_FREE_MEMORY,
        J9NLS_JVMTI_COM_IBM_GET_HEAP_FREE_MEMORY_DESCRIPTION, SIZE_AND_TABLE(jvmtiGetHeapFreeMemory_params),
        SIZE_AND_TABLE(nullPointer_errors) },
    { (jvmtiExtensionFunction)jvmtiGetHeapTotalMemory, COM_IBM_GET_HEAP_TOTAL_MEMORY,
        J9NLS_JVMTI_COM_IBM_GET_HEAP_TOTAL_MEMORY_DESCRIPTION, SIZE_AND_TABLE(jvmtiGetHeapTotalMemory_params),
        SIZE_AND_TABLE(nullPointer_errors) },
    { (jvmtiExtensionFunction)jvmtiIterateSharedCaches, COM_IBM_ITERATE_SHARED_CACHES,
        J9NLS_JVMTI_COM_IBM_JVM_ITERATE_SHARED_CACHES_DESCRIPTION, SIZE_AND_TABLE(jvmtiIterateSharedCaches_params),
        SIZE_AND_TABLE(jvmtiIterateSharedCaches_errors) },
    { (jvmtiExtensionFunction)jvmtiDestroySharedCache, COM_IBM_DESTROY_SHARED_CACHE,
        J9NLS_JVMTI_COM_IBM_JVM_DESTROY_SHARED_CACHE_DESCRIPTION, SIZE_AND_TABLE(jvmtiDestroySharedCache_params),
        SIZE_AND_TABLE(jvmtiDestroySharedCache_errors) },
    { (jvmtiExtensionFunction)jvmtiRemoveAllTags, COM_IBM_REMOVE_ALL_TAGS, J9NLS_JVMTI_COM_IBM_REMOVE_ALL_TAGS, 0, NULL,
        SIZE_AND_TABLE(notAvailable_errors) },
    { (jvmtiExtensionFunction)jvmtiRegisterTraceSubscriber, COM_IBM_REGISTER_TRACE_SUBSCRIBER,
        J9NLS_JVMTI_COM_IBM_JVM_REGISTER_TRACE_SUBSCRIBER_DESCRIPTION,
        SIZE_AND_TABLE(jvmtiRegisterTraceSubscriber_params), SIZE_AND_TABLE(jvmtiRegisterTraceSubscriber_errors) },
    { (jvmtiExtensionFunction)jvmtiDeregisterTraceSubscriber, COM_IBM_DEREGISTER_TRACE_SUBSCRIBER,
        J9NLS_JVMTI_COM_IBM_JVM_DEREGISTER_TRACE_SUBSCRIBER_DESCRIPTION,
        SIZE_AND_TABLE(jvmtiDeregisterTraceSubscriber_params), SIZE_AND_TABLE(jvmtiDeregisterTraceSubscriber_errors) },
    { (jvmtiExtensionFunction)jvmtiFlushTraceData, COM_IBM_FLUSH_TRACE_DATA,
        J9NLS_JVMTI_COM_IBM_JVM_FLUSH_TRACE_DATA_DESCRIPTION, 0, NULL, SIZE_AND_TABLE(jvmtiFlushTraceData_errors) },
    { (jvmtiExtensionFunction)jvmtiGetTraceMetadata, COM_IBM_GET_TRACE_METADATA,
        J9NLS_JVMTI_COM_IBM_JVM_GET_TRACE_METADATA_DESCRIPTION, SIZE_AND_TABLE(jvmtiGetTraceMetadata_params),
        SIZE_AND_TABLE(jvmtiGetTraceMetadata_errors) },
    { (jvmtiExtensionFunction)jvmtiGetMethodAndClassNames, COM_IBM_GET_METHOD_AND_CLASS_NAMES,
        J9NLS_JVMTI_COM_IBM_GET_METHOD_AND_CLASS_NAMES, SIZE_AND_TABLE(jvmtiGetMethodAndClassNames_params),
        SIZE_AND_TABLE(jvmtiFlushTraceData_errors) },
    { (jvmtiExtensionFunction)jvmtiQueryVmDump, COM_IBM_QUERY_VM_DUMP, J9NLS_JVMTI_COM_IBM_QUERY_VM_DUMP, 0, NULL,
        SIZE_AND_TABLE(ras_errors) },
    { (jvmtiExtensionFunction)jvmtiQueryVmLogOptions, COM_IBM_QUERY_VM_LOG_OPTIONS,
        J9NLS_JVMTI_COM_IBM_QUERY_VM_LOG_OPTIONS, SIZE_AND_TABLE(jvmtiQueryVmLogOptions_params),
        SIZE_AND_TABLE(ras_errors) },
    { (jvmtiExtensionFunction)jvmtiSetVmLogOptions, COM_IBM_SET_VM_LOG_OPTIONS, J9NLS_JVMTI_COM_IBM_SET_VM_LOG_OPTIONS,
        SIZE_AND_TABLE(jvmtiSetVmLogOptions_params), SIZE_AND_TABLE(ras_errors) },
    { (jvmtiExtensionFunction)jvmtiJlmDumpStats, COM_IBM_JLM_DUMP_STATS, J9NLS_JVMTI_COM_IBM_JLM_STATS_DUMP_DESCRIPTION,
        SIZE_AND_TABLE(jvmtiJlmDumpStats_params), SIZE_AND_TABLE(jlm_dump_errors) },
    { (jvmtiExtensionFunction)jvmtiGetMemoryCategories, COM_IBM_GET_MEMORY_CATEGORIES,
        J9NLS_JVMTI_COM_IBM_GET_MEMORY_CATEGORIES_DESCRIPTION, SIZE_AND_TABLE(jvmtiGetMemoryCategories_params),
        SIZE_AND_TABLE(jvmtiGetMemoryCategories_errors) },
    { (jvmtiExtensionFunction)jvmtiRegisterVerboseGCSubscriber, COM_IBM_REGISTER_VERBOSEGC_SUBSCRIBER,
    ...
    { (jvmtiExtensionFunction)jvmtiDeregisterVerboseGCSubscriber, COM_IBM_DEREGISTER_VERBOSEGC_SUBSCRIBER,
    ...
    { (jvmtiExtensionFunction)jvmtiGetJ9vmThread, COM_IBM_GET_J9VMTHREAD, J9NLS_JVMTI_COM_IBM_GET_J9VMTHREAD,
        SIZE_AND_TABLE(jvmtiGetJ9vmThread_params), SIZE_AND_TABLE(jvmtiGet_j9vmthread_errors) },
    { (jvmtiExtensionFunction)jvmtiGetJ9method, COM_IBM_GET_J9METHOD, J9NLS_JVMTI_COM_IBM_GET_J9METHOD,
        SIZE_AND_TABLE(jvmtiGetJ9method_params), SIZE_AND_TABLE(jvmtiGet_j9method_errors) },
    { (jvmtiExtensionFunction)jvmtiRegisterTracePointSubscriber, COM_IBM_REGISTER_TRACEPOINT_SUBSCRIBER,
    ...
    { (jvmtiExtensionFunction)jvmtiDeregisterTracePointSubscriber, COM_IBM_DEREGISTER_TRACEPOINT_SUBSCRIBER,
...
#define NUM_EXTENSION_FUNCTIONS \
    (sizeof(J9JVMTIExtensionFunctionInfoTable) / sizeof(J9JVMTIExtensionFunctionInfoTable[0]))
...
jvmtiError JNICALL jvmtiGetExtensionFunctions(
    jvmtiEnv* env, jint* extension_count_ptr, jvmtiExtensionFunctionInfo** extensions)
{
    unsigned char* mem;
...
jvmtiError JNICALL jvmtiGetExtensionEvents(
    jvmtiEnv* env, jint* extension_count_ptr, jvmtiExtensionEventInfo** extensions)
{
    jvmtiError rc = JVMTI_ERROR_NONE;
    unsigned char* mem;
    PORT_ACCESS_FROM_JVMTI(env);
...
jvmtiError JNICALL jvmtiSetExtensionEventCallback(
    jvmtiEnv* env, jint extension_event_index, jvmtiExtensionEvent callback)
{
    ...
    if (rc == JVMTI_ERROR_NONE) {
        ...
        rc = setEventNotificationMode(j9env, currentThread, (callback == NULL) ? JVMTI_DISABLE : JVMTI_ENABLE,
            extension_event_index, NULL, J9JVMTI_LOWEST_EXTENSION_EVENT, J9JVMTI_HIGHEST_EXTENSION_EVENT);
        if (rc == JVMTI_ERROR_NONE) {
            J9JVMTI_EXTENSION_CALLBACK(j9env, extension_event_index)
                = (jvmtiExtensionEvent)J9_COMPATIBLE_FUNCTION_POINTER(callback);
        }
    ...
    jvmtiEnv* env, jvmtiExtensionFunctionInfo* dest, const J9JVMTIExtensionFunctionInfo* source)
{
    ...
    rc = copyString(env, &dest->short_description,
        OMRPORT_FROM_J9PORT(PORTLIB)->nls_lookup_message(OMRPORT_FROM_J9PORT(PORTLIB),
            J9NLS_INFO | J9NLS_DO_NOT_APPEND_NEWLINE, source->descriptionModule, source->descriptionNum, NULL));
    if (rc != JVMTI_ERROR_NONE) {
        return rc;
    }
    ...
    if (rc == JVMTI_ERROR_NONE) {
        ...
        if (rc == JVMTI_ERROR_NONE) {
            ...
            rc = jvmtiInternalGetStackTraceExtended(
                env, type, currentThread, targetThread, start_depth, (UDATA)max_frame_count, frame_buffer, &rv_count);
    ...
    jvmtiEnv* env, jint type, jint max_frame_count, void** stack_info_ptr, jint* thread_count_ptr, ...)
{
    ...
    if (rc == JVMTI_ERROR_NONE) {
        ...
        stackInfo = j9mem_allocate_memory(
            ((sizeof(jvmtiStackInfoExtended) + (max_frame_count * sizeof(jvmtiFrameInfoExtended))) * threadCount)
                + sizeof(jlocation),
            J9MEM_CATEGORY_JVMTI_ALLOCATE);
        if (stackInfo == NULL) {
            rc = JVMTI_ERROR_OUT_OF_MEMORY;
        } else {
            jvmtiFrameInfoExtended* currentFrameInfo
                = (jvmtiFrameInfoExtended*)((((UDATA)(stackInfo + threadCount)) + sizeof(jlocation))
                    & ~sizeof(jlocation));
            ...
            do {
                ...
                if (targetThread->threadObject == NULL) {
                    ...
                    rc = jvmtiInternalGetStackTraceExtended(env, type, currentThread, targetThread, 0,
                        (UDATA)max_frame_count, (void*)currentFrameInfo, &(currentStackInfo->frame_count));
                    if (rc != JVMTI_ERROR_NONE) {
                        j9mem_free_memory(stackInfo);
                        goto fail;
                    }

                    currentStackInfo->thread = (jthread)vm->internalVMFunctions->j9jni_createLocalRef(
                        (JNIEnv*)currentThread, (j9object_t)targetThread->threadObject);
    ...
    const jthread* thread_list, jint max_frame_count, void** stack_info_ptr, ...)
{
    ...
    if (rc == JVMTI_ERROR_NONE) {
        ...
        stackInfo = j9mem_allocate_memory(
            ((sizeof(jvmtiStackInfoExtended) + (max_frame_count * sizeof(jvmtiFrameInfoExtended))) * thread_count)
                + sizeof(jlocation),
            J9MEM_CATEGORY_JVMTI_ALLOCATE);
        if (stackInfo == NULL) {
            rc = JVMTI_ERROR_OUT_OF_MEMORY;
            ...
            jvmtiFrameInfoExtended* currentFrameInfo
                = (jvmtiFrameInfoExtended*)((((UDATA)(stackInfo + thread_count)) + sizeof(jlocation))
                    & ~sizeof(jlocation));
            jvmtiStackInfoExtended* currentStackInfo = stackInfo;
...
static UDATA jvmtiInternalGetStackTraceIteratorExtended(J9VMThread* currentThread, J9StackWalkState* walkState)
{
    ...
    /* In extra info mode when method enter is enabled, exclude natives which have not had method enter reported for
     * them */
    ...
    jboolean useCommandLineValues, jvmtiIterateSharedCachesCallback callback, void* user_data, ...)
{
    ...
    if ((COM_IBM_ITERATE_SHARED_CACHES_VERSION_1 != version) && (COM_IBM_ITERATE_SHARED_CACHES_VERSION_2 != version)
    ...
    if (-1
        == vm->sharedCacheAPI->iterateSharedCaches(
               vm, cacheDir, (UDATA)flags, useCommandLineValues, &iterateSharedCachesCallback, &userdata)) {
        rc = JVMTI_ERROR_INTERNAL;
        goto done;
    }
    ...
    jboolean useCommandLineValues, jint* internalErrorCode, ...)
{
    ...
    if ((COM_IBM_SHARED_CACHE_PERSISTENCE_DEFAULT != cacheType) && (COM_IBM_SHARED_CACHE_PERSISTENT != cacheType)
        && (COM_IBM_SHARED_CACHE_NONPERSISTENT != cacheType) && (COM_IBM_SHARED_CACHE_SNAPSHOT != cacheType)) {
        rc = JVMTI_ERROR_ILLEGAL_ARGUMENT;
        goto done;
    }
...
static jvmtiError JNICALL jvmtiRemoveAllTags(jvmtiEnv* jvmti_env, ...)
{
    J9JVMTIEnv* j9env = (J9JVMTIEnv*)jvmti_env;
    jvmtiError rc = JVMTI_ERROR_NOT_AVAILABLE;
    ...
    if (j9env->objectTagTable != NULL) {
        ...
        j9env->objectTagTable = hashTableNew(OMRPORT_FROM_J9PORT(j9env->vm->portLibrary), J9_GET_CALLSITE(), 0,
            sizeof(J9JVMTIObjectTag), sizeof(jlong), 0, J9MEM_CATEGORY_JVMTI, hashFn, hashEqualFn, NULL, NULL);
        rc = JVMTI_ERROR_NONE;
    }
 ...
 */
omr_error_t subscriberWrapper(UtSubscription* sub)
{
    jvmtiError rc = JVMTI_ERROR_NONE;
    omr_error_t result = OMR_ERROR_NONE;
    ...
    jvmtiTraceSubscriber subscriber, jvmtiTraceAlarm alarm, void* userData, void** subscriptionID, ...)
{
    ...
    if (uteInterface && uteInterface->server) {
        ...
        /* Pass option to the trace engine */
        result = uteInterface->server->RegisterRecordSubscriber(UT_THREAD_FROM_VM_THREAD(currentThread), description,
            subscriberWrapper, alarmWrapper, wrapper, (struct UtTraceBuffer*)-1, NULL,
            (UtSubscription**)&rv_subscriptionID, TRUE);
 ...
 *	JVMTI_ERROR_NOT_AVAILABLE - callback in use, will be deregistered on return and alarm will be called if not
 *NULL. JVMTI_ERROR_ILLEGAL_ARGUMENT - unknown subscriber JVMTI_ERROR_INVALID_ENVIRONMENT - trace is not present or
 *enabled
 *
...
static jvmtiError JNICALL jvmtiDeregisterTraceSubscriber(jvmtiEnv* env, void* subscriberID, ...)
{
    ...
    if (uteInterface && uteInterface->server) {
        ...
        /* Pass option to the trace engine */
        omr_error_t result
            = uteInterface->server->DeregisterRecordSubscriber(UT_THREAD_FROM_VM_THREAD(currentThread), subscriberID);
...
static jvmtiError JNICALL jvmtiFlushTraceData(jvmtiEnv* env, ...)
{
    ...
    if (uteInterface && uteInterface->server) {
        ...
        /* Pass option to the trace engine */
        omr_error_t result
            = uteInterface->server->FlushTraceData(UT_THREAD_FROM_VM_THREAD(currentThread), NULL, NULL, FALSE);
 ...
 */
static jvmtiError jvmtiGetMethodAndClassNames_verifyRamMethod(J9JavaVM* vm, J9Method* ramMethod)
{
    J9Class* clazz;
 ...
 * @param[in/out] ramMethodStringsSize      size of ramMethodStrings in bytes, if not enough we'll change it to indicate
 *the correct size
 ...
 *
 *	The ramMethodDataDescriptors is a buffer large enough to accomodate ramMethodCount number of
 *jvmtiExtensionRamMethodData structures. It is written to by this extension as ram method pointers are processed. The
 *first jvmtiExtensionRamMethodData structure written to the ramMethodDataDescriptors corresponds to the first ram
 *method pointer in the ramMethods array and so on. Ram method pointers that are detected to be no longer valid (e.g.
 *class has been unloaded) will result in className and methodName pointers being set to NULL in the corresponding
 *jvmtiExtensionRamMethodData structure.
 *
 *
 *	The ramMethodStrings serves as a buffer to copy class, method and package name strings for valid ram method
 *pointers. The jvmtiExtensionRamMethodData structures are initialized with pointers into this array. Each string is
 *null terminated. Care should be taken to pass in a large enough buffer to accomodate strings for all requested ram
 *methods. In case the buffer is not sufficiently large, the extension will stop processing any further ram methods
 *pointers and return whatever has been processed up to that point.
 *
 *  ramMethodStringsSize is used by the user to specify the size of ramMethodStrings. If not enough space was provided,
 *this API call will change the value to the required amount of space.
 *
 *	Note that ramMethodDataDescriptors and ramMethodStrings are decoupled into two separate buffers as the user
 *might wish to keep the strings as is to prevent a copy and discard the ramMethodDataDescriptors once the pointers have
 *been reassigned to some other user defined mechanism.
 *
 * */
static jvmtiError JNICALL jvmtiGetMethodAndClassNames(jvmtiEnv* jvmti_env, void* ramMethods, jint ramMethodCount,
    jvmtiExtensionRamMethodData* ramMethodDataDescriptors, jchar* ramMethodStrings, jint* ramMethodStringsSize, ...)
{
    J9JavaVM* vm = JAVAVM_FROM_ENV(jvmti_env);
    jvmtiError rc = JVMTI_ERROR_NONE;
 ...
 */
static jvmtiError JNICALL jvmtiQueryVmLogOptions(
    jvmtiEnv* jvmti_env, jint buffer_size, void* options, jint* data_size_ptr, ...)
{
    J9JavaVM* vm = JAVAVM_FROM_ENV(jvmti_env);
    jvmtiError rc = JVMTI_ERROR_NOT_AVAILABLE;
    jint rv_data_size = 0;
...
static U_32 indexFromCategoryCode(UDATA categories_mapping_size, U_32 cc)
{
    ...
    return ((cc) > OMRMEM_LANGUAGE_CATEGORY_LIMIT)
        ? ((((U_32)categories_mapping_size) - 1) - (OMRMEM_OMR_CATEGORY_INDEX_FROM_CODE(cc)))
        : (cc);
}
 ...
 * @param[out] category_buffer Block of memory to write result into. 0th entry is the first root category. All other
 ...
 * JVMTI_ERROR_OUT_OF_MEMORY - Memory category data was truncated becasue category_buffer/max_categories was not large
 * enough
 *
 */
static jvmtiError JNICALL jvmtiGetMemoryCategories(jvmtiEnv* env, jint version, jint max_categories,
    jvmtiMemoryCategory* categories_buffer, jint* written_count_ptr, jint* total_categories_ptr, ...)
{
    ...
    Trc_JVMTI_jvmtiGetMemoryCategories_Entry(
        env, version, max_categories, categories_buffer, written_count_ptr, total_categories_ptr);
    ...
    if (categories_buffer) {
        ...
        userData.categories_mapping = (jvmtiMemoryCategory**)j9mem_allocate_memory(
            userData.categories_mapping_size * sizeof(jvmtiMemoryCategory*), J9MEM_CATEGORY_JVMTI);
        if (NULL == userData.categories_mapping) {
             ...
             */
            Trc_JVMTI_jvmtiGetMemoryCategories_GetMemoryCategories_J9MemAllocFail_Exit(
                userData.categories_mapping_size * sizeof(jvmtiMemoryCategory*));
...
static void hookVerboseGCOutput(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (NULL != subscriberData) {
        jvmtiError rc = subscriberData->subscriber(
            subscriberData->env, event->string, strlen(event->string), subscriberData->userData);

        if (JVMTI_ERROR_NONE != rc) {
            /* The subscriber returned an error. Call the alarm function. */
            subscriberData->alarm(subscriberData->env, (void*)subscriberData, subscriberData->userData);
 ...
 *
 * As verbose GC data is produced, the subscriber function is called along with the userData and the ASCII-encoded
 * verbose GC XML data. The subscriber must process the data during the function call. Once the subscriber returns the
 * data passed in may no longer be valid. Note that the subscriber is not guaranteed to receive a complete XML file;
 * i.e. the stream may not begin with the <?xml> and <verbosegc> tags, nor end with </verbosegc>.
 *
 ...
 */
jvmtiError JNICALL jvmtiRegisterVerboseGCSubscriber(jvmtiEnv* env, char* description,
    jvmtiVerboseGCSubscriber subscriber, jvmtiVerboseGCAlarm alarm, void* userData, void** subscriptionID, ...)
{
    J9JavaVM* vm = JAVAVM_FROM_ENV(env);
    J9JVMTIEnv* j9env = (J9JVMTIEnv*)env;
    ...
    hookRC = (*gcHooks)->J9HookRegisterWithCallSite(gcHooks, J9HOOK_TAG_AGENT_ID | J9HOOK_MM_OMR_VERBOSE_GC_OUTPUT,
        hookVerboseGCOutput, OMR_GET_CALLSITE(), subscriberData, j9env->gcHook.agentID);
    if (0 != hookRC) {
 ...
 * @return JVMTI_ERROR_NONE on success, or a JVMTI error on failure
 */
jvmtiError JNICALL jvmtiDeregisterVerboseGCSubscriber(jvmtiEnv* env, void* subscriberID, ...)
{
    J9JavaVM* vm = JAVAVM_FROM_ENV(env);
    jvmtiError rc = JVMTI_ERROR_INVALID_ENVIRONMENT;
    J9VMThread* currentThread;
...
/*
 * Registers a tracepoint subscriber. This is the documented JVMTI API for listening to individual formatted
 *tracepoints. This API is distinct from jvmtiRegisterTraceSubscriber(), which is the undocumented JVMTI API for
 *listening to binary trace buffers used by the Health Centre. It replaces the old JVMRI trace listener.
 *
 * Parameters:
 ...
 *	subscriptionID - opaque pointer (internally points to a UtSubscription) identifying the subscriber, needed to
 *unsubscribe.
 *
 * Return values with meaning specific to this function:
    ...
    jvmtiTraceSubscriber subscriber, jvmtiTraceAlarm alarm, void* userData, void** subscriptionID, ...)
{
    ...
    if ((traceInterface != NULL) && (traceInterface->server != NULL)) {
        ...
        /* Call into the trace engine to register the subscriber. Note the use of function wrappers for the user
         * callbacks */
        result = traceInterface->server->RegisterTracePointSubscriber(UT_THREAD_FROM_VM_THREAD(currentThread),
            description, subscriberWrapper, alarmWrapper, wrapper, (UtSubscription**)subscriptionID);
 ...
 */
static jvmtiError JNICALL jvmtiDeregisterTracePointSubscriber(jvmtiEnv* env, void* subscriberID, ...)
{
    J9JavaVM* vm = JAVAVM_FROM_ENV(env);
    jvmtiError rc = JVMTI_ERROR_INVALID_ENVIRONMENT;
    J9VMThread* currentThread = NULL;
    ...
    if ((traceInterface != NULL) && (traceInterface->server != NULL)) {
        ...
        omr_error_t result = traceInterface->server->DeregisterTracePointSubscriber(
            UT_THREAD_FROM_VM_THREAD(currentThread), subscriberID);
