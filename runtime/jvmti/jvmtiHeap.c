...
static jvmtiIterationControl iterateThroughHeapCallback(
...
static jvmtiIterationControl wrap_primitiveFieldCallback(
    J9JavaVM* vm, J9JVMTIHeapData* iteratorData, IDATA wasReportedBefore);
...
static jvmtiError getArrayPrimitiveElements(
    J9JVMTIHeapData* iteratorData, jvmtiPrimitiveType* primitiveType, void** elements, jint elementCount);

static jvmtiIterationControl followReferencesCallback(
    j9object_t* slotPtr, j9object_t referrer, void* userData, IDATA type, IDATA referrerIndex, IDATA wasReportedBefore);
...
static void jvmtiHeapFollowReferencesPrint(
    J9JavaVM* vm, j9object_t object, j9object_t referrer, J9JVMTIHeapData* iteratorData);
    ...
    const jvmtiHeapCallbacks* callbacks, const void* user_data)
{
    ...
    if (rc == JVMTI_ERROR_NONE) {
        ...
        if (initial_object) {
            ...
            vm->memoryManagerFunctions->j9gc_ext_reachable_objects_do(currentThread, followReferencesCallback,
                &iteratorData,
                J9_MU_WALK_OBJECT_BASE | J9_MU_WALK_SKIP_JVMTI_TAG_TABLES | J9_MU_WALK_PREINDEX_INTERFACE_FIELDS);
        }
 ...
 *	    * Classes report a reference to the class loader, protection domain, signers, and resolved entries in the
 *constant pool.
    ...
    j9object_t* slotPtr, j9object_t referrer, void* userData, IDATA type, IDATA referrerIndex, IDATA wasReportedBefore)
{
    ...
    if (iteratorData->callbacks->array_primitive_value_callback && J9ROMCLASS_IS_PRIMITIVE_OR_ARRAY(clazz->romClass)) {
...
static IDATA heapReferenceFilter(J9JVMTIHeapData* iteratorData)
{
    ...
    if (((iteratorData->filter & JVMTI_HEAP_FILTER_TAGGED) && iteratorData->tags.objectTag != (jlong)0)
        || /* filter out tagged objects */
        ((iteratorData->filter & JVMTI_HEAP_FILTER_UNTAGGED)
            && iteratorData->tags.objectTag == (jlong)0)) { /* filter out untagged objects */
        return JVMTI_ITERATION_CONTINUE;
    }

    /* Does it pass the class filters specified by the user? */
    if (((iteratorData->filter & JVMTI_HEAP_FILTER_CLASS_TAGGED) && iteratorData->tags.classTag != (jlong)0)
        || /* filter out tagged classes */
        ((iteratorData->filter & JVMTI_HEAP_FILTER_CLASS_UNTAGGED)
            && iteratorData->tags.classTag == (jlong)0)) { /* filter out untagged classes */
        return JVMTI_ITERATION_CONTINUE;
    }
...
static jvmtiIterationControl wrap_heapReferenceCallback(J9JavaVM* vm, J9JVMTIHeapData* iteratorData)
{
    ...
    /* Callback the user */
    visitRc = iteratorData->callbacks->heap_reference_callback(e->refKind, e->hasRefInfo ? &e->refInfo : NULL,
        iteratorData->tags.classTag,
        (e->type == J9JVMTI_HEAP_EVENT_ROOT || e->type == J9JVMTI_HEAP_EVENT_STACK)
            ? 0
            : iteratorData->tags.referrerClassTag,
        iteratorData->objectSize, &objectTag,
        (e->type == J9JVMTI_HEAP_EVENT_ROOT || e->type == J9JVMTI_HEAP_EVENT_STACK) ? NULL : &referrerObjectTag, length,
    ...
    if (e->type != J9JVMTI_HEAP_EVENT_ROOT && e->type != J9JVMTI_HEAP_EVENT_STACK) {
        if (referrerObjectTag != iteratorData->tags.referrerObjectTag)
            updateObjectTag(
                iteratorData, iteratorData->referrer, &iteratorData->tags.referrerObjectTag, referrerObjectTag);
    }
    ...
    J9JavaVM* vm, j9object_t object, j9object_t referrer, J9JVMTIHeapData* iteratorData)
{
    ...
    jvmtiHeapFR_printf(
        ("\tev=0x%02x [%d]  refType=0x%x [%-17s]  idx=%d   [%s] -> [%s] flds=%-3d  [%08llx:%08llx]->[%08llx:%08llx]\n",
            e->type, e->gcType, e->refKind, refName, e->refInfo.field.index, referrerName, refereeName,
            clazz->romClass->romFieldCount, (rc) ? iteratorData->tags.referrerClassTag & 0xffffffff : 0xffffffff,
            (rc) ? iteratorData->tags.referrerObjectTag & 0xffffffff : 0xffffffff,
            iteratorData->tags.classTag & 0xffffffff, iteratorData->tags.objectTag & 0xffffffff));
    return;
}
#endif
    ...
    jvmtiEnv* env, jint heap_filter, jclass klass, const jvmtiHeapCallbacks* callbacks, const void* user_data)
{
    ...
    if (rc == JVMTI_ERROR_NONE) {
        ...
        /* Walk the heap */
        vm->memoryManagerFunctions->j9mm_iterate_all_objects(
            vm, vm->portLibrary, 0, iterateThroughHeapCallback, &iteratorData);
        rc = iteratorData.rc;
    ...
    J9JavaVM* vm, J9MM_IterateObjectDescriptor* objectDesc, void* userData)
{
...
#ifdef JVMTI_HEAP_DEBUG
    {
        ...
        jvmtiHeapFR_printf(("Class: [%*s] fields=%d class_tag=0x%x\n", J9UTF8_LENGTH(c), J9UTF8_DATA(c),
            clazz->romClass->romFieldCount, iteratorData->tags.classTag));
    }
#endif
...
static jvmtiIterationControl wrap_heapIterationCallback(J9JavaVM* vm, J9JVMTIHeapData* iteratorData)
{
    ...
    visitRc = iteratorData->callbacks->heap_iteration_callback(iteratorData->tags.classTag, iteratorData->objectSize,
        &tag, (isArray) ? arrayLength : -1, iteratorData->userData);
...
static jvmtiIterationControl wrap_arrayPrimitiveValueCallback(J9JavaVM* vm, J9JVMTIHeapData* iteratorData)
{
    ...
    if (primitiveType != 0 && NULL != elements) {
...
#ifdef JVMTI_HEAP_DEBUG
        jvmtiHeapFR_printf(
            ("\t\tArray Primitive type=%d  element_count=%d  [tag=0x%08x]\n", primitiveType, elementCount, tag));
#endif
        ...
        visitRc = iteratorData->callbacks->array_primitive_value_callback(iteratorData->tags.classTag,
            iteratorData->objectSize, &tag, elementCount, primitiveType, elements, iteratorData->userData);
    ...
    J9JavaVM* vm, J9JVMTIHeapData* iteratorData, IDATA wasReportedBefore)
{
    ...
    if (J9VM_IS_INITIALIZED_HEAPCLASS(iteratorData->currentThread, iteratorData->object)
        || (iteratorData->clazz->romClass->modifiers & J9AccInterface)) {
        ...
        walkFlags = J9VM_FIELD_OFFSET_WALK_INCLUDE_INSTANCE | J9VM_FIELD_OFFSET_WALK_INCLUDE_STATIC
            | J9VM_FIELD_OFFSET_WALK_PREINDEX_INTERFACE_FIELDS;
    }
...
#ifdef JVMTI_HEAP_DEBUG
    {
        ...
        jvmtiHeapFR_printf(("\t\tClass: [%*s] fields=%d 0x%08x referenceIndexOffset=%d\n", J9UTF8_LENGTH(c),
            J9UTF8_DATA(c), clazz->romClass->romFieldCount, clazz, state.referenceIndexOffset));
    }
#endif
    ...
    while (field != NULL) {
        ...
        if (field->modifiers & J9FieldFlagObject) {
#ifdef JVMTI_HEAP_DEBUG
            {
                ...
                printf("\t\t%s Object             [idx=%-2d | %d] [cia=%d] [Cclazz=0x%08x]                     "
                       "[%08llx:%08llx] [%s] [%s] \n",
                    (field->modifiers & J9AccStatic) ? "S" : "I",
                    state.referenceIndexOffset + state.fieldOffsetWalkState.result.index - 1,
                    clazz->romClass->romFieldCount, state.referenceIndexOffset, state.currentClass,
                    iteratorData->tags.classTag & 0xffffffff, tag & 0xffffffff, name, sig);
            }
#endif
            goto nextField;
        }
        ...
        fieldInfo.field.index
            = (jint)(state.fieldOffsetWalkState.result.index + state.classIndexAdjust + state.referenceIndexOffset - 1);
        ...
        if (JVMTI_ERROR_NONE == rc) {
...
#ifdef JVMTI_HEAP_DEBUG
            {
                ...
                name[J9UTF8_LENGTH(fieldName)] = 0;
                sig[J9UTF8_LENGTH(fieldSignature)] = 0;
                printf("\t\t%s Primitive type=%d  [idx=%-2d | %d] [cia=%d] [C=0x%08x][CC=0x%08x] [%08llx:%08llx] [%s] "
                       "[%s] [val=0x%llx]\n",
                    (field->modifiers & J9AccStatic) ? "S" : "I", primitiveType, fieldInfo.field.index,
                    clazz->romClass->romFieldCount, state.referenceIndexOffset, clazz, state.currentClass,
                    iteratorData->tags.classTag & 0xffffffff, tag & 0xffffffff, name, sig, *((jlong*)valuePtr));
            }
#endif

            /* Call the user with the field data */
            visitRc = iteratorData->callbacks->primitive_field_callback(fieldKind, &fieldInfo,
                iteratorData->tags.classTag, &tag, primitiveValue, primitiveType, iteratorData->userData);
...
static jvmtiIterationControl wrap_stringPrimitiveCallback(J9JavaVM* vm, J9JVMTIHeapData* iteratorData)
{
    ...
    rc = iteratorData->callbacks->string_primitive_value_callback(iteratorData->tags.classTag, iteratorData->objectSize,
        &tag, (const jchar*)stringValue, stringLength, iteratorData->userData);
