...
static jvmtiIterationControl processHeapRoot(
    J9JVMTIObjectIteratorData* data, J9JVMTIObjectTag* entry, jlong size, jlong classTag, jvmtiHeapRootKind kind);
static jvmtiIterationControl processStackRoot(
    J9JVMTIObjectIteratorData* data, J9JVMTIObjectTag* entry, jlong size, jlong classTag, J9StackWalkState* walkState);
static jvmtiIterationControl wrapHeapIterationCallback(
    J9JavaVM* vm, J9MM_IterateObjectDescriptor* objectDesc, void* userData);
static J9JVMTIHeapEvent mapEventType(J9VMThread* vmThread, J9JVMTIObjectIteratorData* data, IDATA type, IDATA index,
    j9object_t referrer, j9object_t object);
static jvmtiIterationControl wrapObjectIterationCallback(
    j9object_t* slotPtr, j9object_t referrer, void* userData, IDATA type, IDATA referrerIndex, IDATA wasReportedBefore);
static jvmtiIterationControl processObjectRef(J9JVMTIObjectIteratorData* data, J9JVMTIObjectTag* entry, jlong size,
    jlong classTag, jvmtiObjectReferenceKind kind, jlong referrerTag, jint referrerIndex);
    ...
    jvmtiHeapObjectCallback heap_object_callback, const void* user_data)
{
    ...
    if (rc == JVMTI_ERROR_NONE) {
        ...
        vm->memoryManagerFunctions->j9mm_iterate_all_objects(
            vm, vm->portLibrary, 0, wrapHeapIterationCallback, &iteratorData);
    ...
    jvmtiEnv* env, jobject object, jvmtiObjectReferenceCallback object_reference_callback, const void* user_data)
{
    ...
    if (rc == JVMTI_ERROR_NONE) {
        ...
        if (slot != NULL) {
            ...
            iteratorData.objectRefCallback
                = (jvmtiObjectReferenceCallback)J9_COMPATIBLE_FUNCTION_POINTER(object_reference_callback);
            iteratorData.userData = (void*)user_data;

            /* Walk all slots that are reachable from this slot */
            vm->memoryManagerFunctions->j9gc_ext_reachable_from_object_do(
                currentThread, slot, wrapObjectIterationCallback, &iteratorData, 0);
        }
    ...
    const void* user_data)
{
    ...
    if (rc == JVMTI_ERROR_NONE) {
        ...
        iteratorData.objectRefCallback
            = (jvmtiObjectReferenceCallback)J9_COMPATIBLE_FUNCTION_POINTER(object_ref_callback);
        iteratorData.userData = (void*)user_data;

        /* Walk all slots that are reachable from this slot */
        vm->memoryManagerFunctions->j9gc_ext_reachable_objects_do(currentThread, wrapObjectIterationCallback,
            &iteratorData, J9_MU_WALK_SKIP_JVMTI_TAG_TABLES | J9_MU_WALK_TRACK_VISIBLE_FRAME_DEPTH);
    ...
    j9object_t referrer, j9object_t object)
{
    ...
    case J9GC_REFERENCE_TYPE_CLASS:
        /* BOGUS: Classes return their superclass for the ref_type_class.. slime j.l.c. in at the end of the constant
         * pool */
...
static jvmtiIterationControl processHeapRoot(
    J9JVMTIObjectIteratorData* data, J9JVMTIObjectTag* entry, jlong size, jlong classTag, jvmtiHeapRootKind kind)
{
    return data->heapRootCallback(kind, classTag, size, &entry->tag, data->userData);
}

static jvmtiIterationControl processObjectRef(J9JVMTIObjectIteratorData* data, J9JVMTIObjectTag* entry, jlong size,
    jlong classTag, jvmtiObjectReferenceKind kind, jlong referrerTag, jint referrerIndex)
{
    return data->objectRefCallback(kind, classTag, size, &entry->tag, referrerTag, referrerIndex, data->userData);
}
    ...
    J9JVMTIObjectIteratorData* data, J9JVMTIObjectTag* entry, jlong size, jlong classTag, J9StackWalkState* walkState)
{
    ...
    return data->stackRefCallback(
        kind, classTag, size, &entry->tag, result ? result->tag : 0, depth, method, slot, data->userData);
}
    ...
    J9JavaVM* vm, J9MM_IterateObjectDescriptor* objectDesc, void* userData)
{
    ...
    if (iteratorData->clazz == NULL || isSameOrSuperClassOf(iteratorData->clazz, J9OBJECT_CLAZZ_VM(vm, object))) {
        ...
        if ((iteratorData->filter == JVMTI_HEAP_OBJECT_EITHER)
            ...
            rc = iteratorData->callback(classTag ? classTag->tag : 0, objectSize, &tag, iteratorData->userData);
