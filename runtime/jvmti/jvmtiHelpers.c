...
static jvmtiError setGlobalBreakpoint(
    J9VMThread* currentThread, J9Method* ramMethod, IDATA location, J9JVMTIGlobalBreakpoint** globalBreakpointPtr);
static jvmtiError createSingleBreakpoint(
    J9VMThread* currentThread, J9Method* ramMethod, IDATA location, J9JVMTIGlobalBreakpoint** globalBreakpointPtr);
    ...
    J9VMThread* currentThread, jthread thread, J9VMThread** vmThreadPtr, UDATA allowNull, UDATA mustBeAlive)
{
    ...
    if (!J9VMJAVALANGTHREAD_STARTED(currentThread, threadObject)
        || ((targetThread = J9VMJAVALANGTHREAD_THREADREF(currentThread, threadObject)) == NULL)) {
        if (mustBeAlive) {
...
void disposeEnvironment(J9JVMTIEnv* j9env, UDATA freeData)
{
    ...
    if ((j9env->flags & J9JVMTIENV_FLAG_DISPOSED) == 0) {
        ...
        /* Unhook all the events - note this does not guarantee that no further events will be received - some may be in
         * progress already */
...
jint allocateEnvironment(J9InvocationJavaVM* invocationJavaVM, jint version, void** penv)
{
    ...
    if (getCurrentVMThread(vm, &currentThread) == JVMTI_ERROR_NONE) {
        ...
        if (j9env != NULL) {
            ...
            j9env->vm = vm;
#if defined(J9VM_OPT_JAVA_OFFLOAD_SUPPORT)
            /* Jazz 99339: store J9NativeLibrary in J9JVMTIEnv so as to check the zAAP switching flag in the JVMTI event
             * callbacks */
            ...
            j9env->threadDataPool = pool_new(sizeof(J9JVMTIThreadData), 0, 0, 0, J9_GET_CALLSITE(),
                J9MEM_CATEGORY_JVMTI, POOL_FOR_PORT(vm->portLibrary));
            if (j9env->threadDataPool == NULL) {
                goto fail;
            }
            j9env->objectTagTable
                = hashTableNew(OMRPORT_FROM_J9PORT(vm->portLibrary), J9_GET_CALLSITE(), 0, sizeof(J9JVMTIObjectTag),
                    sizeof(jlong), 0, J9MEM_CATEGORY_JVMTI, hashObjectTag, hashEqualObjectTag, NULL, NULL);
            if (j9env->objectTagTable == NULL) {
                goto fail;
            }
            j9env->watchedClasses
                = hashTableNew(OMRPORT_FROM_J9PORT(vm->portLibrary), J9_GET_CALLSITE(), 0, sizeof(J9JVMTIWatchedClass),
                    sizeof(UDATA), 0, J9MEM_CATEGORY_JVMTI, watchedClassHash, watchedClassEqual, NULL, NULL);
            if (j9env->watchedClasses == NULL) {
                goto fail;
            }
            j9env->breakpoints = pool_new(sizeof(J9JVMTIAgentBreakpoint), 0, 0, POOL_ALWAYS_KEEP_SORTED,
                J9_GET_CALLSITE(), J9MEM_CATEGORY_JVMTI, POOL_FOR_PORT(vm->portLibrary));
            if (j9env->breakpoints == NULL) {
                goto fail;
            }
    ...
    jthread* threadRefPtr, UDATA* hadVMAccessPtr, UDATA wantVMAccess, UDATA jniRefSlots, UDATA* javaOffloadOldState)
{
    ...
    if (currentThread->publicFlags & J9_PUBLIC_FLAGS_STOPPED) {
        if (eventNumber != JVMTI_EVENT_VM_DEATH && eventNumber != JVMTI_EVENT_THREAD_END) {
            return FALSE;
        }
    }
    ...
    if (EVENT_IS_ENABLED(eventNumber, &(j9env->globalEventEnable))
        || EVENT_IS_ENABLED(eventNumber, &(THREAD_DATA_FOR_VMTHREAD(j9env, currentThread)->threadEventEnable))) {
        j9object_t* refs;
...
void javaOffloadSwitchOff(J9JVMTIEnv* j9env, J9VMThread* currentThread, UDATA eventNumber, UDATA* javaOffloadOldState)
{
    ...
    if ((NULL != vm->javaOffloadSwitchOffWithReasonFunc) && (TRUE == zAAPSwitchingFlag)) {
        ...
        *javaOffloadOldState = currentThread->javaOffloadState;
        ...
        vm->javaOffloadSwitchOffWithReasonFunc(
            currentThread, reasonCodeFromJVMTIEvent[eventNumber - J9JVMTI_LOWEST_EVENT]);
    }
}
#endif /* J9VM_OPT_JAVA_OFFLOAD_SUPPORT */
...
void finishedEvent(J9VMThread* currentThread, UDATA eventNumber, UDATA hadVMAccess, UDATA javaOffloadOldState)
{
    J9SFJNINativeMethodFrame* frame
        = ((J9SFJNINativeMethodFrame*)(((U_8*)currentThread->sp) + (UDATA)currentThread->literals));
...
void javaOffloadSwitchOn(J9VMThread* currentThread, UDATA eventNumber, UDATA javaOffloadOldState)
{
    ...
    if (NULL != vm->javaOffloadSwitchOnWithReasonFunc) {
        if (0 == currentThread->javaOffloadState) {
            ...
            vm->javaOffloadSwitchOnWithReasonFunc(
                currentThread, reasonCodeFromJVMTIEvent[eventNumber - J9JVMTI_LOWEST_EVENT]);
...
J9Class* getCurrentClass(J9Class* clazz) { return J9_CURRENT_CLASS(clazz); }
...
static void clearSingleBreakpoint(J9VMThread* currentThread, J9JVMTIGlobalBreakpoint* globalBreakpoint)
{
    ...
    J9_BYTECODE_START_FROM_ROM_METHOD(breakpointedMethod->copiedROMMethod)
    [location] = J9_BYTECODE_START_FROM_ROM_METHOD(breakpointedMethod->originalROMMethod)[location];
...
jvmtiError installAgentBreakpoint(J9VMThread* currentThread, J9JVMTIAgentBreakpoint* agentBreakpoint)
{
    ...
    return setGlobalBreakpoint(
        currentThread, ramMethod, agentBreakpoint->location, &(agentBreakpoint->globalBreakpoint));
}
...
jvmtiError setEventNotificationMode(
    J9JVMTIEnv* j9env, J9VMThread* currentThread, jint mode, jint event_type, jthread event_thread, jint low, jint high)
{
    jvmtiError rc = JVMTI_ERROR_NONE;
    ...
    BOOLEAN shouldDecompileAllThreads = J9_FSD_ENABLED(vm)
        && ((event_type == JVMTI_EVENT_METHOD_ENTRY) || (event_type == JVMTI_EVENT_METHOD_EXIT)
               || (event_type == JVMTI_EVENT_SINGLE_STEP));
...
findDecompileInfo(J9VMThread* currentThread, J9VMThread* targetThread, UDATA depth, J9StackWalkState* walkState)
{
    ...
    walkState->flags = J9_STACKWALK_ITERATE_FRAMES | J9_STACKWALK_INCLUDE_NATIVES | J9_STACKWALK_VISIBLE_ONLY
        | J9_STACKWALK_RECORD_BYTECODE_PC_OFFSET | J9_STACKWALK_MAINTAIN_REGISTER_MAP;
