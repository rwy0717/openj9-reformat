...
typedef IDATA (*J9HookRedirectorFunction)(J9JVMTIHookInterfaceWithID* hookInterfaceWithID, UDATA eventNum,
...
#endif /* J9VM_GC_DYNAMIC_CLASS_UNLOADING */
static IDATA hookRegister(J9JVMTIHookInterfaceWithID* hookInterfaceWithID, UDATA eventNum, J9HookFunction function,
...
static IDATA hookReserve(J9JVMTIHookInterfaceWithID* hookInterfaceWithID, UDATA eventNum, J9HookFunction function,
    const char* callsite, void* userData);
...
static UDATA lookupNativeAddressHelper(J9VMThread* currentThread, J9JVMTIData* jvmtiData, J9Method* nativeMethod,
    UDATA prefixOffset, UDATA retransformFlag, UDATA functionArgCount, lookupNativeAddressCallback callback);
static void jvmtiHookFindNativeToRegister(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData);
static UDATA removeMethodPrefixesHelper(
    J9JVMTIData* jvmtiData, U_8* methodName, UDATA methodPrefixSize, UDATA prefixOffset, UDATA retransformFlag);
...
#endif /* J9VM_GC_DYNAMIC_CLASS_UNLOADING */
static IDATA hookUnregister(J9JVMTIHookInterfaceWithID* hookInterfaceWithID, UDATA eventNum, J9HookFunction function,
    const char* callsite, void* userData);
...
static IDATA hookIsDisabled(J9JVMTIHookInterfaceWithID* hookInterfaceWithID, UDATA eventNum, J9HookFunction function,
    const char* callsite, void* userData);
...
static UDATA findFieldIndexFromOffset(
    J9VMThread* currentThread, J9Class* clazz, UDATA offset, UDATA isStatic, J9Class** declaringClass);
static jfieldID findWatchedField(
    J9VMThread* currentThread, J9JVMTIEnv* j9env, UDATA isWrite, UDATA isStatic, UDATA tag, J9Class* fieldClass);
...
static void jvmtiHookThreadEnd(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (callback != NULL) {
        ...
        if (prepareForEvent(j9env, currentThread, currentThread, JVMTI_EVENT_THREAD_END, &threadRef, &hadVMAccess,
                FALSE, 0, &javaOffloadOldState)) {
...
static void jvmtiHookThreadStarted(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (callback != NULL) {
        ...
        if (prepareForEvent(j9env, currentThread, data->vmThread, JVMTI_EVENT_THREAD_START, &threadRef, &hadVMAccess,
                FALSE, 0, &javaOffloadOldState)) {
            callback((jvmtiEnv*)j9env, (JNIEnv*)currentThread, threadRef);
...
static void jvmtiHookVMInitialized(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (callback != NULL) {
        ...
        if (prepareForEvent(j9env, currentThread, currentThread, JVMTI_EVENT_VM_INIT, &threadRef, &hadVMAccess, FALSE,
                0, &javaOffloadOldState)) {
            callback((jvmtiEnv*)j9env, (JNIEnv*)currentThread, threadRef);
...
static void jvmtiHookMethodEnter(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (callback != NULL) {
        ...
        if (prepareForEvent(j9env, currentThread, currentThread, JVMTI_EVENT_METHOD_ENTRY, &threadRef, &hadVMAccess,
                TRUE, 0, &javaOffloadOldState)) {
    ...
    const char* callsite, void* userData)
{
    ...
    return (*hookInterface)
        ->J9HookRegisterWithCallSite(
            hookInterface, J9HOOK_TAG_AGENT_ID | eventNum, function, callsite, userData, hookInterfaceWithID->agentID);
}

/*
 * parameter "callsite" has been added in hookReserve(), hookUnregister() and hookIsDisabled(), but it never been used
 * in those functions it is just for matching hookRegister()(need callsite for hook tracepoint and hook java dump) in
 * order to share code in processEvent().
 */
...
static IDATA processEvent(J9JVMTIEnv* j9env, jint event, J9HookRedirectorFunction redirectorFunction)
{
    ...
    case JVMTI_EVENT_VM_START:
        return redirectorFunction(vmHook, J9HOOK_VM_STARTED, jvmtiHookVMStarted, OMR_GET_CALLSITE(), j9env)
            && redirectorFunction(
                   vmHook, J9HOOK_JAVA_BASE_LOADED, jvmtiHookModuleSystemStarted, OMR_GET_CALLSITE(), j9env);
    ...
    case JVMTI_EVENT_METHOD_ENTRY:
        return redirectorFunction(vmHook, J9HOOK_VM_METHOD_ENTER, jvmtiHookMethodEnter, OMR_GET_CALLSITE(), j9env)
            || redirectorFunction(
                   vmHook, J9HOOK_VM_NATIVE_METHOD_ENTER, jvmtiHookMethodEnter, OMR_GET_CALLSITE(), j9env);
    ...
    case JVMTI_EVENT_FIELD_MODIFICATION:
        return redirectorFunction(vmHook, J9HOOK_VM_PUT_FIELD, jvmtiHookFieldModification, OMR_GET_CALLSITE(), j9env)
            || redirectorFunction(
                   vmHook, J9HOOK_VM_PUT_STATIC_FIELD, jvmtiHookFieldModification, OMR_GET_CALLSITE(), j9env);
    ...
    case JVMTI_EVENT_CLASS_FILE_LOAD_HOOK:
        return redirectorFunction(vmHook,
            (j9env->flags & J9JVMTIENV_FLAG_RETRANSFORM_CAPABLE) ? J9HOOK_VM_CLASS_LOAD_HOOK2
                                                                 : J9HOOK_VM_CLASS_LOAD_HOOK,
            jvmtiHookClassFileLoadHook, OMR_GET_CALLSITE(), j9env);
    ...
    case JVMTI_EVENT_EXCEPTION:
        return redirectorFunction(
            vmHook, J9HOOK_VM_EXCEPTION_THROW, jvmtiHookExceptionThrow, OMR_GET_CALLSITE(), j9env);

    case JVMTI_EVENT_EXCEPTION_CATCH:
        return redirectorFunction(
            vmHook, J9HOOK_VM_EXCEPTION_CATCH, jvmtiHookExceptionCatch, OMR_GET_CALLSITE(), j9env);
    ...
    case JVMTI_EVENT_VM_OBJECT_ALLOC:
        return redirectorFunction(
            vmHook, J9HOOK_VM_OBJECT_ALLOCATE, jvmtiHookObjectAllocate, OMR_GET_CALLSITE(), j9env);
    ...
    case JVMTI_EVENT_METHOD_EXIT:
        return redirectorFunction(vmHook, J9HOOK_VM_METHOD_RETURN, jvmtiHookMethodExit, OMR_GET_CALLSITE(), j9env)
            || redirectorFunction(
                   vmHook, J9HOOK_VM_NATIVE_METHOD_RETURN, jvmtiHookMethodExit, OMR_GET_CALLSITE(), j9env);
    ...
    case JVMTI_EVENT_MONITOR_CONTENDED_ENTER:
        return redirectorFunction(
            vmHook, J9HOOK_VM_MONITOR_CONTENDED_ENTER, jvmtiHookMonitorContendedEnter, OMR_GET_CALLSITE(), j9env);

    case JVMTI_EVENT_MONITOR_CONTENDED_ENTERED:
        return redirectorFunction(
            vmHook, J9HOOK_VM_MONITOR_CONTENDED_ENTERED, jvmtiHookMonitorContendedEntered, OMR_GET_CALLSITE(), j9env);
    ...
    case JVMTI_EVENT_COMPILED_METHOD_LOAD:
        return redirectorFunction(vmHook, J9HOOK_VM_DYNAMIC_CODE_LOAD, jvmtiHookDynamicCodeLoad, OMR_GET_CALLSITE(),
            J9JVMTI_DATA_FROM_ENV(j9env));

    case JVMTI_EVENT_COMPILED_METHOD_UNLOAD:
        return redirectorFunction(vmHook, J9HOOK_VM_DYNAMIC_CODE_UNLOAD, jvmtiHookDynamicCodeUnload, OMR_GET_CALLSITE(),
            J9JVMTI_DATA_FROM_ENV(j9env));
#endif

    case JVMTI_EVENT_RESOURCE_EXHAUSTED:
        return redirectorFunction(
            vmHook, J9HOOK_VM_RESOURCE_EXHAUSTED, jvmtiHookResourceExhausted, OMR_GET_CALLSITE(), j9env);

        /* Extension Events */

    case J9JVMTI_EVENT_COM_IBM_INSTRUMENTABLE_OBJECT_ALLOC:
        return redirectorFunction(
            vmHook, J9HOOK_VM_OBJECT_ALLOCATE_INSTRUMENTABLE, jvmtiHookObjectAllocate, OMR_GET_CALLSITE(), j9env);
    ...
    case J9JVMTI_EVENT_COM_IBM_GARBAGE_COLLECTION_CYCLE_START:
        return redirectorFunction(
            gcOmrHook, J9HOOK_MM_OMR_GC_CYCLE_START, jvmtiHookGCCycleStart, OMR_GET_CALLSITE(), j9env);

    case J9JVMTI_EVENT_COM_IBM_GARBAGE_COLLECTION_CYCLE_FINISH:
        return redirectorFunction(
            gcOmrHook, J9HOOK_MM_OMR_GC_CYCLE_END, jvmtiHookGCCycleEnd, OMR_GET_CALLSITE(), j9env);

    case J9JVMTI_EVENT_COM_IBM_COMPILING_START:
        if (jitHook != NULL) {
            return redirectorFunction(
                jitHook, J9HOOK_JIT_COMPILING_START, jvmtiHookCompilingStart, OMR_GET_CALLSITE(), j9env);
        } else {
            return 0;
        }
    ...
    case J9JVMTI_EVENT_COM_IBM_COMPILING_END:
        if (jitHook != NULL) {
            return redirectorFunction(
                jitHook, J9HOOK_JIT_COMPILING_END, jvmtiHookCompilingEnd, OMR_GET_CALLSITE(), j9env);
        } else {
            return 0;
        }
    }
...
IDATA
hookEvent(J9JVMTIEnv* j9env, jint event) { return processEvent(j9env, event, hookRegister); }

IDATA
unhookEvent(J9JVMTIEnv* j9env, jint event) { return processEvent(j9env, event, hookUnregister); }

IDATA
reserveEvent(J9JVMTIEnv* j9env, jint event) { return processEvent(j9env, event, hookReserve); }
...
static void jvmtiHookVMStartedFirst(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (jvmtiData->compileEventThread != NULL) {
        ...
        vm->internalVMFunctions->initializeAttachedThread(currentThread, "JVMTI event reporting thread",
            vm->systemThreadGroupRef, ((compileEventVMThread->privateFlags & J9_PRIVATE_FLAGS_DAEMON_THREAD) != 0),
...
static void jvmtiHookVMStarted(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (callback != NULL) {
        ...
        if (TRUE == reportEvent) {
            if (prepareForEvent(j9env, currentThread, currentThread, JVMTI_EVENT_VM_START, NULL, &hadVMAccess, FALSE, 0,
                    &javaOffloadOldState)) {
...
static void jvmtiHookClassLoad(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (J9ROMCLASS_IS_PRIMITIVE_OR_ARRAY(data->clazz->romClass) == 0) {
        ...
        if (callback != NULL) {
            ...
            if (prepareForEvent(j9env, currentThread, currentThread, JVMTI_EVENT_CLASS_LOAD, &threadRef, &hadVMAccess,
                    TRUE, 1, &javaOffloadOldState)) {
...
UDATA
isEventHookable(J9JVMTIEnv* j9env, jvmtiEvent event) { return processEvent(j9env, event, hookIsDisabled) == 0; }
...
hookNonEventCapabilities(J9JVMTIEnv* j9env, jvmtiCapabilities* capabilities)
{
    ...
    if (capabilities->can_access_local_variables) {
        if (enableDebugAttribute(
                j9env, J9VM_DEBUG_ATTRIBUTE_LOCAL_VARIABLE_TABLE | J9VM_DEBUG_ATTRIBUTE_CAN_ACCESS_LOCALS)) {
            return 1;
        }
    ...
    if (capabilities->can_pop_frame) {
        if (hookRegister(
                vmHook, J9HOOK_VM_POP_FRAMES_INTERRUPT, jvmtiHookPopFramesInterrupt, OMR_GET_CALLSITE(), jvmtiData)) {
            return 1;
        }
    }
    ...
    if (capabilities->can_retransform_classes) {
        if (enableDebugAttribute(
                j9env, J9VM_DEBUG_ATTRIBUTE_ALLOW_RETRANSFORM | J9VM_DEBUG_ATTRIBUTE_CAN_REDEFINE_CLASSES)) {
            return 1;
        }
...
static void jvmtiHookClassPrepare(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    Trc_JVMTI_jvmtiHookClassPrepare_Entry(data->clazz,
    ...
    /* Do not report the event for arrays or primitive types - currently the VM does not send them, so no code needed
     * here */
    ...
    if (callback != NULL) {
        ...
        if (prepareForEvent(j9env, currentThread, currentThread, JVMTI_EVENT_CLASS_PREPARE, &threadRef, &hadVMAccess,
                TRUE, 1, &javaOffloadOldState)) {
            j9object_t* classRef = (j9object_t*)currentThread->arg0EA;
...
static void jvmtiHookSingleStep(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (callback != NULL) {
        ...
        if (prepareForEvent(j9env, currentThread, currentThread, JVMTI_EVENT_SINGLE_STEP, &threadRef, &hadVMAccess,
                TRUE, 0, &javaOffloadOldState)) {
            J9JavaVM* vm = currentThread->javaVM;
            jmethodID methodID;
...
static void jvmtiHookFieldAccess(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (callback != NULL) {
        ...
        if (fieldID != NULL) {
            ...
            if (prepareForEvent(j9env, currentThread, currentThread, JVMTI_EVENT_FIELD_ACCESS, &threadRef, &hadVMAccess,
                    TRUE, (object == NULL) ? 1 : 2, &javaOffloadOldState)) {
                J9JavaVM* vm = currentThread->javaVM;
                jmethodID methodID;
                j9object_t* classRef = (j9object_t*)currentThread->arg0EA;
                ...
                *classRef = J9VM_J9CLASS_TO_HEAPCLASS(clazz);
                ...
                if (methodID != NULL) {
                    callback((jvmtiEnv*)j9env, (JNIEnv*)currentThread, threadRef, methodID, (jlocation)location,
                        (jclass)classRef, (jobject)objectRef, fieldID);
                }
...
static void jvmtiHookFieldModification(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (callback != NULL) {
        ...
        if (fieldID != NULL) {
            ...
            if (prepareForEvent(j9env, currentThread, currentThread, JVMTI_EVENT_FIELD_MODIFICATION, &threadRef,
                    &hadVMAccess, TRUE, refCount, &javaOffloadOldState)) {
                J9JavaVM* vm = currentThread->javaVM;
                jmethodID methodID;
                j9object_t* classRef = (j9object_t*)currentThread->arg0EA;
                J9Class* clazz;
                ...
                *classRef = J9VM_J9CLASS_TO_HEAPCLASS(clazz);
                ...
                if (methodID != NULL) {
                    callback((jvmtiEnv*)j9env, (JNIEnv*)currentThread, threadRef, methodID, (jlocation)location,
                        (jclass)classRef, (jobject)objectRef, fieldID, signatureType, newValue);
                }
                vm->internalVMFunctions->internalEnterVMFromJNI(currentThread);
                if (NULL != object) {
                    J9VMPutFieldEvent* data = eventData;
    ...
    J9VMThread* currentThread, J9JVMTIEnv* j9env, UDATA isWrite, UDATA isStatic, UDATA tag, J9Class* fieldClass)
{
    ...
    if (J9_ARE_ANY_BITS_SET(fieldClass->classFlags, J9ClassHasWatchedFields)) {
        ...
        if (NULL != watchedClass) {
            ...
            if (isWrite) {
                found = watchBits[J9JVMTI_WATCHED_FIELD_ARRAY_INDEX(index)]
                    & J9JVMTI_WATCHED_FIELD_MODIFICATION_BIT(index);
            } else {
...
static void jvmtiHookVMShutdown(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (callback != NULL) {
        ...
        if (prepareForEvent(j9env, currentThread, currentThread, JVMTI_EVENT_VM_DEATH, NULL, &hadVMAccess, FALSE, 0,
                &javaOffloadOldState)) {
            callback((jvmtiEnv*)j9env, (JNIEnv*)currentThread);
...
static void jvmtiHookBreakpoint(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (callback != NULL) {
        ...
        /* See if this agent has a breakpoint at the current location - assume the current thread has VM access at this
         * point */

        if ((agentBreakpoint = findAgentBreakpoint(currentThread, j9env, method, location)) != NULL) {
            if (prepareForEvent(j9env, currentThread, currentThread, JVMTI_EVENT_BREAKPOINT, &threadRef, &hadVMAccess,
                    TRUE, 0, &javaOffloadOldState)) {
...
static void jvmtiHookExceptionThrow(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (callback != NULL) {
        ...
        case J9_EXCEPT_SEARCH_JIT_HANDLER:
            catchMethod = vm->jitConfig->jitGetExceptionCatcher(currentThread, currentThread->stackWalkState->userData2,
                currentThread->stackWalkState->jitInfo, &catchLocation);
            break;
        ...
        if (prepareForEvent(j9env, currentThread, currentThread, JVMTI_EVENT_EXCEPTION, &threadRef, &hadVMAccess, TRUE,
                1, &javaOffloadOldState)) {
            ...
            *exceptionRef = exception;
            ...
            if (throwMethodID != NULL) {
                callback((jvmtiEnv*)j9env, (JNIEnv*)currentThread, threadRef, throwMethodID, (jlocation)throwLocation,
                    (jobject)exceptionRef, catchMethodID, (jlocation)catchLocation);
            }
...
static void jvmtiHookMethodExit(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (callback != NULL) {
        ...
        if (!poppedByException) {
            J9UTF8* signature = J9ROMMETHOD_GET_SIGNATURE(
                UNTAGGED_METHOD_CP(method)->ramClass->romClass, J9_ROM_METHOD_FROM_RAM_METHOD(method));

            if ((J9UTF8_DATA(signature)[J9UTF8_LENGTH(signature) - 2] == '[')
                || ((signatureType = J9UTF8_DATA(signature)[J9UTF8_LENGTH(signature) - 1]) == ';')) {
                signatureType = 'L';
                if (*((void**)valueAddress) != NULL) {
                    ++refCount;
                }
        ...
        if (prepareForEvent(j9env, currentThread, currentThread, JVMTI_EVENT_METHOD_EXIT, &threadRef, &hadVMAccess,
                TRUE, refCount, &javaOffloadOldState)) {
            J9JavaVM* vm = currentThread->javaVM;
            jmethodID methodID;
            jvalue returnValue;
...
static void jvmtiHookUserInterrupt(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (callback != NULL) {
        ...
        rc = (jint)vm->internalVMFunctions->AttachCurrentThreadAsDaemon(
            (JavaVM*)vm, (void**)&currentThread, (void*)&thr_args);
        if (rc == JNI_OK) {

            if (prepareForEvent(j9env, currentThread, currentThread, JVMTI_EVENT_DATA_DUMP_REQUEST, NULL, &hadVMAccess,
                    FALSE, 0, &javaOffloadOldState)) {
...
static void jvmtiHookFindMethodFromPC(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (breakpointedMethod == NULL) {
        /* scan the ROM methods of the class (to handle the case of PCs in stack traces which came from breakpointed
         *methods
...
static void jvmtiHookGetEnv(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (data->rc == JNI_EVERSION) {
        ...
        if ((version == JVMTI_VERSION_1_0) || (version == JVMTI_VERSION_1_1) || (version == JVMTI_VERSION_1_2)
            || (version == JVMTI_VERSION_9_0)) {
            /* Jazz 99339: obtain the pointer to J9JavaVM from J9InvocationJavaVM so as to store J9NativeLibrary in
             * J9JVMTIEnv */
...
hookGlobalEvents(J9JVMTIData* jvmtiData)
{
...
#ifdef J9VM_JIT_FULL_SPEED_DEBUG
    if (jitHook != NULL) {
        if ((*jitHook)->J9HookRegisterWithCallSite(jitHook, J9HOOK_JIT_CHECK_FOR_DATA_BREAKPOINT,
                jvmtiHookCheckForDataBreakpoint, OMR_GET_CALLSITE(), jvmtiData)) {
            return 1;
        }
    }
#endif
    ...
    if ((*vmHook)->J9HookRegisterWithCallSite(
            vmHook, J9HOOK_VM_CLASS_UNLOAD, jvmtiHookClassUnload, OMR_GET_CALLSITE(), jvmtiData)) {
        return 1;
    }
#endif
    ...
    if ((*vmHook)->J9HookRegisterWithCallSite(
            vmHook, J9HOOK_VM_GETENV, jvmtiHookGetEnv, OMR_GET_CALLSITE(), jvmtiData)) {
        return 1;
    }

    if ((*vmHook)->J9HookRegisterWithCallSite(
            vmHook, J9HOOK_VM_PERMANENT_PC, jvmtiHookPermanentPC, OMR_GET_CALLSITE(), jvmtiData)) {
        return 1;
    }

    if ((*vmHook)->J9HookRegisterWithCallSite(
            vmHook, J9HOOK_VM_FIND_METHOD_FROM_PC, jvmtiHookFindMethodFromPC, OMR_GET_CALLSITE(), jvmtiData)) {
        return 1;
    }

    if ((*vmHook)->J9HookRegisterWithCallSite(
            vmHook, J9HOOK_VM_LOOKUP_NATIVE_ADDRESS, jvmtiHookLookupNativeAddress, OMR_GET_CALLSITE(), jvmtiData)) {
        return 1;
    }

    if ((*vmHook)->J9HookRegisterWithCallSite(
            vmHook, J9HOOK_VM_FIND_NATIVE_TO_REGISTER, jvmtiHookFindNativeToRegister, OMR_GET_CALLSITE(), jvmtiData)) {
        return 1;
    }

    if ((*vmHook)->J9HookRegisterWithCallSite(vmHook, J9HOOK_TAG_AGENT_ID | J9HOOK_VM_INITIALIZED,
            jvmtiHookVMInitializedFirst, OMR_GET_CALLSITE(), jvmtiData, J9HOOK_AGENTID_FIRST)) {
        return 1;
    }

    if ((*vmHook)->J9HookRegisterWithCallSite(vmHook, J9HOOK_TAG_AGENT_ID | J9HOOK_VM_STARTED, jvmtiHookVMStartedFirst,
            OMR_GET_CALLSITE(), jvmtiData, J9HOOK_AGENTID_FIRST)) {
        return 1;
    }

    if ((*vmHook)->J9HookRegisterWithCallSite(vmHook, J9HOOK_TAG_AGENT_ID | J9HOOK_VM_SHUTTING_DOWN,
            jvmtiHookVMShutdownLast, OMR_GET_CALLSITE(), jvmtiData, J9HOOK_AGENTID_LAST)) {
        return 1;
    }
...
#ifdef J9VM_JIT_FULL_SPEED_DEBUG
    if (jitHook != NULL) {
        (*jitHook)->J9HookUnregister(
            jitHook, J9HOOK_JIT_CHECK_FOR_DATA_BREAKPOINT, jvmtiHookCheckForDataBreakpoint, NULL);
    }
#endif
...
static void jvmtiHookMonitorContendedEnter(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (callback != NULL) {
        ...
        if (prepareForEvent(j9env, currentThread, currentThread, JVMTI_EVENT_MONITOR_CONTENDED_ENTER, &threadRef,
                &hadVMAccess, TRUE, 0, &javaOffloadOldState)) {
            J9JavaVM* vm = currentThread->javaVM;
            jobject objectRef = 0;

            if (lock && ((lock->flags & J9THREAD_MONITOR_OBJECT) == J9THREAD_MONITOR_OBJECT)) {
                objectRef = (jobject)vm->internalVMFunctions->j9jni_createLocalRef(
                    (JNIEnv*)currentThread, (j9object_t)lock->userData);
            }
...
static void jvmtiHookMonitorContendedEntered(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (callback != NULL) {
        ...
        if (prepareForEvent(j9env, currentThread, currentThread, JVMTI_EVENT_MONITOR_CONTENDED_ENTERED, &threadRef,
                &hadVMAccess, TRUE, 0, &javaOffloadOldState)) {
            J9JavaVM* vm = currentThread->javaVM;
            jobject objectRef = 0;
...
static void jvmtiHookMonitorWait(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (callback != NULL) {
        ...
        if (prepareForEvent(j9env, currentThread, currentThread, JVMTI_EVENT_MONITOR_WAIT, &threadRef, &hadVMAccess,
                TRUE, 0, &javaOffloadOldState)) {
            J9JavaVM* vm = currentThread->javaVM;
            jobject objectRef = 0;
...
static void jvmtiHookMonitorWaited(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (callback != NULL) {
        ...
        if (prepareForEvent(j9env, currentThread, currentThread, JVMTI_EVENT_MONITOR_WAITED, &threadRef, &hadVMAccess,
                TRUE, 0, &javaOffloadOldState)) {
            J9JavaVM* vm = currentThread->javaVM;
            jobject objectRef = 0;
...
static void jvmtiHookFramePop(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (callback != NULL) {
        ...
        if (prepareForEvent(j9env, currentThread, currentThread, JVMTI_EVENT_FRAME_POP, &threadRef, &hadVMAccess, TRUE,
                0, &javaOffloadOldState)) {
            J9JavaVM* vm = currentThread->javaVM;
            J9Method* method = data->method;
            jmethodID methodID;
            ...
            if (methodID != NULL) {
                callback(
                    (jvmtiEnv*)j9env, (JNIEnv*)currentThread, threadRef, methodID, (jboolean)data->poppedByException);
            }
...
static void jvmtiHookExceptionCatch(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (callback != NULL) {
        ...
        if (prepareForEvent(j9env, currentThread, currentThread, JVMTI_EVENT_EXCEPTION_CATCH, &threadRef, &hadVMAccess,
                TRUE, (exception == NULL) ? 0 : 1, &javaOffloadOldState)) {
            jmethodID catchMethodID;
            j9object_t* exceptionRef = (j9object_t*)currentThread->arg0EA;
            ...
            vm->internalVMFunctions->internalExitVMToJNI(currentThread);
            if (catchMethodID != NULL) {
                callback((jvmtiEnv*)j9env, (JNIEnv*)currentThread, threadRef, catchMethodID, (jlocation)catchLocation,
...
static void jvmtiHookResourceExhausted(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (callback != NULL) {
        ...
        if (prepareForEvent(j9env, currentThread, currentThread, JVMTI_EVENT_RESOURCE_EXHAUSTED, &threadRef,
                &hadVMAccess, TRUE, 0, &javaOffloadOldState)) {
            J9JavaVM* vm = currentThread->javaVM;
...
static void jvmtiHookGCEnd(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (deletedHead != NULL) {
        ...
        reportObjectFreeEvents = (phase == JVMTI_PHASE_LIVE) && (objectFreeCallback != NULL)
        ...
        do {
            ...
            if (reportObjectFreeEvents) {
                ...
                if (J9_ARE_ALL_BITS_SET(
                        currentThread->javaVM->extendedRuntimeFlags, J9_EXTENDED_RUNTIME_RESTRICT_IFA)) {
        ...
        if (gcFinishCallback != NULL) {
            ...
            */
            ...
            if (EVENT_IS_ENABLED(JVMTI_EVENT_GARBAGE_COLLECTION_FINISH, &(j9env->globalEventEnable))) {
                ...
                if (J9_ARE_ALL_BITS_SET(
                        currentThread->javaVM->extendedRuntimeFlags, J9_EXTENDED_RUNTIME_RESTRICT_IFA)) {
                    javaOffloadSwitchOff(
                        j9env, currentThread, JVMTI_EVENT_GARBAGE_COLLECTION_FINISH, &javaOffloadOldState);
                }
#endif /* J9VM_OPT_JAVA_OFFLOAD_SUPPORT */
...
static void jvmtiHookGCCycleStart(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    jvmtiExtensionEvent callback
        = J9JVMTI_EXTENSION_CALLBACK(j9env, J9JVMTI_EVENT_COM_IBM_GARBAGE_COLLECTION_CYCLE_START);
    ...
    if (callback != NULL) {
        ...
        */
        ...
        if (EVENT_IS_ENABLED(J9JVMTI_EVENT_COM_IBM_GARBAGE_COLLECTION_CYCLE_START, &(j9env->globalEventEnable))) {
            ...
            if (J9_ARE_ALL_BITS_SET(currentThread->javaVM->extendedRuntimeFlags, J9_EXTENDED_RUNTIME_RESTRICT_IFA)) {
                javaOffloadSwitchOff(
                    j9env, currentThread, J9JVMTI_EVENT_COM_IBM_GARBAGE_COLLECTION_CYCLE_START, &javaOffloadOldState);
            }
#endif /* J9VM_OPT_JAVA_OFFLOAD_SUPPORT */
            callback((jvmtiEnv*)j9env);
#if defined(J9VM_OPT_JAVA_OFFLOAD_SUPPORT)
            ...
            if (J9_ARE_ALL_BITS_SET(currentThread->javaVM->extendedRuntimeFlags, J9_EXTENDED_RUNTIME_RESTRICT_IFA)) {
                javaOffloadSwitchOn(
                    currentThread, J9JVMTI_EVENT_COM_IBM_GARBAGE_COLLECTION_CYCLE_START, javaOffloadOldState);
            }
#endif /* J9VM_OPT_JAVA_OFFLOAD_SUPPORT */
        }
    }
...
static void jvmtiHookGCCycleEnd(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    J9JVMTIEnv* j9env = userData;
    jvmtiExtensionEvent callback
        = J9JVMTI_EXTENSION_CALLBACK(j9env, J9JVMTI_EVENT_COM_IBM_GARBAGE_COLLECTION_CYCLE_FINISH);
    ...
    if (callback != NULL) {
        ...
        */
        ...
        if (EVENT_IS_ENABLED(J9JVMTI_EVENT_COM_IBM_GARBAGE_COLLECTION_CYCLE_FINISH, &(j9env->globalEventEnable))) {
            ...
            if (J9_ARE_ALL_BITS_SET(currentThread->javaVM->extendedRuntimeFlags, J9_EXTENDED_RUNTIME_RESTRICT_IFA)) {
                javaOffloadSwitchOff(
                    j9env, currentThread, J9JVMTI_EVENT_COM_IBM_GARBAGE_COLLECTION_CYCLE_FINISH, &javaOffloadOldState);
            }
#endif /* J9VM_OPT_JAVA_OFFLOAD_SUPPORT */
            callback((jvmtiEnv*)j9env);
#if defined(J9VM_OPT_JAVA_OFFLOAD_SUPPORT)
            ...
            if (J9_ARE_ALL_BITS_SET(currentThread->javaVM->extendedRuntimeFlags, J9_EXTENDED_RUNTIME_RESTRICT_IFA)) {
                javaOffloadSwitchOn(
                    currentThread, J9JVMTI_EVENT_COM_IBM_GARBAGE_COLLECTION_CYCLE_FINISH, javaOffloadOldState);
            }
#endif /* J9VM_OPT_JAVA_OFFLOAD_SUPPORT */
        }
    }
 ...
 */
static BOOLEAN canClassBeInstrumented(J9VMClassLoadHookEvent* classLoadData)
{
    BOOLEAN result = TRUE;
...
static void jvmtiHookClassFileLoadHook(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if ((J2SE_VERSION(vm) >= J2SE_19) && (j9env->capabilities.can_generate_early_class_hook_events == 0)) {
    ...
    if (callback != NULL) {
        ...
        J9JavaVM* vm = currentThread->javaVM;
        UDATA hadVMAccess;
        j9object_t classLoaderObject = (data->classLoader == vm->systemClassLoader)
            ? NULL
            : J9CLASSLOADER_CLASSLOADEROBJECT(currentThread, data->classLoader);
        ...
        if (prepareForEvent(j9env, currentThread, currentThread, JVMTI_EVENT_CLASS_FILE_LOAD_HOOK, NULL, &hadVMAccess,
                TRUE, extraRefs, &javaOffloadOldState)) {
            ...
            callback((jvmtiEnv*)j9env, (JNIEnv*)currentThread, (jclass)classRef, (jobject)classLoaderRef,
                (const char*)data->className, (jobject)pdRef, (jint)data->classDataLength,
                (const unsigned char*)data->classData, &newLength, &newData);
...
static void jvmtiHookLookupNativeAddress(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (!J9_NATIVE_METHOD_IS_BOUND(nativeMethod)) {
        ...
        prefixOffset = lookupNativeAddressHelper(
            currentThread, jvmtiData, nativeMethod, 0, J9JVMTIENV_FLAG_RETRANSFORM_CAPABLE, functionArgCount, callback);
        if (!J9_NATIVE_METHOD_IS_BOUND(nativeMethod)) {
            /* Remove prefixes for retransform-incapable agents */

            lookupNativeAddressHelper(
                currentThread, jvmtiData, nativeMethod, prefixOffset, 0, functionArgCount, callback);
        }
    }
    ...
    UDATA prefixOffset, UDATA retransformFlag, UDATA functionArgCount, lookupNativeAddressCallback callback)
{
    ...
    J9UTF8* methodSignature
        = J9ROMMETHOD_GET_SIGNATURE(UNTAGGED_METHOD_CP(nativeMethod)->ramClass->romClass, romMethod);
            ...
            while (prefixCount != 0) {
                ...
                if ((prefixOffset + prefixLength) < nameLength) {
                    if (memcmp(prefix, nameData + prefixOffset, prefixLength) == 0) {
                        ...
                        if (methodExists(
                                methodClass, nameData + prefixOffset, nameLength - prefixOffset, methodSignature)) {
                            U_8* nativeNames = vm->internalVMFunctions->buildNativeFunctionNames(
                                vm, nativeMethod, methodClass, prefixOffset);
                            ...
                            if (nativeNames != NULL) {
                                ...
                                while (nativeLibrary != NULL) {
                                    callback(currentThread, nativeMethod, nativeLibrary, (char*)longJNI,
                                        (char*)shortJNI, functionArgCount, TRUE);
                                    if (J9_NATIVE_METHOD_IS_BOUND(nativeMethod)) {
...
static UDATA methodExists(J9Class* methodClass, U_8* nameData, UDATA nameLength, J9UTF8* signature)
{
    ...
    do {
        ...
        if ((nameLength == J9UTF8_LENGTH(searchName)) && (memcmp(nameData, J9UTF8_DATA(searchName), nameLength) == 0)) {
            if ((signatureLength == J9UTF8_LENGTH(searchSignature))
                && (memcmp(signatureData, J9UTF8_DATA(searchSignature), signatureLength) == 0)) {
...
static void jvmtiHookFindNativeToRegister(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if ((nonNativeRomMethod->modifiers & J9AccNative) == 0) {
        J9UTF8* nonNativeMethodName
            = J9ROMMETHOD_GET_NAME(UNTAGGED_METHOD_CP(nonNativeMethod)->ramClass->romClass, nonNativeRomMethod);
        J9UTF8* nonNativeMethodSignature
            = J9ROMMETHOD_GET_SIGNATURE(UNTAGGED_METHOD_CP(nonNativeMethod)->ramClass->romClass, nonNativeRomMethod);
        ...
        while (count != 0) {
            ...
            if (romMethod->modifiers & J9AccNative) {
                J9UTF8* methodSignature
                    = J9ROMMETHOD_GET_SIGNATURE(UNTAGGED_METHOD_CP(method)->ramClass->romClass, romMethod);

                if ((J9UTF8_LENGTH(methodSignature) == nonNativeMethodSignatureLength)
                    && (memcmp(
                            J9UTF8_DATA(methodSignature), nonNativeMethodSignatureData, nonNativeMethodSignatureLength)
                           == 0)) {
                    J9UTF8* methodName
                        = J9ROMMETHOD_GET_NAME(UNTAGGED_METHOD_CP(method)->ramClass->romClass, romMethod);
                    ...
                    if (methodNameLength > nonNativeMethodNameLength) {
                        ...
                        if (memcmp(methodNameData + prefixSize, nonNativeMethodNameData, nonNativeMethodNameLength)
                            == 0) {
                            UDATA prefixOffset;

                            /* Remove prefixes for retransform-capable agents */

                            prefixOffset = removeMethodPrefixesHelper(
                                jvmtiData, methodNameData, prefixSize, 0, J9JVMTIENV_FLAG_RETRANSFORM_CAPABLE);
                            if (prefixOffset != prefixSize) {
                                /* Remove prefixes for retransform-incapable agents */

                                prefixOffset = removeMethodPrefixesHelper(
                                    jvmtiData, methodNameData, prefixSize, prefixOffset, 0);
                            }
                            if (prefixOffset == prefixSize) {
                                data->method = method;
                                break;
...
static void jvmtiHookCompilingStart(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    /* Call the event callback */
    if (prepareForEvent(j9env, currentThread, currentThread, J9JVMTI_EVENT_COM_IBM_COMPILING_START, NULL, &hadVMAccess,
            TRUE, 0, &javaOffloadOldState)) {
        J9JavaVM* vm = currentThread->javaVM;
        jmethodID methodID;
...
static void jvmtiHookCompilingEnd(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    /* Call the event callback */
    if (prepareForEvent(j9env, currentThread, currentThread, J9JVMTI_EVENT_COM_IBM_COMPILING_END, NULL, &hadVMAccess,
            TRUE, 0, &javaOffloadOldState)) {
        J9JavaVM* vm = currentThread->javaVM;
        jmethodID methodID;
...
static void jvmtiHookObjectAllocate(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (eventNum == J9HOOK_VM_OBJECT_ALLOCATE_INSTRUMENTABLE) {
        jvmtiExtensionEvent callback
            = J9JVMTI_EXTENSION_CALLBACK(j9env, J9JVMTI_EVENT_COM_IBM_INSTRUMENTABLE_OBJECT_ALLOC);
        ...
        if (callback != NULL) {
            ...
            if (prepareForEvent(j9env, currentThread, currentThread, J9JVMTI_EVENT_COM_IBM_INSTRUMENTABLE_OBJECT_ALLOC,
                    &threadRef, &hadVMAccess, TRUE, 2, &javaOffloadOldState)) {
                ...
                *classRef = J9VM_J9CLASS_TO_HEAPCLASS(clazz);
                vm->internalVMFunctions->internalExitVMToJNI(currentThread);
                callback((jvmtiEnv*)j9env, (JNIEnv*)currentThread, threadRef, (jobject)objectRef, (jclass)classRef,
                    (jlong)data->size);
                currentThread->javaVM->internalVMFunctions->internalEnterVMFromJNI(currentThread);
                data->object = J9_JNI_UNWRAP_REDIRECTED_REFERENCE(objectRef);
                finishedEvent(
                    currentThread, J9JVMTI_EVENT_COM_IBM_INSTRUMENTABLE_OBJECT_ALLOC, hadVMAccess, javaOffloadOldState);
            }
        }
    } else {
        ...
        if (callback != NULL) {
            ...
            if (prepareForEvent(j9env, currentThread, currentThread, JVMTI_EVENT_VM_OBJECT_ALLOC, &threadRef,
                    &hadVMAccess, TRUE, 2, &javaOffloadOldState)) {
                j9object_t* objectRef = (j9object_t*)currentThread->arg0EA;
...
static void jvmtiHookJNINativeBind(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (callback != NULL) {
        ...
        if (prepareForEvent(j9env, currentThread, currentThread, JVMTI_EVENT_NATIVE_METHOD_BIND, &threadRef,
                &hadVMAccess, TRUE, 0, &javaOffloadOldState)) {
            J9JavaVM* vm = currentThread->javaVM;
            jmethodID methodID;
            ...
            vm->internalVMFunctions->internalExitVMToJNI(currentThread);
            callback((jvmtiEnv*)j9env, (JNIEnv*)currentThread, threadRef, methodID, data->nativeMethodAddress,
                &(data->nativeMethodAddress));
...
jvmtiError startCompileEventThread(J9JVMTIData* jvmtiData)
{
    ...
    if (NULL == jvmtiData->compileEventThread) {
        jvmtiData->compileEvents = pool_new(sizeof(J9JVMTICompileEvent), 0, 0, 0, J9_GET_CALLSITE(),
            J9MEM_CATEGORY_JVMTI, POOL_FOR_PORT(vm->portLibrary));
        ...
        if (0
            != omrthread_create(&(jvmtiData->compileEventThread), vm->defaultOSStackSize, J9THREAD_PRIORITY_NORMAL,
                   FALSE, compileEventThreadProc, jvmtiData)) {
...
static int J9THREAD_PROC compileEventThreadProc(void* entryArg)
{
    ...
    if (JNI_OK
        == vm->internalVMFunctions->internalAttachCurrentThread(
        ...
        for (;;) {
            ...
            while (J9_LINKED_LIST_IS_EMPTY(jvmtiData->compileEventQueueHead)
                && (jvmtiData->compileEventThreadState == J9JVMTI_COMPILE_EVENT_THREAD_STATE_ALIVE)) {
                omrthread_monitor_notify_all(jvmtiData->compileEventMutex);
                omrthread_monitor_wait(jvmtiData->compileEventMutex);
            }
            ...
            if (compEvent->methodID == NULL) {
                if ((jvmtiData->phase == JVMTI_PHASE_PRIMORDIAL) || (jvmtiData->phase == JVMTI_PHASE_START)
                    || (jvmtiData->phase == JVMTI_PHASE_LIVE)) {
                    omrthread_monitor_exit(jvmtiData->compileEventMutex);
                    ...
                    JVMTI_ENVIRONMENTS_DO(jvmtiData, j9env)
                    {
                        ...
                        if (callback != NULL) {
                            ...
                            if (prepareForEvent(j9env, currentThread, currentThread, JVMTI_EVENT_DYNAMIC_CODE_GENERATED,
                                    &threadRef, &hadVMAccess, FALSE, 0, &javaOffloadOldState)) {
                                callback((jvmtiEnv*)j9env, compEvent->compile_info, compEvent->code_addr,
                                    (jint)compEvent->code_size);
                                finishedEvent(currentThread, JVMTI_EVENT_DYNAMIC_CODE_GENERATED, hadVMAccess,
                                    javaOffloadOldState);
                            }
                        }
                    ...
                    JVMTI_ENVIRONMENTS_DO(jvmtiData, j9env)
                    {
                        ...
                        if (compEvent->isLoad) {
                            ...
                            if (callback != NULL) {
                                ...
                                if (prepareForEvent(j9env, currentThread, currentThread,
                                        JVMTI_EVENT_COMPILED_METHOD_LOAD, &threadRef, &hadVMAccess, FALSE, 0,
                                        &javaOffloadOldState)) {
                                    callback((jvmtiEnv*)j9env, compEvent->methodID, (jint)compEvent->code_size,
                                        compEvent->code_addr, (jint)0, (const jvmtiAddrLocationMap*)NULL,
                                        compEvent->compile_info);
                                    finishedEvent(currentThread, JVMTI_EVENT_COMPILED_METHOD_LOAD, hadVMAccess,
                                        javaOffloadOldState);
                                }
                            }
                        } else {
                            ...
                            if (callback != NULL) {
                                ...
                                if (prepareForEvent(j9env, currentThread, currentThread,
                                        JVMTI_EVENT_COMPILED_METHOD_UNLOAD, &threadRef, &hadVMAccess, FALSE, 0,
                                        &javaOffloadOldState)) {
                                    callback((jvmtiEnv*)j9env, compEvent->methodID, compEvent->code_addr);
                                    finishedEvent(currentThread, JVMTI_EVENT_COMPILED_METHOD_UNLOAD, hadVMAccess,
                                        javaOffloadOldState);
                                }
                            }
                        }
...
enableDebugAttribute(J9JVMTIEnv* j9env, UDATA attributes)
{
    ...
    if ((vm->requiredDebugAttributes & attributes) != attributes) {
        if ((*vmHook)->J9HookRegisterWithCallSite(vmHook, J9HOOK_VM_REQUIRED_DEBUG_ATTRIBUTES,
                jvmtiHookRequiredDebugAttributes, OMR_GET_CALLSITE(), jvmtiData)) {
            return 1;
        }
...
static void jvmtiHookDynamicCodeUnload(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (J9JVMTI_COMPILE_EVENT_THREAD_STATE_DEAD == jvmtiData->compileEventThreadState) {
        ...
        /* No load event was removed, so queue the unload event - currently, only queue compiled method unload, since
         * dynamic code unload is not supported */
...
static void jvmtiHookVmDumpStart(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    /* Call the event callback */
    if (prepareForEvent(j9env, currentThread, currentThread, J9JVMTI_EVENT_COM_IBM_VM_DUMP_START, NULL, &hadVMAccess,
            TRUE, 0, &javaOffloadOldState)) {
        J9JavaVM* vm = currentThread->javaVM;
...
static void jvmtiHookVmDumpEnd(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    /* Call the event callback */
    if (prepareForEvent(j9env, currentThread, currentThread, J9JVMTI_EVENT_COM_IBM_VM_DUMP_END, NULL, &hadVMAccess,
            TRUE, 0, &javaOffloadOldState)) {
        J9JavaVM* vm = currentThread->javaVM;
