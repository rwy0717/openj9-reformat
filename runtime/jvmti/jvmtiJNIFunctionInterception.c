...
jvmtiError JNICALL jvmtiSetJNIFunctionTable(jvmtiEnv* env, const jniNativeInterface* function_table)
{
    ...
    if (jvmtiData->copiedJNITable == NULL) {
        ...
        /* Not copied yet: copy the table into newly-allocated memory (this is strictly internal, so use the port
         * library, not Allocate) */
        ...
        /* Set the global table in the VM (for new threads) and fix all existing threads - need thread list mutex to
         * walk threads */
        ...
        /* Already copied: must re-use the existing copy to avoid freeing something cached by the C compiler in a local
           in a JNI native. Also, must copy the elements a pointer at a time to avoid corrupting the function pointers
           (memcpy may copy byte-wise).
        */
