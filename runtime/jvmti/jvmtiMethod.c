    ...
    jvmtiEnv* env, jmethodID method, char** name_ptr, char** signature_ptr, char** generic_ptr)
{
    ...
    if (name_ptr != NULL) {
        J9UTF8* utf
            = J9ROMMETHOD_GET_NAME(UNTAGGED_METHOD_CP(((J9JNIMethodID*)method)->method)->ramClass->romClass, romMethod);
    ...
    if (signature_ptr != NULL) {
        J9UTF8* utf = J9ROMMETHOD_GET_SIGNATURE(
            UNTAGGED_METHOD_CP(((J9JNIMethodID*)method)->method)->ramClass->romClass, romMethod);
        UDATA length = J9UTF8_LENGTH(utf);
...
jvmtiError JNICALL jvmtiGetMethodDeclaringClass(jvmtiEnv* env, jmethodID method, jclass* declaring_class_ptr)
{
    ...
    if (rc == JVMTI_ERROR_NONE) {
        ...
        rv_declaring_class = (jclass)vm->internalVMFunctions->j9jni_createLocalRef(
            (JNIEnv*)currentThread, J9VM_J9CLASS_TO_HEAPCLASS(methodClass));
...
jvmtiError JNICALL jvmtiGetMethodModifiers(jvmtiEnv* env, jmethodID method, jint* modifiers_ptr)
{
    ...
    rv_modifiers = (jint)(romMethod->modifiers
        & (J9AccPublic | J9AccPrivate | J9AccProtected | J9AccStatic | J9AccFinal | J9AccSynchronized | J9AccNative
              | J9AccAbstract | J9AccStrict | J9AccVarArgs | J9AccBridge));
    rc = JVMTI_ERROR_NONE;
    ...
    jvmtiEnv* env, jmethodID method, jint* entry_count_ptr, jvmtiLineNumberEntry** table_ptr)
{
    ...
    if (methodDebugInfo != NULL) {
        ...
        if (lineTable != NULL) {
            ...
            jvmtiLineTable
                = j9mem_allocate_memory(lineTableSize * sizeof(jvmtiLineNumberEntry), J9MEM_CATEGORY_JVMTI_ALLOCATE);
    ...
    jvmtiEnv* env, jmethodID method, jint* entry_count_ptr, jvmtiLocalVariableEntry** table_ptr)
{
    ...
    if (methodDebugInfo != NULL) {
        ...
        if (values != NULL) {
            ...
            jvmtiVariableTable
                = j9mem_allocate_memory(variableCount * sizeof(jvmtiLocalVariableEntry), J9MEM_CATEGORY_JVMTI_ALLOCATE);
            if (jvmtiVariableTable == NULL) {
                ...
                while (values != NULL) {
                    ...
                    if (((rc = cStringFromUTF(env, values->name, &(jvmtiVariableTable[i].name))) != JVMTI_ERROR_NONE)
                        || ((rc = cStringFromUTF(env, values->signature, &(jvmtiVariableTable[i].signature)))
                               != JVMTI_ERROR_NONE)) {
                        deallocateVariableTable(env, i + 1, jvmtiVariableTable);
                        goto release;
                    }
                    if (values->genericSignature != NULL) {
                        if ((rc = cStringFromUTF(
                                 env, values->genericSignature, &(jvmtiVariableTable[i].generic_signature)))
                            != JVMTI_ERROR_NONE) {
                            deallocateVariableTable(env, i + 1, jvmtiVariableTable);
                            goto release;
                        }
 ...
 * @param bytecodes_ptr 	[OUT] Bytecodes for the specified method. User is responsible for freeing the memory with
 * Deallocate
        ...
        while (index < size) {
            ...
            case JBinvokedynamic:
                ...
                if (bc != JBinvokedynamic) {
                    ...
                    entry.key = (void*)(((UDATA) * ((U_16*)&bytecodes[index + 1]))
                        + ((UDATA)J9ROMCLASS_CALLSITEDATA(class->romClass)));
                }
                if (NULL == (htEntry = hashTableFind(translation.ht, &entry))) {
                    JVMTI_ERROR(JVMTI_ERROR_INTERNAL);
                }
...
#define FLIP_32BIT(index)
            ...
            default:
                /* Any bytecode of size 3 or more which does not have a single word parm first must be added to the
                 * switch */
                if (bytecodeSize >= 3) {
                    bytecodes[index + 1] = j9Bytecodes[index + 2];
                    bytecodes[index + 2] = j9Bytecodes[index + 1];
...
jvmtiError JNICALL jvmtiIsMethodObsolete(jvmtiEnv* env, jmethodID method, jboolean* is_obsolete_ptr)
{
    ...
    rv_is_obsolete
        = (J9CLASS_FLAGS(J9_CLASS_FROM_METHOD(((J9JNIMethodID*)method)->method)) & J9_JAVA_CLASS_HOT_SWAPPED_OUT)
        ? JNI_TRUE
        : JNI_FALSE;
    rc = JVMTI_ERROR_NONE;
...
static jvmtiError setNativeMethodPrefixes(J9JVMTIEnv* j9env, jint prefix_count, char** prefixes)
{
