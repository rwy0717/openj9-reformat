...
static void charReplace(J9UTF8* utf8, char oldChar, char newChar)
{
    IDATA i = 0;
...
static jvmtiError addModuleExportsOrOpens(
    jvmtiEnv* jvmtiEnv, jobject fromModule, const char* pkgName, jobject toModule, BOOLEAN exports)
{
        ...
        if ((JVMTI_ERROR_NONE == rc) && (FALSE == isUnnamedOrOpen)) {
            ...
            if (J2SE_SHAPE(vm) >= J2SE_SHAPE_B148) {
                if (NULL == vm->addExports) {
                    ...
                    if (J2SE_SHAPE(vm) < J2SE_SHAPE_B165) {
                        addExports = (*env)->GetMethodID(env, moduleJClass, "implAddExportsOrOpens",
                            "(Ljava/lang/String;Ljava/lang/reflect/Module;ZZ)V");
                    } else {
                        addExports = (*env)->GetMethodID(
                            env, moduleJClass, "implAddExportsOrOpens", "(Ljava/lang/String;Ljava/lang/Module;ZZ)V");
                    }
                ...
                if (NULL == pkg) {
                    ...
                    (*env)->CallObjectMethod(env, fromModule, vm->addExports, pkg, toModule, !exports, JNI_TRUE);
                }
            } else {
                if (exports) {
                    if (NULL == vm->addExports) {
                        ...
                        addExports = (*env)->GetMethodID(
                            env, moduleJClass, "implAddExports", "(Ljava/lang/String;Ljava/lang/reflect/Module;Z)V");
                        if (NULL == addExports) {
                            rc = JVMTI_ERROR_INTERNAL;
                            goto done;
                        }
                    ...
                    if (NULL == pkg) {
                        ...
                        (*env)->CallObjectMethod(env, fromModule, vm->addExports, pkg, toModule, JNI_TRUE);
                    }
                } else {
                    if (NULL == vm->addOpens) {
                        jmethodID addOpens = NULL;

                        addOpens = (*env)->GetMethodID(
                            env, moduleJClass, "implAddOpens", "(Ljava/lang/String;Ljava/lang/reflect/Module;)V");
                        if (NULL == addOpens) {
                            rc = JVMTI_ERROR_OUT_OF_MEMORY;
                            goto done;
                        }
                    ...
                    if (NULL == pkg) {
                        ...
                        (*env)->CallObjectMethod(env, fromModule, vm->addExports, pkg, toModule);
                    }
                }
            }
 ...
 */
jvmtiError JNICALL jvmtiGetAllModules(jvmtiEnv* jvmtiEnv, jint* module_count_ptr, jobject** modules_ptr)
{
    J9JavaVM* vm = JAVAVM_FROM_ENV(jvmtiEnv);
    jvmtiError rc = JVMTI_ERROR_NONE;
    J9VMThread* currentThread = NULL;
        ...
        if (NULL == modules) {
            ...
            while (NULL != classLoader) {
                ...
                if (vm->systemClassLoader != classLoader) {
                    /* unnamedModule in the ClassLoader */
                    modules[i++] = (jobject)vmFuncs->j9jni_createLocalRef((JNIEnv*)currentThread,
                ...
                while (NULL != modulePtr) {
                    modules[i++]
                        = (jobject)vmFuncs->j9jni_createLocalRef((JNIEnv*)currentThread, (*modulePtr)->moduleObject);
 ...
 * @return jvmtiError, JVMTI_ERROR_NONE on success, non-zero on failure
 */
jvmtiError JNICALL jvmtiGetNamedModule(
    jvmtiEnv* jvmtiEnv, jobject class_loader, const char* package_name, jobject* module_ptr)
{
    J9JavaVM* vm = JAVAVM_FROM_ENV(jvmtiEnv);
    jvmtiError rc = JVMTI_ERROR_NONE;
    J9VMThread* currentThread = NULL;
    ...
    if (JVMTI_ERROR_NONE == rc) {
        ...
        *module_ptr = NULL;
        ...
        if (packageNameLength > JVMTI_PACKAGE_NAME_BUFFER_LENGTH) {
            packageName = j9mem_allocate_memory(
                packageNameLength + sizeof(packageName->length) + 1, J9MEM_CATEGORY_JVMTI_ALLOCATE);
            if (NULL == packageName) {
                rc = JVMTI_ERROR_OUT_OF_MEMORY;
                goto done;
            }
 ...
 */
jvmtiError JNICALL jvmtiAddModuleReads(jvmtiEnv* jvmtiEnv, jobject fromModule, jobject toModule)
{
    J9VMThread* currentThread = NULL;
    J9JavaVM* vm = JAVAVM_FROM_ENV(jvmtiEnv);
    jvmtiError rc = JVMTI_ERROR_NONE;
 ...
 * invalid and JVMTI_ERROR_NULL_POINTER if either module is NULL
 */
jvmtiError JNICALL jvmtiAddModuleExports(jvmtiEnv* jvmtiEnv, jobject fromModule, const char* pkgName, jobject toModule)
{
 ...
 * invalid and JVMTI_ERROR_NULL_POINTER if either module is NULL
 */
jvmtiError JNICALL jvmtiAddModuleOpens(jvmtiEnv* jvmtiEnv, jobject fromModule, const char* pkgName, jobject toModule)
{
 ...
 * @return jvmtiError, JVMTI_ERROR_NONE on success, non-zero on failure
 */
jvmtiError JNICALL jvmtiAddModuleUses(jvmtiEnv* jvmtiEnv, jobject module, jclass service)
{
    J9VMThread* currentThread = NULL;
    J9JavaVM* vm = JAVAVM_FROM_ENV(jvmtiEnv);
    jvmtiError rc = JVMTI_ERROR_NONE;
    ...
    if (JVMTI_ERROR_NONE == rc) {
        ...
        } else if (J9_IS_J9MODULE_UNNAMED(
                       vm, J9OBJECT_ADDRESS_LOAD(currentThread, moduleObject, vm->modulePointerOffset))) {
            isUnnamed = TRUE;
        }
 ...
 * @return jvmtiError, JVMTI_ERROR_NONE on success, non-zero on failure
 */
jvmtiError JNICALL jvmtiAddModuleProvides(jvmtiEnv* jvmtiEnv, jobject module, jclass service, jclass implClass)
{
    J9VMThread* currentThread = NULL;
    J9JavaVM* vm = JAVAVM_FROM_ENV(jvmtiEnv);
    jvmtiError rc = JVMTI_ERROR_NONE;
    ...
    if (JVMTI_ERROR_NONE == rc) {
        ...
        if ((rc == JVMTI_ERROR_NONE) && (FALSE == isUnnamed)) {
            ...
            if (NULL == vm->addProvides) {
                ...
                if (J2SE_SHAPE(vm) < J2SE_SHAPE_B165) {
                    addProvides = (*env)->GetStaticMethodID(env, jimModules, "addProvides",
                        "(Ljava/lang/reflect/Module;Ljava/lang/Class;Ljava/lang/Class;)V");
                } else {
                    addProvides = (*env)->GetStaticMethodID(
                        env, jimModules, "addProvides", "(Ljava/lang/Module;Ljava/lang/Class;Ljava/lang/Class;)V");
                }
                if (NULL == addProvides) {
                    rc = JVMTI_ERROR_INTERNAL;
                    goto done;
