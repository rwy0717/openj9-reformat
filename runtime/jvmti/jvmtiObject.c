...
jvmtiError JNICALL jvmtiGetObjectMonitorUsage(jvmtiEnv* env, jobject object, jvmtiMonitorUsage* info_ptr)
{
    ...
    TRACE_FOUR_JVMTI_RETURN(
        jvmtiGetObjectMonitorUsage2, rv_owner, rv_entry_count, rv_notify_waiter_count, rv_waiter_count);
}
...
static void findMonitorThreads(J9VMThread* vmThread, J9JVMTIMonitorStats* pStats)
{
    ...
    if (lockObject == pStats->lockObject) {
        ...
        if (target) {
            ...
            /* CMVC 87023 - the spec is unclear, but from experimentation it appears that 'waiting to be notified'
             * threads should appear in both lists
             */
