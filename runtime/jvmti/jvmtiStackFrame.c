...
static jvmtiError jvmtiInternalGetStackTrace(jvmtiEnv* env, J9VMThread* currentThread, J9VMThread* targetThread,
    ...
    if (rc == JVMTI_ERROR_NONE) {
        ...
        if (rc == JVMTI_ERROR_NONE) {
            ...
            rc = jvmtiInternalGetStackTrace(
                env, currentThread, targetThread, start_depth, (UDATA)max_frame_count, frame_buffer, &rv_count);
    ...
    jvmtiEnv* env, jint max_frame_count, jvmtiStackInfo** stack_info_ptr, jint* thread_count_ptr)
{
    ...
    if (rc == JVMTI_ERROR_NONE) {
        ...
        stackInfo = j9mem_allocate_memory(
            ((sizeof(jvmtiStackInfo) + (max_frame_count * sizeof(jvmtiFrameInfo))) * threadCount) + sizeof(jlocation),
            J9MEM_CATEGORY_JVMTI_ALLOCATE);
        if (stackInfo == NULL) {
            rc = JVMTI_ERROR_OUT_OF_MEMORY;
        } else {
            jvmtiFrameInfo* currentFrameInfo
                = (jvmtiFrameInfo*)((((UDATA)(stackInfo + threadCount)) + sizeof(jlocation)) & ~sizeof(jlocation));
            ...
            do {
                ...
                if (targetThread->threadObject == NULL) {
                    ...
                    rc = jvmtiInternalGetStackTrace(env, currentThread, targetThread, 0, (UDATA)max_frame_count,
                        (void*)currentFrameInfo, &(currentStackInfo->frame_count));
                    if (rc != JVMTI_ERROR_NONE) {
...
jvmtiError JNICALL jvmtiGetThreadListStackTraces(
    jvmtiEnv* env, jint thread_count, const jthread* thread_list, jint max_frame_count, jvmtiStackInfo** stack_info_ptr)
{
    J9JavaVM* vm = JAVAVM_FROM_ENV(env);
    jvmtiError rc;
    J9VMThread* currentThread;
    ...
    if (rc == JVMTI_ERROR_NONE) {
        ...
        stackInfo = j9mem_allocate_memory(
            ((sizeof(jvmtiStackInfo) + (max_frame_count * sizeof(jvmtiFrameInfo))) * thread_count) + sizeof(jlocation),
            J9MEM_CATEGORY_JVMTI_ALLOCATE);
        if (stackInfo == NULL) {
            rc = JVMTI_ERROR_OUT_OF_MEMORY;
        } else {
            jvmtiFrameInfo* currentFrameInfo
                = (jvmtiFrameInfo*)((((UDATA)(stackInfo + thread_count)) + sizeof(jlocation)) & ~sizeof(jlocation));
            jvmtiStackInfo* currentStackInfo = stackInfo;
            ...
            while (thread_count != 0) {
                ...
                if (!isSameOrSuperClassOf(
                        J9VMJAVALANGTHREAD_OR_NULL(vm), J9OBJECT_CLAZZ(currentThread, *((j9object_t*)thread)))) {
...
static UDATA popFrameCheckIterator(J9VMThread* currentThread, J9StackWalkState* walkState)
{
    ...
    if (1 == walkState->framesWalked) {
        ...
        if ((romMethod->modifiers & J9AccStatic) && (J9UTF8_LENGTH(methodName) > 0)
    ...
    jint start_depth, UDATA max_frame_count, jvmtiFrameInfo* frame_buffer, jint* count_ptr)
{
    ...
    walkState.flags = J9_STACKWALK_INCLUDE_NATIVES | J9_STACKWALK_VISIBLE_ONLY | J9_STACKWALK_RECORD_BYTECODE_PC_OFFSET
        | J9_STACKWALK_COUNT_SPECIFIED | J9_STACKWALK_ITERATE_FRAMES;
