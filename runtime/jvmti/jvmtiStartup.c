...
static jint createAgentLibrary(J9JavaVM* vm, const char* libraryAndOptions, UDATA libraryLength, const char* options,
    UDATA optionsLength, UDATA decorate, J9JVMTIAgentLibrary** result);
...
static jint issueAgentOnLoadAttach(
    J9JavaVM* vm, J9JVMTIAgentLibrary* agentLibrary, const char* options, char* loadFunctionName, BOOLEAN* foundLoadFn);
...
static const char* systemAgentNames[]
    = { "jdwp", "hprof", INSTRUMENT_LIBRARY, "hyinstrument", "healthcenter", "dgcollector", NULL };

jint JNICALL JVM_OnLoad(JavaVM* jvm, char* options, void* reserved) { return JNI_OK; }
 ...
 */
static void parseLibraryAndOptions(char* libraryAndOptions, UDATA* libraryLength, char** options, UDATA* optionsLength)
{
    char* optTemp;
 ...
 */
static char* prependSystemAgentPath(J9JavaVM* vm, J9JVMTIAgentLibrary* agentLibrary, char* dllName)
{
    PORT_ACCESS_FROM_JAVAVM(vm);
    ...
    if (vm->j2seRootDirectory) {
        ...
        if (J2SE_LAYOUT(vm) & J2SE_LAYOUT_VM_IN_SUBDIR) {
            ...
            bufferSize
                = strlen(vm->j2seRootDirectory) + 1 + strlen(DIR_SEPARATOR_STR) + strlen(dllName); /* +1 for NUL */
        }
        ...
        if (superSeparatorIndex
            != -1) { /* will be set if we need to clip right after parent dir - do NOT strcpy j2seRootDirectory as it
                        might be longer than the DLL name, resulting in a buffer overflow */
    ...
    *foundLoadFn = TRUE;
    ...
    if (J9_ARE_ALL_BITS_SET(vm->extendedRuntimeFlags, J9_EXTENDED_RUNTIME_RESTRICT_IFA)) {
        ...
        if (NULL == invocationJavaVM) {
            invocationJavaVM
                = (J9InvocationJavaVM*)j9mem_allocate_memory(sizeof(J9InvocationJavaVM), OMRMEM_CATEGORY_VM);
            if (NULL == invocationJavaVM) {
    ...
    if (JNI_OK != rc) {
         ...
         */
        j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_JVMTI_AGENT_INITIALIZATION_FAILED, loadFunctionName,
            agentLibrary->nativeLib.name, rc);
 ...
 */
static jint loadAgentLibraryGeneric(J9JavaVM* vm, J9JVMTIAgentLibrary* agentLibrary, char* loadFunctionName,
    BOOLEAN loadStatically, BOOLEAN* found, const char** errorMessage)
{
    PORT_ACCESS_FROM_JAVAVM(vm);
    jint rc;
    ...
    if (NULL == agentLibrary->xRunLibrary) {
        ...
        Trc_JVMTI_loadAgentLibraryGeneric_openedAgentLibrary(
            agentPath, nativeLib->handle, loadStatically ? "[statically]" : "[dynamically]");
        if (NULL != fullLibName) {
            j9mem_free_memory(fullLibName);
        }
    } else {
...
I_32 JNICALL loadAgentLibraryOnAttach(struct J9JavaVM* vm, const char* library, const char* options, UDATA decorate)
{
    ...
    if (NULL != agentLibrary) {
        ...
        if (J9NATIVELIB_LINK_MODE_STATIC == agentLibrary->nativeLib.linkMode) {
            loadFunctionNameLength = j9str_printf(PORTLIB, loadFunctionName, (J9JVMTI_BUFFER_LENGTH + 1), "%s_%s",
                J9JVMTI_AGENT_ONATTACH, agentLibrary->nativeLib.name);
        ...
        rc = loadAgentLibraryGeneric(vm, agentLibrary, loadFunctionName, TRUE, /* link statically. */
            &found, &errorMessage);
        if (found) {
             ...
             * try dynamic linking.
             */
            rc = loadAgentLibraryGeneric(vm, agentLibrary, J9JVMTI_AGENT_ONATTACH, FALSE, /* link dynamically. */
                &found, &errorMessage);
            if (found) {
                Trc_JVMTI_loadAgentLibraryOnAttach_attachingAgentDynamically(agentLibrary->nativeLib.name);
                omrthread_monitor_enter(jvmtiData->mutex);
...
static jint loadAgentLibrary(J9JavaVM* vm, J9JVMTIAgentLibrary* agentLibrary)
{
    ...
    nameBufferLengh = j9str_printf(
        PORTLIB, nameBuffer, (J9JVMTI_BUFFER_LENGTH + 1), "%s_%s", J9JVMTI_AGENT_ONLOAD, agentLibrary->nativeLib.name);
    ...
    if (!found) {
        ...
        if (!found) {
            if (NULL != errorMessage) {
                j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_JVMTI_AGENT_LIBRARY_OPEN_FAILED, agentLibrary->nativeLib.name,
                    errorMessage);
            } else {
                j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_JVMTI_AGENT_INITIALIZATION_FUNCTION_NOT_FOUND,
                    J9JVMTI_AGENT_ONLOAD, agentLibrary->nativeLib.name);
            }
        }
    }
exit:
    ...
    if (jvmtiData->agentLibraries != NULL) {
        ...
        while (agentLibrary) {
            if (agentLibrary->nativeLib.handle != 0) {
                 ...
                 */
                if (J9NATIVELIB_LINK_MODE_STATIC == agentLibrary->nativeLib.linkMode) {
                    j9str_printf(PORTLIB, nameBuffer, sizeof(nameBuffer), "%s_%s", J9JVMTI_AGENT_ONUNLOAD,
...
static jint initializeJVMTI(J9JavaVM* vm)
{
    ...
    jvmtiData->agentLibraries = pool_new(sizeof(J9JVMTIAgentLibrary), 0, 0, POOL_ALWAYS_KEEP_SORTED, J9_GET_CALLSITE(),
        J9MEM_CATEGORY_JVMTI, POOL_FOR_PORT(vm->portLibrary));
    if (jvmtiData->agentLibraries == NULL) {
        return JNI_ENOMEM;
    }

    jvmtiData->environments = pool_new(sizeof(J9JVMTIEnv), 0, 0, POOL_ALWAYS_KEEP_SORTED, J9_GET_CALLSITE(),
        J9MEM_CATEGORY_JVMTI, POOL_FOR_PORT(vm->portLibrary));
    if (jvmtiData->environments == NULL) {
        return JNI_ENOMEM;
    }

    jvmtiData->breakpoints = pool_new(sizeof(J9JVMTIGlobalBreakpoint), 0, 0, POOL_ALWAYS_KEEP_SORTED, J9_GET_CALLSITE(),
        J9MEM_CATEGORY_JVMTI, POOL_FOR_PORT(vm->portLibrary));
    if (jvmtiData->breakpoints == NULL) {
        return JNI_ENOMEM;
    }

    jvmtiData->breakpointedMethods = pool_new(sizeof(J9JVMTIBreakpointedMethod), 0, 0, POOL_ALWAYS_KEEP_SORTED,
...
static void shutDownJVMTI(J9JavaVM* vm)
{
    ...
    if (jvmtiData != NULL) {
        ...
        /* Dispose any remaining environments, completely freeing their data - assume we are single-threaded at this
         * point */
