...
static void ownedMonitorIterator(
...
jvmtiError JNICALL jvmtiGetThreadState(jvmtiEnv* env, jthread thread, jint* thread_state_ptr)
{
    ...
    if (rc == JVMTI_ERROR_NONE) {
        ...
        if (rc == JVMTI_ERROR_NONE) {
             ...
             * thread so that get a consistent view of the values.  This is needed because if we don't get a consistent
             * view we may think the thread is TERMINATED instead of just starting.
             */
            ...
            do {
                ...
                if (threadObject != NULL) {
                    if (J9VMJAVALANGTHREAD_STARTED(currentThread, threadObject)
                        ...
                        *currentThreadPtr++ = (jthread)vm->internalVMFunctions->j9jni_createLocalRef(
                            (JNIEnv*)currentThread, (j9object_t)threadObject);
                        ++threadCount;
                    }
                }
    ...
    jvmtiEnv* env, jint request_count, const jthread* request_list, jvmtiError* results)
{
    ...
    if (rc == JVMTI_ERROR_NONE) {
        ...
        /* If the current thread appeared in the list (and was marked as suspended), block now until the thread is
         * resumed */
        ...
        if (rc == JVMTI_ERROR_NONE) {
            ...
            if (NULL == targetThread) {
                ...
                if (NULL == threadName) {
                    ...
                    name = vm->internalVMFunctions->copyStringToUTF8WithMemAlloc(
                        currentThread, threadName, J9_STR_NULL_TERMINATE_RESULT, "", 0, NULL, 0, NULL);
                ...
                if (JAVAVM_FROM_ENV(env)->jclFlags & J9_JCL_FLAG_THREADGROUPS) {
                    threadGroup = vm->internalVMFunctions->j9jni_createLocalRef(
                        (JNIEnv*)currentThread, (j9object_t)J9VMJAVALANGTHREAD_GROUP(currentThread, threadObject));
                }
            }

            contextClassLoader = vm->internalVMFunctions->j9jni_createLocalRef(
                (JNIEnv*)currentThread, J9VMJAVALANGTHREAD_CONTEXTCLASSLOADER(currentThread, threadObject));
    ...
    jvmtiEnv* env, jthread thread, jint* monitor_info_count_ptr, jvmtiMonitorStackDepthInfo** monitor_info_ptr)
{
    ...
    if (rc == JVMTI_ERROR_NONE) {
        ...
        if (rc == JVMTI_ERROR_NONE) {
            ...
            monitorEnterRecords = (J9ObjectMonitorInfo*)j9mem_allocate_memory(
                sizeof(J9ObjectMonitorInfo) * (UDATA)maxRecords, J9MEM_CATEGORY_JVMTI);
            if (NULL == monitorEnterRecords) {
                rc = JVMTI_ERROR_OUT_OF_MEMORY;
                goto doneRelease;
            }

            maxRecords = vm->internalVMFunctions->getOwnedObjectMonitors(
                currentThread, targetThread, monitorEnterRecords, maxRecords);
            if (maxRecords < 0) {
                rc = JVMTI_ERROR_INTERNAL;
                goto doneRelease;
            }
            ...
            if (maxRecords > 0) {
                resultArray = j9mem_allocate_memory(
                    (jlong)maxRecords * sizeof(jvmtiMonitorStackDepthInfo), J9MEM_CATEGORY_JVMTI_ALLOCATE);
                if (resultArray == NULL) {
                    maxRecords = 0;
                    resultArray = NULL;
                    rc = JVMTI_ERROR_OUT_OF_MEMORY;
                    ...
                    for (i = 0; i < maxRecords; i++) {
                        ...
                        resultArray[i].monitor = currentThread->javaVM->internalVMFunctions->j9jni_createLocalRef(
...
static int J9THREAD_PROC agentThreadStart(void* entryArg)
{
    ...
    j9sig_protect(wrappedAgentThreadStart, args, vmThread->javaVM->internalVMFunctions->structuredSignalHandler,
        vmThread, J9PORT_SIG_FLAG_SIGALLSYNC | J9PORT_SIG_FLAG_MAY_CONTINUE_EXECUTION, &result);
...
static void ownedMonitorIterator(
    J9VMThread* targetThread, J9StackWalkState* walkState, j9object_t* slot, const void* stackLocation)
{
...
jvmtiError JNICALL jvmtiGetCurrentThread(jvmtiEnv* env, jthread* thread_ptr)
{
    ...
    if (rc == JVMTI_ERROR_NONE) {
        ...
        rv_thread = (jthread)vm->internalVMFunctions->j9jni_createLocalRef(
            (JNIEnv*)currentThread, (j9object_t)currentThread->threadObject);
    done:
        vm->internalVMFunctions->internalExitVMToJNI(currentThread);
    }
