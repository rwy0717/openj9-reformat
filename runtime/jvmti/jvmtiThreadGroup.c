    ...
    if (JAVAVM_FROM_ENV(env)->jclFlags & J9_JCL_FLAG_THREADGROUPS) {
        ...
        if (rc == JVMTI_ERROR_NONE) {
            ...
            if (groups == NULL) {
                ...
                *groups = (jthreadGroup)vm->internalVMFunctions->j9jni_createLocalRef(
                    (JNIEnv*)currentThread, *(vm->systemThreadGroupRef));
                rv_group_count = 1;
                rv_groups = groups;
            }
        done:
    ...
    if (JAVAVM_FROM_ENV(env)->jclFlags & J9_JCL_FLAG_THREADGROUPS) {
        ...
        if (rc == JVMTI_ERROR_NONE) {
            ...
            name = vm->internalVMFunctions->copyStringToUTF8WithMemAlloc(
                currentThread, groupName, J9_STR_NULL_TERMINATE_RESULT, "", 0, NULL, 0, NULL);

            if (NULL == name) {
                rc = JVMTI_ERROR_OUT_OF_MEMORY;
            } else {
                rv_name = name;
                rv_parent = (jthreadGroup)vm->internalVMFunctions->j9jni_createLocalRef((JNIEnv*)currentThread,
                    (j9object_t)J9VMJAVALANGTHREADGROUP_PARENT(currentThread, threadGroupObject));
    ...
    jthread** threads_ptr, jint* group_count_ptr, jthreadGroup** groups_ptr)
{
    ...
    if (JAVAVM_FROM_ENV(env)->jclFlags & J9_JCL_FLAG_THREADGROUPS) {
        ...
        if (rc == JVMTI_ERROR_NONE) {
            ...
            childrenGroupsLock = (j9object_t)currentThread->javaVM->internalVMFunctions->objectMonitorEnter(
                currentThread, childrenGroupsLock);
            ...
            if (groups == NULL) {
                ...
                jint i;
                j9object_t childrenGroups
                    = (j9object_t)J9VMJAVALANGTHREADGROUP_CHILDRENGROUPS(currentThread, threadGroupObject);

                for (i = 0; i < numGroups; ++i) {
                    j9object_t group = J9JAVAARRAYOFOBJECT_LOAD(currentThread, childrenGroups, i);

                    groups[i]
                        = (jthreadGroup)vm->internalVMFunctions->j9jni_createLocalRef((JNIEnv*)currentThread, group);
                }
            }
            ...
            childrenThreadsLock = (j9object_t)currentThread->javaVM->internalVMFunctions->objectMonitorEnter(
                currentThread, childrenThreadsLock);
            ...
            if (threads == NULL) {
                ...
                jint i;
                j9object_t childrenThreads
                    = (j9object_t)J9VMJAVALANGTHREADGROUP_CHILDRENTHREADS(currentThread, threadGroupObject);
                jint numLiveThreads;
                ...
                for (i = 0; i < numThreads; ++i) {
                    ...
                    if (getVMThread(currentThread, (jthread)&thread, &targetThread, FALSE, TRUE) == JVMTI_ERROR_NONE) {
                        threads[numLiveThreads++]
                            = (jthread)vm->internalVMFunctions->j9jni_createLocalRef((JNIEnv*)currentThread, thread);
