...
#define check_segment_merge(S, b, s) (mmap_exec_offset((b), (s)) == (S)->exec_offset)
...
#define set_segment_flags(S, v)                                                                                    \
    (((v) != IS_MMAPPED_BIT) ? (ABORT, (v))                                                                        \
                             : (((S)->exec_offset = mmap_exec_offset((S)->base, (S)->size)),                       \
                                   (mmap_exec_offset((S)->base + (S)->exec_offset, (S)->size) != (S)->exec_offset) \
                                       ? (ABORT, (v))                                                              \
    ...
    if (tbase != CMFAIL) {
        ...
        if (!is_initialized(m)) { /* first-time initialization */
            ...
            while (sp != 0 && tbase != sp->base + sp->size)
                sp = sp->next;
            if (sp != 0 && !is_extern_segment(sp) && check_segment_merge(sp, tbase, tsize)
                && (get_segment_flags(sp) & IS_MMAPPED_BIT) == mmap_flag && segment_holds(sp, m->top)) { /* append */
                sp->size += tsize;
