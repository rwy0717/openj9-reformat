...
#endif
        (((ecif->cif->rtype->type == FFI_TYPE_STRUCT) && !ecif->cif->flags)))
...
ffi_status ffi_prep_cif_machdep(ffi_cif* cif)
{
    ...
    case FFI_TYPE_STRUCT:
        if (cif->rtype->elements[0]->type == FFI_TYPE_STRUCT && cif->rtype->elements[1]) {
            cif->flags = 0;
            break;
        }
...
void ffi_call(ffi_cif* cif, void (*fn)(), void* rvalue, void** avalue)
{
    ...
    if (rvalue == NULL && cif->rtype->type == FFI_TYPE_STRUCT && cif->rtype->size > 8)
...
static void ffi_prep_incoming_args_SYSV(char* stack, void** avalue, ffi_cif* cif)
{
    ...
    for (i = cif->nargs, p_arg = cif->arg_types; (i != 0); i--, p_arg++) {
        ...
        z = (*p_arg)->size;
#ifdef __MINT__
        if (cif->flags && cif->rtype->type == FFI_TYPE_STRUCT && (z == 1 || z == 2)) {
            *p_argv = (void*)(argp + 2);

            z = 4;
        } else if (cif->flags && cif->rtype->type == FFI_TYPE_STRUCT && (z == 3 || z == 4)) {
            *p_argv = (void*)(argp);
...
#endif
        (((cif->rtype->type == FFI_TYPE_STRUCT) && !cif->flags)))
...
#else
    syscall(SYS_cacheflush, codeloc, FLUSH_SCOPE_LINE, FLUSH_CACHE_BOTH, FFI_TRAMPOLINE_SIZE);
#endif
