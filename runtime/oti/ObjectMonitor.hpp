...
public:
     ...
     */
    static VMINLINE j9objectmonitor_t* inlineGetLockAddress(J9VMThread* currentThread, j9object_t object)
    {
     ...
     */
    static VMINLINE bool getMonitorForWait(
        J9VMThread* currentThread, j9object_t object, omrthread_monitor_t* monitorPtr, bool checkOwner = true)
    {
        bool wait = false;
        ...
        if (J9_LOCK_IS_INFLATED(lock)) {
            ...
            if (checkOwner) {
                ...
                if (OBJECT_HEADER_LOCK_RESERVED
                    == (lock & (OBJECT_HEADER_LOCK_RECURSION_MASK | OBJECT_HEADER_LOCK_RESERVED))) {
                    goto done;
                }
#endif
            }
     ...
     * 					if monitor is not NULL, another thread owned this monitor
     */
    static VMINLINE bool getMonitorForNotify(
        J9VMThread* currentThread, j9object_t object, omrthread_monitor_t* monitorPtr, bool checkOwner = true)
    {
        bool notify = false;
        omrthread_monitor_t monitor = (omrthread_monitor_t)(UDATA)1; // invalid but non-NULL value
        ...
        if ((lock & ~(j9objectmonitor_t)OBJECT_HEADER_LOCK_BITS_MASK) == (j9objectmonitor_t)(UDATA)currentThread) {
            ...
            J9ObjectMonitor* objectMonitor
                = (J9ObjectMonitor*)(UDATA)(lock & ~(j9objectmonitor_t)OBJECT_HEADER_LOCK_INFLATED);
            monitor = objectMonitor->monitor;
            notify = true;
        }
    done:
     ...
     */
    static VMINLINE j9objectmonitor_t compareAndSwapLockword(j9objectmonitor_t volatile* lockEA,
        j9objectmonitor_t oldValue, j9objectmonitor_t newValue, bool readBeforeCAS = false)
    {
#if defined(J9VM_INTERP_SMALL_MONITOR_SLOT)
        j9objectmonitor_t contents
            = VM_AtomicSupport::lockCompareExchangeU32(lockEA, oldValue, newValue, readBeforeCAS);
     ...
     */
    static VMINLINE bool inlineFastInitAndEnterMonitor(J9VMThread* currentThread, j9objectmonitor_t volatile* lockEA,
        bool readBeforeCAS = false, j9objectmonitor_t lock = 0)
    {
        bool locked = false;
     ...
     * @returns	true if the lock was acquired, false if not
     */
    static VMINLINE bool inlineFastObjectMonitorEnter(J9VMThread* currentThread, j9object_t object)
    {
        bool locked = false;
     ...
     */
    static VMINLINE bool inlineFastObjectMonitorExit(J9VMThread* currentThread, j9object_t object)
    {
        bool unlocked = false;
#if defined(J9VM_THR_LOCK_NURSERY)
        if (LN_HAS_LOCKWORD(currentThread, object))
     ...
     */
    static VMINLINE IDATA enterObjectMonitor(J9VMThread* currentThread, j9object_t object)
    {
     ...
     */
    static VMINLINE IDATA exitObjectMonitor(J9VMThread* currentThread, j9object_t object)
    {
        IDATA rc = 0;
     ...
     */
    static VMINLINE bool recordMonitorEnter(
        J9VMThread* currentThread, j9object_t object, UDATA* framePointer, J9MonitorEnterRecord** recordList)
    {
        bool success = true;
     ...
     * @returns	true on success, false if out of memory
     */
    static VMINLINE bool recordBytecodeMonitorEnter(J9VMThread* currentThread, j9object_t object, UDATA* arg0EA)
    {
        return recordMonitorEnter(currentThread, object, CONVERT_TO_RELATIVE_STACK_OFFSET(currentThread, arg0EA),
            &currentThread->monitorEnterRecords);
    }
     ...
     * @returns	true on success, false if out of memory
     */
    static VMINLINE bool recordJNIMonitorEnter(J9VMThread* currentThread, j9object_t object)
    {
     ...
     * @param recordList[in/out] pointer to the head of the monitor enter record list to update
     */
    static VMINLINE void recordMonitorExit(
        J9VMThread* currentThread, j9object_t object, J9MonitorEnterRecord** recordList)
    {
        J9Pool* pool = currentThread->monitorEnterRecordPool;
        for (;;) {
            J9MonitorEnterRecord* enterRecord = *recordList;
     ...
     * @param object[in] the object whose monitor was just exitted
     */
    static VMINLINE void recordBytecodeMonitorExit(J9VMThread* currentThread, j9object_t object)
    {
     ...
     * @param object[in] the object whose monitor was just exitted
     */
    static VMINLINE void recordJNIMonitorExit(J9VMThread* currentThread, j9object_t object)
    {
