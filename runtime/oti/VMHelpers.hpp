...
typedef enum { ffiSuccess, ffiFailed, ffiOOM } FFI_Return;
...
public:
     ...
     */
    static VMINLINE UDATA calculateStackUse(J9ROMMethod* romMethod, UDATA stackFrameSize = sizeof(J9SFStackFrame))
    {
     ...
     */
    static VMINLINE UDATA getClassDepth(J9Class* clazz) { return J9CLASS_DEPTH(clazz); }
     ...
     */
    static VMINLINE J9Class* getSuperclass(J9Class* clazz) { return clazz->superclasses[getClassDepth(clazz) - 1]; }
     ...
     */
    static VMINLINE bool vmConstantPoolFieldIsResolved(J9JavaVM* vm, UDATA cpIndex)
    {
     ...
     */
    static VMINLINE void* methodHandleCompiledEntryPoint(
        J9JavaVM* vm, J9VMThread* currentThread, j9object_t methodHandle)
    {
        void* compiledEntryPoint = NULL;
        if (J9_EXTENDED_RUNTIME_I2J_MH_TRANSITION_ENABLED
     ...
     */
    static VMINLINE bool exceptionPending(J9VMThread* const currentThread)
    {
     ...
     */
    static VMINLINE bool classCanBeInstantiated(J9Class* j9clazz)
    {
        J9ROMClass* romClass = j9clazz->romClass;
        return !J9ROMCLASS_IS_ABSTRACT(romClass) && !J9ROMCLASS_IS_INTERFACE(romClass)
            && !J9ROMCLASS_IS_PRIMITIVE_TYPE(romClass) && !J9ROMCLASS_IS_ARRAY(romClass);
    }
     ...
     */
    static VMINLINE bool classRequiresInitialization(J9VMThread* currentThread, J9Class* j9clazz)
    {
     ...
     */
    static VMINLINE bool mustCallWriteAccessBarrier(J9JavaVM* vm)
    {
     ...
     */
    static VMINLINE bool classIsBootstrap(J9JavaVM* vm, J9Class* j9clazz)
    {
     ...
     */
    static VMINLINE J9Class* currentClass(J9Class* j9clazz)
    {
     ...
     */
    static VMINLINE void pushObjectInSpecialFrame(J9VMThread* currentThread, j9object_t object)
    {
     ...
     */
    static VMINLINE j9object_t popObjectInSpecialFrame(J9VMThread* currentThread)
    {
     ...
     * @param currentThread[in] the current J9VMThread
     */
    static VMINLINE void checkIfFinalizeObject(J9VMThread* currentThread, j9object_t object)
    {
     ...
     */
    static VMINLINE bool objectIsArray(J9VMThread* currentThread, j9object_t object)
    {
        J9Class* objectClass = J9OBJECT_CLAZZ(currentThread, object);
     ...
     */
    static VMINLINE bool inlineCheckCast(J9Class* instanceClass, J9Class* castClass, bool updateCache = true)
    {
        ...
        if (!isSameOrSuperclass(castClass, instanceClass)) {
            ...
            if (J9ROMCLASS_IS_INTERFACE(castClass->romClass)) {
                ...
                if (J9CLASS_IS_ARRAY(instanceClass)) {
                    ...
                    /* else we have a base type castClass and it isn't equal (from the trivial case at the top), so it
                     * can't be valid so we fail */
                }
            }
     ...
     */
    static VMINLINE void setExceptionPending(J9VMThread* currentThread, j9object_t exception)
    {
     ...
     * @param currentThread[in] the current J9VMThread
     */
    static VMINLINE void clearException(J9VMThread* currentThread)
    {
     ...
     */
    static VMINLINE bool shouldGrowForSP(J9VMThread* currentThread, UDATA* checkSP)
    {
     ...
     */
    static VMINLINE bool asyncMessagePending(J9VMThread* currentThread)
    {
     ...
     */
    static VMINLINE void indicateAsyncMessagePending(J9VMThread* vmThread)
    {
     ...
     */
    static VMINLINE UDATA checkAsyncMessages(J9VMThread* currentThread)
    {
     ...
     */
    static VMINLINE bool immediateAsyncPending(J9VMThread* currentThread)
    {
     ...
     */
    static VMINLINE j9object_t allocateObject(J9VMThread* currentThread, MM_ObjectAllocationAPI* objectAllocate,
        J9Class* clazz, bool initializeSlots = true, bool memoryBarrier = true)
    {
        j9object_t instance
            = objectAllocate->inlineAllocateObject(currentThread, clazz, initializeSlots, memoryBarrier);
        if (NULL == instance) {
            instance = currentThread->javaVM->memoryManagerFunctions->J9AllocateObject(
                currentThread, clazz, J9_GC_ALLOCATE_OBJECT_NON_INSTRUMENTABLE);
     ...
     * @returns the new object, or NULL if allocation failed
     */
    static VMINLINE j9object_t allocateIndexableObject(J9VMThread* currentThread,
        MM_ObjectAllocationAPI* objectAllocate, J9Class* arrayClass, U_32 size, bool initializeSlots = true,
        bool memoryBarrier = true, bool sizeCheck = true)
    {
        j9object_t instance = objectAllocate->inlineAllocateIndexableObject(
            currentThread, arrayClass, size, initializeSlots, memoryBarrier, sizeCheck);
        if (NULL == instance) {
            instance = currentThread->javaVM->memoryManagerFunctions->J9AllocateIndexableObject(
     ...
     */
    static VMINLINE bool methodHasBeenCompiled(J9Method* method)
    {
     ...
     */
    static VMINLINE UDATA encodedUTF8Length(U_16 unicode)
    {
        UDATA length = 1;
     ...
     * @returns the number of bytes required to encode the character as UTF8
     */
    static VMINLINE UDATA encodedUTF8LengthI8(I_8 unicode) { return (unicode >= 0x01) && (unicode <= 0x7F) ? 1 : 2; }
     ...
     */
    static VMINLINE UDATA encodeUTF8Char(U_16 unicode, U_8* utfChars)
    {
        UDATA length = 1;
        if ((unicode >= 0x01) && (unicode <= 0x7F)) {
     ...
     * @return Size of encoding (1,2,3) on success, 0 on failure
     */
    static VMINLINE UDATA encodeUTF8CharI8(I_8 unicode, U_8* utfChars)
    {
        UDATA length = 1;
        if ((unicode >= 0x01) && (unicode <= 0x7F)) {
            utfChars[0] = (U_8)unicode;
     ...
     */
    static VMINLINE UDATA encodeUTF8CharN(U_16 unicode, U_8* utfChars, UDATA bytesRemaining)
    {
        if (unicode >= 0x01 && unicode <= 0x7f) {
            if (bytesRemaining < 1) {
                return 0;
     ...
     */
    static VMINLINE UDATA decodeUTF8CharN(const U_8* input, U_16* result, UDATA bytesRemaining)
    {
        U_8 c = input[0];
        U_16 unicode = (U_16)c;
        UDATA consumed = 0;
     ...
     */
    static VMINLINE UDATA decodeUTF8Char(const U_8* input, U_16* result)
    {
        U_8 c = input[0];
        U_16 unicode = (U_16)c;
        UDATA consumed = 1;
     ...
     */
    static VMINLINE void* jitToJitStartAddress(void* jitStartAddress)
    {
     ...
     */
    static VMINLINE bool methodBeingTraced(J9JavaVM* vm, J9Method* method)
    {
        bool rc = false;
     ...
     * @returns true if resolved, false if not
     */
    static VMINLINE bool instanceFieldRefIsResolved(UDATA flags, UDATA valueOffset)
    {
     ...
     * @returns true if resolved, false if not
     */
    static VMINLINE bool staticFieldRefIsResolved(IDATA flagsAndClass, UDATA valueOffset)
    {
     ...
     */
    static VMINLINE void* staticFieldAddressFromResolvedRef(IDATA flagsAndClass, UDATA staticAddress)
    {
     ...
     */
    static VMINLINE void beforeJNICall(J9VMThread* currentThread)
    {
        ...
        if (NULL != vm->javaOffloadSwitchOffWithMethodFunc) {
            ...
            J9SFJNINativeMethodFrame* nativeMethodFrame
                = (J9SFJNINativeMethodFrame*)((UDATA)currentThread->sp + (UDATA)currentThread->literals);
     ...
     * @param[in] currentThread the current J9VMThread
     */
    static VMINLINE void afterJNICall(J9VMThread* currentThread)
    {
#if defined(J9VM_OPT_JAVA_OFFLOAD_SUPPORT)
        J9JavaVM* vm = currentThread->javaVM;
        J9VMEntryLocalStorage* els = currentThread->entryLocalStorage;
    ...
    static VMINLINE bool hasDefaultConflictSendTarget(J9Method* method)
    {
    ...
    static VMINLINE bool threadIsInterruptedImpl(J9VMThread* currentThread, j9object_t threadObject)
    {
        bool result = false;
    ...
    static VMINLINE jobject createLocalRef(JNIEnv* env, j9object_t object)
    {
        j9object_t* ref = NULL;
        ...
        if (object != NULL) {
            ...
            J9SFJNINativeMethodFrame* frame
                = (J9SFJNINativeMethodFrame*)((U_8*)vmThread->sp + (UDATA)vmThread->literals);
            if ((0 == (frame->specialFrameFlags & J9_SSF_CALL_OUT_FRAME_ALLOC))
                && ((UDATA)vmThread->literals < (J9_SSF_CO_REF_SLOT_CNT * sizeof(UDATA)))) {
    ...
    static VMINLINE U_32 lookupVarHandleMethodTypeCacheIndex(J9ROMClass* romClass, UDATA cpIndex)
    {
     ...
     */
    static bool doMethodTypesMatchIgnoringLastArgument(
        J9VMThread* currentThread, j9object_t shorterMT, j9object_t longerMT)
    {
        bool matched = false;
        ...
        if (shorterMTReturnType == longerMTReturnType) {
            ...
            if ((longerMTArgumentsLength - 1) == (I_32)J9INDEXABLEOBJECT_SIZE(currentThread, shorterMTArguments)) {
                for (I_32 i = 0; i < longerMTArgumentsLength - 1; i++) {
                    if (J9JAVAARRAYOFOBJECT_LOAD(currentThread, shorterMTArguments, i)
                        != J9JAVAARRAYOFOBJECT_LOAD(currentThread, longerMTArguments, i)) {
                        goto exit;
                    }
                }
                matched = true;
     ...
     */
    static VMINLINE bool isReflectionMethod(J9VMThread* currentThread, J9Method* method)
    {
        J9Class* currentClass = J9_CLASS_FROM_METHOD(method);
        J9JavaVM* vm = currentThread->javaVM;
        return ((method == vm->jlrMethodInvoke) || (method == vm->jliMethodHandleInvokeWithArgs)
     ...
     */
    static VMINLINE bool isHiddenMethod(J9Method* method)
    {
        bool isHidden = false;
        J9Class* currentClass = J9_CLASS_FROM_METHOD(method);
     ...
     */
    static VMINLINE void convertJNIReturnValue(U_8 returnType, UDATA* returnStorage)
    {
        switch (returnType) {
        case J9NtcObject:
    ...
    /**
     * @brief Notify JIT upon the first modification of a final field, a stack frame should be build before calling this
     * method to make the stack walkable
     * @param currentThread
     * @param fieldClass The declaring class of the final field
     */
    static VMINLINE void reportFinalFieldModified(J9VMThread* currentThread, J9Class* fieldClass)
    {
     ...
     */
    static VMINLINE J9SFJNINativeMethodFrame* findNativeMethodFrame(J9VMThread* currentThread)
    {
     ...
     */
    static VMINLINE bool romMethodIsInitializer(J9ROMMethod* romMethod, bool isStatic)
    {
     ...
     */
    static VMINLINE bool romClassChecksFinalStores(J9ROMClass* romClass)
    {
