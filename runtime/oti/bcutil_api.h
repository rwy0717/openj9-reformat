 ...
 */
J9TranslationBufferSet* j9bcutil_allocTranslationBuffers(J9PortLibrary* portLib);
...
j9bcutil_buildRomClassIntoBuffer(U_8* classFileBytes, UDATA classFileSize, J9PortLibrary* portLib,
    ...
    U_8* romSegment, UDATA romSegmentSize, U_8* lineNumberBuffer, UDATA lineNumberBufferSize, U_8* varInfoBuffer,
 ...
 */
IDATA
j9bcutil_buildRomClass(J9LoadROMClassData* loadData, U_8* intermediateData, UDATA intermediateDataLength,
    J9JavaVM* javaVM, UDATA bctFlags, UDATA classFileBytesReplaced, UDATA isIntermediateROMClass,
    J9TranslationLocalBuffer* localBuffer);

void shutdownROMClassBuilder(J9JavaVM* vm);

#if defined(J9DYN_TEST)
IDATA
j9bcutil_compareRomClass(U_8* classFileBytes, U_32 classFileSize, J9PortLibrary* portLib,
 ...
 * @return I_32
 */
I_32 j9bcutil_verifyCanonisizeAndCopyUTF8(U_8* dest, U_8* source, U_32 length);
 ...
 * @param [in/out] localBuffer contains values for entryInfex, loadLocationType and cpEntryUsed. This pointer can't be
 * NULL.
 *
 * @return zero on success, -1 on failure.
 */
IDATA
findLocallyDefinedClass(J9VMThread* vmThread, J9Module* j9module, U_8* className, U_32 classNameLength,
    J9ClassLoader* classLoader, J9ClassPathEntry* classPath, UDATA classPathEntryCount, UDATA options,
    J9TranslationLocalBuffer* localBuffer);
 ...
 */
I_32 initJImageIntf(J9JImageIntf** jimageIntf, J9JavaVM* vm, J9PortLibrary* portLibrary);
 ...
 * @return J9JIMAGE_NO_ERROR on success, negative error code on failure
 */
I_32 initJImageIntfWithLibrary(J9JImageIntf** jimageIntf, J9PortLibrary* portLibrary, const char* libjimage);
 ...
 * @return void
 */
void closeJImageIntf(J9JImageIntf* jimageIntf);
 ...
 * @return J9JIMAGE_NO_ERROR on success; negative error code on failure
 */
I_32 jimageOpen(J9JImageIntf* jimageIntf, const char* name, UDATA* handle);
 ...
 * @return void
 */
void jimageClose(J9JImageIntf* jimageIntf, UDATA handle);
 ...
 * @return J9JIMAGE_NO_ERROR on success; negative error code on failure
 */
I_32 jimageFindResource(J9JImageIntf* jimageIntf, UDATA handle, const char* moduleName, const char* name,
    UDATA* resourceLocation, I_64* size);
 ...
 */
void jimageFreeResourceLocation(J9JImageIntf* jimageIntf, UDATA handle, UDATA resourceLocation);
 ...
 * 		   J9JIMAGE_RESOURCE_TRUNCATED if the buffer size is not large enough to store the whole resource in the
 * buffer; negative error code in other cases
 */
I_32 jimageGetResource(
    J9JImageIntf* jimageIntf, UDATA handle, UDATA resourceLocation, char* buffer, I_64 bufferSize, I_64* resourceSize);
 ...
 */
const char* jimagePackageToModule(J9JImageIntf* jimageIntf, UDATA handle, const char* packageName);
 ...
 */
I_32 j9bcutil_loadJImage(J9PortLibrary* portlib, const char* fileName, J9JImage** pjimage);
 ...
 * @return void
 */
void j9bcutil_dumpJImageInfo(J9PortLibrary* portlib, J9JImage* jimage);
 ...
 * @param [out] j9jimageLocation pointer to J9JImageLocation; if not NULL then on success it is filled with metadata
 ...
 */
I_32 j9bcutil_lookupJImageResource(
    J9PortLibrary* portlib, J9JImage* jimage, J9JImageLocation* j9jimageLocation, const char* resourceName);
 ...
 *
 * Verification is done by comparing the strings corresponding to BASE, PARENT and EXTENSION attribute types with the
 * resource name.
 *
 * @param [in] portlib pointer to J9PortLibrary
 ...
 * @param [out] j9jimageLocation pointer to J9JImageLocation; if non-NULL, then on success it is filled with metadata
 * pointed by imageLocation
 *
 ...
 */
I_32 j9bcutil_createAndVerifyJImageLocation(J9PortLibrary* portlib, J9JImage* jimage, const char* resourceName,
    void* imageLocation, J9JImageLocation* j9jimageLocation);
 ...
 * @param [in] dataBuffer pointer to memory area where the resource data is to be copied; must have at least
 * J9JImageLocation->uncompressedSize bytes of space
 * @param [in] dataBufferSize size of memory area pointed by buffer; must be atleast J9JImageLocation->uncompressedSize
 * bytes
 *
 * @return if no error occurs then returns J9JIMAGE_NO_ERROR and dataBuffer contains the resource data, else returns a
 * negative error code Note: caller should ensure buffer is large enough to hold J9JImageLocation->uncompressedSize
 * bytes of data.
 */
I_32 j9bcutil_getJImageResource(J9PortLibrary* portlib, J9JImage* jimage, J9JImageLocation* j9jimageLocation,
    void* dataBuffer, U_64 dataBufferSize);
 ...
 * @param [out] resourceName on success *resourceName points to \0-terminated name of the resource, on error
 * *resourceName is set to NULL
 *
 ...
 */
I_32 j9bcutil_getJImageResourceName(J9PortLibrary* portlib, J9JImage* jimage, const char* module, const char* parent,
    const char* base, const char* extension, char** resourceName);
 ...
 */
const char* j9bcutil_findModuleForPackage(J9PortLibrary* portlib, J9JImage* jimage, const char* package);
 ...
 * returns void
 */
void j9bcutil_unloadJImage(J9PortLibrary* portlib, J9JImage* jimage);
 ...
 * @return I_32
 */
I_32 checkForJsrs(J9CfrClassFile* classfile);
 ...
 * @return void
 */
void inlineJsrs(I_32 hasRET, J9CfrClassFile* classfile, J9JSRIData* inlineBuffers);
 ...
 * @return void
 */
void releaseInlineBuffers(J9JSRIData* inlineBuffers);
 ...
 * @return char *
 */
const char* buildVerifyErrorString(J9JavaVM* javaVM, J9CfrError* error, U_8* className, UDATA classNameLength);
