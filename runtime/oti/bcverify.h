...
#define BCV_FIRST_STACK() ((J9BranchTargetStack*)(verifyData->stackMaps))

#define BCV_NEXT_STACK(thisStack) ((J9BranchTargetStack*)(((U_8*)(thisStack)) + (verifyData->stackSize)))

#define BCV_INDEX_STACK(stackCount) \
    ((J9BranchTargetStack*)(((U_8*)BCV_FIRST_STACK()) + (verifyData->stackSize * (stackCount))))

#define BCV_STACK_INDEX(thisStack) ((((U_8*)(thisStack)) - ((U_8*)BCV_FIRST_STACK())) / (verifyData->stackSize))
...
#define PUSH_INTEGER_CONSTANT PUSH(BCV_BASE_TYPE_INT);

#define PUSH_LONG_CONSTANT PUSH_PAIR(BCV_BASE_TYPE_LONG);

#define PUSH_FLOAT_CONSTANT PUSH(BCV_BASE_TYPE_FLOAT);

#define PUSH_DOUBLE_CONSTANT PUSH_PAIR(BCV_BASE_TYPE_DOUBLE);

#define PUSH_TEMP_OBJECT(tempIndex)                                     \
    inconsistentStack |= (temps[tempIndex] & BCV_TAG_BASE_TYPE_OR_TOP); \
    PUSH(temps[tempIndex]);

#define CHECK_TEMP(tempIndex, tempType) inconsistentStack |= (temps[(tempIndex)] != tempType);

#define CHECK_TEMP_INTEGER(tempIndex) CHECK_TEMP(tempIndex, (UDATA)BCV_BASE_TYPE_INT);

#define CHECK_TEMP_FLOAT(tempIndex) CHECK_TEMP(tempIndex, (UDATA)BCV_BASE_TYPE_FLOAT);
...
#define CHECK_TEMP_LONG(tempIndex) CHECK_TEMP_PAIR(tempIndex, (UDATA)BCV_BASE_TYPE_LONG);

#define CHECK_TEMP_DOUBLE(tempIndex) CHECK_TEMP_PAIR(tempIndex, (UDATA)BCV_BASE_TYPE_DOUBLE);
 ...
 *    is byte array (calculated via J9JavaBytecodeArrayTypeTable) or boolean array (not in
 * J9JavaBytecodeArrayTypeTable).
 *
...
#define POP_TOS_INTEGER POP_TOS(BCV_BASE_TYPE_INT);

#define POP_TOS_FLOAT POP_TOS(BCV_BASE_TYPE_FLOAT);

#define POP_TOS_2(baseType)     \
    POP_TOS(BCV_BASE_TYPE_TOP); \
    POP_TOS(baseType);

#define POP_TOS_LONG POP_TOS_2(BCV_BASE_TYPE_LONG);

#define POP_TOS_DOUBLE POP_TOS_2(BCV_BASE_TYPE_DOUBLE);
...
#define POP_TOS_INTEGER_ARRAY POP_TOS_BASE_ARRAY(BCV_BASE_TYPE_INT_BIT);

#define POP_TOS_LONG_ARRAY POP_TOS_BASE_ARRAY(BCV_BASE_TYPE_LONG_BIT);

#define POP_TOS_FLOAT_ARRAY POP_TOS_BASE_ARRAY(BCV_BASE_TYPE_FLOAT_BIT);

#define POP_TOS_DOUBLE_ARRAY POP_TOS_BASE_ARRAY(BCV_BASE_TYPE_DOUBLE_BIT);

#define POP_TOS_BYTE_ARRAY POP_TOS_BASE_ARRAY(BCV_BASE_TYPE_BYTE_BIT);

#define POP_TOS_CHAR_ARRAY POP_TOS_BASE_ARRAY(BCV_BASE_TYPE_CHAR_BIT);

#define POP_TOS_SHORT_ARRAY POP_TOS_BASE_ARRAY(BCV_BASE_TYPE_SHORT_BIT);
...
#define STORE_TEMP(tempIndex, tempType)                            \
    ...
    if (tempIndex && (temps[(tempIndex - 1)] & BCV_WIDE_TYPE_MASK) \
        && (temps[(tempIndex - 1)] & BCV_TAG_BASE_TYPE_OR_TOP)) {  \
        temps[(tempIndex - 1)] = BCV_BASE_TYPE_TOP;                \
    }

#define STORE_TEMP_OBJECT(tempIndex, tempType) STORE_TEMP(tempIndex, tempType);

#define STORE_TEMP_INTEGER(tempIndex) STORE_TEMP(tempIndex, (UDATA)(BCV_BASE_TYPE_INT));

#define STORE_TEMP_FLOAT(tempIndex) STORE_TEMP(tempIndex, (UDATA)(BCV_BASE_TYPE_FLOAT));
...
#define STORE_TEMP_LONG(tempIndex) STORE_TEMP_PAIR(tempIndex, (UDATA)(BCV_BASE_TYPE_LONG));

#define STORE_TEMP_DOUBLE(tempIndex) STORE_TEMP_PAIR(tempIndex, (UDATA)(BCV_BASE_TYPE_DOUBLE));
...
#define RELOAD_STACKTOP(liveStack) &(liveStack->stackElements[liveStack->stackTopIndex])

#define RELOAD_STACKBASE(liveStack) &(liveStack->stackElements[liveStack->stackBaseIndex])

#define SAVE_STACKTOP(liveStack, stackTop) liveStack->stackTopIndex = (IDATA)(stackTop - liveStack->stackElements)
