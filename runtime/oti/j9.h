...
#define J9CLASS_IS_MIXED(ramClass)                                                           \
    (((J9CLASS_FLAGS(ramClass) >> J9_JAVA_CLASS_RAM_SHAPE_SHIFT) & OBJECT_HEADER_SHAPE_MASK) \
...
#define IS_STRING_COMPRESSED(vmThread, object)                                                                      \
    (IS_STRING_COMPRESSION_ENABLED(vmThread) ? (J2SE_VERSION((vmThread)->javaVM) >= J2SE_19                         \
...
#define IS_STRING_COMPRESSED_VM(javaVM, object)                                                              \
    (IS_STRING_COMPRESSION_ENABLED_VM(javaVM)                                                                \
            ? (J2SE_VERSION(javaVM) >= J2SE_19 ? (((I_32)J9VMJAVALANGSTRING_CODER_VM(javaVM, object)) == 0)  \
...
#define J9VMJAVALANGSTRING_LENGTH(vmThread, object)                                                         \
    (IS_STRING_COMPRESSION_ENABLED(vmThread)                                                                \
            ? (J2SE_VERSION((vmThread)->javaVM) >= J2SE_19                                                  \
                      ? (J9INDEXABLEOBJECT_SIZE(vmThread, J9VMJAVALANGSTRING_VALUE(vmThread, object))       \
...
#define J9VMJAVALANGSTRING_LENGTH_VM(javaVM, object)                                                          \
    (IS_STRING_COMPRESSION_ENABLED_VM(javaVM)                                                                 \
            ? (J2SE_VERSION(javaVM) >= J2SE_19                                                                \
                      ? (J9INDEXABLEOBJECT_SIZE_VM(javaVM, J9VMJAVALANGSTRING_VALUE_VM(javaVM, object))       \
                            >> ((I_32)J9VMJAVALANGSTRING_CODER_VM(javaVM, object)))                           \
...
#define J9UTF8_DATA_EQUALS(data1, length1, data2, length2) \
    ((((length1) == (length2)) && (memcmp((data1), (data2), (length1)) == 0)))
#define J9UTF8_EQUALS(utf1, utf2) \
    (((utf1) == (utf2))           \
        || (J9UTF8_DATA_EQUALS(J9UTF8_DATA(utf1), J9UTF8_LENGTH(utf1), J9UTF8_DATA(utf2), J9UTF8_LENGTH(utf2))))
#define J9UTF8_LITERAL_EQUALS(data1, length1, cString) \
    (J9UTF8_DATA_EQUALS((data1), (length1), (cString), sizeof(cString) - 1))
...
#define J9_JNI_UNWRAP_REDIRECTED_REFERENCE(ref)                                                              \
    ((*(UDATA*)(ref)) & J9_REDIRECTED_REFERENCE ? **(j9object_t**)(((UDATA)(ref)) - J9_REDIRECTED_REFERENCE) \
                                                : *(j9object_t*)(ref))
...
#define J9FLAGSANDCLASS_FROM_CLASSANDFLAGS(classAndFlags) \
    (((classAndFlags) >> J9_REQUIRED_CLASS_SHIFT) | ((classAndFlags) << (sizeof(UDATA) * 8 - J9_REQUIRED_CLASS_SHIFT)))
#define J9CLASSANDFLAGS_FROM_FLAGSANDCLASS(flagsAndClass) \
    (((flagsAndClass) >> (8 * sizeof(UDATA) - J9_REQUIRED_CLASS_SHIFT)) | ((flagsAndClass) << J9_REQUIRED_CLASS_SHIFT))
#define J9RAMSTATICFIELDREF_CLASS(base) ((J9Class*)((base)->flagsAndClass << J9_REQUIRED_CLASS_SHIFT))
/* In a resolved J9RAMStaticFieldRef, the high bit of valueOffset is set, so it must be masked when adding valueOffset
 * to the ramStatics address. Note that ~IDATA_MIN has all but the high bit set. */
#define J9STATICADDRESS(flagsAndClass, valueOffset)                                        \
    ((void*)(((UDATA)((J9Class*)((flagsAndClass) << J9_REQUIRED_CLASS_SHIFT))->ramStatics) \
        + ((valueOffset) & ~IDATA_MIN)))
#define J9RAMSTATICFIELDREF_VALUEADDRESS(base) \
    ((void*)(((UDATA)J9RAMSTATICFIELDREF_CLASS(base)->ramStatics) + ((base)->valueOffset & ~IDATA_MIN)))
...
#else
#define PORT_ACCESS_FROM_ENV(jniEnv) \
    J9PortLibrary* privatePortLibrary = (J9VMTHREAD_FROM_JNIENV(jniEnv))->javaVM->portLibrary
...
#define PORT_ACCESS_FROM_WALKSTATE(walkState) \
    J9PortLibrary* privatePortLibrary = (walkState)->walkThread->javaVM->portLibrary
...
#define J9_DECLARE_CONSTANT_NAS(instanceName, name, sig) \
    const J9NameAndSignature instanceName = { (J9UTF8*)&name, (J9UTF8*)&sig }
