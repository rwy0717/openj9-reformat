 ...
 *  Obtained from:
 * https://github.com/torvalds/linux/blob/050cdc6c9501abcd64720b8cc3e7941efee9547d/arch/s390/include/asm/elf.h#L94-L109.
 *  If new facility support is required, then it must be defined there (and here), before we can check for it
 * consistently.
 *
 *  The linux kernel will use the defines in the above link to set HWCAP features. This is done inside
 * "setup_hwcaps(void)" routine found in arch/s390/kernel/setup.c in the linux kernel source tree.
 */
...
#define J9PORT_X86_FEATURE_TSC_DEADLINE \
    32 + 24 /* Processor's local APIC timer supports one-shot operation using a TSC deadline value. */
...
#define J9PORT_X86_FEATURE_OSXSAVE                                                                                \
    32 + 27 /* OS has enabled XSETBV/XGETBV instructions to access XCR0, and support for processor extended state \
...
typedef struct J9CacheInfoQuery {
    ...
    int32_t cacheType; /* bit map of cache types. If the cache is unified,
                        * all other types, e.g. J9PORT_CACHEINFO_ICACHE and J9PORT_CACHEINFO_DCACHE, will return the
                        * same data
                        */
} J9CacheInfoQuery;
