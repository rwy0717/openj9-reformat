...
#define J9JVMTI_EXTENSION_CALLBACK(env, index) \
    ((env)->extensionCallbacks.callbacks[(index)-J9JVMTI_LOWEST_EXTENSION_EVENT])
...
#define J9JVMTI_WATCHED_FIELDS_PER_UDATA ((sizeof(UDATA) * 8) / J9JVMTI_WATCHED_FIELD_BITS_PER_FIELD)
...
#define J9JVMTI_FLAG_REDEFINE_CLASS_EXTENSIONS_USED                                                               \
    2 /** Class Redefinition Extensions have been actually used. Set (and remains set) the first time a redefined \
         class with extensions has been used */
    ...
    union type {
        ...
        struct {
            ...
            U_32 classIndex; /*!< The translated index of the class referenced by this item if its from an invokevirtual
                              */
            U_32 nameAndTypeIndex; /*!< The translated index of the NameAndType/Signature referenced by this item if its
...
typedef struct jvmtiGcp_translation {
    ...
    jvmtiGcp_translationEntry**
        cp; /*!< An array of J9HashTable nodes indexed by the translated SUN constant pool indeces */
...
#define ENSURE_JTHREAD(vmThread, jthrd)                                           \
    do {                                                                          \
        if (!isSameOrSuperClassOf(J9VMJAVALANGTHREAD_OR_NULL((vmThread->javaVM)), \
...
#define ENSURE_JCLASS(vmThread, jcls)                                                                                 \
    do {                                                                                                              \
        if (!isSameOrSuperClassOf(                                                                                    \
                J9VMJAVALANGCLASS_OR_NULL((vmThread->javaVM)), J9OBJECT_CLAZZ((vmThread), *((j9object_t*)(jcls))))) { \
...
#define ENSURE_VALID_HEAP_OBJECT_FILTER(var)                                             \
    do {                                                                                 \
        if (((var) != JVMTI_HEAP_OBJECT_TAGGED) && ((var) != JVMTI_HEAP_OBJECT_UNTAGGED) \
            && ((var) != JVMTI_HEAP_OBJECT_EITHER)) {                                    \
            JVMTI_ERROR(JVMTI_ERROR_ILLEGAL_ARGUMENT);                                   \
        }                                                                                \
    } while (0)
...
#define NORMALIZED_EVENT_NUMBER(eventNumber) ((eventNumber)-J9JVMTI_LOWEST_EVENT)

#define EVENT_WORD(eventNumber, eventMap) \
    ((eventMap)->eventEnable[NORMALIZED_EVENT_NUMBER(eventNumber) / J9JVMTI_UDATA_BITS])

#define EVENT_BIT(eventNumber) (((UDATA)1) << (NORMALIZED_EVENT_NUMBER(eventNumber) % J9JVMTI_UDATA_BITS))

#define EVENT_IS_ENABLED(eventNumber, eventMap) (EVENT_WORD(eventNumber, eventMap) & EVENT_BIT(eventNumber))

#define ENABLE_EVENT(eventNumber, eventMap) EVENT_WORD(eventNumber, eventMap) |= EVENT_BIT(eventNumber)

#define DISABLE_EVENT(eventNumber, eventMap) EVENT_WORD(eventNumber, eventMap) &= ~EVENT_BIT(eventNumber)

#ifdef DEBUG
#define TRACE_JVMTI_RETURN_NOT_IMPLEMENTED(func)                                                                    \
    do {                                                                                                            \
        J9PortLibrary* notImplementedPortlib = JAVAVM_FROM_ENV(env)->portLibrary;                                   \
        OMRPORT_FROM_J9PORT(notImplementedPortlib)                                                                  \
            ->tty_printf(                                                                                           \
                OMRPORT_FROM_J9PORT(notImplementedPortlib), "JVMTI call not implemented: " J9_GET_CALLSITE() "\n"); \
