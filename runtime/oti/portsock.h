...
/* This file did redirect to the port socket header but that sort of reaching back into a private module is very unsafe
 * and breaks with vpath since the header will no longer be in the root of port */
/* As a temporary solution, the structures we were looking for will be stored here.  Eventually, this file should be
 * merged into the port_api.h */
...
/* The sockets returned as a j9socket_t (j9socket_struct*) are not actual structs, we just pretend that the pointer is a
 * struct and never dereference it.
 */
