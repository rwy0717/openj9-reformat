 ...
 * The @ref j9gp_register_handler function is partially dependent on J9 internal VM structures, and must be called with
 * NULL as a third (userData) parameter. <br><br>
 */
 ...
 *
 * All threads spawned by the vm start here and all OS signals that will be handled by fn must be registered to the OS
 * here. Upon receiving a signal from the OS, fn is responsible for calling the function specified in @ref
 * j9gp_register_handler if it is determined that a shutdown is required.
 *
...
/**
 * Sets the function that is responsible for preserving/outputting the state of the vm and initiating a graceful
 * shutdown resulting from a gp.
 *
 * @param[in] portLibrary The port library
 * @param[in] fn function responsible for preserving/outputting the state of the vm and initiating a graceful shutdown
 * resulting from a gp.
 ...
 * @param[in] index the index of the item in the specified category. The number of items for each category are defined
 * in the sourceTemplate.
 ...
 * placed in **value corresponds to a 32/64-bit integer or a pointer to a string.
 *
 * @note The program counter and module name also have negative indexes as defined by J9GP_CONTROL_PC and
 * J9GP_MODULE_NAME respectively.
 * @note Above allows the handler function registered in @ref j9gp_register_handler to distinguish (and use) them from
 * the other gp items.
 ...
 *
 * @note Return value must agree with the number of items that @ref j9gp_info makes available for the category
 * specified.
 */
 ...
 */
int32_t j9gp_startup(struct J9PortLibrary* portLibrary) { return 0; }
