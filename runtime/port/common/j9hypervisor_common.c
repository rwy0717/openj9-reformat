 ...
 * @return                   nothing
 */
void check_and_save_hypervisor_startup_error(struct J9PortLibrary* portLibrary, intptr_t errorCode)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    ...
    case J9PORT_ERROR_HYPERVISOR_NO_HYPERVISOR:
        errMsg = omrnls_lookup_message(
            J9NLS_DO_NOT_APPEND_NEWLINE, J9NLS_PORT_NO_HYPERVISOR__MODULE, J9NLS_PORT_NO_HYPERVISOR__ID, NULL);
        break;

    case J9PORT_ERROR_HYPERVISOR_UNSUPPORTED:
        errMsg = omrnls_lookup_message(J9NLS_DO_NOT_APPEND_NEWLINE, J9NLS_PORT_UNSUPPORTED_HYPERVISOR__MODULE,
            J9NLS_PORT_UNSUPPORTED_HYPERVISOR__ID, NULL);
        break;
 ...
 */
int32_t j9hypervisor_startup(struct J9PortLibrary* portLibrary)
{
    intptr_t ret = 0;
 ...
 * @return                   nothing
 */
void j9hypervisor_shutdown(struct J9PortLibrary* portLibrary)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
 ...
 */
intptr_t j9hypervisor_hypervisor_present(struct J9PortLibrary* portLibrary) { return PHD_isVirtual; }
 ...
 */
intptr_t j9hypervisor_get_hypervisor_info(struct J9PortLibrary* portLibrary, J9HypervisorVendorDetails* vendorDetails)
{
    intptr_t ret = 0;
 ...
 */
intptr_t j9hypervisor_vendor_init(struct J9PortLibrary* portLibrary)
{
    intptr_t ret = 0;
    intptr_t hypervisorStatus = 0;
     ...
     */
#if defined(LINUXPPC) || defined(LINUXPPC64) || defined(AIXPPC)
    if ((0 == strcmp(vendorName, HYPE_NAME_POWERVM)) || (0 == strcmp(vendorName, HYPE_NAME_POWERKVM))) {
 ...
 */
static intptr_t initializeHypervisorState(struct J9PortLibrary* portLibrary)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    intptr_t ret = 0;
 ...
 * @return 0 on success and a negative value on failure
 */
intptr_t j9hypervisor_get_guest_processor_usage(struct J9PortLibrary* portLibrary, J9GuestProcessorUsage* gpUsage)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    intptr_t ret = 0;
 ...
 * @return 0 on success and a negative value on failure
 */
intptr_t j9hypervisor_get_guest_memory_usage(struct J9PortLibrary* portLibrary, J9GuestMemoryUsage* gmUsage)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    intptr_t ret = 0;
...
intptr_t detect_hypervisor_from_env(struct J9PortLibrary* portLibrary)
{
    ...
    vmArgumentsList.pool = pool_new(sizeof(J9JavaVMArgInfo), 4, /* max = 1, over allocate for now */
        0, 0, J9_GET_CALLSITE(), OMRMEM_CATEGORY_VM, POOL_FOR_PORT(portLibrary));
