 ...
 */
intptr_t getX86Description(struct J9PortLibrary* portLibrary, J9ProcessorDesc* desc)
{
    ...
    if (0 == strncmp(vendor, CPUID_VENDOR_INTEL, CPUID_VENDOR_LENGTH)) {
        ...
        case CPUID_FAMILYCODE_INTELCORE: {
            ...
            uint32_t extendedModelCode
                = (processorSignature & CPUID_SIGNATURE_EXTENDEDMODEL) >> CPUID_SIGNATURE_EXTENDEDMODEL_SHIFT;
 ...
 *
 */
void getX86CPUID(uint32_t leaf, uint32_t* cpuInfo)
{
    cpuInfo[0] = leaf;
