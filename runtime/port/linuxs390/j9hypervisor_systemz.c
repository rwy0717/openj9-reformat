...
static intptr_t get_speed_file_path(struct J9PortLibrary* portLibrary, char* speedFilePath, const size_t bufSize)
{
 ...
 */
static intptr_t process_machine_speed_file(struct J9PortLibrary* portLibrary, LinuxStaticInfo* staticInfo)
{
    int32_t ret = 0;
    FILE* speedFP = NULL;
 ...
 * @return	0 on success, negative value on failure
 */
static intptr_t get_processor_speed(struct J9PortLibrary* portLibrary, LinuxStaticInfo* staticInfo)
{
    int32_t ret = 0;
 ...
 * @return	0 on success, negative value on failure
 */
static intptr_t check_hypfs_mount(struct J9PortLibrary* portLibrary, LinuxStaticInfo* staticInfo)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    int32_t ret = 0;
 ...
 */
static intptr_t get_guest_info(struct J9PortLibrary* portLibrary, LinuxStaticInfo* staticInfo)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    FILE* sysinfoFP = NULL;
    char lineStr[MAX_LINE_LENGTH] = { 0 };
    ...
    while (NULL != fgets(lineStr, MAX_LINE_LENGTH, sysinfoFP)) {
        ...
        if (tokens >= 2) {
            if ((0 == strcmp(tokenName, sysinfoVmNumber)) && (0 == strcmp(tokenValue1, sysinfoVmName))) {
 ...
 * @return	0 on success, negative value on failure
 */
static intptr_t get_hypfs_attribute(
    struct J9PortLibrary* portLibrary, const char* filepath, const char* attribName, char* attribVal)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    ...
    if (combinedLength >= PATH_MAX) {
        Trc_PRT_get_hypfs_attribute_file_path_size_exceeds_limit(
            combinedLength, PATH_MAX, J9PORT_ERROR_HYPERVISOR_HYPFS_READ_FAILED);
 ...
 */
static intptr_t read_hypfs_data(struct J9PortLibrary* portLibrary, LinuxStaticInfo* staticInfo, LinuxzVMInfo* linuxInfo)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    int32_t ret = 0;
    char* hypName = NULL;
 ...
 */
static intptr_t check_and_update_stat(
    struct J9PortLibrary* portLibrary, LinuxStaticInfo* staticInfo, LinuxzVMInfo* linuxInfo, int32_t* dataCycle)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    int32_t ret = 0;
 ...
 */
static void log_hypervisor_error(struct J9PortLibrary* portLibrary, uint32_t module_name, uint32_t message_num)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    const char* errMsg = OMRPORTLIB->nls_lookup_message(
        OMRPORTLIB, J9NLS_ERROR | J9NLS_DO_NOT_APPEND_NEWLINE, module_name, message_num, NULL);
 ...
 * @return	0 on success, negative value on failure
 */
static intptr_t get_linux_systemz_info(
    struct J9PortLibrary* portLibrary, LinuxStaticInfo* staticInfo, LinuxzVMInfo* linuxInfo)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    int32_t ret = 0;
 ...
 */
static intptr_t get_linux_systemz_processor_usage(struct J9PortLibrary* portLibrary, J9GuestProcessorUsage* gpUsage)
{
    int32_t ret = 0;
 ...
 * @return 0 on success, a negative value on failure
 */
static intptr_t get_linux_systemz_memory_usage(struct J9PortLibrary* portLibrary, J9GuestMemoryUsage* gmUsage)
{
    int32_t ret = 0;
    LinuxzVMInfo linuxInfo;
    LinuxStaticInfo* staticInfo = (struct LinuxStaticInfo*)PHD_vendorPrivateData;
 ...
 * @param [in]  portLibrary The port Library
 */
static void linux_systemz_shutdown(struct J9PortLibrary* portLibrary)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    LinuxStaticInfo* staticInfo = (struct LinuxStaticInfo*)PHD_vendorPrivateData;
 ...
 * @return	0 on success, negative value on failure
 */
intptr_t linux_systemz_startup(struct J9PortLibrary* portLibrary)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    intptr_t ret = 0;
 ...
 */
intptr_t systemz_startup(struct J9PortLibrary* portLibrary) { return linux_systemz_startup(portLibrary); }
