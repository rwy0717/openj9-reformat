...
int ftokWrapper(J9PortLibrary* portLibrary, const char* baseFile, int proj_id)
{
    ...
    if (result == -1) {
        int myerror = errno;
        setPortableError(portLibrary, ftokErrorMsgPrefix, J9PORT_ERROR_SYSV_IPC_FTOK_ERROR, myerror);
    }
    return result;
}
...
int semgetWrapper(J9PortLibrary* portLibrary, key_t key, int nsems, int semflg)
{
    ...
    if (result == -1) {
        int myerror = errno;
        setPortableError(portLibrary, semgetErrorMsgPrefix, J9PORT_ERROR_SYSV_IPC_SEMGET_ERROR, myerror);
    }
    return result;
}
...
int semctlWrapper(J9PortLibrary* portLibrary, BOOLEAN storeError, int semid, int semnum, int cmd, ...)
{
    ...
    if (result == -1) {
        if (TRUE == storeError) {
            setPortableError(portLibrary, semctlErrorMsgPrefix, J9PORT_ERROR_SYSV_IPC_SEMCTL_ERROR, myerror);
        } else {
...
int semopWrapper(J9PortLibrary* portLibrary, int semid, struct sembuf* sops, size_t nsops)
{
    ...
    if (result == -1) {
        int myerror = errno;
        setPortableError(portLibrary, semopErrorMsgPrefix, J9PORT_ERROR_SYSV_IPC_SEMOP_ERROR, myerror);
    }
    return result;
}
...
int shmgetWrapper(J9PortLibrary* portLibrary, key_t key, size_t size, int shmflg)
{
    ...
    if (result == -1) {
        int myerror = errno;
        setPortableError(portLibrary, shmgetErrorMsgPrefix, J9PORT_ERROR_SYSV_IPC_SHMGET_ERROR, myerror);
    }
    return result;
}
...
int shmctlWrapper(J9PortLibrary* portLibrary, BOOLEAN storeError, int shmid, int cmd, struct shmid_ds* buf)
{
    ...
    if (result == -1) {
        if (TRUE == storeError) {
            int myerror = errno;
            setPortableError(portLibrary, shmctlErrorMsgPrefix, J9PORT_ERROR_SYSV_IPC_SHMCTL_ERROR, myerror);
        } else {
...
void* shmatWrapper(J9PortLibrary* portLibrary, int shmid, const void* shmaddr, int shmflg)
{
    ...
    if (result == (void*)-1) {
        int myerror = errno;
        setPortableError(portLibrary, shmatErrorMsgPrefix, J9PORT_ERROR_SYSV_IPC_SHMAT_ERROR, myerror);
    }
    return result;
}
...
int shmdtWrapper(J9PortLibrary* portLibrary, const void* shmaddr)
{
    ...
    if (result == -1) {
        int myerror = errno;
        setPortableError(portLibrary, shmdtErrorMsgPrefix, J9PORT_ERROR_SYSV_IPC_SHMDT_ERROR, myerror);
    }
    return result;
}
...
int getipcWrapper(J9PortLibrary* portLibrary, int id, IPCQPROC* info, size_t length, int cmd)
{
    ...
    if (result == -1) {
        int myerror = errno;
        setPortableError(portLibrary, getipcErrorMsgPrefix, J9PORT_ERROR_SYSV_IPC_GETIPC_ERROR, myerror);
    }
    return result;
}
#endif
...
static void setPortableError(J9PortLibrary* portLibrary, const char* sysvfxName, int32_t sysvfx, int sysverrno)
{
    ...
    errmsglen = omrstr_printf(NULL, 0, "%s%s", sysvfxName, strerror(sysverrno));
    if (errmsglen <= 0) {
    ...
    omrstr_printf(errmsgbuff, errmsglen, "%s%s", sysvfxName, strerror(sysverrno));

    /*Set the error message*/
    omrerror_set_last_error_with_message(portableErrno, errmsgbuff);
