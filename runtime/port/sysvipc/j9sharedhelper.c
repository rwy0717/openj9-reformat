...
#define OUTPUTBUFSIZE 256
...
#if defined(J9ZOS390)
    {
        ...
        /* Set up file descriptors to be used by ipcs - stdin same as us; stdout and stderr to be the write end of the
         * pipe */
        fdCnt = 3;
        fdMap[0] = 0;
        fdMap[1] = pipeFd[1];
...
#pragma convlit(resume)
        ...
        pid = spawn(argv[0], fdCnt, fdMap, &inherit, (const unsigned char**)argv, (const unsigned char**)envp);
        if (-1 == pid) {
    ...
    while (NULL != fgets(outputBuf, OUTPUTBUFSIZE, ipcsOutput)) {
        ...
        if (ipcsField[IPCS_USERID_FIELD_NO] && ipcsField[IPCS_SHMID_FIELD_NO]
            && (!strcmp(ipcsField[IPCS_USERID_FIELD_NO], processOwner) || !strcmp(processOwner, "root"))) {
            ...
            Trc_PRT_shared_cleanSharedMemorySegments_noFieldsOrUseridMismatch(
                ipcsField[IPCS_USERID_FIELD_NO], ipcsField[IPCS_SHMID_FIELD_NO]);
        }
    }
 ...
 * @param[in] groupPerm 1 if the file needs to have group read-write permission, 0 otherwise; used only when creating
 * new control file
 *
 * @return 0 on sucess or less than 0 in the case of an error
 */
intptr_t ControlFileOpenWithWriteLock(struct J9PortLibrary* portLibrary, intptr_t* fd, BOOLEAN* isReadOnlyFD,
    BOOLEAN canCreateNewFile, const char* filename, uintptr_t groupPerm)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
        ...
        if (0 != omrfile_stat(filename, 0, &statbuf)) {
            ...
            case J9PORT_ERROR_FILE_NOENT:
                if (TRUE == canCreateNewFile) {
                    ...
                    Trc_PRT_shared_ControlFileFDWithWriteLock_ExitWithMessage(
                        "Error: j9file_stat() has failed because the file does not exist, and 'FALSE == "
                        "canCreateNewFile'");
                    return J9SH_FILE_DOES_NOT_EXIST;
                }
                break;
            ...
            *fd = omrfile_open(filename, exclcreateflags, mode);
            if (*fd != -1) {
                ...
                if (-1 == omrfile_close(*fd)) {
                    Trc_PRT_shared_ControlFileFDWithWriteLock_ExitWithMessage(
                        "Error: could not close exclusively created file");
                    return J9SH_FAILED;
                }
            } else {
                int32_t porterrno = omrerror_last_error_number();
                if (J9PORT_ERROR_FILE_EXIST != porterrno) {
                    Trc_PRT_shared_ControlFileFDWithWriteLock_ExitWithMessage(
                        "Error: j9file_open() failed to create new control file");
                    return J9SH_FAILED;
                }
            }
            if (0 != omrfile_stat(filename, 0, &statbuf)) {
                ...
                switch (porterrno) {
                case J9PORT_ERROR_FILE_NOENT:
                    /*Our unlinking scheme may cause the file to be deleted ... we go back to the beginning of the loop
                     * in this case*/
                    continue;
                default:
                    Trc_PRT_shared_ControlFileFDWithWriteLock_Message(
                        "Error: j9file_stat() failed after successfully creating file.");
                    return J9SH_FAILED;
                }
            }
        }
        ...
        *isReadOnlyFD = FALSE;
        ...
        if (-1 == *fd) {
            ...
            if ((porterrno == J9PORT_ERROR_FILE_NOENT) || (porterrno == J9PORT_ERROR_FILE_NOTFOUND)) {
                /*Our unlinking scheme may cause the file to be deleted ... we go back to the beginning of the loop in
                 * this case*/
        ...
        if (omrfile_lock_bytes(*fd, lockType | J9PORT_FILE_WAIT_FOR_LOCK, 0, 0) == -1) {
            if (-1 == omrfile_close(*fd)) {
                Trc_PRT_shared_ControlFileFDWithWriteLock_ExitWithMessage(
                    "Error: closing file after fcntl error has failed");
                return J9SH_FAILED;
            }
            Trc_PRT_shared_ControlFileFDWithWriteLock_ExitWithMessage(
                "Error: failed to take a lock on the control file");
            return J9SH_FAILED;
        } else {
            ...
            if (fstat((((int)*fd) - FD_BIAS), &statafter) == -1) {
                ...
                if (-1 == omrfile_close(*fd)) {
                    Trc_PRT_shared_ControlFileFDWithWriteLock_ExitWithMessage(
                        "Error: closing file after fstat error has failed");
                    return J9SH_FAILED;
                }
            ...
            if (0 == statafter.st_nlink) {
                ...
                if (-1 == omrfile_close(*fd)) {
                    Trc_PRT_shared_ControlFileFDWithWriteLock_ExitWithMessage(
                        "Error: closing file after checking link count");
                    return J9SH_FAILED;
                }
                continue;
            }
 ...
 * @param[out] controlFileStatus pointer to J9ControlFileStatus. Stores error code and message if any error occurs
 * during unlinking
 *
