 ...
 * This further reduces the minor level to bits 0-11 and alters the way minor level is obtained from the mod level in
 * shared memory control file on z/OS.
 */
/* PR 74306: increment minor level by 1.
 * Control files with major level 0, minor level 1 can be unlinked by a JVM which creates control files with higher
 * major-minor level, irrespective of the control file permission.
 */
...
#define J9SH_SHM_GET_MODLEVEL_STORAGEKEY(value) \
    (((value)&J9SH_SHM_MOD_MINOR_STORAGEKEY_MASK) >> J9SH_SHM_MOD_MINOR_STORAGEKEY_SHIFT)
#define J9SH_SHM_CREATE_MODLEVEL_WITH_STORAGEKEY(value) \
    (((value) << J9SH_SHM_MOD_MINOR_STORAGEKEY_SHIFT) | J9SH_MODLEVEL)
...
#if defined(AIXPPC)
uintptr_t isPageProtectionPossible(
    struct J9PortLibrary* portLibrary, const char* cacheDirName, uintptr_t groupPerm, uintptr_t regionSize);
#endif
