...
static int32_t createSharedMemory(J9PortLibrary* portLibrary, intptr_t fd, BOOLEAN isReadOnlyFD, const char* baseFile,
    int32_t size, int32_t perm, struct j9shmem_controlFileFormat* controlinfo, uintptr_t groupPerm,
    struct j9shmem_handle* handle);
static intptr_t checkSize(J9PortLibrary* portLibrary, int shmid, int64_t size);
static intptr_t openSharedMemory(J9PortLibrary* portLibrary, intptr_t fd, const char* baseFile, int32_t perm,
    struct j9shmem_controlFileFormat* controlinfo, uintptr_t groupPerm, struct j9shmem_handle* handle,
    BOOLEAN* canUnlink, uintptr_t cacheFileType);
static void getControlFilePath(
    struct J9PortLibrary* portLibrary, const char* cacheDirName, char* buffer, uintptr_t size, const char* name);
static void createshmHandle(J9PortLibrary* portLibrary, int32_t shmid, const char* controlFile,
    OMRMemCategory* category, uintptr_t size, j9shmem_handle* handle, uint32_t perm);
...
static intptr_t readOlderNonZeroByteControlFile(
    J9PortLibrary* portLibrary, intptr_t fd, j9shmem_controlBaseFileFormat* info);
...
intptr_t j9shmem_openDeprecated(struct J9PortLibrary* portLibrary, const char* cacheDirName, uintptr_t groupPerm,
    struct j9shmem_handle** handle, const char* rootname, uint32_t perm, uintptr_t cacheFileType, uint32_t categoryCode)
{
    ...
    if (cacheFileType == J9SH_SYSV_OLDER_EMPTY_CONTROL_FILE) {
        ...
        retval = openSharedMemory(portLibrary, fd, baseFile, perm, (j9shmem_controlFileFormat*)&controlinfo, groupPerm,
            tmphandle, &canUnlink, cacheFileType);
...
intptr_t j9shmem_open(J9PortLibrary* portLibrary, const char* cacheDirName, uintptr_t groupPerm,
    struct j9shmem_handle** handle, const char* rootname, uintptr_t size, uint32_t perm, uint32_t categoryCode,
    uintptr_t flags, J9ControlFileStatus* controlFileStatus)
{
    ...
    *handle = NULL;
    ...
    if ((flags & J9SHMEM_STORAGE_KEY_TESTING) != 0) {
        tmphandle->currentStorageProtectKey
            = (flags >> J9SHMEM_STORAGE_KEY_TESTING_SHIFT) & J9SHMEM_STORAGE_KEY_TESTING_MASK;
    } else {
    ...
    for (retryIfReadOnlyCount = 10; retryIfReadOnlyCount > 0; retryIfReadOnlyCount -= 1) {
        ...
        if ((perm == J9SH_SHMEM_PERM_READ)
            || J9_ARE_ANY_BITS_SET(
                   flags, J9SHMEM_OPEN_FOR_STATS | J9SHMEM_OPEN_FOR_DESTROY | J9SHMEM_OPEN_DO_NOT_CREATE)) {
            canCreateFile = FALSE;
        }
        if (ControlFileOpenWithWriteLock(portLibrary, &fd, &isReadOnlyFD, canCreateFile, baseFile, groupPerm)
            != J9SH_SUCCESS) {
        ...
        if (rc == J9SH_ERROR) {
            ...
            if (J9_ARE_NO_BITS_SET(
                    flags, J9SHMEM_OPEN_FOR_STATS | J9SHMEM_OPEN_FOR_DESTROY | J9SHMEM_OPEN_DO_NOT_CREATE)) {
                rc = createSharedMemory(
                    portLibrary, fd, isReadOnlyFD, baseFile, size, perm, &controlinfo, groupPerm, tmphandle);
                if (rc == J9PORT_ERROR_SHMEM_OPFAILED) {
                    ...
                    if (perm == J9SH_SHMEM_PERM_READ) {
                        ...
                        if (unlinkControlFile(portLibrary, baseFile, controlFileStatus)) {
                            Trc_PRT_shmem_j9shmem_open_Msg(
                                "Info: Control file is unlinked after call to createSharedMemory.");
                        } else {
                            Trc_PRT_shmem_j9shmem_open_Msg(
                                "Info: Failed to unlink control file after call to createSharedMemory.");
                        }
                    } else {
                ...
                Trc_PRT_shmem_j9shmem_open_Msg(
                    "Info: Control file cannot be read. Do not attempt to create shared memory as "
                    "J9SHMEM_OPEN_FOR_STATS, J9SHMEM_OPEN_FOR_DESTROY or J9SHMEM_OPEN_DO_NOT_CREATE is set");
            ...
            rc = openSharedMemory(portLibrary, fd, baseFile, perm, &controlinfo, groupPerm, tmphandle, &canUnlink,
                J9SH_SYSV_REGULAR_CONTROL_FILE);
            if (J9PORT_INFO_SHMEM_OPENED != rc) {
                exitmsg = "Error: open of the memory has failed.";

                if ((TRUE == canUnlink) && (J9PORT_ERROR_SHMEM_ZOS_STORAGE_KEY_READONLY != rc)
                    && J9_ARE_NO_BITS_SET(flags, J9SHMEM_OPEN_FOR_STATS | J9SHMEM_OPEN_DO_NOT_CREATE)) {
                    if (perm == J9SH_SHMEM_PERM_READ) {
                        Trc_PRT_shmem_j9shmem_open_Msg(
                            "Info: Control file was not unlinked (J9SH_SHMEM_PERM_READ is set).");
                    } else {
                        /* PR 74306: If the control file is read-only, do not unlink it unless the control file was
                         * created by an older JVM without this fix, which is identified by the check for major and
                         * minor level in the control file. Reason being if the control file is read-only, JVM acquires
                         * a shared lock in ControlFileOpenWithWriteLock(). This implies in multiple JVM scenario it is
                         * possible that when one JVM has unlinked and created a new control file, another JVM unlinks
                         * the newly created control file, and creates another control file. For shared class cache,
                         * this may result into situations where the two JVMs will have their own shared memory regions
                         * but would be using same semaphore set for synchronization, thereby impacting each other's
                         * performance.
                         */
                        if ((FALSE == isReadOnlyFD)
                            || ((J9SH_GET_MOD_MAJOR_LEVEL(controlinfo.common.modlevel)
                                    == J9SH_MOD_MAJOR_LEVEL_ALLOW_READONLY_UNLINK)
                                   && (J9SH_SHM_GET_MOD_MINOR_LEVEL(controlinfo.common.modlevel)
                                          <= J9SH_MOD_MINOR_LEVEL_ALLOW_READONLY_UNLINK)
                                   && (J9PORT_ERROR_SHMEM_OPFAILED_SHARED_MEMORY_NOT_FOUND == rc))) {
                            if (unlinkControlFile(portLibrary, baseFile, controlFileStatus)) {
                                Trc_PRT_shmem_j9shmem_open_Msg(
                                    "Info: Control file was unlinked after call to openSharedMemory.");
                                ...
                                Trc_PRT_shmem_j9shmem_open_Msg(
                                    "Info: Failed to unlink control file after call to openSharedMemory.");
                            }
                        } else {
                            Trc_PRT_shmem_j9shmem_open_Msg(
                                "Info: Control file was not unlinked after openSharedMemory because it is read-only "
                                "and (its modLevel does not allow unlinking or rc != SHARED_MEMORY_NOT_FOUND).");
                        }
                    }
                } else {
                    Trc_PRT_shmem_j9shmem_open_Msg(
                        "Info: Control file was not unlinked after call to openSharedMemory.");
                }
            }
        } else {
            ...
            if (J9_ARE_NO_BITS_SET(flags, J9SHMEM_OPEN_FOR_STATS | J9SHMEM_OPEN_DO_NOT_CREATE)) {
                if (perm == J9SH_SHMEM_PERM_READ) {
                    Trc_PRT_shmem_j9shmem_open_Msg(
                        "Info: Corrupt control file was not unlinked (J9SH_SHMEM_PERM_READ is set).");
                } else {
                ...
                Trc_PRT_shmem_j9shmem_open_Msg("Info: Control file was found to be corrupt but was not unlinked "
                                               "(J9SHMEM_OPEN_FOR_STATS or J9SHMEM_OPEN_DO_NOT_CREATE is set).");
            }
        }
...
void j9shmem_close(struct J9PortLibrary* portLibrary, struct j9shmem_handle** handle)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
...
void* j9shmem_attach(struct J9PortLibrary* portLibrary, struct j9shmem_handle* handle, uint32_t categoryCode)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    void* region;
    int32_t shmatPerm = 0;
        ...
        if ((handle->flags & J9SHMEM_PRINT_STORAGE_KEY_WARNING) != 0) {
            if (handle->currentStorageProtectKey != handle->controlStorageProtectKey) {
                ...
                if ((J9PORT_ERROR_SYSV_IPC_SHMAT_ERROR + J9PORT_ERROR_SYSV_IPC_ERRNO_EACCES) == myerrno) {
                    omrnls_printf(J9NLS_WARNING, J9NLS_PORT_ZOS_SHMEM_STORAGE_KEY_MISMATCH,
...
intptr_t j9shmem_destroyDeprecated(struct J9PortLibrary* portLibrary, const char* cacheDirName, uintptr_t groupPerm,
    struct j9shmem_handle** handle, uintptr_t cacheFileType)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    ...
    if (cacheFileType == J9SH_SYSV_OLDER_EMPTY_CONTROL_FILE) {
        ...
        if (ControlFileOpenWithWriteLock(portLibrary, &fd, &isReadOnlyFD, FALSE, (*handle)->baseFileName, 0)
            != J9SH_SUCCESS) {
...
intptr_t j9shmem_detach(struct J9PortLibrary* portLibrary, struct j9shmem_handle** handle)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
...
void j9shmem_findclose(struct J9PortLibrary* portLibrary, uintptr_t findhandle)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
...
uintptr_t j9shmem_findfirst(struct J9PortLibrary* portLibrary, char* cacheDirName, char* resultbuf)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
...
int32_t j9shmem_findnext(struct J9PortLibrary* portLibrary, uintptr_t findHandle, char* resultbuf)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    char file[EsMaxPath];
...
uintptr_t j9shmem_stat(struct J9PortLibrary* portLibrary, const char* cacheDirName, uintptr_t groupPerm,
    const char* name, struct J9PortShmemStatistic* statbuf)
{
    ...
    if (checkGid(portLibrary, controlinfo.common.shmid, controlinfo.gid) != 1) {
        /*If no match then this is someone elses sysv obj. The one our filed referred to was destroy and recreated by
         * someone/thing else*/
...
#endif /* defined(J9ZOS390) */
        {
            /*If no match then this is someone elses sysv obj. The one our filed referred to was destroy and recreated
             ...
             * Do not treat it as failure. Get the shared memory stats (which includes the size = 0) and let the caller
...
uintptr_t j9shmem_statDeprecated(struct J9PortLibrary* portLibrary, const char* cacheDirName, uintptr_t groupPerm,
...
void j9shmem_shutdown(struct J9PortLibrary* portLibrary) {}

int32_t j9shmem_startup(struct J9PortLibrary* portLibrary)
{
    ...
    if (NULL != portLibrary->portGlobals) {
        ...
        mos_rv = __moservices(__MO_SHMDUMPPRIORITY, sizeof(mymopl), &mymopl, &mymoptr);
        if (mos_rv != 0) {
...
static uintptr_t isControlFileName(struct J9PortLibrary* portLibrary, char* nameToTest)
{
...
intptr_t j9shmem_getDir(
    struct J9PortLibrary* portLibrary, const char* ctrlDirName, uint32_t flags, char* buffer, uintptr_t bufLength)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    ...
    if (ctrlDirName != NULL) {
        ...
        if (J9_ARE_ALL_BITS_SET(flags, J9SHMEM_GETDIR_USE_USERHOME)) {
            ...
            if (0 == omrsysinfo_get_env("HOME", homeDirBuf, J9SH_MAXPATH)) {
                ...
                if (0 < dirLen && ((dirLen + baseDirLen) < bufLength)) {
                    homeDir = homeDirBuf;
                } else {
    ...
    if (NULL == rootDir) {
        ...
        if (appendBaseDir) {
            ...
            if (omrstr_printf(buffer, bufLength, ('/' == rootDir[strlen(rootDir) - 1]) ? "%s" : "%s/", rootDir)
                == bufLength - 1) {
                Trc_PRT_j9shmem_getDir_ExitFailedOverflow();
                return -1;
            }
...
intptr_t j9shmem_protect(struct J9PortLibrary* portLibrary, const char* cacheDirName, uintptr_t groupPerm,
...
uintptr_t j9shmem_get_region_granularity(
...
int32_t j9shmem_getid(struct J9PortLibrary* portLibrary, struct j9shmem_handle* handle)
{
    if (NULL != handle) {
        return handle->shmid;
    } else {
...
static intptr_t readControlFile(J9PortLibrary* portLibrary, intptr_t fd, j9shmem_controlFileFormat* info)
{
    ...
    if (hasPadding) {
        if (rc == (sizeof(j9shmem_controlFileFormat) - sizeof(uint32_t))) {
            ...
            memcpy(&info->size, buffer + sizeof(j9shmem_controlBaseFileFormat),
                sizeof(j9shmem_controlFileFormat) - ((uintptr_t)&info->size - (uintptr_t)info));
            return J9SH_SUCCESS;
        }
    } else {
        if (rc == (sizeof(j9shmem_controlFileFormat) + sizeof(uint32_t))) {
            ...
            memcpy(info, buffer, sizeof(j9shmem_controlBaseFileFormat));
            memcpy(&info->size, buffer + sizeof(j9shmem_controlBaseFileFormat) + sizeof(uint32_t),
                sizeof(j9shmem_controlFileFormat) - ((uintptr_t)&info->size - (uintptr_t)info));
            return J9SH_SUCCESS;
        }
    }
    ...
    BOOLEAN* canUnlink, uintptr_t cacheFileType)
{
    ...
    Trc_PRT_shmem_j9shmem_openSharedMemory_storageKey(
        handle->controlStorageProtectKey, handle->currentStorageProtectKey);
    /* Switch to readonly mode if memory was created in storage protection key 9 */
    if ((9 == handle->controlStorageProtectKey) && (8 == handle->currentStorageProtectKey)
    ...
    if (-1 == shmid) {
        ...
        if ((lastError == J9PORT_ERROR_SYSV_IPC_ERRNO_ENOENT) || (lastError == J9PORT_ERROR_SYSV_IPC_ERRNO_EIDRM)) {
            Trc_PRT_shmem_j9shmem_openSharedMemory_Msg(
                "The shared SysV obj was deleted, but the control file still exists.");
            ...
            Trc_PRT_shmem_j9shmem_openSharedMemory_MsgWithError(
                "Error: can not open shared memory (shmget failed), portable errorCode = ", lastError);
            rc = J9PORT_ERROR_SHMEM_OPFAILED;
            goto failDontUnlink;
        }
    }
    ...
    do {
        ...
        if (-1 == rc) {
            ...
            if ((lastError == J9PORT_ERROR_SYSV_IPC_ERRNO_EINVAL) || (lastError == J9PORT_ERROR_SYSV_IPC_ERRNO_EIDRM)) {
                Trc_PRT_shmem_j9shmem_openSharedMemory_Msg(
                    "The SysV obj may have been removed just before, or during, our sXmctl call.");
                rc = J9PORT_ERROR_SHMEM_OPFAILED_SHARED_MEMORY_NOT_FOUND;
                goto fail;
            } else {
                if ((lastError == J9PORT_ERROR_SYSV_IPC_ERRNO_EACCES) && (shmid != -1)) {
                    Trc_PRT_shmem_j9shmem_openSharedMemory_Msg(
                        "The SysV obj may have been modified since the call to sXmget.");
                    goto fail;
                } /*Any other error and we will terminate*/

                /*If sXmctl fails our checks below will also fail (they use the same function) ... so we terminate with
                 * an error*/
                Trc_PRT_shmem_j9shmem_openSharedMemory_MsgWithError(
                    "Error: shmctl failed. Can not open shared shared memory, portable errorCode = ", lastError);
                goto failDontUnlink;
            }
        } else {
            ...
            if (getipcWrapper(portLibrary, controlinfo->common.shmid, &info, sizeof(IPCQPROC), IPCQALL) == -1) {
                ...
                if (lastError == J9PORT_ERROR_SYSV_IPC_ERRNO_EINVAL) {
                    Trc_PRT_shmem_j9shmem_openSharedMemory_Msg(
                        "__getipc(): The shared SysV obj was deleted, but the control file still exists.");
                    rc = J9PORT_ERROR_SHMEM_OPFAILED_SHARED_MEMORY_NOT_FOUND;
                    goto fail;
                } else {
                    if ((lastError == J9PORT_ERROR_SYSV_IPC_ERRNO_EACCES) && (shmid != -1)) {
                    ...
                    /*If sXmctl fails our checks below will also fail (they use the same function) ... so we terminate
                     * with an error*/
                    Trc_PRT_shmem_j9shmem_openSharedMemory_MsgWithError(
                        "Error: __getipc(): failed. Can not open shared shared memory, portable errorCode = ",
                        lastError);
                    goto failDontUnlink;
                }
            } else {
                if (info.shm.ipcqkey != controlinfo->common.ftok_key) {
                    Trc_PRT_shmem_j9shmem_openSharedMemory_Msg(
                        "The <key,id> pair in our control file is no longer valid.");
                    rc = J9PORT_ERROR_SHMEM_OPFAILED_SHM_KEY_MISMATCH;
                    goto fail;
                }
            }
...
fail:
    *canUnlink = TRUE;
    Trc_PRT_shmem_j9shmem_openSharedMemory_ExitWithMessage(
        "Error: can not open shared memory (shmget failed). Caller MAY unlink control file.");
    return rc;
failDontUnlink:
    *canUnlink = FALSE;
    Trc_PRT_shmem_j9shmem_openSharedMemory_ExitWithMessage(
        "Error: can not open shared memory (shmget failed). Caller should not unlink control file");
    return rc;
}
    ...
    struct j9shmem_handle* handle)
{
    ...
    if ((TRUE == isReadOnlyFD) || (J9SH_SHMEM_PERM_READ == perm)) {
        Trc_PRT_shmem_j9shmem_createSharedMemory_ExitWithMessage(
            "Error: can not write control file b/c either file descriptor is opened read only or shared memory "
    ...
    for (i = 0; i < maxProjIdRetries; i += 1, projId += 1) {
        ...
        if (-1 == fkey) {
            ...
            if (lastError == J9PORT_ERROR_SYSV_IPC_ERRNO_ENOENT) {
                ...
                Trc_PRT_shmem_j9shmem_createSharedMemory_MsgWithError(
                    "ftok: unexpected errno, portable errorCode = ", lastError);
            }
 ...
 * @param[in] size
 *
 * @return	-1 if we can't get info from shmid, 1 if it match is found, 0 if it does not match, -2 if shmctl()
 * returned size as 0
 */
static intptr_t checkSize(J9PortLibrary* portLibrary, int shmid, int64_t size)
{
