...
static intptr_t getSemHandle(struct J9PortLibrary* portLibrary, const char* baseFile, int setSize, uint32_t permissions,
    uint8_t projid, j9shsem_handle** handle);
static j9shsem_handle* createsemHandle(struct J9PortLibrary* portLibrary, int semid, int nsems, const char* baseFile);
static intptr_t j9shsem_checkControlFileDate(
    J9PortLibrary* portLibrary, struct j9shsem_handle* handle, const char* controlFile);
...
intptr_t j9shsem_destroy(struct J9PortLibrary* portLibrary, struct j9shsem_handle** handle)
{
    ...
    if (semctlWrapper(portLibrary, TRUE, (*handle)->semid, 0, IPC_RMID) == -1) {
        ...
        if ((myerrnoMasked == J9PORT_ERROR_SYSV_IPC_ERRNO_EINVAL)
            ...
            /* CMVC 112164 : The semaphore ID is valid, but we don't have permissions to use it. This either means that
             we've rebooted and someone else now has our semid. OR it means we're trying to access an active cache in
             use by someone else. In the former case, we want to delete the file and go for a new semid. In the latter
             case, we should fail - the file is not ours to delete. However, given the dir permissions, we can still
             unlink it. Therefore, try an operation for which we need
            ...
            rc = omrfile_chown((*handle)->baseFile, OMRPORT_FILE_IGNORE_ID,
                getegid()); /* Completely benign - done anyway when the file is created */
        } else {
            Trc_PRT_shsem_j9shsem_destroy_Debug2((*handle)->semid, myerrno);
            rc = -1;
        }
 ...
 */
int32_t j9shsem_startup(struct J9PortLibrary* portLibrary) { return 0; }
    ...
    if (stat(controlFile, &filestat) != -1) {
        if (semctlWrapper(portLibrary, TRUE, handle->semid, 0, IPC_STAT, semctlarg) != -1) {
            if (semstat.sem_ctime < filestat.st_ctime) {
                Trc_PRT_shsem_checkControlFileDate_isstale(handle->semid, controlFile);
                /*
