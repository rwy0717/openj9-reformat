...
static intptr_t createSemaphore(struct J9PortLibrary* portLibrary, intptr_t fd, BOOLEAN isReadOnlyFD, char* baseFile,
    int32_t setSize, j9shsem_baseFileFormat* controlinfo, uintptr_t groupPerm);
static intptr_t openSemaphore(struct J9PortLibrary* portLibrary, intptr_t fd, char* baseFile,
    j9shsem_baseFileFormat* controlinfo, uintptr_t groupPerm, uintptr_t cacheFileType);
static void createsemHandle(
    struct J9PortLibrary* portLibrary, int semid, int nsems, char* baseFile, j9shsem_handle* handle);
    ...
    *handle = NULL;
    ...
    for (retryIfReadOnlyCount = 10; retryIfReadOnlyCount > 0; retryIfReadOnlyCount -= 1) {
        ...
        if (J9_ARE_ANY_BITS_SET(
                flags, J9SHSEM_OPEN_FOR_STATS | J9SHSEM_OPEN_FOR_DESTROY | J9SHSEM_OPEN_DO_NOT_CREATE)) {
            canCreateFile = FALSE;
        }
        ...
        if (rc == -1) {
                ...
                if (rc == J9PORT_ERROR_SHSEM_OPFAILED) {
                    ...
                    if (isReadOnlyFD == FALSE) {
                        if (unlinkControlFile(portLibrary, baseFile, controlFileStatus)) {
                            Trc_PRT_shsem_j9shsem_open_Msg(
                                "Info: Control file is unlinked after call to createSemaphore.");
                        } else {
                            Trc_PRT_shsem_j9shsem_open_Msg(
                                "Info: Failed to unlink control file after call to createSemaphore.");
                        }
                    } else {
                        Trc_PRT_shsem_j9shsem_open_Msg(
                            "Info: Control file is not unlinked after call to createSemaphore(isReadOnlyFD == TRUE).");
                    }
                }
            } else {
                Trc_PRT_shsem_j9shsem_open_Msg(
                    "Info: Control file cannot be read. Do not attempt to create semaphore as J9SHSEM_OPEN_FOR_STATS, "
                    "J9SHSEM_OPEN_FOR_DESTROY or J9SHSEM_OPEN_DO_NOT_CREATE is set");
            ...
            if (J9PORT_INFO_SHSEM_OPENED != rc) {
                ...
                if ((J9PORT_ERROR_SHSEM_OPFAILED_DONT_UNLINK != rc)
                    ...
                    /* PR 74306: If the control file is read-only, do not unlink it unless the control file was created
                     * by an older JVM without this fix, which is identified by the check for major and minor level in
                     * the control file. Reason being if the control file is read-only, JVM acquires a shared lock in
                     * ControlFileOpenWithWriteLock(). This implies in multiple JVM scenario it is possible that when
                     * one JVM has unlinked and created a new control file, another JVM unlinks the newly created
                     * control file, and creates another control file. For shared class cache, this may result into
                     * situations where the two JVMs, accessing the same cache, would create two different semaphore
                     * sets for same shared memory region causing one of them to detect cache as corrupt due to semid
                     * mismatch.
                     */
                    if ((FALSE == isReadOnlyFD)
                        || ((J9SH_GET_MOD_MAJOR_LEVEL(controlinfo.modlevel)
                                == J9SH_MOD_MAJOR_LEVEL_ALLOW_READONLY_UNLINK)
                               && (J9SH_SEM_GET_MOD_MINOR_LEVEL(controlinfo.modlevel)
                                      <= J9SH_MOD_MINOR_LEVEL_ALLOW_READONLY_UNLINK)
                               && (J9PORT_ERROR_SHSEM_OPFAILED_SEMAPHORE_NOT_FOUND == rc))) {
                        if (unlinkControlFile(portLibrary, baseFile, controlFileStatus)) {
                            Trc_PRT_shsem_j9shsem_open_Msg(
                                "Info: Control file is unlinked after call to openSemaphore.");
                            ...
                            Trc_PRT_shsem_j9shsem_open_Msg(
                                "Info: Failed to unlink control file after call to openSemaphoe.");
                        }
                    } else {
                        Trc_PRT_shsem_j9shsem_open_Msg(
                            "Info: Control file is not unlinked after openSemaphore because it is read-only and (its "
                            "modLevel does not allow unlinking or rc != SEMAPHORE_NOT_FOUND).");
                    }
                } else {
            ...
            if (J9_ARE_NO_BITS_SET(flags, J9SHSEM_OPEN_FOR_STATS | J9SHSEM_OPEN_DO_NOT_CREATE)) {
                if (isReadOnlyFD == FALSE) {
                    ...
                    Trc_PRT_shsem_j9shsem_open_Msg(
                        "Info: Corrupt control file is not unlinked (isReadOnlyFD == TRUE).");
                }
            } else {
                Trc_PRT_shsem_j9shsem_open_Msg("Info: Control file is found to be corrupt but is not unlinked "
                                               "(J9SHSEM_OPEN_FOR_STATS or J9SHSEM_OPEN_DO_NOT_CREATE is set).");
            }
        }
    ...
    struct J9PortLibrary* portLibrary, struct j9shsem_handle* handle, struct J9PortShsemStatistic* statbuf)
{
    ...
    /* Same code as in j9file_stat(). Probably a new function to translate operating system modes to J9 modes is
     * required. */
...
intptr_t j9shsem_deprecated_destroy(struct J9PortLibrary* portLibrary, struct j9shsem_handle** handle)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    ...
    struct J9PortLibrary* portLibrary, struct j9shsem_handle** handle, uintptr_t cacheFileType)
{
    ...
    if (cacheFileType == J9SH_SYSV_OLDER_EMPTY_CONTROL_FILE) {
        Trc_PRT_shsem_deprecated_destroyDeprecated_Message(
            "Info: cacheFileType == J9SH_SYSV_OLDER_EMPTY_CONTROL_FILE.");
        if (ControlFileOpenWithWriteLock(portLibrary, &fd, &isReadOnlyFD, FALSE, (*handle)->baseFile, 0)
            != J9SH_SUCCESS) {
    ...
    int32_t setSize, j9shsem_baseFileFormat* controlinfo, uintptr_t groupPerm)
{
    ...
    if (isReadOnlyFD == TRUE) {
        Trc_PRT_shsem_j9shsem_createsemaphore_ExitWithMessage(
            "Error: can not write control file b/c file descriptor is opened read only.");
        return J9PORT_ERROR_SHSEM_OPFAILED;
    }
    ...
    for (i = 0; i < maxProjIdRetries; i += 1, projId += 1) {
        ...
        if (-1 == fkey) {
            ...
            if (lastError == J9PORT_ERROR_SYSV_IPC_ERRNO_ENOENT) {
                ...
                Trc_PRT_shsem_j9shsem_createsemaphore_MsgWithError(
        ...
        if (-1 == semid) {
            ...
            Trc_PRT_shsem_j9shsem_createsemaphore_ExitWithMessageAndError(
                "Error: semid = -1 with portable errorCode = ", lastError);
            return J9PORT_ERROR_SHSEM_OPFAILED;
        }
        ...
        if (semctlWrapper(portLibrary, TRUE, semid, setSize, SETVAL, sem_union) == -1) {
            Trc_PRT_shsem_j9shsem_createsemaphore_ExitWithMessage(
                "Could not mark semaphore as initialized initialized.");
            /* The code should never ever fail here b/c we just created, and obviously own the new semaphore.
             * If semctl call below fails we are already in an error condition, so
             * don't worry about failure here.
    ...
    j9shsem_baseFileFormat* controlinfo, uintptr_t groupPerm, uintptr_t cacheFileType)
{
    ...
    if (-1 == semid) {
        ...
        if ((lastError == J9PORT_ERROR_SYSV_IPC_ERRNO_ENOENT) || (lastError == J9PORT_ERROR_SYSV_IPC_ERRNO_EIDRM)) {
            ...
            Trc_PRT_shsem_j9shsem_opensemaphore_ExitWithMessage(
            ...
            Trc_PRT_shsem_j9shsem_opensemaphore_MsgWithError(
                "Error: can not open shared semaphore (semget failed), portable errorCode = ", lastError);
            goto failDontUnlink;
        }
    }
    ...
    do {
        ...
        if (-1 == rc) {
            ...
            if ((lastError == J9PORT_ERROR_SYSV_IPC_ERRNO_EINVAL) || (lastError == J9PORT_ERROR_SYSV_IPC_ERRNO_EIDRM)) {
                /*the sysv obj may have been removed just before, or during, our sXmctl call. In this case our key is
                 * free to be reused.*/
                Trc_PRT_shsem_j9shsem_opensemaphore_Msg(
                    "The shared SysV obj was deleted, but the control file still exists.");
                rc = J9PORT_ERROR_SHSEM_OPFAILED_SEMAPHORE_NOT_FOUND;
                goto fail;
            } else {
                 ...
                 * an error*/
                Trc_PRT_shsem_j9shsem_opensemaphore_MsgWithError(
                    "Error: semctl failed. Can not open shared semaphore, portable errorCode = ", lastError);
                goto failDontUnlink;
            }
        } else {
            ...
            if (getipcWrapper(portLibrary, controlinfo->semid, &info, sizeof(IPCQPROC), IPCQALL) == -1) {
                ...
                if (lastError == J9PORT_ERROR_SYSV_IPC_ERRNO_EINVAL) {
                     ...
                     * with an error*/
                    Trc_PRT_shsem_j9shsem_opensemaphore_MsgWithError(
                        "Error: __getipc() failed. Can not open shared shared semaphore, portable errorCode = ",
                        lastError);
                    goto failDontUnlink;
                }
            } else {
...
fail:
    Trc_PRT_shsem_j9shsem_opensemaphore_ExitWithMessage(
        "Error: can not open shared semaphore (semget failed). Caller MAY unlink the control file");
    return rc;
failDontUnlink:
    Trc_PRT_shsem_j9shsem_opensemaphore_ExitWithMessage(
        "Error: can not open shared semaphore (semget failed). Caller should not unlink the control file");
...
int32_t j9shsem_deprecated_getid(struct J9PortLibrary* portLibrary, struct j9shsem_handle* handle)
{
    return handle->semid;
}
