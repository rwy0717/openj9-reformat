...
static intptr_t translateModifiedUtf8ToPlatform(
    struct J9PortLibrary* portLibrary, const uint8_t* inBuffer, uintptr_t inBufferSize, uint8_t** outBuffer);
static void freeTranslatedMemory(struct J9PortLibrary* portLibrary, char** newCommand, uintptr_t commandLength,
    char** execEnv, uintptr_t execEnvLen);
...
intptr_t j9process_create(struct J9PortLibrary* portLibrary, const char* command[], uintptr_t commandLength,
    char* env[], uintptr_t envSize, const char* dir, uint32_t options, intptr_t j9fdInput, intptr_t j9fdOutput,
    intptr_t j9fdError, J9ProcessHandle* processHandle)
{
    ...
    if (FLAG_IS_NOT_SET(J9PORT_PROCESS_INHERIT_STDERR, options)) {
        if (FLAG_IS_NOT_SET(J9PORT_PROCESS_IGNORE_OUTPUT, options)) {
            if ((J9PORT_INVALID_FD == j9fdError)
                && (FLAG_IS_NOT_SET(J9PORT_PROCESS_REDIRECT_STDERR_TO_STDOUT, options))) {
                if (-1 == pipe(newFD[2])) {
                    pipeFailed = 1;
                } else {
                    if (FLAG_IS_SET(J9PORT_PROCESS_NONBLOCKING_IO, options)) {
...

    {
        for (i = 0; i < commandLength; i++) {
            intptr_t translateStatus = translateModifiedUtf8ToPlatform(
                portLibrary, (const uint8_t*)command[i], strlen(command[i]), (uint8_t**)&(newCommand[i]));
    ...
    if (envSize > 0) {
        ...
        for (i = 0; i < envSize; i++) {
            ...
            translateStatus = translateModifiedUtf8ToPlatform(
                portLibrary, (const uint8_t*)env[i], strlen(env[i]), (uint8_t**)&(execEnv[i]));
            if (0 != translateStatus) {
                /* most likely out of memory, free the strings we just converted. */
                freeTranslatedMemory(portLibrary, newCommand, commandLength, execEnv, envSize);
                return translateStatus;
    ...
    if (grdpid == 0) {
        ...
        processHandleStruct = (J9ProcessHandleStruct*)omrmem_allocate_memory(
            sizeof(J9ProcessHandleStruct), OMRMEM_CATEGORY_PORT_LIBRARY);
        if (FLAG_IS_NOT_SET(J9PORT_PROCESS_INHERIT_STDIN, options)) {
...
intptr_t j9process_waitfor(struct J9PortLibrary* portLibrary, J9ProcessHandle processHandle)
{
...
intptr_t j9process_terminate(struct J9PortLibrary* portLibrary, J9ProcessHandle processHandle)
{
...
intptr_t j9process_read(
    struct J9PortLibrary* portLibrary, J9ProcessHandle processHandle, uintptr_t flags, void* buffer, uintptr_t numBytes)
{
    ...
    case J9PORT_PROCESS_STDOUT:
        sHandle = processHandleStruct->outHandle;
        break;
    case J9PORT_PROCESS_STDERR:
        sHandle = processHandleStruct->errHandle;
        break;
    default:
        return J9PORT_ERROR_PROCESS_INVALID_PARAMS;
    }
...
intptr_t j9process_get_available(struct J9PortLibrary* portLibrary, J9ProcessHandle processHandle, uintptr_t flags)
{
    ...
    switch (flags) {
    case J9PORT_PROCESS_STDOUT:
        sHandle = (int)processHandleStruct->outHandle;
        break;
    case J9PORT_PROCESS_STDERR:
        sHandle = (int)processHandleStruct->errHandle;
        break;
    default:
        return J9PORT_ERROR_PROCESS_INVALID_PARAMS;
    }
...
intptr_t j9process_close(struct J9PortLibrary* portLibrary, J9ProcessHandle* processHandle, uint32_t options)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    J9ProcessHandleStruct* processHandleStruct = (J9ProcessHandleStruct*)*processHandle;
    int32_t error = 0;
...
intptr_t j9process_isComplete(struct J9PortLibrary* portLibrary, J9ProcessHandle processHandle)
{
...
intptr_t j9process_get_exitCode(struct J9PortLibrary* portLibrary, J9ProcessHandle processHandle)
{
    J9ProcessHandleStruct* processHandleStruct = (J9ProcessHandleStruct*)processHandle;
    ...
    struct J9PortLibrary* portLibrary, const uint8_t* inBuffer, uintptr_t inBufferSize, uint8_t** outBuffer)
{
    ...
    *outBuffer = NULL;
    bufferLength = omrstr_convert(J9STR_CODE_MUTF8, J9STR_CODE_PLATFORM_RAW, (char*)inBuffer, inBufferSize, NULL, 0);
    ...
    resultLength = omrstr_convert(
        J9STR_CODE_MUTF8, J9STR_CODE_PLATFORM_RAW, (char*)inBuffer, inBufferSize, (char*)result, bufferLength);
    /* do the conversion */
 ...
 */
static void freeTranslatedMemory(
    struct J9PortLibrary* portLibrary, char** newCommand, uintptr_t commandLength, char** execEnv, uintptr_t execEnvLen)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    uintptr_t i;
    if (NULL != newCommand) {
