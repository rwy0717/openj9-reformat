 ...
 */
static int32_t getSocketFamily(struct J9PortLibrary* portLibrary, j9socket_t handle)
{
 ...
 */
int32_t j9sock_fdset_size(struct J9PortLibrary* portLibrary, j9socket_t handle) { return SOCKET_CAST(handle) + 1; }
...
#define bufferSize (ptBuffers->gethostBufferSize)
        ...
        while (TRUE) {
            if (allocBuffer == TRUE || (NULL == buffer)) {
                 ...
                 * buffer.  This bug has not been observed on other platforms, but EXTRA_SPACE is added anyway as a
                 * precaution.*/
...
#if GLIBC_R
            gethostbyaddr_r(
                (char*)&addr->sin_addr, length, addr->sin_family, hostentBuffer, buffer, bufferSize, &result, &herr);
#elif OTHER_R
            result = gethostbyaddr_r(
                (char*)&addr->sin_addr, length, addr->sin_family, hostentBuffer, buffer, bufferSize, &herr);
#endif /* GLIBC_R */
            /* allocate more space if the buffer is too small */
            if (ERANGE == herr) {
                omrmem_free_memory(buffer);
...
#if defined(AIXPPC) || defined(J9ZOS390)
    if ((IPPROTO_IP == platformLevel) && (IP_MULTICAST_TTL == platformOption)
        && (J9ADDR_FAMILY_AFINET6 == portableFamily)) {
        platformLevel = IPPROTO_IPV6;
...
#if defined(AIXPPC) || defined(J9ZOS390)
    if ((IPPROTO_IP == platformLevel) && (IP_MULTICAST_IF == platformOption)
        && (J9ADDR_FAMILY_AFINET6 == portableFamily)) {
        platformLevel = IPPROTO_IPV6;
 ...
 */
uint16_t j9sock_ntohs(struct J9PortLibrary* portLibrary, uint16_t val)
{
    uint16_t rc;
    ...
    j9fdset_t exceptfd_notSupported, j9timeval_t timeout)
{
    ...
    if ((nfds >= FD_SETSIZE) || (NULL == timeout)) {
        ...
        if (TRUE != (readfdHasValidFD || writefdHasValidFD)) {
            rc = omrerror_set_last_error_with_message(
                J9PORT_ERROR_SOCKET_ARGSINVALID, "One of readfd and writefd must contain a valid socket");
            Trc_PRT_sock_j9sock_select_Exit(rc);
            return rc;
        }
        ...
        /**
         * JAZZ103 80030: PMR 46171,001,866 : JDWP module failed to initialize due to socket accept failed with
         * J9PORT_ERROR_SOCKET_OPFAILED
         *
         * In the case of poll fails with EINTR, continue trying to poll since EINTR happens randomly for no reason.
         */
        do {
            /* Make sure revents is initilized to 0 before poll call. revents might not be initilized to 0 if this is
             * not the first try of calling poll */
            pfds[0].revents = 0;
            pfds[1].revents = 0;
    ...
    struct J9PortLibrary* portLibrary, j9socket_t socketP, int32_t optlevel, int32_t optname, int32_t* optval)
{
    ...
    } else if ((IPPROTO_IP == platformLevel) && (IP_MULTICAST_TTL == platformOption)
        && (J9ADDR_FAMILY_AFINET6 == portableFamily)) {
        platformLevel = IPPROTO_IPV6;
        platformOption = IPV6_MULTICAST_HOPS;
    }
    ...
    /* for LINUX in order to enable support sending the ipv6_flowinfo field we have to set the SEND_FLOWINFO option on
       the socket In java one value is set that is used for both the traffic class in the flowinfo field and the IP_TOS
       option.  Therefore  if the caller is setting traffic class then this indicates that we should also be setting the
       flowinfo field so we need to set this option.  Howerver it can only be set on IPv6 sockets */
#if defined(IPv6_FUNCTION_SUPPORT)
#if defined(LINUX) || defined(OSX)
    if ((OS_IPPROTO_IP == platformLevel) && (OS_IP_TOS == platformOption)
        && (socketP->family == J9ADDR_FAMILY_AFINET6)) {
        uint32_t on = 1;
        uint32_t result = 0;
        int32_t err = 0;
 ...
 */
static void mapIPv4AddressToIPv6Address(
    struct J9PortLibrary* portLibrary, struct in_addr* ipv4Addr, struct in6_addr* ipv6Addr)
{
 ...
 * @return	0, if no errors occurred, otherwise the (negative) error code.
 */
static int32_t mapIPv4InterfaceAddressToIPv6InterfaceIndex(
    struct J9PortLibrary* portLibrary, struct in_addr* ipv4Addr, uint32_t* ipv6Interface)
{
    ...
    if (0 == rc) {
        ...
        /*
         * Walk all the network interfaces looking for an interface with either the given ipv4Addr or its equivalent
        ...
        for (i = 0; (rc != 0) && (i < array.length); i++) {
            for (j = 0; j < array.elements[i].numberAddresses; j++) {
                if (((sizeof(*ipv4Addr) == array.elements[i].addresses[j].length)
                        && (memcmp(ipv4Addr, &array.elements[i].addresses[j].addr, sizeof(*ipv4Addr)) == 0))
                    || ((sizeof(ipv6Addr) == array.elements[i].addresses[j].length)
                           && (memcmp(&ipv6Addr, &array.elements[i].addresses[j].addr, sizeof(ipv6Addr)) == 0))) {
 ...
 * @return	0, if no errors occurred, otherwise the (negative) error code.
 */
static int32_t convertIPMulticastRequestFromV4ToV6(
    struct J9PortLibrary* portLibrary, struct ip_mreq* ipmreq, struct ipv6_mreq* ipmreq6)
{
 ...
 * @return	0, if no errors occurred, otherwise the (negative) error code.
 */
static int32_t helperSetoptIpmreq(struct J9PortLibrary* portLibrary, j9socket_t socketP, int32_t optlevel,
    int32_t optname, void* optval, socklen_t optlen)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    ...
    if (J9ADDR_FAMILY_AFINET4 == portableFamily) {
        ...
        /*
         * If the socket is an IPv6 socket, but the caller thinks it is an IPv4 socket, map the multicast request from
         * IPv4 to IPv6. This can happen with a dual-stack sockets implementation if the caller asked for a socket with
         * AF_UNSPEC, then bound it to an IPv4 address and port.
         */
    ...
    struct J9PortLibrary* portLibrary, j9socket_t socketP, int32_t optlevel, int32_t optname, j9ipmreq_t optval)
{
    ...
    return helperSetoptIpmreq(
        portLibrary, socketP, optlevel, optname, (void*)(&optval->addrpair), sizeof(optval->addrpair));
}
...
int32_t j9sock_sockaddr_init(struct J9PortLibrary* portLibrary, j9sockaddr_t handle, int16_t family,
...
int32_t j9sock_sockaddr_init6(struct J9PortLibrary* portLibrary, j9sockaddr_t handle, uint8_t* addr, int32_t addrlength,
    int16_t family, uint16_t portNetworkOrder, uint32_t flowinfo, uint32_t scope_id, j9socket_t sock)
{
    OSSOCKADDR* sockaddr;
#ifdef IPv6_FUNCTION_SUPPORT
...
#ifdef IPv6_FUNCTION_SUPPORT
        if (j9sock_socketIsValid(portLibrary, sock) && (getSocketFamily(portLibrary, sock) == J9ADDR_FAMILY_AFINET6)) {

            /* to talk IPv4 on an IPv6 socket we need to map the IPv4 address to an IPv6 format.  If mapAddress is true
             * then we do this */
            ...
            /* do a check if it is the any address.  we know the top 4 bytes of sockaddr_6->sin6_addr.s6_addr are 0's as
    we just cleared the, so we use them to do the check */
 ...
 * @param[in] step must be one of 3 states (J9_PORT_SOCKET_STEP_START, J9_PORT_SOCKET_STEP_CHECK, and
 * J9_PORT_SOCKET_STEP_DONE)
 ...
 * 				of sockaddr to tell the platform that the subsequent connect call is a request to
 * disconnect
 *
...
static int32_t disconnectSocket(struct J9PortLibrary* portLibrary, j9socket_t sock, socklen_t fromlen)
{
    ...
    if (0 == getsockopt(SOCKET_CAST(sock), SOL_SOCKET, SO_TYPE, (void*)&uCharOptval, &optlen)) {
        if (uCharOptval == J9SOCKET_DGRAM) {
             ...
             * for use later in bind call, if it fails, proceed with the disconnect, and do not bother to bind
             * explicitly */
