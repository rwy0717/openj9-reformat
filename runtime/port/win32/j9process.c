 ...
 * @return	the (negative) portable error code
 */
static intptr_t returnErrorAndClearup(int32_t errorCode, J9ProcessWin32Pipes* pipes)
{
...
static intptr_t getUnicodeCmdline(
    struct J9PortLibrary* portLibrary, const char* command[], uintptr_t commandLength, wchar_t** unicodeCmdline)
{
    char* needToBeQuoted;
    size_t length, l;
    intptr_t rc = 0;
    ...
    intptr_t j9fdError, J9ProcessHandle* processHandle)
{
    ...
    processHandleStruct
        = (J9ProcessHandleStruct*)omrmem_allocate_memory(sizeof(J9ProcessHandleStruct), OMRMEM_CATEGORY_PORT_LIBRARY);
        ...
        if (FLAG_IS_SET(J9PORT_PROCESS_REDIRECT_STDERR_TO_STDOUT, options)) {
            ...
            if (FLAG_IS_NOT_SET(J9PORT_PROCESS_IGNORE_OUTPUT, options)) {
                if (0 == CreatePipe(&(pipes.errR), &(pipes.errW), &sAttrib, 512)) {
                    ...
                    if (0
                        == DuplicateHandle(GetCurrentProcess(), pipes.errR, GetCurrentProcess(), &(pipes.errDup), 0,
    ...
    if (envSize) {
        ...
        unicodeEnv = (wchar_t*)omrmem_allocate_memory(
            (envRemaining + 1) * 2, OMRMEM_CATEGORY_PORT_LIBRARY); /* +1 for final null terminator, * 2 for unicode */
        if (NULL == unicodeEnv) {
            rc = J9PORT_ERROR_PROCESS_OPFAILED;
        } else {
            cursor = unicodeEnv;
...
intptr_t j9process_get_available(struct J9PortLibrary* portLibrary, J9ProcessHandle processHandle, uintptr_t flags)
{
    ...
    switch (flags) {
    case J9PORT_PROCESS_STDOUT:
        sHandle = (HANDLE)processHandleStruct->outHandle;
        break;
    case J9PORT_PROCESS_STDERR:
        sHandle = (HANDLE)processHandleStruct->errHandle;
        break;
    default:
        return J9PORT_ERROR_PROCESS_INVALID_PARAMS;
    }
    retVal = PeekNamedPipe((HANDLE)sHandle, NULL, (DWORD)0, NULL, (LPDWORD)&availBytes, NULL);
    if (!retVal) {
 ...
 **/
uintptr_t j9process_get_private_bytes(struct J9PortLibrary* portLibrary, uintptr_t pid)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    HANDLE hProcess;
    ...
    dynamicSize
        = sizeof(PSAPI_WORKING_SET_INFORMATION) + ((workingSet.NumberOfEntries * 2) * sizeof(PSAPI_WORKING_SET_BLOCK));
