...
static int32_t createMappedFile(
    struct J9PortLibrary* portLibrary, const char* cacheDirName, struct j9shmem_handle* handle, uintptr_t flags);
...
char* getModifiedSharedMemoryPathandFileName(
...
intptr_t j9shmem_openDeprecated(struct J9PortLibrary* portLibrary, const char* cacheDirName, uintptr_t groupPerm,
    struct j9shmem_handle** handle, const char* rootname, uint32_t perm, uintptr_t cacheFileType, uint32_t category)
{
    return J9PORT_ERROR_SHMEM_OPFAILED;
}

intptr_t j9shmem_open(struct J9PortLibrary* portLibrary, const char* cacheDirName, uintptr_t groupPerm,
    ...
    uintptr_t flags, J9ControlFileStatus* controlFileStatus)
{
    ...
    myHandle
        = omrmem_allocate_memory(sizeof(struct j9shmem_handle) + strlen(rootname) + 1, OMRMEM_CATEGORY_PORT_LIBRARY);
    if (NULL == myHandle) {
    ...
    modifiedSharedMemoryFileName
        = getModifiedSharedMemoryPathandFileName(portLibrary, cacheDirName, myHandle->rootName);
    ...
    const char* name, struct J9PortShmemStatistic* statbuf)
{
     ...
     */
    memHandle = CreateFileW(unicodePath, GENERIC_READ, (FILE_SHARE_READ | FILE_SHARE_WRITE), NULL, OPEN_EXISTING,
...
static intptr_t createDirectory(struct J9PortLibrary* portLibrary, char* pathname)
{
    ...
    while (currentIndex < pathlength) {
        ...
        while (*(pathname + currentIndex) != 0 && *(pathname + currentIndex) != DIR_SEPARATOR)
            currentIndex++;
    ...
    struct J9PortLibrary* portLibrary, const char* cacheDirName, struct j9shmem_handle* handle, uintptr_t flags)
{
    ...
    handle->shmHandle = CreateFileMapping((HANDLE)handle->mappedFile, NULL, convertPerm(handle->perm), maxSizeHigh32,
        maxSizeLow32, modifiedSharedMemoryFileName);
    if (NULL == handle->shmHandle) {
        /* Need to clean up the file */
        DWORD lastError = GetLastError();
        Trc_PRT_shmem_createMappedFile_createFileMappingFailed(
            handle->mappedFile, convertPerm(handle->perm), handle->size, modifiedSharedMemoryFileName, lastError);
        omrfile_close(handle->mappedFile);
        ...
        Trc_PRT_shmem_createMappedFile_createFileMappingSucceeded(
            handle->mappedFile, convertPerm(handle->perm), handle->size, modifiedSharedMemoryFileName);
    }
...
intptr_t j9shmem_getDir(
    struct J9PortLibrary* portLibrary, const char* ctrlDirName, uint32_t flags, char* shmemdir, uintptr_t bufLength)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    int32_t rc;
...
int32_t j9shmem_getid(struct J9PortLibrary* portLibrary, struct j9shmem_handle* handle) { return 0; }
