...
static void fill_addrinfo(OSADDRINFO* ipv6_result, j9addrinfo_t result, int flags);

static int32_t ipv4_getaddrinfo(struct J9PortLibrary* portLibrary, char* name, const OSADDRINFO* addr_info_hints,
    OSADDRINFO** ipv6_result, int flags);
 ...
 * @internal updates the socketHandle by putting the socket in either the ipv4 or ipv6 slot
 */
BOOL useIPv4Socket(j9socket_t sockHandle) { return (sockHandle->flags & SOCKET_USE_IPV4_MASK) == SOCKET_USE_IPV4_MASK; }
 ...
 * \arg J9PORT_ERROR_SOCKET_WOULDBLOCK, the socket is marked as nonblocking and no connections are present to be
 * accepted.,
 */
 ...
 */
int32_t j9sock_bind(struct J9PortLibrary* portLibrary, j9socket_t sock, j9sockaddr_t addr)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    int32_t rc = 0;
    ...
    /* use the IPv4 socket, if is an IPv4 address or where  IPv6 socket is null (when preferIPv4Stack=true), it will
     * give a meaningful error msg */
    ...
    if (isAnyAddress
        && ((isIPv4 && (sock->flags & SOCKET_IPV6_OPEN_MASK)) || (!isIPv4 && (sock->flags & SOCKET_IPV4_OPEN_MASK)))) {
 ...
 * \arg J9PORT_ERROR_SOCKET_BADSOCKET, on generic error
 * \arg J9PORT_ERROR_SOCKET_SYSTEMBUSY, if system busy handling other requests
 * \arg J9PORT_ERROR_SOCKET_WOULDBLOCK,  the socket is marked as nonblocking and SO_LINGER is set to a nonzero time-out
 * value.
 */
int32_t j9sock_close(struct J9PortLibrary* portLibrary, j9socket_t* sock)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    int32_t rc1 = 0;
    int32_t rc2 = 0;
...
int32_t j9sock_connect(struct J9PortLibrary* portLibrary, j9socket_t sock, j9sockaddr_t addr)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    int32_t rc = 0;
    DWORD socketType;
    ...
    if (sock->flags & SOCKET_IPV4_OPEN_MASK) {
        if (getsockopt(sock->ipv4, SOL_SOCKET, SO_TYPE, (char*)&socketType, &socketTypeLen) == SOCKET_ERROR) {
            Trc_PRT_sock_j9sock_connect_failure_cause(
                "getsockopt(sock->ipv4,  SOL_SOCKET, SO_TYPE, (char*)&socketType, &socketTypeLen) == SOCKET_ERROR");
            rc = WSAGetLastError();
        ...
        if (getsockopt(sock->ipv6, SOL_SOCKET, SO_TYPE, (char*)&socketType, &socketTypeLen) == SOCKET_ERROR) {
            Trc_PRT_sock_j9sock_connect_failure_cause(
                "getsockopt(sock->ipv6,  SOL_SOCKET, SO_TYPE, (char*)&socketType, &socketTypeLen) == SOCKET_ERROR");
            rc = WSAGetLastError();
            Trc_PRT_sock_j9sock_connect_failure(rc);
            rc = omrerror_set_last_error(rc, findError(rc));
            Trc_PRT_sock_j9sock_connect_Exit(rc);
...
retry:
    ...
    /* here we need to do the connect based on the type of addressed passed in as well as the sockets which are open. If
            a socket with a type that matches the type of the address passed in is open then we use that one.  Otherwise
       we use the socket that is open */
    if (((((OSSOCKADDR*)&addr->addr)->sin_family != OS_AF_UNSPEC)
            && ((((OSSOCKADDR*)&addr->addr)->sin_family == OS_AF_INET4) || !(sock->flags & SOCKET_IPV6_OPEN_MASK))
            && (sock->flags & SOCKET_IPV4_OPEN_MASK))) {
        rc = connect(sock->ipv4, (const struct sockaddr FAR*)&addr->addr, sizeof(addr->addr));
        if (rc == SOCKET_ERROR) {
            Trc_PRT_sock_j9sock_connect_failure_cause(" SOCKET_ERROR == connect( sock->ipv4, (const struct sockaddr "
                                                      "FAR*) &addr->addr,  sizeof(addr->addr) )");
            rc = WSAGetLastError();
        }
        ...
        if (sock->flags & SOCKET_IPV4_OPEN_MASK) {
            j9sock_sockaddr_init6(
                portLibrary, addr, (uint8_t*)nAddrBytes, J9SOCK_INADDR_LEN, J9ADDR_FAMILY_AFINET4, 0, 0, 0, sock);
            rc = connect(sock->ipv4, (const struct sockaddr FAR*)&addr->addr, sizeof(addr->addr));
        ...
        if (rc == 0 && sock->flags & SOCKET_IPV6_OPEN_MASK) {
            j9sock_sockaddr_init6(
                portLibrary, addr, (uint8_t*)nAddrBytes, J9SOCK_INADDR_LEN, J9ADDR_FAMILY_AFINET6, 0, 0, 0, sock);
            rc = connect(sock->ipv6, (const struct sockaddr FAR*)&addr->addr, sizeof(addr->addr));
            if (rc == SOCKET_ERROR) {
                Trc_PRT_sock_j9sock_connect_failure_cause("rc == SOCKET_ERROR");
                rc = WSAGetLastError();
    ...
    if (rc != 0) {
        ...
        if (firstAttempt && (rc == WSAENETDOWN || rc == WSAEHOSTUNREACH)) {
            firstAttempt = FALSE;
            if (ensureConnected(portLibrary))
                goto retry;
        }
        firstAttempt = FALSE;
#endif
 ...
 */
const char* j9sock_error_message(struct J9PortLibrary* portLibrary)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
 ...
 */
int32_t j9sock_fdset_init(struct J9PortLibrary* portLibrary, j9socket_t socketP)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    j9fdset_t fdset;
 ...
 */
int32_t j9sock_fdset_size(struct J9PortLibrary* portLibrary, j9socket_t handle) { return 0; }
 ...
 */
int32_t j9sock_freeaddrinfo(struct J9PortLibrary* portLibrary, j9addrinfo_t handle)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
 ...
 * @note Added for IPv6 support.
 */
int32_t j9sock_getaddrinfo(struct J9PortLibrary* portLibrary, char* name, j9addrinfo_t hints, j9addrinfo_t result)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    ...
    if (PPG_sock_IPv6_FUNCTION_SUPPORT) {
        ...
        if (0 != portLibrary->sock_inetaddr(portLibrary, name, &addr)) {
            ...
            if ((0
                    != (rc = portLibrary->sock_gethostbyaddr(
                            portLibrary, (char*)&addr, sizeof(addr), J9SOCK_AFINET, &j9hostent)))
...
/**
 * Answers a uint8_t array representing the address at "index" in the structure returned from @ref j9sock_getaddrinfo,
 * indexed starting at 0. The address is in network byte order.
 *
 * The address will either be 4 or 16 bytes depending on whether it is an OS_AF_INET  address or an OS_AF_INET6 address.
 * You can check this will a call to @ref j9sock_getaddrinfo_family.  Therefore you should either check the family type
 * before preallocating the "address" or define it as 16 bytes.
 *
 * @param[in]   portLibrary The port library.
 ...
 * @note Added for IPv6 support.
 */
int32_t j9sock_getaddrinfo_address(
 ...
 * @note Added for IPv6 support.
 */
int32_t j9sock_getaddrinfo_create_hints(struct J9PortLibrary* portLibrary, j9addrinfo_t* result, int16_t family,
...
/**
 * Answers the family type of the address at "index" in the structure returned from @ref j9sock_getaddrinfo, indexed
 * starting at 0.
 *
 ...
 * @note Added for IPv6 support.
 */
int32_t j9sock_getaddrinfo_family(struct J9PortLibrary* portLibrary, j9addrinfo_t handle, int32_t* family, int index)
{
    int32_t rc = 0;
    OSADDRINFO* addr;
    int i;
 ...
 * @note Added for IPv6 support.
 */
int32_t j9sock_getaddrinfo_length(struct J9PortLibrary* portLibrary, j9addrinfo_t handle, int32_t* length)
{
...
/**
 * Answers the cannon name of the address at "index" in the structure returned from @ref j9sock_getaddrinfo, indexed
 * starting at 0.
 *
 ...
 * @note Added for IPv6 support.
 */
int32_t j9sock_getaddrinfo_name(struct J9PortLibrary* portLibrary, j9addrinfo_t handle, char* name, int index)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    int32_t rc = 0;
    char** alias_list;
 ...
 * @return	0, if no errors occurred, otherwise the (negative) error code.
 */
int32_t j9sock_gethostbyname(struct J9PortLibrary* portLibrary, const char* name, j9hostent_t handle)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    OSHOSTENT* result;
#if defined(J9_USE_CONNECTION_MANAGER)
 ...
 * @return	0, if no errors occurred, otherwise the (negative) error code
 */
int32_t j9sock_gethostname(struct J9PortLibrary* portLibrary, char* buffer, int32_t length)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
 ...
 */
int32_t j9sock_getnameinfo(struct J9PortLibrary* portLibrary, j9sockaddr_t in_addr, int32_t sockaddr_size, char* name,
    int32_t name_length, int flags)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    OSHOSTENT* result;
    OSSOCKADDR* addr;
    ...
    if (PPG_sock_IPv6_FUNCTION_SUPPORT) {
        ...
        /* Windows code requires that the sockaddr structure be of the right type, rather than just checking to see if
         * it is large enough */
 ...
 * @return	0, if no errors occurred, otherwise the (negative) error code.
 */
int32_t j9sock_getopt_sockaddr(
    struct J9PortLibrary* portLibrary, j9socket_t socketP, int32_t optlevel, int32_t optname, j9sockaddr_t optval)
{
    ...
    if (socketP->flags & SOCKET_IPV4_OPEN_MASK) {
        rc = getsockopt(
            socketP->ipv4, platformLevel, platformOption, (char*)&((OSSOCKADDR*)&optval->addr)->sin_addr, &optlen);
    } else if (socketP->flags & SOCKET_IPV6_OPEN_MASK) {
        rc = getsockopt(
            socketP->ipv6, platformLevel, platformOption, (char*)&((OSSOCKADDR*)&optval->addr)->sin_addr, &optlen);
    }
 ...
 * @return	0, if no errors occurred, otherwise the (negative) error code.
 */
int32_t j9sock_getpeername(struct J9PortLibrary* portLibrary, j9socket_t handle, j9sockaddr_t addrHandle)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    int32_t rc = 0;
    int32_t addrlen = sizeof(addrHandle->addr);
 ...
 * @return	0, if no errors occurred, otherwise the (negative) error code.
 */
int32_t j9sock_getsockname(struct J9PortLibrary* portLibrary, j9socket_t handle, j9sockaddr_t addrHandle)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    int32_t rc = 0;
    int32_t addrlen = sizeof(addrHandle->addr);
    ...
    /* if both sockets are open we cannot retun the address for either one as whichever one we return it is wrong in
   some cases. Therefore,  we reset the address to the ANY address and leave the port as is as it should be the same
 ...
 */
int32_t j9sock_hostent_addrlist(struct J9PortLibrary* portLibrary, j9hostent_t handle, uint32_t index)
{
 ...
 * @return	0, if no errors occurred, otherwise the (negative) error code
 */
int32_t j9sock_hostent_aliaslist(struct J9PortLibrary* portLibrary, j9hostent_t handle, char*** aliasList)
{
 ...
 */
int32_t j9sock_hostent_hostname(struct J9PortLibrary* portLibrary, j9hostent_t handle, char** hostName)
{
 ...
 */
int32_t j9sock_htonl(struct J9PortLibrary* portLibrary, int32_t val) { return htonl(val); }
 ...
 */
uint16_t j9sock_htons(struct J9PortLibrary* portLibrary, uint16_t val) { return htons(val); }
 ...
 * @return	0, if no errors occurred, otherwise the (negative) error code.
 */
int32_t j9sock_inetaddr(struct J9PortLibrary* portLibrary, const char* addrStr, uint32_t* addr)
{
    int32_t rc = 0;
    uint32_t val;
 ...
 * @return	0, if no errors occurred, otherwise the (negative) error code.
 */
int32_t j9sock_inetntoa(struct J9PortLibrary* portLibrary, char** addrStr, uint32_t nipAddr)
{
    int32_t rc = 0;
    char* val;
    struct in_addr addr;
 ...
 */
int32_t j9sock_linger_enabled(struct J9PortLibrary* portLibrary, j9linger_t handle, BOOLEAN* enabled)
{
 ...
 * @return	0, if no errors occurred, otherwise the (negative) error code.
 */
int32_t j9sock_linger_init(struct J9PortLibrary* portLibrary, j9linger_t handle, int32_t enabled, uint16_t timeout)
{
 ...
 * @return	0, the function does not validate the access.
 */
int32_t j9sock_linger_linger(struct J9PortLibrary* portLibrary, j9linger_t handle, uint16_t* linger)
{
 ...
 */
int32_t j9sock_listen(struct J9PortLibrary* portLibrary, j9socket_t sock, int32_t backlog)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    int32_t rc = 0;
 ...
 */
int32_t j9sock_ntohl(struct J9PortLibrary* portLibrary, int32_t val) { return ntohl(val); }
 ...
 */
uint16_t j9sock_ntohs(struct J9PortLibrary* portLibrary, uint16_t val) { return ntohs(val); }

int32_t j9sock_read(struct J9PortLibrary* portLibrary, j9socket_t sock, uint8_t* buf, int32_t nbyte, int32_t flags)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    int32_t rc = 0;
    int32_t bytesRec = 0;
 ...
 */
int32_t j9sock_readfrom(struct J9PortLibrary* portLibrary, j9socket_t sock, uint8_t* buf, int32_t nbyte, int32_t flags,
    j9sockaddr_t addrHandle)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    int32_t rc = 0;
    int32_t bytesRec = 0;
 ...
 *
 * @param[in] portLibrary The port library.
 * @param[in] nfds For all the sockets that are set in readfds, writefds and exceptfds, this is the maximum value
 * returned by @ref j9sock_fdset_size().
 * @param[in] readfd The j9fdset_t representing the descriptor to be checked to see if data can be read without
 * blocking.
 * @param[in] writefd The j9fdset_t representing the descriptor to be checked to see if data can be written without
 * blocking.
 ...
 */
int32_t j9sock_select(struct J9PortLibrary* portLibrary, int32_t nfds, j9fdset_t readfd, j9fdset_t writefd,
    j9fdset_t exceptfd_notSupported, j9timeval_t timeout)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    int32_t rc = 0;
    int32_t result = 0;

    Trc_PRT_sock_j9sock_select_Entry(nfds, readfd, writefd, exceptfd_notSupported,
        timeout == NULL ? 0 : timeout->time.tv_sec, timeout == NULL ? 0 : timeout->time.tv_usec);

    if (NULL != exceptfd_notSupported) {
        if (0 != &exceptfd_notSupported->handle.fd_count) {
            rc = omrerror_set_last_error_with_message(
                J9PORT_ERROR_SOCKET_ARGSINVALID, "exceptfd_notSupported cannot contain a valid fd");
...
int32_t j9sock_select_read(
    struct J9PortLibrary* portLibrary, j9socket_t j9socketP, int32_t secTime, int32_t uSecTime, BOOLEAN isNonBlocking)
{
    ...
    if (0 == secTime && 0 == uSecTime) {
        ...
        if (isNonBlocking) {
            ...
            if (((j9socketP->flags & SOCKET_IPV4_OPEN_MASK) != 0)
                && ((j9socketP->flags & SOCKET_IPV6_OPEN_MASK) == 0)) {
                ...
                return 1;
            } else if (((j9socketP->flags & SOCKET_IPV6_OPEN_MASK) != 0)
                && ((j9socketP->flags & SOCKET_IPV4_OPEN_MASK) == 0)) {
                j9socketP->flags = j9socketP->flags & ~SOCKET_USE_IPV4_MASK;
                /* return value of 1 means there is data to be read */
                Trc_PRT_sock_j9sock_select_read_Exit(1);
                return 1;
    ...
    for (;;) {
        ...
        /* break out of the loop if we should not be looping (timeout is zero) or if an error occured or data ready to
         * be read */
...
int32_t j9sock_set_nonblocking(struct J9PortLibrary* portLibrary, j9socket_t socketP, BOOLEAN nonblocking)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    int32_t rc = 0;
 ...
 */
int32_t j9sock_setflag(struct J9PortLibrary* portLibrary, int32_t flag, int32_t* arg)
{
    int32_t rc = 0;
 ...
 * @note Added for IPv6 support.
 */
int32_t j9sock_setopt_ipv6_mreq(
 ...
 * @return	0, if no errors occurred, otherwise the (negative) error code.
 */
int32_t j9sock_setopt_sockaddr(
    struct J9PortLibrary* portLibrary, j9socket_t socketP, int32_t optlevel, int32_t optname, j9sockaddr_t optval)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    int32_t rc = 0;
 ...
 * @return	0, if no errors occurred, otherwise the (negative) error code
 */
int32_t j9sock_shutdown(struct J9PortLibrary* portLibrary)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    int32_t result = 0;
 ...
 */
int32_t j9sock_shutdown_input(struct J9PortLibrary* portLibrary, j9socket_t sock)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    int32_t rc = 0;
 ...
 * \arg J9PORT_ERROR_SOCKET_NOTINITIALIZED, if the library is not initialized
 */
int32_t j9sock_shutdown_output(struct J9PortLibrary* portLibrary, j9socket_t sock)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    int32_t rc = 0;
 ...
 * @return	0, if no errors occurred, otherwise the (negative) error code.
 */
int32_t j9sock_sockaddr(struct J9PortLibrary* portLibrary, j9sockaddr_t handle, const char* addrStr, uint16_t port)
{
    int32_t rc = 0;
    uint32_t addr = 0;
 ...
 */
int32_t j9sock_sockaddr_address(struct J9PortLibrary* portLibrary, j9sockaddr_t handle)
{
 ...
 * @note Added for IPv6 support.
 */
int32_t j9sock_sockaddr_address6(
 ...
 * @note Added for IPv6 support.
 */
int32_t j9sock_sockaddr_family(struct J9PortLibrary* portLibrary, int16_t* family, j9sockaddr_t handle)
{
    OSSOCKADDR* ipv4;
...
int32_t j9sock_sockaddr_init(
    struct J9PortLibrary* portLibrary, j9sockaddr_t handle, int16_t family, uint32_t nipAddr, uint16_t portNetworkOrder)
{
    OSSOCKADDR* sockaddr;
...
int32_t j9sock_sockaddr_init6(struct J9PortLibrary* portLibrary, j9sockaddr_t handle, uint8_t* addr, int32_t addrlength,
    int16_t family, uint16_t nPort, uint32_t flowinfo, uint32_t scope_id, j9socket_t sock)
{
    OSSOCKADDR* sockaddr;
 ...
 */
uint16_t j9sock_sockaddr_port(struct J9PortLibrary* portLibrary, j9sockaddr_t handle)
{
    ...
    struct J9PortLibrary* portLibrary, j9socket_t* handle, int32_t family, int32_t socktype, int32_t protocol)
{
    ...
    *handle = (j9socket_t)-1;
    ...
    if (family != J9ADDR_FAMILY_AFINET6 && family != J9ADDR_FAMILY_AFINET4 && family != J9ADDR_FAMILY_UNSPEC) {
        Trc_PRT_sock_j9sock_socket_failure_cause(
            "family != J9ADDR_FAMILY_AFINET6 && family != J9ADDR_FAMILY_AFINET4 && family != J9ADDR_FAMILY_UNSPEC");
 ...
 */
int32_t j9sock_socketIsValid(struct J9PortLibrary* portLibrary, j9socket_t handle)
{
 ...
 */
int32_t j9sock_startup(struct J9PortLibrary* portLibrary)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    int32_t rc = 0;
 ...
 * @return	0, if no errors occurred, otherwise the (negative) error code.
 */
int32_t j9sock_timeval_init(struct J9PortLibrary* portLibrary, uint32_t secTime, uint32_t uSecTime, j9timeval_t timeP)
{
 ...
 */
int32_t j9sock_write(struct J9PortLibrary* portLibrary, j9socket_t sock, uint8_t* buf, int32_t nbyte, int32_t flags)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    int32_t rc = 0;
    int32_t bytesSent = 0;
 ...
 */
int32_t j9sock_writeto(struct J9PortLibrary* portLibrary, j9socket_t sock, uint8_t* buf, int32_t nbyte, int32_t flags,
    j9sockaddr_t addrHandle)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    int32_t bytesSent = 0;
...
retry:
    ...
    if (((((OSSOCKADDR*)&addrHandle->addr)->sin_family == OS_AF_INET4) && (sock->flags & SOCKET_IPV4_OPEN_MASK))
        ...
        bytesSent = sendto(sock->ipv4, (char*)buf, nbyte, flags, (const struct sockaddr*)&(addrHandle->addr),
            sizeof(addrHandle->addr));
    } else {
        bytesSent = sendto(sock->ipv6, (char*)buf, nbyte, flags, (const struct sockaddr*)&(addrHandle->addr),
            sizeof(addrHandle->addr));
    }
...
static int32_t addAdapterIpv6(struct J9PortLibrary* portLibrary, struct j9NetworkInterface_struct* interfaces,
    uint32_t currentAdapterIndex, IP_ADAPTER_ADDRESSES* currentAdapter, IP_ADAPTER_ADDRESSES* adaptersList,
    char* baseName, uintptr_t adapterIndex)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    ...
    nameLength = WideCharToMultiByte(
        OS_ENCODING_CODE_PAGE, OS_ENCODING_WC_FLAGS, currentAdapter->Description, -1, NULL, 0, NULL, NULL);

    if (0 != nameLength) {
        interfaces[currentAdapterIndex].displayName
            = omrmem_allocate_memory(nameLength + 1, OMRMEM_CATEGORY_PORT_LIBRARY);
    } else {
...
#endif
    if (nameLength != 0) {
        WideCharToMultiByte(OS_ENCODING_CODE_PAGE, OS_ENCODING_WC_FLAGS, currentAdapter->Description, -1,
            interfaces[currentAdapterIndex].displayName, (int)nameLength, NULL, NULL);
        interfaces[currentAdapterIndex].displayName[nameLength] = 0;
    }

    /* now get the interface information */

    /* first count the number of IP addreses and allocate the memory required for the ip address info that will be
     * returned*/
    numAddresses = 0;
    ...
    if ((NULL != currentAdapter->FirstUnicastAddress)
        ...
        && (127
               == ((struct sockaddr_in*)(currentAdapter->FirstUnicastAddress->Address.lpSockaddr))
    ...
    interfaces[currentAdapterIndex].addresses
        = omrmem_allocate_memory(numAddresses * sizeof(j9ipAddress_struct), OMRMEM_CATEGORY_PORT_LIBRARY);
#if (defined(VALIDATE_ALLOCATIONS))
    ...
    while (currentIPAddress) {
        if (currentIPAddress->Address.iSockaddrLength == sizeof(struct sockaddr_in6)) {
            ...
            interfaces[currentAdapterIndex].addresses[currentIPAddressIndex].scope
                = ((struct sockaddr_in6*)currentIPAddress->Address.lpSockaddr)->sin6_scope_id;
...
static int32_t addAdapterIpv4(struct J9PortLibrary* portLibrary, struct j9NetworkInterface_struct* interfaces,
    uint32_t currentAdapterIndex, IP_ADAPTER_INFO* currentAdapter, IP_ADAPTER_INFO* adaptersList, char* baseName,
    uintptr_t adapterIndex)
{
    OMRPORT_ACCESS_FROM_J9PORT(portLibrary);
    uintptr_t nameLength;
    uint32_t numAddresses = 0;
 ...
 * @return	0 on success, negative portable error code on failure.
                        J9PORT_ERROR_SOCKET_NORECOVERY if system calls required to get the info fail,
 J9PORT_ERROR_SOCKET_NOBUFFERS if memory allocation fails
 */
int32_t j9sock_get_network_interfaces(
 ...
 * @return 0 on success
 */
int32_t j9sock_free_network_interface_struct(
...
/**
 * Establish a connection to a peer with a timeout.  This function is called repeatedly in order to carry out the
 * connect and to allow other tasks to proceed on certain platforms.  The caller must first call with step =
 * J9_SOCK_STEP_START, if the result is J9_ERROR_SOCKET_NOTCONNECTED it will then call it with step = CHECK until either
 * another error or 0 is returned to indicate the connect is complete.  Each time the function should sleep for no more
 * than timeout milliseconds.  If the connect succeeds or an error occurs, the caller must always end the process by
 * calling the function with step = J9_SOCK_STEP_DONE
 *
 * @param[in] portLibrary The port library.
 ...
 * @return	0, if no errors occurred, otherwise the (negative) error code.
 */
int32_t j9sock_connect_with_timeout(struct J9PortLibrary* portLibrary, j9socket_t sock, j9sockaddr_t addr,
...
void j9sock_fdset_zero(struct J9PortLibrary* portLibrary, j9fdset_t j9fdset)
{
...
/* Note: j9fdset_t does not support multiple entries. */
void j9sock_fdset_set(struct J9PortLibrary* portLibrary, j9socket_t aSocket, j9fdset_t j9fdset)
{
...
/* Note: j9fdset_t does not support multiple entries. */
void j9sock_fdset_clr(struct J9PortLibrary* portLibrary, j9socket_t aSocket, j9fdset_t j9fdset)
{
