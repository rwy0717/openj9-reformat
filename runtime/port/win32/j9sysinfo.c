 ...
 */
uintptr_t j9sysinfo_DLPAR_enabled(struct J9PortLibrary* portLibrary) { return FALSE; }
 ...
 * j9sysinfo_startup should be destroyed here.
 *
 * @param[in] portLibrary The port library.
 *
 ...
 */
static CACHE_DESCRIPTOR const* findCacheForTypeAndLevel(struct J9PortLibrary* portLibrary,
    SYSTEM_LOGICAL_PROCESSOR_INFORMATION* procInfoPtr, uint32_t procInfoLength, int32_t const cacheLevel,
    const int32_t cacheType)
{
    SYSTEM_LOGICAL_PROCESSOR_INFORMATION* cursor = procInfoPtr;
    CACHE_DESCRIPTOR* cacheDesc = NULL;
    SYSTEM_LOGICAL_PROCESSOR_INFORMATION* endInfo
        = (SYSTEM_LOGICAL_PROCESSOR_INFORMATION*)(((U_8*)procInfoPtr) + procInfoLength);
        ...
        if ((temp->Level == cacheLevel) && (temp->LineSize > 0)) {
            ...
            case CacheUnified: {
                if (J9_ARE_ANY_BITS_SET(
                        cacheType, J9PORT_CACHEINFO_DCACHE | J9PORT_CACHEINFO_ICACHE | J9PORT_CACHEINFO_UCACHE)) {
                    cacheDesc = temp;
                }
                break;
 ...
 */
static uint32_t getCacheTypes(struct J9PortLibrary* portLibrary, SYSTEM_LOGICAL_PROCESSOR_INFORMATION* procInfoPtr,
 ...
 */
static uint32_t getCacheLevels(
    struct J9PortLibrary* portLibrary, SYSTEM_LOGICAL_PROCESSOR_INFORMATION* procInfoPtr, uint32_t procInfoLength)
{
    SYSTEM_LOGICAL_PROCESSOR_INFORMATION* cursor = procInfoPtr;
    uint32_t result = 0;
    SYSTEM_LOGICAL_PROCESSOR_INFORMATION* endInfo
...
int32_t j9sysinfo_get_cache_info(struct J9PortLibrary* portLibrary, const J9CacheInfoQuery* query)
{
    ...
    case J9PORT_CACHEINFO_QUERY_LINESIZE: {
        CACHE_DESCRIPTOR const* cacheDesc
            = findCacheForTypeAndLevel(portLibrary, procInfoPtr, procInfoLength, query->level, query->cacheType);
        if (NULL != cacheDesc) {
