...
{
    if (fileName[0] != '-') {
        _FileHandle = j9cached_file_open(
            _PortLibrary, fileName, EsOpenWrite | EsOpenCreate | EsOpenTruncate | EsOpenCreateNoTag, 0666);
        _Error = 0;
    }
}
...
bool FileStream::isOpen(void) const { return _FileHandle != -1; }

bool FileStream::hasError(void) const { return _Error != 0; }

/* Method for writing characters described by a pointer and a length to the file*/
void FileStream::writeCharacters(const char* data, IDATA length)
{
...
void FileStream::writeCharacters(const char* data) { writeCharacters(data, strlen(data)); }

/* Method for writing a number to the file */
void FileStream::writeNumber(IDATA data, int length)
{
