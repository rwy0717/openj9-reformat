...
TextFileStream::TextFileStream(J9PortLibrary* portLibrary)
    ...
    , _Error(false)
{
    _Buffer = (char*)OMRPORT_FROM_J9PORT(_PortLibrary)
                  ->mem_allocate_memory(OMRPORT_FROM_J9PORT(_PortLibrary), _BufferSize,
                      "TextFileStream::TextFileStream", OMRMEM_CATEGORY_VM);
    if (_Buffer == NULL) {
        _BufferSize = 0;
    }
}
...
/* Destructor */
TextFileStream::~TextFileStream() { close(); }

/* Method for opening the file */
void TextFileStream::open(const char* fileName, bool cacheWrites)
{
...
bool TextFileStream::isOpen(void) const { return _FileHandle != -1; }

bool TextFileStream::isError(void) const { return _Error; }

/* Method for writing characters described by a pointer and a length to the file*/
void TextFileStream::writeCharacters(const char* data, IDATA length)
{
    PORT_ACCESS_FROM_PORT(_PortLibrary);
    ...
    /* we are now in a cached state.  3 possible cases
            ...
            3) big overflow - finish this buffer, write it out, then write out the remaining data all at once, finally
       leaving _Buffer empty
    */
...
void TextFileStream::writeCharacters(const J9UTF8* data)
{
...
void TextFileStream::writeCharacters(const char* data) { writeCharacters(data, strlen(data)); }

/* Method for writing a pointer as characters to the file*/
void TextFileStream::writePointer(const void* data, bool printPrefix)
{
...
void TextFileStream::writeInteger(UDATA data, const char* format)
{
    char buffer[32];
    PORT_ACCESS_FROM_PORT(_PortLibrary);
    UDATA length;
...
/* Method for writing a number as characters to the file*/
void TextFileStream::writeInteger64(U_64 data, const char* format)
{
    char buffer[32];
    PORT_ACCESS_FROM_PORT(_PortLibrary);
    UDATA length;
...
void TextFileStream::writeVPrintf(const char* format, ...)
{
...
void TextFileStream::writeIntegerWithCommas(U_64 value)
{
