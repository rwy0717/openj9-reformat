...
static const J9RASdumpSpec rasDumpSpecs[]
    = { { "console", "Basic thread dump to stderr", "file=", NULL, "Output file", doConsoleDump,
            { J9RAS_DUMP_ON_GP_FAULT | J9RAS_DUMP_ON_USER_SIGNAL, NULL, 1, 0, NULL, NULL, 5,
                J9RAS_DUMP_DO_EXCLUSIVE_VM_ACCESS, NULL } },
          { "stack", "Print thread information to stderr", "file=", NULL, "Output file", doStackDump,
              { J9RAS_DUMP_ON_GP_FAULT | J9RAS_DUMP_ON_USER_SIGNAL, NULL, 1, 0, NULL, NULL, 5,
                  J9RAS_DUMP_DO_EXCLUSIVE_VM_ACCESS, NULL } },
          { "system", "Capture raw process image",
#if defined(J9ZOS390)
              "dsn=", "JAVA_DUMP_TDUMP_PATTERN", " Output name",
#else
              "file=", "IBM_COREDIR", "Output file",
#endif
              doSystemDump,
              { J9RAS_DUMP_ON_GP_FAULT | J9RAS_DUMP_ON_ABORT_SIGNAL | J9RAS_DUMP_ON_TRACE_ASSERT
                      | J9RAS_DUMP_ON_CORRUPT_CACHE,
                  NULL, 1, 0,
#if defined(J9ZOS390)
#if defined(J9ZOS39064)
                  "%uid.JVM.%job.D%y%m%d.T%H"
                  "%M"
                  "%S.X&DS",
                  NULL,
#else
                  "%uid.JVM.TDUMP.%job.D%y%m%d.T%H"
                  "%M"
                  "%S",
                  NULL,
#endif
                  ...
                  NULL,
#endif
                  999, J9RAS_DUMP_DO_SUSPEND_OTHER_DUMPS, NULL } },
          { "tool", "Run command line program", "exec=", "JAVA_DUMP_TOOL",
              "Command line (special characters should be escaped)", doToolDump,
              { J9RAS_DUMP_ON_GP_FAULT | J9RAS_DUMP_ON_USER_SIGNAL, NULL, 1, 1,
#if defined(WIN32)
                  ...
                  NULL,
#endif /* defined(WIN32) */
                  NULL, 0, J9RAS_DUMP_DO_SUSPEND_OTHER_DUMPS, NULL } },
          { "java", "Write application summary", "file=",
#if defined(J9ZOS390)
              "_CEE_DMPTARG",
#else
              "IBM_JAVACOREDIR",
#endif
              "Output file", doJavaDump,
              { J9RAS_DUMP_ON_GP_FAULT | J9RAS_DUMP_ON_USER_SIGNAL | J9RAS_DUMP_ON_ABORT_SIGNAL
                  ...
                  ,
                  NULL } },
          { "heap", "Capture raw heap image", "file=",
#if defined(J9ZOS390)
              "_CEE_DMPTARG",
#else
              "IBM_HEAPDUMPDIR",
#endif
              "Output file", doHeapDump,
              { J9RAS_DUMP_ON_EXCEPTION_SYSTHROW, "java/lang/OutOfMemoryError", 1, 4,
                  "heapdump.%Y"
                  "%m%d.%H"
                  "%M"
                  "%S.%pid.%seq.phd",
                  ...
                  NULL } },
          { "snap", "Take a snap of the trace buffers", "file=",
#if defined(J9ZOS390)
              "_CEE_DMPTARG",
#else
              "IBM_COREDIR",
#endif
              "Output file", doSnapDump,
              { J9RAS_DUMP_ON_GP_FAULT | J9RAS_DUMP_ON_ABORT_SIGNAL | J9RAS_DUMP_ON_TRACE_ASSERT
                      | J9RAS_DUMP_ON_CORRUPT_CACHE,
                  NULL, 1, 0,
                  "Snap.%Y"
                  ...
                  "%M"
                  "%S.%pid.%seq.trc",
                  NULL, 300, J9RAS_DUMP_DO_SUSPEND_OTHER_DUMPS, NULL } },
#if defined(J9ZOS390)
          { "ceedump", "Take standard LE dump", "dsn=", "JAVA_DUMP_TDUMP_PATTERN", " Output name", doCEEDump,
              { J9RAS_DUMP_ON_GP_FAULT | J9RAS_DUMP_ON_USER_SIGNAL, NULL, 1, 0,
                  "%uid.JVM.CEEDUMP.%job.D%y%m%d.T%H"
                  "%M"
                  "%S",
                  NULL, 888, J9RAS_DUMP_DO_SUSPEND_OTHER_DUMPS, NULL } },
#endif
          { "jit", "Capture JIT diagnostic information", "file=",
#if defined(J9ZOS390)
              "_CEE_DMPTARG",
#else
              "IBM_COREDIR",
#endif
              "Output file", doJitDump,
              { J9RAS_DUMP_ON_GP_FAULT | J9RAS_DUMP_ON_ABORT_SIGNAL, NULL, 1, 0,
                  "jitdump.%Y"
                  "%m%d.%H"
                  "%M"
                  "%S.%pid.%seq.dmp",
                  NULL, 200, J9RAS_DUMP_DO_SUSPEND_OTHER_DUMPS, NULL } },
          { "silent", "Dummy dump agent which does nothing", "file=", NULL, "Output file", doSilentDump,
              { 0, NULL, 1, 0, NULL, NULL, 5, 0, NULL } } };
...
static omr_error_t makePath(J9JavaVM* vm, char* label)
{
    ...
    if (fd == -1) {
        ...
        if (dumpGlobal->noFailover) {
            j9nls_printf(
                PORTLIB, J9NLS_ERROR, J9NLS_DMP_MAKEPATH_CANNOT_USE_DMP_LABEL, label, j9error_last_error_message());
            return OMR_ERROR_INTERNAL;
        }

        /* Issue warning message and carry on to try the failover locations */
        j9nls_printf(
            PORTLIB, J9NLS_WARNING, J9NLS_DMP_MAKEPATH_CANNOT_USE_DMP_LABEL, label, j9error_last_error_message());
...
static UDATA protectedUpdateJ9RAS(struct J9PortLibrary* portLibrary, void* userData)
{
...
omr_error_t doSystemDump(J9RASdumpAgent* agent, char* label, J9RASdumpContext* context)
{
    ...
    if (*label != '-') {
        ...
        j9sig_protect(protectedUpdateJ9RAS, vm, signalHandler, NULL,
            J9PORT_SIG_FLAG_MAY_RETURN | J9PORT_SIG_FLAG_SIGALLSYNC, &retVal);
        ...
        if (retVal == 0) {
            if (label[0] != '\0') {
                ...
                j9nls_printf(PORTLIB, J9NLS_INFO | J9NLS_STDERR, J9NLS_DMP_WRITTEN_DUMP_STR, "System",
                    "{unable to determine dump name}");
...
omr_error_t doToolDump(J9RASdumpAgent* agent, char* label, J9RASdumpContext* context)
{
    ...
    if (*label != '-') {
#if defined(WIN32)
        {
            ...
            MultiByteToWideChar(
                OS_ENCODING_CODE_PAGE, OS_ENCODING_MB_FLAGS, label, -1, unicodePath, (int)strlen(label) + 1);
            ...
            if (retVal) {
                ...
                j9nls_printf(PORTLIB, J9NLS_ERROR | J9NLS_STDERR, J9NLS_DMP_ERROR_IN_DUMP_STR_RC, "Tool",
                    "CreateProcessW()", GetLastError());
            }
...
#elif (defined(LINUX) && !defined(J9ZTPF)) || defined(AIXPPC) || defined(OSX) /* defined(WIN32) */
        {
            ...
            if ((retVal = fork()) == 0) {
                ...
                j9nls_printf(
                    PORTLIB, J9NLS_ERROR | J9NLS_STDERR, J9NLS_DMP_ERROR_IN_DUMP_STR_RC, "Tool", "execl()", errno);
                exit((int)retVal);
...
#elif defined(J9ZOS390) /* defined(WIN32) */
        {
            ...
            if (retVal == -1) {
                j9nls_printf(
                    PORTLIB, J9NLS_ERROR | J9NLS_STDERR, J9NLS_DMP_ERROR_IN_DUMP_STR_RC, "Tool", "spawnp()", errno);
            } else {
                j9nls_printf(PORTLIB, J9NLS_INFO | J9NLS_STDERR, J9NLS_DMP_SPAWNED_DUMP_STR, "Tool", retVal);
...
static omr_error_t doSnapDump(J9RASdumpAgent* agent, char* label, J9RASdumpContext* context)
{
    ...
    if (uteInterface && uteInterface->server) {
        ...
        /* USER_REQUEST is synchronous to prevent Dump API calls returning before the file is written. (USER_SIGNAL is
         * kill -3) J9RAS_DUMP_ON_GP_FAULT, J9RAS_DUMP_ON_ABORT_SIGNAL and J9RAS_DUMP_ON_TRACE_ASSERT are synchronous to
         * prevent us starting a thread under those conditions.
         */
        I_32 sync = (context->eventFlags
                        & (J9RAS_DUMP_ON_USER_REQUEST | J9RAS_DUMP_ON_GP_FAULT | J9RAS_DUMP_ON_ABORT_SIGNAL
        ...
        result = uteInterface->server->TraceSnapWithPriority(
            (context->onThread ? UT_THREAD_FROM_VM_THREAD(context->onThread) : NULL), label, J9THREAD_PRIORITY_MAX,
            &snapResponse, sync);
    ...
    if (*label != '-') {
        ...
        if (retVal == 0) {
            if (label[0] != '\0') {
                ...
                j9nls_printf(PORTLIB, J9NLS_INFO | J9NLS_STDERR, J9NLS_DMP_WRITTEN_DUMP_STR, "CEEDUMP",
                    "{unable to determine dump name}");
...
static char* allocString(J9JavaVM* vm, UDATA numBytes)
{
    ...
    if (rasDumpStrings.table && rasDumpStrings.length >= rasDumpStrings.capacity) {
        ...
        rasDumpStrings.table = (char**)j9mem_reallocate_memory(
            rasDumpStrings.table, rasDumpStrings.capacity * sizeof(char*), OMRMEM_CATEGORY_VM);
    }
    ...
    while (sign == '+' || sign == '-') {
        ...
        for (i = 0; i < J9RAS_DUMP_KNOWN_EVENTS; i++) {
            if (try_scan(cursor, rasDumpEvents[i].name)) {
                mask = (sign == '+') ? (mask | rasDumpEvents[i].bits) : (mask & ~rasDumpEvents[i].bits);
    ...
    while (sign == '+' || sign == '-') {
        ...
        for (i = 0; i < J9RAS_DUMP_KNOWN_REQUESTS; i++) {
            if (try_scan(cursor, rasDumpRequests[i].name)) {
                mask = (sign == '+') ? (mask | rasDumpRequests[i].bits) : (mask & ~rasDumpRequests[i].bits);
    ...
    if (strcmp(spec->labelTag, "file=") == 0) {
        ...
        if (path
            && ((strncmp(path, "%home", strlen("%home")) == 0)
                   || (strncmp(path, "%tenantwd", strlen("%tenantwd")) == 0))) {
            /* This path does not need fixing. */
#ifdef WIN32
        } else if (path && path[0] != '\0' && path[0] != '-' && path[0] != DIR_SEPARATOR && path[0] != ALT_DIR_SEPARATOR
            ...
            if (dumpDirectoryPrefix != NULL) {
            ...
            } else if ((spec->labelHint != NULL)
                && (j9sysinfo_get_env(spec->labelHint, prefix, J9_MAX_DUMP_PATH) == 0)) {
                ...
                if (ok) {
                    WideCharToMultiByte(OS_ENCODING_CODE_PAGE, OS_ENCODING_WC_FLAGS, unicodeTemp, -1, prefix,
                        J9_MAX_DUMP_PATH, NULL, NULL);
                }
...
static UDATA processSettings(J9JavaVM* vm, IDATA kind, char* optionString, J9RASdumpSettings* settings)
{
    ...
    if (action != REMOVE_DUMP_AGENTS && (settings->eventMask & J9RAS_DUMP_ON_OBJECT_ALLOCATION)
    ...
    if (action != REMOVE_DUMP_AGENTS && (0 == (settings->eventMask & J9RAS_DUMP_EXCEPTION_EVENT_GROUP))
...
static J9RASdumpAgent* findAgent(J9JavaVM* vm, IDATA kind, const J9RASdumpSettings* settings)
{
    ...
    while (seekDumpAgent(vm, &agent, dumpFn) == OMR_ERROR_NONE) {
        ...
        if (((agent->detailFilter != NULL) && (settings->detailFilter == NULL))
            ...
            || ((agent->detailFilter != NULL) && (settings->detailFilter != NULL)
                   && (strcmp(agent->detailFilter, settings->detailFilter) != 0))) {
            continue;
        }
        ...
        if (((agent->subFilter != NULL) && (settings->subFilter == NULL))
            ...
            || ((agent->subFilter != NULL) && (settings->subFilter != NULL)
                   && (strcmp(agent->subFilter, settings->subFilter) != 0))) {
            continue;
        }
        ...
        if (((agent->labelTemplate != NULL) && (settings->labelTemplate == NULL))
            ...
            || ((agent->labelTemplate != NULL) && (settings->labelTemplate != NULL)
                   && (strcmp(agent->labelTemplate, settings->labelTemplate) != 0))) {
            continue;
        }
        ...
        if (((agent->dumpOptions != NULL) && (settings->dumpOptions == NULL))
            ...
            || ((agent->dumpOptions != NULL) && (settings->dumpOptions != NULL)
                   && (strcmp(agent->dumpOptions, settings->dumpOptions) != 0))) {
            continue;
        }
...
static J9RASdumpAgent* findAgentToDelete(
    J9JavaVM* vm, IDATA kind, J9RASdumpAgent* agent, const J9RASdumpSettings* settings)
{
    J9RASdumpFn dumpFn = rasDumpSpecs[kind].dumpFn;
    ...
    while (seekDumpAgent(vm, &agent, dumpFn) == OMR_ERROR_NONE) {
        ...
        if ((settings->eventMask == 0) || (settings->eventMask & agent->eventMask) == 0) {
            continue;
        }

        /* If detail filter is set, it needs to match. */
        if (((settings->detailFilter != NULL) && (agent->detailFilter == NULL))
            || ((settings->detailFilter != NULL) && (agent->detailFilter != NULL)
        ...
        if (((settings->subFilter != NULL) && (agent->subFilter == NULL))
            || ((settings->subFilter != NULL) && (agent->subFilter != NULL)
        ...
        if (((settings->dumpOptions != NULL) && (agent->dumpOptions == NULL))
            || ((settings->dumpOptions != NULL) && (agent->dumpOptions != NULL)
        ...
        if (((settings->labelTemplate != NULL) && (agent->labelTemplate == NULL))
            || ((settings->labelTemplate != NULL) && (agent->labelTemplate != NULL)
...
#if (defined(J9VM_RAS_DUMP_AGENTS))
omr_error_t printDumpSpec(struct J9JavaVM* vm, IDATA kind, IDATA verboseLevel)
{
    if (kind < J9RAS_DUMP_KNOWN_SPECS) {
        ...
        if (verboseLevel > 0) {
            ...
            j9tty_err_printf(PORTLIB,
                ...
                "  range=%d..%d\n"
                "  priority=%d\n",
                tmpSettings.detailFilter ? tmpSettings.detailFilter : "", spec->labelTag,
                tmpSettings.labelTemplate ? tmpSettings.labelTemplate : "-", tmpSettings.startOnCount,
                tmpSettings.stopOnCount, tmpSettings.priority);
...
#if (defined(J9VM_RAS_DUMP_AGENTS))
omr_error_t printDumpEvents(struct J9JavaVM* vm, UDATA bits, IDATA verbose)
{
    char* separator = "";
    ...
    for (i = 0; i < J9RAS_DUMP_KNOWN_EVENTS; i++) {
        if (bits & rasDumpEvents[i].bits) {
            ...
            if (verbose) {
                j9tty_err_printf(PORTLIB, "  %s%*c%s\n", rasDumpEvents[i].name,
                    maxNameLength - strlen(rasDumpEvents[i].name) + 2, ' ', rasDumpEvents[i].detail);
            } else {
...
#if (defined(J9VM_RAS_DUMP_AGENTS))
omr_error_t printDumpRequests(struct J9JavaVM* vm, UDATA bits, IDATA verbose)
{
    int i;
    char* separator = "";
    ...
    /* Header */
    if (verbose) {
        j9tty_err_printf(PORTLIB, "  Name      VM action\n  --------  -----------------------\n");
    }
    ...
    for (i = 0; i < J9RAS_DUMP_KNOWN_REQUESTS; i++) {
        if (bits & rasDumpRequests[i].bits) {
            /* Switch between multi-line and single-line styles */
            if (verbose) {
                j9tty_err_printf(PORTLIB, "  %s%*c%s\n", rasDumpRequests[i].name, 10 - strlen(rasDumpRequests[i].name),
                    ' ', rasDumpRequests[i].detail);
            } else {
...
#if (defined(J9VM_RAS_DUMP_AGENTS))
static IDATA writeIntoBuffer(void* buffer, IDATA buffer_length, IDATA* index, char* data)
{
    IDATA len;
...
static const char* getLabelTag(struct J9RASdumpAgent* agent)
{
...
queryAgent(struct J9JavaVM* vm, struct J9RASdumpAgent* agent, IDATA buffer_size, void* buffer, IDATA* index)
{
    ...
    for (i = 0; i < J9RAS_DUMP_KNOWN_EVENTS; i++) {
        if (agent->eventMask & rasDumpEvents[i].bits) {
            /* Switch between multi-line and single-line styles */
            len += j9str_printf(
                PORTLIB, &temp_buf[len], sizeof(temp_buf) - len, "%s%s", separator, rasDumpEvents[i].name);
            separator = "+";
        }
    }
    if (len > 0) {
    ...
    len += j9str_printf(PORTLIB, temp_buf, sizeof(temp_buf),
        ...
        "range=%d..%d,"
        "priority=%d,",
        getLabelTag(agent), agent->labelTemplate ? agent->labelTemplate : "-", agent->startOnCount, agent->stopOnCount,
        agent->priority);
    if (len > 0) {
        rc = writeIntoBuffer(buffer, buffer_size, &next_char, temp_buf);
        if (rc == FALSE) {
    ...
    for (i = 0; i < J9RAS_DUMP_KNOWN_REQUESTS; i++) {
        if (agent->requestMask & rasDumpRequests[i].bits) {
            /* Switch between multi-line and single-line styles */
            len += j9str_printf(
                PORTLIB, &temp_buf[len], sizeof(temp_buf) - len, "%s%s", separator, rasDumpRequests[i].name);
            separator = "+";
        }
    }
...
omr_error_t printDumpAgent(struct J9JavaVM* vm, struct J9RASdumpAgent* agent)
{
    ...
    if (agent->dumpOptions != NULL) {
        j9tty_err_printf(PORTLIB, ",");
        j9tty_err_printf(PORTLIB, "\n    opts=%s", agent->dumpOptions ? agent->dumpOptions : "");
    }
    j9tty_err_printf(PORTLIB, "\n");
...
#if (defined(J9VM_RAS_DUMP_AGENTS))
const char* mapDumpEvent(UDATA eventFlag)
{
    int i;
...
#if (defined(J9VM_RAS_DUMP_AGENTS))
omr_error_t copyDumpSettings(struct J9JavaVM* vm, J9RASdumpSettings* src, J9RASdumpSettings* dst)
{
...
#if (defined(J9VM_RAS_DUMP_AGENTS))
J9RASdumpSettings* copyDumpSettingsQueue(J9JavaVM* vm, J9RASdumpSettings* toCopy)
{
    PORT_ACCESS_FROM_JAVAVM(vm);
    int i;
    omr_error_t retVal = OMR_ERROR_NONE;
    J9RASdumpSettings* queue = (J9RASdumpSettings*)j9mem_allocate_memory(
        sizeof(J9RASdumpSettings) * J9RAS_DUMP_KNOWN_SPECS, OMRMEM_CATEGORY_VM);
...
#if (defined(J9VM_RAS_DUMP_AGENTS))
struct J9RASdumpSettings* initDumpSettings(struct J9JavaVM* vm)
{
    ...
    if (0 == rasDumpStrings.vmCount++) {
        ...
        rasDumpStrings.table
            = (char**)j9mem_allocate_memory(rasDumpStrings.capacity * sizeof(char*), OMRMEM_CATEGORY_VM);
    }
    ...
    settings = (J9RASdumpSettings*)j9mem_allocate_memory(
        sizeof(J9RASdumpSettings) * J9RAS_DUMP_KNOWN_SPECS, OMRMEM_CATEGORY_VM);
...
#if (defined(J9VM_RAS_DUMP_AGENTS))
omr_error_t freeDumpSettings(struct J9JavaVM* vm, struct J9RASdumpSettings* settings)
{
    U_32 i;
...
scanDumpType(char** optionStringPtr)
{
    ...
    for (kind = 0; kind < J9RAS_DUMP_KNOWN_SPECS; kind++) {
        if (try_scan(optionStringPtr, rasDumpSpecs[kind].name)) {
            ...
            if (try_scan(optionStringPtr, "+") || try_scan(optionStringPtr, ":") || *optionStringPtr[0] == '\0') {
                retVal = kind;
...
#if (defined(J9VM_RAS_DUMP_AGENTS))
omr_error_t copyDumpAgent(struct J9JavaVM* vm, J9RASdumpAgent* src, J9RASdumpAgent* dst)
{
    memset(dst, 0, sizeof(*dst));
...
#if (defined(J9VM_RAS_DUMP_AGENTS))
static void freeQueueWithoutRunningShutdown(J9JavaVM* vm, J9RASdumpAgent* toFree)
{
    PORT_ACCESS_FROM_JAVAVM(vm);
...
#if (defined(J9VM_RAS_DUMP_AGENTS))
J9RASdumpAgent* copyDumpAgentsQueue(J9JavaVM* vm, J9RASdumpAgent* toCopy)
{
    PORT_ACCESS_FROM_JAVAVM(vm);
...
#if (defined(J9VM_RAS_DUMP_AGENTS))
omr_error_t loadDumpAgent(struct J9JavaVM* vm, IDATA kind, char* optionString)
{
    J9RASdumpSettings* settings = ((J9RASdumpQueue*)vm->j9rasDumpFunctions)->settings;
    J9RASdumpSettings tmpSettings = (settings ? settings[kind] : rasDumpSpecs[kind].settings);
    J9RASdumpAgent* agent = NULL;
...
#if (defined(J9VM_RAS_DUMP_AGENTS))
omr_error_t deleteMatchingAgents(struct J9JavaVM* vm, IDATA kind, char* optionString)
{
...
#if (defined(J9VM_RAS_DUMP_AGENTS))
omr_error_t unloadDumpAgent(struct J9JavaVM* vm, IDATA kind)
{
    J9RASdumpAgent* agent = NULL;
 ...
 */
omr_error_t createAndRunOneOffDumpAgent(struct J9JavaVM* vm, J9RASdumpContext* context, IDATA kind, char* optionString)
{
    J9RASdumpSettings* settings = ((J9RASdumpQueue*)vm->j9rasDumpFunctions)->settings;
    J9RASdumpSettings tmpSettings = (settings ? settings[kind] : rasDumpSpecs[kind].settings);
    J9RASdumpAgent* agent = NULL;
 ...
 * Returns: OMR_ERROR_NONE on success, OMR_ERROR_INTERNAL or OMR_ERROR_OUT_OF_NATIVE_MEMORY if there was a problem.
 */
omr_error_t runDumpAgent(
    struct J9JavaVM* vm, J9RASdumpAgent* agent, J9RASdumpContext* context, UDATA* state, char* detail, U_64 timeNow)
{
    ...
    if (OMR_ERROR_NONE == retVal) {
        ...
        BOOLEAN userRequestedPrepwalkOrCompact
            = agent->requestMask & (J9RAS_DUMP_DO_PREPARE_HEAP_FOR_WALK | J9RAS_DUMP_DO_COMPACT_HEAP);
        ...
        *state = prepareForDump(vm, agent, context, *state);
        ...
        /* If the dump is a system dump and the customer either requested exclusive and we couldn't get it, or they
         * requested prepwalk or compact without exclusive, print a warning message (although, unlike heapdump, we still
         * take the dump).
         */
        if (agent->dumpFn == doSystemDump) {
            ...
            if (userRequestedPrepwalkOrCompact && !userRequestedExclusive) {
                j9nls_printf(
                    PORTLIB, J9NLS_WARNING | J9NLS_STDERR, J9NLS_DMP_SYSTEM_DUMP_COMPACT_PREPWALK_WITHOUT_EXCLUSIVE);
            }
        }

        /* If the dump is a heap dump and exclusive access hasn't been obtained, refuse to do the dump */
        /* This might be encapsulated more neatly if the triggering code were moved down into the dump functions
         * themselves */
        if (gotExclusive || (agent->dumpFn != doHeapDump)) {
            agent->prepState = *state;
            TRIGGER_J9HOOK_VM_DUMP_START(
                vm->hookInterface, vm->internalVMFunctions->currentVMThread(vm), label, detail);
            ...
            if (context->dumpList) {
                if (agent->dumpFn == doHeapDump) {
                    if (agent->dumpOptions && strstr(agent->dumpOptions, "PHD")) {
                        writeIntoBuffer(
                            context->dumpList, context->dumpListSize, (IDATA*)&(context->dumpListIndex), label);
                        writeIntoBuffer(
                            context->dumpList, context->dumpListSize, (IDATA*)&(context->dumpListIndex), "\t");
                    }
 ...
 */
omr_error_t runDumpFunction(J9RASdumpAgent* agent, char* label, J9RASdumpContext* context)
{
    UDATA rc;
    ...
    if (dumpGlobal->noProtect) {
        ...
        protectedResult = j9sig_protect(protectedDumpFunction, &dumpData, signalHandler, NULL,
            J9PORT_SIG_FLAG_MAY_RETURN | J9PORT_SIG_FLAG_SIGALLSYNC, &rc);
 ...
 */
void reportDumpRequest(struct J9PortLibrary* portLibrary, J9RASdumpContext* context, const char* const dumpType,
    const char* const fileName)
{
    ...
    if (fileName != NULL) {
        if (context->eventFlags & J9RAS_DUMP_ON_USER_REQUEST) {
            /*User driven dump*/
            j9nls_printf(PORTLIB, J9NLS_INFO | J9NLS_STDERR | J9NLS_VITAL, J9NLS_DMP_USER_REQUESTED_DUMP_STR, dumpType,
                fileName, context->eventData != NULL ? context->eventData->detailData : NULL);

            Trc_dump_reportDumpStart_FromUser(
                dumpType, fileName, context->eventData != NULL ? context->eventData->detailData : NULL);
        } else {
            /*Event driven dump*/
            j9nls_printf(PORTLIB, J9NLS_INFO | J9NLS_STDERR | J9NLS_VITAL, J9NLS_DMP_EVENT_TRIGGERED_DUMP_STR, dumpType,
                fileName);
        ...
        if (context->eventFlags & J9RAS_DUMP_ON_USER_REQUEST) {
            /*User driven dump*/
            j9nls_printf(PORTLIB, J9NLS_INFO | J9NLS_STDERR | J9NLS_VITAL, J9NLS_DMP_USER_REQUESTED_DUMP_STR_NOFILE,
                dumpType, context->eventData != NULL ? context->eventData->detailData : NULL);

            Trc_dump_reportDumpStart_FromUser_NoFile(
                dumpType, context->eventData != NULL ? context->eventData->detailData : NULL);
        } else {
            /*Event driven dump*/
            j9nls_printf(
                PORTLIB, J9NLS_INFO | J9NLS_STDERR | J9NLS_VITAL, J9NLS_DMP_EVENT_TRIGGERED_DUMP_STR_NOFILE, dumpType);
