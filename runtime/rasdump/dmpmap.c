...
static const DgDumpCondition dgConditions[] = {
    ...
    { "ONDUMP", "user", "events=user,range=1..0" }, { "ONERROR", "abort", "events=abort,range=1..0" },
 ...
 */
static const DgDumpEnvSwitch dgSwitches[] = { { /* name */ "DISABLE_JAVADUMP",
                                                  /* kind */ JAVA_KIND,
                                                  /* on */ "none",
                                                  /* off */ NULL },
    { /* name */ "IBM_HEAPDUMP",
...
static const OomDefaultSetting oomDefaultTable[] = {
    { HEAP_KIND, SYSTHROW_OOM_1_4 }, { JAVA_KIND, SYSTHROW_OOM_1_4 }, { SNAP_KIND, SYSTHROW_OOM_1_4 },
    { SYSTEM_KIND, "events=systhrow,range=1..1,filter=java/lang/OutOfMemoryError,request=exclusive+compact+prepwalk" }
};
...
static const DgDumpEnvSetting dgSettings[] = { { /* name */ "IBM_XE_COE_NAME",
...
omr_error_t mapDumpActions(J9JavaVM* vm, J9RASdumpOption agentOpts[], IDATA* agentNum, char* buf, IDATA condition)
{
    ...
    for (i = 0; i < numDumpActions; i++) {
        ...
        while ((kind = scanDumpType(&typeString)) >= 0) {
            agentOpts[*agentNum].kind = kind;
            if (countChars) {
                /* there was a dump count specified, so we use the locally allocated event string the first time, and
                 * dup it subsequent times*/
                if (allocatedEventStringAlreadyUsed) {
                    /*We have to dupe the event string so the free logic later won't double-free. The original version
...
void disableDumpOnOutOfMemoryError(J9RASdumpOption agentOpts[], IDATA agentNum)
{
    ...
    for (kindCursor = 0; kindCursor < numOomSettings; ++kindCursor) {
        ...
        IDATA j = 0;
        for (j = 0; j < agentNum; j++) {
            if ((NULL != agentOpts[j].args) && (agentOpts[j].kind == disableKind)
