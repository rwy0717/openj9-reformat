...
static const J9RASdefaultOption defaultAgents[] = {
    { "heap", "events=systhrow,range=1..4,filter=java/lang/OutOfMemoryError" }, { "java", "events=gpf,range=1..0" },
    { "java", "events=user,range=1..0" }, { "java", "events=abort,range=1..0" },
    { "java", "events=traceassert,range=1..0" },
    { "java", "events=systhrow,range=1..4,filter=java/lang/OutOfMemoryError" },
    { "java", "events=corruptcache,range=1..0" }, { "snap", "events=gpf,range=1..0" },
    { "snap", "events=abort,range=1..0" }, { "snap", "events=traceassert,range=1..0" },
    { "snap", "events=systhrow,range=1..4,filter=java/lang/OutOfMemoryError" },
    { "snap", "events=corruptcache,range=1..0" }, { "system", "events=gpf,range=1..0" },
#ifdef J9ZOS390
    { "system", "events=user,range=1..0" },
#endif
    { "system", "events=abort,range=1..0" }, { "system", "events=traceassert,range=1..0" },
    ...
    { "jit", "events=gpf,range=1..0" }, { "jit", "events=abort,range=1..0" }
...
void unloadOMRSIG(J9PortLibrary* portLib)
{
...
static void abortHandler(int sig)
{
    ...
    if (cachedVM && !vmThread) {
        ...
        if (loadInfo && (IS_STAGE_COMPLETED(loadInfo->completedBits, VM_INITIALIZATION_COMPLETE))
            ...
            cachedVM->internalVMFunctions->AttachCurrentThreadAsDaemon(
                (JavaVM*)cachedVM, (void**)&vmThread, &attachArgs);
        }
    }
...
static omr_error_t storeDefaultData(J9JavaVM* vm)
{
...
static IDATA configureDumpAgents(J9JavaVM* vm)
{
    ...
    for (i = 0; i < agentNum; i++) {
        if (agentOpts[i].kind == J9RAS_DUMP_OPT_DISABLED)
            continue;
        if (agentOpts[i].pass != J9RAS_DUMP_OPTS_PASS_ONE)
            continue;
...
static omr_error_t printDumpUsage(J9JavaVM* vm)
{
    ...
    j9tty_err_printf(
        PORTLIB, "  -Xdump:directory=<path> Set the default directory path for dump files to be written to\n");
#if defined(OMR_CONFIGURABLE_SUSPEND_SIGNAL)
...
static omr_error_t pushDumpFacade(J9JavaVM* vm)
{
    ...
    if ((queue = (J9RASdumpQueue*)j9mem_allocate_memory(sizeof(J9RASdumpQueue), OMRMEM_CATEGORY_VM))) {
         ...
         *
         * According to the port library SIGABRT (J9PORT_SIG_FLAG_SIGABRT) is an ASYNC signal, so you'd think we should
         * be checking for J9PORT_SIG_OPTIONS_REDUCED_SIGNALS_ASYNCHRONOUS here. However we want to disable the
         * abortHandler when either of -Xrs / -Xrs:sync are present, so check for
         * J9PORT_SIG_OPTIONS_REDUCED_SIGNALS_SYNCHRONOUS instead
         */
 ...
 * See defect 183803
 */
static void initBackTrace(J9JavaVM* vm)
{
 ...
 */
static void initSystemInfo(J9JavaVM* vm)
{
    PORT_ACCESS_FROM_JAVAVM(vm);
 ...
 * default dump agents.
 */
static omr_error_t initDumpDirectory(J9JavaVM* vm)
{
    IDATA xdumpIndex = 0;
    omr_error_t retVal = OMR_ERROR_NONE;
    PORT_ACCESS_FROM_JAVAVM(vm);
...
static void appendSystemInfoFromFile(J9JavaVM* vm, U_32 key, const char* fileName)
{
    ...
    if (fd != -1) {
        ...
        if (read == &buf[0]) {
             ...
             */
            systemInfo
                = (J9RASSystemInfo*)j9mem_allocate_memory(sizeof(J9RASSystemInfo) + bufLen + 1, OMRMEM_CATEGORY_VM);
            if (systemInfo != NULL) {
    ...
    case TRACE_ENGINE_INITIALIZED:
        ...
        if (((RasGlobalStorage*)vm->j9rasGlobalStorage)->jvmriInterface == NULL) {
            ...
            ((RasGlobalStorage*)vm->j9rasGlobalStorage)->jvmriInterface
                = j9mem_allocate_memory(sizeof(DgRasInterface), OMRMEM_CATEGORY_VM);
            if (((RasGlobalStorage*)vm->j9rasGlobalStorage)->jvmriInterface == NULL) {
                j9tty_err_printf(PORTLIB, "Storage for jvmri interface not available, trace not enabled\n");
                return J9VMDLLMAIN_FAILED;
            }

            if ((vm->internalVMFunctions->fillInDgRasInterface(
                    ((RasGlobalStorage*)vm->j9rasGlobalStorage)->jvmriInterface))
                != JNI_OK) {
