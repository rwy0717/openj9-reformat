...
static jvmtiIterationControl binaryHeapDumpHeapIteratorCallback(
    J9JavaVM* vm, J9MM_IterateHeapDescriptor* heapDescriptor, void* userData);
static jvmtiIterationControl binaryHeapDumpSpaceIteratorCallback(
    J9JavaVM* vm, J9MM_IterateSpaceDescriptor* spaceDescriptor, void* userData);
static jvmtiIterationControl binaryHeapDumpRegionIteratorCallback(
    J9JavaVM* vm, J9MM_IterateRegionDescriptor* regionDescription, void* userData);
static jvmtiIterationControl binaryHeapDumpObjectIteratorCallback(
    J9JavaVM* vm, J9MM_IterateObjectDescriptor* objectDescriptor, void* userData);

static jvmtiIterationControl binaryHeapDumpObjectReferenceIteratorTraitsCallback(J9JavaVM* virtualMachine,
    J9MM_IterateObjectDescriptor* objectDescriptor, J9MM_IterateObjectRefDescriptor* referenceDescriptor,
    void* userData);
static jvmtiIterationControl binaryHeapDumpObjectReferenceIteratorWriterCallback(J9JavaVM* virtualMachine,
    J9MM_IterateObjectDescriptor* objectDescriptor, J9MM_IterateObjectRefDescriptor* referenceDescriptor,
    void* userData);

#define allClassesStartDo(vm, state, loader) vm->internalVMFunctions->allClassesStartDo(state, vm, loader)

#define allClassesNextDo(vm, state) vm->internalVMFunctions->allClassesNextDo(state)

#define allClassesEndDo(vm, state) vm->internalVMFunctions->allClassesEndDo(state)
...
protected:
    ...
    public:
        ...
        inline DataType* data(void) { return (DataType*)((char*)this + sizeof(Buffer)); }

        /* Internal method for accessing the data associated with a header */
        inline const DataType* data(void) const { return (DataType*)((char*)this + sizeof(Buffer)); }
    ...
    inline Strings& appendInternal(const DataType* appendData, LengthType numberToAppend)
    {
...
private:
    ...
    friend jvmtiIterationControl binaryHeapDumpSpaceIteratorCallback(
        J9JavaVM* virtualMachine, J9MM_IterateSpaceDescriptor* spaceDescriptor, void* userData);
    friend jvmtiIterationControl binaryHeapDumpObjectIteratorCallback(
        J9JavaVM* virtualMachine, J9MM_IterateObjectDescriptor* objectDescriptor, void* userData);
    friend jvmtiIterationControl binaryHeapDumpObjectReferenceIteratorTraitsCallback(J9JavaVM* virtualMachine,
        J9MM_IterateObjectDescriptor* objectDescriptor, J9MM_IterateObjectRefDescriptor* referenceDescriptor,
        void* userData);
    friend jvmtiIterationControl binaryHeapDumpObjectReferenceIteratorWriterCallback(J9JavaVM* virtualMachine,
        J9MM_IterateObjectDescriptor* objectDescriptor, J9MM_IterateObjectRefDescriptor* referenceDescriptor,
        void* userData);
    friend jvmtiIterationControl binaryHeapDumpHeapIteratorCallback(
        J9JavaVM* virtualMachine, J9MM_IterateHeapDescriptor* heapDescriptor, void* userData);
    friend jvmtiIterationControl binaryHeapDumpRegionIteratorCallback(
        J9JavaVM* virtualMachine, J9MM_IterateRegionDescriptor* regionDescription, void* userData);
...
/*                                                                                                */
/**************************************************************************************************/
BinaryHeapDumpWriter::ReferenceTraits::ReferenceTraits(BinaryHeapDumpWriter* heapDumpWriter, j9object_t objectAddress)
...
/*                                                                                                */
/**************************************************************************************************/
void BinaryHeapDumpWriter::ReferenceTraits::addReference(j9object_t objectReference)
{
...
/**************************************************************************************************/
UDATA
BinaryHeapDumpWriter::ReferenceTraits::count(void) const { return _Count; }
...
BinaryHeapDumpWriter::ReferenceWriter::ReferenceWriter(
    BinaryHeapDumpWriter* heapDumpWriter, j9object_t objectAddress, UDATA count, int size)
    : _HeapDumpWriter(heapDumpWriter)
    , _ObjectAddress(objectAddress)
    , _Count(count)
    , _Size(size)
...
/*                                                                                                */
/**************************************************************************************************/
void BinaryHeapDumpWriter::ReferenceWriter::writeNumber(j9object_t objectReference)
{
    /* Ignore null references */
    if (objectReference == 0) {
        return;
...
/*                                                                                                */
/**************************************************************************************************/
void BinaryHeapDumpWriter::ClassCache::add(const void* clazz)
{
    _Cache[_Index] = clazz;
    _Index = (_Index + 1) % 4;
}
...
/*                                                                                                */
/**************************************************************************************************/
int BinaryHeapDumpWriter::ClassCache::find(const void* clazz)
{
    for (int i = 0; i < 4; i++) {
        if (_Cache[i] == clazz) {
            return i;
...
/*                                                                                                */
/**************************************************************************************************/
int BinaryHeapDumpWriter::ClassCache::index(void) const { return _Index; }
...
/*                                                                                                */
/**************************************************************************************************/
void BinaryHeapDumpWriter::ClassCache::clear(void)
{
    /* Initialize the class cache */
    for (int i = 0; i < 4; i++) {
        _Cache[i] = 0;
    ...
    , _Error(false)
{
    ...
    _VirtualMachine->memoryManagerFunctions->j9mm_iterate_heaps(
        _VirtualMachine, _PortLibrary, 0, binaryHeapDumpHeapIteratorCallback, this);
...
/*                                                                                                */
/**************************************************************************************************/
BinaryHeapDumpWriter::~BinaryHeapDumpWriter() { /* Nothing to do currently */ }
...
/*                                                                                                */
/**************************************************************************************************/
void BinaryHeapDumpWriter::openNewDumpFile(J9MM_IterateSpaceDescriptor* spaceDescriptor)
{
    ...
    _VirtualMachine->memoryManagerFunctions->j9mm_iterate_regions(_VirtualMachine, _PortLibrary, spaceDescriptor,
        j9mm_iterator_flag_regions_read_only, binaryHeapDumpRegionIteratorCallback, this);
...
/*                                                                                                */
/**************************************************************************************************/
void BinaryHeapDumpWriter::writeDumpFileHeader(void)
{
...
/*                                                                                                */
/**************************************************************************************************/
void BinaryHeapDumpWriter::writeDumpFileTrailer(void)
{
...
/*                                                                                                */
/**************************************************************************************************/
void BinaryHeapDumpWriter::writeFullVersionRecord(void)
{
...
/*                                                                                                */
/**************************************************************************************************/
void BinaryHeapDumpWriter::writeObjectRecord(J9MM_IterateObjectDescriptor* objectDescriptor)
{
...
/*                                                                                                */
/**************************************************************************************************/
void BinaryHeapDumpWriter::writeNormalObjectRecord(J9MM_IterateObjectDescriptor* objectDescriptor)
{
    ...
    _VirtualMachine->memoryManagerFunctions->j9mm_iterate_object_slots(_VirtualMachine, _PortLibrary, objectDescriptor,
        j9mm_iterator_flag_exclude_null_refs, binaryHeapDumpObjectReferenceIteratorTraitsCallback, &referenceTraits);
    ...
    if ((addressOffsetEncoding <= 1) && (referenceTraits.count() <= 3) && (classCacheIndex != -1) && (0 == hashCode)) {
        /* It is so generate a short format record */
        /* Calculate the flags */
        int flags = 0x80 | (classCacheIndex << 5 & 0x60) | (((int)referenceTraits.count() << 3) & 0x18)
            | (addressOffsetEncoding << 2 & 0x04) | (referenceOffsetEncoding & 0x03);
    ...
    } else if ((addressOffsetEncoding <= 1) && (referenceTraits.count() <= 7) && (0 == hashCode)) {
        /* It is so generate a medium format record */
        /* Calculate the flags */
        int flags = 0x40 | (((int)referenceTraits.count() << 3) & 0x38) | (addressOffsetEncoding << 2 & 0x04)
        ...
        _VirtualMachine->memoryManagerFunctions->j9mm_iterate_object_slots(_VirtualMachine, _PortLibrary,
            objectDescriptor, j9mm_iterator_flag_exclude_null_refs, binaryHeapDumpObjectReferenceIteratorWriterCallback,
            &referenceWriter);
...
/*                                                                                                */
/**************************************************************************************************/
void BinaryHeapDumpWriter::writeArrayObjectRecord(J9MM_IterateObjectDescriptor* objectDescriptor)
{
    /* Extract a pointer to the current object */
    j9object_t currentObject = objectDescriptor->object;
    ...
    if ((arrayClass->arity == 1) && J9ROMCLASS_IS_PRIMITIVE_TYPE(leafType)) {
        ...
        default:
            dataType = 0x00;
            break; /* Code allows the debugger to break on this line */
        }
        ...
        writeNumber(hashCode, 2);
            ...
            UDATA size = _VirtualMachine->memoryManagerFunctions->j9gc_get_object_total_footprint_in_bytes(
                _VirtualMachine, currentObject);
        ...
        if (NULL != componentClass) {
            ...
            _VirtualMachine->memoryManagerFunctions->j9mm_iterate_object_slots(_VirtualMachine, _PortLibrary,
                objectDescriptor, j9mm_iterator_flag_exclude_null_refs,
                binaryHeapDumpObjectReferenceIteratorTraitsCallback, &referenceTraits);
            ...
            if (referenceTraits.count() <= 7) {
                ...
                _VirtualMachine->memoryManagerFunctions->j9mm_iterate_object_slots(_VirtualMachine, _PortLibrary,
                    objectDescriptor, j9mm_iterator_flag_exclude_null_refs,
                    binaryHeapDumpObjectReferenceIteratorWriterCallback, &referenceWriter);
            }
...
/*                                                                                                */
/**************************************************************************************************/
void BinaryHeapDumpWriter::writeClassRecord(J9Class* currentClass)
{
    ...
    _VirtualMachine->memoryManagerFunctions->j9mm_initialize_object_descriptor(
        _VirtualMachine, &objectDescriptor, currentObject);
    ...
    /* Calculate the biggest difference as a positive number */
    /* NB : A byte has a range of -128 to +127 but this will say -128 needs 2 bytes */
    IDATA biggestStaticOffset = -staticReferenceMinimumOffset > staticReferenceMaximumOffset
        ? -staticReferenceMinimumOffset
        : staticReferenceMaximumOffset;
    ...
    int combinedReferenceOffsetSize = instanceReferenceOffsetSize > staticReferenceOffsetSize
        ? instanceReferenceOffsetSize
        : staticReferenceOffsetSize;
...
/*                                                                                                */
/**************************************************************************************************/
int BinaryHeapDumpWriter::numberSize(IDATA number)
{
...
/*                                                                                                */
/**************************************************************************************************/
int BinaryHeapDumpWriter::numberSizeEncoding(int numberSize)
{
    switch (numberSize) {
    case 1:
        return 0;
...
/*                                                                                                */
/**************************************************************************************************/
int BinaryHeapDumpWriter::wordSize(void)
{
#ifdef J9VM_ENV_DATA64
    return 8;
#else
...
void BinaryHeapDumpWriter::checkForIOError(void)
{
    PORT_ACCESS_FROM_PORT(_PortLibrary);
    if (_OutputStream.hasError()) {
        j9nls_printf(
            PORTLIB, J9NLS_ERROR | J9NLS_STDERR, J9NLS_DMP_ERROR_IN_DUMP_STR, "Heap", j9error_last_error_message());
        Trc_dump_reportDumpError_Event2("Heap", j9error_last_error_message());
        _Error = true;
    }
}

void BinaryHeapDumpWriter::writeCharacters(const char* data, IDATA length)
{
    if (!_Error) {
...
void BinaryHeapDumpWriter::writeCharacters(const char* data)
{
    if (!_Error) {
...
void BinaryHeapDumpWriter::writeNumber(IDATA data, int length)
{
    if (!_Error) {
    ...
    J9JavaVM* vm, J9MM_IterateHeapDescriptor* heapDescriptor, void* userData)
{
    vm->memoryManagerFunctions->j9mm_iterate_spaces(
        vm, vm->portLibrary, heapDescriptor, 0, binaryHeapDumpSpaceIteratorCallback, userData);
    ...
    J9JavaVM* vm, J9MM_IterateRegionDescriptor* regionDescription, void* userData)
{
    ...
    vm->memoryManagerFunctions->j9mm_iterate_region_objects(
        vm, vm->portLibrary, regionDescription, 0, binaryHeapDumpObjectIteratorCallback, userData);
    return ((BinaryHeapDumpWriter*)userData)->_Error ? JVMTI_ITERATION_ABORT : JVMTI_ITERATION_CONTINUE;
}
...
static jvmtiIterationControl binaryHeapDumpObjectReferenceIteratorTraitsCallback(J9JavaVM* vm,
    J9MM_IterateObjectDescriptor* objectDescriptor, J9MM_IterateObjectRefDescriptor* referenceDescriptor,
    void* userData)
{
...
void writePHD(char* label, J9RASdumpContext* context, J9RASdumpAgent* agent)
{
    BinaryHeapDumpWriter(label, context, agent);
}

/* Primary entry point */
extern "C" void runHeapdump(char* label, J9RASdumpContext* context, J9RASdumpAgent* agent)
{
