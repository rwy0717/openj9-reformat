...
static jvmtiIterationControl hdClassicMultiHeapIteratorCallback(
    J9JavaVM* vm, J9MM_IterateHeapDescriptor* heapDescriptor, void* userData);
static jvmtiIterationControl hdClassicMultiSpaceIteratorCallback(
    J9JavaVM* vm, J9MM_IterateSpaceDescriptor* spaceDescriptor, void* userData);
static jvmtiIterationControl hdClassicHeapIteratorCallback(
    J9JavaVM* vm, J9MM_IterateHeapDescriptor* heapDescriptor, void* userData);
static jvmtiIterationControl hdClassicSpaceIteratorCallback(
    J9JavaVM* vm, J9MM_IterateSpaceDescriptor* spaceDescriptor, void* userData);
static jvmtiIterationControl hdClassicRegionIteratorCallback(
    J9JavaVM* vm, J9MM_IterateRegionDescriptor* regionDescriptor, void* userData);
static jvmtiIterationControl hdClassicObjectIteratorCallback(
    J9JavaVM* vm, J9MM_IterateObjectDescriptor* objectDesc, void* userData);
static jvmtiIterationControl hdClassicObjectRefIteratorCallback(J9JavaVM* javaVM,
    ...
    J9JavaVM* vm, J9MM_IterateHeapDescriptor* heapDescriptor, void* userData)
{
    vm->memoryManagerFunctions->j9mm_iterate_spaces(
        vm, vm->portLibrary, heapDescriptor, 0, hdClassicMultiSpaceIteratorCallback, userData);
    ...
    J9JavaVM* vm, J9MM_IterateSpaceDescriptor* spaceDescriptor, void* userData)
{
    ...
    j9str_printf(
        PORTLIB, insert, sizeof(insert), "%s%.*zX", spaceDescriptor->name, sizeof(UDATA) * 2, spaceDescriptor->id);
    ...
    vm->memoryManagerFunctions->j9mm_iterate_regions(
        vm, vm->portLibrary, spaceDescriptor, 0, hdClassicRegionIteratorCallback, userData);
    writeClasses(ctx);
    writeTotals(ctx);
...
{
    vm->memoryManagerFunctions->j9mm_iterate_spaces(
        vm, vm->portLibrary, heapDescriptor, 0, hdClassicSpaceIteratorCallback, userData);
    return JVMTI_ITERATION_CONTINUE;
}
    ...
    J9JavaVM* vm, J9MM_IterateRegionDescriptor* regionDescriptor, void* userData)
{
    ...
    vm->memoryManagerFunctions->j9mm_iterate_region_objects(
        vm, vm->portLibrary, regionDescriptor, 0, hdClassicObjectIteratorCallback, userData);
    return JVMTI_ITERATION_CONTINUE;
}
    ...
    J9JavaVM* vm, J9MM_IterateObjectDescriptor* objectDesc, void* userData)
{
    ...
    vm->memoryManagerFunctions->j9mm_iterate_object_slots(
        vm, PORTLIB, objectDesc, 0, hdClassicObjectRefIteratorCallback, userData);

    return JVMTI_ITERATION_CONTINUE;
}

static jvmtiIterationControl hdClassicObjectRefIteratorCallback(
    J9JavaVM* vm, J9MM_IterateObjectDescriptor* objectDesc, J9MM_IterateObjectRefDescriptor* refDesc, void* userData)
{
    J9RASHeapdumpContext* ctx = (J9RASHeapdumpContext*)userData;
...
static void writeClasses(J9RASHeapdumpContext* ctx)
{
    ...
    while (clazz) {
        ...
        if (!(J9CLASS_FLAGS(clazz) & J9_JAVA_CLASS_HOT_SWAPPED_OUT) && !(J9CLASS_FLAGS(clazz) & J9_JAVA_CLASS_DYING)) {
            ...
            if (J9VM_IS_INITIALIZED_HEAPCLASS_VM(vm, currentObject)) {
                ...
                if (writeClass) {
                    ...
                    vm->memoryManagerFunctions->j9mm_iterate_object_slots(
                        vm, PORTLIB, &objectDescriptor, 0, hdClassicObjectRefIteratorCallback, ctx);
...
static void printType(J9RASHeapdumpContext* ctx, j9object_t obj)
{
    ...
    if (J9ROMCLASS_IS_ARRAY(clazz->romClass)) {
        ...
        print(ctx, "%.*s", J9UTF8_LENGTH(J9ROMCLASS_CLASSNAME(leafClass->arrayClass->romClass)),
            J9UTF8_DATA(J9ROMCLASS_CLASSNAME(leafClass->arrayClass->romClass)));

        leafType = leafClass->romClass;
        if (J9ROMCLASS_IS_PRIMITIVE_TYPE(leafType)) {
            ctx->stats.nPrimitives += tally;
        } else {
            print(ctx, "%.*s;", J9UTF8_LENGTH(J9ROMCLASS_CLASSNAME(leafType)),
                J9UTF8_DATA(J9ROMCLASS_CLASSNAME(leafType)));
            ctx->stats.nArrays += tally;
        }
    } else {
        print(ctx, "%.*s", J9UTF8_LENGTH(J9ROMCLASS_CLASSNAME(clazz->romClass)),
