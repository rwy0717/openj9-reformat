...
UDATA writeExceptionFrameCallBack(J9VMThread* vmThread, void* userData, J9ROMClass* romClass, J9ROMMethod* romMethod,
    J9UTF8* sourceFile, UDATA lineNumber, J9ClassLoader* classLoader);
...
static UDATA outerMemCategoryCallBack(U_32 categoryCode, const char* categoryName, UDATA liveBytes,
    UDATA liveAllocations, BOOLEAN isRoot, U_32 parentCategoryCode, OMRMemCategoryWalkState* state);
static UDATA innerMemCategoryCallBack(U_32 categoryCode, const char* categoryName, UDATA liveBytes,
    UDATA liveAllocations, BOOLEAN isRoot, U_32 parentCategoryCode, OMRMemCategoryWalkState* state);

/* Function prototypes */
static jvmtiIterationControl heapIteratorCallback(
    J9JavaVM* vm, J9MM_IterateHeapDescriptor* heapDescriptor, void* userData);
static jvmtiIterationControl spaceIteratorCallback(
    J9JavaVM* vm, J9MM_IterateSpaceDescriptor* spaceDescriptor, void* userData);
static jvmtiIterationControl regionIteratorCallback(
    J9JavaVM* vm, J9MM_IterateRegionDescriptor* regionDescription, void* userData);
...
static IDATA vmthread_comparator(
...
/* Macros for working with the category_bitmask in the memcategory_total structure. The range of category codes is not
 * contiguous, so we have
 * to map entries from the end of the range (unknown & port library) onto the end of the entries from the start of the
 * range*/
#define MAP_CATEGORY_TO_BITMASK_ENTRY(category)                                                  \
    (((category) >= OMRMEM_LANGUAGE_CATEGORY_LIMIT)                                              \
            ? ((writer->_TotalCategories - 1) - (OMRMEM_OMR_CATEGORY_INDEX_FROM_CODE(category))) \
            : (category))
...
#define CATEGORY_IS_ANCESTOR(total, category) \
    ((total)->category_bitmask[CATEGORY_WORD_INDEX(category)] & CATEGORY_WORD_MASK(category))
#define SET_CATEGORY_AS_ANCESTOR(total, category) \
    ((total)->category_bitmask[CATEGORY_WORD_INDEX(category)] |= CATEGORY_WORD_MASK(category))

static const UDATA syncEventsMask = J9RAS_DUMP_ON_CLASS_LOAD | J9RAS_DUMP_ON_CLASS_UNLOAD
    | J9RAS_DUMP_ON_EXCEPTION_THROW | J9RAS_DUMP_ON_EXCEPTION_CATCH | J9RAS_DUMP_ON_THREAD_START
    | J9RAS_DUMP_ON_THREAD_END | J9RAS_DUMP_ON_THREAD_BLOCKED | J9RAS_DUMP_ON_EXCEPTION_DESCRIBE
    | J9RAS_DUMP_ON_OBJECT_ALLOCATION | J9RAS_DUMP_ON_SLOW_EXCLUSIVE_ENTER | J9RAS_DUMP_ON_EXCEPTION_SYSTHROW
    | J9RAS_DUMP_ON_TRACE_ASSERT | J9RAS_DUMP_ON_USER_REQUEST;
...
private:
    ...
    friend UDATA writeExceptionFrameCallBack(J9VMThread* vmThread, void* userData, J9ROMClass* romClass,
        J9ROMMethod* romMethod, J9UTF8* sourceFile, UDATA lineNumber, J9ClassLoader* classLoader);
    ...
    friend UDATA outerMemCategoryCallBack(U_32 categoryCode, const char* categoryName, UDATA liveBytes,
        UDATA liveAllocations, BOOLEAN isRoot, U_32 parentCategoryCode, OMRMemCategoryWalkState* state);
    friend UDATA innerMemCategoryCallBack(U_32 categoryCode, const char* categoryName, UDATA liveBytes,
        UDATA liveAllocations, BOOLEAN isRoot, U_32 parentCategoryCode, OMRMemCategoryWalkState* state);

    friend jvmtiIterationControl heapIteratorCallback(
        J9JavaVM* vm, J9MM_IterateHeapDescriptor* heapDescriptor, void* userData);
    friend jvmtiIterationControl spaceIteratorCallback(
        J9JavaVM* vm, J9MM_IterateSpaceDescriptor* spaceDescriptor, void* userData);
    friend jvmtiIterationControl regionIteratorCallback(
        J9JavaVM* vm, J9MM_IterateRegionDescriptor* regionDescription, void* userData);
    ...
    void writeThread(J9VMThread* vmThread, J9PlatformThread* nativeThread, UDATA vmstate, UDATA javaState,
        UDATA javaPriority, j9object_t lockObject, J9VMThread* lockOwnerThread);
    ...
    UDATA writeExceptionFrame(
        void* userData, J9ROMClass* romClass, J9ROMMethod* romMethod, J9UTF8* sourceFile, UDATA lineNumber);
...
public:
    ...
    void invoke(void) { (jcw->*sectionFunction)(); }
};
...
#define CALL_PROTECT(section, retVal)                                                      \
    do {                                                                                   \
        ...
        UDATA sink;                                                                        \
        retVal = j9sig_protect(protectedWriteSection, &closure, handlerWriteSection, this, \
...
/*                                                                                                */
/**************************************************************************************************/
JavaCoreDumpWriter::JavaCoreDumpWriter(const char* fileName, J9RASdumpContext* context, J9RASdumpAgent* agent)
    : _Context(context)
    ...
    , _TotalCategories(-1)
{
    ...
    bufferWrites = !_AvoidLocks && ((_Context->eventFlags & (J9RAS_DUMP_ON_GP_FAULT | J9RAS_DUMP_ON_ABORT_SIGNAL)) == 0)
     ...
     * thread lock as we will attempt to get it again for uninflated locks when calling getVMThreadRawState while
     * looking for waiting threads on any given monitor
     */
...
/*                                                                                                */
/**************************************************************************************************/
JavaCoreDumpWriter::~JavaCoreDumpWriter() { /* Nothing to do currently */ }
...
/*                                                                                                */
/**************************************************************************************************/
bool JavaCoreDumpWriter::avoidLocks(void) { return _AvoidLocks; }
...
/*                                                                                                */
/**************************************************************************************************/
void JavaCoreDumpWriter::writeHeader(void)
{
    _OutputStream.writeCharacters(
        "NULL           ------------------------------------------------------------------------\n");
}
...
/*                                                                                                */
/**************************************************************************************************/
void JavaCoreDumpWriter::writeTitleSection(void)
{
    /* Write the section header */
    _OutputStream.writeCharacters("0SECTION       TITLE subcomponent dump routine\n"
                                  "NULL           ===============================\n");
...
/*                                                                                                */
/**************************************************************************************************/
void JavaCoreDumpWriter::writeEventDrivenTitle(void)
{
...
/*                                                                                                */
/**************************************************************************************************/
void JavaCoreDumpWriter::writeUserRequestedTitle(void)
{
    /* Write the dump event description */
...
/*                                                                                                */
/**************************************************************************************************/
void JavaCoreDumpWriter::writeProcessorSection(void)
{
    PORT_ACCESS_FROM_JAVAVM(_VirtualMachine);

    /* Write the section header */
    _OutputStream.writeCharacters("0SECTION       GPINFO subcomponent dump routine\n"
                                  "NULL           ================================\n");
...
/*                                                                                                */
/**************************************************************************************************/
void JavaCoreDumpWriter::writeEnvironmentSection(void)
{
    /* Write the section header */
    _OutputStream.writeCharacters("0SECTION       ENVINFO subcomponent dump routine\n"
                                  "NULL           =================================\n");
...
/*                                                                                                */
/**************************************************************************************************/
void JavaCoreDumpWriter::writeMemorySection(void)
{
    /* Write the section header */
    _OutputStream.writeCharacters("0SECTION       MEMINFO subcomponent dump routine\n"
                                  "NULL           =================================\n");

    /* Write the GC heap sub-section. Note since 2.6 VM this uses GC iterators rather than the VM segment list */
    _OutputStream.writeCharacters("NULL           \n"
                                  "1STHEAPTYPE    Object Memory\n");
    _VirtualMachine->memoryManagerFunctions->j9mm_iterate_heaps(
        _VirtualMachine, _PortLibrary, 0, heapIteratorCallback, this);

    /* Write the VM memory segments sub-section */
    _OutputStream.writeCharacters("1STSEGTYPE     Internal Memory\n" SEGMENT_HEADER);
    ...
    if (_VirtualMachine->jitConfig) {
        ...
        _OutputStream.writeVPrintf(FORMAT_SIZE_DECIMAL, decimalLength,
            _VirtualMachine->jitConfig->codeCacheTotalKB * 1024); // Needs to be codeCacheTotalKB
        _OutputStream.writeCharacters(" (");
        _OutputStream.writeVPrintf(FORMAT_SIZE_HEX, sizeof(void*) * 2,
            _VirtualMachine->jitConfig->codeCacheTotalKB * 1024); // Needs to be codeCacheTotalKB
        _OutputStream.writeCharacters(")\n");
        _OutputStream.writeCharacters("NULL           \n"
        ...
        _OutputStream.writeCharacters("1STSEGLIMIT    ");
        _OutputStream.writeCharacters("Allocation limit:    ");
        _OutputStream.writeVPrintf(
            FORMAT_SIZE_DECIMAL, decimalLength, _VirtualMachine->jitConfig->dataCacheTotalKB * 1024);
        _OutputStream.writeCharacters(" (");
        _OutputStream.writeVPrintf(
            FORMAT_SIZE_HEX, sizeof(void*) * 2, _VirtualMachine->jitConfig->dataCacheTotalKB * 1024);
        _OutputStream.writeCharacters(")\n");
    }
#endif
    ...
    UDATA liveAllocations, BOOLEAN isRoot, U_32 parentCategoryCode, OMRMemCategoryWalkState* state)
{
    ...
    if (total.liveAllocations > 0) {
        ...
        /* Store liveBytes and liveAllocations away to print the "Other" row after the children have been printed (see
         * top of function) */
...
void JavaCoreDumpWriter::writeMemoryCountersSection(void)
{
    I_32 i;
    PORT_ACCESS_FROM_PORT(_PortLibrary);

    /* Write the section header */
    _OutputStream.writeCharacters("0SECTION       NATIVEMEMINFO subcomponent dump routine\n"
                                  "NULL           =================================\n");
 ...
 */
void JavaCoreDumpWriter::writeNativeAllocator(
    const char* name, U_32 depth, BOOLEAN isRoot, UDATA liveBytes, UDATA liveAllocations)
{
    U_32 i;
...
/*                                                                                                */
/**************************************************************************************************/
void JavaCoreDumpWriter::writeMonitorSection(void)
{
    ...
    /* Write the section header */
    _OutputStream.writeCharacters("0SECTION       LOCKS subcomponent dump routine\n"
                                  "NULL           ===============================\n");
    ...
    for (UDATA i = 0; walkThread != NULL && i < _AllocatedVMThreadCount; i++) {
        ...
        if (j9sig_protect(protectedGetVMThreadRawState, args, handlerGetVMThreadRawState, &stateFault,
                J9PORT_SIG_FLAG_SIGALLSYNC | J9PORT_SIG_FLAG_MAY_RETURN, &stateClean)
            == J9PORT_SIG_EXCEPTION_OCCURRED) {
            // Nothing to do if we couldn't get the details for this thread.
        } else {
            // Only store interesting threads, list will be null terminated.
            if (((stateClean == J9VMTHREAD_STATE_BLOCKED) || (stateClean == J9VMTHREAD_STATE_WAITING)
...
/*                                                                                                */
/**************************************************************************************************/
void JavaCoreDumpWriter::writeThreadSection(void)
{
    PORT_ACCESS_FROM_PORT(_PortLibrary);
    /* Write the section header */
    _OutputStream.writeCharacters("0SECTION       THREADS subcomponent dump routine\n"
                                  "NULL           =================================\n");

    /* Write the thread counts */
    _OutputStream.writeCharacters("NULL\n");
    _OutputStream.writeCharacters("1XMPOOLINFO    JVM Thread pool info:\n");
    _OutputStream.writeCharacters("2XMPOOLTOTAL       Current total number of pooled threads: ");
    _OutputStream.writeInteger(_AllocatedVMThreadCount, "%i");
    _OutputStream.writeCharacters("\n");
    _OutputStream.writeCharacters("2XMPOOLLIVE        Current total number of live threads: ");
    _OutputStream.writeInteger(_VirtualMachine->totalThreadCount, "%i");
    _OutputStream.writeCharacters("\n");
    _OutputStream.writeCharacters("2XMPOOLDAEMON      Current total number of live daemon threads: ");
    ...
    if ((_Agent->requestMask & J9RAS_DUMP_DO_PREEMPT_THREADS)
         ...
         */
        && !(_Context->eventFlags & J9RAS_DUMP_ON_THREAD_START) && !(_Context->eventFlags & J9RAS_DUMP_ON_THREAD_END)
#endif
    ) {
        ...
        closure.jcw = this;
        closure.state = NULL;
        j9sig_protect(protectedWriteThreadsWithNativeStacks, &closure, handlerWriteStacks, this,
            J9PORT_SIG_FLAG_SIGALLSYNC | J9PORT_SIG_FLAG_MAY_RETURN, &sink);
    }
    ...
    if (!_ThreadsWalkStarted) {
        ...
        closure.jcw = this;
        closure.state = NULL;
        j9sig_protect(protectedWriteThreadsJavaOnly, &closure, handlerWriteStacks, this,
            J9PORT_SIG_FLAG_SIGALLSYNC | J9PORT_SIG_FLAG_MAY_RETURN, &sink);
    }
     ...
     */
    if (_Context->onThread && (_Context->eventFlags & syncEventsMask)
    ...
    if (!avoidLocks()) {
        ...
        closure.jcw = this;
        closure.state = NULL;
        j9sig_protect(protectedWriteThreadsUsageSummary, &closure, handlerGetThreadsUsageInfo, this,
            J9PORT_SIG_FLAG_SIGALLSYNC | J9PORT_SIG_FLAG_MAY_RETURN, &sink);
    }
...
void JavaCoreDumpWriter::writeThreadsUsageSummary(void)
{
    ...
    _OutputStream.writeCharacters("1XMTHDSUMMARY  Threads CPU Usage Summary\n"
                                  "NULL           =========================\n");

    if (J9_ARE_ALL_BITS_SET(_VirtualMachine->extendedRuntimeFlags, J9_EXTENDED_RUNTIME_REDUCE_CPU_MONITOR_OVERHEAD)) {
        _OutputStream.writeCharacters(
            "NULL\n1XMTHDCATINFO  Warning: to get more accurate CPU times for the GC, the option "
            "-XX:-ReduceCPUMonitorOverhead can be used. See the user guide for more information.\nNULL\n");
    }
...
void JavaCoreDumpWriter::writeHookInfo(struct OMRHookInfo4Dump* hookInfo)
{
...
void JavaCoreDumpWriter::writeHookInterface(struct J9HookInterface** hookInterface)
{
...
void JavaCoreDumpWriter::writeThreadsWithNativeStacks(void)
{
    ...
    for (i = 0; walkThread != NULL && i < _AllocatedVMThreadCount; i++) {
        ...
        if (J9PORT_SIG_EXCEPTION_OCCURRED
            == j9sig_protect(protectedGetVMThreadObjectState, args, handlerGetVMThreadObjectState, &stateFault,
                   J9PORT_SIG_FLAG_SIGALLSYNC | J9PORT_SIG_FLAG_MAY_RETURN, &stateClean)) {
        ...
        if (J9PORT_SIG_EXCEPTION_OCCURRED
            == j9sig_protect(protectedGetVMThreadRawState, args, handlerGetVMThreadRawState, &stateFault,
                   J9PORT_SIG_FLAG_SIGALLSYNC | J9PORT_SIG_FLAG_MAY_RETURN, &stateClean)) {
        ...
        if (walkThread != NULL && walkThread->publicFlags == J9_PUBLIC_FLAGS_HALT_THREAD_INSPECTION) {
            ...
            if (!restartedWalk) {
                ...
                errorMessage = "Truncating collection of java threads due to multiple threads stopping during walk, "
                               "some java thread details will be omitted";
                break;
            }
        }
    }
    ...
    if (vmThread && vmThread->gpInfo) {
        ...
        returnValue = j9sig_protect(protectedStartDoWithSignal, &closure, handlerNativeThreadWalk, this,
            J9PORT_SIG_FLAG_SIGALLSYNC | J9PORT_SIG_FLAG_MAY_RETURN, (UDATA*)&nativeThread);
    } else {
        returnValue = j9sig_protect(protectedStartDo, &closure, handlerNativeThreadWalk, this,
            J9PORT_SIG_FLAG_SIGALLSYNC | J9PORT_SIG_FLAG_MAY_RETURN, (UDATA*)&nativeThread);
    }
    ...
    if (returnValue == J9PORT_SIG_EXCEPTION_OCCURRED) {
        ...
        while (j9sig_protect(protectedNextDo, &closure, handlerNativeThreadWalk, this,
                   J9PORT_SIG_FLAG_SIGALLSYNC | J9PORT_SIG_FLAG_MAY_RETURN, (UDATA*)&nativeThread)
            == J9PORT_SIG_EXCEPTION_OCCURRED) {
            errorMessage = "GPF received while walking native threads\n";
        }
    ...
    if (nativeThread != NULL || vmthreads.rootNode != NULL) {
        ...
        while (vmthreads.rootNode != NULL) {
            ...
            avl_delete(&vmthreads, (J9AVLTreeNode*)node);
            writeThread(node->vmthread, NULL, node->vmThreadState, node->javaThreadState, node->javaPriority,
                node->lockObject, node->lockOwner);
        }
    }
...
void JavaCoreDumpWriter::writeThreadsJavaOnly(void)
{
...
/*                                                                                                */
/**************************************************************************************************/
void JavaCoreDumpWriter::writeClassSection(void)
{
    ...
    _OutputStream.writeCharacters("1CLTEXTCLLIB   \tClassLoader loaded libraries\n");

    pool_do(_VirtualMachine->classLoaderBlocks, writeLibrariesCallBack, this);

    /* Write the sub-section header */
    _OutputStream.writeCharacters("1CLTEXTCLLOD   \tClassLoader loaded classes\n");
...
void JavaCoreDumpWriter::writeHookSection(void)
{
    /* Write the section header */
 ...
 */
static void writeCudaScaledValue(TextFileStream& stream, U_32 value)
{
 ...
 * NULL           ------------------------------------------------------------------------
 */
void JavaCoreDumpWriter::writeCudaSection(void)
{
    PORT_ACCESS_FROM_PORT(_PortLibrary);
...
/*                                                                                                */
/**************************************************************************************************/
void JavaCoreDumpWriter::writeSharedClassIPCInfo(
    const char* textStart, const char* textEnd, IDATA id, UDATA padToLength)
{
...
void JavaCoreDumpWriter::writeSharedClassLockInfo(const char* lockName, IDATA lockSemid, void* lockTID)
{
...
void JavaCoreDumpWriter::writeSharedClassSection(void)
{
    ...
    if (_VirtualMachine->sharedClassConfig->getJavacoreData(_Context->javaVM, &javacoreData)) {
        /* Write the section and first sub-section header */
        _OutputStream.writeCharacters("0SECTION       SHARED CLASSES subcomponent dump routine\n"
                                      "NULL           ========================================\n"
                                      "NULL\n");

        _OutputStream.writeCharacters("1SCLTEXTCRTW   Cache Created With\n"
                                      "NULL           ------------------\n"
                                      "NULL\n");

        if (0 != (javacoreData.extraFlags & J9SHR_EXTRA_FLAGS_NO_LINE_NUMBERS)) {
            _OutputStream.writeCharacters("2SCLTEXTXNL        -Xnolinenumbers       = true\n");
        } else {
            _OutputStream.writeCharacters("2SCLTEXTXNL        -Xnolinenumbers       = false\n");
        }

        if (0 != (javacoreData.extraFlags & J9SHR_EXTRA_FLAGS_BCI_ENABLED)) {
            _OutputStream.writeCharacters("2SCLTEXTBCI        BCI Enabled           = true\n");
        } else {
            _OutputStream.writeCharacters("2SCLTEXTBCI        BCI Enabled           = false\n");
        }

        if (0 != (javacoreData.extraFlags & J9SHR_EXTRA_FLAGS_RESTRICT_CLASSPATHS)) {
            _OutputStream.writeCharacters("2SCLTEXTBCI        Restrict Classpaths   = true\n");
        } else {
            _OutputStream.writeCharacters("2SCLTEXTBCI        Restrict Classpaths   = false\n");
        }
        ...
        if (0 != (javacoreData.extraFlags & J9SHR_EXTRA_FLAGS_NO_LINE_NUMBER_CONTENT)) {
            _OutputStream.writeCharacters("2SCLTEXTNLC        No line number content                    = true\n");
        } else {
            _OutputStream.writeCharacters("2SCLTEXTNLC        No line number content                    = false\n");
        }

        if (0 != (javacoreData.extraFlags & J9SHR_EXTRA_FLAGS_LINE_NUMBER_CONTENT)) {
            _OutputStream.writeCharacters("2SCLTEXTLNC        Line number content                       = true\n");
        } else {
            _OutputStream.writeCharacters("2SCLTEXTLNC        Line number content                       = false\n");
        }

        _OutputStream.writeCharacters("NULL\n");

        if (javacoreData.ccCount > 1) {
            _OutputStream.writeCharacters("2SCLTEXTCCC        Cachelet count                            = ");
        ...
        _OutputStream.writeCharacters("2SCLTEXTRCS        ROMClass start address                    = ");
        ...
        _OutputStream.writeCharacters("NULL\n"
                                      ...
                                      "NULL           ------------------\n"
                                      "1SCLTEXTCNTD       Cache Name                    Feature                  "
                                      "Memory type              Cache path\n"
                                      "NULL\n");
        _OutputStream.writeCharacters("2SCLTEXTCMDT       ");
...
/*                                                                                                */
/**************************************************************************************************/
void JavaCoreDumpWriter::writeTrailer(void)
{
    _OutputStream.writeCharacters(
...
/*                                                                                                */
/**************************************************************************************************/
void JavaCoreDumpWriter::writeVMRuntimeState(U_32 vmRuntimeState)
{
...
/*                                                                                                */
/**************************************************************************************************/
void JavaCoreDumpWriter::writeExceptionDetail(j9object_t* exceptionRef)
{
    ...
    if (exceptionRef && *exceptionRef) {
        ...
        if (NULL != message) {
            buf = _VirtualMachine->internalVMFunctions->copyStringToUTF8WithMemAlloc(
                vmThread, message, J9_STR_NULL_TERMINATE_RESULT, "", 0, stackBuffer, _MaximumExceptionNameLength, &len);
        }
        ...
        eiieClass = _VirtualMachine->internalVMFunctions->internalFindKnownClass(
            vmThread, J9VMCONSTANTPOOL_JAVALANGEXCEPTIONININITIALIZERERROR, J9_FINDKNOWNCLASS_FLAG_EXISTING_ONLY);
        ...
        if (J9OBJECT_CLAZZ(vmThread, *exceptionRef) == eiieClass) {
            ...
            if (nestedException) {
                ...
                if (nestedExceptionClassName) {
                    ...
                    _OutputStream.writeCharacters(
                        (char*)J9UTF8_DATA(nestedExceptionClassName), (UDATA)J9UTF8_LENGTH(nestedExceptionClassName));
                    _OutputStream.writeCharacters("\"");
                }

                message = J9VMJAVALANGTHROWABLE_DETAILMESSAGE(vmThread, nestedException);
                if (NULL != message) {
                    nestedBuf = _VirtualMachine->internalVMFunctions->copyStringToUTF8WithMemAlloc(vmThread, message,
                        J9_STR_NULL_TERMINATE_RESULT, "", 0, detailMessageStackBuffer, _MaximumExceptionNameLength,
                        &nestedLen);
                }
...
/*                                                                                                */
/**************************************************************************************************/
void JavaCoreDumpWriter::writeGPCategory(void* gpInfo, const char* prefix, U_32 category)
{
    PORT_ACCESS_FROM_PORT(_PortLibrary);
...
/*                                                                                                */
/**************************************************************************************************/
void JavaCoreDumpWriter::writeGPValue(const char* prefix, const char* name, U_32 kind, void* value)
{
    ...
    case J9PORT_SIG_VALUE_16:
        _OutputStream.writeInteger(*(U_16*)value, "%04X");
        break;
    case J9PORT_SIG_VALUE_32:
        _OutputStream.writeInteger(*(U_32*)value, "%08.8X");
        break;
    case J9PORT_SIG_VALUE_64:
        _OutputStream.writeInteger64(*(U_64*)value, "%016.16llX");
        break;
    ...
    case J9PORT_SIG_VALUE_ADDRESS:
        _OutputStream.writeVPrintf("%.*zX", sizeof(void*) * 2, (IDATA)(*(void**)value));
        break;
...
/*                                                                                                */
/**************************************************************************************************/
void JavaCoreDumpWriter::writeJitMethod(J9VMThread* vmThread)
{
#ifdef J9VM_INTERP_NATIVE_SUPPORT
...
/*                                                                                                */
/**************************************************************************************************/
void JavaCoreDumpWriter::writeSegments(J9MemorySegmentList* list, BOOLEAN isCodeCacheSegment)
{
    ...
    while (segment != 0) {
        ...
        if (isCodeCacheSegment) {
            /* The JIT code cache grows from both ends of the segment: the warmAlloc pointer upwards from the start of
             * the segment and the coldAlloc pointer downwards from the end of the segment. The free space in a JIT code
             * cache segment is the space between the warmAlloc and coldAlloc pointers. See
             * jit/codert/MultiCodeCache.hpp, the contract with the JVM is that the address of the TR_MCCCodeCache
             * structure is stored at the beginning of the segment, and that the warmAlloc and coldAlloc pointers are
             * the first two fields in TR_MCCCodeCache.
             */
            ...
            if (mccCodeCache) {
                ...
                /* sanity check, if the TR_MCCCodeCache pointer was null, set default values for warmAlloc and coldAlloc
                 * pointers */
        ...
        _OutputStream.writeCharacters(" ");
        if (isCodeCacheSegment) {
            /* For the JIT code cache segments, we fake up the allocation pointer as the sum of the cold and warm
             * allocations */
...
/*                                                                                                */
/**************************************************************************************************/
void JavaCoreDumpWriter::writeTraceHistory(U_32 type)
{
    ...
    if (iterator != NULL) {
        ...
        j9sig_protect(protectedWriteGCHistoryLines, &closure, handlerWriteSection, this,
            J9PORT_SIG_FLAG_SIGALLSYNC | J9PORT_SIG_FLAG_MAY_RETURN, &sink);
...
void JavaCoreDumpWriter::writeGCHistoryLines(UtThreadData** thr, UtTracePointIterator* iterator, const char* type)
{
    ...
    while (
        NULL != uteInterface->server->FormatNextTracePoint(iterator, stackBuffer, _MaximumFormattedTracePointLength)) {
...
/*                                                                                                */
/**************************************************************************************************/
void JavaCoreDumpWriter::writeDeadLocks(void)
{
    ...
    J9HashTable* deadlocks = hashTableNew(OMRPORT_FROM_J9PORT(PORTLIB), J9_GET_CALLSITE(), 0, sizeof(DeadLockGraphNode),
        0, 0, OMRMEM_CATEGORY_VM, lockHashFunction, lockHashEqualFunction, NULL, NULL);
...
/*                                                                                                */
/**************************************************************************************************/
void JavaCoreDumpWriter::findThreadCycle(J9VMThread* vmThread, J9HashTable* deadlocks)
{
    PORT_ACCESS_FROM_PORT(_PortLibrary);
    J9ThreadAbstractMonitor* lock;
    J9VMThread* owner;
...
/*                                                                                                */
/**************************************************************************************************/
void JavaCoreDumpWriter::writeDeadlockNode(DeadLockGraphNode* node, int count)
{
...
/*                                                                                                */
/**************************************************************************************************/
void JavaCoreDumpWriter::writeMonitorObject(
    J9ThreadMonitor* monitor, j9object_t obj, blocked_thread_record* threadStore)
{
    ...
    while (threadStore[threadIndex].waitingThread != NULL) {
        ...
        UDATA status = threadStore[threadIndex].waitingThreadState;
        J9VMThread* walkThread = threadStore[threadIndex].waitingThread;
        if ((blocking_monitor == monitor)
            && ((status == J9VMTHREAD_STATE_WAITING) || (status == J9VMTHREAD_STATE_WAITING_TIMED))) {
            /* Output the list header */
...
/*                                                                                                */
/**************************************************************************************************/
void JavaCoreDumpWriter::writeMonitor(J9ThreadMonitor* monitor)
{
...
/*                                                                                                */
/**************************************************************************************************/
void JavaCoreDumpWriter::writeSystemMonitor(J9ThreadMonitor* monitor)
{
...
/*                                                                                                */
/**************************************************************************************************/
void JavaCoreDumpWriter::writeObject(j9object_t obj)
{
    J9ROMClass* romClass;
...
/*                                                                                                */
/**************************************************************************************************/
void JavaCoreDumpWriter::writeThreadState(UDATA threadState)
{
    switch (threadState) {
...
/*                                                                                                */
/**************************************************************************************************/
void JavaCoreDumpWriter::writeThread(J9VMThread* vmThread, J9PlatformThread* nativeThread, UDATA vmstate,
    UDATA javaState, UDATA javaPriority, j9object_t lockObject, J9VMThread* lockOwnerThread)
{
    PORT_ACCESS_FROM_PORT(_PortLibrary);
    ...
    if (vmThread != NULL) {
        ...
        if (vmThread->threadObject) {
            ...
            j9sig_protect(protectedWriteJavaLangThreadInfo, &closure, handlerWriteJavaLangThreadInfo, this,
                J9PORT_SIG_FLAG_SIGALLSYNC | J9PORT_SIG_FLAG_MAY_RETURN, &sink);
        }
        ...
        if (osThread) {
            ...
            if (omrthread_get_stack_range((omrthread_t)osThread, &stackStart, &stackEnd) == J9THREAD_SUCCESS) {
                ...
                _OutputStream.writeInteger(
                    stackEnd > stackStart ? (UDATA)stackEnd - (UDATA)stackStart : (UDATA)stackStart - (UDATA)stackEnd);
                _OutputStream.writeCharacters(")\n");
            }
        ...
        if (!avoidLocks()) {
            ...
            j9sig_protect(protectedWriteThreadBlockers, &closure, handlerWriteThreadBlockers, this,
                J9PORT_SIG_FLAG_SIGALLSYNC | J9PORT_SIG_FLAG_MAY_RETURN, &sink);
        }

        if (vmThread->threadObject) {
            UDATA bytesAllocated
                = _VirtualMachine->memoryManagerFunctions->j9gc_get_bytes_allocated_by_thread(vmThread);
        ...
        if (vmThread->threadObject) {
            ...
            j9sig_protect(protectedGetOwnedObjectMonitors, &monitorClosure, handlerGetOwnedObjectMonitors, this,
                J9PORT_SIG_FLAG_SIGALLSYNC | J9PORT_SIG_FLAG_MAY_RETURN, &sink);

            stackWalkState.walkThread = vmThread;

            stackWalkState.flags = J9_STACKWALK_ITERATE_FRAMES | J9_STACKWALK_INCLUDE_NATIVES
                | J9_STACKWALK_VISIBLE_ONLY | J9_STACKWALK_RECORD_BYTECODE_PC_OFFSET;
            ...
            if (j9sig_protect(protectedWalkJavaStack, &closure, handlerJavaThreadWalk, this,
                    J9PORT_SIG_FLAG_SIGALLSYNC | J9PORT_SIG_FLAG_MAY_RETURN, &sink)
                == 0) {
                if (depth == 0) {
                    ...
                    if (!avoidLocks()) {
                        ...
                        if (throwable && *throwable) {
                            ...
                            if (j9sig_protect(protectedIterateStackTrace, &stackClosure, handlerIterateStackTrace, this,
                                    J9PORT_SIG_FLAG_SIGALLSYNC | J9PORT_SIG_FLAG_MAY_RETURN, &sink)
                                == 0) {
                                if (depth == 0) { /* depth == 0 means call succeeded but no frames were written.*/
                                    _OutputStream.writeCharacters(
                                        "3XMTHREADINFO3           No Java callstack associated with throwable\n");
                                }
                            }
...
/*                                                                                                */
/**************************************************************************************************/
void JavaCoreDumpWriter::writeThreadName(J9VMThread* vmThread)
{
    PORT_ACCESS_FROM_PORT(_PortLibrary);
    if (vmThread) {
         ...
         */
        if (j9sig_protect(protectedGetVMThreadName, args, handlerGetVMThreadName, (UDATA*)&nameFault,
                J9PORT_SIG_FLAG_SIGALLSYNC | J9PORT_SIG_FLAG_MAY_RETURN, (UDATA*)&nameClean)
            == J9PORT_SIG_EXCEPTION_OCCURRED) {
...
/*                                                                                                */
/**************************************************************************************************/
void JavaCoreDumpWriter::writeThreadBlockers(
    J9VMThread* vmThread, UDATA vmstate, j9object_t lockObject, J9VMThread* lockOwnerThread)
{
    if (vmThread == NULL) {
        return;
    }
    ...
    if (vmstate == J9VMTHREAD_STATE_BLOCKED) {
        ...
        if (lockObject && !lockOwnerThread) {
            ...
            if (aosClazz) {
                ...
                /* PR 80305 : Do not write back to the castClassCache as this code may be running while the GC is
                 * unloading the class */
                if (instanceOfOrCheckCastNoCacheUpdate(clazz, aosClazz)) {
                    lockOwnerObject = J9VMJAVAUTILCONCURRENTLOCKSABSTRACTOWNABLESYNCHRONIZER_EXCLUSIVEOWNERTHREAD(
                        vmThread, lockObject);
                }
            }
        }
...
JavaCoreDumpWriter::writeFrame(J9StackWalkState* state)
{
    ...
    if (!avoidLocks()) {
        J9UTF8* sourceFile
            = getSourceFileNameForROMClass(_VirtualMachine, methodClass->classLoader, methodClass->romClass);
        if (sourceFile) {
            _OutputStream.writeCharacters("(");
...
/*                                                                                                */
/**************************************************************************************************/
void JavaCoreDumpWriter::writeThreadTime(const char* timerName, I_64 nanoTime)
{
...
/*                                                                                                */
/**************************************************************************************************/
void JavaCoreDumpWriter::writeLoader(J9ClassLoader* classLoader)
{
    ...
    UDATA count = 0;
    J9Class* clazz
        = _VirtualMachine->internalVMFunctions->allClassesStartDo(&classWalkState, _VirtualMachine, classLoader);
#if defined(J9VM_OPT_SHARED_CLASSES)
    UDATA sharedCount = 0;
...
#if defined(J9VM_OPT_SHARED_CLASSES)
        if (sharedROMBoundsStart && (clazz->romClass >= sharedROMBoundsStart)
            && (clazz->romClass < sharedROMBoundsEnd)) {
            sharedCount++;
        }
#endif
...
/*                                                                                                */
/**************************************************************************************************/
void JavaCoreDumpWriter::writeLibraries(J9ClassLoader* classLoader)
{
    PORT_ACCESS_FROM_PORT(_PortLibrary);
    ...
    J9NativeLibrary* sharedLibrary
        = (J9NativeLibrary*)pool_startDo(classLoader->sharedLibraries, &sharedLibrariesWalkState);
...
/*                                                                                                */
/**************************************************************************************************/
void JavaCoreDumpWriter::writeClasses(J9ClassLoader* classLoader)
{
    /* Determine the status of the given loader */
    j9object_t object = getClassLoaderObject(classLoader);
    j9object_t appLdr = getClassLoaderObject(_VirtualMachine->applicationClassLoader);
...
/*                                                                                                */
/**************************************************************************************************/
j9object_t JavaCoreDumpWriter::getClassLoaderObject(J9ClassLoader* loader)
{
...
{
    IDATA monitorCount = 0;
    monitorCount = _VirtualMachine->internalVMFunctions->getOwnedObjectMonitors(
        _Context->onThread, vmThread, monitorInfos, _MaximumMonitorInfosPerThread);
 ...
 * @param vmThread[in] the thread to query (not the current thread)
 */
void JavaCoreDumpWriter::writeJavaLangThreadInfo(J9VMThread* vmThread)
{
...
void JavaCoreDumpWriter::writeCPUinfo(void)
{
    PORT_ACCESS_FROM_PORT(_PortLibrary);
    ...
    _OutputStream.writeCharacters("2CIONLNCPU     Online CPUs: ");
    _OutputStream.writeInteger(j9sysinfo_get_number_CPUs_by_type(J9PORT_CPU_ONLINE), "%i\n");
    _OutputStream.writeCharacters("2CIBOUNDCPU    Bound CPUs: ");
    _OutputStream.writeInteger(bound, "%i\n");
    _OutputStream.writeCharacters("2CIACTIVECPU   Active CPUs: ");
    if (bound != target) {
    ...
    _OutputStream.writeCharacters("2CITARGETCPU   Target CPUs: ");
    _OutputStream.writeInteger(target, "%i\n");
...
UDATA
writeExceptionFrameCallBack(J9VMThread* vmThread, void* userData, J9ROMClass* romClass, J9ROMMethod* romMethod,
    J9UTF8* sourceFile, UDATA lineNumber, J9ClassLoader* classLoader)
{
...
/**************************************************************************************************/
static jvmtiIterationControl heapIteratorCallback(
    J9JavaVM* virtualMachine, J9MM_IterateHeapDescriptor* heapDescriptor, void* userData)
{
    JavaCoreDumpWriter* jcw = (JavaCoreDumpWriter*)userData;

    virtualMachine->memoryManagerFunctions->j9mm_iterate_spaces(
        virtualMachine, virtualMachine->portLibrary, heapDescriptor, 0, spaceIteratorCallback, jcw);

    return JVMTI_ITERATION_CONTINUE;
}

static jvmtiIterationControl spaceIteratorCallback(
    J9JavaVM* virtualMachine, J9MM_IterateSpaceDescriptor* spaceDescriptor, void* userData)
{
    UDATA sizeTotal = 0;
    UDATA sizeTarget = 0;
    UDATA allocTotal = 0;
...
#else
    jcw->_OutputStream.writeCharacters("id         start      end        size       space");
#endif
    ...
    if (J9_GC_POLICY_BALANCED == ((OMR_VM*)virtualMachine->omrVM)->gcPolicy) {
        ...
        virtualMachine->memoryManagerFunctions->j9mm_iterate_regions(virtualMachine, virtualMachine->portLibrary,
            spaceDescriptor, j9mm_iterator_flag_regions_read_only, regionIteratorCallback, &regionTotals);
#endif /* J9VM_GC_VLHGC */
    } else {
        ...
        jcw->_OutputStream.writeCharacters(" \n");
        virtualMachine->memoryManagerFunctions->j9mm_iterate_regions(virtualMachine, virtualMachine->portLibrary,
            spaceDescriptor, j9mm_iterator_flag_regions_read_only, regionIteratorCallback, jcw);
    }
...
static jvmtiIterationControl regionIteratorCallback(
    J9JavaVM* virtualMachine, J9MM_IterateRegionDescriptor* regionDescriptor, void* userData)
{
    if (J9_GC_POLICY_BALANCED == ((OMR_VM*)virtualMachine->omrVM)->gcPolicy) {
        /*
...
protectedStartDoWithSignal(struct J9PortLibrary* portLibrary, void* args)
{
    ...
    return (UDATA)j9introspect_threads_startDo_with_signal(
        closure->heap, (J9ThreadWalkState*)closure->state, closure->gpInfo);
}
...
protectedGetVMThreadObjectState(struct J9PortLibrary* portLibrary, void* args)
{
    ...
    return (UDATA)getVMThreadObjectState(
        (J9VMThread*)parameters[0], (j9object_t*)parameters[1], (J9VMThread**)parameters[3], (UDATA*)parameters[4]);
}
...
protectedGetVMThreadRawState(struct J9PortLibrary* portLibrary, void* args)
{
    ...
    return (UDATA)getVMThreadRawState((J9VMThread*)parameters[0], (j9object_t*)parameters[1],
        (omrthread_monitor_t*)parameters[2], (J9VMThread**)parameters[3], (UDATA*)parameters[4]);
}
UDATA
protectedWalkJavaStack(struct J9PortLibrary* portLibrary, void* args)
{
    struct walkClosure* closure = (struct walkClosure*)args;
    return (UDATA)closure->jcw->_VirtualMachine->walkStackFrames(
        (J9VMThread*)closure->jcw->_Context->onThread, (J9StackWalkState*)closure->state);
}
...
protectedIterateStackTrace(struct J9PortLibrary* portLibrary, void* args)
{
    ...
    closure->jcw->_VirtualMachine->internalVMFunctions->iterateStackTrace(
        vmThread, (j9object_t*)parameters[1], writeExceptionFrameCallBack, (J9StackWalkState*)parameters[2], FALSE);
...
protectedWriteGCHistoryLines(struct J9PortLibrary* portLibrary, void* args)
{
    struct walkClosure* closure = (struct walkClosure*)args;
    void** parameters = (void**)closure->state;
    closure->jcw->writeGCHistoryLines(
        (UtThreadData**)parameters[0], (UtTracePointIterator*)parameters[1], (char*)parameters[2]);
    return 0;
}
...
handlerGetThreadsUsageInfo(struct J9PortLibrary* portLibrary, U_32 gpType, void* gpInfo, void* userData)
{
    ...
    jcw->_OutputStream.writeCharacters(
        "1INTERNAL                    Unable to obtain JVM threads CPU usage summary information\n");
...
handlerWriteStacks(struct J9PortLibrary* portLibrary, U_32 gpType, void* gpInfo, void* userData)
{
    ...
    if (jcw->_ThreadsWalkStarted == true) {
        jcw->_OutputStream.writeCharacters(
            "1INTERNAL     Unable to walk threads. Some or all threads may have been omitted.\n");
    } else {
...
static UDATA lockHashEqualFunction(void* left, void* right, void* user)
{
    return ((JavaCoreDumpWriter::DeadLockGraphNode*)left)->thread
        == ((JavaCoreDumpWriter::DeadLockGraphNode*)right)->thread;
}

/* Primary entry point */
extern "C" void runJavadump(char* label, J9RASdumpContext* context, J9RASdumpAgent* agent)
{
