...
UDATA rasDumpPostponeHooks = J9RAS_DUMP_ON_CLASS_UNLOAD | J9RAS_DUMP_ON_GLOBAL_GC | J9RAS_DUMP_ON_SLOW_EXCLUSIVE_ENTER
    | J9RAS_DUMP_ON_OBJECT_ALLOCATION | J9RAS_DUMP_ON_EXCESSIVE_GC | J9RAS_DUMP_ON_THREAD_START
    | J9RAS_DUMP_ON_THREAD_BLOCKED | J9RAS_DUMP_ON_THREAD_END;
...
static void rasDumpHookAllocationThreshold(
...
static void rasDumpHookExceptionDescribe(
    J9HookInterface** hookInterface, UDATA eventNum, void* eventData, void* userData);
...
static J9RASdumpMatchResult matchesFilter(
    J9VMThread* vmThread, J9RASdumpEventData* eventData, UDATA eventFlags, char* filter, char* subFilter);
static void rasDumpHookExceptionSysthrow PROTOTYPE(
    (J9HookInterface * *hookInterface, UDATA eventNum, void* eventData, void* userData));
...
static void rasDumpHookMonitorContendedEnter(
    J9HookInterface** hookInterface, UDATA eventNum, void* eventData, void* userData);
...
static UDATA countExceptionStackFrame(J9VMThread* vmThread, void* userData, J9ROMClass* romClass,
...
static J9RASdumpMatchResult matchesObjectAllocationFilter(J9RASdumpEventData* eventData, char* filter)
{
...
static J9RASdumpMatchResult matchesSlowExclusiveEnterFilter(J9RASdumpEventData* eventData, char* filter)
{
    char* message = eventData->detailData;
    char* msgPtr;
    IDATA msgValue;
...
static J9RASdumpMatchResult matchesVMShutdownFilter(J9RASdumpEventData* eventData, char* filter)
{
    char* message = eventData->detailData;
    IDATA value;
...
static J9RASdumpMatchResult matchesExceptionFilter(
    J9VMThread* vmThread, J9RASdumpEventData* eventData, UDATA eventFlags, char* filter, char* subFilter)
{
        ...
        if (NULL != hashSignInFilter) {
            ...
            if (eventFlags & J9RAS_DUMP_ON_EXCEPTION_CATCH) {
                ...
                vmThread->javaVM->internalVMFunctions->iterateStackTrace(
                    vmThread, (j9object_t*)eventData->exceptionRef, countExceptionStackFrame, &throwSite, TRUE);
            }
        }
        ...
        if (throwSite.romClass && throwSite.romMethod) {
            ...
            if (throwClassName && throwMethodName) {
                if (stackOffsetFilter) {
                    buflen = J9UTF8_LENGTH(exceptionClassName) + J9UTF8_LENGTH(throwClassName)
                        + J9UTF8_LENGTH(throwMethodName) + strlen(stackOffsetFilter) + 3;
                } else {
                    buflen = J9UTF8_LENGTH(exceptionClassName) + J9UTF8_LENGTH(throwClassName)
                        + J9UTF8_LENGTH(throwMethodName) + 2;
                }
        ...
        if (eventData->exceptionRef && *eventData->exceptionRef) {
            ...
            if (NULL != emessage) {
                buf = vmThread->javaVM->internalVMFunctions->copyStringToUTF8WithMemAlloc(
                    vmThread, emessage, J9_STR_NULL_TERMINATE_RESULT, "", 0, stackBuffer, 256, &buflen);
...
omr_error_t printLabelSpec(struct J9JavaVM* vm)
{
    PORT_ACCESS_FROM_JAVAVM(vm);
    /* Since j9tty_err_printf() is a function macro, #ifdefs can't be used in
     * its argument list.
     */
    const char* labelSpec = "  %%Y     year    1900..????\n"
                            "  %%y     century   00..99\n"
                            "  %%m     month 	01..12\n"
                            "  %%d     day   	01..31\n"
                            "  %%H     hour  	00..23\n"
...
prepareForDump(struct J9JavaVM* vm, struct J9RASdumpAgent* agent, struct J9RASdumpContext* context, UDATA state)
{
    ...
    if ((context->eventFlags & (J9RAS_DUMP_ON_GP_FAULT | J9RAS_DUMP_ON_ABORT_SIGNAL | J9RAS_DUMP_ON_TRACE_ASSERT))
        == 0) {
        /*
        ...
        if (shareVMAccess == 0) {
            ...
            if ((agent->requestMask
                    & (J9RAS_DUMP_DO_PREPARE_HEAP_FOR_WALK | J9RAS_DUMP_DO_COMPACT_HEAP | J9RAS_DUMP_DO_ATTACH_THREAD))
                && (context->eventFlags & J9RAS_DUMP_ON_USER_SIGNAL)) {
    ...
    if ((agent->requestMask & J9RAS_DUMP_DO_COMPACT_HEAP) && ((state & J9RAS_DUMP_HEAP_COMPACTED) == 0)) {
        ...
        if ((newState & J9RAS_DUMP_GOT_EXCLUSIVE_VM_ACCESS) && (vmThread != 0)) {
            ...
            UDATA gcEvent = (context->eventFlags & J9RAS_DUMP_ON_GLOBAL_GC)
                || (context->eventFlags & J9RAS_DUMP_ON_CLASS_UNLOAD)
                || (context->eventFlags & J9RAS_DUMP_ON_EXCESSIVE_GC)
                || (eventData
                       && matchesFilter(vmThread, eventData, context->eventFlags, "*OutOfMemoryError", NULL)
                           == J9RAS_DUMP_MATCH)
                ||
                /* Tracepoint trigger when exclusive is held also indicates we may be in GC */
                (eventData && eventData->detailData && strcmp(eventData->detailData, "-Xtrace:trigger") == 0
                    && exclusiveHeld);

            /*
             * The extra check of this runtime flag is to defer invoking GC till class objects are assigned during
             * startup; otherwise NULL class objects would be captured by GC assertion.
             */
            if (J9_ARE_ALL_BITS_SET(vm->extendedRuntimeFlags, J9_EXTENDED_RUNTIME_CLASS_OBJECT_ASSIGNED) && !gcEvent) {
                vm->memoryManagerFunctions->j9gc_modron_global_collect_with_overrides(
                    vmThread, J9MMCONSTANT_EXPLICIT_GC_RASDUMP_COMPACT);
                newState |= J9RAS_DUMP_HEAP_COMPACTED;
            }
        }
    }

    if ((agent->requestMask & J9RAS_DUMP_DO_PREPARE_HEAP_FOR_WALK) && ((state & J9RAS_DUMP_HEAP_PREPARED) == 0)) {
    ...
    if ((agent->requestMask & J9RAS_DUMP_DO_HALT_ALL_THREADS) && (state & J9RAS_DUMP_THREADS_HALTED) == 0) {
    ...
    /* For fatal events, the first failing thread sets the global rasDumpFirstThread. It then gets higher priority on
     * the serial dump lock, see below. This allows the first failing thread to complete its dumps and exit the VM,
     * reducing the number of dumps written and out-time if multiple threads crash.
     */
    if (0
        != (context->eventFlags & (J9RAS_DUMP_ON_GP_FAULT | J9RAS_DUMP_ON_ABORT_SIGNAL | J9RAS_DUMP_ON_TRACE_ASSERT))) {
 ...
 */
omr_error_t dumpLabel(struct J9JavaVM* vm, J9RASdumpAgent* agent, J9RASdumpContext* context, char* buf, size_t len,
    UDATA* reqLen, I_64 now)
{
    /* Monotonic counter */
    ...
    /* %vmbin is not listed in printLabelSpec as it is only useful for loading internal tools that live in the vm
     * directory. */
    if (j9str_set_token(PORTLIB, stringTokens, "vmbin", "%s",
            (vm->j2seRootDirectory == NULL) ? "" : (char*)vm->j2seRootDirectory)) {
        omrthread_monitor_exit(dump_storage->dumpLabelTokensMutex);
        return OMR_ERROR_INTERNAL;
    }
...
#if (defined(J9VM_RAS_DUMP_AGENTS))
omr_error_t triggerOneOffDump(
    struct J9JavaVM* vm, char* optionString, char* caller, char* fileName, size_t fileNameLength)
{
    IDATA kind;
...
#if (defined(J9VM_RAS_DUMP_AGENTS))
omr_error_t triggerDumpAgents(
    struct J9JavaVM* vm, struct J9VMThread* self, UDATA eventFlags, struct J9RASdumpEventData* eventData)
{
    ...
    if (FIND_DUMP_QUEUE(vm, queue)) {
        ...
        for (node = queue->agents; node != NULL; node = node->nextPtr) {
            if (eventFlags & node->eventMask) {
                if (node->dumpFn == doToolDump) {
                    ...
                    if (node->dumpFn == doHeapDump && strstr(node->dumpOptions, "CLASSIC")
                        && strstr(node->dumpOptions, "PHD")) {
        ...
        if (toolDumpFound && (dumpAgentCount > 0)) {
            /* there is a tool dump, so allocate a buffer for the list of dump labels. Need to account for the \t
             * separators and \0 */
        ...
        for (node = queue->agents; node != NULL; node = node->nextPtr) {
            if (eventFlags & node->eventMask) {
                ...
                if (NULL == eventData
                    || matchesFilter(self, eventData, eventFlags, node->detailFilter, node->subFilter)
                        != J9RAS_DUMP_NO_MATCH) {
                        ...
                        if (printed == 0) {
                            if (node->dumpFn != doSilentDump) {
                                ...
                                j9nls_printf(PORTLIB, J9NLS_INFO | J9NLS_STDERR | J9NLS_VITAL,
                                    J9NLS_DMP_PROCESSING_EVENT_TIME, mapDumpEvent(eventFlags), detailLength, detailData,
                                    dateStamp);
                                printed = 1;
                            }
                        }
        ...
        if (dumpTaken == 1) {
            ...
            if (printed == 1) {
                j9nls_printf(PORTLIB, J9NLS_INFO | J9NLS_STDERR, J9NLS_DMP_PROCESSED_EVENT_STR,
                    mapDumpEvent(eventFlags), detailLength, detailData);
            }
        }
...
omr_error_t rasDumpEnableHooks(J9JavaVM* vm, UDATA eventFlags)
{
    ...
    if (eventFlags & hookFlags) {
        ...
        J9HookInterface** gcOmrHooks
            = vm->memoryManagerFunctions ? vm->memoryManagerFunctions->j9gc_get_omr_hook_interface(vm->omrVM) : NULL;
        ...
        *pendingHooks |= skip;
        ...
        if (eventFlags & J9RAS_DUMP_ON_VM_STARTUP) {
            rc = (*vmHooks)->J9HookRegisterWithCallSite(
                vmHooks, J9HOOK_VM_INITIALIZED, rasDumpHookVmInit, OMR_GET_CALLSITE(), NULL);
        }
        if (eventFlags & J9RAS_DUMP_ON_VM_SHUTDOWN) {
            rc = (*vmHooks)->J9HookRegisterWithCallSite(
                vmHooks, J9HOOK_VM_SHUTTING_DOWN, rasDumpHookVmShutdown, OMR_GET_CALLSITE(), NULL);
        }
        if (eventFlags & J9RAS_DUMP_ON_CLASS_LOAD) {
            rc = (*vmHooks)->J9HookRegisterWithCallSite(
                vmHooks, J9HOOK_VM_CLASS_LOAD, rasDumpHookClassLoad, OMR_GET_CALLSITE(), NULL);
        }
#if defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING)
        if (eventFlags & J9RAS_DUMP_ON_CLASS_UNLOAD) {
            rc = (*vmHooks)->J9HookRegisterWithCallSite(
                vmHooks, J9HOOK_VM_CLASSES_UNLOAD, rasDumpHookClassesUnload, OMR_GET_CALLSITE(), NULL);
        }
#endif
        if (eventFlags & J9RAS_DUMP_ON_EXCEPTION_SYSTHROW) {
            rc = (*vmHooks)->J9HookRegisterWithCallSite(
                vmHooks, J9HOOK_VM_EXCEPTION_SYSTHROW, rasDumpHookExceptionSysthrow, OMR_GET_CALLSITE(), NULL);
        }
        if (eventFlags & J9RAS_DUMP_ON_EXCEPTION_THROW) {
            rc = (*vmHooks)->J9HookRegisterWithCallSite(
                vmHooks, J9HOOK_VM_EXCEPTION_THROW, rasDumpHookExceptionThrow, OMR_GET_CALLSITE(), NULL);
        }
        if (eventFlags & J9RAS_DUMP_ON_EXCEPTION_CATCH) {
            rc = (*vmHooks)->J9HookRegisterWithCallSite(
                vmHooks, J9HOOK_VM_EXCEPTION_CATCH, rasDumpHookExceptionCatch, OMR_GET_CALLSITE(), NULL);
        }
        if (eventFlags & J9RAS_DUMP_ON_THREAD_START) {
            rc = (*vmHooks)->J9HookRegisterWithCallSite(
                vmHooks, J9HOOK_VM_THREAD_STARTED, rasDumpHookThreadStart, OMR_GET_CALLSITE(), NULL);
        }
        if (eventFlags & J9RAS_DUMP_ON_THREAD_BLOCKED) {
            rc = (*vmHooks)->J9HookRegisterWithCallSite(
                vmHooks, J9HOOK_VM_MONITOR_CONTENDED_ENTER, rasDumpHookMonitorContendedEnter, OMR_GET_CALLSITE(), NULL);
        }
        if (eventFlags & J9RAS_DUMP_ON_THREAD_END) {
            rc = (*vmHooks)->J9HookRegisterWithCallSite(
                vmHooks, J9HOOK_VM_THREAD_END, rasDumpHookThreadEnd, OMR_GET_CALLSITE(), NULL);
        }
        if (eventFlags & J9RAS_DUMP_ON_GLOBAL_GC) {
            rc = (*gcOmrHooks)
                     ->J9HookRegisterWithCallSite(
                         gcOmrHooks, J9HOOK_MM_OMR_GLOBAL_GC_START, rasDumpHookGlobalGcStart, OMR_GET_CALLSITE(), NULL);
        }
        if (eventFlags & J9RAS_DUMP_ON_EXCEPTION_DESCRIBE) {
            rc = (*vmHooks)->J9HookRegisterWithCallSite(
                vmHooks, J9HOOK_VM_EXCEPTION_DESCRIBE, rasDumpHookExceptionDescribe, OMR_GET_CALLSITE(), NULL);
        }
        if (eventFlags & J9RAS_DUMP_ON_SLOW_EXCLUSIVE_ENTER) {
            /* wouldn't this event be better handled by a tracepoint? */
            rc = (*vmHooks)->J9HookRegisterWithCallSite(
                vmHooks, J9HOOK_VM_SLOW_EXCLUSIVE, rasDumpHookSlowExclusive, OMR_GET_CALLSITE(), NULL);
        }
        if (eventFlags & J9RAS_DUMP_ON_OBJECT_ALLOCATION) {
            rc = (*gcOmrHooks)
                     ->J9HookRegisterWithCallSite(
                         gcOmrHooks, J9HOOK_MM_OMR_INITIALIZED, rasDumpHookGCInitialized, OMR_GET_CALLSITE(), NULL);
            rc = (*vmHooks)->J9HookRegisterWithCallSite(vmHooks, J9HOOK_VM_OBJECT_ALLOCATE_WITHIN_THRESHOLD,
                rasDumpHookAllocationThreshold, OMR_GET_CALLSITE(), NULL);
        }
        if (eventFlags & J9RAS_DUMP_ON_CORRUPT_CACHE) {
            rc = (*vmHooks)->J9HookRegisterWithCallSite(
                vmHooks, J9HOOK_VM_CORRUPT_CACHE, rasDumpHookCorruptCache, OMR_GET_CALLSITE(), NULL);
        }
        if (eventFlags & J9RAS_DUMP_ON_EXCESSIVE_GC) {
            rc = (*gcOmrHooks)
                     ->J9HookRegisterWithCallSite(gcOmrHooks, J9HOOK_MM_OMR_EXCESSIVEGC_RAISED, rasDumpHookExcessiveGC,
...
static void rasDumpHookVmInit(J9HookInterface** hookInterface, UDATA eventNum, void* eventData, void* userData)
{
    ...
    vmThread->javaVM->j9rasDumpFunctions->triggerDumpAgents(vmThread->javaVM, vmThread, J9RAS_DUMP_ON_VM_STARTUP, NULL);
}
...
static void rasDumpHookVmShutdown(J9HookInterface** hookInterface, UDATA eventNum, void* eventData, void* userData)
{
    ...
    vmThread->javaVM->j9rasDumpFunctions->triggerDumpAgents(
        vmThread->javaVM, vmThread, J9RAS_DUMP_ON_VM_SHUTDOWN, &dumpData);
}
...
static void rasDumpHookClassLoad(J9HookInterface** hookInterface, UDATA eventNum, void* eventData, void* userData)
{
    ...
    vmThread->javaVM->j9rasDumpFunctions->triggerDumpAgents(
        vmThread->javaVM, vmThread, J9RAS_DUMP_ON_CLASS_LOAD, &dumpData);
}
...
static void rasDumpHookExceptionThrow(J9HookInterface** hookInterface, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (globalRef != NULL) {
        ...
        vm->j9rasDumpFunctions->triggerDumpAgents(vm, vmThread, J9RAS_DUMP_ON_EXCEPTION_THROW, &dumpData);
...
static void rasDumpHookExceptionCatch(J9HookInterface** hookInterface, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (globalRef != NULL) {
        ...
        vm->j9rasDumpFunctions->triggerDumpAgents(vm, vmThread, J9RAS_DUMP_ON_EXCEPTION_CATCH, &dumpData);
...
static void rasDumpHookThreadStart(J9HookInterface** hookInterface, UDATA eventNum, void* eventData, void* userData)
{
    ...
    vmThread->javaVM->j9rasDumpFunctions->triggerDumpAgents(
        vmThread->javaVM, vmThread, J9RAS_DUMP_ON_THREAD_START, NULL);
}
...
static void rasDumpHookThreadEnd(J9HookInterface** hookInterface, UDATA eventNum, void* eventData, void* userData)
{
    ...
    vmThread->javaVM->j9rasDumpFunctions->triggerDumpAgents(vmThread->javaVM, vmThread, J9RAS_DUMP_ON_THREAD_END, NULL);
}
    ...
    J9HookInterface** hookInterface, UDATA eventNum, void* eventData, void* userData)
{
    ...
    vmThread->javaVM->j9rasDumpFunctions->triggerDumpAgents(
        vmThread->javaVM, vmThread, J9RAS_DUMP_ON_THREAD_BLOCKED, NULL);
}
    ...
    J9HookInterface** hookInterface, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (globalRef != NULL) {
        ...
        vm->j9rasDumpFunctions->triggerDumpAgents(vm, vmThread, J9RAS_DUMP_ON_EXCEPTION_DESCRIBE, &dumpData);
    ...
    J9HookInterface** hookInterface, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (J9ROMCLASS_IS_ARRAY(romClass)) {
        ...
        length = j9str_printf(PORTLIB, details, sizeof(details), "%zu bytes, type %.*s", data->size,
            J9UTF8_LENGTH(leafClassName), J9UTF8_DATA(leafClassName));
    ...
    vmThread->javaVM->j9rasDumpFunctions->triggerDumpAgents(
        vmThread->javaVM, vmThread, J9RAS_DUMP_ON_OBJECT_ALLOCATION, &dumpData);
...
static void rasDumpHookSlowExclusive(J9HookInterface** hookInterface, UDATA eventNum, void* eventData, void* userData)
{
    ...
    vmThread->javaVM->j9rasDumpFunctions->triggerDumpAgents(
        vmThread->javaVM, vmThread, J9RAS_DUMP_ON_SLOW_EXCLUSIVE_ENTER, &dumpData);
}
...
static void rasDumpHookGlobalGcStart(J9HookInterface** hookInterface, UDATA eventNum, void* eventData, void* userData)
{
    ...
    vmThread->javaVM->j9rasDumpFunctions->triggerDumpAgents(vmThread->javaVM, vmThread, J9RAS_DUMP_ON_GLOBAL_GC, NULL);
}
...
static void rasDumpHookClassesUnload(J9HookInterface** hookInterface, UDATA eventNum, void* eventData, void* userData)
{
    ...
    vmThread->javaVM->j9rasDumpFunctions->triggerDumpAgents(
        vmThread->javaVM, vmThread, J9RAS_DUMP_ON_CLASS_UNLOAD, NULL);
}
#endif /* J9VM_GC_DYNAMIC_CLASS_UNLOADING */

#define CDEV_CURRENT_FUNCTION rasDumpHookExceptionSysthrow
static void rasDumpHookExceptionSysthrow(
    J9HookInterface** hookInterface, UDATA eventNum, void* eventData, void* userData)
{
    J9RASdumpEventData dumpData;
    J9VMExceptionThrowEvent* data = eventData;
    ...
    if (globalRef != NULL) {
        ...
        vm->j9rasDumpFunctions->triggerDumpAgents(vm, vmThread, J9RAS_DUMP_ON_EXCEPTION_SYSTHROW, &dumpData);
...
static void rasDumpHookCorruptCache(J9HookInterface** hookInterface, UDATA eventNum, void* eventData, void* userData)
{
    ...
    vmThread->javaVM->j9rasDumpFunctions->triggerDumpAgents(
        vmThread->javaVM, vmThread, J9RAS_DUMP_ON_CORRUPT_CACHE, NULL);
}
...
static void rasDumpHookExcessiveGC(J9HookInterface** hookInterface, UDATA eventNum, void* eventData, void* userData)
{
    ...
    vmThread->javaVM->j9rasDumpFunctions->triggerDumpAgents(
        vmThread->javaVM, vmThread, J9RAS_DUMP_ON_EXCESSIVE_GC, NULL);
}
