...
static void traceMethodExit(
    J9VMThread* thr, J9Method* method, UDATA isCompiled, void* returnValuePtr, UDATA doParameters);
static void traceMethodExitX(
    J9VMThread* thr, J9Method* method, UDATA isCompiled, void* exceptionPtr, UDATA doParameters);
...
static char* traceMethodReturnVal(
    J9VMThread* thr, J9UTF8* signature, void* returnValuePtr, char* buf, const char* endOfBuf);
static void traceMethodArgBoolean(J9VMThread* thr, UDATA* arg0EA, char* cursor, UDATA length);
static void traceMethodEnter(
    J9VMThread* thr, J9Method* method, void* receiverAddress, UDATA isCompiled, UDATA doParameters);
...
matchMethod(RasMethodTable* methodTable, J9Method* method)
{
    ...
    if (methodTable->methodName == NULL) {
        if (methodTable->className == NULL) {
            ...
            return wildcardMatch(methodTable->classMatchFlag, (const char*)J9UTF8_DATA(methodTable->className),
                J9UTF8_LENGTH(methodTable->className), (const char*)J9UTF8_DATA(className), J9UTF8_LENGTH(className));
        }
    }
    if (methodTable->className == NULL) {
        /*
        ...
        methodName
            = J9ROMMETHOD_GET_NAME(J9_CLASS_FROM_METHOD(method)->romClass, J9_ROM_METHOD_FROM_RAM_METHOD(method));
        return wildcardMatch(methodTable->methodMatchFlag, (const char*)J9UTF8_DATA(methodTable->methodName),
            J9UTF8_LENGTH(methodTable->methodName), (const char*)J9UTF8_DATA(methodName), J9UTF8_LENGTH(methodName));
    } else {
        /*
        ...
        methodName
            = J9ROMMETHOD_GET_NAME(J9_CLASS_FROM_METHOD(method)->romClass, J9_ROM_METHOD_FROM_RAM_METHOD(method));
        if (wildcardMatch(methodTable->methodMatchFlag, (const char*)J9UTF8_DATA(methodTable->methodName),
                J9UTF8_LENGTH(methodTable->methodName), (const char*)J9UTF8_DATA(methodName),
    ...
    J9VMThread* thr, J9Method* method, void* receiverAddress, UDATA isCompiled, UDATA doParameters)
{
    ...
    if (isCompiled) {
        if (modifiers & J9AccStatic) {
            Trc_MethodEntryCS(thr, J9UTF8_LENGTH(className), J9UTF8_DATA(className), J9UTF8_LENGTH(methodName),
            ...
            Trc_MethodEntryC(thr, J9UTF8_LENGTH(className), J9UTF8_DATA(className), J9UTF8_LENGTH(methodName),
                J9UTF8_DATA(methodName), J9UTF8_LENGTH(methodSignature), J9UTF8_DATA(methodSignature), receiver);
        }
    } else {
        char buf[1024];
        ...
        if (modifiers & J9AccStatic) {
            if (modifiers & J9AccNative) {
                Trc_MethodEntryNS(thr, J9UTF8_LENGTH(className), J9UTF8_DATA(className), J9UTF8_LENGTH(methodName),
                    J9UTF8_DATA(methodName), J9UTF8_LENGTH(methodSignature), J9UTF8_DATA(methodSignature));
            } else {
                Trc_MethodEntryS(thr, J9UTF8_LENGTH(className), J9UTF8_DATA(className), J9UTF8_LENGTH(methodName),
                    J9UTF8_DATA(methodName), J9UTF8_LENGTH(methodSignature), J9UTF8_DATA(methodSignature));
            }
            if (doParameters) {
                Trc_MethodArgumentsS(
                    thr, traceMethodArguments(thr, methodSignature, (UDATA*)receiverAddress, buf, buf + sizeof(buf)));
            }
        } else {
            j9object_t receiver = *(j9object_t*)receiverAddress;
            ...
            if (modifiers & J9AccNative) {
                Trc_MethodEntryN(thr, J9UTF8_LENGTH(className), J9UTF8_DATA(className), J9UTF8_LENGTH(methodName),
                    J9UTF8_DATA(methodName), J9UTF8_LENGTH(methodSignature), J9UTF8_DATA(methodSignature), receiver);
            } else {
                Trc_MethodEntry(thr, J9UTF8_LENGTH(className), J9UTF8_DATA(className), J9UTF8_LENGTH(methodName),
                    J9UTF8_DATA(methodName), J9UTF8_LENGTH(methodSignature), J9UTF8_DATA(methodSignature), receiver);
            }

            if (doParameters) {
                Trc_MethodArguments(thr, (U_32)J9UTF8_LENGTH(receiverClassName), J9UTF8_DATA(receiverClassName),
                    receiver,
    ...
    J9VMThread* thr, J9Method* method, UDATA isCompiled, void* returnValuePtr, UDATA doParameters)
{
    ...
    if (isCompiled) {
        if (modifiers & J9AccStatic) {
            Trc_MethodExitCS(thr, J9UTF8_LENGTH(className), J9UTF8_DATA(className), J9UTF8_LENGTH(methodName),
                J9UTF8_DATA(methodName), J9UTF8_LENGTH(methodSignature), J9UTF8_DATA(methodSignature));
        } else {
            Trc_MethodExitC(thr, J9UTF8_LENGTH(className), J9UTF8_DATA(className), J9UTF8_LENGTH(methodName),
                J9UTF8_DATA(methodName), J9UTF8_LENGTH(methodSignature), J9UTF8_DATA(methodSignature));
        }
    } else {
        if (modifiers & J9AccNative) {
            if (modifiers & J9AccStatic) {
                Trc_MethodExitNS(thr, J9UTF8_LENGTH(className), J9UTF8_DATA(className), J9UTF8_LENGTH(methodName),
                    J9UTF8_DATA(methodName), J9UTF8_LENGTH(methodSignature), J9UTF8_DATA(methodSignature));
            } else {
                Trc_MethodExitN(thr, J9UTF8_LENGTH(className), J9UTF8_DATA(className), J9UTF8_LENGTH(methodName),
                    J9UTF8_DATA(methodName), J9UTF8_LENGTH(methodSignature), J9UTF8_DATA(methodSignature));
            }
        } else {
            if (modifiers & J9AccStatic) {
                Trc_MethodExitS(thr, J9UTF8_LENGTH(className), J9UTF8_DATA(className), J9UTF8_LENGTH(methodName),
                    J9UTF8_DATA(methodName), J9UTF8_LENGTH(methodSignature), J9UTF8_DATA(methodSignature));
            } else {
                Trc_MethodExit(thr, J9UTF8_LENGTH(className), J9UTF8_DATA(className), J9UTF8_LENGTH(methodName),
                    J9UTF8_DATA(methodName), J9UTF8_LENGTH(methodSignature), J9UTF8_DATA(methodSignature));
            }
        }
    }
    ...
    J9VMThread* thr, J9Method* method, UDATA isCompiled, void* exceptionPtr, UDATA doParameters)
{
    ...
    if (isCompiled) {
        if (modifiers & J9AccStatic) {
            Trc_MethodExitCXS(thr, J9UTF8_LENGTH(className), J9UTF8_DATA(className), J9UTF8_LENGTH(methodName),
                J9UTF8_DATA(methodName), J9UTF8_LENGTH(methodSignature), J9UTF8_DATA(methodSignature));
        } else {
            Trc_MethodExitCX(thr, J9UTF8_LENGTH(className), J9UTF8_DATA(className), J9UTF8_LENGTH(methodName),
                J9UTF8_DATA(methodName), J9UTF8_LENGTH(methodSignature), J9UTF8_DATA(methodSignature));
        }
    } else {
        if (modifiers & J9AccNative) {
            if (modifiers & J9AccStatic) {
                Trc_MethodExitNXS(thr, J9UTF8_LENGTH(className), J9UTF8_DATA(className), J9UTF8_LENGTH(methodName),
                    J9UTF8_DATA(methodName), J9UTF8_LENGTH(methodSignature), J9UTF8_DATA(methodSignature));
            } else {
                Trc_MethodExitNX(thr, J9UTF8_LENGTH(className), J9UTF8_DATA(className), J9UTF8_LENGTH(methodName),
                    J9UTF8_DATA(methodName), J9UTF8_LENGTH(methodSignature), J9UTF8_DATA(methodSignature));
            }
        } else {
            if (modifiers & J9AccStatic) {
                Trc_MethodExitXS(thr, J9UTF8_LENGTH(className), J9UTF8_DATA(className), J9UTF8_LENGTH(methodName),
                    J9UTF8_DATA(methodName), J9UTF8_LENGTH(methodSignature), J9UTF8_DATA(methodSignature));
            } else {
                Trc_MethodExitX(thr, J9UTF8_LENGTH(className), J9UTF8_DATA(className), J9UTF8_LENGTH(methodName),
                    J9UTF8_DATA(methodName), J9UTF8_LENGTH(methodSignature), J9UTF8_DATA(methodSignature));
            }
        }
    }
...
omr_error_t enableMethodTraceHooks(J9JavaVM* vm)
{
    ...
    if ((*hook)->J9HookRegisterWithCallSite(
            hook, J9HOOK_VM_INTERNAL_CLASS_LOAD, hookRAMClassLoad, OMR_GET_CALLSITE(), NULL)) {
...
static void traceMethodArgObject(J9VMThread* thr, UDATA* arg0EA, char* cursor, UDATA length)
{
    ...
    if (object == NULL) {
        ...
        j9str_printf(
            PORTLIB, cursor, length, "%.*s@%p", (U_32)J9UTF8_LENGTH(className), J9UTF8_DATA(className), object);
    }
}
