...
static void uncompressedStackFrameFormatter(J9VMThread* vmThread, J9Method* method, J9UTF8* className,
    J9UTF8* methodName, J9UTF8* sourceFile, UDATA lineNumber, UDATA programCounter, StackFrameMethodType frameType,
    UDATA frameCount);
static void compressionLevel1StackFrameFormatter(J9VMThread* vmThread, J9Method* method, J9UTF8* className,
    J9UTF8* methodName, J9UTF8* sourceFile, UDATA lineNumber, UDATA programCounter, StackFrameMethodType frameType,
    UDATA frameCount);
static void compressionLevel2StackFrameFormatter(J9VMThread* vmThread, J9Method* method, J9UTF8* className,
    J9UTF8* methodName, J9UTF8* sourceFile, UDATA lineNumber, UDATA programCounter, StackFrameMethodType frameType,
    UDATA frameCount);

typedef void (*StackTraceFormattingFunction)(J9VMThread* vmThread, J9Method* method, J9UTF8* className,
    J9UTF8* methodName, J9UTF8* sourceFile, UDATA lineNumber, UDATA programCounter, StackFrameMethodType frameType,
    UDATA frameCount);
...
static const StackTraceFormattingFunction stackTraceFormattingFunctions[]
    = { uncompressedStackFrameFormatter, compressionLevel1StackFrameFormatter, compressionLevel2StackFrameFormatter };

#define NUM_STACK_TRACE_FORMATTING_FUNCTIONS \
    (sizeof(stackTraceFormattingFunctions) / sizeof(stackTraceFormattingFunctions[0]))
 ...
 * returns     - void
 *************************************************************************/
void rasTriggerMethod(J9VMThread* thr, J9Method* method, I_32 entry, const TriggerPhase phase)
{
    PORT_ACCESS_FROM_VMC(thr);
                ...
                if ((entry && rule->entryAction && rule->entryAction->phase == phase)
                    ...
                    if (oldDelay == 0) {
                        ...
                        do {
                            ...
                            compareAndSwapU32((U_32*)&rule->match, (U_32)oldMatch, (U_32)(oldMatch - 1)) != oldMatch);
    ...
    if (count != 0) {
        if (((mt = j9mem_allocate_memory(sizeof(RasMethodTable) * count, OMRMEM_CATEGORY_TRACE)) != NULL)
            ...
            for (i = 1; i <= count; i++, mt++) {
                ...
                if (length >= 3 && spec[length - 2] == RAS_INPUT_RETURN_CHAR_START
                ...
                if (strchr(spec, RAS_INPUT_RETURN_CHAR_START) || strchr(spec, RAS_INPUT_RETURN_CHAR_END)) {
                ...
                if (NULL != method) {
                    if (strchr(method, RAS_CLASS_METHOD_SEPARATOR) != NULL) {
                        vaReportJ9VMCommandLineError(PORTLIB,
                            "Invalid pattern in method trace specification: '.' "
                ...
                if ((OMR_ERROR_NONE != setMethodSpec(vm, class, &mt->className, &mt->classMatchFlag))
 ...
 * returns     - void
 *************************************************************************/
static int decimalString2Int(J9PortLibrary* portLibrary, const char* decString, I_32 signedAllowed, omr_error_t* rc)
{
    ...
    if (OMR_ERROR_NONE == *rc) {
         ...
         */
        if ((*p != ',') && (*p != '}') && (*p != '\0') && (*p != ' ')) {
            vaReportJ9VMCommandLineError(
                portLibrary, "Invalid character(s) encountered in decimal number \"%s\".", decString);
            *rc = OMR_ERROR_INTERNAL;
        }
    }
 ...
 * returns     - JNI return code
 *************************************************************************/
static omr_error_t addTriggeredMethodSpec(J9VMThread* thr, const char* ptrMethodSpec,
    const struct RasTriggerAction* entryAction, const struct RasTriggerAction* exitAction, int delay, int match)
{
    PORT_ACCESS_FROM_VMC(thr);
    omr_error_t rc = OMR_ERROR_NONE;
    RasMethodTable* mt;
 ...
 * returns     - JNI return code
 *************************************************************************/
omr_error_t processTriggerMethodClause(OMR_VMThread* omrThr, char* clause, BOOLEAN atRuntime)
{
    ...
    if (numParms > 5) {
        vaReportJ9VMCommandLineError(PORTLIB,
            "Too many parameters on trigger property "
            "method clause usage: "
    ...
    if ((OMR_ERROR_NONE == rc) && (NULL != ptrMethodSpec) && (0 == strlen(ptrMethodSpec))) {
        vaReportJ9VMCommandLineError(
            PORTLIB, "Method specification on trigger property (method clause) may not be null.");
        rc = OMR_ERROR_INTERNAL;
    }
    ...
    if (OMR_ERROR_NONE == rc) {
        ...
        for (cp = ptrMethodSpec; *cp != 0; cp++) {
            if ((*cp == RAS_INPUT_RETURN_CHAR_START) || /* '(' character */
                ...
                vaReportJ9VMCommandLineError(
                    PORTLIB, "Method specification for trigger may not include '!', '(' or ')'.");
                rc = OMR_ERROR_INTERNAL;
                break;
            }
        }
...
err:
    vaReportJ9VMCommandLineError(PORTLIB, "stackcompressionlevel takes an unsigned integer value from 0 to %d",
        NUM_STACK_TRACE_FORMATTING_FUNCTIONS - 1);
 ...
 * returns     - void
 *************************************************************************/
void doTriggerActionJstacktrace(OMR_VMThread* omrThr)
{
    J9VMThread* thr = (J9VMThread*)omrThr->_language_vmthread;
    ...
    stackWalkState.flags = J9_STACKWALK_ITERATE_FRAMES | J9_STACKWALK_INCLUDE_NATIVES | J9_STACKWALK_VISIBLE_ONLY
...
static UDATA traceFrameCallBack(J9VMThread* vmThread, J9StackWalkState* state)
{
    ...
    const unsigned int stackCompressionLevel
        = ((RasGlobalStorage*)vmThread->javaVM->j9rasGlobalStorage)->stackCompressionLevel;
    ...
    stackTraceFormatter(
        vmThread, method, className, methodName, sourceFile, lineNumber, offsetPC, frameType, frameCount);
 ...
 */
static void slashesToDots(char* const buf, const char* const end)
{
    char* p = buf;
 ...
 */
static void uncompressedStackFrameFormatter(J9VMThread* vmThread, J9Method* method, J9UTF8* className,
    J9UTF8* methodName, J9UTF8* sourceFile, UDATA lineNumber, UDATA offsetPC, StackFrameMethodType frameType,
    UDATA frameCount)
{
    ...
    cursor += j9str_printf(PORTLIB, cursor, end - cursor, "%.*s.%.*s", J9UTF8_LENGTH(className), J9UTF8_DATA(className),
...
static char getFrameTypeChar(const StackFrameMethodType type)
{
    switch (type) {
    case NATIVE_METHOD:
        return 'N';
    ...
    UDATA frameCount)
{
    ...
    if (NATIVE_METHOD == frameType) {
        ...
        if (sourceFile) {
            if (lineNumber != -1) {
                Trc_CompressedJavaStackFrame_WithFileAndLine(vmThread, frameCount, method, typeChar,
                    J9UTF8_LENGTH(sourceFile), J9UTF8_DATA(sourceFile), lineNumber);
            } else {
                Trc_CompressedJavaStackFrame_WithFile(
                    vmThread, frameCount, method, typeChar, J9UTF8_LENGTH(sourceFile), J9UTF8_DATA(sourceFile));
            }
        } else {
 ...
 * returns     - JNI return code
 *************************************************************************/
omr_error_t setStackDepth(J9JavaVM* vm, const char* str, BOOLEAN atRuntime)
{
    PORT_ACCESS_FROM_JAVAVM(vm);
    int value, length;
    omr_error_t rc = OMR_ERROR_NONE;
