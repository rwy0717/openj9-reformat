    ...
    UtComponentData** componentDataPtr, UtModuleInfo* moduleInfo, const char* componentName)
{
    ...
    UtComponentData* componentData
        = (UtComponentData*)j9mem_allocate_memory(sizeof(UtComponentData), OMRMEM_CATEGORY_TRACE);
    ...
    if (moduleInfo->traceVersionInfo->traceVersion >= 7 && moduleInfo->containerModule != NULL) {
        ...
        j9str_printf(PORTLIB, qualifiedName, MAX_QUALIFIED_NAME_LENGTH, "%s(%s)", moduleInfo->name,
            moduleInfo->containerModule->name);
        componentData->qualifiedComponentName
            = (char*)j9mem_allocate_memory(strlen(qualifiedName) + 1, OMRMEM_CATEGORY_TRACE);
    ...
    if (moduleInfo->formatStringsFileName != NULL) {
        componentData->formatStringsFileName
            = (char*)j9mem_allocate_memory(strlen(moduleInfo->formatStringsFileName) + 1, OMRMEM_CATEGORY_TRACE);
...
omr_error_t initializeComponentList(UtComponentList** componentListPtr)
{
    ...
    UtComponentList* componentList
        = (UtComponentList*)j9mem_allocate_memory(sizeof(UtComponentList), OMRMEM_CATEGORY_TRACE);
    ...
    const char* componentName, unsigned char value, int32_t setActive, BOOLEAN suppressMessages)
{
    ...
    while (*temp != '}' && *temp != '\0') {
        ...
        if (*temp == '-') {
            ...
            rc = setTracePointsForComponent(compName, UT_GLOBAL(componentList), FALSE, firsttpid, lasttpid, value, -1,
...
static omr_error_t loadFormatStringsForComponent(UtComponentData* componentData)
{
    ...
    if (fileContents == NULL) {
        ...
        formatFileFD = openFileFromDirectorySearchList(
            UT_GLOBAL(traceFormatSpec), componentData->formatStringsFileName, EsOpenText | EsOpenRead, 0);
        if (formatFileFD == -1) {
            UT_DBGOUT(1,
        ...
        if (*tempPtr == '\n') {
            ...
            if ((tempLen == componentNameLength) && (0 == strncmp(compName, componentData->componentName, tempLen))) {
                ...
                formatStringsComponentArray[currenttp]
                    = (char*)j9mem_allocate_memory(tempLen + 1, OMRMEM_CATEGORY_TRACE);
...
uint64_t incrementTraceCounter(UtModuleInfo* moduleInfo, UtComponentList* componentList, int32_t tracepoint)
{
    ...
    if (compData->tracepointcounters == NULL) {
        ...
        compData->tracepointcounters
            = (uint64_t*)j9mem_allocate_memory(sizeof(uint64_t) * compData->moduleInfo->count, OMRMEM_CATEGORY_TRACE);
        if (compData->tracepointcounters == NULL) {
