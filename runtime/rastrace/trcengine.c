...
static omr_error_t setJ9VMTraceOption(
...
J9VMDllMain(J9JavaVM* vm, IDATA stage, void* reserved)
{
    ...
    case ALL_LIBRARIES_LOADED:
        ...
        if (vm->j9rasGlobalStorage != NULL) {
            ...
            ((RasGlobalStorage*)vm->j9rasGlobalStorage)->configureTraceEngine
                = (ConfigureTraceFunction)runtimeSetTraceOptions;
        }
    ...
    case TRACE_ENGINE_INITIALIZED:
        ...
        if (getPropertyResult != J9SYSPROP_ERROR_NOT_FOUND && javaHomeProperty != NULL
        ...
        if (OMR_ERROR_NONE
            != (threadStart(tempThr, vm->mainThread, "Initialization thread", vm->mainThread->osThread,
                   vm->mainThread->omrVMThread))) {
            dbg_err_printf(
                1, PORTLIB, "<UT> Trace engine failed to register initialization thread, trace not enabled\n");
        ...
        if (((RasGlobalStorage*)vm->j9rasGlobalStorage)->jvmriInterface == NULL) {
            ...
            ((RasGlobalStorage*)vm->j9rasGlobalStorage)->jvmriInterface
                = j9mem_allocate_memory(sizeof(DgRasInterface), OMRMEM_CATEGORY_TRACE);
            if (((RasGlobalStorage*)vm->j9rasGlobalStorage)->jvmriInterface == NULL) {
            ...
            if (JNI_OK
                != vm->internalVMFunctions->fillInDgRasInterface(
                       ((RasGlobalStorage*)vm->j9rasGlobalStorage)->jvmriInterface)) {
                dbg_err_printf(
                    1, PORTLIB, "<UT> Error initializing jvmri interface not available, trace not enabled\n");
                j9nls_printf(PORTLIB, J9NLS_ERROR | J9NLS_STDERR, J9NLS_TRC_TRACE_INIT_FAILED);
                return J9VMDLLMAIN_FAILED;
            }
        }
        ...
        if ((*hook)->J9HookRegisterWithCallSite(
                hook, J9HOOK_VM_INITIALIZED, hookVmInitialized, OMR_GET_CALLSITE(), NULL)) {
        ...
        if ((*hook)->J9HookRegisterWithCallSite(
                hook, J9HOOK_THREAD_ABOUT_TO_START, hookThreadAboutToStart, OMR_GET_CALLSITE(), NULL)) {
        ...
        omrthread_lib_control(
            J9THREAD_LIB_CONTROL_TRACE_START, (UDATA)((RasGlobalStorage*)vm->j9rasGlobalStorage)->utIntf);
    ...
    case JVM_EXIT_STAGE:
        ...
        if (NULL != ((RasGlobalStorage*)vm->j9rasGlobalStorage)->utIntf) {
            ...
            omrthread_lib_control(
                J9THREAD_LIB_CONTROL_TRACE_STOP, (UDATA)((RasGlobalStorage*)vm->j9rasGlobalStorage)->utIntf);
            /* stop tracing in the hookable library */
            omrhook_lib_control(
                J9HOOK_LIB_CONTROL_TRACE_STOP, (UDATA)((RasGlobalStorage*)vm->j9rasGlobalStorage)->utIntf);
...
jint JNICALL JVM_OnLoad(JavaVM* vm, char* options, void* reserved) { return JNI_OK; }

jint JNICALL JVM_OnUnload(JavaVM* vm, void* reserved) { return JNI_OK; }
...
static IDATA processTraceOptionString(J9JavaVM* vm, char* opts[], IDATA* optIndex, const char* option, IDATA n)
{
                ...
                *optIndex += 2;
                if (*optIndex >= UT_MAX_OPTS - 1) {
                    ...
                    vaReportJ9VMCommandLineError(
                        PORTLIB, "Maximum number of trace options exceeded - use a trace properties file");
                    rc = J9VMDLLMAIN_FAILED;
                }
            }
        }
...
#define trace_option_helper(option) splitCommandLineOption(vm, option, LITERAL_STRLEN(option), opts + i)
    ...
    while (xtraceIndex >= 0) {
        ...
        if (optionsLength > 0) {
            if (optionString[strlen(VMOPT_XTRACE)] == ':') {
                ...
                PORT_ACCESS_FROM_JAVAVM(vm);
                vaReportJ9VMCommandLineError(PORTLIB, "Syntax error in -Xtrace, expecting \":\"; received \"%c\"",
                    optionString[strlen(VMOPT_XTRACE)]);
                rc = J9VMDLLMAIN_FAILED;
            }
        }
...
static IDATA splitCommandLineOption(J9JavaVM* vm, const char* optionString, IDATA optionLength, char* opt[])
{
    ...
    opt[0][length] = '\0';
    if (length < optionLength && optionString[length + 1] != '\0' && optionString[length + 1] != ',') {
    ...
    const OMR_VM* omr_vm, const char* optName, const char* optValue, BOOLEAN atRuntime)
{
    ...
    for (i = 0; i != NUMBER_OF_TRACE_OPTIONS; i++) {
        if (optionNameLength == strlen(TRACE_OPTIONS[i].name)
            && j9_cmdla_stricmp(optName, (char*)TRACE_OPTIONS[i].name) == 0) {
            if (atRuntime && !TRACE_OPTIONS[i].runtimeModifiable) {
                dbg_err_printf(1, PORTLIB,
                    "<UT> Trace option %s cannot be configured at run-time."
...
static IDATA parseTraceOptions(J9JavaVM* vm, const char* optionString, IDATA optionsLength)
{
    ...
    for (length = 0; length < optionsLength; length++) {
        ...
        if (inBraces == 0 && optionString[length] == ',') {
            break;
        }
    }
...
static void displayTraceHelp(J9JavaVM* vm)
{
    ...
    j9tty_err_printf(PORTLIB,
        "     stackdepth=nn                       Set number of frames output by jstacktrace trigger action\n");
    j9tty_err_printf(PORTLIB, "     sleeptime=nnt                       Time delay for sleep trigger action\n");
    j9tty_err_printf(PORTLIB,
        "                                         Recognised suffixes: ms (milliseconds), s (seconds). Default: ms\n");
 ...
 * returns     - void
 *************************************************************************/
static void doTriggerActionJavadump(OMR_VMThread* thr)
{
 ...
 * returns     - void
 *************************************************************************/
static void doTriggerActionCoredump(OMR_VMThread* thr)
{
#if defined(J9VM_RAS_DUMP_AGENTS)
    J9JavaVM* vm = (J9JavaVM*)thr->_vm->_language_vm;
 ...
 * returns     - void
 *************************************************************************/
static void doTriggerActionHeapdump(OMR_VMThread* thr)
{
#if defined(J9VM_RAS_DUMP_AGENTS)
    J9JavaVM* vm = (J9JavaVM*)thr->_vm->_language_vm;
 ...
 * returns     - void
 *************************************************************************/
static void doTriggerActionSnap(OMR_VMThread* thr)
{
    J9JavaVM* vm = (J9JavaVM*)thr->_vm->_language_vm;
 ...
 * returns     - void
 *************************************************************************/
static void doTriggerActionCeedump(OMR_VMThread* thr)
{
#if defined(J9VM_RAS_DUMP_AGENTS)
    J9JavaVM* vm = (J9JavaVM*)thr->_vm->_language_vm;
