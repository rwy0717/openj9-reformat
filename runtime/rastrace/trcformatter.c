...
UtTracePointIterator* trcGetTracePointIteratorForBuffer(UtThreadData** thr, const char* bufferName)
{
    ...
    if ((strcmp(bufferName, GCLOGGER_BUFFER_NAME) == 0) || (strcmp(bufferName, EXCEPTION_BUFFER_NAME) == 0)) {
        if (utGlobal != NULL) {
            ...
            if (trcBuf == NULL) {
                /* this is a valid condition - there is no data in the exception buffer - maybe trace is off or no gc
                 * has occurred */
                UT_DBGOUT_CHECKED(
                    1, ("<UT> trcGetTracePointIteratorForBuffer: %s no data found in buffer\n", bufferName));
            }
        }
    ...
    iter->buffer = (UtTraceBuffer*)j9mem_allocate_memory(
        UT_GLOBAL(bufferSize) + offsetof(UtTraceBuffer, record), OMRMEM_CATEGORY_TRACE);
    if (iter->buffer == NULL) {
    ...
    UT_DBGOUT_CHECKED(4,
        ("<UT> firstEntry: %d, offset of record: %ld buffer size: %d endianness %s\n", iter->start,
            offsetof(UtTraceBuffer, record), UT_GLOBAL(bufferSize),
            (iter->isBigEndian) ? "bigEndian" : "littleEndian"));
...
static uint32_t readConsumeAndSPrintfParameter(J9PortLibrary* portLib, char* rawParameterData,
    ...
    int32_t isBigEndian)
{
    ...
    if (dataType == UT_TRACE_FORMATTER_64BIT_DATA) {
        ...
        if (nWidthAndPrecisions == 2) {
            ...
            temp = j9str_printf(
                PORTLIB, destBuffer + (*offsetInDestBuffer), destBufferLength - (*offsetInDestBuffer), format, u64data);
        }
        *offsetInParameterData += 8;
        ...
        if (nWidthAndPrecisions == 2) {
            ...
            temp = j9str_printf(
                PORTLIB, destBuffer + (*offsetInDestBuffer), destBufferLength - (*offsetInDestBuffer), format, u32data);
        }
        *offsetInParameterData += 4;
        ...
        temp = j9str_printf(
            PORTLIB, destBuffer + (*offsetInDestBuffer), destBufferLength - (*offsetInDestBuffer), format, u8data);
...
static char* parseTracePoint(J9PortLibrary* portLib, UtTraceRecord* record, uint32_t offset, int tpLength,
    uint64_t* timeStampMostSignificantBytes, UtTracePointIterator* iter, char* buffer, uint32_t bufferLength)
{
    ...
    offsetOfParameters = (uint32_t)j9str_printf(PORTLIB, buffer, bufferLength, "%02u:%02u:%02u:%09u GMT %.*s.%u - ",
        hours, minutes, seconds, nanos, modNameLength, modName, traceId);
...
char* omr_trc_formatNextTracePoint(UtTracePointIterator* iter, char* buffer, uint32_t bufferLength)
{
