...
static void traceExternal(UtThreadData** thr, UtListenerWrapper func, void* userData, const char* modName,
...
static void callSubscriber(
    UtThreadData** thr, UtSubscription* subscription, UtModuleInfo* modInfo, uint32_t traceId, va_list args);
...
omr_error_t initTraceHeader(void)
{
    ...
    for (cfg = UT_GLOBAL(config); cfg != NULL; cfg = cfg->next) {
 ...
 * returns     - void
 ******************************************************************************/
static void setTraceType(int bufferType)
{
    ...
    if (oldFlags & UT_TRC_BUFFER_PURGE) {
        if (UT_GLOBAL(traceInCore)) {
            ...
            if (lastQueued != NULL) {
                ...
                } while ((oldFlags & UT_TRC_BUFFER_WRITE)
                    && !twCompareAndSwap32((unsigned int*)(&lastQueued->flags), oldFlags, newFlags));
        ...
        if (UT_GLOBAL(traceDebug) > 0) {
            ...
            for (buf = nextBuf; buf != NULL; buf = buf->next) {
                DBG_ASSERT((UT_GLOBAL(traceInCore) || buf->queueData.next == CLEANING_MSG_FLAG
                               || buf->flags & UT_TRC_BUFFER_NEW)
                    && buf->queueData.referenceCount == 0 && buf->queueData.subscriptions == 0
                    && buf->queueData.pauseCount == 0);
            }
        }
 ...
 ******************************************************************************/
static intptr_t openTraceFile(char* filename)
{
    intptr_t trcFile;
    ...
    if (filename == NULL) {
        ...
        if (UT_GLOBAL(traceGenerations) > 1) {
            *(UT_GLOBAL(generationChar)) = replaceChar[UT_GLOBAL(nextGeneration)];
            (UT_GLOBAL(nextGeneration))++;
            if (UT_GLOBAL(nextGeneration) >= UT_GLOBAL(traceGenerations)) {
     ...
     */
    if (trcFile != -1) {
        if (j9file_write(trcFile, UT_GLOBAL(traceHeader), UT_GLOBAL(traceHeader->header.length))
 ...
 * returns     - Nothing
 ******************************************************************************/
static void closeTraceFile(intptr_t trcFile, char* filename, int64_t maxFileSize)
{
    int rc;
    PORT_ACCESS_FROM_PORT(UT_GLOBAL(portLibrary));
...
UtTraceBuffer* queueWrite(UtTraceBuffer* trcBuf, int flags)
{
    ...
    UT_DBGOUT(5,
        ("<UT> QueueWrite entered for buffer " UT_POINTER_SPEC ", flags 0x%x, existing flags 0x%x\n", trcBuf, flags,
            trcBuf->flags));
    ...
    if ((oldFlags & UT_TRC_BUFFER_ACTIVE) && !(oldFlags & UT_TRC_BUFFER_NEW)) {
        ...
        UT_DBGOUT(1,
            ("<UT> skipping queue write for buffer " UT_POINTER_SPEC
             " with purge set, flags 0x%x, belonging to UT thread " UT_POINTER_SPEC "\n",
...
int subscriptionHandler(void* arg)
{
    ...
    do {
        ...
        if (trcBuf == NULL) {
            UT_DBGOUT(5,
                ("<UT> Subscription handler exiting from NULL message for subscription " UT_POINTER_SPEC "\n",
                    subscription));
            break;
        }
        ...
        if (UT_SUBSCRIPTION_KILLED == subscription->state) {
            UT_DBGOUT(5,
                ("<UT> Subscription handler exiting due to deregistration of subscription " UT_POINTER_SPEC "\n",
                    subscription));
        ...
        if (subscription->description == NULL) {
            UT_DBGOUT(5,
                ("<UT> Passing buffer " UT_POINTER_SPEC " to " UT_POINTER_SPEC "\n", trcBuf, subscription->subscriber));
        } else {
...
cleanup:
    ...
    if (subscription->alarm != NULL) {
        UT_DBGOUT(
            3, ("<UT> Calling alarm function " UT_POINTER_SPEC " for \"%s\"\n", subscription->alarm, description));
...
omr_error_t writeBuffer(UtSubscription* subscription)
{
    ...
    case UT_NORMAL_BUFFER:
        UT_DBGOUT(5,
            ("<UT thr=" UT_POINTER_SPEC "> processing TraceRecord " UT_POINTER_SPEC " of type UT_NORMAL_BUFFER\n", thr,
                trcBuf));
    ...
    case UT_EXCEPTION_BUFFER:
        UT_DBGOUT(5,
            ("<UT thr=" UT_POINTER_SPEC "> processing TraceRecord " UT_POINTER_SPEC " of type UT_EXCEPTION_BUFFER\n",
                thr, trcBuf));
    ...
    if (outputFile != -1) {
        UT_DBGOUT(5,
            ("<UT thr=" UT_POINTER_SPEC "> writeBuffer writing buffer " UT_POINTER_SPEC " to %s\n", thr, trcBuf,
                filename));
        ...
        *fileSize += subscription->dataLength;
        ...
        if (rc != subscription->dataLength) {
            ...
            j9nls_printf(PORTLIB, J9NLS_WARNING | J9NLS_STDERR, J9NLS_TRC_TRACE_WRITE_FAIL_STR,
                subscription->dataLength, filename, rc);
            *fileSize = -1;
            return OMR_ERROR_INTERNAL;
        }
            ...
            if ((bufferType == UT_NORMAL_BUFFER) && (UT_GLOBAL(traceGenerations) > 1)) {
                ...
                if (state->trcFile > 0) {
                    ...
                    *maxFileSize = *fileSize;
                    ...
                    j9nls_printf(
                        PORTLIB, J9NLS_WARNING | J9NLS_STDERR, J9NLS_TRC_NEXT_GEN_FILE_OPEN_FAIL_STR, filename);
                    *fileSize = -1;
                    return OMR_ERROR_INTERNAL;
                }
            } else {
                ...
                *fileSize = j9file_write(outputFile, UT_GLOBAL(traceHeader), UT_GLOBAL(traceHeader->header.length));
                if (*fileSize != UT_GLOBAL(traceHeader->header.length)) {
                    ...
                    j9nls_printf(PORTLIB, J9NLS_WARNING | J9NLS_STDERR, J9NLS_TRC_TRACE_WRITE_FAIL_STR,
                        UT_GLOBAL(traceHeader->header.length), filename, rc);
                    *fileSize = -1;
                    return OMR_ERROR_INTERNAL;
                }
            }
 ...
 ******************************************************************************/
static UtTraceBuffer* getTrcBuf(UtThreadData** thr, UtTraceBuffer* oldBuf, int bufferType)
{
    ...
    if (oldBuf != NULL) {
        ...
        if (UT_GLOBAL(traceInCore)) {
            ...
            if (nextBuf && ((nextBuf->flags & UT_TRC_BUFFER_WRITE) == 0)) {
                DBG_ASSERT(nextBuf->queueData.next == CLEANING_MSG_FLAG && nextBuf->queueData.referenceCount == 0
                    && nextBuf->queueData.subscriptions == 0 && nextBuf->queueData.pauseCount == 0);
            ...
            if (nextBuf && !UT_GLOBAL(dynamicBuffers) && nextBuf->next != oldBuf) {
                ...
                if (UT_GLOBAL(lostRecords) == 0) {
                    UT_DBGOUT(1,
                        ("<UT> Trace buffer discarded. Count of discarded buffers will be printed at VM shutdown\n"));
                }
    ...
    if (trcBuf == NULL) {
        ...
        newBuffer = TRUE;
        trcBuf = j9mem_allocate_memory(UT_GLOBAL(bufferSize) + offsetof(UtTraceBuffer, record), OMRMEM_CATEGORY_TRACE);
        ...
        UT_DBGOUT(1,
            ("<UT> Allocated buffer %i " UT_POINTER_SPEC ", queue tail is " UT_POINTER_SPEC "\n",
    ...
    initHeader(&trcBuf->header, UT_TRACE_BUFFER_NAME, UT_GLOBAL(bufferSize) + offsetof(UtTraceBuffer, record));
    trcBuf->next = NULL;
    trcBuf->lostCount = 0;
...
out:
    ...
    *((char*)&trcBuf->record + trcBuf->record.firstEntry) = '\0';
    ...
    if (newBuffer) {
        ...
        } while (!twCompareAndSwapPtr(
            (uintptr_t*)&UT_GLOBAL(traceGlobal), (uintptr_t)trcBuf->globalNext, (uintptr_t)trcBuf));
    }
...
static void copyToBuffer(
    UtThreadData** thr, int bufferType, char* var, char** p, int length, int* entryLength, UtTraceBuffer** trcBuf)
{
    int bufLeft = (int)((char*)&(*trcBuf)->record + UT_GLOBAL(bufferSize) - *p);
    int remainder;
    char* str;
 ...
 *
 ******************************************************************************/
static char* appFormat(UtThreadData** thr, char* moduleName, int tpId, va_list var)
{
    char* format;
    char* tpTemplate;
    int id;
 ...
 *
 ******************************************************************************/
static void tracePrint(UtThreadData** thr, UtModuleInfo* modInfo, uint32_t traceId, va_list var)
{
    int id;
    char* format;
    ...
    if (modInfo == NULL) {
        ...
        j9str_printf(PORTLIB, qualifiedModuleName, MAX_QUALIFIED_NAME_LENGTH, "%s(%s)", modInfo->name,
    ...
    if (UT_GLOBAL(indentPrint)) {
         ...
         *  Indented print
         */
        j9tty_err_printf(PORTLIB, "%02d:%02d:%02d.%03d%c" UT_POINTER_SPEC "%16s.%-6d %c %s %c ", hh, mm, ss, millis,
        ...
        j9tty_err_printf(PORTLIB, "%02d:%02d:%02d.%03d%c" UT_POINTER_SPEC "%16s.%-6d %c %c ", hh, mm, ss, millis,
 ...
 *
 ******************************************************************************/
static void traceAssertion(UtThreadData** thr, UtModuleInfo* modInfo, uint32_t traceId, va_list var)
{
    int id;
    char* format;
    uint32_t hh, mm, ss, millis;
     ...
     *  Non-indented print
     */
    j9tty_err_printf(PORTLIB, "%02d:%02d:%02d.%03d " UT_POINTER_SPEC "%8s.%-6d *   ", hh, mm, ss, millis, (*thr)->id,
 ...
 ******************************************************************************/
static void traceV(
    UtThreadData** thr, UtModuleInfo* modInfo, uint32_t traceId, const char* spec, va_list var, int bufferType)
{
    UtTraceBuffer* trcBuf;
    int lastSequence;
    int entryLength;
    ...
    if (bufferType == UT_NORMAL_BUFFER) {
        ...
        if ((trcBuf = UT_GLOBAL(exceptionTrcBuf)) == NULL && (trcBuf = getTrcBuf(thr, NULL, bufferType)) == NULL) {
            return;
        }
    } else {
        return;
    ...
    if ((UT_GLOBAL(bufferSize) - trcBuf->record.nextEntry) > (UT_FASTPATH + length + 4)) {
    ...
    if (((*thr)->currentOutputMask & (UT_MAXIMAL | UT_EXCEPTION)) != 0 && (str = (const signed char*)spec) != NULL) {
         ...
         */
        if (length > 0 && p + length + 1 < (char*)&trcBuf->record + UT_GLOBAL(bufferSize)) {
            while (*str != '\0') {
                switch (*str) {
                    /*
                     *  Words
            ...
            *p = (unsigned char)entryLength;
            ...
            int32_t left;
            while (*str != '\0' && (trcBuf->lostCount == lostCount || trcBuf->lostCount == 0)) {
             ...
             */
            if ((char*)&trcBuf->record + UT_GLOBAL(bufferSize) - p > sizeof(char)) {
                *p = (unsigned char)entryLength;
            } else {
 ...
 *
 ******************************************************************************/
static void trace(UtThreadData** thr, UtModuleInfo* modinfo, uint32_t traceId, int bufferType, const char* spec, ...)
{
    va_list var;
...
static void logTracePoint(
    UtThreadData** thr, UtModuleInfo* modInfo, uint32_t traceId, const char* spec, va_list varArgs)
{
    va_list var;
...
void doTracePoint(UtThreadData** thr, UtModuleInfo* modInfo, uint32_t traceId, const char* spec, va_list varArgs)
{
    ...
    if (NULL == modInfo || !MODULE_IS_AUXILIARY(modInfo)) {
        ...
        if ((traceId & (UT_TRIGGER)) != 0) {
            fireTriggerHit(
                thr, (NULL != modInfo) ? modInfo->name : "dg", (traceId >> 8) & UT_TRC_ID_MASK, BEFORE_TRACEPOINT);
        }
    } else {
    ...
    if (UT_GLOBAL(traceSuspend) == 0 && (*thr)->suspendResume >= 0) {
    ...
    if (NULL == modInfo || !MODULE_IS_AUXILIARY(modInfo)) {
        ...
        if ((traceId & (UT_TRIGGER)) != 0) {
            fireTriggerHit(
                thr, (NULL != modInfo) ? modInfo->name : "dg", (traceId >> 8) & UT_TRC_ID_MASK, AFTER_TRACEPOINT);
        }
 ...
 * returns     - void
 ******************************************************************************/
static void cleanupTraceWorkerThread(UtSubscription* subscription)
{
    ...
    if (data->trcFile != -1) {
        closeTraceFile(data->trcFile, UT_GLOBAL(traceFilename), data->maxTrc);
    }

    if (data->exceptFile != -1) {
        closeTraceFile(data->exceptFile, UT_GLOBAL(exceptFilename), data->maxExcept);
    }
...
omr_error_t setupTraceWorkerThread(UtThreadData** thr)
{
    ...
    result = trcRegisterRecordSubscriber(
        thr, "Trace Engine Thread", writeBuffer, cleanupTraceWorkerThread, data, NULL, NULL, &subscription, TRUE);
...
omr_error_t writeSnapBuffer(UtSubscription* subscription)
{
    ...
    UT_DBGOUT(5,
        ("<UT thr=" UT_POINTER_SPEC "> write buffer " UT_POINTER_SPEC " to snap dump file\n", thr, subscription->data));

    /*
     *  Write the record
     */
    if ((uint32_t)j9file_write(UT_GLOBAL(snapFile), subscription->data, (intptr_t)subscription->dataLength)
...
static UtProcessorInfo* getProcessorInfo(void)
{
    ...
    if (NULL != osarch) {
        if ((strcmp(osarch, J9PORT_ARCH_PPC) == 0) || (strcmp(osarch, J9PORT_ARCH_PPC64) == 0)
...
static void raiseAssertion(UtThreadData** thread, UtModuleInfo* modInfo, uint32_t traceId)
{
    ...
    */
    if (UT_GLOBAL(fatalassert)) {
        ...
        j9str_printf(PORTLIB, triggerClause, sizeof(triggerClause), "tpnid{%s.%d,assert}",
            (NULL != modInfo) ? modInfo->name : "dg", (traceId >> 8) & UT_TRC_ID_MASK);
        setTriggerActions(thread, triggerClause, TRUE);
        fireTriggerHit(
            thread, (NULL != modInfo) ? modInfo->name : "dg", (traceId >> 8) & UT_TRC_ID_MASK, AFTER_TRACEPOINT);
...
void omrTraceMem(void* env, UtModuleInfo* modInfo, uint32_t traceId, uintptr_t length, void* memptr)
{
    ...
    j9tty_err_printf(PORTLIB,
        "* ** ASSERTION FAILED ** Obsolete trace function TraceMem called for trace point %s.%-6d", qualifiedModuleName,
        id);
    j9tty_err_printf(PORTLIB, "\n");
...
void omrTraceState(void* env, UtModuleInfo* modInfo, uint32_t traceId, const char* spec, ...)
{
    ...
    j9tty_err_printf(PORTLIB,
        "* ** ASSERTION FAILED ** Obsolete trace function TraceState called for trace point %s.%-6d",
        qualifiedModuleName, id);
    j9tty_err_printf(PORTLIB, "\n");
    ...
    UtThreadData** thr, UtSubscription* subscription, UtModuleInfo* modInfo, uint32_t traceId, va_list args)
{
    ...
    headerSize = j9str_printf(PORTLIB, NULL, 0, "%02d:%02d:%02d.%03d 0x%x %s.%3d", hours, mins, secs, msecs, (*thr)->id,
        qualifiedModuleName, id);
    ...
    cursor = buffer;
    j9str_printf(PORTLIB, cursor, headerSize, "%02d:%02d:%02d.%03d 0x%x %s.%3d", hours, mins, secs, msecs, (*thr)->id,
        qualifiedModuleName, id);
    cursor += headerSize - 1;
    COPY_VA_LIST(argsCopy, args);
