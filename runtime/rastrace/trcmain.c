...
static omr_error_t trcRegisterTracePointSubscriber(UtThreadData** thr, char* description,
    utsSubscriberCallback subscriber, utsSubscriberAlarmCallback alarm, void* userData,
...
static omr_error_t trcTraceSnapWithPriority(
    UtThreadData** thr, char* label, int32_t snapPriority, char** response, int32_t sync);
static omr_error_t internalTraceSnapWithPriority(
    UtThreadData** thr, char* label, int32_t snapPriority, char** response, int32_t sync);
...
static omr_error_t trcTraceRegister(UtThreadData** thr, UtListenerWrapper func, void* userData)
{
    ...
    if (UT_GLOBAL(traceListeners) == NULL) {
        ...
        for (next = UT_GLOBAL(traceListeners); next != NULL; next = next->next) {
            if (next->next == NULL) {
                next->next = this;
                break;
            }
...
static void omrTraceInit(void* env, UtModuleInfo* modInfo) { moduleLoaded(UT_THREAD_FROM_ENV(env), modInfo); }
        ...
        if (!oldFlags) {
            ...
            if (start != NULL) {
                ...
                if (!UT_GLOBAL(externalTrace)) {
                    ...
                    UT_DBGOUT(1,
                        ("<UT thr=" UT_POINTER_SPEC "> Starting Snap write thread, start: " UT_POINTER_SPEC
                         ", stop: " UT_POINTER_SPEC "\n",
                            thr, start, stop));

                    /* Spawn a snap writer with it's subscription as its userData*/
                    /* TODO: if a crash occurs during registration/deregistration the the trace lock is already held and
                     * this can hang. We need to check to see if this thread is already the owner of the trace lock
                     * before trying to snap dump in this fashion.
                     */
                    result = trcRegisterRecordSubscriber(thr, "Snap Dump Thread", writeSnapBuffer,
                        cleanupSnapDumpThread, NULL, start, stop, &subscription, FALSE);
                ...
                UT_DBGOUT(1,
                    ("<UT thr=" UT_POINTER_SPEC "> snap unpausing write queue at " UT_POINTER_SPEC "\n", thr,
                        &start->queueData));
            ...
            UT_DBGOUT(1,
                ("<UT> Snap requested when one is already in progress, therefore ignoring it (no data will be "
                 "lost)\n"));
    ...
    if ((trcBuf = (*thr)->trcBuf) != NULL) {
        if (!UT_GLOBAL(traceInCore)) {
            ...
            if (trcBuf->lostCount != 0) {
                ...
                while (
                    (trcBuf->flags & UT_TRC_BUFFER_FULL) != 0 && (((uint64_t)j9time_current_time_millis()) < endTime)) {
                    omrthread_sleep(1);
                }
            ...
            UT_DBGOUT(5, ("<UT> freeing buffer " UT_POINTER_SPEC " for thread " UT_POINTER_SPEC "\n", trcBuf, thr));
    ...
    *thr = NULL;
    ...
    if (newCount == 0 && UT_GLOBAL(traceFinalized)) {
        ...
        if (global->traceDebug >= 2) {
            j9tty_err_printf(
                PORTLIB, "<UT> ThreadStop entered for final thread " UT_POINTER_SPEC ", freeing buffers\n", thr);
        }
                ...
                if (gNext == NULL) {
                    if (global->traceDebug >= 1) {
                        j9tty_err_printf(PORTLIB,
                            "<UT> NULL global buffer list! " UT_POINTER_SPEC " not found in global list\n", current);
                    }
                    ...
                    if (gNext != NULL && gNext->globalNext == current) {
                        ...
                        if (global->traceDebug >= 1) {
                            j9tty_err_printf(
                                PORTLIB, "<UT> trace buffer " UT_POINTER_SPEC " not found in global list\n", current);
                        }
                    }
                }
            }
...
omr_error_t utTerminateTrace(UtThreadData** thr, char** daemonThreadNames)
{
    ...
    if (!UT_GLOBAL(traceInCore)) {
        ...
        for (trcBuf = UT_GLOBAL(traceGlobal); trcBuf != NULL; trcBuf = trcBuf->globalNext) {
            if ((trcBuf->flags & UT_TRC_BUFFER_ACTIVE) != 0) {
                UT_DBGOUT(2,
                    ("<UT> Flushing buffer " UT_POINTER_SPEC " for thr " UT_POINTER_SPEC "\n", (uintptr_t)trcBuf,
                        (uintptr_t)trcBuf->record.threadId));
    ...
    while (NULL != UT_GLOBAL(subscribers)) {
        ...
        if (J9THREAD_TIMED_OUT == rc) {
            UT_DBGOUT(1,
                ("<UT thr=" UT_POINTER_SPEC "> Trace termination timed out waiting for subscribers to complete\n",
                    thr));
...
void freeTrace(UtThreadData** thr)
{
    ...
    if (UT_GLOBAL(traceFinalized) != TRUE) {
        ...
        UT_DBGOUT(
            1, ("<UT thr=" UT_POINTER_SPEC "> Error: cleanUpTrace called before trace has been finalized\n", thr));
    }
...
omr_error_t threadStart(UtThreadData** thr, const void* threadId, const char* threadName, const void* threadSynonym1,
...
static omr_error_t trcAddComponent(UtModuleInfo* modInfo, const char** format)
{
    ...
    if (tmp_formatStrings == NULL) {
        UT_DBGOUT(1,
            ("<UT> Unable to allocate formatStrings memory for trace module: %s, trace not enabled\n", modInfo->name));
        return OMR_ERROR_OUT_OF_NATIVE_MEMORY;
    }
    overhead = 2;
    for (i = 0; i < arrayElements; i++) {
        ...
        if (formatTemplate == NULL) {
            UT_DBGOUT(1,
                ("<UT> trcAddComponent cannot allocate memory for app trace module: %s, trace not enabled\n",
                    modInfo->name));
            rc = OMR_ERROR_OUT_OF_NATIVE_MEMORY;
            break;
        }
...
static omr_error_t trcGetComponent(char* name, unsigned char** bitMap, int32_t* first, int32_t* last)
{
    ...
    if (cd == NULL) {
        UT_DBGOUT(2,
            ("<UT> trcGetComponent requested data area for component: \"%s\"  that is not currently loaded\n", name));
        *bitMap = NULL;
        *first = 0;
        *last = 0;
    } else {
...
omr_error_t getComponentGroup(char* compName, char* groupName, int32_t* count, int32_t** tracePts)
{
 ...
 ******************************************************************************/
omr_error_t trcRegisterRecordSubscriber(UtThreadData** thr, const char* description, utsSubscriberCallback subscriber,
    utsSubscriberAlarmCallback alarm, void* userData, UtTraceBuffer* start, UtTraceBuffer* stop,
    UtSubscription** subscriptionReference, int32_t attach)
{
    omr_error_t result = OMR_ERROR_NONE;
    UtSubscription* subscription;
    qSubscription* qSub;
    ...
    if (OMR_ERROR_NONE != result) {
        UT_DBGOUT(1,
            ("<UT thr=" UT_POINTER_SPEC "> Failed to subscribe %s to queue " UT_POINTER_SPEC "\n", thr, description,
                UT_GLOBAL(outputQueue)));
        goto out;
    }
...
static omr_error_t trcFlushTraceData(UtThreadData** thr, UtTraceBuffer** first, UtTraceBuffer** last, int32_t pause)
{
    ...
    for (trcBuf = UT_GLOBAL(traceGlobal); trcBuf != NULL; trcBuf = trcBuf->globalNext) {
        ...
        UT_DBGOUT(5,
            ("<UT thr=" UT_POINTER_SPEC "> Flushing buffer " UT_POINTER_SPEC " for thread " UT_POINTER_SPEC "\n", thr,
                trcBuf, (uintptr_t)trcBuf->record.threadId));
        ...
        if (current != NULL) {
            ...
            /* we've discarded the reference to the circular list of buffers, but the final buffer wasn't queued, so
             * free by hand.
             */
            freeBuffers(&trcBuf->queueData);
        }
 ...
 * parameters  - UtThreadData, subscriber description, subscriber function, alarm function, user data pointer,
 *subscription pointer returns     - JNI return codes
 ******************************************************************************/
static omr_error_t trcRegisterTracePointSubscriber(UtThreadData** thr, char* description,
    utsSubscriberCallback subscriber, utsSubscriberAlarmCallback alarm, void* wrapper,
    UtSubscription** subscriptionReference)
{
    UtSubscription* subscription;
...
static omr_error_t trcDeregisterTracePointSubscriber(UtThreadData** thr, UtSubscription* subscriptionID)
{
    ...
    UT_DBGOUT(
        1, ("<UT> trcDeregisterTracePointSubscriber found subscription, wrapper is %p\n", subscription->userData));
    ...
    UT_DBGOUT(1,
        ("<UT> trcDeregisterTracePointSubscriber normal exit, tracePointSubscribers global = %p\n",
            UT_GLOBAL(tracePointSubscribers)));
    return OMR_ERROR_NONE;
}

/* TODO should be combined with runtimeSetTraceOptions()? */
static omr_error_t trcSetOptions(UtThreadData** thr, const char* opts[]) { return setOptions(thr, opts, TRUE); }
...
int32_t getDebugLevel(void) { return UT_GLOBAL(traceDebug); }
