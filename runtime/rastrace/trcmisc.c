...
void incrementRecursionCounter(UtThreadData* thr) { thr->recursion++; }

void decrementRecursionCounter(UtThreadData* thr) { thr->recursion--; }
...
omr_error_t expandString(char* returnBuffer, const char* original, BOOLEAN atRuntime)
{
    ...
    char* dateTokenString = "%Y"
                            "%m"
                            "%d";
    char* timeTokenString = "%H"
                            "%M"
                            "%S";
...
omr_error_t addTraceConfig(UtThreadData** thr, const char* cmd)
{
     ...
     */
    if ((cfg = j9mem_allocate_memory(sizeof(UtTraceCfg) + strlen(cmd) + 1, OMRMEM_CATEGORY_TRACE)) != NULL) {
