 ...
 ******************************************************************************/
static omr_error_t addTraceCmd(UtThreadData** thr, const char* cmd, const char* value, BOOLEAN atRuntime)
{
    ...
    str = (char*)j9mem_allocate_memory(strlen(cmd) + (value == NULL ? 1 : strlen(value) + 2), OMRMEM_CATEGORY_TRACE);
    if (str != NULL) {
        strcpy(str, cmd);
    ...
    if (OMR_ERROR_NONE == rc) {
        ...
        if (p != NULL) {
            if ((length >= 1) && (length <= 2)) {
                ...
                if ((UT_GLOBAL(traceGenerations) < 2) || (UT_GLOBAL(traceGenerations) > 36)) {
                    reportCommandLineError(atRuntime, "Invalid number of trace generations");
                    rc = OMR_ERROR_ILLEGAL_ARGUMENT;
                } else {
                    if ((UT_GLOBAL(generationChar) = strchr(UT_GLOBAL(traceFilename), '#')) == NULL) {
                        reportCommandLineError(atRuntime,
                            "Invalid filename for generation mode,"
 ...
 *
 ******************************************************************************/
omr_error_t setTrigger(UtThreadData** thr, const char* triggerStr, BOOLEAN atRuntime)
{
    omr_error_t rc = OMR_ERROR_NONE;
    char* clause = NULL;
    char* str;
 ...
 * returns     - UTE return code
 ******************************************************************************/
static omr_error_t setFormat(const char* value)
{
    omr_error_t rc = OMR_ERROR_NONE;
 ...
 * returns     - UTE return code
 *************************************************************************/
omr_error_t setSuspendResumeCount(UtThreadData** thr, const char* value, int32_t resume, BOOLEAN atRuntime)
{
    omr_error_t rc = OMR_ERROR_NONE;
    const char* p;
    int length;
 ...
 * returns     - UTE return code
 ******************************************************************************/
static omr_error_t processPropertyFile(UtThreadData** thr, const char* filename, BOOLEAN atRuntime)
{
    intptr_t propFile;
    int64_t size;
    int i;
    ...
    if (rc == OMR_ERROR_NONE) {
        *(buffer + size) = '\0';
         ...
         */
        for (p = buffer; (rc == OMR_ERROR_NONE) && (p < (buffer + size)); p += strlen(p) + 1) {
            if (*p == '\0') {
                continue;
            }
        ...
        if (OMR_ERROR_NONE != rc) {
            if (!atRuntime) {
                if (optValue == NULL) {
                    ...
                    j9nls_printf(
                        PORTLIB, J9NLS_ERROR | J9NLS_STDERR, J9NLS_TRC_ERROR_IN_OPTION_WITH_ARG_STR, optName, optValue);
                }
            }
            break;
        } else {
...
static int selectComponent(const char* cmd, int32_t* first, char traceType, int32_t setActive, BOOLEAN atRuntime)
{
    ...
    if (*cmd == '\0') {
        if (*first) {
             ...
             */
            rc = setTracePointsTo(
                (const char*)UT_ALL, UT_GLOBAL(componentList), TRUE, 0, 0, traceType, -1, NULL, atRuntime, setActive);
            if (OMR_ERROR_NONE != rc) {
