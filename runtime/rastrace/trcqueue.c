 ...
 */
static void clean(qQueue* queue)
{
    qMessage *head, *tail;
    ...
    tail = queue->tail;
    UT_DBGOUT(4,
        ("<UT> checking free queue message " UT_POINTER_SPEC ", (pause=%i, subscriptions=%i)\n", tail,
            tail ? tail->pauseCount : 0, tail ? tail->subscriptions : 0));
    ...
    for (; tail != NULL && tail->subscriptions == 0 && tail->pauseCount == 0 && !queue->pause; tail = queue->tail) {
        ...
        /* make sure that nothing's tried to pause this message since we entered the loop, and stop anything from
         * succeeding in pausing it We set pause back to zero before finishing in this loop in cases where the message
         * could be left on the queue.
         */
        if (compareAndSwapUDATA((uintptr_t*)&tail->next, (uintptr_t)CLEANING_MSG_FLAG, (uintptr_t)CLEANING_MSG_FLAG)
            == (uintptr_t)CLEANING_MSG_FLAG) {
        ...
        if (!twCompareAndSwap32((uint32_t*)(&tail->pauseCount), 0, UNTAGGED)) {
            ...
            UT_DBGOUT(2,
                ("<UT> skipping cleaning for paused free queue message " UT_POINTER_SPEC
                 ", (pause=%i, subscriptions=%i)\n",
        ...
        queueTail = (qMessage*)compareAndSwapUDATA(
            (uintptr_t*)&queue->tail, (uintptr_t)CLEANING_MSG_FLAG, (uintptr_t)CLEANING_MSG_FLAG);
        ...
        UT_DBGOUT(5,
            ("<UT> processing free queue message " UT_POINTER_SPEC ", (pause=%i, subscriptions=%i)\n", tail,
                tail ? tail->pauseCount : 0, tail ? tail->subscriptions : 0));
        /* we make sure that tail->next is non-null. If tail == head this prevents new items being queued while we empty
         * the queue and causing issues for the correct updating of queue->tail. If tail != head then the update fails.
        ...
        if (!twCompareAndSwapPtr((uintptr_t*)&tail->next, (uintptr_t)NULL, (uintptr_t)CLEANING_MSG_FLAG)) {
            ...
            if (!twCompareAndSwapPtr((uintptr_t*)&queue->tail, (uintptr_t)tail, (uintptr_t)tail->next)) {
                ...
                /* this should never happen... we shouldn't be able to get this far with the same message, but leaving
                 * it in for now */
                DBG_ASSERT(0);

                /* a different thread has cleaned tailed from the queue already */
                UT_DBGOUT(1,
                    ("<UT ERROR> not cleaning tail " UT_POINTER_SPEC
                     " because it's either flagged (tail->next = " UT_POINTER_SPEC
                     ") or no longer tail despite the fact we're guarded by the pause count\n",
                        tail, tail->next));
            ...
            if (!twCompareAndSwapPtr((uintptr_t*)&queue->head, (uintptr_t)tail, (uintptr_t)NULL)) {
                 ...
                 * as head. We have to allow the publish to complete before we can clean so we let the next clean take
                 * care of this.
                 */
                UT_DBGOUT(3,
                    ("<UT> consumer run ahead, so letting publishing catch up. Tail is " UT_POINTER_SPEC "\n", tail));

                /* the publish of this message as head could continue without us unsetting the flag, however it'll block
                 * subsequent publishes until this is done */
            ...
            /* this could fail if, once we've nulled head, a new message is published as head&tail before we hit here,
             * so we fail quietly.
             */
    ...
    /* TODO: add in some consistency checks to clean so that if something does damage the state of the queue then we
     * don't swallow all of the memory available. A basic check would be to ensure that we never have more items on the
     * reference queue than we have subscribers given it's a maximum of one each.
     */
}
...
int32_t publishMessage(qQueue* queue, qMessage* msg)
{
    ...
    if (!queue->alive) {
        UT_DBGOUT(
            4, ("<UT> not publishing message " UT_POINTER_SPEC " to dead queue " UT_POINTER_SPEC "\n", msg, queue));
        return FALSE;
    }
    ...
    if (msg == NULL || compareAndSwapU32((uint32_t*)&msg->subscriptions, 0, UNTAGGED) != 0) {
        UT_DBGOUT(1,
            ("<UT> Dropping message " UT_POINTER_SPEC " instead of publishing to queue " UT_POINTER_SPEC
             " due to invalid message state\n",
                msg, queue));
        return FALSE;
    }
    ...
    do {
        ...
        if (head == NULL) {
            ...
            /* It's possible that there'll be a delay here, one long enough that the local "head" gets replaced,
             * processed, cleaned, freed and, potentially, re-queued.
             *
             * In this lifecycle there is only one point at which it is valid for us to execute the instruction below,
             * therefore qMessage->next must be non null throughout the entirety of that lifecycle. This is why next is
             * set to CLEANING_MSG_FLAG when a message is cleaned. We shouldn't be freeing memory used for messages
             * while the queue's still alive unless we can be sure there's nothing in publish.
             */
    ...
    if (head != NULL) {
        ...
        if (queue->tail == NULL) {
            UT_DBGOUT(2,
                ("<UT> tail is null and replacing queue->head " UT_POINTER_SPEC " with " UT_POINTER_SPEC "\n", head,
                    msg));
        }

        /* don't bother preventing reordering of subscriptions update and the head update as it doesn't actually matter.
         * It may block for longer if the subscription update is done first.
         */
    ...
    if (head == NULL) {
        ...
        UT_DBGOUT(4,
            ("<UT> msg " UT_POINTER_SPEC " is new head&tail, tail was " UT_POINTER_SPEC " for queue " UT_POINTER_SPEC
             "\n",
                msg, currentTail, queue));
    }

    /* ensure that we can queue new messages now that msg has a fixed place in the queue */
    /* CMVC 149252: moved this from just after the cmpxchg loop to here so that publishing is guarrenteed to have
     * finished */
    msg->next = NULL;
    return TRUE;
}
...
#pragma option_override(acquireNextMessage, "opt(level, 0)")
        ...
        while (!IS_VALID_MSG_PTR(msg) && sub->valid == TRUE) {
            ...
            if (head != NULL && (sub->current == NULL || head != sub->current)) {
                ...
                if (!IS_VALID_MSG_PTR(msg)) {
                     ...
                     *
                     * under no circumstances will we ever want to pick up a message that's got a subscription count of
                     * zero. This could happen if head's nulled as the queue's emptied, subscriber is set to work from
                     * tail and picks up the expired tail before it's nulled. compareAndSwapX(y, z, z) will return y if
                     * the value is/was y otherwise the actual value of x so this can be used as if it's the variable
                     * but in a reliable fashion when using compare/swap
                     */
                    if (sub->current == NULL || sub->current->subscriptions == 0) {
                        ...
                        if (msg == sub->current || !IS_VALID_MSG_PTR(msg)
                            ...
                            UT_DBGOUT(2,
                                ("<UT> subscription " UT_POINTER_SPEC
                                 " picked up current from tail of queue, reverting to null\n",
                                    sub));
                            msg = NULL;
                            omrthread_yield();
                            continue;
            ...
            if (!IS_VALID_MSG_PTR(msg)) {
                /* TODO: add a new state, moribund, that prevents queuing new buffers and means subscribers exit rather
                 * than wait */
                if (queue->alive) {
                    UtEventSem* alarm = queue->alarm;
                    UT_DBGOUT(2,
                        ("<UT> subscription " UT_POINTER_SPEC
                         " waiting for message to be published to queue " UT_POINTER_SPEC "\n",
                            sub, queue));
                    if (alarm != NULL) {
    ...
    if (!IS_VALID_MSG_PTR(msg) && sub->valid) {
        ...
        UT_DBGOUT(
            1, ("<UT> queue " UT_POINTER_SPEC " is dead or error for subscription " UT_POINTER_SPEC "\n", queue, sub));
        return NULL;
    }
    ...
    if (!sub->valid) {
        UT_DBGOUT(1,
            ("<UT> subscription " UT_POINTER_SPEC " unsubscribed so exiting. current " UT_POINTER_SPEC
             ", last: " UT_POINTER_SPEC "\n",
    ...
    /* we now longer have the possibiity of needing a reference to the current last in unsubscribe, so we're done with
     * it */
...
void releaseCurrentMessage(qSubscription* sub)
{
    ...
    if (sub->currentLocked) {
        ...
        UT_DBGOUT(5,
            ("<UT> message " UT_POINTER_SPEC " has %i subscribers remaining after release\n", current,
 ...
 *				NOTE: *subscription == NULL is currently not allowed due to issues with cleanup on
 *unsubscribe. start - the message to start processing from, this can be three things:
...
omr_error_t subscribe(qQueue* queue, qSubscription** subscription, qMessage* start, qMessage* stop)
{
    ...
    /* malloc memory if none was passed in */
    if (*subscription == NULL) {
        /* TODO: unless we can determine if there is a thread in acquireNextMessage we can't safely free the
         * subscription within unsubscribe. As such this is currently disabled.
         */
        return OMR_ERROR_ILLEGAL_ARGUMENT;
#if 0
    ...
    if (tagValue < threshold) {
        UT_DBGOUT(1,
            ("<UT> reached the maximum number of subscribers (%i) for queue " UT_POINTER_SPEC "\n",
    ...
    /* basic case first*/
    if (queue->head == NULL) {
        /* we can't remove tagged buffers or buffers with the incremented count, so if head is null current should be
         * null */
        workingCurrent = NULL;
    } else {
        /* if the queue was empty or fixupTail has been cleaned then we work from tail. Tagged values can't have been
         * cleaned, nor can messages with the new subscription count */
        if (fixupTail == NULL) {
         ...
         * while looking for untagged values or we could start roaming into the buffers with the new count. Initial
         * fixupTail value can't be tagged, so we start looking at fixupTail->next
         */
        if (fixupHead != NULL) {
        ...
        if (start != (qMessage*)-1) {
            UT_DBGOUT(5,
                ("<UT> revising initial message for subscription " UT_POINTER_SPEC
                 " to match start bound " UT_POINTER_SPEC "\n",
                    sub, start));
    ...
    if (sub->current != NULL) {
        UT_DBGOUT(5,
            ("<UT> reference message for subscription " UT_POINTER_SPEC " is " UT_POINTER_SPEC
             ", first to process is " UT_POINTER_SPEC "\n",
...
omr_error_t unsubscribe(qSubscription* sub)
{
    ...
    /* we can't have tagged anything if head is null, because tagged buffers can't be removed. We must have processed
     * all our buffers or they would be on the queue still, so nothing to be done.
     */
    issueReadBarrier();
    if (queue->head != NULL) {
        /* from the buffer after current we need to decrement the count. If current isn't valid then the buffer after
         * the last processed, i.e. the one that would be current if the consumer retrieved another buffer.
         */
        ...
        if (fixupTail == NULL) {
            ...
            UT_DBGOUT(5,
                ("<UT> subscriber had exited so working from " UT_POINTER_SPEC " instead of " UT_POINTER_SPEC "\n",
                    sub->last, sub->current));
            fixupTail = sub->last;
            if (fixupTail == (qMessage*)-1) {
                /* the subscriber never did any work on the current queue and had no defined start point, so work from
                 * tail */
                fixupTail = queue->tail;
            } else {
                reference = fixupTail;
        ...
        /* unless we were working from tail the message we're working from needs it's reference count decrementing. The
         * unsubscribe is left with this job because we need it to not be cleaned while we're in this code
         */
        if (reference != NULL) {
            UT_ATOMIC_DEC((volatile uint32_t*)&reference->referenceCount);
        }

        /* fixupHead is either tagged or has the original Value if it's non null and is not the same as reference
         * (reference has already been processed by the subscription). Either way we don't want to go past it while
         * looking for untagged values or we could start trampling the buffers with the new count
         */
        if (fixupHead != NULL && fixupHead != reference) {
            while (IS_VALID_MSG_PTR(fixupTail) && fixupTail->subscriptions > tagValue) {
                ...
                UT_DBGOUT(5,
                    ("<UT> fixed up subscription count for " UT_POINTER_SPEC ", new count is %i\n", fixupTail,
                        fixupTail->subscriptions));
    ...
    if (sub->allocd) {
        /* if we allocated the memory for the subscription we need to free it here. If that's the case then it
         * invalidates all state for that subscription at that point so we need to clean up here. If the subscription is
         * being used for acquiring or publishing a message then we can't free the memory because we need to reference
         * the subscription in those functions.
         */
        releaseCurrentMessage(sub);
