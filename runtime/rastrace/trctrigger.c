...
static struct RasTriggerType defaultRasTriggerTypes[] = {
    ...
    { "group", processTriggerGroupClause, TRUE }, { "tpnid", processTriggerTpnidClause, TRUE }
};
...
err:
    reportCommandLineError(atRuntime,
        "Sleeptime takes a positive integer value and, optionally, a suffix of s or ms. Maximum sleeptime is "
...
static uint32_t parseTriggerIndex(OMR_VMThread* thr, const char* name, BOOLEAN atRuntime)
{
    int i;
...
omr_error_t addTriggerAction(OMR_VMThread* thr, const struct RasTriggerAction* newAction)
{
    ...
    newRasTriggerActions
        = omrmem_allocate_memory(sizeof(struct RasTriggerAction) * (numTriggerActions + 1), OMRMEM_CATEGORY_TRACE);
 ...
 * returns     - JNI return code
 *************************************************************************/
static omr_error_t processTriggerTpnidClause(OMR_VMThread* thr, char* clause, BOOLEAN atRuntime)
{
    OMRPORT_ACCESS_FROM_OMRVMTHREAD(thr);
    ...
    if (OMR_ERROR_NONE == rc) {
        ...
        if (actionIndex == -1) {
            ...
            if (OMR_ERROR_NONE == rc) {
                 ...
                 */
                if (OMR_ERROR_NONE == rc) {
                    newTriggerRangeP = (RasTriggerTpidRange*)omrmem_allocate_memory(
                        sizeof(RasTriggerTpidRange), OMRMEM_CATEGORY_TRACE);
 ...
 * returns     - JNI return code
 *************************************************************************/
static omr_error_t processTriggerGroupClause(OMR_VMThread* thr, char* clause, BOOLEAN atRuntime)
{
    OMRPORT_ACCESS_FROM_OMRVMTHREAD(thr);
    omr_error_t rc = OMR_ERROR_NONE;
    int numParms = 0;
    ...
    if (numParms < 2 || numParms > 4) {
        reportCommandLineError(atRuntime,
            "Trigger groups clause has the following usage: group{<groupname>,<action>[,<delay>][,<matchcount>]}");
        rc = OMR_ERROR_INTERNAL;
    }
    ...
    if (numParms >= 3 && ptrDelay != NULL && *ptrDelay != '\0') {
        ...
        if (strlen(ptrDelay) > maxLength) {
            reportCommandLineError(atRuntime,
                "Delay counts must be integer values from -99999 to +99999: group{%s,%s,%s,%s}", ptrGroupName,
                ptrAction, ptrDelay, ptrMatch);
            rc = OMR_ERROR_INTERNAL;
        } else {
    ...
    if (numParms == 4) {
        ...
        if (strlen(ptrMatch) > maxLength) {
            reportCommandLineError(atRuntime,
                "Match counts must be integer values from -99999 to +99999: group{%s,%s,%s,%s}", ptrGroupName,
                ptrAction, ptrDelay, ptrMatch);
            rc = OMR_ERROR_INTERNAL;
        } else {
            /* issues error and sets rc if ptrDelay points to bad number */
...
omr_error_t addTriggerType(OMR_VMThread* thr, const struct RasTriggerType* newType)
{
    ...
    /* Append it to the existing list of options and update */
    newRasTriggerTypes
        = omrmem_allocate_memory(sizeof(struct RasTriggerType) * (numTriggerTypes + 1), OMRMEM_CATEGORY_TRACE);
 ...
 * returns     - JNI return code
 *************************************************************************/
static omr_error_t processTriggerClause(OMR_VMThread* thr, const char* clause, BOOLEAN atRuntime)
{
    OMRPORT_ACCESS_FROM_OMRVMTHREAD(thr);
 ...
 * returns     - void
 *************************************************************************/
static void doTriggerActionSuspend(OMR_VMThread* thr)
{
 ...
 * returns     - void
 *************************************************************************/
static void doTriggerActionResume(OMR_VMThread* thr)
{
 ...
 * returns     - void
 *************************************************************************/
static void doTriggerActionSuspendThis(OMR_VMThread* thr)
{
 ...
 * returns     - void
 *************************************************************************/
static void doTriggerActionResumeThis(OMR_VMThread* thr)
{
 ...
 * returns     - void
 *************************************************************************/
static void doTriggerActionAbort(OMR_VMThread* thr) { abort(); }
 ...
 * returns     - void
 *************************************************************************/
static void doTriggerActionSegv(OMR_VMThread* thr)
{
    long* p = NULL;
 ...
 * returns     - void
 *************************************************************************/
static void checkTriggerGroupsForTpid(
    OMR_VMThread* thr, char* compName, int traceId, const TriggerPhase phase, BOOLEAN actionArray[])
{
    ...
    do {
        ...
        if (oldValue < 0) {
            ...
            return;
                 ...
                 (uintptr_t*)&UT_GLOBAL(triggerOnGroupsReferenceCount), (uintptr_t)oldValue, (uintptr_t)oldValue + 1)
        != (uintptr_t)oldValue);
                ...
                if (traceId == tracePts[i]) {
                    ...
                    if (0 == oldDelay) {
                        ...
                        do {
                            ...
                            compareAndSwapU32((uint32_t*)&groupPtr->match, (uint32_t)oldMatch, (uint32_t)(oldMatch - 1))
                            != oldMatch);
 ...
 * returns     - void
 *************************************************************************/
static void checkTriggerTpidForTpid(
    OMR_VMThread* thr, char* compName, unsigned int traceId, const TriggerPhase phase, BOOLEAN actionArray[])
{
    ...
    do {
        ...
        if (oldValue < 0) {
            ...
            return;
                 ...
                 (uintptr_t*)&UT_GLOBAL(triggerOnTpidsReferenceCount), (uintptr_t)oldValue, (uintptr_t)oldValue + 1)
        != (uintptr_t)oldValue);
        ...
        if (rasTriggerActions[ptr->actionIndex].phase == phase) {
            if (strcmp(compName, ptr->compName) == 0 && traceId >= ptr->startTpid && traceId <= ptr->endTpid) {
                ...
                if (0 == oldDelay) {
                    ...
                    } while (compareAndSwapU32((uint32_t*)&ptr->match, (uint32_t)oldMatch, (uint32_t)(oldMatch - 1))
                        != oldMatch);
                    ...
                    if (oldMatch != 0) {
                         ...
                         */
                        RAS_DBGOUT((stderr,
                            "tpnid %X matches tpnid range %s.%X-%X, "
                            "action=%d\n",
