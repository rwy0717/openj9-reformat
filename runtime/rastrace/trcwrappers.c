...
void twFprintf(const char* template, ...)
{
...
int32_t twCompareAndSwap32(volatile uint32_t* target, uint32_t old, uint32_t new32)
{
    return (compareAndSwapU32((uint32_t*)target, old, new32) == old) ? TRUE : FALSE;
}

int32_t twCompareAndSwapPtr(volatile uintptr_t* target, uintptr_t old, uintptr_t newptr)
{
...
omr_error_t twThreadAttach(UtThreadData** thr, char* name)
{
    ...
    if (NULL != languageIntf->AttachCurrentThreadToLanguageVM) {
        if (OMR_ERROR_NONE
            != languageIntf->AttachCurrentThreadToLanguageVM((OMR_VM*)UT_GLOBAL(vm), name, &currentThread)) {
