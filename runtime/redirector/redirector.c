 ...
 */
static void truncatePath(char* inputPath, BOOLEAN keepSlashChar)
{
 ...
 */
static BOOLEAN hasEnvOption(const char* envOptions, const char* option)
{
 ...
 */
static char* findStartOfMostRightOption(const char* envOptions, const char* option)
{
 ...
 */
static U_64 parseMemorySizeValue(char* option)
{
    U_64 result = 0;
    char* cursor = option;
 ...
 * @param retBuffer The buffer which will be populated with the directory name (must be big enough to contain the name
 * and the NULL byte)
 * @param bufferLength The side of the retBuffer
 *
 * Returns the name of the directory containing the libjvm.so VM library  in the caller-provided retBuffer (with a
...
static void chooseJVM(JavaVMInitArgs* args, char* retBuffer, size_t bufferLength)
{
    ...
    if (NULL != envOptions) {
        /* we need a non-zero index to point to where this occurs to use the first index we don't have - the number of
    ...
    for (i = 0; i < args->nOptions; i++) {
        if (0 == strcmp(args->options[i].optionString, "-Xcompressedrefs")
            || 0 == strcmp(args->options[i].optionString, "-XX:+UseCompressedOops")) {
            xcompressed = i + 1;
            xcompressedstr = args->options[i].optionString;
        } else if (0 == strcmp(args->options[i].optionString, "-Xnocompressedrefs")
            || 0 == strcmp(args->options[i].optionString, "-XX:-UseCompressedOops")) {
            xnocompressed = i + 1;
        ...
        } else if ((0 == strcmp(args->options[i].optionString, "-XXvm:ignoreUnrecognized"))
            || (JNI_TRUE == args->ignoreUnrecognized)) {
...
jint JNICALL JNI_CreateJavaVM(JavaVM** pvm, void** penv, void* vm_args)
{
...
#if defined(J9ZOS390)
    /* since we need to perform some output and look up env vars, we will require the a2e library to be initialized.
       This will happen again, when the VM is actually initialized, but the iconv_init call uses a static flag to make
    ...
    /*
            This is a work-around for a segfault on shut-down on old LinuxPPC GlibC variants (ie:  2.2.5 - where this
       was observed) The core problem may be a VM bug since this library is finalized 3 times when once is expected but
       it looks like a GlibC bug after first-pass investigation.  More investigation will be required when time permits.
...
#if defined(J9ZOS390)
    /**
     * Since any code that looks at strings will fail without iconv_init and testing on z/OS is somewhat inconvenient,
     * this call to initialize the a2e library has been added to the beginning of all the exposed entry points in the
     * file.  A static in iconv_init ensures that only the first init call actually is honoured.
     */
    iconv_init();
#endif
...
jint JNICALL JNI_GetDefaultJavaVMInitArgs(void* vm_args)
{
    ...
    if (globalInitArgs) {
        ...
        if ((jniVersion == JNI_VERSION_1_2) || (jniVersion == JNI_VERSION_1_4) || (jniVersion == JNI_VERSION_1_6)
            || (jniVersion == JNI_VERSION_1_8) || (jniVersion == JNI_VERSION_9) || (jniVersion == JNI_VERSION_10)) {
            return JNI_OK;
        } else {
            return JNI_EVERSION;
        }
 ...
 */
static const char* isPackagedWithCompressedRefs(void) { return strsubdir(COMPRESSEDREFS_DIR); }
...
#if defined(WIN32)
    MultiByteToWideChar(OS_ENCODING_CODE_PAGE, OS_ENCODING_MB_FLAGS, jvmBufferData(buffer), -1, unicodeDLLName,
        (int)strlen(jvmBufferData(buffer)) + 1);
...
static J9StringBuffer* findDir(const char* libraryDir)
{
    ...
    if ((0 == attempted_to_open_master)
        && (NULL == strstr(jvmBufferData(libraryNameWithPath), J9VM_LIB_ARCH_DIR "j9vm/libjvm.so"))) {
        J9StringBuffer* tmpBuffer = jvmBufferCat(NULL, jvmBufferData(buffer));
        char* tmpBufferData = jvmBufferData(tmpBuffer);
...
openLibraries(const char *libraryDir)
{
    ...
    DBG_MSG(("trying %s\n", jvmBufferData(buffer)));
    MultiByteToWideChar(OS_ENCODING_CODE_PAGE, OS_ENCODING_MB_FLAGS, jvmBufferData(buffer), -1, unicodePath,
        (int)strlen(jvmBufferData(buffer)) + 1);
...
static J9StringBuffer* getjvmBin(BOOLEAN removeSubdir)
{
    ...
    WideCharToMultiByte(
        OS_ENCODING_CODE_PAGE, OS_ENCODING_WC_FLAGS, unicodeDLLName, -1, result, J9_MAX_PATH, NULL, NULL);
...
J9StringBuffer* findDirContainingFile(J9StringBuffer* buffer, char* paths, char pathSeparator, char* fileToFind)
{
...
J9StringBuffer* findDirUplevelToDirContainingFile(
    J9StringBuffer* buffer, char* pathEnvar, char pathSeparator, char* fileInPath, int upLevels)
{
