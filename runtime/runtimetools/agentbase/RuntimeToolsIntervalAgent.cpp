 ...
 */
static IDATA J9THREAD_PROC intervalThread(RuntimeToolsIntervalAgent* agent)
{
...
RuntimeToolsIntervalAgent* RuntimeToolsIntervalAgent::newInstance(JavaVM* vm)
{
    ...
    RuntimeToolsIntervalAgent* obj
        = (RuntimeToolsIntervalAgent*)j9mem_allocate_memory(sizeof(RuntimeToolsIntervalAgent), OMRMEM_CATEGORY_VM);
    if (obj) {
...
void RuntimeToolsIntervalAgent::vmStart(jvmtiEnv* jvmti_env, JNIEnv* env)
{
    ...
    createrc = vm->internalVMFunctions->createThreadWithCategory(&generatorThread, 0, J9THREAD_PRIORITY_NORMAL, 0,
        (omrthread_entrypoint_t)intervalThread, agent, J9THREAD_CATEGORY_APPLICATION_THREAD);
