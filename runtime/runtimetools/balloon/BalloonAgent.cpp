 ...
 */
BalloonAgent* BalloonAgent::newInstance(JavaVM* vm)
{
 ...
 */
void BalloonAgent::runAction(void)
{
    if (0 != _balloonSize) {
        PORT_ACCESS_FROM_JAVAVM(getJavaVM());
        J9JavaVM* vm = getJavaVM();

        jobject* allocatedArrays
            = (jobject*)j9mem_allocate_memory(sizeof(jobject) * MAX_BALLOON_SIZE, OMRMEM_CATEGORY_VM);
        ...
        for (int i = 0; i < _balloonSize; i++) {
            ...
            if ((vm->memoryManagerFunctions->j9gc_heap_total_memory(vm) / NUMBER_OF_BYTES_IN_1M)
                < (UDATA)_balloonSize) {
                ...
                getEnv()->SetByteArrayRegion(
                    (jbyteArray)(allocatedArrays[i]), 0, NUMBER_OF_BYTES_IN_1M, initializeBuffer);
 ...
 */
jint BalloonAgent::parseArgument(char* option)
{
    PORT_ACCESS_FROM_JAVAVM(getJavaVM());
