 ...
 */
OSMemoryAgent* OSMemoryAgent::newInstance(JavaVM* vm)
{
 ...
 */
void OSMemoryAgent::runAction(void)
{
    PORT_ACCESS_FROM_JAVAVM(getJavaVM());
            ...
            if (_adjustSoftMx) {
                ...
                if ((_currentSoftMx <= heapTotalMemory)
                    && ((_lastHeapSize < heapTotalMemory)
                           || ((_lastHeapSize == heapTotalMemory) && (0 == freeMemory) && (_freeUnchanged > 1)))) {
                ...
                } else if ((0 != _freeMemoryTarget) && (memoryInfo.availPhysical <= 2 * _freeMemoryTarget)
                        && ((freeMemory > LOTS_OF_FREE_MEMORY) || (FREE_UNCHANGED_OVERRIDE < _freeUnchanged))
#if defined(AIXPPC) && !defined(J9OS_I5)
                    ...
                    || ((_totalFreePhysicalTarget != 0) && (_totalPhysical > 0)
                           && ((GIG * 2.5 - _totalPhysical) < (MEG * _totalFreePhysicalTarget)))
#endif
                ) {
#if defined(AIXPPC) && !defined(J9OS_I5)
                    if (_hpiTarget != HPI_DISABLED) {
                        if ((currentHPI > 2 * _hpiTarget) || (currentHPITime > 2 * _hpiTimeTarget)) {
                            newSoftMx = (UDATA)(((U_64)_maxHeapSize * SOFTMX_RATIO[SOFTMX_BELOW_TARGET_RATIO]) / 100);
                            message("Adjusted based on hpiTarget\n");
                        } else if ((currentHPI > _hpiTarget) || (currentHPITime > _hpiTimeTarget)) {
                            newSoftMx
                                = (UDATA)(((U_64)_maxHeapSize * SOFTMX_RATIO[SOFTMX_BELOW_DOUBLE_TARGET_RATIO]) / 100);
                            message("Adjusted based on hpiTarget\n");
                        }
                        ...
                        if ((SHARED_POOL_MEMORY - _totalPhysical) < (MEG * _totalFreePhysicalTarget)) {
                            if ((SHARED_POOL_MEMORY - _totalPhysical) < ((MEG * _totalFreePhysicalTarget) / 2)) {
                                newSoftMx
                                    = (UDATA)(((U_64)_maxHeapSize * SOFTMX_RATIO[SOFTMX_BELOW_TARGET_RATIO]) / 100);
                            } else {
                                newSoftMx = (UDATA)(
                                    ((U_64)_maxHeapSize * SOFTMX_RATIO[SOFTMX_BELOW_DOUBLE_TARGET_RATIO]) / 100);
                            }
                    ...
                    if (((freeMemory > LOTS_OF_FREE_MEMORY) || (FREE_UNCHANGED_OVERRIDE < _freeUnchanged)
#if defined(AIXPPC) && !defined(J9OS_I5)
                ...
                if (_currentSoftMx != newSoftMx) {
                    ...
                    /*
                     * ok there was no change in target, now let us see if the heap has actually gone down to our
                     * previous target if not we will trigger a gc to attempt to force this.  If after _maxgcs attempts
                     * the heap size has not changed, we give up to avoid triggering too many gcs
                     */
                    ...
                    /*	else if ((_currentSoftMx != _maxHeapSize)&&(_currentSoftMx <heapTotalMemory)&&(MIN_FREE_HEAP >
                       freeMemory)) { _currentSoftMx = OMR_MIN(heapTotalMemory + SOFTMX_GROW_INCREMENT,_maxHeapSize);
...
jint OSMemoryAgent::setup(char* options)
{
    ...
    if (rc == 0) {
        ...
        if (_jvmId != -1) {
            ...
            if (setsockopt(_multicastSocketIn, IPPROTO_IP, IP_ADD_MEMBERSHIP, &multicastReq, sizeof(multicastReq))
                < 0) {
...
jint OSMemoryAgent::parseArgument(char* option)
{
    PORT_ACCESS_FROM_JAVAVM(getJavaVM());
...
#if defined(AIXPPC) && !defined(J9OS_I5)
void OSMemoryAgent::getPerfStatData(PerfStatData* data)
{
...
void OSMemoryAgent::broadcastToHive(HiveData* data)
{
    ...
    if (sendto(_multicastSocketIn, data, sizeof(HiveData), 0, (struct sockaddr*)&multicastAddrOut,
            sizeof(multicastAddrOut))
        < 0) {
        error("Failed to broadcast to hive\n");
    }
}

void OSMemoryAgent::updateFromHive(void)
{
    ...
    while ((bytesReceived = recvfrom(
                _multicastSocketIn, &data, sizeof(HiveData), 0, (struct sockaddr*)&fromAddress, &addressLength))
        >= 0) {
