 ...
 */
static void hookSoftmxOOMEvent(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    MM_SoftmxOOMEvent* event = (MM_SoftmxOOMEvent*)eventData;
    SoftMxTest* agent = (SoftMxTest*)userData;
    agent->adjustSoftmxForOOM(
        event->timestamp, event->maxHeapSize, event->currentHeapSize, event->currentSoftMX, event->bytesRequired);
}
...
SoftMxTest* SoftMxTest::newInstance(JavaVM* vm)
{
...
void SoftMxTest::runAction(void) { PORT_ACCESS_FROM_JAVAVM(getJavaVM()); }

jint SoftMxTest::setup(char* options)
{
    jint rc = RuntimeToolsIntervalAgent::setup(options);
    if (rc == 0) {
        if (-1
            == (*_gcOmrHooks)
                   ->J9HookRegisterWithCallSite(_gcOmrHooks, J9HOOK_MM_OMR_OOM_DUE_TO_SOFTMX, hookSoftmxOOMEvent,
...
jint SoftMxTest::parseArgument(char* option)
{
    PORT_ACCESS_FROM_JAVAVM(getJavaVM());
...
void SoftMxTest::adjustSoftmxForOOM(
    U_64 timestamp, UDATA maxHeapSize, UDATA currentHeapSize, UDATA currentSoftMx, UDATA bytesRequired)
{
    if (_testCallbackOOM) {
    ...
    if (_testCallbackExpand) {
        UDATA newSoftMx = OMR_MIN(
            OMR_MAX(currentHeapSize, currentSoftMx) + OMR_MAX(bytesRequired, SOFTMX_EXPAND_AMOUNT), maxHeapSize);
