...
void JNICALL vmInitWrapper(jvmtiEnv* jvmti_env, JNIEnv* jni_env, jthread thread)
{
...
void JNICALL classPrepareWrapper(jvmtiEnv* jvmti_env, JNIEnv* jni_env, jthread thread, jclass klass)
{
    if (NULL != runtimeStateAgent) {
...
void JNICALL vmDeathWrapper(jvmtiEnv* jvmti_env, JNIEnv* jni_env)
{
    if (NULL != runtimeStateAgent) {
...
void vmRuntimeStateChangeWrapper(J9HookInterface** hook, uintptr_t eventNum, void* eventData, void* userData)
{
 ...
 *
 */
jint VMRuntimeStateAgent::setup(char* options)
{
 ...
 */
bool VMRuntimeStateAgent::init(void)
{
    jbyte counter = 0;
 ...
 */
void VMRuntimeStateAgent::kill(void)
{
    PORT_ACCESS_FROM_JAVAVM(getJavaVM());
    if (NULL != _appClass) {
 ...
 */
VMRuntimeStateAgent* VMRuntimeStateAgent::newInstance(JavaVM* vm)
{
    J9JavaVM* javaVM = ((J9InvocationJavaVM*)vm)->j9vm;
    PORT_ACCESS_FROM_JAVAVM(javaVM);
    VMRuntimeStateAgent* obj
        = (VMRuntimeStateAgent*)j9mem_allocate_memory(sizeof(VMRuntimeStateAgent), OMRMEM_CATEGORY_VM);
 ...
 */
jint VMRuntimeStateAgent::parseArgument(char* option)
{
    PORT_ACCESS_FROM_JAVAVM(getJavaVM());
...
void JNICALL VMRuntimeStateAgent::cbVMInit(jvmtiEnv* jvmti_env, JNIEnv* jni_env, jthread thread)
{
...
void JNICALL VMRuntimeStateAgent::cbClassPrepare(jvmtiEnv* jvmti_env, JNIEnv* jni_env, jthread thread, jclass klass)
{
    ...
    if (NULL == activeToIdleFieldID) {
        errorFormat(
            "%s:%d ERROR: Failed to find field activeToIdleFieldID in class %s\n", __FILE__, __LINE__, _appClass);
        return;
    }
...
void VMRuntimeStateAgent::cbVMRuntimeStateChange(void* eventData)
{
...
void VMRuntimeStateAgent::registerHook(void)
{
    J9JavaVM* vm = getJavaVM();
    ...
    if (NULL != vmHooks) {
        IDATA rc
            = (*vmHooks)->J9HookRegister(vmHooks, J9HOOK_VM_RUNTIME_STATE_CHANGED, vmRuntimeStateChangeWrapper, this);
        if (0 != rc) {
            errorFormat(
                "%s:%d ERROR: Failed to register for J9HOOK_VM_RUNTIME_STATE_CHANGED hook\n", __FILE__, __LINE__);
        }
    }
}

void VMRuntimeStateAgent::verifyResult(void)
{
