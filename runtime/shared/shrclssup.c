...
#define SHRCLSSUP_ERR_TRACE(verbose, var) \
    if (verbose)                          \
    j9nls_printf(PORTLIB, J9NLS_ERROR, var)
#define SHRCLSSUP_ERR_TRACE1(verbose, var, p1) \
    if (verbose)                               \
...
IDATA J9VMDllMain(J9JavaVM* vm, IDATA stage, void* reserved)
{
    ...
    if (vm->sharedCacheAPI == NULL) {
        ...
        runtimeFlags
            |= ((j9shr_isPlatformDefaultPersistent(vm) == TRUE) ? J9SHR_RUNTIMEFLAG_ENABLE_PERSISTENT_CACHE : 0);
        ...
        if ((index = FIND_ARG_IN_VMARGS(OPTIONAL_LIST_MATCH, OPT_XSHARECLASSES, NULL)) >= 0) {
            ...
            if (OPTION_OK == parseRc) {
                ...
                do {
                    ...
                    if ((strcmp(optionsBufferPtr, OPTION_RESET) == 0)
                        && (optionsBufferPtr[strlen(OPTION_RESET) + 1] == 0)) {
                        if ((index = FIND_NEXT_ARG_IN_VMARGS(OPTIONAL_LIST_MATCH, OPT_XSHARECLASSES, NULL, index))
                            >= 0) {
                            ...
                            if (OPTION_OK != rc) {
                                if (OPTION_OVERFLOW == rc) {
                                    SHRCLSSUP_ERR_TRACE1(J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT,
                                        J9NLS_SHRC_SHRCLSSUP_FAILURE_OPTION_BUFFER_OVERFLOW, SHR_SUBOPT_BUFLEN);
                                }
                                return J9VMDLLMAIN_FAILED;
                            }
                        }
                    }
                    /* If disablecorruptcachedumps is found as an extra option to the right of other options, add it to
                     * the command-line */
                    if ((strcmp(optionsBufferPtr, OPTION_DISABLE_CORRUPT_CACHE_DUMPS) == 0)
                        && (optionsBufferPtr[strlen(OPTION_DISABLE_CORRUPT_CACHE_DUMPS) + 1] == 0)) {
                        if ((index = FIND_NEXT_ARG_IN_VMARGS(OPTIONAL_LIST_MATCH, OPT_XSHARECLASSES, NULL, index))
                            >= 0) {
                 ...
                 * These options should be checked before parseArgs() to allow -Xshareclasses:[enable|disable]BCI to
                ...
                vm->sharedCacheAPI->parseResult = parseArgs(vm, optionsBufferPtr, &runtimeFlags, &verboseFlags,
                    &cacheName, &modContext, &expireTime, &ctrlDirName, &cacheDirPermStr, &methodSpecs,
                    &printStatsOptions, &storageKeyTesting);
                if ((RESULT_PARSE_FAILED == vm->sharedCacheAPI->parseResult)) {
                    return J9VMDLLMAIN_FAILED;
                }

                if (cacheName != NULL) {
                    vm->sharedCacheAPI->cacheName
                        = (char*)j9mem_allocate_memory(strlen(cacheName) + 1, J9MEM_CATEGORY_CLASSES);
                ...
                if (ctrlDirName != NULL) {
                    vm->sharedCacheAPI->ctrlDirName
                        = (char*)j9mem_allocate_memory(strlen(ctrlDirName) + 1, J9MEM_CATEGORY_CLASSES);
                ...
                if (modContext != NULL) {
                    vm->sharedCacheAPI->modContext
                        = (char*)j9mem_allocate_memory(strlen(modContext) + 1, J9MEM_CATEGORY_CLASSES);
                ...
                if (expireTime != NULL) {
                    vm->sharedCacheAPI->expireTime
                        = (char*)j9mem_allocate_memory(strlen(expireTime) + 1, J9MEM_CATEGORY_CLASSES);
                ...
                if (NULL != methodSpecs) {
                    vm->sharedCacheAPI->methodSpecs
                        = (char*)j9mem_allocate_memory(strlen(methodSpecs) + 1, J9MEM_CATEGORY_CLASSES);
    ...
    switch (stage) {
    /* This code detects "none" as part of Xshareclasses and unloads the DLL if it is found. Must therefore be in this
     * stage or earlier */
    ...
    case JIT_INITIALIZED: {
        ...
        if ((vm->sharedCacheAPI->sharedCacheEnabled == TRUE)
            && (vm->sharedCacheAPI->parseResult != RESULT_DO_UTILITIES)) {
