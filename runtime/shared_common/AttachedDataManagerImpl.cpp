 ...
 */
SH_AttachedDataManagerImpl* SH_AttachedDataManagerImpl::newInstance(
    J9JavaVM* vm, SH_SharedCache* cache_, SH_AttachedDataManagerImpl* memForConstructor)
{
...
void SH_AttachedDataManagerImpl::initialize(J9JavaVM* vm, SH_SharedCache* cache_, BlockPtr memForConstructor)
{
 ...
 */
bool SH_AttachedDataManagerImpl::storeNew(
...
U_32 SH_AttachedDataManagerImpl::getHashTableEntriesFromCacheSize(UDATA cacheSizeBytes)
{
...
{
    return (UDATA)ADWCACHEOFFSET((AttachedDataWrapper*)ITEMDATA(cacheItem))
        + (UDATA)ADWTYPE((AttachedDataWrapper*)ITEMDATA(cacheItem));
}
