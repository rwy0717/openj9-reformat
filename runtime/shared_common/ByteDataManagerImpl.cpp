...
J9HashTable* SH_ByteDataManagerImpl::localHashTableCreate(J9VMThread* currentThread, U_32 initialEntries)
{
 ...
 */
SH_ByteDataManagerImpl* SH_ByteDataManagerImpl::newInstance(
    J9JavaVM* vm, SH_SharedCache* cache_, SH_ByteDataManagerImpl* memForConstructor)
{
...
void SH_ByteDataManagerImpl::initialize(J9JavaVM* vm, SH_SharedCache* cache_, BlockPtr memForConstructor)
{
...
SH_ByteDataManagerImpl::localInitializePools(J9VMThread* currentThread)
{
    ...
    _linkedListImplPool = pool_new(sizeof(SH_ByteDataManagerImpl::BdLinkedListImpl), 0, 0, 0, J9_GET_CALLSITE(),
...
void SH_ByteDataManagerImpl::localTearDownPools(J9VMThread* currentThread)
{
...
U_32 SH_ByteDataManagerImpl::getHashTableEntriesFromCacheSize(UDATA cacheSizeBytes)
{
 ...
 */
bool SH_ByteDataManagerImpl::storeNew(
 ...
 */
ByteDataWrapper* SH_ByteDataManagerImpl::findSingleEntry(
    J9VMThread* currentThread, const char* key, UDATA keylen, UDATA dataType, U_16 jvmID, UDATA* dataLen)
{
    ...
    if ((found = (BdLinkedListImpl*)hllTableLookup(currentThread, key, (U_16)keylen, true))) {
        ...
        do {
            ...
            if (!_cache->isStale(item) && (dataType == (UDATA)(wrapper->dataType))
 ...
 */
void SH_ByteDataManagerImpl::markAllStaleForKey(J9VMThread* currentThread, const char* key, UDATA keylen)
{
    BdLinkedListImpl *found, *walk;
...
void SH_ByteDataManagerImpl::setDescriptorFields(const ByteDataWrapper* wrapper, J9SharedDataDescriptor* descriptor)
{
 ...
 */
IDATA
SH_ByteDataManagerImpl::find(J9VMThread* currentThread, const char* key, UDATA keylen, UDATA limitDataType,
    UDATA includePrivateData, J9SharedDataDescriptor* firstItem, const J9Pool* descriptorPool)
{
    ...
    if ((found = (BdLinkedListImpl*)hllTableLookup(currentThread, key, (U_16)keylen, true))) {
        ...
        do {
            ...
            if (!_cache->isStale(item) && (!limitDataType || (limitDataType == (UDATA)wrapper->dataType))
                && (includePrivateData || (!includePrivateData && (wrapper->privateOwnerID == 0)))) {
                if (descriptorPool
                    && (newPoolEntry = (J9SharedDataDescriptor*)pool_newElement((J9Pool*)descriptorPool))) {
...
SH_ByteDataManagerImpl::releasePrivateEntry(J9VMThread* currentThread, const J9SharedDataDescriptor* data)
{
    ...
    if (!data || (data->flags & J9SHRDATA_PRIVATE_TO_DIFFERENT_JVM) || !(data->flags & J9SHRDATA_IS_PRIVATE)
        || (data->flags & J9SHRDATA_USE_READWRITE)) {
...
void SH_ByteDataManagerImpl::runExitCode(void)
{
...
UDATA
SH_ByteDataManagerImpl::getUnindexedDataBytes() { return _unindexedBytes; }
 ...
 */
IDATA
SH_ByteDataManagerImpl::primeHashtables(
