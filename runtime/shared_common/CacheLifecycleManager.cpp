...
#define CLM_TRACE(var1) \
    if (verboseFlags)   \
    j9nls_printf(PORTLIB, J9NLS_INFO, var1)
#define CLM_TRACE1(var1, p1) \
    if (verboseFlags)        \
    j9nls_printf(PORTLIB, J9NLS_INFO, var1, p1)
#define CLM_TRACE2(var1, p1, p2) \
    if (verboseFlags)            \
    j9nls_printf(PORTLIB, J9NLS_INFO, var1, p1, p2)
#define CLM_TRACE_NOTAG(var1) \
    if (verboseFlags)         \
    j9nls_printf(PORTLIB, J9NLS_DO_NOT_PRINT_MESSAGE_TAG, var1)
#define CLM_TRACE1_NOTAG(var1, p1) \
    if (verboseFlags)              \
    j9nls_printf(PORTLIB, J9NLS_DO_NOT_PRINT_MESSAGE_TAG, var1, p1)
#define CLM_TRACE2_NOTAG(var1, p1, p2) \
    if (verboseFlags)                  \
    j9nls_printf(PORTLIB, J9NLS_DO_NOT_PRINT_MESSAGE_TAG, var1, p1, p2)
#define CLM_ERR_TRACE(var1) \
    if (verboseFlags)       \
    j9nls_printf(PORTLIB, J9NLS_ERROR, var1)
#define CLM_ERR_TRACE1(var1, p1) \
    if (verboseFlags)            \
    j9nls_printf(PORTLIB, J9NLS_ERROR, var1, p1)
...
static J9Pool* getCacheList(
    struct J9JavaVM* vm, const char* ctrlDirName, UDATA groupPerm, bool includeOldGenerations, UDATA reason);

/* Function to iterate over all shared class caches and snapshots present in a directory. This function is exposed
 * through the sharedCacheAPI table. If useCommandLineValues is TRUE, then get cacheDir from command line option. If
 * useCommandLineValues is FALSE, then use parameter to get cache signature. If cacheDir is still not available then use
 * default value.
 *
    ...
    IDATA (*callback)(J9JavaVM* vm, J9SharedCacheInfo* event_data, void* user_data), void* user_data)
{
    ...
    if (useCommandLineValue == TRUE) {
        ...
        snapshotList = SH_OSCache::getAllCacheStatistics(vm, vm->sharedCacheAPI->ctrlDirName, vmGroupPerm, 0,
            J2SE_VERSION(vm), false, false, SHR_STATS_REASON_ITERATE, false);
    } else {
        /* Don't include old generations */
        cacheList = getCacheList(vm, ctrlDirName, groupPerm, false, SHR_STATS_REASON_ITERATE);
        snapshotList = SH_OSCache::getAllCacheStatistics(
            vm, ctrlDirName, groupPerm, 0, J2SE_VERSION(vm), false, false, SHR_STATS_REASON_ITERATE, false);
    }
    ...
    if (((state->printCompatibleCache == true) && (currentItem->isCompatible))
        ...
        if (state->printIntro) {
            ...
            SH_OSCache::getCacheDir(
                state->vm, state->ctrlDirName, cacheDir, J9SH_MAXPATH, J9PORT_SHR_CACHE_TYPE_PERSISTENT);
        ...
        if (currentItem->nattach == 0) {
            if (J9SH_OSCACHE_UNKNOWN == currentItem->lastdetach) {
                ...
                /*
                 *	For now we will use ctime function - we need to modify str_ftime to take in the time at some
                 *point j9str_ftime(formatted, 30, "%d %b %Y %H:%m", currentItem->lastdetach * 1000);
                 *  j9tty_printf(PORTLIB, "%-15s", formatted);
                 */

                j9tty_printf(PORTLIB, "%-15s", ctime(&t));
            }
        } else if ((currentItem->nattach == J9SH_OSCACHE_UNKNOWN)
            || (currentItem->lastdetach == J9SH_OSCACHE_UNKNOWN)) {
            if (J9PORT_SHR_CACHE_TYPE_SNAPSHOT == currentItem->versionData.cacheType) {
                /* no last detach time for snapshot,  currentItem->nattach and currentItem->lastdetach are both
                 * J9SH_OSCACHE_UNKNOWN */
                j9tty_printf(PORTLIB, "\n");
            } else {
                j9tty_printf(PORTLIB, "%-15s\n", "Unknown");
            }
...
static void deleteSharedCache(void* element, void* param)
{
    ...
    if (J9PORT_SHR_CACHE_TYPE_SNAPSHOT == cacheType) {
        returnVal = j9shr_destroy_snapshot(state->vm, state->ctrlDirName, verboseFlags, cacheInfo->name,
            cacheInfo->generation, cacheInfo->generation, &(cacheInfo->versionData));
    } else {
        returnVal = j9shr_destroy_cache(state->vm, state->ctrlDirName, verboseFlags, cacheInfo->name,
            cacheInfo->generation, cacheInfo->generation, &(cacheInfo->versionData), false);
    }
    if ((J9SH_DESTROY_FAILED_CURRENT_GEN_CACHE == returnVal) || (J9SH_DESTROY_FAILED_OLDER_GEN_CACHE == returnVal)
        || (J9SH_DESTROY_FAILED_CURRENT_GEN_SNAPSHOT == returnVal)
        || (J9SH_DESTROY_FAILED_OLDER_GEN_SNAPSHOT == returnVal) || (J9SH_DESTROYED_NONE == returnVal)) {
        state->result = -1;
    ...
    struct J9JavaVM* vm, const char* ctrlDirName, UDATA groupPerm, bool includeOldGenerations, UDATA reason)
{
    ...
    cacheList = SH_OSCache::getAllCacheStatistics(
        vm, ctrlDirName, groupPerm, 0, J2SE_VERSION(vm), includeOldGenerations, false, reason, true);
 ...
 * It is the responsibility of the caller to free the pool created */
static J9Pool* findIncompatibleCachesForName(
    struct J9JavaVM* vm, const char* ctrlDirName, UDATA groupPerm, const char* name)
{
    J9Pool* cacheList;
    ...
    /* Verbose flags not required - do not want to see permissions error accessing all caches */
    cacheList = SH_OSCache::getAllCacheStatistics(
        vm, ctrlDirName, groupPerm, 0, J2SE_VERSION(vm), true, true, SHR_STATS_REASON_GETNAME, true);
...
j9shr_list_caches(struct J9JavaVM* vm, const char* ctrlDirName, UDATA groupPerm, UDATA verboseFlags)
{
    ...
    snapshotList = SH_OSCache::getAllCacheStatistics(
        vm, ctrlDirName, groupPerm, 0, J2SE_VERSION(vm), false, false, SHR_STATS_REASON_LIST, false);
...
j9shr_destroy_all_snapshot(struct J9JavaVM* vm, const char* ctrlDirName, UDATA groupPerm, UDATA verboseFlags)
{
    ...
    snapshotList = SH_OSCache::getAllCacheStatistics(
        vm, ctrlDirName, groupPerm, 0, J2SE_VERSION(vm), true, false, SHR_STATS_REASON_DESTROY, false);
 ...
 *
 * @returns	J9SH_DESTROYED_ALL_CACHE/J9SH_DESTROYED_ALL_SNAPSHOT when no cache/snapshot exists or successfully
 * destroyed all caches/snapshots, J9SH_DESTROYED_NONE when it fails to destroy any cache/snapshot,
 * 			J9SH_DESTROY_FAILED_CURRENT_GEN_CACHE/J9SH_DESTROY_FAILED_CURRENT_GEN_SNAPSHOT when it failed to
 * destroy cache/snapshot of current generation,
 * 			J9SH_DESTROY_FAILED_OLDER_GEN_CACHE/J9SH_DESTROY_FAILED_OLDER_GEN_SNAPSHOT when it failed to destroy
 * one or more older generation cache/snapshot and either current generation cache/snapshot does not exists or is
 * successfully destroyed.
 *
 * Note: cacheType = 0 is an indicator to use platform default value for cacheType.
 */
IDATA
j9shr_destroySharedCache(
    J9JavaVM* vm, const char* ctrlDirName, const char* name, U_32 cacheType, BOOLEAN useCommandLineValues)
{
    ...
    if (modifyCacheName(
            vm, cacheName, vm->sharedCacheAPI->verboseFlags, &modifiedCacheNamePtr, USER_SPECIFIED_CACHE_NAME_MAXLEN)
        == false) {
        return -1;
    }
    if (versionData.cacheType == 0) {
        versionData.cacheType = (j9shr_isPlatformDefaultPersistent(vm) == TRUE) ? J9PORT_SHR_CACHE_TYPE_PERSISTENT
                                                                                : J9PORT_SHR_CACHE_TYPE_NONPERSISTENT;
    }
    ...
    if (J9PORT_SHR_CACHE_TYPE_SNAPSHOT == cacheType) {
        rc = j9shr_destroy_snapshot(vm, ctrlDir, verboseFlags, modifiedCacheNamePtr, OSCACHE_LOWEST_ACTIVE_GEN,
            OSCACHE_CURRENT_CACHE_GEN, &versionData);
    } else {
        rc = j9shr_destroy_cache(vm, ctrlDir, verboseFlags, modifiedCacheNamePtr, OSCACHE_LOWEST_ACTIVE_GEN,
            OSCACHE_CURRENT_CACHE_GEN, &versionData, false);
    }
 ...
 * 			J9SH_DESTROY_FAILED_OLDER_GEN_CACHE when it failed to destroy one or more older generation cache and
 * either current generation cache does not exists or is successfully destroyed.
 */
IDATA
j9shr_destroy_cache(struct J9JavaVM* vm, const char* ctrlDirName, UDATA verboseFlags, const char* cacheName,
    UDATA generationStart, UDATA generationEnd, J9PortShcVersion* versionData, BOOLEAN isReset)
{
    ...
    for (UDATA i = generationStart; i <= lastGeneration; i++) {
        /* explicitly pass 0 as verboseFlags to j9shr_stat_cache to suppress printing of messages related to cache
         * existence in all cases */
        if (1 == j9shr_stat_cache(vm, cacheDirName, 0, cacheName, versionData, i)) {
            ...
            if (!oscache->startup(vm, ctrlDirName, vm->sharedCacheAPI->cacheDirPerm, cacheName,
                    vm->sharedClassPreinitConfig, 0, J9SH_OSCACHE_OPEXIST_DESTROY, verboseFlags, 0 /*runtime check*/, 0,
                    vm->sharedCacheAPI->storageKeyTesting, versionData, NULL, SHR_STARTUP_REASON_DESTROY)) {
 ...
 * 			J9SH_DESTROY_FAILED_CURRENT_GEN_SNAPSHOT when it failed to destroy snapshot of cache of current
 * generation, J9SH_DESTROY_FAILED_OLDER_GEN_SNAPSHOT when it failed to destroy one or more snapshot of older generation
 * cache and either snapshot of current generation cache does not exists or is successfully destroyed.
 */
IDATA
j9shr_destroy_snapshot(struct J9JavaVM* vm, const char* ctrlDirName, UDATA verboseFlags, const char* snapshotName,
    UDATA generationStart, UDATA generationEnd, J9PortShcVersion* versionData)
{
    ...
    for (UDATA i = generationStart; i <= lastGeneration; i++) {
        SH_OSCache::getCacheVersionAndGen(
            PORTLIB, vm, nameWithVGen, CACHE_ROOT_MAXLEN, snapshotName, versionData, i, false);
        ...
        if (EsIsFile == j9file_attr(pathFileName)) {
            ...
            /* rc is -1 means J9PORT_ERROR_FILE_NOENT occurred while deleting the snapshot file, silently ignore this
             * case*/
        }
    }
    ...
    if (OSCACHE_CURRENT_CACHE_GEN == generationEnd) {
        SH_OSCache::getCacheVersionAndGen(
            PORTLIB, vm, nameWithVGen, CACHE_ROOT_MAXLEN, snapshotName, versionData, OSCACHE_CURRENT_CACHE_GEN, false);
        /* No check for the return value of getCachePathName() as it always returns 0 */
        SH_OSCache::getCachePathName(PORTLIB, cacheDirName, pathFileName, J9SH_MAXPATH, nameWithVGen);
        if (EsIsFile == j9file_attr(pathFileName)) {
            IDATA rc = deleteSnapshot(vm, verboseFlags, pathFileName);
 ...
 */
IDATA
j9shr_stat_cache(struct J9JavaVM* vm, const char* cacheDirName, UDATA verboseFlags, const char* name,
    J9PortShcVersion* versionData, UDATA generation)
{
    ...
    SH_OSCache::getCacheVersionAndGen(
        PORTLIB, vm, nameWithVGen, CACHE_ROOT_MAXLEN, name, versionData, generation, true);
 ...
 */
IDATA
j9shr_report_utility_incompatible(struct J9JavaVM* vm, const char* ctrlDirName, UDATA groupPerm, UDATA verboseFlags,
    const char* name, const char* utility)
{
    J9Pool* list;
    PORT_ACCESS_FROM_JAVAVM(vm);
    IDATA rc = 0;
