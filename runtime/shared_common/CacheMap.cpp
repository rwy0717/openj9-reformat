...
#define CACHEMAP_TRACE(verboseLevel, nlsFlags, var1) \
    if (_verboseFlags & verboseLevel)                \
    j9nls_printf(PORTLIB, nlsFlags, var1)
#define CACHEMAP_TRACE1(verboseLevel, nlsFlags, var1, p1) \
    if (_verboseFlags & verboseLevel)                     \
    j9nls_printf(PORTLIB, nlsFlags, var1, p1)
#define CACHEMAP_TRACE2(verboseLevel, nlsFlags, var1, p1, p2) \
    if (_verboseFlags & verboseLevel)                         \
    j9nls_printf(PORTLIB, nlsFlags, var1, p1, p2)
#define CACHEMAP_TRACE3(verboseLevel, nlsFlags, var1, p1, p2, p3) \
    if (_verboseFlags & verboseLevel)                             \
    j9nls_printf(PORTLIB, nlsFlags, var1, p1, p2, p3)
#define CACHEMAP_TRACE4(verboseLevel, nlsFlags, var1, p1, p2, p3, p4) \
    if (_verboseFlags & verboseLevel)                                 \
    j9nls_printf(PORTLIB, nlsFlags, var1, p1, p2, p3, p4)
...
#define CACHEMAP_PRINT6(nlsFlags, var1, p1, p2, p3, p4, p5, p6) \
    j9nls_printf(PORTLIB, nlsFlags, var1, p1, p2, p3, p4, p5, p6)
#define CACHEMAP_PRINT7(nlsFlags, var1, p1, p2, p3, p4, p5, p6, p7) \
    j9nls_printf(PORTLIB, nlsFlags, var1, p1, p2, p3, p4, p5, p6, p7)
#define CACHEMAP_PRINT8(nlsFlags, var1, p1, p2, p3, p4, p5, p6, p7, p8) \
    j9nls_printf(PORTLIB, nlsFlags, var1, p1, p2, p3, p4, p5, p6, p7, p8)

#define CACHEMAP_FMTPRINT1(nlsFlags, var1, p1) j9nls_printf(PORTLIB, nlsFlags, var1, 1, ' ', p1)

static char* formatAttachedDataString(J9VMThread* currentThread, U_8* attachedData, UDATA attachedDataLength,
...
#define RUNTIME_FLAGS_PREVENT_BLOCK_DATA_UPDATE \
    (J9SHR_RUNTIMEFLAG_DENY_CACHE_UPDATES | J9SHR_RUNTIMEFLAG_BLOCK_SPACE_FULL | J9SHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL)
 ...
 */
IDATA
SH_CacheMap::enterLocalMutex(
    J9VMThread* currentThread, omrthread_monitor_t monitor, const char* name, const char* caller)
{
    if (_isAssertEnabled) {
 ...
 */
IDATA
SH_CacheMap::enterReentrantLocalMutex(
    J9VMThread* currentThread, omrthread_monitor_t monitor, const char* name, const char* caller)
{
    IDATA rc = 0;
 ...
 */
IDATA
SH_CacheMap::exitReentrantLocalMutex(
    J9VMThread* currentThread, omrthread_monitor_t monitor, const char* name, const char* caller)
{
    IDATA rc = 0;
 ...
 */
SH_CacheMap* SH_CacheMap::newInstance(J9JavaVM* vm, J9SharedClassConfig* sharedClassConfig,
    SH_CacheMap* memForConstructor, const char* cacheName, I_32 cacheTypeRequired)
{
    ...
    newCacheMap->initialize(vm, sharedClassConfig, ((BlockPtr)memForConstructor + sizeof(SH_CacheMap)), cacheName,
        cacheTypeRequired, false);
 ...
 */
void SH_CacheMap::dontNeedMetadata(J9VMThread* currentThread)
{
 ...
 */
SH_CacheMapStats* SH_CacheMap::newInstanceForStats(J9JavaVM* vm, SH_CacheMap* memForConstructor, const char* cacheName)
{
    SH_CacheMap* newCacheMap = memForConstructor;
...
/* THREADING: Only ever single threaded */
void SH_CacheMap::initialize(J9JavaVM* vm, J9SharedClassConfig* sharedClassConfig, BlockPtr memForConstructor,
    const char* cacheName, I_32 cacheTypeRequired, bool startupForStats)
{
    ...
    _ccPool = pool_new(SH_CompositeCacheImpl::getRequiredConstrBytes(true, startupForStats), 0, 0, 0, J9_GET_CALLSITE(),
    ...
    _ccHead = _cc = SH_CompositeCacheImpl::newInstance(vm, sharedClassConfig,
        (SH_CompositeCacheImpl*)(allocPtr += SH_Managers::getRequiredConstrBytes()), cacheName, cacheTypeRequired,
        startupForStats);

    _tsm = SH_TimestampManagerImpl::newInstance(vm,
        (SH_TimestampManagerImpl*)(allocPtr
            += SH_CompositeCacheImpl::getRequiredConstrBytesWithCommonInfo(false, startupForStats)),
        sharedClassConfig);

    _cpm = SH_ClasspathManagerImpl2::newInstance(
        vm, this, _tsm, (SH_ClasspathManagerImpl2*)(allocPtr += SH_TimestampManagerImpl::getRequiredConstrBytes()));

    _scm = SH_ScopeManagerImpl::newInstance(
        vm, this, (SH_ScopeManagerImpl*)(allocPtr += SH_ClasspathManagerImpl2::getRequiredConstrBytes()));

    _rcm = SH_ROMClassManagerImpl::newInstance(
        vm, this, _tsm, (SH_ROMClassManagerImpl*)(allocPtr += SH_ScopeManagerImpl::getRequiredConstrBytes()));

    _cmm = SH_CompiledMethodManagerImpl::newInstance(
        vm, this, (SH_CompiledMethodManagerImpl*)(allocPtr += SH_ROMClassManagerImpl::getRequiredConstrBytes()));

    _bdm = SH_ByteDataManagerImpl::newInstance(
        vm, this, (SH_ByteDataManagerImpl*)(allocPtr += SH_CompiledMethodManagerImpl::getRequiredConstrBytes()));

    _adm = SH_AttachedDataManagerImpl::newInstance(
        vm, this, (SH_AttachedDataManagerImpl*)(allocPtr += SH_ByteDataManagerImpl::getRequiredConstrBytes()));
 ...
 * THREADING: Only ever single threaded
 */
void SH_CacheMap::cleanup(J9VMThread* currentThread)
{
...
SH_CacheMap::sanityWalkROMClassSegment(J9VMThread* currentThread, SH_CompositeCacheImpl* cache)
{
    ...
    while (walk < endOfROMSegment) {
        ...
        if ((walk <= prev) || (walk > endOfROMSegment)) {
            ...
            CACHEMAP_TRACE1(
                J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, J9NLS_ERROR, J9NLS_SHRC_CM_READ_CORRUPT_ROMCLASS, walk);
 ...
 */
IDATA
SH_CacheMap::startup(J9VMThread* currentThread, J9SharedClassPreinitConfig* piconfig, const char* rootName,
    const char* cacheDirName, UDATA cacheDirPerm, BlockPtr cacheMemoryUT, bool* cacheHasIntegrity)
{
    IDATA itemsRead = 0;
    IDATA rc = 0;
    ...
    if (omrthread_monitor_init(&_refreshMutex, 0)) {
        CACHEMAP_TRACE(
            J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, J9NLS_ERROR, J9NLS_SHRC_CM_FAILED_CREATE_REFRESH_MUTEX);
    ...
    do {
        ...
        rc = _ccHead->startup(currentThread, piconfig, cacheMemoryUT, _runtimeFlags, _verboseFlags, rootName,
            cacheDirName, cacheDirPerm, &_actualSize, &_localCrashCntr, true, cacheHasIntegrity);
        ...
        if (rc == CC_STARTUP_OK) {
            /* THREADING: We want the cache mutex here as we are reading all available data. Don't want updates
             * happening as we read. */
            if (_ccHead->enterWriteMutex(currentThread, false, fnName) == 0) {
                ...
                if ((*_runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_REDUCE_STORE_CONTENTION)
                 ...
                 * 	- Secondly, when setting cache full flags, the code expects to be holding the write mutex, which
                 * is not done in SH_CompositeCacheImpl::startup().
                 *
                 * Do not call fillCacheIfNearlyFull() in readonly mode, as we cannot write anything to cache.
                 */
                if ((0 == (*_runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_NESTED)) && (!_ccHead->isRunningReadOnly())) {
                ...
                CACHEMAP_TRACE(J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, J9NLS_ERROR,
                    J9NLS_SHRC_CM_FAILED_ENTER_WRITE_MUTEX_STARTUP);
        ...
        if (0 == (*_runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_STATS) && (false == _ccHead->isRunningReadOnly())) {
            ...
            if ((rc == CC_STARTUP_CORRUPT) || (rc == CC_STARTUP_RESET) || (rc == CC_STARTUP_SOFT_RESET)) {
                ...
                deleteRC = _ccHead->deleteCache(currentThread,
                    (rc == CC_STARTUP_SOFT_RESET) && !(_verboseFlags & J9SHR_VERBOSEFLAG_ENABLE_VERBOSE));
                ...
                if (J9_ARE_NO_BITS_SET(*_runtimeFlags, J9SHR_RUNTIMEFLAG_RESTORE_CHECK)) {
                    ...
                    if ((deleteRC == 0) || (rc == CC_STARTUP_SOFT_RESET)) {
                        /* If we deleted the cache, or in the case of SOFT_RESET, even if we failed to delete the cache,
                         * retry */
                        doRetry = true;
                    }
                }
    ...
    enterLocalMutex(
        currentThread, currentThread->javaVM->classMemorySegments->segmentMutex, "class segment mutex", "CM startup");
    /* THREADING: We want the cache mutex here as we are reading all available data. Don't want updates happening as we
     * read. */
    if (_ccHead->enterWriteMutex(currentThread, false, fnName) == 0) {
        /* populate the hashtables */
        itemsRead = readCache(currentThread, _ccHead, -1, false);
        ...
        _ccHead->exitWriteMutex(currentThread, fnName);
        exitLocalMutex(currentThread, currentThread->javaVM->classMemorySegments->segmentMutex, "class segment mutex",
            "CM startup");
        ...
        CACHEMAP_TRACE(
            J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, J9NLS_ERROR, J9NLS_SHRC_CM_FAILED_ENTER_WRITE_MUTEX_STARTUP);
        Trc_SHR_CM_startup_Exit7(currentThread);
        return -1;
    }
#endif
...
J9MemorySegment* SH_CacheMap::addNewROMImageSegment(J9VMThread* currentThread, U_8* segmentBase, U_8* segmentEnd)
{
    ...
    if ((romSegment = createNewSegment(
             currentThread, type, vm->classMemorySegments, segmentBase, segmentBase, segmentEnd, segmentBase))
        != NULL) {
...
J9MemorySegment* SH_CacheMap::createNewSegment(J9VMThread* currentThread, UDATA type, J9MemorySegmentList* segmentList,
 ...
 */
void SH_CacheMap::updateROMSegmentList(J9VMThread* currentThread, bool hasClassSegmentMutex)
{
 ...
 */
void SH_CacheMap::updateROMSegmentListForCache(J9VMThread* currentThread, SH_CompositeCacheImpl* forCache)
{
    J9JavaVM* vm = currentThread->javaVM;
    ...
    if (currentSegment == NULL) {
        if ((currentSegment = addNewROMImageSegment(
                 currentThread, (U_8*)forCache->getBaseAddress(), (U_8*)forCache->getCacheLastEffectiveAddress()))
            == NULL) {
            Trc_SHR_CM_updateROMSegmentList_addFirstSegmentFailed(
                currentThread, forCache, forCache->getBaseAddress(), forCache->getCacheLastEffectiveAddress());
            return;
        }
    ...
    if (currentSegAlloc < cacheAlloc) {
        ...
        while (currentROMClass < cacheAlloc) {
            ...
            if ((currentSegSize + currentROMSize) > maxSegmentSize) {
                ...
                /* Failure to create a new segment is fairly fatal - j9mem_allocate_memory would have failed. Continue
                 * to use existing segment. */
                if ((newSegment = addNewROMImageSegment(
                         currentThread, currentROMClass, (U_8*)forCache->getCacheLastEffectiveAddress()))) {
                    ...
                    Trc_SHR_CM_updateROMSegmentList_addSegmentFailed(currentThread, forCache, currentROMClass,
                        forCache->getCacheLastEffectiveAddress(), currentSegment);
                }
            } else if (currentROMSize <= 0) {
                CACHEMAP_TRACE1(J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, J9NLS_ERROR,
                    J9NLS_SHRC_CM_READ_CORRUPT_ROMCLASS, currentROMClass);
...
SH_CacheMap::initializeROMSegmentList(J9VMThread* currentThread)
{
    ...
    if (!(config->metadataMemorySegment = createNewSegment(currentThread, MEMORY_TYPE_SHARED_META, vm->memorySegments,
              cacheBase, (U_8*)_cc->getMetaAllocPtr(), cacheDebugAreaStart, cacheDebugAreaStart))) {
        result = 0;
    }
...
void SH_CacheMap::updateBytesRead(UDATA numBytes)
{
    UDATA oldNum, value;
 ...
 * Returns the dataType of the successfully started manager, or 0 if the dataType is unknown or manager has been
 * shutdown, or -1 for manager startup failure */
IDATA
 ...
 */
IDATA
SH_CacheMap::readCache(
    J9VMThread* currentThread, SH_CompositeCacheImpl* cache, IDATA expectedUpdates, bool startupForStats)
{
    ShcItem* it = NULL;
    IDATA result = 0;
    ...
    do {
        it = (ShcItem*)cache->nextEntry(
            currentThread, NULL); /* IMPORTANT: Do not skip stale entries (can end up with lone orphans) */
        if (it) {
            IDATA rc;
            ...
            if ((itemType <= TYPE_UNINITIALIZED) || (itemType > MAX_DATA_TYPES)) {
                CACHEMAP_TRACE1(
                    J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, J9NLS_ERROR, J9NLS_SHRC_CM_READ_CORRUPT_DATA, it);
                ...
                if (rc == -1) {
                    ...
                    if (manager->storeNew(currentThread, it, cache)) {
                        ...
                        CACHEMAP_TRACE(
                            J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, J9NLS_ERROR, J9NLS_SHRC_CM_HASHTABLE_ADD_FAILURE);
    ...
    } while ((it != NULL) && (result != CM_READ_CACHE_FAILED) && (result != CM_CACHE_CORRUPT)
        && (expectedCntr == -1 || expectedCntr > 0));
...
SH_CacheMap::checkForCrash(J9VMThread* currentThread, bool hasClassSegmentMutex)
{
    ...
    if (_ccHead->crashDetected(&_localCrashCntr)) {
        CACHEMAP_TRACE(
            J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, J9NLS_INFO, J9NLS_SHRC_CM_UNEXPECTED_TERMINATION_DETECTED);
...
/* THREADING: Will only guarantee to return correct results with either cache read or write mutex held. No need for
 * local mutex. */
IDATA
...
void SH_CacheMap::updateAllManagersWithNewCacheArea(J9VMThread* currentThread, SH_CompositeCacheImpl* newCacheArea)
{
    SH_Manager* walkManager;
    SH_Managers::ManagerWalkState state;
...
SH_CompositeCacheImpl* SH_CacheMap::getCacheAreaForDataType(J9VMThread* currentThread, UDATA dataType, UDATA dataLength)
{
...
void SH_CacheMap::updateAverageWriteHashTime(UDATA actualTimeMicros)
{
...
ClasspathWrapper* SH_CacheMap::updateClasspathInfo(J9VMThread* currentThread, ClasspathItem* cp, I_16 cpeIndex,
    const J9UTF8* partition, const J9UTF8** cachedPartition, const J9UTF8* modContext, const J9UTF8** cachedModContext,
    bool haveWriteMutex)
{
    ...
    if (enteredWriteMutex == 0) { /* false = do not lock cache */
        ...
        if (partition || modContext) {
            ...
            for (IDATA i = 0; i < 2; i++) {
                /* We're only interested in the scope manager if it has been started. It will have been started if there
                 * are scopes in the cache */
...
const J9UTF8* SH_CacheMap::addScopeToCache(J9VMThread* currentThread, const J9UTF8* scope)
{
    ...
    cacheForAllocate = getCacheAreaForDataType(
        currentThread, TYPE_SCOPE, _ccHead->getBytesRequiredForItemWithAlign(itemPtr, SHC_WORDALIGN, 0));
    if (!cacheForAllocate) {
...
/* THREADING: MUST be called with cache write mutex held */
ClasspathWrapper* SH_CacheMap::addClasspathToCache(J9VMThread* currentThread, ClasspathItem* obj)
{
    ...
    cacheAreaForAllocate = getCacheAreaForDataType(
        currentThread, TYPE_CLASSPATH, _ccHead->getBytesRequiredForItemWithAlign(itemPtr, SHC_WORDALIGN, 0));
...
SH_CacheMap::runEntryPointChecks(J9VMThread* currentThread, void* isAddressInCache, const char** p_subcstr)
{
    ...
    hasClassSegmentMutex
        = omrthread_monitor_owned_by_self(currentThread->javaVM->classMemorySegments->segmentMutex) != 0;
    if (_ccHead->isCacheCorrupt()) {
        reportCorruptCache(currentThread);
        if (NULL != p_subcstr) {
            *p_subcstr = j9nls_lookup_message(
                (J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_CACHE_CORRUPT, "cache is corrupt");
        }
    ...
    if (isAddressInCache) {
        if (!_ccHead->isAddressInCache(isAddressInCache)) {
            if (NULL != p_subcstr) {
                *p_subcstr = j9nls_lookup_message((J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG),
                    J9NLS_SHRC_CM_ADDRESS_NOT_IN_CACHE, "address is not in cache");
            }
    ...
    if ((itemsAdded = refreshHashtables(currentThread, hasClassSegmentMutex)) == -1) {
        ...
        if (NULL != p_subcstr) {
            *p_subcstr = j9nls_lookup_message((J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG),
                J9NLS_SHRC_CM_CACHE_REFRESH_FAILED, "cache refresh failed");
        }
 ...
 */
bool SH_CacheMap::allocateROMClass(J9VMThread* currentThread, const J9RomClassRequirements* sizes,
    J9SharedRomClassPieces* pieces, U_16 classnameLength, const char* classnameData, ClasspathWrapper* cpw,
    const J9UTF8* partitionInCache, const J9UTF8* modContextInCache, IDATA callerHelperID, bool modifiedNoContext,
    void*& newItemInCache, void*& cacheAreaForAllocate)
{
    ...
    Trc_SHR_CM_allocateROMClass_Entry3(currentThread, classnameLength, classnameData, sizes->romClassSizeFullSize,
        sizes->romClassMinimalSize, sizes->lineNumberTableSize, sizes->localVariableTableSize);
    ...
    if ((sizes->lineNumberTableSize > 0) || (sizes->localVariableTableSize > 0)) {
        ...
        if (rc == -1) {
            Trc_SHR_CM_allocateROMClass_FailedToAllocDebugAttributes_Event(currentThread, classnameLength,
                classnameData, sizes->lineNumberTableSize, pieces->lineNumberTable, sizes->localVariableTableSize,
                pieces->localVariableTable);
            allocatedDebugMem = false;
        } else {
            Trc_SHR_CM_allocateROMClass_AllocatedDebugAttributes_Event(currentThread, classnameLength, classnameData,
                sizes->lineNumberTableSize, pieces->lineNumberTable, sizes->localVariableTableSize,
                pieces->localVariableTable);
    ...
    pieces->romClass = (void*)allocateROMClassOnly(currentThread, romclassSizeToUse, classnameLength, classnameData,
        cpw, partitionInCache, modContextInCache, callerHelperID, modifiedNoContext, newItemInCache,
    ...
    if ((true == allocatedDebugMem) && (NULL == pieces->romClass)) {
        Trc_SHR_CM_allocateROMClass_FailedToRomClassRollbackDebug_Event(currentThread, classnameLength, classnameData,
            sizes->lineNumberTableSize, pieces->lineNumberTable, sizes->localVariableTableSize,
            pieces->localVariableTable);
    ...
    if (pieces->romClass == NULL) {
        retval = false;
        Trc_SHR_CM_allocateROMClass_Exit3(currentThread, 0, classnameLength, classnameData, pieces->romClass,
            pieces->lineNumberTable, pieces->localVariableTable, pieces->flags);
    } else {
        retval = true;
        Trc_SHR_CM_allocateROMClass_Exit3(currentThread, 1, classnameLength, classnameData, pieces->romClass,
            pieces->lineNumberTable, pieces->localVariableTable, pieces->flags);
    }
    return retval;
}
 ...
 * THREADING: We assume the Segment Mutex, String Table Lock, and Write Area Lock is held by the transaction.
 */
const J9ROMClass* SH_CacheMap::allocateROMClassOnly(J9VMThread* currentThread, U_32 sizeToAlloc, U_16 classnameLength,
    const char* classnameData, ClasspathWrapper* cpw, const J9UTF8* partitionInCache, const J9UTF8* modContextInCache,
    IDATA callerHelperID, bool modifiedNoContext, void*& newItemInCache, void*& cacheAreaForAllocate)
{
    const J9ROMClass* result = NULL;
    bool fullFlagSet = J9_ARE_ANY_BITS_SET(
        *_runtimeFlags, J9SHR_RUNTIMEFLAG_BLOCK_SPACE_FULL | J9SHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL);
    ...
    Trc_SHR_Assert_True(_ccHead->hasWriteMutex(currentThread));
    Trc_SHR_Assert_ShouldHaveLocalMutex(currentThread->javaVM->classMemorySegments->segmentMutex);
    Trc_SHR_CM_allocateROMClassOnly_Entry((UDATA)currentThread, (UDATA)sizeToAlloc, (UDATA)classnameLength,
        classnameData, (UDATA)cpw, (UDATA)partitionInCache, (UDATA)modContextInCache, callerHelperID,
        (UDATA)modifiedNoContext);
    ...
    if ((cpw == NULL) || (modifiedNoContext == true)) {
        ...
        result = allocateFromCache(
            currentThread, sizeToAlloc, sizeof(OrphanWrapper), TYPE_ORPHAN, newItemInCache, cacheAreaForAllocate);
    } else {
        SH_ClasspathManager* localCPM;
        if (!(localCPM = getClasspathManager(currentThread))) {
        ...
        /* If we are storing using a Token and there is already a class of the same name stored under that token, mark
         * the original one stale*/
        this->tokenStoreStaleCheckAndMark(
            currentThread, classnameLength, classnameData, cpw, partitionInCache, modContextInCache, callerHelperID);
        ...
        if (!useScope) {
            ...
            Trc_SHR_CM_allocateROMClassOnly_WillScpopedROMClass_Event(
                currentThread, (UDATA)classnameLength, classnameData);
            wrapperSize = sizeof(ScopedROMClassWrapper);
            wrapperType = TYPE_SCOPED_ROMCLASS;
        }

        result = allocateFromCache(
            currentThread, sizeToAlloc, wrapperSize, wrapperType, newItemInCache, cacheAreaForAllocate);
    }
 ...
 */
void SH_CacheMap::tokenStoreStaleCheckAndMark(J9VMThread* currentThread, U_16 classnameLength,
    const char* classnameData, ClasspathWrapper* cpw, const J9UTF8* partitionInCache, const J9UTF8* modContextInCache,
    UDATA callerHelperID)
{
    /* If we are storing using a Token and there is already a class of the same name stored under that token, mark the
     * original one stale */
    if (((ClasspathItem*)CPWDATA(cpw))->getType() == CP_TYPE_TOKEN) {
        LocateROMClassResult existingTokenResult;
        UDATA tokenRC = 0;

        tokenRC = _rcm->locateROMClass(currentThread, classnameData, classnameLength, ((ClasspathItem*)CPWDATA(cpw)), 0,
            -1, callerHelperID, NULL, partitionInCache, modContextInCache, &existingTokenResult);
 ...
 * THREADING: We assume the Segment Mutex, String Table Lock, and Write Area Lock is held by the transaction.
 */
const J9ROMClass* SH_CacheMap::allocateFromCache(J9VMThread* currentThread, U_32 sizeToAlloc, U_32 wrapperSize,
    U_16 wrapperType, void*& newItemInCache, void*& cacheAreaForAllocate)
{
    ShcItem item;
    ShcItem* itemPtr = &item;
    ShcItem* itemInCache = NULL;
    ...
    Trc_SHR_Assert_True(_ccHead->hasWriteMutex(currentThread));
    Trc_SHR_Assert_ShouldHaveLocalMutex(currentThread->javaVM->classMemorySegments->segmentMutex);
    Trc_SHR_CM_allocateFromCache_Entry(
        (UDATA)currentThread, (UDATA)sizeToAlloc, (UDATA)wrapperSize, (UDATA)wrapperType);

    if (*_runtimeFlags & RUNTIME_FLAGS_PREVENT_BLOCK_DATA_UPDATE) {
        /* Don't update the cache. We should never reach here, if RUNTIME_FLAGS_PREVENT_BLOCK_DATA_UPDATE is set, we
         * should return directly from j9shr_classStoreTransaction_createSharedClass().*/
        Trc_SHR_Assert_ShouldNeverHappen();
        Trc_SHR_CM_allocateFromCache_Flags_Event(currentThread);
        goto done;
    }

    localCacheAreaForAllocate = getCacheAreaForDataType(
        currentThread, wrapperType, (sizeToAllocPadded + sizeof(ShcItem) + sizeof(ShcItemHdr) + wrapperSize + 16));
    if (!localCacheAreaForAllocate) {
        /* This may indicate size required is bigger than the cachelet size. */
        /* TODO: In offline mode, should be fatal. Report unable to grow. */
        Trc_SHR_CM_allocateFromCache_getCacheArea_Failed_Event(
            currentThread, (UDATA)sizeToAlloc, (UDATA)wrapperSize, (UDATA)wrapperType);
        goto done;
    }

    _ccHead->initBlockData(&itemPtr, wrapperSize, wrapperType);
    itemInCache = (ShcItem*)localCacheAreaForAllocate->allocateWithSegment(
        currentThread, itemPtr, sizeToAllocPadded, &romClassBuffer);
 ...
 */
IDATA
SH_CacheMap::allocateClassDebugData(J9VMThread* currentThread, U_16 classnameLength, const char* classnameData,
    const J9RomClassRequirements* sizes, J9SharedRomClassPieces* pieces)
{
 ...
 * THREADING: We assume the Segment Mutex, String Table Lock, and Write Area Lock is held by the transaction.
 */
void SH_CacheMap::rollbackClassDebugData(J9VMThread* currentThread, U_16 classnameLength, const char* classnameData)
{
 ...
 * THREADING: We assume the Segment Mutex, String Table Lock, and Write Area Lock is held by the transaction.
 */
void SH_CacheMap::commitClassDebugData(J9VMThread* currentThread, U_16 classnameLength, const char* classnameData)
{
    this->_ccHead->commitClassDebugData(currentThread, classnameLength, classnameData);
}

void SH_CacheMap::updateLineNumberContentInfo(J9VMThread* currentThread)
{
    J9JavaVM* vm = currentThread->javaVM;

    if (false == this->_cc->getIsNoLineNumberContentEnabled()) {
        if (0
            == (vm->requiredDebugAttributes
 ...
 */
IDATA
SH_CacheMap::commitROMClass(J9VMThread* currentThread, ShcItem* itemInCache,
    SH_CompositeCacheImpl* cacheAreaForAllocate, ClasspathWrapper* cpw, I_16 cpeIndex, const J9UTF8* partitionInCache,
    const J9UTF8* modContextInCache, BlockPtr romClassBuffer, bool commitOutOfLineData)
{
    IDATA retval = 0;
    ...
    Trc_SHR_Assert_True(_ccHead->hasWriteMutex(currentThread));
    Trc_SHR_Assert_ShouldHaveLocalMutex(currentThread->javaVM->classMemorySegments->segmentMutex);
    Trc_SHR_CM_commitROMClass_Entry((UDATA)currentThread, (UDATA)itemInCache, (UDATA)cacheAreaForAllocate, (UDATA)cpw,
        (UDATA)cpeIndex, (UDATA)partitionInCache, (UDATA)modContextInCache, (UDATA)J9UTF8_LENGTH(romClassName),
    ...
    if (cpeiInCache->protocol == PROTO_DIR) {
        srcw.timestamp = _tsm->checkROMClassTimeStamp(currentThread, (const char*)J9UTF8_DATA(romClassName),
            (UDATA)J9UTF8_LENGTH(romClassName), cpeiInCache, ((ROMClassWrapper*)&srcw));
    }
    ...
    if ((true == useWriteHash) && (*_runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_REDUCE_STORE_CONTENTION)) {
        if (_writeHashMaxWaitMicros == 0 && _writeHashContendedResetHash != 0) {
            classNameHash = currentThread->javaVM->internalVMFunctions->computeHashForUTF8(
            ...
            if (classNameHash == _writeHashContendedResetHash) {
                ...
                Trc_SHR_CM_commitROMClass_Event_SetMaxTime(
                    currentThread, "store", _writeHashMaxWaitMicros, _writeHashAverageTimeMicros, 0);
            }
        }
    }
    ...
    /* If storeNew() fails to store a class in the RCM we return 0, to be consistent with old code when running
     * verboseio.
     */
    if (storeResult == false) {
        Trc_SHR_CM_commitROMClass_StoreFail_Event(currentThread, (UDATA)J9UTF8_LENGTH(romClassName),
            (const char*)J9UTF8_DATA(romClassName), (UDATA)itemInCache, (UDATA)cacheAreaForAllocate);
        retval = 0;
    } else {
        retval = 1;
    ...
    if (retval == 0) {
        Trc_SHR_CM_commitROMClass_NothingStored_Event(
            currentThread, (UDATA)J9UTF8_LENGTH(romClassName), J9UTF8_DATA(romClassName), (UDATA)romClassBuffer);
    } else if (retval == 1) {
        Trc_SHR_CM_commitROMClass_Stored_Event(
            currentThread, (UDATA)J9UTF8_LENGTH(romClassName), J9UTF8_DATA(romClassName), (UDATA)romClassBuffer);
    } else {
        Trc_SHR_CM_commitROMClass_Error_Event(
            currentThread, (UDATA)J9UTF8_LENGTH(romClassName), J9UTF8_DATA(romClassName), (UDATA)romClassBuffer);
        Trc_SHR_Assert_ShouldNeverHappen();
    }
 ...
 */
IDATA
SH_CacheMap::commitOrphanROMClass(J9VMThread* currentThread, ShcItem* itemInCache,
    SH_CompositeCacheImpl* cacheAreaForAllocate, ClasspathWrapper* cpw, BlockPtr romClassBuffer)
{
    IDATA retval = 0;
    bool storeResult = false;
    J9UTF8* romClassName = J9ROMCLASS_CLASSNAME(((J9ROMClass*)romClassBuffer));
    ...
    Trc_SHR_Assert_True(_ccHead->hasWriteMutex(currentThread));
    Trc_SHR_Assert_ShouldHaveLocalMutex(currentThread->javaVM->classMemorySegments->segmentMutex);
    Trc_SHR_CM_commitOrphanROMClass_Entry((UDATA)currentThread, (UDATA)itemInCache, (UDATA)cacheAreaForAllocate,
        (UDATA)cpw, (UDATA)J9UTF8_LENGTH(romClassName), J9UTF8_DATA(romClassName));
    ...
    if ((true == useWriteHash) && (*_runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_REDUCE_STORE_CONTENTION)) {
        if (_writeHashMaxWaitMicros == 0 && _writeHashContendedResetHash != 0) {
            ...
            if (classNameHash == _writeHashContendedResetHash) {
                 ...
                 */
                _writeHashMaxWaitMicros = _writeHashSavedMaxWaitMicros;
                Trc_SHR_CM_commitOrphanROMClass_Event_SetMaxTime(
                    currentThread, "store", _writeHashMaxWaitMicros, _writeHashAverageTimeMicros, 0);
            }
        }
    }
     ...
     * verboseio.*/
    if (storeResult == false) {
        Trc_SHR_CM_commitOrphanROMClass_StoreFail_Event(currentThread, (UDATA)J9UTF8_LENGTH(romClassName),
            (const char*)J9UTF8_DATA(romClassName), (UDATA)itemInCache, (UDATA)cacheAreaForAllocate);
        retval = 0;
    } else {
        retval = 1;
    ...
    if (retval == 0) {
        Trc_SHR_CM_commitOrphanROMClass_NothingStored_Event(
            currentThread, (UDATA)J9UTF8_LENGTH(romClassName), J9UTF8_DATA(romClassName), (UDATA)romClassBuffer);
    } else if (retval == 1) {
        Trc_SHR_CM_commitOrphanROMClass_Stored_Event(
            currentThread, (UDATA)J9UTF8_LENGTH(romClassName), J9UTF8_DATA(romClassName), (UDATA)romClassBuffer);
    } else {
        Trc_SHR_CM_commitOrphanROMClass_Error_Event(
            currentThread, (UDATA)J9UTF8_LENGTH(romClassName), J9UTF8_DATA(romClassName), (UDATA)romClassBuffer);
        Trc_SHR_Assert_ShouldNeverHappen();
    }
 ...
 */
IDATA
SH_CacheMap::commitMetaDataROMClassIfRequired(J9VMThread* currentThread, ClasspathWrapper* cpw, I_16 cpeIndex,
    IDATA helperID, const J9UTF8* partitionInCache, const J9UTF8* modContextInCache, J9ROMClass* romclass)
{
    IDATA retval = 0;
    ...
    Trc_SHR_Assert_True(_ccHead->hasWriteMutex(currentThread));
    Trc_SHR_Assert_ShouldHaveLocalMutex(currentThread->javaVM->classMemorySegments->segmentMutex);
    Trc_SHR_CM_commitMetaDataROMClassIfRequired_Entry((UDATA)currentThread, (UDATA)cpw, (UDATA)cpeIndex, helperID,
        (UDATA)partitionInCache, (UDATA)modContextInCache, (UDATA)J9UTF8_LENGTH(romClassName),
        J9UTF8_DATA(romClassName));
    ...
    /* Although we may have the ROMClass we want to use, we may already also have the metadata we want (another JVM has
     * added the class), so check for this */
    rc = _rcm->locateROMClass(currentThread, (const char*)J9UTF8_DATA(romClassName), J9UTF8_LENGTH(romClassName),
        ((ClasspathItem*)CPWDATA(cpw)), cpeIndex, -1, helperID, (J9ROMClass*)romClassBuffer, partitionInCache,
        modContextInCache, &locateResult);

    /* If we are storing using a Token and there is already a class of the same name stored under that token, mark the
     * original one stale Note also that the locateROMClass call above will not necessarily find what we want. If
     * another class is being added under the same token, and the orphan has already been added, the above call will
     * return the exact orphan match which is useful, but not what we need to do the stale mark
     */
    this->tokenStoreStaleCheckAndMark(currentThread, J9UTF8_LENGTH(romClassName),
        (const char*)J9UTF8_DATA(romClassName), cpw, partitionInCache, modContextInCache, helperID);
    ...
    if (rc & LOCATE_ROMCLASS_RETURN_FOUND) {
        ...
        Trc_SHR_CM_commitMetaDataROMClassIfRequired_FoundDuplicate(
            currentThread, J9UTF8_LENGTH(romClassName), J9UTF8_DATA(romClassName));
        ...
        if ((true == useWriteHash) && (*_runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_REDUCE_STORE_CONTENTION)) {
            ...
            if (_writeHashMaxWaitMicros == 0) {
                ...
                if (classNameHash == _writeHashContendedResetHash) {
                    ...
                    UDATA twice;
                    if ((_writeHashAverageTimeMicros > 0)
                        && (_writeHashMaxWaitMicros > (twice = _writeHashAverageTimeMicros * 2))) {
                        /* If the maximum time is more than twice the average, reduce it */
                        _writeHashMaxWaitMicros = twice;
                    }
                }
                Trc_SHR_CM_commitMetaDataROMClassIfRequired_Event_SetMaxTime(
                    currentThread, "found", _writeHashMaxWaitMicros, _writeHashAverageTimeMicros, actualTimeMicros);
            }
        }
    } else {
        /* Classpath could potentially have become stale during locateROMClass above. This is highly unlikely, but
         * possible. We cannot add against a stale classpath, and we have missed our chance to add a new classpath, so
         * in this case, totally fail the add. */
        SH_ClasspathManager* localCPM;
        if (!(localCPM = getClasspathManager(currentThread))) {
            Trc_SHR_CM_commitMetaDataROMClassIfRequired_CPMan_Event(
                currentThread, (UDATA)J9UTF8_LENGTH(romClassName), J9UTF8_DATA(romClassName));
            retval = -1;
            goto done;
        }
        if (localCPM->isStale(cpw)) {
    ...
    if (locateJ9ROMClass != NULL) {
        ...
        Trc_SHR_CM_commitMetaDataROMClassIfRequired_Existing_Event(
            currentThread, (UDATA)J9UTF8_LENGTH(romClassName), J9UTF8_DATA(romClassName), (UDATA)romclass);
        goto done;
    }
    ...
    if (cacheAreaForAllocate == NULL) {
         ...
         */
        Trc_SHR_CM_commitMetaDataROMClassIfRequired_Area_Event(
            currentThread, (UDATA)wrapperType, (UDATA)bytesToReserve);
        retval = 0;
        goto done;
    }
    ...
    if (itemInCache == NULL) {
        /* Not enough space in cache to accomodate this item. */
        Trc_SHR_CM_commitMetaDataROMClassIfRequired_Full_Event(
            currentThread, (UDATA)J9UTF8_LENGTH(romClassName), J9UTF8_DATA(romClassName), (UDATA)romclass);
        retval = -1;
        goto done;
    }
    ...
    retval = commitROMClass(currentThread, itemInCache, cacheAreaForAllocate, cpw, cpeIndex, partitionInCache,
        modContextInCache, romClassBuffer, false);
...
done_skipHashUpdate:
    if (retval == 0) {
        Trc_SHR_CM_commitMetaDataROMClassIfRequired_NothingStored_Event(
            currentThread, (UDATA)J9UTF8_LENGTH(romClassName), J9UTF8_DATA(romClassName), (UDATA)romclass);
    } else if (retval == 1) {
        Trc_SHR_CM_commitMetaDataROMClassIfRequired_Stored_Event(
            currentThread, (UDATA)J9UTF8_LENGTH(romClassName), J9UTF8_DATA(romClassName), (UDATA)romclass);
    } else {
        Trc_SHR_CM_commitMetaDataROMClassIfRequired_Error_Event(
            currentThread, (UDATA)J9UTF8_LENGTH(romClassName), J9UTF8_DATA(romClassName), (UDATA)romclass);
    }
 ...
 */
const J9ROMClass* SH_CacheMap::findNextROMClass(J9VMThread* currentThread, void*& findNextIterator, void*& firstFound,
    U_16 classnameLength, const char* classnameData)
{
    ...
    retval = (J9ROMClass*)_rcm->findNextExisting(
        currentThread, findNextIterator, firstFound, classnameLength, classnameData);
 ...
 */
UDATA
SH_CacheMap::getStringTableBytes(void) { return this->_ccHead->getStringTableBytes(); }
 ...
 * Caller should hold string table write lock
 */
void* SH_CacheMap::getStringTableBase(void) { return this->_ccHead->getStringTableBase(); }
 ...
 * THREADING: This function can be called multi-threaded. Caller must have VM class segment mutex.
 */
const J9ROMClass* SH_CacheMap::findROMClass(J9VMThread* currentThread, const char* path, ClasspathItem* cp,
    const J9UTF8* partition, const J9UTF8* modContext, IDATA confirmedEntries, IDATA* foundAtIndex)
{
    UDATA rc = 0;
    ...
    if (_ccHead->isRunningReadOnly()) {
        /* We've already identified that an element of this classpath is stale. Sharing is therefore disabled for this
         * classpath. */
    ...
    rc = localRCM->locateROMClass(currentThread, path, pathLen, cp, -1, confirmedEntries, cp->getHelperID(), NULL,
        partition, modContext, &locateResult);
    ...
    /* If ROMClass was not found (non-error case), look to see if another JVM is trying to load the same class.
            ...
            overhead of going to disk for the class, trying to store it and then throwing it away because it is already
       stored.
    */
    if ((*_runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_REDUCE_STORE_CONTENTION)
        && (rc & LOCATE_ROMCLASS_RETURN_DO_TRY_WAIT)) {
            ...
            if (result == 1) { /* Another JVM in the middle of loading class with same hash - wait for update */
                ...
                do {
                    ...
                    if ((updates = _ccHead->checkUpdates(currentThread))) {
                        ...
                        IDATA rv = refreshHashtables(currentThread,
                            true); /* We do have the class segment mutex - this is a prereq of findROMClass */
                        if (-1 == rv) {
                            _ccHead->exitReadMutex(currentThread, fnName);
                            break;
                        }
                        rc = localRCM->locateROMClass(currentThread, path, pathLen, cp, -1, confirmedEntries,
                            cp->getHelperID(), NULL, partition, modContext, &locateResult);
                         ...
                         */
                        if ((rc & LOCATE_ROMCLASS_RETURN_MARKED_ITEM_STALE)
                            != LOCATE_ROMCLASS_RETURN_MARKED_ITEM_STALE) {
                            _ccHead->exitReadMutex(currentThread, fnName);
                        }
                        endTime = j9time_usec_clock();
                        actualTimeMicros = (UDATA)(endTime - startTime);
                ...
                Trc_SHR_CM_findROMClass_Event_AfterWriteHash_WithStats(currentThread, locateResult.known,
                    locateResult.foundAtIndex, rc, _writeHashMaxWaitMicros, _writeHashAverageTimeMicros,
                    actualTimeMicros);
            }
        } else {
    ...
    if (returnVal) {
        updateBytesRead(
            returnVal->romSize); /* This is kind of inaccurate as the strings are all external to the ROMClass */
        /* trace event is at level 1 and trace exit message is at level 2 as per CMVC 155318/157683 */
        Trc_SHR_CM_findROMClass_Exit_Found_Event(
            currentThread, path, returnVal, locateResult.foundAtIndex, cp->getHelperID());
...
/* THREADING: MUST be protected by cache write mutex - therefore single-threaded within this JVM */
const void* SH_CacheMap::addROMClassResourceToCache(J9VMThread* currentThread, const void* romAddress,
    SH_ROMClassResourceManager* localRRM, SH_ROMClassResourceManager::SH_ResourceDescriptor* resourceDescriptor,
    const char** p_subcstr)
{
    ...
    cacheAreaForAllocate = getCacheAreaForDataType(
        currentThread, resourceType, _ccHead->getBytesRequiredForItemWithAlign(itemPtr, align, wrapperLength));
    if (!cacheAreaForAllocate) {
        /* This may indicate size required is bigger than the cachelet size. */
        /* TODO: In offline mode, should be fatal */
        if (NULL != p_subcstr) {
            const char* tmpstr = j9nls_lookup_message((J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG),
                J9NLS_SHRC_CM_CANNOT_ALLOC_DATA_SIZE, "no space in cache for %d bytes");
    ...
    if ((romAddress < _cc->getBaseAddress()) || (romAddress > _cc->getCacheLastEffectiveAddress())) {
        /* The address we've been given is in a different supercache - we can't use this for indexing, so we have to
         * fail */
        /* TODO: Tracepoint */
        if (NULL != p_subcstr) {
            *p_subcstr = j9nls_lookup_message((J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG),
...
const void* SH_CacheMap::storeROMClassResource(J9VMThread* currentThread, const void* romAddress,
    SH_ROMClassResourceManager* localRRM, SH_ROMClassResourceManager::SH_ResourceDescriptor* resourceDescriptor,
    UDATA forceReplace, const char** p_subcstr)
{
    ...
    if (!localRRM->permitAccessToResource(currentThread)) {
        if (p_subcstr) {
            *p_subcstr = j9nls_lookup_message((J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG),
                J9NLS_SHRC_CM_NO_ACCESS_TO_RESOURCE, "no access to resource");
        }
    ...
    if (_ccHead->enterWriteMutex(currentThread, false, fnName) != 0) {
        if (p_subcstr) {
            *p_subcstr = j9nls_lookup_message((J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG),
                J9NLS_SHRC_CM_ENTER_WRITE_MUTEX, "enterWriteMutex failed");
        }
    ...
    if ((resourceWrapper = (localRRM->findResource(currentThread, resourceKey))) != 0) {
        if (!forceReplace) {
            ...
            if (p_subcstr) {
                *p_subcstr = j9nls_lookup_message(
                    (J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_DATA_EXISTS, "data already exists");
            }
    ...
    if ((resourceWrapper == (void*)J9SHR_RESOURCE_STORE_FULL)
        || (resourceWrapper == (void*)J9SHR_RESOURCE_STORE_ERROR)) {
     ...
     * being created by the current VM.
     */
    if ((NULL != result) && ((void*)J9SHR_RESOURCE_STORE_FULL != result)
        && ((void*)J9SHR_RESOURCE_STORE_ERROR != result) && _ccHead->isNewCache()) {
#if !defined(J9ZOS390) && !defined(AIXPPC)
#if defined(LINUX)
        if (J9_ARE_ALL_BITS_SET(*_runtimeFlags, J9SHR_RUNTIMEFLAG_ENABLE_PERSISTENT_CACHE))
#endif
...
/* THREADING: This function can be called multi-threaded */
UDATA
SH_CacheMap::updateROMClassResource(J9VMThread* currentThread, const void* addressInCache, I_32 updateAtOffset,
    SH_ROMClassResourceManager* localRRM, SH_ROMClassResourceManager::SH_ResourceDescriptor* resourceDescriptor,
    const J9SharedDataDescriptor* data, bool isUDATA, const char** p_subcstr)
{
    ...
    Trc_SHR_CM_updateROMClassResource_Entry1(currentThread, addressInCache, updateAtOffset, localRRM,
        resourceDescriptor, data, (I_32)isUDATA, UnitTest::unitTest);
    ...
    do {
        ...
        if ((resourceWrapper = (localRRM->findResource(currentThread, resourceKey))) != 0) {
            ...
            if ((UDATA)updateAtOffset + data->length > dataLength) {
                if (NULL != p_subcstr) {
                    const char* tmpcstr = j9nls_lookup_message((J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG),
                        J9NLS_SHRC_CM_DATA_SIZE_LARGER, "data %d larger than available %d");
                    j9str_printf(PORTLIB, (char*)*p_subcstr, VERBOSE_BUFFER_SIZE, tmpcstr,
                        updateAtOffset + (data->length), dataLength);
                }
            ...
            if (NULL != p_subcstr) {
                *p_subcstr = j9nls_lookup_message(
                    (J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_NO_DATA_EXISTS, "no data exists");
            }
 ...
 * THREADING: This function can be called multi-threaded.
 */
const void* SH_CacheMap::findROMClassResource(J9VMThread* currentThread, const void* romAddress,
    SH_ROMClassResourceManager* localRRM, SH_ROMClassResourceManager::SH_ResourceDescriptor* resourceDescriptor,
    bool useReadMutex, const char** p_subcstr, UDATA* flags)
{
    const void* result = NULL;
    ...
    if ((true == useReadMutex) && (_ccHead->enterReadMutex(currentThread, fnName) != 0)) {
        if (NULL != p_subcstr) {
            *p_subcstr = j9nls_lookup_message(
                (J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_ENTER_READ_MUTEX, "enterReadMutex failed");
        }
 ...
 * THREADING: This function can be called multi-threaded
 */
const U_8* SH_CacheMap::storeCompiledMethod(J9VMThread* currentThread, const J9ROMMethod* romMethod,
    const U_8* dataStart, UDATA dataSize, const U_8* codeStart, UDATA codeSize, UDATA forceReplace)
{
    const U_8* result;
    SH_CompiledMethodManager::SH_CompiledMethodResourceDescriptor descriptor(
        dataStart, (U_32)dataSize, codeStart, (U_32)codeSize);
 ...
 * THREADING: This function can be called multi-threaded
 */
const U_8* SH_CacheMap::findCompiledMethod(J9VMThread* currentThread, const J9ROMMethod* romMethod, UDATA* flags)
{
    const U_8* result;
    SH_CompiledMethodManager::SH_CompiledMethodResourceDescriptor descriptor;
    SH_CompiledMethodManager* localCMM;
        ...
        if (_metadataReleased
            ...
            if (TrcEnabled_Trc_SHR_CM_findCompiledMethod_metadataAccess
                ...
                if (NULL != romClass) {
                    ...
                    Trc_SHR_CM_findCompiledMethod_metadataAccess(currentThread, J9UTF8_LENGTH(romClassName),
                        J9UTF8_DATA(romClassName), J9UTF8_LENGTH(romMethodName), J9UTF8_DATA(romMethodName),
                        J9UTF8_LENGTH(romMethodSig), J9UTF8_DATA(romMethodSig), result);
                }
            }
        } else
#if defined(LINUX)
 ...
 */
void SH_CacheMap::updateAccessedShrCacheMetadataBounds(J9VMThread* currentThread, uintptr_t const* metadataAddress)
{
    ...
    if (0 == currentValue) { /* set initial value.  Don't care if someone beats us to this  */
        ...
        compareAndSwapUDATA(updateAddress, currentValue, newValue);
    }
 ...
 **/
UDATA
SH_CacheMap::storeAttachedData(
    J9VMThread* currentThread, const void* addressInCache, const J9SharedDataDescriptor* data, UDATA forceReplace)
{
    UDATA result = 0;
    SH_AttachedDataManager::SH_AttachedDataResourceDescriptor descriptor(
        data->address, (U_32)data->length, (U_16)data->type);
    ...
    if (localVerboseFlags & J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_JITDATA) {
        ...
        subcstr[0] = 0;
        result = (UDATA)storeROMClassResource(
            currentThread, addressInCache, localADM, &descriptor, forceReplace, &pSubcstr);
        ...
        if (addressInCache && _ccHead->isAddressInCache(addressInCache)) {
            ...
            if (0 == result) {
                if (J9SHR_ATTACHED_DATA_TYPE_JITHINT == data->type) {
                    ...
                    CACHEMAP_PRINT8(localVerboseFlags, J9NLS_SHRC_CM_STORED_VERBOSE_JIT_ATTACHED_DATA, pType,
                        formatAttachedDataString(
                            currentThread, data->address, data->length, attachedDataString, sizeof(attachedDataString)),
                        J9UTF8_LENGTH(methodName), J9UTF8_DATA(methodName), J9UTF8_LENGTH(methodSig),
                        J9UTF8_DATA(methodSig), J9UTF8_LENGTH(className), J9UTF8_DATA(className));
                } else {
            ...
            CACHEMAP_PRINT3(localVerboseFlags, J9NLS_SHRC_CM_STORE_FAILED_VERBOSE_ATTACHED_NOCLASSINFO_MSG, pType,
                addressInCache, pSubcstr);
        }
    } else {
 ...
 */
UDATA
SH_CacheMap::updateAttachedData(
    J9VMThread* currentThread, const void* addressInCache, I_32 updateAtOffset, const J9SharedDataDescriptor* data)
{
    ...
    if (localVerboseFlags & J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_JITDATA) {
        ...
        subcstr[0] = 0;
        result = updateROMClassResource(
            currentThread, addressInCache, updateAtOffset, localADM, &descriptor, data, false, &pSubcstr);
        ...
        if (addressInCache && _ccHead->isAddressInCache(addressInCache)) {
            ...
            CACHEMAP_PRINT3(localVerboseFlags, J9NLS_SHRC_CM_UPDATE_FAILED_VERBOSE_ATTACHED_NOCLASSINFO_MSG, pType,
                addressInCache, pSubcstr);
        }
    } else {
        result = updateROMClassResource(
            currentThread, addressInCache, updateAtOffset, localADM, &descriptor, data, false, NULL);
    }
 ...
 */
UDATA
SH_CacheMap::updateAttachedUDATA(
    J9VMThread* currentThread, const void* addressInCache, UDATA type, I_32 updateAtOffset, UDATA value)
{
    ...
    if (localVerboseFlags & J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_JITDATA) {
        ...
        subcstr[0] = 0;
        result = updateROMClassResource(
            currentThread, addressInCache, updateAtOffset, localADM, &descriptor, &data, true, &pSubcstr);
        ...
        result = updateROMClassResource(
            currentThread, addressInCache, updateAtOffset, localADM, &descriptor, &data, true, NULL);
    }
 ...
 * 				if data->address is NULL then data->length is ignored. On success, data->address will point to
 * newly allocated memory filled with the data, if data->address is non NULL then data->length is the size of the buffer
 * pointed by data->address. On success, the buffer is filled with the data.
 * @param[out] corruptOffset If not -1, the updateAtOffset supplied to j9shr_updateAttachedData() when the incomplete
 * write occurred.
 *
 * @return A pointer to the start of the data if successful
 * @return NULL if non-stale data cannot be found, or the data is corrupt. In case data is corrupt, data->address should
 * be freed by the caller if it was passed as NULL.
 ...
 *
 */
const U_8* SH_CacheMap::findAttachedDataAPI(
    ...
    if (localVerboseFlags & J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_JITDATA) {
        ...
        if (addressInCache && _ccHead->isAddressInCache(addressInCache)) {
            ...
            if ((NULL == result) || (J9SHR_RESOURCE_MAX_ERROR_VALUE >= (UDATA)result)) {
                CACHEMAP_PRINT8(localVerboseFlags, J9NLS_SHRC_CM_FIND_FAILED_VERBOSE_ATTACHED_MSG1, pType, pSubcstr,
                    J9UTF8_LENGTH(methodName), J9UTF8_DATA(methodName), J9UTF8_LENGTH(methodSig),
                    J9UTF8_DATA(methodSig), J9UTF8_LENGTH(className), J9UTF8_DATA(className));
            } else {
                if (J9SHR_ATTACHED_DATA_TYPE_JITHINT == data->type) {
                    ...
                    CACHEMAP_PRINT7(localVerboseFlags, J9NLS_SHRC_CM_FOUND_VERBOSE_ATTACHED_MSG, pType,
                        J9UTF8_LENGTH(methodName), J9UTF8_DATA(methodName), J9UTF8_LENGTH(methodSig),
                        J9UTF8_DATA(methodSig), J9UTF8_LENGTH(className), J9UTF8_DATA(className));
                }
            }
        } else {
            CACHEMAP_PRINT3(localVerboseFlags, J9NLS_SHRC_CM_FIND_FAILED_VERBOSE_ATTACHED_NOCLASSINFO_MSG, pType,
                addressInCache, pSubcstr);
        }
    } else {
...
const U_8* SH_CacheMap::findAttachedData(J9VMThread* currentThread, const void* addressInCache,
    J9SharedDataDescriptor* data, IDATA* corruptOffset, const char** p_subcstr)
{
    const U_8* result;
    ...
    *corruptOffset = -1;
    ...
    result = (const U_8*)findROMClassResource(
        currentThread, addressInCache, localADM, &descriptor, false, p_subcstr, NULL);
    if (NULL != result) {
        I_32 corrupt;
        ...
        if (NULL != data->address) {
            ...
            if (NULL == data->address) {
                ...
                if (NULL != p_subcstr) {
                    const char* tmpcstr = j9nls_lookup_message((J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG),
                        J9NLS_SHRC_CM_MEMORY_ALLOC_FAILED, "memory allocation of %d bytes failed");
            ...
            do {
                ...
                if (initialUpdateCount != finalUpdateCount) {
                    if ((++updateCountTryCount) > FIND_ATTACHED_DATA_RETRY_COUNT) {
                        ...
                        if (NULL != p_subcstr) {
                            *p_subcstr = j9nls_lookup_message((J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG),
                                J9NLS_SHRC_CM_TOO_MANY_UPDATES, "too many updates while reading");
                        }
                        goto _exitWithError;
                    } else {
                        continue;
        ...
        if (NULL != p_subcstr) {
            *p_subcstr = j9nls_lookup_message(
                (J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_CACHE_FIND_NOTFOUND, "no data in cache");
        }
    }
 ...
 * THREADING: MUST be protected by cache write mutex - therefore single-threaded within this JVM
 */
SH_CacheMap::BlockPtr SH_CacheMap::addByteDataToCache(J9VMThread* currentThread, SH_Manager* localBDM,
    const J9UTF8* tokenKeyInCache, const J9SharedDataDescriptor* data, SH_CompositeCacheImpl* forceCache,
        ...
        if (data->type != J9SHR_DATA_TYPE_CACHELET) {
            ...
            if (!forceCache) {
                cacheForAllocate = getCacheAreaForDataType(currentThread, wrapperType,
                    _ccHead->getBytesRequiredForItemWithAlign(itemPtr, SHC_WORDALIGN, sizeof(ByteDataWrapper)));
                if (!cacheForAllocate) {
                    /* This may indicate size required is bigger than the cachelet size. */
                    /* TODO: In offline mode, should be fatal */
                    return NULL;
            ...
            if ((itemInCache = (ShcItem*)(cacheForAllocate->allocateBlock(
                     currentThread, itemPtr, SHC_WORDALIGN, sizeof(ByteDataWrapper))))
                == NULL) {
                /* Not enough space in cache to accomodate this item. */
                return NULL;
            }
...
#endif
        itemInCache = (ShcItem*)(cacheForAllocate->allocateWithReadWriteBlock(
            currentThread, itemPtr, (U_32)data->length, &externalBlock));
 ...
 * If data of a different dataType uses the same key, this is added without affecting the other data stored under that
 * key. If J9SHRDATA_SINGLE_STORE_FOR_KEY_TYPE is set, one store for that key/dataType combination is allowed.
 * Subsequent stores will simply return the existing data. If J9SHRDATA_SINGLE_STORE_FOR_KEY_TYPE is not set, the
 * following is true: If data of the same dataType already exists for that key, the original data is marked "stale" and
 * the new data is added. If the exact same data already exists in the cache under the same key and dataType, the data
 * is not duplicated and the cached version is returned. If null is passed as the data argument, all existing data
 * against that key is marked "stale". The function returns null if the cache is full, otherwise it returns a pointer to
 * the shared location of the data.
 *
 ...
 *   J9SHRDATA_SINGLE_STORE_FOR_KEY_TYPE_OVERWRITE - Similar to J9SHRDATA_SINGLE_STORE_FOR_KEY_TYPE, only one record of
 * key/dataType combination is allowed in the shared cache. subsequent stores overwite the existing data. This flag is
 * ignored if J9SHRDATA_NOT_INDEXED, J9SHRDATA_ALLOCATE_ZEROD_MEMORY or J9SHRDATA_USE_READWRITE presents
 *
 * @param[in] currentThread  The current thread
 ...
 * THREADING: This function can be called multi-threaded
 */
const U_8* SH_CacheMap::storeSharedData(
    J9VMThread* currentThread, const char* key, UDATA keylen, const J9SharedDataDescriptor* data)
{
    const U_8* result = NULL;
    ...
    if (J9_ARE_ALL_BITS_SET(data->flags, J9SHRDATA_SINGLE_STORE_FOR_KEY_TYPE_OVERWRITE)) {
        if (J9_ARE_NO_BITS_SET(
                data->flags, J9SHRDATA_NOT_INDEXED | J9SHRDATA_ALLOCATE_ZEROD_MEMORY | J9SHRDATA_USE_READWRITE)
            && (data->length > 0) && (NULL != data->address)) {
            overwrite = true;
        }
    }
    ...
    if (!dataNotIndexed) {
        if (data != NULL) {
            bdwInCache = localBDM->findSingleEntry(currentThread, key, keylen, data->type,
                ((data->flags & J9SHRDATA_IS_PRIVATE) ? _ccHead->getJVMID() : 0), &foundDatalen);
            if ((J9SHR_DATA_TYPE_AOTHEADER == data->type) && (this->_cc->isAOTHeaderPresent(currentThread))) {
                /* If AOT Header has already been added, above call to findSingleEntry() should ideally never return
                 * NULL. AOT Header is expected to be added to the shared cache only once. Once added, every JVM
                 * attached to the cache should be able to locate and re-use it.
                 */
            ...
            if (NULL != bdwInCache) {
                ...
                if (data->address == NULL) {
                    ...
                    if (J9_ARE_ANY_BITS_SET(data->flags,
                            J9SHRDATA_SINGLE_STORE_FOR_KEY_TYPE | J9SHRDATA_SINGLE_STORE_FOR_KEY_TYPE_OVERWRITE)) {
                        if (J9SHR_DATA_TYPE_STARTUP_HINTS == data->type) {
                            Trc_SHR_Assert_True(&_sharedClassConfig->localStartupHints.hintsData
                                == (J9SharedStartupHintsDataDescriptor*)data->address);
                            Trc_SHR_Assert_True(sizeof(J9SharedStartupHintsDataDescriptor) == data->length);
                            updateLocalHintsData(currentThread, &_sharedClassConfig->localStartupHints,
                                (const J9SharedStartupHintsDataDescriptor*)result, overwrite);
                        }
                        if (overwrite) {
                            if (data->length == foundDatalen) {
                                if (memcmp(data->address, result, foundDatalen) != 0) {
                                    memcpy((void*)result, data->address, foundDatalen);
                                    Trc_SHR_CM_storeSharedData_OverwriteExisting(
                                        currentThread, result, data->address, foundDatalen);
                                }
                            } else {
                                Trc_SHR_Assert_ShouldNeverHappen();
                            }
    ...
    if ((data != NULL) && (data->length > 0)
        && ((data->address != NULL) || (data->flags & J9SHRDATA_ALLOCATE_ZEROD_MEMORY))) {
        const J9UTF8* tokenKey = NULL;

        if (!dataNotIndexed) {
            /* Create J9UTF8 struct as key */
            if (keylen >= (STACK_STRINGBUF_SIZE - sizeof(J9UTF8))) {
                if (!(utfKeyPtr = (char*)j9mem_allocate_memory(
                          (keylen * sizeof(U_8)) + sizeof(ShcItem), J9MEM_CATEGORY_CLASSES))) {
 ...
 * @param[in] keylen  The length of the key
 * @param[in] limitDataType  Optional. If used, only data of the type constant specified is returned. If 0, all data
 * stored under that key is returned
 * @param[in] includePrivateData  If non-zero, will also add private data of other JVMs stored under that key into the
 * array
 * @param[out] firstItem The first result found. Can be NULL.
 * @param[out] descriptorPool Populated with all of the results. Note that the pool is not cleaned. Can be NULL - if
 * this is the case, the number of entries is returned.
 *
 ...
 */
IDATA
SH_CacheMap::findSharedData(J9VMThread* currentThread, const char* key, UDATA keylen, UDATA limitDataType,
    UDATA includePrivateData, J9SharedDataDescriptor* firstItem, const J9Pool* descriptorPool)
{
    IDATA result;
 ...
 * If the data entry is private to another JVM and is not in use, it will be made private to this JVM and will be marked
 * "in use". Public data cannot be made private.
 *
 * @param[in] currentThread  The current thread
...
/**
 * If a JVM has finished using a piece of private data and wants to allow another JVM to acquire it, the data entry must
 * be released. This is done automatically when a JVM shuts down, but can also be achieved explicitly using this
 * function. The data descriptor passed to this function must have been returned by findSharedData with the
...
SH_CacheMap::getJavacoreData(J9JavaVM* vm, J9SharedClassJavacoreDataDescriptor* descriptor)
{
    ...
    descriptor->otherBytes = descriptor->cacheSize - ((UDATA)descriptor->metadataStart - (UDATA)descriptor->romClassEnd)
        - descriptor->aotBytes - descriptor->romClassBytes - descriptor->readWriteBytes - descriptor->zipCacheDataBytes
        - descriptor->startupHintBytes - descriptor->jclDataBytes - descriptor->jitHintDataBytes
        - descriptor->jitProfileDataBytes - descriptor->aotDataBytes - descriptor->aotClassChainDataBytes
        - descriptor->aotThunkDataBytes - descriptor->indexedDataBytes - descriptor->objectBytes
    ...
    if ((U_32)-1 != descriptor->softMaxBytes) {
        /* cache header size is not included in descriptor->cacheSize, but it is included in softmx as used bytes. To be
         * consistent, subtract cache header size here */
...
SH_CacheMap::markStale(J9VMThread* currentThread, ClasspathEntryItem* cpei, bool hasWriteMutex)
{
    ...
    while (retryCount < MARK_STALE_RETRY_TIMES) {
        if (hasWriteMutex || (_ccHead->enterWriteMutex(currentThread, true, fnName) == 0)) { /* true = lockCache */
            ...
            CACHEMAP_TRACE3(J9SHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_INFO, J9NLS_SHRC_CM_MARK_STALE, cpeiPathLen,
                cpeiPath, numMarked);
...
void SH_CacheMap::markItemStale(J9VMThread* currentThread, const ShcItem* item, bool isCacheLocked)
{
    if (_ccHead->isRunningReadOnly()) {
        return;
    }
 ...
 */
void SH_CacheMap::markItemStaleCheckMutex(J9VMThread* currentThread, const ShcItem* item, bool isCacheLocked)
{
...
void SH_CacheMap::destroy(J9VMThread* currentThread)
{
...
void SH_CacheMap::reportCorruptCache(J9VMThread* currentThread)
{
    ...
    if (hasRefreshMutex || enteredRefreshMutex) {
        if (!_cacheCorruptReported) {
            ...
            CACHEMAP_TRACE3(J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, J9NLS_ERROR, J9NLS_SHRC_CM_WARN_CORRUPT_CACHE_V2,
                _cacheName, corruptionCode, corruptValue);
            /* Reset the writeHash field in the cache as no more updates can occur */
            if ((*_runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_REDUCE_STORE_CONTENTION)
                && (false == _ccHead->isRunningReadOnly())) {
...
/* THREADING: Can be called multi-threaded */
void SH_CacheMap::resetCorruptState(J9VMThread* currentThread, UDATA hasRefreshMutex)
{
...
/* THREADING: Only ever single-threaded */
IDATA
SH_CacheMap::printAllCacheStats(
    J9VMThread* currentThread, UDATA showFlags, SH_CompositeCacheImpl* cache, U_32* staleBytes)
{
    ...
    /* TODO: Can't handle multiple supercaches - for offline deployment, this isn't a problem as we only ever stat the
     * serialized cache */
        ...
        if (it) {
            ...
            case TYPE_ORPHAN:
                if (showFlags & PRINTSTATS_SHOW_ORPHAN) {
                    ...
                    J9UTF8* romClassName = J9ROMCLASS_CLASSNAME(romClass);
                    CACHEMAP_PRINT5(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_ORPHAN_DISPLAY,
                        ITEMJVMID(it), (UDATA)it, J9UTF8_LENGTH(romClassName), J9UTF8_DATA(romClassName), romClass);
                }
                break;
            ...
            case TYPE_BYTE_DATA: {
                ...
                if (J9SHR_DATA_TYPE_ZIPCACHE == type) {
                    if ((PRINTSTATS_SHOW_ZIPCACHE == (showFlags & PRINTSTATS_SHOW_ZIPCACHE))
                        ...
                        CACHEMAP_PRINT6((J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_PRINTSTATS_ZIPCACHE_DISPLAY,
                            ITEMJVMID(it), (UDATA)it, J9UTF8_LENGTH(pointer), J9UTF8_DATA(pointer), BDWDATA(bdw),
                            BDWLEN(bdw));
                        if (isStale) {
                            CACHEMAP_PRINT((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE),
                                J9NLS_SHRC_CM_PRINTSTATS_STALE);
                        }
                    }
                } else if (J9SHR_DATA_TYPE_JITHINT == type) {
                    if ((PRINTSTATS_SHOW_JITHINT == (showFlags & PRINTSTATS_SHOW_JITHINT))
                        || (isStale && showAllStaleFlag)) {
                        CACHEMAP_PRINT6((J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_PRINTSTATS_JITHINT_DISPLAY,
                            ITEMJVMID(it), (UDATA)it, J9UTF8_LENGTH(pointer), J9UTF8_DATA(pointer), BDWDATA(bdw),
                            BDWLEN(bdw));
                        if (isStale) {
                            CACHEMAP_PRINT((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE),
                    ...
                    if ((PRINTSTATS_SHOW_JCL == (showFlags & PRINTSTATS_SHOW_JCL)) || (isStale && showAllStaleFlag)) {
                        CACHEMAP_PRINT6((J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_PRINTSTATS_JCL_DISPLAY,
                            ITEMJVMID(it), (UDATA)it, J9UTF8_LENGTH(pointer), J9UTF8_DATA(pointer), BDWDATA(bdw),
                            BDWLEN(bdw));
                        if (isStale) {
                            CACHEMAP_PRINT((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE),
                    ...
                    if ((PRINTSTATS_SHOW_AOTCH == (showFlags & PRINTSTATS_SHOW_AOTCH))
                        || (isStale && showAllStaleFlag)) {
                        CACHEMAP_PRINT6((J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_PRINTSTATS_AOTCH_DISPLAY,
                            ITEMJVMID(it), (UDATA)it, J9UTF8_LENGTH(pointer), J9UTF8_DATA(pointer), BDWDATA(bdw),
                            BDWLEN(bdw));
                        if (isStale) {
                            CACHEMAP_PRINT((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE),
                    ...
                    if ((PRINTSTATS_SHOW_AOTTHUNK == (showFlags & PRINTSTATS_SHOW_AOTTHUNK))
                        || (isStale && showAllStaleFlag)) {
                        CACHEMAP_PRINT6((J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_PRINTSTATS_AOTTHUNK_DISPLAY,
                            ITEMJVMID(it), (UDATA)it, J9UTF8_LENGTH(pointer), J9UTF8_DATA(pointer), BDWDATA(bdw),
                            BDWLEN(bdw));
                        if (isStale) {
                            CACHEMAP_PRINT((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE),
                    ...
                    if ((PRINTSTATS_SHOW_AOTDATA == (showFlags & PRINTSTATS_SHOW_AOTDATA))
                        || (isStale && showAllStaleFlag)) {
                        CACHEMAP_PRINT6((J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_PRINTSTATS_AOTDATA_DISPLAY,
                            ITEMJVMID(it), (UDATA)it, J9UTF8_LENGTH(pointer), J9UTF8_DATA(pointer), BDWDATA(bdw),
                            BDWLEN(bdw));
                        if (isStale) {
                            CACHEMAP_PRINT((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE),
                    ...
                    if ((PRINTSTATS_SHOW_BYTEDATA == (showFlags & PRINTSTATS_SHOW_BYTEDATA))
                        || (isStale && showAllStaleFlag)) {
                        CACHEMAP_PRINT6((J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_PRINTSTATS_HELPER_DISPLAY,
                            ITEMJVMID(it), (UDATA)it, J9UTF8_LENGTH(pointer), J9UTF8_DATA(pointer), BDWDATA(bdw),
                            BDWLEN(bdw));
                        if (isStale) {
                            CACHEMAP_PRINT((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE),
                    ...
                    if ((PRINTSTATS_SHOW_BYTEDATA == (showFlags & PRINTSTATS_SHOW_BYTEDATA))
                        || (isStale && showAllStaleFlag)) {
                        CACHEMAP_PRINT6((J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_PRINTSTATS_POOL_DISPLAY,
                            ITEMJVMID(it), (UDATA)it, J9UTF8_LENGTH(pointer), J9UTF8_DATA(pointer), BDWDATA(bdw),
                            BDWLEN(bdw));
                        if (isStale) {
                            CACHEMAP_PRINT((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE),
                    ...
                    if ((PRINTSTATS_SHOW_BYTEDATA == (showFlags & PRINTSTATS_SHOW_BYTEDATA))
                        || (isStale && showAllStaleFlag)) {
                        CACHEMAP_PRINT6((J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_PRINTSTATS_VM_DISPLAY,
                            ITEMJVMID(it), (UDATA)it, J9UTF8_LENGTH(pointer), J9UTF8_DATA(pointer), BDWDATA(bdw),
                            BDWLEN(bdw));
                        if (isStale) {
                            CACHEMAP_PRINT((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE),
                    ...
                    if ((PRINTSTATS_SHOW_BYTEDATA == (showFlags & PRINTSTATS_SHOW_BYTEDATA))
                        || (isStale && showAllStaleFlag)) {
                        CACHEMAP_PRINT6((J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_PRINTSTATS_ROMSTRING_DISPLAY,
                            ITEMJVMID(it), (UDATA)it, J9UTF8_LENGTH(pointer), J9UTF8_DATA(pointer), BDWDATA(bdw),
                            BDWLEN(bdw));
                        if (isStale) {
                            CACHEMAP_PRINT((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE),
                    ...
                    if ((J9_ARE_ANY_BITS_SET(showFlags, PRINTSTATS_SHOW_STARTUPHINT))
                        ...
                        CACHEMAP_PRINT6((J9NLS_DO_NOT_PRINT_MESSAGE_TAG),
                            J9NLS_SHRC_CM_PRINTSTATS_STARTUP_HINTS_DISPLAY, ITEMJVMID(it), (UDATA)it,
                            J9UTF8_LENGTH(pointer), J9UTF8_DATA(pointer), BDWDATA(bdw), BDWLEN(bdw));
                        CACHEMAP_PRINT3((J9NLS_DO_NOT_PRINT_MESSAGE_TAG),
                            J9NLS_SHRC_CM_PRINTSTATS_STARTUP_HINTS_DISPLAY_DETAIL, hints->flags, hints->heapSize1,
                            hints->heapSize2);
                        if (isStale) {
                            CACHEMAP_PRINT((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE),
                                J9NLS_SHRC_CM_PRINTSTATS_STALE);
                    ...
                    || (isStale && showAllStaleFlag)) {
                    CACHEMAP_PRINT6((J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_CM_PRINTSTATS_UNKNOWN_DISPLAY,
                        ITEMJVMID(it), (UDATA)it, J9UTF8_LENGTH(pointer), J9UTF8_DATA(pointer), BDWDATA(bdw),
                        BDWLEN(bdw));
                    if (isStale) {
                        CACHEMAP_PRINT((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE),
            ...
            case TYPE_SCOPED_ROMCLASS: {
                ...
                if ((showFlags & PRINTSTATS_SHOW_ROMCLASS) || (showFlags & PRINTSTATS_SHOW_ROMMETHOD)
                    ...
                    CACHEMAP_PRINT5((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE),
                        J9NLS_SHRC_CM_PRINTSTATS_ROMCLASS_DISPLAY, ITEMJVMID(it), (UDATA)it,
                        J9UTF8_LENGTH(romClassName), J9UTF8_DATA(romClassName), RCWROMCLASS(rcw));
                    if (isStale) {
                        CACHEMAP_PRINT((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE),
                            J9NLS_SHRC_CM_PRINTSTATS_STALE);
                    }
                    ...
                    if (cpi->getType() == CP_TYPE_CLASSPATH) {
                        CACHEMAP_PRINT2(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_ROMCLASS_INDEX_IN_CP,
                            rcw->cpeIndex, (UDATA)cpw);
                    } else if (cpi->getType() == CP_TYPE_URL) {
                        CACHEMAP_PRINT1(
                            J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_ROMCLASS_URL_DISPLAY, (UDATA)cpw);
                    } else if (cpi->getType() == CP_TYPE_TOKEN) {
                        CACHEMAP_PRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_ROMCLASS_TOKEN_DISPLAY,
                            (UDATA)cpw);
                    }
                    if (ITEMTYPE(it) == TYPE_SCOPED_ROMCLASS) {
                        ...
                        if (partition && !modContext) {
                            CACHEMAP_PRINT2(J9NLS_DO_NOT_PRINT_MESSAGE_TAG,
                                J9NLS_SHRC_CM_PRINTSTATS_ROMCLASS_PARTITION_DISPLAY, J9UTF8_LENGTH(partition),
                                J9UTF8_DATA(partition));
                        } else if (modContext && !partition) {
                            CACHEMAP_PRINT2(J9NLS_DO_NOT_PRINT_MESSAGE_TAG,
                                J9NLS_SHRC_CM_PRINTSTATS_ROMCLASS_MODCONTEXT_DISPLAY, J9UTF8_LENGTH(modContext),
                                J9UTF8_DATA(modContext));
                        } else if (modContext && partition) {
                            CACHEMAP_PRINT4(J9NLS_DO_NOT_PRINT_MESSAGE_TAG,
                                J9NLS_SHRC_CM_PRINTSTATS_ROMCLASS_PARTITIONINMOD_DISPLAY, J9UTF8_LENGTH(partition),
                                J9UTF8_DATA(partition), J9UTF8_LENGTH(modContext), J9UTF8_DATA(modContext));
                        }
                    }
                    ...
                    if (showFlags & PRINTSTATS_SHOW_ROMMETHOD) {
                        ...
                        for (i = 0; i < romClass->romMethodCount; i++) {
                            ...
                            if (romMethodName && romMethodSig) {
                                CACHEMAP_PRINT5(J9NLS_DO_NOT_PRINT_MESSAGE_TAG,
                                    J9NLS_SHRC_CM_PRINTSTATS_ROMMETHOD_DISPLAY, J9UTF8_LENGTH(romMethodName),
                                    J9UTF8_DATA(romMethodName), J9UTF8_LENGTH(romMethodSig), J9UTF8_DATA(romMethodSig),
                                    romMethod);
                            }
            ...
            case TYPE_CLASSPATH:
                ...
                if (((showFlags & PRINTSTATS_SHOW_CLASSPATH) && (cpi->getType() == CP_TYPE_CLASSPATH))
                    ...
                    if (cpi->getType() == CP_TYPE_CLASSPATH) {
                        CACHEMAP_PRINT2(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_CLASSPATH_DISPLAY,
                            ITEMJVMID(it), (UDATA)cpw);
                    } else if (cpi->getType() == CP_TYPE_URL) {
                        CACHEMAP_PRINT2(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_CLASSPATH_URL_DISPLAY,
                            ITEMJVMID(it), (UDATA)cpw);
                    } else if (cpi->getType() == CP_TYPE_TOKEN) {
                        CACHEMAP_PRINT2(J9NLS_DO_NOT_PRINT_MESSAGE_TAG,
                            J9NLS_SHRC_CM_PRINTSTATS_CLASSPATH_TOKEN_DISPLAY, ITEMJVMID(it), (UDATA)cpw);
                    }
                    ...
                    for (I_16 i = 0; i < cpi->getItemsAdded(); i++) {
                        ...
                        if (i == (cpi->getItemsAdded() - 1)) {
                            CACHEMAP_PRINT2(J9NLS_DO_NOT_PRINT_MESSAGE_TAG,
                                J9NLS_SHRC_CM_PRINTSTATS_CLASSPATH_DISPLAYLASTITEM, cpeiPathLen, cpeiPath);
                        } else {
                            CACHEMAP_PRINT2(J9NLS_DO_NOT_PRINT_MESSAGE_TAG,
                                J9NLS_SHRC_CM_PRINTSTATS_CLASSPATH_DISPLAYITEM, cpeiPathLen, cpeiPath);
                        }
                    }
                }
                break;
            ...
            case TYPE_COMPILED_METHOD:
                if ((J9_ARE_ALL_BITS_SET(showFlags, PRINTSTATS_SHOW_AOT) && (TYPE_COMPILED_METHOD == ITEMTYPE(it)))
                    || (J9_ARE_ALL_BITS_SET(showFlags, PRINTSTATS_SHOW_INVALIDATEDAOT)
                    ...
                    if (romMethodName) {
                        CACHEMAP_PRINT4((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE),
                            J9NLS_SHRC_CM_PRINTSTATS_AOT_DISPLAY, ITEMJVMID(it), (UDATA)it,
                            J9UTF8_LENGTH(romMethodName), J9UTF8_DATA(romMethodName));
                    }

                    if (romMethodSig) {
                        CACHEMAP_PRINT3((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE),
                            J9NLS_SHRC_CM_PRINTSTATS_AOT_DISPLAY_SIG_ADDR, J9UTF8_LENGTH(romMethodSig),
                            J9UTF8_DATA(romMethodSig), romMethod);
                    }
                    ...
                    if (TYPE_INVALIDATED_COMPILED_METHOD == ITEMTYPE(it)) {
                        j9tty_printf(_portlib, " ");
                        CACHEMAP_PRINT((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE),
                            J9NLS_SHRC_CM_PRINTSTATS_INVALIDATED);
                    }
                    j9tty_printf(_portlib, "\n");
                    if (romClassName) {
                        CACHEMAP_PRINT3(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_AOT_FROM_ROMCLASS,
                            J9UTF8_LENGTH(romClassName), J9UTF8_DATA(romClassName), romClass);
                    }
                }
                break;
            ...
            case TYPE_ATTACHED_DATA:
                if (0 != (showFlags & (PRINTSTATS_SHOW_JITPROFILE | PRINTSTATS_SHOW_JITHINT))
                    ...
                    if ((J9SHR_ATTACHED_DATA_TYPE_JITPROFILE == ADWTYPE(adw))
                        ...
                        J9UTF8* romClassName = NULL;
                        J9ROMClass* romClass
                            = vmFunctions->findROMClassFromPC(currentThread, (UDATA)romMethod, &loader);
                        J9UTF8* romMethodName = J9ROMMETHOD_GET_NAME(romClass, romMethod);
                        J9UTF8* romMethodSig = J9ROMMETHOD_GET_SIGNATURE(romClass, romMethod);
                        ...
                        if (romMethodName) {
                            ...
                            if ((0 != (showFlags & PRINTSTATS_SHOW_JITHINT))
                                && (J9SHR_ATTACHED_DATA_TYPE_JITHINT == ADWTYPE(adw))) {
                                CACHEMAP_PRINT4((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE),
                                    J9NLS_SHRC_CM_PRINTSTATS_ATTACHED_JITHINT_DISPLAY, ITEMJVMID(it), (UDATA)it,
                                    J9UTF8_LENGTH(romMethodName), J9UTF8_DATA(romMethodName));
                            } else if ((0 != (showFlags & PRINTSTATS_SHOW_JITPROFILE))
                                && (J9SHR_ATTACHED_DATA_TYPE_JITPROFILE == ADWTYPE(adw))) {
                                CACHEMAP_PRINT4((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE),
                                    J9NLS_SHRC_CM_PRINTSTATS_JITPROFILE_DISPLAY, ITEMJVMID(it), (UDATA)it,
                                    J9UTF8_LENGTH(romMethodName), J9UTF8_DATA(romMethodName));
                            }
                        }
                        ...
                        if (romMethodSig) {
                            CACHEMAP_PRINT3((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE),
                                J9NLS_SHRC_CM_PRINTSTATS_JITPROFILE_DISPLAY_SIG_ADDR, J9UTF8_LENGTH(romMethodSig),
                                J9UTF8_DATA(romMethodSig), romMethod);
                        }
                        ...
                        if (romClassName) {
                            CACHEMAP_PRINT3(J9NLS_DO_NOT_PRINT_MESSAGE_TAG,
                                J9NLS_SHRC_CM_PRINTSTATS_JITPROFILE_FROM_ROMCLASS, J9UTF8_LENGTH(romClassName),
                                J9UTF8_DATA(romClassName), romClass);
                        }
                    }
                }
                break;
    ...
    if (1 == getJavacoreData(currentThread->javaVM, &javacoreData)) {
        ...
        if (true == this->_cc->getIsNoLineNumberContentEnabled()) {
            if (true == this->_cc->getIsLineNumberContentEnabled()) {
                CACHEMAP_PRINT(J9NLS_DO_NOT_PRINT_MESSAGE_TAG,
                    J9NLS_SHRC_CM_PRINTSTATS_CACHE_CONTAINS_CONTENT_WITH_AND_WITHOUT_LINE_NUMBERS);
            } else {
                CACHEMAP_PRINT(J9NLS_DO_NOT_PRINT_MESSAGE_TAG,
                    J9NLS_SHRC_CM_PRINTSTATS_CACHE_CONTAINS_CONTENT_ONLY_WITHOUT_LINE_NUMBERS);
            }
        } else {
            if (true == this->_cc->getIsLineNumberContentEnabled()) {
                CACHEMAP_PRINT(J9NLS_DO_NOT_PRINT_MESSAGE_TAG,
                    J9NLS_SHRC_CM_PRINTSTATS_CACHE_CONTAINS_CONTENT_ONLY_WITH_LINE_NUMBERS);
            }
        }

        CACHEMAP_FMTPRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_BASEADDRESS_V2,
            javacoreData.romClassStart);
        CACHEMAP_FMTPRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_ENDADDRESS_V2,
            javacoreData.cacheEndAddress);
        CACHEMAP_FMTPRINT1(
            J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_ALLOCPTR_V2, javacoreData.romClassEnd);
        if (runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_DETAILED_STATS) {
            CACHEMAP_FMTPRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_METADATA_STARTADDRESS,
                javacoreData.metadataStart);
            CACHEMAP_FMTPRINT1(
                J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_RUNTIME_FLAGS, javacoreData.runtimeFlags);
            CACHEMAP_FMTPRINT1(
                J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_CACHE_GEN, javacoreData.cacheGen);
        }
        CACHEMAP_FMTPRINT1(
            J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_CACHESIZE_V2, javacoreData.cacheSize);
        CACHEMAP_FMTPRINT1(
            J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_SOFTMXBYTES, javacoreData.softMaxBytes);
        CACHEMAP_FMTPRINT1(
            J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_FREEBYTES_V2, javacoreData.freeBytes);
        CACHEMAP_FMTPRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_ROMCLASS_BYTES_V2,
            javacoreData.romClassBytes);
        if (runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_DETAILED_STATS) {
            CACHEMAP_FMTPRINT1(
                J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_AOT_CODE_BYTES, javacoreData.aotBytes);
            CACHEMAP_FMTPRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_AOT_DATA_BYTES,
                javacoreData.aotDataBytes);
            CACHEMAP_FMTPRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG,
                J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_AOT_CLASS_HIERARCHY_BYTES, javacoreData.aotClassChainDataBytes);
            CACHEMAP_FMTPRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_AOT_THUNK_BYTES,
                javacoreData.aotThunkDataBytes);
            CACHEMAP_FMTPRINT1(
                J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_AOT_MIN, javacoreData.minAOT);
            CACHEMAP_FMTPRINT1(
                J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_AOT_MAX, javacoreData.maxAOT);
            CACHEMAP_FMTPRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_JIT_HINT_BYTES,
                javacoreData.jitHintDataBytes);
            CACHEMAP_FMTPRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_JIT_PROFILE_BYTES,
                javacoreData.jitProfileDataBytes);
            CACHEMAP_FMTPRINT1(
                J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_JIT_MIN, javacoreData.minJIT);
            CACHEMAP_FMTPRINT1(
                J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_JIT_MAX, javacoreData.maxJIT);
        } else {
            CACHEMAP_FMTPRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_AOT_BYTES_V2,
                javacoreData.aotBytes + javacoreData.aotDataBytes + javacoreData.aotClassChainDataBytes
                    + javacoreData.aotThunkDataBytes);
            CACHEMAP_FMTPRINT1(
                J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_AOT_MIN, javacoreData.minAOT);
            CACHEMAP_FMTPRINT1(
                J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_AOT_MAX, javacoreData.maxAOT);
            CACHEMAP_FMTPRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_JIT_DATA_BYTES_V2,
                javacoreData.jitHintDataBytes + javacoreData.jitProfileDataBytes);
            CACHEMAP_FMTPRINT1(
                J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_JIT_MIN, javacoreData.minJIT);
            CACHEMAP_FMTPRINT1(
                J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_JIT_MAX, javacoreData.maxJIT);
        ...
        if (runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_DETAILED_STATS) {
            CACHEMAP_FMTPRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_JAVA_OBJECT_BYTES,
                javacoreData.objectBytes);
        }
        CACHEMAP_FMTPRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_ZIP_CACHE_DATA_BYTES_V2,
            javacoreData.zipCacheDataBytes);
        CACHEMAP_FMTPRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_STARTUP_HINT_BYTES,
            javacoreData.startupHintBytes);

        if (runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_DETAILED_STATS) {
            CACHEMAP_FMTPRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_READWRITE_BYTES,
                javacoreData.readWriteBytes);
            CACHEMAP_FMTPRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_JCL_DATA_BYTES,
                javacoreData.jclDataBytes);
            CACHEMAP_FMTPRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_BYTE_DATA_BYTES,
                javacoreData.indexedDataBytes);
        } else {
            UDATA dataBytes = javacoreData.indexedDataBytes + javacoreData.readWriteBytes + javacoreData.jclDataBytes
                + javacoreData.objectBytes;
            CACHEMAP_FMTPRINT1(
                J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_DATA_BYTES_V2, dataBytes);
        }
        CACHEMAP_FMTPRINT1(
            J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_META_BYTES_V2, javacoreData.otherBytes);
        if ((U_32)-1 == javacoreData.softMaxBytes) {
            ...
            CACHEMAP_FMTPRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_META_PERCENT_V2,
                ((javacoreData.otherBytes * 100) / (javacoreData.cacheSize - javacoreData.freeBytes)));
        } else {
            /* cache header size is not included in javacoreData.cacheSize, but it is included in softmx as used bytes.
             * To be consistent, subtract cache header size here */
            CACHEMAP_FMTPRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_META_PERCENT_V2,
                ((javacoreData.otherBytes * 100)
                    / (javacoreData.softMaxBytes
                          - (javacoreData.totalSize - javacoreData.cacheSize) /* subtract header size */
                          - javacoreData.freeBytes)));
        }
        CACHEMAP_FMTPRINT1(
            J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_DEBUGAREA_SIZE, javacoreData.debugAreaSize);

        if (runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_DETAILED_STATS) {
            CACHEMAP_FMTPRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_DEBUGAREA_LINENUMBERTABLE_BYTES,
                javacoreData.debugAreaLineNumberTableBytes);
            CACHEMAP_FMTPRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG,
                J9NLS_SHRC_CM_PRINTSTATS_DEBUGAREA_LOCALVARIABLETABLE_BYTES_V2,
                javacoreData.debugAreaLocalVariableTableBytes);
        } else {
            CACHEMAP_FMTPRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_DEBUGAREA_USED_BYTES,
                javacoreData.debugAreaLineNumberTableBytes + javacoreData.debugAreaLocalVariableTableBytes);
        }
        CACHEMAP_FMTPRINT1(
            J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_DEBUGAREA_USED, javacoreData.debugAreaUsed);
        ...
        CACHEMAP_FMTPRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_NUM_ROMCLASSES_V2,
            javacoreData.numROMClasses);
        CACHEMAP_FMTPRINT1(
            J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_NUM_AOT_V2, javacoreData.numAOTMethods);
        if (runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_DETAILED_STATS) {
            CACHEMAP_FMTPRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_NUM_AOT_DATA,
                javacoreData.numAotDataEntries);
            CACHEMAP_FMTPRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_NUM_AOT_CLASS_HIERARCHY,
                javacoreData.numAotClassChains);
            CACHEMAP_FMTPRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_NUM_AOT_THUNKS,
                javacoreData.numAotThunks);
            CACHEMAP_FMTPRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_NUM_JIT_HINTS,
                javacoreData.numJitHints);
            CACHEMAP_FMTPRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_NUM_JIT_PROFILES,
                javacoreData.numJitProfiles);
        }
        CACHEMAP_FMTPRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_NUM_CLASSPATHS_V2,
            javacoreData.numClasspaths);
        CACHEMAP_FMTPRINT1(
            J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_NUM_URLS_V2, javacoreData.numURLs);
        CACHEMAP_FMTPRINT1(
            J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_NUM_TOKENS_V2, javacoreData.numTokens);
        if (runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_DETAILED_STATS) {
            CACHEMAP_FMTPRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_NUM_JAVA_OBJECTS,
                javacoreData.numObjects);
        }
        CACHEMAP_FMTPRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_NUM_ZIP_CACHES_V2,
            javacoreData.numZipCaches);
        CACHEMAP_FMTPRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_NUM_STARTUP_HINTS,
            javacoreData.numStartupHints);
        if (runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_DETAILED_STATS) {
            CACHEMAP_FMTPRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_NUM_JCL_ENTRIES,
                javacoreData.numJclEntries);
        }
        CACHEMAP_FMTPRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_NUM_STALE_CLASSES_V2,
            javacoreData.numStaleClasses);
        CACHEMAP_FMTPRINT1(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_PERC_STALE_CLASSES_V2,
            javacoreData.percStale);
        if (javacoreData.cacheSize == javacoreData.softMaxBytes) {
            CACHEMAP_PRINT1(
                J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_PERC_FULL, javacoreData.percFull);
        } else {
            CACHEMAP_PRINT1(
                J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_SHRC_CM_PRINTSTATS_SUMMARY_PERC_SOFT_FULL, javacoreData.percFull);
        }
...
bool SH_CacheMap::isBytecodeAgentInstalled(void)
{
 ...
 */
void SH_CacheMap::printShutdownStats(void)
{
    UDATA bytesStored = 0;
    ...
    CACHEMAP_TRACE2(J9SHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_INFO, J9NLS_SHRC_CM_PRINTSHUTDOWNSTATS_READ_STORED,
        bytesRead, bytesStored);
    CACHEMAP_TRACE3(J9SHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_INFO, J9NLS_SHRC_CM_PRINTSHUTDOWNSTATS_UNSTORED_V1,
        softmxUnstoredBytes, maxAOTUnstoredBytes, maxJITUnstoredBytes);
}

/**
 * Run required code on JVM exit
 */
void SH_CacheMap::runExitCode(J9VMThread* currentThread)
{
    SH_Manager* walkManager;
    SH_Managers::ManagerWalkState state;
    SH_CompositeCacheImpl* cache = _ccHead;
...
/* THREADING: Can be called multi-threaded */
IDATA /*static */
SH_CacheMap::createPathString(J9VMThread* currentThread, J9SharedClassConfig* config, char** pathBuf, UDATA pathBufSize,
    ClasspathEntryItem* cpei, const char* className, UDATA classNameLen, bool* doFreeBuffer)
{
 ...
 */
void SH_CacheMap::getRomClassAreaBounds(void** romClassAreaStart, void** romClassAreaEnd)
{
...
#if defined(J9SHR_CACHELET_SUPPORT)
void SH_CacheMap::getBoundsForCache(
    SH_CompositeCacheImpl* cache, BlockPtr* cacheStart, BlockPtr* romClassEnd, BlockPtr* metaStart, BlockPtr* cacheEnd)
{
    if (cacheStart) {
...
IDATA
SH_CacheMap::enterStringTableMutex(
    J9VMThread* currentThread, BOOLEAN readOnly, UDATA* doRebuildLocalData, UDATA* doRebuildCacheData)
{
    IDATA result = -1;
    ...
    if ((result = _ccHead->enterReadWriteAreaMutex(currentThread, readOnly, doRebuildLocalData, doRebuildCacheData))
        == 0) {
        ...
        if (table != NULL) {
            ...
            if (J9_ARE_ANY_BITS_SET(_sharedClassConfig->runtimeFlags,
                    J9SHR_RUNTIMEFLAG_BLOCK_SPACE_FULL | J9SHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL)) {
 ...
 */
const J9UTF8* SH_CacheMap::getCachedUTFString(J9VMThread* currentThread, const char* local, U_16 localLen)
{
    ...
    if (J9_ARE_ANY_BITS_SET(*_runtimeFlags, RUNTIME_FLAGS_PREVENT_BLOCK_DATA_UPDATE)) {
        /* runtime flags will be updated in runEntryPointChecks(), check runtime flags after runEntryPointChecks() is
         * called */
        allowUpdate = false;
    }
 ...
 */
void SH_CacheMap::notifyClasspathEntryStateChange(J9VMThread* currentThread, const char* path, UDATA newState)
{
...
SH_CompositeCache* SH_CacheMap::getCompositeCacheAPI() { return (SH_CompositeCache*)_ccHead; }

SH_ScopeManager* SH_CacheMap::getScopeManager(J9VMThread* currentThread)
{
...
SH_ClasspathManager* SH_CacheMap::getClasspathManager(J9VMThread* currentThread)
{
...
SH_ROMClassManager* SH_CacheMap::getROMClassManager(J9VMThread* currentThread)
{
...
SH_ByteDataManager* SH_CacheMap::getByteDataManager(J9VMThread* currentThread)
{
...
SH_CompiledMethodManager* SH_CacheMap::getCompiledMethodManager(J9VMThread* currentThread)
{
...
SH_AttachedDataManager* SH_CacheMap::getAttachedDataManager(J9VMThread* currentThread)
{
...
SH_CompositeCacheImpl* SH_CacheMap::initCachelet(
    J9VMThread* currentThread, BlockPtr existingCacheletMemory, bool creatingCachelet)
{
    ...
    newCachelet = SH_CompositeCacheImpl::newInstanceNested(currentThread->javaVM, _cc, (SH_CompositeCacheImpl*)ccMem,
        J9SHR_DEFAULT_CACHELET_SIZE, existingCacheletMemory, creatingCachelet);
    if (!newCachelet) {
        pool_removeElement(_ccPool, ccMem);
        return NULL;
    }
...
/* THREADING: Must have cache write mutex */
SH_CompositeCacheImpl* SH_CacheMap::createNewCachelet(J9VMThread* currentThread)
{
                ...
                if (startupCachelet(currentThread, returnVal) == CC_STARTUP_OK) {
                    if (createdNewChainedCache) {
                        ...
                        if (stringTable) {
                            ...
                            /* TODO: VERY IMPORTANT!! Entering the readWrite area mutex while we have the writeMutex
                             * breaks our threading model for cross-process synchronization. If this were ever used for
                             * multi-JVM, it could/would deadlock. However, we have to get the readWrite mutex here so
                             * that we can reset the string table. Currently, this will cause assertion failures in
                             * CompositeCache for that reason. */
                            _cc->enterReadWriteAreaMutex(currentThread, FALSE, &ignore, &ignore);
                            _cc->setInternCacheHeaderFields(&(stringTable->sharedTailNodePtr),
                                &(stringTable->sharedHeadNodePtr), &(stringTable->totalSharedNodesPtr),
...
/* THREADING: Must have cache write mutex */
SH_CompositeCacheImpl* SH_CacheMap::createNewChainedCache(J9VMThread* currentThread, UDATA requiredSize)
{
    void* ccMem;
    ...
    newCache = SH_CompositeCacheImpl::newInstanceChained(
        currentThread->javaVM, (SH_CompositeCacheImpl*)ccMem, _sharedClassConfig, J9PORT_SHR_CACHE_TYPE_VMEM);
    if (newCache) {
        J9JavaVM* vm = currentThread->javaVM;
        U_32 ignored;
        IDATA rc;
...
SH_CacheMap::buildCacheletMetadata(J9VMThread* currentThread, SH_Manager::CacheletMetadataArray** metadataArray)
{
    ...
    for (counter = 0; counter < array->numMetas; counter++) {
        ...
        total += (U_32)(
            sizeof(UDATA) * ((SH_CompositeCacheImpl*)cacheletMetadata->cachelet)->countROMSegments(currentThread));
...
void SH_CacheMap::freeCacheletMetadata(J9VMThread* currentThread, SH_Manager::CacheletMetadataArray* metaArray)
{
    PORT_ACCESS_FROM_VMC(currentThread);
    SH_Manager* walkManager;
    SH_Managers::ManagerWalkState state;
...
IDATA
SH_CacheMap::readCacheletHints(
    J9VMThread* currentThread, SH_CompositeCacheImpl* cachelet, CacheletWrapper* cacheletWrapper)
{
    UDATA counter = 0;
 ...
 */
bool SH_CacheMap::readCacheletSegments(
    J9VMThread* currentThread, SH_CompositeCacheImpl* cachelet, CacheletWrapper* cacheletWrapper)
{
    ...
    for (i = 0; i < cacheletWrapper->numSegments; ++i) {
        ...
        if (!segment) {
            Trc_SHR_CM_readCacheletSegments_addSegmentFailed(
                currentThread, cachelet, cacheletWrapper, i, baseAddress, baseAddress + *segmentLengths);
            return false;
        }
    ...
    Trc_SHR_CM_readCacheletSegments_addedSegments(currentThread, cachelet, cacheletWrapper,
        cacheletWrapper->numSegments, firstBaseAddress, segment->heapTop, segment->heapAlloc,
...
SH_CacheMap::writeCacheletMetadata(J9VMThread* currentThread, SH_Manager::CacheletMetadata* cacheletMetadata)
{
    ...
    if (itemInCache == NULL) {
        ...
        if (true == _cc->isCacheMarkedFull(currentThread)) {
            ...
            CACHEMAP_PRINT3(
                J9NLS_ERROR, J9NLS_SHRC_CM_INSUFFICIENT_FREE_BLOCK, _cacheName, freeBlockBytes, totalSizeNeeded);
 ...
 * @retval false failed
 */
bool SH_CacheMap::serializeSharedCache(J9VMThread* currentThread)
{
    bool ok = true;
 ...
 *
 */
bool SH_CacheMap::serializeOfflineCache(J9VMThread* currentThread)
{
    ...
    piconfig.sharedClassCacheSize
        = totalMetadataSize + totalCacheletSize + _ccHead->getOSPageSize(); /* Add a page for the header etc */
#if defined(J9SHR_CACHELETS_SAVE_READWRITE_AREA)
    ...
    *_runtimeFlags &= ~J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT; /* TODO: HACK FOR NOW */
    ...
    while (true) {
        serializedCache = SH_CompositeCacheImpl::newInstance(
            vm, &config, (SH_CompositeCacheImpl*)objectMemory, _cacheName, J9PORT_SHR_CACHE_TYPE_PERSISTENT, false);
        if (serializedCache->startup(currentThread, &piconfig, NULL, _runtimeFlags, _verboseFlags, _cacheName,
                _cacheDir, vm->sharedCacheAPI->cacheDirPerm, &actualSize, &localCrashCntr, true, &cacheHasIntegrity)
            != CC_STARTUP_OK) {
            ok = false;
            break;
        }
 ...
 */
void SH_CacheMap::setDeployedROMClassStarts(J9VMThread* currentThread, void* serializedROMClassStartAddress)
{
    J9SharedClassCacheDescriptor* cacheDesc;
    SH_CompositeCacheImpl* walk;
...
void SH_CacheMap::fixupCacheletMetadata(J9VMThread* currentThread, SH_Manager::CacheletMetadataArray* metaArray,
    SH_CompositeCacheImpl* serializedCache, IDATA metadataOffset)
{
    ...
    for (UDATA j = 0; j < metaArray->numMetas; ++j) {
        ...
        IDATA bytesFromNewMetaToNewCacheStart
            = (UDATA)serializedCache->getMetaAllocPtr() - ((UDATA)parentBaseAddress + cacheletOffset);
        CacheletHints* copiedHints; /* hints location in serialized cache */

#ifdef DEBUG
        fprintf(stderr, " old cachelet: start=%p end=%p\n", cachelet->getFirstROMClassAddress(false),
            cachelet->getCacheEndAddress());
        ...
        for (UDATA i = 0; i < copiedWrapper->numHints; ++i) {
            ...
            if (copiedHints->dataType != TYPE_UNINITIALIZED) {
                ...
                if (mgr) {
                    mgr->fixupHintsForSerialization(currentThread, copiedHints->dataType, copiedHints->length,
                        (U_8*)&copiedHints[1], cacheletOffset, (void*)serializedCache->getFirstROMClassAddress(true));
                }
            }
...
#if defined(J9SHR_CACHELETS_SAVE_READWRITE_AREA)
void SH_CacheMap::fixCacheletReadWriteOffsets(J9VMThread* currentThread)
{
    ShcItem* it = NULL;
 ...
 */
J9SharedClassCacheDescriptor* SH_CacheMap::appendCacheDescriptorList(
    J9VMThread* currentThread, J9SharedClassConfig* sharedClassConfig)
{
    PORT_ACCESS_FROM_VMC(currentThread);
    J9SharedClassCacheDescriptor* cacheDesc;

    cacheDesc = (J9SharedClassCacheDescriptor*)j9mem_allocate_memory(
        sizeof(J9SharedClassCacheDescriptor), J9MEM_CATEGORY_CLASSES);
    if (!cacheDesc) {
 ...
 */
void SH_CacheMap::resetCacheDescriptorList(J9VMThread* currentThread, J9SharedClassConfig* sharedClassConfig)
{
    PORT_ACCESS_FROM_VMC(currentThread);
    ...
    sharedClassConfig->cacheDescriptorList = cacheDesc;
    Trc_SHR_Assert_True(sharedClassConfig->cacheDescriptorList == sharedClassConfig->cacheDescriptorList->next);
 ...
 */
bool SH_CacheMap::isAddressInROMClassSegment(const void* address) { return _cc->isAddressInROMClassSegment(address); }
 ...
 * Caller should hold string table write lock
 */
void SH_CacheMap::setStringTableInitialized(bool isInitialized)
{
 ...
 * Caller should hold string table write lock
 */
bool SH_CacheMap::isStringTableInitialized(void) { return this->_ccHead->isStringTableInitialized(); }
 ...
 */
U_32 SH_CacheMap::getDebugBytes(void) { return this->_ccHead->getDebugBytes(); }

SH_Managers* SH_CacheMap::managers() { return this->_managers; }
...
J9SharedClassConfig* SH_CacheMap::getSharedClassConfig() { return _sharedClassConfig; }

UDATA
SH_CacheMap::getReadWriteBytes(void) { return this->_ccHead->getReadWriteBytes(); }

const char* SH_CacheMap::attachedTypeString(UDATA type)
{
    switch (type) {
    case J9SHR_ATTACHED_DATA_TYPE_JITPROFILE:
        return "JITPROFILE";
...
bool SH_CacheMap::isCacheCorruptReported(void) { return _cacheCorruptReported; }
    ...
    char* attachedDataStringBuffer, UDATA bufferLength)
{
    ...
    *stringCursor = '\0'; /* handle the zero-length case */
    while ((bytesRemaining > 0) && ((stringCursor + BYTE_STRING_LENGTH) < stringBufferEnd)) {
        stringCursor += j9str_printf(PORTLIB, stringCursor, bufferLength, "0x%#02x ",
            *dataCursor); /* increment does not include the trailing '\0' */
        ++dataCursor;
        --bytesRemaining;
    }
 ...
 * @param[in] methodSpecs The pointer of the method specifications passed in by
 * "invalidateAotMethods/revalidateAotMethods/findAotMethods=" option
...
SH_CacheMap::aotMethodOperation(J9VMThread* currentThread, char* methodSpecs, UDATA action)
{
    ...
    while (
        ('{' == *optionStartPos) || (';' == *optionStartPos) || (' ' == *optionStartPos) || ('"' == *optionStartPos)) {
    ...
    if (numSpecs <= 0) {
        ...
        CACHEMAP_TRACE1(J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, J9NLS_ERROR,
            J9NLS_SHRC_CM_PARSE_METHOD_SPECS_TOO_MANY_ERROR, SHR_METHOD_SPEC_TABLE_MAX_SIZE);
        return -1;
    }
 ...
 * @param[in] numSpecs The number of the method specifications passed in by
 * "invalidateAotMethods/revalidateAotMethods/findAotMethods=" option
 * @param[in] action SHR_FIND_AOT_METHOTHODS when listing the specified methods
 * 					 SHR_INVALIDATE_AOT_METHOTHODS when invalidating the specified methods
 * 					 SHR_REVALIDATE_AOT_METHOTHODS when revalidating the specified methods
 ...
 * @return the number of AOT methods invalidated/revalidated/found on success or -1 on failure
 */
const IDATA SH_CacheMap::aotMethodOperationHelper(
    J9VMThread* currentThread, MethodSpecTable* specTable, IDATA numSpecs, UDATA action)
{
    IDATA rc = 0;
    ShcItem* it = NULL;
    J9InternalVMFunctions* vmFunctions = currentThread->javaVM->internalVMFunctions;
        ...
        if (TYPE_COMPILED_METHOD == ITEMTYPE(it) || TYPE_INVALIDATED_COMPILED_METHOD == ITEMTYPE(it)) {
            ...
            if (matchAotMethod(specTable, numSpecs, romClassName, romMethodName, romMethodSig)) {
                if (NULL != romMethodName) {
                    CACHEMAP_TRACE4(J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT,
                        (J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE),
                        J9NLS_SHRC_CM_PRINTSTATS_AOT_DISPLAY, ITEMJVMID(it), (UDATA)it, J9UTF8_LENGTH(romMethodName),
                        J9UTF8_DATA(romMethodName));
                }

                if (NULL != romMethodSig) {
                    CACHEMAP_TRACE3(J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT,
                        (J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE),
                        J9NLS_SHRC_CM_PRINTSTATS_AOT_DISPLAY_SIG_ADDR, J9UTF8_LENGTH(romMethodSig),
                        J9UTF8_DATA(romMethodSig), romMethod);
                }
                ...
                if (_ccHead->stale((BlockPtr)ITEMEND(it))) {
                    j9tty_printf(_portlib, " ");
                    CACHEMAP_TRACE(J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT,
                        (J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE), J9NLS_SHRC_CM_PRINTSTATS_STALE);
                }

                if ((SHR_FIND_AOT_METHOTHODS == action) && TYPE_INVALIDATED_COMPILED_METHOD == ITEMTYPE(it)) {
                    /* Do not print tag "INVALIDATED" when invalidating/revalidating, as user may be confused if this is
                     * based on the state before or after the (re)invalidate operation*/
                    j9tty_printf(_portlib, " ");
                    CACHEMAP_PRINT((J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE),
                        J9NLS_SHRC_CM_PRINTSTATS_INVALIDATED);
                }
                ...
                if (NULL != romClassName) {
                    CACHEMAP_TRACE3(J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, J9NLS_DO_NOT_PRINT_MESSAGE_TAG,
                        J9NLS_SHRC_CM_PRINTSTATS_AOT_FROM_ROMCLASS, J9UTF8_LENGTH(romClassName),
                        J9UTF8_DATA(romClassName), romClass);
                }
...
/**
 * This function returns whether an AOT method matches any of the specifications passed in by
 * "invalidateAotMethods/revalidateAotMethods/findAotMethods=" option
 *
 * @param[in] specTable The method specification table
 ...
 */
const bool SH_CacheMap::matchAotMethod(
    MethodSpecTable* specTable, IDATA numSpecs, J9UTF8* romClassName, J9UTF8* romMethodName, J9UTF8* romMethodSig)
{
    bool rc = false;
    ...
    for (i = 0; i < numSpecs; i++) {
        if ((NULL == specTable[i].className) || (UnitTest::COMPILED_METHOD_TEST == UnitTest::unitTest)) {
            /* In COMPILED_METHOD_TEST, romClassName is NULL. In order to let the method to be found, set classMatch to
             * true */
            /* Any class */
            classMatch = true;
            ...
            classMatch = (TRUE
                == wildcardMatch(specTable[i].classNameMatchFlag, (const char*)specTable[i].className,
                       specTable[i].classNameLength, (const char*)J9UTF8_DATA(romClassName),
        ...
        if (NULL == specTable[i].methodName) {
            ...
            methodMatch = (TRUE
                == wildcardMatch(specTable[i].methodNameMatchFlag, (const char*)specTable[i].methodName,
                       specTable[i].methodNameLength, (const char*)J9UTF8_DATA(romMethodName),
        ...
        if (NULL == specTable[i].methodSig) {
            ...
            if (sigLength < 0) {
                ...
                sigMatch = (TRUE
                    == wildcardMatch(specTable[i].methodSigMatchFlag, (const char*)specTable[i].methodSig,
                           specTable[i].methodSigLength, (const char*)sigStart, sigLength));
            }
        }
...
/**
 * This function fills the method specification table with specifications passed in by
 * "invalidateAotMethods/revalidateAotMethods/findAotMethods=" option
 *
 * @param[in] specTable The method specification table
 ...
 */
const IDATA SH_CacheMap::fillMethodSpecTable(MethodSpecTable* specTable, char* inputOption)
{
    ...
    do {
        if (setClass) {
            if ((',' == *curPos) || (' ' == *curPos)) {
                /* strip ' ' and ',' if they exist. It still works in case user pass in consecutive "," by mistake or
                 * there is a following white space. */
                curPos += 1;
                continue;
            } else if ('!' == *curPos) {
 ...
 */
const bool SH_CacheMap::parseWildcardMethodSpecTable(MethodSpecTable* specTable, IDATA numSpecs)
{
    bool rc = true;
    IDATA i = 0;
 ...
 * @return void
 */
void SH_CacheMap::protectPartiallyFilledPages(J9VMThread* currentThread)
{
 ...
 */
I_32 SH_CacheMap::tryAdjustMinMaxSizes(J9VMThread* currentThread, bool isJCLCall)
{
 ...
 * @return void
 */
void SH_CacheMap::updateRuntimeFullFlags(J9VMThread* currentThread)
{
    _ccHead->updateRuntimeFullFlags(currentThread);
    return;
}
 ...
 * @return void
 */
void SH_CacheMap::increaseTransactionUnstoredBytes(U_32 segmentAndDebugBytes, J9SharedClassTransaction* obj)
{
    ...
    Trc_SHR_CM_increaseTransactionUnstoredBytes_Entry(
        segmentAndDebugBytes, (UDATA)obj->classnameLength, (const char*)obj->classnameData);
 ...
 * @return void
 */
void SH_CacheMap::increaseUnstoredBytes(U_32 blockBytes, U_32 aotBytes, U_32 jitBytes)
{
    U_32 itemAndHdrBytes = 0;

    Trc_SHR_CM_increaseUnstoredBytes_Entry(blockBytes, aotBytes, jitBytes);

    if (J9_ARE_ANY_BITS_SET(
            *_runtimeFlags, J9SHR_RUNTIMEFLAG_BLOCK_SPACE_FULL | J9SHR_RUNTIMEFLAG_DENY_CACHE_UPDATES)) {
        return;
    }
    ...
    if (0 != jitBytes) {
        /* SH_CompositeCacheImpl::allocateJIT() passes SHC_DOUBLEALIGN to align parameter of
 ...
 */
void SH_CacheMap::getUnstoredBytes(
    U_32* softmxUnstoredBytes, U_32* maxAOTUnstoredBytes, U_32* maxJITUnstoredBytes) const
{
 ...
 * @param [in] overwrite Whether local startup hints _sharedClassConfig->localStartupHints will overwrite the exiting
 * one in the cache.
 */
void SH_CacheMap::updateLocalHintsData(J9VMThread* currentThread, J9SharedLocalStartupHints* localHints,
    const J9SharedStartupHintsDataDescriptor* hintsDataInCache, bool overwrite)
{
    J9SharedStartupHintsDataDescriptor updatedHintsData = { 0 };

    Trc_SHR_Assert_True(
        J9_ARE_ANY_BITS_SET(localHints->localStartupHintFlags, J9SHR_LOCAL_STARTUPHINTS_FLAG_WRITE_HINTS));
    ...
    if (J9_ARE_ALL_BITS_SET(localHints->localStartupHintFlags, J9SHR_LOCAL_STARTUPHINTS_FLAG_OVERWRITE_HEAPSIZES)) {
        if (overwrite) {
             ...
             */
            Trc_SHR_CM_updateLocalHintsData_OverwriteHeapSizes(currentThread, updatedHintsData.heapSize1,
                updatedHintsData.heapSize2, localHints->hintsData.heapSize1, localHints->hintsData.heapSize2);
        ...
        if (J9_ARE_NO_BITS_SET(updatedHintsData.flags, J9SHR_STARTUPHINTS_HEAPSIZES_SET)) {
            Trc_SHR_CM_updateLocalHintsData_WriteHeapSizes(
                currentThread, localHints->hintsData.heapSize1, localHints->hintsData.heapSize2);
