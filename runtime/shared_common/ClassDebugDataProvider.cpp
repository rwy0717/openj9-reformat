...
#define DA_TRACE2(verboseLevel, nlsFlags, var1, p1, p2) \
    if (_verboseFlags & verboseLevel)                   \
    j9nls_printf(PORTLIB, nlsFlags, var1, p1, p2)
#define DA_TRACE3(verboseLevel, nlsFlags, var1, p1, p2, p3) \
    if (_verboseFlags & verboseLevel)                       \
    j9nls_printf(PORTLIB, nlsFlags, var1, p1, p2, p3)
#define DA_TRACE4(verboseLevel, nlsFlags, var1, p1, p2, p3, p4) \
    if (_verboseFlags & verboseLevel)                           \
    j9nls_printf(PORTLIB, nlsFlags, var1, p1, p2, p3, p4)

void ClassDebugDataProvider::initialize()
{
...
bool ClassDebugDataProvider::Init(J9VMThread* currentThread, J9SharedCacheHeader* ca,
    AbstractMemoryPermission* permSetter, UDATA verboseFlags, U_64* runtimeFlags, bool startupForStats)
{
    bool retval = true;
    ...
    if (!startupForStats) {
        ...
        this->setPermission(currentThread, permSetter, getDebugAreaStartAddress(), getLNTNextAddress(),
            getLVTNextAddress(), getDebugAreaEndAddress());
    }
...
bool ClassDebugDataProvider::isOk(
    J9VMThread* currentThread, bool assertOnFailure, bool setCommitCorruptionCodes, bool assertForCorruptionCode)
{
    bool retval = true;
    UDATA startAddr = 0;
    UDATA endAddr = 0;
    ...
    if (_theca->debugRegionSize > _theca->totalBytes) {
        DA_TRACE2(J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, J9NLS_ERROR, J9NLS_SHRC_DEBUG_AREA_BAD_SIZE,
            _theca->debugRegionSize, _theca->totalBytes);
    ...
    if ((startAddr > endAddr) && (endAddr != 0)) {
        ...
        DA_TRACE2(J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, J9NLS_ERROR, J9NLS_SHRC_DEBUG_AREA_BAD_FREE_SPACE,
            startAddr, endAddr);
    ...
    if ((endAddr - startAddr) > _theca->debugRegionSize) {
        DA_TRACE3(J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, J9NLS_ERROR, J9NLS_SHRC_DEBUG_AREA_BAD_FREE_SPACE_SIZE,
            endAddr, startAddr, _theca->debugRegionSize);
    ...
    if ((startAddr > endAddr) && (endAddr != 0)) {
        DA_TRACE2(J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, J9NLS_ERROR, J9NLS_SHRC_DEBUG_AREA_BAD_LNT_HEADER_INFO,
            startAddr, endAddr);
    ...
    if (startAddr > endAddr) {
        DA_TRACE2(J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, J9NLS_ERROR, J9NLS_SHRC_DEBUG_AREA_BAD_LVT_HEADER_INFO,
            startAddr, endAddr);
    ...
    if ((startAddr > endAddr) && (endAddr != 0)) {
        ...
        if (true == setCommitCorruptionCodes) {
            DA_TRACE4(J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, J9NLS_ERROR, J9NLS_SHRC_DEBUG_AREA_BAD_SIZES_FOR_COMMIT,
                _storedLineNumberTableBytes, _storedLocalVariableTableBytes, startAddr, endAddr);
            failReason = CACHE_DEBUGAREA_BAD_SIZES_FOR_COMMIT;
            failValue = startAddr;
        }
        Trc_SHR_ClassDebugData_isOk_BadSizesForCommitCorrupt(currentThread, (UDATA)getLNTNextAddress(),
            _storedLineNumberTableBytes, (UDATA)getLVTNextAddress(), _storedLocalVariableTableBytes);
...
UDATA
ClassDebugDataProvider::getJavacoreData(
    J9JavaVM* vm, J9SharedClassJavacoreDataDescriptor* descriptor, J9SharedCacheHeader* ca)
{
    ...
    if (descriptor->debugAreaSize > 0) {
        descriptor->debugAreaUsed
            = ((descriptor->debugAreaSize - getFreeDebugSpaceBytes()) * 100) / descriptor->debugAreaSize;
    }
...
IDATA
ClassDebugDataProvider::allocateClassDebugData(J9VMThread* currentThread, U_16 classnameLength,
    const char* classnameData, const J9RomClassRequirements* sizes, J9SharedRomClassPieces* pieces,
    AbstractMemoryPermission* permSetter)
{
    IDATA retval = 0;
    bool allocatedSomething = false;
    U_64 sizeToBeAllocated = sizes->lineNumberTableSize + sizes->localVariableTableSize;

    Trc_SHR_ClassDebugData_allocateClassDebugData_Enter(
        currentThread, classnameLength, classnameData, sizes->lineNumberTableSize, sizes->localVariableTableSize);

    if (getFreeDebugSpaceBytes() < sizeToBeAllocated) {
        Trc_SHR_ClassDebugData_allocateClassDebugData_Event_NotEnoughFreeSpace(
            currentThread, classnameLength, classnameData, sizeToBeAllocated, getFreeDebugSpaceBytes());
        retval = -1;
        goto done;
    }
    ...
    if (0 < sizes->lineNumberTableSize) {
        ...
        if (NULL == pieces->lineNumberTable) {
            Trc_SHR_ClassDebugData_allocateClassDebugData_Event_FailedToAllocLineNumberTable(currentThread,
                classnameLength, classnameData, sizes->lineNumberTableSize, sizes->localVariableTableSize);
            retval = -1;
            goto done;
        } else {
            if (NULL != permSetter) {
                 ...
                 * However, if we are also adding local variable table data, then that page would anyway be marked
                 * read-write when allocating space for local variable table data.
                 */
    ...
    if (0 < sizes->localVariableTableSize) {
        ...
        if (NULL == pieces->localVariableTable) {
            Trc_SHR_ClassDebugData_allocateClassDebugData_Event_FailedToAllocLocalVariableTable(currentThread,
                classnameLength, classnameData, sizes->lineNumberTableSize, sizes->localVariableTableSize);
            retval = -1;
            goto done;
        } else {
            if (NULL != permSetter) {
                ...
                /* Mark the page as read-write where data is to be written */
                permSetter->changePartialPageProtection(
                    currentThread, (void*)((UDATA)pieces->localVariableTable + sizes->localVariableTableSize), false);
                 ...
                 * However, if we are also adding line number table data, then that page would anyway be marked
                 * read-write when allocating space for line number table data.
                 */
    ...
    Trc_SHR_ClassDebugData_allocateClassDebugData_Exit(
        currentThread, classnameLength, classnameData, pieces->localVariableTable, pieces->lineNumberTable);
    return retval;
}

void ClassDebugDataProvider::rollbackClassDebugData(
    J9VMThread* currentThread, U_16 classnameLength, const char* classnameData, AbstractMemoryPermission* permSetter)
{
    Trc_SHR_ClassDebugData_rollbackClassDebugData_Enter(
        currentThread, classnameLength, classnameData, _storedLineNumberTableBytes, _storedLocalVariableTableBytes);
    _storedLineNumberTableBytes = 0;
    _storedLocalVariableTableBytes = 0;
    ...
    Trc_SHR_ClassDebugData_rollbackClassDebugData_Exit(
        currentThread, classnameLength, classnameData, _storedLineNumberTableBytes, _storedLocalVariableTableBytes);
}

bool ClassDebugDataProvider::commitClassDebugData(
    J9VMThread* currentThread, U_16 classnameLength, const char* classnameData, AbstractMemoryPermission* permSetter)
{
    ...
    Trc_SHR_ClassDebugData_commitClassDebugData_Enter(
        currentThread, classnameLength, classnameData, _storedLineNumberTableBytes, _storedLocalVariableTableBytes);
    ...
    if (_storedLineNumberTableBytes <= 0 && _storedLocalVariableTableBytes <= 0) {
        Trc_SHR_ClassDebugData_NothingToCommit_Event(currentThread, classnameLength, classnameData,
            preCommitLineNumberTableBytes, preCommitLocalVariableTableBytes);
        goto done;
    }

    if (_storedLineNumberTableBytes > 0) {
        commitLineNumberTable();
        Trc_SHR_ClassDebugData_commitClassDebugData_LineNumberTable_Event(
            currentThread, classnameLength, classnameData, _storedLineNumberTableBytes, _storedLocalVariableTableBytes);
        commitedData = true;
    }
    if (_storedLocalVariableTableBytes > 0) {
        commitLocalVariableTable();
        Trc_SHR_ClassDebugData_commitClassDebugData_LocalVariableTable_Event(
            currentThread, classnameLength, classnameData, _storedLineNumberTableBytes, _storedLocalVariableTableBytes);
        commitedData = true;
    }
    ...
    this->setPermission(currentThread, permSetter, _lntLastUpdate, lntNextAddr, lvtNextAddr, _lvtLastUpdate);
    ...
    Trc_SHR_ClassDebugData_commitClassDebugData_Exit(
        currentThread, classnameLength, classnameData, preCommitLineNumberTableBytes, preCommitLocalVariableTableBytes);
    return commitedData;
}

bool ClassDebugDataProvider::isEnoughFreeSpace(UDATA size)
{
...
void* ClassDebugDataProvider::getNextLineNumberTable(UDATA size)
{
    UDATA retval = 0;
...
void* ClassDebugDataProvider::getNextLocalVariableTable(UDATA size)
{
    UDATA retval = 0;
...
void ClassDebugDataProvider::commitLineNumberTable()
{
...
void ClassDebugDataProvider::commitLocalVariableTable()
{
    UDATA newdatastart = 0;
...
U_32 ClassDebugDataProvider::getDebugDataSize()
{
...
U_32 ClassDebugDataProvider::getFreeDebugSpaceBytes()
{
    UDATA freeStart = 0;
    UDATA freeEnd = 0;
...
U_32 ClassDebugDataProvider::getLineNumberTableBytes()
{
    UDATA dataStart = 0;
    UDATA dataEnd = 0;
...
U_32 ClassDebugDataProvider::getLocalVariableTableBytes()
{
    UDATA dataStart = 0;
    UDATA dataEnd = 0;
...
bool ClassDebugDataProvider::HeaderInit(J9SharedCacheHeader* ca, U_32 size)
{
    UDATA startaddr = 0;
    UDATA endaddr = 0;
...
U_32 ClassDebugDataProvider::getRecommendedPercentage()
{
...
U_32 ClassDebugDataProvider::recommendedSize(U_32 freeBlockBytesInCache, U_32 align)
{
#if defined(CREATE_CLASS_DEBUG_AREA)
 ...
 * @return void
 */
void ClassDebugDataProvider::setPermission(J9VMThread* currentThread, AbstractMemoryPermission* permSetter,
    void* lntProtectLow, void* lntProtectHigh, void* lvtProtectLow, void* lvtProtectHigh, bool readOnly)
{
    UDATA pageSize = this->_theca->osPageSize;
    PORT_ACCESS_FROM_JAVAVM(currentThread->javaVM);

    Trc_SHR_ClassDebugData_setPermission_Enter(
        currentThread, permSetter, lntProtectLow, lntProtectHigh, lvtProtectLow, lvtProtectHigh, readOnly);
    ...
    if (pageSize > 0) {
        ...
        if (lntProtectHigh == lvtProtectLow) {
            ...
            if (totalRoundedSize > 0) {
                if (permSetter->setRegionPermissions(PORTLIB, (void*)startMprotect, totalRoundedSize, pageProtectFlag)
                    != 0) {
                    Trc_SHR_ClassDebugData_setPermission_CouldNotSetAllDebugData_Event(
                        currentThread, (void*)startMprotect, (void*)endMprotect, totalRoundedSize, pageProtectFlag);
                    Trc_SHR_Assert_ShouldNeverHappen();
                } else {
                    Trc_SHR_ClassDebugData_setPermission_SetAllDebugData_Event(
                        currentThread, (void*)startMprotect, (void*)endMprotect, totalRoundedSize, pageProtectFlag);
                    if (doVerbosePages) {
                        j9tty_printf(PORTLIB,
                            "Set memory region permissions in ClassDebugDataProvider::commitClassDebugData() for debug "
                            "data addresses %p to %p - for %d bytes to %zu\n",
            ...
            if (lntRoundedSize != 0) {
                if (permSetter->setRegionPermissions(
                        PORTLIB, (void*)roundLntProtectLow, lntRoundedSize, pageProtectFlag)
                    != 0) {
                    Trc_SHR_ClassDebugData_setPermission_CouldNotSetLineNumberTableData_Event(currentThread,
                        (void*)roundLntProtectLow, (void*)roundLntProtectHigh, lntRoundedSize, pageProtectFlag);
                    Trc_SHR_Assert_ShouldNeverHappen();
                } else {
                    Trc_SHR_ClassDebugData_setPermission_SetLineNumberTableData_Event(currentThread,
                        (void*)roundLntProtectLow, (void*)roundLntProtectHigh, lntRoundedSize, pageProtectFlag);
                    if (doVerbosePages) {
                        j9tty_printf(PORTLIB,
                            "Set memory region permissions in ClassDebugDataProvider::commitClassDebugData() for "
                            "LineNumberTable addresses %p to %p - for %d bytes to %zu\n",
                            (void*)roundLntProtectLow, (void*)roundLntProtectHigh, lntRoundedSize, pageProtectFlag);
                    }
                }
            }
            ...
            if (lvtRoundedSize != 0) {
                if (permSetter->setRegionPermissions(
                        PORTLIB, (void*)roundLvtProtectLow, lvtRoundedSize, pageProtectFlag)
                    != 0) {
                    Trc_SHR_ClassDebugData_setPermission_CouldNotSetLocalVariableTableData_Event(currentThread,
                        (void*)roundLvtProtectLow, (void*)roundLvtProtectHigh, lvtRoundedSize, pageProtectFlag);
                    Trc_SHR_Assert_ShouldNeverHappen();
                } else {
                    Trc_SHR_ClassDebugData_setPermission_SetLocalVariableTableData_Event(currentThread,
                        (void*)roundLvtProtectLow, (void*)roundLvtProtectHigh, lvtRoundedSize, pageProtectFlag);
                    if (doVerbosePages) {
                        j9tty_printf(PORTLIB,
                            "Set memory region permissions in ClassDebugDataProvider::commitClassDebugData() for "
                            "LocalVariableTable addresses %p to %p - for %d bytes to %zu\n",
                            (void*)roundLvtProtectLow, (void*)roundLvtProtectHigh, lvtRoundedSize, pageProtectFlag);
                    }
                }
            }
    ...
    Trc_SHR_ClassDebugData_setPermission_Exit(
        currentThread, permSetter, lntProtectLow, lntProtectHigh, lvtProtectLow, lvtProtectHigh, readOnly);
}

bool ClassDebugDataProvider::processUpdates(J9VMThread* currentThread, AbstractMemoryPermission* permSetter)
{
    ...
    if (_lntLastUpdate != lntCurrent) {
        foundUpdate = true;
        Trc_SHR_ClassDebugData_processUpdates_FoundNewLineNumData(
            currentThread, permSetter, ((UDATA)lntCurrent - (UDATA)_lntLastUpdate));
    }

    if (_lvtLastUpdate != lvtCurrent) {
        foundUpdate = true;
        Trc_SHR_ClassDebugData_processUpdates_FoundNewLocalVarData(
            currentThread, permSetter, ((UDATA)_lvtLastUpdate - (UDATA)lvtCurrent));
    }
...
void* ClassDebugDataProvider::getLNTNextAddress() { return WSRP_GET(_theca->lineNumberTableNextSRP, void*); }

void* ClassDebugDataProvider::getLVTNextAddress() { return WSRP_GET(_theca->localVariableTableNextSRP, void*); }

void ClassDebugDataProvider::setLNTNextAddress(J9SharedCacheHeader* ca, void* addr)
{
    UDATA newValue = ((UDATA)addr) - ((UDATA) & (ca->lineNumberTableNextSRP));
    ca->lineNumberTableNextSRP = newValue;
}

void ClassDebugDataProvider::setLVTNextAddress(J9SharedCacheHeader* ca, void* addr)
{
    UDATA newValue = ((UDATA)addr) - ((UDATA) & (ca->localVariableTableNextSRP));
    ca->localVariableTableNextSRP = newValue;
}

void ClassDebugDataProvider::updateLNTWithSize(UDATA size)
{
    _theca->lineNumberTableNextSRP = _theca->lineNumberTableNextSRP + size;
}

void ClassDebugDataProvider::updateLVTWithSize(UDATA size)
{
 ...
 * @return void
 */
void ClassDebugDataProvider::protectUnusedPages(J9VMThread* currentThread, AbstractMemoryPermission* permSetter)
{
 ...
 * @return void
 */
void ClassDebugDataProvider::unprotectUnusedPages(J9VMThread* currentThread, AbstractMemoryPermission* permSetter)
{
    void* lntStartAddr = getLNTNextAddress();
    void* lvtStartAddr = getLVTNextAddress();
 ...
 * @return void
 */
void ClassDebugDataProvider::protectPartiallyFilledPages(
    J9VMThread* currentThread, AbstractMemoryPermission* permSetter, bool phaseCheck)
{
    void* lntStartAddr = getLNTNextAddress();
    void* lvtStartAddr = getLVTNextAddress();
 ...
 * @return void
 */
void ClassDebugDataProvider::unprotectPartiallyFilledPages(
    J9VMThread* currentThread, AbstractMemoryPermission* permSetter, bool phaseCheck)
{
    void* lntStartAddr = getLNTNextAddress();
    void* lvtStartAddr = getLVTNextAddress();
