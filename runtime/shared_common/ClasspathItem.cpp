...
ClasspathEntryItem* ClasspathEntryItem::newInstance(
    const char* path, U_16 pathLen, UDATA protocol, ClasspathEntryItem* memForConstructor)
{
...
ClasspathEntryItem::BlockPtr ClasspathEntryItem::writeToAddress(BlockPtr block)
{
...
const char* ClasspathEntryItem::getPath(U_16* pathLen_) const
{
    if (pathLen_) {
...
U_32 ClasspathEntryItem::getSizeNeeded(void) const
{
...
const char* ClasspathEntryItem::getLocation(U_16* locationPathLen_) const
{
    if (locationPathLen_) {
...
void ClasspathEntryItem::cleanup(void) { /* Currently no cleanup */ }

ClasspathItem* ClasspathItem::newInstance(
    J9JavaVM* vm, IDATA entries_, IDATA helperID_, U_16 cpType_, ClasspathItem* memForConstructor)
{
...
void ClasspathItem::initialize(J9JavaVM* vm, IDATA entries_, IDATA helperID_, U_16 cpType_, BlockPtr memForConstructor)
{
    type = cpType_;
    flags = 0;
    entries = entries_;
    ...
    for (IDATA i = 0; i < entries_; i++) {
        items[i] = (ClasspathEntryItem*)(((UDATA)items)
            + ((entries_ * sizeof(ClasspathEntryItem*)) + (sizeof(ClasspathEntryItem) * i)));
    }
...
ClasspathItem::addItem(J9InternalVMFunctions* functionTable, const char* path, U_16 pathLen, UDATA protocol)
{
    ...
    if (entries == itemsAdded) {
        /* Cannot access verbose level, so this is not surpressed by "silent". However, it's a "should never happen"
         * message. */
...
ClasspathEntryItem* ClasspathItem::itemAt(I_16 i) const
{
...
I_16 ClasspathItem::find(J9InternalVMFunctions* functionTable, ClasspathEntryItem* test) const
{
 ...
 * we're interested in.
 */
I_16 ClasspathItem::find(J9InternalVMFunctions* functionTable, ClasspathEntryItem* test, I_16 stopAtIndex) const
{
    I_16 countFrom = 0;
...
IDATA
ClasspathItem::getMaxItems(void) const { return entries; }

I_16 ClasspathItem::getItemsAdded(void) const { return (I_16)itemsAdded; }
...
U_32 ClasspathItem::getSizeNeeded(void) const
{
...
IDATA
ClasspathItem::getFirstDirIndex(void) const { return firstDirIndex; }

UDATA
ClasspathItem::getHashCode(void) const { return hashValue; }

UDATA
ClasspathItem::getType(void) const { return type; }

bool ClasspathItem::isInCache(void) const { return ((flags & IS_IN_CACHE_FLAG) != 0); }

void ClasspathItem::setJarsLockedToIndex(I_16 i)
{
    if (!(flags & IS_IN_CACHE_FLAG)) {
        jarsLockedToIndex = i;
    }
...
I_16 ClasspathItem::getJarsLockedToIndex(void) const
{
    if (!(flags & IS_IN_CACHE_FLAG)) {
...
void ClasspathItem::cleanup(void)
{
    if (!(flags & IS_IN_CACHE_FLAG)) {
        if (items) {
