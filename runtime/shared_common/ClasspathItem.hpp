...
public:
     ...
     */
    static ClasspathEntryItem* newInstance(
     ...
     * If the ClasspathEntryItem is a nested jar, getPath() returns a path string including the entry inside the
     * external jar, while getLocation() returns a path string including the external jar only. e.g. For
     * /path/A.jar!/lib/B.jar, getPath() returns "/path/A.jar!/lib/B.jar", getLocation() returns "/path/A.jar"
     *
     * If object in memory, returns private path field.
     * If object in cache, returns path in cache.
     *
...
public:
     ...
     */
    static ClasspathItem* newInstance(
        J9JavaVM* vm, IDATA entries, IDATA fromHelperID, U_16 cpType, ClasspathItem* memForConstructor);
     ...
     */
    static bool compare(
        J9InternalVMFunctions* vmFunctionTable, ClasspathEntryItem* test, ClasspathEntryItem* compareTo);
