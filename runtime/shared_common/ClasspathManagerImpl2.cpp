...
#define CPM_ZIP_ONLY_TIMESTAMP_ON_INIT                                                                               \
    4 /* Indicates that the timestamp of this container should only be checked once during initialization to support \
         OSGi requirements */
 ...
 */
SH_ClasspathManagerImpl2::CpLinkedListImpl* SH_ClasspathManagerImpl2::CpLinkedListImpl::newInstance(
    I_16 CPEIndex_, const ShcItem* item_, SH_CompositeCache* cachelet_, CpLinkedListImpl* memForConstructor)
{
...
void SH_ClasspathManagerImpl2::CpLinkedListImpl::initialize(
    I_16 CPEIndex_, const ShcItem* item_, SH_CompositeCache* cachelet_)
{
 ...
 */
SH_ClasspathManagerImpl2::CpLinkedListImpl* SH_ClasspathManagerImpl2::CpLinkedListImpl::link(
    CpLinkedListImpl* addToList, I_16 CPEIndex, const ShcItem* item, bool doTag, SH_CompositeCache* cachelet,
    J9Pool* allocationPool)
{
...
/**
 * Searches for a CpLinkedListImpl in the list which belongs to a classpath which is an exact match for the classpath
 ...
 */
SH_ClasspathManagerImpl2::CpLinkedListImpl* SH_ClasspathManagerImpl2::CpLinkedListImpl::forCacheItem(
    J9VMThread* currentThread, ClasspathItem* item, UDATA index)
{
    ...
    do {
        ...
        if ((testIndex == index) && (testItem->staleFromIndex == CPW_NOT_STALE)
            && (ClasspathItem::compare(currentThread->javaVM->internalVMFunctions, ((ClasspathItem*)CPWDATA(testItem)),
                   item))) { /* Note: Compares whole classpath */
 ...
 */
void SH_ClasspathManagerImpl2::CpLinkedListImpl::tag(CpLinkedListImpl* item)
{
 ...
 */
I_16 SH_ClasspathManagerImpl2::CpLinkedListImpl::getCPEIndex(CpLinkedListImpl* item)
{
...
SH_ClasspathManagerImpl2::CpLinkedListHdr::CpLinkedListHdr(
    const char* key_, U_16 keySize_, U_8 isToken_, CpLinkedListImpl* listItem)
    : _isToken(isToken_)
    , _flags(0)
    , _keySize(keySize_)
    , _key(key_)
 ...
 */
SH_ClasspathManagerImpl2* SH_ClasspathManagerImpl2::newInstance(
    J9JavaVM* vm, SH_SharedCache* cache_, SH_TimestampManager* tsm_, SH_ClasspathManagerImpl2* memForConstructor)
{
...
void SH_ClasspathManagerImpl2::initialize(
    J9JavaVM* vm, SH_SharedCache* cache_, SH_TimestampManager* tsm_, BlockPtr memForConstructor)
{
...
U_32 SH_ClasspathManagerImpl2::getHashTableEntriesFromCacheSize(UDATA cacheSizeBytes)
{
...
SH_ClasspathManagerImpl2::localInitializePools(J9VMThread* currentThread)
{
    ...
    _linkedListImplPool = pool_new(sizeof(SH_ClasspathManagerImpl2::CpLinkedListImpl), 0, 0, 0, J9_GET_CALLSITE(),
...
void SH_ClasspathManagerImpl2::localTearDownPools(J9VMThread* currentThread)
{
...
void SH_ClasspathManagerImpl2::localPostCleanup(J9VMThread* currentThread)
{
 ...
 * Returns 1 if item is stampable and time has changed, 0 if it has not changed, JAR_LOCKED if it was not checked and -1
 * for error
 */
IDATA
SH_ClasspathManagerImpl2::hasTimestampChanged(
    J9VMThread* currentThread, ClasspathEntryItem* itemToCheck, CpLinkedListHdr* knownLLH, bool doTryLockJar)
{
    ...
    if ((PROTO_JAR == itemToCheck->protocol) || (PROTO_JIMAGE == itemToCheck->protocol)) {
        U_16 pathLen = 0;
        ...
        if ((header->_flags == CPM_ZIP_OPEN)
            || /* Implies !CPM_ZIP_ONLY_TIMESTAMP_ON_INIT and !CPM_ZIP_FORCE_CHECK_TIMESTAMP */
        ...
        if (header->_flags & CPM_ZIP_ONLY_TIMESTAMP_ON_INIT) {
            ...
            header->_flags |= CPM_ZIP_CHECKED_INIT_TIMESTAMP;
            /* If the container has just been opened, force a single check each time, unless
             * CPM_ZIP_ONLY_TIMESTAMP_ON_INIT */
        ...
        if ((newTS == TIMESTAMP_DOES_NOT_EXIST) || (newTS == TIMESTAMP_DISAPPEARED)) {
            UDATA result
                = (newTS == TIMESTAMP_DISAPPEARED); /* If resource was there and has gone, timestamp has changed */
...
J9HashTable* SH_ClasspathManagerImpl2::localHashTableCreate(J9VMThread* currentThread, U_32 initialEntries)
{
    J9HashTable* returnVal;

    Trc_SHR_CMI_localHashTableCreate_Entry(currentThread, initialEntries);
    returnVal = hashTableNew(OMRPORT_FROM_J9PORT(_portlib), J9_GET_CALLSITE(), initialEntries,
        sizeof(SH_ClasspathManagerImpl2::CpLinkedListHdr), sizeof(char*), 0, J9MEM_CATEGORY_CLASSES,
        SH_ClasspathManagerImpl2::cpeHashFn, SH_ClasspathManagerImpl2::cpeHashEqualFn, NULL,
        (void*)currentThread->javaVM->internalVMFunctions);
...
SH_ClasspathManagerImpl2::cpeHashFn(void* item, void* userData)
{
    ...
    hashValue
        = (internalFunctionTable->computeHashForUTF8((U_8*)itemValue->_key, itemValue->_keySize) + itemValue->_isToken);
...
SH_ClasspathManagerImpl2::CpLinkedListHdr* SH_ClasspathManagerImpl2::cpeTableAddHeader(
    J9VMThread* currentThread, const char* name, U_16 nameLen, CpLinkedListImpl* newItem, U_8 isToken)
{
...
/* Creates a new header and a new linked list entry, which is added to the header. The new header is then added to the
 * hashtable. CPEIndex and doTag are parameters stored by the link and name and isToken are parameters used by the
 * header. Returns a pointer to the new link, or NULL if there was an error */
SH_ClasspathManagerImpl2::CpLinkedListImpl* SH_ClasspathManagerImpl2::cpeTableAdd(J9VMThread* currentThread,
    const char* name, U_16 nameLen, I_16 CPEIndex, const ShcItem* item, U_8 isToken, bool doTag,
    SH_CompositeCache* cachelet)
{
    ...
    /* Note that it is legitimate for item to be NULL if we're being notified about a container opening which we have no
     * cache data for yet */
    if (item != NULL) {
...
/* Finds a header entry in the hashtable, based on the data given. Will return NULL if the header is not found or if
 * there is an error. */
SH_ClasspathManagerImpl2::CpLinkedListHdr* SH_ClasspathManagerImpl2::cpeTableLookup(
    J9VMThread* currentThread, const char* key, U_16 keySize, U_8 isToken)
{
...
/* Finds a header entry in the hashtable, based on the data given. Will return NULL if the header is not found or if
 * there is an error. */
SH_ClasspathManagerImpl2::CpLinkedListHdr* SH_ClasspathManagerImpl2::cpeTableLookupHelper(
    J9VMThread* currentThread, const char* key, U_16 keySize, U_8 isToken)
{
    CpLinkedListHdr dummy(key, keySize, isToken, NULL);
    CpLinkedListHdr* returnVal = NULL;
...
SH_ClasspathManagerImpl2::CpLinkedListHdr* SH_ClasspathManagerImpl2::cpeTableLookupHelper(
    J9VMThread* currentThread, CpLinkedListHdr* searchItem)
{
    CpLinkedListHdr* returnVal = NULL;
...
SH_ClasspathManagerImpl2::CpLinkedListImpl* SH_ClasspathManagerImpl2::cpeTableUpdate(J9VMThread* currentThread,
    const char* name, U_16 nameLen, I_16 CPEIndex, const ShcItem* item, U_8 isToken, bool doTag,
    SH_CompositeCache* cachelet)
{
 ...
 */
ClasspathWrapper* SH_ClasspathManagerImpl2::localUpdate_FindIdentified(
    J9VMThread* currentThread, ClasspathItem* localCP)
{
    ClasspathWrapper* found = NULL;

    Trc_SHR_CMI_localUpdate_FindIdentified_Entry(currentThread, localCP);

    if (_cache->enterLocalMutex(currentThread, _identifiedMutex, "identifiedMutex", "localUpdate_FindIdentified")
        == 0) {
        if (testForClasspathReset(currentThread)) {
            found = (ClasspathWrapper*)getIdentifiedClasspath(
                currentThread, _identifiedClasspaths, localCP->getHelperID(), localCP->getItemsAdded(), NULL, 0, NULL);
        }
 ...
 */
IDATA
SH_ClasspathManagerImpl2::local_StoreIdentified(
    J9VMThread* currentThread, ClasspathItem* localCP, ClasspathWrapper* cpInCache)
{
    ...
    if (_cache->enterLocalMutex(currentThread, _identifiedMutex, "identifiedMutex", "local_StoreIdentified") == 0) {
        if (testForClasspathReset(currentThread)) {
            setIdentifiedClasspath(currentThread, &_identifiedClasspaths, localCP->getHelperID(),
                localCP->getItemsAdded(), NULL, 0, cpInCache);
        }
        ...
        if ((_identifiedClasspaths == NULL) || (_identifiedClasspaths->size == 0)) {
            /* Now that we can choose to make the identified array NULL if we exceed a number of classpaths, this is no
             * longer an error condition */
 ...
 */
ClasspathWrapper* SH_ClasspathManagerImpl2::localUpdate_CheckManually(
    J9VMThread* currentThread, ClasspathItem* cp, CpLinkedListHdr** knownLLH)
{
 ...
 */
IDATA
SH_ClasspathManagerImpl2::update(
    J9VMThread* currentThread, ClasspathItem* localCP, I_16 cpeIndex, ClasspathWrapper** foundCP)
{
    I_16 i;
    ClasspathWrapper* found = NULL;
     ...
     */
    if (found && (localCP->getType() != CP_TYPE_TOKEN)
        && (*_runtimeFlagsPtr & J9SHR_RUNTIMEFLAG_ENABLE_TIMESTAMP_CHECKS)) {
    ...
    /* If we found the classpath in the cache, it wasn't stale and it hasn't already been identified, add it to the
     * identified array */
    if ((*foundCP && !foundIdentified) && (localCP->getType() == CP_TYPE_CLASSPATH)
        && (*_runtimeFlagsPtr & J9SHR_RUNTIMEFLAG_ENABLE_LOCAL_CACHEING)) {
 ...
 */
IDATA
SH_ClasspathManagerImpl2::localValidate_FindIdentified(
    J9VMThread* currentThread, ClasspathWrapper* cpInCache, IDATA walkFromID)
{
    IDATA identifiedID = ID_NOT_FOUND;

    Trc_SHR_CMI_localValidate_FindIdentified_Entry(currentThread, cpInCache);

    if (_cache->enterLocalMutex(currentThread, _identifiedMutex, "identifiedMutex", "localValidate_FindIdentified")
        == 0) {
        if (testForClasspathReset(currentThread)) {
 ...
 * May try to "identify" a classpath if it seems worthwhile. Sets *addToIdentified=true if it successfully matches the
 * classpath with a classpath in the cache. As part of the validation, timestamps appropriate entries and sets
 * *staleItem if it finds a stale item. Returns the index in the caller's classpath (compareTo) that the ROMClass is
 * found at, or -1 if not found.
 */
I_16 SH_ClasspathManagerImpl2::localValidate_CheckAndTimestampManually(J9VMThread* currentThread,
    ClasspathWrapper* cpInCache, I_16 testCPIndex, ClasspathItem* compareTo, IDATA foundIdentified,
    bool* addToIdentified, ClasspathEntryItem** staleItem)
{
    ...
    *staleItem = NULL;
    ...
    if ((indexInCompare < 0) || (indexInCompare > testCPIndex)) {
        if (!(foundIdentified & (ID_NOT_FOUND | ID_NOT_SET)) && (compareTo->getType() == CP_TYPE_CLASSPATH)
            && (*_runtimeFlagsPtr & J9SHR_RUNTIMEFLAG_ENABLE_LOCAL_CACHEING)) {
            Trc_SHR_CMI_localValidate_CheckAndTimestampManually_RegisterFailed(currentThread);
            registerFailedMatch(
                currentThread, _identifiedClasspaths, compareTo->getHelperID(), foundIdentified, testCPIndex, NULL, 0);
        }
     ...
     * Yes if classpaths have same hashcode and number of items, if testItem is at same index in both and if classpath
     * is not part-stale Note: Hashcode is no guarantee of equality.
     */
    if (compareTo->getType() == CP_TYPE_CLASSPATH && (testCPI->getType() == CP_TYPE_CLASSPATH)
        && (*_runtimeFlagsPtr & J9SHR_RUNTIMEFLAG_ENABLE_LOCAL_CACHEING) && (indexInCompare == testCPIndex)
    ...
    for (I_16 i = walkToIndex; i >= 0; i--) {
        ...
        if ((indexInTestCP < 0)) {
             ...
             */
            /* doTryLockJar = false for non-boostrap because we are trying to load from the cache, not from disk, so
               JARs may not yet be locked by the classloader.
    ...
    *addToIdentified = doTryIdentify;
    Trc_SHR_CMI_localValidate_CheckAndTimestampManually_ExitSuccess(
        currentThread, indexInCompare, (*addToIdentified), (*staleItem));
 ...
 * Note also that the function will also return 0 if the entries in compareTo are not confirmed upto and including
 * foundAtIndex
 *
 * @see ClasspathManager.hpp
 ...
 * @param[in] confirmedEntries The entries in the classpath which have been confirmed. Only applies to TYPE_CLASSPATH,
 * otherwise should be -1.
 * @param[out] foundAtIndex Set to the index in the caller's classpath that foundROMClass is found at. If not found, it
 * will be -1.
 * @param[out] staleItem Set if a stale classpath entry is found. It is then the caller's responsibility to do a stale
 * mark as we only own the read mutex.
 *
 * @return 	1 only if the ROMClass found is the same ROMClass which would be found on the filesystem using the given
 * classpath.
 * @return 0 if classpaths do not match, if a stale entry is detected or if the classpath has not been confirmed to the
 * required index
 * @return -1 in the case of an error.
 */
IDATA
SH_ClasspathManagerImpl2::validate(J9VMThread* currentThread, ROMClassWrapper* foundROMClass, ClasspathItem* compareTo,
    IDATA confirmedEntries, I_16* foundAtIndex, ClasspathEntryItem** staleItem)
{
    ...
    if (getState() != MANAGER_STATE_STARTED) {
        ...
        Trc_SHR_CMI_validate_ManagerInNotStartedState_Exception(
            currentThread, foundROMClass, compareTo, confirmedEntries);
    ...
    if ((compareTo->getType() == CP_TYPE_CLASSPATH) && (testCPI->getType() == CP_TYPE_CLASSPATH)
        ...
        do {
            if ((foundIdentified = localValidate_FindIdentified(currentThread, testCP, foundIdentified + 1))
                != ID_NOT_FOUND) {
        ...
        if (foundIdentified == compareToID) {
            ...
            /* At this point, foundIdentified will always be ID_NOT_FOUND. This means that testCP is not exactly the
             * same classpath as the caller classloader's, but we have identified it and it could still be a valid
             * match. Test to see if we have tried this match before and failed it. */
            foundIdentified = prevMatch;
            if (foundIdentified != ID_NOT_FOUND) {
                if (hasMatchFailedBefore(
                        currentThread, _identifiedClasspaths, compareToID, foundIdentified, testCPIndex, NULL, 0)) {
                    /* trace event is at level 1 and trace exit message is at level 2 as per CMVC 155318/157683 */
                    Trc_SHR_CMI_validate_ExitFailedBefore_Event(
                        currentThread, foundROMClass, compareTo, confirmedEntries);
     ...
     * foundIdentified==compareToID: We have either found that testCP and compareTo match exactly - we need to simply
     * timestamp check. foundIdentified!=NULL: TestCP is a different classpath to the caller's which needs to be tested
     * to see if it is a valid match. foundIdentified==NULL: TestCP has not been identified. It may be identified later,
     * or there may not be a helper with that classpath in the VM.
     */
    if (localFoundAtIndex == -1) {
        /* We didn't find an exact classpath match. Walk the classpaths and check for validity. Note that the timestamp
         * check is incorporated for efficiency. */
        localFoundAtIndex = localValidate_CheckAndTimestampManually(
            currentThread, testCP, testCPIndex, compareTo, foundIdentified, &addToIdentified, staleItem);
    } else {
        /* If we found an identified classpath, check timestamps on the entries from jarsLockedToIndex upto and
         * including localFoundAtIndex */
        if ((compareTo->getType() != CP_TYPE_TOKEN)
            && (*_runtimeFlagsPtr & J9SHR_RUNTIMEFLAG_ENABLE_TIMESTAMP_CHECKS)) {
                ...
                if (tsChangedVal == JAR_LOCKED && i == jarsLockedToIndex + 1) {
                    ...
                    *staleItem = foundItem;
                    if (_cache->getCompositeCacheAPI()->isRunningReadOnly()) {
                        /* If we're running in read-only mode, disable sharing for this local classpath as it's too
                         * expensive to keep checking */
    ...
    /* If the classpath has just been identified by localValidate_CheckAndTimestampManually, store it in the identified
     * array */
    if (addToIdentified && (compareTo->getType() == CP_TYPE_CLASSPATH)
        && (*_runtimeFlagsPtr & J9SHR_RUNTIMEFLAG_ENABLE_LOCAL_CACHEING)) {
...
_done:
    ...
    if ((confirmedEntries >= 0) && (localFoundAtIndex >= confirmedEntries)) {
        /* trace event is at level 1 and trace exit message is at level 2 as per CMVC 155318/157683  */
        Trc_SHR_CMI_validate_ExitNotConfirmed_Event(
            currentThread, localFoundAtIndex, confirmedEntries, foundROMClass, compareTo);
 ...
 */
void SH_ClasspathManagerImpl2::setTimestamps(J9VMThread* currentThread, ClasspathWrapper* wrapperInCache)
{
    ...
    for (I_16 i = 0; i < cpi->getItemsAdded(); i++) {
        ...
        if ((PROTO_JAR == current->protocol) || (PROTO_JIMAGE == current->protocol)) {
 ...
 */
bool SH_ClasspathManagerImpl2::storeNew(
    J9VMThread* currentThread, const ShcItem* itemInCache, SH_CompositeCache* cachelet)
{
 ...
 */
void SH_ClasspathManagerImpl2::markClasspathsStale(J9VMThread* currentThread, ClasspathEntryItem* cpei)
{
 ...
 */
bool SH_ClasspathManagerImpl2::isStale(ClasspathWrapper* cpw)
{
 ...
 */
bool SH_ClasspathManagerImpl2::touchForClassFiles(
    J9VMThread* currentThread, const char* className, UDATA classNameLen, ClasspathItem* cp, I_16 toIndex)
{
    ...
    for (i = 0; i < toIndex; i++) {
        ...
        if (current->protocol == PROTO_DIR) {
            ...
            if (SH_CacheMap::createPathString(currentThread, _cache->getSharedClassConfig(), &pathBufPtr,
                    SHARE_PATHBUF_SIZE, current, className, classNameLen, &doFreeBuffer)) {
...
void SH_ClasspathManagerImpl2::notifyClasspathEntryStateChange(
    J9VMThread* currentThread, const J9UTF8* path, UDATA newState)
{
    CpLinkedListHdr* header;
...
void SH_ClasspathManagerImpl2::getNumItemsByType(UDATA* numClasspaths, UDATA* numURLs, UDATA* numTokens)
{
 ...
 */
IDATA
SH_ClasspathManagerImpl2::createHintsForCachelet(
    J9VMThread* vmthread, SH_CompositeCache* cachelet, CacheletHints* hints)
{
 ...
 */
IDATA
SH_ClasspathManagerImpl2::primeHashtables(
    J9VMThread* vmthread, SH_CompositeCache* cachelet, U_8* hintsData, UDATA dataLength)
{
