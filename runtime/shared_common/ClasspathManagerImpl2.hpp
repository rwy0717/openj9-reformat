...
public:
    ...
    static SH_ClasspathManagerImpl2* newInstance(
        J9JavaVM* vm, SH_SharedCache* cache, SH_TimestampManager* tsm, SH_ClasspathManagerImpl2* memForConstructor);
...
private:
    ...
    public:
        static CpLinkedListImpl* newInstance(
            I_16 CPEIndex, const ShcItem* item, SH_CompositeCache* cachelet, CpLinkedListImpl* memForConstructor);
        static CpLinkedListImpl* link(CpLinkedListImpl* addToList, I_16 CPEIndex, const ShcItem* item, bool doTag,
            SH_CompositeCache* cachelet, J9Pool* allocationPool);
    ...
    CpLinkedListHdr* cpeTableAddHeader(
        J9VMThread* currentThread, const char* name, U_16 nameLen, CpLinkedListImpl* newItem, U_8 isToken);
    CpLinkedListImpl* cpeTableAdd(J9VMThread* currentThread, const char* name, U_16 nameLen, I_16 CPEIndex,
        const ShcItem* item, U_8 isToken, bool doTag, SH_CompositeCache* cachelet);

    CpLinkedListImpl* cpeTableUpdate(J9VMThread* currentThread, const char* name, U_16 nameLen, I_16 CPEIndex,
        const ShcItem* item, U_8 isToken, bool doTag, SH_CompositeCache* cachelet);
    ...
    ClasspathWrapper* localUpdate_CheckManually(
        J9VMThread* currentThread, ClasspathItem* cp, CpLinkedListHdr** knownLLH);

    I_16 localValidate_CheckAndTimestampManually(J9VMThread* currentThread, ClasspathWrapper* cpInCache,
        I_16 testCPIndex, ClasspathItem* compareTo, IDATA foundIdentified, bool* addToIdentified,
        ClasspathEntryItem** staleItem);

    IDATA hasTimestampChanged(
