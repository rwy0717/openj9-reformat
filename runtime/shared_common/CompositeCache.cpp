 ...
 *                                                          <--- free --------->
 * ^                             ^                         ^                   ^               ^                   ^
 * _theca				   readWriteSRP                segmentSRP          updateSRP      CADEBUGSTART
 * CAEND
 */
...
#define CC_TRACE(verboseLevel, nlsFlags, var1) \
    if (_verboseFlags & verboseLevel)          \
    j9nls_printf(PORTLIB, nlsFlags, var1)
#define CC_TRACE1(verboseLevel, nlsFlags, var1, p1) \
    if (_verboseFlags & verboseLevel)               \
    j9nls_printf(PORTLIB, nlsFlags, var1, p1)
#define CC_TRACE2(verboseLevel, nlsFlags, var1, p1, p2) \
    if (_verboseFlags & verboseLevel)                   \
    j9nls_printf(PORTLIB, nlsFlags, var1, p1, p2)
#define CC_ERR_TRACE(var) \
    if (_verboseFlags)    \
    j9nls_printf(PORTLIB, J9NLS_ERROR, var)
#define CC_ERR_TRACE1(var, p1) \
    if (_verboseFlags)         \
    j9nls_printf(PORTLIB, J9NLS_ERROR, var, p1)
#define CC_ERR_TRACE2(var, p1, p2) \
    if (_verboseFlags)             \
    j9nls_printf(PORTLIB, J9NLS_ERROR, var, p1, p2)
#define CC_WARNING_TRACE(var, isJCLCall) \
    if (_verboseFlags && !isJCLCall)     \
    j9nls_printf(PORTLIB, J9NLS_WARNING, var)
#define CC_WARNING_TRACE1(var, p1, isJCLCall) \
    if (_verboseFlags && !isJCLCall)          \
    j9nls_printf(PORTLIB, J9NLS_WARNING, var, p1)
#define CC_INFO_TRACE(var, isJCLCall) \
    if (_verboseFlags && !isJCLCall)  \
    j9nls_printf(PORTLIB, J9NLS_INFO, var)
#define CC_INFO_TRACE1(var, p1, isJCLCall) \
    if (_verboseFlags && !isJCLCall)       \
    j9nls_printf(PORTLIB, J9NLS_INFO, var, p1)
#define CC_INFO_TRACE3(var, p1, p2, p3) \
    if (_verboseFlags)                  \
    j9nls_printf(PORTLIB, J9NLS_INFO, var, p1, p2, p3)
...
SH_CompositeCacheImpl::SH_SharedCacheHeaderInit* SH_CompositeCacheImpl::SH_SharedCacheHeaderInit::newInstance(
    SH_SharedCacheHeaderInit* memForConstructor)
{
...
void SH_CompositeCacheImpl::SH_SharedCacheHeaderInit::init(
    BlockPtr data, U_32 len, I_32 minAOT, I_32 maxAOT, I_32 minJIT, I_32 maxJIT, U_32 readWriteLen, U_32 softMaxBytes)
{
 ...
 */
SH_CompositeCacheImpl* SH_CompositeCacheImpl::newInstance(J9JavaVM* vm, J9SharedClassConfig* sharedClassConfig,
    SH_CompositeCacheImpl* memForConstructor, const char* cacheName, I_32 cacheTypeRequired, bool startupForStats)
{
    SH_CompositeCacheImpl* newCC = (SH_CompositeCacheImpl*)memForConstructor;

    new (newCC) SH_CompositeCacheImpl();
    newCC->initializeWithCommonInfo(vm, sharedClassConfig,
        ((BlockPtr)memForConstructor + sizeof(SH_CompositeCacheImpl)), cacheName, cacheTypeRequired, startupForStats);
 ...
 * @return A pointer to the new instance
 */
SH_CompositeCacheImpl* SH_CompositeCacheImpl::newInstanceNested(J9JavaVM* vm, SH_CompositeCacheImpl* parent,
    SH_CompositeCacheImpl* memForConstructor, UDATA nestedSize, BlockPtr nestedMemory, bool creatingCachelet)
{
    SH_CompositeCacheImpl* newCC = (SH_CompositeCacheImpl*)memForConstructor;

    new (newCC) SH_CompositeCacheImpl();
    newCC->initializeNested(vm, parent, ((BlockPtr)memForConstructor + sizeof(SH_CompositeCacheImpl)), nestedSize,
        nestedMemory, creatingCachelet);
...
SH_CompositeCacheImpl* SH_CompositeCacheImpl::newInstanceChained(J9JavaVM* vm, SH_CompositeCacheImpl* memForConstructor,
    J9SharedClassConfig* sharedClassConfig, I_32 cacheTypeRequired)
{
    SH_CompositeCacheImpl* newCC = (SH_CompositeCacheImpl*)memForConstructor;

    new (newCC) SH_CompositeCacheImpl();
    newCC->initialize(vm, ((BlockPtr)memForConstructor + sizeof(SH_CompositeCacheImpl)), sharedClassConfig,
        "CHAINED_CACHE_ROOT", cacheTypeRequired, false);
...
void SH_CompositeCacheImpl::commonInit(J9JavaVM* vm)
{
    _started = false;
    _cacheName = NULL;
    ...
    _oldUpdateCount = 0;
    _totalStoredBytes = _storedMetaUsedBytes = _storedSegmentUsedBytes = _storedAOTUsedBytes = _storedJITUsedBytes
    ...
    _deployedOffset = 0;
#endif
    _doMetaProtect = _doSegmentProtect = _doHeaderProtect = _doHeaderReadWriteProtect = _doReadWriteSync
        = _doPartialPagesProtect = false;
    _useWriteHash = false;
...
void SH_CompositeCacheImpl::initializeNested(J9JavaVM* vm, SH_CompositeCacheImpl* parent, BlockPtr memForConstructor,
    UDATA nestedSize, BlockPtr nestedMemory, bool creatingCachelet)
{
 ...
 */
I_32 SH_CompositeCacheImpl::getFreeBlockBytes(void)
{
    I_32 retVal;
    ...
    if (((-1 == minAOT) && (-1 == minJIT)) || ((-1 == minAOT) && (minJIT <= jitBytes))
        || ((-1 == minJIT) && (minAOT <= aotBytes)) || ((minAOT <= aotBytes) && (minJIT <= jitBytes))) {
    ...
    /* When creating the cache with -Xscminaot/-Xscminjit > real cache size, minAOT/minJIT will be set to the same size
     * to the real cache size by ensureCorrectCacheSizes(), retVal calculated here can be < 0 in this case.
     */
    return (retVal > 0) ? retVal : 0;
}

void SH_CompositeCacheImpl::initializeWithCommonInfo(J9JavaVM* vm, J9SharedClassConfig* sharedClassConfig,
    BlockPtr memForConstructor, const char* cacheName, I_32 cacheTypeRequired, bool startupForStats)
{
...
void SH_CompositeCacheImpl::initialize(J9JavaVM* vm, BlockPtr memForConstructor, J9SharedClassConfig* sharedClassConfig,
    const char* cacheName, I_32 cacheTypeRequired, bool startupForStats)
{
    J9PortShcVersion versionData;

    Trc_SHR_CC_initialize_Entry1(
        memForConstructor, sharedClassConfig, cacheName, cacheTypeRequired, UnitTest::unitTest);
    ...
    if ((UnitTest::NO_TEST != UnitTest::unitTest) && (UnitTest::CORRUPT_CACHE_TEST != UnitTest::unitTest)) {
         ...
         */
        _osPageSize = (j9mmap_capabilities() & J9PORT_MMAP_CAPABILITY_PROTECT)
            ? j9mmap_get_region_granularity(UnitTest::cacheMemory)
            : 0;
        _newHdrPtr = (SH_SharedCacheHeaderInit*)((BlockPtr)memForConstructor);
        _debugData = (ClassDebugDataProvider*)((BlockPtr)memForConstructor
            + sizeof(SH_CompositeCacheImpl::SH_SharedCacheHeaderInit));
        ...
        if ((sharedClassConfig->runtimeFlags & J9SHR_RUNTIMEFLAG_CREATE_OLD_GEN) != 0) {
            _oscache = SH_OSCache::newInstance(_portlib, (SH_OSCache*)memForConstructor, cacheName,
                SH_OSCache::getCurrentCacheGen() - 1, &versionData);
        } else {
            _oscache = SH_OSCache::newInstance(
                _portlib, (SH_OSCache*)memForConstructor, cacheName, SH_OSCache::getCurrentCacheGen(), &versionData);
        }
        _newHdrPtr = (SH_SharedCacheHeaderInit*)((BlockPtr)memForConstructor + SH_OSCache::getRequiredConstrBytes());
        _debugData = (ClassDebugDataProvider*)((BlockPtr)memForConstructor + SH_OSCache::getRequiredConstrBytes()
            + sizeof(SH_CompositeCacheImpl::SH_SharedCacheHeaderInit));
...
SH_CompositeCacheImpl::getRequiredConstrBytes(bool isNested, bool startupForStats)
{
    ...
    if ((isNested == false) && (startupForStats == false)
 ...
 */
void SH_CompositeCacheImpl::cleanup(J9VMThread* currentThread)
{
 ...
 */
bool SH_CompositeCacheImpl::crashDetected(UDATA* localCrashCntr)
{
    if (!_started) {
 ...
 * @param [in] currentThread  Pointer to J9VMThread structure for the current thread
 */
void SH_CompositeCacheImpl::reset(J9VMThread* currentThread)
{
    if (!_started) {
        Trc_SHR_Assert_ShouldNeverHappen();
        return;
 ...
 */
bool SH_CompositeCacheImpl::isCacheCorrupt(void)
{
 ...
 * This method does not generate system dump, which should have been done when the corruption context was set in earlier
 * methods.
 *
 * @see setCorruptCache(J9VMThread *, IDATA, UDATA)
 */
void SH_CompositeCacheImpl::setCorruptCache(J9VMThread* currentThread)
{
    ...
    if (!((UnitTest::CORRUPT_CACHE_TEST == UnitTest::unitTest)
            && (0 != (*_runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_STATS)))) {
 ...
 */
void SH_CompositeCacheImpl::setCorruptCache(J9VMThread* currentThread, IDATA corruptionCode, UDATA corruptValue)
{
    ...
    if ((UnitTest::NO_TEST == UnitTest::unitTest) || (UnitTest::CORRUPT_CACHE_TEST == UnitTest::unitTest)) {
        ...
        SH_OSCache* oscacheToUse
            = ((_ccHead == NULL) ? ((_parent == NULL) ? _oscache : _parent->_oscache) : _ccHead->_oscache);
...
void SH_CompositeCacheImpl::setCacheAreaBoundaries(J9VMThread* currentThread, J9SharedClassPreinitConfig* piConfig)
{
    J9JavaVM* vm = currentThread->javaVM;
    ...
    finalReadWriteSize
        = READWRITEAREASIZE(_theca); /* Note that this does not include the size of the J9SharedCacheHeader */
    /**
     ...
     * 		In this case, sharedClassReadWriteBytes is expected to be -1 and finalReadWriteSize is set to the proportional
     * value of shared cache.
     * 2. If user pass an argument to define the size of shared intern table is 0 then finalReadWriteSize is 0 and it is
     * user intentional. In this case it is user's preference that finalReadWriteSize is 0.
     */
    if ((finalReadWriteSize == 0) && (piConfig->sharedClassReadWriteBytes == -1)) {
        ...
        maxSharedStringTableSize = srpHashTable_requiredMemorySize(
            SHRINIT_MAX_SHARED_STRING_TABLE_NODE_COUNT, sizeof(J9SharedInternSRPHashTableEntry), TRUE);
        ...
        if (!(*_runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_ROUND_TO_PAGE_SIZE)) {
            numOfSharedNodes
                = srpHashTable_calculateTableSize(finalReadWriteSize, sizeof(J9SharedInternSRPHashTableEntry), FALSE);
            ...
            finalReadWriteSize
                = srpHashTable_requiredMemorySize(numOfSharedNodes, sizeof(J9SharedInternSRPHashTableEntry), FALSE);
        }
    }

    /* Work out where the segment area will now start from */
    finalSegmentStart = (BlockPtr)SHC_PAD(
        ((UDATA)((BlockPtr)_theca) + finalReadWriteSize + sizeof(J9SharedCacheHeader)), SHC_WORDALIGN);
    ...
    if (*_runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_ROUND_TO_PAGE_SIZE) {
        ...
        Trc_SHR_CC_setCacheAreaBoundaries_Event_prePageRounding(
            currentThread, finalSegmentStart, origCacheEnd, _theca->totalBytes);
        ...
        Trc_SHR_CC_setCacheAreaBoundaries_Event_postPageRounding(
            currentThread, finalSegmentStart, newCacheEnd, _theca->totalBytes);
     ...
     * This area starts from offset cacheMap->getStringTableBase() and size of this area is
     * cacheMap->getReadWriteBytes() Size of string intern table can be defined by user, currently -Xitsn is the only
     * option that allows user to set shared string table size. In such cases, required memory size for shared string
     * intern table is set to the sharedClassPreinitConfig->sharedClassReadWriteBytes, default value is -1 which
     * indicates that shared intern table will be generated in the default proportion of shared cache. If
     * sharedClassPreinitConfig->sharedClassReadWriteBytes is not -1, we should only use this value for shared string
     * intern table. Otherwise use all readWrite area to generate shared string intern table. ReadWrite area is
     * optimized by page rounding up in most of the cases and if user requested specific size of shared string intern
     * table, using all readWrite area for shared intern table might return bigger shared string intern table because of
     * extra space gained by page rounding up.
     *
...
bool SH_CompositeCacheImpl::isLocked(void)
{
    SH_CompositeCacheImpl* ccToUse;
...
void SH_CompositeCacheImpl::setIsLocked(bool value)
{
    SH_CompositeCacheImpl* ccToUse;
...
void SH_CompositeCacheImpl::notifyPagesRead(BlockPtr start, BlockPtr end, UDATA expectedDirection, bool protect)
{
    /* If the cache is locked, the whole metadata area is unprotected for the duration of the lock and then reprotected
     * when the lock is released therefore don't go through this function if we're in a locked state */
    if ((*_runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT) && !isLocked()) {
        ...
        if (protectStart != protectEnd) {
            IDATA rc = setRegionPermissions(_portlib, (void*)protectStart, (protectEnd - protectStart),
                (doProtect ? J9PORT_PAGE_PROTECT_READ : (J9PORT_PAGE_PROTECT_WRITE | J9PORT_PAGE_PROTECT_READ)));
            PORT_ACCESS_FROM_PORT(_portlib);
            ...
            if (isVerbosePages() == true) {
                j9tty_printf(PORTLIB,
                    "Set memory region permissions in notifyPagesRead for %p to %p - doProtect=%d - rc=%d\n",
...
void SH_CompositeCacheImpl::notifyPagesCommitted(BlockPtr start, BlockPtr end, UDATA expectedDirection)
{
    ...
    if (*_runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_MSYNC) {
        ...
        if (syncStart != syncEnd) {
            _oscache->syncUpdates(
                (void*)syncStart, (syncEnd - syncStart), (J9PORT_MMAP_SYNC_WAIT | J9PORT_MMAP_SYNC_INVALIDATE));
        }
    }
#endif
...
SH_CompositeCacheImpl::startupNested(J9VMThread* currentThread)
{
    ...
    return startup(currentThread, &tempConfig, _nestedMemory, _runtimeFlags, _verboseFlags, "A_CACHELET",
        "CACHELET_ROOT", J9SH_DIRPERM_ABSENT, &actualSize, &ignore, false, &ignore2);
}
    ...
    J9SharedClassPreinitConfig* piconfig, U_32* actualSize, UDATA* localCrashCntr)
{
    ...
    return startup(currentThread, piconfig, NULL, ccHead->_runtimeFlags, ccHead->_verboseFlags, "A_CHAINED_CACHE",
        _sharedClassConfig->ctrlDirName, vm->sharedCacheAPI->cacheDirPerm, actualSize, localCrashCntr, false, &ignore);
}
 ...
 * @param [in] cacheMemory  Used in unit testing to avoid use of _oscache (when non-zero).  In normal system use, always
 * specify zero.
...
SH_CompositeCacheImpl::startup(J9VMThread* currentThread, J9SharedClassPreinitConfig* piconfig, BlockPtr cacheMemory,
    U_64* runtimeFlags, UDATA verboseFlags, const char* rootName, const char* ctrlDirName, UDATA cacheDirPerm,
    U_32* actualSize, UDATA* localCrashCntr, bool isFirstStart, bool* cacheHasIntegrity)
{
    IDATA rc = 0;
    ...
    *cacheHasIntegrity = false;
    ...
    if (_parent == NULL) {
        ...
        } else if (J9_ARE_ANY_BITS_SET(
                       *_runtimeFlags, J9SHR_RUNTIMEFLAG_SNAPSHOT | J9SHR_RUNTIMEFLAG_DO_NOT_CREATE_CACHE)) {
    ...
    if (isFirstStart) {
        ...
        if (*_runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_ROUND_TO_PAGE_SIZE) {
            /* If we're rounding to page sizes, we need at least 2 OS pages - one for the cache header and one for the
             * data */
    ...
    *actualSize = 0;
    ...
    if (cacheMemory != NULL) {
        if (!(((J9SharedCacheHeader*)cacheMemory)->ccInitComplete & CC_STARTUP_COMPLETE)) {
            U_32 readWriteBytes
                = (U_32)((piconfig->sharedClassReadWriteBytes > 0) ? piconfig->sharedClassReadWriteBytes : 0);
            headerInit->init(cacheMemory, (U_32)piconfig->sharedClassCacheSize, (I_32)piconfig->sharedClassMinAOTSize,
                (I_32)piconfig->sharedClassMaxAOTSize, (I_32)piconfig->sharedClassMinJITSize,
                (I_32)piconfig->sharedClassMaxJITSize, readWriteBytes, (U_32)piconfig->sharedClassSoftMaxBytes);
        }
        if (_parent == NULL) {
         ...
         * be recreated by another process whenever we're not holding a lock on it. This can happen until attach() is
         * called */

        OSCStarted = _oscache->startup(vm, ctrlDirName, cacheDirPerm, rootName, piconfig,
            ((_ccHead == NULL) ? SH_CompositeCacheImpl::getNumRequiredOSLocks() : 0), createFlags, _verboseFlags,
            *_runtimeFlags, openMode, vm->sharedCacheAPI->storageKeyTesting, &versionData, headerInit,
            SHR_STARTUP_REASON_NORMAL);
    ...
    if (hasWriteMutex) {
        ...
        if (cacheMemory != NULL) {
            ...
            if ((_theca != 0) && getContainsCachelets()
                && ((J9SHR_RUNTIMEFLAG_ENABLE_STATS == (*_runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_STATS))
                       || isRunningReadOnly())) {
        ...
        if (_theca != 0) {
            ...
            if (!_readOnlyOSCache && !(_theca->ccInitComplete & CC_INIT_COMPLETE)) {
                CC_TRACE1(
                    J9SHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_ERROR, J9NLS_CACHE_BAD_CC_INIT, _theca->ccInitComplete);
            ...
            if ((true == this->isMemProtectEnabled()) && (false == _readOnlyOSCache)) {
                ...
                if ((mprotectSize != 0) && (_osPageSize > 0)) {
                    if (this->setRegionPermissions(PORTLIB, (void*)cacheStart, mprotectSize,
                            (J9PORT_PAGE_PROTECT_READ | J9PORT_PAGE_PROTECT_WRITE))
                        != 0) {
                        I_32 myerror = j9error_last_error_number();
                        Trc_SHR_CC_startup_setRegionPermissions_Failed(myerror);
                        Trc_SHR_Assert_ShouldNeverHappen();
                    } else {
                        Trc_SHR_CC_startup_Init_UnprotectForWin32_Event(
                            currentThread, (void*)cacheStart, (void*)(cacheStart + mprotectSize), mprotectSize);
                        if (doVerbosePages) {
                            j9tty_printf(PORTLIB,
                                "Set initial memory region permissions in SH_CompositeCacheImpl::startup() for entire "
                                "cache area from %p to %p - for %d bytes\n",
            ...
            if (!isCacheCorrupt()) {
                ...
                if (!checkCacheCRC(cacheHasIntegrity, &crcValue)) {
                    CC_TRACE1(J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, J9NLS_ERROR,
                        J9NLS_SHRC_CC_STARTUP_CACHE_CRC_INVALID, crcValue);
            ...
            if (!isCacheCorrupt()) {
                ...
                /* If we're running read-only, we have no write mutex. Resolve this race by waiting for ccInitComplete
                 * flag */
                if (!_readOnlyOSCache) {
                ...
                *actualSize = _theca->totalBytes;
                ...
                if (isCacheInitComplete() == false) {
                    ...
                    if (J9_ARE_NO_BITS_SET(currentThread->javaVM->requiredDebugAttributes,
                    ...
                    if (J9_ARE_ANY_BITS_SET(*_runtimeFlags, J9SHR_RUNTIMEFLAG_ENABLE_CACHERETRANSFORMED)) {
                        if (J9_ARE_NO_BITS_SET(*_runtimeFlags, J9SHR_RUNTIMEFLAG_DISABLE_BCI)) {
                            CC_TRACE(J9SHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_INFO,
                                J9NLS_SHRC_OSCACHE_STARTUP_CACHERETRANSFORMED_IMPLIES_DISABLE_BCI);
                        }
                    ...
                    if (J9_ARE_ALL_BITS_SET(*_runtimeFlags, J9SHR_RUNTIMEFLAG_RESTRICT_CLASSPATHS)) {
                        CC_TRACE(J9SHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_INFO,
                            J9NLS_SHRC_OSCACHE_STARTUP_RESTRICT_CLASSPATHS_ENABLED);
                    ...
                    _canStoreClasspaths = (false == this->isRestrictClasspathsSet(currentThread))
                        || J9_ARE_ALL_BITS_SET(*_runtimeFlags, J9SHR_RUNTIMEFLAG_ALLOW_CLASSPATHS);
                    if (_canStoreClasspaths) {
                        if (J9_ARE_ALL_BITS_SET(*_runtimeFlags, J9SHR_RUNTIMEFLAG_ALLOW_CLASSPATHS)) {
                            CC_TRACE(J9SHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_INFO,
                                J9NLS_SHRC_OSCACHE_STARTUP_ALLOW_CLASSPATHS_ENABLED);
                        } else if (J9_ARE_ALL_BITS_SET(*_runtimeFlags, J9SHR_RUNTIMEFLAG_RESTRICT_CLASSPATHS)) {
                            CC_TRACE(J9SHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_WARNING,
                                J9NLS_SHRC_OSCACHE_STARTUP_RESTRICT_CLASSPATHS_IGNORED);
                            /* Unset the runtime flag */
                            *_runtimeFlags &= ~J9SHR_RUNTIMEFLAG_RESTRICT_CLASSPATHS;
                        }
                    } else {
                        CC_TRACE(J9SHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_INFO,
                            J9NLS_SHRC_OSCACHE_STARTUP_CANNOT_STORE_CLASSPATHS);
                    }
                    ...
                    if (isMprotectPartialPagesSet(currentThread)) {
                        if (J9_ARE_NO_BITS_SET(*_runtimeFlags, J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT_PARTIAL_PAGES)) {
                            CC_TRACE(J9SHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_WARNING,
                                J9NLS_SHRC_OSCACHE_STARTUP_MPROTECT_PARTIAL_PAGES_ENABLED);
                        }
#if defined(J9ZOS390) || defined(AIXPPC)
                        *runtimeFlags
                            |= J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT | J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT_PARTIAL_PAGES;
#else
                        *_runtimeFlags |= J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT | J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT_RW
                        ...
                        if (isMprotectPartialPagesOnStartupSet(currentThread)) {
                            if (J9_ARE_NO_BITS_SET(
                                ...
                                *_runtimeFlags |= J9SHR_RUNTIMEFLAG_MPROTECT_PARTIAL_PAGES_ON_STARTUP;
                                CC_TRACE(J9SHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_WARNING,
                                    J9NLS_SHRC_OSCACHE_STARTUP_MPROTECT_PARTIAL_PAGES_ON_STARTUP_ENABLED);
                            }
                        } else {
                            if (J9_ARE_ANY_BITS_SET(
                                ...
                                *_runtimeFlags &= ~J9SHR_RUNTIMEFLAG_MPROTECT_PARTIAL_PAGES_ON_STARTUP;
                                CC_TRACE(J9SHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_INFO,
                                    J9NLS_SHRC_OSCACHE_STARTUP_MPROTECT_PARTIAL_PAGES_NOT_ENABLED_ON_STARTUP);
                            }
                        }
                    } else {
                        if (J9_ARE_ANY_BITS_SET(*_runtimeFlags,
                                J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT_PARTIAL_PAGES
                                    | J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT_ONFIND)) {
                            /* Unset the runtime flags */
                            *_runtimeFlags &= ~(J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT_PARTIAL_PAGES
                                | J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT_ONFIND);
                            *_runtimeFlags &= ~J9SHR_RUNTIMEFLAG_MPROTECT_PARTIAL_PAGES_ON_STARTUP;
                            CC_TRACE(J9SHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_INFO,
                                J9NLS_SHRC_OSCACHE_STARTUP_MPROTECT_PARTIAL_PAGES_DISABLED);
                        }
                    }
                }
                ...
                *localCrashCntr = _theca->crashCntr;
                ...
                /* Unlock the cache if some JVM crashed while it has locked the cache in
                 * SH_CompositeCacheImpl::startCriticalUpdate(). */
                if (!_readOnlyOSCache && isLocked()) {
                    Trc_SHR_CC_Startup_CacheIsLocked(currentThread, _theca->crashCntr);
                    /* do not call doUnlockCache() as it tries to call unprotectHeaderReadWriteArea() which will assert
                     * on _started == false */
                    setIsLocked(false);
                }
...
#if defined(J9SHR_MSYNC_SUPPORT)
                    *_runtimeFlags &= ~(J9SHR_RUNTIMEFLAG_ENABLE_MSYNC | J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT
                        | J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT_ALL | J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT_RW
                        | J9SHR_RUNTIMEFLAG_MPROTECT_PARTIAL_PAGES_ON_STARTUP
                        | J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT_PARTIAL_PAGES | J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT_ONFIND);
#else
                    *_runtimeFlags &= ~(J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT | J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT_ALL
                        | J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT_RW | J9SHR_RUNTIMEFLAG_MPROTECT_PARTIAL_PAGES_ON_STARTUP
                        | J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT_PARTIAL_PAGES | J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT_ONFIND);
#endif
                    if (!_readOnlyOSCache) {
...
#if defined(J9SHR_MSYNC_SUPPORT)
                _doHeaderSync = _doReadWriteSync = _doSegmentSync = _doMetaSync
                ...
                _doPartialPagesProtect
                    = J9_ARE_ALL_BITS_SET(*_runtimeFlags, J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT_PARTIAL_PAGES);
...
#if !defined(J9ZOS390) && !defined(AIXPPC)
                else if (J9_ARE_ALL_BITS_SET(*_runtimeFlags,
                             J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT | J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT_RW
                                 | J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT_PARTIAL_PAGES)) {
                 ...
                 * Note that on platforms with 1MB page boundaries, the header and readWrite areas will all be in one
                 * page */
                if (*_runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_ROUND_TO_PAGE_SIZE) {
                    U_32 roundedHeaderAndRWSize
                        = (U_32)ROUND_UP_TO(_osPageSize, ((UDATA)CASTART(_theca) - (UDATA)_theca));
                    ...
                    if (_readWriteAreaStart != NULL) {
                        ...
                        if (_readWriteAreaPageStart == _cacheHeaderPageStart) {
                            if (roundedHeaderAndRWSize == _osPageSize) {
                                ...
                                /* Header is smaller than a page, round up the start of the readWrite area to the next
                                 * page to ensure that it can be protected/unprotected independently */
                ...
                if ((_parent == NULL) && (_oscache != NULL)) {
                    ...
                    if ((hasReadWriteMutex) || (_commonCCInfo->readWriteAreaMutexID == CC_READONLY_LOCK_VALUE)) {
                         ...
                         * 	((!hasReadWriteMutex) && (_commonCCInfo->readWriteAreaMutexID !=
        ...
        if ((CC_STARTUP_OK == rc) && (hasWriteMutex)) {
            ...
            if (this->_parent == NULL) {
                if (_debugData->Init(
                        currentThread, _theca, (AbstractMemoryPermission*)this, verboseFlags, runtimeFlags, false)
                    == false) {
 ...
 */
bool SH_CompositeCacheImpl::checkCacheCompatibility(J9VMThread* currentThread)
{
    bool rc = true;
    PORT_ACCESS_FROM_PORT(_portlib);
 ...
 */
SH_CompositeCacheImpl::BlockPtr SH_CompositeCacheImpl::next(J9VMThread* currentThread)
{
    ...
    if ((BlockPtr)(*ih) > free) {
        if ((CCITEMLEN(*ih) <= 0) || (CCITEMLEN(*ih) > maxCCItemLen)) {
             ...
             */
            PORT_ACCESS_FROM_PORT(_portlib);
            CC_TRACE2(J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, J9NLS_ERROR,
                J9NLS_SHRC_CM_READ_CORRUPT_CACHE_ENTRY_HEADER_BAD_LEN, ih, CCITEMLEN(*ih));
 ...
 */
void SH_CompositeCacheImpl::initBlockData(ShcItem** itemBuf, U_32 dataLen, U_16 dataType)
{
    if (_readOnlyOSCache) {
        Trc_SHR_Assert_ShouldNeverHappen();
        return;
 ...
 */
void SH_CompositeCacheImpl::doneReadUpdates(J9VMThread* currentThread, IDATA updates)
{
    UDATA* updateCountAddress = WSRP_GET(_theca->updateCountPtr, UDATA*);
 ...
 *								Otherwise, it skips stale items but counts the number
 *skipped.
 *
 ...
 */
SH_CompositeCacheImpl::BlockPtr SH_CompositeCacheImpl::nextEntry(J9VMThread* currentThread, UDATA* staleItems)
{
    BlockPtr result = 0;
    ShcItemHdr* ih;
...
SH_CompositeCacheImpl::enterWriteMutex(J9VMThread* currentThread, bool lockCache, const char* caller)
{
    ...
    if ((UnitTest::unitTest != UnitTest::COMPOSITE_CACHE_TEST_SKIP_WRITE_COUNTER_UPDATE) && (true == _started)
        && (0 == rc)) {
...
SH_CompositeCacheImpl::exitWriteMutex(J9VMThread* currentThread, const char* caller, bool doDecWriteCounter)
{
    ...
    if ((UnitTest::unitTest != UnitTest::COMPOSITE_CACHE_TEST_SKIP_WRITE_COUNTER_UPDATE) && (true == doDecWriteCounter)
        && (true == _started)) {
        /*This function may be called before _theca is set (e.g SH_CompositeCacheImpl::startup)*/
        unprotectHeaderReadWriteArea(currentThread, false);
 ...
 */
void SH_CompositeCacheImpl::doLockCache(J9VMThread* currentThread)
{
    ...
    if ((CACHE_LOCK_PATIENCE_COUNTER == patienceCntr) && (_theca->readerCount > 0)) {
 ...
 * @pre The calling thread MUST hold the write mutex
 */
void SH_CompositeCacheImpl::doUnlockCache(J9VMThread* currentThread)
{
 ...
 */
bool SH_CompositeCacheImpl::peekForWriteHash(J9VMThread* currentThread)
{
    if (!_started || _readOnlyOSCache) {
        Trc_SHR_Assert_ShouldNeverHappen();
        return false;
...
SH_CompositeCacheImpl::testAndSetWriteHash(J9VMThread* currentThread, UDATA hashValue)
{
    ...
    } else if (maskedHash == (cacheValue & WRITEHASH_MASK)) {
...
SH_CompositeCacheImpl::tryResetWriteHash(J9VMThread* currentThread, UDATA hashValue)
{
    ...
    /* If a FIND is called for a class which could not be found on disk, no STORE is called and the writeHash field
     * therefore does not get reset. To mitigate against this, count the number of times the writeHash value has
     * remained unchanged. Thread safety of the _lastFailedWHCount is not important as it does not have to be exact
     */
    if (cacheValue != 0) {
 ...
 * @param [in] hashValue  Hash value of the name of the class which has just been loaded
 */
void SH_CompositeCacheImpl::setWriteHash(J9VMThread* currentThread, UDATA hashValue)
{
...
void SH_CompositeCacheImpl::incReaderCount(J9VMThread* currentThread)
{
    UDATA oldNum, value;
...
void SH_CompositeCacheImpl::decReaderCount(J9VMThread* currentThread)
{
    UDATA oldNum, value;
 ...
 * @param [in] caller  A string representing the caller of this method
 */
void SH_CompositeCacheImpl::exitReadMutex(J9VMThread* currentThread, const char* caller)
{
    if (!_started) {
        Trc_SHR_Assert_ShouldNeverHappen();
        return;
 ...
 * @param [in] currentThread  Pointer to J9VMThread structure for the current thread
 */
void SH_CompositeCacheImpl::startCriticalUpdate(J9VMThread* currentThread)
{
    if (!_started || _readOnlyOSCache) {
        Trc_SHR_Assert_ShouldNeverHappen();
        return;
 ...
 * @param [in] currentThread  Pointer to J9VMThread structure for the current thread
 */
void SH_CompositeCacheImpl::endCriticalUpdate(J9VMThread* currentThread)
{
    if (!_started || _readOnlyOSCache) {
        Trc_SHR_Assert_ShouldNeverHappen();
        return;
 ...
 *
 * @param [in] currentThread  The current thread
 * @param [in] itemToWrite  ShcItem struct describing the data, including the length which will be written as a header
 * to the data
 ...
 */
SH_CompositeCacheImpl::BlockPtr SH_CompositeCacheImpl::allocateBlock(
    J9VMThread* currentThread, ShcItem* itemToWrite, U_32 align, U_32 alignOffset)
{
 ...
 * @return Address of item header allocated, NULL if cache is full
 */
SH_CompositeCacheImpl::BlockPtr SH_CompositeCacheImpl::allocateAOT(
    J9VMThread* currentThread, ShcItem* itemToWrite, U_32 dataBytes)
{
 ...
 * @return Address of item header allocated, NULL if cache is full
 */
SH_CompositeCacheImpl::BlockPtr SH_CompositeCacheImpl::allocateJIT(
    J9VMThread* currentThread, ShcItem* itemToWrite, U_32 dataBytes)
{
 ...
 * @return Address of item header allocated, NULL if cache is full
 */
SH_CompositeCacheImpl::BlockPtr SH_CompositeCacheImpl::allocateWithSegment(
    J9VMThread* currentThread, ShcItem* itemToWrite, U_32 segmentBufferSize, BlockPtr* segmentBuffer)
{
    return allocate(
        currentThread, ALLOCATE_TYPE_BLOCK, itemToWrite, 0, segmentBufferSize, segmentBuffer, NULL, SHC_WORDALIGN, 0);
}
 ...
 * @return Address of item header allocated, NULL if cache is full
 */
SH_CompositeCacheImpl::BlockPtr SH_CompositeCacheImpl::allocateWithReadWriteBlock(
    J9VMThread* currentThread, ShcItem* itemToWrite, U_32 readWriteBufferSize, BlockPtr* readWriteBuffer)
{
    return allocate(currentThread, ALLOCATE_TYPE_BLOCK, itemToWrite, 0, readWriteBufferSize, NULL, readWriteBuffer,
        SHC_WORDALIGN, 0);
}

U_32 SH_CompositeCacheImpl::getBytesRequiredForItemWithAlign(ShcItem* itemToWrite, U_32 align, U_32 alignOffset)
{
...
U_32 SH_CompositeCacheImpl::getBytesRequiredForItem(ShcItem* itemToWrite)
{
    return getBytesRequiredForItemWithAlign(itemToWrite, SHC_WORDALIGN, 0);
}

SH_CompositeCacheImpl::BlockPtr SH_CompositeCacheImpl::allocate(J9VMThread* currentThread, U_8 type,
    ShcItem* itemToWrite, U_32 codeDataLen, U_32 separateBufferSize, BlockPtr* segmentBuffer, BlockPtr* readWriteBuffer,
    U_32 align, U_32 alignOffset)
{
    BlockPtr result = 0;
    U_32 itemLen;
    I_32 freeBytes = 0;
    ...
    if (itemToWrite->dataLen > 0) {
        ...
        /* It is possible for the metadata length to be zero if we're allocating unindexed byte data into the segment
         * area */
        itemLen = 0;
    }

    Trc_SHR_Assert_False(_storedSegmentUsedBytes | _storedReadWriteUsedBytes | _storedMetaUsedBytes
        | _storedAOTUsedBytes | _storedJITUsedBytes);
    ...
    if (type == ALLOCATE_TYPE_BLOCK) {
        ...
        if (reservedAOT >= (_theca->aotBytes + codeDataLen)) {
            /* Reserved AOT has already been counted as used bytes. If it is not reached, codeDataLen should not be
        ...
        if (reservedJIT >= (_theca->jitBytes + codeDataLen)) {
            /* Reserved JIT has already been counted as used bytes. If it is not reached, codeDataLen should not be
             * counted as used bytes */
            usedBytesInc = itemLen + addBufferSize - codeDataLen;
    ...
    if (enoughAvailableSpace) {
        ...
        if (ALLOCATE_TYPE_BLOCK == type) {
             ...
             * if avalible bytes < CC_MIN_SPACE_BEFORE_CACHE_FULL, J9SHR_AVAILABLE_SPACE_FULL is set in the previous
             * commit. J9SHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL is checked in higher level APIs in SH_CacheMap. If it is
             * set, higher level APIs will return direcly and we would not reach here. if avalible bytes >=
             * CC_MIN_SPACE_BEFORE_CACHE_FULL, do not set J9SHR_AVAILABLE_SPACE_FULL here as it is above the threshold.
             */
            Trc_SHR_Assert_True((softMaxValue - usedBytes) >= CC_MIN_SPACE_BEFORE_CACHE_FULL);
            /* If allocating block data, we would reach here only when allocating something that requires more than
             * CC_MIN_SPACE_BEFORE_CACHE_FULL bytes. As free available space is still > CC_MIN_SPACE_BEFORE_CACHE_FULL
             * so J9SHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL is not set.
             */
        ...
        if (ALLOCATE_TYPE_AOT == type) {
            ...
            if (0 != (*_runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_NESTED)) {
                 ...
                 * 		requested amount > freeBlockBytes < (J9SHR_MIN_GAP_BEFORE_METADATA +
                 * J9SHR_MIN_DUMMY_DATA_SIZE). In first case we don't want to set J9SHR_RUNTIMEFLAG_BLOCK_SPACE_FULL as
                 * free block bytes is above threshold. In second case we don't need to set
                 * J9SHR_RUNTIMEFLAG_BLOCK_SPACE_FULL as it would have been done in previous commit.
                 */
    ...
    Trc_SHR_CC_allocate_Exit2(currentThread, result, _scan, _storedMetaUsedBytes, _storedSegmentUsedBytes,
        _storedReadWriteUsedBytes, _storedAOTUsedBytes, _storedJITUsedBytes);
    return result;
}
 ...
 * @param [in] itemLen	Length of metadata entry. It includes size of ShcItem, ShcItemHdr, actual data length, and
 * padding.
 *
 * @return pointer to allocated metadata item.
 */
SH_CompositeCacheImpl::BlockPtr SH_CompositeCacheImpl::allocateMetadataEntry(
    J9VMThread* currentThread, BlockPtr allocPtr, ShcItem* itemToWrite, U_32 itemLen)
{
    ...
    if (0 != _osPageSize) {
        /* If we are close to filling up the cache, then metadata to be added may spill to the page pointed by
         * segmentSRP, which may not have been marked as read-write in allocate() (for instance when adding AOT/JIT
         * data). So mark the page pointed by segmentSRP as read-write here.
         */
 ...
 * Dummy data is added such that a gap of J9SHR_MIN_GAP_BEFORE_METADATA bytes is maintained between classes and
 * metadata. After adding the dummy data, it sets the runtime flags to mark the cache full.
 *
 ...
 * @return void
 */
void SH_CompositeCacheImpl::fillCacheIfNearlyFull(J9VMThread* currentThread)
{
    I_32 freeBlockBytes = getFreeBlockBytes();
    U_32 usedBytes = getUsedBytes();
    U_32 softMaxValue = _theca->softMaxBytes;
    ...
    Trc_SHR_CC_fillCacheIfNearlyFull_Entry1(
        currentThread, freeBlockBytes, usedBytes, softMaxValue, CC_MIN_SPACE_BEFORE_CACHE_FULL);
...
#if defined(J9SHR_CACHELETS_SAVE_READWRITE_AREA)
SH_CompositeCacheImpl::BlockPtr SH_CompositeCacheImpl::allocateReadWrite(U_32 separateBufferSize)
{
 ...
 * @param [in] currentThread  Pointer to J9VMThread structure for the current thread
 */
void SH_CompositeCacheImpl::rollbackUpdate(J9VMThread* currentThread)
{
    if (!_started || _readOnlyOSCache) {
        Trc_SHR_Assert_ShouldNeverHappen();
        return;
    ...
    Trc_SHR_Assert_Equals(currentThread, _commonCCInfo->hasWriteMutexThread);
    Trc_SHR_CC_rollbackUpdate_Event2(currentThread, _scan, _storedMetaUsedBytes, _storedSegmentUsedBytes,
        _storedReadWriteUsedBytes, _storedAOTUsedBytes, _storedJITUsedBytes);

    _storedMetaUsedBytes = _storedSegmentUsedBytes = _storedAOTUsedBytes = _storedJITUsedBytes
        = _storedReadWriteUsedBytes = 0;
...
void SH_CompositeCacheImpl::updateStoredSegmentUsedBytes(U_32 usedBytes)
{
 ...
 * @pre The caller MUST hold the shared classes cache write mutex
 */
void SH_CompositeCacheImpl::commitUpdateHelper(J9VMThread* currentThread, bool isCachelet)
{
    J9JavaVM* vm = currentThread->javaVM;
    UDATA oldNum = 0;
    ...
    Trc_SHR_CC_commitUpdate_Entry2(currentThread, _scan, _storedMetaUsedBytes, _storedSegmentUsedBytes,
        _storedReadWriteUsedBytes, _storedAOTUsedBytes, _storedJITUsedBytes);
    Trc_SHR_Assert_Equals(currentThread, _commonCCInfo->hasWriteMutexThread);
    ...
    if (_storedSegmentUsedBytes) {
        ...
        if (_doSegmentProtect
            ...
            if ((J9_ARE_ALL_BITS_SET(*_runtimeFlags, J9SHR_RUNTIMEFLAG_MPROTECT_PARTIAL_PAGES_ON_STARTUP)
                    || (J9VM_PHASE_NOT_STARTUP == vm->phase))
                ...
                if ((UDATA)(startAddress + _storedSegmentUsedBytes) % _osPageSize) {
                    notifyPagesCommitted(
                        startAddress, startAddress + _storedSegmentUsedBytes + _osPageSize, DIRECTION_FORWARD);
                } else {
    ...
    _theca->lastMetadataType = *(U_32*)&(
        (ShcItem*)((UDATA)_theca + oldNum - (_storedMetaUsedBytes + _storedAOTUsedBytes + _storedJITUsedBytes)))
                                    ->dataType;
    ...
    *updateCountAddress = *updateCountAddress + 1;
     ...
     * The aotBytes field is not used for calculating free space or pointers - it is used purely for setting the AOT
     * limit. */
    ...
    if (_doMetaProtect
        ...
        if ((J9_ARE_ALL_BITS_SET(*_runtimeFlags, J9SHR_RUNTIMEFLAG_MPROTECT_PARTIAL_PAGES_ON_STARTUP)
            ...
            if (((UDATA)_scan + sizeof(ShcItemHdr)) % _osPageSize) {
                notifyPagesCommitted((BlockPtr)_prevScan + sizeof(ShcItemHdr),
                    (BlockPtr)_scan + sizeof(ShcItemHdr) - _osPageSize, DIRECTION_BACKWARD);
            } else {
                notifyPagesCommitted(
                    (BlockPtr)_prevScan + sizeof(ShcItemHdr), (BlockPtr)_scan + sizeof(ShcItemHdr), DIRECTION_BACKWARD);
            }
        } else {
            notifyPagesCommitted(
                (BlockPtr)_prevScan + sizeof(ShcItemHdr), (BlockPtr)_scan + sizeof(ShcItemHdr), DIRECTION_BACKWARD);
 ...
 * @pre The caller MUST hold the shared classes cache write mutex
 */
void SH_CompositeCacheImpl::commitUpdate(J9VMThread* currentThread, bool isCachelet)
{
 ...
 */
void SH_CompositeCacheImpl::markStale(J9VMThread* currentThread, BlockPtr blockEnd, bool isCacheLocked)
{
    ...
    if (_doMetaProtect && !isCacheLocked) {
        ...
        if (setRegionPermissions(
                _portlib, (void*)areaStart, areaLength, (J9PORT_PAGE_PROTECT_WRITE | J9PORT_PAGE_PROTECT_READ))
            != 0) {
            PORT_ACCESS_FROM_PORT(_portlib);
            I_32 myerror = j9error_last_error_number();
 ...
 */
void SH_CompositeCacheImpl::findStart(J9VMThread* currentThread)
{
    if (!_started) {
        Trc_SHR_Assert_ShouldNeverHappen();
        return;
 ...
 */
void* SH_CompositeCacheImpl::getBaseAddress(void)
{
    if (!_started) {
        Trc_SHR_Assert_ShouldNeverHappen();
        return NULL;
 ...
 */
J9SharedCacheHeader* SH_CompositeCacheImpl::getCacheHeaderAddress(void)
{
    if (!_started) {
        Trc_SHR_Assert_ShouldNeverHappen();
        return NULL;
 ...
 */
void* SH_CompositeCacheImpl::getStringTableBase(void)
{
    if (!_started) {
        Trc_SHR_Assert_ShouldNeverHappen();
        return NULL;
 ...
 */
void* SH_CompositeCacheImpl::getCacheEndAddress(void)
{
    if (!_started) {
        Trc_SHR_Assert_ShouldNeverHappen();
        return NULL;
 ...
 */
void* SH_CompositeCacheImpl::getCacheLastEffectiveAddress(void)
{
    if (!_started) {
        Trc_SHR_Assert_ShouldNeverHappen();
        return NULL;
 ...
 */
void* SH_CompositeCacheImpl::getSegmentAllocPtr(void)
{
    if (!_started) {
        Trc_SHR_Assert_ShouldNeverHappen();
        return NULL;
 ...
 * @return pointer to the next free byte in the segment allocation area.
 */
void* SH_CompositeCacheImpl::getMetaAllocPtr(void)
{
    if (!_started) {
        Trc_SHR_Assert_ShouldNeverHappen();
        return NULL;
 ...
 */
void* SH_CompositeCacheImpl::getReadWriteAllocPtr(void)
{
    if (!_started) {
        Trc_SHR_Assert_ShouldNeverHappen();
        return NULL;
 ...
 */
void SH_CompositeCacheImpl::getUnstoredBytes(
...
SH_CompositeCacheImpl::getFreeAvailableBytes(void)
{
    ...
    if ((U_32)-1 == _theca->softMaxBytes) {
        ...
        /* J9SHR_AVAILABLE_SPACE_FULL is set once (softmx - usedBytes) < CC_MIN_SPACE_BEFORE_CACHE_FULL. (softmx -
         * usedBytes) might be > 0, but no data can be added, keep ret to 0 in this case.
         */
    }
 ...
 */
U_32 SH_CompositeCacheImpl::getDebugBytes(void)
{
    if (!_started) {
        Trc_SHR_Assert_ShouldNeverHappen();
        return 0;
 ...
 */
U_32 SH_CompositeCacheImpl::getFreeDebugSpaceBytes(void)
{
    if (!_started) {
        Trc_SHR_Assert_ShouldNeverHappen();
        return 0;
 ...
 * @return number of bytes
 */
U_32 SH_CompositeCacheImpl::getLineNumberTableBytes(void)
{
    if (!_started) {
        Trc_SHR_Assert_ShouldNeverHappen();
        return 0;
 ...
 * @return number of bytes
 */
U_32 SH_CompositeCacheImpl::getLocalVariableTableBytes(void)
{
    if (!_started) {
        Trc_SHR_Assert_ShouldNeverHappen();
        return 0;
 ...
 */
bool SH_CompositeCacheImpl::isAddressInROMClassSegment(const void* address)
{
    if (!_started) {
        Trc_SHR_Assert_ShouldNeverHappen();
        return false;
 ...
 */
bool SH_CompositeCacheImpl::isAddressInCache(const void* address)
{
    if (!_started) {
        Trc_SHR_Assert_ShouldNeverHappen();
        return false;
 ...
 */
void SH_CompositeCacheImpl::runExitCode(J9VMThread* currentThread)
{
    SH_OSCache* oscacheToUse = ((_ccHead == NULL) ? _oscache : _ccHead->_oscache);
 ...
 */
U_16 SH_CompositeCacheImpl::getJVMID(void) { return _commonCCInfo->vmID; }

/**
 * Set the fields to the values in the cache header
 */
void SH_CompositeCacheImpl::setInternCacheHeaderFields(
 ...
 *              corrupted and this is the first JVM to be notified. The caller is responsible for rebuilding the cached
 *data.
 *
 * @return 0 if call succeeded and -1 for failure
 */
IDATA
SH_CompositeCacheImpl::enterReadWriteAreaMutex(
            ...
            if (rc == 0) {
                ...
                if (!readOnly) {
                    ...
                    if (0 != (*_runtimeFlags & J9SHR_RUNTIMEFLAG_CHECK_STRINGTABLE_RESET_READWRITE)) {
                         ...
                         * Variable conditions required to test RESET_READWRITE rely on RESET_READONLY setting them
                         * first.
                         *
                         * Here variable state is:
                         * 		_theca->readWriteCrashCntr = 1
                         * 		oldReadWriteCrashCntr = 0
                         * 		_theca->readWriteRebuildCntr = -1 or UDATA_MAX (This gets set to
                         * oldReadWriteCrashCntr (0) down the line as the two are unequal).
                         */
                    ...
                    if ((0 != (*_runtimeFlags & J9SHR_RUNTIMEFLAG_CHECK_STRINGTABLE_RESET_READONLY))) {
                        ...
                        *_runtimeFlags &= (~J9SHR_RUNTIMEFLAG_CHECK_STRINGTABLE_RESET_READONLY);
                         ...
                         * 		_theca->readWriteRebuildCntr = 0 (This gets set to oldReadWriteCrashCntr (-1) down
                         * the line as the two are unequal).
                         */
                    }
                }

                if (oldReadWriteCrashCntr != _theca->readWriteRebuildCntr) {
                    Trc_SHR_CC_enterReadWriteAreaMutex_EventRebuildCacheData2(
                        currentThread, oldReadWriteCrashCntr, _theca->readWriteRebuildCntr);
                ...
                if (_localReadWriteCrashCntr != oldReadWriteCrashCntr) {
                    Trc_SHR_CC_enterReadWriteAreaMutex_EventRebuildLocalData2(
                        currentThread, _localReadWriteCrashCntr, oldReadWriteCrashCntr);
...
#if defined(J9SHR_MSYNC_SUPPORT)
        if (_doReadWriteSync) {
            oscacheToUse->syncUpdates(
                (void*)_readWriteAreaStart, _readWriteAreaBytes, (J9PORT_MMAP_SYNC_WAIT | J9PORT_MMAP_SYNC_INVALIDATE));
        }
#endif
...
bool SH_CompositeCacheImpl::hasWriteMutex(J9VMThread* currentThread)
{
...
bool SH_CompositeCacheImpl::hasReadWriteMutex(J9VMThread* currentThread)
{
    return (_commonCCInfo->hasReadWriteMutexThread == currentThread);
}

void SH_CompositeCacheImpl::notifyRefreshMutexEntered(J9VMThread* currentThread)
{
    Trc_SHR_Assert_Equals(NULL, _commonCCInfo->hasRefreshMutexThread);
    _commonCCInfo->hasRefreshMutexThread = currentThread;
}

void SH_CompositeCacheImpl::notifyRefreshMutexExited(J9VMThread* currentThread)
{
 ...
 */
void SH_CompositeCacheImpl::unprotectHeaderReadWriteArea(J9VMThread* currentThread, bool changeReadWrite)
{
    if (!_started) {
        Trc_SHR_Assert_ShouldNeverHappen();
        return;
    ...
    if (_doHeaderProtect || (_doHeaderReadWriteProtect && changeReadWrite)) {
        ...
        if ((J9_ARE_ALL_BITS_SET(*_runtimeFlags, J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT_ALL))
            ...
            if (0 == readWriteAreaMutexAcquired) {
                /* If we acquired readWriteMutex here, it will be released in the next call of
        ...
        Trc_SHR_CC_unprotectHeaderReadWriteArea_Debug_Post1(
            doUnprotectReadWrite, _headerProtectCntr, _readWriteProtectCntr);
        ...
        if (areaStart != NULL) {
            ...
            if ((rc = setRegionPermissions(
                     _portlib, areaStart, areaLength, (J9PORT_PAGE_PROTECT_WRITE | J9PORT_PAGE_PROTECT_READ)))
                != 0) {
                I_32 myerror = j9error_last_error_number();
            ...
            if (isVerbosePages() == true) {
                if (doUnprotectReadWrite) {
                    j9tty_printf(PORTLIB,
                        "Unprotecting cache header and readWrite area - from %x for %d bytes - rc=%d\n", areaStart,
                        areaLength, rc);
                } else {
        ...
        Trc_SHR_CC_unprotectHeaderReadWriteArea_Debug_Pre2(
            areaStart, areaLength, _headerProtectCntr, _readWriteProtectCntr);
 ...
 */
void SH_CompositeCacheImpl::protectHeaderReadWriteArea(J9VMThread* currentThread, bool changeReadWrite)
{
    if (!_started) {
        Trc_SHR_Assert_ShouldNeverHappen();
        return;
    ...
    if (_doHeaderProtect || (_doHeaderReadWriteProtect && changeReadWrite)) {
        ...
        Trc_SHR_CC_protectHeaderReadWriteArea_Debug_Post1(
            doProtectReadWrite, _headerProtectCntr, _readWriteProtectCntr);
        ...
        if (areaStart != NULL) {
            ...
            if (isVerbosePages() == true) {
                if (doProtectReadWrite) {
                    ...
                    j9tty_printf(
                        PORTLIB, "Protecting cache header - from %x for %d bytes - rc=%d\n", areaStart, areaLength, rc);
                }
            }
        }
        if (_doHeaderProtect) {
        ...
        Trc_SHR_CC_protectHeaderReadWriteArea_Debug_Pre2(
            areaStart, areaLength, _headerProtectCntr, _readWriteProtectCntr);
        omrthread_monitor_exit(_headerProtectMutex);
...
void SH_CompositeCacheImpl::unprotectMetadataArea()
{
    if (!_started || _readOnlyOSCache) {
        Trc_SHR_Assert_ShouldNeverHappen();
        return;
...
void SH_CompositeCacheImpl::protectMetadataArea(J9VMThread* currentThread)
{
    if (!_started) {
        return;
    }
    if (_doMetaProtect) {
        ...
        if ((true == isCacheMarkedFull(currentThread))
            || ((J9_ARE_ALL_BITS_SET(*_runtimeFlags, J9SHR_RUNTIMEFLAG_MPROTECT_PARTIAL_PAGES_ON_STARTUP)
                    || (J9VM_PHASE_NOT_STARTUP == currentThread->javaVM->phase))
                   && (true == _doPartialPagesProtect))) {
            areaStart = (BlockPtr)ROUND_DOWN_TO(_osPageSize, (UDATA)areaStart);
        } else {
        ...
        if (isVerbosePages() == true) {
            j9tty_printf(
                PORTLIB, "Protecting entire metadata area - from %x for %d bytes - rc=%d\n", areaStart, areaLength, rc);
        }
...
bool SH_CompositeCacheImpl::isRunningReadOnly(void) { return _readOnlyOSCache; }

bool SH_CompositeCacheImpl::isVerbosePages(void)
{
...
bool SH_CompositeCacheImpl::isMemProtectEnabled(void)
{
...
bool SH_CompositeCacheImpl::isMemProtectPartialPagesEnabled(void) { return _doPartialPagesProtect; }

/* THREADING: Pre-req holds the cache write mutex */
U_32 SH_CompositeCacheImpl::getCacheCRC(void)
{
    U_32 value = 0;
    U_32 areaForCrcSize;
    U_8* areaForCrc;
...
/* THREADING: Pre-req holds the cache write mutex */
U_32 SH_CompositeCacheImpl::getCacheAreaCRC(U_8* areaStart, U_32 areaSize)
{
...
void SH_CompositeCacheImpl::updateCacheCRC(void)
{
    U_32 value;
...
/* THREADING: Pre-req holds the cache write mutex */
bool SH_CompositeCacheImpl::checkCacheCRC(bool* cacheHasIntegrity, UDATA* crcValue)
{
    if (NULL != crcValue) {
        *crcValue = 0;
    }
 ...
 */
U_32 SH_CompositeCacheImpl::getTotalSize(void)
{
...
SH_CompositeCacheImpl::getJavacoreData(J9JavaVM* vm, J9SharedClassJavacoreDataDescriptor* descriptor)
{
    ...
    if (isCacheInitComplete()) {
        ...
        descriptor->softMaxBytes
            = (UDATA)((U_32)-1 == _theca->softMaxBytes ? descriptor->cacheSize : _theca->softMaxBytes);
...
void SH_CompositeCacheImpl::updateMetadataSegment(J9VMThread* currentThread)
{
    J9JavaVM* vm = currentThread->javaVM;
 ...
 */
BOOLEAN
SH_CompositeCacheImpl::isReadWriteAreaHeaderReadOnly() { return _readWriteAreaHeaderIsReadOnly; }

SH_CompositeCacheImpl* SH_CompositeCacheImpl::getNext(void) { return _next; }

void SH_CompositeCacheImpl::setNext(SH_CompositeCacheImpl* next) { _next = next; }

J9MemorySegment* SH_CompositeCacheImpl::getCurrentROMSegment(void) { return _currentROMSegment; }

void SH_CompositeCacheImpl::setCurrentROMSegment(J9MemorySegment* segment) { _currentROMSegment = segment; }

#if defined(J9SHR_CACHELET_SUPPORT)

void SH_CompositeCacheImpl::setContainsCachelets(J9VMThread* currentThread)
{
    if (!_started) {
        return;
    }
    /* If the cache contains cachelets, disable segment page protection as this will be dealt with by the cachelets
     * themselves */
    if (_doSegmentProtect) {
...
bool SH_CompositeCacheImpl::getContainsCachelets(void) { return (_theca->containsCachelets == 1); }

SH_CompositeCacheImpl::BlockPtr SH_CompositeCacheImpl::getFirstROMClassAddress(bool isNested)
{
...
/* Note: In case of failure callers are expected to be able to get the error code using portlibrary API
...
bool SH_CompositeCacheImpl::isStarted(void) { return _started; }

bool SH_CompositeCacheImpl::getIsNoLineNumberEnabled(void)
{
 ...
 * 			false otherwise.
 */
bool SH_CompositeCacheImpl::getIsNoLineNumberContentEnabled(void)
{
    if (NULL == this->_theca) {
        return false;
    }
 ...
 */
void SH_CompositeCacheImpl::setNoLineNumberContentEnabled(J9VMThread* currentThread)
{
    if (NULL == this->_theca) {
        return;
    }
 ...
 * 			false otherwise.
 */
bool SH_CompositeCacheImpl::getIsLineNumberContentEnabled(void)
{
    if (NULL == this->_theca) {
        return false;
    }
...
void SH_CompositeCacheImpl::setLineNumberContentEnabled(J9VMThread* currentThread)
{
    if (NULL == this->_theca) {
        return;
    }
...
bool SH_CompositeCacheImpl::getIsBCIEnabled(void)
{
 ...
 * @pre The caller must hold the shared classes cache write mutex
 */
void SH_CompositeCacheImpl::setAOTHeaderPresent(J9VMThread* currentThread)
{
    Trc_SHR_Assert_True(hasWriteMutex(currentThread));
 ...
 * @pre The caller must hold the shared classes cache write mutex
 */
bool SH_CompositeCacheImpl::isAOTHeaderPresent(J9VMThread* currentThread)
{
 ...
 *
 */
bool SH_CompositeCacheImpl::isRestrictClasspathsSet(J9VMThread* currentThread)
{
    Trc_SHR_Assert_True(NULL != this->_theca);
    return (0 != (this->_theca->extraFlags & J9SHR_EXTRA_FLAGS_RESTRICT_CLASSPATHS));
}

void SH_CompositeCacheImpl::setCacheHeaderExtraFlags(J9VMThread* currentThread, UDATA extraFlags)
{
    Trc_SHR_Assert_True(NULL != this->_theca);
    if (true == _started) {
        unprotectHeaderReadWriteArea(currentThread, false);
...
SH_CompositeCacheImpl::computeDeployedReadWriteOffsets(J9VMThread* currentThread, SH_CompositeCacheImpl* ccHead)
{
    ...
    while (walk) {
        ...
        j9tty_printf(PORTLIB, "%x %x %x %x %x\n", (UDATA)destSegmentStart, (UDATA)CASTART(_theca),
            walk->getFreeReadWriteBytes(), READWRITEAREASIZE(_theca), totalReadWriteLen);
 ...
 * @retval false error
 */
bool SH_CompositeCacheImpl::copyFromCacheChain(
    J9VMThread* currentThread, SH_CompositeCacheImpl* ccHead, IDATA* metadataOffset)
{
...
SH_CompositeCacheImpl* SH_CompositeCacheImpl::getParent(void) { return _parent; }

IDATA
SH_CompositeCacheImpl::getDeployedOffset(void) { return _deployedOffset; }

void SH_CompositeCacheImpl::setDeployedOffset(IDATA offset) { _deployedOffset = offset; }

#if defined(J9SHR_CACHELETS_SAVE_READWRITE_AREA)
IDATA
SH_CompositeCacheImpl::getDeployedReadWriteOffset(void) { return _deployedReadWriteOffset; }

void SH_CompositeCacheImpl::setDeployedReadWriteOffset(IDATA offset) { _deployedReadWriteOffset = offset; }
#endif
...
SH_CompositeCacheImpl::fixupSerializedCompiledMethods(J9VMThread* currentThread, void* serializedROMClassStartAddress)
{
    ...
    while (NULL != (it = (ShcItem*)this->nextEntry(currentThread, NULL))) {
        if (TYPE_COMPILED_METHOD == ITEMTYPE(it)) {
            ...
            if (jitConfig->updateROMClassOffsetsInAOTMethod(currentThread->javaVM, CMWDATA(wrapper),
                    (J9ROMMethod*)CMWROMMETHOD(wrapper), serializedROMClassStartAddress)) {
                rc = -1;
                break;
            }
        }
...
SH_CompositeCacheImpl::BlockPtr SH_CompositeCacheImpl::getNestedMemory(void) { return _nestedMemory; }
...
SH_CompositeCacheImpl::countROMSegments(J9VMThread* currentThread)
{
    ...
    do {
        ...
        Trc_SHR_Assert_False(
            (segment->baseAddress < getBaseAddress()) || (segment->heapTop > getCacheLastEffectiveAddress()));
 ...
 */
UDATA
SH_CompositeCacheImpl::writeROMSegmentMetadata(
    J9VMThread* currentThread, UDATA numSegments, BlockPtr dest, UDATA* lastSegmentAlloc)
{
    J9AVLTree* tree = &currentThread->javaVM->classMemorySegments->avlTreeData;
    const J9MemorySegment* segment = NULL;
    UDATA baseAddress;
 ...
 * @bug HACK. THIS IS TOTALLY UNSAFE FOR WRITEABLE CACHES.
 */
void SH_CompositeCacheImpl::unlockStartupMonitor(J9VMThread* currentThread)
{
    if (_startupMonitor) {
 ...
 * @return void
 */
void SH_CompositeCacheImpl::setStringTableInitialized(bool isInitialized)
{
    if (!_started) {
        Trc_SHR_Assert_ShouldNeverHappen();
    } else {
 ...
 */
bool SH_CompositeCacheImpl::isStringTableInitialized(void)
{
    if (!_started) {
        Trc_SHR_Assert_ShouldNeverHappen();
        return false;
 ...
 */
IDATA
SH_CompositeCacheImpl::allocateClassDebugData(J9VMThread* currentThread, U_16 classnameLength,
    const char* classnameData, const J9RomClassRequirements* sizes, J9SharedRomClassPieces* pieces)
{
    ...
    retval = _debugData->allocateClassDebugData(
        currentThread, classnameLength, classnameData, sizes, pieces, (AbstractMemoryPermission*)this);
 ...
 * THREADING: We assume the Segment Mutex, String Table Lock, and Write Area Lock is held by the transaction.
 */
void SH_CompositeCacheImpl::rollbackClassDebugData(
    J9VMThread* currentThread, U_16 classnameLength, const char* classnameData)
{
 ...
 * THREADING: We assume the Segment Mutex, String Table Lock, and Write Area Lock is held by the transaction.
 */
void SH_CompositeCacheImpl::commitClassDebugData(
    J9VMThread* currentThread, U_16 classnameLength, const char* classnameData)
{
    bool ok = false;
    U_32 storedDebugDataBytes = _debugData->getStoredDebugDataBytes();
    /* set _storedDebugDataBytes before calling commitClassDebugData(), as getStoredDebugDataBytes() returns 0 after
     * debugData is committed */

    ok = _debugData->commitClassDebugData(
        currentThread, classnameLength, classnameData, (AbstractMemoryPermission*)this);
 ...
 */
void* SH_CompositeCacheImpl::getClassDebugDataStartAddress(void)
{
    if (!_started) {
        Trc_SHR_Assert_ShouldNeverHappen();
        return NULL;
 ...
 * @return void
 */
void SH_CompositeCacheImpl::initCommonCCInfoHelper()
{
 ...
 */
IDATA
SH_CompositeCacheImpl::startupForStats(
    J9VMThread* currentThread, SH_OSCache* oscache, U_64* runtimeFlags, UDATA verboseFlags)
{
    ...
    if (!oscache->isRunningReadOnly() && _theca->roundedPagesFlag
    ...
    if (_debugData->Init(currentThread, _theca, (AbstractMemoryPermission*)this, verboseFlags, _runtimeFlags, true)
        == false) {
        retval = CC_STARTUP_CORRUPT;
        goto done;
    }
 ...
 */
bool SH_CompositeCacheImpl::isCacheInitComplete(void)
{
 ...
 */
IDATA
SH_CompositeCacheImpl::getNumRequiredOSLocks() { return CC_NUM_WRITE_LOCKS; }
 ...
 * @return void
 */
void SH_CompositeCacheImpl::getCorruptionContext(IDATA* code, UDATA* value)
{
 ...
 * @return void
 */
void SH_CompositeCacheImpl::setCorruptionContext(IDATA corruptionCode, UDATA corruptValue)
{
 ...
 * @return void
 */
void SH_CompositeCacheImpl::setRuntimeCacheFullFlags(J9VMThread* currentThread)
{
    PORT_ACCESS_FROM_PORT(_portlib);
    Trc_SHR_Assert_True(hasWriteMutex(currentThread));
    ...
    if (0 != (_theca->cacheFullFlags & J9SHR_ALL_CACHE_FULL_BITS)) {
        ...
        *_runtimeFlags |= cacheFullFlags;
        ...
        if (J9_ARE_ANY_BITS_SET(
                cacheFullFlags, J9SHR_RUNTIMEFLAG_BLOCK_SPACE_FULL | J9SHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL)) {
        ...
        if (0 != cacheFullFlags) {
            if (true == isAllRuntimeCacheFullFlagsSet()) {
                ...
                _debugData->protectUnusedPages(currentThread, (AbstractMemoryPermission*)this);
                           ...
                           J9SHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL | J9SHR_RUNTIMEFLAG_AOT_SPACE_FULL
                               | J9SHR_RUNTIMEFLAG_JIT_SPACE_FULL)) {
            ...
            if (true == allRuntimeCacheFullFlagsSet) {
                ...
                if (0 != (cacheFullFlags & J9SHR_RUNTIMEFLAG_BLOCK_SPACE_FULL)) {
                    if (0 != (cacheFullFlags & J9SHR_RUNTIMEFLAG_ENABLE_NESTED)) {
                        ...
                        CC_TRACE1(J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, J9NLS_INFO, J9NLS_SHRC_CM_WARN_FULL_CACHE,
                            _cacheName);
                    } else {
                        /* Only report Block data space full to std-err with verbose enabled */
                        CC_TRACE1(J9SHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_INFO, J9NLS_SHRC_CM_WARN_BLOCK_SPACE_FULL,
                            _cacheName);
                    }
                }
                if (J9_ARE_ALL_BITS_SET(cacheFullFlags, J9SHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL)) {
                    ...
                    CC_TRACE2(J9SHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_INFO, J9NLS_SHRC_CC_WARN_AVAILABLE_SPACE_FULL,
                        _cacheName, OPTION_ADJUST_SOFTMX_EQUALS);
                }
                if (0 != (cacheFullFlags & J9SHR_RUNTIMEFLAG_AOT_SPACE_FULL)) {
                    /* Only report AOT data space full to std-err with verbose enabled */
                    CC_TRACE1(
                        J9SHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_INFO, J9NLS_SHRC_CC_WARN_AOT_SPACE_FULL, _cacheName);
                }
                if (0 != (cacheFullFlags & J9SHR_RUNTIMEFLAG_JIT_SPACE_FULL)) {
                    /* Only report JIT data space full to std-err with verbose enabled */
                    CC_TRACE1(
                        J9SHR_VERBOSEFLAG_ENABLE_VERBOSE, J9NLS_INFO, J9NLS_SHRC_CC_WARN_JIT_SPACE_FULL, _cacheName);
                }
            }
        }
    }
 ...
 * @return void
 */
void SH_CompositeCacheImpl::protectLastUnusedPages(J9VMThread* currentThread)
{
 ...
 */
bool SH_CompositeCacheImpl::isCacheMarkedFull(J9VMThread* currentThread)
{
    Trc_SHR_Assert_True(hasWriteMutex(currentThread));
 ...
 * @return void
 */
void SH_CompositeCacheImpl::setCacheHeaderFullFlags(J9VMThread* currentThread, UDATA flags, bool setRuntimeFlags)
{
    Trc_SHR_Assert_True(hasWriteMutex(currentThread));
 ...
 * @return void
 */
void SH_CompositeCacheImpl::clearCacheHeaderFullFlags(J9VMThread* currentThread)
{
    Trc_SHR_Assert_True(hasWriteMutex(currentThread));
 ...
 * These flag include J9SHR_RUNTIMEFLAG_BLOCK_SPACE_FULL, J9SHR_RUNTIMEFLAG_AOT_SPACE_FULL,
 * J9SHR_RUNTIMEFLAG_JIT_SPACE_FULL and J9SHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL.
 *
 * @return true if all runtime cache full flags are set, false otherwise
 */
bool SH_CompositeCacheImpl::isAllRuntimeCacheFullFlagsSet(void) const
{
    if ((0 != (*_runtimeFlags & J9SHR_RUNTIMEFLAG_BLOCK_SPACE_FULL))
 ...
 * @return void
 */
void SH_CompositeCacheImpl::markReadOnlyCacheFull(void)
{
    /* don't set DENY_CACHE_UPDATES - we still need updates for timestamp optimizations */
    *_runtimeFlags |= (J9SHR_RUNTIMEFLAG_BLOCK_SPACE_FULL | J9SHR_RUNTIMEFLAG_AOT_SPACE_FULL
 ...
 */
I_32 SH_CompositeCacheImpl::getAvailableReservedAOTBytes(J9VMThread* currentThread)
{
    I_32 minAOT = _theca->minAOT;
    I_32 aotBytes = (I_32)_theca->aotBytes;
 ...
 */
I_32 SH_CompositeCacheImpl::getAvailableReservedJITBytes(J9VMThread* currentThread)
{
    I_32 minJIT = _theca->minJIT;
    I_32 jitBytes = (I_32)_theca->jitBytes;
 ...
 */
I_32 SH_CompositeCacheImpl::getFreeAOTBytes(J9VMThread* currentThread)
{
    ...
    if ((-1 == _theca->maxAOT) || ((_theca->maxAOT - (I_32)_theca->aotBytes) >= usableFreeBytes)) {
 ...
 */
I_32 SH_CompositeCacheImpl::getFreeJITBytes(J9VMThread* currentThread)
{
    ...
    if ((-1 == _theca->maxJIT) || ((_theca->maxJIT - (I_32)_theca->jitBytes) >= usableFreeBytes)) {
        return usableFreeBytes;
    } else {
 ...
 */
bool SH_CompositeCacheImpl::isMprotectPartialPagesSet(J9VMThread* currentThread)
{
    Trc_SHR_Assert_True((NULL != this->_theca) && hasWriteMutex(currentThread));
 ...
 */
bool SH_CompositeCacheImpl::isMprotectPartialPagesOnStartupSet(J9VMThread* currentThread)
{
    Trc_SHR_Assert_True((NULL != this->_theca) && hasWriteMutex(currentThread));
 ...
 */
SH_CacheAccess SH_CompositeCacheImpl::isCacheAccessible(void) const { return _oscache->isCacheAccessible(); }
 ...
 * @return bool
 */
bool SH_CompositeCacheImpl::canStoreClasspaths(void) const { return _canStoreClasspaths; }
 ...
 */
void SH_CompositeCacheImpl::dontNeedMetadata(J9VMThread* currentThread, const void* startAddress, size_t length)
{
    _oscache->dontNeedMetadata(currentThread, startAddress, length);
}
/**
 * This function changes the permission of the page containing given address by marking the page as read-only or
 * read-write. The address may belong to either segment region, metadata region or class debug data region. If the
 * address is page aligned, then no action is taken.
 *
 ...
 * @return void
 */
void SH_CompositeCacheImpl::changePartialPageProtection(
    J9VMThread* currentThread, void* addr, bool readOnly, bool phaseCheck)
{
    IDATA rc = 0;
    BlockPtr pageAddr = NULL;
    PORT_ACCESS_FROM_PORT(_portlib);

    Trc_SHR_CC_changePartialPageProtection_Entry(
        currentThread, addr, readOnly, phaseCheck, currentThread->javaVM->phase);
    ...
    if ((J9_ARE_ALL_BITS_SET(*_runtimeFlags, J9SHR_RUNTIMEFLAG_MPROTECT_PARTIAL_PAGES_ON_STARTUP)
        ...
        Trc_SHR_CC_changePartialPageProtection_Event(
            currentThread, pageAddr, pageAddr + _osPageSize, readOnly ? "read-only" : "read-write");

        rc = setRegionPermissions(_portlib, (void*)pageAddr, _osPageSize,
            readOnly ? J9PORT_PAGE_PROTECT_READ : J9PORT_PAGE_PROTECT_READ | J9PORT_PAGE_PROTECT_WRITE);
        if (0 != rc) {
            I_32 myerror = j9error_last_error_number();
 ...
 * @return void
 */
void SH_CompositeCacheImpl::protectPartiallyFilledPages(J9VMThread* currentThread, bool protectSegmentPage,
    bool protectMetadataPage, bool protectDebugDataPages, bool phaseCheck)
{
    if (0 != _osPageSize) {
        /* PR 105482: If segmentSRP and updateSRP are on same page and but only one of them is requested to be
         * protected, then do not protect the page, as the JVM expects the page to be unprotected for writing or
         * updates. For example if protectSegmentPage is true but protectMetadataPage is not, then later the JVM may
         * attempt to write/update data in metadata region, so the page should not be protected.
         */
        BlockPtr segmentPtrPage = (BlockPtr)ROUND_DOWN_TO(_osPageSize, (UDATA)SEGUPDATEPTR(_theca));
 ...
 * @return void
 */
void SH_CompositeCacheImpl::unprotectPartiallyFilledPages(J9VMThread* currentThread, bool unprotectSegmentPage,
    bool unprotectMetadataPage, bool unprotectDebugDataPages, bool phaseCheck)
{
...
/**
 * Get the number of used bytes in the shared classes cache. The headers, RW area, reserved AOT space and reserve JIT
 * data space are counted as used space.
 *
 * @return bytes used in the cache
 */
U_32 SH_CompositeCacheImpl::getUsedBytes(void)
{
    I_64 ret = 0;
    ...
    if (_started) {
        ...
        /* Directly use localVariableTableNextSRP and lineNumberTableNextSRP only when _started is false where
         * _debugData may not be initialized */
        ret = getTotalSize() - getFreeBlockBytes()
            - (_theca->localVariableTableNextSRP - _theca->lineNumberTableNextSRP);
    }
    /* getTotalSize() returns an invalid value in some UnitTest, making ret negative. Always return a non-negative value
     * here */
 ...
 *
 */
void SH_CompositeCacheImpl::setSoftMaxBytes(J9VMThread* currentThread, U_32 softMaxBytes, bool isJCLCall)
{
    PORT_ACCESS_FROM_PORT(_portlib);
    /* add trace points */
    Trc_SHR_Assert_True((NULL != _theca) && hasWriteMutex(currentThread) && (getTotalSize() >= softMaxBytes)
...
/**
 * Try adjusting the value of softMaxBytes, minAOT, maxAOT, minJIT and maxJIT in the cache header. Print out warning
 *message if such passed in value(s) are infeasible.
 *
 * @param[in] currentThread The current vm thread
 * @param [in] isJCLCall True if JCL is invoking this function
 *
 ...
 */
I_32 SH_CompositeCacheImpl::tryAdjustMinMaxSizes(J9VMThread* currentThread, bool isJCLCall)
{
    ...
    if (true == isCacheMarkedFull(currentThread)) {
        /* All cache full bits are set, no more data can be added to the cache anymore. Ignore the attempt to adjust
         * softmx/minAOT/maxAOT/minJIT/maxJIT */
    ...
    if (adjustMinAOT || adjustMaxAOT || adjustSoftMax) {
        ...
        if ((0 < maxAOT) && (maxAOT < minAOT)) {
            ...
            /* when adjustSoftMax is true, the value of maxLimit will be changed, we should perform this check in this
             * case  */
            maxAOT = maxLimit;
    ...
    if (adjustMinAOT || adjustMinJIT) {
        /* The reserved AOT and JIT data space are counted as used space. The increase of used AOTBytes + increased of
         * used JITBytes + already usedBytes should always be <= softMaxBytes/total cache bytes.
         */
        ...
        U_32 maxUsedBytes = (maxLimit < (totalBytes - freeDebugBytes - J9SHR_MIN_GAP_BEFORE_METADATA))
            ? maxLimit
            : (totalBytes - freeDebugBytes - J9SHR_MIN_GAP_BEFORE_METADATA);
        /* The reserved AOT/JIT data space can only come from space between updateSRP and segmentSRP, so free space in
         * debug area should be subtracted. Leave at leaset J9SHR_MIN_GAP_BEFORE_METADATA, so
         * J9SHR_MIN_GAP_BEFORE_METADATA should also be subtracted.
         */
        if ((usedAOTBytesAfter - usedAOTBytesBefore + usedJITBytesAfter - usedJIBytesBefore + usedBytesBefore)
            > maxUsedBytes) {
            CC_WARNING_TRACE(J9NLS_SHRC_CC_TOTAL_USED_BYTES_GRTHAN_MAXLIMIT, isJCLCall);
            Trc_SHR_CC_tryAdjustMinMaxSizes_totalUsedBytesGreaterThanMaxLimit(currentThread, usedAOTBytesBefore,
                usedAOTBytesAfter, usedJIBytesBefore, usedJITBytesAfter, usedBytesBefore, maxUsedBytes);
            goto done;
        }
    }
    ...
    if (adjustMaxAOT) {
        if (maxAOT != _theca->maxAOT) {
            if ((_theca->maxAOT >= 0) && (maxAOT > _theca->maxAOT)) {
    ...
    if (adjustMaxJIT) {
        if (maxJIT != _theca->maxJIT) {
            if ((_theca->maxJIT >= 0) && (maxJIT > _theca->maxJIT)) {
    ...
    if (adjustMinJIT || adjustMinAOT || adjustSoftMax) {
        /* The free block bytes and avaliable bytes (softmx - usedBytes) can be changed if minAOT, minJIT or softmx has
         * been adjusted. fillCacheIfNearlyFull() needs to be called.
         */
        fillCacheIfNearlyFull(currentThread);
    }
 ...
 * @return void
 */
void SH_CompositeCacheImpl::updateRuntimeFullFlags(J9VMThread* currentThread)
{
    ...
    if (J9_ARE_NO_BITS_SET(cacheFullFlags, J9SHR_AVAILABLE_SPACE_FULL)) {
        if (J9_ARE_ALL_BITS_SET(*_runtimeFlags, J9SHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL)) {
            if (J9_ARE_NO_BITS_SET(*_runtimeFlags, J9SHR_RUNTIMEFLAG_BLOCK_SPACE_FULL)) {
                /* J9SHR_BLOCK_SPACE_FULL is always unset together with J9SHR_AVAILABLE_SPACE_FULL, do not need to
                 * remove J9AVLTREE_DISABLE_SHARED_TREE_UPDATES when unseting J9SHR_RUNTIMEFLAG_BLOCK_SPACE_FULL, do it
                 * here is enough.
                 */
                ...
                if (_reduceStoreContentionDisabled) {
                    Trc_SHR_CC_updateRuntimeFullFlags_flagSet(
                        currentThread, J9SHR_RUNTIMEFLAG_ENABLE_REDUCE_STORE_CONTENTION);
                    *_runtimeFlags |= J9SHR_RUNTIMEFLAG_ENABLE_REDUCE_STORE_CONTENTION;
                    _useWriteHash = true;
                    _reduceStoreContentionDisabled = false;
                }
    ...
    if (J9_ARE_NO_BITS_SET(cacheFullFlags, J9SHR_AOT_SPACE_FULL)) {
        if (J9_ARE_ALL_BITS_SET(*_runtimeFlags, J9SHR_RUNTIMEFLAG_AOT_SPACE_FULL)) {
            ...
            /* J9SHR_AOT_SPACE_FULL can be removed when either softmx or maxAOT gets increased by this VM or another VM.
             * Keep a local copy of _maxAOT, so that we know whether it is the increased maxAOT that removes
             * J9SHR_AOT_SPACE_FULL. Set _maxAOTUnstoredBytes to 0 only when maxAOT is increased.
             */
    ...
    if (J9_ARE_NO_BITS_SET(cacheFullFlags, J9SHR_JIT_SPACE_FULL)) {
        if (J9_ARE_ALL_BITS_SET(*_runtimeFlags, J9SHR_RUNTIMEFLAG_JIT_SPACE_FULL)) {
            ...
            /* J9SHR_JIT_SPACE_FULL can be removed when either softmx or maxJIT gets increased by this VM or another VM.
             * Keep a local copy of _maxJIT, so that we know whether it is the increased maxJIT that removes
             * J9SHR_JIT_SPACE_FULL. Set _maxJITUnstoredBytes to 0 only when maxJIT is increased.
             */
    ...
    *_runtimeFlags |= flagSet;
    ...
    if ((holdWriteMutex) || (0 == enterWriteMutex(currentThread, false, fnName))) {
        if (flagSet > 0) {
            if (true == isAllRuntimeCacheFullFlagsSet()) {
                _debugData->protectUnusedPages(currentThread, (AbstractMemoryPermission*)this);
                protectLastUnusedPages(currentThread);
        ...
        if (flagUnset > 0) {
            if (J9_ARE_ANY_BITS_SET(flagUnset, J9SHR_RUNTIMEFLAG_BLOCK_SPACE_FULL)) {
                ...
                unprotectPartiallyFilledPages(currentThread, true, true, true, false);
                           ...
                           flagUnset, J9SHR_RUNTIMEFLAG_AOT_SPACE_FULL | J9SHR_RUNTIMEFLAG_JIT_SPACE_FULL)) {
 ...
 * @return void
 */
void SH_CompositeCacheImpl::unsetCacheHeaderFullFlags(J9VMThread* currentThread, UDATA flagsToUnset)
{
 ...
 *
 */
void SH_CompositeCacheImpl::getMinMaxBytes(U_32* softmx, I_32* minAOT, I_32* maxAOT, I_32* minJIT, I_32* maxJIT)
{
    if (!_started) {
        Trc_SHR_Assert_ShouldNeverHappen();
        return;
 ...
 * @return void
 */
void SH_CompositeCacheImpl::increaseUnstoredBytes(U_32 blockBytes, U_32 aotBytes, U_32 jitBytes)
{
    ...
    if (J9_ARE_ALL_BITS_SET(*_runtimeFlags, J9SHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL)) {
        if ((aotBytes + jitBytes) > 0) {
            if ((_softmxUnstoredBytes + aotBytes + jitBytes) < softmxUnstoredLimit) {
                ...
                Trc_SHR_CC_increaseUnstoredBytes_softmxUnstoredBytesIncreased(
                    aotBytes + jitBytes, _softmxUnstoredBytes);
            } else {
                _softmxUnstoredBytes = softmxUnstoredLimit;
            }
        }
        ...
        if (aotBytes > 0) {
            if (J9_ARE_ALL_BITS_SET(*_runtimeFlags, J9SHR_RUNTIMEFLAG_AOT_SPACE_FULL)) {
                /* Add check for J9SHR_RUNTIMEFLAG_AOT_SPACE_FULL as maxAOT or softmx might have been increased and
                 * J9SHR_RUNTIMEFLAG_AOT_SPACE_FULL is removed. No need to increase _maxAOTUnstoredBytes or
                 * _softmxUnstoredBytes if that's the case.
                 */
                if (-1 == maxAOT) {
                    /* maxAOT is not set. It is the softmx that prevents the AOT data being stored into the cache.
                     * J9SHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL is not set as free available space can still be >
        ...
        if (jitBytes > 0) {
            if (J9_ARE_ALL_BITS_SET(*_runtimeFlags, J9SHR_RUNTIMEFLAG_JIT_SPACE_FULL)) {
                /* Add check for J9SHR_RUNTIMEFLAG_JIT_SPACE_FULL as maxJIT or softmx might have been increased and
                 * J9SHR_RUNTIMEFLAG_JIT_SPACE_FULL is removed. No need to increase _maxJITUnstoredBytes or
                 * _softmxUnstoredBytes if that's the case.
                 */
                if (-1 == maxJIT) {
                    /* maxJIT is not set. It is the softmx that prevents the JIT data being stored into the cache.
                     * J9SHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL is not set as free available space can still be >
                     * CC_MIN_SPACE_BEFORE_CACHE_FULL.
                     */
 ...
 */
bool SH_CompositeCacheImpl::isNewCache(void)
{
    if (!_started) {
        Trc_SHR_Assert_ShouldNeverHappen();
        return false;
