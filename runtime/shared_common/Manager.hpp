...
#define M_ERR_TRACE(var) \
    if (_verboseFlags)   \
    j9nls_printf(PORTLIB, J9NLS_ERROR, var)
#define M_ERR_TRACE2(var, p1, p2) \
    if (_verboseFlags)            \
    j9nls_printf(PORTLIB, J9NLS_ERROR, var, p1, p2)
#define M_ERR_TRACE_NOTAG(var) \
    if (_verboseFlags)         \
    j9nls_printf(PORTLIB, (J9NLS_ERROR | J9NLS_DO_NOT_PRINT_MESSAGE_TAG), var)
#define M_ERR_TRACE2_NOTAG(var, p1, p2) \
    if (_verboseFlags)                  \
    j9nls_printf(PORTLIB, (J9NLS_ERROR | J9NLS_DO_NOT_PRINT_MESSAGE_TAG), var, p1, p2)
#define M_ERR_TRACE3_NOTAG(var, p1, p2, p3) \
    if (_verboseFlags)                      \
    j9nls_printf(PORTLIB, (J9NLS_ERROR | J9NLS_DO_NOT_PRINT_MESSAGE_TAG), var, p1, p2, p3)
...
public:
...
#if defined(J9SHR_CACHELET_SUPPORT)
    SH_CompositeCacheImpl* getCacheAreaForData(
    ...
    virtual void fixupHintsForSerialization(J9VMThread* vmthread, UDATA dataType, UDATA dataLen, U_8* data,
...
protected:
    ...
    HashLinkedListImpl* createLink(const J9UTF8* key, const ShcItem* item, SH_CompositeCache* cachelet,
        UDATA hashPrimeValue, const J9Pool* allocationPool);

    /* This function is the lookup entry point for a manager linked list hashtable */
    HashLinkedListImpl* hllTableLookup(
    ...
    HashLinkedListImpl* hllTableUpdate(J9VMThread* currentThread, const J9Pool* linkPool, const J9UTF8* key,
        const ShcItem* item, SH_CompositeCache* cachelet);
...
private:
    ...
    HashLinkedListImpl* hllTableAdd(J9VMThread* currentThread, const J9Pool* linkPool, const J9UTF8* key,
        const ShcItem* item, UDATA hashPrimeValue, SH_CompositeCache* cachelet, HashLinkedListImpl** addToList);
    HashLinkedListImpl* hllTableLookupHelper(
