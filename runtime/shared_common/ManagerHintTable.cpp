 ...
 */
bool ManagerHintTable::initialize(J9PortLibrary* portlib)
{
    bool rc = true;
    if (!_hinthash) {
        _hinthash = hashTableNew(OMRPORT_FROM_J9PORT(portlib), J9_GET_CALLSITE(), 0, sizeof(HintItem), 0, 0,
            J9MEM_CATEGORY_CLASSES, hashFn, hashEqualFn, NULL, (void*)portlib);
        if (!_hinthash) {
            rc = false;
        }
    }
 ...
 */
bool ManagerHintTable::addHint(J9VMThread* currentThread, UDATA hashValue, SH_CompositeCache* cachelet)
{
    void* rc;
...
/* not currently used */
void ManagerHintTable::removeHint(J9VMThread* currentThread, UDATA hashValue, SH_CompositeCache* cachelet)
{
    HintItem item(hashValue, cachelet);
...
SH_CompositeCache* ManagerHintTable::findHint(J9VMThread* currentThread, UDATA hashValue)
{
