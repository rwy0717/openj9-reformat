 ...
 */
void SH_OSCache::getCacheVersionAndGen(J9PortLibrary* portlib, J9JavaVM* vm, char* buffer, UDATA bufferSize,
    const char* cacheName, J9PortShcVersion* versionData, UDATA generation, bool isMemoryType)
{
    ...
    if (generation <= J9SH_GENERATION_07) {
        J9SH_GET_VERSION_G07ANDLOWER_STRING(PORTLIB, versionStr,
            J9SH_VERSION(versionData->esVersionMajor, versionData->esVersionMinor), versionData->modlevel,
            versionData->addrmode);
    } else {
        U_64 curVMVersion = 0;
        U_64 oldVMVersion = 0;
         ...
         *
         * If we use any thing older than Major=2, and Minor=60 we must use J9SH_GET_VERSION_G07ANDLOWER_STRING to get
         * the file name.
         *
         * When called b/c of '-Xshareclasses:listAllCaches' if the wrong cache name is used the cache will fail to be
         * started, and will not be listed.
         */
        ...
        if (curVMVersion >= oldVMVersion) {
            if (generation <= J9SH_GENERATION_29) {
                J9SH_GET_VERSION_G07TO29_STRING(PORTLIB, versionStr,
                    J9SH_VERSION(versionData->esVersionMajor, versionData->esVersionMinor), versionData->modlevel,
                    versionData->addrmode);
            } else if (versionData->modlevel < 10) {
                J9SH_GET_VERSION_STRING_JAVA9ANDLOWER(PORTLIB, versionStr,
                    J9SH_VERSION(versionData->esVersionMajor, versionData->esVersionMinor), versionData->modlevel,
                    versionData->feature, versionData->addrmode);
            } else {
                J9SH_GET_VERSION_STRING(PORTLIB, versionStr,
                    J9SH_VERSION(versionData->esVersionMajor, versionData->esVersionMinor), versionData->modlevel,
                    versionData->feature, versionData->addrmode);
            }
        } else {
...
#if defined(WIN32)
    j9str_printf(PORTLIB, buffer, bufferSize, "%s%c%s%c%s", versionStr, J9SH_PREFIX_SEPARATOR_CHAR, cacheName,
        J9SH_PREFIX_SEPARATOR_CHAR, genString);
#else
    ...
    if ((J9PORT_SHR_CACHE_TYPE_PERSISTENT == versionData->cacheType)
        ...
        j9str_printf(PORTLIB, buffer, bufferSize, "%s%s%s%c%s", versionStr, identifier, cacheName,
            J9SH_PREFIX_SEPARATOR_CHAR, genString);
    }
#endif
...
SH_OSCache::getCacheDir(J9JavaVM* vm, const char* ctrlDirName, char* buffer, UDATA bufferSize, U_32 cacheType)
{
    ...
    appendBaseDir = (NULL == ctrlDirName) || (J9PORT_SHR_CACHE_TYPE_NONPERSISTENT == cacheType)
        || (J9PORT_SHR_CACHE_TYPE_SNAPSHOT == cacheType);
    if (appendBaseDir) {
    ...
    if ((NULL == ctrlDirName)
        ...
        /* j9shmem_getDir() always tries the CSIDL_LOCAL_APPDATA directory (C:\Documents and Settings\username\Local
         * Settings\Application Data) first on Windows if ctrlDirName is NULL, regardless of whether
         * J9SHMEM_GETDIR_USE_USERHOME is set or not. So J9SHMEM_GETDIR_USE_USERHOME is effective on UNIX only.
         */
...
IDATA
SH_OSCache::getCachePathName(
    J9PortLibrary* portLibrary, const char* cacheDirName, char* buffer, UDATA bufferSize, const char* cacheNameWithVGen)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
...
IDATA
SH_OSCache::commonStartup(J9JavaVM* vm, const char* ctrlDirName, UDATA cacheDirPerm, const char* cacheName,
    J9SharedClassPreinitConfig* piconfig, UDATA createFlag, UDATA verboseFlags, U_64 runtimeFlags, I_32 openMode,
    J9PortShcVersion* versionData)
{
    ...
    if (J9_ARE_ANY_BITS_SET(_createFlags,
            J9SH_OSCACHE_CREATE | J9SH_OSCACHE_OPEXIST_DESTROY | J9SH_OSCACHE_OPEXIST_STATS
                | J9SH_OSCACHE_OPEXIST_DO_NOT_CREATE)) {
    ...
    getCacheVersionAndGen(
        PORTLIB, vm, _cacheNameWithVGen, cacheNameLen, cacheName, versionData, _activeGeneration, true);
...
void SH_OSCache::dontNeedMetadata(J9VMThread* currentThread, const void* startAddress, size_t length) { return; }

/* Function that initializes class variables common to OSCache subclasses */
void SH_OSCache::commonInit(J9PortLibrary* portLibrary, UDATA generation)
{
 ...
 */
SH_OSCache* SH_OSCache::newInstance(J9PortLibrary* portlib, SH_OSCache* memForConstructor, const char* cacheName,
 ...
 */
UDATA
SH_OSCache::getCurrentCacheGen(void) { return OSCACHE_CURRENT_CACHE_GEN; }

void SH_OSCache::setEnableVerbose(
    J9PortLibrary* portLib, J9JavaVM* vm, J9PortShcVersion* versionData, char* cacheNameWithVGen)
{
    U_32 javaVersion = getJCLForShcModlevel(versionData->modlevel);

    /* Disable verbose when only J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT is set for older generation caches belonging
     * to this VM release */
 ...
 *
 */
J9Pool* SH_OSCache::getAllCacheStatistics(J9JavaVM* vm, const char* ctrlDirName, UDATA groupPerm,
    ...
    bool isCache)
{
    ...
    if (false == isCache) {
        ...
        getShmDirVal
            = getCacheDir(vm, ctrlDirName, nonpersistentCacheDir, J9SH_MAXPATH, J9PORT_SHR_CACHE_TYPE_SNAPSHOT);
        if (-1 != getShmDirVal) {
            snapshotFindHandle = SH_OSCacheFile::findfirst(
                PORTLIB, nonpersistentCacheDir, snapshotNameWithVGen, J9PORT_SHR_CACHE_TYPE_SNAPSHOT);
        ...
        if ((getShmDirVal = getCacheDir(
                 vm, ctrlDirName, nonpersistentCacheDir, J9SH_MAXPATH, J9PORT_SHR_CACHE_TYPE_NONPERSISTENT))
            != -1) {
            shmemFindHandle = SH_OSCachesysv::findfirst(PORTLIB, nonpersistentCacheDir, shmemNameWithVGen);
        ...
        if ((getMmapDirVal
                = getCacheDir(vm, ctrlDirName, persistentCacheDir, J9SH_MAXPATH, J9PORT_SHR_CACHE_TYPE_PERSISTENT))
            != -1) {
            if ((mmapFindHandle = SH_OSCacheFile::findfirst(
                     PORTLIB, persistentCacheDir, mmapNameWithVGen, J9PORT_SHR_CACHE_TYPE_PERSISTENT))
                == (UDATA)-1) {
                donePerst = true;
    ...
    if (!ignoreCompatible) {
        cacheList = pool_new(
            sizeof(SH_OSCache_Info), 0, 0, 0, J9_GET_CALLSITE(), J9MEM_CATEGORY_CLASSES, POOL_FOR_PORT(PORTLIB));
        if (cacheList == NULL) {
    ...
    incompatibleList
        = pool_new(sizeof(SH_OSCache_Info), 0, 0, 0, J9_GET_CALLSITE(), J9MEM_CATEGORY_CLASSES, POOL_FOR_PORT(PORTLIB));
    ...
    do {
        if (getCacheStatistics(vm, ctrlDirName, (const char*)nameWithVGen, groupPerm, localVerboseFlags, j2seVersion,
                &tempInfo, reason)
            != -1) {
        ...
        if (!doneSnapshot) {
            if (-1
                == SH_OSCacheFile::findnext(
                       PORTLIB, snapshotFindHandle, snapshotNameWithVGen, J9PORT_SHR_CACHE_TYPE_SNAPSHOT)) {
                doneSnapshot = true;
                continue;
            }
            nameWithVGen = snapshotNameWithVGen;
            ...
            if (SH_OSCacheFile::findnext(PORTLIB, mmapFindHandle, mmapNameWithVGen, J9PORT_SHR_CACHE_TYPE_PERSISTENT)
                == -1) {
                donePerst = true;
            } else {
                nameWithVGen = mmapNameWithVGen;
 ...
 */
IDATA
SH_OSCache::getCacheStatistics(J9JavaVM* vm, const char* ctrlDirName, const char* cacheNameWithVGen, UDATA groupPerm,
    UDATA localVerboseFlags, UDATA j2seVersion, SH_OSCache_Info* result, UDATA reason)
{
    J9PortLibrary* portlib = vm->portLibrary;
    PORT_ACCESS_FROM_PORT(portlib);
    ...
    fileVMVersion
        = SH_OSCache::getCacheVersionToU64(result->versionData.esVersionMajor, result->versionData.esVersionMinor);
    curVMVersion
        = SH_OSCache::getCacheVersionToU64(currentVersionData.esVersionMajor, currentVersionData.esVersionMinor);
    ...
    result->isCompatible = (isCurrentGen
        && isCompatibleShcFilePrefix(
               portlib, (U_32)JAVA_SPEC_VERSION_FROM_J2SE(j2seVersion), getJVMFeature(vm), cacheNameWithVGen));
    ...
    if (result->versionData.cacheType == J9PORT_SHR_CACHE_TYPE_PERSISTENT) {
        ...
        if (0
            == removeCacheVersionAndGen(
                   result->name, CACHE_ROOT_MAXLEN, J9SH_VERSION_STRING_LEN + 1, cacheNameWithVGen)) {
            ...
            /* rc is initialized to -1. If reach this point, getCacheStatistics() is successful for snapshot, set rc to
             * 0 */
            rc = 0;
        }
    } else {
...
void SH_OSCache::initOSCacheHeader(
    OSCache_header_version_current* header, J9PortShcVersion* versionData, UDATA headerLen)
{
...
bool SH_OSCache::isRunningReadOnly() { return _runningReadOnly; }

/* Used for connecting to legacy cache headers */
void* SH_OSCache::getHeaderFieldAddressForGen(void* header, UDATA headerGen, UDATA fieldID)
{
 ...
 */
U_32 SH_OSCache::getDataSize() { return (U_32)_dataLength; }
 ...
 * @return U_64
 */
U_64 SH_OSCache::getCacheVersionToU64(U_32 major, U_32 minor)
{
    U_64 retval = 0;
 ...
 */
bool SH_OSCache::getCacheStatsCommon(J9JavaVM* vm, SH_OSCache* cache, SH_OSCache_Info* cacheInfo)
{
    bool retval = true;
    ...
    /*If startedForStats != CC_STARTUP_OK then shutdownForStats was already called by startupForStats in the failure
     * condition*/
...
void SH_OSCache::setCorruptionContext(IDATA corruptionCode, UDATA corruptValue)
{
 ...
 * if corruptionCode = ACQUIRE_HEADER_WRITE_LOCK_FAILED, corruptValue contains the error code for the failure to acquire
 * lock. if corruptionCode = CACHE_SIZE_INVALID, corruptValue contains invalid cache size.
 *
 ...
 * @return void
 */
void SH_OSCache::getCorruptionContext(IDATA* corruptionCode, UDATA* corruptValue)
{
 ...
 */
void* SH_OSCache::getOSCacheStart() { return _headerStart; }
