 ...
 */
bool SH_OSCacheFile::openCacheFile(bool doCreateFile, LastErrorInfo* lastErrorInfo)
{
    bool result = true;
    PORT_ACCESS_FROM_PORT(_portLibrary);
...
#endif
        if ((_fileHandle == -1) && (openFlags != EsOpenRead)
            && (_openMode & J9OSCACHE_OPEN_MODE_TRY_READONLY_ON_FAIL)) {
 ...
 */
bool SH_OSCacheFile::closeCacheFile()
{
    bool result = true;
    PORT_ACCESS_FROM_PORT(_portLibrary);
 ...
 */
I_32 SH_OSCacheFile::getFileMode()
{
    I_32 perm = 0;
 ...
 */
void SH_OSCacheFile::errorHandler(U_32 moduleName, U_32 id, LastErrorInfo* lastErrorInfo)
{
    PORT_ACCESS_FROM_PORT(_portLibrary);

    if ((NULL != lastErrorInfo) && (0 != lastErrorInfo->lastErrorCode)) {
        Trc_SHR_OSC_Mmap_errorHandler_Entry_V2(
            moduleName, id, lastErrorInfo->lastErrorCode, lastErrorInfo->lastErrorMsg);
    } else {
 ...
 *
 */
void SH_OSCacheFile::setError(IDATA errorCode)
{
...
void* SH_OSCacheFile::getMmapHeaderFieldAddressForGen(void* header, UDATA headerGen, UDATA fieldID)
{
 ...
 */
I_32 SH_OSCacheFile::findnext(J9PortLibrary* portLibrary, UDATA findHandle, char* resultbuf, UDATA cacheType)
{
    I_32 rc = -1;
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    } while ((-1 != rc) && (!isCacheFileName(PORTLIB, resultbuf, cacheType, NULL)));
 ...
 * @param [in] findHandle  The handle of the last file found
 */
void SH_OSCacheFile::findclose(J9PortLibrary* portLibrary, UDATA findhandle)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * provided by operating system, to avoid any unintended access to shared cache file
 *
 ...
 */
SH_CacheFileAccess SH_OSCacheFile::checkCacheFileAccess(
    J9PortLibrary* portLibrary, UDATA fileHandle, I_32 openMode, LastErrorInfo* lastErrorInfo)
{
        ...
        if (statBuf.ownerUid != uid) {
            ...
            if (sameGroup) {
                ...
                if (!J9_ARE_ANY_BITS_SET(openMode, J9OSCACHE_OPEN_MODE_GROUPACCESS)) {
                    /* If 'groupAccess' option is not set, it implies this process wants to access a shared cache file
                     * that it created. But this process is not the owner of the cache file. This implies we should not
                     * allow this process to use the cache.
                     */
...
/**
 * This method checks whether the group access of the shared cache file is successfully set when a new cache/snapshot is
 * created with "groupAccess" suboption
 *
 * @param [in] portLibrary The port library
 ...
 */
I_32 SH_OSCacheFile::verifyCacheFileGroupAccess(
    J9PortLibrary* portLibrary, IDATA fileHandle, LastErrorInfo* lastErrorInfo)
{
    I_32 rc = 1;
#if !defined(WIN32)
