 ...
 * 				area of a new cache
 */
SH_OSCachemmap::SH_OSCachemmap(J9PortLibrary* portLibrary, J9JavaVM* vm, const char* cacheDirName,
    const char* cacheName, J9SharedClassPreinitConfig* piconfig, IDATA numLocks, UDATA createFlag, UDATA verboseFlags,
    U_64 runtimeFlags, I_32 openMode, J9PortShcVersion* versionData, SH_OSCacheInitializer* initializer)
{
    ...
    startup(vm, cacheDirName, J9SH_DIRPERM_ABSENT, cacheName, piconfig, numLocks, createFlag, verboseFlags,
        runtimeFlags, openMode, 0, versionData, initializer, SHR_STARTUP_REASON_NORMAL);
 ...
 */
void SH_OSCachemmap::initialize(J9PortLibrary* portLibrary, char* memForConstructor, UDATA generation)
{
 ...
 */
bool SH_OSCachemmap::startup(J9JavaVM* vm, const char* ctrlDirName, UDATA cacheDirPerm, const char* cacheName,
    J9SharedClassPreinitConfig* piconfig, IDATA numLocks, UDATA createFlag, UDATA verboseFlags, U_64 runtimeFlags,
    I_32 openMode, UDATA storageKeyTesting, J9PortShcVersion* versionData, SH_OSCacheInitializer* initializer,
    UDATA reason)
{
    ...
    Trc_SHR_OSC_Mmap_startup_Entry(cacheName, ctrlDirName,
        (piconfig != NULL) ? piconfig->sharedClassCacheSize : defaultCacheSize, numLocks, createFlag, verboseFlags,
        openMode);
    ...
    if (commonStartup(vm, ctrlDirName, cacheDirPerm, cacheName, piconfig, createFlag, verboseFlags, runtimeFlags,
     ...
     * - destroying an existing cache (if SHR_STARTUP_REASON_DESTROY or SHR_STARTUP_REASON_EXPIRE or
     * J9SH_OSCACHE_OPEXIST_DESTROY is set)
     */
    if (!_isUserSpecifiedCacheDir && (J9_ARE_NO_BITS_SET(_createFlags, J9SH_OSCACHE_OPEXIST_DESTROY))
        && (SHR_STARTUP_REASON_DESTROY != reason) && (SHR_STARTUP_REASON_EXPIRE != reason)) {
    ...
    if ((_cacheSize = (U_32)j9file_flength(_fileHandle)) > 0) {
        ...
        if (_runningReadOnly) {
            retryCntr = 0;
            U_32* initCompleteAddr = (U_32*)getMmapHeaderFieldAddressForGen(
                _headerStart, _activeGeneration, OSCACHE_HEADER_FIELD_CACHE_INIT_COMPLETE);

            /* In readonly, we can't get a header lock, so if the cache is mid-init, give it a chance to complete
             * initialization */
 ...
 */
SH_CacheAccess SH_OSCachemmap::isCacheAccessible(void) const
{
 ...
 */
void SH_OSCachemmap::dontNeedMetadata(J9VMThread* currentThread, const void* startAddress, size_t length)
{
 ...
 */
IDATA SH_OSCachemmap::getWriteLockID() { return J9SH_OSCACHE_MMAP_LOCKID_WRITELOCK; }
 ...
 * @return a non-negative lockID on success
 */
IDATA SH_OSCachemmap::getReadWriteLockID() { return J9SH_OSCACHE_MMAP_LOCKID_READWRITELOCK; }
...
SH_OSCachemmap::acquireWriteLock(UDATA lockID)
{
    ...
    while ((rc == -1) && (j9error_last_error_number() == J9PORT_ERROR_FILE_LOCK_EDEADLK)) {
         ...
         */
        if (lockID == J9SH_OSCACHE_MMAP_LOCKID_READWRITELOCK
            && omrthread_monitor_owned_by_self(_lockMutex[J9SH_OSCACHE_MMAP_LOCKID_WRITELOCK]) == 1) {
            /*	CMVC 153095: Case 1
             * Current thread:
             ...
             *
             */
            Trc_SHR_OSC_Mmap_acquireWriteLockDeadlockMsg(
                "Case 1: Current thread owns W lock & monitor, and RW monitor, but EDEADLK'd on RW lock");
#if defined(WIN32) || defined(WIN64)
            rc = j9file_blockingasync_lock_bytes(_fileHandle, lockFlags, lockOffset, lockLength);
#else
            rc = j9file_lock_bytes(_fileHandle, lockFlags, lockOffset, lockLength);
             ...
             *
             * Note:
             *  - Deadlock might caused by the order in which threads have taken taken locks, when compaired to another
             *JVM.
             ...
             */
            Trc_SHR_OSC_Mmap_acquireWriteLockDeadlockMsg(
                "Case 2: Current thread owns RW mon, but EDEADLK'd on RW lock");
...
SH_OSCachemmap::internalAttach(bool isNewCache, UDATA generation)
{
...
#ifndef WIN32
    {
        ...
        if (0 == rc) {
            if (fileStatFilesystem.freeSizeBytes < (U_64)_actualFileLength) {
                OSC_ERR_TRACE2(
                    J9NLS_SHRC_OSCACHE_MMAP_DISK_FULL, (U_64)fileStatFilesystem.freeSizeBytes, (U_64)_actualFileLength);
                rc = J9SH_OSCACHE_FAILURE;
                goto error;
            }
        }
    ...
    /* Map the file */
    _mapFileHandle = j9mmap_map_file(
        _fileHandle, 0, (UDATA)_actualFileLength, _cachePathName, accessFlags, J9MEM_CATEGORY_CLASSES_SHC_CACHE);
    ...
    if (!isNewCache) {
        ...
        if ((dataLengthField
                = (U_32*)getMmapHeaderFieldAddressForGen(_headerStart, generation, OSCACHE_HEADER_FIELD_DATA_LENGTH))) {
            _dataLength = *dataLengthField;
        }
        if ((dataStartField
                = (J9SRP*)getMmapHeaderFieldAddressForGen(_headerStart, generation, OSCACHE_HEADER_FIELD_DATA_START))) {
 ...
 */
void* SH_OSCachemmap::attach(J9VMThread* currentThread, J9PortShcVersion* expectedVersionData)
{
    PORT_ACCESS_FROM_PORT(_portLibrary);
    OSCachemmap_header_version_current* cacheHeader;
    IDATA headerRc;
 ...
 * THREADING: Pre-req caller holds the cache header write lock
 */
I_32 SH_OSCachemmap::updateLastAttachedTime(OSCachemmap_header_version_current* header)
{
    PORT_ACCESS_FROM_PORT(_portLibrary);
 ...
 * THREADING: Pre-req caller holds the cache header write lock
 */
I_32 SH_OSCachemmap::updateLastDetachedTime()
{
    PORT_ACCESS_FROM_PORT(_portLibrary);
 ...
 * THREADING: Pre-req caller holds the cache header write lock
 */
I_32 SH_OSCachemmap::createCacheHeader(OSCachemmap_header_version_current* cacheHeader, J9PortShcVersion* versionData)
{
    PORT_ACCESS_FROM_PORT(_portLibrary);
    ...
    Trc_SHR_OSC_Mmap_createCacheHeader_header(cacheHeader->eyecatcher, cacheHeader->oscHdr.size,
        cacheHeader->oscHdr.dataStart, cacheHeader->oscHdr.dataLength, cacheHeader->createTime,
 ...
 * THREADING: Pre-req caller holds the cache header write lock
 */
bool SH_OSCachemmap::setCacheLength(U_32 cacheSize, LastErrorInfo* lastErrorInfo)
{
    PORT_ACCESS_FROM_PORT(_portLibrary);
 ...
 * THREADING: Pre-req caller holds the cache header write lock
 */
I_32 SH_OSCachemmap::initializeDataHeader(SH_OSCacheInitializer* initializer)
{
    ...
    Trc_SHR_OSC_Mmap_initializeDataHeader_callinginit3(_dataStart, _dataLength, _config->sharedClassMinAOTSize,
        _config->sharedClassMaxAOTSize, _config->sharedClassMinJITSize, _config->sharedClassMaxJITSize,
        _config->sharedClassSoftMaxBytes, readWriteBytes);
    ...
    initializer->init((char*)_dataStart, _dataLength, (I_32)_config->sharedClassMinAOTSize,
        (I_32)_config->sharedClassMaxAOTSize, (I_32)_config->sharedClassMinJITSize,
        (I_32)_config->sharedClassMaxJITSize, readWriteBytes, softMaxBytes);
...
void SH_OSCachemmap::internalDetach(UDATA generation)
{
    PORT_ACCESS_FROM_PORT(_portLibrary);
 ...
 */
IDATA
SH_OSCachemmap::getCacheStats(
    J9JavaVM* vm, const char* cacheDirName, const char* cacheNameWithVGen, SH_OSCache_Info* cacheInfo, UDATA reason)
{
    J9PortLibrary* portLibrary = vm->portLibrary;
    PORT_ACCESS_FROM_PORT(portLibrary);
    /* Using 'SH_OSCachemmap cacheStruct' breaks the pattern of calling getRequiredConstrBytes(), and then allocating
     * memory. However it is consistent with 'SH_OSCachesysv::getCacheStats'.
     */
    ...
    if (removeCacheVersionAndGen(cacheInfo->name, CACHE_ROOT_MAXLEN, J9SH_VERSION_STRING_LEN + 1, cacheNameWithVGen)
        != 0) {
        return -1;
    }
    ...
    cache = (SH_OSCachemmap*)SH_OSCache::newInstance(
        PORTLIB, &cacheStruct, cacheInfo->name, cacheInfo->generation, &versionData);

    /* We try to open the cache read/write */
    if (!cache->startup(vm, cacheDirName, vm->sharedCacheAPI->cacheDirPerm, cacheInfo->name, &piconfig,
            SH_CompositeCacheImpl::getNumRequiredOSLocks(), J9SH_OSCACHE_OPEXIST_STATS, 0, 0 /*runtime flags*/, 0, 0,
            &versionData, NULL, reasonForStartup)) {

        /* If that fails - try to open the cache read-only */
        if (!cache->startup(vm, cacheDirName, vm->sharedCacheAPI->cacheDirPerm, cacheInfo->name, &piconfig, 0,
                J9SH_OSCACHE_OPEXIST_STATS, 0, 0 /*runtime flags*/, J9OSCACHE_OPEN_MODE_DO_READONLY, 0, &versionData,
                NULL, reasonForStartup)) {
            cache->cleanup();
            return -1;
        }
    ...
    if (SHR_STARTUP_REASON_DESTROY != reasonForStartup) {
        /* The offset of fields createTime, lastAttachedTime, lastDetachedTime in struct OSCache_mmap_header2 are
         * different on 32-bit and 64-bit caches. This depends on OSCachemmap_header_version_current and
         * OSCache_header_version_current not changing in an incompatible way.
         */
        if (J9SH_ADDRMODE == cacheInfo->versionData.addrmode) {
            ...
            if ((timeValue = (I_64*)getMmapHeaderFieldAddressForGen(
                     cacheHeader, cacheInfo->generation, OSCACHEMMAP_HEADER_FIELD_LAST_ATTACHED_TIME))) {
                cacheInfo->lastattach = *timeValue;
            }
            if ((timeValue = (I_64*)getMmapHeaderFieldAddressForGen(
                     cacheHeader, cacheInfo->generation, OSCACHEMMAP_HEADER_FIELD_LAST_DETACHED_TIME))) {
                cacheInfo->lastdetach = *timeValue;
            }
            if ((timeValue = (I_64*)getMmapHeaderFieldAddressForGen(
                     cacheHeader, cacheInfo->generation, OSCACHEMMAP_HEADER_FIELD_CREATE_TIME))) {
    ...
    Trc_SHR_OSC_Mmap_getCacheStats_Exit(cacheInfo->os_shmid, cacheInfo->os_semid, cacheInfo->lastattach,
        cacheInfo->lastdetach, cacheInfo->createtime, cacheInfo->nattach, cacheInfo->versionData.cacheType);
 ...
 * @return true on success, false on failure
 */
bool SH_OSCachemmap::deleteCacheFile(LastErrorInfo* lastErrorInfo)
{
    bool result = true;
    PORT_ACCESS_FROM_PORT(_portLibrary);
 ...
 * 			exit processing
 */
void SH_OSCachemmap::runExitCode()
{
 ...
 */
IDATA
SH_OSCachemmap::getLockCapabilities() { return J9OSCACHE_DATA_WRITE_LOCK | J9OSCACHE_DATA_READ_LOCK; }
 ...
 */
U_32 SH_OSCachemmap::getTotalSize() { return (U_32)_actualFileLength; }
...
void* SH_OSCachemmap::getAttachedMemory()
{
