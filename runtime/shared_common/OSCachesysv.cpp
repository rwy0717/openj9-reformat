 ...
 * This c'tor is currently used during unit testing only. Therefore we pass J9SH_DIRPERM_ABSENT as cacheDirPerm to
 * startup().
 *
 ...
 * 				area of a new cache
 */
SH_OSCachesysv::SH_OSCachesysv(J9PortLibrary* portLibrary, J9JavaVM* vm, const char* cachedirname,
    const char* cacheName, J9SharedClassPreinitConfig* piconfig, IDATA numLocks, UDATA createFlag, UDATA verboseFlags,
    U_64 runtimeFlags, I_32 openMode, J9PortShcVersion* versionData, SH_OSCache::SH_OSCacheInitializer* i)
{
    ...
    startup(vm, cachedirname, J9SH_DIRPERM_ABSENT, cacheName, piconfig, numLocks, createFlag, verboseFlags,
        runtimeFlags, openMode, 0, versionData, i, SHR_STARTUP_REASON_NORMAL);
 ...
 */
void SH_OSCachesysv::initialize(J9PortLibrary* portLib, char* memForConstructor, UDATA generation)
{
 ...
 */
bool SH_OSCachesysv::startup(J9JavaVM* vm, const char* ctrlDirName, UDATA cacheDirPerm, const char* cacheName,
    J9SharedClassPreinitConfig* piconfig, IDATA numLocks, UDATA create, UDATA verboseFlags_, U_64 runtimeFlags_,
    ...
    UDATA reason)
{
    ...
    Trc_SHR_OSC_startup_Entry(
        cacheName, (piconfig != NULL) ? piconfig->sharedClassCacheSize : defaultCacheSize, create);
    ...
    if (commonStartup(vm, ctrlDirName, cacheDirPerm, cacheName, piconfig, create, verboseFlags_, runtimeFlags_,
            openMode, versionData)
        != 0) {
     ...
     * so to connect with a generation 1 or 2 cache (Java5 was only ever G01), remove the _G01 from the semaphore file
     * name */
    ...
    while (retryCount > 0) {
        ...
        if (_openMode & J9OSCACHE_OPEN_MODE_DO_READONLY) {
...
#else
            /* Currently on windows, "flags" passed to j9shsem_deprecated_open() are not used, but its better to pass
             * correct flags */
            ...
            shsemrc = j9shsem_deprecated_open(
                _cacheDirName, _groupPerm, &_semhandle, _semFileName, (int)_totalNumSems, 0, flags, NULL);
        ...
        if (shsemrc == J9PORT_INFO_SHSEM_PARTIAL) {
             ...
             * 	- j9shsem_deprecated_openDeprecated() was called and the control files for the semaphore does not exist,
             * or
             *  - j9shsem_deprecated_openDeprecated() was called and the sysv object matching the control file has been
             * deleted, or
             *  - j9shsem_deprecated_open() failed and flag OPEXIST_STATS is set, or
             *  - j9shsem_deprecated_open() was called with flag OPEXIST_DESTROY and control files for the semaphore
             * does not exist
             *
        ...
        case J9PORT_INFO_SHSEM_OPENED:
             ...
             */
            if (J9_ARE_NO_BITS_SET(_openMode, J9OSCACHE_OPEN_MODE_DO_READONLY) && (J9PORT_INFO_SHSEM_OPENED == shsemrc)
                && (!_isUserSpecifiedCacheDir) && (J9_ARE_NO_BITS_SET(_createFlags, J9SH_OSCACHE_OPEXIST_DESTROY))) {
                _semAccess = checkSemaphoreAccess(&lastErrorInfo);
            }

            /* Ignore _semAccess when opening cache for printing stats, but we do need it later to display cache
             * usability */
            if (J9_ARE_ALL_BITS_SET(_createFlags, J9SH_OSCACHE_OPEXIST_STATS)
                ...
                if (J9_ARE_NO_BITS_SET(_runtimeFlags, J9SHR_RUNTIMEFLAG_RESTORE_CHECK)) {
                    /* When running "restoreFromSnapshot" utility, headerMutex has already been acquired in the first
                     * call of SH_OSCachesysv::startup()*/
                ...
                if (0 == headerMutexRc) {
                    ...
                    if (J9_ARE_NO_BITS_SET(
                            _runtimeFlags, J9SHR_RUNTIMEFLAG_RESTORE | J9SHR_RUNTIMEFLAG_RESTORE_CHECK)) {
        ...
        case J9PORT_ERROR_SHSEM_OPFAILED_SEMAPHORE_NOT_FOUND:
            if (J9_ARE_ALL_BITS_SET(_createFlags, J9SH_OSCACHE_OPEXIST_DESTROY)
                ...
                if ((J9PORT_ERROR_SHSEM_OPFAILED == shsemrc)
                    || (J9PORT_ERROR_SHSEM_OPFAILED_SEMAPHORE_NOT_FOUND == shsemrc)) {
                ...
                if (J9PORT_INFO_CONTROL_FILE_UNLINK_FAILED == _controlFileStatus.status) {
                    j9nls_printf(
                        PORTLIB, J9NLS_ERROR, J9NLS_SHRC_OSCACHE_SEMAPHORE_CONTROL_FILE_UNLINK_FAILED, _semFileName);
        ...
        case OSCACHESYSV_NOT_EXIST:
            /* Currently, this case occurs only when J9SH_OSCACHE_OPEXIST_STATS, J9SH_OSCACHE_OPEXIST_DESTROY or
             * J9SH_OSCACHE_OPEXIST_DO_NOT_CREATE is set and shared memory does not exist. */
...
SH_OSCachesysv::openCache(const char* cacheDirName, J9PortShcVersion* versionData, bool semCreated)
{
    ...
    case J9PORT_INFO_SHMEM_OPENED:
         ...
         */
        if (J9_ARE_ALL_BITS_SET(_createFlags, J9SH_OSCACHE_OPEXIST_STATS) || (J9SH_SHM_ACCESS_ALLOWED == _shmAccess)) {
            /* ALL SET */
    ...
    default:
        if (J9_ARE_ANY_BITS_SET(_createFlags,
                J9SH_OSCACHE_OPEXIST_STATS | J9SH_OSCACHE_OPEXIST_DESTROY | J9SH_OSCACHE_OPEXIST_DO_NOT_CREATE)
            ...
            if (J9_ARE_ALL_BITS_SET(_createFlags, J9SH_OSCACHE_OPEXIST_DESTROY)) {
                ...
                cleanupSysvResources();
                           ...
                           _createFlags, J9SH_OSCACHE_OPEXIST_STATS | J9SH_OSCACHE_OPEXIST_DO_NOT_CREATE)) {
            ...
            if (J9PORT_INFO_CONTROL_FILE_UNLINK_FAILED == _controlFileStatus.status) {
                j9nls_printf(
                    PORTLIB, J9NLS_ERROR, J9NLS_SHRC_OSCACHE_SHARED_MEMORY_CONTROL_FILE_UNLINK_FAILED, _shmFileName);
                OSC_ERR_TRACE1(J9NLS_SHRC_OSCACHE_PORT_ERROR_NUMBER, _controlFileStatus.errorCode);
                OSC_ERR_TRACE1(J9NLS_SHRC_OSCACHE_PORT_ERROR_MESSAGE, _controlFileStatus.errorMsg);
            }
#if !defined(WIN32)
    ...
    if (0 == rc) {
        ...
        if (J9SH_OSCACHE_HEADER_OK == headerRc) {
            if (NULL != _semhandle) {
                ...
                if ((_runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_SEMAPHORE_CHECK) && (header->attachedSemid != 0)
                    && (header->attachedSemid != _semid)) {
 ...
 */
void SH_OSCachesysv::cleanup(void)
{
    PORT_ACCESS_FROM_PORT(_portLibrary);
...
SH_OSCachesysv::initializeHeader(const char* cacheDirName, J9PortShcVersion* versionData, LastErrorInfo lastErrorInfo)
{
    ...
    if (_initializer != NULL) {
        _initializer->init((char*)region, dataLength, (I_32)_config->sharedClassMinAOTSize,
            (I_32)_config->sharedClassMaxAOTSize, (I_32)_config->sharedClassMinJITSize,
            (I_32)_config->sharedClassMaxJITSize, readWriteBytes, softMaxSize);
    }
 ...
 *
 * @param [in] expectedVersionData  If not NULL, function checks the version data of the cache against the values in
 * this struct
 *
 ...
 *
 */
void* SH_OSCachesysv::attach(J9VMThread* currentThread, J9PortShcVersion* expectedVersionData)
{
    ...
    if ((((_runtimeFlags & J9SHR_RUNTIMEFLAG_CREATE_OLD_GEN) == 0) && (_activeGeneration != getCurrentCacheGen()))
        || (((_runtimeFlags & J9SHR_RUNTIMEFLAG_CREATE_OLD_GEN) != 0)
               && (_activeGeneration != getCurrentCacheGen() - 1))) {
...
SH_OSCachesysv::destroy(bool suppressVerbose, bool isReset)
{
    ...
    if (isCacheActive()) {
        ...
        /* Even if cache is active, we destroy the semaphore if the cache has been marked as corrupt due to
         * CACHE_SEMAPHORE_MISMATCH. */
 ...
 */
IDATA
SH_OSCachesysv::getWriteLockID() { return this->getNewWriteLockID(); }
 ...
 */
IDATA
SH_OSCachesysv::getReadWriteLockID() { return this->getNewWriteLockID(); }
...
SH_OSCachesysv::acquireWriteLock(UDATA lockID)
{
    ...
    if (rc == -1) {
        /* CMVC 97181 : Don't print error message because if JVM terminates with ^C signal, this function will return -1
         * and this is not an error*/
        I_32 myerror = j9error_last_error_number();
        if (((I_32)(myerror | 0xFFFF0000)) != J9PORT_ERROR_SYSV_IPC_ERRNO_EINTR) {
#if !defined(WIN32)
            OSC_ERR_TRACE2(
                J9NLS_SHRC_CC_SYSV_AQUIRE_LOCK_FAILED_ENTER_MUTEX, j9shsem_deprecated_getid(_semhandle), myerror);
#else
...
void SH_OSCachesysv::printErrorMessage(LastErrorInfo* lastErrorInfo)
{
 ...
 */
void SH_OSCachesysv::cleanupSysvResources(void)
{
    PORT_ACCESS_FROM_PORT(_portLibrary);
    ...
    if ((NULL != _semhandle) && (J9SH_SEM_ACCESS_ALLOWED == _semAccess)) {
        ...
        if (j9shsem_deprecated_destroy(&_semhandle) == 0) {
            ...
            if ((J9PORT_ERROR_SYSV_IPC_SEMCTL_ERROR == lastSysCall)
                && (J9PORT_ERROR_SYSV_IPC_ERRNO_EPERM == lastError)) {
    ...
    if ((NULL != _shmhandle) && (J9SH_SHM_ACCESS_ALLOWED == _shmAccess)) {
        ...
        if (j9shmem_destroy(_cacheDirName, _groupPerm, &_shmhandle) == 0) {
            ...
            if ((J9PORT_ERROR_SYSV_IPC_SHMCTL_ERROR == lastSysCall)
                && (J9PORT_ERROR_SYSV_IPC_ERRNO_EPERM == lastError)) {
...
void SH_OSCachesysv::errorHandler(U_32 module_name, U_32 message_num, LastErrorInfo* lastErrorInfo)
{
    PORT_ACCESS_FROM_PORT(_portLibrary);
...
void SH_OSCachesysv::setError(IDATA ec) { _errorCode = ec; }
 ...
 */
IDATA
SH_OSCachesysv::getError(void) { return _errorCode; }
...
SH_OSCachesysv::shmemOpenWrapper(const char* cacheName, LastErrorInfo* lastErrorInfo)
{
...
#else
    rc = j9shmem_open(_cacheDirName, _groupPerm, &_shmhandle, cacheName, _cacheSize, perm,
        J9MEM_CATEGORY_CLASSES_SHC_CACHE, flags, NULL);
    ...
    if (J9PORT_ERROR_SHMEM_OPFAILED == rc) {
        ...
        if (_activeGeneration >= 7) {
            ...
            if (_openMode & J9OSCACHE_OPEN_MODE_TRY_READONLY_ON_FAIL) {
                ...
                perm = J9SH_SHMEM_PERM_READ;
                rc = j9shmem_open(_cacheDirName, _groupPerm, &_shmhandle, cacheName, _cacheSize, perm,
                    J9MEM_CATEGORY_CLASSES_SHC_CACHE, J9SHMEM_NO_FLAGS, &_controlFileStatus);
    ...
    if (((rc == J9PORT_INFO_SHMEM_OPENED) || (rc == J9PORT_INFO_SHMEM_OPENED_STALE))
        && (perm == J9SH_SHMEM_PERM_READ)) {
...
void SH_OSCachesysv::runExitCode(void) { /* No action required */ }
 ...
 */
IDATA
SH_OSCachesysv::syncUpdates(void* start, UDATA length, U_32 flags) { return -1; }
#endif
 ...
 */
IDATA
SH_OSCachesysv::getLockCapabilities(void) { return J9OSCACHE_DATA_WRITE_LOCK; }
...
/**
 * Returns the minimum sized region of a shared classes cache on which the process can set permissions, in the number of
 * bytes.
 *
 * @param[in] portLibrary An instance of portLibrary
 *
 ...
 */
U_32 SH_OSCachesysv::getTotalSize()
{
    J9PortShmemStatistic statbuf;
    PORT_ACCESS_FROM_PORT(_portLibrary);
...
UDATA
SH_OSCachesysv::getHeaderSize(void) { return SHM_CACHEHEADERSIZE; }
 ...
 */
IDATA
SH_OSCachesysv::getCacheStatsHelper(J9JavaVM* vm, const char* cacheDirName, UDATA groupPerm,
    const char* cacheNameWithVGen, SH_OSCache_Info* cacheInfo, UDATA reason)
{
    ...
    if (statrc == 0) {
        ...
        if ((J9SH_ADDRMODE != cacheInfo->versionData.addrmode) && (0 == statbuf.size)) {
            /* JAZZ103 PR 79909 + PR 88930: On z/OS, if the shared memory segment of the cache is created by a 64-bit
             * JVM but is being accessed by a 31-bit JVM, then the size of shared memory segment may not be verifiable.
            ...
            j9nls_printf(PORTLIB, J9NLS_INFO, J9NLS_SHRC_OSCACHE_SHARED_CACHE_SIZE_ZERO_AND_DIFF_ADDRESS_MODE,
                J9SH_ADDRMODE_64, J9SH_ADDRMODE_64, cacheInfo->name, J9SH_ADDRMODE_32);
 ...
 */
IDATA
SH_OSCachesysv::getCacheStats(J9JavaVM* vm, const char* ctrlDirName, UDATA groupPerm, const char* cacheNameWithVGen,
    SH_OSCache_Info* cacheInfo, UDATA reason)
{
    ...
    if (SH_OSCachesysv::getCacheStatsHelper(vm, cacheDirName, groupPerm, cacheNameWithVGen, cacheInfo, reason) == 0) {
        /* Using 'SH_OSCachesysv cacheStruct' breaks the pattern of calling getRequiredConstrBytes(), and then
         * allocating memory. However it is consistent with 'SH_OSCachemmap::getCacheStats'.
         */
        ...
        if ((SHR_STATS_REASON_ITERATE == reason) || (SHR_STATS_REASON_LIST == reason)) {
            cache = (SH_OSCachesysv*)SH_OSCache::newInstance(
                PORTLIB, &cacheStruct, cacheInfo->name, cacheInfo->generation, &versionData);

            if (!cache->startup(vm, ctrlDirName, vm->sharedCacheAPI->cacheDirPerm, cacheInfo->name, &piconfig,
                    SH_CompositeCacheImpl::getNumRequiredOSLocks(), J9SH_OSCACHE_OPEXIST_STATS, 0, 0 /*runtime flags*/,
                    J9OSCACHE_OPEN_MODE_TRY_READONLY_ON_FAIL, vm->sharedCacheAPI->storageKeyTesting, &versionData, NULL,
                    reason)) {
                goto done;
            }
 ...
 */
I_32 SH_OSCachesysv::findnext(struct J9PortLibrary* portLibrary, UDATA findHandle, char* resultbuf)
{
    I_32 rc;
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @param [in] findHandle  The handle of the last file found
 */
void SH_OSCachesysv::findclose(struct J9PortLibrary* portLibrary, UDATA findhandle)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
...
void* SH_OSCachesysv::getSysvHeaderFieldAddressForGen(void* header, UDATA headerGen, UDATA fieldID)
{
...
SH_OSCachesysv::getSysvHeaderFieldOffsetForGen(UDATA headerGen, UDATA fieldID)
{
    if ((4 < headerGen) && (headerGen <= OSCACHE_CURRENT_CACHE_GEN)) {
        ...
        default:
            return offsetof(OSCachesysv_header_version_current, oscHdr)
                + getHeaderFieldOffsetForGen(headerGen, fieldID);
        }
    } else if (4 == headerGen) {
        switch (fieldID) {
        case OSCACHESYSV_HEADER_FIELD_IN_DEFAULT_CONTROL_DIR:
...
#else
    if (getCacheStatsHelper(vm, _cacheDirName, _groupPerm, _cacheNameWithVGen, &cacheInfo, SHR_STATS_REASON_ITERATE)
        != 0) {
        return 0;
    }
...
/**
 * This method performs additional checks to catch scenarios that are not handled by permission and/or mode settings
 * provided by operating system, to avoid any unintended access to sempahore set.
 *
 ...
 */
SH_SysvSemAccess SH_OSCachesysv::checkSemaphoreAccess(LastErrorInfo* lastErrorInfo)
{
            ...
            if (statBuf.cuid != uid) {
                if (statBuf.ouid == uid) {
                        ...
                        if (size > 0) {
                            for (i = 0; i < size; i++) {
                                if ((statBuf.ogid == list[i]) || (statBuf.cgid == list[i])) {
                                    sameGroup = true;
                                    Trc_SHR_OSC_Sysv_checkSemaphoreAccess_SupplementaryGroupMatch(
                                        list[i], statBuf.cgid, statBuf.ogid, semid);
                                    break;
                                }
                            }
                        } else {
                    ...
                    if (sameGroup) {
                        ...
                        if (0 == _groupPerm) {
                            /* If 'groupAccess' option is not set, it implies this process wants to attach to a shared
                             * cache that it owns or created. But this process is neither creator nor owner of the
                             * semaphore set. This implies we should not allow this process to use the cache.
                             */
...
/**
 * This method performs additional checks to catch scenarios that are not handled by permission and/or mode settings
 * provided by operating system, to avoid any unintended access to shared memory.
 *
 * @param[in] lastErrorInfo	Pointer to store last portable error code and/or message
 *
 * @return enum SH_SysvShmAccess indicating if the process can access the shared memory or not
 */
SH_SysvShmAccess SH_OSCachesysv::checkSharedMemoryAccess(LastErrorInfo* lastErrorInfo)
{
        ...
        if (statBuf.cuid != uid) {
            if (statBuf.ouid == uid) {
                ...
                if ((statBuf.ogid == gid) || (statBuf.cgid == gid)) {
                    ...
                    if (size > 0) {
                        for (i = 0; i < size; i++) {
                            if ((statBuf.ogid == list[i]) || (statBuf.cgid == list[i])) {
                                sameGroup = true;
                                Trc_SHR_OSC_Sysv_checkSharedMemoryAccess_SupplementaryGroupMatch(
                                    list[i], statBuf.cgid, statBuf.ogid, shmid);
                                break;
                            }
                        }
                    } else {
                ...
                if (sameGroup) {
                    ...
                    if (0 == _groupPerm) {
                        /* If 'groupAccess' option is not set, it implies this process wants to attach to a shared cache
                         * that it owns or created. But this process is neither creator nor owner of the semaphore set.
                        ...
                        if (statBuf.perm.isGroupWriteable) {
                            /* The shared memory has group read-write permission set, so this process can use it if
                             * 'groupAcccess' option is specified. */
                            shmAccess = J9SH_SHM_ACCESS_GROUP_ACCESS_REQUIRED;
                        } else {
                            /* The shared memory does not have group write permission set, so this process can use it in
                             * readonly mode. */
 ...
 */
SH_CacheAccess SH_OSCachesysv::isCacheAccessible(void) const
{
...
SH_OSCachesysv::OpenSysVSemaphoreHelper(J9PortShcVersion* versionData, LastErrorInfo* lastErrorInfo)
{
    ...
    case J9SH_SYSV_REGULAR_CONTROL_FILE:
        rc = j9shsem_deprecated_open(
            _cacheDirName, _groupPerm, &_semhandle, _semFileName, (int)_totalNumSems, 0, flags, &_controlFileStatus);
        break;
    case J9SH_SYSV_OLDER_EMPTY_CONTROL_FILE:
        rc = j9shsem_deprecated_openDeprecated(
            _cacheDirName, _groupPerm, &_semhandle, _semFileName, J9SH_SYSV_OLDER_EMPTY_CONTROL_FILE);
        break;
    case J9SH_SYSV_OLDER_CONTROL_FILE:
        rc = j9shsem_deprecated_openDeprecated(
            _cacheDirName, _groupPerm, &_semhandle, _semFileName, J9SH_SYSV_OLDER_CONTROL_FILE);
        break;
    default:
        Trc_SHR_Assert_ShouldNeverHappen();
        break;
...
#endif
        rc = j9shmem_open(_cacheDirName, _groupPerm, &_shmhandle, cacheName, _cacheSize, perm, J9MEM_CATEGORY_CLASSES,
            flags, &_controlFileStatus);
        break;
    case J9SH_SYSV_OLDER_EMPTY_CONTROL_FILE:
        rc = j9shmem_openDeprecated(_cacheDirName, _groupPerm, &_shmhandle, cacheName, perm,
            J9SH_SYSV_OLDER_EMPTY_CONTROL_FILE, J9MEM_CATEGORY_CLASSES);
        break;
    case J9SH_SYSV_OLDER_CONTROL_FILE:
        rc = j9shmem_openDeprecated(_cacheDirName, _groupPerm, &_shmhandle, cacheName, perm,
            J9SH_SYSV_OLDER_CONTROL_FILE, J9MEM_CATEGORY_CLASSES);
        break;
    default:
        Trc_SHR_Assert_ShouldNeverHappen();
        break;
...
IDATA
SH_OSCachesysv::StatSysVMemoryHelper(J9PortLibrary* portLibrary, const char* cacheDirName, UDATA groupPerm,
    const char* cacheNameWithVGen, J9PortShmemStatistic* statbuf)
{
    ...
    case J9SH_SYSV_REGULAR_CONTROL_FILE:
        ...
        break;
    case J9SH_SYSV_OLDER_EMPTY_CONTROL_FILE:
        rc = j9shmem_statDeprecated(
            cacheDirName, groupPerm, cacheNameWithVGen, statbuf, J9SH_SYSV_OLDER_EMPTY_CONTROL_FILE);
        break;
    case J9SH_SYSV_OLDER_CONTROL_FILE:
 ...
 */
I_32 SH_OSCachesysv::getControlFilePerm(char* cacheDirName, char* filename, bool* isNotReadable, bool* isReadOnly)
{
...
void* SH_OSCachesysv::getAttachedMemory()
{
...
/**
 * This function restore a non-persistent cache from its snapshot file, startup and walk the restored cache to check for
 * corruption.
 *
 ...
 */
IDATA
SH_OSCachesysv::restoreFromSnapshot(
    J9JavaVM* vm, const char* cacheName, UDATA numLocks, SH_OSCache::SH_OSCacheInitializer* i, bool* cacheExist)
{
    PORT_ACCESS_FROM_JAVAVM(vm);
    IDATA rc = 0;
    ...
    if (-1 == fd) {
        ...
        if ((fileSize < MIN_CC_SIZE) || (fileSize > MAX_CC_SIZE)) {
            ...
            OSC_ERR_TRACE4(
                J9NLS_SHRC_OSCACHE_ERROR_SNAPSHOT_FILE_LENGTH, pathFileName, fileSize, MIN_CC_SIZE, MAX_CC_SIZE);
            rc = -1;
            ...
            versionData.cacheType = J9PORT_SHR_CACHE_TYPE_NONPERSISTENT;
            SH_OSCache::getCacheVersionAndGen(
                PORTLIB, vm, nameWithVGen, CACHE_ROOT_MAXLEN, cacheName, &versionData, OSCACHE_CURRENT_CACHE_GEN, true);
            if (1 == SH_OSCache::statCache(PORTLIB, cacheDirName, nameWithVGen, false)) {
#if !defined(WIN32)
                J9PortShmemStatistic statbuf;
                /* The shared memory may be removed without deleting the control files. So check the existence of the
                 * shared memory */
            ...
            rcStartup = startup(vm, ctrlDirName, vm->sharedCacheAPI->cacheDirPerm, cacheName, piconfig, numLocks,
                J9SH_OSCACHE_CREATE, vm->sharedClassConfig->verboseFlags, vm->sharedClassConfig->runtimeFlags, openMode,
                vm->sharedCacheAPI->storageKeyTesting, &versionData, i, SHR_STARTUP_REASON_NORMAL);
            ...
            if (false == rcStartup) {
                ...
                *cacheExist = true;
                ...
                /* remove J9SHR_RUNTIMEFLAG_RESTORE and startup the cache again to check for corruption, cache header
                 * will be checked in SH_CacheMap::startup() */
                ...
                cleanup();
                rc = cm->startup(currentThread, piconfig, cacheName, ctrlDirName, vm->sharedCacheAPI->cacheDirPerm,
                    NULL, &cacheHasIntegrity);
                ...
                if (0 == rc) {
                    ...
                    /* Header mutex is acquired and not released in the first call of SH_OSCachesysv::startup(), release
                     * here */
                    ret = exitHeaderMutex(&lastErrorInfo);
                    if (0 == ret) {
                        /* set osCacheSysvHeader to current _headerStart as it is detached in cleanup() and re-attached
                         * in cm->startup() */
                        osCacheSysvHeader = (OSCachesysv_header_version_current*)_headerStart;
                        /* To prevent the cache being opened by another JVM in read-only mode,
                         * osCacheSysvHeader->oscHdr.cacheInitComplete is always 0 before the restoring operation is
                         * finished. Set it to 1 here
                         */
...
#if !defined(WIN32)
/**
 * This method checks whether the group access of the semaphore is successfully set when a new cache is created with
 ...
 */
I_32 SH_OSCachesysv::verifySemaphoreGroupAccess(LastErrorInfo* lastErrorInfo)
{
    I_32 rc = 1;
    PORT_ACCESS_FROM_PORT(_portLibrary);
    J9PortShsemStatistic statBuf;
...
/**
 * This method checks whether the group access of the shared memory is successfully set when a new cache is created with
 * "groupAccess" suboption
 *
 * @param[in] lastErrorInfo	Pointer to store last portable error code and/or message.
 *
 ...
 * 			1 Group access is set.
 */
I_32 SH_OSCachesysv::verifySharedMemoryGroupAccess(LastErrorInfo* lastErrorInfo)
{
    I_32 rc = 1;
    PORT_ACCESS_FROM_PORT(_portLibrary);
    J9PortShmemStatistic statBuf;
