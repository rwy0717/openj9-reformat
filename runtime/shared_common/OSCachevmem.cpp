...
SH_OSCachevmem::SH_OSCachevmem(J9PortLibrary* portLibrary, const char* cachedirname, const char* cacheName,
    J9SharedClassPreinitConfig* piconfig, IDATA numLocks, UDATA createFlag, UDATA verboseFlags, U_64 runtimeFlags,
    I_32 openMode, J9PortShcVersion* versionData, SH_OSCacheInitializer* initializer)
{
    initialize(portLibrary, NULL, OSCACHE_CURRENT_CACHE_GEN);
    startup(cachedirname, J9SH_DIRPERM_ABSENT, cacheName, piconfig, numLocks, createFlag, verboseFlags, runtimeFlags,
        openMode, 0, versionData, initializer, SHR_STARTUP_REASON_NORMAL);
}

void SH_OSCachevmem::initialize(J9PortLibrary* portLibrary, char* memForConstructor, UDATA generation)
{
...
bool SH_OSCachevmem::startup(const char* ctrlDirName, UDATA cacheDirPerm, const char* cacheName,
    J9SharedClassPreinitConfig* piconfig, IDATA numLocks, UDATA createFlag, UDATA verboseFlags, U_64 runtimeFlags,
    ...
    UDATA reason)
{
    ...
    Trc_SHR_OSC_Vmem_startup_Entry(
        cacheName, ctrlDirName, piconfig->sharedClassCacheSize, numLocks, createFlag, verboseFlags, openMode);
    ...
    if (commonStartup(ctrlDirName, cacheDirPerm, cacheName, piconfig, createFlag, verboseFlags, runtimeFlags, openMode,
            versionData)
        != 0) {
    ...
    if (_cacheSize > 0) {
        ...
        dataLengthField
            = (U_32*)getMmapHeaderFieldAddressForGen(_headerStart, _activeGeneration, OSCACHE_HEADER_FIELD_DATA_LENGTH);
        if (NULL != dataLengthField) {
            _dataLength = *dataLengthField;
        }
        dataStartField
            = (J9SRP*)getMmapHeaderFieldAddressForGen(_headerStart, _activeGeneration, OSCACHE_HEADER_FIELD_DATA_START);
        ...
        Trc_SHR_OSC_Vmem_startup_callinginit2(_dataStart, _dataLength, piconfig->sharedClassMinAOTSize,
            piconfig->sharedClassMaxAOTSize, piconfig->sharedClassMinJITSize, piconfig->sharedClassMaxJITSize,
            readWriteBytes);
        initializer->init((char*)_dataStart, _dataLength, (I_32)piconfig->sharedClassMinAOTSize,
            (I_32)piconfig->sharedClassMaxAOTSize, (I_32)piconfig->sharedClassMinJITSize,
            (I_32)piconfig->sharedClassMaxJITSize, readWriteBytes);
...
IDATA
SH_OSCachevmem::setRegionPermissions(J9PortLibrary* portLibrary, void* address, UDATA length, UDATA flags) { return 0; }
...
IDATA
SH_OSCachevmem::getWriteLockID() { return this->getNewWriteLockID(); }

IDATA
SH_OSCachevmem::getReadWriteLockID() { return this->getNewWriteLockID(); }
...
void* SH_OSCachevmem::attach(J9VMThread* currentThread, J9PortShcVersion* expectedVersionData)
{
...
void SH_OSCachevmem::runExitCode() {}

U_32 SH_OSCachevmem::getTotalSize() { return _dataLength; }
...
void SH_OSCachevmem::errorHandler(U_32 moduleName, U_32 id, LastErrorInfo* lastErrorInfo)
{
    PORT_ACCESS_FROM_PORT(_portLibrary);
...
IDATA
SH_OSCachevmem::getLockCapabilities(void) { return J9OSCACHE_DATA_WRITE_LOCK; }

void* SH_OSCachevmem::getAttachedMemory()
{
