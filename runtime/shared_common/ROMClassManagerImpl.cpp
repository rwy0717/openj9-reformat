...
J9HashTable* SH_ROMClassManagerImpl::localHashTableCreate(J9VMThread* currentThread, U_32 initialEntries)
{
    ...
    returnVal = hashTableNew(OMRPORT_FROM_J9PORT(_portlib), J9_GET_CALLSITE(), initialEntries,
        sizeof(SH_Manager::HashLinkedListImpl*), sizeof(char*), 0, J9MEM_CATEGORY_CLASSES, SH_Manager::hllHashFn,
        SH_Manager::hllHashEqualFn, NULL, (void*)currentThread->javaVM->internalVMFunctions);
 ...
 */
SH_ROMClassManagerImpl* SH_ROMClassManagerImpl::newInstance(
    J9JavaVM* vm, SH_SharedCache* cache_, SH_TimestampManager* tsm_, SH_ROMClassManagerImpl* memForConstructor)
{
...
void SH_ROMClassManagerImpl::initialize(
...
SH_ROMClassManagerImpl::localInitializePools(J9VMThread* currentThread)
{
    ...
    _linkedListImplPool = pool_new(sizeof(SH_Manager::HashLinkedListImpl), 0, 0, 0, J9_GET_CALLSITE(),
 ...
 */
bool SH_ROMClassManagerImpl::storeNew(
    J9VMThread* currentThread, const ShcItem* itemInCache, SH_CompositeCache* cachelet)
{
    ...
    if (ITEMTYPE(itemInCache) == TYPE_ROMCLASS) {
        orphanReunited = reuniteOrphan(
            currentThread, (const char*)J9UTF8_DATA(utf8Name), J9UTF8_LENGTH(utf8Name), itemInCache, romClass);
    }
    if (!orphanReunited) {
...
bool SH_ROMClassManagerImpl::reuniteOrphan(J9VMThread* currentThread, const char* romClassName, UDATA nameLen,
    const ShcItem* item, const J9ROMClass* romClassPtr)
{
 ...
 */
const J9ROMClass* SH_ROMClassManagerImpl::findNextExisting(J9VMThread* currentThread, void*& findNextIterator,
    void*& firstFound, U_16 classnameLength, const char* classnameData)
{
    ...
    if (prev != NULL) {
        ...
        if (prevVal == returnVal) {
            ...
            returnVal
                = this->findNextExisting(currentThread, findNextIterator, firstFound, classnameLength, classnameData);
        }
    }
 ...
 *
 * Searches all cached ROMClasses matching the fully qualified name and returns the one which is valid WRT to the
 * caller's classpath. A number of hints can be provided to the function to optimize the search, depending on the
 * information available. cpeIndex should be provided if it is known what classpath entry in the caller's classpath the
 * ROMClass should exist in cachedROMClass should be provided if it is already known that the ROMClass exists in the
 * cache - it just needs to be validated
 *
 * @note parameter cp here could be local or cached classpath. If it is local, callerHelperID should be provided for
 * tracing purposes
 *
 * @param[in] currentThread The current thread
 ...
 * @param[in] confirmedEntries The number of entries which have been confirmed in the classpath. Only applies to
 * TYPE_CLASSPATH, otherwise should be -1.
 ...
 * @return LOCATE_ROMCLASS_RETURN_DO_MARK_CPEI_STALE indicates that the caller should perform a stale mark immediately.
 * Value to use is staleCPEI in LocateROMClassResult
 * @return LOCATE_ROMCLASS_RETURN_DO_TRY_WAIT 4 indicates that the caller should try waiting for an update as another
 * JVM is likely in the middle of adding the class
 * @return LOCATE_ROMCLASS_RETURN_MARKED_ITEM_STALE indicates an out of date class was found and it was marked stale. If
 * the read mutex was held, it is released.
 * @return LOCATE_ROMCLASS_RETURN_FOUND_SHADOW indicates that the class was found, but a shadow has appeared earlier in
 * the classpath which has made it invalid
 */
/* THREADING: This function can be called multi-threaded */
UDATA
SH_ROMClassManagerImpl::locateROMClass(J9VMThread* currentThread, const char* path, U_16 pathLen, ClasspathItem* cp,
    ...
    const J9UTF8* partition, const J9UTF8* modContext, LocateROMClassResult* result)
{
    ...
    if (getState() != MANAGER_STATE_STARTED) {
        ...
        Trc_SHR_RMI_locateROMClass_ManagerNotInStartedState_Exception(
            currentThread, pathLen, path, callerHelperID, cpeIndex);
    ...
    do {
        ...
        if (TYPE_ORPHAN == ITEMTYPE(currentItem)) {
            ...
            if (!(_cache->isStale(currentItem))) {
                ...
                /* If we have a cachedROMClass (ie. the exact ROMClass we want is already in the cache) we can use that
                ...
                /* If we have been given a cpeIndex, we can use that to eliminate non-matches.
                        Note that, even if we have found a cachedROMClass match, we still have to validate classpaths,
                ...
                if (cpeIndex >= 0) {
                    ...
                    if (ClasspathItem::compare(currentThread->javaVM->internalVMFunctions, storedAt, testCPEI)) {
                        /* If timestamps have changed, fail immediately. Note that timestamp comparisons are only valid
                         * between ClasspathEntryItems in cache. */
                        if ((*_runtimeFlagsPtr & J9SHR_RUNTIMEFLAG_ENABLE_TIMESTAMP_CHECKS)
                            && (storedAt->timestamp != testCPEI->timestamp)) {
                            result->staleCPEI = storedAt;
                            /* trace event is at level 1 and trace exit message is at level 2 as per CMVC 155318/157683
                             */
                            Trc_SHR_RMI_locateROMClass_TimestampMismatch_Event(currentThread, storedAt->timestamp,
                                testCPEI->timestamp, pathLen, path, callerHelperID, cpeIndex);
                            Trc_SHR_RMI_locateROMClass_ExitTimestampMismatch(
                                currentThread, storedAt->timestamp, testCPEI->timestamp);
                ...
                if ((partition != NULL) || (modContext != NULL) || ITEMTYPE(currentItem) == TYPE_SCOPED_ROMCLASS) {
                    ...
                    if (!localSCM) {
                        if (_cache->getAndStartManagerForType(currentThread, TYPE_SCOPE, (SH_Manager**)&localSCM)
                            != TYPE_SCOPE) {
                            goto _notFound;
                        }
                    }
                ...
                if (cp->isInCache()) {
                    ...
                    if (cpInCache == cp) {
                        Trc_SHR_RMI_locateROMClass_FoundCacheClasspath(
                            currentThread, wrapper, cpeIndex, result->staleCPEI);
                        match = wrapper;
                    ...
                    if (!localCPM) {
                        if (_cache->getAndStartManagerForType(currentThread, TYPE_CLASSPATH, (SH_Manager**)&localCPM)
                            != TYPE_CLASSPATH) {
                            goto _notFound;
                        }
                    }
                    if (localCPM->validate(
                            currentThread, wrapper, cp, confirmedEntries, &localFoundAtIndex, &(result->staleCPEI))) {
                        Trc_SHR_RMI_locateROMClass_ValidateSucceeded(
                            currentThread, wrapper, localFoundAtIndex, result->staleCPEI);
                        match = wrapper;
                    }
                }
                ...
                if (match) {
                    if ((cp->getType() != CP_TYPE_TOKEN)
                        ...
                        if (match->timestamp != 0 && checkTimestamp(currentThread, path, pathLen, match, walk->_item)) {
                            ...
                            /* trace event is at level 1 and trace exit message is at level 2 as per CMVC 155318/157683
                             */
                            Trc_SHR_RMI_locateROMClass_TimestampChanged_Event(
                                currentThread, pathLen, path, callerHelperID, cpeIndex);
                        ...
                        if (!localCPM) {
                            if (_cache->getAndStartManagerForType(
                                    currentThread, TYPE_CLASSPATH, (SH_Manager**)&localCPM)
                                != TYPE_CLASSPATH) {
                                goto _notFound;
                            }
                        }
                        if (localCPM->touchForClassFiles(currentThread, path, pathLen, cp, localFoundAtIndex)) {
                            /* trace event is at level 1 and trace exit message is at level 2 as per CMVC 155318/157683
                             */
                            Trc_SHR_RMI_locateROMClass_FoundShadowClass_Event(
                                currentThread, pathLen, path, callerHelperID, cpeIndex);
                    ...
                    if (!result->staleCPEI) {
                        ...
                        Trc_SHR_RMI_locateROMClass_ExitSuccess(
                            currentThread, match, localFoundAtIndex, result->staleCPEI);
        ...
        if (result->staleCPEI) {
            /* trace event is at level 1 and trace exit message is at level 2 as per CMVC 155318/157683  */
            Trc_SHR_RMI_locateROMClass_ExitTValidateFoundStale_Event(
                currentThread, pathLen, path, callerHelperID, cpeIndex);
 ...
 */
bool SH_ROMClassManagerImpl::checkTimestamp(
    J9VMThread* currentThread, const char* path, UDATA pathLen, ROMClassWrapper* rcw, const ShcItem* item)
{
...
bool SH_ROMClassManagerImpl::canCreateHints() { return true; }
 ...
 */
IDATA
SH_ROMClassManagerImpl::primeHashtables(
