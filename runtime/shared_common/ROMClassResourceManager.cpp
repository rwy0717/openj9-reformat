...
J9HashTable* SH_ROMClassResourceManager::localHashTableCreate(J9VMThread* currentThread, U_32 initialEntries)
{
    J9HashTable* returnVal;

    Trc_SHR_RRM_localHashTableCreate_Entry(currentThread, initialEntries);
    returnVal = hashTableNew(OMRPORT_FROM_J9PORT(_portlib), (char*)_rrmHashTableName, initialEntries,
        sizeof(HashTableEntry), sizeof(char*), 0, J9MEM_CATEGORY_CLASSES, SH_ROMClassResourceManager::rrmHashFn,
        SH_ROMClassResourceManager::rrmHashEqualFn, NULL, (void*)currentThread->javaVM->internalVMFunctions);
...
SH_ROMClassResourceManager::HashTableEntry::~HashTableEntry() { /* Nothing to do */ }

SH_ROMClassResourceManager::HashTableEntry* SH_ROMClassResourceManager::rrmTableAddHelper(
    J9VMThread* currentThread, HashTableEntry* newEntry, SH_CompositeCache* cachelet)
{
...
SH_ROMClassResourceManager::HashTableEntry* SH_ROMClassResourceManager::rrmTableAdd(
    J9VMThread* currentThread, const ShcItem* item, SH_CompositeCache* cachelet)
{
 ...
 */
SH_ROMClassResourceManager::HashTableEntry* SH_ROMClassResourceManager::rrmTableAddPrime(
    J9VMThread* currentThread, UDATA key, SH_CompositeCache* cachelet)
{
...
SH_ROMClassResourceManager::HashTableEntry* SH_ROMClassResourceManager::rrmTableLookup(
    J9VMThread* currentThread, UDATA key)
{
 ...
 */
bool SH_ROMClassResourceManager::storeNew(
 ...
 */
const void* SH_ROMClassResourceManager::findResource(J9VMThread* currentThread, UDATA resourceKey)
{
...
bool SH_ROMClassResourceManager::permitAccessToResource(J9VMThread* currentThread) { return _accessPermitted; }
...
void SH_ROMClassResourceManager::getKeyAndItemForHashtableEntry(void* entry, UDATA* key, const ShcItem** item)
{
...
UDATA
SH_ROMClassResourceManager::getDataBytes() { return _dataBytes; }

#if defined(J9SHR_CACHELET_SUPPORT)

bool SH_ROMClassResourceManager::canCreateHints() { return false; }
 ...
 */
IDATA
SH_ROMClassResourceManager::rrmCollectHashes(
    J9VMThread* currentThread, SH_CompositeCache* cachelet, CacheletHints* hints)
{
...
void SH_ROMClassResourceManager::fixupHintsForSerialization(J9VMThread* vmthread, UDATA dataType, UDATA dataLen,
    U_8* data, IDATA deployedOffset, void* serializedROMClassStartAddress)
{
 ...
 */
IDATA
SH_ROMClassResourceManager::createHintsForCachelet(
 ...
 */
IDATA
SH_ROMClassResourceManager::primeHashtables(
