...
j9shr_stringTransaction_start(void* tobj, J9VMThread* currentThread)
{
    ...
    readOnly = J9_ARE_ANY_BITS_SET(
        localRuntimeFlags, J9SHR_RUNTIMEFLAG_BLOCK_SPACE_FULL | J9SHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL);
    ...
    if (table != NULL) {
        ...
        if (cachemap->enterStringTableMutex(currentThread, readOnly, &doRebuildLocalData, &doRebuildCacheData) != 0) {
            Trc_SHR_API_j9shr_stringTransaction_start_STLock_Event(currentThread, localRuntimeFlags,
                cachemap->getStringTableBytes(), readOnly, doRebuildLocalData, doRebuildCacheData);
            retval = -1;
            goto done;
        }
...
j9shr_stringTransaction_stop(void* tobj)
{
    ...
    if ((obj->transactionState != TSTATE_ENTER_READWRITEMUTEX_WITHOUT_WRITEMUTEX)
        && (obj->transactionState != TSTATE_ENTER_READWRITEMUTEX) && (obj->transactionState != TSTATE_ENTER_WRITEMUTEX)
        && (obj->transactionState != TSTATE_ENTER_SEGMENTMUTEX) && (obj->transactionState != TSTATE_STARTED)) {
    ...
    if ((TSTATE_ENTER_READWRITEMUTEX_WITHOUT_WRITEMUTEX == obj->transactionState)
         ...
         */
        if (((SH_CacheMap*)currentThread->javaVM->sharedClassConfig->sharedClassCache)
                ->exitStringTableMutex(currentThread, J9SHR_STRING_POOL_OK)
            != 0) {
    ...
    if ((TSTATE_ENTER_READWRITEMUTEX == obj->transactionState) || (TSTATE_ENTER_WRITEMUTEX == obj->transactionState)) {
 ...
 */
IDATA
j9shr_classStoreTransaction_start(void* tobj, J9VMThread* currentThread, J9ClassLoader* classloader,
    J9ClassPathEntry* classPathEntries, UDATA cpEntryCount, UDATA entryIndex, UDATA loadType, const J9UTF8* partition,
    U_16 classnameLength, U_8* classnameData, BOOLEAN isModifiedClassfile, BOOLEAN takeReadWriteLock)
{
    ...
    Trc_SHR_API_j9shr_classStoreTransaction_start_Entry((UDATA)currentThread, (UDATA)classloader,
        (UDATA)classPathEntries, cpEntryCount, entryIndex, loadType, (UDATA)partition, (UDATA)classnameLength,
        classnameData, (UDATA)isModifiedClassfile, (UDATA)takeReadWriteLock);
    ...
    /*Note:
     * - transactionState, oldVMState, & ownerThread must be set as early as possible (b/c they are used in
    ...
    if ((loadType == J9SHR_LOADTYPE_REDEFINED)
        || ((loadType == J9SHR_LOADTYPE_RETRANSFORMED)
               && (localRuntimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_CACHERETRANSFORMED) == 0)) {
        Trc_SHR_API_j9shr_classStoreTransaction_start_Loadtype_Event(
            currentThread, loadType, (UDATA)classnameLength, classnameData);
        retval = -1;
        goto done;
    }

    if (localRuntimeFlags & J9SHR_RUNTIMEFLAG_DENY_CACHE_ACCESS) {
        Trc_SHR_API_j9shr_classStoreTransaction_start_DenyAccess_Event(
            currentThread, localRuntimeFlags, classnameLength, classnameData);
        retval = -1;
        goto done;
    }
    ...
    if ((entryIndex > MAX_CLASS_PATH_ENTRIES) && (entryIndex != UDATA_MAX)) {
        Trc_SHR_API_j9shr_classStoreTransaction_start_CPIndex_Event(
            currentThread, entryIndex, (UDATA)classnameLength, classnameData);
        retval = -1;
        goto done;
    }

    if (NULL == cachemap->getROMClassManager(currentThread)) {
        Trc_SHR_API_j9shr_classStoreTransaction_start_RCMNotStarted_Event(
    ...
    if (!(localRuntimeFlags & J9SHR_RUNTIMEFLAG_CACHE_INITIALIZATION_COMPLETE)) {
        Trc_SHR_API_j9shr_classStoreTransaction_start_NotInit_Event(
            currentThread, localRuntimeFlags, (UDATA)classnameLength, classnameData);
        retval = -1;
        goto done;
    }

    if (partition != NULL) {
        Trc_SHR_API_j9shr_classStoreTransaction_start_NonNullPartition_Event(
            currentThread, J9UTF8_LENGTH(partition), J9UTF8_DATA(partition));
    }
    ...
    /*NOTE:
     * Must omrthread_monitor_enter(segmentMutex) before enterWriteMutex(). Otherwise we will deadlock with
     * hookFindSharedClass()
     */
    if (omrthread_monitor_enter(vm->classMemorySegments->segmentMutex) != 0) {
        Trc_SHR_API_j9shr_classStoreTransaction_start_SegMutex_Event(
            currentThread, (UDATA)classnameLength, classnameData);
        retval = -1;
        goto done;
    }
    ...
    if (sconfig->classnameFilterPool) {
        if (checkForStoreFilter(
                vm, classloader, (const char*)classnameData, (UDATA)classnameLength, sconfig->classnameFilterPool)) {
            Trc_SHR_API_j9shr_classStoreTransaction_start_StoreFilt_Event(
                currentThread, (UDATA)classnameLength, classnameData);
            retval = -1;
            goto done;
        }
    ...
    if (classloader != NULL) {
        ...
        if ((J2SE_VERSION(vm) >= J2SE_19) || ((NULL != classPathEntries) && (-1 != obj->entryIndex))) {
            ...
            if (!classpath && !infoFound) {
                ...
                if (J2SE_VERSION(vm) >= J2SE_19) {
                    if (classloader == vm->systemClassLoader) {
                        ...
                        /* bootstrap loader searched on path: modulePath:classPathEntries, entryIndex is the index of
                         * classPathEntries if loaded from classPathEntries
            ...
            if (!classpath) {
                ...
                if ((NULL != classPathEntries) || (classloader == vm->systemClassLoader)) {
                    classpath
                        = createClasspath(currentThread, classPathEntries, cpEntryCount, helperID, cpType, infoFound);
                    if (NULL == classpath) {
                        retval = -1;
                        goto done;
                    }
    ...
    /*Take a write mutex on the shared cache. This will be released in when the transaction is stopped.*/
    if (cachemap->startClassTransaction(currentThread, false, fname) != 0) {
        Trc_SHR_API_j9shr_classStoreTransaction_start_WriteLock_Event(
            currentThread, (UDATA)classnameLength, classnameData);
        retval = -1;
        goto done;
    }
    ...
    if (NULL != classpath) {
        ...
        bool readOnly = (localRuntimeFlags & (J9SHR_RUNTIMEFLAG_ENABLE_READONLY | J9SHR_RUNTIMEFLAG_DENY_CACHE_UPDATES))
            ? true
            : false;
         ...
         */
        if (true == readOnly) {
            /* We've already identified that an element of this classpath is stale. Sharing is therefore disabled for
             * this classpath. */
            if (classpath->flags & MARKED_STALE_FLAG) {
                Trc_SHR_API_j9shr_classStoreTransaction_start_CPIsStale_Event(
                    currentThread, classpath->flags, (UDATA)classnameLength, classnameData);
                retval = -1;
                goto done;
            }
        } else {
            if ((cpw = cachemap->updateClasspathInfo(currentThread, classpath, obj->entryIndex, partition,
                     &cachedPartition, modContext, &cachedModContext, true /*we own the write area lock*/))
                == NULL) {
                /* Likely cache is full or read failure updating hashtables */
                Trc_SHR_API_j9shr_classStoreTransaction_start_updateCPInfo_Event(
                    currentThread, obj->entryIndex, (UDATA)classnameLength, classnameData);
                retval = -1;
                goto done;
            }
...
j9shr_classStoreTransaction_stop(void* tobj)
{
    ...
    if (obj->transactionState != TSTATE_ENTER_SEGMENTMUTEX && obj->transactionState != TSTATE_ENTER_READWRITEMUTEX
        && obj->transactionState != TSTATE_ENTER_WRITEMUTEX) {
    ...
    releaseWriteMutex
        = (obj->transactionState == TSTATE_ENTER_WRITEMUTEX || obj->transactionState == TSTATE_ENTER_READWRITEMUTEX);
    releaseSegmentMutex = (obj->transactionState == TSTATE_ENTER_SEGMENTMUTEX
        || obj->transactionState == TSTATE_ENTER_READWRITEMUTEX || obj->transactionState == TSTATE_ENTER_WRITEMUTEX);
    ...
    if (releaseWriteMutex == true) {
        if ((NULL == obj->newItemInCache) && (NULL == obj->findNextRomClass)) {
            Trc_SHR_API_j9shr_classStoreTransaction_stop_NoWork_Event(
                currentThread, (UDATA)obj->classnameLength, obj->classnameData);
            ...
            Trc_SHR_Assert_False(
                ((NULL != obj->allocatedLineNumberTable) || (NULL != obj->allocatedLocalVariableTable)));
            ...
            if (obj->newItemInCache != NULL && cpw != NULL && modifiedNoContext == false) {
                ...
                didWeStore = cachemap->commitROMClass(obj->ownerThread, (ShcItem*)obj->newItemInCache,
                    (SH_CompositeCacheImpl*)obj->cacheAreaForAllocate, (ClasspathWrapper*)cpw, obj->entryIndex,
                    obj->partitionInCache, obj->modContextInCache, (BlockPtr)obj->allocatedMem, true);
            } else if (obj->newItemInCache != NULL) {
                if (modifiedNoContext == true) {
                    Trc_SHR_API_j9shr_classStoreTransaction_stop_StoreModifed_Event(
                        currentThread, (UDATA)obj->classnameLength, obj->classnameData, (UDATA)storedClass);
                }
                storedClass = (J9ROMClass*)obj->allocatedMem;
                didWeStore = cachemap->commitOrphanROMClass(obj->ownerThread, (ShcItem*)obj->newItemInCache,
                    (SH_CompositeCacheImpl*)obj->cacheAreaForAllocate, (ClasspathWrapper*)cpw,
                    (BlockPtr)obj->allocatedMem);
            } else {
                storedClass = (J9ROMClass*)obj->findNextRomClass;
                if (modifiedNoContext == true) {
                    Trc_SHR_API_j9shr_classStoreTransaction_stop_NoNewMetaDataForModBytes_Event(
                        currentThread, (UDATA)obj->classnameLength, obj->classnameData, (UDATA)storedClass);
                } else if (NULL == cpw) {
                    Trc_SHR_API_j9shr_classStoreTransaction_stop_NoNewMetaDataNoCPInfo_Event(
                        currentThread, (UDATA)obj->classnameLength, obj->classnameData, (UDATA)storedClass);
                } else {
                    didWeStore = cachemap->commitMetaDataROMClassIfRequired(obj->ownerThread, (ClasspathWrapper*)cpw,
                        obj->entryIndex, obj->helperID, obj->partitionInCache, obj->modContextInCache,
                        (J9ROMClass*)obj->findNextRomClass);
                }
            }
            ...
            if (NULL != storedClass) {
                ...
                if (NULL != intermediateClassData) {
                    Trc_SHR_Assert_True(
                        j9shr_isAddressInCache(vm, intermediateClassData, storedClass->intermediateClassDataLength));
                }
            }
        }
    }
    ...
    if (cpw != NULL) {
        ...
        storeClassVerboseIO(obj->ownerThread, classpath, obj->entryIndex, obj->classnameLength, obj->classnameData,
            obj->helperID, (didWeStore == 1));
    }
    ...
    if (retval != SCCLASS_STORE_STOP_ERROR && didWeStore == SCCLASS_STORE_STOP_DATA_STORED) {
        Trc_SHR_API_j9shr_classStoreTransaction_stop_StoredData_Event(
            currentThread, (UDATA)J9UTF8_LENGTH(storedClassName), J9UTF8_DATA(storedClassName), storedClass);
...
J9ROMClass* j9shr_classStoreTransaction_nextSharedClassForCompare(void* tobj)
{
    ...
    obj->findNextRomClass = (J9ROMClass*)cachemap->findNextROMClass(
        currentThread, obj->findNextIterator, obj->firstFound, obj->classnameLength, (const char*)obj->classnameData);
    ...
    void* tobj, const J9RomClassRequirements* sizes, J9SharedRomClassPieces* pieces)
{
    ...
    Trc_SHR_API_j9shr_createSharedClass_Entry3(currentThread, classnameLength, classnameData,
        sizes->romClassSizeFullSize, sizes->romClassMinimalSize, sizes->lineNumberTableSize,
        sizes->localVariableTableSize);
    ...
    if (J9_ARE_ANY_BITS_SET(localRuntimeFlags, J9SHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL)) {
        /* Put this block after the checks for J9SHR_RUNTIMEFLAG_BLOCK_SPACE_FULL and
         * J9SHR_RUNTIMEFLAG_DENY_CACHE_UPDATES, as there is no point increasing the unstored bytes if these flags are
         * set
         */
    ...
    if ((0 == sizes->romClassSizeFullSize) || (0 == sizes->romClassMinimalSize)) {
        ...
        Trc_SHR_API_j9shr_createSharedClass_ZeroRomClassSize_Event(
            currentThread, sizes->romClassSizeFullSize, sizes->romClassMinimalSize);
    ...
    allocatedRomClass = cachemap->allocateROMClass(currentThread, sizes, pieces, obj->classnameLength,
        (const char*)(obj->classnameData), (ClasspathWrapper*)obj->ClasspathWrapper, obj->partitionInCache,
        obj->modContextInCache, obj->helperID, modifiedNoContext, obj->newItemInCache, obj->cacheAreaForAllocate);
     ...
     */
    if (((localRuntimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_READONLY) == 0) && (obj->takeReadWriteLock == 1)
        ...
        if (cachemap->enterStringTableMutex(currentThread, FALSE, &doRebuildLocalData, &doRebuildCacheData) != 0) {
            Trc_SHR_API_j9shr_createSharedClass_STLock_Event(currentThread, localRuntimeFlags,
                cachemap->getStringTableBytes(), FALSE, doRebuildLocalData, doRebuildCacheData,
                (UDATA)obj->classnameLength, (const char*)obj->classnameData);
        } else {
            if (doRebuildCacheData) {
                j9shr_resetSharedStringTable(currentThread->javaVM);
            }
...
j9shr_classStoreTransaction_updateSharedClassSize(void* tobj, U_32 sizeUsed)
{
    ...
    if (obj->transactionState != TSTATE_ENTER_WRITEMUTEX && obj->transactionState != TSTATE_ENTER_READWRITEMUTEX) {
        Trc_SHR_API_j9shr_updateSharedClassSize_NotStarted_Event(
            currentThread, (UDATA)obj->classnameLength, obj->classnameData);
        retval = -1;
        goto done;
    }
    ...
    if ((sizeUsed % SHC_DOUBLEALIGN) != 0) {
        ...
        Trc_SHR_API_j9shr_updateSharedClassSize_DblAlign_Event(
            currentThread, (UDATA)obj->classnameLength, obj->classnameData);
        goto done;
    }
    ...
    if (NULL == obj->newItemInCache) {
        Trc_SHR_API_j9shr_updateSharedClassSize_NotCalled1_Event(
            currentThread, (UDATA)obj->classnameLength, obj->classnameData);
        retval = -1;
        goto done;
    }
    if (NULL == obj->cacheAreaForAllocate) {
        Trc_SHR_API_j9shr_updateSharedClassSize_NotCalled2_Event(
            currentThread, (UDATA)obj->classnameLength, obj->classnameData);
        retval = -1;
        goto done;
    }

    Trc_SHR_API_j9shr_updateSharedClassSize_Update_Event(
        currentThread, (UDATA)obj->classnameLength, obj->classnameData, sizeUsed);
 ...
 */
J9ROMClass* j9shr_jclUpdateROMClassMetaData(J9VMThread* currentThread, J9ClassLoader* classloader,
    ...
    const J9ROMClass* existingClass)
{
    ...
    Trc_SHR_API_j9shr_jclUpdateROMClassMetaData_Entry((UDATA)currentThread, (UDATA)classloader, (UDATA)classPathEntries,
        cpEntryCount, entryIndex, (UDATA)partition, (UDATA)classnameLength, classnameData);
    ...
    if ((sconfig->runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_READONLY) != 0) {
         ...
         */
        Trc_SHR_API_j9shr_jclUpdateROMClassMetaData_CacheIsReadonly_Event(
            currentThread, (UDATA)classnameLength, classnameData, (UDATA)sconfig->runtimeFlags);
        goto done;
    }

    if (cachemap->isAddressInROMClassSegment((BlockPtr)existingClass) == false) {
        Trc_SHR_API_j9shr_jclUpdateROMClassMetaData_NotInCache_Event(
            currentThread, (UDATA)classnameLength, classnameData, (UDATA)existingClass);
        goto done;
    }
    ...
    if ((NULL == sconfig->modContext) && (TRUE == isModifiedClassfile)) {
         ...
         */
        Trc_SHR_API_j9shr_jclUpdateROMClassMetaData_ModifiedByteCodes_Event(currentThread, (UDATA)classnameLength,
            classnameData, (UDATA)sconfig->modContext, (UDATA)isModifiedClassfile);
        goto done;
    }

    if (j9shr_classStoreTransaction_start((void*)&tobj, currentThread, classloader, classPathEntries, cpEntryCount,
            entryIndex, J9SHR_LOADTYPE_NORMAL, partition, classnameLength, classnameData, isModifiedClassfile, FALSE)
        != 0) {
        Trc_SHR_API_j9shr_jclUpdateROMClassMetaData_StartFailed_Event(
            currentThread, (UDATA)classnameLength, classnameData);
...
callstopAndExit:
    ...
    if (didWeStoreAnything == SCCLASS_STORE_STOP_ERROR) {
        Trc_SHR_API_j9shr_jclUpdateROMClassMetaData_StopFailed_Event(
            currentThread, (UDATA)classnameLength, classnameData);
        goto done;
    }
    ...
    if (didWeStoreAnything == SCCLASS_STORE_STOP_DATA_STORED) {
        ...
        if (didWeStoreAnything == SCCLASS_STORE_STOP_NOTHING_STORED) {
            Trc_SHR_API_j9shr_jclUpdateROMClassMetaData_ExitNoStore(
                currentThread, (UDATA)classnameLength, classnameData);
        } else {
