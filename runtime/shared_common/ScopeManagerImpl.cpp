...
J9HashTable* SH_ScopeManagerImpl::localHashTableCreate(J9VMThread* currentThread, U_32 initialEntries)
{
    J9HashTable* returnVal;

    Trc_SHR_SMI_localHashTableCreate_Entry(currentThread, initialEntries);
    returnVal = hashTableNew(OMRPORT_FROM_J9PORT(_portlib), J9_GET_CALLSITE(), initialEntries, sizeof(HashEntry),
        sizeof(char*), 0, J9MEM_CATEGORY_CLASSES, SH_ScopeManagerImpl::scHashFn, SH_ScopeManagerImpl::scHashEqualFn,
        NULL, (void*)currentThread->javaVM->internalVMFunctions);
    Trc_SHR_SMI_localHashTableCreate_Exit(currentThread, returnVal);
    return returnVal;
}

IDATA
SH_ScopeManagerImpl::localInitializePools(J9VMThread* currentThread) { return 0; }

void SH_ScopeManagerImpl::localTearDownPools(J9VMThread* currentThread) {}
...
const SH_ScopeManagerImpl::HashEntry* SH_ScopeManagerImpl::scTableAdd(
...
const J9UTF8* SH_ScopeManagerImpl::scTableLookup(J9VMThread* currentThread, const J9UTF8* key)
{
 ...
 */
SH_ScopeManagerImpl* SH_ScopeManagerImpl::newInstance(
    J9JavaVM* vm, SH_SharedCache* cache_, SH_ScopeManagerImpl* memForConstructor)
{
...
void SH_ScopeManagerImpl::initialize(J9JavaVM* vm, SH_SharedCache* cache_, BlockPtr memForConstructor)
{
...
U_32 SH_ScopeManagerImpl::getHashTableEntriesFromCacheSize(UDATA cacheSizeBytes)
{
 ...
 */
bool SH_ScopeManagerImpl::storeNew(J9VMThread* currentThread, const ShcItem* itemInCache, SH_CompositeCache* cachelet)
{
 ...
 */
const J9UTF8* SH_ScopeManagerImpl::findScopeForUTF(J9VMThread* currentThread, const J9UTF8* localScope)
{
 ...
 */
IDATA
SH_ScopeManagerImpl::validate(
    J9VMThread* currentThread, const J9UTF8* partition, const J9UTF8* modContext, const ShcItem* match)
{
    ...
    } else if (scopedMatch->partitionOffset) {
    ...
    } else if (scopedMatch->modContextOffset) {
 ...
 */
IDATA
SH_ScopeManagerImpl::primeHashtables(
