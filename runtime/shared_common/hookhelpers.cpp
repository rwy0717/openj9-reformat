...
#define HOOKHELPERS_ERR_TRACE(verbose, var) \
    if (verbose)                            \
    j9nls_printf(PORTLIB, J9NLS_ERROR, var)
#define HOOKHELPERS_TRACE5_NOTAG(verbose, var, p1, p2, p3, p4, p5) \
    if (verbose)                                                   \
    j9nls_printf(PORTLIB, J9NLS_DO_NOT_PRINT_MESSAGE_TAG, var, p1, p2, p3, p4, p5)
#define HOOKHELPERS_TRACE6_NOTAG(verbose, var, p1, p2, p3, p4, p5, p6) \
    if (verbose)                                                       \
    j9nls_printf(PORTLIB, J9NLS_DO_NOT_PRINT_MESSAGE_TAG, var, p1, p2, p3, p4, p5, p6)
#define HOOKHELPERS_TRACE_NOTAG(verbose, var) \
    if (verbose)                              \
    j9nls_printf(PORTLIB, J9NLS_DO_NOT_PRINT_MESSAGE_TAG, var)

static bool makeClasspathItems(
    J9JavaVM* vm, J9ClassPathEntry* classPathEntries, I_16 entryCount, ClasspathItem* classpath, bool bootstrap);
    ...
    J9JavaVM* vm, J9ClassPathEntry* classPathEntries, I_16 entryCount, ClasspathItem* classpath, bool bootstrap)
{
    ...
    if (bootstrap && (J2SE_VERSION(vm) >= J2SE_19)) {
        ...
        if ((classpath)->addItem(
                vm->internalVMFunctions, (const char*)modulePath->path, modulePath->pathLength, prototype)
            < 0) {
            ret = false;
        }
        if ((NULL != vm->sharedClassConfig) && (ignoreStateChange)) {
            J9VMThread* currentThread = vm->internalVMFunctions->currentVMThread(vm);

            ((SH_CacheMap*)(vm->sharedClassConfig->sharedClassCache))
                ->notifyClasspathEntryStateChange(
                    currentThread, (const char*)modulePath->path, J9ZIP_STATE_IGNORE_STATE_CHANGES);
        }
    }
    ...
    for (I_16 i = 0; i < entryCount; i++) {
        ...
        if ((classpath)->addItem(vm->internalVMFunctions, (const char*)classPathEntries[i].path,
                classPathEntries[i].pathLength, prototype)
            < 0) {
        ...
        if (ignoreStateChange) {
            ...
            ((SH_CacheMap*)(vm->sharedClassConfig->sharedClassCache))
                ->notifyClasspathEntryStateChange(
                    currentThread, (const char*)classPathEntries[i].path, J9ZIP_STATE_IGNORE_STATE_CHANGES);
        }
    }
done:
    return ret;
    ...
    J9JavaVM* vm, J9ClassLoader* classloader, const char* classname, UDATA classnameLen, J9Pool* filterPool)
{
    ...
    while (anElement) {
        if (anElement->classloader == classloader) {
            if ((anElement->classnameLen == classnameLen)
                && (strncmp(anElement->classname, classname, classnameLen) == 0)) {
    ...
    const U_8* classnameData, UDATA helperID, BOOLEAN didWeStore)
{
    ...
    if (classpath != NULL && (sconfig->verboseFlags & J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_IO)) {
        ...
        if (classpathItemType == CP_TYPE_CLASSPATH) {
            ...
            if (didWeStore) {
                HOOKHELPERS_TRACE6_NOTAG(sconfig->verboseFlags, J9NLS_SHRC_SHRINIT_STORED_VERBOSE_MSG, classnameLength,
                    classnameData, helperID, vPathLen, vPath, entryIndex);
            } else {
                HOOKHELPERS_TRACE6_NOTAG(sconfig->verboseFlags, J9NLS_SHRC_SHRINIT_STORE_FAILED_VERBOSE_MSG,
                    classnameLength, classnameData, helperID, vPathLen, vPath, entryIndex);
            }
        } else {
            ...
            if (classpathItemType == CP_TYPE_URL) {
                if (didWeStore) {
                    HOOKHELPERS_TRACE5_NOTAG(sconfig->verboseFlags, J9NLS_SHRC_SHRINIT_STORED_VERBOSE_URL_MSG,
                        classnameLength, classnameData, helperID, vPathLen, vPath);
                } else {
                    HOOKHELPERS_TRACE5_NOTAG(sconfig->verboseFlags, J9NLS_SHRC_SHRINIT_STORE_FAILED_VERBOSE_URL_MSG,
                        classnameLength, classnameData, helperID, vPathLen, vPath);
                }
            } else if (classpathItemType == CP_TYPE_TOKEN) {
                if (didWeStore) {
                    HOOKHELPERS_TRACE5_NOTAG(sconfig->verboseFlags, J9NLS_SHRC_SHRINIT_STORED_VERBOSE_TOKEN_MSG,
                        classnameLength, classnameData, helperID, vPathLen, vPath);
                } else {
                    HOOKHELPERS_TRACE5_NOTAG(sconfig->verboseFlags, J9NLS_SHRC_SHRINIT_STORE_FAILED_VERBOSE_TOKEN_MSG,
                        classnameLength, classnameData, helperID, vPathLen, vPath);
                }
            }
        }
    ...
    IDATA helperID, U_16 cpType, UDATA infoFound)
{
    ...
    if (!cpPtr) {
        HOOKHELPERS_ERR_TRACE(
            currentThread->javaVM->sharedClassConfig->verboseFlags, J9NLS_SHRC_SHRINIT_FAILURE_CREATE_CPI_FIND);
        return NULL;
    }
