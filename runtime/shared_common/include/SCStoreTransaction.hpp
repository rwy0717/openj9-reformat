...
public:
     ...
     */
    SCStoreTransaction(J9VMThread* currentThread, J9ClassLoader* classloader, UDATA entryIndex, UDATA loadType,
        U_16 classnameLength, U_8* classnameData, bool isModifiedClassfile, bool isIntermediateROMClass = false)
    {
        if (currentThread != NULL && currentThread->javaVM != NULL
            && currentThread->javaVM->sharedClassConfig != NULL) {
            ...
            if (sharedapi != NULL) {
                ...
                sharedapi->classStoreTransaction_start((void*)&tobj, currentThread, classloader, classPathEntries,
                    cpEntryCount, entryIndex, loadType, NULL, classnameLength, classnameData,
                    ((isModifiedClassfile == true) ? TRUE : FALSE), TRUE);
            }
        } else {
            sharedapi = NULL;
        }
     ...
     */
    bool hasAllocatedFullSizeRomClass(void) { return (0 != (pieces.flags & J9SC_ROMCLASS_PIECES_USED_FULL_SIZE)); }
     ...
     */
    void* getRomClass(void) { return pieces.romClass; }

    /**
     * Get 'LineNumberTable' memory allocated by call to 'allocateSharedClass'
     */
    void* getLineNumberTable(void) { return pieces.lineNumberTable; }

    /**
     * Get 'LocalVariableTable' memory allocated by call to 'allocateSharedClass'
     */
    void* getLocalVariableTable(void) { return pieces.localVariableTable; }
