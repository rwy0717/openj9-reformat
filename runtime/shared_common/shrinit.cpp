 ...
 * - Class name
 * 		In the helper API, UTF bytes/length are obtained from the JNI calls. These are only required for the
 *duration of the find/store call so are not copied. Class names from the bootstrap loader are also UTF bytes/length.
 *		Class name is passed to shrinit, where it is copied into a temporary stack buffer, "fixed up" and
 *null-terminated. Class names are not written into the cache. Instead the UTF8 name in the ROMClass itself is used.
 * - URL/Token/Classpath strings
 *		In the helper API, getPath() is called on URLs and tokens are strings to begin with. UTF bytes/length are
 *obtained for each. These are then copied and null-terminated because J9ClassPathEntrys are only created/allocated
 *once, so they (and their strings) are kept for the lifetime of the JVM. The copied strings are copied into a single
 *buffer (sharedClassConfig->jclStringFarm) which is freed on shutdown. The classpath strings from the bootstrap loader
 *are not copied. It is assumed that these are not going to change or disappear. ClasspathItems created in shrinit use
 *the same strings from the J9ClassPathEntries as we know these are already safe. When a ClasspathItem is serialized
 *into the cache, its strings are no-longer null-terminated and the length is returned with the getPath(...) calls (see
 *ClasspathItem.cpp).
 * - Partition strings
 *		These are treated the same way as the URL/Token/Classpath strings, except that partitions can only ever
 *come in from the jcl helpers.
...
#define SHRINIT_TRACE(verbose, var) \
    if (verbose)                    \
    j9nls_printf(PORTLIB, J9NLS_INFO, var)
#define SHRINIT_TRACE1(verbose, var, p1) \
    if (verbose)                         \
    j9nls_printf(PORTLIB, J9NLS_INFO, var, p1)
#define SHRINIT_TRACE2(verbose, var, p1, p2) \
    if (verbose)                             \
    j9nls_printf(PORTLIB, J9NLS_INFO, var, p1, p2)
#define SHRINIT_TRACE3(verbose, var, p1, p2, p3) \
    if (verbose)                                 \
    j9nls_printf(PORTLIB, J9NLS_INFO, var, p1, p2, p3)
#define SHRINIT_TRACE_NOTAG(verbose, var) \
    if (verbose)                          \
    j9nls_printf(PORTLIB, J9NLS_DO_NOT_PRINT_MESSAGE_TAG, var)
#define SHRINIT_TRACE1_NOTAG(verbose, var, p1) \
    if (verbose)                               \
    j9nls_printf(PORTLIB, J9NLS_DO_NOT_PRINT_MESSAGE_TAG, var, p1)
#define SHRINIT_TRACE2_NOTAG(verbose, var, p1, p2) \
    if (verbose)                                   \
    j9nls_printf(PORTLIB, J9NLS_DO_NOT_PRINT_MESSAGE_TAG, var, p1, p2)
#define SHRINIT_TRACE3_NOTAG(verbose, var, p1, p2, p3) \
    if (verbose)                                       \
    j9nls_printf(PORTLIB, J9NLS_DO_NOT_PRINT_MESSAGE_TAG, var, p1, p2, p3)
#define SHRINIT_TRACE4_NOTAG(verbose, var, p1, p2, p3, p4) \
    if (verbose)                                           \
    j9nls_printf(PORTLIB, J9NLS_DO_NOT_PRINT_MESSAGE_TAG, var, p1, p2, p3, p4)
#define SHRINIT_INFO_TRACE(verbose, var) \
    if (verbose)                         \
    j9nls_printf(PORTLIB, J9NLS_INFO, var)
#define SHRINIT_ERR_TRACE(verbose, var) \
    if (verbose)                        \
    j9nls_printf(PORTLIB, J9NLS_ERROR, var)
#define SHRINIT_ERR_TRACE1(verbose, var, p1) \
    if (verbose)                             \
    j9nls_printf(PORTLIB, J9NLS_ERROR, var, p1)
#define SHRINIT_ERR_TRACE2(verbose, var, p1, p2) \
    if (verbose)                                 \
    j9nls_printf(PORTLIB, J9NLS_ERROR, var, p1, p2)
#define SHRINIT_ERR_TRACE3(verbose, var, p1, p2, p3) \
    if (verbose)                                     \
    j9nls_printf(PORTLIB, J9NLS_ERROR, var, p1, p2, p3)
#define SHRINIT_WARNING_TRACE(verbose, var) \
    if (verbose)                            \
    j9nls_printf(PORTLIB, J9NLS_WARNING, var)
#define SHRINIT_WARNING_TRACE1(verbose, var, p1) \
    if (verbose)                                 \
    j9nls_printf(PORTLIB, J9NLS_WARNING, var, p1)
#define SHRINIT_WARNING_TRACE2(verbose, var, p1, p2) \
    if (verbose)                                     \
    j9nls_printf(PORTLIB, J9NLS_WARNING, var, p1, p2)
#define SHRINIT_WARNING_TRACE3(verbose, var, p1, p2, p3) \
    if (verbose)                                         \
    j9nls_printf(PORTLIB, J9NLS_WARNING, var, p1, p2, p3)
...
#define SHR_ENTER_TABLE_VERIFICATION(table)                                                                         \
    do {                                                                                                            \
        if ((table != NULL)                                                                                         \
            ...
            SHR_TABLE_VERIFY_ASSERT(                                                                                \
                table, ((abstablemonitor->owner == NULL) || (omrthread_monitor_owned_by_self(tablemonitor) == 1))); \
...
J9SharedClassesHelpText J9SHAREDCLASSESHELPTEXT[] = { { OPTION_HELP, J9NLS_SHRC_SHRINIT_HELPTEXT_HELP, 0, 0 },
...
#if !defined(WIN32) && !defined(WIN64)
    { HELPTEXT_CACHEDIRPERM_OPTION, J9NLS_SHRC_SHRINIT_HELPTEXT_CACHEDIRPERM, 0, 0 }, HELPTEXT_NEWLINE,
    ...
    { OPTION_PRINT_SNAPSHOTNAME, 0, 0, J9NLS_SHRC_SHRINIT_HELPTEXT_PRINT_SNAPSHOT_FILENAME }, HELPTEXT_NEWLINE,
#endif
#if !defined(J9SHR_CACHELET_SUPPORT)
...
#endif
    HELPTEXT_NEWLINE, { OPTION_DESTROY, J9NLS_SHRC_SHRINIT_HELPTEXT_DESTROY, 0, 0 },
    { OPTION_DESTROYALL, J9NLS_SHRC_SHRINIT_HELPTEXT_DESTROYALL, 0, 0 }, HELPTEXT_NEWLINE,
#if !defined(J9SHR_CACHELET_SUPPORT)
    { OPTION_RESET, J9NLS_SHRC_SHRINIT_HELPTEXT_RESET, 0, 0 },
#endif
    { HELPTEXT_EXPIRE_OPTION, J9NLS_SHRC_SHRINIT_HELPTEXT_EXPIRE, 0, 0 }, HELPTEXT_NEWLINE,
#if defined(J9ZOS390)
    { HELPTEXT_STORAGE_KEY_EQUALS, 0, 0, J9NLS_SHRC_SHRINIT_HELPTEXT_STORAGE_KEY }, HELPTEXT_NEWLINE,
#endif
    ...
    { OPTION_PRINTORPHANSTATS, 0, 0, J9NLS_SHRC_SHRINIT_HELPTEXT_PRINTORPHANSTATS }, HELPTEXT_NEWLINE,
    ...
    { OPTION_VERBOSE_PAGES, 0, 0, J9NLS_SHRC_SHRINIT_HELPTEXT_VERBOSE_PAGES }, HELPTEXT_NEWLINE,
    { OPTION_SILENT, J9NLS_SHRC_SHRINIT_HELPTEXT_SILENT, 0, 0 },
    { OPTION_NONFATAL, J9NLS_SHRC_SHRINIT_HELPTEXT_NONFATAL, 0, 0 },
    { OPTION_FATAL, J9NLS_SHRC_SHRINIT_HELPTEXT_FATAL, 0, 0 }, { OPTION_NONE, J9NLS_SHRC_SHRINIT_HELPTEXT_NONE2, 0, 0 },
    { OPTION_UTILITIES, J9NLS_SHRC_SHRINIT_HELPTEXT_UTILITIES, 0, 0 }, HELPTEXT_NEWLINE,
#if !defined(J9SHR_CACHELET_SUPPORT)
    ...
    { HELPTEXT_MPROTECTEQUALS_PARTIAL_PAGES_ON_STARTUP_PRIVATE_OPTION, 0, 0,
        J9NLS_SHRC_SHRINIT_HELPTEXT_MPROTECT_PARTIAL_PAGES_ON_STARTUP },
#endif
    ...
    { OPTION_ALLOW_CLASSPATHS, J9NLS_SHRC_SHRINIT_HELPTEXT_ALLOW_CLASSPATHS, 0, 0 }, HELPTEXT_NEWLINE,
    ...
    { HELPTEXT_FIND_AOT_METHODS_OPTION, J9NLS_SHRC_SHRINIT_HELPTEXT_FIND_AOT_METHODS, 0, 0 }, HELPTEXT_NEWLINE,
...
#endif
    HELPTEXT_NEWLINE, { OPTION_CHECK_STRING_TABLE, 0, 0, J9NLS_SHRC_SHRINIT_HELPTEXT_CHECK_STRING_TABLE },
    ...
    { OPTION_ADDTESTJITHINT, 0, 0, J9NLS_SHRC_SHRINIT_HELPTEXT_ADD_JIT_HINTS }, { NULL, 0, 0, 0, 0 } };

J9SharedClassesOptions J9SHAREDCLASSESOPTIONS[] = { { OPTION_MORE_HELP, PARSE_TYPE_EXACT, RESULT_DO_MORE_HELP, 0 },
    ...
    { OPTION_NO_TIMESTAMP_CHECKS, PARSE_TYPE_EXACT, RESULT_DO_REMOVE_RUNTIMEFLAG,
        J9SHR_RUNTIMEFLAG_ENABLE_TIMESTAMP_CHECKS },
    { OPTION_NO_CLASSPATH_CACHEING, PARSE_TYPE_EXACT, RESULT_DO_REMOVE_RUNTIMEFLAG,
        J9SHR_RUNTIMEFLAG_ENABLE_LOCAL_CACHEING },
    { OPTION_NO_REDUCE_STORE_CONTENTION, PARSE_TYPE_EXACT, RESULT_DO_REMOVE_RUNTIMEFLAG,
    ...
    { OPTION_NO_ROUND_PAGES, PARSE_TYPE_EXACT, RESULT_DO_REMOVE_RUNTIMEFLAG,
        J9SHR_RUNTIMEFLAG_ENABLE_ROUND_TO_PAGE_SIZE },
    { OPTION_MPROTECT_EQUALS, PARSE_TYPE_STARTSWITH, RESULT_DO_MPROTECT_EQUALS, 0 },
    { OPTION_CACHERETRANSFORMED, PARSE_TYPE_EXACT, RESULT_DO_ADD_RUNTIMEFLAG,
        J9SHR_RUNTIMEFLAG_ENABLE_CACHERETRANSFORMED },
    { OPTION_NOBOOTCLASSPATH, PARSE_TYPE_EXACT, RESULT_DO_REMOVE_RUNTIMEFLAG,
        J9SHR_RUNTIMEFLAG_ENABLE_CACHEBOOTCLASSES },
    { OPTION_BOOTCLASSESONLY, PARSE_TYPE_EXACT, RESULT_DO_BOOTCLASSESONLY,
    ...
    { OPTION_NO_COREMMAP, PARSE_TYPE_EXACT, RESULT_NO_COREMMAP_SET,
        0 }, /* Only valid on AIX, but will be ignored on other platforms */
    ...
    { OPTION_PERSISTENT, PARSE_TYPE_EXACT, RESULT_DO_ADD_RUNTIMEFLAG,
        J9SHR_RUNTIMEFLAG_ENABLE_PERSISTENT_CACHE | J9SHR_RUNTIMEFLAG_PERSISTENT_KEYWORD },
    ...
    { OPTION_NO_DETECT_NETWORK_CACHE, PARSE_TYPE_EXACT, RESULT_DO_REMOVE_RUNTIMEFLAG,
    ...
    { OPTION_NO_SEMAPHORE_CHECK, PARSE_TYPE_EXACT, RESULT_DO_REMOVE_RUNTIMEFLAG,
    ...
    { OPTION_CHECK_STRING_TABLE, PARSE_TYPE_EXACT, RESULT_DO_ADD_RUNTIMEFLAG,
        J9SHR_RUNTIMEFLAG_ENABLE_STRING_TABLE_CHECK },
    { OPTION_TEST_BAD_BUILDID, PARSE_TYPE_EXACT, RESULT_DO_ADD_RUNTIMEFLAG, J9SHR_RUNTIMEFLAG_ENABLE_TEST_BAD_BUILDID },
    { OPTION_FORCE_DUMP_IF_CORRUPT, PARSE_TYPE_EXACT, RESULT_DO_ADD_RUNTIMEFLAG,
        J9SHR_RUNTIMEFLAG_FORCE_DUMP_IF_CORRUPT },
    ...
    { OPTION_DISABLE_CORRUPT_CACHE_DUMPS, PARSE_TYPE_EXACT, RESULT_DO_ADD_RUNTIMEFLAG,
        J9SHR_RUNTIMEFLAG_DISABLE_CORRUPT_CACHE_DUMPS },
    { OPTION_CACHEDIRPERM_EQUALS, PARSE_TYPE_STARTSWITH, RESULT_DO_CACHEDIRPERM_EQUALS, 0 },
    { OPTION_CHECK_STRINGTABLE_RESET, PARSE_TYPE_EXACT, RESULT_DO_ADD_RUNTIMEFLAG,
        J9SHR_RUNTIMEFLAG_CHECK_STRINGTABLE_RESET_READONLY | J9SHR_RUNTIMEFLAG_CHECK_STRINGTABLE_RESET_READWRITE },
    ...
    { OPTION_INVALIDATE_AOT_METHODS_EQUALS, PARSE_TYPE_STARTSWITH, RESULT_DO_INVALIDATE_AOT_METHODS_EQUALS,
        J9SHR_RUNTIMEFLAG_DO_NOT_CREATE_CACHE },
    { OPTION_REVALIDATE_AOT_METHODS_EQUALS, PARSE_TYPE_STARTSWITH, RESULT_DO_REVALIDATE_AOT_METHODS_EQUALS,
        J9SHR_RUNTIMEFLAG_DO_NOT_CREATE_CACHE },
    { OPTION_FIND_AOT_METHODS_EQUALS, PARSE_TYPE_STARTSWITH, RESULT_DO_FIND_AOT_METHODS_EQUALS,
        J9SHR_RUNTIMEFLAG_DO_NOT_CREATE_CACHE },
    { NULL, 0, 0 } };
...
static void adjustCacheSizes(
    J9PortLibrary* portlib, UDATA verboseFlags, J9SharedClassPreinitConfig* piconfig, U_64 newSize);
static IDATA checkIfCacheExists(J9JavaVM* vm, const char* ctrlDirName, char* cacheDirName, const char* cacheName,
    J9PortShcVersion* versionData, U_32 cacheType);
static bool isClassFromPatchedModule(
    J9VMThread* vmThread, J9Module* j9module, U_8* className, UDATA classNameLength, J9ClassLoader* classLoader);
...
void j9shr_hookZipLoadEvent(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (vm && vm->sharedClassConfig && (event->returnCode == 0)) {
        ((SH_CacheMap*)(vm->sharedClassConfig->sharedClassCache))
            ->notifyClasspathEntryStateChange(currentThread, (const char*)filename, state);
    }
}
...
static void j9shr_print_cache_filename(
    J9JavaVM* vm, const char* cacheDirName, U_64 runtimeFlags, const char* cacheName);

static void reportUtilityNotApplicable(
    J9JavaVM* vm, const char* ctrlDirName, const char* cacheName, UDATA verboseFlags, U_64 runtimeFlags, UDATA command);

static void printRuntimeFlags(J9PortLibrary* portLibrary, U_64 runtimeFlags, UDATA verboseFlags)
{
...
/**
 * This function replaces '\0' with ',' within {} in the <method_specification> string provided for
...
UDATA
parseArgs(J9JavaVM* vm, char* options, U_64* runtimeFlags, UDATA* verboseFlags, char** cacheName, char** modContext,
    char** expireTime, char** ctrlDirName, char** cacheDirPerm, char** methodSpecs, UDATA* printStatsOptions,
    ...
    while (*options) {
            ...
            if (rc) {
                if (J9SHAREDCLASSESOPTIONS[i].parseType == PARSE_TYPE_EXACT) {
                    if (options[compareToLen] == '\0')
                        break;
                } else if (J9SHAREDCLASSESOPTIONS[i].parseType == PARSE_TYPE_STARTSWITH) {
                    if (options[compareToLen] == '\0') {
                    ...
                    break;
                } else if (J9SHAREDCLASSESOPTIONS[i].parseType == PARSE_TYPE_OPTIONAL) {
                    break;
                }
            }
            i++;
        ...
        case RESULT_DO_ADJUST_MAXJITDATA_EQUALS: {
            ...
            if (J9_ARE_ALL_BITS_SET(*runtimeFlags, J9SHR_RUNTIMEFLAG_ENABLE_READONLY)) {
                *runtimeFlags &= ~J9SHR_RUNTIMEFLAG_ENABLE_READONLY;
                SHRINIT_WARNING_TRACE3(verboseFlags, J9NLS_SHRC_SHRINIT_OPTION_IGNORED_WARNING, OPTION_READONLY,
                    J9SHAREDCLASSESOPTIONS[i].option, OPTION_READONLY);
            }
        ...
        case RESULT_DO_MPROTECT_EQUALS:
            ...
            if (0 == strcmp(tempStr, SUB_OPTION_MPROTECT_NONE)) {
                *runtimeFlags &= ~(J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT | J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT_ALL
...
#else
                *runtimeFlags |= J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT | J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT_RW
                    | J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT_PARTIAL_PAGES;
#endif /* defined(J9ZOS390) || defined(AIXPPC) */
                *runtimeFlags &= ~(
                    J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT_ONFIND | J9SHR_RUNTIMEFLAG_MPROTECT_PARTIAL_PAGES_ON_STARTUP);
                tempInt = strlen(SUB_OPTION_MPROTECT_DEF);
            } else if (0 == strcmp(tempStr, SUB_OPTION_MPROTECT_ONFIND)) {
                /* When enabling mprotect=onfind, ensure flags for mprotect=default along with
                 * J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT_PARTIAL_PAGES are also set */
#if defined(J9ZOS390) || defined(AIXPPC)
                /* Calling mprotect on the RW when using persistent caches on AIX and ZOS
                 *  has a severe performance impact on class loading (approx ~20% slower)
                 */
                ...
                *runtimeFlags |= J9SHR_RUNTIMEFLAG_MPROTECT_PARTIAL_PAGES_ON_STARTUP;
                 ...
                 */
#if defined(J9ZOS390)
                /* Moreover, on z/OS partial page protection needs to be enabled since JVM startup (which is flag
                 * J9SHR_RUNTIMEFLAG_MPROTECT_PARTIAL_PAGES_ON_STARTUP), otherwise if this JVM protects partially filled
                 * pages, then any subsequent JVM can crash if it attempts to write to cache during startup phase.
                 */
                *runtimeFlags |= J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT | J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT_PARTIAL_PAGES
            ...
            while (true) {
                ...
                if ((filterLength == sizeof(SUB_OPTION_PRINTSTATS_ALL))
                    ...
                    *printStatsOptions |= PRINTSTATS_SHOW_STARTUPHINT;
                    /* -Xshareclasses:printallstats=<private options> For private options, it is default to print
                     * details. */
        ...
        case RESULT_DO_RESET:
            if (J9_ARE_NO_BITS_SET(*runtimeFlags, J9SHR_RUNTIMEFLAG_SNAPSHOT)) {
                ...
                SHRINIT_WARNING_TRACE3(*verboseFlags, J9NLS_SHRC_SHRINIT_OPTION_IGNORED_WARNING, OPTION_RESET,
                    OPTION_SNAPSHOTCACHE, OPTION_RESET);
#endif /* !defined(WIN32) */
            }
            break;
        ...
        case RESULT_DO_FIND_AOT_METHODS_EQUALS:
             ...
             * However, option values passed within {}, such as {a,b,c} should be treated as one value passed to one
             * option.
             */
            if (true == recoverMethodSpecSeparator(options + strlen(J9SHAREDCLASSESOPTIONS[i].option), optionEnd)) {
                ...
                *methodSpecs = options + strlen(J9SHAREDCLASSESOPTIONS[i].option);
                ...
                SHRINIT_TRACE1_NOTAG(
                    1, J9NLS_SHRC_SHRINIT_HELPTEXT_AOT_METHODS_OPERATION, J9SHAREDCLASSESOPTIONS[i].option);
                SHRINIT_TRACE1_NOTAG(1, J9NLS_SHRC_SHRINIT_HELPTEXT_METHOD_SPEC, J9SHAREDCLASSESOPTIONS[i].option);
                SHRINIT_TRACE2_NOTAG(1, J9NLS_SHRC_SHRINIT_INFOTEXT_METHOD_SPEC, J9SHAREDCLASSESOPTIONS[i].option,
                    J9SHAREDCLASSESOPTIONS[i].option);
                return RESULT_PARSE_FAILED;
            }
            break;
    ...
    if ((*runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_PERSISTENT_CACHE) != 0) {
        ...
        if (1 == coreMmapSet) {
            ...
            if (j9sysinfo_get_env(OPTION_ENVVAR_COREMMAP, infoString, EsMaxPath) == 0) {
                if ((strlen(infoString) == lengthYesSetting)
                    && (j9_cmdla_strnicmp(infoString, OPTION_YES, lengthYesSetting) == 0)) {
                    coreMmapYesSet = true;
                }
            }
...
static void j9shr_dump_help(J9JavaVM* vm, UDATA more)
{
    PORT_ACCESS_FROM_JAVAVM(vm);

    const char* tmpcstr
        = j9nls_lookup_message((J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_SHRINIT_HELP_HEADER, NULL);
    ...
    while (J9SHAREDCLASSESHELPTEXT[i].option != NULL) {
        ...
        if (!(J9SHAREDCLASSESHELPTEXT[i].nlsHelp1 | J9SHAREDCLASSESHELPTEXT[i].nlsMoreHelp1)) {
            ...
            helptext = OMRPORT_FROM_J9PORT(PORTLIB)->nls_lookup_message(OMRPORT_FROM_J9PORT(PORTLIB),
                (J9NLS_INFO | J9NLS_DO_NOT_APPEND_NEWLINE | J9NLS_DO_NOT_PRINT_MESSAGE_TAG),
                (J9SHAREDCLASSESHELPTEXT[i].nlsHelp1), (J9SHAREDCLASSESHELPTEXT[i].nlsHelp2), NULL);
            morehelptext = OMRPORT_FROM_J9PORT(PORTLIB)->nls_lookup_message(OMRPORT_FROM_J9PORT(PORTLIB),
                (J9NLS_INFO | J9NLS_DO_NOT_APPEND_NEWLINE | J9NLS_DO_NOT_PRINT_MESSAGE_TAG),
    ...
    tmpcstr = j9nls_lookup_message(
        (J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_SHRC_SHRINIT_HELP_OTHEROPTS_HEADER, NULL);
    j9file_printf(PORTLIB, J9PORT_TTY_OUT, "%s", tmpcstr);

    tmpcstr = j9nls_lookup_message(
        (J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_EXELIB_INTERNAL_HELP_XSCMX_V1, NULL);
    j9file_printf(PORTLIB, J9PORT_TTY_OUT, "%s", tmpcstr);

    tmpcstr
        = j9nls_lookup_message((J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_EXELIB_INTERNAL_HELP_XSCDMX, NULL);
    j9file_printf(PORTLIB, J9PORT_TTY_OUT, "%s", tmpcstr);

    tmpcstr = j9nls_lookup_message(
        (J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_EXELIB_INTERNAL_HELP_XSCMINAOT, NULL);
    j9file_printf(PORTLIB, J9PORT_TTY_OUT, "%s", tmpcstr);

    tmpcstr = j9nls_lookup_message(
        (J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_EXELIB_INTERNAL_HELP_XSCMAXAOT, NULL);
    j9file_printf(PORTLIB, J9PORT_TTY_OUT, "%s", tmpcstr);

    tmpcstr = j9nls_lookup_message(
        (J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_EXELIB_INTERNAL_HELP_XSCMINJITDATA, NULL);
    j9file_printf(PORTLIB, J9PORT_TTY_OUT, "%s", tmpcstr);

    tmpcstr = j9nls_lookup_message(
        (J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_EXELIB_INTERNAL_HELP_XSCMAXJITDATA, NULL);
    j9file_printf(PORTLIB, J9PORT_TTY_OUT, "%s", tmpcstr);

    tmpcstr = j9nls_lookup_message(
        (J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_EXELIB_INTERNAL_HELP_XZERONOSHAREZIP, NULL);
    j9file_printf(PORTLIB, J9PORT_TTY_OUT, "%s", tmpcstr);

    tmpcstr = j9nls_lookup_message((J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG),
        J9NLS_EXELIB_INTERNAL_HELP_XXSHARED_CACHE_HARD_LIMIT_EQUALS, NULL);
    j9file_printf(PORTLIB, J9PORT_TTY_OUT, "%s", tmpcstr);

    tmpcstr = j9nls_lookup_message(
        (J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_EXELIB_INTERNAL_HELP_XXSHARECLASSESENABLEBCI, NULL);
    j9file_printf(PORTLIB, J9PORT_TTY_OUT, "%s", tmpcstr);

    tmpcstr = j9nls_lookup_message(
        (J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG), J9NLS_EXELIB_INTERNAL_HELP_XXSHARECLASSESDISABLEBCI, NULL);
    j9file_printf(PORTLIB, J9PORT_TTY_OUT, "%s", tmpcstr);
...
static void fixUpString(J9InternalVMFunctions* functionTable, char* nameBuffer, UDATA bufferSize, const char* className,
...
static void killStringFarm(J9PortLibrary* portlib, J9SharedStringFarm* root)
{
...
static void testForBytecodeModification(J9JavaVM* vm)
{
    if (J9_EVENT_IS_HOOKED(vm->hookInterface, J9HOOK_VM_CLASS_LOAD_HOOK)
        || J9_EVENT_IS_HOOKED(vm->hookInterface, J9HOOK_VM_CLASS_LOAD_HOOK2)) {
...
void registerStoreFilter(
    J9JavaVM* vm, J9ClassLoader* classloader, const char* fixedName, UDATA fixedNameSize, J9Pool** filterPoolPtr)
{
    PORT_ACCESS_FROM_JAVAVM(vm);
    ...
    if (*filterPoolPtr == NULL) {
        *filterPoolPtr = pool_new(sizeof(struct ClassNameFilterData), 0, 0, 0, J9_GET_CALLSITE(),
            J9MEM_CATEGORY_CLASSES, POOL_FOR_PORT(vm->portLibrary));
    }
    ...
    if (*filterPoolPtr) {
        ...
        /* Look for an existing entry for this classloader. If a classloader registered a store filter and a different
         * classloader in the hierarchy loaded it, then the entry will still be in the pool and should be reused */
        ...
        if ((theElement != NULL) || (theElement = (ClassNameFilterData*)pool_newElement(pool))) {
            ...
            /* By default, we allocate SHRINIT_NAMEBUF_SIZE space for the classname. If we need more, allocate this
             * separately. */
            if ((fixedNameSize + 1) > SHRINIT_NAMEBUF_SIZE) {
                if (!(theElement->classname
                        = (char*)j9mem_allocate_memory(fixedNameSize + 1, J9MEM_CATEGORY_CLASSES))) {
...
void freeStoreFilterPool(J9JavaVM* vm, J9Pool* filterPool)
{
    struct ClassNameFilterData* anElement;
    pool_state aState;
...
void hookFindSharedClass(J9HookInterface** hookInterface, UDATA eventNum, void* voidData, void* userData)
{
    ...
    if ((J2SE_VERSION(vm) >= J2SE_19) && (NULL == module)) {
        module = getModule(currentThread, (U_8*)eventData->className, realClassNameLength, eventData->classloader);
    }

    if (isClassFromPatchedModule(
            currentThread, module, (U_8*)eventData->className, realClassNameLength, eventData->classloader)) {
        Trc_SHR_INIT_hookFindSharedClass_exit_Noop(currentThread);
        return;
    }
    ...
    if (!(localRuntimeFlags & J9SHR_RUNTIMEFLAG_CACHE_INITIALIZATION_COMPLETE)
        || (localRuntimeFlags & J9SHR_RUNTIMEFLAG_DENY_CACHE_ACCESS)
        || ((localRuntimeFlags & J9SHR_RUNTIMEFLAG_BYTECODE_AGENT_RUNNING) && (sharedClassConfig->modContext == NULL)
    ...
    if (!classpath) {
        ...
        if ((NULL != eventData->classPathEntries) || (eventData->classloader == vm->systemClassLoader)) {
            classpath = createClasspath(
                currentThread, eventData->classPathEntries, eventData->entryCount, helperID, cpType, infoFound);
            if (classpath == NULL) {
                goto _done;
            }
        }
    ...
    if (eventData->doPreventFind) {
        if (eventData->doPreventStore) {
            registerStoreFilter(
                vm, eventData->classloader, fixedName, strlen(fixedName), &(sharedClassConfig->classnameFilterPool));
        }
        goto _donePostFixedClassname;
    }

    eventData->result = (J9ROMClass*)((SH_CacheMap*)(sharedClassConfig->sharedClassCache))
                            ->findROMClass(currentThread, fixedName, classpath, eventData->partition,
                                sharedClassConfig->modContext, eventData->confirmedCount, entryIndex);

    if ((isBootLoader) && (NULL != eventData->result) && (NULL != entryIndex)) {
        /* As modulesPathEntry(Jimage) is put ahead of classpath entries, entryIndex has been increase by 1 (in
         * j9shr_classStoreTransaction_start()) when stored in the cache. So when loaded from the shared cache,
         * entryIndex needs to be decreased by 1 to recover the real classpath entryIndex here.
         */
        *entryIndex = *entryIndex - 1;
        if ((-1 == *entryIndex) && (NULL == module)) {
    ...
    if (localVerboseFlags & J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_IO) {
        ...
        if (classpathItemType == CP_TYPE_CLASSPATH) {
            if (eventData->result) {
                ...
                SHRINIT_TRACE2_NOTAG(
                    localVerboseFlags, J9NLS_SHRC_SHRINIT_FIND_FAILED_VERBOSE_MSG, fixedName, vHelperID);
            }
        } else {
            U_16 vPathLen;
            ...
            if (classpathItemType == CP_TYPE_URL) {
                if (eventData->result) {
                    SHRINIT_TRACE4_NOTAG(localVerboseFlags, J9NLS_SHRC_SHRINIT_FOUND_VERBOSE_URL_MSG, fixedName,
                        vHelperID, vPathLen, vPath);
                } else {
                    SHRINIT_TRACE4_NOTAG(localVerboseFlags, J9NLS_SHRC_SHRINIT_FIND_FAILED_VERBOSE_URL_MSG, fixedName,
                        vHelperID, vPathLen, vPath);
                }
            } else if (classpathItemType == CP_TYPE_TOKEN) {
                if (eventData->result) {
                    SHRINIT_TRACE4_NOTAG(localVerboseFlags, J9NLS_SHRC_SHRINIT_FOUND_VERBOSE_TOKEN_MSG, fixedName,
                        vHelperID, vPathLen, vPath);
                } else {
                    SHRINIT_TRACE4_NOTAG(localVerboseFlags, J9NLS_SHRC_SHRINIT_FIND_FAILED_VERBOSE_TOKEN_MSG, fixedName,
                        vHelperID, vPathLen, vPath);
                }
            }
        }
    ...
    UDATA dataSize, const U_8* codeStart, UDATA codeSize, UDATA forceReplace)
{
    ...
    returnVal = (U_8*)(cm->storeCompiledMethod(
        currentThread, romMethod, dataStart, dataSize, codeStart, codeSize, forceReplace));
...
{
    return (((SH_CacheMap*)(currentThread->javaVM->sharedClassConfig->sharedClassCache))
                ->existsCachedCodeForROMMethodInline(currentThread, romMethod));
}
 ...
 * The data returned can optionally include private data of other JVMs or data of different types stored under the same
 *key.
 *
 ...
 *     J9SHRDATA_PRIVATE_TO_DIFFERENT_JVM indicates that the data is not private to this JVM, but does not mean that the
 *JVM is still alive. You can attempt to acquire this private data by calling acquirePrivateSharedData.
 *
 * @param[in] vmThread  The current thread
 ...
 * 								The pool can be NULL if only the existence of data needs to be
 *determined
 *
 * @return  The number of data elements found or -1 in the case of error
 */
IDATA
j9shr_findSharedData(J9VMThread* currentThread, const char* key, UDATA keylen, UDATA limitDataType,
    UDATA includePrivateData, J9SharedDataDescriptor* firstItem, const J9Pool* descriptorPool)
{
    ...
    returnVal = (((SH_CacheMap*)(sharedClassConfig->sharedClassCache))
                     ->findSharedData(
                         currentThread, key, keylen, limitDataType, includePrivateData, firstItem, descriptorPool));
 ...
 *key. If data of the same dataType already exists for that key, the original data is marked "stale" and the new data is
 *added. If the exact same data already exists in the cache under the same key and dataType, the data is not duplicated
 *and the cached version is returned. If null is passed as the data argument, all existing data against that key is
 *marked "stale" (except for private data owned by other JVMs) Note that each JVM can create a unique private data
 *record in the cache using the same key The function returns null if the cache is full, otherwise it returns a pointer
 *to the shared location of the data.
 *
 ...
 *     Use J9SHRDATA_NOT_INDEXED to indicate that the data does not need to be indexed (will not be returned by
 *findSharedData)
 *
 * @param[in] vmThread  The current thread
 ...
 */
const U_8* j9shr_storeSharedData(
    J9VMThread* currentThread, const char* key, UDATA keylen, const J9SharedDataDescriptor* data)
{
    ...
    if (!(localRuntimeFlags & J9SHR_RUNTIMEFLAG_CACHE_INITIALIZATION_COMPLETE)
        || (J9_ARE_ANY_BITS_SET(
               localRuntimeFlags, J9SHR_RUNTIMEFLAG_BLOCK_SPACE_FULL | J9SHR_RUNTIMEFLAG_DENY_CACHE_UPDATES))) {
        Trc_SHR_INIT_storeSharedData_exit_Noop(currentThread);
        return NULL;
    }
    ...
    if (J9_ARE_ANY_BITS_SET(localRuntimeFlags, J9SHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL)) {
        if (J9_ARE_ALL_BITS_SET(data->flags, J9SHRDATA_USE_READWRITE)) {
            ...
            U_32 wrapperLength
                = (J9_ARE_ALL_BITS_SET(data->flags, J9SHRDATA_NOT_INDEXED) ? 0 : sizeof(ByteDataWrapper));
    ...
    J9VMThread* currentThread, const void* addressInCache, const J9SharedDataDescriptor* data, UDATA forceReplace)
{
    ...
    if ((J9SHR_ATTACHED_DATA_TYPE_JITPROFILE != data->type) && (J9SHR_ATTACHED_DATA_TYPE_JITHINT != data->type)) {
 ...
 * 				if data->address is NULL then data->length is ignored. On successful return data->address
 *will point to allocated memory filled with the data, if data->address is non NULL then data->length is equal to size
 *of the buffer pointed by data->address. On successful return, the buffer is filled with the data.
 * @param[out] corruptOffset If non -1, the updateAtOffset supplied to j9shr_updateAttachedData() when the incomplete
 *write occurred.
 *
 * The J9SharedDataDescriptor->flags currently has no defined values and must
 * be set to J9SHR_ATTACHED_DATA_NO_FLAGS.
    ...
    J9VMThread* currentThread, const void* addressInCache, J9SharedDataDescriptor* data, IDATA* corruptOffset)
{
    ...
    returnVal = (U_8*)(((SH_CacheMap*)(sharedClassConfig->sharedClassCache))
                           ->findAttachedDataAPI(currentThread, addressInCache, data, corruptOffset));
    ...
    J9VMThread* currentThread, const void* addressInCache, I_32 updateAtOffset, const J9SharedDataDescriptor* data)
{
    ...
    returnVal = ((SH_CacheMap*)(sharedClassConfig->sharedClassCache))
                    ->updateAttachedData(currentThread, addressInCache, updateAtOffset, data);
    ...
    J9VMThread* currentThread, const void* addressInCache, UDATA type, I_32 updateAtOffset, UDATA value)
{
    ...
    returnVal = ((SH_CacheMap*)(sharedClassConfig->sharedClassCache))
                    ->updateAttachedUDATA(currentThread, addressInCache, type, updateAtOffset, value);
    *currentState = oldState;
 ...
 */
static UDATA j9shr_acquirePrivateSharedData(J9VMThread* currentThread, const J9SharedDataDescriptor* data)
{
    if (currentThread->javaVM->sharedClassConfig->runtimeFlags & J9SHR_RUNTIMEFLAG_DENY_CACHE_UPDATES) {
        return 0;
    }
    return ((SH_CacheMap*)(currentThread->javaVM->sharedClassConfig->sharedClassCache))
        ->acquirePrivateSharedData(currentThread, data);
}
 ...
 */
static UDATA j9shr_releasePrivateSharedData(J9VMThread* currentThread, const J9SharedDataDescriptor* data)
{
    if (currentThread->javaVM->sharedClassConfig->runtimeFlags & J9SHR_RUNTIMEFLAG_DENY_CACHE_UPDATES) {
        return 0;
    }
    return ((SH_CacheMap*)(currentThread->javaVM->sharedClassConfig->sharedClassCache))
        ->releasePrivateSharedData(currentThread, data);
}
...
void j9shr_resetSharedStringTable(J9JavaVM* vm)
{
    ...
    if (table) {
        ...
        srpHashTableReset(vm->portLibrary, J9_GET_CALLSITE(), table->sharedInvariantSRPHashtable,
            cacheMap->getStringTableBase(), (U_32)cacheMap->getStringTableBytes(),
            (U_32)sizeof(J9SharedInternSRPHashTableEntry), 0, sharedInternHashFn, sharedInternHashEqualFn, NULL, vm);
    ...
    J9JavaVM* vm, const char* ctrlDirName, const char* cacheName, UDATA verboseFlags, U_64 runtimeFlags, UDATA command)
{
    ...
    if (RESULT_DO_PRINTSTATS == command) {
        optionName
            = ((runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_DETAILED_STATS) != 0) ? OPTION_PRINTDETAILS : OPTION_PRINTSTATS;
    } else {
        optionName = OPTION_PRINTALLSTATS;
    }

    IDATA reportedIncompatibleNum
        = j9shr_report_utility_incompatible(vm, ctrlDirName, groupPerm, verboseFlags, cacheName, optionName);

    if (SH_OSCache::getCacheDir(vm, ctrlDirName, cacheDirName, J9SH_MAXPATH, versionData.cacheType) == -1) {
        return;
    }
    if ((reportedIncompatibleNum == 0)
        && (j9shr_stat_cache(vm, cacheDirName, 0, cacheName, &versionData, OSCACHE_CURRENT_CACHE_GEN))) {
...
static inline U_32 getCacheTypeFromRuntimeFlags(U_64 runtimeFlags)
{
...
static IDATA performSharedClassesCommandLineAction(J9JavaVM* vm, J9SharedClassConfig* sharedClassConfig,
    const char* cacheName, UDATA verboseFlags, U_64 runtimeFlags, char* expireTimeStr, UDATA command,
    UDATA printStatsOptions)
{
    ...
    case RESULT_DO_RESET:
        ...
        j9shr_destroy_cache(vm, sharedClassConfig->ctrlDirName, verboseFlags, cacheName, OSCACHE_LOWEST_ACTIVE_GEN,
            OSCACHE_CURRENT_CACHE_GEN, &versionData, (RESULT_DO_RESET == command));
    ...
    case RESULT_DO_DESTROYSNAPSHOT:
        ...
        j9shr_destroy_snapshot(vm, sharedClassConfig->ctrlDirName, verboseFlags, cacheName, OSCACHE_LOWEST_ACTIVE_GEN,
            OSCACHE_CURRENT_CACHE_GEN, &versionData);
        break;
    case RESULT_DO_DESTROYALLSNAPSHOTS:
    ...
    case RESULT_DO_SNAPSHOTCACHE:
        if (J9_ARE_ALL_BITS_SET(runtimeFlags, J9SHR_RUNTIMEFLAG_ENABLE_PERSISTENT_CACHE)) {
            ...
            const char* optionName
                = (RESULT_DO_RESTORE_FROM_SNAPSHOT == command) ? OPTION_RESTORE_FROM_SNAPSHOT : OPTION_SNAPSHOTCACHE;
            SHRINIT_ERR_TRACE2(verboseFlags, J9NLS_SHRC_SHRINIT_OTHER_NONPERS_TYPE_CACHE_EXISTS, optionName, cacheName);
            break;
        }
        if (RESULT_DO_SNAPSHOTCACHE == command) {
            if (1
                != checkIfCacheExists(
                       vm, sharedClassConfig->ctrlDirName, cacheDirName, cacheName, &versionData, cacheType)) {
        ...
        do {
            ...
            if (!scan_udata(&scanstart, &expireTimeVal)) {
                if (expireTimeVal < ((U_32)-1)) {
                    j9shr_destroy_expire_cache(
                        vm, sharedClassConfig->ctrlDirName, groupPerm, verboseFlags, expireTimeVal);
                    return 0;
                }
            }
    ...
    case RESULT_DO_PRINTORPHANSTATS: {
        ...
        IDATA cacheExists
            = checkIfCacheExists(vm, sharedClassConfig->ctrlDirName, cacheDirName, cacheName, &versionData, cacheType);

        if (0 == cacheExists) {
            if (verboseFlags != 0) {
                reportUtilityNotApplicable(
                    vm, sharedClassConfig->ctrlDirName, cacheName, verboseFlags, runtimeFlags, command);
            }
    ...
    case RESULT_DO_PRINT_SNAPSHOTNAME:
        if (-1
            == SH_OSCache::getCacheDir(
                   vm, sharedClassConfig->ctrlDirName, cacheDirName, J9SH_MAXPATH, J9PORT_SHR_CACHE_TYPE_SNAPSHOT)) {
            return J9VMDLLMAIN_SILENT_EXIT_VM;
        }
...
bool modifyCacheName(J9JavaVM* vm, const char* origName, UDATA verboseFlags, char** modifiedCacheName, UDATA bufLen)
{
    ...
    if (strcmp(origName, CACHE_ROOT_PREFIX) == 0) {
        ...
        if ((origNameLen + 2) < bufLen) {
            ...
            if (j9sysinfo_get_username(&((*modifiedCacheName)[modIndex]), bufLeft) != 0) {
                ...
                if (j9sysinfo_get_username(username, CACHE_ROOT_MAXLEN) == 0) {
                    ...
                    SHRINIT_ERR_TRACE2(
                        verboseFlags, J9NLS_SHRC_SHRINIT_FAILURE_COPYING_USERNAME_TOOLONG, bufLeft, usernamelen);
                } else {
        ...
        while (*cursorOrig && (bufLeft > 0)) {
            ...
            if (*cursorOrig == '/' || *cursorOrig == '\\' || *cursorOrig == '$' || *cursorOrig == '&'
                || *cursorOrig == '*' || ((U_8)*cursorOrig) == SHRINIT_ASCII_OF_POUND_SYMBOL || *cursorOrig == '='
                || *cursorOrig == '?') {
            ...
            if (*cursorOrig == '%') {
                ...
                case 'U':
                    ...
                    if (j9sysinfo_get_username(&((*modifiedCacheName)[modIndex]), bufLeft) != 0) {
                        ...
                        if (j9sysinfo_get_username(username, CACHE_ROOT_MAXLEN) == 0) {
                            IDATA usernamelen = (IDATA)strlen(username);
                            SHRINIT_ERR_TRACE2(verboseFlags, J9NLS_SHRC_SHRINIT_FAILURE_COPYING_USERNAME_TOOLONG,
                                bufLeft, usernamelen);
                        } else {
                            SHRINIT_ERR_TRACE(verboseFlags, J9NLS_SHRC_SHRINIT_FAILURE_COPYING_USERNAME);
                        }
        ...
        if (onlyWhitespace) {
            SHRINIT_ERR_TRACE2(
                verboseFlags, J9NLS_SHRC_SHRINIT_FAILURE_NAME_ONLY_WHITESPACE, modIndex, *modifiedCacheName);
            return false;
        }
    }
...
convertPermToDecimal(J9JavaVM* vm, const char* permStr)
{
    ...
    if ((3 == permStrLen) || (4 == permStrLen)) {
        ...
        if ((0 == decPerm) || (01000 == decPerm) || ((0700 <= decPerm) && (0777 >= decPerm))
    ...
    J9JavaVM* vm, J9PortLibrary* portlib, U_64 runtimeFlags, UDATA verboseFlags, J9SharedClassPreinitConfig* piconfig)
{
    ...
    if (is64BitPlatDefaultSize
        && (piconfig->sharedClassCacheSize > J9_SHARED_CLASS_CACHE_MIN_DEFAULT_CACHE_SIZE_FOR_SOFTMAX)) {
    ...
    if ((piconfig->sharedClassMinAOTSize > 0) && (piconfig->sharedClassMinJITSize > 0)) {
        if (softMaxSet) {
            if ((piconfig->sharedClassMinAOTSize + piconfig->sharedClassMinJITSize)
                > piconfig->sharedClassSoftMaxBytes) {
                SHRINIT_ERR_TRACE3(verboseFlags, J9NLS_SHRC_SHRINIT_TOTAL_MINAOT_MINJIT_GRTHAN_SOFTMX,
                    piconfig->sharedClassMinAOTSize, piconfig->sharedClassMinJITSize,
                    piconfig->sharedClassSoftMaxBytes);
                return 1;
            }
    ...
    if (softMaxSet) {
        if (piconfig->sharedClassMinAOTSize > piconfig->sharedClassSoftMaxBytes) {
            SHRINIT_WARNING_TRACE1(
                verboseFlags, J9NLS_SHRC_SHRINIT_MINAOT_GRTHAN_SOFTMX, piconfig->sharedClassSoftMaxBytes);
        ...
        if (piconfig->sharedClassMaxAOTSize > piconfig->sharedClassSoftMaxBytes) {
            SHRINIT_WARNING_TRACE1(
                verboseFlags, J9NLS_SHRC_SHRINIT_MAXAOT_GRTHAN_SOFTMX, piconfig->sharedClassSoftMaxBytes);
            piconfig->sharedClassMaxAOTSize = -1;
        }
        if (piconfig->sharedClassMinJITSize > piconfig->sharedClassSoftMaxBytes) {
            SHRINIT_WARNING_TRACE1(
                verboseFlags, J9NLS_SHRC_SHRINIT_MINJIT_GRTHAN_SOFTMX, piconfig->sharedClassSoftMaxBytes);
        ...
        if (piconfig->sharedClassMaxJITSize > piconfig->sharedClassSoftMaxBytes) {
            SHRINIT_WARNING_TRACE1(
                verboseFlags, J9NLS_SHRC_SHRINIT_MAXJIT_GRTHAN_SOFTMX, piconfig->sharedClassSoftMaxBytes);
     ...
     *  For any other value, required memory size is calculated and set to sharedClassReadWriteBytes with two
     * exceptions: 1.	<tableSize> should be in the supported range of primeNumberHelper, otherwise, error trace is
     * printed and VM exits 2.	required memory for shared string intern table with <tableSize> elements should not be
     * bigger than shared cache size, otherwise, -Xitsn option is omitted and sharedClassReadWriteBytes is set to -1
     * meaning that it will set to a value proportional to cache size during composite cache creation in
     * CompositeCache.c
     *
     */
    if (piconfig->sharedClassInternTableNodeCount > -1) {
        if (piconfig->sharedClassInternTableNodeCount == 0) {
            ...
            piconfig->sharedClassReadWriteBytes = (UDATA)srpHashTable_requiredMemorySize(
                (U_32)piconfig->sharedClassInternTableNodeCount, sizeof(J9SharedInternSRPHashTableEntry), TRUE);
            if (piconfig->sharedClassReadWriteBytes == PRIMENUMBERHELPER_OUTOFRANGE) {
                SHRINIT_ERR_TRACE2(verboseFlags, J9NLS_SHRC_SHRINIT_VALUE_IS_NOT_SUPPORTED_BY_PRIMENUMBERHELPER,
                    piconfig->sharedClassInternTableNodeCount, getSupportedBiggestNumberByPrimeNumberHelper());
                return 1;
            }
        }

        /* Do not need to compare sharedClassReadWriteBytes and sharedClassSoftMaxBytes here. ReadWrite area is always
         * counted as used bytes within the softmx size. If sharedClassReadWriteBytes > sharedClassSoftMaxBytes, the
         * softmx size will be increased in compositeCache.cpp to make sure softmx size include the ReadWrite area.
         * */

        if ((piconfig->sharedClassReadWriteBytes >= (IDATA)*cacheSize)) {
            SHRINIT_TRACE3(verboseFlags, J9NLS_SHRC_SHRINIT_SHARED_INTERN_TABLE_IS_BIGGER_THAN_SHARED_CACHE,
                piconfig->sharedClassReadWriteBytes, piconfig->sharedClassInternTableNodeCount, *cacheSize);
...
static UDATA initializeSharedStringTable(J9JavaVM* vm)
{
    ...
    cacheMap->getCompositeCacheAPI()->setInternCacheHeaderFields(&(table->sharedTailNodePtr),
        &(table->sharedHeadNodePtr), &(table->totalSharedNodesPtr), &(table->totalSharedWeightPtr));
    ...
    if (cacheMap->isStringTableInitialized()) {
        ...
        table->sharedInvariantSRPHashtable = srpHashTableRecreate(vm->portLibrary, J9_GET_CALLSITE(),
            cacheMap->getStringTableBase(), sharedInternHashFn, sharedInternHashEqualFn, NULL, vm);
    } else {
        table->sharedInvariantSRPHashtable = srpHashTableNewInRegion(vm->portLibrary, J9_GET_CALLSITE(),
            cacheMap->getStringTableBase(), (U_32)cacheMap->getStringTableBytes(),
            (U_32)sizeof(J9SharedInternSRPHashTableEntry), 0, sharedInternHashFn, sharedInternHashEqualFn, NULL, vm);
...
UDATA
j9shr_isBCIEnabled(J9JavaVM* vm) { return (0 != (vm->sharedClassConfig->runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_BCI)); }
...
j9shr_init(J9JavaVM* vm, UDATA loadFlags, UDATA* nonfatal)
{
     ...
     * protected.
     */
    if (((runtimeFlags & J9SHR_RUNTIMEFLAG_VERIFY_TREE_AND_TREE_ACCESS)
            == J9SHR_RUNTIMEFLAG_VERIFY_TREE_AND_TREE_ACCESS)
    ...
    if (parseResult == RESULT_DO_PRINTSTATS || parseResult == RESULT_DO_PRINTALLSTATS
        || parseResult == RESULT_DO_PRINTORPHANSTATS || parseResult == RESULT_DO_PRINTALLSTATS_EQUALS
    ...
    if ((RESULT_DO_INVALIDATE_AOT_METHODS_EQUALS == parseResult)
        ...
        if (J9_ARE_ALL_BITS_SET(runtimeFlags, J9SHR_RUNTIMEFLAG_ENABLE_READONLY)) {
            const char* option = (RESULT_DO_INVALIDATE_AOT_METHODS_EQUALS == parseResult)
                ? OPTION_INVALIDATE_AOT_METHODS_EQUALS
                : OPTION_REVALIDATE_AOT_METHODS_EQUALS;
            runtimeFlags &= ~J9SHR_RUNTIMEFLAG_ENABLE_READONLY;
            SHRINIT_WARNING_TRACE3(
                verboseFlags, J9NLS_SHRC_SHRINIT_OPTION_IGNORED_WARNING, OPTION_READONLY, option, OPTION_READONLY);
            if (verboseFlags > 0) {
                j9tty_printf(PORTLIB, "\n");
            }
        }
    ...
    if (RESULT_DO_RESET == parseResult) {
        /* For 'reset', we need to check for incompatible options before taking any action in
         * performSharedClassesCommandLineAction() */
    ...
    memBytesNeeded
        = sizeof(J9SharedClassConfig) + sizeof(J9SharedClassCacheDescriptor) + cmBytes + nameBytes + modContextBytes;
    ...
    rc = performSharedClassesCommandLineAction(
        vm, tempConfig, modifiedCacheNamePtr, verboseFlags, runtimeFlags, expireTime, parseResult, printStatsOptions);
    ...
    if (RESULT_DO_RESTORE_FROM_SNAPSHOT == parseResult) {
        *nonfatal = 0;
        ...
        if (0 == rcStartup) {
            ...
            if (false == cacheExist) {
                SHRINIT_ERR_TRACE1(
                    vm->sharedClassConfig->verboseFlags, J9NLS_SHRC_SHRINIT_FAILURE_RESTORE_CACHE, cacheName);
            }
        }
    } else {
        rcStartup = cm->startup(currentThread, piconfig, cacheName, ctrlDirName, vm->sharedCacheAPI->cacheDirPerm, NULL,
            &cacheHasIntegrity);
    }
    ...
    if (rcStartup != 0) {
        ...
        config->jclJ9ClassPathEntryPool = pool_new(sizeof(struct J9ClassPathEntry), 0, 0, 0, J9_GET_CALLSITE(),
            J9MEM_CATEGORY_CLASSES, POOL_FOR_PORT(vm->portLibrary));
        ...
        /* Register hooks */
        (*hook)->J9HookRegisterWithCallSite(
            hook, J9HOOK_VM_FIND_LOCALLY_DEFINED_CLASS, hookFindSharedClass, OMR_GET_CALLSITE(), NULL);

#if defined(J9SHR_CACHELET_SUPPORT)
        if (runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_NESTED) {
            (*hook)->J9HookRegisterWithCallSite(
                hook, J9HOOK_VM_SERIALIZE_SHARED_CACHE, hookSerializeSharedCache, OMR_GET_CALLSITE(), NULL);
        }
#endif
         ...
         * 		This is because we can't get any cross-process locks, which means that it's not safe for us to
         * search the table.
         */
        if (translationBuffers && (translationBuffers->flags & BCU_ENABLE_INVARIANT_INTERNING)
            ...
            if (!(vm->sharedClassConfig->runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_READONLY)) {
                 ...
                 * corruption.
                 */
                if (omrthread_monitor_init_with_name(&tableInternFxMutex, 0, "XshareclassesVerifyInternTableMon")
                    != 0) {
                    goto _error;
                }
                ...
                if ((0 != (runtimeFlags & J9SHR_RUNTIMEFLAG_CHECK_STRINGTABLE_RESET_READONLY))
                    || (0 != (runtimeFlags & J9SHR_RUNTIMEFLAG_CHECK_STRINGTABLE_RESET_READWRITE))) {
                    /* CMVC 176498 : Ensure that string table size is large enough for
                     * -Xshareclasses:checkStringTableReset to work. String table should go beyond the page containing
                     * cache header (because cache header is not mprotected by default).
                     */
                    ...
                    UDATA tableSize = srpHashTable_calculateTableSize(
                        (U_32)cm->getStringTableBytes(), (U_32)sizeof(J9SharedInternSRPHashTableEntry), FALSE);

                    if ((0 == tableSize) || (PRIMENUMBERHELPER_OUTOFRANGE == tableSize)) {
                        SHRINIT_ERR_TRACE2(vm->sharedClassConfig->verboseFlags,
                            J9NLS_SHRC_SHRINIT_CHECK_STRING_TABLE_RESET_MAY_FAIL, totalSize, osPageSize);
                        goto _error;
                    }
                ...
                if (cm->startClassTransaction(currentThread, false, fname) == 0) {
                    if (cm->enterStringTableMutex(currentThread, FALSE, &doRebuildLocalData, &doRebuildCacheData)
                        == 0) {
                        UDATA resetReason = J9SHR_STRING_POOL_OK;

                        /*-----SRPHASHTABLE------------------*/
                        vm->sharedInvariantInternTable = (J9SharedInvariantInternTable*)j9mem_allocate_memory(
                            (UDATA)sizeof(J9SharedInvariantInternTable), J9MEM_CATEGORY_CLASSES);
                         ...
                         */
                        cm->updateRuntimeFullFlags(currentThread);
                        if (J9_ARE_ANY_BITS_SET(vm->sharedClassConfig->runtimeFlags,
                        ...
                        vm->sharedInvariantInternTable->performNodeAction = (UDATA(*)(J9SharedInvariantInternTable*,
                            J9SharedInternSRPHashTableEntry*, UDATA, void*))sharedInternTable_performNodeAction;
                        ...
                        if (initializeSharedStringTable(vm)) {
                            if (!doRebuildCacheData
                                && (!cacheHasIntegrity
                                       || ((runtimeFlags & J9SHR_RUNTIMEFLAG_ENABLE_STRING_TABLE_CHECK) != 0))) {
                                J9SharedInvariantInternTable* table = vm->sharedInvariantInternTable;

                                /**
                                 * If we are here than there is shared string intern table  in shared cache.
                                 * Since the changes in getStringTableBytes() implementation, old caches will return 0
                                 * for this function. In such cases, use all readWriteBytes to verify SRPHashTable,
                                 * because old caches used to use all readWriteBytes to generate shared string intern
                                 * table.
                                 */
                                ...
                                if (srpHashTableVerify(table->sharedInvariantSRPHashtable,
                                        (U_32)cm->getStringTableBytes(),
                                    ...
                                    if (numElements != *(table->totalSharedNodesPtr)) {
                                        Trc_SHR_INIT_failedSimplePoolIntegrity(
                                            *(table->totalSharedNodesPtr), numElements);
                                        verifyError = 1;
                                        resetReason = J9SHR_STRING_POOL_FAILED_VERIFY;
                                    } else {
                                        verifyStringTable.simplePool = J9SRPHASHTABLEINTERNAL_NODEPOOL(
                                            table->sharedInvariantSRPHashtable->srpHashtableInternal);
                                        cm->getRomClassAreaBounds(
                                            &verifyStringTable.romClassAreaStart, &verifyStringTable.romClassAreaEnd);
                                        if (!srpHashTable_checkConsistency(table->sharedInvariantSRPHashtable, PORTLIB,
                                                &verifyStringTableElement, (void*)&verifyStringTable,
        ...
        (*shcHooks)->J9HookRegisterWithCallSite(
            shcHooks, J9HOOK_VM_ZIP_LOAD, j9shr_hookZipLoadEvent, OMR_GET_CALLSITE(), NULL);
    }

    if (0 != (runtimeFlags & J9SHR_RUNTIMEFLAG_ADD_TEST_JITHINT)) {
        J9HookInterface** hook = vm->internalVMFunctions->getVMHookInterface(vm);
        (*hook)->J9HookRegisterWithCallSite(
            hook, J9HOOK_VM_FIND_LOCALLY_DEFINED_CLASS, addTestJitHint, OMR_GET_CALLSITE(), NULL);
    }
    ...
    if ((RESULT_DO_ADJUST_SOFTMX_EQUALS == parseResult) || (RESULT_DO_ADJUST_MINAOT_EQUALS == parseResult)
        || (RESULT_DO_ADJUST_MAXAOT_EQUALS == parseResult) || (RESULT_DO_ADJUST_MINJITDATA_EQUALS == parseResult)
        || (RESULT_DO_ADJUST_MAXJITDATA_EQUALS == parseResult)) {
        /* There is no need to check the return value of tryAdjustMinMaxSizes(). JVM will always exit and the
         * corresponding NLS message will be printed out inside tryAdjustMinMaxSizes() no matter whether the
         * softmx/minAOT/maxAOT/minJIT/maxJIT has been adjusted as requested */
...
_error:
    ...
    if (((doPrintStats) && (-2 == rcStartup)) || (RESULT_DO_RESTORE_FROM_SNAPSHOT == parseResult)) {
        return J9VMDLLMAIN_SILENT_EXIT_VM;
    }
...
j9shr_print_stats(J9JavaVM* vm, UDATA parseResult, U_64 runtimeFlags, UDATA printStatsOptions)
{
    ...
    if (RESULT_DO_PRINTALLSTATS == parseResult || RESULT_DO_PRINTALLSTATS_EQUALS == parseResult
        || RESULT_DO_PRINTORPHANSTATS == parseResult || RESULT_DO_PRINTSTATS_EQUALS == parseResult
        || RESULT_DO_PRINTSTATS == parseResult) {
        showFlags = printStatsOptions;
    }
    return ((SH_CacheMap*)vm->sharedClassConfig->sharedClassCache)
        ->printCacheStats(currentThread, showFlags, runtimeFlags);
}
 ...
 *
 */
static void j9shr_print_cache_filename(J9JavaVM* vm, const char* cacheDirName, U_64 runtimeFlags, const char* cacheName)
{
    ...
    SH_OSCache::getCacheVersionAndGen(
        PORTLIB, vm, cacheNameWithVGen, J9SH_MAXPATH, cacheName, &versionData, SH_OSCache::getCurrentCacheGen(), true);
...
{
    ((SH_CacheMap*)vm->sharedClassConfig->sharedClassCache)
        ->getCompositeCacheAPI()
        ->getMinMaxBytes(softmx, minAOT, maxAOT, minJIT, maxJIT);
}
...
I_32 j9shr_setMinMaxBytes(J9JavaVM* vm, U_32 softmx, I_32 minAOT, I_32 maxAOT, I_32 minJIT, I_32 maxJIT)
{
    ...
    return ((SH_CacheMap*)vm->sharedClassConfig->sharedClassCache)
        ->getCompositeCacheAPI()
        ->tryAdjustMinMaxSizes(currentThread, TRUE);
}
...
static void freeClasspathItemsForPool(J9JavaVM* vm, J9Pool* pool, UDATA alsoFreeCpEntries)
{
    PORT_ACCESS_FROM_JAVAVM(vm);
...
{
    return (J9SHR_RUNTIMEFLAG_ENABLE_LOCAL_CACHEING | J9SHR_RUNTIMEFLAG_ENABLE_TIMESTAMP_CHECKS
        | J9SHR_RUNTIMEFLAG_ENABLE_REDUCE_STORE_CONTENTION | J9SHR_RUNTIMEFLAG_ENABLE_ROUND_TO_PAGE_SIZE |
        ...
        J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT_PARTIAL_PAGES |
#endif
        J9SHR_RUNTIMEFLAG_ENABLE_CACHEBOOTCLASSES | J9SHR_RUNTIMEFLAG_ENABLE_CACHE_NON_BOOT_CLASSES
        | J9SHR_RUNTIMEFLAG_ENABLE_BYTECODEFIX | J9SHR_RUNTIMEFLAG_ENABLE_AOT | J9SHR_RUNTIMEFLAG_ENABLE_JITDATA
        | J9SHR_RUNTIMEFLAG_AUTOKILL_DIFF_BUILDID | J9SHR_RUNTIMEFLAG_DETECT_NETWORK_CACHE
        | J9SHR_RUNTIMEFLAG_ENABLE_SEMAPHORE_CHECK);
}
...
void j9shr_populatePreinitConfigDefaults(J9JavaVM* vm, J9SharedClassPreinitConfig* updatedWithDefaults)
{
    ...
    if (NULL != table) {
        updatedWithDefaults->sharedClassInternTableNodeCount
            = (UDATA)srpHashTable_tableSize(table->sharedInvariantSRPHashtable);
    }
}
...
static void addTestJitHint(J9HookInterface** hookInterface, UDATA eventNum, void* voidData, void* userData)
{
    ...
    if (NULL != romMethod) {
        ...
        j9file_printf(PORTLIB, J9PORT_TTY_OUT, "addTestJitHint adding hint to %.*s.%.*s\n", J9UTF8_LENGTH(romclassName),
            J9UTF8_DATA(romclassName), J9UTF8_LENGTH(methodName), J9UTF8_DATA(methodName));
...
j9shr_createCacheSnapshot(J9JavaVM* vm, const char* cacheName)
{
    ...
    if (-1
        == SH_OSCache::getCacheDir(
               vm, vm->sharedClassConfig->ctrlDirName, cacheDirName, J9SH_MAXPATH, J9PORT_SHR_CACHE_TYPE_SNAPSHOT)) {
        ...
        I_32 mode = (J9_ARE_ALL_BITS_SET(vm->sharedClassConfig->runtimeFlags, J9SHR_RUNTIMEFLAG_ENABLE_GROUP_ACCESS))
            ? J9SH_CACHE_FILE_MODE_DEFAULTDIR_WITH_GROUPACCESS
            : J9SH_CACHE_FILE_MODE_DEFAULTDIR_WITHOUT_GROUPACCESS;

        setCurrentCacheVersion(vm, J2SE_VERSION(vm), &versionData);
        versionData.cacheType = J9PORT_SHR_CACHE_TYPE_SNAPSHOT;
        SH_OSCache::getCacheVersionAndGen(
            PORTLIB, vm, nameWithVGen, CACHE_ROOT_MAXLEN, cacheName, &versionData, OSCACHE_CURRENT_CACHE_GEN, false);
        ...
        if (-1 == fd) {
            ...
            OSCachesysv_header_version_current* headerStart
                = (OSCachesysv_header_version_current*)((UDATA)theca - headerSize);
            UDATA ignore = 0;
            I_32 lockRc1 = -1;
            I_32 lockRc2 = -1;
            I_32 lockRc3 = -1;
            ...
            lockRc1
                = j9file_lock_bytes(fd, J9PORT_FILE_WRITE_LOCK | J9PORT_FILE_WAIT_FOR_LOCK, 0, cacheSize + headerSize);
            ...
            if ((lockRc2 < 0) || ((lockRc3 < 0) && (readWriteBytes > 0))) {
                ...
                IDATA cacheInitCompleteOffset = SH_OSCachesysv::getSysvHeaderFieldOffsetForGen(
                    OSCACHE_CURRENT_CACHE_GEN, OSCACHE_HEADER_FIELD_CACHE_INIT_COMPLETE);
                IDATA fileRc = 0;
                ...
                if (fileSize > 0) {
                    /* Snapshot file already exists. When fileSize > nbytes, writing nbytes into the snapshot file is
                     * not able to overwrite everything in the file, so truncate the file first.
                     */
                    if (j9file_set_length(fd, 0) < 0) {
                        ...
                        SHRINIT_WARNING_TRACE2(verboseFlags, J9NLS_SHRC_SHRINIT_OVERWRITE_EXISTING_SNAPSHOT_WARNING,
                            cacheName, pathFileName);
                    }
                }
                ...
                if (fileRc < 0) {
                    ...
                    Trc_SHR_INIT_j9shr_createCacheSnapshot_fileWriteFailed2(
                        currentThread, pathFileName, nbytes, fileRc);
                    SHRINIT_ERR_TRACE1(verboseFlags, J9NLS_SHRC_SHRINIT_ERROR_SNAPSHOT_FILE_WRITE, pathFileName);
                    rc = -1;
                    goto done;
                }
                ...
                if (J9_ARE_ALL_BITS_SET(
                        vm->sharedClassConfig->runtimeFlags, J9SHR_RUNTIMEFLAG_ENABLE_TEST_BAD_BUILDID)) {
                    /* J9SHR_RUNTIMEFLAG_ENABLE_TEST_BAD_BUILDID is for testing only, change the buildID in the snapshot
                     */
                    U_64 theBadBuildID = 0;
                    IDATA buildIDOffset = SH_OSCachesysv::getSysvHeaderFieldOffsetForGen(
                        OSCACHE_CURRENT_CACHE_GEN, OSCACHE_HEADER_FIELD_BUILDID);
...
static IDATA j9shr_restoreFromSnapshot(J9JavaVM* vm, const char* ctrlDirName, const char* cacheName, bool* cacheExist)
{
    ...
    if (J9_ARE_ALL_BITS_SET(vm->sharedClassConfig->runtimeFlags, J9SHR_RUNTIMEFLAG_ENABLE_READONLY)) {
        ...
        SHRINIT_WARNING_TRACE3(verboseFlags, J9NLS_SHRC_SHRINIT_OPTION_IGNORED_WARNING, OPTION_READONLY,
            OPTION_RESTORE_FROM_SNAPSHOT, OPTION_READONLY);
    }
...
static void j9shr_print_snapshot_filename(J9JavaVM* vm, const char* cacheDirName, const char* snapshotName)
{
    ...
    SH_OSCache::getCacheVersionAndGen(PORTLIB, vm, snapshotNameWithVGen, J9SH_MAXPATH, snapshotName, &versionData,
        SH_OSCache::getCurrentCacheGen(), true);
...
const U_8* j9shr_findCompiledMethodEx1(J9VMThread* currentThread, const J9ROMMethod* romMethod, UDATA* flags)
{
    ...
    returnVal = (U_8*)(((SH_CacheMap*)(sharedClassConfig->sharedClassCache))
                           ->findCompiledMethod(currentThread, romMethod, flags));
...
/**
 * This function will find the AOT method(s) that match the passed in method specification(s) and perform the required
 * operation
 * @param[in] vm The current J9JavaVM
 * @param[in] string The pointer of the method specifications passed in by
 * "invalidateAotMethods/revalidateAotMethods/findAotMethods=" option
...
static IDATA j9shr_aotMethodOperation(J9JavaVM* vm, char* methodSpecs, UDATA action)
{
    ...
    rc = ((SH_CacheMap*)vm->sharedClassConfig->sharedClassCache)
             ->aotMethodOperation(currentThread, methodSpecs, action);
...
/**
 * This function will be called if the value passed to the command line option (-Xscmx/-XX:SharedCacheHardLimit=)
 * setting the shared cache size is greater than SHMMAX. It will reduce the cache size to SHMMAX. If necessary,
 * debugRegionSize will also be adjusted in proportion according to its original ratio to the -Xscmx value.
 *
    ...
    J9PortLibrary* portlib, UDATA verboseFlags, J9SharedClassPreinitConfig* piconfig, U_64 newSize)
{
    ...
    if (piconfig->sharedClassDebugAreaBytes > 0) {
        piconfig->sharedClassDebugAreaBytes
            = SHC_PAD((IDATA)(piconfig->sharedClassDebugAreaBytes * ratio), SHC_WORDALIGN);
    }
...
void j9shr_jvmPhaseChange(J9VMThread* currentThread, UDATA phase)
{
    if (J9VM_PHASE_NOT_STARTUP == phase) {
         ...
         * GC decides whether to calls vm->sharedClassConfig->storeGCHints() to store the GC hints into the shared
         * cache. */
        storeStartupHintsToSharedCache(currentThread);
        if (J9_ARE_NO_BITS_SET(
                vm->sharedClassConfig->runtimeFlags, J9SHR_RUNTIMEFLAG_MPROTECT_PARTIAL_PAGES_ON_STARTUP)) {
    ...
    J9PortShcVersion* versionData, U_32 cacheType)
{
    ...
    if (-1 == SH_OSCache::getCacheDir(vm, ctrlDirName, cacheDirName, J9SH_MAXPATH, cacheType)) {
        ...
        setCurrentCacheVersion(vm, J2SE_VERSION(vm), versionData);
        versionData->cacheType = cacheType;
        ret = j9shr_stat_cache(
            vm, cacheDirName, vm->sharedCacheAPI->verboseFlags, cacheName, versionData, OSCACHE_CURRENT_CACHE_GEN);
    }
    return ret;
}
        ...
        if (NULL != classLoader->moduleExtraInfoHashTable) {
            ...
            if ((NULL != moduleInfo) && (NULL != moduleInfo->patchPathEntries) && (moduleInfo->patchPathCount > 0)) {
                ...
                if (TrcEnabled_Trc_SHR_INIT_isClassFromPatchedModule_ClassFromPatchedModule_Event) {
                    ...
                    Trc_SHR_INIT_isClassFromPatchedModule_ClassFromPatchedModule_Event(
                        vmThread, classNameLength, className, (UDATA)J9UTF8_LENGTH(url), (const char*)J9UTF8_DATA(url));
                }
...
static J9Module* getModule(J9VMThread* vmThread, U_8* className, UDATA classNameLength, J9ClassLoader* classLoader)
{
    ...
    if (J9_ARE_NO_BITS_SET(vm->runtimeFlags, J9_RUNTIME_JAVA_BASE_MODULE_CREATED)) {
        ...
        if (NULL != packageEnd) {
            omrthread_monitor_enter(vm->classLoaderModuleAndLocationMutex);
            module
                = vmFuncs->findModuleForPackage(vmThread, classLoader, className, (U_32)((U_8*)packageEnd - className));
            omrthread_monitor_exit(vm->classLoaderModuleAndLocationMutex);
        }
    }
    return module;
...
static bool isFreeDiskSpaceLow(J9JavaVM* vm, U_64* maxsize)
{
    ...
    if (-1
        == SH_OSCache::getCacheDir(
               vm, vm->sharedCacheAPI->ctrlDirName, cacheDirName, J9SH_MAXPATH, J9PORT_SHR_CACHE_TYPE_PERSISTENT)) {
...
/**
 * This function generates a key for the GC hints. The key is a string of all the commandline arguments passed to the
 * current JVM
 * @param[in] vm The current J9JavaVM
 *
...
static char* generateStartupHintsKey(J9JavaVM* vm)
{
    ...
    for (i = 0; i < nOptions; ++i) {
        ...
        if ((NULL != option && strlen(option) > 0) && (NULL == strstr(option, "-Dsun.java.launcher.pid="))) {
    ...
    for (i = 0; i < nOptions; ++i) {
        char* option = actualArgs->options[i].optionString;
        if ((NULL != option && strlen(option) > 0) && (NULL == strstr(option, "sun.java.launcher.pid"))) {
            if (firstOption) {
                firstOption = false;
...
/**
 * This function fetches the startup hints from the shared cache and store it locally to
 * vm->sharedClassConfig->localStartupHints.
 * @param[in] currentThread  The current VM thread
 *
 */
static void fetchStartupHintsFromSharedCache(J9VMThread* currentThread)
{
    J9JavaVM* vm = currentThread->javaVM;

    if (NULL != vm->sharedClassConfig) {
        char* key = NULL;
        if (J9_ARE_ALL_BITS_SET(vm->sharedClassConfig->localStartupHints.localStartupHintFlags,
                J9SHR_LOCAL_STARTUPHINTS_FLAG_FETCHED)) {
        ...
        if (NULL != key) {
            ...
            PORT_ACCESS_FROM_JAVAVM(vm);
            if (0 < j9shr_findSharedData(
                        currentThread, key, strlen(key), J9SHR_DATA_TYPE_STARTUP_HINTS, 0, &dataDescriptor, NULL)) {
                ...
                memcpy(&vm->sharedClassConfig->localStartupHints.hintsData, dataDescriptor.address,
                    sizeof(J9SharedStartupHintsDataDescriptor));
                vm->sharedClassConfig->localStartupHints.localStartupHintFlags |= J9SHR_LOCAL_STARTUPHINTS_FLAG_FETCHED;
                Trc_SHR_INIT_fetchStartupHintsFromSharedCache_Hints_Found(currentThread,
                    vm->sharedClassConfig->localStartupHints.hintsData.flags,
                    vm->sharedClassConfig->localStartupHints.hintsData.heapSize1,
...
const U_8* storeStartupHintsToSharedCache(J9VMThread* currentThread)
{
    J9JavaVM* vm = currentThread->javaVM;
    const U_8* ret = NULL;
    if (J9_ARE_ANY_BITS_SET(vm->sharedClassConfig->localStartupHints.localStartupHintFlags,
        ...
        if (NULL != key) {
            ...
            if (J9_ARE_ANY_BITS_SET(vm->sharedClassConfig->localStartupHints.localStartupHintFlags,
                    J9SHR_LOCAL_STARTUPHINTS_FLAG_OVERWRITE_HINTS)) {
            ...
            if (NULL == ret) {
                ...
                Trc_SHR_INIT_storeStartupHintsToSharedCache_Store_Successful(currentThread,
                    vm->sharedClassConfig->localStartupHints.hintsData.flags,
                    vm->sharedClassConfig->localStartupHints.hintsData.heapSize1,
                    vm->sharedClassConfig->localStartupHints.hintsData.heapSize2);
            }
 ...
 * @param[in] forceReplace TRUE Replace the existing GC hints under the same key (the same command line arguments) if
 * there is one already in the shared cache. FALSE Do not replace existing GC hints under the same key.
 */
void j9shr_storeGCHints(J9VMThread* currentThread, UDATA heapSize1, UDATA heapSize2, BOOLEAN forceReplace)
{
    J9JavaVM* vm = currentThread->javaVM;
    bool heapSizesSet = J9_ARE_ALL_BITS_SET(
    ...
    if (forceReplace || !heapSizesSet) {
        ...
        vm->sharedClassConfig->localStartupHints.hintsData.flags |= J9SHR_STARTUPHINTS_HEAPSIZES_SET;
        if (forceReplace) {
            vm->sharedClassConfig->localStartupHints.localStartupHintFlags
...
j9shr_findGCHints(J9VMThread* currentThread, UDATA* heapSize1, UDATA* heapSize2)
{
    ...
    if (J9_ARE_ALL_BITS_SET(
        ...
        Trc_SHR_INIT_j9shr_findGCHints_Event_Found(currentThread,
            vm->sharedClassConfig->localStartupHints.hintsData.heapSize1,
            vm->sharedClassConfig->localStartupHints.hintsData.heapSize2);
        returnVal = 0;
    }
