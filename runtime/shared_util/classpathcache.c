...
static J9ClasspathByID* findIdentifiedWithPartition(J9VMThread* currentThread, struct J9ClasspathByIDArray* theArray,
    UDATA element, const char* partition, UDATA partitionLen);
static UDATA localMatchCheck(J9VMThread* currentThread, struct J9ClasspathByIDArray* theArray, IDATA callerHelperID,
    IDATA arrayIndex, UDATA indexInCacheHelper, const char* partition, UDATA partitionLen, UDATA doSet);
 ...
 * If an entry exists for given helperID and partition, but itemsAdded is different, the old entry in the array is
 * cleared and the new entry is added. In this case, the pointer to the old classpath is also returned via cpToFree,
 * giving the option for the caller to free the classpath memory if appropriate.
 *
 ...
 * @param [in] walkFrom  The index to in theArray to start looking for ID from (allows the function to be called
 * multiple times to find multiple IDs)
 *
...
/**
 * SPEC: Sets an element in classpath array within an element of "theArrayPtr" linked list. The classpath being added
 * has helper ID "helperID", contains items "itemsAdded" and partition "partition" of length "partitionLen" (partition
 * may be NULL). The "cp" parameter is the actual classpath represented by the array entry. This could be a classpath in
 * the cache or a local classpath. If helperID is greater than or equal to the length of the array, the array is grown
 * and copied and the new reference is passed back through "theArrayPtr". The function always returns the current length
 * of the array, so if it is grown, the new length is returned.
 *
 * @param [in] currentThread  Pointer to the J9VMThread structure for the current thread
 * @param [in,out] theArrayPtr  Pointer to a pointer to the J9ClasspathByIDArray linked list.
 *        If the J9ClasspathByID array within this is grown, it will be updated to the location of the new
 * J9ClasspathByIDArray structure.
 ...
 *
 * @return Current length of the array, if the array has grown this will be the new length.  Please note that as
 * partition support is not currently implemented this will be 1 on success and 0 on failure to allocate an array
 */
UDATA
setIdentifiedClasspath(J9VMThread* currentThread, struct J9ClasspathByIDArray** theArrayPtr, IDATA helperID,
    UDATA itemsAdded, const char* partition, UDATA partitionLen, void* cp)
{
    ...
    if (arrayIndex >= origSize) {
        ...
        *theArrayPtr = NULL;
        ...
        while (walk) {
            ...
            if (!(newArray = initializeIdentifiedClasspathArray(
                      portlib, newSize, walk->partition, walkPartitionLen, walk->partitionHash))) {
                Trc_SHR_CPC_setIdentifiedClasspath_ExitAlloc(currentThread);
                return 0;
            }
    ...
    if (!partition) {
        ...
        if ((newLink
                = findIdentifiedWithPartition(currentThread, (*theArrayPtr), arrayIndex, partition, partitionLen))) {
            Trc_SHR_CPC_setIdentifiedClasspath_ExistingPartition(currentThread);
        }

        if (!newLink) {
            UDATA compareHash
                = currentThread->javaVM->internalVMFunctions->computeHashForUTF8((U_8*)partition, (U_16)partitionLen);
            struct J9ClasspathByIDArray* temp;

            Trc_SHR_CPC_setIdentifiedClasspath_CreatePartitionLink(currentThread);

            if (!(newArray
                    = initializeIdentifiedClasspathArray(portlib, newSize, partition, partitionLen, compareHash))) {
                Trc_SHR_CPC_setIdentifiedClasspath_ExitAlloc(currentThread);
                return 0;
            }
...
/**
 * SPEC: Create and return a linked list element, J9ClasspathByIDArray which includes an array of J9ClasspathByID
 * structs of size "elements".
 *
 * @param [in] portlib  The portlib
 ...
 *
 * @result  If the array is successfully built, returns a pointer to the J9ClasspathByIDArray for the first element of
    ...
    J9PortLibrary* portlib, UDATA elements, const char* partition, UDATA partitionLen, IDATA partitionHash)
{
    ...
    UDATA identifiedSize = (sizeof(struct J9ClasspathByIDArray)
        + ((sizeof(struct J9ClasspathByID*) + sizeof(struct J9ClasspathByID)) * elements)
        + CPC_PAD(sizeof(U_8) * elements * elements));
    ...
    /* If we get into territory where we have more classpaths than MAX_CACHED_CLASSPATHS, stop using classpath cacheing
     * as the array will get too large and searching it will be too expensive */
    ...
    for (i = 0; i < elements; i++) {
        returnVal->array[i] = (struct J9ClasspathByID*)(elementsAddress
            + (i * (sizeof(struct J9ClasspathByID) + (sizeof(U_8) * elements))));
    ...
    UDATA element, const char* partition, UDATA partitionLen)
{
    ...
    while (walk) {
        if (walk->partitionHash == compareHash) {
            Trc_SHR_CPC_getIdentifiedClasspath_ComparePartitions(
                currentThread, walk->partition, partitionLen, partition);
...
{
    localMatchCheck(
        currentThread, theArray, callerHelperID, arrayIndex, indexInCacheHelper, partition, partitionLen, 1);
}
    ...
    IDATA arrayIndex, UDATA indexInCacheHelper, const char* partition, UDATA partitionLen, UDATA doSet)
{
    ...
    if ((arrayIndex > MAX_CACHED_CLASSPATHS) || (callerHelperID > MAX_CACHED_CLASSPATHS)
        || ((UDATA)arrayIndex >= theArray->size) || ((UDATA)callerHelperID >= theArray->size)
 ...
 * @param [in] arrayIndex  Index into theArrayPtr
 * @param [in] indexInCacheHelper Other classloader id.  It is the value to be checked against
 * failedMatches[callerHelperId]
 * @param [in] partition  Pointer to the name of the partition (nul terminated string)
 * @param [in] partitionLen  Length of the partition name string
 *
 * @return ???
 */
UDATA
hasMatchFailedBefore(J9VMThread* currentThread, struct J9ClasspathByIDArray* theArray, IDATA callerHelperID,
    IDATA arrayIndex, UDATA indexInCacheHelper, const char* partition, UDATA partitionLen)
{
    return localMatchCheck(
        currentThread, theArray, callerHelperID, arrayIndex, indexInCacheHelper, partition, partitionLen, 0);
}
