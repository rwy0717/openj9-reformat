...
void* shcReadSharedCacheMetadata(J9JavaVM* vm, UDATA* length, ShcItemHdr** firstEntry)
{
...
/**
 * Walks a cache in or out of process.
 * If limitDataType is zero, all entries are returned. To limit to a specific data type, set limitDataType to one of the
 values in shcdatatypes.h
 ...
 *
 * Note also that state->reloc returns the difference in bytes between the dbgMalloc'd memory address and the dump
 address.
