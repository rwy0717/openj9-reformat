 ...
 */
#define SP_ELEMENT_IS_ALIGNED(simplePool, firstPossibleElementSlot, element) \
    (0 == (((UDATA)element - (UDATA)firstPossibleElementSlot) % simplePool->elementSize))

static BOOLEAN iterateOverSimplePoolElements(J9SimplePool* simplePool, J9PortLibrary* portLib,
 ...
 * 							be a multiple of 4 and greater than or equal to
 * SIMPLEPOOL_MIN_ELEMENT_SIZE
 * @param[in] flags 		None defined
 *
 * @return pointer to the simple pool, or NULL if the simple pool could not be created.
 */
J9SimplePool* simplepool_new(void* poolAddress, U_32 memorySize, U_32 elementSize, U_32 flags)
{
...
simplepool_isElement(J9SimplePool* simplePool, void* address)
{
    ...
    if (!simplePool) {
        ...
        if (((U_8*)address >= firstFreeSlot) || ((U_8*)address < firstPossibleElementSlot)) {
            ...
            Trc_simplepool_isElement_elementAddressOutOfRange(
                simplePool, address, firstPossibleElementSlot, firstFreeSlot);
        } else {
...
simplepool_maxNumElements(J9SimplePool* simplePool)
{
    ...
    if (NULL == simplePool) {
        ...
        maxElements = ((UDATA)J9SIMPLEPOOL_BLOCKEND(simplePool) - (UDATA)SP_FIRST_POSSIBLE_ELEMENT_SLOT(simplePool))
 ...
 */
U_32 simplepool_totalSize(U_32 nodeSize, U_32 numberOfElements)
{
    ...
    BOOLEAN (*doFunction)(void* anElement, void* userData), void* userData, UDATA skipCount)
{
    ...
    allocMapSize
        = ((simplePool->numElements + numElementsInFreeList) / 8) + 1; /* +1 byte to account for possible remainder */
    ...
    if (NULL != allocMap) {
        ...
        while (NULL != freeList) {
            ...
            allocMap[currElementIndex / 8]
                |= 0x80 >> (currElementIndex % 8); /* mark the corresponding bit in the map as free */
            freeList = J9SIMPLEPOOLFREELIST_NEXT(freeList);
        }
...
simplepool_verify(J9SimplePool* simplePool, U_32 memorySize, U_32 elementSize)
{
    ...
    if (NULL != simplePool) {
        ...
        if (elementSize != simplePool->elementSize) {
            ...
            Trc_simplepool_verify_firstFreeSlotOutOfRange(
                simplePool, firstFreeSlot, firstPossibleElementSlot, blockEnd);
        } else if (!SP_ELEMENT_IS_ALIGNED(simplePool, firstPossibleElementSlot, firstFreeSlot)) {
            /* the firstFreeSlot is unaligned */
            Trc_simplepool_verify_firstFreeSlotUnaligned(
                simplePool, firstFreeSlot, firstPossibleElementSlot, elementSize);
        ...
        if (verifies
            && (simplePool->numElements
                   != (((UDATA)firstFreeSlot - (UDATA)simplePool - sizeof(J9SimplePool)) / elementSize)
                       - freeListCounter)) {
            Trc_simplepool_verify_numberOfElementsIncorrect(simplePool, simplePool->numElements,
 ...
 * @return 		Header size
 */
U_32 simplepool_headerSize(void) { return sizeof(J9SimplePool); }
