...
static void debugMapAllLocals(DebugLocalMap* mapData)
{
    ...
    argBitsFromSignature(J9UTF8_DATA(J9ROMMETHOD_GET_SIGNATURE(mapData->romClass, romMethod)), mapData->resultArrayBase,
        (remainingLocals + 31) >> 5, (romMethod->modifiers & J9AccStatic) != 0);
...
#define LOCAL_SCRATCH 2048
    ...
    Trc_Map_j9localmap_DebugLocalBitsForPC_Method(
    ...
    J9VMThread* currentThread, J9Method* ramMethod, U_32 offsetPC, U_32 slot, char slotSignature, UDATA compressTypes)
{
    ...
    if (methodDebugInfo != NULL) {
        ...
        while (values != NULL) {
            if ((values->slotNumber == slot) && (offsetPC >= values->startVisibility)
                && (offsetPC < (values->startVisibility + values->visibilityLength))) {
    ...
    if (rc == J9_SLOT_VALIDATE_ERROR_NONE) {
        ...
        mapRC = j9localmap_DebugLocalBitsForPC(
            PORTLIB, romClass, romMethod, offsetPC, localMap, vm, j9mapmemory_GetBuffer, j9mapmemory_ReleaseBuffer);
        if (mapRC < 0) {
            switch (mapRC) {
...
void installDebugLocalMapper(J9JavaVM* vm) { vm->localMapFunction = j9localmap_DebugLocalBitsForPC; }
