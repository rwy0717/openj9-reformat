...
static void mapAllLocals(J9PortLibrary* portLibrary, J9ROMMethod* romMethod, PARALLEL_TYPE* unknownsByPC, UDATA startPC,
    U_32* resultArrayBase);
static IDATA mapLocalSet(J9PortLibrary* portLibrary, J9ROMMethod* romMethod, PARALLEL_TYPE* unknownsByPC, UDATA startPC,
    UDATA localIndexBase, PARALLEL_TYPE* knownLocals, PARALLEL_TYPE* knownObjects, BOOLEAN* unknownsWereUpdated);
    ...
    U_32* resultArrayBase)
{
    ...
    while (remainingLocals) {
        ...
        knownObjects = 0;
        mapLocalSet(portLibrary, romMethod, unknownsByPC, startPC, localIndexBase, &knownLocals, &knownObjects,
            &unknownsChanged);
            ...
            while (keepLooking) {
                ...
                for (e = 0; e < exceptionCount; e++) {
...
#if defined(DEBUG)
                    {
                        ...
                        j9file_printf(PORTLIB, J9PORT_TTY_OUT, "handler[%d] unknowns raw=0x%X masked=0x%X [", e,
                            rawUnknowns, unknownLocals);
                    ...
                    if ((~unknownsByPC[handler->handlerPC] & unknownLocals) != 0) {
                        ...
                        mapLocalSet(portLibrary, romMethod, unknownsByPC, handler->handlerPC, localIndexBase,
                            &exceptionKnownLocals, &knownObjects, &unknownsChangedWalkingHandler);
    ...
    *unknownsWereUpdated = FALSE;
    ...
    while (bcIndex < bcEnd) {
...
#if defined(DEBUG)
        {
            ...
            UDATA bitIndex;
            j9file_printf(PORTLIB, J9PORT_TTY_OUT, "pc=%d walk=%s seekLocals=0x%X [", pc,
                (shouldBeWalked ? "true" : "false"), seekLocals);
...
{
    argBitsFromSignature(J9UTF8_DATA(J9ROMMETHOD_GET_SIGNATURE(romClass, romMethod)), resultArrayBase,
        (J9_ARG_COUNT_FROM_ROM_METHOD(romMethod) + 31) >> 5, (romMethod->modifiers & J9AccStatic) != 0);
}
    ...
    U_32* resultArrayBase, void* userData, UDATA* (*getBuffer)(void* userData), void (*releaseBuffer)(void* userData))
{
    ...
    UDATA mapWords
        = (UDATA)((J9_TEMP_COUNT_FROM_ROM_METHOD(romMethod) + J9_ARG_COUNT_FROM_ROM_METHOD(romMethod) + 31) >> 5);
...
#define LOCAL_SCRATCH 2048
    ...
    Trc_Map_j9localmap_LocalBitsForPC_Method(
        J9_TEMP_COUNT_FROM_ROM_METHOD(romMethod) + J9_ARG_COUNT_FROM_ROM_METHOD(romMethod), pc,
