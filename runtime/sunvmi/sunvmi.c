...
extern JNIEXPORT jobject JNICALL JVM_FindClassFromClassLoader(
...
static void initializeReflectionGlobalsHook(
...
static UDATA latestUserDefinedLoaderIterator(J9VMThread* currentThread, J9StackWalkState* walkState)
{
    ...
    if ((classLoader != vm->systemClassLoader) && !isPlatformClassLoader) {
        ...
        if ((vm->srMethodAccessor
                && vmFuncs->instanceOfOrCheckCast(
                       currentClass, J9VM_J9CLASS_FROM_HEAPCLASS(currentThread, *((j9object_t*)vm->srMethodAccessor))))
            ...
            || (vm->jliArgumentHelper
                   && vmFuncs->instanceOfOrCheckCast(currentClass,
                          J9VM_J9CLASS_FROM_HEAPCLASS(currentThread, *((j9object_t*)vm->jliArgumentHelper))))) {
...
static UDATA getCallerClassIterator(J9VMThread* currentThread, J9StackWalkState* walkState)
{
    J9JavaVM* vm = currentThread->javaVM;

    if ((J9_ROM_METHOD_FROM_RAM_METHOD(walkState->method)->modifiers & J9_JAVA_METHOD_FRAME_ITERATOR_SKIP)
...
static UDATA getCallerClassJEP176Iterator(J9VMThread* currentThread, J9StackWalkState* walkState)
{
    J9JavaVM* vm = currentThread->javaVM;
    J9InternalVMFunctions* vmFuncs = vm->internalVMFunctions;
    J9Class* currentClass = J9_CLASS_FROM_CP(walkState->constantPool);
    ...
    case 1:
        ...
        if (((vm->systemClassLoader != currentClass->classLoader)
                && (vm->extensionClassLoader != currentClass->classLoader))
            || J9_ARE_NO_BITS_SET(
                   J9_ROM_METHOD_FROM_RAM_METHOD(walkState->method)->modifiers, J9_JAVA_METHOD_CALLER_SENSITIVE)) {
...
#endif /* JAVA_SPEC_VERSION >= 11 */
{
    ...
    if (TRUE == (UDATA)walkState.userData3) {
        vmFuncs->setCurrentExceptionNLS(
            vmThread, J9VMCONSTANTPOOL_JAVALANGINTERNALERROR, J9NLS_VM_CALLER_NOT_ANNOTATED_AS_CALLERSENSITIVE);
    } else {
...
static jint initializeReflectionGlobals(JNIEnv* env, BOOLEAN includeAccessors)
{
    ...
    if (J2SE_SHAPE(vm) != J2SE_SHAPE_RAW) {
        ...
        VM.jliMethodHandles_Lookup_checkSecurity
            = (*env)->GetMethodID(env, clazz, "checkSecurity", "(Ljava/lang/Class;Ljava/lang/Class;I)V");
    ...
    J9HookInterface** hookInterface, UDATA eventNum, void* eventData, void* userData)
{
    ...
    (*hookInterface)
        ->J9HookUnregister(
            hookInterface, J9HOOK_VM_INITIALIZE_REQUIRED_CLASSES_DONE, initializeReflectionGlobalsHook, NULL);
}
...
JNIEXPORT void JNICALL JVM_GCNoCompact_Impl(void)
{
    ...
    VM.javaVM->memoryManagerFunctions->j9gc_modron_global_collect_with_overrides(
        currentThread, J9MMCONSTANT_EXPLICIT_GC_NOT_AGGRESSIVE);
 ...
 * Special cases:
 * --------------
 * 	For JSR292, MethodHandles.lookup().find* methods need to store the Lookup's accessClass into the Class[] rather
 * than MethodHandles.Lookup.class
 *
 ...
 * walkState->userData3 (IDATA) is the index of the frame where accessClass should be inserted if the method
 * jliMethodHandles_Lookup_checkSecurity was found on the stack
 */
static UDATA getClassContextIterator(J9VMThread* currentThread, J9StackWalkState* walkState)
{
    J9JavaVM* vm = currentThread->javaVM;
        ...
        if ((vm->srMethodAccessor
            ...
            if (walkState->userData2 != NULL) {
                ...
                if (walkState->userData3 == walkState->userData1) {
                    ...
                    /* accessClass is inserted at jliMethodHandles_Lookup_checkSecurity + 2, which is where the
                     * SecurityManager expects the caller to be */
...
/**
 * Returns an array of strings for the currently loaded system packages. The name returned for each package is to be '/'
 * separated and end with a '/' character.
 *
            ...
            if (jlsClass) {
                ...
                if (result) {
                    for (i = 0; i < packageCount; i++) {
                         ...
                         */
                        if (J2SE_VERSION(vm) >= J2SE_19) {
                            packageString = vm->memoryManagerFunctions->j9gc_createJavaLangString(
                                vmThread, (U_8*)packageName, packageNameLength, 0);
                        } else {
                            packageString = funcs->catUtfToString4(
                                vmThread, packageName, packageNameLength, (U_8*)"/", 1, NULL, 0, NULL, 0);
                        }
                        if (packageString) {
...
JNIEXPORT jstring JNICALL JVM_GetSystemPackage_Impl(JNIEnv* env, jstring pkgName)
{
    ...
    if (NULL != idEntry) {
        J9ROMClass* resultROMClass
            = (J9ROMClass*)(idEntry->taggedROMClass & ~(UDATA)(J9PACKAGE_ID_TAG | J9PACKAGE_ID_GENERATED));
 ...
 */
static jobject internalFindClassFromClassLoader(
    JNIEnv* env, char* className, jboolean init, jobject classLoader, jboolean throwError)
{
    J9JavaVM* vm;
    ...
    clazz = vmFuncs->internalFindClassUTF8(
        currentThread, (U_8*)className, strlen((const char*)className), loader, options);
 ...
 */
static UDATA gpProtectedInternalFindClassFromClassLoader(void* entryArg)
{
    J9RedirectedFindClassFromClassLoaderArgs* args = (J9RedirectedFindClassFromClassLoaderArgs*)entryArg;
    return (UDATA)internalFindClassFromClassLoader(
        args->env, args->className, args->init, args->classLoader, args->throwError);
}
 ...
 *
 */
JNIEXPORT jobject JNICALL JVM_FindClassFromClassLoader_Impl(
    JNIEnv* env, char* className, jboolean init, jobject classLoader, jboolean throwError)
{
    if (NULL == env) {
        return NULL;
...
static void gcDidComplete(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    PORT_ACCESS_FROM_JAVAVM(VM.javaVM);
    VM.lastGCTime = j9time_current_time_millis();
}

static BOOLEAN initializeThreadFunctions(J9PortLibrary* portLibrary)
{
...
JNIEXPORT void JNICALL JVM_ExtendBootClassPath_Impl(JNIEnv* env, const char* pathSegment)
{
    ...
    if (vm->systemClassLoader->classLoaderObject == NULL) {
        vm->internalVMFunctions->addToBootstrapClassLoaderSearch(
            vm, pathSegment, CLS_TYPE_ADD_TO_SYSTEM_PROPERTY, FALSE);
    } else {
        vm->internalVMFunctions->addToBootstrapClassLoaderSearch(
            vm, pathSegment, CLS_TYPE_ADD_TO_SYSTEM_CLASSLOADER, FALSE);
    }

    return;
}

static SunVMI VMIFunctionTable = { SUNVMI_VERSION_1_1, JVM_AllocateNewArray_Impl, JVM_AllocateNewObject_Impl,
    JVM_FreeMemory_Impl, JVM_GC_Impl, JVM_GCNoCompact_Impl, JVM_GetCallerClass_Impl, JVM_GetClassAccessFlags_Impl,
    JVM_GetClassContext_Impl, JVM_GetClassLoader_Impl, JVM_GetSystemPackage_Impl, JVM_GetSystemPackages_Impl,
    JVM_GetThreadInterruptEvent_Impl, JVM_Halt_Impl, JVM_InvokeMethod_Impl, JVM_LatestUserDefinedLoader_Impl,
    JVM_MaxObjectInspectionAge_Impl, JVM_NewInstanceFromConstructor_Impl, JVM_TotalMemory_Impl, JVM_MaxMemory_Impl,
    JVM_FindClassFromClassLoader_Impl, JVM_ExtendBootClassPath_Impl, JVM_GetClassTypeAnnotations_Impl,
    JVM_GetFieldTypeAnnotations_Impl, JVM_GetMethodTypeAnnotations_Impl, JVM_GetMethodParameters_Impl };

/**
 * Handle a GetEnv() request looking for the SUNVMI_VERSION_X_Y interface.
 */
static void vmGetEnvHook(J9HookInterface** hookInterface, UDATA eventNum, void* eventData, void* userData)
{
...
SunVMI_LifecycleEvent(J9JavaVM* vm, IDATA stage, void* reserved)
{
    ...
    case JCL_INITIALIZED: {
        ...
        anyErrors = (*vmHooks)->J9HookRegisterWithCallSite(vmHooks, J9HOOK_VM_INITIALIZE_REQUIRED_CLASSES_DONE,
            initializeReflectionGlobalsHook, OMR_GET_CALLSITE(), NULL);
        if (anyErrors) {
            return J9VMDLLMAIN_FAILED;
        }

        anyErrors
            = (*vmHooks)->J9HookRegisterWithCallSite(vmHooks, J9HOOK_VM_GETENV, vmGetEnvHook, OMR_GET_CALLSITE(), NULL);
        if (anyErrors) {
            return J9VMDLLMAIN_FAILED;
        }
        break;
    ...
    case VM_INITIALIZATION_COMPLETE: {
        /* the VM is now ready to go so setup the end GC hook we require for getting the time for the last time a GC
         * ended */
        J9HookInterface** gcOmrHooks = vm->memoryManagerFunctions->j9gc_get_omr_hook_interface(vm->omrVM);
        if ((*gcOmrHooks)
                ->J9HookRegisterWithCallSite(
                    gcOmrHooks, J9HOOK_MM_OMR_GLOBAL_GC_END, gcDidComplete, OMR_GET_CALLSITE(), NULL)) {
