...
static J9SimplePoolInputData INPUTDATA[] = {
    ...
    { (SIMPLEPOOL_HEADER_SIZE + SIMPLEPOOL_MIN_ELEMENT_SIZE * 10),
        SIMPLEPOOL_MIN_ELEMENT_SIZE }, /* 10 minimum sized elements */
    { (SIMPLEPOOL_HEADER_SIZE + SIMPLEPOOL_MIN_ELEMENT_SIZE * 50),
        SIMPLEPOOL_MIN_ELEMENT_SIZE }, /* 50 minimum sized elements */
    { (SIMPLEPOOL_HEADER_SIZE + SIMPLEPOOL_MIN_ELEMENT_SIZE * 2000),
        SIMPLEPOOL_MIN_ELEMENT_SIZE }, /* 2000 minimum sized elements */
    { (SIMPLEPOOL_HEADER_SIZE + SIMPLEPOOL_MEDIUM_ELEMENT_SIZE * 5),
        SIMPLEPOOL_MEDIUM_ELEMENT_SIZE }, /* 5 medium sized elements */
    { (SIMPLEPOOL_HEADER_SIZE + SIMPLEPOOL_MEDIUM_ELEMENT_SIZE * 200),
        SIMPLEPOOL_MEDIUM_ELEMENT_SIZE }, /* 200 medium sized elements */
    { (SIMPLEPOOL_HEADER_SIZE + SIMPLEPOOL_MEDIUM_ELEMENT_SIZE * 3000),
        SIMPLEPOOL_MEDIUM_ELEMENT_SIZE }, /* 3000 medium sized elements */
    { (SIMPLEPOOL_HEADER_SIZE + SIMPLEPOOL_LARGE_ELEMENT_SIZE * 2),
        SIMPLEPOOL_LARGE_ELEMENT_SIZE }, /* 2 large sized elements */
    { (SIMPLEPOOL_HEADER_SIZE + SIMPLEPOOL_LARGE_ELEMENT_SIZE * 501),
        SIMPLEPOOL_LARGE_ELEMENT_SIZE }, /* 501 large sized elements */
    { (SIMPLEPOOL_HEADER_SIZE + SIMPLEPOOL_LARGE_ELEMENT_SIZE * 1000),
        SIMPLEPOOL_LARGE_ELEMENT_SIZE }, /* 1000 large sized elements */
    { (SIMPLEPOOL_HEADER_SIZE + SIMPLEPOOL_LARGE_ELEMENT_SIZE * 1023 + SIMPLEPOOL_MEDIUM_ELEMENT_SIZE),
        SIMPLEPOOL_LARGE_ELEMENT_SIZE }, /* 1023 large sized elements, + SIMPLEPOOL_MEDIUM_ELEMENT_SIZE lost space in
                                            memory size */
...
static BOOLEAN createSimplePools(J9PortLibrary* portLib, UDATA* passCount, UDATA* failCount)
{
    ...
    for (poolCntr = 0; poolCntr < NUM_POOLS; poolCntr++) {
        ...
        if (NULL == poolAddress) {
            j9tty_printf(PORTLIB,
                "Error: createSimplePools() - unable to allocate memory for simple pool creation test %u\n", poolCntr);
            (*failCount)++;
            return 0;
        } else {
            ...
            if (resultData->simplePool == NULL) {
                j9tty_printf(PORTLIB,
                    "Error: createSimplePools() - simplepool_new returned NULL for simple pool test %u\n", poolCntr);
                (*failCount)++;
                return FALSE;
            } else {
                /* verify simple pool */
                if (!simplepool_verify(resultData->simplePool, inputData->memorySize, inputData->elementSize)) {
                    j9tty_printf(
                        PORTLIB, "Error: createSimplePools() - simplePool # %u failed verification\n", poolCntr);
                    (*failCount)++;
                    return FALSE;
                }
            }
    ...
    for (poolCntr = 0; poolCntr < NUM_FAULTY_POOLS; poolCntr++) {
        ...
        if (0 < inputData->memorySize && NULL == poolAddress) {
            j9tty_printf(PORTLIB,
                "Error: createSimplePools() - unable to allocate memory for faulty simple pool creation test %u\n",
                poolCntr);
            (*failCount)++;
            return FALSE;
        } else {
            ...
            if (NULL != simplePool) {
                j9tty_printf(PORTLIB,
                    "Error: createSimplePools() - simplepool_new did not return NULL for faulty input test %u\n",
                    poolCntr);
                (*failCount)++;
                return FALSE;
            }
...
static BOOLEAN insertAndRemoveElements(J9PortLibrary* portLib, UDATA* passCount, UDATA* failCount)
{
    ...
    for (poolCntr = 0; poolCntr < NUM_POOLS; poolCntr++) {
        ...
        if (simplepool_newElement(NULL) != NULL) {
            j9tty_printf(PORTLIB,
                "Error: insertAndRemoveElements() - simplepool_newElement(NULL) succeeded simplePool # %u \n",
                poolCntr);
            (*failCount)++;
            return FALSE;
        }
        ...
        for (i = 0; i < maxElements; i++) {
            ...
            if (NULL == element) {
                j9tty_printf(PORTLIB,
                    "Error: insertAndRemoveElements() - simplepool_newElement # %u failed for simplePool # %u \n", i,
                    poolCntr);
                (*failCount)++;
                return FALSE;
            } else {
                ...
                if (i == 0) {
                    ...
                    if (0 == simplepool_removeElement(NULL, element)) {
                        j9tty_printf(PORTLIB,
                            "Error: insertAndRemoveElements() - Removing element from NULL simplePool # %u \n", i,
                            poolCntr);
                        (*failCount)++;
                        return FALSE;
                    }
                    ...
                    if (0 == simplepool_removeElement(resultData->simplePool, NULL)) {
                        j9tty_printf(PORTLIB,
                            "Error: insertAndRemoveElements() - Removing NULL element from simplePool # %u \n", i,
                            poolCntr);
                        (*failCount)++;
                        return FALSE;
                    }
                ...
                if (0 == (i % 3)) {
                    if (0 != simplepool_removeElement(resultData->simplePool, element)) {
                        j9tty_printf(PORTLIB,
                            "Error: insertAndRemoveElements() - Removing element # %u failed for simplePool # %u \n", i,
                            poolCntr);
                        (*failCount)++;
                        return FALSE;
                    }
        ...
        if (!simplepool_verify(resultData->simplePool, inputData->memorySize, inputData->elementSize)) {
            j9tty_printf(PORTLIB,
                "Error: insertAndRemoveElements() - simplePool # %u failed verification in insertAndRemoveElements()\n",
                poolCntr);
            (*failCount)++;
            return FALSE;
        }
        ...
        if (i != 0) {
            j9tty_printf(PORTLIB,
                "Error: insertAndRemoveElements() - simplepool_numElements(NULL) returned %u instead of 0 for "
                "simplePool # %u \n",
                i, poolCntr);
            (*failCount)++;
            return FALSE;
        ...
        if (i != numElementsAdded) {
            j9tty_printf(PORTLIB,
                "Error: insertAndRemoveElements() - simplepool_numElements returned %u instead of %u for simplePool # "
                "%u \n",
        ...
        for (i = 0; i < (maxElements - numElementsAdded); i++) {
            ...
            if (NULL == element) {
                j9tty_printf(PORTLIB,
                    "Error: insertAndRemoveElements() - Filling up simple pool - simplepool_newElement # %u failed for "
                    "simplePool # %u \n",
                    i, poolCntr);
                (*failCount)++;
                return FALSE;
        ...
        if (i != 0) {
            j9tty_printf(PORTLIB,
                "Error: insertAndRemoveElements() - simplepool_maxNumElements returned %u instead of 0 for simplePool "
                "# %u \n",
                i, poolCntr);
            (*failCount)++;
            return FALSE;
        ...
        if (i != maxElements) {
            j9tty_printf(PORTLIB,
                "Error: insertAndRemoveElements() - simplepool_maxNumElements returned %u instead of %u for simplePool "
                "# %u \n",
        ...
        if (NULL != simplepool_newElement(resultData->simplePool)) {
            j9tty_printf(PORTLIB,
                "Error: insertAndRemoveElements() - simplePool should have been full and simplepool_newElement should "
                "have returned NULL for simplePool  # %u \n",
                poolCntr);
            (*failCount)++;
            return FALSE;
        }
        ...
        if (i != maxElements) {
            j9tty_printf(PORTLIB,
                "Error: insertAndRemoveElements() - simplepool_numElements returned %u instead of returning "
                "maxElements = %u after getting filled, simplePool # %u \n",
                i, maxElements, poolCntr);
            (*failCount)++;
            return FALSE;
        }
        ...
        if (!simplepool_verify(resultData->simplePool, inputData->memorySize, inputData->elementSize)) {
            j9tty_printf(PORTLIB,
                "Error: insertAndRemoveElements() - simplePool # %u failed second verification in "
                "insertAndRemoveElements()\n",
                poolCntr);
            (*failCount)++;
            return FALSE;
        }
        ...
        if (i != 0) {
            j9tty_printf(PORTLIB,
                "Error: insertAndRemoveElements() - simplepool_numElements returned %u instead of returning 0 after "
                "simplepool_clear() was called, simplePool # %u \n",
                i, poolCntr);
            (*failCount)++;
            return FALSE;
        }
 ...
 */
static BOOLEAN doVerifyAndRemoveElement(void* element, void* voidUserData)
{
...
static BOOLEAN fillIterateAndRemove(J9PortLibrary* portLib, UDATA* passCount, UDATA* failCount)
{
    ...
    for (poolCntr = 0; poolCntr < NUM_POOLS; poolCntr++) {
        ...
        for (i = 0; i < maxElements; i++) {
            ...
            if (NULL == element) {
                j9tty_printf(PORTLIB,
                    "Error: fillIterateAndRemove() - simplepool_newElement # %u failed for simplePool # %u \n", i,
                    poolCntr);
                (*failCount)++;
                return FALSE;
            } else {
                ...
                if (0 == (i % 4)) {
                    if (0 != simplepool_removeElement(resultData->simplePool, element)) {
                        j9tty_printf(PORTLIB,
                            "Error: fillIterateAndRemove() - Removing element # %u failed for simplePool # %u \n", i,
                            poolCntr);
                        (*failCount)++;
                        return FALSE;
                    }
        ...
        for (i = 0; i < elementCount; i += OMR_MAX(3, elementCount / 5)) {
            if (0 != simplepool_removeElement(resultData->simplePool, elementList[i])) {
                j9tty_printf(PORTLIB,
                    "Error: fillIterateAndRemove() - Removing element # %u %x failed for simplePool # %u \n", i,
                    elementList[i], poolCntr);
                (*failCount)++;
                return FALSE;
            }
            numElementsAdded--;
        ...
        if (!simplepool_verify(resultData->simplePool, inputData->memorySize, inputData->elementSize)) {
            j9tty_printf(PORTLIB,
                "Error: fillIterateAndRemove() - simplePool # %u failed verification in fillIterateAndRemove() after "
                "elements were added and removed from the simple pool\n",
                poolCntr);
            (*failCount)++;
            return FALSE;
        }
        ...
        if (i != numElementsAdded) {
            j9tty_printf(PORTLIB,
                "Error: fillIterateAndRemove() - simplepool_numElements returned %u instead of %u for simplePool # %u  "
                "after initial fill and remove\n",
                i, numElementsAdded, poolCntr);
            (*failCount)++;
            return FALSE;
        }
        ...
        if (numElementsAdded > 0) {
            ...
            if (TRUE == simplepool_do(resultData->simplePool, portLib, doVerifyAndRemoveElement, &userData)) {
                j9tty_printf(PORTLIB,
                    "Error: fillIterateAndRemove() - simplepool_do returned TRUE for simplePool # %u\n", poolCntr);
                (*failCount)++;
                return FALSE;
            }
            ...
            if (i != (numElementsAdded - 1)) {
                j9tty_printf(PORTLIB,
                    "Error: fillIterateAndRemove() - simplepool_numElements returned %u instead of %u for simplePool # "
                    "%u  after simplepool_do stopped\n",
        ...
        if (i != 0) {
            j9tty_printf(PORTLIB,
                "Error: fillIterateAndRemove() - simplepool_numElements returned %u instead of 0 for simplePool # %u "
                "after iterating with simpelpool_do and attempting to remove all elements\n",
                i, poolCntr);
            (*failCount)++;
            return FALSE;
        }
...
static BOOLEAN fillIterateAndRemove2(J9PortLibrary* portLib, UDATA* passCount, UDATA* failCount)
{
    ...
    for (poolCntr = 0; poolCntr < NUM_POOLS; poolCntr++) {
        ...
        if (!simplepool_verify(resultData->simplePool, inputData->memorySize, inputData->elementSize)) {
            j9tty_printf(PORTLIB,
                "Error: fillIterateAndRemove2() - simplePool # %u failed verification at the beginning of "
                "fillIterateAndRemove()\n",
                poolCntr);
            (*failCount)++;
            return FALSE;
        }
        ...
        for (i = 0; i < maxElements; i++) {
            ...
            if (NULL == element) {
                j9tty_printf(PORTLIB,
                    "Error: fillIterateAndRemove2() - simplepool_newElement # %u failed for simplePool # %u \n", i,
                    poolCntr);
                (*failCount)++;
                return FALSE;
            } else {
        ...
        if (!simplepool_verify(resultData->simplePool, inputData->memorySize, inputData->elementSize)) {
            j9tty_printf(PORTLIB,
                "Error: fillIterateAndRemove2() - simplePool # %u failed verification in fillIterateAndRemove() after "
                "elements were added and removed from the simple pool\n",
                poolCntr);
            (*failCount)++;
            return FALSE;
        ...
        if (i != maxElements) {
            j9tty_printf(PORTLIB,
                "Error: fillIterateAndRemove2() - simplepool_numElements returned %u instead of %u for simplePool # %u "
                " after initial fill and remove\n",
                i, maxElements, poolCntr);
            (*failCount)++;
            return FALSE;
        ...
        if (i != 0) {
            j9tty_printf(PORTLIB,
                "Error: fillIterateAndRemove2() - simplepool_numElements returned %u instead of 0 for simplePool # %u "
                "after iterating with simpelpool_do and attempting to remove all elements\n",
                i, poolCntr);
            (*failCount)++;
            return FALSE;
...
static BOOLEAN detectCorruption(J9PortLibrary* portLib, UDATA* passCount, UDATA* failCount)
{
    ...
    for (poolCntr = 0; poolCntr < NUM_POOLS; poolCntr++) {
        ...
        for (i = 0; i < maxElements; i++) {
            ...
            if (NULL == element) {
                j9tty_printf(PORTLIB,
                    "Error: detectCorruption() - simplepool_newElement # %u failed for simplePool # %u \n", i,
                    poolCntr);
                (*failCount)++;
                return FALSE;
            } else {
                ...
                if (0 == (i % 4)) {
                    if (0 != simplepool_removeElement(resultData->simplePool, element)) {
                        j9tty_printf(PORTLIB,
                            "Error: detectCorruption() - Removing element # %u failed for simplePool # %u \n", i,
                            poolCntr);
                        (*failCount)++;
                        return FALSE;
                    }
        ...
        for (i = 0; i < elementCount; i += OMR_MAX(3, elementCount / 5)) {
            if (0 != simplepool_removeElement(resultData->simplePool, elementList[i])) {
                j9tty_printf(PORTLIB,
                    "Error: detectCorruption() - Removing element # %u %x failed for simplePool # %u \n", i,
                    elementList[i], poolCntr);
                (*failCount)++;
                return FALSE;
            }
        ...
        if (simplepool_verify(resultData->simplePool, inputData->memorySize, inputData->elementSize)) {
            j9tty_printf(PORTLIB,
                "Error: detectCorruption() - simplepool_verify did not detect elementSize corruption for simplepool # "
                "%u\n",
                poolCntr);
            (*failCount)++;
            return FALSE;
        ...
        if (simplepool_verify(resultData->simplePool, inputData->memorySize, inputData->elementSize)) {
            j9tty_printf(PORTLIB,
                "Error: detectCorruption() - simplepool_verify did not detect firstFreeSlot corruption for simplepool "
                "# %u\n",
                poolCntr);
            (*failCount)++;
            return FALSE;
        ...
        if (simplepool_verify(resultData->simplePool, inputData->memorySize, inputData->elementSize)) {
            j9tty_printf(PORTLIB,
                "Error: detectCorruption() - simplepool_verify did not detect flags corruption for simplepool # %u\n",
                poolCntr);
            (*failCount)++;
            return FALSE;
        }
        ...
        if (simplepool_verify(resultData->simplePool, inputData->memorySize, inputData->elementSize)) {
            j9tty_printf(PORTLIB,
                "Error: detectCorruption() - simplepool_verify did not detect blockEnd corruption for simplepool # "
                "%u\n",
                poolCntr);
            (*failCount)++;
            return FALSE;
        ...
        if (NULL != freeList) {
            ...
            if (simplepool_verify(resultData->simplePool, inputData->memorySize, inputData->elementSize)) {
                j9tty_printf(PORTLIB,
                    "Error: detectCorruption() - simplepool_verify did not detect unaligned freeList pointer "
                    "corruption for simplepool # %u\n",
                    poolCntr);
                (*failCount)++;
                return FALSE;
            }
            ...
            if (simplepool_verify(resultData->simplePool, inputData->memorySize, inputData->elementSize)) {
                j9tty_printf(PORTLIB,
                    "Error: detectCorruption() - simplepool_verify did not detect out of range freeList pointer "
                    "corruption for simplepool # %u\n",
                    poolCntr);
                (*failCount)++;
                return FALSE;
            }
            SRP_SET(resultData->simplePool->freeList,
                resultData->simplePool); /* set free list before first possible element slot */
            if (simplepool_verify(resultData->simplePool, inputData->memorySize, inputData->elementSize)) {
                j9tty_printf(PORTLIB,
                    "Error: detectCorruption() - simplepool_verify did not detect out of range freeList pointer "
                    "corruption when free list was set before the first possible element slot for simplepool # %u\n",
                    poolCntr);
                (*failCount)++;
                return FALSE;
            }
            ...
            do {
                ...
                if (simplepool_verify(resultData->simplePool, inputData->memorySize, inputData->elementSize)) {
                    j9tty_printf(PORTLIB,
                        "Error: detectCorruption() - simplepool_verify did not detect incorrect (NULL) free list back "
                        "pointer for simplepool # %u\n",
                        poolCntr);
                    (*failCount)++;
                    return FALSE;
                }
                ...
                if (simplepool_verify(resultData->simplePool, inputData->memorySize, inputData->elementSize)) {
                    j9tty_printf(PORTLIB,
                        "Error: detectCorruption() - simplepool_verify did not detect incorrect simple pool back "
                        "pointer freeList corruption for simplepool # %u\n",
                        poolCntr);
                    (*failCount)++;
                    return FALSE;
                }
                ...
                if (simplepool_verify(resultData->simplePool, inputData->memorySize, inputData->elementSize)) {
                    j9tty_printf(PORTLIB,
                        "Error: detectCorruption() - simplepool_verify did not detect out of range free list next "
                        "pointer (before range) for simplepool # %u\n",
                        poolCntr);
                    (*failCount)++;
                    return FALSE;
                }
                SRP_SET(freeList->next,
                    J9SIMPLEPOOL_FIRSTFREESLOT(
                        resultData->simplePool)); /* out of range - after possible element range */
                if (simplepool_verify(resultData->simplePool, inputData->memorySize, inputData->elementSize)) {
                    j9tty_printf(PORTLIB,
                        "Error: detectCorruption() - simplepool_verify did not detect out of range free list next "
                        "pointer (after range) for simplepool # %u\n",
                        poolCntr);
                    (*failCount)++;
                    return FALSE;
                }
                ...
                if (simplepool_verify(resultData->simplePool, inputData->memorySize, inputData->elementSize)) {
                    j9tty_printf(PORTLIB,
                        "Error: detectCorruption() - simplepool_verify did not detect unaligned free list next pointer "
                        "for simplepool # %u\n",
                        poolCntr);
                    (*failCount)++;
                    return FALSE;
            ...
            if (!simplepool_verify(resultData->simplePool, inputData->memorySize, inputData->elementSize)) {
                j9tty_printf(PORTLIB,
                    "Error: detectCorruptionInFreeList() - final simplepool_verify failed for simplepool # %u\n",
                    poolCntr);
                (*failCount)++;
                return FALSE;
            }
...
static BOOLEAN detectCorruptionInRemoveElement(J9PortLibrary* portLib, UDATA* passCount, UDATA* failCount)
{
    ...
    for (poolCntr = 0; poolCntr < NUM_POOLS; poolCntr++) {
        ...
        for (i = 0; i < numElementsToAdd; i++) {
            ...
            if (NULL == element) {
                j9tty_printf(PORTLIB,
                    "Error: detectCorruptionInRemoveElement() - simplepool_newElement # %u failed for simplePool # %u "
                    "\n",
                    i, poolCntr);
                (*failCount)++;
                return FALSE;
                ...
                if (0 == (i % 3)) {
                    if (0
                        == simplepool_removeElement(
                               resultData->simplePool, resultData->simplePool)) { /* element out of range - before */
                        j9tty_printf(PORTLIB,
                            "Error: detectCorruptionInRemoveElement() - simplepool_removeElement() #u did not detect "
                            "that the passed in element address was before the first possible element slot for "
                            "simplePool # %u \n",
                            i, poolCntr);
                        (*failCount)++;
                        return FALSE;
                    } else if (0
                        == simplepool_removeElement(resultData->simplePool,
                               J9SIMPLEPOOL_FIRSTFREESLOT(resultData->simplePool))) { /* element out of range - after */
                        j9tty_printf(PORTLIB,
                            "Error: detectCorruptionInRemoveElement() - simplepool_removeElement() #u did not detect "
                            "that the passed in element address was after the possible range of elements for "
                            "simplePool # %u \n",
                            i, poolCntr);
                        (*failCount)++;
                        return FALSE;
                        ...
                        == simplepool_removeElement(
                               resultData->simplePool, (void*)((UDATA)element + 1))) { /* element unaligned */
                        j9tty_printf(PORTLIB,
                            "Error: detectCorruptionInRemoveElement() - simplepool_removeElement() #u did not detect "
                            "that the passed in element address was unaligned for simplePool # %u \n",
                            i, poolCntr);
                        (*failCount)++;
                        return FALSE;
                    } else if (0
                        != simplepool_removeElement(resultData->simplePool, element)) { /* this one should succeed */
                        j9tty_printf(PORTLIB,
                            "Error: detectCorruptionInRemoveElement() - Removing element # %u failed for simplePool # "
                            "%u \n",
                            i, poolCntr);
                        (*failCount)++;
                        return FALSE;
        ...
        if (!simplepool_verify(resultData->simplePool, inputData->memorySize, inputData->elementSize)) {
            j9tty_printf(PORTLIB,
                "Error: detectCorruptionInRemoveElement() - simplepool_verify failed for simplepool # %u\n", poolCntr);
            (*failCount)++;
            return FALSE;
        }
        (*passCount)++;
 ...
 */
static BOOLEAN doVerifyAndCountElements(void* element, void* voidUserData)
{
    J9SimplePoolUserData* userData = (J9SimplePoolUserData*)voidUserData;
    U_8* currByte = element;
    UDATA i = 0;
...
static BOOLEAN checkConsistency(J9PortLibrary* portLib, UDATA* passCount, UDATA* failCount)
{
    ...
    for (poolCntr = 0; poolCntr < NUM_POOLS; poolCntr++) {
        ...
        if (elementList == NULL) {
            j9tty_printf(
                PORTLIB, "Error: fillCheckConsistencyAndRemove() - out of memory allocating %d bytes\n", listSize);
            (*failCount)++;
            return FALSE;
        }
        ...
        for (i = 0; i < maxElements; i++) {
            ...
            if (NULL == element) {
                j9tty_printf(PORTLIB,
                    "Error: fillCheckConsistencyAndRemove() - simplepool_newElement # %u failed for simplePool # %u \n",
                    i, poolCntr);
                (*failCount)++;
                return FALSE;
            } else {
                ...
                if (0 == (i % 4)) {
                    if (0 != simplepool_removeElement(resultData->simplePool, element)) {
                        j9tty_printf(PORTLIB,
                            "Error: fillCheckConsistencyAndRemove() - Removing element # %u failed for simplePool # %u "
                            "\n",
                            i, poolCntr);
                        (*failCount)++;
                        return FALSE;
        ...
        for (i = 1; i < elementCount; i += OMR_MAX(3, elementCount / 5)) {
            if (0 != simplepool_removeElement(resultData->simplePool, elementList[i])) {
                j9tty_printf(PORTLIB,
                    "Error: fillCheckConsistencyAndRemove() - Removing element # %u %x failed for simplePool # %u \n",
        ...
        if (!simplepool_verify(resultData->simplePool, inputData->memorySize, inputData->elementSize)) {
            j9tty_printf(PORTLIB,
                "Error: fillCheckConsistencyAndRemove() - simplePool # %u failed verification in "
                "fillIterateAndRemove() after elements were added and removed from the simple pool\n",
                poolCntr);
            (*failCount)++;
            return FALSE;
        }
        ...
        if (i != numElementsAdded) {
            j9tty_printf(PORTLIB,
                "Error: fillCheckConsistencyAndRemove() - simplepool_numElements returned %u instead of %u for "
                "simplePool # %u  after initial fill and remove\n",
                i, numElementsAdded, poolCntr);
            (*failCount)++;
            return FALSE;
        }
        ...
        if (numElementsAdded > 0) {
            ...
            userData.resultValue = FALSE;
            if (TRUE
                == simplepool_checkConsistency(
                       resultData->simplePool, portLib, doVerifyAndRemoveElement, &userData, 1)) {
                j9tty_printf(PORTLIB,
                    "Error: fillCheckConsistencyAndRemove() - simplepool_checkConsistency returned TRUE for simplePool "
                    "# %u\n",
                    poolCntr);
                (*failCount)++;
                return FALSE;
            ...
            if (i != numElementsAdded) {
                j9tty_printf(PORTLIB,
                    "Error: fillCheckConsistencyAndRemove() - simplepool_numElements returned %u instead of %u for "
                    "simplePool # %u  after simplepool_checkConsistency stopped\n",
                    i, numElementsAdded, poolCntr);
                (*failCount)++;
                return FALSE;
            }
        ...
        if (FALSE
            == simplepool_checkConsistency(resultData->simplePool, portLib, doVerifyAndCountElements, &userData, 1)) {
            j9tty_printf(PORTLIB,
                "Error: fillCheckConsistencyAndRemove() - simplepool_checkConsistency returned FALSE for simplePool # "
                "%u\n",
                poolCntr);
            (*failCount)++;
            return FALSE;
        ...
        if (((userData.count == 0) && (numElementsAdded != 0))
            ...
            j9tty_printf(PORTLIB,
                "Error: fillCheckConsistencyAndRemove() - simplepool_checkConsistency count is %u for %u elements for "
                "simplePool # %u after iterating with simplepool_checkConsistency\n",
