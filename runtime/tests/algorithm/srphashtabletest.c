...
static BOOLEAN checkIntegrity(J9PortLibrary* portLib, char* id, J9SRPHashTable* srptable, UDATA* data, U_32 dataLength,
    UDATA removeOffset, UDATA i);
static BOOLEAN runTests(J9PortLibrary* portLib, char* id, J9SRPHashTable** srptable, UDATA* data, U_32 dataLength,
    UDATA reverseRemove, BOOLEAN recreateSRPHashTable);
static void testSRPHashtable(
    J9PortLibrary* portLib, char* id, UDATA* data, U_32 dataLength, UDATA* passCount, UDATA* failCount);
 ...
 *
 */
static UDATA hashFn(void* key, void* userData) { return *(UDATA*)key; }
 ...
 *
 * @param 	startOffset	Starting offset of the data. If it is -1, then it is reverse order and we start from the
 * end.
 * @param	dataLength	The size of the data array.
 * @param 	i			Number of iterations to find the next/previous offset starting from
 * startOffset/end.
 ...
 *
 */
static UDATA userCheckIntegrityFn(void* in, void* userData)
{
    ...
    if (*currentNode >= sizeof(userIntegrityCheckData->dup) / sizeof(UDATA)) {
        j9tty_printf(PORTLIB, "SRP Hashtable %s dup detection, values must be < %d\n", userIntegrityCheckData->id,
            sizeof(userIntegrityCheckData->dup) / sizeof(UDATA));
        return FALSE;
    }
    ...
    for (j = userIntegrityCheckData->i + 1; j < userIntegrityCheckData->dataLength; j++) {
        if (*currentNode
            == userIntegrityCheckData
                   ->data[dataOffset(userIntegrityCheckData->removeOffset, userIntegrityCheckData->dataLength, j)]) {
            found = TRUE;
            break;
        }
    }
 ...
 */
static BOOLEAN runTests(J9PortLibrary* portLib, char* id, J9SRPHashTable** srptable, UDATA* data, U_32 dataLength,
    UDATA removeOffset, BOOLEAN recreateSRPHashTable)
{
    PORT_ACCESS_FROM_PORT(portLib);
    UDATA i, entry;
    char* operation = "";
    ...
    for (i = 0; i < dataLength; i++) {
        ...
        if (recreateSRPHashTable && ((executedFunctionCounter % iterationNumber) == 0)) {
            ...
            *srptable = srpHashTableRecreate(
                portLib, "testTable", (*srptable)->srpHashtableInternal, hashFn, hashEqualFn, NULL, NULL);
    ...
    if (tableSize < dataLength) {
        if (srpHashTableGetCount(*srptable) != tableSize) {
            j9tty_printf(PORTLIB,
                "SRP Hashtable %s count failure. dataLength: %d, srphashtable count: %d, expected count=tableSize: "
                "%d\n",
    ...
    for (i = 0; i < dataLength; i++) {
        ...
        if (offset < tableSize) {
            ...
            if (srpHashTableRemove(*srptable, &entry) == 0) {
                j9tty_printf(PORTLIB,
                    "SRP Hashtable %s remove failure. Removing: %d"
 ...
 *
 */
static void testSRPHashtableNewInRegion(
    J9PortLibrary* portLib, char* id, UDATA* data, U_32 dataLength, UDATA* passCount, UDATA* failCount)
{
    PORT_ACCESS_FROM_PORT(portLib);
    ...
    if ((srptable = srpHashTableNewInRegion(portLib, "testTableInRegion", allocatedMemory, memorySize, sizeof(UDATA), 0,
             hashFn, hashEqualFn, NULL, NULL))
        == NULL) {
    ...
    if (srpHashTable_tableSize(srptable) != tableSize) {
        j9tty_printf(PORTLIB, "SRP Hashtable %s generated with wrong tableSize : %d, expected tableSize : %d\n", id,
 ...
 *
 */
static void testSRPHashtableNew(
    J9PortLibrary* portLib, char* id, UDATA* data, U_32 dataLength, UDATA* passCount, UDATA* failCount)
{
    PORT_ACCESS_FROM_PORT(portLib);
    J9SRPHashTable* srptable;
    ...
    if ((srptable
            = srpHashTableNew(portLib, "testTable", dataLength, sizeof(UDATA), 0, hashFn, hashEqualFn, NULL, NULL))
        == NULL) {
        j9tty_printf(PORTLIB, "SRP Hashtable %s creation failure\n", id);
        goto fail;
    }
 ...
 *
 */
static void testSRPHashtableRecreate(
    J9PortLibrary* portLib, char* id, UDATA* data, U_32 dataLength, UDATA* passCount, UDATA* failCount)
{
    PORT_ACCESS_FROM_PORT(portLib);
    J9SRPHashTable* srptable;
 ...
 *
 */
static void testSRPHashtableVerify(
    J9PortLibrary* portLib, char* id, UDATA* data, U_32 dataLength, UDATA* passCount, UDATA* failCount)
{
    PORT_ACCESS_FROM_PORT(portLib);
    J9SRPHashTable* srptable;
    ...
    if (!srpHashTableVerify(srptable, srpHashTable_requiredMemorySize(tableSize, sizeof(UDATA), TRUE), sizeof(UDATA))) {
        j9tty_printf(
            PORTLIB, "SRP Hashtable %s can not be verified during the setup of testing srpHashTableVerify ", id);
        goto fail;
    }
