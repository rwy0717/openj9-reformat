...
jint JNICALL JVM_OnLoad(JavaVM* jvm, char* options, void* reserved)
{
    ...
    if ((*hooks)->J9HookRegisterWithCallSite(
            hooks, J9HOOK_VM_PROFILING_BYTECODE_BUFFER_FULL, profilingListener, OMR_GET_CALLSITE(), NULL)) {
    ...
    if ((*hooks)->J9HookRegisterWithCallSite(
            hooks, J9HOOK_VM_SHUTTING_DOWN, shutdownListener, OMR_GET_CALLSITE(), NULL)) {
    ...
    j9tty_printf(PORTLIB,
        "Profiling bytecodes are not supported on this VM.\n(J9VM_INTERP_PROFILING_BYTECODES is not defined).\n");
