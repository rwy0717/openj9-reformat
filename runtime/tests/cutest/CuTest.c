...
/* Modified from the original CuTest code to add verbose capability */
int verbose;
...
/* Modified from the original CuTest code to use size_t instead of int */
...
void CuStringAppend(CuString* str, const char* text)
{
    ...
    /* Modified from the original CuTest code to cast the strlen() result to int */
...
static void CuFailInternal(CuTest* tc, const char* file, int line, CuString* string)
{
    ...
    /* Modified from the original CuTest code to add cleanup capability */
    /* call the cleanup function before we jump to the next test */
    if (tc->cleanupFunc != NULL) {
        tc->cleanupFunc(tc->cleanupParam);
    }
...
{
    testCase->cleanupFunc = cleanupFunc;
    testCase->cleanupParam = cleanupParam;
}
