...
void JNICALL Java_VMBench_GPTests_GPTest_callInAndTriggerGPReadThenResumeAndReturnToJava(
    JNIEnv* env, jobject rec, jint count);
...
#define ROUNDED_FOOTER_OFFSET(number) \
    (((number) + (ROUNDING_GRANULARITY - 1) + sizeof(J9MemTag)) & ~(UDATA)(ROUNDING_GRANULARITY - 1))
...
void gptest_user_condition_handler(_FEEDBACK* fc, _INT4* token, _INT4* leResult, _FEEDBACK* newfc)
{
    ...
    _INT4 mrcrType = 0; /* Move the resume cursor to the call return point of the stack frame associated with the handle
                           cursor (which is the frame that registered this handler )*/
...
#pragma convlit(resume)
    ...
    fprintf(stderr,
        ...
        fc->tok_msgno, (NULL == asciiFacilityID) ? "NULL" : asciiFacilityID, fc->tok_sev);
    fflush(NULL);
...
void callInAndTriggerGPReadThenResume(JNIEnv* env, jobject rec)
{
    ...
    if (_FBCHECK(fc, CEE000) != 0) {
        ...
        if (256 == fc.tok_msgno) { /* 256 indicates handler was already registered */
            printf("\n\n ***Java_VMBench_GPTests_GPTest_callBackInAndResume: CEEHDLR came back with message %d "
                   "(handler was already registered, this is just a warning)\n\n",
                fc.tok_msgno);
            fflush(NULL);
        } else {
            printf("\n\n *** ERROR: Java_VMBench_GPTests_GPTest_callBackInAndResume: CEEHDLR failed with message "
                   "number %d *** \n\n",
                fc.tok_msgno);
            fflush(NULL);
            exit(31);
...
void JNICALL Java_VMBench_GPTests_GPTest_gpAbort(JNIEnv* env, jclass clazz, jobject arg1) { abort(); }
...
void JNICALL Java_VMBench_GPTests_GPTest_gpSoftwareRead(JNIEnv* env, jclass clazz, jobject arg1)
{
     ...
     *
     * We would need to differentite from pthread_kill(pthread_self()) in the expected results for this to work */
    kill(getpid(), SIGSEGV); /* this is not valid as the java testcase might simply complete execution and hence
                                terminate the process before the abort handler completes executing */
#endif
#endif
}
 ...
 * 2. Calls back into Java.
 */
void JNICALL Java_VMBench_GPTests_GPTest_callBackIn(JNIEnv* env, jobject rec, jobject arg1)
{
...
void JNICALL Java_VMBench_GPTests_GPTest_gpIll(JNIEnv* env, jclass clazz, jobject arg1)
{
    ...
    /* SIGILL test needs to use an an invalid opcode. 0x0b0f is the invalid opcode "UD2" reserved for testing on IA32
     * and AMD64. and is guaranteed not to become legal. On AMD64, however, we get an "ACCESS_VIOLATION" when we execute
     * it instead of an "ILLEGAL_INSTRUCTION", which is likely a bug in the AMD64 Windows OS.
     */
