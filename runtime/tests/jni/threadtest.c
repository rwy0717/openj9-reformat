...
static BOOLEAN enterThreadLock(J9VMThread* currentThread, j9object_t threadObj);
static void exitThreadLock(J9VMThread* currentThread, j9object_t threadObj);
...
jobjectArray JNICALL Java_j9vm_test_thread_NativeHelpers_findDeadlockedThreads(JNIEnv* env, jclass cls)
{
    ...
    deadlockCount = vmfns->findObjectDeadlockedThreads(
        currentThread, &deadlockedThreads, NULL, J9VMTHREAD_FINDDEADLOCKFLAG_ALREADYHAVEEXCLUSIVE);
...
static BOOLEAN enterThreadLock(J9VMThread* currentThread, j9object_t threadObj)
{
    ...
    if (threadObj) {
        ...
        if (threadLock) {
            threadLock
                = (J9Object*)currentThread->javaVM->internalVMFunctions->objectMonitorEnter(currentThread, threadLock);
