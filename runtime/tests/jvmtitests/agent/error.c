...
void throwError(JNIEnv* jni_env, char* error)
{
    jclass clazz;
...
void throwJVMTIError(agentEnv* env, JNIEnv* jni_env, jvmtiError err)
{
    char* error;
...
agentError* errorAtIndex(agentEnv* env, int index)
{
    int i = 0;
 ...
 */
static agentError* __error(
    agentEnv* env, jvmtiError err, agentErrorType errType, char* file, const char* fnname, int line, char* errorMsg)
{
    agentError* e;
...
agentError* _error(
    agentEnv* env, jvmtiError err, agentErrorType errType, char* file, const char* fnname, int line, char* format, ...)
{
    va_list args;
    agentError* agentErr;
...
JNIEXPORT jboolean JNICALL Java_com_ibm_jvmti_tests_util_ErrorControl_hasErrors() { return hasErrors(); }

jint getErrorCount(void) { return errorAgentEnv->errorCount; }

JNIEXPORT jint JNICALL Java_com_ibm_jvmti_tests_util_ErrorControl_getErrorCount() { return getErrorCount(); }
 ...
 */
JNIEXPORT jboolean JNICALL Java_com_ibm_jvmti_tests_util_Error_getErrorDetails(JNIEnv* env, jobject obj, int errorIndex)
{
...
int getErrorMessage(agentEnv* env, agentError* e, char* buf, int bufSize)
{
    ...
    p += sprintf(
        p, "\tError: %d %s  %s\n", e->error, e->type == JVMTI_TEST_ERR_TYPE_HARD ? "" : "[SOFT]", e->errorName);
    p += sprintf(p, "\t\t%s\n", e->message);
    p += sprintf(
        p, "\t\tLocation: %s -> [%s():%d]\n", e->file ? e->file : "null", e->function ? e->function : "null", e->line);
