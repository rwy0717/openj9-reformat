 ...
 */
char* className(agentEnv* env, jclass klass)
{
    jvmtiError err;
    char* name;
 ...
 *  <code>(*jvmti_env)->Deallocate</code>
 */
char* methodName(agentEnv* env, jmethodID method)
{
    JVMTI_ACCESS_FROM_AGENT(env);
    jvmtiError err;
 ...
 *  <code>(*jvmti_env)->Deallocate</code>
 */
char* fieldName(agentEnv* env, JNIEnv* jni_env, jclass klass, jfieldID field)
{
    JVMTI_ACCESS_FROM_AGENT(env);
    jvmtiError err;
    jclass declaringClass;
...
char* getTypeString(agentEnv* env, jthread currentThread, JNIEnv* jni_env, char signatureType, jvalue* jvaluePtr)
{
    JVMTI_ACCESS_FROM_AGENT(env);
...
void getParamKindName(agentEnv* env, jvmtiParamKind paramKind, char* name)
{
    switch (paramKind) {
    case JVMTI_KIND_IN:
...
void getPrimitiveTypeName(agentEnv* env, jvmtiParamTypes baseType, char* name)
{
    switch (baseType) {
...
void getObjectReferenceKindName(jvmtiObjectReferenceKind refKind, char* name)
{
    switch (refKind) {
...
void dumpJVMTIHeapRootKind(jvmtiHeapRootKind refKind, char* name)
{
    switch (refKind) {
