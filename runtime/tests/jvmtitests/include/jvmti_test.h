...
typedef struct agentEnv {
    ...
    int outputLevel;
    int errorCount; /** Total number of errors (not necessarily the same as number of errors on the
                       <code>agentEnv->errorsList</code> linked list */
...
#else
#define error(env, err, format, a...) \
    _error(env, err, JVMTI_TEST_ERR_TYPE_HARD, __FILE__, __FUNCTION__, __LINE__, format, ##a)
#define softError(env, err, format, a...) \
    _error(env, err, JVMTI_TEST_ERR_TYPE_SOFT, __FILE__, __FUNCTION__, __LINE__, format, ##a)
#endif
