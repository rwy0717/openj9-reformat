...
/* the standard agent test context which lives for the duration of the test - this is supposed to be held for error
 * logging */
static agentEnv* env;
...
static void JNICALL objectWasAllocated(
    jvmtiEnv* jvmti_env, JNIEnv* jni_env, jthread thread, jobject object, jclass object_klass, jlong size);
static jint JNICALL followReference(jvmtiHeapReferenceKind reference_kind, const jvmtiHeapReferenceInfo* reference_info,
    jlong class_tag, jlong referrer_class_tag, jlong size, jlong* tag_ptr, jlong* referrer_tag_ptr, jint length,
    void* user_data);

#define JAVA_LANG_CLASS "Ljava/lang/Class;"

jint JNICALL evmoa001(agentEnv* agent_env, char* args)
{
...
static jint JNICALL followReference(jvmtiHeapReferenceKind reference_kind, const jvmtiHeapReferenceInfo* reference_info,
    ...
    void* user_data)
{
    /* don't actually do anything.  If we don't crash by the time all these callbacks have been made, we passed the
     * test.  Just return the "continue" bit */
    return JVMTI_VISIT_OBJECTS;
}

jboolean JNICALL Java_com_ibm_jvmti_tests_eventVMObjectAllocation_evmoa001_didTestRun(JNIEnv* jni_env, jclass cls)
{
    return testDidRun;
}
