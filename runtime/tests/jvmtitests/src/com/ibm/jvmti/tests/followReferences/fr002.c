...
static const char* heapReferenceKindString(jvmtiHeapReferenceKind refKind)
{
...
jint JNICALL fr002(agentEnv* agent_env, char* args)
{
...
unsigned int getTag(jlong tag) { return (unsigned int)(tag); }

static jboolean objectExists(jlong tag)
{
...
static void addReference(reference* r) { free(r); }

static jint JNICALL fr002HeapReferenceCallback(jvmtiHeapReferenceKind reference_kind,
    ...
    jlong* referrer_tag_ptr, jint length, void* user_data)
{
    ...
    printf("\t\t[%08x:%08x]->[%08x:%08x] [%d %s] idx=%d\n", (unsigned int)getTag(referrer_class_tag),
        (referrer_tag_ptr) ? getTag(*referrer_tag_ptr) : 0, getTag(class_tag), getTag(*tag_ptr), reference_kind,
        heapReferenceKindString(reference_kind), (referrer_info) ? referrer_info->field.index : -1);
...
static jint JNICALL fr002PrimitiveFieldCallback(jvmtiHeapReferenceKind reference_kind,
    ...
    jvmtiPrimitiveType value_type, void* user_data)
{
    ...
    printf("\t\t\t[%08x:%08x] [%d %s] idx=%d  vt=%d\n", getTag(class_tag), getTag(*tag_ptr), reference_kind,
        heapReferenceKindString(reference_kind), referrer_info->field.index, value_type);

    return JVMTI_VISIT_OBJECTS;
}

static void printClass(classDescription* c)
{
    classField* f;
...
static void printClasses(classDescription* cListHead)
{
    classDescription* c;
...
static void getClassDetails(jvmtiEnv* jvmti_env, JNIEnv* jni_env, classDescription* c)
{
    int i;
    jvmtiError rc;
...
static void getTaggedObjects(jvmtiEnv* jvmti_env, JNIEnv* jni_env)
{
    jvmtiError rc;
    ...
    rc = (*jvmti_env)->GetObjectsWithTags(jvmti_env, tag_count, tags, &count_ptr, &object_result_ptr, &tag_result_ptr);
...
static void printObject(object* o) { printf("[%llx] [%s]\n", o->tag, o->class->name); }

static void resolveObject(object* o)
{
    jvmtiError rc;
    jlong tags[1];
    jint count_ptr = 0;
    ...
    rc = (*jvmti_env)->GetObjectsWithTags(jvmti_env, 1, tags, &count_ptr, &object_result_ptr, &tag_result_ptr);
    if (rc != JVMTI_ERROR_NONE) {
...
static void printWalkedObjects(void)
{
    object* o;
    int i;
...
jboolean JNICALL Java_com_ibm_jvmti_tests_followReferences_fr002_followRefs(
    JNIEnv* jni_env, jclass boo, jclass clazz, jobject object, jobject tail)
{
    jvmtiError rc;
