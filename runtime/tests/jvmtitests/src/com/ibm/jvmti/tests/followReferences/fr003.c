    ...
    do {                                                                                                            \
        ...
        for (i = 0; i < data->arraySize; i++) {                                                                     \
            if (((type*)elements)[i] != ((type*)data->arrayElements)[i]) {                                          \
                error(_agentEnv, JVMTI_ERROR_INVALID_OBJECT,                                                        \
                    "Array slot %d does not match. Should be %x instead of %x", i, ((type*)data->arrayElements)[i], \
                    ((type*)elements)[i]);                                                                          \
...
static jint JNICALL testFollowReferences_arrayPrimitiveCallback(jlong class_tag, jlong size, jlong* tag_ptr,
...
static jint JNICALL testFollowReferences_stringPrimitiveCallback(
...
static jint JNICALL testFollowReferences_heapRefCallback(jvmtiHeapReferenceKind refKind,
    const jvmtiHeapReferenceInfo* refInfo, jlong classTag, jlong referrerClassTag, jlong size, jlong* tagPtr,
...

{
    ...
    if (refKind == JVMTI_HEAP_REFERENCE_STACK_LOCAL) {
        printf("\tdepth=0x%x  method=0x%p  location=0x%llx slot=0x%x\n", refInfo->stack_local.depth,
            refInfo->stack_local.method, refInfo->stack_local.location, refInfo->stack_local.slot);
    }

    return JVMTI_VISIT_OBJECTS;
}

static void* getArrayElements(JNIEnv* jni_env, jvmtiPrimitiveType primitiveType, jint primitiveArraySize, jobject array)
{
...
jboolean JNICALL Java_com_ibm_jvmti_tests_followReferences_fr003_followFromObject(
...
jboolean JNICALL Java_com_ibm_jvmti_tests_followReferences_fr003_followFromArrayObject(JNIEnv* jni_env, jclass klass,
    jobject initialObject, jclass filterClass, jint primitiveType, jint primitiveArraySize)
{
    jvmtiError err;
    jvmtiHeapCallbacks callbacks;
    JVMTI_ACCESS_FROM_AGENT(_agentEnv);
...
jstring JNICALL Java_com_ibm_jvmti_tests_followReferences_fr003_getStringData(JNIEnv* jni_env, jclass klass)
{
...
jboolean JNICALL Java_com_ibm_jvmti_tests_followReferences_fr003_followFromStringObject(
    JNIEnv* jni_env, jclass klass, jobject initialObject, jclass filterClass)
{
    jvmtiError err;
    jvmtiHeapCallbacks callbacks;
