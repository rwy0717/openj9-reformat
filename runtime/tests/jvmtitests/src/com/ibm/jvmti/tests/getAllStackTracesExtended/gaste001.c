...
jint JNICALL gaste001(agentEnv* agent_env, char* args)
{
...
static jint testGetAllStackTracesExtended(JavaVM* vm, jvmtiEnv* jvmti_env, jint high)
{
    ...
    err = (getAllStackTracesExtended)(
        jvmti_env, COM_IBM_GET_STACK_TRACE_EXTRA_FRAME_INFO, JVMTI_TEST_GASTE001_MAX_FRAMES, &psiExt, &num_threads);
    if (err != JVMTI_ERROR_NONE) {
        ...
        if (high && (num_jitted == 0)) {
            rc = JNI_ERR;
            error(env, JVMTI_ERROR_INTERNAL, "getAllStackTracesExtended: Error when high iteration count");
        }
    }
    return (rc);
}

jboolean JNICALL Java_com_ibm_jvmti_tests_getAllStackTracesExtended_gaste001_anyJittedFrame(
