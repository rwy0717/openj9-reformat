...
jint JNICALL gcvn001(agentEnv* agent_env, char* args)
{
...
jboolean JNICALL Java_com_ibm_jvmti_tests_getClassVersionNumbers_gcvn001_checkVersionNumbers(
    JNIEnv* jni_env, jobject foo, jint minor, jint major)
{
    jvmtiError err;
    ...
    if (klass == NULL) {
        error(env, JVMTI_ERROR_INTERNAL,
            "Unable to load com/ibm/jvmti/tests/getClassVersionNumbers/VersionedClass/VersionedClass");
        return JNI_FALSE;
    }
