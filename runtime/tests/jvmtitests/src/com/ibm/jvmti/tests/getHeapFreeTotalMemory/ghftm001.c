...
jint JNICALL ghftm001(agentEnv* agent_env, char* args)
{
    ...
    err = (*jvmti_env)
              ->SetExtensionEventCallback(jvmti_env, extensionStartEventIndex, (jvmtiExtensionEvent)cbGCCycleStart);
    if (JVMTI_ERROR_NONE != err) {
    ...
    err = (*jvmti_env)
              ->SetExtensionEventCallback(jvmti_env, extensionFinishEventIndex, (jvmtiExtensionEvent)cbGCCycleFinish);
    if (JVMTI_ERROR_NONE != err) {
        error(env, err, "Failed SetExtensionEventCallback");
        goto bailOut;
    }
...
static void JNICALL cbGCCycleFinish(jvmtiEnv* jvmti_env, ...)
{
    ...
    tprintf(env, 0, "Finishing GC heapFree = %d MB, heapTotal = %d MB\n", (jint)heapFree / 1024 / 1024,
        (jint)heapTotal / 1024 / 1024);
    cycleEndCount += 1;
}

jlong JNICALL Java_com_ibm_jvmti_tests_getHeapFreeTotalMemory_ghftm001_getHeapFreeMemory(JNIEnv* jni_env, jclass klass)
{
    jlong heapFree;
...
jlong JNICALL Java_com_ibm_jvmti_tests_getHeapFreeTotalMemory_ghftm001_getHeapTotalMemory(JNIEnv* jni_env, jclass klass)
{
    jlong heapTotal;
...
jint JNICALL Java_com_ibm_jvmti_tests_getHeapFreeTotalMemory_ghftm001_getCycleStartCount(JNIEnv* jni_env, jclass klass)
{
    return cycleStartCount;
}

jint JNICALL Java_com_ibm_jvmti_tests_getHeapFreeTotalMemory_ghftm001_getCycleEndCount(JNIEnv* jni_env, jclass klass)
{
