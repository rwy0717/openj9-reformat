...
jint JNICALL gmc001(agentEnv* agent_env, char* args)
{
 ...
 */
static jboolean validateCategoryPointer(
    jvmtiMemoryCategory* value, jvmtiMemoryCategory* categories_buffer, jint written_count)
{
    if (value < categories_buffer || value > (categories_buffer + written_count)) {
        error(env, JVMTI_ERROR_NOT_FOUND,
            "gmc001:Category pointer out of range. Value = %p, categories_buffer = %p, top of buffer = %p", value,
            categories_buffer, categories_buffer + written_count);
        return JNI_FALSE;
    }

    if (((char*)value - (char*)categories_buffer) % sizeof(jvmtiMemoryCategory)) {
        error(env, JVMTI_ERROR_NOT_FOUND,
            "gmc001:Misaligned category pointer = %p. Categories_buffer=%p, sizeof(jvmtiMemoryCategory)=%u, "
            "remainder=%d",
            value, categories_buffer, (unsigned int)sizeof(jvmtiMemoryCategory),
            (int)((value - categories_buffer) % sizeof(jvmtiMemoryCategory)));
        return JNI_FALSE;
    }
 ...
 */
static jboolean validateCategories(jvmtiMemoryCategory* categories_buffer, jint written_count)
{
    jint i;
    ...
    for (i = 0; i < written_count; i++) {
        ...
        if (thisCategory->liveBytesShallow < 0) {
            error(env, JVMTI_ERROR_NOT_FOUND, "gmc001:liveBytesShallow negative for category %s. Index = %d",
                thisCategory->name, i);
            return JNI_FALSE;
        }

        if (thisCategory->liveAllocationsShallow < 0) {
            error(env, JVMTI_ERROR_NOT_FOUND, "gmc001:liveAllocationsShallow negative for category %s. Index = %d",
                thisCategory->name, i);
            return JNI_FALSE;
        }
        ...
        if (thisCategory->liveBytesShallow > thisCategory->liveBytesDeep) {
            error(env, JVMTI_ERROR_NOT_FOUND,
                "gmc001:liveBytesShallow is larger than liveBytesDeep for category %s. Index = %d", thisCategory->name,
                i);
            return JNI_FALSE;
        }
        ...
        if (thisCategory->liveAllocationsShallow > thisCategory->liveAllocationsDeep) {
            error(env, JVMTI_ERROR_NOT_FOUND,
                "gmc001:liveAllocationsShallow is larger than liveAllocationsDeep for category %s. Index = %d",
                thisCategory->name, i);
            return JNI_FALSE;
        }
        ...
        if (thisCategory->parent != NULL) {
            ...
            if (!validateCategoryPointer(thisCategory->parent, categories_buffer, written_count)) {
                error(env, JVMTI_ERROR_NOT_FOUND, "gmc001:Parent link for category %s  failed validation. Index = %d",
                    thisCategory->name, i);
                return JNI_FALSE;
            }
            ...
            if (thisCategory->parent == thisCategory) {
                error(env, JVMTI_ERROR_NOT_FOUND, "gmc001:Category %s is its own parent. Index = %d",
                    thisCategory->name, i);
                return JNI_FALSE;
            }
        }
        ...
        if (thisCategory->firstChild != NULL) {
            if (!validateCategoryPointer(thisCategory->firstChild, categories_buffer, written_count)) {
                error(env, JVMTI_ERROR_NOT_FOUND,
                    "gmc001:firstChild link for category %s  failed validation. Index = %d", thisCategory->name, i);
                return JNI_FALSE;
            }

            /* firstChild cannot be itself */
            if (thisCategory->firstChild == thisCategory) {
                error(env, JVMTI_ERROR_NOT_FOUND, "gmc001:Category %s is its own firstChild. Index = %d",
                    thisCategory->name, i);
                return JNI_FALSE;
            }
        }
        ...
        if (thisCategory->nextSibling != NULL) {
            ...
            if (!validateCategoryPointer(thisCategory->nextSibling, categories_buffer, written_count)) {
                error(env, JVMTI_ERROR_NOT_FOUND,
                    "gmc001:nextSibling link for category %s  failed validation. Index = %d", thisCategory->name, i);
                return JNI_FALSE;
            }

            if (thisCategory->nextSibling == thisCategory) {
                error(env, JVMTI_ERROR_NOT_FOUND, "gmc001:Category %s is its own nextSibling. Index = %d",
                    thisCategory->name, i);
                return JNI_FALSE;
            }
            ...
            if (ptr2 && ptr2->nextSibling == ptr1) {
                error(env, JVMTI_ERROR_NOT_FOUND,
                    "gmc001:1-2-1 loop found in nextSibling chain for category %s. Index = %d", thisCategory->name, i);
                return JNI_FALSE;
            } else {
                while (ptr1 && ptr2) {
                    ...
                    if (ptr1 == ptr2) {
                        error(env, JVMTI_ERROR_NOT_FOUND,
                            "gmc001:loop found in nextSibling chain for category %s. Index = %d", thisCategory->name,
                            i);
                        return JNI_FALSE;
                    }
                }
 ...
 * Signature: ()Z
 */
JNIEXPORT jboolean JNICALL Java_com_ibm_jvmti_tests_getMemoryCategories_gmc001_check(JNIEnv* jni_env, jobject obj)
{
    ...
    if (JVMTI_ERROR_UNSUPPORTED_VERSION != err) {
        error(env, err,
            "gmc001:getMemoryCategories failed wrong version test. Expected JVMTI_ERROR_UNSUPPORTED_VERSION.");
        return JNI_FALSE;
    }
    ...
    if (JVMTI_ERROR_ILLEGAL_ARGUMENT != err) {
        error(env, err,
            "gmc001:getMemoryCategories failed max_categories > 0, categories_buffer = NULL test. Expected "
    ...
    if (JVMTI_ERROR_ILLEGAL_ARGUMENT != err) {
        error(env, err,
            "gmc001:getMemoryCategories failed all output pointers NULL test. Expected JVMTI_ERROR_ILLEGAL_ARGUMENT.");
        return JNI_FALSE;
    }
    ...
    if (JVMTI_ERROR_NONE != err) {
        error(env, err,
            "gmc001:getMemoryCategories failed poll total number of categories test. Expected JVMTI_ERROR_NONE");
        return JNI_FALSE;
    }
    ...
    err = (*jvmti_env)
              ->Allocate(
                  jvmti_env, sizeof(jvmtiMemoryCategory) * total_categories, (unsigned char**)&categories_buffer);
    ...
    written_count = -1;
    err = getMemoryCategories(jvmti_env, COM_IBM_GET_MEMORY_CATEGORIES_VERSION_1, total_categories - 1,
        categories_buffer, &written_count, NULL);
    ...
    err = getMemoryCategories(
        jvmti_env, COM_IBM_GET_MEMORY_CATEGORIES_VERSION_1, total_categories, categories_buffer, NULL, NULL);

    if (JVMTI_ERROR_ILLEGAL_ARGUMENT != err) {
        error(env, err,
            "gmc001:getMemoryCategories failed NULL written_count_ptr test. Expected JVMTI_ERROR_ILLEGAL_ARGUMENT.");
        return JNI_FALSE;
    }

    /* Positive path: get and validate categories */
    err = getMemoryCategories(
        jvmti_env, COM_IBM_GET_MEMORY_CATEGORIES_VERSION_1, total_categories, categories_buffer, &written_count, NULL);
    ...
    err = (*jvmti_env)
              ->Allocate(
                  jvmti_env, sizeof(jvmtiMemoryCategory) * (total_categories + 1), (unsigned char**)&categories_buffer);
    ...
    if (written_count > total_categories) {
        error(env, JVMTI_ERROR_NOT_FOUND,
            "gmc100: written_count was higher that total categories with oversized buffer. Should be impossible. "
            "written_count=%d, total_categories=%d\n",
