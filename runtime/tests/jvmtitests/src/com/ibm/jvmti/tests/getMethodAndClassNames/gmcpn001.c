...
jint JNICALL gmcpn001(agentEnv* agent_env, char* args)
{
...
jboolean check_single(JNIEnv* jni_env, jclass cls)
{
    ...
    err = (*jvmti_env)
              ->Allocate(
                  jvmti_env, sizeof(jvmtiExtensionRamMethodData) * ramMethodsCount, (unsigned char**)&descriptorBuffer);
    if (err != JVMTI_ERROR_NONE) {
    ...
    *((UDATA*)ramMethodsPtr++) = *((UDATA*)mid1);
    ...
    err = getMethodAndClassNames(
        jvmti_env, (void*)ramMethods, ramMethodsCount, descriptorBuffer, stringBuffer, &stringBufferSize);
...
jboolean check_multiple(JNIEnv* jni_env, jclass cls)
{
    JVMTI_ACCESS_FROM_AGENT(env);
    jvmtiError err = JVMTI_ERROR_NONE;
    jvmtiError deallocErr = JVMTI_ERROR_NONE;
    ...
    *((UDATA*)ramMethodsPtr++) = *((UDATA*)mid3);
    ...
    for (i = 1; i <= 3; i++) {
        ...
        if (strcmp("com/ibm/jvmti/tests/getMethodAndClassNames/gmcpn001", (char*)descriptors->className)) {
            error(env, JVMTI_ERROR_ILLEGAL_ARGUMENT, "Invalid className [%s] expected: [%s]", descriptors->className,
                buf);
            goto done;
        }
        ...
        if (strcmp(buf, (char*)descriptors->methodName)) {
            error(env, JVMTI_ERROR_ILLEGAL_ARGUMENT, "Invalid methodName [%s] expected: [%s]", descriptors->methodName,
                buf);
            goto done;
        }
...
jboolean check_invalid_single(JNIEnv* jni_env, jclass cls) { return JNI_FALSE; }

typedef enum { type_single = 1, type_multiple = 2, type_invalid_single = 3 } check_type_t;

jboolean JNICALL Java_com_ibm_jvmti_tests_getMethodAndClassNames_gmcpn001_check(
    JNIEnv* jni_env, jclass cls, jint check_type)
{
    switch (check_type) {
    case type_single:
