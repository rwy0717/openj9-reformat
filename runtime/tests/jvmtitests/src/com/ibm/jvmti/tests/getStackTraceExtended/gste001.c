...
static void JNICALL compiledMethodLoad(jvmtiEnv* jvmti_env, jmethodID method, jint code_size, const void* code_addr,
    jint map_length, const jvmtiAddrLocationMap* map, const void* compile_info);

static jboolean testMethodJitted = JNI_FALSE;

jint JNICALL gste001(agentEnv* agent_env, char* args)
{
...
static jint testGetStackTraceExtended(JavaVM* vm, jvmtiEnv* jvmti_env, jint high)
{
    int err, i;
    int num_frames = 0;
    int num_jitted = 0;
    ...
    err = (getStackTraceExtended)(jvmti_env, COM_IBM_GET_STACK_TRACE_EXTRA_FRAME_INFO, currentThread, 0,
        JVMTI_TEST_GSTE001_MAX_FRAMES, (void*)frame_buffer, &num_frames);
    ...
    if (err != JVMTI_ERROR_NONE) {
        ...
        if (high && (num_jitted == 0)) {
            rc = JNI_ERR;
            error(env, JVMTI_ERROR_INTERNAL, "getStackTraceExtended: Error when high iteration count");
        }
    }
...
jint JNICALL Java_com_ibm_jvmti_tests_getStackTraceExtended_gste001_anyJittedFrame(
    JNIEnv* jni_env, jclass clazz, jint high)
{
