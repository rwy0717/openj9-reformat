...
jint JNICALL gtlste001(agentEnv* agent_env, char* args)
{
...
static jint testGetThreadListStackTracesExtended(JavaVM* vm, jvmtiEnv* jvmti_env, jint high)
{
    ...
    err = (*jvmti_env)->GetAllThreads(jvmti_env, &num_threads, &thrds);
    ...
    err = (getThreadListStackTracesExtended)(jvmti_env, COM_IBM_GET_STACK_TRACE_EXTRA_FRAME_INFO, num_threads, thrds,
        JVMTI_TEST_GTLSTE001_MAX_FRAMES, &psiExt);
    ...
    if (err != JVMTI_ERROR_NONE) {
        ...
        if (high && (num_jitted == 0)) {
            rc = JNI_ERR;
            error(env, JVMTI_ERROR_INTERNAL, "getThreadListTracesExtended: Error when high iteration count");
        }
    }
...
jint JNICALL Java_com_ibm_jvmti_tests_getThreadListStackTracesExtended_gtlste001_anyJittedFrame(
