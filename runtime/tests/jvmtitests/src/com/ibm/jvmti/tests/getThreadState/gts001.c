...
jint JNICALL gts001(agentEnv* agent_env, char* args)
{
...
jboolean JNICALL Java_com_ibm_jvmti_tests_getThreadState_gts001_getThreadStates(
    JNIEnv* jni_env, jclass klass, jint threadStateEnumCount)
{
    jvmtiError err;
    jthread* threads;
    jint threadCount;
...
jboolean JNICALL Java_com_ibm_jvmti_tests_getThreadState_gts001_getThreadState(
    JNIEnv* jni_env, jclass klass, jthread currThread)
{
    jvmtiError err;
    jint stateCount = 0;
    jint state;
...
void checkThreadState(JNIEnv* jni_env, jthread currThread, jint state, jint* stateCount)
{
    ...
    if (!strcmp(thrName, "RUNNING")) {
        ...
        testThreadState(state,
            (JVMTI_THREAD_STATE_IN_OBJECT_WAIT | JVMTI_THREAD_STATE_WAITING_INDEFINITELY | JVMTI_THREAD_STATE_WAITING
                | JVMTI_THREAD_STATE_ALIVE),
            stateCount);
    } else if (!strcmp(thrName, "WAITING_TIMED")) {
        testThreadState(state,
            (JVMTI_THREAD_STATE_IN_OBJECT_WAIT | JVMTI_THREAD_STATE_WAITING_WITH_TIMEOUT | JVMTI_THREAD_STATE_WAITING
                | JVMTI_THREAD_STATE_ALIVE),
            stateCount);
    } else if (!strcmp(thrName, "PARKED")) {
        testThreadState(state,
            (JVMTI_THREAD_STATE_PARKED | JVMTI_THREAD_STATE_WAITING_INDEFINITELY | JVMTI_THREAD_STATE_WAITING
                | JVMTI_THREAD_STATE_ALIVE),
            stateCount);
    } else if (!strcmp(thrName, "PARKED_TIMED")) {
        testThreadState(state,
            (JVMTI_THREAD_STATE_PARKED | JVMTI_THREAD_STATE_WAITING_WITH_TIMEOUT | JVMTI_THREAD_STATE_WAITING
                | JVMTI_THREAD_STATE_ALIVE),
            stateCount);
    } else if (!strcmp(thrName, "SLEEPING")) {
        testThreadState(state,
            (JVMTI_THREAD_STATE_SLEEPING | JVMTI_THREAD_STATE_WAITING_WITH_TIMEOUT | JVMTI_THREAD_STATE_WAITING
                | JVMTI_THREAD_STATE_ALIVE),
            stateCount);
    } else if (!strcmp(thrName, "SUSPENDED")) {
        testThreadState(
            state, (JVMTI_THREAD_STATE_SUSPENDED | JVMTI_THREAD_STATE_RUNNABLE | JVMTI_THREAD_STATE_ALIVE), stateCount);
    } else if (!strcmp(thrName, "INTERRUPTED_BLOCKED")) {
        testThreadState(state,
            (JVMTI_THREAD_STATE_INTERRUPTED | JVMTI_THREAD_STATE_BLOCKED_ON_MONITOR_ENTER | JVMTI_THREAD_STATE_ALIVE),
            stateCount);
    } else if (!strcmp(thrName, "INTERRUPTED_SUSPENDED")) {
        testThreadState(state,
            (JVMTI_THREAD_STATE_INTERRUPTED | JVMTI_THREAD_STATE_SUSPENDED | JVMTI_THREAD_STATE_RUNNABLE
                | JVMTI_THREAD_STATE_ALIVE),
            stateCount);
