...
static jint JNICALL testIterateThroughHeap_callback(
    jlong class_tag, jlong size, jlong* tag_ptr, jint length, void* user_data);

jint JNICALL ith001(agentEnv* agent_env, char* args)
{
    jvmtiError err;
    ...
    */
    ...
    if (class_tag != 0xc0decafe) {
        error(env, JVMTI_ERROR_INTERNAL,
            "IterateThroughHeap callback called for incorrect"
 ...
 */
static jint JNICALL testIterateThroughHeap_arrayPrimitiveCallback(jlong class_tag, jlong size, jlong* tag_ptr,
    jint element_count, jvmtiPrimitiveType element_type, const void* elements, void* user_data)
{
    int i;
    testIterateThroughHeapData* userData = (testIterateThroughHeapData*)user_data;
...
static jint JNICALL testIterateThroughHeap_primitiveFieldCallback(jvmtiHeapReferenceKind reference_kind,
...
static jint JNICALL testIterateThroughHeap_stringPrimitiveCallback(
...
jboolean JNICALL Java_com_ibm_jvmti_tests_iterateThroughHeap_ith001Sub_testHeapIteration(JNIEnv* jni_env, jclass clazz)
{
    jvmtiError err;
...
jboolean JNICALL Java_com_ibm_jvmti_tests_iterateThroughHeap_ith001Sub_testArrayPrimitive(JNIEnv* jni_env, jobject obj)
{
    jvmtiError err;
    jvmtiHeapCallbacks callbacks;
    testIterateThroughHeapData userData;
    ...
    if (userData.arrayPrimitiveMatches != 8) {
        error(
            env, err, "Retrieved an incorrect number [%d] of array primitive elements", userData.arrayPrimitiveMatches);
        return JNI_FALSE;
    }
...
jboolean JNICALL Java_com_ibm_jvmti_tests_iterateThroughHeap_ith001Sub_testFieldPrimitive(JNIEnv* jni_env, jclass clazz)
{
    jvmtiError err;
    jvmtiHeapCallbacks callbacks;
    testIterateThroughHeapData userData;
    ...
    if (userData.fieldPrimitiveMatches == -1) {
        error(
            env, err, "Retrieved an incorrect number [%d] of field primitive elements", userData.fieldPrimitiveMatches);
        return JNI_FALSE;
    }
...
jboolean JNICALL Java_com_ibm_jvmti_tests_iterateThroughHeap_ith001Sub_testStringPrimitive(
    JNIEnv* jni_env, jclass clazz)
{
    ...
    if (userData.stringCallbackMatches == 0) {
        error(env, JVMTI_ERROR_NOT_FOUND,
            "Failed to find com.ibm.jvmti.tests.iterateThroughHeap.uth001Sub->testString containing "
            "'testStringContent'");
        return JNI_FALSE;
    }
...
jboolean JNICALL Java_com_ibm_jvmti_tests_iterateThroughHeap_ith001Sub_tagObject(
    JNIEnv* jni_env, jobject o, jlong tag, jobject taggedObject)
{
    jvmtiError err;
    JVMTI_ACCESS_FROM_AGENT(env);
