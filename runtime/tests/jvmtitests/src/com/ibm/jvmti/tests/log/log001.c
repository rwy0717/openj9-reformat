...
jint JNICALL log001(agentEnv* agent_env, char* args)
{
...
jboolean JNICALL Java_com_ibm_jvmti_tests_log_log001_tryQueryLogOptions(JNIEnv* jni_env, jclass cls)
{
    int err;
    ...
    if (err != JVMTI_ERROR_NULL_POINTER) {
        error(env, err,
            "tryQueryLogOptions:queryLogOptions(jvmti_env, 0, NULL, NULL) failed to return JVMTI_ERROR_NULL_POINTER");
        return FALSE;
    }
    ...
    if (err != JVMTI_ERROR_NULL_POINTER) {
        error(env, err,
            "tryQueryLogOptions:queryLogOptions(jvmti_env, 20, buffer, NULL) failed to return "
    ...
    if (err != JVMTI_ERROR_NONE) {
        error(env, err,
            "tryQueryLogOptions:queryLogOptions(jvmti_env, 20, buffer, &data_size) failed to return JVMTI_ERROR_NONE");
        return FALSE;
    }
    if (strcmp(buffer, "error,warn") != 0) {
        error(env, err,
            "tryQueryLogOptions:queryLogOptions(jvmti_env, sizeof(buffer), buffer, &data_size) returned invalid result "
            "string. Expecting '%s' got '%s'",
            "error,warn", buffer);
        return FALSE;
    }
    ...
    if (err == JVMTI_ERROR_NONE) {
        ...
        if (err != JVMTI_ERROR_NONE) {
            error(env, err,
                "tryQueryLogOptions:queryLogOptions(jvmti_env, sizeof(buffer), buffer, &data_size) failed to return "
                "JVMTI_ERROR_NONE");
            return FALSE;
        }
        if (strcmp(buffer, "error,warn,info,config,vital") != 0) {
            error(env, err,
                "tryQueryLogOptions:queryLogOptions(jvmti_env, sizeof(buffer), buffer, &data_size) returned invalid "
                "result string. Expecting '%s' got '%s'",
                "error,warn,info,config,vital", buffer);
            return FALSE;
        }
    } else {
        error(env, err,
    ...
    if (err == JVMTI_ERROR_NONE) {
        ...
        if (err == JVMTI_ERROR_NONE) {
            ...
            if (err != JVMTI_ERROR_NONE) {
                error(env, err,
                    "tryQueryLogOptions:queryLogOptions(jvmti_env, sizeof(buffer), buffer, &data_size) failed to "
                    "return JVMTI_ERROR_NONE");
                return FALSE;
            }
            if (strcmp(buffer, "error") != 0) {
                error(env, err,
                    "tryQueryLogOptions:queryLogOptions(jvmti_env, sizeof(buffer), buffer, &data_size) returned "
    ...
    if (err != JVMTI_ERROR_NULL_POINTER) {
        error(env, err,
            "tryQueryLogOptions:queryLogOptions(jvmti_env, sizeof(buffer)+ 2, NULL, &data_size) failed to return "
            "JVMTI_ERROR_NULL_POINTER");
        return FALSE;
    }
    ...
    if (err != JVMTI_ERROR_ILLEGAL_ARGUMENT) {
        error(env, err,
            "tryQueryLogOptions:queryLogOptions(jvmti_env, 2, buffer, &data_size) failed to return "
...
jboolean JNICALL Java_com_ibm_jvmti_tests_log_log001_trySetLogOptions(JNIEnv* jni_env, jclass cls)
{
    int err = 0;
    jvmtiEnv* jvmti_env = env->jvmtiEnv;
    ...
    if (err != JVMTI_ERROR_ILLEGAL_ARGUMENT) {
        error(env, err,
            "trySetLogOptions:setLogOptions(\"ferror     ,config\") failed to return JVMTI_ERROR_ILLEGAL_ARGUMENT");
        return FALSE;
    }
    ...
    if (err != JVMTI_ERROR_NONE) {
        error(
            env, err, "trySetLogOptions:setLogOptions(\"error     ,   config   \") failed to return JVMTI_ERROR_NONE");
        return FALSE;
    }
