...
static void JNICALL re001_callback(
    jvmtiEnv* jvmti_env, JNIEnv* jni_env, jint flags, const void* reserved, const char* description);

static jint JNICALL resourceExhaustedSetup(agentEnv* agent_env, char* args)
{
    jvmtiError err;
...
jint JNICALL re001(agentEnv* agent_env, char* args)
{
    jint ret;
...
jint JNICALL re002(agentEnv* agent_env, char* args)
{
    jint ret;
...
static jboolean hasBeenCalledBack(void)
{
    jboolean ret;
...
jboolean JNICALL Java_com_ibm_jvmti_tests_resourceExhausted_re001_hasBeenCalledBack(JNIEnv* jni_env, jclass c)
{
    return hasBeenCalledBack();
}

jboolean JNICALL Java_com_ibm_jvmti_tests_resourceExhausted_re002_hasBeenCalledBack(JNIEnv* jni_env, jclass c)
{
    return hasBeenCalledBack();
}
