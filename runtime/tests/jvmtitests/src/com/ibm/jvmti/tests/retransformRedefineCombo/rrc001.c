...
enum { MODIFY = 0, DONT_MODIFY, REDEFINE_MODIFY, REDEFINE_DONT_MODIFY };
    ...
    jint* new_class_data_len, unsigned char** new_class_data)
{
    if ((DONT_MODIFY == transformerData.action) || (REDEFINE_DONT_MODIFY == transformerData.action)) {
        ...
        if ((NULL != transformerData.retransformedClassName)
            && (0 == strcmp(name, transformerData.retransformedClassName))) {
...
jboolean JNICALL Java_com_ibm_jvmti_tests_retransformRedefineCombo_rrc001_setTransformerData(
    JNIEnv* jni_env, jobject obj, jstring className, jbyteArray transformedClassBytes, jint action)
{
    ...
    if (NULL != transformedClassBytes) {
        ...
        (*jni_env)->GetByteArrayRegion(
            jni_env, transformedClassBytes, 0, transformerData.classDataSize, (jbyte*)newClassData);
    } else {
...
jboolean JNICALL Java_com_ibm_jvmti_tests_retransformRedefineCombo_rrc001_retransformClass(JNIEnv* jni_env, jobject obj,
    jclass originalClass, jstring className, jbyteArray transformedClassBytes, jint action)
{
    JVMTI_ACCESS_FROM_AGENT(env);
    jvmtiError err;
    unsigned char* newClassData = NULL;
...
jboolean JNICALL Java_com_ibm_jvmti_tests_retransformRedefineCombo_rrc001_redefineClass(JNIEnv* jni_env, jobject obj,
    jclass originalClass, jstring className, jbyteArray redefinedClassBytes, jbyteArray transformedClassBytes,
    jint action)
{
    JVMTI_ACCESS_FROM_AGENT(env);
    jvmtiError err;
