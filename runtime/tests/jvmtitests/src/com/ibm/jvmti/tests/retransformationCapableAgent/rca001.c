 ...
 * 		- if the stage is CLASS_LOADING or CLASS_REDEFINING sets its expected version to
 * cma001.currentClassVersion.
 ...
 */
static void JNICALL classFileLoadHook(jvmtiEnv* jvmti_env, JNIEnv* jni_env, jclass class_being_redefined,
    ...
    jint* new_class_data_len, unsigned char** new_class_data)
{
    ...
    if ((0 == strcmp(name, CLASS_PACKAGE "/" TEST_CLASS_NAME)) && (NULL != agentJavaClass)) {
        ...
        if (CLASS_RETRANSFORMING == stage) {
            ...
            if (JNI_TRUE == riHookCalled) {
                fprintf(stderr,
                    "%s: ClassFileLoadHook event for Re-transformation Incapable agent is incorrectly called\n",
                    agentName);
                setResultValue(jni_env, JNI_FALSE);
                return;
            }
        ...
        classBytesArray
            = (*jni_env)->CallStaticObjectMethod(jni_env, agentJavaClass, getClassBytesMethodId, (jint)expectedVersion);
        if (NULL != classBytesArray) {
            ...
            fprintf(
                stderr, "%s: Failed to get class bytes for expected class version %d\n", agentName, expectedVersion);
            setResultValue(jni_env, JNI_FALSE);
            return;
        }

        classBytesArray
            = (*jni_env)->CallStaticObjectMethod(jni_env, agentJavaClass, getClassBytesMethodId, (jint)newClassVersion);
        if (NULL != classBytesArray) {
...
static jint getJNIIDs(JNIEnv* jni_env)
{
    if (NULL != agentJavaClass) {
        ...
        riHookCalledFieldId
            = (*jni_env)->GetStaticFieldID(jni_env, agentJavaClass, "retransformationIncapableHookCalled", "Z");
        if (NULL == riHookCalledFieldId) {
            return JNI_ERR;
        }
        getClassBytesMethodId
            = (*jni_env)->GetStaticMethodID(jni_env, agentJavaClass, "getClassBytesForVersion", "(I)[B");
 ...
 * 	newClassVersion: set to the version passed to it as argument. ClassFileLoadHook event callback will transform
 * the class to this version. expectedVersion: set in its ClassFileLoadHook event callback when the stage is
 * CLASS_LOADING or CLASS_REDEFINING. Used by ClassFileLoadHook event callback to verify incoming class bytes are as
 * expected.
 *
