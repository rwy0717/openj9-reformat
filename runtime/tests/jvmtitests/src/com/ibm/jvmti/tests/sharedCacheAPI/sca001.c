...
static jint JNICALL validateSharedCacheInfoVersion2(
    jvmtiEnv* jvmti, jvmtiSharedCacheInfo* cache_info, jobject user_data);
static jint JNICALL validateSharedCacheInfoVersion3(
    jvmtiEnv* jvmti, jvmtiSharedCacheInfo* cache_info, jobject user_data);
static jint JNICALL validateSharedCacheInfoVersion4(
    jvmtiEnv* jvmti, jvmtiSharedCacheInfo* cache_info, jobject user_data);
...
jint JNICALL sca001(agentEnv* agent_env, char* args)
{
    env = agent_env;
    return JNI_OK;
}
...
static jint JNICALL validateSharedCacheInfo(jvmtiEnv* jvmti, jvmtiSharedCacheInfo* cache_info, jobject user_data)
{
    if ((NULL != cache_info) && ((jint)0 != cache_info->cacheType)) {
        error(env, JVMTI_ERROR_INVALID_TYPESTATE,
            "Field cacheType of jvmtiSharedCacheInfo has been modified when passing "
            "COM_IBM_ITERATE_SHARED_CACHES_VERSION_1 to iterateSharedCacheFunction \n");
        return JNI_EINVAL;
    }
    if (0 != checkAddrMode((int)cache_info->addrMode, COM_IBM_ITERATE_SHARED_CACHES_VERSION_1)) {
        error(env, JVMTI_ERROR_INVALID_TYPESTATE,
            "Field addrMode of jvmtiSharedCacheInfo is wrong when passing COM_IBM_ITERATE_SHARED_CACHES_VERSION_1 to "
            "iterateSharedCacheFunction \n");
        return JNI_EINVAL;
    }
    ...
    jvmtiEnv* jvmti, jvmtiSharedCacheInfo* cache_info, jobject user_data)
{
    ...
    if (0 != checkAddrMode((int)cache_info->addrMode, COM_IBM_ITERATE_SHARED_CACHES_VERSION_2)) {
        error(env, JVMTI_ERROR_INVALID_TYPESTATE,
            "Field addrMode of jvmtiSharedCacheInfo is wrong when passing COM_IBM_ITERATE_SHARED_CACHES_VERSION_2 to "
            "iterateSharedCacheFunction \n");
        return JNI_EINVAL;
    }
    ...
    jvmtiEnv* jvmti, jvmtiSharedCacheInfo* cache_info, jobject user_data)
{
    if (0 != checkAddrMode((int)cache_info->addrMode, COM_IBM_ITERATE_SHARED_CACHES_VERSION_3)) {
        error(env, JVMTI_ERROR_INVALID_TYPESTATE,
            "Field addrMode of jvmtiSharedCacheInfo is wrong when passing COM_IBM_ITERATE_SHARED_CACHES_VERSION_3 to "
            "iterateSharedCacheFunction \n");
        return JNI_EINVAL;
    }
    ...
    jvmtiEnv* jvmti, jvmtiSharedCacheInfo* cache_info, jobject user_data)
{
    if (0 != checkAddrMode((int)cache_info->addrMode, COM_IBM_ITERATE_SHARED_CACHES_VERSION_4)) {
        error(env, JVMTI_ERROR_INVALID_TYPESTATE,
            "Field addrMode of jvmtiSharedCacheInfo is wrong when passing COM_IBM_ITERATE_SHARED_CACHES_VERSION_4 to "
            "iterateSharedCacheFunction \n");
        return JNI_EINVAL;
    }
...
jint JNICALL Java_tests_sharedclasses_options_TestSharedCacheJvmtiAPI_iterateSharedCache(
    JNIEnv* jni_env, jclass clazz, jstring cacheDir, jint flags, jboolean useCommandLineValues)
{
    jint extensionCount;
    ...
    err = (iterateSharedCacheFunction)(jvmti_env, INVALID_COM_IBM_ITERATE_SHARED_CACHES_VERSION, dir, flags,
        useCommandLineValues, validateSharedCacheInfo, NULL);
    ...
    err = (iterateSharedCacheFunction)(jvmti_env, COM_IBM_ITERATE_SHARED_CACHES_VERSION_1, dir,
        INVALID_COM_IBM_ITERATE_SHARED_CACHES_NO_FLAGS, useCommandLineValues, validateSharedCacheInfo, NULL);
    if (JVMTI_ERROR_ILLEGAL_ARGUMENT != err) {
        if (cacheDir != NULL) {
            (*jni_env)->ReleaseStringUTFChars(jni_env, cacheDir, (const char*)dir);
        }
        error(env, err,
            "iterateSharedCacheFunction: invalid flags field test failed when passing in "
            "COM_IBM_ITERATE_SHARED_CACHES_VERSION_1 \n");
        return (jint)-1;
    }

    cacheCount = 0;
    err = (iterateSharedCacheFunction)(jvmti_env, COM_IBM_ITERATE_SHARED_CACHES_VERSION_1, dir, flags,
        useCommandLineValues, validateSharedCacheInfo, NULL);
    ...
    if (err != JVMTI_ERROR_NONE) {
        ...
        error(env, err,
            "iterateSharedCacheFunction: iteration of shared caches failed when passing in "
            "COM_IBM_ITERATE_SHARED_CACHES_VERSION_1\n");
        return (jint)-1;
    }

    /* check for invalid flags field when passing in COM_IBM_ITERATE_SHARED_CACHES_VERSION_2 */
    err = (iterateSharedCacheFunction)(jvmti_env, COM_IBM_ITERATE_SHARED_CACHES_VERSION_2, dir,
        INVALID_COM_IBM_ITERATE_SHARED_CACHES_NO_FLAGS, useCommandLineValues, validateSharedCacheInfoVersion2, NULL);
    if (JVMTI_ERROR_ILLEGAL_ARGUMENT != err) {
        if (cacheDir != NULL) {
            (*jni_env)->ReleaseStringUTFChars(jni_env, cacheDir, (const char*)dir);
        }
    ...
    err = (iterateSharedCacheFunction)(jvmti_env, COM_IBM_ITERATE_SHARED_CACHES_VERSION_2, dir, flags,
        useCommandLineValues, validateSharedCacheInfoVersion2, NULL);
    ...
    if (cacheCountVersion2 != cacheCount) {
        ...
        error(env, JVMTI_ERROR_INTERNAL,
            "iterateSharedCacheFunction: Number of caches found using COM_IBM_ITERATE_SHARED_CACHES_VERSION_1 and "
            "COM_IBM_ITERATE_SHARED_CACHES_VERSION_2 are different \n");
        return (jint)-1;
    }

    /* check for invalid flags field when passing in COM_IBM_ITERATE_SHARED_CACHES_VERSION_3 */
    err = (iterateSharedCacheFunction)(jvmti_env, COM_IBM_ITERATE_SHARED_CACHES_VERSION_3, dir,
        INVALID_COM_IBM_ITERATE_SHARED_CACHES_NO_FLAGS, useCommandLineValues, validateSharedCacheInfoVersion3, NULL);
    if (JVMTI_ERROR_ILLEGAL_ARGUMENT != err) {
        if (cacheDir != NULL) {
            (*jni_env)->ReleaseStringUTFChars(jni_env, cacheDir, (const char*)dir);
        }
    ...
    err = (iterateSharedCacheFunction)(jvmti_env, COM_IBM_ITERATE_SHARED_CACHES_VERSION_3, dir, flags,
        useCommandLineValues, validateSharedCacheInfoVersion3, NULL);
    ...
    if (cacheCountVersion3 != cacheCount) {
        ...
        error(env, JVMTI_ERROR_INTERNAL,
            "iterateSharedCacheFunction: Number of caches found using COM_IBM_ITERATE_SHARED_CACHES_VERSION_1 and "
            "COM_IBM_ITERATE_SHARED_CACHES_VERSION_3 are different \n");
        return (jint)-1;
    }

    /* check for invalid flags field when passing in COM_IBM_ITERATE_SHARED_CACHES_VERSION_4 */
    err = (iterateSharedCacheFunction)(jvmti_env, COM_IBM_ITERATE_SHARED_CACHES_VERSION_4, dir,
        INVALID_COM_IBM_ITERATE_SHARED_CACHES_NO_FLAGS, useCommandLineValues, validateSharedCacheInfoVersion4, NULL);
    if (JVMTI_ERROR_ILLEGAL_ARGUMENT != err) {
        if (cacheDir != NULL) {
            (*jni_env)->ReleaseStringUTFChars(jni_env, cacheDir, (const char*)dir);
        }
    ...
    err = (iterateSharedCacheFunction)(jvmti_env, COM_IBM_ITERATE_SHARED_CACHES_VERSION_4, dir, flags,
        useCommandLineValues, validateSharedCacheInfoVersion4, NULL);
    ...
    if (cacheCountVersion4 != cacheCount) {
        ...
        error(env, JVMTI_ERROR_INTERNAL,
            "iterateSharedCacheFunction: Number of caches found using COM_IBM_ITERATE_SHARED_CACHES_VERSION_1 and "
            "COM_IBM_ITERATE_SHARED_CACHES_VERSION_4 are different \n");
        return (jint)-1;
    }
...
jboolean JNICALL Java_tests_sharedclasses_options_TestSharedCacheJvmtiAPI_destroySharedCache(
    JNIEnv* jni_env, jclass clazz, jstring cacheDir, jstring cacheName, jint cacheType, jboolean useCommandLineValues)
{
