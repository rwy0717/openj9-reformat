...
jint JNICALL ts002(agentEnv* agent_env, char* args)
{
    jvmtiError result;
    jint extensionCount;
...
jboolean JNICALL Java_com_ibm_jvmti_tests_traceSubscription_ts002_tryRegisterTracePointSubscribers(
    JNIEnv* jni_env, jclass clazz)
{
    ...
    result = tracePointRegisterFunction(
        jvmti_env, "ts002 subscriber 1", ts002DataCallback1, ts002AlarmCallback1, NULL, &subscription1);
    if (result != JVMTI_ERROR_NONE) {
        error(env, result, "RegisterTraceSubscriber failed");
        return FALSE;
    }
    result = tracePointRegisterFunction(
        jvmti_env, "ts002 subscriber 2", ts002DataCallback2, ts002AlarmCallback2, NULL, &subscription2);
    if (result != JVMTI_ERROR_NONE) {
        error(env, result, "RegisterTraceSubscriber failed");
        return FALSE;
    }
...
jboolean JNICALL Java_com_ibm_jvmti_tests_traceSubscription_ts002_tryDeregisterTracePointSubscribers(
    JNIEnv* jni_env, jclass clazz)
{
    jvmtiError result;
    jvmtiEnv* jvmti_env = env->jvmtiEnv;
...
jint JNICALL Java_com_ibm_jvmti_tests_traceSubscription_ts002_getTracePointCount1(JNIEnv* jni_env, jclass clazz)
{
    return tracePointCount1;
}
jint JNICALL Java_com_ibm_jvmti_tests_traceSubscription_ts002_getTracePointCount2(JNIEnv* jni_env, jclass clazz)
{
    return tracePointCount2;
}
jboolean JNICALL Java_com_ibm_jvmti_tests_traceSubscription_ts002_hasAlarmTriggered1(JNIEnv* jni_env, jclass clazz)
{
    return alarmTriggered1;
}
jboolean JNICALL Java_com_ibm_jvmti_tests_traceSubscription_ts002_hasAlarmTriggered2(JNIEnv* jni_env, jclass clazz)
{
