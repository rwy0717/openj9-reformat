...
static const Limit deviceLimits[] = {
...
static const J9CudaError errors[] = { J9CUDA_NO_ERROR, J9CUDA_ERROR_MISSING_CONFIGURATION,
    ...
    J9CUDA_ERROR_NOT_SUPPORTED, J9CUDA_ERROR_STARTUP_FAILURE, J9CUDA_ERROR_NOT_FOUND };
 ...
 */
static void fillPattern(void* buffer, uintptr_t size, uint32_t seed)
{
 ...
 */
static BOOLEAN verifyPattern(const void* buffer, uintptr_t size, uint32_t seed)
{
 ...
 */
static BOOLEAN verifyFill(const void* buffer, uintptr_t size, const void* fill, uintptr_t fillSize)
{
    const uint8_t* bufferBytes = (const uint8_t*)buffer;
 ...
 */
static int32_t testBasic(struct J9PortLibrary* portLibrary, uint32_t* deviceCountOut)
{
 ...
 */
static BOOLEAN isErrorStringRequired(int32_t error, uint32_t deviceCount)
{
    if (0 != deviceCount) {
        return TRUE;
    }
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
static int32_t testErrors(struct J9PortLibrary* portLibrary, uint32_t deviceCount)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
static int32_t testEvents(struct J9PortLibrary* portLibrary, uint32_t deviceCount)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
static int32_t testLinker(struct J9PortLibrary* portLibrary, uint32_t deviceCount)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    for (deviceId = 0; deviceId < deviceCount; ++deviceId) {
        ...
        rc = j9cuda_linkerAddData(
            deviceId, linker, J9CUDA_JIT_INPUT_TYPE_PTX, fragment1_ptx, sizeof(fragment1_ptx), "fragment1", NULL);
        ...
        rc = j9cuda_linkerAddData(
            deviceId, linker, J9CUDA_JIT_INPUT_TYPE_PTX, fragment2_ptx, sizeof(fragment2_ptx), "fragment2", NULL);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
static int32_t testFunction(struct J9PortLibrary* portLibrary, uint32_t deviceId, J9CudaFunction function)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    for (index = 0; index < LENGTH_OF(sharedMemConfigs); ++index) {
        ...
        if (0 != rc) {
            outputComment(
                PORTLIB, "Failed to set function shared memory config(%d): %d\n", sharedMemConfigs[index], rc);
            goto done;
        }
    }
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
static int32_t testLaunch(struct J9PortLibrary* portLibrary, uint32_t deviceCount)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    for (deviceId = 0; deviceId < deviceCount; ++deviceId) {
        ...
        if (result.pointer != expected.pointer) {
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "Launch pointer result: %p, expected %p\n", result.pointer, expected.pointer);
        }
        ...
        if (result.intValue != expected.intValue) {
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "Launch intValue result: %d, expected %d\n", result.intValue, expected.intValue);
        }
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
static int32_t testModules(struct J9PortLibrary* portLibrary, uint32_t deviceCount)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 */
static void testPeerTransfer(struct J9PortLibrary* portLibrary, uint32_t deviceId, uint32_t peerDeviceId)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
static int32_t testPeerAccess(struct J9PortLibrary* portLibrary, uint32_t deviceCount)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    for (deviceId = 0; deviceId < deviceCount; ++deviceId) {
        ...
        for (peerDeviceId = 0; peerDeviceId < deviceCount; ++peerDeviceId) {
            ...
            if (0 != rc) {
                outputErrorMessage(PORTTEST_ERROR_ARGS, "Could not query peer access from device %d to device %d\n",
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
static int32_t testProperties(struct J9PortLibrary* portLibrary, uint32_t deviceCount)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    for (deviceId = 0; deviceId < deviceCount; ++deviceId) {
        ...
        { // cache config
            ...
            for (index = 0; index < LENGTH_OF(cacheConfigs); ++index) {
                ...
                if (0 != rc) {
                    outputErrorMessage(
                        PORTTEST_ERROR_ARGS, "Failed to set device cache config(%d): %d\n", cacheConfigs[index], rc);
                }
            }
        }
            ...
            for (; NULL != entry->name; ++entry) {
                ...
                if (computeCapability < entry->minComputeCapability) {
                    ...
                    if (0 != rc) {
                        ...
                        if (0 != rc) {
                            outputErrorMessage(
                                PORTTEST_ERROR_ARGS, "Could not set limit %s to %zu: %d\n", entry->name, value, rc);
                        }
                    } else {
                        outputErrorMessage(PORTTEST_ERROR_ARGS, "%s: %d not in expected range [%d,%d]\n", entry->name,
                            value, entry->minValue, entry->maxValue);
                    }
                }
            }
        }
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
static int32_t testGlobalMemory(struct J9PortLibrary* portLibrary, uint32_t deviceCount)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
static int32_t testInvalidDevices(struct J9PortLibrary* portLibrary, uint32_t deviceCount)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
...
#define DO_BAD_CALL(func, args)                                                                          \
    do {                                                                                                 \
        ...
        if (expectedError != rc) {                                                                       \
            outputErrorMessage(PORTTEST_ERROR_ARGS, "Unexpected return value from %s: %d\n", #func, rc); \
        }                                                                                                \
    } while (0)
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
static int32_t testStreams(struct J9PortLibrary* portLibrary, uint32_t deviceCount)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
...
int32_t j9cuda_runTests(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
