 ...
 */
int j9error_test0(struct J9PortLibrary* portLibrary)
{
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9error_test1(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    if (200 != errorCode) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "j9error_last_error_number() returned %d expected %d\n", errorCode, 200);
    }
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9error_test2(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    if (0 != strlen(message)) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "j9error_last_error_message() returned message of length %d expected %d\n", strlen(message), 1);
    }
    ...
    if (200 != errorCode) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "j9error_set_last_error_with_message() returned %d expected %d\n", errorCode, 200);
    }
 ...
 */
I_32 j9error_runTests(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    int rc;
