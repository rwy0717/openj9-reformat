...
static void verifySubAllocMem(
    struct J9PortLibrary* portLibrary, void* subAllocMem, UDATA allocSize, J9Heap* heapBase, const char* testName);
...
static void verifyHeapOutofRegionWrite(
    struct J9PortLibrary* portLibrary, U_8* memAllocStart, U_8* heapEnd, UDATA heapStartOffset, const char* testName);
 ...
 */
int j9heap_test0(struct J9PortLibrary* portLibrary)
{
 ...
 * It then enters a loop that calls j9heap_create on each of the first 16 byte-aligned locations with a heap size of
 * 1MB. After each j9heap_create, we perform a series of heap_alloc and heap_free from size 0 to the maximum possible.
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9heap_test1(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
        ...
        for (i = 0; i < 16; i++) {
            ...
            do {
                ...
                if (subAllocPtr) {
                    ...
                    /*The size returned may have been rounded-up to the nearest U64, and may have an extra 2 slots added
                     * in some circumstances*/
                    if (querySize < subAllocSize || querySize > (subAllocSize + 3 * sizeof(U_64))) {
                        outputErrorMessage(PORTTEST_ERROR_ARGS,
                            "j9heap_query_size returned the wrong size. Expected %zu, got %zu\n", subAllocSize,
                            querySize);
            ...
            verifyHeapOutofRegionWrite(
                portLibrary, allocPtr, allocPtrAdjusted + heapSize[j], heapStartOffset + i, testName);
        }
    }
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9heap_test2(struct J9PortLibrary* portLibrary, int randomSeed)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    allocPool = pool_new(
        sizeof(AllocListElement), 0, 0, 0, J9_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY, POOL_FOR_PORT(portLibrary));
    if (NULL == allocPool) {
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9heap_test3(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9heap_realloc_test0(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9heap_grow_test(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
     ...
     * So when j9heap created initially, firstFreeBlock always has the value 2(on 64bit) or 1 (on 32bit) and heapSize
     *should be 128 slots in this test heap. And firstFreeBlock should point to the first slot after header which should
     *have value 128 - sizeof(j9heap)/sizeof(U_64) - 2(head and tail of the slot) = 124 (on 64bit) or 125 (on 32bit)
     *
     *
     * |--------|-----|--------------------------------------------------------------------------------------------------------
     * |heapSize|first|    |    |																						 |
    ...
    if (128 != heapBase->firstFreeBlock) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "j9heap_grow() failed to set firstFreeBlock correctly. Expected value = 128, Found : %zu \n",
            heapBase->firstFreeBlock);
        goto exit;
    }
 ...
 *
 */
int j9heap_test_pmr_28277_999_760(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    struct J9PortLibrary* portLibrary, void* subAllocMem, UDATA allocSize, J9Heap* heapBase, const char* testName)
{
    ...
    if ((subAllocMem < (void*)&basePtr[SIZE_OF_J9HEAP_HEADER / sizeof(U_64)])
    ...
    if (0 == allocSize) {
        ...
        if (!(allocSize <= ((UDATA)(-blockSizeStart) * sizeof(U_64))
                && ((UDATA)(-blockSizeStart) * sizeof(U_64)) < (allocSize + 3 * sizeof(U_64)))) {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                "The allocated block seems to be overly large (3 or more free slots): 0x%p, request size: %zu, block "
                "size: %lld\n",
                subAllocMem, allocSize, blockSizeStart);
 ...
 */
I_32 j9heap_runTests(struct J9PortLibrary* portLibrary, int randomSeed)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    int rc = 0;
    int i;
        ...
        for (i = 0; i < 10; ++i) {
            ...
            if (rc) {
                ...
                if ((j9time_current_time_millis() - timeStart) >= testDurationMin * 60 * 1000) {
                    outputComment(
                        portLibrary, "We've run for too long, ending the test. This is not a test failure.\n\n");
                    break;
                }
                outputComment(portLibrary, "\n\n");
            }
