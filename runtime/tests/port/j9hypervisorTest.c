 ...
 */
int validate_processor_stats(J9PortLibrary* portLibrary, J9GuestProcessorUsage currguestprocessorUsage,
    J9GuestProcessorUsage prevguestprocessorUsage)
{
    ...
    outputComment(
        portLibrary, "\t The CPU Entitlement of the current VM: %f\n", currguestprocessorUsage.cpuEntitlement);
#else
...
#endif
#if !(defined(LINUX) && defined(S390)) && !defined(J9ZOS390) && !(defined(PPC64) && defined(J9VM_ENV_LITTLE_ENDIAN))
 ...
 *
 */
int j9hypervisor_test_hypervisor_present(
    J9PortLibrary* portLibrary, char* hypervisor, char* hvOptions, BOOLEAN negative)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    if (!isVirtual) {
        ...
        default: { /* Capture any possible error codes reported (other than hypdetect unsupported). */
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                "j9hypervisor_hypervisor_present() returned: %i "
        ...
        case J9HYPERVISOR_NOT_PRESENT: {
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "Expecting virtualized environment whereas no hypervisor detected\n");
            break;
        }
        default: {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
 ...
 * @return the return value from @reportTestExit
 */
int j9hypervisor_test_get_hypervisor_info(
    J9PortLibrary* portLibrary, char* hypervisor, char* hvOptions, BOOLEAN negative)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    if ((NULL == hypervisor) || (J9PORT_ERROR_HYPERVISOR_NO_HYPERVISOR == rc)) {
        ...
        outputComment(PORTLIB,
            "\tj9hypervisor_get_hypervisor_info() returned -856 "
        ...
        if ((strcmp(hypervisor, VMWARE) == 0) && (strcmp(details.hypervisorName, "VMWare") == 0)) {
            ...
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                "\tThe hypervisor name returned by j9hypervisor_get_hypervisor_info is not correct \n"
 ...
 * @return the return value from @reportTestExit
 */
int j9hypervisor_test_get_guest_memory_usage(J9PortLibrary* portLibrary, char* hypervisor, char* hvOptions)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    if (J9PORT_ERROR_HYPERVISOR_NO_HYPERVISOR == rc || NULL == hypervisor) {
        ...
        if (0 == isSupported) {
            outputComment(portLibrary,
                "\tCannot proceed with the test. Guest Memory "
        ...
        if (strcmp(hvOptions, NOHYPFS) == 0) {
            outputComment(portLibrary,
                "\tCannot proceed with the test. Cannot retrieve Guest Memory "
                "Usage statistics as HYPFS is not mounted\n");
        } else {
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "j9hypervisor_get_guest_memory_usage() failed as hypfs is not mounted\n");
        }
        goto exit;
        ...
        if (strcmp(hvOptions, NOUSERHYPFS) == 0) {
            ...
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "j9hypervisor_get_guest_memory_usage() failed as hypfs is mounted readonly\n");
        }
        goto exit;
    } else if ((0 == rc) && (strcmp(hvOptions, HYPFS) != 0)) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "Testing for failure case, but test passing!!\n"
    ...
    outputComment(
        portLibrary, "\t Maximum memory that can be used by the GuestOS: %lld MB\n", prevguestMemUsage.maxMemLimit);
     ...
     */
    if ((prevguestMemUsage.timestamp < currguestMemUsage.timestamp) && (currguestMemUsage.memUsed > 0)) {
 ...
 * @return the return value from @reportTestExit
 */
int j9hypervisor_test_get_guest_processor_usage(J9PortLibrary* portLibrary, char* hypervisor, char* hvOptions)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 */
I_32 j9hypervisor_runTests(struct J9PortLibrary* portLibrary, char* hypervisor, char* hvOptions, BOOLEAN negative)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    int rc = TEST_PASS;
