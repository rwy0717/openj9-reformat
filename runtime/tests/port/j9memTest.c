...
#define J9MEM_ALLOCATE_FAKE_RETURN_VALUE \
    ((void*)0xBADC0DE) /**<@internal A known return for @ref fake_j9mem_allocate_memory */
 ...
 */
static void* fake_j9mem_allocate_memory(struct J9PortLibrary* portLibrary, UDATA byteAmount, const char* callSite)
{
    return J9MEM_ALLOCATE_FAKE_RETURN_VALUE;
}
 ...
 */
int j9mem_test0(struct J9PortLibrary* portLibrary)
{
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9mem_test1(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9mem_test2(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9mem_test4(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * platforms.
 */
int j9mem_test5(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * platforms.
 */
int j9mem_test6(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
...
static void shuffleArray(struct J9PortLibrary* portLibrary, UDATA* array, UDATA length)
{
    UDATA i;
 ...
 * HEAP_SIZE_BYTES in j9mem32helpers.c):
 * 1. size requests that are much smaller than the heap size are satisfied with j9heap suballocation.
 * 2. size requests that are slightly smaller than the heap size will not use j9heap suallocation (because they cannot
 * be satisfied by suballocating a j9heap due to heap management overhead), but rather given their own vmem allocated
 * chunk.
 * 3. size requests that are greater or equal to the heap size also will not use j9heap suallocation, but rather given
 ...
 * platforms.
 */
int j9mem_test7_allocate32(struct J9PortLibrary* portLibrary, int randomSeed)
{
    void* pointer;
#if defined(J9VM_ENV_DATA64)
     ...
     */
#if defined(J9VM_ENV_DATA64)
    UDATA allocBlockSizes[]
        = { 0, 1, 512, 4096, 1024 * 1024, HEAP_SIZE_BYTES / 8, HEAP_SIZE_BYTES / 4, HEAP_SIZE_BYTES / 3,
              HEAP_SIZE_BYTES / 2, HEAP_SIZE_BYTES - 6, HEAP_SIZE_BYTES - 10, HEAP_SIZE_BYTES, HEAP_SIZE_BYTES + 6 };
#else
...
static U_32 childrenOfDummyCategoryOne[]
...
static UDATA categoryWalkFunction(U_32 categoryCode, const char* categoryName, UDATA liveBytes, UDATA liveAllocations,
    BOOLEAN isRoot, U_32 parentCategoryCode, OMRMemCategoryWalkState* walkState)
{
 ...
 */
static void getCategoriesState(struct J9PortLibrary* portLibrary, struct CategoriesState* state)
{
 ...
 */
int j9mem_test8_categories(struct J9PortLibrary* portLibrary)
{
 ...
 */
static UDATA categoryWalkFunction2(U_32 categoryCode, const char* categoryName, UDATA liveBytes, UDATA liveAllocations,
    BOOLEAN isRoot, U_32 parentCategoryCode, OMRMemCategoryWalkState* walkState)
{
    categoriesWalked++;
 ...
 */
int j9mem_test9_category_walk(struct J9PortLibrary* portLibrary)
{
 ...
 */
I_32 j9mem_runTests(struct J9PortLibrary* portLibrary, int randomSeed)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    int rc;
