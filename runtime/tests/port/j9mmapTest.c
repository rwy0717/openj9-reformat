 ...
 */
int j9mmap_test0(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 */
int j9mmap_test1(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
...
#define J9MMAP_TEST1_BUFFER_SIZE 100
    ...
    mmapHandle = (J9MmapHandle*)j9mmap_map_file(
        fd, 0, (UDATA)fileLength, NULL, J9PORT_MMAP_FLAG_READ, OMRMEM_CATEGORY_PORT_LIBRARY);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9mmap_test2(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
...
#define J9MMAP_TEST2_BUFFER_SIZE 100
    ...
    mmapHandle = (J9MmapHandle*)j9mmap_map_file(
        fd, 0, (UDATA)fileLength, NULL, J9PORT_MMAP_FLAG_WRITE, OMRMEM_CATEGORY_PORT_LIBRARY);
    if ((NULL == mmapHandle) || (NULL == mmapHandle->pointer)) {
        lastErrorMessage = (char*)j9error_last_error_message();
        lastErrorNumber = j9error_last_error_number();
        outputErrorMessage(PORTTEST_ERROR_ARGS,
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9mmap_test3(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
...
#define J9MMAP_TEST3_BUFFER_SIZE 100
    ...
    if (!(mmapCapabilities & J9PORT_MMAP_CAPABILITY_COPYONWRITE)) {
        outputComment(
            portLibrary, "J9PORT_MMAP_CAPABILITY_COPYONWRITE not supported on this platform, bypassing test\n");
        goto exit;
    }
    ...
    mmapHandle = (J9MmapHandle*)j9mmap_map_file(
        fd, 0, (UDATA)fileLength, NULL, J9PORT_MMAP_FLAG_COPYONWRITE, OMRMEM_CATEGORY_PORT_LIBRARY);
    if ((NULL == mmapHandle) || (NULL == mmapHandle->pointer)) {
        lastErrorMessage = (char*)j9error_last_error_message();
        lastErrorNumber = j9error_last_error_number();
        outputErrorMessage(PORTTEST_ERROR_ARGS,
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9mmap_test4(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
...
#define J9MMAP_TEST4_BUFFER_SIZE 100
    ...
    if (!(mmapCapabilities & J9PORT_MMAP_CAPABILITY_WRITE) || !(mmapCapabilities & J9PORT_MMAP_CAPABILITY_MSYNC)) {
        outputComment(
            portLibrary, "J9PORT_MMAP_CAPABILITY_WRITE or _MSYNC not supported on this platform, bypassing test\n");
        goto exit;
    }
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9mmap_test5(J9PortLibrary* portLibrary, char* argv0)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
...
int j9mmap_test5_child_1(J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
...
#define J9MMAP_TEST5_BUFFER_SIZE 100
    ...
    mmapHandle = (J9MmapHandle*)j9mmap_map_file(
        fd, 0, (UDATA)fileLength, mappingName, J9PORT_MMAP_FLAG_WRITE, OMRMEM_CATEGORY_PORT_LIBRARY);
    if ((NULL == mmapHandle) || (NULL == mmapHandle->pointer)) {
        lastErrorMessage = (char*)j9error_last_error_message();
        lastErrorNumber = j9error_last_error_number();
        outputErrorMessage(PORTTEST_ERROR_ARGS,
...
int j9mmap_test5_child_2(J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9mmap_test6(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    if (mmapCapabilities & J9PORT_MMAP_CAPABILITY_PROTECT) {
        ...
        if ((rc & J9PORT_PAGE_PROTECT_NOT_SUPPORTED) != 0) {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                "Mmap is claims to be capable of PROTECT, however, j9mmap_protect() returns "
        ...
        if ((rc & J9PORT_PAGE_PROTECT_NOT_SUPPORTED) == 0) {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                "Mmap is not capable of PROTECT, however, j9mmap_protect() does not return "
                "J9PORT_PAGE_PROTECT_NOT_SUPPORTED\n");
            goto exit;
        }
    }
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9mmap_test7(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9mmap_test8(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9mmap_test9(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    mmapHandle = (J9MmapHandle*)j9mmap_map_file(
        fd, 0, (UDATA)fileLength, NULL, J9PORT_MMAP_FLAG_WRITE | J9PORT_MMAP_FLAG_SHARED, OMRMEM_CATEGORY_PORT_LIBRARY);
    if ((NULL == mmapHandle) || (NULL == mmapHandle->pointer)) {
        lastErrorMessage = (char*)j9error_last_error_message();
        lastErrorNumber = j9error_last_error_number();
        outputErrorMessage(PORTTEST_ERROR_ARGS,
    ...
    if (rc != 0) {
        if (rc == J9PORT_PAGE_PROTECT_NOT_SUPPORTED) {
            outputComment(
                portLibrary, "J9PORT_MMAP_CAPABILITY_PROTECT not supported on this platform, bypassing test\n");
        } else {
            lastErrorMessage = (char*)j9error_last_error_message();
            lastErrorNumber = j9error_last_error_number();
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                "j9mmap_protect() failed: lastErrorNumber=%d, lastErrorMessage=%s\n", lastErrorNumber,
                lastErrorMessage);
        }
        goto exit;
    }
 ...
 */
int j9mmap_test10(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
...
int j9mmap_test11(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9mmap_test12(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9mmap_test13(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
...
int j9mmap_testDontNeed(struct J9PortLibrary* portLibrary)
{
 ...
 */
I_32 j9mmap_runTests(struct J9PortLibrary* portLibrary, char* argv0, char* j9mmap_child)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    int rc;
