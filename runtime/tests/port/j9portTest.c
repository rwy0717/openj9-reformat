 ...
 */
static void setupVersionInformation(
    struct J9PortLibraryVersion* version, U_16 majorVersionNumber, U_16 minorVersionNumber, U_64 capabilities)
{
 ...
 */
static void verifyVersionInformation(struct J9PortLibrary* portLibrary, const char* testName,
    struct J9PortLibrary* fakePortLibrary, struct J9PortLibraryVersion* expectedVersion, UDATA fields)
{
 ...
 */
static void verifyCreateTableContents(
    struct J9PortLibrary* portLibrary, const char* testName, struct J9PortLibraryVersion* version, UDATA size)
{
 ...
 */
static I_32 fake_mem_startup(struct OMRPortLibrary* portLibrary, UDATA portGlobalSize) { return failMemStartup; }
 ...
 * @return negative error code
 */
static I_32 fake_time_startup(struct OMRPortLibrary* portLibrary) { return failMemStartup; }
 ...
 */
int j9port_test0(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9port_test1(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    verifyVersionInformation(
        portLibrary, testName, fakePtr, &expectedVersion, J9VERSION_VERIFY_MINOR | J9VERSION_VERIFY_CAPABILITIES);
    if (3 != fakePtr->portVersion.majorVersionNumber) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "j9port_getVersion() corrupted majorVersionNumber found %d expected %d\n",
    ...
    fakePtr->portVersion.padding = 0xBADC0DE;
    verifyVersionInformation(
        portLibrary, testName, fakePtr, &expectedVersion, J9VERSION_VERIFY_MAJOR | J9VERSION_VERIFY_CAPABILITIES);
    if (72 != fakePtr->portVersion.minorVersionNumber) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "j9port_getVersion() corrupted minorVersionNumber found %d expected %d\n",
    ...
    verifyVersionInformation(
        portLibrary, testName, fakePtr, &expectedVersion, J9VERSION_VERIFY_MAJOR | J9VERSION_VERIFY_MINOR);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9port_test2(struct J9PortLibrary* portLibrary)
{
    ...
    if (actualSize != expectedSize) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "j9port_getSize() returned %zu expected %zu\n", actualSize, expectedSize);
    }

    /* Pass, capabilities don't matter (Except J9PORT_CAPABILITY_STANDARD) */
    setupVersionInformation(
        &version, J9PORT_MAJOR_VERSION_NUMBER, J9PORT_MINOR_VERSION_NUMBER, J9PORT_CAPABILITY_STANDARD);
    actualSize = j9port_getSize(&version);
    if (actualSize != expectedSize) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "j9port_getSize() returned %zu expected %zu\n", actualSize, expectedSize);
    ...
    setupVersionInformation(
        &version, J9PORT_MAJOR_VERSION_NUMBER, J9PORT_MINOR_VERSION_NUMBER, 0xBADC0DE | J9PORT_CAPABILITY_STANDARD);
    actualSize = j9port_getSize(&version);
    if (actualSize != expectedSize) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "j9port_getSize() returned %zu expected %zu\n", actualSize, expectedSize);
 ...
 */
int j9port_test3(struct J9PortLibrary* portLibrary)
{
    ...
    if (J9PORT_ERROR_INIT_WRONG_CAPABILITIES != rc) {
        outputErrorMessage(PORTTEST_ERROR_ARGS, "j9port_create_library() returned %d expected %d\n", rc,
            J9PORT_ERROR_INIT_WRONG_CAPABILITIES);
    }
    /* Ensure we didn't write anything, being lazy only checking first byte */
    if (eyeCatcher != bigBuffer[0]) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
    ...
    if (J9PORT_ERROR_INIT_WRONG_MAJOR_VERSION != rc) {
        outputErrorMessage(PORTTEST_ERROR_ARGS, "j9port_create_library() returned %d expected %d\n", rc,
            J9PORT_ERROR_INIT_WRONG_MAJOR_VERSION);
    }
    /* Ensure we didn't write anything, being lazy only checking one byte.  Note that buffer will have the first U_16
     * set to the major version */
    if (eyeCatcher != bigBuffer[sizeof(U_16)]) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "j9port_create_library() trampled memory expected \"%x\" found \"%x\"\n", eyeCatcher,
            bigBuffer[sizeof(U_16)]);
    }
 ...
 * @see j9port.c::j9port_init_library "j9port_init_library()"
 */
int j9port_test4(struct J9PortLibrary* portLibrary)
{
    ...
    if (failMemStartup != rc) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "j9port_startup_library() returned %d expected %d\n", rc, failMemStartup);
    }
 ...
 * @see j9port.c::j9port_startup_library "j9port_startup_library()"
 */
int j9port_test5(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9port_test6(struct J9PortLibrary* portLibrary)
{
 ...
 * function.
 */
int j9port_test7(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9port_test8(struct J9PortLibrary* portLibrary)
{
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9port_test9(struct J9PortLibrary* portLibrary)
{
    ...
    rc = OMRPORT_FROM_J9PORT(fakePtr)->port_isFunctionOverridden(
        OMRPORT_FROM_J9PORT(fakePtr), offsetof(OMRPortLibrary, mem_startup));
    if (1 != rc) {
        outputErrorMessage(PORTTEST_ERROR_ARGS, "j9port_isFunctionOverridden returned %d expected 1\n", rc);
    }

    rc = OMRPORT_FROM_J9PORT(fakePtr)->port_isFunctionOverridden(
        OMRPORT_FROM_J9PORT(fakePtr), offsetof(OMRPortLibrary, mem_shutdown));
    if (1 != rc) {
        outputErrorMessage(PORTTEST_ERROR_ARGS, "j9port_isFunctionOverridden returned %d expected 1\n", rc);
    }

    rc = OMRPORT_FROM_J9PORT(fakePtr)->port_isFunctionOverridden(
        OMRPORT_FROM_J9PORT(fakePtr), offsetof(OMRPortLibrary, time_hires_clock));
    if (1 != rc) {
        outputErrorMessage(PORTTEST_ERROR_ARGS, "j9port_isFunctionOverridden returned %d expected 1\n", rc);
    }

    rc = OMRPORT_FROM_J9PORT(fakePtr)->port_isFunctionOverridden(
        OMRPORT_FROM_J9PORT(fakePtr), offsetof(OMRPortLibrary, time_startup));
    if (1 != rc) {
        outputErrorMessage(PORTTEST_ERROR_ARGS, "j9port_isFunctionOverridden returned %d expected 1\n", rc);
    }

    rc = OMRPORT_FROM_J9PORT(fakePtr)->port_isFunctionOverridden(
        OMRPORT_FROM_J9PORT(fakePtr), offsetof(OMRPortLibrary, time_hires_frequency));
    if (0 != rc) {
        outputErrorMessage(PORTTEST_ERROR_ARGS, "j9port_isFunctionOverridden returned %d expected 0\n", rc);
    }

    rc = OMRPORT_FROM_J9PORT(fakePtr)->port_isFunctionOverridden(
        OMRPORT_FROM_J9PORT(fakePtr), offsetof(OMRPortLibrary, tty_printf));
    if (0 != rc) {
        outputErrorMessage(PORTTEST_ERROR_ARGS, "j9port_isFunctionOverridden returned %d expected 0\n", rc);
    }
 ...
 */
I_32 j9port_runTests(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    int rc;
