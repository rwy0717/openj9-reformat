...
static void j9process_fireWriteAndReadImpl(
    struct J9PortLibrary* portLibrary, const char* testName, char** command, UDATA commandLength);
static void j9process_fireAndCheckIfCompleteImpl(
    struct J9PortLibrary* portLibrary, const char* testName, char** command, UDATA commandLength);
static void j9process_fireAndForgetImpl(
    struct J9PortLibrary* portLibrary, const char* testName, char** command, UDATA commandLength);
static void j9process_fireAndTerminateImpl(
    struct J9PortLibrary* portLibrary, const char* testName, char** command, UDATA commandLength);
static void j9process_fireAndWaitForImpl(
    struct J9PortLibrary* portLibrary, const char* testName, char** command, UDATA commandLength);
static void testNonBlockingIOAndIgnoreOutputImpl(
    struct J9PortLibrary* portLibrary, const char* testName, char** command, UDATA commandLength, U_32 options);
static void j9process_fireWriteAndReadWithRedirectImpl(struct J9PortLibrary* portLibrary, const char* testName,
    char** command, UDATA commandLength, U_32 options, IDATA fdInput, IDATA fdOutput, IDATA fdError);
static BOOLEAN verifyFileContent(
    struct J9PortLibrary* portLibrary, const char* testName, char* file1name, char* expectedResult);
static UDATA processGroupSignalHandler(
    struct J9PortLibrary* portLibrary, U_32 gpType, void* handlerInfo, void* userData);
 ...
 */
static int processCrash(struct J9PortLibrary* portLibrary)
{
 ...
 * @return 0
 */
static int processSuccess(struct J9PortLibrary* portLibrary)
{
 ...
 * @return 0
 */
static int processSaveEnv(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    if (0 < rc) {
        ...
        /* This memory is a placeholder for the environment, for the purposes of passing it into j9process_create. The
         * child process will replicate it, so once j9process_create returns, we can free it */
        sysinfo_env_iterator_buffer = j9mem_allocate_memory(envSize, OMRMEM_CATEGORY_PORT_LIBRARY);
 ...
 * @return 0
 */
static int processSleep(struct J9PortLibrary* portLibrary, unsigned int seconds)
{
 ...
 * @return 0
 */
static int processWrite(struct J9PortLibrary* portLibrary)
{
    char buf[1024];
    int i = 0;
 ...
 */
static int setTestProcessGroupSignalHandler(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    int rc = -1;
    ...
    if (FALSE == setConsoleCtrlHandlerRC) {
        j9error_set_last_error(GetLastError(),
            1); /* this is a hack to get outputErrorMessage to report the error that occurred in SetConsoleCtrlHandler*/
        rc = -1;
    } else {
        rc = 0;
    }
 ...
 *  @return 0
 */
static int processRegisterHandlerAndSpin(struct J9PortLibrary* portLibrary)
{
 ...
 * 				such that the child's handler would have had time to be invoked and write the ack to
 *stdout.
 * *
 ...
 */
static IDATA j9process_testNewProcessGroupByTriggeringSignal(struct J9PortLibrary* portLibrary)
{
#define MAX_BUF_SIZE 256
    ...
    /********************************/
    /********************************/
    /* Test 1: When J9PORT_PROCESS_CREATE_NEW_PROCESS_GROUP is not used, the child's CTRL_C_EVENT handler should be
     * invoked */
    /********************************/
    /********************************/
    ...
    retVal = j9process_create((const char**)command, commandLength, env, envSize, dir, 0, J9PORT_INVALID_FD,
        J9PORT_INVALID_FD, J9PORT_INVALID_FD, &processHandle);
    ...
    if (retVal != 0) {
        ...
        /* trigger the CTRL_C_EVENT, then sleep for 3 seconds, which should leave enough time for the child handler to
         * be invoked and write to stdout Note that the parent (this) process' handler will also be invoked */
#if defined(WIN32)
        ...
        if (retVal != -1) {
            if (strcmp(buffer, HANDLER_INVOKED) != 0) {
                ...
                outputErrorMessage(PORTTEST_ERROR_ARGS,
                    "Expected: unexpected output received from child: \"%s\", expected: \"%s\"\n", buffer,
                    HANDLER_INVOKED);
            } else {
    ...
    memset(buffer, 0, sizeof(buffer));
    retVal = j9process_create((const char**)command, commandLength, env, envSize, dir,
        J9PORT_PROCESS_CREATE_NEW_PROCESS_GROUP, J9PORT_INVALID_FD, J9PORT_INVALID_FD, J9PORT_INVALID_FD,
        &processHandle);
    ...
    if (retVal != 0) {
        ...
        /* same as before, wait until the child has registered its handler, which it will indicate by writing "t" to
         * stdout */
        j9tty_printf(portLibrary, " Waiting for child to register handler\n");
        waitForChar(portLibrary, processHandle, timeoutMillis, J9PORT_PROCESS_STDOUT);
        ...
        /* trigger the signal/event, then sleep for 3 seconds, which should leave enough time for the event to propagate
         * to the child Note that the parent (this) process' handler will also be invoked, and write the #defined
         * HANDLER_INVOKED message to the console */
#if defined(WIN32)
        ...
        if (0 < numBytesAvailable) {
            ...
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                "The child handle was invoked when it should not have been (we know this because the child wrote this "
                "to stdout: \"%s\")\n",
                buffer);
        }
 ...
 * @return 0
 */
static int processWriteProcessGroupID(struct J9PortLibrary* portLibrary)
{
    char buf[1024];
    int i = 0;
 ...
 * @param[in] commandLength The number of elements in the command line where by convention the first element is the file
 *to be executed
 *
 * @return TEST_PASS on success, TEST_FAIL on error
 */
static IDATA j9process_testCreateInNewProcessGroupUnix(struct J9PortLibrary* portLibrary)
{
#define MAX_BUF_SIZE 256
    PORT_ACCESS_FROM_PORT(portLibrary);
    J9ProcessHandle processHandle;
    ...
    /********************************/
    /********************************/
    /* Test 1: When J9PORT_PROCESS_CREATE_NEW_PROCESS_GROUP is not used, the child should have the same pgid as the
     * parent */
    /********************************/
    /********************************/
    j9tty_printf(portLibrary, " Starting process %s %s\n", command[0], command[1]);
    ...
    if (childProcessGroupID != parentProcessGroupID) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "parent and child should have same process group ID, instead parent pgid = %u, child pgid = %u\n",
    ...
    /********************************/
    /********************************/
    /* Test 2: When J9PORT_PROCESS_CREATE_NEW_PROCESS_GROUP is used, the child should not have the same pgid as the
     * parent */
    /********************************/
    /********************************/
    j9tty_printf(portLibrary, " Starting process %s %s\n", command[0], command[1]);
    ...
    if (childProcessGroupID == parentProcessGroupID) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "J9PORT_PROCESS_CREATE_NEW_PROCESS_GROUP used, so parent and child should not have same process group ID, "
            "instead parent pgid = %u, child pgid = %u\n",
            parentProcessGroupID, childProcessGroupID);
    } else {
 ...
 * @return 0
 */
static int processWriteMore(struct J9PortLibrary* portLibrary)
{
    int i = 0;
    PORT_ACCESS_FROM_PORT(portLibrary);
#define MESSAGE                                                                      \
    "This is exactly 128 characters if I keep on going for this amount until I get " \
    "bored...........................................\n"
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9process_verifyPortLibrary(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9process_fireAndForget(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9process_fireAndWaitFor(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    struct J9PortLibrary* portLibrary, const char* testName, char** command, UDATA commandLength)
{
    ...
    if (retVal != 0) {
        ...
        if (retVal != 0) {
            ...
            if (0 == retVal) {
                outputErrorMessage(PORTTEST_ERROR_ARGS,
                    "j9process_isComplete returned that the process has not completed yet to get the exit code.\n");
            } else if (1 != retVal) {
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9process_fireAndCheckIfComplete(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9process_fireAndTerminate(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
...
#define MAX_BUF_SIZE 256
    ...
    if (MAX_BUF_SIZE < numBytesAvailable) {
        ...
        outputComment(portLibrary,
            "j9process_get_available reports %d character(s) to read from the process output stream.\n",
            numBytesAvailable);
        ...
        if (0 <= retVal) {
            ...
            if (0 != strcmp(buffer, HELLO_STANDARD_OUT)) {
                outputErrorMessage(
                    PORTTEST_ERROR_ARGS, "j9process_read incorrect message read from J9PORT_PROCESS_STDOUT\n", retVal);
            }
        } else {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                "j9process_read trying to read from the output stream returned error code %d.\n", retVal);
        }
    } else if (0 == numBytesAvailable) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "j9process_get_available reports that there are no characters to read from the process output stream.\n");
    } else {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "j9process_get_available failed with error code: %d\n", numBytesAvailable);
    }
    ...
    if (MAX_BUF_SIZE < numBytesAvailable) {
        ...
        outputComment(portLibrary,
            "j9process_get_available reports %d character(s) to read from the process error stream.\n",
            numBytesAvailable);
        ...
        if (0 <= retVal) {
            ...
            if (0 != strcmp(buffer, HELLO_STANDARD_ERR)) {
                outputErrorMessage(
                    PORTTEST_ERROR_ARGS, "j9process_read incorrect message read from J9PORT_PROCESS_STDERR\n", retVal);
            }
        } else {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                "j9process_read trying to read from the error stream returned error code %d.\n", retVal);
        }
    } else if (0 == numBytesAvailable) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "j9process_get_available reports that there are no characters to read from the process error stream.\n");
    } else {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "j9process_get_available failed with error code: %d\n", numBytesAvailable);
    }
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9process_envTest(struct J9PortLibrary* portLibrary)
{
#define J9_MAX_ENV 32767
    ...
    if (0 == j9sysinfo_get_executable_name("pltest", (char**)&command[0])) {
        ...
        retVal = j9process_create(command, commandLength, env, envSize, dir, 0, J9PORT_INVALID_FD, J9PORT_INVALID_FD,
            J9PORT_INVALID_FD, &processHandle);
        if (0 == retVal) {
            retVal = pollProcessForComplete(portLibrary, testName, processHandle, timeoutMillis);
            if (-1 != retVal) {
                IDATA rc = -1;
                            ...
                            if (0 == rc) {
                                while (j9sysinfo_env_iterator_hasNext(&state)) {
                                    ...
                                    if (0 == rc) {
                                        ...
                                        char* buffer = (char*)j9mem_allocate_memory(
                                            elementLen + 1, OMRMEM_CATEGORY_PORT_LIBRARY);
                                        ...
                                        if (-1 != rc) {
                                            ...
                                            if (0 != rc) {
                                                outputErrorMessage(PORTTEST_ERROR_ARGS,
                                                    "Matching failed: rc = %d, str1 = %s, str2=%s\n", retVal, buffer,
                                            ...
                                            outputErrorMessage(PORTTEST_ERROR_ARGS,
                                                "Failed to read env var %s from file\n", retVal, buffer);
                                            j9mem_free_memory(buffer);
                                            break;
                                        }
                                        j9mem_free_memory(buffer);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9process_fireWriteAndRead(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9process_testExecFailure(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    if (0 == retVal) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "j9process_create returned successfully even though the non-existent process %s was called.\n", command[0]);
        j9process_terminate(processHandle);
    } else {
        outputComment(PORTLIB,
            "j9process_create successfully detected that process doesn't exist, returned error code %d.\n", retVal);
    }
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9process_testChdirFailure(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    if (0 == retVal) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "j9process_create returned successfully even though the non-existent directory %s was passed in as a "
            "parameter.\n",
            dir);
        j9process_terminate(processHandle);
    } else {
        outputComment(PORTLIB,
            "j9process_create successfully detected that directory doesn't exist, returned error code %d.\n", retVal);
    }
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9process_testNonBlockingIOAndIgnoreOutput(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
...
#define SLEEP_MILLIS 100
    ...
    retVal = j9process_create((const char**)command, commandLength, env, envSize, dir, options, J9PORT_INVALID_FD,
        J9PORT_INVALID_FD, J9PORT_INVALID_FD, &processHandle);
    if (retVal != 0) {
        outputErrorMessage(PORTTEST_ERROR_ARGS, "j9process_create returned error code %d.\n", retVal);
        return;
 ...
 *
 */
static void j9process_testCloseScenarioImpl(
    struct J9PortLibrary* portLibrary, const char* testName, char** command, UDATA commandLength)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    IDATA retVal;
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9process_testCloseScenario(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    char* testName = "j9process_fireAndForget";
    char* command[2];
 ...
 *			j9process_fireWriteAndReadWithRedirectImpl(portLibrary, testName, command, commandLength, options,
 *J9PORT_INVALID_FD, J9PORT_INVALID_FD, J9PORT_INVALID_FD); typing "hi quit\n" at pltest console window when test
 *stopped with following output: options: J9PORT_PROCESS_INHERIT_STDIN Starting process
 *E:\IVE\IVEjres\HEAD\jvmwi3270\jvmwi3270-20100805\jre\bin\default\pltest.exe -child_j9process_processWrite Test should
 *continue without failure otherwise a failure will be generated for "Child did not finish before timeout"
 *
 * @param[in] portLibrary The port library under test
 *
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9process_testRedirect(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    if (0 == j9sysinfo_get_executable_name("pltest", &command[0])) {
        ...
        j9process_fireWriteAndReadWithRedirectImpl(portLibrary, testName, command, commandLength, options,
            J9PORT_INVALID_FD, J9PORT_INVALID_FD, J9PORT_INVALID_FD);
        ...
        if (-1 != fd1) {
            ...
            if (0 != rc) {
                ...
                j9process_fireWriteAndReadWithRedirectImpl(portLibrary, testName, command, commandLength, options,
                    fdInput, J9PORT_INVALID_FD, J9PORT_INVALID_FD);
        ...
        if (fdOutput != -1) {
            j9process_fireWriteAndReadWithRedirectImpl(
                portLibrary, testName, command, commandLength, options, J9PORT_INVALID_FD, fdOutput, J9PORT_INVALID_FD);
        ...
        if (fdError != -1) {
            j9process_fireWriteAndReadWithRedirectImpl(
                portLibrary, testName, command, commandLength, options, J9PORT_INVALID_FD, J9PORT_INVALID_FD, fdError);
        ...
        if (fdOutput != -1) {
            ...
            if (verifyFileContent(portLibrary, testName, "fdOutput.txt", HELLO_STANDARD_OUTNERR) == FALSE) {
                outputErrorMessage(PORTTEST_ERROR_ARGS,
                    "verifyFileContent of fdOutput.txt with J9PORT_PROCESS_REDIRECT_STDERR_TO_STDOUT NOT successful "
                    "\n");
            } else {
                outputComment(PORTLIB,
                    "verifyFileContent of fdOutput.txt with J9PORT_PROCESS_REDIRECT_STDERR_TO_STDOUT successful \n");
            }
            j9file_unlink("fdOutput.txt");
        } else {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                "j9file_open fdOutput.txt with J9PORT_PROCESS_REDIRECT_STDERR_TO_STDOUT NOT successful \n");
        }
        ...
        if (fdOutput != -1 && fdError != -1) {
            j9process_fireWriteAndReadWithRedirectImpl(
                portLibrary, testName, command, commandLength, options, J9PORT_INVALID_FD, fdOutput, fdError);
            j9file_close(fdOutput);
            if (verifyFileContent(portLibrary, testName, "fdOutput.txt", HELLO_STANDARD_OUT) == FALSE) {
                outputErrorMessage(PORTTEST_ERROR_ARGS, "verifyFileContent of fdOutput.txt NOT successful \n");
            } else {
...
verifyFileContent(struct J9PortLibrary* portLibrary, const char* testName, char* file1name, char* expectedResult)
{
    ...
    if (strcmp(readBufPtr, expectedResult)) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "j9file_read_text() returned \"%s\" expected \"%s\"\n", readBufPtr, expectedResult);
        goto exit;
    }
#endif /* J9ZOS390 */
...
#define MAX_BUF_SIZE 256
    ...
    retVal = j9process_create(
        (const char**)command, commandLength, env, envSize, dir, options, fdInput, fdOutput, fdError, &processHandle);
    if (retVal != 0) {
        outputErrorMessage(PORTTEST_ERROR_ARGS, "j9process_create returned error code %d.\n", retVal);
        return;
    }
    ...
    if ((options & J9PORT_PROCESS_INHERIT_STDOUT) == J9PORT_PROCESS_INHERIT_STDOUT) {
        ...
        if (MAX_BUF_SIZE < numBytesAvailable) {
            ...
            if (0 <= retVal) {
                buffer[numBytesAvailable] = '\0';
                outputComment(portLibrary,
                    "j9process_read read %d character(s) from the process output stream: \"%s\"\n", retVal, buffer);
                if ((options & J9PORT_PROCESS_REDIRECT_STDERR_TO_STDOUT) == J9PORT_PROCESS_REDIRECT_STDERR_TO_STDOUT) {
                    if (0 != strcmp(buffer, HELLO_STANDARD_OUTNERR)) {
                        outputErrorMessage(PORTTEST_ERROR_ARGS,
                            "j9process_read incorrect message read from J9PORT_PROCESS_STDOUT with option "
                            "J9PORT_PROCESS_REDIRECT_STDERR_TO_STDOUT\n",
                            retVal);
                    }
                } else {
                    if (0 != strcmp(buffer, HELLO_STANDARD_OUT)) {
                        outputErrorMessage(PORTTEST_ERROR_ARGS,
                            "j9process_read incorrect message read from J9PORT_PROCESS_STDOUT\n", retVal);
                    }
                }
            } else {
                outputErrorMessage(PORTTEST_ERROR_ARGS,
            ...
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                "j9process_get_available reports that there are no characters to read from the process output "
                "stream.\n");
        } else {
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "j9process_get_available failed with error code: %d\n", numBytesAvailable);
    ...
    if ((options & J9PORT_PROCESS_INHERIT_STDERR) == J9PORT_PROCESS_INHERIT_STDERR) {
        ...
        if (MAX_BUF_SIZE < numBytesAvailable) {
            ...
            if (0 <= retVal) {
                buffer[numBytesAvailable] = '\0';
                outputComment(portLibrary,
                    "j9process_read read %d character(s) from the process error stream: \"%s\"\n", retVal, buffer);
                if (0 != strcmp(buffer, HELLO_STANDARD_ERR)) {
                    outputErrorMessage(PORTTEST_ERROR_ARGS,
                        "j9process_read incorrect message read from J9PORT_PROCESS_STDERR\n", retVal);
                }
            } else {
                outputErrorMessage(PORTTEST_ERROR_ARGS,
                    "j9process_read trying to read from the error stream returned error code %d.\n", retVal);
            ...
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                "j9process_get_available reports that there are no characters to read from the process error "
                "stream.\n");
        } else {
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "j9process_get_available failed with error code: %d\n", numBytesAvailable);
 ...
 */
I_32 j9process_runTests(struct J9PortLibrary* portLibrary, char* argv0, char* j9process_child)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    int rc = 0;
