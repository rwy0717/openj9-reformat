...
static void getControlDirectoryName(struct J9PortLibrary* portLibrary, char* baseDir)
{
...
static void initializeControlDirectory(struct J9PortLibrary* portLibrary, char* baseDir)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
...
int j9shsem_test1(J9PortLibrary* portLibrary)
{
...
int j9shsem_test2(J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
...
int j9shsem_test3(J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    IDATA rc = 0;
...
int j9shsem_test4(J9PortLibrary* portLibrary, char* argv0)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
...
int j9shsem_test4_child(J9PortLibrary* portLibrary)
{
...
int j9shsem_test5(J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
...
int j9shsem_test6(J9PortLibrary* portLibrary, const char* argv0)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
...
int j9shsem_test6_child(J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    struct j9shsem_handle* sem0 = NULL;
    IDATA rc;
...
static void test7_cleanup(J9PortLibrary* portLibrary, J9PortShSemParameters* params, struct j9shsem_handle* myhandleA,
    struct j9shsem_handle* myhandleB)
{
    ...
    j9str_printf(PORTLIB, mybaseFilePath, J9SH_MAXPATH, "%s%s", params->controlFileDir, params->semName);
    j9file_unlink(mybaseFilePath);
    j9str_printf(PORTLIB, mybaseFilePath, J9SH_MAXPATH, "%s%s_new", params->controlFileDir, params->semName);
    j9file_unlink(mybaseFilePath);
}

static IDATA destroyAndReopenSemaphore(
    J9PortLibrary* portLibrary, char* handleName, J9PortShSemParameters* params, struct j9shsem_handle** handle)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    if (1 == params->deleteBasefile) {
         ...
         * j9shsem_destroy deletes the file and j9file_open with a different name ensures that j9shsem_open doesn't
         * reuse the same inode.
         */
...
int j9shsem_test7(J9PortLibrary* portLibrary)
{
     ...
     * The open operation uses the old file (same inode) to create the same key as the previous
     * destroyAndReopenSemaphore. We will get semaphore 1.
     */
     ...
     */
    /*
     * Note that if deleteBasefile=0 (normal test case) then j9shsem_destroy retains the file that was used to create
     * semaphore 1, so subsequent j9shsem_open will open the existing file and get a reference to the existing
     * semaphore.
     *
     * If deleteBasefile=1 (i.e. to force an error) then j9shsem_destroy deletes the file that was used to create
     * semaphore 1, j9shsem_open creates a new file and a new semaphore (semaphore 1). handleB would point to semaphore
     * 1, but the file would generate a key to semaphore 2.
     */
     ...
     * If deleteBasefile=1, j9shsem_close and j9shsem_open would replace handleB's reference to semaphore 1 with
     * semaphore 2.
     */
...
int j9shsem_runTests(struct J9PortLibrary* portLibrary, char* argv0, char* shsem_child)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    int rc = TEST_PASS;
    char baseDir[J9SH_MAXPATH];
