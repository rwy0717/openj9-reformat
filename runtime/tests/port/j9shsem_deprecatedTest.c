...
static void verifyShsemStats(
    J9PortLibrary* portLibrary, j9shsem_handle* handle, J9PortShsemStatistic* statbuf, const char* testName)
{
    ...
    if (statbuf->ouid != uid) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "Expected owner uid=%zu, owner uid obtained by sem stats=%zu\n", uid, statbuf->ouid);
    }
    ...
    if (statbuf->ogid != gid) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "Expected ownder gid=%zu, owner gid obtained by sem stats=%zu\n", gid, statbuf->ogid);
    }
    ...
    if (statbuf->nsems != handle->nsems + 1) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "Expected number of semaphores in the set=%zu, number of semaphores in the set obtained by sem stats=%zu\n",
...
int j9shsem_deprecated_test1(J9PortLibrary* portLibrary)
{
...
int j9shsem_deprecated_test2(J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
...
int j9shsem_deprecated_test3(J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    IDATA rc = 0;
...
int j9shsem_deprecated_test4(J9PortLibrary* portLibrary, char* argv0)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
...
cleanup:
    ...
    j9shsem_deprecated_open(
        cacheDir, 0, &childrensemaphore, SEMAPHORECHILD, SEMAPHORESETSIZE, 0, J9SHSEM_NO_FLAGS, NULL);
...
int j9shsem_deprecated_test4_child(J9PortLibrary* portLibrary)
{
    ...
    case J9PORT_ERROR_SHSEM_WAIT_FOR_CREATION_MUTEX_TIMEDOUT:
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "Error opening shared semaphore: J9PORT_ERROR_SHSEM_WAIT_FOR_CREATION_MUTEX_TIMEDOUT\n");
        result = 4;
        break;
    default:
        result = 5;
...
int j9shsem_deprecated_test5(J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
...
int j9shsem_deprecated_test6(J9PortLibrary* portLibrary, const char* argv0)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
...
int j9shsem_deprecated_test6_child(J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    struct j9shsem_handle* sem0 = NULL;
    IDATA rc;
 ...
 * at the same time.
 */
int j9shsem_deprecated_test7(J9PortLibrary* portLibrary, char* argv0)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    int rc = 0;
    int i = 0;
    ...
    rc = j9shsem_deprecated_open(
        basedir, 0, &sem0, TEST7_SEMAPHORE_NAME, TEST7_SEMAPHORE_SIZE, 0600, J9SHSEM_NO_FLAGS, NULL);
    ...
    for (i = 0; i < 11; i += 1) {
        ...
        case 7: {
            /*IMPORTANT NOTE: there is a little 'hack' at the bottom of this loop to set the marker back ... otherwise
        ...
        case 10: {
            outputComment(
                PORTLIB, "\tSub Test %d: try with control file ... same as test4 only 1 should create\n", i + 1);
            shouldUnlink = FALSE;
            shouldOpenFail = FALSE;
            doFileWrite = FALSE;
            doDestroy = TRUE;
        ...
        if (doDestroy == TRUE) {
            ...
            j9shsem_deprecated_open(
                basedir, 0, &sem0, TEST7_SEMAPHORE_NAME, TEST7_SEMAPHORE_SIZE, 0, J9SHSEM_NO_FLAGS, NULL);
            if (sem0 != NULL) {
                j9shsem_deprecated_destroy(&sem0);
            }
        }
        ...
        if (i == 8) {
            ...
            if (semctlarg.buf != NULL) {
                ...
                if (semctl(newfileinfo.semid, TEST7_SEMAPHORE_SIZE, IPC_SET, semctlarg) == -1) {
                    outputErrorMessage(
                        PORTTEST_ERROR_ARGS, "\tcould not reset semaphore permissions\n", mybaseFilePath);
                    goto cleanup;
                }
            }
        }
...
cleanup:
    ...
    if (i == 8) {
        ...
        if (semctlarg.buf != NULL) {
            ...
            if (semctl(oldfileinfo.semid, TEST7_SEMAPHORE_SIZE, IPC_SET, semctlarg) == -1) {
                outputErrorMessage(
                    PORTTEST_ERROR_ARGS, "\tcould not recover/clean semaphore permissions\n", mybaseFilePath);
            }
        }
    }
    if (launchSemaphore != -1) {
    ...
    /*Some tests intentionally fail open ... so sem0 may be NULL ... we call an unchecked open just to set sem0 if it
     * can be done*/
...
int j9shsem_deprecated_test7_testchild(J9PortLibrary* portLibrary)
{
    ...
    rc = j9shsem_deprecated_open(
        cacheDir, 0, &childrensemaphore, TEST7_SEMAPHORE_NAME, TEST7_SEMAPHORE_SIZE, 0, J9SHSEM_NO_FLAGS, NULL);
...
#if !(defined(WIN32) || defined(WIN64))
int j9shsem_deprecated_test8(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    if (-1 == rc) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "j9shsem_deprecated_test8 j9shsem_deprecated_handle_stat call failed \n");
        goto cleanup;
    }
...
int j9shsem_deprecated_runTests(struct J9PortLibrary* portLibrary, char* argv0, char* shsem_child)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    int rc = TEST_PASS;
    IDATA rc2;
