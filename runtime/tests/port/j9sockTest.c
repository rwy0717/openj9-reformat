...
static I_32 connectClientToServerNonBlocking(struct J9PortLibrary* portLibrary, const char* testName, char* addrStr,
    U_16 port, j9socket_t* session_client_socket, j9sockaddr_t session_client_addr);
 ...
 */
int j9sock_verify_function_slots(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 */
I_32 startServer(struct J9PortLibrary* portLibrary, const char* testName, char* addrStr, U_16 port,
    j9socket_t* server_socket, j9sockaddr_t server_addr)
{
    ...
    if (0 != j9sock_listen(*server_socket, 1)) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "error binding listening to socket: %s\n", j9error_last_error_message());
        return -1;
    }
 ...
 * @return 0 on succes, non-zero otherwise
 */
I_32 connectClientToServer(struct J9PortLibrary* portLibrary, const char* testName, char* addrStr, U_16 port,
    j9socket_t* session_client_socket, j9sockaddr_t session_client_addr)
{
...
#define MAX_LOOP_TIMES 300
    ...
    if (0 != j9sock_set_nonblocking(*session_client_socket, TRUE)) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "error setting socket to non-blocking: %s\n", j9error_last_error_message());
        return -1;
    }
 ...
 */
int j9sock_test1_basic(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    /* TODO: DEFECT it appears that the address is not optional as win32 does not test for NULL in sock_accept. Fix or
     * DOC */
    ...
    if (0 != j9sock_close(&session_server_socket)) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "error closing session_server_socket_socket: %s\n", j9error_last_error_message());
    }
    if (0 != j9sock_close(&session_client_socket)) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "error closing session_client_socket: %s\n", j9error_last_error_message());
    }
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9sock_test2_fdset_basic(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    if (TRUE == j9sock_fdset_isset(sock1, fdset)) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "error: fdset should not include sock1: %s\n", j9error_last_error_message());
        goto exit;
    }
    ...
    if (FALSE == j9sock_fdset_isset(sock1, fdset)) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "error: fdset should include sock1: %s\n", j9error_last_error_message());
        goto exit;
    }
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9sock_test3_fdset_basic_select(struct J9PortLibrary* portLibrary)
{
    ...
    if (selectRC <= 0) {
        ...
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "j9sock_select should have returned exactly one socket in the read fdset: %s\n",
            j9error_last_error_message());
        goto exit;
    }
    if (TRUE != j9sock_fdset_isset(session_server_socket, writeFdset)) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "session_server_socket should be set %s\n", j9error_last_error_message());
        goto exit;
    }
    if (FALSE != j9sock_fdset_isset(session_client_socket, writeFdset)) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "session_client_socket should NOT be set %s\n", j9error_last_error_message());
        goto exit;
    }

    /* test that a read would block as we haven't written anything yet, we should get a timeout from any call to select
     * at this point */
    ...
    if (selectRC != J9PORT_ERROR_SOCKET_TIMEOUT) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "expected J9PORT_ERROR_SOCKET_TIMEOUT from j9sock_select, instead got: %i, %s\n", selectRC,
            j9error_last_error_message());
        goto exit;
    }
    ...
    /* we've written to the server socket so the client should have something to read, this j9sock_select should return
     * right away */
    ...
    if (TRUE != j9sock_fdset_isset(session_client_socket, readFdset)) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "session_client_socket should be set %s\n", j9error_last_error_message());
        goto exit;
    }

    if (FALSE != j9sock_fdset_isset(session_server_socket, readFdset)) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "session_server_socket should NOT be set %s\n", j9error_last_error_message());
        goto exit;
    }
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9sock_test4_fdset_advanced(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
...
#else
#define NUM_SOCKS                                                                                                   \
    1 /* FD_SETSIZE is much bigger on linux, so much so that it can't handle creating so many file descriptor's for \
         the sockets */
#endif
    ...
    for (i = 0; i < NUM_SOCKS; i++) {
        if (TRUE == j9sock_fdset_isset(sockArray[i], fdset)) {
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "error: fdset should not include sock[%i]: %s\n", i, j9error_last_error_message());
            goto exit;
        }
    }
    ...
    for (i = 0; i < NUM_SOCKS; i++) {
        if (FALSE == j9sock_fdset_isset(sockArray[i], fdset)) {
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "error: fdset should include sock[%i]: %s\n", i, j9error_last_error_message());
            goto exit;
        }
    }
    ...
    if (FALSE == j9sock_fdset_isset(sockArray[0], fdset)) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "error: fdset should include sock[0]: %s\n", j9error_last_error_message());
        goto exit;
    }
    ...
    if (TRUE == j9sock_fdset_isset(sockArray[0], fdset)) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "error: fdset should not include sock[0]: %s\n", j9error_last_error_message());
        goto exit;
    }
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9sock_test5_basic_options(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
...
#define NUM_TEST_FIVE_OPTIONS_TO_TEST NUM_TEST_FIVE_OPTIONS
    ...
    I_32 options[NUM_TEST_FIVE_OPTIONS][2] = { { J9_SOL_SOCKET, J9_SO_REUSEADDR },
        ...
        { J9_IPPROTO_TCP, J9_TCP_NODELAY }, { J9_IPPROTO_IP, J9_IP_MULTICAST_LOOP } };
    ...
    for (i = 0; i < NUM_TEST_FIVE_OPTIONS_TO_TEST; i++) {
        ...
        if (0 != j9sock_setopt_bool(server_socket, options[i][LEVEL], options[i][OPTION], &setValue)) {
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "error setting socket option index %i: %s\n", i, j9error_last_error_message());
            return -1;
        }
        ...
        if (setValue != getValue) {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                "j9sock_getopt_bool() was not returned the value set by j9sock_setopt_bool(): index %i, setValue %i, "
                "getValue %i: %s\n",
        ...
        if (0 != j9sock_bind(server_socket, server_addr)) {
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "error binding socket, index %i: %s, \n", i, j9error_last_error_message());
            return -1;
        }

        if (0 != j9sock_listen(server_socket, 1)) {
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "error listening to socket, index %i: %s\n", i, j9error_last_error_message());
            return -1;
        }

        if (0 != j9sock_close(&server_socket)) {
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "error closing server_socket, index %i: %s\n", i, j9error_last_error_message());
        }
    }
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9sock_test6_nonblocking_connect(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    if (0 != startServer(portLibrary, testName, address, port, &server_socket, server_addr)) {
        ...
        if (0
            != connectClientToServerNonBlocking(
                   portLibrary, testName, address, port, &session_client_socket, session_client_addr)) {
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "error creating/connecting client: %s\n", j9error_last_error_message());
        }
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9sock_test7_nonblocking_read(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    if (0 != j9sock_set_nonblocking(session_client_socket, TRUE)) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "error calling j9sock_set_nonblocking: %s\n", j9error_last_error_message());
        return -1;
    }
    ...
    if (J9PORT_ERROR_SOCKET_WOULDBLOCK != bytesRead) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "j9sock_read on nonblocking socket (with no queued data) did not return J9PORT_ERROR_SOCKET_WOULDBLOCK. "
            "\tInstead it returned: %i. Expected: %i",
            bytesRead, J9PORT_ERROR_SOCKET_WOULDBLOCK);
    }
    ...
    for (;;) {
        ...
        if (bytesRead < 0) {
            if (J9PORT_ERROR_SOCKET_WOULDBLOCK == bytesRead) {
                /* just because we wrote to the server doesn't mean the data was queued on the client in time for this
                 * call to j9sock_read */
                continue;
            } else {
                outputErrorMessage(
                    PORTTEST_ERROR_ARGS, "Error reading from socket. j9sock_read returned: %i", bytesRead);
                goto exit;
            }
        }
        bytesLeft -= bytesRead;
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9sock_test8_select_read_closed_socket(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
...
I_32 startServerForIPv6(struct J9PortLibrary* portLibrary, const char* testName, j9socket_t* server_socket,
    unsigned char* sa, I_32 family, U_32 scopeID, j9addrinfo_struct* res, I_32 index)
{
    ...
    j9error_last_error_message()); return -1;
                    }
...
I_32 j9sock_test10_get_addrinfo(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    if (0
        != j9sock_getaddrinfo_create_hints(
               &hints, (I_16)J9ADDR_FAMILY_UNSPEC, SOCK_STREAM, J9PROTOCOL_FAMILY_UNSPEC, 0)) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "error calling getaddrinfo_create_hints: %s\n", j9error_last_error_message());
        return reportTestExit(portLibrary, testName);
    }
    ...
    if (0 != j9sock_getaddrinfo_length(&res, &length)) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "error calling j9sock_getaddrinfo_length: %s\n", j9error_last_error_message());
        return reportTestExit(portLibrary, testName);
    }
    ...
    for (i = 0; i < length; i++) {
        ...
        if (0 != j9sock_getaddrinfo_family(&res, &family, i)) {
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "error calling j9sock_getaddrinfo_family: %s\n", j9error_last_error_message());
            return reportTestExit(portLibrary, testName);
        }
    ...
    if (0 != j9sock_freeaddrinfo(&res)) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "error calling j9sock_freeaddrinfo: %s\n", j9error_last_error_message());
        return reportTestExit(portLibrary, testName);
    }
#endif
    return reportTestExit(portLibrary, testName);
...
I_32 j9sock_test11_getnameinfo(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
...
#define CANONNAMEFLAG 0x02
    ...
    if (0
        != j9sock_getaddrinfo_create_hints(
               &hints, (I_16)J9ADDR_FAMILY_UNSPEC, SOCK_STREAM, J9PROTOCOL_FAMILY_UNSPEC, CANONNAMEFLAG)) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "error calling getaddrinfo_create_hints: %s\n", j9error_last_error_message());
        return reportTestExit(portLibrary, testName);
    }
    ...
    for (i = 0; i < numOfAddrinfo; i++) {
        ...
        if ((family == J9ADDR_FAMILY_AFINET4)) {
            ...
            if (0 != j9sock_getaddrinfo_name(&res, hostname3, 0)) {
                outputErrorMessage(
                    PORTTEST_ERROR_ARGS, "error calling j9sock_getaddrinfo_name: %s\n", j9error_last_error_message());
                return reportTestExit(portLibrary, testName);
            }
            break;
        }
    ...
    if (0 != j9sock_sockaddr_init6(local_addr, nipAddr, 4, family, 0, 0, 0, server_socket)) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "error calling j9sock_sockaddr_init6: %s\n", j9error_last_error_message());
        return reportTestExit(portLibrary, testName);
    }
    ...
    if ((0 == strncmp(hostname, hostname2, nameLength)) || (0 == strncmp(hostname, "localhost", strlen("localhost")))
        || (0 == strncmp(hostname, "loopback", strlen("loopback")))
        || (0 == strncmp(hostname, "127.0.0.1", strlen("127.0.0.1")))) {
    } else {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "test fails: getnameinfo returns %s, gethostname returns %s\n", hostname, hostname2);
        return reportTestExit(portLibrary, testName);
    }
 ...
 */
I_32 j9sock_runTests(struct J9PortLibrary* portLibrary)
{
