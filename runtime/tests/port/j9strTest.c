 ...
 */
typedef UDATA (*J9STR_VPRINTF_FUNC)(
    struct OMRPortLibrary* portLibrary, char* buf, UDATA bufLen, const char* format, va_list args);
 ...
 */
static UDATA fake_j9str_vprintf(
    struct OMRPortLibrary* portLibrary, char* buf, UDATA bufLen, const char* format, va_list args)
{
    return J9STR_PRIVATE_RETURN_VALUE;
}
 ...
 */
static void validate_j9str_vprintf(struct J9PortLibrary* portLibrary, const char* testName, char* buffer,
    UDATA bufLength, const char* expectedResult, const char* format, va_list args)
{
    ...
    if (strlen(buffer) != rc) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "j9str_vprintf(\"%s\") with \"%s\" returned buffer with length %d, expected %d\n", format, expectedResult,
            strlen(buffer), rc);
    }
 ...
 */
static void validate_j9str_vprintf_with_NULL(struct J9PortLibrary* portLibrary, const char* testName, U_32 bufLength,
    U_32 expectedLength, const char* format, va_list args)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    UDATA rc;
    ...
    if (expectedLength != rc) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "j9str_vprintf(\"%s\") returned %d expected %d\n", format, rc, expectedLength);
    }
}
 ...
 */
static void test_j9str_vprintf(
    struct J9PortLibrary* portLibrary, const char* testName, const char* format, const char* expectedResult, ...)
{
    ...
    va_start(args, expectedResult);
    validate_j9str_vprintf(
        portLibrary, testName, actualResult, strlen(expectedResult) + 1, expectedResult, format, args);
    va_end(args);
    ...
    if (strlen(expectedResult) > 1) {
        ...
        va_start(args, expectedResult);
        validate_j9str_vprintf(
            portLibrary, testName, actualResult, strlen(expectedResult), truncatedExpectedResult, format, args);
        va_end(args);
        ...
        if (strlen(expectedResult) > 2) {
            ...
            va_start(args, expectedResult);
            validate_j9str_vprintf(
                portLibrary, testName, actualResult, strlen(expectedResult) - 1, truncatedExpectedResult, format, args);
            va_end(args);
        }
    }
    ...
    va_start(args, expectedResult);
    validate_j9str_vprintf_with_NULL(
        portLibrary, testName, (U_32)strlen(expectedResult), (U_32)strlen(expectedResult) + 1, format, args);
    va_end(args);
}
 ...
 */
static void test_j9str_vprintfNulChar(struct J9PortLibrary* portLibrary, const char* testName, ...)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    UDATA rc;
 ...
 */
static void test_j9str_ftime(struct J9PortLibrary* portLibrary, const char* testName, char* buf, UDATA bufLen,
 ...
 */
static void test_j9str_tokens(struct J9PortLibrary* portLibrary, const char* testName, char* buf, UDATA bufLen,
    const char* format, struct J9StringTokens* tokens, const char* expectedBuf, UDATA expectedRet)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    UDATA rc;
 ...
 */
int j9str_test0(struct J9PortLibrary* portLibrary)
{
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9str_test1(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9str_test2(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9str_test3(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    test_j9str_ftime(portLibrary, testName, buf, J9STR_BUFFER_SIZE, "%Y %m %b %d %S %pid %uid %job %home %last %seq",
        timeMillis, expected);
    ...
    test_j9str_ftime(
        portLibrary, testName, buf, J9STR_BUFFER_SIZE, "%Y %m %b %d %S %zzz = %%zzz", timeMillis, expected);
    ...
    test_j9str_ftime(
        portLibrary, testName, buf, J9STR_BUFFER_SIZE, "%y,(%Y) %m,(%b) %d XX:%M:%S %", timeMillis, expected);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9str_test4(struct J9PortLibrary* portLibrary)
{
...
#define TEST_OVERFLOW_LEN 8
    ...
    if (NULL == tokens) {
        ...
        /* Test 1: No tokens */
        test_j9str_tokens(portLibrary, testName, buf, TEST_BUF_LEN, "Teststring", tokens, "Teststring", 10);
        /* Test 2: Single token */
        test_j9str_tokens(portLibrary, testName, buf, TEST_BUF_LEN, "%Y", tokens, "2004", 4);
        /* Test 3: End with a token */
        test_j9str_tokens(portLibrary, testName, buf, TEST_BUF_LEN, "Teststring %Y", tokens, "Teststring 2004", 15);
        /* Test 4: Start with a token */
        test_j9str_tokens(portLibrary, testName, buf, TEST_BUF_LEN, "%y Teststring", tokens, "04 Teststring", 13);
        /* Test 5: Many tokens */
        test_j9str_tokens(portLibrary, testName, buf, TEST_BUF_LEN, "%Y/%m/%d %S seconds %longtkn", tokens,
            "2004/02/29 00 seconds Long Token Value", 38);
        /* Test 6: Tokens and strings combined */
        test_j9str_tokens(portLibrary, testName, buf, TEST_BUF_LEN, "Test1 %Y Test2-%m%d%y-%S %longtkn, %longtkn",
            tokens, "Test1 2004 Test2-022904-00 Long Token Value, Long Token Value", 61);
        /* Test 7: %% and end with % */
        test_j9str_tokens(
            portLibrary, testName, buf, TEST_BUF_LEN, "99%% is the same as 99%", tokens, "99% is the same as 99%", 22);
        /* Test 8: Invalid tokens */
        test_j9str_tokens(portLibrary, testName, buf, TEST_BUF_LEN, "%zzz = %%zzz", tokens, "%zzz = %zzz", 11);
        /* Test 9: Excessive % stuff */
        test_j9str_tokens(
            portLibrary, testName, buf, TEST_BUF_LEN, "%Y%%%m%%%d %zzz% %%%%%%", tokens, "2004%02%29 %zzz% %%%", 20);
        ...
        test_j9str_tokens(portLibrary, testName, bufOverflow, TEST_OVERFLOW_LEN, "Teststring", tokens, "Teststr", 11);
        /* Test 13: Test for overflow with an actual short buffer (token) */
        test_j9str_tokens(portLibrary, testName, bufOverflow, TEST_OVERFLOW_LEN, "Test %Y", tokens, "Test 20", 10);
        /* Test 14: Test for token precedence based on length */
        test_j9str_tokens(portLibrary, testName, buf, TEST_BUF_LEN, "%yyy is not %yy is not %y", tokens,
            "yup yup yup is not 04y is not 04", 32);
        /* Test 15: Test the read only mode (should return the required buf len) */
        test_j9str_tokens(portLibrary, testName, NULL, 0, "%yyy is not %yy is not %y", tokens, NULL,
            33); /* 33 because must include \0 */
        /* Test 16: Test an empty token */
        test_j9str_tokens(portLibrary, testName, buf, TEST_BUF_LEN, "x%emptyx == xx", tokens, "xx == xx", 8);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9str_test5(struct J9PortLibrary* portLibrary)
{
#define TEST_BUF_LEN 1024
#define TEST_OVERFLOW_LEN 8
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    (void)j9str_subst_tokens(buf, TEST_BUF_LEN, "%Y/%m/%d %H:%M:%S", tokens);
    j9tty_printf(
        PORTLIB, "\tExpecting local time (in Ottawa, Eastern Standard Time): 2006/02/14 16:30:06 \n", timeMillis);
    ...
    (void)j9str_subst_tokens(buf, TEST_BUF_LEN, "%Y/%m/%d %H:%M:%S", tokens);
    j9tty_printf(
        PORTLIB, "\tExpecting local time (in Ottawa, Eastern Daylight Time): 2006/06/14 17:30:06 \n", timeMillis);
    j9tty_printf(PORTLIB, "\t                                       ... converted to: %s\n", buf);
    ...
    (void)j9str_subst_tokens(buf, TEST_BUF_LEN, "%Y/%m/%d %H:%M:%S", tokens);
    j9tty_printf(
        PORTLIB, "\tExpecting local time (in Ottawa, Eastern Daylight Time): 2006/10/12 17:30:06 \n", timeMillis);
    j9tty_printf(PORTLIB, "\t                                       ... converted to: %s\n", buf);
    ...
    (void)j9str_subst_tokens(buf, TEST_BUF_LEN, "%Y/%m/%d %H:%M:%S", tokens);
    j9tty_printf(
        PORTLIB, "\tExpecting local time (in Ottawa, Eastern Standard Time): 2006/12/11 16:30:06 \n", timeMillis);
    j9tty_printf(PORTLIB, "\t                                       ... converted to: %s\n", buf);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9str_test6(struct J9PortLibrary* portLibrary)
{
#define TEST_BUF_LEN 1024
#define TEST_OVERFLOW_LEN 8
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    tokens = j9str_create_tokens(timeMillis);
    test_j9str_tokens(portLibrary, testName, buf, TEST_BUF_LEN, "%Y/%m/%d %H:%M:%S", tokens, "1970/01/01 00:00:00", 19);
    return reportTestExit(portLibrary, testName);
}
...
static void dumpStringBytes(const char* message, const char* buffer, U_32 length)
{
    U_32 index = 0;
...
static BOOLEAN compareBytes(const char* expected, const char* actual, U_32 length)
{
    U_32 index = 0;
    for (index = 0; index < length; ++index) {
 ...
 * Verify string platform->MUTF8 conversion, basic sanity
 */
int j9str_convPlatToU8(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    convertedStringLength = j9str_convert(
    ...
    /* test string length */
    convertedStringLength
        = j9str_convert(J9STR_CODE_PLATFORM_RAW, J9STR_CODE_MUTF8, platformString, originalStringLength, NULL, 0);
    if (convertedStringLength != expectedStringLength) {
        outputErrorMessage(PORTTEST_ERROR_ARGS, "buffer length wrong.  Expected %d actual %d\n", expectedStringLength,
            convertedStringLength);
    }
...
int j9str_convLongString(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    /* test string length */
    mutf8StringLength
        = j9str_convert(J9STR_CODE_PLATFORM_RAW, J9STR_CODE_MUTF8, longPlatformString, originalStringLength, NULL, 0);
    if (mutf8StringLength < 0) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "Length calculation for modified UTF8 failed, error code %d\n", mutf8StringLength);
    }
    /* do the actual conversion */
    mutf8StringLength = j9str_convert(J9STR_CODE_PLATFORM_RAW, J9STR_CODE_MUTF8, longPlatformString,
    ...
    returnStringLength
        = j9str_convert(J9STR_CODE_MUTF8, J9STR_CODE_PLATFORM_RAW, mutf8Buff, mutf8StringLength, NULL, 0);
    ...
    returnStringLength = j9str_convert(
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9str_convU8ToPlat(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    /* test string length */
    convertedStringLength
        = j9str_convert(J9STR_CODE_MUTF8, J9STR_CODE_PLATFORM_RAW, utf8String, originalStringLength, NULL, 0);
    if (convertedStringLength != expectedStringLength) {
        outputErrorMessage(PORTTEST_ERROR_ARGS, "buffer length wrong.  Expected %d actual %d\n", originalStringLength,
            convertedStringLength);
    }
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9str_convU8ToWide(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    memset(outBuff, 0, sizeof(outBuff));
    convertedStringLength
        = j9str_convert(J9STR_CODE_MUTF8, J9STR_CODE_WIDE, utf8String, originalStringLength, outBuff, sizeof(outBuff));
    if (convertedStringLength != expectedStringLength) {
        outputErrorMessage(PORTTEST_ERROR_ARGS, "buffer length wrong.  Expected %d actual %d\n", expectedStringLength,
            convertedStringLength);
    }
    ...
    /* test string length */
    convertedStringLength = j9str_convert(J9STR_CODE_MUTF8, J9STR_CODE_WIDE, utf8String, originalStringLength, NULL, 0);
    if (convertedStringLength != expectedStringLength) {
        outputErrorMessage(PORTTEST_ERROR_ARGS, "buffer length wrong.  Expected %d actual %d\n", originalStringLength,
            convertedStringLength);
    }
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9str_convWideToU8(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    memset(outBuff, 0, sizeof(outBuff));
    convertedStringLength
        = j9str_convert(J9STR_CODE_WIDE, J9STR_CODE_MUTF8, utf16String, utf16StringLength, outBuff, sizeof(outBuff));
    if (convertedStringLength != expectedStringLength) {
        outputErrorMessage(PORTTEST_ERROR_ARGS, "buffer length wrong.  Expected %d actual %d\n", expectedStringLength,
            convertedStringLength);
    }
    ...
    /* test string length */
    convertedStringLength = j9str_convert(J9STR_CODE_WIDE, J9STR_CODE_MUTF8, utf16String, utf16StringLength, NULL, 0);
    if (convertedStringLength != expectedStringLength) {
        outputErrorMessage(PORTTEST_ERROR_ARGS, "buffer length wrong.  Expected %d actual %d\n", expectedStringLength,
            convertedStringLength);
    }
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9str_convWideToU8_Null(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9str_convU8ToWide_Null(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    convertedStringLength
        = j9str_convert(J9STR_CODE_MUTF8, J9STR_CODE_WIDE, inputString, inputStringLength, outBuff, sizeof(outBuff));
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9str_convU8ToPlat_Null(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9str_convWideToU8NoBom(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9str_convWideToU8NoLittleEndian(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9str_convUtf8ToMUtf8(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    char outBuff[TEST_BUF_LEN];
    ...
    const char utf8Data[] = {
        ...
        0, 0xD0, 0xB0, 0xD0, 0xB1, 0xD0, 0xB2, /* 2-byte UTF-8 */
    ...
    const char mutf8Data[] = { 0xc0, 0x80, /* embedded null */
        ...
        0x44, 0x45, 0x46, 0xef, 0xbf, 0xbd, 0xc0, 0x80 };
    ...
    memset(outBuff, 0, sizeof(outBuff));
    convertedStringLength
        = j9str_convert(J9STR_CODE_UTF8, J9STR_CODE_MUTF8, utf8Data, utf8DataLength, outBuff, sizeof(outBuff));
    if (convertedStringLength != expectedStringLength) {
        outputErrorMessage(PORTTEST_ERROR_ARGS, "buffer length wrong.  Expected %d actual %d\n", expectedStringLength,
            convertedStringLength);
    }
    ...
    /* test string length */
    convertedStringLength = j9str_convert(J9STR_CODE_UTF8, J9STR_CODE_MUTF8, utf8Data, utf8DataLength, NULL, 0);
    if (convertedStringLength != expectedStringLength) {
        outputErrorMessage(PORTTEST_ERROR_ARGS, "buffer length wrong.  Expected %d actual %d\n", expectedStringLength,
            convertedStringLength);
    }
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9str_convRandomUtf8ToMUtf8(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    char outBuff[TEST_BUF_LEN];
    const char* testName = "j9str_convRandomUtf8ToMUtf8";
    const char utf8Data[] = { 0x41, 0xf2, 0x86, 0x84, 0xaf, 0xf0, 0xbc, 0x8f, 0x9b, 0xee, 0xb5, 0xa0, 0xf4, 0x86, 0xbf,
        0xae, 0xf0, 0xbe, 0xa7, 0xac, 0xf4, 0x86, 0x81, 0x84, 0xf0, 0xb3, 0x93, 0x99, 0xf2, 0xb6, 0x91, 0xae, 0xf2,
        0x96, 0xba, 0xa9, 0xf0, 0xbc, 0xb6, 0x92, 0xf1, 0x8c, 0x9c, 0xa4, 0xe4, 0x99, 0xad, 0xf2, 0xb3, 0xa4, 0x95,
        0xf0, 0xa6, 0x85, 0xab, 0xf1, 0xaa, 0xbb, 0xa4, 0xf3, 0x8c, 0x9e, 0xb8, 0xf2, 0x98, 0x9e, 0xb8, 0xf3, 0xb8,
        0x8c, 0xa5, 0xf3, 0x84, 0x8b, 0x8a, 0xf3, 0x94, 0x95, 0xb3, 0xf0, 0x9b, 0xa3, 0xa6, 0xf1, 0xbb, 0x9b, 0xa7,
        0xf1, 0x88, 0x9a, 0x81, 0xf4, 0x83, 0xa8, 0x83, 0xf2, 0xb4, 0x88, 0xb7, 0xf1, 0xa0, 0xa2, 0xb6, 0xf3, 0x97,
        0x88, 0xb7, 0xf2, 0x88, 0x83, 0xb8, 0xf0, 0xa9, 0x9b, 0x9e, 0xf0, 0xa7, 0x92, 0xab, 0x5a };
    const char mutf8Data[] = {  };
    I_32 utf8DataLength = sizeof(utf8Data); /* skip the terminating null */
    I_32 expectedStringLength = sizeof(mutf8Data);
    I_32 convertedStringLength = 0;
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9str_convRoundTrip(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    convertedStringLength = j9str_convert(
        J9STR_CODE_WIDE, J9STR_CODE_MUTF8, (const char*)unicodeData, 2 * numCodePoints, (char*)mutf8Buffer, bufferSize);
    ...
    convertedStringLength = j9str_convert(J9STR_CODE_MUTF8, J9STR_CODE_WIDE, (const char*)mutf8Buffer,
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9str_Latin1ToMutf8(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    char expectedMutf8[] = { 0xc0, 0x80, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0xa, 0xb, 0xc, 0xd, 0xe, 0xf,
        ...
        , 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7 };
    ...
    convertedStringLength
        = j9str_convert(J9STR_CODE_LATIN1, J9STR_CODE_MUTF8, inBuff, originalStringLength, outBuff, sizeof(outBuff));
    if (convertedStringLength != expectedStringLength) {
        outputErrorMessage(PORTTEST_ERROR_ARGS, "buffer length wrong.  Expected %d actual %d\n", expectedStringLength,
            convertedStringLength);
    }
    ...
    /* test string length */
    convertedStringLength = j9str_convert(J9STR_CODE_LATIN1, J9STR_CODE_MUTF8, inBuff, originalStringLength, NULL, 0);
    if (convertedStringLength != expectedStringLength) {
        outputErrorMessage(PORTTEST_ERROR_ARGS, "buffer length wrong.  Expected %d actual %d\n", expectedStringLength,
            convertedStringLength);
    }
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9str_WinacpToMutf8(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    char winacpData[] = { 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x81, 0x91, 0xa1, 0xb1, 0xc1, 0xd1, 0xe1, 0xf1 };
    char expectedMutf8[] = { 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0xc2, 0x81, 0xe2, 0x80, 0x98, 0xc2, 0xa1, 0xc2,
        0xb1, 0xc3, 0x81, 0xc3, 0x91, 0xc3, 0xa1, 0xc3, 0xb1 };
 ...
 *         TEST_FAIL on error
 */
static int printToBuffer(struct J9PortLibrary* portLibrary, BOOLEAN precision, char* buffer, size_t bufferLength,
 ...
 *         TEST_FAIL on error
 */
static int j9str_test_atoe_vsnprintf(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    j9tty_printf(PORTLIB, "\n\tTesting case 7\n");
    rc |= printToBuffer(
        portLibrary, TRUE, buffer0, bufferLength0, expectedOutput1, "%*.*s", 4, 4, nonNullTerminatedString);

    /* the length of inputString is equal to precision but less than min_width */
    j9tty_printf(PORTLIB, "\n\tTesting case 8\n");
    rc |= printToBuffer(
        portLibrary, TRUE, buffer0, bufferLength0, expectedOutput3, "%*.*s", 10, 4, nonNullTerminatedString);
 ...
 */
I_32 j9str_runTests(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    int rc;
