 ...
 */
int j9time_test0(struct J9PortLibrary* portLibrary)
{
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9time_test1(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    if (0 == omrthread_attach_ex(&self, J9THREAD_ATTR_DEFAULT)) {
        ...
        if (timeCheck == timeStart) {
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "j9time_current_time_millis did not change after a half-second forced delay\n");
        }
        if (mtimeStart == mtimeCheck) {
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "j9time_msec_clock did not change after a half-second forced delay\n");
        }
        if (utimeStart == utimeCheck) {
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "j9time_usec_clock did not change after a half-second forced delay\n");
        }
        if (ntimeStart == ntimeCheck) {
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "j9time_nano_time did not change after a half-second forced delay\n");
        }
        ...
        if (0 != j9time_hires_frequency()) {
            ...
            if (hiresTimeCheck == hiresTimeStart) {
                ...
                if (hiresTimeCheck == hiresTimeStart) {
                    outputErrorMessage(
                        PORTTEST_ERROR_ARGS, "j9time_hires_clock has not advanced after a half-second forced delay\n");
                }
            } else {
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9time_test2(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    if (simulatedValue != expectedValue) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "j9time_hires_delta returned %llu expected %llu\n", simulatedValue, expectedValue);
    }
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9time_test3(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    if (1 == j9sysinfo_get_number_CPUs_by_type(J9PORT_CPU_ONLINE)) {
        ...
        for (i = 0; i < J9TIME_REPEAT_TEST; i++) {
            ...
            hiresDeltaAsMillis
                = (U_32)j9time_hires_delta(hiresTimeStart, hiresTimeStop, J9PORT_TIME_DELTA_IN_MILLISECONDS);
            hiresDeltaAsMicros
                = (U_32)j9time_hires_delta(hiresTimeStart, hiresTimeStop, J9PORT_TIME_DELTA_IN_MICROSECONDS);
            mtimeDelta = mtimeStop - mtimeStart;
            utimeDelta = utimeStop - utimeStart;
            timeDelta = newTime - oldTime;

            outputComment(portLibrary, "%10d %10d %10d %10d %12d %12d\n", millires, (I_32)timeDelta, (I_32)mtimeDelta,
                (I_32)utimeDelta, (I_32)hiresDeltaAsMillis, (I_32)hiresDeltaAsMicros);
 ...
 */
static BOOLEAN compareMillis(U_64 value1, U_64 value2, U_64 epsilon)
{
    return (value2 >= value1) && (value2 - value1 < epsilon);
}

static U_64 portGetNanos(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
...
int j9time_test4(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    for (i = 0; i < NUMBER_OF_ITERATIONS; i++) {
        ...
        if (delta > EPSILON) {
            ...
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "j9time_current_time_millis() is not consistent with j9time_hires_clock()!");
            break;
        }
 ...
 */
int j9time_nano_time_direction(struct J9PortLibrary* portLibrary)
{
    omrthread_t self;
    ...
    if (0 == omrthread_attach_ex(&self, J9THREAD_ATTR_DEFAULT)) {
        ...
        if (0 == omrthread_monitor_init(&tds.monitor, 0)) {
            ...
            threads = j9mem_allocate_memory(
                j9timeTestDirectionNumThreads * sizeof(omrthread_t), OMRMEM_CATEGORY_PORT_LIBRARY);

            if (NULL != threads) {
                if (0 == omrthread_monitor_enter(tds.monitor)) {
                    for (i = 0; i < j9timeTestDirectionNumThreads; i++) {
                        IDATA rc = omrthread_create(
                    ...
                    if (tds.failed) {
                        outputErrorMessage(PORTTEST_ERROR_ARGS,
                            "j9time_hires_clock() did not go forward in at least one of the launched threads");
                    }
...
static int J9THREAD_PROC nanoTimeDirectionTest(void* arg)
{
    ...
    for (i = 0; i < numLoops; i++) {
        ...
        if (finish <= start) {
            j9tty_printf(
                PORTLIB, "\tTime did not go forward after omrthread_sleep, start=%llu, finish=%llu\n", start, finish);
            omrthread_monitor_enter(tds->monitor);
            tds->failed = TRUE;
            omrthread_monitor_exit(tds->monitor);
            break;
 ...
 */
static double j9time_test_compute_error_pct(double exp, double actual)
{
    double error = 0.0;
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
static int j9time_test_hires_delta_rounding(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 */
I_32 j9time_runTests(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    int rc;
