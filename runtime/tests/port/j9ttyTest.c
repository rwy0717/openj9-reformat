 ...
 * @ref j9fileText.c::j9file_write_file "j9file_write_file()".  Verify that the TTY operations call the appropriate
 * functions.
 *
 */
#include <string.h>
...
#define J9TTY_STARTUP_PRIVATE_RETURN_VALUE \
    ((I_32)0xDEADBEEF) /** <@internal A known return for @ref fake_j9tty_startup */
 ...
 */
static I_32 fake_j9tty_startup(struct OMRPortLibrary* portLibrary) { return J9TTY_STARTUP_PRIVATE_RETURN_VALUE; }
 ...
 *
 * @ref j9file.c::j9file_vprintf  "j9file_vprintf()" does not return a value, the caller needs a way to know this
 * overriding function has been called.  As a result this function simply replaces the port library function table slot
 * tty_startup with a second overriding function.  The caller then calls @ref j9tty.c::j9tty_startup "j9tty_startup()"
 * to verify they have indeed overridden @ref j9file.c::j9file_vprintf  "j9file_vprintf()" .
 *
 * @param[in] portLibrary The port library
 ...
 */
void fake_j9file_vprintf(struct OMRPortLibrary* portLibrary, IDATA fd, const char* format, va_list args)
{
 ...
 * functions that  should be calling this function.
 *
 * @ref j9tty.c::j9tty_vprintf  "j9tty_vprintf()" does not return a value, the caller needs a way to know this
 * overriding function has been called.  As a result this function simply replaces the port library function table slot
 * tty_startup with a second overriding function.  The caller then calls @ref j9tty.c::j9tty_startup "j9tty_startup()"
 * to verify they have indeed overridden @ref j9tty.c::j9tty_vprintf  "j9tty_vprintf()" .
 *
 * @param[in] portLibrary The port library.
 * @param[in] format The format String.
 * @param[in] args Variable argument list..
 ...
 * @note The caller is reponsible for restoring the tty_startup slot in the port library function table.
 */
void fake_j9tty_vprintf(struct OMRPortLibrary* portLibrary, const char* format, va_list args)
{
    /* No need to save the real value, it is not restored here */
    portLibrary->tty_startup = fake_j9tty_startup;
}
 ...
 * functions that  should be calling this function.
 *
 * @ref j9tty.c::j9tty_err_vprintf  "j9tty_err_vprintf()" does not return a value, the caller needs a way to know this
 * overriding function has been called.  As a result this function simply replaces the port library function table slot
 * tty_startup with a second overriding function.  The caller then calls @ref j9tty.c::j9tty_startup "j9tty_startup()"
 * to verify they have indeed overridden @ref j9tty.c::j9tty_err_vprintf  "j9tty_err_vprintf()" .
 *
 * @param[in] portLibrary The port library.
 * @param[in] format The format String.
 * @param[in] args Variable argument list..
 ...
 * @note The caller is reponsible for restoring the tty_startup slot in the port library function table.
 */
void fake_j9tty_err_vprintf(struct OMRPortLibrary* portLibrary, const char* format, va_list args)
{
    /* No need to save the real value, it is not restored here */
    portLibrary->tty_startup = fake_j9tty_startup;
}
 ...
 */
int j9tty_test0(struct J9PortLibrary* portLibrary)
{
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9tty_test1(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 */
static void j9tty_vprintf_wrapper(struct J9PortLibrary* portLibrary, const char* format, ...)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    va_list args;
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9tty_test2(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9tty_test3(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9tty_test4(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9tty_test5(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9tty_test6(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9tty_test7(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 */
I_32 j9tty_runTests(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    int rc;
