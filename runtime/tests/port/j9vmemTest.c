...
static void verifyMemory(
    struct J9PortLibrary* portLibrary, const char* testName, char* memPtr, UDATA byteAmount, const char* allocName)
{
 ...
 */
int j9vmem_verify_functiontable_slots(struct J9PortLibrary* portLibrary)
{
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9vmem_test_verify_there_are_page_sizes(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9vmem_test1(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    for (i = 0; pageSizes[i] != 0; i++) {
        ...
        memPtr = j9vmem_reserve_memory(0, pageSizes[i], &vmemID,
            J9PORT_VMEM_MEMORY_MODE_READ | J9PORT_VMEM_MEMORY_MODE_WRITE | J9PORT_VMEM_MEMORY_MODE_COMMIT, pageSizes[i],
...
static void setDisclaimVirtualMemory(struct J9PortLibrary* portLibrary, BOOLEAN disclaim)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    if (disclaim) {
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9vmem_bench_write_and_decommit_memory(
    struct J9PortLibrary* portLibrary, UDATA pageSize, UDATA byteAmount, BOOLEAN disclaim, UDATA numIterations)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    if (memPtr == NULL) {
        ...
        j9tty_printf(
            portLibrary, "\treserved 0x%zx bytes with page size 0x%zx at 0x%zx\n", byteAmount, vmemID.pageSize, memPtr);
    }
    ...
    for (i = 0; numIterations != i; i++) {
        ...
        if (NULL == memPtr) {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                "j9vmem_commit_memory returned NULL when trying to commit 0x%zx bytes backed by 0x%zx-byte pages\n",
                byteAmount, pageSize);
            goto exit;
        }
        ...
        if (rc != 0) {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                "j9vmem_decommit_memory returned 0x%zx when trying to decommit 0x%zx bytes backed by 0x%zx-byte "
                "pages\n",
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9vmem_bench_force_overcommit_then_decommit(
    struct J9PortLibrary* portLibrary, UDATA physicalMemorySize, UDATA pageSize, BOOLEAN disclaim)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    deltaMillis = (j9time_nano_time() - startTimeNanos) / (1000 * 1000);
    j9tty_printf(portLibrary,
        "%s pageSize 0x%zx, byteAmount 0x%zx, millis: [force page, decommit pagedout region, write decommitted region] "
        "test: %lli\n",
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9vmem_bench_reserve_write_decommit_and_free_memory(
    struct J9PortLibrary* portLibrary, UDATA pageSize, UDATA byteAmount, BOOLEAN disclaim, UDATA numIterations)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    j9tty_printf(portLibrary,
        "%s pageSize 0x%zx, byteAmount 0x%zx, millis for %u iterations: [reserve/write_to_all/decommit/free] test: "
        "%lli\n",
        disclaim ? "disclaim" : "nodisclaim", pageSize, byteAmount, numIterations, deltaMillis);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9vmem_exhaust_virtual_memory(struct J9PortLibrary* portLibrary, UDATA pageSize, UDATA byteAmount, BOOLEAN disclaim)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9vmem_decommit_memory_test(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 */
int j9vmem_testReserveMemoryEx(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    if (memoryIsAvailable(portLibrary, FALSE)) {
        ...
        outputComment(portLibrary,
            "***Did not find enough memory available on system, not running test j9vmem_testReserveMemoryEx\n");
        return TEST_PASS;
    }
}
 ...
 * See @ref j9vmem_testReserveMemoryEx_impl
 */
int j9vmem_testReserveMemoryExTopDown(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    if (memoryIsAvailable(portLibrary, FALSE)) {
        return j9vmem_testReserveMemoryEx_StandardAndQuickMode(
            portLibrary, "j9vmem_testReserveMemoryExTopDown", J9PORT_VMEM_ALLOC_DIR_TOP_DOWN);
    } else {
        outputComment(portLibrary,
            "***Did not find enough memory available on system, not running test j9vmem_testReserveMemoryExTopDown\n");
        return TEST_PASS;
    }
}
 ...
 * See @ref j9vmem_testReserveMemoryEx_impl
 */
int j9vmem_testReserveMemoryExStrictAddress(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    if (memoryIsAvailable(portLibrary, TRUE)) {
        return j9vmem_testReserveMemoryEx_StandardAndQuickMode(
            portLibrary, "j9vmem_testReserveMemoryExStrictAddress", J9PORT_VMEM_STRICT_ADDRESS);
    } else {
        outputComment(portLibrary,
            "***Did not find enough memory available on system, not running test "
            "j9vmem_testReserveMemoryExStrictAddress\n");
        return TEST_PASS;
    }
}
 ...
 * See @ref j9vmem_testReserveMemoryEx_impl
 */
int j9vmem_testReserveMemoryExStrictPageSize(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    if (memoryIsAvailable(portLibrary, FALSE)) {
        return j9vmem_testReserveMemoryEx_StandardAndQuickMode(
            portLibrary, "j9vmem_testReserveMemoryExStrictPageSize", J9PORT_VMEM_STRICT_PAGE_SIZE);
    } else {
        outputComment(portLibrary,
            "***Did not find enough memory available on system, not running test "
            "j9vmem_testReserveMemoryExStrictPageSize\n");
        return TEST_PASS;
    }
}
 ...
 * See @ref j9vmem_testReserveMemoryEx_impl
 */
int j9vmem_testReserveMemoryEx_zOS_useExtendedPrivateAreaMemory(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    if (memoryIsAvailable(portLibrary, FALSE)) {
        return j9vmem_testReserveMemoryEx_StandardAndQuickMode(portLibrary,
            "j9vmem_testReserveMemoryEx_zOS_useExtendedPrivateAreaMemory", J9PORT_VMEM_ZOS_USE_EXTENDED_PRIVATE_AREA);
    } else {
        outputComment(portLibrary,
            "***Did not find enough memory available on system, not running test "
            "j9vmem_testReserveMemoryEx_zOS_useExtendedPrivateAreaMemory\n");
        return TEST_PASS;
    }
}
 ...
 * See @ref j9vmem_testReserveMemoryEx_impl
 */
int j9vmem_testReserveMemoryExStrictPageSize_zOS_useExtendedPrivateAreaMemory(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    if (memoryIsAvailable(portLibrary, FALSE)) {
        return j9vmem_testReserveMemoryEx_StandardAndQuickMode(portLibrary,
            "j9vmem_testReserveMemoryExStrictPageSize_zOS_useExtendedPrivateAreaMemory",
            J9PORT_VMEM_STRICT_PAGE_SIZE | J9PORT_VMEM_ZOS_USE_EXTENDED_PRIVATE_AREA);
    } else {
        outputComment(portLibrary,
            "***Did not find enough memory available on system, not running test "
            "j9vmem_testReserveMemoryExStrictPageSize_zOS_useExtendedPrivateAreaMemory\n");
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
static int j9vmem_testReserveMemoryEx_StandardAndQuickMode(
    struct J9PortLibrary* portLibrary, const char* testName, UDATA options)
{
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
static int j9vmem_testReserveMemoryEx_impl(struct J9PortLibrary* portLibrary, const char* testName, UDATA options)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
#define NUM_SEGMENTS 3
    ...
    BOOLEAN useExtendedPrivateAreaMemory
        = (J9PORT_VMEM_ZOS_USE_EXTENDED_PRIVATE_AREA == (options & J9PORT_VMEM_ZOS_USE_EXTENDED_PRIVATE_AREA));
    ...
    for (i = 0; pageSizes[i] != 0; i++) {
        for (j = 0; j < NUM_SEGMENTS; j++) {
            ...
            if (memPtr[j] == NULL) {
                if (strictPageSize) {
                    ...
                    if (J9PORT_ERROR_VMEM_INSUFFICENT_RESOURCES == lastErrorNumber) {
                        outputComment(portLibrary, "\tPortable error J9PORT_ERROR_VMEM_INSUFFICENT_RESOURCES...\n");
                        outputComment(
                            portLibrary, "\t\tREBOOT THE MACHINE to free up resources AND TRY THE TEST AGAIN\n");
                    }
                }
            ...
            } else if (useExtendedPrivateAreaMemory
                && (((U_64)memPtr[j] < TWO_GB) && ((U_64)memPtr[j] >= SIXTY_FOUR_GB))) {
                outputErrorMessage(PORTTEST_ERROR_ARGS,
                    "\t useExtendedPrivateAreaMemory flag is set and returned pointer [0x%zx] is outside of range.\n",
                    memPtr[j]);
            ...
            if (NULL != memPtr[j]) {
                ...
                if (vmemID[j].pageSize != j9vmem_get_page_size(&(vmemID[j]))) {
                    outputErrorMessage(PORTTEST_ERROR_ARGS,
                        "vmemID[j].pageSize (0x%zx)!= j9vmem_get_page_size (0x%zx) .\n", vmemID[j].pageSize,
                        j9vmem_get_page_size(vmemID));
                }

                /* are the page types stored and reported correctly? */
                if (vmemID[j].pageFlags != j9vmem_get_page_flags(&(vmemID[j]))) {
                    outputErrorMessage(PORTTEST_ERROR_ARGS,
                        "vmemID[j].pageFlags (0x%zx)!= j9vmem_get_page_flags (0x%zx) .\n", vmemID[j].pageFlags,
                        j9vmem_get_page_flags(vmemID));
                }
 ...
 *
 */
static BOOLEAN memoryIsAvailable(struct J9PortLibrary* portLibrary, BOOLEAN strictAddress)
{
#define NUM_SEGMENTS 3
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
static int j9vmem_testReserveMemoryEx_zOSLargePageBelowBar(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
static int j9vmem_testReserveMemoryExStrictAddress_zOSLargePageBelowBar(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    UDATA* pageSizes = NULL;
    UDATA* pageFlags = NULL;
    ...
    for (i = 0; pageSizes[i] != 0; i++) {
        ...
        if (TWO_GB <= pageSizes[i]) {
             ...
             * If the page size is >= 2G, then attempt to allocate memory with J9PORT_VMEM_STRICT_ADDRESS flag is
             * expected to fail.
             */
            continue;
        }
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
static int j9vmem_testReserveMemoryEx_zOS_useExtendedPrivateAreaMemoryLargePage(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    UDATA* pageSizes = NULL;
    UDATA* pageFlags = NULL;
    ...
    for (i = 0; pageSizes[i] != 0; i++) {
        ...
        if (NULL == memPtr) {
            ...
            if (((U_64)memPtr < TWO_GB) || ((U_64)memPtr >= SIXTY_FOUR_GB)) {
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9vmem_test_commitOutsideOfReservedRange(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    for (i = 0; pageSizes[i] != 0; i++) {
        ...
        if (NULL == memPtr) {
            ...
            if (NULL != commitResult) {
                outputErrorMessage(PORTTEST_ERROR_ARGS,
                    "j9vmem_commit_memory did not return an error when attempting to commit more memory"
                    "than was reserved. \n");
            } else {
                ...
                if (NULL == commitResult) {
                    outputErrorMessage(PORTTEST_ERROR_ARGS,
                        "j9vmem_commit_memory returned an error while attempting to commit reserved memory: %s\n",
                    ...
                    if (0 == rc) {
                        outputErrorMessage(PORTTEST_ERROR_ARGS,
                            "j9vmem_decommit_memory did not return an error when attempting to decommit "
                        ...
                        if (rc != 0) {
                            outputErrorMessage(PORTTEST_ERROR_ARGS,
                                "j9vmem_decommit_memory returned an error when attempting to decommit "
            ...
            if (rc != 0) {
                outputErrorMessage(PORTTEST_ERROR_ARGS,
                    "j9vmem_free_memory returned %d when trying to free 0x%zx bytes backed by 0x%zx-byte pages\n", rc,
                    params.byteAmount, pageSizes[i]);
            }
        }
    }
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9vmem_test_reserveExecutableMemory(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    for (i = 0; pageSizes[i] != 0; i++) {
        j9vmem_vmem_params_init(&params);
        params.byteAmount = pageSizes[i];
        params.mode |= J9PORT_VMEM_MEMORY_MODE_READ | J9PORT_VMEM_MEMORY_MODE_WRITE | J9PORT_VMEM_MEMORY_MODE_EXECUTE
        ...
        if (NULL == memPtr) {
            ...
            *memPtr = 0x4e800020; /* blr instruction (equivalent to RET on x86)*/
            ...
            if (FALSE == isPageSizeSupportedBelowBar(pageSizes[i], pageFlags[i])) {
                ...
                outputComment(
                    PORTLIB, "About to call dynamically created function, pageSize=0x%zx...\n", params.pageSize);
                function();
                outputComment(PORTLIB, "Dynamically created function returned successfully\n");
#if defined(J9ZOS39064)
            }
 ...
 * @return TEST_PASS on success, TEST_FAIL on error
 */
int j9vmem_test_numa(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    if ((detailReturnCode != 0) || (0 == totalNumaNodeCount)) {
        ...
        if (nodeArray == NULL) {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                "j9vmem_free_memory returned NULL when trying to allocate J9MemoryNodeDetail array for NUMA test "
                "(FAIL)\n");
            goto exit;
        }
        ...
        if ((detailReturnCode != rc) || (totalNumaNodeCount != originalNodeCount)) {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                "j9vmem_numa_get_node_details returned different results on second call (return %zd versus %zd, count "
                "%zu versus %zu)\n",
        ...
        for (i = 0; pageSizes[i] != 0; i++) {
            ...
            /* reserve and commit */
#if defined(J9ZOS390)
            /* On z/OS skip this test for newly added large pages as obsolete j9vmem_reserve_memory() does not support
             * them */
            if (TRUE == isNewPageSize(pageSizes[i], pageFlags[i])) {
                continue;
            }
#endif /* J9ZOS390 */
            memPtr = j9vmem_reserve_memory(NULL, reserveSizeInBytes, &vmemID,
                J9PORT_VMEM_MEMORY_MODE_READ | J9PORT_VMEM_MEMORY_MODE_WRITE, pageSize, OMRMEM_CATEGORY_PORT_LIBRARY);
            ...
            if (memPtr != NULL) {
                ...
                for (nodeIndex = 0; nodeIndex < totalNumaNodeCount; nodeIndex++) {
                    if (policyForMemoryNode == nodeArray[nodeIndex].memoryPolicy) {
                        ...
                        if (rc != 0) {
                            outputErrorMessage(PORTTEST_ERROR_ARGS,
                                "j9vmem_numa_set_affinity returned 0x%zx when trying to set affinity for 0x%zx bytes "
                                "at 0x%p to node %zu\n",
                ...
                if (rc != 0) {
                    outputErrorMessage(PORTTEST_ERROR_ARGS,
                        "j9vmem_free_memory returned 0x%zx when trying to free 0x%zx bytes backed by 0x%zx-byte "
                        "pages\n",
...
void verifyFindValidPageSizeOutput(struct J9PortLibrary* portLibrary, char* testName, UDATA pageSizeExpected,
    UDATA pageFlagsExpected, UDATA isSizeSupportedExpected, UDATA pageSizeReturned, UDATA pageFlagsReturned,
    UDATA isSizeSupportedReturned)
{
    ...
    if ((pageSizeExpected == pageSizeReturned) && (pageFlagsExpected == pageFlagsReturned)) {
        ...
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "Expected j9vmem_find_valid_page_size() to return "
...
static int j9vmem_testFindValidPageSize_impl(struct J9PortLibrary* portLibrary, char* testName)
{
#define MODE_NOT_USED 0
    ...
    verifyFindValidPageSizeOutput(portLibrary, testName, expectedPageSize, expectedPageFlags, expectedIsSizeSupported,
...
int j9vmem_testFindValidPageSize(struct J9PortLibrary* portLibrary)
{
...
#define MODE_NOT_USED 0
    ...
    for (i = 0; pageSizes[i] != 0; i++) {
#if defined(J9VM_ENV_DATA64)
        if ((8 * TWO_GB == pageSizes[i]) && (0 != (J9PORT_VMEM_PAGE_FLAG_NOT_USED & pageFlags[i]))) {
            sixteenGBPageSize = TRUE;
        }
#endif /* defined(J9VM_ENV_DATA64) */
        if ((16 * ONE_MB == pageSizes[i]) && (0 != (J9PORT_VMEM_PAGE_FLAG_NOT_USED & pageFlags[i]))) {
            sixteenMBPageSize = TRUE;
        }

        if ((16 * FOUR_KB == pageSizes[i]) && (0 != (J9PORT_VMEM_PAGE_FLAG_NOT_USED & pageFlags[i]))) {
...
static int j9vmem_testFindValidPageSize_impl(struct J9PortLibrary* portLibrary, char* testName)
{
    ...
    for (i = 0; pageSizes[i] != 0; i++) {
#if defined(J9VM_ENV_DATA64)
        if ((TWO_GB == pageSizes[i]) && (0 != (J9PORT_VMEM_PAGE_FLAG_FIXED & pageFlags[i]))) {
            twoGBFixed = TRUE;
        }

        if ((ONE_MB == pageSizes[i]) && (0 != (J9PORT_VMEM_PAGE_FLAG_FIXED & pageFlags[i]))) {
            oneMBFixed = TRUE;
        }
#endif /* defined(J9VM_ENV_DATA64) */
        if ((ONE_MB == pageSizes[i]) && (0 != (J9PORT_VMEM_PAGE_FLAG_PAGEABLE & pageFlags[i]))) {
            oneMBPageable = TRUE;
        }
    }
    ...
    if (0 == defaultLargePageSize) {
        ...
        verifyFindValidPageSizeOutput(portLibrary, testName, expectedPageSize, expectedPageFlags,
 ...
 */
I_32 j9vmem_disclaimPerfTests(struct J9PortLibrary* portLibrary, char* disclaimPerfTestArg, BOOLEAN shouldDisclaim)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    int rc;
    UDATA byteAmount = 0;
    ...
    if (TEST_PASS == rc) {
        ...
        rc |= j9vmem_bench_reserve_write_decommit_and_free_memory(
            portLibrary, pageSize, byteAmount, shouldDisclaim, numIterations);
#if defined(J9VM_ENV_DATA64)
 ...
 */
J9PortVmemParams* getParams(struct J9PortLibrary* portLibrary, J9PortVmemParams* vmemParams)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return 0 on success, -1 on failure
 */
int j9vmem_testOverlappingSegments(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    I_32 result = 0;
    ...
    for (i = 0; i < CYCLES; i++) {
        ...
        outputComment(PORTLIB, "%.16d  %.16X  %.16X  %.16X  %d\n", i, (UDATA)vmemID[i].address,
            (UDATA)vmemID[i].address + vmemID[i].size, vmemID[i].size, keepCycles[i]);
        ...
        for (j = 0; j < i; j++) {
            if (keepCycles[j] >= i) { /* If not already freed */
                if (checkForOverlap(&vmemID[i], &vmemID[j])) {
                    ...
                    outputComment(PORTLIB, "%.16d  %.16X  %.16X  %.16X  %d\n", j, (UDATA)vmemID[j].address,
                        (UDATA)vmemID[j].address + vmemID[j].size, vmemID[j].size, keepCycles[j]);
                    keepCycles[i] = i
 ...
 * @return 0 on success, -1 on failure
 */
I_32 j9vmem_runTests(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    int rc;
...
#if (defined(AIXPPC) || defined(LINUX) || defined(WIN32) || defined(WIN64) || defined(J9ZOS390)) \
    && !(defined(HARDHAT) && defined(LINUXPPC))
