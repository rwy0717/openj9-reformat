...
extern int j9file_runTests(struct J9PortLibrary* portLibrary, char* argv0, char* j9file_child,
    BOOLEAN asynch); /** @see j9fileTest.c::j9file_runTests */
...
extern int j9process_runTests(struct J9PortLibrary* portLibrary, char* argv0,
    char* j9process_child); /** @see j9processTest.c::j9process_runTests */
...
extern int j9hypervisor_runTests(
    struct J9PortLibrary* portLibrary, char* hypervisor, char* hvOptions, BOOLEAN negative);
extern int j9sysinfo_numcpus_runTests(struct J9PortLibrary* portLibrary, UDATA boundTest);
extern int j9vmem_disclaimPerfTests(struct J9PortLibrary* portLibrary, char* disclaimPerfTestArg,
    BOOLEAN shouldDisclaim); /** @see j9vmemTest.c::j9vmem_runTests */
...
static int consumeOption(char** s, char* prefix)
{
...
static UDATA parseForAreasToTests(struct J9PortLibrary* portLibrary, char* options)
{
    ...
    if (serverName[0]) {
        ...
        if (J9PORT_TEST_J9VMEM == (areasToTest & J9PORT_TEST_J9VMEM)) {
            if (NULL != disclaimPerfTestArg) {
                 ...
                 * +/-disclaimPerfTest:<pageSize>,<byteAmount>,<totalMemory>[,<numIterations>] shouldDisclaim is set
                 * from the +/- prefix, numIterations is optional and defaults to 50 */
