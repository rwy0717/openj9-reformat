...
/* The define FD_BIAS is defined in portpriv.h ... however we can't and shouldn't use this here ... so for now we
 * redefine. In the long run fstat will be implemented in the port librart for test 7 so we don't need this.
 */
#ifdef J9ZOS390
...
static UDATA protectedWriter(J9PortLibrary* portLibrary, void* arg)
{
...
static void verifyShmemStats(J9PortLibrary* portLibrary, struct j9shmem_handle* handle,
    struct J9PortShmemStatistic* statbuf, const char* testName)
{
    ...
    if (statbuf->ouid != uid) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "Expected owner uid=%zu, owner uid obtained by shm stats=%zu\n", uid, statbuf->ouid);
    }
    ...
    if (statbuf->ogid != gid) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "Expected ownder gid=%zu, owner gid obtained by shm stats=%zu\n", gid, statbuf->ogid);
    }
...
int j9shmem_test1(J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    rc = j9shmem_open(cacheDir, 0, &myhandle, TESTMEM_NAME, 10, J9SH_SHMEM_PERM_READ_WRITE,
        OMRMEM_CATEGORY_PORT_LIBRARY, J9SHMEM_NO_FLAGS, NULL);
...
int j9shmem_test2(J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    rc = j9shmem_open(cacheDir, 0, &shm0, SHAREDMEMORYA, SHMSIZE, J9SH_SHMEM_PERM_READ_WRITE,
        OMRMEM_CATEGORY_PORT_LIBRARY, J9SHMEM_NO_FLAGS, NULL);
    ...
    rc = j9shmem_open(cacheDir, 0, &shm1, SHAREDMEMORYB, SHMSIZE, J9SH_SHMEM_PERM_READ_WRITE,
        OMRMEM_CATEGORY_PORT_LIBRARY, J9SHMEM_NO_FLAGS, NULL);
    if (J9PORT_ERROR_SHMEM_OPFAILED == rc || J9PORT_ERROR_SHMEM_WAIT_FOR_CREATION_MUTEX_TIMEDOUT == rc) {
...
int j9shmem_test3(J9PortLibrary* portLibrary)
{
    ...
    rc = j9shmem_open(cacheDir, 0, &myhandle, TESTMEM_NAME, 1024, J9SH_SHMEM_PERM_READ_WRITE,
        OMRMEM_CATEGORY_PORT_LIBRARY, J9SHMEM_NO_FLAGS, NULL);
    ...
    rc = j9shmem_open(cacheDir, 0, &myhandle, TESTMEM_NAME, 200, J9SH_SHMEM_PERM_READ_WRITE,
        OMRMEM_CATEGORY_PORT_LIBRARY, J9SHMEM_NO_FLAGS, NULL);
...
int j9shmem_test4(J9PortLibrary* portLibrary, char* argv0)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    rc = j9shmem_open(cacheDir, 0, &mem0, SHAREDMEMORYA, SHMSIZE, J9SH_SHMEM_PERM_READ_WRITE,
        OMRMEM_CATEGORY_PORT_LIBRARY, J9SHMEM_NO_FLAGS, NULL);
    if (J9PORT_ERROR_SHMEM_OPFAILED == rc || J9PORT_ERROR_SHMEM_WAIT_FOR_CREATION_MUTEX_TIMEDOUT == rc) {
...
int j9shmem_test4_child(J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    IDATA rc;
    struct j9shmem_handle* mem0 = NULL;
...
int j9shmem_test5(J9PortLibrary* portLibrary)
{
    ...
    rc = j9shmem_open(cacheDir, 0, &myhandle, TESTMEM_NAME, SHMSIZE, J9SH_SHMEM_PERM_READ_WRITE,
        OMRMEM_CATEGORY_PORT_LIBRARY, J9SHMEM_NO_FLAGS, NULL);
    ...
    if (rc != J9PORT_INFO_SHMEM_CREATED) {
        ...
        if (rc == J9PORT_INFO_SHMEM_OPENED) {
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "\tInfo: We opened an existing 'shared memory' instead of creating a new one.");
        } else {
 ...
 */
int j9shmem_test7(J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    UDATA findHandle = 0;
    char result[100];
...
#define J9SHMEM_TEST7_NTEST 4
    ...
    for (i = 0; i < J9SHMEM_TEST7_NTEST; i++) {
        checked[i] = 0;
        rc = j9shmem_open(cacheDir, 0, &handles[i], name[i], 1024, J9SH_SHMEM_PERM_READ_WRITE,
            OMRMEM_CATEGORY_PORT_LIBRARY, J9SHMEM_NO_FLAGS, NULL);
        if (J9PORT_ERROR_SHMEM_OPFAILED == rc || J9PORT_ERROR_SHMEM_WAIT_FOR_CREATION_MUTEX_TIMEDOUT == rc) {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                "j9shmem_test7 opening shmem area failed for name '%s' and cacheDir '%s'\n", name[i], cacheDir);
            goto cleanup;
        }
    }
    ...
    for (i = 0; i < 3; i++) {
        if (checked[i] == 0) {
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "error: j9shmem_test7 findfirst/findnext missing entry: %s\n", name[i]);
            goto cleanup;
        }
    }
 ...
 * @return TEST_PASS on success, TEST_FAIL on failure
 */
int j9shmem_test8(J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    /* z/OS rounds up the size to 1M boundary, so create a 1M shared memory region to avoid platform specific test
     * condition when verifying the size */
    rc = j9shmem_open(cacheDir, 0, &handle, testName, SHMSIZE, J9SH_SHMEM_PERM_READ_WRITE, OMRMEM_CATEGORY_PORT_LIBRARY,
        J9SHMEM_NO_FLAGS, NULL);
...
int j9shmem_test9(J9PortLibrary* portLibrary, const char* argv0)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    char cacheDir[J9SH_MAXPATH];
    ...
    j9shmem_open(cacheDir, 0, &childrenmemory, J9SHMEM_TEST9_SHMEM_NAME, 10240, J9SH_SHMEM_PERM_READ_WRITE,
        OMRMEM_CATEGORY_PORT_LIBRARY, J9SHMEM_NO_FLAGS, NULL);
...
int j9shmem_test9_child(J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    char cacheDir[J9SH_MAXPATH];
    ...
    rc = j9shmem_open(cacheDir, 0, &childrenmemory, J9SHMEM_TEST9_SHMEM_NAME, 10240, J9SH_SHMEM_PERM_READ_WRITE,
        OMRMEM_CATEGORY_PORT_LIBRARY, J9SHMEM_NO_FLAGS, NULL);
...
int j9shmem_test10(J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    struct j9shmem_handle* mem0 = NULL;
    ...
    rc = j9shmem_open(cacheDir, 0, &mem0, SHAREDMEMORYA, size, J9SH_SHMEM_PERM_READ_WRITE, OMRMEM_CATEGORY_PORT_LIBRARY,
        J9SHMEM_NO_FLAGS, NULL);
 ...
 */
int j9shmem_test11(J9PortLibrary* portLibrary)
{
    ...
    if (j9shmem_protect(cacheDir, 0, regionA, SHMSIZE, 0) == J9PORT_PAGE_PROTECT_NOT_SUPPORTED) {
        if (j9shmem_get_region_granularity(cacheDir, 0, regionA) != 0) {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                "j9shmem_protect() returned J9PORT_PAGE_PROTECT_NOT_SUPPORTED yet, j9shmem_get_region_granularity did "
                "not return 0.)\n");
            goto exit;
        }
    }
 ...
 */
int j9shmem_test12(J9PortLibrary* portLibrary)
{
    ...
    regionCharA = (char*)regionA;
    outputComment(
        PORTLIB, "If SIGSEGV here then shmem_protect is allowing us to write to a region we just set to writeable\n");
    for (i = 0; i < SHMSIZE; i++) {
        regionCharA[i] = (char)((i % 26) + 'a');
        aChar = regionCharA[i];
    }
 ...
 */
int j9shmem_test13(J9PortLibrary* portLibrary)
{
    ...
    if (rc != 0) {
        if (rc == J9PORT_PAGE_PROTECT_NOT_SUPPORTED) {
            ...
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                "j9shmem_protect() failed: lastErrorNumber=%d, lastErrorMessage=%s\n", lastErrorNumber,
                lastErrorMessage);
        }
        goto exit;
    }
    ...
    regionCharA = (char*)regionA;
    outputComment(
        PORTLIB, "Reading from readonly region - If SIGSEGV here then shared memory is not allocated properly\n");
    for (i = 0; i < SHMSIZE; i++) {
        aChar = regionCharA[i];
    }
 ...
 */
int j9shmem_test14(J9PortLibrary* portLibrary)
{
    ...
    rc = j9shmem_open(cacheDir, 0, &shm0, SHAREDMEMORYA, pageSize * 2, J9SH_SHMEM_PERM_READ_WRITE,
        OMRMEM_CATEGORY_PORT_LIBRARY, J9SHMEM_NO_FLAGS, NULL);
    if (J9PORT_ERROR_SHMEM_OPFAILED == rc || J9PORT_ERROR_SHMEM_WAIT_FOR_CREATION_MUTEX_TIMEDOUT == rc) {
        outputErrorMessage(PORTTEST_ERROR_ARGS, "creation of shared memory area 0 failed\n");
        goto exit;
 ...
 * at the same time.
 */
int j9shmem_test15(J9PortLibrary* portLibrary, char* argv0)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    int rc = 0;
    int i = 0;
    ...
    /*This first call to j9shxxx_open is simply to ensure that we don't use any old SysV objects. If j9shxxx_open
     * returns 'OPENED' then we destroy the object.*/
    rc = j9shmem_open(cacheDir, 0, &mem0, TEST15_MEM_NAME, TEST15_MEM_SIZE, 0600, OMRMEM_CATEGORY_PORT_LIBRARY,
        J9SHMEM_NO_FLAGS, NULL);
    if (rc == J9PORT_ERROR_SHMEM_OPFAILED) {
        ...
        if (j9shmem_destroy(cacheDir, 0, &mem0) == -1) {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                "A precondition of this test is that we don't use any old SysV objects: %s\n", cacheDir);
            goto cleanup;
        }
    } else {
    ...
    for (i = 0; i < 12; i += 1) {
        ...
        case 9: {
            /*IMPORTANT NOTE: there is a little 'hack' at the bottom of this loop to set the permissions back ...
        ...
        case 11: {
            outputComment(
                PORTLIB, "\tSub Test %d: try with control file ... same as test9 only 1 should create\n", i + 1);
            shouldUnlink = FALSE;
            shouldOpenFail = FALSE;
            doDestroy = TRUE;
            doFileWrite = FALSE;
        ...
        if (doFileWrite == TRUE) {
            ...
            if (j9file_write(writefd, &newfileinfo, sizeof(j9shmem_controlFileFormat))
                != sizeof(j9shmem_controlFileFormat)) {
            ...
            if (setFileReadOnly == TRUE) {
                if (chmod(mybaseFilePath, S_IRUSR) == -1) {
                    outputErrorMessage(
                        PORTTEST_ERROR_ARGS, "\tcannot chmod 0400 the control file: %s\n", mybaseFilePath);
                    goto cleanup;
                }
            }
        ...
        if (doFileWrite == TRUE) {
            ...
            if (shouldUnlink == TRUE) {
                if (statafter.st_nlink != 0) {
                    outputErrorMessage(
                        PORTTEST_ERROR_ARGS, "\tOur control file was NOT unlinked!: %s\n", mybaseFilePath);
                    goto cleanup;
                }
            } else {
                if (statafter.st_nlink == 0) {
        ...
        if (doDestroy == TRUE) {
            ...
            j9shmem_open(cacheDir, 0, &mem0, TEST15_MEM_NAME, TEST15_MEM_SIZE, 0, OMRMEM_CATEGORY_PORT_LIBRARY,
                J9SHMEM_NO_FLAGS, NULL);
            if (mem0 != NULL) {
                j9shmem_destroy(cacheDir, 0, &mem0);
            }
...
cleanup:
    ...
    /*Some tests intentionally fail open ... so mem0 may be NULL ... we call an unchecked open just to set mem0 if it
     * can be done*/
    j9shmem_open(
        cacheDir, 0, &mem0, TEST15_MEM_NAME, TEST15_MEM_SIZE, 0, OMRMEM_CATEGORY_PORT_LIBRARY, J9SHMEM_NO_FLAGS, NULL);
    if (mem0 != NULL) {
        j9shmem_destroy(cacheDir, 0, &mem0);
    }
    j9file_unlink(mybaseFilePath);
...
int j9shmem_test15_testchild(J9PortLibrary* portLibrary)
{
...
int j9shmem_test16(J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    struct j9shmem_handle* shm0 = NULL;
    IDATA rc = 0, i;
    ...
    if (NULL != shm0) {
        ...
        if (finalBlocks != initialBlocks) {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                "shmem_destroy without attach didn't update the memory block counters as expected. Initial blocks=%zu, "
                "final blocks=%zu\n",
        ...
        if (finalBytes != initialBytes) {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                "shmem_destroy without attach didn't update the memory byte counters as expected. Initial bytes=%zu, "
                "final bytes=%zu\n",
    ...
    if (finalBlocks <= initialBlocks) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "shmem_open & attach didn't update the memory block counters as expected. Initial blocks=%zu, final "
            "blocks=%zu\n",
            initialBlocks, finalBlocks);
    }
    ...
    if (finalBytes < (initialBytes + SHMSIZE)) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "shmem_open & attach didn't update the memory byte counters as expected. Initial bytes=%zu, final "
            "bytes=%zu\n",
            initialBytes, finalBytes);
    }
...
exit:
    ...
    if (NULL != shm0) {
        ...
        if (finalBlocks != initialBlocks) {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                "shmem_destroy didn't update the memory block counters as expected. Initial blocks=%zu, final "
                "blocks=%zu\n",
                initialBlocks, finalBlocks);
        }
        ...
        if (finalBytes != initialBytes) {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                "shmem_destroy didn't update the memory byte counters as expected. Initial bytes=%zu, final "
                "bytes=%zu\n",
                initialBytes, finalBytes);
        }
    }
 ...
 * @return TEST_PASS on success, TEST_FAIL on failure
 */
int j9shmem_test17(J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
...
int j9shmem_runTests(J9PortLibrary* portLibrary, char* argv0, const char* shmem_child)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    int rc = TEST_PASS;
    IDATA rc2;
