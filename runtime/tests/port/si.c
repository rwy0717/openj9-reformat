...
#define J9SYSINFO_TEST3_OSNAME_LENGTH 1024
    ...
    if (NULL == strstr(osName, "Windows")) {
         ...
         */
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "j9sysinfo_get_OS_version returned the default Windows version.\n", 0, 1);
        return reportTestExit(portLibrary, testName);
    }
#endif
...
int j9sysinfo_test_sysinfo_ulimit_iterator(J9PortLibrary* portLibrary)
{
    ...
    while (j9sysinfo_limit_iterator_hasNext(&state)) {
        ...
        if (0 == rc) {
            ...
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "\tj9sysinfo_limit_iterator_next returned: %i when 0 was expected\n", rc);
        }
    }
...
#undef SI_DEBUG
    ...
    while (j9sysinfo_env_iterator_hasNext(&state)) {
        ...
        if (0 == rc) {
            ...
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "\tj9sysinfo_env_iterator_next returned: %i when 0 was expected\n", rc);
            goto done;
            break;
        }
    }
    ...
    if (rc < 0) {
        ...
        j9tty_printf(portLibrary,
            "Should have a buffer of size %x bytes, using one of size %x instead, which will result in a truncated "
            "environment\n",
...
#if defined(SI_DEBUG)
        j9tty_printf(
            portLibrary, "\tsi.c element.nameAndValue @ 0x%p: %s\n", element.nameAndValue, element.nameAndValue);
#endif
...
int j9sysinfo_test_sysinfo_set_limit_ADDRESS_SPACE(J9PortLibrary* portLibrary)
{
    ...
    if (geteuid()) {
        ...
        if (rc == -1) {
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "restoring the original hard limit failed j9sysinfo_set_limit returns -1.\n");
            return reportTestExit(portLibrary, testName);
        }
    }
    ...
    if (rc == -1) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "restoring the original soft limit failed j9sysinfo_set_limit returns -1.\n");
        return reportTestExit(portLibrary, testName);
    }
...
int j9sysinfo_test_sysinfo_set_limit_CORE_FLAGS(J9PortLibrary* portLibrary)
{
    ...
    if (geteuid()) {
        ...
        if (J9PORT_ERROR_FILE_NOPERMISSION == lastErrorNumber) {
            ...
            outputErrorMessage(
                PORTTEST_ERROR_ARGS, "j9sysinfo_set_limit CORE_FLAGS did not fail with the proper error.\n");
            return reportTestExit(portLibrary, testName);
        }
        ...
        rc = j9sysinfo_get_limit(J9PORT_RESOURCE_CORE_FLAGS, &currentLimit);
        if ((rc == J9PORT_LIMIT_LIMITED) && (0 == currentLimit)) {
        ...
        rc = j9sysinfo_get_limit(J9PORT_RESOURCE_CORE_FLAGS, &currentLimit);
        if ((J9PORT_LIMIT_UNLIMITED == rc) && (U_64_MAX == currentLimit)) {
        ...
        if (rc == -1) {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                "restoring the original AIX full core value failed j9sysinfo_set_limit returns -1.\n");
            return reportTestExit(portLibrary, testName);
        }
    }
...
int j9sysinfo_test_sysinfo_get_limit_FILE_DESCRIPTORS(J9PortLibrary* portLibrary)
{
    ...
    if (J9PORT_LIMIT_UNLIMITED == rc) { /* Not an error, just that it is not configured. */
        ...
        if (RLIM_INFINITY == curLimit) {
            outputComment(PORTLIB, "j9sysinfo_get_limit(nofiles) soft limit (unlimited), RLIM_INFINITY.\n");
        } else {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
        ...
        if ((((I_64)curLimit) > 0) && (((I_64)curLimit) <= LLONG_MAX)) {
            ...
            outputErrorMessage(PORTTEST_ERROR_ARGS, "j9sysinfo_get_limit(nofiles) failed: bad limit received!\n");
            return reportTestExit(portLibrary, testName);
        }
    } else { /* The port library failed! */
        outputErrorMessage(PORTTEST_ERROR_ARGS, "j9sysinfo_get_limit(nofiles): failed with error code=%d.\n",
    ...
    if (J9PORT_LIMIT_UNLIMITED == rc) {
        ...
        if (RLIM_INFINITY == maxLimit) {
            outputComment(PORTLIB, "j9sysinfo_get_limit(nofiles) hard limit (unlimited), RLIM_INFINITY.\n");
        } else {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
    ...
    if (curLimit > maxLimit) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "j9sysinfo_get_limit(nofiles): current limit exceeds the hard limit.\n");
        return reportTestExit(portLibrary, testName);
    }
    return reportTestExit(portLibrary, testName);
}
...
int j9sysinfo_test_sysinfo_get_processor_description(J9PortLibrary* portLibrary)
{
    ...
    if (desc.processor < PROCESSOR_X86_UNKNOWN) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "j9sysinfo_test_sysinfo_get_processor_description() processor detection failed.\n");
    }
    if (desc.physicalProcessor < PROCESSOR_X86_UNKNOWN) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "j9sysinfo_test_sysinfo_get_processor_description() physical processor detection failed.\n");
    }
    ...
    for (i = 0; i < J9PORT_SYSINFO_FEATURES_SIZE * 32; i++) {
        ...
        if ((TRUE != feature) && (FALSE != feature)) {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                "j9sysinfo_test_sysinfo_get_processor_description() feature %d detection failed: %d\n", i, feature);
        }
    }
 ...
 */
int j9sysinfo_testMemoryInfo(J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 */
static I_32 onlineProcessorCount(const struct J9ProcessorInfos* procInfo)
{
...
static UDATA cpuBurner(J9PortLibrary* portLibrary, char* myText)
{
    /* burn up CPU */
    UDATA counter = 0;
 ...
 * @return TEST_PASS on success, TEST_FAIL on failure.
 */
int j9sysinfo_testProcessorInfo(J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on failure.
 */
int j9sysinfo_testOnlineProcessorCount(J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on failure.
 */
int j9sysinfo_testTotalProcessorCount(J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
...
int j9sysinfo_test_get_CPU_utilization(J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 */
I_32 j9sysinfo_test_get_tmp1(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    IDATA rc = 0;
 ...
 * Expected result size of buffer required
 */
I_32 j9sysinfo_test_get_tmp2(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    IDATA rc = 0;
 ...
 */
I_32 j9sysinfo_test_get_tmp3(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    const char utf8_file[] = { 0x63, 0x3A, 0x5C, 0xD0, 0xB6, 0xD0, 0xB0, 0xD0, 0xB1, 0xD0, 0xB0, 0x5C, 0x74, 0x65, 0x73,
        0x74, 0x2E, 0x74, 0x78, 0x74, 0x00 };
 ...
 * Expected result size of buffer required
 */
I_32 j9sysinfo_test_get_tmp4(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    IDATA rc = 0;
        ...
        if (NULL == buffer) {
            ...
            if (0 != rc) {
                ...
                if (strcmp(modifiedTmpDir, buffer)) {
                    outputErrorMessage(PORTTEST_ERROR_ARGS,
                        "expected j9sysinfo_get_tmp to return same value as TMPDIR. TMPDIR: %s, returned: %s\n",
    ...
    if (rc <= 0) {
        ...
        if (NULL == buffer) {
            ...
            if (0 != rc) {
                ...
                if (!strcmp(modifiedTmpDir, buffer)) {
                    outputErrorMessage(PORTTEST_ERROR_ARGS,
                        "expected j9sysinfo_get_tmp to ignore TMPDIR. TMPDIR: %s, returned: %s\n", modifiedTmpDir,
                        buffer);
                }
            }
            j9mem_free_memory(buffer);
 ...
 * Expected result size of buffer required
 */
I_32 j9sysinfo_test_get_cwd1(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    IDATA rc = 0;
 ...
 * Expected result size of buffer required
 */
I_32 j9sysinfo_test_get_cwd2(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    IDATA rc = 0;
 ...
 * j9sysinfo_get_cwd returns valid value.
 */
I_32 j9sysinfo_test_get_cwd3(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    IDATA rc = 0;
    ...
    const wchar_t unicode[] = { 0x0063, 0x003A, 0x005C, 0x6211, 0x7684, 0x7236, 0x4EB2, 0x662F, 0x6536, 0x68D2, 0x5B50,
        0x7684, 0x005C, 0x00 };
    const char utf8[] = { 0x63, 0x3A, 0x5C, 0xE6, 0x88, 0x91, 0xE7, 0x9A, 0x84, 0xE7, 0x88, 0xB6, 0xE4, 0xBA, 0xB2,
        0xE6, 0x98, 0xAF, 0xE6, 0x94, 0xB6, 0xE6, 0xA3, 0x92, 0xE5, 0xAD, 0x90, 0xE7, 0x9A, 0x84, 0x5C, 0x00 };
 ...
 * @return TEST_PASS on success, TEST_FAIL on failure
 */
I_32 j9sysinfo_test_get_groups(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    IDATA rc = 0;
    IDATA i;
    ...
    if (-1 != rc) {
        ...
        for (i = 0; i < rc; i++) {
            ...
            if (NULL == grent) {
                if (0 == error) {
                    outputComment(PORTLIB,
                        "\tthis group id is not found in group database (not an error as per getgrgid "
                        "documentation)\n");
                } else {
                    outputErrorMessage(PORTTEST_ERROR_ARGS,
        ...
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "j9sysinfo_get_groups returned %zi\n"
 ...
 * @return TEST_PASS on success, TEST_FAIL on failure
 */
I_32 j9sysinfo_test_get_levels_and_types(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    I_32 rc = 0;
    ...
    for (levelCount = 1; levelCount <= numLevels; ++levelCount) {
        ...
        outputComment(PORTLIB, "DCache size = %d line size = %d, ICache size %d line size = %d\n", DCacheSize,
            DCacheLineSize, ICacheSize, ICacheLineSize);
        if (isUCache && (ICacheSize != DCacheSize)) {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                "DCache size = %d ICache size = %d do not match for unified cache\n", DCacheLineSize, ICacheLineSize,
                rc);
            return reportTestExit(portLibrary, testName);
        }
        if (isUCache && (ICacheLineSize != DCacheLineSize)) {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                "DCache line size = %d ICache line size = %d do not match for unified cache\n", DCacheLineSize,
                ICacheLineSize, rc);
            return reportTestExit(portLibrary, testName);
        }
    }
...
int j9sysinfo_test_get_open_file_count(J9PortLibrary* portLibrary)
{
    ...
    if (J9PORT_LIMIT_UNLIMITED == rc) {
        ...
        if (RLIM_INFINITY == curLimit) {
            outputComment(PORTLIB, "j9sysinfo_get_limit(nofiles): soft limit=RLIM_INFINITY (unlimited).\n");
        } else {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
                "j9sysinfo_get_limit(nofiles): soft limit (unlimited), bad maximum reported %lld.\n", ((I_64)curLimit));
            return reportTestExit(portLibrary, testName);
        ...
        if ((((I_64)curLimit) > 0) && (((I_64)curLimit) <= LLONG_MAX)) {
            ...
            outputErrorMessage(PORTTEST_ERROR_ARGS, "j9sysinfo_get_limit(nofiles) failed: bad limits received!\n");
            return reportTestExit(portLibrary, testName);
        }
    } else { /* The port library failed! */
        outputErrorMessage(PORTTEST_ERROR_ARGS, "j9sysinfo_get_limit(nofiles): failed with error code=%d.\n",
    ...
    if (((I_64)openCount) > ((I_64)curLimit)) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "j9sysinfo_get_open_file_count() failed: reports more files opened than allowed!\n");
        return reportTestExit(portLibrary, testName);
    }
    return reportTestExit(portLibrary, testName);
}
...
I_32 j9sysinfo_test_get_l1dcache_line_size(struct J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    IDATA rc = 0;
 ...
 */
int j9sysinfo_runTests(struct J9PortLibrary* portLibrary, char* argv0)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    int rc = TEST_PASS;
