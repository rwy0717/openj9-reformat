...
int j9sysinfo_numcpus_test1(J9PortLibrary* portLibrary)
{
    ...
    if (0 == numberPhysicalCPUs) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "\tj9sysinfo_get_number_CPUs_by_type(J9PORT_CPU_PHYSICAL) returned invalid value 0.\n");
    }
...
/**
 * Get inital target number of CPUS. Set user-specified to 3, then to 0. Get target number of CPUs. Validate number
...
int j9sysinfo_numcpus_test6(J9PortLibrary* portLibrary)
{
    ...
    if (0 == numberOnlineCPUs) {
        outputErrorMessage(
            PORTTEST_ERROR_ARGS, "\tj9sysinfo_get_number_CPUs_by_type(J9PORT_CPU_ONLINE) returned invalid value 0.\n");
    }
...
int j9sysinfo_numcpus_test7(J9PortLibrary* portLibrary)
{
    ...
    if (numberOnlineCPUs > numberPhysicalCPUs) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "\tj9sysinfo_get_number_CPUs_by_type(J9PORT_CPU_ONLINE) returned value greater than "
            "j9sysinfo_get_number_CPUs_by_type(J9PORT_CPU_PHYSICAL).\n");
    }
...
int j9sysinfo_numcpus_test8(J9PortLibrary* portLibrary)
{
    ...
    if (result > 0) {
        outputErrorMessage(PORTTEST_ERROR_ARGS,
            "\tj9sysinfo_get_number_CPUs_by_type returned non-zero value when given invalid type.\n");
    }
 ...
 */
int j9sysinfo_numcpus_testOnlineProcessorCount(J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 * @return TEST_PASS on success, TEST_FAIL on failure.
 */
int j9sysinfo_numcpus_testTotalProcessorCount(J9PortLibrary* portLibrary)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    ...
    if (0 != rc) {
        ...
        if ((procInfo.totalProcessorCount > 0) && (procInfo.totalProcessorCount == n_cpus_physical)) {
 ...
 *
 * @param[in] portLibrary The port library under test
 * @param[in] userSpecificedBoundCPUs Test argument - indicates whether the process is expected to have CPU
 * restrictions.
 *
 * @return TEST_PASSED on success, TEST_FAILED on failure
 */
int j9sysinfo_numcpus_runTests(struct J9PortLibrary* portLibrary, UDATA userSpecificedBoundCPUs)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    int rc = TEST_PASS;
