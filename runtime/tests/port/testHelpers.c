 ...
 */
void operationNotSupported(struct J9PortLibrary* portLibrary, const char* operationName)
{
 ...
 */
void reportTestEntry(struct J9PortLibrary* portLibrary, const char* testName)
{
 ...
 * @ref engine_getTraceLevel is sufficient
 */
int reportTestExit(struct J9PortLibrary* portLibrary, const char* testName)
{
    if (engine_getTraceLevel()) {
        PORT_ACCESS_FROM_PORT(portLibrary);
 ...
 */
void outputComment(struct J9PortLibrary* portLibrary, const char* format, ...)
{
    if (engine_getTraceLevel()) {
        PORT_ACCESS_FROM_PORT(portLibrary);
        va_list args;
...
static void logTestFailure(struct J9PortLibrary* portLibrary, const char* fileName, I_32 lineNumber,
    const char* testName, I_32 portErrorNumber, const char* portErrorMessage, const char* testErrorMessage)
{
 ...
 * are always displayed.
 */
void outputErrorMessage(struct J9PortLibrary* portLibrary, const char* fileName, I_32 lineNumber, const char* testName,
    const char* format, ...)
{
 ...
 */
void HEADING(struct J9PortLibrary* portLibrary, const char* string)
{
    PORT_ACCESS_FROM_PORT(portLibrary);
    char* dash = "----------------------------------------";
 ...
 */
UDATA
verifyFileExists(struct J9PortLibrary* portLibrary, const char* pltestFileName, I_32 lineNumber, const char* testName,
    const char* fileName)
{
    ...
    if (0 == fileStatRC) {
        if (fileStat.isFile) {
            ...
            outputErrorMessage(
                PORTLIB, pltestFileName, lineNumber, testName, "\tfile: %s does not exist!\n", -1, fileName);
            return -1;
        }
    } else {
        /* error in file_stat */
