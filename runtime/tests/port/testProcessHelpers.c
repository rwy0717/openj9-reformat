...
IDATA openLaunchSemaphore(J9PortLibrary* portLibrary, const char* name, UDATA nProcess) { return -1; }
IDATA ReleaseLaunchSemaphore(J9PortLibrary* portLibrary, IDATA semaphore, UDATA nProcess) { return -1; }
IDATA WaitForLaunchSemaphore(J9PortLibrary* portLibrary, IDATA semaphore) { return -1; }
IDATA CloseLaunchSemaphore(J9PortLibrary* portLibrary, IDATA semaphore) { return -1; }
    ...
    J9PortLibrary* portLibrary, const char* testname, const char* argv0, const char* options)
{
    ...
    if (retVal != 0) {
        ...
        if (NULL == errMsg) {
            j9tty_printf(portLibrary,
                "%s: launchChildProcess: j9sysinfo_get_executable_name failed!\n\tportableErrno = %d\n", testname,
                portableErrno);
        } else {
            j9tty_printf(portLibrary,
    ...
    retVal = j9process_create((const char**)command, commandLength, env, envSize, dir, exeoptions, J9PORT_INVALID_FD,
 ...
 *
 * Terminates the process if j9process_isComplete() does not indicate the process has completed before the timeout has
 * elapsed
 *
 ...
 *
 * @return -1 if j9process_get_available() reports there are no bytes available, otherwise, the return vale from
 *j9process_read();
 */
IDATA
readFromProcess(J9PortLibrary* portLibrary, char* testName, J9ProcessHandle processHandle, UDATA stream, char* buffer,
    UDATA lenBytesBuffer)
{
...
waitForTestProcess(J9PortLibrary* portLibrary, J9ProcessHandle processHandle)
{
    ...
    if (0 != j9process_waitfor(processHandle)) {
        ...
        if (NULL == errMsg) {
            j9tty_printf(
                portLibrary, "waitForTestProcess: j9process_waitfor() failed\n\tportableErrno = %d\n", portableErrno);
        } else {
            j9tty_printf(portLibrary,
    ...
    if (0 != j9process_close(&processHandle, 0)) {
        ...
        if (NULL == errMsg) {
            j9tty_printf(
                portLibrary, "waitForTestProcess: j9process_close() failed\n\tportableErrno = %d\n", portableErrno);
        } else {
            j9tty_printf(portLibrary,
