...
static void myAbortHook(void) { fprintf(stderr, "myAbortHook called. JVM terminated abnormally\n"); }

/* Example JVM exit hook function */
static void myExitHook(int status) { fprintf(stdout, "myExitHook called with status %d\n", status); }

static void printUsage(void)
{
    int i;
    fprintf(stderr,
        "Usage: glauncher jvmlibpath [-genlauncher-specific options] [-java options] class [args...]\n\n"
