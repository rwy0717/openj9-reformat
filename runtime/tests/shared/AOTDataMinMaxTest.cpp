...
struct minmaxStruct AOTminmaxvalue[] = {
    ...
    /* This should fail when storing last object as cumulative size overshoot cache freesize, minAOT sets back to
       cachesize */
...
AOTDataMinMaxTest::openTestCache(I_32 cacheSize, I_32 minaot, I_32 maxaot)
{
    ...
    cacheDetails = (J9SharedClassPreinitConfig*)j9mem_allocate_memory(
    ...
    rc = OpenCacheHelper::openTestCache(cacheType, cacheSize, NULL, true, NULL, NULL, NULL, extraRuntimeFlags,
        unsetRuntimeFlags, UnitTest::MINMAX_TEST, extraVerboseFlags, "openTestCache", startupWillFail, true,
        cacheDetails, sharedclassconfig);
...
void AOTDataMinMaxTest::closeTestCache(bool freeCache)
{
...
AOTDataMinMaxTest::StoreAOTDataFailureOnMinMaxTest(IDATA iteration)
{
    ...
    for (IDATA i = 0; i < NUM_DATA_OBJECTS; i++) {
        UnitTest::unitTest = UnitTest::NO_TEST;
        storedMethod
            = vm->sharedClassConfig->storeCompiledMethod(currentThread, (const J9ROMMethod*)dataList[i].romMethod,
                dataList[i].data, dataList[i].dataSize, dataList[i].code, dataList[i].codeSize, FALSE);
        ...
        case J9SHR_RESOURCE_STORE_FULL:
            if ((FAIL != rc) && (descriptor.aotBytes <= (U_32)descriptor.maxAOT)
                && (AOTminmaxvalue[iteration].failingIndex <= i)) {
                INFOPRINTF5("Test passed with aotBytes=%d minAOT=%d maxAOT=%d failingIndex=%d for data index: %d",
                    descriptor.aotBytes, descriptor.minAOT, descriptor.maxAOT, AOTminmaxvalue[iteration].failingIndex,
                    i);
            } else {
                ERRPRINTF4("Failed with aotBytes=%d maxAOT=%d failingIndex=%d for data index: %d", descriptor.aotBytes,
                    descriptor.maxAOT, AOTminmaxvalue[i].failingIndex, i);
                rc = FAIL;
            }
            break;
...
testAOTDataMinMax(J9JavaVM* vm)
{
    ...
    for (IDATA i = 0; i < numtests; i++) {
        ...
        INFOPRINTF3("Running test with new cache for cache size:%d minAOT:%d maxAOT:%d", SMALL_CACHE_SIZE,
            AOTminmaxvalue[i].min, AOTminmaxvalue[i].max);
