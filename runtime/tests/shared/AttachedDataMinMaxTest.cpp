...
struct minmaxStruct JITminmaxvalue[] = {
    ...
    /* This should fail when storing last object as cumulative size overshoot cache freesize, minJIT sets back to
       cachesize */
...
void AttachedDataMinMaxTest::closeTestCache(bool freeCache)
{
...
void AttachedDataMinMaxTest::setType(U_16 type)
{
    for (IDATA i = 0; i < NUM_DATA_OBJECTS; i++) {
...
AttachedDataMinMaxTest::StoreAttachedDataFailureOnMinMaxTest(IDATA iteration, U_16 attachedDataType)
{
    ...
    for (IDATA i = 0; i < NUM_DATA_OBJECTS; i++) {
        ...
        case 0:
            INFOPRINTF2(
                "j9shr_storeAttachedData successfully stored data for data index %d with return code = %d", i, rV);
            break;
        case J9SHR_RESOURCE_STORE_FULL: {
            ...
            if ((FAIL != rc) && (attachedDataBytes <= (U_32)descriptor.maxJIT)
                && (JITminmaxvalue[iteration].failingIndex <= i)) {
                INFOPRINTF6("Test passed with %s=%d minJIT=%d maxJIT=%d failingIndex=%d for data index: %d",
                    attachedDataTypeName, descriptor.jitProfileDataBytes, descriptor.minJIT, descriptor.maxJIT,
                    JITminmaxvalue[iteration].failingIndex, i);
            } else {
                ERRPRINTF5("Failed with %s=%d maxJIT=%d failingIndex=%d for data index: %d", attachedDataTypeName,
                    attachedDataBytes, descriptor.maxJIT, JITminmaxvalue[i].failingIndex, i);
                rc = FAIL;
            }
        } break;
...
testAttachedDataMinMax(J9JavaVM* vm, U_16 attachedDataType)
{
    ...
    for (IDATA i = 0; i < numtests; i++) {
        ...
        INFOPRINTF3("Running test with new cache for cache size:%d minJIT:%d maxJIT:%d", SMALL_CACHE_SIZE,
            JITminmaxvalue[i].min, JITminmaxvalue[i].max);
