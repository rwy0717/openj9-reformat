...
extern "C" {
IDATA createThread(
    J9JavaVM* vm, omrthread_t* osThread, J9VMThread** vmThread, omrthread_entrypoint_t entryPoint, void* entryArg);
...
private:
    ...
    IDATA FindAttachedData(J9JavaVM* vm, const void* addressInCache, J9SharedDataDescriptor* expectedData,
        bool expectCorruptData, I_32 expectedCorruptOffset, const char* caller);
...
public:
    ...
    IDATA openTestCache(J9JavaVM* vm, BlockPtr preallocatedCache, UDATA preallocatedCacheSize, I_32 cacheType,
        U_64 extraRunTimeFlag, U_64 unsetTheseRunTimeFlag);
...
bool AttachedDataTest::isThreadSuspended(void) { return ((osThread->flags & J9THREAD_FLAG_SUSPENDED) ? true : false); }

J9ROMMethod* AttachedDataTest::addMethodToCache(J9JavaVM* vm)
{
    ...
    SCStoreTransaction transaction(currentThread, (J9ClassLoader*)vm->systemClassLoader, 0, J9SHR_LOADTYPE_NORMAL,
        J9UTF8_LENGTH(romclassName), (U_8*)J9UTF8_DATA(romclassName), false);
...
IDATA
AttachedDataTest::openTestCache(J9JavaVM* vm, BlockPtr preallocatedCache, UDATA preallocatedCacheSize, I_32 cacheType,
    U_64 extraRunTimeFlag, U_64 unsetTheseRunTimeFlag)
{
    void* memory;
    IDATA rc = 0;
    ...
    classMemorySegments = vm->internalVMFunctions->allocateMemorySegmentListWithFlags(
    ...
    sharedClassConfig->cacheDescriptorList
        = (J9SharedClassCacheDescriptor*)((UDATA)sharedClassConfig + sizeof(J9SharedClassConfig));
    ...
    sharedClassConfig->verboseFlags
        |= (J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_JITDATA | J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT);
    ...
    if (NULL == preallocatedCache) {
        if (NULL == cacheMemory) {
             ...
             */
            cacheAllocated
                = (BlockPtr)j9mem_allocate_memory(piConfig->sharedClassCacheSize + osPageSize, J9MEM_CATEGORY_CLASSES);
    ...
    rc = cacheMap->startup(
        currentThread, piConfig, "attacheddatacache", NULL, J9SH_DIRPERM_ABSENT, cacheMemory, &cacheHasIntegrity);
    if (0 != rc) {
...
AttachedDataTest::closeTestCache(J9JavaVM* vm, bool freeCache)
{
    ...
    if ((true == freeCache) && (NULL != cacheAllocated)) {
         ...
         */
        j9mmap_protect(
            cacheMemory, piConfig->sharedClassCacheSize, (J9PORT_PAGE_PROTECT_READ | J9PORT_PAGE_PROTECT_WRITE));
        j9mem_free_memory(cacheAllocated);
        cacheAllocated = NULL;
        cacheMemory = NULL;
    }
...
void AttachedDataTest::populateAttachedDataList(J9JavaVM* vm, SharedAttachedData* dataList)
{
    I_32 i;
    PORT_ACCESS_FROM_JAVAVM(vm);
...
AttachedDataTest::initializeAttachedData(J9JavaVM* vm)
{
    ...
    nextValue = 0;
    dataList = (SharedAttachedData*)j9mem_allocate_memory(
        sizeof(SharedAttachedData) * NUM_DATA_OBJECTS, J9MEM_CATEGORY_CLASSES);
    if (NULL == dataList) {
...
AttachedDataTest::StoreAttachedDataSuccess(J9JavaVM* vm)
{
    ...
    for (i = 0; i < NUM_DATA_OBJECTS; i++) {
        rV = (UDATA)vm->sharedClassConfig->storeAttachedData(
            currentThread, dataList[i].keyAddress, &dataList[i].data, false);
#if defined(J9SHR_CACHELET_SUPPORT)
        if (J9SHR_RESOURCE_STORE_ERROR != rV) {
            ERRPRINTF2("j9shr_storeAttachedData returned incorrect error code for realtime cache, expected error code: "
                       "%d, returned: %d",
                J9SHR_RESOURCE_STORE_ERROR, rV);
            rc = FAIL;
        } else {
        ...
        if (true == readOnlyCache) {
            if (J9SHR_RESOURCE_STORE_ERROR != rV) {
                ERRPRINTF2(
                    "j9shr_storeAttachedData returned incorrect code for read only cache, rV: %d for data index: %d\n",
                    rV, i);
                rc = FAIL;
            } else {
                INFOPRINTF2("j9shr_storeAttachedData returned correct error code for read only cache, rV: %d for data "
                            "index: %d\n",
                    rV, i);
                rc = PASS;
            }
...
AttachedDataTest::StoreAttachedDataFailure(J9JavaVM* vm)
{
    ...
    UDATA rv
        = vm->sharedClassConfig->storeAttachedData(currentThread, attachedData.keyAddress, &attachedData.data, false);
    if (true == readOnlyCache) {
        if (J9SHR_RESOURCE_STORE_ERROR != rv) {
            ERRPRINTF2("j9shr_storeAttachedData did not return correct error code for read only cache, expected error "
                       "code: %d, returned: %d",
                J9SHR_RESOURCE_STORE_ERROR, rv);
            rc = FAIL;
            goto _exit;
        ...
        ERRPRINTF2("j9shr_storeAttachedData did not return correct error code, expected error code: %d, returned: %d",
            J9SHR_RESOURCE_STORE_EXISTS, rv);
        rc = FAIL;
        goto _exit;
    }
    ...
    if (true == readOnlyCache) {
        ...
        ERRPRINTF2("j9shr_storeAttachedData did not return correct error code, expected error code: %d, returned: %d",
            J9SHR_RESOURCE_PARAMETER_ERROR, rv);
        rc = FAIL;
        goto _exit;
    }
...
IDATA
AttachedDataTest::FindAttachedData(J9JavaVM* vm, const void* addressInCache, J9SharedDataDescriptor* expectedData,
    bool expectCorruptData, I_32 expectedCorruptOffset, const char* caller)
{
    IDATA rc = PASS;
    J9SharedDataDescriptor data;
    ...
    if (J9SHR_RESOURCE_STORE_ERROR != rc) {
        ERRPRINTF2("FindAttachedData: j9shr_findAttachedData returned incorrect error code for realtime cache, "
                   "expected error code: %d, returned: %d",
            J9SHR_RESOURCE_STORE_ERROR, rc);
        rc = FAIL;
    } else {
    ...
    default:
        if (true == expectCorruptData) {
            if (-1 != corruptOffset) {
                if (NULL != (U_8*)rc) {
                    ERRPRINTF1(
                        "%s: j9shr_findAttachedData successfully found corrupt data but did not return NULL", caller);
                    rc = FAIL;
                } else {
                    if (corruptOffset == expectedCorruptOffset) {
                        ...
                        ERRPRINTF3("%s: j9shr_findAttachedData successfully found corrupt data but corruptOffset is "
                                   "incorrect. Expected = %d, got = %d",
                            caller, expectedCorruptOffset, corruptOffset);
                        rc = FAIL;
                    }
                }
            ...
            if (-1 != corruptOffset) {
                ...
                if (NULL != (U_8*)rc) {
                    U_8* contents = (U_8*)rc;
                    if ((0 != memcmp(contents, expectedData->address, expectedData->length))
...
AttachedDataTest::FindAttachedDataFailure(J9JavaVM* vm)
{
    ...
    rc = (IDATA)vm->sharedClassConfig->findAttachedData(
        currentThread, attachedData.keyAddress, &attachedData.data, &corruptOffset);
    if (J9SHR_RESOURCE_PARAMETER_ERROR != rc) {
        ERRPRINTF2("j9shr_findAttachedData did not return correct error code \t expected error code: %d, returned: %d",
            J9SHR_RESOURCE_PARAMETER_ERROR, rc);
        vm->sharedClassConfig->freeAttachedDataDescriptor(currentThread, &attachedData.data);
        rc = FAIL;
        goto _exit;
    } else if (NULL != attachedData.data.address) {
        ERRPRINTF2("j9shr_findAttachedData returned correct error code: %d but data is not NULL: 0x%p",
            J9SHR_RESOURCE_PARAMETER_ERROR, attachedData.data.address);
        vm->sharedClassConfig->freeAttachedDataDescriptor(currentThread, &attachedData.data);
        rc = FAIL;
        goto _exit;
    }
...
AttachedDataTest::FindAttachedDataUpdateCountFailure(J9JavaVM* vm)
{
    ...
    if (J9SHR_RESOURCE_TOO_MANY_UPDATES != rc) {
        ERRPRINTF2("j9shr_findAttachedData did not return correct error code \t expected error code: %d, returned: %d",
            J9SHR_RESOURCE_TOO_MANY_UPDATES, rc);
        vm->sharedClassConfig->freeAttachedDataDescriptor(currentThread, &attachedData.data);
        rc = FAIL;
        goto _exit;
    } else {
...
AttachedDataTest::FindAttachedDataCorruptCountFailure(J9JavaVM* vm)
{
    ...
    if (0 != rc) {
        ERRPRINTF2(
            "j9shr_findAttachedData did not return correct error code \t expected error code: %d, returned: %d", 0, rc);
        vm->sharedClassConfig->freeAttachedDataDescriptor(currentThread, &attachedData.data);
        rc = FAIL;
    } else {
        if (-1 != corruptOffset) {
...
AttachedDataTest::ReplaceAttachedData(J9JavaVM* vm)
{
    ...
    rc = (IDATA)vm->sharedClassConfig->storeAttachedData(
        currentThread, attachedData.keyAddress, &attachedData.data, true);
    if (true == readOnlyCache) {
        if (J9SHR_RESOURCE_STORE_ERROR != rc) {
            ERRPRINTF2("j9shr_storeAttachedData returned incorrect code for read only cache, expected error code: %d, "
                       "returned: %d",
                J9SHR_RESOURCE_STORE_ERROR, rc);
            rc = FAIL;
        } else {
...
AttachedDataTest::UpdateAttachedDataSuccess(J9JavaVM* vm)
{
    ...
    rc = (IDATA)vm->sharedClassConfig->updateAttachedData(
        currentThread, attachedData.keyAddress, updateOffset, &attachedData.data);
#if defined(J9SHR_CACHELET_SUPPORT)
    if (J9SHR_RESOURCE_STORE_ERROR != rc) {
        ERRPRINTF2("j9shr_updateAttachedData returned incorrect code for realtime cache, expected error code: %d, "
                   "returned: %d",
            J9SHR_RESOURCE_STORE_ERROR, rc);
        rc = FAIL;
    } else {
    ...
    if (true == readOnlyCache) {
        if (J9SHR_RESOURCE_STORE_ERROR != rc) {
            ERRPRINTF2("j9shr_updateAttachedData returned incorrect code for read only cache, expected error code: %d, "
                       "returned: %d",
                J9SHR_RESOURCE_STORE_ERROR, rc);
            rc = FAIL;
        } else {
...
AttachedDataTest::UpdateAttachedDataFailure(J9JavaVM* vm)
{
    ...
    if (J9SHR_RESOURCE_STORE_ERROR != rc) {
        ERRPRINTF2("j9shr_updateAttachedData did not return correct error code, expected error code: %d, returned: %d",
            J9SHR_RESOURCE_STORE_ERROR, rc);
        rc = FAIL;
        goto _exit;
    }
    ...
    rc = (IDATA)vm->sharedClassConfig->updateAttachedData(
        currentThread, attachedData.keyAddress, 0, &attachedData.data);
    if (true == readOnlyCache) {
        if (J9SHR_RESOURCE_STORE_ERROR != rc) {
        ...
        if (J9SHR_RESOURCE_PARAMETER_ERROR != rc) {
            ERRPRINTF2(
                "j9shr_updateAttachedData did not return correct error code, expected error code: %d, returned: %d",
                J9SHR_RESOURCE_PARAMETER_ERROR, rc);
            rc = FAIL;
            goto _exit;
        }
...
AttachedDataTest::UpdateAttachedUDATASuccess(J9JavaVM* vm)
{
    ...
    updateOffset
        = ((I_32)dataList[1].data.length / 2 > align) ? (I_32)((dataList[1].data.length / 2) & (~(align - 1))) : 0;

    rc = (IDATA)vm->sharedClassConfig->updateAttachedUDATA(
        currentThread, dataList[1].keyAddress, dataList[1].data.type, updateOffset, updateData);
#if defined(J9SHR_CACHELET_SUPPORT)
    if (J9SHR_RESOURCE_STORE_ERROR != rc) {
        ERRPRINTF2("j9shr_updateAttachedUDATA returned incorrect code for realtime cache, expected error code: %d, "
                   "returned: %d",
            J9SHR_RESOURCE_STORE_ERROR, rc);
        rc = FAIL;
    } else {
    ...
    if (true == readOnlyCache) {
        if (J9SHR_RESOURCE_STORE_ERROR != rc) {
            ERRPRINTF2("j9shr_updateAttachedUDATA returned incorrect code for read only cache, expected error code: "
                       "%d, returned: %d",
                J9SHR_RESOURCE_STORE_ERROR, rc);
            rc = FAIL;
        } else {
    ...
    if ((false == readOnlyCache) && (PASS == rc)) {
        ...
        rc = (IDATA)vm->sharedClassConfig->findAttachedData(
            currentThread, dataList[1].keyAddress, &data, &corruptOffset);
        switch (rc) {
        case 0:
        case J9SHR_RESOURCE_PARAMETER_ERROR:
        case J9SHR_RESOURCE_STORE_ERROR:
...
AttachedDataTest::UpdateAttachedUDATAFailure(J9JavaVM* vm)
{
    ...
    rc = (IDATA)vm->sharedClassConfig->updateAttachedUDATA(
        currentThread, dataList[1].keyAddress, dataList[1].data.type, updateOffset, updateData);
    if (J9SHR_RESOURCE_STORE_ERROR != rc) {
        ERRPRINTF2("j9shr_updateAttachedUDATA did not return correct error code, expected error code: %d, returned: %d",
            J9SHR_RESOURCE_STORE_ERROR, rc);
        rc = FAIL;
        goto _exit;
    }
    ...
    j9tty_printf(PORTLIB, "\t");
    rc = (IDATA)vm->sharedClassConfig->updateAttachedUDATA(
        currentThread, (U_8*)ca, J9SHR_ATTACHED_DATA_TYPE_JITPROFILE, 0, updateData);
    if (true == readOnlyCache) {
        if (J9SHR_RESOURCE_STORE_ERROR != rc) {
            ERRPRINTF1("j9shr_storeAttachedData returned incorrect code for read only cache, rc: %d", rc);
            rc = FAIL;
    ...
    J9JavaVM* vm, omrthread_t* osThread, J9VMThread** vmThread, omrthread_entrypoint_t entryPoint, void* entryArg)
{
    ...
    *vmThread
        = vm->internalVMFunctions->allocateVMThread(vm, *osThread, 0, currentThread->omrVMThread->memorySpace, NULL);
    if (NULL == (*vmThread)) {
...
int J9THREAD_PROC startReader(void* entryArg)
{
    ...
    rc = adt->openTestCache(
        vm, writerCacheMemory, writerCacheSize, J9PORT_SHR_CACHE_TYPE_PERSISTENT, J9SHR_RUNTIMEFLAG_ENABLE_READONLY, 0);
    if (FAIL == rc) {
...
testFindReadOnly(J9JavaVM* vm)
{
     ...
     * It should exit from findAttachedData() with J9SHR_RESOURCE_TOO_MANY_UPDATES as updateCountTryCount exceeds
     * FIND_ATTACHED_DATA_RETRY_COUNT.
     */
    omrthread_resume(adtReader.osThread);
