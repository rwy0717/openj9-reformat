...
J9SharedDataDescriptor INPUTDATA[] = { { (U_8*)data1, strlen(data1), 1, 0 }, { (U_8*)data1, strlen(data1), 2, 0 },
    { (U_8*)data1a, strlen(data1a), 3, 0 }, { (U_8*)data2, strlen(data2), 1, 0 }, { (U_8*)data2, strlen(data2), 1, 0 },
    { (U_8*)data2a, strlen(data2a), 1, 0 }, { (U_8*)data3, strlen(data3), 1, J9SHRDATA_IS_PRIVATE },
    { (U_8*)data3, strlen(data3), 1, J9SHRDATA_IS_PRIVATE }, { (U_8*)data3, strlen(data3), 2, J9SHRDATA_IS_PRIVATE },
    { (U_8*)data4, strlen(data4), 1, J9SHRDATA_IS_PRIVATE }, { (U_8*)data2b, strlen(data2b), 1, J9SHRDATA_IS_PRIVATE },
...
SH_CacheMap* createTestCache(J9JavaVM* vm, UDATA size, char* existingCache, char** resultCache)
{
    ...
    sharedConfig->cacheDescriptorList
        = (J9SharedClassCacheDescriptor*)((UDATA)sharedConfig + sizeof(J9SharedClassConfig));

    J9SharedClassPreinitConfig* sharedpiConfig = (J9SharedClassPreinitConfig*)j9mem_allocate_memory(
    ...
    sharedConfig->runtimeFlags = J9SHR_RUNTIMEFLAG_ENABLE_LOCAL_CACHEING | J9SHR_RUNTIMEFLAG_ENABLE_TIMESTAMP_CHECKS
        | J9SHR_RUNTIMEFLAG_ENABLE_REDUCE_STORE_CONTENTION | J9SHR_RUNTIMEFLAG_ENABLE_BYTECODEFIX;
    ...
    rc = returnVal->startup(
        vm->mainThread, sharedpiConfig, "Root1", NULL, J9SH_DIRPERM_ABSENT, cache, &cacheHasIntegrity);
    if (rc != 0) {
        return NULL;
    }
    *resultCache = cache;
...
static IDATA checkStoreValues(J9JavaVM* vm, U_8* result, UDATA resultShouldBeNULL, U_8* compareTo1, U_8* oldResult,
    UDATA shouldMatchOldResult, UDATA testID)
{
    PORT_ACCESS_FROM_JAVAVM(vm);
...
static IDATA compareDescriptors(J9JavaVM* vm, J9SharedDataDescriptor* compare1, J9SharedDataDescriptor* compare2,
    UDATA testID, const char* descName)
{
...
static IDATA checkFindValues(J9JavaVM* vm, IDATA resultCount, IDATA expectedResultCount,
    J9SharedDataDescriptor* firstEntry, J9Pool* descriptorPool, J9SharedDataDescriptor* expectedDesc1,
    J9SharedDataDescriptor* expectedDesc2, J9SharedDataDescriptor* expectedDesc3, UDATA testID)
{
    PORT_ACCESS_FROM_JAVAVM(vm);
    pool_state state;
    ...
    if (expectedResultCount > 0) {
        ...
        if (descriptorPool) {
            ...
            if (pool_numElements(descriptorPool) != (UDATA)expectedResultCount) {
                j9tty_printf(
                    PORTLIB, "Number of elements in descriptor pools is not as expected for test %d\n", testID);
                return 0;
            }
            ...
            if (expectedDesc3) {
                ...
                for (i = 0; i < expectedResultCount; i++) {
                    if ((i != found1) && (i != found2)
                        && compareDescriptors(vm, walk, expectedDesc3, testID, "poolEntry2")) {
                        found3 = i;
                        break;
                    }
                    walk = (J9SharedDataDescriptor*)pool_nextDo(&state);
            ...
            if (expectedDesc1) {
                if (expectedDesc2) {
                    if (expectedDesc3) {
                        if ((found1 == found2) || (found2 == found3) || (found1 == -1) || (found2 == -1)
                            || (found3 == -1)) {
                            j9tty_printf(PORTLIB,
                                "Unexpected results for pool: found1=%d, found2=%d, found3=%d in test", found1, found2,
                                found3, testID);
                            return 0;
                        }
                    } else {
                        if ((found1 == found2) || (found1 == -1) || (found2 == -1)) {
                            j9tty_printf(PORTLIB, "Unexpected results for pool: found1=%d, found2=%d in test", found1,
                                found2, testID);
                            return 0;
                        }
                    }
...
IDATA checkFindResults(J9JavaVM* vm, struct CachePointers* cachePointers, UDATA testJustRun)
{
    ...
    resultsPool = pool_new(
        sizeof(J9SharedDataDescriptor), 0, 0, 0, J9_GET_CALLSITE(), J9MEM_CATEGORY_CLASSES, POOL_FOR_PORT(PORTLIB));
    ...
    if (testJustRun == ++test) {
        for (i = 1; i < 3; i++) {
            ...
            resCount1
                = testCache->findSharedData(vm->mainThread, key1, strlen(key1), FALSE, FALSE, &firstEntry, resultsPool);
            if (!checkFindValues(
                    vm, resCount1, 2, &firstEntry, resultsPool, &INPUTDATA[0], &INPUTDATA[1], NULL, test)) {
                return 0;
            }
            ...
            pool_clear(resultsPool);
            resCount1
                = testCache->findSharedData(vm->mainThread, key1, strlen(key1), 1, FALSE, &firstEntry, resultsPool);
            if (!checkFindValues(vm, resCount1, 1, &firstEntry, resultsPool, &INPUTDATA[0], NULL, NULL, test)) {
                return 0;
            }
            ...
            pool_clear(resultsPool);
            resCount1
                = testCache->findSharedData(vm->mainThread, key1, strlen(key1), 2, FALSE, &firstEntry, resultsPool);
    ...
    if (testJustRun >= ++test) {
        for (i = 1; i < 3; i++) {
            ...
            resCount1
                = testCache->findSharedData(vm->mainThread, key1, strlen(key1), FALSE, FALSE, &firstEntry, resultsPool);
            if (!checkFindValues(
                    vm, resCount1, 3, &firstEntry, resultsPool, &INPUTDATA[0], &INPUTDATA[1], &INPUTDATA[2], test)) {
                return 0;
            }
            ...
            pool_clear(resultsPool);
            resCount1
                = testCache->findSharedData(vm->mainThread, key1, strlen(key1), 3, FALSE, &firstEntry, resultsPool);
    ...
    if ((testJustRun >= ++test) && (testJustRun < 6)) {
        for (i = 1; i < 3; i++) {
            ...
            resCount1
                = testCache->findSharedData(vm->mainThread, key2, strlen(key2), FALSE, FALSE, &firstEntry, resultsPool);
    ...
    if ((testJustRun >= ++test) && (testJustRun < 9)) {
        for (i = 1; i < 3; i++) {
            ...
            resCount1
                = testCache->findSharedData(vm->mainThread, key3, strlen(key3), FALSE, FALSE, &firstEntry, resultsPool);
            ...
            resCount1
                = testCache->findSharedData(vm->mainThread, key3, strlen(key3), FALSE, TRUE, &firstEntry, resultsPool);
    ...
    if (testJustRun >= ++test) {
        for (i = 1; i < 3; i++) {
            ...
            /* We now have some different data under a different key */
            resCount1
                = testCache->findSharedData(vm->mainThread, key4, strlen(key4), FALSE, TRUE, &firstEntry, resultsPool);
            if (!checkFindValues(vm, resCount1, 1, &firstEntry, resultsPool, &tempDesc, NULL, NULL, test)) {
                return 0;
            }
        }
    ...
    if ((testJustRun >= ++test) && (testJustRun < 12)) {
        for (i = 1; i < 3; i++) {
            ...
            /* We now have some different data under a different key */
            resCount1
                = testCache->findSharedData(vm->mainThread, key2, strlen(key2), FALSE, TRUE, &firstEntry, resultsPool);
    ...
    if ((testJustRun >= ++test) && (testJustRun < 15)) {
        for (i = 1; i < 3; i++) {
            ...
            resCount1
                = testCache->findSharedData(vm->mainThread, key2, strlen(key2), FALSE, TRUE, &firstEntry, resultsPool);
            if (!checkFindValues(
                    vm, resCount1, 3, &firstEntry, resultsPool, &INPUTDATA[5], &tempDesc1, &tempDesc2, test)) {
                return 0;
            }
        }
    }
    ...
    if (testJustRun >= ++test) {
        ...
        for (i = 1; i < 3; i++) {
            ...
            /* We now have some different data under a different key */
            resCount1
                = testCache->findSharedData(vm->mainThread, key5, strlen(key5), FALSE, FALSE, &firstEntry, resultsPool);
    ...
    if (testJustRun >= ++test) {
        for (i = 1; i < 3; i++) {
            ...
            resCount1
                = testCache->findSharedData(vm->mainThread, key6, strlen(key6), FALSE, FALSE, &firstEntry, resultsPool);
    ...
    if (testJustRun >= ++test) {
        ...
        for (i = 1; i < 3; i++) {
            ...
            resCount1
                = testCache->findSharedData(vm->mainThread, key7, strlen(key7), FALSE, FALSE, &firstEntry, resultsPool);
            if (!checkFindValues(vm, resCount1, 1, &firstEntry, resultsPool, &tempDesc, NULL, NULL, test)) {
                return 0;
            }
        }
