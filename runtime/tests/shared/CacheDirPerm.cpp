...
testCacheDirPerm(J9JavaVM* vm)
{
    ...
    for (int i = 0; i <= 15; i++) {
        ...
        if (0 != cacheType) {
            INFOPRINTF6("Test %d: cacheType: %s, useExistingDir: %d, cacheDirPermStr: %s, expectedDirPerm: %d, "
                        "useDefaultDir: %d\n",
            ...
            if (useDefaultDir) {
                ...
                rc = cdp.openTestCache(
                    cacheType, CACHE_SIZE, cdp.cacheName, false, NULL, cdp.cacheDir, cacheDirPermStr, 0, 0);
            }
            if (FAIL == rc) {
            ...
            if ((false == useExistingDir) && (false == useDefaultDir) && (NULL == cacheDirPermStr)) {
                ...
                if (J9SH_PARENTDIRPERM != perm) {
                    ERRPRINTF2(
                        "Incorrect permission on parent directory. Expected: %d, found: %d ", J9SH_PARENTDIRPERM, perm);
                    rc = FAIL;
                    goto _close;
                }
            }
