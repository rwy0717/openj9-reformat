...
public:
    ...
    static UDATA getRequiredConstrBytes(void) { return sizeof(TestOSCache); }

    IDATA releaseWriteLock(UDATA lockid) { return 0; }

    virtual IDATA acquireWriteLock(UDATA lockid) { return 0; }
...
IDATA test1(J9JavaVM* vm)
{
    ...
    if (cacheHelper.openTestCache(J9PORT_SHR_CACHE_TYPE_PERSISTENT, cacheSize, NULL, true, NULL, NULL, NULL,
            protectRunTimeFlags, 0, UnitTest::CACHE_FULL_TEST)
        == FAIL) {
...
IDATA test2(J9JavaVM* vm)
{
    ...
    if (ca->updateSRP != expectedUpdateSRP) {
        ERRPRINTF2("Error: updateSRP is not set to expected value.\n"
                   "current updateSRP: 0x%zx\t expectedUpdateSRP: 0x%zx",
            ca->updateSRP, expectedUpdateSRP);
    }
    ...
    if (myOSCache->protectedLength != pageSizeToUse) {
        ERRPRINTF1("Error: Expected to mprotect 1 page of data. "
                   "Amount of data mprotected: 0x%zx",
            myOSCache->protectedLength);
        rc = FAIL;
        goto done;
    } else {
    ...
    if (TYPE_UNINDEXED_BYTE_DATA != ITEMTYPE(dummyDataItem)) {
        ERRPRINTF2("Error: Did not find expected data type in last metedata item.\n"
                   "Expected type for dummy data = %d, data type found = %d",
            TYPE_UNINDEXED_BYTE_DATA, ITEMTYPE(dummyDataItem));
        rc = FAIL;
        goto done;
    }
    ...
    for (i = 0; i < dummyDataLen; i++) {
        if (J9SHR_DUMMY_DATA_BYTE != dummyData[i]) {
            ERRPRINTF2("Error: Did not find dummy data of expected length. Expected dummy data length: %d bytes, "
                       "found: %d bytes",
                dummyDataLen, i);
            rc = FAIL;
            goto done;
...
IDATA test3(J9JavaVM* vm)
{
    ...
    expectedUpdateSRP = (UDATA)SEGUPDATEPTR(ca) + pageSizeToUse - (2 * sizeof(U_32)) + orphanItemSize
        + (J9SHR_MIN_GAP_BEFORE_METADATA + J9SHR_MIN_DUMMY_DATA_SIZE - sizeof(U_32)) - (UDATA)ca;
    ...
    if (cc->getFreeBlockBytes() <= (I_32)pageSizeToUse) {
        ERRPRINTF2("Error: updateSRP and segmentSRP are not properly set.\n"
            ...
            pageSizeToUse, cc->getFreeBlockBytes());
        rc = FAIL;
        goto done;
    }
    ...
    if (FAIL == cacheHelper.addDummyROMClass(romClassName, romClassSize)) {
        ...
        INFOPRINTF("Added a dummy ROMClass to set free block bytes less than (J9SHR_MIN_GAP_BEFORE_METADATA + "
                   "J9SHR_MIN_DUMMY_DATA_SIZE)");
    }
    ...
    if (cc->getFreeBlockBytes() >= (I_32)(J9SHR_MIN_GAP_BEFORE_METADATA + J9SHR_MIN_DUMMY_DATA_SIZE)) {
        ERRPRINTF2(
            "Error: Expected free bytes to be less than (J9SHR_MIN_GAP_BEFORE_METADATA + J9SHR_MIN_DUMMY_DATA_SIZE)\n"
            "Free bytes available: %d, expected to be < %d",
            cc->getFreeBlockBytes(), (J9SHR_MIN_GAP_BEFORE_METADATA + J9SHR_MIN_DUMMY_DATA_SIZE));
        rc = FAIL;
        goto done;
    }
...
/**
 * This test is for the case when free block bytes is exactly equal to (J9SHR_MIN_GAP_BEFORE_METADATA +
 * J9SHR_MIN_DUMMY_DATA_SIZE). There is no reserved space for AOT/JIT data.
 *
 * Add data to the cache to set up free block bytes equal to (J9SHR_MIN_GAP_BEFORE_METADATA +
 * J9SHR_MIN_DUMMY_DATA_SIZE). At this point verify dummy data is added to the cache and all the runtime cache full
 * flags are set.
 */
IDATA test4(J9JavaVM* vm)
{
    ...
    expectedUpdateSRP = (UDATA)SEGUPDATEPTR(ca) + pageSizeToUse + orphanItemSize
        + (J9SHR_MIN_GAP_BEFORE_METADATA + J9SHR_MIN_DUMMY_DATA_SIZE) - (UDATA)ca;
    ...
    if (freeBlockBytes <= pageSizeToUse) {
        ERRPRINTF2("Error: updateSRP and segmentSRP are not properly set.\n"
                   ...
                   "Free block bytes available: ",
            pageSizeToUse, freeBlockBytes);
        rc = FAIL;
        goto done;
    }
    ...
    if (FAIL == cacheHelper.addDummyROMClass(romClassName, romClassSize)) {
        ...
        INFOPRINTF("Added a dummy ROMClass to set available block bytes equal to (J9SHR_MIN_GAP_BEFORE_METADATA + "
                   "J9SHR_MIN_DUMMY_DATA_SIZE)");
    }

    if (0 == (vm->sharedClassConfig->runtimeFlags & J9SHR_RUNTIMEFLAG_BLOCK_SPACE_FULL)) {
        ERRPRINTF1(
            "Error: Expected block space to be marked full. runtimeFlags: 0x%llx", vm->sharedClassConfig->runtimeFlags);
        rc = FAIL;
        goto done;
    } else {
    ...
    if (freeBlockBytes != J9SHR_MIN_GAP_BEFORE_METADATA) {
        ERRPRINTF2("Error: Expected free block bytes to be equal to (J9SHR_MIN_GAP_BEFORE_METADATA + "
                   "J9SHR_MIN_DUMMY_DATA_SIZE)\n"
                   "Free block bytes available: %d, expected to be %d",
...
IDATA test5(J9JavaVM* vm)
{
    ...
    piConfig = (J9SharedClassPreinitConfig*)j9mem_allocate_memory(
        sizeof(J9SharedClassPreinitConfig), J9MEM_CATEGORY_CLASSES);
    if (NULL == piConfig) {
    ...
    sharedClassConfig = (J9SharedClassConfig*)j9mem_allocate_memory(
    ...
    if (cacheHelper.openTestCache(J9PORT_SHR_CACHE_TYPE_PERSISTENT, cacheSize, NULL, true, NULL, NULL, NULL,
            protectRunTimeFlags, 0, UnitTest::CACHE_FULL_TEST, J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT,
            "openTestCache", false, true, piConfig, sharedClassConfig)
        == FAIL) {
        ERRPRINTF1("Failed to open cache for %s", testName);
        rc = FAIL;
        goto done;
    ...
    itemLen = (U_32)(
        freeBlockBytes - (J9SHR_MIN_GAP_BEFORE_METADATA + ONE_K_BYTES) - (sizeof(ShcItem) + sizeof(ShcItemHdr)));
    cc->initBlockData(&itemPtr, itemLen, TYPE_UNINDEXED_BYTE_DATA);
    ...
    if ((J9SHR_AVAILABLE_SPACE_FULL | J9SHR_BLOCK_SPACE_FULL | J9SHR_JIT_SPACE_FULL)
        != (ca->cacheFullFlags & J9SHR_ALL_CACHE_FULL_BITS)) {
        ERRPRINTF1("Error: Expected (J9SHR_AVAILABLE_SPACE_FULL | J9SHR_BLOCK_SPACE_FULL | J9SHR_JIT_SPACE_FULL) to be "
                   "bet in cache header."
                   "cacheFullFlags: 0x%zx",
            ca->cacheFullFlags);
        rc = FAIL;
        goto done;
    }
    ...
    if (ca->minAOT != cc->getAvailableReservedAOTBytes(vm->mainThread)) {
        ERRPRINTF2("Error: Expected available reserved AOT bytes to be equal to minAOT\n"
                   "minAOT=%d, available reserved AOT bytes=%d",
            ca->minAOT, cc->getAvailableReservedAOTBytes(vm->mainThread));
        cc->exitWriteMutex(vm->mainThread, testName);
        rc = FAIL;
        goto done;
    }
    ...
    result = (BlockPtr)vm->sharedClassConfig->storeCompiledMethod(
        vm->mainThread, (J9ROMMethod*)SEGUPDATEPTR(ca), dataStart, dataLength, NULL, 0, true);
...
IDATA test6(J9JavaVM* vm)
{
    ...
    if ((J9SHR_AVAILABLE_SPACE_FULL | J9SHR_BLOCK_SPACE_FULL | J9SHR_AOT_SPACE_FULL)
        != (ca->cacheFullFlags & J9SHR_ALL_CACHE_FULL_BITS)) {
        ERRPRINTF1("Error: Expected (J9SHR_AVAILABLE_SPACE_FULL| J9SHR_BLOCK_SPACE_FULL | J9SHR_AOT_SPACE_FULL) to be "
                   "bet in cache header."
                   "cacheFullFlags: 0x%zx",
            ca->cacheFullFlags);
        rc = FAIL;
    ...
    if (ca->minJIT != cc->getAvailableReservedJITBytes(vm->mainThread)) {
        ERRPRINTF2("Error: Expected available reserved JIT bytes to be equal to minJIT\n"
                   "minAOT=%d, available reserved JIT bytes=%d",
            ca->minJIT, cc->getAvailableReservedJITBytes(vm->mainThread));
        cc->exitWriteMutex(vm->mainThread, testName);
        rc = FAIL;
        goto done;
    }
...
IDATA test7(J9JavaVM* vm)
{
    ...
    itemLen = (U_32)(
        oldFreeBlockBytes - CC_MIN_SPACE_BEFORE_CACHE_FULL - ONE_K_BYTES - (sizeof(ShcItem) + sizeof(ShcItemHdr)));
    cc->initBlockData(&itemPtr, itemLen, TYPE_UNINDEXED_BYTE_DATA);
 ...
 * 	1) Except J9SHR_MIN_GAP_BEFORE_METADATA + 1K, consume all free block bytes. This also sets the flags for BLOCK
 * space full. Break up of free space: Free block bytes = (J9SHR_MIN_GAP_BEFORE_METADATA + 1K) (not usable to store
 * block data as the flags for BLOCK space full are set). Reserved AOT = TEST7_MIN_AOT_SIZE Reserve JIT =
 * TEST7_MIN_JIT_SIZE 2) Allocate (TEST7_MIN_AOT_SIZE - 2K) of AOT data. This should pass. Break up of free space: Free
 * block bytes = (J9SHR_MIN_GAP_BEFORE_METADATA + 1K) (not usable to store block data as the flags for BLOCK space full
 * are set). Reserved AOT = 2K Reserve JIT = 5K 3) Allocate AOT data > (available reserved AOT bytes + free block
 * bytes). This should fail and set the cache header flag and runtime flag for AOT space full. Break up of free space:
 * same as in (2) above. 4) Allocate (TEST7_MIN_JIT_SIZE - 2K) of JIT data. This should pass. Break up of free space:
 * 		Free block bytes = (J9SHR_MIN_GAP_BEFORE_METADATA + 1K) (not usable to store block data as the flags for
 * BLOCK space full are set). Reserved AOT = 2K Reserve JIT = 2K 5) Allocate JIT data > (available reserved JIT bytes +
 * free block bytes). This should fail and set the cache header flag and runtime flag for JIT space full. Break of free
 * space: same as in (4) above.
 */
IDATA test8(J9JavaVM* vm)
{
    ...
    U_64 unsetRuntimeFlags = J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT | J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT_RW
    ...
    if (cacheHelper.openTestCache(J9PORT_SHR_CACHE_TYPE_PERSISTENT, cacheSize, NULL, true, NULL, NULL, NULL, 0,
            unsetRuntimeFlags, UnitTest::CACHE_FULL_TEST, J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, "openTestCache",
            false, true, piConfig, sharedClassConfig)
        == FAIL) {
        ERRPRINTF1("Failed to open cache for %s", testName);
        rc = FAIL;
        goto done;
    ...
    /* Add enough metadata to the cache to mark it as full */
    itemLen = (U_32)(
        oldFreeBlockBytes - (J9SHR_MIN_GAP_BEFORE_METADATA + ONE_K_BYTES) - (sizeof(ShcItem) + sizeof(ShcItemHdr)));
    cc->initBlockData(&itemPtr, itemLen, TYPE_UNINDEXED_BYTE_DATA);
    ...
    if (ca->minJIT != cc->getAvailableReservedJITBytes(vm->mainThread)) {
        ERRPRINTF2("Error: Expected available reserved JIT bytes to be equal to minJIT\n"
                   "minJIT=%d, available reserved JIT bytes=%d",
            ca->minJIT, cc->getAvailableReservedJITBytes(vm->mainThread));
        cc->exitWriteMutex(vm->mainThread, testName);
        rc = FAIL;
        goto done;
     ...
     * This is expected to fail, as only (available reserved JIT bytes + free block bytes) of space is left for JIT
     * data.
     */
...
IDATA test9(J9JavaVM* vm)
{
    ...
    /* Phase 1: 1) Test the case that when available space is less than CC_MIN_SPACE_BEFORE_CACHE_FULL,
     * J9SHR_RUNTIMEFLAG_AVAILABLE_SPACE_FULL will be set (cache is soft full). 2) Test the case that when cache is soft
     * full, JIT data can still be added into the reserved JIT data space. 3) Test the case that when cache is soft
     * full, AOT data can still be added into the reserved AOT space.
     */

    /* Try adding a ROMClass so that the cache becomes soft full*/
    usedBytes = cc->getUsedBytes();
    /* Make CC_MIN_SPACE_BEFORE_CACHE_FULL > free avalibale space > CC_MIN_SPACE_BEFORE_CACHE_FULL - romClassSize2, so
     * that increasing the softmx according to the
     * unstoredBytes in phase 3 can take effect. Otherwise, it has no effect as free avalibale space can still be less
     * than CC_MIN_SPACE_BEFORE_CACHE_FULL. */
    ...
    /* Try adding JIT data of size 3K bytes into the cache. This should success because of the 4K reserved JIT space
     * even when cache is soft full. */
    ...
    /* Try adding AOT data of size 3K bytes into the cache. This should success because of the 4K reserved AOT space
     * even when cache is soft full. */
    ...
    result = (BlockPtr)vm->sharedClassConfig->storeCompiledMethod(
        vm->mainThread, (J9ROMMethod*)SEGUPDATEPTR(ca), aotDataStart, aotDataLength, NULL, 0, true);
    if (true == cc->isAddressInCache(result)) {
     ...
     * 			2) Try adding JIT data when the cache is soft full (free reserved JIT data space is not big enough), should
     * fail. 3) Try adding AOT data when the cache is soft full (free reserved AOT space is not big enough), should
     * fail.
     */
    ...
    if ((romClassSize2 + SHC_WORDALIGN + sizeof(ShcItem) + sizeof(ShcItemHdr) + sizeof(OrphanWrapper))
        != unstoredBytes) {
        ERRPRINTF2("Error: unstoredBytes is %u, but should be %u", unstoredBytes,
            romClassSize2 + SHC_WORDALIGN + sizeof(ShcItem) + sizeof(ShcItemHdr) + sizeof(OrphanWrapper));
        rc = FAIL;
        goto done;
    }
    ...
    /* Try adding JIT data of size 3K bytes into the cache. This should fail because the cache is soft full and reserved
     * JIT data space is not big enough. */
    rc = (IDATA)vm->sharedClassConfig->storeAttachedData(vm->mainThread, SEGUPDATEPTR(ca), &dataDescriptor, true);
    if (J9SHR_RESOURCE_STORE_FULL == rc) {
        INFOPRINTF("Cache is soft full the reserved JIT data space is not big enough. JIT data is not stored into the "
                   "cache as expected");
    } else if (0 == rc) {
        ERRPRINTF("Error: JIT data is not expected to be added into the cache as the cache is soft full and reserved "
                  "JIT data space is not big enough");
        rc = FAIL;
        goto done;
    } else {
    ...
    /* Try adding AOT data of size 3K bytes into the cache. This should fail because the cache is soft full and reserved
     * aot space is not big enough. */
    result = (BlockPtr)vm->sharedClassConfig->storeCompiledMethod(
        vm->mainThread, (J9ROMMethod*)SEGUPDATEPTR(ca), aotDataStart, aotDataLength, NULL, 0, true);
    if ((BlockPtr)J9SHR_RESOURCE_STORE_FULL == result) {
        INFOPRINTF("Cache is soft full and the reserved AOT space is not big enough. AOT data is not stored into the "
                   "cache as expected\n");
    } else if (true == cc->isAddressInCache(result)) {
        ERRPRINTF("Error: AOT data is not expected to be added into the cache as the cache is soft full and the "
                  "reserved AOT space is not big enough\n");
        rc = FAIL;
        goto done;
    } else {
    ...
    if (!J9_ARE_ALL_BITS_SET(
            vm->sharedClassConfig->runtimeFlags, J9SHR_RUNTIMEFLAG_AOT_SPACE_FULL | J9SHR_RUNTIMEFLAG_JIT_SPACE_FULL)) {
    ...
    cc->tryAdjustMinMaxSizes(vm->mainThread);
    /* increase softMaxBytes to a big value so that it is the setting of maxAOT/maxJIT that prevents AOT/JIT data being
     * added. */
