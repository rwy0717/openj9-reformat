...
public:
    ...
    bool isVerbosePages(void) { return true; }

    bool isMemProtectEnabled() { return true; }

    bool isMemProtectPartialPagesEnabled() { return true; }
    ...
    UDATA getNumberOfProtects() { return numberOfProtects; }

    UDATA getAmountProtected() { return amountProtected; }

    UDATA getNumberOfPartialPageReadOnlyProtects() { return numberOfPartialPageReadOnlyProtects; }

    UDATA getNumberOfPartialPageReadWriteProtects() { return numberOfPartialPageReadWriteProtects; }
...
testClassDebugDataTests(J9JavaVM* vm)
{
    ...
    if (ClassDebugDataProvider::recommendedSize(onemeg, 4) > 0) {
        ...
        /* This is being done only to support the initial checking of this code which will create a debug area size of
         * 0. This is being done just to ensure there are no regressions.
         */
...
void debugmemfree(J9JavaVM* vm, void* memory)
{
    PORT_ACCESS_FROM_JAVAVM(vm);
...
DebugAreaUnitTests::debugmemtest2(J9JavaVM* vm)
{
    ...
    j9tty_printf(PORTLIB,
        "%s: Test empty debug area (Init, getDebugDataSize, getFreeDebugSpaceBytes, getLineNumberTableBytes, and "
        "getLocalVariableTableBytes).\n",
        testName);
...
DebugAreaUnitTests::debugmemtest3(J9JavaVM* vm)
{
    ...
    if (debugarea->allocateClassDebugData(
            vm->mainThread, (U_16)strlen(testName), testName, (const J9RomClassRequirements*)&sizes, &pieces, NULL)
        != 0) {
...
DebugAreaUnitTests::debugmemtest7(J9JavaVM* vm)
{
    ...
    sizes.lineNumberTableSize = allocsize;
    if (debugarea->allocateClassDebugData(
            vm->mainThread, (U_16)strlen(testName), testName, (const J9RomClassRequirements*)&sizes, &pieces, &prot)
        != 0) {
        ERRPRINTF("allocateClassDebugData() failed\n");
        retval = TEST_ERROR;
        goto done;
...
DebugAreaUnitTests::debugmemtest11(J9JavaVM* vm)
{
    ...
    if (debugarea->allocateClassDebugData(
        ...
        ERRPRINTF("allocateClassDebugData() failed\n");
        j9tty_printf(PORTLIB,
            "\tError: allocateClassDebugData() failed (free:%u sizes.lineNumberTableSize:%u "
            "sizes.localVariableTableSize:%u)\n",
...
DebugAreaUnitTests::debugmemtest13(J9JavaVM* vm)
{
    ...
    *theca = origHeader;
    ...
    if (debugarea->getFailureReason() != CACHE_DEBUGAREA_BAD_FREE_SPACE) {
        ERRPRINTF2(
            "debugarea->getFailureReason(...) returned %d when CACHE_DEBUGAREA_BAD_FREE_SPACE, or %d, was expected\n",
    ...
    *theca = origHeader;
    ...
    if (debugarea->getFailureReason() != CACHE_DEBUGAREA_BAD_FREE_SPACE_SIZE) {
        ERRPRINTF2("debugarea->getFailureReason(...) returned %d when CACHE_DEBUGAREA_BAD_FREE_SPACE_SIZE, or %d, was "
                   "expected\n",
    ...
    *theca = origHeader;
    ...
    if (debugarea->getFailureReason() != CACHE_DEBUGAREA_BAD_LNT_HEADER_INFO) {
        ERRPRINTF2("debugarea->getFailureReason(...) returned %d when CACHE_DEBUGAREA_BAD_LNT_HEADER_INFO, or %d, was "
                   "expected\n",
    ...
    *theca = origHeader;
    ...
    if (debugarea->getFailureReason() != CACHE_DEBUGAREA_BAD_LVT_HEADER_INFO) {
        ERRPRINTF2("debugarea->getFailureReason(...) returned %d when CACHE_DEBUGAREA_BAD_LVT_HEADER_INFO, or %d, was "
                   "expected\n",
    ...
    *theca = origHeader;
    ...
    debugarea->_storedLineNumberTableBytes
        = (U_32)((UDATA)(debugarea->_lvtLastUpdate) - (UDATA)(debugarea->_lntLastUpdate)) + 1;
    if (debugarea->isOk(vm->mainThread, true, true, true) == true) {
    ...
    if (debugarea->getFailureReason() != CACHE_DEBUGAREA_BAD_SIZES_FOR_COMMIT) {
        ERRPRINTF2("debugarea->getFailureReason() returned %d when CACHE_DEBUGAREA_BAD_SIZES_FOR_COMMIT, or %d, was "
                   "expected\n",
    ...
    debugarea->_storedLocalVariableTableBytes
        = (U_32)((UDATA)(debugarea->_lvtLastUpdate) - (UDATA)(debugarea->_lntLastUpdate)) + 1;
    if (debugarea->isOk(vm->mainThread, true, true, true) == true) {
        ERRPRINTF1("debugarea->isOk(...) pass unexpectedly (_storedLineNumberTableBytes:%d)\n",
            debugarea->_storedLineNumberTableBytes);
...
DebugAreaUnitTests::debugmemtest14(J9JavaVM* vm)
{
    ...
    if (percent1 < expectedPercent - 1) { /* give a leeway of 1% to account for truncation due to integer arithmetic */
        ERRPRINTF4("Expected percent of ClassDebugDataArea size is %d%%, but got %d%% (cache size: %u bytes, "
                   "ClassDebugDataArea: %u bytes)\n",
