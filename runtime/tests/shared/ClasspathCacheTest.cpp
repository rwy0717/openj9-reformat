...
public:
    static IDATA initializeTest(
        J9PortLibrary* portLibrary, J9ClasspathByIDArray** cp1, J9ClasspathByIDArray** cp2, J9ClasspathByIDArray** cp3);
    static IDATA setTest(
        J9JavaVM* vm, J9ClasspathByIDArray** cp1, J9ClasspathByIDArray** cp2, J9ClasspathByIDArray** cp3);
    static IDATA getTest(
        J9JavaVM* vm, J9ClasspathByIDArray** cp1, J9ClasspathByIDArray** cp2, J9ClasspathByIDArray** cp3);
    static IDATA getIDTest(
        J9PortLibrary* portLibrary, J9ClasspathByIDArray** cp1, J9ClasspathByIDArray** cp2, J9ClasspathByIDArray** cp3);
    static IDATA clearTest(
        J9JavaVM* vm, J9ClasspathByIDArray** cp1, J9ClasspathByIDArray** cp2, J9ClasspathByIDArray** cp3);
    static IDATA freeTest(
        J9PortLibrary* portLibrary, J9ClasspathByIDArray** cp1, J9ClasspathByIDArray** cp2, J9ClasspathByIDArray** cp3);
...
void ClasspathCacheTest::setClasspathByIDInt(J9ClasspathByID* cp, UDATA i, UDATA arraySize)
{
...
ClasspathCacheTest::isClasspathByIDSetToInt(J9ClasspathByID* cp, UDATA i, UDATA arraySize)
{
    ...
    if ((generic->magic == (U_8)i) && (generic->type == (U_8)i) && (generic->id == (U_16)i)
        && (generic->jclData == (struct J9ClassPathEntry*)i) && (generic->cpData == (void*)i)
        && (cp->entryCount == i)) {
        UDATA cntr;
...
IDATA
ClasspathCacheTest::initializeTest(
    J9PortLibrary* portLibrary, J9ClasspathByIDArray** cp1, J9ClasspathByIDArray** cp2, J9ClasspathByIDArray** cp3)
{
    ...
    J9JavaVM* vm, J9ClasspathByIDArray** cp1, J9ClasspathByIDArray** cp2, J9ClasspathByIDArray** cp3)
{
    ...
    for (i = 0; i < arraySize + 1; i++) {
        previous = *cp2;
        setIdentifiedClasspath(
            vm->mainThread, cp2, i, ITEMS_OFFSET + i, PARTITION1, strlen(PARTITION1), (void*)(CP_OFFSET2 + i));
        if (i < arraySize) {
            /* Check that array hasn't grown and that it hasn't been replaced */
    ...
    for (i = 0; i < arraySize + 1; i++) {
        previous = *cp3;
        setIdentifiedClasspath(
            vm->mainThread, cp3, i, ITEMS_OFFSET + i, PARTITION2, strlen(PARTITION2), (void*)(CP_OFFSET3 + i));
        if (i < arraySize) {
            /* Check that array hasn't grown and that it hasn't been replaced */
        ...
        oldSize = (*cp3)->size;
        setIdentifiedClasspath(
            vm->mainThread, cp3, i, ITEMS_OFFSET + i, PARTITION3, strlen(PARTITION3), (void*)(CP_OFFSET4 + i));
        /* Check that array hasn't grown even further */
        if ((*cp3)->size != oldSize) {
            return 12;
        }
        setIdentifiedClasspath(
            vm->mainThread, cp3, i, ITEMS_OFFSET + i, PARTITION4, strlen(PARTITION4), (void*)(CP_OFFSET5 + i));
        /* Check that array hasn't grown even further */
        if ((*cp3)->size != oldSize) {
            return 13;
        }
    ...
    J9JavaVM* vm, J9ClasspathByIDArray** cp1, J9ClasspathByIDArray** cp2, J9ClasspathByIDArray** cp3)
{
    ...
    for (j = 0; j < 3; j++) {
        ...
        for (i = 0; i < arraySize; i++) {
            ...
            result = getIdentifiedClasspath(
                vm->mainThread, current, i, ITEMS_OFFSET + i, PARTITION1, strlen(PARTITION1), &cpToFree);
            if (j == 1) {
                if (result != (void*)(CP_OFFSET2 + i))
                    return 4;
            } else {
                if (result)
                    return 5;
            ...
            result = getIdentifiedClasspath(
                vm->mainThread, current, i, ITEMS_OFFSET + i, PARTITION2, strlen(PARTITION2), &cpToFree);
            if (j == 2) {
                if (result != (void*)(CP_OFFSET3 + i))
                    return 7;
            } else {
                if (result)
                    return 8;
            ...
            result = getIdentifiedClasspath(
                vm->mainThread, current, i, ITEMS_OFFSET + i, PARTITION3, strlen(PARTITION3), &cpToFree);
            if (j == 2) {
                if (result != (void*)(CP_OFFSET4 + i))
                    return 10;
            } else {
                if (result)
                    return 11;
            ...
            result = getIdentifiedClasspath(
                vm->mainThread, current, i, ITEMS_OFFSET + i, PARTITION4, strlen(PARTITION4), &cpToFree);
            if (j == 2) {
                if (result != (void*)(CP_OFFSET5 + i))
                    return 13;
            } else {
                if (result)
                    return 14;
    ...
    for (i = 0; i < arraySize; i++) {
        ...
        setIdentifiedClasspath(
            vm->mainThread, &current, i, ITEMS_OFFSET + i, PARTITION1, strlen(PARTITION1), (void*)(CP_OFFSET2 + i));
        setIdentifiedClasspath(
            vm->mainThread, &current, i, ITEMS_OFFSET + i, PARTITION2, strlen(PARTITION2), (void*)(CP_OFFSET3 + i));
        setIdentifiedClasspath(
            vm->mainThread, &current, i, ITEMS_OFFSET + i, PARTITION3, strlen(PARTITION3), (void*)(CP_OFFSET4 + i));
        setIdentifiedClasspath(
            vm->mainThread, &current, i, ITEMS_OFFSET + i, PARTITION4, strlen(PARTITION4), (void*)(CP_OFFSET5 + i));
    }
    ...
    for (j = 0; j < 5; j++) {
        ...
        for (i = 0; i < arraySize; i++) {
            ...
            result = getIdentifiedClasspath(
                vm->mainThread, current, i, ITEMS_OFFSET + i, partition, partitionLen, &cpToFree);
            if (result != (void*)(offset + i))
                return 16;
            if (cpToFree)
                return 17;
            ...
            result = getIdentifiedClasspath(
                vm->mainThread, current, i, (ITEMS_OFFSET + i + 1), partition, partitionLen, &cpToFree);
            if (cpToFree != (void*)(offset + i))
                return 18;
            if (result)
                return 19;
            ...
            setIdentifiedClasspath(
                vm->mainThread, &current, i, (ITEMS_OFFSET + i + 1), partition, partitionLen, (void*)(offset + i));
            result = getIdentifiedClasspath(
                vm->mainThread, current, i, (ITEMS_OFFSET + i + 1), partition, partitionLen, &cpToFree);
            if (result != (void*)(offset + i))
                return 22;
    ...
    J9JavaVM* vm, J9ClasspathByIDArray** cp1, J9ClasspathByIDArray** cp2, J9ClasspathByIDArray** cp3)
{
    ...
    for (j = 0; j < 3; j++) {
        ...
        for (i = 0; i < arraySize; i++) {
            ...
            if (iresult != ID_NOT_FOUND)
                return 1;

            /* Check that identified array is now clear (2) */
            vresult
                = getIdentifiedClasspath(vm->mainThread, current, i, ITEMS_OFFSET + i, partition1, plen1, &cpToFree);
            if (vresult)
                return 2;
            if (cpToFree)
                return 3;
            ...
            if (j == 2) {
                vresult = getIdentifiedClasspath(
                    vm->mainThread, current, i, ITEMS_OFFSET + i, partition2, plen2, &cpToFree);
                if (vresult)
                    return 4;
                if (cpToFree)
                    return 5;
                ...
                vresult = getIdentifiedClasspath(
                    vm->mainThread, current, i, ITEMS_OFFSET + i, partition3, plen3, &cpToFree);
                if (vresult)
                    return 6;
                if (cpToFree)
                    return 7;
            ...
            if (i < (arraySize - 1)) {
                vresult = getIdentifiedClasspath(
                    vm->mainThread, current, i + 1, ITEMS_OFFSET + i + 1, partition1, plen1, &cpToFree);
                if (vresult != (void*)(offset1 + i + 1))
                    return 8;
                if (cpToFree)
                    return 9;
            }
...
testClasspathCache(J9JavaVM* vm)
{
    ...
    SHC_TEST_ASSERT("initializeIdentifiedClasspathArray", ClasspathCacheTest::initializeTest(PORTLIB, &cp1, &cp2, &cp3),
        success, rc);
    ...
    SHC_TEST_ASSERT(
        "freeIdentifiedClasspathArray", ClasspathCacheTest::freeTest(PORTLIB, &cp1, &cp2, &cp3), success, rc);
