...
private:
    ...
    static ClasspathItem* createClasspathItem(J9JavaVM* vm, IDATA entries, IDATA entryLen, IDATA fromHelperID,
        U_16 cpType, UDATA protocols, char** stringCursor);
    static IDATA compareGetters(J9JavaVM* vm, ClasspathItem* local, ClasspathItem* serial, UDATA cntr);
};

ClasspathEntryItem* ClasspathItemTest::createClasspathEntryItem(
    J9JavaVM* vm, const char* path, U_16 pathlen, UDATA protocol)
{
...
ClasspathItemTest::classpathEntryTest(J9JavaVM* vm)
{
    ...
    const char* PATH3 = "abcdefghijklmnopqrstuvwxyz/abcdefghijklmnopqrstuvwxyz/abcdefghijklmnopqrstuvwxyz/"
                        "abcdefghijklmnopqrstuvwxyz/jar.jar";
    ...
    if (compareLen1 != strlen(PATH1))
        return 5;
    if (compareLen2 != strlen(PATH2))
        return 6;
    if (compareLen3 != strlen(PATH3))
        return 7;
    if (compareLen4 != strlen(PATH4))
        return 8;

    if (strncmp(PATH1, compare1, compareLen1))
        return 9;
    if (strncmp(PATH2, compare2, compareLen2))
        return 10;
    if (strncmp(PATH3, compare3, compareLen3))
        return 11;
    if (strncmp(PATH4, compare4, compareLen4))
        return 12;
    ...
    if (hash1 != serial1->hash(vm->internalVMFunctions))
        return 33;
    if (hash2 != serial2->hash(vm->internalVMFunctions))
        return 34;
    if (hash3 != serial3->hash(vm->internalVMFunctions))
        return 35;
    if (hash4 != serial4->hash(vm->internalVMFunctions))
        return 36;

    if (!ClasspathItem::compare(vm->internalVMFunctions, cpei1, serial1))
        return 37;
    if (!ClasspathItem::compare(vm->internalVMFunctions, cpei2, serial2))
        return 38;
    if (!ClasspathItem::compare(vm->internalVMFunctions, cpei3, serial3))
        return 39;
    if (!ClasspathItem::compare(vm->internalVMFunctions, cpei4, serial4))
        return 40;

    if (ClasspathItem::compare(vm->internalVMFunctions, cpei1, serial2))
        return 41;
    if (ClasspathItem::compare(vm->internalVMFunctions, cpei2, serial3))
        return 42;
    if (ClasspathItem::compare(vm->internalVMFunctions, cpei3, serial4))
        return 43;
    if (ClasspathItem::compare(vm->internalVMFunctions, cpei4, serial1))
        return 44;

    if (ClasspathItem::compare(vm->internalVMFunctions, cpei1, cpei4))
        return 45;
    if (ClasspathItem::compare(vm->internalVMFunctions, cpei2, cpei3))
        return 46;
    if (ClasspathItem::compare(vm->internalVMFunctions, cpei3, cpei2))
        return 47;
    if (ClasspathItem::compare(vm->internalVMFunctions, cpei4, cpei1))
        return 48;

    if (!ClasspathItem::compare(vm->internalVMFunctions, serial1, serial1))
        return 49;
    if (!ClasspathItem::compare(vm->internalVMFunctions, serial2, serial2))
        return 50;
    if (!ClasspathItem::compare(vm->internalVMFunctions, serial3, serial3))
        return 51;
    if (!ClasspathItem::compare(vm->internalVMFunctions, serial4, serial4))
        return 52;

    if (!ClasspathItem::compare(vm->internalVMFunctions, cpei1, cpei1))
        return 53;
    if (!ClasspathItem::compare(vm->internalVMFunctions, cpei2, cpei2))
        return 54;
    if (!ClasspathItem::compare(vm->internalVMFunctions, cpei3, cpei3))
        return 55;
    if (!ClasspathItem::compare(vm->internalVMFunctions, cpei4, cpei4))
        return 56;
    ...
    if (ClasspathItem::compare(vm->internalVMFunctions, cpei1, cpei2))
        return 61;
    if (ClasspathItem::compare(vm->internalVMFunctions, cpei2, cpei3))
        return 62;
    if (ClasspathItem::compare(vm->internalVMFunctions, cpei3, cpei1))
        return 63;
...
ClasspathItem* ClasspathItemTest::createClasspathItem(
    J9JavaVM* vm, IDATA entries, IDATA entryLen, IDATA fromHelperID, U_16 cpType, UDATA protocols, char** stringCursor)
{
    UDATA sizeNeeded;
...
ClasspathItemTest::compareTest(J9JavaVM* vm, ClasspathItem** cpArray, ClasspathItem** serialArray)
{
    ...
    for (i = 0; i < CPLENGTH; i++) {
        for (j = 0; j < CPLENGTH; j++) {
            ...
            j9tty_printf(PORTLIB, ".");
            /* Comparing 1 and 4 (and the partition/context versions) should pass because only difference is
               URL/Classpath type, which is not tested by compare, quite intentionally */
            if (i == j || ((i % 6) == 1 && (j % 6 == 4) && (i / 6 == j / 6))
                || ((i % 6) == 4 && (j % 6 == 1) && (i / 6 == j / 6))) {
                if (!result1) {
                ...
                if (!result3) {
                    j9tty_printf(
                        PORTLIB, "Compare failed unexpectedly when comparing serial %d with serial %d\n", i, j);
                    return 3;
                }
            } else {
                if (result1) {
    ...
    for (i = 0; i < CPLENGTH; i++) {
        ...
        for (j = 0; j < expectedResult + 1; j++) {
            ...
            if (j < expectedResult) {
                ...
                if (j > 0 && find6 != -1) {
                    j9tty_printf(PORTLIB,
                        "find() with stopAtIndex should have returned -1 for local classpath #%d and item %d\n", i, j);
                    return 7777;
                }
                if (find7 != j) {
                    j9tty_printf(PORTLIB,
                        "find() with stopAtIndex>itemsAdded failed for local classpath #%d and item %d\n", i, j);
                    return 8888;
                }
...
/*
        ...
        (classpath 0 entry | classpath 1 entry | classpath 10 entries | classpath 1000 entries | URL 1 entry | Token 1
   entry) = 6 ClasspathItems
*/
IDATA
testClasspathItem(J9JavaVM* vm)
{
    ...
    for (j = 1; j < 30; j++) {
        ...
        for (i = 0; i < CPLENGTH; i++) {
            if (cpArrayPtr[i])
                j9mem_free_memory(cpArrayPtr[i]);
            if (serialArrayPtr[i])
                j9mem_free_memory(serialArrayPtr[i]);
        }
