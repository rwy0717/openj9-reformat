...
IDATA storeAndFindTest(J9JavaVM* vm)
{
    ...
    sharedConfig = (J9SharedClassConfig*)j9mem_allocate_memory(
    ...
    sharedpiConfig = (J9SharedClassPreinitConfig*)j9mem_allocate_memory(
    ...
    cacheAllocated
        = (BlockPtr)j9mem_allocate_memory(sharedpiConfig->sharedClassCacheSize + osPageSize, J9MEM_CATEGORY_CLASSES);
    ...
    INFOPRINTF5("Store And Find Test cos=%d cs=%d co=%x cba=%x rc=%d\n", cacheObjectSize,
        sharedpiConfig->sharedClassCacheSize, cacheObject1, cache, rc);
    if (0 != rc) {
    ...
    /* We will set method name and signature for romMethod1. Reserve enough space between romMethod1 and its next method
     * (romMethod5) in the cache */
    ...
    j9str_printf(
        PORTLIB, (BlockPtr)(J9UTF8_DATA(methodNameAndSigMem)), sizeof(TEST_METHOD_NAME), "%s", TEST_METHOD_NAME);
    ...
    j9str_printf(PORTLIB, (BlockPtr)(J9UTF8_DATA(methodNameAndSigMem + METHOD_NAME_SIZE)), sizeof(TEST_METHOD_SIG),
        "%s", TEST_METHOD_SIG);
     ...
     */
    storedMethod1 = cacheObject1->storeCompiledMethod(
        vm->mainThread, romMethod1, (const U_8*)dataArray[0], dataSize1, (const U_8*)codeArray[0], codeSize1, FALSE);
    storedMethod2 = cacheObject1->storeCompiledMethod(
        vm->mainThread, romMethod2, (const U_8*)dataArray[1], dataSize2, (const U_8*)codeArray[1], codeSize2, FALSE);
    storedMethod3 = cacheObject1->storeCompiledMethod(
        vm->mainThread, romMethod3, (const U_8*)dataArray[2], dataSize3, (const U_8*)codeArray[2], codeSize3, FALSE);
     ...
     */
    cacheObject2->storeCompiledMethod(
        vm->mainThread, romMethod5, (const U_8*)dataArray[3], dataSize5, (const U_8*)codeArray[3], codeSize5, FALSE);
     ...
     */
    if (cacheObject1->storeCompiledMethod(
            vm->mainThread, romMethod5, (const U_8*)dataArray[3], dataSize5, (const U_8*)codeArray[3], codeSize5, FALSE)
        != (U_8*)J9SHR_RESOURCE_STORE_EXISTS) {
    ...
    duplicateMethod = cacheObject1->storeCompiledMethod(
        vm->mainThread, romMethod5, (const U_8*)dataArray[3], dataSize5, (const U_8*)codeArray[3], codeSize5, TRUE);
    ...
    if (cacheObject1->storeCompiledMethod(
            vm->mainThread, romMethod1, (const U_8*)dataArray[0], dataSize1, (const U_8*)codeArray[0], codeSize1, FALSE)
        != (U_8*)J9SHR_RESOURCE_STORE_INVALIDATED) {
    ...
    duplicateMethod = cacheObject1->storeCompiledMethod(
        vm->mainThread, romMethod1, (const U_8*)dataArray[0], dataSize1, (const U_8*)codeArray[0], codeSize1, TRUE);
