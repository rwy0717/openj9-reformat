...
static IDATA createTestCompositeCache(
    J9JavaVM* vm, SH_CompositeCacheImpl** cc, J9SharedClassPreinitConfig* piconfig, const char* testName)
{
    ...
    if (actualCC->startup(vm->mainThread, piconfig, cache, &runtimeFlags, 1, "myRoot", NULL, J9SH_DIRPERM_ABSENT,
...
testCompositeCacheSizes(J9JavaVM* vm)
{
    ...
    rc |= test1(vm);
    rc |= test2(vm);
    /* Exclude test3 from AIX machines because of memory allocation limits on some AIX machines. Fails to allocate big
     * enough memory for this test. */
#if !defined(AIXPPC)
    rc |= test3(vm);
#endif
    rc |= test4(vm);
...
static IDATA test1(J9JavaVM* vm)
{
    ...
    requiredCacheSize
        = srpHashTable_requiredMemorySize(MAIN_SHARED_TABLE_NODE_COUNT, sizeof(J9SharedInternSRPHashTableEntry), TRUE);
...
static IDATA test2(J9JavaVM* vm)
{
    ...
    j9tty_printf(PORTLIB,
        "%s: create default composite cache without using -Xitsn option and check string table bytes and read/write "
        "are bytes are same\n",
        testName);
     ...
     * Unless string table bytes set externally with -Xitsn option, read/write are bytes and string table bytes must be
     * same.
     *
     */
...
static IDATA test3(J9JavaVM* vm)
{
    ...
    j9tty_printf(PORTLIB,
        "%s: create composite cache with read/write area smaller than cache size but larger than maximum allowed r/w "
        "size\n",
        testName);
    ...
    piconfig->sharedClassDebugAreaBytes = -1;
    piconfig->sharedClassSoftMaxBytes = -1;
    /* 1.1 * (maxSharedStringTableSize * 300) might be greater than SHMMAX and pass
     * J9SHR_RUNTIMEFLAG_ENABLE_PERSISTENT_CACHE to avoid cache size to be resized to SHMMAX */
...
static IDATA test4(J9JavaVM* vm)
{
    ...
    j9tty_printf(
        PORTLIB, "%s: create composite cache, fill it, start it again and ensure the cache is marked full\n", testName);
...
static IDATA test5(J9JavaVM* vm)
{
    ...
    j9tty_printf(PORTLIB,
        "%s: create composite cache with -Xitsn value bigger than max shared intern table node count and check -Xitsn "
        "option is not restricted by max shared table node count \n",
        testName);
    ...
    sharedStringTableSize = srpHashTable_requiredMemorySize(
...
static IDATA test6(J9JavaVM* vm)
{
    ...
    j9tty_printf(
        PORTLIB, "%s: create composite cache with -Xitsn that requires more memory than cache size \n", testName);
    ...
    maxSharedStringTableSize = (IDATA)srpHashTable_requiredMemorySize(
        SHRINIT_MAX_SHARED_STRING_TABLE_NODE_COUNT, sizeof(J9SharedInternSRPHashTableEntry), TRUE);
...
static IDATA test7(J9JavaVM* vm)
{
    ...
    j9tty_printf(
        PORTLIB, "%s: create composite cache with minaot size that reserves memory more than cache size \n", testName);
...
static IDATA test9(J9JavaVM* vm)
{
    ...
    j9tty_printf(PORTLIB,
        "%s: create composite cache with minjit data size that reserves memory more than cache size \n", testName);
...
static IDATA test13(J9JavaVM* vm)
{
    ...
    j9tty_printf(
        PORTLIB, "%s: create composite cache with maxaot data size is lesser than minaot data size\n", testName);
...
static IDATA test14(J9JavaVM* vm)
{
    ...
    j9tty_printf(
        PORTLIB, "%s: create composite cache with maxjit data size is lesser than minjit data size \n", testName);
