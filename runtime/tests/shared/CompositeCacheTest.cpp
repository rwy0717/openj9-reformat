...
public:
    static IDATA createTest(J9JavaVM* vm, IDATA testCacheSize, SH_CompositeCacheImpl** cc1,
        SH_CompositeCacheImpl** cc1a, SH_CompositeCacheImpl** cc2, SH_CompositeCacheImpl** cc2a, U_64* runtimeFlags0,
        U_64* runtimeFlags1);
    static IDATA allocateAndStats(
        J9JavaVM* vm, IDATA testCacheSize, SH_CompositeCacheImpl* cc1, SH_CompositeCacheImpl* cc1a);
...
private:
    static IDATA localTestStats(J9JavaVM* vm, IDATA testCacheSize, UDATA metaBytes, UDATA allocBytes, UDATA AOTBytes,
        UDATA JITBytes, UDATA updateCntr, SH_CompositeCacheImpl* cc, char* cacheBase);
    static IDATA checkUpdateResults(J9JavaVM* vm, SH_CompositeCacheImpl* cc, UDATA expectedCheckUpdates,
        bool tryNextEntry, char* expectedNextEntry, bool useStaleItems, UDATA expectedStale, bool expectSegment,
        char* segment);
};

IDATA
CompositeCacheTest::createTest(J9JavaVM* vm, IDATA testCacheSize, SH_CompositeCacheImpl** cc1,
    SH_CompositeCacheImpl** cc1a, SH_CompositeCacheImpl** cc2, SH_CompositeCacheImpl** cc2a, U_64* runtimeFlags0,
    U_64* runtimeFlags1)
{
    IDATA result = PASS;
    ...
    if (!(piconfig = (J9SharedClassPreinitConfig*)j9mem_allocate_memory(
    ...
    if (actualCC->startup(vm->mainThread, piconfig, cache, runtimeFlags1, 1, "myRoot", NULL, J9SH_DIRPERM_ABSENT,
            &cacheSize, &localCrashCntr, true, &cacheHasIntegrity)
        != 0) {
        return 2;
    }
    ...
    cache = (char*)((char*)actualCC + requiredBytes);
    if (actualCC->startup(vm->mainThread, piconfig, cache, runtimeFlags1, 1, "myCache1", NULL, J9SH_DIRPERM_ABSENT,
            &cacheSize, &localCrashCntr, true, &cacheHasIntegrity)
        != 0) {
        return 8;
    }
    *cc1 = actualCC;
    ...
    if (actualCC->startup(vm->mainThread, piconfig, cache, runtimeFlags0, 1, "myCache1a", NULL, J9SH_DIRPERM_ABSENT,
            &cacheSize, &localCrashCntr, true, &cacheHasIntegrity)
        != 0) {
        return 10;
    }
    *cc1a = actualCC;
    ...
    cache = (char*)((char*)actualCC + requiredBytes);
    if (actualCC->startup(vm->mainThread, piconfig, cache, runtimeFlags1, 1, "myCache2", NULL, J9SH_DIRPERM_ABSENT,
            &cacheSize, &localCrashCntr, true, &cacheHasIntegrity)
        != 0) {
        return 12;
    }
    *cc2 = actualCC;
    ...
    if (actualCC->startup(vm->mainThread, piconfig, cache, runtimeFlags0, 1, "myCache2a", NULL, J9SH_DIRPERM_ABSENT,
            &cacheSize, &localCrashCntr, true, &cacheHasIntegrity)
        != 0) {
        return 14;
    }
...
IDATA
CompositeCacheTest::localTestStats(J9JavaVM* vm, IDATA testCacheSize, UDATA metaBytes, UDATA allocBytes, UDATA AOTBytes,
    UDATA JITBytes, UDATA updateCntr, SH_CompositeCacheImpl* cc, char* cacheBase)
{
    ...
    uResult = (expectedTotalCacheSize - allocBytes
        - (debugBytes + metaBytes + AOTBytes + JITBytes + (updateCntr * sizeof(ShcItemHdr))));
...
IDATA
CompositeCacheTest::allocateAndStats(
    J9JavaVM* vm, IDATA testCacheSize, SH_CompositeCacheImpl* cc1, SH_CompositeCacheImpl* cc1a)
{
    IDATA result = PASS;
    char* segBuf = NULL;
    ...
    /* Check all is 0'd */
    if (0 != (result |= localTestStats(vm, testCacheSize, 0, 0, 0, 0, 0, cc1, cacheBase)))
        goto _done;
    if (0 != (result |= localTestStats(vm, testCacheSize, 0, 0, 0, 0, 0, cc1a, cacheBase)))
        goto _done;
    ...
    if (0 != (result |= localTestStats(vm, testCacheSize, runningTotal, 0, 0, 0, 1, cc1, cacheBase)))
        goto _done;
    if (0 != (result |= localTestStats(vm, testCacheSize, runningTotal, 0, 0, 0, 1, cc1a, cacheBase)))
        goto _done;
    ...
    cc1->commitUpdate(vm->mainThread, false);
    runningTotal += itemPtr->dataLen;
    if (0 != (result |= localTestStats(vm, testCacheSize, runningTotal, 256, 0, 0, 2, cc1, cacheBase)))
        goto _done;
    if (0 != (result |= localTestStats(vm, testCacheSize, runningTotal, 256, 0, 0, 2, cc1a, cacheBase)))
        goto _done;
    ...
    cc1->commitUpdate(vm->mainThread, false);
    runningTotal += itemPtr->dataLen;
    if (0 != (result |= localTestStats(vm, testCacheSize, runningTotal, 264, 0, 0, 3, cc1, cacheBase)))
        goto _done;
    if (0 != (result |= localTestStats(vm, testCacheSize, runningTotal, 264, 0, 0, 3, cc1a, cacheBase)))
        goto _done;
    if (cc1->isAddressInROMClassSegment(address)) {
        j9tty_printf(PORTLIB, "Address %p should not be in romclass segment\n", address);
        result = 10;
    ...
    cc1->commitUpdate(vm->mainThread, false);
    runningTotal += itemPtr->dataLen;
    if (0 != (result |= localTestStats(vm, testCacheSize, runningTotal, 264, 0, 0, 4, cc1, cacheBase)))
        goto _done;
    if (0 != (result |= localTestStats(vm, testCacheSize, runningTotal, 264, 0, 0, 4, cc1a, cacheBase)))
        goto _done;
    if (cc1->isAddressInROMClassSegment(address)) {
        j9tty_printf(PORTLIB, "Address %p should not be in romclass segment\n", address);
        result = 16;
    ...
    if (0 != (result |= localTestStats(vm, testCacheSize, runningTotal, 264, 96, 0, 5, cc1, cacheBase)))
        goto _done;
    if (0 != (result |= localTestStats(vm, testCacheSize, runningTotal, 264, 96, 0, 5, cc1a, cacheBase)))
        goto _done;
    if (cc1->isAddressInROMClassSegment(address)) {
        j9tty_printf(PORTLIB, "Address %p should not be in romclass segment\n", address);
        result = 17;
...
CompositeCacheTest::basicMutexTest(J9JavaVM* vm, SH_CompositeCacheImpl* cc1, SH_CompositeCacheImpl* cc1a)
{
    ...
    /* Not a normal scenario, but forces enterReadMutex to go down different code path without waiting (as it should if
     * the write mutex is held) */
...
IDATA
CompositeCacheTest::checkUpdateResults(J9JavaVM* vm, SH_CompositeCacheImpl* cc, UDATA expectedCheckUpdates,
    bool tryNextEntry, char* expectedNextEntry, bool useStaleItems, UDATA expectedStale, bool expectSegment,
    char* segment)
{
    UDATA staleItems;
    ...
    if ((uResult = cc->checkUpdates(vm->mainThread)) != expectedCheckUpdates) {
        j9tty_printf(
            PORTLIB, "checkUpdates(vm->mainThread) returned %d - expected %d\n", uResult, expectedCheckUpdates);
        return 1;
    }
    if (tryNextEntry) {
        if (useStaleItems) {
        ...
        if (useStaleItems) {
            if (staleItems != expectedStale) {
                j9tty_printf(
                    PORTLIB, "staleItems returned from nextEntry were %d - expected %d\n", staleItems, expectedStale);
                return 3;
            }
        }
    }
    ...
    J9JavaVM* vm, IDATA testCacheSize, SH_CompositeCacheImpl* cc1, SH_CompositeCacheImpl* cc1a)
{
    ...
    */
    ...
    /* commitUpdate will reset checkUpdates() for CompositeCacheImpl that calls it.
            Assumption is that CacheMap will have refreshed its hashtables and got a write mutex before allocating and
       committing. */
...
IDATA
CompositeCacheTest::writeHashTest(
    J9JavaVM* vm, IDATA testCacheSize, SH_CompositeCacheImpl* cc1, SH_CompositeCacheImpl* cc1a)
{
    /* Tests:
                    testAndSetWriteHash
...
testCompositeCache(J9JavaVM* vm)
{
    ...
    UnitTest::unitTest = UnitTest::COMPOSITE_CACHE_TEST;
    SHC_TEST_ASSERT("createTest",
        CompositeCacheTest::createTest(
            vm, testCacheSize, &testCache1, &testCache1a, &testCache2, &testCache2a, &runtimeFlags0, &runtimeFlags1),
        success, rc);
    SHC_TEST_ASSERT("allocateAndStats",
        CompositeCacheTest::allocateAndStats(vm, testCacheSize, testCache1, testCache1a), success, rc);
    ...
    SHC_TEST_ASSERT(
        "updateTest", CompositeCacheTest::updateTest(vm, testCacheSize, testCache2, testCache2a), success, rc);
    /* TODO: Test made temporarily invalid by static _vmID
    SHC_TEST_ASSERT("writeHashTest", CompositeCacheTest::writeHashTest(vm, testCacheSize, testCache2, testCache2a),
    success, rc); */
    ...
    if (testCache1)
        j9mem_free_memory(testCache1);
    if (testCache1a)
        j9mem_free_memory(testCache1a);
    if (testCache2)
        j9mem_free_memory(testCache2);
    if (testCache2a)
        j9mem_free_memory(testCache2a);
