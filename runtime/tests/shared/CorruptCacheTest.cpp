...
typedef enum CorruptionStage { INVALID_STAGE, DURING_STARTUP, AFTER_STARTUP } CorruptionStage;
...
static const char* CorruptionTypeStrings[] = { " ------- CORRUPTION TYPE START ------- ", "CACHE_CRC_INVALID_TYPE",
    "WALK_ROMCLASS_CORRUPT_CASE1_TYPE", "WALK_ROMCLASS_CORRUPT_CASE2_TYPE", "ITEM_TYPE_CORRUPT_UNINITIALIZED_TYPE",
    "ITEM_TYPE_CORRUPT_MAX_DATA_TYPES_TYPE", "ITEM_LENGTH_CORRUPT_TYPE", "ITEM_LENGTH_CORRUPT_TYPE_LEN2LONG",
    "ITEM_LENGTH_CORRUPT_AFTER_STARTUP_TYPE", "CACHE_HEADER_INCORRECT_DATA_LENGTH_TYPE",
    "CACHE_HEADER_INCORRECT_DATA_START_ADDRESS_TYPE", "CACHE_HEADER_BAD_EYECATCHER_TYPE",
    "CACHE_HEADER_INCORRECT_CACHE_SIZE_TYPE", "ACQUIRE_HEADER_WRITE_LOCK_FAILED_TYPE", "CACHE_SIZE_INVALID_TYPE",
    "UPDATE_ROMCLASS_CORRUPT_CASE1_TYPE", "UPDATE_ROMCLASS_CORRUPT_CASE2_TYPE", "BAD_FREESPACE_DEBUG_AREA",
    "BAD_FREESPACE_SIZE_DEBUG_AREA", "BAD_LVT_BOUNDS_DEBUG_AREA", "BAD_LNT_BOUNDS_DEBUG_AREA", "CACHE_DATA_NULL_TYPE",
    "CACHE_SEMAPHORE_MISMATCH_TYPE", "CACHE_CCINITCOMPLETE_UNINITIALIZED", "NUM_CORRUPTION_TYPE",
    " ------- CORRUPTION TYPE END ------- " };
...
public:
    ...
    IDATA openTestCache(J9JavaVM* vm, I_32 cacheType, I_32 cacheSize, U_64 extraRunTimeFlag = 0,
        ...
        UDATA extraVerboseFlag = J9SHR_VERBOSEFLAG_ENABLE_VERBOSE_DEFAULT, const char* testName = "openTestCache",
        bool startupWillFail = false, bool doCleanupOnFail = true);
...
CorruptCacheTest::corruptCache(J9JavaVM* vm, I_32 cacheType, IDATA corruptionType)
{
    ...
    case CACHE_HEADER_BAD_EYECATCHER_TYPE:
        ...
        if (J9PORT_SHR_CACHE_TYPE_NONPERSISTENT == cacheType) {
            strncpy(((OSCachesysv_header_version_current*)header)->eyecatcher, INVALID_EYECATCHER,
                INVALID_EYECATCHER_LENGTH);
        } else {
            strncpy(((OSCachemmap_header_version_current*)header)->eyecatcher, INVALID_EYECATCHER,
                INVALID_EYECATCHER_LENGTH);
        }
        break;
    ...
    case ITEM_LENGTH_CORRUPT_AFTER_STARTUP_TYPE: {
        ...
        CCSETITEMLEN(ih, 0);
        cacheMap->enterLocalMutex(
            vm->mainThread, vm->classMemorySegments->segmentMutex, "class segment mutex", "corruptCache");
        cacheMap->runEntryPointChecks(vm->mainThread, NULL, NULL);
        cacheMap->exitLocalMutex(
            vm->mainThread, vm->classMemorySegments->segmentMutex, "class segment mutex", "corruptCache");
...
IDATA
CorruptCacheTest::openTestCache(J9JavaVM* vm, I_32 cacheType, I_32 cacheSize, U_64 extraRunTimeFlag,
    U_64 unsetTheseRunTimeFlag, UDATA extraVerboseFlag, const char* testName, bool startupWillFail,
    bool doCleanupOnFail)
{
    ...
    rc = cacheMap->startup(
        vm->mainThread, piConfig, BROKEN_TEST_CACHE, NULL, J9SH_DIRPERM_ABSENT, NULL, &cacheHasIntegrity);
        ...
        if (cc->isStarted()) {
            ...
            if (NULL != ca) {
                ...
                if (true == saveCRC) {
                    ...
                    /* clear J9SHR_RUNTIMEFLAG_DENY_CACHE_UPDATES flag in runtimeFlags which is set in the call to
                     * SH_CompositeCacheImpl::runExitCode() above. */
...
CorruptCacheTest::openCorruptCache(J9JavaVM* vm, I_32 cacheType, bool readOnly, I_32 cacheSize, U_64 extraRuntimeFlags)
{
    ...
    return this->openTestCache(vm, cacheType, cacheSize, extraRuntimeFlags, unsetRuntimeFlags, extraVerboseFlags,
        "openCorruptCache", startupWillFail, false);
}
...
CorruptCacheTest::findDummyROMClass(J9JavaVM* vm, const char* romClassName)
{
    ...
    cacheMap = (SH_CacheMap*)vm->sharedClassConfig->sharedClassCache;
    cacheMap->enterLocalMutex(
        vm->mainThread, vm->classMemorySegments->segmentMutex, "class segment mutex", "findDummyROMClass");
...
zeroOutCache(J9JavaVM* vm, I_32 cacheType)
{
    ...
    SH_OSCache::getCacheVersionAndGen(
        PORTLIB, vm, cacheName, J9SH_MAXPATH, BROKEN_TEST_CACHE, &versionData, OSCACHE_CURRENT_CACHE_GEN, true);
    ...
    mapFileHandle
        = j9mmap_map_file(fd, 0, (UDATA)cacheSize, fullPath, J9PORT_MMAP_FLAG_WRITE, J9MEM_CATEGORY_CLASSES_SHC_CACHE);
...
addDummyROMClass(J9JavaVM* vm, const char* romClassName, IDATA corruptionType)
{
    ...
    PORT_ACCESS_FROM_JAVAVM(vm);
    {
        SCStoreTransaction transaction(vm->mainThread, (J9ClassLoader*)NULL, 0 /*entryIndex*/, J9SHR_LOADTYPE_NORMAL,
            (U_16)strlen(romClassName), (U_8*)romClassName, false);
     ...
     * so that next call to SH_CacheMap::refreshHashTables() can detect corruption during
     * SH_CacheMap::updateROMSegmentListForCache().
     */
...
testCorruptCache(J9JavaVM* vm)
{
    ...
    for (i = 0; i < 6; i++) {
        ...
        case 4:
            ...
            cacheTypeString = "J9PORT_SHR_CACHE_TYPE_PERSISTENT";
            readOnly = false;
            extraRuntimeFlags |= (J9SHR_RUNTIMEFLAG_DISABLE_CORRUPT_CACHE_DUMPS | J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT
        ...
        if (0 != cacheType) {
            j9tty_printf(PORTLIB,
                ...
                cacheTypeString, readOnly, extraRuntimeFlags);
            ...
            for (corruptionType = CACHE_CRC_INVALID_TYPE; corruptionType < NUM_CORRUPTION_TYPE; corruptionType++) {
                ...
                j9tty_printf(PORTLIB, "\nCorrupt cache with corruption type: %d(%s)\n", corruptionType,
                    CorruptionTypeStrings[corruptionType]);
#if defined(J9SHR_CACHELET_SUPPORT)
                ...
                if (J9PORT_SHR_CACHE_TYPE_VMEM == cacheType) {
                    /* Cache CRC is not computed for realtime cache. Cachelets are not forced into memory until their
                     * content is required. Having a CRC for the cache will force the entire contents of the cache into
                     * memory when the CRC value is verified. Skip the check for CRC corruption type. Debug area is not
                     * used in realtime cache. Skip corruption types related to debug area.
                     */
                    if ((CACHE_CRC_INVALID_TYPE == corruptionType) || (BAD_FREESPACE_DEBUG_AREA == corruptionType)
                ...
                if (J9PORT_SHR_CACHE_TYPE_NONPERSISTENT == cacheType) {
                    if ((CACHE_HEADER_INCORRECT_CACHE_SIZE_TYPE == corruptionType)
                        || (CACHE_SIZE_INVALID_TYPE == corruptionType) || (CACHE_DATA_NULL_TYPE == corruptionType)) {
                ...
                if (UPDATE_ROMCLASS_CORRUPT_CASE1_TYPE == corruptionType) {
                    /*
                     * To simulate this case, we are going to add a corrupt ROMClass (i.e. set the ROMClass size to zero
                     * after adding it to cache), and the corruption will be caught in updateROMSegmentListForCache()
                     * called by the destructor of SCStoreTransaction.
                     *
                 ...
                 *
                 * Open the cache with J9SHR_RUNTIMEFLAG_ENABLE_STATS set to simulate using
                 * -Xshareclasses:printXXXStats. If the cache is deleted incorrectly during this, then the below call to
                 * verifyCorruptionContext() will not find the expected corruption context, because the cache was
                 * deleted.
                 *
                ...
                /* Set the flag to open the cache for stats so that cache is not recreated if corruption is detected
                 * during startup. This will ensure corruption context is preserved. Opening the cache for stats does
                 * not detect CACHE_SEMAPHORE_MISMATCH_TYPE corruption.
                 */
                if (CACHE_SEMAPHORE_MISMATCH_TYPE != corruptionType) {
#if defined(J9SHR_CACHELET_SUPPORT)
                    if ((WALK_ROMCLASS_CORRUPT_CASE1_TYPE == corruptionType)
                        || (WALK_ROMCLASS_CORRUPT_CASE2_TYPE == corruptionType)) {
                 ...
                 */
                if ((corruptionInfo[corruptionType].corruptionStage == DURING_STARTUP) && (false == readOnly)) {
                    /* Clear the flag for stats so that corrupt cache is removed and new cache is recreated in case
                     * corruption is detected during startup */
                    extraRuntimeFlags &= ~J9SHR_RUNTIMEFLAG_ENABLE_STATS;
                    corruptCacheTest.openCorruptCache(vm, cacheType, readOnly, cacheSize, extraRuntimeFlags);
