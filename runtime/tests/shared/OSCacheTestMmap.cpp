...
SH_OSCacheTestMmap::testBasic(J9PortLibrary* portLibrary, J9JavaVM* vm)
{
    ...
    osc = new (j9mem_allocate_memory(SH_OSCache::getRequiredConstrBytes(), J9MEM_CATEGORY_CLASSES)) SH_OSCachemmap(
        PORTLIB, vm, cacheDir, CACHE_NAME, piconfig, 1, J9SH_OSCACHE_CREATE, 1, 0, 0, &versionData, init);
    if (NULL == osc) {
...
SH_OSCacheTestMmap::testConstructor(J9PortLibrary* portLibrary, J9JavaVM* vm)
{
    ...
    osc = new (osc) SH_OSCachemmap(PORTLIB, vm, cacheDir, OSCACHETEST_CONSTRUCTOR_NAME, piconfig, 1,
 ...
 */
IDATA
SH_OSCacheTestMmap::testMultipleCreate(
    J9PortLibrary* portLibrary, J9JavaVM* vm, struct j9cmdlineOptions* arg, UDATA child)
{
    ...
    if (!child) {
        ...
        oscHandle = new (j9mem_allocate_memory(SH_OSCache::getRequiredConstrBytes(), J9MEM_CATEGORY_CLASSES))
            SH_OSCachemmap(PORTLIB, vm, cacheDir, OSCACHE_AREA_NAME, piconfig, 1, J9SH_OSCACHE_CREATE, 1, 0, 0,
                &versionData, NULL);
        ...
        childargc = buildChildCmdlineOption(
            argc, argv, (OSCACHETEST_CMDLINE_STARTSWITH OSCACHETESTMMAP_CMDLINE_MULTIPLECREATE), childargv);
    ...
    oscHandle = new (j9mem_allocate_memory(SH_OSCache::getRequiredConstrBytes(), J9MEM_CATEGORY_CLASSES))
        SH_OSCachemmap(PORTLIB, vm, cacheDir, OSCACHE_AREA_NAME, piconfig, 1, J9SH_OSCACHE_OPEXIST_DESTROY, 1, 0, 0,
            &versionData, NULL);
    oscHandle->destroy(false);
    CloseLaunchSemaphore(PORTLIB, semhandle);
...
#define NUMBER_OF_CACHES_TO_TEST 3
    ...
    for (UDATA i = 0; i < NUMBER_OF_CACHES_TO_TEST; i++) {
        ...
        tmposc = new (tmposc) SH_OSCachemmap(
            PORTLIB, vm, cacheDirName, cacheName[i], piconfig, 1, J9SH_OSCACHE_CREATE, 1, 0, 0, &versionData, NULL);
    ...
    for (UDATA i = 0; i < NUMBER_OF_CACHES_TO_TEST; i++) {
        ...
        osc[i] = new (osc[i]) SH_OSCachemmap(
            PORTLIB, vm, cacheDirName, cacheName[i], piconfig, 1, J9SH_OSCACHE_CREATE, 1, 0, 0, &versionData, NULL);
        if (osc[i]->getError() < 0) {
...
SH_OSCacheTestMmap::testMutex(J9PortLibrary* portLibrary, J9JavaVM* vm, struct j9cmdlineOptions* arg, UDATA child)
{
    ...
    if (NULL
        == (osc = (SH_OSCachemmap*)j9mem_allocate_memory(
    ...
    /* Parent */
    childargc = buildChildCmdlineOption(
        argc, argv, (OSCACHETEST_CMDLINE_STARTSWITH OSCACHETESTMMAP_CMDLINE_MUTEX), childargv);
...
SH_OSCacheTestMmap::testMutexHang(J9PortLibrary* portLibrary, J9JavaVM* vm, struct j9cmdlineOptions* arg, UDATA child)
{
    ...
    piconfig->sharedClassCacheSize = 10240;
    osc = new (osc) SH_OSCachemmap(PORTLIB, vm, cacheDir, OSCACHETEST_MUTEX_HANG_NAME, piconfig, 1, J9SH_OSCACHE_CREATE,
    ...
    /* Parent */
    childargc = buildChildCmdlineOption(
        argc, argv, (OSCACHETEST_CMDLINE_STARTSWITH OSCACHETESTMMAP_CMDLINE_MUTEXHANG), childargv);
...
SH_OSCacheTestMmap::testDestroy(J9PortLibrary* portLibrary, J9JavaVM* vm, struct j9cmdlineOptions* arg, UDATA child)
{
    ...
    childargc = buildChildCmdlineOption(
        argc, argv, (OSCACHETEST_CMDLINE_STARTSWITH OSCACHETESTMMAP_CMDLINE_DESTROY), childargv);
