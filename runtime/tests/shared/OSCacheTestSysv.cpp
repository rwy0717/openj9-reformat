...
SH_OSCacheTestSysv::testBasic(J9PortLibrary* portLibrary, J9JavaVM* vm)
{
    ...
    if (!osc->startup(vm, cacheDir, J9SH_DIRPERM_ABSENT, CACHE_NAME, piconfig, 1, J9SH_OSCACHE_CREATE, 0, 0, 0, 0,
            &versionData, initObj, SHR_STARTUP_REASON_NORMAL)) {
    ...
    osc1 = new (j9mem_allocate_memory(SH_OSCache::getRequiredConstrBytes(), J9MEM_CATEGORY_CLASSES)) SH_OSCachesysv(
        PORTLIB, vm, cacheDir, CACHE_NAME1, piconfig, 1, J9SH_OSCACHE_CREATE, 1, 0, 0, &versionData, initObj);
    if (NULL == osc1) {
        rc = FAIL;
        goto cleanup;
    }
 ...
 */
IDATA
SH_OSCacheTestSysv::testMultipleCreate(
    J9PortLibrary* portLibrary, J9JavaVM* vm, struct j9cmdlineOptions* arg, UDATA child)
{
    ...
    if (!child) {
        ...
        oscHandle = new (j9mem_allocate_memory(SH_OSCache::getRequiredConstrBytes(), J9MEM_CATEGORY_CLASSES))
            SH_OSCachesysv(PORTLIB, vm, cacheDir, OSCACHE_AREA_NAME, piconfig, 1, J9SH_OSCACHE_CREATE, 1, 0, 0,
                &versionData, NULL);
        ...
        childargc = buildChildCmdlineOption(
            argc, argv, (OSCACHETEST_CMDLINE_STARTSWITH OSCACHETEST_CMDLINE_MULTIPLECREATE), childargv);
        ...
        for (UDATA i = 0; i < MAX_PROCESS; i++) {
            ...
            default:
                j9tty_printf(PORTLIB,
                    "testMultipleCreate: Unknown return value from WaitForTestProcess(): failed (procrc = %p)\n",
                    procrc);
                rc = FAIL;
                break;
            }
        ...
        case 0:
            j9tty_printf(
                PORTLIB, "testMultipleCreate: None of the children created the OSCache: failed (opened %u)\n", opened);
            rc = FAIL;
            break;
        case 1:
    ...
    oscHandle = new (j9mem_allocate_memory(SH_OSCache::getRequiredConstrBytes(), J9MEM_CATEGORY_CLASSES))
        SH_OSCachesysv(PORTLIB, vm, cacheDir, OSCACHE_AREA_NAME, piconfig, 1, J9SH_OSCACHE_OPEXIST_DESTROY, 1, 0, 0,
            &versionData, NULL);
    oscHandle->destroy(false);
    CloseLaunchSemaphore(PORTLIB, semhandle);
...
SH_OSCacheTestSysv::testConstructor(J9PortLibrary* portLibrary, J9JavaVM* vm)
{
    ...
    osc = new (osc) SH_OSCachesysv(PORTLIB, vm, cacheDir, OSCACHETEST_CONSTRUCTOR_NAME, piconfig, 1,
        J9SH_OSCACHE_CREATE, 1, 0, 0, &versionData, NULL);
    ...
    osc = new (osc) SH_OSCachesysv(PORTLIB, vm, cacheDir, OSCACHETEST_CONSTRUCTOR_NAME, piconfig, 1,
        J9SH_OSCACHE_OPEXIST_DESTROY, 1, 0, 0, &versionData, NULL);
    if (osc->getError() > 0) {
        j9tty_printf(PORTLIB,
            "testConstructor: opening existing shared memory area with J9SH_OSCACHE_OPEXIST_DESTROY should fail\n");
        rc = FAIL;
    }
...
#define NUMBER_OF_CACHES_TO_TEST 3
    ...
    for (UDATA i = 0; i < NUMBER_OF_CACHES_TO_TEST; i++) {
        ...
        if (NULL == tmposc) {
            j9tty_printf(PORTLIB,
                "testGetAllCacheStatistics: clean up old caches : failed to allocate memory for cache %zu\n", i);
            goto loop_cleanup;
        }

        tmposc = new (tmposc) SH_OSCachesysv(
            PORTLIB, vm, testDir, cacheName[i], piconfig, 1, J9SH_OSCACHE_CREATE, 1, 0, 0, &versionData, NULL);
        if (tmposc->getError() < 0) {
            j9tty_printf(PORTLIB,
                "testGetAllCacheStatistics: clean up old caches : failed to create create cache %zu (getError() "
                "=%zd)\n",
                i, tmposc->getError());
            goto loop_cleanup;
        }
    ...
    cacheStat = SH_OSCache::getAllCacheStatistics(
        vm, ctrlDirName, 0, 1, J2SE_LATEST, false, false, SHR_STATS_REASON_TEST, true);
    ...
    for (UDATA i = 0; i < NUMBER_OF_CACHES_TO_TEST; i++) {
        ...
        osc[i] = new (osc[i]) SH_OSCachesysv(
            PORTLIB, vm, testDir, cacheName[i], piconfig, 1, J9SH_OSCACHE_CREATE, 1, 0, 0, &versionData, NULL);
        if (osc[i]->getError() < 0) {
    ...
    if (cacheStat == NULL) {
        j9tty_printf(PORTLIB,
            "testGetAllCacheStatistics: failed to get cache statistics - there should be at least one cache!\n");
        goto cleanup;
    }
...
SH_OSCacheTestSysv::testMutex(J9PortLibrary* portLibrary, J9JavaVM* vm, struct j9cmdlineOptions* arg, UDATA child)
{
    ...
    if (NULL
        == (osc = (SH_OSCachesysv*)j9mem_allocate_memory(
                SH_OSCache::getRequiredConstrBytes(), J9MEM_CATEGORY_CLASSES))) {
    ...
    piconfig->sharedClassCacheSize = SHM_REGIONSIZE;
    osc = new (osc) SH_OSCachesysv(
        PORTLIB, vm, cacheDir, OSCACHE_TEST_MUTEX_NAME, piconfig, 1, J9SH_OSCACHE_CREATE, 1, 0, 0, &versionData, NULL);
    if (osc->getError() < 0) {
    ...
    /* Parent */
    childargc
        = buildChildCmdlineOption(argc, argv, (OSCACHETEST_CMDLINE_STARTSWITH OSCACHETEST_CMDLINE_MUTEX), childargv);
...
SH_OSCacheTestSysv::testMutexHang(J9PortLibrary* portLibrary, J9JavaVM* vm, struct j9cmdlineOptions* arg, UDATA child)
{
    ...
    piconfig->sharedClassCacheSize = 10240;
    osc = new (osc) SH_OSCachesysv(PORTLIB, vm, cacheDir, OSCACHETEST_MUTEX_HANG_NAME, piconfig, 1, J9SH_OSCACHE_CREATE,
        1, 0, 0, &versionData, NULL);
    ...
    /* Parent */
    childargc = buildChildCmdlineOption(
        argc, argv, (OSCACHETEST_CMDLINE_STARTSWITH OSCACHETEST_CMDLINE_MUTEXHANG), childargv);
...
SH_OSCacheTestSysv::testSize(J9PortLibrary* portLibrary, J9JavaVM* vm)
{
    ...
    piconfig->sharedClassCacheSize = 10240;
    osc = new (osc) SH_OSCachesysv(
        PORTLIB, vm, cacheDir, OSCACHETEST_SIZE_NAME, piconfig, 1, J9SH_OSCACHE_CREATE, 1, 0, 0, &versionData, NULL);
    if (osc->getError() >= 0) {
    ...
    if (J9PORT_LIMIT_LIMITED == getLimitRc) {
        ...
        j9tty_printf(PORTLIB,
            "testSize: rc %u from j9sysinfo_get_limit\n"
    ...
    if (shmmax > (U_64)maxint32) {
        /* CMVC 143434: If shmmax is greater than max int. Then we set it to be max int. This may happen on 64 bit
         * systems. For example: */
        /* lnxem64tvm1% more /proc/sys/kernel/shmmax */
        /* 		3294967296 */
        j9tty_printf(PORTLIB, "testSize: shmmax was %lld setting to max int instead.\n", shmmax);
        shmmax = maxint32;
    }
    ...
    for (i = 0; i < NO_SIZE_TESTS; i++) {
        ...
        osc = new (osc) SH_OSCachesysv(PORTLIB, vm, cacheDir, OSCACHETEST_SIZE_NAME, piconfig, 1, J9SH_OSCACHE_CREATE,
            1, 0, 0, &versionData, NULL);
        ...
        if (osc->getError() < 0) {
            ...
            if ((i == 0) && (defaultSize > 1024) && (defaultSize < shmmax)) {
                ...
                j9tty_printf(PORTLIB,
                    "testSize: shmmax (%lld Bytes) was to big, retrying with default size(%d Bytes).\n", shmmax,
                    defaultSize);
                i = 0;
...
SH_OSCacheTestSysv::testDestroy(J9PortLibrary* portLibrary, J9JavaVM* vm, struct j9cmdlineOptions* arg, UDATA child)
{
    ...
    piconfig->sharedClassCacheSize = 10240;
    osc = new (osc) SH_OSCachesysv(
        PORTLIB, vm, cacheDir, OSCACHETEST_DESTROY_NAME, piconfig, 1, J9SH_OSCACHE_CREATE, 1, 0, 0, &versionData, NULL);
    ...
    childargc
        = buildChildCmdlineOption(argc, argv, (OSCACHETEST_CMDLINE_STARTSWITH OSCACHETEST_CMDLINE_DESTROY), childargv);
