...
IDATA
OpenCacheHelper::openTestCache(I_32 cacheType, I_32 cacheSize, const char* cacheName, bool inMemoryCache,
    BlockPtr cacheBuffer, char* cacheDir, const char* cacheDirPermStr, U_64 extraRunTimeFlag,
    U_64 unsetTheseRunTimeFlag, IDATA unitTest, UDATA extraVerboseFlag, const char* testName, bool startupWillFail,
    bool doCleanupOnFail, J9SharedClassPreinitConfig* cacheDetails, J9SharedClassConfig* sharedclassconfig)
{
    IDATA rc = 0;
    ...
    if (true == inMemoryCache) {
        if (NULL == cacheBuffer) {
            if (NULL == cacheMemory) {
                cacheAllocated = (BlockPtr)j9mem_allocate_memory(
                    piConfig->sharedClassCacheSize + osPageSize, J9MEM_CATEGORY_CLASSES);
    ...
    rc = cacheMap->startup(
        vm->mainThread, piConfig, this->cacheName, this->cacheDir, cacheDirPerm, cacheMemory, &cacheHasIntegrity);
...
void OpenCacheHelper::setRomClassName(J9ROMClass* rc, const char* name)
{
