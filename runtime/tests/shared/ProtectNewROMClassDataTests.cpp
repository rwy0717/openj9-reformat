...
public:
    ...
    static UDATA getRequiredConstrBytes(void) { return sizeof(FakeOSCache); }
...
IDATA testProtectNewROMClassData_test1(J9JavaVM* vm)
{
    ...
    if (NULL
        == (OSCacheMem = (char*)j9mem_allocate_memory(FakeOSCache::getRequiredConstrBytes(), J9MEM_CATEGORY_CLASSES))) {
    ...
    if (cachehelper.openTestCache(J9PORT_SHR_CACHE_TYPE_PERSISTENT, CACHE_SIZE, NULL, true, NULL, NULL, NULL,
    ...
    if (myfakeoscache->protectedLength != (romClassSize - (romClassSize % pageSizeToUse))) {
        ERRPRINTF2(
            "After calling addDummyROMClass() the incorrect size was protected. Expected %u, but %u was protected",
    ...
    *updateCountAddress += 1;
    ...
    if (myfakeoscache->protectedLength != (romClassSize - (romClassSize % pageSizeToUse))) {
        ERRPRINTF2(
            "After calling doneReadUpdates() the incorrect size was protected. Expected %u, but %u was protected",
            (romClassSize - (romClassSize % pageSizeToUse)), myfakeoscache->protectedLength);
        rc = FAIL;
        goto done;
    }
    ...
    if (changeInCCRomClassProtectEnd != romClassSize) {
        ERRPRINTF2("After calling doneReadUpdates() verification of getRomClassProtectEnd() has failed."
            ...
            romClassSize, changeInCCRomClassProtectEnd);
        rc = FAIL;
        goto done;
    }
