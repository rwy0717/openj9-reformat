...
public:
    ...
    static UDATA getRequiredConstrBytes(void) { return sizeof(ProtectSharedCacheDataOSCache); }
...
testProtectSharedCacheData_test1(J9JavaVM* vm)
{
    ...
    U_64 unsetProtectRunTimeFlags = J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT_ONFIND
        | J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT_PARTIAL_PAGES | J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT_RW;
#else
    U_64 protectRunTimeFlags = J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT | J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT_RW;
    U_64 unsetProtectRunTimeFlags
        = J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT_ONFIND | J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT_PARTIAL_PAGES;
    ...
    if (NULL
        == (osCacheMem = (char*)j9mem_allocate_memory(
                ProtectSharedCacheDataOSCache::getRequiredConstrBytes(), J9MEM_CATEGORY_CLASSES))) {
    ...
    if (J9VM_PHASE_NOT_STARTUP == vm->phase) {
        ...
        if (myfakeoscache->unprotectedLength != expectedUnprotectedLength) {
            ERRPRINTF2("After calling addDummyROMClass() the incorrect size was unprotected. Expected %zu, but %zu was "
                       "unprotected",
    ...
    expectedProtectedLength = (ROUND_DOWN_TO(pageSizeToUse, (UDATA)currentSegmentPtr)
                                  - ROUND_DOWN_TO(pageSizeToUse, (UDATA)beforeSegmentPtr))
        + (ROUND_UP_TO(pageSizeToUse, (UDATA)beforeMetadataPtr)
              - ROUND_UP_TO(pageSizeToUse, (UDATA)currentMetadataPtr));
    if (myfakeoscache->protectedLength != expectedProtectedLength) {
        ERRPRINTF2(
            "After calling addDummyROMClass() the incorrect size was protected. Expected %zu, but %zu was protected",
    ...
    if (myfakeoscache->protectedLength != expectedProtectedLength) {
        ERRPRINTF2(
            "After calling runEntryPointChecks() the incorrect size was protected. Expected %zu, but %zu was protected",
            expectedProtectedLength, myfakeoscache->protectedLength);
        rc = FAIL;
        goto done;
    }
    ...
    /* Test 3: Test protecting new data in metadata region. Store AOT data of 2.5 pages in size */
    {
        ...
        storedMethod = (U_8*)vm->sharedClassConfig->storeCompiledMethod(
            vm->mainThread, romMethod, data, dataSize, NULL, 0, FALSE);
        ...
        if (J9VM_PHASE_NOT_STARTUP == vm->phase) {
            ...
            expectedUnprotectedLength = 0;
            if (myfakeoscache->unprotectedLength != expectedUnprotectedLength) {
                ERRPRINTF2("After calling storeCompiledMethod() the incorrect size was unprotected. Expected %zu, but "
                           "%zu was unprotected",
                    expectedUnprotectedLength, myfakeoscache->unprotectedLength);
                rc = FAIL;
                goto done;
        ...
        expectedProtectedLength = ROUND_UP_TO(pageSizeToUse, (UDATA)beforeMetadataPtr)
            - ROUND_UP_TO(pageSizeToUse, (UDATA)currentMetadataPtr);
        if (myfakeoscache->protectedLength != expectedProtectedLength) {
            ERRPRINTF2("After calling storeCompiledMethod() the incorrect size was protected. Expected %zu, but %zu "
                       "was protected",
                expectedProtectedLength, myfakeoscache->protectedLength);
            rc = FAIL;
            goto done;
    ...
    /* ::runEntryPointChecks() is responsible for mprotecting new ROMClasses and metadata added by other JVMs and the
    ...
    expectedUnprotectedLength
        = ROUND_UP_TO(pageSizeToUse, (UDATA)metadataStartPtr) - ROUND_DOWN_TO(pageSizeToUse, (UDATA)currentMetadataPtr);
    if (J9VM_PHASE_NOT_STARTUP == vm->phase) {
        if (myfakeoscache->unprotectedLength != expectedUnprotectedLength) {
            ERRPRINTF2(
                "After locking the cache the incorrect size was unprotected. Expected %zu, but %zu was unprotected",
                expectedUnprotectedLength, myfakeoscache->unprotectedLength);
            rc = FAIL;
            goto done;
        }
    ...
    expectedProtectedLength = (ROUND_DOWN_TO(pageSizeToUse, (UDATA)currentSegmentPtr)
                                  - ROUND_DOWN_TO(pageSizeToUse, (UDATA)segmentStartPtr))
        + (ROUND_UP_TO(pageSizeToUse, (UDATA)metadataStartPtr) - ROUND_UP_TO(pageSizeToUse, (UDATA)currentMetadataPtr));
    if (myfakeoscache->protectedLength != expectedProtectedLength) {
        ERRPRINTF2(
            "After calling openTestCache() the incorrect size was protected. Expected %zu, but %zu was protected",
            expectedProtectedLength, myfakeoscache->protectedLength);
        rc = FAIL;
        goto done;
    }
...
testProtectSharedCacheData_test2(J9JavaVM* vm)
{
    ...
    expectedProtectedLength
        = (ROUND_UP_TO(pageSizeToUse, (UDATA)currentSegmentPtr) - ROUND_DOWN_TO(pageSizeToUse, (UDATA)beforeSegmentPtr))
        + (ROUND_UP_TO(pageSizeToUse, (UDATA)beforeMetadataPtr)
              - ROUND_DOWN_TO(pageSizeToUse, (UDATA)currentMetadataPtr));
    if (myfakeoscache->protectedLength != expectedProtectedLength) {
        ERRPRINTF2(
            "After calling addDummyROMClass() the incorrect size was protected. Expected %zu, but %zu was protected",
            expectedProtectedLength, myfakeoscache->protectedLength);
    ...
    myfakeoscache->resetTestVars();
    cc->enterWriteMutex(vm->mainThread, true, NULL); /* locks the cache */
    /* ::runEntryPointChecks() is responsible for mprotecting new ROMClasses added by other JVMs and the partially
     * filled pages. If the cache is locked, then partially filled page in metadata is not protected.
     */
    cacheMap->runEntryPointChecks(vm->mainThread, NULL, NULL);
    /* When cache is unlocked, whole of metadata including partial pages are protected back */
    cc->exitWriteMutex(vm->mainThread, NULL);
    ...
    expectedProtectedLength
        = (ROUND_UP_TO(pageSizeToUse, (UDATA)currentSegmentPtr) - ROUND_DOWN_TO(pageSizeToUse, (UDATA)segmentStartPtr))
        + (ROUND_UP_TO(pageSizeToUse, (UDATA)metadataStartPtr)
              - ROUND_DOWN_TO(pageSizeToUse, (UDATA)currentMetadataPtr));
    if (myfakeoscache->protectedLength != expectedProtectedLength) {
        ERRPRINTF2(
            "After calling openTestCache() the incorrect size was protected. Expected %zu, but %zu was protected",
...
testProtectSharedCacheData_test3(J9JavaVM* vm)
{
    ...
    /* When reading updates, if J9SHR_RUNTIMEFLAG_ENABLE_MPROTECT_ON_FIND is hen partially filled pages are not
     * mprotected. */
    expectedProtectedLength
        = (ROUND_UP_TO(pageSizeToUse, (UDATA)currentSegmentPtr) - ROUND_DOWN_TO(pageSizeToUse, (UDATA)segmentStartPtr))
        + (ROUND_UP_TO(pageSizeToUse, (UDATA)metadataStartPtr)
