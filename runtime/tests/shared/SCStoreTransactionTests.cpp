...
static IDATA AddClassToCacheHelper(J9VMThread* currentThread, J9ClassLoader* classloader, UDATA entryIndex,
    UDATA loadType, U_16 classnameLength, U_8* classnameData, U_32 romclassSize);
...
testSCStoreTransaction(J9JavaVM* vm)
{
    ...
    if (0 < ClassDebugDataProvider::getRecommendedPercentage()) {
        ...
        j9tty_printf(
            PORTLIB, "%s: Skipping some tests b/c the cache has class debug area size of 0 bytes.\n", testName);
    }
    ...
    UDATA loadType, U_16 classnameLength, U_8* classnameData, U_32 romclassSize)
{
    ...
    SCStoreTransaction transaction(currentThread->javaVM->mainThread,
        (J9ClassLoader*)currentThread->javaVM->systemClassLoader, entryIndex, loadType, classnameLength, classnameData,
        false);
...
static IDATA test1(J9JavaVM* vm)
{
    ...
    SCStoreTransaction transaction(vm->mainThread, (J9ClassLoader*)NULL, 0, J9SHR_LOADTYPE_NORMAL,
...
static IDATA test2(J9JavaVM* vm)
{
    ...
    for (romclass = transaction.nextSharedClassForCompare(); romclass != NULL;
         romclass = transaction.nextSharedClassForCompare()) {
        foundMatch = TRUE;
    }
...
static IDATA test4(J9JavaVM* vm)
{
    ...
    SCStoreTransaction transaction(vm->mainThread, (J9ClassLoader*)vm->systemClassLoader, 0 /*entryIndex*/,
        J9SHR_LOADTYPE_NORMAL, (U_16)strlen(romclassName), (U_8*)romclassName, false);
    if (transaction.isOK() == false) {
        j9tty_printf(PORTLIB, "could not allocate the transaction object\n");
        return TEST_ERROR;
    }
...
static IDATA test6(J9JavaVM* vm)
{
    ...
    SCStoreTransaction transaction(vm->mainThread, (J9ClassLoader*)vm->systemClassLoader, 0 /*entryIndex*/,
        J9SHR_LOADTYPE_NORMAL, (U_16)strlen(test5RomclassName), (U_8*)test5RomclassName, false);
    if (transaction.isOK() == false) {
        j9tty_printf(PORTLIB, "could not allocate the transaction object\n");
        return TEST_ERROR;
    }
    ...
    for (romclass = transaction.nextSharedClassForCompare(); romclass != NULL;
        ...
        j9tty_printf(PORTLIB, "\tInfo: Found J9ROMClass named: %.*s\n", J9UTF8_LENGTH(classnameUTF8),
            J9UTF8_DATA(classnameUTF8));
        foundMatch = TRUE;
    }
...
static IDATA test8(J9JavaVM* vm)
{
    ...
    if (AddClassToCacheHelper(vm->mainThread, (J9ClassLoader*)vm->systemClassLoader, entryIndex1, J9SHR_LOADTYPE_NORMAL,
            (U_16)strlen(romclassName), (U_8*)romclassName, romclassSize)
        == TEST_ERROR) {
        retval = TEST_ERROR;
        goto done;
    }
    ...
    do {
        ...
        SCStoreTransaction transaction(vm->mainThread, (J9ClassLoader*)vm->systemClassLoader, entryIndex2,
            J9SHR_LOADTYPE_NORMAL, (U_16)strlen(romclassName), (U_8*)romclassName, false);
        if (transaction.isOK() == false) {
    ...
    do {
        ...
        SCStoreTransaction transaction(vm->mainThread, (J9ClassLoader*)vm->systemClassLoader, entryIndex3,
            J9SHR_LOADTYPE_NORMAL, (U_16)strlen(romclassName), (U_8*)romclassName, false);
        if (transaction.isOK() == false) {
            j9tty_printf(PORTLIB, "could not allocate the transaction object\n");
            retval = TEST_ERROR;
...
static IDATA test9(J9JavaVM* vm)
{
    ...
    /* **Add 1st class** */
    j9tty_printf(PORTLIB, "\tAdding %s against index %d\n", romclassName, entryIndex1);
    if (AddClassToCacheHelper(vm->mainThread, (J9ClassLoader*)NULL, entryIndex1, J9SHR_LOADTYPE_NORMAL,
            (U_16)strlen(romclassName), (U_8*)romclassName, romclassSize)
        == TEST_ERROR) {
        retval = TEST_ERROR;
        goto done;
    ...
    do {
        /*NOTE: doing this in a do/while to ensure destructor/commit runs ...*/
        j9tty_printf(PORTLIB, "\tAdding %s against index %d\n", romclassName, entryIndex2);
        SCStoreTransaction transaction(vm->mainThread, (J9ClassLoader*)NULL, entryIndex2, J9SHR_LOADTYPE_NORMAL,
            (U_16)strlen(romclassName), (U_8*)romclassName, false);
        if (transaction.isOK() == false) {
            ERRPRINTF(("could not allocate the transaction object\n"));
            retval = TEST_ERROR;
    ...
    do {
        ...
        /*NOTE: doing this in a do/while to ensure destructor/commit runs ...*/
        j9tty_printf(PORTLIB, "\tChecking cache contents\n");
        SCStoreTransaction transaction(vm->mainThread, (J9ClassLoader*)NULL, entryIndex3, J9SHR_LOADTYPE_NORMAL,
            (U_16)strlen(romclassName), (U_8*)romclassName, false);
        if (transaction.isOK() == false) {
            j9tty_printf(PORTLIB, "could not allocate the transaction object\n");
            retval = TEST_ERROR;
...
static IDATA test10(J9JavaVM* vm)
{
    ...
    j9tty_printf(PORTLIB,
        "%s: allocate a non-orphan orphan rom class (with pretend modified bytecodes and no context)\n", testName);

    j9tty_printf(PORTLIB, "\t%s: store class %s\n", testName, romclassName);
    SCStoreTransaction transaction(vm->mainThread, (J9ClassLoader*)vm->systemClassLoader, 0 /*entryIndex*/,
        J9SHR_LOADTYPE_NORMAL, (U_16)strlen(romclassName), (U_8*)romclassName, true);
    if (transaction.isOK() == false) {
        j9tty_printf(PORTLIB, "could not allocate the transaction object\n");
        return TEST_ERROR;
    }
...
static IDATA test11(J9JavaVM* vm)
{
    ...
    PORT_ACCESS_FROM_JAVAVM(vm);
    j9tty_printf(PORTLIB,
        "%s: allocate a non-orphan orphan rom class, then store the same class again but pretend byte codes are "
        "modified\n",
        testName);
...
static IDATA test14(J9JavaVM* vm)
{
    ...
    memset((void*)&sizes, 0, sizeof(J9RomClassRequirements));
    j9tty_printf(PORTLIB,
        "%s: allocate a non-orphan class, set the cache full bit, then try to find the rom class in the cache\n",
        testName);
...
static IDATA test15(J9JavaVM* vm)
{
    ...
    memset((void*)&sizes, 0, sizeof(J9RomClassRequirements));
    j9tty_printf(PORTLIB,
        "%s: allocate an orphan rom class, set cache full bit, then try to update the orphan rom class's metadata\n",
        testName);
    ...
    newROMClass = sharedapi->jclUpdateROMClassMetaData(
        vm->mainThread, (J9ClassLoader*)vm->applicationClassLoader, NULL, 1, 0 /*entryIndex*/, NULL, romclass);
...
static IDATA test16(J9JavaVM* vm)
{
    ...
    memset((void*)&sizes, 0, sizeof(J9RomClassRequirements));
    j9tty_printf(PORTLIB,
        "%s: fill the cache with non-orphan rom class larger than the cache, verify cache full bit is not set, "
        "allocate non-orphan rom class larger than the cache\n",
        testName);
...
static IDATA test17(J9JavaVM* vm)
{
    ...
    memset((void*)&sizes, 0, sizeof(J9RomClassRequirements));
    j9tty_printf(PORTLIB,
        "%s: fill the cache with orphan rom class larger than the cache, verify cache full bit is not set, allocate "
        "orphan rom class larger than the cache\n",
        testName);
...
static IDATA test18(J9JavaVM* vm)
{
    ...
    memset((void*)&sizes, 0, sizeof(J9RomClassRequirements));
    j9tty_printf(PORTLIB,
        "%s: allocate non-orphan rom class, fill the cache with non-orphan rom class larger than the size of cache\n",
        testName);
...
static IDATA test19(J9JavaVM* vm)
{
    ...
    memset((void*)&sizes, 0, sizeof(J9RomClassRequirements));
    j9tty_printf(PORTLIB,
        "%s: allocate orphan rom class, then allocate another rom class of size large than the cache, update metadata "
        "of first orphan rom class\n",
        testName);
    ...
    {
        SCStoreTransaction transaction(vm->mainThread, (J9ClassLoader*)vm->systemClassLoader, 0 /*entryIndex*/,
            J9SHR_LOADTYPE_NORMAL, (U_16)strlen(testclassName), (U_8*)testclassName, false);
        if (transaction.isOK() == false) {
            j9tty_printf(PORTLIB, "could not allocate the transaction object\n");
            retval = TEST_ERROR;
            goto done;
...
static IDATA test20(J9JavaVM* vm)
{
    ...
    PORT_ACCESS_FROM_JAVAVM(vm);
    j9tty_printf(PORTLIB,
        "%s: allocate a non-orphan orphan rom class in the cache, but WITHOUT the string table lock\n", testName);
     ...
     */
    sharedapi->classStoreTransaction_start((void*)&tobj, vm->mainThread, (J9ClassLoader*)vm->systemClassLoader,
        vm->systemClassLoader->classPathEntries, vm->systemClassLoader->classPathEntryCount, 0, J9SHR_LOADTYPE_NORMAL,
        NULL, (U_16)strlen(romclassName), (U_8*)romclassName, FALSE, takeStringTableLock);
    ...
    sizes.romClassSizeFullSize = romclassSize;
    sizes.romClassMinimalSize = sizes.romClassSizeFullSize;
    if (sharedapi->classStoreTransaction_createSharedClass((void*)&tobj, (const J9RomClassRequirements*)&sizes, &pieces)
        != 0) {
...
static IDATA test21(J9JavaVM* vm)
{
    ...
    j9tty_printf(
        PORTLIB, "\t\tInfo:j9shr_Query_IsCacheFull()=%d and %d was expected\n", (IDATA)apiResult, (IDATA)cacheIsFull);
...
static IDATA test22(J9JavaVM* vm)
{
    ...
    } tests[] = { { cache->cacheStartAddress, 0, TRUE }, { cache->cacheStartAddress, cache->cacheSizeBytes, TRUE },
        { cache->cacheStartAddress, cache->cacheSizeBytes - 1, TRUE },
        { cache->cacheStartAddress, cache->cacheSizeBytes + 1, FALSE }, { NULL, cache->cacheSizeBytes, FALSE } };
    ...
    for (UDATA i = 0; i < numTests; i++) {
        ...
        if (addressInCache != tests[i].expectedResult) {
            ERRPRINTF4("%u. j9shr_Query_IsAddressInCache(vm, %p, %u) returned %s\n", i, tests[i].address,
                tests[i].length, addressInCache ? "TRUE" : "FALSE");
            retval = TEST_ERROR;
            goto done;
        }
    }
...
static IDATA test23(J9JavaVM* vm)
{
    ...
    PORT_ACCESS_FROM_JAVAVM(vm);
    j9tty_printf(PORTLIB,
        "%s: Alloc 4096 B orphan rom class, 4096 B of line number info, and 4096 B of var table info\n", testName);
...
static IDATA test24(J9JavaVM* vm)
{
    ...
    PORT_ACCESS_FROM_JAVAVM(vm);
    j9tty_printf(PORTLIB,
        "%s: Alloc 4096 B non-orphan rom class, 4096 B of line number info, and 4096 B of var table info\n", testName);
...
static IDATA test25(J9JavaVM* vm)
{
    ...
    PORT_ACCESS_FROM_JAVAVM(vm);
    j9tty_printf(PORTLIB,
        "%s: Verify hasAllocatedFullSizeRomClass(), getLineNumberTable(), and getLocalVariableTable() with 0 bytes of "
        "debug attribute data\n",
        testName);
...
static IDATA test29(J9JavaVM* vm)
{
    ...
    {
        SCStoreTransaction transaction(vm->mainThread, (J9ClassLoader*)NULL, 0 /*entryIndex*/, J9SHR_LOADTYPE_NORMAL,
            (U_16)strlen(romclassName1), (U_8*)romclassName1, false);
        if (false == transaction.isOK()) {
            j9tty_printf(PORTLIB, "could not allocate the transaction object\n");
            return TEST_ERROR;
        }
    ...
    {
        SCStoreTransaction transaction(vm->mainThread, (J9ClassLoader*)NULL, 0 /*entryIndex*/, J9SHR_LOADTYPE_NORMAL,
            (U_16)strlen(romclassName2), (U_8*)romclassName2, false);
        if (false == transaction.isOK()) {
            j9tty_printf(PORTLIB, "could not allocate the transaction object\n");
            return TEST_ERROR;
        }
        ...
        if (firstLineNumberTableAddr == transaction.getLineNumberTable()) {
            ERRPRINTF("Allocated debug data memory is incorrect: firstLineNumberTableAddr == "
                      "transaction.getLineNumberTable()\n");
            retval = TEST_ERROR;
            goto done;
        }
        if (firstLocalVariableTableAddr == transaction.getLocalVariableTable()) {
            ERRPRINTF("Allocated debug data memory is incorrect: firstLocalVariableTableAddr == "
                      "transaction.getLocalVariableTable()\n");
            retval = TEST_ERROR;
            goto done;
        }
