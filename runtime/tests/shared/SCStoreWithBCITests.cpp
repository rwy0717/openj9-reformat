...
static IDATA testRunner(J9JavaVM* vm, struct TestInfo testinfo)
{
    ...
    if (true == testinfo.useSysClassloader) {
        j9tty_printf(
            PORTLIB, "%s: allocate a rom class with %u bytes of debug data\n", testName, testinfo.debugDataSize);
        classloader = (J9ClassLoader*)vm->systemClassLoader;
    } else {
        j9tty_printf(
            PORTLIB, "%s: allocate a orphan rom class with %u bytes of debug data\n", testName, testinfo.debugDataSize);
    }
    SCStoreTransaction transaction(vm->mainThread, classloader, 0 /*entryIndex*/, J9SHR_LOADTYPE_NORMAL,
