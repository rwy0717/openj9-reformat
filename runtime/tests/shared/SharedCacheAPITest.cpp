...
void getCacheDir(J9JavaVM* vm, char* cacheDir)
{
...
void populateCacheInfoList(J9JavaVM* vm, J9SharedClassConfig* sharedClassConfig, const char* cacheName, char* cacheDir)
{
    IDATA i;
...
createTestCache(J9JavaVM* vm, J9SharedClassPreinitConfig* piconfig, J9SharedClassConfig* sharedClassConfig)
{
    ...
    for (i = 0; i < NUM_CACHE; i++) {
        ...
        sharedClassConfig->runtimeFlags = J9SHR_RUNTIMEFLAG_ENABLE_LOCAL_CACHEING
            | J9SHR_RUNTIMEFLAG_ENABLE_TIMESTAMP_CHECKS | J9SHR_RUNTIMEFLAG_ENABLE_REDUCE_STORE_CONTENTION
            | J9SHR_RUNTIMEFLAG_ENABLE_BYTECODEFIX;

        sharedClassConfig->ctrlDirName = cacheInfoList[i].cacheDir;
        sharedClassConfig->cacheDescriptorList = (J9SharedClassCacheDescriptor*)j9mem_allocate_memory(
        ...
        cacheMap[i]
            = SH_CacheMap::newInstance(vm, sharedClassConfig, (SH_CacheMap*)memory, cacheInfoList[i].name, cacheType);

        bool cacheHasIntegrity;
        rc = cacheMap[i]->startup(vm->mainThread, piconfig, cacheInfoList[i].name, cacheInfoList[i].cacheDir,
            J9SH_DIRPERM_ABSENT, NULL, &cacheHasIntegrity);
        if (rc != 0) {
    ...
    if (TRUE == displayCacheInfo) {
        ...
        for (i = 0; i < (NUM_CACHE + NUM_SNAPSHOT); i++) {
            if ((strcmp(cacheInfo->name, cacheInfoList[i].name) == 0)
                && (cacheInfo->cacheType == cacheInfoList[i].cacheType)) {
                testCacheCount++;
            }
        }
    }
...
validateSharedCacheCallback(J9JavaVM* vm, J9SharedCacheInfo* cacheInfo, void* userData)
{
    ...
    if (J9PORT_SHR_CACHE_TYPE_PERSISTENT == cacheInfo->cacheType) {
        ...
        if ((strcmp(cacheInfo->name, cacheInfoList[NUM_CACHE].name) == 0)
            ...
            && (J9SH_OSCACHE_UNKNOWN == (IDATA)cacheInfo->isCorrupt) && (1 == cacheInfo->isCompatible)
            && (addrMode == cacheInfo->addrMode) && (getShcModlevelForJCL(J2SE_VERSION(vm)) == cacheInfo->modLevel)
            && (J9SH_OSCACHE_UNKNOWN == cacheInfo->lastDetach) && (J9SH_OSCACHE_UNKNOWN == (IDATA)cacheInfo->os_shmid)
    ...
    for (i = 0; i < NUM_CACHE; i++) {
        if (strcmp(cacheInfo->name, cacheInfoList[i].name) == 0) {
            if ((cacheInfo->cacheType != cacheInfoList[i].cacheType) || (cacheInfo->isCorrupt != 0)
                || (cacheInfo->isCompatible != 1) || (cacheInfo->cacheSize != cacheSize)
                || (CACHE_SIZE != cacheInfo->softMaxBytes)
                || (cacheInfo->freeBytes != (cacheSize - cacheInfoList[i].debugBytes))
                || (cacheInfo->addrMode != addrMode)
                || (cacheInfo->modLevel != getShcModlevelForJCL(J2SE_VERSION(vm)) || (cacheInfo->lastDetach <= 0))) {
                continue;
            }

            /* for persistent cache, os_shmid and os_semid should always be -1 */
            if ((J9PORT_SHR_CACHE_TYPE_PERSISTENT == cacheInfo->cacheType)
                && (((IDATA)cacheInfo->os_shmid != -1) || ((IDATA)cacheInfo->os_semid != -1))) {
                continue;
            }
#if !defined(WIN32)
            if ((J9PORT_SHR_CACHE_TYPE_NONPERSISTENT == cacheInfo->cacheType)
                && (((IDATA)cacheInfo->os_shmid <= 0) || ((IDATA)cacheInfo->os_semid <= 0))) {
                continue;
            }
#else
            if ((J9PORT_SHR_CACHE_TYPE_NONPERSISTENT == cacheInfo->cacheType)
...
IDATA
iterateSharedCache(J9JavaVM* vm, char* ctrlDirName, UDATA groupPerm, BOOLEAN useCommandLineValues,
    IDATA (*callback)(J9JavaVM* vm, J9SharedCacheInfo* cacheInfo, void* userData))
{
    IDATA rc = PASS;
...
destroySharedCache(J9JavaVM* vm, BOOLEAN useCommandLineValues)
{
    ...
    for (i = 0; i < (NUM_CACHE + NUM_SNAPSHOT); i++) {
        ...
        IDATA expectedRc
            = (J9PORT_SHR_CACHE_TYPE_SNAPSHOT == cacheType) ? J9SH_DESTROYED_ALL_SNAPSHOT : J9SH_DESTROYED_ALL_CACHE;
        rc = j9shr_destroySharedCache(
            vm, cacheInfoList[i].cacheDir, cacheInfoList[i].name, (U_32)cacheType, useCommandLineValues);
        if (expectedRc != rc) {
            j9tty_printf(
                PORTLIB, "j9shr_destroySharedCache failed to destroy all caches of type: %d, rc: %d\n", cacheType, rc);
            rc = FAIL;
            return rc;
        }
    }
...
IDATA
runTestCycle(J9JavaVM* vm, J9SharedClassPreinitConfig* piconfig, J9SharedClassConfig* sharedClassConfig,
    BOOLEAN createCache, char* cacheDir, BOOLEAN useCommandLineValues)
{
    IDATA rc = PASS;
    IDATA i;
    I_32 oldCacheCount;
    ...
    if (createCache == TRUE) {
        if ((rc != PASS) || ((cacheCount - oldCacheCount) != (NUM_CACHE + NUM_SNAPSHOT))) {
            j9tty_printf(PORTLIB,
                "testSharedCacheAPI: failed to iterate newly created shared caches\texpected: %d\tfound: %d\n",
        ...
        for (i = 0; i < (NUM_CACHE + NUM_SNAPSHOT); i++) {
            if (cacheInfoList[i].found != TRUE) {
                j9tty_printf(PORTLIB,
                    "testSharedCacheAPI: failed to iterate newly created shared caches; could not find cache with name "
                    "%s (index: %d)\n",
    ...
    if (createCache == FALSE) {
        if (cacheCount != oldCacheCount) {
            j9tty_printf(PORTLIB,
                "testSharedCacheAPI: no new cache should exist. oldCacheCount = %d\tnewCacheCount = %d \n",
                oldCacheCount, cacheCount);
            rc = FAIL;
            goto exit;
        }
...
testSharedCacheAPI(J9JavaVM* vm)
{
    ...
    sharedClassConfig
        = (J9SharedClassConfig*)j9mem_allocate_memory(sizeof(J9SharedClassConfig), J9MEM_CATEGORY_CLASSES);
