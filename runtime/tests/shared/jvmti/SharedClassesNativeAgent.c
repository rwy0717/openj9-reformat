...
static void JNICALL retransformClassFileLoadHook(jvmtiEnv* jvmti_env, JNIEnv* jni_env, jclass class_being_redefined,
...
void JNICALL Agent_OnUnload(JavaVM* vm) { free(testcaseArguments); }

/* Grab or build the PORTLIB if it hasnt been done already */
static void ensurePortLibrarySetup(JavaVM* vm)
{
...
JNIEXPORT jboolean JNICALL Java_design1474_tests_TestRetransformation_retransform(JNIEnv* jni_env, jobject clazz)
{
...
JNIEXPORT jboolean JNICALL Java_design1474_tests_TestChangingBootclasspath_addToBootstrapClassLoaderSearch(
    JNIEnv* env, jobject jobj, jstring jAdditionalElement)
{
    jvmtiError err;
    char* errorString;
    ...
    if (globalJvmtiEnv == NULL) {
        j9tty_printf(
            PORTLIB, "Agent: ERROR: Please make sure you include the shared library as an agent with -agentlib\n");
        return JNI_FALSE;
    }
...
JNIEXPORT jboolean JNICALL Java_design1474_tests_TestChangingSystemclasspath_addToSystemClassLoaderSearch(
    JNIEnv* env, jobject jobj, jstring jAdditionalElement)
{
    jvmtiError err;
    char* errorString;
    ...
    additionalElement = (const char*)(*env)->GetStringUTFChars(env, jAdditionalElement, NULL);
    j9tty_printf(
        PORTLIB, "Agent: INFO: Attempting to add this entry '%s' to the system classpath\n", additionalElement);
