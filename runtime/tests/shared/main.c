...
static IDATA setupArguments(struct j9cmdlineOptions* startupOptions, JavaVMInitArgs* vm_args, void** vmOptionsTable,
    BOOLEAN useXshareclasses, BOOLEAN enablebci);
...
IDATA
createJavaVM(
    struct j9cmdlineOptions* startupOptions, J9JavaVM** vm_, BOOLEAN useXshareclasses, BOOLEAN enablebci, JNIEnv** env)
{
    ...
    BOOLEAN useXshareclasses, BOOLEAN enablebci)
{
    ...
    if (TRUE == useXshareclasses) {
        if (FALSE == enablebci) {
            if (vmOptionsTableAddOption(
                    vmOptionsTable, "-Xshareclasses:nonpersistent,name=testSCTransactions,reset", NULL)
                != J9CMDLINE_OK) {
                goto cleanup;
            }
        } else {
            if (vmOptionsTableAddOption(
                    vmOptionsTable, "-Xshareclasses:nonpersistent,name=testSCTransactions,enablebci,reset", NULL)
                != J9CMDLINE_OK) {
                goto cleanup;
            }
        }
        /* The test needs a debug area size of 2MB (using -Xscdmx2m). The default cache size is 300MB on 64-bit
         * platforms on Java 9 and up, which might not be allowed by the OS due to the shmmax setting. The cache size
         * will be adjusted to shmmax and debug area size will also be adjusted proportionally, resulting in the debug
         * area size to be much smaller than 2MB. Set cache size to 16MB for this test so that it won't be unstable due
         * to the shmmax setting on the test machines.
         */
