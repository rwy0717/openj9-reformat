...
UDATA InstallService(void)
{
    ...
    hService = CreateService(hSCM, install, srvdesc, SERVICE_ALL_ACCESS, SERVICE_WIN32_OWN_PROCESS,
        SERVICE_DEMAND_START, SERVICE_ERROR_IGNORE, exepath, NULL, NULL, NULL, NULL, NULL);
...
{
    WriteLog("Usage:\n");
    WriteLog("       servicetest -install serviceName -exepath <path to servicetest.exe> -srvdesc \"service "
             "description\"\n");
...
void ErrorHandler(LPTSTR errorFunc)
{
    ...
    FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM, NULL, lastErrorCode,
        MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), (LPTSTR)&errorMsg, 0, NULL);
