...
int abort_main(int argc, char* argv[])
{
...
static IDATA createDefaultThread(omrthread_t* t, int (*entrypoint)(void*), void* entryarg)
{
...
static void printStatus(const char* testName, int pass)
{
...
static int testWaiting(void)
{
    ...
    printStatus("testWaiting", testdata.rc == J9THREAD_INTERRUPTED_MONITOR_ENTER);
    return 0;
}
...
static int testBlocked(void)
{
    ...
    printStatus("testBlocked", testdata.rc == J9THREAD_INTERRUPTED_MONITOR_ENTER);
    return 0;
}
...
static int blockedMain(void* arg)
{
    ...
    testdata->rc = omrthread_monitor_enter_abortable_using_threadId(testdata->blockedSync, omrthread_self());
    if (0 == testdata->rc) {
        omrthread_monitor_exit(testdata->blockedSync);
    } else {
        J9AbstractThread* self = (J9AbstractThread*)omrthread_self();
...
static int testNotified(void)
{
    ...
    printStatus("testNotified", testdata.rc == J9THREAD_INTERRUPTED_MONITOR_ENTER);
    return 0;
}
...
static int testRecursive(void)
{
    ...
    printStatus("testRecursive", testdata.rc == J9THREAD_INTERRUPTED_MONITOR_ENTER);
    return 0;
}
