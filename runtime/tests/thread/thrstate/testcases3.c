...
static void explainThreadIsNotBlocked(JNIEnv* env)
{
    PORT_ACCESS_FROM_ENV(env);

    j9tty_printf(PORTLIB, "Expected failure in getVMThreadStatus:\n");
    j9tty_printf(PORTLIB,
        "The thread is not blocked if it owns the blocking monitor,\nor if the blocking monitor is not owned.\n");
}
...
test_vBfOC_nBMVC(JNIEnv* env, BOOLEAN ignoreExpectedFailures)
{
    ...
    j9tty_printf(PORTLIB, "Expected failure in getVMThreadStatus:\n");
    j9tty_printf(PORTLIB,
        "It reports the thread blocked on the object monitor, which it already owns, instead of the raw monitor.\n");
    if (ignoreExpectedFailures) {
        rc = IGNORE_FAILURES(rc, FAILED_OLDSTATE);
    }
    return rc;
...
test_vBfOC_nBMNC(JNIEnv* env, BOOLEAN ignoreExpectedFailures)
{
    ...
    setVMThreadState(TESTDATA(selfVmthread), TESTDATA(selfOsthread), J9_PUBLIC_FLAGS_THREAD_BLOCKED,
        TESTDATA(blockingObject), getFlatLock(TESTDATA(selfVmthread), 1));
    setJ9ThreadState(TESTDATA(selfOsthread), J9THREAD_FLAG_STARTED | J9THREAD_FLAG_BLOCKED,
        TESTDATA(rawMonitor)->monitor, TESTDATA(unattachedOsthread), 1);

    rc |= checkObjAnswers(env, TESTDATA(selfVmthread), J9VMTHREAD_STATE_RUNNING, NULL, NULL, 0);
    rc |= checkRawAnswers(
        env, TESTDATA(selfVmthread), J9VMTHREAD_STATE_BLOCKED, NULL, TESTDATA(rawMonitor)->monitor, NULL, 1);
    rc |= checkOldAnswers(
        env, TESTDATA(selfVmthread), J9VMTHREAD_STATE_BLOCKED, TESTDATA(rawMonitor)->monitor, NULL, 1);
...
test_vBfOC_nWMNC(JNIEnv* env, BOOLEAN ignoreExpectedFailures)
{
    ...
    rc |= checkObjAnswers(env, TESTDATA(selfVmthread), J9VMTHREAD_STATE_RUNNING, NULL, NULL, 0);
    rc |= checkRawAnswers(
        env, TESTDATA(selfVmthread), J9VMTHREAD_STATE_WAITING, NULL, TESTDATA(rawMonitor)->monitor, NULL, 1);
    rc |= checkOldAnswers(env, TESTDATA(selfVmthread), J9VMTHREAD_STATE_RUNNING, NULL, NULL, 0);
...
test_vBfVC_nStMOC(JNIEnv* env, BOOLEAN ignoreExpectedFailures)
{
    ...
    setJ9ThreadState(
        TESTDATA(selfOsthread), J9THREAD_FLAG_STARTED, TESTDATA(objMonitor)->monitor, TESTDATA(selfOsthread), 1);
...
test_vBfVC_nStMVC(JNIEnv* env, BOOLEAN ignoreExpectedFailures)
{
    ...
    setVMThreadState(TESTDATA(selfVmthread), TESTDATA(selfOsthread), J9_PUBLIC_FLAGS_THREAD_BLOCKED,
        TESTDATA(blockingObject), getFlatLock(TESTDATA(otherVmthread), 0));
    setJ9ThreadState(
        TESTDATA(selfOsthread), J9THREAD_FLAG_STARTED, TESTDATA(objMonitor)->monitor, TESTDATA(otherOsthread), 1);
