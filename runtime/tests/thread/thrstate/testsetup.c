...
static IDATA initOSThread(JNIEnv* env, omrthread_t* osthread, volatile IDATA* doneflag)
{
    ...
    if (0
        == vm->internalVMFunctions->createThreadWithCategory(
               osthread, 0, J9THREAD_PRIORITY_MIN, 0, threadmain, (void*)doneflag, J9THREAD_CATEGORY_SYSTEM_THREAD)) {
...
void setJ9ThreadState(omrthread_t osthread, UDATA flags, omrthread_monitor_t blocker, omrthread_t owner, UDATA count)
{
    J9AbstractThread* thr = (J9AbstractThread*)osthread;
...
void setJ9MonitorState(omrthread_monitor_t monitor, omrthread_t owner, UDATA count)
{
...
UDATA
checkObjAnswers(JNIEnv* env, J9VMThread* vmthread, UDATA exp_vmstate, j9object_t exp_lockObj, J9VMThread* exp_owner,
    UDATA exp_count)
{
    PORT_ACCESS_FROM_ENV(env);
    UDATA rc = 0;
    ...
    if (vmstate != exp_vmstate)
        rc = FAILED_OBJSTATE;
    if (lockObj != exp_lockObj)
        rc = FAILED_OBJSTATE;
    if (owner != exp_owner)
        rc = FAILED_OBJSTATE;
    if (count != exp_count)
...
UDATA
checkRawAnswers(JNIEnv* env, J9VMThread* vmthread, UDATA exp_vmstate, j9object_t exp_lockObj,
    omrthread_monitor_t exp_rawLock, J9VMThread* exp_owner, UDATA exp_count)
{
    ...
    if (rc) {
        ...
        j9tty_printf(PORTLIB, "\texpect: vmstate %d lockObj %p rawLock %p lockOwner %p count %d\n", exp_vmstate,
            exp_lockObj, exp_rawLock, exp_owner, exp_count);
    }
    return rc;
}

UDATA
checkOldAnswers(JNIEnv* env, J9VMThread* vmthread, UDATA exp_vmstate, omrthread_monitor_t exp_rawLock,
...
static UDATA bufferTestDataDesc(JNIEnv* env, char* buf, UDATA buflen)
{
    ...
    rc += j9str_printf(PORTLIB, buf, buflen,
        ...
        TESTDATA(selfVmthread), TESTDATA(selfOsthread), TESTDATA(otherVmthread), TESTDATA(otherOsthread),
        TESTDATA(vmthread3), TESTDATA(osthread3), TESTDATA(unattachedOsthread), TESTDATA(blockingObject),
        TESTDATA(objMonitor), TESTDATA(objMonitor)->monitor, TESTDATA(rawMonitor), TESTDATA(rawMonitor)->monitor,
        TESTDATA(objectNoMonitor), MON_COUNT);
    return rc;
}
...
void cleanupTestData(JNIEnv* env)
{
    PORT_ACCESS_FROM_ENV(env);
