...
static J9ROMClass* createFakeROMClass(
    U_8* buffer, UDATA bufferSize, const char* className, testFieldDef* regularFields);
static IDATA checkForFieldOverlaps(J9JavaVM* vm, J9ROMClass* romClass, J9Class* superClass, UDATA* badOffset);
static IDATA checkForPresenceOfField(J9JavaVM* vm, J9ROMClass* romClass, J9Class* superClass, U_8* fieldNameData,
    UDATA fieldNameLength, UDATA fieldSize, UDATA expectedIndex);
static IDATA testAddHiddenInstanceFields(
    J9PortLibrary* portLib, const char* testName, testFieldDef* regularFields, testFieldDef* hiddenFields);
...
#if VMTEST_DEBUG
        printf("Got field %.*s with index %d and size %d.\n", J9UTF8_LENGTH(resultFieldName),
            J9UTF8_DATA(resultFieldName), walkResult->index, resultFieldSize);
#endif

        if (expectedIndex == walkResult->index) {
            if (J9UTF8_DATA_EQUALS(
                    J9UTF8_DATA(resultFieldName), J9UTF8_LENGTH(resultFieldName), fieldNameData, fieldNameLength)) {
    ...
    J9PortLibrary* portLib, const char* testName, testFieldDef* regularFields, testFieldDef* hiddenFields)
{
    ...
    walkResult
        = fieldOffsetsStartDo(&javaVM, fakeROMClass, NULL, &walkState, J9VM_FIELD_OFFSET_WALK_CALCULATE_INSTANCE_SIZE);
    if (NULL == walkResult) {
    ...
    for (fieldIndex = 0; NULL != hiddenFields[fieldIndex].fieldName; fieldIndex++) {
        ...
        if (0
            != addHiddenInstanceField(&javaVM, className, hiddenFields[fieldIndex].fieldName,
                   hiddenFields[fieldIndex].fieldSignature, offsetReturnPtr)) {
    ...
    if (hiddenFieldCount > 0) {
        if (javaVM.hiddenInstanceFields->offsetReturnPtr != &fieldOffset) {
            outputErrorMessage(TEST_ERROR_ARGS,
                "field->offsetReturnPtr (%p) != &fieldOffset (%p) after addHiddenInstanceField()!\n",
                javaVM.hiddenInstanceFields->offsetReturnPtr != &fieldOffset);
            goto _exit_test;
        }

        walkResult = fieldOffsetsStartDo(
            &javaVM, fakeROMClass, NULL, &walkState, J9VM_FIELD_OFFSET_WALK_CALCULATE_INSTANCE_SIZE);
        if (NULL == walkResult) {
            outputErrorMessage(TEST_ERROR_ARGS, "fieldOffsetsStartDo() returned NULL!\n");
            goto _exit_test;
        }
        ...
        if (javaVM.hiddenInstanceFields->fieldOffset != fieldOffset) {
            outputErrorMessage(TEST_ERROR_ARGS,
                "field->fieldOffset (%d) != fieldOffset (%d) after fieldOffsetsStartDo()!\n",
    ...
    for (fieldIndex = 0; NULL != regularFields[fieldIndex].fieldName; fieldIndex++) {
        ...
        if (0
            != checkForPresenceOfField(
                   &javaVM, fakeROMClass, NULL, (U_8*)fieldName, strlen(fieldName), fieldSize, fieldIndex + 1)) {
    ...
    for (fieldIndex = 0; NULL != hiddenFields[fieldIndex].fieldName; fieldIndex++) {
        ...
        if (0
            != checkForPresenceOfField(
                   &javaVM, fakeROMClass, NULL, (U_8*)fieldName, strlen(fieldName), fieldSize, hiddenIndex)) {
...
static IDATA testAddHiddenInstanceFields1(J9PortLibrary* portLib)
{
    ...
    testFieldDef regularFields[] = { { "a", "I" }, { "b", "J" }, { "c", "I" }, { NULL, NULL } };
    testFieldDef hiddenFields[] = { { "x", "J" }, { "y", "I" }, { NULL, NULL } };
...
static IDATA testAddHiddenInstanceFields2(J9PortLibrary* portLib)
{
    ...
    testFieldDef regularFields[] = { { "a", "I" }, { "b", "[J" }, { NULL, NULL } };
    testFieldDef hiddenFields[] = { { "x", "I" }, { "y", "I" }, { NULL, NULL } };
...
static IDATA testAddHiddenInstanceFields3(J9PortLibrary* portLib)
{
    ...
    testFieldDef regularFields[] = { { NULL, NULL } };
    testFieldDef hiddenFields[] = { { "x", "I" }, { NULL, NULL } };
...
static IDATA testAddHiddenInstanceFields4(J9PortLibrary* portLib)
{
    ...
    testFieldDef regularFields[] = { { NULL, NULL } };
    testFieldDef hiddenFields[] = { { "x", "J" }, { NULL, NULL } };
...
static IDATA testAddHiddenInstanceFields5(J9PortLibrary* portLib)
{
    ...
    testFieldDef regularFields[] = { { "x", "I" }, { NULL, NULL } };
    testFieldDef hiddenFields[] = { { NULL, NULL } };
...
static IDATA testAddHiddenInstanceFields6(J9PortLibrary* portLib)
{
    ...
    testFieldDef regularFields[] = { { "x", "J" }, { NULL, NULL } };
    testFieldDef hiddenFields[] = { { NULL, NULL } };
