...
static void allocateMemoryForAndCopyInto(struct J9PortLibrary* portLibrary, char** dest, const char* source)
{
    ...
    *dest = j9mem_allocate_memory(strLenPlusTerminator, OMRMEM_CATEGORY_VM);
    if (*dest == NULL) {
        return;
    } else {
    ...
    const char* format, ...)
{
    ...
    portErrorBuf = j9mem_allocate_memory(sizePortErrorBuf, OMRMEM_CATEGORY_VM);
    ...
    buf = j9mem_allocate_memory(sizeBuf, OMRMEM_CATEGORY_VM);
    if (NULL != buf) {
        va_start(args, format);
    ...
    const char* fileName)
{
    ...
    if (0 == fileStatRC) {
        if (fileStat.isFile) {
            ...
            outputErrorMessage(
                portLibrary, pltestFileName, lineNumber, testName, "\tfile: %s does not exist!\n", -1, fileName);
            return -1;
        }
    } else {
        /* error in file_stat */
