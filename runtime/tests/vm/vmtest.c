...
static BOOLEAN startsWith(const char* s, const char* prefix)
{
...
static BOOLEAN consumeOption(const char** s, const char* prefix)
{
    size_t sLen = strlen(*s);
    size_t prefixLen = strlen(prefix);
    size_t i;
...
static UDATA parseForAreasToTests(struct J9PortLibrary* portLibrary, const char* options)
{
