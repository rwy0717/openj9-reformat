...
I_32 testMultipleProcessEachWithAJVM(struct J9PortLibrary* portLibrary, struct j9cmdlineOptions* args)
{
    ...
    for (i = 0; i < MAX_CHILD_PROCESS; i++) {
        ...
        default:
            j9tty_printf(PORTLIB,
                "testMultipleCreate: Unknown return value from WaitForProcess(): failed (procrc = %p)\n", procrc);
            rc = FAIL;
    ...
    J9PortLibrary* portLibrary, const char* testname, char* newargv[SHRTEST_MAX_CMD_OPTS], UDATA newargc)
{
    ...
    retVal = j9process_create((const char**)newargv, newargc, env, envSize, dir, exeoptions, J9PORT_INVALID_FD,
