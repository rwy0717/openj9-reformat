...
#define DUMP_UTF(val)                                                                      \
    outputFunction(userData, ((J9UTF8_LENGTH(val) > UTF_MAX_LENGTH) ? "%.*s..." : "%.*s"), \
        ((J9UTF8_LENGTH(val) > UTF_MAX_LENGTH) ? UTF_MAX_LENGTH : J9UTF8_LENGTH(val)), (((char*)(val)) + 2))
    ...
    while ((U_32)pc <= walkEndPC) {
        if (flags & BCT_DumpMaps) {
            for (j = LOCAL_MAP; j < MAP_COUNT; j++) {
                ...
                switch (j) {
                case LOCAL_MAP:
                    result = j9localmap_LocalBitsForPC(portLib, romClass, romMethod, pc, resultArray, NULL, NULL, NULL);
                    mapChar = 'l';
                    outputCount = localsCount;
                    break;
                case DEBUG_MAP:
                    result = j9localmap_DebugLocalBitsForPC(
                        portLib, romClass, romMethod, pc, resultArray, NULL, NULL, NULL);
                    mapChar = 'd';
                    outputCount = localsCount;
                    break;
                case STACK_MAP:
                    /* First call is to get the stack depth */
                    result = j9stackmap_StackBitsForPC(portLib, pc, romClass, romMethod, NULL, 0, NULL, NULL, NULL);
                    result = j9stackmap_StackBitsForPC(
                        portLib, pc, romClass, romMethod, resultArray, result, NULL, NULL, NULL);
                    mapChar = 's';
                    outputCount = result;
                    break;
                }
        ...
        case JBldcw:
            ...
            if (((J9ROMSingleSlotConstantRef*)info)->cpType != BCT_J9DescriptionCpTypeScalar) {
                ...
                case BCT_J9DescriptionCpTypeMethodHandle:
                    /* MethodHandleRef */
                    outputFunction(userData, "(java.dyn.MethodHandle) {0x%X, 0x%X}\"",
                        ((J9ROMMethodHandleRef*)info)->methodOrFieldRefIndex,
                        ((J9ROMMethodHandleRef*)info)->handleTypeAndCpType);
                    break;
                }
                outputFunction(userData, "\n");
            } else {
        ...
        case JBwithfield:
            ...
            info = &constantPool[index];
            outputFunction(userData, "%i ", index);
            DUMP_UTF(J9ROMCLASSREF_NAME(
                (J9ROMClassRef*)&constantPool[((J9ROMFieldRef*)info)->classRefCPIndex])); /* dump declaringClassName */
        ...
        case JBinvokedynamic:
            ...
            DUMP_UTF(J9ROMNAMEANDSIGNATURE_SIGNATURE(nameAndSig)); /* dump signature */
            {
                ...
                /* info will be either a field or a method ref - they both have the same shape so we can pretend it is
                 * always a methodref */
                DUMP_UTF(J9ROMCLASSREF_NAME(
                    (J9ROMClassRef*)&constantPool[((J9ROMMethodRef*)info)->classRefCPIndex])); /* dump
                                                                                                  declaringClassName */
        ...
        case JBinvokespecialsplit:
            ...
            outputFunction(userData, "%i ", index);
            DUMP_UTF(J9ROMCLASSREF_NAME(
                (J9ROMClassRef*)&constantPool[((J9ROMMethodRef*)info)->classRefCPIndex])); /* dump declaringClassName */
            nameAndSig = J9ROMMETHODREF_NAMEANDSIGNATURE((J9ROMMethodRef*)info);
            outputFunction(userData, ".");
            DUMP_UTF(J9ROMNAMEANDSIGNATURE_NAME(nameAndSig)); /* dump name */
            DUMP_UTF(J9ROMNAMEANDSIGNATURE_SIGNATURE(nameAndSig)); /* dump signature */
...
IDATA dumpBytecodes(J9PortLibrary* portLib, J9ROMClass* romClass, J9ROMMethod* romMethod, U_32 flags)
{
    ...
    if (length == 0)
        return BCT_ERR_NO_ERROR; /* catch abstract methods */

    return j9bcutil_dumpBytecodes(portLib, romClass, J9_BYTECODE_START_FROM_ROM_METHOD(romMethod), 0, length - 1, flags,
