    ...
    if (NULL != classLoader->classLocationHashTable) {
        ...
        if (NULL != classLocation) {
            ...
            case LOAD_LOCATION_PATCH_PATH: {
                J9ModuleExtraInfo* moduleInfo
                    = vmFuncs->findModuleInfoForModule(currentThread, classLoader, clazz->module);

                /* If the class is loaded from patch path, then an entry should be present in classloader's
                 * moduleExtraInfoHashTable */
    ...
    if (NULL == jrtURL) {
        if (J9_ARE_ALL_BITS_SET(javaVM->runtimeFlags, J9_RUNTIME_JAVA_BASE_MODULE_CREATED)) {
            ...
            jrtURL = vmFuncs->copyStringToJ9UTF8WithMemAlloc(
                currentThread, module->moduleName, J9_STR_NONE, "jrt:/", 5, NULL, 0);
...
addJarToSystemClassLoaderClassPathEntries(J9JavaVM* vm, const char* filename)
{
    ...
    newEntries = (J9ClassPathEntry*)j9mem_allocate_memory(
        sizeof(J9ClassPathEntry) * (entryCount + 1) + classPathLength, J9MEM_CATEGORY_CLASSES);
    if (NULL != newEntries) {
