...
#define GET_SUPERCLASS(clazz) ((J9CLASS_DEPTH(clazz) == 0) ? NULL : (clazz)->superclasses[J9CLASS_DEPTH(clazz) - 1])
...
#define J9ROMMETHOD_NAME_AND_SIG_IDENTICAL(romClass1, romClass2, o1, o2)                                \
    areUTFPairsIdentical(J9ROMMETHOD_GET_NAME(romClass1, o1), J9ROMMETHOD_GET_SIGNATURE(romClass1, o1), \
        J9ROMMETHOD_GET_NAME(romClass2, o2), J9ROMMETHOD_GET_SIGNATURE(romClass2, o2))
...
static jvmtiError addMethodEquivalence(
    J9VMThread* currentThread, J9Method* oldMethod, J9Method* newMethod, J9HashTable** methodEquivalences, U_32 size);
...
static UDATA areMethodRefsIdentical(
    J9ROMConstantPoolItem* romCP1, U_32 index1, J9ROMConstantPoolItem* romCP2, U_32 index2);
static UDATA areClassRefsIdentical(
    J9ROMConstantPoolItem* romCP1, U_32 index1, J9ROMConstantPoolItem* romCP2, U_32 index2);
static UDATA areFieldRefsIdentical(
    J9ROMConstantPoolItem* romCP1, U_32 index1, J9ROMConstantPoolItem* romCP2, U_32 index2);
static UDATA areNameAndSigsIdentical(J9ROMNameAndSignature* nas1, J9ROMNameAndSignature* nas2);
static UDATA areSingleSlotConstantRefsIdentical(
    J9ROMConstantPoolItem* romCP1, U_32 index1, J9ROMConstantPoolItem* romCP2, U_32 index2);
static UDATA areDoubleSlotConstantRefsIdentical(
    J9ROMConstantPoolItem* romCP1, U_32 index1, J9ROMConstantPoolItem* romCP2, U_32 index2);
...
static jvmtiError addClassesRequiringNewITables(
    J9JavaVM* vm, J9HashTable* classHashTable, UDATA* addedMethodCountPtr, UDATA* addedClassCountPtr);
static jvmtiError verifyFieldsAreSame(J9VMThread* currentThread, UDATA fieldType, J9ROMClass* originalROMClass,
    J9ROMClass* replacementROMClass, UDATA extensionsEnabled, UDATA* extensionsUsed);
static jvmtiError verifyMethodsAreSame(
    J9VMThread* currentThread, J9JVMTIClassPair* classPair, UDATA extensionsEnabled, UDATA* extensionsUsed);
...
static jvmtiError fixJNIMethodID(
    J9VMThread* currentThread, J9Method* oldMethod, J9Method* newMethod, BOOLEAN equivalent, UDATA extensionsUsed);

static jvmtiIterationControl fixHeapRefsHeapIteratorCallback(
    J9JavaVM* vm, J9MM_IterateHeapDescriptor* heapDesc, void* userData);
static jvmtiIterationControl fixHeapRefsSpaceIteratorCallback(
    J9JavaVM* vm, J9MM_IterateSpaceDescriptor* spaceDesc, void* userData);
static jvmtiIterationControl fixHeapRefsRegionIteratorCallback(
    J9JavaVM* vm, J9MM_IterateRegionDescriptor* regionDesc, void* userData);
static jvmtiIterationControl fixHeapRefsObjectIteratorCallback(
    J9JavaVM* vm, J9MM_IterateObjectDescriptor* objectDesc, void* userData);
static void replaceInAllClassLoaders(
    J9VMThread* currentThread, J9Class* originalRAMClass, J9Class* replacementRAMClass);
#ifdef J9VM_INTERP_NATIVE_SUPPORT
static jvmtiError jitEventInitialize(
    J9VMThread* currentThread, jint redefinedClassCount, UDATA redefinedMethodCount, J9JVMTIHCRJitEventData* eventData);
static void jitEventAddMethod(J9VMThread* currentThread, J9JVMTIHCRJitEventData* eventData, J9Method* oldMethod,
    J9Method* newMethod, UDATA equivalent);
static void jitEventAddClass(J9VMThread* currentThread, J9JVMTIHCRJitEventData* eventData, J9Class* originalRAMClass,
...
static char* getClassName(J9Class* c)
{
    static char buf[512];
...
#ifdef J9HSHELP_DEBUG
static char* getMethodName(J9Method* m)
{
    static char buf[512];
...
void hcrSanityCheck(J9JavaVM* vm)
{
...
static UDATA areUTFPairsIdentical(J9UTF8* leftUtf1, J9UTF8* leftUtf2, J9UTF8* rightUtf1, J9UTF8* rightUtf2)
{
    if ((J9UTF8_LENGTH(leftUtf1) != J9UTF8_LENGTH(rightUtf1))
        || (J9UTF8_LENGTH(leftUtf2) != J9UTF8_LENGTH(rightUtf2))) {
        return FALSE;
    }
...
static UDATA fixJNIFieldID(J9VMThread* currentThread, J9JNIFieldID* fieldID, J9Class* replacementRAMClass)
{
    ...
    if (field->modifiers & J9AccStatic) {
        ...
        newFieldAddress = vmFuncs->staticFieldAddress(currentThread, replacementRAMClass, J9UTF8_DATA(fieldName),
            J9UTF8_LENGTH(fieldName), J9UTF8_DATA(fieldSignature), J9UTF8_LENGTH(fieldSignature), &declaringClass,
            (UDATA*)&resolvedField, J9_LOOK_NO_JAVA, NULL);
        ...
        newFieldOffset = vmFuncs->instanceFieldOffset(currentThread, replacementRAMClass, J9UTF8_DATA(fieldName),
            J9UTF8_LENGTH(fieldName), J9UTF8_DATA(fieldSignature), J9UTF8_LENGTH(fieldSignature), &declaringClass,
            (UDATA*)&resolvedField, J9_LOOK_NO_JAVA);
    ...
    if (oldJNIIDs != NULL) {
        ...
        if (newJNIIDs == NULL) {
            ...
            for (oldFieldIndex = oldMethodCount; oldFieldIndex < oldMethodCount + oldFieldCount; ++oldFieldIndex) {
                ...
                if (fieldID != NULL) {
                    /* Always invalidate the old field ID slot (do not free IDs for deleted fields, since a reflect
                     * field might be holding onto it) */
    ...
    J9ROMConstantPoolItem* romCP1, U_32 index1, J9ROMConstantPoolItem* romCP2, U_32 index2)
{
    ...
    return areClassRefsIdentical(romCP1, ref1->classRefCPIndex, romCP2, ref2->classRefCPIndex)
    ...
    J9ROMConstantPoolItem* romCP1, U_32 index1, J9ROMConstantPoolItem* romCP2, U_32 index2)
{
    ...
    return utfsAreIdentical(
        J9ROMSTRINGREF_UTF8DATA((J9ROMStringRef*)ref1), J9ROMSTRINGREF_UTF8DATA((J9ROMStringRef*)ref2));
}
...
#define NEXT_U16(bytecodes) \
    (((U_16)((bytecodes)[index + alreadyCompared + 0])) + (((U_16)((bytecodes)[index + alreadyCompared + 1])) << 8))
#else
#define NEXT_U16(bytecodes) \
    (((U_16)((bytecodes)[index + alreadyCompared + 1])) + (((U_16)((bytecodes)[index + alreadyCompared + 0])) << 8))
#endif
        ...
        while (index < size) {
            ...
            case JBldc:
                if (!areSingleSlotConstantRefsIdentical(
                        romCP1, bytecodes1[index + alreadyCompared], romCP2, bytecodes2[index + alreadyCompared])) {
                    return FALSE;
                }
                alreadyCompared += 1;
                break;
            ...
            if (memcmp(bytecodes1 + index + alreadyCompared, bytecodes2 + index + alreadyCompared,
                    bytecodeSize - alreadyCompared)
                != 0) {
                return FALSE;
            }
...
void fixSubclassHierarchy(J9VMThread* currentThread, J9HashTable* classPairs)
{
    J9JavaVM* vm = currentThread->javaVM;
 ...
 */
void fixITables(J9VMThread* currentThread, J9HashTable* classPairs)
{
    J9HashTableState hashTableState;
    J9JVMTIClassPair* classPair;
...
static UDATA findMethodInVTable(J9Method* method, UDATA* vTable)
{
    ...
    for (searchIndex = 0; searchIndex < vTableSize; searchIndex++) {
        ...
        if (vTableRomMethod->modifiers & J9_JAVA_PUBLIC) {
            if (J9ROMMETHOD_NAME_AND_SIG_IDENTICAL(J9_CLASS_FROM_METHOD(method)->romClass,
                    J9_CLASS_FROM_METHOD(vTableRomMethod)->romClass, romMethod, vTableRomMethod)) {
                return searchIndex;
            }
        }
    }
...
void fixITablesForFastHCR(J9VMThread* currentThread, J9HashTable* classPairs)
{
    J9JVMTIClassPair* classPair;
    J9HashTableState hashTableState;
                ...
                while (superITable != iTable) {
                    ...
                    do {
                        ...
                        if ((NULL != result) && (NULL != result->methodRemap)) {
                            ...
                            for (methodIndex = 0; methodIndex < methodCount; methodIndex++) {
                                UDATA vTableIndex
                                    = findMethodInVTable(&interfaceClass->ramMethods[methodIndex], vTableHeader);
 ...
 *
 */
static UDATA redefinedClassPairHash(void* entry, void* userData)
{
 ...
 *
 */
static UDATA redefinedClassPairEquals(void* left, void* right, void* userData)
{
 ...
 *
 */
static UDATA methodPairHash(void* entry, void* userData)
{
 ...
 *
 */
static UDATA methodPairEquals(void* left, void* right, void* userData)
{
 ...
 */
static jvmtiError preallocMethodHashTable(J9VMThread* currentThread, UDATA methodCount, J9HashTable** methodHashTable)
{
    ...
    *methodHashTable = NULL;
    ...
    ht = hashTableNew(OMRPORT_FROM_J9PORT(PORTLIB), "JVMTIMethodPairs", (U_32)methodCount, sizeof(J9JVMTIMethodPair),
        sizeof(J9Method*), 0, OMRMEM_CATEGORY_VM, methodPairHash, methodPairEquals, NULL, NULL);
    ...
    while (classPair != NULL) {
        ...
        for (i = 0; i < vTableSize; i++) {
            ...
            Trc_hshelp_fixVTables_Search(currentThread, i,
                classPair->replacementClass.ramClass ? classPair->replacementClass.ramClass
                                                     : classPair->originalRAMClass,
                methodPair.oldMethod, getMethodName(methodPair.oldMethod), vm->jitConfig ? oldJitVTable[0 - i] : 0,
                vm->jitConfig ? newJitVTable[0 - i] : 0);
#endif
 ...
 */
void fixArrayClasses(J9VMThread* currentThread, J9HashTable* classPairs)
{
    J9JavaVM* vm = currentThread->javaVM;
    J9InternalVMFunctions* vmFuncs = vm->internalVMFunctions;
    J9Class* clazz;
 ...
 */
void fixJNIMethodIDs(
    J9VMThread* currentThread, J9Class* originalRAMClass, J9Class* replacementRAMClass, UDATA extensionsUsed)
{
    if (originalRAMClass->romClass == replacementRAMClass->romClass) {
        ...
        for (oldMethodIndex = 0; oldMethodIndex < originalROMClass->romMethodCount; ++oldMethodIndex) {
            ...
            for (newMethodIndex = 0; newMethodIndex < replacementROMClass->romMethodCount; ++newMethodIndex) {
                ...
                if (J9ROMMETHOD_NAME_AND_SIG_IDENTICAL(
                        originalROMClass, replacementROMClass, oldROMMethod, newROMMethod)) {
                    deleted = FALSE;
                    equivalent
                        = areMethodsEquivalent(oldROMMethod, originalROMClass, newROMMethod, replacementROMClass);
                    break;
                }
            }
...
void fixJNIRefs(J9VMThread* currentThread, J9HashTable* classPairs, BOOLEAN fastHCR, UDATA extensionsUsed)
{
    J9HashTableState hashTableState;
    J9JVMTIClassPair* classPair;
 ...
 *  For class constant pools we clear the constant pool and run the same type of code as in
 *<code>internalRunPreInitInstructions</code> For jclConstantPool we must reresolve the items in place since they are
 *assumed to be already resolved.
 */
static void reresolveHotSwappedConstantPool(J9ConstantPool* ramConstantPool, J9VMThread* currentThread,
        ...
        for (i = 0; i < ramConstantPoolCount; ++i) {
            ...
            case J9CPTYPE_INTERFACE_INSTANCE_METHOD:
                ...
                if (ramConstantPool != (J9ConstantPool*)vm->jclConstantPool) {
                    ...
                    ((J9RAMMethodRef*)ramConstantPool)[i].methodIndexAndArgCount
                        = (J9VTABLE_INITIAL_VIRTUAL_OFFSET << 8)
                        + getSendSlotsFromSignature(J9UTF8_DATA(J9ROMNAMEANDSIGNATURE_SIGNATURE(nas)));
                } else {
                    /* Try to resolve as virtual and as special */
                    vmFuncs->resolveVirtualMethodRef(currentThread, ramConstantPool, i,
                        resolveFlagsBase | J9_RESOLVE_FLAG_NO_THROW_ON_FAIL, &resolvedMethod);
                    vmFuncs->resolveSpecialMethodRef(
                        currentThread, ramConstantPool, i, resolveFlagsBase | J9_RESOLVE_FLAG_NO_THROW_ON_FAIL);
                }
            ...
            case J9CPTYPE_INTERFACE_STATIC_METHOD:
                ...
                if (ramConstantPool != (J9ConstantPool*)vm->jclConstantPool) {
                    ...
                    ((J9RAMStaticMethodRef*)ramConstantPool)[i].methodIndexAndArgCount
                        = (J9VTABLE_INITIAL_VIRTUAL_OFFSET << 8)
                        + getSendSlotsFromSignature(J9UTF8_DATA(J9ROMNAMEANDSIGNATURE_SIGNATURE(nas)));
                    ((J9RAMStaticMethodRef*)ramConstantPool)[i].method = vm->initialMethods.initialStaticMethod;
                } else {
                    vmFuncs->resolveStaticMethodRef(
                        currentThread, ramConstantPool, i, resolveFlagsBase | J9_RESOLVE_FLAG_NO_THROW_ON_FAIL);
                }
            ...
            case J9CPTYPE_INTERFACE_METHOD:
                ...
                if (ramConstantPool != (J9ConstantPool*)vm->jclConstantPool) {
                    ...
                    ((J9RAMInterfaceMethodRef*)ramConstantPool)[i].methodIndexAndArgCount = J9_ITABLE_INDEX_UNRESOLVED
                        | getSendSlotsFromSignature(J9UTF8_DATA(J9ROMNAMEANDSIGNATURE_SIGNATURE(nas)));
                } else {
                    vmFuncs->resolveInterfaceMethodRef(
                        currentThread, ramConstantPool, i, resolveFlagsBase | J9_RESOLVE_FLAG_NO_THROW_ON_FAIL);
                }
                ...
                if (((J9RAMClassRef*)ramConstantPool)[i].value != NULL) {
                    ...
                    if (J9CLASS_FLAGS(klass) & J9_JAVA_CLASS_HOT_SWAPPED_OUT) {
                        ...
                        if (NULL != result) {
                            ...
                            ((J9RAMClassRef*)ramConstantPool)[i].modifiers
                                = result->replacementClass.romClass->modifiers;
                        }
                    }
                }
                break;
...
static void fixRAMConstantPoolForFastHCR(
    J9ConstantPool* ramConstantPool, J9HashTable* classHashTable, J9HashTable* methodHashTable)
{
    J9ROMClass* romClass = ramConstantPool->ramClass->romClass;
    U_32* cpShapeDescription = J9ROMCLASS_CPSHAPEDESCRIPTION(romClass);
    J9JVMTIMethodPair methodPair;
                        ...
                        if (NULL != classResult) {
                            ...
                            if (NULL != obsoleteClass) {
                                ...
                                if (NULL != methodResult) {
                                    ...
                                    methodRef->methodIndexAndArgCount = ((newMethodIndex << J9_ITABLE_INDEX_SHIFT)
                                        | argCount | J9_ITABLE_INDEX_METHOD_INDEX);
                                }
                            }
                        }
                    }
                        ...
                        if (methodIndex < methodCount) {
                            ...
                            if (NULL != classResult) {
                                ...
                                if (NULL != obsoleteClass) {
                                    ...
                                    if (NULL != methodResult) {
                                        UDATA argCount = (methodRef->methodIndexAndArgCount & 255);
                                        UDATA newMethodIndex
                                            = getITableIndexForMethod(methodResult->newMethod, resolvedClass);
                                        /* Fix the index in the resolved CP entry, retaining the argCount */
                                        methodRef->methodIndexAndArgCount
                                            = ((newMethodIndex << J9_ITABLE_INDEX_SHIFT) | argCount);
                                    }
                                }
                            }
...
static void fixRAMSplitTablesForFastHCR(J9Class* clazz, J9HashTable* methodHashTable)
{
    U_16 i = 0;
    J9JVMTIMethodPair methodPair;
...
void fixConstantPoolsForFastHCR(J9VMThread* currentThread, J9HashTable* classPairs, J9HashTable* methodPairs)
{
    J9ClassWalkState state;
    J9JavaVM* vm = currentThread->javaVM;
    J9InternalVMFunctions* vmFuncs = vm->internalVMFunctions;
...
static jvmtiIterationControl fixHeapRefsHeapIteratorCallback(
    J9JavaVM* vm, J9MM_IterateHeapDescriptor* heapDesc, void* userData)
{
    vm->memoryManagerFunctions->j9mm_iterate_spaces(
        vm, vm->portLibrary, heapDesc, 0, fixHeapRefsSpaceIteratorCallback, userData);
...
static jvmtiIterationControl fixHeapRefsSpaceIteratorCallback(
    J9JavaVM* vm, J9MM_IterateSpaceDescriptor* spaceDesc, void* userData)
{
    vm->memoryManagerFunctions->j9mm_iterate_regions(
        vm, vm->portLibrary, spaceDesc, 0, fixHeapRefsRegionIteratorCallback, userData);
    return JVMTI_ITERATION_CONTINUE;
}
...
static jvmtiIterationControl fixHeapRefsRegionIteratorCallback(
    J9JavaVM* vm, J9MM_IterateRegionDescriptor* regionDesc, void* userData)
{
    vm->memoryManagerFunctions->j9mm_iterate_region_objects(
        vm, vm->portLibrary, regionDesc, 0, fixHeapRefsObjectIteratorCallback, userData);
    return JVMTI_ITERATION_CONTINUE;
}
...
void fixHeapRefs(J9JavaVM* vm, J9HashTable* classHashTable)
{
    PORT_ACCESS_FROM_JAVAVM(vm);
            ...
            if (NULL != mhCache) {
                ...
                while (NULL != weakRef) {
                    ...
                    if (NULL != mhReferent) {
                        ...
                        exemplar.oldMethod
                            = (J9Method*)(UDATA)J9VMJAVALANGINVOKEPRIMITIVEHANDLE_VMSLOT_VM(vm, mhReferent);
...
void copyPreservedValues(J9VMThread* currentThread, J9HashTable* classPairs, UDATA extensionsUsed)
{
    J9HashTableState hashTableState;
    J9JVMTIClassPair* classPair;
...
static void copyStaticFields(J9VMThread* currentThread, J9Class* originalRAMClass, J9Class* replacementRAMClass)
{
    ...
    while (currentField != NULL) {
        ...
        newFieldAddress = vmFuncs->staticFieldAddress(currentThread, replacementRAMClass, J9UTF8_DATA(fieldName),
            J9UTF8_LENGTH(fieldName), J9UTF8_DATA(fieldSignature), J9UTF8_LENGTH(fieldSignature), NULL, NULL,
            J9_LOOK_NO_JAVA, NULL);

        if (newFieldAddress != NULL) {
            UDATA* oldFieldAddress;

            oldFieldAddress = vmFuncs->staticFieldAddress(currentThread, originalRAMClass, J9UTF8_DATA(fieldName),
                J9UTF8_LENGTH(fieldName), J9UTF8_DATA(fieldSignature), J9UTF8_LENGTH(fieldSignature), NULL, NULL,
                J9_LOOK_NO_JAVA, NULL);

            /* printf("copyStaticFields:  [%p:0x%08x] to [%p:0x%08x]\n", oldFieldAddress, oldFieldAddress[0],
             * newFieldAddress, newFieldAddress[0]); */
            ...
            if (newFieldAddress != oldFieldAddress) {
                ...
                if (currentField->modifiers & J9FieldSizeDouble) {
                    ...
                    vm->memoryManagerFunctions->J9WriteBarrierJ9ClassStore(
                        currentThread, replacementRAMClass, *(j9object_t*)newFieldAddress);
                }
            }
        }
    ...
    while (NULL != classPair) {
        ...
        if ((NULL != originalRAMClass) && (NULL != replacementRAMClass)) {
            ...
            for (i = 0; i < nestMemberCount; i++) {
                ...
                J9Class* nestMember = vmFuncs->hashClassTableAt(
                    classLoader, J9UTF8_DATA(nestMemberName), J9UTF8_LENGTH(nestMemberName));
                if (NULL != nestMember) {
...
static void swapClassesForFastHCR(J9Class* originalClass, J9Class* obsoleteClass)
{
...
jvmtiError recreateRAMClasses(J9VMThread* currentThread, J9HashTable* classHashTable, J9HashTable* methodHashTable,
    UDATA extensionsUsed, BOOLEAN fastHCR)
{
    ...
    for (i = 0; i < classCount; ++i) {
        ...
        /* originalRAMClass != originalRAMClass->hostClass : this is an anon class created using
         * Unsafe.defineAnonymousClass. For anon classes created using Unsafe.defineAnonymousClass, we need to preserve
         * the host class. For all other cases, clazz->hostClass can point to itself (clazz).
         */
        if (originalRAMClass != originalRAMClass->hostClass) {
            hostClass = originalRAMClass->hostClass;
        }

        replacementRAMClass = vmFuncs->internalCreateRAMClassFromROMClass(currentThread, classLoader,
            replacementROMClass, options, NULL, protectionDomain, classPairs[i]->methodRemap, J9_CP_INDEX_NONE,
            LOAD_LOCATION_UNKNOWN, originalRAMClass, hostClass);
        ...
        outOfMemory:
            if (!fastHCR) {
                vmFuncs->hashClassTableAtPut(
                    currentThread, classLoader, J9UTF8_DATA(className), J9UTF8_LENGTH(className), originalRAMClass);
                while (i != 0) {
                    replaceInAllClassLoaders(
                        currentThread, classPairs[i]->replacementClass.ramClass, classPairs[i]->originalRAMClass);
                    --i;
                }
            }
        ...
        if (!extensionsUsed && (classPairs[i]->flags & J9JVMTI_CLASS_PAIR_FLAG_REDEFINED)) {
            ...
            for (m = 0; m < methodCount; m++) {
                ...
                Trc_hshelp_recreateRAMClasses_addMethRemap(currentThread, classPairs[i]->originalRAMClass,
                    methodPair.oldMethod, methodPair.newMethod, getMethodName(methodPair.oldMethod));
            ...
            if (NULL == replacementRAMClass->classLoader->redefinedClasses) {
                ...
                replacementRAMClass->classLoader->redefinedClasses = hashTableNew(OMRPORT_FROM_J9PORT(PORTLIB),
                    "JVMTIRedefinedClassesMap", 1, sizeof(J9JVMTIMethodPair), sizeof(J9Method*), 0, OMRMEM_CATEGORY_VM,
                    redefinedClassPairHash, redefinedClassPairEquals, NULL, NULL);
...
#ifdef WIP_D1452
        // TODO: check whether we really need to prune out redefined classes that are a subclass of another redefined
        // class
        ...
        if (!J9_IS_CLASS_OBSOLETE(clazz)) {
            ...
            while (NULL != iTable) {
                ...
                if (NULL != result) {
                    ...
                    /* If methods were added or removed (extensionsUsed) or re-ordered in the interface, the class
                     * should be redefined. */
                    if (extensionsUsed || (NULL != result->methodRemap)) {
                        ...
                        /* An interface of this class is being updated with extensions - this class will require new
                         * iTables. */
...
jvmtiError determineClassesToRecreate(J9VMThread* currentThread, jint classCount, J9JVMTIClassPair* specifiedClasses,
    J9HashTable** classPairsPtr, J9HashTable** methodPairs, J9JVMTIHCRJitEventData* jitEventData, BOOLEAN fastHCR)
{
    PORT_ACCESS_FROM_VMC(currentThread);
    ...
    J9HashTable* classHashTable = hashTableNew(OMRPORT_FROM_J9PORT(PORTLIB), "JVMTIClassPairs", classCount * 2,
        sizeof(J9JVMTIClassPair), sizeof(J9Class*), 0, OMRMEM_CATEGORY_VM, classPairHash, classPairEquals, NULL, NULL);
    ...
    *classPairsPtr = NULL;
    ...
    if (!fastHCR) {
        ...
        jvmtiResult
            = addClassesRequiringNewITables(currentThread->javaVM, classHashTable, &addedClassCount, &addedMethodCount);
    ...
    if (originalFieldCount == replacementFieldCount) {
        ...
        while (originalField != NULL) {
            if ((originalField->modifiers & J9AccStatic) == fieldType) {
                ...
                if (!NAME_AND_SIG_IDENTICAL(
                        originalField, replacementField, J9ROMFIELDSHAPE_NAME, J9ROMFIELDSHAPE_SIGNATURE)
                    || (originalField->modifiers & CFR_FIELD_ACCESS_MASK)
                        != (replacementField->modifiers & CFR_FIELD_ACCESS_MASK)) {
...
static J9Class* getOldestClassVersion(J9Class* clazz)
{
    ...
    J9VMThread* currentThread, J9JVMTIClassPair* classPair, UDATA extensionsEnabled, UDATA* extensionsUsed)
{
    ...
    if (originalROMClass->romMethodCount == replacementROMClass->romMethodCount) {
        ...
        if (originalROMClass->romMethodCount) {
             ...
             * maps to which slot in the original class. The relationship is not necessarily the same as it depends on
             * the compiler */
            classPair->methodRemap
                = j9mem_allocate_memory(originalROMClass->romMethodCount * sizeof(J9ROMMethod*), OMRMEM_CATEGORY_VM);
            if (NULL == classPair->methodRemap) {
                rc = JVMTI_ERROR_OUT_OF_MEMORY;
                goto done;
            }
            classPair->methodRemapIndices
                = j9mem_allocate_memory(originalROMClass->romMethodCount * sizeof(U_32), OMRMEM_CATEGORY_VM);
        ...
        for (j = 0; j < originalROMClass->romMethodCount; ++j) {
            ...
            for (k = 0; k < replacementROMClass->romMethodCount; ++k) {
                if (J9ROMMETHOD_NAME_AND_SIG_IDENTICAL(
                        originalROMClass, replacementROMClass, originalROMMethod, replacementROMMethod)) {
            ...
            if ((originalROMMethod->modifiers & CFR_METHOD_ACCESS_MASK)
                != (replacementROMMethod->modifiers & CFR_METHOD_ACCESS_MASK)) {
    ...
    /* If extensions are enabled, any error aside of OOM (returned with earlier) is a hint that extensions have been
     * used and should not be treated as an error */
...
jvmtiError verifyClassesAreCompatible(J9VMThread* currentThread, jint class_count, J9JVMTIClassPair* classPairs,
    UDATA extensionsEnabled, UDATA* extensionsUsed)
{
    ...
    for (i = 0; i < class_count; ++i) {
        ...
        if (!utfsAreIdentical(
                J9ROMCLASS_SUPERCLASSNAME(originalROMClass), J9ROMCLASS_SUPERCLASSNAME(replacementROMClass))) {
            return JVMTI_ERROR_UNSUPPORTED_REDEFINITION_HIERARCHY_CHANGED;
        }

        /* Verify that the modifiers match */

        if ((originalROMClass->modifiers & J9AccClassCompatibilityMask)
            != (replacementROMClass->modifiers & J9AccClassCompatibilityMask)) {
        ...
        if ((J9ROMCLASS_FINALIZE_NEEDED(originalROMClass) || J9ROMCLASS_HAS_EMPTY_FINALIZE(originalROMClass))
            != (J9ROMCLASS_FINALIZE_NEEDED(replacementROMClass)
                   || J9ROMCLASS_HAS_EMPTY_FINALIZE(replacementROMClass))) {
            if (J9ROMCLASS_FINALIZE_NEEDED(originalROMClass) || J9ROMCLASS_HAS_EMPTY_FINALIZE(originalROMClass)) {
        ...
        if (originalROMClass->interfaceCount == replacementROMClass->interfaceCount) {
            ...
            for (j = 0; j < originalROMClass->interfaceCount; ++j) {
                if (!utfsAreIdentical(NNSRP_PTR_GET(&(originalInterfaces[j]), J9UTF8*),
                        NNSRP_PTR_GET(&(replacementInterfaces[j]), J9UTF8*))) {
                    return JVMTI_ERROR_UNSUPPORTED_REDEFINITION_HIERARCHY_CHANGED;
                }
            }
        } else {
        ...
        rc = verifyFieldsAreSame(
            currentThread, 0, originalROMClass, replacementROMClass, extensionsEnabled, &classUsesExtensions);
        if (rc != JVMTI_ERROR_NONE) {
            return rc;
        }
        ...
        rc = verifyFieldsAreSame(
            currentThread, J9AccStatic, originalROMClass, replacementROMClass, extensionsEnabled, &classUsesExtensions);
        if (rc != JVMTI_ERROR_NONE) {
            return rc;
        }
...
jvmtiError verifyClassesCanBeReplaced(
    J9VMThread* currentThread, jint class_count, const jvmtiClassDefinition* class_definitions)
{
    jint i;
...
jvmtiError reloadROMClasses(J9VMThread* currentThread, jint class_count, const jvmtiClassDefinition* class_definitions,
    J9JVMTIClassPair* classPairs, UDATA options)
{
    J9JavaVM* vm = currentThread->javaVM;
    jint i;
...
jvmtiError verifyNewClasses(J9VMThread* currentThread, jint class_count, J9JVMTIClassPair* classPairs)
{
    J9JavaVM* vm = currentThread->javaVM;
    PORT_ACCESS_FROM_JAVAVM(vm);
    jint i = 0;
    ...
    /* CMVC 192547 : bytecode verification is usually done at runtime verification phase during ram class
     * initialisation, but since we're retransforming a class and the ram class is already initialized, manually run
     * bytecode verification on all the newly created ROM classes.
     */
    for (i = 0; i < class_count; ++i) {
        ...
        if (!J9ROMCLASS_IS_UNSAFE(classPairs[i].replacementClass.romClass)) {
            ...
            result = verifyData->verifyBytecodesFunction(
                PORTLIB, classPairs[i].originalRAMClass, classPairs[i].replacementClass.romClass, verifyData);
            if (-1 == result) {
...
void notifyGCOfClassReplacement(J9VMThread* currentThread, J9HashTable* classPairs, UDATA isFastHCR)
{
    ...
    while (classPair != NULL) {
        if (J9_ARE_ALL_BITS_SET(classPair->flags, J9JVMTI_CLASS_PAIR_FLAG_REDEFINED)) {
            mmFunctions->j9gc_notifyGCOfClassReplacement(
                currentThread, classPair->originalRAMClass, classPair->replacementClass.ramClass, isFastHCR);
        }
        classPair = hashTableNextDo(&hashTableState);
    }
}
    ...
    BOOLEAN fastHCR, J9HashTable** methodEquivalences, UDATA extensionsUsed)
{
    ...
    while (classPair != NULL) {
        ...
        if ((NULL != eventData) && (eventData->initialized) && (classPair->flags & J9JVMTI_CLASS_PAIR_FLAG_REDEFINED)) {
            jitEventAddClass(
                currentThread, eventData, (fastHCR ? replacementRAMClass : originalRAMClass), replacementRAMClass);
        ...
        if (originalRAMClass->romClass == replacementRAMClass->romClass) {
            ...
            for (oldMethodIndex = 0; oldMethodIndex < originalROMClass->romMethodCount; ++oldMethodIndex) {
                ...
                if (!fastHCR) {
                    rc = fixJNIMethodID(
                        currentThread, oldMethod, deleted ? NULL : newMethod, equivalent, extensionsUsed);
                    if (rc != JVMTI_ERROR_NONE) {
                        goto fail;
                    }
                }
    ...
    J9VMThread* currentThread, J9Method* oldMethod, J9Method* newMethod, J9HashTable** methodEquivalences, U_32 size)
{
    ...
    if (NULL == *methodEquivalences) {
        PORT_ACCESS_FROM_VMC(currentThread);
        *methodEquivalences
            = hashTableNew(OMRPORT_FROM_J9PORT(PORTLIB), J9_GET_CALLSITE(), size, sizeof(J9JVMTIMethodEquivalence), 0,
                0, J9MEM_CATEGORY_JVMTI, equivalenceHash, equivalenceEquals, NULL, NULL);
        if (NULL == *methodEquivalences) {
            rc = JVMTI_ERROR_OUT_OF_MEMORY;
            goto fail;
        }
...
void hshelpUTRegister(J9JavaVM* vm) { UT_J9HSHELP_MODULE_LOADED(J9_UTINTERFACE_FROM_VM(vm)); }
...
void jitClassRedefineEvent(J9VMThread* currentThread, J9JVMTIHCRJitEventData* jitEventData, UDATA extensionsEnabled)
{
    ...
    if (NULL != jitConfig) {
        jitConfig->jitClassesRedefined(
            currentThread, jitEventData->classCount, (J9JITRedefinedClass*)jitEventData->data);
        if (extensionsEnabled) {
