...
BOOLEAN list_isEmpty(j9list* aList) { return aList->first ? FALSE : TRUE; }

void* node_data(node* aNode) { return aNode->data; }

char* node_key(node* aNode) { return aNode->key; }

node* list_next(node* aNode) { return aNode->next; }

node* list_first(j9list* aList) { return aList->first; }
