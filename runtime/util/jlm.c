...
#define OBJ_MON_NAME_BUF_MINIMUM_SIZE                                                                             \
    (1 + OBJ_MON_NAME_POINTER_SIZE + 1 + 1 + OBJ_MON_NAME_CLASS_NAME_SIZE + 1 + OBJ_MON_NAME_POINTER_SIZE + 1 + 1 \
        + OBJ_MON_NAME_OBJ_TYPE_SIZE + 1 + 1 + 6)
...
#ifndef SWAP_2BYTES
#define SWAP_2BYTES(v) ((((v) >> (8 * 1)) & 0x00FF) | (((v) << (8 * 1)) & 0xFF00))
#endif

#ifndef SWAP_4BYTES
#define SWAP_4BYTES(v)                                                                                   \
    ((((v) >> (8 * 3)) & 0x000000FF) | (((v) >> (8 * 1)) & 0x0000FF00) | (((v) << (8 * 1)) & 0x00FF0000) \
        | (((v) << (8 * 3)) & 0xFF000000))
#endif

#ifndef SWAP_8BYTES
#define SWAP_8BYTES(v)                                                                                    \
    ((((v) >> (8 * 7)) & 0x000000FF) | (((v) >> (8 * 5)) & 0x0000FF00) | (((v) >> (8 * 3)) & 0x00FF0000)  \
        | (((v) >> (8 * 1)) & 0xFF000000) | (((v)&0xFF000000) << (8 * 1)) | (((v)&0x00FF0000) << (8 * 3)) \
        | (((v)&0x0000FF00) << (8 * 5)) | (((v)&0x000000FF) << (8 * 7)))
#endif
...
#define WRITE_1BYTE(val)   \
    *((U_8*)dump) = (val); \
    dump++;
#define WRITE_2BYTES(val)                          \
    *((U_16*)dump) = FORCE_BIG_ENDIAN_2BYTES(val); \
    dump += sizeof(U_16);
#define WRITE_4BYTES(val)                                \
    *((U_32*)dump) = FORCE_BIG_ENDIAN_4BYTES((U_32)val); \
    dump += sizeof(U_32);
#define WRITE_8BYTES(val)                          \
    *((U_64*)dump) = FORCE_BIG_ENDIAN_8BYTES(val); \
    dump += sizeof(U_64);
...
jint request_MonitorJlmDump(jvmtiEnv* env, J9VMJlmDump* jlmd, jint dump_format)
{
     ...
     */
    while (NULL
        != (lnrl_lock = memoryManagerFunctions->omrgc_walkLWNRLockTracePool(jvm->omrVM, &j9gc_LWNRLock_walk_state))) {
        WRITE_1BYTE(JVMTI_MONITOR_RAW);
        WRITE_1BYTE((unsigned char)held);
...
{
#if defined(OMR_THR_JLM_HOLD_TIMES)
    return (omrthread_jlm_init(J9THREAD_LIB_FLAG_JLM_ENABLED | J9THREAD_LIB_FLAG_JLM_TIME_STAMPS_ENABLED) == 0)
        ? (jint)JLM_SUCCESS
        : (jint)JLM_NOT_AVAILABLE;
#else
    return (jint)JLM_NOT_AVAILABLE;
#endif
}
...
static void GetMonitorName(J9VMThread* vmThread, J9ThreadAbstractMonitor* monitor, char* nameBuf)
{
    ...
    if (monitor->flags & J9THREAD_MONITOR_OBJECT) {
        ...
        j9str_printf(PORTLIB, nameBuf, OBJ_MON_NAME_BUF_SIZE, OBJ_MON_NAME_FORMAT, monitor,
            (length < OBJ_MON_NAME_CLASS_NAME_SIZE) ? length : OBJ_MON_NAME_CLASS_NAME_SIZE, name, object, objType);

        if (free) {
            j9mem_free_memory(name);
        }
    } else {
        j9str_printf(PORTLIB, nameBuf, OBJ_MON_NAME_BUF_SIZE, "[%p] %s", monitor,
...
jint request_MonitorJlmDumpSize(J9JavaVM* jvm, UDATA* dump_size, jint dump_format)
{
#if defined(OMR_THR_JLM)
    {
        ...
        while (NULL
            != (lnrl_lock
                   = memoryManagerFunctions->omrgc_walkLWNRLockTracePool(jvm->omrVM, &j9gc_LWNRLock_walk_state))) {
