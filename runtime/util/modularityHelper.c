...
static J9Package* hashPackageTableAtWithUTF8Name(
    J9VMThread* currentThread, J9ClassLoader* classLoader, J9UTF8* packageName);
static BOOLEAN isPackageExportedToModuleHelper(
    J9VMThread* currentThread, J9Module* fromModule, J9Package* j9package, J9Module* toModule, BOOLEAN toUnnamed);
...
#endif /* J9VM_JAVA9_BUILD >= 156 */
{
    ...
    if ((NULL == j9package->packageName)
        || ((packageNameLength + sizeof(j9package->packageName->length) + 1) > bufLen)) {
        j9package->packageName
            = j9mem_allocate_memory(packageNameLength + sizeof(j9package->packageName->length) + 1, OMRMEM_CATEGORY_VM);
        if (NULL == j9package->packageName) {
...
#else /* J9VM_JAVA9_BUILD >= 156 */
    j9package->packageName = vmFuncs->copyStringToJ9UTF8WithMemAlloc(
        currentThread, packageName, J9_STR_NULL_TERMINATE_RESULT, "", 0, (char*)buf, bufLen);
    if (NULL == j9package->packageName) {
        vmFuncs->setNativeOutOfMemoryError(currentThread, 0, 0);
        return FALSE;
    }
...
{
    return isPackageExportedToModuleHelper(currentThread, fromModule,
        getPackageDefinitionWithName(currentThread, fromModule, packageName, len, errCode), toModule, toUnnamed);
}
