...
#define BE_INT_AT(address)                                                                           \
    (((U_32)((U_8*)address)[3]) + ((U_32)((U_8*)address)[2] << 8) + ((U_32)((U_8*)address)[1] << 16) \
...
static void setReadLocation(UDATA readLoc) { readLocation = readLoc; }

static UDATA getReadLocation() { return readLocation; }
...
#define READ_SRP(ptr, type) \
    (DBGEXT_READFROMCOREFILE == getReadLocation()) ? (type)dbgReadSRP((J9SRP*)(ptr)) : SRP_GET(*((J9SRP*)ptr), type)
#else
...
static UDATA reloadClass(
    J9VMThread* vmThread, J9Class* originalClass, U_8* classFileBytes, UDATA classFileSize, J9ROMClass** result);
...
{
    U_32* ptr = getSRPPtr(
        J9ROMCLASS_OPTIONALINFO(romClass), romClass->optionalFlags, J9_ROMCLASS_OPTINFO_CLASS_ANNOTATION_INFO);
    if (NULL == ptr) {
        return NULL;
    }
...
{
    U_32* ptr = getSRPPtr(
        J9ROMCLASS_OPTIONALINFO(romClass), romClass->optionalFlags, J9_ROMCLASS_OPTINFO_TYPE_ANNOTATION_INFO);
    if (NULL == ptr) {
        return NULL;
    }
    return SRP_PTR_GET(ptr, U_32*);
...
compressLocalVariableTableEntry(I_32 deltaIndex, I_32 deltaStartPC, I_32 deltaLength, U_8* buffer)
{
     ...
     */
    if ((deltaIndex >= 0 && deltaIndex <= 1) && (deltaStartPC == 0)
        ...
        *buffer = result;
        return 1;
    } else if ((deltaIndex >= 0 && deltaIndex <= 1) && ((deltaStartPC >= -16) && (deltaStartPC <= 15)) /* 5 bit */
        ...
        *buffer = deltaLength & 0xFF;
        return 2;
    } else if ((deltaIndex >= 0 && deltaIndex <= 1) && ((deltaStartPC >= -256) && (deltaStartPC <= 255)) /* 9 bit */
    ...
    J9JavaVM* vm, J9ClassLoader* classLoader, J9ROMClass* romClass)
{
    ...
    ptr = getSRPPtr(
        J9ROMCLASS_OPTIONALINFO(romClass), romClass->optionalFlags, J9_ROMCLASS_OPTINFO_SOURCE_DEBUG_EXTENSION);
