    ...
    J9ROMClass* romClass, J9PortLibrary* portLib, J9TranslationBufferSet* translationBuffers, U_32 flags)
{
    ...
    j9tty_printf(PORTLIB, "Intermediate Class Data (%d bytes): %p\n", romClass->intermediateClassDataLength,
        romClass->intermediateClassData);
...
static I_32 dumpCallSiteData(J9PortLibrary* portLib, J9ROMClass* romClass, U_32 flags)
{
    ...
    if (0 != bsmCount) {
        ...
        for (i = 0; i < bsmCount; i++) {
            ...
            /* methodRef will be either a field or a method ref - they both have the same shape so we can pretend it is
             * always a methodref */
...
static I_32 dumpAnnotationInfo(J9PortLibrary* portLib, J9ROMClass* romClass, U_32 flags)
{
    ...
    /* print method, parameter, type, and default annotations */
    {
        ...
        for (i = 0; i < romClass->romMethodCount; i++) {
            ...
            if ((NULL != methodAnnotationData) || (NULL != parametersAnnotationData)
                || (NULL != defaultAnnotationData)) {
                j9tty_printf(PORTLIB, "      Name: ");
        ...
        if (NULL != methodInfo) {
            ...
            for (j = 0; j < getLineNumberCount(methodInfo); j++) {
                if (!getNextLineNumberFromTable(&currentLineNumber, &lineNumber)) {
                    UDATA sizeLeft = getLineNumberCompressedSize(methodInfo)
                        - (currentLineNumber - getLineNumberTable(methodInfo));
            ...
            while (values != NULL) {
                ...
                if (values->genericSignature) {
                    j9tty_printf(PORTLIB, "%.*s\n", J9UTF8_LENGTH(values->genericSignature),
                        J9UTF8_DATA(values->genericSignature));
                } else {
                    j9tty_printf(PORTLIB, "None\n");
                }
    ...
    if (J9ROMMETHOD_HAS_EXCEPTION_INFO(romMethod)) {
        ...
        if (exceptionData->catchCount) {
            ...
            for (i = 0; i < exceptionData->catchCount; i++) {
                j9tty_printf(PORTLIB, "     %5i%6i%10i   ", handler->startPC, handler->endPC, handler->handlerPC, 0);

                if (handler->exceptionClassIndex) {
                    dumpUTF(J9ROMSTRINGREF_UTF8DATA((J9ROMStringRef*)(&constantPool[handler->exceptionClassIndex])),
                        portLib, 0);
                    j9tty_printf(PORTLIB, "\n");
                } else {
 ...
 */
static U_32 dumpModifierWord(J9PortLibrary* portLib, U_32 modifiers, J9ModifierInfo* modInfo)
{
    PORT_ACCESS_FROM_PORT(portLib);
...
void printModifiers(J9PortLibrary* portLib, U_32 modifiers, modifierScope modScope, modifierSource modifierSrc)
{
    ...
    if (INCLUDE_INTERNAL_MODIFIERS == modScope) {
        if (MODIFIERSOURCE_METHOD == modifierSrc) {
            J9ModifierInfo modInfo[] = {
                { CFR_ACC_EMPTY_METHOD, "(empty)" }, { CFR_ACC_FORWARDER_METHOD, "(forwarder)" },
                { CFR_ACC_VTABLE, "(vtable)" }, { CFR_ACC_HAS_EXCEPTION_INFO, "(hasExceptionInfo)" },
                ...
                { J9AccMethodHasDefaultAnnotation, "(has default annotation)" }, { 0, "" } /* terminator */
            };
    ...
    if (modifiers)
        j9tty_printf(PORTLIB, "unknown_flags = 0x%X", modifiers);
}
