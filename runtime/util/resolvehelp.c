    ...
    J9VMThread* vmStruct, J9Class* currentClass, J9Class* resolvedClass, J9Method* method, UDATA lookupOptions)
{
    ...
    if ((J9AccSuper == (currentClass->romClass->modifiers & J9AccSuper))
        ...
        if ((isInterfaceMethod
                || ((currentDepth > superDepth) && (currentClass->superclasses[superDepth] == methodClass)))
            && (J9_ARE_NO_BITS_SET(resolvedClass->romClass->modifiers, J9AccInterface)
                   && J9_ARE_NO_BITS_SET(currentClass->romClass->modifiers, J9AccInterface))) {
            ...
            if (vTableOffset != 0) {
                ...
                if (isInterfaceMethod) {
                     ...
                     */
                    method = (J9Method*)vmFuncs->javaLookupMethod(vmStruct, superclass,
                        J9ROMMETHOD_NAMEANDSIGNATURE(J9_ROM_METHOD_FROM_RAM_METHOD(method)), currentClass,
                        lookupOptions);
                }
            }
        }
...
{
    /**
     * JVMS 4.9.2: If resolvedClass is an interface, ensure that it is a DIRECT superinterface of currentClass (or
     * resolvedClass == currentClass)
     */
    BOOLEAN result = FALSE;
    J9ROMClass* currentROMClass = currentClass->romClass;
    if (J9ROMCLASS_IS_UNSAFE(currentROMClass)) {
        /* If this is an unsafe class, we skip the check so that a lambda "inherit" super interfaces from its host
         * class. */
        result = TRUE;
    } else if (J9_ARE_ANY_BITS_SET(resolvedClass->romClass->modifiers, J9AccInterface)
        ...
        for (i = 0; i < interfaceCount; i++) {
            ...
            interfaceClass = vm->internalVMFunctions->hashClassTableAt(
                resolvedClass->classLoader, J9UTF8_DATA(interfaceName), J9UTF8_LENGTH(interfaceName));
...
static char* createErrorMessage(
    J9VMThread* vmStruct, J9Class* resolvedOrReceiverClass, J9Class* currentClass, const char* errorMsg)
{
    ...
    if (NULL != errorMsg) {
        ...
        UDATA bufLen = j9str_printf(PORTLIB, NULL, 0, errorMsg, J9UTF8_LENGTH(resolvedOrReceiverName),
            J9UTF8_DATA(resolvedOrReceiverName), J9UTF8_LENGTH(currentName), J9UTF8_DATA(currentName));
        if (bufLen > 0) {
            buf = j9mem_allocate_memory(bufLen, OMRMEM_CATEGORY_VM);
            if (NULL != buf) {
                j9str_printf(PORTLIB, buf, bufLen, errorMsg, J9UTF8_LENGTH(resolvedOrReceiverName),
                    J9UTF8_DATA(resolvedOrReceiverName), J9UTF8_LENGTH(currentName), J9UTF8_DATA(currentName));
            }
        }
    }
    ...
    J9VMThread* vmStruct, J9Class* receiverClass, J9Class* currentClass)
{
    ...
    const char* errorMsg = j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE,
        J9NLS_VMUTIL_RECEIVERCLASS_NOT_SAME_OR_SUBTYPE_OF_CURRENTCLASS, NULL);
    ...
    J9VMThread* vmStruct, J9Class* resolvedClass, J9Class* currentClass)
{
    ...
    /* Construct error string */
    const char* errorMsg = j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE,
        J9NLS_VMUTIL_DEFAULT_METHOD_INVALID_SUPERSEND, NULL);
