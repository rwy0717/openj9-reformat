...
static J9ROMMethod* allSlotsInROMMethodDo(
    J9ROMClass* romClass, J9ROMMethod* method, J9ROMClassWalkCallbacks* callbacks, void* userData);
static UDATA allSlotsInROMFieldDo(
    J9ROMClass* romClass, J9ROMFieldShape* field, J9ROMClassWalkCallbacks* callbacks, void* userData);
static void allSlotsInConstantPoolDo(J9ROMClass* romClass, J9ROMClassWalkCallbacks* callbacks, void* userData);
static void allSlotsInEnclosingObjectDo(
    J9ROMClass* romClass, J9EnclosingObject* enclosingObject, J9ROMClassWalkCallbacks* callbacks, void* userData);
static UDATA allSlotsInAnnotationDo(J9ROMClass* romClass, U_32* annotation, const char* annotationSectionName,
    J9ROMClassWalkCallbacks* callbacks, void* userData);
static void allSlotsInExceptionInfoDo(
    J9ROMClass* romClass, J9ExceptionInfo* exceptionInfo, J9ROMClassWalkCallbacks* callbacks, void* userData);
static UDATA allSlotsInMethodDebugInfoDo(
    J9ROMClass* romClass, U_32* cursor, J9ROMClassWalkCallbacks* callbacks, void* userData);
static void allSlotsInOptionalInfoDo(J9ROMClass* romClass, J9ROMClassWalkCallbacks* callbacks, void* userData);
static UDATA allSlotsInStackMapFramesDo(
    J9ROMClass* romClass, U_8* cursor, U_16 frameCount, J9ROMClassWalkCallbacks* callbacks, void* userData);
static void allSlotsInStackMapDo(
    J9ROMClass* romClass, U_8* stackMap, J9ROMClassWalkCallbacks* callbacks, void* userData);
static UDATA allSlotsInVerificationTypeInfoDo(
    J9ROMClass* romClass, U_8* cursor, J9ROMClassWalkCallbacks* callbacks, void* userData);
static void allSlotsInBytecodesDo(
    J9ROMClass* romClass, J9ROMMethod* method, J9ROMClassWalkCallbacks* callbacks, void* userData);
...
static UDATA allSlotsInMethodParametersDataDo(
    J9ROMClass* romClass, U_8* cursor, J9ROMClassWalkCallbacks* callbacks, void* userData);

static void allSlotsInStaticSplitMethodRefIndexesDo(
    J9ROMClass* romClass, J9ROMClassWalkCallbacks* callbacks, void* userData);
static void allSlotsInSpecialSplitMethodRefIndexesDo(
    J9ROMClass* romClass, J9ROMClassWalkCallbacks* callbacks, void* userData);
...
#define SLOT_CALLBACK(romClassPtr, slotType, structure, slotName) \
    callbacks->slotCallback(romClassPtr, slotType, &structure->slotName, #slotName, userData)
/*
 ...
 */
void allSlotsInROMClassDo(J9ROMClass* romClass, J9ROMClassSlotCallback callback,
    J9ROMClassSectionCallback sectionCallback, J9ROMClassValidateRangeCallback validateRangeCallback, void* userData)
{
    ...
    romClassWalkCallbacks.validateRangeCallback
        = (NULL != validateRangeCallback ? validateRangeCallback : defaultValidateRangeCallback);
    ...
    if (rangeValid) {
        callbacks->sectionCallback(
            romClass, srpCursor, count * sizeof(J9SRP) * 2, "cpNamesAndSignaturesSRPs", userData);
    }
    ...
    J9ROMClass* romClass, J9ROMMethod* method, J9ROMClassWalkCallbacks* callbacks, void* userData)
{
    ...
    if (J9ROMMETHOD_HAS_EXCEPTION_INFO(method)) {
        ...
        UDATA exceptionInfoSize = sizeof(J9ExceptionInfo) + exceptionInfo->catchCount * sizeof(J9ExceptionHandler)
...
static void allSlotsInConstantPoolDo(J9ROMClass* romClass, J9ROMClassWalkCallbacks* callbacks, void* userData)
{
    ...
    for (index = 0; index < constPoolCount; index++) {
        ...
        case J9CPTYPE_ANNOTATION_UTF8:
            callbacks->slotCallback(
                romClass, J9ROM_UTF8, &((J9ROMStringRef*)&constantPool[index])->utf8Data, "cpFieldUtf8", userData);
            callbacks->slotCallback(
                romClass, J9ROM_U32, &((J9ROMStringRef*)&constantPool[index])->cpType, "cpFieldType", userData);
            break;

        case J9CPTYPE_INT:
            callbacks->slotCallback(romClass, J9ROM_U32, &constantPool[index], "cpFieldInt", userData);
            callbacks->slotCallback(
                romClass, J9ROM_U32, (void*)((UDATA)&constantPool[index] + sizeof(U_32)), "cpFieldIntUnused", userData);
            break;
        case J9CPTYPE_FLOAT:
            callbacks->slotCallback(romClass, J9ROM_U32, &constantPool[index], "cpFieldFloat", userData);
            callbacks->slotCallback(romClass, J9ROM_U32, (void*)((UDATA)&constantPool[index] + sizeof(U_32)),
                "cpFieldFloatUnused", userData);
            break;
        ...
        case J9CPTYPE_FIELD:
            callbacks->slotCallback(
                romClass, J9ROM_NAS, &((J9ROMFieldRef*)&constantPool[index])->nameAndSignature, "cpFieldNAS", userData);
            callbacks->slotCallback(romClass, J9ROM_U32, &((J9ROMFieldRef*)&constantPool[index])->classRefCPIndex,
                "cpFieldClassRef", userData);
            break;
        ...
        case J9CPTYPE_INTERFACE_STATIC_METHOD:
            callbacks->slotCallback(romClass, J9ROM_NAS, &((J9ROMMethodRef*)&constantPool[index])->nameAndSignature,
                "cpFieldNAS", userData);
            callbacks->slotCallback(romClass, J9ROM_U32, &((J9ROMMethodRef*)&constantPool[index])->classRefCPIndex,
                "cpFieldClassRef", userData);
            break;

        case J9CPTYPE_METHOD_TYPE:
            callbacks->slotCallback(
                romClass, J9ROM_UTF8, &((J9ROMMethodTypeRef*)&constantPool[index])->signature, "cpFieldUtf8", userData);
            callbacks->slotCallback(
                romClass, J9ROM_U32, &((J9ROMMethodTypeRef*)&constantPool[index])->cpType, "cpFieldType", userData);
            break;

        case J9CPTYPE_METHODHANDLE:
            callbacks->slotCallback(romClass, J9ROM_U32,
                &((J9ROMMethodHandleRef*)&constantPool[index])->methodOrFieldRefIndex, "cpFieldMethodOrFieldRef",
                userData);
            callbacks->slotCallback(romClass, J9ROM_U32,
                &((J9ROMMethodHandleRef*)&constantPool[index])->handleTypeAndCpType, "cpFieldHandleTypeAndCpType",
                userData);
            break;
...
static void allSlotsInOptionalInfoDo(J9ROMClass* romClass, J9ROMClassWalkCallbacks* callbacks, void* userData)
{
    ...
    if (J9_ROMCLASS_OPTINFO_CLASS_ANNOTATION_INFO
        == (romClass->optionalFlags & J9_ROMCLASS_OPTINFO_CLASS_ANNOTATION_INFO)) {
        rangeValid = callbacks->validateRangeCallback(romClass, cursor, sizeof(J9SRP), userData);
        if (rangeValid) {
    ...
    J9ROMClass* romClass, J9ROMClassWalkCallbacks* callbacks, void* userData)
{
    ...
    if (count > 0) {
        ...
        if (rangeValid) {
            ...
            callbacks->sectionCallback(
                romClass, cursor, count * sizeof(U_16), "specialSplitMethodRefIndexes", userData);
            for (i = 0; i < count; i++) {
                callbacks->slotCallback(romClass, J9ROM_U16, cursor, "cpIndex", userData);
                cursor += 1;
            }
    ...
    J9ROMClass* romClass, U_32* cursor, J9ROMClassWalkCallbacks* callbacks, void* userData)
{
    ...
    U_8* variableTable;
    /* if data is out of line, then the size of the data inline in the method is a single SRP in sizeof(U_32
     * increments), currently assuming J9SRP is U_32 aligned*/
    ...
    callbacks->slotCallback(
        romClass, J9ROM_U32, &methodDebugInfo->srpToVarInfo, "SizeOfDebugInfo(low tagged)", userData);
    callbacks->slotCallback(
        romClass, J9ROM_U32, &methodDebugInfo->lineNumberCount, "lineNumberCount(low tagged)", userData);
    SLOT_CALLBACK(romClass, J9ROM_U32, methodDebugInfo, varInfoCount);
    if (1 == (methodDebugInfo->lineNumberCount & 0x1)) {
        callbacks->slotCallback(
            romClass, J9ROM_U32, (U_32*)(methodDebugInfo + 1), "compressed line number size", userData);
    }
    ...
    if (NULL != lineNumberTable) {
        U_32 j;
        rangeValid = callbacks->validateRangeCallback(
            romClass, lineNumberTable, lineNumberTableCompressedSize * sizeof(U_8), userData);
        if (rangeValid) {
    ...
    if (NULL != variableTable) {
        ...
        while (NULL != values) {
            ...
            if (NULL != values->genericSignature) {
                rangeValid
                    = callbacks->validateRangeCallback(romClass, values->genericSignature, sizeof(J9UTF8), userData);
                if (rangeValid) {
                    callbacks->slotCallback(
                        romClass, J9ROM_UTF8_NOSRP, values->genericSignature, "genericSignature", userData);
                }
            }
...
static void allSlotsInCallSiteDataDo(J9ROMClass* romClass, J9ROMClassWalkCallbacks* callbacks, void* userData)
{
    ...
    rangeValid = callbacks->validateRangeCallback(
        romClass, callSiteData, callSiteCount * sizeof(J9SRP) + callSiteCount * sizeof(U_16), userData);
    if (FALSE == rangeValid) {
        return;
    }
    ...
    for (index = 0; index < bsmCount; index++) {
        ...
        if (0 != bsmArgumentCount) {
            rangeValid
                = callbacks->validateRangeCallback(romClass, bsmDataCursor, bsmArgumentCount * sizeof(U_16), userData);
            if (FALSE == rangeValid) {
                return;
            }
    ...
    callbacks->sectionCallback(
        romClass, callSiteData, (U_8*)bsmDataCursor - (U_8*)callSiteData, "callSiteData", userData);
}

static void nullSlotCallback(J9ROMClass* romClass, U_32 type, void* slotPtr, const char* slotName, void* userData)
{
    /* does nothing */
}

static void nullSectionCallback(
    J9ROMClass* romClass, void* sectionPtr, UDATA sectionLength, const char* sectionName, void* userData)
{
    /* does nothing */
}

static BOOLEAN defaultValidateRangeCallback(J9ROMClass* romClass, void* address, UDATA length, void* userData)
{
    /* everything is valid */
    return TRUE;
}
