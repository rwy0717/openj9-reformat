...
#define NEXT(srpPointer) \
    ((J9SRP*)((char*)srpPointer + *srpPointer + srptable->srpHashtableInternal->nodeSize - sizeof(J9SRP)))
...
BOOLEAN srpHashTable_checkConsistency(J9SRPHashTable* srptable, J9PortLibrary* portLib,
    BOOLEAN (*doFunction)(void* anElement, void* userData), void* userData, UDATA skipCount);
 ...
 * ->printFn					void  (*)(J9PortLibrary *portLibrary, void *key, void *userData) | |	->portLibrary				struct J9PortLibrary*												   |
 * 	|	->functionUserData			void*
 * |
 *  |______________________________________________________________________________________________________|
 *
 *   _______________________________________________________________________________________________________
 * 	|struct J9SRPHashTableInternal |		Nodes		| 			SimplePool		|		Entry
 * Nodes	   	|
 * 	|							   |	|	|	|	|	|	struct	J9SimplePool	|	|
 * |	|	|	|	| | ->tableSize	  U_32	       |	|	|	|	|	|
 * |	|	|	|	|	|	|
 ...
 * |	|	|	| | ->flags         U_32  	   |	|	|	|	|	|	->firstFreeSlot
 * J9SRP	|	|	|	|	|	|	|
 *  | ->nodes         J9SRP		   |	|	|	|	|	|	->blockEnd		J9SRP	|	|	|
 * |	|	|	| | ->nodePool      J9SRP		   |	|	|	|	|	|	->flags
 * U_32	|	| 	|	|	|	|	|
 ...
 */
J9SRPHashTable* srpHashTableNew(J9PortLibrary* portLibrary, const char* tableName, U_32 tableSize, U_32 entrySize,
    U_32 flags, J9SRPHashTableHashFn hashFn, J9SRPHashTableEqualFn hashEqualFn, J9SRPHashTablePrintFn printFn,
    void* functionUserData)
{
    ...
    Trc_srpHashTableNew_Entry(
        portLibrary, tableName, tableSize, entrySize, flags, hashFn, hashEqualFn, printFn, functionUserData);

    /*Allocate memory for struct J9SRPHashTable*/
    srpHashTable = OMRPORT_FROM_J9PORT(PORTLIB)->mem_allocate_memory(
        OMRPORT_FROM_J9PORT(PORTLIB), sizeof(J9SRPHashTable), tableName, OMRMEM_CATEGORY_VM);
    ...
    allocatedMemory = OMRPORT_FROM_J9PORT(PORTLIB)->mem_allocate_memory(
        OMRPORT_FROM_J9PORT(PORTLIB), totalSize, tableName, OMRMEM_CATEGORY_VM);
    ...
    temppool = simplepool_new((void*)(((char*)srpHashTableInternal) + srpHashTableInternalSize + srpHashTableTotalSize),
        simplePoolTotalSize, simplePoolNodeSize, 0);
 ...
 */
J9SRPHashTable* srpHashTableNewInRegion(J9PortLibrary* portLibrary, const char* tableName, void* address,
    U_32 memorySize, U_32 entrySize, U_32 flags, J9SRPHashTableHashFn hashFn, J9SRPHashTableEqualFn hashEqualFn,
    J9SRPHashTablePrintFn printFn, void* functionUserData)
{
    ...
    /*Allocate memory for struct J9SRPHashTable*/
    srpHashTableTemp = OMRPORT_FROM_J9PORT(PORTLIB)->mem_allocate_memory(
        OMRPORT_FROM_J9PORT(PORTLIB), sizeof(J9SRPHashTable), tableName, OMRMEM_CATEGORY_VM);
#ifdef SRPHASHTABLE_DEBUG
    ...
    srpHashTable = srpHashTableReset(portLibrary, tableName, srpHashTableTemp, address, memorySize, entrySize, flags,
        hashFn, hashEqualFn, printFn, functionUserData);
 ...
 *
 */
J9SRPHashTable* srpHashTableReset(J9PortLibrary* portLibrary, const char* tableName, J9SRPHashTable* srpHashTable,
    void* address, U_32 memorySize, U_32 entrySize, U_32 flags, J9SRPHashTableHashFn hashFn,
    J9SRPHashTableEqualFn hashEqualFn, J9SRPHashTablePrintFn printFn, void* functionUserData)
{
    U_32 tableSize = 0;
    U_32 simplePoolNodeSize = 0;
    J9SRPHashTableInternal* srpHashTableInternal;
    ...
    Trc_srpHashTableReset_Entry(portLibrary, tableName, srpHashTable, address, memorySize, entrySize, flags, hashFn,
        hashEqualFn, printFn, functionUserData);
     ...
     * if the given memory is too big that the number of elements that fits in it is not in the supported range of
     * primeNumberHelper then return NULL
     */
    if (tableSize == 0) {
        Trc_srpHashTableReset_memoryIsTooSmall(
            memorySize, srpHashTable_requiredMemorySize(2, sizeof(J9SharedInternSRPHashTableEntry), TRUE));
    ...
    temppool = simplepool_new((void*)(((char*)srpHashTableInternal) + sizeof(J9SRPHashTableInternal)
                                  + ROUND_TO_SIZEOF_UDATA(sizeof(J9SRP) * tableSize)),
        simplepool_totalSize(simplePoolNodeSize, tableSize), simplePoolNodeSize, 0);
 ...
 * @param portLibrary           The port library
 * @param tableName     		A string giving the name of the SRP table, recommended to use J9_GET_CALLSITE()
 * 								within caller code, this is not a restriction. If a unique name is
 *used, it must be easily searchable, i.e. there should not be more than a few false hits when searching for the name.
 *If necessary to ensure uniqueness or find-ability, create a list of the unique names or name conventions in this file.
 ...
 */
J9SRPHashTable* srpHashTableRecreate(J9PortLibrary* portLibrary, const char* tableName, void* address,
    J9SRPHashTableHashFn hashFn, J9SRPHashTableEqualFn hashEqualFn, J9SRPHashTablePrintFn printFn,
    void* functionUserData)
{
    J9SRPHashTable* srpHashTable;
...
#ifdef SRPHASHTABLE_DEBUG
    j9tty_printf(PORTLIB, "srpHashTableNew <%s>: tableSize=%d, srpTable=%p\n", tableName,
        srpHashTableInternal->tableSize, srpHashTable);
#endif
    if (!srpHashTable) {
        return NULL;
    }
 ...
 */
void srpHashTableFree(J9SRPHashTable* srptable)
{
 ...
 * 							Also it should provide enough info to compare with the existing
 * entries. A SRP HashTable Entry can be passed as a key.
 * @return                  NULL if entry is not present in the table;
 * 							otherwise a pointer to the user-data
 *
 */
void* srpHashTableFind(J9SRPHashTable* srptable, void* key)
{
    J9SRP* srpnode = 0;
    PORT_ACCESS_FROM_PORT(srptable->portLibrary);
 ...
 * 							to do comparison with the entries in SRP hashtable to find out
 * whether such entry already exist or where to add the value.
 * @return                  NULL on failure (to allocate a new node);
 * 							pointer to an existing entry if it already exist,
 * 							otherwise pointer to an empty simplepool node that is added
 * plus 1.
 *
 ...
 */
void* srpHashTableAdd(J9SRPHashTable* srptable, void* key)
{
    J9SRP* srpnode;
    void* newElement;
 ...
 */
U_32 srpHashTableRemove(J9SRPHashTable* srptable, void* key)
{
    J9SRP* srpnode;
    void* removedNode;
 ...
 */
void srpHashTableForEachDo(J9SRPHashTable* srptable, J9SRPHashTableDoFn doFn, void* opaque)
{
    void* node;
    U_32 bucketIndex = 0;
 ...
 */
void srpHashTableDumpDistribution(J9SRPHashTable* srptable)
{
    U_32 i, nodeCount;
    U_32 usedNodes = 0;
    J9SRP* srpnodes;
 ...
 *
 */
void srpHashTablePrintDebugInfo(J9SRPHashTable* srptable, UDATA numberOfNodesToPrint)
{
    J9SRPHashTableInternal* srptableInternal;
    J9SRP* nodes;
    ...
    j9tty_printf(PORTLIB, "SimplePool->freeList : \t\t%d,\tPoints to :\t%d \n", simplepool->freeList,
        SRP_GET(simplepool->freeList, void*));
    j9tty_printf(PORTLIB, "SimplePool->firstFreeSlot :\t%d,\tPoints to :\t%d \n", simplepool->firstFreeSlot,
        SRP_GET(simplepool->firstFreeSlot, void*));
    j9tty_printf(PORTLIB, "SimplePool->blockEnd :\t\t%d,\tPoints to :\t%d \n\n", simplepool->blockEnd,
        SRP_GET(simplepool->blockEnd, void*));
    ...
    for (i = 0; i < numberOfNodesToPrint && i < srptableInternal->tableSize; i++) {
        ...
        j9tty_printf(PORTLIB, "ELEMENT[%d]_SRP Address :\t%d,\tValue :\t%d \n", i,
            (J9SRP*)((char*)sp_element + srptableInternal->nodeSize - sizeof(J9SRP)),
            *((J9SRP*)((char*)sp_element + srptableInternal->nodeSize - sizeof(J9SRP))));
        j9tty_printf(
            PORTLIB, "ELEMENT[%d]_bytes4-8 :\tValue :\t%d \n", i, *((J9SRP*)((char*)sp_element + sizeof(J9SRP))));
        j9tty_printf(PORTLIB, "\n");
...
U_32 srpHashTableGetCount(J9SRPHashTable* srptable) { return srptable->srpHashtableInternal->numberOfNodes; }
 ...
 */
BOOLEAN
srpHashTableVerify(J9SRPHashTable* srptable, U_32 memorySize, U_32 entrySize)
{
    J9SRPHashTableInternal* srptableInternal = srptable->srpHashtableInternal;
    J9SRP* nodes = J9SRPHASHTABLEINTERNAL_NODES(srptableInternal);
    ...
    if (entrySize != srptableInternal->entrySize) {
        ...
        Trc_srpHashTableVerify_numberOfNodesIncorrect(
            srptable, simplepool_numElements(simplePool), srptableInternal->numberOfNodes);
        ...
        if (recalculatedTableSize != srptableInternal->tableSize) {
        ...
        } else if (((UDATA)simplePool)
            != ((UDATA)nodes + ROUND_TO_SIZEOF_UDATA(srptableInternal->tableSize * sizeof(J9SRP)))) {
            Trc_srpHashTableVerify_simplePoolAddressIncorrect(
                srptable, simplePool, ((char*)nodes) + (srptableInternal->tableSize * sizeof(J9SRP)));
            /* Verify SimplePool */
        } else if (!simplepool_verify(simplePool,
            ...
            Trc_srpHashTableVerify_nodesAddressIncorrect(
                srptable, nodes, ((char*)srptableInternal) + sizeof(J9SRPHashTableInternal));
        } else {
            verifies = TRUE;
        }
    }
    ...
    for (i = 0; verifies && (i < srptableInternal->tableSize); i++) {
        ...
        while (verifies && (*currentSRP != 0)) {
            if (!simplepool_isElement(simplePool, SRP_GET(*currentSRP, void*))) {
                Trc_srpHashTableVerify_SRPPointsToNonSimplepoolElement(
                    currentSRP, *currentSRP, SRP_GET(*currentSRP, void*), srptable);
                verifies = FALSE;
            }
            numberOfSRPs++;
 ...
 * @param[in] skipCount		Number of elements to skip after each iteration, 0 means every element will be iterated
 *over
 *
 * @return BOOLEAN	Returns FALSE if the doFunction returns FALSE for any element, TRUE otherwise
 */
BOOLEAN
srpHashTable_checkConsistency(J9SRPHashTable* srptable, J9PortLibrary* portLib,
    BOOLEAN (*doFunction)(void* anElement, void* userData), void* userData, UDATA skipCount)
{
 ...
 *
 */
static J9SRP* srpHashTableFindNode(J9SRPHashTable* srptable, void* key)
{
    UDATA hash;
    J9SRP* srpnode;
    J9SRP* srpnodes;
...
U_32 srpHashTable_requiredMemorySize(U_32 tableSize, U_32 entrySize, BOOLEAN ceilUp)
{
    ...
    if (tableSize == 0) {
        ...
        requiredMinSize = sizeof(J9SRPHashTableInternal) + ROUND_TO_SIZEOF_UDATA(sizeof(J9SRP) * tableSize)
...
/**
 * Calculates the tableSize (max number of elements that can be stored in SRPHashTable) of SRPHashTable for a given
 * memory. If user is not restricted with the given memory size, then tableSize is ceil up to next prime number.
 ...
 */
U_32 srpHashTable_calculateTableSize(U_32 memorySize, U_32 entrySize, BOOLEAN canExtendGivenMemory)
{
    U_32 simplePoolNodeSize = 0;
    ...
    if (canExtendGivenMemory) {
        ...
        /*
         * If tableSize is PRIMENUMBERHELPER_OUTOFRANGE, then given memory size is not supported by primeNumberHelper
         * primes. If this is the case, just return it and do not do next if statement which checks padding.
         */
         ...
         */
        if (srpHashTable_requiredMemorySize((remainingMemorySize / requiredSizePerEntry), entrySize, FALSE)
            > memorySize) {
 ...
 */
U_32 srpHashTable_tableSize(J9SRPHashTable* srptable) { return srptable->srpHashtableInternal->tableSize; }
