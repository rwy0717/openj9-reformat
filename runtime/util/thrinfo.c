...
#define J9THREAD_MONITOR_FROM_LOCKWORD(lockWord) \
    ((J9ThreadAbstractMonitor*)READU(J9_INFLLOCK_OBJECT_MONITOR(lockWord)->monitor))

static j9objectmonitor_t getLockWord(J9VMThread* vmThread, j9object_t object);
static UDATA getVMThreadStateHelper(J9VMThread* targetThread, j9object_t* pLockObject, omrthread_monitor_t* pRawLock,
    J9VMThread** pLockOwner, UDATA* pCount, BOOLEAN includeRawMonitors);
static void getInflatedMonitorState(const J9VMThread* targetThread, const omrthread_t j9self,
    const omrthread_state_t* j9state, UDATA* vmstate, omrthread_monitor_t* rawLock, J9VMThread** lockOwner,
    UDATA* count);
#if defined(J9VM_OUT_OF_PROCESS)
...
static UDATA hashWrappedMonitorHash(void* key, void* userData)
{
    return ((UDATA)((WrappedJ9ObjectMonitor*)key)->object);
}

static UDATA hashWrappedMonitorCompare(void* leftKey, void* rightKey, void* userData)
{
...
{
    return getVMThreadStateHelper(targetThread, pLockObject, NULL, pLockOwner, pCount, DONT_INCLUDE_RAW_MONITORS);
}
 ...
 */
UDATA
getVMThreadRawState(J9VMThread* targetThread, j9object_t* pLockObject, omrthread_monitor_t* pRawMonitor,
...
UDATA
getVMThreadRawStatesAll(J9VMThread* targetThread, j9object_t* pLockObject, omrthread_monitor_t* pRawMonitor,
    J9VMThread** pLockOwner, UDATA* pCount)
{
    return getVMThreadStateHelper(targetThread, pLockObject, pRawMonitor, pLockOwner, pCount, INCLUDE_RAW_MONITORS);
}
    ...
    if (targetThread) {
        ...
        if (publicFlags & (J9_PUBLIC_FLAGS_THREAD_BLOCKED | J9_PUBLIC_FLAGS_THREAD_WAITING)) {
            ...
            if (J9_LOCK_IS_INFLATED(lockWord)) {
                J9ThreadAbstractMonitor* objmon
                    = getInflatedObjectMonitor(targetThread->javaVM, targetThread, lockObject, lockWord);
                ...
                if (objmon) {
                    ...
                    if (publicFlags & J9_PUBLIC_FLAGS_THREAD_BLOCKED) {
                        ...
                        if (!j9self) {
                            ...
                            getInflatedMonitorState(
                                targetThread, j9self, &j9state, &vmstate, &rawLock, &lockOwner, &count);
                        }
                        /*
                ...
                if (lockOwner && (lockOwner != LOCAL_TO_TARGET(targetThread))) {
                    ...
                    rawLock
                        = (omrthread_monitor_t)monitorTablePeekMonitor(targetThread->javaVM, targetThread, lockObject);
                    vmstate = J9VMTHREAD_STATE_BLOCKED;
                }
            }
            /*
            ...
            if (!j9self || (j9state.flags & J9THREAD_FLAG_PARKED)) {
                ...
                if (lockObject) {
                    ...
                    if (aosClazz) {
                        ...
                        /* PR 80305 : Do not write back to the castClassCache as this code may be running while the GC
                         * is unloading the class */
                        if (instanceOfOrCheckCastNoCacheUpdate(clazz, aosClazz)) {
#if defined(J9VM_OUT_OF_PROCESS)
                            {
                                ...
                                fobj = readObjectField(lockObject, clazz, "exclusiveOwnerThread", "Ljava/lang/Thread;",
                                    sizeof(fj9object_t));
                                lockOwnerObject = J9OBJECT_FROM_FJ9OBJECT(targetThread->javaVM, fobj);
                            }
#else
                            /* Simplify the macro usage by using the _VM version so we do not need to pass in the
                             * current thread */
                            lockOwnerObject
                                = J9VMJAVAUTILCONCURRENTLOCKSABSTRACTOWNABLESYNCHRONIZER_EXCLUSIVEOWNERTHREAD_VM(
                                    targetThread->javaVM, lockObject);
#endif
                            if (lockOwnerObject) {
#if defined(J9VM_OUT_OF_PROCESS)
                                lockOwner = (J9VMThread*)(UDATA)readObjectField(lockOwnerObject,
                                    READCLAZZ(targetThread, lockOwnerObject), "threadRef", "J", sizeof(jlong));
#else
                                lockOwner = (J9VMThread*)J9VMJAVALANGTHREAD_THREADREF_VM(
                                    targetThread->javaVM, lockOwnerObject);
#endif
                            }
                        }
                    }
        ...
        if (J9VMTHREAD_STATE_RUNNING == vmstate) {
            if (includeRawMonitors) {
                ...
                lockObject = NULL;
                getInflatedMonitorState(targetThread, j9self, &j9state, &vmstate, &rawLock, &lockOwner, &count);
            }
        }

        /* sanitize irrelevant parameters */
        if ((J9VMTHREAD_STATE_RUNNING == vmstate) || (J9VMTHREAD_STATE_SUSPENDED == vmstate)
        ...
        if (rawLock && pLockObject && (!lockObject)) {
            if ((READU(((J9ThreadAbstractMonitor*)rawLock)->flags) & J9THREAD_MONITOR_OBJECT)
                == J9THREAD_MONITOR_OBJECT) {
...
getVMThreadStatus_DEPRECATED(J9VMThread* thread, J9ThreadAbstractMonitor** pmonitor, J9VMThread** powner, UDATA* pcount)
{
    ...
    if (blockingEnterObject) {
        ...
        if (J9_LOCK_IS_INFLATED(lockWord)) {
            J9ThreadAbstractMonitor* monitorStruct
                = getInflatedObjectMonitor(thread->javaVM, thread, blockingEnterObject, lockWord);
 ...
 */
J9ThreadAbstractMonitor* monitorTablePeekMonitor(J9JavaVM* vm, J9VMThread* targetVMThread, j9object_t object)
{
 ...
 */
J9ObjectMonitor* monitorTablePeek(J9JavaVM* vm, J9VMThread* targetVMThread, j9object_t object)
{
#if defined(J9VM_OUT_OF_PROCESS)
     ...
     * and corrupt heap by expected read-only operation. So "sniff" hash bits before and ignore request if hash bit is
     * not set) If hash bit for object is not set such object can not be found in hash table any way
     *
     * For more information see CMVC 179161
     */
    if (0
        != (TMP_J9OBJECT_FLAGS(object)
               & (OBJECT_HEADER_HAS_BEEN_HASHED_IN_CLASS | OBJECT_HEADER_HAS_BEEN_MOVED_IN_CLASS))) {
...
static U_64 readObjectField(j9object_t object, J9Class* clazz, U_8* fieldName, U_8* fieldSig, UDATA fieldBytes)
{
    ...
    offset = instanceFieldOffset(
        NULL, clazz, fieldName, strlen(fieldName), fieldSig, strlen(fieldSig), NULL, NULL, J9_LOOK_NO_JAVA);
    if (offset >= 0) {
