    ...
    const char* optionValue, UDATA doConsumeArgs)
{
    ...
    if (searchForward) {
        if (stopAtIndex < j9vm_args->nOptions) {
            searchFromIndex = stopAtIndex; /* stopAtIndex is misnamed if searching forwards - actually "startFromIndex"
                                              - the macro adds 1 for you*/
    ...
    while ((optionCntr <= searchToIndex) && (optionCntr >= searchFromIndex)) {
        ...
        if (testString != NULL) {
            ...
            case OPTIONAL_LIST_MATCH:
                if (charCntr == optionLength) { /* startswith or exact match */
                    if (HAS_MAPPING(j9vm_args, optionCntr) && (realMatch == EXACT_MEMORY_MATCH)) {
                        /* once we get here, we know we found the right index, but because the specified option and the
                           mapped option may be different lengths we have to use the specified option length to
                           correctly get the value. */
                    ...
                    } else if (realMatch == OPTIONAL_LIST_MATCH) {
            ...
            if (success) {
                /* Search for a specific parameter after the optionName. eg. -Xverify:none. Only use with
                 * STARTSWITH_MATCH or OPTIONAL_LIST_MATCH. */
                /* Assumes that a value will terminate in a comma, space or EOL */
                /* Search is NOT case sensitive and also does not find substrings eg. searching for "no" in
                 * -Xverify:none will not succeed */
                if (optionValue && ((realMatch == STARTSWITH_MATCH) || (realMatch == OPTIONAL_LIST_MATCH))) {
                    success = FALSE;
                    optionValueOperations(
                        PORTLIB, j9vm_args, optionCntr, GET_OPTION, &valueString, 0, ':', 0, NULL); /* assumes ':' */
                    if (valueString) {
                        cursor = strrchr(
                            valueString, ':'); /* Skip past any additional sub-options eg. -Xdump:java:<value> */
                        if (NULL != cursor) {
                            ++cursor;
                        } else {
                            cursor = valueString;
                ...
                if (success) {
                    if (doConsumeArgs) { /* If we are consuming args, we need to sweep the whole array for duplicates */
                        if (hasBeenSet) {
                            ...
                            j9vm_args->j9Options[optionCntr].flags
                                = NOT_CONSUMABLE_ARG /* override any flags that already exist */
...
/* Support function for optionValueOperations.
        Searches the entire command-line and "builds" an values list from a number of options. Required as mapped values
   need to break "duplicate" rules. Eg. -Xcomp means -Xjit:count=0. So, for "j9 -Xcomp -Xjit:lowopt", -Xcomp would be
   considered a duplicate. This is wrong. In this case, GET_COMPOUND should return "lowopt,count=0\0" and
   GET_COMPOUND_OPTS should return "lowopt\0count=0\0\0". Note that cmd-line options added as a consequence of
   environment variables are treated the same as mapped options. */

static IDATA getCompoundOptions(J9PortLibrary* portLibrary, J9VMInitArgs* j9vm_args, IDATA element, IDATA action,
    char** returnString, UDATA bufSize, char delim, char separator)
{
    ...
    if ((errorFound
            = optionValueOperations(PORTLIB, j9vm_args, element, GET_OPTION, returnString, bufferLeft, delim, 0, NULL))
        != OPTION_OK) {
        return errorFound;
    }
    ...
    if (element == 0)
        goto allOptionsFound; /* If we are the 0th element, don't bother to search for others */
    ...
    if (!(delimPos = strchr(optionSearchStr,
              delim))) /* If call is made to GET_COMPOUND on an option without the delimiter, we can't continue */
        return OPTION_ERROR;
    *(++delimPos) = '\0';
    ...
    currElement = element;
    while (currElement > 0) {
        currElement = findArgInVMArgs(PORTLIB, j9vm_args, (STARTSWITH_MATCH | (currElement << STOP_AT_INDEX_SHIFT)),
            optionSearchStr, NULL, FALSE);
        if (currElement < 0)
            break;

        /* If first element was mapped, find all other mapped values PLUS rightmost non-mapped value. Otherwise, find
         * all mapped values. */
        if (HAS_MAPPING(j9vm_args, currElement) || FROM_ENVVAR(j9vm_args, currElement) || firstElementIsMapped) {
            UDATA cursorLen, returnStrLen;

            if ((errorFound
                    = optionValueOperations(PORTLIB, j9vm_args, currElement, GET_OPTION, &cursor, 0, delim, 0, NULL))
                != OPTION_OK) { /* get correct values string */
                return errorFound;
            }
            ...
            returnStrLen = strlen(*returnString)
                + (bufferLeft < 0 ? bufferLeft : 0); /* If bufferLeft has gone negative, truncate end of string */
            memmove(*returnString + (cursorLen + 1), *returnString,
                returnStrLen); /* Shift right minus null-terminator. The buffer is zero'd so this is safe. */
    ...
    char** valuesBuffer, UDATA bufSize, char delim, char separator, void* reserved)
{
    ...
    while (!done) {
        ...
        case GET_OPTION:
            ...
            if (HAS_MAPPING(j9vm_args, element)) {
                if (!(MAPPING_FLAGS(j9vm_args, element)
                        & (EXACT_MAP_WITH_OPTIONS | EXACT_MAP_NO_OPTIONS | MAP_ONE_COLON_TO_TWO))) {
        ...
        case GET_OPTION_OPT:
            ...
            if ((errorFound
                    = optionValueOperations(PORTLIB, j9vm_args, element, GET_OPTION, &values, 0, delim, 0, NULL))
                != OPTION_OK) { /* get correct values string */
                return errorFound;
            }
        ...
        case GET_OPTIONS:
            /* Takes a buffer and copies in the values, each one separated by a single NULL. Should have at least \0\0
             * at the end, hence bufSize-1 */
            errorFound = optionValueOperations(PORTLIB, j9vm_args, element, GET_OPTION, valuesBuffer, bufSize - 1,
                delim, 0, NULL); /* get correct values string */
            if (*valuesBuffer) {
                cursor = *valuesBuffer;
                while (*cursor) {
                    if (*cursor == separator) {
                ...
                *(++cursor) = '\0'; /* explicity null-terminate the "array". There is no danger of walking beyond the
                                       buffer here because of bufSize-1 in call above. */
            }
        ...
        case GET_COMPOUND_OPTS:
            if ((errorFound
                    = getCompoundOptions(PORTLIB, j9vm_args, element, action, valuesBuffer, bufSize, delim, separator))
                != OPTION_OK) {
                return errorFound;
            }
            done = TRUE;
        ...
        case GET_MAPPED_OPTION:
            ...
            case MAP_WITH_INCLUSIVE_OPTIONS:
                ...
                if (bufSize > 0) {
                    ...
                    /* To get the value of the *actual* arg specified on the command-line, the delimiter may not be the
                     * one specified */
                    ...
                    return OPTION_ERROR; /* MAP_WITH_INCLUSIVE_OPTIONS cannot work without a buffer to build the result.
                                            Use COPY_OPTION_VALUE. */
                }
                break;
            }
            done = TRUE;
        ...
        case GET_PRC_VALUE:
            *((UDATA*)(reserved)) = 0;
            ...
            if (action == GET_INT_VALUE) {
                ...
                /* GET_PRC_VALUE should return value as a percentage (100 * value) and start with zero. Used for options
                   such as -Xmine. */
            ...
            if ((*cursor != '\0')) {
                ...
                if (HAS_MAPPING(j9vm_args, element)) {
                    /* If we have a mapping, then we ignore the option name passed in. Instead we want the one that
                     * corresponds to the actual cmd line */
                    cmdlineoption = &(j9vm_args->actualVMArgs->options[element]
                                          .optionString[strlen(MAPPING_MAPNAME(j9vm_args, element))]);
                }
                j9nls_printf(PORTLIB, J9NLS_WARNING, J9NLS_VMUTIL_MALFORMED_MEMORY_OPTION, cmdlineoption,
                    cursor - scanStart, scanStart, cursor);
            }
...
{
    char optionsArgumentBuffer[sizeof(VMOPT_XOPTIONSFILE_EQUALS) + MAX_PATH + sizeof(OPTIONS_DEFAULT)
        + 1]; /* add an extra byte to detect overflow */
    char* argString = NULL;
    ...
    resultLength = j9str_printf(PORTLIB, optionsArgumentBuffer, sizeof(optionsArgumentBuffer),
        VMOPT_XOPTIONSFILE_EQUALS "%s" DIR_SEPARATOR_STR OPTIONS_DEFAULT, optionsDirectory);
    ...
    char* j9binPath, char* jrebinPath)
{
    ...
    argumentLength = strlen(propertyNameEquals) + strlen(j9binPath) + sizeof(J9JAVA_PATH_SEPARATOR)
        + strlen(jrebinPath); /* the sizeof operation adds the space for the terminating null */
    ...
    j9str_printf(PORTLIB, optionsArgumentBuffer, argumentLength, "%s%s" J9JAVA_PATH_SEPARATOR "%s", propertyNameEquals,
        j9binPath, jrebinPath);
...
addJavaHome(J9PortLibrary* portLib, J9JavaVMArgInfoList* vmArgumentsList, UDATA altJavaHomeSpecified, char* jrelibPath)
{
    ...
    if (altJavaHomeSpecified) {
        ...
        memset(
            altJavaHomeBuffer, 0, sizeof(altJavaHomeBuffer)); /* Safety, since we're not checking the returned sizes */
        pathLength = WideCharToMultiByte(
            OS_ENCODING_CODE_PAGE, OS_ENCODING_WC_FLAGS, unicodeTemp, -1, altJavaHomeBuffer, MAX_PATH, NULL, NULL);
        if (0 == pathLength) {
            return -1;
        }
        ...
        j9str_printf(PORTLIB, optionsArgumentBuffer, argumentLength, JAVA_HOME_EQUALS "%s" J9JAVA_PATH_SEPARATOR "%s",
            altJavaHomeBuffer, jrelibPath);
    } else
#endif /* WIN32 */
    {
        if (NULL == javaHomeEnd) {
...
addJavaPropertiesOptions(J9PortLibrary* portLib, J9JavaVMArgInfoList* vmArgumentsList, UDATA verboseFlags)
{
    ...
    message = j9nls_lookup_message(
        J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE, J9NLS_J2SE_EXTRA_OPTIONS, "");
    ...
    for (cursor = msgBuffer; *cursor != '\0'; cursor++) {
        if (atBeginning) {
            ...
            if (cursor == msgBuffer) {
                optArg->cmdLineOpt.flags
                    |= ARG_MEMORY_ALLOCATION; /* mark the first argument as being the pointer to free */
            }
        }
        if (*cursor == '\n') {
 ...
 * continuation.
 */
static char* adjustNewline(char* continuationStart, U_32 continuationLength)
{
    ...
    memmove(continuationStart, continuationStart + continuationLength,
        whitespaceEnd - (continuationStart + continuationLength));
    ...
    J9ZipFunctionTable* zipFuncs, UDATA verboseFlags)
{
    ...
    if (NULL != jarPath) {
        ...
        if (ZIP_ERR_UNKNOWN_FILE_TYPE == status) {
            status
                = zipFuncs->zip_openZipFile(portLib, (char*)jarPath, &zipFile, NULL, J9ZIP_OPEN_ALLOW_NONSTANDARD_ZIP);
        ...
        if (0 == status) {
            ...
            U_8* dataBuffer
                = NULL; /* executable JARs tend to have huge manifests, so go straight to a heap-allocated buffer */
            status = zipFuncs->zip_getZipEntry(
                portLib, &zipFile, &entry, MANIFEST_FILE, sizeof(MANIFEST_FILE) - 1, zipEntryFlags);
            if (0 != status) {
                ...
                if (NULL == dataBuffer) {
                    ...
                    if (0 == status) {
                        ...
                        do {
                            ...
                            if (NULL != argStart) {
                                /* verify that the search string starts on a section.  Note that the main section
                                 * precedes this so backing up is safe. */
                                char nl0 = *(argStart - 1);
                                if (('\r' == nl0) || ('\n' == nl0)) { /* CR, LF or CRLF  */
                                    foundStart = TRUE;
                                } else {
                        ...
                        if (foundStart) {
                            ...
                            while (!foundEnd) {
                                 ...
                                 * parseOptionsFileText() will eliminate whitespace (a sequence of spaces and tab) at
                                 * the beginning of a line. Move the newline so that the whitespace in the original text
                                 * is preserved.
                                 */
                                if ((('\n' == nl0) || ('\r' == nl0)) && (' ' == nl1)) { /* CR or LF followed by space */
                                    argEnd = adjustNewline(argEnd, 2);
                                } else if (('\r' == nl0) && ('\n' == nl1)
                                    && (' ' == nl2)) { /* CRLF followed by space */
 ...
 */
static IDATA mapEnvVarToArgument(J9PortLibrary* portLibrary, const char* envVar, const char* j9opt,
    J9JavaVMArgInfoList* vmArgumentsList, UDATA mapType, UDATA verboseFlags)
{
    J9JavaVMArgInfo* optArg = NULL;
    IDATA valueSize = 0;
    ...
    JVMINIT_VERBOSE_INIT_TRACE2(
        verboseFlags, "Mapping environment variable %s to command-line option %s\n", envVar, j9opt);
...
static IDATA addEnvironmentVariableArguments(
    J9PortLibrary* portLib, const char* envVarName, J9JavaVMArgInfoList* vmArgumentsList, UDATA verboseFlags)
{
    PORT_ACCESS_FROM_PORT(portLib);
    ...
    J9PortLibrary* portLib, JavaVMInitArgs* launcherArgs, J9JavaVMArgInfoList* vmArgumentsList, UDATA verboseFlags)
{
    IDATA status = 0;
    if ((0
            != mapEnvVarToArgument(portLib, ENVVAR_IBM_MIXED_MODE_THRESHOLD, MAPOPT_XJIT_COUNT_EQUALS, vmArgumentsList,
                   EXACT_MAP_WITH_OPTIONS, verboseFlags))
        || (0
               != mapEnvVarToArgument(portLib, ENVVAR_JAVA_COMPILER, SYSPROP_DJAVA_COMPILER_EQUALS, vmArgumentsList,
                      EXACT_MAP_WITH_OPTIONS, verboseFlags))
        || (0
               != mapEnvVarToArgument(
                      portLib, ENVVAR_IBM_NOSIGHANDLER, VMOPT_XRS, vmArgumentsList, EXACT_MAP_NO_OPTIONS, verboseFlags))
#if defined(J9ZOS390)
        || (0
               != mapEnvVarToArgument(portLib, ENVVAR_JAVA_THREAD_MODEL, MAPOPT_XTHR_TW_EQUALS, vmArgumentsList,
                      EXACT_MAP_WITH_OPTIONS, verboseFlags))
        || (0
               != mapEnvVarToArgument(portLib, ENVVAR_IBM_JAVA_ENABLE_ASCII_FILETAG, VMOPT_XASCII_FILETAG,
                      vmArgumentsList, EXACT_MAP_NO_OPTIONS, verboseFlags))
#endif
        ...
        || (0
               != mapEnvVarToArgument(portLib, ENVVAR_IBM_JAVA_JITLIB, MAPOPT_XXJITDIRECTORY, vmArgumentsList,
                      EXACT_MAP_WITH_OPTIONS, verboseFlags))) {
        status = -1;
    }
    return status;
    ...
    *xServiceBuffer = NULL;
    ...
    for (optInd = 0; optInd < launcherArgs->nOptions; ++optInd) {
        ...
        if (TRUE == convertEncoding) {
            ...
            mutf8Size = j9str_convert(fromCode, J9STR_CODE_MUTF8, (U_8*)optString, optStringSize, NULL, 0);
            ...
            mutf8Size = j9str_convert(
                fromCode, J9STR_CODE_MUTF8, (U_8*)optString, optStringSize, transcodedString, mutf8Size);
            if (mutf8Size < 0) {
                switch (mutf8Size) {
                case J9PORT_ERROR_STRING_ICONV_OPEN_FAILED:
                case J9PORT_ERROR_STRING_MEM_ALLOCATE_FAILED:
        ...
        if (TRUE == convertEncoding) {
            ...
            strcpy(cursor, optString); /* safe because the lengths of all strings have been measured and included in
                                          argumentLength */
            optArg = newJavaVMArgInfo(
                vmArgumentsList, cursor, (consumableFlag | ((0 == optInd) ? ARG_MEMORY_ALLOCATION : 0)));
