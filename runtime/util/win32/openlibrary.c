...
#include "j9protos.h"
#include "ut_j9util.h"
static I_32 util_convertToUTF8(const wchar_t* unicodeString, char* utf8Buffer, UDATA size);
 ...
 */
static wchar_t* util_convertFromUTF8(const char* string, wchar_t* unicodeBuffer, UDATA unicodeBufferSize)
{
 ...
 */
static BOOLEAN checkLibraryType(wchar_t* unicodeLibPath)
{
    HANDLE libHandle = 0;
    USHORT magic = 0;
    ...
    libHandle
        = CreateFileW(unicodeLibPath, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
