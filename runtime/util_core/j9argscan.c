 ...
 * Answer 0 on success
 */
uintptr_t scan_hex(char** scan_start, uintptr_t* result) { return scan_hex_caseflag(scan_start, TRUE, result); }

/* Scan the next hexadecimal (only in lower cases if uppercaseAllowed is false) unsigned number off of the argument
 * string. Store the result in *result Answer 0 on success
 */
 ...
 */
uintptr_t scan_hex_u64(char** scan_start, uint64_t* result) { return scan_hex_caseflag_u64(scan_start, TRUE, result); }
