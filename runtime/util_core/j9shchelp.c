 ...
 */
uintptr_t getValuesFromShcFilePrefix(
    struct J9PortLibrary* portLibrary, const char* filename, struct J9PortShcVersion* result)
{
    if (filename != NULL) {
        uintptr_t temp;
 ...
 */
uintptr_t getGenerationFromName(const char* cacheNameWithVGen)
{
 ...
 */
uint32_t getShcModlevelForJCL(uintptr_t j2seVersion)
{
    uint32_t modLevel = 0;
    switch (j2seVersion) {
    case J2SE_18:
 ...
 */
uint32_t getJCLForShcModlevel(uintptr_t modlevel)
{
    ...
    case J9SH_MODLEVEL_JAVA10:
        /* From Java 10, modLevel equals to java version number. But there might be Java 10 shared cache that has
         * modLevel 6 created before this change.
         */
        javaVersion = 10;
        break;
    default:
 ...
 */
uintptr_t isCompatibleShcFilePrefix(
    J9PortLibrary* portlib, uint32_t javaVersion, uint32_t feature, const char* filename)
{
    ...
    if ((versionData.esVersionMajor == EsVersionMajor) && (versionData.esVersionMinor == EsVersionMinor)
        && (jclLevel == javaVersion) && (versionData.addrmode == J9SH_ADDRMODE) && (versionData.feature == feature)) {
        return 1;
    }
    return 0;
}
 ...
 */
void getStringForShcModlevel(J9PortLibrary* portlib, uint32_t modlevel, char* buffer, uint32_t buffSize)
{
 ...
 * @param [out] buffer  The buffer to copy the string into
 */
void getStringForShcAddrmode(J9PortLibrary* portlib, uint32_t addrmode, char* buffer)
{
    switch (addrmode) {
 ...
 * @param [in] optionalExtraID  Some control files have an extra ID - if this parameter is not NULL, this is also
 * checked
 *
 * @return 1 if the filename is a cache, or 0 otherwise
 */
uintptr_t isCacheFileName(
    J9PortLibrary* portlib, const char* nameToTest, uintptr_t expectCacheType, const char* optionalExtraID)
{
    J9PortShcVersion versionData;
    intptr_t nameToTestLen = 0;
    intptr_t expectedVersionLen
        = ((expectCacheType == J9PORT_SHR_CACHE_TYPE_PERSISTENT) || (J9PORT_SHR_CACHE_TYPE_SNAPSHOT == expectCacheType))
        ? J9SH_VERSION_STRING_LEN + 1
        : J9SH_VERSION_STRING_LEN;
