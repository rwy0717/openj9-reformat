    ...
    J9PortLibrary* portLibrary, const char* fileText, J9JavaVMArgInfoList* vmArgumentsList, uintptr_t verboseFlags)
{
    ...
    char *commentStart, *continueChar, *tokenStart, *lastSpace, *firstCharOfLine, *spaceStart, *quoteStart,
        *optionStart;
    intptr_t count = 0;
    ...
    size_t bufferLength = 0; /* Memory required to hold copies of arguments. This is an overestimate because of
        ...
        default:
            ...
            if (!commentStart) {
                if ((quoteStart == NULL) && (lastSpace == (cursor - 1)) && (cursor != optionStart)) {
                    spaceStart = NULL; /* We've just had space outside of quotes and current char is not '-'. This space
                                          must be part of an option. Ignore it. */
                }
                if (continueChar) { /* If continuing from previous line, remove all the chars between cursor and
                                       continueChar */
        ...
        if (isEOL || isEOF || isMultiple) {
            ...
            if (continueChar == NULL) {
                if ((commentStart == NULL) && (tokenStart != NULL) && (tokenStart[0] != '\0')) {
                    ...
                    if (quoteStart != NULL) {
                        j9nls_printf(
                            PORTLIB, J9NLS_ERROR, J9NLS_VMUTIL_MALFORMED_OPTIONSFILE_ERROR_STR, firstCharOfLine);
                        return RC_MALFORMED;
                    }
                     ...
                     * will be less than or equal to the size of fileText, which determines the size of
                     * parsedTextBuffer.
                     */
    ...
    uintptr_t verboseFlags, BOOLEAN parseOptionsFileFlag)
{
    ...
    while (*cursor) {
        ...
        if (tokenEnd) {
            if ((0 == strncmp(tokenStart, VMOPT_XOPTIONSFILE_EQUALS, strlen(VMOPT_XOPTIONSFILE_EQUALS)))
                && (TRUE == parseOptionsFileFlag)) {
                ...
                if ((0 == count)
                    && (tokenStart != argumentBuffer)) { /* the first argument string must point to the start of the
                                                            argumentBuffer so it can be used to free the buffer */
