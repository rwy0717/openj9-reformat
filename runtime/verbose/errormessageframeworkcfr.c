...
static void constructCfrMethodContextInfo(
    J9PortLibrary* portLib, MethodContextInfo* methodInfo, J9CfrError* error, U_8* className, UDATA classNameLength);
...
static BOOLEAN printReasonForInvalidStackMapAttribute(MessageBuffer* msgBuf, J9CfrError* error,
    MethodContextInfo* methodInfo, J9CfrAttributeCode* code, StackMapFrame* stackMapFrame);
static void printJ9BcvErrorMessages(MessageBuffer* msgBuf, J9CfrError* error);
static void printJ9CfrErrorMessages(
    MessageBuffer* msgBuf, J9CfrError* error, MethodContextInfo* methodInfo, J9CfrAttributeCode* code);
    ...
    J9PortLibrary* portLib, MethodContextInfo* methodInfo, J9CfrError* error, U_8* className, UDATA classNameLength)
{
     ...
     * constructCfrMethodContextInfo() is called in generateJ9CfrExceptionDetails(), where
     * error->errorMember->codeAttribute is confirmed to be a non-NULL value.
     */
    ...
    /* Register callback functions for retrieving name string in the constant pool and data in the exception handler
     * table */
    ...
    MethodContextInfo* methodInfo, J9CfrAttributeCode* code, StackMapFrame* stackMapFrame)
{
    ...
    if (-1 != error->errorFrameIndex) {
        ...
        if (errorFrameBCI < codeLength) {
            /* Obtain opcode string from the Oracle bytecode instruction set (sunJavaBCNames[] defined in bcnames.h)
             * with the opcode number. Note: errorFrameBCI (exceptionDetails->stackmapFrameBCI) is set to the offset in
             * code (the opcode number is stored in there) when calling checkStackMap (staticverify.c).
             */
            ...
            printMessage(
                msgBuf, "(Stack Map Frame offset overflow. bci=%u, bytecode length=%u)", errorFrameBCI, codeLength);
        }

        /* Walk through the stackmap for the specified stackmap frame and set alll data to stackMapFrame for 'Current
         * Frame'. Note: it only prints out the content existing in the error message uffer even if out-out-memory.
         */
...
static void printJ9BcvErrorMessages(MessageBuffer* msgBuf, J9CfrError* error)
{
    ...
    case J9NLS_BCV_ERR_INCONSISTENT_STACK__ID: {
        ...
        case BCV_ERR_JSR_ILLEGAL_LOAD_OPERATION:
            printMessage(
                msgBuf, "Loading return address from local variable index %u is illegal.", error->errorDataIndex);
            break;
        default:
            Assert_VRB_ShouldNeverHappen();
            break;
    ...
    MessageBuffer* msgBuf, J9CfrError* error, MethodContextInfo* methodInfo, J9CfrAttributeCode* code)
{
    ...
    case J9NLS_CFR_ERR_INVALID_STACK_MAP_ATTRIBUTE__ID: {
        ...
        BOOLEAN printStackFrame
            = printReasonForInvalidStackMapAttribute(msgBuf, error, methodInfo, code, &stackMapFrame);
        /* Current Frame: */
        if (printStackFrame) {
    ...
    U_8* initMsgBuffer, UDATA* msgBufferLength)
{
    ...
    if (NULL != code) {
        ...
        /* Location: */
        printMessage(&msgBuf, "\n%*sLocation:\n%*s%.*s.%.*s%.*s @%u: %s", INDENT(2), INDENT(4), classNameLength,
        ...
        /* Use errorCatalog to differentiate the NLS error code (BCV and CFR) to avoid the numbering conflict between
         * these error codes */
