...
static BOOLEAN pushLiveStackToVerificationTypeBuffer(
    StackMapFrame* stackMapFrame, J9BytecodeVerificationData* verifyData, MethodContextInfo* methodInfo);
static BOOLEAN isWideType2nd(
    J9UTF8Ref* dataType, StackMapFrame* stackMapFrame, MethodContextInfo* methodInfo, UDATA index, U_8 tag);
static void getNonClassTypeName(J9UTF8Ref* dataType, U_8 typeNameIndex, UDATA bcvType);
static U_8 getBCVDataType(
    J9BytecodeVerificationData* verifyData, MethodContextInfo* methodInfo, J9UTF8Ref* dataType, UDATA bcvType);

static BOOLEAN compareStackMapFrameFlag(StackMapFrame* CurrentFrame, StackMapFrame* stackMapFrame);
static BOOLEAN printReasonForIncompatibleType(MessageBuffer* buf, J9BytecodeVerificationData* verifyData,
    MethodContextInfo* methodInfo, StackMapFrame* currentFrame, StackMapFrame* targetFrame);
static BOOLEAN printExpectedTypeFromStackMapFrame(MessageBuffer* msgBuf, J9BytecodeVerificationData* verifyData,
    MethodContextInfo* methodInfo, StackMapFrame* currentFrame, StackMapFrame* targetFrame, U_32 localStackIndex);
static void printExpectedType(
    MessageBuffer* msgBuf, J9BytecodeVerificationData* verifyData, MethodContextInfo* methodInfo);
static void printWrongDataType(
    MessageBuffer* msgBuf, J9BytecodeVerificationData* verifyData, MethodContextInfo* methodInfo);
static BOOLEAN setStackMapFrameWithIndex(
    J9BytecodeVerificationData* verifyData, MethodContextInfo* methodInfo, StackMapFrame* targetFrame);
static BOOLEAN printReasonForFlagMismatch(MessageBuffer* msgBuf, J9BytecodeVerificationData* verifyData,
    MethodContextInfo* methodInfo, StackMapFrame* currentFrame, StackMapFrame* targetFrame);
...
/**
 * Retrieve name or type constant from verifyData->classNameList data (Refer to getNameAndLengthFromClassNameList at
 * vrfyhelp.c)
 * @param buf - buffer to hold the return value
 ...
 */
static void getJ9RtvStringfromClassNameList(J9UTF8Ref* buf, void* classNameListPtr, void* romClass, UDATA classIndex)
{
     ...
     * Note: offset[0] is set up in addClassName() to determine where to obtain the class name (classNameSegment or
     * romClass). So, the layout of class name in the class name list includes: offset[0] (0 if in classNameSegment or
     * class name offset in romClass), class name (J9UTF8) in classNameSegment (offset[0] == 0), and NULL.
     */
    if (0 == offset[0]) {
    ...
    StackMapFrame* stackMapFrame, J9BytecodeVerificationData* verifyData, MethodContextInfo* methodInfo)
{
     ...
     * In this case, we need to count all data types from data1 to data5 including placeholders ('top') for later
     * printing as the verification error may occur on data4 or data5.
     *
    ...
    /* Convert all data type on 'locals' (below liveStack->stackBaseIndex) so as to push them to the verification type
     * buffer for later printing */
    while (slot <= lastIndex) {
        wideType = convertBcvToCfrType(
            methodInfo, stackMapFrame, &currentVerificationTypeEntry, liveStack->stackElements[slot]);
    ...
    /* Convert all data type on 'stack' (starting from liveStack->stackBaseIndex) so as to push them to the verification
     * type buffer for later printing */
    slot = stackBaseIndex;
    while (slot < lastIndex) {
        wideType = convertBcvToCfrType(
            methodInfo, stackMapFrame, &currentVerificationTypeEntry, liveStack->stackElements[slot]);
...
static BOOLEAN compareStackMapFrameFlag(StackMapFrame* currentFrame, StackMapFrame* stackMapFrame)
{
     ...
     * 'flagThisUninit' is used in constructors to mark type states where initialization of this has not yet been
     * completed. If any local variable in Locals has the type 'uninitializedThis', then 'Flags' has the single element
     * 'flagThisUninit', otherwise 'Flags' is an empty list.
     *
    ...
    J9BytecodeVerificationData* verifyData, MethodContextInfo* methodInfo, J9UTF8Ref* dataType, UDATA bcvType)
{
    ...
    case BCV_OBJECT_OR_ARRAY: /* FALLTHROUGH */
    {
         ...
         * Note: according to the JVM Specification, the expected type for aastore is 'java/lang/Object' rather than
         * 'reference'.
         */
        if ((JBaastore != opCode) && (0 != bytecodeTypeInfo) && (BCV_GENERIC_OBJECT == convertedType)
 ...
 */
static BOOLEAN printReasonForIncompatibleType(MessageBuffer* msgBuf, J9BytecodeVerificationData* verifyData,
    MethodContextInfo* methodInfo, StackMapFrame* currentFrame, StackMapFrame* targetFrame)
{
    U_8 tag = 0;
    ...
    printMessage(msgBuf, " (current frame, %.*s[%d]) is not assignable to ", errorFramePositionNameLength,
        errorFramePositionName, localStackIndex);

    /* Case 1: if the frame index is specified in errorTargetFrameIndex, the expected data type will be fetched from
     * this stackmap frame */
    if (verifyData->errorTargetFrameIndex >= 0) {
        result = printExpectedTypeFromStackMapFrame(
            msgBuf, verifyData, methodInfo, currentFrame, targetFrame, localStackIndex);
    ...
    MethodContextInfo* methodInfo, StackMapFrame* currentFrame, StackMapFrame* targetFrame, U_32 localStackIndex)
{
    ...
    while (stackmapFrameIndex != errorTargetFrameIndex) {
        ...
        nextStackmapFrame
            = decodeStackmapFrameData(targetFrame, nextStackmapFrame, stackmapFrameIndex, methodInfo, verifyData);
        /* Return FALSE if out-of-memory during allocating verification buffer for data types in the specified stackmape
         * frame */
    ...
    printMessage(
        msgBuf, " (stack map, %.*s[%u])", errorFramePositionNameLength, errorFramePositionName, localStackIndex);
    ...
    J9BytecodeVerificationData* verifyData, MethodContextInfo* methodInfo, StackMapFrame* targetFrame)
{
    ...
    if (errorTargetFrameIndex >= 0) {
        ...
        while (stackmapFrameIndex != errorTargetFrameIndex) {
            ...
            nextStackmapFrame
                = decodeStackmapFrameData(targetFrame, nextStackmapFrame, stackmapFrameIndex, methodInfo, verifyData);
            /* Return FALSE if out-of-memory during allocating verification buffer for data types in the specified
             * stackmape frame */
            if (NULL == nextStackmapFrame) {
                goto exit;
            }
...
U_8* generateJ9RtvExceptionDetails(J9BytecodeVerificationData* verifyData, U_8* initMsgBuffer, UDATA* msgBufferLength)
{
    ...
    /* The fields of stackMapFrameCurrent will be set up in prepareVerificationTypeBuffer() for the 'Current Frame'
     * section. The fields of stackMapFrameTarget gets initialized here as prepareVerificationTypeBuffer doesn't get
     * invoked for stackMapFrameTarget in the majority of cases since there is no 'Stackmap Frame' section in their
    ...
    printMessage(&msgBuf, "%.*s.%.*s%.*s @%u: %s", methodInfo.className.length,
        methodInfo.className.bytes, /* class name */
    ...
    case BCV_ERR_INCOMPATIBLE_TYPE:
        printStackFrame = printReasonForIncompatibleType(
            &msgBuf, verifyData, &methodInfo, &stackMapFrameCurrent, &stackMapFrameTarget);
        break;
    ...
    case BCV_ERR_INIT_NOT_CALL_INIT:
        printStackFrame
            = printReasonForFlagMismatch(&msgBuf, verifyData, &methodInfo, &stackMapFrameCurrent, &stackMapFrameTarget);
    ...
    case BCV_ERR_WRONG_TOP_TYPE:
        printMessage(
            &msgBuf, "The pair of data types on the top of 'stack' must be long, double or two non-top singles.");
        break;
    ...
    case BCV_ERR_NO_STACKMAP_FRAME:
        printMessage(
            &msgBuf, "No stackmap frame available at this location. BCI on next stack: @%u", verifyData->errorTempData);
        break;
    ...
    case BCV_ERR_ARGUMENTS_MISMATCH:
        printMessage(&msgBuf,
            "The number of arguments (%u) in the method's signature mismatches the number (%u) specified in the class "
            "file.",
            verifyData->errorTempData, verifyData->romMethod->argCount);
        break;
