...
static IDATA obtainArgumentInfo(
    U_8* signatureBytes, UDATA signatureLength, UDATA* argumentStart, UDATA* argumentLength, U_8* argumentType);
...
U_8 convertToOracleOpcodeString(U_8 j9Opcode, U_8 returnType)
{
    U_8 oracleOpcode = 0;
 ...
 */
static VerificationTypeInfo* allocateMemoryToVerificationTypeBuffer(MethodContextInfo* methodInfo,
    StackMapFrame* stackMapFrame, VerificationTypeInfo* currentVerificationTypeEntry, UDATA slotCount)
{
    ...
    if ((numberOfSlots - slotIndex) <= slotCount) {
        ...
        newEntries = (VerificationTypeInfo*)j9mem_reallocate_memory(
            stackMapFrame->entries, (sizeof(VerificationTypeInfo) * numberOfSlots), OMRMEM_CATEGORY_VM);
        if (NULL == newEntries) {
...
VerificationTypeInfo* pushTopTypeToVerificationTypeBuffer(MethodContextInfo* methodInfo, StackMapFrame* stackMapFrame,
    VerificationTypeInfo* currentVerificationTypeEntry, UDATA slotCount)
{
    Assert_VRB_notNull(currentVerificationTypeEntry);

    /* Increase the buffer size of entries if full or the existing buffer size is less than the required slot count */
    currentVerificationTypeEntry
        = allocateMemoryToVerificationTypeBuffer(methodInfo, stackMapFrame, currentVerificationTypeEntry, slotCount);
    if (NULL == currentVerificationTypeEntry) {
        goto exit;
    }

    /* Fill up the required number of 'top' type to the verification type buffer and move forward to the next entry in
     * stackMapFrame->entries after that. Data of 'top' type: (typeTag, typeValueAttribute, typeValue) =
     * (CFR_STACKMAP_TYPE_TOP (0x00), 0, 0)
     */
...
VerificationTypeInfo* pushVerificationTypeInfo(MethodContextInfo* methodInfo, StackMapFrame* stackMapFrame,
    VerificationTypeInfo* currentVerificationTypeEntry, U_8 typeTag, U_8 typeValueAttribute, UDATA typeValue)
{
    Assert_VRB_notNull(currentVerificationTypeEntry);

    /* Increase the buffer size of entries if full */
    currentVerificationTypeEntry
        = allocateMemoryToVerificationTypeBuffer(methodInfo, stackMapFrame, currentVerificationTypeEntry, 1);
    if (NULL == currentVerificationTypeEntry) {
        goto exit;
    }

    /* Set up the information of data type in the entry and then move forward to the next entry in
    ...
    if ((CFR_STACKMAP_TYPE_LONG == typeTag) || (CFR_STACKMAP_TYPE_DOUBLE == typeTag)) {
        currentVerificationTypeEntry = pushVerificationTypeInfo(
            methodInfo, stackMapFrame, currentVerificationTypeEntry, CFR_STACKMAP_TYPE_TOP, 0, 0);
    }
...
VerificationTypeInfo* popVerificationTypeInfo(VerificationTypeInfo* currentVerificationTypeEntry)
{
    Assert_VRB_notNull(currentVerificationTypeEntry);
...
U_8* adjustLocalsAndStack(
    MethodContextInfo* methodInfo, StackMapFrame* stackMapFrame, U_16* topIndex, U_8* entries, IDATA numEntries)
{
    PORT_ACCESS_FROM_PORT(methodInfo->portLib);
    ...
    if (numEntries < 0) { /* POP */
        ...
        for (index = 0; index < numEntries; index++) {
            ...
            if (tag <= CFR_STACKMAP_TYPE_INIT_OBJECT) {
                ...
                currentVerificationTypeEntry
                    = pushVerificationTypeInfo(methodInfo, stackMapFrame, currentVerificationTypeEntry, tag, 0, 0);
            } else {
                /* Set up typeValue for new object, object reference and base array type */
                NEXT_U16(typeValue, entries);
                currentVerificationTypeEntry = pushVerificationTypeInfo(
                    methodInfo, stackMapFrame, currentVerificationTypeEntry, tag, INDEX_CONSTANTPOOL, typeValue);
            }
...
prepareVerificationTypeBuffer(StackMapFrame* stackMapFrame, MethodContextInfo* methodInfo)
{
    ...
    /* Allocate a verification type buffer (maximum size) for data type from locals/stacks on liveStack or stackmap
     * frame. Each entry of stackMapFrame structure stores the information of one data type coming from locals/stacks
     */
    maxSlot = methodInfo->maxLocals + methodInfo->maxStack;
    verificationTypeEntries = (VerificationTypeInfo*)j9mem_allocate_memory(
        (sizeof(VerificationTypeInfo) * maxSlot), J9MEM_CATEGORY_CLASSES);
    ...
    if (!J9_ARE_ALL_BITS_SET(methodInfo->modifier, CFR_ACC_STATIC)) {
        ...
        currentVerificationTypeEntry = pushVerificationTypeInfo(
            methodInfo, stackMapFrame, currentVerificationTypeEntry, vrfyType, INDEX_CLASSNAME, typeValue);
        if (NULL == currentVerificationTypeEntry) {
            result = FALSE;
            goto exit;
        }
    ...
    for (;;) {
         ...
         */
        argumentSlotCount = obtainArgumentInfo(
            (U_8*)methodInfo->signature.bytes, methodInfo->signature.length, &typeValue, &typeLength, &vrfyType);
        ...
        currentVerificationTypeEntry = pushVerificationTypeInfo(
            methodInfo, stackMapFrame, currentVerificationTypeEntry, vrfyType, INDEX_SIGNATURE, typeValue);
...
U_8* decodeStackmapFrameData(StackMapFrame* stackMapFrame, U_8* nextStackmapFrame, I_32 stackmapFrameIndex,
    MethodContextInfo* methodInfo, J9BytecodeVerificationData* verifyData)
{
    if (verifyData->createdStackMap) {
         ...
         */
        nextStackmapFrame = decodeConstuctedStackMapFrameData(
            stackMapFrame, nextStackmapFrame, stackmapFrameIndex, methodInfo, verifyData);
    } else {
...
/*
 * This function refers to checkStackMap (staticverify.c)/ClassFileOracle::walkMethodCodeAttributeAttributes
 * (ClassFileOracle.cpp) against VM specification to decode all data of the specified stackmap frame to stackMapFrame
 * for later printing out to the error message buffer.
 */
U_8* decodeStackFrameDataFromStackMapTable(
    StackMapFrame* stackMapFrame, U_8* nextStackmapFrame, MethodContextInfo* methodInfo)
{
    U_8 frameType = 0;
    ...
    /* Set up the verification type information for each entry on locals/stacks of the stackmap frame against the frame
     * type */
    stackMapFrame->numberOfStack = 0;
...
U_8* decodeConstuctedStackMapFrameData(StackMapFrame* stackMapFrame, U_8* nextStackmapFrame, I_32 stackmapFrameIndex,
    MethodContextInfo* methodInfo, J9BytecodeVerificationData* verifyData)
{
    ...
    /* Convert all data type on 'locals' (below stackBaseIndex) so as to push them to the verification type buffer for
     * later printing */
    while (slot <= lastIndex) {
        dataTypeCode = convertBcvToCfrType(
            methodInfo, stackMapFrame, &currentVerificationTypeEntry, targetStackmapFrame->stackElements[slot]);
    ...
    currentVerificationTypeEntry = pushTopTypeToVerificationTypeBuffer(
        methodInfo, stackMapFrame, currentVerificationTypeEntry, maxLocals - stackMapFrame->numberOfLocals);
    if (NULL == currentVerificationTypeEntry) {
        nextStackmapFrame = NULL;
        goto exit;
    }
    ...
    /* Convert all data type on 'stack' (starting from stackBaseIndex) so as to push them to the verification type
     * buffer for later printing */
    slot = stackBaseIndex;
    while (slot < lastIndex) {
        dataTypeCode = convertBcvToCfrType(
            methodInfo, stackMapFrame, &currentVerificationTypeEntry, targetStackmapFrame->stackElements[slot]);
    ...
    currentVerificationTypeEntry = pushTopTypeToVerificationTypeBuffer(
        methodInfo, stackMapFrame, currentVerificationTypeEntry, maxStack - stackMapFrame->numberOfStack);
    if (NULL == currentVerificationTypeEntry) {
        nextStackmapFrame = NULL;
    }
...
IDATA
convertBcvToCfrType(MethodContextInfo* methodInfo, StackMapFrame* stackMapFrame,
    VerificationTypeInfo** currentVerificationTypeEntry, UDATA bcvType)
{
    ...
    case BCV_TAG_BASE_TYPE_OR_TOP:
        ...
        *currentVerificationTypeEntry
            = pushVerificationTypeInfo(methodInfo, stackMapFrame, *currentVerificationTypeEntry, typeNameIndex, 0, 0);
        break;
    case BCV_TAG_BASE_ARRAY_OR_NULL:
        arity = BCV_ARITY_FROM_TYPE(bcvType);
        typeNameIndex = (U_8)bcvToBaseTypeNameIndex(bcvType);
        *currentVerificationTypeEntry = pushVerificationTypeInfo(
            methodInfo, stackMapFrame, *currentVerificationTypeEntry, typeNameIndex, 0, arity);
        break;
    case BCV_SPECIAL_NEW:
        *currentVerificationTypeEntry = pushVerificationTypeInfo(methodInfo, stackMapFrame,
            *currentVerificationTypeEntry, CFR_STACKMAP_TYPE_NEW_OBJECT, 0, BCV_INDEX_FROM_TYPE(bcvType));
        break;
    case BCV_SPECIAL_INIT:
        *currentVerificationTypeEntry = pushVerificationTypeInfo(
            methodInfo, stackMapFrame, *currentVerificationTypeEntry, CFR_STACKMAP_TYPE_INIT_OBJECT, 0, 0);
        break;
    case BCV_OBJECT_OR_ARRAY:
    default:
        *currentVerificationTypeEntry = pushVerificationTypeInfo(methodInfo, stackMapFrame,
            *currentVerificationTypeEntry, CFR_STACKMAP_TYPE_OBJECT, INDEX_CLASSNAMELIST, bcvType);
        break;
    }
...
/*
 * This function refers to checkStackMap (staticverify.c)/ClassFileOracle::walkMethodCodeAttributeAttributes
 * (ClassFileOracle.cpp) against VM specification to walk through the stackmap table so as to print out data type of all
 * stackmap frames to the error message buffer.
 */
void printSimpleStackMapTable(MessageBuffer* buf, MethodContextInfo* methodInfo)
{
    ...
    while ((entryCount < stackMapCount) && (remainingItemSlots > 0)) {
        ...
        if (frameType < CFR_STACKMAP_SAME_LOCALS_1_STACK) {
            ...
            if (CFR_STACKMAP_SAME_LOCALS_1_STACK_EXTENDED == frameType) {
                ...
                && (frameType < CFR_STACKMAP_CHOP_BASE)) { /* CFR_STACKMAP_CHOP_1/3 */
                /* VM specification says that the last k local variables are absent and that the operand stack is empty
                 * for CHOP frame. The value of k is given by the formula 251 - frame_type.
                 */
            ...
            } else if ((frameType > CFR_STACKMAP_APPEND_BASE)
                && (frameType < CFR_STACKMAP_FULL)) { /* CFR_STACKMAP_APPEND_1/3 */
                /* VM specification says that the k additional locals are defined and that the operand stack is empty
                 * for APPEND frame. The value of k is given by the formula frame_type - 251. So, print all information
                 * for these k local variables into the message buffer.
                 */
...
void releaseVerificationTypeBuffer(StackMapFrame* stackMapFrame, MethodContextInfo* methodInfo)
{
...
U_8 mapDataTypeToUTF8String(
    J9UTF8Ref* dataType, StackMapFrame* stackMapFrame, MethodContextInfo* methodInfo, UDATA index)
{
    ...
    case CFR_STACKMAP_TYPE_OBJECT: {
        ...
        case INDEX_SIGNATURE: {
            UDATA typeLength = 0;
            IDATA argumentSlotCount = obtainArgumentInfo(
                (U_8*)methodInfo->signature.bytes, methodInfo->signature.length, &typeValue, &typeLength, NULL);
            Assert_VRB_false(argumentSlotCount < 0);
        ...
        case INDEX_CLASSNAMELIST: /* Only used for runtime verification */
            methodInfo->getStringfromClassNameList(
                dataType, methodInfo->classNameList, methodInfo->romClass, typeValue);
            break;
        default:
            Assert_VRB_ShouldNeverHappen();
            break;
...
void printStackMapFrameBCI(MessageBuffer* buf, StackMapFrame* stackMapFrame)
{
    printMessage(buf, "\n%*sbci: @%d", INDENT(4), stackMapFrame->bci);
}

void printStackMapFrameFlag(MessageBuffer* buf, StackMapFrame* stackMapFrame)
{
    IDATA index = 0;
...
printTypeInfoToBuffer(MessageBuffer* buf, U_8 tag, J9UTF8Ref* dataType, BOOLEAN print2nd)
{
    ...
    case CFR_STACKMAP_TYPE_TOP:
        /* Only print the 2nd slot of long/double type to the error message buffer for the "Reason" section in the error
         * message framework. */
    ...
    case CFR_STACKMAP_TYPE_DOUBLE: {
        /* Print long/double (e.g. "long" or "double") or long/double plus the 2nd slot (e.g. "long, long_2nd" or
         * "double, double_2nd") to the error message buffer for other section in the error message framework if
         * required.
         */
        printMessage(buf, "%.*s", dataType->length, dataType->bytes);
    ...
    case CFR_STACKMAP_TYPE_OBJECT:
        ...
        printMessage(buf, "'%.*s%.*s%.*s%.*s'", dataType->arity, arrayPrefix, /* array prefix '[' */
...
void printStackMapFrameData(MessageBuffer* buf, StackMapFrame* stackMapFrame, MethodContextInfo* methodInfo,
    const char* title, UDATA titleLength, UDATA startFrom, UDATA numOfItems)
{
...
void printTheStackMapFrame(MessageBuffer* buf, StackMapFrame* stackMapFrame, MethodContextInfo* methodInfo)
{
...
void printExceptionTable(MessageBuffer* buf, MethodContextInfo* methodInfo)
{
...
U_8* printVerificationTypeInfo(MessageBuffer* buf, U_8* cursor, UDATA itemCount, I_32* remainingItemSlots)
{
    UDATA index = 0;
    U_8 itemType = 0;
...
U_8* printFullStackFrameInfo(MessageBuffer* buf, U_8* cursor, I_32* remainingItemSlots)
{
    U_16 itemCount = 0;
