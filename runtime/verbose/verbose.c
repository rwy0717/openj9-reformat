...
#define GET_SUPERCLASS(clazz) \
    ((J9CLASS_DEPTH(classDepthAndFlags) == 0) ? NULL : (clazz)->superclasses[J9CLASS_DEPTH(clazz) - 1])
...
#endif /* J9VM_OPT_DYNAMIC_LOAD_SUPPORT */
static void dumpQualifiedSize(
    J9PortLibrary* portLib, UDATA byteSize, const char* optionName, U_32 module_name, U_32 message_num);
static void verboseEmptyOSlotIterator(
...
static void releaseVerboseVerificationBuffer(
    J9PortLibrary* portLibrary, VerboseVerificationBuffer* buf, char byteArray[]);
static J9UTF8* toExternalQualifiedName(J9PortLibrary* portLibrary, VerboseVerificationBuffer* buf, J9UTF8* utf);
static void printStackMapFrame(J9PortLibrary* portLibrary, VerboseVerificationBuffer* buf,
    J9BytecodeVerificationData* verifyData, J9BranchTargetStack* stackMapFrame);
static void printVerificationInfo(
    J9PortLibrary* portLibrary, VerboseVerificationBuffer* buf, const char* msgFormat, ...);
static void flushVerificationBuffer(J9PortLibrary* portLibrary, VerboseVerificationBuffer* buf);
static UDATA printDataType(J9PortLibrary* portLibrary, VerboseVerificationBuffer* buf,
    J9BytecodeVerificationData* verifyData, UDATA encodedType, const char* fmt);
...
static void dumpMemorySizes(J9JavaVM* jvm)
{
    ...
    J9MemoryManagerVerboseInterface* mmFuncTable
        = (J9MemoryManagerVerboseInterface*)jvm->memoryManagerFunctions->getVerboseGCFunctionTable(jvm);
    ...
    if (J2SE_VERSION(jvm) && jvm->sharedClassPreinitConfig) { /* Only show for J2SE */
        ...
        dumpQualifiedSize(PORTLIB, updatedWithDefaults.sharedClassCacheSize,
            "-XX:SharedCacheHardLimit=", J9NLS_VERB_SIZES_XXSHARED_CACHE_HARD_LIMIT_EQUALS);
        ...
        dumpQualifiedSize(
            PORTLIB, updatedWithDefaults.sharedClassMinJITSize, "-Xscminjitdata", J9NLS_VERB_SIZES_XSCMINJITDATA);
        dumpQualifiedSize(
            PORTLIB, updatedWithDefaults.sharedClassMaxJITSize, "-Xscmaxjitdata", J9NLS_VERB_SIZES_XSCMAXJITDATA);
    }
#endif
    ...
    if (NULL != jitConfig) {
        ...
        if (TRUE == executableLargePageSupported) {
            ...
            optionDescription = j9nls_lookup_message(
                J9NLS_DO_NOT_APPEND_NEWLINE | J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_VERB_SIZES_XLP_CODECACHE, NULL);
            ...
            optionDescription = j9nls_lookup_message(J9NLS_DO_NOT_APPEND_NEWLINE | J9NLS_DO_NOT_PRINT_MESSAGE_TAG,
                J9NLS_VERB_SIZES_AVAILABLE_XLP_CODECACHE, NULL);
            ...
            for (pageIndex = 0; 0 != pageSizes[pageIndex]; pageIndex++) {
                ...
                j9vmem_find_valid_page_size(
                    J9PORT_VMEM_MEMORY_MODE_EXECUTE, &pageSizeEntry, &pageFlagsEntry, &isSupported);
                if (TRUE == isSupported) {
    ...
    J9PortLibrary* portLib, UDATA byteSize, const char* optionName, U_32 module_name, U_32 message_num)
{
    ...
    optionDescription = OMRPORT_FROM_J9PORT(PORTLIB)->nls_lookup_message(OMRPORT_FROM_J9PORT(PORTLIB),
        J9NLS_DO_NOT_APPEND_NEWLINE | J9NLS_DO_NOT_PRINT_MESSAGE_TAG, module_name, message_num, NULL);
    ...
    struct J9TranslationLocalBuffer* localBuffer)
{
    ...
    if (NULL != localBuffer->cpEntryUsed) {
        j9tty_printf(PORTLIB,
            "<Loaded %.*s from %.*s>\n<  Class size %i; ROM size %i; debug size %i>\n<  Read time %i usec; Load time "
            "%i usec; Translate time %i usec>\n",
            dynamicLoadStats->nameLength, dynamicLoadStats->name, localBuffer->cpEntryUsed->pathLength,
            localBuffer->cpEntryUsed->path, dynamicLoadStats->sunSize, dynamicLoadStats->romSize,
            dynamicLoadStats->debugSize, dynamicLoadStats->readEndTime - dynamicLoadStats->readStartTime,
        ...
        if (NULL != jrtURL) {
            j9tty_printf(PORTLIB,
                "<Loaded %.*s from %.*s>\n<  Class size %i; ROM size %i; debug size %i>\n<  Read time %i usec; Load "
                "time %i usec; Translate time %i usec>\n",
                J9UTF8_LENGTH(J9ROMCLASS_CLASSNAME(romClass)), J9UTF8_DATA(J9ROMCLASS_CLASSNAME(romClass)),
                J9UTF8_LENGTH(jrtURL), J9UTF8_DATA(jrtURL), dynamicLoadStats->sunSize, dynamicLoadStats->romSize,
                dynamicLoadStats->debugSize, dynamicLoadStats->readEndTime - dynamicLoadStats->readStartTime,
                dynamicLoadStats->loadEndTime - dynamicLoadStats->loadStartTime,
                dynamicLoadStats->translateEndTime - dynamicLoadStats->translateStartTime);
        } else {
            j9tty_printf(PORTLIB,
                "<Loaded %.*s>\n<  Class size %i; ROM size %i; debug size %i>\n<  Read time %i usec; Load time %i "
                "usec; Translate time %i usec>\n",
                J9UTF8_LENGTH(J9ROMCLASS_CLASSNAME(romClass)), J9UTF8_DATA(J9ROMCLASS_CLASSNAME(romClass)),
                dynamicLoadStats->sunSize, dynamicLoadStats->romSize, dynamicLoadStats->debugSize,
...
static void printClass(J9VMThread* vmThread, J9Class* clazz, char* message, UDATA bootLoaderOnly)
{
    ...
    romClass = clazz->romClass;
    if (J9ROMCLASS_IS_PRIMITIVE_OR_ARRAY(romClass) == 0) {
        if (!bootLoaderOnly || (clazz->classLoader == vmThread->javaVM->systemClassLoader)
            ...
            if (NULL == path) {
                ...
                j9tty_printf(
                    PORTLIB, template, message, (UDATA)J9UTF8_LENGTH(utf), J9UTF8_DATA(utf), pathLength, path, extra);
            }
        }
    }
}
...
static UDATA parseVerboseArgument(char* options, J9VerboseSettings* verboseOptions, char** errorString)
{
    UDATA result = TRUE;
    ...
    if (!options || !*options) {
        ...
        while (*options) {
            if (strcmp(options, OPT_NOCLASS) == 0) {
                /* clearing class verbosity is different from not enabling it. enable and disable cancel. disable alone
                 * invokes explicit action */
                verboseOptions->vclass = (VERBOSE_SETTINGS_SET == verboseOptions->vclass) ? VERBOSE_SETTINGS_IGNORE
                                                                                          : VERBOSE_SETTINGS_CLEAR;
                ...
                verboseOptions->gc
                    = (VERBOSE_SETTINGS_SET == verboseOptions->gc) ? VERBOSE_SETTINGS_IGNORE : VERBOSE_SETTINGS_CLEAR;
            } else
...
parseVerboseArgumentList(J9JavaVM* vm, J9VMDllLoadInfo* loadInfo, char** errorString)
{
    ...
    memset(&verboseOptions, 0, sizeof(J9VerboseSettings));
    do {
        verboseIndex = FIND_AND_CONSUME_ARG(
            SEARCH_FORWARD | OPTIONAL_LIST_MATCH | (verboseIndex << STOP_AT_INDEX_SHIFT), OPT_VERBOSE, NULL);
        /* start the search from where we left off last time */
        if (verboseIndex >= 0) {
            foundArg = 1;

            GET_OPTION_VALUES(verboseIndex, ':', ',', &valuesBufferPtr,
                VERBOSE_OPTION_BUF_SIZE); /* Returns values separated by single NULL chars */
...
#define VERBOSE_TEARDOWN_STAGE HEAP_STRUCTURES_FREED /* defined separately for consistency of dependencies */
    ...
    J9MemoryManagerVerboseInterface* mmFuncTable
        = (J9MemoryManagerVerboseInterface*)((vm->memoryManagerFunctions == NULL)
                ? NULL
                : vm->memoryManagerFunctions->getVerboseGCFunctionTable(vm));
            ...
            if (!initializeVerbosegclog(vm, verbosegclogIndex)) {
                loadInfo->fatalErrorStr
                    = (char*)j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE,
                        J9NLS_VERB_FAILED_TO_INITIALIZE, "Failed to initialize.");
                goto _error;
            }
        ...
        if (FIND_AND_CONSUME_ARG(EXACT_MATCH, OPT_XSNW, NULL) >= 0) {
            ...
            (*gcOmrHook)
                ->J9HookRegisterWithCallSite(
                    gcOmrHook, J9HOOK_MM_OMR_GLOBAL_GC_START, sniffAndWhackHookGC, OMR_GET_CALLSITE(), NULL);
            (*gcOmrHook)
                ->J9HookRegisterWithCallSite(
                    gcOmrHook, J9HOOK_MM_OMR_GLOBAL_GC_END, sniffAndWhackHookGC, OMR_GET_CALLSITE(), NULL);
            (*gcOmrHook)
                ->J9HookRegisterWithCallSite(
                    gcOmrHook, J9HOOK_MM_OMR_LOCAL_GC_START, sniffAndWhackHookGC, OMR_GET_CALLSITE(), NULL);
            (*gcOmrHook)
                ->J9HookRegisterWithCallSite(
                    gcOmrHook, J9HOOK_MM_OMR_LOCAL_GC_END, sniffAndWhackHookGC, OMR_GET_CALLSITE(), NULL);
...
static IDATA initializeVerbosegclogFromOptions(J9JavaVM* vm, char* vbgclogBuffer)
{
...
setVerboseState(J9JavaVM* vm, J9VerboseSettings* verboseOptions, char** errorString)
{
    ...
    J9MemoryManagerVerboseInterface* mmFuncTable
        = (J9MemoryManagerVerboseInterface*)vm->memoryManagerFunctions->getVerboseGCFunctionTable(vm);
    IDATA result = TRUE;
    ...
    if (VERBOSE_SETTINGS_CLEAR == verboseOptions->vclass) {
        ...
        if (!(vm->verboseLevel & VERBOSE_CLASS)) {
            ...
            (*vmHooks)->J9HookRegisterWithCallSite(
                vmHooks, J9HOOK_VM_CLASS_LOAD, verboseHookClassLoad, OMR_GET_CALLSITE(), NULL);

#ifdef J9VM_OPT_ZIP_SUPPORT
            vmZipCachePoolHooks = zip_getVMZipCachePoolHookInterface(vm->zipCachePool);
            (*vmZipCachePoolHooks)
                ->J9HookRegisterWithCallSite(
                    vmZipCachePoolHooks, J9HOOK_VM_ZIP_LOAD, zipCachePoolHookCallback, OMR_GET_CALLSITE(), NULL);
#endif

#ifdef J9VM_GC_DYNAMIC_CLASS_UNLOADING
            (*vmHooks)->J9HookRegisterWithCallSite(
                vmHooks, J9HOOK_VM_CLASS_UNLOAD, verboseHookClassUnload, OMR_GET_CALLSITE(), NULL);
#endif
        }
    }
    if (VERBOSE_SETTINGS_SET == verboseOptions->gcterse) {
        ...
        (*gcOmrHooks)
            ->J9HookRegisterWithCallSite(
                gcOmrHooks, J9HOOK_MM_OMR_LOCAL_GC_START, verboseHookGC, OMR_GET_CALLSITE(), vm->portLibrary);
        (*gcOmrHooks)
            ->J9HookRegisterWithCallSite(
                gcOmrHooks, J9HOOK_MM_OMR_LOCAL_GC_END, verboseHookGC, OMR_GET_CALLSITE(), vm->portLibrary);
        (*gcOmrHooks)
            ->J9HookRegisterWithCallSite(
                gcOmrHooks, J9HOOK_MM_OMR_GLOBAL_GC_START, verboseHookGC, OMR_GET_CALLSITE(), vm->portLibrary);
        (*gcOmrHooks)
            ->J9HookRegisterWithCallSite(
                gcOmrHooks, J9HOOK_MM_OMR_GLOBAL_GC_END, verboseHookGC, OMR_GET_CALLSITE(), vm->portLibrary);
    }
    ...
    if (VERBOSE_SETTINGS_SET == verboseOptions->verification) {
        vmHooks = vm->internalVMFunctions->getVMHookInterface(vm);
        (*vmHooks)->J9HookRegisterWithCallSite(
            vmHooks, J9HOOK_VM_CLASS_VERIFICATION_START, verboseClassVerificationStart, OMR_GET_CALLSITE(), NULL);
        (*vmHooks)->J9HookRegisterWithCallSite(
            vmHooks, J9HOOK_VM_CLASS_VERIFICATION_FALLBACK, verboseClassVerificationFallback, OMR_GET_CALLSITE(), NULL);
        (*vmHooks)->J9HookRegisterWithCallSite(
            vmHooks, J9HOOK_VM_CLASS_VERIFICATION_END, verboseClassVerificationEnd, OMR_GET_CALLSITE(), NULL);
        (*vmHooks)->J9HookRegisterWithCallSite(
            vmHooks, J9HOOK_VM_METHOD_VERIFICATION_START, verboseMethodVerificationStart, OMR_GET_CALLSITE(), NULL);
        (*vmHooks)->J9HookRegisterWithCallSite(
            vmHooks, J9HOOK_VM_STACKMAPFRAME_VERIFICATION, verboseStackMapFrameVerification, OMR_GET_CALLSITE(), NULL);
        ...
        (*vmHooks)->J9HookUnregister(
            vmHooks, J9HOOK_VM_CLASS_VERIFICATION_FALLBACK, verboseClassVerificationFallback, NULL);
        (*vmHooks)->J9HookUnregister(vmHooks, J9HOOK_VM_CLASS_VERIFICATION_END, verboseClassVerificationEnd, NULL);
        (*vmHooks)->J9HookUnregister(
            vmHooks, J9HOOK_VM_METHOD_VERIFICATION_START, verboseMethodVerificationStart, NULL);
        (*vmHooks)->J9HookUnregister(
            vmHooks, J9HOOK_VM_STACKMAPFRAME_VERIFICATION, verboseStackMapFrameVerification, NULL);
    }

    /* Jazz 82615: Register the callback funtions for the error message framework if the VerifyErrorDetails option is
     * specified */
...
static void verboseClassVerificationStart(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (NULL != qualified) {
        printVerificationInfo(PORTLIB, &buf, "Verifying class %.*s with %s format\n", (UDATA)J9UTF8_LENGTH(qualified),
            J9UTF8_DATA(qualified), newFormat ? "new" : "old");
...
static void verboseMethodVerificationStart(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    if (NULL != qualified) {
        printVerificationInfo(PORTLIB, &buf, "Verifying method %.*s.%.*s%.*s\n", (UDATA)J9UTF8_LENGTH(qualified),
            J9UTF8_DATA(qualified), (UDATA)J9UTF8_LENGTH(utfMethod), J9UTF8_DATA(utfMethod),
...
static const char* baseTypeNames[] = { "top", "integer", "float", "long", "double", "short", "byte", "char", "null",
    "I", "F", "J", "D", "S", "B", "C" };

/*
 * convert BCV_BASE_TYPE to a index to baseTypeNames table.
 */
static IDATA baseTypeToIndex(UDATA encodedType)
{
    IDATA index;
