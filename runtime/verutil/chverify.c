 ...
 */
static VMINLINE I_32 checkNameImpl(J9CfrConstantPoolInfo* info, BOOLEAN isClass, BOOLEAN isMethod)
{
    BOOLEAN slash = FALSE;
    I_32 arity = 0;
...
static VMINLINE I_32 isInitOrClinitImpl(J9CfrConstantPoolInfo* info)
{
    U_8* c = info->bytes;
 ...
 *
 * @returns 0 if name is a normal name, 1 if '<init>' and 2 if '<clinit>' , -1 if it starts with '<' but is not a valid
 * class name.
 * @note result is positive if the name is "<init>" or "<clinit>", result is negative if the name is illegal
 */
I_32 bcvIsInitOrClinit(J9CfrConstantPoolInfo* info) { return isInitOrClinitImpl(info); }

/**
 * Determine if this a valid name for Methods.
 *
 * @returns 1 if '<init>' and 2 if '<clinit>', otherwise 0 or positive if a valid name; negative value if class name is
 * invalid
 */
 ...
 */
I_32 bcvCheckName(J9CfrConstantPoolInfo* info) { return checkNameImpl(info, FALSE, FALSE); }
