...
/* The conversion table stores the CFR_STACKMAP_TYPE_XXX values (as the index of dataTypeNames[] in
 * errormessage_internal.c) so as to identify the data type string of argument in signature defined in dataTypeNames[].
 ...
 */
static const U_32 argumentTypeConversion[] = { 0, CFR_STACKMAP_TYPE_INT, CFR_STACKMAP_TYPE_INT,
    CFR_STACKMAP_TYPE_DOUBLE, 0, CFR_STACKMAP_TYPE_FLOAT, 0, 0, CFR_STACKMAP_TYPE_INT, CFR_STACKMAP_TYPE_LONG, 0,
    CFR_STACKMAP_TYPE_OBJECT, 0, 0, 0, 0, 0, 0, CFR_STACKMAP_TYPE_INT, 0, 0, 0, 0, 0, 0, CFR_STACKMAP_TYPE_INT, 0 };
 ...
 */
static VMINLINE IDATA verifyIdentifierUtf8Impl(U_8* identifierStart, U_8* limit, BOOLEAN allowSlashes)
{
...
static VMINLINE IDATA checkSignatureInlined(
    U_8* signatureBytes, UDATA signatureLength, UDATA* currentIndex, U_8* argumentType)
{
    IDATA argCount = 1;
    IDATA arity = 0;
    ...
    if (*cursor == 'L') {
        ...
        if (0 != arity) {
            ...
            /* Jazz 82615: Identify the data type of argument at the specified location in signature against the
             * conversion table */
            if (NULL != argumentType) {
...
verifyIdentifierUtf8(U_8* identifierStart, UDATA identifierLength)
{
    ...
    return (identifierLength > 0)
        && (charactersParsed == identifierLength); /* we should have consumed all the characters */
}
