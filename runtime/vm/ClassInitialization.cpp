...
#define STATE_NAME(state) \
    (J9_ARE_ANY_BITS_SET(state, ~(UDATA)J9ClassInitStatusMask) ? "IN_PROGRESS" : statusNames[state])
...
void initializeImpl(J9VMThread* currentThread, J9Class* clazz)
{
    Trc_VM_initializeImpl_Entry(currentThread, J9UTF8_LENGTH(J9ROMCLASS_CLASSNAME(clazz->romClass)),
        J9UTF8_DATA(J9ROMCLASS_CLASSNAME(clazz->romClass)), clazz);
 ...
 */
static void performVerification(J9VMThread* currentThread, J9Class* clazz)
{
    J9JavaVM* vm = currentThread->javaVM;
    J9ROMClass* romClass = clazz->romClass;
    Trc_VM_performVerification_Entry(currentThread, J9UTF8_LENGTH(J9ROMCLASS_CLASSNAME(romClass)),
        J9UTF8_DATA(J9ROMCLASS_CLASSNAME(romClass)), clazz);
    ...
    if (vm->runtimeFlags & J9_RUNTIME_VERIFY) {
        ...
        if (!J9ROMCLASS_IS_UNSAFE(romClass) && (0 == (romClass->optionalFlags & J9_ROMCLASS_OPTINFO_VERIFY_EXCLUDE))) {
            ...
            if ((J9ROMCLASS_HAS_MODIFIED_BYTECODES(romClass)
                ...
                Trc_VM_verification_Start(currentThread, J9UTF8_LENGTH(J9ROMCLASS_CLASSNAME(clazz->romClass)),
                    J9UTF8_DATA(J9ROMCLASS_CLASSNAME(clazz->romClass)), clazz->classLoader);
                ...
                if (NULL != verifyErrorStringUTF) {
                    PORT_ACCESS_FROM_JAVAVM(vm);
                    /* vmStruct is already up to date */
                    j9object_t verifyErrorStringObject = vm->memoryManagerFunctions->j9gc_createJavaLangString(
                        currentThread, verifyErrorStringUTF, strlen((char*)verifyErrorStringUTF), 0);
                    j9mem_free_memory(verifyErrorStringUTF);
                    setCurrentException(
                        currentThread, J9VMCONSTANTPOOL_JAVALANGVERIFYERROR, (UDATA*)verifyErrorStringObject);
                    goto done;
                }

                Trc_VM_verification_End(currentThread, J9UTF8_LENGTH(J9ROMCLASS_CLASSNAME(clazz->romClass)),
                    J9UTF8_DATA(J9ROMCLASS_CLASSNAME(clazz->romClass)), clazz->classLoader);
            } else {
...
static j9object_t setInitStatus(J9VMThread* currentThread, J9Class* clazz, UDATA status, j9object_t initializationLock)
{
    Trc_VM_setInitStatus_newStatus(currentThread, status, STATE_NAME(status));
    initializationLock = (j9object_t)VM_ObjectMonitor::enterObjectMonitor(currentThread, initializationLock);
...
static void classInitStateMachine(J9VMThread* currentThread, J9Class* clazz, J9ClassInitState desiredState)
{
    ...
    Trc_VM_classInitStateMachine_Entry(currentThread, J9UTF8_LENGTH(J9ROMCLASS_CLASSNAME(clazz->romClass)),
        J9UTF8_DATA(J9ROMCLASS_CLASSNAME(clazz->romClass)), clazz, clazz->classLoader, desiredStateNames[desiredState]);
    ...
    if (NULL == initializationLock) {
        ...
        while (true) {
            ...
            Trc_VM_classInitStateMachine_status(currentThread, unlockedStatus, STATE_NAME(unlockedStatus));
            ...
            doVerify:
                ...
                if (NULL != superclazz) {
                    ...
                    if (VM_VMHelpers::exceptionPending(currentThread)) {
                        initializationLock
                            = setInitStatus(currentThread, clazz, J9ClassInitUnverified, initializationLock);
                        clazz = VM_VMHelpers::currentClass(clazz);
                        goto done;
                    }
                }
            ...
            case J9ClassInitFailed: {
                /* J9ClassInitFailed can only be set when unlockedStatus is J9ClassInitNotInitialized, and desiredState
                 * is J9_CLASS_INIT_INITIALIZED. J9ClassInitFailed can be ignored if desiredState is not
                 * J9_CLASS_INIT_INITIALIZED, i.e., J9_CLASS_INIT_VERIFIED or J9_CLASS_INIT_PREPARED.
                 */
            ...
            case J9ClassInitNotInitialized: {
                ...
                if (!J9_ARE_ANY_BITS_SET(clazz->romClass->modifiers, J9AccInterface)) {
                    ...
                    Trc_VM_classInitStateMachine_initSuperInterfacesWithNonStaticNonAbstractMethods(
                        currentThread, clazz);
                    ...
                    while (iTable != superITable) {
                        ...
                        if (J9_ARE_ANY_BITS_SET(
                                interfaceClazz->romClass->extraModifiers, J9AccClassHasNonStaticNonAbstractMethods)) {
                            PUSH_OBJECT_IN_SPECIAL_FRAME(currentThread, initializationLock);
            ...
            default: { // IN PROGRESS (status contains a thread in the high bits)
                ...
                VM_ObjectMonitor::exitObjectMonitor(currentThread, initializationLock);
                if (VM_VMHelpers::exceptionPending(currentThread)
                    || VM_VMHelpers::immediateAsyncPending(currentThread)) {
