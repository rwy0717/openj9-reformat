...
public:
     ...
     */
    const VMINLINE ffi_type* getFFIType(J9Class* type)
    {
     ...
     */
    VMINLINE UDATA getCustomFFIType(ffi_type** typeFFI, j9object_t layoutStringObject)
    {
        ...
        *typeFFI = NULL;
        ...
        char* layout = copyStringToUTF8WithMemAlloc(_currentThread, layoutStringObject, J9_STR_NULL_TERMINATE_RESULT,
            "", 0, layoutBuffer, sizeof(layoutBuffer), NULL);

        /* Subsequent calls will modify the contents of this pointer, so preserve the original pointer for
         * j9mem_free_memory */
     ...
     * @param inPtr[in] Describes whether the current symbols are in a pointer. No ffi_type should be allocated for
     * symbols in a pointer.
     * @return A ffi_type* describing the array
     */
    VMINLINE ffi_type* getArrayFFIType(char** layout, UDATA nElements, bool inPtr)
    {
        PORT_ACCESS_FROM_JAVAVM(_vm);
        ...
        if (!inPtr && (elementType != NULL)) {
            ...
            typeFFI->elements
                = (ffi_type**)j9mem_allocate_memory(sizeof(ffi_type*) * (nElements + 1), OMRMEM_CATEGORY_VM);
     ...
     */
    VMINLINE ffi_type* getPrimativeFFITypeElement(char symb)
    {
        ffi_type* typeFFI = NULL;
    ...
    /**
     * @brief Convert the preceding integer of a layout string from string to UDATA, and put layout at the position
     * after the integer
     ...
     */
    VMINLINE UDATA getIntFromLayout(char** layout)
    {
     ...
     */
    ffi_type** getStructFFITypeElements(char** layout, bool inPtr);
     ...
     *		"[4i2[ii]*[ii]]" is a struct with 3 arrays: int[4], Point[2], and Point[], where Point is a struct with 2
     *ints
     * @param inPtr[in] Describes whether the current symbols are in a pointer. No ffi_type should be allocated for
     *symbols in a pointer.
     * @return The ffi_type of type FFI_TYPE_STRUCT
     */
    ffi_type* getStructFFIType(char** layout, bool inPtr);

    /**
     * @brief Frees the elements of a struct FFI type
     * @param elements[in] The elements of a struct FFI type
     */
    void freeStructFFITypeElements(ffi_type** elements);

    /**
     * @brief Free a custom FFI type
     * @param ffi[in] A pointer to a ffi type
     */
    void freeStructFFIType(ffi_type* ffi);
