...
static bool removePrefixes(J9JVMTIData* jvmtiData, U_8** methodNameData, UDATA* methodNameLength, const char* entryData,
    UDATA entryLength, UDATA retransformFlag)
{
    bool match = false;
    ...
    if (romMethod->modifiers & J9AccNative) {
        ...
        while (NULL != classDescriptor->className) {
            if (J9UTF8_DATA_EQUALS(
                    classNameData, classNameLength, classDescriptor->className, classDescriptor->classNameLength)) {
                ...
                while (NULL != methodDescriptor->methodName) {
                    if (J9UTF8_DATA_EQUALS(methodSignatureData, methodSignatureLength,
                            methodDescriptor->methodSignature, methodDescriptor->methodSignatureLength)) {
                        ...
                        found:
                            ...
                            Trc_VM_fastJNINativeFound(currentThread, jniNativeMethod, classNameLength, classNameData,
                                methodNameLength, methodNameData, methodSignatureLength, methodSignatureData, flags,
                                address);
#if defined(DEBUG)
                            {
                                PORT_ACCESS_FROM_VMC(currentThread);
                                j9tty_printf(PORTLIB, "Fast JNI found: %.*s.%.*s%.*s flags %p function %p\n",
                                    classNameLength, classNameData, methodNameLength, methodNameData,
                                    methodSignatureLength, methodSignatureData, flags, address);
                            }
#endif /* DEBUG */
                            ...
                            if (0 == (flags & J9_FAST_JNI_NO_EXCEPTION_THROW)) {
                                Assert_VM_true(
                                    0 == (flags & (J9_FAST_JNI_NOT_GC_POINT | J9_FAST_JNI_NO_SPECIAL_TEAR_DOWN)));
                            }
                        ...
                        if (NULL != jvmtiData) {
                            ...
                            if (removePrefixes(jvmtiData, &tempData, &tempLength, entryData, entryLength,
                                    J9JVMTIENV_FLAG_RETRANSFORM_CAPABLE)) {
                                goto found;
                            }
        ...
        if ((0 == flags) && (0 != (((UDATA)jniNativeMethod->constantPool) & J9_STARTPC_JNI_NATIVE))) {
...
#if defined(DEBUG)
            {
                ...
                j9tty_printf(PORTLIB, "Normal JNI found: %.*s.%.*s%.*s function %p\n", classNameLength, classNameData,
                    methodNameLength, methodNameData, methodSignatureLength, methodSignatureData, address);
            }
#endif /* DEBUG */
        }
    }
