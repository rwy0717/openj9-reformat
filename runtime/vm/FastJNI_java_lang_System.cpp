...
jlong JNICALL Fast_java_lang_System_currentTimeMillis(J9VMThread* currentThread)
{
...
jlong JNICALL Fast_java_lang_System_nanoTime(J9VMThread* currentThread)
{
    PORT_ACCESS_FROM_VMC(currentThread);
...
static VMINLINE VM_BytecodeAction doReferenceArrayCopy(J9VMThread* currentThread, j9object_t srcObject, I_32 srcStart,
    j9object_t destObject, I_32 destStart, I_32 elementCount, I_32* aiobIndex)
{
            ...
            *aiobIndex = value;
            ...
            if (0 != value) {
                *aiobIndex = value;
                ...
                value = VM_ArrayCopyHelpers::referenceArrayCopy(
                    currentThread, srcObject, srcStart, destObject, destStart, elementCount);
                if (-1 != value) {
...
static VMINLINE VM_BytecodeAction doPrimitiveArrayCopy(J9VMThread* currentThread, j9object_t srcObject, I_32 srcStart,
    j9object_t destObject, I_32 destStart, U_32 arrayShape, I_32 elementCount, I_32* aiobIndex)
{
    VM_BytecodeAction rc = EXECUTE_BYTECODE;
    /* Primitive array */
    if (elementCount < 0) {
                ...
                *aiobIndex = value;
                ...
                if (elementCount > 0) {
                    VM_ArrayCopyHelpers::primitiveArrayCopy(
                        currentThread, srcObject, srcStart, destObject, destStart, elementCount, arrayShape);
                }
            }
        }
    }
 ...
 */
void JNICALL Fast_java_lang_System_arraycopy(
    J9VMThread* currentThread, j9object_t src, jint srcPos, j9object_t dest, jint destPos, jint length)
{
            ...
            if (!J9ROMCLASS_IS_ARRAY(destRom)) {
                ...
                if (OBJECT_HEADER_SHAPE_POINTERS == J9CLASS_SHAPE(srcClass)) {
                    ...
                    if (OBJECT_HEADER_SHAPE_POINTERS != J9CLASS_SHAPE(destClass)) {
                        ...
                        VM_BytecodeAction rc
                            = doReferenceArrayCopy(currentThread, src, srcPos, dest, destPos, length, &aiobIndex);
                        switch (rc) {
                        case THROW_AIOB:
                            goto throwAIOB;
                    ...
                    if (srcClass != destClass) {
                        ...
                        VM_BytecodeAction rc = doPrimitiveArrayCopy(currentThread, src, srcPos, dest, destPos,
                            ((J9ROMArrayClass*)srcClass->romClass)->arrayShape & 0x0000FFFF, length, &aiobIndex);
                        if (rc == THROW_AIOB) {
                        throwAIOB:
