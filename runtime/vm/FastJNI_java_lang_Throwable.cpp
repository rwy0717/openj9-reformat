...
j9object_t JNICALL Fast_java_lang_Throwable_fillInStackTrace(J9VMThread* currentThread, j9object_t receiver)
{
...
#endif
                walkback = objectAllocate.inlineAllocateIndexableObject(
                    currentThread, arrayClass, (U_32)framesWalked, false);
                if (NULL == walkback) {
                    PUSH_OBJECT_IN_SPECIAL_FRAME(currentThread, receiver);
                    walkback = vm->memoryManagerFunctions->J9AllocateIndexableObject(
