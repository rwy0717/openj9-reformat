...
static void addLocationGeneratedClass(J9VMThread* vmThread, J9ClassLoader* classLoader,
    KeyHashTableClassEntry* existingPackageEntry, IDATA entryIndex, I_32 locationType);
...
static UDATA classHashGetName(KeyHashTableClassEntry* entry, const U_8** name, UDATA* nameLength)
{
    ...
    } else if ((TAG_UTF_QUERY == (tag & MASK_QUERY)) || (TAG_PACKAGE_UTF_QUERY == (tag & MASK_QUERY))) {
...
static UDATA classHashEqualFn(void* tableNode, void* queryNode, void* userData)
{
    ...
    if (TAG_PACKAGE_UTF_QUERY == ((KeyHashTableClassEntry*)queryNode)->tag) {
        if (queryNodeLength < tableNodeLength) {
            ...
            return J9UTF8_DATA_EQUALS(
                tableNodeName, (UDATA)className - (UDATA)tableNodeName, queryNodeName, queryNodeLength);
        }
        return FALSE;
    }
...
J9HashTable* hashClassTableNew(J9JavaVM* javaVM, U_32 initialSize)
{
    ...
    return hashTableNew(OMRPORT_FROM_J9PORT(javaVM->portLibrary), J9_GET_CALLSITE(), initialSize,
        sizeof(KeyHashTableClassEntry), sizeof(char*), flags, J9MEM_CATEGORY_CLASSES, classHashFn, classHashEqualFn,
        NULL, javaVM);
}
...
static KeyHashTableClassEntry* growClassHashTable(
    J9JavaVM* vm, J9ClassLoader* classLoader, KeyHashTableClassEntry* newEntry)
{
    ...
    if (J9_ARE_ALL_BITS_SET(vm->extendedRuntimeFlags, J9_EXTENDED_RUNTIME_FAST_CLASS_HASH_TABLE)) {
        ...
        J9HashTable* newTable = hashTableNew(oldTable->portLibrary, J9_GET_CALLSITE(), oldTable->tableSize + 1,
            sizeof(KeyHashTableClassEntry), sizeof(char*),
            J9HASH_TABLE_DO_NOT_GROW | J9HASH_TABLE_ALLOW_SIZE_OPTIMIZATION, J9MEM_CATEGORY_CLASSES, classHashFn,
            classHashEqualFn, NULL, vm);
        if (NULL != newTable) {
    ...
    J9VMThread* vmThread, J9ClassLoader* classLoader, J9Class* originalClass, J9Class* replacementClass)
{
    ...
    if ((NULL != result) && (result->ramClass == originalClass)) {
        ...
        /* Issue a GC write barrier when modifying the class hash table */
        vmThread->javaVM->memoryManagerFunctions->j9gc_objaccess_postStoreClassToClassLoader(
            vmThread, classLoader, replacementClass);
    }
}
...
J9HashTable* hashClassLocationTableNew(J9JavaVM* javaVM, U_32 initialSize)
{
    ...
    return hashTableNew(OMRPORT_FROM_J9PORT(javaVM->portLibrary), J9_GET_CALLSITE(), initialSize,
        sizeof(J9ClassLocation), sizeof(char*), flags, J9MEM_CATEGORY_CLASSES, classLocationHashFn,
        classLocationHashEqualFn, NULL, javaVM);
}
...
/**
 * @brief If a generated class is loaded from a package, then package information should not be available. If a
 * non-generated class is loaded from the same package at a later point, then package information should be available.
 * In order to make package information available, this function adds a class location for the generated class in the
 * classLocationHashTable, which points at the entryIndex of the non-generated class. Then, it also removes the
 * TAG_GENERATED_PACKAGE from the existing package entry of the generated class.
    ...
    KeyHashTableClassEntry* existingPackageEntry, IDATA entryIndex, I_32 locationType)
{
    ...
    J9ROMClass* resultROMClass
        = (J9ROMClass*)(existingPackageEntry->tag & ~(UDATA)(TAG_ROM_CLASS | TAG_GENERATED_PACKAGE));
    ...
    if (NULL != clazz) {
        ...
        if (NULL == classLocation) {
             ...
             * should make sure that the existing entry has locationType for non-generated class i.e. (locationType <
             * 0).
             */
