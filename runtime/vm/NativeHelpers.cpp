...
j9object_t getInterfacesHelper(J9VMThread* currentThread, j9object_t clazz)
{
    ...
    if (NULL == arrayClass) {
        ...
        arrayClass = internalCreateArrayClass(
            currentThread, (J9ROMArrayClass*)J9ROMIMAGEHEADER_FIRSTCLASS(romHeader), javaLangClass);
    ...
    array = vm->memoryManagerFunctions->J9AllocateIndexableObject(
        currentThread, arrayClass, interfaceCount, J9_GC_ALLOCATE_OBJECT_NON_INSTRUMENTABLE);
    ...
    for (UDATA i = 0; i < interfaceCount; i++, interfaces++) {
        ...
        J9Class* interfaceClass = internalFindClassUTF8(currentThread, J9UTF8_DATA(interfaceName),
            J9UTF8_LENGTH(interfaceName), j9clazz->classLoader, J9_FINDCLASS_FLAG_EXISTING_ONLY);
...
j9object_t convertCStringToByteArray(J9VMThread* currentThread, const char* cString)
{
    J9JavaVM* vm = currentThread->javaVM;
    U_32 size = (U_32)strlen(cString);
    j9object_t result = vm->memoryManagerFunctions->J9AllocateIndexableObject(
        currentThread, vm->byteArrayClass, size, J9_GC_ALLOCATE_OBJECT_NON_INSTRUMENTABLE);
    if (NULL != result) {
