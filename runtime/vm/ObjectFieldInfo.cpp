 ...
 */
void ObjectFieldInfo::countInstanceFields(void)
{
    ...
    if (isContendedClassLayout()) { /* all instance fields are contended. Reassign them and fix the total counts */
        ...
        _instanceFieldBackfillEligible
            = (_instanceSingleCount > 0) || (_objectCanUseBackfill && (_instanceSingleCount > 0));
    }
}

U_32 ObjectFieldInfo::countAndCopyHiddenFields(
    J9HiddenInstanceField* hiddenFieldList, J9HiddenInstanceField* hiddenFieldArray[])
{
...
U_32 ObjectFieldInfo::calculateTotalFieldsSizeAndBackfill()
{
    U_32 accumulator = 0;
    if (isContendedClassLayout()) {
        ...
        accumulator += (_contendedObjectCount * sizeof(J9Object)) + (_contendedSingleCount * sizeof(U_32))
            + (_contendedDoubleCount * sizeof(U_64)); /* add the contended fields */
        accumulator = ROUND_UP_TO_POWEROF2((UDATA)accumulator, (UDATA)_cacheLineSize)
            - sizeof(J9Object); /* Rounding takes care of the odd number of 4-byte fields. Remove the header */
    } else {
        accumulator = _superclassFieldsSize + (_totalObjectCount * sizeof(J9Object))
            + (_totalSingleCount * sizeof(U_32)) + (_totalDoubleCount * sizeof(U_64));
        /* if the superclass is not end aligned but we have doubleword fields, use the space before the first field as
         * the backfill */
        if (((getSuperclassObjectSize() % OBJECT_SIZE_INCREMENT_IN_BYTES) != 0) && /* superclass is not end-aligned */
            ((_totalDoubleCount > 0)
                || (!_objectCanUseBackfill && (_totalObjectCount > 0)))) { /* our fields start on a 8-byte boundary */
        ...
        if (((accumulator + sizeof(J9Object)) % OBJECT_SIZE_INCREMENT_IN_BYTES) != 0) {
            /* we have consumed the superclass's backfill (if any), so let our subclass use the residue at the end of
             * this class. */
