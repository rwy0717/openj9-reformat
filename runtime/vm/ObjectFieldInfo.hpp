...
private:
    ...
    /* the following count fields annotated with @Contended, including regular fields in classes annotated with
     * @Contended. */
    U_32 _contendedObjectCount;
    U_32 _contendedSingleCount;
    U_32 _contendedDoubleCount;
    bool _hiddenFieldOffsetResolutionRequired;
    bool _instanceFieldBackfillEligible; /* use this to give instance fields priority over the hidden fields for
                                            backfill slots */
    ...
    /**
     * Calculate a position guaranteed to be on a different cache line from the header, superclass fields, and hidden
     * fields.
     * @return offset from the start of the header
     */
    VMINLINE U_32 getPaddedTrueNonContendedSize(void) const
    {
        U_32 accumulator = _superclassFieldsSize + sizeof(J9Object); /* get the true size with header */
        accumulator += (_totalObjectCount * sizeof(J9Object)) + (_totalSingleCount * sizeof(U_32))
            + (_totalDoubleCount * sizeof(U_64)); /* add the non-contended and hidden fields */
        accumulator = ROUND_DOWN_TO_POWEROF2(accumulator, OBJECT_SIZE_INCREMENT_IN_BYTES)
            + _cacheLineSize; /* get the worst-case cache line boundary and add a cache size */
        return accumulator;
    }
...
public:
     ...
     */
    U_32 countAndCopyHiddenFields(J9HiddenInstanceField* hiddenFieldList, J9HiddenInstanceField* hiddenFieldArray[]);

    VMINLINE U_32 getTotalDoubleCount(void) const { return _totalDoubleCount; }

    VMINLINE U_32 getTotalObjectCount(void) const { return _totalObjectCount; }

    VMINLINE U_32 getTotalSingleCount(void) const { return _totalSingleCount; }
     ...
     */
    VMINLINE U_32 getNonBackfilledObjectCount(void) const
    {
    ...
    VMINLINE U_32 getNonBackfilledSingleCount(void) const
    {
    ...
    VMINLINE U_32 getNonBackfilledInstanceObjectCount(void) const
    {
    ...
    VMINLINE U_32 getNonBackfilledInstanceSingleCount(void) const
    {
    ...
    VMINLINE U_32 getInstanceDoubleCount(void) const { return _instanceDoubleCount; }

    VMINLINE U_32 getInstanceObjectCount(void) const { return _instanceObjectCount; }

    VMINLINE U_32 getInstanceSingleCount(void) const { return _instanceSingleCount; }

    VMINLINE U_32 getHiddenFieldCount(void) const { return _hiddenFieldCount; }

    VMINLINE bool isBackfillSuitableSingleAvailable(void) const { return (0 != getTotalSingleCount()); }

    VMINLINE bool isBackfillSuitableObjectAvailable(void) const
    {
        return (_objectCanUseBackfill && (0 != getTotalObjectCount()));
    }

    VMINLINE bool isBackfillSuitableInstanceSingleAvailable(void) const { return (0 != getInstanceSingleCount()); }

    VMINLINE bool isBackfillSuitableInstanceObjectAvailable(void) const
    {
        return (_objectCanUseBackfill && (0 != getInstanceObjectCount()));
    }

    VMINLINE bool isBackfillSuitableFieldAvailable(void) const
    {
     ...
     */
    VMINLINE IDATA getMyBackfillOffsetForHiddenField(void) const { return _myBackfillOffset + sizeof(J9Object); }

    VMINLINE U_32 getSuperclassFieldsSize(void) const { return _superclassFieldsSize; }

    VMINLINE U_32 getSuperclassObjectSize(void) const { return _superclassFieldsSize + sizeof(J9Object); }

    VMINLINE void setSuperclassFieldsSize(U_32 superTotalSize) { this->_superclassFieldsSize = superTotalSize; }

    VMINLINE bool const isMyBackfillSlotAvailable(void) const { return (_myBackfillOffset >= 0); }

    VMINLINE bool const isSuperclassBackfillSlotAvailable(void) const { return (_superclassBackfillOffset >= 0); }
     ...
     */
    U_32 calculateTotalFieldsSizeAndBackfill(void);
     ...
     */
    VMINLINE U_32 calculateFieldDataStart(void) const
    {
            ...
            if (((getSuperclassObjectSize() % OBJECT_SIZE_INCREMENT_IN_BYTES) != 0)
                ...
                ((_totalDoubleCount > 0)
                    || (!_objectCanUseBackfill
                           && (_totalObjectCount > 0)))) { /* our fields start on a 8-byte boundary */
     ...
     */
    VMINLINE UDATA addDoublesArea(UDATA start) const
    {
     ...
     */
    VMINLINE UDATA addObjectsArea(UDATA start) const
    {
        return start
            + (isContendedClassLayout() ? _contendedObjectCount : getNonBackfilledObjectCount()) * sizeof(fj9object_t);
    }

    VMINLINE bool isHiddenFieldOffsetResolutionRequired(void) const { return _hiddenFieldOffsetResolutionRequired; }
    ...
    VMINLINE IDATA getMyBackfillOffset(void) const { return _myBackfillOffset; }

    VMINLINE IDATA getSubclassBackfillOffset(void) const { return _subclassBackfillOffset; }

    VMINLINE void setSuperclassBackfillOffset(IDATA _superclassBackfillOffset)
    {
        this->_superclassBackfillOffset = _superclassBackfillOffset;
    }

    VMINLINE IDATA getSuperclassBackfillOffset(void) const { return _superclassBackfillOffset; }

    VMINLINE bool isInstanceFieldBackfillEligible(void) const { return _instanceFieldBackfillEligible; }

    VMINLINE bool isContendedClassLayout(void) const { return _useContendedClassLayout; }
};
