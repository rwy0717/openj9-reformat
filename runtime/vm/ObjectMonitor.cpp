...
/* SET_IGNORE_ENTER to avoid reporting enter/exit JLM events for internal artifacts which are not representative of the
 * hold time for the lock */
...
static bool spinOnFlatLock(J9VMThread* currentThread, j9objectmonitor_t volatile* lwEA, j9object_t object);

static bool spinOnTryEnter(
    J9VMThread* currentThread, J9ObjectMonitor* objectMonitor, j9objectmonitor_t volatile* lwEA, j9object_t object);

void clearLockWord(J9VMThread* currentThread, j9objectmonitor_t* lockWord)
{
 ...
 */
static bool saveBlockingEnterObject(J9VMThread* currentThread)
{
 ...
 */
static j9object_t restoreBlockingEnterObject(J9VMThread* currentThread, bool collapseFrame)
{
...
objectMonitorEnterBlocking(J9VMThread* currentThread)
{
    ...
    restart:
        ...
        for (;;) {
            ...
            j9objectmonitor_t volatile* lwEA = VM_ObjectMonitor::inlineGetLockAddress(
                currentThread, J9MONITORTABLE_OBJECT_LOAD(currentThread, &((J9ThreadMonitor*)monitor)->userData));
    ...
    if (NULL == lwEA) {
        ...
        if (J9_FLATLOCK_OWNER(incremented) == currentThread) {
            ...
            *lwEA = incremented;
            ...
            /* check to see if object is unlocked (JIT did not do initial inline sequence due to insufficient static
             * type info) */
...
static bool spinOnFlatLock(J9VMThread* currentThread, j9objectmonitor_t volatile* lwEA, j9object_t object)
{
    ...
    if (NULL != option) {
        ...
        Trc_VM_MonitorEnterNonBlocking_CustomSpinOption_Set1(
            option->className, object, spinCount1, spinCount2, yieldCount);
    }
    ...
    J9VMThread* currentThread, J9ObjectMonitor* objectMonitor, j9objectmonitor_t volatile* lwEA, j9object_t object)
{
    ...
    if (NULL != option) {
        ...
        Trc_VM_MonitorEnterNonBlocking_CustomSpinOption_Set2(
            option->className, object, tryEnterSpinCount1, tryEnterSpinCount2, tryEnterYieldCount);
    }
#else /* J9VM_INTERP_CUSTOM_SPIN_OPTIONS */
    ...
    /* we have the monitor object from the lock word so prime the cache with the monitor so we do not later look it up
