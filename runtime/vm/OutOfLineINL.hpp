...
public:
    static VMINLINE void returnVoid(J9VMThread* currentThread, UDATA slotCount)
    {
        currentThread->pc += 3;
        currentThread->sp += slotCount;
    }

    static VMINLINE void returnSingle(J9VMThread* currentThread, I_32 value, UDATA slotCount)
    {
        currentThread->pc += 3;
    ...
    static VMINLINE void returnDouble(J9VMThread* currentThread, I_64 value, UDATA slotCount)
    {
        currentThread->pc += 3;
    ...
    static VMINLINE void returnObject(J9VMThread* currentThread, j9object_t value, UDATA slotCount)
    {
        currentThread->pc += 3;
        currentThread->sp += (slotCount - 1);
    ...
    static VMINLINE void returnThis(J9VMThread* currentThread, UDATA slotCount)
    {
        currentThread->pc += 3;
        currentThread->sp += (slotCount - 1);
    }

    static VMINLINE UDATA* buildSpecialStackFrame(J9VMThread* currentThread, UDATA type, UDATA flags, bool visible)
    {
    ...
    static VMINLINE UDATA jitStackFrameFlags(J9VMThread* currentThread, UDATA constantFlags)
    {
    ...
    static VMINLINE void restoreSpecialStackFrameLeavingArgs(J9VMThread* currentThread, UDATA* bp)
    {
    ...
    static VMINLINE UDATA* buildInternalNativeStackFrame(J9VMThread* currentThread, J9Method* method)
    {
        UDATA* bp = buildSpecialStackFrame(
            currentThread, J9SF_FRAME_TYPE_NATIVE_METHOD, jitStackFrameFlags(currentThread, 0), true);
        *--currentThread->sp = (UDATA)method;
        currentThread->arg0EA = bp + J9_ROM_METHOD_FROM_RAM_METHOD(method)->argCount;
        return bp;
    }

    static VMINLINE void restoreInternalNativeStackFrame(J9VMThread* currentThread)
    {
