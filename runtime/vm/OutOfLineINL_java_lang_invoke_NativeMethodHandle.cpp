...
#ifdef J9VM_OPT_PANAMA
/* java.lang.invoke.NativeMethodHandle: private native void initJ9NativeCalloutDataRef(java.lang.String[]
 * argLayoutStrings); */
VM_BytecodeAction OutOfLineINL_java_lang_invoke_NativeMethodHandle_initJ9NativeCalloutDataRef(
    J9VMThread* currentThread, J9Method* method)
{
    ...
    J9Class* returnTypeClass = J9VM_J9CLASS_FROM_HEAPCLASS(
        currentThread, J9VMJAVALANGINVOKEMETHODTYPE_RETURNTYPE(currentThread, methodType));
    ...
    /**
     * args[0] stores the ffi_type of the return argument of the method. The remaining args array stores the ffi_type of
     * the input arguments. typeCount is the number of input arguments, plus one for the return argument.
     */
    ...
    /* Zero out the memory because if an error occurs before all the entries in this array are initialized then the
     * error handling code at freeAllMemoryThenExit will attempt to free all the pointers, some of which will not be
     * initialized.
     */
    ...
    /* In the common case we expect that layoutStringsExist will be NULL, which is why args[0] is written first then
     * layoutStringsExist is checked after */
    ...
    if (layoutStringsExist) {
        for (U_8 i = 0; i < typeCount - 1; i++) {
            ...
            if (NULL == layoutStringObject) {
                args[i + 1] = (ffi_type*)FFIHelpers.getFFIType(J9VM_J9CLASS_FROM_HEAPCLASS(
                    currentThread, J9JAVAARRAYOFOBJECT_LOAD(currentThread, argTypesObject, i)));
            } else {
                /**
                 * Pointers are converted to longs before the callout so the ffi type for pointers is sint64.
                 * Structs are also converted to longs before the callout, and are identified by the non-null layout
                 * string of the argument. The struct ffi type is created after the struct layout string is parsed in
                 * getCustomFFIType(ffi_type**, j9object_t).
                 */
        ...
        for (U_8 i = 0; i < typeCount - 1; i++) {
            args[i + 1] = (ffi_type*)FFIHelpers.getFFIType(
                J9VM_J9CLASS_FROM_HEAPCLASS(currentThread, J9JAVAARRAYOFOBJECT_LOAD(currentThread, argTypesObject, i)));
        }
    }
    ...
    J9VMJAVALANGINVOKENATIVEMETHODHANDLE_SET_J9NATIVECALLOUTDATAREF(
        currentThread, methodHandle, (void*)nativeCalloutData);
...
VM_BytecodeAction OutOfLineINL_java_lang_invoke_NativeMethodHandle_freeJ9NativeCalloutDataRef(
    J9VMThread* currentThread, J9Method* method)
{
    J9JavaVM* vm = currentThread->javaVM;
    ...
    J9NativeCalloutData* nativeCalloutData
        = J9VMJAVALANGINVOKENATIVEMETHODHANDLE_J9NATIVECALLOUTDATAREF(currentThread, methodHandle);

    /* Manually synchronize methodHandle since declaring the method as synchronized in Java will have no effect (for all
     * INLs). */
