...
genericStackDumpIterator(J9VMThread* currentThread, J9StackWalkState* walkState)
{
    ...
    if (NULL == method) {
        printFunction(
            cookie, "0x%p %.*s (unknown method)%s", pc, J9UTF8_LENGTH(className), J9UTF8_DATA(className), endOfLine);
    } else {
        ...
        if (NULL == walkState->jitInfo) {
            if (J9_ARE_ANY_BITS_SET(romMethod->modifiers, J9AccNative)) {
                printFunction(cookie, " NATIVE   %.*s.%.*s%.*s%s", J9UTF8_LENGTH(className), J9UTF8_DATA(className),
                    J9UTF8_LENGTH(name), J9UTF8_DATA(name), J9UTF8_LENGTH(sig), J9UTF8_DATA(sig), endOfLine);
            } else {
                printFunction(cookie, " %08x %.*s.%.*s%.*s%s", pc - method->bytecodes, J9UTF8_LENGTH(className),
                    J9UTF8_DATA(className), J9UTF8_LENGTH(name), J9UTF8_DATA(name), J9UTF8_LENGTH(sig),
                    J9UTF8_DATA(sig), endOfLine);
            }
        } else {
            if (0 == walkState->inlineDepth) {
                printFunction(cookie, " %08x %.*s.%.*s%.*s  (@%p)%s", pc - (U_8*)method->extra,
                    J9UTF8_LENGTH(className), J9UTF8_DATA(className), J9UTF8_LENGTH(name), J9UTF8_DATA(name),
                    J9UTF8_LENGTH(sig), J9UTF8_DATA(sig), pc, endOfLine);
            } else {
                printFunction(cookie, " INLINED  %.*s.%.*s%.*s  (@%p)%s", J9UTF8_LENGTH(className),
                    J9UTF8_DATA(className), J9UTF8_LENGTH(name), J9UTF8_DATA(name), J9UTF8_LENGTH(sig),
                    J9UTF8_DATA(sig), pc, endOfLine);
            }
        }
    }
...
void dumpStackTrace(J9VMThread* currentThread)
{
