...
void internalAcquireVMAccess(J9VMThread* currentThread)
{
    VM_VMAccess::inlineAcquireVMAccessExhaustiveCheck(currentThread);
}

void internalReleaseVMAccess(J9VMThread* currentThread) { VM_VMAccess::inlineReleaseVMAccess(currentThread); }
    ...
    if (++(vmThread->omrVMThread->exclusiveCount) == 1) {
        ...
        if (J9_XACCESS_NONE != vm->exclusiveAccessState) {
            ...
            if (J9_LINEAR_LINKED_LIST_IS_EMPTY(vm->exclusiveVMAccessQueueHead)) {
                ...
                J9_LINEAR_LINKED_LIST_ADD(exclusiveVMAccessQueueNext, exclusiveVMAccessQueuePrevious,
                    vm->exclusiveVMAccessQueueHead, vmThread);
            } else {
                Trc_VM_acquireExclusiveVMAccess_QueueNonEmpty(vmThread, vm->exclusiveVMAccessQueueHead);
                J9_LINEAR_LINKED_LIST_ADD_AFTER(exclusiveVMAccessQueueNext, exclusiveVMAccessQueuePrevious,
                    vm->exclusiveVMAccessQueueHead, vm->exclusiveVMAccessQueueTail, vmThread);
            }
            ...
            Assert_VM_true((J9_XACCESS_HANDING_OFF == vm->exclusiveAccessState)
                || (J9_XACCESS_HANDING_OFF_FROM_EXTERNAL_THREAD == vm->exclusiveAccessState));
            ...
            while ((currentThread = currentThread->linkNext) != vmThread) {
                ...
                VM_VMAccess::setPublicFlags(currentThread,
                    J9_PUBLIC_FLAGS_HALT_THREAD_EXCLUSIVE | J9_PUBLIC_FLAGS_NOT_COUNTED_BY_EXCLUSIVE, true);
        ...
        Assert_VM_true(
            (J9_XACCESS_PENDING == vm->exclusiveAccessState) || (J9_XACCESS_HANDED_OFF == vm->exclusiveAccessState));
...
void internalAcquireVMAccessNoMutexWithMask(J9VMThread* vmThread, UDATA haltMask)
{
    ...
    if (J9_ARE_ALL_BITS_SET(vmThread->publicFlags, J9_PUBLIC_FLAGS_JNI_CRITICAL_ACCESS)) {
        ...
        if (J9_ARE_ANY_BITS_SET(vmThread->publicFlags, J9_PUBLIC_FLAGS_HALT_THREAD_EXCLUSIVE)) {
            ...
            if (vm->jniCriticalResponseCount == 0) {
                U_64 timeTaken = j9time_hires_delta(
                    vm->omrVM->exclusiveVMAccessStats.startTime, timeNow, J9PORT_TIME_DELTA_IN_MILLISECONDS);
                ...
                if (timeTaken > slowTolerance) {
                    TRIGGER_J9HOOK_VM_SLOW_EXCLUSIVE(
                        vm->hookInterface, vmThread, (UDATA)timeTaken, J9_EXCLUSIVE_SLOW_REASON_JNICRITICAL);
                }
    ...
    if (reacquireJNICriticalAccess) {
        VM_VMAccess::setPublicFlags(
            vmThread, J9_PUBLIC_FLAGS_VMACCESS_ACQUIRE_BITS | J9_PUBLIC_FLAGS_JNI_CRITICAL_ACCESS);
    } else {
...
static void internalReleaseVMAccessNoMutexNoCheck(J9VMThread* vmThread)
{
    ...
    if (J9_ARE_ANY_BITS_SET(vmThread->publicFlags, J9_PUBLIC_FLAGS_EXCLUSIVE_RESPONSE_MASK)) {
        ...
        if (J9_PUBLIC_FLAGS_HALT_THREAD_EXCLUSIVE
            == (vmThread->publicFlags
                   & (J9_PUBLIC_FLAGS_HALT_THREAD_EXCLUSIVE | J9_PUBLIC_FLAGS_NOT_COUNTED_BY_EXCLUSIVE))) {
            U_64 timeNow = updateExclusiveVMAccessStats(vmThread);

            --vm->exclusiveAccessResponseCount;
            if (vm->exclusiveAccessResponseCount == 0) {
                VM_VMAccess::respondToExclusiveRequest(
                    vmThread, vm, PORTLIB, timeNow, J9_EXCLUSIVE_SLOW_REASON_EXCLUSIVE);
            }
        }
    ...
    if (--(vmThread->omrVMThread->exclusiveCount) == 0) {
        ...
        if (!J9_LINEAR_LINKED_LIST_IS_EMPTY(vm->exclusiveVMAccessQueueHead)) {
            ...
            J9_LINEAR_LINKED_LIST_REMOVE(
                exclusiveVMAccessQueueNext, exclusiveVMAccessQueuePrevious, vm->exclusiveVMAccessQueueHead, nextThread);
            ...
            VM_VMAccess::clearPublicFlags(
                nextThread, J9_PUBLIC_FLAGS_HALT_THREAD_EXCLUSIVE | J9_PUBLIC_FLAGS_NOT_COUNTED_BY_EXCLUSIVE);
            omrthread_monitor_exit(vm->exclusiveAccessMutex);
            ...
            if (J9_ARE_ALL_BITS_SET(
                    vmThread->javaVM->extendedRuntimeFlags, J9_EXTENDED_RUNTIME_FAST_CLASS_HASH_TABLE)) {
            ...
            while ((currentThread = currentThread->linkNext) != vmThread) {
                ...
                currentThread->lastDecompilation = NULL;
                VM_VMAccess::clearPublicFlags(
                    currentThread, J9_PUBLIC_FLAGS_HALT_THREAD_EXCLUSIVE | J9_PUBLIC_FLAGS_NOT_COUNTED_BY_EXCLUSIVE);
            }
            omrthread_monitor_notify_all(vm->exclusiveAccessMutex);
            omrthread_monitor_exit(vm->exclusiveAccessMutex);
            omrthread_monitor_exit(vmThread->publicFlagsMutex);
 ...
 */
static BOOLEAN synchronizeRequestsFromExternalThread(J9JavaVM* vm, BOOLEAN block)
{
    omrthread_monitor_enter(vm->exclusiveAccessMutex);
...
static void waitForResponseFromExternalThread(J9JavaVM* vm, UDATA vmResponsesExpected, UDATA jniResponsesExpected)
{
    PORT_ACCESS_FROM_JAVAVM(vm);
...
void acquireExclusiveVMAccessFromExternalThread(J9JavaVM* vm)
{
    ...
    do {
        omrthread_monitor_enter(currentThread->publicFlagsMutex);
        VM_VMAccess::setPublicFlags(
            currentThread, J9_PUBLIC_FLAGS_HALT_THREAD_EXCLUSIVE | J9_PUBLIC_FLAGS_NOT_COUNTED_BY_EXCLUSIVE, true);
        /* Because the previous line writes atomically to the same field read below, there is likely
         * no barrier required here, but do a full fence to be sure.  The barrier may also be required
         * before reading inNative below.
         */
...
/**
 ...
 returns immediately with the priority at which the gc collector threads should run.  The main collector thread running
 at the required priority completes the acquisition of vm access for gc by waiting for the responses from the mutator
 threads,
*/
UDATA
    ...
    *gcPriority = J9THREAD_PRIORITY_MAX;
    ...
    do {
        if (!(thread->privateFlags & J9_PRIVATE_FLAGS_GC_MASTER_THREAD)) {
            ...
            VM_VMAccess::setPublicFlags(
                thread, J9_PUBLIC_FLAGS_HALT_THREAD_EXCLUSIVE | J9_PUBLIC_FLAGS_NOT_COUNTED_BY_EXCLUSIVE, true);
            /* Because the previous line writes atomically to the same field read below, there is likely
             * no barrier required here, but do a full fence to be sure.  The barrier may also be required
             * before reading inNative below.
             */
...
void internalEnterVMFromJNI(J9VMThread* currentThread)
{
    ...
    if (J9_UNEXPECTED(currentThread->publicFlags != J9_PUBLIC_FLAGS_VM_ACCESS)) {
         ...
         */
        if (J9_ARE_ANY_BITS_SET(currentThread->publicFlags,
                J9_PUBLIC_FLAGS_RELEASE_ACCESS_REQUIRED_MASK | J9_PUBLIC_FLAGS_VMACCESS_OUTOFLINE_MASK)) {
...
void internalExitVMToJNI(J9VMThread* currentThread)
{
...
void acquireSafePointVMAccess(J9VMThread* vmThread)
{
    ...
    if (++(vmThread->safePointCount) == 1) {
        ...
        do {
            omrthread_monitor_enter(currentThread->publicFlagsMutex);
            VM_VMAccess::setPublicFlags(
                currentThread, J9_PUBLIC_FLAGS_REQUEST_SAFE_POINT | J9_PUBLIC_FLAGS_NOT_COUNTED_BY_SAFE_POINT, true);
        ...
        do {
            ...
            if (currentThread->inNative) {
                ...
                VM_VMAccess::clearPublicFlags(currentThread, J9_PUBLIC_FLAGS_REQUEST_SAFE_POINT, true);
                           ...
                           currentThread->publicFlags, J9_PUBLIC_FLAGS_NOT_AT_SAFE_POINT | J9_PUBLIC_FLAGS_VM_ACCESS)) {
#if defined(J9VM_INTERP_TWO_PASS_EXCLUSIVE)
    ...
    retry:
        ...
        // Not necessary?
        VM_VMAccess::clearPublicFlags(
            vmThread, J9_PUBLIC_FLAGS_HALTED_AT_SAFE_POINT | J9_PUBLIC_FLAGS_NOT_COUNTED_BY_SAFE_POINT);
    }
    Assert_VM_mustHaveVMAccess(vmThread);
...
void releaseSafePointVMAccess(J9VMThread* vmThread)
{
    ...
    if (--(vmThread->safePointCount) == 0) {
        ...
        do {
            VM_VMAccess::clearPublicFlags(
                currentThread, J9_PUBLIC_FLAGS_HALTED_AT_SAFE_POINT | J9_PUBLIC_FLAGS_NOT_COUNTED_BY_SAFE_POINT, true);
        } while ((currentThread = currentThread->linkNext) != vmThread);
        omrthread_monitor_enter(vm->exclusiveAccessMutex);
    ...
    if (currentThread != vmThread) {
        ...
        if (vmThread->publicFlags & (J9_PUBLIC_FLAGS_VM_ACCESS | J9_PUBLIC_FLAGS_QUEUED_FOR_EXCLUSIVE)) {
            ...
            if (VM_VMAccess::mustWaitForVMAccessRelease(vmThread)) {
                while (J9_ARE_ANY_BITS_SET(
                    vmThread->publicFlags, J9_PUBLIC_FLAGS_VM_ACCESS | J9_PUBLIC_FLAGS_QUEUED_FOR_EXCLUSIVE)) {
                    omrthread_monitor_wait(vmThread->publicFlagsMutex);
                }
            }
            omrthread_monitor_exit(vmThread->publicFlagsMutex);
            ...
            internalAcquireVMAccessNoMutexWithMask(
                currentThread, J9_PUBLIC_FLAGS_HALT_THREAD_ANY - J9_PUBLIC_FLAGS_HALT_THREAD_INSPECTION);
            omrthread_monitor_exit(currentThread->publicFlagsMutex);

            /* If currentThread is being halted, cancel vmThread's pending inspection request */
            if (J9_PUBLIC_FLAGS_HALT_THREAD_INSPECTION
                == (currentThread->publicFlags & J9_PUBLIC_FLAGS_HALT_THREAD_INSPECTION)) {
...
/* Note that VM access is released and reacquired by this call - direct object pointers must not be held across this
 * call */
...
void resumeThreadForInspection(J9VMThread* currentThread, J9VMThread* vmThread)
{
    ...
    if (currentThread != vmThread) {
        ...
        /* (It is safe to read the publicFlags without a mutex since we're only really interested if it was set before
         * we acquired VM access) */
