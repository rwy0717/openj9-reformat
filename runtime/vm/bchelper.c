...
static j9object_t allocate_dimension(J9VMThread* vmStruct, J9ArrayClass* currentClass, U_32 dimensions,
    U_32 currentDimension, U_32* dimensionArray, UDATA allocationType);

j9object_t helperMultiANewArray(
    J9VMThread* vmStruct, J9ArrayClass* classPtr, UDATA dimensionsUDATA, I_32* dimensionArray, UDATA allocationType)
{
    ...
    for (i = 0; i < dimensions; i++) {
        if (dimensionArray[i] < 0) {
            setCurrentException(vmStruct, J9VMCONSTANTPOOL_JAVALANGNEGATIVEARRAYSIZEEXCEPTION,
                NULL); /* Negative array size exception */
            return NULL;
        }
    }
    ...
    if (arrayOfObject == NULL) {
        ...
        arrayOfObject = internalCreateArrayClass(
            vmStruct, (J9ROMArrayClass*)J9ROMIMAGEHEADER_FIRSTCLASS(vm->arrayROMClasses), elementClass);
        if (arrayOfObject == NULL) {
    ...
    saveTable = mmFuncs->J9AllocateIndexableObject(vmStruct, arrayOfObject, dimensions, allocationType);
    if (saveTable == NULL) {
    ...
    result = allocate_dimension(vmStruct, (J9ArrayClass*)(classPtr->componentType), dimensions, dimensions - 1,
        (U_32*)dimensionArray, allocationType);
    ...
    U_32 currentDimension, U_32* dimensionArray, UDATA allocationType)
{
    ...
    J9JAVAARRAYOFOBJECT_STORE(
        vmStruct, saveTable, currentDimension, parentResult); /* save the result to protect it from GCs */

    /* recurse if we have to */
    if (currentDimension > 0) {
        for (i = 0; i < dimensionArray[currentDimension]; i++) {
            if (NULL
                == (childResult = allocate_dimension(vmStruct, (J9ArrayClass*)(currentClass->componentType), dimensions,
                        currentDimension - 1, dimensionArray, allocationType))) {
                return NULL;
            }
            saveTable = PEEK_OBJECT_IN_SPECIAL_FRAME(vmStruct, 0);
            parentResult = J9JAVAARRAYOFOBJECT_LOAD(
                vmStruct, saveTable, currentDimension); /* reload the result from the array in case a GC happened */
