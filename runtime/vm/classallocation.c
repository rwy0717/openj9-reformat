...
J9ClassLoader* internalAllocateClassLoader(J9JavaVM* javaVM, j9object_t classLoaderObject)
{
    ...
    for (i = 0; i < CLASS_PROPAGATION_TABLE_SIZE; i++) {
        knownClasses[i]
            = internalFindKnownClass(vmThread, classPropagationTable[i], J9_FINDKNOWNCLASS_FLAG_EXISTING_ONLY);
    }
...
retry:
    ...
    for (i = 0; i < CLASS_PROPAGATION_TABLE_SIZE; i++) {
        ...
        if (NULL != knownClass) {
            ...
            UDATA hashTableResult = hashClassTableAtPut(
                vmThread, classLoader, J9UTF8_DATA(className), J9UTF8_LENGTH(className), knownClass);
...
J9ClassLoader* allocateClassLoader(J9JavaVM* javaVM)
{
    ...
    if (classLoader != NULL) {
        ...
        if (NULL == javaVM->systemClassLoader) {
            classLoader->classLocationHashTable
                = hashClassLocationTableNew(javaVM, INITIAL_CLASSLOCATION_HASHTABLE_SIZE);
        }

        if ((NULL == classLoader->classHashTable) || (NULL == classLoader->moduleHashTable)
...
void freeClassLoader(J9ClassLoader* classLoader, J9JavaVM* javaVM, J9VMThread* vmThread, UDATA needsFrameBuild)
{
    ...
    if (classLoader->sharedLibraries != NULL) {
        ...
        while (NULL != nativeLibrary) {
            ...
            if (needsFrameBuild) {
                 ...
                 *		self
                 *			buildSpecialStackFrame: J9SFJNINativeMethodFrame forThread: aVMThread flags: 0
                 *visible: false; push: nil "method"
                 */
            ...
            if (J9NATIVELIB_LINK_MODE_STATIC == nativeLibrary->linkMode) {
                ...
                if (NULL == onUnloadRtnName) {
                    ...
                    j9str_printf(
                        PORTLIB, onUnloadRtnName, nameLength, "%s%s", J9STATIC_ONUNLOAD, nativeLibrary->logicalName);
    ...
    if (NULL != classLoader->moduleExtraInfoHashTable) {
        ...
        J9ModuleExtraInfo* moduleExtraInfoPtr
            = (J9ModuleExtraInfo*)hashTableStartDo(classLoader->moduleExtraInfoHashTable, &moduleExtraInfoWalkState);
