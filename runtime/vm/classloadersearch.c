...
static UDATA addZipToLoader(
    J9JavaVM* vm, const char* filename, J9ClassLoader* classLoader, BOOLEAN enforceJarRestriction);
 ...
 * @param enforceJarRestriction 	when true, fail if the pathSegment is not a jar file and we are adding to the
 * system classloader
 * @return error code
 */
UDATA
...
static UDATA addToSystemProperty(J9JavaVM* vm, const char* propertyName, const char* segment)
{
    ...
    if (getSystemProperty(vm, propertyName, &systemProperty) == J9SYSPROP_ERROR_NONE) {
        ...
        newValue = j9mem_allocate_memory(
            currentLength + strlen(segment) + 2, OMRMEM_CATEGORY_VM); /* +1 for nul, +1 for separator */
        if (newValue == NULL) {
    ...
    if ((classLoader == vm->systemClassLoader) && (J2SE_VERSION(vm) >= J2SE_19)) {
        ...
        if (currentThread != NULL) {
            ...
            if (J2SE_SHAPE(vm) >= J2SE_SHAPE_B165) {
                ...
                loadModule = (*env)->GetStaticMethodID(
                    env, jimModules, "loadModule", "(Ljava/lang/String;)Ljava/lang/Module;");
                if (NULL == loadModule) {
                    rc = CLS_ERROR_NOT_FOUND;
                    goto cleanup;
                }
            ...
            mid = (*env)->GetMethodID(
                env, classLoaderClass, "appendToClassPathForInstrumentation", "(Ljava/lang/String;)V");
            if (mid == NULL) {
