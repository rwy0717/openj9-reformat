 ...
 */
static UDATA calculateAppropriateSegmentSize(
    J9JavaVM* javaVM, UDATA requiredSize, UDATA segmentType, J9ClassLoader* classLoader, UDATA allocationIncrement)
{
    ...
    if ((classLoader != javaVM->systemClassLoader) && (requiredSize < segmentSize)
        && (classLoader != javaVM->applicationClassLoader) && (classLoader != javaVM->anonClassLoader)) {
 ...
 * @note allocationIncrement should typically be javaVM->ramClassAllocationIncrement or
    ...
    J9JavaVM* javaVM, UDATA requiredSize, UDATA segmentType, J9ClassLoader* classLoader, UDATA allocationIncrement)
{
    ...
    appropriateSize
        = calculateAppropriateSegmentSize(javaVM, requiredSize, segmentType, classLoader, allocationIncrement);
    memorySegment = allocateMemorySegmentInList(
        javaVM, javaVM->classMemorySegments, appropriateSize, segmentType, J9MEM_CATEGORY_CLASSES);
