...
static J9ContendedLoadTableEntry* contendedLoadTableAddThread(
    J9VMThread* vmThread, J9ClassLoader* classLoader, U_8* className, UDATA classNameLength, UDATA status);
static J9ContendedLoadTableEntry* contendedLoadTableGet(
    J9JavaVM* vm, J9ClassLoader* classLoader, U_8* className, UDATA classNameLength);
static UDATA contendedLoadTableRemoveThread(J9VMThread* vmThread, J9ContendedLoadTableEntry* tableEntry, UDATA status);
...
static J9Class* internalFindArrayClass(J9VMThread* vmThread, J9Module* j9module, UDATA arity, U_8* name, UDATA length,
...
J9Class* internalCreateArrayClass(J9VMThread* vmThread, J9ROMArrayClass* romClass, J9Class* elementClass)
{
    ...
    result
        = internalCreateRAMClassFromROMClass(vmThread, elementClass->classLoader, arrayRomClass, options, /* options */
            elementClass, protectionDomain, NULL, J9_CP_INDEX_NONE, LOAD_LOCATION_UNKNOWN, NULL, NULL);
    return result;
}
    ...
    J9VMThread* currentThread, j9object_t moduleName, j9object_t className, J9ClassLoader* classLoader, UDATA options)
{
    ...
    if (NULL == result) {
        ...
        utf8Name = copyStringToUTF8WithMemAlloc(currentThread, className, J9_STR_NULL_TERMINATE_RESULT | J9_STR_XLAT,
            "", 0, localBuf, J9VM_PACKAGE_NAME_BUFFER_LENGTH, &utf8Length);
        if (NULL == utf8Name) {
            /* Throw out-of-memory */
...
void internalRunPreInitInstructions(J9Class* ramClass, J9VMThread* vmThread)
{
    ...
    if (ramConstantPoolCount != 0) {
        ...
        for (i = 0; i < ramConstantPoolCount; ++i) {
            ...
            case J9CPTYPE_INT:
            case J9CPTYPE_FLOAT:
                *((U_32*)&(((J9RAMSingleSlotConstantRef*)ramConstantPool)[i].value))
                    = ((J9ROMSingleSlotConstantRef*)romConstantPool)[i].data;
                break;
    ...
    if (NULL != clazz) {
        ...
        for (i = 0; i < constPoolCount; i++) {
            if ((J9CPTYPE_FIELD == J9_CP_TYPE(cpShapeDescription, i))
                ...
                if (-1
                    == resolveInstanceFieldRef(currentThread, NULL, jclConstantPool, i,
                           J9_RESOLVE_FLAG_RUNTIME_RESOLVE | J9_RESOLVE_FLAG_NO_THROW_ON_FAIL, NULL)) {
                    Trc_VM_resolveKnownClass_resolveInstanceFieldRef_Failed(currentThread, i, index, clazz);
                } else
                    Trc_VM_resolveKnownClass_resolvedInstanceFieldRef(
                        currentThread, i, index, clazz, ramConstantPool[i].valueOffset);
            }
        }
    }
...
J9Class* foundROMClass(J9VMThread* vmThread, J9ClassLoader* classLoader, UDATA options, J9ROMClass* romClass,
    J9ROMClass** errorRomClassP, UDATA entryIndex, I_32 locationType)
{
    ...
    *errorRomClassP = NULL;
    if (NULL == checkRomClassForError(romClass, vmThread)) {
        *errorRomClassP = romClass;
        ...
        foundClass = internalCreateRAMClassFromROMClass(
            vmThread, classLoader, romClass, options, NULL, NULL, NULL, entryIndex, locationType, NULL, NULL);
    }
    return foundClass;
}
...
static IDATA callFindLocallyDefinedClass(J9VMThread* vmThread, J9Module* j9module, U_8* className,
    UDATA classNameLength, J9ClassLoader* classLoader, UDATA options, J9TranslationLocalBuffer* localBuffer)
{
    IDATA findResult = -1;
    ...
    if (NULL != dynamicLoadBuffers) {
        ...
        TRIGGER_J9HOOK_VM_FIND_LOCALLY_DEFINED_CLASS(vmThread->javaVM->hookInterface, vmThread, classLoader, j9module,
            (char*)className, classNameLength, classPathEntries, classLoader->classPathEntryCount, -1, NULL, 0, 0,
        ...
        if (0 == findResult) {
            TRIGGER_J9HOOK_VM_FIND_LOCALLY_DEFINED_CLASS_FROM_FILESYSTEM(vmThread->javaVM->hookInterface, vmThread,
                (const char*)className, classNameLength, classLoader, &classFound, returnPointer);

            if (returnVal != NULL) {
                findResult = classFound;
            } else {
                findResult = dynamicLoadBuffers->findLocallyDefinedClassFunction(vmThread, j9module, className,
                    (U_32)classNameLength, classLoader, classPathEntries, classLoader->classPathEntryCount, options,
                    localBuffer);
...
static J9Class* attemptDynamicClassLoad(J9VMThread* vmThread, J9Module* j9module, U_8* className, UDATA classNameLength,
    J9ClassLoader* classLoader, UDATA options)
{
    J9Class* foundClass = NULL;

    Trc_VM_internalFindClass_attemptDynamicClassLoad_entry(
        vmThread, classLoader->classLoaderObject, classNameLength, className);
    ...
    if ((J2SE_VERSION(vmThread->javaVM) >= J2SE_19)
        ...
        findResult = callFindLocallyDefinedClass(
            vmThread, j9module, className, classNameLength, classLoader, options, &localBuffer);
            ...
            if (0 == findResult) {
                ...
                if (NULL == dynamicLoadBuffers) {
                    setCurrentExceptionUTF(
                        vmThread, J9VMCONSTANTPOOL_JAVALANGNOCLASSDEFFOUNDERROR, "dynamic loader is unavailable");
                } else {
                    UDATA defineClassOptions = options
                        & ~J9_FINDCLASS_FLAG_USE_LOADER_CP_ENTRIES; /* allow define class to use normal findClass */
                    ...
                    foundClass = dynamicLoadBuffers->internalDefineClassFunction(vmThread, className, classNameLength,
                        dynamicLoadBuffers->sunClassFileBuffer, dynamicLoadBuffers->currentSunClassFileSize, NULL,
                        classLoader, NULL, defineClassOptions, NULL, NULL,
                        &localBuffer); /* this function exits the class table mutex */
                }
            } else {
                ...
                if ((NULL != vmThread->javaVM->sharedClassConfig)
                    && (0 != vmThread->javaVM->sharedClassConfig->isBCIEnabled(vmThread->javaVM))) {
                    UDATA defineClassOptions = options
                        & ~J9_FINDCLASS_FLAG_USE_LOADER_CP_ENTRIES; /* allow define class to use normal findClass */
                    ...
                    foundClass = dynamicLoadBuffers->internalDefineClassFunction(vmThread, className, classNameLength,
                        J9ROMCLASS_INTERMEDIATECLASSDATA(romClass), romClass->intermediateClassDataLength, NULL,
                        classLoader, NULL, defineClassOptions, romClass, NULL, &localBuffer);
                } else {
                    J9ROMClass* errorRomClass;
                    /* this function exits the class table mutex */
                    foundClass = foundROMClass(vmThread, classLoader, options, romClass, &errorRomClass,
                        localBuffer.entryIndex, localBuffer.loadLocationType);
...
static VMINLINE J9Class* callLoadClass(J9VMThread* vmThread, U_8* className, UDATA classNameLength,
    J9ClassLoader* classLoader, j9object_t* classNotFoundException)
{
    ...
    classNameString = vmThread->javaVM->memoryManagerFunctions->j9gc_createJavaLangString(
        vmThread, className, classNameLength, J9_STR_XLAT);
    if (classNameString != NULL) {
        J9JavaVM* vm = vmThread->javaVM;

        Trc_VM_internalFindClass_sendLoadClass(
            vmThread, classNameLength, className, classNameString, classLoader->classLoaderObject);
        ...
        if (NULL == sendLoadClassResult) {
            ...
            if (exception != NULL) {
                ...
                notFound = internalFindKnownClass(
                    vmThread, J9VMCONSTANTPOOL_JAVALANGCLASSNOTFOUNDEXCEPTION, J9_FINDKNOWNCLASS_FLAG_EXISTING_ONLY);
            ...
            if (!J9UTF8_DATA_EQUALS(
                    className, classNameLength, J9UTF8_DATA(foundClassName), J9UTF8_LENGTH(foundClassName))) {
                /* force failure */
                foundClass = NULL;
            } else {
                ...
                if (NULL == ramClass) {
                    ...
                    if (vm->runtimeFlags & J9_RUNTIME_VERIFY) {
                        J9Class* loadingConstraintError
                            = j9bcv_satisfyClassLoadingConstraint(vmThread, classLoader, foundClass);
                    ...
                    if (hashClassTableAtPut(vmThread, classLoader, className, classNameLength, foundClass)) {
                        ...
                        omrthread_monitor_exit(vmThread->javaVM->classTableMutex);
                        vmThread->javaVM->memoryManagerFunctions->j9gc_modron_global_collect_with_overrides(
...
static J9Class* waitForContendedLoadClass(
    J9VMThread* vmThread, J9ContendedLoadTableEntry* tableEntry, U_8* className, UDATA classNameLength)
{
    ...
    Trc_VM_waitForContendedLoadClass_getObjectMonitorOwner(
        vmThread, vmThread, tableEntry->classLoader, classNameLength, className);
    Assert_VM_mustHaveVMAccess(vmThread);
    /* get here if and only if someone else is loading the class */
    /* give up the classloader monitor to allow other threads to run */
    monitorOwner = getObjectMonitorOwner(
        vmThread->javaVM, vmThread, tableEntry->classLoader->classLoaderObject, &recursionCount);
    if (monitorOwner == vmThread) {
        Trc_VM_waitForContendedLoadClass_release_object_monitor(
            vmThread, vmThread, tableEntry->classLoader, classNameLength, className);
    ...
    Trc_VM_waitForContendedLoadClass_waited(
        vmThread, vmThread, tableEntry->classLoader, classNameLength, className, status);
    foundClass = hashClassTableAt(tableEntry->classLoader, className, classNameLength);
    omrthread_monitor_exit(vmThread->javaVM->classTableMutex);
    internalAcquireVMAccess(vmThread);
    Trc_VM_waitForContendedLoadClass_acquired_vm_access(
        vmThread, vmThread, tableEntry->classLoader, classNameLength, className);
    for (i = 0; i < recursionCount; ++i) {
    ...
    J9ClassLoader* classLoader, j9object_t* classNotFoundException)
{
    ...
    *classNotFoundException = NULL;
    ...
    if ((NULL == vmThread->javaVM->contendedLoadTable) /* allow contended classloading in all loaders*/
        || (J9CLASSLOADER_PARALLEL_CAPABLE
               == (J9CLASSLOADER_PARALLEL_CAPABLE & classLoader->flags)) /* this loader is parallel capable */
    ) {
        /* start old algorithm */
        omrthread_monitor_exit(vmThread->javaVM->classTableMutex);
        ...
        J9ContendedLoadTableEntry* tableEntry = contendedLoadTableAddThread(
            vmThread, classLoader, className, classNameLength, CLASSLOADING_LOAD_IN_PROGRESS);
        U_8 done = 1;
        UDATA count = 0;

        do {
            if (tableEntry->thread == vmThread) {
                /* get here if and only if I am the first to try loading the class or if I am retrying because the first
                 * guy failed. */

                omrthread_monitor_exit(vmThread->javaVM->classTableMutex);
                Trc_VM_arbitratedLoadClass_callLoadClass(vmThread, vmThread, classLoader, classNameLength, className);
                foundClass = callLoadClass(
                    vmThread, className, classNameLength, tableEntry->classLoader, classNotFoundException);
                omrthread_monitor_enter(vmThread->javaVM->classTableMutex);
                done = 1;
            } else {
        ...
        count = contendedLoadTableRemoveThread(
            vmThread, tableEntry, (NULL == foundClass) ? CLASSLOADING_LOAD_FAILED : CLASSLOADING_LOAD_SUCCEEDED);
        if (count > 0) { /* only the owner(s) should do the notification */
            if (tableEntry->thread == vmThread) {
                ...
                Trc_VM_arbitratedLoadClass_notify(
                    vmThread, vmThread, tableEntry->classLoader, classNameLength, className);
 ...
 * @return loaded class object if success, NULL if fail
 *
 * Precondition: either J9_FINDCLASS_FLAG_EXISTING_ONLY set or classTableMutex not owned by this thread; this thread has
 * vm access Postcondition: either J9_FINDCLASS_FLAG_EXISTING_ONLY set or classTableMutex not owned by this thread; this
 * thread has vm access
 */

static VMINLINE J9Class* loadNonArrayClass(J9VMThread* vmThread, J9Module* j9module, U_8* className,
    UDATA classNameLength, J9ClassLoader* classLoader, UDATA options, j9object_t* exception)
{
    J9Class* foundClass = NULL;
    BOOLEAN lockLoaderMonitor = FALSE;
    BOOLEAN fastMode
        = J9_ARE_ALL_BITS_SET(vmThread->javaVM->extendedRuntimeFlags, J9_EXTENDED_RUNTIME_FAST_CLASS_HASH_TABLE);
    ...
    if (J9CLASSLOADER_PARALLEL_CAPABLE == (J9CLASSLOADER_PARALLEL_CAPABLE & classLoader->flags)) {
        ...
        lockLoaderMonitor = (classLoader != vmThread->javaVM->systemClassLoader)
            && (NULL != classLoader->classLoaderObject) && (0 == (options & J9_FINDCLASS_FLAG_EXISTING_ONLY))
    ...
    if (NULL != foundClass) {
        ...
        if (options & J9_FINDCLASS_FLAG_EXISTING_ONLY) {
            ...
            if (0 == (vmThread->javaVM->runtimeFlags & J9_RUNTIME_XFUTURE)) {
                if ((classNameLength <= 7)
                    && ((classLoader == vmThread->javaVM->systemClassLoader)
                           || (classLoader == vmThread->javaVM->applicationClassLoader))) {
                    ...
                    case 3:
                        ...
                        primitiveClass:
                            ...
                            if (loaderMonitorLocked) {
                                Trc_VM_loadNonArrayClass_exit_object_monitor(
                                    vmThread, classLoader, classNameLength, className);
            ...
            /* go directly to the dynamic loader if the flag is set or we are the bootstrap loader and we do not have a
             * Java object to send loadClass() to */
            if (((classLoader == vmThread->javaVM->systemClassLoader) && (NULL == classLoader->classLoaderObject))
                ...
                foundClass
                    = attemptDynamicClassLoad(vmThread, j9module, className, classNameLength, classLoader, options);
                /* class table mutex is now unlocked */
#endif
            } else {
 ...
 */
J9Class* internalFindClassUTF8(
    J9VMThread* vmThread, U_8* className, UDATA classNameLength, J9ClassLoader* classLoader, UDATA options)
{
    ...
    J9ClassLoader* classLoader, UDATA options)
{
    ...
    if (0 != arity) {
        foundClass
            = internalFindArrayClass(vmThread, j9module, arity, className, classNameLength, classLoader, options);
    } else {
        foundClass
            = loadNonArrayClass(vmThread, j9module, className, classNameLength, classLoader, options, &exception);
    ...
    if (NULL == foundClass) {
        ...
        if (NULL == vmThread->currentException) {
            if (0
                == (vmThread->publicFlags
                       & J9_PUBLIC_FLAGS_POP_FRAMES_INTERRUPT)) { /* no immediate async interrupt pending */
                /* Don't throw an exception if we are loading required classes. It can lead to OOM */
                BOOLEAN reportErrorFlags
                    = (J9_PRIVATE_FLAGS_REPORT_ERROR_LOADING_CLASS | J9_PRIVATE_FLAGS_FAILED_LOADING_REQUIRED_CLASS);
                ...
                if (reportErrorFlags == (reportErrorFlags & vmThread->privateFlags)) {
                    ...
                    j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_VM_UNABLE_TO_FIND_AND_INITIALIZE_REQUIRED_CLASS,
                        classNameLength, className);
#else
                    j9nls_printf(PORTLIB, J9NLS_ERROR | J9NLS_BEGIN_MULTI_LINE,
                        J9NLS_VM_UNABLE_TO_FIND_AND_INITIALIZE_REQUIRED_CLASS, classNameLength, className);
#endif
                    ...
                    PUSH_OBJECT_IN_SPECIAL_FRAME(vmThread, exception);
                    detailMessage
                        = gcFuncs->j9gc_createJavaLangString(vmThread, className, classNameLength, J9_STR_XLAT);
                    exception = POP_OBJECT_IN_SPECIAL_FRAME(vmThread);
                    if (NULL != detailMessage) {
                        setCurrentExceptionWithCause(
                            vmThread, J9VMCONSTANTPOOL_JAVALANGNOCLASSDEFFOUNDERROR, (UDATA*)detailMessage, exception);
                    }
                }
            }
        }
...
J9Class* internalFindKnownClass(J9VMThread* vmThread, UDATA index, UDATA flags)
{
    J9JavaVM* vm = vmThread->javaVM;
    J9ConstantPool* jclConstantPool = (J9ConstantPool*)vm->jclConstantPool;
    ...
    if (NULL == knownClass) {
        ...
        if ((J9VMCONSTANTPOOL_JAVALANGCLASSNOTFOUNDEXCEPTION == index)
            || (J9VMCONSTANTPOOL_JAVALANGNOCLASSDEFFOUNDERROR == index)) {
        ...
        knownClass
            = internalFindClassUTF8(vmThread, J9UTF8_DATA(utfWrapper), J9UTF8_LENGTH(utfWrapper), classLoader, options);

        if ((J9VMCONSTANTPOOL_JAVALANGCLASSNOTFOUNDEXCEPTION == index)
            || (J9VMCONSTANTPOOL_JAVALANGNOCLASSDEFFOUNDERROR == index)) {
            vmThread->privateFlags &= (~J9_PRIVATE_FLAGS_LOADING_KNOWN_CLASS);
        }
        /* If a doReturn is pending, do not cache the found class.  It will be looked up and correctly initialized the
        ...
        if (NULL == knownClass) {
            /* If J9_PRIVATE_FLAGS_FAILED_LOADING_REQUIRED_CLASS is set, then we have already generated error message in
            ...
            if ((NULL != classLocation) && (J9_CP_INDEX_NONE != classLocation->entryIndex)
                ...
                if (NULL != cpEntry) {
                    if (0 == (CPE_FLAG_BOOTSTRAP & cpEntry->flags)) {
                        j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_VM_REQUIRED_CLASS_ON_WRONG_PATH,
                            J9UTF8_LENGTH(utfWrapper), J9UTF8_DATA(utfWrapper), cpEntry->pathLength, cpEntry->path);
                        goto _exit;
                    }
                }
            }
    ...
    if (NULL != knownClass) {
        if (0 != (J9_FINDKNOWNCLASS_FLAG_INITIALIZE & flags)) {
            if ((J9ClassInitSucceeded != knownClass->initializeStatus)
                && ((UDATA)vmThread != knownClass->initializeStatus)) {
...
#if defined(J9VM_SIZE_SMALL_CODE)
        j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_VM_UNABLE_TO_FIND_AND_INITIALIZE_REQUIRED_CLASS,
            J9UTF8_LENGTH(utfWrapper), J9UTF8_DATA(utfWrapper));
#else
        j9nls_printf(PORTLIB, J9NLS_ERROR | J9NLS_BEGIN_MULTI_LINE,
            J9NLS_VM_UNABLE_TO_FIND_AND_INITIALIZE_REQUIRED_CLASS, J9UTF8_LENGTH(utfWrapper), J9UTF8_DATA(utfWrapper));
#endif
        for (i = 0; i < classLoader->classPathEntryCount; i++) {
            J9ClassPathEntry* entry = &classLoader->classPathEntries[i];
            j9nls_printf(PORTLIB, J9NLS_INFO, J9NLS_VM_SEARCHED_IN_DIR, entry->pathLength, entry->path);
...
static UDATA classAndLoaderHashEqualFn(void* leftKey, void* rightKey, void* userData)
{
    ...
    return (leftEntry->classLoader == rightEntry->classLoader)
        && J9UTF8_DATA_EQUALS(
               leftEntry->className, leftEntry->classNameLength, rightEntry->className, rightEntry->classNameLength);
}
...
J9HashTable* contendedLoadTableNew(J9JavaVM* vm, J9PortLibrary* portLib)
{
    ...
    result = vm->contendedLoadTable
        = hashTableNew(OMRPORT_FROM_J9PORT(portLib), J9_GET_CALLSITE(), 64, sizeof(J9ContendedLoadTableEntry),
            sizeof(U_8*), 0, J9MEM_CATEGORY_CLASSES, classAndLoaderHashFn, classAndLoaderHashEqualFn, NULL, vm);
    return result;
}
 ...
 * Note: a thread must call this exactly once per loading attempt. The className pointer becomes null when the loading
 * thread completes the load.
 * @param vmThread
 ...
 */
static J9ContendedLoadTableEntry* contendedLoadTableAdd(
    J9VMThread* vmThread, J9ClassLoader* classLoader, U_8* className, UDATA classNameLength)
{
    ...
    query.hashValue = classAndLoaderHashFn(&query,
        NULL); /* SO I can get the hash table if the className pointer is null but I have a pointer to the record */
 ...
 */
static void contendedLoadTableDelete(J9VMThread* vmThread, J9ContendedLoadTableEntry* entry)
{
    if (NULL == entry->className) {
    ...
    J9VMThread* vmThread, J9ClassLoader* classLoader, U_8* className, UDATA classNameLength, UDATA status)
{
    ...
    Trc_VM_contendedLoadTableAddThreadExit(
        vmThread, vmThread, classLoader, classNameLength, className, (NULL != result ? result->count : 0));
    return result;
}
...
static UDATA contendedLoadTableRemoveThread(J9VMThread* vmThread, J9ContendedLoadTableEntry* tableEntry, UDATA status)
{
    ...
    if (NULL == tableEntry->className) {
        ...
        Trc_VM_contendedLoadTableRemoveThread(vmThread, vmThread, tableEntry->classLoader, tableEntry->classNameLength,
            tableEntry->className, tableEntry->count);
    }
...
void fixCPShapeDescription(J9Class* clazz, UDATA cpIndex)
{
    ...
    cpShapeDescription[wordIndex] = (cpShapeDescription[wordIndex] & ~(J9_CP_DESCRIPTION_MASK << shiftAmount))
        | (J9CPTYPE_INSTANCE_METHOD << shiftAmount);
}

void fixUnsafeMethods(J9VMThread* currentThread, jclass clazz)
{
    ...
    while (currentMethod != endOfMethods) {
        ...
        while (pc < endPC) {
            ...
            case JBinvokevirtual: {
                ...
                J9Class* resolvedClass = resolveClassRef(
                    currentThread, ramCP, romMethodRef->classRefCPIndex, J9_RESOLVE_FLAG_RUNTIME_RESOLVE);
                if (NULL == resolvedClass) {
                    currentThread->currentException = NULL;
                    currentThread->privateFlags &= ~(UDATA)J9_PRIVATE_FLAGS_REPORT_EXCEPTION_THROW;
                } else {
                    J9Method* method = (J9Method*)javaLookupMethod(currentThread, resolvedClass,
                        J9ROMMETHODREF_NAMEANDSIGNATURE(romMethodRef), NULL, J9_LOOK_VIRTUAL | J9_LOOK_NO_THROW);
                    if (NULL != method) {
