...
/* Static J9ITable used as a non-NULL iTable cache value by classes that don't implement any interfaces. Defined in
 * hshelp.c. */
...
static J9Class* markInterfaces(J9ROMClass* romClass, J9Class* superclass, J9ClassLoader* classLoader,
    BOOLEAN* foundCloneable, UDATA* markedInterfaceCount, UDATA* inheritedInterfaceCount, IDATA* maxInterfaceDepth);
static void unmarkInterfaces(J9Class* interfaceHead);
static void createITable(J9VMThread* vmStruct, J9Class* ramClass, J9Class* interfaceClass, J9ITable*** previousLink,
    UDATA** currentSlot, UDATA depth);
static UDATA* initializeRAMClassITable(J9VMThread* vmStruct, J9Class* ramClass, J9Class* superclass, UDATA* currentSlot,
    J9Class* interfaceHead, IDATA maxInterfaceDepth);
static UDATA addInterfaceMethods(J9VMThread* vmStruct, J9ClassLoader* classLoader, J9Class* interfaceClass,
    UDATA vTableMethodCount, UDATA* vTableAddress, J9Class* superclass, J9ROMClass* romClass,
    UDATA* defaultConflictCount, J9Pool* equivalentSets, UDATA* equivSetCount, J9OverrideErrorData* errorData);
static UDATA* computeVTable(J9VMThread* vmStruct, J9ClassLoader* classLoader, J9Class* superclass,
    J9ROMClass* taggedClass, UDATA packageID, J9ROMMethod** methodRemapArray, J9Class* interfaceHead,
    UDATA* defaultConflictCount, UDATA interfaceCount, UDATA inheritedInterfaceCount, J9OverrideErrorData* errorData);
static void copyVTable(
    J9VMThread* vmStruct, J9Class* ramClass, J9Class* superclass, UDATA* vTable, UDATA defaultConflictCount);
static UDATA processVTableMethod(J9VMThread* vmThread, J9ClassLoader* classLoader, UDATA* vTableAddress,
    J9Class* superclass, J9ROMClass* romClass, J9ROMMethod* romMethod, UDATA localPackageID, UDATA vTableMethodCount,
    void* storeValue, J9OverrideErrorData* errorData);
...
static VMINLINE BOOLEAN loadSuperClassAndInterfaces(J9VMThread* vmThread, J9ClassLoader* classLoader,
    J9ROMClass* romClass, J9Class* elementClass, UDATA packageID, BOOLEAN hotswapping, UDATA classPreloadFlags,
    J9Class** superclassOut, J9Module* module);
#if defined(J9VM_OPT_VALHALLA_VALUE_TYPES)
static BOOLEAN loadFlattenableFieldValueClasses(J9VMThread* vmThread, J9ClassLoader* classLoader, J9ROMClass* romClass,
    UDATA classPreloadFlags, UDATA packageID, J9Module* module);
#endif /* J9VM_OPT_VALHALLA_VALUE_TYPES */
static J9Class* internalCreateRAMClassDropAndReturn(
    J9VMThread* vmThread, J9ROMClass* romClass, J9CreateRAMClassState* state);
static J9Class* internalCreateRAMClassDoneNoMutex(
    J9VMThread* vmThread, J9ROMClass* romClass, UDATA options, J9CreateRAMClassState* state);
static J9Class* internalCreateRAMClassDone(J9VMThread* vmThread, J9ClassLoader* classLoader, J9ROMClass* romClass,
    UDATA options, J9Class* elementClass, J9UTF8* className, J9CreateRAMClassState* state);
static J9Class* internalCreateRAMClassFromROMClassImpl(J9VMThread* vmThread, J9ClassLoader* classLoader,
    J9ROMClass* romClass, UDATA options, J9Class* elementClass, J9ROMMethod** methodRemapArray, IDATA entryIndex,
    I_32 locationType, J9Class* classBeingRedefined, UDATA packageID, J9Class* superclass, J9CreateRAMClassState* state,
    J9ClassLoader* hostClassLoader, J9Class* hostClass, J9Module* module);
static J9MemorySegment* internalAllocateRAMClass(J9JavaVM* javaVM, J9ClassLoader* classLoader,
    RAMClassAllocationRequest* allocationRequests, UDATA allocationRequestCount);
...
static void trcModulesSettingPackage(
    J9VMThread* vmThread, J9Class* ramClass, J9ClassLoader* classLoader, J9UTF8* className);
    ...
    if (interfaceCount != 0) {
        ...
        for (i = 0; i < interfaceCount; i++) {
            ...
            J9Class* interfaceClass
                = hashClassTableAt(classLoader, J9UTF8_DATA(interfaceName), J9UTF8_LENGTH(interfaceName));
            /* the interface classes are not NULL, as this was checked by the caller */
            if ((foundCloneable != NULL)
                && (J9_JAVA_CLASS_CLONEABLE == (J9CLASS_FLAGS(interfaceClass) & J9_JAVA_CLASS_CLONEABLE))) {
                *foundCloneable = TRUE;
            }
...
static void addITableMethods(J9VMThread* vmStruct, J9Class* ramClass, J9Class* interfaceClass, UDATA** currentSlot)
{
        ...
        while (count-- > 0) {
            ...
            while (searchIndex < vTableSize) {
                ...
                if (vTableRomMethod->modifiers & J9_JAVA_PUBLIC) {
                    if ((J9UTF8_LENGTH(interfaceMethodName) == J9UTF8_LENGTH(vTableMethodName))
                        ...
                        && (memcmp(J9UTF8_DATA(interfaceMethodName), J9UTF8_DATA(vTableMethodName),
                                J9UTF8_LENGTH(vTableMethodName))
                               == 0)
                        && (memcmp(J9UTF8_DATA(interfaceMethodSig), J9UTF8_DATA(vTableMethodSig),
                                J9UTF8_LENGTH(vTableMethodSig))
                               == 0)) {
...
#if defined(J9VM_TRACE_ITABLE)
            {
                ...
                j9tty_printf(PORTLIB, "\n  map %.*s%.*s to vTableOffset=%d (%d)", J9UTF8_LENGTH(interfaceMethodName),
                    J9UTF8_DATA(interfaceMethodName), J9UTF8_LENGTH(interfaceMethodSig),
                    J9UTF8_DATA(interfaceMethodSig), vTableOffset, searchIndex);
            }
#endif
    ...
    J9Class* interfaceHead, IDATA maxInterfaceDepth)
{
...
#if defined(J9VM_TRACE_ITABLE)
    {
        ...
        j9tty_printf(PORTLIB, "\n<initializeRAMClassITable: %.*s %d>", J9UTF8_LENGTH(className), J9UTF8_DATA(className),
            (romClass->modifiers & J9_JAVA_INTERFACE));
    }
#endif
    ...
    if (J9ROMCLASS_IS_ARRAY(romClass) && (booleanArrayClass != NULL)) {
        ...
        while (interfaceHead != NULL) {
            ...
            createITable(vmStruct, ramClass, interfaceHead, &previousLink, &currentSlot,
                ((J9ITable*)interfaceHead->iTable)->depth);
 ...
 */
static bool VMINLINE areNamesAndSignaturesEqual(J9UTF8* name1, J9UTF8* sig1, J9UTF8* name2, J9UTF8* sig2)
{
    /* utf8 pointers may be equal due to utf sharing */
    return ((name1 == name2) && (sig1 == sig2)) || /* Do length checks first as they are less expensive than memcmp */
        (((J9UTF8_LENGTH(name1) == J9UTF8_LENGTH(name2)) && (J9UTF8_LENGTH(sig1) == J9UTF8_LENGTH(sig2))
        ...
        for (j = 0; j < count; j++) {
            ...
            if (J9_ARE_NO_BITS_SET(romMethod->modifiers, J9_JAVA_PRIVATE | J9_JAVA_STATIC)) {
                ...
                while (tempIndex > 0) {
                    ...
                    if (areNamesAndSignaturesEqual(
                            interfaceMethodNameUTF, interfaceMethodSigUTF, vTableMethodNameUTF, vTableMethodSigUTF)) {
                        switch (state) {
                        ...
                        case SLOT_IS_CONFLICT_METHOD:
                            /* conflict detected when building the superclass vtable.  Replace with current interface
                             * method and allow the conflict to be re-detected if it hasn't been resolved by subsequent
                             * interfaces
                             */
                        ...
                        case SLOT_IS_INTERFACE_RAM_METHOD:
                            ...
                            if (((J9ITable*)methodClass->iTable)->depth < interfaceDepth) {
                                ...
                                if (isSameOrSuperInterfaceOf(interfaceClass, methodClass)) {
                                    ...
                                    const UDATA combinedModifiers
                                        = J9_ROM_METHOD_FROM_RAM_METHOD(interfaceMethod)->modifiers
                                        | vTableMethod->modifiers;
                                    if (J9_ARE_ANY_BITS_SET(combinedModifiers, J9_JAVA_ABSTRACT)) {
                                        ...
                                        *equivSetCount += 1;
                                        ...
                                        /* Conflict detected */
                                        vTableMethods[tempIndex] = (J9Method*)((UDATA)vTableMethods[tempIndex]
                                            | DEFAULT_CONFLICT_METHOD_ID_TAG);
                        ...
                        add_existing:
                            J9EquivalentEntry* existing_entry
                                = (J9EquivalentEntry*)((UDATA)vTableMethods[tempIndex] & ~EQUIVALENT_SET_ID_TAG);
                            J9EquivalentEntry* previous_entry = existing_entry;
                            while (NULL != existing_entry) {
                                if (isSameOrSuperInterfaceOf(
                                        interfaceClass, J9_CLASS_FROM_METHOD(existing_entry->method))) {
                        ...
                        if (J9_JAVA_PUBLIC == (vTableMethod->modifiers & J9_JAVA_PUBLIC)) {
                            ...
                            if (interfaceLoader != vTableMethodLoader) {
                                if (0
                                    != j9bcv_checkClassLoadingConstraintsForSignature(vmStruct, vTableMethodLoader,
                                           interfaceLoader, vTableMethodSigUTF, interfaceMethodSigUTF)) {
...
#if defined(J9VM_TRACE_VTABLE_ACCESS)
                {
                    ...
                    j9tty_printf(PORTLIB, "\n<vtbl_init: adding @ index=%d %.*s.%.*s%.*s (0x%x)>", vTableMethodCount,
                        J9UTF8_LENGTH(classNameUTF), J9UTF8_DATA(classNameUTF), J9UTF8_LENGTH(interfaceMethodNameUTF),
                        J9UTF8_DATA(interfaceMethodNameUTF), J9UTF8_LENGTH(interfaceMethodSigUTF),
                        J9UTF8_DATA(interfaceMethodSigUTF), romMethod);
                }
#endif
            }
 ...
 */
static void processEquivalentSets(
    J9VMThread* vmStruct, UDATA* defaultConflictCount, UDATA vTableMethodCount, UDATA* vTableAddress)
{
        ...
        if (J9ROMCLASS_IS_ARRAY(romClass) == 0) {
            ...
            if (count != 0) {
                ...
                for (i = count; i > 0; i--) {
                    ...
                    if (J9_ARE_ANY_BITS_SET(romMethod->modifiers, J9AccMethodVTable | J9_JAVA_PRIVATE)
                        ...
                        vTableMethodCount = processVTableMethod(vmStruct, classLoader, vTableAddress, superclass,
                            romClass, romMethod, packageID, vTableMethodCount,
                            (J9ROMMethod*)((UDATA)romMethod + ROM_METHOD_ID_TAG), errorData);
            ...
            if (interfaceCount > 0) {
                ...
                if (totalInterfaces <= LOCAL_INTERFACE_ARRAY_SIZE) {
                    ...
                    interfaces
                        = (J9Class**)j9mem_allocate_memory(totalInterfaces * sizeof(J9Class*), J9MEM_CATEGORY_CLASSES);
                    if (NULL == interfaces) {
                        goto fail;
                    }
                }
...
#if defined(VERBOSE_INTERFACE_METHODS)
                {
                    J9UTF8* className = J9ROMCLASS_CLASSNAME(romClass);
                    j9tty_printf(PORTLIB,
                        "\nAdding interfaceMethods for class <%.*s> using interface[%i]=%p.  #Local=%i\t#Inherited=",
                        J9UTF8_LENGTH(className), J9UTF8_DATA(className), totalInterfaces, interfaces, interfaceCount,
                        inheritedInterfaceCount);
                }
#endif /* VERBOSE_INTERFACE_METHODS */
                J9Pool* equivalentSet = pool_new(sizeof(J9EquivalentEntry), 0, 0, 0, J9_GET_CALLSITE(),
...
#endif /* VERBOSE_INTERFACE_METHODS */
                    vTableMethodCount = addInterfaceMethods(vmStruct, classLoader, interfaces[i - 1], vTableMethodCount,
                        vTableAddress, superclass, romClass, defaultConflictCount, equivalentSet, &equivSetCount,
                        errorData);
                    if ((UDATA)-1 == vTableMethodCount) {
                        if (NULL != equivalentSet) {
    ...
    for (index = 0; index < count; index++) {
        ...
        if (ROM_METHOD_ID_TAG == (temp & VTABLE_SLOT_TAG_MASK)) {
            ...
            while (vTableMethod != afterLastMethod) {
                if ((J9ROMMethod*)temp == J9_ROM_METHOD_FROM_RAM_METHOD(vTableMethod))
                    goto found;
                vTableMethod++;
            }
        ...
        if (vTableWriteIndex != 0) {
            if ((jitConfig->runtimeFlags & J9JIT_TOSS_CODE) != 0) {
                ...
                for (; vTableWriteIndex > 0; vTableWriteIndex--) {
                    ...
                    /* Always consume an entry from the super vTable.  Note that once the size hits zero and superclass
                     * becomes NULL, superVTableReadCursor will never be dereferenced again, so it's value does not
                     * matter.  Also, there's no possibilty of superVTableSize rolling over again (once it's decremented
                     * beyond 0), and it wouldn't matter if it did, since superclass will already be NULL.
                     */
                    superVTableSize--;
    ...
    void* storeValue, J9OverrideErrorData* errorData)
{
    ...
    if (J9_ARE_NO_BITS_SET(newModifiers, J9_JAVA_PRIVATE)) {
        ...
        if (NULL != superclass) {
            ...
            while ((superclassVTableIndex = getVTableIndexForNameAndSigStartingAt(
                        superclassVTableMethods, nameUTF, sigUTF, superclassVTableIndex))
                != (UDATA)-1) {
                ...
                do {
                    ...
                    J9ROMMethod* currentSuperclassVTableROMMethod
                        = J9_ROM_METHOD_FROM_RAM_METHOD(currentSuperclassVTableMethod);
                ...
                if (overridden) {
                    if ((((UDATA)storeValue & VTABLE_SLOT_TAG_MASK) == ROM_METHOD_ID_TAG)
                        ...
                        if (superclassVTableMethodLoader != classLoader) {
                            ...
                            if (0
                                != j9bcv_checkClassLoadingConstraintsForSignature(vmThread, classLoader,
                                       superclassVTableMethodLoader, sigUTF, superclassVTableMethodSigUTF)) {
                                J9UTF8* superclassVTableMethodClassNameUTF
                                    = J9ROMCLASS_CLASSNAME(superclassVTableMethodClass->romClass);
...
#if defined(J9VM_TRACE_VTABLE_ACCESS)
                        {
                            ...
                            J9UTF8* classNameUTF = J9ROMCLASS_CLASSNAME(romClass);
                            j9tty_printf(PORTLIB, "\noverriding @ index %d with %.*s.%.*s%.*s", superclassVTableIndex,
                                J9UTF8_LENGTH(classNameUTF), J9UTF8_DATA(classNameUTF), J9UTF8_LENGTH(nameUTF),
                                J9UTF8_DATA(nameUTF), J9UTF8_LENGTH(sigUTF), J9UTF8_DATA(sigUTF));
                        }
                    } else {
                        if (ROM_METHOD_ID_TAG != ((UDATA)storeValue & VTABLE_SLOT_TAG_MASK)) {
                            ...
                            J9Class* methodClass
                                = ((J9ConstantPool*)((UDATA)method->constantPool & ~J9_STARTPC_STATUS))->ramClass;
                            ...
                            j9tty_printf(PORTLIB, "\nnot overriding %.*s.%.*s%.*s in class %.*s - already overridden",
                                ...
                                J9UTF8_LENGTH(nameUTF), J9UTF8_DATA(nameUTF), J9UTF8_LENGTH(sigUTF),
                                J9UTF8_DATA(sigUTF), J9UTF8_LENGTH(classNameUTF), J9UTF8_DATA(classNameUTF));
                        }
#endif
                    }
                }
        ...
        if (newSlotRequired) {
...
#if defined(J9VM_TRACE_VTABLE_ACCESS)
            {
                ...
                j9tty_printf(PORTLIB, "\n<vtbl_init: adding @ index=%d %.*s.%.*s%.*s (0x%x)>", vTableMethodCount,
                    J9UTF8_LENGTH(classNameUTF), J9UTF8_DATA(classNameUTF), J9UTF8_LENGTH(nameUTF),
                    J9UTF8_DATA(nameUTF), J9UTF8_LENGTH(sigUTF), J9UTF8_DATA(sigUTF), romMethod);
            }
#endif
        }
    }
...
static UDATA getVTableIndexForNameAndSigStartingAt(UDATA* vTable, J9UTF8* name, J9UTF8* signature, UDATA vTableIndex)
{
    ...
    while (vTableIndex != 0) {
        ...
        J9UTF8* nameUTF = J9ROMMETHOD_NAME(romMethod);
        J9UTF8* sigUTF = J9ROMMETHOD_SIGNATURE(romMethod);
        if ((nameLength == J9UTF8_LENGTH(nameUTF)) && (signatureLength == J9UTF8_LENGTH(sigUTF))
...
getVTableOffsetForMethod(J9Method* method, J9Class* clazz, J9VMThread* vmThread)
{
    ...
    if ((modifiers & J9_JAVA_INTERFACE) == J9_JAVA_INTERFACE) {
        ...
        if (vTableSize == 0) {
            ...
            vTableIndex = getVTableIndexForNameAndSigStartingAt(
                (UDATA*)J9VTABLE_FROM_HEADER(vTable), nameUTF, sigUTF, vTableSize);
...
static void setCurrentExceptionForBadClass(J9VMThread* vmThread, J9UTF8* badClassName, UDATA exceptionIndex)
{
    ...
    j9object_t detailMessage = gcFuncs->j9gc_createJavaLangString(
        vmThread, J9UTF8_DATA(badClassName), J9UTF8_LENGTH(badClassName), J9_STR_XLAT);
...
static BOOLEAN verifyClassLoadingStack(J9VMThread* vmThread, J9ClassLoader* classLoader, J9ROMClass* romClass)
{
    ...
    while (currentElement != NULL) {
        ...
        if (currentElement->classLoader == classLoader) {
            ...
            if (compareUTF8Length(J9UTF8_DATA(currentRomName), J9UTF8_LENGTH(currentRomName), J9UTF8_DATA(className),
    ...
    if (superclassName == NULL) {
        ...
        *superclassOut = superclass;
        ...
        if (!hotswapping) {
            ...
            if (compareUTF8Length(J9UTF8_DATA(superclassName), J9UTF8_LENGTH(superclassName), J9UTF8_DATA(className),
                    J9UTF8_LENGTH(className))
                == 0) {
                setCurrentException(vmThread, J9VMCONSTANTPOOL_JAVALANGCLASSCIRCULARITYERROR, NULL);
                return FALSE;
        ...
        superclass = internalFindClassUTF8(
            vmThread, J9UTF8_DATA(superclassName), J9UTF8_LENGTH(superclassName), classLoader, classPreloadFlags);
        *superclassOut = superclass;
        Trc_VM_CreateRAMClassFromROMClass_loadedSuperclass(
            vmThread, J9UTF8_LENGTH(superclassName), J9UTF8_DATA(superclassName), superclass);
        if (NULL == superclass) {
            return FALSE;
        }
        ...
        if (!hotswapping) {
            ...
            if ((superclass->romClass->modifiers & J9_JAVA_INTERFACE) != 0) {
                ...
                setCurrentExceptionForBadClass(
                    vmThread, superclassName, J9VMCONSTANTPOOL_JAVALANGINCOMPATIBLECLASSCHANGEERROR);
                return FALSE;
            }
            ...
            if (!isROMClassUnsafe) {
                ...
                if ((!superClassIsPublic && (packageID != superclass->packageID))
                    || (superClassIsPublic
                           && (J9_VISIBILITY_ALLOWED
                                  != checkModuleAccess(vmThread, vm, romClass, module, superclass->romClass,
                                         superclass->module, superclass->packageID, 0)))) {
                    Trc_VM_CreateRAMClassFromROMClass_superclassNotVisible(
                        vmThread, superclass, superclass->classLoader, classLoader);
                    setCurrentExceptionForBadClass(
                        vmThread, superclassName, J9VMCONSTANTPOOL_JAVALANGILLEGALACCESSERROR);
                    return FALSE;
                }
            }
            ...
            if (romClass->interfaceCount != 0) {
                ...
                for (i = 0; i < romClass->interfaceCount; i++) {
                    J9UTF8* interfaceName = NNSRP_GET(interfaceNames[i], J9UTF8*);
                    J9Class* interfaceClass = internalFindClassUTF8(vmThread, J9UTF8_DATA(interfaceName),
                        J9UTF8_LENGTH(interfaceName), classLoader, classPreloadFlags);

                    Trc_VM_CreateRAMClassFromROMClass_loadedInterface(
                        vmThread, J9UTF8_LENGTH(interfaceName), J9UTF8_DATA(interfaceName), interfaceClass);
                    ...
                    if ((interfaceClass->romClass->modifiers & J9_JAVA_INTERFACE) != J9_JAVA_INTERFACE) {
                        ...
                        setCurrentExceptionForBadClass(vmThread, J9ROMCLASS_CLASSNAME(interfaceClass->romClass),
                    ...
                    if (!isROMClassUnsafe) {
                         ...
                         */
                        bool interfaceIsPublic
                            = J9_ARE_ALL_BITS_SET(interfaceClass->romClass->modifiers, J9_JAVA_PUBLIC);
                        if ((!interfaceIsPublic && (packageID != interfaceClass->packageID))
                            || (interfaceIsPublic
                                   && (J9_VISIBILITY_ALLOWED
                                          != checkModuleAccess(vmThread, vm, romClass, module, interfaceClass->romClass,
                                                 interfaceClass->module, interfaceClass->packageID, 0)))) {
                            Trc_VM_CreateRAMClassFromROMClass_interfaceNotVisible(
                                vmThread, interfaceClass, interfaceClass->classLoader, classLoader);
                            setCurrentExceptionForBadClass(vmThread, J9ROMCLASS_CLASSNAME(interfaceClass->romClass),
 ...
 * appropriate Java error on the VM.
 */
static BOOLEAN loadFlattenableFieldValueClasses(J9VMThread* currentThread, J9ClassLoader* classLoader,
    J9ROMClass* romClass, UDATA classPreloadFlags, UDATA packageID, J9Module* module)
{
    ...
    while (NULL != field) {
        ...
        if (('Q' == signatureChars[0]) && J9_ARE_NO_BITS_SET(modifiers, J9AccStatic)) {
            J9Class* valueClass = internalFindClassUTF8(
                currentThread, signatureChars + 1, J9UTF8_LENGTH(signature) - 2, classLoader, classPreloadFlags);
            if (NULL == valueClass) {
                result = FALSE;
                goto done;
            } else {
                ...
                if (J9_ARE_NO_BITS_SET(valueROMClass->modifiers, J9AccValueType)) {
                    ...
                    setCurrentExceptionNLSWithArgs(currentThread, J9NLS_VM_ERROR_QTYPE_NOT_VALUE_TYPE,
                        J9VMCONSTANTPOOL_JAVALANGINCOMPATIBLECLASSCHANGEERROR, J9UTF8_LENGTH(badClass),
                        J9UTF8_DATA(badClass));
                    result = FALSE;
                    goto done;
                }
                ...
                if ((!classIsPublic && (packageID != valueClass->packageID))
                    || (classIsPublic
                           && (J9_VISIBILITY_ALLOWED
                                  != checkModuleAccess(currentThread, currentThread->javaVM, romClass, module,
                                         valueROMClass, valueClass->module, valueClass->packageID, 0)))) {
                    Trc_VM_CreateRAMClassFromROMClass_nestedValueClassNotVisible(
                        currentThread, valueClass, valueClass->classLoader, classLoader);
                    setCurrentExceptionForBadClass(currentThread, J9ROMCLASS_CLASSNAME(valueROMClass),
                        J9VMCONSTANTPOOL_JAVALANGILLEGALACCESSERROR);
                    result = FALSE;
                    goto done;
                }
    ...
    UDATA options, J9Class* elementClass, J9UTF8* className, J9CreateRAMClassState* state)
{
    ...
    if (state->ramClass != NULL) {
        ...
        if (failed) {
            ...
            javaVM->memoryManagerFunctions->j9gc_modron_global_collect_with_overrides(
                vmThread, J9MMCONSTANT_EXPLICIT_GC_NATIVE_OUT_OF_MEMORY);
            ...
            alreadyLoaded:
                ...
                J9STATIC_OBJECT_STORE(vmThread, state->ramClass, (j9object_t*)&state->ramClass->classObject,
                    (j9object_t)alreadyLoadedClass->classObject);
        ...
        if (state->classObject != NULL) {
            ...
            J9STATIC_OBJECT_STORE(
                vmThread, state->ramClass, (j9object_t*)&state->ramClass->classObject, (j9object_t)state->classObject);
        ...
        if ((!fastHCR) && (0 == J9ROMCLASS_IS_PRIMITIVE_OR_ARRAY(romClass))
            && J9_ARE_NO_BITS_SET(options, J9_FINDCLASS_FLAG_ANON)) {
            if (hashClassTableAtPut(
                    vmThread, classLoader, J9UTF8_DATA(className), J9UTF8_LENGTH(className), state->ramClass)) {
                if (hotswapping) {
                    omrthread_monitor_exit(javaVM->classTableMutex);
                    state->ramClass = NULL;
                    return internalCreateRAMClassDoneNoMutex(vmThread, romClass, options, state);
                ...
                if (elementClass == NULL) {
                    alreadyLoadedClass
                        = hashClassTableAt(classLoader, J9UTF8_DATA(className), J9UTF8_LENGTH(className));
                } else {
                    alreadyLoadedClass = elementClass->arrayClass;
                }
                if (alreadyLoadedClass != NULL) {
            ...
            if (J9ROMCLASS_IS_ARRAY(romClass)) {
                 ...
                 */
                javaVM->memoryManagerFunctions->j9gc_objaccess_postStoreClassToClassLoader(
                    vmThread, classLoader, state->ramClass);
            }
        }
    }
...
static void checkForCustomSpinOptions(void* element, void* userData)
{
    ...
    J9UTF8* className = J9ROMCLASS_CLASSNAME(ramClass->romClass);
    if (J9UTF8_DATA_EQUALS(
            J9UTF8_DATA(className), J9UTF8_LENGTH(className), option->className, strlen(option->className))) {
...
#undef UNNAMED_NAMED_MODULE
        ...
        moduleNameUTF = vmFuncs->copyStringToUTF8WithMemAlloc(vmThread, ramClass->module->moduleName,
    ...
    if (NULL != classLoaderName) {
        classLoaderNameUTF = vmFuncs->copyStringToUTF8WithMemAlloc(vmThread, classLoaderName,
            J9_STR_NULL_TERMINATE_RESULT, "", 0, classLoaderNameBuf, J9VM_PACKAGE_NAME_BUFFER_LENGTH, NULL);
    } else {
...
#undef UNNAMED_NAMED_MODULE
    ...
    if ((NULL != classLoaderNameUTF) && (NULL != moduleNameUTF)) {
        Trc_MODULE_setting_package(
            vmThread, J9UTF8_LENGTH(className), J9UTF8_DATA(className), classLoaderNameUTF, moduleNameUTF);
    ...
    J9ClassLoader* hostClassLoader, J9Class* hostClass, J9Module* module)
{
    ...
    if (!hotswapping) {
        ...
        if (jlClass != NULL) {
            ...
            /* Note: in RTJ, tenured implies immortal. Classes must be in the immortal space so we must set the tenured
             * flag. */
            state->classObject = javaVM->memoryManagerFunctions->J9AllocateObject(vmThread, jlClass,
                J9_GC_ALLOCATE_OBJECT_TENURED | J9_GC_ALLOCATE_OBJECT_NON_INSTRUMENTABLE
                    | J9_GC_ALLOCATE_OBJECT_HASHED);
            if (state->classObject == NULL) {
            ...
            /* If the lock class is not yet loaded, skip allocating the instance - will be fixed up once the class is
             * loaded */
            if (lockClass != NULL) {
                ...
                J9VMJAVALANGJ9VMINTERNALSCLASSINITIALIZATIONLOCK_SET_THECLASS(
                    vmThread, lockObject, (j9object_t)state->classObject);
                J9VMJAVALANGCLASS_SET_INITIALIZATIONLOCK(vmThread, (j9object_t)state->classObject, lockObject);
            }
        }
    }

    /* Now that all required classes are loaded, reacquire the classTableMutex and see if the new class has appeared in
     * the table. If so, return that one.  If not, create the new class and put it in the class table.
     */
    omrthread_monitor_enter(javaVM->classTableMutex);
    /* computeRAMSizeForROMClass */
    {
        ...
        if (fastHCR) {
            ...
            interfaceHead = markInterfaces(romClass, superclass, hostClassLoader, &foundCloneable, &interfaceCount,
                &inheritedInterfaceCount, &maxInterfaceDepth);
            /* Compute the number of slots required for the interpreter and jit (if enabled) vTables. */
            vTable = computeVTable(vmThread, hostClassLoader, superclass, romClass, packageID, methodRemapArray,
                interfaceHead, &defaultConflictCount, interfaceCount, inheritedInterfaceCount, &errorData);
            if (vTable == NULL) {
                unmarkInterfaces(interfaceHead);
                popFromClassLoadingStack(vmThread);
                omrthread_monitor_exit(javaVM->classTableMutex);
                if (NULL != errorData.loader1) {
                    ...
                    setClassLoadingConstraintOverrideError(vmThread, J9ROMCLASS_CLASSNAME(romClass), errorData.loader1,
                        errorData.class1NameUTF, errorData.loader2, errorData.class2NameUTF,
                        errorData.exceptionClassNameUTF, J9UTF8_DATA(methodNameUTF), J9UTF8_LENGTH(methodNameUTF),
                        J9UTF8_DATA(methodSigUTF), J9UTF8_LENGTH(methodSigUTF));
                } else {
                    setNativeOutOfMemoryError(vmThread, 0, 0);
                }
                return internalCreateRAMClassDoneNoMutex(vmThread, romClass, options, state);
    ...
    if ((javaVM->runtimeFlags & J9_RUNTIME_VERIFY) && (badMethod != NULL)) {
        ...
        Trc_VM_CreateRAMClassFromROMClass_overriddenFinalMethod(
            vmThread, J9UTF8_LENGTH(badName), J9UTF8_DATA(badName), J9UTF8_LENGTH(badSig), J9UTF8_DATA(badSig));
    ...
    if (classSize != 0) {
        if (ramClass == NULL) {
            ...
            allocationRequests[RAM_METHODS_FRAGMENT].alignedSize
                = (romClass->romMethodCount + defaultConflictCount) * sizeof(J9Method);
            ...
            allocationRequests[RAM_SUPERCLASSES_FRAGMENT].alignedSize
                = OMR_MAX(superclassSizeBytes, minimumSuperclassArraySizeBytes);
            ...
            allocationRequests[RAM_INSTANCE_DESCRIPTION_FRAGMENT].alignedSize
                = instanceDescriptionSlotCount * sizeof(UDATA);
            ...
            allocationRequests[RAM_CONSTANT_POOL_FRAGMENT].alignedSize
                = romClass->ramConstantPoolCount * 2 * sizeof(UDATA);
            ...
            allocationRequests[RAM_VARHANDLE_METHOD_TYPES_FRAGMENT].alignedSize
                = romClass->varHandleMethodTypeCount * sizeof(j9object_t);
            ...
            allocationRequests[RAM_STATIC_SPLIT_TABLE_FRAGMENT].alignedSize
                = romClass->staticSplitMethodRefCount * sizeof(J9Method*);
            ...
            allocationRequests[RAM_SPECIAL_SPLIT_TABLE_FRAGMENT].alignedSize
                = romClass->specialSplitMethodRefCount * sizeof(J9Method*);
            allocationRequests[RAM_SPECIAL_SPLIT_TABLE_FRAGMENT].address = NULL;

            if (fastHCR) {
                /* For shared fragments, set alignedSize and prefixSize to 0 to make internalAllocateRAMClass() ignore
                 * them. */
                ...
                allocationRequests[RAM_INSTANCE_DESCRIPTION_FRAGMENT].address
                    = classBeingRedefined->instanceDescription;
                allocationRequests[RAM_INSTANCE_DESCRIPTION_FRAGMENT].prefixSize = 0;
            ...
            if (NULL != segment) {
                ...
                *(J9Class**)segment->heapBase = ramClass;
                ...
                ramClass->varHandleMethodTypes
                    = (j9object_t*)allocationRequests[RAM_VARHANDLE_METHOD_TYPES_FRAGMENT].address;
                ramClass->staticSplitMethodTable
                    = (J9Method**)allocationRequests[RAM_STATIC_SPLIT_TABLE_FRAGMENT].address;
                ...
                ramClass->specialSplitMethodTable
                    = (J9Method**)allocationRequests[RAM_SPECIAL_SPLIT_TABLE_FRAGMENT].address;
        ...
        /* initialize RAM Class */
        {
            ...
            if (romClass->romMethodCount != 0) {
                ...
                for (i = 0; i < methodCount; i++) {
                    ...
                    Trc_VM_internalCreateRAMClassFromROMClass_createRAMMethod(vmThread, J9UTF8_LENGTH(className),
                        J9UTF8_DATA(className), J9UTF8_LENGTH(methodName), J9UTF8_DATA(methodName),
                        J9UTF8_LENGTH(methodSig), J9UTF8_DATA(methodSig), currentRAMMethod);
            ...
            if (superclass == NULL) {
                ...
                *ramClass->superclasses++ = superclass;
                ...
                tempClassDepthAndFlags &= ~(J9_JAVA_CLASS_HOT_SWAPPED_OUT | J9_JAVA_CLASS_HAS_BEEN_OVERRIDDEN
                    | J9_JAVA_CLASS_HAS_JDBC_NATIVES | J9_JAVA_CLASS_RAM_ARRAY
                    | (OBJECT_HEADER_SHAPE_MASK << J9_JAVA_CLASS_RAM_SHAPE_SHIFT));
                tempClassDepthAndFlags++;

#if defined(J9VM_GC_FINALIZATION)
                /* if this class has an empty finalize() method, make sure not to inherit javaFlagsClassFinalizeNeeded
                 * from the superclass */
            ...
            tempClassDepthAndFlags
                |= ((romClass->instanceShape & OBJECT_HEADER_SHAPE_MASK) << J9_JAVA_CLASS_RAM_SHAPE_SHIFT);
            if (J9ROMCLASS_IS_ARRAY(romClass)) {
            ...
            if (!fastHCR) {
                ...
                calculateInstanceDescription(
                    vmThread, ramClass, superclass, instanceDescription, &romWalkState, romWalkResult);
            }
            ...
            if (J9CLASS_FLAGS(ramClass) & J9_JAVA_CLASS_CLONEABLE) {
                ...
                for (i = 0; i < sizeof(uncloneableClasses) / sizeof(UDATA); i++) {
                    ...
                    if (uncloneableClass != NULL) {
                        ...
                        if ((currentClassDepth > uncloneableClassDepth)
                            && (ramClass->superclasses[uncloneableClassDepth]) == uncloneableClass) {
                            ramClass->classDepthAndFlags &= ~J9_JAVA_CLASS_CLONEABLE;
                            break;
                        }
                    }
            ...
            Trc_VM_initializeRAMClass_End(
                vmThread, J9UTF8_LENGTH(className), J9UTF8_DATA(className), classSize, classLoader);
        }
        ...
        if (FALSE == J9ROMCLASS_IS_PRIMITIVE_OR_ARRAY(romClass)) {
            ...
            if (NULL != javaVM->customSpinOptions) {
                ...
                if (NULL != option) {
                    ...
                    Trc_VM_CreateRAMClassFromROMClass_CustomSpinOption(option->className,
                        j9monitorOptions->thrMaxSpins1BeforeBlocking, j9monitorOptions->thrMaxSpins2BeforeBlocking,
                    ...
                    Trc_VM_CreateRAMClassFromROMClass_CustomSpinOption2(option->className,
                        j9threadOptions->customThreeTierSpinCount1, j9threadOptions->customThreeTierSpinCount2,
                    ...
                    Trc_VM_CreateRAMClassFromROMClass_CustomSpinOption3(
                        option->className, j9threadOptions->customAdaptSpin);
                ...
                if (existingClass != NULL) {
                    ...
                    if (hotswapping) {
                        return internalCreateRAMClassDone(
                            vmThread, classLoader, romClass, options, elementClass, className, state);
                    }
                    popFromClassLoadingStack(vmThread);
                    omrthread_monitor_exit(javaVM->classTableMutex);
            ...
            if (classLoader == javaVM->systemClassLoader) {
                /* We don't add class location entries with locationType, LOAD_LOCATION_UNKNOWN, in the
                 * classLocationHashTable. This should save footprint. Same applies for classes created with
                 * Unsafe.defineclass or Unsafe.defineAnonClass.
                 */
                if ((NULL == classBeingRedefined) && (LOAD_LOCATION_UNKNOWN != locationType)
                    && (!J9ROMCLASS_IS_UNSAFE(romClass))) {
...
retry:
    ...
    if (J2SE_VERSION(javaVM) >= J2SE_19) {
        if (NULL == classBeingRedefined) {
            if (J9ROMCLASS_IS_ARRAY(romClass)) {
                ...
                /* Ignore locationType and assign all classes created before the java.base module is created to
                 * java.base. This matches the RI implementation. Validated on JDK9 through JDK11.
                 */
    ...
    if (!loadSuperClassAndInterfaces(vmThread, hostClassLoader, romClass, elementClass, packageID, hotswapping,
            classPreloadFlags, &superclass, module)
#if defined(J9VM_OPT_VALHALLA_VALUE_TYPES)
    ...
    result = internalCreateRAMClassFromROMClassImpl(vmThread, classLoader, romClass, options, elementClass,
        methodRemapArray, entryIndex, locationType, classBeingRedefined, packageID, superclass, &state, hostClassLoader,
        hostClass, module);
    if (state.retry) {
        goto retry;
    }
...
static VMINLINE void addBlockToLargeFreeList(J9ClassLoader* classLoader, J9RAMClassFreeListLargeBlock* block)
{
...
static void addBlockToFreeList(J9ClassLoader* classLoader, UDATA address, UDATA size)
{
 ...
 */
static void removeBlockFromLargeFreeList(J9ClassLoader* classLoader, J9RAMClassFreeListLargeBlock** freeListBlockPtr,
    J9RAMClassFreeListLargeBlock* freeListBlock)
{
 ...
 */
static BOOLEAN allocateRAMClassFragmentFromFreeList(
    RAMClassAllocationRequest* request, J9RAMClassFreeListBlock** freeList, J9ClassLoader* classLoader)
{
    ...
    while (NULL != freeListBlock) {
        ...
        if (freeListBlock->size >= alignmentShift + fragmentSize) {
            ...
            Trc_VM_internalAllocateRAMClass_AllocatedFromFreeList(request->index, freeListBlock, freeListBlock->size,
                request->address, request->prefixSize, request->alignedSize, request->alignment);

            if (islargeBlocksList) {
                removeBlockFromLargeFreeList(classLoader, (J9RAMClassFreeListLargeBlock**)freeListBlockPtr,
                    (J9RAMClassFreeListLargeBlock*)freeListBlock);
            } else {
                *freeListBlockPtr = freeListBlock->nextFreeListBlock;
                freeListBlock->nextFreeListBlock = NULL;
            ...
            if (0 != newBlockSize) {
                addBlockToFreeList(
                    classLoader, ((UDATA)freeListBlock) + alignmentShift + request->fragmentSize, newBlockSize);
            }
    ...
    RAMClassAllocationRequest* allocationRequests, UDATA allocationRequestCount)
{
        ...
        for (request = requests; NULL != request; request = request->next) {
            if ((sizeof(UDATA) == request->fragmentSize) && (NULL != classLoader->ramClassUDATABlockFreeList)) {
                ...
                if (NULL != request->address) {
                    ...
                    Trc_VM_internalAllocateRAMClass_AllocatedFromFreeList(request->index, block, sizeof(UDATA),
                        request->address, request->prefixSize, request->alignedSize, request->alignment);
            ...
            if ((RAM_CLASS_SMALL_FRAGMENT_LIMIT > request->fragmentSize)
                && (NULL != classLoader->ramClassTinyBlockFreeList)) {
                if (allocateRAMClassFragmentFromFreeList(
                        request, &classLoader->ramClassTinyBlockFreeList, classLoader)) {
                    prev->next = request->next;
                    continue;
                }
            }
            /* Avoid scanning the small free block list to allocate RAM class headers. The alignment constraint will
             * rarely be satisfied. */
            if ((RAM_CLASS_FRAGMENT_LIMIT > request->fragmentSize + request->alignment)
                && (NULL != classLoader->ramClassSmallBlockFreeList)) {
                if (allocateRAMClassFragmentFromFreeList(
                        request, &classLoader->ramClassSmallBlockFreeList, classLoader)) {
                    prev->next = request->next;
                    continue;
                }
            }
            if (NULL != classLoader->ramClassLargeBlockFreeList) {
                if (allocateRAMClassFragmentFromFreeList(
                        request, &classLoader->ramClassLargeBlockFreeList, classLoader)) {
                    prev->next = request->next;
                    continue;
                }
            }
    ...
    if (NULL != requests) {
        ...
        Trc_VM_internalAllocateRAMClass_AllocateClassMemorySegment(
            fragmentsLeftToAllocate, newSegmentSize, classAllocationIncrement);
        newSegment = allocateClassMemorySegment(
            javaVM, newSegmentSize, MEMORY_TYPE_RAM_CLASS, classLoader, classAllocationIncrement);
        ...
        *(J9Class**)newSegment->heapBase = NULL;
        ...
        for (request = requests; NULL != request; request = request->next) {
            ...
            Trc_VM_internalAllocateRAMClass_AllocatedFromNewSegment(request->index, newSegment, request->address,
    ...
    for (i = 0; i < allocationRequestCount; i++) {
        if (NULL != allocationRequests[i].address) {
            memset((void*)(((UDATA)allocationRequests[i].address) - allocationRequests[i].prefixSize), 0,
                allocationRequests[i].fragmentSize);
        }
    }
