...
IDATA shutdownDLL(J9JavaVM* vm, UDATA descriptor, UDATA shutdownDueToExit)
{
    ...
    if (0 == j9sl_lookup_name(descriptor, "JVM_OnUnload", (void*)&j9OnUnLoadFunc, "iLL")) {
        ...
        if ((j9OnUnLoadFunc)((J9JavaVM*)vm, (void*)shutdownDueToExit))
            return -2;
    }
...
UDATA loadJ9DLLWithPath(J9JavaVM* vm, J9VMDllLoadInfo* info, char* dllName)
{
    ...
    UDATA openFlags
        = (info->loadFlags & XRUN_LIBRARY) ? J9PORT_SLOPEN_DECORATE | J9PORT_SLOPEN_LAZY : J9PORT_SLOPEN_DECORATE;
    ...
    if (NULL != dllDirectory) {
        ...
        if ((info->loadFlags & XRUN_LIBRARY) && (J2SE_LAYOUT(vm) & J2SE_LAYOUT_VM_IN_SUBDIR)) {
            ...
            bufferSize = j9str_printf(PORTLIB, NULL, 0, "%s%s%s", dllDirectory, DIR_SEPARATOR_STR, dllName);
        }
        ...
        if (superSeparatorIndex
            != -1) { /* will be set if we need to clip right after parent dir - do NOT strcpy dirDirectory as it might
                        be longer than the DLL name, resulting in a buffer overflow */
            memcpy(localBuffer, dllDirectory, superSeparatorIndex + 1);
            localBuffer[superSeparatorIndex + 1] = (char)0;
            strcat(localBuffer, dllName);
        } else {
            j9str_printf(PORTLIB, localBuffer, bufferSize, "%s%s%s", dllDirectory, DIR_SEPARATOR_STR, dllName);
        }
    } else {
        localBuffer = dllName;
    }
