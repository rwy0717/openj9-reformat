...
typedef UDATA (*callback_func_t)(J9VMThread* vmThread, void* userData, J9ROMClass* romClass, J9ROMMethod* romMethod,
...
static void printExceptionInThread(J9VMThread* vmThread)
{
    ...
    format = j9nls_lookup_message(J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE,
...
static void printExceptionMessage(J9VMThread* vmThread, j9object_t exception)
{
    ...
    if (NULL != detailMessage) {
        buf = copyStringToUTF8WithMemAlloc(
            vmThread, detailMessage, J9_STR_NULL_TERMINATE_RESULT, "", 0, stackBuffer, 256, &length);
    ...
    j9tty_err_printf(PORTLIB, "%.*s%s%.*s\n", (UDATA)J9UTF8_LENGTH(exceptionClassName), J9UTF8_DATA(exceptionClassName),
        separator, length, buf);
...
/* assumes VM access */
static UDATA printStackTraceEntry(J9VMThread* vmThread, void* voidUserData, J9ROMClass* romClass,
    J9ROMMethod* romMethod, J9UTF8* sourceFile, UDATA lineNumber, J9ClassLoader* classLoader)
{
    ...
    if (romMethod == NULL) {
        format = j9nls_lookup_message(J9NLS_INFO | J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_VM_STACK_TRACE_UNKNOWN, NULL);
        ...
        if (j2seVersion >= J2SE_19) {
            ...
            if (J9_ARE_ALL_BITS_SET(vm->runtimeFlags, J9_RUNTIME_JAVA_BASE_MODULE_CREATED)) {
                ...
                if ((NULL != module) && (module != vm->javaBaseModule)) {
                    moduleNameUTF = copyStringToUTF8WithMemAlloc(vmThread, module->moduleName,
                        J9_STR_NULL_TERMINATE_RESULT, "", 0, nameBuf, J9VM_PACKAGE_NAME_BUFFER_LENGTH, NULL);
                    if (nameBuf != moduleNameUTF) {
                        freeModuleName = TRUE;
                    }
                    if (NULL != moduleNameUTF) {
                        moduleVersionUTF = copyStringToUTF8WithMemAlloc(vmThread, module->version,
                            J9_STR_NULL_TERMINATE_RESULT, "", 0, versionBuf, J9VM_PACKAGE_NAME_BUFFER_LENGTH, NULL);
        ...
        if (NULL != moduleNameUTF) {
            if (NULL != moduleVersionUTF) {
                if (lineNumber) {
                    format = j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG,
                        J9NLS_VM_STACK_TRACE_WITH_MODULE_VERSION_LINE, "\tat %.*s.%.*s (%s@%s/%.*s:%u)\n");
                } else {
                    format = j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG,
                        J9NLS_VM_STACK_TRACE_WITH_MODULE_VERSION, "\tat %.*s.%.*s (%s@%s/%.*s)\n");
                }
                j9tty_err_printf(PORTLIB, (char*)format, (UDATA)J9UTF8_LENGTH(className), J9UTF8_DATA(className),
                    (UDATA)J9UTF8_LENGTH(methodName), J9UTF8_DATA(methodName), moduleNameUTF, moduleVersionUTF,
                ...
                if (lineNumber) {
                    format = j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_VM_STACK_TRACE_WITH_MODULE_LINE,
                        "\tat %.*s.%.*s (%s/%.*s:%u)\n");
                } else {
                    format = j9nls_lookup_message(
                        J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_VM_STACK_TRACE_WITH_MODULE, "\tat %.*s.%.*s (%s/%.*s)\n");
                }
                j9tty_err_printf(PORTLIB, (char*)format, (UDATA)J9UTF8_LENGTH(className), J9UTF8_DATA(className),
                    (UDATA)J9UTF8_LENGTH(methodName), J9UTF8_DATA(methodName), moduleNameUTF, sourceFileNameLen,
                    sourceFileName,
                    lineNumber); /* line number will be ignored in if it's not used in the format string */
            }
            ...
            if (lineNumber) {
                format = j9nls_lookup_message(
                    J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_VM_STACK_TRACE_WITH_LINE, "\tat %.*s.%.*s (%.*s:%u)\n");
            } else {
                format = j9nls_lookup_message(
                    J9NLS_DO_NOT_PRINT_MESSAGE_TAG, J9NLS_VM_STACK_TRACE, "\tat %.*s.%.*s (%.*s)\n");
            }
            j9tty_err_printf(PORTLIB, (char*)format, (UDATA)J9UTF8_LENGTH(className), J9UTF8_DATA(className),
                (UDATA)J9UTF8_LENGTH(methodName), J9UTF8_DATA(methodName), sourceFileNameLen, sourceFileName,
                lineNumber); /* line number will be ignored in if it's not used in the format string */
        }
    }
    ...
    */
    ...
    if (walkback) {
        ...
        while (currentElement != arraySize) {
            ...
            UDATA methodPC = J9JAVAARRAYOFUDATA_LOAD(
                vmThread, J9VMJAVALANGTHROWABLE_WALKBACK(vmThread, (*exception)), currentElement);
            ...
            if ((callback != NULL) || pruneConstructors) {
                ...
                inlinedEntry:
                    ...
                    if (inlineDepth == 0) {
                        if (inlineMap == NULL) {
                            ...
                            methodPC = jitConfig->getCurrentByteCodeIndexAndIsSameReceiver(
                                metaData, inlineMap, NULL, &isSameReceiver);
                        }
                        ramMethod = metaData->ramMethod;
                    } else {
                        methodPC = jitConfig->getCurrentByteCodeIndexAndIsSameReceiver(
                            metaData, inlineMap, inlinedCallSite, &isSameReceiver);
                ...
                if (romMethod != NULL) {
                    lineNumber
                        = getLineNumberForROMClassFromROMMethod(vm, romMethod, romClass, offset, classLoader, methodPC);
                ...
                if (callback != NULL) {
                    callbackResult
                        = callback(vmThread, userData, romClass, romMethod, fileName, lineNumber, classLoader);
                }
 ...
 */
static UDATA gpProtectedExceptionDescribe(void* entryArg)
{
...
{
    /* If the exception is NULL, do nothing.  Do not fetch the exception value into a local here as we do not have VM
     * access yet. */
    ...
    if (vmThread->currentException != NULL) {
        ...
        /* If the VM has completed initialization, try to print it in Java.  If initialization is not complete of the
         * java call fails, print the exception natively. */
        ...
        do {
            ...
            if (eiieClass == NULL) {
                eiieClass = internalFindKnownClass(vmThread, J9VMCONSTANTPOOL_JAVALANGEXCEPTIONININITIALIZERERROR,
