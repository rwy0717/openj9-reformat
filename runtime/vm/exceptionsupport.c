...
static void internalSetCurrentExceptionWithCause(
...
void setCurrentExceptionUTF(J9VMThread* vmThread, UDATA exceptionNumber, const char* detailUTF)
{
    ...
    if ((detailUTF != NULL) && (NULL != J9VMJAVALANGSTRING_OR_NULL(vmThread->javaVM))) {
        detailString = vmThread->javaVM->memoryManagerFunctions->j9gc_createJavaLangString(
            vmThread, (U_8*)detailUTF, (UDATA)strlen(detailUTF), 0);
    }
...
void setCurrentExceptionNLS(J9VMThread* vmThread, UDATA exceptionNumber, U_32 moduleName, U_32 messageNumber)
{
    ...
    msg = OMRPORT_FROM_J9PORT(PORTLIB)->nls_lookup_message(OMRPORT_FROM_J9PORT(PORTLIB),
        J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE, moduleName, messageNumber, NULL);
...
void setCurrentExceptionNLSWithArgs(J9VMThread* vmThread, U_32 nlsModule, U_32 nlsID, UDATA exceptionIndex, ...)
{
    ...
    nlsMsgFormat
        = omrnls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE, nlsModule, nlsID, "");
...
static UDATA decompStackHeadSearch(J9VMThread* currentThread, J9StackWalkState* walkState)
{
 ...
 * @param rewalkAllWalkedFrames  set to true in order to rewalk the same amount of frames as upto the frame of the
 *exceptionHandlerSearch that called us
 * @return 					none
 *
...
static void syncDecompilationStackAfterReleasingVMAccess(
    J9VMThread* currentThread, J9StackWalkState* walkState, UDATA rewalkAllWalkedFrames)
{
                ...
                if (ranges != 0) {
                    ...
                    do {
                        if ((deltaPC >= handlerCursor->startPC) && (deltaPC < handlerCursor->endPC)) {
                            if (isExceptionTypeCaughtByHandler(currentThread, walkState->userData4,
                                    walkState->constantPool, handlerCursor->exceptionClassIndex, walkState)
                                != 0) {
    ...
    if ((((UDATA)walkState->userData1) & J9_EXCEPT_SEARCH_NO_UNWIND) == 0) {
        ...
        if (IS_SPECIAL_FRAME_PC(walkState->pc)) {
            if (walkState->pc == (U_8*)J9SF_FRAME_TYPE_JNI_NATIVE_METHOD) {
...
#endif
                {
                    ...
                    if (traced || hooked) {
                        ...
                        if (traced) {
                            UTSI_TRACEMETHODEXIT_FROMVM(
                                vm, currentThread, walkState->method, walkState->restartException, NULL, 0);
                        }
                        if (hooked) {
                            ALWAYS_TRIGGER_J9HOOK_VM_NATIVE_METHOD_RETURN(
                                vm->hookInterface, currentThread, walkState->method, TRUE, NULL, NULL);
                        }
            ...
            if (traced || hooked) {
                ...
                currentThread->stackWalkState = &tempWalkState;
                if (traced) {
                    UTSI_TRACEMETHODEXIT_FROMVM(vm, currentThread, walkState->method, walkState->restartException, NULL,
                        HOOK_METHOD_TYPE(walkState));
                }
                if (hooked) {
                    ALWAYS_TRIGGER_J9HOOK_VM_METHOD_RETURN(
                        vm->hookInterface, currentThread, walkState->method, TRUE, NULL, HOOK_METHOD_TYPE(walkState));
                }
                currentThread->stackWalkState = tempWalkState.previous;
                walkState->restartException = POP_OBJECT_IN_SPECIAL_FRAME(currentThread);
    ...
    UDATA handlerIndex, J9StackWalkState* walkState)
{
    ...
    if (caughtExceptionClass == NULL) {
        ...
        tempWalkState.previous = currentThread->stackWalkState;
        currentThread->stackWalkState = &tempWalkState;
        caughtExceptionClass
            = resolveClassRef(currentThread, constantPool, handlerIndex, J9_RESOLVE_FLAG_RUNTIME_RESOLVE);
        currentThread->stackWalkState = tempWalkState.previous;
        walkState->restartException = POP_OBJECT_IN_SPECIAL_FRAME(currentThread);
    ...
    J9VMThread* currentThread, UDATA exceptionNumber, UDATA* detailMessage, const char* utfMessage, j9object_t cause)
{
     ...
     * OOM_STATE_2 = Already throwing OOM. Use cached OOM and call exception constructor
     * (internalSendExceptionConstructor) and set currentThread->privateFlags |=
     * J9_PRIVATE_FLAGS_FINAL_CALL_OUT_OF_MEMORY OOM_STATE_3 = Already throwing cached OOM. Skip calling the constructor
     ...
     */
    exceptionClass = internalFindKnownClass(
        currentThread, index, J9_FINDKNOWNCLASS_FLAG_INITIALIZE | J9_FINDKNOWNCLASS_FLAG_NON_FATAL);
...
sendConstructor:
    ...
    *((j9object_t*)(currentThread->sp)) = exception;
    internalSendExceptionConstructor(
        currentThread, exceptionClass, exception, (j9object_t)detailMessage, constructorIndex);
    currentThread->privateFlags &= ~J9_PRIVATE_FLAGS_FILL_EXISTING_TRACE;
 ...
 */
static UDATA gpProtectedSetCurrentExceptionWithCause(void* entryArg)
{
    ...
    internalSetCurrentExceptionWithCause(
        args->currentThread, args->exceptionNumber, args->detailMessage, args->utfMessage, args->cause);
...
void setClassCastException(J9VMThread* currentThread, J9Class* instanceClass, J9Class* castClass)
{
    ...
    setCurrentException(
        currentThread, J9_EX_CTOR_CLASS_CLASS + J9VMCONSTANTPOOL_JAVALANGCLASSCASTEXCEPTION, (UDATA*)&detailMessage);
}
...
void setClassLoadingConstraintError(J9VMThread* currentThread, J9ClassLoader* initiatingLoader, J9Class* existingClass)
{
    ...
    const char* nlsMessage = j9nls_lookup_message(
        J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE, J9NLS_VM_LOADING_CONSTRAINT_VIOLATION, NULL);
    if (nlsMessage != NULL) {
        ...
        UDATA msgLen = j9str_printf(PORTLIB, NULL, 0, nlsMessage, initiatingLoaderClassNameLength,
            initiatingLoaderClassName, initiatingLoaderHash, existingClassNameLength, existingClassName,
        ...
        j9str_printf(PORTLIB, msg, msgLen, nlsMessage, initiatingLoaderClassNameLength, initiatingLoaderClassName,
            initiatingLoaderHash, existingClassNameLength, existingClassName, definingLoaderClassNameLength,
            definingLoaderClassName, definingLoaderHash);
    }
    ...
    U_8* signature, UDATA signatureLength)
{
    ...
    char* msg = NULL;
    const char* nlsMessage = j9nls_lookup_message(
        J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE, J9NLS_VM_LOADING_CONSTRAINT_SIG_VIOLATION, NULL);
    if (nlsMessage != NULL) {
        ...
        UDATA msgLen = j9str_printf(PORTLIB, NULL, 0, nlsMessage, exceptionClassNameLength, exceptionClassName,
            methodNameLength, methodName, signatureLength, signature, loader1ClassNameLength, loader1ClassName,
            loader1Hash, class1ClassNameLength, class1ClassName, loader2ClassNameLength, loader2ClassName, loader2Hash,
            class2ClassNameLength, class2ClassName

        );
        msg = j9mem_allocate_memory(msgLen, OMRMEM_CATEGORY_VM);
        /* msg NULL check omitted since str_printf accepts NULL (as above) */
        j9str_printf(PORTLIB, msg, msgLen, nlsMessage, exceptionClassNameLength, exceptionClassName, methodNameLength,
            methodName, signatureLength, signature, loader1ClassNameLength, loader1ClassName, loader1Hash,
            class1ClassNameLength, class1ClassName, loader2ClassNameLength, loader2ClassName, loader2Hash,
            class2ClassNameLength, class2ClassName);
    }
    ...
    U_8* methodName, UDATA methodNameLength, U_8* signature, UDATA signatureLength)
{
    ...
    const char* nlsMessage = j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE,
        J9NLS_VM_LOADING_CONSTRAINT_OVERRIDE_VIOLATION, NULL);
    if (nlsMessage != NULL) {
        /* Loader1 name and length */
        j9object_t loader1Object = loader1->classLoaderObject;
        J9Class* loader1Class = J9OBJECT_CLAZZ(currentThread, loader1Object);
        ...
        UDATA msgLen = j9str_printf(PORTLIB, NULL, 0, nlsMessage, exceptionClassNameLength, exceptionClassName,
            methodNameLength, methodName, signatureLength, signature, loader1ClassNameLength, loader1ClassName,
            loader1Hash, class1ClassNameLength, class1ClassName, loader2ClassNameLength, loader2ClassName, loader2Hash,
            class2ClassNameLength, class2ClassName, newClassNameLength, newClassName
...
static char* nlsMessageForMethod(J9VMThread* currentThread, J9Method* method, U_32 module_name, U_32 message_num)
{
    PORT_ACCESS_FROM_VMC(currentThread);
    char* msg = NULL;
    const char* nlsMessage = OMRPORT_FROM_J9PORT(PORTLIB)->nls_lookup_message(OMRPORT_FROM_J9PORT(PORTLIB),
        J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE, module_name, message_num, NULL);
    if (nlsMessage != NULL) {
        ...
        UDATA msgLen = j9str_printf(PORTLIB, NULL, 0, nlsMessage, classNameLength, className, methodNameLength,
            methodName, methodSignatureLength, methodSignature);
        msg = j9mem_allocate_memory(msgLen, OMRMEM_CATEGORY_VM);
        /* msg NULL check omitted since str_printf accepts NULL (as above) */
        j9str_printf(PORTLIB, msg, msgLen, nlsMessage, classNameLength, className, methodNameLength, methodName,
...
void setNativeBindOutOfMemoryError(J9VMThread* currentThread, J9Method* method)
{
    ...
    setCurrentExceptionUTF(currentThread, J9_EX_OOM_OS_HEAP | J9VMCONSTANTPOOL_JAVALANGOUTOFMEMORYERROR,
        msg == NULL ? NATIVE_MEMORY_OOM_MSG : msg);
    j9mem_free_memory(msg);
}
...
{
    setCurrentException(
        currentThread, J9VMCONSTANTPOOL_JAVALANGUNSATISFIEDLINKERROR, (UDATA*)methodToString(currentThread, method));
}
...
void setHeapOutOfMemoryError(J9VMThread* currentThread)
{
    ...
    const char* nlsMessage = j9nls_lookup_message(
        J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE, J9NLS_VM_HEAP_OUT_OF_MEMORY, "Java heap space");

    setCurrentExceptionWithUtfCause(currentThread, J9VMCONSTANTPOOL_JAVALANGOUTOFMEMORYERROR,
        (UDATA*)((NULL == heapOOMStringRef) ? NULL : *heapOOMStringRef), nlsMessage, NULL);
}

void setArrayIndexOutOfBoundsException(J9VMThread* currentThread, IDATA index)
{
    setCurrentException(
        currentThread, J9_EX_CTOR_INT + J9VMCONSTANTPOOL_JAVALANGARRAYINDEXOUTOFBOUNDSEXCEPTION, (UDATA*)(UDATA)index);
}
...
void setNativeOutOfMemoryError(J9VMThread* vmThread, U_32 moduleName, U_32 messageNumber)
{
    ...
    msg = OMRPORT_FROM_J9PORT(PORTLIB)->nls_lookup_message(OMRPORT_FROM_J9PORT(PORTLIB),
        J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE, moduleName, messageNumber, NATIVE_MEMORY_OOM_MSG);
...
void setIncompatibleClassChangeErrorForDefaultConflict(J9VMThread* vmThread, J9Method* method)
{
    ...
    const char* nlsMessage = j9nls_lookup_message(
        J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE, J9NLS_VM_DEFAULT_METHOD_CONFLICT, NULL);
    if (nlsMessage != NULL) {
...
void setIllegalAccessErrorNonPublicInvokeInterface(J9VMThread* vmThread, J9Method* method)
{
     ...
     */
    const char* nlsMessage = j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE,
        J9NLS_VM_INVOKEINTERFACE_OF_NONPUBLIC_METHOD, NULL);
    if (nlsMessage != NULL) {
...
jint initializeHeapOOMMessage(J9VMThread* currentThread)
{
    ...
    const char* nlsMessage = j9nls_lookup_message(
        J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE, J9NLS_VM_HEAP_OUT_OF_MEMORY, "Java heap space");
    j9object_t message
        = gcfns->j9gc_createJavaLangString(currentThread, (U_8*)nlsMessage, (U_32)strlen(nlsMessage), J9_STR_TENURE);
    ...
    J9VMThread* currentThread, UDATA isStatic, J9ROMClass* romClass, J9ROMFieldShape* field, J9ROMMethod* romMethod)
{
    PORT_ACCESS_FROM_VMC(currentThread);
    char* msg = NULL;
    /* J9NLS_VM_ILLEGAL_ACCESS_STATIC_FINAL=Attempt to set static final field %2$.*1$s.%4$.*3$s from method "%6$.*5$s"
     * that is not <clinit> */
    const char* nlsMessage = OMRPORT_FROM_J9PORT(PORTLIB)->nls_lookup_message(OMRPORT_FROM_J9PORT(PORTLIB),
        J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE, J9NLS_VM_ILLEGAL_ACCESS_STATIC_FINAL__MODULE,
        isStatic ? J9NLS_VM_ILLEGAL_ACCESS_STATIC_FINAL__ID : J9NLS_VM_ILLEGAL_ACCESS_NONSTATIC_FINAL__ID, NULL);
    ...
    if (nlsMessage != NULL) {
        ...
        U_16 methodNameLength = J9UTF8_LENGTH(methodNameUTF);
        U_8* methodName = J9UTF8_DATA(methodNameUTF);
        UDATA msgLen = j9str_printf(PORTLIB, NULL, 0, nlsMessage, classNameLength, className, fieldNameLength,
            fieldName, methodNameLength, methodName);
        msg = j9mem_allocate_memory(msgLen, OMRMEM_CATEGORY_VM);
        /* msg NULL check omitted since str_printf accepts NULL (as above) */
        j9str_printf(PORTLIB, msg, msgLen, nlsMessage, classNameLength, className, fieldNameLength, fieldName,
