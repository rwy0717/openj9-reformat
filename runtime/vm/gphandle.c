...
static UDATA recursiveCrashHandler(
    struct J9PortLibrary* portLibrary, U_32 gpType, void* gpInfo, void* recursiveCrashHandlerData);
...
static void dumpVmDetailString(struct J9PortLibrary* portLibrary, J9JavaVM* vm)
{
    ...
    j9tty_printf(portLibrary, "CPU=%s (%d logical CPUs) (0x%llx RAM)\n", details.cpuBuf ? details.cpuBuf : "unknown",
        details.cpus, details.mem);
}
...
static UDATA isCallerClassJavaNio(J9VMThread* currentThread, J9StackWalkState* walkState)
{
    ...
    if (NULL != walkState->method) {
        ...
        if (J9UTF8_LITERAL_EQUALS(data, length, "sun/misc/Unsafe")
            || J9UTF8_LITERAL_EQUALS(data, length, "jdk/internal/misc/Unsafe")) {
            goto done;
        }
        if (length >= 9) {
...
static UDATA throwConditionException(J9VMThread* vmThread, struct ConditionExceptionConstructorArgs* ceArgs)
{
    ...
    (*env)->SetByteArrayRegion(
        env, rawTokenBytesJBArray, 0, ceArgs->lenBytesRawTokenBytes, (jbyte*)ceArgs->rawTokenBytes);
    if ((*env)->ExceptionCheck(env)) {
        goto cleanup;
    }
    ...
    ceObject = (*env)->NewObject(env, cls, cid, failingRoutineJString, ceArgs->offset, rawTokenBytesJBArray,
        facilityIDJString, ceArgs->c1, ceArgs->c2, ceArgs->caze, ceArgs->severity, ceArgs->control, ceArgs->iSInfo);
    if (NULL == ceObject) {
        goto cleanup;
    }
...
static UDATA handleJNIZOSLECondition(struct J9PortLibrary* portLibrary, J9VMThread* vmThread, void* gpInfo)
{
    ...
    if ((vmThread->omrVMThread->vmState & J9VMSTATE_MAJOR) == J9VMSTATE_JNI) {
        ...
        infoType
            = j9sig_info(gpInfo, J9PORT_SIG_SIGNAL, J9PORT_SIG_SIGNAL_ZOS_CONDITION_SEVERITY, &infoName, &infoValue);
        ...
        infoType
            = j9sig_info(gpInfo, J9PORT_SIG_SIGNAL, J9PORT_SIG_SIGNAL_ZOS_CONDITION_FACILITY_ID, &infoName, &infoValue);
        ...
        if ((0 == strncmp("CEE", facilityID, 3)) && (3 == severity)) {
            ...
            infoType = j9sig_info(
                gpInfo, J9PORT_SIG_SIGNAL, J9PORT_SIG_SIGNAL_ZOS_CONDITION_MESSAGE_NUMBER, &infoName, &infoValue);
            if (J9PORT_SIG_VALUE_16 != infoType) {
                return K8ZOS_GO_DOWN;
            }
...
static UDATA checkForTryExceptHandler(struct J9PortLibrary* portLibrary, void* userData)
{
...
vmSignalHandler(struct J9PortLibrary* portLibrary, U_32 gpType, void* gpInfo, void* userData)
{
    ...
    j9sig_protect(checkForTryExceptHandler, &crashData, recursiveCrashHandler, &recursiveCrashData,
        J9PORT_SIG_FLAG_MAY_RETURN | J9PORT_SIG_FLAG_SIGALLSYNC, &result);
    ...
    j9sig_protect(setupRasCrashInfo, &crashData, recursiveCrashHandler, &recursiveCrashData,
        J9PORT_SIG_FLAG_MAY_RETURN | J9PORT_SIG_FLAG_SIGALLSYNC, &result);
#endif

    recursiveCrashData.protectedFunctionName = "writeCrashDataToConsole";
    j9sig_protect(writeCrashDataToConsole, &crashData, recursiveCrashHandler, &recursiveCrashData,
        J9PORT_SIG_FLAG_MAY_RETURN | J9PORT_SIG_FLAG_SIGALLSYNC, &result);

    recursiveCrashData.protectedFunctionName = "generateDiagnosticFiles";
    j9sig_protect(generateDiagnosticFiles, &crashData, recursiveCrashHandler, &recursiveCrashData,
        J9PORT_SIG_FLAG_MAY_RETURN | J9PORT_SIG_FLAG_SIGALLSYNC, &result);

    recursiveCrashData.protectedFunctionName = "reportThreadCrash";
    j9sig_protect(reportThreadCrash, &crashData, recursiveCrashHandler, &recursiveCrashData,
        J9PORT_SIG_FLAG_MAY_RETURN | J9PORT_SIG_FLAG_SIGALLSYNC, &result);

    recursiveCrashData.protectedFunctionName = "executeAbortHook";
    j9sig_protect(executeAbortHook, &crashData, recursiveCrashHandler, &recursiveCrashData,
        J9PORT_SIG_FLAG_MAY_RETURN | J9PORT_SIG_FLAG_SIGALLSYNC, &result);
    ...
    if (J9PORT_SIG_OPTIONS_ZOS_USE_CEEHDLR == (J9PORT_SIG_OPTIONS_ZOS_USE_CEEHDLR & j9sig_get_options())) {
        ...
        if (J9_SIG_PERCOLATE_CONDITIONS == (vm->sigFlags & J9_SIG_PERCOLATE_CONDITIONS)) {
            ...
            /* record that we already triggered diagnostics, so that any le condition handlers further down the call
             * chain do not */
...
static UDATA writeGPInfo(struct J9PortLibrary* portLibrary, void* writeGPInfoCrashData)
{
    ...
    for (info = 0; info < infoCount; info++) {
        ...
        if (((info % INFO_COLS) == INFO_COLS - 1) || (info == infoCount - 1) || (infoKind == J9PORT_SIG_VALUE_STRING)
            || (infoKind == J9PORT_SIG_VALUE_FLOAT_64)) {
            c = '\n';
        } else {
            c = ' ';
        }
        ...
        case J9PORT_SIG_VALUE_FLOAT_64:
            ...
            n = j9str_printf(PORTLIB, cursor, length, "%s %016.16llx (f: %f, d: %e)%c", name, *(U_64*)value,
                (float)LOW_U32_FROM_DBL_PTR(value), *(double*)value, c);
            break;
...
static UDATA writeJITInfo(J9VMThread* vmThread, char* s, UDATA length, void* gpInfo)
{
    ...
    if ((vmThread->omrVMThread->vmState & J9VMSTATE_MAJOR) == J9VMSTATE_JIT_CODEGEN) {
        if (vmThread->jitMethodToBeCompiled) {
            ...
            n = j9str_printf(PORTLIB, s, length, "\nMethod_being_compiled=%.*s.%.*s%.*s\n",
                (UDATA)J9UTF8_LENGTH(className), J9UTF8_DATA(className), (UDATA)J9UTF8_LENGTH(methName),
                J9UTF8_DATA(methName), (UDATA)J9UTF8_LENGTH(methSig), J9UTF8_DATA(methSig));
        } else {
    ...
    if (infoKind == J9PORT_SIG_VALUE_ADDRESS) {
        ...
        if (jitExceptionTable) {
            ...
            n = j9str_printf(PORTLIB, s, length, "\nCompiled_method=%.*s.%.*s%.*s\n", (UDATA)J9UTF8_LENGTH(className),
                J9UTF8_DATA(className), (UDATA)J9UTF8_LENGTH(methName), J9UTF8_DATA(methName),
...
static UDATA writeCrashDataToConsole(struct J9PortLibrary* portLibrary, void* userData)
{
    ...
    *s = '\0';
    ...
    j9tty_printf(portLibrary, "Unhandled exception\nType=%s vmState=0x%08.8x\n", description,
        vmThread ? vmThread->omrVMThread->vmState : -1);

    /* the usable length of the buffer might have been decreased for 8-alignment and to make space for eyecatchers but
     * by how much? */
    ...
    for (category = 0; category < J9PORT_SIG_NUM_CATEGORIES; category++) {
        ...
        j9sig_protect(writeGPInfo, &writeGPInfoCrashData, recursiveCrashHandler, &recursiveCrashData,
            J9PORT_SIG_FLAG_MAY_RETURN | J9PORT_SIG_FLAG_SIGALLSYNC, &result);
