...
#define PAINT_STACK_MEMORY(x) \
    memset((void*)vmThread->tempSlot, GROW_STACK_PAINT_VALUE, ((UDATA)(x)) - vmThread->tempSlot)

#endif

#define IS_IN_STACK(value) ((((UDATA)(value)) >= oldStackStart) && (((UDATA)(value)) < oldStackEnd))

#define FIX_IF_IN_RANGE(slot)                           \
    if (IS_IN_STACK(slot)) {                            \
        *(UDATA**)&(slot) = (((UDATA*)(slot)) + delta); \
    }
...
#if (defined(J9VM_INTERP_NATIVE_SUPPORT))
static void growSlotIterator(
    J9VMThread* vmThread, J9StackWalkState* walkState, j9object_t* objectSlotInNewStack, const void* stackLocation);
#endif /* J9VM_INTERP_NATIVE_SUPPORT */
...
static UDATA internalGrowJavaStack(J9VMThread* vmThread, UDATA newStackSize)
{
    ...
    Trc_VM_growJavaStack_Entry(vmThread, oldStack->size, newStackSize, vmThread->sp, vmThread->stackOverflowMark,
        vmThread->stackOverflowMark2);
    ...
    if (vmThread->javaVM->jitConfig) {
        walkState.restartPoint
            = pool_new(sizeof(J9I2JState*), 0, 0, 0, J9_GET_CALLSITE(), OMRMEM_CATEGORY_VM, POOL_FOR_PORT(PORTLIB));
...

    {
        ...
        if (J9_EVENT_SOM_VALUE != currentStackOverflowMark) {
            ...
            VM_AtomicSupport::lockCompareExchange(
                (UDATA*)&vmThread->stackOverflowMark, currentStackOverflowMark, (UDATA)newStackOverflowMark);
        }
...
static UDATA growFrameIterator(J9VMThread* vmThread, J9StackWalkState* walkState)
{
    ...
    if (!walkState->jitInfo) {
        ...
        case J9SF_FRAME_TYPE_JNI_NATIVE_METHOD: {
            J9SFMethodFrame* methodFrame
                = (J9SFMethodFrame*)((U_8*)walkState->bp - sizeof(J9SFMethodFrame) + sizeof(UDATA));
            ...
            Trc_VM_growFrameIterator_JNIEntry(
                vmThread, walkState->bp, refCount, walkState->frameFlags, walkState->method);
            ...
            if (!(walkState->frameFlags & J9_SSF_JNI_REFS_REDIRECTED)) {
                ...
                /* If this frame is a JIT JNI callout frame, there are no pushed args, but we need to remember the
                   original frame pointer so the enclosing JIT frame can be walked in the old stack (the refs passed to
                   the native method are going to be addresses of args/temps of the enclosing JIT method).
                */
...
#endif
                    /* Note: Do not fall out to the common code here, as we do not want to run the savedAg0EA fixup
                     * outside the case */
                ...
                if (NULL != nativeMethod) {
                    ...
                    while (argPtr != walkState->bp) {
                        ...
                        if (description & J9_REDIRECTED_REFERENCE) {
                            Trc_VM_growFrameIterator_FixArg(
                                vmThread, argPtr + delta, ((UDATA)argPtr) | J9_REDIRECTED_REFERENCE);
...
#ifdef PAINT_OLD_STACK
                /* Do it here to avoid painting the transition frame, which the jit stack walker will read after we
                 * return */
