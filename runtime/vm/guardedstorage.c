...
void invokeJ9ReadBarrier(J9VMThread* currentThread)
{
...
int32_t j9gs_initializeThread(J9VMThread* vmThread)
{
    ...
    if (IS_THREAD_GS_INITIALIZED(vmThread)) {
        ...
        J9GSControlBlock* gsControlBlock
            = (J9GSControlBlock*)j9mem_allocate_memory(sizeof(J9GSControlBlock), J9MEM_CATEGORY_VM);
        if (NULL != gsControlBlock) {
            /* Determine the shift value */
            J9JavaVM* javaVM = vmThread->javaVM;
            int32_t compressedRefShift
                = javaVM->memoryManagerFunctions->j9gc_objaccess_compressedPointersShift(vmThread);
...
int32_t j9gs_deinitializeThread(J9VMThread* vmThread)
{
    PORT_ACCESS_FROM_VMC(vmThread);
    int32_t success = 0;
    BOOLEAN supportsGuardedStorageFacility = FALSE;
