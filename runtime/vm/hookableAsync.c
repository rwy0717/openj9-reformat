...
/*
 * Signal an async event (optionally interrupting the thread) on a specific thread, or all threads if targetThread is
 * NULL.
 *
...
J9RegisterAsyncEvent(J9JavaVM* vm, J9AsyncEventHandler eventHandler, void* userData)
{
    ...
    for (handlerKey = 0; handlerKey < J9ASYNC_MAX_HANDLERS; ++handlerKey) {
        ...
        if (record->handler == NULL) {
            /* No need to worry about write order/barriers here since the event cannot be posted until this function
             * returns the key */
