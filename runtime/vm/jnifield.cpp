...
static J9Method* findFieldContext(J9VMThread* currentThread, IDATA* pcOffset)
{
        ...
        if (J9_ARE_ANY_BITS_SET(J9OBJECT_CLAZZ(currentThread, object)->classFlags, J9ClassHasWatchedFields)) {
            ...
            if (NULL != method) {
                ALWAYS_TRIGGER_J9HOOK_VM_GET_FIELD(
                    vm->hookInterface, currentThread, method, location, object, j9FieldID->offset);
            }
        }
    }
        ...
        if (J9_ARE_ANY_BITS_SET(J9OBJECT_CLAZZ(currentThread, object)->classFlags, J9ClassHasWatchedFields)) {
            ...
            if (NULL != method) {
                ...
                *(I_32*)&newValue = (I_32)value;
                ALWAYS_TRIGGER_J9HOOK_VM_PUT_FIELD(
                    vm->hookInterface, currentThread, method, location, object, j9FieldID->offset, newValue);
            }
        }
    }
...
jint JNICALL getStaticIntField(JNIEnv* env, jclass clazz, jfieldID fieldID)
{
    ...
    if (J9_EVENT_IS_HOOKED(vm->hookInterface, J9HOOK_VM_GET_STATIC_FIELD)) {
        if (J9_ARE_ANY_BITS_SET(declaringClass->classFlags, J9ClassHasWatchedFields)) {
            ...
            J9Method* method = findFieldContext(currentThread, &location);
            if (NULL != method) {
                ALWAYS_TRIGGER_J9HOOK_VM_GET_STATIC_FIELD(
                    vm->hookInterface, currentThread, method, location, declaringClass, valueAddress);
            }
        }
    }
    ...
    jint result
        = (jint)objectAccessBarrier.inlineStaticReadU32(currentThread, declaringClass, (U_32*)valueAddress, isVolatile);
    VM_VMAccess::inlineExitVMToJNI(currentThread);
    return result;
}
...
void JNICALL setStaticIntField(JNIEnv* env, jclass clazz, jfieldID fieldID, jint value)
{
    ...
    if (J9_EVENT_IS_HOOKED(vm->hookInterface, J9HOOK_VM_PUT_STATIC_FIELD)) {
        if (J9_ARE_ANY_BITS_SET(declaringClass->classFlags, J9ClassHasWatchedFields)) {
            ...
            if (NULL != method) {
                U_64 newValue = 0;
                *(I_32*)&newValue = (I_32)value;
                ALWAYS_TRIGGER_J9HOOK_VM_PUT_STATIC_FIELD(
                    vm->hookInterface, currentThread, method, location, declaringClass, valueAddress, newValue);
            }
        }
    }
    ...
    objectAccessBarrier.inlineStaticStoreU32(
        currentThread, declaringClass, (U_32*)valueAddress, (U_32)value, isVolatile);
    VM_VMAccess::inlineExitVMToJNI(currentThread);
}

void JNICALL setStaticFloatField(JNIEnv* env, jclass cls, jfieldID fieldID, jfloat value)
{
...
jlong JNICALL getStaticLongField(JNIEnv* env, jclass clazz, jfieldID fieldID)
{
    ...
    bool isVolatile = J9_ARE_ANY_BITS_SET(modifiers, J9AccVolatile);
    MM_ObjectAccessBarrierAPI objectAccessBarrier(currentThread);
    jlong result = (jlong)objectAccessBarrier.inlineStaticReadU64(
        currentThread, declaringClass, (U_64*)valueAddress, isVolatile);
    VM_VMAccess::inlineExitVMToJNI(currentThread);
    return result;
}

static void JNICALL setStaticDoubleFieldIndirect(JNIEnv* env, jobject clazz, jfieldID fieldID, void* value)
{
    J9VMThread* currentThread = (J9VMThread*)env;
    J9JavaVM* vm = currentThread->javaVM;
    VM_VMAccess::inlineEnterVMFromJNI(currentThread);
    ...
    if (J9_EVENT_IS_HOOKED(vm->hookInterface, J9HOOK_VM_PUT_STATIC_FIELD)) {
        if (J9_ARE_ANY_BITS_SET(declaringClass->classFlags, J9ClassHasWatchedFields)) {
            ...
            if (NULL != method) {
                ALWAYS_TRIGGER_J9HOOK_VM_PUT_STATIC_FIELD(
                    vm->hookInterface, currentThread, method, location, declaringClass, valueAddress, *(U_64*)value);
            }
        }
    }
    ...
    objectAccessBarrier.inlineStaticStoreU64(
        currentThread, declaringClass, (U_64*)valueAddress, *(U_64*)value, isVolatile);
    VM_VMAccess::inlineExitVMToJNI(currentThread);
}

void JNICALL setStaticDoubleField(JNIEnv* env, jclass cls, jfieldID fieldID, jdouble value)
{
    setStaticDoubleFieldIndirect(env, cls, fieldID, &value);
}

void JNICALL setStaticLongField(JNIEnv* env, jclass cls, jfieldID fieldID, jlong value)
{
    setStaticDoubleFieldIndirect(env, cls, fieldID, &value);
}
...
jobject JNICALL getStaticObjectField(JNIEnv* env, jclass clazz, jfieldID fieldID)
{
    ...
    bool isVolatile = J9_ARE_ANY_BITS_SET(modifiers, J9AccVolatile);
    MM_ObjectAccessBarrierAPI objectAccessBarrier(currentThread);
    j9object_t resultObject = objectAccessBarrier.inlineStaticReadObject(
        currentThread, declaringClass, (j9object_t*)valueAddress, isVolatile);
...
void JNICALL setStaticObjectField(JNIEnv* env, jclass clazz, jfieldID fieldID, jobject value)
{
    ...
    objectAccessBarrier.inlineStaticStoreObject(
        currentThread, declaringClass, (j9object_t*)valueAddress, valueObject, isVolatile);
    VM_VMAccess::inlineExitVMToJNI(currentThread);
}
