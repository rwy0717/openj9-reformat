...
jint JNICALL DestroyJavaVM(JavaVM* javaVM)
{
    ...
    if (j9sig_protect(protectedDestroyJavaVM, vmThread, structuredSignalHandler, vmThread,
...
jint JNICALL DetachCurrentThread(JavaVM* javaVM)
{
    ...
    if ((result = (UDATA)verifyCurrentThreadAttached(vm, &vmThread)) == JNI_OK) {
        ...
        if (j9sig_protect(protectedDetachCurrentThread, vmThread, structuredSignalHandler, vmThread,
                J9PORT_SIG_FLAG_SIGALLSYNC | J9PORT_SIG_FLAG_MAY_CONTINUE_EXECUTION, &result)) {
            result = JNI_ERR;
        }
    ...
    if (thr_args != NULL) {
        ...
        if (NULL != threadName) {
            ...
            if (!isValidUtf8((U_8*)threadName, threadNameLength)) {
                ...
                modifiedThreadName
                    = j9mem_allocate_memory(errorMessageLength + threadNameLength + 3, OMRMEM_CATEGORY_VM);
                ...
                if (NULL != modifiedThreadName) {
                    ...
                    fixBadUtf8((U_8*)threadName, (U_8*)modifiedThreadName + strlen(modifiedThreadName),
                        threadNameLength); /* concatenate the corrected name with the error string */
...
#endif /* J9VM_INTERP_ATOMIC_FREE_JNI */
        initializeAttachedThread(env, threadName, threadGroup, (threadType & J9_PRIVATE_FLAGS_DAEMON_THREAD) != 0, env);
    ...
    J9JavaVM* vm, J9VMThread** p_env, J9JavaVMAttachArgs* thr_args, UDATA threadType, void* osThread)
{
    ...
    if (j9sig_protect(protectedInternalAttachCurrentThread, &args, structuredSignalHandlerVM, vm,
            J9PORT_SIG_FLAG_SIGALLSYNC | J9PORT_SIG_FLAG_MAY_CONTINUE_EXECUTION, &result)) {
        result = JNI_ERR;
    }
...
static J9ThreadEnv threadEnv = { omrthread_get_priority, omrthread_set_priority, omrthread_self, omrthread_global,
    omrthread_attach, omrthread_sleep, omrthread_create, omrthread_exit, omrthread_abort, omrthread_priority_interrupt,
    omrthread_clear_interrupted, omrthread_monitor_enter, omrthread_monitor_exit, omrthread_monitor_init_with_name,
    omrthread_monitor_destroy, omrthread_monitor_wait, omrthread_monitor_notify_all, omrthread_tls_get,
    omrthread_tls_set, omrthread_tls_alloc, omrthread_tls_free };
...
attachSystemDaemonThread(J9JavaVM* vm, J9VMThread** p_env, const char* threadName)
{
    ...
    rc = internalAttachCurrentThread(
        vm, p_env, &attachArgs, J9_PRIVATE_FLAGS_SYSTEM_THREAD | J9_PRIVATE_FLAGS_DAEMON_THREAD, osThread);
...
static jint attachCurrentThread(JavaVM* vm, void** p_env, void* thr_args, UDATA threadType)
{
    ...
    rc = (jint)internalAttachCurrentThread(
        j9vm, (J9VMThread**)p_env, (J9JavaVMAttachArgs*)thr_args, threadType, osThread);
    if (JNI_OK == rc) {
...
static UDATA terminateRemainingThreads(J9VMThread* vmThread)
{
    ...
    /* Wait for any zombie threads (java notified of death, but vmThread not freed and threadProc not exitted) to
...
static jint verifyCurrentThreadAttached(J9JavaVM* vm, J9VMThread** pvmThread)
{
    ...
    /* A thread can only detach if it has not called into the VM (the current stack frame must be the only frame on
     * stack) */
...
#if (defined(J9VM_OPT_SIDECAR))
jint JNICALL QueryJavaVM(JavaVM* vm, jint nQueries, JavaVMQuery* queries) { return JNI_ERR; }

jint JNICALL ResetJavaVM(JavaVM* vm) { return JNI_ERR; }
