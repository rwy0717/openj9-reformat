 ...
 */
static void javaOffloadSwitchOnWithReason(J9VMThread* currentThread, UDATA reason)
{
 ...
 * @param reason[in] the reason code
 */
static void javaOffloadSwitchOffWithReason(J9VMThread* currentThread, UDATA reason)
{
    J9JavaVM* vm = currentThread->javaVM;
 ...
 */
static VMINLINE J9Class* fetchArrayClass(J9VMThread* currentThread, J9Class* elementTypeClass)
{
 ...
 */
static J9ClassLoader* getCurrentClassLoader(J9VMThread* currentThread)
{
    J9JavaVM* vm = currentThread->javaVM;
    ...
    if (NULL == nativeMethod) {
        ...
        /* special case - if loadLibraryWithPath is the current native method, use the class loader passed to that
         * native */
 ...
 */
static bool checkString(const char* data, UDATA* lengthPtr)
{
    UDATA length = 0;
 ...
 */
U_8* compressUTF8(J9VMThread* currentThread, U_8* data, UDATA length, UDATA* compressedLengthPtr)
{
 ...
 */
static U_8* getVerifiedUTF8(J9VMThread* currentThread, const char* cString, UDATA* compressedLengthPtr)
{
 ...
 */
static UDATA encodeUnverifiedUTF8(const char* source, UDATA sourceLength, U_8* target)
{
...
jobject JNICALL allocObject(JNIEnv* env, jclass clazz)
{
    ...
    if (VM_VMHelpers::classCanBeInstantiated(j9clazz)) {
        ...
        resultObject = currentThread->javaVM->memoryManagerFunctions->J9AllocateObject(
            currentThread, j9clazz, J9_GC_ALLOCATE_OBJECT_INSTRUMENTABLE);
        if (NULL == resultObject) {
            gpCheckSetHeapOutOfMemoryError(currentThread);
        }
    } else {
        gpCheckSetCurrentException(
            currentThread, J9_EX_CTOR_CLASS + J9VMCONSTANTPOOL_JAVALANGINSTANTIATIONEXCEPTION, (UDATA*)classObject);
    }
done:
...
jclass JNICALL findClass(JNIEnv* env, const char* name)
{
    ...
    if (NULL != verifiedName) {
        ...
        J9Class* clazz = internalFindClassUTF8(
            currentThread, trimmedName, nameLength, classLoader, J9_FINDCLASS_FLAG_THROW_ON_FAIL);
        if (NULL != clazz) {
...
jstring JNICALL newStringUTF(JNIEnv* env, const char* bytes)
{
    ...
    if (NULL != bytes) {
        ...
        JAVA_OFFLOAD_SWITCH_ON_WITH_REASON_IF_LIMIT_EXCEEDED(
            currentThread, J9_JNI_OFFLOAD_SWITCH_NEW_STRING_UTF, length);
        /* If there are no characters > 127, the string can be used directly */
        ...
        resultObject = currentThread->javaVM->memoryManagerFunctions->j9gc_createJavaLangString(
            currentThread, data, length, J9_STR_INSTRUMENTABLE);
        if (data != (U_8*)bytes) {
            jniArrayFreeMemoryFromThread(currentThread, (void*)data);
        }
    done:
        JAVA_OFFLOAD_SWITCH_OFF_WITH_REASON_IF_LIMIT_EXCEEDED(
            currentThread, J9_JNI_OFFLOAD_SWITCH_NEW_STRING_UTF, length);
    }
...
jobjectArray JNICALL newObjectArray(JNIEnv* env, jsize length, jclass clazz, jobject initialElement)
{
    ...
    if (length < 0) {
        ...
        if (NULL != arrayClass) {
            resultObject = currentThread->javaVM->memoryManagerFunctions->J9AllocateIndexableObject(
                currentThread, arrayClass, (U_32)length, J9_GC_ALLOCATE_OBJECT_INSTRUMENTABLE);
            if (NULL == resultObject) {
                gpCheckSetHeapOutOfMemoryError(currentThread);
...
static VMINLINE void getOrSetArrayRegion(JNIEnv* env, jarray array, jsize start, jsize len, void* buf, bool isGet)
{
    J9VMThread* currentThread = (J9VMThread*)env;
    VM_VMAccess::inlineEnterVMFromJNI(currentThread);
    ...
    if ((ustart >= size) || (end > size) || (end < ustart) /* overflow */
    ) {
        ...
        UDATA logElementSize
            = ((J9ROMArrayClass*)J9OBJECT_CLAZZ(currentThread, arrayObject)->romClass)->arrayShape & 0x0000FFFF;
        UDATA byteCount = ulen << logElementSize;
        /* No guarantee of native memory alignment, so copy byte-wise */
        if (isGet) {
            JAVA_OFFLOAD_SWITCH_ON_WITH_REASON_IF_LIMIT_EXCEEDED(
                currentThread, J9_JNI_OFFLOAD_SWITCH_GET_ARRAY_REGION, byteCount);
            VM_ArrayCopyHelpers::memcpyFromArray(
                currentThread, arrayObject, (UDATA)0, ustart << logElementSize, byteCount, buf);
            JAVA_OFFLOAD_SWITCH_OFF_WITH_REASON_IF_LIMIT_EXCEEDED(
                currentThread, J9_JNI_OFFLOAD_SWITCH_GET_ARRAY_REGION, byteCount);
        } else {
            JAVA_OFFLOAD_SWITCH_ON_WITH_REASON_IF_LIMIT_EXCEEDED(
                currentThread, J9_JNI_OFFLOAD_SWITCH_SET_ARRAY_REGION, byteCount);
...
void* JNICALL getArrayElements(JNIEnv* env, jarray array, jboolean* isCopy)
{
    J9VMThread* currentThread = (J9VMThread*)env;
    J9JavaVM* vm = currentThread->javaVM;
    void* elems = NULL;
    if (J9_ARE_ANY_BITS_SET(vm->extendedRuntimeFlags, J9_EXTENDED_RUNTIME_ALWAYS_USE_JNI_CRITICAL)) {
        ...
        if (NULL == elems) {
            ...
            JAVA_OFFLOAD_SWITCH_ON_WITH_REASON_IF_LIMIT_EXCEEDED(
                currentThread, J9_JNI_OFFLOAD_SWITCH_GET_ARRAY_ELEMENTS, byteCount);
            /* No guarantee of native memory alignment, so copy byte-wise */
...
void JNICALL releaseArrayElements(JNIEnv* env, jarray array, void* elems, jint mode)
{
    ...
    if (J9_ARE_ANY_BITS_SET(vm->extendedRuntimeFlags, J9_EXTENDED_RUNTIME_ALWAYS_USE_JNI_CRITICAL)) {
        ...
        if (JNI_ABORT != mode) {
            ...
            UDATA byteCount = (UDATA)J9INDEXABLEOBJECT_SIZE(currentThread, arrayObject) << logElementSize;
            JAVA_OFFLOAD_SWITCH_ON_WITH_REASON_IF_LIMIT_EXCEEDED(
                currentThread, J9_JNI_OFFLOAD_SWITCH_RELEASE_ARRAY_ELEMENTS, byteCount);
            /* No guarantee of native memory alignment, so copy byte-wise */
...
jobject newBaseTypeArray(JNIEnv* env, IDATA length, UDATA arrayClassOffset)
{
    J9VMThread* currentThread = (J9VMThread*)env;
    j9object_t resultObject = NULL;
    VM_VMAccess::inlineEnterVMFromJNI(currentThread);
    if (length < 0) {
        ...
        resultObject = vm->memoryManagerFunctions->J9AllocateIndexableObject(
            currentThread, arrayClass, (U_32)length, J9_GC_ALLOCATE_OBJECT_INSTRUMENTABLE);
        if (NULL == resultObject) {
            gpCheckSetHeapOutOfMemoryError(currentThread);
        }
...
const jchar* JNICALL getStringChars(JNIEnv* env, jstring string, jboolean* isCopy)
{
    ...
    if (J9_ARE_ANY_BITS_SET(vm->extendedRuntimeFlags, J9_EXTENDED_RUNTIME_ALWAYS_USE_JNI_CRITICAL)) {
        ...
        if (NULL == chars) {
            ...
            JAVA_OFFLOAD_SWITCH_ON_WITH_REASON_IF_LIMIT_EXCEEDED(
                currentThread, J9_JNI_OFFLOAD_SWITCH_GET_STRING_CHARS, allocateLength);
...
void JNICALL releaseStringChars(JNIEnv* env, jstring string, const jchar* chars)
{
    J9VMThread* currentThread = (J9VMThread*)env;
    J9JavaVM* vm = currentThread->javaVM;
    if (J9_ARE_ANY_BITS_SET(vm->extendedRuntimeFlags, J9_EXTENDED_RUNTIME_ALWAYS_USE_JNI_CRITICAL)) {
...
void JNICALL releaseStringCharsUTF(JNIEnv* env, jstring string, const char* chars)
{
    J9VMThread* currentThread = (J9VMThread*)env;
    J9JavaVM* vm = currentThread->javaVM;
    /* Allow the VM to crash on NULL input if -Xfuture is enabled */
...
const char* JNICALL getStringUTFChars(JNIEnv* env, jstring string, jboolean* isCopy)
{
    ...
    if (NULL == utfChars) {
        ...
        JAVA_OFFLOAD_SWITCH_ON_WITH_REASON_IF_LIMIT_EXCEEDED(
            currentThread, J9_JNI_OFFLOAD_SWITCH_GET_STRING_UTF_CHARS, utfLength);
        copyStringToUTF8Helper(currentThread, stringObject, J9_STR_NULL_TERMINATE_RESULT, 0,
            J9VMJAVALANGSTRING_LENGTH(currentThread, stringObject), utfChars, utfLength);
...
void JNICALL getStringUTFRegion(JNIEnv* env, jstring str, jsize start, jsize len, char* buf)
{
    J9VMThread* currentThread = (J9VMThread*)env;
    VM_VMAccess::inlineEnterVMFromJNI(currentThread);
    if ((start < 0) || (len < 0) || (((U_32)(start + len)) > I_32_MAX)) {
    outOfBounds:
        ...
        JAVA_OFFLOAD_SWITCH_ON_WITH_REASON_IF_LIMIT_EXCEEDED(
            currentThread, J9_JNI_OFFLOAD_SWITCH_GET_STRING_UTF_REGION, (UDATA)length * sizeof(U_16));
        copyStringToUTF8Helper(
            currentThread, stringObject, J9_STR_NULL_TERMINATE_RESULT, start, len, (U_8*)buf, UDATA_MAX);
        JAVA_OFFLOAD_SWITCH_OFF_WITH_REASON_IF_LIMIT_EXCEEDED(
            currentThread, J9_JNI_OFFLOAD_SWITCH_GET_STRING_UTF_REGION, (UDATA)length * sizeof(U_16));
    }
    VM_VMAccess::inlineExitVMToJNI(currentThread);
...
jint JNICALL registerNatives(JNIEnv* env, jclass clazz, const JNINativeMethod* methods, jint nMethods)
{
    ...
    if (NULL == copiedNativeMethods) {
        ...
        /* First verify that each method exists and is native, then apply JVMTI translations and JNI redirection if
         * necessary */
        jint count = nMethods;
        ...
        while (0 != count) {
            ...
            if ((NULL == method)
                || (J9_ARE_NO_BITS_SET(J9_ROM_METHOD_FROM_RAM_METHOD(method)->modifiers, J9AccNative))) {
...
jint JNICALL unregisterNatives(JNIEnv* env, jclass clazz)
{
    ...
    if ((NULL != vm->jitConfig) && (NULL != vm->jitConfig->jitDiscardPendingCompilationsOfNatives)) {
...
void JNICALL getStringRegion(JNIEnv* env, jstring str, jsize start, jsize len, jchar* buf)
{
    ...
    outOfBounds:
        ...
        JAVA_OFFLOAD_SWITCH_ON_WITH_REASON_IF_LIMIT_EXCEEDED(
            currentThread, J9_JNI_OFFLOAD_SWITCH_GET_STRING_REGION, byteCount);
        if (IS_STRING_COMPRESSED(currentThread, stringObject)) {
            ...
            /* No guarantee of native memory alignment, so copy byte-wise */
            VM_ArrayCopyHelpers::memcpyFromArray(
                currentThread, charArray, (UDATA)0, start * sizeof(U_16), byteCount, (void*)buf);
        }
        JAVA_OFFLOAD_SWITCH_OFF_WITH_REASON_IF_LIMIT_EXCEEDED(
            currentThread, J9_JNI_OFFLOAD_SWITCH_GET_STRING_REGION, byteCount);
    }
...
void returnFromJNI(J9VMThread* currentThread, void* vbp)
{
    UDATA* bp = (UDATA*)vbp;
