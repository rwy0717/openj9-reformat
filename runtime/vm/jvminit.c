...
static const struct J9VMIgnoredOption ignoredOptionTable[] = {
    ...
    { VMOPT_XQUICKSTART, EXACT_MATCH }, /* sorta bogus... j9c now uses this option in the romclass.c which can be run
                                           with a Micro JIT, which does not take this option */
    /* also note we had this inside the #if defined(J9VM_OPT_SIDECAR) before the j9c thing because sov launchers specify
       with JIT off (sigh) */
    { VMOPT_XNOQUICKSTART,
        EXACT_MATCH }, /* since we eat -Xquickstart, we should eat -Xnoquickstart for the same reason. */
...
static IDATA setMemoryOptionToOptElse(
    J9JavaVM* vm, UDATA* thingToSet, char* optionName, UDATA defaultValue, UDATA doConsumeArg);
static IDATA setIntegerValueOptionToOptElse(
    J9JavaVM* vm, UDATA* thingToSet, char* optionName, UDATA defaultValue, UDATA doConsumeArg);
...
static void generateMemoryOptionParseError(
    J9JavaVM* vm, J9VMDllLoadInfo* loadInfo, UDATA errorType, char* optionWithError);
...
static IDATA (*portLibrary_file_write_text)(struct OMRPortLibrary* portLibrary, IDATA fd, const char* buf, IDATA nbytes)
    = NULL;
...
static jint initializeBytecodePairs(J9JavaVM* vm)
{
...
static void freeBytecodePairs(J9JavaVM* vm)
{
    PORT_ACCESS_FROM_JAVAVM(vm);
    UDATA* matrix = (UDATA*)vm->debugField1;
...
static void print_verbose_stackusage_of_nonsystem_threads(J9VMThread* vmThread)
{
    ...
    if ((vm->runtimeFlags & J9_RUNTIME_REPORT_STACK_USE) && vmThread->stackObject
        ...
        if ((NULL == vm->vmThreadListMutex) || omrthread_monitor_try_enter(vm->vmThreadListMutex)) {
            ...
            /*If omrthread_monitor_try_enter returns true, it failed to get the lock. If it succeeds to enter, then it
             * returns 0*/
            PORT_ACCESS_FROM_JAVAVM(vm);
            ...
            while (currentThread != vmThread) {
                ...
                if (currentThread->privateFlags & J9_PRIVATE_FLAGS_SYSTEM_THREAD) {
                     ...
                     * When system.exit() is called, it prints stack usage info for system thread and a thread that
                     * calls system.exit()
                     */
...
void print_verbose_stackUsage(J9VMThread* vmThread, UDATA stillRunning)
{
    ...
    if (vmThread->threadObject) {
        ...
        if (stillRunning == FALSE) {
            ...
            /* J9NLS_VERB_STACK_USAGE_FOR_RUNNING_THREADS=Verbose stack: Running \"%2$.*1$s\" is using %3$zd/%4$zd bytes
             * on Java/C stacks*/
            j9nls_printf(PORTLIB, J9NLS_INFO, J9NLS_VERB_STACK_USAGE_FOR_RUNNING_THREADS, strlen(name), name, nbyteUsed,
                cbyteUsed);
        }
        releaseOMRVMThreadName(vmThread->omrVMThread);
    }
    if (nbyteUsed > vm->maxStackUse)
        vm->maxStackUse = nbyteUsed;
    if (cbyteUsed > vm->maxCStackUse)
        vm->maxCStackUse = cbyteUsed;
}
...
void freeJavaVM(J9JavaVM* vm)
{
     ...
     * J2SE/Sidecar builds:
     *			This applies to Windows only. Shutdown hooks for all other platforms are handled by the Hursley
     *JCLs
     *
     * J2ME/Embedded builds:
     * 			This applies to Windows only for now. Since we don't have Hursley JCLs for this, we will need to provide
     *our own support for all other cases/platforms.
     */
    ...
    /* Close the trace DLL. This has to be after all hashtable and pool free events, otherwise we'll crash on pool
     * tracepoints */
 ...
 */
static BOOLEAN isValidProcessorAndOperatingSystem(J9PortLibrary* portLibrary)
{
#if defined(WIN32)
    PORT_ACCESS_FROM_PORT(portLibrary);
#if defined(_WIN32_WINNT_WINBLUE) && (_WIN32_WINNT_MAXVER >= _WIN32_WINNT_WINBLUE)
...
jint initializeJavaVM(void* osMainThread, J9JavaVM** vmPtr, J9CreateJavaVMParams* createParams)
{
    ...
    *vmPtr = vm;
    ...
    if (0
        != j9sig_protect(protectedInitializeJavaVM, &initArgs, structuredSignalHandlerVM, vm,
...
{
    va_list args;
    jint(JNICALL * *vprintfHookFunctionPtr)(FILE * fp, const char* format, va_list args)
        = GLOBAL_DATA(vprintfHookFunction);
...
static void loadDLL(void* dllLoadInfo, void* userDataTemp)
{
    ...
    if (J9_ARE_NO_BITS_SET(entry->loadFlags, (MAGIC_LOAD | NOT_A_LIBRARY | BUNDLED_COMP))
        && J9_ARE_ANY_BITS_SET(entry->loadFlags, userData->flags)) {
        I_64 start = 0;
        I_64 end = 0;
        BOOLEAN loadDllSuccess = FALSE;
        const char* const dllName
            = (entry->loadFlags & ALTERNATE_LIBRARY_USED) ? entry->alternateDllName : entry->dllName;
        ...
        JVMINIT_VERBOSE_INIT_VM_TRACE2(
            userData->vm, "\t\tcompleted with rc=%d in %lld nano sec.\n", loadDllSuccess, (end - start));
...
{
    J9Pool* returnVal
        = pool_new(sizeof(J9VMDllLoadInfo), 0, 0, 0, J9_GET_CALLSITE(), OMRMEM_CATEGORY_VM, POOL_FOR_PORT(portLibrary));
    IDATA i;
    ...
    if (NULL
        == createLoadInfo(portLibrary, returnVal, J9_JIT_DEBUG_DLL_NAME, (MAGIC_LOAD | NO_J9VMDLLMAIN | SILENT_NO_DLL),
               NULL, verboseFlags))
        goto _error;

    if (NULL
        == createLoadInfo(
               portLibrary, returnVal, J9_VERIFY_DLL_NAME, NOT_A_LIBRARY, (void*)&j9bcv_J9VMDllMain, verboseFlags))
        goto _error;
    if (NULL
        == createLoadInfo(portLibrary, returnVal, J9_GC_DLL_NAME, (LOAD_BY_DEFAULT | FATAL_NO_DLL), NULL, verboseFlags))
        goto _error;
    if (NULL
        == createLoadInfo(
               portLibrary, returnVal, J9_DYNLOAD_DLL_NAME, NOT_A_LIBRARY, (void*)&bcutil_J9VMDllMain, verboseFlags))
        goto _error;
    ...
    if (!createLoadInfo(
            portLibrary, returnVal, J9_DEFAULT_JCL_DLL_NAME, (LOAD_BY_DEFAULT | FATAL_NO_DLL), NULL, verboseFlags))
        goto _error;

    if (NULL
        == createLoadInfo(
               portLibrary, returnVal, FUNCTION_ZERO_INIT, NOT_A_LIBRARY, (void*)&zeroInitStages, verboseFlags))
        goto _error;
    if (!createLoadInfo(portLibrary, returnVal, FUNCTION_VM_INIT, NOT_A_LIBRARY, (void*)&VMInitStages, verboseFlags))
        goto _error;
    if (NULL
        == createLoadInfo(
               portLibrary, returnVal, FUNCTION_THREAD_INIT, NOT_A_LIBRARY, (void*)&threadInitStages, verboseFlags))
        goto _error;
    ...
    for (i = (j9vm_args->actualVMArgs->nOptions - 1); i >= 0; i--) {
        ...
        if (strstr(testString, VMOPT_XRUN) == testString) {
            /* No need to scan for -Xrunjdwp as a special case. We should never see that option directly as it is either
             * mapped or translated. */
            ...
            newEntry = createLoadInfo(portLibrary, returnVal, dllNameBuffer,
                (LOAD_BY_DEFAULT | XRUN_LIBRARY | NO_J9VMDLLMAIN | FATAL_NO_DLL), NULL, verboseFlags);
            if (NULL != newEntry) {
                if (OPTION_OK
                    != optionValueOperations(PORTLIB, j9vm_args, i, GET_OPTION, &options, 0, ':', 0,
                           NULL)) { /* knows how to deal with mapped options */
                    goto _error;
                }
...
initializeClassPathEntry(J9JavaVM* javaVM, J9ClassPathEntry* cpEntry)
{
    ...
    if (EsIsFile == attr) {
        ...
        if (NULL != zipFile) {
            ...
            rc = zipFunctions->zip_openZipFile(
                VMI, (char*)cpEntry->path, zipFile, ZIP_FLAG_OPEN_CACHE | ZIP_FLAG_BOOTSTRAP);
...
initializeModulesPath(J9JavaVM* vm)
{
     ...
     */
    modulesPathLen = strlen(javaHome->value) + LITERAL_STRLEN(DIR_SEPARATOR_STR) + LITERAL_STRLEN("lib")
    ...
    j9str_printf(PORTLIB, (char*)modulesPath, (U_32)modulesPathLen + 1,
        "%s" DIR_SEPARATOR_STR "lib" DIR_SEPARATOR_STR "modules", javaHome->value);
    ...
    if (CPE_TYPE_UNUSABLE == rc) {
        ...
        j9str_printf(
            PORTLIB, (char*)modulesPath, (U_32)modulesPathLen + 1, "%s" DIR_SEPARATOR_STR "modules", javaHome->value);
        vm->modulesPathEntry->pathLength = (U_32)modulesPathLen;
        rc = initializeClassPathEntry(vm, vm->modulesPathEntry);
        if (CPE_TYPE_UNUSABLE == rc) {
            goto _error;
...
setBootLoaderModulePatchPaths(J9JavaVM* javaVM, J9Module* j9module, const char* moduleName)
{
    ...
    while (NULL != property) {
        if ((0 == strncmp(property->name, patchProperty, patchPropLen)) && (strlen(property->value) >= (length + 1))
            && ('=' == property->value[length]) && (0 == strncmp(moduleName, property->value, length))) {
            ...
            moduleInfo.patchPathCount = initializeClassPath(
                javaVM, property->value + length + 1, pathSeparator, 0, FALSE, &moduleInfo.patchPathEntries);
...
static VMINLINE void processMemoryInterleaveOptions(J9JavaVM* vm)
{
    PORT_ACCESS_FROM_JAVAVM(vm);
...
static VMINLINE void dumpClassLoader(J9JavaVM* vm, J9ClassLoader* loader, IDATA fd)
{
...
void dumpLoadedClassList(J9HookInterface** hookInterface, uintptr_t eventNum, void* eventData, void* userData)
{
...
static void j9print_internal_version(J9PortLibrary* portLib)
{
    ...
    j9tty_err_printf(PORTLIB,
        "Eclipse OpenJ9 %s %s-bit Server VM (%s) from %s-%s JRE with %s %s, built on %s %s by %s with %s\n",
        J9PRODUCT_NAME, J9TARGET_CPU_BITS, J9VERSION_STRING, J9TARGET_OS, J9TARGET_CPU_OSARCH, J9JDK_EXT_NAME,
        J9JDK_EXT_VERSION, __DATE__, __TIME__, J9USERNAME, J9COMPILER_VERSION_STRING);
#else
    j9tty_err_printf(PORTLIB, "Eclipse OpenJ9 %s %s-bit Server VM (%s) from %s-%s JRE, built on %s %s by %s with %s\n",
        J9PRODUCT_NAME, J9TARGET_CPU_BITS, J9VERSION_STRING, J9TARGET_OS, J9TARGET_CPU_OSARCH, __DATE__, __TIME__,
        J9USERNAME, J9COMPILER_VERSION_STRING);
...
IDATA VMInitStages(J9JavaVM* vm, IDATA stage, void* reserved)
{
    ...
    case PORT_LIBRARY_GUARANTEED:
        ...
        if (OPTION_OK
            != (parseError = setMemoryOptionToOptElse(vm, &(vm->classLoadingMaxStack), VMOPT_XMSCL, 0, TRUE))) {
        ...
        if (OPTION_OK
            != (parseError = setIntegerValueOptionToOptElse(vm, &(vm->maxInvariantLocalTableNodeCount), VMOPT_XITN,
                    J9_INVARIANT_INTERN_TABLE_NODE_COUNT, TRUE))) {
...
#if defined(J9VM_OPT_SHARED_CLASSES)
        {
            ...
            if ((!J9_SHARED_CACHE_DEFAULT_BOOT_SHARING(vm)) && (argIndex < 0)) {
                ...
                if (NULL
                    == (piConfig = (J9SharedClassPreinitConfig*)j9mem_allocate_memory(
                            sizeof(J9SharedClassPreinitConfig), OMRMEM_CATEGORY_VM))) {
                    goto _error;
                }
                if (OPTION_OK
                    != (parseError = setMemoryOptionToOptElse(
                            vm, (UDATA*)&(piConfig->sharedClassDebugAreaBytes), VMOPT_XSCDMX, -1, FALSE))) {
                    parseErrorOption = VMOPT_XSCDMX;
                    goto _memParseError;
                }
                if (OPTION_OK
                    != (parseError = setMemoryOptionToOptElse(
                            vm, (UDATA*)&(piConfig->sharedClassMinAOTSize), VMOPT_XSCMINAOT, -1, FALSE))) {
                    parseErrorOption = VMOPT_XSCMINAOT;
                    goto _memParseError;
                }
                if (OPTION_OK
                    != (parseError = setMemoryOptionToOptElse(
                            vm, (UDATA*)&(piConfig->sharedClassMaxAOTSize), VMOPT_XSCMAXAOT, -1, FALSE))) {
                    parseErrorOption = VMOPT_XSCMAXAOT;
                    goto _memParseError;
                }
                if (OPTION_OK
                    != (parseError = setIntegerValueOptionToOptElse(
                            vm, (UDATA*)&(piConfig->sharedClassInternTableNodeCount), VMOPT_XITSN, -1, FALSE))) {
                    parseErrorOption = VMOPT_XITSN;
                    goto _memParseError;
                }
                if (OPTION_OK
                    != (parseError = setMemoryOptionToOptElse(
                            vm, (UDATA*)&(piConfig->sharedClassMinJITSize), VMOPT_XSCMINJITDATA, -1, FALSE))) {
                    parseErrorOption = VMOPT_XSCMINJITDATA;
                    goto _memParseError;
                }
                if (OPTION_OK
                    != (parseError = setMemoryOptionToOptElse(
                            vm, (UDATA*)&(piConfig->sharedClassMaxJITSize), VMOPT_XSCMAXJITDATA, -1, FALSE))) {
                ...
                if (argIndex9 >= 0) {
                    ...
                    if (OPTION_OK
                        != (parseError = setMemoryOptionToOptElse(
                                vm, (UDATA*)&(piConfig->sharedClassSoftMaxBytes), VMOPT_XSCMX, -1, FALSE))) {
                        parseErrorOption = VMOPT_XSCMX;
                        goto _memParseError;
                    }
                    if (OPTION_OK
                        != (parseError = setMemoryOptionToOptElse(vm, &(piConfig->sharedClassCacheSize),
                                VMOPT_XXSHARED_CACHE_HARD_LIMIT_EQUALS, 0, FALSE))) {
                    ...
                    if (OPTION_OK
                        != (parseError = setMemoryOptionToOptElse(
                                vm, &(piConfig->sharedClassCacheSize), VMOPT_XSCMX, 0, FALSE))) {
                        parseErrorOption = VMOPT_XSCMX;
                        goto _memParseError;
                    }
                }
        ...
        /* Parse options related to idle tuning */
        {
            ...
            IDATA argIndexCompactOnIdleEnable
                = FIND_AND_CONSUME_ARG(EXACT_MATCH, VMOPT_XXIDLETUNINGCOMPACTONIDLEENABLE, NULL);
            IDATA argIndexCompactOnIdleDisable
                = FIND_AND_CONSUME_ARG(EXACT_MATCH, VMOPT_XXIDLETUNINGCOMPACTONIDLEDISABLE, NULL);
            IDATA argIndexIgnoreUnrecognizedOptionsEnable
                = FIND_AND_CONSUME_ARG(EXACT_MATCH, VMOPT_XXIDLETUNINGIGNOREUNRECOGNIZEDOPTIONSENABLE, NULL);
            IDATA argIndexIgnoreUnrecognizedOptionsDisable
                = FIND_AND_CONSUME_ARG(EXACT_MATCH, VMOPT_XXIDLETUNINGIGNOREUNRECOGNIZEDOPTIONSDISABLE, NULL);
            ...
            if ((argIndexGcOnIdleEnable > argIndexGcOnIdleDisable)
                || ((J2SE_VERSION(vm) >= J2SE_19) && (argIndexGcOnIdleDisable == -1)
                       && J9_ARE_ANY_BITS_SET(vm->runtimeFlags, J9_RUNTIME_TUNE_VIRTUALIZED))) {
            ...
            if (J9_ARE_ANY_BITS_SET(vm->vmRuntimeStateListener.idleTuningFlags,
                    J9_IDLE_TUNING_GC_ON_IDLE | J9_IDLE_TUNING_COMPACT_ON_IDLE)) {
                vm->vmRuntimeStateListener.minIdleWaitTime = 180000; /* in msecs */
                vm->vmRuntimeStateListener.idleMinFreeHeap = 0;
            }

            if ((argIndex = FIND_AND_CONSUME_ARG(STARTSWITH_MATCH, VMOPT_XXIDLETUNINGMINIDLEWATITIME_EQUALS, NULL))
                >= 0) {
                UDATA value;
            ...
            if ((argIndex = FIND_AND_CONSUME_ARG(STARTSWITH_MATCH, VMOPT_XXIDLETUNINGMINFREEHEAPONIDLE_EQUALS, NULL))
                >= 0) {
                UDATA value;
        ...
        parseError = setMemoryOptionToOptElse(
            vm, &(vm->directByteBufferMemoryMax), VMOPT_XXMAXDIRECTMEMORYSIZEEQUALS, (UDATA)-1, TRUE);
        if (OPTION_OK != parseError) {
    ...
    case ALL_DEFAULT_LIBRARIES_LOADED:
        vm->exitHook = (J9_EXIT_HANDLER_PROC)J9_COMPATIBLE_FUNCTION_POINTER(
            getOptionExtraInfo(PORTLIB, vm->vmArgsArray, EXACT_MATCH, VMOPT_EXIT));
        vm->abortHook = (J9_ABORT_HANDLER_PROC)J9_COMPATIBLE_FUNCTION_POINTER(
            getOptionExtraInfo(PORTLIB, vm->vmArgsArray, EXACT_MATCH, VMOPT_ABORT));
    ...
    case ALL_LIBRARIES_LOADED:
        ...
        detectAgentXruns(vm);
#endif
        /* Idle GC tuning is currently enabled for Linux x86-64, i386. For other supported GC policies and platforms,
         * need further study as heap layout/structure and commit/decommit native support differs. For example, balanced
         * GC policy: the logic needs to arrive at the right no. & regions from right set needs to picked up for
         * releasing the pages. Line Item: 50329 continued to be extended for other GC policies and platforms.
         */
        if (J9_ARE_ANY_BITS_SET(vm->vmRuntimeStateListener.idleTuningFlags,
                J9_IDLE_TUNING_GC_ON_IDLE | J9_IDLE_TUNING_COMPACT_ON_IDLE)) {
            BOOLEAN idleGCTuningSupported = FALSE;
#if (defined(LINUX) && (defined(J9HAMMER) || defined(J9X86) || defined(S39064) || defined(PPC64))) \
    || defined(J9ZOS39064)
            ...
            if (!idleGCTuningSupported) {
                vm->vmRuntimeStateListener.idleTuningFlags
                    &= ~(J9_IDLE_TUNING_GC_ON_IDLE | J9_IDLE_TUNING_COMPACT_ON_IDLE);
                if (J9_IDLE_TUNING_IGNORE_UNRECOGNIZED_OPTIONS
                    != (vm->vmRuntimeStateListener.idleTuningFlags & J9_IDLE_TUNING_IGNORE_UNRECOGNIZED_OPTIONS)) {
    ...
    case HEAP_STRUCTURES_INITIALIZED:
        ...
        if (NULL
            == (vm->jniGlobalReferences = pool_new(sizeof(UDATA), 0, 0, POOL_NO_ZERO, J9_GET_CALLSITE(),
                    J9MEM_CATEGORY_JNI, POOL_FOR_PORT(vm->portLibrary)))) {
            goto _error;
        }
        ...
        if (NULL
            == (vm->jniWeakGlobalReferences = pool_new(sizeof(UDATA), 0, 0, POOL_NO_ZERO, J9_GET_CALLSITE(),
                    J9MEM_CATEGORY_JNI, POOL_FOR_PORT(vm->portLibrary))))
            goto _error;

        if (NULL
            == (vm->classLoadingStackPool = pool_new(sizeof(J9ClassLoadingStackElement), 0, 0, 0, J9_GET_CALLSITE(),
                    J9MEM_CATEGORY_CLASSES, POOL_FOR_PORT(vm->portLibrary))))
            goto _error;

        if (NULL
            == (vm->classLoaderBlocks = pool_new(sizeof(J9ClassLoader), 0, 0, 0, J9_GET_CALLSITE(),
                    J9MEM_CATEGORY_CLASSES, POOL_FOR_PORT(vm->portLibrary))))
            goto _error;
        if (J2SE_VERSION(vm) >= J2SE_19) {
            vm->modularityPool = pool_new(OMR_MAX(sizeof(J9Package), sizeof(J9Module)), 0, 0, 0, J9_GET_CALLSITE(),
                J9MEM_CATEGORY_MODULES, POOL_FOR_PORT(vm->portLibrary));
            if (NULL == vm->modularityPool) {
                goto _error;
            }
        }
    ...
    case ALL_VM_ARGS_CONSUMED:
        ...
        if ((argIndex = FIND_AND_CONSUME_ARG(STARTSWITH_MATCH, VMOPT_XXDUMPLOADEDCLASSLIST, NULL)) >= 0) {
            ...
            (*vmHooks)->J9HookRegisterWithCallSite(
                vmHooks, J9HOOK_VM_SHUTTING_DOWN, dumpLoadedClassList, OMR_GET_CALLSITE(), optionValue);
        }
    ...
    case SYSTEM_CLASSLOADER_SET:
        ...
        if ((NULL != vm->dynamicLoadBuffers)
            && (J9_ARE_ANY_BITS_SET(vm->dynamicLoadBuffers->flags, BCU_ENABLE_INVARIANT_INTERNING))) {
            /* vm->sharedClassPreinitConfig is not NULL if shareclasses is enabled */
            if (NULL != vm->sharedClassPreinitConfig) {
                if ((0 == vm->maxInvariantLocalTableNodeCount)
                    && ((0 == vm->sharedClassPreinitConfig->sharedClassInternTableNodeCount)
    ...
    case JIT_INITIALIZED:
...
#ifdef J9VM_RAS_EYECATCHERS
        Trc_VM_Created(vm->mainThread, vm, vm->internalVMFunctions, vm->portLibrary, vm->j9ras);
#endif

#if defined(OMR_THR_YIELD_ALG) && defined(LINUX)
        Trc_VM_yieldAlgorithmSelected(vm->mainThread, j9util_sched_compat_yield_value(vm),
            **(UDATA**)omrthread_global("yieldAlgorithm"), **(UDATA**)omrthread_global("yieldUsleepMultiplier"));
        ...
        Trc_VM_classloaderLocking(
            J9_ARE_NO_BITS_SET(vm->extendedRuntimeFlags, J9_EXTENDED_RUNTIME_CLASSLOADER_LOCKING_ENABLED) ? "disabled"
                                                                                                          : "enabled");
        ...
        if (J9_ARE_ANY_BITS_SET(vm->extendedRuntimeFlags, J9_EXTENDED_RUNTIME_SET_HW_PREFETCH)) {
            ...
            IDATA xxIdx = (xxSetPrefetchNoneIndex > xxSetPrefetchOSDfltIndex) ? xxSetPrefetchNoneIndex
                                                                              : xxSetPrefetchOSDfltIndex;
            UDATA prefetchValue
                = (xxIdx == xxSetPrefetchOSDfltIndex) ? XXSETHWPREFETCH_OS_DEFAULT_VALUE : XXSETHWPREFETCH_NONE_VALUE;
    ...
    case AGENTS_STARTED:
        ...
        if (JNI_OK != initializeXruns(vm)) {
            return J9VMDLLMAIN_SILENT_EXIT_VM; /* We have already complained about the Xrun failure, so no need to say
                                                  "VMInitStages failed" */
        }
        break;
    ...
    case VM_INITIALIZATION_COMPLETE:
        if ((NULL != vm->jitConfig) && (NULL != vm->jitConfig->samplerThread)
...
static UDATA checkArgsConsumed(J9PortLibrary* portLibrary, J9VMInitArgs* j9vm_args)
{
    ...
    xxIgnoreUnrecognizedVMOptionsEnableIndex
        = findArgInVMArgs(PORTLIB, j9vm_args, EXACT_MATCH, VMOPT_XXIGNOREUNRECOGNIZEDVMOPTIONSENABLE, NULL, TRUE);
    if (xxIgnoreUnrecognizedVMOptionsEnableIndex >= 0) {
        xxIgnoreUnrecognizedVMOptionsDisableIndex
            = findArgInVMArgs(PORTLIB, j9vm_args, EXACT_MATCH, VMOPT_XXIGNOREUNRECOGNIZEDVMOPTIONSDISABLE, NULL, TRUE);
    ...
    for (i = 0; i < j9vm_args->nOptions; i++) {
        if (IS_CONSUMABLE(j9vm_args, i) && !IS_CONSUMED(j9vm_args, i)) {
            ...
            /* If ignoreUnrecognized is set to JNI_TRUE, we should not reject any unrecognized options beginning with -X
             * or _ */
            ...
            if (REQUIRES_LIBRARY(j9vm_args, i)) {
                ...
                /* If ignoreUnrecongizedTopLevelOption is set to JNI_TRUE, we should ignore any unrecognized top-level
                 * option */
...
static void checkDllInfo(void* dllLoadInfo, void* userDataTemp)
{
    ...
    if (entry->fatalErrorStr != NULL && strlen(entry->fatalErrorStr) > 0) {
        ...
        if ((entry->loadFlags & FAILED_TO_LOAD)) {
            ...
            if (isInitialization) {
                j9nls_printf(PORTLIB, J9NLS_WARNING, J9NLS_VM_INITIALIZATION_ERROR_IN_FUNCTION, entry->dllName,
                    userData->stage, entry->fatalErrorStr);
            } else {
                j9nls_printf(PORTLIB, J9NLS_WARNING, J9NLS_VM_SHUTDOWN_ERROR_IN_FUNCTION, entry->dllName,
                    userData->stage, entry->fatalErrorStr);
            }
        } else {
            if (isInitialization) {
                j9nls_printf(PORTLIB, J9NLS_WARNING, J9NLS_VM_INITIALIZATION_ERROR_FOR_LIBRARY, entry->dllName,
                    userData->stage, entry->fatalErrorStr);
            } else {
                j9nls_printf(PORTLIB, J9NLS_WARNING, J9NLS_VM_SHUTDOWN_ERROR_FOR_LIBRARY, entry->dllName,
                    userData->stage, entry->fatalErrorStr);
            }
        }

        /* If library failed to unload, or library failed to load and this is not fatal, do not exit */
        if ((entry->loadFlags & FAILED_TO_UNLOAD)
            || ((entry->loadFlags & FAILED_TO_LOAD) && !(entry->loadFlags & FATAL_NO_DLL))) {
...
static void consumeVMArgs(J9JavaVM* vm, J9VMInitArgs* j9vm_args)
{
    ...
    assertOptionFound
        |= findArgInVMArgs(PORTLIB, j9vm_args, OPTIONAL_LIST_MATCH, VMOPT_ENABLE_ASSERTIONS, NULL, TRUE) >= 0;
    assertOptionFound |= findArgInVMArgs(PORTLIB, j9vm_args, OPTIONAL_LIST_MATCH, VMOPT_DA, NULL, TRUE) >= 0;
    assertOptionFound
        |= findArgInVMArgs(PORTLIB, j9vm_args, OPTIONAL_LIST_MATCH, VMOPT_DISABLE_ASSERTIONS, NULL, TRUE) >= 0;
    assertOptionFound |= findArgInVMArgs(PORTLIB, j9vm_args, EXACT_MATCH, VMOPT_ESA, NULL, TRUE) >= 0;
    assertOptionFound
        |= findArgInVMArgs(PORTLIB, j9vm_args, EXACT_MATCH, VMOPT_ENABLE_SYSTEM_ASSERTIONS, NULL, TRUE) >= 0;
    assertOptionFound |= findArgInVMArgs(PORTLIB, j9vm_args, EXACT_MATCH, VMOPT_DSA, NULL, TRUE) >= 0;
    assertOptionFound
        |= findArgInVMArgs(PORTLIB, j9vm_args, EXACT_MATCH, VMOPT_DISABLE_SYSTEM_ASSERTIONS, NULL, TRUE) >= 0;
...
#define SET_FLAG_AT_INDEX(flag, index) \
    j9vm_args->j9Options[index].flags = flag | (j9vm_args->j9Options[index].flags & ARG_MEMORY_ALLOCATION)

    xverify
        = ((xverifyIndex = findArgInVMArgs(PORTLIB, j9vm_args, OPTIONAL_LIST_MATCH, VMOPT_XVERIFY, NULL, FALSE)) >= 0);

    xxverboseverificationIndex
        = findArgInVMArgs(PORTLIB, j9vm_args, EXACT_MATCH, VMOPT_XXVERBOSEVERIFICATION, NULL, FALSE);
    xxnoverboseverificationIndex
        = findArgInVMArgs(PORTLIB, j9vm_args, EXACT_MATCH, VMOPT_XXNOVERBOSEVERIFICATION, NULL, FALSE);
    xxverboseverification = (xxverboseverificationIndex > xxnoverboseverificationIndex);

    xxverifyerrordetailsIndex
        = findArgInVMArgs(PORTLIB, j9vm_args, EXACT_MATCH, VMOPT_XXVERIFYERRORDETAILS, NULL, FALSE);
    xxnoverifyerrordetailsIndex
        = findArgInVMArgs(PORTLIB, j9vm_args, EXACT_MATCH, VMOPT_XXNOVERIFYERRORDETAILS, NULL, FALSE);
    ...
    verbose
        = ((verboseIndex = findArgInVMArgs(PORTLIB, j9vm_args, OPTIONAL_LIST_MATCH, OPT_VERBOSE, NULL, FALSE)) >= 0);
    xverbosegclog = ((xverbosegclogIndex
                         = findArgInVMArgs(PORTLIB, j9vm_args, OPTIONAL_LIST_MATCH, OPT_XVERBOSEGCLOG, NULL, FALSE))
        >= 0);
    xnolinenumbers = (findArgInVMArgs(PORTLIB, j9vm_args, EXACT_MATCH, VMOPT_XNOLINENUMBERS, NULL, FALSE) >= 0);
    xshareclasses = ((xshareclassesIndex
                         = findArgInVMArgs(PORTLIB, j9vm_args, OPTIONAL_LIST_MATCH, VMOPT_XSHARECLASSES, NULL, FALSE))
        >= 0);
    ...
    for (i = (vm_args->nOptions - 1); i >= 0; i--) {
...
#if defined(J9VM_OPT_JVMTI)
        if ((strstr(testString, VMOPT_AGENTLIB_COLON) == testString)
            || (strstr(testString, VMOPT_AGENTPATH_COLON) == testString)) {
        ...
        } else if ((strcmp(testString, VMOPT_XJIT) == 0) || (strstr(testString, VMOPT_XJIT_COLON) == testString)) {
            if (xint || xnojit) {
                SET_FLAG_AT_INDEX(NOT_CONSUMABLE_ARG, i);
            } else {
                ADD_FLAG_AT_INDEX(ARG_REQUIRES_LIBRARY, i);
        ...
        } else if ((strcmp(testString, VMOPT_XAOT) == 0) || (strstr(testString, VMOPT_XAOT_COLON) == testString)) {
            if (xint || xnoaot) {
                SET_FLAG_AT_INDEX(NOT_CONSUMABLE_ARG, i);
            } else {
                xaot = TRUE;
 ...
 */
    ...
    if (!((xnojit && xnoaot) || xint) || xjit) {
        ...
        if (xxjitdirectoryIndex >= 0) {
            ...
            UDATA openFlags = (entry->loadFlags & XRUN_LIBRARY) ? J9PORT_SLOPEN_DECORATE | J9PORT_SLOPEN_LAZY
                                                                : J9PORT_SLOPEN_DECORATE;
            UDATA jitFileHandle = 0;
            UDATA rc = 0;

            optionValueOperations(PORTLIB, j9vm_args, xxjitdirectoryIndex, GET_OPTION, &jitdirectoryValue, 0, '=', 0,
                NULL); /* get option value for xxjitdirectory= */
            ...
            j9str_printf(PORTLIB, dllCheckPathPtr, expectedPathLength, "%s%s%s", jitdirectoryValue, DIR_SEPARATOR_STR,
                entry->dllName);
    ...
    if (xverify) {
        ...
        optionValueOperations(PORTLIB, j9vm_args, xverifyIndex, GET_OPTION, &optionValue, 0, ':', 0,
            NULL); /* get option value for xverify: */
    } else {
        optionValue = NULL;
    }
...
#if defined(J9VM_INTERP_VERBOSE)
    if (verbose) {
        optionValueOperations(PORTLIB, j9vm_args, verboseIndex, GET_OPTION, &optionValue, 0, ':', 0,
            NULL); /* get option value for verbose: */
        if (!(optionValue != NULL && strcmp(optionValue, OPT_NONE) == 0)) {
    ...
    if (xshareclasses) {
        ...
        do {
            option_rc = optionValueOperations(
                PORTLIB, j9vm_args, xshareclassesIndex, GET_OPTIONS, &optionsBufferPtr, buflen, ':', ',', NULL);
...
static jint processVMArgsFromFirstToLast(J9JavaVM* vm)
{
    ...
    vm->extendedRuntimeFlags |= (UDATA)J9_EXTENDED_RUNTIME_CLASSLOADER_LOCKING_ENABLED
        | J9_EXTENDED_RUNTIME_REDUCE_CPU_MONITOR_OVERHEAD; /* enabled by default */
    vm->extendedRuntimeFlags
        |= (UDATA)J9_EXTENDED_RUNTIME_ENABLE_CPU_MONITOR; /* Cpu monitoring is enabled by default */
    vm->extendedRuntimeFlags
        |= (UDATA)J9_EXTENDED_RUNTIME_ALLOW_CONTENDED_FIELDS; /* Allow contended fields on bootstrap classes */
    if (J2SE_VERSION(vm) >= J2SE_19) {
        vm->extendedRuntimeFlags
            |= (UDATA)J9_EXTENDED_RUNTIME_RESTRICT_IFA; /* Enable zAAP switching for Registered Natives and JVMTI
                                                           callbacks by default in Java 9 and later. */
    }
    ...
    for (i = 0; i < vm_args->nOptions; ++i) {
        ...
        if (strcmp(testString, VMOPT_XXNOSTACKTRACEINTHROWABLE) == 0) {
...
#if defined(J9ZOS390)
            PORT_ACCESS_FROM_JAVAVM(vm);
            /* Disabling this option on z/OS as this introduces a 50% startup regression and smaller throughput
             * regresssions */
            j9nls_printf(PORTLIB, J9NLS_ERROR, J9NLS_VM_NO_REDUCE_CPU_OVERHEAD_UNSUPPORTED_ZOS,
                VMOPT_XXNOREDUCECPUMONITOROVERHEAD);
            return JNI_ERR;
#else
            ...
            == strcmp(testString, VMOPT_XXNORESTRICTCONTENDED)) { /* enabling application contended fields implicitly
                                                                     turns on bootstrap contended fields */
            vm->extendedRuntimeFlags |= (UDATA)J9_EXTENDED_RUNTIME_ALLOW_APPLICATION_CONTENDED_FIELDS;
            vm->extendedRuntimeFlags |= (UDATA)J9_EXTENDED_RUNTIME_ALLOW_CONTENDED_FIELDS;
        } else if (0
            == strcmp(testString, VMOPT_XXNOCONTENDEDFIELDS)) { /* disabling bootstrap contended fields implicitly turns
                                                                   off application contended fields  */
            vm->extendedRuntimeFlags &= ~(UDATA)J9_EXTENDED_RUNTIME_ALLOW_CONTENDED_FIELDS;
            vm->extendedRuntimeFlags &= ~(UDATA)J9_EXTENDED_RUNTIME_ALLOW_APPLICATION_CONTENDED_FIELDS;
            ...
            vm->extendedRuntimeFlags
                &= ~(UDATA)(J9_EXTENDED_RUNTIME_OSR_SAFE_POINT | J9_EXTENDED_RUNTIME_OSR_SAFE_POINT_FV);
...
static void runJ9VMDllMain(void* dllLoadInfo, void* userDataTemp)
{
    ...
    if (userData->filterFlags == 0 || ((userData->filterFlags & entry->loadFlags) == userData->filterFlags)) {
        ...
        /* Loaded libraries are guaranteed to have a descriptor. NOT_A_LIBRARY entries will already have a j9vmdllmain
         * entry */
        if (!entry->j9vmdllmain && entry->descriptor) {
            if (j9sl_lookup_name(entry->descriptor, "J9VMDllMain", (void*)&J9VMDllMainFunc, "PLpL")) {
                entry->fatalErrorStr = (char*)j9nls_lookup_message(
                    J9NLS_DO_NOT_APPEND_NEWLINE | J9NLS_ERROR, J9NLS_VM_J9VMDLLMAIN_NOT_FOUND, NULL);
                return;
            } else {
        ...
        if (entry->j9vmdllmain) {
            ...
            if (rc == J9VMDLLMAIN_FAILED && (entry->fatalErrorStr == NULL || strlen(entry->fatalErrorStr) == 0)) {
                entry->fatalErrorStr = (char*)j9nls_lookup_message(
                    J9NLS_DO_NOT_APPEND_NEWLINE | J9NLS_ERROR, J9NLS_VM_J9VMDLLMAIN_FAILED, NULL);
            }
 ...
 */
static void runShutdownStageJvmtiFirst(J9JavaVM* vm, RunDllMainData* userData)
{
...
static jint runShutdownStage(J9JavaVM* vm, IDATA stage, void* reserved, UDATA filterFlags)
{
    ...
    userData.filterFlags
        = filterFlags; /* only run J9VMDllMain for DLL table entries whos flags match ALL filterFlags */
...
static void registerIgnoredOptions(J9PortLibrary* portLibrary, J9VMInitArgs* j9vm_args)
{
    UDATA i;
    char* option;
    const struct J9VMIgnoredOption* ignoredOptionTablePtr
        = (const struct J9VMIgnoredOption*)GLOBAL_TABLE(ignoredOptionTable);
    ...
    for (i = 0; i < ignoredOptionTableSize; i++) {
        findArgInVMArgs(
            PORTLIB, j9vm_args, ignoredOptionTablePtr[i].match, ignoredOptionTablePtr[i].optionName, NULL, TRUE);
    }
}
 ...
 */
static IDATA setJ9ThreadSchedulingAlgorithm(J9JavaVM* javaVM)
{
...
threadInitStages(J9JavaVM* vm, IDATA stage, void* reserved)
{
    ...
    case PORT_LIBRARY_GUARANTEED:
        ...
        if (0
            != (parseError
                   = setMemoryOptionToOptElse(vm, &(vm->defaultOSStackSize), VMOPT_XMSO, J9_OS_STACK_SIZE, TRUE))) {
        ...
        if (0
            != (parseError
                   = setMemoryOptionToOptElse(vm, &(vm->initialStackSize), VMOPT_XISS, J9_INITIAL_STACK_SIZE, TRUE))) {
            parseErrorOption = VMOPT_XISS;
            goto _memParseError;
        }
        if (0
            != (parseError = setMemoryOptionToOptElse(
                    vm, &(vm->stackSizeIncrement), VMOPT_XSSI, J9_STACK_SIZE_INCREMENT, TRUE))) {
        ...
        if (J2SE_VERSION(vm) >= J2SE_19) {
            ...
            while (argIndex1 >= 0) {
                ...
                if (strcmp(optionString, VMOPT_XZERO) == 0) {
                    ...
                    /* If the option does not start with -Xzero:, do not consume it so it later causes option
                     * unrecognised */
                    argIndex1 = FIND_NEXT_ARG_IN_VMARGS_FORWARD(STARTSWITH_MATCH, VMOPT_XZERO, NULL, argIndex1);
                    continue;
                }
...
#if (defined(J9VM_OPT_SIDECAR))
/* If a command-line option exists that must be mapped to a J9 option, this creates the mapping struct and associates it
   with the option

        This function can be called from the internal function table so that libraries themselves can register their own
   mappings. This must be done at the earliest opportunity possible stage.
*/
...
IDATA registerCmdLineMapping(J9JavaVM* vm, char* sov_option, char* j9_option, UDATA mapFlags)
{
    ...
    if (mapFlags & EXACT_MAP_NO_OPTIONS) {
        match = EXACT_MATCH;
    } else if (mapFlags & EXACT_MAP_WITH_OPTIONS) {
        match = STARTSWITH_MATCH;
    } else if (mapFlags & STARTSWITH_MAP_NO_OPTIONS) {
        match = STARTSWITH_MATCH;
    } else if (mapFlags & MAP_TWO_COLONS_TO_ONE) {
        match = STARTSWITH_MATCH;
    } else if (mapFlags & MAP_ONE_COLON_TO_TWO) {
        match = STARTSWITH_MATCH;
    } else if (mapFlags & MAP_WITH_INCLUSIVE_OPTIONS) {
        match = STARTSWITH_MATCH;
    } else if (mapFlags & MAP_MEMORY_OPTION) {
        match = EXACT_MEMORY_MATCH;
    } else if (mapFlags & STARTSWITH_INVALID_OPTION) {
        match = STARTSWITH_MATCH;
    } else if (mapFlags & EXACT_INVALID_OPTION) {
        match = EXACT_MATCH;
    }

    /* Walk the command-line and create a mapping for each instance of the option */
    do {
        /* Note: (index << STOP_AT_INDEX_SHIFT) is copied from FIND_NEXT_ARG_IN_VMARGS which we can't use here as we
         * have no function table yet */
        index = findArgInVMArgs(
            PORTLIB, vm->vmArgsArray, (match | (index << STOP_AT_INDEX_SHIFT)), sov_option, NULL, FALSE);
    ...
    } while (index > 0); /* If index == 0, this would cause findArgInVMArgs to search from the start again, which would
...
static IDATA registerVMCmdLineMappings(J9JavaVM* vm)
{
...
#ifdef J9VM_OPT_JVMTI
    if (registerCmdLineMapping(
            vm, MAPOPT_JAVAAGENT_COLON, MAPOPT_AGENTLIB_INSTRUMENT_EQUALS, MAP_WITH_INCLUSIVE_OPTIONS)
        == RC_FAILED) {
        return RC_FAILED;
    }
#endif
    ...
    if (registerCmdLineMapping(vm, SYSPROP_DJAVA_COMPILER_EQUALS, VMOPT_XINT, STARTSWITH_MAP_NO_OPTIONS)
        == RC_FAILED) { /* any other -Djava.compiler= found is mapped to -Xint */
        return RC_FAILED;
    }
    if (registerCmdLineMapping(vm, MAPOPT_XDISABLEJAVADUMP, MAPOPT_XDUMP_JAVA_NONE, EXACT_MAP_NO_OPTIONS)
        == RC_FAILED) {
        return RC_FAILED;
    }
    ...
    if (registerCmdLineMapping(vm, MAPOPT_XHEALTHCENTER, VMOPT_AGENTLIB_HEALTHCENTER, EXACT_MAP_NO_OPTIONS)
        == RC_FAILED) {
        return RC_FAILED;
    }
    if (registerCmdLineMapping(
            vm, MAPOPT_XHEALTHCENTER_COLON, VMOPT_AGENTLIB_HEALTHCENTER_EQUALS, MAP_WITH_INCLUSIVE_OPTIONS)
        == RC_FAILED) {
        return RC_FAILED;
    }
    /* Need to support -Xsoftrefthreshold (lowercase) for backwards compatibility */
    if (registerCmdLineMapping(vm, MAPOPT_XSOFTREFTHRESHOLD, VMOPT_XSOFTREFTHRESHOLD, STARTSWITH_MAP_NO_OPTIONS)
        == RC_FAILED) {
        return RC_FAILED;
    }
    ...
    if (registerCmdLineMapping(vm, MAPOPT_XSHARE_OFF, MAPOPT_XSHARECLASSES_UTILITIES, EXACT_MAP_NO_OPTIONS)
        == RC_FAILED) {
        return RC_FAILED;
    }
    /* Map -Xshare:auto to -Xshareclasses:nonfatal */
    if (registerCmdLineMapping(vm, MAPOPT_XSHARE_AUTO, MAPOPT_XSHARECLASSES_NONFATAL, EXACT_MAP_NO_OPTIONS)
        == RC_FAILED) {
        return RC_FAILED;
    }
    /* Map -XX:+DisableExplicitGC to -Xdisableexplicitgc */
    if (registerCmdLineMapping(vm, MAPOPT_XXDISABLEEXPLICITGC, "-Xdisableexplicitgc", EXACT_MAP_NO_OPTIONS)
        == RC_FAILED) {
        return RC_FAILED;
    }
    /* Map -XX:+EnableExplicitGC to -Xenableexplicitgc */
    if (registerCmdLineMapping(vm, MAPOPT_XXENABLEEXPLICITGC, "-Xenableexplicitgc", EXACT_MAP_NO_OPTIONS)
        == RC_FAILED) {
        return RC_FAILED;
    }
    /* Map -XX:HeapDumpPath= to -Xdump:directory= */
    if (registerCmdLineMapping(vm, MAPOPT_XXHEAPDUMPPATH_EQUALS, VMOPT_XDUMP_DIRECTORY_EQUALS, EXACT_MAP_WITH_OPTIONS)
        == RC_FAILED) {
        return RC_FAILED;
    }
...
#define PASS ("Passed ")
#define SET_TO(element, string)                                            \
    vm->vmArgsArray->actualVMArgs->options[element].optionString = string; \
    printf("\nTesting: %s \t\t", string)
...
#define REMOVE_MAPPING(element)                                     \
    j9mem_free_memory(vm->vmArgsArray->j9Options[element].mapping); \
    vm->vmArgsArray->j9Options[element].mapping = NULL
    ...
    TEST_INT(intResult, 2);
    intResult = FIND_ARG_IN_VMARGS(
        STARTSWITH_MATCH, "-Xfooj92", "wobble"); /* this is not considered a value as we're mapping 2 colons to 1 */
    TEST_INT(intResult, -1);
    REMOVE_MAPPING(2);
    SET_TO(2, "");
...
#define REMOVE_MAPPING(element)                                     \
    ...
    TEST_INT(returnVal, OPTION_OK);
    COMPARE(result,
        "wobble"); /* With exact map, we don't want the sov option value, we actually want the j9 option value */
    memset(copyBufferPtr, 0, copyBufSize);
    returnVal = COPY_OPTION_VALUE(1, ':', &copyBufferPtr, copyBufSize);
    TEST_INT(returnVal, OPTION_OK);
    ...
    vm->vmArgsArray->j9Options[1].fromEnvVar
        = "anEnvVar"; /* Since this arg is now from an environment variable, it should be included */
    memset(copyBufferPtr, 0, copyBufSize);
    returnVal = GET_COMPOUND_VALUE(6, ':', &copyBufferPtr, copyBufSize);
    TEST_INT(returnVal, OPTION_OK);
...
static IDATA checkDjavacompiler(J9PortLibrary* portLibrary, J9VMInitArgs* j9vm_args)
{
    ...
    if (index >= 0) {
        ...
        if (jitValue && strcmp(jitValue, OPT_NONE) != 0 && strcmp(jitValue, OPT_NONE_CAPS) != 0
            && strcmp(jitValue, "") != 0 && strcmp(jitValue, J9_JIT_DLL_NAME) != 0
...
#if defined(WIN32)
static void findLargestUnallocatedRegion(UDATA* regionStart, UDATA* regionSize)
{
...
static IDATA preloadUser32Dll(J9JavaVM* vm)
{
    PORT_ACCESS_FROM_JAVAVM(vm);
    ...
    if (preloadUser32) {
        ...
        if (0 != contiguousRegionStart) {
            contiguousRegionStart
                = (UDATA)VirtualAlloc((LPVOID)contiguousRegionStart, contiguousRegionSize, MEM_RESERVE, PAGE_NOACCESS);
        }
...
static UDATA protectedInitializeJavaVM(J9PortLibrary* portLibrary, void* userData)
{
    ...
    if (J9_ARE_NO_BITS_SET(vm->sigFlags, J9_SIG_XRS_ASYNC)) {
         ...
         */
        if (0
            != j9sig_set_async_signal_handler(
                   shutDownHookWrapper, vm, J9PORT_SIG_FLAG_SIGTERM | J9PORT_SIG_FLAG_SIGINT)) {
            goto error;
        }
    }
#endif
    ...
    registerIgnoredOptions(
        PORTLIB, vm->vmArgsArray); /* Tags -D java options and options in ignoredOptionTable as not consumable */
    ...
    if (0
        != (*vmHooks)->J9HookRegisterWithCallSite(
               vmHooks, J9HOOK_VM_CLASS_UNLOAD, jniIDTableClassUnload, OMR_GET_CALLSITE(), NULL)) {
        goto error;
    }
#endif

    /* env is not used, but must be passed for compatibility */
    /* use NO_OBJECT, because it's too early to allocate an object -- we'll take care of that later in standardInit() or
     * tinyInit() */
...
#if !defined(WIN32)
static UDATA sigxfszHandler(struct J9PortLibrary* portLibrary, U_32 gpType, void* gpInfo, void* userData) { return 0; }
#endif
...
static UDATA initializeVprintfHook(J9JavaVM* vm)
{
    ...
    if (index >= 0) {
        ...
        *vprintfHookFunctionPtr
            = (jint(JNICALL*)(FILE*, const char*, va_list))vm->vmArgsArray->actualVMArgs->options[index].extraInfo;
...
static void setSignalOptions(J9JavaVM* vm)
{
    ...
    if ((argIndex = FIND_AND_CONSUME_ARG(OPTIONAL_LIST_MATCH, VMOPT_XRS, NULL)) >= 0) {
        ...
        if ((NULL != optionValue) && (0 == strcmp(optionValue, "sync"))) {
            ...
            sigOptions
                |= (J9PORT_SIG_OPTIONS_REDUCED_SIGNALS_SYNCHRONOUS | J9PORT_SIG_OPTIONS_REDUCED_SIGNALS_ASYNCHRONOUS);
        }
    }
...
static IDATA updateJavaAgentClasspath(J9JavaVM* vm)
{
    ...
    index = findArgInVMArgs(
        PORTLIB, vm->vmArgsArray, (STARTSWITH_MATCH | SEARCH_FORWARD), MAPOPT_JAVAAGENT_COLON, NULL, FALSE);
    if (index >= 0) {
        ...
        do {
            ...
            result = optionValueOperations(
                PORTLIB, vm->vmArgsArray, index, GET_OPTION, &cursor, bufSizeLeft, ':', 0, NULL);
            ...
            cursor += charsToKeep;
            /* Monstrous ORing/shifting guff copied from jvminit.h - can't use the macro yet as no internal function
             * table */
            index = findArgInVMArgs(PORTLIB, vm->vmArgsArray,
                ((STARTSWITH_MATCH | ((index + 1) << STOP_AT_INDEX_SHIFT)) | SEARCH_FORWARD), MAPOPT_JAVAAGENT_COLON,
                NULL, FALSE);
        } while (index >= 0);
 ...
 * @return void
 */
static void signalDispatch(J9VMThread* vmThread, I_32 signal)
{
    J9JavaVM* vm = vmThread->javaVM;
...
static jint initializeDDR(J9JavaVM* vm)
{
    ...
    if (NULL != j9ddrDatDir) {
        filename = j9mem_allocate_memory(strlen(j9ddrDatDir) + 1 + sizeof(J9DDR_DAT_FILE),
            OMRMEM_CATEGORY_VM); /* +1 for slash, sizeof includes nul char already */
        if (filename == NULL) {
            rc = JNI_ENOMEM;
            goto done;
        }
...
#undef CPU_NAME_SIZE
    ...
    if (0 == j9_cmdla_strnicmp(cpu_name, "403", 3))
        return FALSE;
    if (0 == j9_cmdla_strnicmp(cpu_name, "405", 3))
        return FALSE;
    if (0 == j9_cmdla_strnicmp(cpu_name, "440GP", 5))
        return FALSE;
    if (0 == j9_cmdla_strnicmp(cpu_name, "601", 3))
        return FALSE;
    if (0 == j9_cmdla_strnicmp(cpu_name, "603", 3))
        return FALSE;
    if (0 == j9_cmdla_strnicmp(cpu_name, "7400", 4))
        return FALSE;
    if (0 == j9_cmdla_strnicmp(cpu_name, "82xx", 4))
        return FALSE;
    if (0 == j9_cmdla_strnicmp(cpu_name, "750FX", 5))
        return FALSE;
    if (0 == j9_cmdla_strnicmp(cpu_name, "604", 3))
        return FALSE;
#endif /* AIXPPC */
...
void installJitBytecodes(J9JavaVM* javaVM) {}

void initializeExecutionModel(J9VMThread* currentThread)
{
