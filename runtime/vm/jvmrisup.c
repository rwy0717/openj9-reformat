...
static const char* rasinfoNames[]
    = { "Get types of RAS information available", "Get trace component names", "Get trace component information" };
...
static void JNICALL rasDynamicVerbosegc(
    JNIEnv* env, int vgc_switch, int vgccon, char* file_path, int number_of_files, int number_of_cycles);
static int JNICALL rasRunDumpRoutine(
    JNIEnv* env, int componentID, int level, void (*printrtn)(void* env, const char* tagName, const char* fmt, ...));
...
static void rasListenerWrapper(void* userData, OMR_VMThread* env, void** thrLocal, const char* modName, U_32 traceId,
...
static int JNICALL rasGetRasInfo(JNIEnv* env, RasInfo* info_ptr)
{
    ...
    case RASINFO_TRACE_COMPONENT:
        ...
        rc = utserverinterface->omrf.GetComponent(info_ptr->info.trace_component.name,
            &(info_ptr->info.trace_component.bitMap), &(info_ptr->info.trace_component.first),
...
static int JNICALL rasInjectSigsegv(JNIEnv* env)
{
    ...
    j9sig_protect(protectedInjectSigsegv, NULL, vmThread->javaVM->internalVMFunctions->structuredSignalHandler,
...
static int J9THREAD_PROC rasThreadStartFuncWrapper(void* arg_struct)
{
    ...
    j9sig_protect(rasThreadProtectedStartFuncWrapper, arg_struct, vm->internalVMFunctions->structuredSignalHandler,
...
static int rasCreateThreadImmediately(J9JavaVM* vm, void(JNICALL* startFunc)(void*), void* args)
{
    ...
    ret = (int)vm->internalVMFunctions->createThreadWithCategory(
        &thread, 0, J9THREAD_PRIORITY_NORMAL, 0, rasThreadStartFuncWrapper, &t_args, J9THREAD_CATEGORY_SYSTEM_THREAD);
