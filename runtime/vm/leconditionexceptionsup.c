...
long getReturnAddressOfJNINative(void* builderDSA)
{
    ...
    while ((dsaptr != builderDSA)) {
        ...
        dsaptr = __dsa_prev(
            dsaptr, __EDCWCCWI_PHYSICAL, (int)dsa_format, NULL, NULL, (int*)&callers_dsa_format, NULL, NULL);
     ...
     *
     * 			Bug 1: 		call_instruction_address (of RUNCALLINMETHOD) is not correct if the callee is XPLINK and had used
     * CEEHDLR Workaround: set dsa_ptr to callee (JNI native) and use the callers_call_instruction_address
     *
    ...
    /* call CEETBCK */
    ceetbck(&ceetbckDSAPtr, &ceetbckDSAFormat, &caaptr, &member_id, program_unit_name, &program_unit_name_length,
        &program_unit_address, &call_instruction_address, entryName, &entryNameLength, &entry_address,
    ...
    /* The callout instruction is one of BASR, BALR or BASSM, as documented by CEETBCK in "z/OS Language Environment
     * Vendor Interfaces " The size of each of these instructions is 2 bytes, per "The Enterprise Systems
     * Architecture/390 Principles of Operation " */
    ...
    ceetbckTargetAddress
        = ceetbckTargetAddress | 0x80000000; /* __far_jump() expects 31-bit z/OS addresses to the have high bit */
#endif
 ...
 * @param ceArgs		the parameters required by the constructor for
 * com.ibm.le.conditionhandling.ConditionException.ConditionException. this function allocates memory for
 * ceArgs->failingRoutine. It is the responsibility of the caller to free this memory If ceArgs is not NULL, the memory
    ...
    struct J9PortLibrary* portLibrary, void* gpInfo, struct ConditionExceptionConstructorArgs* ceArgs)
{
    ...
    if (_FBCHECK(rcFc, CEE000) == 0) {
        ...
        /* CEE3GRN left justifies the name and fills everything to the right of it with blanks, so the result of
         * e2a_func is a char array that is the same size as nameEBCDIC */
    ...
    infoType
        = j9sig_info(gpInfo, J9PORT_SIG_SIGNAL, J9PORT_SIG_SIGNAL_ZOS_CONDITION_FEEDBACK_TOKEN, &infoName, &infoValue);
