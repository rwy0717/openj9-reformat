...
static UDATA lswAnnotationHash(void* key, void* userData)
{
...
static UDATA lswAnnotationHashEqual(void* leftKey, void* rightKey, void* userData)
{
...
lswInitialize(J9JavaVM* vm, J9StackWalkState* walkState)
{
    ...
    annotationPool = pool_new(sizeof(J9SWFrame), 0, 0, POOL_ALWAYS_KEEP_SORTED, J9_GET_CALLSITE(), OMRMEM_CATEGORY_VM,
    ...
    stringPool = pool_new(LSW_STRING_POOL_MAX, 0, 0, POOL_ALWAYS_KEEP_SORTED, J9_GET_CALLSITE(), OMRMEM_CATEGORY_VM,
        POOL_FOR_PORT(vm->portLibrary));
    if (NULL == stringPool) {
...
static char* lswDescribeObject(J9VMThread* vmThread, char* dataPtr, J9Class** class, J9Object* object)
{
    ...
    if (J9VMJAVALANGCLASS_OR_NULL(vm) == *class) {
        ...
        *class = (J9Class*)J9OBJECT_ADDRESS_LOAD(
            vm, object, J9VMCONSTANTPOOL_ADDRESS_OFFSET(vm, J9VMCONSTANTPOOL_JAVALANGCLASS_VMREF));
...
static void lswDescribeSlot(
    J9VMThread* vmThread, J9StackWalkState* walkState, J9SWSlot* slot, char* data, UDATA dataSize)
{
    J9Class* class;
    J9Object* object;
    J9UTF8* className;
...
static IDATA lswPrintFrameSlots(J9VMThread* vmThread, J9StackWalkState* walkState, J9SWFrame* frame)
{
    UDATA i;
    IDATA topSlotIndex;
    UDATA slotCount;
...
#endif
            frame->topSlot + i, slot->data, slot->name, annotation ? annotation->description : "", stringBuf);
    }
    ...
    while (frame) {
        ...
        if (method) {
            ...
            lswPrintf(privatePortLibrary, "[%s%-20s%s] j9method 0x%x  %s%.*s.%.*s%.*s%s\n", CYAN,
                getFrameName(frame, frameName), COL_RESET, frame->method, CYAN, (U_32)J9UTF8_LENGTH(className),
                J9UTF8_DATA(className), (U_32)J9UTF8_LENGTH(name), J9UTF8_DATA(name), (U_32)J9UTF8_LENGTH(sig),
                J9UTF8_DATA(sig), COL_RESET);
        } else {
        ...
        case LSW_FRAME_TYPE_JIT_INLINE:
            lswPrintf(privatePortLibrary,
                "\tpc = 0x%08lx  cp = 0x%08lx  jitinfo = 0x%08lx  bc index = %d  inlineDepth = %d  PC offset = 0x%lx\n",
                frame->pc, frame->cp, frame->ftd.jit.jitInfo, frame->ftd.jit.bytecodeIndex, frame->ftd.jit.inlineDepth,
                frame->ftd.jit.pcOffset);
            break;

        case LSW_FRAME_TYPE_JNI_NATIVE_METHOD:
            lswPrintf(privatePortLibrary,
                "\tpc (frame type) = 0x%08lx  cp = 0x%08lx  literals (obj bytes pushed) = 0x%08lx  flags = 0x%08lx\n",
...
static IDATA lswSanityCheck(J9JavaVM* vm, J9StackWalkState* walkState)
{
    ...
    if (slotCount != (lastBottomSlot - slotWalker->sp + 1)) {
        ...
        lswPrintf(privatePortLibrary, "***********************************************************************\n");
        lswPrintf(
            privatePortLibrary, "slotCount = 0x%x  should be 0x%x\n", slotCount, lastBottomSlot - slotWalker->sp + 1);
        lswPrintf(privatePortLibrary, "***********************************************************************\n");
    }
...
static IDATA getSlotIndex(J9StackWalkState* walkState, UDATA* addr)
{
    ...
    if (slotIndex > ((UDATA)slotWalker->stackBottom - (UDATA)slotWalker->sp)) {
        ...
        lswPrintf(privatePortLibrary, "OUT OF BOUNDS LSW SLOT ACCESS [addr=%p not between %p and %p]\n", addr,
            slotWalker->stackBottom, slotWalker->sp);
        return -1;
    }
...
static IDATA allocateStringPoolSlab(J9SlotWalker* slotWalker)
{
...
static char* lswStrDup(J9SlotWalker* slotWalker, char* str)
{
    ...
    if ((NULL == slotWalker->stringSlab)
        || (stringSize >= (UDATA)(slotWalker->stringSlab + LSW_STRING_POOL_MAX - slotWalker->stringFree))) {
...
static void lswPrintf(struct J9PortLibrary* privatePortLibrary, const char* format, ...)
{
    char buf[LSW_STRING_MAX];
    va_list args;
