...
static char* defaultMethodConflictExceptionMessage(J9VMThread* currentThread, J9Class* targetClass, UDATA nameLength,
    U_8* name, UDATA sigLength, U_8* sig, J9Method** methods, UDATA methodsLength);
static J9Method* searchClassForMethodCommon(
    J9Class* clazz, U_8* name, UDATA nameLength, U_8* sig, UDATA sigLength, BOOLEAN partialMatch);
static J9Method* javaResolveInterfaceMethods(J9VMThread* currentThread, J9Class* targetClass,
    J9ROMNameAndSignature* nameAndSig, J9Class* senderClass, UDATA lookupOptions, J9InterfaceResolveData* data);
        ...
        if (J9_ARE_ALL_BITS_SET(romClass->extraModifiers, J9AccClassUseBisectionSearch)) {
            ...
            do {
                ...
                IDATA result = partialMatch
                    ? compareMethodNameAndPartialSignature(name, (U_16)nameLength, sig, (U_16)sigLength,
                          J9UTF8_DATA(nameUTF), J9UTF8_LENGTH(nameUTF), J9UTF8_DATA(sigUTF), J9UTF8_LENGTH(sigUTF))
                    : compareMethodNameAndSignature(name, (U_16)nameLength, sig, (U_16)sigLength, J9UTF8_DATA(nameUTF),
...
static J9Method* processMethod(J9VMThread* currentThread, UDATA lookupOptions, J9Method* method, J9Class* methodClass,
    UDATA* exception, J9Class** exceptionClass, IDATA* errorType, J9ROMNameAndSignature* nameAndSig,
    J9Class* senderClass, J9Class* targetClass)
{
    ...
    if (J9_ARE_NO_BITS_SET(lookupOptions, J9_LOOK_NO_VISIBILITY_CHECK) && (NULL != senderClass)
        && (!J9ROMCLASS_IS_UNSAFE(senderClass->romClass))) {
        ...
        if (methodClass->packageID != senderClass->packageID) {
            if (J9_ARE_ANY_BITS_SET(modifiers, J9AccProtected) && (J9_ARE_NO_BITS_SET(modifiers, J9AccStatic))) {
    ...
    if (lookupOptions & J9_LOOK_CLCONSTRAINTS) {
        if (vm->runtimeFlags & J9_RUNTIME_VERIFY) {
            ...
            if (cl1 != cl2) {
                ...
                if (j9bcv_checkClassLoadingConstraintsForSignature(currentThread, cl1, cl2, lookupSig, methodSig)
                    != 0) {
 ...
 *
 * ***The caller must free the memory from the pointer data->errorModuleMsg after invoking setCurrentExceptionUTF() with
 * this pointer***
 *
    ...
    J9ROMNameAndSignature* nameAndSig, J9Class* senderClass, UDATA lookupOptions, J9InterfaceResolveData* data)
{
    ...
    while (iTable != NULL) {
        ...
        if (interfaceClass == targetClass) {
            J9Method* foundMethod = searchClassForMethodCommon(interfaceClass, name, nameLength, sig, sigLength,
                J9_ARE_ANY_BITS_SET(lookupOptions, J9_LOOK_PARTIAL_SIGNATURE));
            if (NULL != foundMethod) {
                /* As per spec, prevent super-interface private or static methods from being processed */
                if (J9_ARE_NO_BITS_SET(
                        J9_ROM_METHOD_FROM_RAM_METHOD(foundMethod)->modifiers, J9AccPrivate | J9AccStatic)) {
                    resultMethod
                        = processMethod(currentThread, lookupOptions, foundMethod, interfaceClass, &data->exception,
                            &data->exceptionClass, &data->errorType, nameAndSig, senderClass, targetClass);
                    return resultMethod;
                }
            }
    ...
    while (iTable != NULL) {
        ...
        foundMethod = searchClassForMethodCommon(interfaceClass, name, nameLength, sig, sigLength,
            J9_ARE_ANY_BITS_SET(lookupOptions, J9_LOOK_PARTIAL_SIGNATURE));
        ...
        if (NULL != foundMethod) {
            ...
            newMethod = processMethod(currentThread, lookupOptions, foundMethod, interfaceClass, &data->exception,
                &data->exceptionClass, &data->errorType, nameAndSig, senderClass, targetClass);

            /* Exit loop if exception have been directly set since direct exception must be from access control
             * (Nestmates) */
            ...
            if (shouldAdd) {
                ...
                if (!added) {
                    ...
                    if ((maxUsedSlotIndex + 1) >= arrayLength) {
                        ...
                        J9Method** newArray
                            = j9mem_allocate_memory(sizeof(J9Method*) * newArrayLength, OMRMEM_CATEGORY_VM);
                        if (NULL == newArray) {
...
doneItableSearch:
    if (1 == numElements) {
        ...
        for (; i <= maxUsedSlotIndex; i++) {
            ...
            if (NULL != current) {
                BOOLEAN abstract
                    = J9_ARE_ANY_BITS_SET(J9_ROM_METHOD_FROM_RAM_METHOD(current)->modifiers, J9AccAbstract);
                if (!abstract) {
                    if (foundDefault) {
 ...
 * 		J9_LOOK_INTERFACE						Search for an interface method - assumes given class
 *is interface class and searches only it and its superinterfaces (i.e. does not search Object) J9_LOOK_JNI
 *Use JNI behaviour (allow virtual lookup to succeed when it finds a method in an interface class, throw only
 *NoSuchMethodError on failure, VERIFIED C strings for name and sig)
 * 		J9_LOOK_NO_CLIMB						Search only the given class, no superclasses or
 *superinterfaces J9_LOOK_NO_INTERFACE					Do not search superinterfaces J9_LOOK_NO_THROW
 *Do not set the exception if lookup fails
 * 		J9_LOOK_NO_JAVA							Do not run java code for any reason (implies
 *NO_THROW)
 * 		J9_LOOK_NEW_INSTANCE					Use newInstance behaviour (translate IllegalAccessError
 *-> IllegalAccessException, all other errors -> InstantiationException) J9_LOOK_DIRECT_NAS
 *NAS contains direct pointers to UTF8, not SRPs (this option is mutually exclusive with lookupOptionsJNI)
 * 		J9_LOOK_CLCONSTRAINTS					Check that the found method doesn't violate any class
 *loading constraints between the found class and the sender class.
 * 		J9_LOOK_PARTIAL_SIGNATURE				Allow the search to match a partial signature
 *		J9_LOOK_ALLOW_FWD						Allow lookup to follow the forwarding
 *chain J9_LOOK_NO_VISIBILITY_CHECK				Do not perform any visilbity checking
 *		J9_LOOK_NO_JLOBJECT						When doing an interface lookup, do not consider
 *method in java.lang.Object
 *		J9_LOOK_REFLECT_CALL					Use reflection behaviour when dealing with module
 *visilibity J9_LOOK_HANDLE_DEFAULT_METHOD_CONFLICTS	Handle default method conflicts
 *		J9_LOOK_IGNORE_INCOMPATIBLE_METHODS		If a static/virtual conflict occurs, ignore and move on to
 *the next class
 *
 *  	If J9_LOOK_JNI is not specified, virtual and static lookup will fail with AbstractMethodError if the method is
 *found in an interface class.
 *
...
UDATA
javaLookupMethod(J9VMThread* currentThread, J9Class* targetClass, J9ROMNameAndSignature* nameAndSig,
    J9Class* senderClass, UDATA lookupOptions)
{
    return javaLookupMethodImpl(currentThread, targetClass, nameAndSig, senderClass, lookupOptions, NULL);
}

j9object_t getClassPathString(J9VMThread* currentThread, J9Class* clazz)
{
    ...
    if (NULL != clazz) {
        ...
        if (NULL != path) {
            result = currentThread->javaVM->memoryManagerFunctions->j9gc_createJavaLangString(
                currentThread, path, pathLength, 0);
        }
    }
    return result;
}
...
UDATA
javaLookupMethodImpl(J9VMThread* currentThread, J9Class* targetClass, J9ROMNameAndSignature* nameAndSig,
    J9Class* senderClass, UDATA lookupOptions, BOOLEAN* foundDefaultConflicts)
{
    ...
    retry:
        ...
        /* Interface lookup assumes that the targetClass is an interface and searches only interfaces, never real
         * classes. */
        ...
        while (lookupClass != NULL) {
            J9Method* foundMethod = searchClassForMethodCommon(lookupClass, name, nameLength, sig, sigLength,
                J9_ARE_ANY_BITS_SET(lookupOptions, J9_LOOK_PARTIAL_SIGNATURE));

            if (foundMethod != NULL) {
                resultMethod = processMethod(currentThread, lookupOptions, foundMethod, lookupClass, &exception,
                    &exceptionClass, &errorType, nameAndSig, senderClass, targetClass);
                if (NULL != currentThread->currentException) {
                    goto end;
                }
        ...
        if ((lookupOptions & J9_LOOK_NO_INTERFACE) == 0) {
            ...
            tempResultMethod = javaResolveInterfaceMethods(
                currentThread, targetClass, nameAndSig, senderClass, lookupOptions, &data);
            if (NULL != currentThread->currentException) {
            ...
            if ((NULL == resultMethod) && data.elements > 1) {
                ...
                if ((lookupOptions & (J9_LOOK_NO_THROW | J9_LOOK_NO_JAVA)) == 0) {
                    ...
                    if (J9_VISIBILITY_NON_MODULE_ACCESS_ERROR == data.errorType) {
                        ...
                        buf = defaultMethodConflictExceptionMessage(
                            currentThread, targetClass, nameLength, name, sigLength, sig, data.methods, data.elements);
                    } else {
    ...
    if ((NULL == resultMethod) && (FALSE == exceptionThrown)) {
        ...
        if (J9UTF8_DATA_EQUALS(
                J9UTF8_DATA(classNameUTF), J9UTF8_LENGTH(classNameUTF), STRINGBUILDER, sizeof(STRINGBUILDER) - 1)) {
            if (J9UTF8_DATA_EQUALS(name, nameLength, APPEND_NAME, sizeof(APPEND_NAME) - 1)) {
                if (J9UTF8_DATA_EQUALS(sig, sigLength, APPEND_SIG, sizeof(APPEND_SIG) - 1)) {
                    ...
                    /* The loading constraints function requires a ROM nameAndSig, so skip the check when we lookup the
                     * replacement method */
        ...
        if ((lookupOptions & (J9_LOOK_NO_THROW | J9_LOOK_NO_JAVA)) == 0) {
            ...
            if (errorString == NULL) {
                if ((badMethod != NULL)
                    ...
                    buf = illegalAccessMessage(
                        currentThread, badRomMethod->modifiers, senderClass, targetClass, errorType);
                    ...
                    setClassLoadingConstraintSignatureError(currentThread, cl1, senderClass, cl2, lookupClass,
                        exceptionClass, name, nameLength, sig, sigLength);
                } else {
                    ...
                    j9object_t methodSigStr = catUtfToString4(currentThread, J9UTF8_DATA(classNameUTF),
                        J9UTF8_LENGTH(classNameUTF), (U_8*)".", 1, name, nameLength, sig, sigLength);
                    if (NULL != currentThread->currentException) {
                    ...
                    if (NULL == senderClass) {
                        ...
                        case J9SF_FRAME_TYPE_JIT_JNI_CALLOUT:
                            nativeMethod = ((J9SFJNINativeMethodFrame*)((U_8*)currentThread->sp
                                                + (UDATA)currentThread->literals))
                                               ->method;
                        }
                    ...
                    {
                        UDATA args[] = { (UDATA)methodSigStr, (UDATA)J9VM_J9CLASS_TO_HEAPCLASS(targetClass),
                            (UDATA)targetClassPath, (UDATA)J9VM_J9CLASS_TO_HEAPCLASS(senderClass),
    ...
    U_8* name, UDATA sigLength, U_8* sig, J9Method** methods, UDATA methodsLength)
{
    ...
    errorMsg = j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE,
        J9NLS_VM_DEFAULT_METHOD_CONFLICT_LIST,
        "conflicting default methods for '%2$s%4$s' in %6$s from classes [%7$s]");
    ...
    bufLen = j9str_printf(PORTLIB, NULL, 0, errorMsg, nameLength, name, sigLength, sig,
        (UDATA)J9UTF8_LENGTH(targetClassNameUTF), J9UTF8_DATA(targetClassNameUTF), listString);
    if (bufLen > 0) {
        buf = j9mem_allocate_memory(bufLen, OMRMEM_CATEGORY_VM);
        if (NULL != buf) {
            bufLen = j9str_printf(PORTLIB, buf, bufLen, errorMsg, nameLength, name, sigLength, sig,
                J9UTF8_LENGTH(targetClassNameUTF), J9UTF8_DATA(targetClassNameUTF), listString);
        }
    }
...
#define NAMED_MODULE "module "
        nameBuffer = copyStringToUTF8WithMemAlloc(currentThread, module->moduleName, J9_STR_NULL_TERMINATE_RESULT,
            NAMED_MODULE, strlen(NAMED_MODULE), buffer, bufferLength, NULL);
#undef NAMED_MODULE
    }
    return nameBuffer;
}
 ...
 *
 * @return a char pointer to the detailed message, or NULL if NativeOutOfMemoryError occurred which a
 * JAVALANGILLEGALACCESSEXCEPTION will take precedence.
 */
    ...
    J9VMThread* currentThread, IDATA badMemberModifier, J9Class* senderClass, J9Class* targetClass, IDATA errorType)
{
    ...
    if ((J9_VISIBILITY_NEST_HOST_LOADING_FAILURE_ERROR == errorType)
        ...
        if (J9_VISIBILITY_NEST_HOST_LOADING_FAILURE_ERROR == errorType) {
            errorMsg = j9nls_lookup_message(
                J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE, J9NLS_VM_NEST_HOST_FAILED_TO_LOAD, NULL);
        } else if (J9_VISIBILITY_NEST_HOST_DIFFERENT_PACKAGE_ERROR == errorType) {
            errorMsg = j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE,
                J9NLS_VM_NEST_HOST_HAS_DIFFERENT_PACKAGE, NULL);
        } else if (J9_VISIBILITY_NEST_MEMBER_NOT_CLAIMED_ERROR == errorType) {
            errorMsg = j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE,
                J9NLS_VM_NEST_MEMBER_NOT_CLAIMED_BY_NEST_HOST, NULL);
        }

        bufLen = j9str_printf(PORTLIB, NULL, 0, errorMsg, J9UTF8_LENGTH(nestMemberNameUTF),
            J9UTF8_DATA(nestMemberNameUTF), J9UTF8_LENGTH(nestHostNameUTF), J9UTF8_DATA(nestHostNameUTF));
        ...
        if (bufLen > 0) {
            ...
            j9str_printf(PORTLIB, buf, bufLen, errorMsg, J9UTF8_LENGTH(nestMemberNameUTF),
                J9UTF8_DATA(nestMemberNameUTF), J9UTF8_LENGTH(nestHostNameUTF), J9UTF8_DATA(nestHostNameUTF));
        }
    } else
        ...
        /* caller of illegalAccessMessage already performed Assert_VM_true((NULL != srcClassObject) && (NULL !=
         * destClassObject)); */
        ...
        destModuleMsg
            = getModuleNameUTF(currentThread, destModuleObject, destModuleBuf, J9VM_PACKAGE_NAME_BUFFER_LENGTH);
        ...
        if (J9_VISIBILITY_MODULE_READ_ACCESS_ERROR == errorType) {
            ...
            errorMsg = j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE,
                J9NLS_VM_ILLEGAL_ACCESS_MODULE_READ, NULL);
            bufLen = j9str_printf(PORTLIB, NULL, 0, errorMsg, J9UTF8_LENGTH(senderClassNameUTF),
                J9UTF8_DATA(senderClassNameUTF), srcModuleMsg, J9UTF8_LENGTH(targetClassNameUTF),
                J9UTF8_DATA(targetClassNameUTF), destModuleMsg);
            if (bufLen > 0) {
                buf = j9mem_allocate_memory(bufLen, OMRMEM_CATEGORY_VM);
                if (NULL != buf) {
                    j9str_printf(PORTLIB, buf, bufLen, errorMsg, J9UTF8_LENGTH(senderClassNameUTF),
                        J9UTF8_DATA(senderClassNameUTF), srcModuleMsg, J9UTF8_LENGTH(targetClassNameUTF),
                        J9UTF8_DATA(targetClassNameUTF), destModuleMsg);
                } else {
                    goto allocationFailure;
            ...
            *(packageNameMsg + packageNameLength) = '\0';
            errorMsg = j9nls_lookup_message(J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE,
                J9NLS_VM_ILLEGAL_ACCESS_MODULE_EXPORT, NULL);
            bufLen = j9str_printf(PORTLIB, NULL, 0, errorMsg, J9UTF8_LENGTH(senderClassNameUTF),
                J9UTF8_DATA(senderClassNameUTF), srcModuleMsg, J9UTF8_LENGTH(targetClassNameUTF),
                J9UTF8_DATA(targetClassNameUTF), destModuleMsg, packageNameMsg);
            if (bufLen > 0) {
                buf = j9mem_allocate_memory(bufLen, OMRMEM_CATEGORY_VM);
                if (NULL != buf) {
                    j9str_printf(PORTLIB, buf, bufLen, errorMsg, J9UTF8_LENGTH(senderClassNameUTF),
        ...
        if (badMemberModifier == -1) { /* visibility failed from Class level */
            errorMsg = j9nls_lookup_message(
                J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE, J9NLS_VM_ILLEGAL_ACCESS_CLASS, NULL);
            ...
            errorMsg = j9nls_lookup_message(
                J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE, J9NLS_VM_ILLEGAL_ACCESS_DETAIL, NULL);
         ...
         */
        bufLen = j9str_printf(PORTLIB, NULL, 0, errorMsg, J9UTF8_LENGTH(senderClassNameUTF),
            J9UTF8_DATA(senderClassNameUTF), modifierStr, J9UTF8_LENGTH(targetClassNameUTF),
