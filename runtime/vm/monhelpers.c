    ...
    if (J9_FLATLOCK_OWNER(lock) == vmStruct) {
        ...
        if (count == 0x00) {
            ...
            *lockEA = lock;
            ...
            /* lock is reserved but unowned (if it were owned the count would be >=
            ...
            if (0 == monitor->pinCount) {
                if (deflate) {
                    ...
                    case J9VM_DEFLATION_POLICY_SMART:
                        Trc_VM_objectMonitorExit_SmartDecisionPoint(
                            vmStruct, objectMonitor->proDeflationCount, objectMonitor->antiDeflationCount);
                ...
                if (J9_EVENT_IS_HOOKED(vmStruct->javaVM->hookInterface, J9HOOK_VM_MONITOR_CONTENDED_EXIT)) {
                    ...
                    if (!(monitor->flags & J9THREAD_MONITOR_SUPPRESS_CONTENDED_EXIT)) {
                        ...
                        if ((monitor->pinCount - omrthread_monitor_num_waiting((omrthread_monitor_t)monitor)) > 0) {
                            ALWAYS_TRIGGER_J9HOOK_VM_MONITOR_CONTENDED_EXIT(
                                vmStruct->javaVM->hookInterface, vmStruct, (omrthread_monitor_t)monitor);
