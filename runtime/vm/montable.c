...
#define TRACE(message)                                                                                               \
    j9tty_printf(PORTLIB, "%s in monitorTableAt in %p. Monitor=%p. Monitor-count=%d. Cache=%p (%d/%d)\n", (message), \
        vmStruct, monitor, hashTableGetCount(vm->monitorTable), vmStruct->eventReportData1, hits, hits + misses);
#else
#define HIT()
#define MISS()
#define TRACE(message)
...
#define J9_OBJECT_MONITOR_LOOKUP_SLOT(object, vm) \
    ((((UDATA)object) >> vm->omrVM->_objectAlignmentShift) & (J9VMTHREAD_OBJECT_MONITOR_CACHE_SIZE - 1))
#endif
...
static UDATA hashMonitorCompare(void* tableEntryKey, void* userKey, void* userData)
{
     ...
     * Only access to the table entry needs the barrier. The user provided key should already have an updated location
     * of the objects,
     * since a read barrier had to be executed some time prior to the construction of the key, whereever the value is
     * read from */
...
{
    j9object_t object
        = J9MONITORTABLE_OBJECT_LOAD(vmStruct, &((J9ThreadAbstractMonitor*)(objectMonitor->monitor))->userData);

    vmStruct->objectMonitorLookupCache[J9_OBJECT_MONITOR_LOOKUP_SLOT(object, vm)]
        = (j9objectmonitor_t)((UDATA)objectMonitor);
}
#endif
...
#define MONTABLE_FLAGS 0
    ...
    return hashTableNew(OMRPORT_FROM_J9PORT(vm->portLibrary), tableName, 64, sizeof(J9ObjectMonitor), 0, MONTABLE_FLAGS,
        OMRMEM_CATEGORY_VM, hashMonitorHash, hashMonitorCompare, NULL, vm);
...
initializeMonitorTable(J9JavaVM* vm)
{
    ...
    if (FALSE
        == vm->memoryManagerFunctions->j9gc_modron_getConfigurationValueForKey(
               vm, j9gc_modron_configuration_gcThreadCount, (void*)&tableCount)) {
        return -1;
    }
    ...
    vm->monitorTableListPool = pool_new(sizeof(J9MonitorTableListEntry), 0, 0, 0, J9_GET_CALLSITE(), OMRMEM_CATEGORY_VM,
...
J9ObjectMonitor* monitorTableAt(J9VMThread* vmStruct, j9object_t object)
{
...
#ifdef J9VM_THR_LOCK_NURSERY
    Trc_VM_monitorTableAt_Entry(
        vmStruct, object, J9OBJECT_CLAZZ(vmStruct, object), J9OBJECT_MONITOR_OFFSET(vmStruct, object));

    if (TrcEnabled_Trc_VM_monitorTableAtObjectWithNoLockword) {
        if (!LN_HAS_LOCKWORD(vmStruct, object)) {
            /* includes name of object */
            Trc_VM_monitorTableAtObjectWithNoLockword(vmStruct,
                J9UTF8_LENGTH(J9ROMCLASS_CLASSNAME(J9OBJECT_CLAZZ(vmStruct, object)->romClass)),
                J9UTF8_DATA(J9ROMCLASS_CLASSNAME(J9OBJECT_CLAZZ(vmStruct, object)->romClass)), object);
        }
    }
...
#ifdef J9VM_THR_LOCK_NURSERY
    objectMonitor
        = (J9ObjectMonitor*)((UDATA)vmStruct->objectMonitorLookupCache[J9_OBJECT_MONITOR_LOOKUP_SLOT(object, vm)]);
#else
    ...
    if ((objectMonitor != NULL) && (((J9ThreadAbstractMonitor*)objectMonitor->monitor)->userData == (UDATA)object)) {
        ...
        Trc_VM_monitorTableAtCacheMiss(vmStruct,
            J9UTF8_LENGTH(J9ROMCLASS_CLASSNAME(J9OBJECT_CLAZZ(vmStruct, object)->romClass)),
            J9UTF8_DATA(J9ROMCLASS_CLASSNAME(J9OBJECT_CLAZZ(vmStruct, object)->romClass)), object);
        MISS();
    }
        ...
        if (objectMonitor == NULL) {
            ...
            if (omrthread_monitor_init_with_name(&monitor, monitorFlags, NULL) == 0) {
                ...
                if (NULL != option) {
                    ...
                    Trc_VM_MonitorTableAt_CustomSpinOption(option->className, object,
                    ...
                    Trc_VM_MonitorTableAt_CustomSpinOption2(
                        option->className, object, j9threadOptions->customAdaptSpin);
