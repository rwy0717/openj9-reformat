...
void flushBytecodeProfilingData(J9VMThread* vmThread)
{
    ...
    if (vmThread->profilingBufferEnd) {
        ...
        ALWAYS_TRIGGER_J9HOOK_VM_PROFILING_BYTECODE_BUFFER_FULL(
            vmThread->javaVM->hookInterface, vmThread, bufferStart, vmThread->profilingBufferCursor - bufferStart);
    } else {
...
void profilingBytecodeBufferFullHookRegistered(J9JavaVM* vm)
{
    ...
    if ((*vmHooks)->J9HookRegisterWithCallSite(
            vmHooks, J9HOOK_VM_THREAD_DESTROY, cleanupBytecodeProfilingData, OMR_GET_CALLSITE(), iprofilerBufferSize)) {
    ...
    if ((*vmHooks)->J9HookRegisterWithCallSite(
            vmHooks, J9HOOK_VM_CLASSES_UNLOAD, flushForClassesUnload, OMR_GET_CALLSITE(), iprofilerBufferSize)
        || (*vmHooks)->J9HookRegisterWithCallSite(vmHooks, J9HOOK_VM_ANON_CLASSES_UNLOAD, flushForClassesUnload,
               OMR_GET_CALLSITE(), iprofilerBufferSize)) {
