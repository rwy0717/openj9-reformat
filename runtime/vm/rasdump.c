...
static omr_error_t primordialTriggerDumpAgents(
...
static omr_error_t primordialTriggerOneOffDump(
...
/* Basic dump facade */
static const J9RASdumpFunctions primordialDumpFacade = { NULL, primordialTriggerOneOffDump, primordialInsertDumpAgent,
    primordialRemoveDumpAgent, primordialSeekDumpAgent, primordialTriggerDumpAgents, primordialSetDumpOption,
    primordialResetDumpOption, primordialQueryVmDump };
    ...
    struct J9JavaVM* vm, char* optionString, char* caller, char* fileName, size_t fileNameLength)
{
    ...
    if (strcmp(optionString, "console") == 0) {
        ...
        j9nls_printf(
            PORTLIB, J9NLS_WARNING, J9NLS_VM_MISSING_DUMP_DLL_TRIGGERONEOFFDUMP, optionString, J9_RAS_DUMP_DLL_NAME);
    }
...
void j9rasSetServiceLevel(J9JavaVM* vm, const char* runtimeVersion)
{
    ...
    if (NULL != serviceLevel) {
        j9str_printf(PORTLIB, serviceLevel, size + 1, formatString, javaVersion, osname, osarch, ossize, openBracket,
            runtimeVersion, closeBracket);
        serviceLevel[size] = '\0';
...
static J9RAS* allocateRASStruct(J9PortLibrary* portLib)
{
    ...
    params.endAddress
        = OMR_MIN((void*)candidate, (void*)(UDATA)0xffffffff); /* don't bother allocating after the static */
...
static void freeRASStruct(J9PortLibrary* portLib, J9RAS* rasStruct)
{
            ...
            if (buf[0] == '|') {
                ...
                while (*cursor != '\0') {
                    ...
                    if (*cursor == '%') {
                        ...
                        if (*cursor != '\0' && *cursor != '%') {
                            j9nls_printf(
                                PORTLIB, J9NLS_WARNING | J9NLS_VITAL, J9NLS_VM_CHECK_DUMP_COREPATTERN_FORMAT, buf);
