...
static J9ROMFieldShape* findFieldInTable(J9VMThread* vmThread, J9Class* clazz, U_8* fieldName, UDATA fieldNameLength,
    U_8* signature, UDATA signatureLength, UDATA* offsetOrAddress);
#endif
#endif

#define SUPERCLASS(clazz) dbgReadClass(((clazz)->superclasses[J9CLASS_DEPTH(clazz) - 1]))

static J9ROMFieldShape* findFieldInClass(J9VMThread* vmStruct, J9Class* clazz, U_8* fieldName, UDATA fieldNameLength,
    U_8* signature, UDATA signatureLength, UDATA* offsetOrAddress, J9Class** definingClass);
static J9ROMFieldShape* findFieldAndCheckVisibility(J9VMThread* vmStruct, J9Class* clazz, U_8* fieldName,
    UDATA fieldNameLength, U_8* signature, UDATA signatureLength, J9Class** definingClass, UDATA* offsetOrAddress,
    UDATA options, J9Class* sourceClass);
static J9ROMFieldShape* findField(J9VMThread* vmStruct, J9Class* clazz, U_8* fieldName, UDATA fieldNameLength,
    U_8* signature, UDATA signatureLength, J9Class** definingClass, UDATA* offsetOrAddress, UDATA options);
...
static void initFakeJ9ROMFieldShape(
    J9ROMFieldShape* shape, U_16 nameLength, U_8* nameData, U_16 signatureLength, U_8* signatureData);
static J9ROMFieldShape* allocAndInitFakeJ9ROMFieldShape(J9JavaVM* vm, const char* name, const char* signature);
VMINLINE static J9HiddenInstanceField* initJ9HiddenField(J9HiddenInstanceField* field, J9UTF8* classNameUTF8,
    J9ROMFieldShape* shape, UDATA* offsetReturn, J9HiddenInstanceField* next);
...
/*
                ...
                offsetOrAddress is set to the instance offset (for instance field found) or static address (for static
   field found) if it is not NULL."
*/
static J9ROMFieldShape* findField(J9VMThread* vmStruct, J9Class* clazz, U_8* fieldName, UDATA fieldNameLength,
    U_8* signature, UDATA signatureLength, J9Class** definingClass, UDATA* offsetOrAddress, UDATA options)
{
    ...
    do {
        ...
        field = findFieldInClass(vmStruct, currentClass, fieldName, fieldNameLength, signature, signatureLength,
        ...
        for (i = 0; i < romClass->interfaceCount; i++, interfaces++) {
            ...
            J9Class* interfaceClass = internalFindClassUTF8(vmStruct, J9UTF8_DATA(interfaceName),
                J9UTF8_LENGTH(interfaceName), currentClass->classLoader, J9_FINDCLASS_FLAG_EXISTING_ONLY);

            if (interfaceClass != NULL) { /* shouldn't be NULL, but be safe */
                J9ITable* iTable = NULL;
                for (;;) {
                    field = findFieldInClass(vmStruct, interfaceClass, fieldName, fieldNameLength, signature,
                        signatureLength, offsetOrAddress, definingClass);
                    if (field != NULL) {
                        return field;
                    }
    ...
    if ((options & (J9_LOOK_NO_THROW | J9_LOOK_NO_JAVA)) == 0) {
        ...
        j9object_t message = catUtfToString4(vmStruct, J9UTF8_DATA(className), J9UTF8_LENGTH(className),
            (const U_8*)".", 1, fieldName, fieldNameLength, NULL, 0);
    ...
    U_8* signature, UDATA signatureLength, UDATA* offsetOrAddress, J9Class** definingClass)
{
    ...
    if (romClass->romFieldCount > javaVM->fieldIndexThreshold) {
        J9ROMFieldShape* field;
        field = findFieldInTable(
            vmStruct, clazz, fieldName, fieldNameLength, signature, signatureLength, offsetOrAddress);
        if (field != NULL) {
            shape = field;
        } else {
...
{
    return instanceFieldOffsetWithSourceClass(vmStruct, clazz, fieldName, fieldNameLength, signature, signatureLength,
        definingClass, instanceField, options, NULL);
}
    ...
    J9Class* sourceClass)
{
    ...
    field = findFieldAndCheckVisibility(vmStruct, clazz, fieldName, fieldNameLength, signature, signatureLength,
        definingClass, &offsetOrAddress, options, sourceClass);
    ...
    UDATA options, J9Class* sourceClass)
{
    ...
    field = findField(
        vmStruct, clazz, fieldName, fieldNameLength, signature, signatureLength, &defClass, offsetOrAddress, options);
        ...
        if (checkResult < J9_VISIBILITY_ALLOWED) {
            if ((options & (J9_LOOK_NO_THROW | J9_LOOK_NO_JAVA)) == 0) {
                ...
                if (J9_VISIBILITY_NON_MODULE_ACCESS_ERROR == checkResult) {
                    errorMsg = illegalAccessMessage(
                        vmStruct, field->modifiers, sourceClass, defClass, J9_VISIBILITY_NON_MODULE_ACCESS_ERROR);
                } else {
    ...
    UDATA signatureLength, J9Class** definingClass, UDATA* staticField, UDATA options, J9Class* sourceClass)
{
    ...
    /* Find a field matching the name and signature */
    field = findFieldAndCheckVisibility(vmStruct, clazz, fieldName, fieldNameLength, signature, signatureLength,
        &defClass, &offsetOrAddress, options, sourceClass);
...
static J9ROMFieldShape* allocAndInitFakeJ9ROMFieldShape(J9JavaVM* vm, const char* name, const char* signature)
{
    ...
    J9ROMFieldShape* shape = (J9ROMFieldShape*)j9mem_allocate_memory(
        calculateFakeJ9ROMFieldShapeSize(nameLength, signatureLength), OMRMEM_CATEGORY_VM);
...
initializeHiddenInstanceFieldsList(J9JavaVM* vm)
{
    ...
    vm->hiddenLockwordFieldShape
        = allocAndInitFakeJ9ROMFieldShape(vm, "lockword", (ObjectFieldInfo::LOCKWORD_SIZE == sizeof(U_32) ? "I" : "J"));
    if (NULL == vm->hiddenLockwordFieldShape) {
        goto destroyMutexAndCleanup;
    }

    vm->hiddenFinalizeLinkFieldShape = allocAndInitFakeJ9ROMFieldShape(
        vm, "finalizeLink", (ObjectFieldInfo::FINALIZE_LINK_SIZE == sizeof(U_32) ? "I" : "J"));
    ...
    J9JavaVM* vm, const char* className, const char* fieldName, const char* fieldSignature, UDATA* offsetReturn)
{
    ...
    for (field = vm->hiddenInstanceFields; NULL != field; field = field->next) {
        if (J9UTF8_DATA_EQUALS(
                J9UTF8_DATA(field->className), J9UTF8_LENGTH(field->className), className, classNameLength)) {
            matchedFields++;
        }
    }
    ...
    initFakeJ9ROMFieldShape(
        shape, (U_16)fieldNameLength, (U_8*)fieldName, (U_16)fieldSignatureLength, (U_8*)fieldSignature);
    ...
    J9JavaVM* vm, J9ROMClass* romClass, J9Class* superClazz, J9ROMFieldOffsetWalkState* state, U_32 flags)
{
    ...
    if (J9_ARE_ANY_BITS_SET(state->walkFlags,
            (J9VM_FIELD_OFFSET_WALK_INCLUDE_INSTANCE | J9VM_FIELD_OFFSET_WALK_INCLUDE_HIDDEN
                | J9VM_FIELD_OFFSET_WALK_CALCULATE_INSTANCE_SIZE))) {
        ...
        if ((NULL != superClazz) && (0 != superClazz->finalizeLinkOffset)) {
             ...
             * We add the field if the class marked as needing finalization.  In addition when HCR is enabled we also
             * add the field if the the class is not the class for java.lang.Object (superclass name in java.lang.object
             * isnull) and the class has a finalize method regardless of whether this method is empty or not.  We need
             * to do this because when HCR is enabled it is possible that the method will be re-transformed such that
             * there finalization is needed and in that case we need the field to be in the shape of the existing
             * objects
             */
            if ((J9ROMCLASS_FINALIZE_NEEDED(romClass))
        ...
        if (LOCKWORD_NEEDED == state->lockOffset) {
            extraHiddenFields = initJ9HiddenField(&state->hiddenLockwordField, NULL,
                dbgReadROMFieldShape((J9ROMFieldShape*)DBG_ARROW(vm, hiddenLockwordFieldShape)), &state->lockOffset,
                extraHiddenFields);
        }

        /*
         * Step 3: Calculate the number of various categories of fields: single word primitive, double word primitive,
         * and object references. Iterate over fields to count instance fields by size.
         */

        fieldInfo.countInstanceFields();

        if (NULL != extraHiddenFields) {
            state->hiddenInstanceFieldCount
                = fieldInfo.countAndCopyHiddenFields(extraHiddenFields, state->hiddenInstanceFields);
            Assert_VM_true(state->hiddenInstanceFieldCount
                < sizeof(state->hiddenInstanceFields) / sizeof(state->hiddenInstanceFields[0]));
        }
        ...
        /*
         * Calculate offsets (from the object header) for hidden fields.  Hidden fields follow immediately the instance
         * fields of the same type. Give instance fields priority for backfill slots. Note that the hidden fields
         * remember their offsets, so this need be done once only.
         */
        if (fieldInfo.isHiddenFieldOffsetResolutionRequired()) {
            ...
            if (fieldInfo
                    .isContendedClassLayout()) { /* hidden fields go immediately after the superclass fields and the
                                                    instance fields which are placed on the following cache line */
                ...
                hiddenSingleOffset
                    += state->firstSingleOffset + (fieldInfo.getNonBackfilledInstanceSingleCount() * sizeof(U_32));
                hiddenDoubleOffset += state->firstDoubleOffset + (fieldInfo.getInstanceDoubleCount() * sizeof(U_64));
                hiddenObjectOffset += state->firstObjectOffset
                    + (fieldInfo.getNonBackfilledInstanceObjectCount() * sizeof(fj9object_t));
            }
            bool useBackfillForObject = false;
            bool useBackfillForSingle = false;

            if (fieldInfo.isMyBackfillSlotAvailable()
                && J9_ARE_NO_BITS_SET(state->walkFlags,
                       J9VM_FIELD_OFFSET_WALK_BACKFILL_OBJECT_FIELD | J9VM_FIELD_OFFSET_WALK_BACKFILL_SINGLE_FIELD)) {
    ...
    if (J9_ARE_NO_BITS_SET(state->walkFlags,
        ...
        Trc_VM_romFieldOffsetsStartDo_earlyExit(NULL, state->result.field, state->result.offset, state->result.index,
            state->result.totalInstanceSize, state->result.superTotalInstanceSize);
        return &state->result;
    }
    ...
    Trc_VM_romFieldOffsetsStartDo_Exit(NULL, state->result.field, state->result.offset, state->result.index,
        state->result.totalInstanceSize, state->result.superTotalInstanceSize);
            ...
            if (state->walkFlags & J9VM_FIELD_OFFSET_WALK_INCLUDE_STATIC) {
                if (modifiers & J9FieldFlagObject) {
                    ...
                    if (modifiers & J9FieldSizeDouble) {
                        /* Add single scalar and object counts together, round up to 2 and divide by 2 to get number of
                         * doubles used by singles */
                        ...
                        state->result.offset = romClass->objectStaticCount * sizeof(j9object_t)
                            + state->singleStaticsSeen * sizeof(UDATA);
    ...
    J9JavaVM* vm, J9Class* clazz, J9ROMFullTraversalFieldOffsetWalkState* state, U_32 flags)
{
    ...
    if (state->walkFlags & J9VM_FIELD_OFFSET_WALK_PREINDEX_INTERFACE_FIELDS) {
        ...
        while (iTable) {
             ...
             *   2. for interface classes we want to count only the fields in the super interfaces, not in the interface
             * class being looked at */
    ...
    while (state->currentClass) {
        ...
        result = fieldOffsetsStartDo(state->javaVM, state->currentClass->romClass, SUPERCLASS(state->currentClass),
            &(state->fieldOffsetWalkState), state->walkFlags);
        field = result->field;
        if (NULL != field) {
 ...
 * "greater than" and "less than" are implementation-dependent functions of the fieldName, fieldNameLength, signature,
 * signatureLength
 */

static IDATA compareNameAndSignature(
    U_8* aName, IDATA aNameLength, U_8* aSig, IDATA aSigLength, J9UTF8* bName, J9UTF8* bSig)
{
    U_8* aData;
    U_8* bData;
    IDATA index;
...
#if !defined(J9VM_OUT_OF_PROCESS)
static IDATA compareFieldIDs(J9FieldTableEntry* a, J9FieldTableEntry* b)
{
    J9UTF8 *aName, *bName;
    J9UTF8 *aSig, *bSig;
    ...
    return compareNameAndSignature(
        J9UTF8_DATA(aName), J9UTF8_LENGTH(aName), J9UTF8_DATA(aSig), J9UTF8_LENGTH(aSig), bName, bSig);
}

static IDATA compareNameAndSigWithFieldID(
    U_8* aName, IDATA aNameLength, U_8* aSig, IDATA aSigLength, J9FieldTableEntry* b)
{
    J9UTF8* bName;
    J9UTF8* bSig;
...
static void sortFieldIndex(J9FieldTableEntry* list, IDATA start, IDATA end)
{
     ...
     * invariant: if scanUp == end, the entire list, with the possible exception of the last element, is less than or
     * equal to the pivot invariant: everything from scanUp+1 and up is greater than the pivot value the value at scanUp
     * may be greater, less, or same as the pivot
     */
...
static J9FieldTable* createFieldTable(J9VMThread* vmThread, J9Class* clazz)
{
    J9ROMClass* romClass = clazz->romClass;
    J9ROMFieldOffsetWalkState state;
    J9ROMFieldOffsetWalkResult* result;
    ...
    fieldList = (J9FieldTableEntry*)j9mem_allocate_memory(
        romClass->romFieldCount * sizeof(J9FieldTableEntry), OMRMEM_CATEGORY_VM);

    /* get the field names and put them in the list in unsorted order */
    result = fieldOffsetsStartDo(javaVM, romClass, SUPERCLASS(clazz), &state,
        J9VM_FIELD_OFFSET_WALK_INCLUDE_STATIC | J9VM_FIELD_OFFSET_WALK_INCLUDE_INSTANCE);
    while (result->field != NULL) {
        ...
        if (TrcEnabled_Trc_VM_FieldOffset) {
            ...
            Trc_VM_FieldOffset(vmThread, J9UTF8_LENGTH(className), J9UTF8_DATA(className), J9UTF8_LENGTH(fieldName),
                J9UTF8_DATA(fieldName), result->offset);
        }
        result = fieldOffsetsNextDo(&state);
        ++count;
    }
    ...
    U_8* signature, UDATA signatureLength, UDATA* offsetOrAddress)
{
    ...
    while (
        ((mag = compareNameAndSigWithFieldID(fieldName, fieldNameLength, signature, signatureLength, &fieldList[probe]))
            != 0)
...
static UDATA ramClassHashFn(void* key, void* userData)
{
 ...
 * @param userData not used
 */
static UDATA ramClassHashEqualFn(void* leftKey, void* rightKey, void* userData)
{
 ...
 * userData: java VM
 */
static void hookFieldTablePurge(J9HookInterface** hook, UDATA eventNum, void* eventData, void* userData)
{
    ...
    while (fitEntry) {
        Trc_VM_hookFieldTablePurge_Entry(
            fitEntry, fitEntry->table, (fitEntry->table ? (fitEntry->table->fieldList) : NULL));
...
J9HashTable* fieldIndexTableNew(J9JavaVM* vm, J9PortLibrary* portLib)
{
    ...
    (*vmHooks)->J9HookRegisterWithCallSite(
        vmHooks, J9HOOK_VM_CLASSES_UNLOAD, hookFieldTablePurge, OMR_GET_CALLSITE(), vm);
#endif
    result = vm->fieldIndexTable
        = hashTableNew(OMRPORT_FROM_J9PORT(portLib), J9_GET_CALLSITE(), initialSize, sizeof(fieldIndexTableEntry),
            sizeof(U_8*), 0, OMRMEM_CATEGORY_VM, ramClassHashFn, ramClassHashEqualFn, NULL, vm);
