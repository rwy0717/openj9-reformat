...
static bool finalFieldSetAllowed(J9VMThread* currentThread, bool isStatic, J9Method* method, J9Class* fieldClass,
    J9Class* callerClass, J9ROMFieldShape* field, bool canRunJavaCode);
 ...
 */
static bool threadEventsPending(J9VMThread* currentThread)
{
...
{
    UDATA legal = FALSE;
    j9object_t security = J9VMJAVALANGSYSTEM_SECURITY(
        currentThread, J9VMCONSTANTPOOL_CLASSREF_AT(currentThread->javaVM, J9VMCONSTANTPOOL_JAVALANGSYSTEM)->value);
    if (NULL == security) {
        legal = TRUE;
...
j9object_t resolveStringRef(J9VMThread* vmStruct, J9ConstantPool* ramCP, UDATA cpIndex, UDATA resolveFlags)
{
    J9UTF8* utf8Wrapper;
    ...
    stringRef = vmStruct->javaVM->memoryManagerFunctions->j9gc_allocStringWithSharedCharData(
        vmStruct, J9UTF8_DATA(utf8Wrapper), J9UTF8_LENGTH(utf8Wrapper), resolveFlags);
...
J9Class* findFieldSignatureClass(J9VMThread* vmStruct, J9ConstantPool* ramCP, UDATA fieldRefCpIndex)
{
    ...
    if ('[' == J9UTF8_DATA(signature)[0]) {
        resolvedClass = internalFindClassUTF8(
            vmStruct, J9UTF8_DATA(signature), J9UTF8_LENGTH(signature), classLoader, J9_FINDCLASS_FLAG_THROW_ON_FAIL);
    } else {
        ...
        resolvedClass = internalFindClassUTF8(vmStruct, &J9UTF8_DATA(signature)[1], J9UTF8_LENGTH(signature) - 2,
            classLoader, J9_FINDCLASS_FLAG_THROW_ON_FAIL);
    }
...
J9Class* resolveClassRef(J9VMThread* vmStruct, J9ConstantPool* ramCP, UDATA cpIndex, UDATA resolveFlags)
{
    ...
    bool jitCompileTimeResolve
        = J9_ARE_ANY_BITS_SET(resolveFlags, J9_RESOLVE_FLAG_JIT_COMPILE_TIME | J9_RESOLVE_FLAG_AOT_LOAD_TIME);
...
tryAgain:
    ...
    if ((NULL != resolvedClass)
        && (J9_ARE_NO_BITS_SET(resolveFlags, J9_RESOLVE_FLAG_INSTANTIABLE)
    ...
    if (ramClassRefWrapper->modifiers == (UDATA)-1) {
        if ((findClassFlags & J9_FINDCLASS_FLAG_THROW_ON_FAIL) == J9_FINDCLASS_FLAG_THROW_ON_FAIL) {
            detailString
                = vm->memoryManagerFunctions->j9gc_createJavaLangString(vmStruct, classNameData, classNameLength, 0);
            if (throwException) {
    ...
    resolvedClass = internalFindClassUTF8(vmStruct, classNameData, classNameLength, classLoader, findClassFlags);
    ...
    if (NULL == resolvedClass) {
        ...
        if (J9_ARE_ANY_BITS_SET(resolveFlags, J9_RESOLVE_FLAG_AOT_LOAD_TIME)) {
            ...
            if (0
                == compareUTF8Length(classNameData, classNameLength, J9UTF8_DATA(currentClassName),
                       J9UTF8_LENGTH(currentClassName))) {
    ...
    {
        IDATA checkResult
            = checkVisibility(vmStruct, J9_CLASS_FROM_CP(ramCP), accessClass, accessModifiers, lookupOptions);
        if (checkResult < J9_VISIBILITY_ALLOWED) {
            ...
            if (canRunJavaCode && (!VM_VMHelpers::exceptionPending(vmStruct))) {
                ...
                if (J9_VISIBILITY_NON_MODULE_ACCESS_ERROR == checkResult) {
                    errorMsg = illegalAccessMessage(vmStruct, accessModifiers, J9_CLASS_FROM_CP(ramCP), accessClass,
...
tryAgain:
        ...
        if (J2SE_VERSION(vmStruct->javaVM) >= J2SE_19) {
            ...
            if ((cpClass != NULL) && (cpClass->romClass != NULL)) {
                ...
                if (isResolvedClassAnInterface) {
                    if ((J9CPTYPE_INTERFACE_STATIC_METHOD != cpType) && (J9CPTYPE_INTERFACE_INSTANCE_METHOD != cpType)
    ...
    method = (J9Method*)javaLookupMethod(
        vmStruct, resolvedClass, J9ROMMETHODREF_NAMEANDSIGNATURE(romMethodRef), cpClass, lookupOptions);
...
tryAgain:
    ...
    if (resolvedClass != NULL) {
        ...
        checkResult
            = checkVisibility(vmStruct, classFromCP, resolvedClass, resolvedClass->romClass->modifiers, lookupOptions);
        if (checkResult < J9_VISIBILITY_ALLOWED) {
        ...
        signature = J9ROMNAMEANDSIGNATURE_SIGNATURE(nameAndSig);
        staticAddress = staticFieldAddress(vmStruct, resolvedClass, J9UTF8_DATA(name), J9UTF8_LENGTH(name),
            J9UTF8_DATA(signature), J9UTF8_LENGTH(signature), &definingClass, (UDATA*)&field, lookupOptions,
            classFromCP);
        ...
        if (staticAddress != NULL) {
            ...
            if ((resolveFlags & J9_RESOLVE_FLAG_FIELD_SETTER) != 0 && (modifiers & J9_JAVA_FINAL) != 0) {
                ...
                illegalAccess:
                    ...
                    if (canRunJavaCode && !threadEventsPending(vmStruct)) {
                        ...
                        PORT_ACCESS_FROM_VMC(vmStruct);
                        if (J9_VISIBILITY_NON_MODULE_ACCESS_ERROR == checkResult) {
                            errorMsg = illegalAccessMessage(vmStruct, badMemberModifier, classFromCP, targetClass,
                                J9_VISIBILITY_NON_MODULE_ACCESS_ERROR);
                        } else {
            ...
            if ((javaVM->runtimeFlags & J9_RUNTIME_VERIFY) != 0) {
                ...
                if (cl1 != cl2) {
                    ...
                    if (j9bcv_checkClassLoadingConstraintsForSignature(vmStruct, cl1, cl2, signature, fieldSignature)
                        != 0) {
                        if (throwException) {
            ...
            if (ramCPEntry != NULL) {
                ...
                /* Set the high bit in valueOffset to ensure that a resolved static field ref is always interpreted as
                 * an unresolved instance fieldref. */
                ramCPEntry->valueOffset
                    = ((UDATA)staticAddress - (UDATA)localClassAndFlags->ramStatics) | (UDATA)IDATA_MIN;
            }
        }
    }
    ...
    UDATA resolveFlags, J9ROMFieldShape** resolvedField)
{
    ...
    staticAddress
        = resolveStaticFieldRefInto(vmStruct, method, ramCP, cpIndex, resolveFlags, resolvedField, ramStaticFieldRef);
    ...
    UDATA resolveFlags, J9ROMFieldShape** resolvedField, J9RAMFieldRef* ramCPEntry)
{
    ...
    if (resolvedClass != NULL) {
        ...
        name = J9ROMNAMEANDSIGNATURE_NAME(nameAndSig);
        signature = J9ROMNAMEANDSIGNATURE_SIGNATURE(nameAndSig);
        fieldOffset = instanceFieldOffsetWithSourceClass(vmStruct, resolvedClass, J9UTF8_DATA(name),
            J9UTF8_LENGTH(name), J9UTF8_DATA(signature), J9UTF8_LENGTH(signature), &definingClass, (UDATA*)&field,
            lookupOptions, classFromCP);
        ...
        if (fieldOffset != -1) {
             ...
             */
            if (J9_ARE_ALL_BITS_SET(modifiers, J9AccProtected) && J9_ARE_NO_BITS_SET(modifiers, J9AccStatic)
            ...
            if ((resolveFlags & J9_RESOLVE_FLAG_FIELD_SETTER) != 0 && (modifiers & J9_JAVA_FINAL) != 0) {
                ...
                illegalAccess:
                    ...
                    if (throwException) {
                        PORT_ACCESS_FROM_VMC(vmStruct);
                        if (J9_VISIBILITY_NON_MODULE_ACCESS_ERROR == checkResult) {
                            nlsStr = illegalAccessMessage(vmStruct, badMemberModifier, classFromCP, targetClass,
                                J9_VISIBILITY_NON_MODULE_ACCESS_ERROR);
                        } else {
                ...
                if (
                    ...
                    /* The withfield bytecode is allowed to set a final field. However, the invoker of withfield must
                     * have private access to the field (similar to a constructor setting a final field). The private
                     * access check is done above, so if we get
...
J9Method* resolveInterfaceMethodRefInto(
    J9VMThread* vmStruct, J9ConstantPool* ramCP, UDATA cpIndex, UDATA resolveFlags, J9RAMInterfaceMethodRef* ramCPEntry)
{
    ...
    if ((interfaceClass->romClass->modifiers & J9_JAVA_INTERFACE) != J9_JAVA_INTERFACE) {
        if (throwException) {
            ...
            j9object_t detailMessage = vm->memoryManagerFunctions->j9gc_createJavaLangString(
                vmStruct, J9UTF8_DATA(className), J9UTF8_LENGTH(className), J9_STR_XLAT);
    ...
    J9VMThread* vmStruct, J9ConstantPool* ramCP, UDATA cpIndex, UDATA resolveFlags, J9RAMSpecialMethodRef* ramCPEntry)
{
     ...
     * Virtual invocation modes may still find a method in the receiver's vtable that resolves the default method
     * conflict. If not, the method in the vtable will be a special method for throwing the exception.
     *
     * Special invocations (defender supersends) will not look at the receiver's vtable, but instead invoke the result
     * of javaLookupMethod. Default method conflicts must therefore be handled by the lookup code.
     */
        ...
        if (currentClass != NULL) {
            if ((resolvedClass->romClass != NULL) && (currentClass->romClass != NULL)) {
                ...
                if (J9_JAVA_INTERFACE == (resolvedClass->romClass->modifiers & J9_JAVA_INTERFACE)) {
                    if ((J9CPTYPE_INTERFACE_INSTANCE_METHOD != cpType) && (J9CPTYPE_INTERFACE_STATIC_METHOD != cpType)
                        && (J9CPTYPE_INTERFACE_METHOD != cpType)) {
                    incompat:
                        if (throwException) {
                            setCurrentException(vmStruct, J9VMCONSTANTPOOL_JAVALANGINCOMPATIBLECLASSCHANGEERROR, NULL);
    ...
    method = getMethodForSpecialSend(vmStruct, currentClass, resolvedClass, method,
        lookupOptions | J9_LOOK_NO_VISIBILITY_CHECK | J9_LOOK_IGNORE_INCOMPATIBLE_METHODS);
    if (NULL == method) {
        goto done;
    }
    ...
    J9Method** resolvedMethod, J9RAMVirtualMethodRef* ramCPEntry)
{
    ...
    if (resolvedClass != NULL) {
        ...
        if ((NULL != cpShapeDescription) && (resolvedClass == J9VMJAVALANGINVOKEMETHODHANDLE(vm))) {
            ...
            if ((J9CPTYPE_HANDLE_METHOD == J9_CP_TYPE(cpShapeDescription, cpIndex))
                ...
                if (MAX_STACK_SLOTS == (ramMethodRef->methodIndexAndArgCount & MAX_STACK_SLOTS)) {
                    if (throwException) {
                        setCurrentExceptionNLS(
                            vmStruct, J9VMCONSTANTPOOL_JAVALANGLINKAGEERROR, J9NLS_VM_TOO_MANY_ARGUMENTS);
                    }
                    goto done;
                }
                ...
                if (threadEventsPending(vmStruct)) {
                    ...
                    j9object_t lookupSigString = vm->memoryManagerFunctions->j9gc_createJavaLangString(
                        vmStruct, J9UTF8_DATA(sigUTF), J9UTF8_LENGTH(sigUTF), 0);
                    if (throwException) {
                        if (NULL == vmStruct->currentException) {
                            setCurrentException(
                                vmStruct, J9VMCONSTANTPOOL_JAVALANGNULLPOINTEREXCEPTION, (UDATA*)lookupSigString);
                        }
                    } else {
                        VM_VMHelpers::clearException(vmStruct);
                    }
            ...
            if (isVarHandle) {
                ...
                if (canRunJavaCode) {
                     ...
                     */
                    sendFromMethodDescriptorString(vmStruct, sigUTF, J9_CLASS_FROM_CP(ramCP)->classLoader,
                        J9VMJAVALANGINVOKEVARHANDLE_OR_NULL(vm), 0);
                    methodType = (j9object_t)vmStruct->returnValue;
        ...
        if (method != NULL) {
            ...
            J9ROMMethod* romMethod = J9_ROM_METHOD_FROM_RAM_METHOD(method);
            /* Only allow non-interface method to call invokePrivate, private interface method should use
             * "invokeInterface" bytecode The else case will throw ICCE for private interface method
             */
            if (J9_ARE_ALL_BITS_SET(romMethod->modifiers, J9AccPrivate)
                && J9_ARE_NO_BITS_SET(resolvedClass->romClass->modifiers, J9_JAVA_INTERFACE)) {
            ...
            {
                /* Fill in the constant pool entry. Don't bother checking for failure on the vtable index, since we know
                 * the method is there. */
                ...
                if (vTableOffset == 0) {
                    if (throwException) {
                        ...
                        setCurrentException(
                            vmStruct, J9VMCONSTANTPOOL_JAVALANGINCOMPATIBLECLASSCHANGEERROR, (UDATA*)errorString);
                    }
                } else {
                    if (ramCPEntry != NULL) {
...
j9object_t resolveMethodTypeRefInto(
    J9VMThread* vmThread, J9ConstantPool* ramCP, UDATA cpIndex, UDATA resolveFlags, J9RAMMethodTypeRef* ramCPEntry)
{
    ...
    if (threadEventsPending(vmThread)) {
        ...
        /* Resolved MethodType was null - throw NPE that includes the lookupSignature from the NaS */
        j9object_t lookupSigString = vmThread->javaVM->memoryManagerFunctions->j9gc_createJavaLangString(
            vmThread, J9UTF8_DATA(lookupSig), J9UTF8_LENGTH(lookupSig), 0);
        if (throwException) {
    ...
    if (NULL != methodType) {
        ...
        J9Class* returnTypeClass
            = J9VM_J9CLASS_FROM_HEAPCLASS(vmThread, J9VMJAVALANGINVOKEMETHODTYPE_RETURNTYPE(vmThread, methodType));
        ...
        visibilityReturnCode = checkVisibility(
            vmThread, senderClass, returnTypeClass, returnTypeClass->romClass->modifiers, lookupOptions);
        ...
        if (J9_VISIBILITY_ALLOWED != visibilityReturnCode) {
            ...
            for (UDATA i = 0; i < typeCount; i++) {
                J9Class* paramClass
                    = J9VM_J9CLASS_FROM_HEAPCLASS(vmThread, J9JAVAARRAYOFOBJECT_LOAD(vmThread, argTypesObject, i));
                ...
                visibilityReturnCode = checkVisibility(
                    vmThread, senderClass, paramClass, paramClass->romClass->modifiers, lookupOptions);
        ...
        if (NULL != illegalClass) {
            if (throwException) {
                char* errorMsg = illegalAccessMessage(
                    vmThread, illegalClass->romClass->modifiers, senderClass, illegalClass, visibilityReturnCode);
                if (NULL == errorMsg) {
...
j9object_t resolveMethodTypeRef(J9VMThread* vmThread, J9ConstantPool* ramCP, UDATA cpIndex, UDATA resolveFlags)
{
...
j9object_t resolveMethodHandleRefInto(
    J9VMThread* vmThread, J9ConstantPool* ramCP, UDATA cpIndex, UDATA resolveFlags, J9RAMMethodHandleRef* ramCPEntry)
{
    ...
    case MH_REF_INVOKEVIRTUAL:
        /* InvokeVirtual */
        {
             ...
             * skip method/signature checking to enable MethodHandle invocation only when the resolved class is
             * MethodHandle and the corresponding method is invokeExact() or invoke().
             */
            ...
            if (resolveVirtualMethodRef(vmThread, ramCP, fieldOrMethodIndex, resolveFlags, NULL) == 0) {
                ...
                if (resolveSpecialMethodRefInto(vmThread, ramCP, fieldOrMethodIndex, resolveFlags, &ramSpecialMethodRef)
                    == NULL) {
                    /* Only the class and {Name, Signature} are used by the java code so it is safe to use the fake
                     * specialMethodRef */
                    goto _done;
                }
            }
    ...
    if (NULL != methodHandle) {
        methodHandle = vmThread->javaVM->memoryManagerFunctions->j9gc_objaccess_asConstantPoolObject(vmThread,
            methodHandle,
...
j9object_t resolveMethodHandleRef(J9VMThread* vmThread, J9ConstantPool* ramCP, UDATA cpIndex, UDATA resolveFlags)
{
...
j9object_t resolveConstantDynamic(J9VMThread* vmThread, J9ConstantPool* ramCP, UDATA cpIndex, UDATA resolveFlags)
{
...
j9object_t resolveInvokeDynamic(J9VMThread* vmThread, J9ConstantPool* ramCP, UDATA callSiteIndex, UDATA resolveFlags)
{
    Assert_VM_true(J9_RESOLVE_FLAG_RUNTIME_RESOLVE == resolveFlags);
    ...
    if (NULL != methodHandle) {
        ...
        methodHandle = gcFuncs->j9gc_objaccess_asConstantPoolObject(vmThread, methodHandle,
            J9_GC_ALLOCATE_OBJECT_TENURED | J9_GC_ALLOCATE_OBJECT_NON_INSTRUMENTABLE | J9_GC_ALLOCATE_OBJECT_HASHED);
        ...
        if (NULL == methodHandle) {
            ...
            if (0
                == gcFuncs->j9gc_objaccess_staticCompareAndSwapObject(
                       vmThread, j9class, callSite, NULL, methodHandle)) {
                /* Another thread beat this thread to updating the call site, ensure both threads return the same method
                 * handle. */
