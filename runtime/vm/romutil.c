...
void setExceptionForErroredRomClass(J9ROMClass* romClass, J9VMThread* vmThread)
{
    ...
    if (romError->errorMethod == -1) {
        errorString = getJ9CfrErrorDetailMessageNoMethod(
            PORTLIB, &localError, J9UTF8_DATA(className), J9UTF8_LENGTH(className));
        ...
        errorString = getJ9CfrErrorDetailMessageForMethod(PORTLIB, &localError, J9UTF8_DATA(className),
            J9UTF8_LENGTH(className), J9ROMCLASSCFRCONSTANTPOOLINFO_BYTES(name), name->slot1,
            J9ROMCLASSCFRCONSTANTPOOLINFO_BYTES(sig), sig->slot1, NULL, 0);
    }

    if (errorString != NULL) {
        errorStringObject = vmThread->javaVM->memoryManagerFunctions->j9gc_createJavaLangString(
            vmThread, (U_8*)errorString, strlen(errorString), 0);
    }
    vmThread->javaVM->internalVMFunctions->setCurrentException(
        vmThread, romError->errorAction, (UDATA*)errorStringObject);
    ...
    J9VMThread* vmStruct, U_8* clsName, UDATA clsNameLength, U_8* romClassBytes, UDATA romClassLength)
{
    ...
    case J9_ROM_CLASS_COOKIE_TYPE_SHARED_CLASS: {
        ...
        if (verbose) {
            /* Don't report dynload events for shared classes - it is useful to see dynload verbose for those classes
             * which are not cached */
            memset(&vmStruct->javaVM->dynamicLoadBuffers->dynamicLoadStats->nameLength, 0,
                sizeof(J9DynamicLoadStats) - sizeof(UDATA) - sizeof(U_8*));
        }
