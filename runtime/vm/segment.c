...
static void* allocateMemoryForSegment(
    J9JavaVM* javaVM, J9MemorySegment* segment, J9PortVmemParams* vmemParams, U_32 memoryCategory);
...
static J9MemorySegment* allocateVirtualMemorySegmentInListInternal(J9JavaVM* javaVM, J9MemorySegmentList* segmentList,
    UDATA size, UDATA type, J9PortVmemParams* vmemParams, U_32 memoryCategory);
    ...
    J9JavaVM* javaVM, U_32 numberOfMemorySegments, UDATA flags, U_32 memoryCategory)
{
    ...
    segmentList
        = allocateMemorySegmentListWithSize(javaVM, numberOfMemorySegments, sizeof(J9MemorySegment), memoryCategory);
    if (NULL != segmentList) {
...
void freeMemorySegment(J9JavaVM* javaVM, J9MemorySegment* segment, BOOLEAN freeDescriptor)
{
    ...
    Trc_VM_freeMemorySegment(
        currentVMThread(javaVM), segment, segment->heapBase, segment->heapTop, segment->classLoader, segment->type);
    ...
    if (segment->type & MEMORY_TYPE_ALLOCATED) {
        BOOLEAN useAdvise = (J9_EXTENDED_RUNTIME_FLAG_JSCRATCH_ADV_ON_FREE
            == (javaVM->extendedRuntimeFlags & J9_EXTENDED_RUNTIME_FLAG_JSCRATCH_ADV_ON_FREE));
...
#if defined(J9VM_THR_PREEMPTIVE)
    if (segmentList->segmentMutex)
        omrthread_monitor_destroy(segmentList->segmentMutex);
#endif
    ...
    J9JavaVM* javaVM, J9MemorySegment* segment, J9PortVmemParams* vmemParams, U_32 memoryCategory)
{
    ...
    if (J9_ARE_ANY_BITS_SET(segment->type, MEMORY_TYPE_CODE | MEMORY_TYPE_VIRTUAL)) {
         ...
         */
        Assert_VM_true(J9_ARE_NO_BITS_SET(segment->type, MEMORY_TYPE_VIRTUAL)
            || J9_ARE_ANY_BITS_SET(segment->type, ~MEMORY_TYPE_VIRTUAL));
...
#if defined(J9VM_THR_PREEMPTIVE)
    if (vm->classMemorySegments->segmentMutex)
...
{
    return allocateVirtualMemorySegmentInListInternal(
        javaVM, segmentList, size, type, vmemParams, OMRMEM_CATEGORY_UNKNOWN);
}
    ...
    J9JavaVM* javaVM, U_32 numberOfMemorySegments, UDATA sizeOfElements, U_32 memoryCategory)
{
    ...
    segmentList->segmentPool = pool_new(
        sizeOfElements, numberOfMemorySegments, 0, 0, J9_GET_CALLSITE(), memoryCategory, POOL_FOR_PORT(PORTLIB));
    ...
    segmentList->avlTreeData.insertionComparator
        = (IDATA(*)(J9AVLTree*, J9AVLTreeNode*, J9AVLTreeNode*))segmentInsertionComparator;
...
static void prepareClassWalkState(J9ClassWalkState* state, J9JavaVM* vm, J9ClassLoader* classLoader)
{
    state->vm = vm;
    if (NULL == classLoader) {
...
VMINLINE static BOOLEAN shouldSkipClassLoader(J9ClassLoaderWalkState* state, J9ClassLoader* classLoader)
{
