 ...
 */
UDATA
compareCompressedUnicodeToUncompressedUnicode(
    J9VMThread* vmThread, j9object_t unicodeBytes1, j9object_t unicodeBytes2, UDATA length)
{
    UDATA result = 1;
    UDATA i = 0;
    while (0 != length) {
        ...
        if (stringLength1 != stringLength2) {
            ...
            if (isString1Compressed) {
                if (isString2Compressed) {
                    ...
                    result = compareCompressedUnicodeToUncompressedUnicode(
                        vmThread, unicodeBytes1, unicodeBytes2, stringLength1);
                }
            } else {
                if (isString2Compressed) {
                    result = compareCompressedUnicodeToUncompressedUnicode(
                        vmThread, unicodeBytes2, unicodeBytes1, stringLength1);
                } else {
    ...
    UDATA stringLength, U_8* utf8Data, UDATA utf8DataLength)
{
    ...
    if (IS_STRING_COMPRESSED(vmThread, string)) {
        ...
        if ((stringFlags & J9_STR_XLAT) == 0) {
            ...
            for (UDATA i = stringOffset; i < stringOffset + stringLength; i++) {
                UDATA encodedLength
                    = VM_VMHelpers::encodeUTF8CharI8(J9JAVAARRAYOFBYTE_LOAD(vmThread, stringValue, i), data);
        ...
        if ((stringFlags & J9_STR_XLAT) == 0) {
            ...
            for (UDATA i = stringOffset; i < stringOffset + stringLength; i++) {
                UDATA encodedLength
                    = VM_VMHelpers::encodeUTF8Char(J9JAVAARRAYOFCHAR_LOAD(vmThread, stringValue, i), data);
    ...
    UDATA prependStrLength, char* buffer, UDATA bufferLength, UDATA* utf8Length)
{
    ...
    if (NULL != result) {
        ...
        computedUtf8Length = copyStringToUTF8Helper(
            vmThread, string, stringFlags, 0, stringLength, result + prependStrLength, length - prependStrLength);
    ...
    const char* prependStr, UDATA prependStrLength, char* buffer, UDATA bufferLength)
{
    ...
    if (NULL != result) {
        ...
        computedUtf8Length = copyStringToUTF8Helper(vmThread, string, stringFlags, 0, stringLength,
            result + sizeof(((J9UTF8*)0)->length) + prependStrLength,
            length - sizeof(((J9UTF8*)0)->length) - prependStrLength);
...
j9object_t catUtfToString4(J9VMThread* vmThread, const U_8* data1, UDATA length1, const U_8* data2, UDATA length2,
    const U_8* data3, UDATA length3, const U_8* data4, UDATA length4)
{
    PORT_ACCESS_FROM_VMC(vmThread);
...
j9object_t methodToString(J9VMThread* vmThread, J9Method* method)
{
    ...
    return catUtfToString4(vmThread, J9UTF8_DATA(className), J9UTF8_LENGTH(className), (U_8*)".", 1, J9UTF8_DATA(name),
        J9UTF8_LENGTH(name), J9UTF8_DATA(sig), J9UTF8_LENGTH(sig));
}
}
