...
static void walkIndirectDescribedPushes(
    J9StackWalkState* walkState, UDATA* highestIndirectSlot, UDATA slotCount, U_32* descriptionSlots);
...
static void walkDescribedPushes(
    J9StackWalkState* walkState, UDATA* highestSlot, UDATA slotCount, U_32* descriptionSlots, UDATA argCount);
static void walkObjectPushes(J9StackWalkState* walkState);
static void walkPushedJNIRefs(J9StackWalkState* walkState);
static void getStackMap(J9StackWalkState* walkState, J9ROMClass* romClass, J9ROMMethod* romMethod, UDATA offsetPC,
    UDATA pushCount, U_32* result);
static void getLocalsMap(J9StackWalkState* walkState, J9ROMClass* romClass, J9ROMMethod* romMethod, UDATA offsetPC,
    U_32* result, UDATA argTempCount, UDATA alwaysLocalMap);
#if (!defined(J9VM_OUT_OF_PROCESS))
...
{
    UDATA rc = (walkState->walkThread->privateFlags & J9_PRIVATE_FLAGS_STACK_CORRUPT) ? J9_STACKWALK_RC_STACK_CORRUPT
                                                                                      : J9_STACKWALK_RC_NONE;
    ...
    void (*savedOSlotIterator)(struct J9VMThread * vmThread, struct J9StackWalkState * walkState,
    ...
    swPrintf(walkState, 1, "*** BEGIN STACK WALK, flags = %p, walkThread = %p, walkState = %p ***\n", walkState->flags,
        REMOTE_ADDR(walkState->walkThread), walkState);

    if (walkState->flags & J9_STACKWALK_RESUME)
        swPrintf(walkState, 2, "\tJ9_STACKWALK_RESUME\n");
    if (walkState->flags & J9_STACKWALK_START_AT_JIT_FRAME)
        swPrintf(walkState, 2, "\tSTART_AT_JIT_FRAME\n");
    if (walkState->flags & J9_STACKWALK_CACHE_CPS)
        swPrintf(walkState, 2, "\tCACHE_CPS\n");
    if (walkState->flags & J9_STACKWALK_CACHE_PCS)
        swPrintf(walkState, 2, "\tCACHE_PCS\n");
    if (walkState->flags & J9_STACKWALK_COUNT_SPECIFIED)
        swPrintf(walkState, 2, "\tCOUNT_SPECIFIED\n");
    if (walkState->flags & J9_STACKWALK_INCLUDE_ARRAYLET_LEAVES)
        swPrintf(walkState, 2, "\tINCLUDE_ARRAYLET_LEAVES\n");
    if (walkState->flags & J9_STACKWALK_INCLUDE_NATIVES)
        swPrintf(walkState, 2, "\tINCLUDE_NATIVES\n");
    if (walkState->flags & J9_STACKWALK_INCLUDE_CALL_IN_FRAMES)
    ...
    if (walkState->flags & J9_STACKWALK_WALK_TRANSLATE_PC)
        swPrintf(walkState, 2, "\tWALK_TRANSLATE_PC\n");
    ...
    if (walkState->flags & J9_STACKWALK_DO_NOT_SNIFF_AND_WHACK)
        swPrintf(walkState, 2, "\tDO_NOT_SNIFF_AND_WHACK\n");
    if (walkState->flags & J9_STACKWALK_CHECK_I_SLOTS_FOR_OBJECTS)
        swPrintf(walkState, 2, "\tCHECK_I_SLOTS_FOR_OBJECTS\n");
    if (walkState->flags & J9_STACKWALK_SAVE_STACKED_REGISTERS)
        swPrintf(walkState, 2, "\tSAVE_STACKED_REGISTERS\n");

#ifndef J9VM_OUT_OF_PROCESS
    if ((walkState->flags & (J9_STACKWALK_MAINTAIN_REGISTER_MAP | J9_STACKWALK_INCLUDE_CALL_IN_FRAMES))
        == (J9_STACKWALK_MAINTAIN_REGISTER_MAP | J9_STACKWALK_INCLUDE_CALL_IN_FRAMES)) {
        ...
        if (NULL != detailMessage) {
            detail = walkState->walkThread->javaVM->internalVMFunctions->copyStringToUTF8WithMemAlloc(currentThread,
                detailMessage, J9_STR_NULL_TERMINATE_RESULT, ": ", 2, detailStackBuffer,
                J9VM_PACKAGE_NAME_BUFFER_LENGTH, NULL);
        }
    ...
    swPrintf(walkState, 2,
        "Initial values: walkSP = %p, PC = %p, literals = %p, A0 = %p, j2iFrame = %p, ELS = %p, decomp = %p\n",
        REMOTE_ADDR(walkState->walkSP), walkState->pc, walkState->literals, REMOTE_ADDR(walkState->arg0EA),
#ifdef J9VM_INTERP_NATIVE_SUPPORT
        REMOTE_ADDR(walkState->j2iFrame), REMOTE_ADDR(walkState->walkedEntryLocalStorage),
#ifdef J9VM_JIT_FULL_SPEED_DEBUG
    ...
    if (J9_PRIVATE_FLAGS_STACKS_OUT_OF_SYNC
        == (walkState->walkThread->privateFlags & J9_PRIVATE_FLAGS_STACKS_OUT_OF_SYNC)) {
#ifdef J9VM_INTERP_STACKWALK_TRACING
    ...
    while (1) {
...
#ifdef J9VM_OUT_OF_PROCESS
            if ((walkState->pc == walkState->walkThread->javaVM->callInReturnPC)
                || (walkState->pc == (walkState->walkThread->javaVM->callInReturnPC + 3)))
#else
        ...
        /* Fetch the PC value before calling the frame walker since the debugger modifies PC values when breakpointing
         * methods */
    ...
    resumeInterpreterWalk:
        ...
        /* Fetch the remaining frame values after calling the frame walker since the stack could grow during the
         * iterator, changing all stack pointer values */
...
UDATA walkFrame(J9StackWalkState* walkState)
{
    if (walkState->flags & J9_STACKWALK_VISIBLE_ONLY) {
        ...
        if ((((UDATA)walkState->pc == J9SF_FRAME_TYPE_NATIVE_METHOD)
                || ((UDATA)walkState->pc == J9SF_FRAME_TYPE_JNI_NATIVE_METHOD))
            && !(walkState->flags & J9_STACKWALK_INCLUDE_NATIVES)) {
        ...
        if (walkState->flags & J9_STACKWALK_HIDE_EXCEPTION_FRAMES) {
            ...
            if (!(romMethod->modifiers & J9_JAVA_STATIC)) {
                if (J9UTF8_DATA(
                        J9ROMMETHOD_GET_NAME(UNTAGGED_METHOD_CP(walkState->method)->ramClass->romClass, romMethod))[0]
                    == '<') {
    ...
    if (walkState->flags & J9_STACKWALK_CACHE_MASK) {
            ...
            if (walkState->flags & J9_STACKWALK_WALK_TRANSLATE_PC) {
...
#endif
                {
                    ...
                    if ((cachePC < (U_8*)romClass) || (cachePC >= (((U_8*)romClass) + romClass->romSize))) {
                        TRIGGER_J9HOOK_VM_PERMANENT_PC(
                            walkState->walkThread->javaVM->hookInterface, walkState->walkThread, cachePC);
                    }
                }
            }
        ...
        if (walkState->flags & J9_STACKWALK_CACHE_CPS)
            *walkState->cacheCursor++ = (UDATA)walkState->constantPool;
        if (walkState->flags & J9_STACKWALK_CACHE_METHODS)
            *walkState->cacheCursor++ = (UDATA)walkState->method;
    }
...
static UDATA allocateCache(J9StackWalkState* walkState)
{
    ...
    if (walkState->walkThread->javaVM->jitConfig) {
        if (!(walkState->flags & J9_STACKWALK_SKIP_INLINES)) {
            /* computations above assume 1 cacheElement per frame, in reality there may be (maxInlinedMethods + 1)
             * elements per frame */
    ...
    if ((walkState != walkState->walkThread->stackWalkState)
        || ((UDATA)(walkState->walkThread->sp - stackStart) < cacheSize)
    ...
    while (slotCount) {
...
#ifdef J9VM_INTERP_LINEAR_STACKWALK_TRACING
        {
            ...
            if (walkState->slotType == J9_STACKWALK_SLOT_TYPE_METHOD_LOCAL) {
                j9str_printf(PORTLIB, indexedTag, 64, "%s-Slot: %s%d", (description & 1) ? "O" : "I",
                    (walkState->slotIndex >= (IDATA)argCount) ? "t" : "a", walkState->slotIndex);
            } else {
                j9str_printf(
                    PORTLIB, indexedTag, 64, "%s-Slot: p%d", (description & 1) ? "O" : "I", walkState->slotIndex);
            }
...
static void walkMethodFrame(J9StackWalkState* walkState)
{
    ...
    switch ((UDATA)walkState->pc) {
    case J9SF_FRAME_TYPE_METHOD:
        printFrameType(walkState,
            (walkState->frameFlags & J9_STACK_FLAGS_JIT_NATIVE_TRANSITION) ? "JIT generic transition"
                                                                           : "Generic method");
        break;
    case J9SF_FRAME_TYPE_NATIVE_METHOD:
        printFrameType(walkState,
            (walkState->frameFlags & J9_STACK_FLAGS_JIT_NATIVE_TRANSITION) ? "JIT INL transition"
                                                                           : "INL native method");
        break;
    case J9SF_FRAME_TYPE_JNI_NATIVE_METHOD:
        printFrameType(walkState,
            (walkState->frameFlags & J9_STACK_FLAGS_JIT_NATIVE_TRANSITION) ? "JIT JNI transition"
                                                                           : "JNI native method");
        break;
    default:
        printFrameType(walkState,
            (walkState->frameFlags & J9_STACK_FLAGS_JIT_NATIVE_TRANSITION) ? "JIT unknown transition"
                                                                           : "Unknown method");
        break;
    }
#endif
    ...
    if (NULL == walkState->method) {
        ...
        if ((walkState->pc != vm->impdep1PC) && (walkState->pc != (vm->impdep1PC + 3))) {
            walkState->bytecodePCOffset
                = walkState->pc - (U_8*)REMOTE_ADDR(J9_BYTECODE_START_FROM_RAM_METHOD(walkState->method));
        } else {
        ...
        if (walkState->flags & J9_STACKWALK_ITERATE_O_SLOTS) {
            WALK_METHOD_CLASS(walkState);
            walkBytecodeFrameSlots(walkState, walkState->method, walkState->bytecodePCOffset, walkState->unwindSP - 1,
                walkState->unwindSP - walkState->walkSP, walkState->arg0EA, argTempCount, FALSE);
        }
    }
}
        ...
        if (walkState->walkSP != walkState->unwindSP) {
            ...
            if (walkState->pc != (walkState->walkThread->javaVM->callInReturnPC + 3)) {
                swPrintf(walkState, 0,
                    "Error: PC should have been advanced in order to push return value, pc = %p, cipc = %p !!!\n",
                    walkState->pc, walkState->walkThread->javaVM->callInReturnPC);
            }
#endif
...
static void printFrameType(J9StackWalkState* walkState, char* frameType)
{
    swPrintf(walkState, 2, "%s frame: bp = %p, sp = %p, pc = %p, cp = %p, arg0EA = %p, flags = %p\n", frameType,
        REMOTE_ADDR(walkState->bp), REMOTE_ADDR(walkState->walkSP), walkState->pc, REMOTE_ADDR(walkState->constantPool),
        REMOTE_ADDR(walkState->arg0EA), walkState->frameFlags);
...
void swPrintMethod(J9StackWalkState* walkState)
{
    ...
    if (method) {
        ...
        swPrintf(walkState, 2,
            ...
            (U_32)J9UTF8_LENGTH(sig), J9UTF8_DATA(sig), REMOTE_ADDR(walkState->method));
    }
}
#endif /* J9VM_INTERP_STACKWALK_TRACING */
    ...
    J9StackWalkState* walkState, UDATA* highestIndirectSlot, UDATA slotCount, U_32* descriptionSlots)
{
    ...
    while (slotCount) {
        ...
        if (description & 1) {
            /* There's no need to mark indirect slots as object since they will never point to valid objects (so will
             * not be whacked) */
...
void swWalkObjectSlot(J9StackWalkState* walkState, j9object_t* objectSlot, void* indirectSlot, void* tag)
{
     ...
     * If it is non-NULL, then objectSlot is a pointer into remote memory, and indirectSlot is a pointer onto the local
     * stack
     */
    ...
    if (indirectSlot != NULL) {
        swPrintf(walkState, 4, "\t\t%s[%p -> %p] = %p\n", (tag ? tag : "O-Slot"), REMOTE_ADDR(indirectSlot), objectSlot,
            oldValue);
#ifdef J9VM_INTERP_LINEAR_STACKWALK_TRACING
        lswRecordSlot(walkState, REMOTE_ADDR((void*)(((UDATA)indirectSlot) & ~(UDATA)1)), LSW_TYPE_INDIRECT_O_SLOT,
            tag ? tag : "O-Slot");
#endif
    } else {
...
{
    if (indirectSlot) {
        swPrintf(walkState, 5, "\t\t%s[%p -> %p] = %p\n", (tag ? tag : "I-Slot"), REMOTE_ADDR(indirectSlot),
            REMOTE_ADDR(intSlot), *intSlot);
    } else {
...
static void initializeObjectSlotBitVector(J9StackWalkState* walkState)
{
    ...
    if ((walkState->walkThread->javaVM->runtimeFlags & J9_RUNTIME_SNIFF_AND_WHACK)
        && !(walkState->flags & J9_STACKWALK_DO_NOT_SNIFF_AND_WHACK)) {
        PORT_ACCESS_FROM_WALKSTATE(walkState);
    ...
    if (walkState->objectSlotBitVector) {
        ...
        for (registerNumber = 0; registerNumber < J9SW_POTENTIAL_SAVED_REGISTERS; ++registerNumber) {
            if ((walkState->elsBitVector & ((UDATA)1 << registerNumber)) == 0) {
                sniffAndWhackPointer(walkState,
                    (j9object_t*)(walkState->walkedEntryLocalStorage->jitGlobalStorageBase + registerNumber));
            }
        }
    }
...
void swMarkSlotAsObject(J9StackWalkState* walkState, j9object_t* objectSlot)
{
    ...
    if ((((UDATA*)objectSlot) >= walkState->walkThread->sp)
        && (((UDATA*)objectSlot) < walkState->walkThread->stackObject->end)) {
    ...
    U_32* result, UDATA argTempCount, UDATA alwaysLocalMap)
{
    ...
    if (!alwaysLocalMap) {
        /*	Detect method entry vs simply executing at PC 0.  If the bytecode frame is invisible (method monitor
           enter or stack growth) or the previous frame was the special frame indicating method entry (reporting method
           enter), then use the signature mapper instead of the local mapper.  This keeps the receiver alive for use
           within method enter events, even if the receiver is never used (in which case the local mapper would mark it
           as int).
        */
        ...
        if ((*walkState->bp & J9SF_A0_INVISIBLE_TAG) || (walkState->previousFrameFlags & J9_SSF_METHOD_ENTRY)) {
            ...
            /* j9localmap_ArgBitsForPC0 only deals with args, so zero out the result array to make sure the temps are
             * non-object */
...
#endif
    errorCode = vm->localMapFunction(
        PORTLIB, romClass, romMethod, offsetPC, result, vm, j9mapmemory_GetBuffer, j9mapmemory_ReleaseBuffer);
    ...
    UDATA pushCount, U_32* result)
{
    ...
    errorCode = j9stackmap_StackBitsForPC(PORTLIB, offsetPC, romClass, romMethod, result, pushCount,
        walkState->walkThread->javaVM, j9mapmemory_GetBuffer, j9mapmemory_ReleaseBuffer);
    if (errorCode < 0) {
#ifdef J9VM_OUT_OF_PROCESS
