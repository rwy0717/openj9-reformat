...
void threadParkImpl(J9VMThread* vmThread, IDATA timeoutIsEpochRelative, I_64 timeout)
{
    ...
    if (rc != J9THREAD_TIMED_OUT) {
        ...
        J9VMTHREAD_SET_BLOCKINGENTEROBJECT(
            vmThread, vmThread, J9VMJAVALANGTHREAD_PARKBLOCKER(vmThread, vmThread->threadObject));
        ...
        while (1) {
            ...
            if (!(timeoutIsEpochRelative && rc == J9THREAD_TIMED_OUT
                    && ((timeNow = j9time_current_time_millis()) < timeout))) {
                break;
            }
            millis = timeout - timeNow;
            nanos = 0;
