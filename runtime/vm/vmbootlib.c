 ...
 */
typedef UDATA (*openFunc_t)(void* userData, J9ClassLoader* classLoader, const char* logicalName, char* physicalName,
    J9NativeLibrary** libraryPtr, char* errorBuffer, UDATA bufferLength, UDATA flags);

/**
 * Prototypes for locally defined functions.
 */
static UDATA classLoaderRegisterLibrary(void* voidVMThread, J9ClassLoader* classLoader, const char* logicalName,
    char* physicalName, J9NativeLibrary** libraryPtr, char* errBuf, UDATA bufLen, UDATA flags);
static BOOLEAN isAbsolutePath(const char* path);
static J9ClassLoader* findLoadedSharedLibrary(
    J9VMThread* vmThread, const char* sharedLibraryName, J9NativeLibrary** libraryPtr);
static void freeSharedLibrary(J9VMThread* vmThread, J9ClassLoader* classLoader, J9NativeLibrary* library);
static UDATA openNativeLibrary(J9JavaVM* vm, J9ClassLoader* classLoader, const char* libName, char* libraryPath,
    J9NativeLibrary** libraryPtr, openFunc_t openFunction, void* userData, char* errorBuffer, UDATA bufferLength);
 ...
 */
static IDATA sendLifecycleEventCallback(
    ...
    J9NativeLibrary** libraryPtr, openFunc_t openFunction, void* userData, char* errorBuffer, UDATA bufferLength)
{
    ...
    if (libraryPath && (!isAbsolutePath(libName))) {
        ...
        for (;;) {
            ...
            if (pathLength) {
                ...
                j9str_printf(PORTLIB, fullPathPtr, expectedPathLength, "%.*s%s%s", pathLength, libraryPath,
                    dirSeparator, libName);
                result = openFunction(userData, classLoader, libName, fullPathPtr, libraryPtr, errorBuffer,
                    bufferLength, lazy | J9PORT_SLOPEN_DECORATE | J9PORT_SLOPEN_NO_LOOKUP_MSG_FOR_NOT_FOUND);
                if (result == J9NATIVELIB_LOAD_ERR_NOT_FOUND) {
                    result = openFunction(userData, classLoader, libName, fullPathPtr, libraryPtr, errorBuffer,
                        bufferLength, lazy | J9PORT_SLOPEN_NO_LOOKUP_MSG_FOR_NOT_FOUND);
                    if (result != J9NATIVELIB_LOAD_ERR_NOT_FOUND) {
                        goto exit;
                    }
                } else {
                    /* J9NATIVELIB_LOAD_ERR_NOT_FOUND indicates an error opening the library, all others are immediate
                     * failure */
                    goto exit;
                }
            }
    ...
    if (result == J9NATIVELIB_LOAD_ERR_NOT_FOUND) {
        result = openFunction(userData, classLoader, libName, (char*)libName, libraryPtr, errorBuffer, bufferLength,
            lazy | J9PORT_SLOPEN_DECORATE);
    }
...
{
    return openNativeLibrary(vmThread->javaVM, classLoader, libName, libraryPath, libraryPtr,
        classLoaderRegisterLibrary, vmThread, errorBuffer, bufferLength);
}
...
registerBootstrapLibrary(J9VMThread* vmThread, const char* libName, J9NativeLibrary** libraryPtr, UDATA suppressError)
{
    ...
    result = registerNativeLibrary(vmThread, vmThread->javaVM->systemClassLoader, libName, bootLibraryPath, libraryPtr,
 ...
 * \warning If vmThread is non-NULL, the vmThread should have a call-out frame built on top of stack which should be
 * discarded after calling this function to remove any JNI refs created in the JNI_OnLoad code. \note If bufLen is > 0,
 * errorBuffer will be filled in with an error description
 */
static UDATA classLoaderRegisterLibrary(void* voidVMThread, J9ClassLoader* classLoader, const char* logicalName,
    char* physicalName, J9NativeLibrary** libraryPtr, char* errBuf, UDATA bufLen, UDATA flags)
{
    ...
    if (classLoader->sharedLibraries == NULL) {
        classLoader->sharedLibraries = pool_new(sizeof(J9NativeLibrary), 0, 0, 0, J9_GET_CALLSITE(),
            J9MEM_CATEGORY_CLASSES, (j9memAlloc_fptr_t)pool_portLibAlloc, (j9memFree_fptr_t)pool_portLibFree, PORTLIB);
    }
    ...
    if (J9_EVENT_IS_HOOKED(javaVM->hookInterface, J9HOOK_VM_LOAD_JNI_LIBRARY)) {
        ...
        ALWAYS_TRIGGER_J9HOOK_VM_LOAD_JNI_LIBRARY(javaVM->hookInterface, vmThread, classLoader, physicalName,
            newNativeLibrary, result, errBuf, bufLen, flags);
    ...
    if ((J9NATIVELIB_LOAD_OK == rc) && !loadWasIntercepted) {
        ...
        if (0 == j9sysinfo_get_executable_name(NULL, &executableName)) {
            slOpenResult = j9sl_open_shared_library(
                executableName, &newNativeLibrary->handle, (flags | J9PORT_SLOPEN_OPEN_EXECUTABLE));
            if (J9PORT_SL_FOUND == slOpenResult) {
                ...
                jniVersion
                    = (*newNativeLibrary->send_lifecycle_event)(vmThread, newNativeLibrary, onloadRtnName, (UDATA)-1);
#if defined(J9VM_INTERP_ATOMIC_FREE_JNI)
                ...
                if ((JNI_VERSION_1_8 <= jniVersion) && ((UDATA)-1 != jniVersion)) {
                    ...
                    if (((UDATA)-1 != jniVersion) || (NULL != vmThread->currentException)) {
                        ...
                        j9str_printf(PORTLIB, msgBuffer, MAXIMUM_MESSAGE_LENGTH,
                            "0x%.8x not a valid JNI version for static linking (1.8 or later required)", jniVersion);
            ...
            Trc_VM_classLoaderRegisterLibrary_execNameNotFound(vmThread, j9error_last_error_number());
        }
    }
     ...
     */
    if ((J9NATIVELIB_LINK_MODE_UNINITIALIZED == newNativeLibrary->linkMode) && (J9NATIVELIB_LOAD_OK == rc)
        && !loadWasIntercepted) {
        ...
        if (J9NATIVELIB_LOAD_OK == rc) {
            ...
            exitVMToJNI(vmThread);
#endif /* J9VM_INTERP_ATOMIC_FREE_JNI */
            jniVersion = (*newNativeLibrary->send_lifecycle_event)(
                vmThread, newNativeLibrary, J9DYNAMIC_ONLOAD, JNI_VERSION_1_1);
#if defined(J9VM_INTERP_ATOMIC_FREE_JNI)
            enterVMFromJNI(vmThread);
            releaseVMAccess(vmThread);
#endif /* J9VM_INTERP_ATOMIC_FREE_JNI */
            ...
            if ((FALSE == jniVersionIsValid(jniVersion)) || (NULL != vmThread->currentException)) {
                ...
                j9str_printf(
                    PORTLIB, msgBuffer, MAXIMUM_MESSAGE_LENGTH, "0x%.8x is not a valid JNI version", jniVersion);
