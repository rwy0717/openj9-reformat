...
static vmiError JNICALL vmi_iterateSystemProperties(
    VMInterface* vmi, vmiSystemPropertyIterator iterator, void* userData);
 ...
 *
 * @note The CheckVersion function allows a class library to verify that the VM provides the required interface
 * functions. If the version requested is @ref VMI_VERSION_UNKNOWN, then the function will reply with the current
 * version and not return an error. If a specific version is passed, it will be compatibility checked against the
 * current, and @ref VMI_ERROR_UNSUPPORTED_VERSION may be returned.
 */
static vmiError JNICALL vmi_checkVersion(VMInterface* vmi, vmiVersion* version)
{
 ...
 */
static JavaVM* JNICALL vmi_getJavaVM(VMInterface* vmi) { return (JavaVM*)(((J9VMInterface*)vmi)->javaVM); }
 ...
 * @see j9port.c
 */
static J9PortLibrary* JNICALL vmi_getPortLibrary(VMInterface* vmi) { return ((J9VMInterface*)vmi)->portLibrary; }
 ...
 * <TR><TD>java.vm.info</TD>			<TD>"JRE 1.8.0 Linux amd64-64-Bit Compressed References 20180601_201
(JIT enabled, AOT enabled) <BR>OpenJ9   - 1ca0ab98 <BR>OMR      - 05d2b8a2 <BR>JCL      - c2aa0348 based on
jdk8u172-b11"</TD></TR>
 * <TR><TD>java.compiler</TD>			<TD>"j9jit29"</TD></TR>
 * <TR><TD>java.class.version</TD>		<TD>"52.0"</TD></TR>
 * <TR><TD>java.home</TD>			<TD>the absolute path of the parent directory of the directory containing
the vm <BR>i.e. for a vm /clear/bin/vm.exe, java.home is /clear</TD></TR>
 ...
 */
static struct VMIZipFunctionTable* JNICALL vmi_getZipFunctions(VMInterface* vmi) { return &VMIZipLibraryTable; }
...
static char* copyString(J9PortLibrary* portLibrary, char* original)
{
...
static vmiError JNICALL vmi_setSystemProperty(VMInterface* vmi, char* key, char* value)
{
    ...
    if (J9SYSPROP_ERROR_NOT_FOUND == rc) {
        ...
        rc = j9vmi->javaVM->internalVMFunctions->addSystemProperty(
            j9vmi->javaVM, copiedKey, copiedValue, J9SYSPROP_FLAG_NAME_ALLOCATED | J9SYSPROP_FLAG_VALUE_ALLOCATED);
    } else {
