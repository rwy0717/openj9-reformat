...
static UDATA vmRuntimeStateListenerProc(J9PortLibrary* portLib, void* userData)
{
    ...
    while (J9VM_RUNTIME_STATE_LISTENER_STOP != listener->runtimeStateListenerState) {
        do {
            ...
            || ((J9VM_RUNTIME_STATE_LISTENER_STOP != listener->runtimeStateListenerState)
...
static int J9THREAD_PROC vmRuntimeStateListenerProcWrapper(void* entryarg)
{
    ...
    if (JNI_OK == result) {
        UDATA rc = 0;
        j9sig_protect(vmRuntimeStateListenerProc, vm, structuredSignalHandler, listener->runtimeStateListenerVMThread,
...
I_32 startVMRuntimeStateListener(J9JavaVM* vm)
{
    ...
    rc = (I_32)createThreadWithCategory(NULL, vm->defaultOSStackSize, J9THREAD_PRIORITY_NORMAL, 0,
        vmRuntimeStateListenerProcWrapper, vm, J9THREAD_CATEGORY_RESOURCE_MONITOR_THREAD);
...
U_32 getVMRuntimeState(J9JavaVM* vm) { return vm->vmRuntimeStateListener.vmRuntimeState; }
...
U_32 getVMMinIdleWaitTime(J9JavaVM* vm) { return vm->vmRuntimeStateListener.minIdleWaitTime; }
