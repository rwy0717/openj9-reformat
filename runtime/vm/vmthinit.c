...
{
    if (omrthread_monitor_init_with_name(&vm->vmThreadListMutex, 0, "VM thread list")
        ...
        || (J9_IS_PROCESS_REFERENCE_MONITOR_ENABLED(vm)
               && omrthread_monitor_init_with_name(&vm->processReferenceMonitor, 0, "VM GC process reference"))
        ||
#endif
        ...
        omrthread_monitor_init_with_name(
            &vm->vmRuntimeStateListener.runtimeStateListenerMutex, 0, "VM state notification mutex")
        ||
...
#ifdef J9VM_THR_PREEMPTIVE
    if (vm->segmentMutex)
        omrthread_monitor_destroy(vm->segmentMutex);
    if (vm->classTableMutex)
        omrthread_monitor_destroy(vm->classTableMutex);
    if (vm->classLoaderModuleAndLocationMutex)
        omrthread_monitor_destroy(vm->classLoaderModuleAndLocationMutex);
    if (vm->classLoaderBlocksMutex)
        omrthread_monitor_destroy(vm->classLoaderBlocksMutex);
    if (vm->jniFrameMutex)
        omrthread_monitor_destroy(vm->jniFrameMutex);
#endif

    if (vm->runtimeFlagsMutex)
        omrthread_monitor_destroy(vm->runtimeFlagsMutex);
    if (vm->extendedMethodFlagsMutex)
        omrthread_monitor_destroy(vm->extendedMethodFlagsMutex);
    if (vm->vmThreadListMutex)
        omrthread_monitor_destroy(vm->vmThreadListMutex);
    if (vm->exclusiveAccessMutex)
        omrthread_monitor_destroy(vm->exclusiveAccessMutex);
    if (vm->statisticsMutex)
        omrthread_monitor_destroy(vm->statisticsMutex);
    if (vm->jniCriticalLock)
        omrthread_monitor_destroy(vm->jniCriticalLock);
#if defined(J9VM_JIT_CLASS_UNLOAD_RWMONITOR)
    if (vm->classUnloadMutex)
        omrthread_rwmutex_destroy(vm->classUnloadMutex);
#else
    if (vm->classUnloadMutex)
        omrthread_monitor_destroy(vm->classUnloadMutex);
#endif
    if (vm->bindNativeMutex)
        omrthread_monitor_destroy(vm->bindNativeMutex);

#ifdef J9VM_GC_FINALIZATION
    if (vm->finalizeMasterMonitor)
        omrthread_monitor_destroy(vm->finalizeMasterMonitor);
    if (NULL != vm->processReferenceMonitor)
        omrthread_monitor_destroy(vm->processReferenceMonitor);
    if (vm->finalizeRunFinalizationMutex)
        omrthread_monitor_destroy(vm->finalizeRunFinalizationMutex);
#endif

    if (vm->aotRuntimeInitMutex)
        omrthread_monitor_destroy(vm->aotRuntimeInitMutex);
    if (vm->jclCacheMutex)
        omrthread_monitor_destroy(vm->jclCacheMutex);
    if (vm->asyncEventMutex)
        omrthread_monitor_destroy(vm->asyncEventMutex);
    if (vm->osrGlobalBufferLock)
        omrthread_monitor_destroy(vm->osrGlobalBufferLock);
    if (vm->nativeLibraryMonitor)
        omrthread_monitor_destroy(vm->nativeLibraryMonitor);
    if (vm->vmRuntimeStateListener.runtimeStateListenerMutex)
        omrthread_monitor_destroy(vm->vmRuntimeStateListener.runtimeStateListenerMutex);
    if (vm->constantDynamicMutex)
        omrthread_monitor_destroy(vm->constantDynamicMutex);
