...
#if defined(J9ZOS390)
static IDATA setFailedToForkThreadException(
    J9VMThread* currentThread, IDATA retVal, omrthread_os_errno_t os_errno, omrthread_os_errno_t os_errno2);
#else /* !J9ZOS390 */
...
static UDATA startJavaThreadInternal(J9VMThread* currentThread, UDATA privateFlags, UDATA osStackSize, UDATA priority,
...
#undef VMTHR_INITIAL_STACK_SIZE
    ...
    if (J9_LINKED_LIST_IS_EMPTY(vm->deadThreadList)) {
        ...
        omrthread_monitor_init_with_name(
            &newThread->publicFlagsMutex, J9THREAD_MONITOR_JLM_TIME_STAMP_INVALIDATOR, "Thread public flags mutex");
    ...
    if (0 != vm->segregatedAllocationCacheSize) {
        newThread->segregatedAllocationCache = (J9VMGCSegregatedAllocationCacheEntry*)(((UDATA)newThread)
    ...
    newThread->monitorEnterRecordPool = pool_new(
        sizeof(J9MonitorEnterRecord), 0, 0, 0, J9_GET_CALLSITE(), OMRMEM_CATEGORY_VM, POOL_FOR_PORT(PORTLIB));
    ...
    /* Initialize the thread for memory management purposes (vm->memoryManagerFunctions will be NULL if we failed to
     * load the gc dll) */
...
IDATA J9THREAD_PROC javaThreadProc(void* entryarg)
{
    ...
    j9sig_protect(javaProtectedThreadProc, vmThread, structuredSignalHandler, vmThread,
...
#if (defined(J9VM_DBG))
static void badness(char* description) { printf("\n<badness: %s>\n", description); }
#endif /* J9VM_DBG */
...
J9VMThread* currentVMThread(J9JavaVM* vm) { return getVMThreadFromOMRThread(vm, omrthread_self()); }
...
void threadCleanup(J9VMThread* vmThread, UDATA forkedByVM)
{
    ...
    /* Inform ThreadGroup about any uncaught exception.  Tiny VMs do not have ThreadGroup, so they just dump the
     * exception. */
    ...
    /* Increment zombie thread counter - indicates threads which have notified java of their death, but have not
     * deallocated their vmThread and exitted their thread proc */
     ...
     */
    if (vm->memoryManagerFunctions->j9gc_concurrent_scavenger_enabled(vm) && (NULL != vm->jitConfig)) {
    ...
    /* Deallocate the vmThread - if this thread was not forked by the VM, decrement the zombie counter now as the VM is
     * not in control of the native thread */
...
static void printCustomSpinOptions(void* element, void* userData)
{
    ...
    j9tty_printf(PORTLIB, ",\n" LEADING_SPACE_EXTRA "customTryEnterSpin1=%zu",
        j9monitorOptions->thrMaxTryEnterSpins1BeforeBlocking);
    j9tty_printf(PORTLIB, ",\n" LEADING_SPACE_EXTRA "customTryEnterSpin2=%zu",
        j9monitorOptions->thrMaxTryEnterSpins2BeforeBlocking);
    j9tty_printf(PORTLIB, ",\n" LEADING_SPACE_EXTRA "customTryEnterYield=%zu",
...
jint threadParseArguments(J9JavaVM* vm, char* optArg)
{
    ...
    **(UDATA**)omrthread_global("yieldUsleepMultiplier") = 1;
    ...
    /* If we aren't realtime, we can still be vanilla metronome (formerly known as SoftRT) and that means we still need
     * to set these values but they are different */
    vm->priorityPosixSignalDispatch = PRIORITY_INDICATOR_VALUE(J9THREAD_PRIORITY_MAX)
        + PRIORITY_INDICATOR_ADJUSTED_TYPE(PRIORITY_INDICATOR_J9THREAD_PRIORITY);
    vm->priorityPosixSignalDispatchNH = PRIORITY_INDICATOR_VALUE(J9THREAD_PRIORITY_MAX)
        + PRIORITY_INDICATOR_ADJUSTED_TYPE(PRIORITY_INDICATOR_J9THREAD_PRIORITY);
    vm->priorityAsyncEventDispatch = PRIORITY_INDICATOR_VALUE(J9THREAD_PRIORITY_MAX)
        + PRIORITY_INDICATOR_ADJUSTED_TYPE(PRIORITY_INDICATOR_J9THREAD_PRIORITY);
    vm->priorityAsyncEventDispatchNH = PRIORITY_INDICATOR_VALUE(J9THREAD_PRIORITY_MAX)
        + PRIORITY_INDICATOR_ADJUSTED_TYPE(PRIORITY_INDICATOR_J9THREAD_PRIORITY);
    vm->priorityTimerDispatch = PRIORITY_INDICATOR_VALUE(J9THREAD_PRIORITY_MAX)
        + PRIORITY_INDICATOR_ADJUSTED_TYPE(PRIORITY_INDICATOR_J9THREAD_PRIORITY);
    vm->priorityTimerDispatchNH = PRIORITY_INDICATOR_VALUE(J9THREAD_PRIORITY_MAX)
        + PRIORITY_INDICATOR_ADJUSTED_TYPE(PRIORITY_INDICATOR_J9THREAD_PRIORITY);
    vm->priorityMetronomeAlarm = PRIORITY_INDICATOR_VALUE(J9THREAD_PRIORITY_MAX)
        + PRIORITY_INDICATOR_ADJUSTED_TYPE(PRIORITY_INDICATOR_J9THREAD_PRIORITY);
    vm->priorityMetronomeTrace = PRIORITY_INDICATOR_VALUE(J9THREAD_PRIORITY_MIN)
        + PRIORITY_INDICATOR_ADJUSTED_TYPE(PRIORITY_INDICATOR_J9THREAD_PRIORITY);
    vm->priorityJitSample = PRIORITY_INDICATOR_VALUE(J9THREAD_PRIORITY_MAX)
        + PRIORITY_INDICATOR_ADJUSTED_TYPE(PRIORITY_INDICATOR_J9THREAD_PRIORITY);
    vm->priorityJitCompile = PRIORITY_INDICATOR_VALUE(J9THREAD_PRIORITY_MAX)
        + PRIORITY_INDICATOR_ADJUSTED_TYPE(PRIORITY_INDICATOR_J9THREAD_PRIORITY);
    ...
    **(UDATA**)omrthread_global("maxWakeThreads") = OMRTHREAD_MINIMUM_WAKE_THREADS;
    ...
    while (scan_start < scan_limit) {
        ...
        if (try_scan(&scan_start, "customSpinOptions=")) {
            vm->customSpinOptions = pool_new(sizeof(J9VMCustomSpinOptions), 0, 0, 0, J9_GET_CALLSITE(),
                OMRMEM_CATEGORY_VM, POOL_FOR_PORT(vm->portLibrary));
        ...
        if (try_scan(&scan_start, "tw=")) {
            ...
            if (tw && !strcmp(tw, "HEAVY")) {
                ...
                && (!strcmp(tw, "MEDIUM") || !strcmp(tw, "NATIVE") || !strcmp(tw, "native") || !strcmp(tw, ""))) {
                tw = "medium";
            }
            *gtw = (UDATA)tw;
            if (tw && !strcmp(tw, "heavy")) {
...
static void dumpThreadingInfo(J9JavaVM* jvm)
{
    ...
    j9tty_printf(PORTLIB, LEADING_SPACE "notifyPolicy=%s,\n",
        J9_ARE_ALL_BITS_SET(omrthread_lib_get_flags(), J9THREAD_LIB_FLAG_NOTIFY_POLICY_BROADCAST) ? "broadcast"
                                                                                                  : "signal");
#endif /* !defined(WIN32) && defined(OMR_NOTIFY_POLICY_CONTROL) */
    j9tty_printf(PORTLIB, LEADING_SPACE "deflationPolicy=%s",
        (jvm->thrDeflationPolicy == J9VM_DEFLATION_POLICY_ASAP)
            ? "asap"
            : (jvm->thrDeflationPolicy == J9VM_DEFLATION_POLICY_NEVER) ? "never" : "smart");
#if defined(OMR_THR_THREE_TIER_LOCKING)
    j9tty_printf(PORTLIB, ",\n");
    j9tty_printf(
        PORTLIB, LEADING_SPACE "threeTierSpinCount1=%zu,\n", **(UDATA**)omrthread_global("defaultMonitorSpinCount1"));
    j9tty_printf(
        PORTLIB, LEADING_SPACE "threeTierSpinCount2=%zu,\n", **(UDATA**)omrthread_global("defaultMonitorSpinCount2"));
    j9tty_printf(
        PORTLIB, LEADING_SPACE "threeTierSpinCount3=%zu,\n", **(UDATA**)omrthread_global("defaultMonitorSpinCount3"));
    j9tty_printf(PORTLIB, LEADING_SPACE "%sastNotify",
        J9_ARE_ALL_BITS_SET(omrthread_lib_get_flags(), J9THREAD_LIB_FLAG_FAST_NOTIFY) ? "f" : "noF");
#if defined(OMR_THR_SPIN_WAKE_CONTROL)
    ...
    j9tty_printf(PORTLIB, ",\n" LEADING_SPACE "%secondarySpinForObjectMonitors",
        J9_ARE_ALL_BITS_SET(omrthread_lib_get_flags(), J9THREAD_LIB_FLAG_SECONDARY_SPIN_OBJECT_MONITORS_ENABLED)
            ? "s"
            : "noS");
#endif /* defined(OMR_THR_THREE_TIER_LOCKING) */

#ifdef OMR_THR_ADAPTIVE_SPIN
    if ((0 != *(UDATA*)omrthread_global("adaptSpinHoldtimeEnable"))
        || (0 != *(UDATA*)omrthread_global("adaptSpinSlowPercentEnable"))) {
        j9tty_printf(PORTLIB, ",\n" LEADING_SPACE "adaptSpin");
        j9tty_printf(
            PORTLIB, ",\n" LEADING_SPACE "adaptSpinHoldtime=%zu", **(UDATA**)omrthread_global("adaptSpinHoldtime"));
        j9tty_printf(PORTLIB, ",\n" LEADING_SPACE "adaptSpinSlowPercent=%zu",
            **(UDATA**)omrthread_global("adaptSpinSlowPercent"));
        j9tty_printf(PORTLIB, ",\n" LEADING_SPACE "adaptSpinSampleThreshold=%zu",
            **(UDATA**)omrthread_global("adaptSpinSampleThreshold"));
        j9tty_printf(PORTLIB, ",\n" LEADING_SPACE "adaptSpinSampleStopCount=%zu",
            **(UDATA**)omrthread_global("adaptSpinSampleStopCount"));
        j9tty_printf(PORTLIB, ",\n" LEADING_SPACE "adaptSpinSampleCountStopRatio=%zu",
            **(UDATA**)omrthread_global("adaptSpinSampleCountStopRatio"));
...
static UDATA printMethodInfo(J9VMThread* currentThread, J9StackWalkState* state)
{
    ...
    cursor += j9str_printf(PORTLIB, cursor, end - cursor, "\tat %.*s.%.*s%.*s", J9UTF8_LENGTH(className),
        J9UTF8_DATA(className), J9UTF8_LENGTH(methodName), J9UTF8_DATA(methodName), J9UTF8_LENGTH(sig),
        J9UTF8_DATA(sig));
    ...
    if (romMethod->modifiers & J9AccNative) {
        ...
        if (sourceFile) {
            ...
            cursor += j9str_printf(
                PORTLIB, cursor, end - cursor, " (%.*s", J9UTF8_LENGTH(sourceFile), J9UTF8_DATA(sourceFile));
            if (lineNumber != -1) {
...
void printThreadInfo(J9JavaVM* vm, J9VMThread* self, char* toFile, BOOLEAN allThreads)
{
    ...
    if (toFile != NULL) {
        ...
        if ((tracefd = j9file_open(fileName, EsOpenWrite | EsOpenCreate | EsOpenTruncate, 0666)) == -1) {
            j9tty_err_printf(
                PORTLIB, "Error: Failed to open dump file %s.\nWill output to stderr instead:\n", fileName);
        }
    } else if (vm->sigquitToFileDir != NULL) {
        j9str_printf(PORTLIB, fileName, EsMaxPath, "%s%s%s%d%s", vm->sigquitToFileDir, DIR_SEPARATOR_STR,
            SIGQUIT_FILE_NAME, j9time_usec_clock(), SIGQUIT_FILE_EXT);
        if ((tracefd = j9file_open(fileName, EsOpenWrite | EsOpenCreate | EsOpenTruncate, 0666)) == -1) {
            j9tty_err_printf(
                PORTLIB, "Error: Failed to open trace file %s.\nWill output to stderr instead:\n", fileName);
        }
    }
    ...
    if (currentThread) {
        do {
            if (currentThread->threadObject) {
                ...
                trace_printf(PORTLIB, tracefd, "Thread=%s (%p) Status=%s\n", threadName, currentThread->osThread,
                    getJ9ThreadStatus(currentThread));
                ...
                walkState.flags = J9_STACKWALK_ITERATE_FRAMES | J9_STACKWALK_VISIBLE_ONLY | J9_STACKWALK_INCLUDE_NATIVES
...
#if (defined(J9VM_RAS_DUMP_AGENTS)) || (defined(J9VM_INTERP_SIG_QUIT_THREAD))
static void printJ9ThreadStatusMonitorInfo(J9VMThread* vmThread, IDATA tracefd)
{
    ...
    if ((((J9ThreadAbstractMonitor*)blockingMonitor)->flags & J9THREAD_MONITOR_OBJECT) == J9THREAD_MONITOR_OBJECT) {
        ...
        trace_printf(PORTLIB, tracefd, "Monitor=%p (%s monitor for %.*s @ %p) ", blockingMonitor, objType,
            J9UTF8_LENGTH(className), J9UTF8_DATA(className), object);
    } else {
...
j9object_t createCachedOutOfMemoryError(J9VMThread* currentThread, j9object_t threadObject)
{
    ...
    jlOutOfMemoryError = internalFindKnownClass(
        currentThread, J9VMCONSTANTPOOL_JAVALANGOUTOFMEMORYERROR, J9_FINDKNOWNCLASS_FLAG_INITIALIZE);
    threadObject = POP_OBJECT_IN_SPECIAL_FRAME(currentThread);

    outOfMemoryError
        = gcFuncs->J9AllocateObject(currentThread, jlOutOfMemoryError, J9_GC_ALLOCATE_OBJECT_NON_INSTRUMENTABLE);
    ...
    UDATA priority, omrthread_entrypoint_t entryPoint, void* entryArg, j9object_t schedulingParameters)
{
    ...
    if (rc != J9_THREAD_START_NO_ERROR) {
        ...
        gcFuncs->j9gc_modron_global_collect_with_overrides(
            currentThread, J9MMCONSTANT_EXPLICIT_GC_NATIVE_OUT_OF_MEMORY);
        rc = startJavaThreadInternal(
            currentThread, privateFlags, osStackSize, priority, entryPoint, entryArg, setException);
    }
    ...
    omrthread_entrypoint_t entryPoint, void* entryArg, UDATA setException)
{
    ...
    retVal = vm->internalVMFunctions->createThreadWithCategory(&osThread, osStackSize,
        vm->java2J9ThreadPriorityMap[priority], TRUE, entryPoint, entryArg, J9THREAD_CATEGORY_APPLICATION_THREAD);
    if (retVal != J9THREAD_SUCCESS) {
        if (retVal & J9THREAD_ERR_OS_ERRNO_SET) {
            ...
            Trc_VM_startJavaThread_failedToCreateOSThreadWithErrno2(
                currentThread, -retVal, omrthread_get_errordesc(-retVal), os_errno, os_errno, os_errno2, os_errno2);
            if (setException) {
...
#else /* !J9ZOS390 */
            omrthread_os_errno_t os_errno = omrthread_get_os_errno();
            Trc_VM_startJavaThread_failedToCreateOSThreadWithErrno(
                currentThread, -retVal, omrthread_get_errordesc(-retVal), os_errno, os_errno);
            if (setException) {
            ...
            Trc_VM_startJavaThread_failedToCreateOSThread(currentThread, -retVal, omrthread_get_errordesc(-retVal));
        }
    ...
    if (J2SE_SHAPE(vm) == J2SE_SHAPE_RAW) {
        ...
        if (NULL != unicodeChars) {
            threadName = copyStringToUTF8WithMemAlloc(
                currentThread, unicodeChars, J9_STR_NULL_TERMINATE_RESULT, "", 0, NULL, 0, NULL);
        }
    } else {
 ...
 * @retval 0 success
 * @retval -1 failure This function may fail if the NLS message can't be found, or if there is insufficient memory to
    ...
    J9VMThread* currentThread, IDATA retVal, omrthread_os_errno_t os_errno, omrthread_os_errno_t os_errno2)
{
    ...
    errorMessage = j9nls_lookup_message(
        J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE, J9NLS_VM_THREAD_CREATE_FAILED_WITH_ERRNO2, NULL);
    ...
    if (errorMessage) {
        ...
        if (bufLen > 0) {
            ...
            if (buf) {
                ...
                if ((bufLen - 1)
                    == j9str_printf(
                           PORTLIB, buf, bufLen, errorMessage, retVal, os_errno, os_errno, os_errno2, os_errno2)) {
                    setCurrentExceptionUTF(
                        currentThread, J9_EX_OOM_THREAD | J9VMCONSTANTPOOL_JAVALANGOUTOFMEMORYERROR, buf);
                    rc = 0;
                }
...
static IDATA setFailedToForkThreadException(J9VMThread* currentThread, IDATA retVal, omrthread_os_errno_t os_errno)
{
    ...
    errorMessage = j9nls_lookup_message(
        J9NLS_DO_NOT_PRINT_MESSAGE_TAG | J9NLS_DO_NOT_APPEND_NEWLINE, J9NLS_VM_THREAD_CREATE_FAILED_WITH_ERRNO, NULL);
...
static UDATA tmapHash(void* key, void* userData) { return (UDATA)((ThreadMap*)key)->thread; }

static UDATA tmapHashEqual(void* leftKey, void* rightKey, void* userData)
{
    ...
    J9VMThread* currentThread, j9object_t** pDeadlockedThreads, j9object_t** pBlockingObjects, UDATA flags)
{
     ...
     */
    hashTable = hashTableNew(OMRPORT_FROM_J9PORT(PORTLIB), J9_GET_CALLSITE(), 0, sizeof(ThreadMap), 0, 0,
        OMRMEM_CATEGORY_THREADS, tmapHash, tmapHashEqual, NULL, NULL);
    if (NULL == hashTable) {
