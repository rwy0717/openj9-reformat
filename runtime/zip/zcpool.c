...
J9ZipCachePool* zipCachePool_new(J9PortLibrary* portLib, void* userData)
{
    ...
    if (p != NULL) {
        ...
        if (MUTEX_INIT(p->mutex)) {
            p->pool = pool_new(
                sizeof(J9ZipCachePoolEntry), 0, 0, 0, J9_GET_CALLSITE(), J9MEM_CATEGORY_VM_JCL, POOL_FOR_PORT(portLib));
            if (p->pool) {
...
void zipCachePool_doFindHandler(J9ZipCachePoolEntry* entry, J9ZipCachePool* zcp)
{
    ...
    if (!zipCache_isSameZipFile(
            entry->cache, (IDATA)zcp->zipTimeStamp, zcp->zipFileSize, zcp->zipFileName, zcp->zipFileNameLength)) {
        return;
    }
...
void zipCachePool_doKillHandler(J9ZipCachePoolEntry* entry, J9ZipCachePool* zcp) { zipCache_kill(entry->cache); }
