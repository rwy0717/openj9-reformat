...
J9ZipDirEntry* zipCache_searchDirListCaseInsensitive(
    J9ZipDirEntry* dirEntry, const char* namePtr, UDATA nameSize, BOOLEAN isClass);
J9ZipChunkHeader* zipCache_allocateChunk(J9PortLibrary* portLib);
J9ZipFileEntry* zipCache_addToFileList(J9PortLibrary* portLib, J9ZipCacheEntry* zce, J9ZipDirEntry* dirEntry,
    const char* namePtr, IDATA nameSize, BOOLEAN isClass, UDATA elementOffset);
static UDATA* zipCache_reserveEntry(
    J9ZipCacheEntry* zce, J9ZipChunkHeader* chunk, UDATA entryBytes, UDATA stringBytes, char** namePtrOut);
J9ZipFileEntry* zipCache_searchFileList(J9ZipDirEntry* dirEntry, const char* namePtr, UDATA nameSize, BOOLEAN isClass);
J9ZipDirEntry* zipCache_addToDirList(J9PortLibrary* portLib, J9ZipCacheEntry* zce, J9ZipDirEntry* dirEntry,
...
J9ZipDirEntry* zipCache_copyDirEntry(
    J9ZipCacheEntry* orgzce, J9ZipDirEntry* orgDirEntry, J9ZipCacheEntry* zce, J9ZipDirEntry* rootEntry);
...
#define J9ZIPFILEENTRY_NEXT(entry) \
    ((J9ZipFileEntry*)((UDATA)entry + sizeof(J9ZipFileEntry) + ALIGN_ENTRY(entry->nameLength)))
...
J9ZipCache* zipCache_new(
    J9PortLibrary* portLib, char* zipName, IDATA zipNameLength, IDATA zipFileSize, I_64 zipTimeStamp)
{
...
const char* zipCache_uniqueId(J9ZipCache* zipCache)
{
    ...
    sizeRequired = j9str_printf(
        PORTLIB, NULL, 0, "%s_%d_%lld_%d", fileName, zce->zipFileSize, zce->zipTimeStamp, ZIP_CACHE_VERSION);
    buf = j9mem_allocate_memory(sizeRequired, J9MEM_CATEGORY_VM_JCL);
    if (!buf) {
        return NULL;
    }
    j9str_printf(
        PORTLIB, buf, sizeRequired, "%s_%d_%lld_%d", fileName, zce->zipFileSize, zce->zipTimeStamp, ZIP_CACHE_VERSION);
...
BOOLEAN zipCache_copy(J9ZipCache* zipCache, void* cacheData, UDATA dataSize)
{
    ...
    if (NULL != record) {
        while (record) {
            J9ZipFileEntry* fileEntry = record->entry;
            for (i = 0; i < record->entryCount; i++) {
                if (!zipCache_addToFileList(portLib, zce, &zce->root, J9ZIPFILEENTRY_NAME(fileEntry),
                        fileEntry->nameLength, (fileEntry->zipFileOffset & ISCLASS_BIT) != 0,
                        fileEntry->zipFileOffset & OFFSET_MASK)) {
                    return FALSE;
                }
                fileEntry = J9ZIPFILEENTRY_NEXT(fileEntry);
            }
    ...
    if (orgDirEntry->dirList
        && !zipCache_copyDirEntry(orgzce, ZIP_SRP_GET(orgDirEntry->dirList, J9ZipDirEntry*), zce, &zce->root)) {
        return FALSE;
    }
 ...
 */
BOOLEAN zipCache_isSameZipFile(
    J9ZipCache* zipCache, IDATA zipTimeStamp, IDATA zipFileSize, const char* zipFileName, IDATA zipFileNameLength)
{
    J9ZipCacheInternal* zci = (J9ZipCacheInternal*)zipCache;
    J9ZipCacheEntry* zce = zci->entry;
    ...
    J9ZipCacheEntry* orgzce, J9ZipDirEntry* orgDirEntry, J9ZipCacheEntry* zce, J9ZipDirEntry* rootEntry)
{
    ...
    while (orgDirEntry) {
        ...
        if (orgDirEntry->dirList
            && !zipCache_copyDirEntry(orgzce, ZIP_SRP_GET(orgDirEntry->dirList, J9ZipDirEntry*), zce, dirEntry)) {
            return NULL;
        }
...
zipCache_addElement(J9ZipCache* zipCache, char* elementName, IDATA elementNameLength, UDATA elementOffset)
{
    ...
    if (!zipCache || (elementNameLength == 0) || ((elementName[0] == 0) && (elementNameLength == 1))
        || ((elementOffset & ISCLASS_BIT) != 0) || ((elementOffset & OFFSET_MASK) == IMPLICIT_ENTRY))
        return FALSE;
 ...
 *
 * @param[in] handle returned from @ref zipCache_enumNew. Used to enumerate the elements corresponding to the directory
name returned by @ref zipCache_enumGetDirName
 ...
 * @return the required size of nameBuf if nameBufSize is insufficient to hold the entire name (does not skip the
element)
 *
...
IDATA helper_memicmp(const void* src1, const void* src2, UDATA length)
{
    ...
    UDATA i;
    for (i = 0; i < length; i++) {
        if (j9_ascii_toupper(s1[i]) > j9_ascii_toupper(s2[i]))
            return 1;
        if (j9_ascii_toupper(s1[i]) < j9_ascii_toupper(s2[i]))
            return -1;
    }
