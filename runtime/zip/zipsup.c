...
J9ZipFunctionTable zipFunctions = { zip_freeZipComment, zip_freeZipEntry, zip_getNextZipEntry, zip_getZipComment,
    zip_getZipEntry, zip_getZipEntryComment, zip_getZipEntryData, zip_getZipEntryExtraField, zip_getZipEntryFromOffset,
    zip_getZipEntryRawData, zip_initZipEntry, zip_openZipFile, zip_releaseZipFile, zip_resetZipFile };

#define ZIP_NEXT_U8(value, index) (value = *(index++))
#define ZIP_NEXT_U16(value, index) ((value = (index[1] << 8) | index[0]), index += 2, value)
#define ZIP_NEXT_U32(value, index)                                                                                   \
    ((value = ((U_32)index[3] << 24) | ((U_32)index[2] << 16) | ((U_32)index[1] << 8) | (U_32)index[0]), index += 4, \
        value)
...
static I_32 zip_populateCache(
    J9PortLibrary* portLib, J9ZipFile* zipFile, J9ZipCentralEnd* endEntry, IDATA startCentralDir);
static I_32 inflateData(
    struct workBuffer* workBuf, U_8* inputBuffer, U_32 inputBufferSize, U_8* outputBuffer, U_32 outputBufferSize);
...
static I_32 readZipEntry(J9PortLibrary* portLib, J9ZipFile* zipFile, J9ZipEntry* zipEntry, const char* filename,
    IDATA filenameLength, IDATA* enumerationPointer, IDATA* entryStart, BOOLEAN findDirectory, BOOLEAN readDataPointer);
...
I_32 initZipLibrary(J9PortLibrary* portLib, char* dir)
{
    ...
    if (dir != NULL) {
        ...
        if (j9sl_open_shared_library(correctPathPtr, &zipDLLDescriptor, TRUE))
            goto openFailed;
    } else {
        /* dir is NULL. It shouldn't happen, but in case, revert back to original dlload that
         * replies on LIBPATH
         */
        if (j9sl_open_shared_library(J9_ZIP_DLL_NAME, &zipDLLDescriptor, TRUE))
            goto openFailed;
    }

    /* look up the functions */
    if (j9sl_lookup_name(zipDLLDescriptor, "j9zlib_inflateInit2_", (void*)&inflateInit2Func, "ILILI"))
        goto loadFailed;
    if (j9sl_lookup_name(zipDLLDescriptor, "j9zlib_inflate", (void*)&inflateFunc, "IPI"))
        goto loadFailed;
    if (j9sl_lookup_name(zipDLLDescriptor, "j9zlib_inflateEnd", (void*)&inflateEndFunc, "IP"))
        goto loadFailed;
...
openFailed:
    ...
    /* Unable to open %s (%s) */
    j9nls_printf(
        PORTLIB, J9NLS_WARNING, J9NLS_ZIP_UNABLE_TO_OPEN_ZIP_DLL, J9_ZIP_DLL_NAME, j9error_last_error_message());
 ...
 * @return true if seekResult is negative or larger than the maximum U_32 positive value or does not match the expected
 * position
 */
static VMINLINE BOOLEAN isSeekFailure(I_64 seekResult, I_64 expectedValue)
{
 ...
 */
static VMINLINE BOOLEAN isOutside4Gig(I_64 value) { return (value < 0) || (value > UINT32_MAX); }
        ...
        for (i = size; i--; dataSize++, commentOffsetFromEnd++) {
            ...
            case 3:
                ...
                if (buffer[i] == 'P' && dataSize >= ZIPFILE_COMMENT_OFFSET) {
                     ...
                     */
                    if (*commentLength
                        != (commentOffsetFromEnd - ZIPFILE_COMMENT_OFFSET - loopCount * MIN_ZIPFILE_SIZE)) {
...
/*
        Scan backward from end of file for a central end header. Read from zipFile and update the J9ZipCentralEnd
   provided.
        ...
        for (i = size; i--; dataSize++) {
            ...
            case 3:
                ...
                if (buffer[i] == 'P' && dataSize >= ZIPFILE_COMMENT_OFFSET) {
                    ...
                    if (dataSize >= (U_32)(ZIPFILE_COMMENT_OFFSET + endEntry->commentLength))
                        return 0;
    ...
    while (unreadSize) {
        ...
        while (current + 46 < buffer + bufferedSize) {
            ...
            if (((entry.compressionMethod == ZIP_CM_Deflated) && (entry.flags & 0x8))
                ...
                result = (I_32)zipCache_addElement(
                    zipFile->cache, (char*)filename, (IDATA)entry.filenameLength, entryPointer);

            } else {
                result = (I_32)zipCache_addElement(
                    zipFile->cache, (char*)filename, (IDATA)entry.filenameLength, localHeaderOffset);
            }
...
/*
        Read the next zip entry for the zipFile into the zipEntry provided.  If filename is non-NULL, it is expected to
   match the filename read for the entry.  If (cachePointer != -1) the filename of the entry will be looked up in the
   cache (assuming there is one) to help detect use of an invalid cache.  If enumerationPointer is non-NULL, sequential
   access is assumed and either a local zip entry or a data descriptor will be accepted, but a central zip entry will
   cause ZIP_ERR_NO_MORE_ENTRIES to be returned.  If enumerationPointer is NULL, random access is assumed and either a
   local zip entry or a central zip entry will be accepted.
...
retry:
    ...
    if ((NULL != zipFile->cache) && ((IDATA)zipFile->pointer >= zipCache_getStartCentralDir(zipFile->cache))) {
    ...
    if ((readResult < 22)
        || (filename && !(readResult == readLength || (findDirectory && readResult == (readLength - 1))))) {
    ...
    if (filename) {
        if (zipFile->cache) {
            if (!(readLength == zipEntry->filenameLength
                    || (findDirectory && (readLength - 1) == zipEntry->filenameLength))) {
    ...
    if (readLength < zipEntry->filenameLength) {
        readResult
            = j9file_read(zipFile->fd, zipEntry->filename + readLength, (IDATA)(zipEntry->filenameLength - readLength));
    ...
    if (filename) {
        if (!((findDirectory && zipEntry->filenameLength == (filenameLength + 1)
    ...
    if (sig == ZIP_CentralHeader) {
        ...
        zipEntry->fileCommentPointer
            = (U_32)(currentEntryPointer + headerSize + zipEntry->filenameLength + zipEntry->extraFieldLength);
    ...
    if ((sig == ZIP_LocalHeader) && (zipEntry->compressionMethod == ZIP_CM_Deflated) && (zipEntry->flags & 0x8)) {
 ...
 * Otherwise, file descriptor is closed only when there is no more reference to this zip file. See zipCache_kill in
 * zipcache.c.
 *
 * @param[in] portLib the port library
...
finished:
    if (cachePool) {
        TRIGGER_J9HOOK_VM_ZIP_LOAD(cachePool->hookInterface, portLib, cachePool->userData,
            (const struct J9ZipFile*)zipFile, J9ZIP_STATE_CLOSED, zipFile->filename, result);
    }
...
I_32 zip_searchCache(J9PortLibrary* portLib, char* filename, J9ZipCachePool* cachePool, J9ZipCache** cache)
{
 ...
 * @return	ZIP_ERR_INTERNAL_ERROR if there was an internal error
 */
I_32 zip_setupCache(J9PortLibrary* portLib, J9ZipFile* zipFile, J9ZipCache* cache, J9ZipCachePool* cachePool)
{
    PORT_ACCESS_FROM_PORT(portLib);
    I_32 result = 0;
    I_64 timeStamp, actualFileSize;
    ...
    if (zipFile->cache) {
        if (zipFile->cachePool) {
            /* Invalidate the cache to keep other people from starting to use it (we will create a new one for them
                    to start to use instead).  Once all the current users of the cache have stopped using it, it will go
               away */
    ...
    if (NULL != cache) {
        ...
        TRIGGER_J9HOOK_VM_ZIP_LOAD(cachePool->hookInterface, portLib, cachePool->userData,
            (const struct J9ZipFile*)zipFile, J9ZIP_STATE_OPEN, zipFile->filename, 0);
    } else {
...
finished:
    if (result) {
        /* Only report failure, success is reported by zip_readCacheData(). Call the hook before freeing the
         * zipFile->filename. */
        if (NULL != cachePool) {
            TRIGGER_J9HOOK_VM_ZIP_LOAD(cachePool->hookInterface, portLib, cachePool->userData,
                (const struct J9ZipFile*)zipFile, J9ZIP_STATE_OPEN, zipFile->filename, result);
        }
        if (zipFile->cache) {
 ...
 * @return	ZIP_ERR_INTERNAL_ERROR if there was an internal error
 */
I_32 zip_readCacheData(J9PortLibrary* portLib, J9ZipFile* zipFile)
{
    I_32 result = 0;
...
void zip_initZipEntry(J9PortLibrary* portLib, J9ZipEntry* entry) { memset(entry, 0, sizeof(*entry)); }
...
I_32 zip_getNextZipEntry(
    J9PortLibrary* portLib, J9ZipFile* zipFile, J9ZipEntry* zipEntry, IDATA* nextEntryPointer, BOOLEAN readDataPointer)
{
    PORT_ACCESS_FROM_PORT(portLib);
    I_32 result = 0;
 ...
 * J9ZIP_GETENTRY_USE_CENTRAL_DIRECTORY: if there is no cache and this bit is set, look in the central directory ,
 *otherwise do a linear scan of the local headers.
 *
 * @return 0 on success or one of the following:
 * @return	ZIP_ERR_FILE_READ_ERROR if there is an error reading from zipFile
 * @return	ZIP_ERR_FILE_CORRUPT if zipFile is corrupt
...
I_32 zip_getZipEntry(J9PortLibrary* portLib, J9ZipFile* zipFile, J9ZipEntry* entry, const char* filename,
    IDATA fileNameLength, U_32 flags)
{
    PORT_ACCESS_FROM_PORT(portLib);
    I_32 status = 0;
    I_32 result = 0;
...
retry:
    if (zipFile->cache) {
        ...
        /* Read the entry */
        result = readZipEntry(
            portLib, zipFile, entry, filename, fileNameLength, NULL, NULL, findDirectory, readDataPointer);
        if (0 != result) {
            if (!retryAllowed) {
                status = result;
                goto finished;
    ...
    const char* fileName, IDATA fileNameLength, BOOLEAN readDataPointer)
{
    ...
    nextEntryPointer
        = endEntry.endCentralDirRecordPosition - endEntry.dirSize; /* assume no ZIP64 records or digital signature */
            ...
            if (entryNameLength == fileNameLength) {
                ...
                if (0 == strncmp(fileName, (const char*)readBuffer, fileNameLength)) {
                    ...
                    if (isSeekFailure(seekResult, zipFile->pointer)) {
                        ...
                        result = readZipEntry(
                            portLib, zipFile, entry, fileName, fileNameLength, NULL, NULL, FALSE, readDataPointer);
                    }
                    break;
                }
            }
...
I_32 zip_getZipEntryData(J9PortLibrary* portLib, J9ZipFile* zipFile, J9ZipEntry* entry, U_8* buffer, U_32 bufferSize)
{
    ...
    if (buffer) {
        ...
        /* Note that this is the first zalloc. This memory must be available to the calling method and is freed
         * explicitly in zip_freeZipEntry. */
...
I_32 zip_getZipEntryRawData(
    J9PortLibrary* portLib, J9ZipFile* zipFile, J9ZipEntry* entry, U_8* buffer, U_32 bufferSize, U_32 offset)
{
    PORT_ACCESS_FROM_PORT(portLib);
...
I_32 zip_getZipEntryExtraField(
    J9PortLibrary* portLib, J9ZipFile* zipFile, J9ZipEntry* entry, U_8* buffer, U_32 bufferSize)
{
    PORT_ACCESS_FROM_PORT(portLib);
 ...
 */
void zip_shutdownZipCachePoolHookInterface(J9ZipCachePool* cachePool)
{
    J9HookInterface** mhookInterface = J9_HOOK_INTERFACE(cachePool->hookInterface);
    if (*mhookInterface) {
...
finished:
    if (cachePool) {
        TRIGGER_J9HOOK_VM_ZIP_LOAD(cachePool->hookInterface, portLib, cachePool->userData,
            (const struct J9ZipFile*)zipFile, J9ZIP_STATE_OPEN, (U_8*)filename, result);
    }
...
void zip_resetZipFile(J9PortLibrary* portLib, J9ZipFile* zipFile, IDATA* nextEntryPointer)
{
    *nextEntryPointer = 0;
    if (zipFile) {
        ...
        if (cachePool) {
            TRIGGER_J9HOOK_VM_ZIP_LOAD(cachePool->hookInterface, portLib, cachePool->userData, zipFile,
                J9ZIP_STATE_RESET, zipFile->filename, result);
        }
    }
}
...
I_32 zip_getZipEntryFromOffset(
    J9PortLibrary* portLib, J9ZipFile* zipFile, J9ZipEntry* entry, IDATA offset, BOOLEAN readDataPointer)
{
    PORT_ACCESS_FROM_PORT(portLib);
    I_32 result;
    I_64 seekResult;
